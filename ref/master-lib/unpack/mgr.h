...
#define cline(x1, y1, x2, y2, color)                                           \
  grcg_setcolor(GC_RMW, color), grcg_line(x1, y1, x2, y2), grcg_off()
#define gbox(x1, y1, x2, y2, color)                                            \
  grcg_setcolor(GC_RMW, color), grcg_boxfill(x1, y1, x2, y2), grcg_off()
...
#define circlex(x, y, r, color)                                                \
  grcg_setcolor(GC_RMW, color), grcg_circle_x(x, y, r), grcg_off()
...
#define grPalTone(pal, tone)                                                   \
  palette_set_all_16(pal), palette_settone((tone) * 100 / 128),                \
      (PaletteTone = 100)
#define grPal1(color, r, g, b)                                                 \
  palette_set(color, (r) * 17, (g) * 17, (b) * 17), palette_show()
...
#define grByteBox(x1, y1, x2, y2, color)                                       \
  grcg_setcolor(GC_RMW, color), grcg_byteboxfill_x(x1, y1, x2, y2), grcg_off()
#define grCircle(x, y, r, color)                                               \
  grcg_setcolor(GC_RMW, color), grcg_circle(x, y, r), grcg_off()
#define grCircleFill(x, y, r, color)                                           \
  grcg_setcolor(GC_RMW, color), grcg_circlefill(x, y, r), grcg_off()
#define grCirclexFill(x, y, r, color) grCircleFill(x, y, r, color)
#define grRectangle(x1, y1, x2, y2, color)                                     \
  grcg_setcolor(GC_RMW, color), grcg_hline(x1, x2, y1),                        \
      grcg_hline(x1, x2, y2), grcg_vline(x1, y1, y2), grcg_vline(x2, y1, y2),  \
      grcg_off()
#define grHLine(x1, x2, y, color)                                              \
  grcg_setcolor(GC_RMW, color), grcg_hline(x1, x2, y), grcg_off()
#define grVLine(x, y1, y2, color)                                              \
  grcg_setcolor(GC_RMW, color), grcg_vline(x, y1, y2), grcg_off()
#define grPSet(x, y, color)                                                    \
  grcg_setcolor(GC_RMW, color), grcg_pset(x, y), grcg_off()
...
#define grScrollXY(x, y)                                                       \
  graph_scroll(graph_VramLines - (y), (x) + (y) * 40, (x))
#define grSetResPal(makeNew)                                                   \
  (makenew ? respal_create() : respal_exist()), respal_set_palettes()
...
#define grcgPlane(activeplane)                                                 \
  grcg_setcolor(GC_RMW | (~(activeplane) & 0x0f), 15)
#define grcgPlaneRev(activeplane)                                              \
  grcg_setcolor(GC_RMW | (~(activeplane) & 0x0f), 0)
#define grcgPlaneColor(activeplane, color)                                     \
  grcg_setcolor(GC_RMW | (~(activeplane) & 0x0f), (color))
#define grcgOff() grcg_off()
#define grcgPlaneTDW(activeplane)                                              \
  grcg_setcolor(GC_TDW | (~(activeplane) & 0x0f), 15)
#define grcgPlaneRevTDW(activeplane)                                           \
  grcg_setcolor(GC_TDW | (~(activeplane) & 0x0f), 0)

/*
 *	外字 BFNT 表示
 */
#define gfLoad(fontfile, no)                                                   \
  gaiji_backup(), (gaiji_entry_bfnt(fontfile) ? 0 : DATA_ER)
...
#define mfDispSdw(x, y, col, col2, str)                                        \
  graph_gaiji_puts((x) * 8 + 1, (y) + 1, 16, str, col2), mfDisp(x, y, col, str)
#define mfDispChrEdg(x, y, col, col2, c) mfDispChr(x, y, col, c)
#define mfDispChrSdw(x, y, col, col2, c)                                       \
  graph_gaiji_putc((x) * 8 + 1, (y) + 1, c, col2), mfDispChr(x, y, col, c)
#if 0
...
#define mouseSetCsrMode(mode)                                                  \
  ((mode) ? (void)cursor_show() : (cursor_hide(), cursor_init()))
...
#define emsReallocatePages(handle, pageNum)                                    \
  ems_reallocate(handle, (pageNum) * 16384L)
#define emsMapHandlePage(frame, handle, page)                                  \
  ems_maphandlepage(frame, handle, page)
...
#define strGetEnv(env, buf)                                                    \
  (buf ? strcpy(buf, dos_getenv(0, env)) : dos_getenv(0, env))
