...
#if !defined(MASTER_NEAR) && !defined(MASTER_FAR) &&                           \
    !defined(MASTER_COMPACT) && !defined(MASTER_MEDIUM)
#if defined(__SMALL__) || defined(__TINY__) || defined(M_I86SM) ||             \
    defined(M_I86TM)
#define MASTER_NEAR
...
#elif defined(__LARGE__) || defined(__HUGE__) || defined(M_I86LM) ||           \
    defined(M_I86HM)
#define MASTER_FAR
#endif
#endif
...
#define TX_GETSIZE(x1, y1, x2, y2)                                             \
  (((x2) - (x1) + 1) * ((y2) - (y1) + 1) * TX_GET_UNIT)
...
void MASTER_RET text_boxfilla(unsigned x1, unsigned y1, unsigned x2,
                              unsigned y2, unsigned atrb);
void MASTER_RET text_boxfillca(unsigned x1, unsigned y1, unsigned x2,
                               unsigned y2, unsigned fillchar, unsigned atrb);
...
void MASTER_RET text_putns(unsigned x, unsigned y, const char MASTER_PTR *str,
                           unsigned wid);
void MASTER_RET text_putnp(unsigned x, unsigned y, const char MASTER_PTR *pstr,
                           unsigned wid);
void MASTER_RET text_putnsa(unsigned x, unsigned y, const char MASTER_PTR *str,
                            unsigned wid, unsigned atrb);
void MASTER_RET text_putnpa(unsigned x, unsigned y, const char MASTER_PTR *pstr,
                            unsigned wid, unsigned atrb);
...
void MASTER_RET text_putsa(unsigned x, unsigned y, const char MASTER_PTR *str,
                           unsigned atrb);
void MASTER_RET text_putpa(unsigned x, unsigned y, const char MASTER_PTR *pstr,
                           unsigned atrb);
void MASTER_RET text_vputs(unsigned x, unsigned y, const char MASTER_PTR *str);
void MASTER_RET text_vputsa(unsigned x, unsigned y, const char MASTER_PTR *str,
                            unsigned atrb);
...
void MASTER_RET text_worddota(int x, int y, unsigned image, unsigned dotlen,
                              unsigned atr);
void MASTER_RET text_showpage(int page);
void MASTER_RET text_frame(int x1, int y1, int x2, int y2, unsigned wattr,
                           unsigned iattr, int round);
...
void MASTER_RET vtext_putns(unsigned x, unsigned y, const char MASTER_PTR *str,
                            unsigned wid);
void MASTER_RET vtext_putnsa(unsigned x, unsigned y, const char MASTER_PTR *str,
                             unsigned wid, unsigned atrb);
void MASTER_RET vtext_putsa(unsigned x, unsigned y, const char MASTER_PTR *strp,
                            unsigned atrb);
...
void MASTER_RET vtext_frame(int x1, int y1, int x2, int y2, unsigned attrl,
                            unsigned attri, int dummy);
...
#define vtext_cursor_shown()                                                   \
  (((vtext_getcursor() >> 8) & 0x3f) <= (vtext_getcursor() & 7))
...
#define text_boxfilla(x1, y1, x2, y2, atrb)                                    \
  vtext_boxfilla(x1, y1, x2, y2, vtext_color_98(atrb))
...
#define text_putca(x, y, ch, atrb) vtext_putca(x, y, ch, vtext_color_98(atrb))
#define text_putns(x, y, str, wid) vtext_putns(x, y, str, wid)
#define text_putnp(x, y, pstr, wid) /* ? */
#define text_putnsa(x, y, str, wid, atrb)                                      \
  vtext_putnsa(x, y, str, wid, vtext_color_98(atrb))
#define text_putnpa(x, y, pstr, wid, atrb) /* ? */
#define text_puts(x, y, str) vtext_puts(x, y, str)
#define text_putp(x, y, pstr) /* ? */
#define text_putsa(x, y, str, atrb) vtext_putsa(x, y, str, vtext_color_98(atrb))
...
#define text_roll_up_ca(fillchar, atrb)                                        \
  vtext_roll_up_ca(fillchar, vtext_color_98(atrb))
#define text_roll_down_ca(fillchar, atrb)                                      \
  vtext_roll_down_ca(fillchar, vtext_color_98(atrb))
...
#define text_frame(x1, y1, x2, y2, wattr, iattr, round)                        \
  vtext_frame(x1, y1, x2, y2, vtext_color_98(wattr), vtext_color_98(iattr),    \
              round)
...
int MASTER_RET
ems_movememoryregion(const struct EMS_move_source_dest MASTER_PTR *block);
void MASTER_RET ems_enablepageframe(int enable);

int MASTER_RET ems_write(unsigned handle, long offset, const void far *mem,
                         long size);
int MASTER_RET ems_read(unsigned handle, long offset, void far *mem, long size);
unsigned long MASTER_RET ems_space(void);
int MASTER_RET ems_maphandlepage(int phys_page, unsigned handle,
                                 unsigned log_page);
...
long MASTER_RET ems_dos_read(int file_handle, unsigned short ems_handle,
                             unsigned long ems_offs, long read_bytes);
long MASTER_RET ems_dos_write(int file_handle, unsigned short ems_handle,
...
void MASTER_RET graph_byteput(int cx1, int y1, int cx2, int y2,
                              const void far *buf);
void MASTER_RET graph_move(int x1, int y1, int x2, int y2, int nx, int ny);
void MASTER_RET graph_pack_put_8(int x, int y, const void far *linepat,
                                 int len);
void MASTER_RET graph_pack_put_down_8(int x, int y, const void far *pat,
                                      int patwidth, int width, int height);
void MASTER_RET graph_pack_get_8(int x, int y, void far *linepat, int len);
void MASTER_RET graph_unpack_put_8(int x, int y, const void far *linepat,
                                   int len);
void MASTER_RET graph_unpack_large_put_8(int x, int y, const void far *linepat,
                                         int len);
...
#define graph_ank_put(x, y, chp, color)                                        \
  graph_ank_putc((x), (y), *(chp), (color))
void MASTER_RET graph_ank_puts(int x, int y, int step,
                               const char MASTER_PTR *ank, int color);
void MASTER_RET graph_ank_putp(int x, int y, int step,
                               const char MASTER_PTR *passtr, int color);
void MASTER_RET graph_font_put(int x, int y, const char MASTER_PTR *str,
                               int color);
void MASTER_RET graph_font_puts(int x, int y, int step,
                                const char MASTER_PTR *str, int color);
void MASTER_RET graph_font_putp(int x, int y, int step,
                                const char MASTER_PTR *passtr, int color);
void MASTER_RET graph_kanji_large_put(int x, int y, const char MASTER_PTR *str,
                                      int color);
void MASTER_RET graph_kanji_put(int x, int y, const char MASTER_PTR *str,
                                int color);
void MASTER_RET graph_kanji_puts(int x, int y, int step,
                                 const char MASTER_PTR *str, int color);

void MASTER_RET graph_wank_putc(int x, int y, int c);
void MASTER_RET graph_wank_puts(int x, int y, int step,
                                const char MASTER_PTR *str);
void MASTER_RET graph_wank_putca(int x, int y, int ch, int color);
void MASTER_RET graph_wank_putsa(int x, int y, int step,
                                 const char MASTER_PTR *str, int color);
void MASTER_RET graph_wfont_put(int x, int y, const char MASTER_PTR *str);
void MASTER_RET graph_wfont_puts(int x, int y, int step,
                                 const char MASTER_PTR *str);
...
void MASTER_RET graph_wkanji_put_right(int x, int y,
                                       const char MASTER_PTR *str);
void MASTER_RET graph_wkanji_puts(int x, int y, int step,
                                  const char MASTER_PTR *str);
void MASTER_RET graph_gaiji_putc(int x, int y, int c, int color);
void MASTER_RET graph_gaiji_puts(int x, int y, int step,
                                 const char MASTER_PTR *str, int color);
void MASTER_RET graph_bfnt_puts(int x, int y, int step,
                                const char MASTER_PTR *str, int color);
void MASTER_RET graph_bfnt_putp(int x, int y, int step,
                                const char MASTER_PTR *passtr, int color);
...
void MASTER_RET vga4_byteput(int cx1, int y1, int cx2, int y2,
                             const void far *buf);
void MASTER_RET vga4_pack_put_8(int x, int y, const void far *linepat, int len);
void MASTER_RET vga4_unpack_put_8(int x, int y, const void far *linepat,
                                  int len);
...
void MASTER_RET vga4_byte_move(int x1, int y1, int x2, int y2, int tox,
                               int toy);

void MASTER_RET vga4_wfont_puts(int x, int y, int step,
                                const char MASTER_PTR *str);
void MASTER_RET vga4_bfnt_puts(int x, int y, int step,
                               const char MASTER_PTR *anks, int color);
...
#define graph_200line(t)                                                       \
  (vga4_start(0x8e, 640, 200),                                                 \
   graph_VramSeg = ClipYT_seg = (0xa000 + ((t) & 1) * 400 * 5),                \
   at98_Offset = 0, at98_showpage(at98_VPage), at98_accesspage(at98_APage),    \
   vtextx_start())
...
#define graph_xlat_dot(x, y, tr)                                               \
  vgc_setcolor(VGA_PSET, tr[vga4_readdot(x, y)]), vgc_pset(x, y)
...
#define graph_xorboxfill(x1, y1, x2, y2, c)                                    \
  (vgc_setcolor(VGA_XOR, c), vgc_boxfill(x1, y1, x2, y2),                      \
   vgc_setcolor(VGA_PSET, 0))
#define graph_copy_page(to)                                                    \
  (graph_accesspage(0),                                                        \
...
#define graph_scrollup(up)                                                     \
  at98_scroll(graph_VramLines - (up), (up) * graph_VramWidth / 2)
...
#define graph_unpack_large_put_8(x, y, linepat, len)                           \
  vga4_pack_put_8(x, y, linepat, (len) * 2) /* ? */
#define graph_pack_get_8(x, y, buf, len) vga4_pack_get_8(x, y, buf, len)
#define graph_unpack_put_8(x, y, linepat, len)                                 \
  vga4_unpack_put_8(x, y, linepat, len)
...
#define graph_bfnt_puts(x, y, s, str, c)                                       \
  (vgc_setcolor(VGA_PSET, c), vgc_bfnt_puts(x, y, s, str))
#define graph_bfnt_putc(x, y, ank, c)                                          \
  (vgc_setcolor(VGA_PSET, c), vgc_bfnt_putc(x, y, ank))
#define graph_gaiji_puts(x, y, s, str, c)                                      \
  graph_bfnt_puts(x, y, s, str, c)                                   /* ? */
...
#define graph_font_puts(x, y, s, str, c)                                       \
  (vgc_setcolor(VGA_PSET, c), vgc_font_puts(x, y, s, str))
#define graph_font_put(x, y, str, c)                                           \
  (vgc_setcolor(VGA_PSET, c), vgc_font_put(x, y, str))
#define graph_kanji_put(x, y, str, c)                                          \
  (vgc_setcolor(VGA_PSET, c),                                                  \
   vgc_kanji_putc(x, y, _rotr(*(unsigned short far *)(str), 8)))
#define graph_kanji_puts(x, y, s, str, c)                                      \
  (vgc_setcolor(VGA_PSET, c), vgc_kanji_puts(x, y, s, str))
#define graph_kanji_putc(x, y, knj, c)                                         \
  (vgc_setcolor(VGA_PSET, c), vgc_kanji_putc(x, y, knj))

#endif
#endif

int MASTER_RET mag_load_pack(const char MASTER_PTR *filename,
                             MagHeader MASTER_PTR *header,
                             void far *MASTER_PTR *bufptr);
int MASTER_RET graph_pi_load_pack(const char MASTER_PTR *filename,
                                  PiHeader MASTER_PTR *header,
                                  void far *MASTER_PTR *bufptr);
void far *MASTER_RET graph_pi_load_unpack(const char MASTER_PTR *filename,
                                          PiHeader MASTER_PTR *header);
int MASTER_RET graph_pi_comment_load(const char MASTER_PTR *filename,
                                     PiHeader MASTER_PTR *header);
void MASTER_RET graph_pi_free(PiHeader MASTER_PTR *header,
                              const void far *image);
void MASTER_RET mag_free(MagHeader MASTER_PTR *header, const void far *image);

#define graph_wfont_plane(a, b, c)                                             \
  (wfont_Plane1 = (unsigned char)(a), wfont_Plane2 = (unsigned char)(b),       \
   wfont_Reg = (unsigned char)(c))
...
#define palette_set(n, r, g, b)                                                \
  (Palettes[n][0] = (unsigned char)(r), Palettes[n][1] = (unsigned char)(g),   \
   Palettes[n][2] = (unsigned char)(b))
...
unsigned MASTER_RET resdata_exist(const char MASTER_PTR *id, unsigned idlen,
                                  unsigned parasize);
unsigned MASTER_RET resdata_create(const char MASTER_PTR *id, unsigned idlen,
                                   unsigned parasize);
...
void MASTER_RET file_splitpath(const char MASTER_PTR *path,
                               char MASTER_PTR *drv, char MASTER_PTR *dir,
                               char MASTER_PTR *name, char MASTER_PTR *ext);
void MASTER_RET file_splitpath_slash(char MASTER_PTR *path,
                                     char MASTER_PTR *drv, char MASTER_PTR *dir,
                                     char MASTER_PTR *name,
                                     char MASTER_PTR *ext);
...
void MASTER_RET dos_absread(int drive, void far *buf, unsigned pow,
                            long sector);
void MASTER_RET dos_abswrite(int drive, const void far *buf, unsigned pow,
                             long sector);
...
void(interrupt far *MASTER_RET dos_setvect(int vect,
                                           void(interrupt far *intfunc)()))();

const char far *MASTER_RET dos_getenv(unsigned envseg,
                                      const char MASTER_PTR *envname);
...
unsigned MASTER_RET dos_findmany(const char far *path, int attribute,
                                 struct find_many_t far *buffer,
                                 unsigned max_dir);
...
int MASTER_RET dos_get_driveinfo(int drive, unsigned MASTER_PTR *cluster,
                                 unsigned MASTER_PTR *sector,
                                 unsigned MASTER_PTR *bytes);
...
#define vsync_proc_set(proc)                                                   \
  (CLI(), (vsync_Proc = (void(far *)(void))(proc)), STI())
...
#define vsync_start()                                                          \
  vga_vsync_start(),                                                           \
      (vsync_Delay = ((vsync_Freq < 21147)                                     \
                          ? (unsigned)((21147 - vsync_Freq) * 65536UL / 21147) \
                          : 0))
...
char MASTER_PTR *MASTER_RET str_pastoc(char MASTER_PTR *CString,
                                       const char MASTER_PTR *PascalString);
char MASTER_PTR *MASTER_RET str_ctopas(char MASTER_PTR *PascalString,
                                       const char MASTER_PTR *CString);
...
int MASTER_RET xms_movememory(long destOff, unsigned destHandle, long srcOff,
                              unsigned srcHandle, long Length);
...
void MASTER_RET gaiji_putsa(unsigned x, unsigned y, const char MASTER_PTR *str,
                            unsigned atrb);
void MASTER_RET gaiji_putp(unsigned x, unsigned y, const char MASTER_PTR *pstr);
void MASTER_RET gaiji_putpa(unsigned x, unsigned y, const char MASTER_PTR *pstr,
                            unsigned atrb);
void MASTER_RET gaiji_putni(unsigned x, unsigned y, unsigned val,
                            unsigned width);
void MASTER_RET gaiji_putnia(unsigned x, unsigned y, unsigned val,
...
#define gaiji_putsa(x, y, s, a)                                                \
  vga4_bfnt_puts((x) * 8, (y) * 16, 16, (s),                                   \
                 vtext_colortable[vtext_color_98(a) & 0x0f] +                  \
                     (vga4_readdot((x) * 8, (y) * 16) << 4))
#define gaiji_puts(x, y, s)                                                    \
  vga4_bfnt_puts((x) * 8, (y) * 16, 16, (s), 0x07) /* ? */
#define gaiji_putnia(x, y, v, w, a)                                            \
  {                                                                            \
    char ___s[8];                                                              \
    str_printf(___s, "%07d", (v));                                             \
    gaiji_putsa(x, y, ___s + 7 - (w), a);                                      \
  }
#define gaiji_putca(x, y, c, a)                                                \
  vga4_bfnt_putc((x) * 8, (y) * 16, (c),                                       \
                 vtext_colortable[vtext_color_98(a) & 0x0f] +                  \
                     (vga4_readdot((x) * 8, (y) * 16) << 4))
...
#define mouse_get(ms)                                                          \
  (CLI(), (ms)->x = mouse_X, (ms)->y = mouse_Y, (ms)->button = mouse_Button,   \
   STI())
...
void MASTER_RET cursor_pattern(int px, int py, int blc, int whc,
                               const void far *pattern);
void MASTER_RET cursor_pattern2(int px, int py, int whc,
                                const void far *pattern);
...
#define cursor_setpattern(pat, blc, whc)                                       \
  cursor_pattern(pat.px, pat.py, (blc), (whc), pat.pattern)
...
/* バイト数の所要パラグラフ数を得る */
#define BYTE2PARA(byteval)                                                     \
  (unsigned)(((((long)(byteval)) & 0xfffffL) + 15) >> 4)
/* セグメント値をポインタに変換する */
...
#define sio_error_reset(port)                                                  \
  (sio_bit_on((port), SIO_ERRCLR), sio_bit_off((port), SIO_ERRCLR))
...
unsigned MASTER_RET sio_write(int port, const void MASTER_PTR *senddata,
                              unsigned sendlen);
unsigned MASTER_RET sio_read(int port, void MASTER_PTR *recbuf,
                             unsigned reclen);
...
void MASTER_RET pcm_convert(void far *dest, const void far *src, unsigned rate,
                            unsigned long size);
#if MASTER98
void MASTER_RET pcm_play(const void far *pcm, unsigned rate,
                         unsigned long size);
...
#define js_keyassign(func, group, maskbit)                                     \
  js_key((unsigned)func, (group) + 0x52a, (maskbit))
#define js_key2player(flag)                                                    \
  (((js_2player = (flag)) != 0)                                                \
...
#define at_js_keyassign(f, g, m)                                               \
  js_key((unsigned)f, (g) + (unsigned)js_map, (m))
#define at_js_key2player(flag)                                                 \
  (((js_2player = (flag)) != 0) ? (at_js_keyassign(AT_JS_1P1, 10, 0x04),       \
                                   at_js_keyassign(AT_JS_1P2, 9, 0x10))        \
                                : (at_js_keyassign(AT_JS_1P1, 5, 0x10),        \
                                   at_js_keyassign(AT_JS_1P2, 5, 0x20)))

int MASTER_RET at_js_wait(Point *p);
void MASTER_RET at_js_calibrate(const Point far *min, const Point far *max,
                                const Point far *center);
#define at_js_resptr ((struct AT_JS_CALIBDATA far *)SEG2FP(at_js_resseg))
#define at_js_get_calibrate()                                                  \
  (at_js_resseg = resdata_exist(AT_JS_RESID, AT_JS_RESIDLEN, AT_JS_RESPARASIZE))
...
#if !MASTER98
#define js_start(m)                                                            \
  ((at_js_fast = 1), at_js_start(m),                                           \
   (at_js_get_calibrate() != 0)                                                \
       ? (at_js_calibrate(&at_js_resptr->dmin, &at_js_resptr->dmax,            \
...
#define pokeb2(seg, off, data)                                                 \
  ((*(unsigned char far *)MK_FP(seg, off)) = (data))
#ifndef __TURBOC__
...
int MASTER_RET bfnt_header_load(const char MASTER_PTR *filename,
                                BfntHeader MASTER_PTR *header);
...
int MASTER_RET super_entry_pat(int patsize, const void far *image,
                               int clear_color);
int MASTER_RET super_entry_pack(const void far *image, unsigned image_width,
                                int patsize, int clear_color);
void MASTER_RET super_free(void);
int MASTER_RET super_backup_ems(unsigned MASTER_PTR *handle, int first_pat,
                                int last_pat);
...
void MASTER_RET super_put_1plane(int x, int y, int num, int pattern_plane,
                                 unsigned put_plane);
void MASTER_RET super_put_1plane_8(int x, int y, int num, int pattern_plane,
                                   unsigned put_plane);
...
#endif
#if MASTERV
#define vga4_repair_back(x, y, num)                                            \
  (graph_accesspage(0),                                                        \
   vga4_byte_move(                                                             \
...
void MASTER_RET vga4_super_put_1plane(int x, int y, int num, int pattern_plane,
                                      unsigned put_plane);
void MASTER_RET vga4_super_put_1plane_8(int x, int y, int num,
...
#define super_put_1plane(x, y, num, pat, put)                                  \
  vga4_super_put_1plane(x, y, num, pat, put)
#define super_put_1plane_8(x, y, num, pat, put)                                \
  vga4_super_put_1plane_8(x, y, num, pat, put)
...
void MASTER_RET super_roll_put_1plane(int x, int y, int num, int pattern_plane,
                                      unsigned put_plane);
void MASTER_RET super_roll_put_1plane_8(int x, int y, int num,
                                        int pattern_plane, unsigned put_plane);
void MASTER_RET super_roll_put_8(int x, int y, int num);
void MASTER_RET super_zoom(int x, int y, int num, int zoom);
void MASTER_RET super_zoom_put(int x, int y, int num, unsigned x_rate,
                               int y_rate);
void MASTER_RET super_zoom_put_1plane(int x, int y, int num, unsigned x_rate,
                                      unsigned y_rate, int pattern_plane,
                                      unsigned put_plane);
#endif
...
void MASTER_RET vga4_super_zoom_put(int x, int y, int num, unsigned x_rate,
                                    int y_rate);
...
#if MASTER98
void MASTER_RET super_wave_put(int x, int y, int num, int len, char amp,
                               int ph);
void MASTER_RET super_wave_put_1plane(int x, int y, int num, int len, char amp,
                                      int ph, int pattern_plane,
                                      unsigned put_plane);
void MASTER_RET super_vibra_put(int x, int y, int num, int len, int ph);
void MASTER_RET super_vibra_put_1plane(int x, int y, int num, int len, int ph,
                                       int pattern_plane, unsigned put_plane);
void MASTER_RET super_zoom_v_put(int x, int y, int num, unsigned y_rate);
void MASTER_RET super_zoom_v_put_1plane(int x, int y, int num, unsigned rate,
                                        int pattern_plane, unsigned put_plane);
...
void MASTER_RET vga4_super_wave_put(int x, int y, int num, int len, char amp,
                                    int ph);
void MASTER_RET vga4_super_vibra_put(int x, int y, int num, int len, int ph);
void MASTER_RET vga4_super_wave_put_1plane(int x, int y, int num, int len,
                                           char amp, int ph, int pattern_plane,
                                           unsigned put_plane);

void MASTER_RET vga4_super_zoom_v_put_1plane(int x, int y, int num,
                                             unsigned rate, int pattern_plane,
                                             unsigned put_plane);

#if !MASTER98
#define super_wave_put(x, y, num, len, amp, ph)                                \
  vga4_super_wave_put(x, y, num, len, amp, ph)
#define super_wave_put_1plane(x, y, num, len, amp, ph, pat, put)               \
  vga4_super_wave_put_1plane(x, y, num, len, amp, ph, pat, put)
#define super_vibra_put(x, y, num, len, ph)                                    \
  vga4_super_vibra_put(x, y, num, len, ph)
#define super_vibra_put_1plane(x, y, num, len, ph, pat, put)                   \
  super_put_1plane(x, y, num, pat, put)
#define super_zoom_v_put(x, y, num, yrate) super_zoom_put(x, y, num, 256, yrate)
#define super_zoom_v_put_1plane(x, y, num, rate, pat, put)                     \
  vga4_super_zoom_v_put_1plane(x, y, num, rate, pat, put)

#define over_dot_8(x, y)                                                       \
  vgc_setcolor(VGA_PSET, 0),                                                   \
      (*(char far *)MK_FP(graph_VramSeg, (y) * graph_VramWidth + (x) / 8) =    \
           '\xff')
#define over_blk16_8(x, y)                                                     \
  vgc_setcolor(VGA_PSET, 0), vgc_byteboxfill_x_pset((x), y, (x) + 1, (y) + 15)
#endif
#endif
...
#if MASTERV
#if !MASTER98
#define egc_shift_down_all(dots)                                               \
  vga4_byte_move(0, 0, graph_VramWidth - 1, graph_VramLines - (dots), 0, (dots))
#define egc_shift_up_all(dots)                                                 \
  vga4_byte_move(0, (dots), graph_VramWidth - 1, graph_VramLines - (dots), 0, 0)
#endif
#endif
...
/* 台形 */
void MASTER_RET grcg_trapezoid(int y1, int x11, int x12, int y2, int x21,
                               int x22);
...
void MASTER_RET grcg_thick_line(int x1, int y1, int x2, int y2, unsigned wid,
                                unsigned hei);
...
void MASTER_RET vgc_thick_line(int x1, int y1, int x2, int y2, unsigned wid,
                               unsigned hei);
...
void MASTER_RET vgc_trapezoid(int y1, int x11, int x12, int y2, int x21,
                              int x22);
...
void MASTER_RET vgc_font_puts(int x, int y, int step,
                              const char MASTER_PTR *str);
void MASTER_RET vgc_font_put(int x, int y, const char MASTER_PTR *str);

void MASTER_RET vgc_bfnt_puts(int x, int y, int step,
                              const char MASTER_PTR *str);
void MASTER_RET vgc_bfnt_putc(int x, int y, int ank);
void MASTER_RET vgc_kanji_puts(int x, int y, int step,
                               const char MASTER_PTR *kanji);
...
#define grcg_trapezoid(y1, x11, x12, y2, x21, x22)                             \
  vgc_trapezoid(y1, x11, x12, y2, x21, x22)
...
#define grcg_triangle(x1, y1, x2, y2, x3, y3)                                  \
  vgc_triangle(x1, y1, x2, y2, x3, y3)
...
#define grcg_thick_line(x1, y1, x2, y2, wid, hei)                              \
  vgc_thick_line(x1, y1, x2, y2, wid, hei)
...
#define grcg_round_boxfill(x1, y1, x2, y2, r)                                  \
  vgc_round_boxfill(x1, y1, x2, y2, r)
...
#define grcg_byteboxfill_x(x1, y1, x2, y2)                                     \
  vgc_byteboxfill_x_pset(x1, y1, x2, y2)
...
#define gdc_setaccessplane(plane)                                              \
  OUTW(SEQ_PORT, SEQ_MAP_MASK_REG | (((~(plane)) & 15) << 8))
