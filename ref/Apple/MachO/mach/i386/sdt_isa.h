 ...
 *
 * Using any of the non volatile register causes a spill to stack which is
 * almost certainly a net performance loss. Also, note that the array ref
 * (__dtrace_args) consumes one free register. If all three of the volatile regs
 * are used for load/store, the compiler will spill a register to hold the array
 * ref.
 *
...
#define DTRACE_CALL0ARGS(provider, name)                                       \
  asm volatile(                                                                \
      DTRACE_CALL(provider, name) "# eat trailing nl +tabfrom DTRACE_CALL"     \
      :                                                                        \
      :);
...
#define DTRACE_CALL1ARG(provider, name)                                        \
  asm volatile("subl\t$0x10,%%esp"                                             \
               ...
               "\n\t"                                                          \
               "movl\t%%eax,0x0(%%esp)"                                        \
               "\n\t" DTRACE_CALL(provider, name) "addl\t$0x10,%%esp"          \
               :                                                               \
               : "r"(__dtrace_args)                                            \
               : "memory", "eax");
...
#define DTRACE_CALL5ARGS(provider, name)                                       \
  asm volatile("subl\t$0x20,%%esp"                                             \
               ...
               "\n\t"                                                          \
               "movl\t%%eax,0x10(%%esp)"                                       \
               "\n\t" DTRACE_CALL(provider, name) "addl\t$0x20,%%esp"          \
               :                                                               \
               : "r"(__dtrace_args)                                            \
               : "memory", "eax", "edx");
...
#define DTRACE_CALL9ARGS(provider, name)                                       \
  asm volatile("subl\t$0x30,%%esp"                                             \
               ...
               "\n\t"                                                          \
               "movl\t%%eax,0x20(%%esp)"                                       \
               "\n\t" DTRACE_CALL(provider, name) "addl\t$0x30,%%esp"          \
               :                                                               \
               : "r"(__dtrace_args)                                            \
               : "memory", "eax", "edx");
