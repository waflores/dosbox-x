...
#ifdef __thumb__
#define DTRACE_ALLOC_STACK(n) "sub sp, #" #n "\n\t"
...
#define DTRACE_CALL0ARGS(provider, name)                                       \
  asm volatile(                                                                \
      DTRACE_CALL(provider, name) "# eat trailing nl+tab from DTRACE_CALL"     \
      :                                                                        \
      :);
...
/*
 * One of our ARM32 ABIs (armv7k) mandates that the stack be aligned to 16
 * bytes. We currently apply this constraint to all ARM32 DTRACE_CALL macros;
 * hence the macros below will overallocate for some ABIs.
 */
#define DTRACE_CALL5ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldr r0, [%0, #16]"                      \
                                      "\n\t"                                   \
                                      "str r0, [sp]"                           \
                                      "\n\t"                                   \
                                      "ldr r3, [%0, #12]"                      \
...
#define DTRACE_CALL6ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldr r1, [%0, #20]"                      \
                                      "\n\t"                                   \
                                      "ldr r0, [%0, #16]"                      \
                                      "\n\t"                                   \
                                      "str r1, [sp, #4]"                       \
...
#define DTRACE_CALL7ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldr r2, [%0, #24]"                      \
                                      "\n\t"                                   \
                                      "ldr r1, [%0, #20]"                      \
                                      "\n\t"                                   \
                                      "ldr r0, [%0, #16]"                      \
...
#define DTRACE_CALL8ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldr r3, [%0, #28]"                      \
                                      "\n\t"                                   \
                                      "ldr r2, [%0, #24]"                      \
                                      "\n\t"                                   \
                                      "ldr r1, [%0, #20]"                      \
...
#define DTRACE_CALL9ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(32) "ldr r0, [%0, #32]"                      \
                                      "\n\t"                                   \
                                      "str r0, [sp, #16]"                      \
                                      "\n\t"                                   \
                                      "ldr r3, [%0, #28]"                      \
...
#define DTRACE_CALL10ARGS(provider, name)                                      \
  asm volatile(DTRACE_ALLOC_STACK(32) "ldr r1, [%0, #36]"                      \
                                      "\n\t"                                   \
                                      "ldr r0, [%0, #32]"                      \
                                      "\n\t"                                   \
                                      "str r1, [sp, #20]"                      \
...
#define DTRACE_CALL9ARGS(provider, name)                                       \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldr x0, [%0, #64]"                      \
                                      "\n\t"                                   \
                                      "str x0, [sp]"                           \
                                      "\n\t"                                   \
                                      "ldp x6, x7, [%0, #48]"                  \
...
#define DTRACE_CALL10ARGS(provider, name)                                      \
  asm volatile(DTRACE_ALLOC_STACK(16) "ldp x0, x1, [%0, #64]"                  \
                                      "\n\t"                                   \
                                      "stp x0, x1, [sp]"                       \
                                      "\n\t"                                   \
                                      "ldp x6, x7, [%0, #48]"                  \
