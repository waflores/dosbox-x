 ...
 * by dyld whenever an image is loaded or unloaded.  During a call to
 * _dyld_register_func_for_add_image() the callback func is called for every
 * existing image.  Later, it is called as each new image is loaded and bound
 * (but initializers not yet run).  The callback registered with
 * _dyld_register_func_for_remove_image() is called after any terminators in an
 * image are run and before the image is un-memory-mapped.
 */
 ...
 * currently dylib specifed by the libraryName.  The libraryName parameter would
 * be "bar" for /path/libbar.3.dylib and "Foo" for
 * /path/Foo.framework/Versions/A/Foo.  It returns -1 if no such library is
 * loaded.
 */
...
/*
 * NSVersionOfLinkTimeLibrary() returns the current_version number that the main
 * executable was linked against at build time.  The libraryName parameter would
 * be "bar" for /path/libbar.3.dylib and "Foo" for
 * /path/Foo.framework/Versions/A/Foo.  It returns -1 if the main executable did
 * not link against the specified library.
 */
 ...
 * buffer. The bufsize parameter should initially be the size of the buffer. The
 * function returns 0 if the path was successfully copied, and *bufsize is left
 * unchanged. It returns -1 if the buffer is not large enough, and *bufsize is
 ...
 * not the real file. With deep directories the total bufsize needed could be
 * more than MAXPATHLEN.
 */
 ...
 * Called by c++ compiler to implement destructors on thread_local object
 * variables.
 */
 ...
 * the thread local is prepared, the pointer changes to a real handler such as
 * tlv_get_addr.
 */
extern void _tlv_bootstrap(void)
    __OSX_AVAILABLE_STARTING(__MAC_10_10,
                             __IPHONE_8_0) DYLD_DRIVERKIT_UNAVAILABLE;

/*
 * Dylibs that are incorporated into the dyld cache are removed from disk. That
 * means code cannot stat() the file to see if it "exists".  This function is
 * like a stat() call that checks if a path is to a dylib that was removed from
 * disk and is incorporated into the active dyld cache.
 */
extern bool _dyld_shared_cache_contains_path(const char *path)
    __API_AVAILABLE(macos(11.0), ios(14.0), watchos(7.0),
                    tvos(14.0)) DYLD_DRIVERKIT_UNAVAILABLE;
...
NSCreateObjectFileImageFromFile(const char *pathName,
    ...
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
...
NSSymbolReferenceCountInObjectFileImage(NSObjectFileImage objectFileImage)
    ...
    __OSX_DEPRECATED(10.1, 10.5, "");
extern const char *
NSSymbolReferenceNameInObjectFileImage(NSObjectFileImage objectFileImage,
                                       uint32_t ordinal,
...
extern void *
NSGetSectionDataInObjectFileImage(NSObjectFileImage objectFileImage,
                                  const char *segmentName,
                                  const char *sectionName, size_t *size)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
...
extern NSModule NSLinkModule(NSObjectFileImage objectFileImage,
                             const char *moduleName, uint32_t options)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.5, "dlopen()");
...
extern bool NSIsSymbolNameDefined(const char *symbolName)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.4, "dlsym()");
extern bool NSIsSymbolNameDefinedWithHint(const char *symbolName,
                                          const char *libraryNameHint)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.4, "dlsym()");
extern bool NSIsSymbolNameDefinedInImage(const struct mach_header *image,
                                         const char *symbolName)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.4, "dlsym()");
...
extern void
NSInstallLinkEditErrorHandlers(const NSLinkEditErrorHandlers *handlers)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.5, "");
...
extern void _dyld_lookup_and_bind(const char *symbol_name, void **address,
                                  NSModule *module)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.4, "dlsym()");
extern void _dyld_lookup_and_bind_with_hint(const char *symbol_name,
                                            const char *library_name_hint,
                                            void **address, NSModule *module)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.4, "dlsym()");
extern void _dyld_lookup_and_bind_fully(const char *symbol_name, void **address,
                                        NSModule *module)
    __API_UNAVAILABLE(ios, tvos, watchos) DYLD_DRIVERKIT_UNAVAILABLE
    __OSX_DEPRECATED(10.1, 10.5, "dlsym()");
