...
// 0: complex scalers off, scaler cache off, some simple scalers off, memory
// requirements reduced 1: complex scalers off, scaler cache off, all simple
// scalers on 2: complex scalers off, scaler cache on 3: complex scalers on
...
typedef struct {
  ...
  TTF_Font *SDL_fonti;
  TTF_Font *SDL_fontbi;
  bool DOSBox; // is DOSBox-X internal TTF loaded, pointsizes should be even to
               // look really nice
  int pointsize;
...
struct ttf_cell {
  uint16_t chr; // unicode code point OR just the raw code point. set to ' '
                // (0x20) for empty space.
  unsigned int fg : 4;         // foreground color (one of 16)
  unsigned int bg : 4;         // background color (one of 16)
  unsigned int doublewide : 1; // double-wide (e.g. PC-98 JIS), therefore skip
                               // next character cell.
  ...
  bool operator==(const ttf_cell &lhs) const {
    return chr == lhs.chr && fg == lhs.fg && bg == lhs.bg &&
           doublewide == lhs.doublewide && blink == lhs.blink &&
           skipped == lhs.skipped && underline == lhs.underline &&
           unicode == lhs.unicode;
  }
  bool operator!=(const ttf_cell &lhs) const { return !(*this == lhs); }
};
// FIXME: Perhaps the TTF output code should just render unicode code points and
// vga_draw should do the code page conversion
