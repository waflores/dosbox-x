...
public:
  ...
  ClockDomain(unsigned long long freq_new, unsigned long long div)
      : freq(freq_new), freq_div(div) {}

public:
  void set_name(const char *s) { name = s; }
  void set_frequency(unsigned long long freq_new,
                     unsigned long long div_new = 1) {
    counter = 0;
    freq = freq_new;
    freq_div = div_new;
  }
...
public:
   ...
   *       - Do not set clock time by floating point time (only the toplevel
   * clocks in the tree should do that)
   *       - Must rebase at the same reference time as the master
   *       - Must maintain time according to master time divided by master's
   * clock divider */
  unsigned long long freq,
      freq_div = 1;               /* frequency of clock as integer ratio */
