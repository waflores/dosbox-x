...
typedef IO_ReadHandler *(IO_ReadCalloutHandler)(IO_CalloutObject & co,
                                                Bitu port, Bitu iolen);
typedef IO_WriteHandler *(IO_WriteCalloutHandler)(IO_CalloutObject & co,
                                                  Bitu port, Bitu iolen);
...
public:
  ...
  void Install(Bitu port, Bitu portmask /*IOMASK_ISA_10BIT, etc.*/,
...
static INLINE void IO_Write(Bitu port, uint8_t val) { IO_WriteB(port, val); }
static INLINE uint8_t IO_Read(Bitu port) { return IO_ReadB(port); }
...
static inline constexpr uint32_t IO_Callout_t_comb(const enum IO_Type_t t,
                                                   const uint32_t idx) {
