...
/* NTS: MIXED is not EXPERIMENTAL. Do not enable EXPERIMENTAL level instructions
 * in "auto" aka MIXED mode. For example, FISTTP is a Pentium 4 SSE3
 * instruction, which does not belong in MIXED mode, but users have requested
 * it's addition for some 256-byte DOS demos that use it. */
...
/* Last prefix encountered, needed for Pentium III "mandatory opcode prefixes"
 * needed to differentiate SSE instructions given the opcode. Keeping it small
 * and sequential should help your C++ compiler optimize the switch statement
 * you're probably going to use in the normal core code. */
enum { MP_NONE = 0, MP_66, MP_F2, MP_F3 };
...
public:
  ...
  LinearPt GetBase(void) const {
    if (CPU_ArchitectureType >= CPU_ARCHTYPE_386) {
      return (LinearPt)(((LinearPt)saved.seg.base_24_31 << (LinearPt)24U) |
                        ((LinearPt)saved.seg.base_16_23 << (LinearPt)16U) |
                        (LinearPt)saved.seg.base_0_15);
    } else {
      return (LinearPt)(((LinearPt)saved.seg.base_16_23 << (LinearPt)16U) |
                        (LinearPt)saved.seg.base_0_15);
    }
  }
  ...
  Bitu GetLimit(void) const {
    const Bitu limit =
        ((Bitu)saved.seg.limit_16_19 << (Bitu)16U) | (Bitu)saved.seg.limit_0_15;
    if (saved.seg.g && CPU_ArchitectureType >= CPU_ARCHTYPE_386)
      return ((Bitu)limit << (Bitu)12U) | (Bitu)0xFFFU;
    return limit;
  }
  Bitu GetOffset(void) const {
    if (CPU_ArchitectureType >= CPU_ARCHTYPE_386)
      return ((Bitu)saved.gate.offset_16_31 << (Bitu)16U) |
             (Bitu)saved.gate.offset_0_15;
    else
      return (Bitu)saved.gate.offset_0_15;
  }
  Bitu GetSelector(void) const { return saved.gate.selector; }
  Bitu Type(void) const { return saved.seg.type; }
...
public:
  ...
  Bitu SLDT(void) const { return ldt_value; }
...
public:
  ...
  void SetBusy(const bool busy) {
    if (busy)
      ...
      saved.seg.type &= (~2U); /* -Wconversion cannot silence without
                                  hard-coding ~2U & 0x1F */
  }
};
...
// SSE instructions are available if bit 9 is on in CR4, also enables FXSAVE and
// FXRESTOR
static INLINE bool CPU_SSE(void) {
  // TODO: The architecture test may be replaced with a general global variable
  // that says "sse" allowed
  return (cpu.cr4 & 0x200u) &&
...
enum dyncore_alloc_t {
  DYNCOREALLOC_NONE = 0,
  DYNCOREALLOC_MALLOC, /* allocated with malloc(), base pointer and size rounded
                          up to page size */
  DYNCOREALLOC_MMAP_ANON, /* allocated with mmap() and MAP_ANONYMOUS|MAP_PRIVATE
                           */
  DYNCOREALLOC_SHM_MMAP, /* allocated as a file in shared memory (i.e. /dev/shm)
                            using shmget */
...
enum dyncore_method_t {
  ...
  DYNCOREM_MPROTECT_RW_RX, /* map with read/execute, mprotect to read/write when
                              changes needed, then mprotect back */
  DYNCOREM_DUAL_RW_X,      /* map twice in process space: once with read/write
                              permissions, once with execute permissions */
...
#define DYNCOREF_IMPOSSIBLE                                                    \
  (1u << 1u) /* system policy makes dynamic core generation impossible (for    \
                example, SELinux) */
