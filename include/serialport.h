...
public:
  ...
  INLINE Bitu getFree(void) { return size - used; }
  bool isEmpty() { return used == 0; }
  bool isFull() { return (size - used) == 0; }

  INLINE Bitu getUsage(void) { return used; }
...
#define THR_OFFSET                                                             \
  0            // w Transmit Holding Register
               // Data: whole byte
  uint8_t IER; //	r/w		Interrupt Enable Register, also MSB of
               //Divisor Latch
#define IER_OFFSET 1
...
#define Modem_Status_INT_Enable_MASK 0x8
  ...
  uint8_t ISR; //	r				Interrupt Status
               //Register
#define ISR_OFFSET 2
...
const uint16_t serial_defaultaddr[] = {0x3f8, 0x2f8, 0x3e8, 0x2e8, 0,
                                       0,     0,     0,     0};
const char *const serial_comname[] = {"COM1", "COM2", "COM3", "COM4", "COM5",
                                      "COM6", "COM7", "COM8", "COM9"};
...
public:
  // Creates a COM device that communicates with the num-th parallel port, i.e.
  // is LPTnum
