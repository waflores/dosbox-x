...
#define BIOS_EXTENDED_BIOS_DATA_SEGMENT                                        \
  0x40e                          /* uint16_t (newer systems since PS/2) */
...
#define BIOS_TRUE_MEMORY_SIZE                                                  \
  0x415 /* uint16_t, in KB, conventional memory (XT) */
...
#define BIOS_FLOPPY_CONTROLLER_STAT_CMD_REGS                                   \
  0x442                                 /* uint8_t[7] status/command bytes */
...
#define BIOS_POST_RESET_VECTOR                                                 \
  0x467 /* uint32_t FAR 16:16 address to jump to after specific 286 reset */
#define BIOS_LAST_UNEXPECTED_IRQ 0x46b
#define BIOS_TIMER 0x46c /* uint32_t Timer ticks (at 18.2Hz) since midnight */
#define BIOS_24_HOURS_FLAG                                                     \
  0x470 /* nonzero if timer tick rollover past midnight */
#define BIOS_CTRL_BREAK_FLAG 0x471 /* CTRL+Break flag (bit 7=1) */
#define BIOS_CTRL_ALT_DEL_FLAG                                                 \
  0x472 /* uint16_t POST reset flag 1234h=warm boot */
...
#define BIOS98_FLAG2                                                           \
  0x400 /* bit 7: 1=RAM drive function present [6:0]=a lot of other various    \
           stuff */
#define BIOS98_EXTMEM_286                                                      \
  0x401 /* extended memory between 0x100000-0xFFFFFF in 128KB units (286+      \
           BIOS) */
#define BIOS98_SYS_SEL                                                         \
  0x402 /* SHIFT/CTRL/GRPH held down, [3:0] = hard disk boot partition         \
           selector (Hires) */
        /* 0x403 ITF ROM work area */
#define BIOS98_286_RESET_VECTOR                                                \
  0x404 /* uint32_t FAR 16:16, sets SS:SP to this value and RETF */
        /* 0x408-0x40F various */
#define BIOS98_KB_SHIFT_CODE_HIRES                                             \
  0x408 /* Hi-res shift key redefinition table */
#define BIOS98_KB_BUFFER_ADR                                                   \
  0x410 /* uint32_t FAR 16:16 keyboard buffer address (hires) */
#define BIOS98_KB_ENTRY_TBL_ADR                                                \
  0x414 /* uint32_t FAR 16:16 key data conversion table (hires) */
#define BIOS98_KB_INT_ADR                                                      \
  0x418 /* uint32_t FAR 16:16 keyboard interrupt table (hires) */
...
#define BIOS98_CAL_USER_OFFSEG                                                 \
  0x44C /* uint32_t FAR 16:16 single-event timer timeout callback (hires) */
#define BIOS98_CRT_FONT                                                        \
  0x450 /* uint8_t[2] (port A1h,port A3h) value to output to CG (hires) */
...
#define BIOS98_RT_P3 0x454 /* (hires) cursor blink, bottom of cursor */
                           /* 0x455-0x456 various */
#define BIOS98_IDE_HDD_CAPACITY                                                \
  0x457 /* bit 7=1st IDE 512/256 byte/sect bit 6=2nd IDE 512/256 byte/sec bit  \
           5-3=1st IDE capacity 2-0=2nd IDE capacity 0x00=Does not support IDE \
           0x3F=supports IDE but no drives */
#define BIOS98_MODEL_ID 0x458 /* bits that identify various models */
#define BIOS98_CRT_EXT_STS                                                     \
  0x459 /* [7:6]=ext attr mode [5:4]=various [3]=hires interlace [2:1]=various \
           [0]=640x480 if set, 640x400 if not */
#define BIOS98_BIOS_FLAG6                                                      \
  0x45A /* either [7:0]=CPU clock or [7:4]=CPU type [3:0]=CPU clock */
#define BIOS98_BIOS_FLAG7                                                      \
  0x45B /* time stamper and port 5Fh wait among other various things */
        /* 0x45C-0x45F various */
#define BIOS98_SCSI_PARAMETER_TABLE                                            \
  0x460 /* struct { uint8_t various[4]; } scsidev[8]; */
#define BIOS98_BIOS_FLAG1                                                      \
  0x480 /* [7]=SASI/IDE new sense [6]=EGC supported [5:0]=various */
#define BIOS98_BIOS_FLAG3                                                      \
  0x481 /* [6,3]=keyboard ID [5]=EMS page frame B0000h [4]=B0000h status       \
           (VRAM/EMS) various */
#define BIOS98_DISK_EQUIPS                                                     \
  0x482 /* [7:0]=SCSI #7-SCSI #0 (matches bit) HDD connected or not */
#define BIOS98_SCSI_STATUS 0x483
#define BIOS98_BIOS_FLAG4                                                      \
  0x484 /* [7:6]=HDD DMA channel [5]=builtin SCSI [4]=builtin SASI [3:0]=CPU   \
           speed */
#define BIOS98_DISK_MOTOR_STOP_COUNTER 0x485 /* in 100ms units */
#define BIOS98_CPU386                                                          \
  0x486 /* uint16_t DX value after CPU reset, 386 or higher */
#define BIOS98_RDISK_EQUIP                                                     \
  0x488 /* [7:4]=640KB FD unit 3,2,1,0 [3:0]=1MB FD unit 3,2,1,0 is/isn't RAM  \
           drive */
#define BIOS98_RDISK_WORK_JMP 0x489 /* uint8_t[5] JMP FAR instruction to BIOS  \
                                     */
#define BIOS98_RDISK_FLAGS                                                     \
  0x48E /* [7]=processing [6:4]=SMM out [3]=FDD processing [2]=RAM drive       \
           number [1:0]=I/F mode */
        /* 0x48F unused */
#define BIOS98_YET_ANOTHER_BEEP_PITCH 0x490 /* uint16_t hires beep pitch */
#define BIOS98_DISK_RESET                                                      \
  0x492 /* [7:4]=recalibrate request 640KB unit 3,2,1,0 [3:0]=recalibrate      \
           request 1MB unit 3,2,1,0 */
#define BIOS98_F2HD_MODE                                                       \
  0x493 /* [7:4]=640KB FDD unit 3,2,1,0 80/40 cylinders [3:0]=640KB FDD unit   \
           3,2,1,0 double/single sided */
#define BIOS98_DISK_EQUIP2                                                     \
  0x494 /* [7:4]=external FDD unit 3,2,1,0 1MB drive in 640KB mode */
#define BIOS98_GRAPH_CHG                                                       \
  0x495 /* GRCG/EGC mode register setting for use by interrupt handlers like   \
           mouse drivers */
#define BIOS98_GRAPH_TAL 0x496 /* uint8_t[4] GRCG tile registers */
                               /* 0x49A-0x4AB various */
#define BIOS98_XROM_PTR                                                        \
  0x49C /* uint32_t FAR 16:16 JMP destination address from INT 1Bh to extended \
           BIOS */
...
Bitu ROMBIOS_GetMemory(Bitu bytes, const char *who = NULL, Bitu alignment = 1,
                       Bitu must_be_at = 0);
...
// Is it safe to call mem_read/write as if part of the guest i.e. from the main
// GUI? If the guest is running in protected mode, NO. If the guest is not
// running under our own DOS kernel, NO. This is required to avoid problems with
// helpful code in this project causing BSODs in Windows NT/2000/XP because the
// BIOS data area addresses are invalid pages in that 32-bit environment!
...
bool ISAPNP_RegisterSysDev(const unsigned char *raw, Bitu len,
                           bool already = false);
...
public:
  ...
  void write_begin_SMALLTAG(const ISAPnPDevice::SmallTags stag,
                            unsigned char len);
  void write_begin_LARGETAG(const ISAPnPDevice::LargeTags stag,
                            unsigned int len);
  ...
  void end_write_res();
  void write_END();
  void write_ISAPnP_version(unsigned char major, unsigned char minor,
                            unsigned char vendor);
  void write_Identifier_String(const char *str);
  void write_Logical_Device_ID(const char c1, const char c2, const char c3,
                               const char c4, const char c5, const char c6,
                               const char c7);
  void write_Compatible_Device_ID(const char c1, const char c2, const char c3,
                                  const char c4, const char c5, const char c6,
                                  const char c7);
  void write_Device_ID(const char c1, const char c2, const char c3,
                       const char c4, const char c5, const char c6,
                       const char c7);
  void write_Dependent_Function_Start(
      const ISAPnPDevice::DependentFunctionConfig cfg,
      const bool force = false);
  void write_IRQ_Format(const uint16_t IRQ_mask,
                        const unsigned char IRQ_signal_type = 0);
  void write_DMA_Format(const uint8_t DMA_mask,
                        const unsigned char transfer_type_preference,
                        const bool is_bus_master, const bool byte_mode,
                        const bool word_mode,
                        const unsigned char speed_supported);
  void write_IO_Port(const uint16_t min_port, const uint16_t max_port,
                     const uint8_t count, const uint8_t alignment = 1,
                     const bool full16bitdecode = true);
...
#define ISAPNP_ID(a, b, c, d, e, f, g)                                         \
  (((a) & 0x1F) << 2) | (((b) & 0x1F) >> 3) | ((((b) & 0x1F) & 7) << 13) |     \
      (((c) & 0x1F) << 8) | (((d) & 0x1F) << 20) | (((e) & 0x1F) << 16) |      \
      (((f) & 0x1F) << 28) | (((g) & 0x1F) << 24)

#define ISAPNP_TYPE(a, b, c) (a), (b), (c)
#define ISAPNP_SMALL_TAG(t, l) (((t) << 3) | (l))

/* WARNING: do not wrap type macro parameter in parens i.e. (type), it breaks
 * PNP structures and Windows 9x/ME will fail to boot! */
#define ISAPNP_SYSDEV_HEADER(id, type, ctrl)                                   \
  ((id) & 0xFF), (((id) >> 8) & 0xFF), (((id) >> 16) & 0xFF),                  \
      (((id) >> 24) & 0xFF), type, ((ctrl) & 0xFF), (((ctrl) >> 8) & 0xFF)

#define ISAPNP_IO_RANGE(info, min, max, align, len)                            \
  ISAPNP_SMALL_TAG(8, 7), (info), ((min) & 0xFF), (((min) >> 8) & 0xFF),       \
      ((max) & 0xFF), (((max) >> 8) & 0xFF), (align), (len)

#define ISAPNP_IRQ_SINGLE(irq, flags)                                          \
  ISAPNP_SMALL_TAG(4, 3), ((1 << (irq)) & 0xFF), (((1 << (irq)) >> 8) & 0xFF), \
      (flags)

#define ISAPNP_IRQ(irqmask, flags)                                             \
  ISAPNP_SMALL_TAG(4, 3), ((irqmask) & 0xFF), (((irqmask) >> 8) & 0xFF), (flags)

#define ISAPNP_DMA_SINGLE(dma, flags)                                          \
  ISAPNP_SMALL_TAG(5, 2), ((1 << (dma)) & 0xFF), (flags)

#define ISAPNP_DMA(dma, flags) ISAPNP_SMALL_TAG(5, 2), ((dma) & 0xFF), (flags)

#define ISAPNP_END ISAPNP_SMALL_TAG(0xF, 1), 0x00
