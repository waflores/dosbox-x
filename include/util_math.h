...
//! \brief Get value sign, i.e. less than zero: -1, zero: 0, greater than
//! zero: 1.
...
struct DOSBox_Vector2 {
  ...
  DOSBox_Vector2 clamp(const DOSBox_Vector2 min,
                       const DOSBox_Vector2 max) const {
    float x = this->X;
    float y = this->Y;
    float xmin = min.X;
    float xmax = max.X;
  ...
  inline float magnitude(void) const { return sqrtf(sqrMagnitude()); }

  inline float sqrMagnitude(void) const { return X * X + Y * Y; }

  DOSBox_Vector2 normalized(void) const {
    float m = this->magnitude();
    return m > 0.0f ? DOSBox_Vector2(this->X / m, this->Y / m)
                    : DOSBox_Vector2();
  }
