...
enum DMAEvent {
  ...
  DMA_MASKED,
  DMA_UNMASKED,
  DMA_READ_COUNTER, /* the guest is reading the counter, which if accuracy is
                       required, means process up to current time */
  //	DMA_TRANSFEREND, this shouldn't really be a signal
};

enum DMATransfer { DMAT_VERIFY = 0, DMAT_WRITE = 1, DMAT_READ = 2 };
...
public:
  ...
  //  I/O port 29h:
  //    bits [7:4] = 0
  //    bits [3:2] = increment mode   0=64KB wraparound (no incr)  1=1MB
  //    boundary wrap   2=invalid   3=16MB boundary wrap bits [1:0] = which DMA
  //    channel to set
  //
  ...
  //    3 = 0xFF
  //
  // TODO: Does this setting stick or does it reset after normal legacy
  // programming?
  // TODO: When the bank auto increments does it increment the actual register
  // or just
  //       an internal copy?
  ...
  void page_bank_increment(void) { // to be called on DMA wraparound
    if (page_bank_increment_wraparound != 0u) {
      // FIXME: Improve this.
      // Currently this code assumes that the auto increment in PC-98 modifies
      // the register value (and therefore visible to the guest). Change this
      // code if that model is wrong.
      const uint8_t add = increment ? 0x01u : 0xFFu;
      const uint8_t nv = (pagenum & (~page_bank_increment_wraparound)) +
  ...
  void Set128KMode(bool en) {
    ...
    //    addr mask = 0x1FFFF   (all bits 0-15 become bits 1-16, bit 15 of addr
    //    takes the place of page register bit 0)
    // 64KB mode (modern PCI including Intel chipsets) (en=false):
    //    page shift = 0        (all 8 bits of page register are used)
    //    addr mask = 0xFFFF    (discard bit 15, bits 0-14 become bits 1-15 on
    //    ISA bus)
    DMA16_PAGESHIFT =
        (en && DMA16)
            ? 0x1
            : 0x0; // nonzero if we're to discard bit 0 of page register
    DMA16_ADDRMASK = (1UL << ((en && DMA16) ? 17UL : 16UL)) -
                     1UL; // nonzero if (addrreg << 1) to cover 128KB, zero if
                          // (addrreg << 1) to discard MSB, limit to 64KB
  }
  ...
  void SetPage(uint8_t val) {
    pagenum = val;
    pagebase = (uint32_t)(pagenum >> DMA16_PAGESHIFT)
               << (uint32_t)((uint8_t)16u + DMA16_PAGESHIFT);
  }
  void Raise_Request(void) { request = true; }
  void Clear_Request(void) { request = false; }
