...
public:
  ...
  Section_line *AddSection_line(char const *const _name,
  ...
  bool PrintConfig(char const *const configfilename, int everything = -1,
                   bool norem = false) const;
  ...
  void ClearExtraData() {
    Section_prop *sec_prop;
    Section_line *sec_line;
    for (const_it tel = sectionlist.begin(); tel != sectionlist.end(); ++tel) {
      sec_prop = dynamic_cast<Section_prop *>(*tel);
      sec_line = dynamic_cast<Section_line *>(*tel);
      if (sec_prop)
        sec_prop->data = "";
      else if (sec_line)
        sec_line->data = "";
...
public:
  ...
  bool opt_langcp = false; // True if command line option -langcp is specified,
                           // always use codepage specified by the language file
