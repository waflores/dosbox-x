...
#pragma pack(push, 1)
typedef union {
  // TODO: The configure script needs to use "long double" on x86/x86_64 and
  // verify sizeof(long double) == 10,
  //       else undef a macro to let the code emulate long double 80-bit IEEE.
  //       Also needs to determine host byte order here so host long double
  //       matches our struct.
  struct {
...
// ^ Remember that in 80-bit extended, the mantissa contains both the fraction
// and integer bit. There is no
...
static const uint64_t FPU_Reg_64_implied_bit =
    ((uint64_t)1ULL << (uint64_t)52ULL);
...
static const uint32_t FPU_Reg_32_implied_bit =
    ((uint32_t)1UL << (uint32_t)23UL);
...
union alignas(8) MMX_reg {
  ...
  struct { /* MMX registers can contain single precision float if the program
              uses AMD 3DNow! instructions */
    FPU_Reg_32 f0, f1;
  } f32;
...
typedef struct {
  ...
  uint32_t m2;
  uint16_t m3;
  // Padding to make the structure 16 bytes so the inline asm in
  // fpu_instructions_x86.h can shift by 4 to index FPU registers
  uint16_t d1;
  uint32_t d2;
} FPU_P_Reg;
static_assert(sizeof(FPU_P_Reg) == 16, "FPU_P_Reg error");

// memory barrier macro. to ensure that reads/stores to one half of the FPU reg
// struct do not overlap with reads/stores from the other half. things can go
// wrong if the compiler writes code to write the mantissa, then load the
// overall as float, then store the exponent. note this is not a hardware level
// memory barrier, this is a compiler level memory barrier against the
// optimization engine.
#if defined(__GCC__)
...
enum FPU_Tag { TAG_Valid = 0, TAG_Zero = 1, TAG_Weird = 2, TAG_Empty = 3 };

template <class T, unsigned bitno, unsigned nbits = 1> struct RegBit {
...
struct FPUControlWord {
  ...
  enum RoundMode { Nearest = 0, Down = 1, Up = 2, Chop = 3 };
  ...
  template <class T> FPUControlWord &operator=(T val) {
    reg = (val & (FPU_ArchitectureType <= FPU_ARCHTYPE_8087 ? mask8087
                                                            : maskNon8087)) |
          reservedMask;
    return *this;
  }
...
struct FPUStatusWord {
  ...
  template <class T> FPUStatusWord &operator=(T val) {
    reg = val;
    return *this;
  }
  operator unsigned() const { return reg; }
  ...
  enum { conditionMask = 0x4700, conditionAndExceptionMask = 0x47bf };
  std::string to_string() const;
};

typedef struct {
#if defined(HAS_LONG_DOUBLE) // probably shouldn't allow struct to change size
                             // based on this
...
// NTS: DOSBox-X until 2023/03/11 and all other forks have dynamic core code
// that generates memory loads
//      from (&TOP). That code is flawed because while you think you are loading
//      the top of the stack, what you are actually doing is taking the address
//      of a bitfield (which doesn't do what you think it does!) and generating
//      code to read that address. Code that you think is using the FPU top of
//      stack is in reality using the entire FPU status word as top of stack!
//
//      This issue has since been resolved by adding a right shift instruction
//      after the load. To make what is actually happening clearer for
//      development going forward, all dynamic core code has been changed to use
//      &FPUSW instead of &TOP.
//
//      FPUSW is a macro that resolves to the "reg" union field which is a plain
//      16-bit unsigned integer containing all FPU status word bits.
...
static INLINE void FPU_SET_C0(Bitu C) { fpu.sw.C0 = !!C; }

static INLINE void FPU_SET_C1(Bitu C) { fpu.sw.C1 = !!C; }

static INLINE void FPU_SET_C2(Bitu C) { fpu.sw.C2 = !!C; }

static INLINE void FPU_SET_C3(Bitu C) { fpu.sw.C3 = !!C; }

static INLINE void FPU_SET_D(Bitu C) { fpu.sw.DE = !!C; }
...
static INLINE void FPU_LOG_WARN(Bitu tree, bool ea, Bitu group, Bitu sub) {
  LOG(LOG_FPU, LOG_WARN)("ESC %lu%s:Unhandled group %lu subfunction %lu",
                         (long unsigned int)tree, ea ? " EA" : "",
                         (long unsigned int)group, (long unsigned int)sub);
}
