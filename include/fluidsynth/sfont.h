...
struct _fluid_sfloader_t {
  ...
  /**
   * Method to load an instrument file (does not actually need to be a real file
   * name, could be another type of string identifier that the \a loader
   * understands).
   * @param loader SoundFont loader
...
struct _fluid_sfont_t {
   ...
   * @param sfont Virtual SoundFont
   * @param preset Caller supplied preset to fill in with current preset
   * information
...
struct _fluid_preset_t {
   ...
   * should be as efficient as possible and not perform any operations
   * considered bad for realtime audio output (memory allocations and other OS
   * calls).
   *
   ...
   * #fluid_sample_t structure and returns a pointer to the opaque
   * #fluid_voice_t structure. To set or increment the values of a
   * generator, use fluid_voice_gen_set() or fluid_voice_gen_incr(). When you
   * are finished initializing the voice call fluid_voice_start() to start
   * playing the synthesis voice.  Starting with FluidSynth 1.1.0 all voices
   * created will be started at the same time.
   */
  int (*noteon)(fluid_preset_t *preset, fluid_synth_t *synth, int chan, int key,
                int vel);
...
struct _fluid_sample_t {
  ...
  unsigned int end;   /**< End index, index of last valid sample point (contrary
                         to SF spec) */
  unsigned int loopstart;  /**< Loop start index */
  unsigned int loopend;    /**< Loop end index, first point following the loop
  ...
  int sampletype; /**< Values: #FLUID_SAMPLETYPE_MONO, FLUID_SAMPLETYPE_RIGHT,
                     FLUID_SAMPLETYPE_LEFT, FLUID_SAMPLETYPE_ROM */
  int valid;   /**< Should be TRUE if sample data is valid, FALSE otherwise (in
                  which case it will not be synthesized) */
  short *data; /**< Pointer to the sample's data */

  int amplitude_that_reaches_noise_floor_is_valid; /**< Indicates if \a
                                                      amplitude_that_reaches_noise_floor
                                                      is valid (TRUE), set to
                                                      FALSE initially to
                                                      calculate. */
  double amplitude_that_reaches_noise_floor; /**< The amplitude at which the
                                                sample's loop will be below the
                                                noise floor.  For voice off
                                                optimization, calculated
                                                automatically. */

  unsigned int refcount; /**< Count of voices using this sample (use
                            #fluid_sample_refcount to access this field) */

  /**
   * Implement this function to receive notification when sample is no longer
   * used.
...
#define fluid_sample_refcount(_sample)                                         \
  ((_sample)->refcount) /**< Get the reference count of a sample.  Should only \
                           be called from within synthesis context (noteon     \
                           method for example) */

#define FLUID_SAMPLETYPE_MONO                                                  \
  1 /**< Flag for #fluid_sample_t \a sampletype field for mono samples */
#define FLUID_SAMPLETYPE_RIGHT                                                 \
  2 /**< Flag for #fluid_sample_t \a sampletype field for right samples of a   \
       stereo pair */
#define FLUID_SAMPLETYPE_LEFT                                                  \
  4 /**< Flag for #fluid_sample_t \a sampletype field for left samples of a    \
       stereo pair */
#define FLUID_SAMPLETYPE_LINKED                                                \
  8 /**< Flag for #fluid_sample_t \a sampletype field, not used currently */
#define FLUID_SAMPLETYPE_ROM                                                   \
  0x8000 /**< Flag for #fluid_sample_t \a sampletype field, ROM sample, causes \
