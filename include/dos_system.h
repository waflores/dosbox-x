...
public:
  ...
  virtual bool LockFile(uint8_t mode, uint32_t pos, uint16_t size) {
    (void)mode;
    (void)pos;
    (void)size;
...
public:
  ...
  uint16_t CallDeviceFunction(uint8_t command, uint8_t length, uint16_t seg,
...
public:
  ...
  void AddEntryDirOverlay(const char *path, char *sfile,
...
private:
  ...
  char *CreateEntry(CFileInfo *dir, const char *name, const char *sname,
                    bool is_directory, bool skipSort = false);
...
public:
  ...
  virtual bool FindFirst(const char *_dir, DOS_DTA &dta,
  ...
  virtual bool AllocationInfo(uint16_t *_bytes_sector,
                              uint8_t *_sectors_cluster,
                              uint16_t *_total_clusters,
                              uint16_t *_free_clusters) = 0;
  virtual bool AllocationInfo32(uint32_t *_bytes_sector,
                                uint32_t *_sectors_cluster,
                                uint32_t *_total_clusters,
                                uint32_t *_free_clusters) {
    (void)_bytes_sector;
    (void)_sectors_cluster;
    (void)_total_clusters;
    (void)_free_clusters;
    return false;
  }
  virtual bool AllocationInfo64(uint32_t *_bytes_sector,
                                uint32_t *_sectors_cluster,
  ...
  /* these 4 may only be used by DOS_Drive_Cache because they have special
   * calling conventions */
  ...
  virtual bool read_directory_first(void *handle, char *entry_name,
  ...
  virtual bool read_directory_next(void *handle, char *entry_name,
                                   char *entry_sname, bool &is_directory) {
    (void)handle;
    (void)entry_name;
    (void)entry_sname;
  ...
  virtual uint32_t GetSectorSize(void) {
    return 0;
  } // LOGICAL sector size (from the FAT driver) not PHYSICAL disk sector size
