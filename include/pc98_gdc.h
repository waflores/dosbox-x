...
#define PC98_GDC_FIFO_SIZE                                                     \
  32 /* taken from Neko Project II, but what is it really? */
#define GDC_COMMAND_BYTE 0x100

enum { GDC_MASTER = 0, GDC_SLAVE = 1 };
...
//				bit2	grcg bit6
//				bit3	grcg bit7
//				bit4	analog enable (16/256-color mode).
//8-color mode if not. 				bit5	pc9821 vga
...
struct PC98_GDC_state {
  ...
  void set_vectw(uint8_t ope, uint8_t dir, uint16_t dc, uint16_t d, uint16_t d2,
  ...
  enum { RT_MULBIT = 15, RT_TABLEBIT = 12, RT_TABLEMAX = (1 << RT_TABLEBIT) };
  struct VECTDIR {
    int16_t x;
    int16_t y;
    int16_t x2;
  ...
  uint16_t
      fifo[PC98_GDC_FIFO_SIZE]; /* NTS: Neko Project II uses one big FIFO for
                                   command and data, which makes sense to me */
