...
class Value {
  /*
   * Multitype storage container that is aware of the currently stored type in
   * it. Value st = "hello"; Value in = 1; st = 12 //Exception in = 12 //works
   */
private:
  Hex _hex;
  bool _bool = false;
...
public:
  ...
  Value(std::string const &in)
      : _string(new std::string(in)), type(V_STRING) {};
...
public:
  ...
  bool modified() const {
    ...
    if (default_value.ToString() == "") {
      if (propname == "sensitivity" && value.ToString() == "100")
        return false;
      if (propname == "pixelshader" && value.ToString() == "none")
        return false;
      if (propname == "priority" && value.ToString() == "higher,normal")
        return false;
      if (propname == "scaler" && value.ToString() == "normal2x")
        return false;
      if (propname == "monochrome_pal" && value.ToString() == "green")
        return false;
      if (propname == "cycles" && value.ToString() == "auto")
        return false;
      if ((propname == "serial1" || propname == "serial2") &&
          value.ToString() == "dummy")
        return false;
      if (propname.substr(0, 6) == "serial" && propname[6] >= '3' &&
          propname[6] <= '9' && value.ToString() == "disabled")
        return false;
    }
...
protected:
  // Set interval value to in or default if in is invalid. force always sets the
  // value. Can be overridden to set a different value if invalid.
...
public:
  ...
  Prop_int(std::string const &_propname, Changeable::Value when, int _min,
           int _max, int _value)
  ...
  // Override SetVal, so it takes min,max in account when there are no suggested
  // values
...
public:
  Prop_double(std::string const &_propname, Changeable::Value when,
              double _value)
      : Property(_propname, when), min(-1.0), max(-1.0) {
    default_value = value = _value;
  }
  Prop_double(std::string const &propname, Changeable::Value when,
...
public:
  Prop_string(std::string const &_propname, Changeable::Value when,
...
public:
  std::string realpath;
  Prop_path(std::string const &_propname, Changeable::Value when,
            char const *const _value)
...
void AddExitFunction(SectionFunction func, const char *name,
                     bool canchange = false);

/* for use with AddExitFunction and a name of a function.
 * this turns it into function pointer and function name. it turns one param
 * into two. */
#define AddExitFunctionFuncPair(x) &x, #x

/* array of list of functions to call for various virtual machine events */
enum vm_event {
  VM_EVENT_POWERON = 0, // emulation has started to power on hardware. it is
                        // safe to connect I/O, memory, IRQ resources, etc. to
                        // the bus. BIOS not initialized yet.
  VM_EVENT_RESET,       // reset signal (at the hardware level), whether by the
                        // keyboard controller, reset button, etc.
  VM_EVENT_RESET_END,   // reset signal switched off, permitting the system to
                        // begin booting.
  VM_EVENT_BIOS_INIT, // BIOS is going to reinitialize the system (after reset)
  VM_EVENT_BIOS_BOOT, // BIOS in the boot stage. usually leads to DOS kernel
                      // init or guest OS boot.

  VM_EVENT_GUEST_OS_BOOT =
      5, // BIOS or DOS kernel (BOOT command) is running a guest OS. just after
         // loading boot sector into memory but before executing it.
  VM_EVENT_DOS_BOOT, // emulation has decided to boot the built-in DOS kernel.
                     // just prior to starting the DOS kernel.
  VM_EVENT_DOS_INIT_KERNEL_READY,    // DOS kernel init. Prior to CONFIG.SYS
                                     // handling.
  VM_EVENT_DOS_INIT_CONFIG_SYS_DONE, // DOS kernel init. After CONFIG.SYS
                                     // handling, all devices inited.
  VM_EVENT_DOS_INIT_SHELL_READY,     // DOS kernel init. After COMMAND.COM
                                     // initialization, before AUTOEXEC.BAT
                                     // execution.

  VM_EVENT_DOS_INIT_AUTOEXEC_BAT_DONE =
      10, // DOS kernel init. COMMAND.COM just finished AUTOEXEC.BAT.
  VM_EVENT_DOS_INIT_AT_PROMPT, // DOS kernel init complete. After this event,
                               // the user is immediately given the DOS prompt.
  VM_EVENT_DOS_EXIT_BEGIN,     // DOS kernel is just starting to exit (user used
                               // BOOT command)
  VM_EVENT_DOS_EXIT_KERNEL,    // DOS kernel has just finished exiting
  VM_EVENT_DOS_EXIT_REBOOT_BEGIN, // DOS kernel is just starting to exit (hard
                                  // reset, outside of DOS's control)

  VM_EVENT_DOS_EXIT_REBOOT_KERNEL =
      15, // DOS kernel has just finished exiting (hard reset)
  VM_EVENT_DOS_SURPRISE_REBOOT, // DOS kernel asked to boot, when apparently
                                // having never been shut down (jmp to
                                // FFFF:0000)
...
public:
  ...
  void end_event() { event_in_progress = false; }
...
void AddVMEventFunction(enum vm_event event, SectionFunction func,
                        const char *name, bool canchange = false);
...
public:
  ...
  Prop_int *Add_int(std::string const &_propname,
                    Property::Changeable::Value when, int _value = 0);
  Prop_string *Add_string(std::string const &_propname,
                          Property::Changeable::Value when,
                          char const *const _value = NULL);
  Prop_path *Add_path(std::string const &_propname,
                      Property::Changeable::Value when,
                      char const *const _value = NULL);
  Prop_bool *Add_bool(std::string const &_propname,
                      Property::Changeable::Value when, bool _value = false);
  Prop_hex *Add_hex(std::string const &_propname,
                    Property::Changeable::Value when, Hex _value = 0);
  Prop_double *Add_double(std::string const &_propname,
                          Property::Changeable::Value when,
                          double _value = 0.0);
  Prop_multival *Add_multi(std::string const &_propname,
                           Property::Changeable::Value when,
                           std::string const &sep);
  Prop_multival_remain *Add_multiremain(std::string const &_propname,
                                        Property::Changeable::Value when,
                                        std::string const &sep);
  ...
  void PrintData(FILE *outfile, int everything = -1,
                 bool norem = false) override;
...
public:
  Prop_multival(std::string const &_propname, Changeable::Value when,
                std::string const &sep)
      : Property(_propname, when), separator(sep) {
    default_value = value = "";
  }
  ...
  bool SetValue(std::string const &input) override {
    return SetValue(input, /*init*/ false);
  };
  const std::vector<Value> &GetValues() const override;
  virtual ~Prop_multival() { delete section; }
}; // value bevat totale string. setvalue zet elk van de sub properties en
   // checked die.

class Prop_multival_remain : public Prop_multival {
public:
  Prop_multival_remain(std::string const &_propname, Changeable::Value when,
                       std::string const &sep)
...
public:
  ...
  virtual ~Module_base() { /*LOG_MSG("executed")*/
    ;
  }; // Destructors are required
