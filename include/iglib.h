 ...
 * In Watcom C we can do this by declaring all global variables __based on
 * _CODE. */
#ifdef TARGET_VXD
...
#define DOSBOX_ID_REG_DOSBOX_PLATFORM_TYPE                                     \
  (0x00000008UL) /* a code describing the platform, see src/ints/bios.cpp */
#define DOSBOX_ID_REG_DOSBOX_MACHINE_TYPE                                      \
  (0x00000009UL) /* machine type code (MCH_* constant), see include/dosbox.h   \
                  */

#define DOSBOX_ID_REG_DEBUG_OUT (0x0000DEB0UL)
#define DOSBOX_ID_REG_DEBUG_CLEAR (0x0000DEB1UL)

#define DOSBOX_ID_REG_USER_MOUSE_STATUS                                        \
  (0x00434D54UL) /* status (cursor capture, etc) */
#define DOSBOX_ID_REG_USER_MOUSE_STATUS_CAPTURE (0x00000001UL)

#define DOSBOX_ID_REG_USER_MOUSE_CURSOR                                        \
  (0x00434D55UL) /* screen coordinates, pixels */
#define DOSBOX_ID_REG_USER_MOUSE_CURSOR_NORMALIZED                             \
  (0x00434D56UL) /* screen coordinates, normalized to 0...65535 for            \
                    Windows 3.x */

#define DOSBOX_ID_REG_RELEASE_MOUSE_CAPTURE                                    \
  (0x0052434DUL) /* release mouse capture (W) / mouse capture status (R) */

#define DOSBOX_ID_REG_GET_VGA_SIZE (0x006845C0UL) /* size of the VGA screen */
#define DOSBOX_ID_REG_GET_WINDOW_SIZE                                          \
  (0x006845FFUL) /* size of the emulator window (to give USER_MOUSE_CURSOR     \
                    meaning) */
...
#define DOSBOX_ID_8042_KB_STATUS_LED_STATE_MASK                                \
  (0xFFUL << DOSBOX_ID_8042_KB_STATUS_LED_STATE_SHIFT)
#define DOSBOX_ID_8042_KB_STATUS_SCANSET_SHIFT (8UL)
#define DOSBOX_ID_8042_KB_STATUS_SCANSET_MASK                                  \
  (0x3UL << DOSBOX_ID_8042_KB_STATUS_SCANSET_SHIFT)
...
#define DOSBOX_ID_REG_CPU_CYCLES                                               \
  (0x55504300UL) /* fixed cycle count, which may vary if max or auto */
#define DOSBOX_ID_REG_CPU_MAX_PERCENT                                          \
  (0x55504301UL) /* for max cycles, percentage from 0 to 100 */
#define DOSBOX_ID_REG_CPU_CYCLES_INFO                                          \
  (0x55504302UL) /* info about cycles, including fixed, auto, max */
...
#define DOSBOX_ID_SCREENSHOT_IMAGE                                             \
  (1UL << 0UL) /* trigger a screenshot. wait for vertical retrace, then read   \
                  the register to check it happened */
#define DOSBOX_ID_SCREENSHOT_VIDEO                                             \
  (1UL << 1UL)                                 /* toggle on/off video capture */
#define DOSBOX_ID_SCREENSHOT_WAVE (1UL << 2UL) /* toggle on/off WAVE capture   \
                                                */
/* DOSBOX_ID_REG_SCREENSHOT_TRIGGER readback */
#define DOSBOX_ID_SCREENSHOT_STATUS_IMAGE_IN_PROGRESS                          \
  (1UL << 0UL) /* if set, DOSBox is prepared to write a screenshot on vertical \
                  retrace. will clear itself when it happens */
#define DOSBOX_ID_SCREENSHOT_STATUS_VIDEO_IN_PROGRESS                          \
  (1UL << 1UL) /* if set, DOSBox is capturing video. */
#define DOSBOX_ID_SCREENSHOT_STATUS_WAVE_IN_PROGRESS                           \
  (1UL << 2UL) /* if set, DOSBox is capturing WAVE audio */
#define DOSBOX_ID_SCREENSHOT_STATUS_NOT_ENABLED                                \
  (1UL << 30UL) /* if set, DOSBox has not enabled this register. */
#define DOSBOX_ID_SCREENSHOT_STATUS_NOT_AVAILABLE                              \
  (1UL << 31UL) /* if set, DOSBox was compiled without screenshot/video        \
                   support (C_SSHOT not defined) */
