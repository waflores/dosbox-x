...
#define GR_MIPMAPLEVELMASK_BOTH                                                \
  (GR_MIPMAPLEVELMASK_EVEN | GR_MIPMAPLEVELMASK_ODD)
...
#define GR_COMBINE_FUNCTION_BLEND                                              \
...
#define GR_COMBINE_FUNCTION_BLEND_LOCAL                                        \
...
#define GR_COMBINE_FACTOR_ONE_MINUS_DETAIL_FACTOR                              \
...
FX_ENTRY void FX_CALL grDrawPlanarPolygon(int nverts, const int ilist[],
                                          const GrVertex vlist[]);

FX_ENTRY void FX_CALL grDrawPlanarPolygonVertexList(int nverts,
                                                    const GrVertex vlist[]);

FX_ENTRY void FX_CALL grDrawPolygon(int nverts, const int ilist[],
                                    const GrVertex vlist[]);

FX_ENTRY void FX_CALL grDrawPolygonVertexList(int nverts,
                                              const GrVertex vlist[]);
...
FX_ENTRY FxBool FX_CALL grSstWinOpen(FxU hWnd,
...
FX_ENTRY FxBool FX_CALL guTexChangeAttributes(
    GrMipMapId_t mmid, int width, int height, GrTextureFormat_t fmt,
...
FX_ENTRY void FX_CALL guTexDownloadMipMapLevel(GrMipMapId_t mmid, GrLOD_t lod,
                                               const void **src);
...
#if defined(GLIDE3) && defined(GLIDE3_ALPHA)
FX_ENTRY FxBool FX_CALL grLfbWriteRegion(GrBuffer_t dst_buffer, FxU32 dst_x,
                                         FxU32 dst_y, GrLfbSrcFmt_t src_format,
                                         FxU32 src_width, FxU32 src_height,
                                         FxBool pixelPipeline, FxI32 src_stride,
                                         void *src_data);
#else
FX_ENTRY FxBool FX_CALL grLfbWriteRegion(GrBuffer_t dst_buffer, FxU32 dst_x,
                                         FxU32 dst_y, GrLfbSrcFmt_t src_format,
...
FX_ENTRY FxBool FX_CALL grLfbReadRegion(GrBuffer_t src_buffer, FxU32 src_x,
                                        FxU32 src_y, FxU32 src_width,
                                        FxU32 src_height, FxU32 dst_stride,
                                        void *dst_data);
