...
/* HostPt and ConstHostPt is for holding linear addresses within this emulator
 * i.e. a normal pointer.
 *
 * PhysPt is for 32-bit physical memory addresses within the emulation
 * environment. PhysPt64 is for 64-bit physical memory addresses for code and
 * device/memory emulation that supports addresses above 4GB. LinearPt is a
 * 32-bit linear memory address from the point of view of the CPU execution
 * context, meaning the linear address of the code prior to translation through
 * the page tables to physical addresses. RealPt is a 32-bit value that holds
 * segment in the upper 16 bits, offset in the lower 16 bits.
 *
 * Please do not mix these types up in the code, even if they happen to have the
 * same underlying data types */
...
typedef uint32_t
    RealPt; /* guest real-mode memory address (16:16 -> seg:offset) */
...
Bitu MEM_AllocatedPages(
    MemHandle handle); // amount of allocated pages of handle
...
static constexpr bool build_memlimit_32bit(void) { return sizeof(void *) < 8; }

/*
    The following six functions are used everywhere in the end so these should
   be changed for Working on big or little endian machines
*/

static INLINE uint8_t host_readb(ConstHostPt const off) { return *off; }
...
/* !defined(C_UNALIGNED_MEMORY) meaning: we're probably being compiled for a
   processor that doesn't like unaligned WORD access, on such processors
   typecasting memory as uint16_t and higher can cause a fault if the address is
   not aligned to that datatype when we read/write through it. */

static INLINE uint16_t host_readw(ConstHostPt const off) {
  return (uint16_t)host_readb(off) +
         ((uint16_t)host_readb(off + (uintptr_t)1U) << (uint16_t)8U);
}
static INLINE uint32_t host_readd(ConstHostPt const off) {
  return (uint32_t)host_readw(off) +
         ((uint32_t)host_readw(off + (uintptr_t)2U) << (uint32_t)16U);
}
static INLINE uint64_t host_readq(ConstHostPt const off) {
  return (uint64_t)host_readd(off) +
         ((uint64_t)host_readd(off + (uintptr_t)4U) << (uint64_t)32U);
}
...
static inline constexpr LinearPt PhysMake(const uint16_t seg,
                                          const uint16_t off) {
...
static inline constexpr RealPt RealMake(const uint16_t seg,
                                        const uint16_t off) {
  return (((RealPt)seg << 16U) + (RealPt)off);
}

/* convert physical address to 4:16 real pointer (example: 0xABCDE ->
 * 0xA000:0xBCDE) */
static inline constexpr RealPt PhysToReal416(const LinearPt phys) {
  return RealMake((uint16_t)((phys >> 4U) & 0xF000U),
...
static INLINE void real_writeb(const uint16_t seg, const uint16_t off,
                               const uint8_t val) {
  mem_writeb(PhysMake(seg, off), val);
}
static INLINE void real_writew(const uint16_t seg, const uint16_t off,
                               const uint16_t val) {
  mem_writew(PhysMake(seg, off), val);
}
static INLINE void real_writed(const uint16_t seg, const uint16_t off,
                               const uint32_t val) {
