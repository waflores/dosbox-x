...
enum SCREEN_TYPES {
  SCREEN_SURFACE,
  SCREEN_OPENGL // [FIXME] cannot make this conditional because somehow SDL2
                // code uses it while C_OPENGL is definitely disabled by C_SDL2
                // so SCREEN_OPENGL is unavailable
#if C_DIRECT3D
  ,
  SCREEN_DIRECT3D
#endif
...
public:
  struct wxh {
    ...
    void clear(void) { width = height = -1; }
  };
  struct xvy {
    double x = 0;
    double y = 0;
...
public:
  xvy screen_position_pixels; // position of the screen on the "virtual" overall
                              // desktop
