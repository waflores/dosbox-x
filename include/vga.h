...
struct vsync_state {
  ...
  bool persistent; // use persistent timer (to keep in sync even after internal
                   // mode switches)
...
#define S3_CLOCK(_M, _N, _R)                                                   \
  ((S3_CLOCK_REF * (((Bitu)_M) + 2ul)) /                                       \
   ((((Bitu)_N) + 2ul) * ((Bitu)1ul << ((Bitu)_R))))
...
typedef struct {
  ...
  /* Pixel Scrolling */
  uint8_t
      pel_panning; /* Amount of pixels to skip when starting horizontal line */
  uint8_t hlines_skip;
  uint8_t bytes_skip;
  uint8_t addr_shift;
...
typedef enum { DRAWLINE, EGALINE } Drawmode;

enum MonochromeColor { Green, Amber, Gray, White, First = Green, Last = White };
...
typedef struct {
  ...
  double screen_ratio;
  uint8_t font[516 * 1024]; /* enlarged to 516KB for PC-98 character font data
                               (256*16) + (128*2*128*16) */
...
typedef struct {
  ...
  struct {
    ...
    uint8_t psctl_psfc;  // [30:28] PSFC Primary Stream Filter Characteristics
                         // 0=RGB-8 CLUT or XRGB-32 (X.8.8.8)  3=KRGB-16
                         // (1.5.5.5)  5=RGB-16 (5.6.5)
    uint8_t psctl_psidf; // [26:24] PSIDF Primary Stream Input Data Format
                         // 0=Primary Stream  1=..for 2X stretch (replication)
                         // 2=..bilinear for 2X stretch (interpolation)
    ...
    uint8_t ckctl_kc;     // [28:28] Key Control
                          ...
                          // 1=Enable color or chroma keying for all modes other
                          // than KRGB-16

    // (MM8190 Secondary Stream Control)
    int16_t ssctl_dda_haccum; // [11: 0] DDA Horizontal Accumulator (signed 2's
                              // complement) Set to: 2*(W0-1) - (W1-1) [FIXME:
                              // Am I reading that right? Datasheet says 2
                              // (W0-1) - (W1-1) Windows 3.1 DCI seems to
                              // compute instead: ((2*(W0-1) - (W1-1)) / 2) W0 =
                              // line length in pixels before scaling W1 = line
                              // length in pixels after scaling
    uint8_t ssctl_sdif; // [26:24] SDIF Secondary Stream Input Data Format
                        // 1=YCbCr-16 (4.2.2), 16-240 input range (NTS: known as
                        // 'YUY2', 8-bit YUYV, for MPEG playback) 2=YUV-16
                        // (4.2.2), 0-255 input range (NTS: 8-bit YUYV but full
                        // range, perhaps appropriate for JPEG) 3=KRGB-16
                        // (1.5.5.5) 4=YUV (2.1.1) (NTS: I think this means
                        // planar YUV, U and V are one quarter resolution. Also
                        // known in the industry as 4:2:0) 5=RGB-16 (5.6.5)
                        // 7=XRGB-32 (X.8.8.8)
    uint8_t ssctl_sfc; // [30:28] SFC - Secondary Stream Filter Characteristics
                       // 0=Secondary Stream  1=..linear 0-2-4-2-0, for X
                       // stretch  2=..bilinear for 2X to 4X stretch 3=..linear
                       // 1-2-2-2-1, for 4X stretch
    ...
    uint16_t ssctl_k1_hscale; // [10: 0] K1 horizontal scale factor
                              // Set to: W0-1, where W0 is width of pixels of
                              // the initial output window before scaling
    int16_t ssctl_k2_hscale;  // [26:16] K2 horizontal scale factor
                              // Set to: W0-W1, where W1 is the width of pixels
                             // of the final scaled output window. "This value
                             // is signed and will always be negative" (FIXME:
                             // Does that imply the card cannot *downscale* YUV
                             // overlays?)
    ...
    uint8_t
        blendctl_composemode; //[26:24] compose mode
                              // 0=secondary stream opaque overlay on primary
                              // stream 1=primary stream opaque overlay on
                              // secondary stream 2=dissolve, [Pp x Kp + Ps x (8
                              // - Kp)]/8, ignore Ks 3=Fade, [Pp x Kp + Ps x
                              // Ks]/8, where Kp + Ks must be <= 8 5=Color key
                              // on primary stream (secondary stream overlay on
                              // primary stream) 6=Color or chroma key on
                              // secondary stream (primary stream overlay on
                              // secondary stream)
    ...
    uint8_t ps_bufsel; // [ 0: 0] Primary Stream Buffer Select (0=address 0
                       // 1=address 1)
    uint8_t ss_bufsel; // [ 2: 1] Secondary Stream Buffer Select
                       // 0=address 0
                       // 1=address 1
                       // 2=address (LPB input buffer select ^ 0) and opposite
                       // is for LPB 3=address (LPB input buffer select ^ 1) and
                       // opposite is for LPB
    ...
    uint8_t ooc_tss; // [30:30] Top Stream Select  0=Secondary on top  1=Primary
                     // on top
    uint8_t ooc_ooc_enable; // [31:31] Opaque Overlay Control Enable  0=disabled
                            // 1=enabled

    // (MM81E0 K1 Vertical Scale Factor)
    uint16_t k1_vscale_factor; // [10: 0] K1 Vertical Scale Factor
                               // Set to: [height in lines of initial output
                               // window before scaling] - 1

    // (MM81E4 K2 Vertical Scale Factor)
    int16_t k2_vscale_factor; // [10: 0] K2 Vertical Scale Factor
                              // Set to: (height in lines before scale) -
                              // (height in lines of final window after scaling)
    ...
    uint8_t
        fifo_alloc_ps; // Interpretation of [4:0], where 5 bits are number of
                       // slots allotted to secondary stream. N = secondary
                       // stream slots	 This value is set to 24 - N
    uint8_t fifo_alloc_ss;      // Interpretation of [4:0], set to N (up to 24)
    uint8_t fifo_ss_threshhold; // Threshold at which FIFO refill of secondary
                                // stream is triggered (low water point). Must
                                // be <= alloc_ss
    uint8_t fifo_ps_threshhold; // Threshold at which FIFO refill of primary
                                // stream is triggered (low water point). Must
                                // be <= alloc_ps
...
typedef struct {
  ...
  uint8_t disabled; // Used for disabling the screen.
                    ...
                    // These are put together in one variable for performance
                    // reasons: the line drawing function is called maybe
                    // 60*480=28800 times/s, and we only need to check one
                    // variable for zero this way.
} VGA_Attr;
...
typedef struct VGA_Memory_t {
  ...
  uint32_t memsize = 0;
  uint32_t memmask = 0;
  uint32_t memmask_crtc =
      0; // in CRTC-visible units (depends on byte/word/dword mode)
...
enum {
  VGACMPLX_MAP_MASK =
      (1u << 0u), // at least one bit is clear in the sequencer map mask
                  // register, problem for chained 256-color mode
  VGACMPLX_NON_EXTENDED =
      (1u
       << 1u), // the extended memory bit is cleared in the sequencer register
  VGACMPLX_ODDEVEN =
      (1u << 2u), // Odd/Even addressing mode set in sequencer register
  VGACMPLX_BITMASK = (1u << 3u), // Bit mask in graphics controller is not 0xFF
  VGACMPLX_COLORDONTCARE =
      (1u << 4u), // At least one bit set in the color don't care graphics
                  // controller register
  ...
  VGACMPLX_ROPROT = (1u << 7u), // Graphics controller raster op is nonzero or
                                // data rotate nonzero
  VGACMPLX_SETRESET =
      (1u << 8u) // Graphics controller set/reset enable is nonzero
};
 ...
 * your C++ compiler should be smart enough to inline these into the body of
 * this function. */
 ...
 *
 *       Each bitplane will be 64KB (0x10000) bytes long, and the page flip bit
 * in 0xA6 will select which 32KB half within the 64KB block to use, or if
 * another bit is set as documented in Undocumented PC-98, the full 64KB block.
 *
 *       The 512KB + 32KB will be reduced slightly to 512KB + 16KB to match the
 * layout.
 *
 *       The bitplane layout change will permit emulating an 8 bitplane
 * 256-color mode suggested by Yksoft1 that early PC-9821 systems supported and
 * that the 256-color driver shipped with Windows 3.1 (for PC-98) uses. Based on
 * Windows 3.1 behavior that also means the linear framebuffer at 0xF00000 must
 * also change in planar mode to spread all 8 bits across the planes on write
 * and gather all 8 bits on read. As far as I can tell the Windows 3.1 256-color
 * driver uses planar and EGC functions as it would in 16-color mode, but draws
 * bitmaps using the LFB. The picture is wrong EXCEPT when Windows icons and
 * bitmaps are drawn.
 *
 *       256-color packed mode will be retained as direct LFB mapping from the
 * start of graphics RAM.
 *
 *       On a real PC-9821 laptop, contents accessible to the CPU noticeably
 * shift order and position when you switch on/off 256-color packed mode,
 * suggesting that the planar mode is simply reordered memory access in hardware
 * OR that 256-color mode is "chained" (much like 256-color packed mode on IBM
 * VGA hardware) across bitplanes. */

#define PC98_VRAM_TEXT_OFFSET                                                  \
  (0x00000u) /* 16KB memory (8KB text + 8KB attributes) */
...
#define PC98_VRAM_PAGEFLIP_SIZE                                                \
  (0x08000u) /* add this amount for the second page in 8/16/256-color planar   \
                mode */
#define PC98_VRAM_PAGEFLIP256_SIZE                                             \
  (0x40000u) /* add this amount for the second page in 256-color packed mode   \
              */

#define PC98_VRAM_256BANK_SIZE                                                 \
  (0x08000u) /* window/bank size (256-color packed) */
...
pc98_pgram_bitplane_offset(const unsigned int b) {
  /* WARNING: b is not range checked for performance! Do not call with b >= 8 if
   * memsize = 512KB or b >= 4 if memsize >= 256KB */
  return (b * PC98_VRAM_BITPLANE_SIZE);
}

static inline unsigned char *
pc98_vram_256bank_from_window(const unsigned int b) {
