...
public:
  ...
  inline size_t
  get_dest_last_written(void) const { /* in units of sizeof(dstT) */
    return dst_adv;
  }
...
class _IconvBase : public _Iconv_CommonBase {
public:
...
public:
  ...
  void set_dest(dstT *const dst,
                const size_t len /*in units of sizeof(dstT)*/) {
    set_dest(dst, dst + len);
  }
  void set_dest(dstT *const dst) =
      delete; /* <- NO! Prevent C-string calls to std::string &dst function! */
...
public:
  virtual int _do_convert(void) { return err_noinit; }
  int string_convert(dst_string &dst, const src_string &src) {
    dst.resize(
        std::max(dst.size(), ((src.length() + 4u) * 4u) +
                                 2u)); // maximum 4 bytes/char expansion UTF-8
                                       // or bigger if caller resized already
...
public:
  inline bool eof(void) const { return src_ptr >= src_ptr_fence; }
  inline bool eof_dest(void) const { return dst_ptr >= dst_ptr_fence; }
  inline const srcT *get_srcp(void) const { return src_ptr; }
  inline const dstT *get_destp(void) const { return dst_ptr; }

protected:
  static inline size_t my_strlen(const char *s) { return strlen(s); }
  static inline size_t my_strlen(const wchar_t *s) { return wcslen(s); }
...
protected:
  void set_dest(dst_string &dst) { /* PRIVATE: External use can easily cause
                                      use-after-free bugs */
    set_dest(&dst[0], dst.size());
  }
  void set_src(const src_string &src) { /* PRIVATE: External use can easily
...
public:
  int _do_convert(void) override {
    if (context != NULL) {
      ...
      size_t src_left = (size_t)((uintptr_t)((char *)pclass::src_ptr_fence) -
                                 (uintptr_t)((char *)pclass::src_ptr));
      size_t dst_left = (size_t)((uintptr_t)((char *)pclass::dst_ptr_fence) -
                                 (uintptr_t)((char *)pclass::dst_ptr));

      iconv(context, NULL, NULL, NULL, NULL);

      /* Ref: [http://man7.org/linux/man-pages/man3/iconv.3.html] */
      int ret = iconv(context, (ICONV_IN_TYPE)(&(pclass::src_ptr)), &src_left,
                      (char **)(&(pclass::dst_ptr)), &dst_left);
...
public:
  static _Iconv<srcT, dstT> *
  create(const char *nw) { /* factory function, wide to char, or char to wide */
    if (sizeof(dstT) == sizeof(char) && sizeof(srcT) > sizeof(char)) {
      const char *wchar_encoding = _get_wchar_encoding<srcT>();
      if (wchar_encoding == NULL)
        return NULL;

      iconv_t ctx = iconv_open(
          /*TO*/ nw, /*FROM*/ wchar_encoding); /* from wchar to codepage nw */
      if (ctx != iconv_t(-1))
        return new (std::nothrow) _Iconv<srcT, dstT>(ctx);
    } else if (sizeof(dstT) > sizeof(char) && sizeof(srcT) == sizeof(char)) {
      const char *wchar_encoding = _get_wchar_encoding<dstT>();
      if (wchar_encoding == NULL)
        return NULL;

      iconv_t ctx = iconv_open(/*TO*/ wchar_encoding,
                               /*FROM*/ nw); /* from codepage new to wchar */
      if (ctx != iconv_t(-1))
        return new (std::nothrow) _Iconv<srcT, dstT>(ctx);
    }
  ...
  static _Iconv<srcT, dstT> *create(const char *to,
                                    const char *from) { /* factory function */
    if (sizeof(dstT) == sizeof(char) && sizeof(srcT) == sizeof(char)) {
...
/* Most of the time the Iconv form will be used, for Mac OS X and Linux
 * platforms where UTF-8 is common.
 *
 * Conversion to/from wchar is intended for platforms like Microsoft Windows
 * 98/ME/2000/XP/Vista/7/8/10/etc where the Win32 API functions take WCHAR
 * (UTF-16 or UCS-16), in which case, the code will continue to use UTF-8
 * internally but convert to WCHAR when needed. For example, Win32 function
 * CreateFileW().
 *
 * Note that because of the UTF-16 world of Windows, Microsoft C++ defines
 * wchar_t as an unsigned 16-bit integer.
 *
 * Linux and other OSes however define wchar_t as a 32-bit integer, but do not
 * use wchar_t APIs, and often instead use UTF-8 for unicode, so the wchar_t
 * versions will not see much use there. */
...
/* Alternative implementation (char to WCHAR, or WCHAR to char only) using
 * Microsoft Win32 APIs instead of libiconv. For use with embedded or low memory
 * Windows installations or environments where the added load of libiconv would
 * be undesirable. */

/* _IconvWin32 implementation of _IconvBase using Microsoft Win32 code page and
 * WCHAR support functions for Windows 2000/XP/Vista/7/8/10/etc */
template <typename srcT, typename dstT>
class _IconvWin32 : public _IconvBase<srcT, dstT> {
protected:
  using pclass = _IconvBase<srcT, dstT>;
    ...
    if (codepage != 0u) {
      ...
      if (sizeof(dstT) == sizeof(char) && sizeof(srcT) == sizeof(WCHAR)) {
        ...
        ret = WideCharToMultiByte(
            codepage, 0, (WCHAR *)pclass::src_ptr, src_left / sizeof(srcT),
            (char *)pclass::dst_ptr, dst_left, NULL, NULL);
        pclass::src_adv = src_left;
        pclass::src_ptr += pclass::src_adv;
        pclass::dst_adv = ret;
        pclass::dst_ptr += pclass::dst_adv;
      } else if (sizeof(dstT) == sizeof(WCHAR) &&
                 sizeof(srcT) == sizeof(char)) {
         ...
         */
        ret = MultiByteToWideChar(codepage, 0, (char *)pclass::src_ptr,
                                  src_left, (WCHAR *)pclass::dst_ptr,
                                  dst_left / sizeof(dstT));
        pclass::src_adv = src_left;
        pclass::src_ptr += pclass::src_adv;
        pclass::dst_adv = ret;
        pclass::dst_ptr += pclass::dst_adv;
...
public:
  static _IconvWin32<srcT, dstT> *
  create(const UINT
             codepage) { /* factory function, WCHAR to char or char to WCHAR */
    CPINFO cpi;
