...
struct MENU_Block {
  ...
  bool hidecycles =
      false;           // toggle cycles, fps, cpu usage information on title bar
  bool showrt = false; // show realtime percentage
  bool boot = false;   // verify if boot is being used (if enabled, it is unable
                       // to mount drives)
  bool gui = true; // enable or disable gui system (if disabled, it is unable to
                   // use/toggle menu bar)
  bool resizeusing = false; // check if resizable window can be used
  bool compatible = false;  // compatible mode for win9x/2000 (if enabled, GUI
