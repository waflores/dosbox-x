...
#else
/* As long as FILE_OFFSET_BITS==64 Linux is quite good at allowing stat to work
 * with 64-bit sizes */
...
// ARMv7 (Raspberry Pi) does not have long double, sizeof(long double) ==
// sizeof(double) ARM 64 has a quadruple-precision float instead of the 80-bit
// extended precision one used by x87
#else
...
enum HerculesCard { HERC_GraphicsCard, HERC_GraphicsCardPlus, HERC_InColor };
...
enum S3Card {
  ...
  S3_Vision868, // Vision868
                // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Vision868%20Multimedia%20Accelerator%20%281995%2d04%29%2epdf]
  S3_Vision964, // Vision964
                // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Vision964%20Graphics%20Accelerator%20%281995%2d01%29%2epdf]
  S3_Vision968, // Vision968
                // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Vision968%20Multimedia%20Accelerator%20%281995%2d04%29%2epdf]
  S3_Trio32,    // Trio32
  ...
  S3_Trio64, // Trio64
             // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Trio32%e2%88%95Trio64%20Integrated%20Graphics%20Accelerators%20%281995%2d03%29%2epdf]
  S3_Trio64V, // Trio64V+
              // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Trio64V%2b%20Integrated%20Graphics%20and%20Video%20Accelerator%20%281995%2d07%29%2epdf]
  ...
  S3_ViRGEVX // ViRGE VX
             // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20ViRGE%E2%88%95VX%20Integrated%203D%20Accelerator%20(1996-06).pdf]
};
...
void MSG_Add(const char *,
             const char *); // Add messages to the internal languagefile
const char *
MSG_Get(char const *); // Get messages from the internal languagefile
std::string formatString(const char *format,
                         ...); // Generates a formatted string using a format
...
#define TANDY_ARCH_CASE                                                        \
  MCH_TANDY:                                                                   \
  case MCH_PCJR
#define EGAVGA_ARCH_CASE                                                       \
  MCH_EGA:                                                                     \
  case MCH_VGA
...
/* for DOS filename handling we want a toupper that uses the MS-DOS code page
 * within not the locale of the host */
int ascii_toupper(int c);

enum { BOOTHAX_NONE = 0, BOOTHAX_MSDOS };
...
static inline constexpr bytecount_t operator"" _parabytes(
    const bytecount_t x) { /* AKA bytes per segment increment in real mode */
...
static inline constexpr bytecount_t operator"" _pagebytes(
    const bytecount_t x) { /* bytes per 4KB page in protected mode */
...
static inline constexpr bytecount_t _bytes(const bytecount_t x) { return x; }

static inline constexpr bytecount_t _parabytes(
    const bytecount_t x) { /* AKA bytes per segment increment in real mode */
  return x << bytecount_t(4u);
}
...
static inline constexpr bytecount_t
_pagebytes(const bytecount_t x) { /* bytes per 4KB page in protected mode */
  return x << bytecount_t(12u);
}
...
enum { PREVCAP_NONE = 0, PREVCAP_BLANK, PREVCAP_INVISIBLE };
...
public:
  ...
  static const size_t MAX_PAGE = 10,
                      SLOT_COUNT = 10; // slot: [0,...,SLOT_COUNT - 1]
  ...
  void registerComponent(const std::string &uniqueName,
                         Component &comp); // comp must have global lifetime!
...
template <class T> void writePOD(std::ostream &stream, const T &data);

template <class T> void readPOD(std::istream &stream, T &data);
...
protected:
  void getBytes(std::ostream &stream) override {
    std::for_each(podRef.begin(), podRef.end(),
                  std::bind1st(WriteGlobalPOD(), &stream));
  }

  void setBytes(std::istream &stream) override {
    std::for_each(podRef.begin(), podRef.end(),
                  std::bind1st(ReadGlobalPOD(), &stream));
  }

private:
  struct POD {
    template <class T> POD(T &pod) : address(&pod), size(sizeof(T)) {}
    void *address;
    size_t size;
  };

  struct WriteGlobalPOD
      : public std::binary_function<std::ostream *, POD, void> {
  ...
  struct ReadGlobalPOD
      : public std::binary_function<std::istream *, POD, void> {
...
template <class T> inline void writePOD(std::ostream &stream, const T &data) {
  stream.write(reinterpret_cast<const char *>(&data), sizeof(T));
}

template <class T> inline void readPOD(std::istream &stream, T &data) {
  stream.read(reinterpret_cast<char *>(&data), sizeof(T));
}

inline void writeString(std::ostream &stream, const std::string &data) {
...
inline void readString(std::istream &stream, std::string &data) {
  size_t stringSize = 0;
