...
typedef PageHandler *(MEM_CalloutHandler)(MEM_CalloutObject & co,
                                          Bitu phys_page);
...
static const Bitu MEMMASK_ISA_20BIT =
    0x000000FFU; /* ISA 20-bit decode (20 - 12 = 8) */
static const Bitu MEMMASK_ISA_24BIT =
    0x00000FFFU; /* ISA 24-bit decode (24 - 12 = 12) */
static const Bitu MEMMASK_FULL =
    0x000FFFFFU; /* full 32-bit decode (32 - 12 = 20) */
 ...
 */
static inline Bitu MEMMASK_Range(const Bitu x) { return ~(x - 1); }

/* combine range mask with MEMMASK value.
 */
static inline Bitu MEMMASK_Combine(const Bitu a, const Bitu b) { return a & b; }
...
/* NTS: Despite that all devices respond to these page handlers, all addresses
 * are still linear addresses. Devices that need physical memory addresses will
 * need to call PAGING_GetPhysicalAddress() to translate linear to physical, or
 * PAGING_GetPhysicalAddress64() if the device supports responding to addresses
 *      above 4GB. This sounds crazy, but this is one aspect of emulation
 * inherited from DOSBox SVN and I don't think it's wise to break code to change
 * it.  */
class PageHandler {
public:
  ...
  void setFlags(const Bitu flagsNew) { flags = flagsNew; }
...
/* NTS: To explain the Install() method, the caller not only provides the
 * IOMASK_.. value, but ANDs the least significant bits to define the range of
 * I/O ports to respond to. An ISA Sound Blaster for example would set portmask
 * = (IOMASK_ISA_10BIT & (~0xF)) in order to respond to 220h-22Fh, 240h-24Fh,
 * etc. At I/O callout time, the callout object is tested if (cpu_ioport &
 * io_mask) == (m_port & io_mask)
 *
 *      This does not prevent emulation of devices that start on non-aligned
 * ports or strange port ranges, because the callout handler is free to decline
 * the I/O request, leading the callout process to move on to the next device or
 * mark the I/O port as empty. */
class MEM_CalloutObject {
public:
  MEM_CalloutObject() {};
  void InvalidateCachedHandlers(void);
  void Install(Bitu page, Bitu pagemask /*MEMMASK_ISA_24BIT, etc.*/,
...
public:
  inline bool MatchPage(const Bitu p) {
     ...
     * apparently modern x86 processors are faster at addition/subtraction than
     * bitmasking. for this to work, m_port must be a multiple of the I/O range.
     * For example, if the I/O range is 16 ports, then m_port must be a multiple
     * of 16. */
    return ((p - m_base) & mem_mask) == 0;
  }
  inline bool isInstalled(void) { return installed; }
};
...
static inline uint32_t MEM_Callout_t_comb(const enum MEM_Type_t t,
                                          const uint32_t idx) {
...
static constexpr tlbentry_t PHYSPAGE_ACCESS_BITS =
    bitop::bitcount2masklsb<2u, PHYSPAGE_ACCESS_BITS_SHIFT, tlbentry_t>();
/* bit 29: dirty bit */
static constexpr tlbentry_t PHYSPAGE_DIRTY_SHIFT = tlbentry_bits - 3u;
static constexpr tlbentry_t PHYSPAGE_DIRTY =
    bitop::bit2mask<PHYSPAGE_DIRTY_SHIFT, tlbentry_t>();
/* bits 28: not defined */
/* bits 27-0: physical page. 40-bit addresses, that's as far as we can go, and
 * the limits of PSE anyway */
static constexpr tlbentry_t PHYSPAGE_ADDR =
    build_memlimit_32bit() ? bitop::bitcount2masklsb<20, 0, tlbentry_t>()
                           : /* 32-bit builds limited to 1GB no point emulating
                                past 4GB (20+12) = 32 */
        bitop::bitcount2masklsb<28, 0,
                                tlbentry_t>(); /* 64-bit builds allowed up to
                                                  1TB (28+12) = 40 */

struct X86_PageEntryBlock { // Page Table Entry, though it keeps the
                            // PageEntryBlock name to avoid breaking all this
                            // code
...
struct X86_PageDirEntryBlock {
  ...
  uint32_t pcd : 1;  // [ 4: 4]
  uint32_t a : 1;    // [ 5: 5]
  uint32_t avl6 : 1; // [ 6: 6] See Note [*1], this bit changed meaning between
                     // the 486 and Pentium!
  uint32_t ps : 1;   // [ 7: 7] Page Size Extension (Pentium), even though Intel
                     // didn't document it until later
  uint32_t g : 1;    // [ 8: 8]
  uint32_t avl : 3;  // [11: 9]
  uint32_t base : 20; // [31:12]
#endif
} GCC_ATTRIBUTE(packed);
/* Note [*1]: The i386 defined just the Page Table Entry to represent the PDE
 * and PTE, because they had the same layout.
 *
 *            The i486 lists the PDE and PTE, even though they have the same bit
 * layout.
 *
 *            The Pentium however, seems to have decided that bit 6 of the PDE
 * is no longer the "dirty" bit and is redefined as AVL. Although, if PSE is
 * enabled and the PDE is the 4MB format, bit 6 is once again a "dirty" but.
 *
 *            Ref:
 * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/CPU/80386/Intel/386DX%20Microprocessor%20Programmer%27s%20Reference%20Manual%20%281990%29%2epdf]
 *            Ref:
 * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/CPU/80486/Intel/i486%20Microprocessor%20%281989%2d04%29%2epdf]
 *            Ref:
 * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/CPU/Pentium/Pentium%20Processor%20Family%20Developer%27s%20Manual%20%2d%20Volume%203%3a%20Architecture%20and%20Programming%20Manual%20%281995%2d07%29%2epdf]
 */
...
union X86PageEntry {
  ...
  template <const uint8_t bitpos /*for constant compile time optimization*/>
  inline uint8_t accbits(void) const {
    if (bitpos == 0)
      return (load >> 1u) & 3u; /* ex: (load >> 1u) & 0x3 -> U/S:R/W->[1:0]*/
    else                        /* usually 2 */
      return (load << (bitpos - 1u)) &
             (3u << bitpos); /* ex: (load << 1u) & 0xc -> U/S:R/W->[3:2]*/
  }
};
...
struct PagingBlock {
  ...
  uint32_t firstmb[LINK_START]; /* does not use flags and does not reach beyond
                                   1MB, does not need more than 32 bits */
  bool enabled;
  bool wp;
};
 ...
 * functions */
/* NTS: 12-bit shift 32-bit constant, upper bits get shifted out, therefore no
 * need to bitmask */
...
static INLINE PhysPt64 PAGING_GetPhysicalAddress64(const LinearPt linAddr) {
  return ((PhysPt64)(paging.tlb.phys_page[linAddr >> 12] & PHYSPAGE_ADDR)
          << (PhysPt64)12) |
         (linAddr & 0xfff);
}
...
static INLINE uint8_t mem_readb_inline(const LinearPt address) {
  ...
  if (tlb_addr)
    ...
    return (uint8_t)(get_tlb_readhandler(address))->readb(address);
}
...
static INLINE uint16_t mem_readw_inline(const LinearPt address) {
  if ((address & 0xfff) < 0xfff) {
    ...
    if (tlb_addr)
      ...
      return (uint16_t)(get_tlb_readhandler(address))->readw(address);
  } else
...
static INLINE uint32_t mem_readd_inline(const LinearPt address) {
  if ((address & 0xfff) < 0xffd) {
    ...
    if (tlb_addr)
      ...
      return (uint32_t)(get_tlb_readhandler(address))->readd(address);
  } else
    return mem_unalignedreadd(address);
}

static INLINE void mem_writeb_inline(const LinearPt address,
                                     const uint8_t val) {
...
static INLINE void mem_writew_inline(const LinearPt address,
                                     const uint16_t val) {
...
static INLINE void mem_writed_inline(const LinearPt address,
                                     const uint32_t val) {
...
static INLINE bool mem_readb_checked(const LinearPt address,
                                     uint8_t *const val) {
  const HostPt tlb_addr = get_tlb_read(address);
  if (tlb_addr) {
...
static INLINE bool mem_readw_checked(const LinearPt address,
                                     uint16_t *const val) {
  if ((address & 0xfffu) < 0xfffu) {
    const HostPt tlb_addr = get_tlb_read(address);
    if (tlb_addr) {
...
static INLINE bool mem_readd_checked(const LinearPt address,
                                     uint32_t *const val) {
  if ((address & 0xfffu) < 0xffdu) {
    const HostPt tlb_addr = get_tlb_read(address);
    if (tlb_addr) {
...
static INLINE bool mem_writeb_checked(const LinearPt address,
                                      const uint8_t val) {
  const HostPt tlb_addr = get_tlb_write(address);
  if (tlb_addr) {
    host_writeb(tlb_addr + address, val);
...
static INLINE bool mem_writew_checked(const LinearPt address,
                                      const uint16_t val) {
  if ((address & 0xfffu) < 0xfffu) {
    const HostPt tlb_addr = get_tlb_write(address);
    if (tlb_addr) {
...
static INLINE bool mem_writed_checked(const LinearPt address,
                                      const uint32_t val) {
  if ((address & 0xfffu) < 0xffdu) {
    const HostPt tlb_addr = get_tlb_write(address);
    if (tlb_addr) {
...
extern bool
    dosbox_allow_nonrecursive_page_fault; /* when set, do nonrecursive mode
...
public:
  ...
  GuestPageFaultException(LinearPt n_lin_addr, Bitu n_page_addr,
                          Bitu n_faultcode)
      : lin_addr(n_lin_addr), page_addr(n_page_addr), faultcode(n_faultcode) {}
