...
public:
  ...
  virtual uint8_t Read_Sector(uint32_t head, uint32_t cylinder, uint32_t sector,
                              void *data, unsigned int req_sector_size = 0);
  virtual uint8_t Write_Sector(uint32_t head, uint32_t cylinder,
                               uint32_t sector, const void *data,
                               unsigned int req_sector_size = 0);
  ...
  virtual void Set_Geometry(uint32_t setHeads, uint32_t setCyl,
                            uint32_t setSect, uint32_t setSectSize);
  virtual void Get_Geometry(uint32_t *getHeads, uint32_t *getCyl,
                            uint32_t *getSect, uint32_t *getSectSize);
  virtual uint8_t GetBiosType(void);
  virtual uint32_t getSectSize(void);
  imageDisk(class DOS_Drive *useDrive, unsigned int letter, uint32_t freeMB,
            int timeout);
  imageDisk(FILE *imgFile, const char *imgName, uint32_t imgSizeK,
            bool isHardDisk);
  imageDisk(FILE *diskimg, const char *diskName, uint32_t cylinders,
  ...
  /* this is intended only for when the disk can change out from under us while
   * mounted */
  virtual bool detectDiskChange(void) {
    const bool r = diskChangeFlag;
...
private:
  ...
  std::vector<bool> partition_in_use; /* used by FAT driver to prevent mounting
                                         a partition twice */
  ...
  bool partitionMarkUse(const size_t i, bool inUse) {
    if (i < 256) {
      if (partition_in_use.size() != 256) {
        partition_in_use.resize(
            256); // keep it simple, this is a bitfield, 256 bits = 32 bytes
        for (size_t i = 0; i < partition_in_use.size(); i++)
          partition_in_use[i] = false;
      }
...
public:
  uint8_t Read_Sector(uint32_t head, uint32_t cylinder, uint32_t sector,
                      void *data, unsigned int req_sector_size = 0) override;
  uint8_t Write_Sector(uint32_t head, uint32_t cylinder, uint32_t sector,
                       const void *data,
                       unsigned int req_sector_size = 0) override;
...
public:
  ...
  imageDiskD88(FILE *imgFile, const char *imgName, uint32_t imgSizeK,
               bool isHardDisk);
  ...
  struct vfdentry {
    ...
    uint32_t entry_offset =
        0; // offset of the 12-byte entry this came from (if nonzero)
  ...
  vfdentry *findSector(uint8_t head, uint8_t track,
...
public:
  ...
  imageDiskNFD(FILE *imgFile, const char *imgName, uint32_t imgSizeK,
               bool isHardDisk, unsigned int revision);
...
public:
  ...
  imageDiskVFD(FILE *imgFile, const char *imgName, uint32_t imgSizeK,
               bool isHardDisk);
  ...
  struct vfdentry {
    ...
    uint16_t getSectorSize(void) const { return 128 << sizebyte; }
  };
...
public:
  ...
  void Set_Geometry(uint32_t setHeads, uint32_t setCyl, uint32_t setSect,
                    uint32_t setSectSize) override;
  ...
  imageDiskMemory(uint16_t cylinders, uint16_t heads, uint16_t sectors,
...
private:
  ...
  bool CalculateFAT(uint32_t partitionStartingSector, uint32_t partitionLength,
                    bool isHardDrive, uint32_t rootEntries,
                    uint32_t *rootSectors, uint32_t *sectorsPerCluster,
                    bool *isFat16, uint32_t *fatSectors,
...
public:
  ...
  uint8_t Read_AbsoluteSector(uint32_t sectnum, void *data) override;
  uint8_t Write_AbsoluteSector(uint32_t sectnum, const void *data) override;
  static ErrorCodes Open(const char *fileName, const bool readOnly,
                         imageDisk **disk);
  ...
  static uint32_t CreateDifferencing(const char *filename,
                                     const char *basename);
  ...
  bool MergeSnapshot(uint32_t *totalSectorsMerged,
...
private:
  ...
  static ErrorCodes TryOpenParent(const char *childFileName,
                                  const ParentLocatorEntry &entry,
                                  const uint8_t *data,
                                  const uint32_t dataLength, imageDisk **disk,
                                  const uint8_t *uniqueId);
  static ErrorCodes Open(const char *fileName, const bool readOnly,
                         imageDisk **disk, const uint8_t *matchUniqueId);
  virtual bool loadBlock(const uint32_t blockNumber);
  static bool convert_UTF16_for_fopen(std::string &string, const void *data,
...
public:
  ...
  uint8_t Read_AbsoluteSector(uint32_t sectnum, void *data) override {
    ...
    if (!GetMSCDEXDrive(CDROM_drive - 'A', &src_drive))
      return 0x05;

    if (!src_drive->ReadSectorsHost(
            buffer, false,
            cdrom_sector_offset +
                (sectnum >>
                 2) /*512 byte/sector to 2048 byte/sector conversion*/,
            1))
      return 0x05;
  ...
  imageDiskElToritoFloppy(unsigned char new_CDROM_drive,
                          unsigned long new_cdrom_sector_offset,
                          unsigned char floppy_emu_type)
      : imageDisk((FILE *)NULL, NULL, 0, false), CDROM_drive(new_CDROM_drive),
...
struct _PC98RawPartition {
  uint8_t mid;        /* 0x00: unused, 0x20: MS-DOS(not bootable), 0xa1-0xaf:
                         MS-DOS(bootable) */
  uint8_t sid;        /* 0x00: unused, 0x81,0x91,0xa1,0xe1: MS-DOS(active),
                         0x01,0x11,0x21,0x61: MS-DOS(sleep) */
...
//   in the BIOS first (the imageDiskList array), so the IDE controller can
//   obtain a reference to the drive in the imageDiskList array
...
extern uint16_t imgDTASeg; /* Real memory location of temporary DTA pointer for
                              fat image disk access */
extern RealPt imgDTAPtr;   /* Real memory location of temporary DTA pointer for
                              fat image disk access */
...
public:
  ...
  void Set_Geometry(uint32_t setHeads, uint32_t setCyl, uint32_t setSect,
                    uint32_t setSectSize) override;
  void Get_Geometry(uint32_t *getHeads, uint32_t *getCyl, uint32_t *getSect,
                    uint32_t *getSectSize) override;
  uint8_t GetBiosType(void) override;
