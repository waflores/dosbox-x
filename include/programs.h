...
public:
  enum opt_style {
    ...
    gnu_getopt, // GNU style --long or -a -b -c -d or -abcd (short as single
                // char), switch parsing stops at --
...
public:
  CommandLine(int argc, char const *const argv[],
              enum opt_style opt = CommandLine::either);
  CommandLine(char const *const name, char const *const cmdline,
              enum opt_style opt = CommandLine::either, bool squote = false);
  const char *GetFileName() { return file_name.c_str(); }

  bool FindExist(char const *const name, bool remove = false);
  bool FindHex(char const *const name, unsigned int &value,
               bool remove = false);
  bool FindInt(char const *const name, int &value, bool remove = false);
  bool FindString(char const *const name, std::string &value,
                  bool remove = false);
  bool FindCommand(unsigned int which, std::string &value);
  bool FindStringBegin(char const *const begin, std::string &value,
                       bool remove = false);
  bool FindStringFullBegin(char const *const begin, std::string &value,
                           bool squote, bool remove = false);
...
private:
  ...
  std::string opt_gnu_getopt_singlechar; /* non-empty if we hit GNU options like
                                            -abcd => -a -b -c -d */
  cmd_it opt_scan;
 ...
 *
 *  \description    This provides the base class for built-in programs
 * registered on drive Z:. In most cases, the class will override just the Run()
 * method.
 */
class Program {
public:
  ...
  DOS_PSP *psp; //! DOS kernel Program Segment Prefix associated with this
                //! program at runtime
  virtual void Run(void) = 0; //! Run() method, called when the program is run.
                              //! Subclass must override this
  bool
  GetEnvStr(const char *entry,
            std::string &result); //! Return an environment variable by name
  bool
  GetEnvNum(Bitu want_num,
            std::string &result); //! Return an environment variable by index
  Bitu GetEnvCount(void); //! Return the number of environmental variables
  bool SetEnv(const char *entry,
              const char *new_string); //! Set environment variable
  ...
  virtual int WriteOut_NoParsing(
      const char *format,
      bool dbcs = false); //! Write to standard output, no parsing
  void ChangeToLongCmd(); //! Get command line from shell instead of PSP
  void DebugDumpEnv();    //! Dump environment block to log
  void WriteExitStatus(); //! Write exit status to CPU register AL for return to
                          //! MS-DOS
};
