...
#define VERIFY_SGET_ARGS(n, s, f) assert_macro_args_ok<n, s, decltype(s::f)>()
...
#define IS_DOS_JAPANESE                                                        \
  (!dos_kernel_disabled &&                                                     \
   mem_readb(Real2Phys(dos.tables.dbcs) + 0x02) == 0x81 &&                     \
   mem_readb(Real2Phys(dos.tables.dbcs) + 0x03) == 0x9F)
#define IS_DOS_CJK                                                             \
  (!dos_kernel_disabled &&                                                     \
   ((mem_readb(Real2Phys(dos.tables.dbcs) + 0x02) == 0x81 ||                   \
     mem_readb(Real2Phys(dos.tables.dbcs) + 0x02) == 0xA1) &&                  \
    (mem_readb(Real2Phys(dos.tables.dbcs) + 0x03) == 0x9F ||                   \
     mem_readb(Real2Phys(dos.tables.dbcs) + 0x03) == 0xFE)))
#define IS_DOSV                                                                \
  (dos.set_jdosv_enabled || dos.set_kdosv_enabled || dos.set_pdosv_enabled ||  \
   dos.set_tdosv_enabled)
...
constexpr uint16_t CoreDevicesMask =
    CurrentStdIn | CurrentStdOut | CurrentNul | CurrentClock;
} // namespace DeviceAttributeFlags

/* Routines for File Class */
void DOS_SetupFiles(void);
bool DOS_ReadFile(uint16_t entry, uint8_t *data, uint16_t *amount,
                  bool fcb = false);
bool DOS_WriteFile(uint16_t entry, const uint8_t *data, uint16_t *amount,
                   bool fcb = false);
bool DOS_SeekFile(uint16_t entry, uint32_t *pos, uint32_t type,
                  bool fcb = false);
...
bool DOS_FindFirst(const char *search, uint16_t attr,
...
#define sSave(s, m, val)                                                       \
  SaveIt(sizeof(((s *)&pt)->m), (PhysPt)sOffset(s, m), val)
...
public:
  ...
  inline void SaveIt(const uint32_t size, const PhysPt addr,
                     const uint32_t val) {
    switch (size) {
    case 1:
      mem_writeb(pt + addr, (uint8_t)val);
      break;
    case 2:
      mem_writew(pt + addr, (uint16_t)val);
      break;
    case 4:
  ...
  inline void SetPt(const uint16_t seg, const uint16_t off) {
    pt = PhysMake(seg, off);
  }
...
#pragma pack(1)
  ...
  struct sPSP {
    ...
    uint16_t
        next_seg; /* Segment of first byte beyond memory allocated or program */
...
#pragma pack(1)
  ...
  struct sDIB {
    ...
    uint16_t
        maxSectorLength; //  0x10 maximum bytes per sector of any block device;
    RealPt diskInfoBuffer;   //  0x12 pointer to disk info buffer
    RealPt curDirStructure;  //  0x16 pointer to current array of directory
                             //  structure
    ...
    uint32_t
        diskBufferHeadPt; //  0x47 pointer to least-recently used buffer header
    ...
    uint16_t
        minMemForExec; //  0x64 minimum paragraphs needed for current program
...
public:
  ...
  void SetResult(const char *_name, const char *_lname, uint32_t _size,
  ...
  void GetResult(char *_name, char *_lname, uint32_t &_size, uint32_t &_hsize,
                 uint16_t &_date, uint16_t &_time, uint8_t &_attr);
...
#pragma pack(1)
  ...
  struct sDTA {
    ...
    uint16_t dirID; /* custom: dir-search ID for multiple searches at the same
                       time */
    uint32_t dirCluster; /* custom (drive_fat only): cluster number for multiple
                            searches at the same time. 32-bit wide on FAT32
                            aware MS-DOS 7.1 or higher. */
    uint8_t fill[2];
    uint8_t attr;
    uint16_t time;
    uint16_t date;
...
public:
  ...
  enum class MCBType : uint8_t { ValidBlock = 'M', LastBlock = 'Z' };
...
#pragma pack(1)
  ...
  struct sSDA {
    ...
    uint8_t drive_crit_error;     /* 0x02 Drive on which current critical error
                                     occurred or FFh */
    ...
    uint16_t return_code;  /* 0x14 return code from last process termination
                              (zerod after reading with AH=4Dh) */
    uint8_t current_drive; /* 0x16 current drive */
    uint8_t extended_break_flag; /* 0x17 extended break flag */
    uint8_t fill[2]; /* 0x18 flag: code page switching || flag: copy of previous
                        byte in case of INT 24 Abort*/
  } GCC_ATTRIBUTE(packed);
#ifdef _MSC_VER
#pragma pack()
#endif
...
struct DOS_Block {
  ...
  void
  psp(uint16_t _seg) const; //{ DOS_SDA(DOS_SDA_SEG,DOS_SDA_OFS).SetPSP(_seg);};
  RealPt dta() const; //{return DOS_SDA(DOS_SDA_SEG,DOS_SDA_OFS).GetDTA();};
  void
  dta(RealPt _dta) const; //{DOS_SDA(DOS_SDA_SEG,DOS_SDA_OFS).SetDTA(_dta);};
  ...
  struct {
    ...
    RealPt upcase = 0;
    uint8_t *country = NULL; // Will be copied to dos memory. resides in real
                             // mem
    uint16_t dpb = 0; // Fake Disk parameter system using only the first entry
                      // so the drive letter matches
    uint16_t dpb_size = 0x21; // bytes per DPB entry (MS-DOS 4.x-6.x size)
    uint16_t mediaid_offset =
        0x17; // media ID offset in DPB (MS-DOS 4.x-6.x case)
  } tables;
...
const std::map<std::string, int> country_code_map{
    // reference:
    // https://gitlab.com/FreeDOS/base/keyb_lay/-/blob/master/DOC/KEYB/LAYOUTS/LAYOUTS.TXT
