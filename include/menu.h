...
#define AUTOMOUNT(name, name2)                                                 \
  (((GetDriveType(name) == 2) || (GetDriveType(name) == 3) ||                  \
    (GetDriveType(name) == 4) || (GetDriveType(name) == 5) ||                  \
    (GetDriveType(name) == 6))) &&                                             \
      (!Drives[name2 - 'A'])
...
// menu.cpp replacements; the optimizer will completely remove code based on
// these
...
#if C_FORCE_MENU_SDLDRAW /* Programmer/Dev wants to compile with SDL drawn     \
                            menus even if host OS offers menus (shrug) Ok */
...
/* Whether or not the menu exists, and is NOT drawn by ourself (Windows and Mac
 * OS X) */
...
public:
  static constexpr item_handle_t unassigned_item_handle =
      ((item_handle_t)(0xFFFFU));
  static constexpr callback_t unassigned_callback = NULL;
  static const mapper_event_t unassigned_mapper_event; /* empty std::string */
public:
  struct accelerator {
    accelerator() {}
    accelerator(char _key, unsigned char _instance = 0)
        : key(_key), key_instance(_instance) {}

    char key = 0; /* ascii code i.e. 'g' */
    unsigned char key_instance =
        0; /* which occurrence of the letter in the text */
  };
...
public:
  ...
  protected:
    struct status {
      status()
          : changed(false), allocated(false), enabled(true), checked(false),
            in_use(false) {};
  ...
  public:
    ...
    void placeItemFinal(DOSBoxMenu &menu, int finalwidth,
  ...
  public:
    inline bool isHilight(void) const { return itemHilight; }
#endif
  protected:
    item &allocate(const DOSBoxMenu::item_handle_t id,
  ...
  public:
...
#if DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW
      bool r = needRedraw || (itemHilight != itemHilightDrawn) ||
               (itemHover != itemHoverDrawn);
      needRedraw = false;
      return r;
#else
      return false;
    ...
    inline item_handle_t get_master_id(void) const { return master_id; }
    inline bool is_allocated(void) const {
      return master_id != unassigned_item_handle;
    }
    inline bool has_vis_text(void) const { return type <= submenu_type_id; }
    inline bool has_vis_shortcut_text(void) const {
      return type <= item_type_id;
    }
    inline bool has_vis_description(void) const { return false; }
    inline bool has_vis_accelerator(void) const { return type <= item_type_id; }
    inline bool has_vis_enabled(void) const { return type <= submenu_type_id; }
    inline bool can_enable(void) const { return type <= submenu_type_id; }
    inline bool has_vis_checked(void) const { return type <= item_type_id; }
    inline bool can_check(void) const { return type <= item_type_id; }

  public:
    void refresh_item(DOSBoxMenu &menu);
    inline bool has_changed(void) const { return status.changed; }
    void clear_changed(void) { status.changed = false; }
...
#endif
    inline bool is_checked(void) const { return status.checked; }
  ...
  public:
    ...
    inline bool is_enabled(void) const { return status.enabled; }

  public:
    inline item_type_t get_type(void) const { return type; }
  ...
  public:
    inline mapper_event_t get_mapper_event(void) const { return mapper_event; }
  ...
  public:
    inline const std::string &get_text(void) const { return text; }
...
#if DOSBOXMENU_TYPE ==                                                         \
    DOSBOXMENU_NSMENU /* Mac OS X NSMenu / NSMenuItem handle */
protected:
  void *nsMenu = NULL;
...
public:
  inline bool isVisible(void) const { return menuVisible; }
  inline bool needsRedraw(void) const { return needRedraw; }
  inline void setRedraw(void) { needRedraw = true; }
  inline void clearRedraw(void) { needRedraw = false; }
...
public:
  void displaylist_append(displaylist &ls,
