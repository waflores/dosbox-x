...
private:
  ...
  uint8_t read_allocated_data(uint64_t file_offset, uint8_t *data,
                              uint64_t data_size);
  ...
  uint8_t read_l2_table(uint64_t l2_table_offset, uint64_t address,
                        uint64_t &data_cluster_offset);

  uint8_t read_refcount_table(uint64_t data_cluster_offset,
                              uint64_t &refcount_cluster_offset);

  uint8_t read_table(uint64_t entry_offset, uint64_t entry_mask,
  ...
  uint8_t update_reference_count(uint64_t cluster_offset,
                                 uint8_t *cluster_buffer);

  uint8_t write_data(uint64_t file_offset, const uint8_t *data,
                     uint64_t data_size);

  uint8_t write_l1_table_entry(uint64_t address, uint64_t l2_table_offset);

  uint8_t write_l2_table_entry(uint64_t l2_table_offset, uint64_t address,
                               uint64_t data_cluster_offset);

  uint8_t write_refcount(uint64_t cluster_offset,
                         uint64_t refcount_cluster_offset, uint16_t refcount);

  uint8_t write_refcount_table_entry(uint64_t cluster_offset,
                                     uint64_t refcount_cluster_offset);
...
public:
  QCow2Disk(QCow2Image::QCow2Header &qcow2Header, FILE *qcow2File,
            const char *imgName, uint32_t imgSizeK, uint32_t sectorSizeBytes,
            bool isHardDisk);
