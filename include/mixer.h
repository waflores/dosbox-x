...
public:
  ...
  void SetLowpassFreq(
      Bitu _freq,
      unsigned int order = 2);  // _freq / 1 Hz. call with _freq == 0 to disable
  void SetSlewFreq(Bitu _freq); // denominator provided by call to SetFreq. call
                                // with _freq == 0 to disable
  ...
  void lowpassUpdate();
  int32_t lowpassStep(int32_t in, const unsigned int iteration,
  ...
  bool lowpass_on_load; // apply lowpass on sample load (if source rate > mixer
                        // rate)
  bool lowpass_on_out;  // apply lowpass on rendered output (if source rate <=
                        // mixer rate)
  ...
  int32_t msbuffer[2048][2]; // more than enough for 1ms of audio, at mixer
                             // sample rate
...
/* Object to maintain a mixerchannel; As all objects it registers itself with
 * create and removes itself when destroyed. */
class MixerObject {
private:
