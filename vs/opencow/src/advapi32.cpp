...
EXTERN_C {
  ...
  OCOW_DEF(BOOL, GetUserNameW, (OUT LPWSTR lpBuffer, IN OUT LPDWORD nSize)) {
    ...
    int nRequiredSize =
        ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, dwBufSize, 0, 0);
  ...
  OCOW_DEF(LONG, RegConnectRegistryW,
           (IN LPCWSTR lpMachineName, IN HKEY hKey, OUT PHKEY phkResult)) {
  ...
  OCOW_DEF(LONG, RegCreateKeyExW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, IN DWORD Reserved,
            IN LPWSTR lpClass, IN DWORD dwOptions, IN REGSAM samDesired,
  ...
  OCOW_DEF(LONG, RegCreateKeyW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, OUT PHKEY phkResult)) {
    CMbcsBuffer mbcsSubKey;
    if (!mbcsSubKey.FromUnicode(lpSubKey))
      return ERROR_NOT_ENOUGH_MEMORY;
  ...
  OCOW_DEF(LONG, RegDeleteKeyW, (IN HKEY hKey, IN LPCWSTR lpSubKey)) {
    CMbcsBuffer mbcsSubKey;
    if (!mbcsSubKey.FromUnicode(lpSubKey))
      return ERROR_NOT_ENOUGH_MEMORY;
  ...
  OCOW_DEF(LONG, RegDeleteValueW, (IN HKEY hKey, IN LPCWSTR lpValueName)) {
  ...
  OCOW_DEF(LONG, RegEnumKeyExW,
           (IN HKEY hKey, IN DWORD dwIndex, OUT LPWSTR lpName,
            IN OUT LPDWORD lpcbName, IN LPDWORD lpReserved,
            IN OUT LPWSTR lpClass, IN OUT LPDWORD lpcbClass,
    ...
    LONG lResult = ::RegEnumKeyExA(
        hKey, dwIndex, mbcsName, &dwNameSize, lpReserved, mbcsClass,
        mbcsClass ? &dwClassSize : NULL, lpftLastWriteTime);
    if (lResult != ERROR_SUCCESS && lResult != ERROR_MORE_DATA)
      return lResult;

    int nChars = ::MultiByteToWideChar(CP_ACP, 0, mbcsName, dwNameSize, lpName,
                                       *lpcbName);
    *lpcbName = nChars - 1; // don't include null terminator

    if (mbcsClass) {
      nChars = ::MultiByteToWideChar(CP_ACP, 0, mbcsClass, dwClassSize, lpClass,
                                     *lpcbClass);
  ...
  OCOW_DEF(
      LONG, RegEnumKeyW,
      (IN HKEY hKey, IN DWORD dwIndex, OUT LPWSTR lpName, IN DWORD cbName)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.SetCapacity(cbName))
      return ERROR_NOT_ENOUGH_MEMORY;

    LONG lResult =
        ::RegEnumKeyA(hKey, dwIndex, mbcsName, mbcsName.BufferSize());
    if (lResult != ERROR_SUCCESS && lResult != ERROR_MORE_DATA)
      return lResult;
  ...
  OCOW_DEF(LONG, RegEnumValueW,
           (IN HKEY hKey, IN DWORD dwIndex, OUT LPWSTR lpValueName,
            IN OUT LPDWORD lpcbValueName, IN LPDWORD lpReserved,
            OUT LPDWORD lpType, OUT LPBYTE lpData, IN OUT LPDWORD lpcbData)) {
    CMbcsBuffer mbcsValueName;
    ...
    LONG lResult = ::RegEnumValueA(hKey, dwIndex, mbcsValueName,
                                   &dwValueNameLen, lpReserved, &dwType,
                                   (LPBYTE)mbcsData.get(), &dwDataLen);
    if (lResult != ERROR_SUCCESS)
      return lResult;

    int nChars = ::MultiByteToWideChar(CP_ACP, 0, mbcsValueName, dwValueNameLen,
  ...
  OCOW_DEF(LONG, RegLoadKeyW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, IN LPCWSTR lpFile)) {
    CMbcsBuffer mbcsSubKey;
    if (!mbcsSubKey.FromUnicode(lpSubKey))
      return ERROR_NOT_ENOUGH_MEMORY;
    ...
    DWORD dwResult =
        ::GetShortPathNameA(mbcsFile, mbcsFile, mbcsFile.BufferSize());
  ...
  OCOW_DEF(LONG, RegOpenKeyExW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, IN DWORD ulOptions,
  ...
  OCOW_DEF(LONG, RegQueryInfoKeyW,
           (IN HKEY hKey, OUT LPWSTR lpClass, IN OUT LPDWORD lpcbClass,
            IN LPDWORD lpReserved, OUT LPDWORD lpcSubKeys,
            OUT LPDWORD lpcbMaxSubKeyLen, OUT LPDWORD lpcbMaxClassLen,
            OUT LPDWORD lpcValues, OUT LPDWORD lpcbMaxValueNameLen,
            OUT LPDWORD lpcbMaxValueLen, OUT LPDWORD lpcbSecurityDescriptor,
            OUT PFILETIME lpftLastWriteTime)) {
    CMbcsBuffer mbcsClass;
    if (lpClass && lpcbClass) {
      if (!mbcsClass.SetCapacity(*lpcbClass))
    ...
    DWORD dwClassSize = mbcsClass.BufferSize();
    LONG lResult = ::RegQueryInfoKeyA(
        hKey, mbcsClass, mbcsClass ? &dwClassSize : NULL, lpReserved,
    ...
    if (mbcsClass) {
      int nChars = ::MultiByteToWideChar(CP_ACP, 0, mbcsClass, dwClassSize,
                                         lpClass, *lpcbClass);
      *lpcbClass = nChars - 1; // don't include null terminator
    }
  ...
  OCOW_DEF(LONG, RegQueryValueExW,
           (IN HKEY hKey, IN LPCWSTR lpValueName, IN LPDWORD lpReserved,
            OUT LPDWORD lpType, IN OUT LPBYTE lpData,
    ...
    DWORD dwType;
    DWORD dwDataLen = mbcsData.BufferSize();
    LONG lResult = ::RegQueryValueExA(hKey, mbcsValueName, lpReserved, &dwType,
                                      (LPBYTE)mbcsData.get(), &dwDataLen);
    if (lResult != ERROR_SUCCESS)
      return lResult;
  ...
  OCOW_DEF(LONG, RegQueryValueW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, OUT LPWSTR lpValue,
    ...
    int nChars = ::MultiByteToWideChar(CP_ACP, 0, mbcsValue, lValueSize,
                                       lpValue, *lpcbValue);
  ...
  OCOW_DEF(LONG, RegReplaceKeyW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, IN LPCWSTR lpNewFile,
  ...
  OCOW_DEF(LONG, RegSaveKeyW,
           (IN HKEY hKey, IN LPCWSTR lpFile,
  ...
  OCOW_DEF(LONG, RegSetValueExW,
           (IN HKEY hKey, IN LPCWSTR lpValueName, IN DWORD Reserved,
    ...
    return ::RegSetValueExA(hKey, mbcsValueName, Reserved, dwType, pData,
                            cbData);
  }

  OCOW_DEF(LONG, RegSetValueW,
           (IN HKEY hKey, IN LPCWSTR lpSubKey, IN DWORD dwType,
            IN LPCWSTR lpData, IN DWORD cbData)) {
    CMbcsBuffer mbcsSubKey;
    if (!mbcsSubKey.FromUnicode(lpSubKey))
      return ERROR_NOT_ENOUGH_MEMORY;
  ...
  OCOW_DEF(LONG, RegUnLoadKeyW, (IN HKEY hKey, IN LPCWSTR lpSubKey)) {
    CMbcsBuffer mbcsSubKey;
    if (!mbcsSubKey.FromUnicode(lpSubKey))
      return ERROR_NOT_ENOUGH_MEMORY;
