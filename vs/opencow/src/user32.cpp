...
EXTERN_C {
  ...
  OCOW_DEF(LRESULT, CallWindowProcW,
           (IN WNDPROC lpPrevWndFunc, IN HWND hWnd, IN UINT Msg,
  ...
  OCOW_DEF(DWORD, CharLowerBuffW, (IN OUT LPWSTR lpsz, IN DWORD cchLength)) {
    wchar_t c;
  ...
  OCOW_DEF(LPWSTR, CharLowerW, (IN OUT LPWSTR lpsz)) {
    if (HIWORD(lpsz) == 0) {
  ...
  OCOW_DEF(LPWSTR, CharNextW, (IN LPCWSTR lpsz)) {
    if (*lpsz)
      ++lpsz;
    return const_cast<LPWSTR>(lpsz);
  }

  OCOW_DEF(LPWSTR, CharPrevW, (IN LPCWSTR lpszStart, IN LPCWSTR lpszCurrent)) {
  ...
  OCOW_DEF(DWORD, CharUpperBuffW, (IN OUT LPWSTR lpsz, IN DWORD cchLength)) {
    wchar_t c;
    for (DWORD dwCount = 0; dwCount < cchLength; ++dwCount) {
      c = lpsz[dwCount];
      if (c >= 'a' && c <= 'z')
  ...
  OCOW_DEF(LPWSTR, CharUpperW, (IN OUT LPWSTR lpsz)) {
    if (HIWORD(lpsz) == 0) {
      wchar_t c = LOWORD(lpsz);
      if (c >= 'a' && c <= 'z')
        c = (unsigned short)((c - 'a') + 'A');
  ...
  OCOW_DEF(HWND, CreateWindowExW,
           (IN DWORD dwExStyle, IN LPCWSTR lpClassName, IN LPCWSTR lpWindowName,
            IN DWORD dwStyle, IN int X, IN int Y, IN int nWidth, IN int nHeight,
            IN HWND hWndParent, IN HMENU hMenu, IN HINSTANCE hInstance,
            IN LPVOID lpParam)) {
    ...
    return ::CreateWindowExA(dwExStyle, pmbcsClassName, mbcsWindowName, dwStyle,
  ...
  OCOW_DEF(LRESULT, DefWindowProcW,
           (IN HWND hWnd, IN UINT Msg, IN WPARAM wParam, IN LPARAM lParam)) {
    // TODO: is this sufficient?
  ...
  OCOW_DEF(LRESULT, DispatchMessageW, (IN CONST MSG * lpMsg)) {
    // TODO: is this sufficient?
  ...
  OCOW_DEF(int, GetClassNameW,
           (IN HWND hWnd, OUT LPWSTR lpClassName, IN int nMaxCount)) {
    CMbcsBuffer mbcsClassName;
    ...
    int nResult;
    do {
      nResult = ::MultiByteToWideChar(CP_ACP, 0, mbcsClassName, nLen--,
                                      lpClassName, nMaxCount);
    } while (nLen > 0 && nResult == 0 &&
  ...
  OCOW_DEF(int, GetClipboardFormatNameW,
           (IN UINT format, OUT LPWSTR lpszFormatName, IN int cchMaxCount)) {
    CMbcsBuffer mbcsFormatName;
    if (!mbcsFormatName.SetCapacity(cchMaxCount))
      return 0;

    int nLen = ::GetClipboardFormatNameA(format, mbcsFormatName,
                                         mbcsFormatName.BufferSize());
    if (nLen < 1)
      return nLen;
    ...
    int nResult;
    do {
      nResult = ::MultiByteToWideChar(CP_ACP, 0, mbcsFormatName, nLen--,
  ...
  OCOW_DEF(BOOL, GetMessageW,
           (OUT LPMSG lpMsg, IN HWND hWnd, IN UINT wMsgFilterMin,
  ...
  OCOW_DEF(BOOL, InsertMenuItemW,
           (HMENU hMenu, UINT item, BOOL fByPosition, LPCMENUITEMINFOW lpmi)) {
    if (!lpmi)
      return FALSE;
    ...
    MENUITEMINFOA mbsmi = {
        lpmi->cbSize,      lpmi->fMask,
        lpmi->fType,       lpmi->fState,
        lpmi->wID,         lpmi->hSubMenu,
        lpmi->hbmpChecked, lpmi->hbmpUnchecked,
        lpmi->dwItemData,  (LPSTR)lpmi->dwTypeData,
        lpmi->cch,
  ...
  OCOW_DEF(int, MessageBoxExW,
           (IN HWND hWnd, IN LPCWSTR lpText, IN LPCWSTR lpCaption,
  ...
  OCOW_DEF(
      int, MessageBoxW,
      (IN HWND hWnd, IN LPCWSTR lpText, IN LPCWSTR lpCaption, IN UINT uType)) {
    CMbcsBuffer mbcsText;
    if (!mbcsText.FromUnicode(lpText))
      return 0;
  ...
  OCOW_DEF(
      ...
      (IN DWORD idThread, IN UINT Msg, IN WPARAM wParam, IN LPARAM lParam)) {
    return ::PostThreadMessageA(idThread, Msg, wParam, lParam);
  }

  // RegisterClassExW

  OCOW_DEF(ATOM, RegisterClassW, (IN CONST WNDCLASSW * lpWndClass)) {
    WNDCLASSA wndClassA;
  ...
  OCOW_DEF(UINT, RegisterClipboardFormatW, (IN LPCWSTR lpszFormat)) {
  ...
  OCOW_DEF(LRESULT, SendMessageTimeoutW,
           (IN HWND hWnd, IN UINT Msg, IN WPARAM wParam, IN LPARAM lParam,
            IN UINT fuFlags, IN UINT uTimeout, OUT PDWORD_PTR lpdwResult)) {
    ...
    _ASSERTE(Msg == WM_SETTEXT || Msg == WM_SETICON || Msg == WM_SETFONT ||
             Msg == WM_SETTINGCHANGE);

    if (Msg == WM_SETTEXT) {
      CMbcsBuffer mbcsString;
      if (!mbcsString.FromUnicode((LPWSTR)lParam))
        return ::SendMessageTimeoutA(hWnd, WM_SETTEXT, 0, 0, fuFlags, uTimeout,
                                     lpdwResult); // error value depends on hWnd

      return ::SendMessageTimeoutA(hWnd, WM_SETTEXT, wParam,
                                   (LPARAM)mbcsString.get(), fuFlags, uTimeout,
                                   lpdwResult);
    }
    ...
    if (Msg == WM_SETTINGCHANGE) {
      ...
      return ::SendMessageTimeoutA(hWnd, WM_SETTINGCHANGE, wParam,
                                   (LPARAM)mbcsString.get(), fuFlags, uTimeout,
                                   lpdwResult);
    }
  ...
  OCOW_DEF(LRESULT, SendMessageW,
    ...
    if (Msg == WM_SETTEXT) {
      CMbcsBuffer mbcsString;
      if (!mbcsString.FromUnicode((LPWSTR)lParam))
        return ::SendMessageA(hWnd, WM_SETTEXT, 0,
                              0); // error value depends on hWnd
    ...
    if (Msg == WM_SETTINGCHANGE) {
      ...
      if (!mbcsString.FromUnicode((LPWSTR)lParam))
        mbcsString.SetNull(); // don't set the string
      return ::SendMessageA(hWnd, WM_SETTINGCHANGE, wParam,
                            (LPARAM)mbcsString.get());
    }
  ...
  OCOW_DEF(BOOL, UnregisterClassW,
           (IN LPCWSTR lpClassName, IN HINSTANCE hInstance)) {
    // class name is only a string if the HIWORD is not 0
    CMbcsBuffer mbcsClassName;
    LPCSTR pmbcsClassName = (LPCSTR)lpClassName;
    if (HIWORD(lpClassName) != 0) {
