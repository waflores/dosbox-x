...
EXTERN_C {
  ...
  OCOW_DEF(LONG, DocumentPropertiesW,
           (IN HWND hWnd, IN HANDLE hPrinter, IN LPWSTR pDeviceName,
    ...
    if (fMode & DM_IN_BUFFER) {
      if (pDevModeInput->dmDriverExtra > 0) {
        pDevModeInputA = (LPDEVMODEA)::malloc(sizeof(DEVMODEA) +
                                              pDevModeInput->dmDriverExtra);
        if (!pDevModeInputA) {
          ::SetLastError(ERROR_NOT_ENOUGH_MEMORY);
          return -1;
        }
        ::CopyMemory(pDevModeInputA + sizeof(DEVMODEA),
                     pDevModeInput + pDevModeInput->dmSize,
                     pDevModeInput->dmDriverExtra);
      } else {
        pDevModeInputA = &devModeInputAStatic;
      }
      ::ZeroMemory(pDevModeInputA,
                   sizeof(DEVMODEA)); // don't zero the extra bytes
    ...
    if (fMode & DM_OUT_BUFFER) {
      if (pDevModeOutput->dmDriverExtra > 0) {
        pDevModeOutputA = (LPDEVMODEA)::malloc(sizeof(DEVMODEA) +
                                               pDevModeOutput->dmDriverExtra);
        if (!pDevModeOutputA) {
          SetLastError(ERROR_NOT_ENOUGH_MEMORY);
          return -1;
        }
      ...
      ::ZeroMemory(pDevModeOutputA,
                   sizeof(DEVMODEA) + pDevModeOutput->dmDriverExtra);
    ...
    LONG lResult = ::DocumentPropertiesA(
        hWnd, hPrinter, mbcsDeviceName, pDevModeOutputA, pDevModeInputA, fMode);
    ...
    if (lResult == IDOK && pDevModeOutputA) {
      if (pDevModeOutputA->dmDriverExtra > 0) {
        ::CopyMemory(pDevModeOutput + pDevModeOutput->dmSize,
                     pDevModeOutputA + pDevModeOutputA->dmSize,
      ...
      ::MultiByteToWideChar(CP_ACP, 0, (LPSTR)pDevModeOutputA->dmDeviceName, -1,
                            (LPWSTR)pDevModeOutput->dmDeviceName,
                            CCHDEVICENAME);
