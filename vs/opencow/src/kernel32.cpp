...
static void CopyFindDataAtoW(LPWIN32_FIND_DATAA lpFindDataA,
...
static int __cdecl OCOW_Compare(const void *first, const void *second) {
...
EXTERN_C {
  ...
  OCOW_DEF(ATOM, AddAtomW, (IN LPCWSTR lpString)) {
  ...
  OCOW_DEF(BOOL, CopyFileW,
           (IN LPCWSTR lpExistingFileName, IN LPCWSTR lpNewFileName,
  ...
  OCOW_DEF(BOOL, CreateDirectoryExW,
    ...
    return ::CreateDirectoryExA(mbcsTemplateDirectory, mbcsNewDirectory,
                                lpSecurityAttributes);
  }

  OCOW_DEF(
      BOOL, CreateDirectoryW,
      (IN LPCWSTR lpPathName, IN LPSECURITY_ATTRIBUTES lpSecurityAttributes)) {
  ...
  OCOW_DEF(HANDLE, CreateEventW,
           (IN LPSECURITY_ATTRIBUTES lpEventAttributes, IN BOOL bManualReset,
  ...
  OCOW_DEF(HANDLE, CreateFileMappingW,
           (IN HANDLE hFile, IN LPSECURITY_ATTRIBUTES lpFileMappingAttributes,
  ...
  OCOW_DEF(HANDLE, CreateFileW,
           (IN LPCWSTR lpFileName, IN DWORD dwDesiredAccess,
            IN DWORD dwShareMode, IN LPSECURITY_ATTRIBUTES lpSecurityAttributes,
  ...
  OCOW_DEF(HANDLE, CreateMailslotW,
           (IN LPCWSTR lpName, IN DWORD nMaxMessageSize, IN DWORD lReadTimeout,
            IN LPSECURITY_ATTRIBUTES lpSecurityAttributes)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return INVALID_HANDLE_VALUE;

    return ::CreateMailslotA(mbcsName, nMaxMessageSize, lReadTimeout,
                             lpSecurityAttributes);
  }

  OCOW_DEF(HANDLE, CreateMutexW,
           (IN LPSECURITY_ATTRIBUTES lpMutexAttributes, IN BOOL bInitialOwner,
            IN LPCWSTR lpName)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return NULL;
  ...
  OCOW_DEF(BOOL, CreateProcessW,
           (IN LPCWSTR lpApplicationName, IN LPWSTR lpCommandLine,
    ...
    return ::CreateProcessA(
        mbcsApplicationName, mbcsCommandLine, lpProcessAttributes,
        lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment,
        mbcsCurrentDirectory, &startupInfoA, lpProcessInformation);
  }

  OCOW_DEF(HANDLE, CreateSemaphoreW,
           (IN LPSECURITY_ATTRIBUTES lpSemaphoreAttributes,
            IN LONG lInitialCount, IN LONG lMaximumCount, IN LPCWSTR lpName)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return NULL;

    return ::CreateSemaphoreA(lpSemaphoreAttributes, lInitialCount,
                              lMaximumCount, mbcsName);
  }

  typedef HANDLE(WINAPI * fpCreateWaitableTimerA)(
      IN LPSECURITY_ATTRIBUTES lpTimerAttributes, IN BOOL bManualReset,
      IN LPCSTR lpTimerName);

  static fpCreateWaitableTimerA
      pCreateWaitableTimerA; // move out to emit cxa_guard_* for libstdc++
                             // dependency
  OCOW_DEF(HANDLE, CreateWaitableTimerW,
    ...
    return pCreateWaitableTimerA(lpTimerAttributes, bManualReset,
                                 mbcsTimerName);
  }

  OCOW_DEF(BOOL, DeleteFileW, (IN LPCWSTR lpFileName)) {
    CMbcsBuffer mbcsFileName;
    if (!mbcsFileName.FromUnicode(lpFileName))
      return FALSE;
  ...
  OCOW_DEF(DWORD, ExpandEnvironmentStringsW,
           (IN LPCWSTR lpSrc, OUT LPWSTR lpDst, IN DWORD nSize)) {
    CMbcsBuffer mbcsSrc;
    ...
    do {
      ...
      dwResult = ::ExpandEnvironmentStringsA(mbcsSrc, mbcsDst,
                                             (DWORD)mbcsDst.BufferSize());
      if (!dwResult)
        return 0;
    ...
    int nRequiredSize =
        ::MultiByteToWideChar(CP_ACP, 0, mbcsDst, nDstLen + 1, 0, 0);
  ...
  OCOW_DEF(VOID, FatalAppExitW, (IN UINT uAction, IN LPCWSTR lpMessageText)) {
  ...
  OCOW_DEF(ATOM, FindAtomW, (IN LPCWSTR lpString)) {
    CMbcsBuffer mbcsString;
    if (!mbcsString.FromUnicode(lpString))
      return 0;
  ...
  OCOW_DEF(
      ...
      (IN LPCWSTR lpPathName, IN BOOL bWatchSubtree, IN DWORD dwNotifyFilter)) {
    CMbcsBuffer mbcsPathName;
    if (!mbcsPathName.FromUnicode(lpPathName))
      return INVALID_HANDLE_VALUE;

    return ::FindFirstChangeNotificationA(mbcsPathName, bWatchSubtree,
                                          dwNotifyFilter);
  }

  OCOW_DEF(HANDLE, FindFirstFileW,
           (IN LPCWSTR lpFileName, OUT LPWIN32_FIND_DATAW lpFindFileData)) {
    CMbcsBuffer mbcsFileName;
    if (!mbcsFileName.FromUnicode(lpFileName))
      return INVALID_HANDLE_VALUE;
  ...
  OCOW_DEF(BOOL, FindNextFileW,
           (IN HANDLE hFindFile, OUT LPWIN32_FIND_DATAW lpFindFileData)) {
    WIN32_FIND_DATAA findDataA;
  ...
  OCOW_DEF(HRSRC, FindResourceExW,
           (IN HMODULE hModule, IN LPCWSTR lpType, IN LPCWSTR lpName,
            IN WORD wLanguage)) {
  ...
  OCOW_DEF(HRSRC, FindResourceW,
           (IN HMODULE hModule, IN LPCWSTR lpName, IN LPCWSTR lpType)) {
    LPCSTR lpTypeA = (LPCSTR)lpType;
    CMbcsBuffer mbcsType;
    if (!IS_INTRESOURCE(lpType)) {
      if (!mbcsType.FromUnicode(lpType))
  ...
  OCOW_DEF(BOOL, FreeEnvironmentStringsW, (IN LPWSTR lpStrings)) {
    if (lpStrings)
      ::free(lpStrings);
    return TRUE;
  }
  ...
  OCOW_DEF(UINT, GetAtomNameW,
           (IN ATOM nAtom, OUT LPWSTR lpBuffer, IN int nSize)) {
    CMbcsBuffer mbcsBuffer;
    UINT uiLen = ::GetAtomNameA(nAtom, mbcsBuffer, mbcsBuffer.BufferSize());
    if (!uiLen)
      return 0;

    int nLen = ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, uiLen + 1, lpBuffer,
                                     nSize);
    if (!nLen) {
  ...
  BOOL WINAPI OCOW_GetCPInfo(IN UINT CodePage, OUT LPCPINFO lpCPInfo) {
  ...
  OCOW_DEF(BOOL, GetCPInfoExW,
           (IN UINT CodePage, IN DWORD dwFlags, OUT LPCPINFOEXW lpCPInfoEx)) {
    if (g_nPlatform <= MZ_PLATFORM_95 ||
        (g_nPlatform > MZ_PLATFORM_ME && g_nPlatform < MZ_PLATFORM_2K)) {
      SetLastError(ERROR_CALL_NOT_IMPLEMENTED);
      return FALSE;
    }
    ...
    ::MultiByteToWideChar(CP_ACP, 0, cpInfoA.CodePageName, -1,
                          lpCPInfoEx->CodePageName, MAX_PATH);
  ...
  OCOW_DEF(BOOL, GetComputerNameW,
           (OUT LPWSTR lpBuffer, IN OUT LPDWORD lpnSize)) {
    CMbcsBuffer mbcsBuffer;
    ...
    int nRequiredLen =
        ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, (int)dwLen + 1, 0, 0);
    ...
    ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, (int)dwLen + 1, lpBuffer,
                          *lpnSize);
    *lpnSize = (DWORD)(nRequiredLen - 1);
    return TRUE;
  }
  ...
  OCOW_DEF(DWORD, GetCurrentDirectoryW, (IN DWORD uSize, OUT LPWSTR lpBuffer)) {
    DWORD dwResult = 0;
    CMbcsBuffer mbcsBuffer;
    do {
      ...
      dwResult =
          ::GetCurrentDirectoryA((DWORD)mbcsBuffer.BufferSize(), mbcsBuffer);
      if (!dwResult)
        return 0;
    ...
    // ensure the supplied buffer is big enough
    int nRequiredSize =
        ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, nBufferLen + 1, 0, 0);
    if (uSize < (DWORD)nRequiredSize)
      return (DWORD)nRequiredSize;

    ::MultiByteToWideChar(CP_ACP, 0, mbcsBuffer, nBufferLen + 1, lpBuffer,
                          uSize);
  ...
  static fpGetDiskFreeSpaceExA
      pGetDiskFreeSpaceExA; // move out to emit cxa_guard_* for libstdc++
                            // dependency
  OCOW_DEF(BOOL, GetDiskFreeSpaceExW,
    ...
    if (!pGetDiskFreeSpaceExA) {
      pGetDiskFreeSpaceExA = (fpGetDiskFreeSpaceExA)::GetProcAddress(
  ...
  OCOW_DEF(BOOL, GetDiskFreeSpaceW,
           (IN LPCWSTR lpRootPathName, OUT LPDWORD lpSectorsPerCluster,
  ...
  OCOW_DEF(UINT, GetDriveTypeW, (IN LPCWSTR lpRootPathName)) {
    CMbcsBuffer mbcsRootPathName;
    if (!mbcsRootPathName.FromUnicode(lpRootPathName))
  ...
  OCOW_DEF(LPWSTR, GetEnvironmentStringsW, (VOID)) {
    ...
    int nRequiredLen =
        ::MultiByteToWideChar(CP_ACP, 0, pStringsA, nTotalLen, 0, 0);
    if (nRequiredLen < 1) {
    ...
    ::MultiByteToWideChar(CP_ACP, 0, pStringsA, nTotalLen, pStringsW,
                          nRequiredLen);
    ::FreeEnvironmentStringsA(pStringsA);
  ...
  OCOW_DEF(DWORD, GetEnvironmentVariableW,
           (IN LPCWSTR lpName, OUT LPWSTR lpBuffer, IN DWORD nSize)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return 0;
    ...
    do {
      ...
      dwResult = ::GetEnvironmentVariableA(mbcsName, mbcsBuffer,
                                           (DWORD)mbcsBuffer.BufferSize());
      if (!dwResult)
        return 0;
    } while (dwResult > (DWORD)mbcsBuffer.BufferSize());
    int nBufferLen = ::lstrlenA(mbcsBuffer);
  ...
  typedef BOOL(WINAPI * fpGetFileAttributesExA)(LPCSTR, GET_FILEEX_INFO_LEVELS,
                                                LPVOID);
  static fpGetFileAttributesExA pGetFileAttributesExA;

  OCOW_DEF(BOOL, GetFileAttributesExW,
           (IN LPCWSTR lpFileName, IN GET_FILEEX_INFO_LEVELS fInfoLevelId,
            OUT LPVOID lpFileInformation)) {
    CMbcsBuffer mbcsFileName;
    if (!mbcsFileName.FromUnicode(lpFileName))
      return INVALID_FILE_ATTRIBUTES;

    if ((g_nPlatform > MZ_PLATFORM_95 && g_nPlatform <= MZ_PLATFORM_ME) ||
      ...
      return pGetFileAttributesExA(mbcsFileName, fInfoLevelId,
                                   lpFileInformation);
    }
    ...
    WIN32_FILE_ATTRIBUTE_DATA *lpData =
  ...
  OCOW_DEF(DWORD, GetFileAttributesW, (IN LPCWSTR lpFileName)) {
    CMbcsBuffer mbcsFileName;
    if (!mbcsFileName.FromUnicode(lpFileName))
      return INVALID_FILE_ATTRIBUTES;
  ...
  OCOW_DEF(DWORD, GetFullPathNameW,
           (IN LPCWSTR lpFileName, IN DWORD uSize, OUT LPWSTR lpBuffer,
    ...
    do {
      ...
      char *pszFilePart;
      dwResult =
          ::GetFullPathNameA(mbcsFileName, (DWORD)mbcsBuffer.BufferSize(),
                             mbcsBuffer, &pszFilePart);
      if (!dwResult)
        return 0;
    } while (dwResult > (DWORD)mbcsBuffer.BufferSize());
    int nBufferLen = ::lstrlenA(mbcsBuffer);
  ...
  OCOW_DEF(DWORD, GetLogicalDriveStringsW,
           (IN DWORD nBufferLength, OUT LPWSTR lpBuffer)) {
    CMbcsBuffer mbcsDriveStrings;
    if (!mbcsDriveStrings.SetCapacity(nBufferLength))
      return 0;

    int len = ::GetLogicalDriveStringsA(mbcsDriveStrings.BufferSize(),
                                        mbcsDriveStrings);
    if (len > 0 && len < mbcsDriveStrings.BufferSize()) {
      // include terminating null character by using len + 1
      int nResult = ::MultiByteToWideChar(CP_ACP, 0, mbcsDriveStrings, len + 1,
                                          lpBuffer, nBufferLength);
      if (nResult == 0) {
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
          len =
              ::MultiByteToWideChar(CP_ACP, 0, mbcsDriveStrings, len + 1, 0, 0);
        else
          len = 0;
      } else {
  ...
  OCOW_DEF(
      ...
      (IN LPCWSTR lpszShortPath, OUT LPWSTR lpszLongPath, IN DWORD cchBuffer)) {
    ...
    do {
      ...
      if ((g_nPlatform > MZ_PLATFORM_95 && g_nPlatform <= MZ_PLATFORM_ME) ||
        ...
        dwResult = pGetLongPathNameA
                       ? pGetLongPathNameA(mbcsShortPath, mbcsLongPath,
                                           (DWORD)mbcsLongPath.BufferSize())
                       : 0;
      } else {
    ...
    // ensure the supplied buffer is big enough
    int nRequiredSize =
        ::MultiByteToWideChar(CP_ACP, 0, mbcsLongPath, nLongPathLen + 1, 0, 0);
    if (cchBuffer < (DWORD)nRequiredSize)
      return (DWORD)nRequiredSize;

    ::MultiByteToWideChar(CP_ACP, 0, mbcsLongPath, nLongPathLen + 1,
                          lpszLongPath, cchBuffer);
    return nRequiredSize - 1; // do not include the NULL
  }

  OCOW_DEF(DWORD, GetModuleFileNameW,
           (IN HMODULE hModule, OUT LPWSTR lpFilename, IN DWORD nSize)) {
    CMbcsBuffer mbcsFilename;
    ...
    return (DWORD)::MultiByteToWideChar(CP_ACP, 0, mbcsFilename, -1, lpFilename,
                                        nSize);
  }

  OCOW_DEF(HMODULE, GetModuleHandleW, (IN LPCWSTR lpModuleName)) {
...
// GetPrivateProfileStructW
  ...
  FARPROC WINAPI OCOW_GetProcAddress(IN HMODULE hModule, IN LPCSTR lpProcName) {
    ...
    // The contents of the OCOW.def section for msvcrt.dll has been added
    // manually.
    ...
    if (!::GetModuleFileNameA(hModule, szModulePath, sizeof(szModulePath)))
      return NULL;
    char *pszModuleName =
        (char *)::_mbsrchr((unsigned char *)szModulePath, '\\');
    if (!pszModuleName) {
    ...
    *pszModuleName++ = 0;
    ...
    if (0 == ::strcmp(szModulePath, szSystemPath)) {
      ...
      MsluExport *pEntry = (MsluExport *)::bsearch(
          ...
          sizeof(rgCheckedExports[0]), OCOW_Compare);
      if (pEntry) {
        ...
        if (g_nDebug >= 2) {
          ...
          lstrcatA(szMessage,
                   "'. Press cancel to disable getprocaddress notifications.");
  ...
  OCOW_DEF(
      ...
      (IN LPCWSTR lpszLongPath, OUT LPWSTR lpszShortPath, IN DWORD cchBuffer)) {
    CMbcsBuffer mbcsLongPath;
    ...
    do {
      ...
      dwResult = ::GetShortPathNameA(mbcsLongPath, mbcsShortPath,
                                     (DWORD)mbcsShortPath.BufferSize());
      if (!dwResult)
        return 0;
    ...
    // ensure the supplied buffer is big enough
    int nRequiredSize = ::MultiByteToWideChar(CP_ACP, 0, mbcsShortPath,
                                              nShortPathLen + 1, 0, 0);
    if (cchBuffer < (DWORD)nRequiredSize)
      return (DWORD)nRequiredSize;

    ::MultiByteToWideChar(CP_ACP, 0, mbcsShortPath, nShortPathLen + 1,
    ...
    ))
{
  ...
  OCOW_DEF(BOOL, GetStringTypeW,
           (IN DWORD dwInfoType, IN LPCWSTR lpSrcStr, IN int cchSrc,
            OUT LPWORD lpCharType)) {
    CMbcsBuffer mbcsString;
    if (!mbcsString.FromUnicode(lpSrcStr, cchSrc))
      return FALSE;

    return ::GetStringTypeA(LOCALE_SYSTEM_DEFAULT, dwInfoType, mbcsString,
                            mbcsString.Length(), lpCharType);
  }

  OCOW_DEF(UINT, GetSystemDirectoryW, (OUT LPWSTR lpBuffer, IN UINT uSize)) {
    DWORD dwResult = 0;
    CMbcsBuffer mbcsBuffer;
    do {
      if (dwResult > (DWORD)mbcsBuffer.BufferSize()) {
      ...
      dwResult =
          ::GetSystemDirectoryA(mbcsBuffer, (DWORD)mbcsBuffer.BufferSize());
      if (!dwResult)
        return 0;
    } while (dwResult > (DWORD)mbcsBuffer.BufferSize());
    int nBufferLen = ::lstrlenA(mbcsBuffer);
  ...
  OCOW_DEF(UINT, GetSystemWindowsDirectoryW,
           (OUT LPWSTR lpBuffer, IN UINT uSize)) {
    DWORD dwResult = 0;
    CMbcsBuffer mbcsBuffer;
    do {
      if (dwResult > (DWORD)mbcsBuffer.BufferSize()) {
      ...
      // The GetSystemWindowsDirectoryA function doesn't exist on Windows
      // 95/98/ME. As the result is the shared windows directory, this is the
      // same as the standard GetWindowsDirectoryA call on these platforms.
      dwResult =
          ::GetWindowsDirectoryA(mbcsBuffer, (DWORD)mbcsBuffer.BufferSize());
      if (!dwResult)
        return 0;
    } while (dwResult > (DWORD)mbcsBuffer.BufferSize());
    int nBufferLen = ::lstrlenA(mbcsBuffer);
  ...
  OCOW_DEF(UINT, GetTempFileNameW,
           (IN LPCWSTR lpPathName, IN LPCWSTR lpPrefixString, IN UINT uUnique,
    ...
    UINT uiResult = ::GetTempFileNameA(mbcsPathName, mbcsPrefixString, uUnique,
                                       szTempFileName);
    if (uiResult == 0)
      return 0;

    ::MultiByteToWideChar(CP_ACP, 0, szTempFileName, -1, lpTempFileName,
                          MAX_PATH);
    return uiResult;
  }

  OCOW_DEF(DWORD, GetTempPathW, (IN DWORD uSize, OUT LPWSTR lpBuffer)) {
    DWORD dwResult = 0;
    CMbcsBuffer mbcsBuffer;
    do {
      if (dwResult > (DWORD)mbcsBuffer.BufferSize()) {
  ...
  OCOW_DEF(UINT, GetWindowsDirectoryW, (OUT LPWSTR lpBuffer, IN UINT uSize)) {
    DWORD dwResult = 0;
    CMbcsBuffer mbcsBuffer;
    do {
      if (dwResult > (DWORD)mbcsBuffer.BufferSize()) {
  ...
  OCOW_DEF(ATOM, GlobalAddAtomW, (IN LPCWSTR lpString)) {
    CMbcsBuffer mbcsString;
    if (!mbcsString.FromUnicode(lpString))
      return 0;
  ...
  OCOW_DEF(ATOM, GlobalFindAtomW, (IN LPCWSTR lpString)) {
    CMbcsBuffer mbcsString;
    if (!mbcsString.FromUnicode(lpString))
      return 0;
  ...
  OCOW_DEF(UINT, GlobalGetAtomNameW,
    ...
    CMbcsBuffer mbcsBuffer;
    UINT uiLen =
        ::GlobalGetAtomNameA(nAtom, mbcsBuffer, mbcsBuffer.BufferSize());
    if (!uiLen)
      return 0;
  ...
  OCOW_DEF(BOOL, IsBadStringPtrW, (IN LPCWSTR lpsz, IN UINT_PTR ucchMax)) {
    if (ucchMax == 0)
      return 0;
  ...
  // TODO: MSLU adds support for CP_UTF7 and CP_UTF8
  BOOL WINAPI OCOW_IsValidCodePage(IN UINT CodePage) {
  ...
  OCOW_DEF(int, LCMapStringW,
           (IN LCID Locale, IN DWORD dwMapFlags, IN LPCWSTR lpSrcStr,
            IN int cchSrc, OUT LPWSTR lpDestStr, IN int cchDest)) {
    CMbcsBuffer mbcsString;
    if (!mbcsString.FromUnicode(lpSrcStr, cchSrc))
      return 0;

    // get the buffer size required
    int nRequired = ::LCMapStringA(Locale, dwMapFlags, mbcsString,
                                   mbcsString.Length(), 0, 0);
    if (nRequired == 0)
      return 0;
    ...
    nRequired =
        ::LCMapStringA(Locale, dwMapFlags, mbcsString, mbcsString.Length(),
                       mbcsDest, mbcsDest.BufferSize());
    if (nRequired == 0)
      return 0;
    ...
    return ::MultiByteToWideChar(CP_ACP, 0, mbcsDest, nRequired, lpDestStr,
                                 cchDest);
  }

  OCOW_DEF(HMODULE, LoadLibraryExW,
           (IN LPCWSTR lpLibFileName, IN HANDLE hFile, IN DWORD dwFlags)) {
  ...
  OCOW_DEF(HMODULE, LoadLibraryW, (IN LPCWSTR lpLibFileName)) {
    CMbcsBuffer mbcsLibFileName;
    if (!mbcsLibFileName.FromUnicode(lpLibFileName))
      return NULL;
  ...
  int WINAPI OCOW_MultiByteToWideChar(
      IN UINT CodePage, IN DWORD dwFlags, IN LPCSTR lpMultiByteStr,
      IN int cbMultiByte, OUT LPWSTR lpWideCharStr, IN int cchWideChar) {
  ...
  OCOW_DEF(
      HANDLE, OpenEventW,
      (IN DWORD dwDesiredAccess, IN BOOL bInheritHandle, IN LPCWSTR lpName)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return NULL;
  ...
  typedef HANDLE(WINAPI * fpOpenWaitableTimerA)(
      IN DWORD dwDesiredAccess, IN BOOL bInheritHandle, IN LPCSTR lpTimerName);

  static fpOpenWaitableTimerA
      pOpenWaitableTimerA; // move out to emit cxa_guard_* for libstdc++
                           // dependency
  OCOW_DEF(HANDLE, OpenWaitableTimerW,
           (IN DWORD dwDesiredAccess, IN BOOL bInheritHandle,
            IN LPCWSTR lpTimerName)) {
  ...
  OCOW_DEF(VOID, OutputDebugStringW, (IN LPCWSTR lpOutputString)) {
  ...
  OCOW_DEF(BOOL, RemoveDirectoryW, (IN LPCWSTR lpPathName)) {
    CMbcsBuffer mbcsPathName;
    if (!mbcsPathName.FromUnicode(lpPathName))
      return FALSE;
  ...
  OCOW_DEF(BOOL, SetComputerNameW, (IN LPCWSTR lpComputerName)) {
  ...
  OCOW_DEF(BOOL, SetConsoleTitleW, (IN LPCWSTR lpConsoleTitle)) {
  ...
  OCOW_DEF(BOOL, SetCurrentDirectoryW, (IN LPCWSTR lpPathName)) {
    CMbcsBuffer mbcsPathName;
    if (!mbcsPathName.FromUnicode(lpPathName))
      return FALSE;
  ...
  OCOW_DEF(BOOL, SetEnvironmentVariableW,
           (IN LPCWSTR lpName, IN LPCWSTR lpValue)) {
    CMbcsBuffer mbcsName;
    if (!mbcsName.FromUnicode(lpName))
      return FALSE;
  ...
  OCOW_DEF(BOOL, SetFileAttributesW,
           (IN LPCWSTR lpFileName, IN DWORD dwFileAttributes)) {
    CMbcsBuffer mbcsFileName;
    if (!mbcsFileName.FromUnicode(lpFileName))
      return FALSE;
  ...
  OCOW_DEF(BOOL, SetVolumeLabelW,
           (IN LPCWSTR lpRootPathName, IN LPCWSTR lpVolumeName)) {
    CMbcsBuffer mbcsRootPathName;
    if (!mbcsRootPathName.FromUnicode(lpRootPathName))
      return FALSE;
  ...
  // TODO: MSLU adds support for CP_UTF7 and CP_UTF8 to Windows 95
  int WINAPI OCOW_WideCharToMultiByte(
      IN UINT CodePage, IN DWORD dwFlags, IN LPCWSTR lpWideCharStr,
      IN int cchWideChar, OUT LPSTR lpMultiByteStr, IN int cbMultiByte,
      IN LPCSTR lpDefaultChar, OUT LPBOOL lpUsedDefaultChar) {
  ...
  OCOW_DEF(LPWSTR, lstrcatW, (IN OUT LPWSTR lpString1, IN LPCWSTR lpString2)) {
  ...
  // lstrcmpW
  // lstrcmpiW
  OCOW_DEF(LPWSTR, lstrcpyW, (OUT LPWSTR lpString1, IN LPCWSTR lpString2)) {
    if (!lpString1 || !lpString2)
      return NULL;
  ...
  OCOW_DEF(LPWSTR, lstrcpynW,
           (OUT LPWSTR lpString1, IN LPCWSTR lpString2, IN int iMaxLength)) {
    if (!lpString1 || !lpString2)
      return NULL;
    if (iMaxLength < 1)
      return lpString1;
  ...
  int WINAPI OCOW_lstrlenW(IN LPCWSTR lpString) {
    int len = 0;
    while (*lpString++)
      ++len;
    return len;
