...
*/
Pleasanton, CA 94588(USA)slouken @devolution.com * /
...
char *image_files[NUM_IMAGES] = {"quit.bmp", "scroll_up.bmp", "scroll_dn.bmp"};
...
void SendHello(char *name) {
  ...
  if (tcpsock != NULL) {
    /* Get our chat handle */
    if ((name == NULL) && ((name = getenv("CHAT_USER")) == NULL) &&
...
int HandleServerData(Uint8 *data) {
  ...
  case CHAT_ADD: {
    ...
    termwin->AddText("* New client on %d from %d.%d.%d.%d:%d (%s)\n", which,
                     (newip.host >> 24) & 0xFF, (newip.host >> 16) & 0xFF,
                     (newip.host >> 8) & 0xFF, newip.host & 0xFF, newip.port,
                     people[which].name);
...
void HandleClient(void) {
  ...
  while (n-- > 0) {
    if (packets[n]->channel >= 0) {
      termwin->AddText("[%s] ", people[packets[n]->channel].name);
...
void InitGUI(SDL_Surface *screen) {
  ...
  widget = new GUI_ScrollButtons(2, y1, images[IMAGE_SCROLL_UP], empty_rect, 2,
                                 y2, images[IMAGE_SCROLL_DN],
...
extern "C" void cleanup(int exitcode) {
  int i;
