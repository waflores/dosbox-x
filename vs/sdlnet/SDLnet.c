...
int SDLNet_GetLastError(void) { return errno; }

void SDLNet_SetLastError(int err) { errno = err; }
...
#define _SIZEOF_ADDR_IFREQ sizeof
  ...
  if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) ==
    ...
    PIP_ADAPTER_INFO pNewAdapterInfo =
        (IP_ADAPTER_INFO *)SDL_realloc(pAdapterInfo, ulOutBufLen);
  ...
  if (dwRetVal == NO_ERROR) {
    for (pAdapter = pAdapterInfo; pAdapter; pAdapter = pAdapter->Next) {
      for (pAddress = &pAdapterInfo->IpAddressList; pAddress;
...
#undef SDLNet_Read32
Uint32 SDLNet_Read32(void *areap) { return (SDL_SwapBE32(*(Uint32 *)(areap))); }
