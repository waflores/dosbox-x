...
/* Ensure that all version numbers in png.h are consistent with one another. */
    ...
    (PNG_LIBPNG_VER_SHAREDLIB !=                                               \
     PNG_LIBPNG_VER_MAJOR * 10 + PNG_LIBPNG_VER_MINOR) ||                      \
 ...
 */
    ...
    defined PNG_READ_bKGD_SUPPORTED && defined PNG_READ_cHRM_SUPPORTED &&      \
    defined PNG_READ_gAMA_SUPPORTED && defined PNG_READ_oFFs_SUPPORTED &&      \
    defined PNG_READ_pCAL_SUPPORTED && defined PNG_READ_pHYs_SUPPORTED &&      \
    defined PNG_READ_sBIT_SUPPORTED && defined PNG_READ_sCAL_SUPPORTED &&      \
    defined PNG_READ_sRGB_SUPPORTED && defined PNG_READ_sPLT_SUPPORTED &&      \
    defined PNG_READ_tEXt_SUPPORTED && defined PNG_READ_tIME_SUPPORTED &&      \
...
static void PNGCBAPI read_row_callback(png_structp png_ptr,
...
static void PNGCBAPI write_row_callback(png_structp png_ptr,
                                        png_uint_32 row_number, int pass) {
  /* The callback should always receive correct parameters. */
  if (png_ptr == NULL)
 ...
 */
static void PNGCBAPI read_user_callback(png_structp png_ptr,
...
static void PNGCBAPI count_zero_samples(png_structp png_ptr,
                                        png_row_infop row_info,
                                        png_bytep data) {
  png_bytep dp = data;
...
void pngtest_check_io_state(png_structp png_ptr, size_t data_length,
                            png_uint_32 io_op) {
...
static void PNGCBAPI pngtest_read_data(png_structp png_ptr, png_bytep data,
                                       size_t length) {
  size_t check = 0;
  png_voidp io_ptr;
 ...
 * than changing the library.
 */
static void PNGCBAPI pngtest_write_data(png_structp png_ptr, png_bytep data,
                                        size_t length) {
  size_t check;
...
static void PNGCBAPI pngtest_warning(png_structp png_ptr,
 ...
 */
static void PNGCBAPI pngtest_error(png_structp png_ptr,
                                   png_const_charp message) {
  ++error_count;
...
static void init_user_chunk_info(png_const_infop info_ptr,
                                 user_chunk_info *chunk_data) {
  memset(chunk_data, 0, sizeof(*chunk_data));
  chunk_data->info_ptr = info_ptr;
}

static int set_chunk_location(png_structp png_ptr, user_chunk_info *chunk_data,
                              int what) {
  int location;
...
#ifdef PNG_WRITE_SUPPORTED
static void write_sTER_chunk(png_structp write_ptr, user_chunk_info *data) {
...
static void write_vpAg_chunk(png_structp write_ptr, user_chunk_info *data) {
  ...
  if (verbose != 0)
    fprintf(STDERR, " vpAg = %lu x %lu, units = %d\n",
            (unsigned long)data->vpAg_width, (unsigned long)data->vpAg_height,
            data->vpAg_units);
...
static void write_chunks(png_structp write_ptr, user_chunk_info *data,
                         int location) {
  int i;
...
static int test_one_file(const char *inname, const char *outname) {
...
#ifdef PNG_mDCV_SUPPORTED
  {
    ...
    if (png_get_mDCV_fixed(read_ptr, read_info_ptr, &white_x, &white_y, &red_x,
                           &red_y, &green_x, &green_y, &blue_x, &blue_y, &maxDL,
                           &minDL) != 0)
      png_set_mDCV_fixed(write_ptr, write_info_ptr, white_x, white_y, red_x,
                         red_y, green_x, green_y, blue_x, blue_y, maxDL, minDL);
  }
#endif
...
#ifdef PNG_cHRM_SUPPORTED
  {
    ...
    if (png_get_cHRM(read_ptr, read_info_ptr, &white_x, &white_y, &red_x,
                     &red_y, &green_x, &green_y, &blue_x, &blue_y) != 0) {
      png_set_cHRM(write_ptr, write_info_ptr, white_x, white_y, red_x, red_y,
...
#ifdef PNG_mDCV_SUPPORTED
  {
    ...
    if (png_get_mDCV(read_ptr, read_info_ptr, &white_x, &white_y, &red_x,
                     &red_y, &green_x, &green_y, &blue_x, &blue_y, &maxDL,
                     &minDL) != 0)
      png_set_mDCV(write_ptr, write_info_ptr, white_x, white_y, red_x, red_y,
                   green_x, green_y, blue_x, blue_y, maxDL, minDL);
  }
#endif
...
#define pass_height height
...
#ifdef PNG_WRITE_UNKNOWN_CHUNKS_SUPPORTED
  {
    png_unknown_chunkp unknowns;
    int num_unknowns =
        png_get_unknown_chunks(read_ptr, end_info_ptr, &unknowns);
  ...
  if (error_count > 0) {
     ...
     */
    fprintf(STDERR, "\n  %s: %d libpng errors found (%d warnings)", inname,
  ...
  /* If there is no write support nothing was written! */
    ...
    fprintf(STDERR, "\n  %s: unsupported chunks (%d)%s", inname,
            unsupported_chunks, strict ? ": IGNORED --strict!" : "");
  }
#endif
