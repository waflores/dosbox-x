...
static int chromaticities_match_sRGB(const png_xy *xy) {
 ...
 */
static int png_image_is_not_sRGB(png_const_structrp png_ptr) {
...
static int png_image_read_header(png_voidp argument) {
...

  {
     ...
     */
    if ((format & PNG_FORMAT_FLAG_COLOR) != 0 && png_image_is_not_sRGB(png_ptr))
...
static int png_image_read_direct(png_voidp argument) {
  ...
  /* Now check the format to see if it was modified. */
  {
    ...
    if (do_local_background != 0) {
       ...
       * final value.
       */
      if (png_muldiv(&gtest, output_gamma, png_resolve_file_gamma(png_ptr),
                     PNG_FP_1) != 0 &&
