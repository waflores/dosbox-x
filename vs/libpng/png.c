...
static png_int_32 png_fp_add(png_int_32 addend0, png_int_32 addend1,
                             int *error) {
...
static png_int_32 png_fp_sub(png_int_32 addend0, png_int_32 addend1,
                             int *error) {
...
static int png_safe_add(png_int_32 *addend0_and_result, png_int_32 addend1,
                        png_int_32 addend2) {
  ...
  int result = png_fp_add(*addend0_and_result,
                          png_fp_add(addend1, addend2, &error), &error);
  if (!error)
...
png_XYZ_from_xy(png_XYZ *XYZ, const png_xy *xy) {
   ...
   */
  {
    ...
    if (png_muldiv(&left, xy->greenx - xy->bluex, xy->whitey - xy->bluey, 8) ==
        0)
      return 1;
    ...
    if (png_muldiv(&red_inverse, xy->whitey, denominator,
                   png_fp_sub(left, right, &error)) == 0 ||
        error || red_inverse <= xy->whitey /* r+g+b scales = white scale */)
      return 1;

    /* Similarly for green_inverse: */
    if (png_muldiv(&left, xy->redy - xy->bluey, xy->whitex - xy->bluex, 8) == 0)
      return 1;
    if (png_muldiv(&right, xy->redx - xy->bluex, xy->whitey - xy->bluey, 8) ==
        0)
      return 1;
    if (png_muldiv(&green_inverse, xy->whitey, denominator,
                   png_fp_sub(left, right, &error)) == 0 ||
        error || green_inverse <= xy->whitey)
      return 1;
...
png_icc_check_header(png_const_structrp png_ptr, png_const_charp name,
  ...
  if (temp > 357913930 || /* (2^32-4-132)/12: maximum possible tag count */
    ...
    return png_icc_profile_error(png_ptr, name, temp, "tag count too large");
  ...
  if (temp != 0x61637370)
    return png_icc_profile_error(png_ptr, name, temp, "invalid signature");
...
static int have_chromaticities(png_const_structrp png_ptr) {
...
png_uint_32 png_fixed_ITU(png_const_structrp png_ptr, double fp,
                          png_const_charp text) {
