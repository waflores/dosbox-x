 ...
 */
    ...
    defined(PNG_STDIO_SUPPORTED) && defined(PNG_READ_SUPPORTED)
...
static int ancillaryb(const png_byte *name) {
  return PNG_CHUNK_ANCILLARY(PNG_U32(name[0], name[1], name[2], name[3]));
}
#endif
...
static void init_display(display *d, const char *program) {
  memset(d, 0, sizeof *d);
  d->png_ptr = NULL;
...
static int display_rc(const display *d, int strict) {
...
static void PNGCBAPI warning(png_structp png_ptr, const char *message) {
  display *d = (display *)png_get_error_ptr(png_ptr);
...
static int PNGCBAPI read_callback(png_structp pp, png_unknown_chunkp pc) {
...
static int check(FILE *fp, int argc, const char **argv,
                 png_uint_32p flags /*out*/, display *d, int set_callback) {
  int i, npasses, ipass;
  png_uint_32 height;
   ...
   */
  d->png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, d, error, warning);
  if (d->png_ptr == NULL) {
    ...
    if (equals != NULL) {
      ...
      case 4: /* chunk name */
        ...
        if (chunk >= 0) {
           ...
           */
#if PNG_LIBPNG_VER >= 10700 && !defined(PNG_SAVE_UNKNOWN_CHUNKS_SUPPORTED)
...
static void check_error(display *d, png_uint_32 flags, const char *message) {
  while (flags) {
...
static void check_handling(display *d, int def, png_uint_32 chunks,
                           png_uint_32 known, png_uint_32 unknown,
                           const char *position, int set_callback) {
  while (chunks) {
    ...
    if (errorx != NULL) {
      ...
      fprintf(stderr, "%s(%s%s): %s %s %s: %s\n", d->file, d->test,
              set_callback ? ",callback" : "", type, chunk_info[i].name,
              position, errorx);
    }
...
static void perform_one_test(FILE *fp, int argc, const char **argv,
...
static void perform_one_test_safe(FILE *fp, int argc, const char **argv,
                                  png_uint_32 *default_flags, display *d,
                                  const char *test) {
...
static PNG_NORETURN void usage(const char *program, const char *reason) {
  fprintf(stderr,
          "pngunknown: %s: usage:\n %s [--strict] "
...
int main(int argc, const char **argv) {
  ...
  if (!default_tests) {
    ...
    while (*test) {
      ...
      perform_one_test_safe(fp, arg_count, test, default_flags, &d, this_test);
