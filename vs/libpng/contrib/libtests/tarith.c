...
int validation_ascii_to_fp(int count, int argc, char **argv) {
  ...
  do {
    ...
    if (!isfinite(test)) {
      ...
      double allow =
          .5 / pow(10, (precision >= DBL_DIG) ? DBL_DIG - 1 : precision - 1);
       ...
       */
      if (change >= allow &&
          (isfinite(out) || fabs(test / DBL_MAX) <= 1 - allow)) {
 ...
 */
typedef enum checkfp_state { start, fraction, exponent, states } checkfp_state;
...
static int check_one_character(checkfp_command *co, checkfp_control c, int ch) {
  ...
  /* Testing an invalid character */
    ...
    if (number_is_valid != c.number_was_valid) {
      fprintf(stderr, "%s: character '%c' [0x%.2x] changed number validity\n",
              co->number, ch, ch);
      return 0;
    }
...
int validation_gamma(int argc, char **argv) {
  double gamma[9] = {2.2,      1.8,      1.52,    1.45,   1.,
                     1 / 1.45, 1 / 1.52, 1 / 1.8, 1 / 2.2};
  double maxerr;
  ...
  if (!onlygamma) {
    ...
    for (i = 0; i <= 0xfffff; ++i) {
      ...
      {
        fprintf(stderr, "32-bit exp error: %d: got %u, expected %f error: %f\n",
    ...
    for (i = 0; i <= 0xfffff; ++i) {
      ...
      {
        fprintf(stderr, "8-bit exp error: %d: got %u, expected %f error: %f\n",
    ...
    for (i = 0; i <= 0xfffff; ++i) {
      ...
      {
        fprintf(stderr, "16-bit exp error: %d: got %u, expected %f error: %f\n",
