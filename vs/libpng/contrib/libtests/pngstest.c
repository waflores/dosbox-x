...
static png_byte sRGB(double linear /*range 0.0 .. 1.0*/) {
  return u8d(255 * sRGB_from_linear(linear));
}

static png_byte isRGB(int fixed_linear) { return sRGB(fixed_linear / 65535.); }
...
#if ALLOW_UNUSED_GPC
static png_uint_16 ilineara_g22(int fixed_srgb, int alpha) {
...
#define GBG_ERROR                                                              \
  1024 /* do not ignore the gamma+background_rgb_to_gray                       \
        * libpng warning. */
...
static png_uint_32 formatof(const char *arg) {
  char *ep;
...
static void checkbuffer(Image *image, const char *arg) {
 ...
 */
static int logerror(Image *image, const char *a1, const char *a2,
                    const char *a3) {
  fflush(stdout);
 ...
 * function call.)
 */
static int logclose(Image *image, FILE *f, const char *name,
...
static int checkopaque(Image *image) {
   ...
   */
           ...
           (strcmp(image->image.message,
                ...
                ? (image->opts & GBG_ERROR) != 0
                : (image->opts & STRICT) != 0))
    return logerror(image, image->file_name,
                    (image->opts & GBG_ERROR) != 0 ? " --fault-gbg-warning"
                                                   : " --strict",
                    "");
 ...
 */
/* Simple copy: */
static void gpc_noop(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  out->r = in->r;
  out->g = in->g;
  out->b = in->b;
...
#if ALLOW_UNUSED_GPC
static void gpc_nop8(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  if (in->a == 0)
...
#if ALLOW_UNUSED_GPC
static void gpc_nop6(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  if (in->a == 0)
...
/* bckc: composite on color background */
static void gpc_bckc(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0) {
    out->r = back->ir;
    out->g = back->ig;
    out->b = back->ib;
...
/* g8: convert sRGB components to sRGB grayscale */
static void gpc_g8(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* g8b: convert sRGB components to grayscale and composite on gray background */
static void gpc_g8b(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0)
    out->r = out->g = out->b = back->ig;
    ...
    out->r = out->g = out->b = sRGB(
        a * YfromRGB(sRGB_to_d[in->r], sRGB_to_d[in->g], sRGB_to_d[in->b]) +
        back->dg * (1 - a));
  }
...
/* lin: make sRGB components linear, alpha := 65535 */
static void gpc_lin(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* pre: make sRGB components linear and premultiply by alpha (scale alpha) */
static void gpc_pre(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* pre': as 'pre' but alpha := 65535 */
static void gpc_preq(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* glin: make sRGB components linear, convert to grayscale, alpha := 65535 */
static void gpc_glin(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  ...
  if (in->r == in->g && in->g == in->b)
    ...
    out->r = out->g = out->b = u16d(
        65535 * YfromRGB(sRGB_to_d[in->r], sRGB_to_d[in->g], sRGB_to_d[in->b]));

  out->a = 65535;
}

/* gpre: make sRGB components grayscale and linear and premultiply by alpha */
static void gpc_gpre(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* gpr': as 'gpre' but alpha := 65535 */
static void gpc_gprq(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
static void gpc_Glin(Pixel *out, const Pixel *in, const Background *back) {
  ...
  if (in->r == in->g && in->g == in->b)
    ...
    out->r = out->g = out->b = u16d(
        65535 * YfromRGB(g22_to_d[in->r], g22_to_d[in->g], g22_to_d[in->b]));
...
/* sRGB: convert linear components to sRGB, alpha := 255 */
static void gpc_sRGB(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* unpg: unpremultiply gray component and convert to sRGB (scale alpha) */
static void gpc_unpg(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* unpc: unpremultiply color components and convert to sRGB (scale alpha) */
static void gpc_unpc(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* b16g: composite linear onto gray background and convert the result to sRGB */
static void gpc_b16g(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0)
    out->r = out->g = out->b = back->ig;
...
/* b16c: composite linear onto color background and convert the result to sRGB*/
static void gpc_b16c(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0) {
    out->r = back->ir;
    out->g = back->ig;
    out->b = back->ib;
...
/* sG: convert linear RGB to sRGB grayscale */
static void gpc_sG(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* sGp: unpremultiply RGB then convert to sRGB grayscale */
static void gpc_sGp(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* sCp: unpremultiply RGB then convert to sRGB */
static void gpc_sCp(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* gb16: composite linear onto background and convert to sRGB grayscale */
/*  (order doesn't matter, the composite and grayscale operations permute) */
static void gpc_gb16(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0)
    out->r = out->g = out->b = back->ig;
...
/* cb16: composite linear onto background and convert to sRGB */
static void gpc_cb16(Pixel *out, const Pixel *in, const Background *back) {
  if (in->a <= 0) {
    out->r = back->ir;
    out->g = back->ig;
    out->b = back->ib;
...
/* A:    set alpha to 65535 */
static void gpc_A(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  out->r = in->r;
  out->g = in->g;
  out->b = in->b;
...
/* g16:  convert linear RGB to linear grayscale (alpha := 65535) */
static void gpc_g16(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
...
/* g16': as 'g16' but alpha is unchanged */
static void gpc_g16q(Pixel *out, const Pixel *in, const Background *back) {
  (void)back;
  out->r = out->g = out->b = u16d(YfromRGBint(in->r, in->g, in->b));
  out->a = in->a;
}
...
void (*const gpc_unused[])(Pixel *out, const Pixel *in,
...
static void (*const gpc_fn[8 /*in*/][8 /*out*/])(Pixel *out, const Pixel *in,
                                                 const Background *back) = {
    /*out: G-8     GA-8     RGB-8    RGBA-8    G-16     GA-16   RGB-16  RGBA-16
     */
    {gpc_noop, gpc_noop, gpc_noop, gpc_noop, gpc_Lin, gpc_Lin, gpc_Lin,
     gpc_Lin},
    {gpc_bckg, gpc_noop, gpc_bckc, gpc_noop, gpc_preq, gpc_pre, gpc_preq,
     gpc_pre},
    {gpc_g8, gpc_g8, gpc_noop, gpc_noop, gpc_glin, gpc_glin, gpc_lin, gpc_lin},
    {gpc_g8b, gpc_g8, gpc_bckc, gpc_noop, gpc_gprq, gpc_gpre, gpc_preq,
     gpc_pre},
    {gpc_sRGB, gpc_sRGB, gpc_sRGB, gpc_sRGB, gpc_noop, gpc_noop, gpc_noop,
     gpc_noop},
...
static void transform_from_formats(Transform *result, Image *in_image,
  ...
  if (in_format & ~out_format & PNG_FORMAT_FLAG_ALPHA) {
    ...
    if (out_format & PNG_FORMAT_FLAG_LINEAR || via_linear) {
      if (out_format & PNG_FORMAT_FLAG_COLORMAP) {
        result->background_color.ir = result->background_color.ig =
            result->background_color.ib = 0;
        result->background_color.dr = result->background_color.dg =
...
static void print_pixel(char string[64], const Pixel *pixel,
                        png_uint_32 format) {
  ...
  case PNG_FORMAT_FLAG_ALPHA:
    sprintf(string, "%s(%d,%d)", format_names[format], pixel->g, pixel->a);
    break;

  case PNG_FORMAT_FLAG_COLOR:
    sprintf(string, "%s(%d,%d,%d)", format_names[format], pixel->r, pixel->g,
            pixel->b);
    break;

  case PNG_FORMAT_FLAG_COLOR | PNG_FORMAT_FLAG_ALPHA:
    sprintf(string, "%s(%d,%d,%d,%d)", format_names[format], pixel->r, pixel->g,
            pixel->b, pixel->a);
    break;
...
static int logpixel(const Transform *transform, png_uint_32 x, png_uint_32 y,
...
static int cmppixel(Transform *transform, png_const_voidp in,
                    png_const_voidp out, png_uint_32 x,
                    png_uint_32 y /*or palette index*/) {
  int maxerr;
...
static png_byte component_loc(png_byte loc[4], png_uint_32 format) {
 ...
 */
static int compare_two_images(Image *a, Image *b, int via_linear,
                              png_const_colorp background) {
  ...
  if (width != b->image.width || height != b->image.height)
    return logerror(a, a->file_name,
                    ": width x height changed: ", b->file_name);
  ...
  if (formata & formatb & PNG_FORMAT_FLAG_COLORMAP) {
     ...
     */
    for (y = 0, match = 1; y < height && match;
         ++y, ppa += stridea, ppb += strideb) {
      png_uint_32 x;
...
#define BYTE_CHARS 3 /* 2^8: real maximum value */
  ...
  if (!(((formata ^ formatb) & PNG_FORMAT_FLAG_LINEAR) |
        (formata & (formatb ^ PNG_FORMAT_FLAG_COLOR) &
 ...
 */
static int read_file(Image *image, png_uint_32 format,
                     png_const_colorp background) {
  memset(&image->image, 0, sizeof image->image);
  ...
  if (image->input_memory != NULL) {
    ...
    return logerror(image, "unsupported file/stdio init: ", image->file_name,
                    "");
  }
#endif
   ...
   */
  {
    ...
    if (image->opts & VERBOSE) {
      printf("%s %lu x %lu %s -> %s", image->file_name,
             ...
             (format & FORMAT_NO_CHANGE) != 0 || image->image.format == format
                 ? "no change"
                 : format_names[format & FORMAT_MASK]);
    ...
    result =
        png_image_finish_read(&image->image, background, image->buffer + 16,
      ...
      else /* memory */
      {
        if (fseek(f, 0, SEEK_END) == 0) {
          ...
          if (cb > 0) {
            ...
            return logclose(image, f, image->file_name, ": zero length: ");
...
static int write_one_file(Image *output, Image *image, int convert_to_8bit) {
  ...
  if (image->opts & USE_STDIO) {
    ...
    if (f != NULL) {
      if (png_image_write_to_stdio(
              &image->image, f, convert_to_8bit, image->buffer + 16,
              (png_int_32)image->stride, image->colormap)) {
        if (fflush(f) == 0) {
          rewind(f);
    ...
    if (png_image_write_to_file(&image->image, name, convert_to_8bit,
                                ...
                                image->colormap)) {
      initimage(output, image->opts, output->tmpfile_name, image->stride_extra);
  ...
  else /* use memory */
  {
    ...
    if (png_image_write_get_memory_size(
            image->image, size, convert_to_8bit, image->buffer + 16,
            (png_int_32)image->stride, image->colormap)) {
      ...
      if (output->input_memory != NULL) {
        ...
        if (png_image_write_to_memory(
                &image->image, output->input_memory, &output->input_memory_size,
                convert_to_8bit, image->buffer + 16, (png_int_32)image->stride,
                image->colormap)) {
  ...
  if (read_file(output, image->image.format | FORMAT_NO_CHANGE, NULL)) {
    ...
    if ((output->image.format & BASE_FORMATS) !=
      ...
      return logerror(image, image->file_name,
                      ": format changed on read: ", output->file_name);

    return compare_two_images(image, output, 0 /*via linear*/, NULL);
  }

  else
    return logerror(output, output->tmpfile_name, ": read of new file failed",
                    "");
}
#endif
    ...
    for (counter = 0; counter < 2 * FORMAT_COUNT; ++counter)
      if (format_isset(pf, counter >> 1)) {
        ...
        if ((format & PNG_FORMAT_FLAG_LINEAR) != 0 &&
           ...
           */
          result =
              compare_two_images(image, &output, 1 /*via_linear*/, background);
          if (!result)
            break;
        }
...
static int test_one_file(const char *file_name, format_list *formats,
                         png_uint_32 opts, int stride_extra, int log_pass) {
  int result;
  Image image;
      ...
      if (arg[0] == '-') {
        ...
        do {
          ...
          if (ch == EOF || ch == term || ch == 0) {
            ...
            if (ich > 0 &&
                !test_one_file(buffer, &formats, opts, stride_extra, log_pass))
              retval = 1;
