 ...
 */
#if defined PNG_WRITE_SUPPORTED && (defined PNG_PROGRESSIVE_READ_SUPPORTED) && \
 ...
 */
#define anon_context(ps)                                                       \
  struct exception_context *the_exception_context = &(ps)->exception_context
#define context(ps, fault)                                                     \
  anon_context(ps);                                                            \
  png_store *fault
...
static const char *colour_types[8] = {"grayscale",
                                      invalid,
                                      "truecolour",
                                      "indexed-colour",
...
static void randomize_bytes(void *pv, size_t size) {
...
static int random_choice(void) { return random_byte() & 1; }
...
#define FILEID(col, depth, palette, interlace, width, height, do_interlace)    \
  ((png_uint_32)((col) + ((depth) << 3) + ((palette) << 8) +                   \
                 ((interlace) << 13) + (((do_interlace) != 0) << 15) +         \
                 ((width) << 16) + ((height) << 24)))
...
static size_t standard_name(char *buffer, size_t bufsize, size_t pos,
...
static size_t standard_name_from_id(char *buffer, size_t bufsize, size_t pos,
                                    png_uint_32 id) {
  return standard_name(buffer, bufsize, pos, COL_FROM_ID(id), DEPTH_FROM_ID(id),
                       PALETTE_FROM_ID(id), INTERLACE_FROM_ID(id),
                       WIDTH_FROM_ID(id), HEIGHT_FROM_ID(id),
...
static int next_format(png_bytep colour_type, png_bytep bit_depth,
                       unsigned int *palette_number, int low_depth_gray,
                       int tRNS) {
  if (*bit_depth == 0) {
    *colour_type = 0;
    if (low_depth_gray)
...
#ifdef PNG_READ_TRANSFORMS_SUPPORTED
static unsigned int sample(png_const_bytep row, png_byte colour_type,
...
static void pixel_copy(png_bytep toBuffer, png_uint_32 toIndex,
                       png_const_bytep fromBuffer, png_uint_32 fromIndex,
  ...
  if (pixelSize < 8) /* Sub-byte */
  {
    ...
    memmove(toBuffer + (toIndex >> 3), fromBuffer + (fromIndex >> 3),
            pixelSize >> 3);
}
 ...
 * bytes at the end.
 */
static void row_copy(png_bytep toBuffer, png_const_bytep fromBuffer,
                     unsigned int bitWidth, int littleendian) {
 ...
 * given buffers.
 */
static int pixel_cmp(png_const_bytep pa, png_const_bytep pb,
...
static size_t store_message(png_store *ps, png_const_structp pp, char *buffer,
                            size_t bufsize, size_t pos, const char *msg) {
...
static void store_verbose(png_store *ps, png_const_structp pp,
...
static void store_log(png_store *ps, png_const_structp pp,
                      png_const_charp message, int is_error) {
...
static void PNGCBAPI store_error(png_structp ppIn,
                                 png_const_charp message) /* PNG_NORETURN */
{
...
static void PNGCBAPI store_warning(png_structp ppIn, png_const_charp message) {
  png_const_structp pp = ppIn;
  png_store *ps = voidcast(png_store *, png_get_error_ptr(pp));
...
static png_bytep store_image_row(const png_store *ps, png_const_structp pp,
...
#ifdef PNG_READ_SUPPORTED
static void store_image_check(const png_store *ps, png_const_structp pp,
                              int iImage) {
...
static void PNGCBAPI store_write(png_structp ppIn, png_bytep pb, size_t st) {
  png_const_structp pp = ppIn;
...
static size_t store_read_chunk(png_store *ps, png_bytep pb, size_t max,
                               size_t min) {
  png_uint_32 chunklen = ps->chunklen;
  png_uint_32 chunktype = ps->chunktype;
  png_uint_32 chunkpos = ps->chunkpos;
  ...
  if (st > 0)
    do {
      ...
      if (chunktype == CHUNK_IDAT) {
        ...
        if (IDAT_pos < 8U) /* Return the header */
          do {
            png_uint_32 b;
            unsigned int shift;
      ...
      else /* !IDAT */
      {
        ...
        if (chunkpos < 8U) /* Return the header */
          do {
            png_uint_32 b;
            unsigned int shift;
...
static void PNGCBAPI store_read(png_structp ppIn, png_bytep pb, size_t st) {
  png_const_structp pp = ppIn;
  png_store *ps = voidcast(png_store *, png_get_io_ptr(pp));
...
static void store_progressive_read(png_store *ps, png_structp pp,
                                   png_infop pi) {
...
static store_palette_entry *store_write_palette(png_store *ps, int npalette) {
  ...
  if (npalette > 0) {
    ps->palette =
        voidcast(store_palette_entry *, malloc(npalette * sizeof *ps->palette));
...
#ifdef PNG_READ_SUPPORTED
static store_palette_entry *store_current_palette(png_store *ps,
                                                  int *npalette) {
 ...
 * all the memory.
 */
static void store_pool_error(png_store *ps, png_const_structp pp,
                             const char *msg) {
  if (pp != NULL)
    png_error(pp, msg);
...
static void store_memory_free(png_const_structp pp, store_pool *pool,
                              store_memory *memory) {
  ...
  /* It should be safe to read the size field now. */
    ...
    else if (memcmp((png_bytep)(memory + 1) + cb, pool->mark,
                    sizeof pool->mark) != 0)
...
static void store_pool_delete(png_store *ps, store_pool *pool) {
  if (pool->list != NULL) {
    fprintf(stderr, "%s: %s %s: memory lost (list follows):\n", ps->test,
            ...
            pool == &ps->read_memory_pool
                ? (ps->current != NULL ? ps->current->name : "unknown file")
                : ps->wname);
    ++ps->nerrors;
...
static png_voidp PNGCBAPI store_malloc(png_structp ppIn, png_alloc_size_t cb) {
  png_const_structp pp = ppIn;
  store_pool *pool = voidcast(store_pool *, png_get_mem_ptr(pp));
  store_memory *new = voidcast(
      store_memory *, malloc(cb + (sizeof *new) + (sizeof pool->mark)));
...
static void PNGCBAPI store_free(png_structp ppIn, png_voidp memory) {
  png_const_structp pp = ppIn;
  store_pool *pool = voidcast(store_pool *, png_get_mem_ptr(pp));
 ...
 */
static png_structp set_store_for_write(png_store *ps, png_infopp ppi,
                                       const char *name) {
  anon_context(ps);
...
#ifdef PNG_USER_MEM_SUPPORTED
    if (!ps->speed)
      ps->pwrite = png_create_write_struct_2(
          PNG_LIBPNG_VER_STRING, ps, store_error, store_warning,
 ...
 */
static png_structp set_store_for_read(png_store *ps, png_infopp ppi,
                                      png_uint_32 id, const char *name) {
...
#ifdef PNG_USER_MEM_SUPPORTED
  if (!ps->speed)
    ps->pread = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, ps, store_error,
                                         store_warning, &ps->read_memory_pool,
...
static double chromaticity_x(CIE_color c) { return c.X / (c.X + c.Y + c.Z); }

static double chromaticity_y(CIE_color c) { return c.Y / (c.X + c.Y + c.Z); }
...
static int fail(png_modifier *pm) {
  return !pm->log && !pm->this.verbose &&
         (pm->this.nerrors > 0 ||
          (pm->this.treat_warnings_as_errors && pm->this.nwarnings > 0));
}

static void modifier_init(png_modifier *pm) {
...
static int output_quantization_factor(const png_modifier *pm, int in_depth,
                                      int out_depth) {
  if (out_depth == 16 && in_depth != 16 && pm->calculations_use_input_precision)
    return 257;
  else
    return 1;
}
...
typedef struct png_modification {
  ...
  int (*modify_fn)(struct png_modifier *pm, struct png_modification *me,
                   int add);
...
#ifdef PNG_READ_RGB_TO_GRAY_SUPPORTED
static void modifier_current_encoding(const png_modifier *pm,
                                      color_encoding *ce) {
...
static unsigned int modifier_total_encodings(const png_modifier *pm) {
  return 1 +              /* (1) nothing */
          ...
          * modifier_reset, below, preserves the setting until after it has
          * called the iterate function (also below.)
          *
...
static void modifier_reset(png_modifier *pm) {
 ...
 */
static void modifier_set_encoding(png_modifier *pm) {
 ...
 */
static int modifier_color_encoding_is_sRGB(const png_modifier *pm) {
  return pm->current_encoding != 0 && pm->current_encoding == pm->encodings &&
         pm->current_encoding->gamma == pm->current_gamma;
}

static int modifier_color_encoding_is_set(const png_modifier *pm) {
  return pm->current_gamma != 0;
}

/* The guts of modification are performed during a read. */
static void modifier_crc(png_bytep buffer) {
...
static void modifier_setbuffer(png_modifier *pm) {
 ...
 * png_struct.
 */
static void modifier_read_imp(png_modifier *pm, png_bytep pb, size_t st) {
  while (st > 0) {
    size_t cb;
...
/* The callback: */
static void PNGCBAPI modifier_read(png_structp ppIn, png_bytep pb, size_t st) {
  png_const_structp pp = ppIn;
 ...
 * need a local buffer.
 */
static void modifier_progressive_read(png_modifier *pm, png_structp pp,
                                      png_infop pi) {
  if (pm->this.pread != pp || pm->this.current == NULL || pm->this.next == NULL)
    png_error(pp, "store state damaged (progressive)");
...
/* Set up a modifier. */
static png_structp set_modifier_for_read(png_modifier *pm, png_infopp ppi,
                                         png_uint_32 id, const char *name) {
...
static int gama_modify(png_modifier *pm, png_modification *me, int add) {
  UNUSED(add)
...
static void gama_modification_init(gama_modification *me, png_modifier *pm,
                                   double gammad) {
  double g;
...
static int chrm_modify(png_modifier *pm, png_modification *me, int add) {
  UNUSED(add)
...
static void chrm_modification_init(chrm_modification *me, png_modifier *pm,
                                   const color_encoding *encoding) {
...
static int srgb_modify(png_modifier *pm, png_modification *me, int add) {
  UNUSED(add)
...
static void srgb_modification_init(srgb_modification *me, png_modifier *pm,
                                   png_byte intent) {
  modification_init(&me->this);
...
static int sbit_modify(png_modifier *pm, png_modification *me, int add) {
...
static void sbit_modification_init(sbit_modification *me, png_modifier *pm,
                                   png_byte sbit) {
  modification_init(&me->this);
  me->this.chunk = CHUNK_sBIT;
 ...
 */
static store_palette_entry *make_standard_palette(png_store *ps, int npalette,
                                                  int do_tRNS) {
 ...
 */
static int npasses_from_interlace_type(png_const_structp pp,
                                       int interlace_type) {
...
static unsigned int bit_size(png_const_structp pp, png_byte colour_type,
                             png_byte bit_depth) {
  switch (colour_type) {
  default:
...
static size_t transform_rowsize(png_const_structp pp, png_byte colour_type,
                                png_byte bit_depth) {
...
static void transform_row(png_const_structp pp,
 ...
 */
static void interlace_row(png_bytep buffer, png_const_bytep imageRow,
...
#ifdef PNG_READ_SUPPORTED
static void deinterlace_row(png_bytep buffer, png_const_bytep row,
                            unsigned int pixel_size, png_uint_32 w, int pass,
                            int littleendian) {
...
static void make_transform_image(png_store *const ps,
                                 png_byte const colour_type,
                                 png_byte const bit_depth,
...
static void make_transform_images(png_modifier *pm) {
  ...
  while (next_format(&colour_type, &bit_depth, &palette_number, 1, 1)) {
    ...
    for (interlace_type = PNG_INTERLACE_NONE; interlace_type < INTERLACE_LAST;
         ++interlace_type) {
      char name[FILE_NAME_SIZE];
 ...
 */
static void size_row(png_byte buffer[SIZE_ROWMAX], png_uint_32 bit_width,
                     png_uint_32 y) {
...
static void make_size_image(png_store *const ps, png_byte const colour_type,
      ...
      for (pass = 0; pass < npasses; ++pass) {
        ...
        for (y = 0; y < h; ++y) {
           ...
           */
          {
            ...
            if (pass == 0 && y == 0 && (filters < PNG_FILTER_UP || w == 1U))
              filters |= PNG_FILTER_UP;
...
static void make_size(png_store *const ps, png_byte const colour_type, int bdlo,
                      int const bdhi) {
  for (; bdlo <= bdhi; ++bdlo) {
    png_uint_32 width;
    ...
    for (width = 1; width <= 16; ++width) {
      ...
      for (height = 1; height <= 16; ++height) {
         ...
         * libpng doing it.
         */
        make_size_image(ps, colour_type, DEPTH(bdlo), PNG_INTERLACE_NONE, width,
                        height, 0);
        make_size_image(ps, colour_type, DEPTH(bdlo), PNG_INTERLACE_NONE, width,
                        height, 1);
#ifdef PNG_WRITE_INTERLACING_SUPPORTED
...
static void standard_row(png_const_structp pp, png_byte std[STANDARD_ROWMAX],
  ...
  if (WIDTH_FROM_ID(id) == 0)
    ...
    size_row(std,
             WIDTH_FROM_ID(id) *
                 bit_size(pp, COL_FROM_ID(id), DEPTH_FROM_ID(id)),
             y);
}
#endif /* PNG_READ_SUPPORTED */
...
static void make_error(png_store *const ps, png_byte const colour_type,
                       png_byte bit_depth, int interlace_type, int test,
  ...
  Try {
    ...
    w = transform_width(pp, colour_type, bit_depth);
    gnu_volatile(w) h = transform_height(pp, colour_type, bit_depth);
    gnu_volatile(h)
        png_set_IHDR(pp, pi, w, h, bit_depth, colour_type, interlace_type,
                     PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
...
static int make_errors(png_modifier *const pm, png_byte const colour_type,
  ...
  for (; bdlo <= bdhi; ++bdlo) {
    ...
    for (interlace_type = PNG_INTERLACE_NONE; interlace_type < INTERLACE_LAST;
      ...
      for (test = 0; test < ARRAY_SIZE(error_test); ++test) {
        make_error(&pm->this, colour_type, DEPTH(bdlo), interlace_type, test,
                   name);
...
static void standard_display_init(standard_display *dp, png_store *ps,
 ...
 */
static int read_palette(store_palette palette, int *npalette,
 ...
 * non-transform case).
 */
static void standard_palette_validate(standard_display *dp,
                                      png_const_structp pp, png_infop pi) {
  int npalette;
  store_palette palette;
 ...
 */
static void standard_info_part1(standard_display *dp, png_structp pp,
                                png_infop pi) {
 ...
 * image.
 */
static void standard_info_part2(standard_display *dp, png_const_structp pp,
...
static void standard_info_imp(standard_display *dp, png_structp pp,
                              png_infop pi, int nImages) {
...
static void PNGCBAPI standard_info(png_structp pp, png_infop pi) {
  standard_display *dp =
      voidcast(standard_display *, png_get_progressive_ptr(pp));
...
static void PNGCBAPI progressive_row(png_structp ppIn, png_bytep new_row,
                                     png_uint_32 y, int pass) {
  png_const_structp pp = ppIn;
  const standard_display *dp =
      voidcast(standard_display *, png_get_progressive_ptr(pp));
  ...
  if (new_row != NULL) {
...
#endif /* READ_INTERLACING */
    {
      ...
      png_progressive_combine_row(pp, row, new_row);
           ...
           PNG_ROW_IN_INTERLACE_PASS(y, pass) && PNG_PASS_COLS(dp->w, pass) > 0)
...
static void sequential_row(standard_display *dp, png_structp pp, png_infop pi,
  ...
  int do_interlace =
      dp->do_interlace && dp->interlace_type == PNG_INTERLACE_ADAM7;
  ...
  for (pass = 0; pass < npasses; ++pass) {
    ...
    for (y = 0; y < height; ++y) {
      if (do_interlace) {
        ...
        png_read_row(
            pp, iImage >= 0 ? store_image_row(ps, pp, iImage, y) : NULL,
...
#ifdef PNG_TEXT_SUPPORTED
static void standard_check_text(png_const_structp pp, png_const_textp tp,
  ...
  if (tp->compression != TEXT_COMPRESSION) {
    ...
    sprintf(buf, "compression [%d->%d], ", TEXT_COMPRESSION, tp->compression);
...
static void standard_text_validate(standard_display *dp, png_const_structp pp,
  ...
  if (num_text == 2 && tp != NULL) {
    ...
    sprintf(msg, "expected two text items, got %lu", (unsigned long)num_text);
    png_error(pp, msg);
  }
}
#else
...
static void standard_row_validate(standard_display *dp, png_const_structp pp,
...
static void PNGCBAPI standard_end(png_structp ppIn, png_infop pi) {
  png_const_structp pp = ppIn;
  standard_display *dp =
      voidcast(standard_display *, png_get_progressive_ptr(pp));
  ...
  standard_text_validate(
      dp, pp, pi, PNG_LIBPNG_VER >= 10518 /*check_end: see comments above*/);
  standard_image_validate(dp, pp, 0, -1);
}

/* A single test run checking the standard image to ensure it is not damaged. */
static void standard_test(png_store *const psIn, png_uint_32 const id,
  ...
  Try {
     ...
     */
    pp = set_store_for_read(
        d.ps, &pi, d.id,
        d.do_interlace
            ? (d.ps->progressive ? "pngvalid progressive deinterlacer"
...
static int test_standard(png_modifier *const pm, png_byte const colour_type,
                         int bdlo, int const bdhi) {
  for (; bdlo <= bdhi; ++bdlo) {
    int interlace_type;
    ...
    for (interlace_type = PNG_INTERLACE_NONE; interlace_type < INTERLACE_LAST;
         ++interlace_type) {
      standard_test(&pm->this,
                    FILEID(colour_type, DEPTH(bdlo), 0 /*palette*/,
                           interlace_type, 0, 0, 0),
                    do_read_interlace, pm->use_update_info);
...
static void perform_standard_test(png_modifier *pm) {
...
/********************************** SIZE TESTS ********************************/
static int test_size(png_modifier *const pm, png_byte const colour_type,
                     int bdlo, int const bdhi) {
  ...
  for (; bdlo <= bdhi; ++bdlo) {
    ...
    for (h = 1; h <= 16; h += hinc[bdlo]) {
      for (w = 1; w <= 16; w += winc[bdlo]) {
        ...
        standard_test(&pm->this,
                      FILEID(colour_type, DEPTH(bdlo), 0 /*palette*/,
                             PNG_INTERLACE_NONE, w, h, 0),
                      0 /*do_interlace*/, pm->use_update_info);
        ...
        standard_test(&pm->this,
                      FILEID(colour_type, DEPTH(bdlo), 0 /*palette*/,
                             PNG_INTERLACE_NONE, w, h, 0),
                      1 /*do_interlace*/, pm->use_update_info);
...
static void perform_size_test(png_modifier *pm) {
  /* Test each colour type over the valid range of bit depths (expressed as
   * log2(bit_depth) in turn, stop as soon as any error is detected.
   */
...
static void image_pixel_setf(image_pixel *this, unsigned int rMax,
 ...
 * reset.
 */
static void image_pixel_init(
    image_pixel *this, png_const_bytep row, png_byte colour_type,
  ...
  else /* not palette */
  {
    ...
    if (colour_type & 2) {
      ...
      this->green =
          sample(row, colour_type, bit_depth, x, ++i, swap16, littleendian);
      ...
      if (format != 0 && format->mono_inverted)
   ...
   */
  image_pixel_setf(this, sig_bits ? (1U << format->red_sBIT) - 1 : max,
 ...
 * records this.
 */
static void image_pixel_add_alpha(image_pixel *this,
                                  const standard_display *display,
                                  int for_background) {
  if (this->colour_type == PNG_COLOR_TYPE_PALETTE)
...
typedef struct image_transform {
   ...
   * point running it.
   */
  int (*add)(struct image_transform *this, const struct image_transform **that,
             png_byte colour_type, png_byte bit_depth);
} image_transform;
...
static void image_transform_ini_end(const image_transform *this,
                                    transform_display *that) {
  UNUSED(this)
  UNUSED(that)
}

static void image_transform_set_end(const image_transform *this,
...
static void image_transform_mod_end(const image_transform *this,
 ...
 * ones.
 */
static void transform_display_init(transform_display *dp, png_modifier *pm,
                                   png_uint_32 id,
                                   const image_transform *transform_list) {
  memset(dp, 0, sizeof *dp);
...
static void transform_info_imp(transform_display *dp, png_structp pp,
                               png_infop pi) {
   ...
   */
  {
    ...
    if (test_pixel.colour_type == PNG_COLOR_TYPE_PALETTE &&
      ...
      png_error(pp, "pngvalid: internal: bad unpacked pixel depth");
             ...
             test_pixel.colour_type != PNG_COLOR_TYPE_PALETTE &&
             test_pixel.bit_depth != test_pixel.sample_depth) {
      char message[128];
      size_t pos =
          safecat(message, sizeof message, 0, "internal: sample depth ");
       ...
       */
      char message[128];
      size_t pos = safecat(message, sizeof message, 0, "internal: bit depth ");
...
static void PNGCBAPI transform_info(png_structp pp, png_infop pi) {
  transform_info_imp(voidcast(transform_display *, png_get_progressive_ptr(pp)),
                     pp, pi);
}

static void transform_range_check(png_const_structp pp, unsigned int r,
                                  unsigned int g, unsigned int b,
                                  unsigned int a, unsigned int in_digitized,
                                  double in, unsigned int out,
                                  png_byte sample_depth, double err,
                                  double limit, const char *name,
...
static void transform_image_validate(transform_display *dp,
                                     png_const_structp pp, png_infop pi) {
  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      image_pixel_init(&in_pixel, std, in_ct, in_bd, x, dp->this.palette, NULL);
       ...
       * changed.
       */
      if (in_ct == PNG_COLOR_TYPE_PALETTE && out_ct == PNG_COLOR_TYPE_PALETTE) {
      ...
      if (in_pixel.red != out_pixel.red)
        transform_range_check(pp, r, g, b, a, in_pixel.red, in_pixel.redf,
                              ...
                              dp->pm->limit +
                                  1. / (2 * ((1U << in_pixel.red_sBIT) - 1)),
                              "red/gray", digitization_error);

      if ((out_ct & PNG_COLOR_MASK_COLOR) != 0 &&
          in_pixel.green != out_pixel.green)
        transform_range_check(pp, r, g, b, a, in_pixel.green, in_pixel.greenf,
                              out_pixel.green, sample_depth, in_pixel.greene,
                              dp->pm->limit +
                                  1. / (2 * ((1U << in_pixel.green_sBIT) - 1)),
      ...
      if ((out_ct & PNG_COLOR_MASK_COLOR) != 0 &&
        ...
        transform_range_check(pp, r, g, b, a, in_pixel.blue, in_pixel.bluef,
                              ...
                              dp->pm->limit +
                                  1. / (2 * ((1U << in_pixel.blue_sBIT) - 1)),
                              "blue", digitization_error);

      if ((out_ct & PNG_COLOR_MASK_ALPHA) != 0 &&
          in_pixel.alpha != out_pixel.alpha)
        transform_range_check(pp, r, g, b, a, in_pixel.alpha, in_pixel.alphaf,
                              out_pixel.alpha, sample_depth, in_pixel.alphae,
                              dp->pm->limit +
                                  1. / (2 * ((1U << in_pixel.alpha_sBIT) - 1)),
...
static void PNGCBAPI transform_end(png_structp ppIn, png_infop pi) {
  png_const_structp pp = ppIn;
  transform_display *dp =
      voidcast(transform_display *, png_get_progressive_ptr(pp));
...
/* A single test run. */
static void transform_test(png_modifier *pmIn, png_uint_32 idIn,
...
static int image_transform_default_add(image_transform *this,
...
static void image_transform_png_set_palette_to_rgb_mod(
...
static int image_transform_png_set_palette_to_rgb_add(
    image_transform *this, const image_transform **that, png_byte colour_type,
    png_byte bit_depth) {
  UNUSED(bit_depth)
...
static int image_transform_png_set_tRNS_to_alpha_add(
    image_transform *this, const image_transform **that, png_byte colour_type,
    png_byte bit_depth) {
  UNUSED(bit_depth)
...
/* png_set_gray_to_rgb */
static void image_transform_png_set_gray_to_rgb_set(const image_transform *this,
                                                    transform_display *that,
                                                    png_structp pp,
                                                    png_infop pi) {
...
static int image_transform_png_set_gray_to_rgb_add(image_transform *this,
                                                   const image_transform **that,
                                                   png_byte colour_type,
                                                   png_byte bit_depth) {
  UNUSED(bit_depth)
...
#ifdef PNG_READ_EXPAND_SUPPORTED
/* png_set_expand */
static void image_transform_png_set_expand_set(const image_transform *this,
                                               transform_display *that,
                                               png_structp pp, png_infop pi) {
  png_set_expand(pp);
...
static int image_transform_png_set_expand_add(image_transform *this,
                                              const image_transform **that,
                                              png_byte colour_type,
                                              png_byte bit_depth) {
  UNUSED(bit_depth)
...
static void image_transform_png_set_expand_gray_1_2_4_to_8_mod(
  ...
  if (that->colour_type == PNG_COLOR_TYPE_GRAY && that->bit_depth < 8)
    that->sample_depth = that->bit_depth = 8;
...
static int image_transform_png_set_expand_gray_1_2_4_to_8_add(
    image_transform *this, const image_transform **that, png_byte colour_type,
    png_byte bit_depth) {
#if PNG_LIBPNG_VER != 10700
  return image_transform_png_set_expand_add(this, that, colour_type, bit_depth);
#else
  UNUSED(bit_depth)
...
/* png_set_expand_16 */
static void image_transform_png_set_expand_16_set(const image_transform *this,
                                                  transform_display *that,
                                                  png_structp pp,
                                                  png_infop pi) {
...
static int image_transform_png_set_expand_16_add(image_transform *this,
                                                 const image_transform **that,
                                                 png_byte colour_type,
                                                 png_byte bit_depth) {
  UNUSED(colour_type)
...
/* png_set_scale_16 */
static void image_transform_png_set_scale_16_set(const image_transform *this,
                                                 transform_display *that,
                                                 png_structp pp, png_infop pi) {
  png_set_scale_16(pp);
#if PNG_LIBPNG_VER != 10700
...
static int image_transform_png_set_scale_16_add(image_transform *this,
                                                const image_transform **that,
                                                png_byte colour_type,
                                                png_byte bit_depth) {
  UNUSED(colour_type)
...
/* png_set_strip_16 */
static void image_transform_png_set_strip_16_set(const image_transform *this,
                                                 transform_display *that,
                                                 png_structp pp, png_infop pi) {
  png_set_strip_16(pp);
#if PNG_LIBPNG_VER != 10700
...
static int image_transform_png_set_strip_16_add(image_transform *this,
                                                const image_transform **that,
                                                png_byte colour_type,
                                                png_byte bit_depth) {
  UNUSED(colour_type)
...
/* png_set_strip_alpha */
static void image_transform_png_set_strip_alpha_set(const image_transform *this,
                                                    transform_display *that,
                                                    png_structp pp,
                                                    png_infop pi) {
...
static int image_transform_png_set_strip_alpha_add(image_transform *this,
                                                   const image_transform **that,
                                                   png_byte colour_type,
                                                   png_byte bit_depth) {
  UNUSED(bit_depth)
...
static void image_transform_png_set_rgb_to_gray_ini(const image_transform *this,
                                                    transform_display *that) {
...
#endif
      that->pm->limit +=
          pow((that->this.bit_depth == 16 || that->max_gamma_8 > 14
                   ? 8.
...
static void image_transform_png_set_rgb_to_gray_set(const image_transform *this,
                                                    transform_display *that,
                                                    png_structp pp,
                                                    png_infop pi) {
  ...
  if ((that->colour_type & PNG_COLOR_MASK_COLOR) != 0) {
...
#if DIGITIZE
    {
      ...
      unsigned int calc_depth =
          (pm->assume_16_bit_calculations ? 16 : sample_depth);
      unsigned int gamma_depth =
          (sample_depth == 16
               ? display->max_gamma_8
               : (pm->assume_16_bit_calculations ? display->max_gamma_8
                                                 : sample_depth));
      int isgray;
      double r, g, b;
      ...
      if (r == g && r == b) {
        ...
        if (data.gamma == 1) {
          ...
          err = re * data.red_coefficient + ge * data.green_coefficient +
                be * data.blue_coefficient + 2. / 32768 +
                gray * 5 * DBL_EPSILON;
        }
          ...
          double out_qe =
              (that->sample_depth > 8 ? .5 / 65535
                                      : (pm->assume_16_bit_calculations
                                             ? .5 / (1 << display->max_gamma_8)
                                             : .5 / 255));
...
static int image_transform_png_set_rgb_to_gray_add(image_transform *this,
                                                   const image_transform **that,
                                                   png_byte colour_type,
                                                   png_byte bit_depth) {
  UNUSED(bit_depth)
...
static void image_transform_png_set_background_set(const image_transform *this,
                                                   transform_display *that,
                                                   png_structp pp,
                                                   png_infop pi) {
  ...
  image_pixel_init(&data, random_bytes, colour_type, bit_depth, 0 /*x*/,
                   0 /*unused: palette*/, NULL /*format*/);
...
image_transform_png_set_background_mod(const image_transform *this,
  ...
  if (that->alphaf < 1) {
    ...
    else /* 0 < alpha < 1 */
    {
      ...
      that->rede = that->rede * that->alphaf + data.rede * alf + DBL_EPSILON;
      that->greenf = that->greenf * that->alphaf + data.greenf * alf;
      that->greene =
          that->greene * that->alphaf + data.greene * alf + DBL_EPSILON;
      that->bluef = that->bluef * that->alphaf + data.bluef * alf;
      that->bluee = that->bluee * that->alphaf + data.bluee * alf + DBL_EPSILON;
    }
...
static void image_transform_png_set_invert_alpha_mod(
    const image_transform *this, image_pixel *that, png_const_structp pp,
    const transform_display *display) {
...
static int image_transform_png_set_invert_alpha_add(
    image_transform *this, const image_transform **that, png_byte colour_type,
    png_byte bit_depth) {
  UNUSED(bit_depth)
 ...
 */
static void image_transform_png_set_bgr_set(const image_transform *this,
                                            transform_display *that,
                                            png_structp pp, png_infop pi) {
  png_set_bgr(pp);
  this->next->set(this->next, that, pp, pi);
...
static void image_transform_png_set_bgr_mod(const image_transform *this,
                                            image_pixel *that,
...
static int image_transform_png_set_bgr_add(image_transform *this,
                                           const image_transform **that,
                                           png_byte colour_type,
                                           png_byte bit_depth) {
  UNUSED(bit_depth)
 ...
 */
static void image_transform_png_set_swap_alpha_set(const image_transform *this,
                                                   transform_display *that,
                                                   png_structp pp,
                                                   png_infop pi) {
...
static int image_transform_png_set_swap_alpha_add(image_transform *this,
                                                  const image_transform **that,
                                                  png_byte colour_type,
                                                  png_byte bit_depth) {
  UNUSED(bit_depth)
 ...
 */
static void image_transform_png_set_swap_set(const image_transform *this,
                                             transform_display *that,
                                             png_structp pp, png_infop pi) {
  png_set_swap(pp);
  this->next->set(this->next, that, pp, pi);
...
static void image_transform_png_set_swap_mod(const image_transform *this,
                                             image_pixel *that,
                                             png_const_structp pp,
                                             const transform_display *display) {
  if (that->bit_depth == 16)
...
static int image_transform_png_set_swap_add(image_transform *this,
                                            const image_transform **that,
                                            png_byte colour_type,
                                            png_byte bit_depth) {
  UNUSED(colour_type)
...
static void image_transform_png_set_filler_set(const image_transform *this,
                                               transform_display *that,
                                               png_structp pp, png_infop pi) {
...
image_transform_png_set_filler_mod(const image_transform *this,
                                   image_pixel *that, png_const_structp pp,
                                   const transform_display *display) {
  if (that->bit_depth >= 8 && (that->colour_type == PNG_COLOR_TYPE_RGB ||
                               that->colour_type == PNG_COLOR_TYPE_GRAY)) {
...
static int image_transform_png_set_filler_add(image_transform *this,
                                              const image_transform **that,
                                              png_byte colour_type,
                                              png_byte bit_depth) {
  this->next = *that;
...
static void image_transform_png_set_add_alpha_set(const image_transform *this,
                                                  transform_display *that,
                                                  png_structp pp,
                                                  png_infop pi) {
  /* Need a random choice for 'before' and 'after' as well as for the
...
static int image_transform_png_set_add_alpha_add(image_transform *this,
                                                 const image_transform **that,
                                                 png_byte colour_type,
                                                 png_byte bit_depth) {
  this->next = *that;
 ...
 * per pixel.
 */
static void image_transform_png_set_packing_set(const image_transform *this,
                                                transform_display *that,
                                                png_structp pp, png_infop pi) {
  png_set_packing(pp);
  that->unpacked = 1;
...
static int image_transform_png_set_packing_add(image_transform *this,
                                               const image_transform **that,
                                               png_byte colour_type,
                                               png_byte bit_depth) {
  UNUSED(colour_type)
 ...
 */
static void image_transform_png_set_packswap_set(const image_transform *this,
                                                 transform_display *that,
                                                 png_structp pp, png_infop pi) {
  png_set_packswap(pp);
...
static int image_transform_png_set_packswap_add(image_transform *this,
                                                const image_transform **that,
                                                png_byte colour_type,
                                                png_byte bit_depth) {
  UNUSED(colour_type)
 ...
 */
static void image_transform_png_set_invert_mono_set(const image_transform *this,
                                                    transform_display *that,
                                                    png_structp pp,
                                                    png_infop pi) {
...
static int image_transform_png_set_invert_mono_add(image_transform *this,
                                                   const image_transform **that,
                                                   png_byte colour_type,
                                                   png_byte bit_depth) {
  UNUSED(bit_depth)
...
static void image_transform_png_set_shift_set(const image_transform *this,
                                              transform_display *that,
                                              png_structp pp, png_infop pi) {
...
static int image_transform_png_set_shift_add(image_transform *this,
                                             const image_transform **that,
                                             png_byte colour_type,
                                             png_byte bit_depth) {
  UNUSED(bit_depth)
...
static void image_transform_png_set_
    @_set(const image_transform *this, transform_display *that, png_structp pp,
          png_infop pi) {
  png_set_ @(pp);
  this->next->set(this->next, that, pp, pi);
}
...
static void image_transform_png_set_
    @_mod(const image_transform *this, image_pixel *that, png_const_structp pp,
          const transform_display *display) {
  this->next->mod(this->next, that, pp, display);
}

static int image_transform_png_set_
    @_add(image_transform *this, const image_transform **that,
          png_byte colour_type, png_byte bit_depth) {
  this->next = *that;
  *that = this;
...
static void transform_enable(const char *name) {
...
static void transform_disable(const char *name) {
  image_transform *list = image_transform_first;
...
static int image_transform_test_counter(png_uint_32 counter, unsigned int max) {
...
static png_uint_32 image_transform_add(const image_transform **this,
                                       unsigned int max, png_uint_32 counter,
                                       char *name, size_t sizeof_name,
                                       size_t *pos, png_byte colour_type,
                                       png_byte bit_depth) {
...
static void perform_transform_test(png_modifier *pm) {
  png_byte colour_type = 0;
  png_byte bit_depth = 0;
  unsigned int palette_number = 0;
  ...
  while (next_format(&colour_type, &bit_depth, &palette_number, pm->test_lbg,
    ...
    for (;;) {
       ...
       * command line.
       */
      counter = image_transform_add(&list, 1 /*max*/, counter, name,
      ...
      do {
        pm->repeat = 0;
        transform_test(pm,
                       FILEID(colour_type, bit_depth, palette_number,
...
gamma_display_init(gamma_display *dp, png_modifier *pm, png_uint_32 id,
                   ...
                   int expand16, int do_background,
                   const png_color_16 *pointer_to_the_background_color,
                   double background_gamma) {
  /* Standard fields */
  standard_display_init(&dp->this, &pm->this, id, do_read_interlace,
                        pm->use_update_info);
  ...
  if (dp->do_background >= ALPHA_MODE_OFFSET) {
    ...
    if (dp->do_background) {
      ...
      png_set_background(pp, &dp->background_color, dp->do_background,
                         0 /*need_expand*/, bg);
#else
      png_set_background_fixed(pp, &dp->background_color, dp->do_background,
                               0 /*need_expand*/, g);
#endif
#else
...
static void PNGCBAPI gamma_info(png_structp pp, png_infop pi) {
...
static void init_validate_info(validate_info *vi, gamma_display *dp,
                               png_const_structp pp, int in_depth,
                               int out_depth) {
 ...
 */
static double gamma_component_compose(int do_background, double input_sample,
                                      double alpha, double background,
                                      int *compose) {
...
static double gamma_component_validate(
    const char *name, const validate_info *vi, unsigned int id, unsigned int od,
   ...
   */
  {
    ...
    if (alpha >= 0) {
      ...
      tcompose = 0;
      input_sample = gamma_component_compose(do_background, input_sample, alpha,
                                             background, &tcompose);
    ...
    (defined PNG_READ_ALPHA_MODE_SUPPORTED)
        ...
        if ((!compose && alpha >= 0) || pass != 0) {
          ...
          if (vi->gamma_correction > 0) {
            ...
            pos = safecat(msg, sizeof msg, pos, " [no gamma correction] =");
...
static void gamma_image_validate(gamma_display *dp, png_const_structp pp,
                                 png_infop pi) {
   ...
   */
  init_validate_info(&vi, dp, pp, in_ct == 3 ? 8 : in_bd,
                     out_ct == 3 ? 8 : out_bd);
  ...
  for (y = 0; y < h; ++y) {
    ...
    if (processing) {
      ...
      for (x = 0; x < w; ++x) {
        ...
        if ((in_ct & PNG_COLOR_MASK_ALPHA) != 0 ||
          ...
          if (out_ct == 3) {
            ...
            output_alpha =
                sample(pRow, out_ct, out_bd, x, samples_per_pixel, 0, 0);
        ...
        if ((in_ct & PNG_COLOR_MASK_COLOR) == 0) /* grayscale */
          (void)gamma_component_validate(
              "gray", &vi, sample(std, in_ct, in_bd, x, 0, 0, 0),
              sample(pRow, out_ct, out_bd, x, 0, 0, 0), alpha /*component*/,
              vi.background_red);
        else /* RGB or palette */
        {
          (void)gamma_component_validate(
      ...
      sprintf(msg, "gamma: below threshold row %lu changed", (unsigned long)y);
...
static void PNGCBAPI gamma_end(png_structp ppIn, png_infop pi) {
  png_const_structp pp = ppIn;
 ...
 */
static void gamma_test(png_modifier *pmIn, png_byte colour_typeIn,
                       png_byte bit_depthIn, int palette_numberIn,
                       int interlace_typeIn, const double file_gammaIn,
                       const double screen_gammaIn, png_byte sbitIn,
                       int threshold_testIn, const char *name,
                       int use_input_precisionIn, int scale16In, int expand16In,
                       int do_backgroundIn, const png_color_16 *bkgd_colorIn,
                       double bkgd_gammaIn) {
  gamma_display d;
  context(&pmIn->this, fault);

  gamma_display_init(&d, pmIn,
                     FILEID(colour_typeIn, bit_depthIn, palette_numberIn,
                            interlace_typeIn, 0, 0, 0),
                     file_gammaIn, screen_gammaIn, sbitIn, threshold_testIn,
                     use_input_precisionIn, scale16In, expand16In,
                     do_backgroundIn, bkgd_colorIn, bkgd_gammaIn);
...
static void gamma_threshold_test(png_modifier *pm, png_byte colour_type,
  ...
  (void)gamma_test(pm, colour_type, bit_depth, 0 /*palette*/, interlace_type,
                   file_gamma, screen_gamma, 0 /*sBIT*/, 1 /*threshold test*/,
                   name, 0 /*no input precision*/, 0 /*no scale16*/,
                   0 /*no expand16*/, 0 /*no background*/, 0 /*hence*/,
                   0 /*no background gamma*/);
}

static void perform_gamma_threshold_tests(png_modifier *pm) {
  png_byte colour_type = 0;
  png_byte bit_depth = 0;
  unsigned int palette_number = 0;
...
static void gamma_transform_test(png_modifier *pm, png_byte colour_type,
                                 png_byte bit_depth, int palette_number,
          ...
          for (j = 0; j < pm->ngamma_tests; ++j) {
            if (i != j) {
              gamma_transform_test(pm, colour_type, bit_depth, npalette,
                                   pm->interlace_type, 1 / pm->gammas[i],
                                   pm->gammas[j], sbit,
                                   pm->use_input_precision_sbit, 0 /*scale16*/);
...
#if defined(PNG_READ_BACKGROUND_SUPPORTED) ||                                  \
    defined(PNG_READ_ALPHA_MODE_SUPPORTED)
static void gamma_composition_test(png_modifier *pm, png_byte colour_type,
                                   png_byte bit_depth, int palette_number,
                                   int interlace_type, const double file_gamma,
...
#if DIGITIZE
    if (expand_16 &&
        (do_background == PNG_BACKGROUND_GAMMA_UNIQUE ||
...
static void perform_gamma_composition_tests(png_modifier *pm, int do_background,
                                            int expand_16) {
  png_byte colour_type = 0;
  png_byte bit_depth = 0;
  unsigned int palette_number = 0;
      ...
      for (i = 0; i < pm->ngamma_tests; ++i) {
        for (j = 0; j < pm->ngamma_tests; ++j) {
          gamma_composition_test(pm, colour_type, bit_depth, palette_number,
                                 pm->interlace_type, 1 / pm->gammas[i],
                                 pm->gammas[j], pm->use_input_precision,
...
static void print_one(const char *leader, double err) {
  if (err != -1.)
    printf(" %s %.5f\n", leader, err);
}

static void summarize_gamma_errors(png_modifier *pm, png_const_charp who,
...
static void perform_gamma_test(png_modifier *pm, int summary) {
  ...
  if (pm->test_gamma_transform) {
    ...
    if (summary)
      summarize_gamma_errors(pm, NULL /*who*/, 1 /*low bit depth*/,
                             1 /*indexed*/);
 ...
 */
static const png_byte adam7[8][8] = {
...
int main(int argc, char **argv) {
   ...
   */
  static double gammas[] = {2.2, 1.0, 2.2 / 1.45, 1.8, 1.5,
                            2.4, 2.5, 2.62,       2.9};
  ...
  gnu_volatile(summary) gnu_volatile(memstats) gnu_volatile(touch)
  ...
  while (--argc >= 1) {
    ...
    if (strcmp(*argv, "-v") == 0)
    ...
    else if (strcmp(*argv, "-w") == 0 || strcmp(*argv, "--strict") == 0)
      ...
      transform_disable(*argv + sizeof "--transform-disable");
                     ...
                     "--transform-enable=", sizeof "--transform-enable") == 0) {
      pm.test_transform = 1;
      ...
      pm.test_lbg = pm.test_lbg_gamma_threshold = pm.test_lbg_gamma_transform =
          pm.test_lbg_gamma_sbit = pm.test_lbg_gamma_composition = 1;
      ...
      pm.calculations_use_input_precision = pm.assume_16_bit_calculations = 0;
  ...
  if (pm.ngamma_tests > 0 && pm.test_gamma_threshold == 0 &&
      pm.test_gamma_transform == 0 && pm.test_gamma_sbit == 0 &&
      pm.test_gamma_scale16 == 0 && pm.test_gamma_background == 0 &&
      pm.test_gamma_alpha_mode == 0) {
    pm.test_gamma_threshold = 1;
    pm.test_gamma_transform = 1;
    pm.test_gamma_sbit = 1;
  ...
  if (summary) {
    printf("%s: %s (%s point arithmetic)\n",
           (pm.this.nerrors ||
            (pm.this.treat_warnings_as_errors && pm.this.nwarnings))
               ? "FAIL"
               : "PASS",
           command,
