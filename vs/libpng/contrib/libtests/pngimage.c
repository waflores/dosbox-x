...
#define T(name, chunk, cm_required, cm_absent, bd, when)                       \
  {#name,                                                                      \
   PNG_TRANSFORM_##name,                                                       \
   CHUNK_##chunk,                                                              \
   COLOR_MASK_##cm_required,                                                   \
   COLOR_MASK_##cm_absent,                                                     \
   BD_##bd,                                                                    \
   TRANSFORM_##when,                                                           \
   0 /*!tested*/}
...
static void buffer_start_write(struct buffer *buffer) {
...
static void buffer_start_read(struct buffer *buffer) {
 ...
 */
static int buffer_from_file(struct buffer *buffer, FILE *fp) {
...
static void display_cache_file(struct display *dp, const char *filename)
...
static void buffer_read(struct display *dp, struct buffer *bp, png_bytep data,
  ...
  while (size > 0) {
    ...
    if (last == NULL || (last == bp->last && read_count >= bp->end_count)) {
...
static void read_png(struct display *dp, struct buffer *bp,
                     const char *operation, int transforms) {
  png_structp pp;
  png_infop ip;
 ...
 */
{
  ...
  if (!png_get_IHDR(pp, ip, &dp->width, &dp->height, &dp->bit_depth,
                    &dp->color_type, &dp->interlace_method,
...
#define C(item)                                                                \
  ...
  display_log(dp, APP_WARNING, "IHDR " #item "(%lu) changed to %lu",           \
              (unsigned long)dp->item, (unsigned long)item),                   \
      e = #item
   ...
   */
  {
    unsigned long chunks = png_get_valid(dp->read_pp, dp->read_ip, 0xffffffff);
   ...
   */
  {
    ...
    if ((applied_transforms & PNG_TRANSFORM_SHIFT) == 0 ||
      ...
      for (y = 0; y < height; ++y) {
        ...
        if (memcmp(row, orig, rowbytes - (mask != 0)) != 0 ||
            (mask != 0 &&
          ...
          display_log(dp, APP_FAIL, "byte(%lu,%lu) changed 0x%.2x -> 0x%.2x",
...
#ifdef PNG_sBIT_SUPPORTED
    {
      ...
      default:
        display_log(dp, LIBPNG_ERROR, "invalid colour type %d", color_type);
        /*NOTREACHED*/
...

      {
        ...
        for (b = 0; 8 * b < bpp; ++b) {
          ...
          if (sig_bits[b] == 0 || sig_bits[b] > bit_depth /*!palette*/)
            display_log(
                dp, LIBPNG_BUG,
                "invalid sBIT[%u]  value %d returned for PNG bit depth %d", b,
                sig_bits[b], bit_depth);
        }
      }
      ...
      if (bpp < 8 && bpp != bit_depth) {
         ...
         * code above.
         */
        display_log(dp, INTERNAL_ERROR, "invalid bpp %u for bit_depth %u", bpp,
                    bit_depth);
      }
      ...
      if (mask != 0) {
        ...
        if (bpp != 1 || mask == 0)
          display_log(dp, INTERNAL_ERROR, "mask calculation error %u, %u", bpp,
                      mask);
      }
        ...
        for (x = 0; x < (width - (mask != 0)); ++x) {
          ...
          for (b = 0; b < bpp; ++b) {
            if ((*row++ & sig_bits[b]) != (*orig++ & sig_bits[b])) {
              display_log(
                  dp, APP_FAIL,
                  "significant bits at (%lu[%u],%lu) changed %.2x->%.2x", x, b,
                  y, orig[-1], row[-1]);
              return 0;
            }
          }
...
#ifdef PNG_WRITE_PNG_SUPPORTED
static void buffer_write(struct display *dp, struct buffer *buffer,
                         png_bytep data, size_t size)
...
static void write_png(struct display *dp, png_infop ip, int transforms) {
   ...
   */
  if (transforms & (PNG_TRANSFORM_PACKING | PNG_TRANSFORM_STRIP_FILLER |
                    PNG_TRANSFORM_STRIP_FILLER_BEFORE)) {
    int ct = dp->color_type;

    if (transforms &
        (PNG_TRANSFORM_STRIP_FILLER | PNG_TRANSFORM_STRIP_FILLER_BEFORE))
      ct &= ~PNG_COLOR_MASK_ALPHA;

    png_set_IHDR(dp->write_pp, ip, dp->width, dp->height, dp->bit_depth, ct,
                 dp->interlace_method, dp->compression_method,
                 dp->filter_method);
  }
...
static void test_one_file(struct display *dp, const char *filename) {
    ...
    for (;;) {
      ...
      if (exhaustive) /* all combinations */
      {
        ...
        while ((next & read_transforms) <= current ||
               ...
               || (next & bad_transforms) != 0 || skip_transform(dp, next));
    ...
    if (dp->options & FIND_BAD_COMBOS) {
      ...
      if (bad_combo != ~0U)
        printf("%s[0x%x]: PROBLEM: 0x%x[0x%x] ANTIDOTE: 0x%x\n", dp->filename,
               active, bad_combo, bad_combo_list,
...
static int do_test(struct display *dp, const char *file)
