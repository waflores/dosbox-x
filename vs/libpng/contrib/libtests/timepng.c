...
static PNG_CALLBACK(void, no_warnings,
                    (png_structp png_ptr, png_const_charp warning)) {
  (void)png_ptr;
  (void)warning;
}
...
static int read_png(FILE *fp, png_int_32 transforms, FILE *write_file) {
  png_structp png_ptr =
      png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, 0, no_warnings);
...
static void usage(FILE *fp) {
  ...
  fprintf(
      ...
      "  Read the files into <assembly>, output the count.  Options are "
      "ignored.\n"
      " timepng --dissemble <assembly> <count> [options]\n"
      "  Time <count> files from <assembly>, additional files may not be "
      "given.\n"
      " Otherwise:\n"
  ...
  fprintf(stderr,
          "{files}:\n"
          "  PNG files to copy into the assembly and time.  Invalid files are "
          "skipped\n"
          "  with appropriate error messages.  If no files are given the list "
          "of files\n"
          "  is read from stdin with each file name terminated by a newline\n"
          "Output:\n"
          "  For --assemble the output is the name of the assembly file "
          "followed by the\n"
          "  count of the files it contains; the arguments for --dissemble.  "
          "Otherwise\n"
...
int main(int argc, char **argv) {
  ...
  if (argc > 2 && strcmp(argv[1], "--assemble") == 0) {
    ...
    nfiles = atoi(argv[3]);
    if (nfiles <= 0) {
      fprintf(stderr, "timepng --dissemble <file> <count>: %s is not a count\n",
              argv[3]);
      exit(99);
    }
#ifdef __COVERITY__
...
#define OPT(name)                                                              \
  else if (strcmp(opt, #name) == 0) transforms |= PNG_TRANSFORM_##name
