 ...
 */
#define IMAGE_LICENSING                                                        \
  "Dedicated to the public domain per Creative Commons "                       \
 ...
 * an unsigned:
 */
static unsigned int flooru(double d) {
  d = floor(d);
  return (unsigned int)d;
}

static png_byte floorb(double d) {
  d = floor(d);
  return (png_byte)d;
}
...
static unsigned int channels_of_type(int color_type) {
...
static unsigned int pixel_depth_of_type(int color_type, int bit_depth) {
  return channels_of_type(color_type) * bit_depth;
}

static unsigned int image_size_of_type(int color_type, int bit_depth,
...
static void set_color(png_colorp color, png_bytep trans, unsigned int red,
...
static int generate_palette(png_colorp palette, png_bytep trans, int bit_depth,
...
#define xyinterp(x, y, c1, c2, c3, c4)                                         \
  (((size * size / 2) +                                                        \
   ...
   (size * size))
          ...
          set_color(palette + ip, trans + ip,
                    ...
                    /* alpha:        0, 102, 204, 255) */
                    xyinterp(x, y, 0, 102, 204, 255), gamma_table);
        }
      }
...
static void set_value(png_bytep row, size_t rowbytes, png_uint_32 x,
...
generate_row(png_bytep row, size_t rowbytes, unsigned int y, int color_type,
  ...
  if (colors[0] == 0 && small) {
    ...
    case 64:
      ...
      while (rowbytes > 0) {
        ...
        if (y == 0)
          ...
          const size_t byte_offset = rowbytes - pixel_index * pixel_depth;

          row[rowbytes] = 0xFFU & (pixel_index + (byte_offset * 2 * y) + 1);
        }
      }
      filters = (y == 0 ? PNG_FILTER_SUB + PNG_FILTER_UP : PNG_FILTER_UP);
      break;
    ...
    switch (channels_of_type(color_type)) {
      /* 1 channel: a square image with a diamond, the least luminous colors are
       * on the edge of the image, the most luminous in the center.
       */
    case 1: {
      png_uint_32 x;
      ...
      /* 2 channels: the color channel increases in luminosity from top to
       * bottom, the alpha channel increases in opacity from left to right.
       */
    case 2: {
    ...
    case 3: {
       ...
       */
      for (x = 0; x <= size_max; ++x) {
        set_value(row, rowbytes, 3 * x + 0, bit_depth, /* red */ Y, gamma_table,
                  conv);
        set_value(row, rowbytes, 3 * x + 1, bit_depth, /* green */
                  (depth_max * x * 2 + size_max) / (2 * size_max), gamma_table,
                  conv);
        set_value(row, rowbytes, 3 * x + 2, bit_depth, /* blue */
                  (Y * x * 2 + size_max) / (2 * size_max), gamma_table, conv);
      }
    } break;
    ...
    case 4: {
      ...
      for (x = 0; x <= size_max; ++x) {
        ...
        set_value(row, rowbytes, 4 * x + 2, bit_depth, /* blue */
                  Y - (Y * x * 2 + size_max) / (2 * size_max), gamma_table,
                  conv);
    ...
    memset(row, 0, rowbytes);
           ...
           channels_of_type(color_type)) switch (channels_of_type(color_type)) {
  case 1: {
  ...
  case 2: {
    ...
    for (x = 0; x < size_max; ++x) {
      set_value(row, rowbytes, 2 * x, bit_depth, luma, gamma_table, conv);
      set_value(row, rowbytes, 2 * x + 1, bit_depth, alpha, gamma_table, conv);
    }
  } break;
  ...
  case 3: {
    ...
    for (x = 0; x <= size_max; ++x) {
      set_value(row, rowbytes, 3 * x + 0, bit_depth, red, gamma_table, conv);
      set_value(row, rowbytes, 3 * x + 1, bit_depth, green, gamma_table, conv);
      set_value(row, rowbytes, 3 * x + 2, bit_depth, blue, gamma_table, conv);
    }
  } break;
  ...
  case 4: {
    ...
    for (x = 0; x <= size_max; ++x) {
      set_value(row, rowbytes, 4 * x + 0, bit_depth, red, gamma_table, conv);
      set_value(row, rowbytes, 4 * x + 1, bit_depth, green, gamma_table, conv);
      set_value(row, rowbytes, 4 * x + 2, bit_depth, blue, gamma_table, conv);
      set_value(row, rowbytes, 4 * x + 3, bit_depth, alpha, gamma_table, conv);
    }
  } break;
  ...
  default:
    ...
    fprintf(stderr, "makepng: --color: count(%u) does not match channels(%u)\n",
...
static void PNGCBAPI makepng_warning(png_structp png_ptr,
...
static void PNGCBAPI makepng_error(png_structp png_ptr,
                                   png_const_charp message) {
...
write_png(const char **name, FILE *fp, int color_type, int bit_depth,
  ...
  png_structp png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, name,
                                                makepng_error, makepng_warning);
...

  {
    ...
    if (color_type & PNG_COLOR_MASK_PALETTE) {
      ...
      npalette =
          generate_palette(palette, trans, bit_depth, gamma_table, colors);
      ...
      for (pass = 0; pass < passes; ++pass) {
        ...
        for (y = 0; y < ysize; ++y) {
          unsigned int row_filters =
              generate_row(row, rowbytes, y, color_type, bit_depth, gamma_table,
                           conv, colors, small);
...
static size_t load_file(png_const_charp name, png_bytepp result) {
  FILE *fp = tmpfile();
...
static size_t load_fake(png_charp param, png_bytepp profile) {
  char *endptr = NULL;
...
static void check_param_count(int nparams, int expect) {
  if (nparams != expect) {
...
static void insert_iCCP(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  ...
  case '9': {
    ...
    if (fake_len > 0) /* else a simple parameter */
    {
      ...
      {
        fprintf(stderr, "%s: fake data too long (%lu) for an ICC profile\n",
...
static void clear_text(png_text *text, png_charp keyword) {
...
static void set_text(png_structp png_ptr, png_infop info_ptr, png_textp text,
                     png_charp param) {
  switch (param[0]) {
  case '<': {
    png_bytep file = NULL;
...
static void insert_tEXt(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  png_text text;
...
static void insert_zTXt(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  png_text text;
...
static void insert_iTXt(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  png_text text;
...
static void insert_hIST(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  int i;
...
static png_byte bval(png_const_structrp png_ptr, png_charp param,
                     unsigned int maxval) {
  char *endptr = NULL;
...
static void insert_sBIT(png_structp png_ptr, png_infop info_ptr, int nparams,
                        png_charpp params) {
  int ct = png_get_color_type(png_ptr, info_ptr);
  int c =
      (ct & PNG_COLOR_MASK_COLOR ? 3 : 1) + (ct & PNG_COLOR_MASK_ALPHA ? 1 : 0);
  unsigned int maxval =
...
static int find_parameters(png_const_charp what, png_charp param,
                           png_charp *list, int nparams) {
...
static void bad_parameter_count(png_const_charp what, int nparams) {
  fprintf(stderr, "--insert %s: bad parameter count %d\n", what, nparams);
  exit(1);
}

static chunk_insert *make_insert(png_const_charp what,
                                 void (*insert)(png_structp, png_infop, int,
                                                png_charpp),
  ...
  cip =
      malloc(offsetof(chunk_insert, parameters) + nparams * sizeof(png_charp));
...
static chunk_insert *find_insert(png_const_charp what, png_charp param) {
  png_uint_32 chunk = 0;
 ...
 */
static png_charp strstash(png_const_charp foo) {
...
static png_charp strstash_list(const png_const_charp *text) {
  size_t foo = 0;
 ...
 */
static chunk_insert *add_tEXt(const char *key, const png_const_charp *text) {
...
static chunk_insert *add_iTXt(const char *key, const char *language,
 ...
 * combinations.
 */
static void parse_color(char *arg, unsigned int *colors) {
...
int main(int argc, char **argv) {
  ...
  while (--argc > 0) {
    ...
    if (strncmp(arg, "gray", 4) == 0) {
      ...
      else if (strcmp(arg + 4, "a") == 0 || strcmp(arg + 4, "alpha") == 0 ||
    ...
    if (strncmp(arg, "rgb", 3) == 0) {
      ...
      else if (strcmp(arg + 3, "a") == 0 || strcmp(arg + 3, "alpha") == 0 ||
  ...
  {
    unsigned int lim =
        (color_type == PNG_COLOR_TYPE_PALETTE ? 255U : (1U << bit_depth) - 1);
    unsigned int i;
  ...
  {
    static png_const_charp copyright[] = {COPYRIGHT, /* ISO-Latin-1 */
                                          NULL};
    static png_const_charp licensing[] = {IMAGE_LICENSING, /* UTF-8 */
                                          NULL};
