...
void PngFileInitialize(HWND hwnd) {
  static TCHAR szFilter[] =
      TEXT("PNG Files (*.PNG)\0*.png\0") TEXT("All Files (*.*)\0*.*\0\0");
...
BOOL PngLoadImage(PTSTR pstrFileName, png_byte **ppbImageData, int *piWidth,
  ...
  Try {
    ...
    png_get_IHDR(png_ptr, info_ptr, piWidth, piHeight, &iBitDepth, &iColorType,
                 NULL, NULL, NULL);
    ...
    if (png_get_bKGD(png_ptr, info_ptr, &pBackground)) {
      png_set_background(png_ptr, pBackground, PNG_BACKGROUND_GAMMA_FILE, 1,
                         1.0);
    ...
    *piChannels = ulChannels;
    ...
    if ((*piHeight) > ((size_t)(-1)) / ulRowBytes) {
      ...
      *ppbImageData = pbImageData;
      ...
      if ((ppbRowPointers =
               (png_bytepp)malloc((*piHeight) * sizeof(png_bytep))) == NULL) {
        png_error(png_ptr, "Visual PNG: out of memory");
      }
  ...
  BOOL PngSaveImage(PTSTR pstrFileName, png_byte * pDiData, int iWidth,
                    int iHeight, png_color bkgColor) {
    ...
    Try {
      ...
      if ((ppbRowPointers = (png_bytepp)malloc(iHeight * sizeof(png_bytep))) ==
          NULL)
