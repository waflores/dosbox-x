...
BOOL LoadImageFile(HWND hwnd, PTSTR pstrPathName, png_byte **ppbImage,
                   int *pxImgSize, int *pyImgSize, int *piChannels,
                   png_color *pBkgColor);

BOOL DisplayImage(HWND hwnd, BYTE **ppDib, BYTE **ppDiData, int cxWinSize,
                  int cyWinSize, BYTE *pbImage, int cxImgSize, int cyImgSize,
                  int cImgChannels, BOOL bStretched);

BOOL InitBitmap(BYTE *pDiData, int cxWinSize, int cyWinSize);

BOOL FillBitmap(BYTE *pDiData, int cxWinSize, int cyWinSize, BYTE *pbImage,
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine,
  ...
  ixBorders =
      2 * (GetSystemMetrics(SM_CXBORDER) + GetSystemMetrics(SM_CXDLGFRAME));
  iyBorders =
      2 * (GetSystemMetrics(SM_CYBORDER) + GetSystemMetrics(SM_CYDLGFRAME)) +
      GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CYMENUSIZE) +
      1; /* WvS: don't ask me why?  */

  hwnd = CreateWindow(szProgName, szAppName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                      CW_USEDEFAULT, 512 + 2 * MARGIN + ixBorders,
                      384 + 2 * MARGIN + iyBorders,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_CREATE:
    ...
    if (szCmdFileName[0] != '\0') {
      ...
      if (!LoadImageFile(hwnd, szImgPathName, &pbImage, &cxImgSize, &cyImgSize,
                         &cImgChannels, &bkgColor))
        return 0;
      ...
      DisplayImage(hwnd, &pDib, &pDiData, cxWinSize, cyWinSize, pbImage,
  ...
  case WM_COMMAND:
    ...
    case IDM_FILE_SAVE:
      ...
      bOk =
          PngSaveImage(szImgPathName, pDiData, cxWinSize, cyWinSize, bkgColor);

      ShowCursor(FALSE);
      SetCursor(LoadCursor(NULL, IDC_ARROW));

      if (!bOk)
        MessageBox(hwnd, TEXT("Error in saving the PNG image"), szProgName,
    ...
    case IDM_FILE_NEXT:
      ...
      if (SearchPngList(pPngFileList, iPngFileCount, &iPngFileIndex, NULL,
                        szImgPathName)) {
  ...
  case WM_PAINT:
    ...
    if (pDib)
      SetDIBitsToDevice(hdc, 0, 0, cxWinSize, cyWinSize, 0, 0, 0, cyWinSize,
                        pDiData, (BITMAPINFO *)pDib, DIB_RGB_COLORS);
...
BOOL LoadImageFile(HWND hwnd, PTSTR pstrPathName, png_byte **ppbImage,
  ...
  if (*ppbImage != NULL) {
    ...
    MessageBox(hwnd, TEXT("Error in loading the PNG image"), szProgName,
               MB_ICONEXCLAMATION | MB_OK);
    return FALSE;
  }
...
BOOL DisplayImage(HWND hwnd, BYTE **ppDib, BYTE **ppDiData, int cxWinSize,
  ...
  if (cyWinSize > ((size_t)(-1)) / wDIRowBytes) {
    ...
    if (!(pDib = (BYTE *)malloc(sizeof(BITMAPINFOHEADER) +
      ...
      MessageBox(hwnd, TEXT("Error in displaying the PNG image"), szProgName,
                 MB_ICONEXCLAMATION | MB_OK);
      *ppDib = pDib = NULL;
      return FALSE;
    }
    ...
    *ppDiData = pDiData;
    ...
    if (pbImage) {
      FillBitmap(pDiData, cxWinSize, cyWinSize, pbImage, cxImgSize, cyImgSize,
                 cImgChannels, bStretched);
    }
      ...
      if (cyNewSize > ((size_t)(-1)) / (cImgChannels * cxNewSize)) {
        ...
        for (yNew = 0; yNew < cyNewSize; yNew++) {
          ...
          for (xNew = 0; xNew < cxNewSize; xNew++) {
            ...
            *pImg++ = b;
            if (cImgChannels == 4) {
              a = *(pbImage + cImgChannels * ((yOld * cxImgSize) + xOld) + 3);
              *pImg++ = a;
            }
          }
        }
