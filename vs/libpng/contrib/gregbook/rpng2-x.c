...
static void rpng2_x_redisplay_image(ulg startcol, ulg startrow, ulg width,
                                    ulg height);
#ifdef FEATURE_LOOP
...
int main(int argc, char **argv) {
  ...
  if (!(infile = fopen(filename, "rb"))) {
    ...
    if (incount < 8 || !readpng2_check_sig(inbuf, 8)) {
      fprintf(stderr,
              PROGNAME ":  [%s] is not a PNG file: incorrect signature\n",
              filename);
      ++error;
      ...
      case 4:
        ...
        break;
      default:
        fprintf(stderr, PROGNAME ":  unknown readpng2_init() error\n");
        break;
      }
      ++error;
    } else {
  ...
  for (;;) {
    Trace((stderr, "about to call readpng2_decode_data()\n")) if (
        readpng2_decode_data(&rpng2_info, inbuf, incount))++ error;
    Trace((stderr, "done with readpng2_decode_data()\n"))
      ...
      if (rpng2_info.state == kDone) {
        ...
        fprintf(stderr, PROGNAME ":  error while reading PNG image file\n");
        exit(3);
      } else if (feof(infile)) {
        fprintf(stderr,
    ...
    Trace((stderr, "entering -loop loop (FEATURE_LOOP)\n")) for (;;) {
      ...
      if (use_sleep || gettimeofday(&now, NULL) != 0) {
        ...
        if (now.tv_sec < then.tv_sec ||
          ...
          if (gettimeofday(&now, NULL) == 0) {
            usleep_usec = 1000000L * (then.tv_sec - now.tv_sec) + then.tv_usec -
                          now.tv_usec;
            if (usleep_usec > 0)
...
static int rpng2_x_create_window(void) {
  ...
  if (depth != 16 && depth != 24 && depth != 32) {
    ...
    Trace((stderr, "XGetVisualInfo() returned %d 24-bit visuals\n",
           visuals_matched)) visual = visual_list[0].visual;
  ...
  window =
      XCreateWindow(display, root, 0, 0, rpng2_info.width, rpng2_info.height, 0,
  ...
  if ((size_hints = XAllocSizeHints()) != NULL) {
    ...
    size_hints->min_height = size_hints->max_height = (int)rpng2_info.height;
  }
  ...
  XSetWMProperties(display, window, pWindowName, pIconName, NULL, 0, size_hints,
  ...
  ximage = XCreateImage(display, visual, depth, ZPixmap, 0, (char *)xdata,
                        rpng2_info.width, rpng2_info.height, pad, 0);
  ...
  if (!bg_image) {
    if (depth == 24 || depth == 32) {
      bg_pixel =
          (bg_red << RShift) | (bg_green << GShift) | (bg_blue << BShift);
    } else if (depth == 16) {
...
static int rpng2_x_load_bg_image(void) {
  ...
  if (!bg_data) {
    fprintf(stderr,
            PROGNAME ":  unable to allocate memory for background image\n");
    bg_image = 0;
    return 1;
  }
  ...
  if ((bg[pat].type & 0x07) == 0) {
    ...
    fprintf(stderr, "%s:  computing radial background...", PROGNAME);
    fflush(stderr);
    ...
    for (row = 0; row < rpng2_info.height; ++row) {
      ...
      for (i = 0; i < rpng2_info.width; ++i) {
        ...
        intensity =
            cos((angle + (rotate * dist * PI)) * freq) * gray * saturate;
  ...
  if (depth == 24 || depth == 32) {
    ...
    for (row = 0; row < rpng2_info.height; ++row) {
      ...
      for (i = rpng2_info.width; i > 0; --i) {
        ...
        blue = ((ush)(*src) << 8);
        ++src;
        pixel = ((red >> RShift) & RMask) | ((green >> GShift) & GMask) |
                ((blue >> BShift) & BMask);
        /* recall that we set ximage->byte_order = MSBFirst above */
        *dest++ = (char)((pixel >> 8) & 0xff);
        *dest++ = (char)(pixel & 0xff);
...
static void rpng2_x_display_row(ulg row) {
  ...
  if (demo_timing && (row - firstrow >= 16 || row >= rpng2_info.height - 2)) {
    XPutImage(display, window, gc, ximage, 0, (int)firstrow, 0, (int)firstrow,
              rpng2_info.width, row - firstrow + 1);
    XFlush(display);
    rows = 0;
    usleep(usleep_duration);
  } else if (!demo_timing &&
             ((rows & 0xf) == 0 || row >= rpng2_info.height - 2)) {
    XPutImage(display, window, gc, ximage, 0, (int)firstrow, 0, (int)firstrow,
              rpng2_info.width, row - firstrow + 1);
    XFlush(display);
    rows = 0;
...
static void rpng2_x_redisplay_image(ulg startcol, ulg startrow, ulg width,
  ...
  Trace((stderr, "beginning display loop (image_channels == %d)\n",
      ...
      /*---------------------------------------------------------------------------
          Aside from the use of the rpng2_info struct and of src2 (for
        background image), this routine is identical to rpng_x_display_image()
        in the non- progressive version of the program--for the simple reason
        that redisplay of the image against a new background happens after the
        image is fully decoded and therefore is, by definition, non-progressive.
        ---------------------------------------------------------------------------*/
    ...
    for (lastrow = row = startrow; row < startrow + height; ++row) {
      ...
      if (((row + 1) & 0xf) == 0) {
        XPutImage(display, window, gc, ximage, 0, (int)lastrow, 0, (int)lastrow,
                  rpng2_info.width, 16);
        XFlush(display);
        lastrow = row + 1;
      }
  ...
  Trace((stderr, "calling final XPutImage()\n")) if (lastrow <
                                                     startrow + height) {
    XPutImage(display, window, gc, ximage, 0, (int)lastrow, 0, (int)lastrow,
              rpng2_info.width, rpng2_info.height - lastrow);
    XFlush(display);
  }
