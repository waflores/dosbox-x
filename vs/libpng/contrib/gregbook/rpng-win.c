...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, PSTR cmd,
  ...
  if (have_bg) {
    ...
    fprintf(stderr,
            PROGNAME ":  libpng error while checking for background color\n");
    exit(2);
  }
  ...
  Trace((stderr, "calling readpng_get_image()\n")) image_data =
      readpng_get_image(display_exponent, &image_channels, &image_rowbytes);
  ...
  Trace((stderr, "done with rpng_win_display_image()\n"))
      ...
      printf(
#ifndef __CYGWIN__
          "Done.  Press Q, Esc or mouse button 1 (within image window) to "
          "quit.\n"
#else
...
static int rpng_win_create_window(HINSTANCE hInst, int showmode) {
  ...
  global_hwnd =
      CreateWindow(progname, titlebar, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                   CW_USEDEFAULT, image_width + extra_width,
...
static int rpng_win_display_image() {
  ...
  Trace((stderr, "beginning display loop (image_channels == %d)\n",
      ...
      /*---------------------------------------------------------------------------
          ...
          message loop begins, so there's no real point in any
        pseudo-progressive display...
        ---------------------------------------------------------------------------*/
...
LRESULT CALLBACK rpng_win_wndproc(HWND hwnd, UINT iMsg, WPARAM wP, LPARAM lP) {
  ...
  case WM_PAINT:
    ...
    rc = StretchDIBits(hdc, 0, 0, image_width, image_height,
                       /*                    source                        */
                       0, 0, image_width, image_height, wimage_data,
                       (BITMAPINFO *)bmih,
                       /*              iUsage: no clue                     */
                       0, SRCCOPY);
    EndPaint(hwnd, &ps);
