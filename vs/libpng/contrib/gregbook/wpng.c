...
int main(int argc, char **argv) {
  ...
  if (!inname) {
    if (isatty(0)) {
      ...
      if ((wpng_info.infile = fdopen(fileno(stdin), "rb")) == NULL) {
        fprintf(stderr, PROGNAME ":  unable to reopen stdin in binary mode\n");
        ++error;
      } else if ((wpng_info.outfile = fdopen(fileno(stdout), "wb")) == NULL) {
        fprintf(stderr, PROGNAME ":  unable to reopen stdout in binary mode\n");
  ...
  if (!error) {
    ...
    if (pnmline[0] != 'P' ||
        ((pnmchar = pnmline[1]) != '5' && pnmchar != '6' && pnmchar != '8')) {
      fprintf(stderr,
              PROGNAME
      ...
      if (wpng_info.width <= 0L || wpng_info.height <= 0L || maxval != 255) {
        fprintf(stderr, PROGNAME
      ...
      if (!wpng_info.filter) {
        ...
        if ((p = strrchr(inname, '.')) == NULL || (p - inname) != (len - 4)) {
  ...
  if (error) {
    ...
    fprintf(stderr,
            "\n"
            "Usage:  %s [-gamma exp] [-bgcolor bg] [-text] [-time] "
            "[-interlace] pnmfile\n"
            "or: ... | %s [-gamma exp] [-bgcolor bg] [-text] [-time] "
            "[-interlace] | ...\n"
            ...
            "\n"
            "pnmfile or stdin must be a binary PGM (`P5'), PPM (`P6') or "
            "(extremely\n"
            "unofficial and unsupported!) PAM (`P8') file.  Currently it is "
            "required\n"
            "to have maxval == 255 (i.e., no scaling).  If pnmfile is "
            "specified, it\n"
            "is converted to the corresponding PNG file with the same base "
            "name but a\n"
            "``.png'' extension; files read from stdin are converted and sent "
            "to stdout.\n"
            "The conversion is progressive (low memory usage) unless "
            "interlacing is\n"
            "requested; in that case the whole image will be buffered in "
            "memory and\n"
            "written in one call.\n"
            "\n",
  ...
  if (text &&
    ...
    fprintf(stderr, "Enter text info (no more than 72 characters per line);\n");
    ...
    do {
      ...
      if (FGETS(p, 74, keybd) && (len = strlen(p)) > 1) {
        ...
        if ((result = wpng_isvalid_latin1((uch *)p, len)) >= 0) {
          fprintf(stderr,
                  "    " PROGNAME " warning:  character code"
  ...
  if ((rc = writepng_init(&wpng_info)) != 0) {
    ...
    case 11:
      fprintf(stderr,
              PROGNAME ":  internal logic error (unexpected PNM type)\n");
      break;
    default:
  ...
  if (wpng_info.interlaced) {
    ...
    for (j = wpng_info.height; j > 0L; --j) {
      ...
      if (bytes != rowbytes) {
        fprintf(stderr,
                PROGNAME ":  expected %lu bytes, got %lu bytes (row %ld)\n",
                rowbytes, bytes, wpng_info.height - j);
        ++error;
        break;
      }
      if (writepng_encode_row(&wpng_info) != 0) {
        fprintf(stderr,
                PROGNAME ":  libpng problem (longjmp) while writing row %ld\n",
                wpng_info.height - j);
        ++error;
        break;
      }
