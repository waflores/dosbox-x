...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, PSTR cmd,
                   int showmode) {
...
static int rpng2_win_create_window() {
  ...
  global_hwnd = CreateWindow(
      progname, titlebar, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
      rpng2_info.width + extra_width, rpng2_info.height + extra_height, NULL,
      NULL, global_hInst, NULL);
...
static void rpng2_win_display_row(ulg row) {
  ...
  Trace((stderr, "beginning rpng2_win_display_row()\n"))
      ...
      if (rows == 0) firstrow = row; /* first row not yet displayed */
...
LRESULT CALLBACK rpng2_win_wndproc(HWND hwnd, UINT iMsg, WPARAM wP, LPARAM lP) {
  ...
  case WM_PAINT:
    ...
    rc = StretchDIBits(hdc, 0, 0, rpng2_info.width, rpng2_info.height, 0, 0,
                       rpng2_info.width, rpng2_info.height, wimage_data,
                       (BITMAPINFO *)bmih, 0, SRCCOPY);
    EndPaint(hwnd, &ps);
    return 0;
