...
BOOL pnm2png(FILE *pnm_file, FILE *png_file, FILE *alpha_file, BOOL interlace,
             BOOL alpha);
BOOL do_pnm2png(png_struct *png_ptr, png_info *info_ptr, FILE *pnm_file,
                FILE *alpha_file, BOOL interlace, BOOL alpha);
...
BOOL pnm2png(FILE *pnm_file, FILE *png_file, FILE *alpha_file, BOOL interlace,
  ...
  png_ptr = png_create_write_struct(png_get_libpng_ver(NULL), NULL, NULL, NULL);
  if (!png_ptr)
...
BOOL do_pnm2png(png_struct *png_ptr, png_info *info_ptr, FILE *pnm_file,
  ...
  row_pointers = (png_byte **)png_calloc(png_ptr, height * sizeof(png_byte *));
...
int fscan_pnm_magic(FILE *pnm_file, char *magic_buf, size_t magic_buf_size) {
  ...
  if ((magic_buf[1] < '1') || (magic_buf[1] > '9'))
    return 0;
  if (magic_buf[2] != '\0')
    return 0;
