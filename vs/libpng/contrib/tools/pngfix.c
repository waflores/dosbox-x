...
#define aligncast(type, value) static_cast<type>(static_cast<void *>(value))
...
static int uarb_set(uarb result, png_alloc_size_t val)
...
static int uarb_copy(uarb to, uarb from, int idigits)
...
static int uarb_inc(uarb num, int in_digits, png_int_32 add)
...
static int uarb_add32(uarb num, int in_digits, png_uint_32 add)
...
static int uarb_mult_digit(uarb acc, int a_digits, uarb num, int n_digits,
                           png_uint_16 val)
...
static int uarb_mult32(uarb acc, int a_digits, uarb num, int n_digits,
                       png_uint_32 val)
...
static int uarb_shift(uarb inout, int ndigits, unsigned int right_shift)
...
static int uarb_cmp(uarb a, int adigits, uarb b, int bdigits)
...
static void uarb_printx(uarb num, int digits, FILE *out)
...
static void uarb_print(uarb num, int digits, FILE *out)
...
static void clear(void *pv, size_t size) {
...
static png_uint_32 crc_one_byte(png_uint_32 crc, int b) {
...
static int chunk_type_valid(png_uint_32 c)
...
static void IDAT_list_init(struct IDAT_list *list) {
  CLEAR(*list);
...
static size_t IDAT_list_size(struct IDAT_list *list, unsigned int length)
/* Return the size in bytes of an IDAT_list of the given length. */
{
  if (list != NULL)
    length = list->length;

  return sizeof *list - sizeof list->lengths + length * sizeof list->lengths[0];
}
...
static struct IDAT_list *IDAT_list_extend(struct IDAT_list *tail) {
...
static int global_end(struct global *global) {
...
static void global_init(struct global *global)
...
static int skip_chunk_type(const struct global *global, png_uint_32 type)
...
static void emit_string(const char *str, FILE *out)
...
static const char *strcode(int code) {
  switch (code) {
...
static void emit_error(struct file *file, int code, const char *what)
...
static int file_end(struct file *file) {
  int rc;
...
static int file_init(struct file *file, struct global *global,
...
static void log_error(struct file *file, int code, const char *what)
...
static char type_char(png_uint_32 v) {
...
static void type_name(png_uint_32 type, FILE *out) {
...
PNG_NORETURN static void stop(struct file *file, int code, const char *what)
...
PNG_NORETURN static void stop_invalid(struct file *file, const char *what) {
  stop(file, INVALID_ERROR_CODE, what);
}

static void type_message(struct file *file, png_uint_32 type, const char *what)
 ...
 */
static void file_getpos(struct file *file, fpos_t *pos) {
...
static void file_setpos(struct file *file, const fpos_t *pos) {
...
static void getpos(struct file *file)
...
static png_byte reread_byte(struct file *file)
...
static png_uint_32 reread_4(struct file *file)
...
static void skip_12(struct file *file)
...
static void write_byte(struct file *file, int b)
...
static unsigned int read_4(struct file *file, png_uint_32 *pu)
...
static int crc_read_many(struct file *file, png_uint_32 length)
...
static int calc_image_size(struct file *file)
 ...
 */
{
  ...
  if (file->width < 1 || file->width > 0x7fffffff)
       ...
       */
      {
        ...
        for (pass = 0; pass <= 6; ++pass) {
          ...
          if (pw > 0) {
             ...
             */
            image_digits =
                uarb_mult32(file->image_bytes, image_digits, row_bytes, digits,
    ...
    case PNG_INTERLACE_NONE: {
      ...
      digits = uarb_mult_digit(row_bytes, uarb_set(row_bytes, 7), row_width,
                               uarb_set(row_width, file->width), pd);
      digits = uarb_shift(row_bytes, digits, 3);
      digits = uarb_inc(row_bytes, digits, 1);

      /* Set row_bytes * image-height to the file image_bytes field */
      file->image_digits =
          uarb_mult32(file->image_bytes, 0, row_bytes, digits, file->height);
    } break;
...
static void chunk_message(struct chunk *chunk, const char *message) {
  type_message(chunk->file, chunk->chunk_type, message);
}

static void chunk_end(struct chunk **chunk_var) {
...
static void chunk_init(struct chunk *const chunk, struct file *const file)
...
/* Guess the actual chunk type that causes a stop() */
{
  ...
  if (file->chunk != NULL) {
     ...
     */
    if (code <= LIBPNG_ERROR_CODE && type == png_IDAT && file->write_count == 8)
      type = 0; /* magic */
...
static void setpos(struct chunk *chunk)
...
static int process_zTXt_iCCP(struct file *file)
...
static int process_iTXt(struct file *file) {
...
static void IDAT_end(struct IDAT **idat_var) {
...
static void IDAT_init(struct IDAT *const idat, struct file *const file)
...
static png_uint_32 rechunk_length(struct IDAT *idat, int start)
...
static int process_IDAT(struct file *file)
 ...
 */
{
  ...
  if (zlib_check(file, 0)) {
    ...
    cmp = uarb_cmp(file->image_bytes, file->image_digits,
                   file->chunk->uncompressed_bytes,
                   file->chunk->uncompressed_digits);
...
static const char *zlib_flevel(struct zlib *zlib) {
...
static const char *zlib_rc(struct zlib *zlib)
...
static void zlib_message(struct zlib *zlib, int unexpected)
...
static void zlib_end(struct zlib *zlib) {
  ...
  if (!zlib->global->quiet) {
    if (zlib->ok_bits < 16) /* stream was read ok */
    {
      ...
      putc(' ', stdout);
      uarb_print(zlib->uncompressed_bytes, zlib->uncompressed_digits, stdout);
      putc(' ', stdout);
...
static int zlib_reset(struct zlib *zlib, int window_bits)
...
static int zlib_init(struct zlib *zlib, struct IDAT *idat, struct chunk *chunk,
...
static int max_window_bits(uarbc size, int ndigits)
...
static int zlib_advance(struct zlib *zlib, png_uint_32 nbytes)
...
#define ZLIB_FATAL 2
{
  ...
  while (in_bytes < nbytes && endrc == ZLIB_OK) {
     ...
     */
    for (; endrc == ZLIB_OK; flush = Z_SYNC_FLUSH, zlib->z.next_out = &bOut,
                             zlib->z.avail_out = 1, ++out_bytes) {
    ...
    zlib->uncompressed_digits = uarb_add32(
        zlib->uncompressed_bytes, zlib->uncompressed_digits, out_bytes);
  ...
  zlib->compressed_digits =
      uarb_add32(zlib->compressed_bytes, zlib->compressed_digits,
                 in_bytes - zlib->z.avail_in);
  ...
  if (endrc == ZLIB_STREAM_END && zlib->window_bits < zlib->ok_bits) {
    ...
    chunk->uncompressed_digits =
        uarb_copy(chunk->uncompressed_bytes, zlib->uncompressed_bytes,
                  zlib->uncompressed_digits);
    chunk->compressed_digits =
        uarb_copy(chunk->compressed_bytes, zlib->compressed_bytes,
                  zlib->compressed_digits);
...
static int zlib_run(struct zlib *zlib)
 ...
 */
{
  ...
  if (zlib_init(&zlib, file->idat, file->chunk, 0 /*window bits*/, offset)) {
    ...
    case ZLIB_STREAM_END:
      if (!zlib.global->optimize_zlib && zlib.window_bits == zlib.file_bits &&
          !zlib.cksum) {
      ...
      max_bits =
          max_window_bits(zlib.uncompressed_bytes, zlib.uncompressed_digits);
      ...
      if (zlib_reset(&zlib, test_bits)) {
        ...
        case ZLIB_TOO_FAR_BACK:
          ...
          if (min_bits > max_bits) {
            ...
            if (zlib.z.msg == NULL)
              zlib.z.msg = PNGZ_MSG_CAST("invalid distance too far back");
            zlib_message(&zlib, 0 /*stream error*/);
            zlib_end(&zlib);
            return 0;
          }
...
static void process_chunk(struct file *file, png_uint_32 file_crc,
                          png_uint_32 next_length, png_uint_32 next_type)
...
static png_uint_32 get32(png_bytep buffer, int offset)
/* Read a 32-bit value from an 8-byte circular buffer (used only below).
 */
{
  return (buffer[offset & 7] << 24) + (buffer[(offset + 1) & 7] << 16) +
         (buffer[(offset + 2) & 7] << 8) + (buffer[(offset + 3) & 7]);
}

static void sync_stream(struct file *file)
...
static void PNGCBAPI error_handler(png_structp png_ptr,
                                   png_const_charp message) {
  stop(get_control(png_ptr), LIBPNG_ERROR_CODE, message);
}

static void PNGCBAPI warning_handler(png_structp png_ptr,
                                     png_const_charp message) {
 ...
 */
static void PNGCBAPI read_callback(png_structp png_ptr, png_bytep buffer,
                                   size_t count)
    ...
    default:
          ...
          while (idat->idat_index >= idat->idat_length) {
            ...
            if (++(idat->idat_count) >= cur->count) {
              ...
              if (cur == NULL || cur->count == 0)
                stop(file, UNEXPECTED_ERROR_CODE, "read beyond end of IDAT");
      ...
      case 11:
        ...
        if (file->state == STATE_IDAT &&
           ...
           */
          length = chunk->chunk_length = rechunk_length(file->idat, 0 /*end*/);
...
static int control_end(struct control *control) {
...
static struct file *get_control(png_const_structrp png_ptr) {
  ...
  struct control *control =
      voidcast(struct control *, png_get_error_ptr(png_ptr));
  return &control->file;
}

static void allocate(struct file *file, int allocate_idat) {
...
static int control_init(struct control *control, struct global *global,
                        const char *file_name, const char *out_name)
...
static int read_png(struct control *control)
...
static int one_file(struct global *global, const char *file_name,
                    const char *out_name) {
  int rc;
  struct control control;
...
static void usage(const char *prog) {
  ...
  static const char *usage_string[] = {
      ...
      "OPTIONS",
      "  OPERATION",
      "      By default files are just checked for readability with a summary "
      "of the",
      "      of zlib issues founds for each compressed chunk and the IDAT "
      "stream in",
      "      the file.",
      "    --optimize (-o):",
      ...
      "        unsafe: Remove chunks that may be unsafe to retain if the image "
      "data",
      "                is modified.  This is set automatically if --max is "
      "given but",
      "                may be cancelled by a later --strip=none.",
      "        unused: Remove chunks not used by libpng when decoding an "
      "image.",
      "                This retains any chunks that might be used by libpng "
      "image",
      "                transformations.",
      "        transform: unused+bKGD.",
      "        color:  transform+iCCP and cHRM.",
      "        all:    color+gAMA and sRGB.",
      "      Only ancillary chunks are ever removed.  In addition the tRNS and "
      "sBIT",
      "      chunks are never removed as they affect exact interpretation of "
      "the",
      "      image pixel values.  The following known chunks are treated "
      "specially",
      "      by the above options:",
      "        gAMA, sRGB [all]: These specify the gamma encoding used for the "
      "pixel",
      "            values.",
      "        cHRM, iCCP [color]: These specify how colors are encoded.  iCCP "
      "also",
      ...
      "      chunks will be the maximum size permitted; 2^31-1 bytes.  If the "
      "option",
      "      is omitted the original chunk sizes will not be changed.  When "
      "the",
      "      option is given --strip=unsafe is set automatically. This may be",
      "      cancelled if you know that all unknown unsafe-to-copy chunks "
      "really are",
      "      safe to copy across an IDAT size change.  This is true of all "
      "chunks",
      "      that have ever been formally proposed as PNG extensions.",
      "  MESSAGES",
      "      By default the program only outputs summaries for each file.",
      "    --quiet (-q):",
      "      Do not output the summaries except for files that cannot be read. "
      "With",
      ...
      "    --out=<file>:",
      "      Write the optimized/corrected version of the next PNG to <file>.  "
      "This",
      ...
      "    --prefix=<prefix>:",
      "      Set --out=<prefix><name> for all the following files unless "
      "overridden",
      "      on a per-file basis by explicit --out.",
      "      These two options can be used together to produce a suffix and "
      "prefix.",
      "  INTERNAL OPTIONS",
#if 0 /*NYI*/
...
"      output as this would produce a broken file.",
      ...
      "EXIT CODES",
      "  *** SUBJECT TO CHANGE ***",
      "  The program exit code is value in the range 0..127 holding a bit mask "
      "of",
      "  the following codes.  Notice that the results for each file are "
      "combined",
      ...
      "  Errors less than 16 are potentially recoverable, for a single file if "
      "the",
      "  exit code is less than 16 the file could be read (with corrections if "
      "a",
      ...
      "  If the command line arguments are incorrect the program exits with "
      "exit",
      "  255.  Some older operating systems only support 7-bit exit codes, on "
      "those",
      "  systems it is suggested that this program is first tested by "
      "supplying",
      "  invalid arguments.",
      0,
      "DESCRIPTION",
      "  " PROGRAM_NAME ":",
      "  checks each PNG file on the command line for errors.  By default "
      "errors are",
      "  not output and the program just returns an exit code and prints a "
      "summary.",
      ...
      "  errors).",
      "  Various known problems in PNG files are fixed while the file is being "
      "read",
      "  The exit code says what problems were fixed.  In particular the zlib "
      "error:",
      0,
      "        \"invalid distance too far back\"",
      0,
      "  caused by an incorrect optimization of a zlib stream is fixed in any",
      "  compressed chunk in which it is encountered.  An integrity problem of "
      "the",
      "  PNG stream caused by a bug in libpng which wrote an incorrect chunk "
      "length",
      ...
      0,
      "  Notice that some PNG files with the zlib optimization problem can "
      "still be",
      "  read by libpng under some circumstances.  This program will still "
      "detect",
      "  and, if requested, correct the error.",
      0,
      "  The program will reliably process all files on the command line "
      "unless",
      "  either an invalid argument causes the usage message (this message) to "
      "be",
      "  produced or the program crashes.",
      0,
      "  The summary lines describe issues encountered with the zlib "
      "compressed",
      "  stream of a chunk.  They have the following format, which is SUBJECT "
      "TO",
      "  CHANGE in the future:",
      0,
      "     chunk reason comp-level p1 p2 p3 p4 file",
      0,
      "  p1 through p4 vary according to the 'reason'.  There are always 8 "
      "space",
      ...
      0,
      "$1 chunk:      The chunk type of a chunk in the file or 'HEAD' if a "
      "problem",
      ...
      "          OK : No errors were detected in the zlib stream and "
      "optimization",
      "               was not requested, or was not possible.",
      "          OPT: The zlib stream window bits value could be improved (and "
      "was).",
      "          SKP: The chunk was skipped because of a zlib issue (zlib-rc) "
      "with",
      "               explanation 'message'",
      "          ERR: The read of the file was aborted.  The parameters "
      "explain why.",
      "$3 status:     For 'ERR' the accumulated status code from 'EXIT CODES' "
      "above.",
      ...
      "$4 code:       The file exit code; where stop was called, as a fairly "
      "terse",
      "               string "
      "{warning,libpng,zlib,invalid,read,write,unexpected}.",
      "   file-bits:  The zlib window bits recorded in the file.",
      "$5 read-errno: A system errno value from a read translated by "
      "strerror(3).",
      "   zlib-rc:    A zlib return code as a string (see zlib.h).",
      "   ok-bits:    The smallest zlib window bits value that works.",
      "$6 write-errno:A system errno value from a write translated by "
      "strerror(3).",
      "   compressed: The count of compressed bytes in the zlib stream, when "
      "the",
      "               reason is 'SKP'; this is a count of the bytes read from "
      "the",
      "               stream when the fatal error was encountered.",
      "$7 message:    An error message (spaces replaced by _, as in all "
      "parameters),",
      "   uncompress: The count of bytes from uncompressing the zlib stream; "
      "this",
...
int main(void) {
  fprintf(stderr, "pngfix needs libpng with a zlib >=1.2.4 (not 0x%x)\n",
          ZLIB_VERNUM);
  return 77;
}
#endif /* ZLIB_VERNUM */
