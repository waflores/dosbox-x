...
static const value_list
    ...
    vl_compression[] =
        {
            /* Overall compression control.  The order controls the search order
             * for 'all'.  Since the search is for the smallest the order used
             * is low memory then high speed.
             */
    ...
    vl_windowBits_text[] = {{"default", MAX_WBITS /*from zlib*/},
                            {"minimum", 8},
                            RANGE(8, MAX_WBITS /*from zlib*/),
                            {all, 0}},
#endif /* text compression */
    vl_level[] = {{"default", Z_DEFAULT_COMPRESSION /* this is -1 */},
...
static value_list vl_windowBits_IDAT[] = {
    {"default", MAX_WBITS},
    {"small", 9},
...
#define S(n, v) {#n, v, 0, 2, vl_on_off},
    S(verbose, VERBOSE) S(warnings, WARNINGS) S(errors, ERRORS) S(quiet, QUIET)
        S(strict, STRICT) S(log, LOG) S(continue, CONTINUE) S(sizes, SIZES)
            S(search, SEARCH) S(nowrite, NOWRITE)
#ifdef IGNORE_INDEX
...
#define VLSIZE(name)                                                           \
  voidcast(png_byte, (sizeof VLNAME(name)) / (sizeof VLNAME(name)[0]))
...
#define VLC(name) VLCIDAT(name) VLCiCCP(name) VLCzTXt(name)
     ...
     */
    VLO("compression", compression, 0) VLO("text-compression", compression, 0)
        VLO("ICC-profile-compression", compression, 0)
#endif /* SW_COMPRESS_png_level */
            VLC(strategy) VLO("windowBits", windowBits_IDAT, 1)
#ifdef PNG_SW_COMPRESS_windowBits
                VLO("ICC-profile-windowBits", windowBits_text /*sic*/, 0)
#endif
                    VLO("text-windowBits", windowBits_text, 0) VLC(level)
                        VLC(memLevel) VLO("IDAT-size", IDAT_size, 0)
...
static void display_clean_read(struct display *dp, int freeinfo) {
  if (dp->read_pp != NULL)
...
static void display_clean_write(struct display *dp, int freeinfo) {
  if (dp->fp != NULL) {
    FILE *fp = dp->fp;
    dp->fp = NULL;
    (void)fclose(fp);
...
#define VGATTR
#endif
static void VGATTR display_log(struct display *dp, error_level level,
                               const char *fmt, ...)
...
#if PNG_LIBPNG_VER < 10700 && defined PNG_TEXT_SUPPORTED
static void text_stash(struct display *dp) {
...
static int get_option(struct display *dp, const char *opt, int *value) {
...
static int set_opt_string_(struct display *dp, unsigned int sp, png_byte opt,
...
static void set_opt_string(struct display *dp, unsigned int sp)
/* Add the appropriate option string to dp->curr. */
{
  dp->stack[sp].opt_string_end = set_opt_string_(
      dp, sp, dp->stack[sp].opt,
...
static int opt_list_end(struct display *dp, png_byte opt, png_byte entry) {
  if (options[opt].values[entry].name == range_lo)
    return entry + 1U >= options[opt].value_count /* missing range_hi */ ||
           ...
           options[opt].values[entry + 1U].value <=
               dp->value[opt] /* range end */;
...
static void push_opt(struct display *dp, unsigned int sp, png_byte opt,
                     int search)
 ...
 */
{
  ...
  dp->tsp = sp + 1U;
  dp->stack[sp].best_size = dp->stack[sp].lo_size = dp->stack[sp].hi_size =
      MAX_SIZE;
...
static void next_opt(struct display *dp, unsigned int sp)
...
static int compare_option(const struct display *dp, unsigned int sp) {
...
static int advance_opt(struct display *dp, png_byte opt, int search) {
...
static int getsearchopts(struct display *dp, const char *opt_str, int *value)
...
static int find_val(struct display *dp, png_byte opt, const char *str,
                    size_t len)
...
static int opt_check(struct display *dp, const char *arg) {
  assert(dp->errset == 0);
      ...
      if (options[j].opt != LIST) {
        ...
        if (negate) {
          if (options[j].opt < OPTION)
            ...
            display_log(dp, USER_ERROR, "%.*s: option=arg cannot be negated", i,
                        arg);
            abort(); /* NOT REACHED */
          }
        }
    ...
    if (options[j].opt < LEVEL_MASK) {
      ...
      if (val) /* Set this level */
        ...
        display_log(dp, USER_ERROR,
                    "%.*s: messages cannot be turned off individually; set a "
                    "message level",
                    i, arg);
    }
...
#ifdef PNG_PNGCP_TIMING_SUPPORTED
static void set_timer(struct display *dp, struct timespec *timer) {
...
static void start_timer(struct display *dp, int what) {
  if ((dp->value[OPTIND(dp, time)] & what) != 0)
    set_timer(dp, what == PNGCP_TIME_READ ? &dp->read_time : &dp->write_time);
}

static void end_timer(struct display *dp, int what) {
  if ((dp->value[OPTIND(dp, time)] & what) != 0) {
...
static void print_time(const char *what, struct timespec t) {
 ...
 * directory:
 */
static int checkdir(const char *pathname) {
  struct stat buf;
 ...
 */
static int isdir(struct display *dp, const char *pathname) {
  if (pathname == NULL)
    return 0; /* stdout */
    ...
    else /* an error */
    {
      ...
      if (errno != ENOENT)
        display_log(dp, USER_ERROR, "%s: invalid output name (%s)", pathname,
                    strerror(errno));
...
static void makename(struct display *dp, const char *dir, const char *infile) {
...
static void PNGCBAPI display_warning(png_structp pp, png_const_charp warning) {
...
static void PNGCBAPI display_error(png_structp pp, png_const_charp error) {
  struct display *dp = get_dp(pp);

  display_log(dp, LIBPNG_ERROR, "%s", error);
}

static void display_start_read(struct display *dp, const char *filename) {
  if (filename != NULL) {
...
static void PNGCBAPI read_function(png_structp pp, png_bytep data,
                                   size_t size) {
  struct display *dp = get_dp(pp);
...
static void read_png(struct display *dp, const char *filename) {
  ...
  dp->read_pp = png_create_read_struct(PNG_LIBPNG_VER_STRING, dp, display_error,
                                       display_warning);
  if (dp->read_pp == NULL)
...
static void display_start_write(struct display *dp, const char *filename) {
  assert(dp->fp == NULL);
...
static void PNGCBAPI write_function(png_structp pp, png_bytep data,
                                    size_t size) {
  struct display *dp = get_dp(pp);
...
#ifdef PNG_WRITE_CUSTOMIZE_COMPRESSION_SUPPORTED
static void search_compression(struct display *dp) {
  /* Like set_compression below but use a more restricted search than 'all' */
  int val;

#define SET(name, func)                                                        \
  if (getsearchopts(dp, #name, &val))                                          \
    png_set_compression_##func(dp->write_pp, val);
  SET_COMPRESSION
#undef SET
}

static void set_compression(struct display *dp) {
  int val;

#define SET(name, func)                                                        \
  if (getallopts(dp, #name, &val))                                             \
    png_set_compression_##func(dp->write_pp, val);
  SET_COMPRESSION
#undef SET
}
...
static void set_ICC_profile_compression(struct display *dp) {
  int val;

#define SET(name, func)                                                        \
  if (getallopts(dp, "ICC-profile-" #name, &val))                              \
...
#ifdef PNG_WRITE_CUSTOMIZE_ZTXT_COMPRESSION_SUPPORTED
static void set_text_compression(struct display *dp) {
  int val;

#define SET(name, func)                                                        \
  if (getallopts(dp, "text-" #name, &val))                                     \
...
static void write_png(struct display *dp, const char *destname) {
...
static void set_windowBits_hi(struct display *dp) {
  ...
  while (--i >= 0)
    if (VLNAME(windowBits_IDAT)[i].name == range_hi)
      break;
...
static int better_options(const struct display *dp) {
...
static void print_search_results(struct display *dp) {
...
static void log_search(struct display *dp, unsigned int log_depth) {
...
static void cp_one_file(struct display *dp, const char *filename,
                        const char *destname) {
  ...
  if (filename != NULL && access(filename, R_OK) != 0)
    display_log(dp, USER_ERROR, "%s: invalid file name (%s)", filename,
                strerror(errno));
...
static int cppng(struct display *dp, const char *file, const char *dest) {
...
int main(int argc, char **argv) {
  ...
  d.operation = "options";
  for (option_end = 1; option_end < argc && opt_check(&d, argv[option_end]);
       ++option_end) {
  }
   ...
   */
  if (!(d.options & NOWRITE) && option_end + 2 < argc &&
      !checkdir(argv[argc - 1])) {
    fprintf(stderr,
    ...
    do {
      ...
      if (ret) {
        ...
        printf("%s [%ld x %ld %d bpp %s, %lu bytes] %lu -> %lu [0x%lx]\n",
               infile, (unsigned long)d.w, (unsigned long)d.h, d.bpp, cts(d.ct),
       ...
       */
      {
        int pass = (d.options & STRICT) ? RESULT_STRICT(d.results)
                                        : RESULT_RELAXED(d.results);
