...
double fsRGB(double l) { return sRGB_from_linear(l / max_input); }

double sRGB(unsigned int i) { return fsRGB(i); }

double finvsRGB(unsigned int i) { return 65535 * linear_from_sRGB(i / 255.); }

png_uint_16 invsRGB(unsigned int i) {
  ...
  for (ibase = 0; ibase < 65536; ibase += 128) {
    ...
    if (base != ob || delta != od) {
      printf("/* table[%u]={%u,%u} -> {%u,%u} %u -> %u errors */\n", ibase >> 7,
             ob, od, base, delta, eco, ecbase);
    } else if (0)
      printf("/* table[%u]={%u,%u} %u errors */\n", ibase >> 7, ob, od, ecbase);
  }
  ...
  for (i = 0; i <= max_input; ++i) {
    ...
    if (icalc != iexact) {
      ...
      if (err > (max_error + .001) || err < (min_error - .001)) {
        printf("/* 0x%08x: exact: %3d, got: %3d [tables: %08x, %08x] (%f) */\n",
  ...
  for (i16 = 0; i16 <= 65535; ++i16) {
    ...
    if (icalc != iexact) {
      ...
      if (abs(icalc - iexact) > 1)
        printf("/* 0x%04x: exact: %3d, got: %3d [tables: %08x, %08x] (%f) */\n",
  ...
  for (i16 = 0; i16 <= 255; ++i16) {
    ...
    if (icalc != i16) {
      ...
      printf("/* 8-bit roundtrip error: %d -> %f -> %d(%f) */\n", i16, finv,
             icalc, fsRGB(255 * finv));
    }
  }

  printf("/* error: %g - %g, %u (%g%%) of readings inexact */\n", min_error,
         max_error, error_count, (100. * error_count) / max_input);
  printf("/* 16-bit error: %g - %g, %u (%g%%) of readings inexact */\n",
         min_error16, max_error16, error_count16,
