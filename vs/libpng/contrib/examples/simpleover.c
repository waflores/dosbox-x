...
static void sprite_op(const struct sprite *sprite, int x_offset, int y_offset,
...
#define compose(c)                                                             \
  ...
  out_pixel[c] = tmp + in_pixel[c]
            ...
            out_pixel[2] = in_pixel[2], out_pixel[3] = in_pixel[3];
        }
      } while (++x < image->width);
      ...
      if (png_image_begin_read_from_file(&image, (*argv)[0])) {
        ...
        if (buffer != NULL) {
          if (png_image_finish_read(
                  &image, NULL /*background*/, buffer, 0 /*row_stride*/,
...
static int add_sprite(png_imagep output, png_bytep out_buf,
                      struct sprite *sprite, int *argc, const char ***argv) {
  ...
  while (*argc > 0) {
    ...
    if (sscanf((*argv)[0], "%d,%d%c", &x, &y, &tombstone) == 2) {
       ...
       * will fit.
       */
      if (x < 0 || y < 0 || (unsigned)/*SAFE*/ x >= output->width ||
...
static int simpleover_process(png_imagep output, png_bytep out_buf, int argc,
                              const char **argv) {
  ...
  while (argc > 0) {
    ...
    if (strncmp(argv[0], "--sprite=", 9) == 0) {
      ...
      if (nsprites < csprites) {
        ...
        if ((n == 2 || n == 3) && sprites[nsprites].width > 0 &&
  ...
  if (argc >= 2) {
    ...
    if (png_image_begin_read_from_file(&image, argv[1])) {
      ...
      if (buffer != NULL) {
        ...
        if (png_image_finish_read(
                &image, &background, buffer, 0 /*row_stride*/,
                NULL /*colormap for PNG_FORMAT_FLAG_COLORMAP */)) {
          ...
          if (simpleover_process(&image, buffer, argc - argi, argv + argi)) {
            /* Write the output: */
            if ((output != NULL &&
                 png_image_write_to_file(&image, output, 0 /*convert_to_8bit*/,
                                         buffer, 0 /*row_stride*/,
                                         NULL /*colormap*/)) ||
                (output == NULL &&
                 png_image_write_to_stdio(&image, stdout, 0 /*convert_to_8bit*/,
                                          buffer, 0 /*row_stride*/,
                                          NULL /*colormap*/)))
              result = 0;
