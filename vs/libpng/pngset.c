...
void PNGAPI png_set_cICP(png_const_structrp png_ptr, png_inforp info_ptr,
...
png_set_cLLI_fixed(png_const_structrp png_ptr, png_inforp info_ptr,
                   /* The values below are in cd/m2 (nits) and are scaled by
                    * 10,000; not 100,000 as in the case of png_fixed_point.
                    */
...
#ifdef PNG_FLOATING_POINT_SUPPORTED
void PNGAPI png_set_cLLI(png_const_structrp png_ptr, png_inforp info_ptr,
...
static png_uint_16 png_ITU_fixed_16(int *error, png_fixed_point v) {
...
void PNGAPI png_set_mDCV_fixed(png_const_structrp png_ptr, png_inforp info_ptr,
  ...
  if (error) {
    png_chunk_report(png_ptr, "mDCV chromaticities outside representable range",
                     PNG_CHUNK_WRITE_ERROR);
    return;
  }
...
#ifdef PNG_FLOATING_POINT_SUPPORTED
void PNGAPI png_set_mDCV(png_const_structrp png_ptr, png_inforp info_ptr,
                         double white_x, double white_y, double red_x,
                         double red_y, double green_x, double green_y,
                         double blue_x, double blue_y, double maxDL,
                         double minDL) {
  png_set_mDCV_fixed(png_ptr, info_ptr,
                     /* The ITU approach is to scale by 50,000, not 100,000 so
                      * just divide the input values by 2 and use png_fixed:
                      */
