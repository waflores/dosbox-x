 ...
 */
static int check_chunk_name(png_uint_32 name) {
  png_uint_32 t;
 ...
 */
static int png_crc_error(png_structrp png_ptr, int handle_as_ancillary) {
 ...
 * duplication.
 */
static int png_crc_finish_critical(png_structrp png_ptr, png_uint_32 skip,
...
static png_int_32 png_get_int_32_checked(png_const_bytep buf, int *error) {
  png_uint_32 uval = png_get_uint_32(buf);
  if ((uval & 0x80000000) == 0) /* non-negative */
    return (png_int_32)uval;
      ...
      if (keyword_length + 1 < read_length &&
        ...
        if (png_inflate_claim(png_ptr, png_iCCP) == Z_OK) {
          ...
          if (size == 0) {
            ...
            if (png_icc_check_length(png_ptr, keyword, profile_length) != 0) {
              ...
              if (png_icc_check_header(png_ptr, keyword, profile_length,
                ...
                png_bytep profile = png_read_buffer(png_ptr, profile_length);
                ...
                if (profile != NULL) {
                   ...
                   */
                  if (size == 0) {
                    if (png_icc_check_tag_table(png_ptr, keyword,
                                                profile_length, profile) != 0) {
...
png_handle_mDCV(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  png_set_mDCV_fixed(
      png_ptr, info_ptr, chromaticities.whitex, chromaticities.whitey,
      chromaticities.redx, chromaticities.redy, chromaticities.greenx,
      chromaticities.greeny, chromaticities.bluex, chromaticities.bluey,
...
png_handle_hIST(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  if (length != num * 2 || num != (unsigned int)png_ptr->num_palette ||
 ...
 */
static const struct {
  png_handle_result_code (*handler)(png_structrp, png_inforp,
                                    png_uint_32 length);
...
static png_index png_chunk_index_from_name(png_uint_32 chunk_name) {
...
png_handle_chunk(png_structrp png_ptr, png_inforp info_ptr,
  ...
  if (chunk_index == PNG_INDEX_unknown ||
    ...
    handled = png_handle_unknown(png_ptr, info_ptr, length,
