...
int main(int argc, char *argv[]) {
  ...
  dllflag = subsysflag = subsysver = osver = subsysver_set = osver_set =
      hdr_changed = bin_changed = oldchksum = newchksum = 0;

  if (argc < 2) {
    fprintf(stderr,
            "Missing input file.\n\nUsage: %s <PE-file> [+|-dll] [+|-win] "
            "[+|-con] [-osver <number.number>] [-subsysver <number.number>]",
            argv[0]);
    return 1;
  }
  ...
  for (i = 2; i < argc; i++) {
    ...
    if (strncmp(argv[i], "-subsysver", 10) == 0 && (i + 1 < argc)) {
      if (sscanf(argv[i + 1], "%d.%d", &j, &k)) {
        ...
        subsysver_set = 1;
        i++;
        subsysver = (j & 0xffff) << 16 | (k & 0xffff);
        continue;
      }
    }
  }
  ...
  if (!fread(buf, 64, 1, fh)) {
    fprintf(stderr, "Unable to read %d bytes, @%ld.\n", 2, ftell(fh));
    perror(0);
    fclose(fh);
    return 1;
  }
  ...
  if (buf[0] != 0x4d || buf[1] != 0x5a) {
    fprintf(stderr, "%s is missing Mark Zbikowski header.\n", argv[1]);
    fclose(fh);
    return 2;
  }
  ...
  if (buf[0] != 0x50 || buf[1] != 0x45 || buf[2] != 0x00 || buf[3] != 0x00) {
    fprintf(stderr, "%s is missing valid Portable Executable signature.\n",
            argv[1]);
    fclose(fh);
    return 2;
  }
  ...
  */
  ...
  fprintf(stdout, "ImageVersion = %d.%d\n", *(unsigned short*)(buf+44),
  *(unsigned short*)(buf+46));*/
  ...
  if (bin_changed) {
    ...
    *(unsigned long *)(buf + 64) =
        newchksum + bin_size - 1; // need to add file length
    free(csbuf);
