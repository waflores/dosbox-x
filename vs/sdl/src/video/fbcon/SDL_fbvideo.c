...
static const SDL_Rect checkres[] = {
    ...
    {0, 0, 512, 384},   {0, 0, 320, 240}, {0, 0, 320, 200}};
static const struct {
  int xres;
  int yres;
  int pixclock;
...
static void *do_mmap(void *start, size_t length, int prot, int flags, int fd,
                     off_t offset) {
  void *ret;
  ret = mmap(start, length, prot, flags, fd, offset);
  if (ret == (char *)-1 && (flags & MAP_SHARED)) {
    ret = mmap(start, length, prot, (flags & ~MAP_SHARED) | MAP_PRIVATE, fd,
               offset);
  }
  return ret;
}
...
static SDL_VideoDevice *FB_CreateDevice(int devindex) {
  ...
  if (this) {
    ...
    this->hidden =
        (struct SDL_PrivateVideoData *)SDL_malloc((sizeof *this->hidden));
  }
...
VideoBootStrap FBCON_bootstrap = {"fbcon", "Linux Framebuffer Console",
...
static int FB_CheckMode(_THIS, struct fb_var_screeninfo *vinfo, int index,
...
static int FB_AddMode(_THIS, int index, unsigned int w, unsigned int h,
  ...
  if (check_timings) {
    ...
    for (i = 0; i < (sizeof(vesa_timings) / sizeof(vesa_timings[0])); ++i) {
      if ((w == vesa_timings[i].xres) && (h == vesa_timings[i].yres) &&
          vesa_timings[i].pixclock) {
        found_timing = 1;
        break;
      }
    }
...
static int FB_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  mapped_offset =
      (((long)finfo.smem_start) - (((long)finfo.smem_start) & ~(pagesize - 1)));
  mapped_memlen = finfo.smem_len + mapped_offset;
  mapped_mem = do_mmap(NULL, mapped_memlen, PROT_READ | PROT_WRITE, MAP_SHARED,
                       console_fd, 0);
  ...
  if (finfo.accel && finfo.mmio_len) {
    ...
    mapped_io = do_mmap(NULL, mapped_iolen, PROT_READ | PROT_WRITE, MAP_SHARED,
                        console_fd, mapped_memlen);
    if (mapped_io == (char *)-1) {
...
static SDL_Surface *FB_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (!SDL_ReallocFormat(current, vinfo.bits_per_pixel, Rmask, Gmask, Bmask,
                         0)) {
    return (NULL);
  }
  ...
  if (shadow_fb) {
    if (vinfo.bits_per_pixel == 16) {
      blitFunc = (rotate == FBCON_ROTATE_NONE || rotate == FBCON_ROTATE_UD)
                     ? FB_blit16
                     : FB_blit16blocked;
    } else {
#ifdef FBCON_DEBUG
  ...
  surfaces_len =
      (shadow_fb) ? 0 : (mapped_memlen - (surfaces_mem - mapped_mem));
...
static void FB_WaitIdle(_THIS) { return; }
...
static void FB_blit16(Uint8 *byte_src_pos, int src_right_delta,
                      int src_down_delta, Uint8 *byte_dst_pos,
...
static void FB_blit16blocked(Uint8 *byte_src_pos, int src_right_delta,
                             int src_down_delta, Uint8 *byte_dst_pos,
                             int dst_linebytes, int width, int height) {
  int w;
  Uint16 *src_pos = (Uint16 *)byte_src_pos;
  ...
  while (height > 0) {
    ...
    for (w = width; w > 0; w -= BLOCKSIZE_W) {
      FB_blit16((Uint8 *)src, src_right_delta, src_down_delta, (Uint8 *)dst,
                dst_linebytes, min(w, BLOCKSIZE_W), min(height, BLOCKSIZE_H));
...
static void FB_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; i++) {
    ...
    blitFunc((Uint8 *)src_start, shadow_right_delta, shadow_down_delta,
             (Uint8 *)dst_start, physlinebytes, scr_x2 - scr_x1,
             scr_y2 - scr_y1);
  }
}
...
static void FB_SavePalette(_THIS, struct fb_fix_screeninfo *finfo,
  ...
  */
  if (finfo->visual == FB_VISUAL_DIRECTCOLOR) {
    ...
    for (i = 0; i < 256; ++i) {
      new_entries[(0 * 256) + i] = new_entries[(1 * 256) + i] =
