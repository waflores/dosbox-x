...
static SDL_Surface *XBIOS_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
VideoBootStrap XBIOS_bootstrap = {XBIOS_VID_DRIVER_NAME, "Atari Xbios driver",
...
static int XBIOS_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  for (i = 0; i < NUM_MODELISTS; ++i) {
    ...
    SDL_xbiosmode[i] = (xbiosmode_t **)SDL_malloc((SDL_nummodes[i] + 1) *
                                                  sizeof(xbiosmode_t *));
...
static SDL_Surface *XBIOS_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  for (i = 0; i < num_buffers; i++) {
    ...
    if (XBIOS_screensmem[i] == NULL) {
      XBIOS_FreeBuffers(this);
      SDL_SetError("Can not allocate %d KB for buffer %d",
                   new_screen_size >> 10, i);
      return (NULL);
    }
    SDL_memset(XBIOS_screensmem[i], 0, new_screen_size);

    XBIOS_screens[i] =
        (void *)(((long)XBIOS_screensmem[i] + 256) & 0xFFFFFF00UL);
  }
...
static int XBIOS_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }

static void XBIOS_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

static int XBIOS_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }

static void XBIOS_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void XBIOS_UpdateRects(_THIS, int numrects, SDL_Rect *rects) {
  ...
  if (XBIOS_current->flags & XBIOSMODE_C2P) {
    ...
    for (i = 0; i < numrects; i++) {
      ...
      SDL_Atari_C2pConvert(source, destination, x2 - x1, rects[i].h, doubleline,
...
static int XBIOS_FlipHWSurface(_THIS, SDL_Surface *surface) {
  if (XBIOS_current->flags & XBIOSMODE_C2P) {
    ...
    /* Convert chunky to planar screen */
    SDL_Atari_C2pConvert(surface->pixels, destscr, surface->w, surface->h,
                         doubleline, surface->pitch, XBIOS_pitch);
  }
