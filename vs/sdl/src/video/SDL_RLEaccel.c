...
#define ALPHA_BLIT32_888_50MMX(to, from, length, bpp, alpha)                   \
  do {                                                                         \
    ...
    if (i & 1) {                                                               \
      ...
      Uint32 d = *dstp;                                                        \
      *dstp++ =                                                                \
          (((s & 0x00fefefe) + (d & 0x00fefefe)) >> 1) + (s & d & 0x00010101); \
      i--;                                                                     \
    }                                                                          \
    for (; i > 0; --i) {                                                       \
      ...
      paddd_r2r(mm6,                                                           \
                mm5); /* (dst & 0x00fefefe) + (dst & 0x00fefefe) -> mm5 */     \
      psrld_i2r(1, mm5);                                                       \
  ...
  do {                                                                         \
    if (alpha == 255) {                                                        \
      ...
      case 2:                                                                  \
        ...
        case 0xffff:                                                           \
          if (fmt->Gmask == 0x07e0 || fmt->Rmask == 0x07e0 ||                  \
              fmt->Bmask == 0x07e0) {                                          \
            if (alpha == 128)                                                  \
          ...
          break;                                                               \
                                                                               \
        case 0x7fff:                                                           \
          if (fmt->Gmask == 0x03e0 || fmt->Rmask == 0x03e0 ||                  \
              fmt->Bmask == 0x03e0) {                                          \
            if (alpha == 128)                                                  \
 ...
 */
static void RLEClipBlit(int w, Uint8 *srcbuf, SDL_Surface *dst, Uint8 *dstbuf,
                        SDL_Rect *srcrect, unsigned alpha) {
      ...
      if (run) {                                                               \
        ...
        if (ofs < right) {                                                     \
          ...
          if (len > right - startcol)                                          \
            len = right - startcol;                                            \
          do_blit(dstbuf + startcol * bpp, srcbuf + start * bpp, len, bpp,     \
                  alpha);                                                      \
        }                                                                      \
...
int SDL_RLEBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                SDL_Rect *dstrect) {
  Uint8 *dstbuf;
  Uint8 *srcbuf;
  int x, y;
  ...
  y = dstrect->y;
  dstbuf =
      (Uint8 *)dst->pixels + y * dst->pitch + x * src->format->BytesPerPixel;
...
#undef RLESKIP
  ...
  alpha =
      (src->flags & SDL_SRCALPHA) == SDL_SRCALPHA ? src->format->alpha : 255;
        ...
        if (run) {                                                             \
          ...
          if (crun > 0)                                                        \
            PIXEL_COPY(dstbuf + cofs * sizeof(Ptype),                          \
                       srcbuf + (cofs - ofs) * sizeof(Ptype), (unsigned)crun,  \
                       sizeof(Ptype));                                         \
...
int SDL_RLEAlphaBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                     SDL_Rect *dstrect) {
  int x, y;
  int w = src->w;
  Uint8 *srcbuf, *dstbuf;
  ...
  do {                                                                         \
    ...
    do {                                                                       \
      ...
      do {                                                                     \
        ...
        srcbuf += 2 * sizeof(Ctype);                                           \
        if (run) {                                                             \
          PIXEL_COPY(dstbuf + ofs * sizeof(Ptype), srcbuf, run,                \
                     sizeof(Ptype));                                           \
          srcbuf += run * sizeof(Ptype);                                       \
          ofs += run;                                                          \
        } else if (!ofs)                                                       \
...
static int copy_opaque_16(void *dst, Uint32 *src, int n, SDL_PixelFormat *sfmt,
...
static int uncopy_opaque_16(Uint32 *dst, void *src, int n, RLEDestFormat *sfmt,
                            SDL_PixelFormat *dfmt) {
  int i;
  Uint16 *s = src;
...
static int copy_transl_565(void *dst, Uint32 *src, int n, SDL_PixelFormat *sfmt,
                           SDL_PixelFormat *dfmt) {
  int i;
  Uint32 *d = dst;
  for (i = 0; i < n; i++) {
...
static int copy_transl_555(void *dst, Uint32 *src, int n, SDL_PixelFormat *sfmt,
                           SDL_PixelFormat *dfmt) {
  int i;
  Uint32 *d = dst;
  for (i = 0; i < n; i++) {
...
static int uncopy_transl_16(Uint32 *dst, void *src, int n, RLEDestFormat *sfmt,
                            SDL_PixelFormat *dfmt) {
  int i;
  Uint32 *s = src;
  for (i = 0; i < n; i++) {
...
static int copy_32(void *dst, Uint32 *src, int n, SDL_PixelFormat *sfmt,
                   SDL_PixelFormat *dfmt) {
  int i;
  Uint32 *d = dst;
  for (i = 0; i < n; i++) {
...
static int uncopy_32(Uint32 *dst, void *src, int n, RLEDestFormat *sfmt,
                     SDL_PixelFormat *dfmt) {
  int i;
  Uint32 *s = src;
  for (i = 0; i < n; i++) {
...
static int RLEAlphaSurface(SDL_Surface *surface) {
  ...
  unsigned masksum;
  Uint8 *rlebuf, *dst;
  int (*copy_opaque)(void *, Uint32 *, int, SDL_PixelFormat *,
                     SDL_PixelFormat *);
  int (*copy_transl)(void *, Uint32 *, int, SDL_PixelFormat *,
                     SDL_PixelFormat *);
...
static Uint32 getpix_8(Uint8 *srcbuf) { return *srcbuf; }

static Uint32 getpix_16(Uint8 *srcbuf) { return *(Uint16 *)srcbuf; }
...
static Uint32 getpix_32(Uint8 *srcbuf) { return *(Uint32 *)srcbuf; }

typedef Uint32 (*getpix_func)(Uint8 *);

static getpix_func getpixes[4] = {getpix_8, getpix_16, getpix_24, getpix_32};
...
static SDL_bool UnRLEAlpha(SDL_Surface *surface) {
  ...
  int (*uncopy_opaque)(Uint32 *, void *, int, RLEDestFormat *,
                       SDL_PixelFormat *);
  int (*uncopy_transl)(Uint32 *, void *, int, RLEDestFormat *,
                       SDL_PixelFormat *);
  int w = surface->w;
