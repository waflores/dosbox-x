...
int SDL_VideoInit(const char *driver_name, Uint32 flags) {
  ...
  SDL_VideoSurface =
      SDL_CreateRGBSurface(video_flags, 0, 0, vformat.BitsPerPixel,
  ...
  for (b = 0; !supported && SDL_closest_depths[table][b]; ++b) {
    ...
    for (i = 0; sizes[i]; ++i) {
      ...
      if ((sizes[i]->w >= *w) && (sizes[i]->h >= *h)) {
        ...
        if ((sizes[i]->w <= sizes[best]->w) ||
            (sizes[i]->h <= sizes[best]->h)) {
          /* Now choose the mode that has less pixels */
          if ((sizes[i]->w * sizes[i]->h) <=
              (sizes[best]->w * sizes[best]->h)) {
            best = i;
            supported = 1;
          }
        }
...
static void SDL_CreateShadowSurface(int depth) {
  ...
  SDL_ShadowSurface =
      SDL_CreateRGBSurface(SDL_SWSURFACE, SDL_VideoSurface->w,
                           SDL_VideoSurface->h, depth, Rmask, Gmask, Bmask, 0);
...
SDL_Surface *SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags) {
  ...
  mode =
      video->SetVideoMode(this, prev_mode, video_w, video_h, video_bpp, flags);
  ...
  if ((mode != NULL) && (!is_opengl)) {
    ...
    fprintf(stderr,
            ...
            width, height, bpp, mode->w, mode->h, mode->format->BitsPerPixel,
            mode->offset);
#endif
    mode->w = width;
    mode->h = height;
...
#undef SDL_PROC
  ...
  if ((flags & SDL_OPENGLBLIT) == SDL_OPENGLBLIT) {
     ...
     */
    if ((bpp == 16) &&
        (SDL_strstr((const char *)video->glGetString(GL_EXTENSIONS),
                    "GL_EXT_packed_pixels") ||
         (SDL_atof((const char *)video->glGetString(GL_VERSION)) >= 1.2f))) {
      video->is_32bit = 0;
      SDL_VideoSurface = SDL_CreateRGBSurface(flags, width, height, 16,
                                              31 << 11, 63 << 5, 31, 0);
    } else
#endif /* OpenGL 1.2 */
    {
    ...
    SDL_memset(SDL_VideoSurface->pixels, 255,
    ...
    video->glTexImage2D(GL_TEXTURE_2D, 0, video->is_32bit ? GL_RGBA : GL_RGB,
                        256, 256, 0, video->is_32bit ? GL_RGBA : GL_RGB,
#ifdef GL_VERSION_1_2
                        video->is_32bit ? GL_UNSIGNED_BYTE
  ...
  */
  if (!(SDL_VideoSurface->flags & SDL_OPENGL) &&
      ((!(flags & SDL_ANYFORMAT) &&
       ...
       ((flags & SDL_DOUBLEBUF) && (SDL_VideoSurface->flags & SDL_HWSURFACE) &&
...
static int SetPalette_physical(SDL_Surface *screen, SDL_Color *colors,
                               int firstcolor, int ncolors) {
  SDL_VideoDevice *video = current_video;
  int gotall = 1;
  ...
  if (video->physpal) {
     ...
     */
    SDL_memcpy(video->physpal->colors + firstcolor, colors,
               ncolors * sizeof(*colors));
  }
  if (screen == SDL_ShadowSurface) {
    if (SDL_VideoSurface->flags & SDL_HWPALETTE) {
      ...
      if (video->gamma) {
        if (!video->gammacols) {
          ...
          SDL_ApplyGamma(video->gamma, pp->colors, video->gammacols,
                         pp->ncolors);
        } else {
          SDL_ApplyGamma(video->gamma, colors, video->gammacols + firstcolor,
                         ncolors);
        }
      }
      SDL_UpdateRect(screen, 0, 0, 0, 0);
 ...
 */
int SDL_SetPalette(SDL_Surface *screen, int which, SDL_Color *colors,
                   int firstcolor, int ncolors) {
  SDL_Palette *pal;
  int gotall;
  int palsize;
...
int SDL_SetColors(SDL_Surface *screen, SDL_Color *colors, int firstcolor,
                  int ncolors) {
  return SDL_SetPalette(screen, SDL_LOGPAL | SDL_PHYSPAL, colors, firstcolor,
                        ncolors);
}
...
/* Update rects without state setting and changing (the caller is responsible
 * for it) */
    ...
    for (y = 0; y <= rects[i].h / 256; y++) {
      ...
      for (x = 0; x <= rects[i].w / 256; x++) {
        ...
        this->glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0, update.w, update.h,
        ...
        (this->glTexCoord2f)((float)(update.w / 256.0),
                             (float)(update.h / 256.0));
...
void SDL_GL_Lock() {
  ...
  if (lock_count == -1) {
    ...
    this->glPushAttrib(
        GL_ALL_ATTRIB_BITS); /* TODO: narrow range of what is saved */
    ...
    this->glOrtho(0.0, (GLdouble)this->screen->w, (GLdouble)this->screen->h,
                  0.0, 0.0, 1.0);
...
static void CreateMaskFromColorKeyOrAlpha(SDL_Surface *icon, Uint8 *mask,
                                          int flags) {
  int x, y;
  Uint32 colorkey;
...
/* DOSBox-X Hack API: Allow host program to determine size/position of the
 * fullscreen window */
