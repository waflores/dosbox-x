...
} inputs[] = {{"keyboard", &GUID_SysKeyboard, &c_dfDIKeyboard,
...
static int DX5_DInputInit(_THIS) {
  ...
  for (i = 0; inputs[i].name; ++i) {
    ...
    result = IDirectInput_CreateDevice(dinput, inputs[i].guid, &device, NULL);
    if (result != DI_OK) {
      SetDIerror("DirectInput::CreateDevice", result);
      return (-1);
    }
    result = IDirectInputDevice_QueryInterface(device, &IID_IDirectInputDevice2,
                                               (LPVOID *)&SDL_DIdev[i]);
    ...
    result = IDirectInputDevice2_SetCooperativeLevel(SDL_DIdev[i], topwnd,
                                                     inputs[i].win_level);
    if (result != DI_OK) {
      SetDIerror("DirectInputDevice::SetCooperativeLevel", result);
      return (-1);
    }
    result = IDirectInputDevice2_SetDataFormat(SDL_DIdev[i], inputs[i].format);
    if (result != DI_OK) {
    ...
    result = IDirectInputDevice2_SetProperty(SDL_DIdev[i], DIPROP_BUFFERSIZE,
                                             &dipdw.diph);
    if (result != DI_OK) {
    ...
    result =
        IDirectInputDevice2_SetEventNotification(SDL_DIdev[i], SDL_DIevt[i]);
    if (result != DI_OK) {
  ...
  for (i = 0; i < numevents; ++i) {
    if (keybuf[i].dwData & 0x80) {
      ...
      if (!IM_Context.bCompos &&
          (GetTickCount() - end_ticks > IME_END_CR_WAIT ||
           keybuf[i].dwOfs != 0x1c))
#endif
      {
...
static void post_mouse_motion(int relative, Sint16 x, Sint16 y) {
  ...
  if (SDL_GetAppState() & SDL_APPMOUSEFOCUS) {
    posted = SDL_PrivateMouseMotion(0, relative, x, y);
    ...
    if (!mouse_relative) {
      ...
      at_edge = (current_x == window.left) ||
                ...
                (current_y == window.top) || (current_y == (window.bottom - 1));
...
LRESULT DX5_HandleMessage(_THIS, HWND hwnd, UINT msg, WPARAM wParam,
                          LPARAM lParam) {
  switch (msg) {
    ...
    if ((lParam & 0xff0000) == 0x360000) {
      ...
      if (msg == WM_KEYDOWN) {
        SDL_PrivateKeyboard(SDL_PRESSED,
                            TranslateKey((lParam >> 16) & 0xff, &keysym, 1));
      } else if (msg == WM_KEYUP) {
        SDL_PrivateKeyboard(SDL_RELEASED,
                            TranslateKey((lParam >> 16) & 0xff, &keysym, 0));
      }
    }
    if (msg == WM_KEYDOWN) {
...
static int DX5_CheckInput(_THIS, int timeout, BOOL processInput) {
  ...
  event = MsgWaitForMultipleObjects(SDL_DIndev, SDL_DIevt, FALSE, timeout,
                                    QS_ALLEVENTS);
  if ((event >= WAIT_OBJECT_0) && (event < (WAIT_OBJECT_0 + SDL_DIndev))) {
    ...
    result = IDirectInputDevice2_GetDeviceData(
        SDL_DIdev[event], sizeof(DIDEVICEOBJECTDATA), evtbuf, &numevents, 0);
    if ((result == DIERR_INPUTLOST) || (result == DIERR_NOTACQUIRED)) {
...
void DX5_DInputReset(_THIS, int fullscreen) {
  ...
  for (i = 0; i < MAX_INPUTS; ++i) {
    if (SDL_DIdev[i] != NULL) {
      ...
      topwnd = GetTopLevelParent(SDL_Window);
      result =
          IDirectInputDevice2_SetCooperativeLevel(SDL_DIdev[i], topwnd, level);
      IDirectInputDevice2_Acquire(SDL_DIdev[i]);
      if (result != DI_OK) {
        SetDIerror("DirectInputDevice::SetCooperativeLevel", result);
      }
...
#define DIK_OEM_102 0x56 /* < > | on UK/Germany keyboards */
  ...
  case 0x411: /* JP */
    // DIK_keymap[DIK_KANJI] = SDLK_WORLD_12;  // Hankaku/Zenkaku (FIX ME: This
    // key works as a toggle key)
    DIK_keymap[DIK_CONVERT] = SDLK_WORLD_14;   // henkan
    DIK_keymap[DIK_NOCONVERT] = SDLK_WORLD_13; // muhenkan
    // DIK_keymap[DIK_KANA] = SDLK_WORLD_15;   // Hiragana/Katakana  (FIX ME:
    // This key works as a toggle key)
...
static SDL_keysym *TranslateKey(UINT scancode, SDL_keysym *keysym,
  ...
  if (pressed && SDL_TranslateUNICODE) {
    ...
    } else if (SDL_ToUnicode(vkey, scancode, keystate, wchars,
...
int DX5_SetIMPosition(_THIS, int x, int y) { return 0; }
char *DX5_SetIMValues(_THIS, SDL_imvalue value, int alt) { return NULL; }
char *DX5_GetIMValues(_THIS, SDL_imvalue value, int *alt) { return NULL; }
int DX5_FlushIMString(_THIS, void *buffer) { return 0; }
#endif