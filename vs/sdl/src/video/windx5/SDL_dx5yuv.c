...
static struct private_yuvhwfuncs dx5_yuvfuncs = {
    DX5_LockYUVOverlay, DX5_UnlockYUVOverlay, DX5_DisplayYUVOverlay,
    DX5_FreeYUVOverlay};
...
static LPDIRECTDRAWSURFACE3 CreateYUVSurface(_THIS, int width, int height,
                                             Uint32 format) {
  HRESULT result;
  ...
  SDL_memset(&ddsd, 0, sizeof(ddsd));
  ddsd.dwSize = sizeof(ddsd);
  result = IDirectDrawSurface3_Lock(dd_surface3, NULL, &ddsd, DDLOCK_NOSYSLOCK,
                                    NULL);
  if (result != DD_OK) {
...
SDL_Overlay *DX5_CreateYUVOverlay(_THIS, int width, int height, Uint32 format,
                                  SDL_Surface *display) {
  SDL_Overlay *overlay;
...
int DX5_LockYUVOverlay(_THIS, SDL_Overlay *overlay) {
  ...
  ddsd.dwSize = sizeof(ddsd);
  result =
      IDirectDrawSurface3_Lock(surface, NULL, &ddsd, DDLOCK_NOSYSLOCK, NULL);
...
int DX5_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                          SDL_Rect *dst) {
  HRESULT result;
  LPDIRECTDRAWSURFACE3 surface;
...
#ifdef USE_DIRECTX_OVERLAY
  result = IDirectDrawSurface3_UpdateOverlay(surface, &srcrect, SDL_primary,
