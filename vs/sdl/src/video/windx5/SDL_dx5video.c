...
static int DX5_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors);
...
static int DX5_Available(void) {
  ...
  if (DDrawDLL != NULL) {
    ...
    if ((DDrawCreate != NULL) && !FAILED(DDrawCreate(NULL, &DDraw, NULL))) {
      if (!FAILED(IDirectDraw_SetCooperativeLevel(DDraw, NULL, DDSCL_NORMAL))) {
        DDSURFACEDESC desc;
        ...
        if (!FAILED(
          ...
          if (!FAILED(IDirectDrawSurface_QueryInterface(
...
int DX5_GetWMInfo(_THIS, SDL_SysWMinfo *info) {
  if (info->version.major <= SDL_MAJOR_VERSION) {
    ...
    if (SDL_VERSIONNUM(info->version.major, info->version.minor,
...
static int DX5_Load(void) {
  ...
  if (DInputDLL != NULL) {
    DInputCreate =
        (void *)GetProcAddress(DInputDLL, TEXT("DirectInputCreateA"));
  }
...
VideoBootStrap DIRECTX_bootstrap = {"directx", "Win95/98/2000 DirectX",
...
static HRESULT WINAPI EnumModes2(DDSURFACEDESC *desc, VOID *udata) {
  ...
  case 32:
    bpp /= 8;
    --bpp;
    if (enumlists[bpp] && enumlists[bpp]->r.w == (Uint16)desc->dwWidth &&
    ...
    fprintf(stderr, "New mode for %d bpp: %dx%d at %d Hz\n", (bpp + 1) * 8,
            (int)desc->dwWidth, (int)desc->dwHeight, refreshRate);
#endif
    break;
  }
...
#error Your version of DirectX must be greater than or equal to 5.0
  ...
  /* Find out how much video memory is available */
  {
    ...
    result =
        IDirectDraw2_GetAvailableVidMem(ddraw2, &ddsCaps, &total_mem, NULL);
    if (result != DD_OK) {
...
int DX5_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  result =
      IDirectDraw_QueryInterface(ddraw, &IID_IDirectDraw2, (LPVOID *)&ddraw2);
  IDirectDraw_Release(ddraw);
  if (result != DD_OK) {
  ...
  result = IDirectDraw2_EnumDisplayModes(ddraw2, DDEDM_REFRESHRATES, NULL, this,
                                         EnumModes2);
  if (result != DD_OK) {
  ...
  for (i = 0; i < NUM_MODELISTS; ++i) {
    ...
    SDL_modelist[i] =
        (SDL_Rect **)SDL_malloc((SDL_nummodes[i] + 1) * sizeof(SDL_Rect *));
    ...
    if (SDL_nummodes[i] > 0) {
      SDL_qsort(SDL_modelist[i], SDL_nummodes[i], sizeof *SDL_modelist[i],
                cmpmodes);
    }
  }
...
SDL_Surface *DX5_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  SDL_Surface *video;
  int prev_w = -1;
  int prev_h = -1;
  ...
  DWORD resizestyle = (WS_THICKFRAME | WS_MAXIMIZEBOX);
  DDSURFACEDESC ddsd;
  ...
  if (flags & SDL_OPENGL) {
    ...
    if (bpp == current->format->BitsPerPixel) {
      ...
      video = SDL_CreateRGBSurface(SDL_SWSURFACE, 0, 0, bpp, Rmask, Gmask,
                                   Bmask, 0);
      if (video == NULL) {
        SDL_OutOfMemory();
        return (NULL);
  ...
  if ((flags & SDL_FULLSCREEN) == SDL_FULLSCREEN) {
    ...
    result = IDirectDraw2_SetDisplayMode(ddraw2, width, height, bpp,
                                         maxRefreshRate, 0);
    if (result != DD_OK) {
      result = IDirectDraw2_SetDisplayMode(ddraw2, width, height, bpp, 0, 0);
      if (result != DD_OK) {
        /* We couldn't set fullscreen mode, try window */
        return (DX5_SetVideoMode(this, current, width, height, bpp,
  ...
  result = IDirectDrawSurface_QueryInterface(
      dd_surface1, &IID_IDirectDrawSurface3, (LPVOID *)&SDL_primary);
  if (result != DD_OK) {
  ...
  if (ddsd.ddpfPixelFormat.dwRGBBitCount == 8) {
    ...
    result =
        IDirectDraw2_CreatePalette(ddraw2, (DDPCAPS_8BIT | DDPCAPS_ALLOW256),
  ...
  */
  ...
  if ((flags & SDL_FULLSCREEN) != SDL_FULLSCREEN) {
    ...
    if (SDL_clipper == NULL) {
      result = IDirectDraw2_CreateClipper(ddraw2, 0, &SDL_clipper, NULL);
      if (result != DD_OK) {
        if (video != current) {
          SDL_FreeSurface(video);
        }
...
static int DX5_AllocDDSurface(_THIS, SDL_Surface *surface,
  ...
  surface->hwdata =
      (struct private_hwdata *)SDL_malloc(sizeof(*surface->hwdata));
  ...
  ddsd.dwSize = sizeof(ddsd);
  ddsd.dwFlags =
      (DDSD_WIDTH | DDSD_HEIGHT | DDSD_CAPS | DDSD_PITCH | DDSD_PIXELFORMAT);
  ...
  if (requested != NULL) {
    ...
    result = IDirectDrawSurface_QueryInterface(
        dd_surface1, &IID_IDirectDrawSurface3, (LPVOID *)&dd_surface3);
    IDirectDrawSurface_Release(dd_surface1);
    if (result != DD_OK) {
      SetDDerror("DirectDrawSurface::QueryInterface", result);
      goto error_end;
  ...
  if ((flag & SDL_HWSURFACE) == SDL_HWSURFACE) {
    ...
    */
    ...
    if ((ddsd.ddsCaps.dwCaps & DDSCAPS_VIDEOMEMORY) != DDSCAPS_VIDEOMEMORY) {
  ...
  SDL_memset(&ddsd, 0, sizeof(ddsd));
  ddsd.dwSize = sizeof(ddsd);
  result = IDirectDrawSurface3_Lock(dd_surface3, NULL, &ddsd,
  ...
  if ((ddsd.ddpfPixelFormat.u1.dwRGBBitCount !=
      ...
      (ddsd.ddpfPixelFormat.u4.dwBBitMask != surface->format->Bmask)) {
#else
  if ((ddsd.ddpfPixelFormat.dwRGBBitCount != surface->format->BitsPerPixel) ||
      (ddsd.ddpfPixelFormat.dwRBitMask != surface->format->Rmask) ||
      (ddsd.ddpfPixelFormat.dwGBitMask != surface->format->Gmask) ||
      (ddsd.ddpfPixelFormat.dwBBitMask != surface->format->Bmask)) {
#endif
  ...
  if ((surface->flags & SDL_DOUBLEBUF) == SDL_DOUBLEBUF) {
    ...
    result = IDirectDrawSurface3_GetAttachedSurface(dd_surface3, &ddsd.ddsCaps,
                                                    &dd_writebuf);
    if (result != DD_OK) {
...
static int DX5_HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect,
  ...
  /* Do the blit! */
  result = IDirectDrawSurface3_BltFast(dst_surface, dstrect->x, dstrect->y,
                                       src_surface, &rect, flags);
  if (result != DD_OK) {
...
#endif
  result = IDirectDrawSurface3_Blt(dst_surface, &area, NULL, NULL,
...
static int DX5_SetHWColorKey(_THIS, SDL_Surface *surface, Uint32 key) {
  ...
  result = IDirectDrawSurface3_SetColorKey(surface->hwdata->dd_surface,
...
static int DX5_LockHWSurface(_THIS, SDL_Surface *surface) {
  ...
  if (surface->pitch != ddsd.lPitch) {
    ...
    surface->offset =
        ...
        ((ddsd.dwWidth - surface->w) / 2) * surface->format->BytesPerPixel;
  }
...
static int DX5_FlipHWSurface(_THIS, SDL_Surface *surface) {
  ...
  /* to prevent big slowdown on fast computers, wait here instead of driver ring
   * 0 code */
  /* Dmitry Yakimov (ftech@tula.net) */
#ifdef ENABLE_IM_EVENT
  while (IDirectDrawSurface3_GetFlipStatus(ddFrontBuffer, DDGBS_ISBLTDONE) ==
  ...
  if (_FSIM_IsActive()) {
    ...
    if (IM_Context.video_flags & SDL_DOUBLEBUF) {
      ...
      result = IDirectDrawSurface3_Blt(ddFrontBuffer, NULL, ddBackBuffer, NULL,
                                       DDBLT_WAIT, NULL);
      if (result == DDERR_SURFACELOST) {
        result = IDirectDrawSurface3_Restore(ddFrontBuffer);
        while (IDirectDrawSurface3_GetFlipStatus(
                   ddFrontBuffer, DDGBS_ISBLTDONE) == DDERR_WASSTILLDRAWING)
          ;
        result = IDirectDrawSurface3_Blt(ddFrontBuffer, NULL, ddBackBuffer,
...
static void DX5_CompressPalette(_THIS, SDL_Color *colors, int ncolors,
                                int maxcolors) {
...
int DX5_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  ...
  if (SDL_palette != NULL) {
    if ((this->screen->flags & SDL_FULLSCREEN) == SDL_FULLSCREEN) {
      ...
      IDirectDrawPalette_SetEntries(SDL_palette, 0, firstcolor, ncolors,
...
static int DX5_SetGammaRamp(_THIS, Uint16 *ramp) {
  ...
  result = IDirectDrawSurface3_QueryInterface(
      SDL_primary, &IID_IDirectDrawGammaControl, (LPVOID *)&gamma);
  if (result != DD_OK) {
...
void DX5_PaletteChanged(_THIS, HWND window) {
  ...
  if (!colorchange_expected) {
    ...
    SDL_memcpy(saved, palette->colors, palette->ncolors * sizeof(SDL_Color));
  }
  ...
  if (!colorchange_expected) {
    ...
    for (i = 0; i < palette->ncolors; ++i) {
      mapping[i] = SDL_FindColor(palette, saved[i].r, saved[i].g, saved[i].b);
    }
...
static int setup_ime(_THIS, LPDIRECTDRAWSURFACE3 lpFrontBuffer, Uint32 flags) {
  ...
  if ((flags & SDL_FULLSCREEN) && (flags & SDL_DOUBLEBUF)) {
    if (i = IDirectDrawSurface3_QueryInterface(
                lpFrontBuffer, &IID_IDirectDrawSurface3,
                (LPVOID *)&ddFrontBuffer) != DD_OK) {
      SetDDerror("IDirectDrawSurface3_QueryInterface", i);
      printf("%s\n", SDL_GetError());
    }

    if (i = IDirectDrawSurface3_GetDDInterface(ddFrontBuffer, &_ddraw2) !=
            DD_OK) {
    ...
    if (i = IDirectDrawSurface3_GetAttachedSurface(
                ddFrontBuffer, (LPDDSCAPS)&ddscaps, &ddBackBuffer) != DD_OK) {
      SetDDerror("IDirectDrawSurface3_GetAttachedSurface", i);
      printf("%s\n", SDL_GetError());
    } else {
      if (!_FSIM_Init(SDL_Window, (IDirectDraw *)_ddraw2,
...
HRESULT _FSIM_Update(void) {
  ...
  return IDirectDrawSurface3_Blt(ddFrontBuffer, NULL, ddBackBuffer, NULL,
                                 DDBLT_WAIT, NULL);
}
