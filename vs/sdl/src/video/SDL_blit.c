...
static int SDL_SoftBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                        SDL_Rect *dstrect) {
  int okay;
  int src_locked;
  int dst_locked;
  ...
  if (okay && srcrect->w && srcrect->h) {
    ...
    info.s_pixels = (Uint8 *)src->pixels + (Uint16)srcrect->y * src->pitch +
                    (Uint16)srcrect->x * src->format->BytesPerPixel;
    info.s_width = srcrect->w;
    info.s_height = srcrect->h;
    info.s_skip = src->pitch - info.s_width * src->format->BytesPerPixel;
    info.d_pixels = (Uint8 *)dst->pixels + (Uint16)dstrect->y * dst->pitch +
...
int SDL_CalculateBlit(SDL_Surface *surface) {
  ...
  if (((surface->flags & SDL_HWSURFACE) == SDL_HWSURFACE) &&
      (current_video->displayformatalphapixel)) {
  ...
  if (surface->flags & SDL_SRCALPHA &&
      (surface->format->alpha != SDL_ALPHA_OPAQUE || surface->format->Amask)) {
    blit_index |= 2;
  }
  ...
  if (surface->flags & SDL_RLEACCELOK &&
    ...
    if (surface->map->identity &&
        (blit_index == 1 || (blit_index == 3 && !surface->format->Amask))) {
