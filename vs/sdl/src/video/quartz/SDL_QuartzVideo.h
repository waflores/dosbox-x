...
/*
    ...
    TODO
        ...
        - Fix white OpenGL window on minimize (fixed) (update: broken again
   on 10.2)
        - Find out what events should be sent/ignored if window is minimized
        - Find a way to deal with external resolution/depth switch while app is
   running
    ...
    Problems:
        ...
        - Retained windows don't draw their title bar quite right (OS Bug) (not
   using retained windows)
        - Cursor in 8 bit modes is screwy (might just be Radeon PCI bug)
   (update: not just Radeon)
...
typedef struct SDL_PrivateVideoData {
  ...
  SDLTranslatorResponder
      *field_edit; /* a field editor for keyboard composition processing */
  ...
  NSPoint cursor_loc;    /* saved cursor coords, for activate/deactivate when
                            grabbed */
  BOOL cursor_should_be_visible; /* tells if cursor is supposed to be visible
                                    (SDL_ShowCursor) */
  BOOL cursor_visible;   /* tells if cursor is *actually* visible or not */
  Uint8 *sw_buffers[2];  /* pointers to the two software buffers for
  ...
  SInt32 system_version; /* used to dis-/enable workarounds depending on the
                            system version */
...
enum { QZ_UNGRABBED = 0, QZ_VISIBLE_GRAB, QZ_INVISIBLE_GRAB };

/* grab actions - these can change the grabbed state */
enum { QZ_ENABLE_GRAB = 0, QZ_DISABLE_GRAB, QZ_HIDECURSOR, QZ_SHOWCURSOR };
...
WMcursor *QZ_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                            int hot_x, int hot_y);
