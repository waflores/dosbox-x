...
static struct private_yuvhwfuncs ps3_yuvfuncs = {
    PS3_LockYUVOverlay, PS3_UnlockYUVOverlay, PS3_DisplayYUVOverlay,
    PS3_FreeYUVOverlay};
...
struct private_yuvhwdata {
  ...
  volatile struct yuv2rgb_parms_t *converter_parms
...
SDL_Overlay *PS3_CreateYUVOverlay(_THIS, int width, int height, Uint32 format,
                                  SDL_Surface *display) {
  ...
  hwdata = (struct private_yuvhwdata *)SDL_calloc(
      1, sizeof(struct private_yuvhwdata));
  if (hwdata == NULL) {
    SDL_OutOfMemory();
  ...
  hwdata->converter_parms =
      (struct yuv2rgb_parms_t *)memalign(16, sizeof(struct yuv2rgb_parms_t));
  hwdata->scaler_parms =
      (struct scale_parms_t *)memalign(16, sizeof(struct scale_parms_t));
  ...
  hwdata->pixels =
      (Uint8 *)memalign(16, width * height + ((width * height) >> 1));
...
int PS3_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                          SDL_Rect *dst) {
  ...
  if (hwdata->scale) {
    ...
    hwdata->scaler_out =
        (Uint8 *)memalign(16, dst->w * dst->h + ((dst->w * dst->h) >> 1));
    ...
    hwdata->converter_parms->u_plane =
        hwdata->scaler_out + dst->w * dst->h + ((dst->w * dst->h) >> 2);
  } else {
  ...
  SPE_SendMsg(this, converter_thread_data,
