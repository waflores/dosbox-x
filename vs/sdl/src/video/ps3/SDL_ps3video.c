...
static SDL_Surface *PS3_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
static int PS3_Available() { return 1; }
...
static SDL_VideoDevice *PS3_CreateDevice(int devindex) {
  ...
  if (this) {
    ...
    this->hidden = (struct SDL_PrivateVideoData *)SDL_malloc(
...
VideoBootStrap PS3_bootstrap = {"ps3", "PS3 Cell SPU Driver", PS3_Available,
                                PS3_CreateDevice};
...
static int PS3_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  fb_parms = (struct fb_writer_parms_t *)memalign(
      16, sizeof(struct fb_writer_parms_t));
  ...
  if (fb_bits_per_pixel == 16)
    fb_bits_per_pixel =
        fb_vinfo.red.length + fb_vinfo.green.length + fb_vinfo.blue.length;
...
static SDL_Rect **PS3_ListModes(_THIS, SDL_PixelFormat *format, Uint32 flags) {
  ...
  static SDL_Rect *PS3_modes[] = {&PS3_resolutions[0], &PS3_resolutions[1],
                                  &PS3_resolutions[2], &PS3_resolutions[3],
                                  &PS3_resolutions[4], &PS3_resolutions[5],
                                  &PS3_resolutions[6], &PS3_resolutions[7],
                                  &PS3_resolutions[8], NULL};
...
static SDL_Surface *PS3_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  s_bounded_input_height_offset =
      (s_writeable_height - s_bounded_input_height) >> 1;
  ...
  if (fb_finfo.type != FB_TYPE_PACKED_PIXELS) {
    SDL_SetError("[PS3] type %s not supported", fb_finfo.type);
    return NULL;
  }

  /* Note: on PS3, fb_finfo.smem_len is enough for double buffering */
  if ((frame_buffer =
           (uint8_t *)mmap(0, fb_finfo.smem_len, PROT_READ | PROT_WRITE,
...
static int PS3_AllocHWSurface(_THIS, SDL_Surface *surface) { return -1; }

static void PS3_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

static int PS3_LockHWSurface(_THIS, SDL_Surface *surface) { return 0; }

static void PS3_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
int SPE_Start(_THIS, spu_data_t *spe_data) {
  ...
  /* To allow re-running of context, spe_ctx_entry has to be set before each
   * call */
  ...
  if (pthread_create(&spe_data->thread, NULL, (void *)&SPE_RunContext,
                     (void *)spe_data)) {
...
int SPE_Boot(_THIS, spu_data_t *spe_data) {
  ...
  if (spe_program_load(spe_data->ctx, &spe_data->program)) {
    deprintf(2, "[PS3->SPU] Failed loading program into SPE context: %s\n",
             spe_data->program_name);
    SDL_SetError("[PS3->SPU] Failed loading program into SPE context");
    return -1;
  }
  spe_data->booted = 1;
...
int SPE_SendMsg(_THIS, spu_data_t *spe_data, unsigned int msg) {
  ...
  int in_mbox_write = spe_in_mbox_write(spe_data->ctx, spe_in_mbox_msgs, 1,
...
void SPE_RunContext(void *thread_argp) {
  ...
  if (spe_context_run(args->ctx, &args->entry, 0, (void *)args->argp, NULL,
                      NULL) < 0) {
    deprintf(2, "[PS3->SPU] Failed running SPE context: %s\n",
             args->program_name);
    SDL_SetError("[PS3->SPU] Failed running SPE context: %s",
                 args->program_name);
    exit(1);
  }
