...
static struct private_yuvhwfuncs directfb_yuvfuncs = {
    DirectFB_LockYUVOverlay, DirectFB_UnlockYUVOverlay,
    DirectFB_DisplayYUVOverlay, DirectFB_FreeYUVOverlay};
...
enum_layers_callback(DFBDisplayLayerID id, DFBDisplayLayerDescription desc,
                     void *data) {
...
SDL_Overlay *DirectFB_CreateYUVOverlay(_THIS, int width, int height,
...
int DirectFB_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                               SDL_Rect *dst) {
  DFBResult ret;
  DFBDisplayLayerConfig conf;
  ...
  ret = layer->SetScreenLocation(
      layer, dst->x / (float)conf.width, dst->y / (float)conf.height,
