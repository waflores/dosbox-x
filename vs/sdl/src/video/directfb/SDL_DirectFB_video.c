...
static int DirectFB_Available(void) { return 1; }
...
VideoBootStrap DirectFB_bootstrap = {"directfb", "DirectFB", DirectFB_Available,
                                     DirectFB_CreateDevice};

static DFBSurfacePixelFormat GetFormatForBpp(int bpp,
...
static DFBEnumerationResult EnumModesCallback(int width, int height, int bpp,
                                              void *data) {
...
static DFBSurfacePixelFormat SDLToDFBPixelFormat(SDL_PixelFormat *format) {
  if (format->Rmask && format->Gmask && format->Bmask) {
    ...
    case 16:
      if (format->Rmask == 0xF800 && format->Gmask == 0x07E0 &&
          format->Bmask == 0x001F)
        return DSPF_RGB16;
      /* fall through */

    case 15:
      if (format->Rmask == 0x7C00 && format->Gmask == 0x03E0 &&
          format->Bmask == 0x001F)
...
static int DFBToSDLPixelFormat(DFBSurfacePixelFormat pixelformat,
...
int DirectFB_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  if (HIDDEN->enable_mga_crtc2) {
    ...
    ret =
        HIDDEN->c2layer->SetCooperativeLevel(HIDDEN->c2layer, DLSCL_EXCLUSIVE);
    if (ret) {
    ...
    if (SDL_getenv("SDL_DIRECTFB_MGA_OVERSCAN") != NULL) {
      float overscan = 0;
      if (SDL_sscanf(SDL_getenv("SDL_DIRECTFB_MGA_OVERSCAN"), "%f",
                     &overscan) == 1)
  ...
  if (HIDDEN->enable_mga_crtc2) {
    ...
    if (SDL_getenv("SDL_DIRECTFB_MGA_STRETCH") != NULL) {
      ...
      printf("Source resolution: X: %d, Y: %d, Aspect ratio: %d:%d\n", width,
             height, zoom_aspect_x, zoom_aspect_y);
      printf("CRTC2 resolution: X: %d, Y: %d\n", HIDDEN->c2framesize.w,
             HIDDEN->c2framesize.h);
#endif
      ...
      if ((float)width < (float)HIDDEN->c2framesize.w * 0.95 ||
        ...
        while (
            (float)HIDDEN->c2dsize.w < (float)HIDDEN->c2framesize.w *
                                           HIDDEN->mga_crtc2_stretch_overscan &&
            (float)HIDDEN->c2dsize.h < (float)HIDDEN->c2framesize.h *
                                           HIDDEN->mga_crtc2_stretch_overscan) {
...
static int DirectFB_AllocHWSurface(_THIS, SDL_Surface *surface) {
  ...
  /*  fprintf(stderr, "SDL: DirectFB_AllocHWSurface (%dx%d@%d, flags:
  ...
  /* Create the surface */
  ret =
      HIDDEN->dfb->CreateSurface(HIDDEN->dfb, &dsc, &surface->hwdata->surface);
  if (ret) {
    SetDirectFBerror("dfb->CreateSurface", ret);
    free(surface->hwdata);
    surface->hwdata = NULL;
...
static int DirectFB_CheckHWBlit(_THIS, SDL_Surface *src, SDL_Surface *dst) {
  /*  fprintf(stderr, "SDL: DirectFB_CheckHWBlit (src->hwdata: %p, dst->hwdata:
...
static int DirectFB_FillHWRect(_THIS, SDL_Surface *dst, SDL_Rect *dstrect,
  ...
  /* ugly */
  surface->SetColor(surface, (color & fmt->Rmask) >> (fmt->Rshift - fmt->Rloss),
...
static int DirectFB_FlipHWSurface(_THIS, SDL_Surface *surface) {
  if (HIDDEN->enable_mga_crtc2) {
    ...
    if (HIDDEN->mga_crtc2_stretch)
      HIDDEN->c2frame->StretchBlit(HIDDEN->c2frame, surface->hwdata->surface,
                                   &HIDDEN->c2ssize, &HIDDEN->c2dsize);
    else
      HIDDEN->c2frame->Blit(HIDDEN->c2frame, surface->hwdata->surface, NULL,
...
static int DirectFB_LockHWSurface(_THIS, SDL_Surface *surface) {
  ...
  ret = surface->hwdata->surface->Lock(surface->hwdata->surface, DSLF_WRITE,
                                       &data, &pitch);
  if (ret) {
...
static void DirectFB_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) {
  if (HIDDEN->enable_mga_crtc2) {
    if (HIDDEN->mga_crtc2_stretch)
      ...
      HIDDEN->c2frame->Blit(HIDDEN->c2frame, this->screen->hwdata->surface,
                            NULL, HIDDEN->c2dsize.x, HIDDEN->c2dsize.y);
...
static void DirectFB_WindowedUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  if (region_valid) {
    if (HIDDEN->enable_mga_crtc2) {
      if (HIDDEN->mga_crtc2_stretch)
        HIDDEN->c2frame->StretchBlit(HIDDEN->c2frame, surface, &HIDDEN->c2ssize,
                                     &HIDDEN->c2dsize);
      else
        HIDDEN->c2frame->Blit(HIDDEN->c2frame, surface, NULL, HIDDEN->c2dsize.x,
                              HIDDEN->c2dsize.y);
