 ...
 */
SDL_PixelFormat *SDL_AllocFormat(int bpp, Uint32 Rmask, Uint32 Gmask,
  ...
  if (bpp <= 8) { /* Palettized mode */
    ...
    (format->palette)->colors =
        (SDL_Color *)SDL_malloc((format->palette)->ncolors * sizeof(SDL_Color));
...
SDL_PixelFormat *SDL_ReallocFormat(SDL_Surface *surface, int bpp, Uint32 Rmask,
...
Uint32 SDL_MapRGB(const SDL_PixelFormat *const format, const Uint8 r,
...
Uint32 SDL_MapRGBA(const SDL_PixelFormat *const format, const Uint8 r,
...
void SDL_GetRGBA(Uint32 pixel, const SDL_PixelFormat *const fmt, Uint8 *r,
...
void SDL_GetRGB(Uint32 pixel, const SDL_PixelFormat *const fmt, Uint8 *r,
                Uint8 *g, Uint8 *b) {
  if (fmt->palette == NULL) {
...
static Uint8 *Map1to1(SDL_Palette *src, SDL_Palette *dst, int *identical) {
  ...
  if (identical) {
    if (src->ncolors <= dst->ncolors) {
      ...
      if (SDL_memcmp(src->colors, dst->colors,
  ...
  for (i = 0; i < src->ncolors; ++i) {
    map[i] = SDL_FindColor(dst, src->colors[i].r, src->colors[i].g,
                           src->colors[i].b);
  }
  return (map);
}
...
static Uint8 *Map1toN(SDL_PixelFormat *src, SDL_PixelFormat *dst) {
  ...
  for (i = 0; i < pal->ncolors; ++i) {
    ASSEMBLE_RGBA(&map[i * bpp], dst->BytesPerPixel, dst, pal->colors[i].r,
                  pal->colors[i].g, pal->colors[i].b, alpha);
  }
  return (map);
}
/* Map from BitField to Dithered-Palette to Palette */
static Uint8 *MapNto1(SDL_PixelFormat *src, SDL_PixelFormat *dst,
                      int *identical) {
