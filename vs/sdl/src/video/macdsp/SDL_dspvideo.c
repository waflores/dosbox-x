...
------------------------------------------------------------------------------------
  ...
  Namely, I am using GetOSEvent() instead of WaitNextEvent() so that there are
no context switches (the app hogs the CPU). Eventually a book-keeping system
should be coded to take care of (1) and (2).
...
------------------------------------------------------------------------------------
  ...
  3. *May also require certain graphics hardware for (2). I trust that all Apple
OEM hardware will work. Third party accelerators may work if they have QuickDraw
     acceleration in the drivers and the drivers have been updated for OS 9. The
current Voodoo 3 drivers (1.0b12) do not work.

  Coding suggestions:

  1. Use SDL_UpdateRects !

    If no QuickDraw acceleration is present, double-buffered surfaces will use a
back buffer in System memory. I recommend you use SDL_UpdateRects with
double-buffered surfaces for best performance on these cards, since the overhead
is nearly zero for VRAM back buffer.

  2. Load most-resident surfaces first.

    If you fill up VRAM or AGP memory, there is no contingency for purging to
make room for the next one. Therefore, you should load the surfaces you plan to
use the most frequently first. Sooner or later, I will code LRU replacement to
help this.
  ...
  I can't create a hardware surface the same size as the screen?! How to fix?
   ...
   DSP_TRY_CC_AND_AA - Define if you want to try HWA color-key and alpha
blitters HW color-key blitting gives substantial improvements, but hw alpha is
neck-and-neck with SDL's soft bitter.

   DSP_NO_SYNC_VBL   - Define for HWA double-buffered surfaces: don't sync
                       pseudo-flip to monitor redraw.

   DSP_NO_SYNC_OPENGL - Define for OpenGL surfaces: don't sync buffer swap.
Synching buffer swap may result in reduced performance, but can eliminate some
                        tearing artifacts.
   CHANGELOG:
   09/17/00 Lots of little tweaks. Build modelist in reverse order so largest
contexts list first. Compared various methods with ROM methods and fixed rez
switch crashing bug in GL Tron. (Woohoo!)
*/
...
static SDL_VideoDevice *DSp_CreateDevice(int devindex) {
  ...
  device->hidden->dspinfo =
      (struct DSpInfo *)SDL_malloc((sizeof *device->hidden->dspinfo));
...
VideoBootStrap DSp_bootstrap = {"DSp", "MacOS DrawSprocket", DSp_Available,
                                DSp_CreateDevice};

/* Use DSp/Display Manager to build mode list for given screen */
static SDL_Rect **DSp_BuildModeList(const GDHandle gDevice, int *displayWidth,
                                    int *displayHeight) {
  ...
  *displayHeight = attributes.displayHeight;
  ...
  for (i = 0; i < SDL_arraysize(temp_list); i++) {
    ...
    /* When there are no more contexts to look at, we will quit building the
     * list*/
...
static void DSp_IsHWAvailable(_THIS, SDL_PixelFormat *vformat) {
  ...
  */
  ...
  if (0x00000860 < (versionSystem & 0x0000FFFF)) {
    ...
    err = NewGWorld(&offscreen, vformat->BitsPerPixel, &bounds, NULL,
...
static int DSp_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  if ((dsp_version.majorRev == 1 && dsp_version.minorAndBugRev < 0x73) ||
    ...
    /* StandardAlert (kAlertStopAlert, "\pError!",
                 "\pI need DrawSprocket 1.7.3 or later!\n"
                   "You can find a newer version at
       http://www.apple.com/swupdates.", NULL, NULL);
     */
  ...
  SDL_modelist = DSp_BuildModeList(SDL_Display, &this->info.current_w,
                                   &this->info.current_h);
...
static volatile unsigned int retrace_count =
    0; /* -dw- need volatile because it updates asychronously */
...
rebuild:
  ...
  if ((err = DSpContext_SetState(dsp_context, kDSpContextState_Active)) !=
      noErr) {
  ...
  if (!double_buf) {
    ...
    current->flags |=
        SDL_DOUBLEBUF | SDL_SWSURFACE; /* only front buffer is in VRAM */
    this->UpdateRects = DSp_DSpUpdate;
  } else if (DSp_NewHWSurface(this, &dsp_back_buffer, bpp, width - 1,
                              height - 1) == 0) {
    ...
    this->UpdateRects = DSp_DirectUpdate; /* hardware doesn't do update rects,
                                             must be page-flipped */
  } else {
  ...
  if (flags & SDL_OPENGL) {
    ...
    SDL_Window = NewCWindow(nil, &((**SDL_Display).gdRect), "\p", SDL_TRUE,
...
static int DSp_MakeHWMask(_THIS, SDL_Surface *surface) {
  ...
  if (noErr != NewGWorld(&(surface->hwdata->mask), depth, &rect, 0, SDL_Display,
                         0) < 0) {
  ...
  CopyBits(GetPortBitMapForCopyBits(surface->hwdata->offscreen),
           GetPortBitMapForCopyBits(surface->hwdata->mask), &rect, &rect,
           transparent, NULL);
...
static int DSp_NewHWSurface(_THIS, CGrafPtr *port, int depth, int width,
  ...
  if (dsp_vram_available) {
    /* try VRAM */
    err = NewGWorld(port, depth, &bounds, 0, SDL_Display,
...
static int DSp_AllocHWSurface(_THIS, SDL_Surface *surface) {
  ...
  if (DSp_NewHWSurface(this, &temp, surface->format->BitsPerPixel, surface->w,
                       surface->h) < 0)
    return (-1);
...
static int DSp_HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect,
  ...
  if ((src->flags & SDL_SRCCOLORKEY) && (src->flags & SDL_SRCALPHA)) {
    ...
    CopyDeepMask(GetPortBitMapForCopyBits(src->hwdata->offscreen),
                 ...
                 GetPortBitMapForCopyBits(dst->hwdata->offscreen), &src_rect,
                 &src_rect, &dst_rect, blend, NULL);
  } else {
  ...
  CopyBits(&(((GrafPtr)(src->hwdata->offscreen))->portBits),
           &(((GrafPtr)(dst->hwdata->offscreen))->portBits), &src_rect,
           &dst_rect, srcCopy, NULL);
...
static int DSp_FlipHWSurface(_THIS, SDL_Surface *surface) {
  if ((surface->flags & SDL_HWSURFACE)) {
    ...
    CopyBits(GetPortBitMapForCopyBits(dsp_back_buffer),
             GetPortBitMapForCopyBits(dsp_front_buffer), &rect, &rect, srcCopy,
             NULL);

    SetPort((GrafPtr)save_port);

  } else {
    /* not really page flipping at all: DSp just blits the dirty rectangles from
     * DSp_UpdateRects */
    Boolean busy_flag;
    DSpContext_SwapBuffers(dsp_context, NULL,
                           &busy_flag); /* this  waits for VBL */
    DSpContext_GetBackBuffer(dsp_context, kDSpBufferKind_Normal,
                             &dsp_back_buffer);
