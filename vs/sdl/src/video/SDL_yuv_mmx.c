...
void ColorRGBDitherYV12MMX1X(int *colortab, Uint32 *rgb_2_pix,
                             ...
                             unsigned char *cb, unsigned char *out, int rows,
                             int cols, int mod) {
  Uint32 *row1;
  Uint32 *row2;
  ...
  __asm__ __volatile__(
      ...
      "emms\n"          // reset MMX registers.
      :
      : "m"(cr), "r"(cb), "r"(lum), "r"(row1), "r"(cols), "r"(row2), "m"(x),
        "m"(y), "m"(mod), "m"(MMX_0080w), "m"(MMX_VgrnRGB), "m"(MMX_VredRGB),
        "m"(MMX_FF00w), "m"(MMX_00FFw), "m"(MMX_UgrnRGB), "m"(MMX_UbluRGB));
}
...
void Color565DitherYV12MMX1X(int *colortab, Uint32 *rgb_2_pix,
  ...
  __asm__ __volatile__(
      ...
      "movq           %%mm6,                  %%mm4\n" // lum1
      "paddw          %%mm0,                  %%mm6\n" // lum1 +blue 00 B6 00 B4
                                                       // 00 B2 00 B0
      "movq           %%mm4,                  %%mm5\n" // lum1
      "paddw          %%mm1,                  %%mm4\n" // lum1 +red  00 R6 00 R4
                                                       // 00 R2 00 R0
      "paddw          %%mm2,                  %%mm5\n" // lum1 +green 00 G6 00
                                                       // G4 00 G2 00 G0
      "psraw          $6,                     %%mm4\n" // R1 0 .. 64
      "movq           %%mm7,                  %%mm3\n" // lum2 00 L7 00 L5 00 L3
                                                       // 00 L1
      "psraw          $6,                     %%mm5\n" // G1  - .. +
      "paddw          %%mm0,                  %%mm7\n" // Lum2 +blue 00 B7 00 B5
                                                       // 00 B3 00 B1
      "psraw          $6,                     %%mm6\n" // B1         0 .. 64
      "packuswb       %%mm4,                  %%mm4\n" // R1 R1
      "packuswb       %%mm5,                  %%mm5\n" // G1 G1
      ...
      :
      : "m"(cr), "r"(cb), "r"(lum), "r"(row1), "r"(cols), "r"(row2), "m"(x),
        "m"(y), "m"(mod), "m"(MMX_0080w), "m"(MMX_Ugrn565), "m"(MMX_Ublu5x5),
        "m"(MMX_00FFw), "m"(MMX_Vgrn565), "m"(MMX_Vred5x5), "m"(MMX_Ycoeff),
        "m"(MMX_red565), "m"(MMX_grn565));
}
