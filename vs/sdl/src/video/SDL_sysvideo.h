...
struct SDL_VideoDevice {
   ...
   */
  SDL_Surface *(*SetVideoMode)(_THIS, SDL_Surface *current, int width,
  ...
  /* Create a YUV video surface (possibly overlay) of the given
     ...
     scaling on display.
   */
  SDL_Overlay *(*CreateYUVOverlay)(_THIS, int width, int height, Uint32 format,
                                   SDL_Surface *display);
  ...
  */
  ...
  /* The pixel format used when SDL_CreateRGBSurface creates SDL_HWSURFACEs with
   * alpha */
...
#undef SDL_PROC
  ...
  */
  ...
  WMcursor *(*CreateWMCursor)(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                              int hot_x, int hot_y);
