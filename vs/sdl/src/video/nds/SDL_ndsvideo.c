...
static SDL_Surface *NDS_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
static int NDS_Available(void) { return (1); }
...
static int HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                       SDL_Rect *dstrect) {
  return 0;
}
...
VideoBootStrap NDS_bootstrap = {NDSVID_DRIVER_NAME, "SDL NDS video driver",
...
int NDS_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  videoSetModeSub(MODE_0_2D |
                  DISPLAY_BG0_ACTIVE); // sub bg 0 will be used to print text

  // set the first two banks as background memory and the third as sub
  // background memory D is not used..if you need a bigger background then you
  // will need to map more vram banks consecutivly (VRAM A-D are all 0x20000
  // bytes in size) vramSetMainBanks(VRAM_A_MAIN_BG_0x6000000,
  // VRAM_B_MAIN_BG_0x6020000,VRAM_C_SUB_BG , VRAM_D_LCD);
  vramSetMainBanks(VRAM_A_MAIN_BG, VRAM_B_MAIN_BG, VRAM_C_MAIN_BG,
                   VRAM_D_MAIN_BG);
  ...
  BG_PALETTE_SUB[255] =
      RGB15(31, 31, 31); // by default font will be rendered with color 255
  ...
  // this basicaly gives it a 1:1 translation in x and y so you get a nice flat
  // bitmap
  /*  BG3_XDX = 1<<8;
    BG3_XDY = 0;
    BG3_YDX = 0;
...
//our bitmap looks a bit better if we center it so scroll down (256 - 192) / 2
  ...
  consoleInitDefault((u16 *)SCREEN_BASE_BLOCK_SUB(8),
...
SDL_Surface *NDS_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  ...
  SDL_memset(
      ...
      1024 * 512 *
          ((this->hidden->ndsmode == 4 || this->hidden->ndsmode == 5) ? 2 : 1) *
          ((bpp + 7) / 8));
  ...
  current->flags = flags | SDL_FULLSCREEN | SDL_HWSURFACE |
                   (this->hidden->ndsmode > 0 ? SDL_DOUBLEBUF : 0);
...
static void NDS_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static int NDS_FlipHWSurface(_THIS, SDL_Surface *surface) {
  ...
  // NDS_DISP_MODE(this->hidden->ndsmode)|NDS_DISP_BG2| buf);
  return (0);
}
