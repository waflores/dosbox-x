...
WMcursor *X11_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                             int hot_x, int hot_y) {
  WMcursor *cursor;
  XGCValues GCvalues;
  GC GCcursor;
  ...
  data_image = XCreateImage(SDL_Display, DefaultVisual(SDL_Display, SDL_Screen),
  ...
  mask_image = XCreateImage(SDL_Display, DefaultVisual(SDL_Display, SDL_Screen),
  ...
  XPutImage(SDL_Display, data_pixmap, GCcursor, data_image, 0, 0, 0, 0, w, h);
  XPutImage(SDL_Display, mask_pixmap, GCcursor, mask_image, 0, 0, 0, 0, w, h);
  ...
  /* Create the cursor */
  cursor->x_cursor = XCreatePixmapCursor(SDL_Display, data_pixmap, mask_pixmap,
  ...
  if (SDL_Window) {
    ...
    if (cursor == NULL) {
      if (SDL_BlankCursor != NULL) {
        XDefineCursor(SDL_Display, SDL_Window, SDL_BlankCursor->x_cursor);
      }
    } else {
...
static void SetMouseAccel(_THIS, const char *accel_param) {
  ...
  if (i == 3) {
    XChangePointerControl(SDL_Display, True, True, accel_value[0],
                          accel_value[1], accel_value[2]);
  }
...
void X11_CheckMouseModeNoLock(_THIS) {
  ...
  */
  ...
  if (enable_relative && !(SDL_cursorstate & CURSOR_VISIBLE) &&
    ...
    if (!mouse_relative) {
      ...
      if (!(using_dga & DGA_MOUSE)) {
        ...
        XGetPointerControl(SDL_Display, &mouse_accel.numerator,
                           &mouse_accel.denominator, &mouse_accel.threshold);
    ...
    if (mouse_relative) {
      if (using_dga & DGA_MOUSE) {
        ...
        XChangePointerControl(SDL_Display, True, True, mouse_accel.numerator,
                              mouse_accel.denominator, mouse_accel.threshold);
      }
      mouse_relative = 0;
    }
