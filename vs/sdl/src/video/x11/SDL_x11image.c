...
static void try_mitshm(_THIS, SDL_Surface *screen) {
  ...
  if (!use_mitshm)
    return;
  shminfo.shmid =
      shmget(IPC_PRIVATE, screen->h * screen->pitch, IPC_CREAT | 0777);
  if (shminfo.shmid >= 0) {
...
int X11_SetupImage(_THIS, SDL_Surface *screen) {
  ...
  if (use_mitshm) {
    SDL_Ximage =
        XShmCreateImage(SDL_Display, SDL_Visual, this->hidden->depth, ZPixmap,
                        shminfo.shmaddr, &shminfo, screen->w, screen->h);
    if (!SDL_Ximage) {
...
#endif /* not NO_SHARED_MEMORY */
  {
    ...
    SDL_Ximage =
        XCreateImage(SDL_Display, SDL_Visual, this->hidden->depth, ZPixmap, 0,
                     (char *)screen->pixels, screen->w, screen->h, 32, 0);
    if (SDL_Ximage == NULL)
      goto error;
    /* XPutImage will convert byte sex automatically */
    SDL_Ximage->byte_order =
        (SDL_BYTEORDER == SDL_BIG_ENDIAN) ? MSBFirst : LSBFirst;
...
int X11_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
void X11_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
...
void X11_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }

int X11_FlipHWSurface(_THIS, SDL_Surface *surface) { return (0); }
...
static void X11_NormalUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; ++i) {
    ...
    XPutImage(GFX_Display, SDL_Window, SDL_GC, SDL_Ximage, rects[i].x,
              rects[i].y, rects[i].x, rects[i].y, rects[i].w, rects[i].h);
  }
...
static void X11_MITSHMUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; ++i) {
    ...
    XShmPutImage(GFX_Display, SDL_Window, SDL_GC, SDL_Ximage, rects[i].x,
                 rects[i].y, rects[i].x, rects[i].y, rects[i].w, rects[i].h,
                 False);
  }
  if (SDL_VideoSurface->flags & SDL_ASYNCBLIT) {
...
void X11_DisableAutoRefresh(_THIS) { --enable_autorefresh; }
...
void X11_RefreshDisplay(_THIS) {
  ...
  /* DOSBox-X: SDL_SetVideoMode() can specify a flag not to auto-redraw (to
   * avoid flickering) */
  ...
  if (this->UpdateRects == X11_MITSHMUpdate) {
    XShmPutImage(SDL_Display, SDL_Window, SDL_GC, SDL_Ximage, 0, 0, 0, 0,
                 this->screen->w, this->screen->h, False);
  } else
#endif /* ! NO_SHARED_MEMORY */
  {
    XPutImage(SDL_Display, SDL_Window, SDL_GC, SDL_Ximage, 0, 0, 0, 0,
