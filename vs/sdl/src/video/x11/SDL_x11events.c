...
Uint32 Utf8ToUcs4(const Uint8 *utf8) {
  ...
  if (firstOctet < 0x80) {
    /*
      ...
      The least 6 significant bits of the first octet is the most 6 significant
      nonzero bits of the UCS4 representation.
    */
    noOctets = 1;
    firstOctetMask =
        0x7F; /* 0(1111111) - The most significant bit is ignored */
  } else if ((firstOctet & 0xE0) /* get the most 3 significant bits by AND'ing
                                    with 11100000 */
             == 0xC0) { /* see if those 3 bits are 110. If so, the char is in
                           this range */
    /*
      Characters in the range:
      ...
      are stored in two octets:
        110xxxxx 10xxxxxx
      The least 5 significant bits of the first octet is the most 5 significant
      nonzero bits of the UCS4 representation.
    */
    noOctets = 2;
    firstOctetMask =
        0x1F; /* 000(11111) - The most 3 significant bits are ignored */
  } else if ((firstOctet & 0xF0) /* get the most 4 significant bits by AND'ing
                                    with 11110000 */
             == 0xE0) { /* see if those 4 bits are 1110. If so, the char is in
                           this range */
    /*
      Characters in the range:
      ...
      The least 4 significant bits of the first octet is the most 4 significant
      nonzero bits of the UCS4 representation.
    */
    noOctets = 3;
    firstOctetMask =
        0x0F; /* 0000(1111) - The most 4 significant bits are ignored */
  } else if ((firstOctet & 0xF8) /* get the most 5 significant bits by AND'ing
                                    with 11111000 */
             == 0xF0) { /* see if those 5 bits are 11110. If so, the char is in
                           this range */
    /*
      Characters in the range:
      ...
      The least 3 significant bits of the first octet is the most 3 significant
      nonzero bits of the UCS4 representation.
    */
    noOctets = 4;
    firstOctetMask =
        0x07; /* 11110(111) - The most 5 significant bits are ignored */
  } else if ((firstOctet & 0xFC) /* get the most 6 significant bits by AND'ing
                                    with 11111100 */
             == 0xF8) { /* see if those 6 bits are 111110. If so, the char is in
                           this range */
    /*
      Characters in the range:
      ...
      The least 2 significant bits of the first octet is the most 2 significant
      nonzero bits of the UCS4 representation.
    */
    noOctets = 5;
    firstOctetMask =
        0x03; /* 111110(11) - The most 6 significant bits are ignored */
  } else if ((firstOctet & 0xFE) /* get the most 7 significant bits by AND'ing
                                    with 11111110 */
             == 0xFC) { /* see if those 7 bits are 1111110. If so, the char is
                           in this range */
    /*
      Characters in the range:
      ...
      The least significant bit of the first octet is the most significant
      nonzero bit of the UCS4 representation.
    */
    noOctets = 6;
    firstOctetMask =
        0x01; /* 1111110(1) - The most 7 significant bits are ignored */
  } else
    return 0; /* The given chunk is not a valid UTF-8 encoded Unicode character
               */

  /*
    The least noOctets significant bits of the first octet is the most 2
    significant nonzero bits of the UCS4 representation. The first 6 bits of the
    UCS4 representation is the least 8-noOctets-1 significant bits of firstOctet
    if the character is not ASCII. If so, it's the least 7 significant bits of
    firstOctet. This done by AND'ing firstOctet with its mask to trim the bits
    used for identifying the number of continuing octets (if any) and leave only
    the free bits (the x's) Sample: 1-octet:    0xxxxxxx  &  01111111 = 0xxxxxxx
    2-octets:  110xxxxx  &  00011111 = 000xxxxx
  */
  c = firstOctet & firstOctetMask;

  /* Now, start filling c.ucs4 with the bits from the continuing octets from
   * utf8. */
  for (i = 1; i < noOctets; i++) {
    /* A valid continuing octet is of the form 10xxxxxx */
    if ((utf8[i] &
         0xC0)   /* get the most 2 significant bits by AND'ing with 11000000 */
        != 0x80) /* see if those 2 bits are 10. If not, the is a malformed
                    sequence. */
    ...
    /*
      Take only the least 6 significance bits of the current octet (utf8[i]) and
      fill the created room of c.ucs4 with them. This done by AND'ing utf8[i]
      with 00111111 and the OR'ing the result with c.ucs4.
    */
    c |= utf8[i] & 0x3F;
  }
  return c;
...
static int Utf8ToUtf16(const Uint8 *utf8, const int utf8_length, Uint16 *utf16,
                       const int utf16_max_length) {

  /* p moves over the output buffer.  max_ptr points to the next to the last
   * slot of the buffer.  */
  Uint16 *p = utf16;
  Uint16 const *const max_ptr = utf16 + utf16_max_length;

  /* end_of_input points to the last byte of input as opposed to the next to the
   * last byte.  */
  ...
  while (utf8 <= end_of_input) {
    ...
    if (c < 0x80) {
      ...
      *p++ = (Uint16)(0xCF80 + (c << 6) + utf8[1]);
      ...
      if (end_of_input - utf8 < 2 ||
          (((utf8[1] ^ 0x80) | (utf8[2] ^ 0x80)) & 0xC0)) {
        return -1;
      }
      *p++ = (Uint16)(0xDF80 + (c << 12) + (utf8[1] << 6) + utf8[2]);
      ...
      if (end_of_input - utf8 < 3 ||
          (((utf8[1] ^ 0x80) | (utf8[2] ^ 0x80) | (utf8[3] ^ 0x80)) & 0xC0)) {
        return -1;
      }
      ...
      if (plane == 0) {
        ...
        *p++ = (Uint16)(0xDF80 + (utf8[1] << 12) + (utf8[2] << 6) + utf8[3]);
        ...
        /* This is a legal four byte sequence that corresponds to a surrogate
         * pair.  */
        if (p + 1 >= max_ptr) {
...
static __inline__ int X11_WarpedMotion(_THIS, XEvent *xevent) {
  ...
  if ((xevent->xmotion.x < MOUSE_FUDGE_FACTOR) ||
    ...
    mouse_last.x = w / 2;
    mouse_last.y = h / 2;
    XWarpPointer(SDL_Display, None, SDL_Window, 0, 0, 0, 0, mouse_last.x,
                 mouse_last.y);
    for (i = 0; i < 10; ++i) {
...
static int X11_DispatchEvent(_THIS) {
  ...
  case EnterNotify: {
    ...
    posted =
        SDL_PrivateMouseMotion(0, 0, xevent.xcrossing.x, xevent.xcrossing.y);
  } break;
  ...
  case LeaveNotify: {
    ...
    if ((xevent.xcrossing.mode != NotifyGrab) &&
      ...
      if (this->input_grab == SDL_GRAB_OFF) {
        ...
        posted = SDL_PrivateMouseMotion(0, 0, xevent.xcrossing.x,
                                        xevent.xcrossing.y);
      }
    }
  } break;
  ...
  /* Losing input focus? */
  case FocusOut: {
    /* NTS: X11_GrabInput() will cause a FocusOut event because SDL_Window is
     * grabbing keyboard input and WMwindow will report focus loss */
    if ((!currently_fullscreen && xevent.xany.window == SDL_Window) ||
        (!currently_fullscreen && xevent.xany.window == WMwindow &&
         X11_GrabbedInput == SDL_GRAB_OFF) ||
        (currently_fullscreen && xevent.xany.window == FSwindow)) {
#ifdef DEBUG_XEVENTS
      printf("FocusOut!\n");
#endif
...
#endif
          posted = SDL_PrivateMouseMotion(0, 1, xevent.xmotion.x_root,
                                          xevent.xmotion.y_root);
        } else {
...
#endif
        posted =
            SDL_PrivateMouseMotion(0, 0, xevent.xmotion.x, xevent.xmotion.y);
      }
    }
  } break;
  ...
  /* Mouse button press? */
  case ButtonPress: {
    posted = SDL_PrivateMouseButton(SDL_PRESSED, xevent.xbutton.button, 0, 0);
  } break;

  /* Mouse button release? */
  case ButtonRelease: {
    posted = SDL_PrivateMouseButton(SDL_RELEASED, xevent.xbutton.button, 0, 0);
  } break;
  ...
  case KeyPress: {
    ...
    if (SDL_IC != NULL) {
      ...
      count = Xutf8LookupString(SDL_IC, &xevent.xkey, keydata, sizeof(keybuf),
                                &xkeysym, &status);
      if (XBufferOverflow == status) {
        ...
        count = Xutf8LookupString(SDL_IC, &xevent.xkey, keydata, count,
                                  &xkeysym, &status);
      }
      ...
      case XLookupBoth: {
        ...
        utf16length =
            Utf8ToUtf16((Uint8 *)keydata, count, utf16data, utf16size);
        if (utf16length < 0) {
          /* The keydata contained an invalid byte
             sequence.  It should be a bug of the IM
             or Xlib... */
          SDL_SetError(
              "Oops! Xutf8LookupString returned an invalid UTF-8 sequence!");
          break;
        }
        ...
        if (utf16length > 0) {
          keysym.scancode = keycode;
          keysym.sym =
              (keycode ? X11_TranslateKeycode(SDL_Display, keycode) : 0);
...
#endif
#if 0 /* combined with DOSBox's insistence to SDL_SetVideoMode per resize this \
         is killing the ability to resize the window after one event --J.C */
    ...
    if (SDL_VideoSurface) {
      if ((xevent.xconfigure.width != SDL_VideoSurface->w) ||
        ...
        if (!((xevent.xconfigure.width == 32) &&
          ...
          SDL_PrivateResize(xevent.xconfigure.width, xevent.xconfigure.height);
        }
      } else {
...
void X11_SetKeyboardState(Display *display, const char *key_vec) {
  ...
  if (XQueryPointer(display, DefaultRootWindow(display), &junk_window,
                    &junk_window, &x, &y, &x, &y, &mask)) {
    if (mask & LockMask) {
      modstate |= KMOD_CAPS;
    }
...
void X11_InitOSKeymap(_THIS) { X11_InitKeymap(); }
...
void xim_lookup_key(_THIS, XKeyPressedEvent *event) {
  ...
  if (IM_Context.SDL_XIC) {
    if (!IM_Context.string.im_wide_char_buffer) {
      ...
      IM_Context.string.im_wide_char_buffer =
          (wchar_t *)malloc(IM_Context.im_buffer_len * sizeof(wchar_t));
    }

    // memset(IM_Context.string.im_wide_char_buffer, 0,
    // IM_Context.im_buffer_len*sizeof(wchar_t));
    IM_Context.string.im_wide_char_buffer[0] = '\0';

    // if (SDL_TranslateUNICODE)
    IM_Context.im_compose_len = XwcLookupString(
        IM_Context.SDL_XIC, event, IM_Context.string.im_wide_char_buffer,
        IM_Context.im_buffer_len, &keysym, &status);
    // else
    //	IM_Context.im_compose_len = XmbLookupString(IM_Context.SDL_XIC, event,
    //IM_Context.string.im_multi_byte_buffer,
    //IM_Context.im_buffer_len*sizeof(wchar_t), &keysym, &status);

    if ((status == XBufferOverflow)) {
      IM_Context.im_buffer_len = IM_Context.im_compose_len + 1;
      IM_Context.string.im_wide_char_buffer =
          (wchar_t *)realloc(IM_Context.string.im_wide_char_buffer,
                             IM_Context.im_buffer_len * sizeof(wchar_t));
      // memset(IM_Context.string.im_wide_char_buffer, 0,
      // IM_Context.im_buffer_len*sizeof(wchar_t));
      ...
      IM_Context.im_compose_len = XwcLookupString(
          ...
          IM_Context.im_buffer_len, &keysym, &status);
      // else
      //	IM_Context.im_compose_len = XmbLookupString(IM_Context.SDL_XIC,
      //event, IM_Context.string.im_multi_byte_buffer,
      //IM_Context.im_buffer_len*sizeof(wchar_t), &keysym, &status);
    }
    if (status != XLookupChars) {
      ...
      // else
      //	IM_Context.string.im_multi_byte_buffer[IM_Context.im_compose_len]
      //= '\0';
    }
  ...
  if (IM_Context.SDL_XIC) {
    ...
    case SDL_IM_FLIP:
      ...
      if (IM_Context.preedit_attr_orig == NULL) {
        XGetICValues(IM_Context.SDL_XIC, XNPreeditAttributes,
      ...
      if (IM_Context.status_attr_orig == NULL) {
        XGetICValues(IM_Context.SDL_XIC, XNStatusAttributes,
      ...
      if (alt) {
        ...
        XGetICValues(IM_Context.SDL_XIC, XNInputStyle, &IM_Context.im_style_now,
                     NULL);
        ...
        if (IM_Context.im_style_now &
          ...
          if (IM_Context.im_style_orig &
            ...
            if (IM_Context.bEnable_OverTheSpot) {
              IM_Context.im_style_now = (XIMPreeditPosition | XIMStatusNothing);
            } else if (IM_Context.bEnable_Root) {
              IM_Context.im_style_now = (XIMPreeditNothing | XIMStatusNothing);
            } else {
              SDL_SetError(
          ...
          if (IM_Context.SDL_XIC) {
            ...
            IM_Context.SDL_XIC = XCreateIC(
                IM_Context.SDL_XIM, XNInputStyle, IM_Context.im_style_now,
                XNClientWindow, WMwindow, XNFocusWindow, WMwindow,
            ...
            if (IM_Context.SDL_XIC == NULL) {
              SDL_SetError("Cannot create original style mode XIC.");
              return "SDL_IM_FLIP";
            }
          } else {
    ...
    case SDL_IM_ONOFF:
      if (alt) {
        ...
        if (XSetICValues(IM_Context.SDL_XIC, XNPreeditAttributes, list, NULL) !=
            NULL) {
          SDL_SetError(
              "XSetICValues() failed. (XNPreeditState not supported.)");
        }
        XFree(list);
      } else {
        XVaNestedList list;
...
char *X11_GetIMValues(_THIS, SDL_imvalue value, int *alt) {
  ...
  if (IM_Context.SDL_XIC) {
    ...
    case SDL_IM_FLIP:
      rec = XGetICValues(IM_Context.SDL_XIC, XNInputStyle,
    ...
    case SDL_IM_ONOFF: {
      ...
      if (XGetICValues(IM_Context.SDL_XIC, XNPreeditState, &state, NULL) !=
          NULL) {
...
int X11_FlushIMString(_THIS, void *buffer) {
  ...
  if (buffer && IM_Context.im_compose_len) {
    if (SDL_TranslateUNICODE) {
      ...
      memcpy(buffer, IM_Context.string.im_multi_byte_buffer,
             IM_Context.im_compose_len);
...
int X11_SetIMPosition(_THIS, int x, int y) { return 0; }
char *X11_SetIMValues(_THIS, SDL_imvalue value, int alt) { return NULL; }
char *X11_GetIMValues(_THIS, SDL_imvalue value, int *alt) { return NULL; }
int X11_FlushIMString(_THIS, void *buffer) { return 0; }
#endif
