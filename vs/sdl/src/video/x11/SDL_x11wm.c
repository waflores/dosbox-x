...
void X11_SetIcon(_THIS, SDL_Surface *icon, Uint8 *mask) {
  ...
  sicon = SDL_CreateRGBSurface(SDL_SWSURFACE, icon->w, icon->h, dbpp,
  ...
  if (dbpp == 8) {
    ...
    if (icon->format->palette) {
      ...
      SDL_memcpy(want, icon->format->palette->colors, nwant * sizeof want[0]);
    } else {
  ...
  mask_pixmap = XCreatePixmapFromBitmapData(
      SDL_Display, WMwindow, (char *)LSBmask, sicon->w, sicon->h, 1L, 0L, 1);

  /* Transfer the image to an X11 pixmap */
  icon_image = XCreateImage(SDL_Display, DefaultVisual(SDL_Display, SDL_Screen),
                            DefaultDepth(SDL_Display, SDL_Screen), ZPixmap, 0,
                            sicon->pixels, sicon->w, sicon->h, 32, 0);
  icon_image->byte_order =
  ...
  XPutImage(SDL_Display, icon_pixmap, gc, icon_image, 0, 0, 0, 0, sicon->w,
            sicon->h);
  ...
  if (p && *p) {
    icon_window =
        XCreateSimpleWindow(SDL_Display, SDL_Root, 0, 0, sicon->w, sicon->h, 0,
...
#ifdef X_HAVE_UTF8_STRING
    if (SDL_X11_HAVE_UTF8) {
      status = Xutf8TextListToTextProperty(SDL_Display, (char **)&title, 1,
...
#ifdef X_HAVE_UTF8_STRING
    if (SDL_X11_HAVE_UTF8) {
      status = Xutf8TextListToTextProperty(SDL_Display, (char **)&icon, 1,
...
SDL_GrabMode X11_GrabInputNoLock(_THIS, SDL_GrabMode mode) {
  ...
  if (mode == SDL_GRAB_OFF) {
    ...
    XGrabKeyboard(SDL_Display, SDL_Window, True, GrabModeAsync, GrabModeAsync,
                  CurrentTime);
...
*/
static void lock_display(void) { SDL_Lock_EventThread(); }
...
int X11_GetIMInfo(_THIS, SDL_SysIMinfo *info) {
  ...
  if (info->version.major <= SDL_MAJOR_VERSION) {
    if (SDL_VERSIONNUM(info->version.major, info->version.minor,
                       info->version.patch) >= SDL_VERSIONNUM(1, 2, 8)) {
