...
static struct private_yuvhwfuncs x11_yuvfuncs = {
    X11_LockYUVOverlay, X11_UnlockYUVOverlay, X11_DisplayYUVOverlay,
    X11_FreeYUVOverlay};
...
static void X11_ClearYUVOverlay(SDL_Overlay *overlay) {
  ...
  case SDL_IYUV_OVERLAY:
    for (y = 0; y < overlay->h; y++)
      memset(overlay->pixels[0] + y * overlay->pitches[0], 0, overlay->w);

    for (y = 0; y < (overlay->h / 2); y++) {
      memset(overlay->pixels[1] + y * overlay->pitches[1], -128,
             overlay->w / 2);
      memset(overlay->pixels[2] + y * overlay->pitches[2], -128,
             overlay->w / 2);
    }
    break;
  ...
  case SDL_YVYU_OVERLAY:
    for (y = 0; y < overlay->h; y++) {
      for (x = 0; x < overlay->w; x += 2) {
        Uint8 *pixel_pair =
            overlay->pixels[0] + y * overlay->pitches[0] + x * 2;
        pixel_pair[0] = 0;
        pixel_pair[1] = -128;
        pixel_pair[2] = 0;
        pixel_pair[3] = -128;
...
SDL_Overlay *X11_CreateYUVOverlay(_THIS, int width, int height, Uint32 format,
                                  SDL_Surface *display) {
  SDL_Overlay *overlay;
  ...
  if ((Success ==
...
#endif /* USE_LAST_ADAPTOR */
    {
      ...
      if (!strcmp(ainfo[i].name, "Intel(R) Video Overla"))
        ...
        intel_adapter = False;
#endif
      if ((ainfo[i].type & XvInputMask) && (ainfo[i].type & XvImageMask)) {
        int num_formats;
        SDL_NAME(XvImageFormatValues) * formats;
        formats = SDL_NAME(XvListImageFormats)(GFX_Display, ainfo[i].base_id,
                                               &num_formats);
#ifdef USE_LAST_ADAPTOR
...
#endif /* USE_LAST_ADAPTOR */
        {
          if ((Uint32)formats[j].id == format) {
            for (k = 0; k < ainfo[i].num_ports; ++k) {
              if (Success == SDL_NAME(XvGrabPort)(GFX_Display,
                                                  ainfo[i].base_id + k,
                                                  CurrentTime)) {
  ...
  {
    static const char *attr[] = {"XV_AUTOPAINT_COLORKEY",
  ...
  hwdata->image = SDL_NAME(XvShmCreateImage)(GFX_Display, xv_port, format, 0,
                                             width, height, yuvshm);
  ...
  if (hwdata->yuv_use_mitshm) {
    yuvshm->shmid =
        shmget(IPC_PRIVATE, hwdata->image->data_size, IPC_CREAT | 0777);
    if (yuvshm->shmid >= 0) {
  ...
  {
    hwdata->image =
        SDL_NAME(XvCreateImage)(GFX_Display, xv_port, format, 0, width, height);
    ...
    if (hwdata->image != NULL && hwdata->image->pitches[0] != (width * bpp)) {
      ...
      XFree(hwdata->image);
      width = hwdata->image->pitches[0] / bpp;
      hwdata->image = SDL_NAME(XvCreateImage)(GFX_Display, xv_port, format, 0,
                                              width, height);
    }
#endif /* PITCH_WORKAROUND */
  ...
  for (i = 0; i < overlay->planes; ++i) {
    ...
    overlay->pixels[i] =
        (Uint8 *)hwdata->image->data + hwdata->image->offsets[i];
  }
  ...
  if (intel_adapter) {
    ...
    if (hwdata->yuv_use_mitshm) {
      SDL_NAME(XvShmPutImage)(GFX_Display, hwdata->port, SDL_Window, SDL_GC,
                              hwdata->image, 0, 0, overlay->w, overlay->h, 0, 0,
                              1, 1, False);
    } else
#endif
    {
      SDL_NAME(XvPutImage)(GFX_Display, hwdata->port, SDL_Window, SDL_GC,
                           hwdata->image, 0, 0, overlay->w, overlay->h, 0, 0, 1,
                           1);
    }
...
int X11_LockYUVOverlay(_THIS, SDL_Overlay *overlay) { return (0); }

void X11_UnlockYUVOverlay(_THIS, SDL_Overlay *overlay) { return; }

int X11_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                          SDL_Rect *dst) {
  struct private_yuvhwdata *hwdata;
  ...
  if (hwdata->yuv_use_mitshm) {
    SDL_NAME(XvShmPutImage)(GFX_Display, hwdata->port, SDL_Window, SDL_GC,
                            hwdata->image, src->x, src->y, src->w, src->h,
                            dst->x, dst->y, dst->w, dst->h, False);
  } else
#endif
  {
    SDL_NAME(XvPutImage)(GFX_Display, hwdata->port, SDL_Window, SDL_GC,
                         hwdata->image, src->x, src->y, src->w, src->h, dst->x,
                         dst->y, dst->w, dst->h);
  }
  XSync(GFX_Display, False);
  return (0);
