...
static void restore_mode(_THIS) {
  ...
  if (SDL_NAME(XF86VidModeGetModeLine)(SDL_Display, SDL_Screen, &unused,
                                       &mode)) {
  ...
  if ((saved_view.x != 0) || (saved_view.y != 0)) {
    SDL_NAME(XF86VidModeSetViewPort)(SDL_Display, SDL_Screen, saved_view.x,
                                     saved_view.y);
  }
}
#endif
...
static int cmpmodes(const void *va, const void *vb) {
  const SDL_NAME(XF86VidModeModeInfo) *a =
      *(const SDL_NAME(XF86VidModeModeInfo) **)va;
  const SDL_NAME(XF86VidModeModeInfo) *b =
      *(const SDL_NAME(XF86VidModeModeInfo) **)vb;
...
static void set_best_resolution(_THIS, int width, int height) {
  ...
  if (use_vidmode) {
    ...
    if (SDL_NAME(XF86VidModeGetModeLine)(SDL_Display, SDL_Screen, &i, &mode) &&
        SDL_NAME(XF86VidModeGetAllModeLines)(SDL_Display, SDL_Screen, &nmodes,
                                             &modes)) {
      for (i = 0; i < nmodes; i++) {
      ...
      if (best >= 0 && ((modes[best]->hdisplay != mode.hdisplay) ||
                        (modes[best]->vdisplay != mode.vdisplay))) {
#ifdef X11MODES_DEBUG
        printf("Best Mode %d: %d x %d @ %d\n", best, modes[best]->hdisplay,
               modes[best]->vdisplay,
               (modes[best]->htotal && modes[best]->vtotal)
                   ? (1000 * modes[best]->dotclock /
                      (modes[best]->htotal * modes[best]->vtotal))
                   : 0);
#endif
  ...
  if (use_xme && SDL_modelist) {
    ...
    for (i = 0; SDL_modelist[i]; ++i) {
      if ((SDL_modelist[i]->w >= width) && (SDL_modelist[i]->h >= height)) {
        break;
      }
    }
  ...
  if (use_xrandr && SDL_modelist) {
    ...
    /* find the smallest resolution that is at least as big as the user
     * requested */
    ...
    if (i >= 0 && SDL_modelist[i]) { /* found one, lets try it */
      ...
      if ((SDL_modelist[i]->w != w) || (SDL_modelist[i]->h != h)) {
        ...
        XRRSetScreenConfig(SDL_Display, screen_config, SDL_Root, size_id,
...
static int add_default_visual(_THIS) {
  ...
  for (i = 0; i < n; i++) {
    if (this->hidden->visuals[i].visual ==
...
int X11_GetVideoModes(_THIS) {
  ...
  if (CheckXinerama(this, &xinerama_major, &xinerama_minor)) {
    ...
    for (i = 0; i < screens; i++) {
      ...
      printf("xinerama %d: %dx%d+%d+%d\n", xinerama[i].screen_number,
             xinerama[i].width, xinerama[i].height, xinerama[i].x_org,
             xinerama[i].y_org);
#endif
  ...
  if (CheckXRandR(this, &xrandr_major, &xrandr_minor) && (xrandr_major >= 1)) {
    ...
    */
    ...
    if (nsizes > 0) {
      ...
      for (i = 0; i < nsizes; i++) {
        if ((SDL_modelist[i] = (SDL_Rect *)malloc(sizeof(SDL_Rect))) == NULL)
          break;
#ifdef X11MODES_DEBUG
  ...
  if (!use_xrandr &&
    ...
    for (i = 0; i < nmodes; ++i) {
      ...
      if (i > 0 && modes[i]->hdisplay == modes[i - 1]->hdisplay &&
  ...
  if (CheckXME(this, &xme_major, &xme_minor) && xme_major >= 2 &&
      (nummodes = XiGMiscQueryResolutions(SDL_Display, SDL_Screen, 0, /* view */
                                          &ractive, &modelist)) >
    ...
    for (i = 0, j = nummodes - 1; j >= 0; i++, j--) {
      if ((SDL_modelist[i] = (SDL_Rect *)SDL_malloc(sizeof(SDL_Rect))) == NULL)
        break;
#ifdef X11MODES_DEBUG
...
#if SDL_VIDEO_DRIVER_X11_XINERAMA
  if (use_xinerama && window_w <= xinerama_info.width &&
...
int X11_EnterFullScreen(_THIS) {
  ...
  if (WMwindow) {
    ...
    XTranslateCoordinates(SDL_Display, WMwindow, SDL_Root, 0, 0, &x, &y,
                          &child);
...
#if SDL_VIDEO_DRIVER_X11_XRANDR
    if (use_xrandr) {
      XRRSetScreenConfig(SDL_Display, screen_config, SDL_Root, saved_size_id,
                         saved_rotation, CurrentTime);
    }
#endif
