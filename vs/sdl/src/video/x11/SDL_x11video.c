...
static SDL_Surface *X11_SetVideoMode(_THIS, SDL_Surface *current, int width,
                                     int height, int bpp, Uint32 flags);
static int X11_ToggleFullScreen(_THIS, int on);
static void X11_UpdateMouse(_THIS);
static int X11_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors);
...
static SDL_VideoDevice *X11_CreateDevice(int devindex) {
  ...
  if (SDL_X11_LoadSymbols()) {
    ...
    if (device) {
      ...
      device->gl_data =
          (struct SDL_PrivateGLData *)SDL_malloc((sizeof *device->gl_data));
    }
...
VideoBootStrap X11_bootstrap = {"x11", "X Window System", X11_Available,
                                X11_CreateDevice};
...
static int xext_errhandler(Display *d, _Xconst char *ext,
                           _Xconst char *reason) {
#ifdef X11_DEBUG
...
static void create_aux_windows(_THIS, const unsigned int force) {
  ...
  FSwindow = XCreateWindow(SDL_Display, SDL_Root, x, y, 32, 32, 0,
  ...
  /* Tell KDE to keep the fullscreen window on top */
  {
    ...
    ev.xclient.message_type =
        XInternAtom(SDL_Display, "KWM_KEEP_ON_TOP", False);
  ...
  WMwindow =
      XCreateWindow(SDL_Display, SDL_Root, x, y, 32, 32, 0, this->hidden->depth,
...

  {
    ...
    if (pid > 0 && gethostname(hostname, sizeof(hostname)) > -1) {
      ...
      Atom WM_CLIENT_MACHINE =
          XInternAtom(SDL_Display, "WM_CLIENT_MACHINE", False);
  ...
  if (SDL_X11_HAVE_UTF8) {
    ...
    if (SDL_IM == NULL) {
      ...
      SDL_IC = pXCreateIC(SDL_IM, XNClientWindow, WMwindow, XNFocusWindow,
                          WMwindow, XNInputStyle,
                          XIMPreeditNothing | XIMStatusNothing, XNResourceName,
                          classname, XNResourceClass, classname, NULL);
...
static int X11_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  for (i = 0; i < this->hidden->nvisuals; i++)
    if (this->hidden->visuals[i].depth == DefaultDepth(SDL_Display, SDL_Screen))
      break;
  ...
  if (SDL_Visual == DefaultVisual(SDL_Display, SDL_Screen)) {
    ...
    SDL_XColorMap =
        XCreateColormap(SDL_Display, SDL_Root, SDL_Visual, AllocNone);
  }
  ...
  if (this->hidden->depth == 32) {
    vformat->Amask =
        (0xFFFFFFFF & ~(vformat->Rmask | vformat->Gmask | vformat->Bmask));
  }
  ...
  SDL_BlankCursor =
      this->CreateWMCursor(this, blank_cdata, blank_cmask, BLANK_CWIDTH,
                           BLANK_CHEIGHT, BLANK_CHOTX, BLANK_CHOTY);
    ...
    if (SDL_XPixels) {
      ...
      for (pixel = 0; pixel < numcolors; ++pixel) {
        while (SDL_XPixels[pixel] > 0) {
          XFreeColors(GFX_Display, SDL_DisplayColormap, &pixel, 1, 0);
          --SDL_XPixels[pixel];
        }
      }
...
static void X11_SetSizeHints(_THIS, int w, int h, Uint32 flags) {
  ...
  if (flags & SDL_NOFRAME) {
    ...
    if (WM_HINTS != None) {
      ...
      XChangeProperty(SDL_Display, WMwindow, WM_HINTS, WM_HINTS, 32,
...
static int X11_CreateWindow(_THIS, SDL_Surface *screen, int w, int h, int bpp,
                            Uint32 flags) {
  int i, depth;
  Visual *vis;
  int vis_change;
  ...
  printf("Choosing %s visual at %d bpp - %d colormap entries\n",
         vis->class == PseudoColor
             ...
             : (vis->class == TrueColor
                    ? "TrueColor"
                    : (vis->class == DirectColor ? "DirectColor" : "Unknown")),
         depth, vis->map_entries);
#endif
  /* NTS: Comparing Visual pointers will ALWAYS show a visual change because the
   * pointers differ. If you look at the ACTUAL contents of the struct nothing
   * changes! */
  ...
  if (!SDL_ReallocFormat(screen, bpp, vis->red_mask, vis->green_mask,
                         vis->blue_mask, Amask)) {
    return -1;
  }
  ...
  if (!SDL_windowid) {
    if (flags & SDL_OPENGL) {
      ...
      SDL_Window = XCreateWindow(SDL_Display, WMwindow, 0, 0, w, h, 0, depth,
                                 InputOutput, SDL_Visual,
  ...
  /* Cache the window in the server, when possible */
  {
    ...
    if (a.backing_store != NotUseful) {
      XChangeWindowAttributes(SDL_Display, SDL_Window, CWBackingStore, &a);
    }
  }
  ...
  if (!SDL_windowid) {
    ...
    /* WARNING: If WMwindow is already mapped, X11_WaitMapped() will hang
     * indefinitely unless we check map state first. */

    /*          This code maps WMwindow once and then never unmaps it until
     * shutdown. */
    /*          XFCE's window manager likes to move unmapped windows to the
     * upper left hand corner. */
    /*          Rather than have to reposition the window back to where I had
     * it, it's better not to unmap it in X11_DestroyWindow */
    memset(&a, 0, sizeof(a));
...
static int X11_ResizeWindow(_THIS, SDL_Surface *screen, int w, int h,
                            Uint32 flags) {
  int x = -999, y = -999;
  ...
  if (!SDL_windowid) {
    ...
    if ((flags & SDL_HAX_NORESIZEWINDOW) && (flags & SDL_FULLSCREEN) == 0 &&
...
SDL_Surface *X11_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  Uint32 saved_flags;
...
static void X11_UpdateMouse(_THIS) {
  ...
  /* Lock the event thread, in multi-threading environments */
  SDL_Lock_EventThread();
  if (XQueryPointer(SDL_Display, SDL_Window, &u1, &current_win, &u2, &u2, &x,
                    &y, &mask)) {
    if ((x >= 0) && (x < SDL_VideoSurface->w) && (y >= 0) &&
        (y < SDL_VideoSurface->h)) {
...
static void allocate_nearest(_THIS, SDL_Color *colors, SDL_Color *want,
                             int nwant) {
  /*
...
/* Close the connection with the IM server */
    ...
    if (SDL_iconcolors) {
      unsigned long pixel;
      Colormap dcmap = DefaultColormap(SDL_Display, SDL_Screen);
...
void destroy_callback_func(XIM current_ic, XPointer client_data,
                           XPointer call_data) {
...
void im_callback(XIM xim, XPointer client_data, XPointer call_data) {
   ...
   */
  if (XGetIMValues(IM_Context.SDL_XIM, XNQueryInputStyle, &xim_styles, NULL) ||
      !xim_styles) {
#ifdef DEBUG_XEVENTS
  ...
  if (input_style != (XIMPreeditPosition | XIMStatusNothing)) {
    ...
    if (input_style != (XIMPreeditNothing | XIMStatusNothing)) {
      ...
      if (input_style != (XIMPreeditCallbacks | XIMStatusCallbacks)) {
        SDL_SetError(
            "The XIM doesn't support OverTheSpot, Root, and OnTheSpot mode.");
      }
    }
  }
  ...
  IM_Context.SDL_XIC = XCreateIC(
      IM_Context.SDL_XIM, XNInputStyle, input_style, XNClientWindow, WMwindow,
      XNFocusWindow, WMwindow,
      (input_style & (XIMPreeditPosition | XIMStatusNothing) && preedit_attr)
          ? XNPreeditAttributes
          : NULL,
      preedit_attr, NULL);
  ...
  if (IM_Context.SDL_XIC == NULL) {
    ...
    input_style = (XIMPreeditNothing | XIMStatusNothing);
    IM_Context.SDL_XIC =
        XCreateIC(IM_Context.SDL_XIM, XNInputStyle, input_style, XNClientWindow,
                  WMwindow, XNFocusWindow, WMwindow, NULL);
...
int create_fontset(void) {
  ...
  if (fontset_name == NULL || !isprint(*fontset_name)) {
    ...
    printf("Please set environment variable: SDLIM_FONTSET\nbash ex.\n\texport "
           "SDLIM_FONTSET=*-ISO8859-1,*-BIG5-0\n");
#endif
    // SDL_SetError("Please set environment variable: SDLIM_FONTSET\nbash
    // ex.\n\texport SDLIM_FONTSET=*-ISO8859-1,*-BIG5-0"); fontset_name =
    // "*-ISO8859-1";
   ...
   */
  IM_Context.fontset = XCreateFontSet(SDL_Display, fontset_name, &charset_list,
  ...
  if (charset_count || !IM_Context.fontset) {
    ...
    printf("Error: cannot create fontset. %d %d %s\n", charset_count,
           IM_Context.fontset, def_string);
#endif
    return 0;
  }
  if (fontset_count !=
      XFontsOfFontSet(IM_Context.fontset, &font_structs, &charset_list)) {
...
int xim_init(_THIS) {
  ...
  if (XRegisterIMInstantiateCallback(SDL_Display, NULL, NULL, NULL,
                                     (XIMProc)im_callback, NULL) != True) {
...
static void xim_free(_THIS) {
  ...
  if ((IM_Context.preedit_attr_now != IM_Context.preedit_attr_orig) &&
