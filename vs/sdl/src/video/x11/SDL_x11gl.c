...
XVisualInfo *X11_GL_GetVisual(_THIS) {
  ...
  if (SDL_windowid) {
    ...
    glx_visualinfo = XGetVisualInfo(
        SDL_Display, VisualScreenMask | VisualIDMask, &vi_in, &out_count);
  ...
  glx_visualinfo =
      this->gl_data->glXChooseVisual(GFX_Display, SDL_Screen, attribs);
#ifdef GLX_DIRECT_COLOR
  if (!glx_visualinfo &&
      !SDL_getenv("SDL_VIDEO_X11_NODIRECTCOLOR")) { /* No DirectColor visual?
                                                       Try again.. */
    attribs[i - 3] = None;
    glx_visualinfo =
        this->gl_data->glXChooseVisual(GFX_Display, SDL_Screen, attribs);
...
int X11_GL_CreateWindow(_THIS, int w, int h) {
  ...
  SDL_Window =
      XCreateWindow(SDL_Display, WMwindow, 0, 0, w, h, 0, glx_visualinfo->depth,
...
int X11_GL_CreateContext(_THIS) {
  ...
  glx_context =
      this->gl_data->glXCreateContext(GFX_Display, glx_visualinfo, NULL, True);
...
int X11_GL_GetAttribute(_THIS, SDL_GLattr attrib, int *value) {
  ...
  case SDL_GL_ACCELERATED_VISUAL:
    if (glXExtensionSupported(this, "GLX_EXT_visual_rating")) {
      ...
      retval = this->gl_data->glXGetConfig(GFX_Display, glx_visualinfo,
                                           glx_attrib, value);
...
int X11_GL_LoadLibrary(_THIS, const char *path) {
  ...
  this->gl_data->glXChooseVisual = (XVisualInfo * (*)(Display *, int, int *))
      X11_GL_GetProcAddress(this, "glXChooseVisual");
  this->gl_data->glXCreateContext =
      (GLXContext (*)(Display *, XVisualInfo *, GLXContext,
                      int))X11_GL_GetProcAddress(this, "glXCreateContext");
  this->gl_data->glXDestroyContext = (void (*)(
      Display *, GLXContext))X11_GL_GetProcAddress(this, "glXDestroyContext");
  this->gl_data->glXMakeCurrent =
      (int (*)(Display *, GLXDrawable, GLXContext))X11_GL_GetProcAddress(
          this, "glXMakeCurrent");
  this->gl_data->glXSwapBuffers =
      (void (*)(Display *, GLXDrawable))X11_GL_GetProcAddress(this,
                                                              "glXSwapBuffers");
  this->gl_data->glXGetConfig =
      (int (*)(Display *, XVisualInfo *, int, int *))X11_GL_GetProcAddress(
          this, "glXGetConfig");
  ...
  this->gl_data->glXSwapIntervalEXT =
      (int (*)(Display *, GLXDrawable, int))X11_GL_GetProcAddress(
