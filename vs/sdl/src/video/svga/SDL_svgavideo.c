...
static SDL_Surface *SVGA_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
VideoBootStrap SVGALIB_bootstrap = {"svgalib", "SVGAlib", SVGA_Available,
                                    SVGA_CreateDevice};
...
SDL_Surface *SVGA_SetVideoMode(_THIS, SDL_Surface *current, int width,
                               int height, int bpp, Uint32 flags) {
  int mode;
  int vgamode;
  vga_modeinfo *modeinfo;
...
static int SVGA_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void SVGA_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void SVGA_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void SVGA_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) { return; }
...
int SVGA_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  ...
  for (i = 0; i < ncolors; i++) {
    vga_setpalette(firstcolor + i, colors[i].r >> 2, colors[i].g >> 2,
                   colors[i].b >> 2);
  }
  return (1);
}
