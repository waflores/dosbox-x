...
void SVGA_mousecallback(int button, int dx, int dy, int u1, int u2, int u3,
                        int u4) {
  if (dx || dy) {
...
void SVGA_keyboardcallback(int scancode, int pressed) {
  ...
  if (pressed) {
    posted += SDL_PrivateKeyboard(SDL_PRESSED, TranslateKey(scancode, &keysym));
  } else {
    posted +=
        SDL_PrivateKeyboard(SDL_RELEASED, TranslateKey(scancode, &keysym));
  }
}
