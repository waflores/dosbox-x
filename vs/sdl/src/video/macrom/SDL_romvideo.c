...
#define ONEWORDSTUB(p1)                                                        \
  {0x41FA, 0x0010, 0x209F, (p1),   0x41FA, 0x0008,                             \
   0x2F10, 0x4E75, 0x0000, 0x0000, 0x0000}
...
static pascal Boolean SBIsControlStripVisible(void) {
  ...
  return (
      (Boolean)CallUniversalProc((UniversalProcPtr)procData, procInfo, 0x00));
}
...
static int ROM_Available(void) { return (1); }
...
VideoBootStrap TOOLBOX_bootstrap = {"toolbox", "MacOS ROM Toolbox",
...
static SDL_Surface *ROM_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (SDL_Window) {
    ...
    OffsetRect(&wrect, (SDL_modelist[0]->w - width) / 2,
  ...
  if ((flags & SDL_FULLSCREEN) == SDL_FULLSCREEN) {
...
#else
    SDL_Window =
        NewCWindow(nil, &wrect, "\p", true, plainDBox, (WindowPtr)-1, false, 0);
    ...
    if (SDL_Window && (style == current_style)) {
      ...
      SDL_Window =
          NewCWindow(nil, &wrect, "\p", true, style, (WindowPtr)-1, true, 0);
...
static int ROM_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void ROM_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
static int ROM_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }
static void ROM_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
