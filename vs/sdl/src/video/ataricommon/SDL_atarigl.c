...
int SDL_AtariGL_LoadLibrary(_THIS, const char *path) {
  ...
  if (this->gl_data->glGetIntegerv == NULL) {
    ...
    if ((this->gl_data->glFinish == NULL) && (this->gl_data->glFlush == NULL)) {
      cancel_load = SDL_TRUE;
    }
  }
  if (cancel_load) {
...
static int InitNew(_THIS, SDL_Surface *current) {
  ...
  newaccumsize =
      this->gl_config.accum_red_size + this->gl_config.accum_green_size +
      this->gl_config.accum_blue_size + this->gl_config.accum_alpha_size;
  recreatecontext = 1;
  if (gl_ctx && (gl_curformat == osmesa_format) &&
  ...
  if (recreatecontext) {
    ...
    gl_ctx = this->gl_data->OSMesaCreateContextExt(
        osmesa_format, this->gl_config.depth_size, this->gl_config.stencil_size,
        newaccumsize, NULL);
...
static int InitOld(_THIS, SDL_Surface *current) {
  ...
  /* Try to keep current context if possible */
  recreatecontext = 1;
  if (gl_shadow && (gl_curformat == osmesa_format) &&
  ...
  if (recreatecontext) {
    ...
    gl_shadow = this->gl_data->OSMesaCreateLDG(osmesa_format, GL_UNSIGNED_BYTE,
                                               current->w, current->h);
