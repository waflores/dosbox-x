...
typedef struct tagScreenModeBlock {
  int flags; // mode selector flags, bit 0 = 1, bit 1-7 format specifier, 8-31
             // reserved
  int x_pixels;
  int y_pixels;
...
extern int riscos_backbuffer; /* Create a back buffer in system memory for full
                                 screen mode */
...
/* Fast assembler copy */
extern void RISCOS_Put32(void *to, int pixels, int pitch, int rows, void *from,
                         int src_skip_bytes);

SDL_Surface *FULLSCREEN_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (flags & SDL_DOUBLEBUF) {
    ...
    create_back_buffer =
        0; /* Don't need a back buffer for a double buffered display */
  }
...
/* Reset any device functions that have been changed because we have run in WIMP
 * mode */
void FULLSCREEN_SetDeviceMode(_THIS) {
  ...
  if (riscos_backbuffer && (this->screen->flags & SDL_DOUBLEBUF) == 0) {
    ...
    this->UpdateRects =
        FULLSCREEN_UpdateRects; /* Default do nothing implementation */
...
void FULLSCREEN_BuildModeList(_THIS) {
  ...
  for (j = 0; j < NUM_MODELISTS; ++j) {
    if (SDL_nummodes[j] > 0) {
      SDL_qsort(SDL_modelist[j], SDL_nummodes[j], sizeof *SDL_modelist[j],
                cmpmodes);
    }
  }
}
...
static void FULLSCREEN_UpdateRectsOS(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (j = 0; j < numrects; j++) {
    ...
    _kernel_oswrch(((rects->x + rects->w - 1) << this->hidden->xeig) &
                   0xFF); /* right */
    _kernel_oswrch((((rects->x + rects->w - 1) << this->hidden->xeig) >> 8) &
                   0xFF);
    ...
    _kernel_oswrch(0);
    _kernel_oswrch(((this->screen->w - 1) << this->hidden->xeig) &
                   0xFF); /* right */
    _kernel_oswrch((((this->screen->w - 1) << this->hidden->xeig) >> 8) & 0xFF);
    _kernel_oswrch(((this->screen->h - 1) << this->hidden->yeig) &
                   0xFF); /* top */
    _kernel_oswrch((((this->screen->h - 1) << this->hidden->yeig) >> 8) & 0xFF);
  }
}

int FULLSCREEN_SetColors(_THIS, int firstcolor, int ncolors,
                         SDL_Color *colors) {
  _kernel_swi_regs regs;
  int palette[256];
...
int FULLSCREEN_SetMode(int width, int height, int bpp) {
  ...
  case 8:
    smb.pixel_depth = 3;
    /* Note: Need to set ModeFlags to 128 and NColour variables to 255 get full
     * 8 bit palette */
    smb.mode_vars[0] = 0;
