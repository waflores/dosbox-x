...
unsigned char *WIMP_CreateBuffer(int width, int height, int bpp) {
  ...
  *(unsigned int *)(buffer + 8) = 16;
  ...
  if (bpp == 8) {
    ...
    regs.r[6] =
        (((bpp == 16) ? 5 : 6)
         << 27)      /* Type 6 = 32bpp sprite, 5 = 16bpp sprite */
        | (90 << 14) /* Vertical dpi */
        | (90 << 1)  /* Horizontal dpi */
...
void WIMP_SetupPlotInfo(_THIS) {
  ...
  regs.r[5] =
      1 | 2 | 16; /* R1 - pointer to sprite and can use full palette words */
  regs.r[6] = 0;
  regs.r[7] = 0;
...
void WIMP_PlotSprite(_THIS, int x, int y) {
  ...
  regs.r[4] = y;
  regs.r[5] =
      0 | 32; /* Overwrite screen and pixtrans contains wide colour entries */
...
void WIMP_PaletteChanged(_THIS) { WIMP_SetupPlotInfo(this); }
