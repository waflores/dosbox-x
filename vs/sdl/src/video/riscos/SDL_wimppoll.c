  ...
  while (doPoll) {
    ...
    case 3: /* Close window */
      if (message[0] == sdlWindow) {
        ...
        **    if (SDL_PrivateQuit() == 1) _kernel_swi(Wimp_CloseWindow, &regs,
        *&regs);
        ** However some programs don't process this message and so sit there
        *invisibly
        ** in the background so I just post the quit message and hope the
        *application
