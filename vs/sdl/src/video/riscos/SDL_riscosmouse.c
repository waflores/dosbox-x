...
static Uint8 wimp_cursor_palette[2][5] = {{1, 25, 255, 255, 255},
                                          {3, 25, 255, 255, 255}};
...
WMcursor *RISCOS_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                                int hot_x, int hot_y) {
  WMcursor *cursor;
  Uint8 *cursor_data;
  Uint8 *ptr;
...
int RISCOS_ShowWMCursor(_THIS, WMcursor *cursor) {
  ...
  if (cursor == NULL) {
    ...
    if (cursor != defined_cursor) {
      ...
      cursor_def[6] = ((int)(cursor->data) &
                       0xFF); /* Least significant byte of pointer to data */
      cursor_def[7] = ((int)(cursor->data) >> 8) & 0xFF;  /* ... */
      cursor_def[8] = ((int)(cursor->data) >> 16) & 0xFF; /* ... */
      cursor_def[9] = ((int)(cursor->data) >> 24) &
                      0xFF; /* Most significant byte of pointer to data */
