...
int riscos_backbuffer =
    0; /* Create a back buffer in system memory for full screen mode */
int riscos_closeaction = 1; /* Close icon action */

static int stored_mode =
    -1; /* -1 when in desktop, mode number or pointer when full screen */
...
/**************************************************************************
  ...
  Notes:   Works by getting using OS_GetEnv to get the command line
                ...
                SDL$<name>$BackBuffer - set to 1 to use a system memory
backbuffer in fullscreen mode so updates wait until a call to SDL_UpdateRects.
(default 0) This is required for programmes where they have assumed this is
                                                    always the case which is
contrary to the documentation. SDL$<name>$CloseAction 0 Don't show close icon 1
Show close icon
...
int RISCOS_GetTaskName(char *task_name, size_t maxlen) {
  ...
  if (_kernel_swi(OS_GetEnv, &regs, &regs) == 0) {
    ...
    if (*p) {
      ...
      len =
          SDL_strlen(p) + 18; /* 18 is larger than the biggest variable name */
...
int RISCOS_GetWimpVersion() { return wimp_version; }

int RISCOS_GetTaskHandle() { return task_handle; }
