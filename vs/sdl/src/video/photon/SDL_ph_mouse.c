...
WMcursor *ph_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                            int hot_x, int hot_y) {
  WMcursor *cursor;
  int clen, i;
...
void ph_UpdateMouse(_THIS) {
  ...
  /* synchronizing photon mouse cursor position and SDL mouse position, if
   * cursor appears over window. */
  PtGetAbsPosition(window, &abs_x, &abs_y);
