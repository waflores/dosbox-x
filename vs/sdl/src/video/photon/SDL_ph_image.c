...
int ph_SetupImage(_THIS, SDL_Surface *screen) {
  ...
  if ((bpp == 8) && (desktoppal == SDLPH_PAL_EMULATE)) {
    ...
    if ((SDL_Image = PhCreateImage(NULL, screen->w, screen->h, type, palette,
    ...
    /* using shared memory for speed (set last param to 1) */
    if ((SDL_Image = PhCreateImage(NULL, screen->w, screen->h, type, NULL, 0,
                                   1)) == NULL) {
...
int ph_SetupOCImage(_THIS, SDL_Surface *screen) {
  ...
  /* Currently offscreen contexts with the same bit depth as display bpp only
   * can be created */
...
int ph_SetupFullScreenImage(_THIS, SDL_Surface *screen) {
  ...
  OCImage.offscreen_context = PdCreateOffscreenContext(
      0, 0, 0, Pg_OSC_MAIN_DISPLAY | Pg_OSC_MEM_PAGE_ALIGN | Pg_OSC_CRTC_SAFE);
  if (OCImage.offscreen_context == NULL) {
    SDL_SetError("ph_SetupFullScreenImage(): PdCreateOffscreenContext() "
                 "function failed !\n");
    return -1;
  }

  if ((screen->flags & SDL_DOUBLEBUF) == SDL_DOUBLEBUF) {
    OCImage.offscreen_backcontext = PdDupOffscreenContext(
        OCImage.offscreen_context, Pg_OSC_MEM_PAGE_ALIGN | Pg_OSC_CRTC_SAFE);
    if (OCImage.offscreen_backcontext == NULL) {
      SDL_SetError("ph_SetupFullScreenImage(): PdCreateOffscreenContext(back) "
                   "function failed !\n");
      return -1;
    }
  }
  ...
  if (OCImage.FrameData0 == NULL) {
    SDL_SetError("ph_SetupFullScreenImage(): PdGetOffscreenContextPtr() "
                 "function failed !\n");
  ...
  if ((screen->flags & SDL_DOUBLEBUF) == SDL_DOUBLEBUF) {
    OCImage.FrameData1 = (unsigned char *)PdGetOffscreenContextPtr(
        OCImage.offscreen_backcontext);
    if (OCImage.FrameData1 == NULL) {
      SDL_SetError("ph_SetupFullScreenImage(back): PdGetOffscreenContextPtr() "
                   "function failed !\n");
      ph_DestroyImage(this, screen);
      return -1;
    }
...
int ph_SetupOpenGLImage(_THIS, SDL_Surface *screen) {
  ...
  if (ph_SetupOpenGLContext(this, screen->w, screen->h,
                            screen->format->BitsPerPixel, screen->flags) != 0) {
...
int ph_UpdateHWInfo(_THIS) {
  ...
  if ((vmode.mode_capabilities1 & PgVM_MODE_CAP1_OFFSCREEN) ==
    ...
    /* this is a special test for drivers which tries to lie about offscreen
     * capability */
...
int ph_AllocHWSurface(_THIS, SDL_Surface *surface) {
  ...
  surface->hwdata->offscreenctx = PdCreateOffscreenContext(
...
int ph_HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                   SDL_Rect *dstrect) {
  ...
  if (((src == this->screen) || (src->hwdata != NULL)) &&
    ...
    SDL_SetError("ph_HWAccelBlit(): Source or target surface is not a hardware "
                 "surface !\n");
    return -1;
  }
