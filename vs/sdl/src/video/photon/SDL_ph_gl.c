...
#else
void ph_GL_SwapBuffers(_THIS) { qnxgl_swap_buffers(oglbuffers); }
#endif /* 6.3.0 */
...
int ph_GL_LoadLibrary(_THIS, const char *path) {
  /* if code compiled with SDL_VIDEO_OPENGL, that mean that library already
   * linked */
...
#if (_NTO_VERSION < 630)
void *ph_GL_GetProcAddress(_THIS, const char *proc) { return NULL; }
#else
...
/* This code is actual for the Photon3D Runtime which was available prior to 6.3
 * only */
...
/* This code is actual for the built-in PhGL support, which became available
 * since 6.3 */
...
int ph_SetupOpenGLContext(_THIS, int width, int height, int bpp, Uint32 flags) {
  ...
  if (num_interfaces == 0) {
    SDL_SetError("ph_SetupOpenGLContext(): there are no available OpenGL "
                 "renderers was found !\n");
    return -1;
  }
  ...
  if (this->gl_config.depth_size) {
    ...
    qnxgl_attribs_slide =
        qnxgl_attrib_set_depth(qnxgl_attribs_slide, this->gl_config.depth_size);
  }

  /* Stencil size */
  if (this->gl_config.stencil_size) {
    qnxgl_attribs_slide = qnxgl_attrib_set_stencil(
        qnxgl_attribs_slide, this->gl_config.stencil_size);
  }
  ...
  if ((this->gl_config.accum_red_size != 0) &&
    ...
    qnxgl_attribs_slide = qnxgl_attrib_set_accum(
        qnxgl_attribs_slide, this->gl_config.accum_red_size +
  ...
  oglbuffers = qnxgl_buffers_create(
      ...
      /*                 __QNXGL_BUILD_FORMAT(0, __QNXGL_COLOR_MODEL_RGB,
         this->gl_config.alpha_size, this->gl_config.red_size,
         this->gl_config.green_size, this->gl_config.blue_size), */
  ...
  if (qnxgl_set_current(oglctx) == -1) {
    SDL_SetError("ph_SetupOpenGLContext(): failed to make the OpenGL context "
                 "current !\n");
