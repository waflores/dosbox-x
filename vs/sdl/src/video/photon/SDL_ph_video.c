...
VideoBootStrap ph_bootstrap = {"photon", "QNX Photon video output",
...
static int ph_SetupWindow(_THIS, int w, int h, int flags) {
  ...
  if ((flags & SDL_RESIZABLE) == SDL_RESIZABLE) {
    ...
    PtSetArg(&args[nargs++], Pt_ARG_WINDOW_RENDER_FLAGS, Pt_TRUE,
             Ph_WM_RENDER_RESIZE | Ph_WM_RENDER_MAX | Ph_WM_RENDER_COLLAPSE |
                 Ph_WM_RENDER_RETURN);
  ...
  if (((flags & SDL_NOFRAME) == SDL_NOFRAME) ||
    ...
    PtSetArg(&args[nargs++], Pt_ARG_WINDOW_RENDER_FLAGS, Pt_TRUE,
             Ph_WM_RENDER_BORDER | Ph_WM_RENDER_TITLE | Ph_WM_RENDER_CLOSE |
                 Ph_WM_RENDER_MENU | Ph_WM_RENDER_MIN);
  }
  ...
  if ((flags & SDL_FULLSCREEN) == SDL_FULLSCREEN) {
    ...
    PtSetArg(&args[nargs++], Pt_ARG_WINDOW_STATE, Pt_TRUE,
             Ph_WM_STATE_ISFRONT | Ph_WM_STATE_ISFOCUS | Ph_WM_STATE_ISALTKEY);
  } else {
    PtSetArg(&args[nargs++], Pt_ARG_WINDOW_MANAGED_FLAGS, Pt_FALSE,
    ...
    if (!currently_maximized) {
      ...
      if ((iscentered) ||
          ((windowpos) && (SDL_strcmp(windowpos, "center") == 0))) {
...
static SDL_Surface *ph_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (mask != NULL) {
    SDL_ReallocFormat(current, mask->bpp, mask->red, mask->green, mask->blue,
                      0);
  } else {
    SDL_SetError(
  ...
  if ((current->flags & SDL_OPENGL) == SDL_OPENGL) {
    ...
    if ((current->flags & SDL_FULLSCREEN) == SDL_FULLSCREEN) {
      ...
      /* remove this if we'll have support for the non-fullscreen
       * sw/hw+doublebuf one day */
  ...
  if (desktoppal == SDLPH_PAL_EMULATE) {
    if ((SDL_Image) && (SDL_Image->palette)) {
      for (i = firstcolor; i < firstcolor + ncolors; i++) {
        syspalph[i] = PgRGB(colors[i - firstcolor].r, colors[i - firstcolor].g,
    ...
    if (desktoppal == SDLPH_PAL_SYSTEM) {
      ...
      if ((this->screen->flags & SDL_FULLSCREEN) != SDL_FULLSCREEN) {
        ...
        PgSetPalette(&syspalph[firstcolor], 0, firstcolor, ncolors,
                     Pg_PALSET_GLOBAL, 0);
        /* image needs to be redrawn */
        this->UpdateRects(this, 1, &updaterect);
      } else {
