...
static struct private_yuvhwfuncs ph_yuvfuncs = {
    ph_LockYUVOverlay, ph_UnlockYUVOverlay, ph_DisplayYUVOverlay,
    ph_FreeYUVOverlay};

int grab_ptrs2(PgVideoChannel_t *channel, FRAMEDATA *Frame0,
               FRAMEDATA *Frame1) {
  int planes = 0;
...
SDL_Overlay *ph_CreateYUVOverlay(_THIS, int width, int height, Uint32 format,
                                 SDL_Surface *display) {
  SDL_Overlay *overlay;
  ...
  do {
    ...
    rtncode = PgGetScalerCapabilities(overlay->hwdata->channel, i,
                                      &overlay->hwdata->caps);
    if (rtncode == 0) {
  ...
  planes = grab_ptrs2(overlay->hwdata->channel, overlay->hwdata->FrameData0,
...
int ph_LockYUVOverlay(_THIS, SDL_Overlay *overlay) {
  ...
  /*  overlay->hwdata->current = PgNextVideoFrame(overlay->hwdata->channel);
      ...
      {
          SDL_SetError("ph_LockYUVOverlay: PgNextFrame() failed, bailing
...
int ph_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                         SDL_Rect *dst) {
  int rtncode;
  PhPoint_t pos;
  SDL_Rect backrect;
  ...
  /* If CurrentViewPort position/size has been changed, then move/resize the
   * viewport */
  if ((overlay->hwdata->CurrentViewPort.pos.x != dst->x) ||
    ...
    if (overlay->hwdata->ischromakey == 1) {
      ...
      PgDrawIRect(dst->x, dst->y, dst->x + dst->w - 1, dst->y + dst->h - 1,
                  Pg_DRAW_FILL);
      PgFlush();
    }
    ...
    rtncode = PgConfigScalerChannel(overlay->hwdata->channel,
