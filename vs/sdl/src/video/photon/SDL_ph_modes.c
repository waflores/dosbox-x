...
SDL_Rect **ph_ListModes(_THIS, SDL_PixelFormat *format, Uint32 flags) {
  ...
  for (i = 0; i < mode_list.num_modes; i++) {
    if (PgGetVideoModeInfo(mode_list.modes[i], &mode_info) < 0) {
      SDL_SetError("ph_ListModes(): PgGetVideoModeInfo() function failed on "
                   "mode: 0x%X.\n",
                   mode_list.modes[i]);
      return NULL;
    }
...
void ph_FreeVideoModes(_THIS) { return; }
...
int ph_GetVideoMode(int width, int height, int bpp) {
  ...
  /* if we are here, then no 640x400xbpp mode found and we'll emulate it via
   * 640x480xbpp mode */
  if (modestage == 2) {
    return closestmode;
  }
...
int ph_ToggleFullScreen(_THIS, int on) { return -1; }
...
int ph_EnterFullScreen(_THIS, SDL_Surface *screen, int fmode) {
  ...
  if (!currently_fullscreen) {
    ...
    if (screen->flags & SDL_ANYFORMAT) {
      if ((mode = get_mode_any_format(screen->w, screen->h,
      ...
      if ((mode = ph_GetVideoMode(screen->w, screen->h,
                                  screen->format->BitsPerPixel)) == 0) {
        SDL_SetError(
            "ph_EnterFullScreen(): can't find appropriate video mode !\n");
        return 0;
