...
static int ph_DispatchEvent(_THIS) {
    ...
    if (phevent->num_rects != 0) {
      ...
      if (SDL_VideoSurface) {
        ...
        if (phevent->num_rects > PH_SDL_MAX_RECTS) {
          /* sorry, buffers underrun, we'll update only first PH_SDL_MAX_RECTS
           * rects */
  ...
  case Ph_EV_KEY: {
    ...
    if (Pk_KF_Key_Down & keyEvent->key_flags) {
      ...
      if ((keyEvent->key_cap == Pk_Up) && (keyEvent->key_scan == 0) &&
          ((keyEvent->key_flags & Pk_KF_Scan_Valid) == Pk_KF_Scan_Valid)) {
        posted = SDL_PrivateMouseButton(SDL_PRESSED, SDL_BUTTON_WHEELUP, 0, 0);
        break;
      }
      if ((keyEvent->key_cap == Pk_Down) && (keyEvent->key_scan == 0) &&
          ((keyEvent->key_flags & Pk_KF_Scan_Valid) == Pk_KF_Scan_Valid)) {
        posted =
      ...
      posted =
          SDL_PrivateKeyboard(SDL_PRESSED, ph_TranslateKey(keyEvent, &keysym));
    } else /* must be key release */
    {
      ...
      posted =
          SDL_PrivateKeyboard(SDL_RELEASED, ph_TranslateKey(keyEvent, &keysym));
    }
  } break;
...
void ph_InitOSKeymap(_THIS) { ph_InitKeymap(); }
