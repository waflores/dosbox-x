...
void NX_PumpEvents(_THIS) {
  ...
  while (event.type != GR_EVENT_TYPE_NONE) {
    ...
    case GR_EVENT_TYPE_BUTTON_DOWN: {
      ...
      if (currently_fullscreen) {
        if (check_boundary(this, event.button.x, event.button.y)) {
          SDL_PrivateMouseButton(SDL_PRESSED, button, event.button.x - OffsetX,
                                 event.button.y - OffsetY);
        }
      } else {
        SDL_PrivateMouseButton(SDL_PRESSED, button, event.button.x,
                               event.button.y);
      }
      break;
    }
      ...
      if (currently_fullscreen) {
        if (check_boundary(this, event.button.x, event.button.y)) {
          SDL_PrivateMouseButton(SDL_RELEASED, last_button_down,
                                 event.button.x - OffsetX,
                                 event.button.y - OffsetY);
        }
      } else {
        SDL_PrivateMouseButton(SDL_RELEASED, last_button_down, event.button.x,
                               event.button.y);
      }
      last_button_down = 0;
      break;
