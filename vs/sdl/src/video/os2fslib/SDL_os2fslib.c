...
static ULONG ulFCFToUse = FCF_TITLEBAR | FCF_SYSMENU | FCF_MINBUTTON |
                          FCF_MAXBUTTON | FCF_NOBYTEALIGN | FCF_SIZEBORDER |
                          FCF_TASKLIST;
...
//
/////////////////////////////////////////////////////////////////////
static BOOL SetAccessableWindowPos(HWND hwnd, HWND hwndInsertBehind, LONG x,
...
static UniChar NativeCharToUniChar(int chcode) {
  ...
  rc = UniUconvToUcs(ucoTemp, &pchFrom, &iFromCount, &pucTo, &iToCount,
                     &iNonIdentical);
...
#define CONVERTMOUSEPOSITION()                                                 \
  ...
  if (FSLib_QueryFSMode(hwnd)) {                                               \
    /* We're in FS mode! */                                                                            \
    /* In FS mode our window is as big as fullscreen mode, but not necessary   \
     * as */                                                                   \
    ...
    if (ppts->y < 0)                                                           \
      ppts->y = 0;                                                             \
    if (ppts->x >= pVideo->hidden->SrcBufferDesc.uiXResolution)                \
      ppts->x = pVideo->hidden->SrcBufferDesc.uiXResolution - 1;               \
    if (ppts->y >= pVideo->hidden->SrcBufferDesc.uiYResolution)                \
      ppts->y = pVideo->hidden->SrcBufferDesc.uiYResolution - 1;               \
    pVideo->hidden                                                             \
        ->iSkipWMMOUSEMOVE++; /* Don't take next WM_MOUSEMOVE into account! */                                                                            \
    ptl.x = ppts->x;                                                           \
    ptl.y = ppts->y;                                                           \
    ...
    (ppts->x) = (ppts->x) * pVideo->hidden->SrcBufferDesc.uiXResolution /      \
                swpClient.cx;                                                  \
    (ppts->y) = (ppts->y) * pVideo->hidden->SrcBufferDesc.uiYResolution /      \
                swpClient.cy;                                                  \
    (ppts->y) = pVideo->hidden->SrcBufferDesc.uiYResolution - (ppts->y) - 1;   \
  }
...
fflush(stdout); #endif return (MRESULT) TRUE;
      }
      */
...
//          scan code
        ...
        VK_ code SHORT1FROMMP(mp2), // Character code CHAR4FROMMP(mp1),  // HW
        Scan code &keysym,0));
        }*/

        SDL_PrivateKeyboard(SDL_RELEASED,
                            TranslateKey(SHORT2FROMMP(mp2), // VK_ code
...
//          HW scan code
        ...
        if ((SHORT1FROMMP(mp1) & KC_ALT) && (SHORT2FROMMP(mp2) == VK_HOME)) {
#ifdef DEBUG_BUILD
  ...
  case WM_PAINT: // Window redraw!
    ...
    if (pVideo) {
      if (!pVideo->hidden->pSDLSurface) {
        ...
        if (DosRequestMutexSem(pVideo->hidden->hmtxUseSrcBuffer, 1000) ==
            NO_ERROR) {
...
#endif
          {
            ...
            iXScaleError =
                (pVideo->hidden->SrcBufferDesc.uiXResolution - 1) / swp.cx;
            iYScaleError =
                (pVideo->hidden->SrcBufferDesc.uiYResolution - 1) / swp.cy;
            if (iXScaleError < 0)
              iXScaleError = 0;
            if (iYScaleError < 0)
              iYScaleError = 0;
            iXScaleError2 =
                (swp.cx - 1) / (pVideo->hidden->SrcBufferDesc.uiXResolution);
            iYScaleError2 =
                (swp.cy - 1) / (pVideo->hidden->SrcBufferDesc.uiYResolution);
            if (iXScaleError2 < 0)
              iXScaleError2 = 0;
            if (iYScaleError2 < 0)
              iYScaleError2 = 0;

            iTop = (swp.cy - rcl.yTop) *
                       pVideo->hidden->SrcBufferDesc.uiYResolution / swp.cy -
                   iYScaleError;
            iLeft = rcl.xLeft * pVideo->hidden->SrcBufferDesc.uiXResolution /
                        swp.cx -
                    iXScaleError;
            iWidth = ((rcl.xRight - rcl.xLeft) *
                          pVideo->hidden->SrcBufferDesc.uiXResolution +
                      swp.cx - 1) /
                         swp.cx +
                     2 * iXScaleError;
            iHeight = ((rcl.yTop - rcl.yBottom) *
                           pVideo->hidden->SrcBufferDesc.uiYResolution +
                       swp.cy - 1) /
                          swp.cy +
                      2 * iYScaleError;
            ...
            if (iLeft < 0)
              iLeft = 0;
            if (iTop + iHeight > pVideo->hidden->SrcBufferDesc.uiYResolution)
              iHeight = pVideo->hidden->SrcBufferDesc.uiYResolution - iTop;
            if (iLeft + iWidth > pVideo->hidden->SrcBufferDesc.uiXResolution)
              iWidth = pVideo->hidden->SrcBufferDesc.uiXResolution - iLeft;

#ifdef DEBUG_BUILD
            printf("WM_PAINT : BitBlt: %d %d -> %d %d (Buf %d x %d)\n", iTop,
                   iLeft, iWidth, iHeight,
                   pVideo->hidden->SrcBufferDesc.uiXResolution,
                   pVideo->hidden->SrcBufferDesc.uiYResolution);
            fflush(stdout);
#endif

            FSLIB_BITBLT(hwnd, pVideo->hidden->pchSrcBuffer, iTop, iLeft,
                         iWidth, iHeight);
          }
    ...
    if ((int)mp1 == FSLN_TOGGLEFSMODE) {
      ...
      if (pVideo) {
        if (!pVideo->hidden->pSDLSurface) {
          ...
          // So, don't blit now!
#ifdef DEBUG_BUILD
          printf("WM_FSLIBNOTIFICATION : Can not blit if there is no surface, "
                 "doing nothing.\n");
          fflush(stdout);
#endif
        } else {
          if (DosRequestMutexSem(pVideo->hidden->hmtxUseSrcBuffer, 1000) ==
            ...
            if (pVideo->hidden->pSDLSurface) {
              ...
              if ((!pVideo->hidden->pSDLSurface) ||
                ...
                // So, don't blit now!
#ifdef DEBUG_BUILD
                printf("WM_FSLIBNOTIFICATION : Cannot blit while resizing, "
                       "doing nothing.\n");
                fflush(stdout);
#endif
              } else
#endif
              {
                ...
                fflush(stdout);
#endif
                FSLIB_BITBLT(hwnd, pVideo->hidden->pchSrcBuffer, 0, 0,
                             pVideo->hidden->SrcBufferDesc.uiXResolution,
                             pVideo->hidden->SrcBufferDesc.uiYResolution);
              }
            }
      ...
      if (pVideo->hidden->fInFocus) {
        ...
        if (bMouseCapturable) {
          ...
          bMouseCaptured = 1;
          {
            ...
            ptl.x = 0;
            ptl.y = 0;
            WinMapWindowPoints(pVideo->hidden->hwndClient, HWND_DESKTOP, &ptl,
                               1);
            pVideo->hidden->iSkipWMMOUSEMOVE++; /* Don't take next WM_MOUSEMOVE
                                                   into account!  */
            WinSetPointerPos(HWND_DESKTOP, ptl.x + swpClient.cx / 2,
  ...
  case WM_BUTTON1DOWN:
    ...
    pVideo = FSLib_GetUserParm(hwnd);
    if (pVideo) {
      SDL_PrivateMouseButton(SDL_PRESSED, SDL_BUTTON_LEFT, 0,
                             0); // Don't report mouse movement!
  ...
  case WM_BUTTON1UP:
    ...
    fflush(stdout);
#endif
    SDL_PrivateMouseButton(SDL_RELEASED, SDL_BUTTON_LEFT, 0,
                           0); // Don't report mouse movement!
    break;
  case WM_BUTTON2DOWN:
#ifdef DEBUG_BUILD
    ...
    pVideo = FSLib_GetUserParm(hwnd);
    if (pVideo) {
      SDL_PrivateMouseButton(SDL_PRESSED, SDL_BUTTON_RIGHT, 0,
                             0); // Don't report mouse movement!
  ...
  case WM_BUTTON2UP:
    ...
    fflush(stdout);
#endif
    SDL_PrivateMouseButton(SDL_RELEASED, SDL_BUTTON_RIGHT, 0,
                           0); // Don't report mouse movement!
    break;
  case WM_BUTTON3DOWN:
#ifdef DEBUG_BUILD
    ...
    pVideo = FSLib_GetUserParm(hwnd);
    if (pVideo) {
      SDL_PrivateMouseButton(SDL_PRESSED, SDL_BUTTON_MIDDLE, 0,
                             0); // Don't report mouse movement!
  ...
  case WM_BUTTON3UP:
    ...
    fflush(stdout);
#endif
    SDL_PrivateMouseButton(SDL_RELEASED, SDL_BUTTON_MIDDLE, 0,
                           0); // Don't report mouse movement!
    break;
  case WM_MOUSEMOVE:
#ifdef DEBUG_BUILD
...
//      printf("WM_MOUSEMOVE done\n"); fflush(stdout);
  ...
  case WM_UPDATERECTSREQUEST:
    ...
    if ((pVideo) && (pVideo->hidden->pSDLSurface)) {
      if (DosRequestMutexSem(pVideo->hidden->hmtxUseSrcBuffer,
...
#endif
        {
          ...
          for (i = 0; i < numrects; i++)
            FSLIB_BITBLT(hwnd, pVideo->hidden->pchSrcBuffer, rects[i].y,
                         rects[i].x, rects[i].w, rects[i].h);
        }
        DosReleaseMutexSem(pVideo->hidden->hmtxUseSrcBuffer);
      }
    }
...
//
/////////////////////////////////////////////////////////////////////
static MRESULT EXPENTRY FrameWndProc(HWND hwnd, ULONG msg, MPARAM mp1,
                                     MPARAM mp2) {
  PFNWP pOldFrameProc;
  MRESULT result;
  PTRACKINFO ti;
  ...
  if ((pVideo->hidden->bProportionalResize) && (msg == WM_ADJUSTWINDOWPOS) &&
    ...
    if (pswpTemp->fl & SWP_SIZE) {
      ...
      if (!(pVideo->hidden->ulResizingFlag & TF_TOP)) {
        pswpTemp->y =
            pswpTemp->y + pswpTemp->cy - (rclTemp.yTop - rclTemp.yBottom);
  ...
  if ((pVideo->hidden->bProportionalResize) && (msg == WM_QUERYTRACKINFO)) {
    ...
    if ((ti->fs & TF_LEFT) || (ti->fs & TF_RIGHT) || (ti->fs & TF_TOP) ||
        (ti->fs & TF_BOTTOM))
...
static void PMThreadFunc(void *pParm) {
  ...
  if (hmq == 0) {
    ...
    printf("                 It might be that the application using SDL is not "
           "a PM app!\n");
    fflush(stdout);
#endif
    ...
    rc = FSLib_CreateWindow(HWND_DESKTOP, 0, &fcf, "SDL Application",
                            NULLHANDLE, 0, &(pVideo->hidden->SrcBufferDesc),
                            WndProc, &(pVideo->hidden->hwndClient),
  ...
  */
  ...
  if (pVideo->hidden->iPMThreadStatus == 0) {
    ...
    // If a new PM thread has been spawned (reinitializing video mode), then all
    // right. Otherwise, we have a problem, the app doesn't want to stop. Kill!
    if (iNumOfPMThreadInstances == 0) {
#ifdef DEBUG_BUILD
      printf("[PMThreadFunc] : It seems that the application haven't "
             "terminated itself\n");
      fflush(stdout);
      printf("[PMThreadFunc] : in the last 5 seconds, so we go berserk.\n");
      fflush(stdout);
      printf("[PMThreadFunc] : Brute force mode. :) Killing process! "
             "Dieeeee...\n");
      fflush(stdout);
#endif
...
WMcursor *os2fslib_CreateWMCursor_Win(_THIS, Uint8 *data, Uint8 *mask, int w,
...
WMcursor *os2fslib_CreateWMCursor_FS(_THIS, Uint8 *data, Uint8 *mask, int w,
                                     int h, int hot_x, int hot_y) {
#ifdef DEBUG_BUILD
...
static int os2fslib_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void os2fslib_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

/* We need to wait for vertical retrace on page flipped displays */
static int os2fslib_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }

static void os2fslib_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }

static int os2fslib_SetColors(_THIS, int firstcolor, int ncolors,
                              SDL_Color *colors) {
...
void os2fslib_SetIcon(_THIS, SDL_Surface *icon, Uint8 *mask) {
  ...
  icon_rgb =
      SDL_CreateRGBSurface(SDL_SWSURFACE, icon->w, icon->h, 32, 0, 0, 0, 0);
  if (icon_rgb == NULL) {
    SDL_free(pchTemp);
    return;
  }
  ...
  /* Copy pixels upside-down from RGB surface into BMP, masked with the icon
   * mask */
...
static void os2fslib_InitOSKeymap(_THIS) {
  ...
  HWScanKeyMap[0x56] =
      SDLK_WORLD_1; // Code 161, letter i' on hungarian keyboard
...
int os2fslib_IconifyWindow(_THIS) {
  ...
  WinSetWindowPos(_this->hidden->hwndFrame, HWND_TOP, 0, 0, 0, 0, SWP_MINIMIZE);
  ...
  if (mode == SDL_GRAB_OFF) {
    ...
    if (WinQueryFocus(HWND_DESKTOP) == _this->hidden->hwndClient) {
      ...
      bMouseCaptured = 1;
      {
        ...
        _this->hidden->iSkipWMMOUSEMOVE++; /* Don't take next WM_MOUSEMOVE into
                                              account!  */
        WinSetPointerPos(HWND_DESKTOP, ptl.x + swpClient.cx / 2,
                         ptl.y + swpClient.cy / 2);
      }
...
#ifdef BITBLT_IN_WINMESSAGEPROC
  WinSendMsg(_this->hidden->hwndClient, WM_UPDATERECTSREQUEST, (MPARAM)numrects,
             (MPARAM)rects);
#else
  if (DosRequestMutexSem(_this->hidden->hmtxUseSrcBuffer,
                         SEM_INDEFINITE_WAIT) == NO_ERROR) {
    int i;
...
static SDL_Surface *os2fslib_SetVideoMode(_THIS, SDL_Surface *current,
...
#ifdef DEBUG_BUILD
  printf("[os2fslib_SetVideoMode] : Changed request to %dx%d @ %dBPP, "
         "flags=0x%x\n",
         width, height, bpp, flags);
  fflush(stdout);
#endif
  ...
  if (!pModeInfoFound) {
#ifdef DEBUG_BUILD
    printf("[os2fslib_SetVideoMode] : Requested video mode not found, looking "
           "for a similar one!\n");
    fflush(stdout);
#endif
    ...
    while (pModeInfo) {
      // Check all available fullscreen modes for this resolution
      if ((pModeInfo->uiXResolution >= width) &&
          (pModeInfo->uiYResolution >= height) && (pModeInfo->uiBPP == bpp)) {
        if (!pModeInfoFound)
          pModeInfoFound = pModeInfo;
        else if (((pModeInfoFound->uiXResolution - width) *
                  (pModeInfoFound->uiYResolution - height)) >
                 ((pModeInfo->uiXResolution - width) *
                  (pModeInfo->uiYResolution - height))) {
  ...
  if (flags & SDL_RESIZABLE) {
#ifdef DEBUG_BUILD
    printf("[os2fslib_SetVideoMode] : Requested mode is resizable, changing "
           "width/height\n");
    fflush(stdout);
#endif
  ...
  if (DosRequestMutexSem(_this->hidden->hmtxUseSrcBuffer,
    ...
    pResult = SDL_CreateRGBSurface(
        SDL_SWSURFACE, pModeInfoFound->uiXResolution,
        pModeInfoFound->uiYResolution, pModeInfoFound->uiBPP,
        ((unsigned int)pModeInfoFound->PixelFormat.ucRedMask)
            << pModeInfoFound->PixelFormat.ucRedPosition,
        ((unsigned int)pModeInfoFound->PixelFormat.ucGreenMask)
            << pModeInfoFound->PixelFormat.ucGreenPosition,
        ((unsigned int)pModeInfoFound->PixelFormat.ucBlueMask)
            << pModeInfoFound->PixelFormat.ucBluePosition,
        ((unsigned int)pModeInfoFound->PixelFormat.ucAlphaMask)
            << pModeInfoFound->PixelFormat.ucAlphaPosition);
    ...
    pResult->format->Amask = pResult->format->Ashift = pResult->format->Aloss =
        0;
#endif
    ...
    if (((flags & SDL_RESIZABLE) == 0) || (bFirstCall)) {
      ...
      SetAccessableWindowPos(_this->hidden->hwndFrame, HWND_TOP, 0, 0,
                             (rectl.xRight - rectl.xLeft),
                             (rectl.yTop - rectl.yBottom),
 ...
 */
static SDL_Rect **os2fslib_ListModes(_THIS, SDL_PixelFormat *format,
                                     Uint32 flags) {
#ifdef DEBUG_BUILD
...
//            iPlace);
            ...
            pNewList = (SDL_Rect **)SDL_realloc(_this->hidden->pListModesResult,
                                                (iNumOfSlots + 1) *
                                                    sizeof(SDL_Rect *));
            if (pNewList) {
...
static int os2fslib_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  vformat->Rmask = ((unsigned int)pDesktopMode->PixelFormat.ucRedMask)
                   << pDesktopMode->PixelFormat.ucRedPosition;
  vformat->Rshift = pDesktopMode->PixelFormat.ucRedPosition;
  vformat->Rloss = pDesktopMode->PixelFormat.ucRedAdjust;
  vformat->Gmask = ((unsigned int)pDesktopMode->PixelFormat.ucGreenMask)
                   << pDesktopMode->PixelFormat.ucGreenPosition;
  vformat->Gshift = pDesktopMode->PixelFormat.ucGreenPosition;
  vformat->Gloss = pDesktopMode->PixelFormat.ucGreenAdjust;
  vformat->Bmask = ((unsigned int)pDesktopMode->PixelFormat.ucBlueMask)
                   << pDesktopMode->PixelFormat.ucBluePosition;
  vformat->Bshift = pDesktopMode->PixelFormat.ucBluePosition;
  vformat->Bloss = pDesktopMode->PixelFormat.ucBlueAdjust;
  vformat->Amask = ((unsigned int)pDesktopMode->PixelFormat.ucAlphaMask)
                   << pDesktopMode->PixelFormat.ucAlphaPosition;
  ...
  _this->hidden->pchSrcBuffer = (char *)SDL_malloc(
      _this->hidden->pAvailableFSLibVideoModes->uiScanLineSize *
      _this->hidden->pAvailableFSLibVideoModes->uiYResolution);
  if (!_this->hidden->pchSrcBuffer) {
#ifdef DEBUG_BUILD
    printf("[os2fslib_VideoInit] : Yikes, not enough memory for new video "
           "buffer!\n");
    fflush(stdout);
#endif
...
VideoBootStrap OS2FSLib_bootstrap = {"os2fslib",
                                     "OS/2 Video Output using FSLib",
