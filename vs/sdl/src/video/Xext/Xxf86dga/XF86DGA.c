...
Bool SDL_NAME(XF86DGAQueryExtension)(Display *dpy, int *event_basep,
                                     int *error_basep) {
  return SDL_NAME(XDGAQueryExtension)(dpy, event_basep, error_basep);
}

Bool SDL_NAME(XF86DGAQueryVersion)(Display *dpy, int *majorVersion,
                                   int *minorVersion) {
  return SDL_NAME(XDGAQueryVersion)(dpy, majorVersion, minorVersion);
}

Bool SDL_NAME(XF86DGAGetVideoLL)(Display *dpy, int screen, int *offset,
...
Bool SDL_NAME(XF86DGAGetViewPortSize)(Display *dpy, int screen, int *width,
                                      int *height) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
Bool SDL_NAME(XF86DGAInstallColormap)(Display *dpy, int screen, Colormap cmap) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
static MapPtr FindMap(unsigned long address, unsigned long size) {
  int i;
...
static ScrPtr FindScr(Display *display, int screen) {
  int i;
...
static void *MapPhysAddress(unsigned long address, unsigned long size) {
...
#define PMAP_MAP 0x44
  ...
  if (rc != 0)
    return NULL;
#elif defined(Lynx)
  vaddr = (void *)smem_create("XF86DGA", (char *)offset, size + delta,
                              SM_READ | SM_WRITE);
#else
#ifndef MAP_FILE
#define MAP_FILE 0
...
Bool SDL_NAME(XF86DGAGetVideo)(Display *dis, int screen, char **addr,
