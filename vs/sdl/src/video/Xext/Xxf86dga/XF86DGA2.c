...
Bool SDL_NAME(XDGAMapFramebuffer)(int, char *, unsigned char *, CARD32, CARD32,
                                  CARD32);
...
Bool SDL_NAME(XDGAQueryExtension)(Display *dpy, int *event_basep,
                                  int *error_basep) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
Bool SDL_NAME(XDGAQueryVersion)(Display *dpy, int *majorVersion,
                                int *minorVersion) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
Bool SDL_NAME(XDGAOpenFramebuffer)(Display *dpy, int screen) {
  ...
  ret = SDL_NAME(XDGAMapFramebuffer)(screen, deviceName,
                                     (unsigned char *)(long)rep.mem1, rep.size,
                                     rep.offset, rep.extra);
...
SDL_NAME(XDGAMode) *
    SDL_NAME(XDGAQueryModes)(Display *dpy, int screen, int *num) {
  ...
  *num = 0;
  ...
  if (_XReply(dpy, (xReply *)&rep, 0, xFalse)) {
    if (rep.length) {
      ...
      modes = (SDL_NAME(XDGAMode) *)Xmalloc(
          (rep.number * sizeof(SDL_NAME(XDGAMode))) + size);
...
SDL_NAME(XDGADevice) *
  ...
  if (_XReply(dpy, (xReply *)&rep, 0, xFalse)) {
    if (rep.length) {
      ...
      dev =
          (SDL_NAME(XDGADevice) *)Xmalloc(sizeof(SDL_NAME(XDGADevice)) + size);
...
void SDL_NAME(XDGASetViewport)(Display *dpy, int screen, int x, int y,
                               int flags) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
void SDL_NAME(XDGAFillRectangle)(Display *dpy, int screen, int x, int y,
...
void SDL_NAME(XDGACopyArea)(Display *dpy, int screen, int srcx, int srcy,
                            unsigned int width, unsigned int height, int dstx,
                            int dsty) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
void SDL_NAME(XDGACopyTransparentArea)(Display *dpy, int screen, int srcx,
                                       int srcy, unsigned int width,
...
void SDL_NAME(XDGAChangePixmapMode)(Display *dpy, int screen, int *x, int *y,
                                    int mode) {
  XExtDisplayInfo *info = SDL_NAME(xdga_find_display)(dpy);
...
Colormap SDL_NAME(XDGACreateColormap)(Display *dpy, int screen,
...
static Bool DGAMapPhysical(int, char *, unsigned char *, CARD32, CARD32, CARD32,
                           DGAMapPtr);
...
Bool SDL_NAME(XDGAMapFramebuffer)(int screen,
...
static Bool DGAMapPhysical(int screen, char *name, /* optional device name */
                           unsigned char *base,    /* physical memory */
                           CARD32 size,            /* size */
                           CARD32 offset,          /* optional offset */
                           CARD32 extra,           /* optional extra data */
...
#define PMAP_MAP 0x44
    ...
    pmap.a.phys = base;
    pmap.size = size;
    rc = DosDevIOCtl(hfd, XFREE86_PMAP, PMAP_MAP, (PULONG)&pmap, sizeof(pmap),
                     &plen, (PULONG)&dmap, sizeof(dmap), &dlen);
    if (rc == 0) {
...
static void DGAUnmapPhysical(DGAMapPtr pMap) {
#if !defined(ISC) && !defined(HAS_SVR3_MMAP) && !defined(Lynx) &&              \
    !defined(__EMX__)
