...
void PG_InitOSKeymap(_THIS) { /* We need no keymap */ }

void PG_InitEvents(_THIS) {
  /* Turn on all the mouse and keyboard triggers for our canvas, normally less
   * important events like mouse movement are ignored to save bandwidth. */
  pgSetWidget(this->hidden->wCanvas, PG_WP_TRIGGERMASK,
              pgGetWidget(this->hidden->wCanvas, PG_WP_TRIGGERMASK) |
                  ...
                  PG_TRIGGER_KEYUP | PG_TRIGGER_KEYDOWN | PG_TRIGGER_CHAR,
              0);
