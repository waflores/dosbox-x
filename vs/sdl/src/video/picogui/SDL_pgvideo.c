...
WMcursor *PG_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                            int hot_x, int hot_y);
...
VideoBootStrap PG_bootstrap = {PGVID_DRIVER_NAME, "PicoGUI SDL driver",
...
SDL_Surface *PG_SetVideoMode(_THIS, SDL_Surface *current, int width, int height,
  ...
  current->pixels =
      shmat(shmget(this->hidden->shm.shm_key, this->hidden->shm.shm_length, 0),
            NULL, 0);
   ...
   */
  pgWriteCmd(this->hidden->wCanvas, PGCANVAS_GROP, 5, PG_GROP_SETSRC, 0, 0, 0,
             0);
   ...
   */
  pgWriteCmd(this->hidden->wCanvas, PGCANVAS_GROP, 6, PG_GROP_BITMAP, 0, 0, 0,
             0, this->hidden->bitmap);
  pgWriteCmd(this->hidden->wCanvas, PGCANVAS_GROPFLAGS, 1,
             PG_GROPF_INCREMENTAL);

  /* 2. Normal bitmap rendering
   */
  pgWriteCmd(this->hidden->wCanvas, PGCANVAS_GROP, 6, PG_GROP_BITMAP, 0, 0,
             this->hidden->shm.width, this->hidden->shm.height,
             this->hidden->bitmap);
...
static int PG_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void PG_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

/* We need to wait for vertical retrace on page flipped displays */
static int PG_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }

static void PG_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void PG_UpdateRects(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; i++) {
    ...
    pgWriteCmd(this->hidden->wCanvas, PGCANVAS_MOVEGROP, 4, rects[i].x,
...
int PG_ShowWMCursor(_THIS, WMcursor *cursor) { return 1; }
