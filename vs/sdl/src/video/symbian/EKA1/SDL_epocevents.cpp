...
int EPOC_HandleWsEvent(_THIS, const TWsEvent &aWsEvent) {
  ...
  case EEventPointer: /* Mouse pointer events */
  {
    ...
    if (mode == EPointerCursorNone) {
      return 0; // TODO: Find out why events are get despite of cursor should be
                // off
    }
  ...
  case EEventFocusLost: /* SDL window lost focus */
  {
    ...
    */
    ...
    /*
            Private->EPOC_WsSession.SetWindowGroupOrdinalPosition(Private->EPOC_WsWindowGroupID,
       -1);
            ...
            if(1 <
       Private->EPOC_WsSession.GetWindowGroupOrdinalPriority(Private->EPOC_WsWindowGroupID))
                {
                Private->EPOC_WsSession.SetWindowGroupOrdinalPosition(focus,
       -1); SDL_Delay(500);
  ...
  if (KErrNone == fs.Connect()) {
    ...
    if (err == KErrNone && array.Count() > 0) {
      ...
      for (TInt k = 0; k < array.Count(); k += 2) {
        ...
        if (oldval >= 0 && oldval < MAX_SCANCODE && newval >= 0 &&
            newval < MAX_SCANCODE) {
