...
public:
  static CLockable *Lockable(CFbsBitmap *aBmp) {
    return static_cast<CLockable *>(aBmp);
  }
  void Lock() { LockHeap(); }
...
#else
  if (param) //!! Do not work if the parameter is really 0!!
    RFileLogger::WriteFormat(KLibName, _L("SDL.txt"), EFileLoggingModeAppend,
                             error, param);
  else
...
extern "C" int Debug_AvailMem() { return (Debug_AvailMem2()); }
...
static SDL_Surface *EPOC_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
void DirectDrawRotated(_THIS, int numrects, SDL_Rect *rects,
                       TUint16 *screenBuffer);

/* Mouse functions */

static WMcursor *EPOC_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w,
...
VideoBootStrap EPOC_bootstrap = {"epoc", "EPOC system", EPOC_Available,
                                 EPOC_CreateDevice};
...
static int EPOC_Available(void) { return 1; /* Always available */ }
...
void DisableKeyBlocking(_THIS) {
  ...
  TRawEvent event;
  event.Set(
      (TRawEvent::TType) /*EDisableKeyBlock*/ 51); // !!EDisableKeyBlock not
                                                   // found in epoc32\include!
...
void ConstructWindowL(_THIS) {
  ...
  Private->EPOC_WsScreen =
      new (ELeave) CWsScreenDevice(Private->EPOC_WsSession);
  User::LeaveIfError(Private->EPOC_WsScreen->Construct());
  User::LeaveIfError(
      Private->EPOC_WsScreen->CreateContext(Private->EPOC_WindowGc));
  ...
  // Set window group name (the same as process name)) !!Gives always "EPOC" in
  // WINS
  ...
  User::LeaveIfError(Private->EPOC_WsWindow.Construct(
      Private->EPOC_WsWindowGroup, WindowClientHandle - 1));
...
int EPOC_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  Private->SDL_modelist[0]->w = 800;
  Private->SDL_modelist[0]->h = 250;
  Private->SDL_modelist[1]->w = 640;
  Private->SDL_modelist[1]->h = 480;
  Private->SDL_modelist[2]->w = 480;
  Private->SDL_modelist[2]->h = 600;
  Private->SDL_modelist[3]->w = 640;
  Private->SDL_modelist[3]->h = 400;
  Private->SDL_modelist[4]->w = 352;
  Private->SDL_modelist[4]->h = 416;
  Private->SDL_modelist[5]->w = 416;
  Private->SDL_modelist[5]->h = 352;
  Private->SDL_modelist[6]->w = 416;
  Private->SDL_modelist[6]->h = 312;
  Private->SDL_modelist[7]->w = 352;
  Private->SDL_modelist[7]->h = 264;
  Private->SDL_modelist[8]->w = 800;
  Private->SDL_modelist[8]->h = 240; // for doom all these..
  Private->SDL_modelist[9]->w = 640;
  Private->SDL_modelist[9]->h = 240;
  Private->SDL_modelist[10]->w = 480;
  Private->SDL_modelist[10]->h = 240;
  Private->SDL_modelist[11]->w = 640;
  Private->SDL_modelist[11]->h = 240;
  Private->SDL_modelist[12]->w = 352;
  Private->SDL_modelist[12]->h = 240;
  Private->SDL_modelist[13]->w = 416;
  Private->SDL_modelist[13]->h = 240;
  Private->SDL_modelist[14]->w = 416;
  Private->SDL_modelist[14]->h = 240;
  Private->SDL_modelist[15]->w = 352;
  Private->SDL_modelist[15]->h = 240;
  Private->SDL_modelist[16]->w = 640;
  Private->SDL_modelist[16]->h = 200;
  Private->SDL_modelist[17]->w = 320;
  Private->SDL_modelist[17]->h =
      240; //...for doom, currently engine renders no-higher windows :-(,
           //propably should get fixed
  Private->SDL_modelist[18]->w = 320;
  Private->SDL_modelist[18]->h = 200;
  Private->SDL_modelist[19]->w = 256;
  Private->SDL_modelist[19]->h = 192;
  Private->SDL_modelist[20]->w = 176;
  Private->SDL_modelist[20]->h = 208;
  Private->SDL_modelist[21]->w = 208;
  Private->SDL_modelist[21]->h = 176; // Rotated
  Private->SDL_modelist[22]->w = 160;
  Private->SDL_modelist[22]->h = 144;

  Private->SDL_modelist[23]->w = 640;
  Private->SDL_modelist[2]->h = 200; // s80 some new modes
  Private->SDL_modelist[24]->w = 640;
  Private->SDL_modelist[2]->h = 320; // s90 modes are added
  Private->SDL_modelist[25]->w = 640;
  Private->SDL_modelist[2]->h = 240; // here
  Private->SDL_modelist[26]->w = 640;
  Private->SDL_modelist[4]->h = 200; // now
  ...
  Private->EPOC_FrameBuffer =
      Private->EPOC_HasFrameBuffer ? (TUint8 *)screenInfo.iScreenAddress : NULL;
  Private->EPOC_BytesPerPixel = ((GetBpp(displayMode) - 1) / 8) + 1;

  Private->EPOC_BytesPerScanLine =
      screenInfo.iScreenSize.iWidth * Private->EPOC_BytesPerPixel;
  Private->EPOC_BytesPerScreen =
      Private->EPOC_BytesPerScanLine * Private->EPOC_ScreenSize.iHeight;
  ...
  /* It seems that in SA1100 machines for 8bpp displays there is a 512 palette
   * table at the beginning of the frame buffer. E.g. Series 7 and Netbook. In
   * 12 bpp machines the table has 16 entries.
   */
...
*/
  ...
  Private->EPOC_FrameBuffer =
      NULL; /* Private->EPOC_Bitmap->DataAddress() can change any time */
  Private->EPOC_BytesPerPixel = ((GetBpp(displayMode) - 1) / 8) + 1;
  Private->EPOC_BytesPerScanLine =
      Private->EPOC_WsScreen->SizeInPixels().iWidth *
      Private->EPOC_BytesPerPixel;
  ...
  TRAPD(status, Private->EPOC_DrawDevice =
                    CFbsDrawDevice::NewScreenDeviceL(screenInfo2, displayMode));
  ...
  User::WaitForRequest(
      Private->EPOC_WsEventStatus); // Markus: I added this and ...
  ...
  User::WaitForRequest(
      Private->EPOC_RedrawEventStatus); //...this, if not catches a stray event
                                        //is risen if there are active objects
                                        // used, or confucing actions with
...
int EPOC_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  if ((firstcolor + ncolors) > 256)
    return -1;
  //    SDL_TRACE1("colors %d",
  //    (TDisplayModeUtils::NumDisplayModeColors(Private->EPOC_DisplayMode)));
  ...
  } else if (TDisplayModeUtils::NumDisplayModeColors(
                 Private->EPOC_DisplayMode) == 65536) {
    for (int i = firstcolor; i < ncolors; i++) {
      ...
      EPOC_HWPalette_256_to_Screen[i] = color64K;
                 ...
                 Private->EPOC_DisplayMode) == 16777216) {
    for (int i = firstcolor; i < ncolors; i++) {
      // 16M-colour
...
SDL_Surface *EPOC_SetVideoMode(_THIS, SDL_Surface *current, int width,
                               int height, int bpp, Uint32 /*flags*/) {
  SDL_TRACE("SDL:EPOC_SetVideoMode");
  /* Check parameters */
#ifdef SYMBIAN_CRYSTAL
  if (!(bpp == 8 || bpp == 12 || bpp == 16) &&
      ((width == 640 && height == 200) || (width == 640 && height == 400) ||
       (width == 640 && height == 480) || (width == 320 && height == 200) ||
  ...
  if (!(bpp == 8 || bpp == 12 || bpp == 16) &&
      ((width == 320 && height == 200) || (width == 320 && height == 240) ||
       (width == 256 && height == 192) || (width == 176 && height == 208) ||
  ...
  if (bpp == 8)
    current->flags =
        (SDL_FULLSCREEN | SDL_SWSURFACE | SDL_PREALLOC | SDL_HWPALETTE);
  else // 12 bpp, 16 bpp
  ...
  /*
   *  Logic for getting suitable screen dimensions, offset, scaling and
   * orientation
   */
  ...
  /*
          if ((width>Private->EPOC_ScreenSize.iWidth) &&  (width>height)) {
                  Private->EPOC_ScreenOrientation =
     CFbsBitGc::EGraphicsOrientationRotated270; w = current->h; h = current->w;
          }
  */
  // Get nearest stepwise scale values for width and height. The smallest
  // supported scaled screen is 1/2.
  TInt scaleValue = 0;
  Private->EPOC_ScreenXScaleValue = 1;
  Private->EPOC_ScreenYScaleValue = 1;
  if (w > Private->EPOC_ScreenSize.iWidth) {
    // Find the biggest scale value that result the width that fits in the
    // screen HW
    ...
    w = (w * (Private->EPOC_ScreenXScaleValue - 1)) /
        Private->EPOC_ScreenXScaleValue;
  }
  if (h > Private->EPOC_ScreenSize.iHeight) {
    // Find the biggest scale value that result the height that fits in the
    // screen HW
    for (scaleValue = 2; scaleValue++;) {
    ...
    h = (h * (Private->EPOC_ScreenYScaleValue - 1)) /
        Private->EPOC_ScreenYScaleValue;
  }
...
void RedrawWindowL(_THIS) {
  ...
  if (Private->EPOC_ScreenOrientation ==
  ...
  UnlockHeap(Private->EPOC_Bitmap);
  ; // Unlock bitmap heap
...
static void EPOC_FreeHWSurface(_THIS, SDL_Surface * /*surface*/) { return; }

static int EPOC_LockHWSurface(_THIS, SDL_Surface * /*surface*/) { return (0); }
static void EPOC_UnlockHWSurface(_THIS, SDL_Surface * /*surface*/) { return; }

static int EPOC_FlipHWSurface(_THIS, SDL_Surface * /*surface*/) { return (0); }

static void EPOC_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) {
  // TInt focusWindowGroupId =
  // Private->EPOC_WsSession.GetFocusWindowGroup();//these are async services
  // if (focusWindowGroupId != Private->EPOC_WsWindowGroupID) { //for that
  ...
  if (!Private->EPOC_IsWindowFocused) {
    ...
    /*
            ...
            TRect  rect = TRect(Private->EPOC_WsWindow.Size());
            Private->EPOC_WsWindow.Invalidate(rect);
            Private->EPOC_WsWindowGroup.SetOrdinalPosition(pos,
        ...
        */
    //    if (Private->EPOC_WsSession.GetFocusWindowGroup() !=
    //    Private->EPOC_WsWindowGroupID) {

    /* !!TODO: Could call GetRedraw() etc. for WsSession and redraw the screen
       if needed. That might be needed if a small dialog comes in front of Game
       screen.
    */
    // while (Private->EPOC_WsSession.GetFocusWindowGroup() !=
    // Private->EPOC_WsWindowGroupID)
...
//		return;
  ...
  Private->EPOC_DrawDevice->UpdateRegion(
      rect2); // Should we update rects parameter area only??
  ...
  */
  ...
  if((foo % 200) == 0)
      ...
      int e = b->Create(TSize(currentRect.w, currentRect.h),
  Private->EPOC_DisplayMode);
...
void DirectDraw(_THIS, int numrects, SDL_Rect *rects, TUint16 *screenBuffer) {
  ...
  const TInt sourceNumBytesPerPixel =
      ((_this->screen->format->BitsPerPixel - 1) >> 3) + 1;
  ...
  for (i = 0; i < numrects; ++i) {
    ...
    const TInt sourceRectWidthInBytes =
        sourceRectWidth * sourceNumBytesPerPixel;
    ...
    TInt targetStartOffset = fixedOffset.iX + rect2.x +
                             (fixedOffset.iY + rect2.y) * targetScanlineLength;
    ...
    case 12: {
      TUint16 *bitmapLine =
          (TUint16 *)_this->screen->pixels + sourceStartOffset;
      ...
      if (skipValue == 1) {
        ...
        for (TInt y = 0; y < sourceRectHeight; y++) {
          // TODO: optimize: separate loops for 1, 2 and n skip. Mem::Copy() can
          // be used in unscaled case.
          ...
          for (TInt x = 0; x < sourceRectWidth; x++) {
            __ASSERT_DEBUG(
                screenMemory <
                    (screenBuffer + Private->EPOC_ScreenSize.iWidth *
                                        Private->EPOC_ScreenSize.iHeight),
    ...
    default: {
      if (Private->EPOC_BytesPerPixel <= 2) {
        TUint8 *bitmapLine =
            (TUint8 *)_this->screen->pixels + sourceStartOffset;
        TUint16 *screenMemory = screenBuffer + targetStartOffset;
        for (TInt y = 0; y < sourceRectHeight; y++) {
        ...
        TUint8 *bitmapLine =
            (TUint8 *)_this->screen->pixels + sourceStartOffset;
        TUint32 *screenMemory =
            reinterpret_cast<TUint32 *>(screenBuffer + targetStartOffset);
        for (TInt y = 0; y < sourceRectHeight; y++) {
          TUint8 *bitmapPos = bitmapLine;              /* 1 byte per pixel */
          TUint32 *screenMemoryLinePos = screenMemory; /* 2 bytes per pixel */
          /* Convert each pixel from 256 palette to 4k color values */
          for (TInt x = 0; x < sourceRectWidth; x++) {
            __ASSERT_DEBUG(screenMemoryLinePos <
                               (reinterpret_cast<TUint32 *>(screenBuffer) +
                                (Private->EPOC_ScreenSize.iWidth *
                                 Private->EPOC_ScreenSize.iHeight)),
                           User::Panic(_L("SDL"), KErrCorrupt));
            __ASSERT_DEBUG(screenMemoryLinePos >=
                               reinterpret_cast<TUint32 *>(screenBuffer),
...
void DirectDraw(_THIS, int numrects, SDL_Rect *rects, TUint16* screenBuffer)
{
        TInt i;
    const TInt sourceNumBytesPerPixel =
((_this->screen->format->BitsPerPixel-1)>>3) + 1; const TPoint fixedOffset =
Private->EPOC_ScreenOffset; const TInt screenW = _this->screen->w; const TInt
screenH = _this->screen->h; const TInt sourceScanlineLength = screenW; const
...
/*		TInt sourceRectWidth = maxX - rect2.x + 1;
        const TInt sourceRectWidthInBytes = sourceRectWidth *
   sourceNumBytesPerPixel; const TInt sourceRectHeight = maxY - rect2.y + 1;
        const TInt sourceStartOffset = rect2.x + rect2.y * sourceScanlineLength;
                const TUint skipValue = Private->EPOC_ScreenXScaleValue; //1; //
   no skip

                const TInt targetStartOffset = // = (fixedOffset.iX + (rect2.x /
   skipValue) + (fixedOffset.iY + rect2.y) * targetScanlineLength ) ; (skipValue
   > 1 ? (fixedOffset.iX + (rect2.x / skipValue) + (fixedOffset.iY + rect2.y) *
   targetScanlineLength ) : (fixedOffset.iX +  rect2.x              +
   (fixedOffset.iY + rect2.y) * targetScanlineLength ));

                __ASSERT_DEBUG(skipValue >= 1, User::Panic(KLibName,
   KErrArgument));
        ...
        switch (_this->screen->format->BitsPerPixel)
                ...
                case 12:
                        {
                        TUint16* bitmapLine = (TUint16*)_this->screen->pixels +
   sourceStartOffset; TUint16* screenMemory = screenBuffer + targetStartOffset;
                        if (skipValue == 1)
                                {
                                for(TInt y = 0 ; y < sourceRectHeight ; y++)
                                        {
                                        Mem::Copy(screenMemory, bitmapLine,
   sourceRectWidthInBytes);
                                        }
                                        bitmapLine += sourceScanlineLength;
                                        screenMemory += targetScanlineLength;
                                }
                                ...
                                for(TInt y = 0 ; y < sourceRectHeight ; y++)
                                        {
                                        //TODO: optimize: separate loops for 1,
   2 and n skip. Mem::Copy() can be used in unscaled case. TUint16* bitmapPos =
   bitmapLine; /* 2 bytes per pixel */
/*					TUint16* screenMemoryLinePos =
 * screenMemory; /* 2 bytes per pixel */
/*					for(TInt x = 0 ; x < sourceRectWidth ;
x++)
                                                {
                                                __ASSERT_DEBUG(screenMemory <
(screenBuffer + Private->EPOC_ScreenSize.iWidth *
Private->EPOC_ScreenSize.iHeight), User::Panic(KLibName, KErrCorrupt));
                                                __ASSERT_DEBUG(screenMemory >=
screenBuffer, User::Panic(KLibName, KErrCorrupt));
                                                __ASSERT_DEBUG(bitmapLine <
((TUint16*)_this->screen->pixels + (_this->screen->w * _this->screen->h)),
User::Panic(KLibName, KErrCorrupt));
                                                __ASSERT_DEBUG(bitmapLine >=
(TUint16*)_this->screen->pixels, User::Panic(KLibName, KErrCorrupt));

                                                *screenMemoryLinePos++ =
*bitmapPos; bitmapPos+=skipValue;
                                                }
                                        bitmapLine += sourceScanlineLength;
                                        screenMemory += targetScanlineLength;
                                        }
        ...
        //
                default:
                        {
                        TUint8* bitmapLine = (TUint8*)_this->screen->pixels +
sourceStartOffset; TUint16* screenMemory = screenBuffer + targetStartOffset; if
(skipValue > 1) sourceRectWidth /= skipValue; #if defined __MARM_ARMI__
                        __asm volatile("
                                mov		%4, %4, lsl #1	@
targetScanLineLength is in pixels, we need it in bytes 1: mov		r6, %0
@ bitmapLine mov		r7, %2			@ screenMemory mov
r8, %6			@ sourceRectWidth 2: ldrb	r4, [%0], %7
@ r4 = *bitmapPos; bitmapPos += skipValue ldr		r5, [%1, r4, lsl #2]
@ only 16 lower bits actually used subs	r8, r8, #1 @ x-- strh	r5, [%2], #2
@ *screenMemoryLinePos++ = r4 bne		2b

                                add		%0, r6, %3		@
bitmapLine += sourceScanlineLength add		%2, r7, %4		@
screenMemory += targetScanlineLength subs    %5, %5, #1		@
sourceRectHeight-- bne		1b
                                "
                                : // no output
                                //		%0
%1							%2
%3							%4
%5							%6
%7 : "r" (bitmapLine), "r" (&EPOC_HWPalette_256_to_Screen[0]), "r"
(screenMemory), "r" (sourceScanlineLength), "r" (targetScanlineLength), "r"
(sourceRectHeight), "r" (sourceRectWidth), "r" (skipValue) : "r4", "r5", "r6",
"r7", "r8"
                        );
#else
                        for(TInt y = 0 ; y < sourceRectHeight ; y++)
                                {
                                TUint8* bitmapPos = bitmapLine; /* 1 byte per
pixel */
/*				TUint16* screenMemoryLinePos = screenMemory; /*
 * 2 bytes per pixel */
/* Convert each pixel from 256 palette to 4k color values */
/*				for (TInt x = 0 ; x < sourceRectWidth ; x++)
                                        {
...
Private->EPOC_ScreenSize.iHeight)), User::Panic(KLibName, KErrCorrupt));
                                        //__ASSERT_DEBUG(screenMemoryLinePos >=
screenBuffer, User::Panic(KLibName, KErrCorrupt));
                                        //__ASSERT_DEBUG(bitmapPos <
((TUint8*)_this->screen->pixels + (_this->screen->w * _this->screen->h)),
User::Panic(KLibName, KErrCorrupt));
                                        //__ASSERT_DEBUG(bitmapPos >=
(TUint8*)_this->screen->pixels, User::Panic(KLibName, KErrCorrupt));

                                        *screenMemoryLinePos++ =
EPOC_HWPalette_256_to_Screen[*bitmapPos]; bitmapPos += skipValue;
                                        }
                                bitmapLine += sourceScanlineLength;
                                screenMemory += targetScanlineLength;
                                }
...
void DirectDrawRotated(_THIS, int numrects, SDL_Rect *rects,
  ...
  for (i = 0; i < numrects; ++i) {
    ...
    TInt sourceRectWidth = maxX - rect2.x + 1;
    //        TInt sourceRectWidthInBytes = sourceRectWidth *
    //        sourceNumBytesPerPixel;
    TInt sourceRectHeight = maxY - rect2.y + 1;
    TInt sourceStartOffset = rect2.x + rect2.y * sourceScanlineLength;
    TInt targetStartOffset =
        fixedScreenOffset.iX + (targetW - 1 - rect2.y) +
        (fixedScreenOffset.iY + rect2.x) * targetScanlineLength;
    ...
    if (_this->screen->format->BitsPerPixel == 12) {
      ...
      /* !!TODO: not yet implemented
          ...
          TUint16* bitmapLine = (TUint16*)_this->screen->pixels +
      sourceStartOffset; TUint16* screenMemory = screenBuffer +
      targetStartOffset; for(TInt y = 0 ; y < sourceRectHeight ; y++) {
                          //TODO: optimize: separate loops for 1, 2 and n skip
                  //Mem::Copy(screenMemory, bitmapLine, sourceRectWidthInBytes);
          TUint16* bitmapPos = bitmapLine; // 2 bytes per pixel
          TUint16* screenMemoryLinePos = screenMemory; // 2 bytes per pixel
        ...
        if (--screenXScaleValue) {
          ...
          for (TInt x = 0; x < sourceRectWidth; x++) {
            if (--screenYScaleValue) {
              ...
              *screenMemoryLinePos =
                  TUint16(EPOC_HWPalette_256_to_Screen[*bitmapPos]);
              screenMemoryLinePos += targetScanlineLength;
              debug_xcount++;
            } else
...
WMcursor *EPOC_CreateWMCursor(_THIS, Uint8 * /*data*/, Uint8 * /*mask*/,
