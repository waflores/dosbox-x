...
public:
  ...
  inline void Copy(TUint32 * aTarget, const TUint8 *aSrc, TInt aBytes,
                   TInt aHeight) const;
...
inline void TDsa::Copy(TUint32 *aTarget, const TUint8 *aSrc, TInt aBytes,
                       TInt aHeight) const {
  iDsa.iCopyFunction(iDsa, aTarget, aSrc, aBytes, aHeight);
}

template <class T, class S>
void ClipCopy(const TDsa &iDsa, TUint8 *aTarget, const TUint8 *aSource,
              const TRect &aUpdateRect, const TRect &aSourceRect) {
  ...
  if (iDsa.IsFlip()) {
    ...
    {
      iDsa.Copy(reinterpret_cast<TUint32 *>(targetPtr),
                reinterpret_cast<const TUint8 *>(source), copyLen, height);
      source += lineMove;
...
void CDsaA::UnlockHWSurfaceRequestComplete() { PANIC(KErrNotSupported); }
...
CDsbSurface::CDsbSurface(MDsbObs &aDsb)
    : CActive(CActive::EPriorityHigh), iDsb(aDsb) {
  CActiveScheduler::Add(this);
}

CDsbSurface::~CDsbSurface() { Cancel(); }
...
void CDsbSurface::RunL() { iDsb.SurfaceReady(); }
...
CDsaB::CDsaB(RWsSession &aSession, TInt aFlags)
    : CDsa(aSession), iType(aFlags) {}
...
void CDsaB::UnlockHwSurface() { EpocSdlEnv::Request(CDsa::ERequestUpdate); }
...
void CDsaB::SurfaceReady() { SetUpdating(EFalse); }

CDirectScreenBitmap &CDsaB::Dsb() { return *iDsb; }
...
void CDsa::SetSuspend() { iStateFlags |= ESdlThreadSuspend; }

void CDsa::ReleaseStop() { iStateFlags &= ~ESdlThreadExplicitStop; }

TBool CDsa::Stopped() const { return (iStateFlags & ESdlThreadExplicitStop); }
...
void CDsa::ConstructL(RWindow &aWindow, CWsScreenDevice &aDevice) {
  ...
  iDsa = CDirectScreenAccess::NewL(iSession, aDevice, aWindow, *this);
...
TInt CDsa::AppendOverlay(MOverlay &aOverlay, TInt aPriority) {
  TInt i;
  for (i = 0; i < iOverlays.Count() && iOverlays[i].iPriority < aPriority;
       i++) {
  }
...
void CDsa::RestartL() {
  ...
  // iScreenRect = rect; //to ensure properly set, albeit may not(?) match to
  // value SDL has - therefore may has to clip targetrect shall no change
  SetTargetRect();
  RecreateL();
...
CDsa::CDsa(RWsSession &aSession) : iSession(aSession), iStateFlags(0) {
  //	CActiveScheduler::Add(this);
  iCFTable[0] = CopyMem;
...
RWsSession &CDsa::Session() { return iSession; }
...
TInt CDsa::AllocSurface(TBool aHwSurface, const TSize &aSize,
                        TDisplayMode aMode) {
...
/*
void SaveBmp(const TDesC& aName, const TAny* aData, TInt aLength, const TSize&
aSz, TDisplayMode aMode)
        {
...
void ClipCopy(TUint8* aTarget, const TUint8* aSource, const TRect& aRect, const
TPoint& aTargetPos)
        {
        ...
        const TInt targetStartOffset = iTargetBpp * (aRect.iTl.iX +
aTargetPos.iX); target += targetStartOffset; TUint32* targetPtr =
reinterpret_cast<TUint32*>(target); const TInt targetWidth = iScanLineWidth >>
2; const TInt height = aRect.Height();
        }
*/
/*
void CDsa::ClipCopy(TUint8* aTarget,
        ...
        targetPtr +=  (aSourceRect.iTl.iY + aUpdateRect.iTl.iY ) *
SwSize().iWidth; const TInt targetStartOffset =  (aUpdateRect.iTl.iX +
aSourceRect.iTl.iX);
...
//	TUint32* targetPtr = reinterpret_cast<TUint32*>(target);
        ...
        if(iStateFlags & EOrientation180)
                ...
                for(TInt i = 0; i < height; i++) //source is always smaller
                        {
                        iCopyFunction(*this, targetPtr, (TUint8*)source,
copyLen, height); source += lineMove; targetPtr -= targetWidth32;
                        }
                }
        else
                {
                ...
                for(TInt i = 0; i < height; i++) //source is always smaller
                        {
                        iCopyFunction(*this, targetPtr, (TUint8*)source,
copyLen, height); source += lineMove; targetPtr += targetWidth32; // >> 2;
                        }
                }
...
void CDsa::ClipCopy(TUint8 *aTarget, const TUint8 *aSource, const TRect &aRect,
                    const TPoint &aTargetPos) const {
  TUint8 *target = aTarget;
  const TUint8 *source = aSource;
  const TInt lineWidth = aRect.Width();
  source += iSourceBpp * (aRect.iTl.iY * lineWidth);
...
void CDsa::ClipCopy(TUint8* aTarget,
        ...
        switch(iSourceBpp)
                {
                case 1:
                        ::ClipCopy<TUint32, TUint8>(dsa, aTarget, aSource,
aUpdateRect, aSourceRect); break; case 2:
                        ::ClipCopy<TUint32, TUint16>(dsa, aTarget, aSource,
aUpdateRect, aSourceRect); break; case 4:
                        ::ClipCopy<TUint32, TUint32>(dsa, aTarget, aSource,
aUpdateRect, aSourceRect); break;
                }
        }
...
void CDsa::SetCopyFunction() {
  // calculate offset to correct function in iCFTable according to given
  // parameters
  TInt function = 0;
...
*/
/*
TBool CDsa::AddUpdateRect(const TUint8* aBits, const TRect& aUpdateRect, const
TRect& aRect)
        {
...
//	TPoint move(0, 0);
        ...
        if(iSourceMode != DisplayMode() ||  targetRect != sourceRect ||
targetRect != updateRect || ((iStateFlags & EOrientationFlags) != 0))
                {
                sourceRect.Intersection(targetRect); //so source always smaller
or equal than target
                ...
                {
                const TInt byteCount = aRect.Width() * aRect.Height() *
iSourceBpp; //this could be stored Mem::Copy(target, aBits, byteCount);
                }
...
TBool CDsa::AddUpdateRect(const TUint8 *aBits, const TRect &aUpdateRect,
                          const TRect &aRect) {
  ...
  if (iSourceMode != DisplayMode() || targetRect != sourceRect ||
    ...
    sourceRect.Intersection(
        targetRect); // so source always smaller or equal than target
    updateRect.Intersection(targetRect);
    ClipCopy(target, aBits, updateRect, sourceRect.iTl);
  } else {
    const TInt byteCount =
        aRect.Width() * aRect.Height() * iSourceBpp; // this could be stored
...
void CDsa::UpdateSwSurface() {
  iTargetAddr = NULL;
  UnlockHwSurface(); // could be faster if does not use AO, but only check
                     // status before redraw, then no context switch needed
}
...
void CDsa::SetBlitter(MBlitter *aBlitter) { iBlitter = aBlitter; }
...
void CDsa::SetTargetRect() {
  ...
  if (iStateFlags & EResizeRequest &&
    ...
    rect.Move((asz.iWidth - rect.Size().iWidth) >> 1,
              (asz.iHeight - rect.Size().iHeight) >> 1);
...
void CDsa::Copy256(const CDsa &aDsa, TUint32 *aTarget, const TUint8 *aSource,
                   TInt aBytes, TInt) {
...
void CDsa::Copy256Reversed(const CDsa &aDsa, TUint32 *aTarget,
                           const TUint8 *aSource, TInt aBytes, TInt) {
...
void CDsa::Copy256Flip(const CDsa &aDsa, TUint32 *aTarget,
                       const TUint8 *aSource, TInt aBytes, TInt aLineLen) {
  TUint32 *target = aTarget;
  const TUint32 *endt = target + aBytes;
...
void CDsa::Copy256FlipReversed(const CDsa &aDsa, TUint32 *aTarget,
                               const TUint8 *aSource, TInt aBytes,
                               TInt aLineLen) {
  const TUint32 *target = aTarget;
  TUint32 *endt = aTarget + aBytes;
  const TUint8 *column = aSource;
...
void CDsa::CopyMem(const CDsa & /*aDsa*/, TUint32 *aTarget,
                   const TUint8 *aSource, TInt aBytes, TInt) {
  const TUint32 *src = reinterpret_cast<const TUint32 *>(aSource);
  Mem::Copy(aTarget, src, aBytes << 2);
}

void CDsa::CopyMemFlip(const CDsa & /*aDsa*/, TUint32 *aTarget,
                       const TUint8 *aSource, TInt aBytes, TInt aLineLen) {
  TUint32 *target = aTarget;
  const TUint32 *endt = target + aBytes;
...
void CDsa::CopyMemReversed(const CDsa & /*aDsa*/, TUint32 *aTarget,
                           const TUint8 *aSource, TInt aBytes, TInt) {
  const TUint32 *target = aTarget;
  TUint32 *endt = aTarget + aBytes;
...
void CDsa::CopyMemFlipReversed(const CDsa & /*aDsa*/, TUint32 *aTarget,
                               const TUint8 *aSource, TInt aBytes,
                               TInt aLineLen) {
  const TUint32 *target = aTarget;
  TUint32 *endt = aTarget + aBytes;
...
public:
  virtual void Copy(TUint32 * aTarget, const TUint8 *aSource, TInt aBytes,
                    TBool aReversed) = 0;
  virtual void FlipCopy(TUint32 * aTarget, const TUint8 *aSource, TInt aBytes,
                        TInt aLineLen, TBool aReversed) = 0;
};

template <class T> NONSHARABLE_CLASS(TRgbCopy) : public MRgbCopy {
public:
  ...
  void Copy(TUint32 * aTarget, const TUint8 *aSource, TInt aBytes,
            TBool aReversed);
  void FlipCopy(TUint32 * aTarget, const TUint8 *aSource, TInt aBytes,
                TInt aLineLen, TBool aReversed);
...
template <class T> TRgbCopy<T>::TRgbCopy(TDisplayMode aMode) {
  switch (aMode) {
  case EGray256:
    iFunc = (TRgbFunc)Gray256;
    break;
  case EColor256:
    iFunc = (TRgbFunc)Color256;
    break;
  case EColor4K:
    iFunc = (TRgbFunc)Color4K;
    break;
  case EColor64K:
    iFunc = (TRgbFunc)Color64K;
    break;
  case EColor16M:
    iFunc = (TRgbFunc)Color16M;
    break;
  case EColor16MU:
    iFunc = (TRgbFunc)Color16MU;
    break;
  case EColor16MA:
    iFunc = (TRgbFunc)Color16MA;
    break;
  default:
...
template <class T>
void TRgbCopy<T>::FlipCopy(TUint32 *aTarget, const TUint8 *aSource, TInt aBytes,
                           TInt aLineLen, TBool aReversed) {
...
void CDsa::CopySlowFlipReversed(const CDsa &aDsa, TUint32 *aTarget,
                                const TUint8 *aSource, TInt aBytes,
                                TInt aLineLen) {
  TStackMem mem = 0;
  GetCopy(&mem, aDsa.iSourceMode)
      ->FlipCopy(aTarget, aSource, aBytes, aLineLen, ETrue);
}

void CDsa::CopySlowFlip(const CDsa &aDsa, TUint32 *aTarget,
                        const TUint8 *aSource, TInt aBytes, TInt aLineLen) {
  TStackMem mem = 0;
  GetCopy(&mem, aDsa.iSourceMode)
      ->FlipCopy(aTarget, aSource, aBytes, aLineLen, EFalse);
}

void CDsa::CopySlow(const CDsa &aDsa, TUint32 *aTarget, const TUint8 *aSource,
                    TInt aBytes, TInt) {
  TStackMem mem = 0;
  GetCopy(&mem, aDsa.iSourceMode)->Copy(aTarget, aSource, aBytes, EFalse);
}

void CDsa::CopySlowReversed(const CDsa &aDsa, TUint32 *aTarget,
                            const TUint8 *aSource, TInt aBytes, TInt) {
  TStackMem mem = 0;
