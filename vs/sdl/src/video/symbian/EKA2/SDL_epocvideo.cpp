...
VideoBootStrap EPOC_bootstrap = {"epoc\0\0\0", "EPOC system", EPOC_Available,
                                 EPOC_CreateDevice};
...
/*
void AllocHWSurfaceL(CFbsBitmap*& aBitmap, const TDisplayMode& aMode, const
TSize& aSize)
        {
        ...
        //...if it fails - should we use wsbitmaps???
                {//the good reason to use hw bitmaps is that they wont need lock
heap PANIC_IF_ERROR(aBitmap->Create(aSize, aMode));
                }
        }
...
&& TDisplayModeUtils::NumDisplayModeBitsPerPixel(TDisplayMode(dmode)) !=
        ...
        __ASSERT_ALWAYS(TDisplayModeUtils::IsDisplayModeColor(TDisplayMode(dmode)),
PANIC(KErrNotSupported)); TRAPD(err, AllocHWSurfaceL(Private->iFrame,
TDisplayMode(dmode), TSize(surface->w, surface->h))); return err == KErrNone ? 0
: -1;
    }
*/
...
TDisplayMode GetDisplayMode(TInt aBitsPerPixel) {
  const TDisplayMode displayMode = EpocSdlEnv::DisplayMode();
  TInt dmode = EColorLast;
  if (TDisplayModeUtils::NumDisplayModeBitsPerPixel(displayMode) ==
      aBitsPerPixel) {
    dmode = displayMode;
  } else {
    --dmode;
...
static int EPOC_AllocHWSurface(_THIS, SDL_Surface *surface) {
  return KErrNone == EpocSdlEnv::AllocHwSurface(
                         TSize(surface->w, surface->h),
                         GetDisplayMode(surface->format->BitsPerPixel));
}
...
int EPOC_ShowWMCursor(_THIS, WMcursor *cursor) { return true; }
