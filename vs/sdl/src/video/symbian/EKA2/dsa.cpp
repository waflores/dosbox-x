...
template <class T> NONSHARABLE_CLASS(CBitmapSurface) : public T {
public:
...
template <class T> void CBitmapSurface<T>::Free() {
  delete iBmp;
  iBmp = NULL;
  T::Free();
}

template <class T> CBitmapSurface<T>::~CBitmapSurface() {
  __ASSERT_DEBUG(iBmp == NULL, PANIC(KErrNotReady));
  delete iCopyBmp;
}

template <class T> TUint8 *CBitmapSurface<T>::LockSurface() {
  iBmp->LockHeap();
  return reinterpret_cast<TUint8 *>(iBmp->DataAddress());
}

template <class T> void CBitmapSurface<T>::UnlockHwSurface() {
  iBmp->UnlockHeap();
  T::SetUpdating(EFalse);
  Update(*iBmp);
}

template <class T> void CBitmapSurface<T>::Update(CFbsBitmap &aBmp) {
  if (!T::Blitter(aBmp)) {
...
template <class T> void CBitmapSurface<T>::CreateSurfaceL() {
  Free();
...
template <class T> TInt CBitmapSurface<T>::ExternalUpdate() {
...
void CDsaBitgdi::CompleteUpdate() { EpocSdlEnv::Request(CDsa::ERequestUpdate); }
...
void CDsaBitgdi::Resume() { Start(); }

CBitmapContext &CDsaBitgdi::Gc() { return *iGc; }
...
inline CDirectScreenAccess &CDsaBase::Dsa() const { return *iDsa; }

CDsaBase::CDsaBase(RWsSession &aSession) : CDsa(aSession) {}

CBitmapContext &CDsaBase::Gc() { return *Dsa().Gc(); }
...
void CDsaA::CompleteUpdate() { iDsa->ScreenDevice()->Update(); }
...
NONSHARABLE_CLASS(CDsaB) : public CDsaBase, public MDsbObs {
public:
...
CDsaB::CDsaB(RWsSession &aSession, TInt aFlags)
    : CDsaBase(aSession), iType(aFlags) {}
...
CDsa::CDsa(RWsSession &aSession)
    : iStateFlags(0), iSession(aSession)
...
void CDsa::ClipCopy(TUint8 *aTarget, const TUint8 *aSource,
                    const TRect &aUpdateRect, const TRect &aSourceRect) const {
  const TDsa dsa(*this);
  switch (iSourceBpp) {
  case 1:
    ::ClipCopy<TUint32, TUint8>(dsa, aTarget, aSource, aUpdateRect,
                                aSourceRect);
    break;
  case 2:
    ::ClipCopy<TUint32, TUint16>(dsa, aTarget, aSource, aUpdateRect,
                                 aSourceRect);
    break;
  case 4:
    ::ClipCopy<TUint32, TUint32>(dsa, aTarget, aSource, aUpdateRect,
                                 aSourceRect);
    break;
  }
}
...
void CDsa::Stop() { iStateFlags &= ~ERunning; }
