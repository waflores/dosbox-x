...
CDsaA::~CDsaA() { __ASSERT_DEBUG(iBmp == NULL, PANIC(KErrNotReady)); }
...
void CDsa::RestartL() {
  ...
  iScreenRect = rect; // to ensure properly set, albeit may not(?) match to
                      // value SDL has - therefore may has to clip
...
LOCAL_C TRgb rgb16MA(TInt aValue) { return TRgb::Color16MA(aValue); }
