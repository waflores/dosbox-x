  ...
  if(KErrNone == fs.Connect())
      ...
      if(err == KErrNone && array.Count() > 0)
          ...
          for(TInt k = 0; k < array.Count(); k+= 2)
              ...
              if(oldval >=  0 && oldval < MAX_SCANCODE && newval >=  0 && newval
  < MAX_SCANCODE)
                  {
...
int EPOC_HandleWsEvent(_THIS, const TWsEvent &aWsEvent) {
  ...
  case EEventPointer: /* Mouse pointer events */
  {
    /*        const TPointerCursorMode mode = EpocSdlEnv::PointerMode();
            ...
            if(mode == EPointerCursorNone)
                {
                return 0; //TODO: Find out why events are get despite of cursor
       should be off
                }
    */
    const TPointerEvent *pointerEvent = aWsEvent.Pointer();
    const TPoint mousePos =
        EpocSdlEnv::WindowCoordinates(pointerEvent->iPosition);

    /*!! TODO Pointer do not yet work properly
    //SDL_TRACE1("SDL: EPOC_HandleWsEvent, pointerEvent->iType=%d",
    pointerEvent->iType); //!!
    ...
    */
    ...
    posted += SDL_PrivateMouseMotion(
        0, 0, mousePos.iX, mousePos.iY); /* Absolute position on screen */
  ...
  case EEventKeyDown: /* Key events */
  {
    ...
    if (aWsEvent.Key()->iScanCode == EStdKeyDeviceA) {
      const TPointerCursorMode mode =
          Private->EPOC_WsSession.PointerCursorMode();
      if (mode != EPointerCursorNone)
        posted += SDL_PrivateMouseButton(SDL_PRESSED, SDL_BUTTON_LEFT, 0, 0);
    }
#endif
  ...
  case EEventKeyUp: /* Key events */
  {
    ...
    posted += SDL_PrivateKeyboard(
        SDL_RELEASED, TranslateKey(_this, aWsEvent.Key()->iScanCode, &keysym));
    break;
  }
...
static SDL_keysym *TranslateKey(_THIS, int scancode, SDL_keysym *keysym) {
  ...
  if (scancode >= ENonCharacterKeyBase) {
    // Non character keys
    keysym->sym =
        keymap[scancode - ENonCharacterKeyBase + 0x0081]; // !!hard coded
  } else {
    keysym->sym = keymap[scancode];
  }
  ...
  /*
     ...
     CFbsBitGc::EGraphicsOrientationRotated270) { switch(keysym->sym) { case
     SDLK_UP:	keysym->sym = SDLK_LEFT;  break; case SDLK_DOWN: keysym->sym =
     SDLK_RIGHT; break; case SDLK_LEFT: keysym->sym = SDLK_DOWN;  break; case
     SDLK_RIGHT:keysym->sym = SDLK_UP;    break;
                  }
          }
  */
  ...
  //!!
  // sprintf(debug, "SDL: TranslateKey: keysym->scancode=%d, keysym->sym=%d,
  // keysym->mod=%d",
