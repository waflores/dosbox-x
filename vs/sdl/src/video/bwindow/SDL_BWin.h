...
public:
  ...
  virtual bool Shown(void) { return (shown); }
  /* If called, the next resize event will not be forwarded to SDL. */
  virtual void InhibitResize(void) { inhibit_resize = true; }
  ...
  virtual void SetBitmap(BBitmap *bitmap) { SDL_View->SetBitmap(bitmap); }
  ...
  virtual bool BeginDraw(void) { return (Lock()); }
  virtual void DrawAsync(BRect updateRect) { SDL_View->DrawAsync(updateRect); }
...
#endif
  virtual BView *View(void) { return (the_view); }
  ...
  virtual int16 Translate2Unicode(const char *buf) {
    ...
    if ((uchar)buf[0] > 127) {
      ...
      convert_from_utf8(B_UNICODE_CONVERSION, buf, &srclen, (char *)destbuf,
                        &dstlen, &state);
      unicode = destbuf[0];
      unicode <<= 8;
      unicode |= destbuf[1];
