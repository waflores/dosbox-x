...
static int VGL_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors);
...
static int VGL_Available(void) {
  ...
  if (console >= 0) {
    ...
    if ((fstat(console, &sb) < 0) || (ioctl(console, VT_GETMODE, &dummy) < 0)) {
      console = -1;
    }
  }
  ...
  for (i = 0; modes[i] != NULL; i++) {
    if ((modes[i]->ModeInfo.Xsize > 320) && (modes[i]->ModeInfo.Ysize > 200) &&
...
VideoBootStrap VGL_bootstrap = {"vgl", "FreeBSD libVGL", VGL_Available,
                                VGL_CreateDevice};
...
SDL_Surface *VGL_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  int mode_found;
  int i;
  VGLMode **modes;
  ...
  if (flags & SDL_DOUBLEBUF) {
    if (VGLCurMode->ModeInfo.Xsize * 2 <= VGLCurMode->ModeInfo.VYsize) {
      ...
      flip_address[1] = (byte *)current->pixels + current->h * current->pitch;
...
static int VGL_AllocHWSurface(_THIS, SDL_Surface *surface) { return -1; }
static void VGL_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void VGL_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) { return; }

int VGL_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  int i;

  for (i = 0; i < ncolors; i++) {
    VGLSetPaletteIndex(firstcolor + i, colors[i].r >> 2, colors[i].g >> 2,
                       colors[i].b >> 2);
  }
  return 1;
}
...
static VGLMode **VGLListModes(int depth, int mem_model) {
  ...
  for (modenum = 0; modenum < M_VESA_MODE_MAX; modenum++) {
    ...
    (defined(__DragonFly__) || __FreeBSD_version >= 500000)
