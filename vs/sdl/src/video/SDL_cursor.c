...
SDL_Cursor *SDL_CreateCursor(Uint8 *data, Uint8 *mask, int w, int h, int hot_x,
                             int hot_y) {
  ...
  if (video->CreateWMCursor) {
    cursor->wm_cursor =
        video->CreateWMCursor(video, data, mask, w, h, hot_x, hot_y);
  } else {
    cursor->wm_cursor = NULL;
  }
  return (cursor);
...
SDL_Cursor *SDL_GetCursor(void) { return (SDL_cursor); }
...
void SDL_CursorPaletteChanged(void) { palette_changed = 1; }
