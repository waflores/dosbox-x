...
VideoBootStrap DGA_bootstrap = {"dga", "XFree86 DGA 2.0", DGA_Available,
                                DGA_CreateDevice};
...
static void PrintMode(SDL_NAME(XDGAMode) * mode) {
  printf("Mode: %s (%dx%d) at %d bpp (%f refresh, %d pitch) num: %d\n",
         mode->name, mode->viewportWidth, mode->viewportHeight,
  ...
  printf("\tRGB: 0x%8.8x 0x%8.8x 0x%8.8x (%d - %s)\n", mode->redMask,
         mode->greenMask, mode->blueMask, mode->visualClass,
         mode->visualClass == TrueColor     ? "truecolor"
         : mode->visualClass == DirectColor ? "directcolor"
         : mode->visualClass == PseudoColor ? "pseudocolor"
                                            : "unknown");
  printf("\tFlags: ");
...
static int DGA_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  for (i = 0; i < num_modes; ++i) {
    if (((modes[i].visualClass == PseudoColor) ||
      ...
      PrintMode(&modes[i]);
#endif
      DGA_AddMode(this, modes[i].bitsPerPixel, modes[i].viewportWidth,
...
SDL_Surface *DGA_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  SDL_NAME(XDGAMode) * modes;
  int i, num_modes;
  ...
  for (i = 0; i < num_modes; ++i) {
    ...
    if ((depth == bpp) && (modes[i].viewportWidth == width) &&
  ...
  if (!SDL_ReallocFormat(current, mode->mode.bitsPerPixel, mode->mode.redMask,
  ...
  if ((DGA_visualClass == PseudoColor) || (DGA_visualClass == DirectColor)) {
    DGA_colormap =
        SDL_NAME(XDGACreateColormap)(DGA_Display, DGA_Screen, mode, AllocAll);
    ...
    DGA_colormap =
        SDL_NAME(XDGACreateColormap)(DGA_Display, DGA_Screen, mode, AllocNone);
  }
...
static int HWAccelBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
  ...
  if ((src->flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY) {
    SDL_NAME(XDGACopyTransparentArea)(DGA_Display, DGA_Screen, srcx, srcy, w, h,
                                      dstx, dsty, src->format->colorkey);
  } else {
    SDL_NAME(XDGACopyArea)(DGA_Display, DGA_Screen, srcx, srcy, w, h, dstx,
                           dsty);
  }
  if (!(this->screen->flags & SDL_DOUBLEBUF)) {
    XFlush(DGA_Display);
