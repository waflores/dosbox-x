...
static void BlitRGBtoRGBPixelAlphaMMX(SDL_BlitInfo *info) {
  ...
  __asm__ __volatile__("movd %0, %%mm5"
                       :
...
static void BlitRGBtoRGBSurfaceAlphaMMX(SDL_BlitInfo *info) {
  ...
  if (alpha == 128 && (df->Rmask | df->Gmask | df->Bmask) == 0x00FFFFFF) {
    ...
    mm_alpha = _mm_set_pi32(
        0, amult & chanmask); /* 0000AAAA -> mm_alpha, minus 1 chan */
    mm_alpha = _mm_unpacklo_pi8(
        mm_alpha, mm_zero); /* 0A0A0A0A -> mm_alpha, minus 1 chan */
    ...
    while (height--) {
      ...
      for (n >>= 1; n > 0; --n) {
        ...
        dst1 = _mm_packs_pu16(
            dst1, dst2); /* 0A0R0G0B(res1), 0A0R0G0B(res2) -> dst1(ARGBARGB) */
        dst1 = _mm_or_si64(dst1, dsta); /* dsta | dst1 -> dst1 */
    ...
    DUFFS_LOOP4(
        {
          ...
          if (alpha == 0) {
            ...
            *dstp = (*srcp & chanmask) | (*dstp & ~chanmask);
            ...
            mm_alpha =
                _mm_cvtsi32_si64(alpha); /* alpha -> mm_alpha (0000000A) */
            mm_alpha = _mm_srli_si64(
                mm_alpha,
                ashift); /* mm_alpha >> ashift -> mm_alpha(0000000A) */
            mm_alpha = _mm_unpacklo_pi16(mm_alpha,
                                         mm_alpha); /* 00000A0A -> mm_alpha */
            mm_alpha = _mm_unpacklo_pi32(mm_alpha,
                                         mm_alpha); /* 0A0A0A0A -> mm_alpha */
            mm_alpha = _mm_and_si64(
                mm_alpha,
                dmask); /* 000A0A0A -> mm_alpha, preserve dst alpha on add */

            /* blend */
            src1 = _mm_sub_pi16(src1, dst1); /* src1 - dst1 -> src1 */
            src1 = _mm_mullo_pi16(src1,
                                  mm_alpha); /* (src1 - dst1) * alpha -> src1 */
...
/* the permuation vector that takes the high bytes out of all the appropriate
   shorts (vector unsigned char)( 0x00, 0x10, 0x02, 0x12, 0x04, 0x14, 0x06,
   0x16, 0x08, 0x18, 0x0A, 0x1A, 0x0C, 0x1C, 0x0E, 0x1E );
*/
#define VEC_MERGE_PERMUTE()                                                    \
  (vec_add(vec_lvsl(0, (int *)NULL), (vector unsigned char)vec_splat_u16(0x0F)))
#define VEC_U32_24() (vec_add(vec_splat_u32(12), vec_splat_u32(12)))
#define VEC_ALPHA_MASK()                                                       \
  ((vector unsigned char)vec_sl((vector unsigned int)vec_splat_s8(-1),         \
                                VEC_U32_24()))
...
static void Blit32to565PixelAlphaAltivec(SDL_BlitInfo *info) {
  ...
  */
  ...
  vector unsigned char vredalpha2 = (vector unsigned char)(vec_add(
      (vector unsigned int)vredalpha1, vec_sl(v8_32, v16_32)));
  /*
  ...
  */
  ...
  vector unsigned char vgreen2 = (vector unsigned char)(vec_add(
      (vector unsigned int)vgreen1, vec_sl(v8_32, v8_32)));
...
#define ONE_PIXEL_BLEND(condition, widthvar)                                   \
  while (condition) {                                                          \
    ...
    while (width) {
      ...
      vG = vec_sl(vB, v2_16);
      vdst1 = (vector unsigned char)vec_perm(
          (vector unsigned char)vR, (vector unsigned char)vR, vredalpha1);
      vdst1 = vec_perm(vdst1, (vector unsigned char)vB, vblue1);
      vdst1 = vec_perm(vdst1, (vector unsigned char)vG, vgreen1);
      vdst2 = (vector unsigned char)vec_perm(
          (vector unsigned char)vR, (vector unsigned char)vR, vredalpha2);
      ...
      vpixel = (vector unsigned short)vec_packpx((vector unsigned int)vdst1,
                                                 (vector unsigned int)vdst2);
  ...
  while (condition) {                                                          \
    ...
    if (width > 0) {
      ...
      while (width) {
        ...
        vsel =
            (vector unsigned char)vec_cmpeq((vector unsigned int)vsel, vckey);
    ...
    DUFFS_LOOP4(
        {
          ...
          if (alpha == 0) {
            ...
            *dstp = (*srcp & chanmask) | (*dstp & ~chanmask);
            ...
            src1 = _mm_mullo_pi16(src1,
                                  mm_alpha); /* (src - dst) * alpha -> src1 */
            src1 = _mm_srli_pi16(src1, 8);   /* src1 >> 8 -> src1(000R0G0B) */
            dst1 = _mm_add_pi8(src1,
                               dst1); /* src1 + dst1(dst) -> dst1(0A0R0G0B) */
            dst1 = _mm_packs_pu16(dst1, mm_zero); /* 0000ARGB -> dst1 */
    ...
    while (height--) {
      DUFFS_LOOP_QUATRO2(
          ...
          },
          {
            ...
            src2 = src1;
            src2 = _mm_srli_pi16(
                src2, 11); /* src2 >> 11 -> src2 [000r 000r 000r 000r] */

            dst2 = dst1;
            dst2 = _mm_srli_pi16(
                dst2, 11); /* dst2 >> 11 -> dst2 [000r 000r 000r 000r] */
            ...
            src2 = src1;
            src2 = _mm_and_si64(
                src2, bmask); /* src & MASKBLUE -> src2[000b 000b 000b 000b] */

            dst2 = dst1;
            dst2 = _mm_and_si64(
                dst2, bmask); /* dst & MASKBLUE -> dst2[000b 000b 000b 000b] */
            ...
            mm_res =
                _mm_or_si64(mm_res, dst2); /* RED | GREEN | BLUE -> mm_res */
    ...
    DUFFS_LOOP4(
        {
          ...
          if (alpha) {
            if (alpha == (SDL_ALPHA_OPAQUE >> 3)) {
              *dstp = (Uint16)((s >> 8 & 0xf800) + (s >> 5 & 0x7e0) +
                               (s >> 3 & 0x1f));
            } else {
              Uint32 d = *dstp;
              /*
    ...
    DUFFS_LOOP4(
        {
          ...
          if (alpha) {
            if (alpha == (SDL_ALPHA_OPAQUE >> 3)) {
              *dstp = (Uint16)((s >> 9 & 0x7c00) + (s >> 6 & 0x3e0) +
                               (s >> 3 & 0x1f));
            } else {
              Uint32 d = *dstp;
              /*
...
SDL_loblit SDL_CalculateAlphaBlit(SDL_Surface *surface, int blit_index) {
  ...
  if (sf->Amask == 0) {
    if ((surface->flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY) {
      ...
      case 4:
        if (sf->Rmask == df->Rmask && sf->Gmask == df->Gmask &&
            sf->Bmask == df->Bmask && sf->BytesPerPixel == 4) {
#if MMX_ASMBLIT
          if (sf->Rshift % 8 == 0 && sf->Gshift % 8 == 0 &&
...
#if SDL_ALTIVEC_BLITTERS
      if (sf->BytesPerPixel == 4 &&
          !(surface->map->dst->flags & SDL_HWSURFACE) && df->Gmask == 0x7e0 &&
