...
static struct private_yuvhwfuncs gs_yuvfuncs = {
    GS_LockYUVOverlay, GS_UnlockYUVOverlay, GS_DisplayYUVOverlay,
    GS_FreeYUVOverlay};
...
SDL_Overlay *GS_CreateYUVOverlay(_THIS, int width, int height, Uint32 format,
                                 SDL_Surface *display) {
  SDL_Overlay *overlay;
  ...
  */
  ...
  map_offset =
      (mapped_len + (sysconf(_SC_PAGESIZE) - 1)) & ~(sysconf(_SC_PAGESIZE) - 1);
  hwdata->dma_len =
      hwdata->macroblocks * (16 * 16 + 8 * 8 + 8 * 8) + width * height * bpp +
      hwdata->macroblocks * (16 * sizeof(long long)) + 12 * sizeof(long long);
  hwdata->dma_mem = mmap(0, hwdata->dma_len, PROT_READ | PROT_WRITE, MAP_SHARED,
                         memory_fd, map_offset);
  ...
  hwdata->ipu_omem =
      hwdata->ipu_imem + hwdata->macroblocks * (16 * 16 + 8 * 8 + 8 * 8);
  ...
  for (h = height / 16; h; --h) {
    ...
    for (w = width / 16; w; --w) {
      ...
      tags[4] = ((unsigned long long)x << 32) | ((unsigned long long)y << 48);
      tags[5] = PS2_GS_TRXPOS;
      tags[6] = (unsigned long long)16 | ((unsigned long long)16 << 32);
  ...
  tags[2] = ((screen_image.y + screen_image.h) * screen_image.w) / 64 +
            ((unsigned long long)fbw << 14) + ((unsigned long long)psm << 20) +
  ...
  tags[2] = 6 + (1 << 4) + (1 << 8);
  tags[3] = PS2_GS_PRIM;
  tags[4] = ((unsigned long long)0 * 16) + (((unsigned long long)0 * 16) << 16);
  tags[5] = PS2_GS_UV;
  tags[6] = 0; /* X1, Y1 */
  tags[7] = PS2_GS_XYZ2;
...
int GS_LockYUVOverlay(_THIS, SDL_Overlay *overlay) { return (0); }

void GS_UnlockYUVOverlay(_THIS, SDL_Overlay *overlay) { return; }

int GS_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src,
                         SDL_Rect *dst) {
  struct private_yuvhwdata *hwdata;
  __u32 cmd;
  struct ps2_packet packet;
  ...
  packet.len = overlay->w * overlay->h * this->screen->format->BytesPerPixel;
