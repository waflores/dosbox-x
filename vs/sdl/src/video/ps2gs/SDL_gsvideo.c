...
VideoBootStrap PS2GS_bootstrap = {"ps2gs", "PlayStation 2 Graphics Synthesizer",
...
static unsigned long long head_tags[]
    __attribute__((aligned(16))) = {4 | (1LL << 60), /* GIFtag */
                                    0x0e,            /* A+D */
                                    0,               /* 2 */
                                    PS2_GS_BITBLTBUF,
                                    0, /* 4 */
...
static inline int loadimage_nonblock(int fd, struct ps2_image *image, int size,
  ...
  hm[6] = (unsigned long long)image->w | ((unsigned long long)image->h << 32);
...
static unsigned long long tex_tags[]
    __attribute__((aligned(16))) = {3 | (1LL << 60), /* GIFtag */
                                    0x0e,            /* A+D */
                                    0,               /* 2 */
                                    PS2_GS_TEX0_1,
                                    (1 << 5) + (1 << 6),
...
int scaleimage_nonblock(int fd, unsigned long long *tm,
...
static SDL_Rect **GS_ListModes(_THIS, SDL_PixelFormat *format, Uint32 flags) {
  static SDL_Rect GS_vesa_mode_list[] = {{0, 0, 1280, 1024},
                                         {0, 0, 1024, 768},
                                         {0, 0, 800, 600},
                                         {0, 0, 640, 480}};
  static SDL_Rect *GS_vesa_modes[] = {
      &GS_vesa_mode_list[0], &GS_vesa_mode_list[1], &GS_vesa_mode_list[2],
      &GS_vesa_mode_list[3], NULL};
...
static SDL_Surface *GS_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (!SDL_ReallocFormat(current, GS_pixelmasks[vinfo.psm].bpp,
                         GS_pixelmasks[vinfo.psm].r, GS_pixelmasks[vinfo.psm].g,
  ...
  if (!mapped_mem) {
    ...
    mapped_mem =
        mmap(0, mapped_len, PROT_READ | PROT_WRITE, MAP_SHARED, memory_fd, 0);
    ...
    image_tags_mem =
        (unsigned long long *)((caddr_t)head_tags_mem + sizeof(head_tags));
    SDL_memcpy(head_tags_mem, head_tags, sizeof(head_tags));
    if (saved_vinfo.mode != PS2_GS_VESA) {
      tex_tags_mem = (unsigned long long *)((caddr_t)image_tags_mem +
                                            ((2 * MAXTAGS) * 16));
      scale_tags_mem =
          (unsigned long long *)((caddr_t)tex_tags_mem + sizeof(tex_tags));
      SDL_memcpy(tex_tags_mem, tex_tags, sizeof(tex_tags));
      tex_tags_mem[2] = (vinfo.h * vinfo.w) / 64 +
      ...
      scale_tags_mem[8] = ((unsigned long long)screen_image.w * 16) +
                          (((unsigned long long)screen_image.h * 16) << 16);
      scale_tags_mem[10] = ((unsigned long long)vinfo.w * 16) +
...
static int GS_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void GS_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
static int GS_LockHWSurface(_THIS, SDL_Surface *surface) {
...
static void GS_DMAFullUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  loadimage_nonblock(console_fd, &screen_image, screen_image_size,
