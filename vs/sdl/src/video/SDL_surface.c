...
SDL_Surface *SDL_CreateRGBSurfaceFrom(void *pixels, int width, int height,
  ...
  surface = SDL_CreateRGBSurface(SDL_SWSURFACE, 0, 0, depth, Rmask, Gmask,
                                 Bmask, Amask);
  if (surface != NULL) {
 ...
 */
static __inline__ SDL_bool SDL_IntersectRect(const SDL_Rect *A,
                                             const SDL_Rect *B,
...
int SDL_UpperBlit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                  SDL_Rect *dstrect) {
  SDL_Rect fulldst;
  int srcx, srcy, w, h;
...
SDL_Surface *SDL_ConvertSurface(SDL_Surface *surface, SDL_PixelFormat *format,
  ...
  if (format->palette && convert->format->palette) {
    SDL_memcpy(convert->format->palette->colors, format->palette->colors,
...
void SDL_FreeSurface(SDL_Surface *surface) {
  ...
  if ((surface == NULL) || (current_video && ((surface == SDL_ShadowSurface) ||
                                              (surface == SDL_VideoSurface)))) {
    return;
  }
