...
void GEM_PumpEvents(_THIS) {
  ...
  for (;;) {
    ...
    if (!GEM_fullscreen && (GEM_handle >= 0)) {
      ...
      event_mask |= MU_M1;
      mouse_event =
          ((SDL_GetAppState() & SDL_APPMOUSEFOCUS) == SDL_APPMOUSEFOCUS)
              ? MO_LEAVE
              : MO_ENTER;
    }

    resultat = evnt_multi(event_mask, 0x101, 7, maskmouseb, mouse_event, x2, y2,
                          w2, h2, 0, 0, 0, 0, 0, buffer, 10, &mousex, &mousey,
                          &mouseb, &kstate, &kc, &dummy);
    ...
    if (resultat & MU_M1) {
      if (this->input_grab == SDL_GRAB_OFF) {
        ...
        SDL_PrivateAppActive((mouse_event == MO_ENTER), SDL_APPMOUSEFOCUS);
      }
  ...
  if (GEM_refresh_name) {
    const char *window_name =
        (SDL_GetAppState() & SDL_APPACTIVE) ? GEM_title_name : GEM_icon_name;
    if (window_name) {
      wind_set(GEM_handle, WF_NAME, (short)(((unsigned long)window_name) >> 16),
...
static int do_messages(_THIS, short *message) {
  ...
  case AP_TERM:
    ...
    break;
  case WM_MOVED:
    wind_set(message[3], WF_CURRXYWH, message[4], message[5], message[6],
             message[7]);
    break;
  case WM_TOPPED:
  ...
  case WM_ICONIFY:
  case WM_ALLICONIFY:
    wind_set(message[3], WF_ICONIFY, message[4], message[5], message[6],
             message[7]);
    ...
    if (GEM_refresh_name && GEM_icon_name) {
      wind_set(GEM_handle, WF_NAME,
               ...
               (short)(((unsigned long)GEM_icon_name) & 0xffff), 0, 0);
      GEM_refresh_name = SDL_FALSE;
    }
    check_mouse_mode = 1;
    break;
  case WM_UNICONIFY:
    wind_set(message[3], WF_UNICONIFY, message[4], message[5], message[6],
             message[7]);
