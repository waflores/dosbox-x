...
static unsigned char vdi_index[256] = {0, 2,  3,  6,  4,  7,  5,  8,
...
VideoBootStrap GEM_bootstrap = {GEM_VID_DRIVER_NAME, "Atari GEM video driver",
...
int GEM_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  if (GEM_version >= 0x0410) {
    ...
    GEM_wfeatures = 0;
    errorcode = appl_getinfo(AES_WINDOW, &ap_gout[0], &ap_gout[1], &ap_gout[2],
                             &ap_gout[3]);
  ...
  if (!wind_get(DESKTOP_HANDLE, WF_WORKXYWH, &GEM_desk_x, &GEM_desk_y,
                &GEM_desk_w, &GEM_desk_h)) {
...
SDL_Surface *GEM_SetVideoMode(_THIS, SDL_Surface *current, int width,
                              int height, int bpp, Uint32 flags) {
  ...
  if (!SDL_ReallocFormat(current, VDI_bpp, VDI_redmask, VDI_greenmask,
                         VDI_bluemask, VDI_alphamask)) {
  ...
  if (flags & SDL_FULLSCREEN) {
    ...
    if ((old_win_type != GEM_win_type) || (GEM_handle < 0)) {
      ...
      if (!wind_calc(WC_BORDER, GEM_win_type, 0, 0, width, height, &x2, &y2,
                     &w2, &h2)) {
        GEM_FreeBuffers(this);
      ...
      wind_set(GEM_handle, WF_NAME,
               ...
               (short)(((unsigned long)GEM_title_name) & 0xffff), 0, 0);
      ...
      if (wind_calc(WC_BORDER, GEM_win_type, x2, y2, width, height, &x2, &y2,
                    &w2, &h2)) {
...
static int GEM_AllocHWSurface(_THIS, SDL_Surface *surface) { return -1; }

static void GEM_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

static int GEM_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }

static void GEM_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void GEM_UpdateRectsFullscreen(_THIS, int numrects, SDL_Rect *rects) {
  ...
  if (GEM_bufops & (B2S_C2P_1TO2 | B2S_C2P_1TOS)) {
    ...
    for (i = 0; i < numrects; i++) {
      ...
      SDL_Atari_C2pConvert(source, destination, x2 - x1, rects[i].h, SDL_FALSE,
...
static void GEM_UpdateRectsWindowed(_THIS, int numrects, SDL_Rect *rects) {
  ...
  if (wind_get(GEM_handle, WF_WORKXYWH, &wind_pxy[0], &wind_pxy[1],
               &wind_pxy[2], &wind_pxy[3]) == 0) {
    return;
  }
...
static int GEM_FlipHWSurfaceFullscreen(_THIS, SDL_Surface *surface) {
  ...
  if (GEM_bufops & (B2S_C2P_1TO2 | B2S_C2P_1TOS)) {
    ...
    SDL_Atari_C2pConvert(surface->pixels, destscr, surf_width, surface->h,
...
void GEM_wind_redraw(_THIS, int winhandle, short *inside) {
  ...
  if (wind_get(winhandle, WF_FIRSTXYWH, &todo[0], &todo[1], &todo[2],
               &todo[3]) != 0) {
    ...
    while (todo[2] && todo[3]) {
      ...
      if (wind_get(winhandle, WF_NEXTXYWH, &todo[0], &todo[1], &todo[2],
                   &todo[3]) == 0) {
        break;
      }
    }
...
static void refresh_window(_THIS, int winhandle, short *rect) {
  ...
  /*	if (GEM_wfeatures & (1<<WF_ICONIFY))*/ {
    if (wind_get(winhandle, WF_ICONIFY, &wind_pxy[0], &wind_pxy[1],
                 &wind_pxy[2], &wind_pxy[3]) != 0) {
      iconified = wind_pxy[0];
    }
  }

  if (wind_get(winhandle, WF_WORKXYWH, &wind_pxy[0], &wind_pxy[1], &wind_pxy[2],
               &wind_pxy[3]) == 0) {
    return;
  }
  ...
  if (iconified && GEM_icon) {
    ...
    printf("sdl:video:gem:  clip(0,0,%d,%d) to (%d,%d,%d,%d)\n", surface->w - 1,
           surface->h - 1, dst_rect[0], dst_rect[1], dst_rect[2], dst_rect[3]);
    printf("sdl:video:gem:  icon(%d,%d,%d,%d)\n", icon_rect[0], icon_rect[1],
  ...
  if (GEM_bufops & B2S_C2P_1TO2) {
    ...
    SDL_Atari_C2pConvert(src, dest, x2 - x1, rect[3] - rect[1] + 1, SDL_FALSE,
  ...
  printf("sdl:video:gem: redraw %dx%d: (%d,%d,%d,%d) to (%d,%d,%d,%d)\n",
         surface->w, surface->h, pxy[0], pxy[1], pxy[2], pxy[3], pxy[4], pxy[5],
         pxy[6], pxy[7]);
#endif
