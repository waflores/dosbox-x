...
WMcursor *GEM_CreateWMCursor(_THIS, Uint8 *data, Uint8 *mask, int w, int h,
                             int hot_x, int hot_y) {
  WMcursor *cursor;
  MFORM *new_mform;
  int i;
  ...
  if ((w > MAXCURWIDTH) || (h > MAXCURHEIGHT)) {
    SDL_SetError("Only cursors of dimension (%dx%d) are allowed", MAXCURWIDTH,
                 MAXCURHEIGHT);
    return (NULL);
  }
  ...
  for (i = 0; i < h; i++) {
    printf("sdl:video:gem: cursor: line %d: data=0x%04x, mask=0x%04x\n", i,
           new_mform->mf_data[i], new_mform->mf_mask[i]);
  }
