...
#pragma message(                                                               \
    "Warning: Using GapiEmu in release build. I assume you'd like to set USE_GAPI_EMU to zero.")
#endif
...
static SDL_Surface *GAPI_SetVideoMode(_THIS, SDL_Surface *current, int width,
...
static int GAPI_Available(void) {
  ...
  HDC hdc = GetDC(NULL);
  int result =
      ExtEscape(hdc, GETRAWFRAMEBUFFER, 0, NULL, sizeof(RawFrameBufferInfo),
                (char *)&g_RawFrameBufferInfo);
  ReleaseDC(NULL, hdc);
  g_bRawBufferAvailable = result > 0;

  // My Asus MyPAL 696 reports the RAWFRAMEBUFFER as available, but with a size
  // of 0 x 0
  if (g_RawFrameBufferInfo.cxPixels <= 0 ||
      g_RawFrameBufferInfo.cyPixels <= 0) {
...
static int GAPI_AddMode(_THIS, int bpp, int w, int h) {
  ...
  gapi->SDL_modelist[index] = (SDL_Rect **)SDL_realloc(
      gapi->SDL_modelist[index], (1 + next_mode + 1) * sizeof(SDL_Rect *));
...
VideoBootStrap GAPI_bootstrap = {GAPIVID_DRIVER_NAME, "WinCE GAPI video driver",
...
static void FillStructs(_THIS, BOOL useVga) {
  ...
  if (!useVga) {
    ...
    // buggy iPaq38xx
    if ((oemstr[12] == 'H') && (oemstr[13] == '3') && (oemstr[14] == '8') &&
        (gapi->gxProperties.cbxPitch > 0)) {
...
#endif
    {
      ...
      gxInfo.Version = 100;
      result =
          ExtEscape(hdc, GETGXINFO, 0, NULL, sizeof(gxInfo), (char *)&gxInfo);
      if (result > 0) {
  ...
  } else if ((gapi->gxProperties.cbxPitch > 0) &&
             (gapi->gxProperties.cbyPitch > 0)) {
    gapi->gapiOrientation = SDL_ORIENTATION_UP;
  } else if ((gapi->gxProperties.cbxPitch > 0) &&
             (gapi->gxProperties.cbyPitch < 0)) {
...
int GAPI_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  /* add hi-res mode */
  if (g_bRawBufferAvailable && !((gapi->gxProperties.cxWidth ==
  ...
  vformat->BitsPerPixel =
      gapi->gxProperties.cBPP < 8 ? 16 : (unsigned char)gapi->gxProperties.cBPP;
  ...
  if (gapi->gxProperties.ffFormat & kfDirect565) {
  ...
  } else if (gapi->gxProperties.ffFormat & kfDirect555) {
    vformat->BitsPerPixel = 16;
    vformat->Rmask = 0x00007c00;
    vformat->Gmask = 0x000003e0;
    vformat->Bmask = 0x0000001f;
    gapi->videoMode = GAPI_DIRECT_555;
  } else if ((gapi->gxProperties.ffFormat & kfDirect) &&
             (gapi->gxProperties.cBPP < 8)) {
    ...
    gapi->colorscale =
        gapi->gxProperties.cBPP < 8 ? 8 - gapi->gxProperties.cBPP : 0;
    gapi->videoMode = GAPI_MONO;
  } else if (gapi->gxProperties.ffFormat & kfPalette) {
...
SDL_Surface *GAPI_SetVideoMode(_THIS, SDL_Surface *current, int width,
                               int height, int bpp, Uint32 flags) {
  SDL_Surface *video;
  ...
  if (bpp == current->format->BitsPerPixel) {
    ...
    video =
        SDL_CreateRGBSurface(SDL_SWSURFACE, 0, 0, bpp, Rmask, Gmask, Bmask, 0);
    if (video == NULL) {
      SDL_OutOfMemory();
      return (NULL);
    }
  ...
  video->flags =
      SDL_FULLSCREEN; /* Clear flags, GAPI supports fullscreen only */
  ...
  if (!gapi->needUpdate && !gapi->videoMem) {
    SDL_SetError("Couldn't get address of video memory, may be unsupported "
                 "device or bug");
    return (NULL);
  }
  ...
  if ((width > height) &&
      (gapi->gxProperties.cxWidth < gapi->gxProperties.cyHeight))
  ...
  case SDL_ORIENTATION_RIGHT:
    ...
    case SDL_ORIENTATION_LEFT:
      if ((gapi->videoMode == GAPI_MONO))
        ...
        gapi->startOffset =
            gapi->gxProperties.cbyPitch * (gapi->gxProperties.cyHeight - 1);
  ...
  */
  ...
  if (gapi->useVga)
    gapi->coordinateTransform =
        (4 - gapi->systemOrientation + gapi->userOrientation) % 4;
  else
...
static int GAPI_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void GAPI_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }

/* We need to wait for vertical retrace on page flipped displays */
static int GAPI_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }

static void GAPI_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }

static int updateLine8to8(_THIS, unsigned char *srcPointer,
                          unsigned char *destPointer, int width, int height,
                          int lines) {
...
static int updateLine16to16(_THIS, PIXEL *srcPointer, PIXEL *destPointer,
  ...
  if (step == 1) /* optimized blitting on most devices */
  {
    ...
    if ((gapi->gapiOrientation != SDL_ORIENTATION_UP) &&
        (gapi->userOrientation ==
         SDL_ORIENTATION_UP)) // iPaq 3660/3800 and user orientation up
    {
...
static int updateLine16to4(_THIS, PIXEL *srcPointer, unsigned char *destPointer,
                           int width, int height, int lines, int yNibble,
                           int xNibble) {
  PIXEL *line1, *line2;
  int step = gapi->dstPixelStep;
...
static void GAPI_UpdateRectsMono(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; i++) {
    ...
    if (gapi->userOrientation == SDL_ORIENTATION_UP)
      destPointer = (unsigned char *)gapi->videoMem + gapi->startOffset -
    ...
    srcPointer = ((unsigned char *)SDL_VideoSurface->pixels) +
    ...
    while (height > 0) {
      ...
      case 4:
        linesProcessed =
            updateLine16to4(this, (PIXEL *)srcPointer, destPointer, rects[i].w,
...
static void GAPI_UpdateRectsColor(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; i++) {
    ...
    unsigned char *srcPointer = ((unsigned char *)SDL_VideoSurface->pixels) +
                                rects[i].y * SDL_VideoSurface->pitch +
                                rects[i].x * bytesPerPixel;
    height = rects[i].h;

    //		fprintf(stderr, "Starting rect %dx%d, dst=0x%x, w = %d, h =
    ...
    while (height > 0) {
      ...
      case 1:
        linesProcessed =
            updateLine8to8(this, srcPointer, (unsigned char *)destPointer,
                           rects[i].w, rects[i].h, height);
        break;
      case 2:
#pragma warning(disable : 4133)
        linesProcessed =
            updateLine16to16(this, (PIXEL *)srcPointer, destPointer, rects[i].w,
                             rects[i].h, height);
        break;
      }
      height -= linesProcessed;
...
void GAPI_VideoQuit(_THIS) {
  ...
  if (SDL_Window) {
    if ((g_hGapiLib != 0) && this && gapi && gapi->gxFunc.GXCloseDisplay &&
        !gapi->useVga)
...
static void GAPI_WinPAINT(_THIS, HDC hdc) {
  ...
  // copy data
  // FIXME: prevent misalignment, but I've never seen non aligned width of
  // screen
