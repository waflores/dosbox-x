...
struct GXKeyList {
  ...
  POINT ptUp; // x,y position of key/button.  Not on screen but in screen
              // coordinates.
  short vkDown;
  POINT ptDown;
  short vkLeft;
...
typedef int (*PFNGXSetViewport)(DWORD dwTop, DWORD dwHeight, DWORD dwReserved1,
                                DWORD dwReserved2);
...
#define kfDirect555                                                            \
  0x40 // 5 bits each for red, green and blue values in a pixel.
#define kfDirect565 0x80 // 5 red bits, 6 green bits and 5 blue bits per pixel
#define kfDirect888                                                            \
  0x100 // 8 bits each for red, green and blue values in a pixel.
...
*/
struct GapiInfo {
  /* Rotation which has to be applied to the key (arrow keys) and mouse events
   * measured in quarters of a circle counter clockwise */
  int coordinateTransform;
  char hiresFix; /* using hires mode without defining hires resource */
  int invert;    // TODO this is only written but never read, so it should be
              // removed
