...
static SDL_keysym *TranslateKey(int scancode, int modifiers, SDL_keysym *keysym,
                                int pressed);
...
#define cooperative_multitasking 1
  ...
  */
  ...
  if (SDL_GetAppState() & SDL_APPINPUTFOCUS) {
    ...
    */
    GetKeys(keys);
    if ((keysptr[0] != last_keysptr[0]) || (keysptr[1] != last_keysptr[1]) ||
        (keysptr[2] != last_keysptr[2]) || (keysptr[3] != last_keysptr[3])) {
      SDL_keysym keysym;
      int old_bit, new_bit;

#ifdef DEBUG_KEYBOARD
      fprintf(sterr, "New keys: 0x%x 0x%x 0x%x 0x%x\n", new_keys[0],
              new_keys[1], new_keys[2], new_keys[3]);
#endif
      for (i = 0; i < 128; ++i) {
...
#endif
          SDL_PrivateKeyboard(
              new_bit, TranslateKey(i, event.modifiers, &keysym, new_bit));
        }
      }
  ...
  case mouseDown: {
    ...
    case inContent:
      ...
      SDL_PrivateMouseButton(SDL_PRESSED, mouse_button, event.where.h,
                             event.where.v);
      break;
    case inGrow: {
      int newSize;
    ...
    case inZoomOut:
      if (TrackBox(win, event.where, area)) {
        ...
        SDL_PrivateResize(rect.right - rect.left, rect.bottom - rect.top);
      }
      break;
#if TARGET_API_MAC_CARBON
    case inCollapseBox:
  ...
  case mouseUp: {
     ...
     */
    SDL_PrivateMouseButton(SDL_RELEASED, mouse_button, event.where.h,
                           event.where.v);
  } break;
...
void Mac_InitOSKeymap(_THIS) {
  ...
  if (KCHRPtr) {
    ...
    for (i = 0; i < 0x7F; i++) {
      ...
      /* Now we should have an ascii value, or 0. Try to figure out to which SDL
       * symbol it maps */
...
static SDL_keysym *TranslateKey(int scancode, int modifiers, SDL_keysym *keysym,
  ...
  if (pressed && SDL_TranslateUNICODE) {
    ...
    keysym->unicode =
        KeyTranslate(keymap, keysym->scancode | modifiers, &state) & 0xFFFF;
  }
  return (keysym);
}
