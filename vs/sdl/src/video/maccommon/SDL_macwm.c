 ...
 * "Contains: Functions to enable Mac OS device gamma adjustments using 3
 * channel 256 element 8 bit gamma ramps Written by: Geoff Stahl (ggs)
 ...
 * without restriction, though the sample code has been provided "AS IS" and the
 *              responsibility for its operation is 100% yours.  However, what
 * you are not permitted to do is to redistribute the source as "DSC Sample
 * Code" after having made changes. If you're going to re-distribute the source,
 *              we require that you make it clear in the source that the code
 * was descended from Apple Sample Code, but that you've made changes." (The
 * sample code has been integrated into this file, and thus is modified from the
 * original Apple sources.)
 */
...
static Ptr CopyGammaTable(GammaTblPtr pTableGammaIn) {
  ...
  {
    dataWidth =
        (pTableGammaIn->gDataWidth + 7) / 8; /* number of bytes per entry */
    ...
    if (pTableGammaOut)
      BlockMove((Ptr)pTableGammaIn, (Ptr)pTableGammaOut,
                tableSize); /* move everything */
  }
...
static OSErr GetGammaTable(GDHandle hGD, GammaTblPtr *ppTableGammaOut) {
  ...
  *ppTableGammaOut =
      (GammaTblPtr)(DeviceGammaRec.csGTable); /* pull table out of record */
...
static Ptr GetDeviceGamma(GDHandle hGD) {
  ...
  err = GetGammaTable(
      hGD, &pTableGammaDevice); /* get a pointer to the devices table */
  if ((noErr == err) && pTableGammaDevice) /* if succesful */
    pTableGammaReturn =
        (GammaTblPtr)CopyGammaTable(pTableGammaDevice); /* copy to global */
  ...
  if (ppSystemGammas) {
    ...
    if (pSysGammaIn) {
      ...
      for (i = 0; i < pSysGammaIn->numDevices; i++) /* for all devices */
        ...
        {
          DisposeGammaTable((Ptr)pSysGammaIn->devGamma[i]
                                ->pDeviceGamma);     /* dump gamma table */
          DisposePtr((Ptr)pSysGammaIn->devGamma[i]); /* dump device info */
        }
      DisposePtr((Ptr)pSysGammaIn
                     ->devGamma);   /* dump device pointer array		 */
...
static Boolean GetDeviceGammaRampGD(GDHandle hGD, Ptr pRamp) {
  ...
  {
    err = GetGammaTable(
        hGD, &pTableGammaTemp); /* get a pointer to the current gamma */
    if ((noErr == err) && pTableGammaTemp) /* if successful */
    {
      /* fill ramp */
      unsigned char *pEntry = (unsigned char *)&pTableGammaTemp->gFormulaData +
                              pTableGammaTemp->gFormulaSize; /* base of table */
      short bytesPerEntry = (pTableGammaTemp->gDataWidth + 7) /
                            8; /* size, in bytes, of the device table entries */
      short shiftRightValue = pTableGammaTemp->gDataWidth -
                              8; /* number of right shifts device -> ramp */
      short channels = pTableGammaTemp->gChanCnt;
      short entries = pTableGammaTemp->gDataCnt;
      if (3 == channels) /* RGB format */
      { /* note, this will create runs of entries if dest. is bigger (not linear
           interpolate) */
        for (indexChan = 0; indexChan < channels; indexChan++)
          for (indexEntry = 0; indexEntry < 256; indexEntry++)
            *((unsigned char *)pRamp + (indexChan * 256) + indexEntry) =
                *(pEntry + indexChan * entries * bytesPerEntry +
                  indexEntry * entries * bytesPerEntry / 256) >>
                shiftRightValue;
      } else /* single channel format */
      {
        for (indexChan = 0; indexChan < 768;
             indexChan +=
             256) /* repeat for all 3 channels (step by ramp size) */
          for (indexEntry = 0; indexEntry < 256;
               indexEntry++) /* for all entries set vramp value */
            *((unsigned char *)pRamp + indexChan + indexEntry) =
                *(pEntry + indexEntry * entries * bytesPerEntry / 256) >>
                shiftRightValue;
      }
      return true;
    }
...
static Ptr GetSystemGammas(void) {
  ...
  pSysGammaOut = (precSystemGamma)NewPtr(
      sizeof(recSystemGamma)); /* allocate for structure */
  ...
  pSysGammaOut->devGamma = (precDeviceGamma *)NewPtr(
      sizeof(precDeviceGamma) *
      devCount); /* allocate for array of pointers to device records */
  if (pSysGammaOut) {
    ...
    do {
      pSysGammaOut->devGamma[devCount] = (precDeviceGamma)NewPtr(
          sizeof(recDeviceGamma)); /* new device record */
      if (pSysGammaOut
              ->devGamma[devCount]) /* if we actually allocated memory */
      {
        pSysGammaOut->devGamma[devCount]->hGD = hGDevice; /* stuff handle */
        pSysGammaOut->devGamma[devCount]->pDeviceGamma =
            (GammaTblPtr)GetDeviceGamma(hGDevice); /* copy gamma table */
  ...
  if (!fail)                  /* if we did not fail */
    ...
    DisposeSystemGammas(
        (Ptr *)&pSysGammaOut); /* otherwise dump the current structures (dispose
                                  does error checking) */
...
static void RestoreDeviceGamma(GDHandle hGD, Ptr pGammaTable) {
                    ...
                    */
  {
    ...
    err =
        Control((**hGD).gdRefNum, cscSetGamma, (Ptr)&csPtr); /* restore gamma */

    if ((noErr == err) &&
        (8 == (**(**hGD).gdPMap)
                  .pixelSize)) /* if successful and on an 8 bit device */
    {
      hCTabDeviceColors =
          (**(**hGD).gdPMap).pmTable; /* do SetEntries to force CLUT update */
      ...
      err = Control((**hGD).gdRefNum, cscSetEntries,
                    (Ptr)&csPtr); /* SetEntries in CLUT */
    }
  }
}
...
static void RestoreSystemGammas(Ptr pSystemGammas) {
  ...
  if (pSysGammaIn)
    for (i = 0; i < pSysGammaIn->numDevices; i++) /* for all devices */
      RestoreDeviceGamma(
          pSysGammaIn->devGamma[i]->hGD,
          (Ptr)pSysGammaIn->devGamma[i]->pDeviceGamma); /* restore gamma */
}
...
static Boolean SetDeviceGammaRampGD(GDHandle hGD, Ptr pRamp) {
  ...
  Ptr csPtr;
  OSErr err;
  short dataBits, entries,
      channels = 3; /* force three channels in the gamma table */

  if (pRamp) /* ensure pRamp is allocated */
  {
    err = GetGammaTable(hGD,
                        &pTableGammaCurrent); /* get pointer to current table */
    if ((noErr == err) && pTableGammaCurrent) {
      dataBits =
          pTableGammaCurrent->gDataWidth; /* table must have same data width */
      entries = pTableGammaCurrent->gDataCnt; /* table must be same size */
      pTableGammaNew = (GammaTblPtr)CreateEmptyGammaTable(
          channels, entries, dataBits); /* our new table */
      if (pTableGammaNew)               /* if successful fill table */
      {
        unsigned char *pGammaBase =
            (unsigned char *)&pTableGammaNew->gFormulaData +
            pTableGammaNew->gFormulaSize; /* base of table */
        if ((256 == entries) &&
            (8 == dataBits)) /* simple case: direct mapping */
          BlockMove((Ptr)pRamp, (Ptr)pGammaBase,
                    channels * entries); /* move everything */
        else /* tough case handle entry, channel and data size disparities */
        {
          ...
          short bytesPerEntry =
              (dataBits + 7) /
              8; /* size, in bytes, of the device table entries */
          short shiftRightValue =
              8 - dataBits; /* number of right shifts ramp -> device */
          shiftRightValue += ((bytesPerEntry - 1) *
                              8); /* multibyte entries and the need to map a
                                     byte at a time most sig. to least sig. */
          for (indexChan = 0; indexChan < channels;
               indexChan++) /* for all the channels */
            for (indexEntry = 0; indexEntry < entries;
                 indexEntry++) /* for all the entries */
            {
              short currentShift =
                  shiftRightValue; /* reset current bit shift */
              long temp =
                  *((unsigned char *)pRamp + (indexChan << 8) +
                    (indexEntry << 8) / entries); /* get data from ramp */
              short indexByte;
              for (indexByte = 0; indexByte < bytesPerEntry;
                   indexByte++) /* for all bytes */
              {
                if (currentShift <
                    0) /* shift data correctly for current byte */
                  *(pGammaBase++) = temp << -currentShift;
                else
                  *(pGammaBase++) = temp >> currentShift;
                currentShift -=
                    8; /* increment shift to align to next less sig. byte */
              }
            }
        }

        /* set gamma */
        gameRecRestore.csGTable =
            (Ptr)pTableGammaNew; /* setup restore record */
        csPtr = (Ptr)&gameRecRestore;
        err = Control((**hGD).gdRefNum, cscSetGamma,
                      (Ptr)&csPtr); /* restore gamma (note, display drivers may
                                       delay returning from this until VBL) */

        if ((8 == (**(**hGD).gdPMap).pixelSize) &&
            (noErr == err)) /* if successful and on an 8 bit device */
        {
          hCTabDeviceColors =
              (**(**hGD).gdPMap)
                  .pmTable; /* do SetEntries to force CLUT update */
          setEntriesRec.csTable = (ColorSpec *)&(**hCTabDeviceColors).ctTable;
          setEntriesRec.csStart = 0;
          setEntriesRec.csCount = (**hCTabDeviceColors).ctSize;
          csPtr = (Ptr)&setEntriesRec;
