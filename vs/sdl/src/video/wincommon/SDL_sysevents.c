...
#if defined(_MSC_VER)
#pragma warning(                                                               \
    disable : 4996) /* Nobody cares that GetVersionExA() is deprecated */
#endif
...
static int WINAPI ToUnicode9xME(UINT vkey, UINT scancode, const BYTE *keystate,
...
static void LoadAygshell(void) {
  ...
  if ((aygshell != 0) && (SHFullScreen == 0)) {
    SHFullScreen =
        (int(WINAPI *)(struct HWND__ *, unsigned long))SDL_LoadFunction(
            aygshell, "SHFullScreen");
  }
}
...
static VOID CALLBACK TrackMouseTimerProc(HWND hWnd, UINT uMsg, UINT idEvent,
                                         DWORD dwTime) {
  union {
    RECT rect;
    POINT pt;
  } rectpt; /* prevent type-punning issue. */
  POINT pt;
...
LRESULT CALLBACK WinMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_ACTIVATE: {
    ...
    if (active) {
      ...
      if (WINDIB_FULLSCREEN()) {
        LoadAygshell();
        if (SHFullScreen)
          SHFullScreen(SDL_Window, SHFS_HIDESTARTICON | SHFS_HIDETASKBAR |
                                       SHFS_HIDESIPBUTTON);
        else
      ...
      if (SDL_GetAppState() & SDL_APPINPUTFOCUS) {
        ...
        if (WINDIB_FULLSCREEN()) {
          ...
          LoadAygshell();
          if (SHFullScreen)
            SHFullScreen(SDL_Window, SHFS_SHOWSTARTICON | SHFS_SHOWTASKBAR |
                                         SHFS_SHOWSIPBUTTON);
          else
  ...
  case WM_XBUTTONUP: {
    ...
    if (SDL_VideoSurface && !DINPUT()) {
      ...
      /* We need to register mouse cursor position FIRST before registering
         button position. It causes problems with DOSBox-X guest pointer
         integration for SDL to report a button event before reporting that the
         cursor moved to that position.

         Of course, Windows 10 considers holding your finger on the screen as
         something to withhold from the window until released, to signal
         right-click, so touchscreen interaction is a little funny where drag &
         drop is desired in the guest. */
  ...
  case WM_GETMINMAXINFO: {
    ...
    if (SDL_PublicSurface && (SDL_PublicSurface->flags & SDL_RESIZABLE)) {
      return (0);
    }
    ...
    AdjustWindowRect(&size, style,
                     style & WS_CHILDWINDOW ? FALSE : GetMenu(hwnd) != NULL);
  ...
  case WM_WINDOWPOSCHANGED: {
    ...
    if (!SDL_resizing && !IsZoomed(SDL_Window) && SDL_PublicSurface &&
        !(SDL_PublicSurface->flags & SDL_FULLSCREEN)) {
  ...
  case WM_PAINT: {
    ...
    if (!SDL1_hax_inhibit_WM_PAINT && current_video->screen &&
...
#ifndef NO_GETKEYBOARDSTATE
  /* FIXME: Windows will not notify us of input lang change if the user takes
     focus from our window, and then changes input language. How do we get this
     notification even if inactive?? */
  ...
  default: {
    ...
    if (HandleMessage) {
      return (HandleMessage(current_video, hwnd, msg, wParam, lParam));
    }
  } break;
  }
...
void SDL_SetModuleHandle(void *handle) { SDL_handle = handle; }
...
#define CS_BYTEALIGNCLIENT 0
  ...
  class.hCursor = NULL;
  class.hIcon =
      LoadImage(SDL_Instance, SDL_Appname, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR);
  ...
  if (handle) {
    _TrackMouseEvent = (BOOL(WINAPI *)(TRACKMOUSEEVENT *))GetProcAddress(
        handle, "TrackMouseEvent");
  }
...
static int WINAPI ToUnicode9xME(UINT vkey, UINT scancode, const BYTE *keystate,
  ...
  if (ToAsciiEx(vkey, scancode, (PBYTE)keystate, (WORD *)chars, 0,
