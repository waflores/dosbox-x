...
static int WIN_GL_ResetWindow(_THIS) {
  ...
  if (!SDL_windowid && SDL_strcmp(this->name, "windib") == 0) {
    ...
    SDL_resizing = 1;
    SDL_Window = CreateWindow(SDL_Appname, SDL_Appname, style, 0, 0,
    ...
    if (SDL_Window) {
      SetWindowPos(SDL_Window, HWND_TOP, 0, 0, rect.right - rect.left,
...
static int ChoosePixelFormatARB(_THIS, const int *iAttribs,
                                const FLOAT *fAttribs) {
  HWND hwnd;
  HDC hdc;
  HGLRC hglrc;
  ...
  hwnd = CreateWindow(SDL_Appname, SDL_Appname, WS_POPUP | WS_DISABLED, 0, 0,
  ...
  wglGetExtensionsStringARB =
      (const char *(WINAPI *)(HDC))this->gl_data->wglGetProcAddress(
          "wglGetExtensionsStringARB");
  ...
  if (ExtensionSupported("WGL_ARB_pixel_format", extensions)) {
    ...
    wglChoosePixelFormatARB = (BOOL(WINAPI *)(
        HDC, const int *, const FLOAT *, UINT, int *,
        UINT *))this->gl_data->wglGetProcAddress("wglChoosePixelFormatARB");
    if (wglChoosePixelFormatARB &&
        wglChoosePixelFormatARB(hdc, iAttribs, fAttribs, 1, &pformat,
                                &matches) &&
        pformat) {
...
int WIN_GL_SetupWindow(_THIS) {
  ...
  GL_pfd.cAccumBits = (GL_pfd.cAccumRedBits + GL_pfd.cAccumGreenBits +
  ...
  *iAttr = 0;
  ...
  if (this->gl_data->WGL_ARB_pixel_format) {
    this->gl_data->wglGetPixelFormatAttribivARB = (BOOL(WINAPI *)(
        HDC, int, int, UINT, const int *,
        int *))this->gl_data->wglGetProcAddress("wglGetPixelFormatAttribivARB");
  } else {
...
int WIN_GL_GetAttribute(_THIS, SDL_GLattr attrib, int *value) {
  ...
  if (this->gl_data->wglGetPixelFormatAttribivARB) {
    ...
    case SDL_GL_ACCELERATED_VISUAL:
      ...
      this->gl_data->wglGetPixelFormatAttribivARB(GL_hdc, pixel_format, 0, 1,
                                                  &wgl_attrib, value);
...
void WIN_GL_SwapBuffers(_THIS) { SwapBuffers(GL_hdc); }
...
int WIN_GL_LoadLibrary(_THIS, const char *path) {
  ...
  this->gl_data->wglGetProcAddress =
      (void *(WINAPI *)(const char *))GetProcAddress(handle,
                                                     "wglGetProcAddress");
  this->gl_data->wglCreateContext =
      (HGLRC(WINAPI *)(HDC))GetProcAddress(handle, "wglCreateContext");
  this->gl_data->wglDeleteContext =
      (BOOL(WINAPI *)(HGLRC))GetProcAddress(handle, "wglDeleteContext");
  this->gl_data->wglMakeCurrent =
      (BOOL(WINAPI *)(HDC, HGLRC))GetProcAddress(handle, "wglMakeCurrent");
  this->gl_data->wglSwapIntervalEXT =
      (void(WINAPI *)(int))GetProcAddress(handle, "wglSwapIntervalEXT");
  this->gl_data->wglGetSwapIntervalEXT =
      (int(WINAPI *)(void))GetProcAddress(handle, "wglGetSwapIntervalEXT");
