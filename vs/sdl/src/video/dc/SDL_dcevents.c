...
static void mouse_update(void) {
  const static char sdl_mousebtn[] = {MOUSE_LEFTBUTTON, MOUSE_RIGHTBUTTON,
                                      MOUSE_SIDEBUTTON, MOUSE_WHEELUP,
                                      MOUSE_WHEELDOWN};
  ...
  for (i = 0; i < sizeof(sdl_mousebtn); i++) {
    if (changed & sdl_mousebtn[i]) {
      SDL_PrivateMouseButton(
          (buttons & sdl_mousebtn[i]) ? SDL_PRESSED : SDL_RELEASED, i, 0, 0);
    }
  }
...
static void keyboard_update(void) {
  ...
  for (i = 0; i < sizeof(sdl_shift); i++) {
    if ((shiftkeys >> i) & 1) {
      ...
      SDL_PrivateKeyboard(
          ((state->shift_keys >> i) & 1) ? SDL_PRESSED : SDL_RELEASED, &keysym);
    }
  }
  ...
  for (i = 0; i < sizeof(sdl_key); i++) {
    if (state->matrix[i] != old_state.matrix[i]) {
      ...
      if (key) {
        keysym.sym = key;
        SDL_PrivateKeyboard(state->matrix[i] ? SDL_PRESSED : SDL_RELEASED,
                            &keysym);
      }
    }
  }
...
void DC_InitOSKeymap(_THIS) { /* do nothing. */ }
