...
static void Blit1to1(SDL_BlitInfo *info) {
  ...
  while (height--) {
    ...
    DUFFS_LOOP({ *dst = map[*src]; } dst++; src++;, width);
#else
    for (c = width; c; --c) {
      *dst = map[*src];
      dst++;
...
static SDL_loblit one_blit[] = {NULL, Blit1to1, Blit1to2, Blit1to3, Blit1to4};

static SDL_loblit one_blitkey[] = {NULL, Blit1to1Key, Blit1to2Key, Blit1to3Key,
                                   Blit1to4Key};
