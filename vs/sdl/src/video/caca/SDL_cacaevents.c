  ...
  do {
    ...
    if (event & (CACA_EVENT_KEY_PRESS | CACA_EVENT_KEY_RELEASE)) {
      ...
      posted += SDL_PrivateKeyboard(
          (event & CACA_EVENT_KEY_PRESS) ? SDL_PRESSED : SDL_RELEASED, &keysym);
      ...
      posted += SDL_PrivateMouseButton(
          (event & CACA_EVENT_MOUSE_PRESS) ? SDL_PRESSED : SDL_RELEASED, button,
          0, 0);
...
void Caca_InitOSKeymap(_THIS) { return; }
