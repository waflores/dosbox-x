...
#else
#define ChangeDisplaySettings(lpDevMode, dwFlags)                              \
  ChangeDisplaySettingsEx(NULL, (lpDevMode), 0, (dwFlags), 0)
#endif
#endif
#ifndef WS_MAXIMIZE
...
static int DIB_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors);
...
static int DIB_Available(void) { return (1); }
...
VideoBootStrap WINDIB_bootstrap = {"windib", "Win95/98/NT/2000/CE GDI",
...
static int DIB_AddMode(_THIS, int bpp, int w, int h) {
  ...
  SDL_modelist[index] = (SDL_Rect **)SDL_realloc(
      SDL_modelist[index], (1 + next_mode + 1) * sizeof(SDL_Rect *));
...
int DIB_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  for (i = 0; EnumDisplaySettings(NULL, i, &settings); ++i) {
    DIB_AddMode(this, settings.dmBitsPerPel, settings.dmPelsWidth,
                settings.dmPelsHeight);
#ifdef _WIN32_WCE
    if (this->hidden->supportRotation)
      DIB_AddMode(this, settings.dmBitsPerPel, settings.dmPelsHeight,
  ...
  DIB_AddMode(this, vformat->BitsPerPixel, GetDeviceCaps(GetDC(NULL), HORZRES),
...
#ifndef SDL_WIN32_NO_PARENT_WINDOW
  SetWindowPos(ParentWindowHWND, top, 0, 0, 0, 0,
               SWP_SHOWWINDOW | SWP_NOMOVE | SWP_NOSIZE);
#else
  SetWindowPos(SDL_Window, top, 0, 0, 0, 0,
               SWP_SHOWWINDOW | SWP_NOMOVE | SWP_NOSIZE);
#endif
}
...
static void DIB_ResizeWindow(_THIS, int width, int height, int prev_width,
                             int prev_height, Uint32 flags) {
#if defined(SDL_WIN32_HX_DOS)
  /* Windows XP understands the window is maximized, and route cursor input like
   * so. HX DOS needs a little help to let cursor input work correctly */
  if (SDL_VideoSurface != NULL)
    SetWindowPos(SDL_Window, HWND_TOP, 0, 0, GetSystemMetrics(SM_CXSCREEN),
  ...
  if (!SDL_windowid) {
...
#ifndef _WIN32_WCE
    AdjustWindowRectEx(&bounds, GetWindowLong(ParentWindowHWND, GWL_STYLE),
                       (GetMenu(ParentWindowHWND) != NULL), 0);
#else
    // The bMenu parameter must be FALSE; menu bars are not supported
    AdjustWindowRectEx(&bounds, GetWindowLong(ParentWindowHWND, GWL_STYLE), 0,
                       0);
#endif
    ...
    if ((flags & SDL_FULLSCREEN) || center) {
      // TODO move to header
      int SDL_windib_multimonitor_adjust_from_window(int *x, int *y, int width,
                                                     int height, HWND hwnd);

      x = y = 0;
      if (SDL_windib_multimonitor_adjust_from_window(&x, &y, width, height,
                                                     ParentWindowHWND) < 0) {
...
#ifndef SDL_WIN32_NO_PARENT_WINDOW
    if (SDL_VideoSurface != NULL)
      SetWindowPos(SDL_Window, HWND_TOP, 0, 0, SDL_VideoSurface->w,
                   SDL_VideoSurface->h, SWP_NOACTIVATE | SWP_SHOWWINDOW);

    EnterCriticalSection(&ParentWindowCritSec);
    /* Windows 10 has developed a strange deadlock that can happen if the user
       is resizing the parent window and we call SetWindowPos() to confirm the
       size here. Use SWP_NOMOVE just in case that's problematic too. */
    if (ParentWindowIsBeingResized) {
      /* record the window position/size to be applied when the user is finished
       * resizing */
...
SDL_Surface *DIB_SetVideoMode(_THIS, SDL_Surface *current, int width,
  ...
  if (SDL_Window &&
      ...
      (current->format->BitsPerPixel == bpp) && (flags & SDL_OPENGL) &&
...
static int DIB_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void DIB_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
static int DIB_LockHWSurface(_THIS, SDL_Surface *surface) { return (0); }
static void DIB_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void DIB_NormalUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  for (i = 0; i < numrects; ++i) {
    BitBlt(hdc, rects[i].x, rects[i].y, rects[i].w, rects[i].h, mdc, rects[i].x,
           rects[i].y, SRCCOPY);
  }
  DeleteDC(mdc);
  ReleaseDC(SDL_Window, hdc);
...
static BOOL CheckPaletteEntry(LOGPALETTE *pal, int index, BYTE r, BYTE g,
                              BYTE b) {
  PALETTEENTRY *entry;
  BOOL moved = 0;
...
int DIB_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  ...
  if (screen_pal) {
    ...
    if (GetSystemPaletteUse(hdc) != SYSPAL_NOSTATIC256) {
      ...
      moved_entries += CheckPaletteEntry(
          screen_logpal, screen_logpal->palNumEntries - 1, 0xff, 0xff, 0xff);
    }
    /* FIXME:
...
#endif
    SetPaletteEntries(screen_pal, 0, screen_logpal->palNumEntries,
                      screen_logpal->palPalEntry);
    SelectPalette(hdc, screen_pal, FALSE);
    RealizePalette(hdc);
  }
...
static void DIB_WinPAINT(_THIS, HDC hdc) {
  ...
  mdc = CreateCompatibleDC(hdc);
  SelectObject(mdc, screen_bmp);
  BitBlt(hdc, 0, 0, SDL_VideoSurface->w, SDL_VideoSurface->h, mdc, 0, 0,
         SRCCOPY);
  DeleteDC(mdc);
}
...
void DX5_SoundFocus(HWND hwnd) { return; }
#endif
