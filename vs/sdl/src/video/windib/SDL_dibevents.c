...
unsigned char SDL1_hax_hasLayoutChanged(void) { return hLayoutChanged; }

void SDL1_hax_ackLayoutChanged(void) { hLayoutChanged = 0; }

/* The translation table from a Microsoft VK keysym to a SDL keysym */
static SDLKey VK_keymap[SDLK_LAST];
static SDL_keysym *TranslateKey(WPARAM vkey, UINT scancode, SDL_keysym *keysym,
                                int pressed);
...
static void GapiTransform(GapiInfo *gapiInfo, LONG *x, LONG *y) {
  ...
  // 0 3 0
  if ((!gapiInfo->userOrientation && gapiInfo->systemOrientation &&
       !gapiInfo->gapiOrientation) ||
      // 3 0 3
      (gapiInfo->userOrientation && !gapiInfo->systemOrientation &&
       gapiInfo->gapiOrientation) ||
      // 3 0 0
      (gapiInfo->userOrientation && !gapiInfo->systemOrientation &&
       !gapiInfo->gapiOrientation)) {
...
SDL_DOSBox_X_Hack_Set_Toggle_Key_WM_USER_Hack(unsigned char x) {
...
LRESULT DIB_HandleMessage(_THIS, HWND hwnd, UINT msg, WPARAM wParam,
                          LPARAM lParam) {
#if defined(WIN32)
  ...
  /* DOSBox-X deviation: alternate route for user Num/Scroll/Caps from LL
   * keyboard hook */
#if defined(WIN32)
  ...
  case WM_KILLFOCUS:
    ...
    if (!SDL_resizing && SDL_PublicSurface &&
      ...
      /* In fullscreen mode, this window must have focus... or else we must exit
       * fullscreen mode! */
  ...
  case WM_KEYDOWN: {
    ...
    case VK_SHIFT:
      /* EXTENDED trick doesn't work here */
      {
        ...
        if (state[SDLK_RSHIFT] == SDL_RELEASED &&
    ...
    /* this is the workaround for the missing ToAscii() and ToUnicode() in CE
     * (not necessary at KEYUP!) */
...
#ifdef ENABLE_IM_EVENT
      if (wParam != VK_PROCESSKEY && !IM_Context.bCompos &&
          (GetTickCount() - end_ticks > IME_END_CR_WAIT || key->sym != 0x0d))
#endif
      {
  ...
  case WM_IME_ENDCOMPOSITION:
    ...
    FLIP_BREAK;
  case WM_IME_NOTIFY:
    if (IM_Context.notify_func) /* && IM_Context.bEnable &&
...
static void DIB_GenerateMouseMotionEvent(_THIS) {
  ...
  POINT mouse;
#ifdef _WIN32_WCE
  if (!GetCursorPos(&mouse) && !GetLastStylusPos(&mouse))
    return;
#else
...
void DIB_PumpEvents(_THIS) {
  /* NTS: Impose a 60Hz cap on mouse motion polling because
     SetCursorPos/GetCursorPos no longer have an immediate effect on cursor
     position in the way that the DIB mouse motion code expects. In Windows 10
     there seems to be enough of a round-trip delay between
     SetCursorPos/GetCursorPos and the compositor to cause the DIB mouse motion
     code to often register many duplicate mouse movements that never happened.
     Imposing a time interval between polling seems to fix this. This should not
     have any negative effects on older versions of Windows. --J.C. */
...
void DIB_InitOSKeymap(_THIS) { DIB_InitOSKeymapPriv(); }
...
#if 0 /* set to 1 to debug VK scancodes i.e. if debugging foreign keyboard     \
         layouts and SDL 1.x */
	{
		char tmp[128];
		char tmp2[256];
...
static SDL_keysym *TranslateKey(WPARAM vkey, UINT scancode, SDL_keysym *keysym,
  ...
  if (pressed && SDL_TranslateUNICODE) {
    ...
    if ((keystate[VK_NUMLOCK] & 1) && vkey >= VK_NUMPAD0 &&
        vkey <= VK_NUMPAD9) {
      keysym->unicode = (Uint16)(vkey - VK_NUMPAD0 + '0');
    } else if (SDL_ToUnicode((UINT)vkey, scancode, keystate, wchars,
                             sizeof(wchars) / sizeof(wchars[0]), 0) > 0) {
...
LRESULT CALLBACK ParentWinMessage(HWND hwnd, UINT msg, WPARAM wParam,
                                  LPARAM lParam) {
  if (msg == WM_CREATE) {
    return (0);
    ...
    SetWindowPos(SDL_Window, HWND_TOP, 0, 0, LOWORD(lParam), HIWORD(lParam),
                 SWP_NOACTIVATE);
    return (0);
  } else if (msg == WM_SYSCOMMAND) {
    /* CAREFUL! We only want to forward custom system menu items. Anything
     * standard to Windows must be handled ourselves. */
    if (wParam < 0xF000) {
      ...
      /* Windows 10 has recently developed a problem where calling
         SetWindowPos() from the main thread on this window while the user is
         resizing the window eventually results in a deadlock where the user is
         unable to move or resize the window anymore.

         To avoid this, set a flag and run DefWindowProc() here so that
         SDL_SetVideoMode() will know NOT to use SetWindowPos() */
      ...
      /* SDL_dibvideo.c gave us a deferred window position/size to apply after
resize. PROBLEM: The rect is calculated with AdjustWindowRectEx() which computes
the menu height as if the menu is normal and does not take into consideration
cases where the menu bar has doubled or tripled due to wrapping. To avoid the
window jumping and snapping erratically, don't set the new size unless it's
BIGGER than the current window size.

      We can't use GetMenuBarInfo() here since that appeared only in Windows
Vista or higher and this code is intended to work as low as Windows XP, or the
DOS HX extender.

      Actually according to Microsoft winuser.h where the function resides it
should be compatible with Windows XP, it's just that VS2019 doesn't let this
code see the constants and structures needed to use it in this source file for
some reason.

      Annoyingly their MSDN site has completely removed any information on when
APIs showed up in what version of Windows at all, instead of showing what
version it appeared in but pretending that nothing before Windows 2000 ever
existed. */
      if (nr.right > 0 && nr.bottom > 0) {
        ...
        if ((nr.right - nr.left) > (r.right - r.left) ||
            (nr.bottom - nr.top) > (r.bottom - r.top)) {
          SetWindowPos(ParentWindowHWND, NULL, nr.left, nr.top,
    ...
    /* FIXME: Why do MinGW builds crash here on thread shutdown, as if
     * SDL_PublicSurface never existed? */
    if (ParentWindowShutdown)
      return (0);
    ...
    */
    ...
    /* Before we forward to the child, we need to get the new dimensions, resize
       the child, and THEN forward it to the child. Note that this forwarding is
       required if SDL is to keep the window position intact when resizing the
       DIB window. */
    {
      RECT rc;

      GetClientRect(hwnd, &rc);
      SetWindowPos(SDL_Window, HWND_TOP, 0, 0, rc.right, rc.bottom,
                   SWP_NOACTIVATE);
    }
...
unsigned int __stdcall ParentWindowThreadProc(void *arg) {
  ...
  ParentWindowHWND = CreateWindow(
      SDL_AppnameParent, SDL_Appname,
  ...
  while (!ParentWindowShutdown &&
         GetMessage(&msg, ParentWindowHWND, 0, 0) > 0) {
...
int InitParentWindow(void) {
  if (ParentWindowThread == INVALID_HANDLE_VALUE) {
    ...
    ParentWindowThread = (HANDLE)_beginthreadex(NULL, 0, ParentWindowThreadProc,
                                                NULL, 0, &ParentWindowThreadID);
    ...
    if (!ParentWindowReady) {
      ...
      if (ParentWindowThreadRunning()) {
        ...
        /* this is brutal and may cause memory leaks in the Windows API, but it
         * must be done */
...
int DIB_CreateWindow(_THIS) {
  ...
  if (SDL_windowid) {
    ...
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, windowid, -1, windowid_t,
                        SDL_strlen(windowid) + 1);
...
#endif
#else
    SDL_Window = CreateWindow(SDL_Appname, SDL_Appname, WS_CHILD, 0, 0, 640,
                              480, ParentWindowHWND, NULL, SDL_Instance, NULL);
#endif
    if (SDL_Window == NULL) {
...
void DIB_DestroyWindow(_THIS) {
  ...
  // NTS: DOSBox-X likes to call SQL_Quit/SQL_QuitSubSystem just to reinit the
  // window.
...
int DIB_HandleComposition(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (lParam & GCS_RESULTSTR) {
    ...
    if (SDL_TranslateUNICODE) {
      IM_Context.im_compose_sz =
          ImmGetCompositionStringW(imc, GCS_RESULTSTR, NULL, 0);
    } else {
      IM_Context.im_compose_sz =
          ImmGetCompositionStringA(imc, GCS_RESULTSTR, NULL, 0);
    }
    if (!IM_Context.im_compose_sz) {
      ...
      if (IM_Context.im_compose_sz >= IM_Context.im_buffer_sz) {
        IM_Context.im_buffer_sz =
            IM_Context.im_compose_sz + 1 * sizeof(wchar_t);
        IM_Context.string.im_multi_byte_buffer = (char *)realloc(
    ...
    if (IM_Context.string.im_multi_byte_buffer) {
      ...
      if (SDL_TranslateUNICODE) {
        ImmGetCompositionStringW(imc, GCS_RESULTSTR,
                                 ...
                                 IM_Context.im_buffer_sz);
        IM_Context.string.im_wide_char_buffer[IM_Context.im_compose_sz / 2] =
            '\0';
      } else {
  ...
  /*
          if (lParam & GCS_COMPSTR) {
                  ...
                  if (!ImmSetCompositionWindow(imc, &form)) {
                          SDL_SetError("ImmSetCompositionWindow: fail to set
     composition form.\n");
                  }
                  ImmReleaseContext(hWnd, imc);
          }
...
int DIB_SetIMPosition(_THIS, int x, int y) { return 0; }
char *DIB_SetIMValues(_THIS, SDL_imvalue value, int alt) { return NULL; }
char *DIB_GetIMValues(_THIS, SDL_imvalue value, int *alt) { return NULL; }
int DIB_FlushIMString(_THIS, void *buffer) { return 0; }
#endif
