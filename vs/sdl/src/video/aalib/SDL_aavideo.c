...
static int AA_Available(void) { return 1; /* Always available ! */ }
...
VideoBootStrap AALIB_bootstrap = {"aalib", "ASCII Art Library", AA_Available,
                                  AA_CreateDevice};
...
int AA_VideoInit(_THIS, SDL_PixelFormat *vformat) {
  ...
  fprintf(stderr, "Using AAlib driver: %s (%s)\n", AA_context->driver->name,
          AA_context->driver->shortname);
...
/* From aavga.c
   AAlib does not give us the choice of the actual resolution, thus we have to
   simulate additional resolution by scaling down manually each frame
*/
static void fastscale(register char *b1, register char *b2, int x1, int x2,
                      int y1, int y2) {
...
SDL_Surface *AA_SetVideoMode(_THIS, SDL_Surface *current, int width, int height,
...
static void AA_ResizeHandler(aa_context *context) {
  ...
  fastscale(local_this->hidden->buffer, aa_image(context),
            local_this->hidden->w, aa_imgwidth(context), local_this->hidden->h,
            aa_imgheight(context));
  aa_renderpalette(context, local_this->hidden->palette,
                   local_this->hidden->rparams, 0, 0, aa_scrwidth(context),
                   aa_scrheight(context));
  aa_flush(context);
}

/* We don't actually allow hardware surfaces other than the main one */
static int AA_AllocHWSurface(_THIS, SDL_Surface *surface) { return (-1); }
static void AA_FreeHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void AA_UnlockHWSurface(_THIS, SDL_Surface *surface) { return; }
...
static void AA_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) {
  ...
  fastscale(AA_buffer, aa_image(AA_context), AA_w, aa_imgwidth(AA_context),
            AA_h, aa_imgheight(AA_context));
#if 1
  ...
  for (i = 0; i < numrects; ++i) {
    ...
    aa_renderpalette(AA_context, AA_palette, AA_rparams, rect->x * AA_x_ratio,
                     rect->y * AA_y_ratio, rect->w * AA_x_ratio,
                     rect->h * AA_y_ratio);
  }
  printf("\n");
#endif
...
int AA_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
  ...
  for (i = 0; i < ncolors; i++) {
    aa_setpalette(AA_palette, firstcolor + i, colors[i].r >> 2,
