...
void SDL_SYS_WaitThread(SDL_Thread *thread) { return; }

void SDL_SYS_KillThread(SDL_Thread *thread) { return; }
...
static int sig_list[] = {SIGHUP,    SIGINT,  SIGQUIT, SIGPIPE,
...
Uint32 SDL_ThreadID(void) { return ((Uint32)pthread_self()); }

void SDL_SYS_WaitThread(SDL_Thread *thread) { pthread_join(thread->handle, 0); }
