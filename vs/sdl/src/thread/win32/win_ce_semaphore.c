/* win_ce_semaphore.c
   ...
   4. Mutexes are used for critical sections. These could be replaced with
      ...
      capability.
   5. The implementation shows several interesting aspects of synchronization,
   some of which are specific to Win32 and some of which are general. These are
   pointed out in the comments as appropriate.
...
SYNCHHANDLE CreateSemaphoreCE(
    ...
    LPSECURITY_ATTRIBUTES
        lpSemaphoreAttributes, /* pointer to security attributes */
...

{
  ...
  __try {
    ...
    /*  Create the event. It is initially signaled if and only if the
        initial count is > 0 */
    hSynch->hEvent =
        CreateEvent(lpSemaphoreAttributes, FALSE, lInitialCount > 0, NULL);
...
BOOL ReleaseSemaphoreCE(SYNCHHANDLE hSemCE, LONG cReleaseCount,
...
/* Windows CE semaphore equivalent of WaitForSingleObject. */
{
  ...
  while (hSemCE->CurCount <= 0) {
    ...
    /*  This is where the properties of setting of an autoreset event is
       critical to assure that, even if the semaphore state changes between the
...
static SYNCHHANDLE
CleanUp(SYNCHHANDLE hSynch,
        DWORD Flags) { /* Prepare to return from a create of a synchronization
                          handle. If there was any failure, free any allocated
                          resources. "Flags" indicates which Win32 objects are
                          required in the synchronization handle. */
