...
static int SDLCALL win32_file_open(SDL_RWops *context, const char *filename,
                                   const char *mode) {
#ifndef _WIN32_WCE
  UINT old_error_mode;
#endif
...
#ifdef _WIN32_WCE
  {
    ...
    h = CreateFile(
        filenameW, (w_right | r_right), (w_right) ? 0 : FILE_SHARE_READ, NULL,
        (must_exist | truncate | a_mode), FILE_ATTRIBUTE_NORMAL, NULL);
    SDL_stack_free(filenameW);
  }
#else
  {
    ...
    if (unicode_support) { /* everything but Win95/98/ME. */
      wchar_t *filenameW = SDL_stack_alloc(wchar_t, size);
      if (MultiByteToWideChar(CP_UTF8, 0, filename, -1, filenameW, (int)size) ==
          0) {
        SDL_stack_free(filenameW);
        SDL_free(context->hidden.win32io.buffer.data);
        context->hidden.win32io.buffer.data = NULL;
      ...
      char *utf16 =
          SDL_iconv_string("UCS2", "UTF8", filename, SDL_strlen(filename) + 1);
      ...
      if (!utf16 ||
          !WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)utf16, -1, filenameA,
                               (int)(size * 6), 0, &bDefCharUsed) ||
          bDefCharUsed) {
      ...
      old_error_mode =
          SetErrorMode(SEM_NOOPENFILEERRORBOX | SEM_FAILCRITICALERRORS);
      h = CreateFile(
          filenameA, (w_right | r_right), (w_right) ? 0 : FILE_SHARE_READ, NULL,
          (must_exist | truncate | a_mode), FILE_ATTRIBUTE_NORMAL, NULL);
      /* restore old behaviour */
      SetErrorMode(old_error_mode);
...
static int SDLCALL win32_file_read(SDL_RWops *context, void *ptr, int size,
                                   int maxnum) {
  int total_need;
  int total_read = 0;
  int read_ahead;
  ...
  if (total_need < READAHEAD_BUFFER_SIZE) {
    if (!ReadFile(context->hidden.win32io.h,
                  context->hidden.win32io.buffer.data, READAHEAD_BUFFER_SIZE,
                  &byte_read, NULL)) {
    ...
    if (!ReadFile(context->hidden.win32io.h, ptr, total_need, &byte_read,
                  NULL)) {
      SDL_Error(SDL_EFREAD);
      return 0;
    }
...
static int SDLCALL win32_file_write(SDL_RWops *context, const void *ptr,
                                    int size, int num) {
  ...
  if (context->hidden.win32io.buffer.left) {
    SetFilePointer(context->hidden.win32io.h,
                   -context->hidden.win32io.buffer.left, NULL, FILE_CURRENT);
    context->hidden.win32io.buffer.left = 0;
  }
  ...
  if (!WriteFile(context->hidden.win32io.h, ptr, total_bytes, &byte_written,
                 NULL)) {
    SDL_Error(SDL_EFWRITE);
    return 0;
  }
...
static int SDLCALL stdio_read(SDL_RWops *context, void *ptr, int size,
                              int maxnum) {
  size_t nread;
...
static int SDLCALL stdio_write(SDL_RWops *context, const void *ptr, int size,
                               int num) {
  size_t nwrote;
...
static int SDLCALL mem_read(SDL_RWops *context, void *ptr, int size,
                            int maxnum) {
  size_t total_bytes;
...
static int SDLCALL mem_write(SDL_RWops *context, const void *ptr, int size,
                             int num) {
...
static int SDLCALL mem_writeconst(SDL_RWops *context, const void *ptr, int size,
                                  int num) {
