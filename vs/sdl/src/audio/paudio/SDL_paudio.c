...
AudioBootStrap Paud_bootstrap = {Paud_DRIVER_NAME, "AIX Paudio",
...
#ifdef DEBUG_AUDIO
      fprintf(stderr, "Waiting for write_buf_time=%ld,%ld\n", timeout.tv_sec,
              timeout.tv_usec);
#endif
    }
...
static Uint8 *Paud_GetAudioBuf(_THIS) { return mixbuf; }
...
static int Paud_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  if (paud_bufinfo.request_buf_cap == 1) {
    spec->samples =
        paud_bufinfo.write_buf_cap / bytes_per_sample / spec->channels;
  } else {
    spec->samples =
        paud_bufinfo.write_buf_cap / bytes_per_sample / spec->channels / 2;
  }
  ...
  paud_change.output =
      OUTPUT_1; /* EXTERNAL_SPEAKER,INTERNAL_SPEAKER,OUTPUT_1 */
