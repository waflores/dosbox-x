...
#ifdef __MACOSX__ /* Mac OS X uses threaded audio, so normal thread code is    \
                     okay */
  this->LockAudio = NULL;
...
AudioBootStrap SNDMGR_bootstrap = {"sndmgr", "MacOS SoundManager 3.0",
...
static void callBackProc(SndChannel *chan, SndCommand *cmd_passed) {
  ...
  fill_me =
      cmd_passed
          ->param2;   /* buffer that has just finished playing, so fill it */
...
static void Mac_LockAudio(_THIS) { /* no-op. */ }

static void Mac_UnlockAudio(_THIS) { /* no-op. */ }
...
/* This function is called by Sound Manager when it has exhausted one of
   ...
   we're not paused.
*/
static pascal void sndDoubleBackProc(SndChannelPtr chan,
  ...
  if (!audio->paused) {
    if (audio->convert.needed) {
      ...
      audio->spec.callback(audio->spec.userdata, (Uint8 *)newbuf->dbSoundData,
                           audio->spec.size);
    }
  }
