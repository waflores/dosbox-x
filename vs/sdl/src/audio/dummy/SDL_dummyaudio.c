...
AudioBootStrap DUMMYAUD_bootstrap = {DUMMYAUD_DRIVER_NAME,
...
static void DUMMYAUD_PlayAudio(_THIS) { /* no-op...this is a null driver. */ }

static Uint8 *DUMMYAUD_GetAudioBuf(_THIS) { return (this->hidden->mixbuf); }
...
static int DUMMYAUD_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  bytes_per_sec =
      (float)(((spec->format & 0xFF) / 8) * spec->channels * spec->freq);
