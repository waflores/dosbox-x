...
static Sint32 MS_ADPCM_nibble(struct MS_ADPCM_decodestate *state, Uint8 nybble,
                              Sint16 *coeff) {
  const Sint32 max_audioval = ((1 << (16 - 1)) - 1);
  const Sint32 min_audioval = -(1 << (16 - 1));
  const Sint32 adaptive[] = {230, 230, 230, 230, 307, 409, 512, 614,
...
static int MS_ADPCM_decode(Uint8 **audio_buf, Uint32 *audio_len) {
  ...
  *audio_buf = (Uint8 *)SDL_malloc(*audio_len);
  ...
  while (encoded_len >= MS_ADPCM_state.wavefmt.blockalign) {
    ...
    samplesleft =
        (MS_ADPCM_state.wSamplesPerBlock - 2) * MS_ADPCM_state.wavefmt.channels;
    while (samplesleft > 0) {
      nybble = (*encoded) >> 4;
...
static void Fill_IMA_ADPCM_block(Uint8 *decoded, Uint8 *encoded, int channel,
                                 int numchannels,
