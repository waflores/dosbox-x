...
static int Audio_Available(void) { return 1; }
...
AudioBootStrap UMS_bootstrap = {UMS_DRIVER_NAME, "AIX UMS audio",
...
static void UMS_PlayAudio(_THIS) {
  ...
  do {
    rc =
        UADWrite(this, &this->hidden->playbuf, samplesToWrite, &samplesWritten);
...
static int UMS_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  rc = UADSetChannels(this,
                      spec->channels); /* functions reduces to mono or stereo */
...
static UMSAudioDevice_ReturnCode UADSetSampleRate(_THIS, long rate,
  ...
  return UMSAudioDevice_set_sample_rate(this->hidden->umsdev, this->hidden->ev,
                                        rate, set_rate);
}

static UMSAudioDevice_ReturnCode UADSetByteOrder(_THIS, string byte_order) {
  return UMSAudioDevice_set_byte_order(this->hidden->umsdev, this->hidden->ev,
                                       byte_order);
}
...
UADSetTimeFormat(_THIS, UMSAudioTypes_TimeFormat fmt) {
   ...
   */
  return UMSAudioDevice_set_time_format(this->hidden->umsdev, this->hidden->ev,
                                        fmt);
}
...
static UMSAudioDevice_ReturnCode UADWriteBuffSize(_THIS, long *buff_size) {
   ...
   */
  return UMSAudioDevice_write_buff_size(this->hidden->umsdev, this->hidden->ev,
                                        buff_size);
}
...
static UMSAudioDevice_ReturnCode UADWriteBuffUsed(_THIS, long *buff_size) {
   ...
   * in the current time format
   */
  return UMSAudioDevice_write_buff_used(this->hidden->umsdev, this->hidden->ev,
                                        buff_size);
}
...
static UMSAudioDevice_ReturnCode UADSetDMABufferSize(_THIS, long bytes,
  ...
  return UMSAudioDevice_set_DMA_buffer_size(this->hidden->umsdev,
                                            this->hidden->ev, bytes, bytes_ret);
}
...
static UMSAudioDevice_ReturnCode UADSetVolume(_THIS, long volume) {
   ...
   */
  return UMSAudioDevice_set_volume(this->hidden->umsdev, this->hidden->ev,
                                   volume);
}
...
static UMSAudioDevice_ReturnCode UADSetBalance(_THIS, long balance) {
   ...
   * Takes effect immediately.
   */
  return UMSAudioDevice_set_balance(this->hidden->umsdev, this->hidden->ev,
                                    balance);
}
...
static UMSAudioDevice_ReturnCode UADOpen(_THIS, string device, string mode,
                                         long flags) {
  return UMSAudioDevice_open(this->hidden->umsdev, this->hidden->ev, device,
                             mode, flags);
}

static UMSAudioDevice_ReturnCode UADWrite(_THIS, UMSAudioTypes_Buffer *buff,
                                          long samples, long *samples_written) {
  return UMSAudioDevice_write(this->hidden->umsdev, this->hidden->ev, buff,
...
static UMSAudioDevice_ReturnCode
UADEnableOutput(_THIS, string output, long *left_gain, long *right_gain) {
  return UMSAudioDevice_enable_output(this->hidden->umsdev, this->hidden->ev,
