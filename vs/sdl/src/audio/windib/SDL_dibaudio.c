...
AudioBootStrap WAVEOUT_bootstrap = {"waveout", "Win95/98/NT/2000 WaveOut",
...
void DIB_WaitAudio(_THIS) {
  ...
  if (ret == WAIT_TIMEOUT) {
     ...
     * hanging on shutdown. See also
     * [https://github.com/joncampbell123/dosbox-x/issues/1021] */
    OutputDebugString(
        "SDL windib audio warning: Timeout waiting for audio to complete, "
        "possible audio driver problem. Forcing reset.\n");
    ...
    /* FIXME: This DOES resolve the hang on shutdown, however audio will always
     * stutter after this point. What can be done to fix that? */
    if (waveOutReset(sound) == MMSYSERR_NOERROR) {
      /* waveOutReset SHOULD call the callback function for any pending buffers
       * which will then reorient the semaphore */
      next_buffer = 0;
    } else {
      OutputDebugString(
          "SDL windib audio warning: Failed to reset audio output\n");
#if !defined(_WIN32_WCE)
      fprintf(stderr,
              "SDL windib audio warning: Failed to reset audio output\n");
#endif
...
int DIB_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  result = waveOutOpen(&sound, WAVE_MAPPER, &waveformat, (DWORD_PTR)FillSound,
