...
AudioBootStrap MINTAUDIO_MCSN_bootstrap = {MINT_AUDIO_DRIVER_NAME,
  ...
  case MCSN_FALCON: /* Also Mac */
    for (i = 1; i < 12; i++) {
      ...
      SDL_MintAudio_AddFrequency(
          this, MASTERCLOCK_FALCON1 / (MASTERPREDIV_FALCON * (i + 1)), CLK25M,
          i + 1, -1);
    }
    if (cookie_mcsn->res1 != 0) {
      for (i = 1; i < 4; i++) {
        SDL_MintAudio_AddFrequency(
            this, (cookie_mcsn->res1) / (MASTERPREDIV_FALCON * (1 << i)),
            CLKEXT, (1 << i) - 1, -1);
      }
    }
