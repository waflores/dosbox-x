...
void SDL_MintAudio_AddFrequency(_THIS, Uint32 frequency, Uint32 clock,
  ...
  if (MINTAUDIO_freqcount > 0) {
    for (i = MINTAUDIO_freqcount; i > p; i--) {
      SDL_memcpy(&MINTAUDIO_frequencies[i], &MINTAUDIO_frequencies[i - 1],
...
int SDL_MintAudio_SearchFrequency(_THIS, int desired_freq) {
  ...
  for (i = 0; i < MINTAUDIO_freqcount; i++) {
    if (desired_freq >= ((MINTAUDIO_frequencies[i].frequency +
                          MINTAUDIO_frequencies[i + 1].frequency) >>
                         1)) {
      return i;
    }
  }
    ...
    if (((unsigned long)pointers.play >=
      ...
      if (!buffers_filled[1]) {
        ...
        Setbuffer(0, SDL_MintAudio_audiobuf[1],
                  SDL_MintAudio_audiobuf[1] + SDL_MintAudio_audiosize);
      ...
      if (!buffers_filled[0]) {
        ...
        Setbuffer(0, SDL_MintAudio_audiobuf[0],
                  SDL_MintAudio_audiobuf[0] + SDL_MintAudio_audiosize);
