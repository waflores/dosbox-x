...
AudioBootStrap MINTAUDIO_XBIOS_bootstrap = {
    MINT_AUDIO_DRIVER_NAME, "MiNT XBIOS audio driver", Audio_Available,
    Audio_CreateDevice};
...
#define SIZE_BUF_CLOCK_MEASURE (44100 / 10)
  ...
  for (i = 0; i < 2; i++) {
    ...
    Xbtimer(XB_TIMERA, 5, 38,
            SDL_MintAudio_XbiosInterruptMeasureClock); /* delay mode timer A,
                                                          prediv /64, 1KHz */
    Jenabint(MFP_TIMERA);
    ...
    if ((Buffoper(-1) & 1) == 0) {
      if (SDL_MintAudio_clocktics) {
        ...
        khz = ((SIZE_BUF_CLOCK_MEASURE / SDL_MintAudio_clocktics) + 1) &
              0xFFFFFFFE;
        DEBUG_PRINT((DEBUG_NAME "measure %d: freq=%lu KHz\n", i + 1, khz));

        if (khz == 44) {
          for (j = 1; j < 4; j++) {
            SDL_MintAudio_AddFrequency(
                this, MASTERCLOCK_44K / (MASTERPREDIV_FALCON * (1 << j)),
                MASTERCLOCK_44K, (1 << j) - 1, 2 + i);
          }
        } else if (khz == 48) {
          for (j = 1; j < 4; j++) {
            SDL_MintAudio_AddFrequency(
                this, MASTERCLOCK_48K / (MASTERPREDIV_FALCON * (1 << j)),
                MASTERCLOCK_48K, (1 << j) - 1, 2 + i);
          }
        }
      } else {
...
static int Mint_CheckAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  for (i = 1; i < 12; i++) {
    ...
    SDL_MintAudio_AddFrequency(
        this, MASTERCLOCK_FALCON1 / (MASTERPREDIV_FALCON * (i + 1)),
  ...
  for (i = 0; i < MINTAUDIO_freqcount; i++) {
    DEBUG_PRINT((DEBUG_NAME "freq %d: %lu Hz, clock %lu, prediv %d\n", i,
                 MINTAUDIO_frequencies[i].frequency,
