...
//	//		silence =  0;
//	//	}
//	//	memset(sdl_nds_audiodevice->convert.buf, silence,
//sdl_nds_audiodevice->convert.len);
//	//
//sdl_nds_audiodevice->spec.callback(sdl_nds_audiodevice->spec.userdata,
//	//		(Uint8
//*)sdl_nds_audiodevice->convert.buf,sdl_nds_audiodevice->convert.len);
//	//	SDL_ConvertAudio(&sdl_nds_audiodevice->convert);
//	//	memcpy(buffer, sdl_nds_audiodevice->convert.buf,
//sdl_nds_audiodevice->convert.len_cvt);
//	//} else
//	{
//		sdl_nds_audiodevice->spec.callback(sdl_nds_audiodevice->spec.userdata,
//buffer, size);
...
//		int i;
//		for(i = 0; i < size; i++){
//			//((short*)stream)[i] =(short)buffer[i] << 8;
//// sound 8bit ---> buffer 16bit
//			//if (buffer[i] &0x80)
...
void SoundMixCallback(void *stream, u32 len) {
  ...
  if (!audio->paused) {
    if (audio->convert.needed) {
      ...
      (*audio->spec.callback)(audio->spec.userdata, (Uint8 *)audio->convert.buf,
                              audio->convert.len);
...
void MixSound(void) {
  ...
  if (soundsystem->format == 8) {
    if ((soundsystem->soundcursor + soundsystem->numsamples) >
        soundsystem->buffersize) {
      SoundMixCallback(&soundsystem->mixbuffer[soundsystem->soundcursor],
                       soundsystem->buffersize - soundsystem->soundcursor);
      remain = soundsystem->numsamples -
               (soundsystem->buffersize - soundsystem->soundcursor);
    ...
    if ((soundsystem->soundcursor + soundsystem->numsamples) >
      ...
      SoundMixCallback(&soundsystem->mixbuffer[soundsystem->soundcursor << 1],
                       (soundsystem->buffersize >> 1) -
                           soundsystem->soundcursor);
      remain = soundsystem->numsamples -
...
void InterruptHandler(void) { framecounter++; }
...
AudioBootStrap NDSAUD_bootstrap = {"nds", "NDS audio", Audio_Available,
                                   Audio_CreateDevice};
