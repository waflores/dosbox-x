...
AudioBootStrap DSOUND_bootstrap = {"dsound", "Win95/98/2000 DirectSound",
...
/* */
void DX5_SoundFocus(HWND hwnd) { mainwin = hwnd; }
...
static Uint8 *DX5_GetAudioBuf(_THIS) {
  ...
  result = IDirectSoundBuffer_Lock(
      mixbuf, cursor, mixlen, (LPVOID *)&locked_buf, &rawlen, NULL, &junk, 0);
  if (result == DSERR_BUFFERLOST) {
    IDirectSoundBuffer_Restore(mixbuf);
    result = IDirectSoundBuffer_Lock(
        mixbuf, cursor, mixlen, (LPVOID *)&locked_buf, &rawlen, NULL, &junk, 0);
...
static int CreatePrimary(LPDIRECTSOUND sndObj, HWND focus,
  ...
  result = IDirectSound_SetCooperativeLevel(sndObj, focus, DSSCL_WRITEPRIMARY);
  if (result != DS_OK) {
#ifdef DEBUG_SOUND
...
static int CreateSecondary(LPDIRECTSOUND sndObj, HWND focus,
  ...
  /* Try to set primary mixing privileges */
  if (focus) {
    result = IDirectSound_SetCooperativeLevel(sndObj, focus, DSSCL_PRIORITY);
  } else {
    result = IDirectSound_SetCooperativeLevel(sndObj, GetDesktopWindow(),
                                              DSSCL_NORMAL);
  }
  if (result != DS_OK) {
#ifdef DEBUG_SOUND
  ...
  result = IDirectSoundBuffer_Lock(
      *sndbuf, 0, format.dwBufferBytes, (LPVOID *)&pvAudioPtr1, &dwAudioBytes1,
      (LPVOID *)&pvAudioPtr2, &dwAudioBytes2, DSBLOCK_ENTIREBUFFER);
  if (result == DS_OK) {
    ...
    IDirectSoundBuffer_Unlock(*sndbuf, (LPVOID)pvAudioPtr1, dwAudioBytes1,
...
static int CreateAudioEvent(_THIS) {
  ...
  result = IDirectSoundBuffer_QueryInterface(mixbuf, &IID_IDirectSoundNotify,
                                             (void *)&notify);
  if (result != DS_OK) {
    goto done;
  }
  ...
  result = IDirectSoundNotify_SetNotificationPositions(notify, NUM_BUFFERS,
                                                       notify_positions);
  if (result == DS_OK) {
    retval = 0;
  }
...
static int DX5_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  if (mainwin) {
    NUM_BUFFERS =
        CreatePrimary(sound, mainwin, &mixbuf, &waveformat, spec->size);
  }
#endif /* USE_PRIMARY_BUFFER */
  if (NUM_BUFFERS < 0) {
    NUM_BUFFERS =
        CreateSecondary(sound, mainwin, &mixbuf, &waveformat, spec->size);
    if (NUM_BUFFERS < 0) {
      return (-1);
    }
#ifdef DEBUG_SOUND
