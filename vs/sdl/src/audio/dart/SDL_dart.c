...
//---------------------------------------------------------------------
LONG APIENTRY DARTEventFunc(ULONG ulStatus, PMCI_MIX_BUFFER pBuffer,
                            ULONG ulFlags) {
...
int DART_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  // pszDeviceType should contain the device type in low word, and device
  ...
  rc = mciSendCommand(iDeviceOrd, MCI_MIXSETUP, MCI_WAIT | MCI_MIXSETUP_INIT,
                      &(_this->hidden->MixSetupParms), 0);
  ...
  _this->hidden->BufferParms.pBufList =
      _this->hidden->pMixBuffers; // getting descriptorts into this list
  // Allocate buffers!
  rc = mciSendCommand(iDeviceOrd, MCI_BUFFER, MCI_WAIT | MCI_ALLOCATE_MEMORY,
  ...
  // Ok, we have all the buffers allocated, let's mark them!
  {
    ...
    for (i = 0; i < iNumBufs; i++) {
      ...
      if ((!(_this->hidden->pMixBuffers[i].pBuffer)) ||
        ...
        // and cleanup
        mciSendCommand(iDeviceOrd, MCI_BUFFER, MCI_WAIT | MCI_DEALLOCATE_MEMORY,
                       &(_this->hidden->BufferParms), 0);
        SDL_free(_this->hidden->pMixBuffers);
        _this->hidden->pMixBuffers = NULL;
        mciSendCommand(iDeviceOrd, MCI_CLOSE, MCI_WAIT, &GenericParms, 0);
      ...
      _this->hidden->pMixBuffers[i].ulBufferLength =
          _this->hidden->BufferParms.ulBufferSize;
      _this->hidden->pMixBuffers[i].ulUserParm =
          (ULONG)pBufferDesc; // User parameter: Description of buffer
      _this->hidden->pMixBuffers[i].ulFlags =
          0; // Some stuff should be flagged here for DART, like end of
  ...
  if (DosCreateEventSem(NULL, &(_this->hidden->hevAudioBufferPlayed), 0,
                        FALSE) != NO_ERROR) {
...
void DART_ThreadInit(_THIS) { return; }
...
void DART_WaitAudio(_THIS) {
  ...
  DosWaitEventSem(_this->hidden->hevAudioBufferPlayed,
...
void DART_PlayAudio(_THIS) {
  ...
  _this->hidden->MixSetupParms.pmixWrite(
      _this->hidden->MixSetupParms.ulMixHandle,
...
void DART_WaitDone(_THIS) {
  ...
  pBufDesc =
      (pMixBufferDesc)_this->hidden->pMixBuffers[_this->hidden->iLastPlayedBuf]
          .ulUserParm;
  rc = NO_ERROR;
  while ((pBufDesc->iBufferUsage != BUFFER_EMPTY) && (rc == NO_ERROR)) {
    DosResetEventSem(_this->hidden->hevAudioBufferPlayed, &ulPostCount);
    rc = DosWaitEventSem(_this->hidden->hevAudioBufferPlayed,
...
AudioBootStrap DART_bootstrap = {"dart", "OS/2 Direct Audio RouTines (DART)",
