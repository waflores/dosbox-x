...
/* FIXME: need to add in the near future flexible logic with frag_size and frags
 * count */
...
static SDL_AudioDevice *NTO_CreateAudioDevice(int devindex) {
  ...
  if (this) {
    ...
    this->hidden = (struct SDL_PrivateAudioData *)SDL_malloc(
        sizeof(struct SDL_PrivateAudioData));
  }
...
AudioBootStrap QNXNTOAUDIO_bootstrap = {DRIVER_NAME, "QNX6 QSA-NTO Audio",
                                        NTO_AudioAvailable,
      ...
      if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
        ...
        if ((errno == EINVAL) || (errno == EIO)) {
          ...
          if ((cstatus.status == SND_PCM_STATUS_UNDERRUN) ||
            ...
            if ((rval = snd_pcm_plugin_prepare(audio_handle,
...
static Uint8 *NTO_GetAudioBuf(_THIS) { return pcm_buf; }
...
static void NTO_CloseAudio(_THIS) {
  ...
  if (audio_handle != NULL) {
    if ((rval = snd_pcm_plugin_flush(audio_handle, SND_PCM_CHANNEL_PLAYBACK)) <
        0) {
...
static int NTO_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  if (!NTO_CheckBuggyCards(this, QSA_MMAP_WORKAROUND)) {
    ...
    if ((rval = snd_pcm_plugin_set_disable(audio_handle, PLUGIN_DISABLE_MMAP)) <
        0) {
  ...
  if (pcm_len == 0) {
    pcm_len = csetup.buf.block.frag_size * spec->channels *
  ...
  */
  ...
  if ((audio_fd = snd_pcm_file_descriptor(audio_handle,
                                          SND_PCM_CHANNEL_PLAYBACK)) < 0) {
    SDL_SetError(
