...
static void (*NAS_AuSetElements)(AuServer *, AuFlowID, AuBool, int, AuElement *,
                                 AuStatus *);
static void (*NAS_AuWriteElement)(AuServer *, AuFlowID, int, AuUint32,
                                  AuPointer, AuBool, AuStatus *);
static AuServer *(*NAS_AuOpenServer)(_AuConst char *, int, _AuConst char *, int,
                                     _AuConst char *, char **);
static AuEventHandlerRec *(*NAS_AuRegisterEventHandler)(AuServer *, AuMask, int,
                                                        AuID,
...
AudioBootStrap NAS_bootstrap = {NAS_DRIVER_NAME, "Network Audio System",
...
static void NAS_PlayAudio(_THIS) {
  while (this->hidden->mixlen >
         this->hidden->buf_free) { /* We think the buffer is full? Yikes! Ask
       the server for events, in the hope that some of them is LowWater events
       telling us more of the buffer is free now than what we think. */
    AuEvent ev;
    NAS_AuNextEvent(this->hidden->aud, AuTrue, &ev);
    NAS_AuDispatchEvent(this->hidden->aud, &ev);
  }
...
static Uint8 *NAS_GetAudioBuf(_THIS) { return (this->hidden->mixbuf); }
...
static AuBool event_handler(AuServer *aud, AuEvent *ev,
...
static AuDeviceID find_device(_THIS, int nch) {
...
static int NAS_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  if ((this->hidden->dev == AuNone) ||
      (!(this->hidden->flow = NAS_AuCreateFlow(this->hidden->aud, NULL)))) {
    NAS_AuCloseServer(this->hidden->aud);
    this->hidden->aud = 0;
