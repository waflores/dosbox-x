...
AudioBootStrap SUNAUDIO_bootstrap = {"audio", "UNIX /dev/audio interface",
...
Uint8 *DSP_GetAudioBuf(_THIS) { return (mixbuf); }
...
int DSP_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  for (;;) {
    ...
    info.play.precision =
        (enc == AUDIO_ENCODING_ULAW) ? 8 : spec->format & 0xff;
    info.play.encoding = enc;
    if (ioctl(audio_fd, AUDIO_SETINFO, &info) == 0) {

      /* Check to be sure we got what we wanted */
      if (ioctl(audio_fd, AUDIO_GETINFO, &info) < 0) {
        SDL_SetError("Error getting audio parameters: %s", strerror(errno));
        return -1;
      }
    ...
    default:
      /* oh well... */
      SDL_SetError("Error setting audio parameters: %s", strerror(errno));
      return -1;
    }
  }
#endif /* AUDIO_SETINFO */
