...
AudioBootStrap COREAUDIO_bootstrap = {"coreaudio", "Mac OS X CoreAudio",
...
void Core_WaitAudio(_THIS) { return; }

void Core_PlayAudio(_THIS) { return; }

Uint8 *Core_GetAudioBuf(_THIS) { return (NULL); }
...
void Core_CloseAudio(_THIS) {
  ...
  if (result != noErr) {
    SDL_SetError("Core_CloseAudio: AudioUnitSetProperty "
...
int Core_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  result = AudioUnitSetProperty(
      outputAudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input,
      0, &requestedDesc, sizeof(requestedDesc));
