...
int SDL_OpenAudioPath(char *path, int maxlen, int flags, int classic) {
  ...
  if ((audio_fd < 0) && (SDL_strlen(audiodev) < (sizeof(audiopath) - 3))) {
    ...
    do { /* Don't use errno ENOENT - it may not be thread-safe */
      SDL_snprintf(audiopath, SDL_arraysize(audiopath), "%s%d", audiodev,
                   instance++);
      exists = 0;
...
int SDL_OpenAudioPath(char *path, int maxlen, int flags, int classic) {
  ...
  while (devsettings[cycle][0] != '\0') {
    SDL_snprintf(audiopath, SDL_arraysize(audiopath), _PATH_DEV_DSP,
                 devsettings[cycle][0], devsettings[cycle][1],
                 devsettings[cycle][2]);
