...
AudioBootStrap BSD_AUDIO_bootstrap = {BSD_AUDIO_DRIVER_NAME,
                                      BSD_AUDIO_DRIVER_DESC, Audio_Available,
                                      Audio_CreateDevice};
...
static Uint8 *OBSD_GetAudioBuf(_THIS) { return (mixbuf); }
...
void OBSD_Status(_THIS) {
  ...
  fprintf(stderr,
          ...
          "active		:   %s\n"
          "",
          info.play.buffer_size, info.play.sample_rate, info.play.channels,
          info.play.precision, info.play.encoding, info.play.seek,
          info.play.samples, info.play.eof, info.play.pause ? "yes" : "no",
          info.play.error ? "yes" : "no", info.play.waiting ? "yes" : "no",
...
static int OBSD_OpenAudio(_THIS, SDL_AudioSpec *spec) {
  ...
  mixbuf = NULL;
  AUDIO_INITINFO(&info);
  for (format = SDL_FirstAudioFormat(spec->format); format;
