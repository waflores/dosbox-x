...
static int DCAUD_Available(void) { return 1; }
...
AudioBootStrap DCAUD_bootstrap = {"dcaudio", "Dreamcast AICA audio",
...
static void spu_memload_stereo8(int leftpos, int rightpos, void *src0,
                                size_t size) {
  uint8 *src = src0;
...
static void spu_memload_stereo16(int leftpos, int rightpos, void *src0,
                                 size_t size) {
  uint16 *src = src0;
  uint32 *left = (uint32 *)(leftpos + SPU_RAM_BASE);
  uint32 *right = (uint32 *)(rightpos + SPU_RAM_BASE);
...
static void DCAUD_PlayAudio(_THIS) {
  ...
  if (spec->channels == 1) {
    ...
    if ((this->spec.format & 255) == 8) {
      spu_memload_stereo8(this->hidden->leftpos + offset,
                          this->hidden->rightpos + offset, this->hidden->mixbuf,
                          this->hidden->mixlen);
    } else {
  ...
  if (!this->hidden->playing) {
    ...
    if (spec->channels == 1) {
      aica_play(0, mode, this->hidden->leftpos, 0, spec->samples * 2,
                spec->freq, 255, 128, 1);
    } else {
      aica_play(0, mode, this->hidden->leftpos, 0, spec->samples * 2,
                spec->freq, 255, 0, 1);
      aica_play(1, mode, this->hidden->rightpos, 0, spec->samples * 2,
                spec->freq, 255, 255, 1);
    }
  }
}

static Uint8 *DCAUD_GetAudioBuf(_THIS) { return (this->hidden->mixbuf); }
