...
void TDump::Dump(const TDesC8 &aDes) { iFile.Write(aDes); }
#endif
...
void CSimpleWait::RunL() { CActiveScheduler::Stop(); }

const TInt KAudioBuffers(2);

NONSHARABLE_CLASS(CEpocAudio)
    : public CBase, public MStreamObs, public MStreamProvider {
public:
...
CEpocAudio::CEpocAudio(TInt aBufferSize)
...
void CEpocAudio::ThreadInitL(TAny *aDevice) {
  ...
  EpocSdlEnv::AppendCleanupItem(
      TSdlCleanupItem((TSdlCleanupOperation)EPOC_CloseAudio, aDevice));
...
void CEpocAudio::Play() { iPosition = 0; }
...
AudioBootStrap EPOCAudio_bootstrap = {"epoc\0\0\0",
...
static int EPOC_OpenAudio(SDL_AudioDevice *thisdevice, SDL_AudioSpec *spec) {
  ...
  TRAPD(err, thisdevice->hidden = static_cast<SDL_PrivateAudioData *>(
                 CEpocAudio::NewL(buflen, spec->silence)));
  if (err != KErrNone)
    return -1;
...
static void EPOC_PlayAudio(SDL_AudioDevice *thisdevice) {
  if (CEpocAudio::Current(thisdevice)
