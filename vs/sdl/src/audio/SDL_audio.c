...
int SDL_OpenAudio(SDL_AudioSpec *desired, SDL_AudioSpec *obtained) {
  ...
  if (obtained != NULL) {
    ...
    if (SDL_BuildAudioCVT(&audio->convert, desired->format, desired->channels,
                          desired->freq, audio->spec.format,
                          audio->spec.channels, audio->spec.freq) < 0) {
      SDL_CloseAudio();
      return (-1);
    }
    if (audio->convert.needed) {
      audio->convert.len =
          (int)(((double)audio->spec.size) / audio->convert.len_ratio);
      audio->convert.buf = (Uint8 *)SDL_AllocAudioMem(audio->convert.len *
...
void SDL_CloseAudio(void) { SDL_QuitSubSystem(SDL_INIT_AUDIO); }
