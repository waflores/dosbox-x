...
Uint32 SDL_EventThreadID(void) { return (event_thread); }
...
SDL_EventFilter SDL_GetEventFilter(void) { return (SDL_EventOK); }
...
#if 0 // This does not seem necessary, and in fact results in lost keystrokes
      // when entering/leaving keystrokes. Disabled
        while ( SDL_PollEvent(&bitbucket) > 0 )
			;
#endif
...
#define IME_END_CR_WAIT 50
  return IM_Context.bCompos ||
                 (end_ticks && GetTickCount() - end_ticks < IME_END_CR_WAIT)
             ? 1
             : 0;
#else
  return 0;
