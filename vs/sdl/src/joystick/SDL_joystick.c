 ...
 */
int SDL_NumJoysticks(void) { return SDL_numjoysticks; }
...
SDL_Joystick *SDL_JoystickOpen(int device_index) {
  ...
  if (joystick->nballs > 0) {
    joystick->balls = (struct balldelta *)SDL_malloc(joystick->nballs *
...
int SDL_PrivateJoystickBall(SDL_Joystick *joystick, Uint8 ball, Sint16 xrel,
                            Sint16 yrel) {
  int posted;
...
int SDL_PrivateJoystickButton(SDL_Joystick *joystick, Uint8 button,
                              Uint8 state) {
  int posted;
#if !SDL_EVENTS_DISABLED
  SDL_Event event;
