...
struct report {
  ...
  int rid;     /* Report ID */
  enum { SREPORT_UNINIT, SREPORT_CLEAN, SREPORT_DIRTY } status;
};
...
static struct {
  ...
  hid_kind_t kind;
  const char *name;
} const repinfo[] = {{UHID_INPUT_REPORT, hid_input, "input"},
                     {UHID_OUTPUT_REPORT, hid_output, "output"},
                     {UHID_FEATURE_REPORT, hid_feature, "feature"}};

enum { REPORT_INPUT = 0, REPORT_OUTPUT = 1, REPORT_FEATURE = 2 };
...
static int usage_to_joyaxe(unsigned usage) {
  int joyaxe;
  switch (usage) {
  case HUG_X:
    joyaxe = JOYAXE_X;
...
int SDL_SYS_JoystickOpen(SDL_Joystick *joy) {
  ...
  if (hw->repdesc == NULL) {
    SDL_SetError("%s: USB_GET_REPORT_DESC: %s", hw->path, strerror(errno));
    goto usberr;
  }
  rep = &hw->inreport;
#if defined(__FREEBSD__) && (__FreeBSD_kernel_version > 800063) ||             \
  ...
  if (rep->size <= 0) {
    SDL_SetError("%s: Input report descriptor has invalid length", hw->path);
    goto usberr;
  }
    ...
    case hid_collection:
      ...
      case HUP_GENERIC_DESKTOP:
        ...
        case HUG_GAME_PAD:
          ...
          SDL_snprintf(sp, SDL_strlen(s) + 5, "%s (%d)", s, joy->index);
...
void SDL_SYS_JoystickUpdate(SDL_Joystick *joy) {
  struct hid_item hitem;
  struct hid_data *hdata;
  struct report *rep;
  int nbutton, naxe = -1;
    ...
    case hid_input:
      ...
      case HUP_GENERIC_DESKTOP: {
        ...
        if (joyaxe >= 0) {
          ...
          v *=
              32768 / ((hitem.logical_maximum - hitem.logical_minimum + 1) / 2);
          if (v != joy->axes[naxe]) {
...
void SDL_SYS_JoystickClose(SDL_Joystick *joy) {
...
static int report_alloc(struct report *r, struct report_desc *rd, int repind) {
  ...
  if (r->size > 0) {
    r->buf = SDL_malloc(sizeof(*r->buf) - sizeof(REP_BUF_DATA(r)) + r->size);
    if (r->buf == NULL) {
      SDL_OutOfMemory();
      return (-1);
    }
