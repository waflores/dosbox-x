...
#define MAX_NAME 19 /* 31 - strlen (SUFFIX) */
  ...
  if (noErr != FindFolder(kOnSystemDisk, kPreferencesFolderType,
                          kDontCreateFolder, &volume_ref_number, &directory_id))
    exit(-1);
  ...
  if (noErr !=
    ...
    if (!create)
      ...
      FSpCreateResFile(prefs_fsp, 0x3f3f3f3f, 'pref',
                       0); // '????' parsed as trigraph
...
#define DEFAULT_VIDEO_DRIVER                                                   \
  "\ptoolbox" /* pascal string for default video driver name */
...
#define VIDEO_ID_TOOLBOX 2
  ...
  PrefsRecord prefs = {DEFAULT_ARGS, DEFAULT_VIDEO_DRIVER,
...
#else
    GetDialogItem(commandDialog, kCL_File, &dummyType, &dummyHandle,
                  &dummyRect); /* MJS */
    ...
    do {
      ...
      if (itemHit == kCL_File) {
        ...
        SetControlValue((ControlHandle)dummyHandle,
                        !GetControlValue((ControlHandle)dummyHandle));
#endif
      }
