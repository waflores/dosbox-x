...
void CCurrentAppUi::DisableKeyBlocking() { SetKeyBlockMode(ENoKeyBlock); }
...
CEventQueue::~CEventQueue() { iCS.Close(); }
...
TBool CEventQueue::HasData() { return iVector.Size() > 0; }

void CEventQueue::Lock() { iCS.Wait(); }

void CEventQueue::Unlock() { iCS.Signal(); }
...
#define MAINFUNC(x)                                                            \
  EXPORT_C TMainFunc::TMainFunc(mainfunc##x aFunc) {                           \
    Mem::FillZ(iMainFunc, sizeof(iMainFunc));                                  \
    iMainFunc[x - 1] = (void *)aFunc;                                          \
  }
...
EXPORT_C TMainFunc::TMainFunc() { Mem::FillZ(iMainFunc, sizeof(iMainFunc)); }
...
CSdlAppServ::CSdlAppServ()
    : CActive(CActive::EPriorityHigh), iMainId(RThread().Id()) {}

MSDLObserver *CSdlAppServ::Observer() { return iObserver; }
...
void CSdlAppServ::Init() { PANIC_IF_ERROR(iAppThread.Open(iMainId)); }

void CSdlAppServ::SetParam(TInt aParam) { iReturnValue = aParam; }

void CSdlAppServ::HandleObserverValue(TInt aService, TInt aReturnValue,
                                      TBool aMainThread) {
  ...
  if (iStatus == KErrNone) {
    ...
    case EAppSrvWindowPointerCursorMode:
      iReturnValue = gEpocEnv->iDsa != NULL
                         ? gEpocEnv->iDsa->Session().PointerCursorMode()
                         : KErrNotReady;
      break;
...
void EpocSdlEnv::Resume() {
  ...
  const TInt value =
      gEpocEnv->iAppSrv->ObserverEvent(MSDLObserver::EEventResume, 0);
  gEpocEnv->iAppSrv->HandleObserverValue(MSDLObserver::EEventResume, value,
                                         ETrue);
}
...
void EpocSdlEnv::UnlockHwSurface() { gEpocEnv->iDsa->UnlockHwSurface(); }

TUint8 *EpocSdlEnv::LockHwSurface() { return gEpocEnv->iDsa->LockHwSurface(); }

void EpocSdlEnv::UpdateSwSurface() { gEpocEnv->iDsa->UpdateSwSurface(); }

TBool EpocSdlEnv::AddUpdateRect(TUint8 *aAddress, const TRect &aUpdateRect,
                                const TRect &aRect) {
...
TSize EpocSdlEnv::WindowSize(const TSize &aRequestedSize) {
  ...
  if (EpocSdlEnv::Flags(CSDL::EAllowImageResize) &&
      gEpocEnv->iDsa->WindowSize() != aRequestedSize) {
...
TDisplayMode EpocSdlEnv::DisplayMode() { return gEpocEnv->iDsa->DisplayMode(); }

TPointerCursorMode EpocSdlEnv::PointerMode() {
  return static_cast<TPointerCursorMode>(gEpocEnv->iAppSrv->RequestValue(
      CSdlAppServ::EAppSrvWindowPointerCursorMode));
}

TInt EpocSdlEnv::SetPalette(TInt aFirstcolor, TInt aColorCount,
                            TUint32 *aPalette) {
  return gEpocEnv->iDsa->SetPalette(aFirstcolor, aColorCount, aPalette);
}

void EpocSdlEnv::PanicMain(TInt aErr) { gEpocEnv->iAppSrv->PanicMain(aErr); }
...
EXPORT_C void CSDL::ReInitL(TFlags aFlags)
      ...
      gEpocEnv->iEpocEnvFlags = aFlags;
      TInt err = KErrNone;
      if(((prevFlags & EDrawModeDSB) != (aFlags & EDrawModeDSB)) &&
gEpocEnv->iDsa)
              {
              delete gEpocEnv->iDsa;
              gEpocEnv->iDsa = NULL;
              gEpocEnv->iDsa =
CDsa::RecreateL(EpocSdlEnv::Flags(CSDL::EDrawModeDSB));
              }
      }
*/
...
EXPORT_C TThreadId CSDL::CallMainL(const TMainFunc &aFunc,
                                   TRequestStatus *const aStatus,
                                   const CDesC8Array *const aArg, TInt aFlags,
                                   TInt aStackSize) {
  ...
  gEpocEnv->iArgv =
      (char **)User::AllocL(sizeof(char *) * (gEpocEnv->iArgc + 1));
...
EXPORT_C TInt CSDL::SDLCodesCount() const { return MAX_SCANCODE; }

EXPORT_C void CSDL::ResetSDLCodes() { ResetKeyMap(); }
...
EXPORT_C void CSDL::Resume() { EpocSdlEnv::Resume(); }

EXPORT_C void CSDL::Suspend() { gEpocEnv->iDsa->DoStop(); }
...
TBool EnvUtils::Rendezvous(RThread &aThread, TRequestStatus &aStatus) {
  if (gEpocEnv->iId != TThreadId(0) && aThread.Open(gEpocEnv->iId) &&
