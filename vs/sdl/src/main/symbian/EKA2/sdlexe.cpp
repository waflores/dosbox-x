...
void TVirtualCursor::Toggle() { iIsOn = !iIsOn; }

TBool TVirtualCursor::IsOn() const { return iIsOn; }

void TVirtualCursor::Set(const TRect &aRect, CFbsBitmap *aBmp,
                         CFbsBitmap *aAlpha) {
  iRect = aRect;
  iCBmp = aBmp;
  iAlpha = aAlpha;
...
void TVirtualCursor::MakeEvent(TWsEvent &aEvent, const TPoint &aBasePos) const {
  aEvent.SetType(EEventPointer), aEvent.SetTimeNow();
...
void TVirtualCursor::Draw(CBitmapContext &aGc, const TRect & /*aTargetRect*/,
                          const TSize & /*aSize*/) {
  if (iIsOn && iCBmp != NULL) {
...
public:
  TSdlClass();
  void SetMain(const TMainFunc &aFunc, TInt aFlags, MSDLMainObs *aObs,
               TInt aExeFlags);
  TInt SdlFlags() const;
...
void TSdlClass::AppFlags(TInt aFlags) { iExeFlags |= aFlags; }
...
TInt TSdlClass::AppFlags() const { return iExeFlags; }

void TSdlClass::SetMain(const TMainFunc &aFunc, TInt aFlags, MSDLMainObs *aObs,
                        TInt aExeFlags) {
  iFunc = aFunc;
  iSdlFlags = aFlags;
  iExeFlags = aExeFlags;
...
const TMainFunc &TSdlClass::Main() const { return iFunc; }

TInt TSdlClass::SdlFlags() const { return iSdlFlags; }

TSdlClass::TSdlClass() { Mem::FillZ(this, sizeof(this)); }
...
NONSHARABLE_CLASS(CSDLAppUi)
    : public CAknAppUi, public MExitWait, MSDLObserver {
public:
  ~CSDLAppUi();
...
TUid CSDLApplication::AppDllUid() const { return iUid; }
...
TFileName CSDLApplication::ResourceFileName() const { return KNullDesC(); }
...
CEikAppUi *CSDLDocument::CreateAppUiL() { return new (ELeave) CSDLAppUi; }
...
RWindow &CSDLWin::GetWindow() const { return Window(); }
...
void CSDLWin::SetNoDraw() { iNoDraw = ETrue; }
...
void CSDLAppUi::StartL() {
  if (gSDLClass.AppFlags() & SDLEnv::EParamQuery) {
    ...
    RFile file;
    TInt err =
        file.Open(iEikonEnv->FsSession(), _L("sdl_param.txt"), EFileRead);
    if (err == KErrNone) {
      file.Read(cmd);
      file.Close();
      MakeCCmdLineL(cmd, *iParams);
    }
    if (err != KErrNone || gSDLClass.AppFlags() & (SDLEnv::EParamQueryDialog ^
                                                   SDLEnv::EParamQuery)) {
      TBuf<256> buffer;
  ...
  iSdl->CallMainL(gSDLClass.Main(), &iWait->iStatus, iParams,
...
TBool CSDLAppUi::HandleKeyL(const TWsEvent &aEvent) {
  ...
  if ((key.iScanCode == EStdKeyYes) &&
      (gSDLClass.AppFlags() & SDLEnv::EVirtualMouse)) {
    if (type == EEventKeyUp) {
      iCursor.Toggle();
      iSdl->RedrawRequest();
    }
...
CApaApplication *NewApplication() { return new CSDLApplication(); }

EXPORT_C TInt SDLEnv::SetMain(const TMainFunc &aFunc, TInt aSdlFlags,
                              MSDLMainObs *aObs, TInt aSdlExeFlags) {
...
TInt SDLUiPrint(const TDesC8 & /*aInfo*/) { return KErrNotFound; }
