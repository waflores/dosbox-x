...
/*
   ...
   탎ec.
          ...
          * We can also try Microseconds() which is emulated : Overhead = 36
   탎ec.

        On PowerPC machines, we avoid the following:
          * OpenTransport is available on all PCI and some NuBus PowerMacs, but
   it uses UpTime() if available and falls back to Microseconds() otherwise.
        ...
        Another PowerPC note: certain configurations, especially 3rd party
   upgrade cards, may return inaccurate timings for the CPU or memory bus --
   causing skew in various system routines (up to 20% drift!). The VIA chip is
   very accurate, and it's the basis for the Time Manager and Microseconds().
        Unfortunately, it's also very slow because the MacOS has to (a) switch
   to 68K and (b) poll for a VIA event.
        ...
        Under MacOS 9, "new world" Macs (iMacs, B+W G3s and G+W G4s) have a
   native Time Manager implementation: UpTime(), Microseconds(), and TickCount()
   are all based on the same underlying counter. This makes it silly to
   calibrate UpTime() against TickCount(). We now check for this feature using
   Gestalt(), and skip the whole calibration step if possible.
...
void FastInitialize() {
  ...
  if (!gInited) {
    ...
    gUpTime = (UpTimeProcPtr)FindFunctionInSharedLib("\pDriverServicesLib",
                                                     "\pUpTime");
    if (gUpTime)
      gA2NS = (A2NSProcPtr)FindFunctionInSharedLib("\pDriverServicesLib",
...
static Ptr FindFunctionInSharedLib(StringPtr libName, StringPtr funcName) {
  ...
  if (/* error = */ GetSharedLibrary(libName, kCompiledCFragArch, kLoadCFrag,
                                     &connID, &entry, errorStr))
    return (NULL);
