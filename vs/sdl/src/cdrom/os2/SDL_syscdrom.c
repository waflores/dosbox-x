...
int SDL_SYS_CDInit(void) {
  ...
  if (LOUSHORT(mciSendCommand(0, MCI_SYSINFO, MCI_SYSINFO_QUANTITY | MCI_WAIT,
                              (PVOID)&msp, 0)) != MCIERR_SUCCESS)
    return (CD_ERROR);
...
static int SDL_SYS_CDOpen(int drive) {
  ...
  if (LOUSHORT(mciSendCommand(mop.usDeviceID, MCI_SET,
                              MCI_WAIT | MCI_SET_TIME_FORMAT, &msp, 0)) ==
      MCIERR_SUCCESS)
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  if (LOUSHORT(mciSendCommand(cdrom->id, MCI_STATUS, MCI_WAIT | MCI_STATUS_ITEM,
  ...
  for (i = 0; i < cdrom->numtracks; i++) {
    ...
    if (LOUSHORT(mciSendCommand(cdrom->id, MCI_STATUS,
                                MCI_WAIT | MCI_TRACK | MCI_STATUS_ITEM, &msp,
                                0)) != MCIERR_SUCCESS) {
      SDL_free(mtr);
      return (CD_ERROR);
    }
    ...
    /* Set Track Length - values from MCI are in MMTIMEs - 3000 MMTIME = 1
     * second */
  ...
  if (position != NULL) /* The SDL $&$&%# CDROM call sends NULL pointer here! */
  {
    if ((status == CD_PLAYING) || (status == CD_PAUSED)) {
      ...
      if (LOUSHORT(mciSendCommand(cdrom->id, MCI_STATUS,
          ...
          MCIERR_SUCCESS)
        return (CD_ERROR);
      /* Convert from MSF (format selected in the Open process) to Frames
       * (format that will be returned) */
      *position =
          MSF_TO_FRAMES(MSF_MINUTE(msp.ulReturn), MSF_SECOND(msp.ulReturn),
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  if (LOUSHORT(mciSendCommand(cdrom->id, MCI_PLAY, MCI_FROM | MCI_TO, &mpp,
                              0)) == MCIERR_SUCCESS)
    return 0;
  return (CD_ERROR);
}
...
static int SDL_SYS_CDEject(SDL_CD *cdrom) {
  ...
  msp.ulItem = (ULONG)NULL;        // No item
  msp.ulValue = (ULONG)NULL;       // No value for item flag
  if (LOUSHORT(mciSendCommand(cdrom->id, MCI_SET, MCI_WAIT | MCI_SET_DOOR_OPEN,
                              &msp, 0)) == MCIERR_SUCCESS)
    return 0;
  return (CD_ERROR);
}
