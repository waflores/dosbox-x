...
#ifdef DEBUG_CDROM
  char *devtype[] = {"Disk",    "Tape",   "Printer", "Processor",
                     "WORM",    "CD-ROM", "Scanner", "Optical",
                     "Changer", "Comm",   "Unknown"};
#endif
...
int SDL_SYS_CDInit(void) {
  ...
  struct {
    char *dir;
    char *name;
  } checklist[] = {{"/dev/rdisk", "cdrom"}, {"/dev", "rrz"}, {NULL, NULL}};
  char drive[32];
  char *SDLcdrom;
  int i, j, exists;
  struct stat stbuf;
      ...
      while (devent = readdir(devdir))
        if (SDL_memcmp(checklist[i].name, devent->d_name, name_len) == 0)
          if (devent->d_name[devent->d_namlen - 1] == 'c') {
            SDL_snprintf(drive, SDL_arraysize(drive), "%s/%s", checklist[i].dir,
                         devent->d_name);
#ifdef DEBUG_CDROM
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  toc.toc_alloc_length =
      (hdr.th_data_len1 << 8) + hdr.th_data_len0 + sizeof(hdr);
  ...
  for (i = 0; i <= cdrom->numtracks; ++i) {
    ...
    cdrom->track[i].offset = cdte[i].te_absaddr.lba.addr3 << 24 |
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
  ...
  if (position) {
    if (status == CD_PLAYING || (status == CD_PAUSED)) {
      *position = scd.scd_position_data.scp_absaddr.lba.addr3 << 24 |
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  FRAMES_TO_MSF(end + 150, /* LBA = 4500*M + 75*S + F - 150 */
                &msf.msf_ending_M_unit, &msf.msf_ending_S_unit,
