...
int msf_to_frame(int minute, int second, int frame) {
...
static int scsi_cmd(int fd, struct scsi_cdb *cdb, int cdblen, int rw,
                    caddr_t data, int datalen, struct scsi_user_cdb *sus) {
  int scsistatus;
  unsigned char *cp;
...
unsigned char *Inquiry(int fd) {
  static struct scsi_cdb6 cdb = {0x12, 0, 0, 0, 56, 0};
  static unsigned char Inqbuffer[56];

  if (scsi_cmd(fd, (cdb_t *)&cdb, 6, SUC_READ, Inqbuffer, sizeof(Inqbuffer), 0))
    return ("\377");
  return (Inqbuffer);
}
...
again:
  sts = scsi_cmd(fd, (cdb_t *)&cdb, 6, SUC_READ, 0, 0, &sus);
...
int SDL_SYS_CDInit(void) {
  /* checklist: /dev/rsr?c */
  static char *checklist[] = {"?0 rsr?", NULL};
  char *SDLcdrom;
  int i, j, exists;
  char drive[32];
  struct stat stbuf;
...
static const char *SDL_SYS_CDName(int drive) { return (SDL_cdlist[drive]); }
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  cdb[7] = toc_size >> 8;
  cdb[8] = toc_size & 0xff;
  sts = scsi_cmd(cdrom->id, (cdb_t *)cdb, 10, SUC_READ, toc, toc_size, &sus);
  if (sts < 0) {
    SDL_free(toc);
    return (-1);
  }
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
  ...
  sts = scsi_cmd(cdrom->id, (cdb_t *)cdb, 10, SUC_READ, buf, sizeof(buf), &sus);
  if (sts < 0)
    return (-1);
  if (sts) {
...
static int pauseresume(SDL_CD *cdrom, int flag) {
  u_char cdb[10];
  struct scsi_user_cdb sus;
...
/* Pause play */
static int SDL_SYS_CDPause(SDL_CD *cdrom) { return (pauseresume(cdrom, 0)); }

/* Resume play */
static int SDL_SYS_CDResume(SDL_CD *cdrom) { return (pauseresume(cdrom, 1)); }
...
static void SDL_SYS_CDClose(SDL_CD *cdrom) { close(cdrom->id); }
