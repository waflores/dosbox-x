...
static int FileReaderThread_TryNextRead(FileReaderThread *frt,
                                        AudioFileManager *inItem) {
  int didLock = 0;
  int succeeded = 0;
...
static void FileReaderThread_RemoveReader(FileReaderThread *frt,
                                          AudioFileManager *inItem) {
  if (frt->mNumReaders > 0) {
...
static int FileReaderThread_StartFixedPriorityThread(FileReaderThread *frt) {
  ...
  if (result)
    return 0; /*THROW_RESULT("pthread_attr_init - Thread attributes could not be
                 created.")*/

  result =
      pthread_attr_setdetachstate(&theThreadAttrs, PTHREAD_CREATE_DETACHED);
  if (result)
    return 0; /*THROW_RESULT("pthread_attr_setdetachstate - Thread attributes
  ...
  result = thread_policy_set(
      pthread_mach_thread_np(pThread), THREAD_EXTENDED_POLICY,
      (thread_policy_t)&theFixedPolicy, THREAD_EXTENDED_POLICY_COUNT);
  if (result)
    return 0; /*THROW_RESULT("thread_policy - Couldn't set thread as fixed
                 priority.")*/
  /* set priority */
  /* precedency policy's "importance" value is relative to spawning thread's
   * priority */
  relativePriority =
...
static void FileReaderThread_ReadNextChunk(FileReaderThread *frt) {
  ...
  for (;;) {
    ...
    /* construct pointer */
    char *writePtr =
        (char *)(theItem->GetFileBuffer(theItem) +
    ...
    theItem->mWriteToFirstBuffer =
        !theItem->mWriteToFirstBuffer; /* switch buffers */
...
static OSStatus AudioFileManager_Read(AudioFileManager *afm, char *buffer,
                                      ByteCount *len) {
  return FSReadFork(afm->mForkRefNum, fsFromStart,
                    afm->mReadFilePosition + afm->mAudioDataOffset, *len,
                    buffer, len);
}

static OSStatus AudioFileManager_GetFileData(AudioFileManager *afm,
                                             void **inOutData,
  ...
  if (afm->mReadFromFirstBuffer == afm->mWriteToFirstBuffer) {
    ...
    *inOutData = afm->mReadFromFirstBuffer
                     ? afm->mFileBuffer
                     : (afm->mFileBuffer + afm->mChunkSize);
  }
...
static OSStatus AudioFileManager_FileInputProc(
...
static OSStatus AudioFileManager_Render(AudioFileManager *afm,
...
AudioFileManager *new_AudioFileManager(AudioFilePlayer *inParent,
                                       SInt16 inForkRefNum, SInt64 inFileLength,
                                       UInt32 inChunkSize) {
