...
#define kAudioCDFilesystemID                                                   \
  (UInt16)(('J' << 8) | 'H') /* 'JH'; this avoids compiler warning */
...
void Unlock() { SDL_mutexV(apiMutex); }
...
int DetectAudioCDVolumes(FSVolumeRefNum *volumes, int numVolumes) {
  ...
  for (volumeIndex = 1; result == noErr || result != nsvErr; volumeIndex++) {
    ...
    result = FSGetVolumeInfo(kFSInvalidVolumeRefNum, volumeIndex, &actualVolume,
                             kFSVolInfoFSInfo, &volumeInfo, NULL, NULL);

    if (result == noErr) {
      if (volumeInfo.filesystemID ==
          kAudioCDFilesystemID) /* It's an audio CD */
      {
      ...
      /*SDL_SetError ("DetectAudioCDVolumes: FSGetVolumeInfo returned %d",
       * result);*/
    }
  }
...
int ReadTOCData(FSVolumeRefNum theVolume, SDL_CD *theCD) {
  ...
  theErr = FSOpenFork(&tocPlistFSRef, dataForkName.length, dataForkName.unicode,
                      fsRdPerm, &forkRefNum);
  if (theErr != noErr) {
    error = "FSOpenFork";
    goto bail;
  ...
  propertyListRef = CFPropertyListCreateFromXMLData(
      kCFAllocatorDefault, dataRef, kCFPropertyListImmutable, NULL);
  ...
  if (CFGetTypeID(propertyListRef) == CFDictionaryGetTypeID()) {
    ...
    theRawTOCDataRef =
        (CFDataRef)CFDictionaryGetValue(dictRef, CFSTR(kRawTOCDataString));

    /* Get the session array info. */
    theSessionArrayRef =
        (CFArrayRef)CFDictionaryGetValue(dictRef, CFSTR(kSessionsString));
    ...
    for (index = 0; index < numSessions; index++) {
      ...
      leadoutBlock = (CFNumberRef)CFDictionaryGetValue(
          theSessionDict, CFSTR(kLeadoutBlockString));

      trackArray = (CFArrayRef)CFDictionaryGetValue(theSessionDict,
      ...
      for (trackIndex = 0; trackIndex < numTracks; trackIndex++) {
        ...
        trackNumber = (CFNumberRef)CFDictionaryGetValue(theTrackDict,
                                                        CFSTR(kPointKeyString));
        sessionNumber = (CFNumberRef)CFDictionaryGetValue(
            theTrackDict, CFSTR(kSessionNumberKeyString));
        startBlock = (CFNumberRef)CFDictionaryGetValue(
            theTrackDict, CFSTR(kStartBlockKeyString));
        isDataTrack = (CFBooleanRef)CFDictionaryGetValue(theTrackDict,
                                                         CFSTR(kDataKeyString));
        ...
        if (trackIndex > 0) {
          theCD->track[idx - 1].length =
              theCD->track[idx].offset - theCD->track[idx - 1].offset;
        }
      }
...
int ListTrackFiles(FSVolumeRefNum theVolume, FSRef *trackFiles, int numTracks) {
  ...
  result = FSGetVolumeInfo(theVolume, 0, NULL, kFSVolInfoFSInfo, NULL, NULL,
                           &rootDirectory);
  ...
  if (result == noErr) {
    do {
      result = FSGetCatalogInfoBulk(iterator, 1, &actualObjects, NULL,
                                    kFSCatInfoNone, NULL, &ref, NULL, &nameStr);
      if (result == noErr) {

        CFStringRef name;
        name =
            CFStringCreateWithCharacters(NULL, nameStr.unicode, nameStr.length);
