...
static int AudioFilePlayer_SetDestination(AudioFilePlayer *afp,
                                          AudioUnit *inDestUnit) {
  /*if (afp->mConnected) throw static_cast<OSStatus>(-1);*/ /* can't set dest if
                                                               already engaged
                                                             */
  if (afp->mConnected)
    return 0;
  ...
  result = AudioUnitSetProperty(
      *inDestUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0,
      &afp->mFileDescription, sizeof(afp->mFileDescription));
...
static int AudioFilePlayer_Connect(AudioFilePlayer *afp) {
  ...
  if (!afp->mConnected) {
    ...
    /* set the render callback for the file data to be supplied to the sound
     * converter AU */
    ...
    OSStatus result = AudioUnitSetProperty(
        afp->mPlayUnit, kAudioUnitProperty_SetRenderCallback,
        kAudioUnitScope_Input, 0, &afp->mInputCallback,
...
/* #warning This should redirect the calling of notification code to some other
 * thread */
static void AudioFilePlayer_DoNotification(AudioFilePlayer *afp,
                                           OSStatus inStatus) {
  if (afp->mNotifier) {
...
static int AudioFilePlayer_OpenFile(AudioFilePlayer *afp, const FSRef *inRef,
  ...
  result = FSOpenFork(inRef, dfName.length, dfName.unicode, fsRdPerm,
                      &afp->mForkRefNum);
  if (result)
    return 0; /*THROW_RESULT("AudioFilePlayer::OpenFile(): FSOpenFork")*/

  /* Read the file header, and check if it's indeed an AIFC file */
  result = FSReadFork(afp->mForkRefNum, fsAtMark, 0, sizeof(chunkHeader),
                      &chunkHeader, &actual);
  if (result)
  ...
  if (SDL_SwapBE32(chunkHeader.ckID) != 'FORM') {
    result = -1;
    if (result)
      return 0; /*THROW_RESULT("AudioFilePlayer::OpenFile(): chunk id is not
                   'FORM'");*/
  }
  ...
  if (SDL_SwapBE32(chunkHeader.formType) != 'AIFC') {
    result = -1;
    if (result)
      return 0; /*THROW_RESULT("AudioFilePlayer::OpenFile(): file format is not
                   'AIFC'");*/
  }
  ...
  */
  offset = 0;
  do {
    result = FSReadFork(afp->mForkRefNum, fsFromMark, offset, sizeof(chunk),
                        &chunk, &actual);
    if (result)
      return 0; /*THROW_RESULT("AudioFilePlayer::OpenFile(): FSReadFork")*/
  ...
  result = FSReadFork(afp->mForkRefNum, fsAtMark, 0, sizeof(ssndData),
                      &ssndData, &actual);
  if (result)
    return 0; /*THROW_RESULT("AudioFilePlayer::OpenFile(): FSReadFork")*/
...
#undef SET_AUDIOFILEPLAYER_METHOD
  ...
  int bytesPerSecond = (UInt32)(4 * afp->mFileDescription.mSampleRate *
  ...
  afp->mAudioFileManager =
      new_AudioFileManager(afp, afp->mForkRefNum, fileDataSize, bytesPerSecond);
