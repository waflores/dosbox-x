...
static int LoadTracks(SDL_CD *cdrom) {
  ...
  /* Load tracks */
  if (ListTrackFiles(volumes[cdrom->id], tracks[cdrom->id], cdrom->numtracks) <
      0)
    return -1;
...
static FSRef *GetFileForOffset(SDL_CD *cdrom, int start, int length,
                               int *outStartFrame, int *outStopFrame) {
  int i;
...
/* Setup another file for playback, or stop playback (called from another
 * thread) */
static void CompletionProc(SDL_CD *cdrom) {
  ...
  if (nextTrackFrame > 0 && nextTrackFramesRemaining > 0) {
    ...
    file = GetFileForOffset(cdrom, nextTrackFrame, nextTrackFramesRemaining,
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  if (ref == NULL) {
    SDL_SetError("SDL_SYS_CDPlay: No file for start=%d, length=%d", start,
                 length);
    return -5;
  }
