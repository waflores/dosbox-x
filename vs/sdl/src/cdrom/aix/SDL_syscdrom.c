...
#ifdef DEBUG_CDROM
          fprintf(stderr, "Found unmounted CD ROM drive with device name %s\n",
                  entry.fs_spec);
#endif
...
static int SDL_SYS_CDOpen(int drive) {
  ...
  if (fd < 0) {
    ...
    if (ret < 0) {
      fprintf(stderr, "Could not get drive mode for %s (%s)\n", cdromname,
              strerror(errno));
    } else {
      switch (cdMode.cd_mode_form) {
      case CD_MODE1:
        fprintf(stderr, "Drive mode for %s is %s\n", cdromname,
                "CD-ROM Data Mode 1");
        break;
      case CD_MODE2_FORM1:
        fprintf(stderr, "Drive mode for %s is %s\n", cdromname,
                "CD-ROM XA Data Mode 2 Form 1");
        break;
      case CD_MODE2_FORM2:
        fprintf(stderr, "Drive mode for %s is %s\n", cdromname,
                "CD-ROM XA Data Mode 2 Form 2");
        break;
      case CD_DA:
        fprintf(stderr, "Drive mode for %s is %s\n", cdromname, "CD-DA");
        break;
    ...
    if (ret < 0) {
#ifdef DEBUG_CDROM
      fprintf(stderr, "Could not set drive mode for %s (%s)\n", cdromname,
              strerror(errno));
#endif
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  for (i = 0; i <= cdrom->numtracks; ++i) {
    ...
    if (SDL_SYS_CDioctl(cdrom->id, DKAUDIO, &entry) < 0) {
      ...
      cdrom->track[i].offset = MSF_TO_FRAMES(entry.indexing.track_msf.mins,
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  cmd.msf_flag = TRUE;
  FRAMES_TO_MSF(start, &cmd.indexing.msf.first_mins,
                &cmd.indexing.msf.first_secs, &cmd.indexing.msf.first_frames);
  FRAMES_TO_MSF(start + length, &cmd.indexing.msf.last_mins,
                &cmd.indexing.msf.last_secs, &cmd.indexing.msf.last_frames);
  cmd.volume_type = CD_VOLUME_ALL;
  cmd.all_channel_vol =
      255; /* This is a uchar. What is a good value? No docu! */
  ...
  fprintf(stderr, "Trying to play from %d:%d:%d to %d:%d:%d\n",
          cmd.indexing.msf.first_mins, cmd.indexing.msf.first_secs,
          cmd.indexing.msf.first_frames, cmd.indexing.msf.last_mins,
          cmd.indexing.msf.last_secs, cmd.indexing.msf.last_frames);
#endif
