  ...
  for (i = 'A'; i <= 'Z'; i++) {
    ...
    if (metainit.drives_map & (1 << (i - 'A'))) {
      ...
      if (handle == 0) {
        ...
        if ((Metaioctl(i, METADOS_IOCTL_MAGIC, CDROMSUBCHNL, &info) == 0) ||
...
void SDL_SYS_CDQuit(void) { SDL_numcds = 0; }
...
static int SDL_SYS_CDOpen(int drive) {
  ...
  handle = Metaopen(metados_drives[drive].device[0],
...
static int SDL_SYS_CDioctl(int id, int command, void *arg) {
  ...
  retval = Metaioctl(metados_drives[id].device[0], METADOS_IOCTL_MAGIC, command,
                     arg);
  if (retval < 0) {
  ...
  for (i = 0; i <= cdrom->numtracks; ++i) {
    ...
    if (SDL_SYS_CDioctl(cdrom->id, CDROMREADTOCENTRY, &entry) < 0) {
      ...
      cdrom->track[i].offset =
          MSF_TO_FRAMES(entry.cdte_addr.msf.minute, entry.cdte_addr.msf.second,
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  FRAMES_TO_MSF(start, &playtime.cdmsf_min0, &playtime.cdmsf_sec0,
