...
static const char *SDL_SYS_CDName(int drive) { return "/cd"; }
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  for (i = 0; i < cdrom->numtracks; i++) {
    ...
    cdrom->track[i].type = (TOC_CTRL(toc.entry[i]) == TRACK_CDDA)
                               ? SDL_AUDIO_TRACK
                               : SDL_DATA_TRACK;
    cdrom->track[i].offset = TOC_LBA(entry) - 150;
    cdrom->track[i].length =
        TOC_LBA((i + 1 < toc.last) ? toc.entry[i + 1] : toc.leadout_sector) -
        TOC_LBA(entry);
  }
...
/* Eject the CD-ROM */
static int SDL_SYS_CDEject(SDL_CD *cdrom) { return -1; }
