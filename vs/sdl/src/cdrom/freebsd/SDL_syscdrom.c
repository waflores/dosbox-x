...
int SDL_SYS_CDInit(void) {
  ...
  static char *checklist[] = {"cdrom",   "?0 cd?",  "?0 acd?", "?0 matcd?",
                              "?0 mcd?", "?0 scd?", NULL};
  char *SDLcdrom;
  int i, j, exists;
  char drive[32];
    ...
    for (i = 0; i <= cdrom->numtracks; ++i) {
      ...
      if (SDL_SYS_CDioctl(cdrom->id, CDIOREADTOCENTRYS, &entry) < 0) {
        ...
        cdrom->track[i].offset = MSF_TO_FRAMES(
            data.addr.msf.minute, data.addr.msf.second, data.addr.msf.frame);
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
  ...
  if (position) {
    if (status == CD_PLAYING || (status == CD_PAUSED)) {
      *position = MSF_TO_FRAMES(data.what.position.absaddr.msf.minute,
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  FRAMES_TO_MSF(start, &playtime.start_m, &playtime.start_s, &playtime.start_f);
  FRAMES_TO_MSF(start + length, &playtime.end_m, &playtime.end_s,
                &playtime.end_f);
#ifdef DEBUG_CDROM
