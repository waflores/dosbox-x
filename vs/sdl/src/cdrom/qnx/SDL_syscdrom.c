...
static int CheckDrive(char *drive, struct stat *stbuf) {
  ...
  if (S_ISCHR(stbuf->st_mode) || S_ISBLK(stbuf->st_mode)) {
    ...
    if (cdfd >= 0) {
      devctlret =
          devctl(cdfd, DCMD_CAM_DEVINFO, &dinfo, sizeof(cam_devinfo_t), NULL);
      ...
      if (devctlret == EOK) {
        ...
        /* in the near future need to add more checks for splitting cdroms from
         * other devices */
        if ((atapi) && (removable)) {
          is_cd = 1;
        }
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
  ...
  devctlret =
      devctl(cdrom->id, DCMD_CAM_DEVINFO, &dinfo, sizeof(cam_devinfo_t), NULL);
  ...
  do {
    devctlret =
        devctl(cdrom->id, DCMD_CAM_CDROMSUBCHNL, &info, sizeof(info), NULL);
    if (devctlret == EIO) {
    ...
    if (eagaincnt == 2) {
      /* workaround for broken cdroms, which can return always EAGAIN when its
       * not ready, */
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
  ...
  if (devctl(cdrom->id, DCMD_CAM_CDROMPLAYMSF, &playtime, sizeof(playtime),
             NULL) != 0) {
    return -1;
  } else {
    return 0;
