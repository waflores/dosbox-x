...
static int SDL_SYS_CDOpen(int drive) { return (drive); }
...
static int SDL_SYS_CDGetTOC(SDL_CD *cdrom) {
  ...
  cdrom->numtracks = SDL_SYS_BCDToShort(cdpb.csParam.bytes[1]) -
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
  ...
  if (1 || SDL_cdlist[cdrom->id].hasAudio) {
    ...
    if (spinning && position)
      *position = MSF_TO_FRAMES(SDL_SYS_BCDToShort(cdpb.csParam.cd.minute),
...
static int SDL_SYS_CDEject(SDL_CD *cdrom) {
  ...
  for (driveElem = (DrvQEl *)driveQ->qHead; driveElem;
       driveElem = (driveElem) ? ((DrvQEl *)driveElem->qLink)
                               : ((DrvQEl *)driveQ->qHead)) {
    if (driveQ->qTail) {
      driveQ->qTail->qLink = 0;
    }
...
static void SDL_SYS_CDClose(SDL_CD *cdrom) { return; }
