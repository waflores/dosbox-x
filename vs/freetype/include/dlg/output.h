...
extern "C"
{
  ...
  // The style will always be applied (using escape sequences), independent of
  // the given stream. On windows escape sequences don't work out of the box,
  // see dlg_win_init_ansi().
  DLG_API int dlg_styled_fprintf( FILE*            stream,
                                  struct dlg_style style,
                                  const char*      format,
                                  ... ) DLG_PRINTF_ATTRIB( 3, 4 );
  ...
  // args to print) for everything it has to print in printf format. See also
  // the *_stream and *_buf specializations for common usage. The given output
  // function must not be NULL.
  typedef void ( *dlg_generic_output_handler )( void*       data,
                                                const char* format,
                                                ... );
  DLG_API void dlg_generic_output( dlg_generic_output_handler output,
                                   void*                      data,
  ...
  // written as it is.
  DLG_API void dlg_generic_outputf( dlg_generic_output_handler output,
                                    void*                      data,
  ...
  // assure correct utf-8 even on windows consoles. Locks the stream (i.e.
  // assures threadsafe access) when the associated feature is passed (note that
  // stdout/stderr might still mix from multiple threads).
  DLG_API void dlg_generic_output_stream( FILE*                    stream,
                                          unsigned int             features,
                                          const struct dlg_origin* origin,
                                          const char*              string,
                                          const struct dlg_style   styles[6] );
  DLG_API void dlg_generic_outputf_stream( FILE*       stream,
                                           const char* format_string,
                                           const struct dlg_origin* origin,
                                           const char*              string,
                                           const struct dlg_style   styles[6],
  ...
  DLG_API void dlg_generic_output_buf( char*                    buf,
                                       size_t*                  size,
                                       unsigned int             features,
                                       const struct dlg_origin* origin,
                                       const char*              string,
                                       const struct dlg_style   styles[6] );
  DLG_API void dlg_generic_outputf_buf( char*                    buf,
                                        size_t*                  size,
                                        const char*              format_string,
                                        const struct dlg_origin* origin,
                                        const char*              string,
  ...
  // range (will probably result in a buffer overflow or garbage being printed).
  // If all member of style are 'none' will simply nullterminate the first buf
  // char.
