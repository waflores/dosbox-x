...
#define DLG_CREATE_TAGS( ... )               \
  ( const char* const[] )                    \
  {                                          \
    DLG_DEFAULT_TAGS_TERM, __VA_ARGS__, NULL \
  }
#endif
...
extern "C"
{
  ...
  typedef void ( *dlg_handler )( const struct dlg_origin* origin,
                                 const char*              string,
                                 void*                    data );
...
#define dlg__assert_or( level, tags, expr, code, msg )             \
  ...
  DLG_API void        dlg__do_log( enum dlg_level lvl,
                                   const char* const*,
                                   const char*,
                                   int,
                                   const char*,
                                   const char*,
                                   const char* );
  DLG_API const char* dlg__strip_root_path( const char* file,
                                            const char* base );
...
#define dlg__assert_or( level, tags, expr, code, msg ) \
  ...
  // Sets the handler that is responsible for formatting and outputting log
  // calls. This function is not thread safe and the handler is set globally.
  ...
  // threads.
  DLG_API void dlg_default_output( const struct dlg_origin*,
                                   const char* string,
                                   void* );
  ...
  // Adds the given tag associated with the given function to the thread
  // specific list. If func is not NULL the tag will only applied to calls from
  // the same function. Remove the tag again calling dlg_remove_tag (with
  // exactly the same pointers!). Does not check if the tag is already present.
...
#define dlg_assert_or( expr, code ) \
  dlg_assertl_or( DLG_DEFAULT_ASSERT, expr, code )
