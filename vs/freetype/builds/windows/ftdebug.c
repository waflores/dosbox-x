...
OutputDebugStringA( LPCSTR lpOutputString )
{
  ...
  len = MultiByteToWideChar( CP_ACP, MB_ERR_INVALID_CHARS, lpOutputString, -1,
                             NULL, 0 );
  ...
  MultiByteToWideChar( CP_ACP, MB_ERR_INVALID_CHARS, lpOutputString, -1,
                       lpOutputStringW, len );
...
#define FEATURES_TIMESTAMP_COMPONENT "[%h:%m %t] "
    ...
    if ( ft_component_flag )
    {
      ...
      for ( i = 0; i < FT_MAX_TRACE_LEVEL_LENGTH - tag_length; i++ )
        *bufp++ = ' ';
    }
  }
  ...
  *bufp   = '\0';
  ...
  dlg_generic_outputf_stream( ft_fileptr, (const char*)features_buf, origin,
                              string, dlg_default_output_styles, true );
