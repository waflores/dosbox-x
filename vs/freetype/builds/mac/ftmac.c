                                 ...
                                 FT_Long*    face_index )
{
  ...
  *face_index = 0;
  ...
  cf_fontName =
      CFStringCreateWithCString( NULL, fontName, kCFStringEncodingMacRoman );
  ats_font_id =
      ATSFontFindFromName( cf_fontName, kATSOptionFlagsUnRestrictedScope );
                              ...
                              FT_Long*    face_index )
{
  ...
  if ( noErr !=
       FSGetCatalogInfo( &ref, kFSCatInfoNone, NULL, NULL, pathSpec, NULL ) )
    return FT_THROW( Unknown_File_Format );
...
static OSErr
FT_FSPathMakeSpec( const UInt8* pathname, FSSpec* spec_p, Boolean isDirectory )
{
  const char *p, *q;
  short       vRefNum;
  long        dirID;
...
static OSErr
FT_FSpMakePath( const FSSpec* spec_p, UInt8* path, UInt32 maxPathSize )
{
  OSErr  err;
  FSSpec spec = *spec_p;
  short  vRefNum;
...
static OSErr
FT_FSPathMakeRes( const UInt8* pathname, ResFileRefNum* res )
{
#if HAVE_FSREF
...
get_file_type_from_path( const UInt8* pathname )
{
  ...
  if ( noErr !=
       FSGetCatalogInfo( &ref, kFSCatInfoFinderInfo, &info, NULL, NULL, NULL ) )
    return (OSType)0;
...
static void
create_lwfn_name( char* ps_name, Str255 lwfn_file_name )
{
  int      max = 5, count = 0;
...
count_faces_scalable( char* fond_data )
{
  ...
  face_all =
      EndianS16_BtoN( *( (short*)( fond_data + sizeof( FamRec ) ) ) ) + 1;
                     ...
                     int              path_size )
{
  ...
  if ( noErr !=
       FSGetCatalogInfo( &ref, kFSCatInfoNone, NULL, NULL, NULL, &par_ref ) )
    return FT_THROW( Invalid_Argument );
  ...
  if ( noErr !=
       FSGetCatalogInfo( &ref, kFSCatInfoNone, NULL, NULL, NULL, NULL ) )
    return FT_THROW( Cannot_Open_Resource );
...
static short
count_faces( Handle fond, const UInt8* pathname )
{
  ResID    sfnt_id;
                       ...
                       FT_Face*     aface )
{
  ...
  return open_face_from_buffer( library, pfb_data, pfb_size, face_index,
                                "type1", aface );
}
                       ...
                       FT_Face*   aface )
{
  ...
  if ( is_sfnt_ps )
  {
    ...
    if ( !open_face_PS_from_sfnt_stream( library, stream, face_index, 0, NULL,
                                         aface ) )
    {
...
Try_OpenType:
  error = open_face_from_buffer( library, sfnt_data, sfnt_size, face_index,
                       ...
                       FT_Face*   aface )
{
  ...
  if ( lwfn_file_name[0] )
  {
...

    {
      ...
      err = FSGetForkCBInfo( res, kFSInvalidVolumeRefNum, NULL, NULL, NULL,
                             &ref, NULL );
      if ( noErr != err )
        goto found_no_lwfn_file;
      ...
      error = lookup_lwfn_by_fond( path_fond, lwfn_file_name, path_lwfn,
                                   sizeof( path_lwfn ) );
      if ( !error )
        have_lwfn = 1;
    }
...

    {
      ...
      err =
          FSMakeFSSpec( pb.ioFCBVRefNum, pb.ioFCBParID, fond_file_name, &spec );
      if ( noErr != err )
        goto found_no_lwfn_file;
             ...
             FT_Face*    aface )
{
  ...
  *aface = NULL;
  ...
  error =
      FT_New_Face_From_Resource( library, (UInt8*)pathname, face_index, aface );
  if ( error || *aface )
    return error;
