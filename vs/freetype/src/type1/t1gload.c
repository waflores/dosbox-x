                                    ...
                                    FT_Bool*   force_scaling )
{
  ...
  if ( !error )
  {
    ...
    if ( decoder->builder.metrics_only )
      error =
          decoder_funcs->parse_metrics( decoder, (FT_Byte*)char_string->pointer,
                                        (FT_UInt)char_string->length );
#endif
    else
    {
...
T1_Compute_Max_Advance( T1_Face face, FT_Pos* max_advance )
{
  ...
  *max_advance = 0;
  ...
  error =
      psaux->t1_decoder_funcs->init( &decoder, (FT_Face)face, 0, /* size */
                                     0, /* glyph slot */
                                     (FT_Byte**)type1->glyph_names, face->blend,
                                     0, FT_RENDER_MODE_NORMAL, T1_Parse_Glyph );
  if ( error )
    return error;
  ...
  if ( !error )
  {
    ...
    else
    {
      ...
      if ( load_flags & FT_LOAD_VERTICAL_LAYOUT )
      {
        ...
        metrics->vertAdvance =
            ( face->type1.font_bbox.yMax - face->type1.font_bbox.yMin ) >> 16;
