...
pfr_extra_item_load_bitmap_info( FT_Byte* p, FT_Byte* limit, void* phy_font_ )
{
 ...
 */
FT_CALLBACK_DEF( FT_Error )
pfr_extra_item_load_font_id( FT_Byte* p, FT_Byte* limit, void* phy_font_ )
{
  PFR_PhyFont phy_font = (PFR_PhyFont)phy_font_;
  FT_Error    error    = FT_Err_Ok;
  FT_Memory   memory   = phy_font->memory;
...
FT_CALLBACK_DEF( FT_Error )
pfr_extra_item_load_stem_snaps( FT_Byte* p, FT_Byte* limit, void* phy_font_ )
{
  PFR_PhyFont phy_font = (PFR_PhyFont)phy_font_;
...
FT_CALLBACK_DEF( FT_Error )
pfr_extra_item_load_kerning_pairs( FT_Byte* p, FT_Byte* limit, void* phy_font_ )
{
  PFR_PhyFont  phy_font = (PFR_PhyFont)phy_font_;
                   ...
                   FT_UInt32   size )
{
  ...
  if ( !phy_font->outline_resolution || !phy_font->metrics_resolution )
  {
    error = FT_THROW( Invalid_Table );
  ...
  if ( num_aux > 0 )
  {
    ...
    while ( num_aux > 0 )
    {
      ...
      case 1:
        ...
        error =
            pfr_aux_name_load( q, length - 4U, memory, &phy_font->family_name );
        if ( error )
          goto Exit;
        break;
      ...
      case 3:
        ...
        /* an even number of bytes with a zero byte appended if needed  */
        error =
            pfr_aux_name_load( q, length - 4U, memory, &phy_font->style_name );
        if ( error )
          goto Exit;
        break;
  ...
  /* read the character descriptors */
  {
    ...
    for ( n = 0; n < count; n++ )
    {
      ...
      cur->char_code = ( flags & PFR_PHY_2BYTE_CHARCODE ) ? PFR_NEXT_USHORT( p )
                                                          : PFR_NEXT_BYTE( p );
      ...
      if ( flags & PFR_PHY_ASCII_CODE )
        p += 1;
#endif
      cur->gps_size = ( flags & PFR_PHY_2BYTE_GPS_SIZE ) ? PFR_NEXT_USHORT( p )
                                                         : PFR_NEXT_BYTE( p );
