                                      ...
                                      GXV_Validator gxvalid )
{
  ...
  if ( variantsAxis != 0x64756374L ) /* 'duct' */
    GXV_TRACE( ( "variantsAxis 0x%08lx is non default value", variantsAxis ) );
                                  ...
                                  GXV_Validator gxvalid )
{
  ...
  if ( subFeatureFlags )
    GXV_TRACE(
        ( "  justClassTable: nonzero value (0x%08lx)"
                            ...
                            GXV_Validator gxvalid )
{
  ...
  gxv_odtect_add_range( p, gxvalid->subtable_length, "just_LookupTable",
                        odtect );

  if ( wdcTableOffset )
  {
    gxv_just_widthDeltaClusters_validate( gxvalid->root->base + wdcTableOffset,
                                          limit, gxvalid );
  ...
  if ( pcTableOffset )
  {
    gxv_just_postcompTable_validate( gxvalid->root->base + pcTableOffset, limit,
                                     gxvalid );
