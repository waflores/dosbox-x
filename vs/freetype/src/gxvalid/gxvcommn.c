...
FT_COMPARE_DEF( int )
gxv_compare_ushort_offset( const void* a, const void* b )
{
...
FT_COMPARE_DEF( int )
gxv_compare_ulong_offset( const void* a, const void* b )
{
                                ...
                                GXV_Validator gxvalid )
{
  ...
  ft_qsort( buff, ( nmemb + 1 ), sizeof( FT_ULong ), gxv_compare_ulong_offset );
                                     ...
                                     GXV_Validator      gxvalid )
{
  ...
  {
    if ( binSrchHeader->searchRange == 0 && binSrchHeader->entrySelector == 0 &&
  ...
  for ( searchRange = 1, entrySelector = 1;
        ( searchRange * 2 ) <= binSrchHeader->nUnits && searchRange < 0x8000U;
       ...
       rangeShift != binSrchHeader->rangeShift )
  {
    ...
    GXV_TRACE(
        ( "calculated: unitSize=%d, nUnits=%d, "
          "searchRange=%d, entrySelector=%d, "
          "rangeShift=%d\n",
          binSrchHeader->unitSize, binSrchHeader->nUnits, searchRange,
...
gxv_lookup_value_load( FT_Bytes p, GXV_LookupValue_SignSpec signspec )
{
                       ...
                       GXV_Validator gxvalid )
{
  ...
  error =
      FT_Load_Glyph( face, gid, FT_LOAD_NO_BITMAP | FT_LOAD_IGNORE_TRANSFORM );
  if ( error )
    FT_INVALID_GLYPH_ID;
                         ...
                         GXV_Validator gxvalid )
{
  ...
  for ( entry = 0; entry <= maxEntry; entry++ )
  {
    ...
    if ( gxvalid->statetable.entry_validate_func )
      gxvalid->statetable.entry_validate_func( state, flags, &glyphOffset,
                                               statetable_table,
                                               statetable_limit, gxvalid );
  }
...
gxv_StateTable_validate( FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid )
{
  ...
  setup_func( (FT_UShort)( limit - table ), classTable, stateArray, entryTable,
              &classTable_length, &stateArray_length, &entryTable_length,
              gxvalid );

  GXV_TRACE( ( "StateTable Subtables\n" ) );

  if ( classTable != 0 )
    gxv_ClassTable_validate( table + classTable, &classTable_length, stateSize,
                             &maxClassID, gxvalid );
  else
    maxClassID = (FT_Byte)( stateSize - 1 );

  if ( stateArray != 0 )
    gxv_StateArray_validate( table + stateArray, &stateArray_length, maxClassID,
                             stateSize, &maxState, &maxEntry, gxvalid );
  else
  {
#if 0
  ...
  if ( entryTable != 0 )
    gxv_EntryTable_validate( table + entryTable, &entryTable_length, maxEntry,
                             stateArray, stateArray_length, maxClassID, table,
                             limit, gxvalid );
                                    ...
                                    GXV_Validator       gxvalid )
{
  ...
  /* XXX: check range? */
  offset =
      (FT_UShort)( base_value_p->u + relative_gindex * sizeof( FT_UShort ) );
                          ...
                          GXV_Validator gxvalid )
{
  ...
  for ( entry = 0; entry <= maxEntry; entry++ )
  {
    ...
    if ( gxvalid->xstatetable.entry_validate_func )
      gxvalid->xstatetable.entry_validate_func( state, flags, &glyphOffset,
                                                xstatetable_table,
                                                xstatetable_limit, gxvalid );
  }
                          ...
                          GXV_Validator gxvalid )
{
  ...
  setup_func( (FT_ULong)( limit - table ), classTable, stateArray, entryTable,
              &classTable_length, &stateArray_length, &entryTable_length,
              gxvalid );
  ...
  if ( classTable != 0 )
  {
    ...
    gxv_LookupTable_validate( table + classTable,
                              table + classTable + classTable_length, gxvalid );
#if 0
  ...
  if ( stateArray != 0 )
    gxv_XStateArray_validate(
        table + stateArray, &stateArray_length, gxvalid->xstatetable.maxClassID,
        gxvalid->xstatetable.nClasses, &maxState, &maxEntry, gxvalid );
  else
  {
#if 0
      maxState = 1; /* 0:start of text, 1:start of line are predefined */
  ...
  if ( entryTable != 0 )
    gxv_XEntryTable_validate(
        table + entryTable, &entryTable_length, maxEntry, stateArray_length,
        gxvalid->xstatetable.maxClassID, table, limit, gxvalid );
  ...
  for ( i = 0; i < odtect->nRanges; i++ )
    for ( j = 0; j < i; j++ )
      if ( 0 != gxv_compare_ranges(
                    odtect->range[i].start, odtect->range[i].length,
                    odtect->range[j].start, odtect->range[j].length ) )
      {
