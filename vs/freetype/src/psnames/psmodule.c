...
static void
ps_check_extra_glyph_unicode( FT_UInt32 uni_char, FT_UInt* states )
{
  FT_UInt n;
                  ...
                  FT_Pointer           glyph_data )
{
  ...
  if ( !FT_QNEW_ARRAY( table->maps, num_glyphs + EXTRA_GLYPH_LIST_SIZE ) )
  {
    ...
    else
    {
      ...
      {
        FT_MEM_QRENEW_ARRAY( table->maps, num_glyphs + EXTRA_GLYPH_LIST_SIZE,
                             count );
        error = FT_Err_Ok;
      }
...
ps_unicodes_char_index( PS_Unicodes table, FT_UInt32 unicode )
{
...
FT_CALLBACK_DEF( FT_UInt )
ps_unicodes_char_next( PS_Unicodes table, FT_UInt32* unicode )
{
  FT_UInt   result    = 0;
...
static FT_Pointer
psnames_get_service( FT_Module module, const char* service_id )
{
  FT_UNUSED( module );
...
FT_DEFINE_MODULE(
    ...
    PUT_PS_NAMES_SERVICE(
        psnames_get_service ) /* FT_Module_Requester   get_interface */
)
