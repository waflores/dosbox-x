...
dlg_init_fls( PINIT_ONCE io, void* param, void** lpContext )
{
  (void)io;
  (void)param;
...
static bool
win_write_heap( void* handle, int needed, const char* format, va_list args )
{
  ...
  bool ret =
      ( needed != 0 && WriteConsoleW( handle, buf2, needed, NULL, NULL ) != 0 );
  free( buf1 );
  return ret;
}

static bool
win_write_stack( void* handle, int needed, const char* format, va_list args )
{
  ...
  needed = MultiByteToWideChar( CP_UTF8, 0, buf1, needed, buf2, needed + 1 );
  return ( needed != 0 &&
           WriteConsoleW( handle, buf2, needed, NULL, NULL ) != 0 );
}
...
// general
void
dlg_escape_sequence( struct dlg_style style, char buf[12] )
{
  int          nums[3];
...
int
dlg_vfprintf( FILE* stream, const char* format, va_list args )
{
...
int
dlg_fprintf( FILE* stream, const char* format, ... )
{
  va_list args;
...
int
dlg_styled_fprintf( FILE*            stream,
...
void
dlg_generic_output( dlg_generic_output_handler output,
                    void*                      data,
                    ...
                    const struct dlg_style     styles[6] )
{
  ...
  if ( features & ( dlg_output_time | dlg_output_file_line | dlg_output_tags |
                    dlg_output_func ) )
  {
...
void
dlg_generic_outputf( dlg_generic_output_handler output,
                     void*                      data,
...
static void
print_size( void* size, const char* format, ... )
{
  va_list args;
  va_start( args, format );
...
static void
print_buf( void* dbuf, const char* format, ... )
{
...
void
dlg_generic_output_buf( char*                    buf,
                        size_t*                  size,
                        unsigned int             features,
                        const struct dlg_origin* origin,
                        const char*              string,
...
void
dlg_generic_outputf_buf( char*                    buf,
                         size_t*                  size,
                         const char*              format_string,
                         const struct dlg_origin* origin,
                         const char*              string,
                         const struct dlg_style   styles[6] )
{
  if ( buf )
  {
    ...
    mbuf.buf  = buf;
    mbuf.size = size;
    dlg_generic_outputf( print_buf, &mbuf, format_string, origin, string,
                         styles );
  }
  else
  {
    *size = 0;
    dlg_generic_outputf( print_size, size, format_string, origin, string,
                         styles );
  }
}

static void
print_stream( void* stream, const char* format, ... )
{
  va_list args;
  va_start( args, format );
...
void
dlg_generic_output_stream( FILE*                    stream,
                           unsigned int             features,
                           const struct dlg_origin* origin,
                           const char*              string,
                           const struct dlg_style   styles[6] )
...
void
dlg_generic_outputf_stream( FILE*                    stream,
                            const char*              format_string,
                            const struct dlg_origin* origin,
                            const char*              string,
                            const struct dlg_style   styles[6],
                            bool                     lock_stream )
{
  ...
  dlg_generic_outputf( print_stream, stream, format_string, origin, string,
                       styles );
  if ( lock_stream )
  {
    unlock_file( stream );
...
void
dlg_default_output( const struct dlg_origin* origin,
                    const char*              string,
                    void*                    data )
{
  FILE*        stream = data ? (FILE*)data : stdout;
  unsigned int features =
      dlg_output_file_line | dlg_output_newline | dlg_output_threadsafe;
  ...
  dlg_generic_output_stream( stream, features, origin, string,
                             dlg_default_output_styles );
  fflush( stream );
}

bool
dlg_win_init_ansi( void )
{
#if defined( DLG_OS_WIN ) && defined( DLG_WIN_CONSOLE )
  // TODO: use init once
  ...
  while ( status == 1 )
    ;  // currently initialized in another thread, spinlock
  return ( status == 4 );
#else
  return true;
#endif
...
static void*
vec_do_create( unsigned int typesize, unsigned int cap, unsigned int size )
{
...
static void
vec_do_erase( void* vec, unsigned int pos, unsigned int size )
{
...
static void*
vec_do_add( void** vec, unsigned int size )
{
  unsigned int* begin  = vec__raw( *vec );
...
#define vec_create( type, size ) \
  (type*)vec_do_create( sizeof( type ), size * 2, size )
#define vec_create_reserve( type, size, capacity ) \
  (type*)vec_do_create( sizeof( type ), capcity, size )
#define vec_init( array, size ) \
  array = vec_do_create( sizeof( *array ), size * 2, size )
#define vec_init_reserve( array, size, capacity ) \
  *( (void**)&array ) = vec_do_create( sizeof( *array ), capacity, size )
#define vec_free( vec ) ( free( ( vec ) ? vec__raw( vec ) : NULL ), vec = NULL )
#define vec_erase_range( vec, pos, count ) \
  vec_do_erase( vec, pos * sizeof( *vec ), count * sizeof( *vec ) )
#define vec_erase( vec, pos ) \
  vec_do_erase( vec, pos * sizeof( *vec ), sizeof( *vec ) )
...
#define vec_addc( vec, count ) \
  ( vec_do_add( (void**)&vec, sizeof( *vec ) * count ) )
#define vec_push( vec, value ) \
  ( vec_do_add( (void**)&vec, sizeof( *vec ) ), vec_last( vec ) = ( value ) )
...
void
dlg_add_tag( const char* tag, const char* func )
{
...
bool
dlg_remove_tag( const char* tag, const char* func )
{
  struct dlg_data* data = dlg_data();
...
void
dlg_set_handler( dlg_handler handler, void* data )
{
  g_handler = handler;
  g_data    = data;
}
...
const char*
dlg__printf_format( const char* str, ... )
{
  va_list vlist;
  va_start( vlist, str );
...
void
dlg__do_log( enum dlg_level     lvl,
...
const char*
dlg__strip_root_path( const char* file, const char* base )
{
  if ( !file )
  {
    return NULL;
  ...
  {  // relative path detected
    while ( *( ++file ) == '.' || *file == '/' || *file == '\\' )
      ;
    if ( *file == '\0' )
    {  // weird case: purely relative path without file
      return saved;
    }
  ...
  if ( base )
  {
    ...
    if ( fn == '\0' || bn != '\0' )
    {  // weird case: base isn't prefix of file
      return saved;
    }
  }
