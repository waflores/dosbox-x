                            ...
                            const FT_Vector* origin )
{
  ...
  case FT_RENDER_MODE_MONO:
    ...
    {
      if ( ( ( cbox.xMin + 31 ) & 63 ) - 31 + ( ( cbox.xMax + 32 ) & 63 ) - 32 <
           0 )
        pbox.xMin -= 1;
      else
        pbox.xMax += 1;
    ...
    {
      if ( ( ( cbox.yMin + 31 ) & 63 ) - 31 + ( ( cbox.yMax + 32 ) & 63 ) - 32 <
           0 )
        pbox.yMin -= 1;
      else
        pbox.yMax += 1;
...
static void
ft_glyphslot_grid_fit_metrics( FT_GlyphSlot slot, FT_Bool vertical )
{
...
FT_Load_Glyph( FT_Face face, FT_UInt glyph_index, FT_Int32 load_flags )
{
  ...
  {
    error =
        driver->clazz->load_glyph( slot, face->size, glyph_index, load_flags );
    if ( error )
      goto Exit;
...
Load_Ok:
    ...
    if ( internal->transform_flags )
    {
      ...
      else if ( slot->format == FT_GLYPH_FORMAT_OUTLINE )
      {
        ...
        if ( internal->transform_flags & 2 )
          FT_Outline_Translate( &slot->outline, internal->transform_delta.x,
  ...
  FT_TRACE5(
      ( "  bitmap %dx%d %s, %s (mode %d)\n", slot->bitmap.width,
        slot->bitmap.rows,
        slot->outline.points ? slot->bitmap.buffer ? "rendered" : "preset"
        : slot->internal->flags & FT_GLYPH_OWN_BITMAP ? "owned"
                                                      : "unowned",
        pixel_modes[slot->bitmap.pixel_mode], slot->bitmap.pixel_mode ) );
  FT_TRACE5( ( "\n" ) );
...
static void
destroy_size( FT_Memory memory, void* size_, void* driver_ )
{
...
static void
destroy_charmaps( FT_Face face, FT_Memory memory )
{
  FT_Int n;
...
static void
destroy_face( FT_Memory memory, void* face_, void* driver_ )
{
...
{
  FT_List_Finalize( &driver->faces_list, destroy_face, driver->root.memory,
                    driver );
}
           ...
           FT_Face*      aface )
{
  ...
  *astream           = face->stream;
  *anexternal_stream = ( face->face_flags & FT_FACE_FLAG_EXTERNAL_STREAM ) != 0;
  if ( error )
    goto Fail;
                       ...
                       FT_Face*    aface )
{
  ...
  error = new_memory_stream( library, base, size, memory_stream_close,
                             &args.stream );
  if ( error )
  {
    FT_FREE( base );
                        ...
                        FT_Face*   aface )
{
  ...
  return open_face_from_buffer( library, pfb_data, pfb_pos, face_index, "type1",
                                aface );
             ...
             FT_Face*   aface )
{
  ...
  dlen = ( header[0x53] << 24 ) | ( header[0x54] << 16 ) |
         ( header[0x55] << 8 ) | header[0x56];
#if 0
    rlen = ( header[0x57] << 24 ) |
           ( header[0x58] << 16 ) |
           ( header[0x59] <<  8 ) |
...
#define FT_COMPONENT raccess
  ...
  for ( i = 0; i < FT_RACCESS_N_RULES; i++ )
  {
    ...
    FT_TRACE3(
        ( "Try rule %d: %s (offset=%ld) ...", i, args2.pathname, offsets[i] ) );
...
#define FT_COMPONENT objs
  ...
  if ( ( FT_ERR_EQ( error, Unknown_File_Format ) ||
       ...
       ( args->flags & FT_OPEN_PATHNAME ) )
    error =
        load_face_in_embedded_rfork( library, stream, face_index, aface, args );
  return error;
}
#endif
    ...
    for ( ; cur < limit; cur++ )
    {
      ...
      if ( FT_MODULE_IS_DRIVER( cur[0] ) )
      {
             ...
             FT_ERR_EQ( error, Table_Missing ) )
        {
          ...
          error = open_face_PS_from_sfnt_stream( library, stream, face_index,
                                                 num_params, params, aface );
          if ( !error )
          {
            FT_Stream_Free( stream, external_stream );
            return error;
...
static void
ft_recompute_scaled_metrics( FT_Face face, FT_Size_Metrics* metrics )
{
...
FT_Select_Size( FT_Face face, FT_Int strike_index )
{
...
#ifdef FT_DEBUG_LEVEL_TRACE
  {
    ...
    FT_TRACE5( ( "  x scale: %ld (%f)\n", metrics->x_scale,
                 (double)metrics->x_scale / 65536 ) );
    FT_TRACE5( ( "  y scale: %ld (%f)\n", metrics->y_scale,
                 (double)metrics->y_scale / 65536 ) );
            ...
            if ( akerning->x != orig_x_rounded ||
              ...
              FT_TRACE5(
                  ( "FT_Get_Kerning: horizontal kerning"
                    ...
                    orig_x_rounded / 64, orig_y_rounded / 64, akerning->x / 64,
                    akerning->y / 64 ) );
          }
#endif
        }
  ...
  if ( cmap )
  {
    ...
    for ( i = 0; i < face->num_charmaps; i++ )
    {
      if ( (FT_CMap)face->charmaps[i] == cmap )
      {
        ...
        if ( FT_QRENEW_ARRAY( face->charmaps, face->num_charmaps,
                              face->num_charmaps - 1 ) )
          return;
                             ...
                             FT_ULong variantSelector )
{
  ...
  if ( face && face->charmap && face->charmap->encoding == FT_ENCODING_UNICODE )
  {
    ...
    if ( charmap )
    {
      ...
      result = vcmap->clazz->char_var_index( vcmap, ucmap, (FT_UInt32)charcode,
...
FT_Face_GetVariantsOfChar( FT_Face face, FT_ULong charcode )
{
  ...
  if ( face )
  {
    ...
    if ( charmap )
    {
      ...
      result =
          vcmap->clazz->charvariant_list( vcmap, memory, (FT_UInt32)charcode );
    }
  }
  return result;
}
                          ...
                          FT_Render_Mode render_mode )
{
  ...
  default:
    if ( slot->internal->load_flags & FT_LOAD_COLOR )
    {
      ...
      iterator.p  = NULL;
      have_layers = FT_Get_Color_Glyph_Layer( face, base_glyph, &glyph_index,
                                              &color_index, &iterator );
      if ( have_layers )
      {
        error = FT_New_GlyphSlot( face, NULL );
        if ( !error )
        {
          ...
          } while ( FT_Get_Color_Glyph_Layer( face, base_glyph, &glyph_index,
                                              &color_index, &iterator ) );
...
#define FT_COMPONENT checksum
  ...
  if ( !error && ft_trace_levels[trace_checksum] >= 7 && slot->bitmap.buffer )
  {
...
#define FREETYPE_VER_FIXED \
  ( ( (FT_Long)FREETYPE_MAJOR << 16 ) | FREETYPE_MINOR )
                          ...
                          FT_LayerIterator* iterator )
{
  ...
  if ( sfnt->get_colr_layer )
    return sfnt->get_colr_layer( ttface, base_glyph, aglyph_index, acolor_index,
                                 iterator );
  else
    return 0;
}
                          ...
                          FT_OpaquePaint*         paint )
{
  ...
  if ( sfnt->get_colr_layer )
    return sfnt->get_colr_glyph_paint( ttface, base_glyph, root_transform,
                                       paint );
  else
    return 0;
}
                            ...
                            FT_ClipBox* clip_box )
{
  ...
  if ( sfnt->get_color_glyph_clipbox )
    return sfnt->get_color_glyph_clipbox( ttface, base_glyph, clip_box );
  else
    return 0;
}
