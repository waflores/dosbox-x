...
static FT_Pointer
ft_mem_table_alloc( FT_MemTable table, FT_Long size )
{
...
static void
ft_mem_table_free( FT_MemTable table, FT_Pointer block )
{
  FT_Memory memory = table->memory;
...
ft_mem_table_resize( FT_MemTable table )
{
  ...
  if ( new_size != table->size )
  {
    ...
    new_buckets = (FT_MemNode*)ft_mem_table_alloc(
        table, new_size * (FT_Long)sizeof( FT_MemNode ) );
    if ( !new_buckets )
      return;
...
static FT_MemNode*
ft_mem_table_get_nodep( FT_MemTable table, FT_Byte* address )
{
  FT_PtrDist  hash;
    ...
    if ( node )
    {
      ...
      else
      {
        ...
        ft_mem_debug_panic(
            ...
            " %p, of size %ld)\n"
            "org=%s:%d new=%s:%d\n",
            node->address, node->size, FT_FILENAME( node->source->file_name ),
            node->source->line_no, FT_FILENAME( ft_debug_file_ ),
            ft_debug_lineno_ );
      }
    }
...
static void
ft_mem_table_remove( FT_MemTable table, FT_Byte* address, FT_Long delta )
{
  if ( table )
  {
    FT_MemNode *pnode, node;
    ...
    if ( node )
    {
      ...
      if ( node->size < 0 )
        ft_mem_debug_panic(
            ...
            address, FT_FILENAME( ft_debug_file_ ), ft_debug_lineno_,
            FT_FILENAME( node->source->file_name ), node->source->line_no,
      ...
      ft_mem_debug_panic( "trying to free unknown block at %p in (%s:%ld)\n",
                          address, FT_FILENAME( ft_debug_file_ ),
                          ft_debug_lineno_ );
  }
}

static FT_Pointer
ft_mem_debug_alloc( FT_Memory memory, FT_Long size )
{
...
static void
ft_mem_debug_free( FT_Memory memory, FT_Pointer block )
{
  FT_MemTable table = (FT_MemTable)memory->user;
                      ...
                      FT_Pointer block )
{
  ...
  if ( new_size <= 0 )
    ft_mem_debug_panic(
        "trying to reallocate %p to size 0 (current is %ld) in (%s:%ld)", block,
        cur_size, file_name, line_no );
  ...
  ft_memcpy( new_block, block,
             cur_size < new_size ? (size_t)cur_size : (size_t)new_size );
...
FT_COMPARE_DEF( int )
ft_mem_source_compare( const void* p1, const void* p2 )
{
...
FT_DumpMemory( FT_Memory memory )
{
  if ( memory->free == ft_mem_debug_free )
  {
    ...
    ft_qsort( sources, (size_t)count, sizeof( *sources ),
    ...
    for ( nn = 0; nn < count; nn++ )
    {
      ...
      printf( fmt, source->cur_blocks, source->max_blocks, source->cur_size,
              source->max_size, source->cur_max,
