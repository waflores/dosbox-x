      ...
      default: /* FT_CURVE_TAG_CUBIC */
      {
        ...
        if ( point <= limit )
        {
          ...
          FT_TRACE5( (
              ...
              (double)vec.x / 64, (double)vec.y / 64, (double)vec1.x / 64,
              (double)vec1.y / 64, (double)vec2.x / 64, (double)vec2.y / 64 ) );
        ...
        FT_TRACE5( (
            "  cubic to (%.2f, %.2f)"
            " with controls (%.2f, %.2f) and (%.2f, %.2f)\n",
            (double)v_start.x / 64, (double)v_start.y / 64, (double)vec1.x / 64,
            (double)vec1.y / 64, (double)vec2.x / 64, (double)vec2.y / 64 ) );
...
#define FT_OUTLINE_GET_CONTOUR( outline, c, first, last )                    \
  ...
  {                                                                          \
    ( first ) = ( c > 0 )                                                    \
                    ? ( outline )->points + ( outline )->contours[c - 1] + 1 \
                    : ( outline )->points;                                   \
...
FT_Outline_Get_Orientation( FT_Outline* outline )
{
  ...
  xshift =
      FT_MSB( (FT_UInt32)( FT_ABS( cbox.xMax ) | FT_ABS( cbox.xMin ) ) ) - 14;
  ...
  for ( c = 0; c < outline->n_contours; c++ )
  {
    ...
    for ( n = first; n <= last; n++ )
    {
      ...
      area =
          ADD_LONG( area, MUL_LONG( v_cur.y - v_prev.y, v_cur.x + v_prev.x ) );
