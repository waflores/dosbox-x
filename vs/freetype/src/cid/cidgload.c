                            ...
                            FT_ULong* off2_p )
{
  ...
  if ( fd_select >= cid->num_dicts )
  {
    ...
    {
      FT_TRACE1(
          ( "cid_load_glyph: fail for glyph index %d:\n", glyph_index ) );
      FT_TRACE1(
          ( "                FD number %ld is the maximum\n", fd_select ) );
...
cid_load_glyph( T1_Decoder decoder, FT_UInt glyph_index )
{
...
#endif /* FT_CONFIG_OPTION_INCREMENTAL */
  {
    ...
    error = cid_compute_fd_and_offsets( face, glyph_index, &fd_select, &off1,
                                        &off2 );
    if ( error )
      goto Exit;
    ...
    if ( glyph_length == 0 || FT_QALLOC( charstring, glyph_length ) ||
         FT_STREAM_READ_AT( cid->data_offset + off1, charstring,
                            glyph_length ) )
      goto Exit;
  }
  ...
  /* Now set up the subrs array and parse the charstrings. */
  {
    ...
    {
      FT_TRACE0(
          ( "cid_load_glyph: fail for glyph_index=%d, "
