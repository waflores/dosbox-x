...
cid_get_offset( FT_Byte** start, FT_UInt offsize )
{
  FT_ULong result;
  FT_Byte* p = *start;
...
static FT_Error
cid_load_keyword( CID_Face face, CID_Loader* loader, const T1_Field keyword )
{
  FT_Error     error;
  ...
  if ( keyword->type == T1_FIELD_TYPE_INTEGER_ARRAY ||
    ...
    error =
        cid_parser_load_field_table( &loader->parser, keyword, &dummy_object );
  else
        ...
        T1_FIELD_CALLBACK( "FontMatrix", cid_parse_font_matrix, 0 )
            T1_FIELD_CALLBACK( "ExpansionFactor", parse_expansion_factor, 0 )
                T1_FIELD_CALLBACK( "FontName", parse_font_name, 0 )
                    ...
                    { 0, T1_FIELD_LOCATION_CID_INFO, T1_FIELD_TYPE_NONE, 0, 0,
                      0, 0, 0, 0 } };
...
static void
cid_init_loader( CID_Loader* loader, CID_Face face )
{
  FT_UNUSED( face );
    ...
    if ( FT_QALLOC( face->binary_data, parser->binary_length ) ||
         FT_SET_ERROR(
             cid_hex_to_binary( face->binary_data, parser->binary_length,
                                parser->data_offset, face, &binary_length ) ) )
      goto Exit;

    FT_Stream_OpenMemory( face->cid_stream, face->binary_data, binary_length );
    cid->data_offset = 0;
  }
  else
  {
