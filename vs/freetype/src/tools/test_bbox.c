...
#if 1
    XVEC( 408.9111, 535.3164 ), XVEC( 455.8887, 634.396 ),
    XVEC( -37.8765, 786.2207 ), XVEC( 164.6074, 535.3164 )
#else
...
static char dummy_tag_1[4] = { FT_CURVE_TAG_ON, FT_CURVE_TAG_CUBIC,
...
/* dummy outline #2 */
static FT_Vector dummy_vec_2[4] = { XVEC( 100.0, 100.0 ), XVEC( 100.0, 200.0 ),
                                    XVEC( 200.0, 200.0 ),
                                    XVEC( 200.0, 133.0 ) };
...
static FT_Vector dummy_vec_3[4] = { XVEC( 100.0, 127.0 ), XVEC( 200.0, 127.0 ),
                                    XVEC( 0.0, 136.0 ), XVEC( 0.0, 100.0 ) };

static FT_Outline dummy_outline_3 = {
    1, 4, dummy_vec_3, dummy_tag_1, dummy_contour_1, 0 };
...
dump_outline( FT_Outline* outline )
{
  ...
  printf( "cbox = [%.2f %.2f %.2f %.2f]\n", XVAL( bbox.xMin ),
          XVAL( bbox.yMin ), XVAL( bbox.xMax ), XVAL( bbox.yMax ) );
...
static void
profile_outline( FT_Outline* outline, long repeat )
{
  FT_BBox bbox;
  long    count;
  long    time0;
