...
dump_fontinfo( AFM_FontInfo fi )
{
  ...
  printf( "FontBBox: %.2f %.2f %.2f %.2f\n", fi->FontBBox.xMin / 65536.,
          fi->FontBBox.yMin / 65536., fi->FontBBox.xMax / 65536.,
  ...
  for ( i = 0; i < fi->NumTrackKern; i++ )
  {
    ...
    printf( "\t%2d: %5.2f %5.2f %5.2f %5.2f\n", tk->degree,
            tk->min_ptsize / 65536., tk->min_kern / 65536.,
  ...
  for ( i = 0; i < fi->NumKernPair; i++ )
  {
    ...
    printf( "\t%3d + %3d => (%4d, %4d)\n", kp->index1, kp->index2, kp->x,
            kp->y );
  }
}

int
dummy_get_index( const char* name, FT_Offset len, void* user_data )
{
  if ( len )
    return name[0];
  else
...
FT_Error
parse_afm( FT_Library library, FT_Stream stream, AFM_FontInfo fi )
{
  PSAux_Service psaux;
  ...
  error = psaux->afm_parser_funcs->init( &parser, library->memory,
