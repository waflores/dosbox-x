 ...
 */
static Bool
New_Profile( RAS_ARGS TStates aState, Bool overshoot )
{
  if ( !ras.fProfile )
  {
 ...
 */
static Bool
Line_Up( RAS_ARGS Long x1, Long y1, Long x2, Long y2, Long miny, Long maxy )
{
  Long Dx, Dy;
 ...
 */
static Bool
Line_Down( RAS_ARGS Long x1, Long y1, Long x2, Long y2, Long miny, Long maxy )
{
  Bool result, fresh;
 ...
 */
static Bool
Line_To( RAS_ARGS Long x, Long y )
{
 ...
 */
static Bool
Conic_To( RAS_ARGS Long cx, Long cy, Long x, Long y )
{
  ...
  do
  {
    ...
    else
    {
      ...
      else if ( Bezier_Down( RAS_VARS 2, arc, Split_Conic, ras.minY,
                             ras.maxY ) )
        goto Fail;
      arc -= 2;
    }
 ...
 */
static Bool
Cubic_To( RAS_ARGS Long cx1, Long cy1, Long cx2, Long cy2, Long x, Long y )
{
  ...
  do
  {
    ...
    else
    {
      ...
      else if ( Bezier_Down( RAS_VARS 3, arc, Split_Cubic, ras.minY,
                             ras.maxY ) )
        goto Fail;
      arc -= 3;
    }
 ...
 */
static Bool
Decompose_Curve( RAS_ARGS Int first, Int last, Int flipped )
{
  FT_Vector v_last;
  FT_Vector v_control;
  FT_Vector v_start;
 ...
 */
static void
InsNew( PProfileList list, PProfile profile )
{
 ...
 */
static void
DelOld( PProfileList list, const PProfile profile )
{
  PProfile *old, current;
...
static void
Vertical_Sweep_Init( RAS_ARGS Short min, Short max )
{
  FT_UNUSED( max );
...
static void
Horizontal_Sweep_Init( RAS_ARGS Short min, Short max )
{
  /* nothing, really */
  FT_UNUSED_RASTER;
  FT_UNUSED( min );
 ...
 */
static int
Render_Single_Pass( RAS_ARGS Bool flipped, Int y_min, Int y_max )
{
  Int y_mid;
  Int band_top = 0;
...
static int
ft_black_new( void* memory, FT_Raster* araster )
{
...
static void
ft_black_reset( FT_Raster raster, PByte pool_base, ULong pool_size )
{
  FT_UNUSED( raster );
  FT_UNUSED( pool_base );
  FT_UNUSED( pool_size );
...
static int
ft_black_set_mode( FT_Raster raster, ULong mode, void* args )
{
  FT_UNUSED( raster );
  FT_UNUSED( mode );
  FT_UNUSED( args );
...
static int
ft_black_render( FT_Raster raster, const FT_Raster_Params* params )
{
