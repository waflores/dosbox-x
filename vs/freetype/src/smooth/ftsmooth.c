...
static FT_Error
ft_smooth_set_mode( FT_Renderer render, FT_ULong mode_tag, FT_Pointer data )
{
...
static void
ft_smooth_get_cbox( FT_Renderer render, FT_GlyphSlot slot, FT_BBox* cbox )
{
  FT_ZERO( cbox );
                      ...
                      FT_Bitmap*  bitmap )
{
  ...
  if ( bitmap->pitch < 0 )
    ...
    target.origin =
        bitmap->buffer + ( bitmap->rows - 1 ) * (unsigned int)bitmap->pitch;
...
static void
ft_smooth_overlap_spans( int y, int count, const FT_Span* spans, void* target_ )
{
  TOrigin* target = (TOrigin*)target_;
