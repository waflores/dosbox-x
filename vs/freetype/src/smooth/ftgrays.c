...
#define ADD_INT( a, b ) (int)( (unsigned int)( a ) + (unsigned int)( b ) )
...
int
FT_Throw( int error, int line, const char* file )
{
  FT_UNUSED( error );
  FT_UNUSED( line );
  FT_UNUSED( file );
...
#define FT_THROW( e )                                              \
  ( FT_Throw( FT_ERR_CAT( Smooth_Err_, e ), __LINE__, __FILE__ ) | \
 ...
 */
static void
gray_set_cell( RAS_ARG_ TCoord ex, TCoord ey )
{
 ...
 */
static void
gray_render_line( RAS_ARG_ TPos to_x, TPos to_y )
{
 ...
 */
#if defined( __SSE2__ ) || ( defined( __x86_64__ ) && !defined( __VMS ) ) || \
...
#if FT_SSE2 || defined( __aarch64__ ) || defined( _M_ARM64 )
...
static void
gray_render_conic( RAS_ARG_ const FT_Vector* control, const FT_Vector* to )
{
  FT_Vector p0, p1, p2;
  ...
  if ( ( TRUNC( p0.y ) >= ras.max_ey && TRUNC( p1.y ) >= ras.max_ey &&
         TRUNC( p2.y ) >= ras.max_ey ) ||
       ( TRUNC( p0.y ) < ras.min_ey && TRUNC( p1.y ) < ras.min_ey &&
...
static int
gray_move_to( const FT_Vector* to, void* worker_ ) /* gray_PWorker */
{
...
static int
gray_line_to( const FT_Vector* to, void* worker_ ) /* gray_PWorker */
{
  gray_PWorker worker = (gray_PWorker)worker_;
    ...
    while ( point < limit )
    {
      ...
      Do_Conic:
        if ( point < limit )
        {
          ...
          FT_TRACE5(
              ( "  conic to (%.2f, %.2f)"
                " with control (%.2f, %.2f)\n",
                v_middle.x / 64.0, v_middle.y / 64.0, v_control.x / 64.0,
                v_control.y / 64.0 ) );
      ...
      default: /* FT_CURVE_TAG_CUBIC */
      {
        ...
        if ( point <= limit )
        {
          ...
          FT_TRACE5(
              ( "  cubic to (%.2f, %.2f)"
                ...
                vec.x / 64.0, vec.y / 64.0, vec1.x / 64.0, vec1.y / 64.0,
        ...
        FT_TRACE5(
            ( "  cubic to (%.2f, %.2f)"
              " with controls (%.2f, %.2f) and (%.2f, %.2f)\n",
              v_start.x / 64.0, v_start.y / 64.0, vec1.x / 64.0, vec1.y / 64.0,
              vec2.x / 64.0, vec2.y / 64.0 ) );
...
static int
gray_convert_glyph_inner( RAS_ARG_ int continued )
{
  volatile int error;
...
static int
gray_raster_render( FT_Raster raster, const FT_Raster_Params* params )
{
...
static int
gray_raster_new( void* memory, FT_Raster* araster )
{
...
static int
gray_raster_new( void* memory_, FT_Raster* araster_ )
{
...
static int
gray_raster_set_mode( FT_Raster raster, unsigned long mode, void* args )
{
  FT_UNUSED( raster );
  FT_UNUSED( mode );
  FT_UNUSED( args );
