  ...
  if ( FT_IS_SFNT( face ) )
  {
    ...
    if ( flags & FT_LOAD_VERTICAL_LAYOUT )
    {
      ...
      for ( nn = 0; nn < count; nn++ )
      {
        ...
        ( (SFNT_Service)cffface->sfnt )
            ->get_metrics( cffface, 1, start + nn, &dummy, &ah );
    ...
    else
    {
      ...
      for ( nn = 0; nn < count; nn++ )
      {
        ...
        ( (SFNT_Service)cffface->sfnt )
            ->get_metrics( cffface, 0, start + nn, &dummy, &aw );
 ...
 */
FT_CALLBACK_DEF( FT_Error )
cff_get_cmap_info( FT_CharMap charmap, TT_CMapInfo* cmap_info )
{
                     ...
                     FT_Int* avalue )
{
  ...
  FT_Service_MetricsVariations var =
      (FT_Service_MetricsVariations)cffface->tt_var;
...
FT_DEFINE_SERVICE_CFFLOADREC(
    ...
    cff_get_standard_encoding, /* FT_Get_Standard_Encoding_Func
