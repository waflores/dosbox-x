               ...
               FT_Int32      load_flags )
{
       ...
       face->svg )
  {
    ...
    if ( size &&
         ( size->root.metrics.x_ppem < 1 || size->root.metrics.y_ppem < 1 ) )
    {
    ...
    if ( !error )
    {
       ...
       */
      sfnt->get_metrics( face, FALSE, glyph_index, &dummy, &advanceX );
      sfnt->get_metrics( face, TRUE, glyph_index, &dummy, &advanceY );
    ...
    else
    {
      ...
      if ( face->horizontal.number_Of_HMetrics )
      {
        ...
        ( (SFNT_Service)face->sfnt )
            ->get_metrics( face, 0, glyph_index, &horiBearingX, &horiAdvance );
      ...
      if ( has_vertical_info )
      {
        ...
        ( (SFNT_Service)face->sfnt )
            ->get_metrics( face, 1, glyph_index, &vertBearingY, &vertAdvance );
      ...
      if ( has_vertical_info )
      {
        ...
        metrics->vertBearingY =
            FT_MulFix( metrics->vertBearingY, glyph->y_scale );
      }
      else
      {
        if ( load_flags & FT_LOAD_VERTICAL_LAYOUT )
