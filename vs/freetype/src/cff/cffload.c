...
static FT_ULong
cff_index_read_offset( CFF_Index idx, FT_Error* errorp )
{
  FT_Error  error;
...
static FT_Error
cff_index_init( CFF_Index idx, FT_Stream stream, FT_Bool load, FT_Bool cff2 )
{
  FT_Error  error;
                        ...
                        FT_ULong*  pool_size )
{
  ...
  *table = NULL;
  ...
  if ( idx->count > 0 && !FT_QNEW_ARRAY( tbl, idx->count + 1 ) &&
       ( !pool || !FT_ALLOC( new_bytes, new_size ) ) )
  {
    ...
    for ( n = 1; n <= idx->count; n++ )
    {
      ...
      else
      {
        ...
        if ( next_offset != cur_offset )
        {
          FT_MEM_COPY( tbl[n - 1], org_bytes + cur_offset,
                       tbl[n] - tbl[n - 1] );
          tbl[n][0] = '\0';
          tbl[n] += 1;
          extra++;
...
static void
CFF_Done_FD_Select( CFF_FDSelect fdselect, FT_Stream stream )
{
  if ( fdselect->data )
...
static void
cff_charset_free_cids( CFF_Charset charset, FT_Memory memory )
{
...
static void
cff_charset_done( CFF_Charset charset, FT_Stream stream )
{
  FT_Memory memory = stream->memory;
...
static void
cff_vstore_done( CFF_VStoreRec* vstore, FT_Memory memory )
{
  FT_UInt i;
                 ...
                 FT_ULong       offset )
{
  ...
  if ( offset )
  {
    ...
    if ( FT_READ_ULONG( regionListOffset ) || FT_READ_USHORT( dataCount ) )
      goto Exit;
    ...
    if ( FT_STREAM_SEEK( vsOffset + regionListOffset ) ||
         FT_READ_USHORT( vstore->axisCount ) || FT_READ_USHORT( regionCount ) )
      goto Exit;
...
cff_blend_doBlend( CFF_SubFont subFont, CFF_Parser parser, FT_UInt numBlends )
{
  ...
  if ( subFont->blend_used + size > subFont->blend_alloc )
  {
    ...
    if ( FT_QREALLOC( subFont->blend_stack, subFont->blend_alloc,
                        ...
                        FT_Fixed* NDV )
{
  if ( !blend->builtBV || blend->lastVsindex != vsindex ||
       blend->lenNDV != lenNDV ||
       ( lenNDV &&
         ft_memcmp( NDV, blend->lastNDV, lenNDV * sizeof( *NDV ) ) != 0 ) )
  {
                   ...
                   FT_MM_Var** mm_var )
{
  ...
  return mm->get_var_blend( face, num_coords, coords, normalizedcoords,
                            mm_var );
}
                  ...
                  CFF_Face    face )
{
  ...
  error = cff_parser_init( &parser, code, top, font->library, stackSize, 0, 0 );
  if ( error )
    goto Exit;
...
static void
cff_subfont_done( FT_Memory memory, CFF_SubFont subfont )
{
  if ( subfont )
  {
...
#define FT_STRUCTURE CFF_FontRec
      ...
      FT_FRAME_BYTE( version_minor ), FT_FRAME_BYTE( header_size ),
      FT_FRAME_END };
  ...
  if ( dict->cid_registry != 0xFFFFU || cff2 )
  {
    ...
    for ( idx = 0; idx < fd_index.count; idx++ )
    {
      ...
      error = cff_subfont_load( sub, &fd_index, idx, stream, base_offset,
    ...
    if ( !cff2 || fd_index.count > 1 )
      error = CFF_Load_FD_Select( &font->fd_select,
                                  font->charstrings_index.count, stream,
  ...
  if ( !cff2 && font->num_glyphs > 0 )
  {
    ...
    {
      error =
          cff_encoding_load( &font->encoding, &font->charset, font->num_glyphs,
