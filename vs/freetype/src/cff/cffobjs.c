...
static void
cff_make_private_dict( CFF_SubFont subfont, PS_Private priv )
{
...
static FT_String*
cff_strcpy( FT_Memory memory, const FT_String* source )
{
  FT_Error   error;
  FT_String* result;
...
static void
remove_style( FT_String* family_name, const FT_String* style_name )
{
               ...
               FT_Parameter* params )
{
  ...
  /* now load and parse the CFF table in the file */
  {
    ...
    face->extra.data = cff;
    error =
        cff_font_load( library, stream, face_index, cff, face, pure_cff, cff2 );
    if ( error )
      goto Exit;
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
    {
      ...
      if ( FT_HAS_MULTIPLE_MASTERS( cffface ) && instance_index > 0 )
      {
    ...
    for ( i = cff->num_subfonts; i > 0; i-- )
    {
      ...
      if ( sub->has_font_matrix )
      {
        ...
        if ( top->has_font_matrix )
        {
          ...
          FT_Matrix_Multiply_Scaled( &top->font_matrix, &sub->font_matrix,
                                     scaling );
          FT_Vector_Transform_Scaled( &sub->font_offset, &top->font_matrix,
                                      scaling );

          sub->units_per_em = (FT_ULong)FT_MulDiv(
              (FT_Long)sub->units_per_em, (FT_Long)top->units_per_em, scaling );
        }
      }
      else
      {
