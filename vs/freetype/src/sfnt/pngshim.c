...
premultiply_data( png_structp png, png_row_infop row_info, png_bytep data )
{
  ...
  /* The Intel check must be first. */
    ...
    defined( __OPTIMIZE__ ) && defined( __SSE__ ) &&                        \
...
static void
error_callback( png_structp png, png_const_charp error_msg )
{
...
static void
warning_callback( png_structp png, png_const_charp error_msg )
{
  FT_UNUSED( png );
  FT_UNUSED( error_msg );
               ...
               FT_Bool         metrics_only )
{
  ...
  if ( !populate_map_and_metrics &&
       ( (FT_UInt)x_offset + metrics->width > map->width ||
         (FT_UInt)y_offset + metrics->height > map->rows || pix_bits != 32 ||
  ...
  png = png_create_read_struct( PNG_LIBPNG_VER_STRING, &error, error_callback,
                                warning_callback );
  if ( !png )
  {
    error = FT_THROW( Out_Of_Memory );
  ...
  png_get_IHDR( png, info, &imgWidth, &imgHeight, &bitdepth, &color_type,
                &interlace, NULL, NULL );

  if ( error || ( !populate_map_and_metrics &&
