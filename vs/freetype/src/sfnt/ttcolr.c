    ...
    if ( face->variation_support & TT_FACE_FLAG_VAR_FVAR )
    {
      ...
      if ( var_store_offset )
      {
        ...
        error = mm->load_item_var_store(
            FT_FACE( face ), colr_offset_in_stream + var_store_offset,
            &colr->var_store );
        if ( error != FT_Err_Ok )
          goto InvalidTable;
      }
      ...
      if ( colr->var_store.axisCount && var_idx_map_offset )
      {
        error = mm->load_delta_set_idx_map(
            FT_FACE( face ), colr_offset_in_stream + var_idx_map_offset,
            &colr->delta_set_idx_map, &colr->var_store, table_size );
        if ( error != FT_Err_Ok )
          goto InvalidTable;
      }
    }
                        ...
                        FT_LayerIterator* iterator )
{
  ...
  if ( iterator->layer >= iterator->num_layers || iterator->p < colr->layers ||
...
static FT_Bool
read_paint( TT_Face face, Colr* colr, FT_Byte* p, FT_COLR_Paint* apaint )
{
                         ...
                         FT_COLR_PAINTFORMAT_INTERNAL_VAR_LINEAR_GRADIENT ) ) )
  {
    if ( !read_color_line( colr, child_table_p,
                           &apaint->u.linear_gradient.colorline, do_read_var ) )
      return 0;
                    ...
                    FT_COLR_PAINTFORMAT_INTERNAL_VAR_RADIAL_GRADIENT ) ) )
  {
    ...
    if ( !read_color_line( colr, child_table_p,
                           &apaint->u.radial_gradient.colorline, do_read_var ) )
      return 0;
  ...
  {
    if ( !read_color_line( colr, child_table_p,
                           &apaint->u.sweep_gradient.colorline, do_read_var ) )
      return 0;

    ENSURE_READ_BYTES( 8 );

    apaint->u.sweep_gradient.center.x = INT_TO_FIXED( FT_NEXT_SHORT( p ) );
    apaint->u.sweep_gradient.center.y = INT_TO_FIXED( FT_NEXT_SHORT( p ) );

    apaint->u.sweep_gradient.start_angle =
        F2DOT14_TO_FIXED( FT_NEXT_SHORT( p ) );
    apaint->u.sweep_gradient.end_angle = F2DOT14_TO_FIXED( FT_NEXT_SHORT( p ) );
                              ...
                              FT_OpaquePaint*         opaque_paint )
{
  ...
  if ( colr->version < 1 || !colr->num_base_glyphs_v1 || !colr->base_glyphs_v1 )
    return 0;
                                 ...
                                 FT_ClipBox* clip_box )
{
  ...
  for ( i = 0; i < num_clip_boxes; ++i )
  {
    ...
    if ( base_glyph >= gid_start && base_glyph <= gid_end )
    {
      ...
      font_clip_box.xMin =
          FT_MulFix( FT_NEXT_SHORT( p1 ), face->root.size->metrics.x_scale );
      font_clip_box.yMin =
          FT_MulFix( FT_NEXT_SHORT( p1 ), face->root.size->metrics.y_scale );
      font_clip_box.xMax =
          FT_MulFix( FT_NEXT_SHORT( p1 ), face->root.size->metrics.x_scale );
      font_clip_box.yMax =
          FT_MulFix( FT_NEXT_SHORT( p1 ), face->root.size->metrics.y_scale );
                          ...
                          FT_OpaquePaint*   opaque_paint )
{
   ...
   * 4-byte ULONG.
   */
  if ( p < colr->layers_v1 || p > (FT_Byte*)colr->table + colr->table_size - 4 )
    return 0;
