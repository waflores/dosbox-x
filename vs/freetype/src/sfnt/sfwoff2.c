...
static FT_Error
Read255UShort( FT_Stream stream, FT_UShort* value )
{
...
static FT_Error
ReadBase128( FT_Stream stream, FT_ULong* value )
{
  FT_ULong result = 0;
  FT_Int   i;
  FT_Byte  code;
           ...
           FT_Memory memory )
{
  ...
  if ( ( *offset + size ) > *dst_size )
  {
    ...
    if ( FT_QREALLOC( dst, (FT_ULong)( *dst_size ),
...
static FT_ULong
compute_ULong_sum( FT_Byte* buf, FT_ULong size )
{
  FT_ULong checksum     = 0;
  ...
  for ( i = 0; i < aligned_size; i += 4 )
    checksum += ( (FT_ULong)buf[i] << 24 ) | ( (FT_ULong)buf[i + 1] << 16 ) |
                ( (FT_ULong)buf[i + 2] << 8 ) | ( (FT_ULong)buf[i + 3] << 0 );
                  ...
                  FT_ULong       src_size )
{
  ...
  result = BrotliDecoderDecompress( src_size, src, &uncompressed_size, dst );
...
static WOFF2_Table
find_table( WOFF2_Table* tables, FT_UShort num_tables, FT_Tag tag )
{
  FT_Int i;
...
static FT_Error
read_num_hmetrics( FT_Stream stream, FT_UShort* num_hmetrics )
{
  FT_Error  error = FT_Err_Ok;
...
static FT_Int
with_sign( FT_Byte flag, FT_Int base_val )
{
...
/* An auxiliary function for overflow-safe addition. */
static FT_Int
safe_int_addition( FT_Int a, FT_Int b, FT_Int* result )
{
                ...
                FT_ULong*      in_bytes_used )
{
  ...
  for ( i = 0; i < n_points; ++i )
  {
    ...
    {
      dx = 0;
      dy = with_sign( flag, ( ( flag & 14 ) << 7 ) + in[triplet_index] );
    }
    else if ( flag < 20 )
    {
      dx = with_sign( flag,
    ...
    {
      b0 = flag - 84;
      dx = with_sign( flag, 1 + ( ( b0 / 12 ) << 8 ) + in[triplet_index] );
      dy = with_sign( flag >> 1, 1 + ( ( ( b0 % 12 ) >> 2 ) << 8 ) +
                                     in[triplet_index + 1] );
    }
    else if ( flag < 124 )
    {
      b2 = in[triplet_index + 1];
      dx = with_sign( flag, ( in[triplet_index] << 4 ) + ( b2 >> 4 ) );
      dy = with_sign( flag >> 1,
    ...
    {
      dx =
          with_sign( flag, ( in[triplet_index] << 8 ) + in[triplet_index + 1] );
      dy = with_sign( flag >> 1,
              ...
              FT_ULong*         glyph_size )
{
  ...
  if ( xy_bytes < x_bytes || flag_offset + xy_bytes < flag_offset ||
                  ...
                  FT_Memory  memory )
{
  ...
  expected_loca_length =
      ( index_format ? 4 : 2 ) * ( (FT_ULong)num_glyphs + 1 );
  ...
  for ( i = 0; i < num_substreams; ++i )
  {
    ...
    FT_TRACE5( ( "  Substream %d: offset = %lu; size = %lu;\n", i,
                 substreams[i].offset, substreams[i].size ) );
  ...
  for ( i = 0; i < num_glyphs; ++i )
  {
    ...
    if ( FT_STREAM_SEEK( bbox_offset ) || FT_READ_BYTE( bbox_bitmap ) )
      goto Fail;
    ...
    if ( n_contours == 0xffff )
    {
      ...
      if ( compositeGlyph_size( stream, substreams[COMPOSITE_STREAM].offset,
    ...
    else if ( n_contours > 0 )
    {
      ...
      if ( substreams[GLYPH_STREAM].size < ( substreams[GLYPH_STREAM].offset -
      ...
      if ( triplet_decode( flags_buf, triplet_buf, triplet_size, total_n_points,
      ...
      size_needed =
          12 + ( 2 * n_contours ) + ( 5 * total_n_points ) + instruction_size;
      if ( glyph_buf_size < size_needed )
      {
        if ( FT_QREALLOC( glyph_buf, glyph_buf_size, size_needed ) )
          goto Fail;
      ...
      if ( store_points( total_n_points, points, n_contours, instruction_size,
                         have_overlap, glyph_buf, glyph_buf_size,
                         &glyph_size ) )
        goto Fail;
  ...
  if ( store_loca( loca_values, num_glyphs + 1, index_format, loca_checksum,
            ...
            FT_Memory    memory )
{
  ...
  const WOFF2_Table maxp_table = find_table( tables, num_tables, TTAG_maxp );
  const WOFF2_Table head_table = find_table( tables, num_tables, TTAG_head );
                  ...
                  FT_Memory    memory )
{
  ...
  for ( nn = 0; nn < num_tables; nn++ )
  {
    ...
    FT_TRACE3( ( "Table tag: %c%c%c%c.\n", (FT_Char)( table.Tag >> 24 ),
                 (FT_Char)( table.Tag >> 16 ), (FT_Char)( table.Tag >> 8 ),
                 (FT_Char)( table.Tag ) ) );
    ...
    else
    {
      ...
      if ( table.Tag == TTAG_glyf )
      {
        ...
        if ( reconstruct_glyf( stream, &checksum, &loca_checksum, &sfnt,
                               sfnt_size, &dest_offset, info, memory ) )
          goto Fail;
      ...
      else if ( table.Tag == TTAG_hmtx )
      {
        ...
        if ( reconstruct_hmtx( stream, info->num_glyphs, info->num_hmetrics,
                               info->x_mins, &checksum, &sfnt, sfnt_size,
                               &dest_offset, memory ) )
          goto Fail;
      }
      else
...
#define FT_STRUCTURE WOFF2_HeaderRec
  ...
  if ( woff2.length != stream->size || woff2.num_tables == 0 ||
  ...
  for ( nn = 0; nn < woff2.num_tables; nn++ )
  {
    ...
    FT_TRACE2( ( "  %c%c%c%c  %08d  %08d   %08ld    %08ld    %08ld\n",
                 ...
                 table->FlagByte & 0x3f, ( table->FlagByte >> 6 ) & 0x03,
                 table->dst_length, table->TransformLength,
                 table->src_offset ) );
  ...
  woff2.uncompressed_size = last_table->src_offset + last_table->src_length;
  ...
  file_offset = ROUND4( woff2.compressed_offset + woff2.totalCompressedSize );
  ...
  *num_faces = woff2.num_fonts;
  ...
  if ( woff2.header_version )
  {
    ...
    if ( FT_QNEW_ARRAY( temp_indices, ttc_font->num_tables ) )
      goto Exit;
    ...
    if ( FT_QRENEW_ARRAY( indices, woff2.num_tables, ttc_font->num_tables ) )
      goto Exit;
  ...
  /* Write sfnt header. */
  if ( FT_QALLOC( sfnt, sfnt_size ) || FT_NEW( sfnt_stream ) )
    goto Exit;
  ...
  /* Sort tables by tag. */
  ft_qsort( indices, woff2.num_tables, sizeof( WOFF2_Table ), compare_tags );
    ...
    if ( tag == indices[nn - 1]->Tag )
    {
      FT_ERROR(
          ( "woff2_open_font:"
            ...
            (FT_Char)( tag >> 24 ), (FT_Char)( tag >> 16 ),
            (FT_Char)( tag >> 8 ), (FT_Char)( tag ) ) );
      error = FT_THROW( Invalid_Table );
      goto Exit;
    }
  }
  ...
  error = reconstruct_font( uncompressed_buf, woff2.uncompressed_size, indices,
                            &woff2, &info, &sfnt, &sfnt_size, memory );
  ...
  if ( woff2.actual_sfnt_size < sfnt_size )
  {
    FT_TRACE5( ( "Trimming sfnt stream from %lu to %lu.\n", sfnt_size,
                 woff2.actual_sfnt_size ) );
    if ( FT_QREALLOC( sfnt, (FT_ULong)( sfnt_size ),
  ...
  FT_Stream_Free( face->root.stream, ( face->root.face_flags &
                                       FT_FACE_FLAG_EXTERNAL_STREAM ) != 0 );
