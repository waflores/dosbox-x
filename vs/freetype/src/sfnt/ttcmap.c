...
static FT_Byte*
tt_cmap2_get_subheader( FT_Byte* table, FT_UInt32 char_code )
{
  FT_Byte* result = NULL;
...
static FT_Int
tt_cmap4_set_range( TT_CMap4 cmap, FT_UInt range_index )
{
...
static FT_UInt
tt_cmap4_char_map_linear( TT_CMap cmap, FT_UInt32* pcharcode, FT_Bool next )
{
  TT_Face  face  = (TT_Face)FT_CMAP_FACE( cmap );
  FT_Byte* limit = face->cmap_table + face->cmap_size;
...
static FT_UInt
tt_cmap4_char_map_binary( TT_CMap cmap, FT_UInt32* pcharcode, FT_Bool next )
{
  TT_Face  face  = (TT_Face)FT_CMAP_FACE( cmap );
  FT_Byte* limit = face->cmap_table + face->cmap_size;
...
static FT_UInt
tt_cmap12_char_map_binary( TT_CMap cmap, FT_UInt32* pchar_code, FT_Bool next )
{
  FT_UInt   gindex     = 0;
...
static FT_UInt
tt_cmap13_char_map_binary( TT_CMap cmap, FT_UInt32* pchar_code, FT_Bool next )
{
  FT_UInt   gindex     = 0;
  FT_Byte*  p          = cmap->data + 12;
  FT_UInt32 num_groups = TT_PEEK_ULONG( p );
...
static FT_Error
tt_cmap14_ensure( TT_CMap14 cmap, FT_UInt32 num_results, FT_Memory memory )
{
...
FT_CALLBACK_DEF( FT_UInt )
tt_cmap14_char_index( FT_CMap cmap, FT_UInt32 char_code )
{
  FT_UNUSED( cmap );
  FT_UNUSED( char_code );
...
FT_CALLBACK_DEF( FT_UInt )
tt_cmap14_char_next( FT_CMap cmap, FT_UInt32* pchar_code )
{
  FT_UNUSED( cmap );
...
FT_CALLBACK_DEF( FT_Error )
tt_cmap14_get_info( FT_CharMap cmap, TT_CMapInfo* cmap_info )
{
  FT_UNUSED( cmap );
...
static FT_UInt
tt_cmap14_char_map_def_binary( FT_Byte* base, FT_UInt32 char_code )
{
...
static FT_UInt
tt_cmap14_char_map_nondef_binary( FT_Byte* base, FT_UInt32 char_code )
{
...
static FT_Byte*
tt_cmap14_find_variant( FT_Byte* base, FT_UInt32 variantCode )
{
                          ...
                          FT_UInt32 variantSelector )
{
  ...
  FT_Byte* p = tt_cmap14_find_variant( ttcmap->data + 6, variantSelector );
  FT_ULong defOff;
  FT_ULong nondefOff;
                              ...
                              FT_UInt32 variantSelector )
{
  ...
  if ( nondefOff != 0 && tt_cmap14_char_map_nondef_binary(
                             ttcmap->data + nondefOff, charcode ) != 0 )
    return 0;
...
static FT_UInt32*
tt_cmap14_get_def_chars( TT_CMap cmap, FT_Byte* p, FT_Memory memory )
{
  TT_CMap14  cmap14 = (TT_CMap14)cmap;
  FT_UInt32  numRanges;
  FT_UInt    cnt;
                         ...
                         FT_UInt32 variantSelector )
{
  ...
  if ( defOff == 0 )
    ...
    return tt_cmap14_get_def_chars( ttcmap, ttcmap->data + defOff, memory );
  else
  {
                      ...
                      FT_Pointer pointer )
{
  ...
  return psnames->unicodes_init( memory, unicodes, face->root.num_glyphs,
                                 &tt_get_glyph_name, (PS_FreeGlyphNameFunc)NULL,
                                 (FT_Pointer)face );
}
