...
tt_face_lookup_table( TT_Face face, FT_ULong tag )
{
  ...
  FT_TRACE4( ( "tt_face_lookup_table: %p, `%c%c%c%c' -- ", (void*)face,
...
/*                                                                  */
static FT_Error
check_table_dir( SFNT_Header sfnt, FT_Stream stream, FT_UShort* valid )
{
  FT_Error  error;
      ...
      if ( entry.Tag == TTAG_hmtx || entry.Tag == TTAG_vmtx )
      {
        ...
        FT_TRACE2(
            ( "  %c%c%c%c  %08lx  %08lx  %08lx"
              ...
              (FT_Char)( entry.Tag >> 8 ), (FT_Char)( entry.Tag ), entry.Offset,
              entry.Length, entry.CheckSum, old_length ) );
      }
      else
        continue;
    }
 ...
 */
static FT_Error
tt_face_load_generic_header( TT_Face face, FT_Stream stream, FT_ULong tag )
{
  FT_Error   error;
  TT_Header* header;
...
#define FT_STRUCTURE TT_MaxProfile
      ...
      FT_FRAME_START( 6 ), FT_FRAME_LONG( version ),
      FT_FRAME_USHORT( numGlyphs ), FT_FRAME_END };
...
#define FT_STRUCTURE TT_LangTagRec
  ...
  /* load name records */
  {
    ...
    FT_MEM_QRENEW_ARRAY( names, table->numNameRecords, valid );
    table->names          = names;
    names                 = NULL;
...
#define FT_STRUCTURE TT_Postscript
  ...
  if ( post->FormatType != 0x00030000L && post->FormatType != 0x00025000L &&
       post->FormatType != 0x00020000L && post->FormatType != 0x00010000L )
...
tt_face_load_gasp( TT_Face face, FT_Stream stream )
{
  ...
  for ( j = 0; j < num_ranges; j++ )
  {
    ...
    FT_TRACE3( ( "gaspRange %hu: rangeMaxPPEM %5hu, rangeGaspBehavior 0x%hx\n",
                 j, gasp_ranges[j].maxPPEM, gasp_ranges[j].gaspFlag ) );
  }
