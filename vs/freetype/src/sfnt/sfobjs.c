...
static FT_String*
tt_name_ascii_from_utf16( TT_Name entry, FT_Memory memory )
{
...
static FT_String*
tt_name_ascii_from_other( TT_Name entry, FT_Memory memory )
{
  FT_String* string = NULL;
  FT_UInt    len, code, n;
  FT_Byte*   read = (FT_Byte*)entry->string;
...
static FT_Encoding
sfnt_find_encoding( int platform_id, int encoding_id )
{
...
retry:
  ...
  if ( tag == TTAG_wOF2 )
  {
    ...
    error =
        woff2_open_font( stream, face, face_instance_index, woff2_num_faces );
    if ( error )
      return error;
                ...
                FT_Parameter* params )
{
  ...
  if ( !face->tt_var )
  {
    ...
    face->tt_var =
        ft_module_get_service( tt_module, FT_SERVICE_ID_METRICS_VARIATIONS, 0 );
  }

  if ( !face->face_var )
    face->face_var = ft_module_get_service(
        &face->root.driver->root, FT_SERVICE_ID_METRICS_VARIATIONS, 0 );
#endif

  FT_TRACE2( ( "SFNT driver\n" ) );

  error =
      sfnt_open_font( stream, face, &face_instance_index, &woff2_num_faces );
  if ( error )
    return error;
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
  {
    ...
    if ( face->goto_table( face, TTAG_fvar, stream, &fvar_len ) ||
         fvar_len < 20 || FT_READ_ULONG( version ) ||
         FT_READ_USHORT( offset ) || FT_STREAM_SKIP( 2 ) /* reserved */ ||
         FT_READ_USHORT( num_axes ) || FT_READ_USHORT( axis_size ) ||
         FT_READ_USHORT( num_instances ) || FT_READ_USHORT( instance_size ) )
    {
      version       = 0;
      offset        = 0;
      num_axes      = 0;
                ...
                FT_Parameter* params )
{
  ...
  if ( sfnt->load_eblc && ( has_CBLC || has_EBLC || has_bloc || has_sbix ) )
    LOAD_( eblc );
  ...
  /* now set up root fields */
  {
    ...
    /* set the encoding fields */
    {
      ...
      for ( m = 0; m < root->num_charmaps; m++ )
      {
        ...
        charmap->encoding =
            sfnt_find_encoding( charmap->platform_id, charmap->encoding_id );
      ...
      if ( !has_unicode && root->face_flags & FT_FACE_FLAG_GLYPH_NAMES )
      {
        ...
        error = FT_CMap_New( (FT_CMap_Class)&tt_cmap_unicode_class_rec, NULL,
                             &cmaprec, NULL );
     ...
     * Set up metrics.
     */
    if ( FT_IS_SCALABLE( root ) || FT_HAS_SBIX( root ) )
    {
      ...
      else
      {
        ...
        root->height =
            root->ascender - root->descender + face->horizontal.Line_Gap;
