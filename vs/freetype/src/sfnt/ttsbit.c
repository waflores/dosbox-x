                             ...
                             FT_Size_Metrics* metrics )
{
  ...
  case TT_SBIT_TABLE_TYPE_CBLC:
  {
    ...
    metrics->x_scale =
        FT_DivFix( metrics->x_ppem * 64, face->header.Units_Per_EM );
    metrics->y_scale =
        FT_DivFix( metrics->y_ppem * 64, face->header.Units_Per_EM );
  ...
  case TT_SBIT_TABLE_TYPE_SBIX:
  {
    ...
    metrics->height =
        FT_MulFix( hori->Ascender - hori->Descender + hori->Line_Gap, scale );
...
static FT_Error
tt_sbit_decoder_alloc_bitmap( TT_SBitDecoder decoder, FT_Bool metrics_only )
{
...
#ifdef FT_CONFIG_OPTION_USE_PNG
    error = Load_SBit_Png( face->root.glyph, 0, 0, 32, metrics, stream->memory,
                           stream->cursor, glyph_end - glyph_start - 8, TRUE,
                           metrics_only );
#else
    error = FT_THROW( Unimplemented_Feature );
#endif
