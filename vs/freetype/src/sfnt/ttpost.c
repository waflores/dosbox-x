                ...
                FT_ULong      post_len )
{
  ...
  if ( num_names )
  {
    ...
    if ( FT_QALLOC( name_strings,
                    num_names * sizeof( FT_Byte* ) + post_len + 1 ) )
      goto Fail;
                ...
                FT_ULong      post_len )
{
  ...
  if ( num_glyphs > post_len || num_glyphs > 258 + 128 )
  {
    error = FT_THROW( Invalid_File_Format );
    goto Exit;
  }
...
load_post_names( TT_Face face )
{
  ...
  if ( post_len < 34 || FT_STREAM_SKIP( 32 ) || FT_READ_USHORT( num_glyphs ) ||
       num_glyphs > face->max_profile.numGlyphs || num_glyphs == 0 )
    goto Exit;

  /* now read postscript names data */
  if ( format == 0x00020000L )
    error = load_format_20( &face->postscript_names, stream, num_glyphs,
                            post_len - 34 );
  else if ( format == 0x00025000L )
    error = load_format_25( &face->postscript_names, stream, num_glyphs,
                            post_len - 34 );
...
tt_face_get_ps_name( TT_Face face, FT_UInt idx, FT_String** PSname )
{
  ...
  *PSname = MAC_NAME( 0 );
  ...
  else if ( format == 0x00020000L || format == 0x00025000L )
  {
    TT_Post_Names names = &face->postscript_names;
