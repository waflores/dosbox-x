...
#define SVG_MINIMUM_SIZE \
  ( SVG_TABLE_HEADER_SIZE + SVG_DOCUMENT_LIST_MINIMUM_SIZE )
...
tt_face_load_svg( TT_Face face, FT_Stream stream )
{
  ...
  if ( offsetToSVGDocumentList < SVG_TABLE_HEADER_SIZE ||
       offsetToSVGDocumentList > table_size - SVG_DOCUMENT_LIST_MINIMUM_SIZE )
    goto InvalidTable;
...
static FT_Int
compare_svg_doc( Svg_doc doc, FT_UInt glyph_index )
{
...
tt_face_load_svg_doc( FT_GlyphSlot glyph, FT_UInt glyph_index )
{
  ...
  error = find_doc( doc_list + 2, svg->num_entries, glyph_index, &doc_offset,
                    &doc_length, &doc_start_glyph_id, &doc_end_glyph_id );
  if ( error != FT_Err_Ok )
    goto Exit;

  doc_limit = svg->table_size - (FT_ULong)( doc_list - (FT_Byte*)svg->table );
  ...
  if ( doc_length > 6 && doc[0] == 0x1F && doc[1] == 0x8B && doc[2] == 0x08 )
  {
    ...
    error = FT_Gzip_Uncompress( memory, uncomp_buffer, &uncomp_size, doc,
                                doc_length );
    if ( error )
    {
