...
static voidpf
ft_gzip_alloc( voidpf opaque, uInt items, uInt size )
{
...
static void
ft_gzip_free( voidpf opaque, voidpf address )
{
  FT_Memory memory = (FT_Memory)opaque;
...
static FT_Error
ft_gzip_file_init( FT_GZipFile zip, FT_Stream stream, FT_Stream source )
{
...
static FT_Error
ft_gzip_file_skip_output( FT_GZipFile zip, FT_ULong count )
{
  FT_Error error = FT_Err_Ok;
