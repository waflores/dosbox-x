...
byte_swap( z_word_t word )
{
  ...
  return ( word & 0xff00000000000000 ) >> 56 |
         ( word & 0xff000000000000 ) >> 40 | ( word & 0xff0000000000 ) >> 24 |
         ( word & 0xff00000000 ) >> 8 | ( word & 0xff000000 ) << 8 |
         ( word & 0xff0000 ) << 24 | ( word & 0xff00 ) << 40 |
         ( word & 0xff ) << 56;
#else /* W == 4 */
...
local int
test_and_set( int volatile* flag )
{
  int was;
 ...
 */
local void
write_table( FILE* out, const z_crc_t FAR* table, int k )
{
  int n;
 ...
 */
local void
write_table32hi( FILE* out, const z_word_t FAR* table, int k )
{
  int n;
 ...
 */
local void
write_table64( FILE* out, const z_word_t FAR* table, int k )
{
  int n;
