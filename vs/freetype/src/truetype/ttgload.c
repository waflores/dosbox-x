...
#define IS_DEFAULT_INSTANCE( _face ) \
  ( !( FT_IS_NAMED_INSTANCE( _face ) || FT_IS_VARIATION( _face ) ) )
#else
                 ...
                 FT_UShort* ah )
{
  ...
  else if ( face->os2.version != 0xFFFFU )
  {
    ...
    *ah =
        (FT_UShort)FT_ABS( face->os2.sTypoAscender - face->os2.sTypoDescender );
  }
...
static FT_Error
tt_get_metrics( TT_Loader loader, FT_UInt glyph_index )
{
...
static void
tt_get_metrics_incremental( TT_Loader loader, FT_UInt glyph_index )
{
  TT_Face face = loader->face;
...
{
  zone->n_points =
      (FT_UShort)load->outline.n_points + 4 - (FT_UShort)start_point;
 ...
 */
static FT_Error
TT_Hint_Glyph( TT_Loader loader, FT_Bool is_composite )
{
...
TT_Process_Simple_Glyph( TT_Loader loader )
{
  ...
  if ( !IS_DEFAULT_INSTANCE( FT_FACE( loader->face ) ) )
  {
    ...
    error = TT_Vary_Apply_Glyph_Deltas( loader, outline, unrounded );
    if ( error )
      goto Exit;
  }
  ...
  if ( IS_HINTED( loader->load_flags ) )
  {
    ...
    FT_ARRAY_COPY( loader->zone.orus, loader->zone.cur, loader->zone.n_points );
  }
...
static FT_ListNode
ft_list_get_node_at( FT_List list, FT_UInt idx )
{
  FT_ListNode cur;
                     ...
                     FT_Bool   header_only )
{
  ...
  if ( loader->byte_len > 0 )
  {
    ...
    error = face->access_glyph_frame(
        loader, glyph_index, face->glyf_offset + offset, loader->byte_len );
    if ( error )
      goto Exit;
  ...
  else if ( loader->n_contours < 0 )
  {
         ...
         FT_IS_VARIATION( FT_FACE( face ) ) )
    {
      ...
      if ( FT_SET_ERROR(
               TT_Vary_Apply_Glyph_Deltas( loader, &outline, unrounded ) ) )
        goto Exit1;
...

    {
      ...
      for ( n = 0; n < num_subglyphs; n++ )
      {
        ...
        error = TT_Process_Composite_Component( loader, subglyph, start_point,
                                                num_base_points );
        if ( error )
          goto Exit;
      }
      ...
      if ( IS_HINTED( loader->load_flags ) &&
#ifdef TT_USE_BYTECODE_INTERPRETER
           subglyph && subglyph->flags & WE_HAVE_INSTR &&
#endif
...
static FT_Error
compute_glyph_metrics( TT_Loader loader, FT_UInt glyph_index )
{
  TT_Face      face  = loader->face;
  ...
  /* create some metrics manually                                   */
  {
    ...
    {
      top =
          (FT_Short)FT_DivFix( SUB_LONG( loader->pp3.y, bbox.yMax ), y_scale );

      if ( loader->pp3.y <= loader->pp4.y )
        advance = 0;
      else
        advance = (FT_UShort)FT_DivFix(
            SUB_LONG( loader->pp3.y, loader->pp4.y ), y_scale );
    }
    else
    {
      FT_Pos height;
      ...
      if ( face->os2.version != 0xFFFFU )
        advance =
            (FT_Pos)( face->os2.sTypoAscender - face->os2.sTypoDescender );
      else
        advance =
            (FT_Pos)( face->horizontal.Ascender - face->horizontal.Descender );
...
#ifdef FT_CONFIG_OPTION_INCREMENTAL
    {
      ...
      if ( incr && incr->funcs->get_glyph_metrics )
      {
        ...
        error = incr->funcs->get_glyph_metrics( incr->object, glyph_index, TRUE,
                                                &incr_metrics );
        if ( error )
          return error;
  ...
  if ( IS_HINTED( load_flags ) && !glyf_table_only )
  {
    ...
    if ( driver->interpreter_version == TT_INTERPRETER_VERSION_40 )
    {
      ...
      grayscale_cleartype = FT_BOOL(
          subpixel_hinting_lean && !( ( load_flags & FT_LOAD_TARGET_LCD ) ||
    ...
    if ( driver->interpreter_version == TT_INTERPRETER_VERSION_38 )
    {
      ...
      exec->symmetrical_smoothing = FT_BOOL(
          FT_LOAD_TARGET_MODE( load_flags ) != TT_LOAD_SYMMETRICAL_SMOOTHING );
      exec->bgr = FT_BOOL( FT_LOAD_TARGET_MODE( load_flags ) != TT_LOAD_BGR );
      exec->vertical_lcd =
          FT_BOOL( FT_LOAD_TARGET_MODE( load_flags ) != TT_LOAD_VERTICAL_LCD );
     ...
     */
    if ( driver->interpreter_version == TT_INTERPRETER_VERSION_40 &&
         subpixel_hinting_lean && !FT_IS_TRICKY( glyph->face ) )
    ...
    if ( IS_HINTED( loader->load_flags ) &&
         ...
         !( driver->interpreter_version == TT_INTERPRETER_VERSION_38 &&
            ...
            FT_LOAD_TARGET_MODE( loader->load_flags ) != FT_RENDER_MODE_MONO &&
               ...
               FT_Int32     load_flags )
{
       ...
       IS_DEFAULT_INSTANCE( glyph->face ) )
  {
    ...
    {
      if ( FT_IS_SCALABLE( glyph->face ) || FT_HAS_SBIX( glyph->face ) )
      {
  ...
  /* check for OT-SVG */
  if ( ( load_flags & FT_LOAD_NO_SVG ) == 0 && ( load_flags & FT_LOAD_COLOR ) &&
       face->svg )
  {
    SFNT_Service sfnt = (SFNT_Service)face->sfnt;
    ...
    if ( !error )
    {
      ...
      sfnt->get_metrics( face, FALSE, glyph_index, &leftBearing, &advanceX );
      sfnt->get_metrics( face, TRUE, glyph_index, &topBearing, &advanceY );
  ...
  if ( load_flags & FT_LOAD_ADVANCE_ONLY &&
       !( load_flags & FT_LOAD_VERTICAL_LAYOUT ) && loader.widthp )
  {
    glyph->metrics.horiAdvance = loader.widthp[glyph_index] * 64;
    goto Done;
  }
  ...
  FT_TRACE1(
      ( "  subglyphs = %u, contours = %hd, points = %hd,"
        " flags = 0x%.3x\n",
        loader.gloader->base.num_subglyphs, glyph->outline.n_contours,
        glyph->outline.n_points, glyph->outline.flags ) );
