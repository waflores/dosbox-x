...
static FT_UInt32
tt_synth_sfnt_checksum( FT_Stream stream, FT_ULong length )
{
  FT_Error  error;
...
static FT_ULong
tt_get_sfnt_checksum( TT_Face face, FT_UShort i )
{
              ...
              FT_Parameter* params )
{
  ...
  if ( FT_IS_SCALABLE( ttface ) || FT_HAS_SBIX( ttface ) )
  {
...
tt_size_run_prep( TT_Size size, FT_Bool pedantic )
{
  ...
  for ( i = 0; i < size->cvt_size; i++ )
  {
    ...
    FT_TRACE6( ( "  %3d: %f (%f)\n", i, (double)face->cvt[i] / 64,
                 (double)size->cvt[i] / 64 ) );
  }
  FT_TRACE6( ( "\n" ) );
...
static FT_Error
tt_size_init_bytecode( FT_Size ftsize, FT_Bool pedantic )
{
  FT_Error  error;
  TT_Size   size   = (TT_Size)ftsize;
  TT_Face   face   = (TT_Face)ftsize->face;
