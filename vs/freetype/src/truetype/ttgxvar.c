...
#define FT_fdot6ToFixed( i )  ( (FT_Fixed)( (FT_ULong)( i ) << 10 ) )
#define FT_fixedToInt( x )    ( (FT_Short)( ( ( x ) + 0x8000U ) >> 16 ) )
#define FT_fixedToFdot6( x )  ( (FT_Pos)( ( ( x ) + 0x200 ) >> 10 ) )
 ...
 */
static FT_UShort*
ft_var_readpackedpoints( FT_Stream stream, FT_ULong size, FT_UInt* point_cnt )
{
 ...
 */
static FT_Fixed*
ft_var_readpackeddeltas( FT_Stream stream, FT_ULong size, FT_UInt delta_cnt )
{
...
ft_var_load_avar( TT_Face face )
{
  ...
  for ( i = 0; i < axisCount; i++, segment++ )
  {
    ...
    {
      segment->correspondence[j].fromCoord = FT_fdot14ToFixed( FT_GET_SHORT() );
      segment->correspondence[j].toCoord   = FT_fdot14ToFixed( FT_GET_SHORT() );
  ...
  if ( store_offset )
  {
    error = tt_var_load_item_variation_store(
        FT_FACE( face ), table_offset + store_offset, &table->itemStore );
    if ( error )
      goto Exit;
  }
  ...
  if ( axisMap_offset )
  {
    error = tt_var_load_delta_set_index_mapping(
        FT_FACE( face ), table_offset + axisMap_offset, &table->axisMap,
        &table->itemStore, table_len );
    if ( error )
      goto Exit;
  }
#endif
                                  ...
                                  GX_ItemVarStore itemStore )
{
  ...
  if ( FT_READ_ULONG( region_offset ) || FT_READ_USHORT( data_count ) )
    goto Exit;
  ...
  if ( FT_READ_USHORT( axis_count ) || FT_READ_USHORT( region_count ) )
    goto Exit;
  ...
  for ( i = 0; i < data_count; i++ )
  {
    ...
    if ( FT_READ_USHORT( item_count ) || FT_READ_USHORT( word_delta_count ) ||
    ...
    if ( FT_NEW_ARRAY( varData->deltaSet, per_region_size * item_count ) )
      goto Exit;
    if ( FT_Stream_Read( stream, varData->deltaSet,
 ...
 */
static FT_Error
ft_var_load_hvvar( TT_Face face, FT_Bool vertical )
{
  FT_Stream stream = FT_FACE_STREAM( face );
  FT_Memory memory = stream->memory;
  ...
  {
    error = tt_var_load_delta_set_index_mapping(
        FT_FACE( face ), table_offset + widthMap_offset, &table->widthMap,
        &table->itemStore, table_len );
    if ( error )
      goto Exit;
  }
  ...
  for ( master = 0; master < varData->regionIdxCount; master++ )
  {
    ...
    for ( j = 0; j < itemStore->axisCount; j++, axis++ )
    {
      ...
      /* cumulative product of all the axis scalars */
        ...
        scalar = FT_MulDiv(
            scalar, ttface->blend->normalizedcoords[j] - axis->startCoord,
                     ...
                     FT_Bool vertical )
{
  ...
  delta = tt_var_get_item_delta( FT_FACE( face ), &table->itemStore, outerIndex,
                                 innerIndex );
...
static FT_Short*
ft_var_get_value_pointer( TT_Face face, FT_ULong mvar_tag )
{
  FT_Short* p;
...
ft_var_load_mvar( TT_Face face )
{
  ...
  error = tt_var_load_item_variation_store( FT_FACE( face ),
...
static FT_Error
ft_size_reset_iterator( FT_ListNode node, void* user )
{
...
tt_apply_mvar( FT_Face face ) /* TT_Face */
{
  ...
  for ( ; value < limit; value++ )
  {
    ...
    delta = tt_var_get_item_delta( face, &blend->mvar_table->itemStore,
  ...
  /* adjust all derived values */
  {
     ...
     */
    FT_Short current_line_gap = face->height - face->ascender + face->descender;

    face->ascender  = face->ascender + mvar_hasc_delta;
    face->descender = face->descender + mvar_hdsc_delta;
    face->height =
        face->ascender - face->descender + current_line_gap + mvar_hlgp_delta;
    ...
    if ( var && var->size_reset )
      FT_List_Iterate( &face->sizes_list, ft_size_reset_iterator, (void*)var );
  }
}
...
#define FT_STRUCTURE GX_GVar_Head
  ...
  offsets_len =
      ( gvar_head.glyphCount + 1 ) * ( ( gvar_head.flags & 1 ) ? 4L : 2L );
  ...
  if ( gvar_head.globalCoordCount != 0 )
  {
    if ( FT_STREAM_SEEK( gvar_start + gvar_head.offsetToCoord ) ||
         FT_FRAME_ENTER( gvar_head.globalCoordCount * gvar_head.axisCount *
                         2L ) )
    {
      FT_TRACE2(
          ( "ft_var_load_gvar:"
                    ...
                    FT_Fixed* im_end_coords )
{
  ...
  for ( i = 0; i < blend->num_axis; i++ )
  {
    FT_TRACE6( ( "    axis %d coordinate %.5f:\n", i,
                 (double)blend->normalizedcoords[i] / 65536 ) );
    ...
    else
    {
      ...
      if ( blend->normalizedcoords[i] < tuple_coords[i] )
        apply =
            FT_MulDiv( apply, blend->normalizedcoords[i] - im_start_coords[i],
                       tuple_coords[i] - im_start_coords[i] );
      else
        apply = FT_MulDiv( apply, im_end_coords[i] - blend->normalizedcoords[i],
  ...
  for ( i = 0; i < num_coords; i++, a++ )
  {
    ...
    if ( coord > a->maximum || coord < a->minimum )
    {
      FT_TRACE1( ( "ft_var_to_normalized: design coordinate %.5f\n",
                   (double)coord / 65536 ) );
      FT_TRACE1(
          ( "                      is out of range [%.5f;%.5f];"
            " clamping\n",
            (double)a->minimum / 65536, (double)a->maximum / 65536 ) );
    }

    if ( coord > a->def )
      normalized[i] = coord >= a->maximum
                          ? 0x10000L
                          : FT_DivFix( SUB_LONG( coord, a->def ),
                                       SUB_LONG( a->maximum, a->def ) );
    else if ( coord < a->def )
      normalized[i] = coord <= a->minimum
                          ? -0x10000L
                          : FT_DivFix( SUB_LONG( coord, a->def ),
  ...
  if ( blend->avar_table )
  {
    ...
    if ( table->itemStore.varData )
    {
      ...
      for ( i = 0; i < mmvar->num_axis; i++ )
      {
        ...
        delta = tt_var_get_item_delta( FT_FACE( face ), &table->itemStore,
  ...
  if ( blend->avar_table && blend->avar_table->avar_segment )
  {
    ...
    for ( i = 0; i < nc; i++, av++ )
    {
      for ( j = 1; j < (FT_UInt)av->pairCount; j++ )
      {
        ...
        {
          design[i] = FT_MulDiv( design[i] - av->correspondence[j - 1].toCoord,
                                 av->correspondence[j].fromCoord -
                                     av->correspondence[j - 1].fromCoord,
                                 av->correspondence[j].toCoord -
                                     av->correspondence[j - 1].toCoord ) +
...
#define FT_STRUCTURE GX_FVar_Axis
  ...
  if ( need_init )
  {
    ...
    if ( FT_SET_ERROR(
             ttface->goto_table( ttface, TTAG_fvar, stream, &table_len ) ) )
    {
...
#define ALIGN_SIZE( n ) \
  ...
  next_coords_size =
      ALIGN_SIZE( num_instances * num_axes * sizeof( FT_Fixed ) );
  next_name_size = num_axes * 5;

  if ( need_init )
  {
    ttface->blend->mmvar_len = mmvar_size + axis_flags_size + axis_size +
      ...
      *axis_flags = axis_rec.flags;
      ...
      FT_TRACE5(
          ( "  %3d  `%s'"
            ...
            i, a->name, (double)a->minimum / 65536, (double)a->def / 65536,
            (double)a->maximum / 65536, *axis_flags,
    ...
    for ( i = 0; i < fvar_head.instanceCount; i++, ns++ )
    {
...
#ifdef FT_DEBUG_LEVEL_TRACE
      {
        ...
        if ( ns->strid != 0xFFFF )
        {
          (void)sfnt->get_name( ttface, (FT_UShort)ns->strid, &strname );
        ...
        if ( ns->psid != 0xFFFF )
        {
          (void)sfnt->get_name( ttface, (FT_UShort)ns->psid, &psname );
        ...
        FT_TRACE5( ( "  named instance %d (%s%s%s, %s%s%s)\n", i,
                     strname ? "name: `" : "", strname ? strname : "unnamed",
                     strname ? "'" : "", psname ? "PS name: `" : "",
    ...
    if ( num_instances != fvar_head.instanceCount )
    {
      ...
      found = sfnt->get_name_id( ttface, TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY,
                                 &dummy1, &dummy2 );
      if ( found )
        strid = TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY;
      else
      {
        found = sfnt->get_name_id( ttface, TT_NAME_ID_FONT_SUBFAMILY, &dummy1,
                                   &dummy2 );
        if ( found )
      ...
      {
        found =
            sfnt->get_name_id( ttface, TT_NAME_ID_PS_NAME, &dummy1, &dummy2 );
        if ( found )
        {
          FT_TRACE5(
              ( "TT_Get_MM_Var:"
                 ...
                 FT_Bool   set_design_coords )
{
  ...
  if ( coords )
    FT_MEM_COPY( blend->normalizedcoords, coords,
                       ...
                       FT_UInt instance_index )
{
  ...
  if ( instance_index > 0 )
  {
    ...
    error =
        sfnt->get_name( ttface, (FT_UShort)named_style->strid, &style_name );
    if ( error )
      goto Exit;
 ...
 */
FT_LOCAL_DEF( FT_Error )
TT_Get_Default_Named_Instance( FT_Face face, FT_UInt* instance_index )
{
  TT_Face  ttface = (TT_Face)face;
  FT_Error error  = FT_Err_Ok;
...
static FT_Error
tt_cvt_ready_iterator( FT_ListNode node, void* user )
{
    ...
    else if ( localpoints == ALL_POINTS )
    {
      ...
      for ( j = 0; j < face->cvt_size; j++ )
      {
        ...
        if ( old_cvt_delta != cvt_deltas[j] )
        {
          FT_TRACE7(
              ( "      %d: %f -> %f\n", j,
                (double)( FT_fdot6ToFixed( face->cvt[j] ) + old_cvt_delta ) /
                    65536,
                (double)( FT_fdot6ToFixed( face->cvt[j] ) + cvt_deltas[j] ) /
                    65536 ) );
          count++;
        }
#endif
    ...
    else
    {
      ...
      for ( j = 0; j < point_count; j++ )
      {
        ...
        if ( old_cvt_delta != cvt_deltas[pindex] )
        {
          FT_TRACE7( (
              "      %d: %f -> %f\n", pindex,
              (double)( FT_fdot6ToFixed( face->cvt[pindex] ) + old_cvt_delta ) /
                  65536,
...
Exit:
  ...
  FT_List_Iterate( &root->sizes_list, tt_cvt_ready_iterator, NULL );
  ...
  do
  {
    ...
    if ( point <= end_point )
    {
      ...
      while ( point <= end_point )
      {
        ...
        if ( has_delta[point] )
        {
          tt_delta_interpolate( cur_delta + 1, point - 1, cur_delta, point,
      ...
      else
      {
        ...
        tt_delta_interpolate( cur_delta + 1, end_point, cur_delta, first_delta,
                              in_points, out_points );

        if ( first_delta > 0 )
          tt_delta_interpolate( first_point, first_delta - 1, cur_delta,
                            ...
                            FT_Vector*  unrounded )
{
  ...
  if ( !( face->variation_support & TT_FACE_FLAG_VAR_HADVANCE ) )
  {
    ...
    loader->linear =
        FT_PIX_ROUND( unrounded[n_points - 3].x - unrounded[n_points - 4].x ) /
        64;
  }
  if ( !( face->variation_support & TT_FACE_FLAG_VAR_VADVANCE ) )
  {
    ...
    loader->vadvance =
        FT_PIX_ROUND( unrounded[n_points - 1].y - unrounded[n_points - 2].y ) /
        64;
  }
...
FT_LOCAL_DEF( void )
tt_var_done_item_variation_store( FT_Face face, GX_ItemVarStore itemStore )
{
  FT_Memory memory = FT_FACE_MEMORY( face );
  FT_UInt   i;
...
tt_done_blend( FT_Face face )
{
  ...
  if ( blend )
  {
    ...
    if ( blend->avar_table )
    {
      ...
      tt_var_done_item_variation_store( face, &blend->avar_table->itemStore );

      tt_var_done_delta_set_index_map( face, &blend->avar_table->axisMap );
    ...
    if ( blend->hvar_table )
    {
      tt_var_done_item_variation_store( face, &blend->hvar_table->itemStore );

      tt_var_done_delta_set_index_map( face, &blend->hvar_table->widthMap );
      FT_FREE( blend->hvar_table );
    }

    if ( blend->vvar_table )
    {
      tt_var_done_item_variation_store( face, &blend->vvar_table->itemStore );

      tt_var_done_delta_set_index_map( face, &blend->vvar_table->widthMap );
      FT_FREE( blend->vvar_table );
    }

    if ( blend->mvar_table )
    {
      tt_var_done_item_variation_store( face, &blend->mvar_table->itemStore );
