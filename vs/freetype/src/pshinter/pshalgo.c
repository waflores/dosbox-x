...
static FT_Int
psh_hint_overlap( PSH_Hint hint1, PSH_Hint hint2 )
{
...
/* destroy hints table */
static void
psh_hint_table_done( PSH_Hint_Table table, FT_Memory memory )
{
  FT_FREE( table->zones );
  table->num_zones = 0;
  table->zone      = NULL;
...
static void
psh_hint_table_record( PSH_Hint_Table table, FT_UInt idx )
{
...
static void
psh_hint_table_record_mask( PSH_Hint_Table table, PS_Mask hint_mask )
{
  FT_Int   mask = 0, val = 0;
...
static void
psh_hint_table_activate_mask( PSH_Hint_Table table, PS_Mask hint_mask )
{
  FT_Int   mask = 0, val = 0;
  FT_Byte* cursor = hint_mask->bytes;
...
#if 1
static FT_Pos
psh_dimension_quantize_len( PSH_Dimension dim, FT_Pos len, FT_Bool do_snapping )
{
  if ( len <= 64 )
    len = 64;
  else
...
static FT_Fixed
psh_hint_snap_stem_side_delta( FT_Fixed pos, FT_Fixed len )
{
...
static PSH_Dir
psh_compute_dir( FT_Pos dx, FT_Pos dy )
{
  FT_Pos  ax, ay;
                ...
                PSH_Globals globals )
{
  ...
  error = psh_hint_table_init(
      &glyph->hint_tables[0], &ps_hints->dimension[0].hints,
      &ps_hints->dimension[0].masks, &ps_hints->dimension[0].counters, memory );
  if ( error )
    goto Exit;

  error = psh_hint_table_init(
      &glyph->hint_tables[1], &ps_hints->dimension[1].hints,
      &ps_hints->dimension[1].masks, &ps_hints->dimension[1].counters, memory );
  if ( error )
    goto Exit;
                                   ...
                                   PSH_Dir        major_dir )
{
  ...
  for ( ; count > 0; count--, point++ )
  {
    ...
    point_dir = (PSH_Dir)( ( point->dir_in | point->dir_out ) & major_dir );
...
psh_glyph_find_strong_points( PSH_Glyph glyph, FT_Int dimension )
{
  ...
  FT_UInt        first     = 0;
  PSH_Dir        major_dir =
      ( dimension == 0 ) ? PSH_DIR_VERTICAL : PSH_DIR_HORIZONTAL;
...
static void
psh_glyph_find_blue_points( PSH_Blues blues, PSH_Glyph glyph )
{
    ...
    if ( hint )
    {
      ...
      else
      {
        ...
        if ( delta <= 0 )
          ...
          point->cur_u =
              hint->cur_pos + FT_MulDiv( delta, hint->cur_len, hint->org_len );
      }
      psh_point_set_fitted( point );
    }
  }
  ...
  for ( point = points; point < points_end; point++ )
  {
    ...
    /* find best enclosing point coordinates then interpolate */
    {
      ...
      if ( nn == 0 ) /* point before the first strong point */
      {
        ...
        point->cur_u =
            after->cur_u + FT_MulFix( point->org_u - after->org_u, scale );
      }
      else
      {
        before = strongs[nn - 1];
        ...
        if ( nn == num_strongs ) /* point is after last strong point */
        {
          ...
          point->cur_u =
              before->cur_u + FT_MulFix( point->org_u - before->org_u, scale );
        }
        else
        {
          FT_Pos u;
          ...
          if ( u == before->org_u )
            ...
            point->cur_u =
                before->cur_u + FT_MulDiv( u - before->org_u,
...
static void
psh_glyph_interpolate_other_points( PSH_Glyph glyph, FT_Int dimension )
{
  PSH_Dimension dim          = &glyph->globals->dimension[dimension];
  FT_Fixed      scale        = dim->scale_mult;
  FT_Fixed      delta        = dim->scale_delta;
