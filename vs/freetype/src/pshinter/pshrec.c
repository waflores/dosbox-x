...
/* destroy hints table */
static void
ps_hint_table_done( PS_Hint_Table table, FT_Memory memory )
{
  FT_FREE( table->hints );
  table->num_hints = 0;
  table->max_hints = 0;
...
static FT_Error
ps_hint_table_ensure( PS_Hint_Table table, FT_UInt count, FT_Memory memory )
{
...
static FT_Error
ps_hint_table_alloc( PS_Hint_Table table, FT_Memory memory, PS_Hint* ahint )
{
...
/* destroy mask */
static void
ps_mask_done( PS_Mask mask, FT_Memory memory )
{
  FT_FREE( mask->bytes );
  mask->num_bits  = 0;
  mask->max_bits  = 0;
...
static FT_Error
ps_mask_ensure( PS_Mask mask, FT_UInt count, FT_Memory memory )
{
...
static FT_Int
ps_mask_test_bit( PS_Mask mask, FT_UInt idx )
{
...
static FT_Error
ps_mask_set_bit( PS_Mask mask, FT_UInt idx, FT_Memory memory )
{
  FT_Error error = FT_Err_Ok;
  FT_Byte* p;
...
/* destroy mask table */
static void
ps_mask_table_done( PS_Mask_Table table, FT_Memory memory )
{
...
static FT_Error
ps_mask_table_ensure( PS_Mask_Table table, FT_UInt count, FT_Memory memory )
{
...
static FT_Error
ps_mask_table_alloc( PS_Mask_Table table, FT_Memory memory, PS_Mask* amask )
{
  FT_UInt  count;
  FT_Error error = FT_Err_Ok;
  PS_Mask  mask  = NULL;
...
static FT_Error
ps_mask_table_last( PS_Mask_Table table, FT_Memory memory, PS_Mask* amask )
{
  FT_Error error = FT_Err_Ok;
  FT_UInt  count;
  PS_Mask  mask;
...
/*                                                                 */
static FT_Error
ps_mask_table_merge_all( PS_Mask_Table table, FT_Memory memory )
{
...
static void
ps_dimension_done( PS_Dimension dimension, FT_Memory memory )
{
...
static void
ps_dimension_end_mask( PS_Dimension dim, FT_UInt end_point )
{
...
static FT_Error
ps_dimension_reset_mask( PS_Dimension dim, FT_UInt end_point, FT_Memory memory )
{
  PS_Mask mask;
...
static FT_Error
ps_dimension_end( PS_Dimension dim, FT_UInt end_point, FT_Memory memory )
{
  /* end hint mask table */
  ps_dimension_end_mask( dim, end_point );
...
static void
ps_hints_open( PS_Hints hints, PS_Hint_Type hint_type )
{
  hints->error     = FT_Err_Ok;
...
static void
ps_hints_stem( PS_Hints hints, FT_UInt dimension, FT_Int count, FT_Long* stems )
{
  PS_Dimension dim;
                  ...
                  FT_Fixed* stems )
{
  ...
  if ( !hints->error )
  {
    ...
    if ( hints->hint_type == PS_HINT_TYPE_1 )
    {
      ...
      {
        error = ps_dimension_add_t1stem( dim, (FT_Int)FIXED_TO_INT( stems[0] ),
                  ...
                  FT_UInt  end_point )
{
  ...
  if ( !hints->error )
  {
    ...
    {
      error =
          ps_dimension_reset_mask( &hints->dimension[0], end_point, memory );
      if ( error )
        goto Fail;

      error =
          ps_dimension_reset_mask( &hints->dimension[1], end_point, memory );
      if ( error )
        goto Fail;
    }
    else
                    ...
                    const FT_Byte* bytes )
{
  ...
  if ( !hints->error )
  {
    ...
    error =
        ps_dimension_set_mask_bits( &dim[1], bytes, count1, count2, 0, memory );
    if ( error )
      goto Fail;
  }
  return;
...
static FT_Error
ps_hints_close( PS_Hints hints, FT_UInt end_point )
{
  FT_Error error;
...
static FT_Error
t1_hints_close( T1_Hints hints, FT_UInt end_point )
{
  return ps_hints_close( (PS_Hints)hints, end_point );
}

static void
t1_hints_stem( T1_Hints hints, FT_UInt dimension, FT_Fixed* coords )
{
  FT_Pos stems[2];
...
static FT_Error
t2_hints_close( T2_Hints hints, FT_UInt end_point )
{
  return ps_hints_close( (PS_Hints)hints, end_point );
}
