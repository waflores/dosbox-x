...
af_latin2_metrics_init_widths( AF_LatinMetrics metrics, FT_Face face )
{
...

  {
    ...
    glyph_index =
        FT_Get_Char_Index( face, metrics->root.style_class->standard_char );
    if ( glyph_index == 0 )
      goto Exit;
...
static const char af_latin2_blue_chars[AF_LATIN_MAX_BLUES]
                                      [AF_LATIN_MAX_TEST_CHARACTERS + 1] = {
                                          "THEZOCQS", "HEZLOCUS", "fijkdbh",
                                          "xzroesc",  "xzroesc",  "pqgjy" };

static void
af_latin2_metrics_init_blues( AF_LatinMetrics metrics, FT_Face face )
{
...
af_latin2_metrics_init( AF_LatinMetrics metrics, FT_Face face )
{
  ...
  static const FT_Encoding latin_encodings[] = {
      FT_ENCODING_UNICODE, FT_ENCODING_APPLE_ROMAN, FT_ENCODING_ADOBE_STANDARD,
      FT_ENCODING_ADOBE_LATIN_1, FT_ENCODING_NONE /* end of list */
  };
...
af_latin2_hints_compute_edges( AF_GlyphHints hints, AF_Dimension dim )
{
  ...
  for ( seg = segments; seg < segment_limit; seg++ )
  {
    ...
    if ( !found )
    {
      ...
      error =
          af_axis_hints_new_edge( axis, seg->pos, seg->dir, 0, memory, &edge );
      if ( error )
        goto Exit;
...
static FT_Error
af_latin2_hints_init( AF_GlyphHints hints, AF_LatinMetrics metrics )
{
  FT_Render_Mode mode;
...
static FT_Pos
af_latin2_snap_width( AF_Width widths, FT_UInt count, FT_Pos width )
{
  FT_UInt n;
  FT_Pos  best      = 64 + 32 + 2;
...
static void
af_latin2_align_serif_edge( AF_GlyphHints hints, AF_Edge base, AF_Edge serif )
{
  FT_UNUSED( hints );
...
af_latin2_hint_edges( AF_GlyphHints hints, AF_Dimension dim )
{
  ...
  for ( edge = edges; edge < edge_limit; edge++ )
  {
    ...
    if ( !anchor )
    {
      ...
      cur_len = af_latin2_compute_stem_width( hints, dim, org_len, edge->flags,
                                              edge2->flags );
      if ( cur_len <= 64 )
        u_off = d_off = 32;
      else
      ...
      FT_TRACE5(
          ( "ANCHOR: edge %d (opos=%.2f) and %d (opos=%.2f)"
            " snapped to (%.2f) (%.2f)\n",
            edge - edges, edge->opos / 64.0, edge2 - edges, edge2->opos / 64.0,
      ...
      anchor_drift =
          ( ( anchor->pos - anchor->opos ) + ( edge2->pos - edge2->opos ) ) >>
          1;
    ...
    else
    {
      ...
      FT_TRACE5( ( "ALIGN: left=%.2f right=%.2f ", org_left / 64.0,
                   org_right / 64.0 ) );
      ...
      else
      {
        ...
        if ( cur_len <= 96 )
        {
          ...
          if ( frac_left >= 22 && frac_left <= 42 && frac_right >= 22 &&
               frac_right <= 42 )
          {
            org                  = frac_left;
            fit                  = ( org <= 32 ) ? 16 : 48;
    ...
    AlignStem:
      ...
      FT_TRACE5(
          ( "STEM1: %d (opos=%.2f) to %d (opos=%.2f)"
            ...
            edge - edges, edge->opos / 64.0, edge2 - edges, edge2->opos / 64.0,
            edge->pos / 64.0, edge2->pos / 64.0, org_len / 64.0,
            cur_len / 64.0 ) );
...
AF_DEFINE_WRITING_SYSTEM_CLASS(
    ...
    (AF_WritingSystem_ScaleMetricsFunc)
        af_latin2_metrics_scale,            /* style_metrics_scale   */
