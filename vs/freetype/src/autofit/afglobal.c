...
af_face_globals_compute_style_coverage( AF_FaceGlobals globals )
{
  ...
  for ( ss = 0; af_style_classes[ss]; ss++ )
  {
    ...
    if ( style_class->coverage == AF_COVERAGE_DEFAULT )
    {
      ...
      for ( range = script_class->script_uni_nonbase_ranges; range->first != 0;
            range++ )
      {
        FT_ULong charcode = range->first;
        FT_UInt  gindex;
...
af_face_globals_new( FT_Face face, AF_FaceGlobals* aglobals, AF_Module module )
{
  ...
  if ( FT_QALLOC( globals, sizeof( *globals ) + (FT_ULong)face->num_glyphs *
                                                    sizeof( FT_UShort ) ) )
    goto Exit;
...
Again:
  ...
  if ( !metrics )
  {
    ...
    {
      error =
          writing_system_class->style_metrics_init( metrics, globals->face );
      if ( error )
      {
        if ( writing_system_class->style_metrics_done )
        ...
        {
          style =
              (AF_Style)( globals->glyph_styles[gindex] & AF_STYLE_UNASSIGNED );
