...
static char*
af_print_idx( char* p, size_t n, int idx )
{
  if ( idx == -1 )
  {
    p[0] = '-';
...
static int
af_get_segment_index( AF_GlyphHints hints, int point_idx, int dimension )
{
...
static int
af_get_edge_index( AF_GlyphHints hints, int segment_idx, int dimension )
{
  AF_AxisHints axis    = &hints->axis[dimension];
...
#endif
  void
  af_glyph_hints_dump_points( AF_GlyphHints hints, FT_Bool to_stdout )
  {
    ...
    for ( point = points; point < limit; point++ )
    {
      ...
      AF_DUMP( (
          ...
          point->fx, point->fy, (double)point->ox / 64, (double)point->oy / 64,
          (double)point->x / 64, (double)point->y / 64,

          af_print_idx( buf5, 16,
                        af_get_strong_edge_index( hints, point->before, 1 ) ),
          af_print_idx( buf6, 16,
                        af_get_strong_edge_index( hints, point->after, 1 ) ),
          af_print_idx( buf7, 16,
                        af_get_strong_edge_index( hints, point->before, 0 ) ),
          af_print_idx(
              buf8, 16,
              af_get_strong_edge_index( hints, point->after, 0 ) ) ) );
    }
    AF_DUMP( ( "\n" ) );
  }
#ifdef __cplusplus
...
#endif
  void
  af_glyph_hints_dump_segments( AF_GlyphHints hints, FT_Bool to_stdout )
  {
    FT_Int dimension;
      ...
      for ( seg = segments; seg < limit; seg++ )
        AF_DUMP(
            ( "  %5d  %5d  %5d  %5s  %4d  %4d"
              ...
              af_print_idx( buf1, 16, AF_INDEX_NUM( seg->link, segments ) ),
              af_print_idx( buf2, 16, AF_INDEX_NUM( seg->serif, segments ) ),
              af_print_idx( buf3, 16, AF_INDEX_NUM( seg->edge, edges ) ),

              seg->height, seg->height - ( seg->max_coord - seg->min_coord ),
...
#endif
  void
  af_glyph_hints_dump_edges( AF_GlyphHints hints, FT_Bool to_stdout )
  {
    FT_Int dimension;
    ...
    for ( dimension = 1; dimension >= 0; dimension-- )
    {
      ...
      for ( edge = edges; edge < limit; edge++ )
        AF_DUMP( (
            "  %5d  %7.2f  %5s  %4s  %5s"
            "    %c   %7.2f  %7.2f  %11s\n",
            AF_INDEX_NUM( edge, edges ), (double)(int)edge->opos / 64,
            af_dir_str( (AF_Direction)edge->dir ),
            af_print_idx( buf1, 16, AF_INDEX_NUM( edge->link, edges ) ),
            af_print_idx( buf2, 16, AF_INDEX_NUM( edge->serif, edges ) ),

            edge->blue_edge ? 'y' : 'n', (double)edge->opos / 64,
            (double)edge->pos / 64, af_edge_flags_to_string( edge->flags ) ) );
      AF_DUMP( ( "\n" ) );
    }
  }
#ifdef __cplusplus
...
static void
af_iup_shift( AF_Point p1, AF_Point p2, AF_Point ref )
{
  AF_Point p;
...
static void
af_iup_interp( AF_Point p1, AF_Point p2, AF_Point ref1, AF_Point ref2 )
{
  AF_Point p;
