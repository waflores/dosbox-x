...
static void
af_latin_sort_blue( FT_UInt count, AF_LatinBlue* table )
{
  FT_UInt      i, j;
  AF_LatinBlue swap;
...
static int
af_latin_metrics_init_blues( AF_LatinMetrics metrics, FT_Face face )
{
  ...
  for ( ; bs->string != AF_BLUE_STRING_MAX; bs++ )
  {
    ...
    while ( *p )
    {
      ...
      for ( i = 0; i < num_idx; i++ )
      {
        ...
        glyph_index = af_shaper_get_elem( &metrics->root, shaper_buf, i, NULL,
                                          &y_offset );
        if ( glyph_index == 0 )
        {
          FT_TRACE5( ( "  U+%04lX unavailable\n", ch ) );
        ...
        if ( best_point >= 0 )
        {
          ...
          if ( best_on_point_first >= 0 && best_on_point_last >= 0 &&
               ( FT_ABS( points[best_on_point_last].x -
                         points[best_on_point_first].x ) ) > flat_threshold )
            round = 0;
          else
          ...
          if ( dist == 0 )
          {
            ...
            FT_TRACE5(
                ( "                           "
                  ...
                  " from %.5f to %.5f (by %ld%%)\n",
                  (double)scale / 65536, (double)new_scale / 65536,
        ...
        if ( point->out_dir != segment_dir || point == last )
        {
          ...
          else
          {
            ...
            if ( prev_segment->last->in_dir == point->in_dir )
            {
              ...
              prev_segment->height =
                  prev_segment->max_coord - prev_segment->min_coord;
            }
            else
            {
                   ...
                   FT_ABS( max_coord - min_coord ) )
              {
                ...
                prev_segment->last = point;
                prev_segment->pos =
                    (FT_Short)( ( prev_min_pos + prev_max_pos ) >> 1 );
                prev_segment->delta =
                    (FT_Short)( ( prev_max_pos - prev_min_pos ) >> 1 );
              }
              else
              {
...
af_latin_hints_compute_edges( AF_GlyphHints hints, AF_Dimension dim )
{
  ...
  for ( seg = segments; seg < segment_limit; seg++ )
  {
    ...
    if ( !found )
    {
      ...
      error = af_axis_hints_new_edge( axis, seg->pos, (AF_Direction)seg->dir,
...
static FT_Pos
af_latin_snap_width( AF_Width widths, FT_UInt count, FT_Pos width )
{
  FT_UInt n;
  FT_Pos  best      = 64 + 32 + 2;
                            ...
                            AF_Edge       stem_edge )
{
  ...
  FT_TRACE5(
      ( "  LINK: edge %ld (opos=%.2f) linked to %.2f,"
        " dist was %.2f, now %.2f\n",
        stem_edge - hints->axis[dim].edges, (double)stem_edge->opos / 64,
        (double)stem_edge->pos / 64, (double)dist / 64,
...
static void
af_latin_align_serif_edge( AF_GlyphHints hints, AF_Edge base, AF_Edge serif )
{
  FT_UNUSED( hints );
      ...
      if ( !anchor )
        FT_TRACE5(
            ( "  BLUE_ANCHOR: edge %ld (opos=%.2f) snapped to %.2f,"
              ...
              edge1 - edges, (double)edge1->opos / 64, (double)blue->fit / 64,
    ...
    if ( !anchor )
    {
      ...
      FT_TRACE5(
          ( "  ANCHOR: edge %ld (opos=%.2f) and %ld (opos=%.2f)"
            ...
            edge - edges, (double)edge->opos / 64, edge2 - edges,
            (double)edge2->opos / 64, (double)edge->pos / 64,
            (double)edge2->pos / 64 ) );
      ...
      if ( delta < 64 + 16 )
      {
        ...
        FT_TRACE5(
            ( "  SERIF: edge %ld (opos=%.2f) serif to %ld (opos=%.2f)"
              " aligned to %.2f\n",
              edge - edges, (double)edge->opos / 64, edge->serif - edges,
              (double)edge->serif->opos / 64, (double)edge->pos / 64 ) );
      }
      else if ( !anchor )
      {
        edge->pos = FT_PIX_ROUND( edge->opos );
        ...
        FT_TRACE5(
            ( "  SERIF_ANCHOR: edge %ld (opos=%.2f)"
              " snapped to %.2f\n",
              edge - edges, (double)edge->opos / 64, (double)edge->pos / 64 ) );
      }
      else
      {
