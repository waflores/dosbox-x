...
#define COVERAGE( name, NAME, description, tag1, tag2, tag3, tag4 ) \
  static const hb_tag_t name##_coverage[] = {                       \
      HB_TAG( tag1, tag2, tag3, tag4 ), HB_TAG_NONE };
                        ...
                        FT_Bool        default_script )
{
  ...
  for ( idx = HB_SET_VALUE_INVALID; hb_set_next( gsub_lookups, &idx ); )
  {
    ...
    hb_ot_layout_lookup_collect_glyphs( face, HB_OT_TAG_GSUB, idx, NULL, NULL,
                                        NULL, gsub_glyphs );
  }
...
void
af_shaper_buf_destroy( FT_Face face, void* buf )
{
  FT_UNUSED( face );
