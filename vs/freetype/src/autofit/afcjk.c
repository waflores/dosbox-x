...
static void
af_cjk_metrics_init_blues( AF_CJKMetrics metrics, FT_Face face )
{
  ...
  for ( nn = 0; nn < axis->blue_count; nn++ )
  {
    ...
    if ( dist <= 48 && dist >= -48 )
    {
      ...
      FT_TRACE5( ( ">> active cjk blue zone %c%d[%ld/%ld]:\n",
                   ( dim == AF_DIMENSION_HORZ ) ? 'H' : 'V', nn, blue->ref.org,
                   blue->shoot.org ) );
      FT_TRACE5( ( "     ref:   cur=%.2f fit=%.2f\n",
                   (double)blue->ref.cur / 64, (double)blue->ref.fit / 64 ) );
...
static FT_Error
af_cjk_hints_compute_segments( AF_GlyphHints hints, AF_Dimension dim )
{
...
static void
af_cjk_hints_link_segments( AF_GlyphHints hints, AF_Dimension dim )
{
  AF_AxisHints axis          = &hints->axis[dim];
  AF_Segment   segments      = axis->segments;
  AF_Segment   segment_limit = FT_OFFSET( segments, axis->num_segments );
...
static FT_Error
af_cjk_hints_compute_edges( AF_GlyphHints hints, AF_Dimension dim )
{
  AF_AxisHints axis   = &hints->axis[dim];
  ...
  for ( seg = segments; seg < segment_limit; seg++ )
  {
    ...
    if ( !found )
    {
      ...
      error = af_axis_hints_new_edge( axis, seg->pos, (AF_Direction)seg->dir, 0,
                                      memory, &edge );
      if ( error )
        goto Exit;
...
static FT_Error
af_cjk_hints_detect_features( AF_GlyphHints hints, AF_Dimension dim )
{
  FT_Error error;
...
static FT_Pos
af_cjk_snap_width( AF_Width widths, FT_UInt count, FT_Pos width )
{
  FT_UInt n;
  FT_Pos  best      = 64 + 32 + 2;
...
static void
af_cjk_align_serif_edge( AF_GlyphHints hints, AF_Edge base, AF_Edge serif )
{
  FT_UNUSED( hints );
...
static void
af_cjk_hint_edges( AF_GlyphHints hints, AF_Dimension dim )
{
  AF_AxisHints axis       = &hints->axis[dim];
  AF_Edge      edges      = axis->edges;
  AF_Edge      edge_limit = FT_OFFSET( edges, axis->num_edges );
    ...
    if ( before >= edges || after < edge_limit )
    {
      ...
      else
      {
        if ( after->fpos == before->fpos )
          ...
          edge->pos = before->pos + FT_MulDiv( edge->fpos - before->fpos,
...
static void
af_cjk_align_edge_points( AF_GlyphHints hints, AF_Dimension dim )
{
  AF_AxisHints axis       = &hints->axis[dim];
  AF_Edge      edges      = axis->edges;
  AF_Edge      edge_limit = FT_OFFSET( edges, axis->num_edges );
