...
static FT_Error
sdf_property_get( FT_Module module, const char* property_name, void* value )
{
  FT_Error     error  = FT_Err_Ok;
  SDF_Renderer render = SDF_RENDERER( FT_RENDERER( module ) );
...
FT_DEFINE_SERVICEDESCREC1( sdf_services,

                           FT_SERVICE_ID_PROPERTIES,
                           &sdf_service_properties )

static FT_Module_Interface
ft_sdf_requester( FT_Module module, const char* module_interface )
{
  FT_UNUSED( module );
...
static void
ft_sdf_get_cbox( FT_Renderer render, FT_GlyphSlot slot, FT_BBox* cbox )
{
  FT_ZERO( cbox );
...
static FT_Error
ft_sdf_set_mode( FT_Renderer render, FT_ULong mode_tag, FT_Pointer data )
{
                ...
                const FT_Vector* origin )
{
  ...
  {
    FT_ERROR(
        ( "ft_bsdf_render: can't generate SDF from"
