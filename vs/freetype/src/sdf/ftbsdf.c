 ...
 *       in this rasterizer.
 *
 *     - Stefan Gustavson, Robin Strand: Anti-aliased Euclidean distance
 * transform. http://weber.itn.liu.se/~stegu/aadist/edtaa_preprint.pdf
 *
...
#define CHECK_NEIGHBOR( x_offset, y_offset )                           \
  do                                                                   \
  {                                                                    \
    if ( x + x_offset >= 0 && x + x_offset < w && y + y_offset >= 0 && \
         y + y_offset < r )                                            \
    {                                                                  \
      num_neighbors++;                                                 \
                                                                       \
 ...
 */
static FT_16D16_Vec
compute_edge_distance( ED* current, FT_Int x, FT_Int y, FT_Int w, FT_Int r )
{
  /*
  ...
  g.x = -alphas[0] - FT_MulFix( alphas[3], 92681 ) - alphas[6] + alphas[2] +
        FT_MulFix( alphas[5], 92681 ) + alphas[8];

  g.y = -alphas[0] - FT_MulFix( alphas[1], 92681 ) - alphas[2] + alphas[6] +
        FT_MulFix( alphas[7], 92681 ) + alphas[8];
  ...
  else
  {
    ...
    if ( current_alpha < a1 )
      dist = ( gx + gy ) / 2 -
             square_root( 2 * FT_MulFix( gx, FT_MulFix( gy, current_alpha ) ) );
      ...
      dist = -( gx + gy ) / 2 +
             square_root(
                 2 * FT_MulFix( gx, FT_MulFix( gy, ONE - current_alpha ) ) );
  }
...
bsdf_approximate_edge( BSDF_Worker* worker )
{
  ...
  for ( j = 0; j < worker->rows; j++ )
  {
    for ( i = 0; i < worker->width; i++ )
    {
      ...
      if ( bsdf_is_edge( worker->distance_map + index, i, j, worker->width,
                         worker->rows ) )
      {
        /* approximate the edge distance for edge pixels */
        ed[index].prox = compute_edge_distance( ed + index, i, j, worker->width,
                                                worker->rows );
 ...
 */
static FT_Error
bsdf_init_distance_map( const FT_Bitmap* source, BSDF_Worker* worker )
{
  FT_Error error = FT_Err_Ok;
 ...
 */
static void
compare_neighbor( ED* current, FT_Int x_offset, FT_Int y_offset, FT_Int width )
{
  ED*          to_check;
  FT_16D16     dist;
 ...
 */
static FT_Error
finalize_sdf( BSDF_Worker* worker, const FT_Bitmap* target )
{
  FT_Error error = FT_Err_Ok;
  ...
  sp_sq =
      (FT_16D16)FT_INT_16D16( worker->params.spread * worker->params.spread );
#else
...
/* unused */
static FT_Error
bsdf_raster_set_mode( FT_Raster raster, unsigned long mode, void* args )
{
  FT_UNUSED( raster );
  FT_UNUSED( mode );
  FT_UNUSED( args );
...
static FT_Error
bsdf_raster_render( FT_Raster raster, const FT_Raster_Params* params )
{
  FT_Error  error  = FT_Err_Ok;
  ...
  FT_TRACE0(
      ( "bsdf_raster_render: Total memory used = %ld\n",
        worker.width * worker.rows * (long)sizeof( *worker.distance_map ) ) );
