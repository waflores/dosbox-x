...
#define VEC_26D6_DOT( p, q ) ( MUL_26D6( p.x, q.x ) + MUL_26D6( p.y, q.y ) )
...
static const FT_Vector zero_vector = { 0, 0 };

static const SDF_Edge null_edge = { { 0, 0 }, { 0, 0 },           { 0, 0 },
                                    { 0, 0 }, SDF_EDGE_UNDEFINED, NULL };

static const SDF_Contour null_contour = { { 0, 0 }, NULL, NULL };

static const SDF_Shape null_shape = { NULL, NULL };

static const SDF_Signed_Distance max_sdf = { INT_MAX, 0, 0 };

/* Create a new @SDF_Edge on the heap and assigns the `edge` */
/* pointer to the newly allocated memory.                    */
static FT_Error
sdf_edge_new( FT_Memory memory, SDF_Edge** edge )
{
...
static void
sdf_edge_done( FT_Memory memory, SDF_Edge** edge )
{
  if ( !memory || !edge || !*edge )
    return;
...
static FT_Error
sdf_contour_new( FT_Memory memory, SDF_Contour** contour )
{
  FT_Error     error = FT_Err_Ok;
...
static void
sdf_contour_done( FT_Memory memory, SDF_Contour** contour )
{
  SDF_Edge* edges;
  SDF_Edge* temp;
...
static FT_Error
sdf_shape_new( FT_Memory memory, SDF_Shape** shape )
{
  FT_Error   error = FT_Err_Ok;
  SDF_Shape* ptr   = NULL;
...
static FT_Error
sdf_move_to( const FT_26D6_Vec* to, void* user )
{
...
/* stops at `to`.                                           */
static FT_Error
sdf_line_to( const FT_26D6_Vec* to, void* user )
{
  SDF_Shape*   shape   = (SDF_Shape*)user;
  SDF_Edge*    edge    = NULL;
  SDF_Contour* contour = NULL;
...
static FT_Error
sdf_conic_to( const FT_26D6_Vec* control_1, const FT_26D6_Vec* to, void* user )
{
  SDF_Shape*   shape   = (SDF_Shape*)user;
  SDF_Edge*    edge    = NULL;
  SDF_Contour* contour = NULL;
...
FT_DEFINE_OUTLINE_FUNCS( sdf_decompose_funcs,
...
static FT_Error
sdf_outline_decompose( FT_Outline* outline, SDF_Shape* shape )
{
  FT_Error error = FT_Err_Ok;
...
get_control_box( SDF_Edge edge )
{
  ...
  case SDF_EDGE_CONIC:
    if ( is_set )
    {
      cbox.xMin = edge.control_a.x < cbox.xMin ? edge.control_a.x : cbox.xMin;
      cbox.xMax = edge.control_a.x > cbox.xMax ? edge.control_a.x : cbox.xMax;

      cbox.yMin = edge.control_a.y < cbox.yMin ? edge.control_a.y : cbox.yMin;
      cbox.yMax = edge.control_a.y > cbox.yMax ? edge.control_a.y : cbox.yMax;
    }
    else
    {
  ...
  case SDF_EDGE_LINE:
    if ( is_set )
    {
      cbox.xMin = edge.start_pos.x < cbox.xMin ? edge.start_pos.x : cbox.xMin;
      cbox.xMax = edge.start_pos.x > cbox.xMax ? edge.start_pos.x : cbox.xMax;

      cbox.yMin = edge.start_pos.y < cbox.yMin ? edge.start_pos.y : cbox.yMin;
      cbox.yMax = edge.start_pos.y > cbox.yMax ? edge.start_pos.y : cbox.yMax;
    }
    else
    {
    ...
    cbox.xMin = edge.end_pos.x < cbox.xMin ? edge.end_pos.x : cbox.xMin;
    cbox.xMax = edge.end_pos.x > cbox.xMax ? edge.end_pos.x : cbox.xMax;

    cbox.yMin = edge.end_pos.y < cbox.yMin ? edge.end_pos.y : cbox.yMin;
    cbox.yMax = edge.end_pos.y > cbox.yMax ? edge.end_pos.y : cbox.yMax;
    ...
    while ( edge_list )
    {
      ...
      case SDF_EDGE_LINE:
        FT_TRACE5( ( "Line:  (%ld, %ld) -- (%ld, %ld)\n", edge->start_pos.x,
                     edge->start_pos.y, edge->end_pos.x, edge->end_pos.y ) );
        total_lines++;
        break;

      case SDF_EDGE_CONIC:
        FT_TRACE5( ( "Conic: (%ld, %ld) .. (%ld, %ld) .. (%ld, %ld)\n",
                     edge->start_pos.x, edge->start_pos.y, edge->control_a.x,
                     edge->control_a.y, edge->end_pos.x, edge->end_pos.y ) );
        total_conic++;
        break;
      ...
      case SDF_EDGE_CUBIC:
        FT_TRACE5(
            ( "Cubic: (%ld, %ld) .. (%ld, %ld)"
              " .. (%ld, %ld) .. (%ld %ld)\n",
              edge->start_pos.x, edge->start_pos.y, edge->control_a.x,
              edge->control_a.y, edge->control_b.x, edge->control_b.y,
...
cube_root( FT_16D16 val )
{
  ...
  if ( val == 0 || val == -FT_INT_16D16( 1 ) || val == FT_INT_16D16( 1 ) )
    return val;
...
static FT_UShort
solve_quadratic_equation( FT_26D6 a, FT_26D6 b, FT_26D6 c, FT_16D16 out[2] )
{
...
resolve_corner( SDF_Signed_Distance sdf1, SDF_Signed_Distance sdf2 )
{
                       ...
                       SDF_Signed_Distance* out )
{
  ...
  factor =
      ( p_sub_a.x * line_segment.x ) / 64 + ( p_sub_a.y * line_segment.y ) / 64;
  ...
  nearest_point.x = FT_MulFix( FT_26D6_16D16( line_segment.x ), factor );
  nearest_point.y = FT_MulFix( FT_26D6_16D16( line_segment.y ), factor );
                        ...
                        SDF_Signed_Distance* out )
{
  ...
  c = 2 * VEC_26D6_DOT( bB, bB ) + VEC_26D6_DOT( aA, p0 ) -
      VEC_26D6_DOT( aA, p );

  d = VEC_26D6_DOT( p0, bB ) - VEC_26D6_DOT( p, bB );
  ...
  for ( i = 0; i < num_roots; i++ )
  {
    ...
    /* B(t) = t^2 * A + 2t * B + p0 - p */
    curve_point.x = FT_MulFix( aA.x, t2 ) + 2 * FT_MulFix( bB.x, t ) + p0.x;
    curve_point.y = FT_MulFix( aA.y, t2 ) + 2 * FT_MulFix( bB.y, t ) + p0.y;
                        ...
                        SDF_Signed_Distance* out )
{
  ...
  for ( iterations = 0; iterations <= MAX_NEWTON_DIVISIONS; iterations++ )
  {
    ...
    for ( steps = 0; steps < MAX_NEWTON_STEPS; steps++ )
    {
      ...
      /* B(t) = t^2 * A + t * B + p0 */
      curve_point.x =
          FT_MulFix( aA.x, factor2 ) + FT_MulFix( bB.x, factor ) + cC.x;
      curve_point.y =
          FT_MulFix( aA.y, factor2 ) + FT_MulFix( bB.y, factor ) + cC.y;
      ...
      temp2 = VEC_26D6_DOT( d1, d1 ) + VEC_26D6_DOT( dist_vector, d2 );
                        ...
                        SDF_Signed_Distance* out )
{
  ...
  for ( iterations = 0; iterations <= MAX_NEWTON_DIVISIONS; iterations++ )
  {
    ...
    for ( steps = 0; steps < MAX_NEWTON_STEPS; steps++ )
    {
      ...
      curve_point.x = FT_MulFix( aA.x, factor3 ) + FT_MulFix( bB.x, factor2 ) +
                      FT_MulFix( cC.x, factor ) + dD.x;
      curve_point.y = FT_MulFix( aA.y, factor3 ) + FT_MulFix( bB.y, factor2 ) +
      ...
      /* B'(t) = 3t^2 * A + 2t * B + C */
      d1.x =
          FT_MulFix( aA.x, 3 * factor2 ) + FT_MulFix( bB.x, 2 * factor ) + cC.x;
      d1.y =
          FT_MulFix( aA.y, 3 * factor2 ) + FT_MulFix( bB.y, 2 * factor ) + cC.y;
                           ...
                           const FT_Bitmap* bitmap )
{
  ...
  if ( FT_ALLOC( dists, bitmap->width * bitmap->rows * sizeof( *dists ) ) )
    goto Exit;
    ...
    while ( edges )
    {
      ...
      for ( y = cbox.yMin; y < cbox.yMax; y++ )
      {
        for ( x = cbox.xMin; x < cbox.xMax; x++ )
        {
          ...
          FT_CALL( sdf_edge_get_min_distance( edges, grid_point, &dist ) );
  ...
  for ( j = 0; j < rows; j++ )
  {
    ...
    for ( i = 0; i < width; i++ )
    {
      ...
      dists[index].distance *=
          internal_params.flip_sign ? -current_sign : current_sign;
                            ...
                            const FT_Bitmap* bitmap )
{
  ...
  if ( FT_ALLOC( bitmaps, (FT_UInt)num_contours * sizeof( *bitmaps ) ) )
    goto Exit;
  ...
  for ( i = 0; i < num_contours; i++ )
  {
    ...
    if ( FT_ALLOC( bitmaps[i].buffer, bitmap->rows * (FT_UInt)bitmap->pitch ) )
      goto Exit;
    ...
    FT_CALL( sdf_generate_subdivision( internal_params, &temp_shape, spread,
                                       &bitmaps[i] ) );
...
static FT_Error
sdf_raster_set_mode( FT_Raster raster, unsigned long mode, void* args )
{
  FT_UNUSED( raster );
  FT_UNUSED( mode );
  FT_UNUSED( args );
...
static FT_Error
sdf_raster_render( FT_Raster raster, const FT_Raster_Params* params )
{
  FT_Error                 error      = FT_Err_Ok;
  ...
  if ( sdf_params->overlaps )
    FT_CALL( sdf_generate_with_overlaps(
        internal_params, shape, sdf_params->spread, sdf_params->root.target ) );
  else
    FT_CALL( sdf_generate_subdivision(
        internal_params, shape, sdf_params->spread, sdf_params->root.target ) );
