...
ft_svg_done( FT_Module module )
{
  ...
  if ( render->loaded == TRUE && render->hooks_set == TRUE )
...
static FT_Error
ft_svg_preset_slot( FT_Module module, FT_GlyphSlot slot, FT_Bool cache )
{
  ...
  {
    FT_TRACE3(
        ( "ft_svg_preset_slot: first presetting call,"
          " calling init hook\n" ) );
                     ...
                     FT_Bool     value_is_string )
{
  ...
  if ( !ft_strcmp( property_name, "svg-hooks" ) )
  {
    ...
    if ( !hooks->init_svg || !hooks->free_svg || !hooks->render_svg ||
         !hooks->preset_slot )
    {
      FT_TRACE0(
          ( "ft_svg_property_set:"
...
static FT_Error
ft_svg_property_get( FT_Module module, const char* property_name, void* value )
{
  FT_Error     error    = FT_Err_Ok;
  SVG_Renderer renderer = (SVG_Renderer)module;
...
FT_DEFINE_SERVICEDESCREC1( ft_svg_services,
                           FT_SERVICE_ID_PROPERTIES,
                           &ft_svg_service_properties )

FT_CALLBACK_DEF( FT_Module_Interface )
ft_svg_get_interface( FT_Module module, const char* ft_svg_interface )
{
...
FT_DEFINE_RENDERER(
    ...
    (const void*)
        PUT_SVG_MODULE( &svg_interface ), /* module specific interface */

    PUT_SVG_MODULE( ft_svg_init ), /* FT_Module_Constructor module_init   */
    PUT_SVG_MODULE( ft_svg_done ), /* FT_Module_Destructor  module_done   */
    PUT_SVG_MODULE(
        ft_svg_get_interface ), /* FT_Module_Requester   get_interface */

    SVG_GLYPH_FORMAT,

    PUT_SVG_MODULE(
        ft_svg_render ), /* FT_Renderer_RenderFunc    render_glyph    */
    PUT_SVG_MODULE(
        ft_svg_transform ), /* FT_Renderer_TransformFunc transform_glyph */
