...
static size_t
cf2_hintmask_setCounts( CF2_HintMask hintmask, size_t bitCount )
{
...
static void
cf2_doBlend( const CFF_Blend blend, CF2_Stack opStack, CF2_UInt numBlends )
{
  CF2_UInt delta;
  CF2_UInt base;
  CF2_UInt i, j;
...
#define PS_STORAGE_SIZE 3
  ...
  cf2_arrstack_init( &hStemHintArray, memory, error,
                     sizeof( CF2_StemHintRec ) );
  cf2_arrstack_init( &vStemHintArray, memory, error,
                     sizeof( CF2_StemHintRec ) );
  ...
  *charstring     = *buf; /* structure copy     */
  ...
  while ( 1 )
  {
    ...
    case cf2_cmdBLEND:
    {
      ...
      if ( font->cffload->blend_check_vector( &font->blend, font->vsindex,
    ...
    case cf2_cmdCALLSUBR:
    {
      ...
      FT_TRACE4(
          ( "%s", op1 == cf2_cmdCALLGSUBR ? " callgsubr" : " callsubr" ) );
    ...
    case cf2_cmdESC:
    {
      ...
      case cf2_escHFLEX:
      {
        static const FT_Bool readFromStack[12] = {
            TRUE /* dx1 */, FALSE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */,
            TRUE /* dx3 */, FALSE /* dy3 */, TRUE /* dx4 */, FALSE /* dy4 */,
            TRUE /* dx5 */, FALSE /* dy5 */, TRUE /* dx6 */, FALSE /* dy6 */
        };

        FT_TRACE4( ( " hflex\n" ) );

        cf2_doFlex( opStack, &curX, &curY, &glyphPath, readFromStack,
      ...
      {
        static const FT_Bool readFromStack[12] = {
            TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */,
            TRUE /* dx3 */, TRUE /* dy3 */, TRUE /* dx4 */, TRUE /* dy4 */,
        ...
        else
        {
          ...
          case cf2_escHSTEM3:
             ...
             */
            {
              ...
              else
              {
                ...
                cf2_doStems( font, opStack,
                             isV ? &vStemHintArray : &hStemHintArray, width,
                             &haveWidth,
          ...
          case cf2_escSEAC:
            ...
            else
            {
              ...
              error2 = cf2_getT1SeacComponent( decoder, (FT_UInt)bchar_index,
                                               &component );
              if ( error2 )
              {
                lastError = error2; /* pass FreeType error through */
              ...
              cf2_interpT2CharString( font, &component, callbacks, translation,
                                      TRUE, 0, 0, &dummyWidth );
              ...
              error2 = cf2_getT1SeacComponent( decoder, (FT_UInt)achar_index,
                                               &component );
              if ( error2 )
              {
                lastError = error2; /* pass FreeType error through */
          ...
          case cf2_escCALLOTHERSUBR:
            ...
            else
            {
              ...
              case 0: /* end flex feature */
                ...
                if ( initial_map_ready && ( !decoder->flex_state ||
              ...
              case 18: /* multiple masters */
              {
                ...
                for ( nn = 0; nn < num_points; nn++ )
                {
                  ...
                  for ( mm = 1; mm < blend->num_designs; mm++ )
                    tmp = ADD_INT32(
                        tmp, FT_MulFix( cf2_stack_getReal( opStack, delta++ ),
              ...
              case 28:
                ...
                /*     onto stack                             */
                {
                  ...
                  r = (CF2_F16Dot16)( ( decoder->current_subfont->random &
                                        0xFFFF ) +
                                      1 );
          ...
          case cf2_escRANDOM: /* in spec */
          {
            ...
            /* only use the lower 16 bits of `random'  */
            /* to generate a number in the range (0;1] */
            r = (CF2_F16Dot16)( ( decoder->current_subfont->random & 0xFFFF ) +
                                1 );
    ...
    case cf2_cmdENDCHAR:
      ...
      if ( !font->isCFF2 && !font->isT1 && cf2_stack_count( opStack ) > 1 )
      {
        ...
        cf2_interpT2CharString( font, &component, callbacks, translation, TRUE,
                                curX, curY, &dummyWidth );
    ...
    case cf2_cmdHINTMASK:
      ...
      else
      {
        ...
        cf2_hintmap_init( &counterHintMap, font, &glyphPath.initialHintMap,
        ...
        cf2_hintmask_read( &counterMask, charstring,
                           cf2_arrstack_size( &hStemHintArray ) +
                               cf2_arrstack_size( &vStemHintArray ) );
        cf2_hintmap_build( &counterHintMap, &hStemHintArray, &vStemHintArray,
                           &counterMask, 0, FALSE );
      }
      break;
    ...
    default:
      /* numbers */
      {
        ...
        else /* op1 == 255 */
        {
          ...
          v = (CF2_Fixed)( ( byte1 << 24 ) | ( byte2 << 16 ) | ( byte3 << 8 ) |
                           byte4 );
