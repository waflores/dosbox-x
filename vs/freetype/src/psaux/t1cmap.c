...
static void
t1_cmap_std_init( T1_CMapStd cmap, FT_Int is_expert )
{
  ...
  cmap->code_to_sid =
      is_expert ? psnames->adobe_expert_encoding : psnames->adobe_std_encoding;
...
FT_CALLBACK_DEF( FT_UInt )
t1_cmap_std_char_next( FT_CMap cmap, FT_UInt32* pchar_code )
{
  FT_UInt   result    = 0;
...
psaux_get_glyph_name( void* face_, FT_UInt idx )
{
                      ...
                      FT_Pointer pointer )
{
  ...
  return psnames->unicodes_init(
      memory, unicodes, (FT_UInt)face->type1.num_glyphs, &psaux_get_glyph_name,
