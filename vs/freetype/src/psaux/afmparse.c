...
afm_parser_read_vals( AFM_Parser parser, AFM_Value vals, FT_Int n )
{
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    case AFM_VALUE_TYPE_FIXED:
      val->u.f =
          PS_Conv_ToFixed( (FT_Byte**)(void*)&str, (FT_Byte*)str + len, 0 );
      break;
...
static AFM_Token
afm_tokenize( const char* key, FT_Offset len )
{
  int n;
...
static FT_Error
afm_parser_skip_section( AFM_Parser parser, FT_Int n, AFM_Token end_section )
{
  char*     key;
  FT_Offset len;
