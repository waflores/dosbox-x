...
#define FT_STRUCTURE WinMZ_HeaderRec
    ...
    FT_FRAME_START( 64 ), FT_FRAME_USHORT_LE( magic ),
    FT_FRAME_SKIP_BYTES( 29 * 2 ), FT_FRAME_ULONG_LE( lfanew ), FT_FRAME_END };
...
static FT_Error
fnt_font_load( FNT_Font font, FT_Stream stream )
{
  FT_Error         error;
...
static FT_Error
fnt_face_get_dll_font( FNT_Face face, FT_Int face_instance_index )
{
  FT_Error        error;
  FT_Stream       stream = FT_FACE( face )->stream;
  FT_Memory       memory = FT_FACE( face )->memory;
                ...
                k++ )
          {
            ...
            if ( dir_entry1.name == 8 /* RT_FONT */ )
            {
              ...
              FT_TRACE2( (
                  ...
                  "size %04lx, cp %lu\n",
                  dir_entry2.name,
                  pe32_section.pointer_to_raw_data + data_entry.offset_to_data -
               ...
               FT_Pointer pointer )
{
  ...
  fntcmap->count = (FT_UInt32)( font->header.last_char - fntcmap->first + 1 );
               ...
               FT_Parameter* params )
{
  ...
  /* with relevant information                   */
  {
    ...
    FT_MEM_COPY( font->family_name,
                 font->fnt_frame + font->header.face_name_offset, family_size );

    font->family_name[family_size] = '\0';

    /* shrink it to the actual length */
    if ( FT_QREALLOC( font->family_name, family_size + 1,
...
static FT_Error
FNT_Size_Select( FT_Size size, FT_ULong strike_index )
{
...
static FT_Error
FNT_Size_Request( FT_Size size, FT_Size_Request req )
{
  FNT_Face         face   = (FNT_Face)size->face;
  FT_WinFNT_Header header = &face->font->header;
...
static FT_Error
winfnt_get_header( FT_Face face, FT_WinFNT_HeaderRec* aheader )
{
...
winfnt_get_service( FT_Module module, const FT_String* service_id )
{
  FT_UNUSED( module );
