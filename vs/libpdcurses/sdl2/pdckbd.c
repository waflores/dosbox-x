...
bool PDC_check_key(void) {
  ...
  if (haveevent || current < pdc_lastupdate ||
      ((current - pdc_lastupdate) > 30))
    PDC_update_rects();
...
static int _process_mouse_event(void) {
  ...
  if (event.type == SDL_MOUSEMOTION) {
    ...
    if (!event.motion.state || (pdc_mouse_status.x == old_mouse_status.x &&
    ...
    short action =
        (event.button.state == SDL_PRESSED) ? BUTTON_PRESSED : BUTTON_RELEASED;
...
int PDC_get_key(void) {
  ...
  case SDL_WINDOWEVENT:
    ...
    case SDL_WINDOWEVENT_RESIZED:
      if (pdc_own_window && (event.window.data2 / pdc_fheight != LINES ||
