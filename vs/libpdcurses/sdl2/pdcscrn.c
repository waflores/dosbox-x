...
int PDC_scr_open(int argc, char **argv) {
  ...
  if (!pdc_ttffont) {
    ...
    pdc_ttffont = TTF_OpenFont(fname ? fname : PDC_FONT_PATH, pdc_font_size);
  }
  ...
  if (pdc_own_window && !pdc_icon) {
    ...
    if (!pdc_icon)
      pdc_icon = SDL_LoadBMP_RW(SDL_RWFromMem(deficon, sizeof(deficon)), 0);
  }
  ...
  if (pdc_own_window) {
    ...
    pdc_window = SDL_CreateWindow(
        (argc ? argv[0] : "PDCurses"), SDL_WINDOWPOS_UNDEFINED,
        SDL_WINDOWPOS_UNDEFINED, pdc_swidth, pdc_sheight, SDL_WINDOW_RESIZABLE);
    if (pdc_window == NULL) {
