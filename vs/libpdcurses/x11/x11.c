...
static int font_height, font_width, font_ascent, font_descent, window_width,
    window_height;
...
static int state_mask[8] = {ShiftMask, LockMask, ControlMask, Mod1Mask,
...
static int selection_start_x = 0, selection_start_y = 0, selection_end_x = 0,
           selection_end_y = 0;
...
#define RINT(name1, name2, value)                                              \
  {#name1,       #name2,          XtRInt, sizeof(int), APPDATAOFF(name1),      \
   XtRImmediate, (XtPointer)value}
...
#define RSTRINGP(name1, name2, param)                                          \
  {#name1,    #name2,          XtRString, MAX_PATH, APPDATAOFF(name1),         \
   XtRString, (XtPointer)param}
...
static XrmOptionDescRec options[] = {COPT(lines),
                                     COPT(cols),
                                     COPT(normalFont),
                                     COPT(italicFont),
                                     COPT(boldFont),
                                     ...
                                     COPT(composeKey),
                                     COPT(clickPeriod),
                                     COPT(doubleClickPeriod),
                                     COPT(scrollbarWidth),
                                     COPT(pointerForeColor),
                                     COPT(pointerBackColor),
                                     COPT(cursorBlinkRate),
                                     COPT(cursorColor),
                                     COPT(textCursor),
                                     COPT(textBlinkRate),
...
static const char *default_translations = {"<Key>: XCursesKeyPress() \n"
...
void XCursesSigwinchHandler(int signo) {
  PDC_LOG(
      ("%s:XCursesSigwinchHandler() - called: SIGNO: %d\n", XCLOGMSG, signo));
...
#endif
{
  ...
  if (blinked_off && (sysattrs & A_BLINK) && (attr & A_BLINK)) {
    ...
    XFillRectangle(XCURSESDISPLAY, XCURSESWIN, gc, xpos, bounds.y, bounds.width,
                   font_height);
  } else {
    /* Draw it */
    ...
    if (attr & (A_LEFT | A_RIGHT | A_UNDERLINE)) {
      ...
      if (attr & A_LEFT)
        for (k = 0; k < len; k++) {
          ...
          XDrawLine(XCURSESDISPLAY, XCURSESWIN, gc, x, ypos - font_ascent, x,
                    ypos + font_descent);
        }
...
#define RGB(R, G, B)                                                           \
  (((unsigned long)(R) << 16) | ((unsigned long)(G) << 8) |                    \
   ((unsigned long)(B)))
...
static void _get_icon(void) {
  ...
  unsigned icon_bitmap_width = 0, icon_bitmap_height = 0, file_bitmap_width = 0,
           file_bitmap_height = 0;
  ...
  rc =
      XGetIconSizes(XtDisplay(topLevel), RootWindowOfScreen(XtScreen(topLevel)),
  ...
  if (rc && size_count) {
    ...
    for (i = 0; i < size_count; i++) {
      ...
      PDC_LOG(("%s:min: %d %d\n", XCLOGMSG, icon_size[i].min_width,
               icon_size[i].min_height));

      PDC_LOG(("%s:max: %d %d\n", XCLOGMSG, icon_size[i].max_width,
               icon_size[i].max_height));

      PDC_LOG(("%s:inc: %d %d\n", XCLOGMSG, icon_size[i].width_inc,
  ...
  if (xc_app_data.pixmap && xc_app_data.pixmap[0]) /* supplied pixmap */
  {
    XpmReadFileToPixmap(
        ...
        (char *)xc_app_data.pixmap, &icon_pixmap, &icon_pixmap_mask, NULL);
    return;
  }
#endif
  ...
  if (xc_app_data.bitmap && xc_app_data.bitmap[0]) /* supplied bitmap */
  {
    ...
    rc = XReadBitmapFile(XtDisplay(topLevel),
                         ...
                         (char *)xc_app_data.bitmap, &file_bitmap_width,
                         &file_bitmap_height, &icon_bitmap, &x_hot, &y_hot);
...
static void _draw_border(void) {
  ...
  if (xc_app_data.borderWidth)
    XDrawRectangle(XCURSESDISPLAY, XCURSESWIN, border_gc,
                   xc_app_data.borderWidth / 2, xc_app_data.borderWidth / 2,
...
static void _display_screen(void) {
  ...
  for (row = 0; row < XCursesLINES; row++) {
    ...
    _display_text((const chtype *)(Xcurscr + XCURSCR_Y_OFF(row)), row, 0, COLS,
                  FALSE);
...
static void XCursesKeyPress(Widget w, XEvent *event, String *params,
...
#ifdef PDC_XIM
  count =
      XwcLookupString(Xic, &(event->xkey), buffer, buflen, &keysym, &status);
#else
  count =
      XLookupString(&(event->xkey), (char *)buffer, buflen, &keysym, &compose);
#endif
  ...
  if (keysym == compose_key) {
    ...
    XDrawRectangle(XCURSESDISPLAY, XCURSESWIN, rect_cursor_gc, xpos + 1,
                   ypos - font_height + xc_app_data.normalFont->descent + 1,
  ...
  case STATE_CHAR:
    ...
    _send_key_to_curses(compose_keys[compose_index][char_idx], NULL, FALSE);
  ...
  for (i = 0; key_table[i].keycode; i++) {
    if (key_table[i].keycode == keysym) {
      ...
      if ((event->xkey.state & ShiftMask) ||
          (key_table[i].numkeypad && (event->xkey.state & Mod2Mask))) {
...
static void _paste_utf8(Widget w, XtPointer event, Atom *selection, Atom *type,
  ...
  if (!*type || !*length) {
    XtGetSelectionValue(w, XA_PRIMARY, XA_STRING, _paste_string, event,
...
static void XCursesPasteSelection(Widget w, XButtonEvent *button_event) {
  ...
  XtGetSelectionValue(w, XA_PRIMARY, XA_UTF8_STRING(XtDisplay(w)), _paste_utf8,
...
static Boolean _convert_proc(Widget w, Atom *selection, Atom *target,
  ...
  if (*target == XA_TARGETS(XtDisplay(topLevel))) {
    ...
    XmuConvertStandardSelection(topLevel, req->time, selection, target,
                                type_return, &std_targets, &std_length,
                                format_return);
    ...
    *type_return = *target;
    ...
    return XmuConvertStandardSelection(topLevel, CurrentTime, selection, target,
                                       type_return, (XPointer *)value_return,
...
static void _selection_extend(int x, int y) {
  int temp, current_start, current_end, current_start_x, current_end_x,
      current_start_y, current_end_y, new_start, new_end, new_start_x,
      new_end_x, new_start_y, new_end_y;
  ...
  if (new_end > current_end)
    _show_selection(current_end_x, current_end_y, new_end_x, new_end_y, TRUE);
  else if (new_end < current_end)
    _show_selection(new_end_x, new_end_y, current_end_x, current_end_y, FALSE);
  else if (new_start < current_start)
    _show_selection(new_start_x, new_start_y, current_start_x, current_start_y,
                    TRUE);
  else if (new_start > current_start)
    _show_selection(current_start_x, current_start_y, new_start_x, new_start_y,
                    FALSE);
  else
    _show_selection(current_start_x, current_start_y, new_start_x, new_start_y,
                    TRUE);
...
static void _selection_set(void) {
  int i, j, start, end, start_x, end_x, start_y, end_y, num_cols, start_col,
      row, num_chars, ch, last_nonblank, length, newlen;
  chtype *ptr = NULL;
...
static void _display_cursor(int old_row, int old_x, int new_row, int new_x) {
  ...
  if (old_row >= XCursesLINES || old_x >= COLS || new_row >= XCursesLINES ||
      new_x >= COLS)
    return;
  ...
  if (vertical_cursor) {
    ...
    for (i = 1; i <= SP->visibility; i++)
      XDrawLine(XCURSESDISPLAY, XCURSESWIN, rect_cursor_gc, xpos + i,
                ypos - xc_app_data.normalFont->ascent, xpos + i,
                ypos - xc_app_data.normalFont->ascent + font_height - 1);
  } else {
    ...
    XFillRectangle(XCURSESDISPLAY, XCURSESWIN, rect_cursor_gc, xpos, yp,
                   font_width, yh);
  }
...
static void _handle_enter_leave(Widget w, XtPointer client_data, XEvent *event,
                                Boolean *unused) {
  ...
  default:
    PDC_LOG(
        ("%s:_handle_enter_leave - unknown event %d\n", XCLOGMSG, event->type));
  }
}
...
static void _blink_cursor(XtPointer unused, XtIntervalId *id) {
  ...
  XtAppAddTimeOut(app_context, xc_app_data.cursorBlinkRate, _blink_cursor,
                  NULL);
}
...
static void _blink_text(XtPointer unused, XtIntervalId *id) {
  ...
  if ((SP->termattrs & A_BLINK) || !blinked_off)
    XtAppAddTimeOut(app_context, xc_app_data.textBlinkRate, _blink_text, NULL);
}
...
static void XCursesButton(Widget w, XEvent *event, String *params,
  ...
  case ButtonPress:
    ...
    if ((event->xbutton.time - last_button_press_time) <
      ...
      XSendEvent(event->xbutton.display, event->xbutton.window, True, 0, event);
  ...
  case MotionNotify:
    MOUSE_LOG(("\nMotionNotify: y: %d x: %d Width: %d "
               "Height: %d\n",
               event->xbutton.y, event->xbutton.x, font_width, font_height));

    MOUSE_X_POS = (event->xbutton.x - xc_app_data.borderWidth) / font_width;
    MOUSE_Y_POS = (event->xbutton.y - xc_app_data.borderWidth) / font_height;
  ...
  case ButtonRelease:
    if (remove_release) {
      ...
      if (!handle_real_release) {
        if ((event->xbutton.time - last_button_press_time) < SP->mouse_wait &&
          ...
          if (button_no == 1 && mouse_selection &&
            ...
            if (XtOwnSelection(topLevel, XA_PRIMARY, event->xbutton.time,
  ...
  MOUSE_LOG(("Button: %d x: %d y: %d Button status: %x "
             "Mouse status: %x\n",
             button_no, MOUSE_X_POS, MOUSE_Y_POS, BUTTON_STATUS(button_no),
             Mouse_status.changes));

  MOUSE_LOG(("Send: %d Button1: %x Button2: %x Button3: %x %d %d\n", send_key,
             BUTTON_STATUS(1), BUTTON_STATUS(2), BUTTON_STATUS(3), XCursesLINES,
             XCursesCOLS));
...
static void _resize(void) {
  ...
  SP->lines = XCursesLINES =
      ((resize_window_height - (2 * xc_app_data.borderWidth)) / font_height);

  LINES = XCursesLINES - SP->linesrippedoff - SP->slklines;

  SP->cols = COLS = XCursesCOLS =
      ((resize_window_width - (2 * xc_app_data.borderWidth)) / font_width);
...
static void _get_color(void) {
  ...
  int index = tmp->pixel;
  Colormap cmap =
      DefaultColormap(XCURSESDISPLAY, DefaultScreen(XCURSESDISPLAY));
...
static void _set_selection(void) {
  ...
  if (XtOwnSelection(topLevel, XA_PRIMARY, CurrentTime, _convert_proc,
...
static void _process_curses_requests(XtPointer client_data, int *fid,
  ...
  if ((s = select(FD_SETSIZE, (FD_SET_CAST)&xc_readfds, NULL, NULL,
                  &socket_timeout)) < 0)
    _exit_process(2, SIGKILL,
  ...
  if (FD_ISSET(xc_display_sock, &xc_readfds)) {
    ...
    case CURSES_BLINK_ON:
      if (!(SP->termattrs & A_BLINK)) {
        ...
        blinked_off = FALSE;
        XtAppAddTimeOut(app_context, xc_app_data.textBlinkRate, _blink_text,
                        NULL);
      }
      break;
...
#ifdef PDC_XIM
static void _dummy_handler(Widget w, XtPointer client_data, XEvent *event,
                           Boolean *unused) {}
#endif
...
int XCursesSetupX(int argc, char *argv[]) {
  ...
  italic_font_valid =
      font_width == xc_app_data.italicFont->max_bounds.rbearing -
                        xc_app_data.italicFont->min_bounds.lbearing;

  bold_font_valid = font_width == xc_app_data.boldFont->max_bounds.rbearing -
  ...
  window_width = font_width * XCursesCOLS + 2 * xc_app_data.borderWidth;

  window_height = font_height * XCursesLINES + 2 * xc_app_data.borderWidth;
  ...
  if (xc_app_data.pixmap && xc_app_data.pixmap[0])
    XtVaSetValues(topLevel, XtNminWidth, minwidth, XtNminHeight, minheight,
                  XtNbaseWidth, xc_app_data.borderWidth * 2, XtNbaseHeight,
                  xc_app_data.borderWidth * 2, XtNiconPixmap, icon_pixmap,
                  XtNiconMask, icon_pixmap_mask, NULL);
  else
#endif
    XtVaSetValues(topLevel, XtNminWidth, minwidth, XtNminHeight, minheight,
                  XtNbaseWidth, xc_app_data.borderWidth * 2, XtNbaseHeight,
                  xc_app_data.borderWidth * 2, XtNiconPixmap, icon_bitmap,
                  NULL);

  /* Create a BOX widget in which to draw */

  if (xc_app_data.scrollbarWidth && sb_started) {
    scrollBox = XtVaCreateManagedWidget(
        program_name, scrollBoxWidgetClass, topLevel, XtNwidth,
        window_width + xc_app_data.scrollbarWidth, XtNheight,
        window_height + xc_app_data.scrollbarWidth, XtNwidthInc, font_width,
        XtNheightInc, font_height, NULL);

    drawing = XtVaCreateManagedWidget(program_name, boxWidgetClass, scrollBox,
                                      XtNwidth, window_width, XtNheight,
                                      window_height, XtNwidthInc, font_width,
                                      XtNheightInc, font_height, NULL);

    scrollVert = XtVaCreateManagedWidget(
        "scrollVert", scrollbarWidgetClass, scrollBox, XtNorientation,
    ...
    scrollHoriz = XtVaCreateManagedWidget(
        "scrollHoriz", scrollbarWidgetClass, scrollBox, XtNorientation,
    ...
    drawing = XtVaCreateManagedWidget(program_name, boxWidgetClass, topLevel,
                                      XtNwidth, window_width, XtNheight,
                                      window_height, XtNwidthInc, font_width,
                                      XtNheightInc, font_height, NULL);

    XtVaSetValues(topLevel, XtNwidthInc, font_width, XtNheightInc, font_height,
                  NULL);
  }

  /* Process any default translations */

  XtAugmentTranslations(drawing, XtParseTranslationTable(default_translations));
  ...
  SP->termattrs =
      A_COLOR | A_ITALIC | A_UNDERLINE | A_LEFT | A_RIGHT | A_REVERSE;
  ...
  PDC_LOG(("%s:shmid_Xcurscr %d shmkey_Xcurscr %d LINES %d COLS %d\n", XCLOGMSG,
           shmid_Xcurscr, shmkey_Xcurscr, LINES, COLS));
  ...
  _get_gc(&italic_gc,
          italic_font_valid ? xc_app_data.italicFont : xc_app_data.normalFont,
          COLOR_WHITE, COLOR_BLACK);

  _get_gc(&bold_gc,
          bold_font_valid ? xc_app_data.boldFont : xc_app_data.normalFont,
          COLOR_WHITE, COLOR_BLACK);

  _get_gc(&rect_cursor_gc, xc_app_data.normalFont, COLOR_CURSOR, COLOR_BLACK);

  _get_gc(&border_gc, xc_app_data.normalFont, COLOR_BORDER, COLOR_BLACK);

  XSetLineAttributes(XCURSESDISPLAY, rect_cursor_gc, 2, LineSolid, CapButt,
                     JoinMiter);
  ...
  XRecolorCursor(XCURSESDISPLAY, xc_app_data.pointer, &pointerforecolor,
                 &pointerbackcolor);
