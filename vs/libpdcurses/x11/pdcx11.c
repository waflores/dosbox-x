...
void XC_release_line_lock(int row) { *(Xcurscr + XCURSCR_FLAG_OFF + row) = 0; }
...
#ifdef PDCDEBUG
void XC_say(const char *msg) { PDC_LOG(("%s:%s", XCLOGMSG, msg)); }
#endif
...
static int _setup_curses(void) {
  ...
  if ((shmid_Xcurscr =
           shmget(shmkey_Xcurscr, SP->XcurscrSize + XCURSESSHMMIN, 0700)) < 0) {
...
void XCursesExitCursesProcess(int rc, char *msg) {
  PDC_LOG(
      ("%s:XCursesExitCursesProcess() - called: %d %s\n", XCLOGMSG, rc, msg));
