...
bool PDC_check_key(void) {
  ...
  s = select(FD_SETSIZE, (FD_SET_CAST)&xc_readfds, NULL, NULL, &socket_timeout);
  if (s < 0)
    XCursesExitCursesProcess(3, "child - exiting from "
                                "PDC_check_key select failed");

  PDC_LOG(
      ("%s:PDC_check_key() - returning %s\n", XCLOGMSG, s ? "TRUE" : "FALSE"));
...
int PDC_get_key(void) {
  ...
  if (key == KEY_MOUSE && SP->key_code) {
    if (XC_read_socket(xc_key_sock, &pdc_mouse_status, sizeof(MOUSE_STATUS)) <
        0)
      XCursesExitCursesProcess(2, "exiting from PDC_get_key");
  }
