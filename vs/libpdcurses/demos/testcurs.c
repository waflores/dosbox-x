...
COMMAND command[MAX_OPTIONS] = {
    {"Intro Test", introTest},         {"Pad Test", padTest},
#if HAVE_RESIZE
    {"Resize Test", resizeTest},
#endif
    {"Scroll Test", scrollTest},       {"Input Test", inputTest},
    {"Output Test", outputTest},       {"ACS Test", acsTest},
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    case KEY_DOWN:
      old_option = new_option;
      new_option =
          (new_option == MAX_OPTIONS - 1) ? new_option : new_option + 1;
      display_menu(old_option, new_option);
      break;
#ifdef KEY_RESIZE
    case KEY_RESIZE:
  ...
  while (1) {
    ...
    if (c == KEY_MOUSE) {
      ...
      if (MOUSE_MOVED)
        ...
        waddstr(win, "clicked: ");
      else if ((BUTTON_STATUS(button) & BUTTON_ACTION_MASK) ==
...
#define ACSNUM 25
#endif
  static const char *acs_names[] = {"ACS_ULCORNER",
                                    "ACS_URCORNER",
                                    "ACS_LLCORNER",
                                    "ACS_LRCORNER",
                                    "ACS_LTEE",
...
#if HAVE_WIDE
  cchar_t *wacs_values[] = {WACS_ULCORNER,
                            WACS_URCORNER,
                            WACS_LLCORNER,
                            WACS_LRCORNER,
                            WACS_LTEE,
  ...
  static const wchar_t russian[] = {0x0420, 0x0443, 0x0441, 0x0441, 0x043a,
                                    0x0438, 0x0439, L' ',   0x044f, 0x0437,
                                    0x044b, 0x043a, 0};

  static const wchar_t greek[] = {0x0395, 0x03bb, 0x03bb, 0x03b7, 0x03bd,
                                  0x03b9, 0x03ba, 0x03ac, 0};

  static const wchar_t georgian[] = {0x10e5, 0x10d0, 0x10e0, 0x10d7,
                                     0x10e3, 0x10da, 0x10d8, L' ',
...
void colorTest(WINDOW *win) {
  static const short colors[] = {COLOR_BLACK,  COLOR_RED,  COLOR_GREEN,
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 16; j++) {
      ...
      mvaddch(
          tmarg + i + 5, col3 + j,
          fill | (widecol ? COLOR_PAIR(i + 12) : (COLOR_PAIR(i + 4) | A_BOLD)));
    }
  }
  ...
  if (can_change_color()) {
    ...
    for (i = 0; i < maxcol; i++)
      color_content(i, &(orgcolors[i].red), &(orgcolors[i].green),
                    &(orgcolors[i].blue));
