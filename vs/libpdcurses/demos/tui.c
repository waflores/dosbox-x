...
static void idle(void) {
  ...
  tp = localtime(&t);
  sprintf(buf, " %.2d-%.2d-%.4d  %.2d:%.2d:%.2d", tp->tm_mday, tp->tm_mon + 1,
...
WINDOW *bodywin(void) { return wbody; }

void rmerror(void) { rmline(wstat, 0); }

void rmstatus(void) { rmline(wstat, 1); }
...
int getstrings(char *desc[], char *buf[], int field) {
  ...
  winput = mvwinputbox(wbody, (maxy - nlines) / 2, (maxx - ncols) / 2, nlines,
                       ncols);
