...
int PDC_resize_screen(int nlines, int ncols) {
  ...
  PDC_LOG(
      ("PDC_resize_screen() - called. Lines: %d Cols: %d\n", nlines, ncols));
...
static bool _screen_mode_equals(VIOMODEINFO *oldmode) {
  ...
  return ((current.cb == oldmode->cb) && (current.fbType == oldmode->fbType) &&
          (current.color == oldmode->color) && (current.col == oldmode->col) &&
          (current.row == oldmode->row) && (current.hres == oldmode->vres) &&
...
bool PDC_can_change_color(void) { return FALSE; }
