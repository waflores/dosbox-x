...
void PDC_set_keyboard_default(void) { KbdSetStatus(&kbdinfo, 0); }
...
static int _process_mouse_events(void) {
  MOUEVENTINFO event;
  static const USHORT button_mask[] = {6, 96, 24}, move_mask[] = {2, 32, 8},
                      press_mask[] = {4, 64, 16};
  USHORT count = 1;
  short shift_flags = 0;
  int i;
...
int PDC_mouse_set(void) {
  ...
  if (mbe && mouse_handle) {
    USHORT mask =
        ((mbe & (BUTTON1_PRESSED | BUTTON1_CLICKED | BUTTON1_MOVED)) ? 6 : 0) |

        ((mbe & (BUTTON3_PRESSED | BUTTON3_CLICKED | BUTTON3_MOVED)) ? 24 : 0) |

        ((mbe & (BUTTON2_PRESSED | BUTTON2_CLICKED | BUTTON2_MOVED)) ? 96 : 0);
