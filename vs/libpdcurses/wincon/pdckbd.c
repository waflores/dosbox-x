...
/* HACK! */
INPUT_RECORD *_pdcurses_hax_inputrecord(void) { return &save_ip; }
...
static KPTAB kptab[] = {
    ...
    {PADMINUS, SHF_PADMINUS, CTL_PADMINUS, ALT_PADMINUS,
     999},                                  /* 109 VK_SUBTRACT*/
...
static int _process_mouse_event(void) {
  ...
  if (MEV.dwEventFlags == 8) {
    pdc_mouse_status.changes = (MEV.dwButtonState & 0xFF000000)
                                   ? PDC_MOUSE_WHEEL_RIGHT
                                   : PDC_MOUSE_WHEEL_LEFT;
...
int PDC_mouse_set(void) {
  ...
  SetConsoleMode(pdc_con_in, SP->_trap_mbe ? (ENABLE_MOUSE_INPUT | 0x0088)
                                           : (pdc_quick_edit | 0x0088));
