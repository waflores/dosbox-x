...
typedef BOOL(WINAPI *SetConsoleScreenBufferInfoExFn)(
    HANDLE hConsoleOutput,
    PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx);
typedef BOOL(WINAPI *GetConsoleScreenBufferInfoExFn)(
    HANDLE hConsoleOutput,
    PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx);
...
int PDC_scr_open(int argc, char **argv) {
  ...
  if ((str = getenv("PDC_RESTORE_SCREEN")) == NULL || *str != '0') {
    ...
    pdc_con_out = CreateConsoleScreenBuffer(
  ...
  pGetConsoleScreenBufferInfoEx =
      (GetConsoleScreenBufferInfoExFn)GetProcAddress(
          h_kernel, "GetConsoleScreenBufferInfoEx");
  pSetConsoleScreenBufferInfoEx =
      (SetConsoleScreenBufferInfoExFn)GetProcAddress(
          h_kernel, "SetConsoleScreenBufferInfoEx");
...
bool PDC_can_change_color(void) { return is_nt && !pdc_conemu; }
  ...
  if (color < 16) {
    ...
    if (color_table) {
      color_table[pdc_curstoreal[color]] =
          RGB(DIVROUND(red * 255, 1000), DIVROUND(green * 255, 1000),
