...
/* Tool for Mac OS X builds to patch .dylib files so they always load relative
   ...
   but that tool succumbed to a debilitating neurotic break due to excess
   worrying over whether or not a __LINKEDIT segment is completely occupied by
   link edit data. So to continue enabling Mac OS releases, we now use this tool
   which does the one thing we need it to do: Change .dylib references in MachO
   binaries from absolute paths to @executable_path@ relative references so they
   can be bundled in the DOSBox-X app bundle without requiring the user to
...
/* replace /opt/homebrew/lib/libsomething.dylib with
 * @executable_path/libsomething.dylib */
string dylib_replace(string path) {
  ...
  if (str_startswith(
          s, "@loader_path/")) { /* often in Brew followed by ../../.. etc */
    s = fn;
    while (!strncmp(s, "../", 3))
      s += 3;
    printf("'%s' = '%s'\n", path.c_str(), s);
...
string get_macho_lcstr(union lc_str str, const uint8_t *base,
                       const uint8_t *fence) {
  string r;
...
int main(int argc, char **argv) {
  ...
  if (st.st_size >
      (512 * 1024 *
       1024)) { // If your dylibs are that large you have a bloat problem
    fprintf(stderr, "%s is too large\n", fpath.c_str());
    return 1;
  }
  src_mmap_sz = (size_t)((st.st_size + 0xFFFll) & ~0xFFFll); /* round up */

  if ((src_mmap = (const uint8_t *)mmap(NULL, src_mmap_sz, PROT_READ,
                                        MAP_SHARED, src_fd, 0)) == MAP_FAILED) {
  ...
  if (!(filetype == MH_EXECUTE || filetype == MH_DYLIB)) {
    fprintf(stderr,
            "This tool is intended for use with executables or dylib files\n");
    return 1;
  }
  ...
  while ((src_scan + 8) <= load_cmd_fence) {
    ...
    if ((src_bits == 32 && (cmd->cmdsize & 3) != 0) ||
      ...
      fprintf(stderr,
              "Load command size does not meet Apple alignment requirements\n");
      return 1;
    }
    ...
    if (cmd->cmd == LC_ID_DYLIB || cmd->cmd == LC_LOAD_DYLIB ||
      ...
      string name = get_macho_lcstr(dycmd->dylib.name, (const uint8_t *)cmd,
                                    (const uint8_t *)src_scan);
      ...
      /* construct a new entry with a possibly altered path */
      {
        ...
        if (write(tmp_fd, (void *)tmp, (size_t)(w - tmp)) != (size_t)(w - tmp))
          return 2;
      }
    } else {
      /* copy as-is */
      if (write(tmp_fd, (void *)cmd, cmd->cmdsize) != cmd->cmdsize)
        return 2;
    }
  }
  ...
  /* update size of commands */
  {
    ...
    if (pos > tgt) {
      /* If it doesn't expand much there's a good chance it just goes a bit more
         into the empty space before the first sector or section. As segments
         are page aligned there should be enough to do this so long as we're not
         expanding it too far! Note that it is necessary to scan the sections
         because some declare segments that start at file offset zero but the
         sections do not.

         TODO: It really would be a good idea to scan the segment load commands
         and the sections within and error out if the newly written dylib
         commands really do extend into valid segment data. At some point, this
         code should support arbitrary expansion by adjusting segment file
         offsets farther out if necessary. */
      uint64_t add = uint64_t(pos - tgt);

      fprintf(stderr,
              "Modification expanded load command list by %llu bytes.\n",
  ...
  if (src_scan < src_mmap_fence) {
    ...
    if (write(tmp_fd, src_scan, rem) != rem)
      return 2;
    src_scan += rem;
    assert(src_scan == src_mmap_fence);
  }
