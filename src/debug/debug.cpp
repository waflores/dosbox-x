...
/* [https://github.com/joncampbell123/dosbox-x/issues/1264] ncurses non-ASCII
 * keys are outside ASCII range (start at octal 0400 == hex 0x100) */
...
void DEBUG_DrawInput(void) { DrawInput(); }
...
static void LogEMUMachine(void) {
...

  {
    ...
    case SVGA_S3Trio:
      ...
      break;
    case SVGA_TsengET4K:
      cardName = "Tseng ET4000";
      break;
    case SVGA_TsengET3K:
      cardName = "Tseng ET3000";
      break;
    case SVGA_ParadisePVGA1A:
      cardName = "Paradise PVGA1A";
      break;
    case SVGA_ATI:
      cardName = "ATI";
      break;
...
static void LogEMUMem(void) {
  ...
  DEBUG_ShowMsg("A20 gate:                    %s",
                MEM_A20_Enabled() ? "ON" : "OFF");
  DEBUG_ShowMsg("CPU address bits:            %u", MEM_get_address_bits());
  DEBUG_ShowMsg("CPU address mask:            0x%lx",
                ((unsigned long)MEM_PageMask() << 12UL) | 0xFFFUL);
  DEBUG_ShowMsg("CPU address mask current:    0x%lx",
                ((unsigned long)MEM_PageMaskActive() << 12UL) | 0xFFFUL);
  DEBUG_ShowMsg("Memory reported size:        %lu bytes",
                (unsigned long)MEM_TotalPages() << 12UL);
...
bool XMS_GetHandleInfo(Bitu &phys_location, Bitu &size, Bitu &lockcount,
                       bool &free, Bitu handle);
...
bool IsDebuggerActive(void) { return debugging; }

bool IsDebuggerRunwatch(void) { return debug_running; }

bool IsDebuggerRunNormal(void) { return runnormal; }
...
#if C_FPU_X86
  snprintf(dest, 11, "%08.2Lf",
           reinterpret_cast<long double &>(fpu.p_regs[regIndex]));
...
static bool F80TestUpdate(int regIndex) {
  if (fpu.sw.top != oldfpu.sw.top) { /* If the top changed then all registers
                                        rotated places, thus updated. */
    return true;
  }

#if C_FPU_X86
  return !(fpu.p_regs[regIndex].m1 == oldfpu.p_regs[regIndex].m1 &&
           fpu.p_regs[regIndex].m2 == oldfpu.p_regs[regIndex].m2 &&
           fpu.p_regs[regIndex].m3 == oldfpu.p_regs[regIndex].m3);
#elif defined(HAS_LONG_DOUBLE)
  return fpu.regs_80[regIndex].v !=
         oldfpu.regs_80[regIndex]
             .v; /* I'm certain that strict float equality can be used here. */
#else
...
uint64_t GetAddress(uint16_t seg, uint32_t offset) {
  ...
  if (seg == SegValue(cs))
    return SegPhys(cs) + (uint64_t)offset;
...
bool GetDescriptorInfo(char *selname, char *out1, char *out2) {
  ...
  if (cpu.gdt.GetDescriptor(sel, desc)) {
    switch (desc.Type()) {
    case DESC_TASK_GATE:
      sprintf(out1, "%s: s:%08lX type:%02X p", selname,
              (unsigned long)desc.GetSelector(), (int)desc.saved.gate.type);
      sprintf(out2, "    TaskGate   dpl : %01X %1X", desc.saved.gate.dpl,
              desc.saved.gate.p);
      return true;
    case DESC_LDT:
    case DESC_286_TSS_A:
    ...
    case DESC_386_TSS_A:
    case DESC_386_TSS_B:
      sprintf(out1, "%s: b:%08lX type:%02X pag", selname,
              (unsigned long)desc.GetBase(), (int)desc.saved.seg.type);
      sprintf(out2, "    l:%08lX dpl : %01X %1X%1X%1X",
              (unsigned long)desc.GetLimit(), desc.saved.seg.dpl,
              desc.saved.seg.p, desc.saved.seg.avl, desc.saved.seg.g);
      return true;
    case DESC_286_CALL_GATE:
    case DESC_386_CALL_GATE:
      sprintf(out1, "%s: s:%08lX type:%02X p params: %02X", selname,
              (unsigned long)desc.GetSelector(), desc.saved.gate.type,
              desc.saved.gate.paramcount);
      sprintf(out2, "    o:%08lX dpl : %01X %1X",
              (unsigned long)desc.GetOffset(), desc.saved.gate.dpl,
              desc.saved.gate.p);
      return true;
    ...
    sprintf(out1, "%s: b:%08lX type:%02X parbg", selname,
            (unsigned long)desc.GetBase(), desc.saved.seg.type);
    sprintf(out2, "    l:%08lX dpl : %01X %1X%1X%1X%1X%1X",
            (unsigned long)desc.GetLimit(), desc.saved.seg.dpl,
            desc.saved.seg.p, desc.saved.seg.avl, desc.saved.seg.r,
            desc.saved.seg.big, desc.saved.seg.g);
    return true;
  } else {
    strcpy(out1, "                                  ");
    strcpy(out2, "                                  ");
...
public:
  ...
  static CBreakpoint *AddIntBreakpoint(uint8_t intNum, uint16_t ah, uint16_t al,
                                       bool once);
  ...
  static bool CheckIntBreakpoint(PhysPt adr, uint8_t intNr, uint16_t ahValue,
                                 uint16_t alValue);
...
CBreakpoint::CBreakpoint(void)
    : type(BKPNT_UNKNOWN), location(0),
#if !C_HEAVY_DEBUG
      oldData(0xCC),
#endif
      segment(0), offset(0), intNr(0), ahValue(0), alValue(0), active(false),
      once(false) {
}
  ...
  if (GetType() == BKPNT_PHYSICAL) {
    if (_active) {
      ...
      if (mem_readb(location) == 0xCC) {
        if (oldData == 0xCC)
          DEBUG_ShowMsg(
              "DEBUG: Internal error while deactivating breakpoint.\n");

        // Check if we are the last active breakpoint at this location
        bool otherActive =
            (FindOtherActiveBreakpoint(location, this) != nullptr);
  ...
  for (auto i = BPoints.begin(); i != BPoints.end(); ++i) {
    ...
    // Memory breakpoint support
      ...
      if ((bp->GetType() == BKPNT_MEMORY) ||
          ...
          (bp->GetType() == BKPNT_MEMORY_FREEZE)) {
        ...
        uint8_t value = 0;
        if (mem_readb_checked((PhysPt)address, &value))
          return false;
        if (bp->GetValue() != value) {
          ...
          DEBUG_ShowMsg(
              ...
              bp->GetSegment(), bp->GetOffset(), bp->GetValue(), value);
          bp->SetValue(value);
          return true;
        }
      }
...
bool CBreakpoint::CheckIntBreakpoint(PhysPt adr, uint8_t intNr,
...
CBreakpoint *CBreakpoint::FindPhysBreakpoint(uint16_t seg, uint32_t off,
                                             bool once) {
  if (BPoints.empty())
    return nullptr;
#if !C_HEAVY_DEBUG
...
CBreakpoint *CBreakpoint::FindOtherActiveBreakpoint(PhysPt adr,
                                                    CBreakpoint *skip) {
  std::list<CBreakpoint *>::iterator i;
  for (i = BPoints.begin(); i != BPoints.end(); ++i) {
    CBreakpoint *bp = (*i);
...
static bool StepOver() {
  ...
  if (strstr(dline, "call") || strstr(dline, "int") || strstr(dline, "loop") ||
      strstr(dline, "rep")) {
    // Don't add a temporary breakpoint if there's already one here
    if (!CBreakpoint::FindPhysBreakpoint(SegValue(cs),
                                         (uint32_t)(reg_eip + size), true))
      CBreakpoint::AddBreakpoint(SegValue(cs), (uint32_t)(reg_eip + size),
                                 true);
    debugging = false;
...
static void DrawData(void) {
  ...
  if ((paging.enabled || cpu.pmode) &&
      dbg.data_view != DBGBlock::DATV_PHYSICAL) {
...
static void DrawRegisters(void) {
  ...
  char x87buf[12] = {};
  SetColor(F80TestUpdate(STV(0)));
  mvwprintw(dbg.win_reg, 4, 4, "%s", F80ToString(STV(0), x87buf));
  SetColor(F80TestUpdate(STV(4)));
  mvwprintw(dbg.win_reg, 5, 4, "%s", F80ToString(STV(4), x87buf));

  SetColor(F80TestUpdate(STV(1)));
  mvwprintw(dbg.win_reg, 4, 18, "%s", F80ToString(STV(1), x87buf));
  SetColor(F80TestUpdate(STV(5)));
  mvwprintw(dbg.win_reg, 5, 18, "%s", F80ToString(STV(5), x87buf));

  SetColor(F80TestUpdate(STV(2)));
  mvwprintw(dbg.win_reg, 4, 32, "%s", F80ToString(STV(2), x87buf));
  SetColor(F80TestUpdate(STV(6)));
  mvwprintw(dbg.win_reg, 5, 32, "%s", F80ToString(STV(6), x87buf));

  SetColor(F80TestUpdate(STV(3)));
  mvwprintw(dbg.win_reg, 4, 46, "%s", F80ToString(STV(3), x87buf));
  SetColor(F80TestUpdate(STV(7)));
  mvwprintw(dbg.win_reg, 5, 46, "%s", F80ToString(STV(7), x87buf));
  ...
  if (CPU_IsHLTed())
    mvwprintw(dbg.win_reg, 3, 73, "HLT ");
  else
    mvwprintw(dbg.win_reg, 3, 73, "RUN ");
...
static void DrawInput(void) {
  ...
  if (!debugging) {
    ...
    mvwprintw(dbg.win_inp, 0, 0, "%s",
              "^ Paged content: Hit ENTER to continue, Q to exit paging");
    wclrtoeol(dbg.win_inp);
  } else {
    // TODO long lines
  ...
  for (int i = 0; i < h; i++) {
    ...
    if (start != mem_no_address) {
      ...
      if (cpu.gdt.GetDescriptor(codeViewData.useCS, desc))
        drawsize = size =
            DasmI386(dline, (PhysPt)start, disEIP, desc.saved.seg.big);
      else
    ...
    if (i == 0)
      codeViewData.firstInstSize = (uint16_t)size;
    if (i == 4)
...
void DEBUG_CheckCSIP() { SetCodeWinStart(); }
...
uint32_t GetHexValue(char *const str, char *&hex, bool *parsed, int exprge) {
  ...
  if (*hex == '(') {
    ...
    // In this code base, scan the string for alphanumeric characters, store the
    // string, THEN process for identifiers. This is to make sure "DFE" is not
    // confused with the DF flag, for example.
    if (*hex == '\"') {
      quoted = true;
    ...
    // The user can enclose a value in double quotations to enter hex values
    // that would collide with a flag name (AC, AF, CF, and DF).
    if (!quoted) {
      if (something == "EFLAGS") {
        regval = (uint32_t)reg_flags;
      } else if (something == "FLAGS") {
        regval = (uint32_t)reg_flags;
      } else if (something == "IOPL") {
        regval = (reg_flags & FLAG_IOPL) >> 12u;
      } else if (something == "CR0") {
        regval = (uint32_t)cpu.cr0;
      } else if (something == "CR2") {
        regval = (uint32_t)paging.cr2;
      } else if (something == "CR3") {
        regval = (uint32_t)paging.cr3;
      } else if (something == "CR4") {
        regval = (uint32_t)cpu.cr4;
      } else if (something == "SYSENTER") {
        regval = (uint32_t)cpu_sep_eip;
      } else if (something == "EAX") {
        regval = reg_eax;
      } else if (something == "EBX") {
        regval = reg_ebx;
      } else if (something == "ECX") {
        regval = reg_ecx;
      } else if (something == "EDX") {
        regval = reg_edx;
      } else if (something == "ESI") {
        regval = reg_esi;
      } else if (something == "EDI") {
        regval = reg_edi;
      } else if (something == "EBP") {
        regval = reg_ebp;
      } else if (something == "ESP") {
        regval = reg_esp;
      } else if (something == "EIP") {
        regval = reg_eip;
      } else if (something == "AX") {
        regval = reg_ax;
      } else if (something == "BX") {
        regval = reg_bx;
      } else if (something == "CX") {
        regval = reg_cx;
      } else if (something == "DX") {
        regval = reg_dx;
      } else if (something == "SI") {
        regval = reg_si;
      } else if (something == "DI") {
        regval = reg_di;
      } else if (something == "BP") {
        regval = reg_bp;
      } else if (something == "SP") {
        regval = reg_sp;
      } else if (something == "IP") {
        regval = reg_ip;
      } else if (something == "AL") {
        regval = reg_al;
      } else if (something == "BL") {
        regval = reg_bl;
      } else if (something == "CL") {
        regval = reg_cl;
      } else if (something == "DL") {
        regval = reg_dl;
      } else if (something == "AH") {
        regval = reg_ah;
      } else if (something == "BH") {
        regval = reg_bh;
      } else if (something == "CH") {
        regval = reg_ch;
      } else if (something == "DH") {
        regval = reg_dh;
      } else if (something == "CS") {
        regval = SegValue(cs);
      } else if (something == "DS") {
        regval = SegValue(ds);
      } else if (something == "ES") {
        regval = SegValue(es);
      } else if (something == "FS") {
        regval = SegValue(fs);
      } else if (something == "GS") {
        regval = SegValue(gs);
      } else if (something == "SS") {
        regval = SegValue(ss);
      } else if (something == "AC") {
        regval = GETFLAG(AC);
      } else if (something == "AF") {
        regval = GETFLAG(AF);
      } else if (something == "CF") {
        regval = GETFLAG(CF);
      } else if (something == "DF") {
        regval = GETFLAG(DF);
      } else if (something == "ID") {
        regval = GETFLAG(ID);
      } else if (something == "IF") {
        regval = GETFLAG(IF);
      } else if (something == "NT") {
        regval = GETFLAG(NT);
      } else if (something == "OF") {
        regval = GETFLAG(OF);
      } else if (something == "PF") {
        regval = GETFLAG(PF);
      } else if (something == "SF") {
        regval = GETFLAG(SF);
      } else if (something == "TF") {
        regval = GETFLAG(TF);
      } else if (something == "VM") {
        regval = GETFLAG(VM);
      } else if (something == "ZF") {
        regval = GETFLAG(ZF);
      }

      else if (something == "DTASEG") {
        regval = (!dos_kernel_disabled) ? (dos.dta() >> 16u) : 0;
      } else if (something == "DTAOFF") {
        regval = (!dos_kernel_disabled) ? (dos.dta() & 0xFFFFu) : 0;
      } else if (something == "PSPSEG") {
        regval = (!dos_kernel_disabled) ? dos.psp() : 0;
      } else if (hexnumber) {
        regval = (uint32_t)strtoul(something.c_str(), NULL, 16 /*hexadecimal*/);
      } else {
        if (parsed)
          *parsed = 0;
        return 0;
  ...
  while (*hex != 0) {
    ...
    if (*hex == '+') {
      if (exprge >= EXPR_ADDSUB)
        break; /* if order of operations says we're handling something higher
                  precedence, stop now */
      hex++;
      ...
      if (exprge >= EXPR_MULDIV)
        break; /* if order of operations says we're handling something higher
                  precedence, stop now */
      hex++;
      uint32_t r = GetHexValue(hex, hex, parsed,
                               EXPR_MULDIV); /* whoah now, check for zero! */
      if (r != 0)
        regval /= r;
      else
        regval = 0;
...
bool ChangeRegister(char *const str) {
  ...
  while (*hex) {
    ...
    if (strstr(hex, "EFLAGS") == hex) {
      hex += 6;
      CPU_SetFlags(GetHexValue(hex, hex), FMASK_ALL);
    } else if (strstr(hex, "FLAGS") == hex) {
      hex += 5;
      CPU_SetFlags(GetHexValue(hex, hex), FMASK_ALL & 0xFFFFu);
    } else
      ...
      if (strncmp(hex, "EAX", 3) == 0) {
        ...
        reg_ip = (uint16_t)GetHexValue(hex, hex);
          ...
          if (strncmp(hex, "AL", 2) == 0) {
        hex += 2;
        reg_al = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "BL", 2) == 0) {
        hex += 2;
        reg_bl = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "CL", 2) == 0) {
        hex += 2;
        reg_cl = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "DL", 2) == 0) {
        hex += 2;
        reg_dl = (uint8_t)GetHexValue(hex, hex);
      } else

          if (strncmp(hex, "AH", 2) == 0) {
        hex += 2;
        reg_ah = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "BH", 2) == 0) {
        hex += 2;
        reg_bh = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "CH", 2) == 0) {
        hex += 2;
        reg_ch = (uint8_t)GetHexValue(hex, hex);
      } else if (strncmp(hex, "DH", 2) == 0) {
        hex += 2;
        reg_dh = (uint8_t)GetHexValue(hex, hex);
      } else

          if (strncmp(hex, "CS", 2) == 0) {
        hex += 2;
        ...
        SegSet16(ss, (uint16_t)GetHexValue(hex, hex));
          ...
          if (strncmp(hex, "AC", 2) == 0) {
        hex += 2;
        SETFLAGBIT(AC, GetHexValue(hex, hex));
      ...
      } else if (strncmp(hex, "ID", 2) == 0) {
        hex += 2;
        SETFLAGBIT(ID, GetHexValue(hex, hex));
      } else if (strncmp(hex, "IF", 2) == 0) {
        hex += 2;
        SETFLAGBIT(IF, GetHexValue(hex, hex));
      } else
        //             "NT"
        if (strncmp(hex, "OF", 2) == 0) {
          hex += 2;
        ...
        } else if (strncmp(hex, "TF", 2) == 0) {
          hex += 2;
          SETFLAGBIT(TF, GetHexValue(hex, hex));
        } else
          //             "VM"
          if (strncmp(hex, "ZF", 2) == 0) {
            hex += 2;
...
void DEBUG_PrintMMX(int which, char format) {
  ...
  w += sprintf(w, "mm%d(%c): H->L ", which,
               format); /* let the user know we're printing from little endian
                           order highest to lowest */
  if (format == 'B') {
    w += sprintf(w, "%02x|%02x|%02x|%02x|%02x|%02x|%02x|%02x", mmx.ub.b7,
                 mmx.ub.b6, mmx.ub.b5, mmx.ub.b4, mmx.ub.b3, mmx.ub.b2,
                 mmx.ub.b1, mmx.ub.b0);
  } else if (format == 'W') {
    w += sprintf(w, "%04x|%04x|%04x|%04x", mmx.uw.w3, mmx.uw.w2, mmx.uw.w1,
                 mmx.uw.w0);
  } else if (format == 'D') {
    w += sprintf(w, "%08lx|%08lx", (unsigned long)mmx.ud.d1,
                 (unsigned long)mmx.ud.d0);
  } else if (format == 'S') {
    w += sprintf(w, "%.20f|%.20f", mmx.f32.f1.v, mmx.f32.f0.v);
  } else { /* 'Q' */
    w += sprintf(w, "%08lx%08lx", (unsigned long)mmx.ud.d1,
                 (unsigned long)
                     mmx.ud.d0); /* little endian combine to print 64-bit hex */
  }
...
void DEBUG_PrintSSE(int which, char format) {
  ...
  if (format == 'B') {
    ...
    for (int i = 3; i >= 0;
         i--) { /* rather than fight with printf() differences between MSVC++
                   and glibc just print 32-bit values anyway */
      w += sprintf(w, "%08lx", (unsigned long)xmm.u32[(unsigned int)i]);
      if ((i & 1u) == 0 && i != 0)
        *w++ = '|';
    }
...
bool ParseCommand(char *str) {
  ...
  copy_str += '\0'; /* paranoid */
  char *found =
      &(copy_str[0]); /* cannot use std::string c_str(), that is const char* */
  ...
  if (command == "SHOWWIN") { // SHOW WINDOW <name>
    ...
    if (win >= 0) {
      ...
      LOG_MSG("No such window '%s'. Windows are: %s", found,
              dbg.windowlist_by_name().c_str());
      return false;
    }
  }
  ...
  if (command == "EV") { // echo value (for viewing contents through GetHexValue
    ...
    while (*found) {
      uint32_t value = GetHexValue(found, found, &parsed);
      SkipSpace(found);
      if (!parsed) {
  ...
  if (command ==
      "SMV") { // Set memory with following values (virtual linear address)
    uint32_t ofs = GetHexValue(found, found);
    SkipSpace(found);
    uint16_t count = 0;
    bool parsed;
  ...
  if (command == "BPINT") { // Add Interrupt Breakpoint
    ...
    if ((valAH == 0x00) && (*found == '*' || all)) {
      ...
      if ((valAL == 0x00) && (*found == '*' || all)) {
        ...
        DEBUG_ShowMsg(
            "DEBUG: Set interrupt breakpoint at INT %02X AH=%02X AL=%02X\n",
            intNr, valAH, valAL);
      }
    }
    return true;
  ...
  if (command == "PIC") { // interrupt controller state/controls
    ...
    if (command == "MASKIRQ") {
      ...
      DEBUG_PICMask(irq, false);
               ...
               "ACKIRQ") { /* debugging: manually acknowledge an IRQ where the
                              DOS program failed to do so */
      std::string what;
      stream >> what;
      int irq = atoi(what.c_str());
      DEBUG_PICAck(irq);
  ...
  if (command == "MMX") { // MMX [=B|=W|=D|=Q|=S] [regindex] [command ...]
    ...
    int which = (*found != 0 && isxdigit(*found))
                    ? (int)strtol(found, &found, 16)
                    : -1; /* accept index as hexadecimal to be consistent with
    ...
    if (subcommand.empty()) {
      ...
      return true;
    } else if (subcommand == "SET") { /* remember this function capitalizes the
                                         entire string */
      if (which < 0 || which > 7)
        return false; // regindex is REQUIRED here
      // MMX [=B|=W|=D|=Q|=S] <regindex> SET [val,val,...]
      //
      // if you don't want to change a particular part of the MMX register, set
      // val to nothing or "" i.e. to set only the low 16 bits use =W 0 ,,,val
      ...
      while (*found != 0 && pi < param_max) {
        if (*found == ',') {
          ...
          found += 2;
          paramexist[pi++] = false; // none specified, ok.
        } else if (found[0] == 'M' && found[1] == 'M' &&
                   isxdigit(found[2])) { // i.e. "MM3", allow using other MMX
                                         // registers as a value
          found += 2;
          ...
          // FIXME: GetHexValue has a 32-bit datatype limit and therefore is
          // unsuitable for 64-bit constants
          bool parsed = false;
      ...
      // FIXME: This could be simple for() loops without copy-pasta if the
      // MMX_reg struct had arrays
      if (format == 'B') {
        if (paramexist[0])
          reg_mmx[which]->ub.b7 = param[0];
        if (paramexist[1])
          reg_mmx[which]->ub.b6 = param[1];
        if (paramexist[2])
          reg_mmx[which]->ub.b5 = param[2];
        if (paramexist[3])
          reg_mmx[which]->ub.b4 = param[3];
        if (paramexist[4])
          reg_mmx[which]->ub.b3 = param[4];
        if (paramexist[5])
          reg_mmx[which]->ub.b2 = param[5];
        if (paramexist[6])
          reg_mmx[which]->ub.b1 = param[6];
        if (paramexist[7])
          reg_mmx[which]->ub.b0 = param[7];
      } else if (format == 'W') {
        if (paramexist[0])
          reg_mmx[which]->uw.w3 = param[0];
        if (paramexist[1])
          reg_mmx[which]->uw.w2 = param[1];
        if (paramexist[2])
          reg_mmx[which]->uw.w1 = param[2];
        if (paramexist[3])
          reg_mmx[which]->uw.w0 = param[3];
      } else if (format == 'D') {
        if (paramexist[0])
        ...
        if (paramexist[0]) {
          if (paramfset[0])
            *((float *)(&param[0])) =
                (float)paramf[0]; /* which becomes u32[] */
          reg_mmx[which]->ud.d1 = param[0];
        }
        if (paramexist[1]) {
          if (paramfset[1])
            *((float *)(&param[1])) =
                (float)paramf[1]; /* which becomes u32[] */
          reg_mmx[which]->ud.d0 = param[1];
        }
      } else { // 'Q'
        if (paramexist[0])
          reg_mmx[which]->q = param[0];
      }
  ...
  // NTS: S = single precision float   F = double precision float   X = 128-bit
  ...
  if (command == "SSE") { // SSE [=B|=W|=D|=Q|=X|=S|=F] [regindex] [command ...]
    ...
    if (subcommand.empty()) {
      if (format == 'a')
        format = 'S'; // =S is default (TODO: Wouldn't it be nice if XMM regs
                      // remembered the last datatype used with them so we can
                      // use that?)
      if (which >= 0 && which <= 7) {
      ...
      while (*found != 0 && pi < param_max) {
        if (*found == ',') {
          ...
          found += 2;
          paramexist[pi++] = false; // none specified, ok.
        } else if (found[0] == 'X' && found[1] == 'M' && found[2] == 'M' &&
                   isxdigit(found[3])) { // i.e. "XMM3", allow using other XMM
                                         // registers as a value
          found += 3;
          int idx = strtol(found, &found, 16);
          if (idx >= 0 && idx <= 7) {
      ...
      if (format == 'B') {
        for (unsigned int i = 0; i < 16; i++) {
          if (paramexist[i])
            fpu.xmmreg[which].u8[15 - i] = param[i].u8[0];
        }
      } else if (format == 'W') {
        for (unsigned int i = 0; i < 8; i++) {
          if (paramexist[i])
            fpu.xmmreg[which].u16[7 - i] = param[i].u16[0];
        }
      } else if (format == 'S') {
        for (unsigned int i = 0; i < 4; i++) {
          if (paramexist[i]) {
            if (paramfset[i])
              param[i].f32[0].v = (float)paramf[i]; /* which becomes u32[] */
        ...
        for (unsigned int i = 0; i < 2; i++) {
          if (paramexist[i]) {
            if (paramfset[i])
              param[i].f64[0].v = paramf[i]; /* which becomes u64[] */
  ...
  if (command == "VGA") {
    ...
    if (command == "DS") {
      ...
      /* Rule: If the file extension is .BIN, write the entire contents of
       * bitplane 2 where the font resides (64KB). If the file extension is
       * .BMP, write only the visible font characters in a neat 8x8 (256)
       * matrix. If you run this when EGA/VGA graphics are active, you will get
       * "jibberish" based on the graphics on screen, and that is your fault.
      ...
      if (IS_PC98_ARCH) {
        ...
        if (!strcasecmp(fext, ".bin")) {
          ...
          LOG_MSG(
              "Unknown file extension '%s' in '%s'. Please use .BIN or .BMP",
              fext, filename.c_str());
          return false;
        }
      }
    } else if (command == "CRTC") {
      DEBUG_ShowMsg("VGA CRTC info: index=%02xh readonly=%u", vga.crtc.index,
                    vga.crtc.read_only ? 1 : 0);
      DEBUG_ShowMsg(
          "htotal=%02xh hdend=%02xh strhb=%02xh endhb=%02xh strrt=%02xh "
          "endrt=%02xh",
      ...
      DEBUG_ShowMsg("vtotal=%02xh overflow=%02xh prerwscn=%02xh maxscnl=%02xh "
                    "offset=%02xh",
                    vga.crtc.vertical_total, vga.crtc.overflow,
                    vga.crtc.preset_row_scan, vga.crtc.maximum_scan_line,
                    vga.crtc.offset);
      DEBUG_ShowMsg(
          "curs-st=%02xh curs-en=%02xh start-addr=%02x%02xh curs-loc=%02x%02xh",
      ...
      DEBUG_ShowMsg("strvrt=%02xh endvrt=%02xh vdend=%02xh underline=%02xh "
                    "modectrl=%02xh",
                    vga.crtc.vertical_retrace_start,
                    vga.crtc.vertical_retrace_end,
      ...
      DEBUG_ShowMsg("strvhb=%02xh endvhb=%02xh linecomp=%02xh",
                    vga.crtc.start_vertical_blanking,
                    vga.crtc.end_vertical_blanking, vga.crtc.line_compare);
    } else if (command == "GC") {
      DEBUG_ShowMsg("VGA Graphics controller info: index=%02xh setreset=%02xh",
                    vga.gfx.index, vga.gfx.set_reset);
      DEBUG_ShowMsg("enablesetreset=%02xh color-comp=%02xh data-rotate=%02xh "
                    "rdmapsel=%02xh",
                    vga.gfx.enable_set_reset, vga.gfx.color_compare,
                    vga.gfx.data_rotate, vga.gfx.read_map_select);
      DEBUG_ShowMsg("mode=%02xh misc=%02xh color-dont-care=%02xh bitmask=%02xh",
                    vga.gfx.mode, vga.gfx.miscellaneous,
                    vga.gfx.color_dont_care, vga.gfx.bit_mask);
    } else if (command == "DAC") {
      ...
      DEBUG_ShowMsg(
          "state=%02xh write_index=%02xh read_index=%02xh first_changed=%u",
          vga.dac.state, vga.dac.write_index, vga.dac.read_index,
          (unsigned int)vga.dac.first_changed);
      DEBUG_ShowMsg("hidac_counter=%u reg02=%02xh", vga.dac.hidac_counter,
                    vga.dac.reg02);
      ...
      for (unsigned int i = 0; i < 256; i += 8) {
        ...
        for (unsigned int c = 0; c < 8; c++) {
          sprintf(tmp, "%02x%02x%02x%c", vga.dac.rgb[i + c].red,
                  vga.dac.rgb[i + c].green, vga.dac.rgb[i + c].blue,
                  c == 3 ? '-' : ' ');
          cpptmp += tmp;
        }
      ...
      DEBUG_ShowMsg("reset=%02xh clockmode=%02xh map_mask=%02xh "
                    "charmapsel=%02xh memmode=%02xh",
      ...
      DEBUG_ShowMsg("VGA Attribute Controller info: attrindexmode=%u",
                    vga.internal.attrindex ? 1 : 0);
      DEBUG_ShowMsg(
          "mode_control=%02xh horz-pel-pan=%02xh overscan-color=%02xh",
      ...
      DEBUG_ShowMsg("color-plane-en=%02xh color-select=%02xh index=%02xh",
                    vga.attr.color_plane_enable, vga.attr.color_select,
                    vga.attr.index);
      DEBUG_ShowMsg(
          "disabled-by-index=%u disabled-by-idx1-bit5=%u index-written=%u",
          (vga.attr.disabled & 1) ? 1 : 0, (vga.attr.disabled & 2) ? 1 : 0,
          vga.internal.attrindex);

      cpptmp = " ";
      for (unsigned int i = 0; i < 16; i++) {
        sprintf(tmp, "%02x%c", vga.attr.palette[i],
                ((i & 3) == 3 && i != 15) ? '-' : ' ');
        cpptmp += tmp;
      }
      ...
      DEBUG_ShowMsg(
          "VGA draw info: resizing=%u width=%lu height=%lu blocks=%lu",
          vga.draw.resizing ? 1 : 0, (unsigned long)vga.draw.width,
          (unsigned long)vga.draw.height, (unsigned long)vga.draw.blocks);
      DEBUG_ShowMsg(
          "address=%lxh panning=%lu bytes-skip=%lu lin-mask=%lx pln-mask=%lx",
          (unsigned long)vga.draw.address, (unsigned long)vga.draw.panning,
          (unsigned long)vga.draw.bytes_skip,
          (unsigned long)vga.draw.linear_mask,
          (unsigned long)vga.draw.planar_mask);
      DEBUG_ShowMsg(
          "addr-add=%lxh line-length=%lu addrline-total=%lu addrline=%lu",
          (unsigned long)vga.draw.address_add,
          (unsigned long)vga.draw.line_length,
          (unsigned long)vga.draw.address_line_total,
          (unsigned long)vga.draw.address_line);
      DEBUG_ShowMsg(
          "line-total=%lu vblank-skip=%lu lines-done=%lu split-line=%lu",
          (unsigned long)vga.draw.lines_total,
          (unsigned long)vga.draw.vblank_skip,
          (unsigned long)vga.draw.lines_done,
          (unsigned long)vga.draw.split_line);
      DEBUG_ShowMsg(
          "byte-pan-shft=%lu render-stop=%lu render-max=%lu scrn-ratio=%.3f",
          (unsigned long)vga.draw.byte_panning_shift,
          (unsigned long)vga.draw.render_step,
          (unsigned long)vga.draw.render_max, vga.draw.screen_ratio);
      DEBUG_ShowMsg(
          "blinking=%lu blink=%u char9dot=%u has-split=%u vret-trig=%u",
      ...
      DEBUG_ShowMsg(
          "bpp=%lu curs-addr=%lxh curs-s=%u curs-e=%u curs-c=%u curs-d=%u "
          "curs-en=%u",
          (unsigned long)vga.draw.bpp, (unsigned long)vga.draw.cursor.address,
          vga.draw.cursor.sline, vga.draw.cursor.eline, vga.draw.cursor.count,
          vga.draw.cursor.delay, vga.draw.cursor.enabled);
      // TODO: Remove vga.draw.refresh, appears to be unused
    } else if (command == "MODE") {
      DEBUG_ShowMsg("VGA mode info:");
      DEBUG_ShowMsg(
          "mode=%s vref=%.3fHz href=%.3fHz chrclk=%.3fHz dotclk=%.3fHz",
          mode_texts[vga.mode], 1000.0 / vga.draw.delay.vtotal,
          1000.0 / vga.draw.delay.htotal, vga.draw.clock, vga.draw.oscclock);
      DEBUG_ShowMsg("disp-start=%lxh real-start=%lxh retrace=%u scanlen=%lu "
                    "cursor-start=%lxh",
      ...
      DEBUG_ShowMsg("line-compare=%lu chained=%u compat-chain4=%u pel-pan=%u "
                    "hline-skip=%u",
                    (unsigned long)vga.config.line_compare, vga.config.chained,
                    (unsigned int)vga.config.compatible_chain4,
                    (unsigned int)vga.config.pel_panning,
                    (unsigned int)vga.config.hlines_skip);
      DEBUG_ShowMsg(
          "byte-skip=%u addr-shift=%u rd-mode=%u wr-mode=%u rdmap-sel=%u",
      ...
      DEBUG_ShowMsg(
          "col-dont-care=%u color-compare=%u data-rotate=%u raster-op=%02xh",
      ...
      DEBUG_ShowMsg(
          "fbmsk=%x fmmsk=%x fnmmsk=%x fsr=%x fnesr=%x fesr=%x feasr=%x",
          vga.config.full_bit_mask, vga.config.full_map_mask,
          vga.config.full_not_map_mask, vga.config.full_set_reset,
          ...
          vga.config.full_enable_and_set_reset);
      {
        ...
        if (vga.complexity.flags & VGACMPLX_MAP_MASK)
          s += "mapmask|";
        if (vga.complexity.flags & VGACMPLX_NON_EXTENDED)
          s += "nonextend|";
        if (vga.complexity.flags & VGACMPLX_ODDEVEN)
          s += "oddeven|";
        if (vga.complexity.flags & VGACMPLX_BITMASK)
          s += "bitmask|";
        if (vga.complexity.flags & VGACMPLX_COLORDONTCARE)
          s += "clrdontcare|";
        if (vga.complexity.flags & VGACMPLX_WRITEMODE)
          s += "wrmode|";
        if (vga.complexity.flags & VGACMPLX_READMODE)
          s += "rdmode|";
        if (vga.complexity.flags & VGACMPLX_ROPROT)
          s += "roprot|";
        if (vga.complexity.flags & VGACMPLX_SETRESET)
          s += "setreset|";
        DEBUG_ShowMsg("complexity-flags=0x%lx=%s",
                      (unsigned long)vga.complexity.flags, s.c_str());
      }
    } else {
      return false;
    }
  ...
  if (command == "PC98") {
    ...
    if (!IS_PC98_ARCH) {
      DEBUG_ShowMsg(
          "PC-98 debugger commands not available except in PC-98 mode");
      return false;
    }
    ...
    if (command == "MODE") {
      ...
      if (egc_enable_enable) /* Port 6Ah, 0x06/0x07 to enable EGC enable
                                commands 0x04/0x05 */
        cpptmp += "EGC-ENABL ";
      if (pc98_gdc_vramop & (1 << VOPBIT_EGC)) /* Port 6Ah, 0x04/0x05 */
        cpptmp += "EGC ";
      if (pc98_gdc_vramop &
          (2 << VOPBIT_GRCG)) /* Port 7Ch, bits [7:6]. bit 7 (2 << VOPBIT_GRCG)
                                 is the enable. */
        cpptmp += "GRCG ";
      ...
      cpptmp.clear();
      DEBUG_ShowMsg("PC-98 page display: cpu=%u display-pending=%u "
                    "display-active=%u pitch=%u",
                    (pc98_gdc_vramop & (1 << VOPBIT_ACCESS)) ? 1 : 0,
                    GDC_display_plane_pending, GDC_display_plane,
                    gdc.display_pitch);
      ...
      DEBUG_ShowMsg("PC-98 status: gdc5mhz=%u vsync-int-trig=%u rowheight=%u "
                    ...
                    (unsigned int)vga.draw.lines_done);
      DEBUG_ShowMsg("  cur-row-line=%u cur-scan=0x%x cur-partition=%u/%u "
                    "part-remline=%u adw=%u",
                    gdc.row_line, gdc.scan_address, gdc.display_partition,
                    gdc.display_partition_mask + 1,
                    gdc.display_partition_rem_lines,
                    gdc.active_display_words_per_line);
      ...
      cpptmp.clear();
      {
        char tmp[16];
        for (unsigned int i = 0; i < 16; i++) {
          sprintf(tmp, "%02x%c", gdc.param_ram[i],
                  ((i & 3) == 3 && i != 15) ? '-' : ' ');
          cpptmp += tmp;
        }
      }
      DEBUG_ShowMsg("PC-98 GDC PRAM: wptr=%u %s", gdc.param_ram_wptr,
                    cpptmp.c_str());

      /*---------------------*/
      cpptmp.clear();
      DEBUG_ShowMsg(
          "PC-98 GRCG: active=%u mode=%s tilecount=%u modereg(7Ch)=%02xh",
          (pc98_gdc_vramop & (2 << VOPBIT_GRCG)) ? 1 : 0,
          (pc98_gdc_vramop & (1 << VOPBIT_GRCG)) ? "Read/Modify/Write"
                                                 : "TCR/TDW",
          pc98_gdc_tile_counter, pc98_gdc_modereg);

      /*---------------------*/
      cpptmp.clear();
      DEBUG_ShowMsg(
          "PC-98 GRCG tiles: [%02x %02x] [%02x %02x] [%02x %02x] [%02x %02x]",
          pc98_gdc_tiles[0].b[0], pc98_gdc_tiles[0].b[1],
          pc98_gdc_tiles[1].b[0], pc98_gdc_tiles[1].b[1],
          pc98_gdc_tiles[2].b[0], pc98_gdc_tiles[2].b[1],
          pc98_gdc_tiles[3].b[0], pc98_gdc_tiles[3].b[1]);
      ...
      cpptmp.clear();
      DEBUG_ShowMsg("PC-98 display/cursor: pitch=%u curs-en=%u curs-blink=%u "
                    "curs-blinkrate=%u",
                    gdc.display_pitch, gdc.cursor_enable, gdc.cursor_blink,
                    gdc.cursor_blink_rate);

      DEBUG_ShowMsg("  curs-blink-count=%u curs-blink-state=%u/4 crt-mode=%u",
                    gdc.cursor_blink_count, gdc.cursor_blink_state,
                    pc98_crt_mode ? 1 : 0);
      ...
      cpptmp.clear();
      DEBUG_ShowMsg("PC-98 status: gdc5mhz=%u vsync-int-trig=%u rowheight=%u "
                    "lines-drawn=%u pitch=%u",
                    gdc_5mhz_mode, GDC_vsync_interrupt, gdc.row_height,
                    (unsigned int)vga.draw.lines_done, gdc.display_pitch);
      DEBUG_ShowMsg("  cur-row-line=%u cur-scan=0x%x cur-partition=%u/%u "
                    "part-remline=%u adw=%u",
                    gdc.row_line, gdc.scan_address, gdc.display_partition,
                    gdc.display_partition_mask + 1,
      ...
      DEBUG_ShowMsg("PC-98 CG raster: row-scan=[start=%u/32 end-incl=%u/32] "
                    "blank-at-in-charcell=%u/32",
                    pc98_text_first_row_scanline_start,
                    pc98_text_first_row_scanline_end,
                    pc98_text_row_scanline_blank_at);

      DEBUG_ShowMsg(
          "PC-98 CG scrollregion: countstart=%u char-rows=%u lines=%u",
          pc98_text_row_scroll_count_start, pc98_text_row_scroll_num_lines,
          pc98_text_row_scroll_lines);
    } else if (command == "CGIO") {
      DEBUG_ShowMsg(
          "PC-98 CG I/O port state (A1h-A9h odd): char-row=%u char-code=0x%04x",
          a1_font_char_offset, a1_font_load_addr);
    } else if (command == "EGC") {
      DEBUG_ShowMsg("PC-98 EGC Raw registers:");
      DEBUG_ShowMsg("  4A0h even: %04xh %04xh %04xh %04xh",
                    pc98_egc_raw_values[0], pc98_egc_raw_values[1],
                    pc98_egc_raw_values[2], pc98_egc_raw_values[3]);
      DEBUG_ShowMsg(
          "  4A8h even: %04xh %04xh %04xh %04xh bitplane-access=%02xh",
          pc98_egc_raw_values[4], pc98_egc_raw_values[5],
          pc98_egc_raw_values[6], pc98_egc_raw_values[7], pc98_egc_access);
      DEBUG_ShowMsg("  bg-color=%02xh bgcm=[%02xh %02xh] [%02xh %02xh] [%02xh "
                    "%02xh] [%02xh %02xh]",
                    pc98_egc_background_color, pc98_egc_bgcm[0].b[0],
                    pc98_egc_bgcm[0].b[1], pc98_egc_bgcm[1].b[0],
                    pc98_egc_bgcm[1].b[1], pc98_egc_bgcm[2].b[0],
                    pc98_egc_bgcm[2].b[1], pc98_egc_bgcm[3].b[0],
                    pc98_egc_bgcm[3].b[1]);
      DEBUG_ShowMsg("  fg-color=%02xh fgcm=[%02xh %02xh] [%02xh %02xh] [%02xh "
                    "%02xh] [%02xh %02xh]",
                    pc98_egc_foreground_color, pc98_egc_fgcm[0].b[0],
                    pc98_egc_fgcm[0].b[1], pc98_egc_fgcm[1].b[0],
                    pc98_egc_fgcm[1].b[1], pc98_egc_fgcm[2].b[0],
                    pc98_egc_fgcm[2].b[1], pc98_egc_fgcm[3].b[0],
                    pc98_egc_fgcm[3].b[1]);
      DEBUG_ShowMsg("  fgc-select=%d=%s lead-plane=%u srcmask=[%02xh %02xh]",
                    pc98_egc_fgc, egc_fgc_modes[pc98_egc_fgc],
                    pc98_egc_lead_plane, pc98_egc_srcmask[0],
                    pc98_egc_srcmask[1]);
      DEBUG_ShowMsg(
          "  mask=[%02xh %02xh] mask-effective=[%02xh %02xh] ROP=%02xh",
          pc98_egc_mask[0], pc98_egc_mask[1], pc98_egc_maskef[0],
          pc98_egc_maskef[1], pc98_egc_rop);
      DEBUG_ShowMsg(
          "  compare-lead-plane=%u lightsource=%u shiftinput=%u regload=%u",
          pc98_egc_compare_lead, pc98_egc_lightsource, pc98_egc_shiftinput,
          pc98_egc_regload);
      DEBUG_ShowMsg(
          "  shift-desc=%u shf-destbit=%u shf-srcbit=%u shf-length=%u",
          pc98_egc_shift_descend, pc98_egc_shift_destbit, pc98_egc_shift_srcbit,
          pc98_egc_shift_length);
      DEBUG_ShowMsg("  %s", pc98_egc_shift_debug_status().c_str());
    } else {
      return false;
    }
  ...
  if (command == "TIME") { // Current time
    ...
    if (command != "" &&
        sscanf(command.c_str(), "%u%c%u%c%u", &hour, &c1, &min, &c2, &sec) ==
            5 &&
        c1 == c && c2 == c && hour < 24 && min < 60 && sec < 60) {
      uint32_t ticks =
          (uint32_t)(((double)(hour * 3600 + min * 60 + sec + 0.2)) *
                     18.206481481);
  ...
  if (command == "DATE") { // Current date
    ...
    int n = dos.tables.country[0] == 1
                ? sscanf(command.c_str(), "%u%c%u%c%u", &day, &c1, &month, &c2,
                         &year)
                : (dos.tables.country[0] == 2
                       ? sscanf(command.c_str(), "%u%c%u%c%u", &year, &c1,
                                &month, &c2, &day)
                       : sscanf(command.c_str(), "%u%c%u%c%u", &month, &c1,
                                &day, &c2, &year));
    if (command != "" && n == 5 && c1 == c && c2 == c && year >= 1980 &&
        month && month <= 12 && day &&
  ...
  if (command == "HELP" || command == "?") {
    ...
    DEBUG_ShowMsg("BPINT  [intNr] [ah] [al]  - Set interrupt breakpoint with "
                  "ah and al.\n");
#if C_HEAVY_DEBUG
    DEBUG_ShowMsg(
        "BPM    [segment]:[offset] - Set memory breakpoint (memory change).\n");
    DEBUG_ShowMsg("BPPM   [selector]:[offset]- Set pmode-memory breakpoint "
                  "(memory change).\n");
    DEBUG_ShowMsg("BPLM   [linear address]   - Set linear memory breakpoint "
                  "(memory change).\n");
#endif
    ...
    DEBUG_ShowMsg("DV [addr]                 - Set data view to linear "
                  "(virtual) address\n");
    DEBUG_ShowMsg(
        "DP [addr]                 - Set data view to physical address\n");
    DEBUG_ShowMsg(
    ...
    DEBUG_ShowMsg("LOGS/LOGL/LOGC [num]      - Write short/long/cs:ip-only CPU "
                  "log file.\n");
    DEBUG_ShowMsg("HEAVYLOG                  - Enable/Disable automatic CPU "
                  "log when DOSBox-X exits.\n");
    DEBUG_ShowMsg("ZEROPROTECT               - Enable/Disable zero code "
                  "execution detection.\n");
#endif
    DEBUG_ShowMsg("ADDLOG [message]          - Add message to the log file.\n");
    DEBUG_ShowMsg("SR [reg] [value]          - Set register value. Multiple "
                  "pairs allowed.\n");
    DEBUG_ShowMsg(
        "SM [seg]:[off] [val] [.]..- Set memory with following values.\n");
    DEBUG_ShowMsg("SMV [addr] [val] [.]..    - Set memory with following "
                  "values at linear (virtual) address.\n");
    DEBUG_ShowMsg(
        "FM [seg]:[off]            - Freeze memory value at address.\n");
    ...
    DEBUG_ShowMsg("MMX [=t] [reg]            - Display MMX register file (t "
                  "can be B,W,D,Q)\n");
    DEBUG_ShowMsg("MMX [=t] [reg] SET [val]  - Set MMX register (t can be "
                  "B,W,D,Q), val is comma-separated\n");
    DEBUG_ShowMsg("SSE [=t] [reg]            - Display SSE register file (t "
                  "can be B,W,D,Q,X,S,F)\n");
    DEBUG_ShowMsg("SSE [=t] [reg] SET [val]  - Set SSE register (t can be "
                  "B,W,D,Q,X,S,F), val is comma-separated\n");
    DEBUG_ShowMsg(
    ...
    DEBUG_ShowMsg("VRT                       - Run, then enter debugger at "
    ...
    DEBUG_ShowMsg("ALT + D/E/S/X/B           - Set data view to "
                  "DS:SI/ES:DI/SS:SP/DS:DX/ES:BX.\n");
    ...
    DEBUG_ShowMsg("Home/End                  - Move to begin/end of the "
                  "current window.\n");
    ...
    case 'G': {
      if (instu[2] == 'E') {
        ...
        jmp = (get_ZF() ? false : true) &&
              ((get_SF() ? true : false) == (get_OF() ? true : false)); // JG
      }
    } break;
    case 'L': {
      if (instu[2] == 'E') {
        jmp = (get_ZF() ? true : false) ||
              ((get_SF() ? true : false) != (get_OF() ? true : false)); // JLE
      } else {
...
uint32_t DEBUG_CheckKeys(void) {
  ...
  if (key == KEY_RESIZE) {
#ifdef WIN32 /* BUG: pdcurses notifies us immediately upon getting a resize    \
                event but does not update it's internal structures to reflect                                                \
                the new console size. For example a Win32 console event        \
                     reporting a change to 80x41 would still return 80x40 if   \
                we asked pdcurses right now. To make resizing the console                                                  \
                window less painful, look at pdcurses internal structure       \
                     directly instead.                                         \
                                                                               \
                     Note that we need to call resize_term() or pdcurses will  \
                never notify us about console resize again. */
    /* FIXME: Windows will notify us if the user enlarges the window vertically,
       but will NOT notify us if the user vertically shrinks the window (adds
       scrollbar instead). How do we disable that behavior? */
    {
#ifdef _MSC_VER
      INPUT_RECORD *r = _pdcurses_hax_inputrecord();
      if (r->EventType == WINDOW_BUFFER_SIZE_EVENT) {
        resize_term(r->Event.WindowBufferSizeEvent.dwSize.Y,
  ...
  if (key > 0 || numberrun) {
    ...
    case KEY_F(3): // previous command
      ...
      if (histBuffPos == histBuff.end()) {
        ...
        safe_strncpy(codeViewData.suspInputStr, codeViewData.inputStr,
                     sizeof(codeViewData.suspInputStr));
      }
      safe_strncpy(codeViewData.inputStr, (*--histBuffPos).c_str(),
    ...
    case KEY_F(4): // next command
      ...
      if (++histBuffPos != histBuff.end()) {
        ...
        safe_strncpy(codeViewData.inputStr, codeViewData.suspInputStr,
                     sizeof(codeViewData.inputStr));
      }
      codeViewData.inputPos = (int)strlen(codeViewData.inputStr);
      break;
    case KEY_F(5): // Run Program
      ...
      inhibit_int_breakpoint = false;
      mainMenu.get_item("debugger_rundebug")
          .check(false)
          .refresh_item(mainMenu);
      mainMenu.get_item("debugger_runnormal")
          .check(true)
          .refresh_item(mainMenu);
      mainMenu.get_item("debugger_runwatch")
          .check(false)
          .refresh_item(mainMenu);
    ...
    case KEY_DC: // delete character
      if ((codeViewData.inputPos < 0) || (codeViewData.inputPos >= MAXCMDLEN))
        break;
      if (codeViewData.inputStr[codeViewData.inputPos] != 0) {
        codeViewData.inputStr[MAXCMDLEN] = '\0';
        for (char *p = &codeViewData.inputStr[codeViewData.inputPos];
             (*p = *(p + 1)); p++) {
        }
      }
      break;
...
Bits DEBUG_NullCPUCore(void) { return CBRET_NONE; }
...
Bitu DEBUG_Loop(void) {
  if (debug_running) {
...
#if (C_DYNAMIC_X86)
    if (GetDynamicType() > 0) {
      /* Force dynamic core to flush whatever internal FPU state to the FPU
       * state we can see */
    ...
    if (check_rescroll) {
      ...
      SetCodeWinStart();
      if (ocs != codeViewData.useCS || oip != codeViewData.useEIP) {
        DEBUG_DrawScreen();
      } else {
...
void DEBUG_Enable_Handler(bool pressed) {
  ...
  if (!allow) {
#if defined(MACOSX)
    LOG_MSG("Debugger in Mac OS X is not available unless you start DOSBox-X "
  ...
  } else if (debugrunmode == 2) {
    char command[] = "RUNWATCH";
    ParseCommand(command);
  }
}
...
static void DEBUG_RaiseTimerIrq(void) { PIC_ActivateIRQ(0); }
...
static void LogMCBChain(uint16_t mcb_segment) {
  ...
  uint16_t DOS_dataOfs =
      static_cast<uint16_t>(dataOfs); // Realmode addressing only
  PhysPt dataAddr = PhysMake(
      dataSeg, DOS_dataOfs); // location being viewed in the "Data Overview"
  ...
  while (true) {
    ...
    DEBUG_ShowMsg("   %04X  %12u     %04X %-7s  %s", mcb_segment,
                  mcb.GetSize() << 4, mcb.GetPSPSeg(), psp_seg_note, filename);
    ...
    if (dataAddr >= mcbStartAddr && dataAddr < mcbEndAddr) {
      DEBUG_ShowMsg("   (data addr %04hX:%04X is %u bytes past this MCB)",
                    dataSeg, DOS_dataOfs, dataAddr - mcbStartAddr);
    }
...
static void LogBIOSMem(void) {
  ...
  for (auto i = rombios_alloc.alist.begin(); i != rombios_alloc.alist.end();
       i++) {
    sprintf(tmp, "%08lx-%08lx %s %s", (unsigned long)(i->start),
            (unsigned long)(i->end), i->free ? "FREE  " : "ALLOC ",
...
static void LogEMS(void) {
  ...
  for (Bitu h = 0; h < EMS_Max_Handles(); h++) {
    if (EMS_GetHandle(/*&*/ h_size, /*&*/ xh_addr, /*&*/ h_name, h)) {
      DEBUG_ShowMsg("%6lu %08lx %08lx %s", (unsigned long)h,
                    (unsigned long)xh_addr, (unsigned long)h_size,
                    h_name.c_str());
    }
  }
  ...
  for (Bitu p = 0; p < (GetEMSPageFrameSize() >> 14UL); p++) {
    ...
    if (EMS_GetMapping(handle, log_page, p)) {
      ...
      DEBUG_ShowMsg(
          "%6lu %4lu/%4lu %08lx-%08lx%s", (unsigned long)handle,
...
static void LogXMS(void) {
  ...
  for (Bitu h = 1; h < XMS_GetTotalHandles(); h++) {
    if (XMS_GetHandleInfo(/*&*/ phys_location, /*&*/ size, /*&*/ lockcount,
                          /*&*/ free, h)) {
      if (!free || phys_location != 0 || size != 0) {
        DEBUG_ShowMsg("%6lu %s 0x%08lx %lu", (unsigned long)h,
                      free ? "FREE " : "ALLOC ", (unsigned long)phys_location,
...
static void LogDOSKernMem(void) {
  ...
  if (dos_kernel_disabled) {
    DEBUG_ShowMsg(
        "Cannot enumerate DOS kernel memory while DOS kernel is inactive.");
    return;
  }
  ...
  for (auto i = DOS_GetMemLog.begin(); i != DOS_GetMemLog.end(); i++) {
    sprintf(tmp, "%04x     %8lu     ", (unsigned int)(i->segbase),
    ...
    if (boothax == BOOTHAX_MSDOS) {
      if (guest_msdos_LoL == 0 || guest_msdos_mcb_chain == 0) {
        DEBUG_ShowMsg("Cannot enumerate MCB list while DOS kernel is inactive, "
                      "and DOSBox-X has not yet determined the MCB list of the "
                      "guest MS-DOS operating system");
        return;
      }
...
static void LogGDT(void) {
  ...
  DEBUG_ShowMsg("GDT Base:%08lX Limit:%08lX", (unsigned long)address,
                (unsigned long)length);
  while (address < max) {
    desc.Load(address);
    sprintf(out1, "%04X: b:%08lX type: %02X parbg", (int)(i << 3),
            (unsigned long)desc.GetBase(), desc.saved.seg.type);
...
static void LogLDT(void) {
  ...
  DEBUG_ShowMsg("LDT Base:%08lX Limit:%08lX", (unsigned long)address,
                (unsigned long)length);
  while (address < max) {
    desc.Load(address);
    sprintf(out1, "%04X: b:%08lX type: %02X parbg", (int)((i << 3) | 4),
            (unsigned long)desc.GetBase(), desc.saved.seg.type);
    DEBUG_ShowMsg("%s", out1);
    sprintf(out1, "      l:%08lX dpl : %01X  %1X%1X%1X%1X%1X",
            (unsigned long)desc.GetLimit(), desc.saved.seg.dpl,
...
static void LogIDT(void) {
  ...
  while (address < 256 * 8) {
    if (cpu.idt.GetDescriptor(address, desc)) {
      sprintf(out1, "%04X: sel:%04X off:%02X", (unsigned int)(address / 8),
              (int)desc.GetSelector(), (int)desc.GetOffset());
      DEBUG_ShowMsg("%s", out1);
    }
    address += 8;
  }
  ...
  if (paging.enabled) {
    ...
    if ((sel == 0x00) && ((*selname == 0) || (*selname == '*'))) {
      ...
      if (table.block.p) {
        ...
        sprintf(out1,
                "page %05lXxxx -> %04lXxxx  flags [puw] %x:%x::%x:%x::%x:%x",
                (unsigned long)sel, (unsigned long)entry.block.base,
                entry.block.p, table.block.p, entry.block.us, table.block.us,
                entry.block.wr, table.block.wr);
        DEBUG_ShowMsg("%s", out1);
      } else {
        sprintf(out1, "pagetable %03X not present, flags [puw] %x::%x::%x",
                (int)(sel >> 10), (int)table.block.p, (int)table.block.us,
                (int)table.block.wr);
        DEBUG_ShowMsg("%s", out1);
      }
    }
...
static void LogFPUInfo(void) {
  ...
  for (unsigned int i = 0; i < 8; i++) {
    ...
    DEBUG_ShowMsg(" st(%u): %s val=%.20Lg (0x%04x%08x%08x)", i,
                  FPU_tag(fpu.tags[adj]),
                  reinterpret_cast<long double &>(fpu.p_regs[adj]),
    ...
    DEBUG_ShowMsg(" st(%u): %s val=%.20Lg (0x%04x%016llx)", i,
                  FPU_tag(fpu.tags[adj]), fpu.regs_80[adj].v,
                  fpu.regs_80[adj].raw.h,
                  (unsigned long long)fpu.regs_80[adj].raw.l);
#else
    DEBUG_ShowMsg(" st(%u): %s use80=%u val=%.16g (0x%016llx)", i,
                  FPU_tag(fpu.tags[adj]), fpu.use80[adj], fpu.regs[adj].d,
                  (unsigned long long)fpu.regs[adj].ll);
#endif
  }
...
static void LogCPUInfo(void) {
  ...
  sprintf(out1, "cr0:%08lX cr2:%08lX cr3:%08lX  cpl=%lx",
          (unsigned long)cpu.cr0, (unsigned long)paging.cr2,
          (unsigned long)paging.cr3, (unsigned long)cpu.cpl);
  DEBUG_ShowMsg("%s", out1);
  sprintf(out1, "eflags:%08lX [vm=%x iopl=%x nt=%x]", (unsigned long)reg_flags,
          (int)(GETFLAG(VM) >> 17), (int)(GETFLAG(IOPL) >> 12),
          (int)(GETFLAG(NT) >> 14));
  DEBUG_ShowMsg("%s", out1);
  sprintf(out1, "GDT base=%08lX limit=%08lX", (unsigned long)cpu.gdt.GetBase(),
          (unsigned long)cpu.gdt.GetLimit());
  DEBUG_ShowMsg("%s", out1);
  sprintf(out1, "IDT base=%08lX limit=%08lX", (unsigned long)cpu.idt.GetBase(),
  ...
  Descriptor desc;
  if (cpu.gdt.GetDescriptor(sel, desc)) {
    sprintf(out1, "TR selector=%04X, base=%08lX limit=%08lX*%X", (int)sel,
            (unsigned long)desc.GetBase(), (unsigned long)desc.GetLimit(),
            desc.saved.seg.g ? 0x4000 : 1);
    DEBUG_ShowMsg("%s", out1);
  }
  sel = CPU_SLDT();
  if (cpu.gdt.GetDescriptor(sel, desc)) {
    sprintf(out1, "LDT selector=%04X, base=%08lX limit=%08lX*%X", (int)sel,
            (unsigned long)desc.GetBase(), (unsigned long)desc.GetLimit(),
            desc.saved.seg.g ? 0x4000 : 1);
    DEBUG_ShowMsg("%s", out1);
  }
...
#if C_HEAVY_DEBUG
static void LogInstruction(uint16_t segValue, uint32_t eipValue,
                           ofstream &out) {
  ...
  if (cpuLogType == 0) {
    ...
    out << setw(4) << SegValue(cs) << ":" << setw(8) << reg_eip << "  " << dline
        << "  " << res;
  ...
  if (cpuLogType == 0) {
    out << " SS:" << setw(4) << SegValue(ss) << " C" << (get_CF() > 0) << " Z"
        << (get_ZF() > 0) << " S" << (get_SF() > 0) << " O" << (get_OF() > 0)
        << " I" << GETFLAGBOOL(IF);
  } else {
...
static void SaveMemoryBin(uint16_t seg, uint32_t ofs1, uint32_t num) {
  ...
  for (uint32_t x = 0; x < num; x++) {
    uint8_t val;
    if (mem_readb_checked((PhysPt)GetAddress(seg, ofs1 + x), &val))
      val = 0;
    fwrite(&val, 1, 1, f);
  }
...
static void DrawVariables(void) {
  ...
  for (std::vector<CDebugVar *>::size_type i = 0; i != s; i++) {
    ...
    if (varchanges) {
      ...
      mvwprintw(dbg.win_var, (int)y, ((int)x + DEBUG_VAR_BUF_LEN + 1), "%s",
                buffer);
...
void DEBUG_HeavyWriteLogInstruction(void) {
  ...
  do {
    ...
    /*		fprintf(f,"%04X:%08X   %s  %s  EAX:%08X EBX:%08X ECX:%08X
       EDX:%08X ESI:%08X EDI:%08X EBP:%08X ESP:%08X DS:%04X ES:%04X FS:%04X
       GS:%04X SS:%04X CF:%01X ZF:%01X SF:%01X OF:%01X AF:%01X PF:%01X
       IF:%01X\n",
...
bool DEBUG_HeavyIsBreakpoint(void) {
  ...
  if (!CBreakpoint::BPoints.empty() &&
      CBreakpoint::CheckBreakpoint(SegValue(cs), reg_eip)) {
    return true;
  }
  return false;
}
