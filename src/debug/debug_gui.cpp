...
const char *dbg_win_names[DBGBlock::WINI_MAX_INDEX] = {"REG", "DATA", "CODE",
                                                       "VAR", "OUT"};
...
WINDOW *DBGBlock::get_win(int idx) { return get_win_ref(idx); }

WINDOW *DBGBlock::get_active_win(void) { return get_win((int)active_win); }
...
void DBGUI_DrawBlankOutputLine(int y) {
  ...
  if (logBuffHasDiscarded)
    mvwprintw(dbg.win_out, y, 0,
              "<LOG BUFFER ENDS, OLDER CONTENT DISCARDED BEYOND THIS POINT>");
  else
...
void DBGUI_DrawDebugOutputLine(int y, std::string line) {
  ...
  /* Const cast is needed for pdcurses which has no const char in mvwprintw (bug
   * maybe) */
...
void DEBUG_RefreshPage(char scroll) {
  ...
  /* NTS: Often, i == logBuff.end() unless the user scrolled up in the
   * interface.
   *
   *      So the original intent of this code is that the iterator is one entry
   * PAST the line to begin displaying at the bottom just as end() is a sentinel
   *      value just past the last element of the list. So if i ==
   * logBuff.begin() then there's nothing to display.
   *
   *      Note that the iterator defines what is drawn on the bottom-most line,
   *      and iteration is done backwards until we've drawn the line at i ==
   * logBuff.begin() or until we've drawn something at line 0 of the subwin.
   *
...
void DrawBars(void) {
  ...
  for (unsigned int wnd = 0; wnd < DBGBlock::WINI_MAX_INDEX; wnd++) {
    ...
    if (ref != NULL)
      DrawSubWinBox(ref, dbg.get_wintitle((int)wnd));
  }
...
void DEBUG_GUI_DestroySubWindows(void) { DestroySubWindows(); }
...
static void MakeSubWindows(void) {
  ...
  for (unsigned int wndi = 0; wndi < DBGBlock::WINI_MAX_INDEX; wndi++) {
    ...
    /* window must have been freed, must have height greater than 1 (more than
     * just titlebar) must be visible and there must be room for the window on
     * the screen, where the last row is reserved for the input bar */
    if (dbg.win_height[wnd] > 1 && dbg.win_vis[wnd] && (outy + 1) < win_limit) {
      outy++; // header
      height = (int)dbg.win_height[wnd] - 1;
      if ((outy + height) > win_limit)
        height = win_limit - outy;
      assert(height > 0);
  ...
  for (unsigned int wndi = 0; wndi < DBGBlock::WINI_MAX_INDEX; wndi++) {
    if (yheight[wndi] != 0) {
      ...
      WINDOW *&ref = dbg.get_win_ref((int)wnd);
      assert(ref == NULL);
      ref = subwin(dbg.win_main, (int)yheight[wndi], win_main_maxx,
                   (int)yofs[wndi], 0);
    }
  }
...
void DEBUG_GUI_OnResize(void) { DEBUG_GUI_Rebuild(); }

static void MakePairs(void) {
  init_pair(PAIR_BLACK_BLUE, COLOR_BLACK, COLOR_CYAN);
  init_pair(PAIR_BYELLOW_BLACK, COLOR_YELLOW /*| FOREGROUND_INTENSITY */,
            COLOR_BLACK);
  init_pair(PAIR_GREEN_BLACK, COLOR_GREEN /*| FOREGROUND_INTENSITY */,
            COLOR_BLACK);
  init_pair(PAIR_BLACK_GREY, COLOR_BLACK /*| FOREGROUND_INTENSITY */,
            COLOR_WHITE);
  init_pair(PAIR_GREY_RED, COLOR_WHITE /*| FOREGROUND_INTENSITY */, COLOR_RED);
  init_pair(PAIR_BLACK_GREEN, COLOR_BLACK, COLOR_GREEN);
  init_pair(PAIR_WHITE_BLUE, COLOR_WHITE /* | FOREGROUND_INTENSITY*/,
            COLOR_BLUE);
}
...
bool DEBUG_IsDebuggerConsoleVisible(void) { return (dbg.win_main != NULL); }

void DEBUG_FlushInput(void) {
  if (dbg.win_main != NULL) {
    while (getch() >= 0)
      ; /* remember nodelay() is called to make getch() non-blocking */
  }
}
...
#ifdef WIN32
  /* Tell Windows 10 we DON'T want a thin tall console window that fills the
     screen top to bottom. It's a nuisance especially when the user attempts to
     move the window up to the top to see the status, only for Windows to
     auto-maximize. 30 lines is enough, thanks. */
  {
    if (dbg.win_main) {
      int win_main_maxy, win_main_maxx;
      getmaxyx(dbg.win_main, win_main_maxy, win_main_maxx);
      if (win_main_maxx > 100)
        win_main_maxy = 100;
      if (win_main_maxy > 40)
        win_main_maxy = 40;
...
bool DEBUG_IsPagingOutput(void) { return debugPageStopAt > 0; }
...
void DEBUG_ShowMsg(char const *format, ...) {
  ...
  /* SDLmain() sets control first thing at startup and zeros the pointer at
   * shutdown. control == NULL means we're being called outside SDLmain() when
   * everything has likely been shut down around us. Don't do it. */
  if (control == NULL) {
    fprintf(stderr, "BUG: DEBUG_ShowMsg() called before or after main() with "
                    "nothing fully initialized.\n");
  ...
  if (format == NULL || (log_dev_con == 2 && !logging_con) ||
      control->opt_nolog)
    return;
  in_debug_showmsg = true;

  // in case of runaway error from the CPU core, user responsiveness can be
  // helpful
  ...
  va_start(msg, format);
  len = (size_t)vsnprintf(buf, sizeof(buf) - 2u, format,
                          msg); /* <- NTS: Did you know sprintf/vsnprintf
  ...
  if (logBuff.size() > MAX_LOG_BUFFER) {
    ...
    if (logBuffPos == logBuff.begin())
      ++logBuffPos; /* keep the iterator valid */
    logBuff.pop_front();
  }
      ...
      do {
        ...
        if (key > 0) {
          if (key == ' ' || key == 0x0A) {
            /* continue */
            break;
          } else if (key == 0x27 /*ESC*/ || key == 0x7F /*DEL*/ ||
                     key == 0x08 /*BKSP*/ || key == 'q' || key == 'Q') {
...
#if defined(WIN32)
        /* help inspire confidence in users and in Windows by keeping the event
           loop going. This is to prevent Windows from graying out the main
           window during this loop as "application not responding" */
        SDL_Event ev;
...
void LOG::operator()(char const *format, ...) {
  ...
  if (d_type >= LOG_MAX)
    return;
  if (d_severity < loggrp[d_type].min_severity)
    return;
  DEBUG_ShowMsg("%10u%s %s:%s\n", static_cast<uint32_t>(cycle_count),
                s_severity, loggrp[d_type].front, buf);
}

void LOG::ParseEnableSetting(_LogGroup &group, const char *setting) {
  if (!strcmp(setting, "true") || !strcmp(setting, "1") ||
      !strcmp(setting, "normal"))
    group.min_severity = LOG_NORMAL; /* original code's handling is equivalent
                                        to our "normal" setting */
  else if (!strcmp(setting, "false") || !strcmp(setting, "0") ||
           !strcmp(setting, ""))
    group.min_severity = LOG_ERROR; /* original code's handling is equivalent to
                                       our "error" setting */
...
void LOG::Init() {
  ...
  /* announce */
  LOG_MSG("Logging init: beginning logging proper. This is the end of the "
          "early init logging");

  const Section_prop *sect =
      static_cast<Section_prop *>(control->GetSection("dos"));
  assert(sect != NULL);
  std::string logstr = sect->Get_string("log console");
  log_dev_con = control->opt_log_con || logstr == "true" || logstr == "1"
                    ? 1
                    : (logstr == "quiet" ? 2 : 0);
  ...
  if (logfile.size()) {
    ...
    if ((debuglog = fopen(logfile.c_str(), "wt+")) != NULL) {
      LOG_MSG("Logging: opened logfile '%s' successfully. All further logging "
              "will go to this file.",
              logfile.c_str());
      setbuf(debuglog, NULL);
    } else {
      LOG_MSG("Logging: failed to open logfile '%s'. All further logging will "
              "be discarded. Error: %s",
              logfile.c_str(), strerror(errno));
    }
  } else {
    LOG_MSG("Logging: No logfile was given. All further logging will be "
            "discarded.");
    debuglog = nullptr;
  }
  if (control->opt_nolog && !control->opt_test) {
    // If the user says no log, that means NO LOGGING AT ALL.
    // [https://github.com/joncampbell123/dosbox-x/issues/4405]
  ...
  for (Bitu i = LOG_ALL + 1; i < LOG_MAX;
       i++) { // Skip LOG_ALL, it is always enabled
...
void LOG::SetupConfigSection(void) {
  ...
  Pstring = sect->Add_string("debuggerrun", Property::Changeable::OnlyAtStart,
                             "debugger");
