...
static void bank_setup_pvga1a() {
  // Note: There is some inconsistency in available documentation. Most sources
  // tell that PVGA1A used
  //       only 7 bits of bank index (VGADOC and Ferraro agree on that) but also
  //       point that there are implementations with 1M of RAM which is simply
  //       not possible with 7-bit banks. This implementation assumes that the
  //       eighth bit was actually wired and could be used. This does not
  //       conflict with anything and actually works in WHATVGA just fine.
  if (pvga1a.PR1 & 0x08) {
...
void write_p3cf_pvga1a(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0x09:
    ...
    // Maps to A800h-AFFFh if PR1 bit 3 set and 64k config B000h-BFFFh if 128k
    // config. A000h-AFFFh otherwise.
    pvga1a.PR0A = val;
    bank_setup_pvga1a();
    break;
  case 0x0a:
    ...
    break;
  case 0x0b:
    // Memory size. We only allow to mess with bit 3 here (enable bank B) - this
    // may break some detection schemes
  ...
  case 0x0d:
    ...
    // TODO: Implement bit 2 (CRT address doubling - this mechanism is present
    // in other chipsets as well, but not implemented in DosBox core)
    pvga1a.PR3 = val;
    vga.config.display_start =
        (vga.config.display_start & 0xffffu) | ((val & 0x18u) << 13u);
    vga.config.cursor_start =
        (vga.config.cursor_start & 0xffffu) | ((val & 0x18u) << 13u);
    break;
  case 0x0e:
    // Video control
    // TODO: Figure out if there is anything worth implementing here.
  ...
  case 0x0f:
    ...
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:PVGA1A:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3cf_pvga1a(Bitu reg, Bitu iolen) {
  ...
  case 0x0f:
    return pvga1a.PR5;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:PVGA1A:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
...
void FinishSetMode_PVGA1A(Bitu /*crtc_base*/, VGA_ModeExtraData *modeData) {
  ...
  // Reset to single bank and set it to 0. May need to unlock first (DPaint
  // locks on exit)
  IO_Write(0x3ce, 0x0f);
...
void DetermineMode_PVGA1A() {
  ...
  if (vga.attr.mode_control & 1) {
    if (vga.gfx.mode & 0x40)
      VGA_SetMode((pvga1a.biosMode <= 0x13) ? M_VGA : M_LIN8);
    else if (vga.gfx.mode & 0x20)
      VGA_SetMode(M_CGA4);
    else if ((vga.gfx.miscellaneous & 0x0c) == 0x0c)
      VGA_SetMode(M_CGA2);
    else
      VGA_SetMode((pvga1a.biosMode <= 0x13) ? M_EGA : M_LIN4);
  } else {
    VGA_SetMode(M_TEXT);
  }
}
...
Bitu GetClock_PVGA1A() { return pvga1a.clockFreq[(vga.misc_output >> 2) & 3]; }
