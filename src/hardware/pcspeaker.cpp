 ...
 *        It was written back when all mixer callbacks were potentially done
 * from the SDL audio thread. DOSBox-X has since moved to a model where within
...
typedef enum { LABEL_CONTROL, LABEL_COUNTER, LABEL_OUTPUT } state_label_e;
...
#ifdef SPKR_DEBUGGING
  if (index < 0 || index > 1) {
    LOG_MSG("AddDelayEntry: index out of range %f at %f", index,
            PIC_FullIndex());
  }
#endif
...
static void CheckPITSynchronization(void) {
  if (spkr.pit_clock_gate_enabled) {
    ...
    // FIXME: This code is also detecting many sync errors regarding Mode 0 aka
    // the
    //        "Realsound" (PWM) method of playing digitized speech, though
    //        ironically this bludgeon seems to vastly improve the sound
    //        quality!
    // FIXME: This code maintains good synchronization EXCEPT in the case of
    // Mode 3
    //        with rapid changes to the counter WITHOUT writing port 43h
    //        (new_mode) first. This bludgeon is here to correct that. This is
    //        the problem with timer.cpp and pcspeaker.cpp tracking the counter
    //        separately.
...
static void ForwardPIT(pic_tickindex_t newindex) {
  ...
  if (newindex < 0 || newindex > 1) {
    LOG_MSG("ForwardPIT: index out of range %f at %f", newindex,
            PIC_FullIndex());
  }
#endif
...
void PCSPEAKER_SetCounter_NoNewMode(Bitu cntr) {
  ...
  case 3: /* Square wave generator */
    if (cntr < spkr.minimum_counter) {
      // #ifdef SPKR_DEBUGGING
      //			LOG_MSG(
      //				"SetCounter: too high frequency %u (cntr
      //%u) at %f", 				PIT_TICK_RATE/cntr, 				cntr, 				PIC_FullIndex()); #endif
...
void PCSPEAKER_SetCounter(Bitu cntr, Bitu mode) {
  ...
  ForwardPIT(newindex);
  spkr.pit_index =
      0; // THIS function is always called on the port 43h reset case
  switch (mode) {
  case 0: /* Mode 0 one shot, used with "realsound" (PWM) */
    // if (cntr>80) {
    //	cntr=80;
...
void PCSPEAKER_SetType(bool pit_clock_gate_enabled, bool pit_output_enabled) {
...
#ifdef SPKR_DEBUGGING
  fprintf(PCSpeakerLog, "%f output: %s, clock gate %s\n", PIC_FullIndex(),
  ...
  temp.state_specific_data.output_state.pit_clock_gate_enabled =
  ...
  spkr.pit_clock_gate_enabled =
      pit_clock_gate_enabled && TIMER2_ClockGateEnabled();
 ...
 *      could have been done better. The event queue idea isn't needed anymore
 * because DOSBox-X allows any code to render audio "up to" the current time.
 *
 *      Second, looking at this code tells me why it didn't work properly with
 * the "sample accurate" mixer mode. This code assumes that whatever length of
 *      audio there is to render, that all events within the 1ms tick interval
 * are to be squashed and stretched to fill it. In the new DOSBox-X model mixer
 *      callback code must not assume the call is for 1ms of audio, because any
 *      code at any time can trigger a mixer render "up to" the current time
 * with the tick. */
static void PCSPEAKER_CallBack(Bitu len) {
  ...
  while (count--) {
    ...
    while (index < end) {
      ...
      if (spkr.used && spkr.entries[pos].index <= index) {
        spkr.volwant =
            SPKR_VOLUME * (pic_tickindex_t)spkr.entries[pos].output_level;
        ...
        if (spkr.pit_mode == 3 && spkr.pit_max < (1000.0 / spkr.rate)) {
          /* Unless the speaker is cycling at ultrasonic frequencies, meaning
           * games that "silence" the output by setting the counter way above
           * audible frequencies. */
          ultrasonic = true;
        } else {
          spkr.last_ticks = PIC_Ticks;
...
#ifdef SPKR_DEBUGGING
        fprintf(PCSpeakerOutputLevelLog, "%f %u\n",
  ...
  // Turn off speaker after 10 seconds of idle or one second idle when in off
  // mode
  bool turnoff = false;
  Bitu test_ticks = PIC_Ticks;
  if ((spkr.last_ticks + 10000) < test_ticks)
    turnoff = true;
  if ((!spkr.pit_output_enabled) && ((spkr.last_ticks + 1000) < test_ticks))
    turnoff = true;
  ...
  if (turnoff) {
    if (spkr.volwant == 0) {
      ...
      if (spkr.chan) {
        if (!spkr.pit_output_enabled)
          LOG(LOG_MISC,
              LOG_DEBUG)("Silencing PC speaker output (output disabled)");
        else if (ultrasonic)
          LOG(LOG_MISC, LOG_DEBUG)(
              "Silencing PC speaker output (timeout and ultrasonic frequency)");
        else
          LOG(LOG_MISC, LOG_DEBUG)(
              "Silencing PC speaker output (timeout and non-changing output)");
  ...
  if (spkr.used != 0) {
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("PC speaker queue render, %u entries left, %u rendered",
                   (unsigned int)spkr.used, (unsigned int)pos);
    LOG(LOG_MISC, LOG_DEBUG)("Next entry waits for index %.3f, stopped at %.3f",
                             spkr.entries[0].index, sample_base);
  }
}
...
public:
  PCSPEAKER(Section *configuration) : Module_base(configuration) {
    ...
    if (!section->Get_bool("pcspeaker") || control->opt_silent)
      return;
    ...
    /* set minimum counter value with some headroom so that when games "silence"
     * the PC speaker by setting the counter to an ultrasonic frequency, it
     * averages out into a quiet hiss rather than noisy aliasing noise. */
    spkr.minimum_counter = PIT_TICK_RATE / (spkr.rate * 10);
    SPKR_SPEED =
        (pic_tickindex_t)((SPKR_VOLUME * 2 * 44100) / (0.010 * spkr.rate));
    spkr.used = 0;
    ...
    if (PCSpeakerState == NULL) {
      E_Exit(__FILE__
             ": Unable to create a PC speaker state file for debugging.");
    }
    PCSpeakerOutput = fopen("PCSpeakerOutput.dat", "wb");
    if (PCSpeakerOutput == NULL) {
      E_Exit(__FILE__
             ": Unable to create a PC speaker output file for debugging.");
    }
    PCSpeakerOutputLevelLog = fopen("PCSpeakerOutputLevelLog.txt", "w");
    if (PCSpeakerOutputLevelLog == NULL) {
      E_Exit(__FILE__
             ": Unable to create a PC speaker output level log for debugging.");
    }
#endif
  }
};
...
void PCSPEAKER_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(PCSPEAKER_OnReset));
}
