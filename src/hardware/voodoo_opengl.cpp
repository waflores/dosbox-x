...
void GFX_PreventFullscreen(bool lockout), GFX_SetResizeable(bool enable),
    change_output(int output), SetWindowTransparency(int trans);
#if defined(C_SDL2)
...
static void ogl_get_depth(voodoo_state *VV, INT32 ITERZ, INT64 ITERW,
                          INT32 *depthval, INT32 *out_wfloat) {
  INT32 wfloat;
  ...
  if ((ITERW)&LONGTYPE(0xffff00000000))
    ...
    if ((temp & 0xffff0000) == 0)
      ...
      wfloat = ((exp << 12) |
                (INT32)(((~temp) >> (unsigned int)(19 - exp)) & 0xfffu));
      if (wfloat < 0xffff)
        wfloat++;
    }
  }
...
void ogl_get_fog_blend(voodoo_state *v, INT32 wfloat, INT32 ITERZ, INT64 ITERW,
                       INT32 *fogblend) {
  ...
  *fogblend = 0;
  if (FOGMODE_ENABLE_FOG(FOGMODE) && !FOGMODE_FOG_CONSTANT(FOGMODE)) {
    ...
    case 0: /* fog table */
    {
      ...
      deltaval = (delta & v->fbi.fogdelta_mask) * ((wfloat >> 2) & 0xff);
...
void ogl_get_vertex_data(INT32 x, INT32 y, const void *extradata,
                         ogl_vertex_data *vd) {
  ...
  for (Bitu i = 0; i < extra->texcount; i++) {
    ...
    iterw = v->tmu[i].startw + ((dy * v->tmu[i].dwdy) >> 4) +
            ((dx * v->tmu[i].dwdx) >> 4);
    iters = v->tmu[i].starts + ((dy * v->tmu[i].dsdy) >> 4) +
            ((dx * v->tmu[i].dsdx) >> 4);
    itert = v->tmu[i].startt + ((dy * v->tmu[i].dtdy) >> 4) +
            ((dx * v->tmu[i].dtdx) >> 4);
    ...
    if (s != 0)
      vd->m[i].s = (float)((float)s / (float)(smax * (1u << (18u + ilod))));
    else
      vd->m[i].s = 0.0f;
    if (t != 0)
      vd->m[i].t = (float)((float)t / (float)(tmax * (1u << (18u + ilod))));
    else
      vd->m[i].t = 0.0f;
    if (iterw != 0)
    ...
    if ((TEXMODE_TC_MSELECT(TEXMODE) == 4) ||
        (TEXMODE_TCA_MSELECT(TEXMODE) == 4)) {
      if (v->tmu[i].detailbias <= lod)
        lodblend = 0;
      else {
        lodblend =
            (((v->tmu[i].detailbias - lod) << v->tmu[i].detailscale) >> 8);
        if (lodblend > v->tmu[i].detailmax)
          lodblend = v->tmu[i].detailmax;
      }
    } else if ((TEXMODE_TC_MSELECT(TEXMODE) == 5) ||
               (TEXMODE_TCA_MSELECT(TEXMODE) == 5))
      lodblend = lod & 0xff;
...
void ogl_cache_texture(const poly_extra_data *extra, ogl_texture_data *td) {
  ...
  for (UINT32 j = 0; j < num_tmus; j++) {
    UINT32 TEXMODE =
        (UINT32)(j == 0 ? extra->r_textureMode0 : extra->r_textureMode1);
    ...
    if (extra->texcount && (extra->texcount >= j) &&
        (v->tmu[j].lodmin < (8 << 8))) {
      ...
      if (t != textures[j].end()) {
        if (t->second.valid_pal) {
          ...
          if (u != t->second.ids->end()) {
            ...
            valid_texid = false;
            //						LOG_MSG("texture
            //removed... size %d",t->second.ids->size());
      ...
      if (!valid_texid) {
        ...
        if (TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) < 8) {
          if (TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) != 5) {
            for (int i = 0; i < (smax * tmax); i++) {
              UINT8 *texptr8 =
                  (UINT8 *)&v->tmu[j].ram[(texboffset)&v->tmu[j].mask];
        ...
        } else if (TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) >= 10 &&
                   TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) <= 12) {
          for (int i = 0; i < (smax * tmax); i++) {
            UINT16 *texptr16 =
                (UINT16 *)&v->tmu[j].ram[(texboffset)&v->tmu[j].mask];
          ...
          for (int i = 0; i < (smax * tmax); i++) {
            UINT16 *texptr16 =
                (UINT16 *)&v->tmu[j].ram[(texboffset)&v->tmu[j].mask];
            UINT32 data = (v->tmu[j].lookup[*texptr16 & 0xFF] & 0xFFFFFF) |
                          ((*texptr16 & 0xff00u) << 16u);
            *texrgbp = data;
            texboffset += 2;
            texrgbp++;
          }
          ...
          for (int i = 0; i < (smax * tmax); i++) {
            ...
            UINT32 data = (v->tmu[j].lookup[texel1] & 0xFFFFFF) |
                          ((*texptr16 & 0xff00u) << 16u);
            *texrgbp = data;
            texboffset += 2;
            texrgbp++;
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                        TEXMODE_CLAMP_S(TEXMODE) ? GL_CLAMP_TO_EDGE
                                                 : GL_REPEAT);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                        TEXMODE_CLAMP_T(TEXMODE) ? GL_CLAMP_TO_EDGE
                                                 : GL_REPEAT);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, smax, tmax, 0, GL_BGRA_EXT,
                     GL_UNSIGNED_INT_8_8_8_8_REV, texrgbp);
        extern PFNGLGENERATEMIPMAPEXTPROC glGenerateMipmapEXT;
        glGenerateMipmapEXT(GL_TEXTURE_2D);
        if ((TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) == 0x05) ||
            (TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u) == 0x0e)) {
          UINT32 palsum = calculate_palsum(j);
          if (t == textures[j].end()) {
            std::map<const UINT32, GLuint> *ids =
                new std::map<const UINT32, GLuint>();
            (*ids)[palsum] = texID;
            ogl_texmap tex = {true, true,
                              TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u),
                              texID, ids};
          ...
          ogl_texmap tex = {true, true,
                            TEXMODE_FORMAT(v->tmu[j].reg[textureMode].u), texID,
                            NULL};
          textures[j][texbase] = tex;
        }
      }
...
void ogl_sh_tex_combine(std::string *strFShader, const int TMU,
                        const poly_extra_data *extra) {
  v = (voodoo_state *)extra->state;
...
void ogl_sh_color_path(std::string *strFShader, const poly_extra_data *extra) {
  ...
  if (FBZMODE_ENABLE_CHROMAKEY(FBZMODE)) {
    ...
    *strFShader +=
        "  if (distance (cother.rgb , chromaKey.rgb) < 0.0001) discard;\n";
    //		else {
    //			*strFShader +=	"  if ((cother.rgb >=
    //(chromaKey.rgb-0.01)) && (cother.rgb <= (chromaRange.rgb+0.01)))
    //discard;";
    //		}
  }
...
void ogl_shaders(const poly_extra_data *extra) {
  ...
  if (!extra->info->shader_ready) {
    ...
    glGetObjectParameterivARB(m_hVertexShader, GL_OBJECT_COMPILE_STATUS_ARB,
                              &res);
    if (res == 0) {
    ...
    strFShader = "varying float f_fogblend;\n"
                 "varying float f_lodblend0;\n"
                 "varying float f_lodblend1;\n"
    ...
    glGetObjectParameterivARB(m_hFragmentShader, GL_OBJECT_COMPILE_STATUS_ARB,
                              &res);
    if (res == 0) {
    ...
    glGetObjectParameterivARB(m_hProgramObject, GL_OBJECT_LINK_STATUS_ARB,
                              &res);
    if (res == 0) {
  ...
  if (extra->info->shader_ulocations[0] >= 0)
    glUniform4fARB(
        extra->info->shader_ulocations[0], v->reg[chromaKey].rgb.r / 255.0f,
        v->reg[chromaKey].rgb.g / 255.0f, v->reg[chromaKey].rgb.b / 255.0f, 0);
  if (extra->info->shader_ulocations[1] >= 0)
    glUniform4fARB(extra->info->shader_ulocations[1],
                   v->reg[chromaRange].rgb.r / 255.0f,
                   v->reg[chromaRange].rgb.g / 255.0f,
                   v->reg[chromaRange].rgb.b / 255.0f, 0);
  if (extra->info->shader_ulocations[2] >= 0)
    glUniform4fARB(extra->info->shader_ulocations[2],
                   v->reg[color0].rgb.r / 255.0f, v->reg[color0].rgb.g / 255.0f,
                   v->reg[color0].rgb.b / 255.0f,
                   v->reg[color0].rgb.a / 255.0f);
  if (extra->info->shader_ulocations[3] >= 0)
    glUniform4fARB(extra->info->shader_ulocations[3],
                   v->reg[color1].rgb.r / 255.0f, v->reg[color1].rgb.g / 255.0f,
                   v->reg[color1].rgb.b / 255.0f,
                   v->reg[color1].rgb.a / 255.0f);
  if (extra->info->shader_ulocations[4] >= 0)
    glUniform1fARB(extra->info->shader_ulocations[4],
                   v->reg[alphaMode].rgb.a / 255.0f);
  if (extra->info->shader_ulocations[5] >= 0)
    glUniform1fARB(extra->info->shader_ulocations[5],
                   (float)((UINT16)v->reg[zaColor].u) / 65535.0f);
  if (extra->info->shader_ulocations[8] >= 0)
    glUniform4fARB(
        extra->info->shader_ulocations[8], v->reg[fogColor].rgb.r / 255.0f,
        v->reg[fogColor].rgb.g / 255.0f, v->reg[fogColor].rgb.b / 255.0f, 1.0f);
}
...
void voodoo_ogl_draw_triangle(poly_extra_data *extra) {
  ...
  if (FBZMODE_DEPTH_SOURCE_COMPARE(FBZMODE) &&
      VOGL_CheckFeature(VOGL_HAS_STENCIL_BUFFER)) {
    ...
    if ((FBZMODE_ENABLE_DEPTHBUF(FBZMODE)) &&
        (FBZMODE_ENABLE_ALPHA_PLANES(FBZMODE) == 0)) {
    ...
    for (unsigned int t = 0; t < 2; t++)
      if (td[t].enable) {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                        (int)GL_NEAREST +
                            (int)TEXMODE_MAGNIFICATION_FILTER(TEXMODE));
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                        TEXMODE_CLAMP_S(TEXMODE) ? GL_CLAMP_TO_EDGE
                                                 : GL_REPEAT);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
  ...
  if (ALPHAMODE_ALPHABLEND(ALPHAMODE)) {
    VOGL_SetAlphaMode(
        1, ogl_sfactor[ALPHAMODE_SRCRGBBLEND(ALPHAMODE)],
        ogl_dfactor[ALPHAMODE_DSTRGBBLEND(ALPHAMODE)],
    ...
    for (unsigned int t = 0; t < 2; t++)
      if (td[t].enable) {
        ...
        if (extra->info->shader_ulocations[10u + t] >= 0)
          glVertexAttrib1fARB((GLuint)extra->info->shader_ulocations[10u + t],
                              vd[i].m[t].lodblend);
      }

    if (extra->info->shader_ulocations[9] >= 0)
      glVertexAttrib1fARB((GLuint)extra->info->shader_ulocations[9],
                          vd[i].fogblend);
...
void voodoo_ogl_draw_pixel(int x, int y, bool has_rgb, bool has_alpha, int r,
                           int g, int b, int a) {
  if (m_hProgramObject != 0) {
    glUseProgramObjectARB(0);
    m_hProgramObject = 0;
  ...
  glColor4ub((GLubyte)(r & 0xff), (GLubyte)(g & 0xff), (GLubyte)(b & 0xff),
             (GLubyte)(a & 0xff));
...
void voodoo_ogl_draw_pixel_pipeline(int x, int y, int r, int g, int b) {
  ...
  if (ALPHAMODE_ALPHABLEND(v->reg[alphaMode].u)) {
    VOGL_SetAlphaMode(
        1, ogl_sfactor[ALPHAMODE_SRCRGBBLEND(v->reg[alphaMode].u)],
...
void voodoo_ogl_fastfill(void) {
  ...
  if ((sx == 0) && (sy == 0)) {
    if (((Bitu)ex == (Bitu)v->fbi.width) && ((Bitu)ey == (Bitu)v->fbi.height))
  ...
  if (FBZMODE_RGB_BUFFER_MASK(v->reg[fbzMode].u)) {
    ...
    //		if (FBZMODE_AUX_BUFFER_MASK(v->reg[fbzMode].u) && v->fbi.auxoffs
    //!= (UINT32)(~0)) ...
    VOGL_SetColorMaskMode(true, true);
...
UINT32 voodoo_ogl_read_pixel(int x, int y) {
  UINT32 data[2];
  if ((x < 0) || (y < 0) || (x >= (INT32)v->fbi.width) ||
      (y >= (INT32)v->fbi.height))
    return 0xffffffff;
  ...
  case 0: /* front buffer */
    ...
    if ((cached_line_front_y != y) || (x + 1 >= cached_line_front_width)) {
      ...
      glReadPixels(0, (int)new_height - y, (int)new_width, 1, mode,
                   GL_UNSIGNED_BYTE, cached_line_front_data);
  ...
  case 1: /* back buffer */
    ...
    if ((cached_line_back_y != y) || (x + 1 >= cached_line_back_width)) {
      ...
      glReadPixels(0, (int)new_height - y, (int)new_width, 1, mode,
                   GL_UNSIGNED_BYTE, cached_line_back_data);
  ...
  default:
    E_Exit("read from invalid buf %x",
           LFBMODE_READ_BUFFER_SELECT(v->reg[lfbMode].u));
    break;
  }

  return ((RGB_BLUE(data[0]) >> 3) << 11) | ((RGB_GREEN(data[0]) >> 2) << 5) |
         (RGB_RED(data[0]) >> 3) | ((RGB_BLUE(data[1]) >> 3) << 27) |
         ((RGB_GREEN(data[1]) >> 2) << 21) | ((RGB_RED(data[1]) >> 3) << 16);
}
...
void voodoo_ogl_set_window(voodoo_state *v) {
  ...
  if ((new_width != last_width) || (new_height != last_height))
    size_changed = true;
  if (size_changed ||
      (last_orientation != (INT32)FBZMODE_Y_ORIGIN(v->reg[fbzMode].u))) {
    glLoadIdentity();
    if (FBZMODE_Y_ORIGIN(v->reg[fbzMode].u))
  ...
  if (size_changed) {
    ...
    if (new_height < 480 && GFX_IsFullscreen())
      adjust_y = (480 - new_height) / 2;
    if (new_width < 640 && GFX_IsFullscreen())
      adjust_x = (640 - new_width) / 2;
...
void voodoo_ogl_reset_videomode(void) {
  ...
  // broken on windows (longstanding SDL bug), may help other platforms to force
  ...
  void GFX_LosingFocus(void), GFX_ReleaseMouse(void),
      GFX_ForceFullscreenExit(void);
  ...
  if (ogl_surface == NULL) {
    ...
    if (ogl_surface == NULL) {
      ...
      if ((ogl_surface = SDL_SetVideoMode(new_width, new_height, 32,
                                          sdl_flags)) == NULL) {
        has_stencil = false;
        ...
        LOG_MSG(
            "VOODOO: Graphics mode does not support Stencil/Alpha channels");
      } else {
  ...
  if (few_colors)
    LOG_MSG("opengl: warning: graphics mode with insufficient color depth");

  if (SDL_GL_GetAttribute(SDL_GL_DEPTH_SIZE, &value) == 0) {
    if (value < 24)
      LOG_MSG("opengl: warning: depth buffer with insufficient resolution");
  }
  ...
  if (has_stencil)
    VOGL_FlagFeature(VOGL_HAS_STENCIL_BUFFER);
  if (has_alpha)
    VOGL_FlagFeature(VOGL_HAS_ALPHA_PLANE);
...
#if defined(WIN32) && !defined(C_SDL2)
  // Windows SDL 1.x builds may destroy and re-create the window, and lose our
  // menu bar. make sure to put it back.
  ...
  /* Something in Windows keeps changing the shade model on us from the last
   * glShadeModel() call. Change it back. */
  glShadeModel(GL_SMOOTH);
...
bool voodoo_ogl_init(voodoo_state *v) {
  ...
  transparency = 0;
  SetWindowTransparency(static_cast<Section_prop *>(control->GetSection("sdl"))
                            ->Get_int("transparency"));

  /*	std::string features = "";
          if (VOGL_CheckFeature(VOGL_HAS_SHADERS)) features += " shader";
          if (VOGL_CheckFeature(VOGL_HAS_ALPHA_PLANE)) features += "
     alpha-plane"; if (VOGL_CheckFeature(VOGL_HAS_STENCIL_BUFFER)) features += "
     stencil-buffer";
    ...
    for (; info; info = info->next) {
      if (info->shader_ready) {
        ...
        if (info->so_shader_program > 0) {
          if (info->so_vertex_shader > 0)
            glDetachObjectARB((GLhandleARB)info->so_shader_program,
                              (GLhandleARB)info->so_vertex_shader);
          if (info->so_fragment_shader > 0)
            glDetachObjectARB((GLhandleARB)info->so_shader_program,
                              (GLhandleARB)info->so_fragment_shader);
          if (info->so_vertex_shader > 0)
            glDeleteObjectARB((GLhandleARB)info->so_vertex_shader);
          if (info->so_fragment_shader > 0)
            glDeleteObjectARB((GLhandleARB)info->so_fragment_shader);
...
#if defined(C_SDL2)
    if (Direct3D_using()) {
      GFX_SetSDLWindowMode(currentWindowWidth, currentWindowHeight,
                           SCREEN_SURFACE);
      change_output(6);
    }
#endif
...
void voodoo_ogl_shutdown(voodoo_state *v) { (void)v; }
...
void voodoo_ogl_clear(void) { E_Exit("invalid call to voodoo_ogl_clear"); }
