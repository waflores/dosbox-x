...
struct PIC_Controller {
  ...
  uint8_t input;      // input signal (directly set by raise/lower irq) used to
  ...
  void lower_irq(uint8_t val) {
    ...
    if (irr & bit) { // value will change (as it is currently active)
      ...
      if ((bit & imrr) & isrr) { // not masked and not in service
        // This irq might have toggled PIC_IRQCheck/caused irq 2 on master, when
        // it was raised. If it is active, then recheck it, we can't just
        // deactivate as there might be more IRQS raised.
  ...
  // handles all bits and logic related to starting this IRQ, it does NOT start
  // the interrupt on the CPU.
...
Bitu PIC_Ticks = 0;
Bitu PIC_IRQCheck = 0; // Maybe make it a bool and/or ensure 32bit size (x86
                       // dynamic core seems to assume 32 bit variable size)
Bitu PIC_IRQCheckPending =
    0; // Maybe make it a bool and/or ensure 32bit size (x86 dynamic core seems
       // to assume 32 bit variable size)
bool enable_slave_pic =
    true; /* if set, emulate slave with cascade to master. if clear, emulate
             only master, and no cascade (IRQ 2 is open) */
...
void PIC_Controller::check_for_irq() {
  ...
  if (possible_irq) {
    ...
    if (ignore_cascade_in_service && this == &master &&
        a_irq == (unsigned char)master_cascade_irq)
      a_irq++;
...
void PIC_Controller::raise_irq(uint8_t val) {
  ...
  if ((irr & bit) == 0) { // value changed (as it is currently not active)
    ...
    if ((bit & imrr) & isrr) { // not masked and not in service
      if (special || val < active_irq)
        activate();
      else if (ignore_cascade_in_service && this == &master &&
               val == (unsigned char)master_cascade_irq)
        activate();
    }
  }
}
...
void PIC_Controller::set_imr(uint8_t val) {
  ...
  // Those bits have impact on whether the cpu emulation should be paused or
  // not.
  if ((irr & change) & isrr)
    check_for_irq();
}
...
void PIC_Controller::activate() {
  ...
  if (this == &master) {
    ...
    if (!PIC_IRQCheckPending) {
      /* NTS: PIC_AddEvent by design caps CPU_Cycles to make the event happen on
       * time */
...
void PIC_Controller::deactivate() {
  ...
  if (this == &master) {
     ...
     *
     *      What would happen if one device raised an IRQ (setting
     * PIC_IRQCheck=1) then before dispatching IRQs, another device lowered an
     * IRQ (setting PIC_IRQCheck=0).
     *
     *      Lowering the IRQ would reset the flag, and PIC_runIRQs() would never
     * process any pending IRQs even though some are waiting.
     *
     *      It's better if we set the flag when raising an IRQ, and leave the
     * flag set until PIC_runIRQs() determines there are no more IRQs to
     * dispatch. Then and only then will PIC_runIRQs() clear the flag. */
  } else {
    /* just because ONE IRQ on the slave finished doesn't mean there aren't any
     * others needing service! */
    if ((irr & imrr) == 0)
      master.lower_irq(master_cascade_irq);
    else
      LOG_MSG("Slave PIC: still to handle irr=%02x imrr=%02x isrr=%02x", irr,
              imrr, isrr);
  }
}
...
void PIC_Controller::start_irq(uint8_t val) {
  irr &= ~(1 << (val));
  if (!auto_eoi) {
    if (never_mark_cascade_in_service && this == &master &&
        val == master_cascade_irq) {
      /* do nothing */
    } else {
      active_irq = val;
      isr |= 1 << (val);
...
static void write_command(Bitu port, Bitu val, Bitu iolen) {
  (void)iolen; // UNUSED
  PIC_Controller *pic =
      &pics[(port == 0x20 /*IBM*/ || port == 0x00 /*PC-98*/) ? 0 : 1];

  if (GCC_UNLIKELY(val & 0x10)) { // ICW1 issued
    if (val & 0x04)
      LOG_MSG("PIC: 4 byte interval not handled");
    if (val & 0x08)
      LOG_MSG("PIC: level triggered mode not handled");
    if (val & 0xe0)
      LOG_MSG("PIC: 8080/8085 mode not handled");
    pic->set_imr(0);
    ...
    if (val & 0x04)
      LOG_MSG("PIC: poll command not handled");
    if (val & 0x02) { // function select
      if (val & 0x01)
        pic->request_issr =
            true; /* select read interrupt in-service register */
      else
    ...
    if (val & 0x40) { // special mask select
      if (val & 0x20)
        ...
        pic->special = false;
      // Check if there are irqs ready to run, as the priority system has
      // possibly been changed.
      pic->check_for_irq();
      LOG(LOG_PIC, LOG_NORMAL)("port %X : special mask %s", (int)port,
                               (pic->special) ? "ON" : "OFF");
    }
  } else {            // OCW2 issued
    if (val & 0x20) { // EOI commands
      if (GCC_UNLIKELY(val & 0x80))
        LOG_MSG("rotate mode not supported");
      if (val & 0x40) { // specific EOI
        ...
        if (pic->active_irq != 8) {
          // If there is no irq in service, ignore the call, some games send an
          // eoi to both pics when a sound irq happens (regardless of the irq).
...
static void write_data(Bitu port, Bitu val, Bitu iolen) {
  (void)iolen; // UNUSED
  PIC_Controller *pic =
      &pics[(port == 0x21 /*IBM*/ || port == 0x02 /*PC-98*/) ? 0 : 1];
  ...
  case 0: /* mask register */
    ...
    break;
  case 1: /* icw2          */
    LOG(LOG_PIC, LOG_NORMAL)("%d:Base vector %X", port == 0x21 ? 0 : 1,
                             (int)val);
  ...
  case 3: /* icw 4 */
    ...
    */
    ...
    if ((val & 0x01) == 0)
      LOG_MSG("PIC:ICW4: %x, 8085 mode not handled", (int)val);
    if ((val & 0x10) != 0)
      LOG_MSG("PIC:ICW4: %x, special fully-nested mode not handled", (int)val);
...
static Bitu read_command(Bitu port, Bitu iolen) {
  ...
  if (pic->request_issr) {
    ...
    /* HACK: I found a PC-98 game "Steel Gun Nyan" that relies on setting the
     * timer to Mode 3 (Square Wave) then polling the output through the master
     * PIC's IRR to do delays. */
    if (pic == &master) {
...
void PIC_EdgeTrigger(Bitu irq, bool set) {
  if (IS_PC98_ARCH) {
    if (irq == 7)
      return;
  } else if (enable_slave_pic) { /* PC/AT emulation with slave PIC cascade to
                                    master */
    if (irq == 2)
      irq = 9;
  ...
  if (set)
    pic->edge |= 1u << (unsigned char)t;
  else
    pic->edge &= ~(1u << (unsigned char)t);
}

/* FIXME: This should be called something else that's true to the ISA bus, like
 * PIC_PulseIRQ, not Activate IRQ. ISA interrupts are edge triggered, not level
 * triggered. */
void PIC_ActivateIRQ(Bitu irq) {
  /* Remember what was once IRQ 2 on PC/XT is IRQ 9 on PC/AT */
  if (IS_PC98_ARCH) {
    if (irq == 7) {
      LOG(LOG_PIC, LOG_ERROR)("Attempted to raise IRQ %u, which is cascade IRQ",
                              (int)irq);
    ...
    if (irq == 9)
      irq = 2;
    if (irq >= 8) {
      LOG(LOG_PIC, LOG_ERROR)(
          "Attempted to raise IRQ %u when slave PIC does not exist", (int)irq);
      return;
    }
  }
...
void PIC_DeActivateIRQ(Bitu irq) {
  ...
  if (IS_PC98_ARCH) {
    ...
    if (irq >= 8) {
      LOG(LOG_PIC, LOG_ERROR)(
          "Attempted to lower IRQ %u when slave PIC does not exist", (int)irq);
      return;
    }
  }
...
static void slave_startIRQ() {
  ...
  if (GCC_UNLIKELY(pic1_irq == 8)) {
    if (!skipped_irq) {
      /* we have an IRQ routing problem. this code is supposed to emulate the
       * fact that what was once IRQ 2 on PC/XT is routed to IRQ 9 on AT
       * systems, because IRQ 8-15 cascade to IRQ 2 on such systems. but it's
       * nothing to E_Exit() over. */
      LOG(LOG_PIC,
          LOG_ERROR)("ISA PIC problem: IRQ %d (cascade) is active on master "
                     "PIC without active IRQ 8-15 on slave PIC.",
                     master_cascade_irq);
...
void PIC_runIRQs(void) {
  if (!GETFLAG(IF))
    return;
  if (GCC_UNLIKELY(!PIC_IRQCheck))
    return;
  if (GCC_UNLIKELY(cpudecoder == CPU_Core_Normal_Trap_Run))
    return; // FIXME: Why?
  if (GCC_UNLIKELY(CPU_NMI_active))
    return;
  ...
  for (i = 0, s = 1; i < max; i++, s <<= 1) {
    if (p & s) {
      ...
      if ((int)i == master_cascade_irq) { // second pic, or will not match if
...
bool PIC_RunQueue(void) {
  ...
  /* I/O delay can cause negative CPU_Cycles and PIC event / audio rendering
   * issues */
  ...
  // WARNING: If the problem is the cascade interrupt un-acknowledged, this will
  // give a false positive
  if (!PIC_IRQCheck && !PIC_IRQCheckPending &&
      ((master.irr & master.imrr) != 0 || (slave.irr & slave.imrr) != 0))
  ...
  if (CPU_CycleLeft > 0) {
    ...
    while (pic_queue.next_entry &&
      ...
      if (entry->pic_event != NULL)
        ...
        LOG(LOG_MISC, LOG_WARN)(
            "PIC: Event in queue with NULL handler"); // This can happen after
    ...
    if (pic_queue.next_entry) {
      Bits cycles =
          (Bits)(pic_queue.next_entry->index * CPU_CycleMax - index_nd);
  ...
  /* if we're out of cycles, then return false. don't execute any more
   * instructions */
...
void TIMER_ShutdownTickHandlers() {
  ...
  if (leftovers != 0)
    LOG(LOG_MISC, LOG_DEBUG)("TIMER: %u leftover handlers (clean up!).",
                             leftovers);
}
...
void TIMER_AddTick(void) {
  ...
  if ((PIC_Ticks & 0x3fff) == 0) {
    ...
    int delta =
        int((PIC_Ticks - PIC_tickstart) * 10000 / (ticks - PIC_benchstart) + 5);
...
void PIC_Reset(Section *sec) {
  ...
  /* NTS: Parsing this on reset allows PIC configuration changes on reboot
   * instead of restarting the entire emulator */
  ...
  never_mark_cascade_in_service =
      section->Get_bool("cascade interrupt never in service");

  {
    ...
    LOG(LOG_MISC, LOG_DEBUG)("PIC: Ignore cascade in service=%u",
                             ignore_cascade_in_service);
  }

  if (enable_slave_pic && machine == MCH_PCJR && enable_pc_xt_nmi_mask) {
    LOG(LOG_MISC,
        LOG_DEBUG)("PIC_Reset(): PCjr emulation with NMI mask register "
                   "requires disabling slave PIC (IRQ 8-15)");
    enable_slave_pic = false;
  }

  if (!enable_slave_pic && IS_PC98_ARCH)
    LOG(LOG_MISC, LOG_DEBUG)(
        "PIC_Reset(): PC-98 emulation without slave PIC (IRQ 8-15) is unusual");

  /* NTS: This is a good guess. But the 8259 is static circuitry and not driven
   * by a clock. But the ability to respond to interrupts is limited by the CPU,
   * too. */
  PIC_irq_delay_ns = 1000000000UL / (unsigned long)PIT_TICK_RATE;
  {
    int x = section->Get_int("irq delay ns");
    if (x >= 0)
      PIC_irq_delay_ns = (unsigned int)x;
  }
  ...
  // LOG
  LOG(LOG_MISC,
      LOG_DEBUG)("PIC_Reset(): reinitializing PIC controller (cascade=%d)",
                 master_cascade_irq);
  ...
  if (IS_PC98_ARCH) {
    ...
    if (pc98_section != NULL &&
        pc98_section->Get_bool("pc-98 pic init to read isr"))
  ...
  if (master_cascade_irq >= 0) {
    PIC_SetIRQMask((unsigned int)master_cascade_irq,
                   false); /* Enable second pic */
...
void DEBUG_LogPIC_C(PIC_Controller &pic) {
  LOG_MSG("%s interrupt controller state",
          &pic == &master ? "Master" : "Slave");
  LOG_MSG("ICW %u/%u special=%u auto-eoi=%u rotate-eoi=%u single=%u "
          "request_issr=%u vectorbase=0x%02x active_irq=%u isr=%02x isrr=%02x "
          "isrignore=%02x",
          (unsigned int)pic.icw_index, (unsigned int)pic.icw_words,
          pic.special ? 1 : 0, pic.auto_eoi ? 1 : 0,
          pic.rotate_on_auto_eoi ? 1 : 0, pic.single ? 1 : 0,
          pic.request_issr ? 1 : 0, pic.vector_base, pic.active_irq, pic.isr,
          pic.isrr, pic.isr_ignore);
  ...
  for (unsigned int si = 0; si < 8; si++) {
    ...
    LOG_MSG("%3u 0x%02X   %c    %c    %c   %s", IRQ, CPUINT,
            (pic.irr & (1U << si)) ? 'R' : ' ',
            (pic.imr & (1U << si)) ? 'M' : ' ',
            (pic.isr & (1U << si)) ? 'S' : ' ',
...
// extern void *NE2000_TX_Event_PIC_Event;
// // Ne2000.cpp

// PIC_TimerHandlers
#if C_IPX
// extern void *IPX_ClientLoop_PIC_Timer;
// // Ipx.cpp extern void *IPX_ServerLoop_PIC_Timer;
// // Ipxserver.cpp
#endif
...
// extern void *NE2000_Poller_PIC_Event;
// // Ne2000.cpp
...
const void *pic_state_event_table[] = {
    NULL, cmos_timerevent_PIC_Event, DISNEY_disable_PIC_Event,
    GUS_TimerEvent_PIC_Event,
#if C_IPX
//	IPX_AES_EventHandler_PIC_Event,
#endif
    KEYBOARD_TransferBuffer_PIC_Event, MOUSE_Limit_Events_PIC_Event,
    MPU401_Event_PIC_Event, DMA_Silent_Event_PIC_Event,
    DSP_FinishReset_PIC_Event, DSP_RaiseIRQEvent_PIC_Event,
    DMA_DAC_Event_PIC_Event, END_DMA_Event_PIC_Event, END_DMA_Event_PIC_Event,
    Serial_EventHandler_PIC_Event, PIT0_Event_PIC_Event,
    VGA_DisplayStartLatch_PIC_Event, VGA_DrawEGASingleLine_PIC_Event,
    // VGA_DrawPart_PIC_Event,
    VGA_DrawSingleLine_PIC_Event, VGA_Other_VertInterrupt_PIC_Event,
    VGA_PanningLatch_PIC_Event, VGA_SetupDrawing_PIC_Event,
    VGA_VertInterrupt_PIC_Event, VGA_VerticalTimer_PIC_Event,
    fmport_a_pic_event_PIC_Event, fmport_b_pic_event_PIC_Event,
...
#endif
    KEYBOARD_TickHandler_PIC_Timer, KEYBOARD_TickHandler_PIC_Timer,
...
uint16_t PIC_State_FindEvent(Bitu addr) {
  ...
  for (int lcv = 0; lcv < size; lcv++) {
    if (addr == (Bitu)(pic_state_event_table[lcv]))
      return lcv;
  }
...
uint16_t PIC_State_FindTimer(Bitu addr) {
  ...
  for (int lcv = 0; lcv < size; lcv++) {
    if (addr == (Bitu)(pic_state_timer_table[lcv]))
      return lcv;
  }
...
private:
  void getBytes(std::ostream &stream) override {
    ...
    stream.write(reinterpret_cast<const char *>(&PIC_IRQCheck),
                 sizeof(PIC_IRQCheck));
    stream.write(reinterpret_cast<const char *>(&PIC_IRQCheckPending),
                 sizeof(PIC_IRQCheckPending));
    ...
    for (int lcv = 0; lcv < PIC_QUEUESIZE; lcv++) {
      ...
      // - data
      stream.write(
          reinterpret_cast<const char *>(&pic_queue.entries[lcv].index),
          sizeof(pic_queue.entries[lcv].index));
      stream.write(
          reinterpret_cast<const char *>(&pic_queue.entries[lcv].value),
          sizeof(pic_queue.entries[lcv].value));

      // - function ptr
      event_idx = PIC_State_FindEvent((Bitu)(pic_queue.entries[lcv].pic_event));
      stream.write(reinterpret_cast<const char *>(&event_idx),
                   sizeof(event_idx));

      // - reloc ptr
      stream.write(reinterpret_cast<const char *>(&pic_next_ptr[lcv]),
                   sizeof(pic_next_ptr[lcv]));

      if (&pic_queue.entries[lcv] == pic_queue.free_entry)
        pic_free_idx = lcv;
    ...
    // - reloc ptrs
    stream.write(reinterpret_cast<const char *>(&pic_free_idx),
                 sizeof(pic_free_idx));
    stream.write(reinterpret_cast<const char *>(&pic_next_idx),
                 sizeof(pic_next_idx));

    // - data
    stream.write(reinterpret_cast<const char *>(&InEventService),
                 sizeof(InEventService));
    stream.write(reinterpret_cast<const char *>(&srv_lag), sizeof(srv_lag));

    // - reloc ptr
    stream.write(reinterpret_cast<const char *>(&ticker_size),
                 sizeof(ticker_size));
    ...
    for (int lcv = 0; lcv < ticker_size; lcv++) {
      ...
      stream.write(reinterpret_cast<const char *>(&ticker_handler_idx),
                   sizeof(ticker_handler_idx));

      // - reloc new ptr (leave alone)
      // stream.write(reinterpret_cast<const char*>(&ticker_ptr->next),
      // sizeof(ticker_ptr->next) );

      ticker_ptr = ticker_ptr->next;
    }

    // - system (leave alone)
    // stream.write(reinterpret_cast<const char*>(&PIC_benchstart),
    // sizeof(PIC_benchstart) ); stream.write(reinterpret_cast<const
    // char*>(&PIC_tickstart), sizeof(PIC_tickstart) );
  ...
  void setBytes(std::istream &stream) override {
    ...
    stream.read(reinterpret_cast<char *>(&PIC_IRQCheckPending),
                sizeof(PIC_IRQCheckPending));
    ...
    for (int lcv = 0; lcv < PIC_QUEUESIZE; lcv++) {
      ...
      // - data
      stream.read(reinterpret_cast<char *>(&pic_queue.entries[lcv].index),
                  sizeof(pic_queue.entries[lcv].index));
      stream.read(reinterpret_cast<char *>(&pic_queue.entries[lcv].value),
                  sizeof(pic_queue.entries[lcv].value));

      // - function ptr
      stream.read(reinterpret_cast<char *>(&event_idx), sizeof(event_idx));
      pic_queue.entries[lcv].pic_event =
          (PIC_EventHandler)PIC_State_IndexEvent(event_idx);
    ...
    // - data
    stream.read(reinterpret_cast<char *>(&InEventService),
                sizeof(InEventService));
    ...
    if (ticker_size) {
      ...
      for (int lcv = 0; lcv < ticker_size; lcv++) {
        ...
        ticker_ptr->handler =
            (TIMER_TickHandler)PIC_State_IndexTimer(ticker_idx);
    ...
    // - system (leave alone)
    // stream.read(reinterpret_cast<char*>(&PIC_benchstart),
    // sizeof(PIC_benchstart) );
    // stream.read(reinterpret_cast<char*>(&PIC_tickstart),
    // sizeof(PIC_tickstart) );
