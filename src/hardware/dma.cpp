...
enum { DMA_INCREMENT, DMA_DECREMENT };

/* DMA block anti-copypasta common code */
template <const unsigned int dma_mode>
static inline void DMA_BlockReadCommonSetup(
    /*output*/ PhysPt &o_xfer, unsigned int &o_size,
    /*input*/ PhysPt const spage, PhysPt offset, Bitu size, const uint8_t dma16,
    const uint32_t DMA16_ADDRMASK) {
  assert(size != 0u);

  const Bitu highpart_addr_page = spage >> 12;
  size <<= dma16;
  offset <<= dma16;
  const uint32_t dma_wrap =
      (((0xfffful << dma16) + dma16) & DMA16_ADDRMASK) | dma_wrapping;
  offset &= dma_wrap;
  ...
  if (page < EMM_PAGEFRAME4K)
    ...
    page = paging.firstmb[page];
  /* check our work, should not cross 4KB, then transfer linearly. Do not remove
   * {} curly braces, assert() may compile to nothing. */
  if (dma_mode == DMA_INCREMENT) {
    assert((offset + size - ((Bitu)1u << dma16)) < 4096);
  } /* should stop with offset at or before first byte of next page */
  else if (dma_mode == DMA_DECREMENT) {
    assert(offset >= (size - ((Bitu)1u << dma16)));
  } /* should stop with offset at or after the last byte of the previous page */
  /* result */
 ...
 * This code assumes new DMA read code that transfers 4KB at a time (4KB byte or
 * 2KB word) therefore it is safe to compute the page and offset once and
 * transfer in a tight loop. */
template <const unsigned int dma_mode>
static void DMA_BlockRead4KB(const PhysPt spage, const PhysPt offset,
                             void *data, const Bitu size, const uint8_t dma16,
                             const uint32_t DMA16_ADDRMASK) {
  uint8_t *write = (uint8_t *)data;
  unsigned int o_size;
  PhysPt xfer;

  DMA_BlockReadCommonSetup<dma_mode>(/*&*/ xfer, /*&*/ o_size, spage, offset,
                                     size, dma16, DMA16_ADDRMASK);
  if (!dma16) { // 8-bit
    for (; o_size; o_size--, (dma_mode == DMA_DECREMENT ? (xfer--) : (xfer++)))
      *write++ = phys_readb(xfer);
  } else { // 16-bit
    assert((o_size & 1u) == 0);
    assert((xfer & 1u) == 0);
    for (; o_size; o_size -= 2,
                   (dma_mode == DMA_DECREMENT ? (xfer -= 2) : (xfer += 2)),
                   write += 2)
...
template <const unsigned int dma_mode>
static void DMA_BlockWrite4KB(PhysPt spage, PhysPt offset, const void *data,
                              Bitu size, uint8_t dma16,
                              const uint32_t DMA16_ADDRMASK) {
  ...
  if (!dma16) { // 8-bit
    ...
    for (; o_size; o_size -= 2,
                   (dma_mode == DMA_DECREMENT ? (xfer -= 2) : (xfer += 2)),
                   read += 2)
      phys_writew(xfer, host_readw(read));
  }
}
...
static void DMA_Write_Port(Bitu port, Bitu val, Bitu /*iolen*/) {
  if (IS_PC98_ARCH) {
    ...
    if (port < 0x20u)
      ...
      switch ((port >> 1u) & 3u) {
      case 0: /* 21h DMA channel 1 */
        port = 0x83;
        break;
      case 1: /* 23h DMA channel 2 */
        port = 0x81;
        break;
      case 2: /* 25h DMA channel 3 */
        port = 0x82;
        break;
      case 3: /* 27h DMA channel 0 */
        port = 0x87;
        break;
      default:
        abort();
      ...
      DmaControllers[0]->GetChannel(val & 3)->page_bank_increment_wraparound =
          ((val & 0x08) ? 0xF0 : 0x00) + ((val & 0x04) ? 0x0F : 0x00);
#if 0
  ...
  if (port < 0x10) {
    ...
    case 0x81:
      GetDMAChannel(2)->SetPage((uint8_t)val);
      break;
    case 0x82:
      GetDMAChannel(3)->SetPage((uint8_t)val);
      break;
    case 0x83:
      GetDMAChannel(1)->SetPage((uint8_t)val);
      break;
    case 0x87:
      GetDMAChannel(0)->SetPage((uint8_t)val);
      break;
    case 0x89:
      GetDMAChannel(6)->SetPage((uint8_t)val);
      break;
    case 0x8a:
      GetDMAChannel(7)->SetPage((uint8_t)val);
      break;
    case 0x8b:
      GetDMAChannel(5)->SetPage((uint8_t)val);
      break;
    case 0x8f:
      GetDMAChannel(4)->SetPage((uint8_t)val);
      break;
    default:
      if (!enable_dma_extra_page_registers)
        LOG(LOG_DMACONTROL, LOG_NORMAL)(
            "Trying to write undefined DMA page register %x", (int)port);
      break;
    }
  }
}
...
static Bitu DMA_Read_Port(Bitu port, Bitu iolen) {
  ...
  if (port < 0x10) {
    ...
    case 0x8f:
      return GetDMAChannel(4)->pagenum;
    default:
      if (enable_dma_extra_page_registers)
        return dma_extra_page_registers[port & 0xF];

      LOG(LOG_DMACONTROL, LOG_NORMAL)(
          "Trying to read undefined DMA page register %x", (int)port);
      break;
    }
  }
...
void DmaController::WriteControllerReg(Bitu reg, Bitu val, Bitu /*len*/) {
  ...
  case 0xb: /* Mode Register */
    ...
    chan->increment = (!allow_decrement_mode) ||
                      ((val & 0x20) == 0); /* 0=increment 1=decrement */
    chan->transfer_mode = ((val >> 2) & 3);
    // TODO Maybe other bits? Like bits 6-7 to select
    // demand/single/block/cascade mode? */
    break;
...
Bitu DmaController::ReadControllerReg(Bitu reg, Bitu /*len*/) {
  ...
  case 0x8: /* Status Register */
    ...
    return ret;
  case 0xc: /* Clear Flip/Flip (apparently most motherboards will treat read OR
               write as reset) */
    flipflop = false;
    break;
  case 0xf: /* Multiple Mask register, undocumented read back */
    ...
    return ret;
  default:
    LOG(LOG_DMACONTROL, LOG_NORMAL)("Trying to read undefined DMA port %x",
                                    (int)reg);
    break;
  }
  return 0xffffffff;
...
DmaChannel::DmaChannel(uint8_t num, bool dma16) {
  ...
  LOG(LOG_DMACONTROL,
      LOG_DEBUG)("DMA channel %u. DMA16_PAGESHIFT=%u DMA16_ADDRMASK=0x%lx",
                 (unsigned int)channum, (unsigned int)DMA16_PAGESHIFT,
                 (unsigned long)DMA16_ADDRMASK);
  pagenum = 0;
  pagebase = 0;
  baseaddr = 0;
  curraddr = 0;
...
Bitu DmaChannel::Read(Bitu want, uint8_t *buffer) {
  ...
  /* ISA devices cannot cycle DMA if the controller has masked the channel! Fix
   * your code! */
  if (masked) {
    LOG(LOG_DMACONTROL,
        LOG_WARN)("BUG: Attempted DMA channel read while channel masked");
    return 0;
  }
  /* You cannot read a DMA channel configured for writing (to memory) */
  if (transfer_mode != DMAT_READ) {
    LOG(LOG_DMACONTROL,
        LOG_WARN)("BUG: Attempted DMA channel read when DMA channel is "
                  "configured by guest for writing (to memory)");
    char psp_name[9];
    DOS_MCB psp_mcb(dos.psp() - 1);
    psp_mcb.GetFileName(psp_name);
    if (strcmp(
            psp_name,
            "DIAGNOSE")) // Wengier: Hack for Creative DIAGNOSE.EXE tool for now
                         // until the DMA recording function is implemented
      return 0;
  }
  ...
  const uint32_t addrmask =
      0xFFFu >> DMA16; /* 16-bit ISA style DMA needs 0x7FFF, else 0xFFFF. Use
                          0x7FF/0xFFF (4KB) for simplicity reasons. */
  while (want > 0) {
    const uint32_t addr = curraddr & addrmask;
    const Bitu wrapdo =
        increment ?
                  /*inc*/ ((addrmask + 1u) - addr)
                  : /* how many transfer units until (end of 4KB page) + 1 */
            /*dec*/ (
                addr +
                1u); /* how many transfer units until (start of 4KB page) - 1 */
    ...
    if (increment) {
      assert((curraddr & (~addrmask)) ==
             ((curraddr + ((uint32_t)cando - 1u)) &
              (~addrmask))); // check our work, must not cross a 4KB boundary
      DMA_BlockRead4KB<DMA_INCREMENT>(pagebase, curraddr, buffer, cando, DMA16,
                                      DMA16_ADDRMASK);
      ...
      assert((curraddr & (~addrmask)) ==
             ((curraddr - ((uint32_t)cando - 1u)) &
              (~addrmask))); // check our work, must not cross a 4KB boundary
      DMA_BlockRead4KB<DMA_DECREMENT>(pagebase, curraddr, buffer, cando, DMA16,
                                      DMA16_ADDRMASK);
      curraddr -= (uint32_t)cando;
    }
    ...
    if (IS_PC98_ARCH) {
       ...
       *
       * The PC-98 port of Sim City 2000 needs this to properly play digitized
       * speech, especially "reticulating splines". */
    ...
    if (currcnt == 0xFFFF) {
      ...
      if (autoinit) {
        ...
        /* NTS: This is what the 8237 actually does: Sets TC and sets the mask
         * bit of the channel.
         *
         *      "8237A
         *      Table 1. Pin Description (Continued)
         *      Symbol Type Name and Function
         *      EOP I/O END OF PROCESS: End of Process is an active low
         * bidirectional signal. Information concerning the completion of DMA
         * services is available at the bidirectional EOP pin. The 8237A allows
         * an external signal to terminate an active DMA service. This is
         ...
         *      which is output through the EOP line. The reception of EOP,
         * either internal or external, will cause the 8237A to terminate the
         * service, reset the request, and, if Autoinitialize is enabled, to
         * write the base registers to the current registers of that channel.
         * The mask bit and TC bit in the status word will be set for the
         * currently active channel by EOP unless the channel is programmed for
         ...
         * channel 1 occurs. EOP should be tied high with a pull-up resistor if
         * it is not used to prevent erroneous end of process inputs."
         *
...
Bitu DmaChannel::Write(Bitu want, uint8_t *buffer) {
  ...
  if (masked) {
    LOG(LOG_DMACONTROL,
        LOG_WARN)("BUG: Attempted DMA channel write while channel masked");
    return 0;
  }
  /* You cannot write a DMA channel configured for reading (from memory) */
  if (transfer_mode != DMAT_WRITE) {
    LOG(LOG_DMACONTROL,
        LOG_WARN)("BUG: Attempted DMA channel write when DMA channel is "
                  "configured by guest for reading (from memory)");
    return 0;
  }
  ...
  while (want > 0) {
    ...
    if (increment) {
      assert((curraddr & (~addrmask)) ==
             ((curraddr + ((uint32_t)cando - 1u)) &
              (~addrmask))); // check our work, must not cross a 4KB boundary
      DMA_BlockWrite4KB<DMA_INCREMENT>(pagebase, curraddr, buffer, cando, DMA16,
                                       DMA16_ADDRMASK);
      curraddr += (uint32_t)cando;
    } else {
      assert((curraddr & (~addrmask)) ==
             ((curraddr - ((uint32_t)cando - 1u)) &
              (~addrmask))); // check our work, must not cross a 4KB boundary
      DMA_BlockWrite4KB<DMA_DECREMENT>(pagebase, curraddr, buffer, cando, DMA16,
                                       DMA16_ADDRMASK);
      curraddr -= (uint32_t)cando;
    }
...
void DMA_SetWrapping(uint32_t wrap) { dma_wrapping = wrap; }
...
void DMA_Destroy(Section * /*sec*/) { DMA_FreeControllers(); }
...
void DMA_Reset(Section * /*sec*/) {
  ...
  /* NTS: parsing on reset means a reboot of the VM (and possibly other
   * conditions) can permit the user to change DMA emulation settings and have
   * them take effect on VM reboot. */
  enable_2nd_dma = section->Get_bool("enable 2nd dma controller");
  enable_1st_dma =
      enable_2nd_dma || section->Get_bool("enable 1st dma controller");
  enable_dma_extra_page_registers =
      section->Get_bool("enable dma extra page registers");
  dma_page_register_writeonly =
      section->Get_bool("dma page registers write-only");
  ...
  for (i = 0; i < 0x10; i++) {
    ...
    if (enable_1st_dma) {
      ...
      DmaControllers[0]->DMA_WriteHandler[i].Install(
          IS_PC98_ARCH ? ((i * 2u) + 1u) : i, DMA_Write_Port, mask);
      DmaControllers[0]->DMA_ReadHandler[i].Install(
          IS_PC98_ARCH ? ((i * 2u) + 1u) : i, DMA_Read_Port, mask);
    }
    if (enable_2nd_dma) {
      assert(!IS_PC98_ARCH);
      /* install handler for second DMA controller ports */
      DmaControllers[1]->DMA_WriteHandler[i].Install(0xc0u + i * 2u,
                                                     DMA_Write_Port, mask);
      DmaControllers[1]->DMA_ReadHandler[i].Install(0xc0u + i * 2u,
                                                    DMA_Read_Port, mask);
    }
  }
  ...
  if (enable_1st_dma) {
    if (IS_PC98_ARCH) {
      ...
      for (i = 0; i < 5; i++) {
        DmaControllers[0]->DMA_WriteHandler[0x10 + i].Install(
            0x21 + (i * 2u), DMA_Write_Port, IO_MB, 1);
        DmaControllers[0]->DMA_ReadHandler[0x10 + i].Install(
            0x21 + (i * 2u), DMA_Read_Port, IO_MB, 1);
      }
    } else {
      /* install handlers for ports 0x81-0x83 (on the first DMA controller) */
      DmaControllers[0]->DMA_WriteHandler[0x10].Install(0x80, DMA_Write_Port,
                                                        IO_MB, 8);
      DmaControllers[0]->DMA_ReadHandler[0x10].Install(0x80, DMA_Read_Port,
                                                       IO_MB, 8);
    }
  }
  ...
  if (enable_2nd_dma) {
    ...
    DmaControllers[1]->DMA_WriteHandler[0x10].Install(0x88, DMA_Write_Port,
                                                      IO_MB, 8);
    DmaControllers[1]->DMA_ReadHandler[0x10].Install(0x88, DMA_Read_Port, IO_MB,
                                                     8);
  }
...
const void *dma_state_callback_table[] = {NULL,
...
uint8_t POD_State_Find_DMA_Callback(Bitu addr) {
  ...
  for (int lcv = 0; lcv < size; lcv++) {
    if ((Bitu)dma_state_callback_table[lcv] == addr)
      return lcv;
  }
