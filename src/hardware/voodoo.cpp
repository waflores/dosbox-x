...
void voodoo_lfb_cb_init() {
  if (voodoo_lfb_cb == MEM_Callout_t_none) {
    ...
    if (voodoo_lfb_cb == MEM_Callout_t_none)
      E_Exit("Unable to allocate voodoo cb for LFB");
  }

  {
    ...
    if (voodoo_current_lfb != 0 && voodoo_pci_enabled) {
      ...
      cb->Install(voodoo_current_lfb >> 12UL,
                  MEMMASK_Combine(MEMMASK_FULL, MEMMASK_Range(VOODOO_PAGES)),
                  voodoo_lfb_memio_cb);
    } else {
...
public:
  VOODOO(Section *configuration) : Module_base(configuration) {
    ...
    if (needs_pci_device) {
      LOG(LOG_MISC, LOG_DEBUG)("Voodoo 3Dfx LFB at 0x%lx",
                               (unsigned long)voodoo_current_lfb);
...
bool VOODOO_PCI_CheckLFBPage(Bitu page) {
  if (voodoo_current_lfb != 0 && (page >= (voodoo_current_lfb >> 12)) &&
...
void VOODOO_Init() {
  ...
  AddVMEventFunction(VM_EVENT_POWERON,
                     AddVMEventFunctionFuncPair(VOODOO_OnPowerOn));
}
