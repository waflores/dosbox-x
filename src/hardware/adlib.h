...
public:
  ...
  virtual uint8_t ReadbackReg(uint32_t reg) {
    (void)reg;
    return 0xff;
  }
  // Sets the card back to emulation mode if it was in native mode
  // (ESFM-specific)
...
public:
  ...
  uint32_t lastUsed; // Ticks when adlib was last used to turn of mixing after a
                     // few second
  bool esfm_nativemode; // When using MODE_ESFM, whether the synth is in native
                        // mode or not - affects port mapping
