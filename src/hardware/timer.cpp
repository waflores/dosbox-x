...
// There is at least one game (PC-98 port of Thexder) that depends on PC-9801
// PIT 1 behavior where the counter cycles at all times whether or not the PC
// speaker is "on". This does not force the PC speaker output on (does not force
// an audible beep), it only forces the clock gate on and PIT 1 to cycle.
bool speaker_clock_lock_on = false;

static INLINE void BIN2BCD(uint16_t &val) {
  uint16_t temp = val % 10 + (((val / 10) % 10) << 4) +
                  (((val / 100) % 10) << 8) + (((val / 1000) % 10) << 12);
  val = temp;
}

static INLINE void BCD2BIN(uint16_t &val) {
  uint16_t temp = (val & 0x0f) + ((val >> 4) & 0x0f) * 10 +
                  ((val >> 8) & 0x0f) * 100 + ((val >> 12) & 0x0f) * 1000;
  val = temp;
}
...
struct PIT_Block {
  ...
  Bitu cntr =
      0; /* counter value written to 40h-42h as the interval. may take effect
            immediately (after port 43h) or after count expires */
  ...
  uint8_t read_state = 0;  /* 0=read MSB, switch to LSB, 1=LSB only, 2=MSB only,
                              3=read LSB, switch to MSB, latch next value */
  uint8_t write_state = 0; /* 0=write MSB, switch to LSB, 1=LSB only, 2=MSB
                              only, 3=write MSB, switch to LSB, accept value */
  uint8_t cycle_base = 0;

  bool bcd = false;           /* BCD mode */
  bool go_read_latch = false; /* reading should latch another value */
  bool new_mode =
      false; /* a new mode has been written to port 43h for this timer */
  bool counterstatus_set = false; /* set by status_latch(), when using 8254
                                     command to latch multiple counters */
  ...
  read_counter_result
      last_counter; /* what to return when gate == false (not counting) */
  ...
  void set_active_counter(Bitu new_cntr) {
    ...
    cntr_cur = new_cntr;
    if (mode == 2)
      delay = ((pic_tickindex_t)(1000ul * (cntr_cur - 1u))) /
              PIT_TICK_RATE; /* counts down to ONE, not ZERO */
    else
      delay = ((pic_tickindex_t)(1000ul * cntr_cur)) / PIT_TICK_RATE;

    /* Make sure the new counter value is returned if read back even if the gate
     * is off! Some games like to constantly reprogram PIT 2 with precise event
     * timey stuff and might shut the PC speaker PIT gate off during that time.
     *
     * Previous versions of this code failed to update the last_counter value
     * when the game wrote a new counter and the PIT gate was off, causing the
     * game to read back a stale counter value that was wrong.
     *
     * This fixes "Tony & Friends in Kellogg's Land" which does some rather
     * weird elaborate timing stuff with both PIT 0 (timer) and PIT 2 (PC
     * speaker but the output is off) to do its event timing and to modify the
     * VGA DAC mask mid-frame precisely to do that effect of making the bottom
     * half look like there is water. Ref:
     * [https://github.com/joncampbell123/dosbox-x/issues/4467] */
    last_counter.cycle = 0;
    last_counter.counter = cntr_cur;
  }
  void latch_next_counter(void) { set_active_counter(cntr); }
  ...
  void restart_counter_at(pic_tickindex_t t, uint16_t counter) {
    ...
    /* NTS: Remember, the counter counts DOWN, not up, so the delay is how long
     * it takes to get there */
    if (mode == 2)
      c_delay = ((pic_tickindex_t)(1000ull * (0xFFFFu - counter))) /
                PIT_TICK_RATE; /* counts down to ONE, not ZERO */
    else
      c_delay =
          ((pic_tickindex_t)(1000ull * (0x10000u - counter))) / PIT_TICK_RATE;
  ...
  void track_time(pic_tickindex_t t) {
     ...
     * Mode 1 will count down and stop. TODO: Writing a new counter without "new
     * mode" starts another countdown? */
  ...
  pic_tickindex_t reltime(void) const { return now - start; }
  ...
  bool get_output_from_counter(const read_counter_result &res) {
    // Timer:
    // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Timer/8253/8254%20programmable%20interval%20timer%20%281993%2d09%29%2epdf]
    // PIC:
    // [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Interrupt%20controller/8259/8259A%20Programmable%20Interrupt%20Controller%20%288259A%e2%88%958259A%2d2%29%20%281988%2d12%29%2epdf]
    //
    ...
    // Mode 3: Square wave
    //
    //    [new mode]  [begin count]   [count=0]       [count==0] counts down by
    //    2
    // _______________________________                 _________
    // XXX                            |_______________|
    //
    // Mode 4: Software Triggered Interrupt (looks like mode 2) but writing a
    // new counter takes effect right away

    switch (mode) {
    case 0:
      if (new_mode)
        return false;
      if (res.cycle != 0u /*index > delay*/)
        return true;
      else
        return false;
    case 1:
  ...
  read_counter_result
  read_counter(void) const { // This assumes you call track_time()
    if (!gate || new_mode ||
        (mode == 0 &&
         write_state ==
             0) /*mode 0 midway through 16-bit write also halts counter*/)
      return last_counter;
    ...
    case 0: /* Interrupt on Terminal Count */
    {
      ...
      if (bcd) {
        tmp = pic_tickfmod(index, ((pic_tickindex_t)(1000ul * 10000ul)) /
                                      PIT_TICK_RATE);
        ret.counter =
            (uint16_t)(((unsigned long)(cntr_cur -
                                        ((tmp * PIT_TICK_RATE) / 1000.0))) %
                       10000ul);
      } else {
        tmp = pic_tickfmod(index, ((pic_tickindex_t)(1000ul * 0x10000ul)) /
                                      PIT_TICK_RATE);
        ret.counter =
            (uint16_t)(((unsigned long)(cntr_cur -
                                        ((tmp * PIT_TICK_RATE) / 1000.0))) %
    ...
    case 2: /* Rate Generator */
      ret.counter =
          (uint16_t)(cntr_cur -
                     ((pic_tickfmod(index, delay) / delay) * cntr_cur));
      break;
    case 3: /* Square Wave Rate Generator */
    {
      ...
      if (tmp < 0) {
        fprintf(stderr, "tmp %.9f index %.9f delay %.9f now %.3f start %.3f\n",
                (double)tmp, (double)index, (double)delay, (double)now,
                (double)start);
        abort();
      }
      ...
      ret.counter = ((uint16_t)(cntr_cur - ((tmp * cntr_cur) / delay))) &
                    0xFFFEu; /* always even value */
    } break;
    default:
      break;
    }
...
static void PIT0_Event(Bitu /*val*/) {
  ...
  /* NTS: "Days of Thunder" leaves PIT 0 in mode 1 for some reason, which
   * triggers once and then stops. "start" does not advance in that mode. For
   * any non-periodic mode, this code would falsely detect an ever increasing
   * error and act badly. */
  if (pit[0].mode == 2 || pit[0].mode == 3) {
    ...
    if (finaldelay <= 0) { /* do not emit delays of zero */
      ...
      // LOG_MSG("PIT0_Event delay value negative (%.6fms), reset to 0",
      // (double)finaldelay);
...
uint32_t PIT0_GetAssignedCounter(void) { return (uint32_t)pit[0].cntr; }
...
void TIMER_IRQ0Poll(void) { counter_latch(0, false /*do not latch*/); }

pic_tickindex_t speaker_pit_delta(void) {
  unsigned int speaker_pit = IS_PC98_ARCH ? 1 : 2;
  return pic_tickfmod(pit[speaker_pit].now - pit[speaker_pit].start,
                      pit[speaker_pit].delay);
}
...
bool TIMER2_ClockGateEnabled(void) {
  /* PC speaker emulation should treat "new mode" as if the clock gate is
   * disabled. On real hardware, mode 3 does not cycle if you write a control
   * word but then do not write a counter value. */
  return !pit[IS_PC98_ARCH ? 1 : 2].new_mode;
}

// 8254 real hardware notes:
// True to Intel documentation, mode 0 halts the count upon writing the mode
// byte, and it also halts the count when given the first byte of a 16-bit count
// written to the timer. Reading the counter during this state does not start
// it. Writing the mode byte again does not start it. Twiddling the BCD bit back
// and forth (1996, Pyromania demoscene entry) does nothing.
//
// NOTES, 1996 demoscene entry "Pyromania":
// - At the start of the credits section (animation followed by credits), the
// demo will hang
//   and not continue. This is caused by misprogramming the 8254 PIT 0 in a way
//   that causes all IRQ 0 timer interrupts to cease. The demo appears to write
//   the mode 0 interval as expected, but then writes the first byte again
//   (that's a total of 3 bytes). That leaves the 8254 in the halted case
//   waiting for the other byte. The demo also uses a delay loop that uses a
//   combined call to INT 1Ah to read BIOS_TIMER and the high byte of the
//   counter value to wait a specific period of time. However in this state the
//   timer counter is not ticking, and the lack of IRQ 0 means the IRQ 0
//   interrupt handler is not there to increment BIOS_TIMER, therefore nothing
//   happens. Also noted is that the demo, for whatever reason, sets the mode
//   byte twice, once normally, and then again with the BCD mode set. Therefore,
//   emulation is correct that the demo will hang at that point and the hang is
//   not DOSBox-X's fault.
//
//   This is either the result of extremely sloppy code that happened to work on
//   the democoder's machine (non-Intel hardware that minimally implements a
//   8254?) or perhaps a race condition between the program and its own IRQ 0
//   interrupt.
//
//   Additional notes from testing: It is indeed some sort of race condition.
//   There is code to set PIT 0 to mode 2 counter 0 momentarily before going
//   back to mode 0. Interrupts are enabled at that point, which may be
//   interrupted at that key point.
//
//   Additional notes: Indeed, the problem is that a flaw in this code at the
//   time acted upon the demo's programming of PIT 0 mode 2 counter == 0
//   momentarily, then switching back to mode 0. When the demo wrote the first
//   byte of the 16-bit count, an interrupt occurred immediately, and the IRQ
//   wrote the count in the middle, then returned for the code to write the
//   second byte. This race condition confused the 8254. Fixing the code not to
//   do that fixes the demo crash.
static void write_latch(Bitu port, Bitu val, Bitu /*iolen*/) {
  ...
  if (p->write_state != 0) {
    ...
    } else if (p->write_latch == 1 &&
               (p->mode == 2 /*rate generator*/ ||
                p->mode ==
                    3 /*square wave, count by 2*/)) { /* counter==1 and mode==3
                                                         makes a low frequency
                                                         buzz (Paratrooper) */
      if (p->bcd == false)
    ...
    if (p->new_mode || p->mode == 0 || p->mode == 4) {
      ...
      if ((p->mode == 2 /*common IBM PC mode*/ ||
           p->mode == 3 /*common PC-98 mode*/) &&
          (counter == 0)) {
      ...
      if (p->mode == 0 || p->mode == 4) {
        /* Mode 0 is the only mode NOT to wait for the current counter to finish
         * if you write another counter value according to the Intel 8254
         * datasheet.
         *
         * For timer 0 (system timer) this is used by DoWhackaDo as a sort of
         * one-shot timer interrupt. For timer 2 (PC speaker) this is used to do
         * PWM "realsound" digitized speech in some games. */
      } else {
        // this debug message will help development trace down cases where
        // writing without a new mode would incorrectly restart the counter
        // instead of letting the current count complete before writing a new
        // one.
        LOG(LOG_PIT,
            LOG_NORMAL)("WARNING: Writing counter %u in mode %u without "
                        "writing port 43h not yet supported, will be handled "
                        "as if new mode and reset of the cycle",
                        (int)counter, (int)p->mode);
      }
    }
    ...
    case 0x00: /* Timer hooked to IRQ 0 */
      ...
      // please do not spam the log and console if a game is writing the SAME
      // counter value constantly, and do not spam the console if Mode 0 is used
      // because events are not consistent.
      if (p->cntr != old_cntr && p->mode != 0)
        LOG(LOG_PIT, LOG_NORMAL)("PIT 0 Timer at %.4f Hz mode %d",
                                 (double)(1000.0 / p->delay), p->mode);
...
static void write_p43(Bitu /*port*/, Bitu val, Bitu /*iolen*/) {
  ...
  case 2:
    if ((val & 0x30) == 0) {
      ...
      // NTS: Do not latch counter here. Some DOS games rely on latching a
      // counter,
      //      setting a control word, then reading the latched value later.
      //
      //      Calling reset_count_at() and latching here will cause Lemmings to
      //      program a very high interrupt rate and hang. Lemmings appears to
      //      do a mode == 0 timer count down from 0xFFFF, set the control word
      //      to latch PIT 0, then program mode == 2, then read back the latched
      //      value, then NOT the value and program it with mode == 2 into the
      //      counter of PIT 0 as the tick rate. reset_count_at() after latch
      //      causes Lemmings to read back 0xFFFC, which when NOT'd and written
      //      back, becomes 0x0003, and therefore a very high tick rate that
      //      prevents it from working.
      //
      //      Because of this latching across control words behavior, this code
      //      must not set the go_read_latch flag either.
      //
      //      Intel 8254 PIT datasheet:
      //
      //      "The selected Counter's output latch (OL) latches the count at the
      //      time the Counter Latch Command is received. The count is held in
      //      the latch until it is read by the CPU (or until the Counter is
      //      reprogrammed). The count is then unlatched automatically and the
      //      OL returns to "following" the counting element (CE)".
      //
      //      So the datasheet, and the behavior of Lemmings, confirms that the
      //      latch stays in effect even if you write the control word.

      // save the current count value to be re-used in undocumented newmode
      counter_latch(latch, false); /* update counter but do not affect held
                                      counter if latched by program */
      ...
      // When the mode byte is written, count stops. When the new count is
      // written, THEN call reset_count_at()
       ...
       * ) If the line goes to low => disable irq. Mode 0 starts with a low
       * line. (so always disable irq) Mode 2,3 start with a high line.
       * counter_output tells if the current counter is high or low
       * So actually a mode 3 timer enables and disables irq al the time. (not
       * handled) */

      /* Jon C: Oh yeah? Nobody abuses counter == 0 on IBM PC that way, but
       * there is a PC-98 game that relies on that behavior: Steel Gun Nyan! */
...
// FIXME: I am assuming that the "buzzer inhibit" on PC-98 controls the
// "trigger" pin
//        that either enables the PIT to count or stops it and resets the
//        counter. Verify this on real hardware (DOSLIB TPCRAPI6.EXE)
//
//        Note that on IBM PC/XT hardware, ports 60h-63h are the same PPI used
//        in the PC-98 systems, though wired differently. It is configured
//        (According to IBM).
//           - Port A (input)           Keyboard scan code / SW1 dip switches
//           (depends on port 61h bit 7)
//           - Port B (output)          Timer 2 gate speaker / Speaker data aka
//           output gate / ... / bit 7 set to clear keyboard and read SW1
//           - Port C (input)           I/O Read/Write Memory SW2 / Cassette
//           Data In / Timer Channel 2 Out / ...
...
//        Port 62h
//        - bit 5 PIT 2 counter output (read). The connection point lies BEFORE
//        the AND gate.
//            You will see the output toggle even if the speaker was muted by
//            clearing the output gate bit.
//
//        IBM PC/AT:
//
//        Port 61h
...
//        - bit 3 PIT 1 counter gate (there is no output gate). Setting the bit
//        inhibits the counter (and therefore PC speaker)
//        - On PC-9821, this bit controls the clock gate of PIT 1 and therefore
//        whether the PC speaker makes sound
//        - On PC-9801, the clock gate of PIT 1 is always on, and this bit
//        controls whether the PC speaker makes sound
//
//        IBM PC/XT/AT:
//
...
bool TIMER_GetOutput2() {
  unsigned int speaker_pit =
      IS_PC98_ARCH ? 1 : 2; // NTS: For completion sake, even though there is no
                            // readback bit on PC-98
...
void TIMER_BIOS_INIT_Configure() {
  ...
  int pcspeaker_pit =
      IS_PC98_ARCH
          ? 1
          : 2; /* IBM: PC speaker on output 2   PC-98: PC speaker on output 1 */
  ...
  if (IS_PC98_ARCH) {
     ...
     */
    phys_writeb(0x501, (phys_readb(0x501) & 0x7F) |
                           ((PIT_TICK_RATE == PIT_TICK_RATE_PC98_8MHZ)
                                ? 0x80
                                : 0x00) /* bit 7: 1=8MHz  0=5MHz/10MHz */
    );
     ...
     * There's a bit 6 port 434h that might gate it on some hardware, but that
     * doesn't seem to be the case on anything I have.
     *
     * NTS: If you run 8254.EXE from DOSLIB on PC-98 hardware and notice PIT 2
     * isn't cycling, try writing values to 75h and see if it begins counting
     * again. A PC-9821Lt2 laptop seems to have a bios that writes a mode byte
     * for it to 77h but then never writes to 75h, which leaves the timer idle.
     */
    pit[2].track_time(PIC_FullIndex());
    pit[2].set_gate(true);
  }
...
void TIMER_OnPowerOn(Section *) {
  ...
  // log
  LOG(LOG_MISC,
      LOG_DEBUG)("TIMER_OnPowerOn(): Reinitializing PIT timer emulation");
  ...
  if (IS_PC98_ARCH) {
    ...
    /* PC-98 has two different rates: 5/10MHz base or 8MHz base. Let the user
     * choose via dosbox-x.conf */
    pc98rate = pc98_section->Get_int("pc-98 timer master frequency");
    if (pc98rate > 6)
      pc98rate /= 2;
    if (pc98rate == 0)
      pc98rate = 5; /* Pick the most likely to work with DOS games (FIXME: This
                       is a GUESS!! Is this correct?) */
    else if (pc98rate < 5)
      pc98rate = 4;
    else
      pc98rate = 5;
...
void TIMER_OnEnterPC98_Phase2_UpdateBDA(void) {
  if (!cpu.pmode) {
    ...
    LOG_MSG("PC-98 warning: PIT timer change cannot be reflected to BIOS data "
            "area in protected/vm86 mode");
  }
}

void TIMER_Destroy(Section *) { PIC_RemoveEvents(PIT0_Event); }
...
void TIMER_Init() {
  ...
  AddVMEventFunction(VM_EVENT_POWERON,
                     AddVMEventFunctionFuncPair(TIMER_OnPowerOn));
}
