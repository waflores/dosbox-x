...
/*
        DOSBox implementation of a combined Yamaha YMF262 and Yamaha YM3812
   emulator. Enabling the opl3 bit will switch the emulator to stereo opl3
   output instead of regular mono opl2 Except for the table generation it's all
   integer math Can choose different types of generators, using muls and bigger
   tables, try different ones for slower platforms The generation was based on
   the MAME implementation but tried to have it use less memory and be faster in
   general MAME uses much bigger envelope tables and this will be the biggest
   cause of it sounding different at times

        //TODO Don't delay first operator 1 sample in opl3 mode
        //TODO Maybe not use class method pointers but a regular function
   pointers with operator as first parameter
        //TODO Fix panning for the Percussion channels, would any opl3 player
   use it and actually really change it though?
...
static const uint8_t AttackSamplesTable[13] = {69, 55, 46, 40, 35, 29, 23,
                                               20, 19, 15, 11, 10, 9};
...
static const int8_t VibratoTable[8] = {1 - 0x00, 0 - 0x00, 1 - 0x00, 30 - 0x00,
...
#if (DBOPL_WAVE == WAVE_HANDLER)
/*
        Generate the different waveforms out of the sine/exponential table using
   handlers
*/
...
/*inline*/ void Operator::UpdateAttenuation() {
  ...
  totalLevel =
      (int32_t)(tl << (ENV_BITS - 7)); // Total level goes 2 bits below max
...
/*INLINE*/ void Operator::Prepare(const Chip *chip) {
  currentLevel =
      (uint32_t)(totalLevel + (int32_t)(chip->tremoloValue & tremoloMask));
  waveCurrent = waveAdd;
...
/*INLINE*/ void Channel::GeneratePercussion(Chip *chip, int32_t *output) {
  ...
  uint32_t phaseBit =
      (((c2 & 0x88) ^ ((c2 << 5) & 0x80)) | ((c5 ^ (c5 << 2)) & 0x20)) ? 0x02
                                                                       : 0x00;
...
#define REGCHAN(_FUNC_)                                                        \
  ...
  if (ChanOffsetTable[index]) {                                                \
    Channel *regChan =                                                         \
        (Channel *)(((char *)this) + ChanOffsetTable[index] - 1);              \
...
void Chip::WriteReg(uint32_t reg, uint8_t val) {
  ...
  case 0x00 >> 4:
    if (reg == 0x01) {
      // When the chip is running in opl3 compatible mode, you can't actually
      // disable the waveforms
      ...
      // This isn't how the real card handles it but need to switch to stereo
      // generating handlers
      UpdateSynths();
    } else if (reg == 0x08) {
      reg08 = val;
...
void Chip::Setup(uint32_t rate) {
  ...
  for (uint8_t i = 0; i < 76; i++) {
    ...
    linearRates[i] = (uint32_t)(scale * (EnvelopeIncreaseTable[index]
  ...
  for (uint8_t i = 0; i < 62; i++) {
    ...
    int32_t originalAmount =
        (int32_t)((uint32_t)((AttackSamplesTable[index] << shift) / scale));

    int32_t guessAdd =
        (int32_t)((uint32_t)(scale * (EnvelopeIncreaseTable[index]
                                      << (RATE_SH - shift - 3))));
    ...
    for (uint32_t passes = 0; passes < 16; passes++) {
      ...
      // Below our target
      if (diff < 0) {
        // Always add one here for rounding, an overshoot will get corrected by
        // another pass decreasing
        guessAdd++;
      }
    }
...
void InitTables(void) {
  ...
  for (int i = 0; i < 512; i++) {
    SinTable[i] = (int16_t)(0.5 - log10(sin((i + 0.5) * (PI / 512.0))) /
                                      log10(2.0) * 256);
  }
#endif
#if (DBOPL_WAVE == WAVE_TABLEMUL)
  ...
  for (int i = 0; i < 512; i++) {
    WaveTable[0x0200 + i] =
        (int16_t)(0.5 -
                  log10(sin((i + 0.5) * (PI / 512.0))) / log10(2.0) * 256);
  ...
  for (Bitu i = 0; i < 64; i++) {
    ...
    Bitu opNum = (i % 8) / 3;
    OpOffsetTable[i] =
        ChanOffsetTable[chNum] + (uint16_t)(opNum * sizeof(DBOPL::Operator));
  }
#if 0
	DBOPL::Chip* chip = 0;
...
void Handler::WriteReg(uint32_t addr, uint8_t val) { chip.WriteReg(addr, val); }
...
void Handler::SaveState(std::ostream &stream) {
  ...
  for (int lcv1 = 0; lcv1 < 18; lcv1++) {
    for (int lcv2 = 0; lcv2 < 2; lcv2++) {
      ...
      wavebase_idx[lcv1][lcv2] =
          (Bitu)chip.chan[lcv1].op[lcv2].waveBase - (Bitu)&WaveTable;
    }

    synthhandler_idx[lcv1] = 0xff;
    if (0) {
    } else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3FMFM>)
      synthhandler_idx[lcv1] = 0x00;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3AMFM>)
      synthhandler_idx[lcv1] = 0x01;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3FMAM>)
      synthhandler_idx[lcv1] = 0x02;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3AMAM>)
      synthhandler_idx[lcv1] = 0x03;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3AM>)
      synthhandler_idx[lcv1] = 0x04;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm3FM>)
      synthhandler_idx[lcv1] = 0x05;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm2AM>)
      synthhandler_idx[lcv1] = 0x06;
    else if (chip.chan[lcv1].synthHandler == &Channel::BlockTemplate<sm2FM>)
      synthhandler_idx[lcv1] = 0x07;
    else if (chip.chan[lcv1].synthHandler ==
             &Channel::BlockTemplate<sm3Percussion>)
      synthhandler_idx[lcv1] = 0x08;
    else if (chip.chan[lcv1].synthHandler ==
             &Channel::BlockTemplate<sm2Percussion>)
      synthhandler_idx[lcv1] = 0x09;
  }
...
void Handler::LoadState(std::istream &stream) {
  ...
  for (int lcv1 = 0; lcv1 < 18; lcv1++) {
    for (int lcv2 = 0; lcv2 < 2; lcv2++) {
      chip.chan[lcv1].op[lcv2].volHandler =
          VolumeHandlerTable[volhandler_idx[lcv1][lcv2]];

      chip.chan[lcv1].op[lcv2].waveBase =
          (int16_t *)((Bitu)wavebase_idx[lcv1][lcv2] + (Bitu)&WaveTable);
    }

    switch (synthhandler_idx[lcv1]) {
    case 0x00:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3FMFM>;
      break;
    case 0x01:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3AMFM>;
      break;
    case 0x02:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3FMAM>;
      break;
    case 0x03:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3AMAM>;
      break;
    case 0x04:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3AM>;
      break;
    case 0x05:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3FM>;
      break;
    case 0x06:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm2AM>;
      break;
    case 0x07:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm2FM>;
      break;
    case 0x08:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm3Percussion>;
      break;
    case 0x09:
      chip.chan[lcv1].synthHandler = &Channel::BlockTemplate<sm2Percussion>;
      break;
    }
  }
}
