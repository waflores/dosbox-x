...
#endif
  IDEEventPack(const unsigned int interface, const unsigned int device)
      : v(pack(interface, device)) {}
  IDEEventPack(const unsigned int nv) : v(nv) {}

  static inline constexpr unsigned int pack(const unsigned int interface,
                                            const unsigned int device) {
    return (interface << 1u) | device;
  }

  inline constexpr unsigned int get(void) const { return v; }

  inline constexpr unsigned int interface(void) const { return (v >> 1u); }

  inline constexpr unsigned int device(void) const { return (v & 1u); }

  inline void set(const unsigned int pk) { v = pk; }
...
public:
  ...
  uint16_t feature, count,
      lba[3]; /* feature = BASE+1  count = BASE+2   lba[3] = BASE+3,+4,+5 */
  uint8_t command, drivehead,
      status; /* command/status = BASE+7  drivehead = BASE+6 */
  ...
  /* feature: 0x1F1 (Word 00h in ATA specs)
   ...
   drivehead: 0x1F6 (copy of last value written)
     command: 0x1F7 (Word 05h)
      status: 0x1F7 (value read back to IDE controller, including busy and drive
  ready bits as well as error status)
  ...
  controller's drivehead value. it is only updated on write, and not returned on
  read.

  "allow_writing" if set allows the DOS program/OS to write the registers. It is
  clear during command execution, obviously, so the state of the device is not
  confused while executing the command.
...
public:
  ...
  double ide_spinup_delay; /* time it takes to spin the hard disk motor up to
                              speed */
  double
      ide_spindown_delay; /* time it takes for hard disk motor to spin down */
...
public:
  ...
  virtual ~IDEDevice();
  virtual void
  host_reset_begin(); /* IDE controller -> upon setting bit 2 of alt (0x3F6) */
  virtual void host_reset_complete(); /* IDE controller -> upon setting bit 2 of
                                         alt (0x3F6) */
  ...
  virtual Bitu
  data_read(Bitu iolen); /* read from 1F0h data port from IDE device */
  virtual void
  data_write(Bitu v, Bitu iolen); /* write to 1F0h data port to IDE device */
...
public:
  ...
  virtual Bitu
  data_read(Bitu iolen) override; /* read from 1F0h data port from IDE device */
  virtual void
  data_write(Bitu v,
             Bitu iolen) override; /* write to 1F0h data port to IDE device */
...
public:
  ...
  virtual void prepare_read(Bitu offset, Bitu size);
  virtual void prepare_write(Bitu offset, Bitu size);
  virtual void set_sense(unsigned char SK, unsigned char ASC = 0,
                         unsigned char ASCQ = 0, unsigned int len = 0);
...
public:
  ...
  Bitu host_maximum_byte_count; /* host maximum byte count during PACKET
                                   transfer */
...
public:
  int IRQ;
  bool int13fakeio; /* on certain INT 13h calls, force IDE state as if BIOS had
                       carried them out */
  bool int13fakev86io; /* on certain INT 13h calls in virtual 8086 mode, trigger
                          fake CPU I/O traps */
  bool enable_pio32;   /* enable 32-bit PIO (if disabled, attempts at 32-bit PIO
                          are handled as if two 16-bit I/O) */
  bool ignore_pio32;   /* if 32-bit PIO enabled, but ignored, writes do nothing,
...
private: // Sorry, IDE devices and external code don't get to force IDE IRQs
         // anymore
  void raise_irq();
  void lower_irq();
};

const char *ideslot[] = {"Primary",    "Secondary",   "Tertiary",
                         "Quaternary", "Quinternary", "Sexternary",
                         "Septernary", "Octernary"};
const char *master_slave[] = {"Master", "Slave"};

static IDEController *idecontroller[MAX_IDE_CONTROLLERS] = {
      ...
      for (unsigned int d = 0; d < 2; d++) {
        ...
        if (dev != NULL) {
          if (dev->type == IDE_TYPE_CDROM) {
            IDEATAPICDROMDevice *atapi =
                reinterpret_cast<IDEATAPICDROMDevice *>(dev);
...
static void IDE_ATAPI_SpinDown(Bitu pk /*which IDE device*/) {
  ...
  if (dev->type == IDE_TYPE_HDD) {
    ...
    if (atapi->loading_mode == LOAD_DISC_READIED ||
...
/* returns "true" if command should proceed as normal, "false" if sense data was
 * set and command should not proceed. this function helps to enforce virtual
 * "spin up" and "ready" delays. */
bool IDEATAPICDROMDevice::common_spinup_response(bool trigger, bool wait) {
  const unsigned int pk =
      IDEEventPack(controller->interface_index, slave ? 1u : 0u).get();
  ...
  /* if the CD-ROM drive has mounted the empty drive, then ALWAYS return Medium
   * Not Present */
  ...
  case LOAD_DISC_LOADING:
    if (has_changed && !wait /*if command will block until LOADING complete*/) {
      set_sense(/*SK=*/0x02, /*ASC=*/0x04,
                /*ASCQ=*/0x01); /* Medium is becoming available */
      return false;
    }
    break;
  case LOAD_DISC_READIED:
    ...
    if (has_changed) {
      if (trigger)
        has_changed = false;
      set_sense(/*SK=*/0x02, /*ASC=*/0x28,
                /*ASCQ=*/0x00); /* Medium is ready (has changed) */
      return false;
    }
    break;
  case LOAD_IDLE:
...
void IDEATAPICDROMDevice::read_subchannel() {
  ...
  *write++ = 0x00;
  ...
  prepare_read(0, MIN((unsigned int)(write - sector),
...
void IDEATAPICDROMDevice::mechanism_status() {
  ...
  *write++ =
      (0 << 5u) /* mechanism state=idle=0 (TODO) */ | (0x00) /*door open=0*/;
...
void IDEATAPICDROMDevice::mode_sense() {
  ...
  *write++ = 0x00; /*                                      (LSB) */
  /* NTS: MMC-3 Table 342 says that BLOCK DESCRIPTOR LENGTH is zero, where it
   * would be 8 for legacy units */

  /* Mode Page Format MMC-3 Table 341 */
  *write++ = PAGE; /* PS|reserved|Page Code */
  *write++ = 0x00; /* Page Length (n - 1) ... Length in bytes of the mode
                      parameters that follow */
  switch (PAGE) {
  case 0x01:         /* Read error recovery MMC-3 Section 6.3.4 table 344 */
    *write++ = 0x00; /* +2 Error recovery Parameter
    ...
    *write++ =
        0x00; /* +10 Recovery Time Limit (should be zero)         (MSB) */
    *write++ =
        0x00; /* +11                                              (LSB) */
    break;
  ...
  case 0x2A: /* CD-ROM mechanical status MMC-3 Section 6.3.11 table 361 */
     ...
     */
    *write++ =
        0x07; /* +2 Reserved       |Reserved     |DVD-RAM read |DVD-R read
                 |DVD-ROM read  |   Method 2    | CD-RW read   | CD-R read */
    *write++ =
        0x00; /* +3 Reserved       |Reserved     |DVD-RAM write|DVD-R write  |
                 Reserved   |  Test Write   | CD-RW write  | CD-R write */
    *write++ =
        0x71; /* +4 Buffer Underrun|Multisession |Mode 2 form 2|Mode 2 form
                 1|Digital Port 2|Digital Port 1 |  Composite   | Audio play */
    *write++ =
        0xFF; /* +5 Read code bar  |UPC          |ISRC         |C2 Pointers |R-W
                 deintcorr | R-W supported |CDDA accurate |CDDA support */
    *write++ = 0x2F; /* +6 Loading mechanism type                     |Reserved
                        |Eject         |Prevent Jumper |Lock state    |Lock */
                     /*      0 (0x00) = Caddy
                      *      1 (0x20) = Tray
                      *      2 (0x40) = Popup
                      *      3 (0x60) = Reserved
                      ...
                      *      6 (0xC0) = Reserved
                      *      6 (0xE0) = Reserved */
    *write++ = 0x03; /* +7 Reserved       |Reserved     |R-W in leadin|Side chg
                        cap |S/W slot sel  |Changer disc pr|Sep. ch. mute |Sep.
                        volume levels */
    ...
    *write++ = x;
    ...
    x = 176 *
        8; /* +14 current read speed selected in kB: 8X  (obsolete in MMC-3) */
    *write++ = x >> 8;
    *write++ = x;

    *write++ = 0;    /* +16 Reserved */
    *write++ = 0x00; /* +17 Reserved | Reserved | Length | Length | LSBF | RCK |
                        BCK | Reserved */
...
void IDEATAPICDROMDevice::play_audio_msf() {
  ...
  if (atapi_cmd[3] == 0xFF && atapi_cmd[4] == 0xFF && atapi_cmd[5] == 0xFF)
    ...
    start_lba =
        (atapi_cmd[3] * 60u * 75u) + (atapi_cmd[4] * 75u) + atapi_cmd[5];
  ...
  if (start_lba == end_lba) {
    /* The play length field specifies the number of contiguous logical blocks
     * that shall be played. A play length of zero indicates that no audio
     * operation shall occur. This condition is not an error. */
    /* TODO: How do we interpret that? Does that mean audio playback stops? Or
     * does it mean we do nothing to the state of audio playback? */
    sector_total = 0;
    return;
  }
...
void IDEATAPICDROMDevice::play_audio10() {
  ...
  play_length = ((uint16_t)atapi_cmd[7] << 8) + ((uint16_t)atapi_cmd[8] << 0);
...
void IDEATAPICDROMDevice::read_toc() {
   ...
   *      is longer than it's allocation length. But here's the thing: some
   * MS-DOS CD-ROM drivers will ask for the TOC but only provide enough room for
   * one entry (OAKCDROM.SYS) and if we signal more data than its buffer, it
   * will reject our response and render the CD-ROM drive inaccessible. So to
   * make this emulation work, we have to cut our response short to the driver's
   *      allocation length */
  unsigned int AllocationLength =
      ((unsigned int)atapi_cmd[7] << 8) + atapi_cmd[8];
  unsigned char Format = atapi_cmd[2] & 0xF;
  unsigned char Track = atapi_cmd[6];
  bool TIME = !!(atapi_cmd[1] & 2);
  unsigned char *write;
    ...
    *write++ = 0x00;                 /* entry+3 RESERVED */
    ...
    if (TIME) {
      ...
      *write++ = start.fr;
      ...
      uint32_t sec =
          (start.min * 60u * 75u) + (start.sec * 75u) + start.fr - 150u;
      *write++ = (unsigned char)(sec >> 24u);
      *write++ = (unsigned char)(sec >> 16u);
      *write++ = (unsigned char)(sec >> 8u);
      *write++ = (unsigned char)(sec >> 0u);
      ...
      *write++ = 0x00;
      if (TIME) {
        ...
        *write++ = leadOut.fr;
        ...
        uint32_t sec =
            (leadOut.min * 60u * 75u) + (leadOut.sec * 75u) + leadOut.fr - 150u;
        *write++ = (unsigned char)(sec >> 24u);
        *write++ = (unsigned char)(sec >> 16u);
        *write++ = (unsigned char)(sec >> 8u);
        *write++ = (unsigned char)(sec >> 0u);
  ...
  prepare_read(0, MIN(MIN((unsigned int)(write - sector),
                          (unsigned int)host_maximum_byte_count),
                      AllocationLength));
}
...
void IDEATAPICDROMDevice::on_atapi_busy_time() {
  ...
  switch (atapi_cmd[0]) {
  case 0x03: /* REQUEST SENSE */
    prepare_read(0, MIN((unsigned int)sense_length,
                        (unsigned int)host_maximum_byte_count));
    ...
    /* ATAPI protocol also says we write back into LBA 23:8 what we're going to
  ...
  case 0x25: /* READ CAPACITY */ {
    ...
    prepare_read(0,
                 MIN((unsigned int)8, (unsigned int)host_maximum_byte_count));
    sector[0] = sec >> 24u;
    sector[1] = sec >> 16u;
    sector[2] = sec >> 8u;
    sector[3] = sec & 0xFF;
  ...
  case 0x2B: /* SEEK */
    ...
    /* Except... Windows 95's CD player expects the SEEK command to interrupt CD
     * audio playback. In fact it depends on it to the exclusion of commands
     * explicitly standardized to... you know... stop or pause playback. Oh
     * Microsoft, you twits... */
    {
      CDROM_Interface *cdrom = getMSCDEXDrive();
      if (cdrom) {
  ...
  case 0x12: /* INQUIRY */
    ...
    prepare_read(0,
                 MIN((unsigned int)36, (unsigned int)host_maximum_byte_count));
  ...
  case 0xA8: /* READ(12) */
    if (TransferLength == 0) {
      /* this is legal. the SCSI MMC standards say so.
         and apparently, MSCDEX.EXE issues READ(10) commands with transfer
         length == 0 to test the drive, so we have to emulate this */
      feature = 0x00;
      ...
      CDROM_Interface *cdrom = getMSCDEXDrive();
      bool res =
          (cdrom != NULL ? cdrom->ReadSectorsHost(/*buffer*/ sector, false, LBA,
                                                  TransferLength)
                         : false);
      if (res) {
        prepare_read(0, MIN((unsigned int)(TransferLength * 2048),
                            (unsigned int)host_maximum_byte_count));
        ...
        if (cdrom->class_id == CDROM_Interface::INTERFACE_TYPE::ID_FAKE) {
          set_sense(/*SK=*/0x02,
                    /*ASC=*/0x3A); /* Not Ready: Medium Not Present */
          LOG_MSG("ATAPI: Rejecting read %lu sectors at %lu as Medium Not "
                  "Present\n",
                  (unsigned long)TransferLength, (unsigned long)LBA);
        } else {
          set_sense(/*SK=*/0x03,
                    /*ASC=*/0x11); /* Medium Error: Unrecovered Read Error */
  ...
  case 0xBE: /* READ CD */
    if (TransferLength == 0 || TransferSectorSize == 0) {
      ...
      /* TODO: Implement remaining types and better comply with the standard
       * (i.e. validate track type, don't allow reading across data and CDDA
       * tracks) */
      if (TransferSectorType == 2 /*Mode 1*/ ||
          TransferSectorType == 4 /*Mode 2 form 1*/) {
        if (TransferSectorSize == 2048) {
          if (cdrom && cdrom->ReadSectorsHost(/*buffer*/ sector, false,
                                              (unsigned long)LBA,
          ...
          LOG(LOG_MISC, LOG_DEBUG)("ATAPI READ CD warning: Unsupported sector "
                                   "type %u byte9 %x size %u",
                                   (unsigned int)TransferSectorType,
                                   (unsigned int)TransferReadCD9,
                                   (unsigned int)TransferSectorSize);
        }
      } else if (TransferSectorType == 0 /*raw*/ ||
                 TransferSectorType == 1 /*CDDA*/) {
        if (TransferSectorSize == 2352) {
          if (cdrom && cdrom->ReadSectorsHost(/*buffer*/ sector, true,
                                              (unsigned long)LBA,
                                              (unsigned long)TransferLength)) {
            res = true;
            prepare_read(0, MIN((unsigned int)(TransferLength * 2352),
                                (unsigned int)host_maximum_byte_count));
          }
        } else {
          LOG(LOG_MISC, LOG_DEBUG)("ATAPI READ CD warning: Unsupported sector "
...
void IDEATAPICDROMDevice::set_sense(unsigned char SK, unsigned char ASC,
                                    unsigned char ASCQ, unsigned int len) {
  if (len < 18)
    len = 18;
  memset(sense, 0, len);
  sense_length = len;
...
IDEATAPICDROMDevice::IDEATAPICDROMDevice(IDEController *c,
                                         unsigned char drive_index, bool _slave)
    : IDEDevice(c, _slave) {
  ...
  /* FIXME: Spinup/down times should be dosbox-x.conf configurable, if the
  ...
  spindown_timeout =
      10000; /* drive spins down automatically after 10 seconds */
  ...
  if (IS_PC98_ARCH) {
     ...
     * It will NOT talk to this emulation unless we report ourself as an NEC
     * CD-ROM drive. Yes, really. It will issue an INQUIRY and then do a REP
     * CMPSW on the first 20 bytes to make sure it says "NEC CD-ROM DRIVE". */
    id_mmc_vendor_id = "NEC";
    id_mmc_product_id = "CD-ROM DRIVE";
    id_mmc_product_rev = "FAKE";
    /* Second thing: That particular driver seems to assume the CD-ROM drive
     * only transfers one CD-ROM sector per DRQ (like many CD-ROM drives in the
     * 1990s did because they had small transfer buffers).
     *
     * We HAVE to emulate this behavior or else the PC-98 driver will stall and
     * error out if any attempt is made to read more than one sector at a time!
     */
...
void IDEATAPICDROMDevice::on_mode_select_io_complete() {
  ...
  scan = sector + 8;
  fence =
      sector + MIN((unsigned int)sector_total, (unsigned int)AllocationLength);
  ...
  while ((scan + 2) < fence) {
    ...
    if ((scan + LEN) > fence) {
      LOG_MSG("ATAPI MODE SELECT warning, page_0 length extends %u bytes past "
              "buffer\n",
...
void IDEATAPICDROMDevice::atapi_io_completion() {
  ...
  if (count !=
      0x03) { /* the command was expecting data. now it can act on it */
    switch (atapi_cmd[0]) {
    case 0x28: /*READ(10)*/
    case 0xA8: /*READ(12)*/
      /* How much does the guest want to transfer? */
      /* NTS: This is required to work correctly with the ide-cd driver in the
       * Linux kernel. The Linux kernel appears to negotiate a 32KB or 64KB
       * transfer size here even if the total transfer from a CD READ would
       * exceed that size, and it expects the full result in those DRQ block
       * transfer sizes. */
      sector_total = (lba[1] & 0xFF) | ((lba[2] & 0xFF) << 8);

      /* FIXME: We actually should NOT be capping the transfer length, but
         instead should be breaking the larger transfer into smaller DRQ block
         transfers like most IDE ATAPI drives do. Writing the test IDE code
         taught me that if you go to most drives and request a transfer length
         of 0xFFFE the drive will happily set itself up to transfer that many
         sectors in one IDE command! */
      /* NTS: In case you're wondering, it's legal to issue READ(10) with
         transfer length == 0. MSCDEX.EXE does it when starting up, for example
       */
      ...
      if (TransferLength != 0) {
        //                  LOG_MSG("ATAPI CD READ LBA=%x xfer=%x xferrem=%x
        //                  continued",(unsigned int)LBA,(unsigned
        //                  int)TransferLength,(unsigned
        //                  int)TransferLengthRemaining);
        ...
        //                  LOG_MSG("ATAPI CD READ LBA=%x xfer=%x xferrem=%x
        //                  transfer complete",(unsigned int)LBA,(unsigned
        //                  int)TransferLength,(unsigned
        //                  int)TransferLengthRemaining);
      }
      break;
    case 0xBE: /*READ CD*/
      ...
      if (TransferLength != 0) {
        LOG_MSG("ATAPI CD READ CD LBA=%x xfer=%x xferrem=%x continued",
                (unsigned int)LBA, (unsigned int)TransferLength,
                (unsigned int)TransferLengthRemaining);
        ...
        LOG_MSG("ATAPI CD READ CD LBA=%x xfer=%x xferrem=%x transfer complete",
                (unsigned int)LBA, (unsigned int)TransferLength,
                (unsigned int)TransferLengthRemaining);
      }
      break;
...
void IDEATAPICDROMDevice::io_completion() {
  ...
  default: /* most commands: signal drive ready, return to ready state */
    /* NTS: Some MS-DOS CD-ROM drivers will loop endlessly if we never set
       "drive seek complete" because they like to hit the device with DEVICE
       RESET (08h) whether or not it's a hard disk or CD-ROM drive */
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRIVE_SEEK_COMPLETE;
    state = IDE_DEV_READY;
    allow_writing = true;
    count = 0x03; /* no more data (command/data=1, input/output=1) */
...
void IDEATADevice::io_completion() {
  ...
  case 0x20: /* READ SECTOR */
    ...
    /* NTS: Remember that count == 0 means the host wanted to transfer 256
     * sectors */
    progress_count++;
    if ((count & 0xFF) == 1) {
      /* end of the transfer */
  ...
  case 0x30: /* WRITE SECTOR */
    /* this is where the drive has accepted the sector, lowers DRQ, and begins
    ...
    PIC_AddEvent(
        IDE_DelayedCommand,
        ((progress_count == 0 && !faked_command) ? 0.1 : 0.00001) /*ms*/, pk);
    break;
...
Bitu IDEATAPICDROMDevice::data_read(Bitu iolen) {
  ...
  /* NTS: Some MS-DOS CD-ROM drivers like OAKCDROM.SYS use byte-wide I/O for the
...
static const uint16_t ReadCDTransferSectorSizeTable
    [5 /*SectorType-1*/][0x20 /*READ CD byte 9 >> 3*/] = {
        /* Sector type 0: Any
         * Sector type 1: CDDA */
        {
...
/* TODO: Your code should also be paying attention to the "transfer length"
   field in many of the commands here. Right now it doesn't matter. */
void IDEATAPICDROMDevice::atapi_cmd_completion() {
  ...
  case 0x00: /* TEST UNIT READY */
    ...
    state = IDE_DEV_READY;
    feature =
        ((sense[2] & 0xF) << 4) | ((sense[2] & 0xF) ? 0x04 /*abort*/ : 0x00);
    status =
  ...
  case 0xBE: /* READ CD */
    if (common_spinup_response(/*spin up*/ true, /*wait*/ true)) {
      ...
      /* How much does the guest want to transfer? */
      /* NTS: This is required to work correctly with Windows NT 4.0. Windows NT
       * will emit a READ CD command at startup with transfer length == 0. If an
       * error is returned, NT ignores the CD-ROM drive entirely and acts like
       * it's in a perpetual error state. */
      sector_total = (lba[1] & 0xFF) | ((lba[2] & 0xFF) << 8);
      LBA = ((Bitu)atapi_cmd[2] << 24UL) | ((Bitu)atapi_cmd[3] << 16UL) |
            ((Bitu)atapi_cmd[4] << 8UL) | ((Bitu)atapi_cmd[5] << 0UL);
      TransferLength = ((Bitu)atapi_cmd[6] << 16UL) |
                       ((Bitu)atapi_cmd[7] << 8UL) | ((Bitu)atapi_cmd[8]);

      /* Sector size? */
      TransferSectorType =
          (atapi_cmd[1] >> 2) & 7u; /* RESERVED=[7:5] ExpectedSectorType=[4:2]
                                       RESERVED=[1:1] RELOAD=[0:0] */
      TransferReadCD9 =
          atapi_cmd[9]; /* SYNC=[7:7] HeaderCodes=[6:5] UserData=[4:4]
                           EDCECC=[3:3] ErrorField=[2:1] RESERVED=[0:0] */

      if (TransferSectorType <= 5) /* Treat unspecified sector type == 0 the
                                      same as CDDA with regard to sector size */
        TransferSectorSize =
            ReadCDTransferSectorSizeTable[(TransferSectorType > 0)
                                              ? (TransferSectorType - 1)
                                              : 0][TransferReadCD9 >> 3u];
      else
  ...
  case 0xA8: /* READ(12) */
    if (common_spinup_response(/*spin up*/ true, /*wait*/ true)) {
      ...
      /* FIXME: MSCDEX.EXE appears to test the drive by issuing READ(10) with
         transfer length == 0. This is all well and good but our response seems
         to cause a temporary 2-3 second pause for each attempt. Why? */
      LBA = ((Bitu)atapi_cmd[2] << 24UL) | ((Bitu)atapi_cmd[3] << 16UL) |
            ((Bitu)atapi_cmd[4] << 8UL) | ((Bitu)atapi_cmd[5] << 0UL);
      TransferLength = ((Bitu)atapi_cmd[6] << 24UL) |
                       ((Bitu)atapi_cmd[7] << 16UL) |
                       ((Bitu)atapi_cmd[8] << 8UL) | ((Bitu)atapi_cmd[9]);
      ...
      //              LOG_MSG("ATAPI CD READ LBA=%x xfer=%x
      //              xferrem=%x",(unsigned int)LBA,(unsigned
      //              int)TransferLength,(unsigned int)TransferLengthRemaining);
  ...
  case 0x28: /* READ(10) */
    if (common_spinup_response(/*spin up*/ true, /*wait*/ true)) {
      ...
      LBA = ((Bitu)atapi_cmd[2] << 24UL) | ((Bitu)atapi_cmd[3] << 16UL) |
            ((Bitu)atapi_cmd[4] << 8UL) | ((Bitu)atapi_cmd[5] << 0UL);
      TransferLength = ((Bitu)atapi_cmd[7] << 8) | ((Bitu)atapi_cmd[8]);
  ...
  default:
    ...
    LOG_MSG("Unknown ATAPI command %02x %02x %02x %02x %02x %02x %02x %02x "
            "%02x %02x %02x %02x\n",
            atapi_cmd[0], atapi_cmd[1], atapi_cmd[2], atapi_cmd[3],
            atapi_cmd[4], atapi_cmd[5], atapi_cmd[6], atapi_cmd[7],
            atapi_cmd[8], atapi_cmd[9], atapi_cmd[10], atapi_cmd[11]);
...
void IDEATAPICDROMDevice::data_write(Bitu v, Bitu iolen) {
  if (state == IDE_DEV_ATAPI_PACKET_COMMAND) {
    ...
    if (state != IDE_DEV_DATA_WRITE) {
      LOG_MSG("ide atapi warning: data write when device not in data_write "
              "state\n");
      return;
    }
    if (!(status & IDE_STATUS_DRQ)) {
...
void IDEATADevice::data_write(Bitu v, Bitu iolen) {
  if (state != IDE_DEV_DATA_WRITE) {
    LOG_MSG(
        "ide ata warning: data write when device not in data_write state\n");
    return;
  }
  if (!(status & IDE_STATUS_DRQ)) {
...
void IDEATAPICDROMDevice::generate_identify_device() {
  ...
  host_writew(sector + (0 * 2),
              0x85C0U); /* ATAPI device, command set #5 (what the fuck does that
                           mean?), removable, */
  ...
  host_writew(sector + (68 * 2), /* PIO cycle time */
              0x0078UL);
  host_writew(sector + (80 * 2), 0x007E); /* major version number. Here we say
                                             we support ATA-1 through ATA-8 */
  host_writew(sector + (81 * 2), 0x0022); /* minor version */
  host_writew(
      sector + (82 * 2),
      0x4008); /* command set: NOP, DEVICE RESET[XXXXX], POWER MANAGEMENT */
...
void IDEATADevice::generate_identify_device() {
  ...
  host_writew(sector + (4 * 2),
              phys_sects * 512);      /* unformatted bytes per track */
  ...
  host_writew(sector + (20 * 2),
              1); /* ATA-1: single-ported single sector buffer */
  ...
  if (multiple_sector_max != 0)
    host_writew(
        sector + (47 * 2),
        0x80 | multiple_sector_max); /* <- READ/WRITE MULTIPLE MAX SECTORS */

  host_writew(sector + (48 * 2),
              0x0000); /* :0  0=we do not support doubleword (32-bit) PIO */
  host_writew(sector + (49 * 2),
              0x0A00); /* :13 0=Standby timer values managed by device */
  ...
  host_writew(sector + (51 * 2),
              0x00F0); /* PIO data transfer cycle timing mode */
  host_writew(sector + (52 * 2),
              0x00F0); /* DMA data transfer cycle timing mode */
  host_writew(sector + (53 * 2),
              0x0007); /* :2  1=the fields in word 88 are valid */
  ...
  if (multiple_sector_count != 0)
    host_writew(
        sector + (59 * 2),
        0x0100 |
            multiple_sector_count); /* :8  multiple sector setting is valid */
  /* 7:0 current setting for number of log. sectors per DRQ of READ/WRITE
   * MULTIPLE */

  host_writed(sector + (60 * 2),
              ptotal); /* total user addressable sectors (LBA) */
  host_writew(sector + (62 * 2), 0x0000); /* FIXME: ??? */
  host_writew(sector + (63 * 2),
              0x0000); /* :10 0=Multiword DMA mode 2 not selected */
  /* TODO: Basically, we don't do DMA. Fill out this comment */
  host_writew(sector + (64 * 2),
              0x0003); /* 7:0 PIO modes supported (FIXME ???) */
  ...
  host_writew(
      sector + (82 * 2),
      0x4208); /* command set: NOP, DEVICE RESET[XXXXX], POWER MANAGEMENT */
...
IDEATADevice::IDEATADevice(IDEController *c, unsigned char disk_index,
                           bool _slave)
    : IDEDevice(c, _slave), id_serial("8086"), id_firmware_rev("8086"),
      id_model("DOSBox-X IDE disk"), bios_disk_index(disk_index) {
  sector_i = sector_total = 0;
...
void IDEATADevice::update_from_biosdisk() {
  ...
  if (heads > 16) {
    ...
    /* One additional correction: The disk image is probably using BIOS-style
       geometry translation (such as C/H/S 1024/64/63) which is impossible given
       that the IDE standard only allows up to 16 heads. So we have to translate
       the geometry. */
    while (heads > 16 && (heads & 1) == 0)
      heads >>= 1U;

    /* If we can't divide the heads down, then pick a LBA-like mapping that is
     * good enough. Note that if what we pick does not evenly map to the INT 13h
     * geometry, and the partition contained within is not an LBA type
     * FAT16/FAT32 partition, then Windows 95's IDE driver will ignore this
     * device and fall back to using INT 13h. For user convenience we will print
     * a warning to reminder the user of exactly that. */
    if (heads > 16) {
      sects = 63;
      heads = 16;
    }
    ...
    {
      uint64_t tmp = ((uint64_t)dsk->diskSizeK << (uint64_t)10) /
                     (uint64_t)dsk->sector_size;
      cyls = (tmp + (uint64_t)(sects * heads) - (uint64_t)1) /
             ((uint64_t)(sects * heads));
    }

    LOG_MSG("Mapping BIOS DISK C/H/S %u/%u/%u as IDE %u/%u/%u\n",
            (unsigned int)dsk->cylinders, (unsigned int)dsk->heads,
            (unsigned int)dsk->sectors, (unsigned int)cyls, (unsigned int)heads,
            (unsigned int)sects);
  }
...
bool IDE_controller_occupied(
    signed char index,
    bool slave) { // Return true if specified slot is occupied
...
/* drive_index = drive letter 0...A to 25...Z */
void IDE_CDROM_Attach(signed char index, bool slave,
                      unsigned char drive_index) {
  IDEController *c;
  ...
  if (c == NULL) {
    LOG_MSG("IDE: WARNING: IDE %s controller not available. Check setting or "
  ...
  if (c->device[slave ? 1 : 0] != NULL) {
    LOG_MSG("IDE: WARNING: IDE controller %s %s already occupied, specify "
            "another slot.",
            ideslot[index], master_slave[slave ? 1 : 0]);
    return;
  }
  ...
  if (dev == NULL) {
    LOG_MSG("IMGMOUNT: Failed to allocate CD-ROM drive %c to IDE %s %s",
            drive_index + 'A', ideslot[index], master_slave[slave ? 1 : 0]);
    return;
  }
  dev->update_from_cdrom();
  c->device[slave ? 1 : 0] = (IDEDevice *)dev;
  LOG_MSG("IMGMOUNT: CD-ROM image mounted to drive %c (IDE %s %s)",
          drive_index + 'A', ideslot[index], master_slave[slave ? 1 : 0]);
}
...
void IDE_CDROM_Detach_Ret(signed char &indexret, bool &slaveret,
                          unsigned char drive_index) {
  indexret = -1;
  for (int index = 0; index < MAX_IDE_CONTROLLERS; index++) {
    IDEController *c = idecontroller[index];
...
void IDE_Hard_Disk_Attach(signed char index, bool slave, unsigned char bios_disk_index /*not INT13h, the index into DOSBox-X's BIOS drive emulation*/) {
  ...
  c->device[slave ? 1 : 0] = (IDEDevice *)dev;
  LOG_MSG("IMGMOUNT: HDD image mounted to drive no. %d (IDE %s %s)",
          bios_disk_index, ideslot[index], master_slave[slave ? 1 : 0]);
}
  ...
  for (int index = 0; index < MAX_IDE_CONTROLLERS; index++) {
    ...
    if (c)
      for (int slave = 0; slave < 2; slave++) {
        // IDEATADevice *dev = dynamic_cast<IDEATADevice*>(c->device[slave]);
        // UNUSED
        info += "IDE position " + std::to_string(index + 1) +
                (slave ? 's' : 'm') + ": ";
...
void Get_IDECD_drives(std::vector<int> &v) {
  for (int index = 0; index < MAX_IDE_CONTROLLERS; index++) {
    ...
    if (c)
      for (int slave = 0; slave < 2; slave++) {
        IDEATAPICDROMDevice *d =
            dynamic_cast<IDEATAPICDROMDevice *>(c->device[slave]);
        if (d)
          v.push_back(d->drive_index);
      }
  }
}
...
static Bitu IDE_SelfIO_In(IDEController *ide, Bitu port, Bitu len) {
  if (ide->int13fakev86io && IDE_CPU_Is_Vm86()) {
     ...
     * Windows 95 uses V86 traps to help "autodetect" what IDE drive and port
     * the BIOS uses on INT 13h so that its internal IDE driver can take over,
     * which is the whole reason for this hack. */
...
static void IDE_SelfIO_Out(IDEController *ide, Bitu port, Bitu val, Bitu len) {
  if (ide->int13fakev86io && IDE_CPU_Is_Vm86()) {
    /* Trigger I/O in virtual 8086 mode, where the OS can trap it and act on it.
     * Windows 95 uses V86 traps to help "autodetect" what IDE drive and port
     * the BIOS uses on INT 13h so that it's internal IDE driver can take over,
     * which is the whole reason for this hack. */
  ...
  for (idx = 0; idx < MAX_IDE_CONTROLLERS; idx++) {
    ...
    for (ms = 0; ms < 2; ms++) {
      ...
      if (dev->type == IDE_TYPE_HDD) {
        ...
        if (ata->bios_disk_index == disk && ata->phys_heads != 0 &&
            ata->phys_sects != 0 && ata->phys_cyls != 0) {
...
void IDE_EmuINT13DiskReadByBIOS_LBA(unsigned char disk, uint64_t lba) {
  ...
  for (idx = 0; idx < MAX_IDE_CONTROLLERS; idx++) {
    ...
    /* TODO: Print a warning message if the IDE controller is busy
    ...
    for (ms = 0; ms < 2; ms++) {
      ...
      /* TODO: Print a warning message if the IDE device is busy or in the
       * middle of a command */
      ...
      if (dev->type == IDE_TYPE_HDD) {
        ...
        if ((ata->bios_disk_index - 2) == (disk - 0x80)) {
          ...
          if (ide->int13fakev86io && vm86) {
            ...
            /* for brevity assume it worked. we're here to bullshit Windows 95
             * after all */
            for (unsigned int i = 0; i < 256; i++)
              IDE_SelfIO_In(ide, ide->base_io + 0u,
                            2); /* 16-bit IDE data read */
            ...
            if (ide->IRQ >= 8)
              IDE_SelfIO_Out(ide, 0xA0, 0x60u + (unsigned int)ide->IRQ - 8u,
                             1); /* specific EOI */
            else
              IDE_SelfIO_Out(ide, 0x20, 0x60u + (unsigned int)ide->IRQ,
                             1); /* specific EOI */
             ...
             * Windows 3.11 Windows for Workgroups 32-bit disk access (WDCTRL),
             * because the driver "tests" the controller by issuing INT 13h
             * calls then reading back IDE registers to see if they match the
             * C/H/S it requested */
            dev->feature = 0x00; /* clear error (WDCTRL test phase 5/C/13) */
            dev->count =
                0x00; /* clear sector count (WDCTRL test phase 6/D/14) */
            dev->lba[0] = lba & 0xFF; /* leave sector number the same (WDCTRL
                                         test phase 7/E/15) */
            dev->lba[1] =
                (lba >> 8u) &
                0xFF; /* leave cylinder the same (WDCTRL test phase 8/F/16) */
            dev->lba[2] = (lba >> 16u) & 0xFF; /* ...ditto */
            dev->drivehead = 0xE0u | (ms << 4u) |
                             (lba >> 24u); /* drive head and master/slave
                                              (WDCTRL test phase 9/10/17) */
            dev->status = IDE_STATUS_DRIVE_READY |
                          IDE_STATUS_DRIVE_SEEK_COMPLETE; /* status (WDCTRL test
                                                             phase A/11/18) */
            dev->allow_writing = true;
            static bool vm86_warned = false;

            if (vm86 && !vm86_warned) {
              LOG_MSG("IDE warning: INT 13h extensions read from virtual 8086 "
                      "mode.\n");
              LOG_MSG("             If using Windows 95 OSR2, please set "
                      "int13fakev86io=true for proper 32-bit disk access\n");
              vm86_warned = true;
            }
          }
...
/* this is called after INT 13h AH=0x02 READ DISK to change IDE state to
 * simulate the BIOS in action. this is needed for old "32-bit disk drivers"
 * like WDCTRL in Windows 3.11 Windows for Workgroups, which issues INT 13h to
 * read-test and then reads IDE registers to see if they match expectations */
      ...
      if (dev->type == IDE_TYPE_HDD) {
        ...
        if ((ata->bios_disk_index - 2) == (disk - 0x80)) {
          ...
          if (ata->sects != ata->phys_sects || ata->heads != ata->phys_heads ||
            ...
            LOG_MSG("INT 13 WARNING: I/O issued on drive attached to IDE "
                    "emulation with changed logical geometry!\n");
          ...
          if (ata->geo_translate) {
            ...
            if (dsk == NULL)
              return;
            lba = (head * dsk->sectors) + (cyl * dsk->sectors * dsk->heads) +
                  sect - 1;
          ...
          if (ide->int13fakev86io && vm86) {
            ...
            IDE_SelfIO_In(ide, ide->base_io + 7u,
                          1); /* dum de dum reading status */
            IDE_SelfIO_Out(ide, ide->base_io + 6u, (ms << 4u) + 0xA0u + head,
                           1); /* drive and head */
            IDE_SelfIO_In(ide, ide->base_io + 7u,
                          1); /* dum de dum reading status */
            IDE_SelfIO_Out(ide, ide->base_io + 2u, 0x01, 1); /* sector count */
             ...
             * after all */
            for (unsigned int i = 0; i < 256; i++)
              IDE_SelfIO_In(ide, ide->base_io + 0,
                            2); /* 16-bit IDE data read */
            ...
            dev->count =
                0x00; /* clear sector count (WDCTRL test phase 6/D/14) */
            dev->lba[0] = sect; /* leave sector number the same (WDCTRL test
                                   phase 7/E/15) */
            dev->lba[1] =
                cyl; /* leave cylinder the same (WDCTRL test phase 8/F/16) */
            dev->lba[2] = cyl >> 8u; /* ...ditto */
            dev->drivehead =
                0xA0u | (ms << 4u) | head; /* drive head and master/slave
                                              (WDCTRL test phase 9/10/17) */
            dev->status = IDE_STATUS_DRIVE_READY |
                          IDE_STATUS_DRIVE_SEEK_COMPLETE; /* status (WDCTRL test
                                                             phase A/11/18) */
            ...
            if (vm86 && !vm86_warned) {
              ...
              LOG_MSG("             If using Windows 95, please set "
                      "int13fakev86io=true for proper 32-bit disk access\n");
              vm86_warned = true;
            }
          }
...
/* this is called by src/ints/bios_disk.cpp whenever INT 13h AH=0x00 is called
 * on a hard disk. this gives us a chance to update IDE state as if the BIOS had
 * gone through with a full disk reset as requested. */
      ...
      if (dev->type == IDE_TYPE_HDD) {
        ...
        if ((ata->bios_disk_index - 2) == (disk - 0x80)) {
          LOG_MSG("IDE %d%c reset by BIOS disk 0x%02x\n",
                  (unsigned int)(idx + 1), ms ? 's' : 'm', (unsigned int)disk);
...
static void IDE_DelayedCommand(Bitu pk /*which IDE device*/) {
  ...
  if (dev->type == IDE_TYPE_HDD) {
    IDEATADevice *ata = (IDEATADevice *)dev;
    uint32_t sectorn =
        0; /* FIXME: expand to uint64_t when adding LBA48 emulation */
    ...
    case 0x30: /* WRITE SECTOR */
      ...
      if (drivehead_is_lba(ata->drivehead)) {
        ...
        if (ata->lba[0] == 0) {
          ...
          dev->raise_irq();
          return;
        } else if ((unsigned int)(ata->drivehead & 0xFu) >=
          ...
          LOG_MSG("C/H/S %u/%u/%u out of bounds %u/%u/%u\n",
                  ...
                  (unsigned int)(ata->lba[0]), (unsigned int)ata->cyls,
                  (unsigned int)ata->heads, (unsigned int)ata->sects);
          ata->abort_error();
          dev->raise_irq();
          return;
        }
      ...
      /* NTS: the way this command works is that the drive writes ONE sector,
         then fires the IRQ and lets the host read it, then reads another
         sector, fires the IRQ, etc. One IRQ signal per sector. We emulate that
         here by adding another event to trigger this call unless the sector
         count has just dwindled to zero, then we let it stop. */
      if ((ata->count & 0xFF) == 1) {
        /* end of the transfer */
        ata->count = 0;
    ...
    case 0x20: /* READ SECTOR */
      ...
      if (drivehead_is_lba(ata->drivehead)) {
        /* LBA */
        sectorn = (((unsigned int)ata->drivehead & 0xFu) << 24u) |
        ...
        if (ata->lba[0] == 0) {
          ...
          dev->raise_irq();
          return;
        } else if ((unsigned int)(ata->drivehead & 0xF) >=
                       (unsigned int)ata->heads ||
                   (unsigned int)ata->lba[0] > (unsigned int)ata->sects ||
                   (unsigned int)(ata->lba[1] |
                                  ((unsigned int)ata->lba[2] << 8u)) >=
                       (unsigned int)ata->cyls) {
          LOG_MSG(
              "C/H/S %u/%u/%u out of bounds %u/%u/%u\n",
              (unsigned int)(ata->lba[1] | ((unsigned int)ata->lba[2] << 8u)),
              (unsigned int)(ata->drivehead & 0xF), (unsigned int)ata->lba[0],
              (unsigned int)ata->cyls, (unsigned int)ata->heads,
      ...
      /* NTS: the way this command works is that the drive reads ONE sector,
         then fires the IRQ and lets the host read it, then reads another
         sector, fires the IRQ, etc. One IRQ signal per sector. We emulate that
         here by adding another event to trigger this call unless the sector
         count has just dwindled to zero, then we let it stop. */
      /* NTS: The sector advance + count decrement is done in the I/O completion
       * function */
      ...
      if (drivehead_is_lba(ata->drivehead)) {
        ...
        if (ata->lba[0] == 0) {
          ...
          LOG_MSG(
              "C/H/S %u/%u/%u out of bounds %u/%u/%u\n",
              (unsigned int)(ata->lba[1] | ((unsigned int)ata->lba[2] << 8u)),
              (unsigned int)(ata->drivehead & 0xFu), (unsigned int)ata->lba[0],
              (unsigned int)ata->cyls, (unsigned int)ata->heads,
              (unsigned int)ata->sects);
          ata->abort_error();
          dev->raise_irq();
        ...
        sectorn =
            (((unsigned int)ata->drivehead & 0xFu) * ata->sects) +
            (((unsigned int)ata->lba[1] | ((unsigned int)ata->lba[2] << 8u)) *
             ata->sects * ata->heads) +
            ((unsigned int)ata->lba[0] - 1u);
      }
    ...
    case 0xC4: /* READ MULTIPLE */
      ...
      for (unsigned int cc = 0;
           cc < MIN((Bitu)ata->multiple_sector_count, (Bitu)sectcount); cc++) {
        /* it would be great if the disk object had a "read multiple sectors"
         * member function */
        if (disk->Read_AbsoluteSector(sectorn + cc, ata->sector + (cc * 512)) !=
            0) {
          LOG_MSG("ATA read failed\n");
          ata->abort_error();
          dev->raise_irq();
      ...
      dev->status = IDE_STATUS_DRQ | IDE_STATUS_DRIVE_READY |
                    IDE_STATUS_DRIVE_SEEK_COMPLETE;
      ata->prepare_read(
          0, 512 * MIN((Bitu)ata->multiple_sector_count, (Bitu)sectcount));
      dev->raise_irq();
      break;
    ...
    case 0xC5: /* WRITE MULTIPLE */
      ...
      if (drivehead_is_lba(ata->drivehead)) {
        /* LBA */
        sectorn = (((unsigned int)ata->drivehead & 0xF) << 24) |
                  (unsigned int)ata->lba[0] | ((unsigned int)ata->lba[1] << 8) |
        ...
        sectorn =
            ((unsigned int)(ata->drivehead & 0xF) * ata->sects) +
            (((unsigned int)ata->lba[1] | ((unsigned int)ata->lba[2] << 8)) *
             ata->sects * ata->heads) +
            ((unsigned int)ata->lba[0] - 1);
      }
      ...
      for (unsigned int cc = 0;
           cc < MIN((Bitu)ata->multiple_sector_count, (Bitu)sectcount); cc++) {
        /* it would be great if the disk object had a "write multiple sectors"
         * member function */
...
void IDEController::check_device_irq() {
  ...
  if (dev)
    sig = dev->irq_signal && interrupt_enable;
...
void IDEDevice::host_reset_complete() {
  ...
  /* device passed, according to ATA standard regarding result of diagnostics
   * test. */
  /* NTS: The Linux kernel WILL NOT talk to this IDE emulation unless we signal
   * success through the feature/error register. If we don't respond this way,
   * then according to the source code, it considers what we provide a possibly
   * phantom drive. See Linux kernel 6.1.29 drivers/ata/libata-sff.c, functions
   * ata_sff_softreset, ata_sff_dev_classify. Another side effect of failing to
   * set feature is that the Linux kernel will talk to our hard drive emulation
   * though with a warning about diagnostics failure, but will then completely
   * ignore any ATAPI CD-ROM emulation we provide.
   *
   *      Windows XP never issues a reset or DIAGNOSTIC to the drive and doesn't
   * care, apparently. */
  feature = 0x01;
...
bool IDEDevice::command_interruption_ok(uint8_t cmd) {
  ...
  if (state != IDE_DEV_READY && state != IDE_DEV_BUSY && cmd == 0x08) {
    LOG_MSG("Device reset while another (%02x) is in progress (state=%u). "
            "Aborting current command to begin another\n",
            command, state);
    abort_silent();
    return true;
  }
  ...
  if (state != IDE_DEV_READY) {
    LOG_MSG("Command %02x written while another (%02x) is in progress "
            "(state=%u). Aborting current command\n",
            cmd, command, state);
    abort_error();
    return false;
  }
...
void IDEATAPICDROMDevice::writecommand(uint8_t cmd) {
  ...
  case 0x08: /* DEVICE RESET */
    ...
    lba[2] = 0xEB;
    /* NTS: Testing suggests that ATAPI devices do NOT trigger an IRQ on receipt
     * of this command */
    allow_writing = true;
    break;
  case 0x20: /* READ SECTOR */
  ...
  case 0xA0: /* ATAPI PACKET */
    if (feature & 1) {
      ...
      host_maximum_byte_count =
          ((unsigned int)lba[2] << 8) +
          (unsigned int)lba[1]; /* LBA field bits 23:8 are byte count */
  ...
  case 0xA1: /* IDENTIFY PACKET DEVICE */
    ...
    status = IDE_STATUS_BUSY;
    PIC_RemoveSpecificEvents(IDE_DelayedCommand, pk);
    PIC_AddEvent(IDE_DelayedCommand,
                 (faked_command ? 0.000001 : ide_identify_command_delay), pk);
    break;
  case 0xEC: /* IDENTIFY DEVICE */
    /* "devices that implement the PACKET command set shall post command aborted
       and place PACKET command feature set in the appropriate fields". We have
       to do this. Unlike OAKCDROM.SYS Windows 95 appears to autodetect IDE
       devices by what they do when they're sent command 0xEC out of the
       blue---Microsoft didn't write their IDE drivers to use command 0x08
       DEVICE RESET. */
    abort_normal();
    status = IDE_STATUS_ERROR | IDE_STATUS_DRIVE_READY;
    drivehead &= 0x30;
  ...
  case 0xEF: /* SET FEATURES */
    if (feature == 0x66 /*Disable reverting to power on defaults*/ ||
        feature == 0xCC /*Enable reverting to power on defaults*/ || feature == 0x03 /*Set transfer mode according to sector count register (required by Linux kernel)*/) {
      /* ignore */
      status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRIVE_SEEK_COMPLETE;
      state = IDE_DEV_READY;
    } else {
      LOG_MSG("SET FEATURES %02xh SC=%02x SN=%02x CL=%02x CH=%02x", feature,
              count, lba[0], lba[1], lba[2]);
      abort_error();
    }
    allow_writing = true;
    raise_irq();
...
void IDEATADevice::writecommand(uint8_t cmd) {
  ...
  /* FIXME: OAKCDROM.SYS is sending the hard disk command 0xA0 (ATAPI packet)
   * for some reason. Why? */
  ...
  case 0x08: /* DEVICE RESET */
    ...
    /* NTS: Testing suggests that ATA hard drives DO fire an IRQ at this stage.
            In fact, Windows 95 won't detect hard drives that don't fire an IRQ
       in desponse */
    raise_irq();
    allow_writing = true;
    break;
  ...
  case 0x1F:
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRIVE_SEEK_COMPLETE;
    /* "if the command is executed in CHS mode, then ... sector number register
     * shall be 1. if executed in LAB mode, then ... sector number register
     * shall be 0" */
    if (drivehead_is_lba(drivehead))
      lba[0] = 0x00;
    else
  ...
  case 0x20: /* READ SECTOR */
    ...
    break;
  case 0x30: /* WRITE SECTOR */
    /* the drive does NOT signal an interrupt. it sets DRQ and waits for a
     * sector to be transferred to it before executing the command */
    progress_count = 0;
    state = IDE_DEV_DATA_WRITE;
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRQ;
    prepare_write(0, 512);
  ...
  case 0x91: /* INITIALIZE DEVICE PARAMETERS */
    if ((unsigned int)count != (unsigned int)sects ||
        (unsigned int)((drivehead & 0xF) + 1) != (unsigned int)heads) {
      if (count == 0) {
        LOG_MSG("IDE warning: OS attempted to change geometry to invalid H/S "
                "%u/%u\n",
        ...
        /* the OS is changing logical disk geometry, so update our head/sector
         * count (needed for Windows ME) */
        LOG_MSG("IDE warning: OS is changing logical geometry from C/H/S "
                "%u/%u/%u to logical H/S %u/%u/%u\n",
                (int)cyls, (int)heads, (int)sects, (int)ncyls,
                (int)((drivehead & 0xF) + 1), (int)count);
        LOG_MSG("             Compatibility issues may occur if the OS tries "
                "to use INT 13 at the same time!\n");
    ...
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRIVE_SEEK_COMPLETE;
    allow_writing = true;
    raise_irq(); // NTS: The Linux kernel will pause for up to 30 seconds
                 // waiting for this command to issue an IRQ if we don't do this
    break;
  case 0xC4: /* READ MULTIPLE */
    progress_count = 0;
    state = IDE_DEV_BUSY;
  ...
  case 0xC5: /* WRITE MULTIPLE */
    ...
    state = IDE_DEV_DATA_WRITE;
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRQ;
    prepare_write(0UL, 512UL * MIN((unsigned long)multiple_sector_count,
  ...
  case 0xA1: /* IDENTIFY PACKET DEVICE */
     ...
     * Most MS-DOS drivers and Windows 95 like to issue both IDENTIFY ATA and
     * IDENTIFY ATAPI commands. I also gather from some contributors on the
     * github comments that people think our "Unknown IDE/ATA command" error
     * message is part of some other error in the emulation. Rather than put up
     * with that, we'll just silently abort the command with an error. */
    abort_normal();
    status = IDE_STATUS_ERROR | IDE_STATUS_DRIVE_READY |
             IDE_STATUS_DRIVE_SEEK_COMPLETE | 0x20 /*write fault*/;
    drivehead &= 0x30;
    count = 0x01;
    lba[0] = 0x01;
    feature = 0x04; /* abort */
  ...
  case 0xE7: /* FLUSH CACHE */
    /* NTS: Windows 2000 and Windows XP like this command a lot. They REALLY
     * REALLY like to issue this command a lot, especially during the install
     * phase. This is here to avoid filling your log file with many repetitions
     * of "Unknown IDE/ATA command E7" */
    status = IDE_STATUS_DRIVE_READY | IDE_STATUS_DRIVE_SEEK_COMPLETE;
    state = IDE_DEV_READY;
    allow_writing = true;
    raise_irq();
...
/* the hard disk or CD-ROM class override of this member is responsible for
   checking the head value and clamping within range if C/H/S mode is selected
 */
void IDEDevice::select(uint8_t ndh, bool switched_to) {
  (void)switched_to; // UNUSED
  (void)ndh;         // UNUSED
  /* NTS: I thought there was some delay between selecting a drive and sending a
     command. Apparently I was wrong. */
  if (allow_writing)
    drivehead = ndh;
  //  status =
  //  (!asleep)?(IDE_STATUS_DRIVE_READY|IDE_STATUS_DRIVE_SEEK_COMPLETE):0;
  //  allow_writing = !asleep;
  //  state = IDE_DEV_READY;
}

IDEController::IDEController(Section *configuration, unsigned char index)
  ...
  if (i >= 0x100 && i <= 0x3FF)
    base_io = (unsigned int)(i & ~7);

  i = section->Get_hex("altio");
  if (i >= 0x100 && i <= 0x3FF)
    alt_io = (unsigned int)(i & ~1);

  if (IS_PC98_ARCH) {
    /* PC-98 evidently maps all IDE controllers to one set of I/O ports and
     * allows "bank switching" between them through I/O ports 430h-435h. This is
     * also why the IDE emulation will NOT register I/O ports per controller in
     * PC-98 mode. If PC-98 supports IDE controllers outside this scheme, then
     * that shall be implemented later. */
    ...
    if (IRQ < 0 || alt_io == 0 || base_io == 0)
      LOG_MSG("WARNING: IDE interface %u: Insufficient resources assigned by "
              "dosbox-x.conf, and no appropriate default resources for this "
              "interface.",
              index);
  }
}
...
void IDEController::register_isapnp() {
  ...
  if (register_pnp && base_io > 0 && alt_io > 0) {
    ...
    const unsigned char h1[9] = {ISAPNP_SYSDEV_HEADER(
        ISAPNP_ID('P', 'N', 'P', 0x0, 0x6, 0x0,
                  0x0), /* PNP0600 Generic ESDI/IDE/ATA compatible hard disk
                           controller */
        ISAPNP_TYPE(0x01, 0x01,
                    0x00), /* type: Mass Storage Device / IDE / Generic */
        0x0001 | 0x0002)};
    ...
    if (alt_io == 0x3F6 && fdc_takes_port_3F7())
      tmp[i + 7] = 0x01; /* length 1 (so as not to conflict with floppy
                            controller at 0x3F7) */
    else
    ...
    if (IRQ > 0) {
      /* NTS: Even though the ISA Plug & Play standard documents a 2 byte
       * version (only the IRQ mask) and a 3 byte version (IRQ mask + additional
       * bitmask for IRQ edge/level), Microsoft Windows only supports the 2 byte
       * version.
       *
       *      Windows 2000 apparently does not like the 3 byte version and it's
       * IDE driver will fail to detect the IDE ATA drive it booted from.
       * Meaning, for some reason it will only probe the ATA drive like a CD-ROM
       * drive and then give up without ever checking for an ATA device. Weird.
       */
...
static void ide_altio_w(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (ide == NULL) {
    LOG_MSG("WARNING: port read from I/O port not registered to IDE, yet "
            "callback triggered\n");
    return;
  }
...
static void ide_baseio_w(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (dev) {
    if (dev->status & IDE_STATUS_BUSY) {
      if (port == 6 && ((val >> 4) & 1) == ide->select) {
        /* some MS-DOS drivers like ATAPICD.SYS are just very pedantic about
         * writing to port +6 to ensure the right drive is selected */
        return;
      } else {
        LOG_MSG("W-%03X %02X BUSY DROP [DEV]\n", (int)(port + ide->base_io),
                (int)val);
        return;
      }
    }
  ...
  if (port >= 1 && port <= 5 && dev && !dev->allow_writing) {
    LOG_MSG("IDE WARNING: Write to port %u val %02x when device not ready to "
            "accept writing\n",
            (unsigned int)port, (unsigned int)val);
  }
  ...
  case 0: /* 1F0 */
    if (dev)
      dev->data_write(val, iolen); /* <- TODO: what about 32-bit PIO modes? */
    break;
  case 1:                          /* 1F1 */
...
void IDE_Primary_Init(Section *sec) { IDE_Init(sec, 0); }

void IDE_Secondary_Init(Section *sec) { IDE_Init(sec, 1); }

void IDE_Tertiary_Init(Section *sec) { IDE_Init(sec, 2); }

void IDE_Quaternary_Init(Section *sec) { IDE_Init(sec, 3); }

void IDE_Quinternary_Init(Section *sec) { IDE_Init(sec, 4); }

void IDE_Sexternary_Init(Section *sec) { IDE_Init(sec, 5); }

void IDE_Septernary_Init(Section *sec) { IDE_Init(sec, 6); }

void IDE_Octernary_Init(Section *sec) { IDE_Init(sec, 7); }
...
void (*ide_inits[MAX_IDE_CONTROLLERS])(Section *) = {
    &IDE_Primary_Init,    &IDE_Secondary_Init,   &IDE_Tertiary_Init,
    &IDE_Quaternary_Init, &IDE_Quinternary_Init, &IDE_Sexternary_Init,
    &IDE_Septernary_Init, &IDE_Octernary_Init};

IO_ReadHandleObject PC98_ReadHandler[8], PC98_ReadHandlerAlt[2],
    PC98_ReadHandlerSel[3];
IO_WriteHandleObject PC98_WriteHandler[8], PC98_WriteHandlerAlt[2],
    PC98_WriteHandlerSel[3];
...
void IDE_OnReset(Section *sec) {
  if ((bootguest || (use_quick_reboot && !bootvm)) && bootdrive >= 0)
    return;
  (void)sec; // UNUSED

  for (size_t i = 0; i < MAX_IDE_CONTROLLERS; i++)
    ide_inits[i](control->GetSection(ide_names[i]));
