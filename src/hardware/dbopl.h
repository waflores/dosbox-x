...
// Use 8 handlers based on a small logarithmic wavetable and an exponential
// table for volume
...
typedef Channel *(DBOPL::Channel::*SynthHandler)(Chip *chip, uint32_t samples,
                                                 int32_t *output);
...
public:
  ...
  uint32_t chanData; // Frequency/octave and derived data coming from whatever
  ...
  uint8_t rateZero; // Bits for the different states of the envelope having no
                    // changes
...
struct Channel {
  ...
  // Change in the chandata, check for new values and if we have to forward to
  // operators
  ...
  template <bool opl3Mode> void GeneratePercussion(Chip *chip, int32_t *output);
...
struct Chip {
  // 18 channels with 2 operators each. Leave on top of struct for simpler
  // pointer math.
  Channel chan[18];
