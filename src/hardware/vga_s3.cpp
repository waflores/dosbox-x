...
void S3_UpdateXGAColorMode(void) {
  ...
  case S3_XGA_16BPP:
    ...
    break;
  case S3_XGA_8BPP:
    /* FIXME: 4/8bpp packed is controlled by the advanced function control
     * register (0x4AE8) bit 2 which is not yet emulated here */
    vga.s3.xga_color_mode = M_LIN8;
    if ((vga.s3.misc_control_2 >> 4) == 0xF /*hacked 4bpp*/)
      vga.s3.xga_color_mode = M_LIN4;
    break;
  }
}
...
void SVGA_S3_WriteCRTC(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0x31: /* CR31 Memory Configuration */
    ...
    vga.config.display_start =
        (vga.config.display_start & ~0x30000ul) | ((val & 0x30u) << 12ul);
    VGA_DetermineMode();
    VGA_SetupHandlers();
    break;
    /*
  ...
  case 0x4c: /* HGC start address high byte*/
    ...
    if ((((Bitu)vga.s3.hgc.startaddr) << 10) + ((64 * 64 * 2) / 8) >
        vga.mem.memsize) {
      vga.s3.hgc.startaddr &= 0xff; // put it back to some sane area;
                                    // if read back of this address is ever
                                    // implemented this needs to change
      LOG(LOG_VGAMISC,
          LOG_NORMAL)("VGA:S3:CRTC: HGC pattern address beyond video memory");
    }
    break;
  ...
  case 0x50: // Extended System Control 1
    ...
    case S3_XGA_1024:
      vga.s3.xga_screen_width = 1024;
      break;
    case S3_XGA_1152:
      vga.s3.xga_screen_width = 1152;
      break;
    case S3_XGA_640:
      vga.s3.xga_screen_width = 640;
      break;
    case S3_XGA_800:
      vga.s3.xga_screen_width = 800;
      break;
    case S3_XGA_1280:
      vga.s3.xga_screen_width = 1280;
      break;
    case S3_XGA_1600:
      vga.s3.xga_screen_width = 1600;
      break;
    default:
      vga.s3.xga_screen_width = 1024;
      break;
  ...
  case 0x55: /* Extended Video DAC Control */
    ...
    /*
       ...
       RAMDACs. If this field is 0, 3d4h index 43h bit 1 is active. 2   Enable
       General Input Port Read. If set DAC reads are disabled and the STRD
       strobe for reading the General Input Port is enabled for reading while
       DACRD is active, if clear DAC reads are enabled. 3   (928) Enable
       External SID Operation if set. If set video data is passed directly from
       the VRAMs to the DAC rather than through the VGA chip 4   Hardware Cursor
       MS/X11 Mode. If set the Hardware Cursor is in X11 mode, if clear in
       MS-Windows mode 5   (80x,928) Hardware Cursor External Operation Mode. If
       set the two bits of cursor data ,is output on the HC[0-1] pins for the
       video DAC The SENS pin becomes HC1 and the MID2 pin becomes HC0. 6   ??
  ...
  case 0x5e: /* Extended Vertical Overflow */
    vga.config.line_compare =
        (vga.config.line_compare & 0x3ffu) | ((uint8_t)val & 0x40u) << 4u;
  ...
  case 0x63: /* Extended Control Register CR63 */
    if (s3Card == S3_86C928 || s3Card == S3_Vision864 ||
        s3Card == S3_Vision868 || s3Card == S3_Vision964 ||
        s3Card == S3_Vision968)
      return; /* not mentioned in datasheet, does not exist */
    if (s3Card >= S3_ViRGE && ((val ^ vga.s3.reg_63) & 2u /*RST*/))
      SD3_Reset(!!(val & 2u));
    vga.s3.reg_63 = (uint8_t)val;
    break;
    /* S3 Vision864
     * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20Vision864%20Graphics%20Accelerator%20%281994%2d10%29%2epdf]
     *
     *      Datasheet does not mention this register, so it probably does not
     * exist.
     */
    /* S3 Trio32/Trio64
     ...
     *      This value specifies the number of character clocks the HSYNC reset
     * in the slave is de- layed from the falling edge of the VSYNC input from
     * the master during genlocking. Remote mode (bit 0 of CRS6 set to 1) must
     * be enabled for this field to take effect. 7-4  CHAR-CLK-RST-DELAY -
     * Character clock reset delay This specifies the number of DCLKs to delay
     * the resetting of the character clock at the end of the scan line. This is
     * used to sync the master and slave character clocks during genlocking.
     * Remote mode (bit 0 of CR56 set to 1) must be enabled for this field to
     *      take effect.
     */
    /* S3 ViRGE/VX
     * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20ViRGE%e2%88%95VX%20Integrated%203D%20Accelerator%20%281996%2d06%29%2epdf]
     *
     ...
     *      0 = No effect
     *      1 = An attempt to write data with the Command FIFO or LPB Output
     * FIFO full or to read data with the Command FIFO not empty generates a PCI
     * bus disconnect cycle 7-4  DELAY HSYNC/VSYNC value = number of DCLKs the
     * HSYNC and VSYNC active pulses are delayed
     */
  ...
  case 0x6b: // BIOS scratchpad: LFB address
    ...
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:S3:CRTC:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu SVGA_S3_ReadCRTC(Bitu reg, Bitu iolen) {
  ...
  case 0x26:
    return ((vga.attr.disabled & 1u) ? 0x00u : 0x20u) |
           (vga.attr.index & 0x1fu);
  case 0x2d: /* Extended Chip ID (high byte of PCI device ID) */
    /* NTS: As the high byte of the PCI ID, this should match the device ID in
     * src/hardware/pci_bus.cpp regarding PCI VGA emulation */
    /* NTS: Windows 98 S3 drivers will refuse to talk to the card if these
     * registers (0x2D-0x2F) do not match the PCI ID or specific values */
    switch (s3Card) {
    case S3_86C928:
  ...
  case 0x2e: /* New Chip ID  (low byte of PCI device ID) */
    /* NTS: Windows 98 S3 drivers will refuse to talk to the card if these
     * registers (0x2D-0x2F) do not match the PCI ID or specific values */
    /* "TESTING FOR THE PRESENCE OF A ViRGE/VX CHIP" section 13.3 PDF page 98 of
     * 394 "After unlocking, an ViRGENX chip can be identified via CR2E." (ASM
     * code reading via 3D4h index 2Eh, comparing to 3Dh) Ref:
     * [http://nas.jmc/jmcs/docs/browse/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20ViRGE%e2%88%95VX%20Integrated%203D%20Accelerator%20%281996%2d06%29%2epdf]
     */
    /* NTS: As the low byte of the PCI ID, this should match the device ID in
     * src/hardware/pci_bus.cpp regarding PCI VGA emulation */
    switch (s3Card) {
    case S3_86C928:
      return 0x00; // not mentioned in datasheet, does not exist
    ...
    case S3_Vision868:
      return 0x80; // Vision868, 0x8880 or 0x8881. S3 didn't list this in their
                   // datasheet, but Windows 95 INF files listed it anyway
    case S3_Vision964:
    ...
    return 0x11; // Trio64 DOSBox SVN default even though SVN is closer to
                 // Vision864 functionally
  case 0x2f:     /* Revision */
    /* NTS: As the low byte of the PCI ID, this should match the revision in
     * src/hardware/pci_bus.cpp regarding PCI VGA emulation */
    // revision ID
    if (s3Card == S3_86C928)
      return 0x0; // not mentioned in datasheet, does not exist
    else if (s3Card == S3_Trio64V)
      return 0x40; // Trio64V+ datasheet, page 280, PCI "class code". "Hardwired
                   // to 0300004xh" (revision is 40h or more)
    else
      return 0x00; // Trio32/Trio64 datasheet, page 242, PCI "class code".
  ...
  case 0x30:       /* CR30 Chip ID/REV register */
    ...
    else if (s3Card == S3_86C928) /* PCI and ISA versions differ here according
                                     to @TC1995 */
      return (enable_pci_vga && has_pcibus_enable()) ? 0xb0 /*PCI*/
                                                     : 0x90 /*ISA/EISA*/;
    else
      return 0x00;
  case 0x31: /* CR31 Memory Configuration */
  ...
  case 0x51: /* Extended System Control 2 */
    return ((vga.config.display_start >> 16u) & 3u) |
           ...
           ((vga.config.scan_len & 0x300u) >> 4u) | vga.s3.reg_51;
...
void SVGA_S3_WriteSEQ(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0x15:
    ...
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:S3:SEQ:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu SVGA_S3_ReadSEQ(Bitu reg, Bitu iolen) {
  ...
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:S3:SEQ:Read from illegal index %2X",
                                 (int)reg);
    return 0;
  }
}
...
Bitu SVGA_S3_GetClock(void) {
  ...
  if (clock == 0)
    ...
    clock = 1000ul * S3_CLOCK(vga.s3.clk[clock].m, vga.s3.clk[clock].n,
                              vga.s3.clk[clock].r);
  /* Check for dual transfer, master clock/2 */
  if (vga.s3.pll.cmd & 0x10)
    clock /= 2;
  return clock;
}

bool SVGA_S3_HWCursorActive(void) { return (vga.s3.hgc.curmode & 0x1) != 0; }
...
void SVGA_Setup_S3Trio(void) {
  ...
  if (vga.mem.memsize == 0)
    vga.mem.memsize = vga.mem.memsize_original =
        2 * 1024 * 1024; // the most common S3 configuration
...
void VGA_DetermineMode_S3(void) {
  ...
  case 0:
    if (vga.attr.mode_control & 1) { // graphics mode
      ...
      // NTS: Two things here. One is that CGA 2-color mode (and the MCGA
      ...
      //
      //      The only catch here is that a contributor (Wengier, I think?) tied
      //      a DOS/V CGA rendering mode into M_CGA2 that we need to watch for.
      //
...
void SetClock_S3(Bitu which, Bitu target) {
  ...
  for (Bitu n = 1; n <= 31; n++) {
    Bits m = (Bits)((target * (n + 2u) * ((Bitu)1u << (Bitu)r) +
                     (S3_CLOCK_REF / 2u)) /
                    S3_CLOCK_REF) -
             2;
    if (0 <= m && m <= 127) {
