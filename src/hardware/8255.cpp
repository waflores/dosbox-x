...
void Intel8255::reset(void) {
  ...
  INTE_1 = INTE_2 = 0;
  INTR_A = INTR_B = 0;
  writeControl(0x9B); /* default: port A input, port B input, port C input mode
                         0 mode 0 */
  latchOutPortA = 0;
  latchOutPortB = 0;
  latchOutPortC = 0;
...
uint8_t Intel8255::readPortA(void) {
  ...
  latchOutPortA =
      (latchOutPortA & portAWriteMask) + (inPortA() & (~portAWriteMask));
...
uint8_t Intel8255::readPortB(void) {
  ...
  latchOutPortB =
      (latchOutPortB & portBWriteMask) + (inPortB() & (~portBWriteMask));
...
uint8_t Intel8255::readPortC(void) {
  latchOutPortC =
      (latchOutPortC & portCWriteMask) + (inPortC() & (~portCWriteMask));
...
void Intel8255::writeControl(const uint8_t data) {
  if (data & 0x80) {
    ...
    if (mode & 0x04) { /* port B mode 1 */
       ...
       * bit[1:1] = Output buffer full aka CPU has written data to this port
       * (OUT) bit[0:0] = Interrupt request B (OUT)
       *
       * input:
    ...
    if (mode & 0x40) { /* port A mode 2 */
      /* port C meanings:
       *
       * bit[7:7] = Output buffer full aka CPU has written data to this port
       * (OUT) bit[6:6] = Acknowledge, from device. This latches the output.
       * Else output is high impedance (IN) bit[5:5] = Input buffer full (OUT)
       ...
       * output:
       * bit[7:7] = Output buffer full aka CPU has written data to this port
       * (OUT) bit[6:6] = Acknowledge, from device (IN) bit[3:3] = Interrupt
       * request A (OUT)
       *
       * input:
    ...
    /* according to PC-98 hardware it seems changing a port to input makes the
     * latch forget its contents */
...
uint8_t Intel8255::inPortA(void) const { return 0x00U; /* override this */ }

uint8_t Intel8255::inPortB(void) const { return 0x00U; /* override this */ }

uint8_t Intel8255::inPortC(void) const { return 0x00U; /* override this */ }
...
void Intel8255::updateINTR_A(void) {
  if (mode & 0x40) { /* mode 2 */
    INTR_A = (INTE_1 && (!OBF_A)) ^
             (INTE_2 &&
              IBF_A); /* OBF goes low when CPU writes, goes high when cleared */
    ...
    if (mode & 0x10)        /* input */
      ...
      INTR_A =
          INTE_A &&
          (!OBF_A); /* OBF goes low when CPU writes, goes high when cleared */
  } else {
    INTR_A = false;
  }
}
...
void Intel8255::updateINTR_B(void) {
  if (mode & 0x04) { /* mode 1 */
    if (mode & 0x02) /* input */
      ...
      INTR_B =
          INTE_B &&
          (!OBF_B); /* OBF goes low when CPU writes, goes high when cleared */
  } else {
    INTR_B = false;
  }
}
...
void Intel8255::sigINTR_A(void) { /* override this */ }

void Intel8255::sigINTR_B(void) { /* override this */ }

void Intel8255::strobePortA(void) { readPortA(); /* override this */ }
