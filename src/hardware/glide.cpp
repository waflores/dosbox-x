...
#if LOG_GLIDE
  if (ret_value == G_OK)
    LOG_MSG("Glide:Port read. Return address: 0x%x, value: %d", ret,
            mem_readd(ret));
  else if (ret_value == G_FAIL)
    LOG_MSG("Glide:Port read. Return address: 0x%x, value: %d. Writing G_FAIL "
            "to port",
            ret, mem_readd(ret));
  else
...
#if LOG_GLIDE
      LOG_MSG("Glide:Memory allocated at 0x%x (segment: %hu)", glsegment << 4,
              glsegment);
#endif
    }
    ret_value = glsegment;
...
private:
  ...
  HostPt lfb_addr[GLIDE_BUFFERS];  // Address offset from base_addr for the
                                   // readable/writable buffers

/* Calculate physical address for access */
#define LFB_getAddr(addr)                                                      \
  (lfb_addr[((addr - base_addr[0]) >> 12) >> GLIDE_PAGE_BITS] + addr)
...
public:
...
#if LOG_GLIDE
      LOG_MSG(
          "Glide:LFB for buffer %d offset set to 0x%p (addr: 0x%p), clear TLB",
          buffer, lfb_addr[buffer], addr + base_addr[buffer]);
#endif
  ...
  HostPt GetHostReadPt(PageNum phys_page) override {
    ...
    if (!locked[buffer])
      LOG_MSG("Glide:Read from unlocked LFB at: 0x%x", phys_page << 12);
#endif
    //	LOG_MSG("Glide:GetHostReadPt called with %d, returning 0x%p", phys_page,
    //LFB_getAddr((phys_page*MEM_PAGESIZE)));
  ...
  HostPt GetHostWritePt(PageNum phys_page) override {
    ...
    // This only makes sense if full lfb access is used...
    if (!locked[buffer])
      LOG_MSG("Glide:Write to unlocked LFB at: 0x%x", phys_page << 12);
#endif
    //	LOG_MSG("Glide:GetHostWritePt called with %d, returning 0x%p",
    //phys_page, LFB_getAddr((phys_page*MEM_PAGESIZE)));
    return lfb_addr[buffer] + (phys_page << 12);
  }
...
#if LOG_GLIDE
      if (fn_pt[i] == NULL) {
        LOG_MSG("Glide:Warning, unable to load %s from glide2x",
                grTable[i].name);
      }
#endif
    }
...
void GLIDE_PowerOn(Section *sec) { test = new GLIDE(sec); }
...
void GLIDE_ResetScreen(bool update) {
  ...
  if (glide.width && (
    ...
    sdl.window = GFX_SetSDLWindowMode(glide.width, glide.height,
    ...
    SDL_Surface *SDL_SetVideoMode(int width, int height, int bpp,
                                  uint32_t flags);
    sdl.surface = SDL_SetVideoMode(glide.width, glide.height, 0,
                                   (glide.fullscreen[0] ? SDL_FULLSCREEN : 0) |
                                       SDL_ANYFORMAT);
#endif
  }
  ApplyPreventCap();
...
void GLIDE_DisableScreen(void) {
  glide.enabled = false; /* if not disabled, GFX_ResetScreen() will call
                            GLIDE_ResetScreen() */
...
typedef FxBool(FX_CALL *pfxSplashInit)(FxU32 hWnd, FxU32 screenWidth,
                                       FxU32 screenHeight, FxU32 numColBuf,
                                       FxU32 numAuxBuf,
                                       GrColorFormat_t colorFormat);
typedef void(FX_CALL *pfxSplash)(float x, float y, float w, float h,
                                 FxU32 frameNumber);
...
static void grSplash(void) {
  ...
  pfxSplashInit fxSplashInit =
      (pfxSplashInit)GetProcAddress(dll, "_fxSplashInit@24");
...
static void process_msg(Bitu value) {
  ...
  if ((i > GLIDE_MAX) || (fn_pt[i] == NULL)) {
    LOG_MSG("Glide:Invalid function pointer for call %s",
            (i > GLIDE_MAX) ? "(invalid)" : grTable[i].name);
    return;
  }

#if LOG_GLIDE
  LOG_MSG("Glide:Processing call %s (%d), return address: 0x%x",
          grTable[i].name, value, ret);
  GLIDE_count[i]++;
#endif
  ...
  case _grAADrawPolygon12:
    // void grAADrawPolygon(int nVerts, const int ilist[], const GrVertex
    // vlist[])
  ...
  case _grAADrawTriangle24:
    ...
    FP.grFunction3p3i(&vertex[0], &vertex[1], &vertex[2], param[4], param[5],
                      param[6]);
    break;
  case _grAlphaBlendFunction16:
    // void grAlphaBlendFunction(GrAlphaBlendFnc_t rgb_sf, GrAlphaBlendFnc_t
    // rgb_df,
  ...
  case _grBufferSwap4:
    ...
    // if(!(glide.lfb_pagehandler->locked[0] ||
    // glide.lfb_pagehandler->locked[1])) b_swap = !b_swap;
#if LOG_GLIDE
  ...
  case _grConstantColorValue416:
    ...
    FP.grFunction4f(int_to_float(param[1]), int_to_float(param[2]),
                    int_to_float(param[3]), int_to_float(param[4]));
    break;
  ...
  case _grDrawTriangle12:
    // void grDrawTriangle(const GrVertex *a, const GrVertex *b, const GrVertex
    // *c)
    ...
    /*
        case _grErrorSetCallback4:
            // void grErrorSetCallback(void (*function)(const char *string,
       FxBool fatal)) FP.grFunction1i = (pfunc1i)fn_pt[i];
            FP.grFunction1i(*param[1]);
            break;
    */
  ...
  case _grGlideInit0:
    ...
    if (glide.enabled)
      break; /* Tie Break Tennis */
    FP.grFunction0 = (pfunc0)fn_pt[i];
    FP.grFunction0();

    // Enable Tomb Rider displaying shadow
    if (!strncasecmp(RunningProgram, "Tombub", 6))
      tomb = 2;
    else if (!strncasecmp(RunningProgram, "Tomb", 4))
      tomb = 1;
    else
      tomb = 0;
  ...
  case _grGlideShutdown0: {
    ...
    for (j = 0; j < (GLIDE_MAX + 1); j++) {
      if (GLIDE_count[j]) {
        LOG_MSG("Glide:%6d calls function %s (%d)", GLIDE_count[j],
                grTable[j].name, j);
      }
    }
    LOG_MSG("Glide: %d framebuffer locks (%d read, %d write)",
            GLIDE_count[_grLfbLock24], GLIDE_count[GLIDE_MAX + 1],
            GLIDE_count[_grLfbLock24] - GLIDE_count[GLIDE_MAX + 1]);
    SDL_memset(GLIDE_count, 0, sizeof(GLIDE_count));
#endif
    break;
  ...
  case _grLfbConstantDepth4:
    ...
    break;
  case _grLfbLock24:
    // FxBool grLfbLock(GrLock_t type, GrBuffer_t buffer, GrLfbWriteMode_t
    // writeMode,
    //		GrOriginLocation_t origin, FxBool pixelPipeline, GrLfbInfo_t
    //*info)
    ...
    // Buffer address should stay the same after BufferSwap, so check which
    // buffer to lock (currently disabled, see _grBufferSwap4)
    ...
    if ((buffer > 1) && (lfbacc < 4))
      j = 0; // Disable access to AUX buffers

    if (j) {
      // Lock the buffer
      k = FP.grRFunction5i1p(param[1], param[2], param[3], param[4], param[5],
                             &lfbinfo);
      if (k == FXTRUE) {
      ...
      if (param[3] == GR_LFBWRITEMODE_ANY)
        dblfbinfo.writeMode = GR_LFBWRITEMODE_565;
      else
        dblfbinfo.writeMode = param[3];

      if (param[4] == GR_ORIGIN_ANY)
        dblfbinfo.origin = GR_ORIGIN_UPPER_LEFT;
      else
        dblfbinfo.origin = param[4];

      dblfbinfo.strideInBytes = ((glide.width > 0) ? glide.width : 1600) * 4;
      dblfbinfo.lfbPtr = glide.lfb_pagehandler->GetLinPt(buffer);

      // if(!(param[1]&GR_LFB_WRITE_ONLY)) SDL_memset(texmem, 0, 1600*1200*4);
      // // Clear memory on read-lock
    ...
    if (dblfbinfo.strideInBytes > 1600 * 4)
      LOG_MSG("Glide:WARNING LFB stride (%d) is larger than max supported: %d!",
              dblfbinfo.strideInBytes, 1600 * 4);

    if (param[1] & GR_LFB_WRITE_ONLY) { // Is a write-only lock
#if LOG_GLIDE
      LOG_MSG("Glide:W/O lock (%d). %s LFB: 0x%p. Returning 0x%x (stride: %d)",
              param[2], (j ? "Real" : "Fake"), lfbinfo.lfbPtr, dblfbinfo.lfbPtr,
              dblfbinfo.strideInBytes);
#endif
    } else { // Is a read-only lock
#if LOG_GLIDE
      LOG_MSG("Glide:R/O lock (%d). %s LFB: 0x%p. Returning 0x%x (stride: %d)",
              param[2], (j ? "Real" : "Fake"), lfbinfo.lfbPtr, dblfbinfo.lfbPtr,
              dblfbinfo.strideInBytes);
  ...
  case _grLfbReadRegion28:
    // FxBool grLfbReadRegion(GrBuffer_t src_buffer, FxU32 src_x, FxU32 src_y,
    // FxU32 src_width,
    ...
    mem_writed(ret, FP.grRFunction6i1p(param[1], param[2], param[3], param[4],
                                       param[5], param[6], ptr16));
  ...
  case _grLfbUnlock8:
    ...
    buffer = param[2];
    if (buffer < 2)
      buffer = buffer ^ b_swap;
#if LOG_GLIDE
    LOG_MSG("Glide:LFB unlock buffer (%d), %d locks active", param[2],
            glide.lfb_pagehandler->locked[buffer]);
#endif
    if (glide.lfb_pagehandler->locked[buffer]) {
      k = FP.grRFunction2i(param[1], param[2]);
      glide.lfb_pagehandler->locked[buffer]--;
      glide.lfb_pagehandler->SetLFBAddr(
          (HostPt)texmem, buffer); // Reset page handler and clear TLB
    } else {
      // k = FXFALSE;
    }
    mem_writed(ret, k);
  ...
  case _grLfbWriteRegion32:
    ...
    mem_writed(ret, FP.grRFunction7i1p(param[1], param[2], param[3], param[4],
                                       param[5], param[6], param[7], ptr16));
    ret_value = G_OK;
    break;
  ...
  case _grSplash20:
    ...
    FP.grFunction4f1i(int_to_float(param[1]), int_to_float(param[2]),
                      int_to_float(param[3]), int_to_float(param[4]), param[5]);
    break;
    /*
  ...
  case _grSstWinOpen28:
    ...
    //	GrColorFormat_t cformat, GrOriginLocation_t org_loc, int num_buffers,
    //int num_aux_buffers)
    ...
    k = FP.grRFunction1p6i(hwnd, param[2], param[3], param[4], param[5],
                           param[6], param[7]);
    if (k == FXFALSE) {
  ...
  case _grTexClampMode12:
    // void grTexClampMode(GrChipID_t tmu, GrTextureClampMode_t sClampMode,
  ...
  case _grTexCombine28:
    ...
    FP.grFunction7i(param[1], param[2], param[3], param[4], param[5], param[6],
                    param[7]);
    break;
  ...
  case _grTexDetailControl16:
    // void grTexDetailControl(GrChipID_t tmu, int lodBias, FxU8 detailScale,
    // float detailMax)
  ...
  case _grTexDownloadMipMap16:
    // void grTexDownloadMipMap(GrChipID_t tmu, FxU32 startAddress, FxU32
  ...
  case _grTexDownloadMipMapLevel32:
    // void grTexDownloadMipMapLevel(GrChipID_t tmu, FxU32 startAddress, GrLOD_t
    // thisLod,
...
#if LOG_GLIDE
    LOG_MSG("Read %d (%d %d %d %d %d) bytes of texture data", texsize, param[3],
            param[4], param[5], param[6], param[7]);
#endif

    FP.grFunction7i1p = (pfunc7i1p)fn_pt[i];
    FP.grFunction7i1p(param[1], param[2], param[3], param[4], param[5],
                      param[6], param[7], texmem);
    break;
  case _grTexDownloadMipMapLevelPartial40:
    ...
    //    GrTextureFormat_t format, FxU32 evenOdd, void *data, int start, int
    //    end);
    FP.grRFunction1i1p = (prfunc1i1p)fn_pt[_grTexTextureMemRequired8];
    if (FP.grRFunction1i1p == NULL) {
      LOG_MSG("Glide:Unable to get pointer to grTexTextureMemRequired");
    ...
    FP.grFunction7i1p2i(param[1], param[2], param[3], param[4], param[5],
                        param[6], param[7], texmem, param[9], param[10]);
    break;
  ...
  case _grTexDownloadTablePartial20:
    // void grTexDownloadTablePartial(GrChipID_t tmu, GrTexTable_t type, void
  ...
  case _grTexMipMapMode12:
    // void grTexMipMapMode(GrChipID_t tmu, GrMipMapMode_t mode, FxBool
    // lodBlend)
    FP.grFunction3i = (pfunc3i)fn_pt[i];
    FP.grFunction3i(param[1], param[2], param[3]);
    break;
  case _grTexMultibase8:
    ...
    /*
        case _grTexMultibaseAddress20:
            // void grTexMultibaseAddress(GrChipID_t tmu, GrTexBaseRange_t
    range, FxU32 startAddress,
            //	FxU32 evenOdd, GrTexInfo *info)
            FP.grFunction4i1p = (pfunc4i1p)fn_pt[i];

            // This is a bit more complicated since *info contains a pointer to
    data texinfo = (GrTexInfo*)param[5]; data = (PhysPt)texinfo->data;
    // Store for later reference texinfo->data = VIRTOREAL(data); #if LOG_GLIDE
            if(log_func[value-_grAADrawLine8] == 1) {
                LOG_MSG("Glide:Replacing pointer 0x%x with 0x%x in function %d",
    data, texinfo->data, _grTexMultibaseAddress20);
                log_func[value-_grAADrawLine8] = 2;
            }
    #endif
            FP.grFunction20(param[1], param[2], param[3], param[4],
    (int)texinfo); texinfo->data = (void*)data;			// Change the
    pointer back break;
    */
  ...
  case _gu3dfLoad8:
    ...
    // Although glide ref specifies *info should be filled by gu3dfGetInfo
    // before calling gu3dfLoad, OpenGlide will re-read the header in gu3dfLoad
    // as well
    MEM_BlockRead32(param[2], &dbguinfo, sizeof(DBGu3dfInfo));
    MEM_StrCopy(param[1], filename, 512);
  ...
  case _guAADrawTriangleWithClip12:
    // void guAADrawTriangleWithClip(const GrVertex *va, const GrVertex *vb,
    // const GrVertex *vc)
    FP.grFunction3p = (pfunc3p)fn_pt[i];
    MEM_BlockRead32(param[1], &vertex[0], sizeof(GrVertex));
    MEM_BlockRead32(param[2], &vertex[1], sizeof(GrVertex));
  ...
  case _guDrawTriangleWithClip12:
    // void guDrawTriangleWithClip(const GrVertex *va, const GrVertex *vb, const
    // GrVertex *vc)
    FP.grFunction3p = (pfunc3p)fn_pt[i];
    MEM_BlockRead32(param[1], &vertex[0], sizeof(GrVertex));
    MEM_BlockRead32(param[2], &vertex[1], sizeof(GrVertex));
  ...
  case _guFogGenerateExp28:
    // void guFogGenerateExp2(GrFog_t fogTable[GR_FOG_TABLE_SIZE], float
    // density)
  ...
  case _guFogGenerateLinear12:
    // void guFogGenerateLinear(GrFog_t fogTable[GR_FOG_TABLE_SIZE], float
    // nearW, float farW)
  ...
  case _guTexAllocateMemory60:
    // GrMipMapId_t guTexAllocateMemory(GrChipID_t tmu, FxU8 evenOddMask, int
    // width, int height,
    ...
    //minFilterMode, 	    GrTextureFilterMode_t magFilterMode, float lodBias, FxBool
    //lodBlend)
    ...
    mem_writed(ret,
               FP.grRFunction13i1f1i(
                   param[1], param[2], param[3], param[4], param[5], param[6],
                   param[7], param[8], param[9], param[10], param[11],
                   param[12], param[13], int_to_float(param[14]), param[15]));
    ret_value = G_OK;
    break;
  case _guTexChangeAttributes48:
    // FxBool guTexChangeAttributes(GrMipMapID_t mmid, int width, int height,
    // GrTextureFormat_t format,
    ...
    mem_writed(ret,
               FP.grRFunction12i(param[1], param[2], param[3], param[4],
  ...
  case _guTexDownloadMipMap12:
    // void guTexDownloadMipMap(GrMipMapId_t mmid, const void *src, const
    // GuNccTable *nccTable)
    ...
    if (mipmap) {
      ...
      MEM_BlockRead32(param[3], (uint8_t *)texmem + texsize,
                      sizeof(GuNccTable));
    ...
    /*
        case _guTexDownloadMipMapLevel12:
            // void guTexDownloadMipMapLevel(GrMipMapId_t mmid, GrLOD_t lod,
       const void **src) FP.grFunction2i1p = (pfunc2i1p)fn_pt[i];
  ...
  case _guTexGetCurrentMipMap4:
    ...
    /*
        case _guTexGetMipMapInfo4:
            ...
            if(ret == 0) {
                LOG_MSG("Glide:Invalid return value handle for %s!",
       grTable[i].name); return;
            }
  ...
  case _ConvertAndDownloadRle64: {
    // void ConvertAndDownloadRle(GrChipID_t tmu, FxU32 startAddress, GrLOD_t
    // thisLod, GrLOD_t largeLod,
    //				GrAspectRatio_t aspectRatio, GrTextureFormat_t
    //format, FxU32 evenOdd, 				FxU8 *bm_data, long  bm_h, FxU32 u0, FxU32 v0,
    //FxU32 width, FxU32 height, 				FxU32 dest_width, FxU32 dest_height, FxU16
    //*tlut)

#if LOG_GLIDE
    LOG_MSG("Glide: RLE width: %d, height: %d, bm_h: %d, u0: %d, v0: %d, dest "
            "%dx%d",
            param[12], param[13], param[9], param[10], param[11], param[14],
            param[15]);
#endif
    ...
    for (k = 0; k < param[13]; k++) {
      ...
      while ((c = mem_readb(param[8] + scount)) != 0xE0) {
        ...
        if (c > 0xE0) {
          for (int count = 0; count < (c & 0x1f); count++) {
            ...
            // tlut is FxU16*
            src[dcount] =
                mem_readw(param[16] + (mem_readb(param[8] + scount + 1) << 1));
            dcount++;
          }
          scount += 2;
      ...
      SDL_memcpy(ptr16 + (k * param[14]), src + param[10],
                 sizeof(FxU16) * param[14]);
      offset += mem_readb(param[8] + 4 + j++);
      dcount = 0;
    }
