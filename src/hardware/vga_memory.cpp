...
unsigned char pc98_pegc_mmio[0x200] = {
    0}; /* PC-98 memory-mapped PEGC registers at E0000h */
...
void VGAMEM_USEC_read_delay() {
  if (vga_memio_delay_ns > 0) {
    ...
    //		if(GCC_UNLIKELY(CPU_Cycles < 3*delaycyc)) delaycyc = 0; //Else
    //port access will set cycles to 0. which might trigger problem with games
    //which read 16 bit values
    CPU_Cycles -= delaycyc;
...
template <class baseLFBHandler>
class VGA_SlowLFBHandler : public baseLFBHandler {
public:
...
template <class Size> static INLINE void hostWrite(HostPt off, Bitu val) {
  if (sizeof(Size) == 1)
...
template <class Size> static INLINE Bitu hostRead(HostPt off) {
  if (sizeof(Size) == 1)
...
INLINE static uint32_t ModeOperation(uint8_t val) {
  ...
  case 0x00:
    ...
    // Rotate Count field, then the Enable Set/Reset mechanism selects data from
    // this or the Set/Reset field. Then the selected Logical Operation is
    // performed on the resulting data and the data in the latch register. Then
    // the Bit Mask field is used to select which bits come from the resulting
    // data and which come from the latch register. Finally, only the bit planes
    // enabled by the Memory Plane Write Enable field are written to memory.
    val = ((val >> vga.config.data_rotate) |
           (val << (8 - vga.config.data_rotate)));
    full = ExpandTable[val];
    full = (full & vga.config.full_not_enable_set_reset) |
  ...
  case 0x02:
    ...
    // register. Then the Bit Mask field is used to select which bits come from
    // the resulting data and which come from the latch register. Finally, only
    // the bit planes enabled by the Memory Plane Write Enable field are written
    // to memory.
    full = RasterOp(FillTable[val & 0xF], vga.config.full_bit_mask);
    break;
  case 0x03:
    // Write Mode 3: In this mode, the data in the Set/Reset field is used as if
    // the Enable Set/Reset field were set to 1111b. Then the host data is first
    // rotated as per the Rotate Count field, then logical ANDed with the value
    // of the Bit Mask field. The resulting value is used on the data obtained
    // from the Set/Reset field in the same way that the Bit Mask field would
    // ordinarily be used. to select which bits come from the expansion of the
    // Set/Reset field and which come from the latch register. Finally, only the
    // bit planes enabled by the Memory Plane Write Enable field are written to
    // memory.
    val = ((val >> vga.config.data_rotate) |
           (val << (8 - vga.config.data_rotate)));
    full = RasterOp(vga.config.full_set_reset,
                    ExpandTable[val] & vga.config.full_bit_mask);
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:Unsupported write mode %d",
                                 vga.config.write_mode);
    full = 0;
    break;
  }
  return full;
...
void pc98_pegc_mmio_write(unsigned int reg, uint8_t val) {
  ...
  case 0x004: // bank 0
    ...
    break;
  case 0x005: // unknown (WORD size write seen by bank switched (battle skin)
              // and LFB (DOOM, DOOM2) games)
    // ignore
    break;
  case 0x006: // bank 1
    ...
    pc98_pegc_mmio[reg] = val;
    break;
  case 0x007: // unknown (WORD size write seen by bank switched (battle skin)
              // and LFB (DOOM, DOOM2) games)
    // ignore
    break;
  case 0x100: // 256-color memory access  (0=packed  1=planar)
    ...
    if (enable_pc98_256color_planar) {
      ...
      if (val & 1)
        LOG_MSG("PC-98 PEGC warning: Guest application/OS attempted to enable "
                "256-color planar mode, which is not enabled in your "
                "configuration");
  ...
  case 0x102: // linear framebuffer (at F00000-F7FFFFh) enable/disable
    ...
    break;
  default:
    LOG_MSG("PC-98 PEGC warning: Unhandled write to %xh val %xh",
            reg + 0xE0000u, val);
    break;
  }
}
...
static inline Bitu VGA_Generic_Read_Handler(PhysPt planeaddr, PhysPt rawaddr,
                                            unsigned char plane) {
  const unsigned char hobit_n =
      ((vga.seq.memory_mode & 2 /*Extended Memory*/) ||
   ...
   *
   * NTS: Real hardware experience says that despite the name, the Odd/Even bit
   * affects reading as well */
  if (!(vga.seq.memory_mode & 4) &&
      !non_cga_ignore_oddeven_engage) /* Odd Even Host Memory Write Addressing
                                         Disable (is not set) */
   ...
   * probably A14 or A16 depending on Extended Memory bit 1 in Sequencer
   * register 04h memory mode */
  if ((vga.gfx.miscellaneous & 2) &&
      !non_cga_ignore_oddeven_engage) { /* Odd/Even enable */
    const PhysPt mask =
        (vga.config.compatible_chain4 ? 0u : ~0xFFFFu) + (1u << hobit_n) - 2u;
    ...
    const PhysPt mask =
        (vga.config.compatible_chain4 ? 0u : ~0xFFFFu) + (1u << hobit_n) - 1u;
  ...
  case 1:
    VGA_Latch templatch;
    templatch.d =
        (vga.latch.d & FillTable[vga.config.color_dont_care]) ^
        FillTable[vga.config.color_compare & vga.config.color_dont_care];
    return (uint8_t)~(templatch.b[0] | templatch.b[1] | templatch.b[2] |
                      templatch.b[3]);
  }
...
template <const bool chained>
static inline void VGA_Generic_Write_Handler(PhysPt planeaddr, PhysPt rawaddr,
                                             uint8_t val) {
  const unsigned char hobit_n =
      ((vga.seq.memory_mode & 2 /*Extended Memory*/) ||
       (vga_ignore_extended_memory_bit && IS_VGA_ARCH))
...
// In most cases all the remapping, bit operations, and such are unnecessary,
// and having an alternate path for this case hopefully addresses complaints by
// other DOSBox forks and users about "worse VGA performance"
...
public:
  ...
  template <typename T> static INLINE bool withinplanes(const PhysPt a) {
    /* NTS: Comparing against template parameter is optimized down to the line
     * of code matching size of type T with no runtime compare */
    if (sizeof(T) == 4)
      return (a & 3u) == 0;
    if (sizeof(T) == 2)
      return (a & 1u) == 0;
  ...
  static INLINE PhysPt chain4remap(const PhysPt addr) {
    /* This is how chained 256-color mode actually works when mapping to planar
     * memory underneath. This is why 256-color mode needs the DWORD mode of the
     * CRTC and why switching to BYTE mode messes up the display (except on
     * Tseng Labs ET3000/ET4000 cards). */
  ...
  static INLINE PhysPt map(const PhysPt addr) {
    return chain4remap((PAGING_GetPhysicalAddress(addr) & vgapages.mask) +
                       (PhysPt)vga.svga.bank_read_full) &
           vga.mem.memmask;
  }
  template <typename T = uint8_t>
  static INLINE T do_read_aligned(const PhysPt a) {
  ...
  template <typename T = uint8_t> static INLINE T do_read(const PhysPt a) {
    if (withinplanes<T>(a)) /* aligned, do a fast typecast read */
      ...
      return (uint32_t)do_read<uint16_t>(a) +
             ((uint32_t)do_read<uint16_t>(a + 2) << (uint32_t)16u);
    else if (sizeof(T) == 2) /* not aligned, split 16-bit to two 8-bit */
      return (uint16_t)do_read<uint8_t>(a) +
             ((uint16_t)do_read<uint8_t>(a + 1) << (uint32_t)8u);
    else
      return 0xFF; /* should not happen, byte I/O is always aligned */
  }
  template <typename T = uint8_t>
  static INLINE void do_write_aligned(const PhysPt a, const T v) {
    *((T *)(&vga.mem.linear[a])) = v;
  }
  template <typename T = uint8_t>
  static INLINE void do_write(const PhysPt a, const T v) {
    if (withinplanes<T>(a)) /* aligned, do a fast typecast write */
      do_write_aligned<T>(map(a), v);
    else if (sizeof(T) == 4) /* not aligned, split 32-bit to two 16-bit */
    {
      do_write<uint16_t>(a, uint16_t(v));
      do_write<uint16_t>(a + 2, uint16_t(v >> (T)16u));
    } else if (sizeof(T) == 2) /* not aligned, split 16-bit to two 8-bit */
    {
      do_write<uint8_t>(a, uint8_t(v));
      do_write<uint8_t>(a + 1, uint8_t(v >> (T)8u));
    }
  }

  uint8_t readb(PhysPt addr) override {
    VGAMEM_USEC_read_delay();
    return do_read<uint8_t>(addr);
  }
  uint16_t readw(PhysPt addr) override {
    VGAMEM_USEC_read_delay();
    return do_read<uint16_t>(addr);
  }
  uint32_t readd(PhysPt addr) override {
    VGAMEM_USEC_read_delay();
    return do_read<uint32_t>(addr);
  }
  void writeb(PhysPt addr, uint8_t val) override {
    VGAMEM_USEC_write_delay();
    do_write<uint8_t>(addr, val);
  }
  void writew(PhysPt addr, uint16_t val) override {
    VGAMEM_USEC_write_delay();
    do_write<uint16_t>(addr, val);
  }
  void writed(PhysPt addr, uint32_t val) override {
    VGAMEM_USEC_write_delay();
    do_write<uint32_t>(addr, val);
  }
};

// Slow accurate emulation.
// This version takes the Graphics Controller bitmask and ROPs into account.
// This is needed for demos that use the bitmask to do color combination or
// bitplane "page flipping" tricks. This code will kick in if running in a
// chained VGA mode and the graphics controller bitmask register is changed to
// anything other than 0xFF.
//
// Impact Studios "Legend"
//  - The rotating objects, rendered as dots, needs this hack because it uses a
//  combination of masking off
//    bitplanes using the VGA DAC pel mask and drawing on the hidden bitplane
//    using the Graphics Controller bitmask. It also relies on loading the VGA
//    latches with zeros as a form of "overdraw". Without this version the
//    effect will instead become a glowing ball of flickering yellow/red.
...
public:
  ...
  static INLINE PhysPt map(const PhysPt addr) {
    return (PAGING_GetPhysicalAddress(addr) & vgapages.mask) +
           (PhysPt)vga.svga
               .bank_read_full; /* masking with vga.mem.memmask not necessary */
  }

  // planar byte offset = addr & ~3u      (discard low 2 bits)
  // planer index = addr & 3u             (use low 2 bits as plane index)
  static INLINE uint8_t readHandler8(PhysPt addr) {
    // FIXME: Does chained mode use the lower 2 bits of the CPU address or does
    // it use the read mode select???
    return VGA_Generic_Read_Handler(addr & ~3u, addr, (uint8_t)(addr & 3u));
  }
  static INLINE void writeHandler8(PhysPt addr, uint8_t val) {
    return VGA_Generic_Write_Handler<true /*chained*/>(addr & ~3u, addr,
                                                       (uint8_t)val);
  }

  /* Your C++ compiler can probably optimize this down to just the optimal calls
   * to based on type T... at least GCC does! */
  template <typename T = uint8_t> static INLINE T do_read(const PhysPt a) {
...
public:
  ...
  static INLINE void writeHandler8(PhysPt addr, uint8_t val) {
    return VGA_Generic_Write_Handler<true /*chained*/>(addr >> 2u, addr,
                                                       (uint8_t)val);
  }
...
public:
  ...
  static INLINE PhysPt map(const PhysPt addr) {
    return (PAGING_GetPhysicalAddress(addr) & vgapages.mask) +
           (PhysPt)vga.svga.bank_read_full; /* masking with vga.mem.memmask>>2
                                               not necessary */
  }
...
// This version assumes that no raster ops, bit shifts, bit masking, or
// complicated stuff is enabled so that DOS games using 256-color unchained mode
// in a simple way, or games with simple handling of 16-color planar modes, can
// see a performance improvement. Reading still uses the slower generic code
// because it's uncommon to read back and the unchained mode has planar actions
// to it.
...
public:
  ...
  static INLINE void writeHandler8(PhysPt addr, uint8_t val) {
    ((uint32_t *)vga.mem.linear)[addr] =
        (((uint32_t *)vga.mem.linear)[addr] & vga.config.full_not_map_mask) +
        (ExpandTable[val] & vga.config.full_map_mask);
  }
...
public:
  VGA_CGATEXT_PageHandler() { flags = PFLAG_NOCODE; }
  uint8_t readb(PhysPt addr) override {
    VGAMEM_USEC_read_delay();
  ...
  void writeb(PhysPt addr, uint8_t val) override {
    ...
    if (enableCGASnow) {
      /* NTS: We can't use PIC_FullIndex() exclusively because it's not precise
       * enough with respect to when DOSBox CPU emulation is writing. We have to
       * use other variables like CPU_Cycles to gain additional precision */
...
public:
  VGA_MCGATEXT_PageHandler() { flags = PFLAG_NOCODE; }
  uint8_t readb(PhysPt addr) override {
...
struct pc98_egc_shifter {
  ...
  void reinit(void) { /* from global vars set by guest */
    ...
    remain =
        pc98_egc_shift_length + 1; /* the register is length - 1 apparently */
  ...
  std::string debug_status(void) {
    ...
    sprintf(tmp,
            "decrement=%u remain=%u srcbit=%u dstbit=%u shf8l=%u shf8br=%u "
            "shf8bl=%u",
            decrement ? 1 : 0, remain, srcbit, dstbit, shft8load, shft8bitr,
            shft8bitl);
  ...
  template <class AWT> inline void bi_adv(void) {
    bufi +=
        pc98_egc_shift_descend ? (sizeof(buffer) - sizeof(AWT)) : sizeof(AWT);
  ...
  template <class AWT>
  inline void input(const AWT a, const AWT b, const AWT c, const AWT d,
                    uint8_t odd) {
    bi<AWT>((pc98_egc_shift_descend ? (sizeof(buffer) + 1 - sizeof(AWT)) : 0) +
                0,
            a);
    bi<AWT>((pc98_egc_shift_descend ? (sizeof(buffer) + 1 - sizeof(AWT)) : 0) +
                4,
  ...
  template <class AWT>
  inline void output(AWT &a, AWT &b, AWT &c, AWT &d, uint8_t odd,
    ...
    if (sizeof(AWT) == 2) {
      ...
      if (pc98_egc_shift_descend) {
        output<uint8_t>(((uint8_t *)(&a))[1], ((uint8_t *)(&b))[1],
                        ((uint8_t *)(&c))[1], ((uint8_t *)(&d))[1], 1, true);
        if (remain != 0)
          output<uint8_t>(((uint8_t *)(&a))[0], ((uint8_t *)(&b))[0],
                          ((uint8_t *)(&c))[0], ((uint8_t *)(&d))[0], 0, true);
        else
          pc98_egc_srcmask[0] = 0;
      } else {
        output<uint8_t>(((uint8_t *)(&a))[0], ((uint8_t *)(&b))[0],
                        ((uint8_t *)(&c))[0], ((uint8_t *)(&d))[0], 0, true);
        if (remain != 0)
...
void pc98_egc_shift_reinit() { pc98_egc_shift.reinit(); }

template <class AWT>
static inline void egc_fetch_planar(egc_quad &dst, const PhysPt vramoff) {
  dst[0].w = *((uint16_t *)(pc98_pgraph_current_cpu_page + vramoff +
                            pc98_pgram_bitplane_offset(0)));
  dst[1].w = *((uint16_t *)(pc98_pgraph_current_cpu_page + vramoff +
                            pc98_pgram_bitplane_offset(1)));
  dst[2].w = *((uint16_t *)(pc98_pgraph_current_cpu_page + vramoff +
                            pc98_pgram_bitplane_offset(2)));
  dst[3].w = *((uint16_t *)(pc98_pgraph_current_cpu_page + vramoff +
                            pc98_pgram_bitplane_offset(3)));
}

/* Generic EGC ROP handling according to Neko Project II.
 * I assume it uses specialized functions for common ROPs as an optimization
 * method since this generic handler does a lot of conditional bit operations.
 * This is ope_xx transcribed for DOSBox-X. */
static egc_quad &ope_xx(uint8_t ope, const PhysPt vramoff) {
  ...
  case 2:
    ...
    // TODO: NP2kai source code (Neko Project II KAI) suggests the illegal value
    // 11b (3) returns one foreground and one background color.
    //       Ref: https://github.com/AZO234/NP2kai mem/memegc.c line 774 ope_nd
    //       and ope_xx. I don't know if any games rely on that, but it might
    //       improve emulation accuracy to support it.
...
static const PC98_OPEFN pc98_egc_opfn[256] = {
    ope_00, ope_xx, ope_xx, ope_np, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_nd, ope_xx, ope_np, ope_xx, ope_xx, ope_0f, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_np, ope_xx, ope_xx, ope_np, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_np, ope_xx, ope_xx,
    ope_np, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_nd,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_nd, ope_xx, ope_xx, ope_xx, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_gg, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_gg, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_gg, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_gg, ope_xx, ope_xx, ope_xx, ope_nd, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx, ope_xx, ope_nd,
    ope_xx, ope_gg, ope_xx, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_c0, ope_xx, ope_xx, ope_np, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_gg, ope_xx, ope_gg, ope_xx, ope_np, ope_gg, ope_xx,
    ope_np, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_gg, ope_xx, ope_f0, ope_xx, ope_xx,
    ope_np, ope_xx, ope_nd, ope_xx, ope_xx, ope_xx, ope_xx, ope_nd, ope_xx,
    ope_fc, ope_xx, ope_xx, ope_ff};

template <class AWT>
static egc_quad &egc_ope(const PhysPt vramoff, const AWT val) {
  *((uint16_t *)pc98_egc_maskef) = *((uint16_t *)pc98_egc_mask);
  ...
  case 1: /* 0x0800 */
    if (pc98_egc_shiftinput) {
      pc98_egc_shift.input<AWT>(val, val, val, val, vramoff & 1);

      pc98_egc_shift.output<AWT>(*((AWT *)(pc98_egc_src[0].b + (vramoff & 1))),
...
void pc98_mem_msw_write(unsigned char which, unsigned char val) {
  LOG_MSG("WARNING: PC-98 NVRAM write to 0x%x value 0x%x, not implemented yet",
          which, val);
  // TODO: Add code to write NVRAM.
  //       According to documentation writing is only enabled if a register is
  //       written elsewhere to allow it.
}
 ...
 *   A2000-A3FFF      Attributes (4KB WORDs). For each 16-bit WORD only the
 * lower 8 bits are read/writeable. A4000-A5FFF      Unknown ?? (4KB WORDs)
 ...
 * 0xA3FFE      MSW8
 *
 * TODO: Study real hardware to determine what the bytes between the NVRAM bytes
 * are. Are they repeats of the MSW bytes, some other value, or just 0xFF?
 */
...
public:
  ...
  uint8_t readb(PhysPt addr) override {
    ...
    if (addr >= 0x3FE0u)
      ...
      return (uint8_t)(~0u); /* Odd bytes of attribute RAM do not exist,
                                apparently */
...
/* according to real hardware, memory address does not affect char offset (port
 * 0xA5) */
class VGA_PC98_CG_PageHandler : public PageHandler {
public:
  VGA_PC98_CG_PageHandler() : PageHandler(PFLAG_NOCODE) {}
  uint8_t readb(PhysPt addr) override {
    /* uses the low 12 bits and therefore does not need
     * PAGING_GetPhysicalAddress() */
    uint8_t high = a1_font_load_addr & 0xff;
    if ((high >= 0x09 && high <= 0x0b) || (high >= 0x0c && high <= 0x0f) ||
        (high >= 0x56 && high <= 0x5f)) {
      if (addr & 1)
        return pc98_font_char_read(a1_font_load_addr, (addr >> 1) & 0xF,
                                   (a1_font_char_offset & 0x20) ? 0 : 1);
      else
        return 0;
    }
  ...
  void writeb(PhysPt addr, uint8_t val) override {
    /* uses the low 12 bits and therefore does not need
     * PAGING_GetPhysicalAddress() */
    if ((a1_font_load_addr & 0x007E) == 0x0056 &&
        (a1_font_load_addr & 0xFF00) != 0x0000)
      pc98_font_char_write(a1_font_load_addr, (addr >> 1) & 0xF, addr & 1, val);
    else
      LOG_MSG("A4xxx MMIO attempt to write FONT ROM char 0x%x",
              a1_font_load_addr);
  }
};
...
// However it makes sense to make it a 64KB region because 8 planes x 64KB =
// 512KB of RAM. Right? By the way real PEGC hardware seems to prefer WORD
// (16-bit) sized read/write aligned on WORD boundaries. In fact Windows 3.1's
// 256-color driver never uses byte-sized read/write in this planar mode.
...
public:
  ...
  uint8_t readb(PhysPt addr) override {
    ...
    // LOG_MSG("PEGC 256-color planar warning: Readb from %lxh",(unsigned
    // long)addr);
    return (uint8_t)(~0);
  }
  void writeb(PhysPt addr, uint8_t val) override {
    ...
    // LOG_MSG("PEGC 256-color planar warning: Writeb to %lxh val
    // %02xh",(unsigned long)addr,(unsigned int)val);
  }
  uint16_t readw(PhysPt addr) override {
    (void)addr;

    // LOG_MSG("PEGC 256-color planar warning: Readw from %lxh",(unsigned
    // long)addr);
    return (uint16_t)(~0);
  }
  void writew(PhysPt addr, uint16_t val) override {
    ...
    // LOG_MSG("PEGC 256-color planar warning: Writew to %lxh val
    // %04xh",(unsigned long)addr,(unsigned int)val);
  }
};

// A8000h is bank 0
// B0000h is bank 1
template <const unsigned int bank>
class VGA_PC98_256BANK_PageHandler : public PageHandler {
public:
  VGA_PC98_256BANK_PageHandler() : PageHandler(PFLAG_NOCODE) {}
  uint8_t readb(PhysPt addr) override {
    return pc98_vram_256bank_from_window(
        bank)[PAGING_GetPhysicalAddress(addr) & 0x7FFFu];
  }
  void writeb(PhysPt addr, uint8_t val) override {
    pc98_vram_256bank_from_window(
        bank)[PAGING_GetPhysicalAddress(addr) & 0x7FFFu] = val;
  }
};
...
public:
  ...
  template <class AWT>
  static inline AWT mode8_r(const unsigned int plane, const PhysPt vramoff) {
    AWT r, b;
  ...
  template <class AWT>
  static inline void mode8_w(const unsigned int plane, const PhysPt vramoff) {
    AWT tb;
  ...
  template <class AWT>
  static inline void modeC_w(const unsigned int plane, const PhysPt vramoff,
                             const AWT mask, const AWT val) {
    AWT t, tb;
  ...
  template <class AWT>
  static inline AWT modeEGC_r(const PhysPt vramoff, const PhysPt fulloff) {
    /* assume: vramoff is even IF AWT is 16-bit wide */
    *((AWT *)(pc98_egc_last_vram[0].b + (vramoff & 1))) =
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(0)));
    ...
    *((AWT *)(pc98_egc_last_vram[3].b + (vramoff & 1))) =
    ...
    if (!pc98_egc_shiftinput) {
      pc98_egc_shift.input<AWT>(
          ...
          *((AWT *)(pc98_egc_last_vram[1].b + (vramoff & 1))),
          *((AWT *)(pc98_egc_last_vram[2].b + (vramoff & 1))),
          *((AWT *)(pc98_egc_last_vram[3].b + (vramoff & 1))), vramoff & 1);
    ...
    if (pc98_egc_regload &
        1) { /* load VRAM data into pattern/tile... (or INVALID) */
      *((AWT *)(pc98_gdc_tiles[0].b + (vramoff & 1))) =
          *((AWT *)(pc98_egc_last_vram[0].b + (vramoff & 1)));
      *((AWT *)(pc98_gdc_tiles[1].b + (vramoff & 1))) =
          *((AWT *)(pc98_egc_last_vram[1].b + (vramoff & 1)));
      *((AWT *)(pc98_gdc_tiles[2].b + (vramoff & 1))) =
          *((AWT *)(pc98_egc_last_vram[2].b + (vramoff & 1)));
      *((AWT *)(pc98_gdc_tiles[3].b + (vramoff & 1))) =
          *((AWT *)(pc98_egc_last_vram[3].b + (vramoff & 1)));
    }
    ...
    if (pc98_egc_compare_lead) {
      if (!pc98_egc_shiftinput)
        return *(
            (AWT *)(pc98_egc_src[pc98_egc_lead_plane & 3].b + (vramoff & 1)));
      else
        return *(
            (AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                    ((pc98_egc_lead_plane & 3) * PC98_VRAM_BITPLANE_SIZE)));
    }
  ...
  template <class AWT>
  static inline void modeEGC_w(const PhysPt vramoff, const AWT val) {
    /* assume: vramoff is even IF AWT is 16-bit wide */
    ...
    if (pc98_egc_regload &
        2) { /* load VRAM data before writing on VRAM write (or INVALID) */
      *((AWT *)(pc98_gdc_tiles[0].b + (vramoff & 1))) =
          *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                    pc98_pgram_bitplane_offset(0)));
      *((AWT *)(pc98_gdc_tiles[1].b + (vramoff & 1))) =
    ...
    if (accmask != 0) {
      if (!(pc98_egc_access & 1)) {
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(0))) &= ~accmask;
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(0))) |=
            accmask & *((AWT *)(ropdata[0].b + (vramoff & 1)));
      }
      if (!(pc98_egc_access & 2)) {
        ...
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(1))) |=
            accmask & *((AWT *)(ropdata[1].b + (vramoff & 1)));
      }
      if (!(pc98_egc_access & 4)) {
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(2))) &= ~accmask;
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(2))) |=
            accmask & *((AWT *)(ropdata[2].b + (vramoff & 1)));
      }
      if (!(pc98_egc_access & 8)) {
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(3))) &= ~accmask;
        *((AWT *)(pc98_pgraph_current_cpu_page + vramoff +
                  pc98_pgram_bitplane_offset(3))) |=
            accmask & *((AWT *)(ropdata[3].b + (vramoff & 1)));
      }
    }
  }
  ...
  template <class AWT> AWT readc(PhysPt addr) {
     ...
     * bit 2: VOPBIT_GRCG  1=Read/Modify/Write when writing  0=TCR mode at read,
     * TDW mode at write  (from bit 6) */
    ...
    case 0x0D:
      return *((AWT *)(pc98_pgraph_current_cpu_page + addr +
                       pc98_pgram_bitplane_offset(plane)));
    case 0x08: /* TCR/TDW */
    case 0x09: {
      AWT r = 0;
  ...
  template <class AWT> void writec(PhysPt addr, AWT val) {
    ...
    case 0x06:
    case 0x07:
      *((AWT *)(pc98_pgraph_current_cpu_page + addr +
                pc98_pgram_bitplane_offset(plane))) = val;
      break;
    case 0x08: /* TCR/TDW write tile data, no masking */
    case 0x09: {
      /* this writes to multiple bitplanes at once.
       * notice that the value written has no meaning, only the tile data and
       * memory address. */
      if (!(pc98_gdc_modereg & 1)) // blue channel
                ...
                */
    {
      ...
      /* this writes to multiple bitplanes at once */
      if (!(pc98_gdc_modereg & 1)) // blue channel
        modeC_w<AWT>(0 /*plane*/, addr + pc98_pgram_bitplane_offset(0), mask,
                     val);

      if (!(pc98_gdc_modereg & 2)) // red channel
        modeC_w<AWT>(1 /*plane*/, addr + pc98_pgram_bitplane_offset(1), mask,
                     val);

      if (!(pc98_gdc_modereg & 4)) // green channel
        modeC_w<AWT>(2 /*plane*/, addr + pc98_pgram_bitplane_offset(2), mask,
                     val);

      if (!(pc98_gdc_modereg & 8)) // extended channel
        modeC_w<AWT>(3 /*plane*/, addr + pc98_pgram_bitplane_offset(3), mask,
                     val);
    } break;
    case 0x0A: /* EGC write */
    case 0x0B:
...
public:
  VGA_PC98_LFB_Handler()
      : PageHandler(PFLAG_READABLE | PFLAG_WRITEABLE | PFLAG_NOCODE) {}
  VGA_PC98_LFB_Handler(const unsigned int fl) : PageHandler(fl) {}
  HostPt GetHostReadPt(PageNum phys_page) override {
    return &vga.mem.linear[(phys_page & 0x7F) * 4096 +
                           PC98_VRAM_GRAPHICS_OFFSET]; /* 512KB mapping */
  }
...
public:
  ...
  uint8_t readb(PhysPt addr) override {
    delay();
    return vga.tandy
        .mem_base[(PAGING_GetPhysicalAddress(addr) - 0xb8000) & 0x3FFF];
  }
  void writeb(PhysPt addr, uint8_t val) override {
    delay();
    vga.tandy.mem_base[(PAGING_GetPhysicalAddress(addr) - 0xb8000) & 0x3FFF] =
        val;
  }
};
...
public:
  ...
  static INLINE void do_write_aligned(const PhysPt a, const T v) {
    const uint8_t plane =
        (vga.mode == M_AMSTRAD) ? vga.amstrad.write_plane : 0x01; // 0x0F?
    if (plane & 0x08)
      *((T *)(&vga.tandy.mem_base[a + 0xC000u])) = v;
    if (plane & 0x04)
      *((T *)(&vga.tandy.mem_base[a + 0x8000u])) = v;
    if (plane & 0x02)
      *((T *)(&vga.tandy.mem_base[a + 0x4000u])) = v;
    if (plane & 0x01)
      *((T *)(&vga.tandy.mem_base[a])) = v;
  }
  template <typename T = uint8_t>
  static INLINE void do_write(const PhysPt a, const T v) {
    if (sizeof(T) == 4) /* not aligned, split 32-bit to two 16-bit */
...
public:
  ...
  uint8_t readb(PhysPt /*addr*/) override {
    //		LOG(LOG_VGA, LOG_NORMAL ) ( "Read from empty memory space at
    //%x", addr );
    return 0xff;
  }
  void writeb(PhysPt /*addr*/, uint8_t /*val*/) override {
    //		LOG(LOG_VGA, LOG_NORMAL ) ( "Write %x to empty memory space at
    //%x", val, addr );
  }
};

void InColorLoadLatch(const VGA_Latch &latch) {
  const uint32_t nochangemask =
      vga.herc.latchprotect * 0x01010101u; /* i.e. 0x5A -> 0x5A5A5A5A */
  vga.herc.latch =
      (vga.herc.latch & nochangemask) + (latch.d & (~nochangemask));
}

/* Text region of the InColor which is documented to write to all bitplanes
 * bypassing the masking functions entirely */
/* NTS: If the InColor is documented as writing all bitplanes unconditionally in
 * monochrome backwards compatible mode, then it stands to reason that it
 * doesn't have the Odd/Even mode EGA/VGA does and the text and attribute bytes
 *      remain interleaved even as four copies across all four bitplanes. Am I
 * right? */
...
public:
  ...
  static uint8_t readHandler(PhysPt start) {
    ...
    for (unsigned int c = 0; c < 4; c++) {
      result |= (((tmp & idcmask) == (vga.herc.bgcolor & idcmask)) ? 1u : 0u)
                << (7 - c);
      tmp >>= 8u;
    }
  ...
  static void writeHandler(PhysPt start, uint8_t val) {
    ...
    ((uint32_t *)vga.mem.linear)[start] =
        (((uint32_t *)vga.mem.linear)[start] & nochangemask) +
        (pl.d & (~nochangemask));
  }
...
/* backdoor PC-98 memory I/O interface for GDC drawing code in
 * vga_pc98_gdc_draw.cpp. The GDC drawing code must not access video memory
 * through CPU memory I/O functions to avoid needless address translation and
 * possible emulation stability issues that can arise if the CPU is running in
 * 386 protected mode and paging is enabled.
 *
 * According to nanshiki, the GDC's read/modify/write I/O may also go through
 * the EGC circuitry because there are references that mention using EGC
 * functions to enhance GDC draw commands. Which means just directly
 * reading/writing planar memory will not emulate things correctly.
 * [https://github.com/joncampbell123/dosbox-x/pull/3508#issuecomment-1135452117]
 *
 * The code submitted by nanshiki already adds the drawing base address to the
 * address when calling these functions (e.g. A8000, B0000, B8000, E0000) so
 * there is no need to adjust them.
 *
 * This code calls readc/writec directly because read/write must translate CPU
 * virtual addresses to physical due to a design inherited from DOSBox SVN in
 * which the address is NOT translated to physical by the underlying emulator
 * code before calling this interface. If I had designed this, read/write would
 * concern themselves only with physical addresses. --Jonathan C. */
...
void VGA_ChangedBank(void) { VGA_SetupHandlers(); }
...
void VGA_SetupHandlers(void) {
  /* This code inherited from DOSBox SVN confuses bank size with bank
   * granularity. This code is enforcing bank granularity. Bank size concerns
   * how much memory is mapped to A0000-BFFFF. Most cards expose a window of
   * 64KB. Most cards also have a bank granularity of 64KB, but some, like
   * Paradise and Cirrus, have 64KB windows and 4KB granularity. */
  ...
  case MCH_CGA:
    ...
    goto range_done;
  case MCH_MCGA: // Based on real hardware, A0000-BFFFF is the 64KB of RAM
                 // mapped twice
    MEM_SetPageHandler(VGA_PAGE_A0, 16,
                       &vgaph.mcgatext); // A0000-AFFFF is the 64KB of video RAM
    MEM_ResetPageHandler_Unmapped(VGA_PAGE_B0, 8); // B0000-B7FFF is unmapped
    MEM_SetPageHandler(VGA_PAGE_B8, 8,
                       &vgaph.mcgatext); // B8000-BFFFF is the last 32KB half of
                                         // video RAM, alias
    goto range_done;
  case MCH_PCJR:
    ...
    MEM_SetPageHandler(VGA_PAGE_B8, 8,
                       vga_memio_lfb_delay ? &vgaph.pcjr_slow : &vgaph.pcjr);
    goto range_done;
  case MCH_MDA:
  case MCH_HERC:
    //		MEM_SetPageHandler( VGA_PAGE_A0, 16, &vgaph.empty );        //
    //already done by VGA setup memory function, also interferes with "allow
    //more than 640KB" option
    vgapages.base = VGA_PAGE_B0;
    /* NTS: Implemented according to
     * [http://www.seasip.info/VintagePC/hercplus.html#regs] */
    if (vga.herc.enable_bits & 0x2) { /* bit 1: page in upper 32KB */
      /* NTS: I don't know what Hercules graphics cards do if you set bit 1 but
       * not bit 0. For the time being, I'm assuming that they respond to
       * 0xB8000+ because of bit 1 but only map to the first 4KB because of bit
       * 0. Basically, a configuration no software would actually use. */
      if (vga.herc.enable_bits &
          0x1) { /* allow graphics and enable 0xB1000-0xB7FFF */
        vgapages.mask = 0xffff;
        MEM_SetPageHandler(VGA_PAGE_B0, 16,
                           (machine == MCH_HERC && hercCard == HERC_InColor)
                               ? (PageHandler *)(&vgaph.herc_incolor_graphics)
                               : (PageHandler *)(vga_memio_lfb_delay
                                                     ? &vgaph.map_slow
                                                     : &vgaph.map));
      } else {
        vgapages.mask = 0xfff;
    ...
    if (machine == MCH_HERC && hercCard == HERC_InColor &&
        !(vga.herc.mode_control &
          0x2) /*not in graphics mode (bit 1 of 3b8h)*/) {
  ...
  case MCH_TANDY:
    ...
    MEM_SetPageHandler(VGA_PAGE_A0, 32,
                       vga_memio_lfb_delay ? &vgaph.map_slow : &vgaph.map);
    if (vga.tandy.extended_ram & 1) {
      // You seem to be able to also map different 64kb banks, but have to
      // figure that out This seems to work so far though
      ...
      MEM_SetPageHandler(VGA_PAGE_B8, 8,
                         vga_memio_lfb_delay ? &vgaph.tandy_slow
                                             : &vgaph.tandy);
    }
    goto range_done;
    //		MEM_SetPageHandler(vga.tandy.mem_bank<<2,vga.tandy.is_32k_mode ?
    //0x08 : 0x04,range_handler);
  ...
  case PC98_ARCH_CASE:
    MEM_SetPageHandler(
        VGA_PAGE_A0 + 0x00, 0x02,
        &vgaph.pc98_text); /* A0000-A1FFFh text layer, character data */
    MEM_SetPageHandler(VGA_PAGE_A0 + 0x02, 0x02,
                       &vgaph.pc98_text); /* A2000-A3FFFh text layer, attribute
                                             data + non-volatile RAM */
    MEM_SetPageHandler(
        VGA_PAGE_A0 + 0x04, 0x01,
        &vgaph
             .pc98_cg); /* A4000-A4FFFh character generator memory-mapped I/O */
    MEM_ResetPageHandler_Unmapped(VGA_PAGE_A0 + 0x05,
                                  0x03); /* A5000-A7FFFh not mapped */

    if (pc98_gdc_vramop & (1 << VOPBIT_VGA)) {
      if (pc98_gdc_vramop & (1 << VOPBIT_PEGC_PLANAR)) {
        MEM_SetPageHandler(
            VGA_PAGE_A0 + 0x08, 0x10,
            &vgaph.pc98_256planar); /* A8000-B7FFFh planar graphics (???) */
        MEM_ResetPageHandler_Unmapped(
            VGA_PAGE_A0 + 0x18,
            0x08); /* B8000-BFFFFh graphics layer, not mapped */
      } else {
        MEM_SetPageHandler(
            VGA_PAGE_A0 + 0x08, 0x08,
            &vgaph.pc98_256bank0); /* A8000-AFFFFh graphics layer, bank 0 */
        MEM_SetPageHandler(
            VGA_PAGE_A0 + 0x10, 0x08,
            &vgaph.pc98_256bank1); /* B0000-B7FFFh graphics layer, bank 1 */
        MEM_ResetPageHandler_Unmapped(
            VGA_PAGE_A0 + 0x18,
            0x08); /* B8000-BFFFFh graphics layer, not mapped */
      }
      ...
      MEM_SetPageHandler(
          VGA_PAGE_A0 + 0x08, 0x08,
          &vgaph.pc98); /* A8000-AFFFFh graphics layer, B bitplane */
      MEM_SetPageHandler(
          VGA_PAGE_A0 + 0x10, 0x08,
          &vgaph.pc98); /* B0000-B7FFFh graphics layer, R bitplane */
      MEM_SetPageHandler(
          VGA_PAGE_A0 + 0x18, 0x08,
          &vgaph.pc98); /* B8000-BFFFFh graphics layer, G bitplane */
    }

    /* E0000-E7FFFh graphics layer
     *  - In 8-color mode, E0000-E7FFFh is not mapped
     *  - In 16-color mode, E0000-E7FFFh is the 4th bitplane (E)
     *  - In 256-color mode, E0000-E7FFFh is memory-mapped I/O that controls the
     * 256-color mode */
    if (pc98_gdc_vramop & (1 << VOPBIT_VGA))
    ...
    /* If the system has 15MB or less RAM, OR the ISA memory hole at 15MB is
     * enabled */
    if (MEM_TotalPages() <= 0xF00 || isa_memory_hole_15mb) {
       ...
       *  - Switching from 256-color mode immediately unmaps this linear
       * framebuffer.
       *  - Switching to 256-color mode will immediately map the linear
       * framebuffer if the enable bit is set in the PEGC MMIO registers */
      if ((pc98_gdc_vramop & (1 << VOPBIT_VGA)) &&
          pc98_pegc_linear_framebuffer_enabled())
        MEM_SetPageHandler(0xF00, 512 /*kb*/ / 4 /*kb*/,
                           vga_memio_lfb_delay ? &vgaph.map_lfb_pc98_slow
                                               : &vgaph.map_lfb_pc98);
      else
        MEM_ResetPageHandler_Unmapped(0xF00, 512 /*kb*/ / 4 /*kb*/);
    }

    /* There is a confirmed alias of the PEGC framebuffer at
     * 0xFFF00000-0xFFF7FFFF. Some PC-9821 CD-ROM games require this alias or
     * else they display nothing on the screen. These games run in 32-bit
     * protected mode and for whatever reason direct the DPMI server to map
     * linear address 0xF00000-0xF7FFFF to physical memory address
     * 0xFFF00000-0xFFF7FFFF, so that they can write to the linear address just
     * as if a 386SX where that mapping existed. This is basically the same idea
     * as "just under the BIOS at top of CPU addressable RAM" so if memalias is
     * more than 24 bits map it to (1 << memalias) - 1MB to be consistent
     * because that address will map to wherever PEGC's LFB is even if, say,
     * running on a 486SX with 26 address pins. */
    if (MEM_get_address_bits() > 24) {
      Bitu page = ((Bitu)1 << (Bitu)(MEM_get_address_bits4GB() -
                                     12 /*pages not bytes*/)) -
                  (Bitu)0x100 /*1MB in pages*/;
      if ((pc98_gdc_vramop & (1 << VOPBIT_VGA)) &&
          pc98_pegc_linear_framebuffer_enabled())
        MEM_SetPageHandler(page, 512 /*kb*/ / 4 /*kb*/,
                           vga_memio_lfb_delay ? &vgaph.map_lfb_pc98_slow
                                               : &vgaph.map_lfb_pc98);
      else
    ...
    if (vga.config.chained) {
      if (vga.config.compatible_chain4) {
        if (vga.complexity.flags == 0 && memio_complexity_optimization) {
          /* The DOS program is not using anything beyond basic memory I/O and
           * does not need the raster op, data rotate, and bit planar features
           * at all. Therefore VGA memory I/O performance can be improved by
           * assigning a simplified handler that omits that logic */
          if (svgaCard == SVGA_TsengET3K || svgaCard == SVGA_TsengET4K)
            newHandler = vga_memio_lfb_delay ? &vgaph.map_slow : &vgaph.map;
          else
            newHandler = &vgaph.cvga;
        } else {
          /* NTS: ET4000AX cards appear to have a different chain4
           * implementation from everyone else: the planar memory byte address
           * is address >> 2 and bits A0-A1 select the plane, where all other
           * clones I've tested seem to write planar memory byte (address & ~3)
           *      (one byte per 4 bytes) and bits A0-A1 select the plane. Note
           * that the et4000 emulation implemented so far will not trigger this
           * if() condition for 256-color mode. */
          /* FIXME: Different chain4 implementation on ET4000 noted---is it true
           * also for ET3000? */
          if (svgaCard == SVGA_TsengET3K || svgaCard == SVGA_TsengET4K)
      ...
      } else if ((vga.complexity.flags != 0 ||
                  !memio_complexity_optimization) &&
                 (svgaCard == SVGA_TsengET3K || svgaCard == SVGA_TsengET4K) &&
                 vga.mode == M_VGA) {
        /* NTS: Tseng ET4000AX emulation CLEARS the "compatible chain4" flag for
         * 256-color mode which is why this extra check is needed */
        newHandler = &vgaph.cvga_et4000_slow;
      } else {
  ...
  if ((dos.loaded_codepage == 936 || dos.loaded_codepage == 950 ||
      ...
      strlen(RunningProgram) > 3 && !strncmp(RunningProgram, "ET", 2))
    enveten = true;
  runeten =
      !vga_fill_inactive_ram && enveten &&
  ...
  case 0:
    ...
    case SVGA_TsengET4K:
      ...
      break;
      /* NTS: Looking at the official ET4000 programming guide, it does in fact
       * support the full 128KB */
    case SVGA_S3Trio:
    default:
      vgapages.mask = 0xffff & vga.mem.memmask;
      break;
    }
    if (CurMode && CurMode->mode >= 0x14 /*VESA BIOS or extended mode*/ &&
        vbe_window_size > 0 /*user override of window size*/) {
      unsigned int pages =
          (vbe_window_size + 0xFFFu) >> 12u; /* bytes to pages, round up */
      if (pages > 32)
        pages = 32;
      assert(pages != 0u);
  ...
  /* NTS: Based on a discussion on GitHub
   * [https://github.com/joncampbell123/dosbox-x/issues/4042] and on Vogons
   * [https://www.vogons.org/viewtopic.php?t=45808] it makes logical sense that
   * Chain 4 would take precedence over odd/even mode. There are some DOS games
   * that accidentally clear bit 2 and would normally enable odd/even mode in
   * 256-color mode, but they displayed correctly anyway. This fixes "Seal:
   * Magic Eye" and a 1994 256-byte Demoscene entry.
   *
   *      If chain 4 (or "compatible chain 4") is enabled, then ignore odd/even
   * mode. If neither text nor CGA display mode and requested by the user
   * through dosbox.conf, ignore odd/even mode. */
  non_cga_ignore_oddeven_engage =
      (vga.seq.memory_mode & 8 /*Chain 4 enable*/) ||
      (non_cga_ignore_oddeven &&
       !(vga.mode == M_TEXT || vga.mode == M_CGA2 || vga.mode == M_CGA4));
...
void VGA_StartUpdateLFB(void) {
   ...
   * Windows 3.1 S3 driver will reprogram the linear framebuffer down to 0xA0000
   * when entering a DOSBox and assuming the full VRAM size will cause a LOT of
   * problems! */
  Bitu winsz = 0x10000;
   ...
   *      and so on.
   *
   * From the S3 Trio32/Trio64 documentation regarding the Linear Address Window
   * Position Registers: "The Linear Address Window resides on a 64KB, 1MB, 2MB,
   * or 4MB (Trio64 only) memory boundary (size-aligned) ... Some LSBs of this
   * register are ignored because of the size-aligned boundary scheme" */
  const unsigned int la_winmsk =
      ~((winsz - 1u) >>
        16u); /* Register holds the upper 16 bits of the linear address */
  ...
  /* if the DOS application or Windows 3.1 driver attempts to put the linear
   * framebuffer below the top of memory, then we're probably entering a DOS VM
   * and it's probably a 64KB window. If it's not a 64KB window then print a
   * warning. */
  else if ((unsigned long)(vga.s3.la_window << 4UL) <
           (unsigned long)MEM_TotalPages()) {
    if (winsz != 0x10000) // 64KB window normal for entering a DOS VM in
                          // Windows 3.1 or legacy bank switching in DOS
      LOG(LOG_MISC, LOG_WARN)("S3 warning: Window size != 64KB and address "
                              "conflict with system RAM!");
    ...
    MEM_SetLFB((unsigned int)(vga.s3.la_window & la_winmsk) << 4u,
               (unsigned int)vga.mem.memsize / 4096u, vga.lfb.handler,
               &vgaph.mmio);
  }
}
...
void VGA_SetupMemory() {
  ...
  if (vga.mem.linear == NULL) {
    ...
    vga.mem.linear =
        (uint8_t *)(((uintptr_t)vga.mem.linear_orgptr + 16ull - 1ull) &
                    ~(16ull - 1ull));
    ...
    pc98_pgraph_current_display_page =
        vga.mem.linear + PC98_VRAM_GRAPHICS_OFFSET;
  ...
  /* obey user override for "bank size", which this code inherited from DOSBox
   * SVN confuses with "bank granularity". If "bank size" were truly a concern
   * it would affect how much of the A0000-BFFFF region VGA mapping would
   * expose. */
  if (vbe_window_granularity > 0)
    vga.svga.bank_size =
        vbe_window_granularity; /* allow different sizes for dev testing */
  else
