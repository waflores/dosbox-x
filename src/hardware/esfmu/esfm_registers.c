...
static const int16 kslrom[16] = {0,  32, 40, 45, 48, 51, 53, 55,
...
static const emu_slot_channel_mapping emu_slot_map[36] = {
    {0, 0},  {1, 0},  {2, 0},  {0, 1},  {1, 1},  {2, 1},  {3, 0},  {4, 0},
    {5, 0},  {3, 1},  {4, 1},  {5, 1},  {6, 0},  {7, 0},  {8, 0},  {6, 1},
    {7, 1},  {8, 1},  {9, 0},  {10, 0}, {11, 0}, {9, 1},  {10, 1}, {11, 1},
    {12, 0}, {13, 0}, {14, 0}, {12, 1}, {13, 1}, {14, 1}, {15, 0}, {16, 0},
    {17, 0}, {15, 1}, {16, 1}, {17, 1}};
...
static const int emu_4op_secondary_to_primary[18] = {
    -1, -1, -1, 0, 1, 2, -1, -1, -1, -1, -1, -1, 9, 10, 11, -1, -1, -1};
...
/* ------------------------------------------------------------------------- */
static void ESFM_emu_rearrange_connections(esfm_channel *channel) {
  ...
  if (channel->emu_mode_4op_enable && (channel->channel_idx % 9) < 3 &&
      channel->chip->emu_newmode) {
    esfm_channel *secondary =
...
/* ------------------------------------------------------------------------- */
static void ESFM_emu_to_native_switch(esfm_chip *chip) {
...
/* ------------------------------------------------------------------------- */
static void ESFM_native_to_emu_switch(esfm_chip *chip) {
  size_t channel_idx;
  for (channel_idx = 0; channel_idx < 18; channel_idx++) {
...
/* ------------------------------------------------------------------------- */
static void ESFM_slot_update_keyscale(esfm_slot *slot) {
...
/* ------------------------------------------------------------------------- */
static void ESFM_emu_channel_update_keyscale(esfm_channel *channel) {
  int secondary_to_primary;
...
/* ------------------------------------------------------------------------- */
static inline uint8_t ESFM_slot_readback(esfm_slot *slot,
...
/* ------------------------------------------------------------------------- */
static inline void ESFM_slot_write(esfm_slot *slot, uint8_t register_idx,
                                   uint8_t data) {
  switch (register_idx & 0x07) {
  case 0x00:
...
/* ------------------------------------------------------------------------- */
static uint8_t ESFM_readback_reg_native(esfm_chip *chip, uint16_t address) {
  int i;
  uint8_t data = 0;
  address = address & 0x7ff;
...
/* ------------------------------------------------------------------------- */
static void ESFM_write_reg_emu(esfm_chip *chip, uint16_t address,
                               uint8_t data) {
  ...
  if (reg == 0xbd) {
    ...
    if (chip->emu_rhy_mode_flags & 0x20) {
      // TODO: check if writes to 0xbd actually affect the readable key-on flags
      // at 0x246, 0x247, 0x248; and if there's any visible effect from the SD
      // and TC flags
  ...
  case 0x30:
    if (emu_slot_idx >= 0) {
      ESFM_slot_write(&chip->channels[natv_chan_idx].slots[natv_slot_idx], 0x0,
                      data);
    }
    break;
  case 0x40:
  case 0x50:
    if (emu_slot_idx >= 0) {
      ESFM_slot_write(&chip->channels[natv_chan_idx].slots[natv_slot_idx], 0x1,
                      data);
  ...
  case 0x70:
    if (emu_slot_idx >= 0) {
      ESFM_slot_write(&chip->channels[natv_chan_idx].slots[natv_slot_idx], 0x2,
                      data);
    }
    break;
  case 0x80:
  case 0x90:
    if (emu_slot_idx >= 0) {
      ESFM_slot_write(&chip->channels[natv_chan_idx].slots[natv_slot_idx], 0x3,
                      data);
    }
    break;
  case 0xa0:
  ...
  case 0xb0:
    if (emu_chan_idx >= 0) {
      ...
      // TODO: check if emulation mode actually writes to the native mode key on
      // registers it might only use slot 0's emu key on field...
  ...
  case 0xf0:
    if (emu_slot_idx >= 0) {
      ESFM_slot_write(&chip->channels[natv_chan_idx].slots[natv_slot_idx], 0x7,
                      data);
    }
    break;
  }
...
/* ------------------------------------------------------------------------- */
void ESFM_set_mode(esfm_chip *chip, bool native_mode) {
