...
static const uint16_t exprom[256] = {
    0xff4, 0xfea, 0xfde, 0xfd4, 0xfc8, 0xfbe, 0xfb4, 0xfa8, 0xf9e, 0xf92, 0xf88,
    0xf7e, 0xf72, 0xf68, 0xf5c, 0xf52, 0xf48, 0xf3e, 0xf32, 0xf28, 0xf1e, 0xf14,
    0xf08, 0xefe, 0xef4, 0xeea, 0xee0, 0xed4, 0xeca, 0xec0, 0xeb6, 0xeac, 0xea2,
    0xe98, 0xe8e, 0xe84, 0xe7a, 0xe70, 0xe66, 0xe5c, 0xe52, 0xe48, 0xe3e, 0xe34,
    0xe2a, 0xe20, 0xe16, 0xe0c, 0xe04, 0xdfa, 0xdf0, 0xde6, 0xddc, 0xdd2, 0xdca,
    0xdc0, 0xdb6, 0xdac, 0xda4, 0xd9a, 0xd90, 0xd88, 0xd7e, 0xd74, 0xd6a, 0xd62,
    0xd58, 0xd50, 0xd46, 0xd3c, 0xd34, 0xd2a, 0xd22, 0xd18, 0xd10, 0xd06, 0xcfe,
    0xcf4, 0xcec, 0xce2, 0xcda, 0xcd0, 0xcc8, 0xcbe, 0xcb6, 0xcae, 0xca4, 0xc9c,
    0xc92, 0xc8a, 0xc82, 0xc78, 0xc70, 0xc68, 0xc60, 0xc56, 0xc4e, 0xc46, 0xc3c,
    0xc34, 0xc2c, 0xc24, 0xc1c, 0xc12, 0xc0a, 0xc02, 0xbfa, 0xbf2, 0xbea, 0xbe0,
    0xbd8, 0xbd0, 0xbc8, 0xbc0, 0xbb8, 0xbb0, 0xba8, 0xba0, 0xb98, 0xb90, 0xb88,
    0xb80, 0xb78, 0xb70, 0xb68, 0xb60, 0xb58, 0xb50, 0xb48, 0xb40, 0xb38, 0xb32,
    0xb2a, 0xb22, 0xb1a, 0xb12, 0xb0a, 0xb02, 0xafc, 0xaf4, 0xaec, 0xae4, 0xade,
    0xad6, 0xace, 0xac6, 0xac0, 0xab8, 0xab0, 0xaa8, 0xaa2, 0xa9a, 0xa92, 0xa8c,
    0xa84, 0xa7c, 0xa76, 0xa6e, 0xa68, 0xa60, 0xa58, 0xa52, 0xa4a, 0xa44, 0xa3c,
    0xa36, 0xa2e, 0xa28, 0xa20, 0xa18, 0xa12, 0xa0c, 0xa04, 0x9fe, 0x9f6, 0x9f0,
    0x9e8, 0x9e2, 0x9da, 0x9d4, 0x9ce, 0x9c6, 0x9c0, 0x9b8, 0x9b2, 0x9ac, 0x9a4,
    0x99e, 0x998, 0x990, 0x98a, 0x984, 0x97c, 0x976, 0x970, 0x96a, 0x962, 0x95c,
    0x956, 0x950, 0x948, 0x942, 0x93c, 0x936, 0x930, 0x928, 0x922, 0x91c, 0x916,
    0x910, 0x90a, 0x904, 0x8fc, 0x8f6, 0x8f0, 0x8ea, 0x8e4, 0x8de, 0x8d8, 0x8d2,
    0x8cc, 0x8c6, 0x8c0, 0x8ba, 0x8b4, 0x8ae, 0x8a8, 0x8a2, 0x89c, 0x896, 0x890,
    0x88a, 0x884, 0x87e, 0x878, 0x872, 0x86c, 0x866, 0x860, 0x85a, 0x854, 0x850,
    0x84a, 0x844, 0x83e, 0x838, 0x832, 0x82c, 0x828, 0x822, 0x81c, 0x816, 0x810,
    0x80c, 0x806, 0x800};
...
/* ------------------------------------------------------------------------- */
static inline int13 ESFM_envelope_wavegen(uint3 waveform, int16 phase,
                                          uint10 envelope) {
  int13 out;
...
/* ------------------------------------------------------------------------- */
static void ESFM_envelope_calc(esfm_slot *slot) {
  uint8 nonzero;
  uint8 rate;
  uint5 rate_hi;
  uint2 rate_lo;
  ...
  if (!slot->chip->native_mode) {
    int pair_primary_idx =
        emu_4op_secondary_to_primary[slot->channel->channel_idx];
    if (pair_primary_idx >= 0) {
      esfm_channel *pair_primary =
          &slot->channel->chip->channels[pair_primary_idx];
      if (pair_primary->emu_mode_4op_enable) {
        key_on = *pair_primary->slots[0].in.key_on;
      }
    } else if ((slot->channel->channel_idx == 7 ||
                slot->channel->channel_idx == 8) &&
               slot->slot_idx == 1) {
  ...
  if (slot->tremolo_en) {
    ...
    if (slot->chip->native_mode) {
      tremolo =
          slot->channel->chip->tremolo >> ((!slot->tremolo_deep << 1) + 2);
    } else {
      tremolo = slot->channel->chip->tremolo >>
                ((!slot->chip->emu_tremolo_deep << 1) + 2);
    }
  ...
  // TODO: is this really how the chip behaves? Can it only transition the
  // envelope delay once? Am I implementing this in a sane way? I feel like this
  // is a roundabout hack.
  if ((slot->in.eg_delay_transitioned_10 &&
       !slot->in.eg_delay_transitioned_10_gate) ||
      (slot->in.eg_delay_transitioned_01 &&
       !slot->in.eg_delay_transitioned_01_gate)) {
    slot->in.eg_delay_counter_compare = 0;
    if (slot->env_delay > 0) {
      slot->in.eg_delay_counter_compare = 256 << slot->env_delay;
    }
  ...
  if (key_on && slot->in.eg_state == EG_RELEASE) {
    ...
    if ((slot->in.eg_delay_counter >= slot->in.eg_delay_counter_compare) ||
        !slot->chip->native_mode) {
      reset = 1;
...
/* ------------------------------------------------------------------------- */
static void ESFM_phase_generate_emu(esfm_slot *slot) {
  esfm_chip *chip;
  uint3 block;
  uint10 f_num;
  uint32 basefreq;
 ...
 */
/* ------------------------------------------------------------------------- */
static int16 ESFM_slot3_noise3_mod_input_calc(esfm_slot *slot) {
  ...
  // (we skip slot 0 because its calculation happens at the end, not at the
  // beginning)
  for (i = 1; i < 3; i++) {
    // double the pitch
    ...
    output_buf = ESFM_envelope_wavegen(channel->slots[2].waveform, phase,
                                       channel->slots[i].in.eg_output);
  }
...
/* ------------------------------------------------------------------------- */
static void ESFM_slot_generate(esfm_slot *slot) {
  ...
  slot->in.output =
      ESFM_envelope_wavegen(slot->waveform, phase, slot->in.eg_output);
...
/* ------------------------------------------------------------------------- */
static void ESFM_slot_generate_emu(esfm_slot *slot) {
  const esfm_chip *chip = slot->chip;
  uint3 waveform = slot->waveform & (chip->emu_newmode != 0 ? 0x07 : 0x03);
  bool rhythm_slot_double_volume =
      (slot->chip->emu_rhy_mode_flags & 0x20) != 0 &&
  ...
  output_value = (slot->in.output & slot->in.emu_output_enable)
                 << rhythm_slot_double_volume;
  if (chip->emu_newmode) {
    slot->channel->output[0] +=
        output_value & slot->channel->slots[0].out_enable[0];
    slot->channel->output[1] +=
        output_value & slot->channel->slots[0].out_enable[1];
  } else {
...
static void ESFM_process_feedback(esfm_chip *chip) {
  int channel_idx;
  ...
  for (channel_idx = 0; channel_idx < 18; channel_idx++) {
    ...
    !defined(_ESFMU_DISABLE_ASM_OPTIMIZATIONS)
      asm("movzbq  %[wave], %%r8               \n\t"
          ...
          "decl    %%edx                       \n\t"
          "jne     1b                          \n\t"
          : [p_fb] "=&r"(phase_feedback), [p_acc] "+r"(phase_acc),
            [out] "=&r"(wave_out)
          : [p_off] "r"(phase_offset), [mod_in] "r"(mod_in_shift),
            [wave] "g"(waveform), [eg_out] "g"(eg_output),
            [sinrom] "m"(logsinrom), [exprom] "m"(exprom)
    ...
    !defined(_ESFMU_DISABLE_ASM_OPTIMIZATIONS)
      ...
      asm("movzbl  %b[wave], %%eax             \n\t"
          ...
          "decl    %[i]                        \n\t"
          "jne     1b                          \n\t"
          : [p_fb] "=&m"(phase_feedback), [p_acc] "+r"(phase_acc),
            [out] "=&r"(wave_out), [last] "=&m"(wave_last),
            [eg_out] "+m"(eg_output)
          : [p_off] "m"(phase_offset), [mod_in] "m"(mod_in_shift),
            [wave] "m"(waveform), [sinrom] "m"(logsinrom_addr),
            [exprom] "m"(exprom_addr), [i] "m"(iter_counter)
          : "cc", "ax", "bx", "cx", "di");
    ...
    !defined(_ESFMU_DISABLE_ASM_OPTIMIZATIONS)
      asm("movs    r3, #0                     \n\t"
          ...
          : [p_fb] "=&r"(phase_feedback), [p_acc] "+r"(phase_acc),
            [out] "=&r"(wave_out)
          : [p_off] "r"(phase_offset), [mod_in] "r"(mod_in_shift),
            [eg_out] "r"(eg_output), [sinrom] "r"(logsinrom + waveform * 1024),
            [exprom] "r"(exprom)
          : "cc", "r0", "r1", "r2", "r3", "r8");
#else
      wave_out = 0;
...
/* ------------------------------------------------------------------------- */
static void ESFM_process_channel(esfm_channel *channel) {
  int slot_idx;
...
/* ------------------------------------------------------------------------- */
static void ESFM_process_channel_emu(esfm_channel *channel) {
  int slot_idx;
  channel->output[0] = channel->output[1] = 0;
...
/* ------------------------------------------------------------------------- */
static int16_t ESFM_clip_sample(int32 sample) {
...
/* ------------------------------------------------------------------------- */
static void ESFM_update_timers(esfm_chip *chip) {
  int i;
  // Tremolo
...
/* ------------------------------------------------------------------------- */
int ESFM_reg_write_chan_idx(esfm_chip *chip, uint16_t reg) {
  int which_reg = -1;
  if (chip->native_mode) {
    bool is_key_on_reg =
        reg >= KEY_ON_REGS_START && reg < (KEY_ON_REGS_START + 20);
    if (is_key_on_reg) {
...
/* ------------------------------------------------------------------------- */
void ESFM_update_write_buffer(esfm_chip *chip) {
  ...
  while ((write_buf = &chip->write_buf[chip->write_buf_start]),
         write_buf->valid &&
             write_buf->timestamp <= chip->write_buf_timestamp) {
    ...
    if (is_which_note_on_reg >= 0) {
      if ((chip->native_mode && (write_buf->data & 0x01) == 0) ||
        ...
        // this is a note off command; note down that we got note off for this
        // channel
        note_off_written[is_which_note_on_reg] = true;
      } else {
        // this is a note on command; have we gotten a note off for this channel
        // in this cycle?
        if (note_off_written[is_which_note_on_reg]) {
          // we have a conflict; let the note off be processed first and defer
          // the rest of the buffer to the next cycle
          break;
        }
      }
    }
