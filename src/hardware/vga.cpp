 ...
 *       addr = addr                             (addr >> 2) byte in planar
 * space, plane select by (addr & 3)
 *
 *   Other VGA cards:
 *       addr = ((addr & ~3) << 2) + (addr & 3)  (addr & ~3) byte in planar
 * space, plane select by (addr & 3)
 *
 *   I suspect that this difference may be the reason several 1992-1993-ish DOS
 * demos have problems clearing VRAM. It's possible they noticed that zeroing
 * RAM was faster in planar mode, and coded their routines around ET4000 cards,
 * not knowing that Trident, Cirrus, and every VGA clone thereafter implemented
 * the chained 256-color modes differently.
 */
...
bool vga_render_on_demand = false; // Render at vsync or specific changes to
                                   // hardware instead of every scanline
...
void VGA_DetermineMode_StandardVGA(
    void) { /* and EGA, the extra regs are not used for machine=ega */
  if (vga.attr.mode_control & 1) { // graphics mode
    ...
    // NTS: Two things here. One is that CGA 2-color mode (and the MCGA 640x480
    // 2-color mode)
    //      are just EGA planar modes with fewer bitplanes enabled. The planar
    //      render mode can display them just fine. The other is that checking
    //      for 2-color CGA mode entirely by whether video RAM is mapped to
    //      B8000h is a really lame way to go about it.
    //
    //      The only catch here is that a contributor (Wengier, I think?) tied a
    //      DOS/V CGA rendering mode into M_CGA2 that we need to watch for.
    //
...
void VGA_StartResize(Bitu delay /*=50*/) {
  if (!vga.draw.resizing) {
    /* even if the user disables the delay, we can avoid a lot of window
     * resizing by at least having 1ms of delay */
    if (!enable_vga_resize_delay && delay > 1)
      delay = 1;
...
void VGA_SetCGA2Table(uint8_t val0, uint8_t val1) {
  ...
  for (Bitu i = 0; i < 16u; i++) {
    CGA_2_Table[i] =
        ...
        ((Bitu)total[(i >> 0u) & 1u] << 0u) |
        ((Bitu)total[(i >> 1u) & 1u] << 8u) |
        ((Bitu)total[(i >> 2u) & 1u] << 16u) |
        ((Bitu)total[(i >> 3u) & 1u] << 24u);
#else
        ((Bitu)total[(i >> 3u) & 1u] << 0u) |
        ((Bitu)total[(i >> 2u) & 1u] << 8u) |
        ((Bitu)total[(i >> 1u) & 1u] << 16u) |
        ((Bitu)total[(i >> 0u) & 1u] << 24u);
#endif
  }
...
void VGA_SetCGA4Table(uint8_t val0, uint8_t val1, uint8_t val2, uint8_t val3) {
  ...
  for (Bitu i = 0; i < 256u; i++) {
    CGA_4_Table[i] =
#ifdef WORDS_BIGENDIAN
        ((Bitu)total[(i >> 0u) & 3u] << 0u) |
        ((Bitu)total[(i >> 2u) & 3u] << 8u) |
        ((Bitu)total[(i >> 4u) & 3u] << 16u) |
        ((Bitu)total[(i >> 6u) & 3u] << 24u);
#else
        ((Bitu)total[(i >> 6u) & 3u] << 0u) |
        ((Bitu)total[(i >> 4u) & 3u] << 8u) |
        ((Bitu)total[(i >> 2u) & 3u] << 16u) |
        ((Bitu)total[(i >> 0u) & 3u] << 24u);
#endif
    CGA_4_HiRes_Table[i] =
#ifdef WORDS_BIGENDIAN
        ((Bitu)total[((i >> 0u) & 1u) | ((i >> 3u) & 2u)] << 0u) |
        (Bitu)(total[((i >> 1u) & 1u) | ((i >> 4u) & 2u)] << 8u) |
        ((Bitu)total[((i >> 2u) & 1u) | ((i >> 5u) & 2u)] << 16u) |
        (Bitu)(total[((i >> 3u) & 1u) | ((i >> 6u) & 2u)] << 24u);
#else
        ((Bitu)total[((i >> 3u) & 1u) | ((i >> 6u) & 2u)] << 0u) |
        (Bitu)(total[((i >> 2u) & 1u) | ((i >> 5u) & 2u)] << 8u) |
        ((Bitu)total[((i >> 1u) & 1u) | ((i >> 4u) & 2u)] << 16u) |
        (Bitu)(total[((i >> 0u) & 1u) | ((i >> 3u) & 2u)] << 24u);
#endif
    CGA_4_HiRes_TableNP[i] =
#ifdef WORDS_BIGENDIAN
        ((Bitu)(((i >> 0u) & 1u) | ((i >> 3u) & 2u)) << 0u) |
        (Bitu)((((i >> 1u) & 1u) | ((i >> 4u) & 2u)) << 8u) |
        ((Bitu)(((i >> 2u) & 1u) | ((i >> 5u) & 2u)) << 16u) |
        (Bitu)((((i >> 3u) & 1u) | ((i >> 6u) & 2u)) << 24u);
#else
        ((Bitu)(((i >> 3u) & 1u) | ((i >> 6u) & 2u)) << 0u) |
        (Bitu)((((i >> 2u) & 1u) | ((i >> 5u) & 2u)) << 8u) |
        ((Bitu)(((i >> 1u) & 1u) | ((i >> 4u) & 2u)) << 16u) |
        (Bitu)((((i >> 0u) & 1u) | ((i >> 3u) & 2u)) << 24u);
#endif
  }
...
void SetRate(char *x) {
  if (!strncasecmp(x, "off", 3))
    ...
    if (*x == '/' || *x == ':') {
      x++;
      minor = strtol(x, NULL, 0);
    }

    if (major > 0) {
      vga_force_refresh_rate = (double)major;
      if (minor > 1)
        vga_force_refresh_rate /= minor;
    }
  }
...
#if defined(USE_TTF)
void resetFontSize();
static void resetSize(Bitu /*val*/) { resetFontSize(); }
#endif
...
public:
  void Run(void) override {
    if (cmd->FindExist("/?", false)) {
      ...
      WriteOut(
          "  SET rate  Lock to fractional frame rate, e.g. 60000/1001\n\n");
      WriteOut("  PITSYNC <ON|OFF> Make PIT timer tick at refresh rate if "
               "close enough\n\n");
      WriteOut(
          "Type VFRCRATE without a parameter to show the current status.\n");
      return;
    }
...
void VFRCRATE_ProgramStart(Program **make) { *make = new VFRCRATE; }
...
public:
  ...
  void Run(void) override {
    if (cmd->FindExist("/?", false)) {
      ...
      WriteOut(
          "Type CGASNOW without a parameter to show the current status.\n");
      return;
    }
    if (cmd->FindExist("ON")) {
...
void CGASNOW_ProgramStart(Program **make) { *make = new CGASNOW; }
...
VGA_Vsync VGA_Vsync_Decode(const char *vsyncmodestr) {
  if (!strcasecmp(vsyncmodestr, "off"))
    return VS_Off;
  else if (!strcasecmp(vsyncmodestr, "on"))
    return VS_On;
  else if (!strcasecmp(vsyncmodestr, "force"))
    return VS_Force;
  else if (!strcasecmp(vsyncmodestr, "host"))
    return VS_Host;
  else
...
void VGA_Reset(Section *) {
  ...
  if (str == "1" || str == "true")
    ...
    enable_supermegazeux_256colortext =
        false; // TODO: Default true if emulating a chipset that supports
               // SuperMegazeux 256-color text mode

  GDC_display_plane_wait_for_vsync =
      pc98_section->Get_bool("pc-98 buffer page flip");
  ...
  if (S3_LFB_BASE == 0) {
    if (cpu_addr_bits >= 32)
      ...
      S3_LFB_BASE =
          (enable_pci_vga && has_pcibus_enable()) ? 0x02000000 : 0x03400000;
  ...
  /* if the user WANTS the base address to be PCI misaligned, then turn off PCI
   * VGA emulation */
  if (enable_pci_vga && has_pcibus_enable() && (S3_LFB_BASE & 0x1FFFFFFul)) {
    if (!lfb_default)
      LOG(LOG_VGA,
          LOG_DEBUG)("S3 linear framebuffer was set by user to an address not "
                     "aligned to 32MB, switching off PCI VGA emulation");
  ...
  if (cpu_addr_bits < 26) {
    if (IS_VGA_ARCH && enable_pci_vga && has_pcibus_enable())
      LOG(LOG_VGA, LOG_DEBUG)("CPU memalias setting is below 26 bits, "
                              "switching off PCI VGA emulation");
  ...
  if (S3_LFB_BASE >= 0xFE000000UL)
    E_Exit("S3 LFB base 0x%lx would overlap BIOS", (unsigned long)S3_LFB_BASE);

  /* if the constraints we imposed make it impossible to maintain the alignment
   * required for PCI, then just switch off PCI VGA emulation. */
  if (IS_VGA_ARCH && enable_pci_vga && has_pcibus_enable()) {
    if (S3_LFB_BASE & 0x1FFFFFFUL) { /* not 32MB aligned */
      LOG(LOG_VGA, LOG_DEBUG)("S3 linear framebuffer is not 32MB aligned, "
                              "switching off PCI VGA emulation");
      enable_pci_vga = false;
    }
  }
  ...
  if (IS_VGA_ARCH && svgaCard == SVGA_S3Trio)
    LOG(LOG_VGA,
        LOG_DEBUG)("S3 linear framebuffer at 0x%lx%s as %s",
   ...
   *   If the LFB is set to an address below 16MB, the driver will program the
   * base to something odd like 0x73000000 and access MMIO through 0x74000000.
   *
   *   Because of this, if memalias < 31 and LFB is below 16MB mark, Windows
   * won't use the accelerated features of the S3 emulation properly.
   *
   *   If memalias=24, the driver hangs and nothing appears on screen.
   *
   *   As far as I can tell, it's mapping for the LFB, not the MMIO. It uses the
   * MMIO in the A0000-AFFFF range anyway. The failure to blit and draw seems to
   * be caused by mapping the LFB out of range like that and then trying to draw
   * on the LFB.
   *
   *   As far as I can tell from http://www.vgamuseum.info and the list of S3
   * cards, the S3 chipsets emulated by DOSBox-X and DOSBox SVN here are all
   * EISA and PCI cards, so it's likely the driver is written around the
   * assumption that memory addresses are the full 32 bits to the card, not just
   * the low 24 seen on the ISA slot. So it is unlikely the driver could ever
   * support the card on a 386SX nor could such a card work on a 386SX. It
   * shouldn't even work on a 486SX (26-bit limit), but it could. */
  if (IS_VGA_ARCH && svgaCard == SVGA_S3Trio && cpu_addr_bits <= 24)
    LOG(LOG_VGA, LOG_WARN)("S3 linear framebuffer warning: memalias setting is "
                           "known to cause the Windows 3.1 S3 driver to crash");
  if (IS_VGA_ARCH && svgaCard == SVGA_S3Trio && cpu_addr_bits < 31 &&
      S3_LFB_BASE < 0x1000000ul) /* below 16MB and memalias == 31 bits */
    LOG(LOG_VGA,
        LOG_WARN)("S3 linear framebuffer warning: A linear framebuffer below "
                  "the 16MB mark in physical memory when memalias < 31 is "
                  "known to have problems with the Windows 3.1 S3 driver");

  pc98_allow_scanline_effect =
      pc98_section->Get_bool("pc-98 allow scanline effect");
  ...
  enable_pc98_256color_planar =
      pc98_section->Get_bool("pc-98 enable 256-color planar");

#if 0 // TODO: Do not enforce until 256-color mode is fully implemented.
      //       Some users out there may expect the EGC, GRCG, 16-color options
      //       to disable the emulation. Having 256-color mode on by default,
      //       auto-enable them, will cause surprises and complaints.
  ...
  int10_vesa_map_as_128kb =
      section->Get_bool("vesa map non-lfb modes to 128kb region");
  ...
  vga_palette_update_on_full_load =
      section->Get_bool("vga palette update on full load");
  non_cga_ignore_oddeven =
      section->Get_bool("ignore odd-even mode in non-cga modes");
  vga_ignore_extended_memory_bit =
      section->Get_bool("ignore extended memory bit");
  enable_vretrace_poll_debugging_marker =
      section->Get_bool("vertical retrace poll debug line");
  vga_double_buffered_line_compare =
      section->Get_bool("double-buffered line compare");
  ...
  allow_vesa_lowres_modes =
      section->Get_bool("allow low resolution vesa modes");
  allow_vesa_4bpp_packed = section->Get_bool("allow 4bpp packed vesa modes");
  allow_explicit_vesa_24bpp =
      section->Get_bool("allow explicit 24bpp vesa modes");
  ...
  vga_ignore_hdispend_change_if_smaller =
      section->Get_bool("resize only on vga active display width increase");
  vesa_bios_modelist_in_info =
      section->Get_bool("vesa vbe put modelist in vesa information");

  /* It turns out various S3 cards and their BIOSes vary their preference for
   * 32bpp or 24bpp. For example most S3 drivers are perfectly happy with and
   * will prefer 32bpp, but S3 ViRGE drivers for some reason prefer 24bpp, and
   * they even expect the base VESA BIOS modes to be 24bpp. The Windows drivers
   * for ViRGE chipsets in fact do not even offer "Truecolor (32-bit)" even
   * though the chipset documentation says that if you turn on "full streams
   * processor operation" you can take your pick of either one, but then again,
   * the Windows drivers don't have their own modesetting code anyway, they just
   * call INT 10h VESA BIOS extensions to change video modes and make
   * assumptions about video modes and formats. No attempt by the drivers are
   * made to modify the primary streams processor during modesettings.
   *
   * This code does respond to those register writes even if not used yet, so
   * maybe someday if we get Windows NT to work in DOSBox-X, then assuming the
   * NT drivers do not call INT 10h, we can see a driver do the modesetting work
   * itself and enhance emulation appropriately.
   *
   * Therefore "auto" as added as an option to forestall millions of users
   * complaining about garbled displays whenever they select "16.7 million
   * colors" in Windows 3.1 or "Truecolor (24-bit)" in Windows 95 with
   * machine=svga_s3virge. It would be much easier to automatically select on
   * auto than to answer the same question over and over with "just change this
   * option to false if you're using machine=svga_s3virge".
   *
   * This automatic selection may cause DOS games that assume or require 32bpp
   * to fail with machine=svga_s3virge, but then again, such games would likely
   * fail the same way if run on real hardware with a real ViRGE PCI card. If
   * your DOS game assumes things about the mode without using the "get mode"
   * call that has been there since VBE 1.0, then your game deserves to
   * misrender on screen and it's your fault. Fortunately by 1996 when the ViRGE
   * chipset came out, most DOS games by that point were using the "get mode"
   * call, except for some Demoscene coders who evidently did not have VESA BIOS
   * documentation and had to guess and assume from what they could figure out.
   */
  ...
  else if (svgaCard == SVGA_S3Trio && s3Card >= S3_ViRGE &&
           s3Card <= S3_ViRGEVX)
    vesa12_modes_32bpp =
        false; /* ViRGE Windows drivers only support 24bpp and they expect the
                  base VESA modes to provide 24bpp */
  else
    vesa12_modes_32bpp = true;

  /* sanity check: "VBE 1.2 modes 32bpp" doesn't make any sense if neither 24bpp
   * or 32bpp is enabled */
  if (!allow_vesa_32bpp && !allow_vesa_24bpp)
    vesa12_modes_32bpp = 0;
  /* sanity check: "VBE 1.2 modes 32bpp=true" doesn't make sense if the user
   * disabled 32bpp */
  else if (vesa12_modes_32bpp && !allow_vesa_32bpp)
    vesa12_modes_32bpp = 0;
  /* sanity check: "VBE 1.2 modes 32bpp=false" doesn't make sense if the user
   * disabled 24bpp */
  else if (!vesa12_modes_32bpp && !allow_vesa_24bpp && allow_vesa_32bpp)
    vesa12_modes_32bpp = 1;

  if (vga_force_refresh_rate > 0)
    LOG(LOG_VGA, LOG_NORMAL)("VGA forced refresh rate active = %.3f",
                             vga_force_refresh_rate);
  ...
  if (memio_complexity_optimization)
    LOG_MSG("Memory I/O complexity optimization enabled aka option 'memory io "
            "optimization 1'. If the game or demo is unable to draw to the "
            "screen properly, set the option to false.");

  if (vga_render_on_demand_user > 0)
    LOG_MSG(
        "'scanline render on demand' option is enabled. If this option breaks "
        "the game or demo effects or display, set the option to false.");
  else if (vga_render_on_demand_user < 0)
    LOG_MSG(
        "The 'scanline render on demand' option is available and may provide a "
        "modest boost in video render performance if set to true.");
  ...
  if (vga_memio_delay_ns < 0) {
    if (IS_EGAVGA_ARCH) {
      if (pcibus_enable) {
        /* some delay based on PCI bus protocol with frame start, turnaround,
         * and burst transfer */
        double t = (1000000000.0 * clockdom_PCI_BCLK.freq_div *
                    (1 /*turnaround*/ + 1 /*frame start*/ + 1 /*burst*/ -
                     0.25 /*fudge*/)) /
                   clockdom_PCI_BCLK.freq;
        vga_memio_delay_ns = (int)floor(t);
      } else {
        /* very optimistic setting, ISA bus cycles are longer than 2, but also
         * the 386/486/Pentium pipeline instruction decoding. so it's not a
         * matter of sucking up enough CPU cycle counts to match the duration of
         * a memory I/O cycle, because real hardware probably has another
         * instruction decode going while it does it.
         *
         * this is long enough to fix some demo's raster effects to work
         * properly but not enough to significantly bring DOS games to a crawl.
         * Apparently, this also fixes Future Crew "Panic!" by making the
         * shadebob take long enough to allow the 3D rotating dot object to
         * finish it's routine just in time to become the FC logo, instead of
         * sitting there waiting awkwardly for 3-5 seconds. */
        double t = (1000000000.0 * clockdom_ISA_BCLK.freq_div * 3.75) /
                   clockdom_ISA_BCLK.freq;
        vga_memio_delay_ns = (int)floor(t);
      }
    } else if (machine == MCH_CGA || machine == MCH_HERC ||
               machine == MCH_MDA) {
      /* default IBM PC/XT 4.77MHz timing. this is carefully tuned so that
       * Trixter's CGA test program times our CGA emulation as having about
       * 305KB/sec reading speed. */
      double t = (1000000000.0 * clockdom_ISA_OSC.freq_div * 143) /
                 (clockdom_ISA_OSC.freq * 3);
      vga_memio_delay_ns = (int)floor(t);
    } else {
      /* dunno. pick something */
      double t = (1000000000.0 * clockdom_ISA_BCLK.freq_div * 6) /
                 clockdom_ISA_BCLK.freq;
      vga_memio_delay_ns = (int)floor(t);
    }
  }
  ...
  vbe_window_granularity =
      (unsigned int)section->Get_int("vbe window granularity")
      << 10u; /* KB to bytes */
  vbe_window_size = (unsigned int)section->Get_int("vbe window size")
                    << 10u; /* KB to bytes */

  if (vbe_window_granularity != 0 && !bitop::ispowerof2(vbe_window_granularity))
    LOG(LOG_VGA, LOG_WARN)("User specified VBE window granularity is not a "
                           "power of 2. May break some programs.");
  if (vbe_window_size != 0 && !bitop::ispowerof2(vbe_window_size))
    LOG(LOG_VGA, LOG_WARN)("User specified VBE window size is not a power of "
                           "2. May break some programs.");
  if (vbe_window_size != 0 && vbe_window_granularity != 0 &&
      vbe_window_size < vbe_window_granularity)
    LOG(LOG_VGA, LOG_WARN)(
        "VBE window size is less than window granularity, which prevents full "
        "access to video memory and may break some programs.");
   ...
   *        Chips & Tech VGA implementations? */
  {
    ...
    if (sz_m >= 0 || sz_k > 0) {
      ...
      if (vga.mem.memsize == 0)
        vga.mem.memsize = _KB_bytes(512);
      vga.mem.memsize_original = vga.mem.memsize;

      /* round up to the nearest power of 2 (TODO: Any video hardware that uses
       * non-power-of-2 sizes?). A lot of DOSBox's VGA emulation code assumes
       * power-of-2 VRAM sizes especially when wrapping memory addresses with (a
       * & (vmemsize - 1)) type code. */
      if (!is_power_of_2(vga.mem.memsize)) {
        vga.mem.memsize = 1u << (int_log2(vga.mem.memsize) + 1u);
        LOG(LOG_VGA, LOG_WARN)(
            "VGA RAM size requested is not a power of 2, rounding up to %uKB",
            vga.mem.memsize >> 10);
      }
    } else {
  ...
  case MCH_HERC:
    if (hercCard >= HERC_InColor) {
      if (vga.mem.memsize < _KB_bytes(256))
        vga.mem.memsize = _KB_bytes(256);
    } else {
      if (vga.mem.memsize < _KB_bytes(64))
        vga.mem.memsize = _KB_bytes(64);
    }
    break;
  case MCH_MDA:
    if (vga.mem.memsize < _KB_bytes(4))
      vga.mem.memsize = _KB_bytes(4);
    break;
  case MCH_CGA:
    if (vga.mem.memsize < _KB_bytes(16))
      vga.mem.memsize = _KB_bytes(16);
    break;
  case MCH_TANDY:
  case MCH_PCJR:
    if (vga.mem.memsize < _KB_bytes(128))
      vga.mem.memsize = _KB_bytes(128); /* FIXME: Right? */
    break;
  case MCH_EGA:
    // EGA cards supported either 64KB, 128KB or 256KB.
    if (vga.mem.memsize == 0)
      vga.mem.memsize = _KB_bytes(256); // default
    else if (vga.mem.memsize <= _KB_bytes(64))
      vga.mem.memsize = _KB_bytes(64);
    else if (vga.mem.memsize <= _KB_bytes(128))
      vga.mem.memsize = _KB_bytes(128);
    else
      vga.mem.memsize = _KB_bytes(256);
    break;
  case MCH_VGA:
    // TODO: There are reports of VGA cards that have less than 256KB in the
    // early days of VGA.
    //       How does that work exactly, especially when 640x480 requires about
    //       37KB per plane? Did these cards have some means to chain two
    //       bitplanes odd/even in the same way than EGA did it?
  ...
  case MCH_AMSTRAD:
    if (vga.mem.memsize < _KB_bytes(64))
      vga.mem.memsize = _KB_bytes(64); /* FIXME: Right? */
    break;
  case MCH_PC98:
    if (vga.mem.memsize < _KB_bytes(544))
      vga.mem.memsize = _KB_bytes(544); /* 544 = 512KB graphics + 32KB text */
    break;
  case MCH_MCGA:
    if (vga.mem.memsize < _KB_bytes(64))
      vga.mem.memsize = _KB_bytes(64);
  ...
  if (IS_EGA_ARCH && vga.mem.memsize < _KB_bytes(128))
    LOG_MSG("WARNING: EGA 64KB emulation is very experimental and not well "
            "supported");
  ...
  if (vbe_window_granularity != 0 && !IS_PC98_ARCH) {
    ...
    LOG(LOG_VGA, LOG_NORMAL)("Video RAM size %uKB exceeds maximum possible "
                             "%uKB given VBE window granularity, reducing",
                             vga.mem.memsize >> 10, (unsigned int)(sz >> 10ul));

    /* reduce by half, until video memory is reported or larger but not more
     * than 2x */
  ...
  if (want_fm_towns) {
    if (vga.mem.memsize < _KB_bytes(640))
      vga.mem.memsize =
          _KB_bytes(640); /* "640KB of RAM, 512KB VRAM and 128KB sprite RAM" */
  }

  if (!IS_PC98_ARCH)
    SVGA_Setup_Driver(); // svga video memory size is set here, possibly
                         // over-riding the user's selection

  // NTS: This is WHY the memory size must be a power of 2
  vga.mem.memmask = bitop::rounduppow2mask(vga.mem.memsize - 1u);

  LOG(LOG_VGA, LOG_NORMAL)("Video RAM: %uKB (mask 0x%x)", vga.mem.memsize >> 10,
                           (unsigned int)vga.mem.memmask);

  // TODO: If S3 emulation, and linear framebuffer bumps up against the CPU
  // memalias limits,
  //       trim Video RAM to fit (within reasonable limits) or else E_Exit() to
  //       let the user know of impossible constraints.

  mainMenu.get_item("debug_pageflip")
      .check(enable_page_flip_debugging_marker)
      .refresh_item(mainMenu);
  mainMenu.get_item("debug_retracepoll")
      .check(enable_vretrace_poll_debugging_marker)
      .refresh_item(mainMenu);
  ...
  if (!IS_PC98_ARCH) {
    ...
    if (machine == MCH_HERC || machine == MCH_MDA) {
      /* alternate table for blend mode, expect mapping 0=black 1-7=gray 8=black
       * 9-f=white for use with blend code that reads this table and sums the
       * output with a delayed version of the same */
      const uint8_t total[2] = {0x0,
                                0x05}; /* blending will yield {0,5,10} which is
                                          sufficient to recreate the effect */
      for (Bitu i = 0; i < 16u; i++) {
        HercBlend_2_Table[i] =
#ifdef WORDS_BIGENDIAN
            ((Bitu)total[(i >> 0u) & 1u] << 0u) |
  ...
  Section_prop *section2 =
      static_cast<Section_prop *>(control->GetSection("vsync"));
...
void VGA_OnEnterPC98(Section *sec) {
...

  {
    for (unsigned int i = 0; i < 8; i++) {
      ...
      if (GFX_bpp >= 24) /* FIXME: Assumes 8:8:8. What happens when desktops
                            start using the 10:10:10 format? */
        pc98_text_palette[i] =
            ((Bitu)(((Bitu)b << GFX_Bshift) + ((Bitu)g << GFX_Gshift) +
                    ((Bitu)r << GFX_Rshift) + (Bitu)GFX_Amask));
      else {
        /* FIXME: PC-98 mode renders as 32bpp regardless (at this time), so we
         * have to fake 32bpp order */
        /*        Since PC-98 itself has 4-bit RGB precision, it might be best
         * to offer a 16bpp rendering mode, or even just have PC-98 mode stay in
         * 16bpp entirely. */
        if (GFX_Bshift == 0)
          pc98_text_palette[i] =
              (Bitu)(((Bitu)b << 0U) + ((Bitu)g << 8U) + ((Bitu)r << 16U));
        else
          pc98_text_palette[i] =
              (Bitu)(((Bitu)b << 16U) + ((Bitu)g << 8U) + ((Bitu)r << 0U));
      }
    }
  }
  ...
  } else { // Set up 31-KHz mode. Values guessed according to other 640x400
           // modes in int10_modes.cpp.
...
void updateGDCpartitions4(bool enable) {
  ...
  pc98_gdc[GDC_SLAVE].display_partition_mask =
      pc98_allow_4_display_partitions ? 3 : 1;
}
...
void VGA_OnEnterPC98_phase2(Section *sec) {
  ...
  /* GDC 2.5/5.0MHz setting is also reflected in BIOS data area and DIP switch
   * registers */
   ...
   * NTS: On real hardware, doing 8-bit I/O on these ports will often hang the
   * system. */
...
bool debugpollvga_pf_menu_callback(DOSBoxMenu *const xmenu,
...
bool debugpollvga_rtp_menu_callback(DOSBoxMenu *const xmenu,
                                    DOSBoxMenu::item *const menuitem) {
  (void)xmenu;    // UNUSED
  (void)menuitem; // UNUSED
...
void VGA_Init() {
  ...
  for (unsigned int i = 0; i < 8; i++)
    vga.herc.palette[i] = i;
  for (unsigned int i = 8; i < 16; i++)
    vga.herc.palette[i] = i + 0x30;
  ...
  LOG(LOG_MISC, LOG_DEBUG)("Render scaler maximum resolution is %u x %u",
                           SCALER_MAXWIDTH, SCALER_MAXHEIGHT);

  /* the purpose of this is so that, if everything is crammed up at the top to
   * make room for system RAM, the S3 and 3Dfx do not conflict */
  if (IS_VGA_ARCH && svgaCard != SVGA_None)
    assigned_lfb = MEM_HardwareAllocate("VGA", 32ul << 20ul /*32MB*/);
  else
    LOG(LOG_MISC, LOG_DEBUG)(
        "Emulation does not require allocating or assigning any LFB");
...
#ifdef WORDS_BIGENDIAN
    FillTable[i] =
        ((i & 1u) ? 0xff000000u : 0u) | ((i & 2u) ? 0x00ff0000u : 0u) |
        ((i & 4u) ? 0x0000ff00u : 0u) | ((i & 8u) ? 0x000000ffu : 0u);
    TXT_Font_Table[i] =
        ((i & 1u) ? 0x000000ffu : 0u) | ((i & 2u) ? 0x0000ff00u : 0u) |
        ((i & 4u) ? 0x00ff0000u : 0u) | ((i & 8u) ? 0xff000000u : 0u);
#else
    FillTable[i] =
        ((i & 1u) ? 0x000000ffu : 0u) | ((i & 2u) ? 0x0000ff00u : 0u) |
        ((i & 4u) ? 0x00ff0000u : 0u) | ((i & 8u) ? 0xff000000u : 0u);
...
#ifdef WORDS_BIGENDIAN
      Expand16Table[j][i] =
          ((i & 1u) ? 1u << j : 0u) | ((i & 2u) ? 1u << (8u + j) : 0u) |
          ((i & 4u) ? 1u << (16u + j) : 0u) | ((i & 8u) ? 1u << (24u + j) : 0u);
#else
      Expand16Table[j][i] = ((i & 1u) ? 1u << (24u + j) : 0u) |
                            ((i & 2u) ? 1u << (16u + j) : 0u) |
                            ((i & 4u) ? 1u << (8u + j) : 0u) |
                            ((i & 8u) ? 1u << j : 0u);
#endif
  ...
  mainMenu.alloc_item(DOSBoxMenu::item_type_id, "debug_pageflip")
      .set_text("Page flip debug line")
      .set_callback_function(debugpollvga_pf_menu_callback);
  mainMenu.alloc_item(DOSBoxMenu::item_type_id, "debug_retracepoll")
      .set_text("Retrace poll debug line")
      .set_callback_function(debugpollvga_rtp_menu_callback);
...
void JEGA_writeFont() {
  ...
  if ((chr >= 0x40 && chr <= 0x7e) || (chr >= 0x80 && chr <= 0xfc)) {
    ...
    if (jega.fontIndex < 16)
      ...
      jfont_dbcs_16[chr * 32 + (jega.fontIndex - 16) * 2 + 1] =
          jega.RDFAP; // w16xh16 font
  } else {
    if (jega.fontIndex >= 19)
      jega.fontIndex = 0;
...
void write_p3d5_jega(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0xbf: // font r/w register
    ...
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:JEGA:Write to illegal index %2X",
                                 (unsigned int)reg);
    break;
  }
}
// CRT Control Register can be read from I/O 3D5h, after setting index at I/O
// 3D4h
Bitu read_p3d5_jega(Bitu reg, Bitu iolen) {
  ...
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:JEGA:Read from illegal index %2X",
                                 (unsigned int)reg);
    break;
  }
  return 0x0;
...
private:
  void getBytes(std::ostream &stream) override {
    ...
    if (vga.tandy.draw_base == vga.mem.linear)
      tandy_drawbase_idx = 0xffffffff;
    else
      tandy_drawbase_idx = vga.tandy.draw_base - MemBase;

    if (vga.tandy.mem_base == vga.mem.linear)
      tandy_membase_idx = 0xffffffff;
    else
      tandy_membase_idx = vga.tandy.mem_base - MemBase;
    ...
    // - 'new' data
    // WRITE_POD_SIZE( vga.fastmem_orgptr, sizeof(uint8_t) * ((vga.vmemsize <<
    // 1) + 4096 + 16) );
    ...
    // - 'new' data
    WRITE_POD_SIZE(vga.changes.map,
                   sizeof(uint8_t) * (VGA_MEMORY >> VGA_CHANGE_SHIFT) + 32);
  ...
  void setBytes(std::istream &stream) override {
    ...
    // - 'new' data
    // READ_POD_SIZE( vga.fastmem_orgptr, sizeof(uint8_t) * ((vga.vmemsize << 1)
    // + 4096 + 16) );
    ...
    if (tandy_drawbase_idx == 0xffffffff)
      vga.tandy.draw_base = vga.mem.linear;
    else
      vga.tandy.draw_base = MemBase + tandy_drawbase_idx;

    if (tandy_membase_idx == 0xffffffff)
      vga.tandy.mem_base = vga.mem.linear;
    else
      vga.tandy.mem_base = MemBase + tandy_membase_idx;
  }
} dummy;
} // namespace
