...
Bitu vga_read_p3d4(Bitu port, Bitu iolen) {
  ...
  /* NOTES: Paradise/Western Digital SVGA decodes only bits [5:0] inclusive and
   * repeat every 0x40 */
...
void vga_write_p3d5(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0x01: /* Horizontal Display End Register */
    ...
    if (val != crtc(horizontal_display_end)) {
      /* we permit a configuration option that if set, means do NOT call
       * VGA_StartResize() if the new value is less than the current value, to
       * protect against rapid changes from demos like DoWhackaDo. */
      if (vga_ignore_hdispend_change_if_smaller &&
          val < crtc(horizontal_display_end)) {
        /* do not call VGA_StartResize, allow change */
        crtc(horizontal_display_end) = (uint8_t)val;
        LOG_MSG("VGA Horz. Display End: accepting change but will not call "
                "VGA_StartResize()");
      } else {
        crtc(horizontal_display_end) = (uint8_t)val;
        VGA_StartResize();
  ...
  case 0x03: /* End Horizontal Blanking Register */
    ...
    /*
       ...
       Horizontal Total has been reached. 7	Access to Vertical Retrace
       registers if set. If clear reads to 3d4h index 10h and 11h access the
       Lightpen read back registers ??
    */
    break;
  case 0x04: /* Start Horizontal Retrace Register */
    if (crtc(read_only))
      break;
    if (crtc(start_horizontal_retrace) ^ (uint8_t)val)
      dbg_event_hretrace = true;
    crtc(start_horizontal_retrace) = (uint8_t)val;
    /*	0-7  Horizontal Retrace starts when the Character Counter reaches this
     * value. */
    break;
  case 0x05: /* End Horizontal Retrace Register */
    ...
    /*
       ...
       counter equals this value. 5-6	Number of character clocks to delay
       start of display after Horizontal Retrace. 7	bit 5 of the End
       Horizontal Blanking count (See 3d4h index 3 bit 0-4)
    */
    break;
  case 0x06: /* Vertical Total Register */
    ...
    /*	0-7	Lower 8 bits of the Vertical Total. Bit 8 is found in 3d4h index
       7 bit 0. Bit 9 is found in 3d4h index 7 bit 5. Note: For the VGA this
       value is the number of scan lines in the display -2.
    */
    break;
  case 0x07: /* Overflow Register */
    ...
    vga.config.line_compare = (vga.config.line_compare & 0x6ff) | (val & 0x10)
                                                                      << 4;
    if (crtc(read_only))
      break;
  ...
  case 0x08: /* Preset Row Scan Register */
    ...
    //		LOG_DEBUG("Skip lines %d bytes
    //%d",vga.config.hlines_skip,vga.config.bytes_skip);
    /*
            0-4	Number of lines we have scrolled down in the first character
       row. Provides Smooth Vertical Scrolling.b 5-6	Number of bytes to skip
       at the start of scanline. Provides Smooth Horizontal Scrolling together
       with the Horizontal Panning Register (3C0h index 13h).
    */
    break;
  case 0x09: /* Maximum Scan Line Register */
  {
    ...
    /*
       0-4	Number of scan lines in a character row -1. In graphics modes
       this is the number of times (-1) the line is displayed before passing on
       to the next line (0: normal, 1: double, 2: triple...). This is
       independent of bit 7, except in CGA modes which seems to require this
       field to be 1 and bit 7 to be set to work. 5	Bit 9 of Start Vertical
       Blanking 6	Bit 9 of Line Compare Register 7	Doubles each
       scan line if set. I.e. displays 200 lines on a 400 display.
       */
    if ((old ^ val) & 0x9F /*number of lines+double*/)
      dbg_event_maxscan = true;
    break;
  }
  ...
  case 0x0C: /* Start Address High Register */
    ...
    vga.config.display_start =
        (vga.config.display_start & 0xFF00FF) | (val << 8);
  ...
  case 0x0E: /*Cursor Location High Register */
    ...
    /* S3 86C928 emulation: In "enhanced mode" (256-color SVGA), this register
     * is also used to hold the hardware cursor foreground color index
     * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%2086C928%20GUI%20Accelerator%20%281992%2d09%29%2epdf]
     */
    break;
  case 0x0F: /* Cursor Location Low Register */
    ...
    /* S3 86C928 emulation: In "enhanced mode" (256-color SVGA), this register
     * is also used to hold the hardware cursor background color index
     * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%2086C928%20GUI%20Accelerator%20%281992%2d09%29%2epdf]
     */
    break;
  case 0x10: /* Vertical Retrace Start Register */
    ...
    /*
            0-7	Lower 8 bits of Vertical Retrace Start. Vertical Retrace starts
       when the line counter reaches this value. Bit 8 is found in 3d4h index 7
  ...
  case 0x11: /* Vertical Retrace End Register */
    ...
    /*
       ...
       equals this value. 4	if clear Clears pending Vertical Interrupts. 5
       Vertical Interrupts (IRQ 2) disabled if set. Can usually be left
                    disabled, but some systems (including PS/2) require it to be
       enabled. 6	If set selects 5 refresh cycles per scanline rather
       than 3. 7	Disables writing to registers 0-7 if set 3d4h index 7
       bit 4 is not affected by this bit.
    */
    break;
  case 0x12: /* Vertical Display End Register */
    if (val != crtc(vertical_display_end)) {
      if (abs(static_cast<int>((Bits)val - (Bits)crtc(vertical_display_end))) <
          3) {
    ...
    /*
            0-7	Lower 8 bits of Vertical Display End. The display ends when the
       line counter reaches this value. Bit 8 is found in 3d4h index 7 bit 1.
  ...
  case 0x13: /* Offset register */
    if (vga_render_on_demand)
      VGA_RenderOnDemandUpTo();
    if (crtc(offset) ^ val)
      dbg_event_scanstep = true;
    crtc(offset) = (uint8_t)val;
    vga.config.scan_len &= 0x300;
    vga.config.scan_len |= val;
    VGA_CheckScanLength();
    /*
            0-7	Number of bytes in a scanline / K. Where K is 2 for byte mode, 4
       for word mode and 8 for Double Word mode.
    */
    break;
  case 0x14: /* Underline Location Register */
    ...
    /*
       ...
       clock. 6	Double Word mode addressing if set
    */
    break;
  ...
  case 0x16: /*  End Vertical Blank Register */
    ...
    /*
            0-6	Vertical blanking stops when the lower 7 bits of the line
       counter equals this field. Some SVGA chips uses all 8 bits! IBM actually
       says bits 0-7.
    */
    break;
  case 0x17: /* Mode Control Register */
    ...
    /*
            0	If clear use CGA compatible memory addressing system
                    by substituting character row scan counter bit 0 for address
       bit 13, thus creating 2 banks for even and odd scan lines. 1	If clear
       use Hercules compatible memory addressing system by substituting
       character row scan counter bit 1 for address bit 14, thus creating 4
       banks. 2	If set increase scan line counter only every second line. 3
       If set increase memory address counter only every other character clock.
            5	When in Word Mode bit 15 is rotated to bit 0 if this bit is set
       else bit 13 is rotated into bit 0. 6	If clear system is in word mode.
       Addresses are rotated 1 position up bringing either bit 13 or 15 into bit
       0. 7	Clearing this bit will reset the display system until the bit is
       set again.
    */
    break;
  case 0x18: /* Line Compare Register */
    ...
    /*
            0-7	Lower 8 bits of the Line Compare. When the Line counter reaches
       this value, the display address wraps to 0. Provides Split Screen
  ...
  default:
    if (svga.write_p3d5) {
      ...
      LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:Write to unknown index %X",
                                   crtc(index));
    }
    break;
  }
...
Bitu vga_read_p3d5x(Bitu port, Bitu iolen) {
  ...
  default:
    if (svga.read_p3d5) {
      ...
      LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:Read from unknown index %X",
                                   crtc(index));
      return 0x0;
    }
  }
