...
static void DISNEY_analyze(Bitu channel) {
  ...
  case DS_FINISH: {
    ...
    // find leader channel (the one with higher rate) [this good for the stereo
    // case?]
    ...
    for (Bitu i = 0; i < 2; i++) {
      if (disney.da[i].used > 1 /*avoid divide-by-zero!*/) {
        ch_speed[i] =
            (Bitu)(1.0 / ((disney.da[i].speedcheck_sum / 1000.0) /
...
static void DISNEY_CallBack(Bitu len) {
  ...
  if (disney.stereo) {
    ...
    if (disney.da[1].used < real_used)
      real_used = disney.da[1].used;
  } else
  ...
  if (real_used >= len) { // enough data for now
    ...
    if (disney.stereo) {
      ...
      memset(disney.da[0].buffer + real_used, gapfiller0, len - real_used);
      memset(disney.da[1].buffer + real_used, gapfiller1, len - real_used);
      ...
      // LOG_MSG("gapfiller %x, fill len %d, realused
      // %d",gapfiller,len-real_used,real_used);
...
private:
  ...
  // FIXME: this conflicts with MPU-401, is this really needed?
  // //IO_WriteHandleObject WriteHandler_cvm; MixerObject MixerChan;
public:
  DISNEY(Section *configuration) : Module_base(configuration) {
    // This is determined by DISNEY_Init() and DISNEY_ShoudldInit() now.
    // We may be initialized by the parallel port emulation if the user has
    // "parallel1=disney" in their dosbox.conf
    //		Section_prop * section=static_cast<Section_prop
    //*>(configuration); 		if(!section->Get_bool("disney")) return;
...
Bitu DISNEY_BasePort() { return DISNEY_BASE; }
...
bool DISNEY_HasInit() { return (test != NULL); }

// parallel port init code will call this depending on port allocation and
// config.
void DISNEY_Init(unsigned int base_port) {
  if (test == NULL) {
    DISNEY_BASE = base_port;
    LOG(LOG_MISC, LOG_DEBUG)("Allocating Disney Sound emulation on port %xh",
                             DISNEY_BASE);
...
void POD_Save_Disney(std::ostream &stream) {
  ...
  for (int lcv = 0; lcv < 2; lcv++) {
    if (disney.leader == &disney.da[lcv]) {
      dac_leader_idx = lcv;
      break;
    }
  }
...
void POD_Load_Disney(std::istream &stream) {
  ...
  disney.leader = NULL;
  if (dac_leader_idx != 0xff)
    disney.leader = &disney.da[dac_leader_idx];
