...
ECBClass::ECBClass(uint16_t segment, uint16_t offset) {
  assert(!dos_kernel_disabled); // Do NOT touch guest memory if the DOSBox
  ...
  LOG_IPX("ECB: SN%7d created.   Number of ECBs: %3d, ESR %4x:%4x, ECB %4x:%4x",
          ...
          real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 6),
          real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 4), segment, offset);
#endif
  isInESRList = false;
  prevECB = NULL;
  nextECB = NULL;
...
RealPt ECBClass::getESRAddr(void) {
  assert(!dos_kernel_disabled); // Do NOT touch guest memory if the DOSBox
                                // kernel is not running!
  return RealMake(real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 6),
                  real_readw(RealSeg(ECBAddr), RealOff(ECBAddr) + 4));
}
...
static void DumpAllSockets(void) {
   ...
   * Managing IPX socket state in memory is not a good idea once a guest OS is
   * running. */
...
static void CloseSocket(void) {
  ...
  for (i = 0; i < socketCount - 1; i++) {
    if (opensockets[i] == sockNum) {
      ...
      memcpy(&opensockets[i], &opensockets[i + 1],
             size_t(SOCKTABLESIZE - (i + 1)));
      break;
    }
  }
  --socketCount;
  ...
  while (tmpECB) {
    ...
    if (tmpECB->getSocket() == sockNum) {
      ...
      /* If we're about to delete the head of the list, update ECBList to the
       * next entry to avoid possible use-after-free bugs. If the next entry is
       * NULL, then the list is now empty. */
      if (ECBList == tmpECB)
        ECBList = tmp2ECB;
...
void DisconnectFromServer(bool unexpected) {
  if (unexpected)
    LOG_MSG("IPX: Server disconnected unexpectedly");
  if (incomingPacket.connected) {
...
void ethernetSendToIPX(const unsigned char *outptr, unsigned int outlen,
                       unsigned int vari) {
  UDPpacket outPacket;
  Bits result;
...
static void sendPacket(ECBClass *sendecb) {
  ...
  for (i = 0; i < fragCount; i++) {
    ...
    if (i == 0) {
      ...
      // Source socket
      real_writew(tmpFrag.segment, tmpFrag.offset + 28,
                  swapByte(sendecb->getSocket()));
    ...
    for (t = 0; t < tmpFrag.size; t++) {
      ...
      if (packetsize >= IPXBUFFERSIZE) {
        LOG_MSG("IPX: Packet size to be sent greater than %d bytes.",
                IPXBUFFERSIZE);
  ...
  real_writew(tmpFrag.segment, tmpFrag.offset + 2,
              swapByte((uint16_t)packetsize));
  ...
  addrptr = (uint8_t *)&localIpxAddr.netnum;
  for (Bitu m = 0; m < 4; m++) {
    if (addrptr[m] != outbuffer[m + 0x6])
      isloopback = false;
  }
  addrptr = (uint8_t *)&localIpxAddr.netnode;
  for (Bitu m = 0; m < 6; m++) {
    if (addrptr[m] != outbuffer[m + 0xa])
      isloopback = false;
    if (immedAddr[m] != 0xff)
  ...
  if (isloopback || islocalbroadcast) {
    ...
    LOG_IPX("Packet back: loopback:%d, broadcast:%d", isloopback,
            islocalbroadcast);
  }
  sendecb->NotifyESR();
}
    ...
    if (ipxClientSocket) {
      ...
      if (NE2K_IsInit()) {
        ...
        if (NE2K_GetMacAddress(regHeaderRaw + 30)) {
          ...
          LOG(LOG_MISC, LOG_DEBUG)("IPX: NE2000 emulation active, using NE2000 "
                                   "MAC address if supported by IPX server.");
      ...
      if (!numsent) {
        ...
        while (true) {
          ...
          result = SDLNet_UDP_Recv(ipxClientSocket, &regPacket);
          if (result != 0) {
            memcpy(localIpxAddr.netnode, regHeader.dest.addr.byNode.node,
                   sizeof(localIpxAddr.netnode));
            memcpy(localIpxAddr.netnum, regHeader.dest.network,
...
public:
  void HelpCommand(const char *helpStr) {
    ...
    if (strcasecmp("connect", helpStr) == 0) {
      WriteOut("IPXNET CONNECT opens a connection to an IPX tunneling server "
               "running on another\n");
      WriteOut("DOSBox-X session.  The \"address\" parameter specifies the IP "
               "address or the host\n");
      WriteOut("name of the server computer.  One can also specify the UDP "
               "port to use.\n");
      WriteOut("By default IPXNET uses port 213, the assigned IANA port for "
               "IPX tunneling, for\nits connection.\n\n");
    ...
    if (strcasecmp("disconnect", helpStr) == 0) {
      WriteOut("IPXNET DISCONNECT closes the connection to the IPX tunneling "
               "server.\n\n");
    ...
    if (strcasecmp("startserver", helpStr) == 0) {
      WriteOut("IPXNET STARTSERVER starts and IPX tunneling server on this "
               "DOSBox-X session.\n");
      WriteOut("By default, the server will accept connections on UDP port "
               "213, though this\n");
      WriteOut("can be changed.  Once the server is started, DOSBox-X will "
               "automatically\n");
    ...
    if (strcasecmp("stopserver", helpStr) == 0) {
      WriteOut("IPXNET STOPSERVER stops the IPX tunneling server running on "
               "this DOSBox-X\nsession.");
      WriteOut("  Care should be taken to ensure that all other connections "
               "have\nterminated ");
      WriteOut("as well since stopping the server may cause lockups on "
    ...
    if (strcasecmp("ping", helpStr) == 0) {
      WriteOut("IPXNET PING broadcasts a ping request through the IPX tunneled "
               "network.  In    \n");
      WriteOut("response, all other connected computers will respond to the "
               "ping and report\n");
    ...
    if (strcasecmp("status", helpStr) == 0) {
      WriteOut("IPXNET STATUS reports the current state of this DOSBox-X's "
               "sessions IPX\n");
      WriteOut("tunneling network.  For a list of the computers connected to "
               "the network use\n");
  ...
  void Run(void) override {
    ...
    if (!cmd->GetCount()) {
      ...
      WriteOut("IPXNET [ CONNECT | DISCONNECT | STARTSERVER | STOPSERVER | "
               "PING | HELP |\n         STATUS ]\n\n");
      return;
    }
    ...
    if (cmd->FindCommand(1, temp_line)) {
      if (strcasecmp("help", temp_line.c_str()) == 0) {
        if (!cmd->FindCommand(2, temp_line)) {
          ...
          WriteOut("IPXNET CONNECT        IPXNET DISCONNECT       IPXNET "
                   "STARTSERVER\n");
          WriteOut("IPXNET STOPSERVER     IPXNET PING             IPXNET "
                   "STATUS\n\n");
      ...
      if (strcasecmp("startserver", temp_line.c_str()) == 0) {
        if (!isIpxServer) {
          if (incomingPacket.connected) {
            WriteOut("IPX Tunneling Client already connected to another "
          ...
          if (startsuccess) {
            ...
            if (udpPort < 1024)
              WriteOut("Try a port number above 1024. See IPXNET HELP CONNECT "
                       "on how to specify a port.\n");
          }
        } else {
      ...
      if (strcasecmp("stopserver", temp_line.c_str()) == 0) {
        if (!isIpxServer) {
          WriteOut(
              "IPX Tunneling Server not running in this DOSBox-X session.\n");
        } else {
          isIpxServer = false;
...
void IPXNET_ProgramStart(Program **make) { *make = new IPXNET; }

Bitu IPX_ESRHandler(void) {
  LOG_IPX("ESR: >>>>>>>>>>>>>>>");
  while (ESRList != NULL) {
    // LOG_IPX("ECB: SN%7d notified.", ESRList->SerialNumber);
    if (ESRList->databuffer)
      ESRList->writeData();
...
public:
  IPX(Section *configuration) : Module_base(configuration) {
    ...
    if (!dospage)
      dospage = DOS_GetMemory(2, "IPX dospage"); // can not be freed yet
  ...
  void RemoveISR(void) {
     ...
     * The most likely reason for this call is that the emulator is booting a
     * guest OS. In that case it is pointless to hold onto dospage because DOS
     * memory allocation has no meaning once a guest OS is running. */
    if (dospage != 0) {
  ...
  ~IPX() {
    ...
    //        We should do this elsewhere, such as booting a guest OS or "power
    //        off"
    ...
    if (addipx)
      VFILE_Remove("IPXNET.COM", "SYSTEM");
  }
};
...
void IPX_DOSExit(Section *) {
  if (test != NULL) {
    ...
    if ((incomingPacket.connected || isIpxServer) && NE2K_IsInit()) {
      LOG(LOG_MISC, LOG_DEBUG)("IPX: Leaving connection open and switching on "
                               "NE2000 IPX redirection");
...
void IPX_Init() {
  ...
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(IPX_DOSBeginExit));
  AddVMEventFunction(VM_EVENT_DOS_EXIT_KERNEL,
                     AddVMEventFunctionFuncPair(IPX_DOSExit));
}
