...
// TODO #ifdef WIN32 and not HX_DOS, a Win32 HANDLE to the memory file and a
// HANDLE to the memory map object. Memory mapping a file in Windows is
// completely different from Linux/Mac OS/etc.
//
// TODO: When above 4GB mapping is available, there will either be two memory
// mappings or the memory map will
//       just reflect physical memory layout, not yet decided. If we're able to
//       do sparse files on most systems the 64MB gap in memory will be empty
//       file space that doesn't take up disk space.
//
// TODO: If we're going to allow the memory file as a means for cheat/hack
// programs to alter guest memory,
//       then there needs to be an API here to flush modified pages to disk. Or
//       at least provide one so that when you open the debugger, modified pages
//       are flushed to disk for your analysis.
...
public:
  ACPIPageHandler()
      : PageHandler(PFLAG_NOCODE | PFLAG_READABLE | PFLAG_WRITEABLE) {}
  ...
  HostPt GetHostReadPt(PageNum phys_page) override {
    ...
    if (phys_page >= (ACPI_buffer_size >> 12))
      phys_page = (ACPI_buffer_size >> 12) - 1;
...
PageHandler *acpi_memio_cb(MEM_CalloutObject &co, Bitu phys_page) {
  ...
  if (ACPI_buffer != NULL && ACPI_REGION_SIZE != 0 &&
      phys_page >= (ACPI_BASE / 4096) &&
      phys_page < ((ACPI_BASE + ACPI_REGION_SIZE) / 4096))
...
void ACPI_mem_enable(const bool enable) {
  if (enable && !acpi_mem_setup) {
    if (ACPI_BASE != 0 && ACPI_REGION_SIZE != 0) {
      MEM_SetPageHandler(ACPI_BASE / 4096, ACPI_REGION_SIZE / 4096,
                         &acpi_mem_handler);
      acpi_mem_setup = true;
      PAGING_ClearTLB();
    }
...
uint32_t MEM_get_address_bits() { return memory.address_bits; }

uint32_t MEM_get_address_bits4GB() { /* some code cannot yet handle values
                                        larger than 32 */
...
/* WARNING: When DOSBox-X enables emulation of more than 4GB of RAM, this
 * MemBase and MemSize will only reflect the memory below 4GB. Which means
 * phys_readx/writex(), which are limited to the first 4GB anyway (32-bit
 * addresses), cannot be used to poke at memory above 4GB. Instead of extending
 * MemBase and the singular allocation block to 4GB or larger, the memory above
 * 4GB is a different block. The reason for this is that the gap that needs to
 * be left open for PCI devices and the ROM BIOS is large enough that such an
 * arrangement would lead to the waste of about 64MB of emulator memory, which
 * is significant, while the 384KB wasted at the 8086 1MB limit is too small to
 * worry about. */
HostPt MemBase = NULL;
size_t MemSize = 0;
...
public:
  ...
  uint8_t readb(PhysPt addr) override {
    (void)addr;
#if C_DEBUG
    LOG_MSG("Warning: Illegal read from %lx (lin=%x), CS:IP %8x:%8x",
            (unsigned long)PAGING_GetPhysicalAddress64(addr), addr,
            SegValue(cs), reg_eip);
#else
    static Bits lcount = 0;
    if (lcount < 1000) {
      lcount++;
      // LOG_MSG("Warning: Illegal read from %x, CS:IP
      // %8x:%8x",addr,SegValue(cs),reg_eip);
    }
#endif
    return 0xFF; /* Real hardware returns 0xFF not 0x00 */
  }
  void writeb(PhysPt addr, uint8_t val) override {
    ...
    (void)val;  // UNUSED
#if C_DEBUG
    LOG_MSG("Warning: Illegal write to %lx (lin=%x), CS:IP %8x:%8x",
            (unsigned long)PAGING_GetPhysicalAddress64(addr), addr,
            SegValue(cs), reg_eip);
#else
    static Bits lcount = 0;
    if (lcount < 1000) {
      lcount++;
      // LOG_MSG("Warning: Illegal write to %x, CS:IP
      // %8x:%8x",addr,SegValue(cs),reg_eip);
    }
#endif
  }
...
public:
  ...
  HostPt GetHostReadPt(PageNum phys_page) override {
    if (!a20_fast_changeable ||
        (phys_page & (~0xFul /*64KB*/)) == 0x100ul /*@1MB*/)
      return MemBase +
             (phys_page & memory.mem_alias_pagemask_active) * MEM_PAGESIZE;
...
public:
  ROMAliasPageHandler() { flags = PFLAG_READABLE | PFLAG_HASROM; }
  HostPt GetHostReadPt(PageNum phys_page) override {
...
public:
  ROMPageHandler() { flags = PFLAG_READABLE | PFLAG_HASROM; }
  void writeb(PhysPt addr, uint8_t val) override {
    if (IS_PC98_ARCH &&
        (addr & ~0x7FFF) == 0xE0000u) { /* Many PC-98 games and programs will
                                           zero 0xE0000-0xE7FFF whether or not
                                           the 4th bitplane is mapped */
    } else
      LOG(LOG_CPU,
          LOG_ERROR)("Write %x to rom at lin=%x phys=%llx", (int)val, (int)addr,
                     (unsigned long long)PAGING_GetPhysicalAddress64(addr));
  }
...
PageHandler &Get_ROM_page_handler(void) { return rom_page_handler; }

extern bool pcibus_enable;

template <enum MEM_Type_t iotype>
static unsigned int MEM_Gen_Callout(Bitu &ret, PageHandler *&f, Bitu page) {
  ...
  while (scan < vec.size()) {
    ...
    if (!obj.isInstalled())
      continue;
    if (obj.m_handler == NULL)
      continue;
    if (!obj.MatchPage(page))
      continue;
    ...
    if (t_f != NULL) {
      if (match == 0) {
        ...
        /* device conflict! */
        /* TODO: to handle it properly, we would need to know whether this was a
         * memory read or memory write, and then have some way for the slow mem
         * page handler to call each page handler one by one and either write to
         * each or read from each and combine. */
        /* TODO: as usual, if iotype is PCI, multiple writes are permitted, but
         * break out after finding one match for read. */
        break;
      }
      match++;
    }
...
static unsigned int MEM_Motherboard_Callout(Bitu &ret, PageHandler *&f,
                                            Bitu page) {
...
static PageHandler *MEM_SlowPath(Bitu page) {
  ...
  if (page < memory.reported_pages) {
    if (page >= 0xf00 && page <= 0xfff &&
        isa_memory_hole_15mb) { /* 0xF00000-0xFFFFFF (15MB-16MB) */
                                /* ignore, ISA memory hole */
    } else {
      LOG(LOG_MISC,
          LOG_WARN)("MEM_SlowPath called within system RAM at page %x",
                    (unsigned int)page);
  ...
  LOG(LOG_MISC, LOG_DEBUG)("MEM slow path page=%x: device matches=%u",
                           (unsigned int)page, (unsigned int)match);
  if (match <= 1)
    memory.phandlers[page] = f;

  return f;
}

void MEM_RegisterHandler(Bitu phys_page, PageHandler *handler,
                         Bitu page_range) {
...
void MEM_InvalidateCachedHandler(Bitu phys_page, Bitu range) {
  ...
  while (range--)
    memory.phandlers[phys_page++] = NULL;
}
...
void MEM_CalloutObject::InvalidateCachedHandlers(void) {
  ...
  /* for both the base page, as well as its aliases, revert the pages back to
   * "slow path" */
  for (p = m_base; p < memory.handler_pages; p += alias_mask + 1)
    MEM_InvalidateCachedHandler(p, range_mask + 1);
}

void MEM_CalloutObject::Install(Bitu page,
                                Bitu pagemask /*MEMMASK_ISA_10BIT, etc.*/,
                                MEM_CalloutHandler *handler) {
  if (!installed) {
    if (pagemask == 0 || (pagemask & ~0xFFFFFFFU)) {
      LOG(LOG_MISC,
          LOG_ERROR)("MEM_CalloutObject::Install: Page mask %x is invalid",
                     (unsigned int)pagemask);
      return;
    }

    /* we need a mask for the distance between aliases of the port, and the
     * range of I/O ports. */
    /* only the low part of the mask where bits are zero, not the upper.
     * This loop is the reason portmask cannot be ~0 else it would become an
     * infinite loop. This also serves to check that the mask is a proper
     * combination of ISA masking and I/O port range (example: IOMASK_ISA_10BIT
     * & (~0x3) == 0x3FF & 0xFFFFFFFC = 0x3FC for a device with 4 I/O ports). A
     * proper mask has (from MSB to LSB):
     ...
     *   - zero or more 0 bits to LSB */
    {
      ...
      /* DEBUG */
      if ((pagemask & range_mask) != 0 || ((range_mask + 1) & range_mask) != 0 /* should be a mask, therefore AND by itself + 1 should be zero (think (0xF + 1) & 0xF = 0x10 & 0xF = 0x0) */) {
        LOG(LOG_MISC, LOG_ERROR)(
            "MEM_CalloutObject::Install: pagemask(%x) & range_mask(%x) != 0 "
            "(%x). You found a corner case that broke this code, fix it.",
            (unsigned int)pagemask, (unsigned int)range_mask,
      ...
      test =
          pagemask + range_mask; /* will break if portmask & range_mask != 0 */
      while ((test & m) == m) {
        alias_mask = m;
        m = (m << 1) + 1;
      }
      ...
      /* Example:
       *
       *    Sound Blaster at port 220-22Fh with 10-bit ISA decode would be
       * 0x03F0 therefore: portmask =    0x03F0 range_mask =  0x000F alias_mask
       * =  0x03FF
       *
      ...
      if ((pagemask ^ range_mask ^ alias_mask) != 0 || ((alias_mask + 1) & alias_mask) != 0 /* should be a mask, therefore AND by itself + 1 should be zero */) {
        LOG(LOG_MISC, LOG_ERROR)(
            "MEM_CalloutObject::Install: pagemask(%x) ^ range_mask(%x) ^ "
            "alias_mask(%x) != 0 (%x). Invalid portmask.",
            (unsigned int)pagemask, (unsigned int)range_mask,
      ...
      if (page & range_mask) {
        LOG(LOG_MISC, LOG_ERROR)("MEM_CalloutObject::Install: page %x and page "
                                 "mask %x not aligned (range_mask %x)",
                                 (unsigned int)page, (unsigned int)pagemask,
     ...
     * don't register any I/O handlers. those will be registered during the
     * "slow path" callout process when the CPU goes to access them. to
     * encourage that to happen, we invalidate the I/O ranges */
    LOG(LOG_MISC,
        LOG_DEBUG)("MEM_CalloutObject::Install added device with page=0x%x "
                   "mem_mask=0x%x rangemask=0x%x aliasmask=0x%x",
                   (unsigned int)page, (unsigned int)mem_mask,
                   (unsigned int)range_mask, (unsigned int)alias_mask);
 ...
 * if they need to touch it, they get a pointer, which they then have to put
 * back. The way DOSBox/DOSbox-X code handles MEM callbacks it's common to
 * declare an MEM object, call the install, but then never touch it again, so
 * this should work fine.
 *
 * this allows us to maintain ready-made MEM callout objects to return quickly
 * rather than write more complicated code where the caller has to make an
 * MEM_CalloutObject and then call install and we have to add its pointer to a
 * list/vector/whatever. It also avoids problems where if we have to resize the
 * vector, the pointers become invalid, because callers have only handles and
 * they have to put all the pointers back in order for us to resize the vector.
 */
...
try_again:
  while (vec.alloc_from < vec.size()) {
    ...
    if (!obj.alloc) {
      ...
      return MEM_Callout_t_comb(
          t,
          vec.alloc_from++); /* make combination, then increment alloc_from */
    }
   ...
   * if anyone has pointers out to our elements, then we cannot resize.
   * vector::resize() may invalidate them. */
  if (vec.size() < 4096 && vec.getcounter == 0) {
    size_t nsz = vec.size() * 2;

    LOG(LOG_MISC, LOG_WARN)("MEM_AllocateCallout type %u expanding array to %u",
                            (unsigned int)t, (unsigned int)nsz);
    vec.alloc_from =
        (unsigned int)vec.size(); /* allocate from end of old vector size */
    vec.resize(nsz);
    goto try_again;
  }

  LOG(LOG_MISC, LOG_WARN)("MEM_AllocateCallout type %u no free entries",
                          (unsigned int)t);
  return MEM_Callout_t_none;
}
...
void MEM_FreeCallout(MEM_Callout_t c) {
  ...
  if (vec.alloc_from > idx)
    vec.alloc_from =
        idx; /* an empty slot just opened up, you can alloc from there */
}
...
PageHandler *lfb_memio_cb(MEM_CalloutObject &co, Bitu phys_page) {
  ...
  if (phys_page >= memory.lfb_mmio.start_page &&
      phys_page < memory.lfb_mmio.end_page)
...
void lfb_mem_cb_init() {
  if (lfb_mem_cb == MEM_Callout_t_none) {
    lfb_mem_cb =
        MEM_AllocateCallout(pcibus_enable ? MEM_TYPE_PCI : MEM_TYPE_ISA);
    if (lfb_mem_cb == MEM_Callout_t_none)
      E_Exit("Unable to allocate mem cb for LFB");
  }
  if (lfb_mmio_cb == MEM_Callout_t_none) {
    lfb_mmio_cb =
        MEM_AllocateCallout(pcibus_enable ? MEM_TYPE_PCI : MEM_TYPE_ISA);
    if (lfb_mmio_cb == MEM_Callout_t_none)
      E_Exit("Unable to allocate mmio cb for LFB");
  }

  {
    ...
    if (memory.lfb.pages != 0) {
      ...
      while (p2sz < memory.lfb.pages)
        p2sz <<= (Bitu)1;
      cb->Install(memory.lfb.start_page,
                  MEMMASK_Combine(MEMMASK_FULL, MEMMASK_Range(p2sz)),
                  lfb_memio_cb);
    }
...

  {
    ...
    if (memory.lfb_mmio.pages != 0) {
      Bitu p2sz = 1;
      /* make p2sz the largest power of 2 that covers the LFB */
      while (p2sz < memory.lfb_mmio.pages)
        p2sz <<= (Bitu)1;
      cb->Install(memory.lfb_mmio.start_page,
                  MEMMASK_Combine(MEMMASK_FULL, MEMMASK_Range(p2sz)),
                  lfb_memio_cb);
    }
...
/* TODO: At some point, this common code needs to be removed and the S3
 * emulation (or whatever else) needs to provide LFB and/or MMIO mapping. */
void MEM_SetLFB(Bitu page, Bitu pages, PageHandler *handler,
  ...
  memory.lfb_mmio.handler = mmiohandler;
  if (mmiohandler != NULL) {
    /* FIXME: Why is this hard-coded? There's more than just S3 emulation in
     * this code's future! */
    if (svgaCard == SVGA_S3Trio &&
        (s3Card == S3_Trio64V || s3Card == S3_Vision868 ||
         s3Card == S3_Vision968 || s3Card == S3_ViRGE ||
         s3Card == S3_ViRGEVX)) {
      /* 64MB BAR. According to the datasheet, this 64MB region is split into
       * two 32MB halves, the lower half "little endian" and the upper half "big
       * endian". Within the 32MB region, the low 16MB is video memory and the
       * high 16MB is MMIO. */
  ...
  if (pages == 0 || page == 0) {
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("MEM: Linear framebuffer is now set to 0x%lx-0x%lx (%uKB)",
    ...
    LOG(LOG_MISC, LOG_DEBUG)(
        "MEM: Linear framebuffer MMIO is now set to 0x%lx-0x%lx (%uKB)",
...
PageHandler *MEM_GetPageHandler(Bitu phys_page) {
  ...
  if (glide.enabled && (phys_page >= (GLIDE_LFB >> 12)) &&
      (phys_page < (GLIDE_LFB >> 12) + GLIDE_PAGES))
    ...
    return MEM_SlowPath(
        phys_page); /* will also fill in phandlers[] if zero or one matches, so
                       the next access is very fast */
  }

  if (phys_page >= 0x100000ul &&
      phys_page < (0x100000ul + (unsigned long)memory.reported_pages_4gb)) {
    assert(memory_file_base != NULL);
...
void mem_strcpy(LinearPt dest, LinearPt src) {
  uint8_t r;
  while ((r = mem_readb(src++)))
    mem_writeb_inline(dest++, r);
  mem_writeb_inline(dest, 0);
}

void mem_memcpy(LinearPt dest, LinearPt src, Bitu size) {
  while (size--)
    mem_writeb_inline(dest++, mem_readb_inline(src++));
}
...
void MEM_BlockWrite(LinearPt pt, const void *data, size_t size) {
  ...
  if ((pt >> 12) == ((pt + size - 1) >> 12)) { // Always same TLB entry
    ...
    MEM_BlockWrite((LinearPt)(pt + current),
                   reinterpret_cast<uint8_t const *>(data) + current,
                   remainder);
  }
}
...
Bitu MEM_TotalPages(void) { return memory.reported_pages; }

Bitu MEM_TotalPagesAt4GB(void) { return memory.reported_pages_4gb; }
...
INLINE uint32_t BestMatch_A20_friendly(Bitu size) {
  ...
  /* if the memory to allocate is more than 1MB this function will never work.
   * give up now. */
  if (size > 0x100)
    return 0;

  /*  TODO: For EMS allocation this would put things in the middle of extended
   * memory space, which would increase possible memory fragmentation! Could we
   * avoid memory fragmentation by instead scanning from the top down i.e. so
   * the EMS system memory takes the top of extended memory and the DOS program
   * is free to gobble up a large continuous range from below? */
  while (index < memory.reported_pages) {
    /* Check if we are searching for first free page */
    if (!first) {
  ...
  if (sequence) {
    ...
    *next = -1;
    ...
    while (pages) {
      ...
      if (!index)
        E_Exit("MEM:corruption during allocate");
...
#if C_DEBUG
    if (index & 0x100)
      E_Exit("MEM_AllocatePages_A20_friendly failed to make sure address has "
             "bit 20 == 0");
    if ((index + pages - 1) & 0x100)
      E_Exit("MEM_AllocatePages_A20_friendly failed to make sure last page has "
             "bit 20 == 0");
#endif
    MemHandle *next = &ret;
    while (pages) {
...
void MEM_ReleasePages(MemHandle handle) {
  if (memory.mhandles == NULL) {
    LOG(LOG_MISC, LOG_WARN)(
        "MEM_ReleasePages() called when mhandles==NULL, nothing to release");
    return;
  }
...
bool MEM_ReAllocatePages(MemHandle &handle, Bitu pages, bool sequence) {
  ...
  if (old_pages > pages) {
    ...
    if (sequence) {
      index = last + 1;
      Bitu free = 0;
      while ((index < (MemHandle)memory.reported_pages) &&
      ...
      if (free >= need) {
        ...
        if (!newhandle)
          return false;
        MEM_BlockCopy((unsigned int)newhandle * 4096u,
                      (unsigned int)handle * 4096u,
                      (unsigned int)old_pages * 4096u);
        MEM_ReleasePages(handle);
        handle = newhandle;
        return true;
      }
...
MemHandle MEM_NextHandle(MemHandle handle) { return memory.mhandles[handle]; }
...
*/
bool MEM_A20_Enabled(void) { return memory.a20.enabled; }
...
void MEM_A20_Enable(bool enabled) {
  ...
  if (!a20_fake_changeable && (memory.mem_alias_pagemask & 0x100ul)) {
    if (memory.a20.enabled)
      memory.mem_alias_pagemask_active |= 0x100ul;
    else
      memory.mem_alias_pagemask_active &= ~0x100ul;
    PAGING_ClearTLB();
  }
...
void mem_unalignedwritew(LinearPt address, uint16_t val) {
  mem_writeb_inline(address, (uint8_t)val);
  val >>= 8u;
...
void mem_unalignedwrited(LinearPt address, uint32_t val) {
  ...
  mem_writeb_inline(address + 1u, (uint8_t)val);
  val >>= 8u;
  mem_writeb_inline(address + 2u, (uint8_t)val);
  val >>= 8u;
...
bool mem_unalignedwritew_checked(LinearPt address, uint16_t val) {
  if (mem_writeb_checked(address, (uint8_t)(val & 0xff)))
    return true;
  val >>= 8;
  if (mem_writeb_checked(address + 1, (uint8_t)(val & 0xff)))
    return true;
  return false;
}
...
bool mem_unalignedwrited_checked(LinearPt address, uint32_t val) {
  ...
  if (mem_writeb_checked(address + 1, (uint8_t)(val & 0xff)))
    return true;
  val >>= 8;
  if (mem_writeb_checked(address + 2, (uint8_t)(val & 0xff)))
    return true;
  val >>= 8;
  if (mem_writeb_checked(address + 3, (uint8_t)(val & 0xff)))
    return true;
  return false;
}

uint8_t mem_readb(const LinearPt address) { return mem_readb_inline(address); }

uint16_t mem_readw(const LinearPt address) { return mem_readw_inline(address); }

uint32_t mem_readd(const LinearPt address) { return mem_readd_inline(address); }
...
void On_Software_286_int15_block_move_return(unsigned char code) {
  ...
  /* read the reset vector from the BIOS data area. this time, it's a stack
   * pointer */
  ...
  if (cmos_reset_type_9_sarcastic_win31_comments) {
    ...
    LOG_MSG(
        "CMOS Shutdown byte 0x%02x says to do INT 15 block move reset "
        "%04x:%04x. Only weirdos like Windows 3.1 use this... NOT WELL TESTED!",
        code, vec_seg, vec_off);
  }

  /* set stack pointer. prepare to emulate BIOS returning from INT 15h block
   * move, 286 style */
  CPU_SetSegGeneral(cs, 0xF000);
  CPU_SetSegGeneral(ss, vec_seg);
  reg_esp = vec_off;

  /* WARNING! WARNING! This is based on what Windows 3.1 standard mode
   * (cputype=286) expects. We need more comprehensive documentation on what
   * actual 286 BIOSes do. This code, order-wise, is a guess! No documentation
   * exists on what actually happens! But so far, this allows Windows 3.1 to run
   * in full Standard Mode when cputype=286 without crashing. Be warned that
   * while it works DOSBox-X will spit a shitload of messages about the
   * triple-fault reset trick it's using. */
...
void On_Software_286_reset_vector(unsigned char code) {
  ...
  /* TODO: If cputype=386 or cputype=486, and A20 gate disabled, treat it as an
   * intentional trick to trigger a reset + invalid opcode exception through
   * which the program can then read the CPU stepping ID register */

  LOG_MSG("CMOS Shutdown byte 0x%02x says to jump to reset vector %04x:%04x",
          code, vec_seg, vec_off);
...
void On_Software_CPU_Reset() {
  ...
  if (custom_bios) {
    ...
    if (PC98_SHUT0) {
      ...
      /* following CPU reset, and coming from the BIOS, CPU registers are
       * trashed */
      /* FIXME: VEM486.EXE appears to use this reset vector trick, then when
       * regaining control, checks to see if DX is 0x00F0 just as it was when it
       * issued the OUT DX,AL instruction. Why? If DX != 0x00F0 it writes
       * whatever DX is to 0000:0486 and then proceeds anyway. */
      reg_eax = 0x2010000;
      reg_ebx = 0x2111;
      reg_ecx = 0;
      reg_edx = 0xABCD;
      ...
      LOG_MSG("PC-98 reset and continue: SS:SP = %04x:%04x", reset_ss,
              reset_sp);
      ...
      LOG_MSG("PC-98 reset and continue: RETF to %04x:%04x", SegValue(cs),
              reg_ip);
    ...
    /* software-initiated CPU reset. but the intent may not be to reset the
     * system but merely the CPU. check the CMOS shutdown byte */
    ...
    case 0x0A: /* JMP double word pointer without EOI */
      ...
      return;
    case 0x09: /* INT 15h block move return to real mode (to appease Windows 3.1
                  KRNL286.EXE and cputype=286, yuck) */
 ...
 * instead pf looking at the BIOS data area. Including homebrew development like
 * KOARMADA.EXE */
...
static Bitu read_PC98_43B_memspace(Bitu /*port*/, Bitu /*iolen*/) {
  ...
  if (isa_memory_hole_15mb ||
      MEM_TotalPages() <= 0xF00 /*15MB or less*/) { /* used by system */
  } else
...
static void write_p92(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (val & 1) {
    if (allow_port_92_reset) {
      ...
      LOG_MSG("WARNING: port 92h written with bit 0 set. Is the guest OS or "
              "application attempting to reset the system?\n");
    }
  }
}
...
static Bitu read_pc98_a20(Bitu port, Bitu iolen) {
  ...
  if (port == 0xF2)
    return (memory.a20.enabled
                ? 0x00
                : 0x01); // bit 0 indicates whether A20 is MASKED, not ENABLED
...
static void write_pc98_a20(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (port == 0xF2) {
    MEM_A20_Enable(
        1); // writing port 0xF2 unmasks (enables) A20 regardless of value
  } else if (port == 0xF6) {
...
bool MEM_unmap_physmem(Bitu start, Bitu end) {
  ...
  if (start >= memory.handler_pages || end >= memory.handler_pages)
    E_Exit("%s: attempt to map pages beyond handler page limit (0x%lx-0x%lx >= "
           "0x%lx)",
           __FUNCTION__, (unsigned long)start, (unsigned long)end,
           (unsigned long)memory.handler_pages);
...
bool MEM_map_RAM_physmem(Bitu start, Bitu end) {
  ...
  for (p = start; p <= end; p++) {
    if (memory.phandlers[p] != NULL &&
        memory.phandlers[p] != &illegal_page_handler &&
        memory.phandlers[p] != &unmapped_page_handler &&
        memory.phandlers[p] != &ram_page_handler)
      return false;
  }
...
/*! \brief          REDOS.COM utility command on drive Z: to trigger restart of
 * the DOS kernel
 */
...
void REDOS_ProgramStart(Program **make) { *make = new REDOS; }
...
public:
  ...
  void Run(void) override {
    if (cmd->FindExist("-?", false) || cmd->FindExist("/?", false)) {
      ...
      WriteOut("A20GATE [ON | OFF | SET [off | off_fake | on | on_fake | mask "
               "| fast]]\n\n"
               "  [ON | OFF | SET] Turns the A20 gate ON/OFF, or sets the A20 "
               "gate mode.\n\n"
               "Type A20GATE with no parameters to display the current A20 "
               "gate status.\n");
      ...
      MEM_A20_Enable(true);
      if (!MEM_A20_Enabled())
        WriteOut("Error: A20 gate cannot be enabled.\n");
    } else if (cmd->FindExist("OFF")) {
      WriteOut("Disabling A20 gate...\n");
      MEM_A20_Enable(false);
      if (MEM_A20_Enabled())
        WriteOut("Error: A20 gate cannot be disabled.\n");
    } else {
...
void A20GATE_ProgramStart(Program **make) { *make = new A20GATE; }
...
void Init_AddressLimitAndGateMask() {
  ...
  // TODO: this option should be handled by the CPU init since it concerns
  // emulation
  //       of older 386 and 486 boards with fewer than 32 address lines:
  //
  //         24-bit addressing on the 386SX vs full 32-bit addressing on the
  //         386DX 26-bit addressing on the 486SX vs full 32-bit addressing on
  //         the 486DX
  //
  //       Also this code should automatically cap itself at 24 for 286
  //       emulation and 20 for 8086 emulation.
  memory.address_bits = (unsigned int)section->Get_int("memalias");

  if (memory.address_bits == 0) {
    /* TODO: We don't know the memsize yet. If memsize is 60GB or more, 40 bits,
     * else 36 bits. Pentium II/III systems are PSE-36 type PSE extensions. For
     * similar reasons for 486, if 60MB or more, 32 bits, else 26 bits. For
     * similar reasons for 386, if 14mB or more, 32 bits, else 24 bits. */
    if (CPU_ArchitectureType >= CPU_ARCHTYPE_PENTIUMII)
      memory.address_bits = 36;
    else if (CPU_ArchitectureType >= CPU_ARCHTYPE_386)
      memory.address_bits = 32; /* NTS: 26 is also valid for 486SX emulation, 24
                                   for 386SX emulation */
  ...
  /* WARNING: Binary arithmetic done with 64-bit integers because under
     Microsoft C++
  ...
  /* log */
  LOG(LOG_MISC, LOG_DEBUG)("Memory: address_bits=%u alias_pagemask=%lx",
                           (unsigned int)memory.address_bits,
...
uint32_t MEM_HardwareAllocate(const char *name, uint32_t sz) {
  ...
  if (sz != 0ul && bitop::ispowerof2(sz)) {
    ...
    if (memory.hw_next_assign < 0xFE000000ul) {
      ...
      LOG(LOG_MISC, LOG_DEBUG)("Device '%s' assigned address 0x%lx-0x%lx which "
                               "it may treat as minimum\n",
                               name, (unsigned long)assign,
                               (unsigned long)assign + (unsigned long)sz - 1ul);
    }
  }

  if (assign == 0)
    LOG(LOG_MISC, LOG_DEBUG)(
        "Unable to assign device '%s' a physical address of size 0x%lx\n", name,
        (unsigned long)sz);
...
bool alloc_mem_file() {
  ...
  if (fstat(memory_file_fd, &st)) {
    LOG_MSG("Cannot fstat memory file I just opened??? Whut? %s",
            strerror(errno));
    free_mem_file();
    return false;
  }
  if (!S_ISREG(st.st_mode)) {
    E_Exit("I was tricked into opening a non-file as a memory file. Don't do "
           "that.");
    free_mem_file();
    return false;
  }
  ...
  if (ftruncate(memory_file_fd, memory_file_size)) {
    LOG_MSG("Cannot truncate file to %lu %s", (unsigned long)memory_file_size,
            strerror(errno));
    free_mem_file();
    return false;
  }
  ...
  memory_file_base =
      mmap(NULL /*no particular address*/, memory_file_size,
           PROT_READ | PROT_WRITE, MAP_SHARED, memory_file_fd, 0 /*offset*/);
  if (memory_file_base == MAP_FAILED) {
    LOG_MSG("Unable to memory map memory file, %s", strerror(errno));
    memory_file_base = NULL; /* MAP_FAILED might be some nonzero value, such as
                                on Linux where it is -1 */
    free_mem_file();
    return false;
  }
...
void free_mem_file() {
  if (memory_file_base != NULL) {
    if (UnmapViewOfFile(memory_file_base) == 0)
      E_Exit("Windows refused to unmap the file view");
    memory_file_base = NULL;
  }
  if (memory_file_map != INVALID_HANDLE_VALUE && memory_file_map != 0) {
    if (CloseHandle(memory_file_map) == 0)
      E_Exit("Windows refused to close the memory file, err=0x%08x",
             (unsigned int)GetLastError());
    memory_file_map = INVALID_HANDLE_VALUE;
  }
  if (memory_file_fd != INVALID_HANDLE_VALUE) {
    if (CloseHandle(memory_file_fd) == 0)
      E_Exit("Windows refused to close the memory file, err=0x%08x",
             (unsigned int)GetLastError());
...
bool alloc_mem_file() {
  ...
  } else if (attr & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_READONLY |
  ...
  memory_file_fd =
      CreateFile(memory_file.c_str(), GENERIC_READ | GENERIC_WRITE,
...

  {
    ...
    sp.SetSparse = TRUE;
    if (DeviceIoControl(memory_file_fd, FSCTL_SET_SPARSE, &sp, sizeof(sp), NULL,
                        0, &retval, NULL) == 0)
...

  {
    ...
    if (SetFilePointer(memory_file_fd, (DWORD)memory_file_size, &hi,
                       FILE_BEGIN) != (DWORD)memory_file_size) {
      free_mem_file();
      return false;
    }
  }
  ...
  memory_file_map = CreateFileMapping(memory_file_fd, NULL, PAGE_READWRITE,
                                      (DWORD)(memory_file_size >> 32ull),
                                      (DWORD)memory_file_size, NULL);
  if (memory_file_map == INVALID_HANDLE_VALUE || memory_file_map == 0) {
  ...
  memory_file_base = MapViewOfFile(memory_file_map, FILE_MAP_ALL_ACCESS, 0, 0,
                                   memory_file_size);
  if (memory_file_base == NULL) {
    const DWORD err = GetLastError();
    free_mem_file();
...
bool alloc_mem_file() { return false; }
#endif
#else
void free_mem_file() {}
...
void Init_RAM() {
  ...
  uint64_t memsizekb = (uint64_t)section->Get_int("memsizekb");
  {
    ...
    if (memsizekb == 0 && memsize < 1)
      memsize = 1;
    else if (memsizekb != 0 && (Bits)memsize < 0)
      memsize = 0;
  ...
  if ((memory.mem_alias_pagemask + 1) !=
      0 /*32-bit integer overflow avoidance*/) {
    uint64_t maxmem;
    ...
    if ((memsizekb / 4) > maxmem) {
      ...
      if (memory.address_bits <= 32)
        LOG_MSG("If you are attempting more than 4GB of RAM, you need to set "
                "memalias to a value larger than 32");
      memsizekb = maxmem * 4;
    }
  }

  {
    ...
    static_assert(sizeof(size_t) >= sizeof(void *),
                  "why is size_t smaller than a pointer?");

    /* Leave 128MB of space at the top for the BIOS, S3 VGA, and Voodoo 3Dfx
     * emulation. There was a known bug 2024/12/21 where setting the maximum
     * memory size and installing Windows XP caused problems because XP would
     * try to use the Voodoo 3Dfx MMIO as memory when enabled at 0xD0000000.
     *
    ...
    /* 2024/12/25: We now allow 4GB or more of RAM! But, to make it work in this
     * codebase, it has to be divided into a region below 4GB and a region above
     * 4GB. */
  ...
  LOG(LOG_MISC, LOG_DEBUG)("Hardware assignment will begin at 0x%lx",
                           (unsigned long)memory.hw_next_assign);
  ...
  // DEBUG message
  LOG(LOG_MISC, LOG_DEBUG)(
      "Memory: %u pages (%uKB) of RAM, %u (%uKB) reported to OS, %u (0x%x) "
      "(%uKB) pages of memory handlers",
      (unsigned int)memory.pages, (unsigned int)memory.pages * 4,
      (unsigned int)memory.reported_pages,
      (unsigned int)memory.reported_pages * 4,
      (unsigned int)memory.handler_pages, (unsigned int)memory.handler_pages,
  ...
  assert(memory.handler_pages >=
         0x100); /* enough for at minimum 1MB of addressable memory */

  /* Allocate the RAM. We alloc as a large unsigned char array. new[] does not
   * initialize the array, so we then must zero the buffer. */
  memory_file_size = size_t(memory.pages) * size_t(4096u);
  if (memory.reported_pages_4gb > 0 && sizeof(void *) > 4) {
    size_t noff = size_t(0x100000000ul) +
                  (size_t(4096u) * size_t(memory.reported_pages_4gb));
    if (memory_file_size < noff)
      memory_file_size = noff;
  }
  if (!memory_file.empty())
    LOG_MSG("Memory file size will be %lluKB",
            (unsigned long long)memory_file_size >> 10ull);
  if (alloc_mem_file()) {
...
#elif C_HAVE_MMAP
    MemBase =
        (uint8_t *)mmap(NULL, memory.pages * 4096u, PROT_READ | PROT_WRITE,
                        MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
    if (MemBase == (uint8_t *)MAP_FAILED)
      E_Exit("Failed to mmap allocate memory");
#else  // C_GAMELINK
  ...
  if (!MemBase)
    E_Exit("Can't allocate main memory of %d KB", (int)memsizekb);
  /* Clear the memory, as new doesn't always give zeroed memory
   * (Visual C debug mode). We want zeroed memory though. */
  if (memory_file_base && memory_file_already_zero) {
    LOG_MSG(
        "Host OS should treat memory map as all zeros, skipping memory clear");
  } else {
    memset((void *)MemBase, 0, memory.reported_pages * 4096);
  }
  /* the rest of "ROM" is for unmapped devices so we need to fill it
   * appropriately */
  ...
  // sanity check. if this condition is false the loops below will overrun the
  // array!
  ...
  /* FIXME: VGA emulation will selectively respond to 0xA0000-0xBFFFF according
   * to the video mode, what we want however is for the VGA emulation to assign
   * illegal_page_handler for address ranges it is not responding to when
   * mapping changes. */
  for (i = 0xa0; i < 0x100;
       i++) /* we want to make sure adapter ROM is unmapped entirely! */
...
void MEM_cut_RAM_up_to(Bitu addr) {
  ...
  if (memory.reported_pages > pages) {
    LOG(LOG_MISC, LOG_DEBUG)("Memory: Reducing RAM to 0x%lx",
                             (unsigned long)addr);

    do {
      memory.phandlers[--memory.reported_pages] = NULL;
...
void ShutDownMemHandles(Section *sec) {
  /* XMS relies on us, so shut it down first to avoid spurious warnings about
   * freeing when mhandles == NULL */
  XMS_ShutDown(NULL);
...
/* this is called on hardware reset. the BIOS needs the A20 gate ON to boot
 * properly on 386 or higher! this temporarily switches on the A20 gate and lets
 * it function as normal despite user settings. BIOS will POST and then permit
 * the A20 gate to go back to whatever emulation setting given in dosbox-x.conf
 */
...
/* this is called after BIOS boot. the BIOS needs the A20 gate ON to boot
 * properly on 386 or higher! */
...
void Init_A20_Gate() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(A20Gate_OnReset));
}
...
void PS2Port92_OnReset(Section *sec) {
  ...
  if (IS_PC98_ARCH) {
    ...
    if (enable_port92) {
      ...
      // TODO: This should exist in the motherboard emulation code yet to come!
      // The motherboard
      //       determines A20 gating, not the RAM!
      LOG(LOG_MISC, LOG_DEBUG)(
          "Port 92h installed, emulating PS/2 system control port A");
...
void Init_PS2_Port_92h() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(PS2Port92_OnReset));
}
...
void Init_MemHandles() {
  ...
  // LOG
  LOG(LOG_MISC, LOG_DEBUG)("Initializing memory handle array (EMS/XMS handle "
                           "management). mem_pages=%lx",
                           (unsigned long)memory.pages);
  ...
  // ISA memory hole awareness (15MB region). Block off 0xF00000-0xFFFFFF with a
  // dummy handle.
  if (isa_memory_hole_15mb) {
    for (i = 0xF00; i <= 0xFFF && i < memory.pages; i++)
      memory.mhandles[i] = 0x7FFFFFFF;
  }
}
...
void Init_MemoryAccessArray() {
  ...
  // LOG
  LOG(LOG_MISC, LOG_DEBUG)("Initializing memory access array (page handler "
                           "callback system). mem_alias_pagemask=%lx",
                           (unsigned long)memory.mem_alias_pagemask);

  // CHECK: address mask init must have been called!
  assert(memory.mem_alias_pagemask >= 0xFF);

  // we maintain a different page count for page handlers because we want to
  // maintain a "cache" of sorts of what device responds to a given memory
  // address.
  ...
  for (i = 0; i < memory.handler_pages;
       i++) // FIXME: This will eventually init all pages to the "slow path" for
            // device lookup
    memory.phandlers[i] = NULL; //&illegal_page_handler;
}
...
Bitu MEM_PageMask(void) { return memory.mem_alias_pagemask; }

Bitu MEM_PageMaskActive(void) { return memory.mem_alias_pagemask_active; }
...
uint8_t physdev_readb(const PhysPt64 addr) {
  ...
  if (ph->flags & PFLAG_READABLE)
    return *((uint8_t *)(ph->GetHostReadPt(pagenum) +
                         ((unsigned int)addr & 0xFFFu)));
...
uint16_t physdev_readw(const PhysPt64 addr) {
  if (((unsigned int)addr & 0xFFFu) <= 0xFFEu) {
    ...
    if (ph->flags & PFLAG_READABLE)
      return *((uint16_t *)(ph->GetHostReadPt(pagenum) +
                            ((unsigned int)addr & 0xFFFu)));
    ...
    paging.tlb.phys_page[pagenum] = (uint32_t)pagenum;
    const uint16_t ch =
        ph->readw((PhysPt)addr); /* WARNING: 4GB wraparound here */
    paging.tlb.phys_page[pagenum] = orig;
    return ch;
  } else {
...
uint32_t physdev_readd(const PhysPt64 addr) {
  if (((unsigned int)addr & 0xFFFu) <= 0xFFCu) {
    ...
    if (ph->flags & PFLAG_READABLE)
      return *((uint32_t *)(ph->GetHostReadPt(pagenum) +
                            ((unsigned int)addr & 0xFFFu)));
    ...
    paging.tlb.phys_page[pagenum] = (uint32_t)pagenum;
    const uint32_t ch =
        ph->readd((PhysPt)addr); /* WARNING: 4GB wraparound here */
    paging.tlb.phys_page[pagenum] = orig;
    return ch;
  } else {
...
void physdev_writeb(const PhysPt64 addr, const uint8_t val) {
  ...
  if (ph->flags & PFLAG_WRITEABLE) {
    *((uint8_t *)(ph->GetHostReadPt(pagenum) + ((unsigned int)addr & 0xFFFu))) =
        val;
  } else {
    /* This hack is necessary because of the weird way that CPU linear addresses
     * make their way down to the hardware read/write callbacks */
...
void physdev_writew(const PhysPt64 addr, const uint16_t val) {
  if (((unsigned int)addr & 0xFFFu) <= 0xFFEu) {
    ...
    if (ph->flags & PFLAG_WRITEABLE) {
      *((uint16_t *)(ph->GetHostWritePt(pagenum) +
                     ((unsigned int)addr & 0xFFFu))) = val;
    } else {
      /* This hack is necessary because of the weird way that CPU linear
       * addresses make their way down to the hardware read/write callbacks */
      const uint32_t orig = paging.tlb.phys_page[pagenum];
      paging.tlb.phys_page[pagenum] = (uint32_t)pagenum;
...
void physdev_writed(const PhysPt64 addr, const uint32_t val) {
  if (((unsigned int)addr & 0xFFFu) <= 0xFFCu) {
    ...
    if (ph->flags & PFLAG_WRITEABLE) {
      *((uint32_t *)(ph->GetHostWritePt(pagenum) +
                     ((unsigned int)addr & 0xFFFu))) = val;
    } else {
      /* This hack is necessary because of the weird way that CPU linear
       * addresses make their way down to the hardware read/write callbacks */
  ...
  void setBytes(std::istream &stream) override {
    ...
    for (unsigned int lcv = 0; lcv < memory.pages; lcv++) {
      if (pagehandler_idx[lcv] != 0xff)
        memory.phandlers[lcv] =
            (PageHandler *)Memory_PageHandler_table[pagehandler_idx[lcv]];
      else if (lcv >= 0xa0 && lcv <= 0xff) { /* VGA and BIOS emulation does not
                                                handle this right, yet */
      } else
