...
// Nearly full and half full flags (somewhere) on the SN74V2x5/IDT72V2x5
// datasheet (just guessing on the hardware).
#define FIFO_HALF_FULL 0x00
#define FIFO_NEARLY_FULL 0x00

#define FIFO_READ_AVAILABLE                                                    \
  0x10 // High when the interrupt can't do anything but wait (cleared by reading
       // 0200?).
#define FIFO_FULL 0x08  // High when we can't write any more.
#define FIFO_EMPTY 0x04 // High when we can write direct values???
#define FIFO_NEARLY_EMPTY                                                      \
  0x02 // High when we can write more to the FIFO (or, at least, there are 0x700
       // bytes free).
...
static uint8_t PS1SOUND_CalcStatus(void) {
  ...
  if ((ps1.Pending < (FIFO_NEARLY_EMPTY_VAL << FRAC_SHIFT)) &&
      ((ps1.Command & 3) == 3)) {
...
static void PS1DAC_Reset(bool bTotal) {
  ...
  ps1.RDIndexHi = 0;
  if (bTotal)
    ps1.Adder = 0; // Be careful with this, 5 second timeout and Space Quest 4!
  ps1.Pending = 0;
...
static void PS1SOUNDWrite(Bitu port, Bitu data, Bitu iolen) {
  ...
  if ((port != 0x0205) && (port != 0x0200))
    LOG_MSG("PS1 WR %04X,%02X (%04X:%08X)", (int)port, (int)data,
            (int)SegValue(cs), (int)reg_eip);
#endif
  switch (port) {
  case 0x0200:
  ...
  case 0x0203: {
    ...
    if (ps1.Rate > 22050) {
      //					if( ( ps1.Command & 3 ) == 3 ) {
      //						LOG_MSG("Attempt to set
      //DAC rate too high (%dhz).",ps1.Rate);
      //					}
      // ps1.Divisor = 0x2C;
      // ps1.Rate = 22050;
...
static Bitu PS1SOUNDRead(Bitu port, Bitu iolen) {
...
#if C_DEBUG != 0
  LOG_MSG("PS1 RD %04X (%04X:%08X)", (int)port, (int)SegValue(cs),
          (int)reg_eip);
#endif
  switch (port) {
  case 0x0200:
  ...
  case 0x0202: {
    ...
    // Don't do this until we have some better way of detecting the triggering
    // and ending of an IRQ.
    //				ps1.Status &= ~FIFO_IRQ;
    return Status;
  }
  case 0x0203:
...
static void PS1SOUNDUpdate(Bitu length) {
  ...
  while (count) {
    ...
    if (pending <= 0) {
      ...
      while (count--)
        *(buffer++) = 0x80; // Silence.
      break;
      // pos = ( ( ps1.FIFO_RDIndex - 1 ) & FIFOSIZE_MASK ) << FRAC_SHIFT;
      // // Stay on last byte.
    } else {
  ...
  ps1.RDIndexHi = pos;
  //	if( ps1.FIFO_RDIndex != ( pos >> FRAC_SHIFT ) ) ps1.Status &=
  //~FIFO_FULL;
...
//	LOG_MSG("Write PS1 dac %X val %X
//(%04X:%08X)",port,data,SegValue(cs),reg_eip);
// }
...
public:
  PS1SOUND(Section *configuration) : Module_base(configuration) {
    ...
    ps1.chanSN =
        MixerChanSN.Install(&PS1SN76496Update, sample_rate, "PS1 SN76496");
...
// > Jmk wrote:
// > Judging by what I've read in that technical document, it looks like the
// sound chip is fed by a 4 Mhz clock instead of a ~3.5 Mhz clock.
// >
...
// >
// > That should fix it! Mind you, that was with the old code (it was 0.72 I
// worked with) which may have been updated since, but the same principle
// applies.
//
...
void PS1SOUND_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(PS1SOUND_OnReset));
}
