...
int MPU401_GetIRQ() { return (int)mpu.irq; }
...
static void MPU401_WriteCommand(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (mpu.mode == M_UART && val != 0xff)
    return;
  if (mpu.state.reset) {
    /* THIS CODE IN DISPUTE
     * [https://github.com/joncampbell123/dosbox-x/issues/917#issuecomment-538717798]
     */
  ...
  if (val <= 0x2f) {
    ...
    if (val & 0x20)
      LOG(LOG_MISC, LOG_ERROR)("MPU-401:Unhandled Recording Command %x",
                               (int)val);
    switch (val & 0xc) {
    case 0x4: /* Stop */
    ...
    case 0x8: /* Play */
      ...
      if (!mpu.state.playing && !mpu.clock.clock_to_host)
        PIC_AddEvent(MPU401_Event, MPU401_TIMECONSTANT /
    ...
    if (mpu.state.cmask & (1 << (val & 7)))
      QueueByte((uint8_t)mpu.playbuf[val & 7].counter);
  ...
  if (ret == MSG_MPU_COMMAND_REQ) {
    ...
    if (mpu.condbuf.type != T_OVERFLOW) {
      ...
      if (mpu.state.command_byte)
        MPU401_WriteData(0x330, mpu.condbuf.value[1], 1);
    }
    mpu.condbuf.type = T_OVERFLOW;
  }
...
static void MPU401_WriteData(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0xe1: /* Set relative tempo */
    ...
    if (val != 0x40)
      LOG(LOG_MISC,
          LOG_ERROR)("MPU-401:Relative tempo change value 0x%x (%.3f)",
                     (unsigned int)val, (double)val / 0x40);
    return;
  ...
  case 1: /* MIDI */
    ...
    if (!(posd == 1 && val >= 0xf0))
      mpu.playbuf[mpu.state.channel].value[posd - 1] = (uint8_t)val;
    if (posd == length)
      MPU401_EOIHandlerDispatch();
  }
}
...
next_event:
  PIC_AddEvent(MPU401_Event,
               MPU401_TIMECONSTANT / ((mpu.clock.tempo * mpu.clock.timebase *
                                       mpu.clock.tempo_rel) /
                                      0x40));
}
...
public:
  MPU401(Section *configuration) : Module_base(configuration) {
    ...
    if (IS_PC98_ARCH) {
      if (baseio >= 0x80D2 && baseio <= 0x80DE) {          /* OK */
      } else if (baseio >= 0xC000u && baseio <= 0xF8D0u) { /* OK */
      } else
        baseio = 0xE0D0u;
    } else {
    ...
    if (baseio >= 0x80D2 && baseio <= 0x80DE) {
      LOG_MSG("MPU-401 Registering I/O ports as if SB16 MPU-401 at base %xh",
              (unsigned int)baseio);
      /* SB16 mapping
       * C8D2h, C9D2h (PC-98) */
      assert(IS_PC98_ARCH);
      ...
      if (IS_PC98_ARCH)
        LOG_MSG("MPU-401 Registering I/O ports as if PC-98 MPU-401 at base %xh",
                (unsigned int)baseio);
      else
        LOG_MSG(
            "MPU-401 Registering I/O ports as if IBM PC MPU-401 at base %xh",
            (unsigned int)baseio);
      /* 330h, 331h (IBM PC)
       * E0D0h, E0D2h (PC-98) */
      WriteHandler[0].Install(baseio, &MPU401_WriteData, IO_MB);
      WriteHandler[1].Install(baseio + (IS_PC98_ARCH ? 2u : 1u),
                              &MPU401_WriteCommand, IO_MB);
      ReadHandler[0].Install(baseio, &MPU401_ReadData, IO_MB);
      ReadHandler[1].Install(baseio + (IS_PC98_ARCH ? 2u : 1u),
                             &MPU401_ReadStatus, IO_MB);
    }
    ...
    if (IS_PC98_ARCH)
      mpu.irq =
          6; /* This is said to be the MIDI card's factory default on PC-98 */
    ...
    if (IS_PC98_ARCH) {
       ...
       * IRQ detection problems unless the IRQ is masked during the detection
       * phase. It will fail to detect any IRQ that is unmasked during its
       * detection phase. For some reason it's not totally consistent i.e. the
       * MMD.COM driver included with Touhou Project 2 can detect MIDI on IRQ 5
       * when unmasked, but not 3, 12, or 6, or sometimes only on IRQ 6 if I/O
       * delays are lengthened.
       *
       * Perhaps the driver assumes that if the IRQ is unmasked, that it's
       * probably in use and not safe to probe.
       *
       * Also noted: MMD.COM cannot detect IRQs reliably if I/O delay is
       * disabled in the emulation (iodelay=0) */
...
void MPU401_Init() {
  ...
  control->GetSection("midi")->onpropchange.push_back(
      &MIDI_OnSectionPropChange);
}
