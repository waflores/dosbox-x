...
public:
  IO_callout_vector()
      : std::vector<IO_CalloutObject>(), getcounter(0), alloc_from(0) {};
...
static Bitu IO_ReadBlocked(Bitu /*port*/, Bitu /*iolen*/) { return ~0ul; }
...
static Bitu IO_ReadDefault(Bitu port, Bitu iolen) {
  ...
  case 1:
    ...
    return 0xff;
  case 2:
    return (io_readhandlers[0][port + 0](port + 0, 1) << 0) |
           (io_readhandlers[0][port + 1](port + 1, 1) << 8);
  case 4:
    return (io_readhandlers[1][port + 0](port + 0, 2) << 0) |
...
template <enum IO_Type_t iotype>
static unsigned int IO_Gen_Callout_Read(Bitu &ret, IO_ReadHandler *&f,
  ...
  while (scan < vec.size()) {
    ...
    if (!obj.isInstalled())
      continue;
    if (obj.m_r_handler == NULL)
      continue;
    if (!obj.MatchPort((uint16_t)port))
      continue;
    ...
    if (t_f != NULL) {
      if (match != 0) {
        if (iotype == IO_TYPE_ISA)
          ret &=
              t_f(port, iolen); /* ISA pullup resisters vs ISA devices pulling
                                   data lines down (two conflicting devices) */
      } else {
      ...
      match++;
      if (iotype ==
          IO_TYPE_PCI) /* PCI bus only one device can respond, no conflicts */
        break;
    }
  }
...
template <enum IO_Type_t iotype>
static unsigned int IO_Gen_Callout_Write(IO_WriteHandler *&f, Bitu port,
                                         Bitu val, Bitu iolen) {
  int actual = iotype - IO_TYPE_MIN;
  IO_callout_vector &vec = IO_callouts[actual];
  unsigned int match = 0;
...
static unsigned int IO_Motherboard_Callout_Read(Bitu &ret, IO_ReadHandler *&f,
                                                Bitu port, Bitu iolen) {
  return IO_Gen_Callout_Read<IO_TYPE_MB>(ret, f, port, iolen);
}

static unsigned int IO_PCI_Callout_Read(Bitu &ret, IO_ReadHandler *&f,
                                        Bitu port, Bitu iolen) {
  return IO_Gen_Callout_Read<IO_TYPE_PCI>(ret, f, port, iolen);
}

static unsigned int IO_ISA_Callout_Read(Bitu &ret, IO_ReadHandler *&f,
                                        Bitu port, Bitu iolen) {
  return IO_Gen_Callout_Read<IO_TYPE_ISA>(ret, f, port, iolen);
}

static unsigned int IO_Motherboard_Callout_Write(IO_WriteHandler *&f, Bitu port,
                                                 Bitu val, Bitu iolen) {
  return IO_Gen_Callout_Write<IO_TYPE_MB>(f, port, val, iolen);
}

static unsigned int IO_PCI_Callout_Write(IO_WriteHandler *&f, Bitu port,
                                         Bitu val, Bitu iolen) {
  return IO_Gen_Callout_Write<IO_TYPE_PCI>(f, port, val, iolen);
}

static unsigned int IO_ISA_Callout_Write(IO_WriteHandler *&f, Bitu port,
                                         Bitu val, Bitu iolen) {
...
static Bitu IO_ReadSlowPath(Bitu port, Bitu iolen) {
  ...
  if (match == 0) {
     ...
     * is based on experience with an old Pentium system of mine in the late
     * 1990s when I had a Sound Blaster Live PCI! and a Sound Blaster clone both
     * listening to ports 220h-22Fh in Windows 98, in which both cards responded
     * to a DOS game in the DOS box even though read-wise only the Live PCI!'s
     * data was returned. Both cards responded to I/O writes.
     *
     * Based on that experience, my guess is that to keep ISA from going too
     * slow Intel designed the PIIX PCI/ISA bridge to listen for I/O and start
     * an ISA I/O cycle even while another PCI device is preparing to respond to
     * it. Then if nobody takes it, the PCI/ISA bridge completes the ISA bus
     * cycle and takes up the PCI bus transaction. If another PCI device took
     * the I/O write, then the cycle completes quietly and the result is
     * discarded.
     *
     * That's what I think happened, anyway.
     *
     * I wish I had tools to watch I/O transactions on the ISA bus to verify
     * this. --J.C. */
    if (pcibus_enable) {
      ...
      if (match == 0) {
        ...
        /* PCI did match. Based on behavior noted above, probe ISA bus anyway
         * and discard data. */
  ...
  LOG(LOG_MISC,
      LOG_DEBUG)("IO read slow path port=%x iolen=%u: device matches=%u",
                 (unsigned int)port, (unsigned int)iolen, (unsigned int)match);
  if (match == 0)
    ret = f(port, iolen); /* if nobody responded, then call the default */
  if (match <= 1)
    io_readhandlers[porti][port] = f;
...
void IO_WriteSlowPath(Bitu port, Bitu val, Bitu iolen) {
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "IO write slow path port=%x data=%x iolen=%u: device matches=%u",
      (unsigned int)port, (unsigned int)val, (unsigned int)iolen,
      (unsigned int)match);
  if (match == 0)
    f(port, val, iolen); /* if nobody responded, then call the default */
  if (match <= 1)
    io_writehandlers[porti][port] = f;
...
void IO_RegisterReadHandler(Bitu port, IO_ReadHandler *handler, Bitu mask,
                            Bitu range) {
...
void IO_RegisterWriteHandler(Bitu port, IO_WriteHandler *handler, Bitu mask,
                             Bitu range) {
  assert((port + range) <= IO_MAX);
  while (range--) {
    if (mask & IO_MB)
...
void IO_FreeReadHandler(Bitu port, Bitu mask, Bitu range) {
  ...
  while (range--) {
    if (mask & IO_MB)
      io_readhandlers[0][port] = IO_ReadSlowPath;
    if (mask & IO_MW)
      io_readhandlers[1][port] = IO_ReadSlowPath;
    if (mask & IO_MD)
      io_readhandlers[2][port] = IO_ReadSlowPath;
    port++;
  }
}
...
void IO_FreeWriteHandler(Bitu port, Bitu mask, Bitu range) {
  ...
  while (range--) {
    if (mask & IO_MB)
      io_writehandlers[0][port] = IO_WriteSlowPath;
    if (mask & IO_MW)
      io_writehandlers[1][port] = IO_WriteSlowPath;
    if (mask & IO_MD)
      io_writehandlers[2][port] = IO_WriteSlowPath;
    port++;
  }
}
...
void IO_ReadHandleObject::Install(Bitu port, IO_ReadHandler *handler, Bitu mask,
                                  Bitu range) {
  if (!installed) {
    installed = true;
    m_port = port;
    ...
    E_Exit("IO_readHandler already installed port %x", (int)port);
}
...
void IO_WriteHandleObject::Install(Bitu port, IO_WriteHandler *handler,
  ...
  if (!installed) {
    ...
    E_Exit("IO_writeHandler already installed port %x", (int)port);
}
...
inline void IO_USEC_read_delay(const unsigned int szidx) {
  if (io_delay_ns[szidx] > 0 &&
      last_callback == 0 /*NOT running within a callback function*/) {
...
void log_io(Bitu width, bool write, Bitu port, Bitu val) {
  ...
  if (write) {
    ...
    if (port == 0x3d4) {
      if (width == 0)
        crtc_index = (uint8_t)val;
      else if (width == 1)
        crtc_index = (uint8_t)(val >> 8);
    }
    if (crtc_index == 0xe || crtc_index == 0xf) {
      if ((width == 0 && (port == 0x3d4 || port == 0x3d5)) ||
          (width == 1 && port == 0x3d4))
        return;
    }
    ...
    case 0x3c9: // VGA palette
      ...
      break;
    default:
      LOG_MSG("iow%s % 4x % 4x, cs:ip %04x:%04x", len_type[width], (int)port,
              (int)val, (int)SegValue(cs), (int)reg_eip);
      break;
    }
  } else {
    switch (port) {
    ...
    case 0x3da: // display status - a real spammer
      ...
      break;
    default:
      LOG_MSG("ior%s % 4x % 4x,\t\tcs:ip %04x:%04x", len_type[width], (int)port,
              (int)val, (int)SegValue(cs), (int)reg_eip);
      break;
    }
  }
...
void IO_Reset(Section * /*sec*/) { // Reset or power on
  ...
  if (io_delay_ns[0] < 0) { // 8-bit transfers are said to have a transfer cycle
                            // with 4 wait states
    // TODO: How can we emulate Intel 440FX chipsets that allow 8-bit PIO with 1
    // wait state, or zero wait state devices?
    double t = (1000000000.0 * clockdom_ISA_BCLK.freq_div * 8.5) /
               clockdom_ISA_BCLK.freq;
    io_delay_ns[0] = (int)floor(t);
  }

  io_delay_ns[1] = section->Get_int("iodelay16");
  if (io_delay_ns[1] < 0) { // 16-bit transfers are said to have a transfer
                            // cycle with 1 wait state
    // TODO: How can we emulate ISA devices that support zero wait states?
    double t = (1000000000.0 * clockdom_ISA_BCLK.freq_div * 5.5) /
               clockdom_ISA_BCLK.freq;
    io_delay_ns[1] = (int)floor(t);
  }

  io_delay_ns[2] = section->Get_int("iodelay32");
  if (io_delay_ns[2] < 0) { // assume ISA bus details that turn 32-bit PIO into
                            // two 16-bit I/O transfers
    // TODO: If the device is a PCI device, then 32-bit PIO should take the same
    // amount of time as 8/16-bit PIO
    double t = (1000000000.0 * clockdom_ISA_BCLK.freq_div * (5.5 + 5.5)) /
               clockdom_ISA_BCLK.freq;
...
void IO_CalloutObject::InvalidateCachedHandlers(void) {
  ...
  /* for both the base I/O, as well as its aliases, revert the I/O ports back to
   * "slow path" */
  for (p = m_port; p < 0x10000ul; p += alias_mask + 1ul)
    IO_InvalidateCachedHandler(p, range_mask + 1ul);
}

void IO_CalloutObject::Install(Bitu port,
                               Bitu portmask /*IOMASK_ISA_10BIT, etc.*/,
                               IO_ReadCalloutHandler *r_handler,
                               IO_WriteCalloutHandler *w_handler) {
  if (!installed) {
    if (portmask == 0 || (portmask & ~0xFFFFU)) {
      LOG(LOG_MISC,
          LOG_ERROR)("IO_CalloutObject::Install: Port mask %x is invalid",
     ...
     *   - zero or more 0 bits to LSB */
    {
      ...
      /* DEBUG */
      if ((portmask & range_mask) != 0 || ((range_mask + 1) & range_mask) != 0 /* should be a mask, therefore AND by itself + 1 should be zero (think (0xF + 1) & 0xF = 0x10 & 0xF = 0x0) */) {
        LOG(LOG_MISC, LOG_ERROR)(
            "IO_CalloutObject::Install: portmask(%x) & range_mask(%x) != 0 "
            "(%x). You found a corner case that broke this code, fix it.",
            (unsigned int)portmask, (unsigned int)range_mask,
      ...
      test =
          portmask + range_mask; /* will break if portmask & range_mask != 0 */
      while ((test & m) == m) {
      ...
      if ((portmask ^ range_mask ^ alias_mask) != 0 || ((alias_mask + 1) & alias_mask) != 0 /* should be a mask, therefore AND by itself + 1 should be zero */) {
        LOG(LOG_MISC, LOG_ERROR)(
            "IO_CalloutObject::Install: portmask(%x) ^ range_mask(%x) ^ "
      ...
      if (port & range_mask) {
        LOG(LOG_MISC, LOG_ERROR)("IO_CalloutObject::Install: port %x and port "
                                 "mask %x not aligned (range_mask %x)",
                                 (unsigned int)port, (unsigned int)portmask,
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("IO_CalloutObject::Install added device with port=0x%x "
                   "io_mask=0x%x rangemask=0x%x aliasmask=0x%x",
                   (unsigned int)port, (unsigned int)io_mask,
 ...
 * back. The way DOSBox/DOSbox-X code handles IO callbacks it's common to
 * declare an IO object, call the install, but then never touch it again, so
 * this should work fine.
 *
 * this allows us to maintain ready-made IO callout objects to return quickly
 * rather than write more complicated code where the caller has to make an
 * IO_CalloutObject and then call install and we have to add its pointer to a
...
try_again:
  ...
  if (vec.size() < 4096 && vec.getcounter == 0) {
    ...
    LOG(LOG_MISC, LOG_WARN)("IO_AllocateCallout type %u expanding array to %u",
  ...
  LOG(LOG_MISC, LOG_WARN)("IO_AllocateCallout type %u no free entries",
                          (unsigned int)t);
  return IO_Callout_t_none;
}
