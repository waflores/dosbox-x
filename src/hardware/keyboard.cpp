...
static pic_tickindex_t KEYDELAY =
    0.300f; // Considering 20-30 khz serial clock and 11 bits/char
...
enum AuxCommands { ACMD_NONE, ACMD_SET_RATE, ACMD_SET_RESOLUTION };
...
enum MouseMode { MM_REMOTE = 0, MM_WRAP, MM_STREAM };
...
struct ps2mouse {
  ...
  bool int33_taken; /* for compatibility with existing DOSBox code: allow INT
                       33H emulation to "take over" and disable us */
  bool l, m, r;     /* mouse button states */
};

static struct {
  uint8_t buf8042[8]; /* for 8042 responses, taking priority over keyboard
                         responses */
  Bitu buf8042_len;
  Bitu buf8042_pos;
  int pending_key;
...
void PCjr_stuff_scancode(const unsigned char c) { keyb.p60data = c; }
...
bool MouseTypeNone() { return (keyb.ps2mouse.type == MOUSE_NONE); }

/* NTS: INT33H emulation is coded to call this ONLY if it hasn't taken over the
 * role of mouse input */
void KEYBOARD_AUX_Event(float x, float y, Bitu buttons, int scrollwheel) {
  if (IS_PC98_ARCH) {
    LOG_MSG("WARNING: KEYBOARD_AUX_Event called in PC-98 emulation mode. This "
            "is a bug.");
    return;
  }
  ...
  if (keyb.ps2mouse.reporting && keyb.ps2mouse.mode == MM_STREAM) {
    if ((keyb.used + 4) < KEYBUFSIZE) {
      ...
      KEYBOARD_AddBuffer(
          AUX | ((y2 == -256 || y2 == 255) ? 0x80 : 0x00) | /* Y overflow */
      ...
      if (keyb.ps2mouse.intellimouse_btn45) {
        KEYBOARD_AddBuffer(AUX |
                           (scrollwheel & 0xFF)); /* TODO: 4th & 5th buttons */
...
int KEYBOARD_PS2REPORT_Active() {
  /* HACK: INT 15h really needs to issue Enable AUX, but for now, report if PS/2
   * mouse reporting active */
...
static void KEYBOARD_SetPort60(uint16_t val) {
  ...
  if (keyb.auxchanged) {
    ...
    if (keyb.cb_irq1) {
      if (machine == MCH_PCJR)
        CPU_Raise_NMI(); /* NTS: PCjr apparently hooked the keyboard to NMI */
      else
        PIC_ActivateIRQ(1);
    }
  }
...
Bitu Keyboard_Guest_LED_State() { return keyb.led_state; }
...
void KEYBOARD_SetLEDs(uint8_t bits) {
  ...
  /* log for debug info */
  LOG(LOG_KEYBOARD, LOG_DEBUG)("Keyboard LEDs: SCR=%u NUM=%u CAPS=%u", bits & 1,
                               (bits >> 1) & 1, (bits >> 2) & 1);
}
...
static Bitu read_p60(Bitu port, Bitu iolen) {
  ...
  /* NOTES: "Cronologia" by Cascada (1991) polls the keyboard I/O ports
   * directory for part 1 and 2, but part 3 installs a keyboard ISR. That ISR
   * assumes that if IRQ 1 happens, there is valid data in port 60h, it does not
   * poll port 61h to determine if data is available. If the last scan code was
   * a key-up scancode event for the Escape key, then when part 3 enables the
   * IRQ, if this code had not cleared the signal, the ISR will be immediately
   * called and the demo will immediately act as if you hit the escape key and
   * exit right away. */
  if (machine != MCH_PCJR) {
...
unsigned char KEYBOARD_AUX_DevStatus() {
  return (keyb.ps2mouse.mode == MM_REMOTE ? 0x40 : 0x00) |
         (keyb.ps2mouse.reporting << 5) | (keyb.ps2mouse.scale21 << 4) |
         (keyb.ps2mouse.m << 2) | (keyb.ps2mouse.r << 1) |
         (keyb.ps2mouse.l << 0);
}

unsigned char KEYBOARD_AUX_Resolution() { return keyb.ps2mouse.resolution; }

unsigned char KEYBOARD_AUX_SampleRate() { return keyb.ps2mouse.samplerate; }
...
void KEYBOARD_AUX_Write(Bitu val) {
  ...
  case ACMD_SET_RATE:
    ...
    if (keyb.ps2mouse.type >= MOUSE_INTELLIMOUSE) {
      if (keyb.ps2mouse.last_srate[0] == 200 &&
          keyb.ps2mouse.last_srate[2] == 80) {
        ...
        } else if (keyb.ps2mouse.last_srate[1] == 200 &&
                   keyb.ps2mouse.type >= MOUSE_INTELLIMOUSE45) {
          if (!keyb.ps2mouse.intellimouse_btn45) {
            LOG(LOG_KEYBOARD,
                LOG_NORMAL)("Intellimouse 4/5-button mode enabled");
  ...
  case ACMD_SET_RESOLUTION:
    ...
    LOG(LOG_KEYBOARD, LOG_NORMAL)("PS/2 mouse resolution set to %u",
                                  (int)(1 << (val & 3)));
    break;
  }
}
...
static void write_p60(Bitu port, Bitu val, Bitu iolen) {
  ...
  case CMD_NONE: /* None */
    ...
    case 0xf6: /* Reset keyboard and enable scanning */
      ...
      keyb.scanning = true; /* JC: Original DOSBox code was wrong, this command
                               enables scanning */
      KEYBOARD_AddBuffer(0xfa); /* Acknowledge */
      break;
    case 0xff: /* keyboard resets take a long time (about 250ms), most keyboards
                  flash the LEDs during reset */
      KEYBOARD_Reset();
      KEYBOARD_Add8042Response(
          0xFA); /* ACK (TODO: The host has to read the ACK byte first before
                    starting reset?? According to IBM, anyway) */
      keyb.reset = true;
      KEYBOARD_SetLEDs(7); /* most keyboard I test with tend to flash the LEDs
                              during reset */
  ...
  case CMD_WRITEOUTPUT:
    ...
    KEYBOARD_ClrBuffer();
    KEYBOARD_AddBuffer(
        val); /* and now you return the byte as if it were typed in */
    break;
  ...
  case CMD_SETOUTPORT:
    if (!(val & 1)) {
      if (allow_keyb_reset) {
        ...
        LOG_MSG(
            "WARNING: Keyboard output port written with bit 1 clear. Is the "
            "guest OS or application attempting to reset the system?\n");
      }
    }
  ...
  case CMD_SETTYPERATE:
...

    {
      ...
      static const unsigned int repeat[] = {
          33,  37,  42,  46,  50,  54,  58,  63,  67,  75,  83,
          92,  100, 109, 118, 125, 133, 149, 167, 182, 200, 217,
  ...
  case CMD_SETCOMMAND: /* 8042 command, not keyboard */
    /* TODO: If biosps2=true and aux=false, disallow the guest OS from changing
     * AUX port parameters including IRQ */
    keyb.command = CMD_NONE;
...
static Bitu read_p61(Bitu, Bitu) {
  ...
  dbg = ((port_61_data & 0xF) |
         ((TIMER_GetOutput2() || (port_61_data & 1) == 0)
              ? 0x20
              : 0) | // NTS: Timer 2 doesn't cycle if Port 61 gate turned off,
                     // and it becomes '1' when turned off
         ((fmod(PIC_FullIndex(), 0.030) > 0.015) ? 0x10 : 0));

  // Always return bit 6 as set for early PCs and as cleared for PCjr or Tandy.
  // See https://www.vogons.org/viewtopic.php?t=50417.
...
static void write_p64(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0x20: /* read command byte */
    /* TODO: If biosps2=true and aux=false, mask AUX port bits as if AUX isn't
     * there */
    KEYBOARD_Add8042Response((keyb.cb_xlat << 6) | ((!keyb.auxactive) << 5) |
  ...
  case 0xd0: /* Outport on buffer */
    KEYBOARD_SetPort60(
        (MEM_A20_Enabled() ? 0x02 : 0) |
        0x01 /*some programs read the output port then write it back*/);
    break;
  ...
  case 0xd3: /* write AUX output */
    if (keyb.enable_aux)
      ...
      LOG(LOG_KEYBOARD, LOG_ERROR)(
          "Program is writing 8042 AUX. If you intend to use PS/2 mouse "
          "emulation you may consider adding aux=1 to your dosbox-x.conf");
    break;
  ...
  case 0xff:
    ...
    if (!(val & 1)) {
      if (allow_keyb_reset) {
        ...
        LOG_MSG(
            "WARNING: Keyboard output port written (pulsed) with bit 1 clear. "
            "Is the guest OS or application attempting to reset the system?\n");
      }
    }
    break;
  default:
...
static Bitu read_p64(Bitu port, Bitu iolen) {
  ...
  (void)iolen; // UNUSED
  uint8_t status =
      0x1c | (keyb.p60changed ? 0x1 : 0x0) | (keyb.auxchanged ? 0x20 : 0x00);
  return status;
}
...
void KEYBOARD_AddKey3(KBD_KEYS keytype, bool pressed) {
  ...
  /* if the keyboard is disabled, then store the keystroke but don't transmit
   * yet */
  /*
  ...
  */
  ...
  case KBD_rwinmenu:
    ret = 0x8D;
    break;
  case KBD_jp_muhenkan:
    ret = 0x85;
    break;
  case KBD_jp_henkan:
    ret = 0x86;
    break;
  case KBD_jp_hiragana:
    ret = 0x87;
    break; /*also Katakana */
  default:
    LOG(LOG_MISC, LOG_WARN)("Unsupported key press %lu",
...
void KEYBOARD_PC98_AddKey(KBD_KEYS keytype, bool pressed) {
  ...
  switch (keytype) { // NAME or
                     // NM SH KA KA+SH       NM=no-mod SH=shift KA=kana
                     // KA+SH=kana+shift
  case KBD_esc:
    ret = 0x00;
    break; // ESC
  ...
  case KBD_equals:
    ret = 0x0C;
    break; // ^  `  ヘ             US keyboard layout hack
  case KBD_caret:
    ret = 0x0C;
    break; // ^  `  ヘ
  case KBD_jp_yen:
  ...
  case KBD_atsign:
    ...
    break; // @  ~  ﾞ
  case KBD_leftbracket:
    ret = pc98_force_ibm_layout ? 0x1B : 0x1A;
    break; // @  ~  ﾞ
  case KBD_rightbracket:
    ret = pc98_force_ibm_layout ? 0x28 : 0x1B;
    break; // [  {  ﾟ  ｢ / ]  }  ム ｣
  case KBD_enter:
    ret = 0x1C;
    break; // ENTER/RETURN
  ...
  case KBD_quote:
    ret = 0x27;
    break; // :  *  ケ         American US keyboard layout hack
  case KBD_colon:
    ret = 0x27;
    break; // :  *  ケ
  case KBD_backslash:
    ret = pc98_force_ibm_layout ? 0x0D : 0x28;
    break; // ¥  |  ｰ
  case KBD_z:
    ret = 0x29;
    break; // z  Z  ツ ッ
  ...
  case KBD_pagedown:
    ret = 0x36;
    break; // ROLL UP [https://github.com/joncampbell123/dosbox-x/issues/4106]
  case KBD_pageup:
    ret = 0x37;
    break; // ROLL DOWN [https://github.com/joncampbell123/dosbox-x/issues/4106]
  case KBD_insert:
    ret = 0x38;
    break; // INS
  case KBD_delete:
  ...
  case KBD_leftalt:
    ret = 0x73;
    break; // GRPH (handled by Windows as if ALT key)
  case KBD_rightalt:
    ret = 0x73;
    break; // GRPH (handled by Windows as if ALT key)
  case KBD_leftctrl:
  ...
  case KBD_pause: // PAUSE: Map to STOP key since CTRL+BREAK on IBM PC has a
                  // similar function
    ret = 0x60;
    break;
  ...
  case KBD_capslock: // CAPS
    if (pressed) {   // sends only on keypress, does not resend if held down
      ...
      pc98_keyboard_send(
          0x71 |
          (!pc98_caps() ? 0x80 : 0x00)); // make code if caps switched on, break
                                         // if caps switched off
    }
    return;

  case KBD_numlock:        // NUM
    pc98_numlock_toggle(); // TODO: Scan code? NUM LOCK didn't exist until later
                           // PC-9821 systems.
    return;
  ...
  case KBD_kana:   // KANA
    if (pressed) { // sends only on keypress, does not resend if held down
      ...
      pc98_keyboard_send(
          0x72 |
          (!pc98_kana() ? 0x80 : 0x00)); // make code if kana switched on, break
                                         // if caps switched off
    }
    return;

  case KBD_jp_backslash:
    if (!pc98_force_ibm_layout) {
      ret = 0x33;
      break;
    } //    _  ロ
...
void KEYBOARD_AddKeyPCjr(KBD_KEYS keytype, bool pressed) {
  ...
  case KBD_quote:
    ...
    break;
  case KBD_grave:
    ret = (keyb.leftshift_pressed || keyb.rightshift_pressed)
              ? 27 /*rightbracket*/
              : 40 /*quote*/;
    fn = true;
    break; /*Fn+'*/
  ...
  if (pressed) {
    ...
    if (fn)
      KEYBOARD_AddBuffer(0x54 + 0x80); /*Fn*/
  }
}
...
void KEYBOARD_AddKey1(KBD_KEYS keytype, bool pressed) {
  ...
  case KBD_kpcomma:
    ret = 0x7e;
    break; /* Keypad comma and ABNT C2 (Brazilian KP period)*/
  case KBD_backspace:
    ret = 14;
    break;
  case KBD_tab:
  ...
  case KBD_printscreen:
    ...
    if (keyb.leftalt_pressed || keyb.rightalt_pressed) {
      KEYBOARD_AddBuffer(0x54 | (pressed ? 0 : 0x80));
               ...
               keyb.leftshift_pressed || keyb.rightshift_pressed) {
      KEYBOARD_AddBuffer(0xe0);
  ...
  case KBD_yen:
    ...
    break; // for AX layout
  case KBD_conv:
    ret = (INT16_AX_GetKBDBIOSMode() == 0x51) ? 0x5b : 57;
    break; // for AX layout
  case KBD_nconv:
    ret = (INT16_AX_GetKBDBIOSMode() == 0x51) ? 0x5a : 57;
    break; // for AX layout
  case KBD_ax:
  ...
  case KBD_jp_backslash:
    ret = 0x73;
    break; /*JP 106-key: _ \ or ろ (ro)  <-- WARNING: UTF-8 unicode also ABNT
              C1(Brazilian /)*/
  case KBD_jp_yen:
    ret = 0x7d;
    break; /*JP 106-key: | ¥ (yen) or ー (prolonged sound mark)  <-- WARNING:
              UTF-8 unicode */
  case KBD_colon:
    if (!pc98_force_ibm_layout) {
      ret = 0x28;
      break;
    } /*JP106-key : or * same position with Quote key */
  case KBD_caret:
    if (!pc98_force_ibm_layout) {
      ret = 0x0d;
      break;
    } /*JP106-key ^ or ~ same position with Equals key */
  case KBD_atsign:
    if (!pc98_force_ibm_layout) {
      ret = 0x1a;
      break;
    } /*JP106-key @ or ` same position with Left-bracket key */
  default:
    LOG(LOG_MISC, LOG_WARN)("Unsupported key press %lu",
                            (unsigned long)keytype);
    return;
...
void KEYBOARD_AddKey(KBD_KEYS keytype, bool pressed) {
  /* If the BIOS has put the system into APM suspend, let certain keys wake it
   * up again. Send on RELEASE so that the key isn't also typed into the guest
   * OS. */
  ...
  if (IS_PC98_ARCH) {
    ...
    /* emulate typical setup where keyboard generates scan set 2 and controller
     * translates to scan set 1 */
    /* yeah I know... yuck */
    ...
    case 1:
      ...
      break;
    case 2:
      KEYBOARD_AddKey2(keytype, pressed);
      break;
    case 3:
      KEYBOARD_AddKey3(keytype, pressed);
      break;
    }
  }
}
...
public:
  ...
  uint8_t inPortA(void) const override {
     ...
     *       It might help to look at the BIOS setup menus of 1990s PC-98
     * systems that offer toggling virtual versions of these DIP switches to see
     *       what the BIOS menu text says. */
    /* NTS: Return the INITIAL setting of the GDC. Guest applications (like
     * Windows 3.1) can and will change it later. This must reflect the initial
     * setting as if what the BIOS had initially intended. */
    return 0x63 |
           (gdc_5mhz_mode_initial ? 0x00 : 0x80); // taken from a PC-9821 Lt2
  }
  /* port B is input */
  uint8_t inPortB(void) const override {
    /* TODO: Improve this! */
  ...
  void outPortC(const uint8_t mask) override {
    ...
    if (mask & 0x08) { /* PC speaker aka "buzzer". Note this bit is an inhibit,
                          set to zero to turn on */
...
bool pc98_caps(void) { return pc98_keyboard_state.caps; }

bool pc98_kana(void) { return pc98_keyboard_state.kana; }
...
static struct pc98_8251_keyboard_uart {
  enum cmdreg_state { MODE_STATE = 0, SYNC_CHAR1, SYNC_CHAR2, COMMAND_STATE };
  ...
  double io_delay_ms =
      (((1 /*start*/ + 8 /*data*/ + 1 /*parity*/ + 1 /*stop*/) * 1000.0) /
       19200);
  ...
  void tx_load_complete(void) { tx_busy = false; }
  ...
  unsigned char read_status(void) const {
     ...
     * bit[1:1] = RxRDY
     * bit[0:0] = TxRDY */
    r |= (!tx_busy ? 0x01 : 0x00) | (rx_ready ? 0x02 : 0x00) |
         (tx_empty ? 0x04 : 0x00);
  ...
  void writecmd(const unsigned char b) { /* write to command register */
    if (state == MODE_STATE) {
      ...
      if ((b & 3) != 0) {
         ...
         *
         * note that "baud rate factor" means how much to divide the baud rate
         * clock to determine the bit rate that bits are transmitted. Typical
         * PC-98 programming practice is to set the baud rate clock fed to the
         * chip at 16X the baud rate and then specify 16X baud rate factor. */
        /* async mode */
        state = COMMAND_STATE;

        /* keyboard must operate at 19200 baud 8 bits odd parity 16X baud rate
         * factor */
         ...
         * bit[6:6] = external sync detect (1=syndet is an input   0=syndet is
         * an output) bit[5:5] = even/odd parity      (1=even 0=odd) bit[4:4] =
         * parity enable        (1=enable 0=disable) bit[3:2] = character length
         * (0=5  1=6  2=7  3=8) bit[1:0] = baud rate factor     (0=sync mode)
         *
         * I don't think anything uses the keyboard in this manner, therefore,
         * not supported in this emulation. */
        LOG_MSG(
            "8251 keyboard warning: Mode byte synchronous mode not supported");
        state = SYNC_CHAR1;
        valid_state = false;
      }
       ...
       * bit[5:5] = RTS inhibit (1=force RTS to zero, else RTS reflects RxRDY
       * state of the chip) bit[4:4] = error reset bit[3:3] = send break
       * character (0=normal  1=force TxD low). On PC-98 keyboard this is wired
       * to reset pin of the keyboard CPU. bit[2:2] = receive enable bit[1:1] =
       * DTR inhibit (1=force DTR to zero). Connected to PC-98 RTY pin. bit[0:0]
       * = transmit enable */
      if (b & 0x40) {
      ...
      /* TODO: Does the 8251 take any other bits if bit 6 was set to reset the
       * 8251? */
...
static void keyboard_pc98_8251_uart_41_write(Bitu port, Bitu val,
                                             Bitu /*iolen*/) {
  (void)port; // UNUSED
...
static void keyboard_pc98_8251_uart_43_write(Bitu port, Bitu val,
                                             Bitu /*iolen*/) {
  (void)port; // UNUSED
...
void pc98_mouse_movement_apply(int x, int y) {
   ...
   *      they just count.
   *
   *      According to Nanshiki:
   * [https://github.com/joncampbell123/dosbox-x/pull/4697]
   *
   *      -- 2023/12/21 J.C. */
...
static void pc98_mouse_tick_schedule(void) {
  if (p7fd8_8255_mouse_int_enable) {
     ...
     * The PIC event will not reschedule itself if inhibited at the time of the
     * signal. */
  }
}
...
public:
  ...
  uint8_t inPortA(void) const override {
    ...
    if (!(bs & 1))
      r |= 0x80; // left button (inverted bit)
    if (!(bs & 2))
      r |= 0x20; // right button (inverted bit)
    if (!(bs & 4))
      r |= 0x40; // middle button (inverted bit)
    ...
    case 0: // X delta
    case 1:
      r |= (uint8_t)(p7fd9_8255_mouse_x_latch >>
                     ((p7fd9_8255_mouse_sel & 1U) * 4U)) &
           0xF; // sign extend is intentional
      break;
    case 2: // Y delta
    case 3:
      r |= (uint8_t)(p7fd9_8255_mouse_y_latch >>
                     ((p7fd9_8255_mouse_sel & 1U) * 4U)) &
           0xF; // sign extend is intentional
      break;
    }
  ...
  void outPortC(const uint8_t mask) override {
    ...
    if (mask & 0x80) { /* bit 7 */
      ...
      if ((latchOutPortC & 0x80) &&
          !p7fd9_8255_mouse_latch) { // change from 0 to 1 latches counters and
                                     // clears them
        p7fd9_8255_mouse_x_latch = p7fd9_8255_mouse_x;
        p7fd9_8255_mouse_y_latch = p7fd9_8255_mouse_y;
    ...
    if (mask & 0x10) { /* bit 4 */
      ...
      p7fd8_8255_mouse_int_enable =
          ((latchOutPortC >> 4) & 1) ^ 1; // bit 4 is interrupt MASK

      /* Some games use the mouse interrupt as a periodic source by writing this
      ...
      if (p != p7fd8_8255_mouse_int_enable) {
        /* NTS: I'm guessing that if the mouse interrupt has fired, inhibiting
         * the interrupt does not remove the IRQ signal already sent.
         *
         *      Amaranth's mouse driver appears to rapidly toggle this bit. If
         * we re-fire and inhibit the interrupt in reaction the game will get an
         * interrupt rate far higher than normal and animation will run way too
         * fast. */
...
static void write_pbfdb_mouse(Bitu port, Bitu val, Bitu /*iolen*/) {
  ...
  if (pc98_mouse_rate_hz != p_pc98_mouse_rate_hz)
    LOG(LOG_MISC, LOG_DEBUG)("PC-98 mouse interrupt rate: %u",
                             pc98_mouse_rate_hz);
}
//////////
...
void KEYBOARD_OnEnterPC98(Section *sec) {
  ...
  if (IS_PC98_ARCH) {
    ...
    if (pc98_force_ibm_layout)
      LOG_MSG(
          "Forcing PC-98 keyboard to use IBM US-English like default layout");
    pc98_force_jis_layout =
        pc98_section->Get_bool("pc-98 force JIS keyboard layout") &
        !pc98_force_ibm_layout;
...
void KEYBOARD_OnEnterPC98_phase2(Section *sec) {
  ...
  pc98_sys_8255.writePortC(
      0xF8); /* SHUT0=1 SHUT1=1 mask printer RAM parity check buzzer inhibit */
  ...
  for (i = 0; i < 4; i++) {
    ...
    /* printer */
    ReadHandler_8255prn_PC98[i].Uninstall();
    ReadHandler_8255prn_PC98[i].Install(0x40 + (i * 2), pc98_8255prn_read,
                                        IO_MB);

    WriteHandler_8255prn_PC98[i].Uninstall();
    WriteHandler_8255prn_PC98[i].Install(0x40 + (i * 2), pc98_8255prn_write,
                                         IO_MB);
  }
  ...
  if (enable_pc98_bus_mouse) {
    ...
    /* Mouse control port at BFDB (which can be used to reduce the interrupt
     * rate of the mouse) */
  ...
  pc98_mouse_8255.writePortC(0x10); /* start with interrupt inhibited. INT 33h
...
void KEYBOARD_OnReset(Section *sec) {
  ...
  Section_prop *section =
      static_cast<Section_prop *>(control->GetSection("keyboard"));
  ...
  if (machine == MCH_PCJR) // IBM Personal Computer PCjr Hardware Ref Tech Ref
                           // 2-104 Cordless Keyboard
    KEYDELAY =
        0.440 *
        21; // 440us bit cell times 21 bits (START + DATA(8) + PARITY + 11 STOP)
  else
  ...
  keyb.ps2mouse.reset_mode = MM_STREAM; /* NTS: I was wrong: PS/2 mice default
                                           to streaming after reset */
  ...
  if (sbtype != NULL && machine != MCH_PCJR && enable_slave_pic) {
    if (!strcasecmp(sbtype, "2button"))
      ...
      keyb.ps2mouse.type = MOUSE_INTELLIMOUSE;
      LOG(LOG_KEYBOARD, LOG_ERROR)(
          "Assuming PS/2 intellimouse, I don't know what '%s' is", sbtype);
    }
  }
...
void KEYBOARD_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(KEYBOARD_OnReset));
}
...
void AUX_Reset() {
  ...
  if (keyb.ps2mouse.type != MOUSE_NONE && keyb.ps2mouse.int33_taken)
    LOG(LOG_KEYBOARD,
        LOG_NORMAL)("PS/2 mouse emulation: taking over from INT 33h");
  keyb.ps2mouse.int33_taken = 0;
...
void AUX_INT33_Takeover() {
  if (keyb.ps2mouse.type != MOUSE_NONE && keyb.ps2mouse.int33_taken)
    LOG(LOG_KEYBOARD, LOG_NORMAL)("PS/2 mouse emulation: Program is using INT "
                                  "33h, disabling direct AUX emulation");
...
// save state support
void *KEYBOARD_TransferBuffer_PIC_Event =
    (void *)((uintptr_t)KEYBOARD_TransferBuffer);
void *KEYBOARD_TickHandler_PIC_Timer =
    (void *)((uintptr_t)KEYBOARD_TickHandler);
