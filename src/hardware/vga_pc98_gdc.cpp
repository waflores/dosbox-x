...
double gdc_proc_delay =
    0.001; /* time from FIFO to processing in GDC (1us) FIXME: Is this right? */
...
void pc98_update_cpu_page_ptr(void) {
  if (pc98_gdc_vramop & (1 << VOPBIT_VGA)) {
    ...
    pc98_pgraph_current_cpu_page =
        vga.mem.linear + PC98_VRAM_GRAPHICS_OFFSET +
        ((pc98_gdc_vramop & (1 << VOPBIT_ACCESS)) ? PC98_VRAM_PAGEFLIP_SIZE
                                                  : 0);
  }
}
...
void PC98_GDC_state::take_reset_sync_parameters(void) {
  ...
  display_mode =                                         /* CG = [1:0] */
      ((cmd_parm_tmp[0] & 0x20) ? 2 : 0) + ((cmd_parm_tmp[0] & 0x02) ? 1 : 0);
  video_framing = /* IS = [1:0] */
      ((cmd_parm_tmp[0] & 0x08) ? 2 : 0) + ((cmd_parm_tmp[0] & 0x01) ? 1 : 0);

  /* P2 = param[1] = AW = active display words per line - 2. must be even
   * number. */
  ...
  LOG_MSG("GDC: RESET/SYNC MASTER=%u DOOR=%u DRAM=%u DISP=%u VFRAME=%u AW=%u "
          "HS=%u VS=%u HFP=%u HBP=%u VFP=%u AL=%u VBP=%u",
          master_sync, draw_only_during_retrace ? 1 : 0,
          dynamic_ram_refresh ? 1 : 0, display_mode, video_framing,
          active_display_words_per_line, horizontal_sync_width,
          vertical_sync_width, horizontal_front_porch_width,
          horizontal_back_porch_width, vertical_front_porch_width,
          active_display_lines, vertical_back_porch_width);
...
void PC98_GDC_state::idle_proc(void) {
  ...
  if (val & 0x100) { // command
    ...
    case GDC_CMD_VERTICAL_SYNC_MODE +
        1: // 0x6F        M=generate and output vertical sync (0=or else accept
           // external vsync)
    ...
    case GDC_CMD_PARAMETER_RAM_LOAD +
        1: // 0x71       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        2: // 0x72       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        3: // 0x73       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        4: // 0x74       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        5: // 0x75       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        6: // 0x76       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        7: // 0x77       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        8: // 0x78       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        9: // 0x79       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        10: // 0x7A       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        11: // 0x7B       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        12: // 0x7C       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        13: // 0x7D       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        14: // 0x7E       S=starting byte in parameter RAM
    case GDC_CMD_PARAMETER_RAM_LOAD +
        15: // 0x7F       S=starting byte in parameter RAM
    ...
    case GDC_CMD_CURSOR_ADDRESS_READ: // 0xE0       1 1 1 0 0 0 0 0
      ...
      break;
    default:
      LOG_MSG("GDC: %s: Unknown command 0x%x", master_sync ? "master" : "slave",
              current_command);
      break;
    }
  } else {
    ...
    case GDC_CMD_PARAMETER_RAM_LOAD:
       ...
       *
       * PC-98 Windows 3.1 requires us to do this or else lines do not draw
       * properly. */
      if (param_ram_wptr >= 8 && param_ram_wptr <= 15)
        draw.tx[param_ram_wptr - 8] = (uint8_t)val;
      param_ram[param_ram_wptr] = (uint8_t)val;
      if ((++param_ram_wptr) >= 16)
        param_ram_wptr = 0;
      break;
    }
  }
...
bool PC98_GDC_state::fifo_empty(void) { return (fifo_read >= fifo_write); }
...
void PC98_GDC_state::next_line(void) {
  ...
  if (row_line == row_height ||
      /*HACK! See comments!*/ (!master_sync /*graphics layer*/ &&
                               (pc98_gdc_vramop & (1u << VOPBIT_VGA)))) {
    /* NTS: According to real PC-9821 hardware, doublescan is ignored entirely
     * in 256-color mode. The bits are still there in the GDC, and doublescan
     * comes right back when 256-color mode is switched off. Perhaps the
     * hardware uses an entirely different address counting register during
     * video raster? Forcing this case at all times for 256-color mode is meant
     * to emulate that fact. This fixes issues with booting an HDI image of
     * "Alone in the Dark" */
...
void PC98_GDC_state::begin_frame(void) {
  ...
  /* the actual starting address is determined by the display partition in
   * parameter RAM */
  load_display_partition();
}
...
void PC98_GDC_state::load_display_partition(void) {
  ...
  if (master_sync) { /* character mode */
    ...
    IM_bit = !!(pram[3] & 0x40); /* increment the address every other cycle if
                                    set, mixed text/graphics only */
  }
}
...
void PC98_GDC_state::reset_fifo(void) { fifo_read = fifo_write = 0; }

void PC98_GDC_state::reset_rfifo(void) { rfifo_read = rfifo_write = 0; }

void PC98_GDC_state::flush_fifo_old(void) {
  if (fifo_read != 0) {
    unsigned int sz = (fifo_read <= fifo_write)
                          ? ((unsigned int)fifo_write - (unsigned int)fifo_read)
                          : 0u;
...
uint8_t PC98_GDC_state::read_status(void) {
  ...
  if (vga_render_on_demand && display_enable)
    VGA_RenderOnDemandUpTo();
...
void gdc_proc_schedule_done(void) { gdc_proc_delay_set = false; }
...
bool gdc_5mhz_according_to_bios(void) { return !!(mem_readb(0x54D) & 0x04); }
...
void gdc_5mhz_mode_update_vars(void) {
  ...
  /* bit[5:5] = GDC at 5.0MHz at boot up (copy of DIP switch 2-8 at startup)
   * 1=yes 0=no bit[2:2] = GDC clock 1=5MHz 0=2.5MHz */
...
void pc98_gdc_write(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0x04: /* 0x64: set trigger to signal vsync interrupt (IRQ 2) */
    ...
    if (port == 0x64)
      ...
      /* NTS: Testing on real hardware suggests that this bit is NOT double
       * buffered, therefore you can cause a tearline flipping the bit
       * mid-screen without waiting for vertical blanking.
       *
       * But: For the user's preference, we do offer a hack to delay display
       * plane change until vsync to try to alleviate tearlines. */
  ...
  case 0x08: /* 0xA8: One of two meanings, depending on 8 or 16/256--color mode
              */
    /*         8-color: 0xA8-0xAB are 8 4-bit packed fields remapping the 3-bit
     * GRB colors. This defines colors #3 [7:4] and #7 [3:0] 16-color: GRB color
     * palette index */
    /* 0x68: A command */
    /* NTS: Sadly, "undocumented PC-98" reference does not mention the analog
     * 16-color palette. */
    if (port == 0xA8) {
      if (vga_render_on_demand)
        VGA_RenderOnDemandUpTo();
      if (gdc_analog) { /* 16/256-color mode */
        pc98_16col_analog_rgb_palette_index = (uint8_t)
            val; /* it takes all 8 bits I assume because of 256-color mode */
      } else {
    ...
    break;
  case 0x0A:            /* 0xAA:
                           8-color: Defines color #1 [7:4] and color #5 [3:0] (FIXME: Or is
                           it 2 and 6, by undocumented PC-98???)            16-color: 4-bit green
                           intensity. Color index is low 4 bits of palette index.
                           256-color: 4-bit green intensity. Color index is 8-bit palette
                           index. */
    if (port == 0xAA) { /* TODO: If 8-color... else if 16-color... else if
                           256-color... */
      if (vga_render_on_demand)
        VGA_RenderOnDemandUpTo();
      if (gdc_analog) { /* 16/256-color mode */
        if (pc98_gdc_vramop & (1 << VOPBIT_VGA)) {
          pc98_pal_vga[(3 * pc98_16col_analog_rgb_palette_index) + 0] =
              (uint8_t)val;
          vga.dac.rgb[pc98_16col_analog_rgb_palette_index].green = (uint8_t)val;
          VGA_DAC_UpdateColor(pc98_16col_analog_rgb_palette_index);
        } else {
          pc98_pal_analog[(3 * (pc98_16col_analog_rgb_palette_index & 0xF)) +
                          0] = val & 0x0F;
          vga.dac.rgb[pc98_16col_analog_rgb_palette_index & 0xF].green =
              dac_4to6(val & 0xF); /* re-use VGA DAC */
    ...
    break;
  case 0x0C:            /* 0xAC:
                           8-color: Defines color #2 [7:4] and color #6 [3:0] (FIXME: Or is
                           it 1 and 4, by undocumented PC-98???)            16-color: 4-bit red
                           intensity. Color index is low 4 bits of palette index.
                           256-color: 4-bit red intensity. Color index is 8-bit palette
                           index. */
    if (port == 0xAC) { /* TODO: If 8-color... else if 16-color... else if
                           256-color... */
      if (vga_render_on_demand)
        VGA_RenderOnDemandUpTo();
      if (gdc_analog) { /* 16/256-color mode */
        if (pc98_gdc_vramop & (1 << VOPBIT_VGA)) {
          pc98_pal_vga[(3 * pc98_16col_analog_rgb_palette_index) + 1] =
              (uint8_t)val;
          ...
          pc98_pal_analog[(3 * (pc98_16col_analog_rgb_palette_index & 0xF)) +
                          1] = val & 0x0F;
          vga.dac.rgb[pc98_16col_analog_rgb_palette_index & 0xF].red =
              dac_4to6(val & 0xF); /* re-use VGA DAC */
          VGA_DAC_UpdateColor(pc98_16col_analog_rgb_palette_index & 0xF);
        }
      } else {
  ...
  case 0x0E:            /* 0xAE:
                           8-color: Defines color #2 [7:4] and color #6 [3:0] (FIXME: Or is
                           it 1 and 4, by undocumented PC-98???)            16-color: 4-bit blue
                           intensity. Color index is low 4 bits of palette index.
                           256-color: 4-bit blue intensity. Color index is 8-bit palette
                           index. */
    if (port == 0xAE) { /* TODO: If 8-color... else if 16-color... else if
                           256-color... */
      if (vga_render_on_demand)
        VGA_RenderOnDemandUpTo();
      if (gdc_analog) { /* 16/256-color mode */
        if (pc98_gdc_vramop & (1 << VOPBIT_VGA)) {
          pc98_pal_vga[(3 * pc98_16col_analog_rgb_palette_index) + 2] =
              (uint8_t)val;
          ...
          pc98_pal_analog[(3 * (pc98_16col_analog_rgb_palette_index & 0xF)) +
                          2] = val & 0x0F;
          vga.dac.rgb[pc98_16col_analog_rgb_palette_index & 0xF].blue =
              dac_4to6(val & 0xF); /* re-use VGA DAC */
          VGA_DAC_UpdateColor(pc98_16col_analog_rgb_palette_index & 0xF);
        }
      } else {
  ...
  default:
  unknown:
    LOG_MSG("GDC unexpected write to port 0x%x val=0x%x", (unsigned int)port,
            (unsigned int)val);
    break;
  }
}
...
Bitu pc98_gdc_read(Bitu port, Bitu iolen) {
  ...
  case 0x00: /* 0x60/0xA0 read status */
    /* If a game is polling this port a lot, it might be waiting for vertical
     * retrace or doing raster effects */
    if (vga_render_on_demand)
      VGA_RenderOnDemandUpTo();
    return gdc->read_status();
  case 0x02: /* 0x62/0xA2 read fifo */
    if (!gdc->rfifo_has_content())
      return gdc->read_status(); // FIXME this stops "Battle Skin Panic" from
                                 // getting stuck is this correct behavior?
    ...
    if (port == 0xAA) { /* TODO: If 8-color... else if 16-color... else if
      ...
      if (gdc_analog) { /* 16/256-color mode */
        if (pc98_gdc_vramop & (1 << VOPBIT_VGA))
          ...
          return pc98_pal_analog[(3 *
                                  (pc98_16col_analog_rgb_palette_index & 0xF)) +
                                 0];
      } else {
...
const UINT8 gdc_defsyncm15[8] = {0x10, 0x4e, 0x07, 0x25,
                                 0x0d, 0x0f, 0xc8, 0x94};
const UINT8 gdc_defsyncs15[8] = {0x06, 0x26, 0x03, 0x11,
                                 0x86, 0x0f, 0xc8, 0x94};

const UINT8 gdc_defsyncm24[8] = {0x10, 0x4e, 0x07, 0x25,
                                 0x07, 0x07, 0x90, 0x65};
const UINT8 gdc_defsyncs24[8] = {0x06, 0x26, 0x03, 0x11,
                                 0x83, 0x07, 0x90, 0x65};

const UINT8 gdc_defsyncm31[8] = {0x10, 0x4e, 0x47, 0x0c,
                                 0x07, 0x0d, 0x90, 0x89};
const UINT8 gdc_defsyncs31[8] = {0x06, 0x26, 0x41, 0x0c,
                                 0x83, 0x0d, 0x90, 0x89};

const UINT8 gdc_defsyncm31_480[8] = {0x10, 0x4e, 0x4b, 0x0c,
                                     0x03, 0x06, 0xe0, 0x95};
const UINT8 gdc_defsyncs31_480[8] = {0x06, 0x26, 0x41, 0x0c,
                                     0x83, 0x06, 0xe0, 0x95};

/* ^ NTS: Even in 256-color mode the expectation for Active Display Words is 40,
 * not 80. Writing 80 when INT 18h is used to invoke 256-color mode isn't a
 * problem but it does cause erroneous values in debug functions (1280x480 for
 *        "login 256-color paint tool"?). The correct value is 40 according to
 * NP2 source code (in what cases exactly?) and some games that manually set up
 *        640x400 16-color mode AND THEN directly switch to 256-color mode
 * (still 640x400) expect the ADW to remain 40 ("battle skin panic"). */
