...
bool has_pcibus_enable(void) { return pcibus_enable; }

static uint32_t pci_caddress = 0; // current PCI addressing

static PCI_Device *pci_devices[PCI_MAX_PCIBUSSES][PCI_MAX_PCIDEVICES] = {
    {NULL}}; // registered PCI devices
...
static void write_pci(Bitu port, Bitu val, Bitu iolen) {
  if (log_pci)
    LOG(LOG_PCI, LOG_DEBUG)("Write PCI data port %x :=%x (len %d)", (int)port,
                            (int)val, (int)iolen);
  ...
  if (pci_caddress & 0x80000000) {
    ...
    if (log_pci)
      LOG(LOG_PCI,
          LOG_DEBUG)("  Write to device %x register %x (function %x) (:=%x)",
                     (int)devnum, (int)regnum, (int)fctnum, (int)val);

    if (busnum >= PCI_MAX_PCIBUSSES)
      return;
    if (devnum >= PCI_MAX_PCIDEVICES)
      return;
...
static Bitu read_pci(Bitu port, Bitu iolen) {
  ...
  if (pci_caddress & 0x80000000UL) {
    ...
    if (log_pci)
      LOG(LOG_PCI, LOG_DEBUG)("  Read from device %x register %x (function %x)",
                              (int)devnum, (int)regnum, (int)fctnum);
...
PCI_Device::PCI_Device(uint16_t vendor, uint16_t device) {
  ...
  host_writew(config_writemask + 0x04,
              0x0403); /* allow changing mem/io enable and interrupt disable */
}
...
public:
  static uint16_t GetDevice(void) {
    switch (s3Card) {
    case S3_86C928:
      return 0x88B0; // FIXME: Datasheet does not list PCI info at all. @TC1995
                     // suggests the PCI version return 0xb0 for CR30, so this
                     // is probably the same here.
    case S3_Vision864:
      return 0x88C0; // Vision864, 0x88C0 or 0x88C1
    case S3_Vision868:
      return 0x8880; // Vision868, 0x8880 or 0x8881. S3 didn't list this in
                     // their datasheet, but Windows 95 INF files listed it
                     // anyway
    case S3_Vision964:
    ...
    return 0x8811; // Trio64 DOSBox SVN default even though SVN is closer to
  ...
  PCI_VGADevice() : PCI_Device(vendor, GetDevice()) {
    ...
    if (s3Card == S3_Trio64V)
      config[0x08] = 0x40; // Trio64V+ datasheet, page 280, PCI "class code".
                           // "Hardwired to 0300004xh" (revision is 40h or more)
    else
      config[0x08] = 0x00; // Trio32/Trio64 datasheet, page 242, PCI "class
                           // code". "Hardwired to 03000000h"
    ...
    // reset
    config[0x04] = 0x23; // command register (vga palette snoop, ports enabled,
    ...
    host_writew(
        config_writemask + 0x04,
        0x0023); /* allow changing mem/io enable and VGA palette snoop */
    ...
    case S3_Trio64:
      host_writed(
          config_writemask + 0x10,
          0xFF800000); /* BAR0: memory resource 8MB aligned [22:0 reserved] */
      break;
    case S3_Vision868:
    case S3_Vision968:
    case S3_Trio64V:
    ...
    case S3_ViRGEVX:
      host_writed(
          config_writemask + 0x10,
          0xFC000000); /* BAR0: memory resource 64MB aligned [25:0 reserved] */
      break;
    default:
      host_writed(
          config_writemask + 0x10,
...
public:
  PCI_SSTDevice(Bitu type) : PCI_Device(vendor, (type == 2) ? 0x0002 : 0x0001) {
    ...
    host_writew(config_writemask + 0x04,
                0x0123); /* allow changing mem/io enable, B2B enable, and VGA
                            palette snoop */

    host_writed(config_writemask + 0x10,
                0xFF000000); /* BAR0: memory resource 16MB aligned */
    host_writed(config + 0x10,
                (((uint32_t)VOODOO_INITIAL_LFB) & 0xfffffff0) | 0x8);
  ...
  void config_write(uint8_t regnum, Bitu iolen, uint32_t value) override {
    if (iolen == 1) {
      ...
      case 0x13:
        VOODOO_PCI_SetLFB(
            host_readd(config + 0x10) &
            0xfffffff0UL); /* need to act on the new (masked off) value */
        break;
      case 0x40:
      ...
      PCI_Device::config_write(
          regnum, iolen, value); /* which will break down I/O into 8-bit */
    }
  }
    ...
    if (iolen == 1) {
      ...
      case 0x54: /* FIXME: I hope I ported this right --J.C. */
        if (getDeviceID() >= 2) {
          oscillator_ctr++;
          pci_ctr--;
          return (oscillator_ctr |
                  (((unsigned long)pci_ctr << 16ul) & 0x0fff0000ul)) &
                 0xffu;
        }
        break;
      case 0x55:
      ...
      return PCI_Device::config_read(
          regnum, iolen); /* which will break down I/O into 8-bit */
    }
  }
};
...
Bits RegisterPCIDevice(PCI_Device *device, Bits bus = -1, Bits slot = -1) {
  ...
  if (bus < 0 || slot < 0) {
    ...
    try_slot = (slot < 0)
                   ? 0
                   : slot; /* NTS: Most PCI implementations have a motherboard
                              chipset or PCI bus device at slot 0 */
  ...
  if (pci_devices[bus][slot] != NULL)
    E_Exit("PCI interface error: attempted to fill slot already taken");
...
void PCI_AddSVGAS3_Device(void) {
  if (!pcibus_enable || !enable_pci_vga)
    return;
...
void PCI_AddSST_Device(Bitu type) {
  ...
  if (SST_PCI == NULL) {
    ...
    case 2:
      ...
      break;
    default:
      LOG(LOG_PCI, LOG_WARN)("PCI:SST: Invalid board type %x specified",
                             (int)type);
      break;
    }
...
bool PCI_IsInitialized() { return IsInitialized(); }
...
void PCIBUS_Init() {
  ...
  AddVMEventFunction(VM_EVENT_POWERON,
                     AddVMEventFunctionFuncPair(PCI_OnPowerOn));

  /* NTS: PCI emulation does not have to change anything when entering into
   * PC-98 mode. I/O ports for PCI bus control are the SAME on both platforms
   * (0xCF8-0xCFF). */
}
