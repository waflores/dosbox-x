...
extern "C" unsigned char *CGetMemBase() { return MemBase; }
...
REG8 keystat_getjoy(void) { return 0xFF; }
...
void pic_setirq(REG8 irq) { PIC_ActivateIRQ(irq); }

void pic_resetirq(REG8 irq) { PIC_DeActivateIRQ(irq); }
...
UINT8 board86_encodeirqidx(const unsigned char idx) {
  ...
  return ((idx & 1) ? 0x08 : 0x00) + ((idx & 2) ? 0x04 : 0x00);
}
...
// for more information see chapter 13 of
...
static Bitu SOUNDROM_INTD2_PC98_Handler(void) {
  ...
  default:
  unknown:
    LOG_MSG("PC-98 SOUND BIOS (INT D2h) call '%s' with AX=%04X BX=%04X CX=%04X "
            "DX=%04X SI=%04X DI=%04X DS=%04X ES=%04X",
            call_name, reg_ax, reg_bx, reg_cx, reg_dx, reg_si, reg_di,
...
bool PC98_FM_SoundBios_Enabled(void) { return pc98_soundbios_enabled; }
...
void PC98_FM_OnEnterPC98(Section *sec) {
  ...
  if (!pc98fm_init) {
    ...
    pc98_soundbios_rom_load =
        pc98_section->Get_bool("pc-98 load sound bios rom file");
    pc98_set_msw4_soundbios();
    ...
    if (pc98_soundbios_enabled) {
      ...
      if (LoadSoundBIOS()) {
        ...
        soundbios_callback.Install(&SOUNDROM_INTD2_PC98_Handler, CB_IRET,
                                   "Sound ROM INT D2h");

        /* fake INT D2h sound BIOS entry point at CEE0:0000.
         * Unlike the LIO interface there's only one interrupt (if Neko Project
         * II code is correct) */
    ...
    if (board == "board86c" || board == "auto") {
      ...
      LOG_MSG("PC-98 FM board is PC-9801-86c at baseio=0x%x irq=%d", baseio,
              fmtimer_index2irq(fmirqidx));
      ...
      LOG_MSG("PC-98 FM board is PC-9801-86 at baseio=0x%x irq=%d", baseio,
              fmtimer_index2irq(fmirqidx));
      ...
      LOG_MSG("PC-98 FM board is PC-9801-26k at baseio=0x%x irq=%d", baseio,
              fmtimer_index2irq(fmirqidx));
      ...
      LOG_MSG("WARNING: This is not yet implemented!"); // board14 emulation
                                                        // requires emulation of
                                                        // a PIT timer (8253) on
                                                        // the board itself
    ...
    // WARNING: Some parts of the borrowed code assume 44100, 22050, or 11025
    // and
    //          will misrender if given any other sample rate (especially the
    //          OPNA synth).
...
Bitu PC98_AVSDRV_PCM_Handler(void) {
  ...
  case 0x10:
    // $PCMTRN
    {
      ...
      pcb_data[pcb_write].size =
          real_readw(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x0a);
      pcb_data[pcb_write].data_off =
          real_readw(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x06);
      if (real_readb(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 2) ==
          0x01) {
        pcb_data[pcb_write].data_seg = avsdrv_ems_pageframe;
        pcb_data[pcb_write].buffer = new uint8_t[pcb_data[pcb_write].size];
        MEM_BlockRead(PhysMake(pcb_data[pcb_write].data_seg,
                               pcb_data[pcb_write].data_off),
                      pcb_data[pcb_write].buffer, pcb_data[pcb_write].size);
      } else {
        pcb_data[pcb_write].data_seg =
            real_readw(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x08);
        pcb_data[pcb_write].buffer = NULL;
      }
      pcb_data[pcb_write].use_size = 0;

      uint8_t value =
          real_readb(pcb_data[pcb_write].seg, pcb_data[pcb_write].off);
      if (avsdrv_pcm != value && value != 0xff) {
        avsdrv_pcm = value;
        pcm86io_setpcm(avsdrv_pcm);
      }
      value =
          real_readb(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x01);
      if (avsdrv_freq != value && value != 0xff) {
        avsdrv_freq = value;
        pcm86io_setfreq(avsdrv_freq);
      }
      real_writew(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x10,
                  pcb_data[pcb_write].data_seg);
      real_writew(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x0e,
                  pcb_data[pcb_write].data_off);
      uint8_t data[2];
      ...
      for (uint16_t len = 0; len < pcb_data[pcb_write].size; len += step) {
        if (pcm86.realbuf >= PCM86_REALBUFSIZE - 4 ||
            (len >= AVSDRV_OUTPUT_SIZE && pcb_data[pcb_write].buffer == NULL)) {
          break;
        }
        if (pcb_data[pcb_write].buffer) {
          data[0] = pcb_data[pcb_write].buffer[pcb_data[pcb_write].pos++];
        } else {
          data[0] = real_readb(pcb_data[pcb_write].data_seg,
                               pcb_data[pcb_write].pos++);
        }
        ...
        if (step == 2) {
          ...
          if (pcb_data[pcb_write].buffer) {
            ...
            data[1] = real_readb(pcb_data[pcb_write].data_seg,
                                 pcb_data[pcb_write].pos++);
          }
      ...
      real_writew(pcb_data[pcb_write].seg, pcb_data[pcb_write].off + 0x03,
                  0xff);
  ...
  unknown:
    LOG_MSG(
        "PC-98 AVSDRV.SYS PCM (INT D9h) call '%s' with AX=%04X ES=%04X BX=%04X",
        call_name, reg_ax, SegValue(es), reg_bx);
    break;
  }
  CALLBACK_SCF(false);
  return CBRET_NONE;
...
static void avsdrv_check_size(SINT32 size) {
  if (pcb_data[pcb_read].status == 0xff) {
    ...
    if (pcb_data[pcb_read].use_size >
        (pcb_data[pcb_read].pos - pcb_data[pcb_read].data_off) / 2) {
      uint8_t data[2];
      uint16_t step = (avsdrv_pcm & 0x80) ? 2 : 1;
      uint16_t data_end = pcb_data[pcb_read].data_off + pcb_data[pcb_read].size;
      for (uint16_t len = 0;
           len < AVSDRV_OUTPUT_SIZE && pcb_data[pcb_read].pos < data_end;
           len += step) {
        if (pcm86.realbuf >= PCM86_REALBUFSIZE - 4) {
          break;
        }
        if (pcb_data[pcb_read].buffer) {
          ...
          data[0] =
              real_readb(pcb_data[pcb_read].data_seg, pcb_data[pcb_read].pos++);
        }
        pcm86io_outpcm(data[0]);
        if (step == 2) {
          if (pcb_data[pcb_read].buffer) {
            ...
            data[1] = real_readb(pcb_data[pcb_read].data_seg,
                                 pcb_data[pcb_read].pos++);
          }
          pcm86io_outpcm(data[1]);
        }
        if ((avsdrv_pcm & 0x70) != 0x20) {
    ...
    if (pcb_data[pcb_read].use_size >= pcb_data[pcb_read].size) {
      ...
      real_writew(pcb_data[pcb_read].seg, pcb_data[pcb_read].off + 0x12,
                  use_size);
      real_writew(pcb_data[pcb_read].seg, pcb_data[pcb_read].off + 0x0e,
                  pcb_data[pcb_read].data_off + use_size);
      if (pcb_data[pcb_read].buffer) {
  ...
  if (avsdrv_play && pcm86.realbuf == 0) {
    ...
    if (pcb_read != pcb_write && pcm86.realbuf != 0 &&
        (pcm86.fifo & 0x80) == 0) {
      avsdrv_play = true;
      pcm86.fifo |= 0x80;
    }
