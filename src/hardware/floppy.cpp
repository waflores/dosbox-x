...
// largest sector size supported here. Size code represents size as 2 << (7 +
// code) i.e. code == 2 is 2 << 9 = 512 bytes
...
public:
  ...
  bool data_read_expected;  /* 0x3F4 bit 6 (DIO) if set CPU is expected to read
                               from the controller */
  bool non_dma_mode;        /* 0x3F4 bit 5 (NDMA) */
  bool busy_status; /* 0x3F4 bit 4 (BUSY). By "busy" the floppy controller is in
...
bool FDC_AssignINT13Disk(unsigned char drv) {
  ...
  if (IS_PC98_ARCH) {
    // HACK: Enable motor by default, until motor enable port 0xBE is
    // implemented
    dev->set_motor(true);
  }

  LOG_MSG("FDC: Primary controller, drive %u assigned to INT 13h drive %u", drv,
          drv);
  return true;
}
...
bool FDC_UnassignINT13Disk(unsigned char drv) {
  ...
  LOG_MSG("FDC: Primary controller, drive %u unassigned from INT 13h drive %u",
          drv, drv);
  return true;
}
...
void FDC_MotorStep(Bitu idx /*which IDE controller*/) {
  ...
  if (fdc->motor_steps != 0) {
    ...
    if (dev != NULL && dev->current_track != fdc->current_cylinder[devidx])
      LOG_MSG("FDC: warning, after motor step FDC and drive are out of sync "
              "(fdc=%u drive=%u). OS or App needs to recalibrate\n",
              fdc->current_cylinder[devidx], dev->current_track);

    //		LOG_MSG("FDC: motor step finished.
...
double FloppyDevice::floppy_image_motor_position() {
  ...
  if (!motor)
    return 0.0;
  return fmod((PIC_FullIndex() * motor_rpm /*rotations/minute*/) /
                  1000 /*convert to seconds from milliseconds*/ /
                  60 /*rotations/min to rotations/sec*/,
              1.0);
}
...
void FloppyDevice::set_select(bool enable) { select = enable; }

void FloppyDevice::set_motor(bool enable) { motor = enable; }
...
bool FloppyController::drive_motor_on(unsigned char index) {
  if (index < 4)
    return !!((digital_output_register >> (4u + index)) & 1);
  return false;
}

int FloppyController::drive_selected() { return (digital_output_register & 3); }

bool FloppyController::dma_enabled() {
  return (digital_output_register &
          0x08); /* bit 3 of DOR controls DMA/IRQ enable */
}

bool FloppyController::irq_enabled() {
  /* IRQ seems to be enabled, always, on PC-98. There does not seem to be an
   * enable bit. */
  if (IS_PC98_ARCH)
    return true;
...
static void FDC_Init(Section *sec, unsigned char fdc_interface) {
  ...
  LOG(LOG_MISC, LOG_DEBUG)("Initializing floppy controller interface %u",
                           fdc_interface);

  {
    FloppyController *fdc = floppycontroller[fdc_interface] =
        new FloppyController(sec, fdc_interface);
    fdc->install_io_port();
...
void FloppyController::update_ST3() {
  ...
  /* FIXME: Should assemble bits from FloppyDevice objects and their boolean
   * "signal" variables */
  ST[3] = 0x20 /*RDY*/ + 0x08 /*DOUBLE-SIDED SIGNAL*/ +
...
void FloppyController::register_isapnp() {
  if (register_pnp && base_io > 0) {
    ...
    const unsigned char h1[9] = {ISAPNP_SYSDEV_HEADER(
        ISAPNP_ID('P', 'N', 'P', 0x0, 0x7, 0x0,
                  0x0), /* PNP0700 Generic floppy controller */
        ISAPNP_TYPE(0x01, 0x02,
                    0x00), /* type: Mass Storage Device / Floppy / Generic */
        0x0001 | 0x0002)};
    ...
    tmp[i + 6] = 0x01;                 /* align */
    tmp[i + 7] = 0x06; /* length (FIXME: Emit as 7 unless we know IDE interface
                          will not conflict) */
    i += 7 + 1;

    tmp[i + 0] = (8 << 3) | 7; /* IO resource (FIXME: Don't emit this if we know
                                  IDE interface will take port 0x3F7) */
    tmp[i + 1] = 0x01;         /* 16-bit decode */
...
FloppyController::FloppyController(Section *configuration, unsigned char index)
    : Module_base(configuration) {
  Section_prop *section = static_cast<Section_prop *>(configuration);
  int i;

  fdc_motor_step_delay =
      400.0f / 80; /* FIXME: Based on 400ms seek time from track 0 to 80 */
  ...
  if (IS_PC98_ARCH) {
    /* According to Neko Project II source code, and the Undocumented PC-98
     * reference:
     *
     * The primary controller at 0x90-0x94 is for 3.5" drives, uses IRQ 11
     * (INT42), and DMA channel 2. The secondary controller at 0xC8-0xCC is
     * for 5.25" drives, uses IRQ 10 (INT41), and DMA channel 3. */
    if (IRQ < 0)
      IRQ = (index == 1) ? 10 /*INT41*/ : 11 /*INT42*/;
    if (DMA < 0)
      DMA = (index == 1) ? 3 : 2;
...
void FloppyController::install_io_port() {
  if (base_io != 0) {
    ...
    if (IS_PC98_ARCH) {
      WriteHandler[0].Install(base_io + 0, fdc_baseio98_w,
                              IO_MA);                             // 0x90 / 0xC8
      ReadHandler[0].Install(base_io + 0, fdc_baseio98_r, IO_MA); // 0x90 / 0xC8
      WriteHandler[1].Install(base_io + 2, fdc_baseio98_w,
                              IO_MA);                             // 0x92 / 0xCA
      ReadHandler[1].Install(base_io + 2, fdc_baseio98_r, IO_MA); // 0x92 / 0xCA
      WriteHandler[2].Install(base_io + 4, fdc_baseio98_w,
                              IO_MA);                             // 0x94 / 0xCC
...
FloppyController *match_fdc_controller(Bitu port) {
  ...
  for (i = 0; i < MAX_FLOPPY_CONTROLLERS; i++) {
    ...
    if (fdc == NULL)
      continue;
    if (fdc->base_io != 0U && fdc->base_io == (port & 0xFFF8U))
      return fdc;
  }
...
void FloppyController::on_dor_change(unsigned char b) {
  ...
  if (chg & 0x03) {
    ...
    if (o >= 0) {
      ...
      if (device[o] != NULL)
        device[o]->set_select(false);
      if (device[n] != NULL)
        device[n]->set_select(true);
      update_ST3();
    }
  }
  ...
  if (chg & 0xF0) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      if (device[i] != NULL)
        device[i]->set_motor((b & (0x10 << i)) ? true : false);
    }
  }
...
bool fdc_takes_port_3F7() { return (match_fdc_controller(0x3F7) != NULL); }
...
uint8_t FloppyController::fdc_data_read() {
  ...
  //        leave the busy bit set until the response has been read, from my
  //        experience. On the other hand some PC-98 booter games seem to assume
  //        that busy will be cleared when read finishes and will not read
  //        response until busy bit clears (Arsys Star Cruiser).
  //
...
void FloppyController::on_fdc_in_command() {
  ...
  case 0x0A:
    break;
  default:
    LOG_MSG("FDC: Command len=%u %02x %02x %02x %02x %02x %02x %02x %02x %02x "
            "%02x\n",
            in_cmd_len, in_cmd[0], in_cmd[1], in_cmd[2], in_cmd[3], in_cmd[4],
  ...
  case 0x09: /* Write deleted data (1001) */
     ...
     *   7     total
     */
    /* must have a device present. must have an image. device motor and select
     * must be enabled. current physical cylinder position must be within range
     * of the image. request must have MFM bit set. */
    dma = GetDMAChannel(DMA);
    if (dev != NULL && dma != NULL && dev->motor && dev->select &&
        image != NULL && (in_cmd[0] & 0x40) /*MFM=1*/ &&
        current_cylinder[devidx] < image->cylinders &&
        ((in_cmd[1] & 4U) ? 1U : 0U) <= image->heads &&
        ((in_cmd[1] & 4U) ? 1U : 0U) == in_cmd[3] &&
        in_cmd[2] == current_cylinder[devidx] &&
        in_cmd[5] <= FLOPPY_MAX_SECTOR_SIZE_SZCODE && in_cmd[4] > 0U &&
        in_cmd[4] <= image->sectors) {
      ...
      /* TODO: delay related to how long it takes for the disk to rotate around
       * to the sector requested */
      reset_res();
      ST[0] = 0x00 | devidx;

      // FIXME: Real IBM PC hardware (at least Pentium motherboard) behavior
      // says that the FDC is too lazy
      //        to clear/reset ST1/ST2 unless another error occurs.
      //
      //        PC-98 booter game Star Cruiser seems to expect these to zero out
      //        if no error.
      //
      ...
      out_res[4] = in_cmd[3];
      out_res[5] =
          in_cmd[4]; /* the sector passing under the head at this time */
      {
      ...
      if (dma->tcount) {
        LOG(LOG_MISC,
            LOG_DEBUG)("FDC: DMA terminal count at write start. Resetting "
                       "terminal count. Hope a new count was written!");
        dma->tcount = false;
      }
      ...
      while (!fail && !dma->tcount /*terminal count*/) {
        ...
        /* write sector */
        uint8_t err = image->Write_Sector(
            in_cmd[3] /*head*/, in_cmd[2] /*cylinder*/, in_cmd[4] /*sector*/,
            sector, sector_size_bytes);
        if (err != 0x00) {
          fail = true;
          break;
        }

        /* if we're at the last sector of the track according to program, then
         * stop */
        if (in_cmd[4] == in_cmd[6])
          break;
      ...
      /* TODO: real floppy controllers will pause for up to 1/2 a second before
       * erroring out */
      reset_res();
      ST[0] = (ST[0] & 0x1F) | 0x80;
      ST[1] = (1 << 0) /*missing address mark*/ | (1 << 2) /*no data*/;
  ...
  case 0x06: /* Read data (0110) */
    ...
    /* must have a device present. must have an image. device motor and select
     * must be enabled. current physical cylinder position must be within range
     * of the image. request must have MFM bit set. In order to support
     * copy-protected booter games (IBM PC or PC-98) the sector number is NOT
     * range limited in order to support games such as Star Cruiser that look
     * for intentionally out of range sector numbers. */
    dma = GetDMAChannel(DMA);
    if (dev != NULL && dma != NULL && dev->motor && dev->select &&
        image != NULL && (in_cmd[0] & 0x40) /*MFM=1*/ &&
        current_cylinder[devidx] < image->cylinders &&
      ...
      if (dma->tcount) {
        LOG(LOG_MISC,
            LOG_DEBUG)("FDC: DMA terminal count at read start. Resetting "
                       "terminal count. Hope a new count was written!");
        dma->tcount = false;
      }

      while (!fail && !dma->tcount /*terminal count*/) {
        //                  LOG_MSG("FDC: Read sector going to DMA
        //                  0x%x",(unsigned int)dma->pagebase + (unsigned
        //                  int)dma->curraddr);

        /* read sector */
        uint8_t err =
            image->Read_Sector(in_cmd[3] /*head*/, in_cmd[2] /*cylinder*/,
                               in_cmd[4] /*sector*/, sector, sector_size_bytes);
        if (err != 0x00) {
          fail = true;
          break;
        }
    ...
    //        IBM compatible Pentium motherboards might have FDCs that
    //        leave the busy bit set until the response has been read, from my
    //        experience. On the other hand some PC-98 booter games seem to
    //        assume that busy will be cleared when read finishes and will not
    //        read response until busy bit clears (Arsys Star Cruiser).
    //
    //        This is a compromise until further study.
    if (IS_PC98_ARCH)
      busy_status = 0;
  ...
  case 0x07: /* Calibrate drive */
    ...
    if (instant_mode) {
      ...
      motor_steps = 79; /* calibrate is said to max out at 79. motor step
                           routine will STOP when drive sets track0 signal */
      ...
      /* and make it happen */
      PIC_AddEvent(FDC_MotorStep,
                   (motor_steps > 0 ? fdc_motor_step_delay : 0.1) /*ms*/,
                   interface_index);
  ...
  case 0x08: /* Check Interrupt Status */
    ...
    if (irq_pending) {
       ...
       * this also fixes an issue where Windows 3.1 QIC tape backup software
       * like Norton Backup (Norton Desktop 3.0) will "hang" Windows 3.1 polling
       * the FDC in an endless loop if we don't return this error to say that no
  ...
  case 0x0A: /* Read ID */
     ...
     * of the image. request must have MFM bit set. */
    ST[0] &= ~0x20;
    if (dev != NULL && dev->motor && dev->select && image != NULL &&
        (in_cmd[0] & 0x40) /*MFM=1*/ &&
        current_cylinder[devidx] < image->cylinders &&
        ((in_cmd[1] & 4U) ? 1U : 0U) <= image->heads) {
  ...
  case 0x0C: /* Read deleted data (1100) */
     ...
     *   7     total
     */
    /* the raw images used by DOSBox cannot represent "deleted sectors", so
     * always fail */
    reset_res();
    ST[0] = (ST[0] & 0x1F) | 0x80;
    ST[1] = (1 << 0) /*missing address mark*/ | (1 << 2) /*no data*/;
  ...
  case 0x0F: /* Seek Head */
    ...
    if (instant_mode) {
      ...
      /* delay due to stepping the head to the desired cylinder */
      motor_steps =
          (unsigned int)abs((int)in_cmd[2] - (int)current_cylinder[devidx]);
  ...
  case 0x0A:
    break;
  default:
    LOG_MSG("FDC: Response len=%u %02x %02x %02x %02x %02x %02x %02x %02x %02x "
            "%02x\n",
            out_res_len, out_res[0], out_res[1], out_res[2], out_res[3],
            out_res[4], out_res[5], out_res[6], out_res[7], out_res[8],
            out_res[9]);
    break;
  }
}
...
void FloppyController::fdc_data_write(uint8_t b) {
  if (!busy_status) {
    ...
    case 0x0F: /* Seek Head */
       ...
       *   3     total
       */
      if (in_cmd[0] & 0x80) { /* Reject Seek Relative (1xfh) most FDC's I've
                                 tested don't support it */
...
static void fdc_baseio98_w(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (fdc == NULL) {
    LOG_MSG("WARNING: port read from I/O port not registered to FDC, yet "
            "callback triggered\n");
    return;
  }

  //	LOG_MSG("FDC: Write port 0x%03x data 0x%02x
  //irq_at_time=%u\n",port,val,fdc->irq_pending);

  if (iolen > 1) {
    LOG_MSG("WARNING: FDC unusual port write %03xh val=%02xh len=%u, port I/O "
            "should be 8-bit\n",
  ...
  case 2: /* data */
    if (!fdc->data_register_ready) {
      ...
      LOG_MSG("WARNING: FDC data write when data port ready but expecting I/O "
              "read\n");
    } else {
...
static Bitu fdc_baseio98_r(Bitu port, Bitu iolen) {
  ...
  if (iolen > 1) {
    LOG_MSG("WARNING: FDC unusual port read %03xh len=%u, port I/O should be "
            "8-bit\n",
            (int)port, (int)iolen);
  }
  ...
  case 0: /* main status */
    b = (fdc->data_register_ready ? 0x80 : 0x00) +
        ...
        (fdc->non_dma_mode ? 0x20 : 0x00) + (fdc->busy_status ? 0x10 : 0x00) +
  ...
  case 2: /* data */
    if (!fdc->data_register_ready) {
      ...
      LOG_MSG("WARNING: FDC data read when data port ready but expecting I/O "
              "write\n");
      return ~(0UL);
    }
