...
Bitu ATIExtIndex_Read(Bitu /*port*/, Bitu /*len*/) { return ati_state.index; }
...
Bitu ATIExtData_Read(Bitu port, Bitu /*len*/) {
  ...
  LOG(LOG_MISC, LOG_DEBUG)("Unhandled ATI extended read port=%x index=%x",
                           (unsigned int)port, ati_state.index);
  return 0;
}
...
void ATIExtData_Write(Bitu port, Bitu val, Bitu /*len*/) {
  ...
  LOG(LOG_MISC,
      LOG_DEBUG)("Unhandled ATI extended write port=%x index=%x val=%x",
                 (unsigned int)port, ati_state.index, (unsigned int)val);
}
...
void SVGA_Setup_ATI(void) {
  ...
  if (vga.mem.memsize >= (512 * 1024))
    ati_state.input_status_register |= 0x20; /* 512KB, not 256KB, of RAM */
