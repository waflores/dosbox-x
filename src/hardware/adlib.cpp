...
struct Handler : public Adlib::Handler {
  void WriteReg(uint32_t reg, uint8_t val) override { adlib_write(reg, val); }
  ...
  void Init(Bitu rate) override { adlib_init((uint32_t)rate); }
...
struct Handler : public Adlib::Handler {
  ...
  void ESFMSetEmulationMode() override { ESFM_write_port(&chip, 0, 0); }
...
struct Handler : public Adlib::Handler {
  ...
  uint32_t WriteAddr(uint32_t /*port*/, uint8_t val) override { return val; }
  void Generate(MixerChannel *chan, Bitu samples) override {
    int16_t buf[1024 * 2];
    while (samples > 0) {
      Bitu todo = samples > 1024 ? 1024 : samples;
...
struct Handler : public Adlib::Handler {
  ...
  uint32_t WriteAddr(uint32_t /*port*/, uint8_t val) override { return val; }
  void Generate(MixerChannel *chan, Bitu samples) override {
    // We generate data for 4 channels, but only the first 2 are connected on a
    // pc
    int16_t buf[4][1024];
...
struct Handler : public Adlib::Handler {
  Handler(const char *port) { opl2AudioBoard.connect(port); }
  void WriteReg(uint32_t reg, uint8_t val) override {
...
struct Handler : public Adlib::Handler {
  Handler(const char *port) { opl3DuoBoard.connect(port); }
  void WriteReg(uint32_t reg, uint8_t val) override {
...
struct Handler : public Adlib::Handler {
  ...
  Handler(const std::string &bus, const std::string &path,
          const std::string &spi_cs) {
    retrowave_init_dosbox(bus, path, spi_cs);
    LOG_MSG("RetroWave: OPL3 class init");
  }

  ~Handler() { retrowave_opl3_reset(&retrowave_global_context); }
};
} // namespace Retrowave_OPL3
...
struct RawHeader {
  ...
  uint32_t milliseconds; /* 0x10, uint32_t Total milliseconds of data in this
                            chunk */
  uint8_t hardware; /* 0x14, uint8_t Hardware Type 0=opl2,1=dual-opl2,2=opl3 */
  uint8_t format;   /* 0x15, uint8_t Format 0=cmd/data interleaved, 1 maybe all
                       cdms, followed by all data */
...
/*
   ...
   raw data follows immediately till the end of the chunk
*/
...
class Capture {
  ...
  void MakeTables(void) {
    ...
    // Select the entries that are valid and the index is the mapping to the
    // index entry
    ...
    MakeEntry(0xbd, index); // BD: Tremolo Depth / Vibrato Depth / Percussion
                            // Mode / BD/SD/TT/CY/HH On
    // Add the 32 byte range that hold the 18 operators
    for (int i = 0; i < 24; i++) {
      if ((i & 7) < 6) {
        MakeEntry(0x20 + i, index); // 20-35: Tremolo / Vibrato / Sustain / KSR
    ...
    for (int i = 0; i < 9; i++) {
      ...
      MakeEntry(0xb0 + i,
                index); // B0-B8: Key On / Block Number / F-Number(hi bits)
      MakeEntry(0xc0 + i,
                index); // C0-C8: FeedBack Modulation Factor / Synthesis Type
    }
  ...
  void AddWrite(uint32_t regFull, uint8_t val) {
    ...
    /*
            ...
            Although you could pretty much just stick to always doing opl3 on
       the player side
    */
    ...
    if (header.hardware == HW_OPL2 && regFull >= 0x1b0 && regFull <= 0x1b8 &&
        val) {
...
public:
  bool DoWrite(uint32_t regFull, uint8_t val) {
    ...
    if (handle) {
      /*
              Check if we actually care for this to be logged, else just ignore
         it
      */
      uint8_t raw = ToRaw[regMask];
      if (raw == 0xff) {
      ...
      */
      ...
      // If we passed more than 30 seconds since the last command, we'll restart
      // the capture
      if (passed > 30000) {
        CloseFile();
        goto skipWrite;
  ...
  skipWrite:
    ...
    // Check for commands that would start capturing, if it's not one of them
    // return
    if (!(
...
uint8_t Chip::Read() {
  ...
  if (adlib_force_timer_overflow_on_polling) {
    static const double poll_timeout =
        0.1; /* if polling more than 100us per second, do timeout */

    if ((time - last_poll) > poll_timeout) {
      poll_counter = 0;
    } else if (++poll_counter >= 50) {
      //			LOG_MSG("Adlib polling hack triggered. Forcing
      //timers to reset. Hope this helps your DOS game to detect Adlib.");
    ...
    case MODE_ESFM:
      if (!chip[0].Write(reg.normal, (uint8_t)val)) {
        if (reg.normal == 0x105 && (val & 0x80)) {
          ...
          if (capture) {
            LOG_MSG("WARNING: ESFM native mode has been enabled by the "
                    "application, but it's not supported during Raw OPL "
                    "capture. Nothing will be captured after this point.");
          }
        }
...
Bitu Module::PortRead(Bitu port, Bitu iolen) {
  (void)iolen; // UNUSED
  // roughly half a micro (as we already do 1 micro on each port read and some
  // tests revealed it taking 1.5 micros to read an adlib port)
  ...
  case MODE_ESFM:
    ...
    case 1:
      if (esfm_nativemode) {
        if ((reg.normal & 0x500) == 0x400) {
          // Emulation mode register pokehole region at 0x400 (mirrored at
          // 0x600)
          uint8_t val;
...
static void OPL_CallBack(Bitu len) {
  ...
  if ((PIC_Ticks - module->lastUsed) > 30000) {
    ...
    if (i == 0xb9)
      ...
      module->lastUsed = (uint32_t)PIC_Ticks;
  }
}

static Bitu OPL_Read(Bitu port, Bitu iolen) {
  if (IS_PC98_ARCH) {
    if (port == 0xC8D2 && iolen == 1 && module->PortRead(port, iolen) == 0xFF &&
        module->PortRead(port / 0x100, iolen) == 0)
      return 0xFF; // fix for First Queen
    port >>=
        8u; // C8D2h -> C8h, C9D2h -> C9h, OPL emulation looks only at bit 0.
  }
...
void OPL_Write(Bitu port, Bitu val, Bitu iolen) {
  ...
  // if writing the data port, assume a change in OPL state that should be
  // reflected immediately. this is a way to render "sample accurate" without
  // needing "sample accurate" mode in the mixer. CHGOLF's Adlib digital audio
  // hack works fine with this hack.
  if (port & 1)
    module->mixerChan->FillUp();
...
void OPL_SaveRawEvent(bool pressed) {
  ...
  if (module->capture) {
    ...
    if (module->oplmode == OPL_esfm && module->esfm_nativemode) {
      LOG_MSG("ERROR: Cannot capture Raw OPL output because ESFM native mode "
              "is being used by the current application, which is not "
              "supported by the Raw OPL format.");
      if (show_recorded_filename) {
        systemmessagebox("Error", MSG_Get("OPL_CAPTURE_FAILED"), "ok", "error",
                         1);
      }
    } else {
      LOG_MSG(
          "Preparing to capture Raw OPL, will start with first note played.");
      module->capture = new Adlib::Capture(&module->cache);
    }
  }
  pathopl = "";

  mainMenu.get_item("mapper_caprawopl")
      .check(module->capture != NULL)
...
Module::Module(Section *configuration) : Module_base(configuration) {
  ...
  adlib_force_timer_overflow_on_polling =
      section->Get_bool("adlib force timer overflow on detect");

  if (oplmode == OPL_esfm && oplemu != "esfmu") {
    LOG_MSG("Adlib: WARN: an ESFM-capable sbtype (or the 'esfm' oplmode) was "
            "chosen, but the chosen oplemu is not ESFM-capable. ESFM native "
            "mode features will not work!");
  ...
  if (oplemu == "esfmu" && oplmode != OPL_esfm) {
    LOG_MSG("Adlib: WARN: an ESFM-capable oplemu was chosen, but the chosen "
            "sbtype (or oplmode) is not ESFM-capable. ESFM native mode "
  ...
  if (oplemu == "compat") {
    ...
    handler = new Retrowave_OPL3::Handler(retrowave_bus, retrowave_port,
                                          retrowave_spi_cs);
  } else if (oplemu == "mame") {
    if (oplmode == OPL_opl2) {
  ...
  // Used to be 2.0, which was measured to be too high. Exact value depends on
  // card/clone.
  ...
  // MAPPER_AddHandler(OPL_SaveRawEvent,MK_nothing,0,"caprawopl","Cap
  // OPL",&item); item->set_text("Record FM (OPL) output");
}
...
std::string getoplmode() {
  if (Adlib::Module::oplmode == OPL_none)
    return "None";
  else if (Adlib::Module::oplmode == OPL_opl2)
    return "OPL2";
  else if (Adlib::Module::oplmode == OPL_dualopl2)
    return "Dual OPL2";
  else if (Adlib::Module::oplmode == OPL_opl3)
    return "OPL3";
  else if (Adlib::Module::oplmode == OPL_opl3gold)
    return "OPL3 Gold";
  else if (Adlib::Module::oplmode == OPL_hardware)
    return "Hardware";
  else if (Adlib::Module::oplmode == OPL_hardwareCMS)
    return "Hardware CMS";
  else if (Adlib::Module::oplmode == OPL_esfm)
    return "ESFM";
  else
    return "Unknown";
}
