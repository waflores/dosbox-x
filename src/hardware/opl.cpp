...
/*
 * Originally based on ADLIBEMU.C, an AdLib/OPL2 emulation library by Ken
 * Silverman Copyright (C) 1998-2001 Ken Silverman Ken Silverman's official web
 * site: "http://www.advsys.net/ken"
 */

/* WARNING: This file is not compiled directly, it is #included from other *.cpp
 * files */
...
static const fltype frqmul_tab[16] = {0.5, 1, 2,  3,  4,  5,  6,  7,
...
static const uint8_t modulatorbase[9] = {0, 1, 2, 8, 9, 10, 16, 17, 18};

// map a register base to a modulator operator number or operator number
#if defined(OPLTYPE_IS_OPL3)
static const uint8_t regbase2modop[44] = {
    0,  1,  2,  0,  1,  2,  0,  0,  3,  4,  5,
    3,  4,  5,  0,  0,  6,  7,  8,  6,  7,  8, // first set
    18, 19, 20, 18, 19, 20, 0,  0,  21, 22, 23,
    21, 22, 23, 0,  0,  24, 25, 26, 24, 25, 26 // second set
};
static const uint8_t regbase2op[44] = {
    0,  1,  2,  9,  10, 11, 0,  0,  3,  4,  5,
    12, 13, 14, 0,  0,  6,  7,  8,  15, 16, 17, // first set
    18, 19, 20, 27, 28, 29, 0,  0,  21, 22, 23,
    30, 31, 32, 0,  0,  24, 25, 26, 33, 34, 35 // second set
};
#else
...
static const uint32_t waveform[8] = {
    WAVEPREC, WAVEPREC >> 1,       WAVEPREC,     (WAVEPREC * 3) >> 2, 0,
    0,        (WAVEPREC * 5) >> 2, WAVEPREC << 1};

// length of the waveform as mask
static const uint32_t wavemask[8] = {
    WAVEPREC - 1,        WAVEPREC - 1, (WAVEPREC >> 1) - 1,
    (WAVEPREC >> 1) - 1, WAVEPREC - 1, ((WAVEPREC * 3) >> 2) - 1,
    WAVEPREC >> 1,       WAVEPREC - 1};

// where the first entry resides
static const uint32_t wavestart[8] = {0, WAVEPREC >> 1, 0, WAVEPREC >> 2, 0, 0,
                                      0, WAVEPREC >> 3};

// envelope generator function constants
static const fltype attackconst[4] = {
    (fltype)(1 / 2.82624), (fltype)(1 / 2.25280), (fltype)(1 / 1.88416),
    (fltype)(1 / 1.59744)};
static const fltype decrelconst[4] = {
    (fltype)(1 / 39.28064), (fltype)(1 / 31.41608), (fltype)(1 / 26.17344),
    (fltype)(1 / 22.44608)};
...
void operator_advance_drums(op_type *op_pt1, int32_t vib1, op_type *op_pt2,
                            int32_t vib2, op_type *op_pt3, int32_t vib3) {
  uint32_t c1 = op_pt1->tcount / FIXEDPT;
  uint32_t c3 = op_pt3->tcount / FIXEDPT;
  uint32_t phasebit =
      (((c1 & 0x88) ^ ((c1 << 5) & 0x80)) | ((c3 ^ (c3 << 2)) & 0x20)) ? 0x02
                                                                       : 0x00;
...
// (->release) or when the keep-sustained bit is turned off (->sustain_nokeep)
void operator_output(op_type *op_pt, int32_t modulator, int32_t trem) {
  if (op_pt->op_state != OF_TYPE_OFF) {
    ...
    op_pt->cval =
        (int32_t)(op_pt->step_amp * op_pt->vol *
...
void operator_sustain(op_type *op_pt) {
  uint32_t num_steps_add =
      op_pt->generator_pos / FIXEDPT; // number of (standardized) samples
...
// operator in release mode, if output level reaches zero the operator is turned
// off
...
void operator_attack(op_type *op_pt) {
  op_pt->amp = ((op_pt->a3 * op_pt->amp + op_pt->a2) * op_pt->amp + op_pt->a1) *
                   op_pt->amp +
               op_pt->a0;
...
optype_fptr opfuncs[6] = {
    operator_attack,  operator_decay, operator_release,
...
void change_attackrate(Bitu regbase, op_type *op_pt) {
  ...
  if (attackrate) {
    fltype f = (fltype)(pow(FL2, (fltype)attackrate + (op_pt->toff >> 2) - 1) *
                        attackconst[op_pt->toff & 3] * recipsamp);
    ...
    if (step_skip >= 62) {
#endif
      op_pt->a0 = (fltype)(2.0); // something that triggers an immediate
                                 // transition to amp:=1.0
      op_pt->a1 = (fltype)(0.0);
      op_pt->a2 = (fltype)(0.0);
      op_pt->a3 = (fltype)(0.0);
    }
...
void change_decayrate(Bitu regbase, op_type *op_pt) {
  ...
  if (decayrate) {
    ...
    op_pt->decaymul = (fltype)(pow(
        FL2, f * pow(FL2, (fltype)(decayrate + (op_pt->toff >> 2)))));
...
void change_releaserate(Bitu regbase, op_type *op_pt) {
  ...
  if (releaserate) {
    fltype f = (fltype)(-7.4493 * decrelconst[op_pt->toff & 3] * recipsamp);
    op_pt->releasemul = (fltype)(pow(
        FL2, f * pow(FL2, (fltype)(releaserate + (op_pt->toff >> 2)))));
...
void change_frequency(Bitu chanbase, Bitu regbase, op_type *op_pt) {
  // frequency
  uint32_t frn = ((((uint32_t)adlibreg[ARC_KON_BNUM + chanbase]) & 3) << 8) +
                 (uint32_t)adlibreg[ARC_FREQ_NUM + chanbase];
  // block number/octave
  ...
  // 20+a0+b0:
  op_pt->tinc = (uint32_t)(((fltype)(frn << oct)) *
                           frqmul[adlibreg[ARC_TVS_KSR_MUL + regbase] & 15]);
  // 40+a0+b0:
  fltype vol_in = (fltype)((fltype)(adlibreg[ARC_KSL_OUTLEV + regbase] & 63) +
                           kslmul[adlibreg[ARC_KSL_OUTLEV + regbase] >> 6] *
                               kslev[oct][frn >> 6]);
...
void adlib_init(uint32_t samplerate) {
  ...
  for (i = 15; i >= 0; i--) {
    frqmul[i] = (fltype)(frqmul_tab[i] * INTFREQU / (fltype)WAVEPREC *
                         (fltype)FIXEDPT * recipsamp);
  }
  ...
  // vibrato at ~6.1 ?? (opl3 docs say 6.1, opl4 docs say 6.0, y8950 docs
  // say 6.4)
  vibtab_add = static_cast<uint32_t>(VIBTAB_SIZE * FIXEDPT_LFO / 8192 *
                                     INTFREQU / int_samplerate);
  vibtab_pos = 0;
  ...
  for (i = 0; i < TREMTAB_SIZE; i++) {
    ...
    fltype trem_val1 =
        (fltype)(((fltype)trem_table_int[i]) * 4.8 / 26.0 / 6.0); // 4.8db
    fltype trem_val2 = (fltype)((fltype)((int32_t)(trem_table_int[i] / 4)) *
                                1.2 / 6.0 / 6.0); // 1.2db (larger stepping)
  ...
  // tremolo at 3.7hz
  tremtab_add = (uint32_t)((fltype)TREMTAB_SIZE * TREM_FREQ * FIXEDPT_LFO /
                           (fltype)int_samplerate);
  tremtab_pos = 0;
  ...
  if (!initfirstime) {
    ...
    for (i = 0; i < (WAVEPREC >> 1); i++) {
      wavtable[(i << 1) + WAVEPREC] =
          (int16_t)(16384 * sin((fltype)(i << 1) * PI * 2 / WAVEPREC));
      wavtable[(i << 1) + 1 + WAVEPREC] =
          (int16_t)(16384 * sin((fltype)((i << 1) + 1) * PI * 2 / WAVEPREC));
      wavtable[i] = wavtable[(i << 1) + WAVEPREC];
      // alternative: (zero-less)
      /*			wavtable[(i<<1)  +WAVEPREC]	=
         (int16_t)(16384*sin((fltype)((i<<2)+1)*PI/WAVEPREC));
                              wavtable[(i<<1)+1+WAVEPREC]	=
         (int16_t)(16384*sin((fltype)((i<<2)+3)*PI/WAVEPREC)); wavtable[i]
         = wavtable[(i<<1)-1+WAVEPREC]; */
    }
    for (i = 0; i < (WAVEPREC >> 3); i++) {
      wavtable[i + (WAVEPREC << 1)] = wavtable[i + (WAVEPREC >> 3)] - 16384;
      wavtable[i + ((WAVEPREC * 17) >> 3)] =
          wavtable[i + (WAVEPREC >> 2)] + 16384;
    }
...
#if defined(OPLTYPE_IS_OPL3)
      // for 4op channels all four operators are modified to the frequency of
      // the channel
  ...
  case ARC_KON_BNUM: {
    ...
    if (base < 9) {
      ...
      if (val & 32) {
        ...
        enable_operator((Bitu)modbase, &op[opbase],
                        OP_ACT_NORMAL); // modulator (if 2op)
        enable_operator((Bitu)modbase + 3, &op[opbase + 9],
        ...
        if ((adlibreg[0x105] & 1) && op[opbase].is_4op) {
          ...
          enable_operator((Bitu)modbase + 3 + 8, &op[opbase + 3 + 9],
                          OP_ACT_NORMAL);
        }
#endif
      } else {
...
#if defined(OPLTYPE_IS_OPL3)
      Bits wselbase =
          (Bits)(second_set ? (base + 22)
                            : base); // for easier mapping onto wave_sel[]
      // change waveform
      if (adlibreg[0x105] & 1)
        wave_sel[wselbase] =
            val & 7; // opl3 mode enabled, all waveforms accessible
      else
...
void adlib_getsample(int16_t *sndptr, Bits numsamples) {
  ...
  for (Bits cursmp = 0; cursmp < samples_to_process; cursmp += endsamples) {
    ...
    int32_t vib_tshift = ((adlibreg[ARC_PERC_MODE] & 0x40) == 0)
                             ? 1
    ...
    for (i = 0; i < endsamples; i++) {
      ...
      vib_lut[i] = vib_table[vibtab_pos / FIXEDPT_LFO] >>
    ...
    if (adlibreg[ARC_PERC_MODE] & 0x20) {
      ...
      if (adlibreg[ARC_FEEDBACK + 6] & 1) {
        ...
        if (cptr[9].op_state != OF_TYPE_OFF) {
          if (cptr[9].vibrato) {
            vibval1 = vibval_var1;
            for (i = 0; i < endsamples; i++)
              vibval1[i] = (int32_t)((vib_lut[i] * cptr[9].freq_high / 8) *
                                     FIXEDPT * VIBFAC);
          } else
        ...
        if ((cptr[9].op_state != OF_TYPE_OFF) ||
          ...
          if ((cptr[0].vibrato) && (cptr[0].op_state != OF_TYPE_OFF)) {
            vibval1 = vibval_var1;
            for (i = 0; i < endsamples; i++)
              vibval1[i] = (int32_t)((vib_lut[i] * cptr[0].freq_high / 8) *
                                     FIXEDPT * VIBFAC);
          } else
            vibval1 = vibval_const;
          if ((cptr[9].vibrato) && (cptr[9].op_state != OF_TYPE_OFF)) {
            vibval2 = vibval_var2;
            for (i = 0; i < endsamples; i++)
              vibval2[i] = (int32_t)((vib_lut[i] * cptr[9].freq_high / 8) *
                                     FIXEDPT * VIBFAC);
          } else
          ...
          for (i = 0; i < endsamples; i++) {
            ...
            operator_output(
                &cptr[0], (cptr[0].lastcval + cptr[0].cval) * cptr[0].mfbi / 2,
                tremval1[i]);
      ...
      if (op[8].op_state != OF_TYPE_OFF) {
        ...
        if (cptr[0].vibrato) {
          vibval3 = vibval_var1;
          for (i = 0; i < endsamples; i++)
            vibval3[i] = (int32_t)((vib_lut[i] * cptr[0].freq_high / 8) *
                                   FIXEDPT * VIBFAC);
        } else
      ...
      if ((op[7].op_state != OF_TYPE_OFF) || (op[16].op_state != OF_TYPE_OFF) ||
        ...
        if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
          vibval4 = vibval_var2;
          for (i = 0; i < endsamples; i++)
            vibval4[i] = (int32_t)((vib_lut[i] * cptr[9].freq_high / 8) *
                                   FIXEDPT * VIBFAC);
        } else
        ...
        // calculate channel output
        for (i = 0; i < endsamples; i++) {
          operator_advance_drums(&op[7], vibval1[i], &op[7 + 9], vibval2[i],
                                 &op[8 + 9], vibval4[i]);
          ...
          if (adlibreg[ARC_FEEDBACK + k + 3] & 1) {
            ...
            if ((cptr[9].op_state != OF_TYPE_OFF) ||
              ...
              for (i = 0; i < endsamples; i++) {
                ...
                operator_advance(&cptr[3 + 9], 0);
                opfuncs[cptr[3 + 9].op_state](&cptr[3 + 9]);
                operator_output(&cptr[3 + 9], cptr[3].cval * FIXEDPT,
                                tremval3[i]);
