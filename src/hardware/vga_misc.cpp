...
/* allow the user to specify that undefined bits in 3DA/3BA be set to some
 * nonzero value. this is needed for "JOOP #2" by Europe demo, which has some
 * weird retrace tracking code like this:
 *
 *        in    al,dx       ; <- dx = 3DAh
 * l1:    shr   al,1        ; AL >>= 1, CF = bit shifted out
 *        jnc   l1
 *
 * of course, if AL == 0, it becomes an infinite loop. this is why this option
 * exists. */
...
Bitu vga_read_p3da(Bitu port, Bitu iolen) {
  ...
  // If the game or demo is wasting time in a loop polling this register (not
  // merely reading to clear the port 3C0h flip/flop) then now is as good a time
  // as any to render the VGA raster up to the current point.
  if (vga_render_on_demand && !vga.attr.disabled /*screen not disabled*/ &&
      !vga.internal.attrindex /*attribute controller flipflop clear*/)
...
static void write_p3c2(Bitu port, Bitu val, Bitu iolen) {
  (void)port;  // UNUSED
  (void)iolen; // UNUSED
  if ((machine == MCH_EGA) && ((vga.misc_output ^ val) & 0xc))
    VGA_StartResize();
  ...
  /*
          0	If set Color Emulation. Base Address=3Dxh else Mono Emulation.
     Base Address=3Bxh. 2-3	Clock Select. 0: 25MHz, 1: 28MHz 5	When in
     Odd/Even modes Select High 64k bank if set 6	Horizontal Sync
     Polarity. Negative if set 7	Vertical Sync Polarity. Negative if set
...
static Bitu read_p3c2(Bitu port, Bitu iolen) {
  ...
  if (IS_EGAVGA_ARCH) {
    uint8_t setting =
        ~((!IS_VGA_ARCH && ega200) ? 0x08 : 0x09) /*bits are inverted*/;
  ...
  /*
          ...
          7	If set IRQ 2 has happened due to Vertical Retrace.
                  Should be cleared by IRQ 2 interrupt routine by clearing port
...
void VGA_SetupMisc(void) {
  ...
  } else if (machine == MCH_CGA || machine == MCH_MCGA ||
             machine == MCH_AMSTRAD || IS_TANDY_ARCH) {
...
void pc98_clear_graphics(void) {
  for (unsigned int i = 0; i < 0x8000; i += 2) {
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x00000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x08000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x10000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x18000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x20000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x28000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x30000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x38000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x40000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x48000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x50000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x58000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x60000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x68000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x70000)) =
        0;
    *((uint16_t *)(vga.mem.linear + i + PC98_VRAM_GRAPHICS_OFFSET + 0x78000)) =
        0;
  }
}
