...
enum GUSType { GUS_CLASSIC = 0, GUS_MAX, GUS_INTERWAVE };
...
static uint16_t const pantablePDF[16] = {
    0, 13, 26, 41, 57, 72, 94, 116, 141, 169, 203, 244, 297, 372, 500, 4095};
...
static uint8_t const dmatable[8] = {0 /*NO DMA*/,  1, 3, 5, 6, 7,
                                    0 /*invalid*/, 0 /*invalid*/};
...
struct GFGus {
  uint8_t gRegSelectData; // what is read back from 3X3. not necessarily the
                          // index selected, but
  ...
  uint16_t
      DMAControl; /* NTS: bit 8 for DMA TC IRQ status. Only bits [7:0] exist on
                     real hardware. We're taking the DOSBox SVN approach here
  ...
  INLINE int32_t LoadSample8(
      const uint32_t addr /*memory address without fractional bits*/) const {
    return (int8_t)GUSRam[VoiceAddr8(addr)]
           << int32_t(8); /* typecast to sign extend 8-bit value */
  }

  INLINE int32_t LoadSample16(
      const uint32_t addr /*memory address without fractional bits*/) const {
    return (int16_t)host_readw(
        GUSRam + VoiceAddr16(addr)); /* typecast to sign extend 16-bit value */
  }

  INLINE int32_t InterpolateSample(const int32_t w1, const int32_t w2,
                                   const uint32_t scale) const {
  ...
  INLINE int32_t GetSample8(const uint32_t WaveAddr) const {
    ...
    return InterpolateSample(LoadSample8(useAddr), LoadSample8(useAddr + 1u),
                             WaveAddr & WAVE_FRACT_MASK);
  }

  INLINE int32_t GetSample16(const uint32_t WaveAddr) const {
    const uint32_t useAddr = WaveAddr >> WAVE_FRACT;
    return InterpolateSample(LoadSample16(useAddr), LoadSample16(useAddr + 1u),
                             WaveAddr & WAVE_FRACT_MASK);
  }
};
...
void GFGus::updateMasterVolume(void) {
  ...
  if (vol > 6)
    vol = 6; // allow some amplification but don't let it overflow
  masterVolumeMul = (int32_t)((1 << 9) * pow(10.0, vol / 20.0));
  if (AutoAmp > masterVolumeMul)
    AutoAmp = masterVolumeMul;
}
...
public:
  ...
  INLINE int32_t GetSample8() const { return myGUS.GetSample8(WaveAddr); }

  INLINE int32_t GetSample16() const { return myGUS.GetSample16(WaveAddr); }

  void WriteWaveFreq(uint16_t val) {
    WaveFreq = val;
    if (myGUS.fixed_sample_rate_output) {
      double frameadd = double(val >> 1) / 512.0; // Samples / original gus
                                                  // frame
  ...
  void WriteRampRate(uint8_t val) {
    ...
    if (myGUS.fixed_sample_rate_output) {
      double frameadd =
          (double)(RampRate & 63) / (double)(1 << (3 * (val >> 6)));
      double realadd = (frameadd * (double)myGUS.basefreq / (double)GUS_RATE) *
                       (double)(1 << RAMP_FRACT);
      RampAdd = (uint32_t)realadd;
    } else {
      /* NTS: Note RAMP_FRACT == 10, shift = 10 - (3*(val>>6)).
       * From the upper two bits, the possible shift values for 0, 1, 2, 3 are:
       * 10, 7, 4, 1 */
      RampAdd = ((uint32_t)(RampRate & 63))
                << ((uint32_t)(RAMP_FRACT - (3 * (val >> 6))));
  ...
  INLINE void WaveUpdate(void) {
    ...
    if ((WaveCtrl & (WCTRL_STOP | WCTRL_STOPPED)) == 0 /*voice is running*/) {
       ...
       *      If WaveAddr <= WaveAdd going backwards, WaveAddr becomes negative,
       * which as an unsigned integer, means carrying down from the highest
       * possible value of the integer type. Which means that if the start
       * position is less than WaveAdd the WaveAddr will skip over the start
       * pointer and continue playing downward from the top of the GUS memory,
       * without stopping/looping as expected.
       *
       *      This "bug" was implemented on purpose because real Gravis
       * Ultrasound hardware acts this way. */
      uint32_t WaveExtra = 0;
      if (WaveCtrl & WCTRL_DECREASING /*backwards (direction)*/) {
        /* unsigned int subtract, mask, compare. will miss start pointer if
         * WaveStart <= WaveAdd. This bug is deliberate, accurate to real GUS
         * hardware, do not fix. */
        WaveAddr -= WaveAdd;
        WaveAddr &= ((Bitu)1 << ((Bitu)WAVE_FRACT + (Bitu)20 /*1MB*/)) - 1;
        endcondition = (WaveAddr < WaveStart) ? true : false;
        if (endcondition)
          WaveExtra = WaveStart - WaveAddr;
      } else {
        WaveAddr += WaveAdd;
        endcondition = (WaveAddr > WaveEnd) ? true : false;
        WaveAddr &= ((Bitu)1 << ((Bitu)WAVE_FRACT + (Bitu)20 /*1MB*/)) - 1;
        if (endcondition)
          WaveExtra = WaveAddr - WaveEnd;
      }
      ...
      if (endcondition) {
        ...
        if ((RampCtrl & WCTRL_16BIT /*roll over*/) &&
            !(WaveCtrl & WCTRL_LOOP)) {
          /* "3.11. Rollover feature
           *
           * Each voice has a 'rollover' feature that allows an application to
           * be notified when a voice's playback position passes over a
           * particular place in DRAM.  This is very useful for getting seamless
           * digital audio playback.  Basically, the GF1 will generate an IRQ
           * when a voice's current position is  equal to the end position.
           * However, instead of stopping or looping back to the start position,
           * the voice will continue playing in the same direction.  This means
           * that there will be no pause (or gap) in the playback.  Note that
           * this feature is enabled/disabled through the voice's VOLUME control
           * register (since there are no more bits available in the voice
           * control registers).   A voice's loop enable bit takes precedence
           * over the rollover.  This means that if a voice's loop enable is on,
           * it will loop when it hits the end position, regardless of the state
           * of the rollover enable."
           *
           * Despite the confusing description above, that means that looping
           * takes precedence over rollover. If not looping, then rollover means
           * to fire the IRQ but keep moving. If looping, then fire IRQ and
           * carry out loop behavior. Gravis Ultrasound Windows 3.1 drivers
           * expect this behavior, else Windows WAVE output will not work
           * correctly. */
        } else {
          if (WaveCtrl & WCTRL_LOOP) {
            if (WaveCtrl & WCTRL_BIDIRECTIONAL)
              WaveCtrl ^= WCTRL_DECREASING /*change direction*/;
            WaveAddr = (WaveCtrl & WCTRL_DECREASING) ? (WaveEnd - WaveExtra)
                                                     : (WaveStart + WaveExtra);
          } else {
      ...
      /* Undocumented behavior observed on real GUS hardware: A stopped voice
       * will still rapid-fire IRQs if IRQ enabled and current position <= start
       * position OR current position >= end position */
      if (WaveCtrl & WCTRL_DECREASING /*backwards (direction)*/)
        endcondition = (WaveAddr <= WaveStart) ? true : false;
      else
        endcondition = (WaveAddr >= WaveEnd) ? true : false;
  ...
  INLINE void UpdateVolumes(void) {
    ...
    templeft &=
        ~(templeft >>
          31); /* <- NTS: This is a rather elaborate way to clamp negative
                  values to zero using negate and sign extend */
  ...
  INLINE void RampUpdate(void) {
    if (RampCtrl & 0x3)
      return; /* if the ramping is turned off, then don't change the ramp */

    int32_t RampLeft;
    if (RampCtrl & 0x40) {
      RampVol -= RampAdd;
      if ((int32_t)RampVol < (int32_t)0)
        RampVol = 0;
      RampLeft = (int32_t)RampStart - (int32_t)RampVol;
    } else {
      RampVol += RampAdd;
      if (RampVol > ((4096 << RAMP_FRACT) - 1))
        RampVol = ((4096 << RAMP_FRACT) - 1);
    ...
    if (RampCtrl & 0x08) {
      ...
      if (RampCtrl & 0x10)
        RampCtrl ^= 0x40;
      RampVol = (RampCtrl & 0x40)
                    ? (uint32_t)((int32_t)RampEnd - (int32_t)RampLeft)
                    : (uint32_t)((int32_t)RampStart + (int32_t)RampLeft);
    } else {
  ...
  void generateSamples(int32_t *stream, uint32_t len) {
    ...
    /* NTS: The GUS is *always* rendering the audio sample at the current
     * position, even if the voice is stopped. This can be confirmed using
     * DOSLIB, loading the Ultrasound test program, loading a WAV file into
     * memory, then using the Ultrasound test program's voice control dialog to
     * single-step the voice through RAM (abruptly change the current position)
     * while the voice is stopped. You will hear "popping" noises come out the
     * GUS audio output as the current position changes and the piece of the
     * sample rendered abruptly changes as well. */
    if (gus_ics_mixer) {
...
void DEBUG_PrintGUS() { // debugger "GUS" command
  LOG_MSG("GUS regsel=%02x regseld=%02x regdata=%02x DRAMaddr=%06x/%06x "
          "VoiceMask=%06x memsz=%06x curch=%02x MAXctrl=%02x regctl=%02x",
          myGUS.gRegSelect, myGUS.gRegSelectData, myGUS.gRegData,
          myGUS.gDramAddr, myGUS.gDramAddrMask, myGUS.gDramVoiceMask,
          myGUS.memsize, myGUS.gCurChannel, myGUS.gUltraMAXControl,
          myGUS.gRegControl);
  LOG_MSG("DMActrl=%02x (TC=%u) dmaAddr=%04x%01x timerctl=%02x sampctl=%02x "
          "mixctl=%02x activech=%u (want=%u) DACrate=%uHz",
          myGUS.DMAControl & 0xFF, (myGUS.DMAControl & 0x100) ? 1 : 0,
          myGUS.dmaAddr, myGUS.dmaAddrOffset, myGUS.TimerControl,
          myGUS.SampControl, myGUS.mixControl, myGUS.ActiveChannels,
          myGUS.ActiveChannelsUser, myGUS.basefreq);
  LOG_MSG("IRQen=%u IRQstat=%02x IRQchan=%04x RampIRQ=%04x WaveIRQ=%04x",
          myGUS.irqenabled, myGUS.IRQStatus, myGUS.IRQChan, myGUS.RampIRQ,
          myGUS.WaveIRQ);
  for (size_t t = 0; t < 2; t++) {
    LOG_MSG("Timer %u: delay=%.3fms value=%02x reached=%u raiseirq=%u "
            "masked=%u running=%u\n",
            (unsigned int)t + 1u, myGUS.timers[t].delay, myGUS.timers[t].value,
            myGUS.timers[t].reached, myGUS.timers[t].raiseirq,
            myGUS.timers[t].masked, myGUS.timers[t].running);
  }
  for (size_t t = 0; t < (size_t)myGUS.ActiveChannels; t++) {
    ...
    if (ch->WaveCtrl & WCTRL_LOOP)
      line += " LOOP";
    else if (ch->RampCtrl & WCTRL_16BIT /*roll over*/) /* !loop and (rampctl &
                                                          4) == rollover */
      line += " ROLLOVER";
    ...
    LOG_MSG("Voice %u: start=%05x.%03x end=%05x.%03x addr=%05x.%03x "
            "add=%05x.%03x ctl=%02x rampctl=%02x%s",
            (unsigned int)t + 1u, ch->WaveStart >> WAVE_FRACT,
            (ch->WaveStart & WAVE_FRACT_MASK)
                << (12 - WAVE_FRACT), // current WAVE_FRACT == 9
            ch->WaveEnd >> WAVE_FRACT,
            (ch->WaveEnd & WAVE_FRACT_MASK)
                << (12 - WAVE_FRACT), // current WAVE_FRACT == 9
            ch->WaveAddr >> WAVE_FRACT,
            (ch->WaveAddr & WAVE_FRACT_MASK)
                << (12 - WAVE_FRACT), // current WAVE_FRACT == 9
            ch->WaveAdd >> WAVE_FRACT,
            (ch->WaveAdd & WAVE_FRACT_MASK)
                << (12 - WAVE_FRACT), // current WAVE_FRACT == 9
            ch->WaveCtrl, ch->RampCtrl, line.c_str());
    LOG_MSG("    Ramp start=%05x.%03x end=%05x.%03x vol=%05x.%03x "
            "add=%05x.%03x pan=%x",
            ch->RampStart >> RAMP_FRACT,
            (ch->RampStart & RAMP_FRACT_MASK)
                << (12 - RAMP_FRACT), // current RAMP_FRACT == 10
            ch->RampEnd >> RAMP_FRACT,
            (ch->RampEnd & RAMP_FRACT_MASK)
                << (12 - RAMP_FRACT), // current RAMP_FRACT == 10
            ch->RampVol >> RAMP_FRACT,
            (ch->RampVol & RAMP_FRACT_MASK)
                << (12 - RAMP_FRACT), // current RAMP_FRACT == 10
            ch->RampAdd >> RAMP_FRACT,
            (ch->RampAdd & RAMP_FRACT_MASK)
                << (12 - RAMP_FRACT), // current RAMP_FRACT == 10
            ch->PanPot);
  }
}
...
static void GUSReset(void) {
   ...
   *
   *      Global Data Low (3X4) is either a 16-bit transfer, or the low half of
   * a 16-bit transfer with 8-bit I/O.
   *
   *      Global Data High (3X5) is either an 8-bit transfer for one of the GF
   * registers or the high part of a 16-bit wide register with 8-bit I/O.
   *
   *      Prior to 2015/12/29 DOSBox and DOSBox-X contained a programming error
   * here where reset and master IRQ enable were handled from the LOWER 8 bits,
   * when the code should have been checking the UPPER 8 bits. Programming error
   * #2 was the mis-interpretation of bit 0 (bit 8 of the gRegData). According
   * to the SDK, clearing bit 0 triggers RESET, setting bit 0 starts the card
   * running again. The original code had it backwards. */
  ...
  if ((myGUS.gRegData & 0x100) == 0x000) {
    ...
    myGUS.basefreq = (uint32_t)((float)1000000 /
                                (1.619695497 * (float)(myGUS.ActiveChannels)));

    gus_chan->FillUp();
    if (!myGUS.fixed_sample_rate_output)
      gus_chan->SetFreq(myGUS.basefreq);
    else
      gus_chan->SetFreq(GUS_RATE);
  ...
  /* if the card was just put into reset, or the card WAS in reset, bits 1-2 are
   * cleared */
  if ((myGUS.GUS_reset_reg & 1) == 0 || (p_GUS_reset_reg & 1) == 0) {
    /* GUS classic observed behavior: resetting the card, or even coming out of
     * reset, clears bits 1-2. That means, if you write any value to GUS RESET
     * with bit 0 == 0, bits 1-2 become zero as well. And if you take the card
     * out of reset, bits 1-2 are zeroed.
     *
     * test 1:
     ...
     * outb(0x3X3,0x4C); c = inb(0x3X5);      <- you'll get 0x01 as expected,
     * card taken out of reset outb(0x3X3,0x4C); outb(0x3X5,0x07);
     * outb(0x3X3,0x4C); c = inb(0x3X5);      <- you'll get 0x07 as expected
     * outb(0x3X3,0x4C); outb(0x3X5,0x06);    <- bit 0 == 0, we're trying to set
     * bits 1-2 outb(0x3X3,0x4C); c = inb(0x3X5);      <- you'll get 0x00, not
     * 0x06, card is in reset state */
    myGUS.irqenabled =
        myGUS.force_master_irq_enable; // IRQ enable resets, unless user
                                       // specified we force it on
...
static uint8_t GUS_EffectiveIRQStatus(void) {
  ...
  /* Behavior observed on real GUS hardware: Master IRQ enable bit 2 of the
   * reset register affects only voice/wave IRQ signals from the GF1. It does
   * not affect the DMA terminal count interrupt nor does it affect the Adlib
   * timers. This is how "Juice" by Psychic Link is able to play music by GUS
   * timer even though the demo never enables the Master IRQ Enable bit. */
  ...
  /* Timer 1 & 2 */
  ret |= (myGUS.IRQStatus /*Timer 1&2 IRQ*/ &
          myGUS.TimerControl /*Timer 1&2 IRQ Enable*/ & 0x0C);
      ...
      if (gus_prev_effective_irqstat == 0) {
        ...
        if (gus_warn_irq_conflict)
          LOG(LOG_MISC, LOG_WARN)(
              "GUS warning: Both IRQs set to the same signal line WITHOUT "
              "combining! "
...
static uint16_t ExecuteReadRegister(void) {
  ...
  if (myGUS.globalread80alias && effective > 0x8F)
    effective &= 0x7F;
  ...
  case 0x8E: // read active channel register
    // NTS: The GUS SDK documents the active channel count as bits 5-0, which is
    // wrong. it's bits 4-0. bits 7-5 are always 1 on real hardware.
  ...
  case 0x41: // Dma control register - read acknowledges DMA IRQ
    if (dma_enable_on_dma_control_polling) {
      if (!GetDMAChannel(myGUS.dma1)->masked && !(myGUS.DMAControl & 0x01) &&
          !(myGUS.IRQStatus & 0x80)) {
        LOG(LOG_MISC,
            LOG_DEBUG)("GUS: As instructed, switching on DMA ENABLE upon "
                       "polling DMA control register (HACK) as workaround");
    ...
    tmpreg |= (myGUS.DMAControl & 0x100) >>
              2; /* Bit 6 on read is the DMA terminal count IRQ status */
  ...
  case 0x4c: // GUS reset register
    ...
    /* GUS Classic observed behavior: You can read Register 4Ch from both 3X4
     * and 3X5 and get the same 8-bit contents */
...
static void ExecuteGlobRegister(void) {
  int i;
  //	if (myGUS.gRegSelect|1!=0x44) LOG_MSG("write global register %x with
  //%x", myGUS.gRegSelect, myGUS.gRegData);
  switch (myGUS.gRegSelect) {
  case 0x0: // Channel voice control register
    gus_chan->FillUp();
    if (curchan)
      curchan->WriteWaveCtrl((uint16_t)myGUS.gRegData >> 8);
    break;
  ...
  case 0x2: // Channel MSW start address register
    if (curchan) {
      uint32_t tmpaddr = (uint32_t)(myGUS.gRegData & 0x1fff)
                         << 16; /* upper 13 bits of integer portion */
  ...
  case 0x3: // Channel LSW start address register
    if (curchan != NULL) {
      uint32_t tmpaddr =
          (uint32_t)(myGUS.gRegData &
                     0xffe0); /* lower 7 bits of integer portion, and all 4 bits
                                 of fractional portion. bits 4-0 of the incoming
                                 16-bit WORD are not used */
  ...
  case 0xC: // Channel pan pot register
    gus_chan->FillUp();
    if (curchan)
      curchan->WritePanPot((uint16_t)myGUS.gRegData >> 8);
    break;
  case 0xD: // Channel volume control register
    gus_chan->FillUp();
    if (curchan)
      curchan->WriteRampCtrl((uint16_t)myGUS.gRegData >> 8);
    break;
  case 0xE: // Set active channel register
    /* Hack for "Ice Fever" demoscene production:
     * If the DAC is active (bit 1 of GUS reset is set), ignore writes to this
     * register. The demo resets the GUS with 14 channels, then after reset
     * changes it to 16 for some reason. Without this hack, music will sound
     * slowed down and wrong. As far as I know, real hardware will accept the
     * change immediately and produce the same slowed down sound music. --J.C.
     */
    if (ignore_active_channel_write_while_active) {
      if (myGUS.GUS_reset_reg & 0x02 /*DAC enable*/) {
        LOG_MSG("GUS: Attempt to change active channel count while DAC active "
                "rejected");
        break;
      }
    }
    ...
    gus_chan->FillUp();
    myGUS.gRegSelect =
        myGUS.gRegData >> 8; // JAZZ Jackrabbit seems to assume this?
    myGUS.ActiveChannelsUser =
        1 + ((myGUS.gRegData >> 8) &
             31); // NTS: The GUS SDK documents this field as bits 5-0, which is
                  // wrong, it's bits 4-0. 5-0 would imply 64 channels.

    /* The GUS SDK claims that if a channel count less than 14 is written, then
     * it caps to 14. That's not true. Perhaps what the SDK is doing, but the
     * actual hardware acts differently. This implementation is based on what
     * the Gravis Ultrasound MAX actually does with this register. You can
     * apparently achieve higher than 44.1KHz sample rates by programming less
     * than 14 channels, and the sample rate scale ramps up appropriately,
     * except that values 0 and 1 have the same effect as writing 2 and 3. Very
     * useful undocumented behavior! If Gravis were smart, they would have been
     * able to claim 48KHz sample rates by allowing less than 14 channels in
     * their SDK! Not sure why they would cap it like that, unless there are
     * undocumented chipset instabilities with running at higher rates.
     *
     * So far only verified on a Gravis Ultrasound MAX.
     *
     * Does anyone out there have a Gravis Ultrasound Classic (original 1992
     * version) they can test for this behavior?
     *
     * NOTED: Gravis Ultrasound Plug & Play (interwave) cards *do* enforce the
     * 14-channel minimum. You can write less than 14 channels to this register,
     * but unlike the Classic and Max cards they will not run faster
     * than 44.1KHz. */
    myGUS.ActiveChannels = myGUS.ActiveChannelsUser;

    if (gus_type < GUS_INTERWAVE) {
      // GUS MAX behavior seen on real hardware
      if (myGUS.ActiveChannels < 3)
        myGUS.ActiveChannels += 2;
    ...
    myGUS.basefreq =
        (uint32_t)(0.5 +
                   1000000.0 / (1.619695497 * (double)(myGUS.ActiveChannels)));
...
#if LOG_GUS
    LOG_MSG("GUS set to %d channels fixed=%u freq=%luHz", myGUS.ActiveChannels,
            myGUS.fixed_sample_rate_output, (unsigned long)myGUS.basefreq);
#endif
  ...
  case 0x41: // Dma control register
    myGUS.DMAControl &=
        ~0xFFu; // FIXME: Does writing DMA Control clear the DMA TC IRQ?
    myGUS.DMAControl |= (uint8_t)(myGUS.gRegData >> 8);
    GUS_Update_DMA_Event_transfer();
    if (myGUS.DMAControl & 1)
      GUS_StartDMA();
    else
      GUS_StopDMA();
    break;
  ...
  case 0x44: // MSW Peek/poke DRAM position
    myGUS.gDramAddr =
        (0xffff & myGUS.gDramAddr) | ((uint32_t)myGUS.gRegData >> 8) << 16;
    break;
  case 0x45: // Timer control register.  Identical in operation to Adlib's timer
  ...
  default:
#if LOG_GUS
    LOG_MSG("Unimplemented global register %x -- %x", myGUS.gRegSelect,
            myGUS.gRegData);
#endif
    break;
  }
...
/* NTS: This was written and tested only through Ultrasound software and
 * emulation. I do not have a Gravis Ultrasound card with this type of mixer to
 * test against. --J.C. */
struct gus_ICS2101 {
public:
  // ICS2101 and how Gravis wired up the input pairs when using it, according to
  // some BSD and Linux kernel sources
  //
...
public:
  ...
  void dataWrite(uint8_t val) {
    LOG(LOG_MISC, LOG_DEBUG)("GUS ICS-2101 Mixer Data Write val=%02xh to "
                             "attensel=%u(%s) ctrlsel=%u(%s)",
                             (int)val, addr_attenuator,
                             attenuatorName(addr_attenuator), addr_control,
                             controlName(addr_control));

    if (addr_control &
        2) { // attenuator NTS: Only because an existing ICS patch for DOSBox
             // does it this way... does hardware do it this way?
      ...
      // FIXME: Does the panning take effect immediately, or does the chip
      // require the DOS program
  ...
  const char *controlName(const uint8_t c) const {
    switch (c) {
    case 0:
      return "Control Left"; // 000
    case 1:
      return "Control Right"; // 001
    case 2:
      return "Attenuator Left"; // 010
    case 3:
      return "Attenuator Right"; // 011
    case 4:
    case 5: // 10x
      return "Pan/Balance";
    }
  ...
  void updateVolPair(unsigned int pair) {
    if (pair >= MASTER_OUTPUT_PORT) {
      // master volume changes everyone else. do it, through 1 level of
      // recursion.
      ...
      for (unsigned int ch = 0; ch < 2; ch++) {
        volpair[pair].AttenDb[ch] =
            mixpair[pair].AttenDb[ch] + mixpair[MASTER_OUTPUT_PORT].AttenDb[ch];
...
public:
  ...
  public:
    mixcontrol() {
      ...
      MapControl[1] = 0x02;
      setAttenuation(0,
                     0x7F); // FIXME: Because we want DOSBox to come up as if
                            // ULTRINIT/ULTRAMIX were run to configure the mixer
  ...
  public:
      ...
      if (val < 16)
        for (int i = 0; i < (16 - val); i++)
          gain +=
              ...
              .13603 *
                  (i +
                   1); // increasing rate of change, based on datasheet graph
      return gain;
    }
    // end borrow
    ...
    void updateMapControl() {
      ...
      case 4: // stereo (normal or reversed)
        MapControl[0] = (Control[0] & 1)
                            ? 1
                            : 2; // left -> left or left -> right if swapped
        MapControl[1] = (Control[0] & 1)
                            ? 2
                            : 1; // right -> right or right -> left if swapped
        break;
      case 6:              // Mono
      ...
      case 12: // Balance
        ...
        MapControl[1] = (Control[0] & 1)
                            ? 2
                            : 1; // right -> right or right -> left if swapped
        // fixme: Do we update attenuation to reflect panning? or does the ICS
        // chip need the DOS program to write the attenuation registers again
        // for the chip to apply balance/panning?
        break;
      case 10:
      case 14: // Pan
        MapControl[0] = (Control[0] & 1)
    ...
    void setAttenuation(const unsigned int channel, const uint8_t val) {
      // FIXME: I am only able to test the "normal" mode since that's the only
      // mode used by Gravis's DOS and Windows drivers.
      //        The code below has not been tested in "Stereo" and "Pan/Balance"
      //        mode. If any DOS drivers use it, please direct me to them so I
      //        or anyone else can test! ---J.C.

      if ((Control[0] & 0xC) == 0) {
        Attenuation[channel] = val & 0x7F;
        AttenDb[channel] = gain(Attenuation[channel]);
      } else {
        // "stereo & balance use left control & gain, copies to right regs" says
        // the Vogons patch I'm studying...
        Attenuation[0] = Attenuation[1] = val & 0x7F;
        AttenDb[0] = gain(Attenuation[0]);
        AttenDb[1] = AttenDb[0];

        // taken from the same Vogons patch, except potential access past array
        // fixed
        if ((Control[0] & 0xC) == 8 /*Balance/pan mode*/) {
          static const float pan[16 + 1] = {-9,   -9, -8.5, -6.5, -5.5, -4.5,
                                            -3.5, -3, -2.5, -2,   -1.5, -1,
                                            -.5,  0,  0,    0,    0};
    ...
    void debugPrintMixer(const char *name) {
      LOG(LOG_MISC, LOG_DEBUG)("GUS ICS control '%s': %.3fdB %.3fdB", name,
                               AttenDb[0], AttenDb[1]);
    }
  ...
  public:
    ...
    uint8_t Attenuation[2]; // 0x00-0x7F where 0x00 is mute (max atten) and 0x7F
                            // is full volume
    float AttenDb[2];       // in decibels
  };
  struct volpair {
  public:
    volpair() { AttenDb[0] = AttenDb[1] = 0; }
...
/* NOTES:
 *
 *    This 4-port I/O interface, implemented by Crystal Semiconductors, Analog
 * Devices, etc. and used by sound cards in the early 1990s, is the said to be
 * the "standardized" hardware interface of the "Windows Sound System" standard
 * at the time.
 *
 *    According to an AD1848 datasheet, and a CS4231A datasheet, all I/O ports
 * and indirect registers appear to be the same, with the exception that Crystal
 * Semiconductor adds 16 registers with the "MODE 2" bit.
 *
 *    Perhaps at some point, we can untie this from GUS emulation and let it
 * exist as its own C++ class that covers CS4231A, AD1848, and other "WSS"
 * chipset emulation on behalf of GUS and SB emulation, much like the OPL3
 * emulation already present in this source tree for Sound Blaster.
 *
 */
struct gus_cs4231 {
public:
  gus_cs4231()
      : address(0), mode2(false), ia4(false), trd(false), mce(true),
        init(false) {}

public:
  void data_write(uint8_t addr, uint8_t val) {
    //		LOG(LOG_MISC,LOG_DEBUG)("GUS CS4231 write data addr=%02xh
    //val=%02xh",addr,val);

    switch (addr) {
    case 0x00: /* Left ADC Input Control (I0) */
      ADCInputControl[0] = val;
      break;
    case 0x01: /* Right ADC Input Control (I1) */
      ADCInputControl[1] = val;
      break;
    case 0x02: /* Left Auxiliary #1 Input Control (I2) */
      Aux1InputControl[0] = val;
      break;
    case 0x03: /* Right Auxiliary #1 Input Control (I3) */
      Aux1InputControl[1] = val;
      break;
    case 0x06: /* Left DAC Output Control (I6) */
      DACOutputControl[0] = val;
      break;
    case 0x07: /* Left DAC Output Control (I7) */
      DACOutputControl[1] = val;
      break;
    ...
    default:
      LOG(LOG_MISC, LOG_DEBUG)(
          "GUS CS4231 unhandled data write addr=%02xh val=%02xh", addr, val);
      break;
    }
  }
  uint8_t data_read(uint8_t addr) {
    ...
    default:
      LOG(LOG_MISC, LOG_DEBUG)("GUS CS4231 unhandled data read addr=%02xh",
                               addr);
      break;
    }
  ...
  void iowrite(uint8_t reg, uint8_t val) {
    //		LOG(LOG_MISC,LOG_DEBUG)("GUS CS4231 write reg=%u
    //val=%02xh",reg,val);
    ...
    case 0x2: /* Status Register (R2) */
      LOG(LOG_MISC, LOG_DEBUG)(
          "GUS CS4231 attempted write to status register val=%02xh", val);
      break;
  ...
  uint8_t ioread(uint8_t reg) {
    ...
    switch (reg) {
    case 0x0: /* Index Address Register (R0) */
      return address | (trd ? 0x20 : 0x00) | (mce ? 0x40 : 0x00) |
             (init ? 0x80 : 0x00);
    case 0x1: /* Index Data Register (R1) */
...
public:
  ...
  uint8_t ADCInputControl[2] =
      {}; /* left (I0) and right (I1) ADC Input control. bits 7-6 select source.
             bit 5 is mic gain. bits 3-0 controls gain. */
  uint8_t Aux1InputControl[2] =
      {}; /* left (I2) and right (I3) aux. input control. bits 5-0 control gain
             in 1.5dB steps. bit 7 is mute */
  uint8_t DACOutputControl[2] =
      {}; /* left (I6) and right (I7) output control attenuation. bits 5-0
             control in -1.5dB steps, bit 7 is mute */
} GUS_CS4231;

static Bitu read_gus_cs4231(Bitu port, Bitu iolen) {
  (void)iolen; // UNUSED
  if (myGUS.gUltraMAXControl & 0x40 /*codec enable*/)
    return GUS_CS4231.ioread(
        (port - GUS_BASE) & 3); // FIXME: UltraMAX allows this to be relocatable
...
static Bitu read_gus(Bitu port, Bitu iolen) {
   ...
   *
   * More than 10 bits must be decoded in order for GUS MAX extended registers
   * at 7xx to work */
  port &= 0xFFF;
  ...
  case 0x206:
    if (myGUS.clearTCIfPollingIRQStatus) {
       ...
       *
       * If you configure the game to use the Gravis Ultrasound for both music
       * and sound, the GUS support code for digital sound will cause the game
       * to hang if music is playing at the same time within the main menu. The
       * bug is that there is code (in real mode no less) that polls the IRQ
       * status register (2X6) and handles each IRQ event to clear it, however
       * there is no condition to handle clearing the DMA Terminal Count IRQ
       * flag. The routine will not terminate until all bits are cleared, hence,
       * the game hangs after starting a sound effect. Most often, this is
       * visible to the user as causing the game to hang when you click on a
       * button on the main menu.
       *
       * This hack attempts to detect the bug by looking for rapid polling of
       * this register in a short period of time. If detected, we clear the DMA
       * IRQ flag to help the loop terminate so the game continues to run. */
      if (t < (myGUS.lastIRQStatusPollAt + 0.1 /*ms*/)) {
        myGUS.lastIRQStatusPollAt = t;
        myGUS.lastIRQStatusPollRapidCount++;
        if (myGUS.clearTCIfPollingIRQStatus && (myGUS.IRQStatus & 0x80) &&
            myGUS.lastIRQStatusPollRapidCount >= 500) {
          LOG(LOG_MISC, LOG_DEBUG)("GUS: Clearing DMA TC IRQ status, DOS "
    ...
    /* NTS: Contrary to some false impressions, GUS hardware does not report
     * "one at a time", it really is a bitmask. I had the funny idea you read
     * this register "one at a time" just like reading the IRQ reason bits of
     * the RS-232 port --J.C. */
  ...
  case 0x304:
    if (iolen == 2)
      reg16 = ExecuteReadRegister() & 0xffff;
    else
      reg16 = ExecuteReadRegister() & 0xff;

    if (gus_type <
        GUS_INTERWAVE) // Versions prior to the Interwave will reflect last I/O
                       // to 3X2-3X5 when read back from 3X3
  ...
  case 0x307:
    if (warn_out_of_bounds_dram_access && myGUS.gDramAddr >= myGUS.memsize)
      LOG(LOG_MISC, LOG_WARN)("GUS: out of bounds DRAM read %x",
                              (unsigned int)myGUS.gDramAddr);
...
static void write_gus(Bitu port, Bitu val, Bitu iolen) {
  ...
  /* Ok, so get this: There's a demoscene entry, 1997 demo "Atlantis, Deep Like
   * A Sea", with code that does a 16-bit I/O write to port 3x2. Why? Well, if
   * you notice how this switch statement is done, and how DOSBox SVN registers
   * I/O ports, most of these I/O ports are meant for 8-bit I/O access. The only
   * exception to that is port 3x4 which allows writing 16 bits to a GF1
   * register. So what happens if you do a 16-bit write to port 3x2?
   *
   * If the demo's behavior is any indication, the 16-bit write is handled like
   * two 8-bit writes. The lower 8 bits go to port 3x2, the upper to 3x3. If you
   * look at the register map, that's one 16-bit I/O write that writes both
   * gCurChannel (current channel) and gRegSelect (current selected register) in
   * one I/O cycle.
   *
   * If we do not split the I/O up as described, then the demo makes no sound
   * except for popping noises because none of the voices are playing anything
   * or moving at all.
   *
   * This trick happens to work in DOSBox SVN (and produce audible music)
   * because DOSBox SVN GUS emulation installs I/O handlers only for bytewise
   * (IO_MB) I/O, which then forces I/O emulation to subdivide the 16-bit I/O
   * into two 8-bit I/O calls. The only exception is IO_MB|IO_MW for port 3x4
   * (gRegData and execute register).
   *
   * Demo link:
   * [https://files.scene.org/get/mirrors/hornet/demos/1997/a/atl-mnsn.zip] */
  if (iolen == 2) {
    if ((port - GUS_BASE) != 0x304) {
  ...
  case 0x208:
    ...
    break;
  case 0x209:
    // TODO adlib_commandreg should be 4 for this to work else it should just
    // latch the value
    if (val & 0x80) {
    ...
    if (val & 0x2) {
      ...
      myGUS.timers[1].running = false;
    break;
    // TODO Check if 0x20a register is also available on the gus like on the
    // interwave
  case 0x20b:
    ...
    if (myGUS.gRegControl == 6) {
       ...
       * 0: reserved */
      /* TODO: */
      LOG(LOG_MISC, LOG_DEBUG)("GUS: DOS application wrote to 2XB register "
                               "control number 0x06 (Jumper) val=%02xh",
                               (int)val);
    } else if (myGUS.gRegControl == 5) {
      /* write a 0 to clear IRQs on power up (?) */
      LOG(LOG_MISC, LOG_DEBUG)("GUS: DOS application wrote to 2XB register "
                               "control number 0x05 (Clear IRQs) val=%02xh",
                               (int)val);
      ...
      if (myGUS.mixControl & 0x40) {
        ...
        //     Reserved (bit 7)
        //
        //     "If both channels are sharing an IRQ, channel 2's IRQ must be set
        //     to 0 and turn on bit 6. A
        //      bus conflict will occur if both latches are programmed with the
        //      same IRQ #."
        ...
        LOG(LOG_MISC,
            LOG_DEBUG)("GUS IRQ reprogrammed: GF1 IRQ %d, MIDI IRQ %d",
                       (int)myGUS.irq1, (int)myGUS.irq2);

        gus_warn_irq_conflict =
            (!(val & 0x40) && (val & 7) == ((val >> 3) & 7));
      } else {
        ...
        //     Reserved (bit 7)
        //
        //     "If both channels are sharing an DMA, channel 2's DMA must be set
        //     to 0 and turn on bit 6. A
        //      bus conflict will occur if both latches are programmed with the
        //      same DMA #."

        // NTS: This emulation does not use the second DMA channel... yet...
        // which is why we do not bother
        //      unregistering and reregistering the second DMA channel.

        GetDMAChannel(myGUS.dma1)
            ->Register_Callback(nullptr); // FIXME: On DMA conflict this could
                                          // mean kicking other devices off!

        // NTS: Contrary to an earlier commit, DMA channel 0 is not a valid
        // choice
        ...
        LOG(LOG_MISC, LOG_DEBUG)("GUS DMA reprogrammed: DMA1 %d, DMA2 %d",
                                 (int)myGUS.dma1, (int)myGUS.dma2);

        // NTS: The Windows 3.1 Gravis Ultrasound drivers will program the same
        // DMA channel into both without setting the "combining" bit,
        //      even though their own SDK says not to, when Windows starts up.
        //      But it then immediately reprograms it normally, so no bus
        //      conflicts actually occur. Strange.
        gus_warn_dma_conflict =
            (!(val & 0x40) && (val & 7) == ((val >> 3) & 7));
      }
    } else {
      LOG(LOG_MISC, LOG_DEBUG)("GUS warning: Port 2XB register control %02xh "
                               "written (unknown control reg) val=%02xh",
                               (int)myGUS.gRegControl, (int)val);
    }
    break;
  case 0x20f:
    if (gus_type >= GUS_MAX || gus_ics_mixer) {
  ...
  case 0x304:
    if (iolen == 2) {
      if (gus_type <
          GUS_INTERWAVE) // Versions prior to the Interwave will reflect last
                         // I/O to 3X2-3X5 when read back from 3X3
  ...
  case 0x307:
    if (warn_out_of_bounds_dram_access && myGUS.gDramAddr >= myGUS.memsize)
      LOG(LOG_MISC, LOG_WARN)("GUS: out of bounds DRAM write %x val %x",
                              (unsigned int)myGUS.gDramAddr,
                              (unsigned int)val & 0xffu);
  ...
  case 0x706:
    if (gus_type >= GUS_MAX) {
       ...
       *
       * For example, to put the CS4231 codec at port 0x34C, and enable the
       * codec, write 0x44 to this register. If you want to move the codec to
       * base I/O port 0x32C, write 0x42 here. */
      myGUS.gUltraMAXControl = val;

      if (val & 0x40) {
        if ((val & 0xF) != ((port >> 4) & 0xF))
          LOG(LOG_MISC,
              LOG_WARN)("GUS WARNING: DOS application is attempting to "
                        "relocate the CS4231 codec, which is not supported");
      }
    } else if (gus_ics_mixer) {
...
static Bitu GUS_Master_Clock =
    617400; /* NOTE: This is 1000000Hz / 1.619695497. Seems to be a common base
               rate within the hardware. */
static Bitu GUS_DMA_Event_transfer =
    16; /* DMA words (8 or 16-bit) per interval */
static Bitu GUS_DMA_Events_per_sec =
    44100 / 4; /* cheat a little, to improve emulation performance */
...
void GUS_Update_DMA_Event_transfer() {
  /* NTS: From the GUS SDK, bits 3-4 of DMA Control divide the ISA DMA transfer
   * rate down from "approx 650KHz". Bits 3-4 are documented as "DMA Rate
   * divisor" */
  GUS_DMA_Event_transfer = GUS_Master_Clock / GUS_DMA_Events_per_sec /
                           (Bitu)(((Bitu)(myGUS.DMAControl >> 3u) & 3u) + 1u);
  GUS_DMA_Event_transfer &=
      ~1u; /* make sure it's word aligned in case of 16-bit PCM */
...
void GUS_DMA_Event_Transfer(DmaChannel *chan, Bitu dmawords) {
  ...
  // FIXME: What does the GUS do if the DMA address goes beyond the end of
  // memory?

  /* is this a DMA transfer where the GUS memory address is to be translated for
   * 16-bit PCM? Note that there is plenty of code out there that transfers
   * 16-bit PCM with 8-bit DMA, and such transfers DO NOT involve the memory
   * address translation.
   *
   * MODE    XLATE?    NOTE
   * 0x00    NO        8-bit PCM, 8-bit DMA (Most DOS programs)
   * 0x04    DEPENDS   8-bit PCM, 16-bit DMA (Star Control II, see comments
   * below) 0x40    NO        16-bit PCM, 8-bit DMA (Windows 3.1, Quake, for
   * 16-bit PCM over 8-bit DMA) 0x44    YES       16-bit PCM, 16-bit DMA
   * (Windows 3.1, Quake, for 16-bit PCM over 16-bit DMA)
   *
   * Mode 0x04 is marked DEPENDS. It DEPENDS on whether the assigned DMA channel
   * is 8-bit (NO) or 16-bit (YES). Mode 0x04 does not appear to be used by any
   * other DOS application or Windows drivers, only by an erratic bug in Star
   * Control II. FIXME: But, what does REAL hardware do? Drag out the old
   * Pentium 100MHz with the GUS classic and test! --J.C.
   *
   * Star Control II has a bug where, if the GUS DMA channel is 8-bit (DMA
   * channel 0-3), it will upload it's samples to GUS RAM, one DMA transfer per
   * sample, and sometimes set bit 2. Setting bit 2 incorrectly tells the GUS
   * it's a 16-bit wide DMA transfer when the DMA channel is 8-bit. But, if the
   * DMA channel is 16-bit (DMA channel 5-7), Star Control II will correctly set
   * bit 2 in all cases and samples will transfer correctly to GUS RAM.
   * */

  /* FIXME: So, if the GUS DMA channel is 8-bit and Star Control II writes mode
   * 0x04 (8-bit PCM, 16-bit DMA), what does the GF1 do? I'm guessing so far
   * that something happens within the GF1 to transfer as 8-bit anyway, clearly
   * the developers of Star Control II did not hear any audible sign that an
   * invalid DMA control was being used. Perhaps the hardware engineers of the
   * GF1 figured out that case and put something in the silicon to ignore the
   * invalid DMA control state. I won't have any answers until I pull out an old
   * Pentium box with a GUS classic and check. --J.C.
   *
   *        DMA transfers noted by Star Control II that are the reason for this
   * hack (gusdma=1):
   *
   *        LOG:  157098507 DEBUG MISC:GUS DMA: terminal count reached.
   * DMAControl=0x21 LOG:  157098507 DEBUG MISC:GUS DMA transfer 1981 bytes, GUS
   * RAM address 0x0 8-bit DMA 8-bit PCM (ctrl=0x21) LOG:  157098507 DEBUG
   * MISC:GUS DMA: terminal count reached. DMAControl=0x21 LOG:  157100331 DEBUG
   * MISC:GUS DMA: terminal count reached. DMAControl=0x21 LOG:  157100331 DEBUG
   * MISC:GUS DMA transfer 912 bytes, GUS RAM address 0x7c0 8-bit DMA 8-bit PCM
   * (ctrl=0x21) LOG:  157100331 DEBUG MISC:GUS DMA: terminal count reached.
   * DMAControl=0x21 LOG:  157100470 DEBUG MISC:GUS DMA: terminal count reached.
   * DMAControl=0x25 LOG:  157100470 DEBUG MISC:GUS DMA transfer 1053 bytes, GUS
   * RAM address 0xb50 16-bit DMA 8-bit PCM (ctrl=0x25)    <-- What? LOG:
   * 157100470 DEBUG MISC:GUS DMA: terminal count reached. DMAControl=0x25 LOG:
   * 157102251 DEBUG MISC:GUS DMA: terminal count reached. DMAControl=0x21 LOG:
   * 157102251 DEBUG MISC:GUS DMA transfer 1597 bytes, GUS RAM address 0xf80
   * 8-bit DMA 8-bit PCM (ctrl=0x21) LOG:  157102251 DEBUG MISC:GUS DMA:
   * terminal count reached. DMAControl=0x21 LOG:  157104064 DEBUG MISC:GUS DMA:
   * terminal count reached. DMAControl=0x21 LOG:  157104064 DEBUG MISC:GUS DMA
   * transfer 2413 bytes, GUS RAM address 0x15c0 8-bit DMA 8-bit PCM (ctrl=0x21)
   *        LOG:  157104064 DEBUG MISC:GUS DMA: terminal count reached.
   * DMAControl=0x21
   *
   *        (end list)
   *
   *        Noted: Prior to this hack, the samples played by Star Control II
   * sounded more random and often involved leftover sample data in GUS RAM,
   * where with this fix, the music now sounds identical to what is played when
   * using it's Sound Blaster support. */
  if (myGUS.dma1 < 4 /*8-bit DMA channel*/ &&
      (myGUS.DMAControl & 0x44) == 0x04 /*8-bit PCM, 16-bit DMA*/)
    dma16xlate = false; /* Star Control II hack: 8-bit PCM, 8-bit DMA, ignore
                           the bit that says it's 16-bit wide */
  else
    dma16xlate = (myGUS.DMAControl & 0x4) ? true : false;

  if (dma16xlate) {
    // 16-bit wide DMA. The GUS SDK specifically mentions that 16-bit DMA is
    // translated to GUS RAM the same way you translate the play pointer. Eugh.
    // But this allows older demos to work properly even if you set the GUS DMA
    // to a 16-bit channel (5) instead of the usual 8-bit channel (1).
  ...
  // hack: some programs especially Gravis Ultrasound MIDI playback like to
  // upload by DMA but never clear the DMA TC flag on the DMA controller.
  ...
  LOG(LOG_MISC, LOG_DEBUG)("GUS DMA transfer %lu bytes, GUS RAM address 0x%lx "
                           "%u-bit DMA %u-bit PCM (ctrl=0x%02x) tcount=%u",
                           (unsigned long)step, (unsigned long)dmaaddr,
                           (myGUS.DMAControl & 0x4) ? 16 : 8,
                           (myGUS.DMAControl & 0x40) ? 16 : 8, myGUS.DMAControl,
                           chan->tcount);
  ...
  if (chan->tcount) {
    LOG(LOG_MISC, LOG_DEBUG)(
        "GUS DMA transfer hit Terminal Count, setting DMA TC IRQ pending");

    /* Raise the TC irq, and stop DMA */
    myGUS.DMAControl |=
        0x100u; /* NTS: DOSBox SVN approach: Use bit 8 for DMA TC IRQ */
    myGUS.IRQStatus |= 0x80;
    saved_tcount = true;
    GUS_CheckIRQ();
    GUS_StopDMA();
...
void GUS_DMA_Event(Bitu val) {
  ...
  if (chan == NULL) {
    LOG(LOG_MISC, LOG_DEBUG)("GUS DMA event: DMA channel no longer exists, "
                             "stopping DMA transfer events");
  ...
  if (chan->masked) {
    LOG(LOG_MISC,
        LOG_DEBUG)("GUS: Stopping DMA transfer interval, DMA masked=%u",
                   chan->masked ? 1 : 0);
    GUS_DMA_Active = false;
    return;
  }
  ...
  if (!(myGUS.DMAControl & 0x01 /*DMA enable*/)) {
    LOG(LOG_MISC, LOG_DEBUG)("GUS DMA event: DMA control 'enable DMA' bit was "
                             "reset, stopping DMA transfer events");
    GUS_DMA_Active = false;
    return;
  }

  LOG(LOG_MISC,
      LOG_DEBUG)("GUS DMA event: max %u DMA words. DMA: tc=%u mask=%u cnt=%u",
...
void GUS_StartDMA() {
  if (!GUS_DMA_Active) {
    ...
    if (gus_warn_dma_conflict)
      LOG(LOG_MISC, LOG_WARN)(
          "GUS warning: Both DMA channels set to the same channel WITHOUT "
          "combining! "
          "This is documented to cause bus conflicts on real hardware");
  }
}
...
static void GUS_DMA_Callback(DmaChannel *chan, DMAEvent event) {
  ...
  if (event == DMA_UNMASKED) {
    ...
    LOG(LOG_MISC, LOG_DEBUG)(
        "GUS: DMA masked. Perhaps it will stop the DMA transfer event.");
  }
}
...
static void MakeTables(void) {
  ...
  if (gus_fixed_table) {
    ...
    for (i = 8; i < 15; i++)
      pantable[i] =
          (uint32_t)(-128.0 * (log((double)(15 - i) / 7.0) / log(2.0)) *
                     (double)(1 << RAMP_FRACT));
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "GUS pantable (attenuation, left to right in dB): hard left -%.3f, "
      "-%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, center(7) -%.3f, center(8) "
      "-%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, -%.3f, hard right -%.3f",
...
static IO_ReadHandler *gus_cb_port_r(IO_CalloutObject &co, Bitu port,
                                     Bitu iolen) {
  (void)co;
  (void)iolen;

  /* 10-bit ISA decode.
   * NOTE that the I/O handlers still need more than 10 bits to handle GUS
   * MAX/Interwave registers at 0x7xx. */
  port &= 0x3FF;
...
static IO_WriteHandler *gus_cb_port_w(IO_CalloutObject &co, Bitu port,
                                      Bitu iolen) {
  (void)co;
  (void)iolen;
...
public:
  GUS(Section *configuration) : Module_base(configuration) {
    ...
    if (!section->Get_bool("gus") || control->opt_silent)
      return;

    gus_enable = true;
    memset(&myGUS, 0, sizeof(myGUS));

    ignore_active_channel_write_while_active =
        section->Get_bool("ignore channel count while active");
    warn_out_of_bounds_dram_access =
        section->Get_bool("warn on out of bounds dram access");
    ...
    dma_enable_on_dma_control_polling =
        section->Get_bool("dma enable on dma control polling");
    ...
    myGUS.clearTCIfPollingIRQStatus =
        section->Get_bool("clear dma tc irq if excess polling");
    if (myGUS.clearTCIfPollingIRQStatus)
      LOG(LOG_MISC, LOG_DEBUG)(
          "GUS: Will clear DMA TC IRQ if excess polling, as instructed");
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("GUS: using %s sample rate output",
                   myGUS.fixed_sample_rate_output ? "fixed" : "realistic");

    myGUS.force_master_irq_enable =
        section->Get_bool("force master irq enable");
    if (myGUS.force_master_irq_enable)
      LOG(LOG_MISC,
          LOG_DEBUG)("GUS: Master IRQ enable will be forced on as instructed");

    {
      ...
      if (!strcmp(s, "true") || !strcmp(s, "1"))
        ...
        myGUS.globalread80alias =
            false; /* TODO: Test real hardware, which versions of the GUS behave
                      like this? */
    }
    ...
    if (x >= 0)
      myGUS.memsize = (unsigned int)x * 1024u;
    else
    ...
    if ((myGUS.memsize & ((256u << 10u) - 1u)) != 0)
      LOG(LOG_MISC,
          LOG_WARN)("GUS emulation warning: %uKB onboard is an unusual value. "
                    "Usually GUS cards have some multiple of 256KB RAM onboard",
                    myGUS.memsize >> 10);

    if (gus_type <= GUS_MAX && myGUS.memsize > (1024u * 1024u))
      LOG(LOG_MISC,
          LOG_WARN)("GUS emulation warning: %uKB onboard is an unusually large "
                    "(more than 1MB) value for the model of GUS to emuulate.",
                    myGUS.memsize >> 10);
    ...
    assert(myGUS.GUSRam == NULL);
    {
      ...
      LOG(LOG_MISC, LOG_DEBUG)("GUS emulation: %uKB onboard (mask 0x%x)",
                               myGUS.memsize >> 10, (unsigned int)mask);
    ...
    // some demoscene stuff has music that's way too loud if we render at full
    // volume. the GUS mixer emulation won't fix it because it changes the
    // volume at the Mixer level AFTER the code has rendered and clipped samples
    // to 16-bit range.
    ...
    // in other parts of the code to compare against 0x200 and 0x300? That's
    // confusing. Fix!
    myGUS.portbase = (unsigned int)section->Get_hex("gusbase") - 0x200u;

    // TODO: so, if the GUS ULTRASND variable actually mentions two DMA and two
    // IRQ channels,
    //       shouldn't we offer the ability to specify them independently?
    //       especially when GUS NMI is completed to the extent that SBOS and
    //       MEGA-EM can run within DOSBox?
    ...
    if (irq_val > 0) {
      ...
      if (!s.empty() && s != "none") {
        LOG(LOG_MISC, LOG_NORMAL)(
            "GUS emulation: Assigning IRQ hack '%s' as instructed", s.c_str());
...

    {
      ...
      if (obj == NULL)
        E_Exit("Failed to get GUS IO callout");
      obj->Install(0x200 + GUS_BASE,
                   IOMASK_Combine(IOMASK_ISA_10BIT, IOMASK_Range(16)),
                   gus_cb_port_r, gus_cb_port_w);
      IO_PutCallout(obj);
    }
    ...
    if (gus_type >= GUS_MAX) {
      LOG(LOG_MISC, LOG_WARN)(
          "GUS caution: CS4231 UltraMax emulation is new and experimental at "
          "this time and it is not guaranteed to work.");
      LOG(LOG_MISC, LOG_WARN)(
          "GUS caution: CS4231 UltraMax emulation as it exists now may cause "
          "applications to hang or malfunction attempting to play through it.");
    ...
    if (gus_ics_mixer) {
      ...
      GUS_ICS2101.mixpair[gus_ICS2101::MASTER_OUTPUT_PORT].setAttenuation(0,
                                                                          0x7F);
      GUS_ICS2101.mixpair[gus_ICS2101::MASTER_OUTPUT_PORT].setAttenuation(1,
                                                                          0x7F);
  ...
  void DOS_Startup() {
    ...
    // [GUS port], [GUS DMA (recording)], [GUS DMA (playback)], [GUS IRQ
    // (playback)], [GUS IRQ (MIDI)]
    ostringstream temp;
    temp << "@SET ULTRASND=" << hex << setw(3) << portat << "," << dec
         << (Bitu)myGUS.dma1 << "," << (Bitu)myGUS.dma2 << ","
...
void GUS_Init() {
  ...
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(GUS_DOS_Exit));
  ...
  AddVMEventFunction(VM_EVENT_DOS_INIT_SHELL_READY,
                     AddVMEventFunctionFuncPair(GUS_DOS_Boot));
}
