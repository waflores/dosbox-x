 ...
 */
/* FIXME: At some point I would like to roll the Disney Sound Source emulation
 * into this code */
...
#if HAS_CDIRECTLPT && (defined __i386__ || defined __x86_64__) &&              \
...
#if C_PRINTER
  if (parallelPortObjects[num] &&
      parallelPortObjects[num]->parallelType == PARALLEL_TYPE_PRINTER)
...
const char *lptname[] = {"LPT1", "LPT2", "LPT3", "LPT4", "LPT5",
                         "LPT6", "LPT7", "LPT8", "LPT9"};
...
device_LPT::~device_LPT() {
  /* clear reference to myself so that CParallel does not attempt to free me or
   * ask DOS to delete me */
...
void CParallel::setEvent(uint16_t type, float duration) {
  PIC_AddEvent(Parallel_EventHandler, duration,
               ((Bitu)type << 4u) | (Bitu)port_nr);
}

void CParallel::removeEvent(uint16_t type) {
  // TODO
  PIC_RemoveSpecificEvents(Parallel_EventHandler,
                           ((Bitu)type << 4u) | (Bitu)port_nr);
}

void CParallel::handleEvent(uint16_t type) { handleUpperEvent(type); }

static Bitu PARALLEL_Read(Bitu port, Bitu iolen) {
  (void)iolen; // UNUSED
  for (Bitu i = 0; i < 9; i++) {
    /* NTS: The traditional parport range is assigned 8 ports by IBM, but only
     * 0-2 are assigned. EPP ports assign ports 3-4 with possible vendor
     * extensions in 5-6, while ECP assigns ports 0x400-0x402 relative to base
     * and leave 3-7 undefined. */
...
CParallel::CParallel(CommandLine *cmd, Bitu portnr, uint8_t initirq) {
  ...
  for (Bitu i = 0; i < 3; i++) {
    /* bugfix: do not register I/O write handler for the status port. it's a
     * *status* port. also, this is needed for ISA PnP emulation to work
     * properly even if DOSBox is emulating more than one parallel port. */
    if (i != 1)
...
void CParallel::unregisterDOSDevice() {
  if (mydosdevice != NULL) {
    LOG(LOG_MISC, LOG_DEBUG)("LPT%d: Unregistering DOS device",
                             (int)port_nr + 1);
...
CParallel::~CParallel(void) { unregisterDOSDevice(); }
...
CParallel *parallelPortObjects[] = {NULL, NULL, NULL, NULL, NULL,
                                    NULL, NULL, NULL, NULL};
...
void BIOS_Post_register_parports() {
  ...
  for (i = 0; i < 9; i++) {
    ...
    else if (DISNEY_HasInit() &&
             parallel_baseaddr[i] == (uint16_t)DISNEY_BasePort())
...
public:
  PARPORTS(Section *configuration) : Module_base(configuration) {
    ...
    for (Bitu i = 0; i < 9; i++) {
      ...
      CommandLine tmp(nullptr, section->Get_string(pname), CommandLine::either,
                      true);
      ...
      /* DOSBox SVN disney=true compatibility: Reserve LPT1 for Disney Sound
       * Source */
      ...
      if (str == "reallpt") {
        ...
        if (str == "printer" && !printer_used) {
          ...
          parallelPortObjects[i] = nullptr;
          if (!DISNEY_HasInit()) {
            LOG_MSG("LPT%d: User explicitly assigned Disney Sound Source to "
                    "this port",
                    (int)i + 1);
            ...
            LOG_MSG("LPT%d: Disney Sound Source already initialized on a port, "
                    "cannot init again",
                    (int)i + 1);
          }
        } else {
...
void PARALLEL::showPort(int port) {
  if (parallelPortObjects[port] != nullptr)
    WriteOut("LPT%d: %s %s\n", port + 1,
             parallelTypes[parallelPortObjects[port]->parallelType],
             parallelPortObjects[port]->commandLineString.c_str());
...
void PARALLEL::Run() {
  ...
  if (cmd->GetCount() >= 2) {
    ...
    CommandLine tmp("PARALLEL.COM", commandLineString.c_str(),
...
#if C_PRINTER
      if (mode == PARALLEL_TYPE_PRINTER &&
          parallelPortObjects[port - 1]->parallelType !=
              PARALLEL_TYPE_PRINTER &&
          testParallelPortsBaseclass->printer_used) {
        WriteOut("Printer is already assigned to a different port.\n");
        return;
      }
      if (parallelPortObjects[port - 1]->parallelType == PARALLEL_TYPE_PRINTER)
        testParallelPortsBaseclass->printer_used = false;
#endif
      if (mode == PARALLEL_TYPE_DISNEY && disneyport != port &&
          DISNEY_HasInit()) {
...
#if C_PRINTER
    } else if (mode == PARALLEL_TYPE_PRINTER &&
               testParallelPortsBaseclass->printer_used) {
      WriteOut("Printer is already assigned to a different port.\n");
      return;
#endif
    } else if (mode == PARALLEL_TYPE_DISNEY && disneyport != port &&
               DISNEY_HasInit()) {
      WriteOut("Disney is already assigned to a different port.\n");
      return;
    ...
    case PARALLEL_TYPE_REALLPT: {
      ...
      if (cdlpt->InstallationSuccessful)
        parallelPortObjects[port - 1] = cdlpt;
      break;
    }
#endif
    case PARALLEL_TYPE_FILE: {
      CFileLPT *cflpt =
          new CFileLPT(port - 1, defaultirq[port - 1], &cmd, squote);
      if (cflpt->InstallationSuccessful)
        parallelPortObjects[port - 1] = cflpt;
      break;
    }
#if C_PRINTER
    case PARALLEL_TYPE_PRINTER:
      if (!testParallelPortsBaseclass->printer_used) {
        CPrinterRedir *cprd =
            new CPrinterRedir(port - 1, defaultirq[port - 1], &cmd);
        if (cprd->InstallationSuccessful) {
...
void PARALLEL_ProgramStart(Program **make) { *make = new PARALLEL; }
...
void PARALLEL_OnPowerOn(Section *sec) {
  ...
  /* DOSBox SVN compatibility: LPT1 should be reserved for Disney Sound Source
   * if disney=true, now init it */
  if (!DISNEY_HasInit() && DISNEY_ShouldInit() &&
      parallelPortObjects[0] == NULL) {
    LOG_MSG(
        "disney=true. For compatibility with other DOSBox forks and SVN, LPT1 "
        "has been reserved for Disney Sound Source. Initializing it now.");
...
void PARALLEL_Init() {
  ...
  if (!IS_PC98_ARCH) {
    AddVMEventFunction(VM_EVENT_RESET,
                       AddVMEventFunctionFuncPair(PARALLEL_OnReset));
    AddVMEventFunction(VM_EVENT_POWERON,
                       AddVMEventFunctionFuncPair(PARALLEL_OnPowerOn));
    AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                       AddVMEventFunctionFuncPair(PARALLEL_OnDOSKernelExit));
    AddVMEventFunction(VM_EVENT_DOS_INIT_KERNEL_READY,
                       AddVMEventFunctionFuncPair(PARALLEL_OnDOSKernelInit));
  }
}
