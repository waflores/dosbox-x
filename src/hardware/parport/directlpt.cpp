...
CDirectLPT::CDirectLPT(Bitu nr, uint8_t initIrq, CommandLine *cmd)
    : CParallel(cmd, nr, initIrq), InstallationSuccessful(false),
      realbaseaddress(0x378), ecraddress(0), originalecr(0), isecp(false),
      controlreg(0xc0)
#ifdef LINUX
      ,
...
#endif
{
...
#endif
  {
    if (cmd->FindStringBegin("realbase:", str, false)) {
      if (sscanf(str.c_str(), "%hx", &realbaseaddress) != 1) {
        LOG_MSG("parallel%d: Invalid realbase parameter (%s). Pass-through I/O "
                "disabled.",
                (int)nr + 1, str.c_str());
        return;
      }
    ...
    if (cmd->FindStringBegin("realbase:", str, false)) {
      LOG_MSG("parallel%d: realbase (raw I/O) requires root privileges. "
              "Pass-through I/O disabled.",
              (int)nr + 1);
      return;
    } else if (cmd->FindStringBegin("ecpbase:", str, false)) {
      LOG_MSG("parallel%d: ecpbase (raw I/O) requires root privileges. "
              "Pass-through I/O disabled.",
              (int)nr + 1);
      return;
    }
#ifdef BSD
    LOG_MSG("parallel%d: Raw I/O requires root privileges. Pass-through I/O "
            "disabled.",
            (int)nr + 1);
    return;
#endif
  ...
  if (useppdev) {
    if (!cmd->FindStringBegin("realport:", str, false)) {
      LOG_MSG(
          "parallel%d: realport parameter missing. Pass-through I/O disabled.",
          (int)nr + 1);
      return;
    }
    porthandle = open(str.c_str(), O_RDWR);
    if (porthandle == -1) {
      LOG_MSG("parallel%d: Could not open parallel port %s. Pass-through I/O "
              "disabled.",
              (int)nr + 1, str.c_str());
      if (errno == ENOENT)
        LOG_MSG("parallel%d: The specified parallel port does not exist.",
                (int)nr + 1);
      else if (errno == EBUSY)
        LOG_MSG("parallel%d: The specified parallel port is already in use.",
                (int)nr + 1);
      else if (errno == EACCES)
        LOG_MSG("parallel%d: You are not allowed to access this parallel port.",
                (int)nr + 1);
      else
        LOG_MSG("parallel%d: Error (%d): %s", (int)nr + 1, errno,
                strerror(errno));
      return;
    }

    if (ioctl(porthandle, PPCLAIM) == -1) {
      LOG_MSG("parallel%d: Failed to claim parallel port. Pass-through I/O "
              "disabled.",
              (int)nr + 1);
      close(porthandle);
      return;
    ...
    if (ioctl(porthandle, PPSETMODE, &portmode) == -1)
      LOG_MSG("parallel%d: Could not set bidirectional mode.",
              (int)nr + 1); // but pass-through I/O has been enabled
...
#endif
  {
    ...
    if (realbaseaddress < 0x100 ||
        (realbaseaddress & 0x03) || // sanity + mainboard res.
        (realbaseaddress >= 0x1f0 &&
         realbaseaddress <= 0x1f7) || // prim. HDD controller
        (realbaseaddress >= 0x170 &&
         realbaseaddress <= 0x177) || // sec. HDD controller
        (realbaseaddress >= 0x3f0 &&
         realbaseaddress <= 0x3f7) || // floppy + prim. HDD
        (realbaseaddress >= 0x370 && realbaseaddress <= 0x377)) { // sec. hdd
      LOG_MSG("parallel%d: Invalid base address (0x%hx). Pass-through I/O "
              "disabled.",
              (int)nr + 1, realbaseaddress);
      return;
    }
    ...
    */
    if (cmd->FindStringBegin("ecpbase:", str, false)) {
      if (sscanf(str.c_str(), "%hx", &ecraddress) != 1) {
        LOG_MSG("parallel%d: Invalid ecpbase parameter (%s). Pass-through I/O "
                "disabled.",
                (int)nr + 1, str.c_str());
        return;
      }
      ...
      if (isecp)
        ecraddress = realbaseaddress + 0x402;
    }
...
#if 0 // do not perform parallel port I/O that may confuse a connected
      // (non-printing) device check if there is a parallel port at all: the
      // autofeed bit
...
#endif
    // the reference to ecraddress below regardless of isecp is intentional (for
    // troubleshooting)
    LOG_MSG("parallel%d: The parallel port at 0x%x (ECR=0x%x) was %sdetected "
            "as supporting ECP. Pass-through I/O enabled.",
            (int)nr + 1, realbaseaddress, ecraddress, isecp ? "" : "not ");
  }

#if 0 // do not perform parallel port I/O that may confuse a connected
      // (non-printing) device go for it
	initialize();
#else
#ifdef LINUX
  if (!useppdev)
