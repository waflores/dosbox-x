...
private:
  ...
  // Checks if given char belongs to a command and process it. If false, the
  ...
  // Blits the given glyph on the page surface. If add is true, the values of
  // bitmap are added to the values of the pixels in the page
  void blitGlyph(FT_Bitmap bitmap, uint16_t destx, uint16_t desty, bool add);

  // Draws an anti-aliased line from (fromx, y) to (tox, y). If broken is true,
  // gaps are included
  ...
  // Prints out a byte using ASCII85 encoding (only outputs something every four
  // bytes). When b>255, closes the ASCII85 string
  void fprintASCII85(FILE *f, uint16_t b);

  // Closes a multipage document
  void finishMultipage();

  // Returns value of the num-th pixel (counting left-right, top-down) in a safe
  // way
  ...
  bool FSSeen =
      false; // True if last read character was an FS (0x1C) (IBM commands)

  uint8_t numParam = 0,
          neededParam =
              0; // Numbers of parameters already read/needed to process command

  uint8_t params[20] = {};    // Buffer for the read params
  uint16_t style = 0;         // Style of font (see STYLE_* constants)
  double cpi = 0, actcpi = 0; // CPI value set by program and the actual one
                              // (taking in account font types)
  uint8_t score = 0;          // Score for lines (see SCORE_* constants)

  double topMargin = 0, bottomMargin = 0, rightMargin = 0,
         leftMargin = 0;                // Margins of the page (in inch)
  double pageWidth = 0, pageHeight = 0; // Size of page (in inch)
  double defaultPageWidth = 0,
         defaultPageHeight = 0; // Default size of page (in inch)
  ...
  double extraIntraSpace =
      0; // Extra space between two characters (set by program, in inch)

  bool charRead = false; // True if a character was read since the printer was
                         // last initialized
  bool autoFeed = false; // True if a LF should automatically added after a CR
  bool printUpperContr =
      false; // True if the upper command characters should be printed
  ...
  uint8_t densk = 0, densl = 0, densy = 0,
          densz = 0; // Image density modes used in ESC K/L/Y/Z commands

  uint16_t curMap[256] = {};   // Currently used ASCII => Unicode mapping
  uint16_t charTables[4] = {}; // Charactertables

  double definedUnit =
      0; // Unit used by some ESC/P2 commands (negative => use default)
  ...
  uint16_t numPrintAsChar = 0; // Number of bytes to print as characters (even
                               // when normally control codes)
  ...
  void *outputHandle = NULL; // If not null, additional pages will be appended
                             // to the given handle
  bool multipageOutput =
      false; // If true, all pages are combined to one file/print job etc. until
             // the "eject page" button is pressed
