...
void CPrinter::FillPalette(uint8_t redmax, uint8_t greenmax, uint8_t bluemax,
...
CPrinter::CPrinter(uint16_t dpi, uint16_t width, uint16_t height, char *output,
                   bool multipageOutput) {
  if (FT_Init_FreeType(&FTlib)) {
    LOG(LOG_MISC,
        LOG_ERROR)("PRINTER: Unable to init Freetype2. Printing disabled");
    page = NULL;
  } else {
    this->dpi = dpi;
    this->output = output;
    ...
    // Create page
    page = SDL_CreateRGBSurface(SDL_SWSURFACE, (int)(defaultPageWidth * dpi),
                                (int)(defaultPageHeight * dpi), 8, 0, 0, 0, 0);
...
void CPrinter::selectCodepage(uint16_t cp) {
  ...
  if (mapToUse == NULL) {
    LOG(LOG_MISC, LOG_WARN)("Unsupported codepage %i. Using CP437 instead.",
                            cp);
    selectCodepage(437);
    return;
  }
...
void CPrinter::updateFont() {
...
#endif
  {
    ...
    if (stat(fontName.c_str(), &wstat)) {
      ...
      if (stat(fontName.c_str(), &wstat)) {
        ...
        resfont += fontName;
        fontName = resfont;
        if (stat(fontName.c_str(), &wstat) && exepath.size())
          fontName = exepath + CROSS_FILESPLIT + name;
      }
    }
    if ((printdbcs == 1 || (printdbcs == -1 && (isJEGAEnabled() || IS_DOSV ||
                                                ((TTF_using() || showdbcs)
#if defined(USE_TTF)
                                                 && dbcs_sbcs
#endif
    ...
    if (FT_New_Face(FTlib, fontName.c_str(), 0, &curFont)) {
      ...
      case roman:
        ...
        if (stat(fontName.c_str(), &wstat))
          fontName =
              basedir + "truetype/liberation/LiberationSerif-Regular.ttf";
#endif
        break;
      case sansserif:
#if defined(WIN32)
        ...
        if (stat(fontName.c_str(), &wstat))
          fontName = basedir + "truetype/liberation/LiberationSans-Regular.ttf";
#endif
        break;
      case courier:
#if defined(WIN32)
        ...
        if (stat(fontName.c_str(), &wstat))
          fontName = basedir + "truetype/liberation/LiberationMono-Regular.ttf";
#endif
        break;
      case script:
      ...
      if (FT_New_Face(FTlib, fontName.c_str(), 0, &curFont)
#if defined(USE_TTF)
          && FT_New_Memory_Face(FTlib, DOSBoxTTFbi, sizeof(DOSBoxTTFbi), 0,
                                &curFont)
#endif
      ) {
        // LOG(LOG_MISC,LOG_ERROR)("Unable to load font %s", fontName);
        LOG_MSG("Unable to load font %s (or %s)", oldfont.c_str(),
                fontName.c_str());
        curFont = NULL;
      }
    }
  ...
  FT_Set_Char_Size(curFont, (uint16_t)horizPoints * 64,
                   (uint16_t)vertPoints * 64, dpi, dpi);
...
bool CPrinter::processCommandChar(uint8_t ch) {
  if (ESCSeen || FSSeen) {
    ...
    default:
      LOG_MSG(
          "PRINTER: Unknown command %s (%02Xh) %c , unable to skip parameters.",
  ...
  if (ESCCmd == '(') {
    ...
    default:
      // ESC ( commands are always followed by a "number of parameters" word
      // parameter
      // LOG(LOG_MISC,LOG_ERROR)
      LOG_MSG("PRINTER: Skipping unsupported command ESC ( %c (%02X).", ESCCmd,
              ESCCmd);
      neededParam = 2;
      ESCCmd = 0x101;
      return true;
  ...
  if (ESCCmd == 0x42) {
    if (ch == 0 || (numVertTabs > 0 && verttabs[numVertTabs - 1] >
                                           (double)ch * lineSpacing)) // Done
      ESCCmd = 0;
    else if (numVertTabs < 16)
  ...
  if (ESCCmd == 0x44) {
    if (ch == 0 ||
        (numHorizTabs > 0 &&
         horiztabs[numHorizTabs - 1] > (double)ch * (1 / (double)cpi))) // Done
      ESCCmd = 0;
    else if (numHorizTabs < 32)
  ...
  if (ESCCmd != 0) {
    ...
    case 0x20: // Set intercharacter space (ESC SP)
      if (!multipoint) {
        extraIntraSpace =
            (double)params[0] / (printQuality == QUALITY_DRAFT ? 120 : 180);
        hmi = -1;
        updateFont();
      }
      break;
    ...
    case 0x67: // Select 10.5-point, 15-cpi (ESC g)
      ...
      updateFont();
      break;
    case 0x846: // Select forward feed mode (FS F) - set reverse not implemented
                // yet
      if (lineSpacing < 0)
        lineSpacing *= -1;
      break;
    ...
    case 0x274: // Assign character table (ESC (t)
      if (params[2] < 4 && params[3] < 15) {
        ...
        // %d",curCharTable,params[2],params[3]);
    ...
    case 0x263: // Set page format (ESC (c)
      if (definedUnit > 0) {
        ...
        if (newTop >= newBottom)
          break;
        if (newTop < pageHeight)
          topMargin = newTop;
        if (newBottom < pageHeight)
          bottomMargin = newBottom;
        if (topMargin > curY)
          curY = topMargin;
        // LOG_MSG("du %d, p1 %d, p2 %d, newtop %f, newbott %f, nt %f, nb %f, ph
        // %f", 	(Bitu)definedUnit,PARAM16(2),PARAM16(4),topMargin,bottomMargin,
    ...
    default:
      if (ESCCmd < 0x100)
        ...
        LOG_MSG("PRINTER: Skipped unsupported command ESC %c (%02X)", ESCCmd,
                ESCCmd);
      else
        // LOG(LOG_MISC,LOG_WARN)
  ...
  case 0x0a: // Line feed
    ...
    if (curY > bottomMargin - 0.0001) // goto the next page if curY is larger or
                                      // slightly smaller than bottomMargin
      newPage(true, false);
    return true;
...
void CPrinter::printChar(uint8_t ch, int box) {
  ...
  if ((printdbcs == 1 || (printdbcs == -1 && (isJEGAEnabled() || IS_DOSV ||
    ...
    if (box != 0 && last3 && last2 && lastchar && lasttick &&
        (tick - lasttick < 50 || tick - lasttick < printer_timout)) {
      if (ch >= 176 && ch <= 223
#if defined(USE_TTF)
          && autoboxdraw
#endif
    ...
    } else if (box != 0 && box2 && last2 && lastchar && lasttick &&
               (tick - lasttick < 50 || tick - lasttick < printer_timout)) {
      if (ch >= 176 && ch <= 223
#if defined(USE_TTF)
          && autoboxdraw
        ...
        last3 = last2 = 0;
               ...
               (tick - lasttick < 50 || tick - lasttick < printer_timout)) {
      if (box != 0 && lastchar >= 176 && lastchar <= 223 && ch >= 176 &&
          ch <= 223
#if defined(USE_TTF)
          && autoboxdraw
#endif
  ...
  if (!dbchar && dos.loaded_codepage == 932 &&
      (
          ...
          isJEGAEnabled())) {
    if (ch >= 0xA1 && ch <= 0xDF)
      printch = cpMap_PC98[ch];
    else {
      std::map<int, int>::iterator it = lowboxdrawmap.find(ch);
      if (lowboxdrawmap.find(ch) != lowboxdrawmap.end())
        printch = charmap[0].map[it->second];
    }
  }
  ...
  uint16_t penY = (uint16_t)(PIXY - curFont->glyph->bitmap_top +
  ...
  if ((style & STYLE_PROP) || dbcs)
    x_advance =
        (double)((double)(curFont->glyph->advance.x) / (double)(dpi * 64));
  else {
    if (hmi < 0)
  ...
  if ((score != SCORE_NONE) &&
    ...
    double height = (curFont->size->metrics.height >>
    ...
    if (style & STYLE_UNDERLINE)
      ...
      lineY = (uint16_t)PIXY - (uint16_t)(((score == SCORE_DOUBLE) ||
...
void CPrinter::blitGlyph(FT_Bitmap bitmap, uint16_t destx, uint16_t desty,
                         bool add) {
  for (Bitu y = 0; y < bitmap.rows; y++) {
    for (Bitu x = 0; x < bitmap.width; x++) {
      ...
      if (source > 0 && (destx + x < (Bitu)page->w) &&
          (desty + y < (Bitu)page->h)) {
        uint8_t *target =
            (uint8_t *)page->pixels + (x + destx) + (y + desty) * page->pitch;
        source >>= 3;
...
void CPrinter::drawLine(Bitu fromx, Bitu tox, Bitu y, bool broken) {
  ...
  for (Bitu x = fromx; x <= tox; x++) {
    ...
    if ((!broken || (x % breakmod <= gapstart)) && (x < (Bitu)page->w)) {
      ...
      if (y < (Bitu)page->h)
        *((uint8_t *)page->pixels + x + y * (Bitu)page->pitch) =
            !broken ? 255 : 240;
      if (y + 1 < (Bitu)page->h)
        *((uint8_t *)page->pixels + x + (y + 1) * (Bitu)page->pitch) = 240;
    }
...
void CPrinter::setAutofeed(bool feed) { autoFeed = feed; }

bool CPrinter::getAutofeed() { return autoFeed; }
        ...
        for (Bitu xx = 0; xx < pixsizeX; xx++)
          for (Bitu yy = 0; yy < pixsizeY; yy++) {
            if (((PIXX + xx) < (Bitu)page->w) && ((PIXY + yy) < (Bitu)page->h))
              *((uint8_t *)page->pixels + (PIXX + xx) +
                (PIXY + yy) * page->pitch) |= (color | 0x1F);
          }
      } // else white pixel
...
void CPrinter::formFeed() {
  if ((printdbcs == 1 || (printdbcs == -1 && (TTF_using() || showdbcs)
    ...
    if (last3 && last2) {
      printChar(
          last3,
          last3 >= 176 && last3 <= 223 && last2 >= 176 && last2 <= 223 ? 1 : 0);
      lastchar = last3;
      printChar(
          last2,
          last3 >= 176 && last3 <= 223 && last2 >= 176 && last2 <= 223 ? 1 : 0);
...
void CPrinter::doAction(const char *fname) {
  ...
  if (actstd.size()) {
    ...
    fail = (INT_PTR)ShellExecute(NULL, "open", action.c_str(), para.c_str(),
                                 NULL, shellhide ? SW_HIDE : SW_NORMAL) <= 32;
#else
    ...
    if (fail)
      systemmessagebox("Error",
                       "The requested file handler failed to complete.", "ok",
                       "error", 1);
  }
}
...
void CPrinter::outputPage() {
  ...
  if (strcasecmp(output, "printer") == 0) {
    ...
    void *pBits = NULL;
    HBITMAP hBitmap =
        CreateDIBSection(printerDC, &bmi, DIB_RGB_COLORS, &pBits, NULL, 0);
    ...
    for (int y = 0; y < page->h; y++) {
      memcpy((uint8_t *)pBits + y * page->w,
             (uint8_t *)page->pixels + y * page->pitch, page->w);
    }
    SDL_UnlockSurface(page);

    // Stretch and copy the bitmap from the memory DC to the printer DC, scaling
    // it to the printer's physical dimensions
    StretchBlt(printerDC, 0, 0, physW, physH, memHDC, 0, 0, page->w, page->h,
               SRCCOPY);
    ...
    if (!fp) {
      LOG(LOG_MISC, LOG_ERROR)("PRINTER: Can't open file %s for printer output",
                               fname);
      return;
    }
    ...
    png_set_IHDR(png_ptr, info_ptr, page->w, page->h, 8, PNG_COLOR_TYPE_PALETTE,
    ...
    if (psfile == NULL) {
      ...
      if (!psfile) {
        LOG(LOG_MISC,
            LOG_ERROR)("PRINTER: Can't open file %s for printer output", fname);
        return;
      }
      ...
      fprintf(psfile, "%%%%BoundingBox: 0 0 %i %i\n",
              (uint16_t)(defaultPageWidth * 72),
              (uint16_t)(defaultPageHeight * 72));
...
void CPrinter::fprintASCII85(FILE *f, uint16_t b) {
  if (b != 256) {
    ...
    if (ASCII85BufferPos == 4 || b == 257) {
      uint32_t num =
          (uint32_t)ASCII85Buffer[0] << 24 | (uint32_t)ASCII85Buffer[1] << 16 |
          (uint32_t)ASCII85Buffer[2] << 8 | (uint32_t)ASCII85Buffer[3];
      ...
      if (num == 0 && b != 257) {
        ...
        // Make sure a line never starts with a % (which may be mistaken as
        // start of a comment)
...
void PrintScreen(const char *text, uint16_t len) {
  if (!defaultPrinter)
    defaultPrinter = new CPrinter(confdpi, confwidth, confheight,
                                  confoutputDevice, confmultipageOutput);
  for (int i = 0; i < len; i++)
    defaultPrinter->printChar(text[i]);
  FormFeed(true);
}
...
void PrintText(bool pressed) {
  ...
  if (!PRINTER_isInited()) {
    systemmessagebox("Error",
                     "Printer support is not enabled in the configuration.",
                     "ok", "error", 1);
    return;
  }
  if (!CurMode || CurMode->type != M_TEXT) {
    systemmessagebox("Error", "The current DOS screen is not in text mode.",
                     "ok", "error", 1);
    return;
  }
  uint16_t len = 0;
  const char *text =
      Mouse_GetSelected(0, 0, (int)(currentWindowWidth - 1 - sdl.clip.x),
                        (int)(currentWindowHeight - 1 - sdl.clip.y),
                        (int)(currentWindowWidth - sdl.clip.x),
                        (int)(currentWindowHeight - sdl.clip.y), &len);
  if (len)
    PrintScreen(text, len);
  return;
}
...
Bitu PRINTER_readcontrol(Bitu port, Bitu iolen) {
  ...
  return 0xe0 | (defaultPrinter->getAutofeed() ? 0x02 : 0x00) |
         (controlreg & 0xfd);
}
...
bool inited = false;
bool PRINTER_isInited() { return inited; }
...
void PRINTER_Init() {
  ...
  if (!printer_timout)
    timeout_dirty = true; // this will lock up the timeout
  else
    timeout_dirty = false;
  ...
  printdbcs =
      !strcasecmp(dbcsstr, "true") || !strcasecmp(dbcsstr, "1")
          ? 1
          : (!strcasecmp(dbcsstr, "false") || !strcasecmp(dbcsstr, "0") ? 0
                                                                        : -1);
  ...
  if (!inited && !strcasecmp(confoutputDevice, "printer")) {
    ...
    bool fnReturn =
        EnumPrinters(PRINTER_ENUM_LOCAL | PRINTER_ENUM_CONNECTIONS, NULL, 1L,
                     (LPBYTE)NULL, 0L, &dwNeeded, &dwReturned);
    PRINTER_INFO_1 *pInfo = NULL;
    if (dwNeeded > 0)
      pInfo = (PRINTER_INFO_1 *)HeapAlloc(GetProcessHeap(), 0L, dwNeeded);
    if (NULL != pInfo) {
      dwReturned = 0;
      fnReturn =
          EnumPrinters(PRINTER_ENUM_LOCAL | PRINTER_ENUM_CONNECTIONS, NULL, 1L,
                       (LPBYTE)pInfo, dwNeeded, &dwNeeded, &dwReturned);
    }
    if (fnReturn) {
      unsigned int devnum;
      if (1 != sscanf(device.c_str(), "%u", &devnum))
        devnum = -2;
      prtlist = "Printer Device "
                "List\n--------------------------------------------------------"
                "-----\n";
      for (unsigned int i = 1; i < dwReturned; i++) {
        if (devnum > 0 && i == devnum)
          device = pInfo[i].pName;
        prtlist += (i < 10 ? "0" : "") + std::to_string(i) + " " +
                   pInfo[i].pName + "\n";
      }
