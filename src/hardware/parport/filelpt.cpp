...
CFileLPT::CFileLPT(Bitu nr, uint8_t initIrq, CommandLine *cmd, bool sq)
  ...
  if (cmd->FindStringBegin("shellhide", str, false))
    shellhide = true;

  // add a formfeed when closing?
  if (cmd->FindStringBegin("addFF", str, false))
    addFF = true;
  else
    addFF = false;

  // add a formfeed when closing?
  if (cmd->FindStringBegin("addLF", str, false))
    addLF = true;
  else
    addLF = false;
...
char bufput[105];
int bufct = 0;
static char sig1PCL[] = "\x1b%-12345X@", sig2PCL[] = "\x1b\x45",
            sigPS[] = "\n%!";
void CFileLPT::doAction() {
  if (action1.size() || action2.size() || action3.size()) {
    ...
    if ((action1.size() || action2.size()) && bufct > 5) {
      if (!strncmp(bufput, sig1PCL, sizeof(sig1PCL) - 1) ||
          !strncmp(bufput, sig2PCL, sizeof(sig2PCL) - 1)) {
        isPCL = true;
        int max = bufct > 65
                      ? 60
                      : bufct - 5; // A line should start with the signature in
                                   // the first 60 characters or so
        for (int i = 0; i < max; i++)
    ...
    if (filetype == FILE_CAPTURE && pathprt.size())
      name = pathprt;
    std::string action = action1.size() && isPS
                             ? action1
                             : (action2.size() && isPCL ? action2 : action3);
    bool fail = false;
#if defined(WIN32)
    bool q = false;
    int pos = -1;
    ...
    if (fail)
      systemmessagebox(
          "Error", "The requested file printing handler failed to complete.",
          "ok", "error", 1);
  }
  bufct = 0;
}
...
bool CFileLPT::Putchar(uint8_t val) {
  ...
  if (codepage_ptr != NULL) {
    ...
    if (extchar & 0xFF00)
      fputc((int)((uint8_t)(extchar >> 8)), file);
...
Bitu CFileLPT::Read_PR() { return datareg; }
Bitu CFileLPT::Read_COM() { return controlreg; }
...
void CFileLPT::Write_PR(Bitu val) { datareg = (uint8_t)val; }
void CFileLPT::Write_CON(Bitu val) {
  ...
  controlreg = val & 0xF; /* do NOT store bit 5, we do not emulate bidirectional
                             LPT ports, yet */
}
