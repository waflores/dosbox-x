...
std::string pathvid = "", pathwav = "", pathmtw = "", pathmid = "",
            pathopl = "", pathscr = "", pathprt = "", pathpcap = "";
...
void ffmpeg_closeall() {
  if (ffmpeg_fmt_ctx != NULL) {
    ...
    ffmpeg_fmt_ctx = NULL;
    ffmpeg_vid_ctx = NULL; // NTS: avformat_free_context() freed this for us,
                           // don't free again
    ffmpeg_aud_ctx = NULL; // NTS: avformat_free_context() freed this for us,
                           // don't free again
  }
  if (ffmpeg_vid_ctx != NULL) {
    avcodec_close(ffmpeg_vid_ctx);
...
void ffmpeg_audio_frame_send() {
  ...
  if (!pkt)
    E_Exit("Error: Unable to alloc packet");
  ...
  while ((r = avcodec_receive_packet(ffmpeg_aud_ctx, pkt)) >= 0) {
    ...
    av_packet_rescale_ts(pkt, ffmpeg_aud_ctx->time_base,
                         ffmpeg_aud_stream->time_base);
  ...
  if (r != AVERROR(EAGAIN))
    LOG_MSG("WARNING: avcodec_receive_packet() audio failed to encode (err=%d)",
            r);
...
void ffmpeg_take_audio(int16_t *raw, unsigned int samples) {
  if (ffmpeg_aud_codec == NULL || ffmpeg_aud_frame == NULL ||
      ffmpeg_fmt_ctx == NULL)
    return;

  if ((unsigned long)ffmpeg_aud_write >=
      (unsigned long)ffmpeg_aud_frame->nb_samples) {
  ...
  if (ffmpeg_aud_frame->format == AV_SAMPLE_FMT_FLTP) {
    while (samples > 0) {
      unsigned int op =
          (unsigned int)ffmpeg_aud_frame->nb_samples - ffmpeg_aud_write;
      ...
      while (op > 0) {
        ...
        ((float *)ffmpeg_aud_frame->data[0])[ffmpeg_aud_write] =
            (float)raw[0] / 32768;
        ((float *)ffmpeg_aud_frame->data[1])[ffmpeg_aud_write] =
            (float)raw[1] / 32768;
        ffmpeg_aud_write++;
        samples--;
        raw += 2;
    ...
    while (samples > 0) {
      ...
      while (op > 0) {
        ...
        ((int16_t *)ffmpeg_aud_frame->data[0])[(ffmpeg_aud_write * 2) + 0] =
            raw[0];
        ((int16_t *)ffmpeg_aud_frame->data[0])[(ffmpeg_aud_write * 2) + 1] =
            raw[1];
        ffmpeg_aud_write++;
        samples--;
        raw += 2;
    ...
    LOG_MSG("WARNING: Audio encoder expects unknown format %u",
            ffmpeg_aud_frame->format);
  }
}
...
void ffmpeg_flush_video() {
  ...
  if (ffmpeg_fmt_ctx != NULL) {
    if (ffmpeg_vid_frame != NULL) {
      ...
      if (r < 0 && r != AVERROR(EAGAIN))
        LOG_MSG("WARNING: avcodec_send_frame() video flush failed to encode "
                "(err=%d)",
                r);
      ...
      while ((r = avcodec_receive_packet(ffmpeg_vid_ctx, pkt)) >= 0) {
        ...
        av_packet_rescale_ts(pkt, ffmpeg_vid_ctx->time_base,
                             ffmpeg_vid_stream->time_base);
      ...
      if (r != AVERROR(EAGAIN) && r != AVERROR_EOF)
        LOG_MSG("WARNING: avcodec_receive_packet() video flush failed to "
                "encode (err=%d)",
                r);
...
void ffmpeg_flush_audio() {
  ...
  if (ffmpeg_fmt_ctx != NULL) {
    if (ffmpeg_aud_frame != NULL) {
      ...
      if (r < 0 && r != AVERROR(EAGAIN))
        LOG_MSG("WARNING: avcodec_send_frame() audio flush failed to encode "
                "(err=%d)",
                r);
      ...
      if (r != AVERROR(EAGAIN) && r != AVERROR_EOF)
        LOG_MSG("WARNING: avcodec_receive_packet() audio flush failed to "
                "encode (err=%d)",
                r);
...
int ffmpeg_bpp_pick_rgb_format(int bpp) {
  // NTS: This requires some explanation. FFMPEG's pixfmt.h documents RGB555 as
  // (msb)5R 5G 5B(lsb).
  //      So when we typecast two bytes in video ram as uint16_t we get
  //      XRRRRRGGGGGBBBBB.
  //
  //      But, RGB24 means that, byte by byte, we get R G B R G B ... which when
  //      typecast as uint32_t on little endian hosts becomes 0xXXBBGGRR.
  //
  //      RGBA would mean byte order R G B A R G B A ... which when typecast as
  //      uint32_t little endian becomes AABBGGRR.
  //
  //      Most of the platforms DOSBox-X runs on tend to arrange 24-bit bpp as
  //      uint32_t(0xRRGGBB) little endian, and 32-bit bpp as
  //      uint32_t(0xAARRGGBB).
  //
  //      So if the above text makes any sense, the RGB/BGR switch-up between
  //      15/16bpp and 24/32bpp is NOT a coding error or a typo.

  // FIXME: Given the above explanation, this code needs to factor RGBA byte
  // order vs host byte order
  //        to correctly specify the color format on any platform.
  if (bpp == 8)
    return (GFX_GetBShift() == 0)
               ? AV_PIX_FMT_BGRA
               : AV_PIX_FMT_RGBA; // I can't get FFMPEG to do PAL8 -> YUV444P,
                                  // so we'll just convert to RGBA on the fly
  else if (bpp == 15)
    return (GFX_GetBShift() == 0)
               ? AV_PIX_FMT_RGB555
               : AV_PIX_FMT_BGR555; // <- not a typo! not a mistake!
  else if (bpp == 16)
    return (GFX_GetBShift() == 0)
               ? AV_PIX_FMT_RGB565
               : AV_PIX_FMT_BGR565; // <- not a typo! not a mistake!
  else if (bpp == 24)
...
void ffmpeg_reopen_video(double fps, const int bpp) {
  ...
  if (ffmpeg_vid_ctx == NULL)
    E_Exit("Error: Unable to reopen vid codec");
  ...
  /* 4:3 aspect ratio. FFMPEG thinks in terms of Pixel Aspect Ratio not Display
   * Aspect Ratio */
...

  {
    ...
    av_dict_set(&opts, "crf", "14",
                1); /* default is 20, this allows higher quality */
    av_dict_set(&opts, "crf_max", "20",
                1); /* don't let it get too low quality */
  ...
  if (avcodec_parameters_from_context(ffmpeg_vid_stream->codecpar,
                                      ffmpeg_vid_ctx) < 0) {
  ...
  if (ffmpeg_aud_frame == NULL || ffmpeg_vid_frame == NULL ||
      ffmpeg_vidrgb_frame == NULL)
    E_Exit(" ");
  ...
  ffmpeg_sws_ctx = sws_getContext(
      // source
      ffmpeg_vidrgb_frame->width, ffmpeg_vidrgb_frame->height,
      (AVPixelFormat)ffmpeg_vidrgb_frame->format,
      // dest
      ffmpeg_vid_frame->width, ffmpeg_vid_frame->height,
      (AVPixelFormat)ffmpeg_vid_frame->format,
      // and the rest
      ((ffmpeg_vid_frame->width == ffmpeg_vidrgb_frame->width &&
        ffmpeg_vid_frame->height == ffmpeg_vidrgb_frame->height)
           ? SWS_POINT
           : SWS_BILINEAR),
      NULL, NULL, NULL);
  if (ffmpeg_sws_ctx == NULL)
...
std::string GetCaptureFilePath(const char *type, const char *ext) {
  ...
  lowcase(file_start);
  for (char *s = (char *)file_start; *s; s++)
    *s = filtercapname(*s);
  strcat(file_start, "_");
  bool is_directory;
  ...
  for (; testRead;
       testRead = read_directory_next(dir, tempname, sname, is_directory)) {
  ...
  sprintf(file_name, "%s%c%s%03d%s", capturedir.c_str(), CROSS_FILESPLIT,
          file_start, (int)last, ext);
  return file_name;
}

FILE *OpenCaptureFile(const char *type, const char *ext) {
  if (!strcmp(type, "Raw Midi"))
    pathmid = "";
  if (!strcmp(type, "Raw Opl"))
    pathopl = "";
  if (!strcmp(type, "Screenshot"))
    pathscr = "";
  if (!strcmp(type, "Parallel Port Stream"))
    pathprt = "";
  if (capturedir.empty()) {
    LOG_MSG("Please specify a capture directory");
    return nullptr;
  ...
  if (handle) {
    ...
    if (GetFullPathName(path.c_str(), MAX_PATH, fullpath, NULL))
      path = fullpath;
...
#if (C_SSHOT)
static void CAPTURE_AddAviChunk(const char *tag, uint32_t size, void *data,
                                uint32_t flags, unsigned int streamindex) {
  (void)tag; // UNUSED
...
void CAPTURE_VideoEvent(bool pressed) {
  ...
  if (CaptureState & CAPTURE_VIDEO) {
    ...
    if (capture.video.writer != NULL) {
      if (capture.video.audioused) {
        CAPTURE_AddAviChunk("01wb", (uint32_t)(capture.video.audioused * 4),
                            capture.video.audiobuf, 0x10, 1);
      ...
      if (show_recorded_filename && pathvid.size())
        systemmessagebox(
            "Recording completed",
            ("Saved AVI output to the file:\n\n" + pathvid).c_str(), "ok",
            "info", 1);
    }
#if (C_AVCODEC)
    if (ffmpeg_fmt_ctx != NULL) {
  ...
  pathvid = "";
  mainMenu.get_item("mapper_video")
      .check(!!(CaptureState & CAPTURE_VIDEO))
    ...
    for (i = 0; i < height; i++) {
      ...
      case 8:
        if (flags & CAPTURE_FLAG_DBLW) {
          for (Bitu x = 0; x < countWidth; x++)
            doubleRow[x * 2 + 0] = doubleRow[x * 2 + 1] =
                ((uint8_t *)srcLine)[x];
      ...
      case 32:
        if (flags & CAPTURE_FLAG_DBLW) {
          for (Bitu x = 0; x < countWidth; x++) {
            doubleRow[x * 6 + 0] = doubleRow[x * 6 + 3] =
                ((uint8_t *)srcLine)[x * 4 + 0];
            doubleRow[x * 6 + 1] = doubleRow[x * 6 + 4] =
                ((uint8_t *)srcLine)[x * 4 + 1];
            doubleRow[x * 6 + 2] = doubleRow[x * 6 + 5] =
                ((uint8_t *)srcLine)[x * 4 + 2];
          }
        } else {
          for (Bitu x = 0; x < countWidth; x++) {
  ...
  if (CaptureState & CAPTURE_VIDEO) {
    ...
    if (native_zmbv && capture.video.writer == NULL) {
      ...
      capture.video.bufSize =
          capture.video.codec->NeededSize((int)width, (int)height, format);
      ...
      riff_avih_AVIMAINHEADER *mheader =
          avi_writer_main_header(capture.video.writer);
      if (mheader == NULL)
        goto skip_video;

      memset(mheader, 0, sizeof(*mheader));
      __w_le_u32(&mheader->dwMicroSecPerFrame,
                 (uint32_t)(1000000 / fps)); /* NTS: int divided by double */
      __w_le_u32(&mheader->dwMaxBytesPerSec, 0);
      __w_le_u32(&mheader->dwPaddingGranularity, 0);
      __w_le_u32(&mheader->dwFlags,
                 0x110); /* Flags,0x10 has index, 0x100 interleaved */
      __w_le_u32(&mheader->dwTotalFrames,
                 0); /* AVI writer updates this automatically on finish */
      ...
      __w_le_u32(&vsheader->dwLength,
                 0); /* AVI writer updates this automatically */
      ...
      __w_le_u16(
          &vbmp.biPlanes,
          0); /* FIXME: Only repeating what the original DOSBox code did */
      __w_le_u16(
          &vbmp.biBitCount,
          0); /* FIXME: Only repeating what the original DOSBox code did */
      __w_le_u32(&vbmp.biCompression, avi_fourcc_const('Z', 'M', 'B', 'V'));
      __w_le_u32(&vbmp.biSizeImage,
                 (uint32_t)(capture.video.width * capture.video.height * 4));
      ...
      __w_le_u32(&fmt.nAvgBytesPerSec,
                 (uint32_t)(capture.video.audiorate * 2 * 2));

      if (!avi_writer_stream_set_format(astream, &fmt, sizeof(fmt)))
        goto skip_video;

      if (!avi_writer_begin_header(capture.video.writer) ||
          !avi_writer_begin_data(capture.video.writer))
        goto skip_video;
      ...
      std::string path = GetCaptureFilePath(
          "Video", ".mts"); // Use widely recognized .MTS extension
      if (path == "")
        goto skip_video;
      pathvid = path;
      ...
      if (avformat_alloc_output_context2(&ffmpeg_fmt_ctx, NULL, "mpegts",
                                         NULL) < 0) {
      ...
      if (avio_open(&ffmpeg_fmt_ctx->pb, pathvid.c_str(), AVIO_FLAG_WRITE) <
          0) {
      ...
      ffmpeg_vid_ctx = avcodec_alloc_context3(ffmpeg_vid_codec);
      if (ffmpeg_vid_ctx == NULL)
        E_Exit("Error: Unable to open vid context");
      ffmpeg_vid_ctx->bit_rate = 25000000; // TODO: make configuration option!
      ffmpeg_vid_ctx->keyint_min = 15;     // TODO: make configuration option!
      ffmpeg_vid_ctx->time_base.num = 1000000;
      ffmpeg_vid_ctx->time_base.den = (int)(1000000 * fps);
      ...
      ffmpeg_vid_ctx->gop_size = 15; // TODO: make config option
      ffmpeg_vid_ctx->max_b_frames = 0;
      ffmpeg_vid_ctx->pix_fmt = ffmpeg_choose_pixfmt(
          ffmpeg_yuv_format_choice);    // TODO: auto-choose according to what
                                        // codec says is supported, and let user
                                        // choose as well
      ffmpeg_vid_ctx->thread_count = 0; // auto-choose
      ffmpeg_vid_ctx->flags2 = AV_CODEC_FLAG2_FAST;
      ffmpeg_vid_ctx->qmin = 1;
      ...
      /* 4:3 aspect ratio. FFMPEG thinks in terms of Pixel Aspect Ratio not
      ...
      if (ffmpeg_aud_ctx == NULL)
        E_Exit("Error: Unable to open aud context");
      ...
      if (avcodec_parameters_from_context(ffmpeg_aud_stream->codecpar,
                                          ffmpeg_aud_ctx) < 0) {
    ...
    if (native_zmbv) {
      ...
      if ((flags & CAPTURE_FLAG_NOCHANGE) && skip_encoding_unchanged_frames) {
        ...
        CAPTURE_AddAviChunk("00dc", (uint32_t)0, capture.video.buf,
                            (uint32_t)(0x0), 0u);
      } else {
        if (!capture.video.codec->PrepareCompressFrame(
                codecFlags, format, (char *)pal, capture.video.buf,
                capture.video.bufSize))
          goto skip_video;
        ...
        CAPTURE_AddAviChunk("00dc", (uint32_t)written, capture.video.buf,
                            (uint32_t)(codecFlags & 1 ? 0x10 : 0x0), 0u);
        capture.video.frames++;
      }

      if (capture.video.audioused) {
        CAPTURE_AddAviChunk("01wb", (uint32_t)(capture.video.audioused * 4u),
                            capture.video.audiobuf, /*keyframe*/ 0x10u, 1u);
        capture.video.audiowritten = capture.video.audioused * 4;
        capture.video.audioused = 0;
      }
    }
        ...
        E_Exit("Error: Unable to alloc packet");
      {
        ...
        if (bpp == 8 && ffmpeg_vidrgb_frame->format != AV_PIX_FMT_PAL8) {
          for (i = 0; i < height; i++) {
            dstline = ffmpeg_vidrgb_frame->data[0] +
                      ((unsigned int)i *
                       (unsigned int)ffmpeg_vidrgb_frame->linesize[0]);
            ...
            if (flags & CAPTURE_FLAG_DBLW) {
              for (x = 0; x < width; x++)
                ((uint32_t *)dstline)[(x * 2) + 0] =
                    ((uint32_t *)dstline)[(x * 2) + 1] =
                        GFX_palette32bpp[srcline[x]];
            } else {
              for (x = 0; x < width; x++)
        ...
        if (sws_scale(ffmpeg_sws_ctx,
                      // source
                      ffmpeg_vidrgb_frame->data, ffmpeg_vidrgb_frame->linesize,
                      0, ffmpeg_vidrgb_frame->height,
                      // dest
                      ffmpeg_vid_frame->data, ffmpeg_vid_frame->linesize) <= 0)
          LOG_MSG("WARNING: sws_scale() failed");

        // encode it
        ffmpeg_vid_frame->pts =
            (int64_t)capture.video.frames; // or else libx264 complains about
                                           // non-monotonic timestamps
        ffmpeg_vid_frame->key_frame =
            ((capture.video.frames % 15) == 0) ? 1 : 0;

        r = avcodec_send_frame(ffmpeg_vid_ctx, ffmpeg_vid_frame);
        if (r < 0 && r != AVERROR(EAGAIN))
          LOG_MSG(
              "WARNING: avcodec_send_frame() video failed to encode (err=%d)",
              r);
        ...
        if (r != AVERROR(EAGAIN))
          LOG_MSG("WARNING: avcodec_receive_packet() video failed to encode "
                  "(err=%d)",
                  r);
      }
      av_packet_free(&pkt);
      ...
      if (capture.video.audioused) {
        ffmpeg_take_audio((int16_t *)capture.video
                              .audiobuf /*NTS: Ewwwwww.... what if the compiler
                                           pads the 2-dimensional array?*/
                          ,
                          capture.video.audioused);
        capture.video.audiowritten = capture.video.audioused * 4;
        capture.video.audioused = 0;
      }
    }
...
void CAPTURE_MultiTrackAddWave(uint32_t freq, uint32_t len, int16_t *data,
                               const char *name) {
#if !defined(C_EMSCRIPTEN)
  if (CaptureState & CAPTURE_MULTITRACK_WAVE) {
    if (capture.multitrack_wave.writer == NULL) {
      ...
      riff_avih_AVIMAINHEADER *mheader =
          avi_writer_main_header(capture.multitrack_wave.writer);
      if (mheader == NULL)
        goto skip_mt_wav;
      ...
      __w_le_u32(&mheader->dwMicroSecPerFrame,
                 (uint32_t)(1000000 /
                            30)); /* NTS: int divided by double FIXME GUESS */
      __w_le_u32(&mheader->dwMaxBytesPerSec, 0);
      __w_le_u32(&mheader->dwPaddingGranularity, 0);
      __w_le_u32(&mheader->dwFlags,
                 0x110); /* Flags,0x10 has index, 0x100 interleaved */
      ...
      capture.multitrack_wave.name_to_stream_index.clear();
      {
        ...
        while (c != NULL) {
          ...
          avi_writer_stream *astream =
              avi_writer_new_stream(capture.multitrack_wave.writer);
          if (astream == NULL)
            goto skip_mt_wav;

          riff_strh_AVISTREAMHEADER *asheader =
              avi_writer_stream_header(astream);
          if (asheader == NULL)
            goto skip_mt_wav;
          ...
          __w_le_u32(&asheader->dwInitialFrames, 0);
          __w_le_u32(&asheader->dwScale, 1);
          __w_le_u32(&asheader->dwRate,
                     (uint32_t)capture.multitrack_wave.audiorate);
          __w_le_u32(&asheader->dwStart, 0);
          __w_le_u32(&asheader->dwLength,
                     0); /* AVI writer updates this automatically */
          __w_le_u32(&asheader->dwSuggestedBufferSize, 0);
          ...
          __w_le_u16(&fmt.wFormatTag, windows_WAVE_FORMAT_PCM);
          __w_le_u16(&fmt.nChannels, 2); /* stereo */
          __w_le_u32(&fmt.nSamplesPerSec,
                     (uint32_t)capture.multitrack_wave.audiorate);
          __w_le_u16(&fmt.wBitsPerSample, 16); /* 16-bit/sample */
          __w_le_u16(&fmt.nBlockAlign, 2 * 2);
          __w_le_u32(&fmt.nAvgBytesPerSec,
                     (uint32_t)(capture.multitrack_wave.audiorate * 2 * 2));

          if (!avi_writer_stream_set_format(astream, &fmt, sizeof(fmt)))
            goto skip_mt_wav;

          if (c->name != NULL && *(c->name) != 0) {
            LOG_MSG("multitrack audio, mixer channel '%s' is AVI stream %d",
                    c->name, astream->index);
            capture.multitrack_wave.name_to_stream_index[c->name] =
                (size_t)astream->index;
            astream->name = c->name;
          }
      ...
      if (!avi_writer_begin_header(capture.multitrack_wave.writer) ||
          !avi_writer_begin_data(capture.multitrack_wave.writer))
        goto skip_mt_wav;
      ...
      if (realpath(path.c_str(), fullpath) != NULL)
        path = fullpath;
#endif
      LOG_MSG("Started capturing multitrack audio (%u channels) to: %s",
              streams, path.c_str());
    }

    if (capture.multitrack_wave.writer != NULL) {
      std::map<std::string, size_t>::iterator ni =
          capture.multitrack_wave.name_to_stream_index.find(name);
      if (ni != capture.multitrack_wave.name_to_stream_index.end()) {
        ...
        if (index < (size_t)capture.multitrack_wave.writer->avi_stream_alloc) {
          avi_writer_stream *os =
              capture.multitrack_wave.writer->avi_stream + index;
          avi_writer_stream_write(capture.multitrack_wave.writer, os, data,
                                  len * 2 * 2, /*keyframe*/ 0x10);
        } else {
  ...
  return;
skip_mt_wav:
  capture.multitrack_wave.writer =
      avi_writer_destroy(capture.multitrack_wave.writer);
#endif
}
...
void CAPTURE_AddWave(uint32_t freq, uint32_t len, int16_t *data) {
  ...
  if (CaptureState & CAPTURE_WAVE) {
    ...
    while (len > 0) {
      ...
      if (!left) {
        riff_wav_writer_data_write(capture.wave.writer, capture.wave.buf,
                                   2 * 2 * WAVE_BUF);
...
void CAPTURE_MTWaveEvent(bool pressed) {
  ...
  if (CaptureState & CAPTURE_MULTITRACK_WAVE) {
    if (capture.multitrack_wave.writer != NULL) {
      ...
      if (show_recorded_filename && pathmtw.size())
        systemmessagebox(
            "Recording completed",
            ("Saved multi-track AVI output to the file:\n\n" + pathmtw).c_str(),
            "ok", "info", 1);
    }
  } else {
  ...
  mainMenu.get_item("mapper_recmtwave")
      .check(!!(CaptureState & CAPTURE_MULTITRACK_WAVE))
      .refresh_item(mainMenu);
#endif
}
...
void CAPTURE_NetworkEvent(bool pressed) {
  ...
  if (CaptureState & CAPTURE_NETWORK) {
    pcap_writer_close();
    CaptureState &= ~((unsigned int)CAPTURE_NETWORK);
    if (show_recorded_filename && pathpcap.size())
      systemmessagebox(
          "Recording completed",
          ("Saved PCAP output to the file:\n\n" + pathpcap).c_str(), "ok",
          "info", 1);
  } else {
    CaptureState |= CAPTURE_NETWORK;
  }

  mainMenu.get_item("mapper_capnetrf")
      .check(!!(CaptureState & CAPTURE_NETWORK))
      .refresh_item(mainMenu);
#endif
}
...
void CAPTURE_WaveEvent(bool pressed) {
  ...
  mainMenu.get_item("mapper_recwave")
      .check(!!(CaptureState & CAPTURE_WAVE))
      .refresh_item(mainMenu);
#endif
}
...
static void RawMidiAddNumber(uint32_t val) {
  if (val & 0xfe00000)
    RawMidiAdd((uint8_t)(0x80 | ((val >> 21) & 0x7f)));
  if (val & 0xfffc000)
    RawMidiAdd((uint8_t)(0x80 | ((val >> 14) & 0x7f)));
  if (val & 0xfffff80)
    RawMidiAdd((uint8_t)(0x80 | ((val >> 7) & 0x7f)));
...
void CAPTURE_MidiEvent(bool pressed) {
  ...
  if (capture.midi.handle) {
    ...
    if (show_recorded_filename && pathmid.size())
      systemmessagebox("Recording completed",
                       ("Saved MIDI output to the file:\n\n" + pathmid).c_str(),
                       "ok", "info", 1);
  ...
  mainMenu.get_item("mapper_caprawmidi")
      .check(!!(CaptureState & CAPTURE_MIDI))
      .refresh_item(mainMenu);
}

void CAPTURE_Destroy(Section *sec) {
  (void)sec; // UNUSED
             //  if capture is active, fake mapper event to "toggle" it off for
             //  each capture case.
#if (C_SSHOT)
  if (capture.video.writer != NULL)
    CAPTURE_VideoEvent(true);
#endif
  if (capture.multitrack_wave.writer)
    CAPTURE_MTWaveEvent(true);
  if (capture.wave.writer)
    CAPTURE_WaveEvent(true);
  if (capture.midi.handle)
    CAPTURE_MidiEvent(true);
}

bool enable_autosave = false;
int autosave_second = 0, autosave_count = 0, autosave_start[10],
    ...
    if (found == std::string::npos || found == 0) {
      ...
      if (found == std::string::npos || found == 0) {
        ...
        if (start > 0)
          ...
          autosave_start[autosave_count] = -1;
      } else {
        ...
        if (start > 0) {
          autosave_start[autosave_count] = start;
          if (end >= start)
            autosave_end[autosave_count] = end;
        } else if (start < 0)
          autosave_start[autosave_count] = -1;
      }
    }
...
void CAPTURE_Init() {
  ...
  if (savefilename.size()) {
    ...
    mainMenu.get_item("usesavefile")
        .set_text("Use save file (" + savefilename + ")")
        .check(use_save_file);
  ...
  if (hostkey == "ctrlalt")
    hostkeyalt = 1;
  else if (hostkey == "ctrlshift")
    hostkeyalt = 2;
  else if (hostkey == "altshift")
    hostkeyalt = 3;
  else
    hostkeyalt = 0;
  ...
  skip_encoding_unchanged_frames =
      section->Get_bool("skip encoding unchanged frames");
  ...
  if (capfmt == "mpegts-h264") {
    ...
    export_ffmpeg = true;
#else
    LOG_MSG("MPEGTS H.264+AAC not compiled in to this DOSBox-X binary. Using "
            "AVI+ZMBV");
    native_zmbv = true;
    export_ffmpeg = false;
#endif
  ...
  MAPPER_AddHandler(CAPTURE_WaveEvent, MK_w, MMODHOST, "recwave",
                    "Record audio to WAV", &item);
  item->set_text("Record audio to WAV");

  MAPPER_AddHandler(CAPTURE_MTWaveEvent, MK_nothing, 0, "recmtwave",
                    "Record to multi-track AVI", &item);
  item->set_text("Record audio to multi-track AVI");

  MAPPER_AddHandler(CAPTURE_MidiEvent, MK_nothing, 0, "caprawmidi",
                    "Record MIDI output", &item);
  item->set_text("Record MIDI output");

  MAPPER_AddHandler(OPL_SaveRawEvent, MK_nothing, 0, "caprawopl",
                    "Record FM/OPL output", &item);
  item->set_text("Record FM (OPL) output");

  MAPPER_AddHandler(CAPTURE_NetworkEvent, MK_nothing, 0, "capnetrf",
                    "Record Network traffic", &item);
  item->set_text("Record network traffic");

#if (C_SSHOT)
  MAPPER_AddHandler(CAPTURE_VideoEvent, MK_i, MMODHOST, "video",
                    "Record video to AVI", &item);
  item->set_text("Record video to AVI");

  MAPPER_AddHandler(CAPTURE_ScreenShotEvent, MK_p, MMODHOST, "scrshot",
                    "Take screenshot", &item);
  item->set_text("Take screenshot");

  MAPPER_AddHandler(CAPTURE_RawScreenShotEvent, MK_p, MMODHOST | MMOD1,
                    "rawscrshot", "Take raw screenshot", &item);
...
#if (C_SSHOT)
  mainMenu.get_item("capture_fmt_avi_zmbv")
      .check(native_zmbv)
      .refresh_item(mainMenu);
#if (C_AVCODEC)
  mainMenu.get_item("capture_fmt_mpegts_h264")
      .check(export_ffmpeg)
      .refresh_item(mainMenu);
#endif
#endif
