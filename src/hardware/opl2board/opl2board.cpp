/*
OPL2 Board Driver
note:(josephillips85) To perform changes please perform for the specific target
platform Ex. MacOS , Windows , Linux Don't change the behavior of the target
platform if you don't have the way to test it.
*/
#include "opl2board.h"
...
void OPL2AudioBoard::resetBuffer() { sendBuffer = std::queue<uint8_t>(); }
void OPL2AudioBoard::writeBuffer() {
#if !defined(HX_DOS) &&                                                        \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))

#if !defined(MACOS)
  /* Note:(josephillips85) This is a workaround to fix the thread stop issue
   presented on MacOS Probably hitting this BUG
   https://github.com/apple/darwin-libpthread/blob/main/src/pthread.c#L2177
