...
This is DOSBox-X's handler of 93c46 copy-protection dongle connected to LPT
port. At least Rainbow Sentinel Cplus and MicroPhar are 93c46-based dongles.
...
Add this file to the DOSBox-X project, patch dosbox.cpp patch and add to
dosbox-x.conf "dongle=true" under "[speaker]" section.
...
static void dongle_write(Bitu port, Bitu val, Bitu iolen) {
  ...
  */
  ...
  if (last_SK == 0 && SK == 1) // posedge
  {
    ...
    if (queue_filling) {
      queue[queue_idx] = DI;
      if (queue_idx == 8) { // last bit filled

        int OP1 = queue[1];
        int OP2 = queue[2];

        ADR = (queue[3] << 5) | (queue[4] << 4) | (queue[5] << 3) |
              (queue[6] << 2) | (queue[7] << 1) | (queue[8]);

        if (OP1 == 1 && OP2 == 0) // read
        {
          LOG(LOG_MISC,
              LOG_NORMAL)("93c46 dongle: trying to read at address 0x%x", ADR);
          ...
          LOG(LOG_MISC, LOG_NORMAL)(
              "93c46 dongle: OP1=%d; OP2=%d: this command is not handled yet",
              OP1, OP2);
      } else
        queue_idx++;
    }
...
void DONGLE_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(DONGLE_OnReset));
}
