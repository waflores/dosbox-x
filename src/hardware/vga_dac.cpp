...
static void VGA_DAC_SendColor(Bitu index, Bitu src) {
  ...
  const uint8_t green =
      dacexpand(vga.dac.rgb[src].green & dacmask, dacshl, dacshr);
  const uint8_t blue =
      dacexpand(vga.dac.rgb[src].blue & dacmask, dacshl, dacshr);

  /* FIXME: CGA composite mode calls RENDER_SetPal itself, which conflicts with
   * this code */
  if (vga.mode == M_CGA16)
    return;

  /* FIXME: Can someone behind the GCC project explain how (unsigned int) OR
   * (unsigned int) somehow becomes (signed int)?? */

  if (GFX_bpp >= 24) /* FIXME: Assumes 8:8:8. What happens when desktops start
                        using the 10:10:10 format? */
    vga.dac.xlat32[index] = (uint32_t)((blue & 0xffu) << (GFX_Bshift)) |
                            (uint32_t)((green & 0xffu) << (GFX_Gshift)) |
                            (uint32_t)((red & 0xffu) << (GFX_Rshift)) |
                            (uint32_t)GFX_Amask;
  else {
    /* FIXME: Assumes 5:6:5. I need to test against 5:5:5 format sometime.
     * Perhaps I could dig out some older VGA cards and XFree86 drivers that
     * support that format? */
    vga.dac.xlat16[index] = (uint16_t)(((blue & 0xffu) >> 3u) << GFX_Bshift) |
    ...
    if (GFX_Bshift == 0)
      vga.dac.xlat32[index] = (uint32_t)(blue << 0U) | (uint32_t)(green << 8U) |
                              (uint32_t)(red << 16U);
    else
      vga.dac.xlat32[index] = (uint32_t)(blue << 16U) |
                              (uint32_t)(green << 8U) | (uint32_t)(red << 0U);
  }
  ...
  if (IS_VGA_ARCH) {
    if (vga.mode == M_VGA || vga.mode == M_LIN8) {
      ...
      default: // <- just in case
        ...
        break;
      case AC_low4:
        /* Tseng ET4000 behavior, according to the SVGA card I have where only
         * the low 4 bits are translated. --J.C. */
        maskIndex = vga.dac.combine[index & 0xF] & 0x0F;
    ...
    /* the DAC mapping for MDA/Herc must be maintained or colors will not come
     * out right */
    if ((index & 7) != 0)
...
void VGA_DAC_DeferredUpdateColorPalette() {
  if (VGA_DAC_DeferredUpdate > 0) {
    ...
    VGA_DAC_UpdateColorPalette(); // FIXME: Yes, this is very inefficient. Will
                                  // improve later.
  }
}
...
void write_p3c7(Bitu port, Bitu val, Bitu iolen) {
  ...
  vga.dac.read_index = (uint8_t)val; /* NTS: Paradise SVGA behavior, read index
                                        = x, write index = x + 1 */
...
void write_p3c8(Bitu port, Bitu val, Bitu iolen) {
  ...
  vga.dac.write_index =
      (uint8_t)val; /* NTS: Paradise SVGA behavior, this affects write index,
                       but not read index */
...
void write_p3c9(Bitu port, Bitu val, Bitu iolen) {
  ...
  // allow the full 8 bit ONLY if 8-bit DAC emulation is enabled AND 8-bit DAC
  // mode is on. masking to 6 bits is REQUIRED for some games like "Amulets and
  // Armor", where apparently the use of signed char with palette values can
  // cause "overbright" effects if it can read back the full 8 bits.
  if (!vga_8bit_dac)
    val &= 0x3f;
  ...
  if (update) {
    ...
    // the DAC.
    //
    // Perhaps IBM couldn't get the DAC to run fast enough for 640x480 2-color
    // mode.
...
Bitu read_p3c9(Bitu port, Bitu iolen) {
  ...
  default:
    LOG(LOG_VGAMISC,
        LOG_NORMAL)("VGA:DAC:Illegal Pel Index"); // If this can actually happen
