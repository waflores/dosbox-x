...
void retrowave_deinit(RetroWaveContext *ctx) { free(ctx->cmd_buffer); }
...
void retrowave_io_init(RetroWaveContext *ctx) {
  ...
  for (uint8_t i = 0x20; i < 0x28; i++) {
    ...
    ctx->callback_io(ctx->user_data, (uint32_t)1e6, init_sequence_1, NULL,
                     sizeof(init_sequence_1));
    ctx->callback_io(ctx->user_data, (uint32_t)1e6, init_sequence_2, NULL,
                     sizeof(init_sequence_2));
    ctx->callback_io(ctx->user_data, (uint32_t)1e6, init_sequence_3, NULL,
                     sizeof(init_sequence_3));
  }
}

void retrowave_cmd_buffer_init(RetroWaveContext *ctx,
...
void retrowave_flush(RetroWaveContext *ctx) {
  if (ctx->cmd_buffer_used) {
    ...
    ctx->callback_io(ctx->user_data, ctx->transfer_speed_hint, ctx->cmd_buffer,
                     NULL, ctx->cmd_buffer_used);
