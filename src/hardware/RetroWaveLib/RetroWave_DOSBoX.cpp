...
static void retrowave_iocb_empty(void *userp, uint32_t data_rate,
...
void retrowave_init_dosbox(const std::string &bus, const std::string &path,
  ...
  if (bus == "serial") {
...
#ifdef _WIN32
    rc = retrowave_init_win32_serialport(&retrowave_global_context,
                                         path.c_str());
#endif
  } else if (bus == "spi") {
    ...
    if (scgs.size() != 2) {
      puts("error: bad GPIO specification. Please use the `gpiochip,line' "
           "format.");
      exit(2);
    }
    ...
    rc = retrowave_init_linux_spi(&retrowave_global_context, path.c_str(),
                                  scg[0], scg[1]);
#else
