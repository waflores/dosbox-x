...
int set_tty(int fd) {
  ...
  if (ioctl(fd, IOSSIOSPEED, &speed) == -1) {
    fprintf(stderr,
            "%s: FATAL: failed to set MacOS specific tty attributes: %s",
...
static inline void set_device_lock(RetroWavePlatform_POSIXSerialPort *ctx,
                                   int value) {
  errno = 0;
  ...
  if (rc_lock < 0) {
    fprintf(stderr,
            "%s: FATAL: failed to change lock state on tty device node: %s\n",
            log_tag, strerror(errno));
    abort();
  }
}

static void io_callback(void *userp, uint32_t data_rate, const void *tx_buf,
                        void *rx_buf, uint32_t len) {
  ...
  assert(retrowave_protocol_serial_pack(tx_buf, len, packed_data) ==
         packed_len);
  ...
  while (written < len) {
    ssize_t rc =
        write(ctx->fd_tty, packed_data + written, packed_len - written);
    if (rc > 0) {
      written += rc;
    } else {
...
int retrowave_init_posix_serialport(RetroWaveContext *ctx,
                                    const char *tty_path) {
  retrowave_init(ctx);
