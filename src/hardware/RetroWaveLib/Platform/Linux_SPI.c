...
set_cs(RetroWavePlatform_LinuxSPI *ctx, int value) {
...
static inline void set_device_lock(RetroWavePlatform_LinuxSPI *ctx, int value) {
  ...
  if (rc_lock < 0) {
    fprintf(stderr,
            "%s: FATAL: failed to change lock state on SPI device node: %s\n",
...
int retrowave_init_linux_spi(RetroWaveContext *ctx, const char *spi_dev,
