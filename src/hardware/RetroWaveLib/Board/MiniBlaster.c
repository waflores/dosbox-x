...
void retrowave_miniblaster_queue(RetroWaveContext *ctx, uint8_t reg,
                                 uint8_t val) {
  ...
  if (reg < 0x80) {
    ...
    ctx->cmd_buffer[ctx->cmd_buffer_used - 12] =
        0xfd; // A0 = 1, CS# = 0, WR# = 1
    ctx->cmd_buffer[ctx->cmd_buffer_used - 11] = reg;
    ctx->cmd_buffer[ctx->cmd_buffer_used - 10] =
        0xf9; // A0 = 1, CS# = 0, WR# = 0
    ...
    ctx->cmd_buffer[ctx->cmd_buffer_used - 6] =
        0xfc; // A0 = 0, CS# = 0, WR# = 1
    ctx->cmd_buffer[ctx->cmd_buffer_used - 5] = val;
    ctx->cmd_buffer[ctx->cmd_buffer_used - 4] =
        0xf8; // A0 = 0, CS# = 0, WR# = 0
