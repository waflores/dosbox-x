...
static Bitu read_p201(Bitu port, Bitu iolen) {
  ...
  **                        +-------------------------------+
  **                          |   |   |   |   |   |   |   |
  **  Joystick B, Button 2 ---+   |   |   |   |   |   |   +--- Joystick A, X
  *Axis
  **  Joystick B, Button 1 -------+   |   |   |   |   +------- Joystick A, Y
  *Axis
  **  Joystick A, Button 2 -----------+   |   |   +----------- Joystick B, X
  *Axis
  **  Joystick A, Button 1 ---------------+   +--------------- Joystick B, Y
  *Axis
  **/
  uint8_t ret = 0xff;
  if (stick[0].enabled) {
...
static void write_p201(Bitu port, Bitu val, Bitu iolen) {
  ...
  if (stick[1].enabled) {
    stick[1].xcount =
        (Bitu)(((swap34 ? stick[1].ypos : stick[1].xpos) * RANGE) + RANGE);
    stick[1].ycount =
        (Bitu)(((swap34 ? stick[1].xpos : stick[1].ypos) * RANGE) + RANGE);
  }
}
static void write_p201_timed(Bitu port, Bitu val, Bitu iolen) {
  ...
  // ) to reset to 0 Precalculate the time at which each axis hits 0 here
  double currentTick = PIC_FullIndex();
  if (stick[0].enabled) {
    stick[0].xtick =
        currentTick +
        1000.0 * (JOY_S_CONSTANT +
                  S_PER_OHM * (double)((stick[0].xpos + 1.0) * OHMS));
    stick[0].ytick =
        currentTick +
        1000.0 * (JOY_S_CONSTANT +
                  S_PER_OHM * (double)((stick[0].ypos + 1.0) * OHMS));
  }
  if (stick[1].enabled) {
    stick[1].xtick =
        currentTick +
        1000.0 * (JOY_S_CONSTANT +
                  S_PER_OHM *
                      (double)((swap34 ? stick[1].ypos : stick[1].xpos) + 1.0) *
                      OHMS);
    stick[1].ytick =
        currentTick +
        1000.0 * (JOY_S_CONSTANT +
                  S_PER_OHM *
                      (double)((swap34 ? stick[1].xpos : stick[1].ypos) + 1.0) *
                      OHMS);
  }
}

void JOYSTICK_Enable(Bitu which, bool enabled) {
  LOG(LOG_MISC, LOG_DEBUG)("JOYSTICK: Stick %u enable=%u", (int)which,
                           enabled ? 1 : 0);
  if (which < 2)
...
void JOYSTICK_Init() {
  ...
  /* NTS: Joystick emulation does not work if we init joystick type AFTER mapper
   * init. We cannot wait for poweron/reset signal for determination of joystick
   * type. But, I/O port setup can happen later. */
  {
    Section_prop *section =
        static_cast<Section_prop *>(control->GetSection("joystick"));
    ...
    stick[0].xtick = stick[0].ytick = stick[1].xtick = stick[1].ytick =
        PIC_FullIndex();
    ...
    for (auto i = 0; i < joysticks; i++) {
      for (auto j = 0; j < axes; j++) {
        auto propname =
            "joy" + std::to_string(i + 1) + "axis" + std::to_string(j);
  ...
  if (!IS_PC98_ARCH)
    AddVMEventFunction(VM_EVENT_POWERON,
                       AddVMEventFunctionFuncPair(JOYSTICK_OnPowerOn));
}
