...
static void INNOVA_CallBack(Bitu len) {
  ...
  while (delta_t && bufindex != len) {
    bufindex += (Bitu)innova.sid->clock(delta_t, buffer + bufindex,
                                        (int)(len - bufindex));
  }
...
public:
  INNOVA(Section *configuration) : Module_base(configuration) {
    ...
    if (!section->Get_bool("innova") || control->opt_silent)
      return;
    ...
    innova.sid->set_sampling_parameters(SID_FREQ, method, (double)innova.rate,
                                        -1, 0.97);
...
void INNOVA_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(INNOVA_OnReset));
}
