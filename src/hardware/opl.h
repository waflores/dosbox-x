...
/*
        define Bits, Bitu, int32_t, uint32_t, int16_t, uint16_t, int8_t, uint8_t
   here
*/
/*
#include <stdint.h>
...
/* operator struct definition
     For OPL2 all 9 channels consist of two operators each, carrier and
   modulator. Channel x has operators x as modulator and operators (9+x) as
   carrier. For OPL3 all 18 channels consist either of two operators (2op mode)
   or four operators (4op mode) which is determined through register4 of the
   second adlib register set. Only the channels 0,1,2 (first set) and 9,10,11
   (second set) can act as 4op channels. The two additional operators for a
   channel y come from the 2op channel y+3 so the operators y, (9+y), y+3,
   (9+y)+3 make up a 4op channel.
*/
typedef struct operator_struct {
  int32_t cval, lastcval; // current output/last output (used for feedback)
  uint32_t tcount, wfpos,
      tinc;                    // time (position in waveform) and time increment
  ...
  uint32_t
      op_state; // current state of operator (attack/decay/sustain/release/off)
  uint32_t toff;
  int32_t freq_high;  // highest three bits of the frequency, used for vibrato
                      // calculations
  ...
  uint32_t generator_pos; // for non-standard sample rates we need to determine
                          // how many samples have passed
  Bits cur_env_step;      // current (standardized) sample position
  Bits env_step_a, env_step_d, env_step_r; // number of std samples of one step
                                           // (for attack/decay/release mode)
  uint8_t step_skip_pos_a; // position of 8-cyclic step skipping (always 2^x to
                           // check against mask)
  Bits env_step_skip_a;    // bitmask that determines if a step is skipped
                           // (respective bit is zero then)
