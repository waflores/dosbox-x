...
static const uint16_t oplports[] = {0x0, 0x1,   0x2,   0x3,   0x8,
                                    0x9, 0x388, 0x389, 0x38A, 0x38B};
...
static Bitu read_hwcmsio(Bitu port, Bitu /*iolen*/) {
  ...
  if (logfp)
    fprintf(logfp, "%4.3f r\t\t%3x %2x\r\n", (double)PIC_FullIndex(),
            (unsigned)port, (unsigned)retval);
  return retval;
}
...
void HARDOPL_Init(Bitu hardwareaddr, Bitu blasteraddr, bool isCMSp) {
  ...
  regainPrivileges();
  if (geteuid() != (uid_t)0) {
    LOG_MSG("OPL pass-through: Raw I/O requires root privileges. Pass-through "
            "I/O disabled.");
    return;
  }
#endif

  if (!(hardwareaddr == 0x210 || hardwareaddr == 0x220 ||
        hardwareaddr == 0x230 || hardwareaddr == 0x240 ||
        hardwareaddr == 0x250 || hardwareaddr == 0x260 ||
        hardwareaddr == 0x280)) {
    LOG_MSG("OPL pass-through: Invalid hardware base address (0x%hx). "
            "Pass-through I/O disabled.",
            (unsigned short)hardwareaddr);
    return;
  }

  if (!initPassthroughIO()) {
    LOG_MSG("OPL pass-through: Pass-through I/O disabled."); // initPassthroughIO()
                                                             // itself prints
                                                             // why it fails
    return;
  }
  ...
  if (isCMS) {
    ...
    LOG_MSG("%x-%x -> %x-%x", (unsigned)hardwareaddr,
            (unsigned)hardwareaddr + 15, (unsigned)blasteraddr,
            (unsigned)blasteraddr + 15);
  } else {
    for (int i = 0; i < 10; i++) {
