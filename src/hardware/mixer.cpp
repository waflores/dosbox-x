...
#include <string.h>
#include <sys/types.h>
#define _USE_MATH_DEFINES // needed for M_PI in Visual Studio as documented
...
#ifdef C_SDL2
SDL_AudioDeviceID SDL2_AudioDevice =
    0; /* valid IDs are 2 or higher, 1 for compat, 0 is never a valid ID */
#endif
...
uint32_t Mixer_MIXQ(void) {
  return ((uint32_t)mixer.freq) | ((uint32_t)2u /*channels*/ << (uint32_t)20u) |
...
inline void MixerChannel::updateSlew(void) {
  ...
  if (freq_nslew < freq_n)
    freq_nslew = freq_n;

  if (freq_nslew_want > 0 && freq_nslew_want < freq_n)
    max_change =
        ((uint64_t)freq_nslew_want * (uint64_t)0x8000) / (uint64_t)freq_n;
  else
...
MixerChannel *MIXER_AddChannel(MIXER_Handler handler, Bitu freq,
                               const char *name) {
...
MixerChannel *MIXER_FirstChannel(void) { return mixer.channels; }
...
void MixerChannel::SetScale(float f) { SetScale(f, f); }
...
void MixerChannel::lowpassUpdate() {
  if (lowpass_freq != 0) {
    ...
    if (freq_n > freq_d) { // source -> dest mixer rate ratio is > 100%
      timeInterval = (double)freq_d_orig /
                     freq_n; // will filter on sample load at source rate
      lowpass_on_load = true;
      lowpass_on_out = false;
    } else {
      timeInterval =
          (double)1.0 / mixer.freq; // will filter mixer output at mixer rate
    ...
    //      LOG_MSG("Lowpass freq_n=%u freq_d=%u timeInterval=%.12f tau=%.12f
    //      alpha=%.6f onload=%u onout=%u",
...
inline int32_t MixerChannel::lowpassStep(int32_t in,
                                         const unsigned int iteration,
                                         const unsigned int channel) {
  const int64_t m1 = (int64_t)in * (int64_t)lowpass_alpha;
  const int64_t m2 =
      ((int64_t)lowpass[iteration][channel] << ((int64_t)16)) -
      ((int64_t)lowpass[iteration][channel] * (int64_t)lowpass_alpha);
...
void MixerChannel::SetLowpassFreq(Bitu _freq, unsigned int order) {
  if (order > LOWPASS_ORDER)
    order = LOWPASS_ORDER;
  if (_freq == lowpass_freq && lowpass_order == order)
    return;
  lowpass_order = order;
  lowpass_freq = _freq;
  lowpassUpdate();
...
void MixerChannel::EndFrame(Bitu samples) {
  if (CaptureState &
      CAPTURE_MULTITRACK_WAVE) { // TODO: should be a separate call!
    ...
    if (cnv > 0) {
      ...
      for (Bitu i = 0; i < cnv; i++) {
        convert[i][0] =
            MIXER_CLIP(((int64_t)msbuffer[i][0] * (int64_t)volscale1) >>
                       (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
        convert[i][1] =
            MIXER_CLIP(((int64_t)msbuffer[i][1] * (int64_t)volscale2) >>
                       (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
      }
  ...
  if (msbuffer_o <= samples) {
    ...
    msbuffer_o -= samples;
    if (msbuffer_i >= samples)
      msbuffer_i -= samples;
    else
      msbuffer_i = 0;
    memmove(&msbuffer[0][0], &msbuffer[samples][0],
            msbuffer_o * sizeof(int32_t) * 2 /*stereo*/);
  }
...
void MixerChannel::Mix(Bitu whole, Bitu frac) {
  ...
  // HACK: We iterate twice only because of the Sound Blaster emulation. No
  // other emulation seems to need this.
  ...
  if (lowpass_on_out) { /* before rendering out to mixer, process samples with
                           lowpass filter */
...
inline void MixerChannel::loadCurrentSample(Bitu &len, const Type *&data) {
  ...
  if (freq_nslew_want != 0) {
    if (delta[0] < -max_change)
      delta[0] = -max_change;
    else if (delta[0] > max_change)
      delta[0] = max_change;

    if (stereo) {
      if (delta[1] < -max_change)
        delta[1] = -max_change;
...
inline void MixerChannel::padFillSampleInterpolation(const Bitu upto) {
  ...
  if (msbuffer_o < upto) {
    if (freq_f > freq_d)
      freq_f = freq_d; // this is an abrupt stop, so interpolation must not
                       // carry over, to help avoid popping artifacts
...
inline bool MixerChannel::runSampleInterpolation(const Bitu upto) {
  ...
  while (freq_fslew < freq_d) {
    int sample = last[0] + (int)(((int64_t)delta[0] * (int64_t)freq_fslew) /
                                 (int64_t)freq_d);
    msbuffer[msbuffer_o][0] = sample * volmul[0];
    sample = last[1] +
             (int)(((int64_t)delta[1] * (int64_t)freq_fslew) / (int64_t)freq_d);
  ...
  if (lowpass_on_load) {
    for (;;) {
      if (freq_f >= freq_d) {
        if (len == 0)
          break;
        loadCurrentSample<Type, stereo, signeddata, nativeorder, true>(len,
                                                                       data);
        freq_f -= freq_d;
        freq_fslew = freq_f;
      }
    ...
    for (;;) {
      if (freq_f >= freq_d) {
        if (len == 0)
          break;
        loadCurrentSample<Type, stereo, signeddata, nativeorder, false>(len,
                                                                        data);
        freq_f -= freq_d;
        freq_fslew = freq_f;
      }
...
static void MIXER_MixData(Bitu fracs /*render up to*/) {
  ...
  if (whole <= mixer.samples_rendered_ms.w)
    return;

  while (chan) {
    chan->Mix(whole, fracs);
    if (endframe)
      chan->EndFrame(mixer.samples_this_ms.w);
    chan = chan->next;
  }
  ...
  if (CaptureState & (CAPTURE_WAVE | CAPTURE_VIDEO)) {
    ...
    for (Bitu i = 0; i < added; i++) {
      convert[i][0] =
          MIXER_CLIP(((int64_t)mixer.work[readpos][0] * (int64_t)volscale1) >>
                     (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
      convert[i][1] =
          MIXER_CLIP(((int64_t)mixer.work[readpos][1] * (int64_t)volscale2) >>
                     (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
      readpos++;
    }
...
static void MIXER_FillUp(void) {
  ...
  if (index < 0)
    index = 0;
  MIXER_MixData((Bitu)((double)index * ((Bitu)mixer.samples_this_ms.w *
                                        mixer.samples_this_ms.fd)));
#ifdef C_SDL2
...
void MixerChannel::FillUp(void) { MIXER_FillUp(); }
...
static void MIXER_Mix(void) {
  ...
  thr = mixer.blocksize;
  if (thr < mixer.samples_this_ms.w)
    thr = mixer.samples_this_ms.w;
  ...
  memset(&mixer.work[mixer.work_in][0], 0,
         sizeof(int32_t) * 2 * mixer.samples_this_ms.w);
...
static void SDLCALL MIXER_CallBack(void *userdata, Uint8 *stream, int len) {
  ...
  if (mixer.mute) {
    if ((CaptureState &
         (CAPTURE_WAVE | CAPTURE_VIDEO | CAPTURE_MULTITRACK_WAVE)) != 0)
  ...
  if (!mixer.prebuffer_wait && !mixer.mute) {
    ...
    while (need > 0) {
      if (mixer.work_out == mixer.work_in)
        break;
      *output++ = MIXER_CLIP((((int64_t)(*in++)) * (int64_t)volscale1) >>
                             (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
      *output++ = MIXER_CLIP((((int64_t)(*in++)) * (int64_t)volscale2) >>
                             (MIXER_VOLSHIFT + MIXER_VOLSHIFT));
      mixer.work_out++;
  ...
  if ((unsigned long)remains >= (mixer.blocksize * 2UL)) {
    ...
    if ((unsigned long)remains >= (mixer.blocksize * 3UL)) // hard drop
      ...
      drop = (((unsigned int)remains - (unsigned int)(mixer.blocksize * 2)) /
              50U) +
             1;
...
std::string mixerinfo() {
  ...
  sprintf(str, "%-8s %3.0f:%-3.0f  %+3.2f:%-+3.2f\n", "MASTER",
          ...
          20 * log(mixer.mastervol[0]) / log(10.0f),
          20 * log(mixer.mastervol[1]) / log(10.0f));
  info += std::string(str);
  sprintf(str, "%-8s %3.0f:%-3.0f  %+3.2f:%-+3.2f\n", "RECORD",
          (double)mixer.recordvol[0] * 100, (double)mixer.recordvol[1] * 100,
          20 * log(mixer.recordvol[0]) / log(10.0f),
          20 * log(mixer.recordvol[1]) / log(10.0f));
  info += std::string(str);
  MixerChannel *chan = mixer.channels;
  for (chan = mixer.channels; chan; chan = chan->next) {
    sprintf(str, "%-8s %3.0f:%-3.0f  %+3.2f:%-+3.2f\n", chan->name,
            (double)chan->volmain[0] * 100, (double)chan->volmain[1] * 100,
            20 * log(chan->volmain[0]) / log(10.0f),
            20 * log(chan->volmain[1]) / log(10.0f));
    info += std::string(str);
  }
  return info;
}
  ...
  void Run(void) override {
    if (cmd->FindExist("-?", false) || cmd->FindExist("/?", false)) {
      WriteOut("Displays or changes the current sound mixer volumes.\n\n"
               ...
               "  /NOSHOW   Does not show volumes when making changes to "
               "channel volumes.\n"
               "  /LISTMIDI Lists and shows options for the current MIDI "
               "device handler.\n"
               "            You can also add a handler name to show the "
               "specified handler.\n"
               "  channel   A sound channel name (such as MASTER, RECORD, and "
               "SPKR).\n"
               "  volume    An integer between 0 and 100 representing the "
               "sound volume.\n");
      return;
    }
    ...
    if (cmd->FindString("MASTER", temp_line, false)) {
      MakeVolume((char *)temp_line.c_str(), mixer.mastervol[0],
                 mixer.mastervol[1]);
    }
    if (cmd->FindString("RECORD", temp_line, false)) {
      MakeVolume((char *)temp_line.c_str(), mixer.recordvol[0],
                 mixer.recordvol[1]);
    }
    MixerChannel *chan = mixer.channels;
    while (chan) {
      if (cmd->FindString(chan->name, temp_line, false)) {
        MakeVolume((char *)temp_line.c_str(), chan->volmain[0],
                   chan->volmain[1]);
      }
      chan->UpdateVolume();
      chan = chan->next;
...
void MIXER_ProgramStart(Program **make) { *make = new MIXER; }

MixerChannel *MixerObject::Install(MIXER_Handler handler, Bitu freq,
                                   const char *name) {
  if (!installed) {
    if (strlen(name) > 31)
      E_Exit("Too long mixer channel name");
...
bool MENU_get_mute(void) { return mixer.mute; }

void MENU_swapstereo(bool enabled) {
  mixer.swapstereo = enabled;
  mainMenu.get_item("mixer_swapstereo")
      .check(mixer.swapstereo)
      .refresh_item(mainMenu);
}

bool MENU_get_swapstereo(void) { return mixer.swapstereo; }
...
void MAPPER_VolumeUp(bool pressed) {
  ...
  double newvol =
      (((double)mixer.mastervol[0] + mixer.mastervol[1]) / 0.7) * 0.5;
...
void MAPPER_RecVolumeUp(bool pressed) {
  ...
  double newvol =
      (((double)mixer.recordvol[0] + mixer.recordvol[1]) / 0.7) * 0.5;
...
void MIXER_Controls_Init() {
  ...
  MAPPER_AddHandler(MAPPER_VolumeUp, MK_kpplus, MMODHOST, "volup",
                    "Increase volume", &item);
  item->set_text("Increase volume");

  MAPPER_AddHandler(MAPPER_VolumeDown, MK_kpminus, MMODHOST, "voldown",
                    "Decrease volume", &item);
  item->set_text("Decrease volume");

  MAPPER_AddHandler(MAPPER_RecVolumeUp, MK_nothing, 0, "recvolup",
                    "Increase recording volume", &item);
  item->set_text("Increase recording volume");

  MAPPER_AddHandler(MAPPER_RecVolumeDown, MK_nothing, 0, "recvoldown",
                    "Decrease recording volume", &item);
...
void MIXER_Init() {
  ...
  Section_prop *section =
      static_cast<Section_prop *>(control->GetSection("mixer"));
  ...
  if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
    LOG(LOG_MISC, LOG_DEBUG)(
        "MIXER:Can't initialize SDL audio: %s , running in nosound mode.",
        SDL_GetError());
    mixer.nosound = true;
  }
#endif
  ...
  if (mixer.nosound) {
...
#ifdef C_SDL2
  } else if ((SDL2_AudioDevice = SDL_OpenAudioDevice(
    ...
    mixer.nosound = true;
    LOG(LOG_MISC,
        LOG_DEBUG)("MIXER:Can't open audio: %s , running in nosound mode.",
                   SDL_GetError());
    TIMER_AddTickHandler(MIXER_Mix);
    ...
    if (((Bitu)mixer.freq != (Bitu)obtained.freq) ||
        ((Bitu)mixer.blocksize != (Bitu)obtained.samples))
      LOG(LOG_MISC, LOG_DEBUG)(
          "MIXER:Got different values from SDL: freq %d, blocksize %d",
          (int)obtained.freq, (int)obtained.samples);
  ...
  if (mixer.work_wrap <= mixer.blocksize)
    E_Exit("blocksize too large");

  {
    ...
    mixer.prebuffer_samples =
        ((unsigned int)ms * (unsigned int)mixer.freq) / 1000u;
    if (mixer.prebuffer_samples > (mixer.work_wrap / 2))
      mixer.prebuffer_samples = (mixer.work_wrap / 2);
  }

  // how many samples per millisecond? compute as improper fraction (sample rate
  // / 1000)
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "Mixer: sample_accurate=%u blocksize=%u sdl_rate=%uHz mixer_rate=%uHz "
      "channels=%u samples=%u min/max/need=%u/%u/%u per_ms=%u %u/%u samples "
      "prebuffer=%u",
      (unsigned int)mixer.sampleaccurate, (unsigned int)mixer.blocksize,
      (unsigned int)obtained.freq, (unsigned int)mixer.freq,
  ...
  AddVMEventFunction(VM_EVENT_DOS_INIT_KERNEL_READY,
                     AddVMEventFunctionFuncPair(MIXER_DOS_Boot));
