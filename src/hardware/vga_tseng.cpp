...
typedef struct {
  ...
  // HiColor DAC control register. See comments below. Only bits 5-7 are
  // emulated, close to SC11485 version.
  uint8_t hicolorDACcommand;

  // Stored exact values of some registers. Documentation only specifies some
  // bits but hardware checks may expect other bits to be preserved.
  Bitu store_3d4_31;
  Bitu store_3d4_32;
  Bitu store_3d4_33;
  Bitu store_3d4_34;
...
void write_p3d5_et4k(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0x33:
    ...
    vga.config.display_start =
        (vga.config.display_start & 0xffff) | ((val & 0x03) << 16);
    vga.config.cursor_start =
        (vga.config.cursor_start & 0xffff) | ((val & 0x0c) << 14);
    break;

    /*
    3d4h index 34h (R/W): 6845 Compatibility Control Register
    bit    0  Enable CS0 (alternate clock timing)
           1  Clock Select bit 2.  Bits 0-1 in 3C2h bits 2-3, bits 3-4 are in
    3d4h index 31h bits 6-7 2  Tristate ET4000 bus and color outputs if set 3
    Video Subsystem Enable Register at 46E8h if set, at 3C3h if clear. 4  Enable
    Translation ROM for reading CRTC and MISCOUT if set 5  Enable Translation
    ROM for writing CRTC and MISCOUT if set 6  Enable double scan in AT&T
    compatibility mode if set 7  Enable 6845 compatibility if set
    */
  ...
  case 0x35:
    ...
    bit    0  Vertical Blank Start Bit 10 (3d4h index 15h).
           ...
           6  (4000) Read/Modify/Write Enabled if set. Currently not
    implemented. 7  Vertical interlace if set. The Vertical timing registers are
            programmed as if the mode was non-interlaced!!
    */
    et4k.store_3d4_35 = val;
    vga.config.line_compare =
        (vga.config.line_compare & 0x3ff) | ((val & 0x10) << 6);
    // Abusing s3 ex_ver_overflow field. This is to be cleaned up later.
    {
      uint8_t s3val = ((val & 0x01) << 2) | // vbstart
                      ((val & 0x02) >> 1) | // vtotal
                      ((val & 0x04) >> 1) | // vdispend
    ...
    // VGADOC provides a lot of info on this register, Ferraro has significantly
    // less detail. This is unlikely to be used by any games. Bit 4 switches
    // chipset into linear mode - that may be useful in some cases if there is
    // any software actually using it.
  ...
  case 0x37:
    if (val != et4k.store_3d4_37) {
      ...
      vga.mem.memmask =
          (((64u * 1024u * ((val & 8u) ? 4u : 1u)) << ((val & 3u) - 1u)) - 1u) &
          (vga.mem.memsize - 1u);
      VGA_SetupHandlers();
    }
    break;
  ...
  case 0x3f:
    ...
    */
    ...
    break;
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:ET4K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3d5_et4k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET4K(3d4, 3f);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:ET4K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
void write_p3c5_et4k(Bitu reg, Bitu val, Bitu iolen) {
  ...
  switch (reg) {
    ...
    */
    ...
    // Unlikely to be used by games (things like ROM enable/disable and
    // emulation of VGA vs EGA)
    STORE_ET4K(3c4, 07);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:SEQ:ET4K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3c5_et4k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET4K(3c4, 07);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:SEQ:ET4K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
void write_p3c0_et4k(Bitu reg, Bitu val, Bitu iolen) {
  ...
  switch (reg) {
    ...
    // and even those incompletely. The register is used as part of
    // identification scheme. Unlikely to be used by any games but double timing
    // may be useful.
    // TODO: Figure out if this has any practical use
    STORE_ET4K(3c0, 16);
    /*
    3C0h index 17h (R/W):  Miscellaneous 1
    bit   7  If set protects the internal palette ram and redefines the
    attribute bits as follows: Monochrome: bit 0-2  Select font 0-7 3  If set
    selects blinking 4  If set selects underline 5  If set prevents the
    character from being displayed 6  If set displays the character at half
    intensity 7  If set selects reverse video Color: bit 0-1  Selects font 0-3
                    2  Foreground Blue
                    3  Foreground Green
                    4  Foreground Red
                    5  Background Blue
    ...
    */
    ...
    STORE_ET4K(3c0, 17);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:ET4K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3c1_et4k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET4K(3c0, 17);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:ET4K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
static Bitu get_clock_index_et4k() {
  // Ignoring bit 4, using "only" 16 frequencies. Looks like most
  // implementations had only that
  return ((vga.misc_output >> 2) & 3) | ((et4k.store_3d4_34 << 1) & 4) |
         ((et4k.store_3d4_31 >> 3) & 8);
}
...
static void set_clock_index_et4k(Bitu index) {
  ...
  et4k.store_3d4_31 =
      (et4k.store_3d4_31 & ~0xc0u) |
      ((index & 8u)
       << 3u); // (index&0x18) if 32 clock frequencies are to be supported
}

void FinishSetMode_ET4K(Bitu crtc_base, VGA_ModeExtraData *modeData) {
  // Note that switching to a mode always puts the DAC in 15-bit color. An extra
  // BIOS call is necessary to switch to 16-bit color. We are also forcing the
  // mode back to work exactly the way it did on the real hardware
  ...
  uint8_t et4k_hor_overflow = (modeData->hor_overflow & 0x01) |
                              (modeData->hor_overflow & 0x04) |
                              (modeData->hor_overflow & 0x10);
  IO_Write(crtc_base, 0x3f);
  IO_Write(crtc_base + 1, et4k_hor_overflow);
  ...
  uint8_t et4k_ver_overflow =
      ...
      ((modeData->ver_overflow & 0x10) >>
       1) | // vretrace10 (tseng has vsync start?)
      ((modeData->ver_overflow & 0x40) >> 2); // line_compare
  IO_Write(crtc_base, 0x35);
  IO_Write(crtc_base + 1, et4k_ver_overflow);

  // Clear remaining ext CRTC registers
  IO_Write(crtc_base, 0x31);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x32);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x33);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x34);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x36);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x37);
  IO_Write(crtc_base + 1, 0x0c | (vga.mem.memsize == 1024 * 1024  ? 3
                                  : vga.mem.memsize == 512 * 1024 ? 2
                                                                  : 1));
  // Clear ext SEQ
  IO_Write(0x3c4, 0x06);
  IO_Write(0x3c5, 0);
  ...
  // Select SVGA clock to get close to 60Hz (not particularly clean
  // implementation)
  if (modeData->modeNo > 0x13) {
    Bits target =
        static_cast<Bits>(modeData->vtotal * 8 * modeData->htotal * 60);
    Bitu best = 1;
    int dist = 100000000;
    for (Bitu i = 0; i < 16; i++) {
      int cdiff =
          abs(static_cast<int>(target - static_cast<Bits>(et4k.clockFreq[i])));
      if (cdiff < dist) {
        best = i;
        dist = cdiff;
      }
  ...
  // Verified (on real hardware and in a few games): Tseng ET4000 used chain4
  // implementation different from standard VGA. It was also not limited to 64K
  // in regular mode 13h.
...
Bitu GetClock_ET4K() { return et4k.clockFreq[get_clock_index_et4k()]; }
...
// Support is sufficient for proper detection (WHATVGA detects UMC 70c178,
// similar to Sierra SC11487, slightly simpler to work with). Control over
// 15/16-bit color is also implemented. Need better documentation and/or test
// cases to advance the implementation.
/*
...
void write_p3d5_et3k(Bitu reg, Bitu val, Bitu iolen) {
  ...
  case 0x23:
    ...
    bit   0  Cursor start address bit 16
          ...
          7  If set memory address 8 is output on the MBSL pin (allowing access
    to 1MB), if clear the blanking signal is output.
    */
    // Only bits 1 and 2 are supported. Bit 2 is related to hardware zoom, bit 7
    // is too obscure to be useful
    et3k.store_3d4_23 = val;
    vga.config.display_start =
        (vga.config.display_start & 0xffff) | ((val & 0x02) << 15);
    vga.config.cursor_start =
        (vga.config.cursor_start & 0xffff) | ((val & 0x01) << 16);
    break;
  ...
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:ET3K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3d5_et3k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET3K(3d4, 25);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:CRTC:ET3K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
void write_p3c5_et3k(Bitu reg, Bitu val, Bitu iolen) {
  (void)iolen; // UNUSED
  switch (reg) {
    // Both registers deal mostly with hardware zoom which is not implemented.
    // Other bits seem to be useless for emulation with the exception of index 7
    // bit 4 (font select)
    STORE_ET3K(3c4, 06);
    STORE_ET3K(3c4, 07);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:SEQ:ET3K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3c5_et3k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET3K(3c4, 07);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:SEQ:ET3K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
Bitu read_p3cd_et3k(Bitu port, Bitu iolen) {
  (void)port;
  (void)iolen; // UNUSED
  return (Bitu)(((Bitu)vga.svga.bank_read << 3u) | (Bitu)vga.svga.bank_write |
                ((vga.svga.bank_size == 128u * 1024u) ? 0u : 0x40u));
}
...
void write_p3c0_et3k(Bitu reg, Bitu val, Bitu iolen) {
  ...
  switch (reg) {
    ...
    STORE_ET3K(3c0, 17);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:ET3K:Write to illegal index %2X",
                                 (int)reg);
    break;
  }
}
...
Bitu read_p3c1_et3k(Bitu reg, Bitu iolen) {
  ...
  switch (reg) {
    ...
    RESTORE_ET3K(3c0, 17);
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:ET3K:Read from illegal index %2X",
                                 (int)reg);
    break;
  }
  return 0x0;
...
void FinishSetMode_ET3K(Bitu crtc_base, VGA_ModeExtraData *modeData) {
  ...
  IO_Write(crtc_base, 0x23);
  IO_Write(crtc_base + 1, 0);
  IO_Write(crtc_base, 0x24);
  IO_Write(crtc_base + 1, 0);
  // Clear ext SEQ
  IO_Write(0x3c4, 0x06);
  IO_Write(0x3c5, 0);
  IO_Write(0x3c4, 0x07);
  IO_Write(0x3c5, 0x40); // 0 in this register breaks WHATVGA
  // Clear ext ATTR
  IO_Write(0x3c0, 0x16);
  IO_Write(0x3c0, 0);
  IO_Write(0x3c0, 0x17);
  ...
  if (modeData->modeNo > 0x13) {
    ...
    for (Bitu i = 0; i < 8; i++) {
      int cdiff = abs(
          static_cast<int32_t>(target - static_cast<Bits>(et3k.clockFreq[i])));
      if (cdiff < dist) {
        best = i;
        dist = cdiff;
      }
  ...
  // Verified on functioning (at last!) hardware: Tseng ET3000 is the same as
  // ET4000 when it comes to chain4 architecture
  vga.config.compatible_chain4 = false;
  //    vga.vmemwrap = vga.mem.memsize;
...
void DetermineMode_ET3K() {
  ...
  if (vga.attr.mode_control & 1) {
    if (vga.gfx.mode & 0x40)
      VGA_SetMode((et3k.biosMode <= 0x13) ? M_VGA : M_LIN8); // Ugly...
    else if (vga.gfx.mode & 0x20)
      VGA_SetMode(M_CGA4);
    else if ((vga.gfx.miscellaneous & 0x0c) == 0x0c)
      VGA_SetMode(M_CGA2);
    else
      VGA_SetMode((et3k.biosMode <= 0x13) ? M_EGA : M_LIN4);
  } else {
    VGA_SetMode(M_TEXT);
  }
}
...
Bitu GetClock_ET3K() { return et3k.clockFreq[get_clock_index_et3k()]; }

bool AcceptsMode_ET3K(Bitu mode) {
  return mode <= 0x37 && mode != 0x2f &&
         VideoModeMemSize(mode) < vga.mem.memsize;
}
...
void SVGA_Setup_TsengET3K(void) {
  ...
  vga.mem.memsize =
      512 *
      1024; // Cannot figure how this was supposed to work on the real card
}
