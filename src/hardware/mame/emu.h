...
public:
  ...
  virtual void sound_stream_update(sound_stream &stream,
                                   ...
                                   stream_sample_t **outputs, int samples) = 0;
...
public:
  struct machine_t {
    int describe_context() const { return 0; }
  };

  machine_t machine() const { return machine_t(); }

  // int offset, space;

  u32 clock() const { return clockRate; }
  ...
  void SaveState(std::ostream &stream) { WRITE_POD(&clockRate, clockRate); }
  void LoadState(std::istream &stream) { READ_POD(&clockRate, clockRate); }
