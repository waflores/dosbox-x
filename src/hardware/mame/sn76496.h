...
public:
  ...
  //	template <class Object> devcb_base &set_ready_handler(Object &&cb) {
  //return m_ready_handler.set_callback(std::forward<Object>(cb)); }
...
protected:
  sn76496_base_device(const machine_config &mconfig, device_type type,
                      const char *tag, int feedbackmask, int noisetap1,
                      int noisetap2, bool negate, bool stereo, int clockdivider,
...
private:
  ...
  const int32_t m_whitenoise_tap1; // mask for white noise tap 1 (higher one,
                                   // usually bit 14)
  const int32_t m_whitenoise_tap2; // mask for white noise tap 2 (lower one,
                                   // usually bit 13)
  ...
  const bool
      m_ncr_style_psg; // flag to ignore writes to regs 1,3,5,6,7 with bit 7 low
  const bool
      m_sega_style_psg; // flag to make frequency zero acts as if it is one more
                        // than max (0x3ff+1) or if it acts like 0; the initial
                        // register is pointing to 0x3 instead of 0x0; the
                        // volume reg is preloaded with 0xF instead of 0x0
  ...
  int32_t
      m_cycles_to_ready; // number of cycles until the READY line goes active
...
// SN76496: Whitenoise verified, phase verified, periodic verified (by Michael
// Zapf)
class sn76496_device : public sn76496_base_device {
public:
  sn76496_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};

// U8106 not verified yet. todo: verify; (a custom marked sn76489? only used on
// mr. do and maybe other universal games)
class u8106_device : public sn76496_base_device {
public:
  u8106_device(const machine_config &mconfig, const char *tag, device_t *owner,
               uint32_t clock);
};

// Y2404 not verified yet. todo: verify; (don't be fooled by the Y, it's a TI
// chip, not Yamaha)
class y2404_device : public sn76496_base_device {
public:
  y2404_device(const machine_config &mconfig, const char *tag, device_t *owner,
               uint32_t clock);
};

// NCR8496 whitenoise verified, phase verified; verified by ValleyBell &
// NewRisingSun
class sn76489_device : public sn76496_base_device {
public:
  sn76489_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};

// PSSJ-3 whitenoise verified, phase verified; verified by ValleyBell &
// NewRisingSun
class pssj3_device : public sn76496_base_device {
public:
  pssj3_device(const machine_config &mconfig, const char *tag, device_t *owner,
               uint32_t clock);
};

// SN76489A: whitenoise verified, phase verified, periodic verified (by
// plgdavid)
class sn76489a_device : public sn76496_base_device {
public:
  sn76489a_device(const machine_config &mconfig, const char *tag,
                  device_t *owner, uint32_t clock);
};

// SN76494 not verified, (according to datasheet: same as sn76489a but without
// the /8 divider)
class sn76494_device : public sn76496_base_device {
public:
  sn76494_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};

// SN94624 whitenoise verified, phase verified, period verified; verified by
// PlgDavid
class sn94624_device : public sn76496_base_device {
public:
  sn94624_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};
...
public:
  ncr8496_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};
...
public:
  gamegear_device(const machine_config &mconfig, const char *tag,
                  device_t *owner, uint32_t clock);
};
...
public:
  segapsg_device(const machine_config &mconfig, const char *tag,
                 device_t *owner, uint32_t clock);
};
