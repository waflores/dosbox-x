...
**  - implemented CPU -> AUDIO ADPCM synthesis (via writes to the ADPCM data reg
*$08)
**  - implemented support for the Limit address register
**  - supported two bits from the control register 2 ($01): RAM TYPE (x1 bit/x8
*bit), ROM/RAM
**  - implemented external memory access (read/write) via the ADPCM data reg
*reads/writes
**    Thanks go to Frits Hilderink for the example code.
**
** 14-06-2003 Jarek Burczynski:
**  - various fixes to enable proper support for status register flags: BSRDY,
*PCM BSY, ZERO
...
**  - fixed first missing sound in gigandes thanks to previous fix
*(interpolator) by ElSemi
...
**    Thanks go to Stefan Jokisch (stefan.jokisch@gmx.de) for tracking down the
*problem.
**
** TO DO:
  ...
  if ((portstate & 0xe0) == 0x20) {
    ...
    if (now_addr != (end << 1)) {
      ...
      if (status_reset_handler && status_change_BRDY_bit)
        (status_reset_handler)(status_change_which_chip,
                               status_change_BRDY_bit);

      /* setup a timer that will callback us in 10 master clock cycles for Y8950
       * in the callback set the BRDY flag to 1 , which means we have another
       * data ready. For now, we don't really do this; we simply reset and set
       * the flag in zero time, so that the IRQ will work.
       */
      /* set BRDY bit in status register */
...
/* 0-DRAM x1, 1-ROM, 2-DRAM x8, 3-ROM (3 is bad setting - not allowed by the
 * manual) */
...
void YM_DELTAT::ADPCM_Write(int r, int v) {
  ...
  case 0x00:
    /*
    START:
        Accessing *external* memory is started when START bit (D7) is set to
    "1", so you must set all conditions needed for recording/playback before
    starting. If you access *CPU-managed* memory, recording/playback starts
    after read/write of ADPCM data register $08.
    ...
    MEMDATA:
        ...
        1 = external memory (using start/end/limit registers to access memory:
    RAM or ROM)
    ...
    value:   START, REC, MEMDAT, REPEAT, SPOFF, x,x,RESET   meaning:
      C8     1      1    0       0       1      0 0 0       Analysis (recording)
    from AUDIO to CPU (to reg $08), sample rate in PRESCALER register E8     1
    1    1       0       1      0 0 0       Analysis (recording) from AUDIO to
    EXT.MEMORY,       sample rate in PRESCALER register 80     1      0    0 0
    0      0 0 0       Synthesis (playing) from CPU (from reg $08) to
    AUDIO,sample rate in DELTA-N register a0     1      0    1       0       0
    0 0 0       Synthesis (playing) from EXT.MEMORY to AUDIO,        sample rate
    in DELTA-N register

      60     0      1    1       0       0      0 0 0       External memory
    write via ADPCM data register $08 20     0      0    1       0       0 0 0 0
    External memory read via ADPCM data register $08

    */
    /* handle emulation mode */
    if (emulation_mode == EMULATION_MODE_YM2610) {
      v |= 0x20; /*  YM2610 always uses external memory and doesn't even have
                    memory flag bit. */
    }

    portstate =
        v & (0x80 | 0x40 | 0x20 | 0x10 |
    ...
    {
      now_addr = start << 1;
      memread = 2; /* two dummy reads needed before accessing external memory
                      via register $08*/
    ...
    } else /* we access CPU memory (ADPCM data register $08) so we only reset
              now_addr here */
    {
      now_addr = 0;
    }
  ...
  case 0x01: /* L,R,-,-,SAMPLE,DA/AD,RAMTYPE,ROM */
    /* handle emulation mode */
    if (emulation_mode == EMULATION_MODE_YM2610) {
      v |= 0x01; /*  YM2610 always uses ROM as an external memory and doesn't
                    tave ROM/RAM memory flag bit. */
    }

    pan = &output_pointer[(v >> 6) & 0x03];
    if ((control2 & 3) != (v & 3)) {
      /*0-DRAM x1, 1-ROM, 2-DRAM x8, 3-ROM (3 is bad setting - not allowed by
       * the manual) */
  ...
  case 0x08: /* ADPCM data */
    ...
    if ((portstate & 0xe0) == 0x60) {
      ...
      if (now_addr != (end << 1)) {
        ...
        /* reset BRDY bit in status register, which means we are processing the
         * write */
        if (status_reset_handler && status_change_BRDY_bit)
          (status_reset_handler)(status_change_which_chip,
                                 status_change_BRDY_bit);
         ...
         * written the data. For now, we don't really do this; we simply reset
         * and set the flag in zero time, so that the IRQ will work.
         */
        /* set BRDY bit in status register */
        if (status_set_handler && status_change_BRDY_bit)
          (status_set_handler)(status_change_which_chip,
                               status_change_BRDY_bit);
...
void YM_DELTAT::ADPCM_Reset(int panidx, int mode, device_t *dev) {
  ...
  limit = ~0; /* this way YM2610 and Y8950 (both of which don't have limit
                 address reg) will still work */
  volume = 0;
  ...
  control2 = (emulation_mode == EMULATION_MODE_YM2610)
                 ? 0x01
                 : 0; /* default setting depends on the emulation mode. MSX demo
                         called "facdemo_4" doesn't setup control2 register at
                         all and still works */
...
static inline void YM_DELTAT_synthesis_from_external_memory(YM_DELTAT *DELTAT) {
  ...
  if (DELTAT->now_step >= (1 << YM_DELTAT_SHIFT)) {
    ...
    do {
      ...
      if (DELTAT->now_addr ==
          (DELTAT->end << 1)) { /* 12-06-2001 JB: corrected comparison. Was >
                                   instead of == */
  ...
  /* ElSemi: Fix interpolator. */
  DELTAT->adpcml =
      DELTAT->prev_acc * (int)((1 << YM_DELTAT_SHIFT) - DELTAT->now_step);
...
void YM_DELTAT::ADPCM_CALC() {
  ...
  some examples:
  value:   START, REC, MEMDAT, REPEAT, SPOFF, x,x,RESET   meaning:
    80     1      0    0       0       0      0 0 0       Synthesis (playing)
  from CPU (from reg $08) to AUDIO,sample rate in DELTA-N register a0     1 0 1
  0       0      0 0 0       Synthesis (playing) from EXT.MEMORY to AUDIO,
  sample rate in DELTA-N register C8     1      1    0       0       1      0 0
  0       Analysis (recording) from AUDIO to CPU (to reg $08), sample rate in
  PRESCALER register E8     1      1    1       0       1      0 0 0 Analysis
  (recording) from AUDIO to EXT.MEMORY,       sample rate in PRESCALER register

    60     0      1    1       0       0      0 0 0       External memory write
  via ADPCM data register $08 20     0      0    1       0       0      0 0 0
  External memory read via ADPCM data register $08
  ...
  */
  ...
  if ((portstate & 0xe0) == 0x80) {
    ...
    YM_DELTAT_synthesis_from_CPU_memory(
        this); /* change output based on data in ADPCM data reg ($08) */
    return;
  }
