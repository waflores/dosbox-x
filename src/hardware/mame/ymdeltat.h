...
struct YM_DELTAT { /* AT: rearranged and tightened structure */
  ...
  uint8_t control2;  /* control reg: SAMPLE, DA/AD, RAM TYPE (x8bit / x1bit),
                        ROM/RAM */
  ...
  */
  ...
  uint8_t status_change_EOS_bit;  /* 1 on End Of Sample (record/playback/cycle
                                     time of AD/DA converting has passed)*/
  uint8_t status_change_BRDY_bit; /* 1 after recording 2 data (2x4bits) or after
                                     reading/writing 1 data */
  uint8_t status_change_ZERO_bit; /* 1 if silence lasts for more than 290
                                     milliseconds on ADPCM recording */

  /* neither Y8950 nor YM2608 can generate IRQ when PCMBSY bit changes, so
  *instead of above,
  ** the statusflag gets ORed with PCM_BSY (below) (on each read of statusflag
  *of Y8950 and YM2608)
  */
