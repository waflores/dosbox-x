...
Bitu pc98_a1_read(Bitu port, Bitu iolen) {
  ...
  case 0xA9: // an 8-bit I/O port to access font RAM by...
    // NOTES: On a PC-9821 Lt2 laptop, the character ROM doesn't seem to latch
    // valid data beyond
    //        0xxx5D. Often this reads back as zero, but depending on whatever
    //        random data is floating on the bus can read back nonzero. This
    //        doesn't apply to 0x0000-0x00FF of course (single wide characters),
    //        but only to the double-wide character set where (c & 0x007F) >=
    //        0x5D. This behavior should be emulated. */
    return pc98_font_char_read(a1_font_load_addr, a1_font_char_offset & 0xF,
...
void pc98_a1_write(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0xA9: // an 8-bit I/O port to access font RAM by...
             ...
             //
             // I'm also guessing that this RAM is not involved with the
             // single-wide character set, which is why writes to 0x0056/0x0057
             // are redirected to 0x8056/0x8057. Without this hack, Touhou
             // Project 2 will overwrite the letter 'W' when loading its font
             // data (Level 1 will show "Eastern  ind" instead of "Eastern Wind"
             // for the music title as a result).
             //
             // On real hardware it seems, attempts to write anywhere outside
             // 0xxx56/0xxx57 are ignored. They are not remapped. Attempts to
             // write to 0x0056 are ignored by the hardware (since that
             // conflicts with single-wide chars) but you can write to that cell
             // if you write to 0x8056 instead.
    if ((a1_font_load_addr & 0x007E) == 0x0056 &&
        (a1_font_load_addr & 0xFF00) != 0x0000)
      pc98_font_char_write(a1_font_load_addr, a1_font_char_offset & 0xF,
                           (a1_font_char_offset & 0x20) ? 0 : 1, (uint8_t)val);
    else
      LOG_MSG("A1 port attempt to write FONT ROM char 0x%x", a1_font_load_addr);
    break;
  default:
    LOG_MSG("A1 port %lx val %lx unexpected", (unsigned long)port,
            (unsigned long)val);
    break;
  }
}
