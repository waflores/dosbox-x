...
const UINT32 pc9861k_speed[11] = {75,   150,  300,   600,   1200, 2400,
...
static const _PC9861CH pc9861def = {
    0x05, 0xff, 7, 1, 0, 0, 2400, NEVENT_MAXCLOCK, 0, 0, 0};
...
static void pc9861ch1_open(void) {
  ...
  pc9861k.ch1.vect =
      ((np2cfg.pc9861sw[1] >> 1) & 1) | ((np2cfg.pc9861sw[1] << 1) & 2);
  pc9861k.ch1.irq = ch1_irq[pc9861k.ch1.vect];
  pc9861_makeclk(&pc9861k.ch1, 10 * 2);
  nevent_set(NEVENT_PC9861CH1, pc9861k.ch1.clk, pc9861ch1cb, NEVENT_ABSOLUTE);
}
...
static void pc9861ch2_open(void) {
  ...
  pc9861k.ch2.vect =
      ((np2cfg.pc9861sw[1] >> 3) & 1) | ((np2cfg.pc9861sw[1] >> 1) & 2);
  pc9861k.ch2.irq = ch1_irq[pc9861k.ch2.vect];
  pc9861_makeclk(&pc9861k.ch2, 10 * 2);
  nevent_set(NEVENT_PC9861CH2, pc9861k.ch2.clk, pc9861ch2cb, NEVENT_ABSOLUTE);
}

// -------------------------------------------------------------------------

static void IOOUTCALL pc9861data_w8(COMMNG cm, PC9861CH m, UINT port,
                                    REG8 value) {
