...
static const OEMCHAR *rhythmfile[6] = {file_2608bd, file_2608sd,  file_2608top,
...
void rhythm_initialize(UINT rate) {
  ...
  for (i = 0; i < 96; i++) {
    rhythmcfg.voltbl[i] = (UINT)(32768.0 * pow(2.0, (double)i * (-3.0) / 40.0));
  }
}
...
void rhythm_setvol(UINT vol) { rhythmcfg.vol = vol; }

void rhythm_reset(RHYTHM rhy) { ZeroMemory(rhy, sizeof(_RHYTHM)); }
...
void rhythm_update(RHYTHM rhy) {
  ...
  for (i = 0; i < 6; i++) {
    rhy->trk[i].volume =
        (rhythmcfg.voltbl[rhy->vol + rhy->trkvol[i]] * rhythmcfg.vol) >> 10;
  }
}
...
void rhythm_setreg(RHYTHM rhy, UINT reg, REG8 value) {
  ...
  if (reg == 0x10) {
    ...
    trk->volume = (rhythmcfg.voltbl[rhy->vol + value] * rhythmcfg.vol) >> 10;
  }
}
