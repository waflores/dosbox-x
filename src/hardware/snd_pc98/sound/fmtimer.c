...
enum { NEVENT_FMTIMERA = 0, NEVENT_FMTIMERB };
...
static void nevent_ack_event(unsigned int n) { nevent_is_set[n] = 0; }

static unsigned int nevent_iswork(unsigned int n) { return nevent_is_set[n]; }

static const UINT8 irqtable[4] = {0x03, 0x0d, 0x0a, 0x0c};

/* added for DOSBox-X */
UINT8 fmtimer_index2irq(const UINT8 index) { return irqtable[index & 3]; }
