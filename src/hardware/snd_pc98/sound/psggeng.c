...
void SOUNDCALL psggen_getpcm(PSGGEN psg, SINT32 *pcm, UINT count) {
  ...
  do {
    ...
    if (mixer & 0x38) {
      /* NTS: This code relies on signed integer underflow to determine when to
         advance the pseudo-random noise generation sequence. It assumes that it
         can detect SINT32 carry by copying the value, subtracting the original,
         and then testing to see if the result is larger than the original.

              Unfortunately, Clang/LLVM break this code by promoting both to
         long (64-bit) before comparing, which turns the noise into DC bias
         popping noises.

              This hack, with long type masks, emulates the original behavior
         even if the compare is promoted to signed long (64-bit) and allows the
         noise generation to work even with Clang/LLVM --J.C */
      ...
      for (i = 0; i < (1 << PSGADDEDBIT); i++) {
        ...
        if (((long)psg->noise.count & sint32mask) >
            ((long)countbak & sint32mask)) {
          //					psg->noise.base = GETRAND() & (1
          //<< (1 << PSGADDEDBIT));
