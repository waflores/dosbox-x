...
static const int MSADPCMTable[16] = {230, 230, 230, 230, 307, 409, 512, 614,
...
static void msa_decend(GETSND snd) { _MFREE(snd->snd); }
...
static BOOL msa_open(GETSND snd, WAVE_INFOS *wavehead, UINT headsize) {
  ...
  blk *= 2;
  TRACEOUT(
      ("wav: msa: ExtraSize %d / SPB=%d NumOfCoefs=%d", exsize, spb, numcoef));
...
BOOL getwave_open(GETSND snd, UINT8 *ptr, UINT size) {
  ...
  if (!memcmp(riff->fmt, fmt_wave, 4)) {
    ...
    TRACEOUT(("wav: fmt: %x / %dch %dHz %dbit", format, snd->channels,
              snd->samplingrate, snd->bit));
