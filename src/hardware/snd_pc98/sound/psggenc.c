...
static const UINT8 psggen_deftbl[0x10] = {0, 0, 0, 0, 0, 0, 0,    0xbf,
                                          0, 0, 0, 0, 0, 0, 0xff, 0xff};

static const UINT8 psgenv_pat[16] = {PSGENV_ONESHOT,
                                     PSGENV_ONESHOT,
                                     PSGENV_ONESHOT,
                                     PSGENV_ONESHOT,
...
void psggen_initialize(UINT rate) {
  ...
  if (rate) {
    psggencfg.base =
        (5000 * (1 << (32 - PSGFREQPADBIT - PSGADDEDBIT))) / (rate / 25);
  }
}
...
void psggen_setvol(UINT vol) {
  ...
  for (i = 1; i < 16; i++) {
    psggencfg.volume[i] = (psggencfg.voltbl[i] * vol) >> (6 + PSGADDEDBIT);
  }
}
