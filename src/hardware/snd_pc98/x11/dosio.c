...
void dosio_init(void) { /* nothing to do */ }

void dosio_term(void) { /* nothing to do */ }
...
static BOOL cnvdatetime(struct stat *sb, DOSDATE *dosdate, DOSTIME *dostime) {
  struct tm *ftime;
...
short file_getdatetime(FILEH handle, DOSDATE *dosdate, DOSTIME *dostime) {
  ...
  if ((fstat(fileno(handle), &sb) == 0) && (cnvdatetime(&sb, dosdate, dostime)))
    return 0;
  return -1;
}
