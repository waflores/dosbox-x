...
unsigned int ne2k_ipx_highest_variant =
    0; // Windows 95 likes to try all three variations as it runs

#if C_IPX
void ethernetSendToIPX(const unsigned char *outptr, unsigned int outlen,
                       unsigned int vari);

bool is_IPX_ethernet_frame(const unsigned char *buf, unsigned int len,
...
bx_ne2k_c::bx_ne2k_c(void) { s.tx_timer_index = BX_NULL_TIMER_HANDLE; }
...
void bx_ne2k_c::write_cr(uint32_t value) {
  ...
  if (BX_NE2K_THIS s.CR.rdma_cmd == 3) {
    ...
    BX_NE2K_THIS s.remote_bytes =
        *((uint16_t *)&BX_NE2K_THIS s
              .mem[BX_NE2K_THIS s.bound_ptr * 256 + 2 - BX_NE2K_MEMSTART]);
    BX_INFO("Sending buffer #x%x length %d", BX_NE2K_THIS s.remote_start,
            BX_NE2K_THIS s.remote_bytes);
  }
  ...
  if ((value & 0x04) && BX_NE2K_THIS s.TCR.loop_cntl) {
    ...
    if (BX_NE2K_THIS s.TCR.loop_cntl != 1) {
      ...
      rx_frame(&BX_NE2K_THIS s
                    .mem[BX_NE2K_THIS s.tx_page_start * 256 - BX_NE2K_MEMSTART],
    ...
    if (CaptureState & CAPTURE_NETWORK)
      Capture_WritePacket(true /*send*/,
                          &s.mem[s.tx_page_start * 256 - BX_NE2K_MEMSTART],
                          s.tx_bytes);
...

    {
      ...
      if (ne2k_ipx_redirect &&
          is_IPX_ethernet_frame(
              &s.mem[s.tx_page_start * 256 - BX_NE2K_MEMSTART], s.tx_bytes,
              &outptr, &outlen, &vari)) {
        if (ne2k_ipx_highest_variant < vari)
          ne2k_ipx_highest_variant = vari;
        if (vari ==
            IPX_OLD_LLC /*TODO: Make configurable*/ /*Best for Windows 95*/)
          ethernetSendToIPX(outptr, outlen, vari);
      } else {
#endif
    ...
    PIC_AddEvent(
        NE2000_TX_Event,
        (float)((64 + 96 + 4 * 8 + BX_NE2K_THIS s.tx_bytes * 8) / 10000.0), 0);
...
uint32_t bx_ne2k_c::chipmem_read(uint32_t address, unsigned int io_len) {
  ...
  if ((address >= BX_NE2K_MEMSTART) && (address < BX_NE2K_MEMEND)) {
    ...
    if ((io_len == 2u) || (io_len == 4u)) {
      retval |=
          (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 1]
                         << 8u);
    }
    if (io_len == 4u) {
      retval |=
          (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 2]
                         << 16u);
      retval |=
          (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 3]
                         << 24u);
    }
    return (retval);
  }
...
void bx_ne2k_c::chipmem_write(uint32_t address, uint32_t value,
                              unsigned io_len) {
  if ((io_len == 2) && (address & 0x1))
...
uint32_t bx_ne2k_c::asic_read(uint32_t offset, unsigned int io_len) {
  ...
  case 0x0: // Data register
    ...
    if (io_len > BX_NE2K_THIS s.remote_bytes) {
      BX_ERROR("ne2K: dma read underrun iolen=%d remote_bytes=%d", io_len,
               BX_NE2K_THIS s.remote_bytes);
      // return 0;
    }

    // BX_INFO(("ne2k read DMA: addr=%4x remote_bytes=%d",BX_NE2K_THIS
    // s.remote_dma,BX_NE2K_THIS s.remote_bytes));
...
//
uint32_t bx_ne2k_c::read_handler(void *this_ptr, uint32_t address,
                                 unsigned io_len) {
#if !BX_USE_NE2K_SMF
  bx_ne2k_c *class_ptr = (bx_ne2k_c *)this_ptr;
...
uint32_t bx_ne2k_c::read(uint32_t address, unsigned io_len) {
  ...
  uint32_t retval = 0;
  unsigned int offset =
      (unsigned int)address - (unsigned int)(BX_NE2K_THIS s.base_address);
...
// by this ne2000 instance
//
void bx_ne2k_c::write_handler(void *this_ptr, uint32_t address, uint32_t value,
                              unsigned io_len) {
#if !BX_USE_NE2K_SMF
  bx_ne2k_c *class_ptr = (bx_ne2k_c *)this_ptr;
...
#define POLYNOMIAL 0x04c11db6
  ...
  return (uint32_t)((crc & 0xfffffffful) >>
                    26ul); /* WARNING: Caller directly uses our 6-bit return as
                              index. If not truncated, will cause a segfault */
#undef POLYNOMIAL
}
...
void bx_ne2k_c::rx_frame(const void *buf, unsigned io_len) {
  ...
  if (CaptureState & CAPTURE_NETWORK)
    Capture_WritePacket(false /*send*/, pktbuf, io_len);
  ...
  startptr =
      &BX_NE2K_THIS s.mem[BX_NE2K_THIS s.curr_page * 256 - BX_NE2K_MEMSTART];
  if ((nextpage > BX_NE2K_THIS s.curr_page) ||
    ...
    unsigned int endbytes =
        (unsigned int)(BX_NE2K_THIS s.page_stop - BX_NE2K_THIS s.curr_page) *
        256u;
    memcpy(startptr, pkthdr, 4);
    memcpy(startptr + 4, buf, (size_t)(endbytes - 4u));
    startptr = &BX_NE2K_THIS s
                    .mem[BX_NE2K_THIS s.page_start * 256u - BX_NE2K_MEMSTART];
...
static void NE2000_Poller(void) {
  ethernet->GetPackets([](const uint8_t *packet, int len) {
    ...
    if (ne2k_ipx_redirect &&
        is_IPX_ethernet_frame(packet, len, NULL, NULL, NULL))
      return;
#endif
...
public:
  ...
  NE2K(Section *configuration) : Module_base(configuration) {
    ...
    if (!(base == 0x260 || base == 0x280 || base == 0x300 || base == 0x320 ||
          base == 0x340 || base == 0x380)) {
      base = 0x300;
    }
    ...
    } else if (sscanf(macstring, "%02x:%02x:%02x:%02x:%02x:%02x", &macint[0],
    ...
    for (Bitu i = 0; i < 0x20; i++) {
      ReadHandler8[i].Install((i + theNE2kDevice->s.base_address), dosbox_read,
                              IO_MB | IO_MW);
  ...
  ~NE2K() {
    ...
    PIC_RemoveEvents(NE2000_TX_Event);
    if (addne2k)
      VFILE_Remove("NE2000.COM", "SYSTEM");
  }
};
...
bool NE2K_GetMacAddress(unsigned char *buf) {
  if (test == NULL || theNE2kDevice == NULL || ethernet == NULL)
    return false;

#if C_IPX
  // If the MAC address is the stock default, then don't use it, because the
  // DOSBox-X scheme will cause all clients to use the same MAC address and then
  // IPX won't work.
  if (!memcmp(theNE2kDevice->s.physaddr, "\xAC\xDE\x48\x88\x99\xAA", 6))
    return false;
#endif
