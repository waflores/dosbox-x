...
const PhysPt PC98_GDC_state::gram_base[4] = {0xe0000, 0xa8000, 0xb0000,
                                             0xb8000};
...
void PC98_GDC_state::set_mode(uint8_t mode) { draw.mode = mode & 0x03; }
...
void PC98_GDC_state::set_vectw(uint8_t ope, uint8_t dir, uint16_t dc,
                               uint16_t d, uint16_t d2, uint16_t d1,
                               uint16_t dm) {
  draw.ope = ope;
  draw.dir = dir;
  draw.dc = dc;
...
void PC98_GDC_state::draw_dot(uint16_t x, uint16_t y) {
  ...
  addr = x >> 4;
  bit = (x ^ 8) &
        15; /* flip bit 3 to set correct bit despite little endian byte order */
  if (addr >= dpitch)
    return;
  addr += y * dpitch;
  if (addr >= 16384u)
  ...
  //       This is the only way for GDC drawing commands to draw properly in
  //       Windows 3.1, and possibly any other game that uses EGC.
  //       Read/Modify/Write normally with EGC causes drawing artifacts.
  //
  //       Perhaps NEC ran into this themselves? Perhaps EGC hardware can
  //       identify reads coming from the GDC and it returns 0x0000 in that case
  //       to avoid artifacts?
  //
  //       However, the dummy read is required or else stale data stored from
  //       previous ops ends up in the rendering and artifacts occur.
  //
  //       The GDC is documented to issue 16-bit read/modify/write when drawing,
  //       so that's what this code should do. Additionally, drawing with 8-bit
  //       memio and EGC causes minor artifacts.
  ...
  if ((pc98_gdc_vramop & 0xE) == 0xA || (pc98_gdc_vramop & 0xC) == 0xC) {
    ...
    if (dot == 0) {
      // REPLACE
      if (draw.mode == 0x00) {
        pc98_gdc_vwritew(draw.base + addr,
                         pc98_gdc_vreadw(draw.base + addr) & ~(0x8000 >> bit));
      }
    } else {
      // REPLACE or SET
...
void PC98_GDC_state::circle(void) {
  ...
  if (gdc_rt[RT_TABLEMAX] == 0) {
    for (int i = 0; i <= RT_TABLEMAX; i++) {
      ...
      gdc_rt[i] =
          (uint16_t)((double)(1 << RT_MULBIT) *
                     (1 -
                      sqrt(1 - pow((0.70710678118654 * i) / RT_TABLEMAX, 2))));
    }
  }
