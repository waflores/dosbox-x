...
struct XGAStatus {
  ...
  struct XGA_WaitCmd {
    ...
    Bitu datasize;
    Bitu buswidth;
    bool bswap16; /* bit 12 of the CMD register (S3 86C928,
                     Trio32/Trio64/Trio64V+): For any 16-bit word, swap hi/lo
                     bytes (including both 16-bit words in 32-bit transfer) */
  } waitcmd;
  ...
  struct XGA_VirgeState {
    struct reggroup {
      ...
      uint32_t
          right_clip; /* +00DC [LO WORD] "position of last pixel to be drawn"
                         which means left <= x <= right inclusive, right? */
      uint32_t left_clip; /* +00DC [HI WORD] */
      uint32_t
          bottom_clip;   /* +00E0 [LO WORD] "position of last pixel to be drawn"
                            which means top <= y <= bottom inclusive, right? */
      ...
      void set__mono_pat_dword(unsigned int idx,
                               uint32_t val);         /* +00E8, +00EC */
      ...
      uint32_t
          command_execute_on_register; /* if command set bit 0 set, writing this
                                          register will execute command */
    };
    ...
    struct BitBltState {
      ...
      uint32_t src_xrem;
      uint32_t src_drem;
      uint64_t itf_buffer; /* we shift in 32 bits at a time, making this 64-bit
                              simplifies code */
    ...
    union colorpat_t {
      /* NTS: extra 4 bytes so a typecast to pat8 to read 24 bits does not
       * overread the buffer */
    ...
    colorpat_t colorpat;
    unsigned int truecolor_bypp; /* ViRGE cards seem to prefer 24bpp? Windows
                                    drivers act like it */
...
void XGAStatus::XGA_VirgeState::reggroup::set__src_base(uint32_t val) {
  src_base = val & 0x003FFFF8; /* bits [21:3] base address in vmem source data
                                  for 2D operations */
}

void XGAStatus::XGA_VirgeState::reggroup::set__dst_base(uint32_t val) {
  dst_base = val & 0x003FFFF8; /* bits [21:3] base address in vmem source data
                                  for 2D operations */
}

void XGAStatus::XGA_VirgeState::reggroup::set__src_dest_stride_00e4(
    uint32_t val) {
  src_stride = val & 0x0FF8; /* bits [11:3] byte stride */
  dst_stride =
      (val >> 16u) & 0x0FF8; /* bits [27:19] (11+16,3+16) byte stride */
}

void XGAStatus::XGA_VirgeState::reggroup::set__mono_pat_dword(unsigned int idx,
                                                              uint32_t val) {
  /* idx == 0, low 32 bits.
   * idx == 1, high 32 bits.
...
void XGAStatus::XGA_VirgeState::reggroup::set__rect_width_height_0104(
    uint32_t val) {
  rect_height = val & 0x07FF;
  rect_width =
      ((val >> 16ul) & 0x07FF) +
      1; /* <- What? But then the height field is THE number of pixels. Why? */
}
...
void XGAStatus::XGA_VirgeState::reggroup::set__left_right_clip_00dc(
    uint32_t val) {
  right_clip = val & 0x07FF;
  left_clip = (val >> 16ul) & 0x07FF;
}

void XGAStatus::XGA_VirgeState::reggroup::set__top_bottom_clip_00e0(
    uint32_t val) {
...
void XGAStatus::XGA_VirgeState::reggroup::set__lindrawxdelta_0170(
    uint32_t val) {
  lindrawxdelta = (int32_t)val;
}

void XGAStatus::XGA_VirgeState::reggroup::set__lindrawstartx_0174(
    uint32_t val) {
  lindrawstartx = (int32_t)val;
}

void XGAStatus::XGA_VirgeState::reggroup::set__lindrawstartx_0178(
    uint32_t val) {
  lindrawstarty = val & 0x3FFFu; /* bits [10:0] */
}

void XGAStatus::XGA_VirgeState::reggroup::set__lindrawcounty_017c(
    uint32_t val) {
...
void XGA_DrawPoint(Bitu x, Bitu y, Bitu c) {
  if (!(xga.curcommand & 0x1))
    return;
  if (!(xga.curcommand & 0x10))
    return;

  if (x < xga.scissors.x1)
    return;
  if (x > xga.scissors.x2)
    return;
  ...
  /* Need to zero out all unused bits in modes that have any (15-bit or "32"-bit
     -- the last one is actually 24-bit. Without this step there may be some
     graphics corruption (mainly, during windows dragging. */
  switch (XGA_COLOR_MODE) {
  case M_LIN4: {
    uint8_t shf = ((memaddr ^ 1u) & 1u) * 4u;
    if (GCC_UNLIKELY((memaddr / 2) >= vga.mem.memsize))
      break;
    vga.mem.linear[memaddr / 2] =
        (vga.mem.linear[memaddr / 2] & (0xF0 >> shf)) + ((c & 0xF) << shf);
  } break;
  case M_LIN8:
    if (GCC_UNLIKELY(memaddr >= vga.mem.memsize))
      break;
  ...
  case M_LIN16:
    ...
    break;
  case M_LIN32:
    if (GCC_UNLIKELY(memaddr * 4 >= vga.mem.memsize))
      break;
...
#define SWAP(a, b)                                                             \
  ...
  /* S3 Trio64 documentation: The "desty" register is both a destination Y for
   * BitBlt (hence the name) and "Line Parameter Axial Step Constant" for line
   * drawing, in case the name of the variable is confusing here. The "desty"
   * variable name exists as inherited from DOSBox SVN source code.
   *
  ...
  /* S3 Trio64 documentation: The "destx" register is both a destination X for
   * BitBlt (hence the name) and "Line Parameter Diagonal Step Constant" for
   * line drawing, in case the name of the variable is confusing here. The
   * "destx" variable name exists as inherited from DOSBox SVN source code.
   *
   * lpdst = 2 * min(abs(dx),abs(dy)) - max(abs(dx),abs(dy)) */
  destxtmp = (Bits)((int16_t)xga.destx);
  if (xga.destx & 0x2000)
    destxtmp |= ~((Bits)0x1fff);
  ...
  if (xga.ErrTerm & 0x2000)
    e |= ~((Bits)0x1fff); /* sign extend 13-bit error term */
...
void XGA_DrawRectangle(Bitu val) {
  ...
  /* Undocumented, but seen with Windows 3.1 drivers: Horizontal lines are drawn
   * with this XGA command and "skip last pixel" set, else they are one pixel
   * too wide */
  xrun = xga.MAPcount;
  if (skiplast) {
    if (xrun > 0u)
  ...
  // LOG_MSG("XGA: Draw rect (%d, %d)-(%d, %d), %d", x1, y1, x2, y2,
  // xga.forecolor);
}
...
void XGA_DrawWait(Bitu val, Bitu len) {
  ...
  case 2: /* Rectangle */
    ...
    case 0x00: /* FOREMIX always used */
      ...
      /*					switch((mixmode >> 5) & 0x03) {
                                                      case 0x00: // Src is
         background color srcval = xga.backcolor; break; case 0x01: // Src is
         foreground color srcval = xga.forecolor; break; case 0x02: // Src is
         pixel data from PIX_TRANS register
      */
      ...
      case 0x40 | M_LIN8: // 32 bit
        ...
        break;
      case (0x20 | M_LIN32):
        if (len != 4) { // In case of two 16-bit transfers, first combine both
                        // WORDs into a 32-bit DWORD and then operate
          // Needed for Windows 3.1 with S386c928 drivers, and Windows NT/2000
          // to display 16x16 radio buttons and icons properly
...
void XGA_BlitRect(Bitu val) {
  ...
  case 0x03: /* Video memory determines mix */
    // LOG_MSG("XGA: Srcdata: %x, Forecolor %x, Backcolor %x, Foremix: %x
    // Backmix: %x", srcdata, xga.forecolor, xga.backcolor, xga.foremix,
    // xga.backmix);
    break;
  default:
  ...
  for (yat = 0; yat <= xga.MIPcount; yat++) {
    ...
    for (xat = 0; xat <= xga.MAPcount; xat++) {
      ...
      /* For more information, see the "S3 Vision864 Graphics Accelerator"
       * datasheet
       ...
       * Page 203 for "Multifunction Control Miscellaneous Register (MULT_MISC)"
       * which this code holds as xga.control1, and Page 198 for "Color Compare
       * Register (COLOR_CMP)" which this code holds as xga.color_compare. */
      if (xga.control1 & 0x100) { /* COLOR_CMP enabled. control1 corresponds to
                                     XGA register BEE8h */
      ...
      if (doit) {
        destval = XGA_GetMixResult(mixmode, srcval, dstdata);
        // LOG_MSG("XGA: DrawPattern: Mixmode: %x Mixselect: %x", mixmode,
        // mixselect);
...
void XGA_DrawPattern(Bitu val) {
  ...
  case 0x02: /* CPU Data determines mix used */
    ...
    break;
  case 0x03: /* Video memory determines mix */
    // LOG_MSG("XGA: Pixctl: %x, Srcdata: %x, Forecolor %x, Backcolor %x,
    // Foremix: %x Backmix: %x",xga.pix_cntl, srcdata, xga.forecolor,
    // xga.backcolor, xga.foremix, xga.backmix);
    break;
  default:
  ...
  for (yat = 0; yat <= xga.MIPcount; yat++) {
    ...
    for (xat = 0; xat <= xga.MAPcount; xat++) {
      ...
      srcdata =
          XGA_GetPoint((Bitu)srcx + (tarx & 0x7), (Bitu)srcy + (tary & 0x7));
      // LOG_MSG("patternpoint (%3d/%3d)v%x",srcx + (tarx & 0x7), srcy + (tary &
      // 0x7),srcdata);
      dstdata = XGA_GetPoint((Bitu)tarx, (Bitu)tary);

      if (mixselect == 0x3) {
        /* S3 Trio32/Trio64 Integrated Graphics Accelerators, section 13.2
         * Bitmap Access Through The Graphics Engine.
         *
         ...
         *
         * "If bits 7-6 are set to 11b, the current display bit map is selected
         * as the mask bit source. The Read Mask" "register (AAE8H) is set up to
         * indicate the active planes. When all bits of the read-enabled planes
         * for a" "pixel are a 1, the mask bit 'ONE' is generated. If anyone of
         * the read-enabled planes is a 0, then a mask" "bit 'ZERO' is
         * generated. If the mask bit is 'ONE', the Foreground Mix register is
         * used. If the mask bit is"
         * "'ZERO', the Background Mix register is used."
         *
         * Notice that when an application in Windows 3.1 draws a black
         * rectangle, I see foreground=0 background=ff and in this loop,
         * srcdata=ff and readmask=ff. While the original DOSBox SVN "guess"
         * code here would misattribute that to the background color (and
         * erroneously draw a white rectangle), what should actually happen is
         * that we use the foreground color because (srcdata&readmask)==readmask
         * (all bits 1).
         *
         * This fixes visual bugs when running Windows 3.1 and Microsoft
         * Creative Writer, and navigating to the basement and clicking around
         * in the dark to reveal funny random things, leaves white rectangles on
         * the screen where the image was when you released the mouse. Creative
         * Writer clears the image by drawing a BLACK rectangle, while the
         * DOSBox SVN "guess" mistakenly chose the background color and
         * therefore a WHITE rectangle. */
        if ((srcdata & xga.readmask) == xga.readmask)
          mixmode = xga.foremix;
        else
          mixmode = xga.backmix;
...
void XGA_DrawCmd(Bitu val, Bitu len) {
  ...
  case 2: /* Rectangle fill */
    if ((val & 0x100) == 0) {
      xga.waitcmd.wait = false;
#if XGA_SHOW_COMMAND_TRACE == 1
      LOG_MSG("XGA: Draw immediate rect: xy(%3d/%3d), len(%3d/%3d)", xga.curx,
              xga.cury, xga.MAPcount, xga.MIPcount);
#endif
      ...
      /* Windows will always use 101b (+X +Y) to draw left to right, top to
       * bottom. Apparently there is an MS-DOS CAD program out there that wants
       * to draw
      ...
      xga.waitcmd.cmd = 2;
      if (s3Card >=
          S3_Vision864) /* this is when bit 10 became part of bit 9 to allow BUS
                           SIZE == 32 (2-bit value == binary 10 == decimal 2) */
        xga.waitcmd.buswidth = (Bitu)vga.mode | (Bitu)((val & 0x600u) >> 4u);
      else /* S3 86C928 datasheet lists bit 10 as reserved, therefore BUS WIDTH
              can only be 8 or 16 */
        xga.waitcmd.buswidth = (Bitu)vga.mode | (Bitu)((val & 0x200u) >> 4u);
      xga.waitcmd.data = 0;
      xga.waitcmd.datasize = 0;

      if (s3Card < S3_ViRGE) /* NTS: ViRGE datasheets do not mention port 9AE8H
                                except in passing, maybe it was dropped? FIXME:
                                How does this work with BUS SIZE == 32? */
        xga.waitcmd.bswap16 =
            (val & 0x1200u) ==
            0x0200u; // BYTE SWP(12):  0=High byte first (big endian)  1=Low
                     // byte first (little endian)  and BUS SIZE  1=16-bit
                     // 0-8-bit
      else
        xga.waitcmd.bswap16 = false; // we're little endian, dammit!

#if XGA_SHOW_COMMAND_TRACE == 1
      LOG_MSG("XGA: Draw wait rect, w/h(%3d/%3d), x/y1(%3d/%3d), "
              "x/y2(%3d/%3d), %4x",
  ...
  case 3: /* Polygon fill (Trio64) */
    if (s3Card == S3_Trio64) {
       ...
       * would also imply that the card updates current Y position to the
       * minimum of either side so the new coordinates continue properly.
       *
       * NTS: The Windows 3.1 Trio64 driver likes to send this command every
       * single time it updates any coordinate, contrary to the Trio64 datasheet
       * that suggests setting cur/dest X/Y and cur2/dest2 X/Y THEN sending this
       *      command, then setting either dest X/Y and sending the command
       * until the polygon has been rasterized. We can weed those out here by
       * ignoring any command where the cur/dest Y coordinates would result in
       * no movement.
       *
       *      The Windows 3.1 driver also seems to use cur/dest X/Y for the
       * RIGHT side, and cur2/dest2 X/Y for the LEFT side, which is completely
       * opposite from the example given in the datasheet. This also implies
       * that whatever order the vertices end up, they draw a span when
       * rasterizing, and the sides can cross one another if necessary.
       *
       * NTS: You can test this code by bringing up Paintbrush, and drawing with
       * the brush tool. Despite drawing a rectangle, the S3 Trio64 driver uses
       * the Polygon fill command to draw it.
       *
       *      More testing is possible in Microsoft Word 2.0 using the
       * shapes/graphics editor, adding solid rectangles or rounded rectangles
       * (but not circles).
       *
       * Vertex at (*)
       *
      ...
      if (xga.cury < xga.desty && xga.cury2 < xga.desty2) {
        LOG(LOG_MISC,
            LOG_DEBUG)("Trio64 Polygon fill: leftside=(%d,%d)-(%d,%d) "
                       "rightside=(%d,%d)-(%d,%d)",
                       xga.curx, xga.cury, xga.destx, xga.desty, xga.curx2,
                       xga.cury2, xga.destx2, xga.desty2);
  ...
  case 7: /* Pattern fill */
    ...
    LOG_MSG("XGA: Pattern fill: src(%3d/%3d), dest(%3d/%3d), fill(%3d/%3d)",
            xga.curx, xga.cury, xga.destx, xga.desty, xga.MAPcount,
            xga.MIPcount);
#endif
    XGA_DrawPattern(val);
    break;
...
void XGA_SetDualReg(uint32_t &reg, Bitu val) {
  ...
  case M_LIN4:
    ...
    break;
  case M_LIN8:
    reg = (uint8_t)(val & 0xff);
    break;
  case M_LIN15:
  case M_LIN16:
    reg = (uint16_t)(val & 0xffff);
    break;
  case M_LIN32:
...
uint32_t XGA_MixVirgePixel(uint32_t srcpixel, uint32_t patpixel,
                           uint32_t dstpixel, uint8_t rop) {
  switch (rop) {
  ...
  case 0x00 /*0           */:
    return 0;
  case 0x0A /*DPna        */:
    return (~patpixel) & dstpixel;
  case 0x22 /*DSna        */:
    return (~srcpixel) & dstpixel;
  case 0x55 /*Dn          */:
    return ~dstpixel;
  case 0x5A /*DPx         */:
    return dstpixel ^ patpixel;
  case 0x66 /*DSx         */:
    return dstpixel ^ srcpixel;
  case 0x69 /*PDSxxn      */:
    return ~(srcpixel ^ dstpixel ^ patpixel);
  case 0x88 /*DSa         */:
    return dstpixel & srcpixel;
  case 0xA5 /*PDxn        */:
    return ~(patpixel ^ dstpixel);
  case 0xAA /*D           */:
    return dstpixel;
  case 0xB8 /*PSDPxax     */:
    return ((dstpixel ^ patpixel) & srcpixel) ^ patpixel;
  case 0xBB /*DSno        */:
    return (~srcpixel) | dstpixel;
  case 0xC0 /*PSa         */:
    return patpixel & srcpixel;
  case 0xCC /*S           */:
    return srcpixel;
  case 0xE2 /*DSPDxax     */:
    return ((patpixel ^ dstpixel) & srcpixel) ^ dstpixel;
  case 0xEE /*DSo         */:
    return dstpixel | srcpixel;
  case 0xF0 /*P           */:
    return patpixel;
...
uint32_t XGA_VirgePatPixelMono(unsigned int x, unsigned int y) {
  const uint8_t rb = ((unsigned char *)(&xga.virge.bitblt.mono_pat))
      [y & 7]; /* WARNING: Only works on little Endian CPUs */
  return (rb & (0x80 >> (x & 7))) ? xga.virge.bitblt.mono_pat_fgcolor
                                  : xga.virge.bitblt.mono_pat_bgcolor;
}
...
uint32_t XGA_VirgePatPixel(unsigned int x, unsigned int y) {
  ...
  case 2: // 24/32 bits/pixel
    return *((uint32_t *)(&xga.virge.colorpat
                               .pat8[(((y & 7u) << 3u) + (x & 7u)) * 3u])) &
           0xFFFFFF;
  default:
    break;
  }
...
uint32_t XGA_ReadSourceVirgePixel(XGAStatus::XGA_VirgeState::reggroup &rset,
  ...
  case 2: // 24/32 bits/pixel
    memaddr =
        (uint32_t)((y * rset.src_stride) + (x * xga.virge.truecolor_bypp)) +
        rset.src_base;
    if (GCC_UNLIKELY(memaddr >= vga.mem.memsize))
      break;
...
uint32_t XGA_ReadDestVirgePixel(XGAStatus::XGA_VirgeState::reggroup &rset,
                                unsigned int x, unsigned int y) {
  uint32_t memaddr;
  ...
  case 2: // 24/32 bits/pixel
    memaddr =
        (uint32_t)((y * rset.dst_stride) + (x * xga.virge.truecolor_bypp)) +
        rset.dst_base;
    if (GCC_UNLIKELY(memaddr >= vga.mem.memsize))
      break;
    return *((uint32_t *)(vga.mem.linear + memaddr)) & xga.virge.truecolor_mask;
...
void XGA_DrawVirgePixel(XGAStatus::XGA_VirgeState::reggroup &rset,
                        unsigned int x, unsigned int y, uint32_t c) {
  uint32_t memaddr;

  if (!(rset.command_set & 0x20))
    return; /* bit 5 draw enable == 0 means don't update screen */
...
inline void XGA_DrawVirgePixelCR(XGAStatus::XGA_VirgeState::reggroup &rset,
                                 unsigned int x, unsigned int y, uint32_t c) {
  if (rset.command_set & 2) { /* clip enable */
    if (x >= rset.left_clip && x <= rset.right_clip && y >= rset.top_clip &&
        y <= rset.bottom_clip)
...
void XGA_ViRGE_BitBlt_xferport(uint32_t val) {
  ...
  assert(xga.virge.bitbltstate.itf_buffer_bytecount <=
         4u); /* we should be flushing data */

  if (xga.virge.bitbltstate.itf_buffer_bytecount > 0) {
    xga.virge.bitbltstate.itf_buffer |=
        ((uint64_t)val) << (8u * xga.virge.bitbltstate.itf_buffer_bytecount);
  ...
  if (xga.virge.imgxferport->command_set & 0x40) {
    ...
    while (xga.virge.bitbltstate.itf_buffer_bytecount > 0) {
      ...
      msk = 0x80u;
      if (xga.virge.imgxferport->command_set &
          0x100) { /* mono pattern, mono bitmap */
        if (xga.virge.imgxferport->command_set & 0x200) { /* transparent */
          do {
            if (xga.virge.bitbltstate.src_drem > 0) {
              if ((uint8_t)xga.virge.bitbltstate.itf_buffer & msk) {
                ...
                mixpixel = XGA_MixVirgePixel(
                    srcpixel, patpixel, dstpixel,
                    (xga.virge.bitblt.command_set >> 17u) & 0xFFu);
          ...
          do {
            if (xga.virge.bitbltstate.src_drem > 0) {
              srcpixel = ((uint8_t)xga.virge.bitbltstate.itf_buffer & msk)
                             ? xga.virge.bitblt.src_fgcolor
                             : xga.virge.bitblt.src_bgcolor;
              dstpixel = XGA_ReadDestVirgePixel(xga.virge.bitblt, x, y);
              patpixel = XGA_VirgePatPixelMono(x, y);
              mixpixel = XGA_MixVirgePixel(
                  srcpixel, patpixel, dstpixel,
    ...
    case 1:
      ...
      break; // 16 bits/pixel
    case 2:
      bypp = xga.virge.truecolor_bypp;
      bypmsk = xga.virge.truecolor_mask;
      break; // 24/32 bits/pixel
    default:
      break;
    }
    ...
    while (xga.virge.bitbltstate.itf_buffer_bytecount >= bypp) {
      ...
      if (xga.virge.imgxferport->command_set &
          0x100) { /* mono pattern, color bitmap */
        if (xga.virge.imgxferport->command_set & 0x200) { /* transparent */
          // TODO
          ...
          if (xga.virge.bitbltstate.src_drem > 0) {
            ...
            dstpixel = XGA_ReadDestVirgePixel(xga.virge.bitblt, x, y);
            patpixel = XGA_VirgePatPixelMono(x, y);
            mixpixel = XGA_MixVirgePixel(srcpixel, patpixel, dstpixel,
                                         (xga.virge.bitblt.command_set >> 17u) &
                                             0xFFu);
            XGA_DrawVirgePixelCR(xga.virge.bitblt, x, y, mixpixel);
            xga.virge.bitbltstate.src_drem--;
          }
      ...
      if (xga.virge.bitbltstate.src_xrem < bypp) {
        if (y == xga.virge.bitbltstate.stopy) {
          ...
          if (xga.virge.bitbltstate.src_xrem > 0) {
            if (xga.virge.bitbltstate.src_xrem >
                xga.virge.bitbltstate.itf_buffer_bytecount) {
              xga.virge.bitbltstate.src_xrem -=
                  xga.virge.bitbltstate.itf_buffer_bytecount;
              xga.virge.bitbltstate.itf_buffer_initskip =
                  xga.virge.bitbltstate.src_xrem;
              break;
            } else {
              xga.virge.bitbltstate.itf_buffer >>=
                  (uint64_t)(8u * xga.virge.bitbltstate.src_xrem);
              xga.virge.bitbltstate.itf_buffer_bytecount -=
                  xga.virge.bitbltstate.src_xrem;
            }
          }
...
void XGA_ViRGE_BitBlt(XGAStatus::XGA_VirgeState::reggroup &rset) {
  ...
  if (rset.command_set &
      0x80) { /* data will be coming in from the image transfer port */
    ...
    if (rset.command_set & 0x40) {
      ...
      switch ((rset.command_set >> 2u) & 7u) {
      case 1:
        xga.virge.bitbltstate.src_stride *= 2u;
        break; // 16 bits/pixel
      case 2:
        xga.virge.bitbltstate.src_stride *= xga.virge.truecolor_bypp;
        break; // 24/32 bits/pixel
      default:
        break;
      }
    ...
    case 1:
      xga.virge.bitbltstate.src_stride =
          (xga.virge.bitbltstate.src_stride + 1u) & (~1u);
      break; // WORD align
    case 2:
      xga.virge.bitbltstate.src_stride =
          (xga.virge.bitbltstate.src_stride + 3u) & (~3u);
      break; // DWORD align
    default:
      break;
    }
    ...
    if (xga.virge.bitblt.command_set & 0x200) { /* transparent */
      ...
      if (xga.virge.bitblt.rect_width != 0 &&
          xga.virge.bitblt.rect_height != 0) {
...
void XGA_ViRGE_DrawRect(XGAStatus::XGA_VirgeState::reggroup &rset) {
  ...
  // NTS: I don't know if the monochrome pattern is being drawn properly because
  // I can't get Windows 3.1
  //      to use this case for anything other than solid color rectangles. I
  //      don't know if I am reading out the monochrome pattern correctly here.
  //      --J.C.

  // NTS: always use mono pattern as documented by S3.
  //      Command set MP bit must be set anyway.
  for (y = bey; y <= eny; y++) {
    rb = ((unsigned char *)(&rset.mono_pat))[(y - rset.rect_dst_y) &
                                             7]; /* WARNING: Only works on
                                                    little Endian CPUs */
  ...
  /* NTS: From the S3 datasheet "Command Set Register": "The full range of 256
   * ROPs are available for BitBlt. Other operations like Rectangle, Line, etc.
   *      can only use a subset of the ROPs that does not have a source. When a
   * ROP contains a pattern, the pattern must be mono and the hardware forces
   *      the pattern value to the pattern foreground color regardless of the
   * values programmed in the Mono Pattern registers."
   *
   *      True to this statement, Windows 3.1 Virge drivers like to issue XGA
   * rectangle commands with the ROP set to 0xF0 (pattern fill) when drawing
...
void XGA_ViRGE_BitBlt_Execute(bool commandwrite) {
  ...
  switch ((rset.command_set >> 27u) &
          0x1F) { /* bits [31:31] 3D command if set, 2D else. bits [30:27]
                     command */
  case 0x00:      /* 2D BitBlt */
  ...
  case 0x0F: /* NOP */
    break;
  default:
    LOG(LOG_VGA, LOG_DEBUG)("BitBlt unhandled command %08x",
                            (unsigned int)rset.command_set);
    break;
  }
}
...
void XGA_ViRGE_BitBlt_Execute_deferred(void) {
  ...
  default:
    if (rset.command_set & (1u << 31u))
      LOG(LOG_VGA, LOG_DEBUG)("BitBlt execute 3D unhandled command %08x def",
                              (unsigned int)rset.command_set);
    else
      LOG(LOG_VGA, LOG_DEBUG)("BitBlt execute 2D unhandled command %08x def",
                              (unsigned int)rset.command_set);
    rset.command_execute_on_register = 0;
    break;
  };
...
void VIRGELineDDA::adv(void) { xf += xdelta; }

int VIRGELineDDA::read_xtr(void) { return xf >> 20; }
...
void XGA_ViRGE_DrawLine(XGAStatus::XGA_VirgeState::reggroup &rset) {
  ...
  /* HACK: Why doesn't the Windows 98 S3 ViRGE driver set the stride for the
   * line2d register set? I'm beginning to wonder if all dest/source offset and
   * stride registers are really just tied together into one set in the back.
   * This hack is needed to make sure lines and curves aren't jumbled up at the
   * top of the screen when drawn. */
  ...
  xdir = (rset.lindrawcounty & 0x80000000u) ? 1 /*left to right*/
                                            : -1 /*right to left*/;
  ...
  ldda.xdelta = rset.lindrawxdelta; /* S11.20 fixed point signed, 1.0 = 1 << 20
                                       -(dX / dY) */
  ...
  /* S3 ViRGE Integrated 3D Accelerator
   * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/S3%20Graphics%2c%20Ltd/S3%20ViRGE%20Integrated%203D%20Accelerator%20%281996%2d08%29%2epdf]
   * PDF page 238 Line Draw X Start Register.
   *
   * For X major line, +XDELTA, value = (x1 << 20) + (XDELTA/2)
   * For X major line, -XDELTA, value = (x1 << 20) + (XDELTA/2) + ((1 << 20) -
   * 1) For Y major line, value = x1 << 20
   *
   * NTS: Windows 3.1 S3 ViRGE drivers use (x1 << 20) + (1 << 19) equiv (x1 +
   * 0.5) for Y major lines.
   *
   * Line Draw X Delta Register: XDELTA = -(changeInX << 20) / changeInY
   *
   * Also notice that based on how this line rendering works, is it vital to
   * turn on clipping and set the clip region to the area you intend for the
   * line to sit within, else when changeInX is large and changeInY is small,
   * the line segment will extend some pixels past the end pixel. So in reality
   * the XDELTA and start X registers describe horizontal line segments that
   * extend slightly past the clipping region. At least that's how the
   * Windows 3.1 treats this hardware acceleration function. */
   ...
   *
   *      Drawing completely horizontal lines, Windows 3.1 style (ViRGE
   * drivers);
   *      - Set ycount == 1, XDELTA to (xend + 1 - xstart) without the 20-bit
   * shift, which is then a value that is very close to zero, but not quite you
   * lazy hack of a driver. xstart <= x <= xend are the extents of the
   * horizontal line to draw.
   *
   *      Note that small to zero XDELTA values ALSO represent a vertical or
   * near vertical line, so that value alone isn't enough to determine if we're
   * being asked to draw horizontal lines, however when Windows 3.1 does it,
   * ycount == 1, XDELTA is some small value, XF is right in the middle of
   * xstart-xend, and xstart-end are far wider than one pixel. Not sure by what
   * logic or special case S3 would have handled horizontal lines here. Based on
   * driver behavior, if the intent was to draw a 1-pixel high vertical line,
   *      then the xstart/xend values would equal (xf >> 20) without any
   * additional room. */
  ...
  if (ycount <= 1) {
    ...
    /* NTS: xstart/xend must be considered to render Y-major lines correctly
     * when the guest driver wants to draw a line but omit the last pixel when
     * drawing line segments of a polygon or shape. This is needed to correctly
     * render the shape in progress for example when drawing circles or rounded
     * rectangles in Microsoft Word (uses XOR raster operation), and to render
     * curves in Windows 98 correctly (Curves And Colors screen saver). Without
     * the option to NOT render the last pixel, XOR-based poly lines will be
     * missing pixels, since the overlapping pixels cancel each other out. */

    /* check: xstart skip last pixel, when drawing a line going downard, with
     * XGA hardware that only draws upward */
    x = ldda.read_xtr();
    ...
    while (ycount > 0) {
      x = ldda.read_xtr();
      if (ycount == 1) { /* check: xend skip last pixel, when drawing a line
                            going upward */
        if ((xdir > 0 && x > xend) || (xdir < 0 && x < xend)) {
          break;
        }
      ...
      ycount--;
             ...
             0) { // X-major going to the left (draws bottom up, remember?)
    while (ycount > 0) {
      xto = ldda.read_xtr();
      while (x <= xto) {
        if (x >= xstart && x <= xend) {
  ...
  } else { // X-major going to the right, xdelta < 0 (draws bottom up,
           // remember?)
...
void XGA_ViRGE_Line2D_Execute(bool commandwrite) {
  ...
  case 0x0F: /* NOP */
    break;
  default:
    LOG(LOG_VGA, LOG_DEBUG)("Line2D unhandled command %08x",
                            (unsigned int)rset.command_set);
    break;
  }
}
...
void XGA_ViRGE_Line2D_Execute_deferred(void) {
  ...
  default:
    if (rset.command_set & (1u << 31u))
      LOG(LOG_VGA, LOG_DEBUG)("Line2D execute 3D unhandled command %08x def",
                              (unsigned int)rset.command_set);
    else
      LOG(LOG_VGA, LOG_DEBUG)("Line2D execute 2D unhandled command %08x def",
                              (unsigned int)rset.command_set);
    rset.command_execute_on_register = 0;
    break;
  };
...
void XGA_ViRGE_Poly2D_Execute(void) {
  ...
  if (rset.command_set & (1u << 31u))
    LOG(LOG_VGA, LOG_DEBUG)("Poly2D execute 3D unhandled command %08x",
                            (unsigned int)rset.command_set);
  else
    LOG(LOG_VGA, LOG_DEBUG)("Poly2D execute 2D unhandled command %08x",
                            (unsigned int)rset.command_set);
}
...
void XGA_ViRGE_Poly2D_Execute_deferred(void) {
  ...
  default:
    if (rset.command_set & (1u << 31u))
      LOG(LOG_VGA, LOG_DEBUG)("Poly2D execute 3D command %08x def",
                              (unsigned int)rset.command_set);
    else
      LOG(LOG_VGA, LOG_DEBUG)("Poly2D execute 2D command %08x def",
                              (unsigned int)rset.command_set);
    rset.command_execute_on_register = 0;
    break;
  };
...
void XGA_Write(Bitu port, Bitu val, Bitu len) {
  //	LOG_MSG("XGA: Write to port %x, val %8x, len %x", (unsigned int)port,
  ...
  case 0x8100: // drawing control: row (low word), column (high word)
    ...
    if (len == 4)
      xga.curx = (uint16_t)((val >> 16) & 0x0fff);
    break;
  case 0x8102:
  ...
  case 0x8108: // DWORD drawing control: destination Y and axial step
    ...
    if (len == 4)
      xga.destx = (uint16_t)((val >> 16) & 0x3fff);
    break;
  case 0x810a:
  ...
  case 0x8180: // S3 Trio64V+ streams processor, Primary Stream Control (MMIO
               // only)
    ...
    break;
  case 0x8184: // S3 Trio64V+ streams processor, Color/Chroma Key Control (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x8190: // S3 Trio64V+ streams processor, Secondary Stream Control (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x8194: // S3 Trio64V+ streams processor, Chroma Key Upper Bound (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x8198: // S3 Trio64V+ streams processor, Secondary Stream Stretch/Filter
               // Constants (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      uint32_t regmask;

      if (s3Card >= S3_ViRGEVX) /* ViRGE/VX datasheet says the register is 12
                                   bits large */
        regmask = 0xFFFu;
      else /* earlier cards say the register is 11 bits large */
        regmask = 0x7FFu;

      /* Whoah, wait a minute! The S3 ViRGE and S3 Trio64V+ datasheets have a
       * rather irritating error! They say K1 is bits 10-0 and K2 bits 26-16,
       * but the visual diagram says K2 is bits 10-0 and K1 is bits 26-16!
       *
       * Based on what the S3 driver DCI driver writes for a 320x240 YUV
       * overlay, it appears that K1 (initial output window before scaling - 1)
      ...
      if (vga.s3.streams.ssctl_k2_hscale &
          ((regmask + 1u) >> 1u)) /* (0x7FF+1)>>1 = 0x400 */
        vga.s3.streams.ssctl_k2_hscale -=
            (regmask + 1u); /* (0x7FF+1)    = 0x800 */
    }
    break;
  case 0x81A0: // S3 Trio64V+ streams processor, Blend Control (MMIO only)
    ...
    break;
  case 0x81C0: // S3 Trio64V+ streams processor, Primary Stream Frame Buffer
               // Address 0 (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ps_fba[0] = val & 0x3FFFFFu;
    }
    break;
  case 0x81C4: // S3 Trio64V+ streams processor, Primary Stream Frame Buffer
               // Address 1 (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ps_fba[1] = val & 0x3FFFFFu;
    }
    break;
  case 0x81C8: // S3 Trio64V+ streams processor, Primary Stream Stride (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ps_stride = val & 0x1FFFu;
    }
    break;
  case 0x81CC: // S3 Trio64V+ streams processor, Double Buffer/LPB Support (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81D0: // S3 Trio64V+ streams processor, Secondary Stream Frame Buffer
               // Address 0 (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ss_fba[0] = val & 0x3FFFFFu;
    }
    break;
  case 0x81D4: // S3 Trio64V+ streams processor, Secondary Stream Frame Buffer
               // Address 1 (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ss_fba[1] = val & 0x3FFFFFu;
    }
    break;
  case 0x81D8: // S3 Trio64V+ streams processor, Secondary Stream Stride (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.ss_stride = val & 0x1FFFu;
    }
    break;
  case 0x81DC: // S3 Trio64V+ streams processor, Opaque Overlay Control (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81E0: // S3 Trio64V+ streams processor, K1 Vertical Scale Factor (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.k1_vscale_factor = val & 0x7FFu;
    }
    break;
  case 0x81E4: // S3 Trio64V+ streams processor, K2 Vertical Scale Factor (MMIO
               // only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81E8: // S3 Trio64V+ streams processor, DDA Vertical Accumulator
               // Initial Value (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81EC: // S3 Trio64V+ streams processor, Streams FIFO and RAS Controls
               // (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      /* bits [4:0] should be a value from 0 to 24 to shift priority between
       * primary and secondary. The larger the value, the more slots allotted to
       * secondary layer. Allocation is out of 24 slots, therefore values larger
       * than 24 are invalid. */
      uint8_t thr = val & 0x1Fu;
      if (thr > 24u)
        thr -= 16u; // assume some kind of odd malfunction happens on real
    ...
    break;
  case 0x81F0: // S3 Trio64V+ streams processor, Primary Stream Window Start
               // Coordinates (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
      vga.s3.streams.pswnd_start_y = val & 0x3FFu;
      vga.s3.streams.pswnd_start_x = (val >> 16u) & 0x3FFu;
    }
    break;
  case 0x81F4: // S3 Trio64V+ streams processor, Primary Stream Window Size
               // (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81F8: // S3 Trio64V+ streams processor, Secondary Stream Window Start
               // Coordinates (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
    ...
    break;
  case 0x81FC: // S3 Trio64V+ streams processor, Primary Stream Window Size
               // (MMIO only)
    if (s3Card == S3_Trio64V || s3Card >= S3_ViRGE) {
  ...
  case 0x83d5:
    if (len == 1)
      vga_write_p3d5(0, val, 1);
    else
      E_Exit("unimplemented XGA MMIO");
    break;
  case 0xa4d4:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__src_base(val);
    else
      goto default_case;
    break;
  case 0xa8d4:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__src_base(val);
    else
      goto default_case;
    break;
  case 0xacd4:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__src_base(val);
    else
      goto default_case;
    break;
  case 0xa4d8:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__dst_base(val);
    else
      goto default_case;
    break;
  case 0xa8d8:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__dst_base(val);
    else
      goto default_case;
    break;
  case 0xacd8:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__dst_base(val);
    else
      goto default_case;
    break;
  case 0xa4dc:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__left_right_clip_00dc(val);
    else
      goto default_case;
    break;
  case 0xa8dc:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__left_right_clip_00dc(val);
    else
      goto default_case;
    break;
  case 0xacdc:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__left_right_clip_00dc(val);
    else
      goto default_case;
    break;
  case 0xa4e0:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__top_bottom_clip_00e0(val);
    else
      goto default_case;
    break;
  case 0xa8e0:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__top_bottom_clip_00e0(val);
    else
      goto default_case;
    break;
  case 0xace0:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__top_bottom_clip_00e0(val);
    else
      goto default_case;
    break;
  case 0xa4e4:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__src_dest_stride_00e4(val);
    else
      goto default_case;
    break;
  case 0xa8e4:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__src_dest_stride_00e4(val);
    else
      goto default_case;
    break;
  case 0xace4:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__src_dest_stride_00e4(val);
    else
      goto default_case;
    break;
  case 0xa4e8:
  case 0xa4ec:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__mono_pat_dword(
          (port >> 2u) & 1u, val);
    else
      goto default_case;
    break;
  ...
  case 0xacec:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__mono_pat_dword(
          (port >> 2u) & 1u, val);
    else
      goto default_case;
    break;
  case 0xa4f0:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__mono_pat_bgcolor(val);
    else
      goto default_case;
    break;
  case 0xacf0:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__mono_pat_bgcolor(val);
    else
      goto default_case;
    break;
  case 0xa4f4:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__mono_pat_fgcolor(val);
    else
      goto default_case;
    break;
  case 0xa8f4:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__mono_pat_fgcolor(val);
    else
      goto default_case;
    break;
  case 0xacf4:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__mono_pat_fgcolor(val);
    else
      goto default_case;
    break;
  case 0xa4f8:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__src_bgcolor(val);
    else
      goto default_case;
    break;
  case 0xa4fc:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__src_fgcolor(val);
    else
      goto default_case;
    break;
  case 0xa500:
    if (s3Card >= S3_ViRGE) {
      ...
      if (rg.command_set & 1)
        XGA_ViRGE_BitBlt_Execute_deferred();
      else
        XGA_ViRGE_BitBlt_Execute(true);
    } else
      goto default_case;
    break;
  case 0xa900:
    if (s3Card >= S3_ViRGE) {
      ...
      rg.set__command_set(val);
      if (rg.command_set & 1)
        XGA_ViRGE_Line2D_Execute_deferred();
      else
        XGA_ViRGE_Line2D_Execute(true);
    } else
      goto default_case;
    break;
  case 0xA96C:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__lindrawend_016c(val);
    else
      goto default_case;
    break;
  case 0xA970:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__lindrawxdelta_0170(val);
    else
      goto default_case;
    break;
  case 0xA974:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__lindrawstartx_0174(val);
    else
      goto default_case;
    break;
  case 0xA978:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__lindrawstartx_0178(val);
    else
      goto default_case;
    break;
  case 0xA97C:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__lindrawcounty_017c(val);
    else
      goto default_case;
    break;
  case 0xad00:
    if (s3Card >= S3_ViRGE) {
      ...
      rg.set__command_set(val);
      if (rg.command_set & 1)
        XGA_ViRGE_Poly2D_Execute_deferred();
      else
  ...
  case 0xa504:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__rect_width_height_0104(val);
    else
      goto default_case;
    break;
  case 0xa904:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__rect_width_height_0104(val);
    else
      goto default_case;
    break;
  case 0xad04:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__rect_width_height_0104(val);
    else
      goto default_case;
    break;
  case 0xa508:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__rect_src_xy_0108(val);
    else
      goto default_case;
    break;
  case 0xa908:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__rect_src_xy_0108(val);
    else
      goto default_case;
    break;
  case 0xad08:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__rect_src_xy_0108(val);
    else
      goto default_case;
    break;
  case 0xa50c:
    if (s3Card >= S3_ViRGE)
      xga.virge.bitblt_validate_port(port).set__rect_dst_xy_010c(val);
    else
      goto default_case;
    break;
  case 0xa90c:
    if (s3Card >= S3_ViRGE)
      xga.virge.line2d_validate_port(port).set__rect_dst_xy_010c(val);
    else
      goto default_case;
    break;
  case 0xad0c:
    if (s3Card >= S3_ViRGE)
      xga.virge.poly2d_validate_port(port).set__rect_dst_xy_010c(val);
    else
      goto default_case;
    break;
  default:
  default_case:
    if (port <= 0x4000) {
      ...
      LOG_MSG("XGA: Wrote to port %x with %x, len %x", (int)port, (int)val,
              (int)len);
    break;
  }
  ...
  if (s3Card >= S3_ViRGE) {
    ...
    case 0xA400: {
      ...
      if (rset.command_execute_on_register != 0 &&
          rset.command_execute_on_register == (port & 0x3FF))
...
Bitu XGA_Read(Bitu port, Bitu len) {
  ...
  case 0x8504:              // S3 ViRGE Subsystem Status Register
    if (s3Card >= S3_ViRGE) /* HACK: Always say S3D ENGINE is IDLE, or else
                               Windows 98 will hang at startup */
      return (16 << 8) /*S3D FIFO SLOTS FREE*/ | 0x2000 /*S3D ENGINE IDLE*/;
    else
      return 0xffffffff;
  case 0x850c:              // S3 ViRGE Advanced Function Control Register
    if (s3Card >= S3_ViRGE) /* HACK: Always say not busy, or Windows 98 will
                               hang at startup */
      return (8 << 6) /*COMMAND FIFO STATUS*/ | 1 /*Enable SVGA*/ |
             0x10 /*Linear address enable*/;
    else
      return 0xffffffff;
  case 0x83da: {
...
void VGA_SetupXGA(void) {
  ...
  /* FIXME: ViRGE cards like 24bpp rather than 32bpp? Or is that just Windows
   * driver laziness? Leave the option open for 32bpp ViRGE acceleration. */
