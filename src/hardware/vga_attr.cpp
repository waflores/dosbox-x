...
static const uint8_t InColorRGBI[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                        0x06, 0x07, 0x38, 0x39, 0x3A, 0x3B,
                                        0x3C, 0x3D, 0x3E, 0x3F};

static const uint8_t InColorRGBIMono[16] = {0x00, 0x07, 0x07, 0x07, 0x07, 0x07,
                                            0x07, 0x07, 0x38, 0x3F, 0x3F, 0x3F,
                                            0x3F, 0x3F, 0x3F, 0x3F};

EGAMonitorMode egaMonitorMode(void) { return currentMonitorMode; }
...
void VGA_ATTR_SetEGAMonitorPalette(EGAMonitorMode m) {
  ...
  case MONO:
    ...
    for (Bitu i = 0; i < 64; i++) {
      ...
      vga.dac.rgb[i].red = vga.dac.rgb[i].green = vga.dac.rgb[i].blue = value;
    }
    break;
  }
...
void VGA_ATTR_SetPalette(uint8_t index, uint8_t val) {
  ...
  if (IS_VGA_ARCH) {
    ...
    //   color index with the entire color select register. COPPER.EXE line
    //   fading tricks will not work correctly otherwise.
    //
    // - If P54S is not enabled, then do not apply any Color Select register
    // bits.
    //   This is contrary to standard VGA behavior that would always apply Color
    //   Select bits 3-2 to index bits 7-6 in any mode other than 256-color
    //   mode.
    ...
    //
    // - otherwise, if P54S is enabled, replace bits 5-4 with bits 1-0 of color
    // select.
    //
    ...
    // If the card is in graphics mode, then pay attention only to bit 4 to
    // enable color palette. If the card is in text mode, then bit 4 must be set
    // AND bit 5 must be clear to enable color palette. bit 4 = enable palette
    // bit 5 = set for MDA attributes, clear for CGA attributes.
    //
    // NTS: For some strange reason, most programs that support InColor
    // including Microsoft Flight Simulator and Framework III like to write 0x30
    // to the Hercules "exception" register, which is a strange value to write
    // because it means you want both MDA attributes and a color palette.
    // However if you think about it, CGA/MDA attributes have no meaning in
    // graphics mode, because it isn't text mode. Framework III however will
    // write 0x30 to this register while using the InColor/HGC+ 2-color RAMFONT
    // driver. Even though that sets the bit for a color palette, at no time
    // does Framework III ever write a color palette. That would mean that if
    // Framework III had actually managed to enable the color palette, it would
    // display with whatever random garbage happened to exist in the hardware
    // palette, and I don't think such a program would let that stand in that
    // kind of corporate office work type software that probably cost a fair
    // amount in its day.
...
void write_p3c0(Bitu /*port*/, Bitu val, Bitu iolen) {
  ...
  if (!vga.internal.attrindex) {
     ...
     *
     *          If we act on all writes as a cue to render a scanline, what will
     * likely happen is the scanline will be rendered blank. The reason is that
     * when writing the palette in the Attribute Controller you are supposed to
     * set bit 0x20 to disable the screen while you do so. Acting upon all
     * writes will likely lead to a render while screen is disabled. Therefore,
     * only trigger when the screen is enabled.
     *
     *          This means that render on demand will probably not 100% render
     * things correctly for very timing dependent tricks, but will render most
     * DOS games with better performance which for most casual users of DOSBox
     * and DOSBox-X is just fine. Just don't expect this performance enhancement
     * to handle all the tricks of the Demoscene. --J.C. */
    if (vga_render_on_demand && !attr(disabled) /*screen not yet disabled*/)
    ...
    if (val & 0x20)
      ...
      attr(disabled) |= 1;
    /*
       0-4	Address of data register to write to port 3C0h or read from port
       3C1h 5	If set screen output is enabled and the palette can not be
       modified, if clear screen output is disabled and the palette can be
       modified.
       */
    ...
    if (vga_render_on_demand && !attr(disabled) /*screen not disabled*/)
      VGA_RenderOnDemandUpTo();
    ...
    case 0x0f:
      ...
      /*
         0-5	Index into the 256 color DAC table. May be modified by 3C0h
         index 10h and 14h.
         */
      break;
    case 0x10: { /* Mode Control Register */
      ...
      if ((difference & 0x40) &&
          (vga.mode == M_VGA)) // 8BIT changes in 256-color mode must be handled
        VGA_StartResize(50);
      ...
      /*
         ...
         5	If set the PEL panning register (3C0h index 13h) is temporarily
         set to 0 from when the line compare causes a wrap around until the next
         ...
         7	If set bit 4-5 of the index into the DAC table are taken from
         port 3C0h index 14h bit 0-1, else the bits in the palette register are
         used.
         */
      break;
    }
    case 0x11: /* Overscan Color Register */
      ...
      /* 0-5  Color of screen border. Color is defined as in the palette
       * registers. */
      break;
    ...
    case 0x14: /* Color Select Register */
      ...
      /*
         0-1	If 3C0h index 10h bit 7 is set these 2 bits are used as bits 4-5
of the index into the DAC table. 2-3	These 2 bits are used as bit 6-7 of the
index into the DAC table except in 256 color mode. Note: this register does not
...
*/
    ...
    default:
      ...
      LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:Write to unknown Index %2X",
                                   attr(index));
      break;
    }
  }
...
Bitu read_p3c1(Bitu /*port*/, Bitu iolen) {
  ...
  default:
    ...
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:ATTR:Read from unknown Index %2X",
                                 attr(index));
  }
  return 0;
}
