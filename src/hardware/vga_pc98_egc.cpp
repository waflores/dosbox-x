...
Bitu pc98_egc4a0_read(Bitu port, Bitu iolen) {
  ...
  if (!(pc98_gdc_vramop & (1 << VOPBIT_EGC))) {
    //        LOG_MSG("EGC 4A0 read port 0x%x when EGC not enabled",(unsigned
    //        int)port);
    return ~0ul;
  }
...
void pc98_egc4a0_write(Bitu port, Bitu val, Bitu iolen) {
   ...
   * Is that true? */
  if (!(pc98_gdc_vramop & (1 << VOPBIT_EGC))) {
    //        LOG_MSG("EGC 4A0 write port 0x%x when EGC not enabled",(unsigned
    //        int)port);
    return;
  }
  ...
  case 0x0: /* 0x4A0 */
    /* bits [15:8] = 0xFF
     * bits [7:0] = enable writing to plane (NTS: only bits 3-0 have meaning in
     * 16-color mode). as far as I can tell, bits [7:0] correspond to the same
     * enable bits as port 0x7C [3:0] */
  ...
  case 0xE: /* 0x4AE */
    ...
    break;
  default:
    // LOG_MSG("PC-98 EGC: Unhandled write to 0x%x val 0x%x",(unsigned
    // int)port,(unsigned int)val);
    break;
  }
}

// I/O access to 0x4A0-0x4AF must be WORD sized and even port, or the system
// hangs if you try.
Bitu pc98_egc4a0_read_warning(Bitu port, Bitu iolen) {
  ...
  LOG_MSG("PC-98 EGC warning: I/O read from port 0x%x (len=%u) known to "
          "possibly hang the system on real hardware",
...
void pc98_egc4a0_write_warning(Bitu port, Bitu val, Bitu iolen) {
  ...
  switch (port & 0xF) {
  case 0x6:
    /* if the BIOS reports EGC, many early games will write bytewise I/O to port
     * 4A6h */
    pc98_egc_foreground_color = (uint8_t)val;
    pc98_egc_fgcm[0].w = (val & 1) ? 0xFFFF : 0x0000;
    pc98_egc_fgcm[1].w = (val & 2) ? 0xFFFF : 0x0000;
    ...
    break;
  default:
    LOG_MSG("PC-98 EGC warning: I/O write to port 0x%x (val=0x%x len=%u) known "
            "to possibly hang the system on real hardware",
