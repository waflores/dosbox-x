...
static sn76496_device device_sn76496(machine_config(), nullptr, nullptr,
                                     SOUND_CLOCK);
static ncr8496_device device_ncr8496(machine_config(), nullptr, nullptr,
                                     SOUND_CLOCK);
...
static void SN76496Write(Bitu /*port*/, Bitu data, Bitu /*iolen*/) {
  ...
  // this hack allows sample accurate rendering without enabling sample accurate
  // mode in the mixer.
  tandy.chan->FillUp();
...
static void SN76496Update(Bitu length) {
  ...
  static_cast<device_sound_interface &>(device).sound_stream_update(
      stream, nullptr, &outputs, (int)length);
    ...
    if (tandy.dac.frequency != 0) {
      ...
      if ((tandy.dac.mode & 0x0c) == 0x0c) {
        ...
        if (tandy.dac.dma.chan) {
          ...
          //					LOG_MSG("Tandy DAC: playback
          //started with frequency %f, volume %f",freq,vol);
        }
      }
    }
    break;
...
static void TandyDACDMAEnabled(void) { TandyDACModeChanged(); }
...
static void TandyDACWrite(Bitu port, Bitu data, Bitu /*iolen*/) {
  ...
  case 0xc6:
    tandy.dac.frequency =
        (tandy.dac.frequency & 0xf00) | (uint8_t)(data & 0xff);
    switch (tandy.dac.mode & 3) {
    case 0:
      // joystick mode
      break;
  ...
  case 0xc7:
    tandy.dac.frequency =
        (tandy.dac.frequency & 0x00ff) | (((uint8_t)(data & 0xf)) << 8);
...
static Bitu TandyDACRead(Bitu port, Bitu /*iolen*/) {
  ...
  case 0xc7:
    return (uint8_t)(((tandy.dac.frequency >> 8) & 0xf) |
                     (tandy.dac.amplitude << 5));
  }
...
static void TandyDACUpdate(Bitu length) {
  if (tandy.dac.enabled && ((tandy.dac.mode & 0x0c) == 0x0c)) {
     ...
     * jumping to silence quickly here causes loud annoying pops in Prince of
     * Persia when sound effects play. Prince of Persia seems to emit some very
     * non-zero DC value (like 0x08) when not playing sound effects, so this is
     * needed to recover from that without pops. This behavior also seems to
     * match DAC behavior with Prince of Persia according to real Tandy 1000
     * TL/3 hardware, complete with the 8-bit DAC stepping noise on return to
     * zero. In any case, this modification makes the Tandy DAC sound effects in
     * Prince of Persia much more enjoyable to listen to and match the apparent
     * sound of real hardware. */
    /* NTS: This doesn't quite make the DAC step noise heard on real hardware,
    ...
    for (Bitu ct = 0; ct < length; ct++) {
      ...
      if (tandy.dac.dma.last_sample != 128)
        tandy.dac.dma.last_sample =
            (uint8_t)(((((int)tandy.dac.dma.last_sample - 128) * 63) / 64) +
                      128);
    }
  } else {
...
public:
  TANDYSOUND(Section *configuration) : Module_base(configuration) {
    ...
    if (enable_hw_tandy_dac) {
      ...
      tandy.dac.chan =
          MixerChanDAC.Install(&TandyDACUpdate, sample_rate, "TANDYDAC");
...
void TANDYSOUND_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(TANDYSOUND_OnReset));
}
...
void POD_Save_Tandy_Sound(std::ostream &stream) {
  ...
  for (int lcv = 0; lcv < 8; lcv++) {
    if (tandy.dac.dma.chan == GetDMAChannel(lcv)) {
      dma_idx = lcv;
      break;
    }
  }
...
void POD_Load_Tandy_Sound(std::istream &stream) {
  ...
  if (dma_idx != 0xff)
    tandy.dac.dma.chan = GetDMAChannel(dma_idx);
