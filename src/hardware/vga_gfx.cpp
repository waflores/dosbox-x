...
static Bitu read_p3ce(Bitu /*port*/, Bitu /*iolen*/) { return gfx(index); }
...
static void write_p3cf(Bitu /*port*/, Bitu val, Bitu iolen) {
  ...
  case 0: /* Set/Reset Register */
    ...
    vga.config.full_enable_and_set_reset =
        vga.config.full_set_reset & vga.config.full_enable_set_reset;
    /*
  ...
  case 2: /* Color Compare Register */
    ...
    /*
       0-3	In Read Mode 1 each pixel at the address of the byte read is
       compared to this color and the corresponding bit in the output set to 1
       if they match, 0 if not. The Color Don't Care Register (3CEh index 7) can
       exclude bitplanes from the comparison.
     */
  ...
  case 3: /* Data Rotate */
    ...
    cmplx |=
        vga.complexity.setf(VGACMPLX_ROPROT, vga.config.data_rotate != 0 ||
    ...
    /*
       ...
       3-4	In Write Mode 2 this field controls the relation between the
       data written from the CPU, the data latched from the previous read and
       the data written to display memory: 0: CPU Data is written unmodified 1:
       CPU data is ANDed with the latched data 2: CPU data is ORed  with the
       latch data. 3: CPU data is XORed with the latched data.
     */
    break;
  ...
  case 5: /* Mode Register */
    ...
    cmplx |= vga.complexity.setf(VGACMPLX_COLORDONTCARE,
                                 gfx(color_dont_care) != 0 &&
                                     vga.config.read_mode == 1);
    cmplx |=
        vga.complexity.setf(VGACMPLX_WRITEMODE, vga.config.write_mode != 0);
  ...
  case 8: /* Bit Mask Register */
    ...
    cmplx |= vga.complexity.setf(
        ...
        vga.config.full_bit_mask !=
            0xFFFFFFFFu); // at least one bit will be masked off?

    /* check for unusual use of the bit mask register in chained 320x200x256
     * mode and switch to the slow & accurate emulation */
    ...
    /*
       0-7	Each bit if set enables writing to the corresponding bit of a
  ...
  default:
    ...
    if (gfx(index) == 9 && !index9warned) {
      LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:3CF:Write %2X to illegal index 9",
                                   (int)val);
      index9warned = true;
      break;
    }
    LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:3CF:Write %2X to illegal index %2X",
                                 (int)val, (int)gfx(index));
    break;
  }
}
...
static Bitu read_p3cf(Bitu port, Bitu iolen) {
  ...
  default:
    ...
    LOG(LOG_VGAMISC, LOG_NORMAL)("Reading from illegal index %2X in port %4X",
                                 (int)static_cast<uint32_t>(gfx(index)),
                                 (int)port);
    break;
  }
