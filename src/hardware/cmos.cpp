...
const uint8_t BIOS_DATE_months[] = {0,  31, 28, 31, 30, 31, 30,
                                    31, 31, 30, 31, 30, 31};
...
void cmos_sync_time(time_t t) {
  ...
  LOG(LOG_MISC, LOG_DEBUG)("CMOS sync to %04u-%02u-%02u %02u:%02u:%02u",
                           cmos.clock.year, cmos.clock.month, cmos.clock.day,
                           cmos.clock.hour, cmos.clock.min, cmos.clock.sec);
}
...
static void cmos_tick(void) {
  ...
  if (sync_time) {
    ...
    if (labs(dt) >= 5l) {
      ...
      if (dt >= 2l)
        cmos_sync_time(cmos.clock_time_t + 1l);
      else if (dt <= -2l)
        cmos_sync_time(cmos.clock_time_t - 1l);
      if (labs(dt) >= 2l) {
        cmos_sync_flag = true;
  ...
  if (++cmos.clock.sec < 60)
    return;
  cmos.clock.sec = 0;

  if (++cmos.clock.min < 60)
    return;
  cmos.clock.min = 0;

  if (++cmos.clock.hour < 24)
    return;
  cmos.clock.hour = 0;
  ...
  if (cmos.clock.month < 1 || cmos.clock.month > 12)
    cmos.clock.month = 1;
  uint8_t mdays = BIOS_DATE_months[cmos.clock.month];
  if (cmos.clock.month == 2 && cmos.clock.year % 4 == 0 &&
      (cmos.clock.year % 100 != 0 || cmos.clock.year % 400 == 0))
    mdays++; /* Feb 29th leap year */
  if (++cmos.clock.day < mdays)
    return;
  cmos.clock.day = 1;

  if (++cmos.clock.month < 12)
    return;
  cmos.clock.month = 1;
...
static void cmos_checkalarm(void) {
  if (cmos.clock.sec == cmos.alarm.sec && cmos.clock.min == cmos.alarm.min &&
      cmos.clock.hour == cmos.alarm.hour)
...
static void cmos_timerevent(Bitu val) {
  (void)val; // UNUSED
  {
    ...
    double remd = fma((index / (double)cmos.timer.delay),
                      -(double)cmos.timer.delay, index);
    // double remd = fmod(index, (double)cmos.timer.delay); // original delay
    // calculation double remd = index - trunc(index / (double)cmos.timer.delay)
    // * (double)cmos.timer.delay; // alternative fix LOG_MSG("cmos timerevent:
    // index=%f, interval=%f", index, cmos.timer.delay - remd);

    // Periodic Interrupt Flag (PF)
    if (cmos.regs[0xb] & 0x40)
      cmos.regs[0xc] |= 0x40;

    if (index >=
        (cmos.last.ended + 1000 -
         0.001)) { // consider sometimes index is slightly before 1.0sec
      // LOG_MSG("cmos timerevent: index=%f, interval=%f", index,
      // cmos.last.ended - index);
      if (cmos.last.ended < (index - 1000))
        cmos.last.ended = (index - 1000);
      cmos.last.ended -= fmod(cmos.last.ended, 1000);
      cmos.last.ended += 1000;

      if (!cmos.lock) {
        cmos_tick();
        if (cmos.regs[0xb] & 0x20 /*AIE*/)
          cmos_checkalarm();
      }

      // Update-Ended Interrupt Flag (UF)
      if (cmos.regs[0xb] & 0x10)
        cmos.regs[0xc] |= 0x10;
    }
...
static void cmos_checktimer(void) {
  ...
  double index = PIC_FullIndex();
  double remd =
      fma((index / (double)cmos.timer.delay), -(double)cmos.timer.delay, index);
  PIC_AddEvent(cmos_timerevent,
               (float)((double)cmos.timer.delay -
                       remd)); // Should be more like a real pc. Check
...
static void cmos_writereg(Bitu port, Bitu val, Bitu iolen) {
  (void)port;  // UNUSED
  (void)iolen; // UNUSED
  if (cmos.reg <= 0x09 || cmos.reg == 0x32 ||
      cmos.reg == 0x37) { // date/time related registers
    if (cmos.bcd)         // values supplied are BCD, convert to binary values
    {
      if ((val & 0xf0) > 0x90 || (val & 0x0f) > 0x09)
  ...
  case 0x00: /* Seconds */
    if (val > 59) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid second value %d for clock.",
                     (unsigned int)val);
      return;
  ...
  case 0x01: /* Seconds, alarm */
    if (val > 59) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid second value %d for alarm.",
                     (unsigned int)val);
      return;
    } // invalid second value
    cmos.alarm.sec = val;
  ...
  case 0x02: /* Minutes */
    if (val > 59) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid minute value %d for clock.",
                     (unsigned int)val);
      return;
  ...
  case 0x03: /* Minutes, alarm */
    if (val > 59) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid minute value %d for alarm.",
                     (unsigned int)val);
      return;
    } // invalid minute value
    cmos.alarm.min = val;
  ...
  case 0x04:       /* Hours */
    if (cmos.ampm) // 12h am/pm mode
    {
      if (val < 1 || (val > 12 && val < 0x81) || val > 0x8c) {
        LOG(LOG_BIOS,
            LOG_ERROR)("CMOS:Tried to write invalid hour value %d for clock.",
                       (unsigned int)val);
        return;
      } // invalid hour value
      if (val > 12)
        val -= (0x80 - 12); // convert pm to 24h
    } else                  // 24h mode
    {
      if (val > 23) {
        LOG(LOG_BIOS,
  ...
  case 0x05:       /* Hours, alarm */
    if (cmos.ampm) // 12h am/pm mode
    {
      if (val < 1 || (val > 12 && val < 0x81) || val > 0x8c) {
        LOG(LOG_BIOS,
            LOG_ERROR)("CMOS:Tried to write invalid hour value %d for alarm.",
                       (unsigned int)val);
        return;
      } // invalid hour value
      if (val > 12)
  ...
  case 0x06: /* Day of week */
    if (val < 1 || val > 7) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid day of week value %d.",
                     (unsigned int)val);
      return;
    } // invalid day of week value
    cmos.clock.weekday = val;
    break;
  case 0x07: /* Day of month */
    if (val < 1 || val > 31) {
      LOG(LOG_BIOS,
          LOG_ERROR)("CMOS:Tried to write invalid day of month value %d.",
                     (unsigned int)val);
      return;
  ...
  case 0x08: /* Month */
    if (val < 1 || val > 12) {
      LOG(LOG_BIOS, LOG_ERROR)("CMOS:Tried to write invalid month value %d.",
                               (unsigned int)val);
      return;
    } // invalid month value
    cmos.clock.month = val;
    break;
  case 0x09: /* Year */
    if (val > 99) {
      LOG(LOG_BIOS, LOG_ERROR)("CMOS:Tried to write invalid year value %d.",
                               (unsigned int)val);
      return;
    } // invalid year value
  ...
  case 0x37: /* Century (alternate used by Windows NT/2000/XP) */
    if (val < 19) {
      LOG(LOG_BIOS, LOG_ERROR)("CMOS:Tried to write invalid century value %d.",
                               (unsigned int)val);
      return;
    } // invalid century value
    cmos.clock.year %= 100;
  ...
  case 0x0b: /* Status reg B */
  {
    ...
    if (cmos.lock)
      val &= ~0x10; /* Setting bit 7 clears bit 4 (UEI) */
...
unsigned char CMOS_GetShutdownByte() { return cmos.regs[0x0F]; }

#define MAKE_RETURN(_VAL)                                                      \
  ((unsigned char)(cmos.bcd ? (((((unsigned int)_VAL) / 10U) << 4U) |          \
                               (((unsigned int)_VAL) % 10U))                   \
                            : ((unsigned int)_VAL)))

/* NOTES: Some implementations of the RTC and how they differ.
 *
 *        Compaq Elite 486 laptop: If you write port 70h, you are allowed *ONE*
 * read or write to 71h. If you read or write again, it is ignored until you
 * write port 70h again. Here at DOSBox-X, as many DOSBox forks are, we're not
 * that stingy.
 */
...
static Bitu cmos_readreg(Bitu port, Bitu iolen) {
  ...
  (void)iolen; // UNUSED
  if (cmos.reg > 0x3f) {
    LOG(LOG_BIOS, LOG_ERROR)("CMOS:Read attempted from illegal register %x",
                             cmos.reg);
    return 0xff;
  }
  ...
  case 0x0c: /* Status register C */
  {
    ...
    if (((cmos.regs[0xb] & 0x40) > 0) &&
        ((cmos.regs[0xc] & 0x40) > 0)) { // If both PF and PIE are 1
      val |= 0x80 | 0x40; // Set Interrupt Request Flag (IRQF) to 1, PF = 1
    }
    if (((cmos.regs[0xb] & 0x10) > 0) &&
        ((cmos.regs[0xc] & 0x10) > 0)) { // If both UF and UIE are 1
      val |= 0x80 | 0x10; // Set Interrupt Request Flag (IRQF) to 1, UF = 1
    }
    if (((cmos.regs[0xb] & 0x20) > 0) &&
        ((cmos.regs[0xc] & 0x20) > 0)) { // If both AF and AIE are 1
  ...
  case 0x12:
    /* NTS: DOSBox 0.74 mainline has these backwards: the upper nibble is the
       first hard disk, the lower nibble is the second hard disk. It makes a big
       difference to stupid OS's like Windows 95. */
    hdparm = 0;
  ...
  case 0x31: /* Extended memory in KB High Byte */
             //      LOG(LOG_BIOS,LOG_NORMAL)("CMOS:Read from reg %X :
             //      %04X",cmos.reg,cmos.regs[cmos.reg]);
...
void CMOS_SetRegister(Bitu regNr, uint8_t val) { cmos.regs[regNr] = val; }
...
void CMOS_Reset(Section *sec) {
  ...
  cmos.last.ended =
      PIC_FullIndex() - 1000; /* PIC time resets when the guest VM is reset */
  ...
  if (exsize >= 1024)
    ...
    exsize = 0;
  if (exsize > 65535)
    exsize = 65535; /* cap at 64MB. this value is returned as-is by INT 15H
                       AH=0x88 in a 16-bit register */
