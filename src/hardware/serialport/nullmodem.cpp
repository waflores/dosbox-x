...
CNullModem::CNullModem(Bitu id, CommandLine *cmd) : CSerial(id, cmd) {
  ...
  //           otherwise, connections not coming from localhost are rejected for
  //           security reasons.
    ...
    if (cmd->FindStringBegin("server:", tmpstring, false)) {
      ...
      if (dtrrespect) {
        ...
        LOG_MSG("Serial%d: Waiting for DTR...", (int)COMNUMBER);
                     ...
                     socketType, (char *)hostnamebuffer, clientport)))
        return;
    } else {
      // we are a server
...
Bits CNullModem::readChar(uint8_t &val) {
  ...
  if (telnet && rxchar >= 0)
    ...
    Bits rxchar = val;
    if (rxchar == 0xff)
      return rxchar; // 0xff 0xff -> 0xff was meant
...
bool CNullModem::ServerConnect() {
  ...
  /* FIXME: It would be nice if the SDL net library had a bind() call to bind
   * only to a specific interface. Or maybe it does... what am I missing? */
  if (!nonlocal && strcmp((char *)peeripbuf, "127.0.0.1") != 0) {
    LOG_MSG("Serial%d: Non-localhost client (%s) dropped by nonlocal:0 policy. "
            "To accept connections from network, set nonlocal:1",
            (int)COMNUMBER, peeripbuf);
    delete clientsocket;
    clientsocket = NULL;
    return false;
  }
      ...
      if (!CanReceiveByte()) {
        ...
        if (rx_retry >= rx_retry_max) {
          ...
          if (doReceive()) {
            ...
            rx_state = N_RX_IDLE;
#if SERIAL_DEBUG
            log_ser(dbg_aux, "Nullmodem: unblock due to no more data",
                    rx_retry);
#endif
          }
        } // else wait further
    ...
    case N_RX_FASTWAIT:
      if (CanReceiveByte()) {
        ...
        if (rx_state == N_RX_BLOCKED)
          ...
          log_ser(dbg_aux, "Nullmodem: block on continued rx (retry=%d).",
                  rx_retry);
#endif
        setEvent(SERIAL_RX_EVENT, bytetime * 0.65f);
        rx_state = N_RX_BLOCKED;
  ...
  if (telClient.inIAC) {
    if (telClient.recCommand) {
      if ((data != 0) && (data != 1) && (data != 3)) {
        LOG_MSG("Serial%d: Unrecognized telnet option %d", (int)COMNUMBER,
                data);
        if (telClient.command > 250) {
          ...
          if (clientsocket)
            clientsocket->SendArray(response, 3);
        }
      }
...
void CNullModem::setRTSDTR(bool xrts, bool xdtr) {
  if (!transparent) {
    ...
    if (LCR & LCR_BREAK_MASK)
      control[1] |= 4;
    if (clientsocket)
      clientsocket->SendArray(control, 2);
  }
}
