...
bool device_COM::Close() { return false; }
...
device_COM::~device_COM() {
  /* clear reference to myself so that we're not deleted twice (once by
   * DOS_DelDevice the other by CSerial) */
  if (sclass != NULL && sclass->mydosdevice == this)
    sclass->mydosdevice = NULL;
}

// COM1 - COM9 objects
CSerial *serialports[9] = {nullptr, nullptr, nullptr, nullptr, nullptr,
...
#if SERIAL_DEBUG
  const char *const dbgtext[] = {"RHR", "IER", "ISR", "LCR", "MCR",
                                 "LSR", "MSR", "SPR", "DLL", "DLM"};
...
#if SERIAL_DEBUG
  const char *const dbgtext[] = {"THR",  "IER", "FCR", "LCR", "MCR",
                                 "!LSR", "MSR", "SPR", "DLL", "DLM"};
  if (serialports[i]->dbg_register) {
    Bitu debugindex = index;
    if ((index < 2) && ((serialports[i]->LCR) & LCR_DIVISOR_Enable_MASK))
      debugindex += 8;
    serialports[i]->log_ser(serialports[i]->dbg_register, "write 0x%2x to %s.",
                            val, dbgtext[debugindex]);
  }
#endif
  switch (index) {
  case THR_OFFSET:
...
void CSerial::log_ser(bool active, char const *format, ...) {
  if (active) {
    ...
    char buf[512];
    buf[0] = 0;
    sprintf(buf, "%12.3f [%7u] ", (double)PIC_FullIndex(),
...
void CSerial::changeLineProperties() {
  ...
  if (baud_divider == 0)
    bitlen = (1000.0f / (115200.0f * baud_multiplier));
  else
    bitlen = (1000.0f / (115200.0f * baud_multiplier)) * (float)baud_divider;
  bytetime = bitlen * (float)(1 + 5 + 1); // startbit + minimum length + stopbit
  bytetime += bitlen * (float)(LCR & 0x3); // databits
  if (LCR & 0x4)
    bytetime += bitlen; // 2nd stopbit
  if (LCR & 0x8)
    bytetime += bitlen; // parity

#if SERIAL_DEBUG
  const char *const dbgtext[] = {"none", "odd",  "none", "even",
                                 "none", "mark", "none", "space"};
  log_ser(
      dbg_serialtraffic,
      "New COM parameters: baudrate %5.0f, parity %s, wordlen %d, stopbits %d",
      1.0 / bitlen * 1000.0f, dbgtext[(LCR & 0x38) >> 3], (LCR & 0x3) + 5,
      ((LCR & 0x4) >> 2) + 1);
#endif
...
void CSerial::setEvent(uint16_t type, float duration) {
  PIC_AddEvent(Serial_EventHandler, duration,
               (Bitu)(((unsigned int)type << 4u) | (unsigned int)idnumber));
}
...
void CSerial::handleEvent(uint16_t type) {
  ...
  case SERIAL_ERRMSG_EVENT: {
    LOG_MSG("Serial%d: Errors: "
            ...
            (int)COMNUMBER, (int)framingErrors, (int)parityErrors,
            (int)overrunErrors, (int)overrunIF0, (int)txOverrunErrors,
            (int)breakErrors);
...
void CSerial::ComputeInterrupts() {
  ...
  if (val & ERROR_PRIORITY)
    ...
    ISR = ISR_FIFOTIMEOUT_VAL;
  else if (val & RX_PRIORITY)
    ISR = ISR_RX_VAL;
  else if (val & TX_PRIORITY)
    ISR = ISR_TX_VAL;
  else if (val & MSR_PRIORITY)
    ISR = ISR_MSR_VAL;
  else
    ISR = ISR_CLEAR_VAL;
...
/*****************************************************************************/
bool CSerial::CanReceiveByte() { return !rxfifo->isFull(); }
...
void CSerial::receiveByteEx(uint8_t data, uint8_t error) {
#if SERIAL_DEBUG
  log_ser(dbg_serialtraffic,
          data < 0x10 ? "\t\t\t\trx 0x%02x (%u)" : "\t\t\t\trx 0x%02x (%c)",
          data, data);
#endif
  if (!(rxfifo->addb(data))) {
    // Overrun error ;o
  ...
  if (error) {
    ...
    if (error & LSR_RX_BREAK_MASK)
      error |= LSR_FRAMING_ERROR_MASK;
#if SERIAL_DEBUG
    log_ser(dbg_serialtraffic,
            "with error: framing=%d,overrun=%d,break=%d,parity=%d",
            (error & LSR_FRAMING_ERROR_MASK) > 0,
            (error & LSR_OVERRUN_ERROR_MASK) > 0,
            (error & LSR_RX_BREAK_MASK) > 0,
            (error & LSR_PARITY_ERROR_MASK) > 0);
#endif
    if (FCR & FCR_ACTIVATE) {
...
void CSerial::receiveByte(uint8_t data) { receiveByteEx(data, 0); }
...
void CSerial::ByteTransmitting() {
  if (sync_guardtime) {
    ...
    // if(txfifo->isEmpty()) LOG_MSG("Serial port: FIFO empty when it should
    // not");
...
#if SERIAL_DEBUG
    log_ser(dbg_serialtraffic,
            data < 0x10 ? "\t\t\t\t\ttx 0x%02x (%u) (from buffer)"
                        : "\t\t\t\t\ttx 0x%02x (%c) (from buffer)",
            data, data);
#endif
    if (loopback)
      setEvent(SERIAL_TX_LOOPBACK_EVENT, bytetime);
    else
      transmitByte(data, false);
    if (txfifo->isEmpty())
      rise(TX_PRIORITY);
...
void CSerial::Write_THR(uint8_t data) {
  ...
  if (LCR & LCR_DIVISOR_Enable_MASK) {
    ...
    if (LSR & LSR_TX_EMPTY_MASK) { // we were idle before
      ...
      LSR &= (~LSR_TX_EMPTY_MASK);
      if (loopback)
        setEvent(SERIAL_THR_LOOPBACK_EVENT, bytetime / 10);
      else {
#if SERIAL_DEBUG
        log_ser(dbg_serialtraffic,
                data < 0x10 ? "\t\t\t\t\ttx 0x%02x (%u) [FIFO=%2d]"
                            : "\t\t\t\t\ttx 0x%02x (%c) [FIFO=%2d]",
...
void CSerial::Write_IER(uint8_t data) {
  if (LCR & LCR_DIVISOR_Enable_MASK) { // write to DLM
    ...
    IER = data & 0xF;
    if ((FCR & FCR_ACTIVATE) && data & RX_PRIORITY)
      IER |= TIMEOUT_PRIORITY;
    ComputeInterrupts();
  }
}
...
Bitu CSerial::Read_ISR() {
  ...
  if (IER & Modem_Status_INT_Enable_MASK)
    updateMSR();
  uint8_t retval = ISR;
  ...
  if (ISR == ISR_TX_VAL)
    clear(TX_PRIORITY);
  if (FCR & FCR_ACTIVATE)
    retval |= FIFO_STATUS_ACTIVE;

  return retval;
}

#define BIT_CHANGE_H(oldv, newv, bitmask)                                      \
  (!(oldv & bitmask) && (newv & bitmask))
#define BIT_CHANGE_L(oldv, newv, bitmask)                                      \
  ((oldv & bitmask) && !(newv & bitmask))
...
void CSerial::Write_FCR(uint8_t data) {
  ...
  if (FCR & FCR_ACTIVATE) {
    ...
    case 0:
      ...
      break;
    case 1:
      rx_interrupt_threshold = 4;
      break;
    case 2:
      rx_interrupt_threshold = 8;
      break;
    case 3:
      rx_interrupt_threshold = 14;
      break;
    }
  }
}
...
#if SERIAL_DEBUG
    log_ser(dbg_serialtraffic,
            ((LCR & LCR_BREAK_MASK) != 0) ? "break on." : "break off.");
#endif
  }
}
...
Bitu CSerial::Read_MCR() {
  ...
  if (op2)
    retval |= MCR_OP2_MASK;
  if (loopback)
    retval |= MCR_LOOPBACK_Enable_MASK;
  return retval;
}

void CSerial::Write_MCR(uint8_t data) {
  // WARNING: At the time setRTSDTR is called rts and dsr members are still
  // wrong.
  if (data & FIFO_FLOWCONTROL)
    LOG_MSG("Warning: tried to activate hardware handshake.");
...
Bitu CSerial::Read_MSR() {
  ...
  if (d_ri)
    retval |= MSR_dRI_MASK;
  if (d_cts)
    retval |= MSR_dCTS_MASK;
  if (d_dsr)
    retval |= MSR_dDSR_MASK;
...
Bitu CSerial::Read_SPR() { return SPR; }

void CSerial::Write_SPR(uint8_t data) { SPR = data; }
...
/* MSR Access                                                               **/
/*****************************************************************************/
bool CSerial::getRI() { return ri; }

bool CSerial::getCD() { return cd; }

bool CSerial::getDSR() { return dsr; }

bool CSerial::getCTS() { return cts; }
...
CSerial::CSerial(Bitu id, CommandLine *cmd) {
  ...
  if (cmd->FindExist("dbgall", false)) {
    dbg_serialtraffic = dbg_modemcontrol = dbg_register = dbg_interrupt =
        dbg_aux = true;
  }

  if (dbg_serialtraffic | dbg_modemcontrol | dbg_register | dbg_interrupt |
      dbg_aux)
...
void CSerial::registerDOSDevice() {
  if (mydosdevice == NULL) {
    LOG(LOG_MISC, LOG_DEBUG)("COM%d: Registering DOS device",
                             (int)idnumber + 1);
...
void CSerial::unregisterDOSDevice() {
  if (mydosdevice != NULL) {
    LOG(LOG_MISC, LOG_DEBUG)("COM%d: Unregistering DOS device",
                             (int)idnumber + 1);
...
bool CSerial::Getchar(uint8_t *data, uint8_t *lsr, bool wait_dsr,
                      Bitu timeout) {
  ...
  while ((!((*lsr = (uint8_t)Read_LSR()) & 0x1)) &&
...
bool CSerial::Putchar(uint8_t data, bool wait_dsr, bool wait_cts,
                      Bitu timeout) {
...
void BIOS_Post_register_comports() {
  ...
  for (i = 0; i < 9; i++) {
    if (serialports[i] != NULL && serial_baseaddr[i])
      BIOS_SetCOMPort(
          i, serial_baseaddr[i]); // This works only for ports COM1 to COM4.
                                  // There is no space to store more addresses
  }
}
...
public:
  SERIALPORTS(Section *configuration) : Module_base(configuration) {
    ...
    //       COM1 is a 8251 UART, while COM2 and higher if they exist are
    //       8250/16xxx UARTs
    if (IS_PC98_ARCH)
      return;
    ...
    for (uint8_t i = 0; i < 9; i++) {
      ...
      CommandLine tmp(nullptr, p->GetSection()->Get_string("parameters"),
                      CommandLine::either, true);
      double multiplier = 1;
      std::string str;
      bool squote = false;
...
static const char *serialTypes[SERIAL_TYPE_COUNT] = {
    "disabled",     "dummy",    "log", "file", "serialmouse",
#if C_DIRECTSERIAL
    "directserial",
#endif
#if C_MODEM
...
void SERIAL::Run() {
  ...
  if (cmd->FindExist("-?", false) || cmd->FindExist("/?", false)) {
    ...
    for (int x = 0; x < SERIAL_TYPE_COUNT; x++) {
      ...
      if (x < SERIAL_TYPE_COUNT - 1)
        WriteOut(", ");
    }
    WriteOut("\n option Serial options, if any (see [serial] section of the "
             "configuration).\n");
    return;
  }
  // Select COM mode.
  ...
  if (cmd->GetCount() >= 2) {
    ...
    CommandLine tmp("SERIAL.COM", commandLineString.c_str(),
                    CommandLine::either, true);
    std::string str;
    bool squote = false;
    ...
    if (cmd.FindStringBegin("multiplier:", str, false)) {
      ...
      if (baud_multiplier < 1)
        baud_multiplier = 1;
      if (baud_multiplier > 1000000)
        baud_multiplier = 1000000;
    }
...
void SERIAL_ProgramStart(Program **make) { *make = new SERIAL; }
...
void SERIAL_OnReset(Section *sec) {
  ...
  // FIXME: Unregister/destroy the DOS devices, but consider that the DOS kernel
  // at reset is gone.
  for (i = 0; i < 9; i++) {
    if (serialports[i] != NULL)
      serialports[i]->unregisterDOSDevice();
...
void SERIAL_Init() {
  ...
  if (!IS_PC98_ARCH) {
    AddVMEventFunction(VM_EVENT_POWERON,
                       AddVMEventFunctionFuncPair(SERIAL_OnPowerOn));
    AddVMEventFunction(VM_EVENT_RESET,
                       AddVMEventFunctionFuncPair(SERIAL_OnReset));
    AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                       AddVMEventFunctionFuncPair(SERIAL_OnDOSKernelExit));
    AddVMEventFunction(VM_EVENT_DOS_INIT_KERNEL_READY,
                       AddVMEventFunctionFuncPair(SERIAL_OnDOSKernelInit));
  }
}
