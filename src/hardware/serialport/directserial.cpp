...
CDirectSerial::CDirectSerial(Bitu id, CommandLine *cmd) : CSerial(id, cmd) {
...
// D_RX_IDLE + not CanReceive				-> D_RX_BLOCKED, add
// RX_EVENT
...
//											   rx_retry=0
// D_RX_BLOCKED + !CanReceive + doReceive + retry < max	-> D_RX_BLOCKED,
// rx_retry++ D_RX_BLOCKED + !CanReceive + doReceive + retry >=max	->
// rx_retry=0
    ...
    case D_RX_FASTWAIT:
      if (CanReceiveByte()) {
        ...
        if (rx_state == D_RX_BLOCKED) {
          if (!dbgmsg_rx_block) {
            log_ser(dbg_aux, "Directserial: rx still blocked (retry=%d)",
                    rx_retry);
          ...
          log_ser(dbg_aux, "Directserial: block on continued rx (retry=%d).",
                  rx_retry);
#endif
        setEvent(SERIAL_RX_EVENT, bytetime * 0.65f);
        rx_state = D_RX_BLOCKED;
...
void CDirectSerial::updatePortConfig(uint16_t divider, uint8_t lcr) {
  ...
  if (divider == 0)
    ...
    baudrate = 115200u / divider;

  // stopbits
  uint8_t stopbits;
  if (lcr & 0x4) {
    if (bytelength == 5)
      stopbits = SERIAL_15STOP;
    else
      stopbits = SERIAL_2STOP;
  } else
    stopbits = SERIAL_1STOP;

  if (!SERIAL_setCommParameters(comport, (int)baudrate, (char)parity,
                                (char)stopbits, (char)bytelength)) {
#if SERIAL_DEBUG
    log_ser(dbg_aux, "Serial port settings not supported by host.");
#endif
    LOG_MSG("Serial%d: Desired serial mode not supported (%d,%d,%c,%d)",
            (int)(COMNUMBER), (int)baudrate, (int)bytelength, parity,
            (int)stopbits);
  }
...
void CDirectSerial::setBreak(bool value) { SERIAL_setBREAK(comport, value); }
...
void CDirectSerial::setRTS(bool val) { SERIAL_setRTS(comport, val); }

void CDirectSerial::setDTR(bool val) { SERIAL_setDTR(comport, val); }
