...
static inline int GetTicksDiff(const int64_t new_ticks,
                               const int64_t old_ticks) {
...
NETClientSocket *NETClientSocket::NETClientFactory(SocketTypesE socketType,
  ...
  case SOCKET_TYPE_TCP:
    return new TCPClientSocket(destination, port);

#if defined(WITH_ENET_IMPLEMENTATION)
  case SOCKET_TYPE_ENET:
    return new ENETClientSocket(destination, port);
#endif
...
#if defined(WITH_ENET_IMPLEMENTATION)
  case SOCKET_TYPE_ENET:
    return new ENETServerSocket(port);
#endif
...
ENETClientSocket::ENETClientSocket(const char *destination, uint16_t port) {
  ...
  if (client == nullptr) {
    LOG_MSG("ENET: Unable to create socket to %s:%u", destination, port);
    return;
  }
  ...
  if (peer) {
    LOG_MSG("ENET: Initiating connection to server %s:%u", destination, port);
  } else {
    enet_host_destroy(client);
    client = nullptr;
    LOG_MSG("ENET: Unable to connect to server %s:%u", destination, port);
    return;
  }
  ...
  if (enet_host_service(client, &event, connection_timeout_ms) > 0 &&
    ...
    LOG_MSG("ENET: Established connection to server %s:%u", destination, port);
  } else {
    LOG_MSG("ENET: Failed connecting to server %s:%u", destination, port);
...
bool ENETClientSocket::SendArray(const uint8_t *data, size_t n) {
  ...
  const auto packet =
      enet_packet_create(data, packet_bytes, ENET_PACKET_FLAG_RELIABLE);
  ...
  assert(peer);
  if (enet_peer_send(peer, 0, packet) < 0) {
    LOG_MSG("ENET: Failed sending %u-byte packet to peer %s:%u", packet_bytes,
            enet_address_to_string(peer->address), peer->address.port);
    enet_packet_destroy(packet);
    return false;
  }
...
TCPClientSocket::TCPClientSocket(const char *destination, uint16_t port) {
  ...
  IPaddress openip;
  // Ancient versions of SDL_net had this as char*. People still appear to be
  // using this one.
...
bool TCPClientSocket::Putchar(uint8_t val) { return SendArray(&val, 1); }
