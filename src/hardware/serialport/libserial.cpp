...
bool SERIAL_open(const char *portname, COMPORT *port) {
  ...
  cp->porthandle =
      CreateFile(extended_portname, GENERIC_READ | GENERIC_WRITE, 0,
  ...
  if (cp->porthandle == INVALID_HANDLE_VALUE)
    goto cleanup_error;
...
void SERIAL_getErrorString(char *buffer, size_t length) {
  ...
  FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                ...
                (LPTSTR)&sysmessagebuffer, 0, NULL);
...
bool SERIAL_open(const char *portname, COMPORT *port) {
  ...
  termInfo.c_iflag &= ~(IGNBRK | BRKINT | ICRNL | INLCR | PARMRK | INPCK |
                        ISTRIP | IXON); // reset input controls
  termInfo.c_oflag &= ~OPOST;           // raw output
  termInfo.c_lflag &= ~(ECHO | ECHONL | ICANON | IEXTEN |
                        ISIG); // no line processing, raw input
  ...
  if (dwRead == 1) {
    if (chRead == 0xff) // error escape
    {
      ...
      if (chRead == 0x00) // an error
      {
        dwRead = read(port->porthandle, &chRead, 1);
        if (chRead == 0x0)
          error = SERIAL_BREAK_ERR;
        else
          error = SERIAL_FRAMING_ERR;
      }
    }
    retval |= (error << 8);
    retval |= chRead;
...
bool SERIAL_setCommParameters(COMPORT port, int baudrate, char parity,
  ...
  switch (baudrate) {
  case 115200:
    posix_baudrate = B115200;
    break;
  case 57600:
    posix_baudrate = B57600;
    break;
  case 38400:
    posix_baudrate = B38400;
    break;
  case 19200:
    posix_baudrate = B19200;
    break;
  case 9600:
    posix_baudrate = B9600;
    break;
  case 4800:
    posix_baudrate = B4800;
    break;
  case 2400:
    posix_baudrate = B2400;
    break;
  case 1200:
    posix_baudrate = B1200;
    break;
  case 600:
    posix_baudrate = B600;
    break;
...
bool SERIAL_open(const char *portname, COMPORT *port) {
  ...
  APIRET rc = DosOpen(
      (PSZ)portname, &cp->porthandle, &ulAction, 0L, FILE_NORMAL, FILE_OPEN,
      OPEN_ACCESS_READWRITE | OPEN_SHARE_DENYNONE | OPEN_FLAGS_SEQUENTIAL, 0L);
  if (rc != NO_ERROR) {
    goto cleanup_error;
  }

  rc = DosDevIOCtl(cp->porthandle, IOCTL_ASYNC, ASYNC_GETDCBINFO, 0, 0, 0,
                   &cp->orig_dcb, ulParmLen, &ulParmLen);
  if (rc != NO_ERROR) {
    goto cleanup_error;
  }
  // configure the port for polling
  ...
  rc = DosDevIOCtl(cp->porthandle, IOCTL_ASYNC, ASYNC_SETDCBINFO, &newdcb,
                   ulParmLen, &ulParmLen, 0, 0, 0);
  if (rc != NO_ERROR) {
    goto cleanup_error;
  }

  ulParmLen = sizeof(errors);
  rc = DosDevIOCtl(cp->porthandle, IOCTL_ASYNC, ASYNC_GETCOMMERROR, 0, 0, 0,
                   &errors, ulParmLen, &ulParmLen);
  if (rc != NO_ERROR) {
    goto cleanup_error;
  }
...
void SERIAL_setBREAK(COMPORT port, bool value) {
  ...
  DosDevIOCtl(port->porthandle, IOCTL_ASYNC,
              value ? ASYNC_SETBREAKON : ASYNC_SETBREAKOFF, 0, 0, 0, &error,
              ulParmLen, &ulParmLen);
}
...
int SERIAL_getextchar(COMPORT port) {
  ...
  if (DosRead(port->porthandle, &chRead, 1, &dwRead) == NO_ERROR) {
    if (dwRead) {
      ...
      DosDevIOCtl(port->porthandle, IOCTL_ASYNC, ASYNC_GETCOMMEVENT, 0, 0, 0,
                  &event, ulParmLen, &ulParmLen);
      if (event &
          (64 + 128)) { // Break (Bit 6) or Frame or Parity (Bit 7) error
        uint8_t errreg = 0;
        if (event & 64)
          retval |= SERIAL_BREAK_ERR;
        if (event & 128) {
          DosDevIOCtl(port->porthandle, IOCTL_ASYNC, ASYNC_GETCOMMERROR, 0, 0,
                      0, &errors, ulParmLen, &ulParmLen);
          if (errors & 8)
            retval |= SERIAL_FRAMING_ERR;
          if (errors & 4)
            retval |= SERIAL_PARITY_ERR;
        }
      }
      retval |= (chRead & 0xff);
      retval |= 0x10000;
...
int SERIAL_getmodemstatus(COMPORT port) {
  ...
  DosDevIOCtl(port->porthandle, IOCTL_ASYNC, ASYNC_GETMODEMINPUT, 0, 0, 0,
              &dptr, ulParmLen, &ulParmLen);
...
void SERIAL_setDTR(COMPORT port, bool value) {
  ...
  DosDevIOCtl(port->porthandle, IOCTL_ASYNC, ASYNC_SETMODEMCTRL, 0, 0, 0,
              &masks, ulParmLen, &ulParmLen);
}
...
bool SERIAL_setCommParameters(COMPORT port, int baudrate, char parity,
  ...
  rc = DosDevIOCtl(port->porthandle, IOCTL_ASYNC, ASYNC_SETLINECTRL, &paramline,
                   ulParmLen, &ulParmLen, 0, 0, 0);
  if (rc != NO_ERROR)
    return false;
  return true;
