...
bool MODEM_ReadPhonebook(const std::string &filename) {
  ...
  while (std::getline(loadfile, linein)) {
    ...
    LOG_MSG("SERIAL: Mapped phone %s to address %s", phone.c_str(),
            address.c_str());
...
void MODEM_ClearPhonebook() { phones.clear(); }
...
CSerialModem::~CSerialModem() {
  ...
  // remove events
  for (uint8_t i = SERIAL_BASE_EVENT_COUNT + 1; i <= SERIAL_MODEM_EVENT_COUNT;
       i++)
    removeEvent(i);
}
...
void CSerialModem::SendRes(ResTypes response) {
  ...
  if (doresponse != 1) {
    if (doresponse == 2 && (response == ResRING || response == ResCONNECT ||
...
bool CSerialModem::Dial(const char *host) {
  ...
  clientsocket =
      NETClientSocket::NETClientFactory(socketType, destination, port);
...
void CSerialModem::DoCommand() {
  ...
  while (1) {
    ...
    case '+':
      ...
      if (is_next_token("NET", scanbuf)) {
        ...
        if (telnetmode != static_cast<bool>(requested_mode)) {
          ...
          LOG_MSG("SERIAL: Port %u telnet-mode %s", (int)idnumber + 1,
      ...
      if (is_next_token("SOCK", scanbuf)) {
        ...
        // socket types.
        LOG_MSG("SERIAL: Port %u socket type %s", (int)idnumber + 1,
                socketType ? "ENet" : "TCP");
        // Reset port state.
        EnterIdleState();
        break;
    ...
    case 'D': { // Dial
      ...
      // scan for and remove whitespaces; weird bug: with leading spaces in the
      // string, SDLNet_ResolveHost will return no error but not work anyway
      // (win)
        ...
        if (isNum) {
          ...
          for (size_t i = 0; i < foundlen2; i++) {
            ...
            if (foundstr[i] == '0' && foundstr[i - 1] == '.')
              continue;
            if (foundstr[i] == '0' && foundstr[i - 1] == '0' &&
                foundstr[i - 2] == '.')
              continue;
    ...
    case 'I': // Some strings about firmware
      ...
      case 3:
        SendLine("DOSBox-X Emulated Modem Firmware V1.00");
        break;
      case 4:
        SendLine("Modem compiled for DOSBox-X version " VERSION);
        break;
      }
      break;
    case 'E': // Echo on/off
    ...
    case '&': { // & escaped commands
      ...
      case '\0':
        ...
        return;
      default:
        LOG_MSG("Modem: Unhandled command: &%c%u", cmdchar,
  ...
  for (i = 0; i < size; i++) {
    ...
    if (telClient.inIAC) {
      if (telClient.recCommand) {
        ...
        case 251: /* Will */
          if (c == 0)
            telClient.binary[TEL_SERVER] = true;
          if (c == 1)
            telClient.echo[TEL_SERVER] = true;
          if (c == 3)
            telClient.supressGA[TEL_SERVER] = true;
          break;
        case 252: /* Won't */
          if (c == 0)
            telClient.binary[TEL_SERVER] = false;
          if (c == 1)
            telClient.echo[TEL_SERVER] = false;
          if (c == 3)
            telClient.supressGA[TEL_SERVER] = false;
          break;
        case 253: /* Do */
          if (c == 0) {
