...
CSerialFile::CSerialFile(Bitu id, CommandLine *cmd, bool sq)
    : CSerial(id, cmd) {
  ...
  cmd->FindStringBegin("file:", filename,
                       false); // if the user specifies serial1=file
                               // file:something, set it to that
  ResolvePath(filename);
  LOG_MSG("Serial: port %d will write to file %s", int(id) + 1,
          filename.c_str());
...
void CSerialFile::transmitByte(uint8_t val, bool first) {
  ...
  if (timeout != 0)
    setEvent(SERIAL_TX_EVENT, (float)(timeout + 1));
  if (fp == NULL) {
...
void CSerialFile::setRTS(bool val) { setCTS(val); }
