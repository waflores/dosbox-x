...
/* this function is the method the GUI and mouse emulation notifies us of
 * movement/button events. if the serial mouse is not installed, it is ignored
 */
...
void CSerialMouse::start_packet() {
  ...
  if (mouse_delta_x < -128)
    mouse_delta_x = -128;
  else if (mouse_delta_x > 127)
    mouse_delta_x = 127;
  mouse_delta_x &= 0xFF;

  if (mouse_delta_y < -128)
    mouse_delta_y = -128;
  else if (mouse_delta_y > 127)
    mouse_delta_y = 127;
  mouse_delta_y &= 0xFF;

  /* NOTE TO SELF: Do NOT set bit 7. It confuses CTMOUSE.EXE (CuteMouse) serial
   * support. Leaving it clear is the only way to make mouse movement possible.
   *               Microsoft Windows on the other hand doesn't care if bit 7 is
   * set. */
  packet_xmit = 0;
  /* Byte 0:    X   1  LB  RB  Y7  Y6  X7  X6 */
  packet[0] = 0x40 | (mouse_buttons << 4) | (((mouse_delta_y >> 6) & 3) << 2) |
              ((mouse_delta_x >> 6) & 3);
  /* Byte 1:    X   0  X5-X0 */
...
void CSerialMouse::on_mouse_event(int delta_x, int delta_y,
                                  uint8_t buttonstate) {
   ...
   * is already transmitting now then wait for it to finish before transmitting
   * ours, and let the mouse motion accumulate in the meantime */
  if (packet_xmit >= 3)
    start_packet();
  else
...
CSerialMouse::CSerialMouse(Bitu id, CommandLine *cmd) : CSerial(id, cmd) {
  ...
  packet_xmit = 0xFF;
  mouse_buttons = 0;
  serial_mice[id] = this; /* <- NTS: 'id' is just the index of the serial mouse
                             0..3 for COM1...COM4 */
  xmit_another_packet = false;
  mouse_delta_x = mouse_delta_y = 0;
}
