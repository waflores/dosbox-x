...
static void write_crtc_data_other(Bitu /*port*/, Bitu val, Bitu /*iolen*/) {
  ...
  case 0x03: // Horizontal sync width
    if (machine == MCH_TANDY)
      vga.other.vsyncw = (uint8_t)(val >> 4);
    else
  ...
  case 0x04: // Vertical total
    if (vga.other.vtotal ^ val)
      VGA_StartResize();
    if (machine ==
        MCH_TANDY) // FIXME: This is needed for "Math Rabbit", but does Tandy
                   // hardware *really* use all 8 bits? Or is it limited to 7
                   // bits like MDA/CGA/Hercules/PCjr? This is default DOSBox
                   // SVN behavior.
  ...
  case 0x0C: /* Start Address High Register */
    ...
    if (machine == MCH_MCGA)
      vga.config.display_start =
          (vga.config.display_start & 0x00FF) | ((val & 0xFF) << 8);
    else
      vga.config.display_start =
          (vga.config.display_start & 0x00FF) | ((val & 0x3F) << 8);
    break;
  ...
  case 0x14: /* Hercules InColor and HGC+: xMode */
    if (hercCard == HERC_InColor || hercCard == HERC_GraphicsCardPlus) {
      // HGC+ and InColor bit 0 controls whether the RAM font at B4000h is drawn
      // in text mode, and bit 2 the 48k RAMfont mode.
      const uint8_t chg = vga.herc.xMode ^ (uint8_t)val;
      if (chg & 2 /*bit 1: change 80/90 column mode*/)
        VGA_StartResize();
  ...
  case 0x1C: /* Hercules InColor */
    if (hercCard == HERC_InColor) {
      ...
      if (++vga.herc.palette_index >= 0x10)
  ...
  unhandled:
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("MC6845:Write %X to illegal index %x",
                                 (int)val, (int)vga.other.index);
  }
}
static Bitu read_crtc_data_other(Bitu /*port*/, Bitu /*iolen*/) {
  ...
  case 0x03: // Horizontal and vertical sync width
    if (machine == MCH_TANDY)
      return (unsigned int)vga.other.hsyncw |
             (unsigned int)(vga.other.vsyncw << 4u);
    else
  ...
  unhandled:
  default:
    LOG(LOG_VGAMISC, LOG_NORMAL)("MC6845:Read from illegal index %x",
                                 vga.other.index);
  }
  return (Bitu)(~0);
}
...
static void write_crtc_data_mcga(Bitu port, Bitu val, Bitu iolen) {
  if (vga.other.index < 0x10) {
    ...
    if (vga.other.index <= 0x07 && crtc(read_only))
      return;
    ...
    case 0x10: /* MCGA Mode Control */
    {
       ...
       * bit 2: reserved
       * bit 3: 1=horizontal timing parameters computed in hardware for video
       * mode   0=...from timing in registers 0-3 bit 4: 1=enable dot clock bit
       * 5: reserved bit 6: inverse of bit 8 of vertical displayed register 0x06
       * bit 7: 1=write protect registers 0-7 */
      /* NTS: According to real hardware, bit 6 is more than just the 8th bit,
       * clearing it automatically enables the scanline doubling in hardware
       * apparently. If other parameters are not adjusted, weird results happen.
       */
    ...
    default:
      LOG(LOG_VGAMISC, LOG_NORMAL)("MC6845:MCGA Write %X to illegal index %x",
                                   (int)val, (int)vga.other.index);
      break;
    }
  }
...
static Bitu read_crtc_data_mcga(Bitu port, Bitu iolen) {
  if (vga.other.index < 0x10) {
    ...
    default:
      LOG(LOG_VGAMISC, LOG_NORMAL)("MC6845:MCGA Read from illegal index %x",
                                   vga.other.index);
      break;
    }
  }
...
static void write_lightpen(Bitu port, Bitu val, Bitu) {
  ...
  case 0x3dc: // Preset lightpen latch
    if (!vga.other.lightpen_triggered) {
      vga.other.lightpen_triggered =
          true; // TODO: this shows at port 3ba/3da bit 1
      ...
      vga.other.lightpen =
          (uint16_t)((vga.draw.address_add / 2) * (current_scanline / 2));
...
static void update_cga16_color(void) {
  ...
  // Works in all CGA graphics modes/color settings and can simulate older and
  // newer CGA revisions
  ...
  if (machine == MCH_PCJR) {
    ...
    color_sel = 1;
    background_i = 1; // Really foreground intensity, but this is what the CGA
                      // schematic calls it.
    ...
    background_i =
        (cga16_val & 0x10) != 0; // Really foreground intensity, but this is
                                 // what the CGA schematic calls it.
  ...
  if (machine == MCH_PCJR) {
    ...
    if (!bpp1)
      pixel_clock_delay += 25 * ns; // correct for delay of color burst
  } else
    pixel_clock_delay -= 21.5 * ns; // correct for delay of color burst

  double hue_adjust =
      (-(90 - 33) - hue_offset + pixel_clock_delay) * tau / 360.0;
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    for (uint8_t j = 0; j < 6; j++) {
      ...
      // All the duty cycle fractions are the same, just under 0.5 as the rising
      // edge is delayed 2ns more than the falling edge.
      static const double duty = 0.5 - 2 * ns / 360.0;

      // We have a rectangle wave with period 1 (in units of the reciprocal of
      // the color burst frequency) and duty cycle fraction "duty" and phase
      // "phase". We band-limit this wave to frequency 2 and sample it at
      // intervals of 1/4. We model our band-limited wave with 4 frequency
      // components:
      //   f(x) = a + b*sin(x*tau) + c*cos(x*tau) + d*sin(x*2*tau)
      // Then:
      //   a =   integral(0, 1, f(x)*dx) = duty
      //   b = 2*integral(0, 1, f(x)*sin(x*tau)*dx) = 2*integral(0, duty,
      //   sin(x*tau)*dx) = 2*(1-cos(x*tau))/tau c = 2*integral(0, 1,
      //   f(x)*cos(x*tau)*dx) = 2*integral(0, duty, cos(x*tau)*dx) =
      //   2*sin(duty*tau)/tau d = 2*integral(0, 1, f(x)*sin(x*2*tau)*dx) =
      //   2*integral(0, duty, sin(x*4*pi)*dx) = 2*(1-cos(2*tau*duty))/(2*tau)
      double a = duty;
      ...
      chroma_signals[j + 1][i] =
          a + b * sin(x * tau) + c * cos(x * tau) + d * sin(x * 2 * tau);
    }
  }
  Bitu CGApal[4] = {
      overscan, (Bitu)(2 + (color_sel || bw ? 1 : 0) + (background_i ? 8 : 0)),
  ...
  for (uint8_t x = 0; x < 4; x++) { // Position of pixel in question
    ...
    for (uint8_t bits = 0; bits < (even ? 0x10 : 0x40); ++bits) {
      ...
      int r = static_cast<int>(
          255 * pow(1.5073 * R - 0.3725 * G - 0.0832 * B, 1 / gamma));
      if (r < 0)
        r = 0;
      if (r > 255)
        r = 255;
      int g = static_cast<int>(
          255 * pow(-0.0275 * R + 0.9350 * G + 0.0670 * B, 1 / gamma));
      if (g < 0)
        g = 0;
      if (g > 255)
        g = 255;
      int b = static_cast<int>(
          255 * pow(-0.0272 * R - 0.0401 * G + 1.1677 * B, 1 / gamma));
      if (b < 0)
        b = 0;
      if (b > 255)
        b = 255;
      ...
      uint8_t index =
          bits | ((x & 1) == 0 ? 0x30 : 0x80) | ((x & 2) == 0 ? 0x40 : 0);
...
static void write_cga_color_select(Bitu val) {
  ...
  if (vga.other.mcga_mode_control &
      1) /* ignore COMPLETELY in 256-color MCGA mode */
    return;
...
static void write_cga(Bitu port, Bitu val, Bitu /*iolen*/) {
  ...
  case 0x3d8:
    ...
    if (vga.other.mcga_mode_control &
        3) { // MCGA 256-color mode or 2-color 640x480
             // do nothing
      ...
      if (vga.tandy.mode_control & 0x10) {     // highres mode
        if (machine == MCH_AMSTRAD) {
          VGA_SetMode(M_AMSTRAD); // Amstrad 640x200x16 video mode.
                   ...
                   (cga_comp == 1 || (cga_comp == 0 && !(val & 0x4))) &&
                   !mono_cga) { // composite display
  ...
  case 0x3d9: // color select
    ...
    if (machine == MCH_AMSTRAD) {
      vga.amstrad.mask_plane =
          (val | (val << 8) | (val << 16) | (val << 24)) & 0x0F0F0F0F;
    }
    break;
  case 0x3dd:
...
void Composite(bool pressed) {
  ...
  if (++cga_comp > 2)
    cga_comp = 0;
  LOG_MSG("Composite output: %s",
          (cga_comp == 0) ? "auto" : ((cga_comp == 1) ? "on" : "off"));
...
static void tandy_update_palette() {
  ...
  if (machine == MCH_TANDY) {
    ...
    case M_TANDY4:
      if (vga.tandy.gfx_control & 0x8) {
        ...
        VGA_SetCGA4Table(
            ...
            vga.attr
                .palette[(4 | (color_set & r_mask)) & vga.tandy.palette_mask],
    ...
    case M_TANDY2:
      ...
      break;
    case M_TANDY4:
      VGA_SetCGA4Table(vga.attr.palette[0], vga.attr.palette[1],
                       vga.attr.palette[2], vga.attr.palette[3]);
      break;
    default:
      break;
...
static void PCJr_FindMode(void) {
  std::string mtype(static_cast<Section_prop *>(control->GetSection("dosbox"))
                        ->Get_string("machine"));
  if (mtype != "pcjr_composite")
    new_cga = 1;
  if (vga.tandy.mode_control & 0x2) {
...
static void write_tandy_reg(uint8_t val) {
  ...
  case 0x0:
    if (machine == MCH_PCJR) {
      ...
      LOG(LOG_VGAMISC, LOG_NORMAL)("Unhandled Write %2X to tandy reg %X", val,
                                   vga.tandy.reg_index);
    }
    break;
  case 0x1: /* Palette mask */
    vga.tandy.palette_mask =
        val; // FIXME: Wait... does this apply always in every mode or just the
...
void CycleHercPal(bool pressed) {
  if (!pressed)
    return;
  if (++herc_pal > MonochromeColor::Last)
    herc_pal = MonochromeColor::First;
  Herc_Palette();
}
...
void CycleMonoCGAPal(bool pressed) {
  if (!pressed)
    return;
  if (++mono_cga_pal > MonochromeColor::Last)
    mono_cga_pal = MonochromeColor::First;
  Mono_CGA_Palette();
}
...
void Mono_CGA_Palette(void) {
  for (uint8_t ct = 0; ct < 16; ct++) {
    VGA_DAC_SetEntry(
        ct, mono_cga_palettes[2 * mono_cga_pal + mono_cga_bright][ct][0],
  ...
  case 0x3b8: {
...
#if defined(USE_TTF)
        ttf_switch_off(
            false); // Disable TTF output when switching to graphics mode
#endif
    ...
    if (vga.herc.mode_control & 0x80) {
      ...
      if ((val & 0x80) && (vga.herc.enable_bits & 0x2)) {
        ...
        if (hercCard == HERC_InColor)
          vga.tandy.draw_base =
              &vga.mem.linear[32 * 1024 * 4 /*planar memory*/];
        else
...
Bitu read_herc_status(Bitu /*port*/, Bitu /*iolen*/) {
  ...
  double timeInFrame = PIC_FullIndex() - vga.draw.delay.framestart;
  uint8_t retval =
      0x72; // Hercules ident; from a working card (Winbond W86855AF)
...
void VGA_SetupOther(void) {
  ...
  if (machine == MCH_MCGA) { // MCGA uses a 8x16 font, through double-scanning
                             // as if 8x8 CGA text mode
    for (Bitu i = 0; i < 256; i++)
      memcpy(&vga.draw.font[i * 32], &int10_font_16[i * 16], 16);
    vga.draw.font_tables[0] = vga.draw.font_tables[1] = vga.draw.font;
  }
  ...
  if (machine == MCH_CGA || machine == MCH_PCJR || machine == MCH_TANDY) {
    ...
    for (Bitu port_ct = 0; port_ct < 4; port_ct++) {
      IO_RegisterWriteHandler(base + port_ct * 2, write_crtc_index_other,
                              IO_MB);
      IO_RegisterWriteHandler(base + port_ct * 2 + 1, write_crtc_data_other,
                              IO_MB);
      IO_RegisterReadHandler(base + port_ct * 2, read_crtc_index_other, IO_MB);
      IO_RegisterReadHandler(base + port_ct * 2 + 1, read_crtc_data_other,
                             IO_MB);
    }
  }
  if (machine == MCH_AMSTRAD) {
  ...
  if (machine == MCH_MCGA) {
    ...
    for (Bitu port_ct = 0; port_ct < 4; port_ct++) {
      IO_RegisterWriteHandler(base + port_ct * 2, write_crtc_index_other,
                              IO_MB);
      IO_RegisterWriteHandler(base + port_ct * 2 + 1, write_crtc_data_mcga,
                              IO_MB);
      IO_RegisterReadHandler(base + port_ct * 2, read_crtc_index_other, IO_MB);
      IO_RegisterReadHandler(base + port_ct * 2 + 1, read_crtc_data_mcga,
                             IO_MB);
    }
  }
}
