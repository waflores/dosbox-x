...
// NTS: For whatever reason, Windows 3.1 will set the GDC to 5MHz when entering
// a DOS application fullscreen.
void gdc_clock_check(void) {
  bool nresult = gdc_clock_1 && gdc_clock_2;

  if (gdc_5mhz_mode != nresult) {
    gdc_5mhz_mode = nresult;
    LOG_MSG("PC-98: Guest changed GDC clock to %s",
            gdc_5mhz_mode ? "5MHz" : "2.5MHz");
...
void pc98_crtc_write(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0x06:
    pc98_text_row_scroll_lines =
        (unsigned char)val & 0x1F; // 0x0F; Which does real hardware do? 0x0F or
                                   // 0x1F? 0x1F seems likely.
    break;
  case 0x08:
  ...
  case 0x0C: // 0x7C: mode reg / vram operation mode (also, reset tile counter)
    if (enable_pc98_grcg) {
       ...
       * bit 6: 1=Read/Modify/Write when writing  0=TCR mode at read, TDW mode
       * at write */
  ...
  case 0x0E: // 0x7E: tile data
    ...
    break;
  default:
    LOG_MSG("PC98 CRTC w: port=0x%02X val=0x%02X unknown", (unsigned int)port,
            (unsigned int)val);
    break;
  }
}
...
void update_gdc_analog(void) {
  ...
  gdc_analog =
      (pc98_gdc_vramop & ((1 << VOPBIT_ANALOG) | (1 << VOPBIT_VGA))) != 0;
}
...
void pc98_port6A_command_write(unsigned char b) {
  ...
  case 0x00: // 16-color (analog) disable
    ...
    update_gdc_analog();
    VGA_SetupHandlers(); // confirmed on real hardware: this disables access to
                         // E000:0000
    pc98_update_palette(); // Testing on real hardware shows that the "digital"
                           // and "analog" palettes are completely different.
                           // They're both there in hardware, but one or another
                           // is active depending on analog enable. Also, the
                           // 4th bitplane at E000:0000 disappears when switched
                           // off from the display and from CPU access.
    break;
  case 0x01: // or enable
    if (enable_pc98_16color) {
      ...
      pc98_gdc_vramop |= (1 << VOPBIT_ANALOG);
      update_gdc_analog();
      VGA_SetupHandlers(); // confirmed on real hardware: this enables access to
                           // E000:0000
      pc98_update_palette(); // Testing on real hardware shows that the
                             // "digital" and "analog" palettes are completely
                             // different. They're both there in hardware, but
                             // one or another is active depending on analog
                             // enable. Also, the 4th bitplane at E000:0000
                             // disappears when switched off from the display
                             // and from CPU access.
    }
    break;
  case 0x04:
  ...
  case 0x85:
    ...
    gdc_clock_check();
    break;
  // TODO: 0x8E/0x8F VRAM use selection  0=PC-98 graphics  1=Cirrus Logic CL-GD
  // graphics   (VRAM is shared?)
  default:
...
/* Port 0x9A0 readback.
 * This is needed to report the GDC setting so that Windows 3.1 doesn't change
 * the GDC to 5MHz arbitrarily and leave PC-98 games confused afterward */
Bitu pc98_read_9a0(Bitu /*port*/, Bitu /*iolen*/) {
  Bitu retval = 0;

  /* bit 0 depends on what was selected by writing to port 9a0.
   * according to undocumented 9821 (not verified), unknown registers and 0x00
   * will return 0xff here. */
  switch (sel_9a0) {
