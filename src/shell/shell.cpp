...
Bitu DOS_ChangeKeyboardLayout(const char *layoutname, int32_t codepage),
    DOS_SwitchKeyboardLayout(const char *new_layout, int32_t &tried_cp);
...
bool isDBCSCP(), InitCodePage(), isKanji1(uint8_t chr),
    shiftjis_lead_byte(int c), sdl_wait_on_error(),
    CheckDBCSCP(int32_t codepage), TTF_using(void);
...
static Bitu shellstop_handler(void) { return CBRET_STOP; }

void SHELL_ProgramStart(Program **make) { *make = new DOS_Shell; }
    ...
    if ((drives & (1 << i)) && !Drives[i]) {
      ...
      if (type != DRIVE_NO_ROOT_DIR) {
        ...
        if (!Drives[i] && !quiet) {
          ...
          n = (uint16_t)strlen(str);
          DOS_WriteFile(STDOUT, (uint8_t *)str, &n);
        } else if (mountwarning && !quiet && type == DRIVE_FIXED &&
                   (strcasecmp(name, "C:\\") == 0)) {
  ...
  for (auto_it it = autoexec_strings.begin(); it != autoexec_strings.end();) {
    if ((*it) == buf) {
      ...
      if (stringset && first_shell && first_shell->bf &&
          first_shell->bf->filename.find("AUTOEXEC.BAT") != std::string::npos) {
        // Replace entry with spaces if it is a set and from autoexec.bat, as
        // else the location counter will be off.
...
AutoexecObject::~AutoexecObject() { Uninstall(); }

DOS_Shell::~DOS_Shell() {
  if (bf != NULL)
    delete bf; /* free batch file */
}
...
Bitu DOS_Shell::GetRedirection(char *s, char **ifn, char **ofn, char **toc,
                               bool *append) {
  ...
  while ((ch = *lr++)) {
    if (quote &&
        ch != '"') { /* don't parse redirection within quotes. Not perfect yet.
    ...
    } else if ((IS_PC98_ARCH && shiftjis_lead_byte(ch)) ||
               (isDBCSCP() &&
                !((dos.loaded_codepage == 936 || IS_PDOSV) && !gbk) &&
                isKanji1(ch)))
      lead1 = true;
    switch (ch) {
    case '"':
    ...
    case '>':
      ...
      *ofn = lr;
      q = 0;
      lead2 = false;
      while (*lr && (q / 2 * 2 != q || *lr != ' ') && *lr != '<' &&
             !(!lead2 && *lr == '|')) {
        if (lead2)
          lead2 = false;
        else if ((IS_PC98_ARCH && shiftjis_lead_byte(*lr & 0xff)) ||
                 (isDBCSCP() &&
                  !((dos.loaded_codepage == 936 || IS_PDOSV) && !gbk) &&
                  isKanji1(*lr & 0xff)))
          lead2 = true;
...
public:
  ...
  uint16_t GetInformation(void) override {
    return (strcmp(RunningProgram, "WCLIP") ? DeviceInfoFlags::Device : 0) |
           DeviceInfoFlags::EofOnInput;
  }
...
void DOS_Shell::ParseLine(char *line) {
  ...
  if (in) {
    if (DOS_OpenFile(in, OPEN_READ, &dummy)) { // Test if file exists
      ...
      WriteOut(!*in ? "File open error\n"
                    : (dos.errorcode == DOSERR_ACCESS_DENIED
                           ? MSG_Get("SHELL_CMD_FILE_ACCESS_DENIED")
                           : "File open error - %s\n"),
               in);
      if (in)
        free(in);
  ...
  if (toc) {
    ...
    uint16_t fattr;
    if (!(DOS_GetFileAttr(tempPath.c_str(), &fattr) &&
  ...
  if (out || toc) {
    ...
    } else if (!toc && DOS_GetFileAttr(out, &fattr) &&
               fattr & DOS_ATTR_READ_ONLY) {
      DOS_SetError(DOSERR_ACCESS_DENIED);
      status = false;
    } else {
      bool device = DOS_FindDevice(pipetmp) != DOS_DEVICES;
      if (toc && !device && DOS_FindFirst(pipetmp, ~DOS_ATTR_VOLUME) &&
          !DOS_UnlinkFile(pipetmp))
        fail = true;
      status = device ? false
                      : DOS_OpenFileExtended(toc && !fail ? pipetmp : out,
                                             OPEN_READWRITE, DOS_ATTR_ARCHIVE,
                                             0x12, &dummy, &dummy2);
      bool pipetmp_is_zdrive = strncasecmp(pipetmp, "Z:\\", 3) == 0;
      if (toc && (fail || !status) &&
          (!strchr(pipetmp, '\\') || pipetmp_is_zdrive)) {
        Overlay_Drive *da = Drives[0] ? (Overlay_Drive *)Drives[0] : NULL,
                      *dc = Drives[2] ? (Overlay_Drive *)Drives[2] : NULL;
        if (!pipetmp_is_zdrive &&
            ((Drives[0] && !Drives[0]->readonly && !(da && da->ovlreadonly)) ||
             (Drives[2] && !Drives[2]->readonly && !(dc && dc->ovlreadonly)))) {
          ...
          fail = true;
        if (!tmpdev && DOS_FindFirst(pipetmp, ~DOS_ATTR_VOLUME) &&
            !DOS_UnlinkFile(pipetmp))
          fail = true;
        else
          status = DOS_OpenFileExtended(
              pipetmp, OPEN_READWRITE, DOS_ATTR_ARCHIVE, 0x12, &dummy, &dummy2);
      }
    }
  ...
  if (this == first_shell)
    dos_shell_running_program = true;
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  Reflect_Menu();
#endif
  if (toc || (!toc && ((out && DOS_FindDevice(out) != DOS_FindDevice("con")))))
    outcon = false;
  if (toc || (!toc && ((out && DOS_FindDevice(out) != DOS_FindDevice("con")) ||
                       (in && DOS_FindDevice(in) != DOS_FindDevice("con")))))
    usecon = false;
  ...
  if (toc) {
    ...
    if (!fail && DOS_OpenFile(pipetmp, OPEN_READ,
                              &dummy)) // Test if file can be opened for reading
    {
      DOS_CloseFile(dummy);
      if (normalstdin)
        DOS_CloseFile(0);                       // Close stdin
      ...
      WriteOut("\nFailed to create/open a temporary file for piping. Check the "
               "%%TEMP%% variable.\n");
    free(toc);
    if (tmpdev) {
      DOS_DelDevice(tmpdev);
      tmpdev = NULL;
    } else if (DOS_FindFirst(pipetmp, ~DOS_ATTR_VOLUME))
      DOS_UnlinkFile(pipetmp);
  }
  outcon = usecon = true;
}
...
extern void ReadCharAttr(uint16_t col, uint16_t row, uint8_t page,
                         uint16_t *result);
...
const char *ParseMsg(const char *msg) {
  ...
  if (machine != MCH_PC98) {
    ...
    if (theme == "black")
      ...
      msg = str_replace(str_replace(msg, "\033[31m", "\033[34m"), "\033[44;1m",
                        "\033[41;1m");
    else if (theme == "green")
      msg = str_replace(str_replace(str_replace(msg, "\033[36m", "\033[34m"),
                                    "\033[32m", "\033[36m"),
                        "\033[44;1m", "\033[42;1m");
      ...
      msg = str_replace(str_replace(msg, "\033[36m", "\033[34m"), "\033[44;1m",
                        "\033[46;1m");
    else if (theme == "white")
      msg = str_replace(str_replace(msg, "\033[36m", "\033[34m"), "\033[44;1m",
                        "\033[47;1m");
  ...
  if (machine == MCH_PC98)
    ...
    if (real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) > 80)
      msg = str_replace(
          str_replace(str_replace(msg, "\xBA\033[0m", "\xBA\033[0m\n"),
...
static char const *const path_string =
...
void GetExpandedPath(std::string &path) {
  std::string udrive = std::string(1, ZDRIVE_NUM + 'A'),
              ldrive = std::string(1, ZDRIVE_NUM + 'a');
  char pathstr[100];
  strcpy(pathstr, path_string + 5);
  if (path == udrive + ":\\" || path == ldrive + ":\\")
    path = ZDRIVE_NUM == 25
               ? pathstr
               : str_replace(pathstr, "Z:\\", (udrive + ":\\").c_str());
  else if (path.size() > 3 &&
           (path.substr(0, 4) == udrive + ":\\;" ||
            path.substr(0, 4) == ldrive + ":\\;") &&
           path.substr(4).find(udrive + ":\\") == std::string::npos &&
           path.substr(4).find(ldrive + ":\\") == std::string::npos)
    path =
        std::string(ZDRIVE_NUM == 25 ? pathstr
                                     : str_replace(pathstr, "Z:\\",
                                                   (udrive + ":\\").c_str())) +
        path.substr(3);
  else if (path.size() > 3) {
    size_t pos = path.find(";" + udrive + ":\\");
    if (pos == std::string::npos)
      pos = path.find(";" + ldrive + ":\\");
    if (pos != std::string::npos &&
        (!path.substr(pos + 4).size() ||
         (path[pos + 4] == ';' &&
          path.substr(pos + 4).find(udrive + ":\\") == std::string::npos &&
          path.substr(pos + 4).find(ldrive + ":\\") == std::string::npos)))
...
void showWelcome(Program *shell) {
  ...
  std::string verstr = "v" + std::string(VERSION) + ", " + OS_PLATFORM_LONG +
                       " " + SDL_STRING + " " + OS_BIT + "-bit";
  if (machine == MCH_PC98) {
    shell->WriteOut(ParseMsg("\x86\x52\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             ...
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x56\n"));
    shell->WriteOut(
        ParseMsg((std::string("\x86\x46 \033[32m") +
                  (MSG_Get("SHELL_STARTUP_TITLE") + std::string("      "))
                      .substr(0, 22) +
                  std::string("  \033[33m%*s\033[37m \x86\x46\n"))
                     .c_str()),
        42, verstr.c_str());
    shell->WriteOut(ParseMsg("\x86\x46                                         "
                             "                           \x86\x46\n"));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              MSG_Get("SHELL_STARTUP_HEAD1_PC98") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg("\x86\x46                                         "
                             "                           \x86\x46\n"));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT1_PC98"),
                                          "\n", " \x86\x46\n\x86\x46 ") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              MSG_Get("SHELL_STARTUP_EXAMPLE_PC98") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg("\x86\x46                                         "
                             "                           \x86\x46\n"));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT2_PC98"),
                                          "\n", " \x86\x46\n\x86\x46 ") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg("\x86\x46                                         "
                             "                           \x86\x46\n"));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              str_replace(MSG_Get("SHELL_STARTUP_INFO_PC98"),
                                          "\n", " \x86\x46\n\x86\x46 ") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg("\x86\x46                                         "
                             "                           \x86\x46\n"));
    shell->WriteOut(ParseMsg((std::string("\x86\x46 ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT3_PC98"),
                                          "\n", " \x86\x46\n\x86\x46 ") +
                              std::string(" \x86\x46\n"))
                                 .c_str()));
    shell->WriteOut(ParseMsg("\x86\x5A\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             ...
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
                             "\x86\x44\x86\x44\x86\x44\x86\x5E\033[0m\n"));
    shell->WriteOut(ParseMsg((std::string("\033[1m\033[32m") +
                              MSG_Get("SHELL_STARTUP_LAST") + "\033[0m\n")
                                 .c_str()));
  } else {
    shell->WriteOut(ParseMsg(
        "\033[44;"
        "1m\xC9\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xBB\033[0m"));
    shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA \033[32m") +
                              (MSG_Get("SHELL_STARTUP_TITLE") +
                               std::string("             "))
                                  .substr(0, 30) +
                              std::string(" \033[33m%*s\033[37m \xBA\033[0m"))
                                 .c_str()),
                    45, verstr.c_str());
    shell->WriteOut(
        ParseMsg("\033[44;1m\xBA                                               "
                 "                               \xBA\033[0m"));
    shell->WriteOut(
        ParseMsg((std::string("\033[44;1m\xBA ") +
                  MSG_Get("SHELL_STARTUP_HEAD1") + std::string(" \xBA\033[0m"))
                     .c_str()));
    shell->WriteOut(
        ParseMsg("\033[44;1m\xBA                                               "
                 "                               \xBA\033[0m"));
    shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT1"), "\n",
                                          " \xBA\033[0m\033[44;1m\xBA ") +
                              std::string(" \xBA\033[0m"))
                                 .c_str()));
    if (IS_VGA_ARCH)
      shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                                MSG_Get("SHELL_STARTUP_EXAMPLE") +
                                std::string(" \xBA\033[0m"))
                                   .c_str()));
    shell->WriteOut(
        ParseMsg("\033[44;1m\xBA                                               "
    ...
    shell->WriteOut(
        ParseMsg((std::string("\033[44;1m\xBA ") +
                  MSG_Get("SHELL_STARTUP_HEAD2") + std::string(" \xBA\033[0m"))
                     .c_str()));
    shell->WriteOut(
        ParseMsg("\033[44;1m\xBA                                               "
                 "                               \xBA\033[0m"));
    shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT2"), "\n",
                                          " \xBA\033[0m\033[44;1m\xBA ") +
                              std::string(" \xBA\033[0m"))
                                 .c_str()));
    shell->WriteOut(
    ...
    if (IS_DOSV) {
      shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                                str_replace(MSG_Get("SHELL_STARTUP_DOSV"), "\n",
                                            " \xBA\033[0m\033[44;1m\xBA ") +
                                std::string(" \xBA\033[0m"))
                                   .c_str()));
      shell->WriteOut(
      ...
      shell->WriteOut(
          ParseMsg((std::string("\033[44;1m\xBA ") +
                    str_replace(MSG_Get(mono_cga ? "SHELL_STARTUP_CGA_MONO"
                                                 : "SHELL_STARTUP_CGA"),
                                "\n", " \xBA\033[0m\033[44;1m\xBA ") +
                    std::string(" \xBA\033[0m"))
                       .c_str()));
      ...
      shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                                str_replace(MSG_Get("SHELL_STARTUP_HERC"), "\n",
                                            " \xBA\033[0m\033[44;1m\xBA ") +
                                std::string(" \xBA\033[0m"))
                                   .c_str()));
      shell->WriteOut(
    ...
    shell->WriteOut(
        ParseMsg((std::string("\033[44;1m\xBA ") +
                  MSG_Get("SHELL_STARTUP_HEAD3") + std::string(" \xBA\033[0m"))
                     .c_str()));
    shell->WriteOut(
        ParseMsg("\033[44;1m\xBA                                               "
                 "                               \xBA\033[0m"));
    shell->WriteOut(ParseMsg((std::string("\033[44;1m\xBA ") +
                              str_replace(MSG_Get("SHELL_STARTUP_TEXT3"), "\n",
                                          " \xBA\033[0m\033[44;1m\xBA ") +
                              std::string(" \xBA\033[0m"))
                                 .c_str()));
    shell->WriteOut(ParseMsg(
        "\033[44;"
        "1m\xC8\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
        "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xBC\033[0m"));
    shell->WriteOut(ParseMsg(
        (std::string("\033[32m") +
         (MSG_Get("SHELL_STARTUP_LAST") +
          std::string(
              "                                                       "))
             .substr(0, 79) +
         std::string("\033[0m\n"))
...
void DOS_Shell::Prepare(void) {
  if (this == first_shell) {
#if defined(USE_TTF)
    if (CurMode->type == M_TEXT || (IS_PC98_ARCH && is_ttfswitched_on))
      ttf_switch_on(true); // Initialization completed, M_TEXT modes can switch
                           // to TTF mode from now on.
    if (ttf.inUse) {
      int cols = static_cast<Section_prop *>(control->GetSection("ttf"))
                     ->Get_int("cols");
      int lins = static_cast<Section_prop *>(control->GetSection("ttf"))
                     ->Get_int("lins");
      if (cols || lins)
        ttf_setlines(cols, lins);
    }
#endif
...
#if defined(WIN32)
      else if (GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_ICOUNTRY, buffer,
                             128)) {
    ...
    if ((section != NULL && !control->opt_noconfig) || control->opt_langcp) {
      char *countrystr = (char *)section->Get_string("country"),
           *r = strchr(countrystr, ',');
      int country = 0;
      int32_t newCP = dos.loaded_codepage;
      if ((control->opt_langcp && msgcodepage > 0) ||
          CheckDBCSCP(msgcodepage) || msgcodepage == dos.loaded_codepage)
        newCP = msgcodepage;
      if ((r == NULL || !*(r + 1)) && !control->opt_langcp)
        ...
        if (r != NULL)
          *r = 0;
        country = atoi(trim(countrystr));
        newCP = r == NULL || IS_PC98_ARCH || IS_JEGA_ARCH || IS_DOSV
                    ? dos.loaded_codepage
                    : atoi(trim(r + 1));
        if (r != NULL)
          *r = ',';
      }
      if (newCP != dos.loaded_codepage &&
          (!TTF_using() || (TTF_using() && isSupportedCP(newCP)))) {
          ...
          for (std::string line; std::getline(in, line);) {
            ...
            if (p != NULL) {
              *p = 0;
              ...
              cmd = trim(cmd);
              val = trim(val);
              if (strlen(config_data) + strlen(cmd) + strlen(val) + 3 <
                  CONFIG_SIZE) {
              ...
              if (!strncasecmp(cmd, "set ", 4)) {
                vstr = std::string(val);
                ResolvePath(vstr);
                if (zdirpath && !strcmp(cmd, "set path"))
                ...
                if (!DOS_FileExists(name) &&
                    !DOS_FileExists(
                        (std::string("Z:\\SYSTEM\\") + name).c_str()) &&
                    !DOS_FileExists(
                        (std::string("Z:\\BIN\\") + name).c_str()) &&
                    !DOS_FileExists(
                        (std::string("Z:\\DOS\\") + name).c_str()) &&
                    !DOS_FileExists(
                        (std::string("Z:\\4DOS\\") + name).c_str()) &&
                    !DOS_FileExists(
                        (std::string("Z:\\DEBUG\\") + name).c_str()) &&
                    !DOS_FileExists(
                        (std::string("Z:\\TEXTUTIL\\") + name).c_str())) {
    ...
    if (dos.loaded_codepage == 932)
      toSetCodePage(this, 932,
                    -1); // Workaround for corrupted box-drawing characters
    runRescan("-A -Q");
    internal_program = true;
    VFILE_Register("AUTOEXEC.BAT", (uint8_t *)autoexec_data,
                   (uint32_t)strlen(autoexec_data));
    ...
    if (!control->opt_securemode && !control->SecureMode()) {
      const Section_prop *sec =
          static_cast<Section_prop *>(control->GetSection("dos"));
      const char *automountstr = sec->Get_string("automountall");
      if (strcmp(automountstr, "0") && strcmp(automountstr, "false"))
        MountAllDrives(!strcmp(automountstr, "quiet") ||
                       control->opt_fastlaunch);
    }
#endif
    strcpy(i4dos_data, "");
...
void DOS_Shell::Run(void) {
  ...
  char input_line[CMD_MAXLINE] = {0};
  std::string line;
  bool optP = cmd->FindStringRemain("/P", line),
       optC = cmd->FindStringRemainBegin("/C", line), optK = false;
  if (!optC)
    optK = cmd->FindStringRemainBegin("/K", line);
  if (optP)
    perm = true;
  if (optC || optK) {
    ...
    temp.echo = echo;
    temp.exec = true;
    temp.ParseLine(input_line); // for *.exe *.com  |*.bat creates the bf needed
                                // by runinternal;
  ...
  if (this != first_shell && !optInit)
    WriteOut(optK ? "\n"
                  : "DOSBox-X command shell [Version %s %s]\nCopyright "
                    "DOSBox-X Team. All rights reserved.\n\n",
             VERSION, SDL_STRING);
  ...
  do {
    ...
    if (strlen(input_line) != 0) {
      if (bf == NULL /*not running a batch file*/ && shell_keyboard_flush)
        DOS_FlushSTDIN();
      ParseLine(input_line);
      if (echo && !bf)
        WriteOut_NoParsing("\n");
...
bool filename_not_8x3(const char *n), isDBCSCP(), isKanji1(uint8_t chr),
...
public:
      ...
      for (auto &str : control->auto_bat_additional) {
        ...
        if (!strncmp(str.c_str(), "@mount c: ", 10)) {
          ...
          for (unsigned int j = 0; j < str.size(); j++) {
            if (lead)
              lead = false;
            else if ((IS_PC98_ARCH && shiftjis_lead_byte(str[j])) ||
                     (isDBCSCP() && isKanji1(str[j])))
              lead = true;
            else if (str[j] == '\\')
              pos = j;
          }
          if (pos ==
              std::string::npos) { // Only a filename, mount current directory
            batname = str;
            cmd += "@mount c: . -nl -q\n";
          ...
          std::string opt =
              control->opt_o.size() > ind && control->opt_o[ind].size()
                  ? " " + control->opt_o[ind]
                  : "";
          ind++;
          bool templfn = !uselfn && filename_not_8x3(batname.c_str()) &&
                         (enablelfn == -1 || enablelfn == -2);
          cmd += "@config -get lastmount>nul\n";
          cmd += "@set LASTMOUNT=%CONFIG%\n";
          cmd += "@if not '%LASTMOUNT%'=='' %LASTMOUNT%\n";
          cmd += "@cd \\\n";
          if (templfn)
            cmd += "@config -set lfn=true\n";
#if defined(WIN32) && !defined(HX_DOS)
          if (!winautorun)
            cmd += "@config -set startcmd=true\n";
#endif
          cmd += "@CALL \"";
          cmd += batname;
          cmd += "\"" + opt + "\n";
          if (templfn)
            cmd += "@config -set lfn=" +
                   std::string(enablelfn == -1 ? "auto" : "autostart") + "\n";
#if defined(WIN32) && !defined(HX_DOS)
          if (!winautorun)
            cmd += "@config -set startcmd=false\n";
#endif
  ...
  AUTOEXEC(Section *configuration) : Module_base(configuration) {
    ...
    /* add stuff from the configfile unless -noautoexec or -securemode is
     * specified. */
    const char *extra = section->data.c_str();
    if (extra && !secure && !control->opt_noautoexec) {
      ...
      /* Install the stuff from the configfile if anything left after moving
       * echo off */
    ...
    Bitu i = 1;
    for (auto it = control->opt_c.begin();
         i <= 11 && it != control->opt_c.end(); it++) /* -c switches */
      autoexec[i++].Install(*it);

    /* Check for the -exit switch which causes dosbox to when the command on the
     * commandline has finished */
    bool addexit = control->opt_exit;

    if (!control->opt_prerun)
      RunAdditional();

#if 0 /*FIXME: This is ugly. I don't care to follow through on this nonsense   \
         for now. When needed, port to new command line switching. */
...
#else
    if (secure)
      autoexec[i++].Install("z:\\system\\config.com -securemode");
#endif
#if 0
...
void AUTOEXEC_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(AUTOEXEC_ShutDown));
 ...
 *       That's like claiming that DOS memory and device initialization happens
 * from COMMAND.COM! We need to move the DOS kernel initialization into another
 * function, and the VFILE registration to another function, and then message
 * initialization to another function, and then those functions need to be
 * called before SHELL_Init() -J.C. */
void SHELL_Init() {
  ...
  MSG_Add(
      "SHELL_CMD_HELP",
      "If you want a list of all supported internal commands type "
      "\033[33;1mHELP /ALL\033[0m.\nYou can also find external commands on the "
      "Z: drive as programs.\nA short list of the most often used commands:\n");
  MSG_Add("SHELL_CMD_HELP_END1",
          "External commands such as \033[33;1mMOUNT\033[0m and "
          "\033[33;1mIMGMOUNT\033[0m can be found on the Z: drive.\n");
  MSG_Add("SHELL_CMD_HELP_END2",
          "Type \033[33;1mHELP command\033[0m or \033[33;1mcommand /?\033[0m "
          "for help information for the specified command.\n");
  ...
  MSG_Add("SHELL_CMD_CHDIR_HINT_2",
          ...
          "or properly quote them with quotation marks.\n");
  MSG_Add("SHELL_CMD_CHDIR_HINT_3",
          "You are still on drive Z:, and the specified directory cannot be "
          "found.\nFor accessing a mounted drive, change to the drive with a "
          "syntax like \033[31mC:\033[0m.\n");
  ...
  MSG_Add("SHELL_CMD_RMDIR_ERROR",
          "Invalid path, not directory, or directory not empty - %s\n");
  MSG_Add("SHELL_CMD_RMDIR_FULLTREE_CONFIRM",
  ...
  MSG_Add("SHELL_EXECUTE_DRIVE_NOT_FOUND",
          ...
          "more information.\n");
  MSG_Add("SHELL_EXECUTE_DRIVE_ACCESS_CDROM",
          "Do you want to give DOSBox-X access to your real CD-ROM drive %c "
          "[Y/N]?");
  MSG_Add("SHELL_EXECUTE_DRIVE_ACCESS_FLOPPY",
          "Do you want to give DOSBox-X access to your real floppy drive %c "
          "[Y/N]?");
  MSG_Add("SHELL_EXECUTE_DRIVE_ACCESS_REMOVABLE",
          "Do you want to give DOSBox-X access to your real removable drive %c "
          "[Y/N]?");
  MSG_Add("SHELL_EXECUTE_DRIVE_ACCESS_NETWORK",
          "Do you want to give DOSBox-X access to your real network drive %c "
          "[Y/N]?");
  MSG_Add("SHELL_EXECUTE_DRIVE_ACCESS_FIXED",
          "Do you really want to give DOSBox-X access to your real hard drive "
          "%c [Y/N]?");
  MSG_Add("SHELL_EXECUTE_ILLEGAL_COMMAND",
  ...
  /* NTS: MSG_Add() takes the string as const char * but it does make a copy of
   * the string when entering into the message map, so there is no problem here
   * of causing use-after-free crashes when we exit. */
  std::string host_key_help; // SHELL_STARTUP_BEGIN2

  if (machine == MCH_PC98) {
    // "\x86\x46 To activate the keymapper \033[31mhost+M\033[37m. Host key is
    // F12.                 \x86\x46\n"
  } else {
    // "\xBA To activate the keymapper \033[31mhost+M\033[37m. Host key is F12.
    // \xBA\n"
  }

  MSG_Add("SHELL_STARTUP_TITLE", "Welcome to DOSBox-X !");
  MSG_Add("SHELL_STARTUP_HEAD1_PC98",
          "\033[36mGetting Started with DOSBox-X:\033[37m                      "
          "              ");
  MSG_Add(
      "SHELL_STARTUP_TEXT1_PC98",
      "Type \033[32mHELP\033[37m for shell commands, and \033[32mINTRO\033[37m "
      "for a short introduction. \nYou could also complete various tasks "
      "through the \033[33mdrop-down menus\033[37m.");
  MSG_Add("SHELL_STARTUP_EXAMPLE_PC98",
          "\033[32mExample\033[37m: Try select \033[33mTrueType font\033[37m "
          "or \033[33mOpenGL perfect\033[37m output option.");
  MSG_Add(
      "SHELL_STARTUP_TEXT2_PC98",
      (std::string("To launch the \033[33mConfiguration Tool\033[37m, use "
                   "\033[31mhost+C\033[37m. Host key is \033[32m") +
       (mapper_keybind + "\033[37m.                       ").substr(0, 13) +
       std::string(
           "\nTo activate the \033[33mMapper Editor\033[37m for key "
           "assignments, use \033[31mhost+M\033[37m.    \nTo switch between "
           "windowed and full-screen mode, use \033[31mhost+F\033[37m.      "
           "\nTo adjust the emulated CPU speed, use \033[31mhost+Plus\033[37m "
           "and \033[31mhost+Minus\033[37m.   "))
          .c_str());
  MSG_Add("SHELL_STARTUP_INFO_PC98",
          "\033[36mDOSBox-X is now running in \033[32mJapanese NEC "
          "PC-98\033[36m emulation mode.\033[37m     ");
  MSG_Add("SHELL_STARTUP_TEXT3_PC98",
          "\033[32mDOSBox-X project \033[33mhttps://dosbox-x.com/     "
          "\033[36mComplete DOS emulations\033[37m\n\033[32mDOSBox-X guide   "
          "\033[33mhttps://dosbox-x.com/wiki\033[37m \033[36mDOS, Windows 3.x "
          "and 9x\033[37m\n\033[32mDOSBox-X support "
          "\033[33mhttps://github.com/joncampbell123/dosbox-x/issues\033[37m");
  MSG_Add("SHELL_STARTUP_HEAD1",
          "\033[36mGetting started with DOSBox-X:                              "
          "                \033[37m");
  MSG_Add(
      "SHELL_STARTUP_TEXT1",
      "Type \033[32mHELP\033[37m to see the list of shell commands, "
      "\033[32mINTRO\033[37m for a brief introduction.\nYou can also complete "
      "various tasks in DOSBox-X through the \033[33mdrop-down menus\033[37m.");
  MSG_Add(
      "SHELL_STARTUP_EXAMPLE",
      "\033[32mExample\033[37m: Try select the \033[33mTrueType font\033[37m "
      "or \033[33mOpenGL pixel-perfect\033[37m output option.");
  MSG_Add("SHELL_STARTUP_HEAD2",
          "\033[36mUseful default shortcuts:                                   "
          "                \033[37m");
  MSG_Add(
      "SHELL_STARTUP_TEXT2",
      (std::string("- switch between windowed and full-screen mode with key "
                   "combination \033[31m") +
       (default_host + " \033[37m+ \033[31mF\033[37m                        ")
           .substr(0, 23) +
       std::string("\033[37m\n") +
       std::string(
           "- launch \033[33mConfiguration Tool\033[37m using \033[31m") +
       (default_host + " \033[37m+ \033[31mC\033[37m                      ")
           .substr(0, 22) +
       std::string(
           "\033[37m, and \033[33mMapper Editor\033[37m using \033[31m") +
       (default_host + " \033[37m+ \033[31mM\033[37m                     ")
           .substr(0, 24) +
       std::string("\033[37m\n") +
       std::string("- increase or decrease the emulation speed with \033[31m") +
       (default_host + " \033[37m+ \033[31mPlus\033[37m      ").substr(0, 25) +
       std::string("\033[37m or \033[31m") +
       (default_host + " \033[37m+ \033[31mMinus\033[37m       ")
           .substr(0, 29) +
       std::string("\033[37m"))
          .c_str());
  MSG_Add("SHELL_STARTUP_DOSV",
          "\033[32mDOS/V mode\033[37m is now active. Try also \033[32mTTF CJK "
          "mode\033[37m for a general DOS emulation.");
  MSG_Add("SHELL_STARTUP_CGA",
          "Composite CGA mode is supported. Use \033[31mCtrl+F8\033[37m to set "
          "composite output ON/OFF.\nUse \033[31mCtrl+Shift+[F7/F8]\033[37m to "
          "change hue; \033[31mCtrl+F7\033[37m selects early/late CGA model. ");
  MSG_Add("SHELL_STARTUP_CGA_MONO",
          "Use \033[31mCtrl+F7\033[37m to cycle through green, amber, and "
          "white monochrome color,      \nand \033[31mCtrl+F8\033[37m to "
          "change contrast/brightness settings.                         ");
  MSG_Add("SHELL_STARTUP_HERC",
          "Use \033[31mCtrl+F7\033[37m to cycle through white, amber, and "
          "green monochrome color.      \nUse \033[31mCtrl+F8\033[37m to "
          "toggle horizontal blending (only in graphics mode).          ");
  MSG_Add("SHELL_STARTUP_HEAD3",
          "\033[36mDOSBox-X project on the web:                                "
          "                \033[37m");
  MSG_Add("SHELL_STARTUP_TEXT3",
          "\033[32mHomepage of project\033[37m: \033[33mhttps://dosbox-x.com/  "
          "         \033[36mComplete DOS emulations\033[37m\n\033[32mUser "
          "guides on Wiki\033[37m: \033[33mhttps://dosbox-x.com/wiki\033[32m   "
          "    \033[36mDOS, Windows 3.x and 9x\033[37m\n\033[32mIssue or "
          "suggestion\033[37m: "
          "\033[33mhttps://github.com/joncampbell123/dosbox-x/issues      "
          "\033[37m");
  MSG_Add("SHELL_STARTUP_LAST", "HAVE FUN WITH DOSBox-X !");

  MSG_Add("SHELL_CMD_BREAK_HELP", "Sets or clears extended CTRL+C checking.\n");
  MSG_Add("SHELL_CMD_BREAK_HELP_LONG",
          "BREAK [ON | OFF]\n\nType BREAK without a parameter to display the "
          "current BREAK setting.\n");
  ...
  MSG_Add(
      ...
      "DIR [drive:][path][filename] [/[W|B]] [/S] [/P] [/A[D|H|S|R|A]] "
      "[/O[N|E|G|S|D]]\n\n"
      "  [drive:][path][filename]\n"
  ...
  MSG_Add("SHELL_CMD_HELP_HELP_LONG",
          ...
          "\033[0mE.g., \033[37;1mHELP COPY\033[0m or \033[37;1mCOPY /?\033[0m "
          "shows help information for COPY command.\n\n"
          "Note: External commands like \033[33;1mMOUNT\033[0m and "
          "\033[33;1mIMGMOUNT\033[0m are not listed by HELP [/A].\n"
          "      These commands can be found on the Z: drive as programs (e.g. "
          "MOUNT.COM).\n"
          "      Type \033[33;1mcommand /?\033[0m or \033[33;1mHELP "
          "command\033[0m for help information for that command.\n");
  ...
  MSG_Add(
      ...
      "is\n"
      "                    met.  Command can be followed by ELSE command "
      "which\n"
  ...
  MSG_Add("SHELL_CMD_GOTO_HELP_LONG",
          "GOTO label\n\n"
          "   label   Specifies a text string used in the batch program as a "
          "label.\n\n"
          "You type a label on a line by itself, beginning with a colon.\n");
  MSG_Add("SHELL_CMD_HISTORY_HELP",
          "Displays or clears the command history list.\n");
  MSG_Add("SHELL_CMD_HISTORY_HELP_LONG",
  ...
  MSG_Add("SHELL_CMD_FOR_HELP_LONG",
          ...
          "command-parameters\n             Specifies parameters or switches "
          "for the specified command.\n\nTo use the command in a batch "
          "program, specify %%%%variable instead of %%variable.\n");
  MSG_Add(
      "SHELL_CMD_LFNFOR_HELP",
      "Enables or disables long filenames when processing FOR wildcards.\n");
  MSG_Add("SHELL_CMD_LFNFOR_HELP_LONG",
          "LFNFOR [ON | OFF]\n\nType LFNFOR without a parameter to display the "
          "current LFNFOR setting.\n\nThis command is only useful if LFN "
  ...
  MSG_Add("SHELL_CMD_RENAME_HELP_LONG",
          ...
          "filename2]\n\n"
          "Note that you can not specify a new drive or path for your "
          "destination.\n\n"
          "Wildcards (* and ?) are supported for files. For example, the "
          "following command\n"
  ...
  MSG_Add(
      ...
      "                will be deleted.\n"
      "  /P            Prompts for confirmation before deleting one or more "
      "files.\n"
  ...
  MSG_Add(
      ...
      "  [drive2:]path Specifies a mounted local drive and path you want to "
      "assign to.\n"
      "  /D            Deletes a mounted or substituted drive.\n\n"
      "Type SUBST with no parameters to display a list of mounted local "
      "drives.\n");
  ...
  MSG_Add("SHELL_CMD_CHOICE_HELP",
          "Waits for a user keypress to choose one of a set of choices.\n");
  ...
  MSG_Add("SHELL_CMD_PUSHD_HELP",
          "Stores the current directory for use by the POPD command, "
          "then\nchanges to the specified directory.\n");
  MSG_Add(
      "SHELL_CMD_PUSHD_HELP_LONG",
      "PUSHD [path]\n\n"
      "path        Specifies the directory to make the current directory.\n\n"
      "Type PUSHD with no parameters to display currently stored "
      "directories.\n");
  MSG_Add("SHELL_CMD_POPD_HELP",
  ...
  MSG_Add("SHELL_CMD_VERIFY_HELP_LONG",
          "VERIFY [ON | OFF]\n\nType VERIFY without a parameter to display the "
          "current VERIFY setting.\n");
  MSG_Add("SHELL_CMD_VER_HELP",
          "Displays or sets DOSBox-X's reported DOS version.\n");
  MSG_Add("SHELL_CMD_VER_HELP_LONG",
          "VER [/R]\n"
          "VER [SET] number or VER SET [major minor]\n\n"
          "  /R                 Display DOSBox-X's Git commit version and "
          "build date.\n"
          "  [SET] number       Set the specified number as the reported DOS "
          "version.\n"
          "  SET [major minor]  Set the reported DOS version in major and "
          "minor format.\n\n"
          "  \033[0mE.g., \033[37;1mVER 6.0\033[0m or \033[37;1mVER 7.1\033[0m "
          "sets the DOS version to 6.0 and 7.1, respectively.\n"
          "  On the other hand, \033[37;1mVER SET 7 1\033[0m sets the DOS "
          "version to 7.01 instead of 7.1.\n\n"
          "Type VER without parameters to display DOSBox-X and the reported "
          "DOS version.\n");
  ...
  MSG_Add("SHELL_CMD_PROMPT_HELP_LONG",
          "PROMPT [text]\n"
  ...
  MSG_Add("SHELL_CMD_ALIAS_HELP_LONG",
          "ALIAS [name[=value] ... ]\n\nType ALIAS without parameters to "
          "display the list of aliases in the form:\n`ALIAS NAME = VALUE'\n");
  MSG_Add("SHELL_CMD_ASSOC_HELP",
          "Displays or changes file extension associations.\n");
  MSG_Add("SHELL_CMD_ASSOC_HELP_LONG",
          "ASSOC [.ext[=command] ... ]\n\nType ASSOC without parameters to "
          "display the current file associations.\nFile extensions must start "
          "with a dot (.); wildcards (* and ?) are allowed.\n");
  ...
  MSG_Add("SHELL_CMD_CHCP_HELP_LONG",
          ...
          "pages for changing in the TrueType font "
          "output:\n\n437,737,775,808,850,852,853,855-869,872,874,1250-1258,"
          "3021\n\nAlso double-byte code pages including 932, 936, 949, and "
          "950/951.\n\nCustomized code pages are supported by providing code "
          "page files.\n");
  MSG_Add("SHELL_CMD_CHCP_ACTIVE", "Active code page: %d\n");
  MSG_Add("SHELL_CMD_CHCP_MISSING",
          "ASCII characters not defined in TTF font: %d\n");
  MSG_Add("SHELL_CMD_CHCP_INVALID", "Invalid code page number - %s\n");
  MSG_Add("SHELL_CMD_COUNTRY_HELP",
          "Displays or changes the current country.\n");
  MSG_Add(
      "SHELL_CMD_COUNTRY_HELP_LONG",
      "COUNTRY [nnn] \n\n  nnn   Specifies a country code.\n\nCountry-specific "
      "information such as date and time formats will be affected.\n");
  MSG_Add("SHELL_CMD_CTTY_HELP",
          "Changes the terminal device used to control the system.\n");
  MSG_Add("SHELL_CMD_CTTY_HELP_LONG", "CTTY device\n  device        The "
                                      "terminal device to use, such as CON.\n");
  ...
  MSG_Add("SHELL_CMD_TRUENAME_HELP",
          "Finds the fully-expanded name for a file.\n");
  MSG_Add("SHELL_CMD_TRUENAME_HELP_LONG", "TRUENAME [/H] file\n");
  MSG_Add("SHELL_CMD_DXCAPTURE_HELP",
          "Runs program with video or audio capture.\n");
  MSG_Add(
      "SHELL_CMD_DXCAPTURE_HELP_LONG",
      "DX-CAPTURE [/V|/-V] [/A|/-A] [/M|/-M] [/O|/-O] [command] "
      "[options]\n\nIt will start video or audio capture, run program, and "
      "then automatically stop capture when the program exits.\n /V for video, "
      "/A for audio, /M multi-track audio and /O for OPL FM (DROv2 format)");
#if C_DEBUG
  MSG_Add("SHELL_CMD_DEBUGBOX_HELP",
          "Runs program and breaks into debugger at entry point.\n");
  MSG_Add("SHELL_CMD_DEBUGBOX_HELP_LONG",
          "DEBUGBOX [command] [options]\n\nType DEBUGBOX without a parameter "
          "to start the debugger.\n");
#endif
  MSG_Add("SHELL_CMD_COMMAND_HELP",
          ...
          " Loads a permanent copy of the command shell.\n  /INIT Initializes "
          "the command shell.\n");
  ...
  // below a certain memory size, alter memory arrangement and allocation to
  // minimize memory
  if (MEM_ConventionalPages() < 0x8)
    tiny_memory_mode = true;

  // decide shell env size
  if (dosbox_shell_env_size == 0) {
    if (MEM_ConventionalPages() >= 0x10 /*64KB or more*/)
      dosbox_shell_env_size = (0x158u - (0x118u + 19u))
                              << 4u; /* equivalent to DOSBox SVN */
    ...
    dosbox_shell_env_size =
        (dosbox_shell_env_size + 15u) & (~15u); /* round up to paragraph */
  }

  LOG_MSG("COMMAND.COM env size:             %u bytes", dosbox_shell_env_size);

  // According to some sources, 0x0008 is a special PSP segment value used by
  // DOS before the first program is used. We need the current PSP segment to be
  // nonzero so that DOS_AllocateMemory() can properly allocate memory.
  dos.psp(8);

  auto savedMemAllocStrategy = DOS_GetMemAllocStrategy();
  auto shellHigh =
      std::string(static_cast<Section_prop *>(control->GetSection("dos"))
                      ->Get_string("shellhigh"));
  ...
  if (!DOS_AllocateMemory(&env_seg, &tmp))
    E_Exit("COMMAND.COM failed to allocate environment block segment");
  ...
  if (MEM_ConventionalPages() > 1) {
    if (!DOS_AllocateMemory(&psp_seg, &tmp))
      E_Exit("COMMAND.COM failed to allocate main body + PSP segment");
  } else {
  ...
  LOG(LOG_MISC, LOG_DEBUG)("Shell init SS:SP %04x:%04x",
                           (unsigned int)stack_seg, (unsigned int)reg_sp);
  ...
  /* 2023/09/28: Point INT 23h at a vector that calls our callback and then
   * calls INT 21h AH=4Ch. Real COMMAND.COM does this too. */
  if (call_int23 == 0)
    call_int23 = CALLBACK_Allocate();

  RealPt addr_int23 = RealMake(psp_seg, 8 + ((16 + 2) * 16));

  CALLBACK_Setup(call_int23, &INT23_Handler, CB_RETF, Real2Phys(addr_int23),
                 "Shell Int 23 CTRL+C");
  ...
  // NTS: It's apparently common practice to enumerate MCBs by reading the
  // segment value of INT 2Eh and then
  //      scanning forward from there. The assumption seems to be that
  //      COMMAND.COM writes INT 2Eh there using it's PSP segment and an offset
  //      like that of a COM executable even though COMMAND.COM is often an EXE
  //      file.
  RealPt addr_int2e = RealMake(psp_seg, 8 + ((16 + 1) * 16));

  CALLBACK_Setup(call_int2e, &INT2E_Handler, CB_IRET_STI, Real2Phys(addr_int2e),
                 "Shell Int 2e");
  ...
  if (!DOS_OpenFile("PRN", OPEN_READWRITE, &dummy))
...
void SHELL_Run() {
  ...
  if (first_shell != NULL)
    E_Exit("Attempt to start shell when shell already running");
  Section_prop *section =
  ...
  first_shell->Prepare();
  prepared = true;
  if (section != NULL && !control->opt_noconfig && !control->opt_securemode &&
      !control->SecureMode()) {
    ...
    if (strlen(shell)) {
      ...
      if (*name &&
          (DOS_FileExists(name) ||
           DOS_FileExists((std::string("Z:\\SYSTEM\\") + name).c_str()) ||
           DOS_FileExists((std::string("Z:\\BIN\\") + name).c_str()) ||
           DOS_FileExists((std::string("Z:\\DOS\\") + name).c_str()) ||
           DOS_FileExists((std::string("Z:\\4DOS\\") + name).c_str()) ||
           DOS_FileExists((std::string("Z:\\DEBUG\\") + name).c_str()) ||
           DOS_FileExists((std::string("Z:\\TEXTUTIL\\") + name).c_str()))) {
        strreplace(name, '/', '\\');
        altshell = true;
      } else if (*name)
  ...
  i4dos = false;
  if (altshell) {
    if (strstr(name, "4DOS.COM"))
      i4dos = true;
    first_shell->SetEnv("COMSPEC", name);
    if (!strlen(tmp)) {
      char *p = strrchr(name, '\\');
      if (!strcasecmp(p == NULL ? name : p + 1, "COMMAND.COM") ||
          !strcasecmp(name, "Z:COMMAND.COM")) {
        strcpy(tmpstr, init_line);
        tmp = tmpstr;
      } else if (!strcasecmp(p == NULL ? name : p + 1, "4DOS.COM") ||
                 !strcasecmp(name, "Z:4DOS.COM")) {
        strcpy(tmpstr, "AUTOEXEC.BAT");
        tmp = tmpstr;
      }
    }
