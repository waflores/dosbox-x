...
void DOS_Shell::ShowPrompt(void) {
  ...
  dir[0] = 0; // DOS_GetCurrentDir doesn't always return something. (if drive is
              // messed up)
  ...
  while (*promptstr) {
    if (!strcasecmp(promptstr, "$"))
      ...
      case 'C':
        ...
        break;
      case 'D':
        WriteOut("%02d-%02d-%04d", dos.date.day, dos.date.month, dos.date.year);
        break;
      case 'E':
        WriteOut("%c", 27);
        break;
      ...
      case 'L':
        ...
        break;
      case 'N':
        WriteOut("%c", DOS_GetDefaultDrive() + 'A');
        break;
      case 'P':
        WriteOut("%c:\\", DOS_GetDefaultDrive() + 'A');
      ...
      case 'S':
        ...
        break;
      case 'T': {
        Bitu ticks = (Bitu)(((65536.0 * 100.0) / (double)PIT_TICK_RATE) *
      ...
      case 'V':
        WriteOut("DOSBox-X version %s. Reported DOS version %d.%d.", VERSION,
                 dos.version.major, dos.version.minor);
        break;
#else
      case 'V':
        WriteOut("DOSBox-X version %s. Reported DOS version %d.%d.",
                 PACKAGE_VERSION, dos.version.major, dos.version.minor);
        break;
#endif
      case '$':
        WriteOut("$");
...
//! \brief Moves the caret to prev row/last column when column is 0 (video mode
//! 0).
...
bool DOS_Shell::BuildCompletions(char *line, uint16_t str_len) {
  ...
  bool dir_only = (strncasecmp(ltrim(line), "CD ", 3) == 0) ||
                  (strncasecmp(ltrim(line), "MD ", 3) == 0) ||
                  (strncasecmp(ltrim(line), "RD ", 3) == 0) ||
                  (strncasecmp(ltrim(line), "CHDIR ", 6) == 0) ||
                  (strncasecmp(ltrim(line), "MKDIR ", 3) == 0) ||
                  (strncasecmp(ltrim(line), "RMDIR ", 6) == 0);
  int q = 0, r = 0, k = 0;
  ...
  if ((path = strrchr(line + completion_index, ':')))
    completion_index = (uint16_t)(path - line + 1);
  if ((path = strrchr_dbcs(line + completion_index, '\\')))
    completion_index = (uint16_t)(path - line + 1);
  if ((path = strrchr(line + completion_index, '/')))
    completion_index = (uint16_t)(path - line + 1);
  ...
  if (p_completion_start && strlen(p_completion_start) + 3 >= DOS_PATHLENGTH) {
    // TODO: This really should be done in the CON driver so that this code can
    // just print ASCII code 7 instead
    if (IS_PC98_ARCH) {
  ...
  while (res) {
    ...
    if (strcmp(name, ".") && strcmp(name, "..")) {
      if (dir_only) { // Handle the dir only case different (line starts with
                      // cd)
        if (att & DOS_ATTR_DIRECTORY)
          l_completion.emplace_back(qlname);
      } else {
        const char *ext = strrchr(name, '.'); // file extension
        if ((ext && (strcmp(ext, ".BAT") == 0 || strcmp(ext, ".COM") == 0 ||
                     strcmp(ext, ".EXE") == 0)) ||
            hasAssociation(name).size())
          // we add executables to a separate list and place that list in front
          // of the normal files
...
static uint16_t DeleteBackspace(bool delete_flag, char *line,
                                uint16_t &str_index, uint16_t &str_len) {
...
void DOS_Shell::InputCommand(char *line) {
  ...
  while (size) {
    ...
    if (!DOS_ReadFile(input_handle, &c, &n)) {
      LOG(LOG_MISC,
          LOG_ERROR)("SHELL: Lost the input handle, dropping shell input loop");
      n = 0;
    }
    if (!n) {
      input_eof = true;
    ...
    if (input_handle != STDIN) { /* FIXME: Need DOS_IsATTY() or somesuch */
      ...
      /* shift state is needed for some key combinations not directly supported
       * by CON driver. bit 4 = CTRL bit 3 = GRPH/ALT bit 2 = kana bit 1 = caps
       * bit 0 = SHIFT */
      uint8_t shiftstate = mem_readb(0x52A + 0x0E);

      /* NTS: PC-98 keyboards lack the US layout HOME / END keys, therefore
       * there is no mapping here */

      /* NTS: Since left arrow and backspace map to the same byte value, PC-98
       * treats it the same at the DOS prompt. However the PC-98 version of
       * DOSKEY seems to be able to differentiate the two anyway and let the
       * left arrow move the cursor back (perhaps it's calling INT 18h directly
       * then?) */
      if (c == 0x0B)
        ...
        /* IBM extended code left arrow OR backspace. use last scancode to tell
         * which as DOSKEY apparently can. */
    ...
    case 0x4B00: /* LEFT */
      if (IS_PC98_ARCH
        ...
        if ((line[str_index] & 0xFF) != (uint8_t)(get_char & 0xFF)) {
          ...
          if ((uint8_t)(get_char2 & 0xFF) == (line[str_index] & 0xFF))
            INT10_SetCursorPos(row, col - 1, page);
        }
      } else {
        if (isDBCSCP()
#if defined(USE_TTF)
            ...
            (line[str_index - 1] < 0 ||
             ((dos.loaded_codepage == 932 ||
               (dos.loaded_codepage == 936 && gbk) ||
               dos.loaded_codepage == 950 || dos.loaded_codepage == 951) &&
              line[str_index - 1] >= 0x40)) &&
            line[str_index - 2] < 0) {
          backone();
          str_index--;
          MoveCaretBackwards();
    ...
    case 0x4D00: /* RIGHT */
      if (IS_PC98_ARCH || (isDBCSCP()
        ...
        col = CURSOR_POS_COL(page);
        BIOS_NCOLS;
        if (col == ncols - 1 && str_index < str_len &&
            isKanji1(line[str_index]) && isKanji2(line[str_index + 1])) {
          row = CURSOR_POS_ROW(page);
        ...
        if (isDBCSCP()
            ...
            && dbcs_sbcs
#endif
            && str_index < str_len - 1 && line[str_index] < 0 &&
            (line[str_index + 1] < 0 ||
             ((dos.loaded_codepage == 932 ||
               (dos.loaded_codepage == 936 && gbk) ||
               dos.loaded_codepage == 950 || dos.loaded_codepage == 951) &&
              line[str_index + 1] >= 0x40))) {
          if (isKanji1((uint8_t)line[str_index]) &&
              isKanji2((uint8_t)line[str_index + 1])) {
            const uint8_t buf[2] = {(uint8_t)line[str_index],
                                    (uint8_t)line[str_index + 1]};
            uint16_t num = 2;
    ...
    case 0x5200:          /* INS */
      if (IS_PC98_ARCH) { // INS state handled by IBM PC/AT BIOS, faked for
                          // PC-98 mode
        extern bool pc98_doskey_insertmode;

        // NTS: No visible change to the cursor, just like DOSKEY on PC-98
        // MS-DOS
    ...
    case 0x5300: /* DELETE */
      if (IS_PC98_ARCH
        ...
        if (col2 >= ncols - (k == 2 ? 3 : 1) && row < nrows) {
          for (int i = 0; i <= k; i++)
            WriteChar(i, row + 1, page, ' ', 0, false);
        }
        col2 = CURSOR_POS_COL(page); // LOG_MSG("col=%d, a=%d", col,a);
        if (col2 >= a)
          INT10_SetCursorPos(row, col2 - a, page);
        else {
          uint16_t lines_up = (a - col2 - 1) / ncols + 1;
          if (col2 < ncols)
            for (int i = col2; i < ncols; i++)
              WriteChar(i, row, page, ' ', 0, false);
          if (row >= lines_up)
            INT10_SetCursorPos(row - lines_up, col, page);
          else
    ...
    case 0x08: /* BackSpace */
      if (IS_PC98_ARCH
        ...
        if (str_index < str_len)
          memmove(&line[str_index - bytes], &line[str_index],
                  str_len - str_index); // Shift buffer to left
    ...
    case '':    // FAKE CTRL-C
      *line = 0; // reset the line.
      ...
      DOS_BreakFlag =
          false; // clear break flag so the next program doesn't get hit with it
    ...
    case 0x9400: /* Ctrl-Tab */
    {
      ...
      for (col = mrow; col > 0; col--) {
        ...
        if (col == 1)
          break;
        w_count = 0;
        for (std::list<std::string>::iterator source = l_completion.begin();
             source != l_completion.end(); ++source) {
    ...
    case 0x1b: /* ESC */
      ...
      //      the next character input is thrown away before resuming normal
      //      keyboard input.
      //
      //      DOSBox / DOSBox-X have always acted as if DOSKEY is loaded in a
      //      fashion, so we'll emulate the PC-98 DOSKEY behavior here.
      //
      //      DOSKEY on PC-98 is able to clear the whole prompt and even bring
      //      the cursor back up to the first line if the input crosses multiple
      //      lines.

      // NTS: According to real IBM/Microsoft PC/AT DOS:
      //      If DOSKEY is loaded, ESC clears the prompt
      //      If DOSKEY is NOT loaded, ESC prints a backslash and goes to the
      //      next line. The Windows 95 version of DOSKEY puts the cursor at a
      //      horizontal position that matches the DOS prompt (not emulated
      //      here).
      //
      //      DOSBox / DOSBox-X have always acted as if DOSKEY is loaded in a
      //      fashion, so we'll emulate DOSKEY behavior here.
      while (str_index < str_len) {
    ...
    default:
      if (IS_PC98_ARCH
        ...
        if (dos.loaded_codepage == 932 && isKanji1(cr >> 8) && (cr & 0xFF) == 0)
          break;
        bool kanji_flag = false;
            ...
            if (isDBCSCP()) {
              if (read_lead_byte) {
                ...
                for (uint16_t i = str_index + 1; i < str_len; i++) {
                  line[i] = line[i + 1]; // Delete trailing byte of DBCS since
                                         // leading byte is overwritten
                }
...
void DOS_Shell::ProcessCmdLineEnvVarStitution(char *line) {
  ...
  /* continue scanning for the ending '%'. variable names are apparently meant
   * to be alphanumeric, start with a letter, without spaces (if Windows 95
   * COMMAND.COM is any good example). If it doesn't end in '%' or is broken by
   * space or starts with a number, substitution is not carried out. In the
   * middle of the variable name it seems to be acceptable to use hyphens.
   *
   * since spaces break up a variable name to prevent substitution, these
   * commands act differently from one another:
   *
   * C:\>echo %PATH%
   * C:\DOS;C:\WINDOWS
   *
  ...
  if (((size_t)(r + 1 - line)) >= CMD_MAXLINE) {
    LOG_MSG("DOS_Shell::ProcessCmdLineEnvVarStitution WARNING incoming string "
            "to substitute is already too long!\n");
  ...
  // Variable names must start with non-digits. Space and special symbols like _
  // and ~ are apparently valid too (MS-DOS 7/Windows 9x).
  ...
  while (std::regex_search(text, match, re)) {
    ...
    if (variable_name.size() &&
        GetEnvStr(variable_name.c_str(), variable_value)) {
      const size_t equal_pos = variable_value.find_first_of('=');
      /* Replace the original %var% with its corresponding value from the
       * environment */
...
bool DOS_Shell::Execute(char *name, const char *args) {
  ...
  if (((strcmp(name + 1, ":") == 0) || (strcmp(name + 1, ":\\") == 0)) &&
    ...
    if (!DOS_SetDrive(toupper(name[0]) - 'A')) {
      ...
      if (!mountwarning && type != DRIVE_NO_ROOT_DIR)
        goto continue_1;
      if (type == DRIVE_FIXED && (strcasecmp(name, "C:") == 0))
        WriteOut(MSG_Get("PROGRAM_MOUNT_WARNING_WIN"));
    first_1:
      if (type == DRIVE_CDROM)
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_ACCESS_CDROM"), toupper(name[0]));
      else if (type == DRIVE_REMOVABLE &&
               (strcasecmp(name, "A:") == 0 || strcasecmp(name, "B:") == 0))
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_ACCESS_FLOPPY"),
                 toupper(name[0]));
      else if (type == DRIVE_REMOVABLE)
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_ACCESS_REMOVABLE"),
                 toupper(name[0]));
      else if (type == DRIVE_REMOTE)
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_ACCESS_NETWORK"),
                 toupper(name[0]));
      else if (type == DRIVE_FIXED || type == DRIVE_RAMDISK ||
               type == DRIVE_UNKNOWN)
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_ACCESS_FIXED"), toupper(name[0]));
      else {
        WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_NOT_FOUND"), toupper(name[0]));
        return true;
      }
    ...
    continue_1:
      ...
      if (type == DRIVE_CDROM)
        strcat(mountstring, "-t cdrom ");
               ...
               (strcasecmp(name, "A:") == 0 || strcasecmp(name, "B:") == 0))
        strcat(mountstring, "-t floppy ");
      strcat(mountstring, name);
      strcat(mountstring, "\\");
      //			if(GetDriveType(name)==5) strcat(mountstring,"
      //-ioctl");
      nowarn = true;
  ...
  if (strcasecmp(extension, ".bat") == 0) { /* Run the .bat file */
    ...
    memset(&cmdtail.buffer, 0,
           CTBUF); // Else some part of the string is uninitialized (valgrind)
    if (strlen(line) >= CTBUF)
      line[CTBUF - 1] = 0;
     ...
     * notable the one for /AAA => A\0AA Qbix: It is extremely messy, but this
     * was the only way I could get things like /:aa and :/aa to work correctly
     */
    ...
    for (char *pl = line, *q = parseline; *pl; pl++, q++) {
      if (*pl == '=' || *pl == ';' || *pl == ',' || *pl == '\t' || *pl == ' ')
        ...
        *q = *pl; // Replace command separators with 0.
      ...
      if (*p == '/') { // Transform /Hello into H\0ello
        *p = 0;
        ...
        while (*p == 0 && (p - parseline) < 250)
          p++; // Skip empty fields
        if ((p - parseline) <
            250) { // Found something. Lets get the first letter and break it up
          p++;
          memmove(static_cast<void *>(p + 1), static_cast<void *>(p),
                  (250u - (unsigned int)(p - parseline)));
          if ((p - parseline) < 250)
            *p = 0;
        }
      }
    ...
    while (parseline[skip] != 0)
      skip++; // This is safe as there is always a 0 in parseline at the end.
    while (skip < 256 && parseline[skip] == 0)
    ...
    if (packerr && infix < 0 && sec->Get_bool("autoa20fix")) {
      LOG(LOG_DOSMISC,
          LOG_DEBUG)("Attempting autoa20fix workaround for EXEPACK error");
      if (autofixwarn == 1 || autofixwarn == 3)
        WriteOut("\r\n\033[41;1m\033[1;37;1mDOSBox-X\033[0m Failed to load the "
                 "executable\r\n\033[41;1m\033[37;1mDOSBox-X\033[0m Now try "
                 "again with A20 fix...\r\n");
      infix = 0;
      ...
      uint16_t segment;
      uint16_t blocks =
          (uint16_t)(1); /* start with one paragraph, resize up later. see if it
                            comes up below the 64KB mark */
      if (DOS_AllocateMemory(&segment, &blocks)) {
        ...
        mcb.SetPSPSeg(
            0x40); /* FIXME: Wouldn't 0x08, a magic value used to show ownership
                      by MS-DOS, be more appropriate here? */
        LOG(LOG_DOSMISC,
            LOG_DEBUG)("Attempting autoloadfix workaround for EXEPACK error");
        if (autofixwarn == 2 || autofixwarn == 3)
          WriteOut("\r\n\033[41;1m\033[1;37;1mDOSBox-X\033[0m Failed to load "
                   "the executable\r\n\033[41;1m\033[37;1mDOSBox-X\033[0m Now "
                   "try again with LOADFIX...\r\n");
        infix = 1;
        Execute(name, args);
        infix = -1;
...
std::string DOS_Shell::hasAssociation(const char *name) {
  ...
  if (iter != cmd_assoc.end()) {
    assocs =
        strcasecmp(extension.c_str(), iter->second.c_str()) ? iter->second : "";
    return assocs;
  }
  std::transform(extension.begin(), extension.end(), extension.begin(),
                 ::toupper);
  for (cmd_assoc_map_t::iterator iter = cmd_assoc.begin();
       iter != cmd_assoc.end(); ++iter) {
    ...
    if (wild_match(extension.c_str(), (char *)ext.c_str())) {
      assocs = strcasecmp(extension.c_str(), iter->second.c_str())
                   ? iter->second
                   : "";
      return assocs;
    }
  ...
  while (*pathenv) {
    ...
    if (size_t len = strlen(path)) {
      ...
      if (uselfn && len > 3) {
        if (path[len - 1] == '\\')
          path[len - 1] = 0;
        if (DOS_GetSFNPath(("\"" + std::string(path) + "\"").c_str(), s_ret,
                           false))
          strcpy(path, s_ret);
        len = strlen(path);
      }
      ...
      if ((name_len + len + 1) >= DOS_PATHLENGTH)
        continue;
      strcat(path, strchr(name, ' ') ? ("\"" + std::string(name) + "\"").c_str()
                                     : name);

      if (hasAssociation(path).size() && DOS_FileExists(path)) {
        strcpy(which_ret, path);
        return strchr(which_ret, '\"') &&
                       DOS_GetSFNPath(which_ret, s_ret, false)
                   ? s_ret
                   : which_ret;
