 ...
 *  ATTRIB, CHCP, COUNTRY, DELTREE, FOR/LFNFOR, POPD/PUSHD, TREE, TRUENAME,
 * VERIFY commands by Wengier LS command by the DOSBox Staging Team and Wengier
 */
...
SHELL_Cmd cmd_list[] = {
    ...
    //{	"CHOICE",		1,		&DOS_Shell::CMD_CHOICE,
    //"SHELL_CMD_CHOICE_HELP"}, // CHOICE as a program (Z:\DOS\CHOICE.COM)
    ...
    //{	"COUNTRY",		1,		&DOS_Shell::CMD_COUNTRY,
    //"SHELL_CMD_COUNTRY_HELP"}, // COUNTRY as a program (Z:\SYSTEM\COUNTRY.COM)
    //instead of shell command
    ...
    //{	"DELTREE",		1,		&DOS_Shell::CMD_DELTREE,
    //"SHELL_CMD_DELTREE_HELP"}, // DELTREE as a program (Z:\DOS\DELTREE.EXE)
    //instead of shell command
    ...
    //{	"HELP",			1,		&DOS_Shell::CMD_HELP,
    //"SHELL_CMD_HELP_HELP"}, // HELP as a program (Z:\SYSTEM\HELP.COM) instead
    //of shell command
    ...
    //{   "LS",			1,		&DOS_Shell::CMD_LS,
    //"SHELL_CMD_LS_HELP"}, // LS as a program (Z:\BIN\LS.COM) instead of shell
    //command
...
//{	"INT2FDBG",		1,		&DOS_Shell::CMD_INT2FDBG,
//"SHELL_CMD_INT2FDBG_HELP"}, // INT2FDBG as a program (Z:\DEBUG\INT2FDBG.COM)
//instead of shell command
#endif
    // Advanced commands specific to DOSBox-X
    //{	"ADDKEY",		1,		&DOS_Shell::CMD_ADDKEY,
    //"SHELL_CMD_ADDKEY_HELP"}, // ADDKEY as a program (Z:\BIN\ADDKEY.COM)
    //instead of shell command
...
extern bool date_host_forced, usecon, outcon, rsize, autoboxdraw, dbcs_sbcs,
    sync_time, manualtime, inshell, noassoc, dotype, loadlang, chinasea;
...
extern bool isDBCSCP(), isKanji1(uint8_t chr), shiftjis_lead_byte(int c),
    TTF_using(void), Network_IsNetworkResource(const char *filename);
extern bool CheckBoxDrawing(uint8_t c1, uint8_t c2, uint8_t c3, uint8_t c4),
    GFX_GetPreventFullscreen(void), DOS_SetAnsiAttr(uint8_t attr);
...
extern void MAPPER_AutoType(std::vector<std::string> &sequence,
                            const uint32_t wait_ms, const uint32_t pace_ms,
                            bool choice);
...
bool CheckDBCSCP(int32_t codepage),
    SwitchLanguage(int oldcp, int newcp, bool confirm);
...
static char *ExpandDot(char *args, char *buffer, size_t bufsize, bool expand) {
  if (*args == '.') {
    ...
    if ((*(args + 1) != '.') && (*(args + 1) != '\\') && expand) {
      ...
      if (bufsize > 2)
        strncat(buffer, args,
                bufsize - 1 /*used buffer portion*/ - 1 /*trailing zero*/);
      return buffer;
    } else
  ...
  if (find_last != NULL && *find_last == '.') {
    ...
    if ((*(find_last + 1) != '.')) {
      ...
      *find_last = '.';
      ...
      buffer[len + 1] = 0;
      if (bufsize > len + 2)
        strncat(buffer, find_last,
                bufsize - len - 1 /*used buffer portion*/ -
                    1 /*trailing zero*/);
      else
        safe_strncpy(buffer, args, bufsize);
      return buffer;
...
bool DOS_Shell::CheckConfig(char *cmd_in, char *line) {
  bool quote = false;
  if (strlen(cmd_in) > 2 && cmd_in[0] == '"' &&
      cmd_in[strlen(cmd_in) - 1] == '"') {
  ...
  char newcom[1024];
  newcom[0] = 0;
  strcpy(newcom, "z:\\system\\config -set ");
  if (line != NULL) {
    line = trim(line);
    if (*line == '=')
      line = trim(++line);
...
__do_command_begin:
  if (alias_counter > 64) {
    WriteOut(MSG_Get("SHELL_EXECUTE_ALIAS_EXPAND_OVERFLOW"),
             origin_cmd_line.c_str());
  }
  line = trim(line);
  ...
  while (*line) {
    ...
    if ((q & 1) == 0) {
      ...
      if (*line == '.' || *line == ';' ||
          (*line == ':' && !(c == 1 && tolower(*(line - 1)) >= 'a' &&
                             tolower(*(line - 1)) <= 'z')) ||
          *line == '[' || *line == ']' || *line == '\\' || *line == '/' ||
          *line == '\"' ||
          *line == '+') { // allow stuff like cd.. and dir.exe cd\kees
        *cmd_write = 0;
  ...
  *cmd_write = 0;
  if (strlen(cmd_buffer) == 0) {
    if (strlen(line) && line[0] == '/')
      WriteOut(MSG_Get("SHELL_EXECUTE_ILLEGAL_COMMAND"), line);
    return;
  }
  ...
  if (strchr(cmd_buffer, '\"') && DOS_GetSFNPath(cmd_buffer, ldir, false)) {
    ...
    if (uselfn && strchr(p, ' ') &&
        !DOS_FileExists(("\"" + std::string(p) + "\"").c_str())) {
      bool append = false;
      if (DOS_FileExists(("\"" + std::string(p) + ".COM\"").c_str())) {
        append = true;
        strcat(p, ".COM");
      } else if (DOS_FileExists(("\"" + std::string(p) + ".EXE\"").c_str())) {
        append = true;
        strcat(p, ".EXE");
      } else if (DOS_FileExists(("\"" + std::string(p) + ".BAT\"").c_str())) {
        append = true;
        strcat(p, ".BAT");
      }
      if (append && DOS_GetSFNPath(("\"" + std::string(p) + "\"").c_str(),
                                   cmd_buffer, false))
        if (Execute(cmd_buffer, line))
          return;
    }
    if (Execute(p, line))
      return;
  } else if (Execute(cmd_buffer, line))
    return;
  if (enable_config_as_shell_commands && CheckConfig(cmd_buffer, line))
    return;
  std::string errhandler =
      static_cast<Section_prop *>(control->GetSection("dos"))
          ->Get_string("badcommandhandler");
...
static Bitu INT2FDBG_Handler(void) {
  if (reg_ax == 0x1605) { /* Windows init broadcast */
    ...
    LOG_MSG("INT 2Fh debug hook: Caught Windows init broadcast results "
            "(ES:BX=%04x:%04x DS:SI=%04x:%04x CX=%04x DX=%04x DI=%04x)\n",
            SegValue(es), reg_bx, SegValue(ds), reg_si, reg_cx, reg_dx, reg_di);
    ...
    while (st_seg != 0 || st_ofs != 0) {
      ...
      if (--patience <= 0) {
        LOG_MSG("**WARNING: Chain is too long. Something might have gotten "
                "corrupted\n");
        break;
      }
       ...
       * +0x0A: virtual device ref data (pointer to?? or actual data??) or
       * 0000:0000 +0x0E: pointer to instance data records or 0000:0000 Windows
       * 95 or later (v4.0+): +0x12: pointer to optionally-instanced data
       * records or 0000:0000 */
      ...
      idrc_ofs =
          mem_readw(st_o + 0x0A + 4); /* FIXME: 0x0E+0 and 0x0E+2 generates
                                         weird compiler error WTF?? */
...

      {
        ...
        if (name_seg != 0 || name_ofs != 0) {
          ...
          for (i = 0; i < 63 && (c = mem_readb(scan++)) != 0;)
            devname[i++] = (char)c;
          devname[i] = 0;
        }
      }
...
void Int2fhook() {
  ...
  CALLBACK_Setup(int2fdbg_hook_callback, &INT2FDBG_Handler, CB_IRET,
                 "INT 2Fh DBG callback");
  ...
  /* overwrite the callback with code to chain the call down, then invoke our
  ...
  phys_writew(w, (uint16_t)(old_int2Fh & 0xFFFF));
  w += 2; // offset
  phys_writew(w, (uint16_t)((old_int2Fh >> 16) & 0xFFFF));
  w += 2; // seg
  ...
  phys_writew(w, (uint16_t)int2fdbg_hook_callback);
  w += 2; // The immediate word

  /* return */
  phys_writeb(w++, (uint8_t)0xCF); // IRET
}

/* NTS: I know I could just modify the DOS kernel's INT 2Fh code to receive the
 * init call, the problem is that at that point, the registers do not yet
 * contain anything interesting. all the interesting results of the call are
 * added by TSRs on the way back UP the call chain. The purpose of this program
 * therefore is to hook INT 2Fh on the other end of the call chain so that we
 * can see the results just before returning INT 2Fh back to WIN.COM */
...
void DOS_Shell::CMD_DELETE(char *args) {
  ...
  DOS_DTA dta(dos.dta());
  /* If delete accept switches mind the space in front of them. See the dir /p
   * code */
  ...
  if (strlen(args) && args[strlen(args) - 1] != '\\') {
    uint16_t fattr;
    if (strcmp(args, "*.*") && DOS_GetFileAttr(args, &fattr) &&
  ...
  if (strlen(args) && args[strlen(args) - 1] == '\\')
    strcat(args, "*.*");
           ...
           (strlen(args) > 1 &&
            (args[strlen(args) - 2] == ':' || args[strlen(args) - 2] == '\\') &&
            args[strlen(args) - 1] == '.')) {
    args[strlen(args) - 1] = '*';
    strcat(args, ".*");
    ...
    if (find_last == NULL)
      ...
      find_last++;
    if (strlen(find_last) > 0 && args[strlen(args) - 1] == '*' &&
        strchr(find_last, '.') == NULL)
      strcat(args, ".*");
  }
  if (!strcmp(args, "*.*") ||
      (strlen(args) > 3 && (!strcmp(args + strlen(args) - 4, "\\*.*") ||
                            !strcmp(args + strlen(args) - 4, ":*.*")))) {
    if (!optQ) {
    first_1:
...
continue_1:
  ...
  char path[DOS_PATHLENGTH], spath[DOS_PATHLENGTH], pattern[DOS_PATHLENGTH],
      *r = strrchr_dbcs(full, '\\');
  if (r != NULL) {
    *r = 0;
    strcpy(path, full);
  ...
  if (strchr(args, '\"') || uselfn) {
    if (!DOS_GetSFNPath(("\"" + std::string(path) + "\\").c_str(), spath,
                        false))
      strcpy(spath, path);
    if (!strlen(spath) || spath[strlen(spath) - 1] != '\\')
      strcat(spath, "\\");
  }
  ...
  bool res = DOS_FindFirst(
      ((uselfn && pfull.length() && pfull[0] != '"' ? "\"" : "") + pfull +
       (uselfn && pfull.length() && pfull[pfull.length() - 1] != '"' ? "\""
                                                                     : ""))
          .c_str(),
  ...
  strcpy(sfull, full);
  char *end = strrchr_dbcs(full, '\\') + 1;
  *end = 0;
  char *lend = strrchr_dbcs(sfull, '\\') + 1;
  *lend = 0;
  dta = dos.dta();
  bool exist = false;
  lfn_filefind_handle = uselfn ? LFN_FILEFIND_INTERNAL : LFN_FILEFIND_NONE;
  while (res) {
    ...
    if (!optF && (attr & DOS_ATTR_READ_ONLY) && !(attr & DOS_ATTR_DIRECTORY)) {
      ...
      if (optP) {
        ...
        if (c == ch_N) {
          lfn_filefind_handle =
              uselfn ? LFN_FILEFIND_INTERNAL : LFN_FILEFIND_NONE;
          res = DOS_FindNext();
          continue;
        }
      }
      if (strlen(full)) {
        ...
        if (optF && (attr & DOS_ATTR_READ_ONLY) &&
            DOS_SetFileAttr(pfull.c_str(), attr & ~DOS_ATTR_READ_ONLY))
          reset = true;
        if (!DOS_UnlinkFile(pfull.c_str())) {
          if (optF && reset)
            DOS_SetFileAttr(pfull.c_str(), attr);
          WriteOut(MSG_Get("SHELL_CMD_DEL_ERROR"), uselfn ? sfull : full);
        }
      } else
        WriteOut(MSG_Get("SHELL_CMD_DEL_ERROR"), uselfn ? sfull : full);
...
struct DtaResult {
  ...
  static bool groupDef(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
                   (!(lhs.attr & DOS_ATTR_DIRECTORY) &&
                    !(rhs.attr & DOS_ATTR_DIRECTORY))) &&
                  strcmp(lhs.name, rhs.name) < 0);
  }
  static bool groupExt(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
                   (!(lhs.attr & DOS_ATTR_DIRECTORY) &&
                    !(rhs.attr & DOS_ATTR_DIRECTORY))) &&
                  strcmp(lhs.getExtension(), rhs.getExtension()) < 0);
  }
  static bool groupSize(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
                   (!(lhs.attr & DOS_ATTR_DIRECTORY) &&
                    !(rhs.attr & DOS_ATTR_DIRECTORY))) &&
                  lhs.size + lhs.hsize * 0x100000000 <
                      rhs.size + rhs.hsize * 0x100000000);
  }
  static bool groupDate(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
  ...
  static bool groupRevDef(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
  ...
  static bool groupRevExt(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
  ...
  static bool groupRevSize(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
  ...
  static bool groupRevDate(const DtaResult &lhs, const DtaResult &rhs) {
    return (lhs.attr & DOS_ATTR_DIRECTORY) && !(rhs.attr & DOS_ATTR_DIRECTORY)
               ? true
               : ((((lhs.attr & DOS_ATTR_DIRECTORY) &&
                    (rhs.attr & DOS_ATTR_DIRECTORY)) ||
  ...
  static bool compareExt(const DtaResult &lhs, const DtaResult &rhs) {
    return strcmp(lhs.getExtension(), rhs.getExtension()) < 0;
  }
  static bool compareSize(const DtaResult &lhs, const DtaResult &rhs) {
    return lhs.size + lhs.hsize * 0x100000000 <
           rhs.size + rhs.hsize * 0x100000000;
  }
...
static bool doDeltree(DOS_Shell *shell, char *args, DOS_DTA dta, bool optY,
                      bool first) {
  const char ch_y = MSG_Get("INT21_6523_YESNO_CHARS")[0];
  const char ch_n = MSG_Get("INT21_6523_YESNO_CHARS")[1];
  const char ch_Y = toupper(ch_y);
  ...
  char spath[DOS_PATHLENGTH], sargs[DOS_PATHLENGTH + 4],
      path[DOS_PATHLENGTH + 4], full[DOS_PATHLENGTH], sfull[DOS_PATHLENGTH + 2];
  ...
  if (!uselfn || !DOS_GetSFNPath(args, sfull, true))
    strcpy(sfull, full);
  sprintf(sargs, "\"%s\"", spath);
  bool found = false, fdir = false,
       res = DOS_FindFirst(sargs, 0xffff & ~DOS_ATTR_VOLUME);
  if (!res)
    return false;
  // end can't be 0, but if it is we'll get a nice crash, who cares :)
  uint16_t attribute = 0;
  strcpy(path, full);
  if (!first && strlen(args) > 3 &&
      !strcmp(args + strlen(args) - 4, "\\.\\.")) {
    if (strlen(path) && path[strlen(path) - 1] == '\\')
      path[strlen(path) - 1] = 0;
    if (strlen(path) && path[strlen(path) - 1] != ':') {
      bool reset = false;
      if (DOS_GetFileAttr(path, &attribute) &&
          (attribute & DOS_ATTR_READ_ONLY) &&
          DOS_SetFileAttr(path, attribute & ~DOS_ATTR_READ_ONLY))
        reset = true;
      if (!DOS_RemoveDir(path) && !(uselfn && DOS_RemoveDir(sfull))) {
        if (reset)
          DOS_SetFileAttr(path, attribute);
        shell->WriteOut(MSG_Get("SHELL_CMD_RMDIR_ERROR"),
                        uselfn ? sfull : full);
      }
    }
    return true;
  ...
  *lend = 0;
    ...
    if (!((!strcmp(name, ".") || !strcmp(name, "..")) &&
      ...
      if (strlen(full) &&
        ...
        if (attr & DOS_ATTR_DIRECTORY) {
          if (strcmp(name, ".") && strcmp(name, "..")) {
            if (!optY && first) {
              shell->WriteOut(MSG_Get("SHELL_CMD_RMDIR_FULLTREE_CONFIRM"),
                              uselfn ? sfull : full);
              DOS_ReadFile(STDIN, &c, &n);
              if (c == 3) {
                shell->WriteOut("^C\r\n");
          ...
          if (!optY && first) {
            shell->WriteOut(MSG_Get("SHELL_CMD_RMDIR_SINGLE_CONFIRM"),
                            uselfn ? sfull : full);
            DOS_ReadFile(STDIN, &c, &n);
            if (c == 3) {
              shell->WriteOut("^C\r\n");
          ...
          pfull =
              (uselfn || strchr(uselfn ? sfull : full, ' ')
                   ? ((uselfn ? sfull : full)[0] != '"' ? "\"" : "")
                   : "") +
              std::string(uselfn ? sfull : full) +
              (uselfn || strchr(uselfn ? sfull : full, ' ')
                   ? ((uselfn ? sfull
                              : full)[strlen(uselfn ? sfull : full) - 1] != '"'
                          ? "\""
                          : "")
                   : "");
  ...
  if (!first && strlen(args) > 4 && !strcmp(args + strlen(args) - 4, "\\*.*")) {
    end = strrchr_dbcs(full, '\\') + 1;
    *end = 0;
    lend = strrchr_dbcs(sfull, '\\') + 1;
    *lend = 0;
    if (fdir) {
      strcpy(spath, path);
      strcat(spath, ".\\.");
...
bool CheckBreak(DOS_Shell *shell) {
  if (ctrlbrk || dos.errorcode == 77) {
    if (dos.errorcode == 77)
      ...
      uint16_t n = 1;
      DOS_ReadFile(STDIN, &c, &n);
      if (c == 3 && (inshell || dos.errorcode == 77))
        shell->WriteOut("^C\r\n");
      if (dos.errorcode == 77)
        dos.errorcode = 0;
      ctrlbrk = false;
...
bool cont[200];
static bool doTree(DOS_Shell *shell, char *args, DOS_DTA dta, bool optA,
                   bool optF) {
  char *p = strchr(args, ':');
  ...
  if (!uselfn || !DOS_GetSFNPath(args, sfull, true))
    strcpy(sfull, full);
  if (level && strlen(sfull) > 4 &&
      !strcasecmp(sfull + strlen(sfull) - 4, "\\*.*")) {
    *(sfull + strlen(sfull) - 4) = 0;
    p = strrchr_dbcs(sfull, '\\');
    char c = optA ? (last ? '\\' : '+') : (last ? 0xc0 : 0xc3);
    cont[level] = !last;
    for (int i = 1; i < level; i++)
      shell->WriteOut("%c   ", cont[i] ? (optA ? '|' : 0xb3) : ' ');
  ...
  *lend = 0;
  ...
  while (res) {
    if (CheckBreak(shell))
      return false;
    strcpy(spath, ((plast || (level == 1 && last) ? "-" : "") +
    ...
    if (!((!strcmp(name, ".") || !strcmp(name, "..")) &&
      ...
      if (strlen(full) &&
        ...
        if (attr & DOS_ATTR_DIRECTORY) {
          ...
          for (int i = 1; i <= level; i++)
            shell->WriteOut("%c   ", (i == 1 && level > 1 ? !plast : cont[i])
                                         ? (optA ? '|' : 0xb3)
                                         : ' ');
          shell->WriteOut("    %s\n", uselfn ? lname : name);
        }
...
void DOS_Shell::CMD_TREE(char *args) {
  ...
  while (!tdirs.empty()) {
    if (!doTree(this, (char *)tdirs.begin()->c_str(), dta, optA, optF))
      break;
    tdirs.erase(tdirs.begin());
  }
  inshell = false;
...
void DOS_Shell::CMD_HELP(char *args) {
  ...
  bool show = false;
  while (cmd_list[cmd_index].name) {
    if (optall || (*args && !strcmp(args, cmd_list[cmd_index].name)) ||
        (!*args && !cmd_list[cmd_index].flags)) {
      show = true;
      if (*args && !strcmp(args, cmd_list[cmd_index].name) && !optall) {
        ...
        if (cmd == "CD")
          ...
          cmd = "RENAME";
        else if (cmd == "DX-CAPTURE")
          cmd = "DXCAPTURE";
        WriteOut("%s\n%s", MSG_Get(cmd_list[cmd_index].help),
                 MSG_Get(("SHELL_CMD_" + cmd + "_HELP_LONG").c_str()));
      } else {
        WriteOut("<\033[34;1m%-8s\033[0m> %s", cmd_list[cmd_index].name,
  ...
  if (optall && show)
    ...
    if (argc != "" && argc != "CWSDPMI")
      DoCommand((char *)(argc + (argc == "DOS4GW" || argc == "DOS32A" ||
                                         argc == "ZIP" || argc == "UNZIP"
                                     ? ""
                                     : " /?"))
                    .c_str());
...
void DOS_Shell::CMD_RENAME(char *args) {
  ...
  uint8_t attr;
  char name[DOS_NAMELENGTH_ASCII], lname[LFN_NAMELENGTH + 1],
      tname1[LFN_NAMELENGTH + 1], tname2[LFN_NAMELENGTH + 1],
      text1[LFN_NAMELENGTH + 1], text2[LFN_NAMELENGTH + 1],
      tfull[CROSS_LEN + 2];
  // dir_source and target are introduced for when we support multiple files
  // being renamed.
  char sargs[CROSS_LEN], targs[CROSS_LEN],
      dir_source[DOS_PATHLENGTH +
                 4] = {0},
                 dir_target[CROSS_LEN + 4] = {0},
                 target[CROSS_LEN + 4] = {
                     0}; // not sure if drive portion is included in pathlength
  ...
  char path[DOS_PATHLENGTH], spath[DOS_PATHLENGTH], pattern[DOS_PATHLENGTH],
      full[DOS_PATHLENGTH], *r;
  if (!DOS_Canonicalize(arg1, full))
    return;
  r = strrchr_dbcs(full, '\\');
  if (r != NULL) {
    *r = 0;
  ...
  if (!DOS_FindFirst(
          ((uselfn && pfull.length() && pfull[0] != '"' ? "\"" : "") + pfull +
           (uselfn && pfull.length() && pfull[pfull.length() - 1] != '"' ? "\""
                                                                         : ""))
              .c_str(),
          strchr(arg1, '*') != NULL || strchr(arg1, '?') != NULL
              ? 0xffff & ~DOS_ATTR_VOLUME & ~DOS_ATTR_DIRECTORY
              : 0xffff & ~DOS_ATTR_VOLUME)) {
    lfn_filefind_handle = fbak;
    ...
    do { /* File name and extension */
      ...
      if (!(attr & DOS_ATTR_DIRECTORY &&
        ...
        if (uselfn && strchr(arg2, '*') && !strchr(arg2, '.'))
          strcat(arg2, ".*");
        char *dot1 = strrchr(uselfn ? lname : name, '.'),
             *dot2 = strrchr(arg2, '.'), *star;
        if (dot2 == NULL) {
          star = strchr(arg2, '*');
          if (strchr(arg2, '?')) {
            for (unsigned int i = 0;
                 i < (uselfn ? LFN_NAMELENGTH : DOS_NAMELENGTH) &&
                 i < (star ? star - arg2 : strlen(arg2));
                 i++) {
          ...
          *dot2 = '.';
          ...
          if (strchr(tname2, '?')) {
            for (unsigned int i = 0;
                 i < (uselfn ? LFN_NAMELENGTH : DOS_NAMELENGTH) &&
                 i < (star ? star - tname2 : strlen(tname2));
                 i++) {
          ...
          if (dot1) {
            ...
            if (strchr(text2, '?')) {
              for (unsigned int i = 0;
                   i < (uselfn ? LFN_NAMELENGTH : DOS_NAMELENGTH) &&
                   i < (star ? star - text2 : strlen(text2));
                   i++) {
        ...
        sources.emplace_back(
            uselfn ? ((sargs[0] != '"' ? "\"" : "") + std::string(sargs) +
                      (sargs[strlen(sargs) - 1] != '"' ? "\"" : ""))
                         .c_str()
                   : sargs);
        sources.emplace_back(
            uselfn ? ((targs[0] != '"' ? "\"" : "") + std::string(targs) +
                      (targs[strlen(targs) - 1] != '"' ? "\"" : ""))
                         .c_str()
                   : targs);
        sources.emplace_back(
            strlen(sargs) > 2 && sargs[0] == '.' && sargs[1] == '\\' ? sargs + 2
                                                                     : sargs);
      }
      lfn_filefind_handle = uselfn ? LFN_FILEFIND_INTERNAL : LFN_FILEFIND_NONE;
    } while (DOS_FindNext());
    ...
    if (sources.empty())
      ...
      for (std::vector<std::string>::iterator source = sources.begin();
        ...
        source++;
        if (source == sources.end() || oname == NULL || nname == NULL)
          break;
...
void DOS_Shell::CMD_ECHO(char *args) {
  ...
  if (len && args[len - 1] == '\r') {
    LOG(LOG_MISC,
        LOG_WARN)("Hu ? carriage return already present. Is this possible?");
...
void DOS_Shell::CMD_PUSHD(char *args) {
  ...
  if (strlen(args) > 1 && args[1] == ':' && toupper(args[0]) >= 'A' &&
      toupper(args[0]) <= 'Z' && !Drives[toupper(args[0]) - 'A']) {
    WriteOut(MSG_Get("SHELL_ILLEGAL_DRIVE"));
    return;
  }
  ...
  if (*args) {
    ...
    if (strlen(args) > 1 && args[1] == ':')
      DOS_SetDefaultDrive(toupper(args[0]) - 'A');
...
void DOS_Shell::CMD_POPD(char *args) {
  ...
  if (olddrive >= 'A' && olddrive <= 'Z' && Drives[olddrive - 'A']) {
    uint8_t drive = DOS_GetDefaultDrive() + 'A';
    if (olddrive != DOS_GetDefaultDrive() + 'A')
      DOS_SetDefaultDrive(olddrive - 'A');
...
void DOS_Shell::CMD_CHDIR(char *args) {
  ...
  if (!*args) {
    ...
    if (!DOS_GetCurrentDir(targetdrive, dir,
                           true)) { // verify that this should be true
      if (drive == 'Z') {
    ...
    /* Changedir failed. Check if the filename is longer than 8 and/or contains
     * spaces */
...
static void FormatNumber(uint64_t num, char *buf) {
  ...
  numei = numo / 1000;
  if (numei) {
    sprintf(buf, "%u%c%03u%c%03u%c%03u%c%02u G", numei, dos.tables.country[7],
            numpi, dos.tables.country[7], numti, dos.tables.country[7], numgi,
            dos.tables.country[9], nummi);
    return;
  }
  if (numpi) {
    sprintf(buf, "%u%c%03u%c%03u%c%02u G", numpi, dos.tables.country[7], numti,
            dos.tables.country[7], numgi, dos.tables.country[9], nummi);
    return;
  }
  if (numti) {
    sprintf(buf, "%u%c%03u%c%02u G", numti, dos.tables.country[7], numgi,
            dos.tables.country[9], nummi);
    return;
  }
  if (numg) {
    if (numgi > 127)
      sprintf(buf, "%u%c%02u G", numgi, dos.tables.country[9], nummi);
    else
      sprintf(buf, "%u%c%03u%c%03u%c%03u", numg, dos.tables.country[7], numm,
              dos.tables.country[7], numk, dos.tables.country[7], numb);
    return;
  }
  if (numm) {
    sprintf(buf, "%u%c%03u%c%03u", numm, dos.tables.country[7], numk,
            dos.tables.country[7], numb);
    return;
  }
  if (numk) {
...
char *FormatDate(uint16_t year, uint8_t month, uint8_t day) {
  ...
  for (Bitu i = 0; i < 5; i++) {
    if (i == 1 || i == 3) {
      ...
      if (formatstring[i] == 'M')
        bufferptr += (Bitu)sprintf(buffer + bufferptr, "%02u", month);
      if (formatstring[i] == 'D')
        bufferptr += (Bitu)sprintf(buffer + bufferptr, "%02u", day);
      if (formatstring[i] == 'Y')
        bufferptr += (Bitu)sprintf(buffer + bufferptr, "%04u", year);
    }
  }
  return buffer;
}
...
char *FormatTime(Bitu hour, Bitu min, Bitu sec, Bitu msec) {
  ...
  if (sec >= 100 && msec >= 100)
    sprintf(retBuf, "%2u%c%02u%c", (unsigned int)hour, sep, (unsigned int)min,
            *ampm);
  else
    sprintf(retBuf, "%u%c%02u%c%02u%c%02u%s", (unsigned int)hour, sep,
            (unsigned int)min, sep, (unsigned int)sec, dos.tables.country[9],
...
static bool doDir(DOS_Shell *shell, char *args, DOS_DTA dta, char *numformat,
                  Bitu w_size, bool optW, bool optZ, bool optS, bool optP,
                  bool optB, bool optA, bool optAD, bool optAminusD, bool optAS,
                  bool optAminusS, bool optAH, bool optAminusH, bool optAR,
                  bool optAminusR, bool optAA, bool optAminusA, bool optOGN,
                  bool optOGD, bool optOGE, bool optOGS, bool optOG, bool optON,
                  bool optOD, bool optOE, bool optOS, bool reverseSort,
                  bool rev2Sort) {
  char path[DOS_PATHLENGTH];
  char sargs[CROSS_LEN], largs[CROSS_LEN], buffer[CROSS_LEN];
  unsigned int tcols =
      IS_PC98_ARCH ? 80 : real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
  if (!tcols)
    tcols = 80;
  ...
  *(strrchr_dbcs(path, '\\') + 1) = 0;
  ...
  if (!optB && !optS) {
    shell->WriteOut(
        MSG_Get("SHELL_CMD_DIR_INTRO"),
        uselfn && !optZ && DOS_GetSFNPath(path, largs, true) ? largs : sargs);
    if (optP) {
      p_count += optW ? 10 : 2;
  ...
  int fbak = lfn_filefind_handle;
  lfn_filefind_handle =
      uselfn && !optZ ? LFN_FILEFIND_INTERNAL : LFN_FILEFIND_NONE;
  ...
  if (!ret) {
    ...
    if (strlen(args) != len)
      ret = DOS_FindFirst(args, 0xffff & ~DOS_ATTR_VOLUME);
  }
  lfn_filefind_handle = fbak;
  if (ret) {
    ...
    do { /* File name and extension */
      DtaResult result;
      dta.GetResult(result.name, result.lname, result.size, result.hsize,
                    result.date, result.time, result.attr);

      /* Skip non-directories if option AD is present, or skip dirs in case of
       * A-D */
      if (optAD && !(result.attr & DOS_ATTR_DIRECTORY))
        continue;
      else if (optAminusD && (result.attr & DOS_ATTR_DIRECTORY))
        continue;
      else if (optAS && !(result.attr & DOS_ATTR_SYSTEM))
        continue;
      else if (optAminusS && (result.attr & DOS_ATTR_SYSTEM))
        continue;
      else if (optAH && !(result.attr & DOS_ATTR_HIDDEN))
        continue;
      else if (optAminusH && (result.attr & DOS_ATTR_HIDDEN))
        continue;
      else if (optAR && !(result.attr & DOS_ATTR_READ_ONLY))
        continue;
      else if (optAminusR && (result.attr & DOS_ATTR_READ_ONLY))
        continue;
      else if (optAA && !(result.attr & DOS_ATTR_ARCHIVE))
        continue;
      else if (optAminusA && (result.attr & DOS_ATTR_ARCHIVE))
        continue;
      else if (!(optA || optAD || optAminusD || optAS || optAminusS || optAH ||
                 optAminusH || optAR || optAminusR || optAA || optAminusA) &&
               (result.attr & DOS_ATTR_SYSTEM ||
                result.attr & DOS_ATTR_HIDDEN) &&
               strcmp(result.name, ".."))
        continue;
    ...
    if (optON) {
      ...
      std::sort(results.begin(), results.end(),
                oneRev ? DtaResult::groupRevDef : DtaResult::groupDef);
    } else if (optOGE) {
      // Directories first, then files, both sort by extension
      std::sort(results.begin(), results.end(),
                oneRev ? DtaResult::groupRevExt : DtaResult::groupExt);
    } else if (optOGS) {
      // Directories first, then files, both sort by size
      std::sort(results.begin(), results.end(),
                oneRev ? DtaResult::groupRevSize : DtaResult::groupSize);
    } else if (optOGD) {
      // Directories first, then files, both sort by date
      std::sort(results.begin(), results.end(),
                oneRev ? DtaResult::groupRevDate : DtaResult::groupDate);
    }
    if (reverseSort)
      std::reverse(results.begin(), results.end());

    for (std::vector<DtaResult>::iterator iter = results.begin();
      ...
      if (optB) {
        ...
        if (strcmp(".", uselfn && !optZ ? lname : name) &&
            strcmp("..", uselfn && !optZ ? lname : name)) {
          int m =
        ...
        if (first && optS) {
          first = false;
          shell->WriteOut("\n");
          shell->WriteOut(MSG_Get("SHELL_CMD_DIR_INTRO"),
                          uselfn && !optZ && DOS_GetSFNPath(path, largs, true)
                              ? largs
                              : sargs);
          if (optP) {
            p_count += optW ? 15 : 3;
        ...
        if (attr & DOS_ATTR_DIRECTORY) {
          if (optW) {
            ...
            shell->WriteOut("%-8s %-3s   %-16s %s %s", name, ext, "<DIR>",
                            FormatDate(year, month, day),
                            FormatTime(hour, minute, 100, 100));
            if (uselfn && !optZ) {
              shell->WriteOut(" ");
          ...
          if (optW) {
            ...
            shell->WriteOut("%-8s %-3s   %16s %s %s", name, ext, numformat,
                            FormatDate(year, month, day),
                            FormatTime(hour, minute, 100, 100));
            if (uselfn && !optZ) {
              shell->WriteOut(" ");
  ...
  if (!found && !optB && !optS) {
    shell->WriteOut(MSG_Get("SHELL_CMD_FILE_NOT_FOUND"), args);
    if (!dirPaused(shell, w_size, optP, optW, false))
      return false;
  }
  if (optS) {
    ...
    *(sargs + len) = 0;
    if (ret) {
      ...
      do { /* File name and extension */
        ...
        if (result.attr & DOS_ATTR_DIRECTORY && strcmp(result.name, ".") &&
            strcmp(result.name, "..")) {
    ...
    if (found && !optB) {
      ...
      shell->WriteOut(MSG_Get("SHELL_CMD_DIR_BYTES_USED"), cfile_count,
                      numformat);
      if (!dirPaused(shell, w_size, optP, optW, false))
        return false;
    }
...
void DOS_Shell::CMD_DIR(char *args) {
  ...
  b0 = ScanCMDBool(args, "O") || ScanCMDBool(args, "OGN") ||
       ScanCMDBool(args, "O:GN");
  b1 = ScanCMDBool(args, "O-GN") || ScanCMDBool(args, "O:-GN");
  b2 = ScanCMDBool(args, "O-G-N") || ScanCMDBool(args, "O:-G-N"),
  b3 = ScanCMDBool(args, "OG-N") || ScanCMDBool(args, "O:G-N");
  if (b0 || b1 || b2 || b3) {
    optOGN = true;
    reverseSort = b1 || b2;
    rev2Sort = b2 || b3;
  }
  b0 = ScanCMDBool(args, "OGD") || ScanCMDBool(args, "O:GD");
  b1 = ScanCMDBool(args, "O-GD") || ScanCMDBool(args, "O:-GD");
  b2 = ScanCMDBool(args, "O-G-D") || ScanCMDBool(args, "O:-G-D");
  b3 = ScanCMDBool(args, "OG-D") || ScanCMDBool(args, "O:G-D");
  if (b0 || b1 || b2 || b3) {
    optOGD = true;
    reverseSort = b1 || b2;
    rev2Sort = b2 || b3;
  }
  b0 = ScanCMDBool(args, "OGE") || ScanCMDBool(args, "O:GE");
  b1 = ScanCMDBool(args, "O-GE") || ScanCMDBool(args, "O:-GE");
  b2 = ScanCMDBool(args, "O-G-E") || ScanCMDBool(args, "O:-G-E");
  b3 = ScanCMDBool(args, "OG-E") || ScanCMDBool(args, "O:G-E");
  if (b0 || b1 || b2 || b3) {
    optOGE = true;
    reverseSort = b1 || b2;
    rev2Sort = b2 || b3;
  }
  b0 = ScanCMDBool(args, "OGS") || ScanCMDBool(args, "O:GS");
  b1 = ScanCMDBool(args, "O-GS") || ScanCMDBool(args, "O:-GS");
  b2 = ScanCMDBool(args, "O-G-S") || ScanCMDBool(args, "O:-G-S");
  b3 = ScanCMDBool(args, "OG-S") || ScanCMDBool(args, "O:G-S");
  if (b0 || b1 || b2 || b3) {
    optOGS = true;
    reverseSort = b1 || b2;
    rev2Sort = b2 || b3;
  }
  if (optOGN || optOGD || optOGE || optOGS)
    optOG = false;
  ...
  const char *valid[] = {
      "4",      "W",   "P",    "-W",   "-P",    "WP",   "PW",    "Z",
      "-Z",     "S",   "-S",   "B",    "-B",    "A",    "-A",    "AD",
      "A:D",    "A-D", "A:-D", "AS",   "A:S",   "A-S",  "A:-S",  "AH",
      "A:H",    "A-H", "A:-H", "AR",   "A:R",   "A-R",  "A:-R",  "AA",
      "A:A",    "A-A", "A:-A", "O",    "-O",    "ON",   "O:N",   "O-N",
      "O:-N",   "OD",  "O:D",  "O-D",  "O:-D",  "OE",   "O:E",   "O-E",
      "O:-E",   "OS",  "O:S",  "O-S",  "O:-S",  "OG",   "O:G",   "O-G",
      "O:-G",   "OGN", "O:GN", "O-GN", "O:-GN", "OG-N", "O:G-N", "O-G-N",
      "O:-G-N", "OGD", "O:GD", "O-GD", "O:-GD", "OG-D", "O:G-D", "O-G-D",
      "O:-G-D", "OGE", "O:GE", "O-GE", "O:-GE", "OG-E", "O:G-E", "O-G-E",
      "O:-G-E", "OGS", "O:GS", "O-GS", "O:-GS", "OG-S", "O:G-S", "O-G-S",
      "O:-G-S", NULL};
  if (args && strlen(args) > 1)
    for (int i = 0; valid[i] && *args && strchr(args, '/'); i++)
      while (ScanCMDBool(args, valid[i]))
        ;
  char *rem = ScanCMDRemain(args);
  if (rem) {
    WriteOut(MSG_Get("SHELL_ILLEGAL_SWITCH"), rem);
  ...
  if (!strrchr(args, '*') && !strrchr(args, '?')) {
    ...
    if (!DOS_GetSFNPath(args, sargs, false)) {
      if (strlen(args) > 1 && toupper(args[0]) >= 'A' &&
          toupper(args[0]) <= 'Z' && args[1] == ':' &&
          !Drives[toupper(args[0]) - 'A'])
        WriteOut(MSG_Get("SHELL_ILLEGAL_DRIVE"));
      else
        WriteOut(MSG_Get("SHELL_ILLEGAL_PATH"));
      return;
    }
    if (DOS_GetFileAttr(sargs, &attribute) &&
  ...
  *(strrchr_dbcs(path, '\\') + 1) = 0;
  ...
  while (!dirs.empty()) {
    ctrlbrk = false;
    if (!doDir(this, (char *)dirs.begin()->c_str(), dta, numformat, w_size,
               optW, optZ, optS, optP, optB, optA, optAD, optAminusD, optAS,
               optAminusS, optAH, optAminusH, optAR, optAminusR, optAA,
               optAminusA, optOGN, optOGD, optOGE, optOGS, optOG, optON, optOD,
               optOE, optOS, reverseSort, rev2Sort)) {
      dos.dta(save_dta);
      inshell = false;
      return;
    }
  ...
  if (!optB) {
    if (optS) {
      WriteOut("\n");
      if (!dirPaused(this, w_size, optP, optW)) {
        dos.dta(save_dta);
        return;
      }
    ...
    if (Drives[drive]) {
      uint32_t bytes_sector32;
      uint32_t sectors_cluster32;
      uint32_t total_clusters32;
      uint32_t free_clusters32;
      uint64_t total_clusters64;
      uint64_t free_clusters64;
      // Since this is the *internal* shell, we want use maximum available query
      // capability at first
      if (Drives[drive]->AllocationInfo64(&bytes_sector32, &sectors_cluster32,
                                          &total_clusters64,
                                          &free_clusters64)) {
        freec = 0;
        free_space = (uint64_t)bytes_sector32 * (Bitu)sectors_cluster32 *
                     (Bitu)free_clusters64;
      } else if ((dos.version.major > 7 ||
                  (dos.version.major == 7 && dos.version.minor >= 10)) &&
                 Drives[drive]->AllocationInfo32(
                     &bytes_sector32, &sectors_cluster32, &total_clusters32,
                     &free_clusters32)) { /* FAT32 aware extended API */
        freec = 0;
        free_space = (uint64_t)bytes_sector32 * (Bitu)sectors_cluster32 *
                     (Bitu)free_clusters32;
      } else {
        ...
        rsize = true;
        freec = 0;
        Drives[drive]->AllocationInfo(&bytes_sector, &sectors_cluster,
                                      &total_clusters, &free_clusters);
        free_space = (uint64_t)bytes_sector * (Bitu)sectors_cluster *
                     (Bitu)(freec ? freec : free_clusters);
        rsize = false;
      }
    }
#if defined(WIN32) &&                                                          \
...
void DOS_Shell::CMD_LS(char *args) {
  ...
  int fbak = lfn_filefind_handle;
  lfn_filefind_handle = uselfn ? LFN_FILEFIND_INTERNAL : LFN_FILEFIND_NONE;
  bool ret = DOS_FindFirst(
      ((uselfn ? "\"" : "") + std::string(spattern) + (uselfn ? "\"" : ""))
          .c_str(),
      0xffff & ~DOS_ATTR_VOLUME);
  if (!ret) {
    lfn_filefind_handle = fbak;
  ...
  unsigned int max[15], total,
      tcols = IS_PC98_ARCH ? 80 : real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
  if (!tcols)
    tcols = 80;
  ...
  for (col = mrow; col > 0; col--) {
    ...
    for (const auto &entry : results) {
      ...
      if (name == "." || name == "..")
        continue;
      if (!optA &&
          (entry.attr & DOS_ATTR_SYSTEM || entry.attr & DOS_ATTR_HIDDEN))
        continue;
      if (name.size() + 2 > max[w_count % col])
        max[w_count % col] = (unsigned int)(name.size() + 2);
      ++w_count;
    }
    total = 0;
  ...
  for (const auto &entry : results) {
    ...
    if (name == "." || name == "..")
      continue;
    if (!optA && (entry.attr & DOS_ATTR_SYSTEM || entry.attr & DOS_ATTR_HIDDEN))
      continue;
    if (entry.attr & DOS_ATTR_DIRECTORY) {
      ...
      if (col == 1) {
        ...
        WriteOut("\033[34;1m%s\033[0m%-*s", name.c_str(),
                 max[w_count % col] - name.size(), "");
    } else {
      if (!uselfn || optZ)
        lowcase(name);
      ...
      if (col == 1) {
        ...
        WriteOut(is_executable ? "\033[32;1m%-*s\033[0m" : "%-*s",
                 max[w_count % col], name.c_str());
    }
    if (col > 1) {
      ++w_count;
      if (w_count % col == 0) {
...
void DOS_Shell::CMD_COPY(char *args) {
  ...
  if (GetEnvStr("COPYCMD", line)) {
    ...
    if (ScanCMDBool(copycmd, "Y") && !ScanCMDBool(copycmd, "-Y"))
      optY = true;
  }
  if (ScanCMDBool(args, "-Y"))
    optY = false;
  ScanCMDBool(args, "V");
  ...
  // Gather all sources (extension to copy more than 1 file specified at command
  // line) Concatenating files go as follows: All parts except for the last bear
  // the concat flag. This construction allows them to be counted (only the non
  // concat set)
  char q[] = "\"";
  char *source_p = NULL;
      ...
      if (source_x_len > 0) {
        if (source_x[source_x_len - 1] == ':')
          ...
          if (find_last == NULL)
            ...
            find_last++;
          if (strlen(find_last) > 0 && source_x[source_x_len - 1] == '*' &&
              strchr(find_last, '.') == NULL)
            strcat(source_x, ".*");
        }
      }
      if (!has_drive_spec &&
          !strpbrk(source_p, "*?")) { // doubt that fu*\*.* is valid
  ...
  while (sources.size()) {
    ...
    if (!DOS_Canonicalize(const_cast<char *>(source.filename.c_str()),
                          pathSourcePre)) {
      WriteOut(MSG_Get("SHELL_ILLEGAL_PATH"));
      dos.dta(save_dta);
      return;
    ...
    if (uselfn)
      sprintf(pathSource, "\"%s\"", pathSourcePre);
    // cut search pattern
    char *pos = strrchr_dbcs(pathSource, '\\');
    if (pos)
      *(pos + 1) = 0;

    if (!DOS_Canonicalize(const_cast<char *>(target.filename.c_str()),
                          pathTarget)) {
      WriteOut(MSG_Get("SHELL_ILLEGAL_PATH"));
      dos.dta(save_dta);
      return;
    ...
    if (temp && (temp == pathTarget || temp[-1] == '\\'))
    ...
    char sPath[DOS_PATHLENGTH];
    bool ret =
        DOS_GetSFNPath(source.filename.c_str(), sPath, false) &&
        DOS_FindFirst(
            ((strchr(sPath, ' ') && sPath[0] != '"' ? "\"" : "") +
             std::string(sPath) +
             (strchr(sPath, ' ') && sPath[strlen(sPath) - 1] != '"' ? "\""
                                                                    : ""))
                .c_str(),
            0xffff & ~DOS_ATTR_VOLUME);
    if (!ret) {
            ...
            for (unsigned int i = 0; i < source.filename.size(); i++) {
              if (lead)
                lead = false;
                        ...
                        shiftjis_lead_byte(source.filename[i])) ||
                       (isDBCSCP() && isKanji1(source.filename[i])))
                lead = true;
              else if (source.filename[i] == '\\')
                lastSlash = i;
            }
    ...
    while (ret) {
      ...
      if ((attr & DOS_ATTR_DIRECTORY) == 0) {
        ...
        if (DOS_OpenFile(nameSource, 0, &sourceHandle)) {
          ...
          if (!ftdvalid)
            LOG_MSG("WARNING: COPY cannot obtain file date/time");
          ...
          if (nameTarget[strlen(nameTarget) - 1] == '\\')
            strcat(nameTarget, uselfn ? lname : name);
          strcat(nameTarget, q);

          // Special variable to ensure that copy * a_file, where a_file is not
          // a directory concats.
          bool special = second_file_of_current_source && target_is_file &&
                         strchr(target.filename.c_str(), '*') == NULL;
          ...
          if (*nameSource && *nameTarget) {
            ...
            strcat(nametmp, nameSource[strlen(nameSource) - 1] != '\"' &&
                                    nameTarget[strlen(nameTarget) - 1] == '\"'
                                ? "\""
                                : "");
          } else
            strcpy(nametmp, nameSource);
          if (!oldsource.concat && (!strcasecmp(nameSource, nameTarget) ||
                                    !strcasecmp(nametmp, nameTarget))) {
          ...
          if (!(attr & DOS_ATTR_DIRECTORY) &&
              DOS_FindDevice(nameTarget) == DOS_DEVICES) {
            ...
            if (!exist && size) {
              int drive =
                  strlen(nameTarget) > 1 &&
                          (nameTarget[1] == ':' || nameTarget[2] == ':')
                      ? (toupper(nameTarget[nameTarget[0] == '"' ? 1 : 0]) -
                         'A')
                      : -1;
              if (drive >= 0 && Drives[drive]) {
                ...
                if (dos.version.major > 7 ||
                    (dos.version.major == 7 && dos.version.minor >= 10)) {
                  Drives[drive]->AllocationInfo32(&bytes32, &sectors32,
                                                  &clusters32, &free32);
                  no_free_space = (uint64_t)bytes32 * (uint64_t)sectors32 *
                                              (uint64_t)free32 <
                                          size
                                      ? true
                                      : false;
                  // LOG_MSG("drive=%u, no_free_space = %d bytes32=%u,
                  // sectors32=%u, free32 =%u, free_space=%u, size=%u",
                  //   drive, no_free_space ? 1 : 0, bytes32, sectors32, free32,
                  //   bytes32*sectors32*free32, size);
                }
                if (bytes32 == 0 || sectors32 == 0 || dos.version.major < 7 ||
                    (dos.version.major == 7 && dos.version.minor < 10)) {
                  Drives[drive]->AllocationInfo(&bytes_sector, &sectors_cluster,
                                                &total_clusters,
                                                &free_clusters);
                  no_free_space =
                      (Bitu)bytes_sector * (Bitu)sectors_cluster *
                                  (Bitu)(freec ? freec : free_clusters) <
                              size
                          ? true
                          : false;
                  // LOG_MSG("no_free_space = %d bytes=%u, sectors=%u, free =%u,
                  // free_space=%u, size=%u",
                  //  no_free_space ? 1 : 0, bytes_sector, sectors_cluster,
                  //  freec, bytes_sector*sectors_cluster*free_clusters, size);
                }
                rsize = false;
                if (no_free_space) {
                  WriteOut(MSG_Get("SHELL_CMD_COPY_NOSPACE"),
                           uselfn ? lname : name);
                  DOS_CloseFile(sourceHandle);
                  ret = DOS_FindNext();
                  continue;
          ...
          if (oldsource.concat ||
            ...
            if (!oldsource.concat ||
                (DOS_OpenFile(nameTarget, OPEN_READWRITE, &targetHandle) &&
                 DOS_SeekFile(targetHandle, &dummy, DOS_SEEK_END))) {
              // Copy
              static uint8_t
                  buffer[0x8000]; // static, otherwise stack overflow possible.
              bool failed = false;
              ...
              bool cont;
              do {
                if (!DOS_ReadFile(sourceHandle, buffer, &toread))
                  failed = true;
                if (iscon) {
                  if (dos.errorcode == 77) {
                    dos.dta(save_dta);
                  ...
                  if (!DOS_WriteFile(targetHandle, buffer, &toread))
                    failed = true;
                  if (cont)
                    toread = 0x8000;
                } else {
                  if (DOS_FindDevice(nameTarget) == DOS_FindDevice("con") &&
                      CheckBreak(this)) {
                    failed = true;
                    break;
                  }
              ...
              if (!DOS_CloseFile(sourceHandle))
                failed = true;
#if defined(WIN32)
              if (file_access_tries > 0 && DOS_FindDevice(name) == DOS_DEVICES)
                DOS_SetFileDate(targetHandle, ftime, fdate);
#endif
              if (!DOS_CloseFile(targetHandle))
                failed = true;
              if (failed)
                WriteOut(MSG_Get("SHELL_CMD_COPY_ERROR"),
                         uselfn ? lname : name);
              else if (strcmp(name, lname) && uselfn)
                WriteOut(" %s [%s]\n", lname, name);
              else
                WriteOut(" %s\n", uselfn ? lname : name);
              if (!source.concat && !special && !failed)
                count++; // Only count concat files once
            } else {
              DOS_CloseFile(sourceHandle);
              WriteOut(MSG_Get("SHELL_CMD_COPY_FAILURE"),
                       const_cast<char *>(target.filename.c_str()));
            }
          } else {
            DOS_CloseFile(sourceHandle);
            WriteOut(MSG_Get("SHELL_CMD_COPY_FAILURE"),
          ...
          WriteOut(MSG_Get("SHELL_CMD_COPY_FAILURE"),
                   const_cast<char *>(source.filename.c_str()));
      }
      // On to the next file if the previous one wasn't a device
      if ((attr & DOS_ATTR_DEVICE) == 0)
        ret = DOS_FindNext();
      else
        ret = false;
    }
...
void DOS_Shell::CMD_SET(char *args) {
  ...
  enum op_mode_t { show_all_env, set_env, show_env, erase_env, first_env };

  op_mode_t op_mode = show_all_env;
  std::string env_name, env_value;

  const bool zdirpath = static_cast<Section_prop *>(control->GetSection("dos"))
                            ->Get_bool("drive z expand path");

  {
    ...
    while (*args == '/') {
      ...
      if (sw == "P") {
        WriteOut(
            "Set /P is not supported. Use Choice!"); /* TODO: What is SET /P
                                                        supposed to do? */
        return;
      } else if (sw == "ERASE") { /* DOSBox-X extension: Completely erase the
                                     environment block */
        op_mode = erase_env;
      } else if (sw ==
                 "FIRST") { /* DOSBox-X extension: Move the specified variable
                               to the front of the environment block */
        op_mode = first_env;
      } else {
  ...
  if (op_mode == first_env) {
    ...
    if (*args != 0) {
      ...
      if (p == NULL) {
        ...
        /* ASCIIZ snip the args string in two, so that args is C-string name of
         * the variable, and "p" is C-string value of the variable */
        op_mode = set_env;
        *p++ = 0;
        env_name = args;
        env_value = p;
  ...
  case show_env:
    ...
    break;
  case set_env:
    if (zdirpath && !strcasecmp(env_name.c_str(), "path"))
      GetExpandedPath(env_value);

    /* No parsing is needed. The command interpreter does the variable
     * substitution for us */
    /* NTS: If Win95 is any example, the command interpreter expands the
     * variables for us */
...
void DOS_Shell::CMD_IF(char *args) {
  ...
  if (strncasecmp(args, "EXIST ", 6) == 0) {
    ...
    { /* DOS_FindFirst uses dta so set it to our internal dta */
      char spath[DOS_PATHLENGTH], path[DOS_PATHLENGTH], pattern[DOS_PATHLENGTH],
          full[DOS_PATHLENGTH], *r;
      ...
      bool ret = DOS_FindFirst(
          ((uselfn && sfull.length() && sfull[0] != '"' ? "\"" : "") + sfull +
           (uselfn && sfull.length() && sfull[sfull.length() - 1] != '"' ? "\""
                                                                         : ""))
              .c_str(),
          0xffff & ~(DOS_ATTR_VOLUME | DOS_ATTR_DIRECTORY));
      lfn_filefind_handle = fbak;
...
void DOS_Shell::CMD_REM(char *args) { HELP("REM"); }
...
void DOS_Shell::CMD_MORE(char *args) {
  ...
  if (IS_PC98_ARCH) {
    ...
    COLS = 80;
    if (real_readb(0x60, 0x111))
      LINES--; // Function keys displayed
  } else {
    LINES =
        (IS_EGAVGA_ARCH ? real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) : 24) + 1;
    COLS = real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
  }
  LINES--;
...
void DOS_Shell::CMD_DATE(char *args) {
  ...
  int n =
      dos.tables.country[0] == 1
          ? sscanf(args, "%u%c%u%c%u", &newday, &c1, &newmonth, &c2, &newyear)
          : (dos.tables.country[0] == 2 ? sscanf(args, "%u%c%u%c%u", &newyear,
                                                 &c1, &newmonth, &c2, &newday)
                                        : sscanf(args, "%u%c%u%c%u", &newmonth,
                                                 &c1, &newday, &c2, &newyear));
  if (n == 5 && c1 == c && c2 == c) {
  ...
  uint32_t length;
  char day[6] = {0};
  if (sscanf(datestring, "%u", &length) && (length < 7) &&
      (strlen(datestring) == ((size_t)length * 7 + 1))) {
  ...
  if (!dateonly) {
    ...
    sprintf(
        ...
        dos.tables.country[0] == 1
            ? "DD%cMM%cYYYY"
            : (dos.tables.country[0] == 2 ? "YYYY%cMM%cDD" : "MM%cDD%cYYYY"),
        c, c);
...
void DOS_Shell::CMD_TIME(char *args) {
  ...
  if (ScanCMDBool(args, "H")) {
    ...
    uint32_t ticks =
        (uint32_t)(((double)(loctime->tm_hour * 3600 + loctime->tm_min * 60 +
                             loctime->tm_sec)) *
                   18.206481481);
  ...
  if (sscanf(args, "%u%c%u%c%u", &newhour, &c1, &newminute, &c2, &newsecond) ==
          5 &&
      c1 == c && c2 == c) {
  ...
  */
  if (timeonly) {
    WriteOut("%u%c%02u%c%02u\n", reg_ch, dos.tables.country[13], reg_cl,
             dos.tables.country[13], reg_dh);
  } else {
...
void DOS_Shell::CMD_SUBST(char *args) {
  ...
  try {
    ...
    if (!command.GetCount()) {
      ...
      WriteOut(MSG_Get("PROGRAM_MOUNT_STATUS_FORMAT"), "Drive", "Type",
               "Label");
      ...
      for (int d = 0; d < DOS_DRIVES; d++) {
        if (!Drives[d] || strncmp(Drives[d]->GetInfo(), "local ", 6))
          continue;
    ...
    if (Drives[temp_str[0] - 'A'])
      throw 2; // targetdrive in use
    strcat(mountstring, temp_str);
    strcat(mountstring, " ");

    uint8_t drive;
    char dir[DOS_PATHLENGTH + 2], fulldir[DOS_PATHLENGTH];
    if (strchr(arg.c_str(), '\"') == NULL)
      sprintf(dir, "\"%s\"", arg.c_str());
    else
      strcpy(dir, arg.c_str());
...
bool get_param(char *&args, char *&rem, char *&temp, char &wait_char,
               int &wait_sec) {
...
void DOS_Shell::CMD_CHOICE(char *args) {
  HELP("CHOICE");
  static char defchoice[3] = {MSG_Get("INT21_6523_YESNO_CHARS")[0],
                              MSG_Get("INT21_6523_YESNO_CHARS")[1], 0};
  ...
  if (!rem || !*rem)
    rem = defchoice; /* No choices specified use (national) YN */
  ptr = rem;
  uint8_t c;
  if (!optS)
    while ((c = (uint8_t)(*ptr)))
      *ptr++ = (char)toupper(
          c); /* When in no case-sensitive mode. make everything upcase */
  if (args && *args) {
    StripSpaces(args);
  ...
  // TO-DO: Find out how real DOS handles /T option for making a choice after
  // delay; use AUTOTYPE for now
  std::vector<std::string> sequence;
  bool in_char =
      optS ? (strchr(rem, waitchar) != nullptr)
           : (strchr(rem, toupper(waitchar)) || strchr(rem, tolower(waitchar)));
...
static bool doAttrib(DOS_Shell *shell, char *args, DOS_DTA dta, bool optS,
                     bool adda, bool adds, bool addh, bool addr, bool suba,
                     bool subs, bool subh, bool subr) {
  char spath[DOS_PATHLENGTH], sargs[DOS_PATHLENGTH + 4],
      path[DOS_PATHLENGTH + 4], full[DOS_PATHLENGTH], sfull[DOS_PATHLENGTH + 2];
  if (!DOS_Canonicalize(args, full) || strrchr_dbcs(full, '\\') == NULL) {
    shell->WriteOut(MSG_Get("SHELL_ILLEGAL_PATH"));
  ...
  *lend = 0;
  ...
  while (res) {
    ...
    dta.GetResult(name, lname, size, hsize, date, time, attr);
    if (!((!strcmp(name, ".") || !strcmp(name, "..") ||
           strchr(sargs, '*') != NULL || strchr(sargs, '?') != NULL) &&
          attr & DOS_ATTR_DIRECTORY)) {
      found = true;
      strcpy(end, name);
      strcpy(lend, lname);
      if (strlen(full) &&
          DOS_GetFileAttr(
              ((uselfn || strchr(full, ' ') ? (full[0] != '"' ? "\"" : "")
               ...
               (uselfn || strchr(full, ' ')
                  ...
                  .c_str(),
              &fattr)) {
        bool attra = fattr & DOS_ATTR_ARCHIVE, attrs = fattr & DOS_ATTR_SYSTEM,
        ...
        if (adda || adds || addh || addr || suba || subs || subh || subr) {
            ...
            if (DOS_GetFileAttr(
                    ((uselfn || strchr(full, ' ') ? (full[0] != '"' ? "\"" : "")
                     ...
                     (uselfn || strchr(full, ' ')
                        ...
                        .c_str(),
                    &fattr)) {
              shell->WriteOut("  %c  %c%c%c	",
                              fattr & DOS_ATTR_ARCHIVE ? 'A' : ' ',
                              fattr & DOS_ATTR_SYSTEM ? 'S' : ' ',
                              fattr & DOS_ATTR_HIDDEN ? 'H' : ' ',
                              fattr & DOS_ATTR_READ_ONLY ? 'R' : ' ');
          ...
          shell->WriteOut("  %c  %c%c%c	", attra ? 'A' : ' ', attrs ? 'S' : ' ',
                          attrh ? 'H' : ' ', attrr ? 'R' : ' ');
          shell->WriteOut_NoParsing(uselfn ? sfull : full, true);
          shell->WriteOut("\n");
        }
...
void DOS_Shell::CMD_ATTRIB(char *args) {
  ...
  bool adda = false, adds = false, addh = false, addr = false, suba = false,
       subs = false, subh = false, subr = false;
    ...
    if (!strcasecmp(arg1, "+A"))
      ...
      if (uselfn && strchr(sfull, '*')) {
        ...
        if (find_last == NULL)
          ...
          find_last++;
        if (sfull[strlen(sfull) - 1] == '*' && strchr(find_last, '.') == NULL)
          strcat(sfull, ".*");
      }
    }
  } while (*args);
  ...
  while (!adirs.empty()) {
    ctrlbrk = false;
    if (doAttrib(this, (char *)adirs.begin()->c_str(), dta, optS, adda, adds,
                 addh, addr, suba, subs, subh, subr))
      found = true;
    else if (ctrlbrk)
      break;
...
void DOS_Shell::CMD_PATH(char *args) {
  ...
  if (args && *args) {
    ...
    if (args) {
      ...
      bool zdirpath = static_cast<Section_prop *>(control->GetSection("dos"))
                          ->Get_bool("drive z expand path");
      if (zdirpath)
        GetExpandedPath(vstr);
...
void DOS_Shell::CMD_VER(char *args) {
  ...
  if (!optR && args && *args) {
    ...
    WriteOut(MSG_Get("SHELL_CMD_VER_VER"), VERSION, SDL_STRING,
             dos.version.major, dos.version.minor);
    if (optR)
      WriteOut("DOSBox-X Git commit %s, built on %s\nPlatform: %s %d-bit",
               GIT_COMMIT_HASH, UPDATED_STR, OS_PLATFORM_LONG, OS_BIT_INT);
  }
}
...
void DOS_Shell::CMD_VOL(char *args) {
  ...
  WriteOut(tree ? MSG_Get("SHELL_CMD_VOL_SERIAL") + 1
                : MSG_Get("SHELL_CMD_VOL_SERIAL"));
  ...
  while (*args) {
    ...
    if (!word[1]) {
      ...
      const int map[256] = {
          ...
          KBD_8,
          KBD_9,
          KBD_semicolon | shiftflag,
          KBD_semicolon,
          KBD_comma | shiftflag,
          KBD_equals,
          KBD_period | shiftflag,
          KBD_slash | shiftflag,
          KBD_2 | shiftflag,
          KBD_a | shiftflag,
          KBD_b | shiftflag,
          ...
          KBD_y,
          KBD_z,
          KBD_leftbracket | shiftflag,
          KBD_backslash | shiftflag,
          KBD_rightbracket | shiftflag,
          KBD_grave | shiftflag,
          0,
          0,
          KBD_f1,
      ...
      if (map[(unsigned char)word[0]] & shiftflag)
        shift = true;
      if (!scankey && core == 0) {
    ...
    if (word[0] == 'p') {
      ...
      if (tail && *tail) {
        ...
        return;
               ...
               ((scankey = (KBD_KEYS)strtol(word, NULL, 0)) > KBD_NONE &&
                scankey < KBD_LAST)) {
      if (shift) {
        if (delay == 0)
          KEYBOARD_AddKey(KBD_leftshift, true);
        else
          PIC_AddEvent(&delayed_press, delay++, KBD_leftshift);
      }
      if (ctrl) {
        if (delay == 0)
          KEYBOARD_AddKey(KBD_leftctrl, true);
        else
          PIC_AddEvent(&delayed_press, delay++, KBD_leftctrl);
      }
      if (alt) {
        if (delay == 0)
          KEYBOARD_AddKey(KBD_leftalt, true);
        else
          PIC_AddEvent(&delayed_press, delay++, KBD_leftalt);
      }
      if (delay == 0)
        KEYBOARD_AddKey(scankey, true);
      else
        PIC_AddEvent(&delayed_press, delay++, scankey);

      if (delay + duration == 0)
        KEYBOARD_AddKey(scankey, false);
      else
        PIC_AddEvent(&delayed_release, delay++ + duration, scankey);
      if (alt) {
        if (delay + duration == 0)
          KEYBOARD_AddKey(KBD_leftalt, false);
        else
          PIC_AddEvent(&delayed_release, delay++ + duration, KBD_leftalt);
      }
      if (ctrl) {
        if (delay + duration == 0)
          KEYBOARD_AddKey(KBD_leftctrl, false);
        else
          PIC_AddEvent(&delayed_release, delay++ + duration, KBD_leftctrl);
      }
      if (shift) {
        if (delay + duration == 0)
          KEYBOARD_AddKey(KBD_leftshift, false);
        else
          PIC_AddEvent(&delayed_release, delay++ + duration, KBD_leftshift);
      }
    } else if (core != 0) {
      if (core == 1) {
        if (delay == 0)
          ...
          PIC_AddEvent(&delayed_sdlpress, delay++, 1);
      } else if (core == 2) {
        if (delay == 0)
          SetVal("cpu", "core", "simple");
        else
          PIC_AddEvent(&delayed_sdlpress, delay++, 2);
      } else if (core == 3) {
        if (delay == 0)
          SetVal("cpu", "core", "dynamic");
        else
          PIC_AddEvent(&delayed_sdlpress, delay++, 3);
      } else if (core == 4) {
        if (delay == 0)
          SetVal("cpu", "core", "full");
        else
          PIC_AddEvent(&delayed_sdlpress, delay++, 4);
      }
    } else {
      WriteOut(MSG_Get("SHELL_SYNTAXERROR"), word);
      return;
...
void DOS_Shell::CMD_DEBUGBOX(char *args) {
  ...
  args = StripArg(args);
  HELP("DEBUGBOX");
  /* TODO: The command as originally taken from DOSBox SVN supported a /NOMOUSE
   * option to remove the INT 33h vector */
  if (!*args) {
    tohide = false;
...
char *str_replace(const char *orig, const char *rep, const char *with) {
  ...
  char *original_mutable_orig =
      mutable_orig; // Store the original address for freeing below
  ...
  tmp = result =
      (char *)malloc(strlen(mutable_orig) + (len_with - len_rep) * count + 1);
  if (!result)
    return NULL;
...
void DOS_Shell::CMD_FOR(char *args) {
  ...
  strcpy(s, "%%");
  if (*args == '%' &&
      (isalpha(args[1]) || isdigit(args[1]) || strchr("_-/*.;#$", args[1])) &&
      isspace(args[2]))
    s[1] = *(args + 1);
  else {
    SyntaxError();
  ...
  args = ltrim(args + 3);
  if (strncasecmp(args, "IN", 2) || !isspace(args[2])) {
    SyntaxError();
    return;
  }
  args = ltrim(args + 3);
  ...
  *p = ')';
  args = ltrim(p + 2);
  if (strncasecmp(args, "DO", 2) || !isspace(args[2])) {
    SyntaxError();
    return;
  }
  args = ltrim(args + 3);
  ...
  while (*fp) {
    ...
    if (strchr(fp, '?') || strchr(fp, '*')) {
      char name[DOS_NAMELENGTH_ASCII], lname[LFN_NAMELENGTH],
          spath[DOS_PATHLENGTH], path[DOS_PATHLENGTH], pattern[DOS_PATHLENGTH],
          full[DOS_PATHLENGTH], *r;
      if (!DOS_Canonicalize(fp, full))
        return;
      r = strrchr_dbcs(full, '\\');
      if (r != NULL) {
        *r = 0;
      ...
      if (DOS_FindFirst((std::string(spath) + std::string(pattern)).c_str(),
                        ~(DOS_ATTR_VOLUME | DOS_ATTR_DIRECTORY |
                          DOS_ATTR_DEVICE | DOS_ATTR_HIDDEN |
                          DOS_ATTR_SYSTEM))) {
        dta.GetResult(name, lname, size, hsize, date, time, attr);
...
void DOS_Shell::CMD_ALIAS(char *args) {
  ...
  if (!*args || strchr(args, '=') == NULL) {
    for (cmd_alias_map_t::iterator iter = cmd_alias.begin(),
                                   end = cmd_alias.end();
         iter != end; ++iter) {
    ...
    for (unsigned int offset = 0; *args && offset < sizeof(alias_name) - 1;
         ++offset, ++args) {
      if (*args == '=') {
...
void DOS_Shell::CMD_ASSOC(char *args) {
  ...
  args = trim(args);
  if (!*args || strchr(args, '=') == NULL) {
    for (cmd_assoc_map_t::iterator iter = cmd_assoc.begin(),
                                   end = cmd_assoc.end();
         iter != end; ++iter) {
      if (!*args || !strcasecmp(args, iter->first.c_str()))
        WriteOut("%s=%s\n", iter->first.c_str(), iter->second.c_str());
    ...
    for (unsigned int offset = 0; *args && offset < sizeof(assoc_name) - 1;
         ++offset, ++args) {
      if (*args == '=') {
...
//              on purpose to avoid conflicts with any existing DOS
//              applications.
void DOS_Shell::CMD_DXCAPTURE(char *args) {
  ...
  if (post_exit_delay_ms > 0) {
    LOG_MSG("Pausing for post exit delay (%.3f seconds)",
            (double)post_exit_delay_ms / 1000);
...
extern Bitu DOS_LoadKeyboardLayout(const char *layoutname, int32_t codepage,
                                   const char *codepagefile);
void runRescan(const char *str), MSG_Init(), JFONT_Init(), InitFontHandle(),
    ShutFontHandle(), initcodepagefont(), DOSBox_SetSysMenu();
int toSetCodePage(DOS_Shell *shell, int newCP, int opt) {
  if ((TTF_using() && isSupportedCP(newCP)) || !TTF_using()) {
    ...
    if (newCP != cpbak)
      LOG_MSG("SHELL: Invalid codepage %d, set to %d.", cpbak, newCP);
    ...
    if (isDBCSCP()) {
      ...
      JFONT_Init();
      SetupDBCSTable();
      if (newCP == 950 && !chinasea)
        makestdcp950table();
      if (newCP == 951 && chinasea)
        makeseacp951table();
    }
    if (finish_prepare)
      runRescan("-A -Q");
    ...
    shell->WriteOut(MSG_Get("SHELL_CMD_CHCP_INVALID"),
                    std::to_string(newCP).c_str());
  }
  return -1;
}
...
void DOS_Shell::CMD_CHCP(char *args) {
  ...
  if (IS_PC98_ARCH || IS_JEGA_ARCH) {
    WriteOut("Changing code page is not supported for the PC-98 or JEGA/AX "
             "system.\n");
    return;
  }
  if (IS_DOSV || IS_J3100) {
    WriteOut("Changing code page is not supported for the DOS/V or J-3100 "
             "system.\n");
    return;
  }
  int32_t newCP;
  ...
  if (n == 1) {
    if (!TTF_using() || (TTF_using() && isSupportedCP(newCP))) {
      ...
      if (missing > 0)
        WriteOut(MSG_Get("SHELL_CMD_CHCP_MISSING"), missing);
      else if (missing < 0) {
    ...
    if (*buff == ':' && strchr(StripArg(args), ':')) {
      ...
      if (name.empty() && iter != langcp_map.end())
        name = iter->second;
      if (!TTF_using() || (TTF_using() && isSupportedCP(newCP))) {
        CHCP_changed = true;
...
void DOS_Shell::CMD_VTEXT(char *args) {
  ...
  WriteOut(
      MSG_Get(mode == 0x70 ? "SHELL_CMD_VTEXT_ON" : "SHELL_CMD_VTEXT_OFF"));
}
