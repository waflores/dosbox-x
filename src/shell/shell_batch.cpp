...
BatchFile::BatchFile(DOS_Shell *host, char const *const resolved_name,
  ...
  DOS_Canonicalize(resolved_name,
                   totalname); // Get fullname including drive specification
...
bool BatchFile::ReadLine(char *line) {
  ...
  if (!DOS_OpenFile(filename.c_str(), (DOS_NOT_INHERIT | OPEN_READ),
                    &file_handle)) {
    LOG(LOG_MISC, LOG_ERROR)("ReadLine Can't open BatchFile %s",
                             filename.c_str());
    delete this;
    return false;
  }
  ...
  do {
    ...
    if (n > 0) {
      ...
      if (c > 31 || c == 0x1b || c == '\t' || c == 7 || c == 8) {
        // Only add it if room for it (and trailing zero) in the buffer, but do
        // the check here instead at the end So we continue reading till EOL/EOF
        ...
        LOG(LOG_MISC, LOG_DEBUG)("Encountered non-standard control character "
                                 "in batch file: Dec %03u and Hex %#04x.\n",
                                 c, c);
    }
  } while (c != '\n' && n);
  *cmd_write = 0;
    ...
    if (*cmd_read == '%') {
      ...
      if (next > '0' && next <= '9') {
        ...
        if (cmd->GetCount() < (unsigned int)next)
          continue;
        std::string word;
        if (!cmd->FindCommand((unsigned int)next, word))
          continue;
        ...
        if (!first) {
          /* *cmd_write++ = '%';*/
          // check if input contains cycles + max/auto  and that next character
          // is space or empty If so, don't ignore it. This way cycles can still
          // be set from within batch files
          char peak = *(cmd_read);
...
bool BatchFile::Goto(const char *where) {
  ...
  if (!DOS_OpenFile(filename.c_str(), 128, &file_handle)) {
    LOG(LOG_MISC, LOG_ERROR)("SHELL:Goto Can't open BatchFile %s",
                             filename.c_str());
    delete this;
    return false;
  }
...
again:
  ...
  *cmd_write++ = 0;
  ...
  if (nospace[0] == ':') {
    ...
    while (*nospace && (isspace(*reinterpret_cast<unsigned char *>(nospace)) ||
                        (*nospace == '=')))
      nospace++;
...
void BatchFile::Shift(void) { cmd->Shift(1); }
