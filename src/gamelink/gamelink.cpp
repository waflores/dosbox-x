...
static void shared_memory_init() {
  ...
  memset(g_p_shared_memory->peek.addr, 0,
         GameLink::sSharedMMapPeek_R2::PEEK_LIMIT * sizeof(uint32_t));
  memset(g_p_shared_memory->peek.data, 0,
         GameLink::sSharedMMapPeek_R2::PEEK_LIMIT * sizeof(uint8_t));
  ...
  memset(g_p_shared_memory->frame.buffer, 0,
         GameLink::sSharedMMapFrame_R1::MAX_PAYLOAD);
...
//
// \returns 1 if we made one, 0 if it failed or the mutex existed already (also
// a failure).
//
static int create_mutex(const char *p_name) {
  ...
  if (g_mutex_handle != 0) {
    LOG_MSG("GAMELINK: WARNING: MUTEX \"%s\" already exists, assuming it is "
  ...
  if (g_mutex_handle == SEM_FAILED) {
    LOG_MSG("GAMELINK: Creating MUTEX \"%s\" failed. [sem_open=%d, errno=%d]",
            p_name, (int)(size_t)g_mutex_handle, errno);
#ifdef MACOSX
    LOG_MSG("GAMELINK: Might need to manually reboot the system.");
#else  // MACOSX
    LOG_MSG("GAMELINK: Might need to manually tidy up in /dev/shm (or reboot "
            "system).");
#endif // MACOSX
...
static int create_shared_memory() {
  ...
  g_mmap_handle = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE,
                                     0, memory_map_size, GAMELINK_MMAP_NAME);

  if (g_mmap_handle) {
    g_p_shared_memory =
        reinterpret_cast<GameLink::sSharedMemoryMap_R4 *>(MapViewOfFile(
            g_mmap_handle, FILE_MAP_ALL_ACCESS, 0, 0, memory_map_size));
  ...
  g_mmap_handle = shm_open(GAMELINK_MMAP_NAME,
                           O_CREAT
#ifndef MACOSX
                               | O_TRUNC
#endif // !MACOSX
  ...
  if (g_mmap_handle < 0) {
    LOG_MSG("GAMELINK: shm_open( \"" GAMELINK_MMAP_NAME
            "\" ) failed. errno = %d",
            errno);
  } else {
    // set size
    ...
    g_p_shared_memory = reinterpret_cast<GameLink::sSharedMemoryMap_R4 *>(
        mmap(nullptr, memory_map_size, PROT_READ | PROT_WRITE, MAP_SHARED,
             g_mmap_handle, 0));
...
uint8_t *GameLink::AllocRAM(const uint32_t size) {
  ...
  const int memory_map_size = MEMORY_MAP_CORE_SIZE + g_membase_size;
  LOG_MSG("GAMELINK: Initialised. Allocated %d MB of shared memory.",
  ...
  if (!sdl.gamelink.snoop && g_p_shared_memory) {
    if (g_trackonly_mode) {
      ...
      if (g_p_shared_memory->input.ready) {
        ...
        memcpy(p_input, &(g_p_shared_memory->input),
               sizeof(sSharedMMapInput_R2));
...
// GameLink::Out
//------------------------------------------------------------------------------
void GameLink::Out(const uint16_t frame_width, const uint16_t frame_height,
                   const double source_ratio, const bool want_mouse,
                   const char *p_program, const uint32_t *p_program_hash,
                   const uint8_t *p_frame, const uint8_t *p_sysmem) {
  // LOG_MSG("GAMELINK: Out %i %i %i", frame_width, frame_height,
  // g_trackonly_mode);
  ...
  if (mutex_result < 0) {
    LOG_MSG("GAMELINK: MUTEX lock failed with %d. errno = %d", mutex_result,
            errno);
  } else
...

  {
...
#ifdef DEBUG_SNOOP
      LOG_MSG("Hash: %x %x %x %x Flags: %x", g_p_shared_memory->program_hash[0],
              g_p_shared_memory->program_hash[1],
              g_p_shared_memory->program_hash[2],
              g_p_shared_memory->program_hash[3], g_p_shared_memory->flags);
#endif
    }

    {
      ...
      if (sdl.gamelink.snoop || !sdl.gamelink.loadaddr)
        offset = 0;

      // Peek
      for (uint32_t pindex = 0; pindex < g_p_shared_memory->peek.addr_count &&
        ...
        if (!sdl.gamelink.snoop) {
          ...
          LOG_MSG("_____ peek: %04x = %i / %i / %i", address, seek, data,
                  address >= g_membase_size
                      ? 0
                      : *(((uint8_t *)g_p_shared_memory) +
                          MEMORY_MAP_CORE_SIZE + address));
#endif
        }
      }
    }
    ...
    if (!sdl.gamelink.snoop) {
      ...
      ExecTerminal(&(g_p_shared_memory->buf_recv),
                   &(g_p_shared_memory->buf_tohost), &(proc_mech_buffer));

    } else if (RunningProgramLoadAddress &&
               g_p_shared_memory->peek.addr_count) {
#ifdef DEBUG_SNOOP
      LOG_MSG("Load Address = %06x", RunningProgramLoadAddress);
      int matches = 0;
#endif
      for (decltype(g_membase_size) addr = 0; addr < g_membase_size - 1024;
           addr++) {
        bool match = true;
        ...
        for (addr_count_t i = 0; i < g_p_shared_memory->peek.addr_count; i++) {
          ...
          if (g_p_shared_memory->peek.addr[i] > 0x10000000)
            continue;
        ...
        if (match) {
          if (!found_addr)
            found_addr = RunningProgramLoadAddress - addr + base;
#ifdef DEBUG_SNOOP
          LOG_MSG("Match at %06x, Offset %06x, Original Load Address %06x",
                  addr, addr - base, RunningProgramLoadAddress - addr + base);
          if (matches++ > 10)
            break;
#else
    ...
    if (mutex_result < 0) {
      LOG_MSG("GAMELINK: MUTEX unlock failed with %d. errno = %d", mutex_result,
              errno);
    } else {
    ...
    if (sdl.gamelink.snoop && found_addr && found_addr != last_addr) {
      ...
      snprintf(result, sizeof(result),
               "Found possible ram offset. Config setting:\n[sdl]\ngamelink "
               "load address = %i",
               found_addr);
