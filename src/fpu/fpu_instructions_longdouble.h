...
static void FPU_FINIT(void) {
  ...
  // HACK: Disable all FPU exceptions until DOSBox-X can catch and reflect FPU
...
static long double FROUND(long double in) {
  ...
  case FPUControlWord::RoundMode::Nearest:
    if (in - floorl(in) > 0.5)
      return (floorl(in) + 1);
    else if (in - floorl(in) < 0.5)
      return (floorl(in));
    else
      return (((static_cast<int64_t>(floorl(in))) & 1) != 0) ? (floorl(in) + 1)
                                                             : (floorl(in));
    break;
...
static void FPU_FLD_F80(PhysPt addr) { fpu.regs_80[TOP].v = FPU_FLD80(addr); }
...
static void FPU_FST_F80(PhysPt addr) { FPU_ST80(addr, TOP); }
...
static void FPU_FADD(Bitu op1, Bitu op2) {
  ...
  FPU_SET_D(was_not_normal || isdenormal(fpu.regs_80[op1].v) ||
            isdenormal(fpu.regs_80[op2].v));
  // flags and such :)
  return;
}
...
static void FPU_FYL2X(void) {
  fpu.regs_80[STV(1)].v *=
      logl(fpu.regs_80[TOP].v) / logl(static_cast<long double>(2.0));
  FPU_FPOP();
  return;
}
...
static void FPU_FYL2XP1(void) {
  fpu.regs_80[STV(1)].v *=
      logl(fpu.regs_80[TOP].v + 1.0l) / logl(static_cast<long double>(2.0));
  FPU_FPOP();
  return;
}

static void FPU_FSCALE(void) {
  fpu.regs_80[TOP].v *= powl(2.0, static_cast<long double>(static_cast<int64_t>(
                                      fpu.regs_80[STV(1)].v)));
...
static void FPU_FABS(void) { fpu.regs_80[TOP].v = fabsl(fpu.regs_80[TOP].v); }
