...
static void FPU_FCLEX(void) { fpu.sw.clearExceptions(); }

static void FPU_FNOP(void) { return; }

static void FPU_PREP_PUSH(void) {
  TOP = (TOP - 1) & 7;
  //	if (GCC_UNLIKELY(fpu.tags[TOP] != TAG_Empty)) E_Exit("FPU stack
  //overflow");
  fpu.tags[TOP] = TAG_Valid;
  fpu.use80[TOP] = false; // the value given is already 64-bit precision, it's
                          // useless to emulate 80-bit precision
}
...
static double FROUND(double in) {
  ...
  case FPUControlWord::RoundMode::Nearest:
    if (in - floor(in) > 0.5)
      return (floor(in) + 1);
    else if (in - floor(in) < 0.5)
      return (floor(in));
    else
      return (((static_cast<int64_t>(floor(in))) & 1) != 0) ? (floor(in) + 1)
                                                            : (floor(in));
    break;
...
static double FPU_FLD80(PhysPt addr, FPU_Reg_80 &raw) {
  ...
  if (test.eind.l.lower == 0 &&
      (uint32_t)test.eind.l.upper == (uint32_t)0x80000000UL &&
      (test.begin & 0x7fff) == 0x7fff) {
...
static void FPU_ST80(PhysPt addr, Bitu reg, FPU_Reg_80 &raw, bool use80) {
  if (use80) {
    ...
    int64_t sign80 =
        ((uint64_t)fpu.regs[reg].ll & ULONGTYPE(0x8000000000000000)) ? 1 : 0;
    ...
    test.begin =
        (static_cast<int16_t>(sign80) << 15) | static_cast<int16_t>(exp80final);
...
static void FPU_FLD_I64(PhysPt addr, Bitu store_to) {
  ...
  // store the signed 64-bit integer in the 80-bit format mantissa with faked
  // exponent. this is needed for DOS and Windows games that use the Pentium
  // fast memcpy trick, using FLD/FST to copy 64 bits at a time. I wonder if
  // that trick is what helped spur Intel to make the MMX extensions :)
  fpu.regs_80[store_to].raw.l = (uint64_t)blah.ll;
  fpu.regs_80[store_to].raw.h =
      ((blah.ll /*sign bit*/ >> (uint64_t)63) ? 0x8000u : 0x0000u) +
      FPU_Reg_80_exponent_bias + 63u; // FIXME: Verify this is correct!
...
static INLINE void FPU_FLD_F32_EA(PhysPt addr) { FPU_FLD_F32(addr, 8); }
static INLINE void FPU_FLD_F64_EA(PhysPt addr) { FPU_FLD_F64(addr, 8); }
static INLINE void FPU_FLD_I32_EA(PhysPt addr) { FPU_FLD_I32(addr, 8); }
static INLINE void FPU_FLD_I16_EA(PhysPt addr) { FPU_FLD_I16(addr, 8); }
...
static void FPU_FST_I16(PhysPt addr) {
  ...
  mem_writew(addr, (val < 32768.0 && val >= -32768.0)
...
static void FPU_FST_I32(PhysPt addr) {
  double val = FROUND(fpu.regs[TOP].d);
  mem_writed(addr, (val < 2147483648.0 && val >= -2147483648.0)
...
static void FPU_FST_I64(PhysPt addr) {
  FPU_Reg blah;
  if (fpu.use80[TOP] && (fpu.regs_80[TOP].raw.h & 0x7FFFu) ==
                            (0x0000u + FPU_Reg_80_exponent_bias + 63u)) {
    // FIXME: This works so far for DOS demos that use the "Pentium memcpy
    // trick" to copy 64 bits at a time.
    //        What this code needs to do is take the exponent into account and
    //        then clamp the 64-bit int within range. This cheap hack is good
    //        enough for now.
    mem_writed(addr, (uint32_t)(fpu.regs_80[TOP].raw.l));
    mem_writed(addr + 4, (uint32_t)(fpu.regs_80[TOP].raw.l >> (uint64_t)32));
  } else {
    double val = FROUND(fpu.regs[TOP].d);
    blah.ll = (val < 9223372036854775808.0 && val >= -9223372036854775808.0)
...
// WARNING: UNTESTED. Original contributed code only focused on the x86 FPU
// case.
...
#include <math.h>
/* the purpose of this macro is to test for -/+inf. NaN is not inf. If finite or
 * NaN it's not infinity */
...
static void FPU_FADD(Bitu op1, Bitu op2) {
  // HACK: Set the denormal flag according to whether the source or final result
  // is a denormalized number.
  //       This is vital if we don't want certain DOS programs to mis-detect our
  //       FPU emulation as an IIT clone chip when cputype == 286
  bool was_not_normal = isdenormal(fpu.regs[op1].d);
  fpu.regs[op1].d += fpu.regs[op2].d;
  FPU_SET_D(was_not_normal || isdenormal(fpu.regs[op1].d) ||
            isdenormal(fpu.regs[op2].d));
  fpu.use80[op1] =
      false; // we used the less precise version, drop the 80-bit precision
  // flags and such :)
  return;
}
...
static void FPU_FCOM(Bitu st, Bitu other) {
  ...
  /* HACK: If emulating a 286 processor we want the guest to think it's talking
   * to a 287. For more info, read
   * [http://www.intel-assembler.it/portale/5/cpu-identification/asm-source-to-find-intel-cpu.asp].
   */
  /* TODO: This should eventually become an option, say, a dosbox.conf option
   * named fputype where the user can enter "none" for no FPU, 287 or 387 for
   * cputype=286 and cputype=386, or "auto" to match the CPU (8086 => 8087). If
   * the FPU type is 387 or auto, then skip this hack. Else for 8087 and 287,
   * use this hack. */
  if (FPU_ArchitectureType < FPU_ARCHTYPE_387) {
    if ((std::isinf)(fpu.regs[st].d) && (std::isinf)(fpu.regs[other].d)) {
      /* 8087/287 consider -inf == +inf and that's what DOS programs test for to
       * detect 287 vs 387 */
      FPU_SET_C3(1);
      FPU_SET_C2(0);
      FPU_SET_C0(0);
...
static void FPU_FPREM1(void) {
  ...
  if (quot - quotf > 0.5)
    ...
    ressaved = static_cast<int64_t>(
        (((static_cast<int64_t>(quotf)) & 1) != 0) ? (quotf + 1) : (quotf));
  fpu.use80[TOP] =
      false; // we used the less precise version, drop the 80-bit precision
  fpu.regs[TOP].d = valtop - ressaved * valdiv;
  FPU_SET_C0(static_cast<Bitu>(ressaved & 4));
...
static void FPU_FYL2XP1(void) {
  fpu.use80[STV(1)] =
      false; // we used the less precise version, drop the 80-bit precision
  fpu.regs[STV(1)].d *=
      log(fpu.regs[TOP].d + 1.0) / log(static_cast<double>(2.0));
  FPU_FPOP();
  return;
}
...
static void FPU_FSCALE(void) {
  fpu.use80[TOP] =
      false; // we used the less precise version, drop the 80-bit precision
  fpu.regs[TOP].d *=
      pow(2.0, static_cast<double>(static_cast<int64_t>(fpu.regs[STV(1)].d)));
...
static void FPU_FSAVE(PhysPt addr, bool op16) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    FPU_ST80(addr + start, STV(i), /*&*/ fpu.regs_80[STV(i)],
             fpu.use80[STV(i)]);
    start += 10;
  }
  FPU_FINIT();
...
static INLINE void FPU_FADD_EA(Bitu op1) { FPU_FADD(op1, 8); }
static INLINE void FPU_FMUL_EA(Bitu op1) { FPU_FMUL(op1, 8); }
static INLINE void FPU_FSUB_EA(Bitu op1) { FPU_FSUB(op1, 8); }
static INLINE void FPU_FSUBR_EA(Bitu op1) { FPU_FSUBR(op1, 8); }
static INLINE void FPU_FDIV_EA(Bitu op1) { FPU_FDIV(op1, 8); }
static INLINE void FPU_FDIVR_EA(Bitu op1) { FPU_FDIVR(op1, 8); }
static INLINE void FPU_FCOM_EA(Bitu op1) { FPU_FCOM(op1, 8); }
