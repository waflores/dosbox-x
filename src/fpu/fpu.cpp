...
void FPU_FLDCW(PhysPt addr) { fpu.cw = mem_readw(addr); }
...
void FPU_ESC1_EA(Bitu rm, PhysPt addr, bool op16) {
  ...
  case 0x01: /* UNKNOWN */
    LOG(LOG_FPU, LOG_WARN)("ESC EA 1:Unhandled group %d subfunction %d",
                           (int)group, (int)sub);
    break;
  case 0x02: /* FST float*/
    FPU_FST_F32(addr);
...
void FPU_ESC1_Normal(Bitu rm) {
  ...
  case 0x04:
    ...
    case 0x02: /* UNKNOWN */
    case 0x03: /* ILLEGAL */
      LOG(LOG_FPU, LOG_WARN)("ESC 1:Unhandled group %X subfunction %X",
                             (int)group, (int)sub);
      break;
    case 0x04: /* FTST */
      FPU_FTST();
...
void FPU_ESC2_Normal(Bitu rm) {
  ...
  case 0x05:
    ...
    case 0x01: /* FUCOMPP */
      ...
      break;
    default:
      LOG(LOG_FPU, LOG_WARN)("ESC 2:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
      break;
    }
    break;
...
void FPU_ESC3_EA(Bitu rm, PhysPt addr) {
  ...
  case 0x01: /* FISTTP */
    if (CPU_ArchitectureType == CPU_ARCHTYPE_EXPERIMENTAL) {
      ...
      LOG(LOG_FPU, LOG_WARN)("ESC 3 EA:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
    break;
  case 0x02: /* FIST */
    FPU_FST_I32(addr);
...
void FPU_ESC3_Normal(Bitu rm) {
  ...
  case 0x04:
    ...
    case 0x00: // FNENI
      if (FPU_ArchitectureType <= FPU_ARCHTYPE_8087)
        ...
        LOG(LOG_FPU, LOG_ERROR)(
            "8087 only fpu code used esc 3: group 4: subfunction :%d",
            (int)sub);
      break;
    case 0x01: // FNDIS
      if (FPU_ArchitectureType <= FPU_ARCHTYPE_8087)
    ...
    case 0x04: // FNSETPM
    case 0x05: // FRSTPM
      //			LOG(LOG_FPU,LOG_ERROR)("80267 protected mode
      //(un)set. Nothing done");
      FPU_FNOP();
      break;
    default:
      E_Exit("ESC 3:ILLEGAL OPCODE group %d subfunction %d", (int)group,
             (int)sub);
    }
    break;
  ...
  case 0x06: /* FCOMI STi */
    ...
    break;
  default:
    LOG(LOG_FPU, LOG_WARN)("ESC 3:Unhandled group %d subfunction %d",
                           (int)group, (int)sub);
    break;
  }
  return;
...
void FPU_ESC5_EA(Bitu rm, PhysPt addr, bool op16) {
  ...
  case 0x01: /* FISTTP longint*/
    if (CPU_ArchitectureType == CPU_ARCHTYPE_EXPERIMENTAL) {
      ...
      LOG(LOG_FPU, LOG_WARN)("ESC 5 EA:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
    break;
...
void FPU_ESC5_Normal(Bitu rm) {
  ...
  case 0x05: /*FUCOMP STi */
    ...
    break;
  default:
    LOG(LOG_FPU, LOG_WARN)("ESC 5:Unhandled group %d subfunction %d",
                           (int)group, (int)sub);
    break;
  }
}
...
void FPU_ESC6_Normal(Bitu rm) {
  ...
  case 0x03: /*FCOMPP*/
    if (sub != 1) {
      LOG(LOG_FPU, LOG_WARN)("ESC 6:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
      return;
    }
    FPU_FCOM(TOP, STV(1));
...
void FPU_ESC7_EA(Bitu rm, PhysPt addr) {
  ...
  case 0x01: /* FISTTP int16_t */
    if (CPU_ArchitectureType == CPU_ARCHTYPE_EXPERIMENTAL) {
      ...
      LOG(LOG_FPU, LOG_WARN)("ESC 7 EA:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
    break;
...
void FPU_ESC7_Normal(Bitu rm) {
  ...
  case 0x04:
    ...
    case 0x00: /* FNSTSW AX*/
      ...
      break;
    default:
      LOG(LOG_FPU, LOG_WARN)("ESC 7:Unhandled group %d subfunction %d",
                             (int)group, (int)sub);
      break;
    }
    break;
...
void FPU_Selftest_32() {
  ...
  static const struct ftest test[] = {
      // name			// val		// exponent (no bias)
      // // sign		// 23-bit mantissa without 23rd implied bit (max
      // 2^23-1 = 0x7FFFFF)
      {"0.0f", 0.0f, -FPU_Reg_32_exponent_bias, 0,
       0x000000},                       // IEEE standard way to encode zero
  ...
  for (size_t t = 0; t < tests; t++) {
    ...
    if (((int)ft.f.exponent - FPU_Reg_32_exponent_bias) != test[t].exponent ||
      ...
      LOG(LOG_FPU, LOG_WARN)(
          "  expected t.v = %.10f t.s=%u t.exp=%d t.mantissa=%u", test[t].val,
          test[t].sign, (int)test[t].exponent, (unsigned int)test[t].mantissa);
      goto dump;
    }
  }
...
void FPU_Selftest_64() {
  ...
  static const struct ftest test[] = {
      // name			// val		// exponent (no bias)
      // // sign		// 52-bit mantissa without 52rd implied bit (max
      // 2^52-1 = 0x1FFFFFFFFFFFFF)
      {"0.0d", 0.0, -FPU_Reg_64_exponent_bias, 0,
       0x0000000000000ULL}, // IEEE standard way to encode zero
  ...
  for (size_t t = 0; t < tests; t++) {
    ...
    if (((int)ft.f.exponent - FPU_Reg_64_exponent_bias) != test[t].exponent ||
      ...
      LOG(LOG_FPU, LOG_WARN)(
          "  expected t.v = %.10f t.s=%u t.exp=%d t.mantissa=%llu (0x%llx)",
          test[t].val, test[t].sign, (int)test[t].exponent,
...
dump:
  LOG(LOG_FPU, LOG_WARN)(
      "Result: t.v = %.10f t.s=%u t.exp=%d t.mantissa=%llu (0x%llx)", ft.v,
      (int)ft.f.sign, (int)ft.f.exponent - FPU_Reg_64_exponent_bias,
      (unsigned long long)ft.f.mantissa, (unsigned long long)ft.f.mantissa);
}
...
void FPU_Selftest_80() {
  ...
  // we're assuming "long double" means the Intel 80x87 extended precision
  // format, which is true when using GCC on Linux i686 and x86_64 hosts.
  //
  // I understand that other platforms (PowerPC, Sparc, etc) might have other
  // ideas on what makes "long double" and I also understand Microsoft Visual
  // C++ treats long double the same as double. We will disable this test with
  // #ifdefs when compiling for platforms where long double doesn't mean 80-bit
  // extended precision.
  struct ftest {
    const char *name;
    long double val;
  ...
  static const struct ftest test[] = {
      // name			// val		// exponent (no bias)
      // // sign		// 64-bit mantissa WITH whole integer bit #63
      {"0.0L", 0.0, -FPU_Reg_80_exponent_bias, 0,
       0x0000000000000000ULL}, // IEEE standard way to encode zero
  ...
  if (sizeof(long double) == sizeof(double)) {
    LOG(LOG_FPU, LOG_WARN)("FPU80 sizeof(long double) == sizeof(double) so "
                           "your compiler just makes it an alias. skipping "
                           "tests. please recompile with proper config.");
    return;
  } else if (sizeof(long double) < 10 || sizeof(long double) > 16) {
    // NTS: We can't assume 10 bytes. GCC on i686 makes long double 12 or 16
    // bytes long for alignment
  ...
  ft.raw.h = 1U << 15U;
  if (ft.f.sign != 1 || ft.f.exponent != 0 || ft.f.mantissa != 0) {
    LOG(LOG_FPU, LOG_WARN)("FPU80 bitfield test #1 failed. h=%04x l=%016llx",
                           (unsigned int)ft.raw.h,
                           (unsigned long long)ft.raw.l);
    return;
  }
  ft.raw.l = 0;
  ft.raw.h = 1U << 0U;
  if (ft.f.sign != 0 || ft.f.exponent != 1 || ft.f.mantissa != 0) {
    LOG(LOG_FPU, LOG_WARN)("FPU80 bitfield test #2 failed. h=%04x l=%016llx",
                           (unsigned int)ft.raw.h,
                           (unsigned long long)ft.raw.l);
    return;
  }
  ...
  ft.raw.h = 0;
  if (ft.f.sign != 0 || ft.f.exponent != 0 || ft.f.mantissa != 1) {
    LOG(LOG_FPU, LOG_WARN)("FPU80 bitfield test #3 failed. h=%04x l=%016llx",
                           (unsigned int)ft.raw.h,
                           (unsigned long long)ft.raw.l);
    return;
  }
  ...
  for (size_t t = 0; t < tests; t++) {
    ...
    if (((int)ft.f.exponent - FPU_Reg_80_exponent_bias) != test[t].exponent ||
      ...
      LOG(LOG_FPU, LOG_WARN)(
          "  expected t.v = %.10Lf t.s=%u t.exp=%d t.mantissa=%llu (0x%llx)",
          test[t].val, test[t].sign, (int)test[t].exponent,
          (unsigned long long)test[t].mantissa,
          (unsigned long long)test[t].mantissa);
      goto dump;
...
dump:
  LOG(LOG_FPU, LOG_WARN)(
      "Result: t.v = %.10Lf t.s=%u t.exp=%d t.mantissa=%llu (0x%llx)", ft.v,
      (int)ft.f.sign, (int)ft.f.exponent - FPU_Reg_64_exponent_bias,
      (unsigned long long)ft.f.mantissa, (unsigned long long)ft.f.mantissa);
#else
  LOG(LOG_FPU, LOG_DEBUG)("FPU80 selftest skipped, compiler does not have long "
                          "double as 80-bit IEEE");
#endif
}
...
void FPU_Selftest() {
  ...
  if (freg.l.lower != 0x89ABCDEFUL || freg.l.upper != 0x01234567UL) {
    LOG(LOG_FPU, LOG_WARN)(
        "FPU_Reg field order is wrong. ll=0x%16llx l=0x%08lx h=0x%08lx",
        (unsigned long long)freg.ll, (unsigned long)freg.l.lower,
        (unsigned long)freg.l.upper);
  }
#else
...
#else
  LOG(LOG_FPU,
      LOG_NORMAL)("FPU core: double FPU (caution: possible precision errors)");
#endif
...
static INLINE uint16_t fpu_tag_word_from_abridged(const uint8_t b) {
  ...
  /* In the 8 bits they packed the valid/empty bitfield (with 8 bits reserved!)
   * they could have just stored the 16-bit tag word instead! */
  for (i = 0; i < 8; i++) {
    if (b & (1u << i)) {
...
void CPU_FXSAVE(PhysPt eaa) {
  ...
  mem_writeb(eaa + 0x004,
             fpu_tag_word_abridged()); /* +0x004 FPU tag words, abridged to a
                                          bitfield of 1=not empty 0=empty,
                                          register order NOT from TOP */
  mem_writeb(eaa + 0x005, 0x00);       /* +0x005 reserved */
  mem_writew(eaa + 0x006, 0x0000);     /* +0x006 x87 FPU opcode (??) */
  mem_writed(eaa + 0x008,
             reg_eip); /* +0x008 x87 FPU instruction pointer (???) */
  mem_writew(
      eaa + 0x00C,
      Segs.val[cs]); /* +0x00C x87 FPU instruction pointer segment (???) */
  mem_writew(eaa + 0x00E, 0x0000); /* +0x00E reserved */
  mem_writed(eaa + 0x010,
             reg_eip); /* +0x010 x87 FPU instruction operand (???) */
  mem_writew(
      eaa + 0x014,
      Segs.val[ds]); /* +0x014 x87 FPU instruction operand segment (???) */
...
#else
    FPU_ST80(eaa + 0x020 + (i * 16), STV(i), /*&*/ fpu.regs_80[STV(i)],
             fpu.use80[STV(i)]);
#endif
...
#else
    fpu.regs[STV(i)].d =
        FPU_FLD80(eaa + 0x020 + (i * 16), /*&*/ fpu.regs_80[STV(i)]);
    fpu.use80[STV(i)] = true;
#endif
  }

  {
    uint16_t tw = fpu_tag_word_from_abridged(mem_readb(
        eaa + 0x004)); /* +0x004 FPU tag words, abridged to a bitfield of 1=not
                          empty 0=empty, register order NOT from TOP */
    FPU_SetTag(tw);
  }
...
public:
  SerializeFpu() : SerializeGlobalPOD("FPU") { registerPOD(fpu); }
} dummy;
} // namespace

std::string FPUStatusWord::to_string() const {
  return "B=" + std::to_string(B) + " C3-C0=" + std::to_string(C3) +
         std::to_string(C2) + std::to_string(C1) + std::to_string(C0) +
         " ES=" + std::to_string(ES) + " SF=" + std::to_string(SF) +
         " PE=" + std::to_string(PE) + " UE=" + std::to_string(UE) +
         " OE=" + std::to_string(OE) + " ZE=" + std::to_string(ZE) +
         " DE=" + std::to_string(DE) + " IE=" + std::to_string(IE) +
