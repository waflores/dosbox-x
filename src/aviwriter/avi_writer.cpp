...
/* FIXME: I made the mistake of putting critical calls in assert() calls, which
 * under MSVC++ may evaluate to nothing in Release builds */
...
int avi_writer_stream_check_samplecount(avi_writer_stream *s,
                                        unsigned int len) {
  if (s == NULL)
    return 0;
  ...
  if (s->sample_index == NULL) {
    ...
    s->sample_index = (avi_writer_stream_index *)malloc(
        sizeof(avi_writer_stream_index) * s->sample_index_alloc);
    if (s->sample_index == NULL) {
    ...
    avi_writer_stream_index *n = (avi_writer_stream_index *)realloc(
        (void *)(s->sample_index), sizeof(avi_writer_stream_index) * na);
    if (n == NULL)
      return 0;
    s->sample_index = n;
...
int avi_writer_stream_set_format(avi_writer_stream *s, void *data, size_t len) {
  ...
  s->format_len = len;
  if (data)
    memcpy(s->format, data, len);
  else
    memset(s->format, 0, len);
  return 1;
}
...
avi_writer_stream *avi_writer_new_stream(
    avi_writer *w) { /* reminder: you are not required to free this pointer, the
                        writer does it for you on close_file() */
  avi_writer_stream *s;

  if (w == NULL)
    return NULL;
  if (w->state != AVI_WRITER_STATE_INIT)
    return NULL;

  if (w->avi_stream == NULL) {
    w->avi_stream_max = 1;
    w->avi_stream_alloc = 8;
    w->avi_stream = (avi_writer_stream *)malloc(
        sizeof(avi_writer_stream) * (unsigned int)w->avi_stream_alloc);
    if (w->avi_stream == NULL) {
      w->avi_stream_max = 0;
    ...
    avi_writer_stream *n = (avi_writer_stream *)realloc(
        (void *)(w->avi_stream), sizeof(avi_writer_stream) * (unsigned int)na);
    if (n == NULL)
      return NULL;
    w->avi_stream = n;
  ...
  memset(s, 0, sizeof(*s));
  s->index =
      (int)(s - w->avi_stream); /* NTS: C compiler converts pointer math to # of
                                   elements since w->avi_stream */
  return s;
}
...
int avi_writer_begin_data(avi_writer *w) {
  ...
  if (w == NULL)
    return 0;
  if (w->state != AVI_WRITER_STATE_HEADER)
    return 0;
  ...
  assert(riff_stack_set_chunk_list_type(&chunk, riff_LIST,
                                        riff_fourcc_const('m', 'o', 'v', 'i')));
  if (w->enable_stream_writing) {
    assert(riff_stack_enable_placeholder(w->riff, &chunk));
    chunk.disable_sync = 1;
  }
  assert(riff_stack_push(
      w->riff, &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
  w->movi = chunk;
...
int avi_writer_begin_header(avi_writer *w) {
  ...
  /* [1] RIFF:AVI */
  assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
  assert(riff_stack_set_chunk_list_type(&chunk, riff_RIFF,
                                        riff_fourcc_const('A', 'V', 'I', ' ')));
  if (w->enable_stream_writing) {
     ...
     * file past the length given in the LIST:AVI chunk and forcing the
     * placeholder ensure that VLC player will gracefully handle incomplete
     * files written by this library. */
    assert(riff_stack_enable_placeholder(w->riff, &chunk));
    chunk.disable_sync = 1;
  }
  assert(riff_stack_push(
  ...
  assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
  assert(riff_stack_set_chunk_list_type(&chunk, riff_LIST,
                                        riff_fourcc_const('h', 'd', 'r', 'l')));
  assert(riff_stack_push(
      w->riff, &chunk)); /* NTS: we can reuse chunk, the stack copies it here */

  /* [3] avih */
  assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
  assert(riff_stack_set_chunk_data_type(&chunk,
                                        riff_fourcc_const('a', 'v', 'i', 'h')));
  assert(riff_stack_push(
      w->riff, &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
  assert(riff_stack_write(w->riff, riff_stack_top(w->riff), &w->main_header,
                          sizeof(w->main_header)) ==
  ...
  for (stream = 0; stream < w->avi_stream_max; stream++) {
    ...
    if (s->chunk_fourcc == 0) {
      if (s->header.fccType == avi_fccType_video ||
          s->header.fccType == avi_fccType_iavs) {
        if (s->format != NULL &&
            s->format_len >= sizeof(windows_BITMAPINFOHEADER)) {
      ...
      s->chunk_fourcc |=
          ((((unsigned int)s->index / 10U) % 10U) + (unsigned char)('0'))
          << 0UL;
      s->chunk_fourcc |= (((unsigned int)s->index % 10U) + (unsigned char)('0'))
                         << 8UL;
    }
    ...
    assert(riff_stack_set_chunk_list_type(
        &chunk, riff_LIST, riff_fourcc_const('s', 't', 'r', 'l')));
    assert(riff_stack_push(
        w->riff,
        &chunk)); /* NTS: we can reuse chunk, the stack copies it here */

    /* [4] strh */
    assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
    assert(riff_stack_set_chunk_data_type(
        &chunk, riff_fourcc_const('s', 't', 'r', 'h')));
    assert(riff_stack_push(
        w->riff,
        &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
    assert(riff_stack_write(w->riff, riff_stack_top(w->riff), &s->header,
                            sizeof(s->header)) == (int)sizeof(s->header));
    ...
    assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
    assert(riff_stack_set_chunk_data_type(
        &chunk, riff_fourcc_const('s', 't', 'r', 'f')));
    assert(riff_stack_push(
        w->riff,
        &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
    if (s->format && s->format_len > 0)
      assert((int)riff_stack_write(w->riff, riff_stack_top(w->riff), s->format,
                                   (size_t)s->format_len) ==
             (int)s->format_len);
    riff_stack_pop(w->riff);
    ...
    if (s->name != NULL) {
      ...
      assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
      assert(riff_stack_set_chunk_data_type(
          &chunk, riff_fourcc_const('s', 't', 'r', 'n')));
      assert(riff_stack_push(
          w->riff,
          &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
      assert((int)riff_stack_write(w->riff, riff_stack_top(w->riff), s->name,
                                   (size_t)len) == (int)len);
      riff_stack_pop(w->riff);
    }
    ...
    if (w->enable_opendml_index) {
      ...
      assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
      assert(riff_stack_set_chunk_data_type(
          &chunk, riff_fourcc_const('J', 'U', 'N', 'K')));
      assert(riff_stack_push(
          w->riff,
          &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
      memset(tmp, 0, sizeof(tmp));
      for (i = 0; i < (16384 / 512); i++)
        assert(riff_stack_write(w->riff, riff_stack_top(w->riff), tmp, 512) ==
               512);
...
int avi_writer_stream_repeat_last_chunk(avi_writer *w, avi_writer_stream *s) {
  ...
  if (w->state != AVI_WRITER_STATE_BODY)
    return 0;
  if (s->sample_write_chunk ==
      0) /* if there *IS* no previous chunk, then bail */
    return 0;
  ...
  if (w->enable_opendml) {
    /* if we're writing an OpenDML 2.0 compliant file, and we're approaching a
     * movi size of 1GB, then split the movi chunk and start another RIFF:AVIX
     */
    if ((unsigned long long)(w->riff->top->write_offset + 8) >=
        0x3FF00000ULL) { /* 1GB - 16MB */
      riff_stack_writing_sync(
          w->riff);                   /* sync all headers and pop all chunks */
      ...
      assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
      assert(riff_stack_set_chunk_list_type(
          &chunk, riff_RIFF, riff_fourcc_const('A', 'V', 'I', 'X')));
      if (w->enable_stream_writing) {
        assert(riff_stack_enable_placeholder(w->riff, &chunk));
        chunk.disable_sync = 1;
      }
      assert(riff_stack_push(
          ...
          &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
      if (w->enable_stream_writing)
        riff_stack_header_sync(w->riff, riff_stack_top(w->riff));

      /* start the movi chunk */
      assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
      assert(riff_stack_set_chunk_list_type(
          &chunk, riff_LIST, riff_fourcc_const('m', 'o', 'v', 'i')));
      if (w->enable_stream_writing) {
        assert(riff_stack_enable_placeholder(w->riff, &chunk));
        chunk.disable_sync = 1;
      }
    ...
    /* else, if we're about to pass 2GB, then stop allowing any more data,
     * because the traditional AVI format uses 32-bit integers and most
     * implementations treat them as signed. */
    if ((w->movi.absolute_data_offset + w->riff->top->write_offset + 8) >=
        0x7FF00000LL) /* 2GB - 16MB */
      return 0;
  }

  /* write chunk into movi (for consistent timekeeping with older AVI apps that
   * don't read the index) */
  assert(riff_stack_begin_new_chunk_here(w->riff, &chunk));
  ...
  /* NTS: this must come after the check_samplecount() because
   * check_samplecount() uses realloc() to extend the array and realloc() may
   * move the data around to fulfill the request */
...
/* NTS: this code makes no attempt to optimize chunk sizes and combine
 * samples/frames---if you're stupid enough to call this routine with 4-byte
 * long data then 4-byte long chunks is what you'll get, don't blame me if doing
 * that overruns the allocated space set aside for the indexes. */
int avi_writer_stream_write(avi_writer *w, avi_writer_stream *s, void *data,
  ...
  /* calling this function with data == NULL is perfectly valid, it simply means
   * no data */
  if (data == NULL)
    len = 0;
  ...
  if (w->enable_opendml) {
     ...
     * movi size of 1GB, then split the movi chunk and start another RIFF:AVIX
     */
    if (((unsigned long long)w->riff->top->write_offset +
         (unsigned long long)len) >= 0x3FF00000ULL) { /* 1GB - 16MB */
      riff_stack_writing_sync(
          w->riff);                   /* sync all headers and pop all chunks */
      assert(w->riff->current == -1); /* should be at top level */
  ...
  assert(riff_stack_push(w->riff, &chunk));
  if (w->enable_stream_writing) {
    /* use an optimized version of riff_stack_write() that blasts the RIFF chunk
     * header + data in one go */
    if (data != NULL && len > 0)
      assert((int)riff_stack_streamwrite(w->riff, riff_stack_top(w->riff), data,
                                         (size_t)len) == (int)len);
    else
      assert((int)riff_stack_streamwrite(w->riff, riff_stack_top(w->riff), NULL,
                                         (size_t)0) == (int)0);
  } else {
    if (data != NULL && len > 0)
      assert((int)riff_stack_write(w->riff, riff_stack_top(w->riff), data,
                                   (size_t)len) == (int)len);
  }
  riff_stack_pop(w->riff);
  ...
  /* if stream writing is not enabled, then rewrite all RIFF parent chunks to
...
int avi_writer_emit_avioldindex(avi_writer *w) {
  ...
  if (w->group != 0)
    return 0;
  if (avi_io_buffer_init(sizeof(*ie)) == NULL)
    return 0;
  ...
  do {
    ...
    for (c = 0U; c < (unsigned int)w->avi_stream_max; c++) {
      ...
      if (i < s->sample_index_max) {
        ...
        long long ofs = ((long long)sie->offset - 8LL) -
                        ((long long)w->movi.absolute_header_offset + 8LL);
        assert(ofs >= 0);
        if (ofs < 0x80000000LL) { /* the AVIOLDINDEX can only support 32-bit
                                     offsets */
          if ((avi_io_write + sizeof(*ie)) > avi_io_fence) {
            size_t sz = (size_t)(avi_io_write - avi_io_buf);
            /* flush to disk */
            assert(riff_stack_write(w->riff, riff_stack_top(w->riff),
                                    avi_io_buf, sz) == (int)sz);
            /* reset pointer */
            avi_io_write = avi_io_buf;
          }

          /* TODO: FIXME This needs to use the rawint.h macros to set the
           * variables! */
  ...
  if (avi_io_write != avi_io_fence) {
    size_t sz = (size_t)(avi_io_write - avi_io_buf);
    /* flush to disk */
    assert(riff_stack_write(w->riff, riff_stack_top(w->riff), avi_io_buf, sz) ==
           (int)sz);
    /* reset pointer */
    avi_io_write = avi_io_buf;
  }
...
int avi_writer_update_avi_and_stream_headers(avi_writer *w) {
  ...
  if (w == NULL)
    return 0;
  if (w->state == AVI_WRITER_STATE_INIT || w->state == AVI_WRITER_STATE_DONE)
    return 0;

  if (w->enable_avioldindex || w->enable_opendml_index) {
    /* FIXME: This needs to use the rawint.h macros to read and set the value */
    w->main_header.dwFlags |= riff_avih_AVIMAINHEADER_flags_HASINDEX |
  ...
  /* FIXME: When writing OpenDML files, we're actually supposed to set the
   * stream header's dwLength to what would be the maximum length for older
   * programs that do not read OpenDML extensions, and then we write an extended
   * chunk into the stream header LIST that holds the true length. When will we
   * implement this? */
  for (stream = 0; stream < w->avi_stream_max; stream++) {
    ...
    if (s->header.fccType == avi_fccType_video ||
      ...
      if (s->format != NULL && s->format_len >= sizeof(windows_WAVEFORMAT)) {
        ...
        if (nss != 0)
          nlength = s->sample_write_offset / nss;
      } else {
    ...
    if (s->strh.absolute_data_offset != 0LL &&
        s->strh.data_length >= sizeof(riff_strh_AVISTREAMHEADER)) {
      riff_chunk r = s->strh;
      assert(riff_stack_seek(w->riff, &r, 0) == 0);
      assert(riff_stack_write(w->riff, &r, &s->header, sizeof(s->header)) ==
             (int)sizeof(s->header));
    }
  }
  ...
  if (w->avih.absolute_data_offset != 0LL &&
      w->avih.data_length >= sizeof(riff_avih_AVIMAINHEADER)) {
    riff_chunk r = w->avih;
    assert(riff_stack_seek(w->riff, &r, 0) == 0);
    assert(riff_stack_write(w->riff, &r, &w->main_header,
                            sizeof(w->main_header)) ==
           (int)sizeof(w->main_header));
  }
...
/* NTS: this writer keeps the AVISUPERINDEXes in the header, therefore the file
 * offset returned is never larger than about 2MB or so */
uint64_t avi_writer_stream_alloc_superindex(avi_writer *w,
                                            avi_writer_stream *s) {
  riff_chunk chunk;
  ...
  if (s->indx_junk.absolute_data_offset != 0LL) {
    if (s->indx_junk.data_length < 256)
      return 0ULL;

    /* convert the JUNK chunk into an indx chunk */
    {
      assert(
          riff_stack_seek(w->riff, NULL, s->indx_junk.absolute_header_offset) ==
          s->indx_junk.absolute_header_offset);
    ...
    /* emit the header */
    {
      ...
      assert(riff_stack_write(w->riff, &chunk, &s->superindex,
                              sizeof(s->superindex)) ==
             (int)sizeof(s->superindex));
    ...
    if ((s->indx_entryofs + sizeof(riff_indx_AVISUPERINDEX_entry)) >
        s->indx.data_length)
      return 0ULL;
  ...
  if (s->indx.absolute_data_offset != 0LL) {
    ...
    uint64_t ofs =
        (uint64_t)s->indx.absolute_data_offset + (uint64_t)s->indx_entryofs;
...
int avi_writer_emit_opendml_indexes(avi_writer *w) {
  ...
  if (!w->enable_opendml_index)
    return 0;
  if (avi_io_buffer_init(sizeof(*stdie)) == NULL)
    return 0;

  for (stream = 0; stream < w->avi_stream_max; stream++) {
    s = w->avi_stream + stream;
    if (s->indx_entryofs != 0)
      break;
    if (s->sample_index == NULL)
      continue;
    in1 = ((stream / 10) % 10) + '0';
    ...
    for (chunk = 0; chunk < s->sample_index_max;) {
      ...
      assert(riff_stack_set_chunk_data_type(
          &newchunk, riff_fourcc_const('i', 'x', in1, in2)));
      assert(riff_stack_push(
          w->riff,
          &newchunk)); /* NTS: we can reuse chunk, the stack copies it here */
      ...
      assert(riff_stack_write(w->riff, riff_stack_top(w->riff), &stdh,
                              sizeof(stdh)) == (int)sizeof(stdh));
      ...
      while (chunk < s->sample_index_max) {
        ...
        if ((avi_io_write + sizeof(*stdie)) > avi_io_fence) {
          size_t sz = (size_t)(avi_io_write - avi_io_buf);
          /* flush to disk */
          assert(riff_stack_write(w->riff, riff_stack_top(w->riff), avi_io_buf,
                                  sz) == (int)sz);
          /* reset pointer */
          avi_io_write = avi_io_buf;
        }
        ...
        if ((si->dwFlags & riff_idx1_AVIOLDINDEX_flags_KEYFRAME) == 0)
          stdie->dwSize |= (1UL << 31UL);
        out_chunks++;
        chunk++;
      }
      ...
      assert(riff_stack_seek(w->riff, NULL, (int64_t)superindex) ==
             (int64_t)superindex);
      assert(riff_stack_write(w->riff, NULL, &suie, sizeof(suie)) ==
             (int)sizeof(suie));
    }
  }
...
int avi_writer_end_data(avi_writer *w) {
  ...
  /* now, while at this level, emit the AVIOLDINDEX if this is still the first
   * movi */
  if (w->group == 0 && w->enable_avioldindex)
    avi_writer_emit_avioldindex(w);
...
int avi_writer_finish(avi_writer *w) {
  if (w == NULL)
    return 0;
  if (w->state != AVI_WRITER_STATE_FOOTER)
    return 0;
