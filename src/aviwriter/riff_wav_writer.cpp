...
int riff_wav_writer_set_format_old(riff_wav_writer *w,
                                   windows_WAVEFORMATOLD *f) {
  if (w == NULL || f == NULL)
    return 0;
  if (w->state != RIFF_WRITER_INIT)
    return 0;
...
int riff_wav_writer_set_format_ex(riff_wav_writer *w, windows_WAVEFORMATEX *f,
                                  size_t len) {
  if (w == NULL || f == NULL)
    return 0;
  if (w->state != RIFF_WRITER_INIT)
  ...
  if (__le_u16(&f->cbSize) != 0u)
    w->fmt_len += (size_t)2u + __le_u16(&f->cbSize);
  if (w->fmt_len > len)
    return 0;
  if ((w->fmt = malloc(w->fmt_len)) == NULL)
    return 0;
...
int riff_wav_writer_begin_header(riff_wav_writer *w) {
  ...
  assert(riff_stack_set_chunk_data_type(&chunk,
                                        riff_fourcc_const('f', 'm', 't', ' ')));
  assert(riff_stack_push(
      w->riff, &chunk)); /* NTS: we can reuse chunk, the stack copies it here */
  assert((int)riff_stack_write(w->riff, riff_stack_top(w->riff), w->fmt,
                               w->fmt_len) == (int)w->fmt_len);
