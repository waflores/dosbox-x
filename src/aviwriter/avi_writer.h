...
typedef struct avi_writer_stream_index {
  ...
  uint64_t offset;
  uint32_t length;
  uint32_t dwFlags; /* flags in AVIOLDINDEX. In OpenDML 2.0 indexes, only the
                       keyframe bit has meaning */
...
typedef struct avi_writer {
  ...
  unsigned char
      enable_opendml; /* 1=enable RIFF:AVIX extensions to allow files >= 2GB */
  unsigned char
      enable_stream_writing; /* 1=try to always write as a stream to disk at the
                                expense of some AVI structure integrity */
