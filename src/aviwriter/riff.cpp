...
int riff_std_read(void *a, void *b, size_t c) {
  ...
  if (rs->fd < 0)
    return -1;
  if (rs->trk_file_pointer < (int64_t)0)
    return -1;
  rd = (int)read(rs->fd, b, (unsigned int)c);

  if (rd < 0)
    rs->trk_file_pointer = -1LL;
  else
    rs->trk_file_pointer += rd;
...
int64_t riff_std_seek(void *a, int64_t offset) {
  ...
  if (rs->fd < 0)
    return -1;
  if (!rs->always_lseek && offset == rs->trk_file_pointer)
    return offset;
#if defined(_MSC_VER)
...
int riff_buf_read(void *a, void *b, size_t c) {
  ...
  if (rs->buffer == NULL)
    return -1;
  if (rs->bufofs >= rs->buflen)
    return 0;
  if ((rs->bufofs + c) > rs->buflen)
    c = (size_t)(rs->buflen - rs->bufofs);
...
int64_t riff_buf_seek(void *a, int64_t offset) {
  ...
  if (offset < 0LL)
    offset = 0LL;
  else if (offset > (int64_t)(rs->buflen))
    offset = (int64_t)(rs->buflen);
...
int riff_stack_is_empty(riff_stack *s) { return (s->current == -1); }
...
riff_chunk *riff_stack_pop(riff_stack *s) {
  ...
  if (s->wmode) {
    ...
    if (c->placeholder) {
      c->placeholder = 0; /* caller pops a chunk off the stack to complete it,
                             therefore the placeholder must be rewritten */
      c->disable_sync =
          0; /* it MUST be rewritten, disabling sync is no longer an option */
      no_sync = 0; /* <--- ditto */
    }
    ...
    while (pc && c) {
      if ((c->absolute_data_offset + c->absolute_data_length) >
        ...
        pc->data_length =
            (uint32_t)((c->absolute_data_offset + c->absolute_data_length) -
    ...
    if (!no_sync)
      riff_stack_header_sync(s, s->top);
  }
...
int64_t riff_stack_current_chunk_offset(riff_stack *s) {
  ...
  if (s == NULL)
    return -1LL;
  if (s->top->read_offset > (s->top->data_length - 8LL))
    return -1LL;
...
int riff_stack_streamwrite(riff_stack *s, riff_chunk *c, const void *buf,
                           size_t len) {
  if (s->write == NULL)
    return -1;
  ...
  if (c) {
     ...
     *
     * NTS: We allow the caller to streamwrite NULL-length packets with buf=NULL
     * and len=0 */
    if (buf != NULL) {
      int rd;

      if (s->seek(s, c->absolute_data_offset) != c->absolute_data_offset)
        return 0;
...
int riff_stack_write(riff_stack *s, riff_chunk *c, const void *buf,
                     size_t len) {
  int rd;
  ...
  if (c) {
    ...
    if (s->seek(s, c->absolute_data_offset + c->write_offset) !=
        (c->absolute_data_offset + c->write_offset))
      return 0;
    rd = s->write(s, buf, len);
    if (rd > 0)
      c->read_offset = (c->write_offset += rd);
    ...
    if (s->seek(s, s->next_write) != s->next_write)
      return 0;
    rd = s->write(s, buf, len);
    if (rd > 0)
      s->next_read = (s->next_write += rd);
  }
...
int riff_stack_read(riff_stack *s, riff_chunk *c, void *buf, size_t len) {
  ...
  if (c) {
    ...
    if (c->absolute_data_offset == ((int64_t)(-1)))
      return 0;
    if (s->seek(s, c->absolute_data_offset + c->read_offset) !=
        (c->absolute_data_offset + c->read_offset))
      return 0;
    rd = s->read(s, buf, len);
    if (rd > 0)
      c->write_offset = (c->read_offset += rd);
  } else {
    if (s->seek(s, s->next_read) != s->next_read)
      return 0;
    rd = s->read(s, buf, len);
    if (rd > 0)
      s->next_write = (s->next_read += rd);
  }
 ...
 * +0x04 +4     4 / DWORD       data length (not including header but including
 * 4-byte subchunk header that follows) +0x08 +8     4 / DWORD       FOURCC
 * (4-byte ASCII identifying the subchunk) =0x0C +12    12
 *
 * A word of caution about top-level chunks: Most RIFF formats are based on the
 * top level containing one major top-level chunk, and most (all) chunks are
 * within that top level chunk. If the file extends past 2GB, then the file at
 * the top level is a series of very large top-level RIFF chunks. It is very
 * unlikely that you'll ever get a non-list chunk at top level, so if this
 * function returns one at toplevel, it's probably unrelated junk at the end of
 * the file.
 *
 * In most cases, your code should simply read the one chunk at the start of the
 * file and generally stay within the chunk, unless for example OpenDML AVI 2.0,
 * in which you must make sure you read the series of RIFF:AVI and RIFF:AVIX
 * chunks until you reach the end of the file or encounter any other chunk.
 * Again, this code will NOT prevent you from reading junk at the end of the
 * file as an AVI chunk!
 */
int riff_stack_readchunk(riff_stack *s, riff_chunk *pc, riff_chunk *c) {
  unsigned char buf[8];

  if (s == NULL)
    return 0; /* caller must provide (s) stack */
  if (c == NULL)
    return 0; /* caller must provide (c) chunk structure to fill in */
  if (s->wmode)
    return 0; /* no reading chunks while writing RIFF */

  if (pc != NULL) {
    /* if parent chunk provided, read new chunk from parent chunk */
    if (c->absolute_data_offset == ((int64_t)(-1)))
      return 0;
    if ((pc->read_offset + 8) > pc->data_length)
      return 0;
    if (s->seek(s, pc->absolute_data_offset + pc->read_offset) !=
        (pc->absolute_data_offset + pc->read_offset))
      return 0;
  ...
  c->data_length =
      __le_u32(buf + 4); /* <-- NOTE this is why AVI files have a 2GB limit */
  c->absolute_data_length =
      (c->data_length + 1ULL) & (~1ULL); /* <-- RIFF chunks are WORD aligned */
  c->absolute_offset_next_chunk =
  ...
  if (c->fourcc == 0UL) {
    if (pc)
      pc->read_offset = pc->write_offset = pc->data_length;
    else
      s->eof = 1;
    return 0;
  }

  /* some chunks like 'RIFF' or 'LIST' are actually 12 byte headers, with the
   * real FOURCC following the length, then subchunks follow in the data area.
   */
  if (c->fourcc == riff_RIFF || c->fourcc == riff_LIST) {
    ...
    if (c->data_length >= 4) {
      ...
      memset(buf, 0, 4);
      if ((int)s->read(s, buf, c->absolute_data_length) <
          (int)c->absolute_data_length)
        return 0;
      c->fourcc = __le_u32(buf + 0);
      c->absolute_data_offset += c->absolute_data_length;
  ...
  if (pc)
    pc->read_offset = pc->write_offset =
        c->absolute_offset_next_chunk - pc->absolute_data_offset;
  else
    s->next_read = s->next_write = c->absolute_offset_next_chunk;
  return 1;
}
...
int riff_stack_chunk_contains_subchunks(riff_chunk *c) {
  if (c == NULL)
    return 0;
  if (c->list_fourcc == riff_RIFF)
    return 1;
  if (c->list_fourcc == riff_LIST)
    return 1;
  return 0;
}
...
void riff_stack_debug_print(FILE *fp, int level, riff_chunk *chunk) {
  ...
  fprintf(
      fp, "hdr=%llu data=%llu len=%lu data-end=%llu",
...
void riff_stack_debug_chunk_dump(FILE *fp, riff_stack *riff,
                                 riff_chunk *chunk) {
...
int riff_stack_begin_new_chunk_here(riff_stack *s, riff_chunk *c) {
  ...
  /* sanity check.
   * if we're currently in a subchunk, make sure the subchunk allows for
   * subchunks within. you can't just put chunks inside data. */
  if (p != NULL) {
    if (!riff_stack_chunk_contains_subchunks(p)) {
      fprintf(
          stderr,
          "BUG: riff_stack_begin_new_chunk_here() caller attempting to start "
          "new RIFF chunks inside a chunk that does not contain subchunks\n");
      return 0;
    }
  }
...
int riff_stack_header_sync(riff_stack *s, riff_chunk *c) {
  ...
  /* NTS: A placeholder value of 0xFFFFFFFF would technically violate the AVI
   * standard because it is well known older programs treat the length as
   * signed. We use 0x7FFFFFFF instead. */
  if (c->list_fourcc != (riff_fourcc_t)0) {
    ...
    if (c->placeholder)
      ...
      __w_le_u32(tmp + 4,
                 c->data_length +
                     4UL); /* for some reason the length covers the fourcc */
...
int riff_stack_set_chunk_data_type(riff_chunk *c, riff_fourcc_t fcc) {
  ...
  if (c->write_offset != 0LL) {
    fprintf(stderr, "BUG: riff_stack_set_chunk_data_type() caller attempted to "
                    "set type after writing data!\n");
    return 0;
  }

  c->fourcc = fcc;
  c->list_fourcc = (riff_fourcc_t)0;
  c->absolute_data_offset =
      c->absolute_header_offset + 8LL; /* <fourcc> <len> */
  return 1;
}

/* start a list chunk (with subchunks) with list type (list) usually "RIFF" and
 * "LIST" and FOURCC (fcc). For example: RIFF:AVI would be list = "RIFF" fcc =
 * "AVI " */
int riff_stack_set_chunk_list_type(riff_chunk *c, riff_fourcc_t list,
                                   riff_fourcc_t fcc) {
  if (!c->wmode)
    return 0;
  if (c->write_offset != 0LL) {
    fprintf(stderr, "BUG: riff_stack_set_chunk_list_type() caller attempted to "
                    "set type after writing data!\n");
    return 0;
  }
  ...
  c->list_fourcc = list;
  c->absolute_data_offset =
      c->absolute_header_offset + 12LL; /* RIFF <len> <fourcc> */
  return 1;
}
...
/* if I wrote "len" bytes, would I hit or cross the AVI 2GB chunk limit at any
 * level? */
/* if so, the caller must pop down all AVI chunks, emptying the stack, and then
 * create new chunks to continue on */
int riff_stack_chunk_limit(riff_stack *s, int len) {
  ...
  for (; highest >= lowest; highest--) {
    signed long long rel =
        (signed long long)((unsigned long long)highest->absolute_data_offset -
                           base);
