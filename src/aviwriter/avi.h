...
#define avi_fourcc_const(a, b, c, d)                                           \
  ((((uint32_t)(a)) << 0U) | (((uint32_t)(b)) << 8U) |                         \
...
typedef struct { /* (sizeof) (offset hex) (offset dec) */
  /* NTS: The first two fields defined in Microsoft's SDK headers are not
   * repeated here, because they are effectively the AVI RIFF chunk header,
   * which most of our code does not include as part of the read (why did you do
   * that, Microsoft?) */
  /* FOURCC	fcc */
  /* uint32_t	cb */
...
/* AVIOLDINDEX chunk IDs. NOTE that this chunk ID makes the last two bytes of
 * dwChunkId (the upper 16 bits) */
...
#define riff_idx1_AVIOLDINDEX_chunkid_uncompressed_videoframe                  \
  avi_fourcc_const(0, 0, 'd', 'b')
#define riff_idx1_AVIOLDINDEX_chunkid_compressed_videoframe                    \
  avi_fourcc_const(0, 0, 'd', 'c')
#define riff_idx1_AVIOLDINDEX_chunkid_palette_change                           \
  avi_fourcc_const(0, 0, 'p', 'c')
#define riff_idx1_AVIOLDINDEX_chunkid_audio_data                               \
  avi_fourcc_const(0, 0, 'w', 'b')
...
/* AVIMETAINDEX (a meta-structure for all the variations in indx and nnix
 * chunks) */
typedef struct {
  /*	FOURCC          fcc;
          UINT            cb; */
...
/* AVI stream format contents if stream type is 'iavs' (Interleaved audio/video
 * stream) */
...
static const windows_DVINFO WINDOWS_DVINFO_INIT = {0, 0, 0, 0, 0, 0, {0, 0}};
