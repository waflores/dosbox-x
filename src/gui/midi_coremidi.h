...
public:
  ...
  bool Open(const char *conf) {
    ...
    LOG_MSG("coremidi: %u MIDI destinations available",
            (unsigned int)
                numDests); // FIXME: Why isn't LOG(..,..)(...) available here?
    if (conf && *conf) {
      ...
      if (configmidi.fail() && numDests) {
        ...
        for (Bitu i = 0; i < numDests; i++) {
          ...
          if (MIDIObjectGetStringProperty(dummy, kMIDIPropertyDisplayName,
                                          &midiname) == noErr) {
            const char *s =
                CFStringGetCStringPtr(midiname, kCFStringEncodingMacRoman);
            if (s) {
  ...
  void PlayMsg(uint8_t *msg) {
    ...
    MIDIPacketListAdd(packetList, (ByteCount)sizeof(packetBuf), m_pCurPacket,
                      (MIDITimeStamp)0, len, msg);
  ...
  void ListAll(Program *base) {
    ...
    for (Bitu i = 0; i < numDests; i++) {
      ...
      if (!dest)
        continue;
      CFStringRef midiname = 0;
      if (MIDIObjectGetStringProperty(dest, kMIDIPropertyDisplayName,
                                      &midiname) == noErr) {
        const char *s =
            CFStringGetCStringPtr(midiname, kCFStringEncodingMacRoman);
        if (s)
