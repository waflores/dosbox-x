...
static void synth_log(int level,
    ...
    FLUIDSYNTH_VERSION_MAJOR >= 2 // Let version 2.x be the default
                      const
#endif
                      char *message,
                      void *data) {
  ...
  if (synth_soft != NULL) {
    ...
    if (master_volume < 128) {
      for (unsigned int i = 0; i < (len * 2); i++) {
        ((int16_t *)MixTemp)[i] =
            (int16_t)(((((int16_t *)MixTemp)[i]) * master_volume) >> 7);
      }
    }
...
private:
  ...
  void PlayEvent(uint8_t *msg, Bitu len) {
    ...
    if (roland_gs_sysex) {
      if (msg[1] == 0x41 /*Roland*/ && msg[3] == 0x42 /*GS*/ &&
          msg[4] == 0x12 /*Send*/ && len >= 9) {
        const uint32_t addr = ((uint32_t)msg[5] << 16) +
                              ((uint32_t)msg[6] << 8) + (uint32_t)msg[7];

        if (addr == 0x400004) { /* MASTER VOLUME */
          /* Fluidsynth doesn't appear to support this message, so we have to
           * handle it ourself. */
          master_volume = msg[8];
    ...
    case 0xf0:
    case 0xf7:
      LOG(LOG_MISC, LOG_DEBUG)("SYNTH: sysex 0x%02x len %lu", (int)event,
                               (long unsigned)len);
      fluid_synth_sysex(synth_soft, (char *)(msg + 1), (int)(len - 1), NULL,
                        NULL, NULL, 0);
      return;
    case 0xf9:
...
public:
  ...
  const char *GetName(void) override { return "synth"; };
  ...
  bool Open(const char *conf) override {
    ...
    if (!conf || (conf[0] == '\0')) {
      ...
      if (FILE *file = fopen("/usr/share/soundfonts/default.sf2", "r")) {
        ...
        // Ubuntu and Debian don't have a default.sf2...
                     ...
                     fopen("/usr/share/sounds/sf2/FluidR3_GM.sf2", "r")) {
        fclose(file);
        sf = "/usr/share/sounds/sf2/FluidR3_GM.sf2";
      } else if (FILE *file =
                     fopen("/usr/share/sounds/sf2/GeneralUser_GS.sf2", "r")) {
        fclose(file);
    ...
    if (sfont_id == -1) {
      LOG(LOG_MISC, LOG_WARN)(
          "SYNTH: Failed to load MIDI sound font file \"%s\"", sf.c_str());
    ...
    master_volume = 128;
    synthchan = MIXER_AddChannel(synth_CallBack, (unsigned int)synthsamplerate,
                                 "SYNTH");
  ...
  void PlaySysex(uint8_t *sysex, Bitu len) override { PlayEvent(sysex, len); };
...
public:
  ...
  void PlayMsg(uint8_t *msg) override {
    ...
    default:
      LOG(LOG_MISC, LOG_WARN)("MIDI:fluidsynth: Unknown Command: %08lx",
                              (long)msg[0]);
      break;
    }
  }
  ...
  bool Open(const char *conf) override {
    ...
    if (!sf.size()) { // Let's try to find a soundfont before bailing
      ...
      if (FILE *file = fopen("/usr/share/soundfonts/default.sf2", "r")) {
        ...
        LOG_MSG("MIDI:fluidsynth: SoundFont not specified, and no system "
                "SoundFont found");
        return false;
      }
#endif
    ...
    if (strcmp(section->Get_string("fluid.driver"), "default") != 0) {
      fluid_settings_setstr(settings, "audio.driver",
                            section->Get_string("fluid.driver"));
    }
#if defined(__linux__) // Let's use pulseaudio as default on Linux, and not the
                       // FluidSynth default of Jack
    else {
      ...
      fluid_settings_setstr(settings, "audio.driver",
                            "dsound"); // Explicitly set audio driver to be
                                       // dsound as default for Windows
    }
#endif // WIN32

    fluid_settings_setnum(settings, "synth.sample-rate",
                          atof(section->Get_string("fluid.samplerate")));
    fluid_settings_setnum(settings, "synth.gain",
                          atof(section->Get_string("fluid.gain")));
    fluid_settings_setint(settings, "synth.polyphony",
                          section->Get_int("fluid.polyphony"));
    if (strcmp(section->Get_string("fluid.cores"), "default") != 0) {
      fluid_settings_setnum(settings, "synth.cpu-cores",
                            atof(section->Get_string("fluid.cores")));
    }

    std::string period = section->Get_string("fluid.periods"),
                periodsize = section->Get_string("fluid.periodsize");
    if (period == "default") {
#if defined(WIN32)
      period = "8";
#else
    ...
    fluid_settings_setint(settings, "audio.period-size",
                          atoi(periodsize.c_str()));
    fluid_settings_setint(
        settings, "synth.reverb.active",
        !strcmp(section->Get_string("fluid.reverb"), "yes") ? 1 : 0);
    fluid_settings_setint(
        settings, "synth.chorus.active",
        !strcmp(section->Get_string("fluid.chorus"), "yes") ? 1 : 0);
#else
    fluid_settings_setnum(settings, "audio.periods", atof(period.c_str()));
    fluid_settings_setnum(settings, "audio.period-size",
                          atof(periodsize.c_str()));
    fluid_settings_setstr(settings, "synth.reverb.active",
                          section->Get_string("fluid.reverb"));
    fluid_settings_setstr(settings, "synth.chorus.active",
                          section->Get_string("fluid.chorus"));
#endif
    ...
    fluid_synth_set_reverb(synth,
                           atof(section->Get_string("fluid.reverb.roomsize")),
                           atof(section->Get_string("fluid.reverb.damping")),
                           atof(section->Get_string("fluid.reverb.width")),
                           atof(section->Get_string("fluid.reverb.level")));

    fluid_synth_set_chorus(synth, section->Get_int("fluid.chorus.number"),
                           atof(section->Get_string("fluid.chorus.level")),
                           atof(section->Get_string("fluid.chorus.speed")),
                           atof(section->Get_string("fluid.chorus.depth")),
                           section->Get_int("fluid.chorus.type"));
