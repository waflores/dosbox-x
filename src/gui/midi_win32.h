...
private:
  ...
  void MidiHelper_Reset() {
    ...
    if (hMidiHelper) {
      midi_dll = true;
      while (FreeLibrary(hMidiHelper) != 0)
        Sleep(1);
    }
  }
  ...
  int MidiHelper_Start(DWORD devID) {
    ...
    func_ptr = (HMIDIOUT (*)(DWORD))GetProcAddress(hMidiHelper,
                                                   "MIDIHelper_OpenMidiOut");
    if (!func_ptr)
      return -1;
  ...
  void MidiHelper_End() {
    void (*func_ptr)(void);
    func_ptr =
        (void (*)(void))GetProcAddress(hMidiHelper, "MIDIHelper_CloseMidiOut");
    if (!func_ptr)
      return;
...
public:
    ...
    if (conf && *conf) {
      ...
      if (nummer < total) {
...
#if !defined(HX_DOS)
        /* If we're not talking to MIDI hardware, but the default Microsoft
           software synthesizer, and it supports volume control, we want to use
           it, because Microsoft's MIDI synth also does not recognize the Roland
           GS master volume SysEx either. */
        ...
        if (midi_dll == false || strcmp(mididev.szPname, "Roland VSC") != 0)
          res = midiOutOpen(&m_out, nummer, (DWORD_PTR)m_event, 0,
                            CALLBACK_EVENT);
        else {
...
#else
        res =
            midiOutOpen(&m_out, nummer, (DWORD_PTR)m_event, 0, CALLBACK_EVENT);
#endif

        if (res != MMSYSERR_NOERROR) {
          if (strcmp(mididev.szPname, "Roland VSC") == 0)
            MessageBox(0, "Roland VSC failed", "MIDI", MB_OK | MB_TOPMOST);

          if (nummer != 0) {
            LOG_MSG("MIDI:win32 selected %s", "default");
            res = midiOutOpen(&m_out, MIDI_MAPPER, (DWORD_PTR)m_event, 0,
                              CALLBACK_EVENT);
          }
        }
      }
  ...
  void PlayMsg(uint8_t *msg) {
    ...
    /* This must be done AFTER to prevent the notes left hanging from ever
     * becoming audible before reset */
