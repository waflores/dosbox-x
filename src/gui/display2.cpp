...
bool DISP2_Active(void) { return disp2_init; }

static void DISP2_WriteChar(uint16_t row, uint16_t col, uint16_t chat) {
  uint8_t ch = (uint8_t)(chat & 0xff), at = (uint8_t)(chat >> 8);
  if (GCC_UNLIKELY((at & 0x77) == 0))
    attrset(COLOR_PAIR(1) | A_INVIS);
  else if ((at & 0x77) == 0x70)
...
static void DISP2_Refresh(Bitu /*val*/) {
  ...
  if (disp2.cursor.update) {
    if (!disp2.cursor.enable || disp2.cursor.eline < disp2.cursor.sline) {
      ...
      if (disp2.cursor.eline > 10)
        map_ch = 0xdb; // full block
      else
        map_ch = 0xdf; // upper half block
    } else if (disp2.cursor.sline < 10) {
      if (disp2.cursor.eline > 12)
        map_ch = 0xdc; // lower half block
      else
        map_ch = 0x2d; // minus sign
    } else
      map_ch = 0x5f; // underscore
    disp2.cursor.update = false;
  }
  uint8_t new_ch = (++blink & 8) ? map_ch : 0;
  if ((old_ch) && old_pos < 2000 && (new_ch == 0 || new_pos != old_pos)) {
    DISP2_WriteChar(old_pos / 80, old_pos % 80,
                    host_readw(&disp2.memory[old_pos << 1]));
    need_refresh = true;
  }
  if ((new_ch) && new_pos < 2000 && (new_ch != old_ch || new_pos != old_pos)) {
    DISP2_WriteChar(new_pos / 80, new_pos % 80,
                    host_readw(&disp2.memory[new_pos << 1]) & 0xff00 | new_ch);
    need_refresh = true;
  }
  old_ch = new_ch;
  old_pos = new_pos;
  if (need_refresh) {
    refresh();
#ifdef WIN32
    if (dos.loaded_codepage && !(altcp && dos.loaded_codepage == altcp) &&
        !(customcp && dos.loaded_codepage == customcp))
      SetConsoleOutputCP(dos.loaded_codepage);
#endif
  }
...
public:
  DISP2_PageHandler() { flags = PFLAG_NOCODE; }
  // the 4kB map area is repeated in the 32kB range
  uint8_t readb(PhysPt addr) { return disp2.memory[addr & 0xfff]; }
...
void DISP2_SetPageHandler(void) {
  if (machine != MCH_HERC && machine != MCH_MDA && disp2_init)
    MEM_SetPageHandler(0xB0, 8, &disp2_page_handler);
}
...
static void DISP2_write_crtc_data(Bitu /*port*/, Bitu val, Bitu /*iolen*/) {
  ...
  case 0x0e: // cursor position high
    disp2.cursor.pos =
        (disp2.cursor.pos & 0x00ff) | (uint16_t)((val & 0x0f) << 8);
    break;
