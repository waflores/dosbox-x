...
static inline void conc4d_sub_func(const SRCTYPE *&src, SRCTYPE *&cache,
                                   PTYPE *&line0, const unsigned int block_proc,
                                   Bitu &hadChange) {
  ...
  if (memcmp(src, cache, block_proc * sizeof(SRCTYPE)) == 0
      ...
      && !(render.pal.modified[src[0]] | render.pal.modified[src[1]] |
           render.pal.modified[src[2]] | render.pal.modified[src[3]] |
           render.pal.modified[src[4]] | render.pal.modified[src[5]] |
           render.pal.modified[src[6]] | render.pal.modified[src[7]])
#endif
  ) {
    src += block_proc;
    cache += block_proc;
...
#endif
  {
    ...
    BituMove(((uint8_t *)line0) - copyLen + render.scale.outPitch, WC[0],
             copyLen);
#endif
#if (SCALERHEIGHT > 2)
    BituMove(((uint8_t *)line0) - copyLen + render.scale.outPitch * 2, WC[1],
             copyLen);
#endif
#if (SCALERHEIGHT > 3)
    BituMove(((uint8_t *)line0) - copyLen + render.scale.outPitch * 3, WC[2],
             copyLen);
#endif
#if (SCALERHEIGHT > 4)
    BituMove(((uint8_t *)line0) - copyLen + render.scale.outPitch * 4, WC[3],
             copyLen);
#endif
#if (SCALERHEIGHT > 5)
    BituMove(((uint8_t *)line0) - copyLen + render.scale.outPitch * 5, WC[4],
             copyLen);
#endif
#endif // defined(SCALERLINEAR)
  }
...
#else
  // larger blocks encourage memcmp() to optimize, scaler loop to process before
  // coming back to check again.
