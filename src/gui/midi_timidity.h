...
public:
  MidiHandler_timidity()
      : MidiHandler(), _isOpen(false), _device_num(0), _controlbuffer_count(0),
        _controlbuffer_size(0) {
    if (!SDLNetInited) {
      if (SDLNet_Init() == -1) {
...
private:
  ...
  /* send command to the server; printf-like; returns http status and reply
   * string (by reference). buff is expected to have at least BUFSIZ size */
...
private:
  ...
  /* buffer for partial data read from _control_fd - from timidity-io.c, see
   * fdgets() */
...
bool MidiHandler_timidity::Open(const char *conf) {
  ...
  if (conf && conf[0])
    safe_strncpy(timidity_host, conf, 512);
  else
    safe_strncpy(timidity_host, DEFAULT_TIMIDITY_HOST, 512);
   ...
   */
  if ((connect_to_server(timidity_host, timidity_port, &control_socket)) ==
      false) {
    LOG_MSG("TiMidity: can't open control connection (host=%s, port=%d)",
  ...
  if (status != 220) {
    LOG_MSG("TiMidity: bad response from server (host=%s, port=%d): %s",
            timidity_host, timidity_port, res);
    close_all();
    return false;
  }
  ...
  /* should read: "200 OK" */
  status =
      timidity_ctl_command(res, "SETBUF %f %f", BUF_LOW_SYNC, BUF_HIGH_SYNC);
  if (status != 200)
  ...
  if (connect_to_server(timidity_host, data_port, &data_socket) == false) {
    LOG_MSG("TiMidity: can't open data connection (host=%s, port=%d)",
            timidity_host, data_port);
    close_all();
    return false;
  }
...
void MidiHandler_timidity::Close() { teardown(); }
...
void MidiHandler_timidity::timidity_write_data(const void *buf, int nbytes) {
  if (SDLNet_TCP_Send(data_socket, buf, nbytes) < nbytes) {
    LOG_MSG("TiMidity: DATA WRITE FAILED (%s), DISABLING MUSIC OUTPUT",
            SDLNet_GetError());
    close_all();
  }
}
