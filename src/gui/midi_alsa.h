...
public:
  ...
  void PlayMsg(uint8_t *msg) override {
    ...
    default:
      ...
      case 0xFF:
        ...
        break;
      default:
        LOG(LOG_MISC, LOG_DEBUG)("ALSA:Unknown Command: %02X %02X %02X", msg[0],
                                 msg[1], msg[2]);
        send_event(1);
        break;
      }
    ...
    if (snd_seq_client_info_malloc(&sscit) >= 0) {
      if ((status = snd_seq_get_any_client_info(seq_handle, 0, sscit)) >= 0) {
        do {
          ...
          LOG_MSG("ALSA seq enum: id=%d name=\"%s\" ports=%d type=\"%s\"", id,
                  name, ports, ct_str);
  ...
  bool Open(const char *conf) override {
    ...
    // Not many people know how to get the magic numbers needed for this ALSA
    // output to send MIDI to a hardware device (hint: modprobe snd-seq-midi and
    // then aconnect -l) so to assist users, enumerate all ALSA sequencer
    // clients (these are client numbers) and list them in the log file. In the
    // same way NE2000 emulation lists all pcap interfaces for your reference.
    ...
    my_port = snd_seq_create_simple_port(seq_handle, "DOSBOX-X", caps,
    ...
    if (seq_client != SND_SEQ_ADDRESS_SUBSCRIBERS) {
      ...
      if (snd_seq_connect_to(seq_handle, my_port, seq_client, seq_port) < 0) {
        if (defaultport) { // if port "65:0" (default) try "17:0" as well
          ...
          if (snd_seq_connect_to(seq_handle, my_port, seq_client, seq_port) <
              0) { // Try 128:0 Timidity port as well
            //						seq_client = 128;
            //seq_port = 0; //Update reported values
            //						if(snd_seq_connect_to(seq_handle,my_port,seq_client,seq_port)
            //< 0) {
          ...
          snd_seq_close(seq_handle);
          LOG(LOG_MISC, LOG_WARN)("ALSA:Can't subscribe to MIDI port (%d:%d)",
                                  seq_client, seq_port);
          return false;
        }
      }
    ...
    LOG(LOG_MISC, LOG_DEBUG)("ALSA:Client initialised [%d:%d]", seq_client,
                             seq_port);
    return true;
  }
  ...
  void ListAll(Program *base) override{
      ...
      auto print_port = [base, this](snd_seq_client_info_t *client_info,
                                     snd_seq_port_info_t *port_info) {
...
#else
      auto print_port =
          [base, this](auto *client_info, auto *port_info) {
            const auto *addr = snd_seq_port_info_get_addr(port_info);
            const unsigned int type = snd_seq_port_info_get_type(port_info);
        ...
        if ((type & SND_SEQ_PORT_TYPE_SYNTHESIZER) || port_is_writable(caps)) {
          ...
          base->WriteOut("%c %s%3d:%d - %s - %s%s\n", selected ? '*' : ' ',
                         selected ? esc_color : "", addr->client, addr->port,
