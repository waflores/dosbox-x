...
void DOSBox_SetSysMenu(void), GFX_OpenGLRedrawScreen(void),
    InitFontHandle(void), DOSV_FillScreen(void), refreshExtChar(void),
    Add_VFiles(bool usecp), SetAlpha(double alpha),
    SetWindowTransparency(int trans);
void MenuBrowseProgramFile(void), OutputSettingMenuUpdate(void),
    aspect_ratio_menu(void), update_pc98_clock_pit_menu(void),
    AllocCallback1(void), AllocCallback2(void), ToggleMenu(bool pressed);
...
#define IS_OLDMACOS                                                            \
  1 /* FIX_ME: Tested on El Capitan (10.11). If this macro is required for     \
       Sierra (10.12), change to 101300 */
#endif
...
#include "display2.cpp"
    ...
    (!defined IS_OLDMACOS && (defined BSD || defined LINUX))
...
#else
#define PATH_MAX                                                               \
  4096 /* LINUX sets to 4096, while this varies from 260 to 4096 depending on  \
          platforms */
#endif
#endif
...
BOOL CALLBACK EnumDispProc(HMONITOR hMon, HDC dcMon, RECT *pRcMon,
  ...
  curscreen++;
  if (sdl.displayNumber == curscreen)
    monrect = *pRcMon;
  return TRUE;
}
#endif
...
static const GUID __my_IID_ITaskbarList3 = {
    0xEA1AFB91ul, 0x9E28u, 0x4B86u, 0x90u, 0xE9u, 0x9Eu,
    0x9Fu,        0x8Au,   0x5Eu,   0xEFu, 0xAFu};
...
void MAPPER_CheckEvent(SDL_Event *event), MAPPER_CheckKeyboardLayout(),
...
bool CodePageHostToGuestUTF16(char *d /*CROSS_LEN*/,
...
extern bool CodePageHostToGuestUTF8(char *d /*CROSS_LEN*/,
...
typedef HRESULT(WINAPI *PFNSETWINDOWATTRIBUTE)(HWND hWnd, DWORD dwAttribute,
                                               LPCVOID pvAttribute,
                                               DWORD cbAttribute);
...
bool UpdateWindows11RoundCorners(HWND hWnd, CornerPreference cornerPreference) {
  ...
  if (hDwmApi) {
    ...
    if (pfnSetWindowAttribute) {
      auto preference =
          static_cast<DWM_WINDOW_CORNER_PREFERENCE>(cornerPreference);
      HRESULT res = pfnSetWindowAttribute(hWnd, DWMWA_WINDOW_CORNER_PREFERENCE,
                                          &preference,
...
bool HostDarkMode() {
  // Uh, hey, Microsoft. Have you ever thought of providing a damn API function
  // to just tell me whether Dark Mode or Light Mode is active? It could be a
  // BOOL or an enumeration or something. Then you wouldn't have to support
  // random GitHub projects that have to import unnamed ordinals from
  // UXTHEME.DLL just to figure it out. While you're at it, why not provide a
  // way for a Win32 program like myself to say "Hey, I want everything in this
  // program to support Dark Mode, instead of having to hack every single window
  // to try to get it to render Dark Mode".
  return false;
}

bool AllowDarkMode(HWND hwnd, BOOL enable) {
  {
    // NTS: Contrary to first impressions, DWMWA_USE_IMMERSIVE_DARK_MODE does
    // not match your title bar to the system
    //      theme. No, that would make sense. Instead,
    //      DWMWA_USE_IMMERSIVE_DARK_MODE makes your title bar dark AT ALL
    //      TIMES. To know if you should set this attribute, you have to detect
    //      Dark Mode. And Microsoft doesn't provide a damn API to detect API.
    //      They do document some roundabout way involving WinRT to read a color
    //      and compute whether it is dark or light and therefore Dark Mode, but
    //      there's no "Is Dark Mode" active API nor any kind of enumeration on
    //      what mode is active.
    //
    //      So, we can enable dark mode for the title bar, but if we're going to
    //      enable it to match the system, we have to determine if it is active,
    //      and that's not available right now because it's too complicated to
    //      figure it out. So, no dark mode right now.
    HMODULE hDwmApi = ::LoadLibrary("dwmapi.dll");
    if (hDwmApi) {
      auto *pfnSetWindowAttribute = reinterpret_cast<PFNSETWINDOWATTRIBUTE>(
          GetProcAddress(hDwmApi, "DwmSetWindowAttribute"));
      if (pfnSetWindowAttribute) {
        // FIXME: Why doesn't this affect the menu bar and dropdown menus from
        // it? What's the point of a dark title bar when there's this bright
        // white menu below it?
        BOOL val = enable && HostDarkMode();
        HRESULT res = pfnSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE,
                                            &val, sizeof(val));
        return res == S_OK;
      }
      ::FreeLibrary(hDwmApi);
...
std::string GetDOSBoxXPath(bool withexe = false) {
  std::string full;
#if defined(HX_DOS) || defined(_WIN32_WINDOWS) /*wai_* is problematic */ ||    \
...
bool isVirtualBox =
    false; /* OpenGL never works with Windows XP inside VirtualBox */
#endif
...
void WindowsTaskbarUpdatePreviewRegion(void) {
  if (winTaskbarList != NULL) {
    /* Windows 7/8/10: Tell the taskbar which part of our window contains the
     * DOS screen */
    RECT r;
    ...
    /* NTS: The MSDN documentation is misleading. Apparently, despite 30+ years
       of Windows SDK behavior where the "client area" is the area below the
       menu bar and inside the frame, ITaskbarList3's idea of the "client area"
       is the area inside the frame INCLUDING the menu bar. Why? */
    if (GetMenu(GetHWND()) != NULL) {
      ...
      GetMenuBarInfo(GetHWND(), OBJID_MENU, 0,
                     &mb); // returns absolute screen coordinates, apparently.
      rh = mb.rcBar.bottom + 1 -
           mb.rcBar.top; // menu screen space is top <= y <= bottom, inclusive.
...
void WindowsTaskbarResetPreviewRegion(void) {
  if (winTaskbarList != NULL) {
    /* Windows 7/8/10: Tell the taskbar which part of our window contains the
     * client area (not including the menu bar) */
    RECT r;
...
void KeyboardLayoutDetect(void) {
  ...
  if (nlayout == DKM_JPN) {
    LOG_MSG(
        "Engaging Linux/X11 fix for jp xkbmap in order to handle Ro/Yen keys");
  ...
  if (IS_PC98_ARCH) {
    ...
    const char *layoutstr =
        pc98_section->Get_string("pc-98 force ibm keyboard layout");
    if (!strcasecmp(layoutstr, "auto")) {
      pc98_force_ibm_layout = host_keyboard_layout == DKM_US;
      mainMenu.get_item("pc98_use_uskb")
          .check(pc98_force_ibm_layout)
...
void SetMapperKeyboardLayout(const unsigned int dkm) {
  /* TODO: Make mapper re-initialize layout. If the mapper interface is visible,
   * redraw it. */
...
void UpdateWindowMaximized(bool flag) { menu.maxwindow = flag; }
...
void PrintScreenSizeInfo(void) {
  ...
  case METHOD_XRANDR:
    method = "XRandR";
    break;
  case METHOD_WIN98BASE:
    method = "Win98base";
    break;
  case METHOD_COREGRAPHICS:
    method = "CoreGraphics";
    break;
  default:
    break;
  ...
  if (dim_width != screen_size_info.screen_dimensions_pixels.width ||
    ...
    LOG_MSG("Screen report: Method '%s' (%.3f x %.3f pixels) at (%.3f x %.3f) "
            "(%.3f x %.3f mm) (%.3f x %.3f in) (%.3f x %.3f DPI)",
            method,
...
void Windows_GetWindowDPI(ScreenSizeInfo &info) {
  ...
  /* Windows 10 build 1607 and later offer a "Get DPI of window" function */
  {
    ...
    if (__user32) {
      ...
      __GetDpiForWindow =
          (UINT(WINAPI *)(HWND))GetProcAddress(__user32, "GetDpiForWindow");
      if (__GetDpiForWindow) {
        ...
        if (dpi != 0) {
          ...
          info.screen_dimensions_mm.width =
              (25.4 * screen_size_info.screen_dimensions_pixels.width) / dpi;
          info.screen_dimensions_mm.height =
              (25.4 * screen_size_info.screen_dimensions_pixels.height) / dpi;
        }
      }
    }
  }
...
#endif
#if defined(WIN32)
  // When maximized, SDL won't actually tell us our new dimensions, so get it
  // ourselves.
  // FIXME: Instead of GetHWND() we need to track our own handle or add
  // something to SDL 1.x
...
int mouse_start_x = -1, mouse_start_y = -1, mouse_end_x = -1, mouse_end_y = -1,
    fx = -1, fy = -1, paste_speed = 20, wheel_key = 0, mbutton = 3;
...
void SDL_rect_cliptoscreen(SDL_Rect &r) {
  ...
  /* NTS: Apparently r.w and r.h are unsigned, therefore no need to check if
   * negative */
  //    if (r.w < 0) r.w = 0;
  //    if (r.h < 0) r.h = 0;
}

Bitu GUI_JoystickCount(void) { return sdl.num_joysticks; }
...
static void DOSBox_SetOriginalIcon(void) {
  ...
  logos = SDL_CreateRGBSurfaceFrom((void *)logo, 32, 32, 32, 128, 0xff000000,
                                   0x00ff0000, 0x0000ff00, 0);
#else
  logos = SDL_CreateRGBSurfaceFrom((void *)logo, 32, 32, 32, 128, 0x000000ff,
...
#if defined(WIN32)
bool GFX_SDLUsingWinDIB(void) { return sdl.using_windib; }
#endif
...
void GFX_SetIcon(void) {
  ...
  /* Made into a separate call, so it can be called again when we restart the
   * graphics output on win32 */
  if (menu_compatible) {
  ...
  hIcon1 = (HICON)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(dosbox_ico),
...
void GFX_SetTitle(int32_t cycles, int frameskip, Bits timing, bool paused) {
  ...
  Section_prop *section =
      static_cast<Section_prop *>(control->GetSection("SDL"));
  assert(section != NULL);
  ...
  if (showbasic) {
    sprintf(title, "%s%sDOSBox-X %s", dosbox_title.c_str(),
            dosbox_title.empty() ? "" : " - ", VERSION);
    ...
    sprintf(title, "%s%sDOSBox-X", dosbox_title.c_str(),
            dosbox_title.empty() ? "" : " - ");
  ...
  if (sdl.mouse.locked) {
    ...
    strcat(title, key.size() ? (" [" + key + " releases mouse]").c_str()
                             : " [mouse locked]");
  }
...
#endif
bool CodePageGuestToHostUTF16(uint16_t *d /*CROSS_LEN*/,
                              const char *s /*CROSS_LEN*/);
...
bool systemmessagebox(char const *aTitle, char const *aMessage,
  ...
  char *lMessage = (char *)malloc(
      (aMessageLength * 2 + 1) *
      sizeof(char)); // DBCS may expand to 3 to 4 bytes when converted to UTF-8
  char *lTitle = (char *)malloc(
      (aTitleLength * 2 + 1) *
      sizeof(char)); // DBCS may expand to 3 to 4 bytes when converted to UTF-8
  lDialogString =
      replaceNewlineWithEscaped(lDialogString); // String may include "\n" which
                                                // needs to be escaped to "\\n"
  CodePageGuestToHostUTF8(lMessage, lDialogString.c_str());
  lTitleString =
      replaceNewlineWithEscaped(lTitleString); // String may include "\n" which
                                               // needs to be escaped to "\\n"
  CodePageGuestToHostUTF8(lTitle, lTitleString.c_str());
  bool result = tinyfd_messageBox(lTitle, lMessage, aDialogType, aIconType,
                                  aDefaultButton);
  free(lMessage);
  free(lTitle);
#else
  ...
  int result = tinyfd_messageBoxW(
      (wchar_t const *)utf16Title, (wchar_t const *)utf16Message,
      (wchar_t const *)utf16Type, (wchar_t const *)utf16Icon, aDefaultButton);
...
bool CheckQuit(void) {
  ...
  if (warn == "true") {
    if (!quit) {
      systemmessagebox("Quit DOSBox-X warning", MSG_Get("QUIT_DISABLED"), "ok",
                       "warning", 1);
      return false;
    } else
      return systemmessagebox("Quit DOSBox-X warning", MSG_Get("QUIT_CONFIRM"),
                              "yesno", "question", 1);
  } else if (warn == "false")
    return true;
  if (dos_kernel_disabled && strcmp(RunningProgram, "DOSBOX-X")) {
    if (!quit) {
      systemmessagebox("Quit DOSBox-X warning", MSG_Get("QUIT_GUEST_DISABLED"),
                       "ok", "warning", 1);
      return false;
    } else
      return systemmessagebox("Quit DOSBox-X warning",
                              MSG_Get("QUIT_GUEST_CONFIRM"), "yesno",
                              "question", 1);
  }
  if (warn == "autofile")
    for (uint8_t handle = 0; handle < DOS_FILES; handle++) {
      if (Files[handle] &&
          (Files[handle]->GetName() == NULL ||
           strcmp(Files[handle]->GetName(), "CON")) &&
          (Files[handle]->GetInformation() & DeviceInfoFlags::Device) == 0) {
        if (!quit) {
          systemmessagebox("Quit DOSBox-X warning",
                           MSG_Get("QUIT_FILE_OPEN_DISABLED"), "ok", "warning",
                           1);
          return false;
        } else
          return systemmessagebox("Quit DOSBox-X warning",
                                  MSG_Get("QUIT_FILE_OPEN_CONFIRM"), "yesno",
                                  "question", 1);
      }
    }
  else if (RunningProgram && strcmp(RunningProgram, "DOSBOX-X") &&
           strcmp(RunningProgram, "COMMAND") &&
           strcmp(RunningProgram, "4DOS")) {
    if (!quit) {
      systemmessagebox("Quit DOSBox-X warning",
                       MSG_Get("QUIT_PROGRAM_DISABLED"), "ok", "warning", 1);
      return false;
    } else
      return systemmessagebox("Quit DOSBox-X warning",
                              MSG_Get("QUIT_PROGRAM_CONFIRM"), "yesno",
                              "question", 1);
  }
#endif
  return true;
...
static void KillSwitch(bool pressed) {
  ...
  if (sdl.desktop.fullscreen)
    GFX_SwitchFullScreen();
#if 0 /* Re-enable this hack IF DOSBox-X continues to have problems            \
         page-faulting on kill switch */
...
void DoKillSwitch(void) { KillSwitch(true); }
...
void GFX_SDL_Overscan(void) {
  ...
  if (sdl.overscan_width) {
    Bitu border_color =
        GFX_GetRGB(vga.dac.rgb[vga.attr.overscan_color].red << 2,
                   vga.dac.rgb[vga.attr.overscan_color].green << 2,
                   vga.dac.rgb[vga.attr.overscan_color].blue << 2);
    if (border_color != sdl.overscan_color) {
      ...
      rect->x = 0;
      rect->y = 0;
      rect->w = sdl.draw.width + (unsigned int)(2 * sdl.clip.x);
      rect->h = (uint16_t)sdl.clip.y; // top
      if ((Bitu)rect->h > (Bitu)sdl.overscan_width) {
        rect->y += (int)(rect->h - sdl.overscan_width);
        rect->h = (uint16_t)sdl.overscan_width;
      }
      if ((Bitu)sdl.clip.x > (Bitu)sdl.overscan_width) {
        rect->x += (int)sdl.clip.x - (int)sdl.overscan_width;
        rect->w -= (uint16_t)(2 * ((int)sdl.clip.x - (int)sdl.overscan_width));
      }
      rect = &sdl.updateRects[1];
      rect->x = 0;
      rect->y = sdl.clip.y;
      rect->w = (uint16_t)sdl.clip.x;
      rect->h = (uint16_t)sdl.draw.height; // left
      if ((unsigned int)rect->w > (unsigned int)sdl.overscan_width) {
        rect->x += (int)rect->w - (int)sdl.overscan_width;
        rect->w = (uint16_t)sdl.overscan_width;
      }
      rect = &sdl.updateRects[2];
      rect->x = (int)sdl.clip.x + (int)sdl.draw.width;
      rect->y = sdl.clip.y;
      rect->w = (uint16_t)sdl.clip.x;
      rect->h = (uint16_t)sdl.draw.height; // right
      if ((unsigned int)rect->w > (unsigned int)sdl.overscan_width) {
        rect->w = (uint16_t)sdl.overscan_width;
      }
      rect = &sdl.updateRects[3];
      rect->x = 0;
      rect->y = (int)sdl.clip.y + (int)sdl.draw.height;
      rect->w = sdl.draw.width + (unsigned int)(2 * sdl.clip.x);
      rect->h = (uint16_t)sdl.clip.y; // bottom
      if ((Bitu)rect->h > (Bitu)sdl.overscan_width) {
        rect->h = (uint16_t)sdl.overscan_width;
      }
      if ((Bitu)sdl.clip.x > (Bitu)sdl.overscan_width) {
        rect->x += (int)sdl.clip.x - (int)sdl.overscan_width;
        rect->w -=
            (unsigned int)(2 * ((int)sdl.clip.x - (int)sdl.overscan_width));
      }

      if (sdl.surface->format->BitsPerPixel ==
          8) { // SDL_FillRect seems to have some issues with palettized hw
               // surfaces
        uint8_t *pixelptr = (uint8_t *)sdl.surface->pixels;
        Bitu linepitch = sdl.surface->pitch;
        for (Bitu i = 0; i < 4; i++) {
          rect = &sdl.updateRects[i];
          uint8_t *start = pixelptr +
                           (unsigned int)rect->y * (unsigned int)linepitch +
                           (unsigned int)rect->x;
...
bool DOSBox_Paused() { return emu_paused; }
...
void HideMenu_mapper_shortcut(bool pressed) {
  ...
  mainMenu.get_item("mapper_togmenu")
      .check(!menu.toggle)
      .refresh_item(mainMenu);
#if defined(USE_TTF) && DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  int last = 0;
  while (TTF_using() && !sdl.desktop.fullscreen && menu_gui && menu.toggle &&
         menuwidth_atleast(ttf.cols * ttf.width + ttf.offX * 2 +
                           GetSystemMetrics(SM_CXBORDER) * 2) > 0 &&
         ttf.pointsize > last) {
    last = ttf.pointsize;
    increaseFontSize();
  }
...
void PauseWithInterruptsEnabled(Bitu /*val*/) {
  ...
  while (pausewithinterrupts_enable)
    CALLBACK_Idle();
}
...
void PauseWithInterrupts_mapper_shortcut(bool pressed) {
  ...
  mainMenu.get_item("mapper_pauseints")
      .check(pausewithinterrupts_enable)
      .refresh_item(mainMenu);
}
...
void PauseDOSBoxLoop(Bitu /*unused*/) {
  ...
  while (paused) {
    ...
    if (event.type == SDL_SYSWMEVENT && msg == WM_COMMAND &&
        (LOWORD(wparam) == ID_WIN_SYSMENU_PAUSE ||
         LOWORD(wparam) == (mainMenu.get_item("mapper_pause").get_master_id() +
                            DOSBoxMenu::winMenuMinimumID))) {
      paused = false;
      GFX_SetTitle(-1, -1, -1, false);
      break;
    }
    if (event.type == SDL_SYSWMEVENT && msg == WM_SYSCOMMAND &&
        LOWORD(wparam) == ID_WIN_SYSMENU_PAUSE) {
      paused = false;
      GFX_SetTitle(-1, -1, -1, false);
      break;
    ...
    case SDL_KEYDOWN: // Must use Pause/Break or escape Key to resume.
      if (event.key.keysym.sym == SDLK_PAUSE ||
        ...
        break;
                 ...
                 SDLK_SPACE) { /* spacebar = single frame step */
    ...
    case SDL_MOUSEMOTION:
      ...
      if (touchscreen_finger_lock == no_finger_id &&
          ...
          event.motion.which !=
              SDL_TOUCH_MOUSEID) { /* don't handle mouse events faked by
                                      touchscreen */
...
int GetDisplayNumber(void) { return sdl.displayNumber; }

void SetDisplayNumber(int display) { sdl.displayNumber = display; }
...
SDL_Window *GFX_SetSDLWindowMode(uint16_t width, uint16_t height,
  ...
  if (!sdl.window || (lastType != screenType)
      ...
      //          || (glwindow != (0 != (SDL_GetWindowFlags(sdl.window) &
      //          SDL_WINDOW_OPENGL)))
      //          || (fullscreen && (0 == (SDL_GetWindowFlags(sdl.window) &
      //          SDL_WINDOW_FULLSCREEN)))
      //          || (fullscreen != (SDL_WINDOW_FULLSCREEN ==
      //          (SDL_GetWindowFlags(sdl.window) & SDL_WINDOW_FULLSCREEN)))
      //          || (fullscreen && ((width != currWidth) || (height !=
      //          currHeight)))
  ) {
    lastType = screenType;
    if (sdl.window) {
    ...
    sdl.window = SDL_CreateWindow(
        ...
        SDL_WINDOWPOS_UNDEFINED_DISPLAY(
            sdl.displayNumber ? sdl.displayNumber - 1 : 0),
        SDL_WINDOWPOS_UNDEFINED_DISPLAY(
            sdl.displayNumber ? sdl.displayNumber - 1 : 0),
        width, height,
        (GFX_IsFullscreen()
             ? (sdl.desktop.full.display_res ? SDL_WINDOW_FULLSCREEN_DESKTOP
                                             : SDL_WINDOW_FULLSCREEN)
             : 0) |
            ((screenType == SCREEN_OPENGL) ? SDL_WINDOW_OPENGL : 0) |
            (maximize && !TTF_using() ? SDL_WINDOW_MAXIMIZED : 0) |
    ...
    if (screenType == SCREEN_OPENGL) {
      ...
      if (sdl_opengl.context == NULL)
        LOG_MSG("WARNING: SDL2 unable to create GL context");
...
void GFX_SetResizeable(bool enable) {
  if (SDL2_resize_enable != enable) {
    ...
    if (sdl.window != NULL)
      SDL_SetWindowResizable(sdl.window,
                             SDL2_resize_enable ? SDL_TRUE : SDL_FALSE);
  }
}
...
Bitu GFX_GetBestMode(Bitu flags) {
  ...
  if (!retFlags) {
    ...
    if (retFlags == 0)
      LOG_MSG("SDL: Failed everything including falling back to surface "
              "GFX_GetBestMode"); // completely failed it seems
  }
...
/* FIXME: This prepares the SDL library to accept Win32 drag+drop events from
 * the Windows shell. So it should be named something like
 * EnableDragAcceptFiles() not SDL_Prepare() */
...
#if !defined(C_SDL2)
  LOG(LOG_MISC, LOG_DEBUG)("Win32: Preparing main window to accept files "
                           "dragged in from the Windows shell");
...
unsigned int GFX_GetBShift() { return sdl.surface->format->Bshift; }

void GFX_LogSDLState(void) {
  LOG(LOG_MISC, LOG_DEBUG)(
      "SDL video mode: %ux%u (clip %ux%u with upper-left at %ux%u) %ubpp",
      (unsigned)sdl.surface->w, (unsigned)sdl.surface->h, (unsigned)sdl.clip.w,
      (unsigned)sdl.clip.h, (unsigned)sdl.clip.x, (unsigned)sdl.clip.y,
...
void GFX_DrawSDLMenu(DOSBoxMenu &menu, DOSBoxMenu::displaylist &dl) {
  ...
  if (!DOSBox_isMenuVisible() || sdl.desktop.fullscreen) {
    // BUGFIX: If the menu is hidden then silently clear "needs redraw" to avoid
    // excess redraw of nothing
    menu.clearRedraw();
    return;
  }
  ...
  if (&dl == &menu.display_list) { /* top level menu, draw background */
    MenuDrawRect(menu.menuBox.x, menu.menuBox.y, menu.menuBox.w,
                 menu.menuBox.h - 1, GFX_GetRGB(63, 63, 63));
    MenuDrawRect(menu.menuBox.x, menu.menuBox.y + menu.menuBox.h - 1,
                 menu.menuBox.w, 1, GFX_GetRGB(31, 31, 31));
  }
...
Bitu GFX_SetSize(Bitu width, Bitu height, Bitu flags, double scalex,
  ...
  if ((width == 0 || height == 0) && !TTF_using()) {
    E_Exit("GFX_SetSize with width=%d height=%d zero dimensions not allowed",
  ...
  LOG(LOG_MISC, LOG_DEBUG)("GFX_SetSize %ux%u flags=0x%x scale=%.3fx%.3f",
                           ...
                           (unsigned int)flags, scalex, scaley);
  ...
  if (!retFlags) {
    ...
    if (retFlags == 0)
      LOG_MSG("SDL: Failed everything including falling back to surface in "
              "GFX_GetSize"); // completely failed it seems
  }

#if C_GAMELINK
  if (!OUTPUT_GAMELINK_InitTrackingMode() &&
  ...
  if (diff && posx < 0 && posy < 0 && !(posx == -2 && posy == -2)) {
    if (sdl.displayNumber == 0)
      ...
      if (SDL_GetDesktopDisplayMode(
              sdl.displayNumber ? sdl.displayNumber - 1 : 0, &dm) == 0) {
  ...
  if ((!sdl.desktop.fullscreen && menu_gui && menu.toggle &&
       ((width == 640 || (vga.draw.char9_set && width == 720)) &&
        ((machine != MCH_CGA && !IS_VGA_ARCH && !IS_PC98_ARCH &&
          height == 350) ||
         height == 400))) ||
      ((render.aspect || IS_DOSV) && checkmenuwidth)) {
    RECT r;
    bool res = GetWindowRect(GetHWND(), &r);
    unsigned int maxWidth, maxHeight;
    GetMaxWidthHeight(&maxWidth, &maxHeight);
    int tWidth = menuwidth_atleast(r.right - r.left);
    if (tWidth > 0 && tWidth > maxWidth)
      tWidth = maxWidth;
    int tHeight = r.bottom - r.top - height < 0
                      ? (r.bottom - r.top)
                      : (double)(tWidth - ((r.right - r.left - width) < 0
                                               ? 0
                                               : (r.right - r.left - width))) *
                                height / width +
                            (r.bottom - r.top - height);
    if (tHeight > 0 && tHeight > maxHeight)
      tHeight = maxHeight;
...
#if defined(WIN32) && !defined(HX_DOS) && !defined(_WIN32_WINDOWS)
// WARNING: Not recommended, there is danger you cannot exit emulator because
// mouse+keyboard are taken
static bool enable_hook_everything = false;
#endif

// Whether or not to hook the keyboard and block special keys.
// Setting this is recommended so that your keyboard is fully usable in the
// guest OS when you enable the mouse+keyboard capture. But hooking EVERYTHING
// is not recommended because there is a danger you become trapped in the
// DOSBox-X emulator!
static bool enable_hook_special_keys = true;

#if defined(WIN32) && !defined(HX_DOS) && !defined(_WIN32_WINDOWS)
// Whether or not to hook Num/Scroll/Caps lock in order to give the guest OS
// full control of the LEDs on the keyboard (i.e. the LEDs do not change until
// the guest OS changes their state). This flag also enables code to set the
// LEDs to guest state when setting mouse+keyboard capture, and restoring LED
// state when releasing capture.
...
static LRESULT CALLBACK WinExtHookKeyboardHookProc(int nCode, WPARAM wParam,
                                                   LPARAM lParam) {
  if (nCode == HC_ACTION) {
    HWND myHwnd = GetHWND();

    if (exthook_enabled &&
        GetFocus() == myHwnd) { /* intercept only if DOSBox-X is the focus and
                                   the keyboard is hooked */
      if (wParam == WM_SYSKEYDOWN || wParam == WM_KEYDOWN ||
          wParam == WM_SYSKEYUP || wParam == WM_KEYUP) {
        KBDLLHOOKSTRUCT *st_hook = (KBDLLHOOKSTRUCT *)lParam;

        if (st_hook->flags & LLKHF_INJECTED) {
          // injected keys are automatically allowed, especially if we are
          // injecting keyboard input into ourself to control Num/Scroll/Caps
          // Lock LEDs. If we don't check this we cannot control the LEDs.
          // Injecting keydown/keyup for Num Lock is the only means provided by
          // Windows to control those LEDs.
        } else if (st_hook->vkCode == VK_MENU /*alt*/ ||
                   st_hook->vkCode == VK_CONTROL ||
                   st_hook->vkCode == VK_LSHIFT ||
                   st_hook->vkCode == VK_RSHIFT) {
          // always allow modifier keys through, so other applications are not
          // left with state inconsistent from actual keyboard state.
        } else {
          bool nopass =
              enable_hook_everything; // if the user wants us to hook ALL keys
                                      // then that's where this signals it
          bool alternate_message =
              false; // send as WM_USER+0x100 instead of WM_KEYDOWN

          if (!nopass) {
            // hook only certain keys Windows is likely to act on by itself.

            // FIXME: Hooking the keyboard does NOT prevent Fn+SPACE (zoom) from
            // triggering screen resolution
            //        changes in Windows 10! How do we stop that?

            // FIXME: It might be nice to let the user decide whether or not
            // Print Screen is intercepted.

            // TODO: We do not hook the volume up/down/mute keys. This is to be
            // kind to the user. They may appreciate the ability to dial down
            // the volume if a loud DOS program comes up. But if the user WANTS
            // us to, we should allow hooking those keys.

            // TODO: Allow (if instructed) hooking the VK_SLEEP key so pushing
            // the sleep key (the one with the icon of the moon on Microsoft
            // keyboards) can be sent instead to the guest OS. Also add code
            // where if we're not hooking the key, then we should listen for
            // signals the guest OS is suspending or hibernating and
            // auto-disconnect the mouse capture and keyboard hook.
            ...
            case VK_APPS:  // Application key (normally open to the user, but
                           // just in case)
            case VK_PAUSE: // pause key
            case VK_SNAPSHOT: // print screen
            case VK_TAB: // try to catch ALT+TAB too (not blocking VK_TAB will
                         // allow host OS to switch tasks)
            case VK_ESCAPE: // try to catch CTRL+ESC as well (so Windows 95
                            // Start Menu is accessible)
            case VK_SPACE:  // and space (catching VK_ZOOM isn't enough to
                            // prevent Windows 10 from changing res)
            // these keys have no meaning to DOSBox-X and so we hook them by
            // default to allow the guest OS to use them
            ...
            case VK_ZOOM: // Zoom key (the (+) magnifying glass keyboard
                          // shortcut laptops have these days on the spacebar?)
              nopass = true;
              break;
            ...
            case 0xF2:
              ...
              // we allow hooking Num/Scroll/Caps Lock keys so that pressing
              // them does not toggle the LED. we then take Num/Scroll/Caps LED
              // state from the guest and let THAT control the LED state.
            case VK_CAPITAL:
            case VK_NUMLOCK:
            case VK_SCROLL:
          ...
          if (nopass) {
            // convert WM_KEYDOWN/WM_KEYUP if obfuscating the message to
            // distinguish between real and injected events
            ...
            DWORD lParam =
                ...
                ((wParam == WM_KEYUP || wParam == WM_SYSKEYUP) ? 0xC0000000
                                                               : 0);
...
// Microsoft doesn't have an outright "set toggle key state" call, they expect
// you to know the state and then fake input to toggle. Blegh. Fine.
void WinSetKeyToggleState(unsigned int vkCode, bool state) {
  ...
  inps.ki.wVk = vkCode;
  inps.ki.dwFlags =
      KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP; // release, use wVk.
  SendInput(1, &inps, sizeof(INPUT));
}
#endif
...
#if defined(WIN32) && !defined(C_SDL2) &&                                      \
    !defined(HX_DOS)     /* Microsoft Windows */
  if (exthook_enabled) { // ONLY if ext hook is enabled, else we risk infinite
                         // loops with keyboard events
...
void DoExtendedKeyboardHook(bool enable) {
  ...
  if (enable) {
    if (!exthook_winhook) {
      exthook_winhook = SetWindowsHookEx(
          WH_KEYBOARD_LL, WinExtHookKeyboardHookProc, GetModuleHandle(NULL), 0);
      if (exthook_winhook == NULL)
        return;
    }
    ...
    // if hooking Num/Scroll/Caps Lock then record the toggle state of those
    // keys. then read from the keyboard emulation the LED state set by the
    // guest and apply it to the host keyboard.
    if (enable_hook_lock_toggle_keys) {
      // record state
      on_capture_num_lock_was_on = (GetKeyState(VK_NUMLOCK) & 1) ? true : false;
      on_capture_scroll_lock_was_on =
          (GetKeyState(VK_SCROLL) & 1) ? true : false;
      on_capture_caps_lock_was_on =
          (GetKeyState(VK_CAPITAL) & 1) ? true : false;
    ...
    if (exthook_winhook) {
...

      {
        ...
        // before we disable the SDL hack make sure we flush out and handle any
        // pending keyboard events. if we don't do this the posted
        // Num/Scroll/Caps events will stay in the queue and will be handled by
        // SDL after turning off the toggle key hack.
...
void GFX_CaptureMouse(void) { GFX_CaptureMouse(!sdl.mouse.locked); }
...
void GFX_CaptureMouse(bool capture) {
  ...
  if (sdl.mouse.locked) {
...
#endif
    if (enable_hook_special_keys)
      DoExtendedKeyboardHook(true);
    ...
    if (vmware_mouse)
      SDL_ShowCursor(SDL_DISABLE);
    else if (sdl.mouse.autoenable || !sdl.mouse.autolock)
  ...
  if (sdl.mouse.locked) {
    void GFX_SDLMenuTrackHover(DOSBoxMenu & menu,
                               DOSBoxMenu::item_handle_t item_id);
    void GFX_SDLMenuTrackHilight(DOSBoxMenu & menu,
                                 DOSBoxMenu::item_handle_t item_id);
  ...
  if (mainMenu.item_exists("mapper_capmouse"))
    mainMenu.get_item("mapper_capmouse")
        .check(sdl.mouse.locked)
        .refresh_item(mainMenu);
}
...
void CaptureMouseNotify() { CaptureMouseNotify(sdl.mouse.locked); }
...
void GetDesktopResolution(int *width, int *height) {
  ...
  *height = rDdesk.bottom - rDdesk.top;
  ...
  Linux_GetDesktopResolution(width,
                             height); /* this is MESSY but there's too much
                                         namespace collision going on here */
#else
  *width = 1024; // guess
  *height = 768;
#endif
...
void res_init(void) {
  ...
  if (sdl.desktop.type == SCREEN_SURFACE && !sdl.desktop.fullscreen)
    return;
  else {
    GFX_Stop();
    if (sdl.draw.callback)
...
void modeSwitched(bool full) {
  ...
  // ensure mouse capture when fullscreen || (re-)capture if user said so when
  // windowed
  auto locked = sdl.mouse.locked;
  if ((full && !locked) || (!full && locked))
    GFX_CaptureMouse();
}
...
void GFX_SwitchFullScreen(void) {
#if defined(USE_TTF)
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  if (ttf.fullScrn && ttf.inUse && !control->opt_nomenu &&
      static_cast<Section_prop *>(control->GetSection("sdl"))
          ->Get_bool("showmenu")) {
    DOSBox_SetMenu();
    lastmenu = true;
  ...
  if (ttf.inUse) {
    if (ttf.fullScrn) {
      sdl.desktop.fullscreen = false;
      // if (lastfontsize>0)
      //     OUTPUT_TTF_Select(lastfontsize); /* certain lastfontsize will crash
      //     DOSBox-X */
      // else
      OUTPUT_TTF_Select(1);
      resetFontSize();
      ...
      resetFontSize();
      if (lastfontsize < 1)
        PIC_AddEvent(ResetTTFSize, 0.001);
      lastfontsize = 0;
    } else {
    ...
    bool is_showmenu = static_cast<Section_prop *>(control->GetSection("SDL"))
                           ->Get_bool("showmenu");
    if (sdl.desktop.fullscreen || !is_showmenu)
      DOSBox_NoMenu();
    else
      DOSBox_SetMenu();
    return;
  ...
  if (!sdl.desktop.fullscreen) { /*is GOING fullscreen*/
    ...
    if (screen_size_info.screen_dimensions_pixels.width != 0 &&
        screen_size_info.screen_dimensions_pixels.height != 0) {
      if (sdl.desktop.full.width_auto)
        sdl.desktop.full.width =
            (unsigned int)screen_size_info.screen_dimensions_pixels.width;
      if (sdl.desktop.full.height_auto)
        sdl.desktop.full.height =
            (unsigned int)screen_size_info.screen_dimensions_pixels.height;

#if !defined(C_SDL2) && defined(MACOSX)
      /* macOS has this annoying problem with their API where the System
         Preferences app, display settings panel allows setting 720p and
         1080i/1080p modes on monitors who's native resolution is less than
         1920x1080 but supports 1920x1080.

         This is a problem with HDTV sets made in the late 2000s early 2010s
         when "HD" apparently meant LCD displays with 1368x768 resolution that
         will nonetheless accept 1080i (and later, 1080p) and downscale on
         display.

         The problem is that with these monitors, macOS's display API will NOT
         list 1920x1080 as one of the display modes even when the freaking
         desktop is obviously set to 1920x1080 on that monitor!

         If the screen reporting code says 1920x1080 and we try to go
         fullscreen, SDL1 will intervene and say "hey wait there's no 1920x1080
         in the mode list" and then fail the call.

         So to work around this, we have to go check SDL's mode list before
         using the screen size returned by the API.

         Oh and for extra fun, macOS is one of those modern systems where
         "setting the video mode" apparently means NOT setting the video
         hardware mode but just changing how the GPU scales the display...
         EXCEPT when talking to these older displays where if the user set it to
         1080i/1080p, our request to set 1368x768 actually DOES change the video
         mode. This is just wonderful considering the old HDTV set I bought back
         in 2008 takes 1-2 seconds to adjust to the mode change on it's HDMI
         input.

         Frankly I wish I knew how to fix the SDL1 Quartz code to use whatever
         abracadabra magic code is required to enumerate these extra HDTV modes
         so this hack isn't necessary, except that experience says this hack is
         always necessary because it might not always work.

         This magic hidden super secret API bullshit is annoying. You're worse
         than Microsoft with this stuff, Apple --J.C. */
      {
      ...
      SDL_hax_SetFSWindowPosition(
          (int)screen_size_info.screen_position_pixels.x,
          (int)screen_size_info.screen_position_pixels.y,
          (int)screen_size_info.screen_dimensions_pixels.width,
          (int)screen_size_info.screen_dimensions_pixels.height);
#endif
    } else {
#if !defined(C_SDL2) && defined(SDL_DOSBOX_X_SPECIAL)
  ...
  // TODO this will need further changes to accommodate different outputs (e.g.
  // stretched)
  if (full) {
    int width, height;
    GetDesktopResolution(&width, &height);
    ...
    if ((unsigned int)width < width1 || (unsigned int)height < height1) {
      sdl.desktop.fullscreen = false;
      LOG_MSG("WARNING: full screen canceled, surface size (%ix%i) exceeds "
              "screen size (%ix%i).",
  ...
  if (menu.startup) // NOTE should be always true I suppose ???
  {
    ...
    if (vsyncProp) {
      ...
      if (!strcmp(vsyncMode, "host"))
        SetVal("vsync", "vsyncmode", "host");
    }
  }
#endif
#endif
...
bool GFX_GetPreventFullscreen(void) { return sdl.desktop.prevent_fullscreen; }
...
void GFX_RestoreMode(void) {
  ...
  GFX_SetSize(sdl.draw.width, sdl.draw.height, sdl.draw.flags, sdl.draw.scalex,
              sdl.draw.scaley, sdl.draw.callback);
  GFX_UpdateSDLCaptureState();
  GFX_ResetScreen();
}
...
unsigned char *GFX_GetSurfacePtr(size_t *pitch, unsigned int x,
                                 unsigned int y) {
...
#endif
      if (((sdl.desktop.type != SCREEN_OPENGL) || !RENDER_GetForceUpdate()) &&
          !sdl.updating)
    return;
#if C_OPENGL
...
void GFX_SetPalette(Bitu start, Bitu count, GFX_PalEntry *entries) {
  ...
  if (sdl.surface->flags & SDL_HWPALETTE) {
    if (!SDL_SetPalette(sdl.surface, SDL_PHYSPAL, (SDL_Color *)entries,
                        (int)start, (int)count)) {
      E_Exit("SDL: Cannot set palette");
    }
  } else {
    if (!SDL_SetPalette(sdl.surface, SDL_LOGPAL, (SDL_Color *)entries,
                        (int)start, (int)count)) {
      E_Exit("SDL: Cannot set palette");
    }
  }
...
Bitu GFX_GetRGB(uint8_t red, uint8_t green, uint8_t blue) {
  ...
  case SCREEN_OPENGL:
    ...
    !defined(C_SDL2) /* macOS Intel builds use a weird RGBA order (alpha in    \
                        the low 8 bits) */
    // USE BGRA
    return (((unsigned long)blue << 24ul) | ((unsigned long)green << 16ul) |
            ((unsigned long)red << 8ul)) |
           (255ul << 0ul);
#else
    // USE ARGB
    return (((unsigned long)blue << 0ul) | ((unsigned long)green << 8ul) |
            ((unsigned long)red << 16ul)) |
           (255ul << 24ul);
#endif
#endif
...
static void SetPriority(PRIORITY_LEVELS level) {
  ...
  if ((sdl.priority.focus != sdl.priority.nofocus) && (getuid() != 0))
    return;
...
static void OutputString(Bitu x, Bitu y, const char *text, uint32_t color,
                         uint32_t color2, SDL_Surface *output_surface) {
  uint32_t *draw = (uint32_t *)(((uint8_t *)output_surface->pixels) +
                                (y * output_surface->pitch)) +
                   x;
  while (*text) {
    ...
    for (i = 0; i < 14; i++) {
      ...
      for (j = 0; j < 8; j++) {
        if (map & 0x80)
          *((uint32_t *)(draw_line + j)) = color;
        else
          *((uint32_t *)(draw_line + j)) = color2;
        map <<= 1;
      }
...
void Sendkeymapper(bool pressed) {
  if (!pressed)
    return;
  if (sendkeymap == 1)
    SendKey("sendkey_winlogo");
  else if (sendkeymap == 2)
    SendKey("sendkey_winmenu");
  else if (sendkeymap == 3)
    SendKey("sendkey_alttab");
  else if (sendkeymap == 4)
    SendKey("sendkey_ctrlesc");
  else if (sendkeymap == 5)
    SendKey("sendkey_ctrlbreak");
  else
    SendKey("sendkey_cad");
}
...
static void GUI_StartUp() {
...
#if defined(C_SDL2)
  /* while we're here, SDL 2.0.5 has some issues with Linux/X11, encourage the
   * user to update SDL2. */
  {
    SDL_version v;
    SDL_GetVersion(&v);
    ...
    if (v.major == 2 && v.minor == 0 && v.patch == 5)
      LOG_MSG("WARNING: Your SDL2 library is known to have some issues with "
              "Linux/X11, please update your SDL2 library");
#endif
  }
#endif
...
#if defined(C_SDL2)
  {
    ...
    if (SDL_GetDesktopDisplayMode(0 /*FIXME display index*/, &dm) == 0) {
      ...
      LOG_MSG("SDL2 unable to determine desktop display mode, error %s",
              SDL_GetError());
    }
  }
#endif
...
#ifdef WIN32
  /* NTS: This should not print any warning whatsoever because Windows builds by
   * default will use the Windows API to disable DPI scaling of the main window,
   * unless the user modifies the setting through dosbox-x.conf or the command
   * line. */
  /* NTS: macOS has high DPI scaling too, though Apple is wise to enable it by
   * default only for Macbooks with "Retina" displays. On macOS, unless
   * otherwise wanted by the user, it is wise to let macOS scale up the DOSBox-X
   * window by 2x so that the DOS prompt is not a teeny tiny window on the
   * screen. */
  ...
  if (vidinfo) {
    ...
    if (sdl_w != win_w && sdl_h != win_h)
      LOG_MSG("Windows DPI/blurry apps scaling detected as it might be a large "
              "screen.\n"
  ...
  if (!sdl.desktop.full.width || !sdl.desktop.full.height) {
    ...
    // On windows don't use it as SDL returns the values without taking in
    // account the dpi scaling
    const SDL_VideoInfo *vidinfo = SDL_GetVideoInfo();
    if (vidinfo) {
      if (sdl.desktop.full.width == 0) {
        sdl.desktop.full.width_auto = true;
  ...
  if (!strcmp(clip_mouse_button, "middle"))
    mbutton = 2;
  else if (!strcmp(clip_mouse_button, "right"))
    mbutton = 3;
  else if (!strcmp(clip_mouse_button, "arrows"))
    mbutton = 4;
  else
    mbutton = 0;
  modifier = section->Get_string("clip_key_modifier");
  const char *pastebios = section->Get_string("clip_paste_bios");
  if (!strcasecmp(pastebios, "true") || !strcmp(pastebios, "1"))
    clipboard_biospaste = true;
  else if (!strcasecmp(pastebios, "false") || !strcmp(pastebios, "0"))
    clipboard_biospaste = false;
  ...
  MAPPER_AddHandler(
      ResetSystem, MK_r, MMODHOST, "reset", "Reset DOSBox-X",
      &item); /* Host+R (Host+CTRL+R acts funny on my Linux system) */
  item->set_text("Reset virtual machine");

  MAPPER_AddHandler(RebootGuest, MK_b, MMODHOST, "reboot", "Reboot DOS system",
                    &item); /* Reboot guest system or integrated DOS */
  item->set_text("Reboot guest system");

#if !defined(HX_DOS)
  MAPPER_AddHandler(LoadMapFile, MK_nothing, 0, "loadmap", "Load mapper file",
                    &item);
  item->set_text("Load mapper file...");

  MAPPER_AddHandler(QuickLaunch, MK_q, MMODHOST, "quickrun",
                    "Quick launch program", &item);
  item->set_text("Quick launch program...");
#endif

#if defined(MACOSX) && 0
  MAPPER_AddHandler(NewInstanceEvent, MK_nothing, 0, "newinst",
                    "Start new instance", &item);
  ...
  MAPPER_AddHandler(
      KillSwitch, MK_f9, MMOD1, "shutdown", "Quit from DOSBox-X",
      &item); /* KEEP: Most DOSBox-X users may have muscle memory for this */
  item->set_text("Quit from DOSBox-X");
#endif

  if (sdl.desktop.want_type != SCREEN_GAMELINK) {
    MAPPER_AddHandler(
        CaptureMouse, MK_f10, MMOD1, "capmouse", "Capture mouse",
        &item); /* KEEP: Most DOSBox-X users may have muscle memory for this */
    item->set_text("Capture mouse");
  }

#if defined(C_SDL2) || defined(WIN32) || defined(MACOSX)
  MAPPER_AddHandler(QuickEdit, MK_nothing, 0, "fastedit", "Quick edit mode",
                    &item);
  item->set_text("Quick edit: copy on select and paste text");

  MAPPER_AddHandler(CopyAllClipboard, MK_f5, MMOD1, "copyall",
                    "Copy to clipboard", &item);
  item->set_text("Copy all text on the DOS screen");
#endif

  MAPPER_AddHandler(PasteClipboard, MK_f6, MMOD1, "paste",
                    "Paste from clipboard",
                    &item); // end emendelson; improved by Wengier
  item->set_text("Pasting from the clipboard");

  MAPPER_AddHandler(PasteClipStop, MK_nothing, 0, "pasteend",
                    "Stop clipboard paste", &item);
  item->set_text("Stop clipboard pasting");

#if C_PRINTER
  MAPPER_AddHandler(&FormFeed, MK_f2, MMOD1, "ejectpage", "Send form-feed",
                    &item);
  item->set_text("Send form-feed");

  MAPPER_AddHandler(&PrintText, MK_nothing, 0, "printtext", "Print text screen",
                    &item);
  item->set_text("Print DOS text screen");
#endif

  MAPPER_AddHandler(&PauseDOSBox, MK_pause, MMODHOST, "pause",
                    "Pause emulation");

  MAPPER_AddHandler(&PauseWithInterrupts_mapper_shortcut, MK_nothing, 0,
                    "pauseints", "Pause with interrupt", &item);
  item->set_text("Pause with interrupts enabled");

#if DOSBOXMENU_TYPE == DOSBOXMENU_NSMENU
  pause_menu_item_tag = mainMenu.get_item("mapper_pause").get_master_id() +
                        DOSBoxMenu::nsMenuMinimumID;
#endif

  if (sdl.desktop.want_type != SCREEN_GAMELINK) {
    MAPPER_AddHandler(&GUI_Run, MK_c, MMODHOST, "gui", "Configuration tool",
                      &item);
    item->set_text("Configuration tool");

    MAPPER_AddHandler(&MAPPER_Run, MK_m, MMODHOST, "mapper", "Mapper editor",
                      &item);
    item->set_accelerator(DOSBoxMenu::accelerator('m'));
    item->set_description("Bring up the mapper UI");
    item->set_text("Mapper editor");

    MAPPER_AddHandler(SwitchFullScreen, MK_f, MMODHOST, "fullscr",
                      "Toggle fullscreen", &item);
    item->set_text("Toggle fullscreen");

    MAPPER_AddHandler(&GUI_ResetResize, MK_backspace, MMODHOST, "resetsize",
                      "Reset window size", &item);
    item->set_text("Reset window size");
  }

#if defined(USE_TTF)
  void DBCSSBCS_mapper_shortcut(bool pressed);
  MAPPER_AddHandler(&DBCSSBCS_mapper_shortcut, MK_nothing, 0, "dbcssbcs",
                    "CJK: Switch between DBCS/SBCS modes", &item);
  item->set_text("CJK: Switch between DBCS/SBCS modes");

  void AutoBoxDraw_mapper_shortcut(bool pressed);
  MAPPER_AddHandler(&AutoBoxDraw_mapper_shortcut, MK_nothing, 0, "autoboxdraw",
                    "CJK: Auto-detect box-drawing characters", &item);
  item->set_text("CJK: Auto-detect box-drawing characters");

  MAPPER_AddHandler(&TTF_IncreaseSize, MK_uparrow, MMODHOST, "incsize",
                    "Increase TTF size", &item);
  item->set_text("Increase TTF font size");

  MAPPER_AddHandler(&TTF_DecreaseSize, MK_downarrow, MMODHOST, "decsize",
                    "Decrease TTF size", &item);
  item->set_text("Decrease TTF font size");

  void ResetColors_mapper_shortcut(bool pressed);
  MAPPER_AddHandler(&ResetColors_mapper_shortcut, MK_nothing, 0, "resetcolor",
                    "Reset color scheme", &item);
  item->set_text("Reset TTF color scheme");
#endif

  void GEN_PowerButton(bool pressed);
  MAPPER_AddHandler(&GEN_PowerButton, MK_nothing, 0, "pwrbutton",
                    "APM power button", &item);
  item->set_text("APM power button");

  MAPPER_AddHandler(&HideMenu_mapper_shortcut, MK_escape, MMODHOST, "togmenu",
  ...
  if (display >= 0 && display <= numscreen)
    ...
    LOG_MSG("SDL: Display number should be between 0 and %d, fallback to "
            "default display",
            numscreen);
    sdl.displayNumber = 0;
  }
  ...
  if (sdl_xbrz.enable) {
    // xBRZ requirements
    if ((output != "surface") && (output != "direct3d") &&
        (output != "opengl") && (output != "openglhq") &&
        (output != "openglnb") && (output != "openglpp") && (output != "ttf") &&
        (output != "gamelink"))
      output = "surface";
  }
#endif
  ...
  // "overlay" and "ddraw" were removed, pre-map to Direct3D or OpenGL or
  // surface
  if (output == "overlay" || output == "ddraw"
    ...
    if (output == "overlay" || output == "ddraw")
      ...
      LOG_MSG("The %s output is not enabled.",
              output == "ttf" ? "TrueType font (TTF)" : output.c_str());
#if C_DIRECT3D
    output = "direct3d";
#elif C_OPENGL
    output = "opengl";
  ...
  if (output == "surface") {
...
#elif C_OPENGL &&                                                              \
    defined(MACOSX) // JC: This is breaking SDL1 on Linux! Limit this to MAC OS!
                    // Windows users may use this if needed.
    if (!init_output)
      OUTPUT_OPENGL_Select(GLBilinear); // Initialize screen before switching to
                                        // TTF (required for macOS builds)
#endif
    OUTPUT_SURFACE_Select();
    init_output = true;
#if C_OPENGL
    ...
    if (!init_output)
      OUTPUT_OPENGL_Select(
          GLBilinear); // Initialize screen before switching to TTF (required
    ...
    LOG_MSG("SDL: Unsupported output device %s, switching back to surface",
            output.c_str());
#if MACOSX && C_OPENGL
    if (!init_output)
      OUTPUT_OPENGL_Select(GLBilinear); // Initialize screen before switching to
                                        // surface (required for macOS builds)
#endif
  ...
  // Setting SDL1 window position before a call to SDL_SetVideoMode() is made.
  // If the user provided SDL_VIDEO_WINDOW_POS environment variable then
  // "windowposition" setting should have no effect. SDL2 position is set later,
  ...
  if (posx >= 0 && posy >= 0 && SDL_getenv("SDL_VIDEO_WINDOW_POS") == NULL) {
    ...
    safe_strcat(pos,
                (std::to_string(posx) + "," + std::to_string(posy)).c_str());
    SDL_putenv(pos);
#else
    setenv("SDL_VIDEO_WINDOW_POS",
           (std::to_string(posx) + "," + std::to_string(posy)).c_str(), 0);
#endif
#if defined(WIN32) && !defined(HX_DOS) && !defined(_WIN32_WINDOWS)
  } else if (sdl.displayNumber > 0 && !(posx == -2 && posy == -2)) {
    safe_strncpy(pos, "SDL_VIDEO_WINDOW_POS=", sizeof(pos));
    safe_strcat(pos, (std::to_string(info.rcMonitor.left + 200) + "," +
                      std::to_string(info.rcMonitor.top + 200))
                         .c_str());
    SDL_putenv(pos);
#endif
  } else if (posx != -2 || posy != -2)
...
/* Initialize screen for first time */
  ...
  LOG_MSG("SDL: Current window pixel format: %s",
          SDL_GetPixelFormatName(sdl.desktop.pixelFormat));
  ...
  /* Please leave the Splash screen stuff in working order in DOSBox-X. We spend
   * a lot of time making DOSBox-X. */
  // ShowSplashScreen();   /* I will keep the splash screen alive. But now, the
  // BIOS will do it --J.C. */

#if defined(WIN32) && !defined(HX_DOS)
  if (section->Get_bool("forcesquarecorner") &&
      UpdateWindows11RoundCorners(GetHWND(), CornerPreference::DoNotRound))
...
bool Mouse_IsLocked() { return sdl.mouse.locked; }
...
void GFX_HandleVideoResize(int width, int height) {
  ...
  if (!menu.maxwindow && !sdl.desktop.fullscreen && !sdl.init_ignore &&
      NonUserResizeCounter == 0 && !window_was_maximized) {
    UpdateWindowDimensions();
    ...
    if (window_was_maximized && !menu.maxwindow) {
      userResizeWindowWidth = currentWindowWidth == sdl.surface->w
                                  ? 0
                                  : (unsigned int)currentWindowWidth;
      userResizeWindowHeight = currentWindowHeight == sdl.surface->h
                                   ? 0
  ...
  if (screen_size_info.screen_dimensions_pixels.width != 0 &&
      screen_size_info.screen_dimensions_pixels.height != 0) {
    sdl.desktop.full.width =
        (uint16_t)screen_size_info.screen_dimensions_pixels.width;
    sdl.desktop.full.height =
        (uint16_t)screen_size_info.screen_dimensions_pixels.height;
  } else {
    SDL_DisplayMode dm;
    if (SDL_GetDesktopDisplayMode(0 /*FIXME display index*/, &dm) == 0) {
      sdl.desktop.full.width = dm.w;
...
static void HandleVideoResize(void *event) {
  ...
  if (GFX_GetPreventFullscreen()) {
#if C_OPENGL
    if (new_width == ResizeEvent->w && new_height == ResizeEvent->h)
      return;
  ...
  if (!menu.maxwindow && !sdl.desktop.fullscreen && !sdl.init_ignore &&
      NonUserResizeCounter == 0 && !window_was_maximized) {
    UpdateWindowDimensions();
    UpdateWindowDimensions((unsigned int)ResizeEvent->w,
                           (unsigned int)ResizeEvent->h);
...
static void HandleMouseMotion(SDL_MouseMotionEvent *motion) {
  ...
  /* limit mouse input to whenever the cursor is on the screen, or near the edge
   * of the screen. */
  if (is_paused)
    inputToScreen = false;
    ...
    if (mouse_start_x >= 0 && mouse_start_y >= 0) {
      if (fx >= 0 && fy >= 0)
        Mouse_Select(mouse_start_x - sdl.clip.x, mouse_start_y - sdl.clip.y,
                     fx - sdl.clip.x, fy - sdl.clip.y, sdl.clip.w, sdl.clip.h,
                     false);
      Mouse_Select(mouse_start_x - sdl.clip.x, mouse_start_y - sdl.clip.y,
                   motion->x - sdl.clip.x, motion->y - sdl.clip.y, sdl.clip.w,
                   sdl.clip.h, true);
      fx = motion->x;
      fy = motion->y;
    }
  ...
  if (GFX_GetPreventFullscreen()) {
     ...
     */
  } else if (!sdl.mouse.locked && !sdl.desktop.fullscreen &&
             mainMenu.isVisible() && motion->y < mainMenu.menuBox.h &&
             Mouse_GetButtonState() == 0) {
    skipdraw = true;
    GFX_SDLMenuTrackHover(mainMenu, mainMenu.display_list.itemFromPoint(
                                        mainMenu, motion->x, motion->y));
    skipdraw = false;
    SDL_ShowCursor(SDL_ENABLE);
  ...
  if (!inputToScreen) {
    ...
    if (!sdl.mouse.locked)
#else
    /* SDL1 has some sort of weird mouse warping bug in fullscreen mode no
     * matter whether the mouse is captured or not (Windows, Linux/X11) */
  ...
  auto xrel =
      static_cast<float>(motion->xrel) * sdl.mouse.xsensitivity / 100.0f;
  auto yrel =
      static_cast<float>(motion->yrel) * sdl.mouse.ysensitivity / 100.0f;
  auto x = static_cast<float>(motion->x - sdl.clip.x) / (sdl.clip.w - 1) *
           sdl.mouse.xsensitivity / 100.0f;
  auto y = static_cast<float>(motion->y - sdl.clip.y) / (sdl.clip.h - 1) *
           sdl.mouse.ysensitivity / 100.0f;
...
void MenuSaveScreen(void) {
  if (!OpenGL_using()) {
    if (menuSavedScreen.bmp == NULL) {
      ...
      menuSavedScreen.bmp =
          new unsigned char[menuSavedScreen.height * menuSavedScreen.stride];
    }

    if (SDL_MUSTLOCK(sdl.surface))
      SDL_LockSurface(sdl.surface);

    memcpy(menuSavedScreen.bmp, sdl.surface->pixels,
           menuSavedScreen.height * menuSavedScreen.stride);
...
void MenuRestoreScreen(void) {
  if (!OpenGL_using()) {
    ...
    memcpy(sdl.surface->pixels, menuSavedScreen.bmp,
           menuSavedScreen.height * menuSavedScreen.stride);
...
static void HandleMouseButton(SDL_MouseButtonEvent *button,
  ...
  if (GFX_GetPreventFullscreen()) {
    ...
    mainMenu.menuUserHoverAt = DOSBoxMenu::unassigned_item_handle;
             ...
             mainMenu.isVisible() && button->y < mainMenu.menuBox.h) {
    GFX_SDLMenuTrackHover(mainMenu, mainMenu.display_list.itemFromPoint(
                                        mainMenu, button->x, button->y));
    inMenu = true;
  } else {
    GFX_SDLMenuTrackHover(mainMenu, DOSBoxMenu::unassigned_item_handle);
  }
      ...
      if (mainMenu.menuUserHoverAt != DOSBoxMenu::unassigned_item_handle) {
...
#if C_DIRECT3D
        if (sdl.desktop.want_type == SCREEN_DIRECT3D) {
          /* In output=direct3d mode, SDL still has a surface but this code
           * ignores SDL and draws directly to a Direct3D9 backbuffer which is
           * presented to the window client area. However, GDI output to the
           * window still works, and this code uses the SDL surface still.
           * Therefore, for menus to draw correctly atop the Direct3D output,
           * this code copies the Direct3D backbuffer to the SDL surface first.
           *
           * WARNING: This happens to work with Windows (even Windows 10 build
           * 18xx as of 2018/05/21) because Windows appears to permit mixing
           * Direct3D and GDI rendering to the window.
           *
           * Someday, if Microsoft should break that ability, this code will
           * need to be revised to send screen "updates" to the Direct3D
           * backbuffer first, then Present to the window client area. */
          if (d3d)
            d3d->UpdateRectToSDLSurface(0, 0, sdl.surface->w, sdl.surface->h);
        }
#endif

        if (OpenGL_using()) {
#if C_OPENGL
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .setHilight(mainMenu, false);
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .setHover(mainMenu, false);

          /* show the menu */
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .setHilight(mainMenu, true);
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .setHover(mainMenu, true);
          ...
          MenuShadeRect((int)mainMenu.menuBox.x + (int)DOSBoxMenu::dropshadowX,
          ...
          for (auto i = popup_stack.begin(); i != popup_stack.end(); i++) {
            if (mainMenu.get_item(*i).get_type() ==
                DOSBoxMenu::submenu_type_id) {
              mainMenu.get_item(*i).drawBackground(mainMenu);
              mainMenu.get_item(*i).display_list.DrawDisplayList(
                  mainMenu, /*updateScreen*/ false);
            }
          }
          ...
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .display_list.DrawDisplayList(mainMenu, /*updateScreen*/ false);
          mainMenu.get_item(mainMenu.menuUserAttentionAt)
              .updateScreenFromPopup(mainMenu);
        }
        ...
        while (runloop) {
          ...
          case SDL_VIDEORESIZE:
            UpdateWindowDimensions(); // FIXME: Use SDL window dimensions,
                                      // except that on Windows, SDL won't tell
                                      // us our actual dimensions
          ...
          case SDL_MOUSEBUTTONUP:
            ...
            if (choice_item != DOSBoxMenu::unassigned_item_handle) {
              if (choice_item ==
                  psel_item) { /* clicking something twice should dismiss */
                runloop = false;
              } else {
                DOSBoxMenu::item &item = mainMenu.get_item(choice_item);
                if (item.get_type() == DOSBoxMenu::item_type_id &&
                    item.is_enabled())
                  runloop = false;
              }
          ...
          case SDL_MOUSEMOTION: {
            ...
            if (search != popup_stack.begin()) {
              do {
                ...
                sel_item =
                    mainMenu.get_item(*search).display_list.itemFromPoint(
                        mainMenu, event.button.x, event.button.y);
            ...
            if (sel_item == DOSBoxMenu::unassigned_item_handle)
              sel_item = mainMenu.display_list.itemFromPoint(
                  mainMenu, event.button.x, event.button.y);

            /* at this point:
             *  sel_item = item under cursor, or unassigned if no item
             *  search = iterator just past the item's level (to remove items if
             * changing) */

            if (mainMenu.menuUserHoverAt != sel_item) {
              bool noRedrawNew = false, noRedrawOld = false;

              if (mainMenu.menuUserHoverAt !=
                  DOSBoxMenu::unassigned_item_handle) {
                mainMenu.get_item(mainMenu.menuUserHoverAt)
                    .setHover(mainMenu, false);
                if (mainMenu.get_item(mainMenu.menuUserHoverAt).get_type() ==
                    DOSBoxMenu::item_type_id)
                  mainMenu.get_item(mainMenu.menuUserHoverAt)
                      .setHilight(mainMenu, false);
                else if (mainMenu.get_item(mainMenu.menuUserHoverAt)
                             .get_type() == DOSBoxMenu::submenu_type_id) {
              ...
              if (sel_item != DOSBoxMenu::unassigned_item_handle) {
                if (mainMenu.get_item(sel_item).get_type() ==
                    DOSBoxMenu::submenu_type_id) {
                  if (!mainMenu.get_item(sel_item).isHilight()) {
                    /* use a copy of the iterator to scan forward and un-hilight
                     * the menu items. then use the original iterator to erase
                     * from the vector. */
                    for (auto ss = search; ss != popup_stack.end(); ss++) {
                      for (auto &id :
                           mainMenu.get_item(*ss).display_list.get_disp_list())
                        mainMenu.get_item(id)
                            .setHilight(mainMenu, false)
                            .setHover(mainMenu, false);

                      mainMenu.get_item(*ss)
                          .setHilight(mainMenu, false)
                          .setHover(mainMenu, false);
                    }
              ...
              if (mainMenu.menuUserHoverAt !=
                      DOSBoxMenu::unassigned_item_handle &&
                  !OpenGL_using() && !redrawAll && !noRedrawOld) {
                if (mainMenu.get_item(mainMenu.menuUserHoverAt)
                        .checkResetRedraw()) {
                  mainMenu.get_item(mainMenu.menuUserHoverAt)
                      .drawMenuItem(mainMenu);
                  mainMenu.get_item(mainMenu.menuUserHoverAt)
                      .updateScreenFromItem(mainMenu);
                }
              }
              ...
              if (mainMenu.menuUserHoverAt !=
                      DOSBoxMenu::unassigned_item_handle &&
                  !OpenGL_using() && !redrawAll && !noRedrawNew) {
                if (mainMenu.get_item(mainMenu.menuUserHoverAt)
                        .checkResetRedraw()) {
                  mainMenu.get_item(mainMenu.menuUserHoverAt)
                      .drawMenuItem(mainMenu);
          ...
          if (redrawAll) {
            ...
            if (OpenGL_using()) {
              ...
              MenuRestoreScreen();
              mainMenu.display_list.DrawDisplayList(mainMenu,
                                                    /*updateScreen*/ false);
            }

            /* give the menu bar a drop shadow */
            MenuShadeRect((int)mainMenu.menuBox.x +
                              (int)DOSBoxMenu::dropshadowX,
                          (int)mainMenu.menuBox.y + (int)mainMenu.menuBox.h,
                          (int)mainMenu.menuBox.w,
                          (int)DOSBoxMenu::dropshadowY - 1 /*menubar border*/);
  ...
  switch (button->state) {
  case SDL_PRESSED:
    if (inMenu || !inputToScreen)
      return;
#if defined(WIN32) || defined(MACOSX) || defined(C_SDL2)
    if (!sdl.mouse.locked && button->button == SDL_BUTTON_LEFT &&
        isModifierApplied()) {
      if (mbutton == 4 && selsrow > -1 && selscol > -1) {
        Mouse_Select(selscol, selsrow, selmark ? selecol : selscol,
                     selmark ? selerow : selsrow, -1, -1, false);
        selmark = false;
        selsrow = selscol = selerow = selecol = -1;
      } else if (mouse_start_x >= 0 && mouse_start_y >= 0 && fx >= 0 &&
                 fy >= 0) {
        Mouse_Select(mouse_start_x - sdl.clip.x, mouse_start_y - sdl.clip.y,
                     fx - sdl.clip.x, fy - sdl.clip.y, sdl.clip.w, sdl.clip.h,
                     false);
    ...
    if (!sdl.mouse.locked &&
        ((mbutton == 2 && button->button == SDL_BUTTON_MIDDLE) ||
...
#endif
        if (sdl.mouse.requestlock && !vmware_mouse && !sdl.mouse.locked &&
            mouse_notify_mode == 0) {
      CaptureMouseNotify();
      GFX_CaptureMouse();
    ...
    if (((munlock == "manual" && !sdl.mouse.autoenable) ||
         (munlock == "auto" && sdl.mouse.autoenable) || munlock == "both") &&
        mouse_notify_mode == 0 && button->button == SDL_BUTTON_MIDDLE) {
      GFX_CaptureMouse();
      break;
    }
      ...
      if (wheel_key && (wheel_guest || !dos_kernel_disabled)) {
        ...
        if (wheel_key < 4) {
          ...
          SendInput(1, &ip, sizeof(INPUT));
            ...
            if (wheel_key == 7) {
          bool ctrlup =
              sdl.lctrlstate == SDL_KEYUP && sdl.rctrlstate == SDL_KEYUP;
          if (ctrlup)
            KEYBOARD_AddKey(KBD_leftctrl, true);
          ...
          bool ctrlup =
              sdl.lctrlstate == SDL_KEYUP && sdl.rctrlstate == SDL_KEYUP;
          if (wheel_key >= 4 && wheel_key <= 6 && ctrlup)
            KEYBOARD_AddKey(KBD_leftctrl, true);
          KEYBOARD_AddKey(
              wheel_key == 2 || wheel_key == 5
                  ? KBD_left
                  : (wheel_key == 3 || wheel_key == 6 ? KBD_pageup : KBD_up),
              true);
          if (wheel_key >= 4 && wheel_key <= 6 && ctrlup)
            KEYBOARD_AddKey(KBD_leftctrl, false);
          KEYBOARD_AddKey(
    ...
    case SDL_BUTTON_WHEELDOWN: /* Ick, really SDL? */
      if (wheel_key && (wheel_guest || !dos_kernel_disabled)) {
        ...
        if (wheel_key < 4) {
          ...
          if (wheel_key >= 4 && wheel_key <= 6 && ctrlup)
            KEYBOARD_AddKey(KBD_leftctrl, true);
          KEYBOARD_AddKey(wheel_key == 2 || wheel_key == 5
                              ? KBD_right
                              : (wheel_key == 3 || wheel_key == 6 ? KBD_pagedown
                                                                  : KBD_down),
                          true);
          if (wheel_key >= 4 && wheel_key <= 6 && ctrlup)
            KEYBOARD_AddKey(KBD_leftctrl, false);
  ...
  case SDL_RELEASED:
    ...
    if (!sdl.mouse.locked &&
      ...
      if (mouse_start_x == mouse_end_x && mouse_start_y == mouse_end_y) {
        ...
        if (abs(mouse_end_x - mouse_start_x) +
                abs(mouse_end_y - mouse_start_y) <
            5) {
          PasteClipboard(true);
#ifdef USE_TTF
          if (ttf.inUse)
...
bool GFX_IsFullscreen(void) { return sdl.desktop.fullscreen; }

bool sdl_wait_on_error() { return sdl.wait_on_error; }
...
void SetIMPosition() {
  uint8_t x, y;
  uint8_t page =
      IS_PC98_ARCH ? 0 : real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE);
  if (IS_PC98_ARCH) {
...
#endif
    uint8_t height =
        IS_PC98_ARCH ? 16 : real_readb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT);
    uint8_t width =
        CurMode && DOSV_CheckCJKVideoMode() ? CurMode->cwidth : (height / 2);
    SDL_Rect rect;
    rect.h = 0;
#if defined(USE_TTF)
    if (ttf.inUse) {
...
#endif
      double sx = sdl.clip.w > 0 && sdl.draw.width > 0
                      ? ((double)sdl.clip.w / sdl.draw.width)
                      : 1;
      double sy = sdl.clip.h > 0 && sdl.draw.height > 0
                      ? ((double)sdl.clip.h / sdl.draw.height)
                      : 1;
      rect.x = x * width * sx;
      rect.y = y * height * sy -
               (J3_IsJapanese()
                    ? 2
                    : (IS_DOSV ? -1 : (DOSV_CheckCJKVideoMode() ? 2 : 0)));
#if DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW /* SDL drawn menus */
...
void GFX_EventsMouseProcess(const long x, const long y, const long rx,
                            const long ry) {
 ...
 * will make spurious focus gain and loss events when locking the mouse in
 * windowed mode.
 *
 * This has not been tested with DOSBox-X yet because I do not run Xorg 1.20.1,
 * yet */
#if defined(LINUX)
...
void GFX_Events() {
  ...
  // Macs, with this code,  max 250 polls per second. (non-macs unused default
  // max 500) Currently not implemented for all platforms, given the ALT-TAB
  // stuff for WIN32.
#if defined(MACOSX)
  ...
  while (SDL_PollEvent(&event)) {
#if defined(C_SDL2)
    /* SDL2 hack: There seems to be a problem where calling the SetWindowSize
       function, even for the same size, still causes a resize event, and
       sometimes for no apparent reason, will cause an endless feed of Windows
       Restored events. */
    if (event.type == SDL_WINDOWEVENT &&
        event.window.event == SDL_WINDOWEVENT_RESTORED) {
      if (eatRestoredWindow)
        continue;
    } else {
...
#if defined(USE_TTF) && DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
            int last = 0;
            while (TTF_using() && !sdl.desktop.fullscreen && menu_gui &&
                   menu.toggle &&
                   menuwidth_atleast(ttf.cols * ttf.width + ttf.offX * 2 +
                                     GetSystemMetrics(SM_CXBORDER) * 2) > 0 &&
                   ttf.pointsize > last) {
    ...
    case SDL_WINDOWEVENT:
      ...
      if (sdl.priority.nofocus == PRIORITY_LEVEL_PAUSE) {
        if ((event.window.event == SDL_WINDOWEVENT_FOCUS_LOST) ||
           ...
           * This is similar to what PauseDOSBox() does, but the exit criteria
           * is different. Instead of waiting for the user to hit Alt-Break, we
           * wait for the window to regain window or input focus.
           */
          bool paused = true;
          SDL_Event ev;
          ...
          while (paused) {
            ...
            SDL_PollEvent(&ev);
#else
            // WaitEvent waits for an event rather than polling, so CPU usage
            // drops to zero
            SDL_WaitEvent(&ev);
#endif
            ...
            case SDL_QUIT:
              if (CheckQuit())
                throw(0);
              break; // a bit redundant at linux at least as the active events
                     // gets before the quit event.
    ...
    case SDL_MOUSEWHEEL:
      ...
      if (wheel_key && (wheel_guest || !dos_kernel_disabled)) {
        ...
        HandleMouseWheel(event.wheel.direction == SDL_MOUSEWHEEL_NORMAL,
                         event.wheel.y);
      break;
#if (defined(WIN32) && !defined(HX_DOS) || defined(LINUX) && C_X11 ||          \
     defined(MACOSX)) &&                                                       \
            ...
            if (CodePageHostToGuestUTF8((char *)buff, event.text.text)) {
              for (int no = 0; buff[no] != 0; no++) {
                if (IS_PC98_ARCH || isDBCSCP()) {
                  if (dos.loaded_codepage == 932 && isKanji1(buff[no]) &&
                      isKanji2(buff[no + 1])) {
#if defined(MACOSX)
                    if (buff[no] == 0x81 && buff[no + 1] == 0x40)
                      no++;
                    else
#endif
                    {
    ...
    case SDL_KEYUP:
      ...
      if (event.key.keysym.sym == SDLK_LALT)
        sdl.laltstate = event.key.type;
      if (event.key.keysym.sym == SDLK_RALT)
        sdl.raltstate = event.key.type;
      if (event.key.keysym.sym == SDLK_LCTRL)
        sdl.lctrlstate = event.key.type;
      if (event.key.keysym.sym == SDLK_RCTRL)
        sdl.rctrlstate = event.key.type;
      if (event.key.keysym.sym == SDLK_LSHIFT)
        sdl.lshiftstate = event.key.type;
      if (event.key.keysym.sym == SDLK_RSHIFT)
        sdl.rshiftstate = event.key.type;
      ...
      if (dos.im_enable_flag) {
        ...
        if (event.type == SDL_KEYDOWN && IME_GetEnable()) {
          // Enter, BS, TAB, <-, ->
          if (event.key.keysym.sym == 0x0d || event.key.keysym.sym == 0x08 ||
              event.key.keysym.sym == 0x09 ||
              (event.key.keysym.scancode >= 0x4f &&
               event.key.keysym.scancode <= 0x52)) {
            if (ime_text.size() != 0) {
              break;
            }
          } else {
            if (event.key.keysym.scancode == 0x2c && ime_text.size() == 0 &&
    ...
    default:
      if (sdl.desktop.type == SCREEN_GAMELINK)
        break;
      gfx_in_mapper = true;
      if (ticksLocked && event.type == SDL_KEYDOWN &&
          static_cast<Section_prop *>(control->GetSection("cpu"))
              ->Get_bool("stop turbo on key"))
  ...
  while (SDL_PollEvent(&event)) {
     ...
     * windowed mode.
     *
     * This has not been tested with DOSBox-X yet because I do not run
     * Xorg 1.20.1, yet */
#if SDL_XORG_FIX
    // Special code for broken SDL with Xorg 1.20.1, where pairs of inputfocus
    // gain and loss events are generated when locking the mouse in windowed
    // mode.
    if (event.type == SDL_ACTIVEEVENT &&
        event.active.state == SDL_APPINPUTFOCUS && event.active.gain == 0) {
      SDL_Event test; // Check if the next event would undo this one.
      if (SDL_PeepEvents(&test, 1, SDL_PEEKEVENT, SDL_ACTIVEEVENTMASK) == 1 &&
    ...
    case SDL_ACTIVEEVENT:
      ...
      if (event.active.state & SDL_APPMOUSEFOCUS) {
        if (!(event.active.gain & SDL_APPMOUSEFOCUS)) {
          /* losing focus or moving the mouse outside the window should
           * un-hilight the currently selected menu item */
          void GFX_SDLMenuTrackHover(DOSBoxMenu & menu,
                                     DOSBoxMenu::item_handle_t item_id);
          void GFX_SDLMenuTrackHilight(DOSBoxMenu & menu,
                                       DOSBoxMenu::item_handle_t item_id);
    ...
    case SDL_MOUSEBUTTONUP:
      ...
      break;
    case SDL_VIDEORESIZE:
      UpdateWindowDimensions(); // FIXME: Use SDL window dimensions, except that
                                // on Windows, SDL won't tell us our actual
                                // dimensions
      HandleVideoResize(&event.resize);
      break;
    case SDL_QUIT:
    ...
    case SDL_VIDEOEXPOSE:
      if (sdl.draw.callback && !glide.enabled)
        sdl.draw.callback(GFX_CallBackRedraw);
      break;
    case SDL_KEYDOWN:
    case SDL_KEYUP:
        ...
        if ((IS_PC98_ARCH || isDBCSCP()) && event.key.keysym.unicode != 0) {
          ...
          if (CodePageHostToGuestUTF16(chars, uname)) {
            for (size_t i = 0; i < strlen(chars); i++) {
              if (dos.loaded_codepage == 932 && strlen(chars) == 2 &&
                  isKanji1(chars[0]))
                BIOS_AddKeyToBuffer((i == 0 ? 0xf100 : 0xf000) |
      ...
      if ((event.key.keysym.sym == SDLK_TAB) &&
          ((sdl.laltstate == SDL_KEYDOWN) || (sdl.raltstate == SDL_KEYDOWN))) {
        MAPPER_LosingFocus();
        break;
      }
      ...
      if ((event.key.keysym.sym == SDLK_TAB) &&
        ...
        break;
      // ignore tab events that arrive just after regaining focus. (likely the
      // result of alt-tab)
      if ((event.key.keysym.sym == SDLK_TAB) &&
          (GetTicks() - sdl.focus_ticks < 2))
        break;
      if (GetACP() == 932 && GetKeyboardType(0) != 7) {
        // If the Windows code page is 932 and you are using a non-Japanese
        // keyboard
        if (event.key.keysym.scancode == 0x0d)
          event.key.keysym.sym = SDLK_EQUALS;
        else if (event.key.keysym.scancode == 0x2b)
          event.key.keysym.sym = SDLK_BACKSLASH;
        else if (event.key.keysym.scancode == 0x27)
          event.key.keysym.sym = SDLK_SEMICOLON;
        else if (event.key.keysym.scancode == 0x28)
      ...
      if (event.key.keysym.scancode == 0 && event.key.keysym.sym == 0) {
        ...
        if ((len = SDL_FlushIMString(NULL))) {
          uint16_t *buff = (uint16_t *)malloc((len + 1) * sizeof(uint16_t)),
                   uname[2];
  ...
  if (paste_speed && (iPasteTicker++ % paste_speed) ==
                         0) // emendelson: was 20 - good for WP51; Wengier:
                            // changed to 30 for better compatibility
    PasteClipboardNext();   // end added emendelson from dbDOS; improved by
                            // Wengier
}
...
void SDL_SetupConfigSection() {
  ...
  Pbool->Set_help(
      "Start DOSBox-X directly in fullscreen. (Press [F11/F12]+F to go back)");
  Pbool->SetBasic(true);

  Pbool = sdl_sec->Add_bool("fulldouble", Property::Changeable::Always, false);
  Pbool->Set_help("Use double buffering in fullscreen. It can reduce screen "
                  "flickering, but it can also result in a slow DOSBox-X.");
  Pbool->SetBasic(true);
  ...
  Pstring = sdl_sec->Add_string("fullresolution", Property::Changeable::Always,
                                "desktop");
  Pstring->Set_help("What resolution to use for fullscreen: original, desktop "
                    ...
                    "  If you end up with small window on a large screen, try "
                    "an output different from surface.");
  Pstring->SetBasic(true);

  Pstring = sdl_sec->Add_string("windowresolution",
                                Property::Changeable::Always, "original");
  Pstring->Set_help("Scale the window to this size IF the output device "
                    "supports hardware scaling.\n"
                    "  (output=surface does not!)");
  Pstring->SetBasic(true);

  Pstring =
      sdl_sec->Add_string("windowposition", Property::Changeable::Always, "-");
  Pstring->Set_help("Set the window position at startup in the "
                    "positionX,positionY format (e.g.: 1300,200).\n"
                    "The window will be centered with \",\" (or empty), and "
                    "will be in the original position with \"-\".");
  Pstring->SetBasic(true);

  const char *outputs[] = {"default",  "surface",  "overlay",  "ttf",
#if C_OPENGL
                           ...
                           "gamelink",
#endif
                           "ddraw",    "direct3d", nullptr};

  Pint = sdl_sec->Add_int("display", Property::Changeable::Always, 0);
  Pint->Set_help("Specify a screen/display number to use for a multi-screen "
                 "setup (0 = default).");
  Pint->SetBasic(true);

  Pstring =
      sdl_sec->Add_string("output", Property::Changeable::Always, "default");
  Pstring->Set_help("What video system to use for output (surface = software "
                    "(SDL_Surface); openglnb = OpenGL nearest; openglpp = "
                    "OpenGL perfect; ttf = TrueType font output).");
  Pstring->Set_values(outputs);
  Pstring->SetBasic(true);

  Pstring =
      sdl_sec->Add_string("videodriver", Property::Changeable::OnlyAtStart, "");
  Pstring->Set_help("Forces a video driver (e.g. windib/windows, directx, x11, "
                    "fbcon, dummy, etc) for the SDL library to use.");
  Pstring->SetBasic(true);

  Pint = sdl_sec->Add_int("transparency", Property::Changeable::WhenIdle, 0);
  Pint->Set_help("Set the transparency of the DOSBox-X screen (both windowed "
                 "and full-screen modes, on SDL2 and Windows SDL1 builds).\n"
                 "The valid value is from 0 (no transparency, the default "
                 "setting) to 90 (high transparency).");
  Pint->SetMinMax(0, 90);
  Pint->SetBasic(true);

  Pbool =
      sdl_sec->Add_bool("maximize", Property::Changeable::OnlyAtStart, false);
  Pbool->Set_help(
      "If set, the DOSBox-X window will be maximized at start (SDL2 and "
      "Windows SDL1 builds only; use fullscreen for TTF output).");
  Pbool->SetBasic(true);
  ...
  Pstring = sdl_sec->Add_string("autolock_feedback",
                                Property::Changeable::Always, feeds[1]);
  Pstring->Set_help(
      "Autolock status feedback type, i.e. visual, auditive, none.");
  Pstring->Set_values(feeds);
  Pstring->SetBasic(true);

  const char *unlocks[] = {"none", "manual", "auto", "both", nullptr};
  Pstring = sdl_sec->Add_string("middle_unlock", Property::Changeable::Always,
                                unlocks[1]);
  Pstring->Set_help(
      "Whether you can press the middle mouse button to unlock the mouse when "
      "the mouse has been locked.\n"
      "If set to \"manual\", it works only with \"autolock=false\"; if set to "
      "\"auto\", it works only with \"autolock=true\".");
  Pstring->Set_values(unlocks);
  Pstring->SetBasic(true);

  const char *clipboardbutton[] = {"none", "middle", "right", "arrows",
                                   nullptr};
  Pstring = sdl_sec->Add_string("clip_mouse_button",
                                Property::Changeable::Always, "right");
  Pstring->Set_values(clipboardbutton);
  Pstring->Set_help("Select the mouse button or use arrow keys for the shared "
                    "clipboard copy/paste function.\n"
                    "The default mouse button is \"right\", which means using "
                    "the right mouse button to select text, copy to and paste "
                    "from the host clipboard.\n"
                    "Set to \"middle\" to use the middle mouse button, "
                    "\"arrows\" to use arrow keys instead of a mouse button, "
                    "or \"none\" to disable this feature.\n"
                    "For \"arrows\", press Home key (or Fn+Shift+Left on Mac "
                    "laptops) to start selection, and End key (or "
                    "Fn+Shift+Right on Mac laptops) to end selection.");
  Pstring->SetBasic(true);

  const char *clipboardmodifier[] = {
      "none",      "ctrl",      "lctrl",      "rctrl",     "alt",
      "lalt",      "ralt",      "shift",      "lshift",    "rshift",
      "ctrlalt",   "ctrlshift", "altshift",   "lctrlalt",  "lctrlshift",
      "laltshift", "rctrlalt",  "rctrlshift", "raltshift", nullptr};
  Pstring = sdl_sec->Add_string("clip_key_modifier",
                                Property::Changeable::Always, "shift");
  Pstring->Set_values(clipboardmodifier);
  Pstring->Set_help("Change the keyboard modifier for the shared clipboard "
                    "copy/paste function using a mouse button or arrow keys.\n"
                    "The default modifier is \"shift\" (both left and right "
                    "shift keys). Set to \"none\" if no modifier is desired.");
  Pstring->SetBasic(true);

  const char *truefalsedefaultopt[] = {"true", "false",   "1",
                                       "0",    "default", nullptr};
  Pstring = sdl_sec->Add_string("clip_paste_bios",
                                Property::Changeable::WhenIdle, "default");
  Pstring->Set_values(truefalsedefaultopt);
  Pstring->Set_help(
      "Specify whether to use BIOS keyboard functions for the clipboard "
      "pasting instead of the keystroke method.\n"
      "For pasting clipboard text into Windows 3.x/9x applications (e.g. "
      "Notepad), make sure to use the keystroke method.");
  Pstring->SetBasic(true);

  Pint =
      sdl_sec->Add_int("clip_paste_speed", Property::Changeable::WhenIdle, 30);
  Pint->Set_help(
      "Set keyboard speed for pasting text from the shared clipboard.\n"
      "If the default setting of 30 causes lost keystrokes, increase the "
      "number.\n"
      "Or experiment with decreasing the number for applications that accept "
      "keystrokes quickly.");
  Pint->SetBasic(true);

  Pmulti = sdl_sec->Add_multi("sensitivity", Property::Changeable::Always, ",");
  Pmulti->Set_help("Mouse sensitivity. The optional second parameter specifies "
                   "vertical sensitivity (e.g. 100,-50).");
  Pmulti->SetValue("100");
  Pmulti->SetBasic(true);
  Pint =
      Pmulti->GetSection()->Add_int("xsens", Property::Changeable::Always, 100);
  ...
  Pbool->Set_help("Enable this setting to bypass your operating system's mouse "
                  "acceleration and sensitivity settings.\n"
                  "This works in fullscreen or when the mouse is captured in "
                  "window mode (SDL2 builds only).");
#endif

  Pbool = sdl_sec->Add_bool("usesystemcursor",
                            Property::Changeable::OnlyAtStart, false);
  Pbool->Set_help(
      "Use the mouse cursor of the host system instead of drawing a DOS mouse "
      "cursor. Activated when the mouse is not locked.");
  Pbool->SetBasic(true);

  const char *emulation[] = {"integration", "locked", "always", "never",
                             nullptr};
  Pstring = sdl_sec->Add_string("mouse_emulation", Property::Changeable::Always,
                                emulation[1]);
  Pstring->Set_help("When is mouse emulated ?\n"
                    ...
                    "When using a high DPI mouse, the emulation of mouse "
                    "movement can noticeably reduce the\n"
                    "sensitiveness of your device, i.e. the mouse is slower "
                    "but more precise.");
  Pstring->Set_values(emulation);
  Pstring->SetBasic(true);

  Pint =
      sdl_sec->Add_int("mouse_wheel_key", Property::Changeable::WhenIdle, -1);
  Pint->SetMinMax(-7, 7);
  Pint->Set_help("Convert mouse wheel movements into keyboard presses such as "
                 "arrow keys.\n"
                 "0: disabled; 1: up/down arrows; 2: left/right arrows; 3: "
                 "PgUp/PgDn keys.\n"
                 "4: Ctrl+up/down arrows; 5: Ctrl+left/right arrows; 6: "
                 "Ctrl+PgUp/PgDn keys.\n"
                 "7: Ctrl+W/Z, as supported by text editors like WordStar and "
                 "MS-DOS EDIT.\n"
                 "Putting a minus sign in front will disable the conversion "
                 "for guest systems.");
  Pint->SetBasic(true);
  ...
  Pmulti->Set_help("Priority levels for DOSBox-X. Second entry behind the "
                   "comma is for when DOSBox-X is not focused/minimized.\n"
                   "  pause is only valid for the second entry.");
  Pmulti->SetBasic(true);

  const char *actt[] = {"lowest",  "lower", "normal", "higher",
                        "highest", "pause", nullptr};
  Pstring = Pmulti->GetSection()->Add_string(
      "active", Property::Changeable::Always, "higher");
  Pstring->Set_values(actt);

  const char *inactt[] = {"lowest",  "lower", "normal", "higher",
                          "highest", "pause", nullptr};
  Pstring = Pmulti->GetSection()->Add_string(
      "inactive", Property::Changeable::Always, "normal");
  Pstring->Set_values(inactt);

  Pstring =
      sdl_sec->Add_path("mapperfile", Property::Changeable::Always, MAPPERFILE);
  Pstring->Set_help("File used to load/save the key/event mappings from. "
                    "Resetmapper only works with the default value.");
  Pstring->SetBasic(true);

  Pstring =
      sdl_sec->Add_path("mapperfile_sdl1", Property::Changeable::Always, "");
  Pstring->Set_help(
      "File used to load/save the key/event mappings from DOSBox-X SDL1 "
      "builds. If set it will override \"mapperfile\" for SDL1 builds.");

  Pstring =
      sdl_sec->Add_path("mapperfile_sdl2", Property::Changeable::Always, "");
  Pstring->Set_help(
      "File used to load/save the key/event mappings from DOSBox-X SDL2 "
      "builds. If set it will override \"mapperfile\" for SDL2 builds.");

  Pbool = sdl_sec->Add_bool("forcesquarecorner",
                            Property::Changeable::OnlyAtStart, true);
  Pbool->Set_help(
      "If set, DOSBox-X will force square corners (instead of round corners) "
      "for the DOSBox-X window when running in Windows 11.");

  const char *truefalseautoopt[] = {"true", "false", "1", "0", "auto", nullptr};
  Pstring = sdl_sec->Add_string("usescancodes",
                                Property::Changeable::OnlyAtStart, "auto");
  Pstring->Set_values(truefalseautoopt);
  Pstring->Set_help("Avoid usage of symkeys, in favor of scancodes. Might not "
                    "work on all operating systems.\n"
                    "If set to \"auto\" (default), it is enabled when using "
                    "non-US keyboards in SDL1 builds.");
  Pstring->SetBasic(true);

#if C_GAMELINK
  Pbool = sdl_sec->Add_bool("gamelink master",
                            Property::Changeable::OnlyAtStart, false);
  Pbool->Set_help("Allow Game Link connections e.g. from Grid Cartographer; "
                  "required for output=gamelink, otherwise optional.");
  Pbool = sdl_sec->Add_bool("gamelink snoop", Property::Changeable::OnlyAtStart,
                            false);
  Pbool->Set_help("Connect to an existing Game Link session and output link "
                  "data instead of sending own data. Compares memory contents "
                  "to find a suitable memory offset of peeks.");
  Pint = sdl_sec->Add_int("gamelink load address", Property::Changeable::Always,
                          0);
  Pbool->Set_help("Configure the original load address of the software (when "
                  "running in plain DOSBox) so that gamelink accesses are "
                  "adjusted for different load addresses.");
#endif

  Pint = sdl_sec->Add_int("overscan", Property::Changeable::Always, 0);
  Pint->SetMinMax(0, 10);
  Pint->Set_help(
      "Width of the overscan border (0 to 10) for the \"surface\" output.");
  Pint->SetBasic(true);
  ...
  Pbool->Set_help(
      "If set, DOSBox-X will show basic information including the DOSBox-X "
      "version number and current running speed in the title bar.");
  Pbool->SetBasic(true);

  Pbool = sdl_sec->Add_bool("showdetails", Property::Changeable::Always, false);
  Pbool->Set_help("If set, DOSBox-X will show the cycles count (FPS) and "
                  "emulation speed relative to realtime in the title bar.");
  Pbool->SetBasic(true);
...
static void show_warning(char const *const message) {
  ...
  SDL_Surface *splash_surf =
      SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 400, 32, rmask, gmask, bmask, 0);
  if (!splash_surf)
    return;
  ...
  while (m.size()) { // Max 50 characters. break on space before or on a newline
    ...
    m2 = m.substr(0, a);
    m.erase(0, b);
    OutputString((unsigned int)x, (unsigned int)y, m2.c_str(), 0xffffffffu, 0,
                 splash_surf);
    y += 20;
  }
...
static void launcheditor(std::string edit) {
  if (control->configfiles.size() && control->configfiles.front().size())
    execlp(edit.c_str(), edit.c_str(), control->configfiles.front().c_str(),
           (char *)0);
...
static void eraseconfigfile() {
  ...
  if (!f)
    f = fopen("dosbox.conf", "r");
  if (f) {
    fclose(f);
    show_warning(
        "Warning: dosbox-x.conf (or dosbox.conf) exists in current working "
...
static void erasemapperfile() {
  ...
  if (!g)
    g = fopen("dosbox.conf", "r");
  if (g) {
    fclose(g);
    show_warning("Warning: dosbox-x.conf (or dosbox.conf) exists in current "
                 "working directory.\nKeymapping might not be properly reset.\n"
                 "Please reset configuration as well and delete the "
                 "dosbox-x.conf (or dosbox.conf).\n");
  }
...
void DOSBox_ShowConsole() {
  ...
  /* Microsoft Windows: Allocate a console and begin spewing to it.
     DOSBox-X is compiled on Windows platforms as a Win32 application, and
     therefore, no console. */
  /* FIXME: What about "file handles" 0/1/2 emulated by C library, for use with
   * _open/_close/_lseek/etc? */
  AllocConsole();
...
bool DOSBOX_parse_argv() {
  ...
  while (control->cmdline->GetOpt(optname)) {
    ...
    if (optname == "v" || optname == "ver" || optname == "version") {
      ...
      fprintf(stderr, "DOSBox-X project maintainer: joncampbell123 (The Great "
                      "Codeholio)\n\n");
      fprintf(stderr, "DOSBox-X comes with ABSOLUTELY NO WARRANTY.  This is "
                      "free software,\n");
      fprintf(
          stderr,
      ...
      fprintf(stderr, "Options can be started with either \"-\" or \"/\" (e.g. "
                      "\"-help\" or \"/help\"):\n\n");
      fprintf(
          stderr,
          "  -?, -h, -help                           Show this help screen\n");
      fprintf(stderr, "  -v, -ver, -version                      Display "
                      "DOSBox-X version information\n");
      fprintf(stderr, "  -fs, -fullscreen                        Start "
                      "DOSBox-X in fullscreen mode\n");
      fprintf(stderr, "  -conf <configfile>                      Start "
                      "DOSBox-X with the specific config file\n");
      fprintf(stderr, "  -editconf <editor>                      Edit the "
                      "config file with the specific editor\n");
      fprintf(stderr, "  -userconf                               Create user "
                      "level config file\n");
      fprintf(stderr, "  -printconf                              Print config "
                      "file location\n");
      fprintf(stderr, "  -eraseconf (or -resetconf)              Erase loaded "
                      "config file (or user config file and exit)\n");
      fprintf(stderr, "  -erasemapper (or -resetmapper)          Erase loaded "
                      "mapper file (or user mapper file and exit)\n");
      fprintf(stderr,
              "  -opencaptures <param>                   Launch captures\n");
      fprintf(stderr,
              "  -opensaves <param>                      Launch saves\n");
      fprintf(stderr, "  -startui, -startgui, -starttool         Start "
                      "DOSBox-X with GUI configuration tool\n");
      fprintf(stderr, "  -startmapper                            Start "
                      "DOSBox-X with the mapper editor\n");
      fprintf(stderr, "  -promptfolder                           Prompt for "
                      "the working directory when DOSBox-X starts\n");
      fprintf(stderr, "  -nopromptfolder                         Do not prompt "
                      "for the working directory when DOSBox-X starts\n");
      fprintf(stderr,
              "  -nogui                                  Do not show GUI\n");
      fprintf(stderr,
              "  -nomenu                                 Do not show menu\n");
      fprintf(stderr, "  -showcycles                             Show cycles "
                      "count (FPS) in the title\n");
      fprintf(stderr, "  -showrt                                 Show "
                      "emulation speed relative to realtime in the title\n");
      fprintf(stderr, "  -socket <socketnum>                     Specify the "
                      "socket number for null-modem emulation\n");
      fprintf(stderr, "  -savedir <path>                         Set path for "
                      "the save slots\n");
      fprintf(stderr, "  -defaultdir <path>                      Set the "
                      "default working path for DOSBox-X\n");
      fprintf(stderr, "  -defaultconf                            Use the "
                      "default config settings for DOSBox-X\n");
      fprintf(stderr, "  -defaultmapper                          Use the "
                      "default key mappings for DOSBox-X\n");
#if defined(WIN32)
      fprintf(stderr, "  -disable-numlock-check                  Disable "
                      "NumLock check (Windows version only)\n");
#endif
      fprintf(stderr, "  -date-host-forced                       Force "
                      "synchronization of date with host\n");
#if C_DEBUG
      fprintf(stderr, "  -display2 <color>                       Enable "
                      "standard & monochrome dual-screen mode with <color>\n");
#endif
      fprintf(stderr, "  -lang (or -langcp) <message file>       Use specific "
                      "message file instead of language= setting\n");
      fprintf(stderr, "  -machine <type>                         Start "
                      "DOSBox-X with a specific machine <type>\n");
      fprintf(stderr, "  -nodpiaware                             Ignore (do "
                      "not signal) Windows DPI awareness\n");
      fprintf(stderr, "  -securemode                             Enable secure "
                      "mode (no drive mounting etc)\n");
      fprintf(stderr, "  -prerun                                 If [name] is "
                      "given, run it before AUTOEXEC.BAT config section\n");
#if defined(WIN32) && !defined(HX_DOS) || defined(MACOSX) || defined(LINUX)
      fprintf(stderr,
              "  -hostrun                                Enable running host "
              "program via START command and LFN support\n");
#endif
      fprintf(stderr, "  -noconfig                               Do not "
                      "execute CONFIG.SYS config section\n");
      fprintf(stderr, "  -noautoexec                             Do not "
                      "execute AUTOEXEC.BAT config section\n");
      fprintf(stderr, "  -exit                                   Exit after "
                      "executing AUTOEXEC.BAT\n");
      fprintf(stderr, "  -silent                                 Run DOSBox-X "
                      "silently and exit after executing AUTOEXEC.BAT.\n");
      fprintf(stderr, "  -o <option string>                      Provide "
                      "command-line option(s) for [name] if specified.\n");
      fprintf(stderr, "  -c <command string>                     Execute this "
                      "command in addition to AUTOEXEC.BAT.\n");
      fprintf(stderr, "                                          Make sure to "
                      "surround the command in quotes to cover spaces.\n");
      fprintf(stderr, "  -set <section property=value>           Set the "
                      "config option (overriding the config file).\n");
      fprintf(stderr, "                                          Make sure to "
                      "surround the string in quotes to cover spaces.\n");
      fprintf(stderr, "  -time-limit <n>                         Kill the "
                      "emulator after 'n' seconds\n");
      fprintf(stderr, "  -fastlaunch                             Fast launch "
                      "mode (skip the BIOS logo and welcome banner)\n");
#if C_DEBUG
      fprintf(stderr, "  -helpdebug                              Show "
      ...
      fprintf(stderr, "DOSBox-X project maintainer: joncampbell123 (The Great "
                      "Codeholio)\n\n");
      fprintf(stderr,
              "Based on DOSBox by the DOSBox Team (See AUTHORS file)\n\n");
      fprintf(stderr, "Debugging options:\n\n");
      fprintf(stderr, "  -debug                                  Set all "
                      "logging levels to debug\n");
      fprintf(stderr,
              "  -early-debug                            Log early "
              "initialization messages in DOSBox-X (implies -console)\n");
      fprintf(
          stderr,
          "  -keydbg                                 Log all SDL key events\n");
      fprintf(stderr, "  -break-start                            Break into "
                      "debugger at startup\n");
      fprintf(stderr, "  -console                                Show logging "
                      "console (Windows builds only)\n");
      fprintf(stderr, "  -noconsole                              Do not show "
                      "logging console (Windows debug builds only)\n");
      fprintf(stderr, "  -log-con                                Log CON "
                      "output to a log file\n");
      fprintf(stderr, "  -log-int21                              Log calls to "
                      "INT 21h (debug level)\n");
      fprintf(stderr, "  -log-fileio                             Log file I/O "
                      "through INT 21h (debug level)\n");
      fprintf(stderr, "  -nolog                                  Do not log "
                      "anything to log file\n");
      fprintf(stderr, "  -tests                                  Run unit "
                      "tests to test the DOSBox-X code\n");
      fprintf(stderr, "  -print-ticks                            (Debug) Print "
                      "emulator time and SDL_GetTicks()\n");
      fprintf(stderr, "\n");
      ...
      if (!control->cmdline->NextOptArgv(control->opt_machine))
        return false;
    ...
    } else if (optname == "test" || optname == "tests" ||
               optname == "gtest_list_tests") {
      putenv(const_cast<char *>("SDL_VIDEODRIVER=dummy"));
      ...
      control->opt_startui = true;
               ...
               optname == "disable_numlock_check") {
      /* mainline DOSBox expects -disable_numlock_check so we support that here
       * too */
      control->opt_disable_numlock_check = true;
    } else if (optname == "savedir") {
      if (!control->cmdline->NextOptArgv(custom_savedir))
        return false;
#if defined(WIN32) && defined(C_SDL2)
      localname = custom_savedir;
      if (!FileDirExistCP(custom_savedir.c_str()) &&
          FileDirExistUTF8(localname, custom_savedir.c_str()) == 2)
      ...
      if (!control->cmdline->NextOptArgv(control->opt_lang))
        return false;
      if (optname == "langcp")
        control->opt_langcp = true;
      ...
      if (!control->cmdline->NextOptArgv(control->opt_editconf))
        return false;
      ...
      if (control->cmdline->NextOptArgv(tmp)) {
        if (strcasecmp(tmp.c_str(), "amber") == 0)
          disp2_color = 1;
        else if (strcasecmp(tmp.c_str(), "green") == 0)
          disp2_color = 2;
      }
  ...
  while (!control->cmdline->CurrentArgvEnd()) {
    ...
    trim(tmp);
    localname = tmp;
    int rescp = FileDirExistCP(tmp.c_str()),
        resutf8 =
            rescp || !tmp.size() ? 0 : FileDirExistUTF8(localname, tmp.c_str());
    if (!rescp && resutf8) {
      tmp = localname;
      rescp = resutf8;
    }
    const char *ext = strrchr(
        tmp.c_str(), '.'); /* if it looks like a file... with an extension */
    if (rescp) {
      if (rescp == 2 ||
          (ext != NULL && rescp == 1 &&
        ...
        continue;
      } else if (ext != NULL && rescp == 1 &&
                 (!strcasecmp(ext, ".bat") || !strcasecmp(ext, ".exe") ||
                  !strcasecmp(
                      ext, ".com"))) { /* .BAT files given on the command line
                                          trigger automounting C: to run it */
...
#if defined(WIN32)
// NTS: I intend to add code that not only indicates High DPI awareness but also
// queries the monitor DPI
//      and then factor the DPI into DOSBox-X's scaler and UI decisions.
void Windows_DPI_Awareness_Init() {
  // if the user says not to from the command line, or disables it from
  // dosbox-x.conf, then don't enable DPI awareness.
  if (!dpi_aware_enable || control->opt_disable_dpi_awareness)
    return;

  /* log it */
  LOG(LOG_MISC, LOG_DEBUG)("Win32: I will announce High DPI awareness to "
                           "Windows to eliminate upscaling");

  // turn off DPI scaling so DOSBox-X doesn't look so blurry on Windows 8 &
  // Windows 10. use GetProcAddress and LoadLibrary so that these functions are
  // not hard dependencies that prevent us from running under Windows 7 or XP.
  HRESULT(WINAPI * __SetProcessDpiAwareness)(PROCESS_DPI_AWARENESS) =
      NULL;                                         // windows 8.1
  ...
  if (__user32)
    __SetProcessDPIAware =
        (BOOL(WINAPI *)(void))GetProcAddress(__user32, "SetProcessDPIAware");
  if (__shcore)
    __SetProcessDpiAwareness =
        (HRESULT(WINAPI *)(PROCESS_DPI_AWARENESS))GetProcAddress(
            __shcore, "SetProcessDpiAwareness");

  if (__SetProcessDpiAwareness) {
    LOG(LOG_MISC,
        LOG_DEBUG)("SHCORE.DLL exports SetProcessDpiAwareness function, "
                   "calling it to signal we are DPI aware.");
  ...
  if (__SetProcessDPIAware) {
    LOG(LOG_MISC, LOG_DEBUG)("USER32.DLL exports SetProcessDPIAware function, "
                             "calling it to signal we are DPI aware.");
    __SetProcessDPIAware();
  }
}
...
bool VM_Boot_DOSBox_Kernel() {
  if (!dos_kernel_disabled) {
    ...
    DispatchVMEvent(
        VM_EVENT_DOS_SURPRISE_REBOOT); // <- apparently we rebooted without any
                                       // notification (such as jmp'ing to
                                       // FFFF:0000)
  ...
  if (dos_kernel_disabled) {
    ...
    /* DOS kernel init */
    dos_kernel_disabled =
        false; // FIXME: DOS_Init should install VM callback handler to set this
    ...
    void update_pc98_function_row(unsigned char setting,
                                  bool force_redraw = false);

    void DRIVES_Startup(Section * s);
    DRIVES_Startup(NULL);

    /* NEC's function key row seems to be deeply embedded in the CON driver. Am
     * I wrong? */
    if (IS_PC98_ARCH)
      update_pc98_function_row(1);
    ...
    /* Most MS-DOS installations have a DEVICE=C:\HIMEM.SYS somewhere near the
     * top of their CONFIG.SYS */
    ...
    DispatchVMEvent(VM_EVENT_DOS_INIT_CONFIG_SYS_DONE); // <- we just finished
                                                        // executing CONFIG.SYS
    SHELL_Init(); // <- NTS: this will change CPU instruction pointer!
    DispatchVMEvent(
        VM_EVENT_DOS_INIT_SHELL_READY); // <- we just finished loading the shell
                                        // (COMMAND.COM)
    ...
    /* Most MS-DOS installations run MSCDEX.EXE from somewhere in AUTOEXEC.BAT.
     * We do the same here, in a fashion. */
    /* TODO: Can we make this an OPTION if the user doesn't want to make
     * MSCDEX.EXE resident? */
    /* TODO: When we emulate executing AUTOEXEC.BAT between INIT_SHELL_READY and
     * AUTOEXEC_BAT_DONE, can we make a fake MSCDEX.EXE within drive Z:\ and
     * auto-add a Z:\DOS\MSCDEX.EXE to the top of AUTOEXEC.BAT, command line
     * switches and all. if the user has not already added it? */
    void MSCDEX_Startup(Section * sec);
    MSCDEX_Startup(NULL);

    /* Some installations load the MOUSE.COM driver from AUTOEXEC.BAT as well */
    /* TODO: Can we make this an option? Can we add a fake MOUSE.COM to the Z:
     * drive as well? */
    void MOUSE_Startup(Section * sec);
    MOUSE_Startup(NULL);

    DispatchVMEvent(
        VM_EVENT_DOS_INIT_AUTOEXEC_BAT_DONE); // <- we just finished executing
                                              // AUTOEXEC.BAT
    DispatchVMEvent(
        VM_EVENT_DOS_INIT_AT_PROMPT); // <- now, we're at the DOS prompt
    SHELL_Run();
  }
...
bool VM_PowerOn() {
  if (!guest_machine_power_on) {
    // powering on means power on event, followed by reset assert, then reset
    // deassert
...
void SetWindowTransparency(int trans) {
  ...
  SetWindowLong(GetHWND(), GWL_EXSTYLE,
                GetWindowLong(GetHWND(), GWL_EXSTYLE) | WS_EX_LAYERED);
...
#if defined(C_SDL2)
  SDL_DisplayMode dm;
  if (SDL_GetDesktopDisplayMode(sdl.displayNumber ? sdl.displayNumber - 1 : 0,
                                &dm) == 0) {
    maxWidth = dm.w;
    maxHeight = dm.h;
  }
...
#if defined(WIN32) && !defined(HX_DOS)
#include "Shlobj.h"
int CALLBACK FolderBrowserCallback(HWND h_Dlg, UINT uMsg, LPARAM lParam,
                                   LPARAM lpData) {
  (void)lParam;
...
std::wstring win32_prompt_folder(const char *default_folder) {
  std::wstring res;
  const WCHAR text[] = L"Select folder where to run emulation, which will "
                       L"become DOSBox-X's working directory:";
...
#if 0 // Browse for folder using SHBrowseForFolder, which works on Windows XP
      // and higher
...
#if !defined(__MINGW32__) /* MinGW does not have these headers */
  IFileDialog *ifd; /* Windows Vista file/folder picker interface COM object
                       (shobjidl_core.h) */
  /* Try the new picker first (Windows Vista or higher) which makes it possible
   * to pick a folder */
  if (CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER,
                       IID_IFileDialog, (void **)(&ifd)) == S_OK) {
    HRESULT hr;
    HMODULE __shell32 = GetModuleHandle("SHELL32.DLL");
    if (wfolder != NULL && __shell32) {
      PIDLIST_ABSOLUTE pidl = NULL;
      SHParseDisplayName(wfolder, NULL, &pidl, 0, NULL);
      HRESULT(WINAPI * __SHCreateItemFromIDList)(PCIDLIST_ABSOLUTE, REFIID,
                                                 void **) = NULL;
      __SHCreateItemFromIDList =
          (HRESULT(WINAPI *)(PCIDLIST_ABSOLUTE, REFIID, void **))GetProcAddress(
              __shell32, "SHCreateItemFromIDList");
    ...
    ifd->SetOptions(FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM | FOS_PATHMUSTEXIST |
                    FOS_DONTADDTORECENT);
    ifd->SetTitle(text);
  ...
  of.lpstrTitle = text;
  of.Flags =
      OFN_LONGNAMES | OFN_NOCHANGEDIR | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
  of.lpstrFilter = L"DOSBox-X configuration file\0"
                   L"dosbox-x.conf;dosbox.conf\0";
  if (GetOpenFileNameW(&of)) {
    if (of.nFileOffset >= sizeof(tmp))
      return std::wstring();
    while ((of.nFileOffset > 0 && tmp[of.nFileOffset - 1] == '/') ||
           tmp[of.nFileOffset - 1] == '\\')
      of.nFileOffset--;
    if (of.nFileOffset == 0)
      return std::wstring();
...
int main(int argc, char *argv[]) SDL_MAIN_NOEXCEPT {
  ...
  /* -- Early logging init, in case these details are needed to debug problems
   * at this level */
  /*    If --early-debug was given this opens up logging to STDERR until
   * Log::Init() */
  LOG::EarlyInit();
...
#if defined(MACOSX)
  /* The resource system of DOSBox-X relies on being able to locate the
     Resources subdirectory within the DOSBox-X .app bundle. To do this, we have
     to first know where our own executable is, which macOS helpfully puts int
     argv[0] for us */
  /* NTS: Experimental testing shows that when we are run from the desktop
     (double-clicking on the .app bundle from the Finder) the current working
     directory is / (fs root). */
  extern std::string MacOSXEXEPath;
  extern std::string MacOSXResPath;
  MacOSXEXEPath = argv[0];

  /* The path should be something like
   * /blah/blah/dosbox-x.app/Contents/MacOS/DosBox */
    ...
    if (!MacOSXResPath.size() && exepath.size()) {
      ...
      if (path) {
        ...
        if (s != NULL) {
          ...
          if (!strncasecmp(s, "/MacOS/", 7)) {
            MacOSXResPath =
                std::string(path, (size_t)(s - path)) + "/Resources";
            struct stat st;
            if (stat(MacOSXResPath.c_str(), &st))
              MacOSXResPath = "";
          }
        }
        free(path);
  ...
  if (!control->opt_defaultconf && control->config_file_list.empty() &&
      stat("dosbox-x.conf", &st) && stat("dosbox.conf", &st)) {
    ...
    if (!control->configfiles.size() &&
        stat(config_combined.c_str(), &st) == 0 && S_ISREG(st.st_mode))
    ...
    resolveopt =
        resolvestr == "true" || resolvestr == "1"
            ? 1
            : (resolvestr == "dosvar" ? 2 : (resolvestr == "tilde" ? 3 : 0));
  ...
  if (workdiropt == "prompt" && control->opt_promptfolder < 0)
    control->opt_promptfolder = 1;
  else if (((workdiropt == "custom" && !control->opt_used_defaultdir) ||
            workdiropt == "force") &&
           workdirdef.size()) {
    if (chdir(workdirdef.c_str()) == -1) {
      LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change directories "
                              "for workdiropt 'custom' or 'force'.");
    }
    control->opt_used_defaultdir = true;
    usecfgdir = false;
    ...
    if (config_path.size()) {
      if (chdir(config_path.c_str()) == -1) {
        LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change "
                                "directories for workdiropt 'userconfig'.");
      }
    }
    control->opt_used_defaultdir = true;
    usecfgdir = false;
    ...
    if (exepath.size()) {
      if (chdir(exepath.c_str()) == -1) {
        LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change "
                                "directories for workdiropt 'program'.");
      }
    }
    control->opt_used_defaultdir = true;
    usecfgdir = false;
  ...
  /* default do not prompt if -set, -conf, -userconf, -defaultconf, or
   * -defaultdir is used */
  if (control->opt_promptfolder < 0 &&
      (!control->config_file_list.empty() || !control->opt_set.empty() ||
       control->opt_userconf || control->opt_defaultconf ||
       control->opt_used_defaultdir || control->opt_fastlaunch ||
       control->opt_test || workdiropt == "noprompt")) {
    control->opt_promptfolder = 0;
  }
...
#if defined(MACOSX) || defined(LINUX) || (defined(WIN32) && !defined(HX_DOS))
  {
    if (control->opt_promptfolder < 0) {
      ...
      /* if dosbox.conf or dosbox-x.conf already exists in the current working
       * directory, then skip folder prompt */
...
#if defined(WIN32)
    /* A Windows application cannot detect with isatty() if run from the command
     * prompt. isatty() returns true even though STDIN/STDOUT/STDERR do not
     * exist even if run from the command prompt. */
    if (control->opt_promptfolder < 0)
      control->opt_promptfolder = 1;
#else
    std::unique_ptr<char[]> cwd(new char[PATH_MAX]);
    if (control->opt_promptfolder < 0 && getcwd(cwd.get(), PATH_MAX) != nullptr)
      control->opt_promptfolder =
          (!isatty(0) || !strcmp(cwd.get(), "/")) ? 1 : 0;
#endif
    if (control->opt_promptfolder == 1 && workdiropt == "default" &&
        workdirdef.size()) {
      control->opt_promptfolder = 0;
      if (chdir(workdirdef.c_str()) == -1) {
        LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change "
                                "directories for workdiropt 'default'.");
      }
      control->opt_used_defaultdir = true;
      usecfgdir = false;
    }

    /* When we're run from the Finder, the current working directory is often /
       (the root filesystem) and there is no terminal. What to run, what
       directory to run it from, and the dosbox-x.conf to read, is not obvious.
       If run from the Finder, prompt the user where to run from, and then set
       it as the current working directory and continue. If they cancel, then
       exit. */
    /* Assume that if STDIN is not a TTY, or if the current working directory is
       "/", that we were started by the Finder */
    /* FIXME: Is there a better way to detect whether we were started by the
       Finder or any other part of the macOS desktop? */
    if (control->opt_promptfolder > 0) {
#if defined(MACOSX)
      /* NTS: Do NOT call macosx_prompt_folder() to show a modal NSOpenPanel
         without first initializing the SDL video subsystem. SDL1 must
         initialize the Cocoa NS objects first, or else strange things happen,
         like DOSBox-X as an NSWindow with no apparent icon in the task manager,
      ...
      std::string dir =
          workdirdef.empty() ? (!exepath.empty() ? exepath : "") : workdirdef;
      ...
      const char *confirmstr =
          "Do you want to use the selected folder as the DOSBox-X working "
          "directory in future sessions?\n\nIf you select Yes, DOSBox-X will "
          "not prompt for a folder again.\nIf you select No, DOSBox-X will "
          "always prompt for a folder when it runs.\nIf you select Cancel, "
          "DOSBox-X will ask this question again next time.";
      const char *quitstr =
          "You have not selected a valid path. Do you want to run DOSBox-X "
          "with the current path as the DOSBox-X working "
      ...
      if (path.empty()) {
        ...
        int ans = MessageBox(NULL, confirmstr, "DOSBox-X working directory",
                             MB_YESNOCANCEL);
        ...
        if (ans == IDYES) {
          workdirsave = 1;
          workdirsaveas = buffer;
        } else if (ans == IDNO)
          workdirsave = 2;
      }
#else
      char *cpath = tinyfd_selectFolderDialog(
          "Select folder where to run emulation, which will become the "
          "DOSBox-X working directory:",
          default_folder);
      std::string path = (cpath != NULL) ? cpath : "";
      if (path.empty()) {
        if (!systemmessagebox("Run DOSBox-X?", quitstr, "yesno", "question",
                              1)) {
          fprintf(stderr, "No path chosen by user, exiting\n");
          return 1;
        }
        ...
        confirmstr =
            ...
            "not prompt for a folder again.\nIf you select No, DOSBox-X will "
            "always prompt for a folder when it runs.";
        int ans = systemmessagebox("DOSBox-X working directory", confirmstr,
                                   "yesno", "question", 1);
        if (ans == 1) {
          workdirsave = 1;
          workdirsaveas = path;
      ...
      if (!path.empty()) {
        ...
        if (_wchdir(path.c_str())) {
          MessageBoxW(NULL, path.c_str(),
                      L"Failed to change to the selected path.", MB_OK);
          return 1;
        }
#else
        struct stat st;
...
#endif
        LOG_MSG("User selected folder '%s', making that the current working "
                "directory.\n",
                path.c_str());
        control->opt_used_defaultdir = true;
      }
  ...
  config_combined = config_path + tmp;
  {
    ...
    !defined(_WIN32_WINDOWS)
    {
      ...
      do {
        ...
        if (!EnumDisplayDevices(NULL, i, &dd, 0))
          break;
        LOG_MSG("Win32 EnumDisplayDevices #%d: name=%s string=%s", i,
    ...
    if (workdirsave > 0 && configfile.size()) {
      ...
      if (control->configfiles.size()) {
        ...
        if (workdirsave == 1 && workdirsaveas.size()) {
          ...
          tsec->HandleInputline(
              ("working directory default=" + workdirsaveas).c_str());
        } else if (workdirsave == 2)
          tsec->HandleInputline("working directory option=autoprompt");
        if (control->PrintConfig(configfile.c_str(),
                                 static_cast<Section_prop *>(tsec)->Get_bool(
                                     "show advanced options")
                                     ? 1
                                     : -1)) {
          workdirsave = 0;
          LOG_MSG("Saved the DOSBox-X working directory to %s",
                  configfile.c_str());
        }
        control->ClearExtraData();
        control->configfiles.clear();
    ...
    if (control->opt_userconf || workdirsave > 0) {
      ...
      LOG(LOG_MISC,
          LOG_DEBUG)("Loading config file according to -userconf from %s",
      ...
      if (!control->configfiles.size()) {
        ...
        LOG(LOG_MISC, LOG_DEBUG)(
            "Attempting to write config file according to -userconf, to %s",
            config_combined.c_str());
        if (control->PrintConfig(config_combined.c_str())) {
          ...
          if (!control->opt_userconf)
            LOG_MSG("Saved the DOSBox-X working directory to %s",
                    config_combined.c_str());
          LOG(LOG_MISC,
              LOG_NORMAL)("Generating default configuration. Writing it to %s",
                          config_combined.c_str());
    ...
    if (workdirsave > 0)
      LOG_MSG("Unable to save the DOSBox-X working directory.");
    ...
    for (size_t si = 0; si < control->config_file_list.size(); si++) {
      ...
      if (!FileDirExistCP(cfg.c_str()) &&
          FileDirExistUTF8(localname, cfg.c_str()))
        cfg = localname;
#endif
    ...
    if (!control->opt_defaultconf) {
      ...
      const std::string config_paths[] = {
          ...
          res_path.empty()
              ? ""
              : res_path + "dosbox-x.conf", /* resource level conf */
          config_path.empty()
              ? ""
              : config_path + "dosbox-x.conf", /* user level conf */
      ...
      if (!control->configfiles.size()) {
        if (!config_combined.empty()) {
          ...
          control->ParseConfigFile(
              config_combined.c_str()); // Load the conf file created above
          if (control->configfiles.size())
            LOG_MSG("CONFIG: Created and loaded user config file %s",
                    config_combined.c_str());
        } else
          LOG_MSG("CONFIG: Warning: User config file not found. Try specifying "
                  "it with -conf option or put it in the current directory.");
      }

      if (control->configfiles.size()) {
        LOG_MSG("CONFIG: Loaded config file: %s",
        ...
        if (usecfgdir) {
          ...
          if (found != string::npos) {
            if (chdir(configpath.substr(0, found + 1).c_str()) == -1) {
              LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change "
                                      "directories for .conf file.");
            }
          }
        }
      }

      // Redirect existing PC-98 related settings from other sections to the
      // [pc98] section if the latter is empty
      Section_prop *pc98_section =
          static_cast<Section_prop *>(control->GetSection("pc98"));
        ...
        if (extra && strlen(extra)) {
          ...
          if (in)
            for (std::string line; std::getline(in, line);) {
              if (strncasecmp(line.c_str(), "pc-98 ", 6))
                continue;
      ...
      // Redirect existing TTF related settings from [render] section to the
      // [ttf] section if the latter is empty
      ...
      if (!extra || !strlen(extra)) {
        char linestr[CROSS_LEN + 1], *p;
        Section_prop *section =
            static_cast<Section_prop *>(control->GetSection("render"));
        extra = const_cast<char *>(section->data.c_str());
        if (extra && strlen(extra)) {
          std::istringstream in(extra);
          if (in)
            for (std::string line; std::getline(in, line);) {
              if (strncasecmp(line.c_str(), "ttf.", 4))
                continue;
              if (line.length() > CROSS_LEN) {
              ...
              if (p != NULL && ttf_section->HandleInputline(line.substr(4))) {
                *p = 0;
                LOG_MSG("Redirected \"%s\" (\"ttf.%s\") from [render] to [ttf] "
                        "section\n",
      ...
      // Redirect existing video related settings from [dosbox] section to the
      // [video] section if the latter is empty
      ...
      if (!extra || !strlen(extra)) {
        ...
        if (extra && strlen(extra)) {
          ...
          if (in)
            for (std::string line; std::getline(in, line);) {
              if (!strstr(line.c_str(), "vmem") &&
                  !strstr(line.c_str(), "vga") &&
                  !strstr(line.c_str(), "video") &&
                  !strstr(line.c_str(), "dac") &&
                  !strstr(line.c_str(), "cga") &&
                  !strstr(line.c_str(), "CGA") &&
                  !strstr(line.c_str(), "vesa") &&
                  !strstr(line.c_str(), "hpel") &&
                  !strstr(line.c_str(), "hretrace") &&
                  !strstr(line.c_str(), "debug line") &&
                  !strstr(line.c_str(), "memory bit") &&
                  !strstr(line.c_str(), "forcerate") &&
                  !strstr(line.c_str(), "double-buffered") &&
                  !strstr(line.c_str(), "vblank") &&
                  !strstr(line.c_str(), "setmode"))
                continue;
              if (line.length() > CROSS_LEN) {
                strncpy(linestr, line.c_str(), CROSS_LEN);
                linestr[CROSS_LEN] = 0;
      ...
      // Redirect existing files= setting from [dos] section to the [config]
      // section if the latter is empty
      Section_prop *config_section =
          static_cast<Section_prop *>(control->GetSection("config"));
      ...
      if (!extra || !strlen(extra)) {
        ...
        if (extra && strlen(extra)) {
          ...
          if (in)
            for (std::string line; std::getline(in, line);) {
              if (strncasecmp(line.c_str(), "files", 5) &&
                  strncasecmp(line.c_str(), "dos in hma", 10))
                continue;
              if (line.length() > CROSS_LEN) {
                strncpy(linestr, line.c_str(), CROSS_LEN);
                linestr[CROSS_LEN] = 0;
              ...
              if (p != NULL) {
                if (!strncasecmp(line.c_str(), "dos in hma", 10)) {
                  if (!strcasecmp(trim(p + 1), "true"))
                    line = "dos=high";
                  else if (!strcasecmp(trim(p + 1), "false"))
                    line = "dos=low";
                }
                if (config_section->HandleInputline(line)) {
                  *p = 0;
                  LOG_MSG("Redirected \"%s\" (\"%s\") from [dos] to [config] "
                          "section\n",
                          "dos", trim(linestr));
                }
              }
      ...
      Section_prop *ne2000_section =
          static_cast<Section_prop *>(control->GetSection("ne2000"));
      Section_prop *pcap_section =
          static_cast<Section_prop *>(control->GetSection("ethernet, pcap"));
      ...
      if (in)
        for (std::string line; std::getline(in, line);) {
          ...
          if (pcaptimeout_pos != std::string::npos &&
              eq_pos != std::string::npos && pcaptimeout_pos < eq_pos) {
            pcap_section->HandleInputline(
                "timeout=" + line.substr(eq_pos + 1, std::string::npos));
            LOG_MSG("Migrated pcaptimeout from [ne2000] to [ethernet, pcap] "
                    "section");
          }
          if (realnic_pos != std::string::npos && eq_pos != std::string::npos &&
              realnic_pos < eq_pos) {
            pcap_section->HandleInputline(line);
            LOG_MSG(
                "Migrated realnic from [ne2000] to [ethernet, pcap] section");
            if (ne2000_section->Get_bool("ne2000")) {
              LOG_MSG("Set ne2000 backend to pcap during migration");
              Prop_string *backend_prop = static_cast<Prop_string *>(
                  ne2000_section->Get_prop("backend"));
    ...
    for (auto it = control->opt_set.begin(); it != control->opt_set.end();
         it++) { /* -set switches */
      // add rest of command
      std::string rest;
      ...
      if (!strlen(pvars[0].c_str()))
        continue;
      if (pvars[0][0] == '%' || pvars[0][0] == '\0' || pvars[0][0] == '#' ||
          pvars[0][0] == '\n')
        continue;
      ...
      if ((equpos != std::string::npos) &&
        ...
        if (!sec && pvars[0].size() > 4 &&
            !strcasecmp(pvars[0].substr(0, 4).c_str(), "ttf.")) {
          pvars[0].erase(0, 4);
          sec = control->GetSectionFromProperty(pvars[0].c_str());
        }
        if (sec)
        ...
        if (!sec) {
          ...
          if (!sec2) {
            ...
            if (sec3 && !(equpos != std::string::npos &&
      ...
      Section *sec2 = control->GetSectionFromProperty(pvars[1].c_str());
      if (!sec2) {
        LOG_MSG(MSG_Get("PROGRAM_CONFIG_NO_PROPERTY"), pvars[1].c_str(),
                pvars[0].c_str());
        continue;
      }
...

    {
      ...
      if (((workdiropt == "custom" && !control->opt_used_defaultdir) ||
        ...
        if (chdir(workdirdef.c_str()) == -1) {
          LOG(LOG_GUI,
              LOG_ERROR)("sdlmain.cpp main() failed to change directories for "
                         "workdiropt 'custom' or 'force'.");
        }
      } else if (workdiropt == "userconfig") {
        std::string config_path;
        Cross::GetPlatformConfigDir(config_path);
        ...
        if (found != string::npos) {
          if (chdir(configpath.substr(0, found + 1).c_str()) == -1) {
            LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to change "
                                    "directories for workdiropt 'config'.");
          }
        }
      }

      {
        ...
        if (getcwd(cwd.get(), PATH_MAX))
          ...
          LOG(LOG_GUI, LOG_ERROR)("sdlmain.cpp main() failed to get the "
      ...
      if (!strcasecmp(imestr, "auto")) {
        ...
        if (!strcasecmp(machine, "pc98") || !strcasecmp(machine, "pc9801") ||
            !strcasecmp(machine, "pc9821") || !strcasecmp(machine, "jega") ||
            strcasecmp(static_cast<Section_prop *>(control->GetSection("dosv"))
                           ->Get_string("dosv"),
                       "off"))
          enableime = true;
        else {
          ...
          int cp = dos.loaded_codepage;
          if (InitCodePage() && isDBCSCP())
            enableime = true;
          else if (control->opt_langcp)
            tonoime = true;
          ...
          if (!enableime && !tonoime) {
            ...
            if (!strcmp(layoutname, "jp") || !strcmp(layoutname, "ko") ||
                !strcmp(layoutname, "cn") || !strcmp(layoutname, "tw") ||
                !strcmp(layoutname, "hk") || !strcmp(layoutname, "zh") ||
                !strcmp(layoutname, "zhs") || !strcmp(layoutname, "zht") ||
                (!strcmp(layoutname, "auto") &&
                 (cur_kb_layout == 1028 || cur_kb_layout == 1041 ||
                  cur_kb_layout == 1042 || cur_kb_layout == 2052 ||
                  cur_kb_layout == 3076)))
              enableime = true;
          }
#endif
    ...
    /* -- initialize logging first, so that higher level inits can report
     * problems to the log file */
    LOG::Init();
    ...
    if (isVirtualBox)
      LOG_MSG("Win32 VirtualBox graphics adapter detected");
#endif

    /* -- Welcome to DOSBox-X! */
    LOG_MSG("DOSBox-X version %s Commit %s (%s %s %d-bit)", VERSION,
            GIT_COMMIT_HASH, OS_PLATFORM_LONG, SDL_STRING, OS_BIT_INT);
    LOG(LOG_MISC, LOG_NORMAL)("Copyright 2011-%s The DOSBox-X Team. Project "
                              "maintainer: joncampbell123 (The Great "
                              "Codeholio). DOSBox-X published under GNU GPL.",
                              std::string(COPYRIGHT_END_YEAR).c_str());
...
#if !defined(C_SDL2)
    {
      ...
      LOG(LOG_MISC,
          LOG_DEBUG)("DOSBox-X CheckOS results: id=%u major=%u minor=%u", id,
                     major, minor);
    }
#endif
    ...
    /* hack: On debian/ubuntu with older libsdl version as they have done this
     * themselves, but then differently. with this variable they will work
     * correctly. I've only tested the 1.2.14 behaviour against the windows
     * version of libsdl */
...
#if defined(C_SDL2)
      LOG(LOG_GUI, LOG_DEBUG)("Win32 hack: setting SDL_VIDEODRIVER=windows "
                              "because environ variable is not set");
      putenv("SDL_VIDEODRIVER=windows");
#else
      LOG(LOG_GUI, LOG_DEBUG)("Win32 hack: setting SDL_VIDEODRIVER=windib "
                              "because environ variable is not set");
...

    {
      ...
      // boot-time option whether or not to report ourself as "DPI aware" to
      // Windows so the DWM doesn't upscale our window for backwards compat.
      {
...
#define DPI_AWARENESS_CONTEXT_SYSTEM_AWARE ((DPI_AWARENESS_CONTEXT) - 2)
#endif
              DPI_AWARENESS_CONTEXT(WINAPI * __SetThreadDpiAwarenessContext)(
                  DPI_AWARENESS_CONTEXT) = NULL;
              UINT(WINAPI * __GetDpiForSystem)() = NULL;
              __SetThreadDpiAwarenessContext =
                  (DPI_AWARENESS_CONTEXT(WINAPI *)(DPI_AWARENESS_CONTEXT))
                      GetProcAddress(__user32, "SetThreadDpiAwarenessContext");
              __GetDpiForSystem =
                  (UINT(WINAPI *)())GetProcAddress(__user32, "GetDpiForSystem");
              if (__SetThreadDpiAwarenessContext && __GetDpiForSystem) {
                ...
                if (previousDpiContext != NULL)
                  __SetThreadDpiAwarenessContext(previousDpiContext);
              }
            }
            if (dpi && dpi / 96 > 1)
              dpi_aware_enable = false;
...
#ifdef WIN32
    /* Windows Vista/7/8/10 DPI awareness. If we don't tell Windows we're high
     * DPI aware, the DWM will upscale our window to emulate a 96 DPI display
     * which on high res screen will make our UI look blurry. But we obey the
     * user if they don't want us to do that. */
    ...
    LOG_MSG("SDL: version %d.%d.%d, Video %s, Audio %s", sdl_version.major,
            sdl_version.minor, sdl_version.patch, SDL_GetCurrentVideoDriver(),
            SDL_GetCurrentAudioDriver());
#endif // defined (C_SDL2)
#if defined(__MINGW32__) && defined(C_DEBUG)
    LOG_MSG("EXPERIMENTAL: Debugger enabled for MinGW build, DOSBox-X crashes "
            "depending on the terminal software you use. Launching from "
            "command prompt (cmd.exe) is recommended.");
#endif
    ...
    /* -- -- helpful advice */
    LOG(LOG_GUI, LOG_NORMAL)(
        "Press Ctrl-F10 to capture/release mouse, Alt-F10 for configuration.");
    ...
    /* -- -- Initialise Joystick and CD-ROM separately. This way we can warn
     * when it fails instead of exiting the application */
    ...
    if (SDL_InitSubSystem(SDL_INIT_JOYSTICK) >= 0) {
      ...
      LOG(LOG_MISC, LOG_DEBUG)("SDL reports %u joysticks",
                               (unsigned int)sdl.num_joysticks);
    } else {
    ...
    if (sdl_videodrv != NULL && !strcmp(sdl_videodrv, "windows"))
      sdl.using_windib = true;
    ...
    if (getenv("SDL_VIDEODRIVER") == NULL) {
      ...
      LOG(LOG_MISC, LOG_DEBUG)(
          "Win32: SDL_VIDEODRIVER is '%s', so I will obey it", sdl_videodrv);
      if (strcmp(sdl_videodrv, "directx") == 0)
        sdl.using_windib = false;
      else if (strcmp(sdl_videodrv, "windib") == 0)
        sdl.using_windib = true;
    }
#endif
    ...
    /* FIXME: We need a more general "init list", outside of the section-based
     * design, that we then execute serially here. */
    /* TODO: Each section currently uses "AddDestroyFunction" per section. We
     * need to change over that code to a global destroy callback list instead.
     */
    /* TODO: Get rid of "init" and "destroy" callback lists per section. */
    /* TODO: Add a global (within the Config object) init and destroy callback
     * list. On each call, init functions are added to the end of the list, and
     *       destroy functions added to the beginning of the list. That way,
     * init is lowest level to highest, destroy is highest level to lowest. */
    ...
    /* TODO: Each "init" function should do the work of getting the section
     * object, whatever section it wants to read, instead of us doing the work.
     * When that's complete, the call to init should be without parameters
     * (void). The hope is that the init functions can read whatever sections it
     * wants, both newer DOSBox-X sections and existing DOSBox (mainline)
     * compatible sections. */
...

    {
      ...
      if ((control->opt_fullscreen || sdl_sec->Get_bool("fullscreen")) &&
          sdl.desktop.want_type != SCREEN_GAMELINK) {
        LOG(LOG_MISC,
            LOG_DEBUG)("Going fullscreen immediately, during startup");
        ...
        if (sdl.desktop.fullscreen && sdl.mouse.autoenable)
          GFX_CaptureMouse();
      }
      ...
      menu.hidecycles =
          (control->opt_showcycles || sdl_sec->Get_bool("showdetails") ? false
                                                                       : true);
    }
    ...
    /* at this point: If the machine type is PC-98, and the mapper keyboard
     * layout was "Japanese", then change the mapper layout to "Japanese PC-98"
     */
    if (host_keyboard_layout == DKM_JPN && IS_PC98_ARCH)
      SetMapperKeyboardLayout(DKM_JPN_PC98);
    ...
    HARDWARE_Init();
    CPU_PreInit();
    Init_AddressLimitAndGateMask(); /* <- need to init address mask so Init_RAM
                                       knows the maximum amount of RAM possible
                                     */
    Init_MemoryAccessArray(); /* <- NTS: In DOSBox-X this is the "cache" of
                                 devices that responded to memory access */
    ...
    TIMER_Init();
    PCIBUS_Init();
    PAGING_Init(); /* <- NTS: At this time, must come before memory init because
                      paging is so well integrated into emulation code */
    CMOS_Init();
    ROMBIOS_Init();
    CALLBACK_Init(); /* <- NTS: This relies on ROM BIOS allocation and it must
                        happen AFTER ROMBIOS init */
#if C_DEBUG
    ...
    VOODOO_Init();
    GLIDE_Init();
    PROGRAMS_Init(); /* <- NTS: Does not init programs, it inits the callback
                        used later when creating the .COM programs on drive Z:
                      */
    PCSPEAKER_Init();
    TANDYSOUND_Init();
    MPU401_Init();
    ...
    MIDI_Init();
    {
      ...
      MAPPER_AddHandler(Sendkeymapper, MK_delete, MMODHOST, "sendkey_mapper",
                        "Send special key", &item);
    ...
    /* let's assume motherboards are sane on boot because A20 gate is ENABLED on
     * first boot */
    MEM_A20_Enable(true);
...
#if C_DEBUG
    {
      ...
      MAPPER_AddHandler(
          DEBUG_Enable_Handler,
#if defined(MACOSX)
          // MacOS     NOTE: ALT-F12 to launch debugger. pause maps to F16 on
          // macOS, which is not easy to input on a modern mac laptop
          MK_f12
#else
          MK_pause
#endif
      ...
      /* Mac OS X does not have a console for us to just allocate on a whim like
         Windows does. So the debugger interface is useless UNLESS the user has
         started us from a terminal (whether over SSH or from the Terminal app).

         Linux/X11 also does not have a console we can allocate on a whim. You
         either run this program from XTerm for the debugger, or not. */
      bool allow = true;
    ...
    MOUSE_Init(); // FIXME: inits INT 15h and INT 33h at the same time. Also
                  // uses DOS_GetMemory() which is why DOS_Init must come first
    XMS_Init();
    EMS_Init();
    AUTOEXEC_Init();
#if C_IPX
    ...
    /* Init memhandle system. This part is used by DOSBox-X's XMS/EMS emulation
     * to associate handles per page. FIXME: I would like to push this down to
     * the point that it's never called until XMS/EMS emulation needs it. I
     * would also like the code to free the mhandle array immediately upon
     * booting into a guest OS, since memory handles no longer have meaning in
     * the guest OS memory layout. */
    Init_MemHandles();
    ...
    /* stop at this point, and show the configuration tool/mapper editor, if
     * instructed */
    if (control->opt_startui) {
      LOG(LOG_MISC, LOG_DEBUG)(
          "Running Configuration Tool, during startup, as instructed");
      int cp = dos.loaded_codepage;
      if (!cp)
        InitCodePage();
      GUI_Run(false);
    ...
    if (control->opt_startmapper) {
      LOG(LOG_MISC,
          LOG_DEBUG)("Running Mapper Editor, during startup, as instructed");
      int cp = dos.loaded_codepage;
      if (!cp)
        InitCodePage();
      MAPPER_RunInternal();
    ...
    if (!control->opt_silent)
      for (unsigned int i = 0; i < SaveState::SLOT_COUNT; i++) {
        name[4] = '0' + i;
        std::string command =
            SaveState::instance().getName(page * SaveState::SLOT_COUNT + i);
        std::string str = std::string(MSG_Get("SLOT")) + " " +
                          to_string(page * SaveState::SLOT_COUNT + i + 1) +
                          (command == "" ? "" : " " + command);
        mainMenu.get_item(name).set_text(str.c_str());
      }
    mainMenu.get_item("wheel_updown")
        .check(wheel_key == 1)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_leftright")
        .check(wheel_key == 2)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_pageupdown")
        .check(wheel_key == 3)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_ctrlupdown")
        .check(wheel_key == 4)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_ctrlleftright")
        .check(wheel_key == 5)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_ctrlpageupdown")
        .check(wheel_key == 6)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_ctrlwz")
        .check(wheel_key == 7)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_none")
        .check(wheel_key == 0)
        .refresh_item(mainMenu);
    mainMenu.get_item("wheel_guest").check(wheel_guest).refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_winlogo")
        .check(sendkeymap == 1)
        .refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_winmenu")
        .check(sendkeymap == 2)
        .refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_alttab")
        .check(sendkeymap == 3)
        .refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_ctrlesc")
        .check(sendkeymap == 4)
        .refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_ctrlbreak")
        .check(sendkeymap == 5)
        .refresh_item(mainMenu);
    mainMenu.get_item("sendkey_mapper_cad")
        .check(!sendkeymap)
        .refresh_item(mainMenu);
    mainMenu.get_item("hostkey_ctrlalt")
        .check(hostkeyalt == 1)
        .refresh_item(mainMenu);
    mainMenu.get_item("hostkey_ctrlshift")
        .check(hostkeyalt == 2)
        .refresh_item(mainMenu);
    mainMenu.get_item("hostkey_altshift")
        .check(hostkeyalt == 3)
        .refresh_item(mainMenu);
    std::string mapper_keybind = mapper_event_keybind_string("host");
    if (mapper_keybind.empty())
      mapper_keybind = "unbound";
    std::string text = mainMenu.get_item("hostkey_mapper").get_text();
    std::size_t found = text.find(":");
    if (found != std::string::npos)
      text = text.substr(0, found);
    mainMenu.get_item("hostkey_mapper")
        .check(hostkeyalt == 0)
        .set_text(text + ": " + mapper_keybind)
        .refresh_item(mainMenu);

    bool MENU_get_swapstereo(void);
    mainMenu.get_item("mixer_swapstereo")
        .check(MENU_get_swapstereo())
        .refresh_item(mainMenu);

    bool MENU_get_mute(void);
    mainMenu.get_item("mixer_mute")
        .check(MENU_get_mute())
        .refresh_item(mainMenu);

    mainMenu.get_item("mapper_fscaler")
        .check(render.scale.forced)
        .refresh_item(mainMenu);
    mainMenu.get_item("3dfx_voodoo")
        .check(strcasecmp(
            static_cast<Section_prop *>(control->GetSection("voodoo"))
                ->Get_string("voodoo_card"),
            "false"))
        .refresh_item(mainMenu);
    ...
    mainMenu.get_item("debug_logfileio")
        .check(log_fileio)
        .refresh_item(mainMenu);
    ...
    blinking = static_cast<Section_prop *>(control->GetSection("video"))
                   ->Get_bool("high intensity blinking");
    mainMenu.get_item("text_background")
        .enable(!IS_PC98_ARCH && machine != MCH_CGA)
        .check(!blinking)
        .refresh_item(mainMenu);
    mainMenu.get_item("text_blinking")
        .enable(!IS_PC98_ARCH && machine != MCH_CGA)
        .check(blinking)
        .refresh_item(mainMenu);
...
#endif
    mainMenu.get_item("mapper_dbcssbcs")
        .enable(TTF_using() && !IS_PC98_ARCH && !IS_JEGA_ARCH &&
                enable_dbcs_tables)
        .check(dbcs_sbcs || IS_PC98_ARCH || IS_JEGA_ARCH);
    mainMenu.get_item("mapper_autoboxdraw")
        .enable(TTF_using() && !IS_PC98_ARCH && !IS_JEGA_ARCH &&
                enable_dbcs_tables)
        .check(autoboxdraw || IS_PC98_ARCH || IS_JEGA_ARCH);
    mainMenu.get_item("ttf_halfwidthkana")
        .enable(TTF_using() && !IS_PC98_ARCH && !IS_JEGA_ARCH &&
                enable_dbcs_tables)
        .check(halfwidthkana || IS_PC98_ARCH || IS_JEGA_ARCH);
    refreshExtChar();
#endif
#if C_PRINTER
...
#if !defined(C_EMSCRIPTEN)
    mainMenu.get_item("show_console")
        .check(showconsole_init)
        .refresh_item(mainMenu);
    mainMenu.get_item("clear_console")
        .check(false)
        .enable(showconsole_init)
        .refresh_item(mainMenu);
    if (control->opt_display2) {
...
#if C_DEBUG
    if (control->opt_display2)
      mainMenu.get_item("mapper_debugger").enable(false).refresh_item(mainMenu);
#endif
    ...
    */
    ...
    /* Windows 7 taskbar extension support */
    {
      ...
      hr = CoCreateInstance(CLSID_TaskbarList, NULL, CLSCTX_SERVER,
                            IID_ITaskbarList3, (LPVOID *)(&winTaskbarList));
      if (hr == S_OK) {
...

        {
          ...
          b.hIcon =
              LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_MAPPER));
...

        {
          ...
          b.hIcon =
              LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_CFG_GUI));
          b.dwMask = (THUMBBUTTONMASK)(THB_TOOLTIP | THB_FLAGS | THB_ICON);
          b.dwFlags = (THUMBBUTTONFLAGS)(THBF_ENABLED | THBF_DISMISSONCLICK);
...

        {
          ...
          b.dwFlags = (THUMBBUTTONFLAGS)(THBF_DISABLED | THBF_NONINTERACTIVE |
                                         THBF_NOBACKGROUND);
        }
...
#if defined(WIN32) && !defined(C_SDL2)
      if (maximize && !TTF_using() && !GFX_IsFullscreen())
  ...
  fresh_boot:
    ...
    /* NTS: CPU reset handler, and BIOS init, has the instruction pointer poised
     * to run through BIOS initialization, which will then "boot" into the
     * DOSBox-X kernel, and then the shell, by calling VM_Boot_DOSBox_Kernel()
     */
    /* FIXME: throwing int() is a stupid and nondescriptive way to signal
     * shutdown/reset. */
    try {
#if C_DEBUG
      if (control->opt_break_start)
        DEBUG_EnableDebugger();
#endif
      DOSBOX_RunMachine();
    } catch (int x) {
      if (x == 2) { /* booting a guest OS. "boot" has already done the work to
                       load the image and setup CPU registers */
        LOG(LOG_MISC, LOG_DEBUG)("Emulation threw a signal to boot guest OS");

        run_machine = true; /* make note. don't run the whole shebang from an
                               exception handler! */
        dos_kernel_shutdown =
            !dos_kernel_disabled; /* only if DOS kernel enabled */
      } else if (x == 3) {        /* reboot the system */
        LOG(LOG_MISC,
            LOG_DEBUG)("Emulation threw a signal to reboot the system");
        ...
        LOG(LOG_MISC,
            LOG_DEBUG)("Emulation threw a signal to reboot DOS kernel");

        reboot_dos = true;
        dos_kernel_shutdown =
            !dos_kernel_disabled; /* only if DOS kernel enabled */
      } else if (x == 7) { /* DOS kernel corruption error (need to restart the
                              DOS kernel) */
        LOG(LOG_MISC,
            LOG_DEBUG)("Emulation threw a signal to reboot DOS kernel");
        ...
        LOG(LOG_MISC,
            LOG_DEBUG)("Emulation threw a signal to boot into BIOS image");

        run_bios = true;
        dos_kernel_shutdown =
            !dos_kernel_disabled; /* only if DOS kernel enabled */
      } else if (x == 9) { /* BIOS caught a JMP to F000:FFF0 without any other
        ...
        // kill switch (see instances of throw(0) and throw(1) elsewhere in
        // DOSBox)
        run_machine = false;
        dos_kernel_shutdown = false;
      }
    ...
    if (dos_kernel_shutdown) {
      ...
      if (!IS_PC98_ARCH && !IS_JEGA_ARCH && !IS_J3100 &&
      ...
      /* NTS: we take different paths depending on whether we're just shutting
       * down DOS or doing a hard reboot. */
      ...
      /* disable INT 33h mouse services. it can interfere with guest OS paging
      ...
      PROGRAMS_Shutdown(); /* FIXME: Is this safe? Or will this cause
                              use-after-free bug? */
      ...
      std::string core(static_cast<Section_prop *>(control->GetSection("cpu"))
                           ->Get_string("core"));
    ...
    if (run_machine) {
      ...
      bool disable_a20 =
          static_cast<Section_prop *>(control->GetSection("dosbox"))
              ->Get_bool("turn off a20 gate on boot");
      ...
      LOG_MSG(
          "  CS:IP=%04x:%04x SS:SP=%04x:%04x AX=%04x BX=%04x CX=%04x DX=%04x\n",
          SegValue(cs), reg_ip, SegValue(ss), reg_sp, reg_ax, reg_bx, reg_cx,
          reg_dx);
    ...
    if (run_bios) {
      ...
      /* Shutdown everything. For shutdown to work properly we must force CPU to
       * real mode */
      ...
      /* force the mapper to let go of all keys so that the host key is not
       * stuck (Issue #1320) */
      MAPPER_ReleaseAllKeys();
      ...
      if (custom_bios) {
        if (custom_bios_image && custom_bios_image_size != 0 &&
            custom_bios_image_offset != 0) {
          memcpy(GetMemBase() + custom_bios_image_offset, custom_bios_image,
                 custom_bios_image_size);
        }

        if (custom_bios_image)
      ...
      /* run again */
      goto fresh_boot;
    } else if (reboot_dos) { /* typically (at this time) to enter/exit PC-98
                                mode */
    ...
    sticky_keys(true); // Might not be needed if the shutdown function switches
                       // to windowed mode, but it doesn't hurt
    ...
    if (machine != MCH_AMSTRAD) // FIXME
      while (!exitfunctions.empty()) {
        ...
        LOG(LOG_MISC, LOG_DEBUG)("Calling exit function (%p) '%s'",
                                 (void *)((uintptr_t)ent.function),
                                 ent.name.c_str());
        ent.function(NULL);
...
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU && defined(WIN32) &&                   \
    !defined(HX_DOS) &&                                                        \
    (!defined(C_SDL2) && defined(SDL_DOSBOX_X_SPECIAL) || defined(C_SDL2))
    ShowWindow(GetHWND(), SW_HIDE);
    SDL1_hax_SetMenu(NULL); /* detach menu from window, or else Windows will
                               destroy the menu out from under the C++ class */
#endif
#if DOSBOXMENU_TYPE == DOSBOXMENU_NSMENU
...
#if defined(LINUX) && defined(HAVE_ALSA)
    // force ALSA to release global cache, so that it's one less leak reported
    // by Valgrind
    ...
    /* NTS: The "control" object destructor is called here because the "myconf"
     * object leaves scope. The destructor calls all section destroy functions
     * here. After this point, all sections have freed resources. */
    control = NULL; /* any deref past this point and you deserve to segfault */
  }
  return saved_opt_test && testerr ? 1 : 0;
}

void GFX_GetSizeAndPos(int &x, int &y, int &width, int &height,
                       bool &fullscreen) {
  x = sdl.clip.x;
  y = sdl.clip.y;
...
void SDL_GL_SwapBuffers(void) { SDL_GL_SwapWindow(sdl.window); }
#endif
