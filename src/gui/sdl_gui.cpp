...
#if defined(_MSC_VER)
#pragma warning(disable : 4065) /* Please do not warn on default case without  \
...
GUI::Checkbox *advopt, *saveall, *imgfd360, *imgfd400, *imgfd720, *imgfd1200,
    *imgfd1440, *imgfd2880, *imghd250, *imghd520, *imghd1gig, *imghd2gig,
    *imghd4gig, *imghd8gig;
...
static std::map<std::vector<GUI::Char>, GUI::ToplevelWindow *>
    cfg_windows_active;
void getlogtext(std::string &str), getcodetext(std::string &text),
    ApplySetting(std::string pvar, std::string inputline, bool quiet),
    GUI_Run(bool pressed);
void ttf_switch_on(bool ss = true), ttf_switch_off(bool ss = true),
    setAspectRatio(Section_prop *section), GFX_ForceRedrawScreen(void),
    SetWindowTransparency(int trans);
bool CheckQuit(void), OpenGL_using(void);
char tmp1[CROSS_LEN * 2], tmp2[CROSS_LEN];
const char *aboutmsg =
    "DOSBox-X ver." VERSION " (" OS_PLATFORM " " SDL_STRING " " OS_BIT "-bit)\n"
...
void RebootLanguage(std::string filename, bool confirm = false) {
  std::string exepath = GetDOSBoxXPath(true), tmpconfig = "~dbxtemp.conf",
              para = (filename.size() ? "-langcp \"" + filename + "\"" : "");
#if defined(USE_TTF)
  para += ttfswitch || switch_output_from_ttf ? " -set output=ttf" : "";
#endif
  struct stat st;
  if ((!confirm || CheckQuit()) && exepath.size()) {
    if (!stat(tmpconfig.c_str(), &st))
      remove(tmpconfig.c_str());
    if (control->PrintConfig(tmpconfig.c_str(), false, true) &&
        !stat(tmpconfig.c_str(), &st))
      para = "-conf " + tmpconfig + " -eraseconf " + para;
#if defined(WIN32)
    ShellExecute(NULL, "open", exepath.c_str(), para.c_str(), NULL, SW_NORMAL);
#else
    system((exepath + " " + para + " &").c_str());
...
void GUI_LoadFonts(void) {
  GUI::Font::addFont("default",
                     new GUI::BitmapFont(font_14_init && dos.loaded_codepage &&
...
static void getPixel(Bits x, Bits y, int &r, int &g, int &b, int shift) {
  if (x >= (Bits)render.src.width)
    x = (Bits)render.src.width - 1;
  if (y >= (Bits)render.src.height)
    x = (Bits)render.src.height - 1;
  if (x < 0)
    x = 0;
  if (y < 0)
    y = 0;
  ...
  case scalerMode8:
    pixel = *((unsigned int)x +
              (uint8_t *)(src + (unsigned int)y *
                                    (unsigned int)render.scale.cachePitch));
    r += (int)((unsigned int)render.pal.rgb[pixel].red >> (unsigned int)shift);
    g +=
        (int)((unsigned int)render.pal.rgb[pixel].green >> (unsigned int)shift);
    b += (int)((unsigned int)render.pal.rgb[pixel].blue >> (unsigned int)shift);
    break;
  case scalerMode15:
    pixel = *((unsigned int)x +
              (uint16_t *)(src + (unsigned int)y *
                                     (unsigned int)render.scale.cachePitch));
    r += (int)((pixel >> (7u + (unsigned int)shift)) &
               (0xf8u >> (unsigned int)shift));
    g += (int)((pixel >> (2u + (unsigned int)shift)) &
               (0xf8u >> (unsigned int)shift));
    b += (int)((pixel << (3u - (unsigned int)shift)) &
               (0xf8u >> (unsigned int)shift));
    break;
  case scalerMode16:
    pixel = *((unsigned int)x +
  ...
  case scalerMode32:
    pixel = *((unsigned int)x +
              (uint32_t *)(src + (unsigned int)y *
                                     (unsigned int)render.scale.cachePitch));
    r +=
        (int)(((pixel & GFX_Rmask) >> (GFX_Rshift + shift)) & (0xffu >> shift));
    g +=
        (int)(((pixel & GFX_Gmask) >> (GFX_Gshift + shift)) & (0xffu >> shift));
    b +=
        (int)(((pixel & GFX_Bmask) >> (GFX_Bshift + shift)) & (0xffu >> shift));
    break;
  }
}
...
static GUI::ScreenSDL *UI_Startup(GUI::ScreenSDL *screen) {
  ...
  GFX_LosingFocus(); // Release any keys pressed (buffer gets filled again).
                     // (could be in above if, but clearing the mapper input
                     // when exiting the mapper is sensible as well
  SDL_Delay(20);
  ...
  // Comparable to the code of intro.com, but not the same! (the code of
  // intro.com is called from within a com file)
  shell_idle =
      !dos_kernel_disabled && strcmp(RunningProgram, "LOADLIN") &&
      first_shell &&
      (DOS_PSP(dos.psp()).GetSegment() == DOS_PSP(dos.psp()).GetParent());
  ...
  if (sw_draw > 0 && sh_draw > 0) {
    screenshot =
        SDL_CreateRGBSurface(SDL_SWSURFACE, dw, dh, 32, GUI::Color::RedMask,
                             GUI::Color::GreenMask, GUI::Color::BlueMask, 0);
    SDL_FillRect(screenshot, nullptr, 0);

    unsigned int rs = screenshot->format->Rshift,
                 gs = screenshot->format->Gshift,
                 bs = screenshot->format->Bshift;

    // create screenshot for fade effect
    for (unsigned int y = 0; (int)y < sh_draw; y++) {
      uint32_t *bg = (uint32_t *)((y + (unsigned int)sy) *
                                      (unsigned int)screenshot->pitch +
                                  (char *)screenshot->pixels) +
                     (unsigned int)sx;
    ...
    for (int y = 0; y < sh_draw; y++) {
      uint32_t *bg = (uint32_t *)((unsigned int)(y + sy) *
                                      (unsigned int)background->pitch +
                                  (char *)background->pixels) +
                     sx;
      for (int x = 0; x < sw_draw; x++) {
        int r = 0, g = 0, b = 0;
        getPixel(x * (int)render.src.width / sw,
                 y * (int)render.src.height / sh, r, g, b, 3);
        getPixel((x - 1) * (int)render.src.width / sw,
                 y * (int)render.src.height / sh, r, g, b, 3);
        getPixel(x * (int)render.src.width / sw,
                 (y - 1) * (int)render.src.height / sh, r, g, b, 3);
        getPixel((x - 1) * (int)render.src.width / sw,
                 (y - 1) * (int)render.src.height / sh, r, g, b, 3);
        getPixel((x + 1) * (int)render.src.width / sw,
                 y * (int)render.src.height / sh, r, g, b, 3);
...
#endif
        {
          r1 = (int)((r * 393 + g * 769 + b * 189) /
                     1351); // 1351 -- tweak colors
          g1 =
              (int)((r * 349 + g * 686 + b * 168) / 1503); // 1203 -- for a nice
          b1 =
              (int)((r * 272 + g * 534 + b * 131) / 2340); // 2140 -- golden hue
        }
  ...
  SDL_Window *window = OpenGL_using()
                           ? GFX_SetSDLWindowMode(dw, dh, SCREEN_OPENGL)
                           : GFX_SetSDLSurfaceWindow(dw, dh);
  if (window == NULL)
  ...
  SDL_Surface *sdlscreen =
      SDL_SetVideoMode(dw, dh, 32, SDL_SWSURFACE | (fs ? SDL_FULLSCREEN : 0));
  if (sdlscreen == NULL)
    E_Exit("Could not initialize video mode %ix%ix32 for UI: %s", dw, dh,
           SDL_GetError());
  ...
  if (gui_menu_init) {
...

    {
      DOSBoxMenu::item &item =
          guiMenu.alloc_item(DOSBoxMenu::submenu_type_id, "ConfigGuiMenu");
      item.set_text(mainMenu.get_item("mapper_gui").get_text());
    }

    {
      DOSBoxMenu::item &item =
          guiMenu.alloc_item(DOSBoxMenu::item_type_id, "ExitGUI");
    ...
    guiMenu.displaylist_append(guiMenu.display_list,
                               guiMenu.get_item_id_by_name("ConfigGuiMenu"));

    {
      guiMenu.displaylist_append(guiMenu.get_item("ConfigGuiMenu").display_list,
                                 guiMenu.get_item_id_by_name("ExitGUI"));
    }
  ...
  if (null_menu_init) {
    ...
    {
      DOSBoxMenu::item &item =
          nullMenu.alloc_item(DOSBoxMenu::submenu_type_id, "ConfigGuiMenu");
      item.set_text("");
    }

    nullMenu.displaylist_clear(nullMenu.display_list);

    nullMenu.displaylist_append(nullMenu.display_list,
  ...
  if (screen)
    screen->setSurface(sdlscreen);
  else
    screen = new GUI::ScreenSDL(sdlscreen, scale); // CONF:SCALE
...
bool GUI_IsRunning(void) { return in_gui; }
...
VirtualBatch::VirtualBatch(DOS_Shell *host, const std::string &cmds)
    : BatchFile(host, "CON", "", ""), lines(cmds) {}
...
inline std::string stringify(const T &x,
                             std::ios_base &(*pf)(std::ios_base &) = NULL) {
  std::ostringstream o;
  if (pf)
    o << pf;
  if (!(o << x))
...
class PropertyEditor : public GUI::Window,
                       public GUI::ActionEventSource_Callback {
protected:
  Section_prop *section;
  Property *prop;
  ...
  void SetupUI(const bool opts, GUI::Input *&input, GUI::Button *&infoButton) {
    ...
    const auto defaultSpacing =
        static_cast<int>(GUI::CurrentTheme.DefaultSpacing);

    const auto inputWidth = 235 - (opts ? optionsWidth + defaultSpacing : 0);

    const auto optionsPos =
        windowWidth - optionsWidth - RightMarginText - defaultSpacing;

    const auto inputPos =
        opts ? optionsPos - defaultSpacing - inputWidth
             : windowWidth - RightMarginText - defaultSpacing - inputWidth;

    input = new GUI::Input(this, inputPos, 0, inputWidth,
                           static_cast<int>(GUI::CurrentTheme.ButtonHeight));
...
public:
  PropertyEditor(Window *parent, int x, int y, Section_prop *section,
                 Property *prop, bool opts)
      : Window(parent, x, y,
               parent->getParent()->getWidth() - RightMarginWindow, 25),
        section(section), prop(prop) {
    (void)opts;
  }

  virtual bool prepare(std::string &buffer) = 0;

  void actionExecuted(GUI::ActionEventSource *b,
                      const GUI::String &arg) override {
    (void)b; // UNUSED
    ...
    if (prepare(line)) {
      ...
      if (!(!strcmp(section->GetName(), "dosbox") &&
            prop->propname == "language")) {
  ...
  void paintVisGuideLineBetween(GUI::Drawable &d, const GUI::Window *lm,
                                const GUI::Window *rm,
                                const GUI::Window *pm) const {
...
public:
  ShowOptions(GUI::Screen *parent, int x, int y, const char *title,
              const char *msg, Property *prop, GUI::Input *input)
      : MessageBox2(parent, x, y, 310, title, msg) { // 740
    inp = input;
    pv = prop->GetValues();
    Bitu k, j = 0;
    int allowed_dialog_y =
        parent->getHeight() - 25 - (border_top + border_bottom) - 120;
    ...
    bool found = false;
    for (k = 0; k < pv.size(); k++)
      if (pv[k].ToString().size()) {
        opt[k] =
            new GUI::Radiobox(wiw, 5, j * 20 + 5, pv[k].ToString().c_str());
    ...
    if (!found)
      for (k = 0; k < pv.size(); k++)
        if (pv[k].ToString().size() &&
            pv[k].ToString() == prop->GetValue().ToString())
          opt[k]->setChecked(true);
    scroll_h -= (k - j) * 20;
    ...
    (new GUI::Button(this, 70, scroll_h + 90, MSG_Get("OK"), 70))
        ->addActionHandler(this);
    close->move(155, scroll_h + 90);
    resize(310, scroll_h + 156);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
    ...
    for (k = 0; k < pv.size(); k++)
      if (pv[k].ToString().size()) {
        if (arg == pv[k].ToString() && opt[k]->isChecked())
          for (j = 0; j < pv.size(); j++)
            if (pv[j].ToString().size() && j != k)
              opt[j]->setChecked(false);
      }
    if (arg == MSG_Get("OK")) {
      for (k = 0; k < pv.size(); k++)
  ...
  ~ShowOptions() {
    if (!cfg_sname.empty()) {
      ...
      if (i != cfg_windows_active.end())
        cfg_windows_active.erase(i);
    }
  }
};
...
public:
  PropertyEditorString(Window *parent, int x, int y, Section_prop *section,
                       Property *prop, bool opts)
      : PropertyEditor(parent, x, y, section, prop, opts) {
    ...
    /* the FIRST field needs to come first when tabbed to */
    {
      Window *w = this->getChild(0);
      if (w)
        w->raise(); /* NTS: This CHANGES the child element order, getChild(0)
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == "...")
      new ShowOptions(
          getScreen(), 300, 300, MSG_Get("SELECT_VALUE"),
          ("Property: \033[31m" + prop->propname + "\033[0m\n\n" +
           (prop->Get_Default_Value().ToString().size()
                ? "Default value: \033[32m" +
                      prop->Get_Default_Value().ToString() + "\033[0m\n\n"
                : "") +
  ...
  bool prepare(std::string &buffer) override {
    std::string temps = prop->GetValue().ToString();
    if (input->getText() == GUI::String(temps))
      return false;
...
public:
  PropertyEditorFloat(Window *parent, int x, int y, Section_prop *section,
                      Property *prop, bool opts)
      : PropertyEditor(parent, x, y, section, prop, opts) {
    SetupUI(opts, input, infoButton);
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == "...")
      new ShowOptions(
          getScreen(), 300, 300, ("Values for " + prop->propname).c_str(),
          ("Property: \033[31m" + prop->propname + "\033[0m\n\n" +
           (prop->Get_Default_Value().ToString().size()
                ? "Default value: \033[32m" +
                      prop->Get_Default_Value().ToString() + "\033[0m\n\n"
...
public:
  PropertyEditorHex(Window *parent, int x, int y, Section_prop *section,
                    Property *prop, bool opts)
      : PropertyEditor(parent, x, y, section, prop, opts) {
    SetupUI(opts, input, infoButton);
    std::string temps = prop->GetValue().ToString();
...
public:
  PropertyEditorInt(Window *parent, int x, int y, Section_prop *section,
                    Property *prop, bool opts)
      : PropertyEditor(parent, x, y, section, prop, opts) {
    SetupUI(opts, input, infoButton);
  ...
  bool prepare(std::string &buffer) override {
    int val;
    convert(input->getText(), val, false);
    if (val == static_cast<int>(prop->GetValue()))
      return false;
    buffer.append(stringify(val));
    return true;
  };
...
std::string CapName(std::string name) {
  dispname = name;
  if (name == "sdl" || name == "cpu" || name == "midi" || name == "gus" ||
      name == "dos" || name == "ipx" || name == "ne2000")
...
public:
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("DEBUGCMD")) {
      logwin = true;
      auto *np = new EnterDebuggerCommand(static_cast<GUI::Screen *>(parent),
                                          90, 100, "Debugger command");
      np->raise();
    } else if (arg == MSG_Get("CLOSE")) {
      if (shortcut)
        running = false;
...
public:
  HelpWindow(GUI::Screen *parent, int x, int y, Section *section)
    ...
    Section_prop *sec = dynamic_cast<Section_prop *>(
        title.substr(0, 4) == "Ide," ? control->GetSection("ide, primary")
                                     : section);
    if (sec) {
      std::string msg;
      ...
      for (const auto &p : properties) {
        std::string help = title == "4dos" && p->propname == "rem"
                               ? "This is the 4DOS.INI file (if you use 4DOS "
                                 "as the command shell)."
                               : p->Get_help();
        if (title != "4dos" && title != "Config" && title != "Autoexec" &&
            !advopt->isChecked() && !p->basic())
          continue;
        ...
        for (Bitu k = 0; k < pv.size(); k++) {
          ...
          if ((k + 1) < pv.size() &&
              (title != "Config" || p->propname != "numlock" ||
               pv[k + 1].ToString() != ""))
            propvalues += ", ";
        }
        msg += std::string("\033[31m") + p->propname + ":\033[0m\n" + help +
               ...
               (p->Get_Default_Value().ToString().size()
                    ? "\nDefault value: \033[32m" +
                          p->Get_Default_Value().ToString()
                    : "") +
               "\033[0m" +
               (p->getChange() == Property::Changeable::OnlyAtStart
    ...
    wiw->raise(); /* focus on the message, not the close button, so the user can
                     immediately arrow up/down */
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
         parent->getHeight() > this->getHeight()
...
public:
  SectionEditor(GUI::Screen *parent, int x, int y, Section_prop *section)
    ...
    int button_row_padding_y =
        static_cast<int>(GUI::CurrentTheme.DefaultSpacing);

    int num_prop = 0, k = 0;
    while (section->Get_prop(num_prop) != NULL) {
      if (advopt->isChecked() || section->Get_prop(num_prop)->basic())
        k++;
      num_prop++;
    }

    int allowed_dialog_y =
        parent->getHeight() - 25 - (border_top + border_bottom) - 50;
    ...
    wiw = new GUI::WindowInWindow(
        this, 5, 5, width - border_left - border_right - 10, scroll_h);
    ...
    resize((columns * column_width) + border_left + border_right +
               2 /*wiw border*/ + wiw->vscroll_display_width /*scrollbar*/ + 10,
           button_row_y + button_row_h + button_row_padding_y + border_top +
               border_bottom);
    ...
    GUI::Button *b = new GUI::Button(
        this, button_row_cx, button_row_y,
        mainMenu.get_item("HelpMenu").get_text().c_str(), button_w);
    b->addActionHandler(this);

    b = new GUI::Button(this, button_row_cx + (button_w + button_pad_w),
                        button_row_y, MSG_Get("OK"), button_w);
    ...
    for (const auto &prop : properties) {
      if (!advopt->isChecked() && !prop->basic()) {
        i++;
        continue;
      }
      ...
      Prop_multival_remain *pmulti_remain =
          dynamic_cast<Prop_multival_remain *>(prop);
      bool opts =
          !prop->suggested_values.empty() && prop->GetValues().size() > 1;

      PropertyEditor *p;
      if (pbool)
        p = new PropertyEditorBool(wiw, column_width * (j / items_per_col),
                                   (j % items_per_col) * row_height, section,
                                   prop, opts);
      else if (phex)
        p = new PropertyEditorHex(wiw, column_width * (j / items_per_col),
                                  (j % items_per_col) * row_height, section,
                                  prop, opts);
      else if (pint)
        p = new PropertyEditorInt(wiw, column_width * (j / items_per_col),
                                  (j % items_per_col) * row_height, section,
                                  prop, opts);
      else if (pdouble)
        p = new PropertyEditorFloat(wiw, column_width * (j / items_per_col),
                                    (j % items_per_col) * row_height, section,
                                    prop, opts);
      else if (pstring)
        p = new PropertyEditorString(wiw, column_width * (j / items_per_col),
                                     (j % items_per_col) * row_height, section,
                                     prop, opts);
      else if (pmulti)
        p = new PropertyEditorString(wiw, column_width * (j / items_per_col),
    ...
    b = new GUI::Button(this, button_row_cx + (button_w + button_pad_w) * 2,
                        button_row_y, MSG_Get("CANCEL"), button_w);
    b->addActionHandler(this);
    scan_tabbing = true;
    closeButton = b;
    ...
    wiw->resize((columns * column_width) + 2 /*border*/ +
                    wiw->vscroll_display_width,
                scroll_h);
    ...
    if (wiw->scroll_pos_h != 0) {
      ...
      resize((columns * column_width) + border_left + border_right +
                 2 /*wiw border*/ +
                 /*wiw->vscroll_display_width*/ /*scrollbar*/ +10,
             button_row_y + button_row_h + button_row_padding_y + border_top +
                 border_bottom);
    }
    move(parent->getWidth() > this->getWidth()
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK") && proplist.size()) {
      close();
      running = false;
      if (!shortcut)
        resetcfg = true;
    } else if (arg == MSG_Get("OK") || arg == MSG_Get("CANCEL") ||
               arg == MSG_Get("CLOSE")) {
      close();
      if (shortcut)
        running = false;
      ...
      if (!cfg_sname.empty()) {
        ...
        new_cfg_sname[3] = 'p';
        new_cfg_sname[4] = '_';
        new_cfg_sname.insert(new_cfg_sname.end(), cfg_sname.begin(),
                             cfg_sname.end());
      }
      ...
      if (lookup == cfg_windows_active.end()) {
        ...
        if (ny < 0)
          ny = 0;
        auto *np =
            new HelpWindow(static_cast<GUI::Screen *>(parent), nx, ny, section);
...
public:
  ConfigEditor(GUI::Screen *parent, int x, int y, Section_prop *section)
      : ToplevelWindow(parent, x, y, 510, 260 + GUI::titlebar_y_stop, ""),
        section(section) {
    if (section == NULL) {
        ...
        for (std::string line; std::getline(in, line);) {
          ...
          if (p != NULL) {
            *p = 0;
            ...
            cmd = trim(cmd);
            val = trim(val);
            if (strcasecmp(cmd, "rem") &&
                strlen(extra_data) + strlen(cmd) + strlen(val) + 3 < 4096 &&
                (title == "4dos" || !strncasecmp(cmd, "set ", 4) ||
                 !strcasecmp(cmd, "install") ||
    ...
    int button_pad_w = 10;
    int button_row_w =
        ((button_pad_w + button_w) * 4 + button_w) - button_pad_w;
    int button_row_cx = (((columns * column_width) - button_row_w) / 2) + 5;
    ...
    new GUI::Label(this, 5, button_row_y - height,
                   MSG_Get(title == "Config" ? "ADDITION_CONTENT" : "CONTENT"));
    content = new GUI::Input(this, 5, button_row_y - height + 20,
                             510 - border_left - border_right, height - 25);
    content->setText(extra_data);

    GUI::Button *b = new GUI::Button(this, button_row_cx, button_row_y,
                                     MSG_Get("PASTE_CLIPBOARD"), button_w * 2);
    b->addActionHandler(this);

    b = new GUI::Button(
        this, button_row_cx + button_w + (button_w + button_pad_w),
        button_row_y, mainMenu.get_item("HelpMenu").get_text().c_str(),
        button_w);
    b->addActionHandler(this);

    b = new GUI::Button(
        this, button_row_cx + button_w + (button_w + button_pad_w) * 2,
        button_row_y, MSG_Get("OK"), button_w);
    ...
    while ((prop = section->Get_prop(i))) {
      ...
      PropertyEditor *p;
      if (pbool)
        p = new PropertyEditorBool(wiw, column_width * (i / items_per_col),
                                   (i % items_per_col) * row_height, section,
                                   prop, opts);
      else if (phex)
        p = new PropertyEditorHex(wiw, column_width * (i / items_per_col),
                                  (i % items_per_col) * row_height, section,
                                  prop, opts);
      else if (pint)
        p = new PropertyEditorInt(wiw, column_width * (i / items_per_col),
                                  (i % items_per_col) * row_height, section,
                                  prop, opts);
      else if (pdouble)
        p = new PropertyEditorFloat(wiw, column_width * (i / items_per_col),
                                    (i % items_per_col) * row_height, section,
                                    prop, opts);
      else if (pstring)
        p = new PropertyEditorString(wiw, column_width * (i / items_per_col),
                                     (i % items_per_col) * row_height, section,
                                     prop, opts);
      else if (pmulti)
        p = new PropertyEditorString(wiw, column_width * (i / items_per_col),
    ...
    b = new GUI::Button(
        this, button_row_cx + button_w + (button_w + button_pad_w) * 3,
        button_row_y, MSG_Get("CANCEL"), button_w);
    b->addActionHandler(this);
    scan_tabbing = true;
    closeButton = b;
  ...
  void actionExecuted(GUI::ActionEventSource *b,
                      const GUI::String &arg) override {
    if (arg == MSG_Get("OK"))
      section->data = *(std::string *)content->getText();
    ...
    if (arg == MSG_Get("OK") || arg == MSG_Get("CANCEL") ||
      ...
      while (strPasteBuffer.length()) {
        ...
        if (head == 9)
          for (int i = 0; i < 8; i++)
            key = new GUI::Key(' ', ksym, false, false, false, false);
        else if (head == 13)
          key = new GUI::Key(0, GUI::Key::Enter, false, false, false, false);
        else if (head > 31)
          key = new GUI::Key(head, ksym, false, false, false, false);
        if (key != NULL)
          content->keyDown(*key);
...
public:
  AutoexecEditor(GUI::Screen *parent, int x, int y, Section_line *section)
      : ToplevelWindow(parent, x, y, 550, 310 + GUI::titlebar_y_stop, ""),
        section(section) {
    if (section == NULL) {
    ...
    content =
        new GUI::Input(this, 5, 30, 550 - 10 - border_left - border_right, 185);
    content->setText(section->data);
    (pasteButton = new GUI::Button(this, 5, 220, MSG_Get("PASTE_CLIPBOARD")))
        ->addActionHandler(this);
    int last = 25 + pasteButton->getWidth();
    if (first_shell) {
      (appendButton =
           new GUI::Button(this, last, 220, MSG_Get("APPEND_HISTORY")))
          ->addActionHandler(this);
      last += 20 + appendButton->getWidth();
    }
    if (shell_idle)
      (new GUI::Button(this, last, 220, MSG_Get("EXECUTE_NOW")))
          ->addActionHandler(this);
    (new GUI::Button(this, 180, 260, MSG_Get("OK"), 90))
        ->addActionHandler(this);
    (closeButton = new GUI::Button(this, 285, 260, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK") || arg == MSG_Get("CANCEL") ||
      ...
      UI_RunCommands(dynamic_cast<GUI::ScreenSDL *>(getScreen()),
                     content->getText());
    } else
      ToplevelWindow::actionExecuted(b, arg);
  }
...
public:
  SaveDialog(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 5, 60, MSG_Get("USE_PRIMARYCONFIG"), 200))
        ->addActionHandler(this);
    (new GUI::Button(this, 210, 60, MSG_Get("USE_PORTABLECONFIG"), 210))
        ->addActionHandler(this);
    (new GUI::Button(this, 425, 60, MSG_Get("USE_USERCONFIG"), 180))
        ->addActionHandler(this);
    Section_prop *section =
        static_cast<Section_prop *>(control->GetSection("dosbox"));
    saveall = new GUI::Checkbox(this, 5, 95, MSG_Get("CONFIG_SAVEALL"));
    saveall->setChecked(section->Get_bool("show advanced options"));
    (saveButton = new GUI::Button(this, 128, 120, MSG_Get("SAVE"), 90))
        ->addActionHandler(this);
    (new GUI::Button(this, 220, 120, MSG_Get("SAVE_RESTART"), 170))
        ->addActionHandler(this);
    (closeButton = new GUI::Button(this, 392, 120, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
    ...
    name->raise(); /* make sure keyboard focus is on the text field, ready for
                      the user */
    name->posToEnd(); /* position the cursor at the end where the user is most
                         likely going to edit */
  }
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("SAVE") || arg == MSG_Get("SAVE_RESTART"))
      control->PrintConfig(name->getText(), saveall->isChecked() ? 1 : -1);
    if (arg == MSG_Get("SAVE_RESTART"))
      RebootConfig((const char *)name->getText(), true);
    close();
    if (shortcut)
      running = false;
  }
...
public:
  SaveLangDialog(GUI::Screen *parent, int x, int y, const char *title)
    ...
    name = new GUI::Input(this, 5, 30, width - 10 - border_left - border_right);
    name->setText(control->opt_lang != "" ? control->opt_lang.c_str()
                                          : "messages.lng");
    ...
    (saveButton = new GUI::Button(this, 100, 110, MSG_Get("OK"), 90))
        ->addActionHandler(this);
    (closeButton = new GUI::Button(this, 200, 110, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    (void)b; // UNUSED
    if (arg == MSG_Get("OK"))
      MSG_Write(name->getText(), lang->getText());
    close();
    if (shortcut)
      running = false;
  }
...
// override Input field with one that responds to the Enter key as a
// keyboard-based cue to click "OK"
class InputWithEnterKey : public GUI::Input {
public:
  InputWithEnterKey(Window *parent, int x, int y, int w, int h = 0)
      : GUI::Input(parent, x, y, w, h) {};
...
public:
  SetRefreshRate(GUI::Screen *parent, int x, int y, const char *title)
    ...
    name = new InputWithEnterKey(this, 5, 30,
                                 width - 10 - border_left - border_right);
    ...
    (new GUI::Button(this, 200, 60, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK")) {
      ...
      if (vga_force_refresh_rate > 0)
        LOG_MSG("Video refresh rate is locked to %.3f fps.",
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK")) {
      ...
      if (sec) {
        ...
        Prop_multival *p3 =
            static_cast<Section_prop *>(sec)->Get_multival("sensitivity");
...
extern int autosave_second, autosave_count, autosave_start[10],
...
public:
  SetAutoSave(GUI::Screen *parent, int x, int y, const char *title)
    ...
    for (int i = 1; i < 10; i++) {
      new GUI::Label(this, 5, 15 + i * 30,
                     "Program " + std::to_string(i) + " (Optional)");
    ...
    new GUI::Label(this, 15, 315,
                   "Note: 0 for start slot = use current slot; -1 for start "
                   "slot = skip saving");
    (new GUI::Button(this, 225, 335, MSG_Get("OK"), 90))
        ->addActionHandler(this);
    (new GUI::Button(this, 325, 335, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK")) {
      ...
      for (int i = 1; i < 10; i++) {
        ...
        if (autosave_start[i] < -1)
          autosave_start[i] = -1;
        autosave_end[i] = atoi(end[i]->getText());
        if (autosave_end[i] < autosave_start[i])
          autosave_end[i] = 0;
        if ((autosave_start[i] > 1 && autosave_start[i] <= 100 &&
             autosave_last[i] < autosave_start[i]) ||
            (autosave_last[i] >
             (autosave_end[i] >= autosave_start[i] && autosave_end[i] <= 100
                  ? autosave_end[i]
                  : autosave_start[i])))
          autosave_last[i] = -1;
      }
      if (!mainMenu.get_item("enable_autosave").is_enabled() && autosave_second)
        enable_autosave = autosave_second > 0;
      if (autosave_second < 0)
        autosave_second = -autosave_second;
      mainMenu.get_item("enable_autosave")
          .check(enable_autosave)
          .enable(autosave_second > 0)
          .refresh_item(mainMenu);
      mainMenu.get_item("lastautosaveslot")
          .enable(autosave_second > 0)
          .refresh_item(mainMenu);
    }
    close();
...
public:
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (sec)
      LOG_MSG("GUI: Current Vertical Sync Rate: %s Hz",
              sec->Get_string("vsyncrate"));
    close();
    if (shortcut)
      running = false;
  }
...
public:
  SetLocalSize(GUI::Screen *parent, int x, int y, const char *title)
    ...
    new GUI::Label(this, 5, 10,
                   "Enter default local freesize (MB, min=0, max=1024):");
    ...
    (new GUI::Button(this, 200, 70, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK")) {
      ...
      LOG_MSG("GUI: Current default freesize for local disk: %dMB",
              512u * 32u * hdd_defsize / 1024u / 1024u);
    }
    close();
    if (shortcut)
      running = false;
...
public:
  SetAspectRatio(GUI::Screen *parent, int x, int y, const char *title)
      : ToplevelWindow(parent, x, y, 410, 140, title) {
    new GUI::Label(this, 5, 10,
                   "Enter aspect ratio (w:h, -1:-1 = original ratio):");
    name = new GUI::Input(this, 5, 30, 390);
    char buffer[8];
...
public:
  SetTitleText(GUI::Screen *parent, int x, int y, const char *title)
    ...
    title2->setText(t2);
    (new GUI::Button(this, 100, 110, MSG_Get("OK"), 90))
        ->addActionHandler(this);
    (new GUI::Button(this, 200, 110, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
    ...
    title1->raise(); /* make sure keyboard focus is on the text field, ready for
                        the user */
    title1->posToEnd(); /* position the cursor at the end where the user is most
                           likely going to edit */
  }
...
public:
  ShowMixerInfo(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 140, r + 30, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    resize(350, r + 110);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
...
public:
  ShowSBInfo(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 130, r + 30, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
...
public:
  ShowMidiDevice(GUI::Screen *parent, int x, int y, const char *title)
    ...
    std::string name = !midi.handler || !midi.handler->GetName()
                           ? "-"
    ...
    std::string midiinfo =
        "MIDI available: " + std::string(midi.available ? "Yes" : "No") +
      ...
      if (!strncmp(info, "fatDrive ", 9) || !strncmp(info, "isoDrive ", 9)) {
        ...
        if (pdp != NULL) {
          ...
          if (wdir != NULL && strlen(wdir)) {
            readonly = false;
            overlay = std::string(wdir) +
                      (wdir[strlen(wdir) - 1] != CROSS_FILESPLIT
                           ? std::string(1, CROSS_FILESPLIT)
                           : "") +
                      std::string(1, 'A' + statusdrive) + "_DRIVE";
          }
        }
  ...
  ShowDriveNumber(GUI::Screen *parent, int x, int y, const char *title)
    ...
    for (int index = 0; index < MAX_DISK_IMAGES; index++) {
      if (imageDiskList[index]) {
        ...
        if (index < 2)
          str = "Swap position: " +
                std::to_string(swaps == 1 ? 1 : swapPosition + 1) + "/" +
                std::to_string(swaps) + " - " +
                (dynamic_cast<imageDiskElToritoFloppy *>(
                     imageDiskList[index]) != NULL
                     ? "El Torito floppy drive"
                     : imageDiskList[index]->diskname);
        else {
          str = GetIDEPosition(index);
          str = "IDE controller: " + (str.size() ? str : "NA") + " - " +
                imageDiskList[index]->diskname;
        }
      } else
        str = "Not yet mounted";
      new GUI::Label(this, 40, 25 * (index + 1),
                     std::to_string(index) + " - " + str);
    }
    (new GUI::Button(this, 190, 25 * (MAX_DISK_IMAGES + 1) + 5,
                     MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
...
public:
  ShowIDEInfo(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 110, r + 30, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    resize(300, r + 110);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
...
public:
  ShowLoadWarning(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 140, 50, MSG_Get("YES"), 70))
        ->addActionHandler(this);
...
public:
  MakeDiskImage(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 135, 220, MSG_Get("OK"), 90))
        ->addActionHandler(this);
    (new GUI::Button(this, 255, 220, MSG_Get("CANCEL"), 90))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
    ...
    if (arg == MSG_Get("OK")) {
      ...
      if (temp.size()) {
        ...
        char const *lTheSaveFileName =
            tinyfd_saveFileDialog("Select a disk image file", "IMGMAKE.IMG", 2,
                                  lFilterPatterns, lFilterDescription);
#endif
...
public:
  ShowHelpIntro(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 260, 110, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
...
std::string prtlist = "Printer support is not enabled. Check [printer] section "
...
std::string niclist = "The pcap networking for NE2000 Ethernet emulation is "
                      "not currently active.\nPlease check [ne2000] and "
                      "[ethernet, pcap] sections of the configuration.";
...
public:
  ShowHelpAbout(GUI::Screen *parent, int x, int y, const char *title)
    ...
    (new GUI::Button(this, 180, 155, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
             : 0,
...
public:
  ShowHelpCommand(GUI::Screen *parent, int x, int y, const char *title)
    ...
    if (helpcmd == "CD")
      ...
      helpcmd = "RENAME";
    std::string helpinfo =
        std::string(MSG_Get(("SHELL_CMD_" + helpcmd + "_HELP").c_str())) +
        "\n" +
        std::string(MSG_Get(("SHELL_CMD_" + helpcmd + "_HELP_LONG").c_str()));
    std::istringstream in(str_replace(
        str_replace(str_replace(str_replace(helpinfo.c_str(), "%%", "%"),
                                "\033[0m", ""),
                    "\033[33;1m", ""),
        "\033[37;1m", ""));
    int r = 0;
    ...
    (new GUI::Button(this, 350, r + 40, MSG_Get("CLOSE"), 70))
        ->addActionHandler(this);
    resize(750, r + 120);
    move(parent->getWidth() > this->getWidth()
             ? (parent->getWidth() - this->getWidth()) / 2
...
struct ThemePresetEntry {
  ThemePresetEntry(const std::string &_name, const GUI::ThemeWindows31 &&_theme,
                   const unsigned int _special = 0)
      : name(_name), theme(_theme), special(_special) {}
...
public:
  ...
  ConfigurationWindow(GUI::Screen *parent, GUI::Size x, GUI::Size y,
                      GUI::String &title)
      : GUI::ToplevelWindow(parent, (int)x, (int)y, 30 /*initial*/,
                            30 /*initial*/, title) {
    ...
    for (size_t ti = 0; ti < (sizeof(theme_presets) / sizeof(theme_presets[0]));
         ti++) {
    ...
    std::function<std::pair<int, int>(const int)> gridfunc =
        [&/*access to locals here*/](const int i) {
          return std::pair<int, int>(gridbtnx + (i % btnperrow) * xSpace,
                                     gridbtny + (i / btnperrow) * ySpace);
        };
    ...
    for (const auto &sec : sections) {
      ...
      GUI::Button *b = new GUI::Button(this, sz.first, sz.second, sectionTitle,
                                       gridbtnwidth, gridbtnheight);
      b->addActionHandler(this);
    ...
    advopt =
        new GUI::Checkbox(this, gridbtnx, closerow_y, MSG_Get("SHOW_ADVOPT"));
    Section_prop *section =
        static_cast<Section_prop *>(control->GetSection("dosbox"));
    ...
    // apply theme
    {
      ...
      if (theme.empty()) {
        ...
        dark = HostDarkMode();
#endif
        TryApplyTheme(dark ? GUI::ThemeWindows31LCDReversedDark::GetName()
    ...
    (saveButton = new GUI::Button(this, xSave, closerow_y, tmp1, gridbtnwidth,
                                  gridbtnheight))
        ->addActionHandler(this);
    (closeButton = new GUI::Button(this, xExit, closerow_y, MSG_Get("CLOSE"),
                                   gridbtnwidth, gridbtnheight))
        ->addActionHandler(this);

    resize(
        gridbtnx + (xSpace * btnperrow) + gridbtnx + border_left + border_right,
  ...
  void actionExecuted(GUI::ActionEventSource *b,
    ...
    if (arg == MSG_Get("OK") || arg == MSG_Get("CANCEL") ||
      ...
      if (lookup == cfg_windows_active.end()) {
        Section_line *section = static_cast<Section_line *>(
            control->GetSection((const char *)sname));
      ...
      // new GUI::MessageBox2(getScreen(), 100, 150, 330, "About DOSBox-X",
      // aboutmsg);
      new GUI::MessageBox2(
          getScreen(),
          getScreen()->getWidth() > 350 ? (parent->getWidth() - 350) / 2 : 0,
          150, 350, mainMenu.get_item("help_about").get_text().c_str(),
          aboutmsg);
    } else if (arg == MSG_Get("INTRODUCTION")) {
      // new GUI::MessageBox2(getScreen(), 20, 50, 540, "Introduction",
      // intromsg);
      new GUI::MessageBox2(
          getScreen(),
          getScreen()->getWidth() > 540 ? (parent->getWidth() - 540) / 2 : 0,
          150, 540, mainMenu.get_item("help_intro").get_text().c_str(),
          MSG_Get("INTRO_MESSAGE"));
      ...
      // new GUI::MessageBox2(getScreen(), 0, 50, 680, std::string("Getting
      // Started"), msg);
      new GUI::MessageBox2(
          getScreen(),
          getScreen()->getWidth() > 680 ? (parent->getWidth() - 680) / 2 : 0,
          50, 680, MSG_Get("GET_STARTED"), msg.c_str());
    } else if (arg == MSG_Get("CDROM_SUPPORT")) {
      // new GUI::MessageBox2(getScreen(), 20, 50, 640, "CD-ROM Support",
      // MSG_Get("PROGRAM_INTRO_CDROM"));
      new GUI::MessageBox2(
          getScreen(),
          getScreen()->getWidth() > 640 ? (parent->getWidth() - 640) / 2 : 0,
          50, 640, MSG_Get("CDROM_SUPPORT"), MSG_Get("PROGRAM_INTRO_CDROM"));
    } else if (arg == tmp2) {
      strcpy(tmp1,
             (MSG_Get("SAVE_CONFIGURATION") + std::string("...")).c_str());
...
static void UI_Select(GUI::ScreenSDL *screen, int select) {
  ...
  case 19: {
    auto *np4 =
        new SetDOSVersion(screen, 90, 100, "Edit reported DOS version...");
    np4->raise();
  } break;
  case 20: {
  ...
  case 21: {
    auto *np6 = new SetTitleText(
        screen, 90, 130, mainMenu.get_item("set_titletext").get_text().c_str());
    np6->raise();
  } break;
  case 22: {
    auto *np6 = new SetTransparency(
        screen, 90, 100,
        mainMenu.get_item("set_transparency").get_text().c_str());
    np6->raise();
  } break;
  case 23: {
    auto *np7 = new ShowLoadWarning(
        screen, 150, 120, "DOSBox-X version mismatch. Load the state anyway?");
    np7->raise();
  } break;
  case 24: {
    auto *np7 = new ShowLoadWarning(
        screen, 150, 120, "Program name mismatch. Load the state anyway?");
    np7->raise();
  } break;
  case 25: {
    auto *np7 = new ShowLoadWarning(
        screen, 150, 120, "Memory size mismatch. Load the state anyway?");
    np7->raise();
  } break;
  case 26: {
    auto *np7 = new ShowLoadWarning(
        screen, 150, 120, "Machine type mismatch. Load the state anyway?");
    np7->raise();
  } break;
  case 27: {
    auto *np7 = new ShowLoadWarning(
        screen, 150, 120, "Are you sure to remove the state in this slot?");
    np7->raise();
  } break;
  case 28: {
  ...
  case 30: {
    auto *np7 = new SetRefreshRate(
        screen, 0, 0, mainMenu.get_item("refresh_rate").get_text().c_str());
    np7->raise();
  } break;
  case 31:
    if (statusdrive > -1 && statusdrive < DOS_DRIVES && Drives[statusdrive]) {
      auto *np8 =
          new ShowDriveInfo(screen, 120, 50, MSG_Get("DRIVE_INFORMATION"));
      np8->raise();
    }
    break;
  case 32: {
    auto *np9 =
        new ShowDriveNumber(screen, 110, 70, MSG_Get("MOUNTED_DRIVE_NUMBER"));
    np9->raise();
  } break;
  case 33: {
    auto *np10 =
        new ShowIDEInfo(screen, 150, 70, MSG_Get("IDE_CONTROLLER_ASSIGNMENT"));
    np10->raise();
  } break;
  case 34: {
    auto *np11 = new ShowHelpIntro(
        screen, 70, 70, mainMenu.get_item("help_intro").get_text().c_str());
    np11->raise();
  } break;
  case 35: {
    auto *np12 = new ShowHelpAbout(
        screen, 110, 70, mainMenu.get_item("help_about").get_text().c_str());
    np12->raise();
  } break;
  case 36: {
    auto *np13 = new ShowHelpCommand(
        screen, 150, 120,
        (MSG_Get("HELP_COMMAND") + std::string(": ") + helpcmd).c_str());
    np13->raise();
  } break;
  case 37: {
  ...
  case 42: {
    auto *np16 =
        new ShowMidiDevice(screen, 150, 100, MSG_Get("CURRENT_MIDICONFIG"));
    np16->raise();
  } break;
#if C_DEBUG
  case 43: {
