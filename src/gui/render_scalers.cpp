...
/* SDL1 builds are subject to Mac OS X strange BGRA (alpha in low byte) order.
   The code in the #else case happens to work because most OSes put the alpha
   byte in the upper 32 bits, while on Mac OS X the blue channel is up there.
   Integer overflow will trash the blue channel in that case, without this
   alternate code. */
#define interp_w2(P0, P1, W0, W1)                                              \
  ((((uint64_t)(P0 & redblueMask) * (uint64_t)W0 +                             \
     (uint64_t)(P1 & redblueMask) * (uint64_t)W1) /                            \
    (uint64_t)(W0 + W1)) &                                                     \
   redblueMask) |                                                              \
      ((((uint64_t)(P0 & greenMask) * (uint64_t)W0 +                           \
         (uint64_t)(P1 & greenMask) * (uint64_t)W1) /                          \
        (uint64_t)(W0 + W1)) &                                                 \
       greenMask)
#define interp_w3(P0, P1, P2, W0, W1, W2)                                      \
  ((((uint64_t)(P0 & redblueMask) * (uint64_t)W0 +                             \
     (uint64_t)(P1 & redblueMask) * (uint64_t)W1 +                             \
     (uint64_t)(P2 & redblueMask) * (uint64_t)W2) /                            \
    (uint64_t)(W0 + W1 + W2)) &                                                \
   redblueMask) |                                                              \
      ((((uint64_t)(P0 & greenMask) * (uint64_t)W0 +                           \
         (uint64_t)(P1 & greenMask) * (uint64_t)W1 +                           \
         (uint64_t)(P2 & greenMask) * (uint64_t)W2) /                          \
        (uint64_t)(W0 + W1 + W2)) &                                            \
       greenMask)
#define interp_w4(P0, P1, P2, P3, W0, W1, W2, W3)                              \
  ((((uint64_t)(P0 & redblueMask) * (uint64_t)W0 +                             \
     (uint64_t)(P1 & redblueMask) * (uint64_t)W1 +                             \
     (uint64_t)(P2 & redblueMask) * (uint64_t)W2 +                             \
     (uint64_t)(P3 & redblueMask) * (uint64_t)W3) /                            \
    (uint64_t)(W0 + W1 + W2 + W3)) &                                           \
   redblueMask) |                                                              \
      ((((uint64_t)(P0 & greenMask) * (uint64_t)W0 +                           \
         (uint64_t)(P1 & greenMask) * (uint64_t)W1 +                           \
         (uint64_t)(P2 & greenMask) * (uint64_t)W2 +                           \
         (uint64_t)(P3 & greenMask) * (uint64_t)W3) /                          \
        (uint64_t)(W0 + W1 + W2 + W3)) &                                       \
       greenMask)
#else
#define interp_w2(P0, P1, W0, W1)                                              \
  ((((P0 & redblueMask) * W0 + (P1 & redblueMask) * W1) / (W0 + W1)) &         \
   redblueMask) |                                                              \
      ((((P0 & greenMask) * W0 + (P1 & greenMask) * W1) / (W0 + W1)) &         \
       greenMask)
#define interp_w3(P0, P1, P2, W0, W1, W2)                                      \
  ((((P0 & redblueMask) * W0 + (P1 & redblueMask) * W1 +                       \
     (P2 & redblueMask) * W2) /                                                \
    (W0 + W1 + W2)) &                                                          \
   redblueMask) |                                                              \
      ((((P0 & greenMask) * W0 + (P1 & greenMask) * W1 +                       \
         (P2 & greenMask) * W2) /                                              \
        (W0 + W1 + W2)) &                                                      \
       greenMask)
#define interp_w4(P0, P1, P2, P3, W0, W1, W2, W3)                              \
  ((((P0 & redblueMask) * W0 + (P1 & redblueMask) * W1 +                       \
     (P2 & redblueMask) * W2 + (P3 & redblueMask) * W3) /                      \
    (W0 + W1 + W2 + W3)) &                                                     \
   redblueMask) |                                                              \
      ((((P0 & greenMask) * W0 + (P1 & greenMask) * W1 +                       \
         (P2 & greenMask) * W2 + (P3 & greenMask) * W3) /                      \
        (W0 + W1 + W2 + W3)) &                                                 \
       greenMask)
#endif
...
ScalerSimpleBlock_t ScaleNormal2xDw = {
    ...
    4,
    2,
    {{Normal2xDw_8_8_L, Normal2xDw_8_15_L, Normal2xDw_8_16_L,
      Normal2xDw_8_32_L},
     ...
     {Normal2xDw_8_8_L, Normal2xDw_9_15_L, Normal2xDw_9_16_L,
      Normal2xDw_9_32_L}},
    {{Normal2xDw_8_8_R, Normal2xDw_8_15_R, Normal2xDw_8_16_R,
      Normal2xDw_8_32_R},
     ...
     {Normal2xDw_8_8_R, Normal2xDw_9_15_R, Normal2xDw_9_16_R,
      Normal2xDw_9_32_R}}};
...
ScalerSimpleBlock_t ScaleNormal2xDh = {
    ...
    2,
    4,
    {{Normal2xDh_8_8_L, Normal2xDh_8_15_L, Normal2xDh_8_16_L,
      Normal2xDh_8_32_L},
     ...
     {Normal2xDh_8_8_L, Normal2xDh_9_15_L, Normal2xDh_9_16_L,
      Normal2xDh_9_32_L}},
    {{Normal2xDh_8_8_R, Normal2xDh_8_15_R, Normal2xDh_8_16_R,
      Normal2xDh_8_32_R},
     ...
     {Normal2xDh_8_8_R, Normal2xDh_9_15_R, Normal2xDh_9_16_R,
      Normal2xDh_9_32_R}}};
...
/*ScalerSimpleBlock_t ScaleNormal6x = {
    ...
    GFX_CAN_8|GFX_CAN_15|GFX_CAN_16|GFX_CAN_32|GFX_NORMALSCALE,
    6,6,{
{    Normal6x_8_8_L,        Normal6x_8_15_L ,    Normal6x_8_16_L ,
Normal6x_8_32_L }, {    nullptr,               Normal6x_15_15_L,
Normal6x_15_16_L,    Normal6x_15_32_L}, {    nullptr, Normal6x_16_15_L,
Normal6x_16_16_L,    Normal6x_16_32_L}, {    nullptr, Normal6x_32_15_L,
Normal6x_32_16_L,    Normal6x_32_32_L}, {    Normal6x_8_8_L, Normal6x_9_15_L ,
Normal6x_9_16_L ,    Normal6x_9_32_L }
},{
{    Normal6x_8_8_R,        Normal6x_8_15_R ,    Normal6x_8_16_R ,
Normal6x_8_32_R }, {    nullptr,               Normal6x_15_15_R,
Normal6x_15_16_R,    Normal6x_15_32_R}, {    nullptr, Normal6x_16_15_R,
Normal6x_16_16_R,    Normal6x_16_32_R}, {    nullptr, Normal6x_32_15_R,
Normal6x_32_16_R,    Normal6x_32_32_R}, {    Normal6x_8_8_R, Normal6x_9_15_R ,
...
ScalerComplexBlock_t ScaleHQ2x = {"HQ2x",
                                  GFX_CAN_15 | GFX_CAN_16 | GFX_CAN_32 |
                                      GFX_RGBONLY,
                                  2,
                                  2,
...
ScalerComplexBlock_t ScaleHQ3x = {"HQ3x",
                                  GFX_CAN_15 | GFX_CAN_16 | GFX_CAN_32 |
                                      GFX_RGBONLY,
                                  3,
                                  3,
