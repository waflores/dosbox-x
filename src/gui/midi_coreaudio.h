...
public:
  ...
  bool Open(const char *conf) {
    ...
    RequireNoErr(
        AUGraphConnectNodeInput(m_auGraph, synthNode, 0, outputNode, 0));
...
#if USE_DEPRECATED_COREAUDIO_API
    RequireNoErr(
        AUGraphGetNodeInfo(m_auGraph, synthNode, NULL, NULL, NULL, &m_synth));
#else
    ...
    if (conf && conf[0]) {
      ...
      if (!err) {
        err = AudioUnitSetProperty(m_synth, kMusicDeviceProperty_SoundBankFSRef,
                                   kAudioUnitScope_Global, 0, &soundfontRef,
                                   sizeof(soundfontRef));
      }
#else
      // kMusicDeviceProperty_SoundBankFSRef is present on 10.6+, but
      // kMusicDeviceProperty_SoundBankURL was added in 10.5 as a future proof
      // replacement
      CFURLRef url = CFURLCreateFromFileSystemRepresentation(
          kCFAllocatorDefault, (const UInt8 *)soundfont, strlen(soundfont),
          false);
      if (url) {
        err =
            AudioUnitSetProperty(m_synth, kMusicDeviceProperty_SoundBankURL,
      ...
      if (!err) {
        ...
        // NTS: Prior comments said that continuing to use the core audio synth
        // at this point
        //      could cause a crash. Maybe that was an issue 10 years ago with
        //      Mac OS X 10.6 but as far as I can tell with Big Sur, the synth
        //      will correctly use the default sound font in this case without
        //      any issue. --J.C.
        LOG_MSG("MIDI:coreaudio: Error loading CoreAudio SoundFont %s, using "
                "MIDI synth with default soundfont",
                soundfont);
      }
    }
  ...
  void PlayMsg(uint8_t *msg) {
    if (msg[0] == 0xff) {
      ...
      for (unsigned int i = 0; i < 16; i++) {
        ...
        MusicDeviceMIDIEvent(m_synth, 0xB0 + i, 121, 0,
                             0); // Reset the controllers
  ...
  void PlaySysex(uint8_t *sysex, Bitu len) {
    if (roland_gs_sysex) {
      if (sysex[1] == 0x41 /*Roland*/ && sysex[3] == 0x42 /*GS*/ &&
          sysex[4] == 0x12 /*Send*/ && len >= 9) {
        const uint32_t addr = ((uint32_t)sysex[5] << 16) +
                              ((uint32_t)sysex[6] << 8) + (uint32_t)sysex[7];
  ...
  void SetVolume(uint8_t vol) {
    /* Despite the Apple Developer site and their TERRIBLE DOCUMENTATION, I was
       able to figure out these magic incantations that make setting the MIDI
       synthesizer volume possible.

       If you're going to document kMusicDeviceParam_Volume, document what it
       means, how you use it, and even if you figure out that it has something
       to do with AudioUnitSetParameters, document what the float value is
       supposed to mean. For fucks sake, even Microsoft who is traditionally
       terrible at documentation has figured out how to document an API! I had
       to figure out through trial and error that the float value was a decibel
       value! */
    if (vol > 127)
      vol = 127;
    Float32 fvol = vol > 0 ? (((Float32)vol - 127.0) / 2.0)
                           : -999999.0; /* The volume value is decibels? Thanks
                                           for NOT documenting this Apple! */
    OSErr err = 0;

    err = AudioUnitSetParameter(m_synth, kMusicDeviceParam_Volume,
                                kAudioUnitScope_Global, 0, fvol, 0);
  }
};
