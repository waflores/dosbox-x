...
public:
  ...
  uint32_t get() {
    if (startpos == endpos)
      return 0;
    uint32_t data =
        (uint32_t)ringBuffer[startpos]; /* <- FIXME: Um.... really? */
    startpos++;
...
private:
  ...
  bool load_rom_set(std::string romDir, int model) {
    ...
    if (model == 2 ||
        MT32EMU_RC_ADDED_CONTROL_ROM != service->addROMFile(pathName)) {
      strcpy(pathName, romDir.c_str());
    ...
    if (model == 2 ||
        MT32EMU_RC_ADDED_PCM_ROM != service->addROMFile(pathName)) {
      strcpy(pathName, romDir.c_str());
  ...
  static mt32emu_report_handler_i getReportHandlerInterface() {
    ...
    static const mt32emu_report_handler_i_v0 REPORT_HANDLER_V0_IMPL = {
        ReportHandler::getReportHandlerVersionID, ReportHandler::printDebug,
        ReportHandler::onErrorControlROM, ReportHandler::onErrorPCMROM,
    ...
    while (!stopProcessing) {
      ...
      if ((framesToRender == 0) || ((framesToRender < minimumRenderFrames) &&
        ...
        service->renderBit16s(audioBuffer + renderPosSnap,
                              (MT32Emu::uint32_t)framesToRender);
  ...
  uint32_t inline getMidiEventTimestamp() {
    return service->convertOutputToSynthTimestamp(
        uint32_t(playedBuffers * framesPerAudioBuffer + (playPos >> 1)));
  }

public:
  MidiHandler_mt32() : chan(NULL), service(NULL), thread(NULL), open(false) {}

  ~MidiHandler_mt32() { MidiHandler_mt32::Close(); }
  ...
  const char *GetName(void) override { return "mt32"; }
  ...
  bool Open(const char *conf) override {
    ...
    if (!load_rom_set(romDir, model)) {
      ...
      if (!load_rom_set(romDir, model)) {
        ...
        if (!load_rom_set(romDir, model)) {
          ...
          LOG_MSG("The ROM files are: CM32L_CONTROL.ROM and CM32L_PCM.ROM or "
    ...
    service->setAnalogOutputMode(
        (MT32Emu::AnalogOutputMode)section->Get_int("mt32.analog"));
    ...
    service->setSamplerateConversionQuality(
        (MT32Emu::SamplerateConversionQuality)section->Get_int(
            "mt32.src.quality"));
    const uint32_t mt32_samplerate = 32000U;
    const uint32_t mt32_sampleratemodes[] = {mt32_samplerate, mt32_samplerate,
                                             mt32_samplerate / 2 * 3,
                                             mt32_samplerate * 3, 0};
    if (sampleRate != mt32_sampleratemodes[section->Get_int("mt32.analog")])
      LOG_MSG(
          "MIDI: Warning sampling rates of \"mt32.rate\" and \"mt32.analog\" "
          "settings are different. May result in incorrect pitch.");
    ...
    service->setReverbOutputGain(0.01f *
                                 section->Get_int("mt32.reverb.output.gain"));
    service->setDACInputMode(
        (MT32Emu::DACInputMode)section->Get_int("mt32.dac"));
    ...
    if (section->Get_bool("mt32.engage.channel1")) {
      uint8_t channelAssignmentSysex[] = {0x10, 0x00, 0x0d, 0x00, 0x01, 0x02,
                                          0x03, 0x04, 0x05, 0x06, 0x07, 0x09};
      service->writeSysex(16, channelAssignmentSysex,
                          sizeof channelAssignmentSysex);
    }
    ...
    if (renderInThread) {
      ...
      if (latency <= chunkSize) {
        latency = 2 * chunkSize;
        LOG_MSG("MT32: chunk length must be less than prebuffer length, "
                "prebuffer length reset to %i ms.",
                latency);
      }
      ...
      service->renderBit16s(audioBuffer,
                            (MT32Emu::uint32_t)(framesPerAudioBuffer - 1));
  ...
  void PlayMsg(uint8_t *msg) override {
    if (renderInThread) {
      service->playMsgAt(SDL_SwapLE32(*(uint32_t *)msg),
  ...
  void PlaySysex(uint8_t *sysex, Bitu len) override {
    if (renderInThread) {
      service->playSysexAt(sysex, (MT32Emu::uint32_t)len,
                           getMidiEventTimestamp());
    } else {
