...
void RENDER_SetPal(uint8_t entry, uint8_t red, uint8_t green, uint8_t blue) {
  if (GFX_GetBShift() == 0) {
    GFX_palette32bpp[entry] = ((uint32_t)red << (uint32_t)16) +
                              ((uint32_t)green << (uint32_t)8) +
                              ((uint32_t)blue << (uint32_t)0);
  } else {
    GFX_palette32bpp[entry] = ((uint32_t)blue << (uint32_t)16) +
                              ((uint32_t)green << (uint32_t)8) +
...
#endif
static inline bool
cacheHit_AVX2(const Bitu *src, Bitu *cache, Bits count) {
...
#if defined(__SSE__) && !(defined(_M_AMD64) || defined(__amd64__) ||           \
                          defined(__e2k__) || defined(_WIN32_WINDOWS))
...
bool RENDER_StartUpdate(void) {
  ...
  if (GCC_UNLIKELY(render.scale.clearCache)) {
    ...
    // Will always have to update the screen with this one anyway, so let's
    // update already
    if (GCC_UNLIKELY(
...
void RENDER_EndUpdate(bool abort) {
  ...
  if (GCC_UNLIKELY(CaptureState & (CAPTURE_IMAGE | CAPTURE_VIDEO))) {
    ...
    CAPTURE_AddImage(render.src.width, render.src.height, render.src.bpp, pitch,
                     flags, fps, (uint8_t *)&scalerSourceCache,
  ...
  if (render.scale.outWrite) {
    ...
    // works only with Direct3D output (GFX_StartUpdate() was probably not even
    // called)
    if (RENDER_GetForceUpdate())
      GFX_EndUpdate(nullptr);
  }
  render.frameskip.index =
      (render.frameskip.index + 1) & (RENDER_SKIP_CACHE - 1);
  render.updating = false;
...
void RENDER_Reset(void) {
...
#else
  if (render.aspect == ASPECT_TRUE && !render.aspectOffload &&
      !(sdl_xbrz.enable && sdl_xbrz.scale_on))
#endif
  {
  ...
  if (render.scale.prompt && (!dblh || !dblw) &&
      !((dblh || dblw) && !render.scale.hardware) &&
      scalerOpTV != render.scale.op && scaler != "none" &&
      strncasecmp(scaler.c_str(), "normal", 6) && !render.scale.forced &&
      sdl.desktop.want_type != SCREEN_TTF) {
    std::string message =
        formatString(MSG_Get("SCALER_LOAD_WARN"), scaler.c_str());
    render.scale.forced = systemmessagebox("Loading scaler", message.c_str(),
                                           "yesno", "question", 1);
  ...
  if (complexBlock) {
    ...
    if ((simpleBlock->gfxFlags & GFX_NORMALSCALE) && render.scale.hardware) {
      /* Normal (pixel duplication) scalers should not be used for hardware
       * scaling, because the hardware scaling will do all the scaling, AND so
       * that users who use CRT scanline emulation shaders can work with the raw
       * pixels and scanlines of the source video memory.
       *
       * This is not related to the "doublescan" option (on by default) that VGA
       * emulation uses to emulate the hardware scan line duplication used to
       * ensure that the raster to the monitor is 400-line or 480-line nor is it
       * related to the double scan used in PC-98 200-line 8/16-color graphics
       * modes to render as a 400-line raster.
       *
       * A side effect is that all your low res modes will look blurry when
       * scaled up by your GPU. If you want a crisper look from the pixel
       * duplication you're used to, use the none, normal2x, etc. scalers that
       * are default anyway, OR use a shader that processes the video the way
       * you prefer to see it.
       *
       * For this reason, the only difference in hardware_none, hardware2x, etc.
       * is how much to multiply the GFX window scale without duplicating the
       * pixels in software using any of the normal scalers INCLUDING the normal
       * scalers used to double pixels horizontally and/or vertically for
       * 200-line modes or 320-pixel wide modes.
       *
       * Because of integer scaling, the scale factor is restricted to a
       * multiple of 2 if the video mode would normally double pixels in one
       * direction only but not both, such as CGA 80x25 text or CGA/EGA 640x200
       * graphics where normal scaler would normally only double pixels in the Y
       * direction (height). */
      simpleBlock = &ScaleNormal1x;
      xscale = 1;
      yscale = 1;
  ...
  if (gfx_flags & GFX_SCALING) {
    ...
    if (render.scale.hardware) {
      LOG_MSG("Output does not support hardware scaling, switching to normal "
              "scalers");
...
void RENDER_SetSize(Bitu width, Bitu height, Bitu bpp, float fps,
                    double scrn_ratio) {
  RENDER_Halt();
  if (!width || !height || width > SCALER_MAXWIDTH ||
      height > SCALER_MAXHEIGHT) {
    LOG(LOG_MISC, LOG_WARN)("RENDER_SetSize() rejected video mode %u x %u",
  ...
  bool dblw = false;
  bool dblh = false;
  bool do_not_dblh = (IS_VGA_ARCH && vga.draw.doublescan_set) ||
                     machine == MCH_MDA || machine == MCH_HERC;
  ...
  LOG_MSG("pixratio %1.3f, dw %s, dh %s", ratio, dblw ? "true" : "false",
          dblh ? "true" : "false");

  /* this must be done after dblw/dblh so extra room can be added without
   * screwing up the screen. debug information is drawn into the buffer
   * pre-scaler to make sure that if the user wants it in still image or video
   * capture, they can. */
  ...
  if (ratio > 1.0) {
    ...
    // This would alter the width of the screen, we don't care about rounding
    // errors here
  }
...
// extern void GFX_SetTitle(int32_t cycles, int frameskip, Bits timing, bool
// paused);
...
bool RENDER_GetForceUpdate(void) { return render.forceUpdate; }

void RENDER_SetForceUpdate(bool f) { render.forceUpdate = f; }
...
static bool RENDER_GetShader(std::string &shader_path, char *old_src) {
  ...
  if (!empty)
    ;
  else if (shader_path == "advinterp2x")
    buf << advinterp2x_glsl;
  else if (shader_path == "advinterp3x")
    buf << advinterp3x_glsl;
  else if (shader_path == "advmame2x")
    buf << advmame2x_glsl;
  else if (shader_path == "advmame3x")
    buf << advmame3x_glsl;
  else if (shader_path == "rgb2x")
    buf << rgb2x_glsl;
  else if (shader_path == "rgb3x")
    buf << rgb3x_glsl;
  else if (shader_path == "scan2x")
    buf << scan2x_glsl;
  else if (shader_path == "scan3x")
    buf << scan3x_glsl;
  else if (shader_path == "tv2x")
    buf << tv2x_glsl;
  else if (shader_path == "tv3x")
    buf << tv3x_glsl;
  else if (shader_path == "sharp")
    buf << sharp_glsl;
  ...
  if (!buf.str().empty()) {
    ...
    if (first_shell) {
      ...
      if (!pre_defs.empty()) {
        // if "#version" occurs it must be before anything except comments and
        // whitespace
    ...
    if (old_src == NULL || s != old_src) {
      src = strdup(s.c_str());
      if (src == NULL)
        LOG_MSG("WARNING: Couldn't copy shader source");
    } else
      src = old_src;
  } else
    src = NULL;
...
void RENDER_OnSectionPropChange(Section *x) {
  ...
  if (s_aspect == "true" || s_aspect == "1" || s_aspect == "yes")
    render.aspect = ASPECT_TRUE;
#if C_SURFACE_POSTRENDER_ASPECT
  if (s_aspect == "nearest")
    render.aspect = ASPECT_NEAREST;
  if (s_aspect == "bilinear")
    render.aspect = ASPECT_BILINEAR;
#endif
  ...
  if (render.aspect != p_aspect || vga.draw.doublescan_set != p_doublescan ||
    ...
    RENDER_CallBack(GFX_CallBackReset);
  if (vga.draw.doublescan_set != p_doublescan ||
      vga.draw.char9_set != p_char9 || vga.draw.modeswitch_set != p_modeswitch)
    VGA_StartResize();

  mainMenu.get_item("vga_9widetext")
      .check(vga.draw.char9_set)
      .refresh_item(mainMenu);
  mainMenu.get_item("doublescan")
      .check(vga.draw.doublescan_set)
      .refresh_item(mainMenu);
  mainMenu.get_item("mapper_aspratio")
      .check(render.aspect)
      .refresh_item(mainMenu);

#if C_SDL2
  mainMenu.get_item("modeswitch")
      .check(vga.draw.modeswitch_set)
      .refresh_item(mainMenu);
#endif
...
void RENDER_UpdateScalerMenu(void) {
  ...
  for (size_t i = 0; scaler_menu_opts[i][0] != NULL; i++) {
    ...
    mainMenu.get_item(name)
        .check(scaler == scaler_menu_opts[i][0])
        .refresh_item(mainMenu);
  }
}
...
void RENDER_UpdateFromScalerSetting(void) {
  ...
  if (scaler == "none") {
    ...
    render.scale.size = 3;
    render.scale.hardware = false;
  } else if (scaler == "normal4x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 4;
    render.scale.hardware = false;
  } else if (scaler == "normal5x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 5;
    render.scale.hardware = false;
  }
    ...
    render.scale.op = scalerOpGray;
    render.scale.size = 1;
    render.scale.hardware = false;
  } else if (scaler == "gray2x") {
    render.scale.op = scalerOpGray;
    render.scale.size = 2;
    render.scale.hardware = false;
  }
#endif
  else if (scaler == "hardware_none") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 1;
    render.scale.hardware = true;
  } else if (scaler == "hardware2x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 2;
    render.scale.hardware = true;
  } else if (scaler == "hardware3x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 3;
    render.scale.hardware = true;
  } else if (scaler == "hardware4x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 4;
    render.scale.hardware = true;
  } else if (scaler == "hardware5x") {
    render.scale.op = scalerOpNormal;
    render.scale.size = 5;
    render.scale.hardware = true;
  }
...
#if C_XBRZ
  if (old_xBRZ_enable != sdl_xbrz.enable)
    reset = true;
#endif
  if (p_forced != render.scale.forced)
    reset = true;
  if (p_size != render.scale.size)
    reset = true;
  if (p_hardware != render.scale.hardware)
    reset = true;
  if (p_op != render.scale.op)
    reset = true;
...
std::string LoadGLShader(Section_prop *section) {
  ...
  if (f.empty() || f == "none" || f == "default") {
    ...
    std::string path = std::string("glshaders") + CROSS_FILESPLIT + f, pathcfg,
                pathres;
    if (!RENDER_GetShader(path, (char *)shader_src.c_str())) {
      std::string exePath = GetDOSBoxXPath();
      if (exePath.size())
        path = exePath + std::string("glshaders") + CROSS_FILESPLIT + f;
      else
        path = "";
    } else {
      if (initgl == 2)
    ...
    if (path.size() && !RENDER_GetShader(path, (char *)shader_src.c_str())) {
      ...
      if (!RENDER_GetShader(pathcfg, (char *)shader_src.c_str()) &&
          !RENDER_GetShader(pathres, (char *)shader_src.c_str()) &&
          (sh->realpath == f ||
           !RENDER_GetShader(f, (char *)shader_src.c_str()))) {
        sh->SetValue("none");
  ...
  if (shader_src.size() && shader_src.c_str() != render.shader_src)
    shader_src = "";
  return shader_src;
}
#endif
...
void setAspectRatio(Section_prop *section) {
  char *ratiostr = (char *)section->Get_string("aspect_ratio"),
       *p = strchr(ratiostr, ':');
  if (p) {
    *p = 0;
    int ratiox = atoi(ratiostr);
    int ratioy = atoi(p + 1);
    aspect_ratio_x = ratiox > 0 && ratioy > 0
                         ? ratiox
                         : (ratiox == -1 && ratioy == -1 ? -1 : 0);
    aspect_ratio_y = ratiox > 0 && ratioy > 0
                         ? ratioy
                         : (ratiox == -1 && ratioy == -1 ? -1 : 0);
    *p = ':';
  } else
...
void RENDER_Init() {
  ...
  control->GetSection("render")->onpropchange.push_back(
      &RENDER_OnSectionPropChange);
  ...
  MAPPER_AddHandler(DecreaseFrameSkip, MK_nothing, 0, "decfskip",
                    "Decrease frameskip");
  MAPPER_AddHandler(IncreaseFrameSkip, MK_nothing, 0, "incfskip",
                    "Increase frameskip");

  DOSBoxMenu::item *item;

  MAPPER_AddHandler(&AspectRatio_mapper_shortcut, MK_nothing, 0, "aspratio",
                    "Fit to aspect ratio", &item);
  item->set_text("Fit to aspect ratio");

  MAPPER_AddHandler(ForceScaler, MK_nothing, 0, "fscaler", "Force scaler",
                    &item);
  item->set_text("Force scaler");

  if (machine == MCH_HERC || machine == MCH_MDA) {
    void HercBlend(bool pressed), CycleHercPal(bool pressed);
    MAPPER_AddHandler(CycleHercPal, MK_f7, MMOD1, "hercpal",
                      "Hercules Palette");
    MAPPER_AddHandler(HercBlend, MK_f8, MMOD1, "hercblend",
                      "Hercules Blending");
  }

  if (machine == MCH_CGA || machine == MCH_MCGA || machine == MCH_PCJR ||
      machine == MCH_AMSTRAD) {
    if (!mono_cga) {
      void IncreaseHue(bool pressed), DecreaseHue(bool pressed),
          CGAModel(bool pressed), Composite(bool pressed);
      MAPPER_AddHandler(DecreaseHue, MK_f7, MMOD1 | MMOD3, "dechue",
                        "Decrease Hue");
      MAPPER_AddHandler(IncreaseHue, MK_f8, MMOD1 | MMOD3, "inchue",
                        "Increase Hue");
      ...
      MAPPER_AddHandler(CycleMonoCGAPal, MK_f7, MMOD1, "monocgapal",
                        "Mono CGA Palette");
      MAPPER_AddHandler(CycleMonoCGABright, MK_f8, MMOD1, "monocgabri",
                        "Mono CGA Brightness");
    }
  }
  ...
  std::string cline;
  std::string scaler;
  // Check for commandline parameters and parse them through the configclass so
  // they get checked against allowed values
  ...
  //  Only ReInit when there is a src.bpp (fixes crashes on startup and directly
  //  changing the scaler without a screen specified yet)
...
private:
  ...
  void setBytes(std::istream &stream) override {
    ...
    if (render.aspect == ASPECT_FALSE) {
      ...
      RENDER_SetSize(render.src.width, render.src.height, render.src.bpp,
