...
#if !defined(C_SDL2) &&                                                        \
    defined(MACOSX) /* SDL1 builds are subject to Mac OS X strange BGRA (alpha \
                       in low byte) order */
...
#define PMAKE(_VAL)                                                            \
  (((_VAL >> 8) & 0x001F) | ((_VAL >> 7) & 0x01C0) | ((_VAL << 9) & 0xFE00) |  \
   ((_VAL << 4) & 0x0020))
...
#define PMAKE(_VAL) ((_VAL & 31) | ((_VAL & ~31) << 1) | ((_VAL & 0x0200) >> 4))
    ...
    !defined(C_SDL2) /* Mac OS X Intel builds use a weird RGBA order (alpha in \
                        the low 8 bits) */
#define PMAKE(_VAL)                                                            \
  (((_VAL & (31u << 10u)) << 1u) | ((_VAL & (31u << 5u)) << 14u) |             \
   ((_VAL & 31u) << 27u))
#else
#define PMAKE(_VAL)                                                            \
  (((_VAL & (31u << 10u)) << 9u) | ((_VAL & (31u << 5u)) << 6u) |              \
   ((_VAL & 31u) << 3u) | ((_VAL & (7 << 12)) << 4) |                          \
...
#define PMAKE(_VAL)                                                            \
  (((_VAL >> 8) & 0x001F) | ((_VAL >> 9) & 0x0060) | ((_VAL << 7) & 0x7F80))
...
#define PMAKE(_VAL) (_VAL)
    ...
    !defined(C_SDL2) /* Mac OS X Intel builds use a weird RGBA order (alpha in \
                        the low 8 bits) */
#define PMAKE(_VAL)                                                            \
  (((_VAL & (31u << 11u)) << 0u) | ((_VAL & (63u << 5u)) << 13u) |             \
   ((_VAL & 31u) << 27u))
#else
#define PMAKE(_VAL)                                                            \
...
#define PMAKE(_VAL)                                                            \
  (PTYPE)(((_VAL >> 27) & 0x001F) | ((_VAL >> 13) & 0x07E0) | (_VAL & 0xF800))
...
#define PMAKE(_VAL)                                                            \
  (PTYPE)(((_VAL & (31u << 19u)) >> 9u) | ((_VAL & (31u << 11u)) >> 6u) |      \
          ((_VAL & (31u << 3u)) >> 3u))
#elif DBPP == 16 // RRRRRrrrGGGGGGggBBBBBbbb -> RRRRRGGGGGGBBBBB
#define PMAKE(_VAL)                                                            \
  (PTYPE)(((_VAL & (31u << 19u)) >> 8u) | ((_VAL & (63u << 10u)) >> 5u) |      \
          ((_VAL & (31u << 3u)) >> 3u))
#elif DBPP == 32
#define PMAKE(_VAL) (_VAL)
#endif
...
static inline void conc3d(Cache, SBPP, DBPP)(const void *s) {
#if !defined(_MSC_VER) /* Microsoft C++ thinks this is a failed attempt at a   \
                          function call---it's not */
  ...
  /* This should also copy the surrounding pixels but it looks nice enough
   * without */
  for (b = 0; b < render.scale.blocks; b++) {
    ...
    for (Bitu x = 0; x < SCALER_BLOCKSIZE; x++) {
      ...
      if (pixel != fc[x]) {
#else
    for (Bitu x = 0; x < SCALER_BLOCKSIZE;
...
#define SCALERFUNC							   \
        ...
        line5[2] = P; \
        line5[3] = P; \
        line5[4] = P; \ line5[5] = P; #include "render_simple.h" #undef
SCALERNAME #undef SCALERWIDTH #undef SCALERHEIGHT #undef SCALERFUNC
*/
...
#define SCALERFUNC                                                             \
  {                                                                            \
    PTYPE halfpixel =                                                          \
        (((uint64_t)(P & redblueMask) * (uint64_t)5) >> (uint64_t)3) &         \
        redblueMask;                                                           \
    halfpixel |= (((uint64_t)(P & greenMask) * (uint64_t)5) >> (uint64_t)3) &  \
                 greenMask;                                                    \
    line0[0] = P;                                                              \
    line0[1] = P;                                                              \
    line0[2] = P;                                                              \
    ...
    line1[1] = halfpixel;                                                      \
    line1[2] = halfpixel;                                                      \
    halfpixel = (((uint64_t)(P & redblueMask) * (uint64_t)5) >> (uint64_t)4) & \
                redblueMask;                                                   \
    halfpixel |= (((uint64_t)(P & greenMask) * (uint64_t)5) >> (uint64_t)4) &  \
                 greenMask;                                                    \
    line2[0] = halfpixel;                                                      \
    line2[1] = halfpixel;                                                      \
    line2[2] = halfpixel;                                                      \
...
#define SCALERHEIGHT 1
#define SCALERFUNC                                                             \
  PTYPE _red = (P & redMask) >> redShift,                                      \
        _green = (P & greenMask) >> greenShift,                                \
        _blue = (P & blueMask) >> blueShift;                                   \
  double _gray = 0.2125 * (double)_red + 0.7154 * (double)_green +             \
                 0.0721 * (double)_blue;                                       \
  PTYPE _value = _gray > 255 ? 0xff : (uint8_t)(_gray);                        \
  line0[0] =                                                                   \
      ((_value << redShift) | (_value << greenShift) | (_value) << blueShift);
#include "render_simple.h"
#undef SCALERNAME
#undef SCALERWIDTH
#undef SCALERHEIGHT
