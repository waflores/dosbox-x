...
#if SBPP == 32
#define RGBtoYUV(c)                                                            \
  _RGBtoYUV[((c & 0xf80000) >> 8) | ((c & 0x00fc00) >> 5) |                    \
            ((c & 0x0000f8) >> 3)]
#else
#define RGBtoYUV(c) _RGBtoYUV[c]
#endif

inline void conc2d(Hq3x, SBPP)(PTYPE *line0, PTYPE *line1, PTYPE *line2,
                               const PTYPE *fc) {
  ...
  if (_RGBtoYUV == nullptr)
    conc2d(InitLUTs, SBPP)();

  uint32_t pattern = 0;
  const uint32_t YUV4 = RGBtoYUV(C4);

  if (C4 != C0 && diffYUV(YUV4, RGBtoYUV(C0)))
    pattern |= 0x0001;
  if (C4 != C1 && diffYUV(YUV4, RGBtoYUV(C1)))
    pattern |= 0x0002;
  if (C4 != C2 && diffYUV(YUV4, RGBtoYUV(C2)))
    pattern |= 0x0004;
  if (C4 != C3 && diffYUV(YUV4, RGBtoYUV(C3)))
    pattern |= 0x0008;
  if (C4 != C5 && diffYUV(YUV4, RGBtoYUV(C5)))
    pattern |= 0x0010;
  if (C4 != C6 && diffYUV(YUV4, RGBtoYUV(C6)))
    pattern |= 0x0020;
  if (C4 != C7 && diffYUV(YUV4, RGBtoYUV(C7)))
    pattern |= 0x0040;
  if (C4 != C8 && diffYUV(YUV4, RGBtoYUV(C8)))
    pattern |= 0x0080;
