...
#define TTF_HANDLE_STYLE_BOLD(font)                                            \
  (((font)->style & TTF_STYLE_BOLD) && !((font)->face_style & TTF_STYLE_BOLD))
#define TTF_HANDLE_STYLE_ITALIC(font)                                          \
...
#define TTF_HANDLE_STYLE_STRIKETHROUGH(font)                                   \
  ((font)->style & TTF_STYLE_STRIKETHROUGH)

/* Font styles that does not impact glyph drawing */
#define TTF_STYLE_NO_GLYPH_CHANGE                                              \
...
*/
static __inline__ int TTF_Glyph_underline_top_row(TTF_Font *font,
                                                  c_glyph *glyph) {
...
/* Gets the bottom row of the underline. for a given glyph. The outline
   ...
   font_value - font->ascent + glyph->maxy
*/
static __inline__ int TTF_Glyph_underline_bottom_row(TTF_Font *font,
                                                     c_glyph *glyph) {
...
/* Gets the top row of the strikethrough for a given glyph. The outline
   ...
   font_value - font->ascent + glyph->maxy
*/
static __inline__ int TTF_Glyph_strikethrough_top_row(TTF_Font *font,
                                                      c_glyph *glyph) {
  return TTF_strikethrough_top_row(font) - font->ascent + glyph->maxy;
}

static void TTF_initLineMectrics(const TTF_Font *font,
                                 const SDL_Surface *textbuf, const int row,
                                 Uint8 **pdst, int *pheight) {
  Uint8 *dst;
  int height;
...
*/
static void TTF_drawLine_Solid(const TTF_Font *font, const SDL_Surface *textbuf,
                               const int row) {
  int line;
...
/* Draw a shaded line of underline_height (+ optional outline)
   ...
   outline into account.
*/
static void TTF_drawLine_Shaded(const TTF_Font *font,
                                const SDL_Surface *textbuf, const int row) {
  int line;
  Uint8 *dst_check = (Uint8 *)textbuf->pixels + textbuf->pitch * textbuf->h;
  Uint8 *dst;
...
/* Draw a blended line of underline_height (+ optional outline)
   ...
   outline into account.
*/
static void TTF_drawLine_Blended(const TTF_Font *font,
                                 const SDL_Surface *textbuf, const int row,
                                 const Uint32 color) {
  int line;
  Uint32 *dst_check =
 ...
 */
void TTF_ByteSwappedUNICODE(int swapped) { TTF_byteswapped = swapped; }
...
TTF_Font *TTF_OpenFontIndexRW(SDL_RWops *src, int freesrc, int ptsize,
                              long index) {
  TTF_Font *font;
  FT_Error error;
  FT_Face face;
  ...
  for (i = 0; i < face->num_charmaps; i++) {
    ...
    if ((charmap->platform_id == 3 &&
         charmap->encoding_id == 1) /* Windows Unicode */
...
static FT_Error Load_Glyph(TTF_Font *font, Uint16 ch, c_glyph *cached,
                           int want) {
  FT_Face face;
  FT_Error error;
  FT_GlyphSlot glyph;
  ...
  if (ttf.SDL_fontb != NULL && TTF_HANDLE_STYLE_BOLD(font) &&
      !TTF_HANDLE_STYLE_ITALIC(font))
    font = ttf.SDL_fontb;
  else if (ttf.SDL_fonti != NULL && !TTF_HANDLE_STYLE_BOLD(font) &&
           TTF_HANDLE_STYLE_ITALIC(font))
    font = ttf.SDL_fonti;
  else if (ttf.SDL_fontbi != NULL && TTF_HANDLE_STYLE_BOLD(font) &&
           TTF_HANDLE_STYLE_ITALIC(font))
    font = ttf.SDL_fontbi;
  ...
  if (((want & CACHED_BITMAP) && !(cached->stored & CACHED_BITMAP)) ||
    ...
    if ((font->outline > 0) && glyph->format != FT_GLYPH_FORMAT_BITMAP) {
      ...
      FT_Glyph_Stroke(&bitmap_glyph, stroker,
                      1 /* delete the original glyph */);
      FT_Stroker_Done(stroker);
      /* Render the glyph */
      error = FT_Glyph_To_Bitmap(
          &bitmap_glyph, mono ? ft_render_mode_mono : ft_render_mode_normal, 0,
          1);
      if (error) {
      ...
      /* Render the glyph */
      error = FT_Render_Glyph(glyph, mono ? ft_render_mode_mono
...
int TTF_FontHeight(const TTF_Font *font) { return (font->height); }

int TTF_FontAscent(const TTF_Font *font) { return (font->ascent); }

int TTF_FontDescent(const TTF_Font *font) { return (font->descent); }

int TTF_FontLineSkip(const TTF_Font *font) { return (font->lineskip); }

int TTF_GetFontKerning(const TTF_Font *font) { return (font->kerning); }

void TTF_SetFontKerning(TTF_Font *font, int allowed) {
  font->kerning = allowed;
}

long TTF_FontFaces(const TTF_Font *font) { return (font->face->num_faces); }
...
int TTF_SizeUNICODE(TTF_Font *font, const Uint16 *text, int *w, int *h) {
  ...
  for (ch = text; *ch; ++ch) {
    ...
    if (use_kerning && prev_index && glyph->index) {
      FT_Vector delta;
      FT_Get_Kerning(font->face, prev_index, glyph->index, ft_kerning_default,
                     &delta);
      x += delta.x >> 6;
    }
...
SDL_Surface *TTF_RenderUNICODE_Solid(TTF_Font *font, const Uint16 *text,
  ...
  for (ch = text; *ch; ++ch) {
    ...
    for (row = 0; row < (int)current->rows; ++row) {
      ...
      dst = (Uint8 *)textbuf->pixels + (row + glyph->yoffset) * textbuf->pitch +
            xstart + glyph->minx;
...
SDL_Surface *TTF_RenderGlyph_Solid(TTF_Font *font, Uint16 ch, SDL_Color fg) {
  ...
  textbuf = SDL_CreateRGBSurface(SDL_SWSURFACE, glyph->bitmap.width, row, 8, 0,
                                 0, 0, 0);
  if (!textbuf) {
    return (NULL);
  }
...
SDL_Surface *TTF_RenderGlyph_Shaded(TTF_Font *font, Uint16 ch, SDL_Color fg,
  ...
  textbuf = SDL_CreateRGBSurface(SDL_SWSURFACE, glyph->pixmap.width, row, 8, 0,
                                 0, 0, 0);
  if (!textbuf) {
    return NULL;
  }
...
SDL_Surface *TTF_RenderUNICODE_Blended(TTF_Font *font, const Uint16 *text,
                                       SDL_Color fg) {
  int xstart;
  int width, height;
  SDL_Surface *textbuf;
  ...
  /* Create the target surface */
  textbuf = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0x00FF0000,
  ...
  for (ch = text; *ch; ++ch) {
    ...
    for (row = 0; row < (int)glyph->pixmap.rows; ++row) {
      ...
      dst = (Uint32 *)textbuf->pixels +
            (row + glyph->yoffset) * textbuf->pitch / 4 + xstart + glyph->minx;
...
SDL_Surface *TTF_RenderGlyph_Blended(TTF_Font *font, Uint16 ch, SDL_Color fg) {
  ...
  textbuf =
      SDL_CreateRGBSurface(SDL_SWSURFACE, glyph->pixmap.width, row, 32,
...
void TTF_SetFontStyle(TTF_Font *font, int style) {
   ...
   * */
  if ((font->style | TTF_STYLE_NO_GLYPH_CHANGE) !=
      (prev_style | TTF_STYLE_NO_GLYPH_CHANGE)) {
    Flush_Cache(font);
  }
}

int TTF_GetFontStyle(const TTF_Font *font) { return font->style; }
...
int TTF_GetFontOutline(const TTF_Font *font) { return font->outline; }
...
int TTF_WasInit(void) { return TTF_initialized; }
