...
#define BFLG_Hold_Temporary                                                    \
  0x0002 /* Emendelson alternate ctrl+alt host key combinations. Keep it       \
            SEPARATE so it does not disturb user changes to the mapper */
...
enum BC_Types { BC_Mod1, BC_Mod2, BC_Mod3, BC_Host, BC_Hold };
...
static KeyBlock combo_1[14] = {
    {"`~", "grave", KBD_grave},   {"1!", "1", KBD_1},
    {"2@", "2", KBD_2},           {"3#", "3", KBD_3},
    {"4$", "4", KBD_4},           {"5%", "5", KBD_5},
    {"6^", "6", KBD_6},           {"7&", "7", KBD_7},
    {"8*", "8", KBD_8},           {"9(", "9", KBD_9},
    {"0)", "0", KBD_0},           {"-_", "minus", KBD_minus},
...
static KeyBlock combo_3[12] = {
    ...
    {"G", "g", KBD_g},           {"H", "h", KBD_h},
    {"J", "j", KBD_j},           {"K", "k", KBD_k},
    {"L", "l", KBD_l},           {";:", "semicolon", KBD_semicolon},
    {"'\"", "quote", KBD_quote}, {"\\|", "backslash", KBD_backslash},
};
...
public:
  ...
  //!
  //! \description This is used by other parts of the mapper if it needs to
  //! retrieve
  //!              additional information that is only provided by the handler
  //!              event class
  enum event_type { event_t = 0, handler_event_t, mod_event_t };
...
public:
  ...
  //!
  //! \description Retrieve text string to show as the assigned mapper binding
  //! in a
  //!              menu item's displayable area so that the user knows what
  //!              keyboard input will trigger the shortcut.
  virtual std::string GetBindMenuText(void);

  //! \brief Update the menu item for the mapper shortcut with the latest text
  //! and keyboard shortcut
  void update_menu_shortcut(void) {
    if (!eventname.empty()) {
      DOSBoxMenu::item &item =
          mainMenu.get_item(std::string("mapper_") + std::string(eventname));
  ...
  virtual void Active(bool yesno) { active = yesno; }
  ...
  void SetValue(Bits value) { current_value = value; }

  //! \brief Get the value of the event
  Bits GetValue(void) { return current_value; }
...
//! \brief class for events which can be ON/OFF only: key presses, joystick
//! buttons, joystick hat
...
public:
  ...
  // methods below this line have sufficient documentation inherited from the
  // base class

  ~CTriggeredEvent() {}

  bool IsTrigger(void) override { return true; }
...
//! \brief class for events which have a non-boolean state: joystick axis
//! movement
...
public:
  ...
  bool IsTrigger(void) override { return false; }
  ...
  virtual Bitu GetActivityCount(void) { return activity; }

  //! \brief Re-post activity
  virtual void RepostActivity(void) {}
};

//! \brief Base C++ class for a binding assigned in the mapper interface (or by
//! default settings)
class CBind {
public:
...
public:
  virtual ~CBind() { list->remove(this); }

  //! \brief Constructor, to define the binding and type. This constructor adds
  //! the CBind object itself to the list
  ...
  virtual std::string GetBindMenuText(void) { return GetModifierText(); }

  //! \brief Append modifier text to a string, for use in recording bindings to
  //! the mapper file
  ...
  //! \brief Read modifier flags from a string, for use in parsing bindings from
  //! the mapper file
  ...
  virtual void ActivateBind(Bits _value, bool ev_trigger,
...
class CBindGroup {
public:
  CBindGroup() { bindgroups.push_back(this); }
...
/* TODO: This is fine, but it should not call MAPPER functions from a separate
 * thread. These functions are not necessarily reentrant and can cause screw ups
 * when called from multiple threads.
 *
 *       Also the HX-DOS builds cannot use this code because the older MinGW
 * lacks std::thread.
 *
...
public:
  ...
  void Start(std::vector<CEvent *> *ext_events,
             std::vector<std::string> &ext_sequence, const uint32_t wait_ms,
  ...
  void Callback() {
    ...
    for (const auto &button : m_sequence) {
      ...
      if (button == ",") {
        ...
        const std::string lbutton =
            is_cap ? std::string(1, tolower(button[0])) : button;
      ...
      if (m_stop_requested ||
          (m_choice && !dotype)) // quit before the pacing delay
        return;
      std::this_thread::sleep_for(std::chrono::milliseconds(m_pace_ms));
    }
  }
...
static SDLKey sdlkey_map[] = {
    ...
    /*06-0B*/ SDLK_z, SDLK_x, SDLK_c, SDLK_v,
    SDLK_BACKQUOTE /* 'Section' key */, SDLK_b,
    ...
    /*2A-2F*/ SDLK_BACKSLASH, SDLK_COMMA, SDLK_SLASH, SDLK_n, SDLK_m,
    SDLK_PERIOD,
    ...
    /*65-6A*/ SDLK_F9, SDLK_WORLD_17, SDLK_F11, SDLK_WORLD_18, SDLK_F13,
    SDLK_PAUSE /*==SDLK_F16*/,
...
typedef char assert_right_size
    [MAX_SCANCODES == (sizeof(sdlkey_map) / sizeof(sdlkey_map[0])) ? 1 : -1];

#elif defined(__linux__)
#define MAX_SCANCODES 0x90
#include "../../vs/sdl/include/SDL_keysym.h"
static SDLKey sdlkey_map[MAX_SCANCODES] = {
    // Convert hardware scancode (XKB = evdev + 8) to SDL virtual keycode
    /* Refer to
...
Bitu GetKeyCode(SDL_keysym keysym) {
  ...
  if (useScanCode()) {
    ...
    // Hack for 'a' key (QWERTY, QWERTZ), 'u' key (Turkish-F), 'q' key (AZERTY)
    // on Mac (FIX ME if there are other keys returning scancode 0)
    if ((keysym.scancode == 0) &&
        ((keysym.sym == 'a') || (keysym.sym == 'u') || (keysym.sym == 'q')))
      return 'a';
#endif
    ...
    if (keysym.scancode == 0
      ...
      if (keysym.win32_vk >= 0xa6 && keysym.win32_vk <= 0xb7)
        return SDLK_UNKNOWN; // Ignore all media keys
#endif
      switch (keysym.scancode) {
      case 0x46: // Scroll Lock
        // LOG_MSG("Scroll_lock scancode=%x, vk_key=%x", keysym.scancode,
        // keysym.win32_vk);
        return (keysym.win32_vk == VK_CANCEL ? SDLK_BREAK : SDLK_SCROLLOCK);
      case 0x35: // SLASH
        if (keysym.sym != SDLK_KP_DIVIDE) {
          return SDLK_SLASH; // Various characters are allocated to this key in
                             // European keyboards
        }
      case 0x1c:           // ENTER
      case 0x1d:           // CONTROL
      ...
      case 0x5d:           // App
        return keysym.sym; // Hardware scancodes of these keys are not unique,
                           // so virtual keycodes are used instead
        break;
      default:
#elif defined(__linux__)
      switch (keysym.scancode) { // Workaround for some of the keys return
                                 // incorrect virtual keycodes
      case 0x5E:
        return SDLK_LESS; // DE Keyboard on Debian GNU/Linux 12 (bookworm)
      ...
      default:
#endif
        key = (keysym.scancode < MAX_SCANCODES ? sdlkey_map[keysym.scancode]
                                               : SDLK_UNKNOWN);
    ...
    if (key == SDLK_BACKQUOTE) {
      key =
          (isJPkeyboard ? SDLK_WORLD_12
                        : SDLK_BACKQUOTE); // Set to Hankaku key if JP Keyboard
    }
#endif
    return key;
  } else {
    ...
    if ((keysym.sym == SDLK_BACKSLASH) && (keysym.scancode == 0x56))
      return (Bitu)SDLK_LESS;
    /* special case of the \ _ key on Japanese 106-keyboards. seems to be the
     * same code whether or not you've told Windows it's a 106-key */
    /* NTS: SDL source on Win32 maps this key (VK_OEM_102) to SDLK_LESS */
    else if (isJPkeyboard && (keysym.sym == 0) && (keysym.scancode == 0x73))
      return (Bitu)SDLK_WORLD_10; // FIXME: There's no SDLK code for that key!
                                  // Re-use one of the world keys!
    /* another hack, for the Yen \ pipe key on Japanese 106-keyboards.
       sym == 0 if English layout, sym == 0x5C if Japanese layout */
    else if (keysym.win32_vk == VK_CANCEL)
      return (Bitu)SDLK_BREAK;
#if (_WIN32_WINNT >= 0x0500)
    else if (keysym.win32_vk >= 0xa6 && keysym.win32_vk <= 0xb7)
      return (Bitu)SDLK_UNKNOWN; // Ignore all media keys
#endif
    if (isJPkeyboard && (keysym.sym == 0 || keysym.sym == 0x5C) &&
        (keysym.scancode == 0x7D))
      return (Bitu)SDLK_WORLD_11; // FIXME: There's no SDLK code for that key!
                                  // Re-use one of the world keys!
    /* what is ~ ` on American keyboards is "Hankaku" on Japanese keyboards.
     * Same scan code. */
    if (keysym.scancode == 0x29)
      return (Bitu)(isJPkeyboard ? SDLK_WORLD_12
                                 : SDLK_BACKQUOTE); // if JP106 keyboard Hankaku
                                                    // else Backquote(grave)
    /* Muhenkan */
    if (isJPkeyboard && keysym.sym == 0 && keysym.scancode == 0x7B)
      return (Bitu)SDLK_WORLD_13;
    /* Henkan/Zenkouho */
    if (isJPkeyboard && keysym.sym == 0 && keysym.scancode == 0x79)
      return (Bitu)SDLK_WORLD_14;
    /* Hiragana/Katakana */
    if (isJPkeyboard && keysym.sym == 0 && keysym.scancode == 0x70)
      return (Bitu)SDLK_WORLD_15;
#elif defined(__linux__)
      if ((keysym.sym == SDLK_BACKSLASH) && (keysym.scancode == 0x84))
        return (Bitu)SDLK_JP_YEN;
      if ((keysym.sym == SDLK_BACKSLASH) && (keysym.scancode == 0x61))
        return (Bitu)SDLK_JP_RO;
      if ((keysym.sym == SDLK_UNKNOWN) && (keysym.scancode == 0x31))
        return (Bitu)SDLK_WORLD_12;
#endif
    if (keysym.sym)
      return (Bitu)(keysym.sym < SDLK_LAST ? keysym.sym : SDLK_UNKNOWN);
    else
      return (Bitu)(keysym.scancode < MAX_SCANCODES
                        ? sdlkey_map[(keysym.scancode)]
                        : SDLK_UNKNOWN);
  }
}
...
public:
  ...
  virtual void BindName(char *buf) override {
    ...
    if (!strcmp(r, "left super"))
      r = "left Windows";
    else if (!strcmp(r, "right super"))
      r = "right Windows";
    else if (!strcmp(r, "left meta"))
      r = "left Command";
    else if (!strcmp(r, "right meta"))
      r = "right Command";
    else if (!strcmp(r, "left ctrl"))
      r = "Left Ctrl";
    else if (!strcmp(r, "right ctrl"))
      r = "Right Ctrl";
    else if (!strcmp(r, "left alt"))
      r = "Left Alt";
    else if (!strcmp(r, "right alt"))
      r = "Right Alt";
    else if (!strcmp(r, "left shift"))
      r = "Left Shift";
    else if (!strcmp(r, "right shift"))
      r = "Right Shift";
    // LOG_MSG("Key %s", r);
  ...
  virtual std::string GetBindMenuText(void) override {
    ...
    if (s != NULL) {
      ...
      if (r.length() > 0) {
        ...
        if (c == NULL)
          c = (char *)strstr(r.c_str(), " alt");
        if (c == NULL)
          c = (char *)strstr(r.c_str(), " shift");
        if (c != NULL)
          *(c + 1) = toupper(*(c + 1));
        else if (r == "Left super")
          r = "Left Windows";
        else if (r == "Right super")
          r = "Right Windows";
        else if (r == "Left meta")
          r = "Left Command";
        else if (r == "Right meta")
          r = "Right Command";
      }
    }
#endif
...
std::string CEvent::GetBindMenuText(void) {
  ...
  for (auto i = bindlist.begin(); i != bindlist.end(); i++) {
    ...
    if (t != "Right Windows" && t != "Left Windows" && t != "Right Command" &&
        t != "Left Command" && t != "Right Ctrl" && t != "Left Ctrl" &&
        t != "Right Alt" && t != "Left Alt" && t != "Right Shift" &&
        t != "Left Shift")
      break;
    s += t;
  }
  if (s == "Right WindowsLeft Windows" || s == "Left WindowsRight Windows")
    r = "Windows";
  else if (s == "Right CommandLeft Command" || s == "Left CommandRight Command")
    r = "Command";
  else if (s == "Right CtrlLeft Ctrl" || s == "Left CtrlRight Ctrl")
    r = "Ctrl";
  else if (s == "Right AltLeft Alt" || s == "Left AltRight Alt")
    r = "Alt";
  else if (s == "Right ShiftLeft Shift" || s == "Left ShiftRight Shift")
    r = "Shift";
...
public:
...
#if defined(WIN32)
    if (key ==
        SDL_SCANCODE_NONUSBACKSLASH) { // Special consideration for JP Keyboard
      key = (isJPkeyboard ? SDL_SCANCODE_INTERNATIONAL1
  ...
  bool CheckEvent(SDL_Event *event) override {
    ...
    // key = (event->key.keysym.sym ? GetKeyCode(event->key.keysym) :
    // sdlkey_map[(Bitu)(event->key.keysym.scancode)]);
    key = GetKeyCode(event->key.keysym);
    // assert(key < keys);
    if (key >= keys) {
      key = SDLK_UNKNOWN; // a test to avoid assertion failure (key < keys)
      // LOG_MSG("assertion failed: key: %x [keysym.sym:%x keysym.scancode:
      // %x]", key, event->key.keysym.sym, event->key.keysym.scancode);
    }
#endif
...
#if defined(WIN32)
    /* HACK: When setting up the Japanese keyboard layout, I'm seeing some
       bizarre keyboard handling from within Windows when pressing the ~ `
       (grave) aka Hankaku key. I know it's not hardware because when you switch
       back to English the key works normally as the tilde/grave key. What I'm
       seeing is that pressing the key only sends the "down" event (even though
       you're not holding the key down). When you press the key again, an "up"
       event is sent immediately followed by a "down" event. This is confusing
       to the mapper, so we work around it here. */

#if defined(C_SDL2)
    if (isJPkeyboard && key == 0x35 && event->key.keysym.sym == 0x60)
#else
    // if (isJPkeyboard && event->key.keysym.scancode == 0x29 /*Hankaku*/ ||
    // (useScanCode() && key == 0x29 && event->key.keysym.sym == 0))
    ...
    /* HACK: As requested on the issue tracker, on US keyboards, remap the
     * Windows menu key to the "Ro" key.
     *
     *       If the user has un-bound the Ro key, then allow the menu key to go
     * through unmodified.
     *
     * TODO: I understand that later PC-9821 systems (post Windows 95, prior to
     * the platform's demise) actually did have Windows and Windows Menu keys.
     *
     *       I also understand from "Undocumented PC-98" that the actual
     * keyboards for backwards compat reasons will not send these key codes
     * unless a byte is sent to the keyboard on Windows 95 startup to tell the
     * keyboard that it can send those keys.
     *
     *       This would explain why if you boot a PC-9821 system into Windows 95
     * "Safe mode" the Windows key does not work.
     *
     *       So later on, this remap should disable itself IF that byte is sent
     * to signal Windows 95 is running and the Windows keys should be enabled.
     */
...
private:
  const char *ConfigStart(void) override { return configname; }
  const char *BindStart(void) override { return "Key"; }
...
public:
  CJAxisBind(CBindList *_list, CBindGroup *_group, Bitu _joystick, Bitu _axis,
             bool _positive)
      : CBind(_list) {
    group = _group;
    axis = _axis;
  ...
  virtual ~CJAxisBind() {}
  virtual void ConfigName(char *buf) override {
    sprintf(buf, "%s axis %d %d", group->ConfigStart(), (int)axis,
            positive ? 1 : 0);
  }
  virtual void BindName(char *buf) override {
    sprintf(buf, "%s Axis %d%s", group->BindStart(), (int)axis,
            positive ? "+" : "-");
  }
  ...
  static int GetJoystickDeadzone(int joystick, int axis, bool positive) {
    ...
    auto name = "joy" + std::to_string(joystick + 1) + "deadzone" +
                std::to_string(axis) + (positive ? "+" : "-");
    auto value = prop->Get_double(name);
    auto deadzone = static_cast<int>(value * 32767.0);
    return deadzone;
  }

  void ActivateBind(Bits _value, bool ev_trigger,
                    bool skip_action = false) override {
    /* Since codebase is flawed, we do a simple hack:
     * If user-set deadzone exceeds hard-coded value of 25000 we just set it to
     * 25001. Other code works as usual, CTriggeredEvent does not have to check
     * if it handles a joy axis.
     */
...
public:
  CJButtonBind(CBindList *_list, CBindGroup *_group, Bitu _button)
      : CBind(_list) {
    group = _group;
    button = _button;
  }
...
public:
  CJHatBind(CBindList *_list, CBindGroup *_group, Bitu _hat, uint8_t _dir)
      : CBind(_list) {
    group = _group;
    hat = _hat;
    dir = _dir;
  ...
  virtual void BindName(char *buf) override {
    sprintf(buf, "%s Hat %d %s", group->BindStart(), (int)hat,
            (dir == SDL_HAT_UP)
                ? "up"
                : ((dir == SDL_HAT_RIGHT)
                       ? "right"
...
public:
  CStickBindGroup(Bitu _stick, Bitu _emustick, bool _dummy = false)
    ...
    emulated_axes = 0;
    emulated_hats = 0;
    pos_axis_lists = neg_axis_lists =
        NULL; /* <- Initialize the pointers to NULL. The C++ compiler won't do
                 it for us. */
    ...
    LOG_MSG("Using joystick %s with %d axes, %d buttons and %d hat(s)",
            SDL_JoystickNameForIndex((int)stick), (int)axes, (int)buttons,
            (int)hats);
#else
    LOG_MSG("Using joystick %s with %d axes, %d buttons and %d hat(s)",
            SDL_JoystickName((int)stick), (int)axes, (int)buttons, (int)hats);
#endif

    // fetching these at every call simply freezes DOSBox at times so we do it
    // once (game tested : Terminal Velocity @ joystick calibration page)
  ...
  virtual ~CStickBindGroup() {
    ...
    if (pos_axis_lists != NULL)
      delete[] pos_axis_lists;
    if (neg_axis_lists != NULL)
      delete[] neg_axis_lists;
    if (button_lists != NULL)
      delete[] button_lists;
    if (hat_lists != NULL)
      delete[] hat_lists;
  }
  ...
  CBind *CreateConfigBind(char *&buf) override {
    if (is_dummy)
      return nullptr;
    if (strncasecmp(configname, buf, strlen(configname)))
      return nullptr;
    StripWord(buf);
  ...
  CBind *CreateEventBind(SDL_Event *event) override {
    if (event->type == SDL_JOYAXISMOTION) {
      if ((unsigned int)event->jaxis.which != (unsigned int)stick)
        return nullptr;
      ...
      if ((unsigned int)event->jbutton.which != (unsigned int)stick)
        return nullptr;
#if defined(REDUCE_JOYSTICK_POLLING)
      ...
      if ((unsigned int)event->jhat.which != (unsigned int)stick)
        return nullptr;
  ...
  void ActivateJoystickBoundEvents() {
    if (GCC_UNLIKELY(sdl_joystick == NULL))
      return;
    ...
    for (i = 0; i < axes; i++) {
      ...
      if (caxis_pos > 1) {
        ...
        if (caxis_pos != -32768)
          caxis_pos = (Sint16)abs(caxis_pos);
        else
          caxis_pos = 32767;
    ...
    for (i = 0; i < hats; i++) {
      ...
      if ((chat_state & SDL_HAT_UP) != (old_hat_state[i] & SDL_HAT_UP)) {
        if (chat_state & SDL_HAT_UP)
          ActivateBindList(&hat_lists[(i << 2) + 0], 32767, true);
        else
          DeactivateBindList(&hat_lists[(i << 2) + 0], true);
      }
      if ((chat_state & SDL_HAT_RIGHT) != (old_hat_state[i] & SDL_HAT_RIGHT)) {
        if (chat_state & SDL_HAT_RIGHT)
          ActivateBindList(&hat_lists[(i << 2) + 1], 32767, true);
        else
          DeactivateBindList(&hat_lists[(i << 2) + 1], true);
      }
      if ((chat_state & SDL_HAT_DOWN) != (old_hat_state[i] & SDL_HAT_DOWN)) {
        if (chat_state & SDL_HAT_DOWN)
          ActivateBindList(&hat_lists[(i << 2) + 2], 32767, true);
        else
          DeactivateBindList(&hat_lists[(i << 2) + 2], true);
      }
      if ((chat_state & SDL_HAT_LEFT) != (old_hat_state[i] & SDL_HAT_LEFT)) {
        if (chat_state & SDL_HAT_LEFT)
          ActivateBindList(&hat_lists[(i << 2) + 3], 32767, true);
        else
          DeactivateBindList(&hat_lists[(i << 2) + 3], true);
      }
...
private:
  float joy1dz1 = 0, joy1rs1 = 0, joy1dz2 = 0, joy1rs2 = 0, joy2dz1 = 0,
        joy2rs1 = 0;
  CBind *CreateAxisBind(Bitu axis, bool positive) {
    if (axis < axes) {
      if (positive)
        return new CJAxisBind(&pos_axis_lists[axis], this, stick, axis,
                              positive);
      else
        return new CJAxisBind(&neg_axis_lists[axis], this, stick, axis,
                              positive);
    }
    return NULL;
  }
...
#if defined(C_SDL2)
    if (sdl_joystick != NULL)
      return SDL_JoystickNameForIndex((int)stick);
#else
    if (sdl_joystick != NULL)
      return SDL_JoystickName((int)stick);
#endif
    else
  ...
  static float GetAxisResponse(int joystick, int thumbStick) {
    auto section = control->GetSection("joystick");
    auto prop = static_cast<Section_prop *>(section);
    auto name = "joy" + std::to_string(joystick + 1) + "response" +
                std::to_string(thumbStick + 1);
    auto response = static_cast<float>(prop->Get_double(name));
    return response;
  }

  static void ProcessInput(int16_t x, int16_t y, float deadzone,
                           DOSBox_Vector2 &joy) {
...
protected:
  ...
  DOSBox_Vector2 GetJoystickVector(int joystick, int thumbStick, int xAxis,
                                   int yAxis) const {
...
public:
  C4AxisBindGroup(Bitu _stick, Bitu _emustick)
  ...
  bool CheckEvent(SDL_Event *event) override {
    ...
    case SDL_JOYAXISMOTION:
      ...
      if ((unsigned int)jaxis->which == (unsigned int)stick &&
          jaxis->axis < 4) {
        if (jaxis->axis & 1)
          JOYSTICK_Move_Y(jaxis->axis >> 1 & 1,
                          (float)(jaxis->value / 32768.0));
        else
          JOYSTICK_Move_X(jaxis->axis >> 1 & 1,
                          (float)(jaxis->value / 32768.0));
      }
      break;
    case SDL_JOYBUTTONDOWN:
...
public:
  ...
  bool CheckEvent(SDL_Event *event) override {
    ...
    case SDL_JOYHATMOTION:
      jhat = &event->jhat;
      if ((unsigned int)jhat->which == (unsigned int)stick)
...
public:
  ...
  bool CheckEvent(SDL_Event *event) override {
    ...
    SDL_JoyHatEvent *jhat = NULL;
    Bitu but = 0;
    static unsigned const button_magic[6] = {0x02,  0x04, 0x10,
                                             0x100, 0x20, 0x200};
    static unsigned const hat_magic[2][5] = {
...
void CBindGroup::ActivateBindList(CBindList *list, Bits value,
  ...
  for (it = list->begin(); it != list->end(); ++it) {
    if ((*it)->mods == MMODHOST) {
      if ((!hostkeyalt && validmod == (*it)->mods) ||
          (hostkeyalt == 1 &&
           (sdl.lctrlstate == SDL_KEYDOWN || sdl.rctrlstate == SDL_KEYDOWN) &&
           (sdl.laltstate == SDL_KEYDOWN || sdl.raltstate == SDL_KEYDOWN)) ||
          (hostkeyalt == 2 &&
           (sdl.lctrlstate == SDL_KEYDOWN || sdl.rctrlstate == SDL_KEYDOWN) &&
           (sdl.lshiftstate == SDL_KEYDOWN ||
            sdl.rshiftstate == SDL_KEYDOWN)) ||
          (hostkeyalt == 3 &&
           (sdl.laltstate == SDL_KEYDOWN || sdl.raltstate == SDL_KEYDOWN) &&
           (sdl.lshiftstate == SDL_KEYDOWN ||
            sdl.rshiftstate == SDL_KEYDOWN))) {
...
public:
...
#if defined(C_SDL2)
    uint8_t *point = ((uint8_t *)mapper.draw_surface->pixels) +
                     (y * mapper.draw_surface->w) + x;
#else
    uint8_t *point =
        ((uint8_t *)mapper.surface->pixels) + (y * mapper.surface->pitch) + x;
#endif
  ...
  virtual void Draw(void) { Draw(true, true); }
  virtual bool OnTop(Bitu _x, Bitu _y) {
    return (enabled && (_x >= x) && (_x < x + dx) && (_y >= y) &&
            (_y < y + dy));
  }
  ...
  bool GetCanClick(void) { return clickable; }
...
protected:
  ...
  void DrawTextAuto(const char *text, bool centered, uint8_t foreground,
                    uint8_t background) {
    const auto size = strlen(text);
    const auto wide = dx / 8;
    const auto data = dx > 0 && size > wide
                          ? std::string(text, wide - 3) + std::string("...")
                          : std::string(text);
...
public:
  CTextButton(Bitu _x, Bitu _y, Bitu _dx, Bitu _dy, const char *_text)
      : CButton(_x, _y, _dx, _dy) {
    if (strlen(_text) < 100)
      strcpy(text, _text);
    else {
      strncpy(text, _text, 99);
      text[99] = 0;
    }
    invertw = 0;
  }
  ...
  void Draw(void) override {
    ...
    DrawTextAuto(text, true, fg,
                 bg); // TODO add the ability to center text for this class
    ...
    for (Bitu lines = 0; lines < (dy - 1); lines++) {
      ...
      point += mapper.surface->pitch;
#endif
      // draw the borders a second time to prevent the text from overwriting
      // them
...
public:
  CEventButton(Bitu _x, Bitu _y, Bitu _dx, Bitu _dy, const char *_text,
               CEvent *_event)
...
public:
  CCaptionButton(Bitu _x, Bitu _y, Bitu _dx, Bitu _dy, bool centered)
      : CButton(_x, _y, _dx, _dy) {
    caption[0] = 0;
    center = centered;
  }
...
public:
  CBindButton(Bitu _x, Bitu _y, Bitu _dx, Bitu _dy, const char *_text,
              BB_Types _type)
      : CTextButton(_x, _y, _dx, _dy, _text) {
    type = _type;
  }
  ...
  void ClickImpl(void) override {
    ...
    case BB_Capture:
      GFX_CaptureMouse();
      if (mouselocked)
        change_action_text(MSG_Get("CAPTURE_ENABLED"), CLR_WHITE);
      break;
    }
  }
...
//! \brief Modifier trigger event, for modifier keys. This permits the user to
//! change modifier key bindings.
class CModEvent : public CTriggeredEvent {
public:
  //! \brief Constructor to provide entry name and the index of the modifier
  //! button
  CModEvent(char const *const _entry, Bitu _wmod)
      : CTriggeredEvent(_entry), notify_button(NULL) {
    wmod = _wmod;
  ...
  void Active(bool yesno) override {
    ...
    if (yesno)
      mapper.mods |= ((Bitu)1u << (wmod - 1u));
    else
      mapper.mods &= ~((Bitu)1u << (wmod - 1u));
  };

  //! \brief Associate this object with a text button in the mapper UI
  void notifybutton(CTextButton *n) { notify_button = n; }
...
public:
  CCheckButton(Bitu _x, Bitu _y, Bitu _dx, Bitu _dy, const char *_text,
               BC_Types _type)
      : CTextButton(_x, _y, _dx, _dy, _text) {
    type = _type;
  }
  ...
  void Draw(void) override {
    ...
    case BC_Mod1:
      ...
      str = checked && mod_event[1] != NULL ? mod_event[1]->GetBindMenuText()
                                            : LabelMod1;
      strcpy(text, str.size() > 8 ? LabelMod1 : str.c_str());
      break;
    case BC_Mod2:
      checked = (mapper.abind->mods & BMOD_Mod2) > 0;
      str = checked && mod_event[2] != NULL ? mod_event[2]->GetBindMenuText()
                                            : LabelMod2;
      strcpy(text, str.size() > 8 ? LabelMod2 : str.c_str());
      break;
    case BC_Mod3:
      checked = (mapper.abind->mods & BMOD_Mod3) > 0;
      str = checked && mod_event[3] != NULL ? mod_event[3]->GetBindMenuText()
                                            : LabelMod3;
...
#if defined(C_SDL2)
      uint8_t *point = ((uint8_t *)mapper.draw_surface->pixels) +
                       ((y + 2) * mapper.draw_surface->pitch) + x + dx - dy + 2;
#else
      uint8_t *point = ((uint8_t *)mapper.surface->pixels) +
                       ((y + 2) * mapper.surface->pitch) + x + dx - dy + 2;
#endif
...
public:
  //! \brief Constructor to specify mapper event, and KBD_* key enumeration
  //! constant
...
public:
  CMouseButtonEvent(char const *const _entry, uint8_t _button)
...
public:
  //! \brief Constructor, to describe entry, joystick, axis, direction, and the
  //! opposing axis
  CJAxisEvent(char const *const _entry, Bitu _stick, Bitu _axis, bool _positive,
              CJAxisEvent *_opposite_axis)
      : CContinuousEvent(_entry) {
    notify_button = NULL;
    stick = _stick;
    axis = _axis;
    positive = _positive;
  ...
  void Active(bool /*moved*/) override {
    ...
    virtual_joysticks[stick].axis_pos[axis] =
        (int16_t)(GetValue() * (positive ? 1 : -1));
  }
  ...
  //! \brief Associate this object with a text button in the mapper GUI so that
  //! joystick position can be displayed at all times
  void notifybutton(CTextButton *n) { notify_button = n; }
  ...
  for (size_t m = 4u /*Host key first*/; m >= 1u; m--) {
    if ((mods & ((Bitu)1u << (m - 1u))) && mod_event[m] != NULL) {
      ...
      r += m == 4
               ? (hostkeyalt == 1
                      ? "Ctrl+Alt"
                      : (hostkeyalt == 2 ? "Ctrl+Shift"
                                         : (hostkeyalt == 3 ? "Alt+Shift" : t)))
               : t;
    }
  }
...
public:
  //! \brief Constructor, to specify the entry, handler (callback), key
  //! (according to MapKeys enumeration), and text to display for the shortcut
  //! in the mapper UO
  CHandlerEvent(char const *const _entry, MAPPER_Handler *_handler,
                MapKeys _key, Bitu _mod, char const *const _buttonname)
      : CTriggeredEvent(_entry), notify_button(NULL) {
    handler = _handler;
    defmod = _mod;
    defkey = _key;
    if (strlen(_buttonname) < 100)
      strcpy(buttonname, _buttonname);
    else {
      strncpy(buttonname, _buttonname, 99);
      buttonname[99] = 0;
    }
  ...
  const char *ButtonName() { return buttonname; }

  void SetButtonName(const char *name) {
    if (strlen(name) < 100)
      strcpy(buttonname, name);
    else {
      strncpy(buttonname, name, 99);
      buttonname[99] = 0;
    }
  }
  ...
  void MakeDefaultBind(char *buf) {
    ...
    sprintf(buf, "%s \"key %d%s%s%s%s\"", entry, (int)key,
            (defmod & 1) ? " mod1" : "", (defmod & 2) ? " mod2" : "",
            (defmod & 4) ? " mod3" : "", (defmod & 8) ? " host" : "");
  }
#else
  void MakeDefaultBind(char *buf) {
    Bitu key = 0;
...
bool MAPPER_DemoOnly(void) { return !mapper.exit; }
...
CEvent *get_mapper_event_by_name(const std::string &x) {
  ...
  if (i != name_to_events.end()) {
    if (i->second >= events.size())
      E_Exit("Mapper: name to events contains out of range index for \"%s\"",
             x.c_str());
...
static void DrawText(Bitu x, Bitu y, const char *text, uint8_t color,
                     uint8_t bkcolor /*=CLR_BLACK*/) {
#if defined(C_SDL2)
  uint8_t *draw = ((uint8_t *)mapper.draw_surface->pixels) +
                  (y * mapper.draw_surface->w) + x;
#else
  uint8_t *draw =
      ((uint8_t *)mapper.surface->pixels) + (y * mapper.surface->pitch) + x;
  ...
  while (*text) {
    ...
    if (IS_PC98_ARCH || IS_JEGA_ARCH || isDBCSCP()) {
      ...
      prvmc = 0;
    if (font_14_init && dos.loaded_codepage && dos.loaded_codepage != 437 &&
        prvmc != 1)
...
void RedrawMapperEventButtons() {
  ...
  for (std::vector<CEventButton *>::iterator it = ceventbuttons.begin();
       it != ceventbuttons.end(); ++it) {
...
static void DrawButtons(void) {
  ...
  SDL_BlitSurface(mapper.draw_surface, NULL, mapper.draw_surface_nonpaletted,
                  NULL);
  SDL_BlitScaled(mapper.draw_surface_nonpaletted, NULL, mapper.surface,
                 &mapper.draw_rect);
...
static CKeyEvent *AddKeyButtonEvent(Bitu x, Bitu y, Bitu dx, Bitu dy,
                                    ...
                                    char const *const entry, KBD_KEYS key) {
  char buf[64];
  strcpy(buf, "key_");
  strcat(buf, entry);
...
static CMouseButtonEvent *AddMouseButtonEvent(Bitu x, Bitu y, Bitu dx, Bitu dy,
                                              char const *const title,
...
static CJAxisEvent *AddJAxisButton(Bitu x, Bitu y, Bitu dx, Bitu dy,
                                   char const *const title, Bitu stick,
                                   Bitu axis, bool positive,
                                   CJAxisEvent *opposite_axis) {
  char buf[64];
  sprintf(buf, "jaxis_%d_%d%s", (int)stick, (int)axis, positive ? "+" : "-");
  CJAxisEvent *event =
      new CJAxisEvent(buf, stick, axis, positive, opposite_axis);
  CEventButton *button = new CEventButton(x, y, dx, dy, title, event);
  event->notifybutton(button);
  return event;
}
...
static void AddJButtonButton(Bitu x, Bitu y, Bitu dx, Bitu dy,
                             char const *const title, Bitu stick, Bitu button) {
  char buf[64];
...
static void AddJHatButton(Bitu x, Bitu y, Bitu dx, Bitu dy,
                          char const *const title, Bitu _stick, Bitu _hat,
                          Bitu _dir) {
  char buf[64];
...
static void AddModButton(Bitu x, Bitu y, Bitu dx, Bitu dy,
                         char const *const title, Bitu _mod) {
  char buf[64];
...
#define YO 1
  ...
  for (i = 0; i < 12; i++) {
    AddKeyButtonEvent(PX(XO + 2 + i), PY(YO), BU(1), BV(1), combo_f[i].title,
                      combo_f[i].entry, combo_f[i].key);
  }
...
#define YO 2
  ...
  for (i = 0; i < 14; i++) {
    ...
    AddKeyButtonEvent(PX(XO + (i > 0 ? i + 1 : i)), PY(YO), BU(width), BV(1),
                      block[i].title, block[i].entry, block[i].key);
  }
...
#define YO 3
  ...
  for (i = 0; i < 12; i++) {
    AddKeyButtonEvent(PX(XO + 2 + i), PY(YO), BU(1), BV(1), combo_2[i].title,
                      combo_2[i].entry, combo_2[i].key);
  }

  AddKeyButtonEvent(PX(XO + 14), PY(YO), BU(2), BV(2), "Enter", "enter",
                    KBD_enter);
...
#define YO 4
  ...
  caps_lock_event = AddKeyButtonEvent(PX(XO), PY(YO), BU(2), BV(1), "Caps",
                                      "capslock", KBD_capslock);

  for (i = 0; i < 12; i++) {
    const auto block = IS_PC98_ARCH ? combo_3_pc98 : combo_3;
    AddKeyButtonEvent(PX(XO + 2 + i), PY(YO), BU(1), BV(1), block[i].title,
                      block[i].entry, block[i].key);
  }
...
#define YO 5
  ...
  AddKeyButtonEvent(PX(XO), PY(YO), BU(2), BV(1), "Shift", "lshift",
                    KBD_leftshift);

  for (i = 0; i < 11; i++) {
    AddKeyButtonEvent(PX(XO + 2 + i), PY(YO), BU(1), BV(1), combo_4[i].title,
                      combo_4[i].entry, combo_4[i].key);
  }

  AddKeyButtonEvent(PX(XO + 13), PY(YO), BU(3), BV(1), "Shift", "rshift",
                    KBD_rightshift);
...
#define YO 6
  ...
  AddKeyButtonEvent(PX(XO + 00), PY(YO), BU(2), BV(1), "Ctrl", "lctrl",
                    KBD_leftctrl);
  AddKeyButtonEvent(PX(XO + 02), PY(YO), BU(1), BV(1), "Win", "lwindows",
                    KBD_lwindows);
  AddKeyButtonEvent(PX(XO + 03), PY(YO), BU(1), BV(1), "Alt", "lalt",
                    KBD_leftalt);
  AddKeyButtonEvent(PX(XO + 04), PY(YO), BU(7), BV(1), "Space", "space",
                    KBD_space);
  AddKeyButtonEvent(PX(XO + 11), PY(YO), BU(1), BV(1), "Alt", "ralt",
                    KBD_rightalt);
  AddKeyButtonEvent(PX(XO + 12), PY(YO), BU(1), BV(1), "Win", "rwindows",
                    KBD_rwindows);
  AddKeyButtonEvent(PX(XO + 13), PY(YO), BU(1), BV(1), "App", "rwinmenu",
                    KBD_rwinmenu);
  AddKeyButtonEvent(PX(XO + 14), PY(YO), BU(2), BV(1), "Ctrl", "rctrl",
                    KBD_rightctrl);
...
#define YO 0
  ...
  AddKeyButtonEvent(PX(XO + 0), PY(YO + 0), BU(1), BV(1), "Prn", "printscreen",
                    KBD_printscreen);
  AddKeyButtonEvent(PX(XO + 1), PY(YO + 0), BU(1), BV(1), "Scr", "scrolllock",
                    KBD_scrolllock);
  AddKeyButtonEvent(PX(XO + 2), PY(YO + 0), BU(1), BV(1), "Brk", "pause",
                    KBD_pause);

  AddKeyButtonEvent(PX(XO + 0), PY(YO + 2), BU(1), BV(1), "Ins", "insert",
                    KBD_insert);
  AddKeyButtonEvent(PX(XO + 1), PY(YO + 2), BU(1), BV(1), "Hom", "home",
                    KBD_home);
  AddKeyButtonEvent(PX(XO + 2), PY(YO + 2), BU(1), BV(1), "Pg\x18", "pageup",
                    KBD_pageup);

  AddKeyButtonEvent(PX(XO + 0), PY(YO + 3), BU(1), BV(1), "Del", "delete",
                    KBD_delete);
  AddKeyButtonEvent(PX(XO + 1), PY(YO + 3), BU(1), BV(1), "End", "end",
                    KBD_end);
  AddKeyButtonEvent(PX(XO + 2), PY(YO + 3), BU(1), BV(1), "Pg\x19", "pagedown",
                    KBD_pagedown);

  AddKeyButtonEvent(PX(XO + 1), PY(YO + 5), BU(1), BV(1), "\x18", "up", KBD_up);
  AddKeyButtonEvent(PX(XO + 0), PY(YO + 6), BU(1), BV(1), "\x1B", "left",
                    KBD_left);
  AddKeyButtonEvent(PX(XO + 1), PY(YO + 6), BU(1), BV(1), "\x19", "down",
                    KBD_down);
  AddKeyButtonEvent(PX(XO + 2), PY(YO + 6), BU(1), BV(1), "\x1A", "right",
                    KBD_right);
...
#define YO 2
  ...
  AddKeyButtonEvent(PX(XO + 3) - 3, PY(YO - 1), BU(1) - 1, BV(1),
                    "KP=", "kp_equals", KBD_kpequals);
  if (!IS_PC98_ARCH) {
    AddKeyButtonEvent(PX(XO + 2) - 3, PY(YO - 1), BU(1) - 1, BV(1), "KP,",
                      "kp_comma", KBD_kpcomma);
  }
  num_lock_event = AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 0), BU(1) - 1,
                                     BV(1), "Num", "numlock", KBD_numlock);
  AddKeyButtonEvent(PX(XO + 1) - 1, PY(YO + 0), BU(1) - 1, BV(1), "/",
                    "kp_divide", KBD_kpdivide);
  AddKeyButtonEvent(PX(XO + 2) - 2, PY(YO + 0), BU(1) - 1, BV(1), "*",
                    "kp_multiply", KBD_kpmultiply);
  AddKeyButtonEvent(PX(XO + 3) - 3, PY(YO + 0), BU(1) - 1, BV(1), "-",
                    "kp_minus", KBD_kpminus);

  AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 1), BU(1) - 1, BV(1), "7", "kp_7",
                    KBD_kp7);
  AddKeyButtonEvent(PX(XO + 1) - 1, PY(YO + 1), BU(1) - 1, BV(1), "8", "kp_8",
                    KBD_kp8);
  AddKeyButtonEvent(PX(XO + 2) - 2, PY(YO + 1), BU(1) - 1, BV(1), "9", "kp_9",
                    KBD_kp9);
  AddKeyButtonEvent(PX(XO + 3) - 3, PY(YO + 1), BU(1) - 1, BV(2), "+",
                    "kp_plus", KBD_kpplus);

  AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 2), BU(1) - 1, BV(1), "4", "kp_4",
                    KBD_kp4);
  AddKeyButtonEvent(PX(XO + 1) - 1, PY(YO + 2), BU(1) - 1, BV(1), "5", "kp_5",
                    KBD_kp5);
  AddKeyButtonEvent(PX(XO + 2) - 2, PY(YO + 2), BU(1) - 1, BV(1), "6", "kp_6",
                    KBD_kp6);

  AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 3), BU(1) - 1, BV(1), "1", "kp_1",
                    KBD_kp1);
  AddKeyButtonEvent(PX(XO + 1) - 1, PY(YO + 3), BU(1) - 1, BV(1), "2", "kp_2",
                    KBD_kp2);
  AddKeyButtonEvent(PX(XO + 2) - 2, PY(YO + 3), BU(1) - 1, BV(1), "3", "kp_3",
                    KBD_kp3);

  AddKeyButtonEvent(PX(XO + 3) - 3, PY(YO + 3), BU(1) - 1, BV(2), "Ent",
                    "kp_enter", KBD_kpenter);

  if (IS_PC98_ARCH) // TODO
  {
    AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 4), BU(1) - 1, BV(1), "0", "kp_0",
                      KBD_kp0);
    AddKeyButtonEvent(PX(XO + 1) - 1, PY(YO + 4), BU(1) - 1, BV(1), ",",
                      "kp_comma", KBD_kpcomma);
  } else {
    AddKeyButtonEvent(PX(XO + 0) - 0, PY(YO + 4), BU(2) - 2, BV(1), "0", "kp_0",
                      KBD_kp0);
  }

  AddKeyButtonEvent(PX(XO + 2) - 2, PY(YO + 4), BU(1) - 1, BV(1), ".",
                    "kp_period", KBD_kpperiod);
...
#define YO 9
  ...
  AddMouseButtonEvent(PX(XO + 0) + CX * 1, PY(YO), BU(1), BV(1), "L", "left",
                      0);
  AddMouseButtonEvent(PX(XO + 1) + CX * 1, PY(YO), BU(1), BV(1), "M", "middle",
                      2);
  AddMouseButtonEvent(PX(XO + 2) + CX * 1, PY(YO), BU(1), BV(1), "R", "right",
                      1);
...
#define YO 8
  ...
  switch (joytype) {
  case JOY_NONE:
    (new CTextButton(PX(XO + 0), PY(YO + 0) - CY, BU(3), BV(1), "Disabled"))
        ->SetCanClick(false);
    (new CTextButton(PX(XO + 4), PY(YO + 0) - CY, BU(3), BV(1), "Disabled"))
        ->SetCanClick(false);
    (new CTextButton(PX(XO + 8), PY(YO + 0) - CY, BU(3), BV(1), "Disabled"))
        ->SetCanClick(false);
    break;
  case JOY_2AXIS:
...

  {
    ...
    auto x1 = AddJAxisButton(PX(XO + 0), PY(YO + 2) - CY, BU(1), BV(1), "X-", 0,
                             0, false, nullptr);
    auto x2 = AddJAxisButton(PX(XO + 2), PY(YO + 2) - CY, BU(1), BV(1), "X+", 0,
                             0, true, x1);
    (void)x2; // unused
    auto y1 = AddJAxisButton(PX(XO + 1), PY(YO + 1) - CY, BU(1), BV(1), "Y-", 0,
                             1, false, nullptr);
    auto y2 = AddJAxisButton(PX(XO + 1), PY(YO + 2) - CY, BU(1), BV(1), "Y+", 0,
                             1, true, y1);
    (void)y2; // unused
  }
  ...
  if (joytype == JOY_2AXIS) {
    ...
    auto x1 = AddJAxisButton(PX(XO + 4), PY(YO + 2) - CY, BW, BH, "X-", 1, 0,
                             false, nullptr);
    auto x2 = AddJAxisButton(PX(XO + 6), PY(YO + 2) - CY, BW, BH, "X+", 1, 0,
                             true, x1);
    (void)x2; // unused
    auto y1 = AddJAxisButton(PX(XO + 5), PY(YO + 1) - CY, BW, BH, "Y-", 1, 1,
                             false, nullptr);
    auto y2 = AddJAxisButton(PX(XO + 5), PY(YO + 2) - CY, BW, BH, "Y+", 1, 1,
                             true, y1);
    (void)y2; // unused
    ...
    auto x1 = AddJAxisButton(PX(XO + 4), PY(YO + 2) - CY, BU(1), BV(1), "X-", 0,
                             2, false, nullptr);
    auto x2 = AddJAxisButton(PX(XO + 6), PY(YO + 2) - CY, BU(1), BV(1), "X+", 0,
                             2, true, x1);
    (void)x2; // unused
    auto y1 = AddJAxisButton(PX(XO + 5), PY(YO + 1) - CY, BU(1), BV(1), "Y-", 0,
                             3, false, nullptr);
    auto y2 = AddJAxisButton(PX(XO + 5), PY(YO + 2) - CY, BU(1), BV(1), "Y+", 0,
                             3, true, y1);
    (void)y2; // unused
  ...
  AddJHatButton(PX(XO + 9), PY(YO + 1) - CY, BU(1), BV(1), "\x18", 0, 0,
                0); // U
  AddJHatButton(PX(XO + 8), PY(YO + 2) - CY, BU(1), BV(1), "\x1B", 0, 0,
                3); // L
  AddJHatButton(PX(XO + 9), PY(YO + 2) - CY, BU(1), BV(1), "\x19", 0, 0,
                2); // D
  AddJHatButton(PX(XO + 10), PY(YO + 2) - CY, BU(1), BV(1), "\x1A", 0, 0,
                1); // R
...
#define YO 11
  ...
  AddKeyButtonEvent(PX(XO + 0) + CX, PY(YO + 0), BU(3), BV(1), "Hankaku",
                    "jp_hankaku", KBD_jp_hankaku);
  AddKeyButtonEvent(PX(XO + 0) + CX, PY(YO + 1), BU(3), BV(1), "Muhenkan",
                    "jp_muhenkan", KBD_jp_muhenkan);
  AddKeyButtonEvent(PX(XO + 0) + CX, PY(YO + 2), BU(3), BV(1), "Henkan",
                    "jp_henkan", KBD_jp_henkan);
  AddKeyButtonEvent(PX(XO + 0) + CX, PY(YO + 3), BU(3), BV(1), "Hiragana",
                    "jp_hiragana", KBD_jp_hiragana);
  AddKeyButtonEvent(PX(XO + 0) + CX, PY(YO + 4), BU(3), BV(1), "Yen", "jp_yen",
                    KBD_jp_yen);
  AddKeyButtonEvent(PX(XO + 4) + CX, PY(YO + 0), BU(1), BV(1), "\\",
                    "jp_bckslash", KBD_jp_backslash);
  AddKeyButtonEvent(PX(XO + 5) + CX, PY(YO + 0), BU(1), BV(1), ":*", "colon",
                    KBD_colon);
  AddKeyButtonEvent(PX(XO + 6) + CX, PY(YO + 0), BU(1), BV(1), "^`", "caret",
                    KBD_caret);
  AddKeyButtonEvent(PX(XO + 7) + CX, PY(YO + 0), BU(1), BV(1), "@~", "atsign",
                    KBD_atsign);
  AddKeyButtonEvent(PX(XO + 4) + CX, PY(YO + 3), BU(3), BV(1), "Hancha",
                    "kor_hancha", KBD_kor_hancha);
  AddKeyButtonEvent(PX(XO + 4) + CX, PY(YO + 4), BU(3), BV(1), "Hanyong",
...
#pragma region Modifiers & Bindings
  ...
  bind_but.add =
      new CBindButton(PX(0) + CX, PY(20), BU(3), BV(1), MSG_Get("ADD"), BB_Add);
  bind_but.del =
      new CBindButton(PX(3) + CX, PY(20), BU(3), BV(1), MSG_Get("DEL"), BB_Del);
  bind_but.next = new CBindButton(PX(6) + CX, PY(20), BU(3), BV(1),
                                  MSG_Get("NEXT"), BB_Next);

  bind_but.mod1 = new CCheckButton(PX(0) + CX, PY(21) + CY, BU(3), BV(1),
                                   LabelMod1, BC_Mod1);
  bind_but.mod2 = new CCheckButton(PX(0) + CX, PY(22) + CY, BU(3), BV(1),
                                   LabelMod2, BC_Mod2);
  bind_but.mod3 = new CCheckButton(PX(0) + CX, PY(23) + CY, BU(3), BV(1),
                                   LabelMod3, BC_Mod3);
  bind_but.host = new CCheckButton(PX(3) + CX, PY(21) + CY, BU(3), BV(1),
                                   LabelHost, BC_Host);
  bind_but.hold = new CCheckButton(PX(3) + CX, PY(22) + CY, BU(3), BV(1),
                                   LabelHold, BC_Hold);
...
#define YO 0
    ...
    AddKeyButtonEvent(PX(XO + 0), PY(YO), BU(2), BV(1), "STOP", "stop",
                      KBD_stop);
    AddKeyButtonEvent(PX(XO + 2), PY(YO), BU(2), BV(1), "HELP", "help",
                      KBD_help);
    AddKeyButtonEvent(PX(XO + 4), PY(YO), BU(2), BV(1), "COPY", "copy",
                      KBD_copy);
    AddKeyButtonEvent(PX(XO + 6), PY(YO), BU(2), BV(1), "KANA", "kana",
                      KBD_kana);
    AddKeyButtonEvent(PX(XO + 8), PY(YO), BU(2), BV(1), "NFER", "nfer",
                      KBD_nfer);
    AddKeyButtonEvent(PX(XO + 10), PY(YO), BU(2), BV(1), "XFER", "xfer",
                      KBD_xfer);
    AddKeyButtonEvent(PX(XO + 12), PY(YO), BU(2), BV(1), "RO/_", "jp_ro",
                      KBD_jp_ro);
...
#define YO 11
    ...
    AddKeyButtonEvent(PX(XO + 1) + CX, PY(YO + 1), BU(1), BV(1), "VF1", "vf1",
                      KBD_vf1);
    AddKeyButtonEvent(PX(XO + 2) + CX, PY(YO + 1), BU(1), BV(1), "VF2", "vf2",
                      KBD_vf2);
    AddKeyButtonEvent(PX(XO + 3) + CX, PY(YO + 1), BU(1), BV(1), "VF3", "vf3",
                      KBD_vf3);
    AddKeyButtonEvent(PX(XO + 4) + CX, PY(YO + 1), BU(1), BV(1), "VF4", "vf4",
                      KBD_vf4);
    AddKeyButtonEvent(PX(XO + 5) + CX, PY(YO + 1), BU(1), BV(1), "VF5", "vf5",
                      KBD_vf5);
  }
...
#undef YO
  ...
  for (CHandlerEventVector_it hit = handlergroup.begin();
       hit != handlergroup.end(); ++hit) {
    maxpage = page;

    auto button = new CEventButton(PX(10) + CX, PY(11 + ypos), BU(12), BV(1),
                                   (*hit)->ButtonName(), (*hit));
  ...
  bind_but.prevpage = new CBindButton(PX(11), PY(19) - CY - (CY / 2), BU(4),
                                      BV(1), "<-", BB_Prevpage);

  bind_but.pagestat =
      new CCaptionButton(PX(14), PY(19) - CY - (CY / 2), BU(5), BV(1), true);

  bind_but.nextpage = new CBindButton(PX(18), PY(19) - CY - (CY / 2), BU(4),
                                      BV(1), "->", BB_Nextpage);
  ...
  next_handler_xpos =
      xpos; // TODO is this useless, commenting it out changes nothing!?
  next_handler_ypos =
      ypos; // TODO is this useless, commenting it out changes nothing!?

#pragma endregion

#pragma region Capture/Save/Exit

  bind_but.cap = new CBindButton(PX(12), PY(20), BU(3), BV(1),
                                 MSG_Get("CAPTURE"), BB_Capture);
  bind_but.save =
      new CBindButton(PX(15), PY(20), BU(3), BV(1), MSG_Get("SAVE"), BB_Save);
  bind_but.exit =
      new CBindButton(PX(18), PY(20), BU(3), BV(1), MSG_Get("EXIT"), BB_Exit);
...
#pragma region Status/Info
  ...
  // NOTE: screen budget is really tight down there, more than that and drawing
  // crashes

  bind_but.action =
      new CCaptionButton(PX(7), PY(22) - CY, BU(15), BV(1), false);
...
static void CreateStringBind(char *line, bool loading = false) {
  ...
  if (loading) {
    /* NTS: StripWord() updates line pointer after ASCIIZ snipping the event
     * name */
    pending_string_binds[eventname] =
        line; /* perhaps code will register it later (i.e. Herc pal change) */
    LOG(LOG_MISC,
        LOG_WARN)("Can't find matching event for %s = %s yet. It may exist "
                  "later when registered elsewhere in this emulator.",
                  eventname, line);
  } else {
...
foundevent:
  CBind *bind;
  for (char *bindline = StripWord(line); *bindline;
       bindline = StripWord(line)) {
...
static struct {
  ...
  Bitu key;
    ...
    {"jp_bckslash",
     SDL_SCANCODE_INTERNATIONAL1}, // SDL2 returns same code as
                                   // SDL_SCANCODE_NONUSBACKSLASH?
    {"jp_yen", SDL_SCANCODE_INTERNATIONAL3},
...
static struct {
  ...
  Bitu key;
    ...
    /* NTS: IBM PC keyboards as far as I know do not have numeric keypad equals
     * sign. This default assignment should allow Apple Mac users (who's
     * keyboards DO have one) to use theirs as a normal equals sign. */
...
static void CreateDefaultBinds(void) {
  ...
  while (DefaultKeys[i].eventend) {
    sprintf(buffer, "key_%s \"key %d\"", DefaultKeys[i].eventend,
            (int)DefaultKeys[i].key);
  ...
  CreateStringBind(buffer);
#if defined(WIN32) &&                                                                                                                \
    !defined(C_HX_DOS) /* F12 is not a good modifier key in Windows:                                                                 \
                          https://stackoverflow.com/questions/18997754/how-to-disable-f12-to-debug-application-in-visual-studio-2012 \
                        */
  sprintf(buffer, "host \"key %d\"", SDL_SCANCODE_F11);
  CreateStringBind(buffer);
#else
  sprintf(buffer, "host \"key %d\"", SDL_SCANCODE_F12);
  CreateStringBind(buffer);
#endif
#else
  sprintf(buffer, "mod_1 \"key %d\"", SDLK_RCTRL);
  CreateStringBind(buffer);
  sprintf(buffer, "mod_1 \"key %d\"", SDLK_LCTRL);
  CreateStringBind(buffer);
  sprintf(buffer, "mod_2 \"key %d\"", SDLK_RALT);
  CreateStringBind(buffer);
  sprintf(buffer, "mod_2 \"key %d\"", SDLK_LALT);
  CreateStringBind(buffer);
  sprintf(buffer, "mod_3 \"key %d\"", SDLK_RSHIFT);
  CreateStringBind(buffer);
  sprintf(buffer, "mod_3 \"key %d\"", SDLK_LSHIFT);
  CreateStringBind(buffer);
#if defined(WIN32) &&                                                                                                                \
    !defined(C_HX_DOS) /* F12 is not a good modifier key in Windows:                                                                 \
                          https://stackoverflow.com/questions/18997754/how-to-disable-f12-to-debug-application-in-visual-studio-2012 \
                        */
  sprintf(buffer, "host \"key %d\"", SDLK_F11);
  CreateStringBind(buffer);
#else
    sprintf(buffer, "host \"key %d\"", SDLK_F12);
    CreateStringBind(buffer);
#endif
#endif
  ...
  sprintf(buffer, "jbutton_0_0 \"stick_0 button 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_0_1 \"stick_0 button 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_0_2 \"stick_0 button 2\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_0_3 \"stick_0 button 3\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_0_4 \"stick_0 button 4\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_0_5 \"stick_0 button 5\" ");
  CreateStringBind(buffer);
  /* joystick2, buttons 1-2 */
  sprintf(buffer, "jbutton_1_0 \"stick_1 button 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jbutton_1_1 \"stick_1 button 1\" ");
  CreateStringBind(buffer);

  /* joystick1, axes 1-4 */
  sprintf(buffer, "jaxis_0_0- \"stick_0 axis 0 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_0+ \"stick_0 axis 0 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_1- \"stick_0 axis 1 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_1+ \"stick_0 axis 1 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_2- \"stick_0 axis 2 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_2+ \"stick_0 axis 2 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_3- \"stick_0 axis 3 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_0_3+ \"stick_0 axis 3 1\" ");
  CreateStringBind(buffer);
  /* joystick2, axes 1-2 */
  sprintf(buffer, "jaxis_1_0- \"stick_1 axis 0 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_1_0+ \"stick_1 axis 0 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_1_1- \"stick_1 axis 1 0\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jaxis_1_1+ \"stick_1 axis 1 1\" ");
  CreateStringBind(buffer);

  /* joystick1, hat */
  sprintf(buffer, "jhat_0_0_0 \"stick_0 hat 0 1\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jhat_0_0_1 \"stick_0 hat 0 2\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jhat_0_0_2 \"stick_0 hat 0 4\" ");
  CreateStringBind(buffer);
  sprintf(buffer, "jhat_0_0_3 \"stick_0 hat 0 8\" ");
  CreateStringBind(buffer);
}
...
void MAPPER_AddHandler(MAPPER_Handler *handler, MapKeys key, Bitu mods,
  ...
  for (CHandlerEventVector_it it = handlergroup.begin();
       it != handlergroup.end(); ++it) {
  ...
  DOSBoxMenu::item &menuitem =
      mainMenu.alloc_item(DOSBoxMenu::item_type_id,
                          std::string("mapper_") + std::string(eventname));
  ...
  if (mapper_addhandler_create_buttons) {
    // and a button in the mapper UI
    {
      ...
      CEventButton *button =
          new CEventButton(PX(next_handler_xpos * 3), PY(next_handler_ypos),
                           BW * 3 * columns, BH, buttonname, event);
      event->notifybutton(button);
    ...
    // now is the time to register it.
    {
      ...
      if (i != pending_string_binds.end()) {
        LOG(LOG_MISC, LOG_WARN)(
            "Found pending event for %s from user's file, applying now",
            tempname);
...
static void MAPPER_SaveBinds(void) {
  ...
  if (loadfile) {
    ...
    while (fgets(linein, 512, loadfile)) {
      ...
      if (strlen(line) > 2 && line[0] == '[' && line[strlen(line) - 1] == ']') {
        ...
        if (strcasecmp(secname, SDL_STRING))
  ...
  fprintf(savefile, "[%s]\n", SDL_STRING);
  char buf[128];
  for (CEventVector_it event_it = events.begin(); event_it != events.end();
       ++event_it) {
    CEvent *event = *(event_it);
    fprintf(savefile, "%s ", event->GetName());
    for (CBindList_it bind_it = event->bindlist.begin();
         bind_it != event->bindlist.end(); ++bind_it) {
      CBind *bind = *(bind_it);
      bind->ConfigName(buf);
      bind->AddFlags(buf);
...
#if defined(WIN32)
  char path[MAX_PATH];
  if (GetFullPathName(mapper.filename.c_str(), MAX_PATH, path, NULL))
...
#endif
  std::string str =
      mapper.filename.substr(mapper.filename.find_last_of("/\\") + 1);
...
static bool MAPPER_LoadBinds(void) {
  ...
  if (othersec && !hasbind)
    return false;
  LOG(LOG_MISC, LOG_NORMAL)("MAPPER: Loading mapper settings from %s",
...
void MAPPER_CheckEvent(SDL_Event *event) {
  ...
  if (log_keyboard_scan_codes) {
    if (event->type == SDL_KEYDOWN || event->type == SDL_KEYUP)
      LOG_MSG(
          "MAPPER: SDL keyboard event (%s): scancode=0x%X sym=0x%X mod=0x%X",
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_SYSWMEVENT: {
      ...
      case WM_COMMAND:
        ...
        if (GetMenu(GetHWND())) {
#if defined(C_SDL2)
          if (mapperMenu.mainMenuWM_COMMAND(
                  (unsigned int)LOWORD(event.syswm.msg->msg.win.wParam)))
            return;
#else
          if (mapperMenu.mainMenuWM_COMMAND(
                  (unsigned int)LOWORD(event.syswm.msg->wParam)))
            return;
#endif
        }
#endif
...
#if defined(C_SDL2)
      if (event.button.which != SDL_TOUCH_MOUSEID) /* don't handle mouse events
                                                      faked by touchscreen */
    ...
    case SDL_QUIT:
      mapper.exit = true;
      break;
    case SDL_KEYDOWN: /* help the user determine keyboard problems by showing
                         keyboard event, scan code, etc. */
    case SDL_KEYUP: {
...
#if defined(WIN32)
      if (key == SDL_SCANCODE_NONUSBACKSLASH) { // Special consideration for JP
                                                // Keyboard
        key = (isJPkeyboard ? SDL_SCANCODE_INTERNATIONAL1
                            : SDL_SCANCODE_NONUSBACKSLASH);
      }
      ...
      tmpl = (size_t)sprintf(tmp, "%c%02x: scan=%d sym=%d mod=%xh name=%s",
                             ...
                             event_count & 0xFF, key, s.sym, s.mod,
                             SDL_GetScancodeName(key));
#else
      tmpl =
          (size_t)sprintf(tmp, "%c%02x: scan=%u sym=%u mod=%xh u=%xh name=%s",
                          (event.type == SDL_KEYDOWN ? 'D' : 'U'),
                          event_count & 0xFF, s.scancode, s.sym, s.mod,
                          s.unicode, SDL_GetKeyName((SDLKey)GetKeyCode(s)));
      //(s.sym ? SDL_GetKeyName((SDLKey)MapSDLCode((Bitu)s.sym)) :
      //SDL_GetKeyName((SDLKey)MapSDLCode((Bitu)sdlkey_map[(s.scancode ?
      //s.scancode : event.key.keysym.scancode)]))));
#endif
      while (tmpl < (440 / 8))
        tmp[tmpl++] = ' ';
...
#elif defined(SDL_DOSBOX_X_SPECIAL)
      {
        ...
        nm[0] = 0;
#if !defined(                                                                  \
    HX_DOS) /* I assume HX DOS doesn't bother with keyboard scancode names */
        GetKeyNameText((s.scancode ? s.scancode : event.key.keysym.scancode)
                           << 16,
                       nm, sizeof(nm) - 1);
#endif

        tmpl =
            sprintf(tmp, "Win32: VK=0x%x kn=%s", (unsigned int)s.win32_vk, nm);
      }
#endif
#endif
...
#elif defined(SDL_DOSBOX_X_SPECIAL) && defined(LINUX)
      {
        ...
        tmpl = (size_t)sprintf(tmp, "X11: Sym=0x%x sn=%s",
                               (unsigned int)s.x11_sym, name ? name : "");
      }
#endif
#endif
      while (tmpl < (250 / 8))
    ...
    default:
      if (mapper.addbind) {
        for (CBindGroup_it it = bindgroups.begin(); it != bindgroups.end();
             ++it) {
...
static void InitializeJoysticks(void) {
  ...
  if (joytype != JOY_NONE) {
    ...
    LOG(LOG_MISC, LOG_DEBUG)("Joystick type != none, SDL reports %u sticks",
                             (unsigned int)mapper.sticks.num);
    if (joytype == JOY_AUTO) {
      ...
      if (mapper.sticks.num > 1) {
        ...
        // choose joystick type now that we know which physical joysticks are
        // usable
        if (first_usable) {
          if (second_usable) {
            joytype = JOY_2AXIS;
...
void MAPPER_RunEvent(Bitu /*val*/) {
  if (hostbutton != NULL)
    hostbutton->Enable(hostkeyalt == 0);
...
void MAPPER_RunInternal() {
  ...
  if (GFX_GetPreventFullscreen()) {
    systemmessagebox("Mapper Editor", MSG_Get("MAPPEREDITOR_NOT_AVAILABLE"),
                     "ok", "info", 1);
    LOG_MSG("Mapper Editor is not available while 3Dfx OpenGL emulation is "
            "running");
    return;
  }
  ...
  mapper.window = OpenGL_using() ? GFX_SetSDLWindowMode(640, 480, SCREEN_OPENGL)
                                 : GFX_SetSDLSurfaceWindow(640, 480);
  ...
  mapper.draw_surface_nonpaletted = SDL_CreateRGBSurface(
      0, 640, 480, 32, 0x0000ff00, 0x00ff0000, 0xff000000, 0);
...
//            SDL_UpdateWindowSurface(mapper.window);
  ...
  SDL_JoystickEventState(SDL_DISABLE);
#endif
  if ((mousetoggle && !mouselocked) || (!mousetoggle && mouselocked))
    GFX_CaptureMouse();
  ...
  mainMenu.get_item("hostkey_mapper")
      .check2(hostkeyalt == 0)
      .set_text("Mapper-defined: " + mapper_keybind)
...
bool MAPPER_IsRunning(void) { return mapper.running; }
...
bool mapper_menu_exit(DOSBoxMenu *const menu,
...
bool mapper_menu_save(DOSBoxMenu *const menu,
                      DOSBoxMenu::item *const menuitem) {
  (void)menu;     // UNUSED
  (void)menuitem; // UNUSED
  MAPPER_SaveBinds();
...
void MAPPER_Init(void) {
  ...
  if (SDL_GetModState() & KMOD_CAPS) {
    for (CBindList_it bit = caps_lock_event->bindlist.begin();
         bit != caps_lock_event->bindlist.end(); ++bit) {
#if SDL_VERSION_ATLEAST(1, 2, 14)
      (*bit)->ActivateBind(32767, true, false);
      (*bit)->DeActivateBind(false);
#else
      (*bit)->ActivateBind(32767, true,
                           true); // Skip the action itself as bios_keyboard.cpp
                                  // handles the startup state.
#endif
    }
  }
  if (SDL_GetModState() & KMOD_NUM) {
    for (CBindList_it bit = num_lock_event->bindlist.begin();
         bit != num_lock_event->bindlist.end(); ++bit) {
#if SDL_VERSION_ATLEAST(1, 2, 14)
      (*bit)->ActivateBind(32767, true, false);
      (*bit)->DeActivateBind(false);
#else
...
void ReloadMapper(Section_prop *section, bool init) {
  if (!init && control->opt_defaultmapper)
    return;
  Prop_path *pp;
#if defined(C_SDL2)
  ...
  if (!loadfile) {
    ...
    if (!loadfile && mapper.filename.size() && config_path.size()) {
      ...
      if (!loadfile) {
        loadfile =
            fopen((config_path + mapper.filename + ".map").c_str(), "rt");
        if (loadfile)
          appendmap = true;
      }
      if (loadfile) {
        mapper.filename =
            config_path + mapper.filename + (appendmap ? ".map" : "");
        fclose(loadfile);
        if (control->opt_erasemapper) {
          LOG_MSG("Erase mapper file: %s\n", mapper.filename.c_str());
          unlink(mapper.filename.c_str());
    ...
    if (!loadfile && mapper.filename.size() && res_path.size()) {
      ...
      if (loadfile) {
        mapper.filename =
            res_path + mapper.filename + (appendmap ? ".map" : "");
        fclose(loadfile);
        if (control->opt_erasemapper) {
          LOG_MSG("Erase mapper file: %s\n", mapper.filename.c_str());
          unlink(mapper.filename.c_str());
...
// SDL needs to be compiled to use it, else the next makes no sense.
    ...
    if (SDL_GetWMInfo(&info)) {
      ...
      if ((desc = XkbGetMap(info.info.x11.display, XkbAllComponentsMask,
                            XkbUseCoreKbd))) {
        if (XkbGetNames(info.info.x11.display, XkbAllNamesMask, desc) == 0) {
          const char *keycodes =
              XGetAtomName(info.info.x11.display, desc->names->keycodes);
          //                  const char* geom =
          //                  XGetAtomName(info.info.x11.display,
          //                  desc->names->geometry);
          if (keycodes) {
            LOG(LOG_MISC, LOG_NORMAL)("keyboard type %s", keycodes);
            if (strncmp(keycodes, "evdev", 5) == 0)
              evdev_input = true;
          }
...
void MAPPER_StartUp() {
  ...
  {
    DOSBoxMenu::item &item =
        mapperMenu.alloc_item(DOSBoxMenu::submenu_type_id, "MapperMenu");
    item.set_text(mainMenu.get_item("mapper_mapper").get_text());
  }

  {
    DOSBoxMenu::item &item =
        mapperMenu.alloc_item(DOSBoxMenu::item_type_id, "ExitMapper");
  ...
  {
    DOSBoxMenu::item &item =
        mapperMenu.alloc_item(DOSBoxMenu::item_type_id, "SaveMapper");
  ...
  mapperMenu.displaylist_append(mapperMenu.display_list,
                                mapperMenu.get_item_id_by_name("MapperMenu"));

  {
    mapperMenu.displaylist_append(
        mapperMenu.get_item("MapperMenu").display_list,
        mapperMenu.get_item_id_by_name("ExitMapper"));

    mapperMenu.displaylist_append(
        mapperMenu.get_item("MapperMenu").display_list,
        mapperMenu.get_item_id_by_name("_separator_"));

    mapperMenu.displaylist_append(
        mapperMenu.get_item("MapperMenu").display_list,
        mapperMenu.get_item_id_by_name("SaveMapper"));
  }
#endif
...
#if 0 /* FIXME: Is this list simply another copy of other pointers? Allowing   \
         this delete[] to commence triggers double-free warnings */
...
void MapperCapCursorToggle(void) { MAPPER_TriggerEventByName("hand_capmouse"); }
...
std::map<std::string, std::string> get_event_map() {
  ...
  for (CHandlerEventVector_it it = handlergroup.begin();
       it != handlergroup.end(); ++it)
    event_map.insert(std::pair<std::string, std::string>((*it)->eventname,
                                                         (*it)->ButtonName()));
  return event_map;
}

void update_bindbutton_text() {
  if (bind_but.prevpage)
    bind_but.prevpage->SetText("<-");
  if (bind_but.nextpage)
    bind_but.nextpage->SetText("->");
  if (bind_but.add)
    bind_but.add->SetText(MSG_Get("ADD"));
  if (bind_but.del)
    bind_but.del->SetText(MSG_Get("DEL"));
  if (bind_but.next)
    bind_but.next->SetText(MSG_Get("NEXT"));
  if (bind_but.save)
    bind_but.save->SetText(MSG_Get("SAVE"));
  if (bind_but.exit)
    bind_but.exit->SetText(MSG_Get("EXIT"));
  if (bind_but.cap)
    bind_but.cap->SetText(MSG_Get("CAPTURE"));
}
...
void set_eventbutton_text(const char *eventname, const char *buttonname) {
  ...
  for (std::vector<CEventButton *>::iterator it = ceventbuttons.begin();
    ...
    CEventButton *button = (CEventButton *)*it;
    if (button != NULL &&
        !strcmp(eventname, button->GetEvent()->eventname.c_str())) {
