...
#if C_FLUIDSYNTH ||                                                            \
    defined(WIN32) && !defined(HX_DOS) && !defined(_WIN32_WINDOWS)
...
void MIDI_State_SaveMessage() {
  ...
  case 0x8: // Note off
    ...
    memset(&midi_state[channel].code_90[arg1], 0xff,
           sizeof(midi_state[channel].code_90[arg1]));
    memset(&midi_state[channel].code_a0[arg1], 0xff,
           sizeof(midi_state[channel].code_a0[arg1]));
    memset(midi_state[channel].code_d0, 0xff,
           sizeof(midi_state[channel].code_d0));
    break;
  ...
  case 0x9: // Note on
    if (arg2 > 0) {
      ...
      memset(&midi_state[channel].code_80[arg1], 0xff,
             sizeof(midi_state[channel].code_80[arg1]));
    } else {
      // velocity = 0 (note off)
      midi_state[channel].code_80[arg1] = arg2;
  ...
  case 0xB: // Controller #s
    ...
    case 0x64:
      break; // Registered Parameter Number (RPN) LSB
    case 0x65:
      break; // Registered Parameter Number (RPN) MSB
    ...
    case 0x79: // All controllers off
      ...
      Set Registered and Non-registered parameter number LSB and MSB (#98-#101)
      to null value (127) Set pitch bender to center (64/0) Reset channel
      pressure to 0 Reset polyphonic pressure for all notes to 0.
      */
      memset(midi_state[channel].code_a0, 0xff,
             sizeof(midi_state[channel].code_a0));
      memset(midi_state[channel].code_c0, 0xff,
             sizeof(midi_state[channel].code_c0));
      memset(midi_state[channel].code_d0, 0xff,
             sizeof(midi_state[channel].code_d0));
      memset(midi_state[channel].code_e0, 0xff,
             sizeof(midi_state[channel].code_e0));

      memset(midi_state[channel].code_b0 + 0x01, 0xff,
             sizeof(midi_state[channel].code_b0[0x01]));
      memset(midi_state[channel].code_b0 + 0x0b, 0xff,
             sizeof(midi_state[channel].code_b0[0x0b]));
      memset(midi_state[channel].code_b0 + 0x40, 0xff,
             sizeof(midi_state[channel].code_b0[0x40]));

      memset(midi_state[channel].code_rpn_coarse, 0xff,
             sizeof(midi_state[channel].code_rpn_coarse));
      memset(midi_state[channel].code_rpn_fine, 0xff,
             sizeof(midi_state[channel].code_rpn_fine));
    ...
    case 0x7b: // All notes off
      memset(midi_state[channel].code_80, 0xff,
             sizeof(midi_state[channel].code_80));
      memset(midi_state[channel].code_90, 0xff,
             sizeof(midi_state[channel].code_90));
      break;
    ...
    // Roland GS
    case 0x00:
      break; // Bank select MSB
    case 0x05:
      break; // Portamento time
    case 0x20:
      break; // Bank select LSB
    case 0x41:
      break; // Portamento
    case 0x42:
      break; // Sostenuto
    case 0x43:
      break; // Soft pedal
    case 0x54:
      break; // Portamento control
    case 0x5B:
      break; // Effect 1 (Reverb) send level
    case 0x5D:
      break; // Effect 3 (Chorus) send level
    case 0x5E:
      break; // Effect 4 (Variation) send level
    ...
    case 0x78: // All sounds off
      ...
      case 0x7E:
      case 0x7F:
              // CONFIRMED:Mok: A real LAPC-I responds to these controllers as
      follows: parts[part]->setHoldPedal(false); parts[part]->allNotesOff();
              break;
      */
...
void MIDI_State_LoadMessage() {
  ...
  for (int lcv = 0; lcv < 16; lcv++) {
    ...
    for (int lcv2 = 0; lcv2 < 0x80; lcv2++) {
      if (lcv2 >= 0x60)
        break;
      if (midi_state[lcv].code_b0[lcv2] == 0xffff)
        continue;
    ...
    for (int lcv2 = 0; lcv2 < 3; lcv2++) {
      if (midi_state[lcv].code_rpn_coarse[lcv2] == 0xffff &&
          midi_state[lcv].code_rpn_fine[lcv2] == 0xffff)
        continue;
    ...
    for (int lcv2 = 0; lcv2 < 0x80; lcv2++) {
      if (midi_state[lcv].code_90[lcv2] == 0xffff)
        continue;
    ...
    for (int lcv2 = 0; lcv2 < 0x80; lcv2++) {
      if (midi_state[lcv].code_a0[lcv2] == 0xffff)
        continue;
...
void MIDI_RawOutByte(uint8_t data) {
  ...
  if (midi.status == 0xf0) {
    if (!(data & 0x80)) {
      ...
      if ((midi.sysex.start) && (midi.sysex.used >= 4) &&
          (midi.sysex.used <= 9) && (midi.sysex.buf[1] == 0x41) &&
          (midi.sysex.buf[3] == 0x16)) {
        LOG(LOG_ALL, LOG_ERROR)("MIDI:Skipping invalid MT-32 SysEx midi "
                                "message (too short to contain a checksum)");
      } else {
        //				LOG(LOG_ALL,LOG_NORMAL)("Play sysex;
        //address:%02X %02X %02X, length:%4d, delay:%3d", midi.sysex.buf[5],
        //midi.sysex.buf[6], midi.sysex.buf[7], midi.sysex.used,
        //midi.sysex.delay);
        midi.handler->PlaySysex(midi.sysex.buf, midi.sysex.used);

        if (roland_gs_sysex) {
          if (midi.sysex.buf[1] == 0x41 /*Roland*/ &&
              midi.sysex.buf[3] == 0x42 /*GS*/ &&
              midi.sysex.buf[4] == 0x12 /*Send*/) {
            const uint32_t addr = ((uint32_t)midi.sysex.buf[5] << 16) +
        ...
        if (midi.sysex.start) {
          if (midi.sysex.buf[5] == 0x7F) {
            ...
            midi.sysex.delay =
                (Bitu)(((float)(midi.sysex.used) * 1.25f) * 1000.0f / 3125.0f) +
                2;
...
bool MIDI_Available(void) { return midi.available; }
...
public:
  MIDI(Section *configuration) : Module_base(configuration) {
    ...
    //		MAPPER_AddHandler(MIDI_SaveRawEvent,MK_f8,MMOD1|MMOD2,"caprawmidi","Cap
    //MIDI");
    midi.sysex.delay = 0;
    midi.sysex.start = 0;
    ...
    if (strcasecmp(dev, "default")) {
      ...
      if (handler == NULL)
        ...
        LOG_MSG("MIDI:Cannot open device:%s with config:%s. Finding default "
                "handler.",
                dev, conf);
    }
...
void MIDI_ListHandler(Program *caller, const char *name) {
  ...
  if (!found)
    caller->WriteOut("MIDI handler not available - %s\n", name);
}
