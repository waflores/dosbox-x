...
int X11_ErrorHandler(Display *disp, XErrorEvent *xev) {
  ...
  fprintf(stderr,
          "type=%u resid=%lxh serial=%lxh error_code=%lu request_code=%lu "
          "minor_code=%lu\n",
          (unsigned int)xev->type, (unsigned long)xev->resourceid,
          (unsigned long)xev->serial, (unsigned long)xev->error_code,
          (unsigned long)xev->request_code, (unsigned long)xev->minor_code);

  return 0; // ignored anyway
}

void X11_ErrorHandlerInstall(void) { XSetErrorHandler(X11_ErrorHandler); }
#endif
...
bool X11_XRR_check(Display *display) {
  if (!X11_XRR_checked) {
    ...
    LOG_MSG("X11 extension XRANDR is %s",
            X11_XRR_avail ? "available" : "not available");
  }
...
void LinuxX11_OnTop(bool f) {
  ...
  if (SDL_GetWindowWMInfo(GFX_GetSDLWindow(), &wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      Atom wmStateAbove =
          XInternAtom(wminfo.info.x11.display, "_NET_WM_STATE_ABOVE", 1);
      if (wmStateAbove == None)
        return;

      Atom wmNetWmState =
          XInternAtom(wminfo.info.x11.display, "_NET_WM_STATE", 1);
      ...
      XSendEvent(wminfo.info.x11.display,
                 DefaultRootWindow(wminfo.info.x11.display), False,
...
#if C_X11_XKB
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      /* detect xkbmap with Ro and Yen keys mapped to \, determine the scan
       * codes, and then hack the SDL 1.x X11 driver to handle it.
       *
       * Never assume specific scan codes, even though on my system the scan
       * codes are 97 (Yen) and 132 (Ro) both mapped to \ backslash.
       *
       * We can use the index to look for keysyms that mention backslash and
       * underscore (Ro) or backslash and bar (Yen) */
      ...
      for (i = 0; i < 256; i++) {
        for (j = 0; j < 4; j++)
          sym[j] =
              XKeycodeToKeysym(wminfo.info.x11.display, (KeyCode)i, (int)j);
          ...
          if (sym[1] == '_') { /* shift + backslash == _   means we found Ro */
            if (ro < 0)
              ro = (int)i;
                     ...
                     '|') { /* shift + backslash == |   means we found Yen */
            if (yen < 0)
              yen = (int)i;
          }
        }
...
unsigned int Linux_GetKeyboardLayout(void) {
  ...
  if (SDL_GetWindowWMInfo(GFX_GetSDLWindow(), &wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      ...
      XkbDescPtr desc = XkbGetKeyboard(wminfo.info.x11.display,
                                       XkbAllComponentsMask, XkbUseCoreKbd);
      char *group = desc ? XGetAtomName(wminfo.info.x11.display,
                                        desc->names->groups[state.group])
                         : NULL;

      if (group != NULL)
        LOG_MSG("Current X11 keyboard layout (full name) is: '%s'\n", group);

      /* FIXME: Does this allocate anything? Do I need to free it? I am
       * concerned about memory leaks --J.C. */
...
void UpdateWindowDimensions_Linux(void) {
  ...
  if (SDL_GetWindowWMInfo(GFX_GetSDLWindow(), &wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      ...
      XGetWindowAttributes(wminfo.info.x11.display, wminfo.info.x11.window,
                           &attr);

      /* we also want to ask X11 if the various atoms have been set by the
       * window manager to signal that our window has been maximized. generally
       * when the window has been maximized, SDL video mode setting has no
       * effect on the dimensions of the window when it's been maximized so it
       * helps to know in order to make better choices */
      if (GFX_IsFullscreen()) {
      } else {
        Atom atomWmState =
            XInternAtom(wminfo.info.x11.display, "_NET_WM_STATE", False);
        Atom atomMaxVert = XInternAtom(wminfo.info.x11.display,
                                       "_NET_WM_STATE_MAXIMIZED_VERT", False);
        Atom atomMaxHorz = XInternAtom(wminfo.info.x11.display,
        ...
        XGetWindowProperty(wminfo.info.x11.display, wminfo.info.x11.window,
                           /*atom to query*/ atomWmState, /*offset*/ 0,
                           /*length*/ 16384, /*delete*/ False,
                           /*request type*/ AnyPropertyType,
                           /*returns...*/ &type, &format, &nItem, &bytesAfter,
                           &properties);
      ...
      LOG_MSG("X11 main window is %u x %u maximized=%u", attr.width,
              attr.height, maximized);

      UpdateWindowDimensions((unsigned int)attr.width,
  ...
  if (SDL_GetWMInfo(&wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      ...
      memset(&attr, 0, sizeof(attr));
      XGetWindowAttributes(wminfo.info.x11.display,
                           GFX_IsFullscreen() ? wminfo.info.x11.fswindow
...
static bool Linux_TryXRandrGetDPI(ScreenSizeInfo &info, Display *display,
  ...
  XTranslateCoordinates(display, window, DefaultRootWindow(display), 0, 0, &x,
                        &y, &child);

  attr.x = x - attr.x;
  attr.y = y - attr.y;

  if ((xr_screen = XRRGetScreenResources(display,
                                         DefaultRootWindow(display))) != NULL) {
    /* Look for a valid CRTC, don't assume the first is valid (as the
     * StackOverflow example does) */
    for (int c = 0; c < xr_screen->ncrtc; c++) {
      XRRCrtcInfo *chk =
          XRRGetCrtcInfo(display, xr_screen, xr_screen->crtcs[c]);
      if (chk == NULL)
        continue;
      if (chk->width == 0 || chk->height == 0)
        continue;

      LOG_MSG("XRandR CRTC %u: pos=(%d,%d) size=(%d,%d) outputs=%d", c, chk->x,
              chk->y, chk->width, chk->height, chk->noutput);
      ...
      if (match_x >= chk->x && match_x < (chk->x + (int)chk->width) &&
        ...
        LOG_MSG("Our window lies on this CRTC display (window pos=(%d,%d) "
                "size=(%d,%d) match=(%d,%d)).",
                attr.x, attr.y, attr.width, attr.height, match_x, match_y);

        if (chk->noutput > 0) {
          for (int o = 0; o < chk->noutput; o++) {
            XRROutputInfo *ochk =
                XRRGetOutputInfo(display, xr_screen, chk->outputs[o]);
            if (ochk == NULL)
              continue;
            ...
            LOG_MSG("  Goes to output %u: name='%s' size_mm=(%lu x %lu)", o,
                    oname.c_str(), ochk->mm_width, ochk->mm_height);
  ...
  if (SDL_GetWindowWMInfo(GFX_GetSDLWindow(), &wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      if (Linux_TryXRandrGetDPI(info, wminfo.info.x11.display,
        ...
        /* fallback to X11 method, which may not return accurate info on modern
         * systems */
        ...
        if (rootWindow != 0) {
          ...
          info.screen_dimensions_pixels.width =
              DisplayWidth(wminfo.info.x11.display, screen);
          info.screen_dimensions_pixels.height =
              DisplayHeight(wminfo.info.x11.display, screen);

          info.screen_dimensions_mm.width =
              DisplayWidthMM(wminfo.info.x11.display, screen);
          info.screen_dimensions_mm.height =
              DisplayHeightMM(wminfo.info.x11.display, screen);
  ...
  if (SDL_GetWindowWMInfo(GFX_GetSDLWindow(), &wminfo) >= 0) {
    ...
    if (wminfo.subsystem == SDL_SYSWM_X11 && wminfo.info.x11.display != NULL) {
      ...
      if (rootWindow != 0) {
        ...
        LOG_MSG("Root window (ID %lu) is %lu x %lu", (unsigned long)rootWindow,
                (unsigned long)rootWinAttr.width,
                (unsigned long)rootWinAttr.height);
