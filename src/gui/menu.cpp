...
const DOSBoxMenu::mapper_event_t
    DOSBoxMenu::unassigned_mapper_event; /* empty std::string */

static DOSBoxMenu::item_handle_t separator_alloc = 0;
static std::vector<DOSBoxMenu::item_handle_t> separators;

static std::string not_recommended =
...
static const char *def_menu__toplevel[] = {
    "MainMenu",    "CpuMenu", "VideoMenu", "SoundMenu", "DOSMenu", "DriveMenu",
...
/* main menu ("MainMenu") */
static const char *def_menu_main[] = {"mapper_gui",
                                      "mapper_mapper",
#if !defined(HX_DOS)
                                      "--",
                                      "mapper_quickrun",
...
static const char *def_menu_main_sendkey[] = {"sendkey_winlogo",
                                              "sendkey_winmenu",
                                              "sendkey_alttab",
                                              "sendkey_ctrlesc",
                                              "sendkey_ctrlbreak",
...
static const char *def_menu_main_hostkey[] = {
    "hostkey_ctrlalt",  "hostkey_ctrlshift",
    "hostkey_altshift", "--",
    "hostkey_mapper",   NULL};

/* main -> mouse wheel menu ("WheelToArrows") */
static const char *def_menu_main_wheelarrow[] = {"wheel_updown",
                                                 "wheel_leftright",
                                                 "wheel_pageupdown",
                                                 "wheel_ctrlupdown",
                                                 "wheel_ctrlleftright",
...
static const char *def_menu_cpu_speed[] = {
    "cpu88-4",    "cpu286-8",   "cpu286-12",  "cpu286-25",  "cpu386-25",
    "cpu386-33",  "cpu486-33",  "cpu486-66",  "cpu486-100", "cpu486-133",
    "cpu586-60",  "cpu586-66",  "cpu586-75",  "cpu586-90",  "cpu586-100",
    "cpu586-120", "cpu586-133", "cpu586-166", "cpu586-200", "cpuak6-200",
    "cpuak6-300", "cpuath-600", "cpu686-866", NULL};

/* cpu -> core menu ("CpuCoreMenu") */
static const char *def_menu_cpu_core[] = {"mapper_cycauto",
                                          "--",
                                          "mapper_normal",
...
static const char *def_menu_cpu_type[] = {
    "cputype_auto",         "--",
    "cputype_8086",         "cputype_8086_prefetch",
    "cputype_80186",        "cputype_80186_prefetch",
    "cputype_286",          "cputype_286_prefetch",
    "cputype_386",          "cputype_386_prefetch",
    "cputype_486old",       "cputype_486old_prefetch",
    "cputype_486",          "cputype_486_prefetch",
    "cputype_pentium",      "cputype_pentium_mmx",
    "cputype_ppro_slow",    "cputype_pentium_ii",
    "cputype_pentium_iii",  "--",
    "cputype_experimental", NULL};

/* cpu menu ("CpuMenu") */
static const char *def_menu_cpu[] = {
    "mapper_speedlock2", /* NTS: "mapper_speedlock" doesn't work for a menu item
                            because it requires holding the key */
    "mapper_speednorm",
    "mapper_speedup",
    "mapper_slowdown",
    "--",
...
static const char *def_menu_video_frameskip[] = {
    "frameskip_0", "frameskip_1", "frameskip_2",  "frameskip_3",
    "frameskip_4", "frameskip_5", "frameskip_6",  "frameskip_7",
    "frameskip_8", "frameskip_9", "frameskip_10", NULL};

/* video prevent capture menu ("VideoPreventCaptureMenu") */
static const char *def_menu_video_preventcapture[] = {
    "prevcap_none", "prevcap_blank", "prevcap_invisible", NULL};

/* video scaler menu ("VideoRatioMenu") */
static const char *def_menu_video_ratio[] = {"video_ratio_1_1",
                                             "video_ratio_3_2",
                                             "video_ratio_4_3",
                                             "video_ratio_16_9",
                                             "video_ratio_16_10",
...
static const char *def_menu_video_scaler[] = {NULL};

/* video output menu ("VideoOutputMenu") */
static const char *def_menu_video_output[] = {"output_surface",
...
static const char *def_menu_video_3dfx[] = {"3dfx_voodoo", "3dfx_glide", NULL};

/* video text-mode menu ("VideoTextmodeMenu") */
static const char *def_menu_video_textmode[] = {
    "clear_screen",    "vga_9widetext", "--",
    "text_background", "text_blinking", "--",
    "line_80x25",      "line_80x43",    "line_80x50",
    "line_80x60",      "line_132x25",   "line_132x43",
    "line_132x50",     "line_132x60",   NULL};

#if defined(USE_TTF)
/* video TTF menu ("VideoTTFMenu") */
static const char *def_menu_video_ttf[] = {"mapper_incsize",
                                           "mapper_decsize",
                                           "mapper_resetcolor",
                                           "--",
                                           "ttf_showbold",
...
static const char *def_menu_video_vsync[] = {
    "vsync_on", "vsync_force",        "vsync_host", "vsync_off",
    "--",       "vsync_set_syncrate", NULL};

/* video overscan menu ("VideoOverscanMenu") */
static const char *def_menu_video_overscan[] = {
    "overscan_0", "overscan_1", "overscan_2",  "overscan_3",
    "overscan_4", "overscan_5", "overscan_6",  "overscan_7",
    "overscan_8", "overscan_9", "overscan_10", NULL};

/* video output menu ("VideoPC98Menu") */
static const char *def_menu_video_pc98[] = {"pc98_use_uskb",
...
/* DOS menu ("DOSMenu") */
static const char *def_menu_dos[] = {"DOSVerMenu",
                                     "DOSLFNMenu",
...
static const char *def_menu_dos_mouse[] = {"dos_mouse_enable_int33",
                                           "dos_mouse_y_axis_reverse", "--",
                                           "dos_mouse_sensitivity", NULL};

/* DOS version menu ("DOSVerMenu") */
static const char *def_menu_dos_ver[] = {
    "dos_ver_330", "dos_ver_500",  "dos_ver_622", "dos_ver_710",
    "--",          "dos_ver_edit", NULL};

/* DOS LFN menu ("DOSLFNMenu") */
static const char *def_menu_dos_lfn[] = {"dos_lfn_auto", "--", "dos_lfn_enable",
                                         "dos_lfn_disable", NULL};

/* DOS EMS menu ("DOSEMSMenu") */
static const char *def_menu_dos_ems[] = {
    "dos_ems_true", "dos_ems_board", "dos_ems_emm386", "dos_ems_false", NULL};

/* DOS disk rate menu ("DOSDiskRateMenu") */
static const char *def_menu_dos_diskrate[] = {"limit_hdd_rate",
                                              "limit_floppy_rate", NULL};

#if defined(WIN32) && !defined(HX_DOS) || defined(LINUX) || defined(MACOSX)
/* DOS WIN menu ("DOSWinMenu") */
static const char *def_menu_dos_win[] = {"dos_win_autorun", "dos_win_transpath",
                                         "dos_win_wait", "dos_win_quiet", NULL};
#endif

/* sound menu ("SoundMenu") */
static const char *def_menu_sound[] = {"mapper_volup",
                                       "mapper_voldown",
                                       "--",
                                       "mapper_recvolup",
                                       "mapper_recvoldown",
...
static const char *def_menu_capture_format[] = {
    "capture_fmt_avi_zmbv", "capture_fmt_mpegts_h264", NULL};
#endif
#endif

/* Save/load options */
static const char *save_load_options[] = {
    "enable_autosave", "noremark_savestate", "force_loadstate", "usesavefile",
    NULL};

/* Save slots */
static const char *def_save_slots[] = {"current_page",
                                       "prev_page",
                                       "next_page",
                                       "--",
                                       "first_page",
...
static const char *def_menu_drive[] = {
    "DriveA", "DriveB", "DriveC", "DriveD", "DriveE", "DriveF", "DriveG",
    ...
    "DriveN", "DriveO", "DriveP", "DriveQ", "DriveR", "DriveS", "DriveT",
    "DriveU", "DriveV", "DriveW", "DriveX", "DriveY", "DriveZ", NULL};
...
#if C_DEBUG
static const char *def_menu_debug[] = {"mapper_debugger",
                                       "--",
                                       "debugger_rundebug",
                                       "debugger_runnormal",
                                       "debugger_runwatch",
                                       ...
                                       "debug_logfileio",
                                       NULL};
#elif !defined(MACOSX) && !defined(LINUX) && !defined(HX_DOS) &&               \
    !defined(C_EMSCRIPTEN)
static const char *def_menu_help_debug[] = {"show_console", "wait_on_error",
                                            "--", "video_debug_overlay", NULL};
#endif

/* help menu ("HelpMenu") */
static const char *def_menu_help[] = {"help_intro",
                                      "help_about",
#if !defined(HX_DOS)
                                      "--",
                                      "help_homepage",
                                      ...
                                      "help_prt",
#endif
#if !C_DEBUG && !defined(MACOSX) && !defined(LINUX) && !defined(HX_DOS) &&     \
    !defined(C_EMSCRIPTEN)
                                      "HelpDebugMenu",
#endif
#if C_PCAP || C_PRINTER && defined(WIN32) ||                                   \
    !C_DEBUG && !defined(MACOSX) && !defined(LINUX) && !defined(HX_DOS) &&     \
        !defined(C_EMSCRIPTEN)
                                      "--",
#endif
                                      "HelpCommandMenu",
...
void resetFontSize();
#endif
bool DOSBox_isMenuVisible(void) { return menu.toggle; }
...
DOSBoxMenu::item_handle_t
DOSBoxMenu::get_item_id_by_name(const std::string &name) {
  const auto i = name_map.find(name);
...
DOSBoxMenu::item &DOSBoxMenu::alloc_item(const enum item_type_t type,
  ...
  if (newsize < 64)
    newsize = 64;
  if (newsize > master_list_limit)
    newsize = master_list_limit;
...
void DOSBoxMenu::delete_item(const item_handle_t i) {
...

  {
    ...
    if (it != name_map.end()) {
      if (it->second != i)
        E_Exit("DOSBoxMenu::delete_item() master_id mismatch");
      name_map.erase(it);
    }
  }
...
const char *DOSBoxMenu::TypeToString(const enum item_type_t type) {
  ...
  case item_type_id:
    return "Item";
  case submenu_type_id:
    return "Submenu";
  case separator_type_id:
    return "Separator";
  case vseparator_type_id:
    return "VSeparator";
  default:
    break;
  }
...
void DOSBoxMenu::dump_log_displaylist(DOSBoxMenu::displaylist &ls,
                                      unsigned int indent) {
  std::string prep;
  ...
  for (const auto &id : ls.disp_list) {
    ...
    LOG_MSG("%sid=%u type=\"%s\" name=\"%s\" text=\"%s\"", prep.c_str(),
            (unsigned int)item.master_id, TypeToString(item.type),
            item.name.c_str(), item.text.c_str());
...
void DOSBoxMenu::dump_log_debug(void) {
  ...
  for (auto &id : master_list) {
    if (id.is_allocated()) {
      LOG_MSG("+ id=%u type=\"%s\" name=\"%s\" text=\"%s\" shortcut=\"%s\" "
              "desc=\"%s\"",
              (unsigned int)id.master_id, TypeToString(id.type),
              id.name.c_str(), id.text.c_str(), id.shortcut_text.c_str(),
...
DOSBoxMenu::item &DOSBoxMenu::item::allocate(const item_handle_t id,
                                             const enum item_type_t new_type,
...
void DOSBoxMenu::displaylist_append(displaylist &ls,
                                    const DOSBoxMenu::item_handle_t item_id) {
...
bool DOSBoxMenu::mainMenuAction(unsigned int id) {
  if (id < nsMenuMinimumID)
    return false;
  id -= nsMenuMinimumID;

  if (id >= master_list.size())
    return false;

  item &item = master_list[id];
  if (!item.status.allocated || item.master_id == unassigned_item_handle)
    return false;
...
bool DOSBoxMenu::nsMenuInit(void) {
  if (nsMenu == NULL) {
    ...
    /* For whatever reason, Mac OS X will always make the first top level menu
       the Application menu and will put the name of the app there NO MATTER
       WHAT */
    ...
    /* release our handle on the nsMenus. Mac OS X will keep them alive with its
       reference until the menu is destroyed at which point all items and
       submenus will be automatically destroyed */
    for (auto &id : master_list) {
      if (id.nsMenu != NULL) {
...
void *DOSBoxMenu::getNsMenu(void) const { return nsMenu; }
#endif
...
LPWSTR getWString(std::string str, wchar_t *def, wchar_t *&buffer) {
  ...
  if (control->opt_langcp && msgcodepage > 0 && isSupportedCP(msgcodepage) &&
    ...
    cp = msgcodepage;
  else if ((!dos.loaded_codepage || dos_kernel_disabled || force_conversion) &&
           section != NULL && !control->opt_noconfig) {
    char *countrystr = (char *)section->Get_string("country"),
         *r = strchr(countrystr, ',');
    if (r != NULL && *(r + 1) && !IS_PC98_ARCH && !IS_JEGA_ARCH && !IS_DOSV) {
      cp = atoi(trim(r + 1));
      if ((cp < 1 || !isSupportedCP(cp)) && msgcodepage > 0)
        cp = msgcodepage;
    } else if (msgcodepage > 0)
      cp = msgcodepage;
    if (cp < 1 || !isSupportedCP(cp)) {
      if (IS_PC98_ARCH || IS_JEGA_ARCH || IS_JDOSV)
        cp = 932;
      else if (IS_PDOSV)
        cp = 936;
      else if (IS_KDOSV)
  ...
  if (cp > 0) {
    ...
    if (reqsize > 0 && MultiByteToWideChar(cp, 0, str.c_str(), len + 1, buffer,
                                           reqsize) == reqsize)
      ret = (LPWSTR)buffer;
  } else {
  ...
  if (type == separator_type_id) {
    ...
    if (winMenu != NULL) {
      ...
      if (wcscmp(str, L""))
        ...
        AppendMenu(handle, MF_POPUP | MF_STRING, (uintptr_t)winMenu,
    ...
    if (wcscmp(str, L""))
      ...
      AppendMenu(handle, attr, (uintptr_t)(master_id + winMenuMinimumID),
                 winConstructMenuText().c_str());
  }
  if (buffer != NULL) {
    delete[] buffer;
...
HMENU DOSBoxMenu::getWinMenu(void) const { return winMenu; }

/* call this from WM_COMMAND */
bool DOSBoxMenu::mainMenuWM_COMMAND(unsigned int id) {
  if (id < winMenuMinimumID)
    return false;
  id -= winMenuMinimumID;
  ...
  if (menu.winMenu != NULL && status.in_use && status.changed) {
    ...
    if (phandle != NULL) {
      if (type == separator_type_id) {
        ...
        LPWSTR str = getWString(winConstructMenuText(), emptyStr, buffer);
        if (wcscmp(str, L""))
          ModifyMenuW(phandle, (uintptr_t)(master_id + winMenuMinimumID),
                      attr | MF_BYCOMMAND,
                      (uintptr_t)(master_id + winMenuMinimumID), str);
        else
          ModifyMenu(phandle, (uintptr_t)(master_id + winMenuMinimumID),
                     attr | MF_BYCOMMAND,
                     (uintptr_t)(master_id + winMenuMinimumID),
                     winConstructMenuText().c_str());
      }
    }
  }
...
static DOSBoxMenu::item_handle_t
separator_get(const DOSBoxMenu::item_type_t t = DOSBoxMenu::separator_type_id) {
  assert(separator_alloc <= separators.size());
  if (separator_alloc == separators.size()) {
    DOSBoxMenu::item &nitem =
        mainMenu.alloc_item(t, separator_id(separator_alloc));
...
void ConstructSubMenu(DOSBoxMenu::item_handle_t item_id,
  ...
  for (size_t i = 0; list[i] != NULL; i++) {
    ...
    if (!strcmp(ref, "--")) {
      ...
      DOSBoxMenu::item_handle_t separator_handle =
          separator_get(DOSBoxMenu::separator_type_id);
      mainMenu.displaylist_append(mainMenu.get_item(item_id).display_list,
                                  separator_handle);
    } else if (!strcmp(ref, "||")) {
      /* ditto */
      DOSBoxMenu::item_handle_t separator_handle =
      ...
      mainMenu.displaylist_append(mainMenu.get_item(item_id).display_list,
                                  mainMenu.get_item_id_by_name(ref));
    }
  }
}
...
void ConstructMenu(void) {
  ...
  /* main menu */
  ConstructSubMenu(mainMenu.get_item("MainMenu").get_master_id(),
                   def_menu_main);

  /* main sendkey menu */
  ConstructSubMenu(mainMenu.get_item("MainSendKey").get_master_id(),
                   def_menu_main_sendkey);

  /* main hostkey menu */
  ConstructSubMenu(mainMenu.get_item("MainHostKey").get_master_id(),
                   def_menu_main_hostkey);

  /* main mouse wheel movements menu */
  ConstructSubMenu(mainMenu.get_item("WheelToArrow").get_master_id(),
                   def_menu_main_wheelarrow);

  /* shared clipboard menu */
  ConstructSubMenu(mainMenu.get_item("SharedClipboard").get_master_id(),
                   def_menu_main_clipboard);

  /* cpu menu */
  ConstructSubMenu(mainMenu.get_item("CpuMenu").get_master_id(), def_menu_cpu);

  /* cpu speed menu */
  ConstructSubMenu(mainMenu.get_item("CpuSpeedMenu").get_master_id(),
                   def_menu_cpu_speed);

  /* cpu core menu */
  ConstructSubMenu(mainMenu.get_item("CpuCoreMenu").get_master_id(),
                   def_menu_cpu_core);

  /* cpu type menu */
  ConstructSubMenu(mainMenu.get_item("CpuTypeMenu").get_master_id(),
                   def_menu_cpu_type);

  /* video menu */
  ConstructSubMenu(mainMenu.get_item("VideoMenu").get_master_id(),
                   def_menu_video);

  /* video ratio menu */
  ConstructSubMenu(mainMenu.get_item("VideoRatioMenu").get_master_id(),
                   def_menu_video_ratio);

#if defined(WIN32) || defined(MACOSX)
  /* video prevent capture menu */
  ConstructSubMenu(mainMenu.get_item("VideoPreventCaptureMenu").get_master_id(),
                   def_menu_video_preventcapture);
#endif

  /* video frameskip menu */
  ConstructSubMenu(mainMenu.get_item("VideoFrameskipMenu").get_master_id(),
                   def_menu_video_frameskip);

  /* video scaler menu */
  ConstructSubMenu(mainMenu.get_item("VideoScalerMenu").get_master_id(),
                   def_menu_video_scaler);
  {
    size_t count = 0;
  ...
  /* video output menu */
  ConstructSubMenu(mainMenu.get_item("VideoOutputMenu").get_master_id(),
                   def_menu_video_output);

  /* video text-mode menu */
  ConstructSubMenu(mainMenu.get_item("VideoTextmodeMenu").get_master_id(),
                   def_menu_video_textmode);

#if defined(USE_TTF)
  /* video TTF menu */
  ConstructSubMenu(mainMenu.get_item("VideoTTFMenu").get_master_id(),
                   def_menu_video_ttf);
#endif

  /* video vsync menu */
  ConstructSubMenu(mainMenu.get_item("VideoVsyncMenu").get_master_id(),
                   def_menu_video_vsync);

  /* video overscan menu */
  ConstructSubMenu(mainMenu.get_item("VideoOverscanMenu").get_master_id(),
                   def_menu_video_overscan);

  /* video PC-98 menu */
  ConstructSubMenu(mainMenu.get_item("VideoPC98Menu").get_master_id(),
                   def_menu_video_pc98);

  /* video 3dfx menu */
  ConstructSubMenu(mainMenu.get_item("Video3dfxMenu").get_master_id(),
                   def_menu_video_3dfx);

  /* sound menu */
  ConstructSubMenu(mainMenu.get_item("SoundMenu").get_master_id(),
                   def_menu_sound);
  ...
  /* DOS mouse menu */
  ConstructSubMenu(mainMenu.get_item("DOSMouseMenu").get_master_id(),
                   def_menu_dos_mouse);

  /* DOS version menu */
  ConstructSubMenu(mainMenu.get_item("DOSVerMenu").get_master_id(),
                   def_menu_dos_ver);

  /* DOS LFN menu */
  ConstructSubMenu(mainMenu.get_item("DOSLFNMenu").get_master_id(),
                   def_menu_dos_lfn);

  /* DOS EMS menu */
  ConstructSubMenu(mainMenu.get_item("DOSEMSMenu").get_master_id(),
                   def_menu_dos_ems);

  /* DOS disk rate menu */
  ConstructSubMenu(mainMenu.get_item("DOSDiskRateMenu").get_master_id(),
                   def_menu_dos_diskrate);

#if defined(WIN32) && !defined(HX_DOS) || defined(LINUX) || defined(MACOSX)
  /* DOS WIN menu */
  ConstructSubMenu(mainMenu.get_item("DOSWinMenu").get_master_id(),
                   def_menu_dos_win);
#endif

#if !defined(C_EMSCRIPTEN)
  /* capture menu */
  ConstructSubMenu(mainMenu.get_item("CaptureMenu").get_master_id(),
                   def_menu_capture);
#endif
...
#if (C_SSHOT)
  /* capture format menu */
  ConstructSubMenu(mainMenu.get_item("CaptureFormatMenu").get_master_id(),
                   def_menu_capture_format);
#endif
#endif
  ConstructSubMenu(mainMenu.get_item("saveoptionmenu").get_master_id(),
                   save_load_options);
  ConstructSubMenu(mainMenu.get_item("saveslotmenu").get_master_id(),
                   def_save_slots);

  /* Drive menu */
  ConstructSubMenu(mainMenu.get_item("DriveMenu").get_master_id(),
                   def_menu_drive);
  for (char drv = 'A'; drv <= 'Z'; drv++) {
    const std::string dname = std::string("Drive") + drv;
    for (size_t i = 0; drive_opts[i][0] != NULL; i++) {
      const std::string name =
          std::string("drive_") + drv + "_" + drive_opts[i][0];

      if (mainMenu.item_exists(name)) {
        mainMenu.displaylist_append(mainMenu.get_item(dname).display_list,
                                    mainMenu.get_item_id_by_name(name));
      }
    }
  }
  ...
  /* help menu */
  ConstructSubMenu(mainMenu.get_item("HelpMenu").get_master_id(),
                   def_menu_help);
  ...
  while (cmd_list[cmd_index].name) {
    if (!cmd_list[cmd_index].flags) {
      strcpy(help_command_temp[i],
             ("command_" + std::string(cmd_list[cmd_index].name)).c_str());
  ...
  ConstructSubMenu(mainMenu.get_item("HelpCommandMenu").get_master_id(),
                   def_menu_help_command);

#if C_DEBUG
  /* debug menu */
  ConstructSubMenu(mainMenu.get_item("DebugMenu").get_master_id(),
                   def_menu_debug);
#elif !defined(MACOSX) && !defined(LINUX) && !defined(HX_DOS) &&               \
    !defined(C_EMSCRIPTEN)
  /* help debug menu */
  ConstructSubMenu(mainMenu.get_item("HelpDebugMenu").get_master_id(),
                   def_menu_help_debug);
#endif
}
...
bool MENU_SetBool(std::string secname, std::string value) {
  ...
  if (sec)
    SetVal(secname, value, sec->Get_bool(value) ? "false" : "true");
...
std::string MSCDEX_Output(int num) {
  ...
  switch (num) {
  case 0:
    return MSCDEX_MSG + "installed";
  case 1:
    return MSCDEX_MSG + MSCDEX_MSG_Failure +
           "Drive-letters of multiple CDRom-drives have to be continuous.";
  case 2:
    return MSCDEX_MSG + MSCDEX_MSG_Failure + "Not yet supported.";
  case 3:
    return MSCDEX_MSG + MSCDEX_MSG_Failure + "Path not valid.";
  case 4:
    return MSCDEX_MSG + MSCDEX_MSG_Failure +
           "Too many CDRom-drives (max: 5). MSCDEX Installation failed";
  case 5:
    return MSCDEX_MSG + "Mounted subdirectory: limited support.";
  case 6:
    return MSCDEX_MSG + MSCDEX_MSG_Failure + "Unknown error";
  default:
    return {};
  }
}
...
void DOSBox_SetMenu(DOSBoxMenu &altMenu) {
...
#endif
#if DOSBOXMENU_TYPE ==                                                         \
    DOSBOXMENU_NSMENU /* TODO: Move to menu.cpp DOSBox_SetMenu() and add       \
                         setmenu(NULL) to DOSBox_NoMenu() @emendelson request  \
                         showmenu=false */
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "Win32: loading and attaching custom menu resource to DOSBox-X's window");
...
void DOSBox_SetMenu(void) {
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "Win32: loading and attaching menu resource to DOSBox-X's window");
...
void ToggleMenu(bool pressed) {
  ...
  if (!menu.gui || !pressed || fullscreen)
    return;
  if (!menu.toggle) {
    menu.toggle = true;
    DOSBox_SetMenu();
...
#if !defined(WIN32) || defined(HX_DOS)
int Reflect_Menu(void) { return 0; }

void DOSBox_RefreshMenu(void) {}

void DOSBox_CheckOS(int &id, int &major, int &minor) { id = major = minor = 0; }
#endif
...
void MSG_WM_COMMAND_handle(SDL_SysWMmsg &Message) {
...
#if defined(C_SDL2)
  if (Message.msg.win.msg != WM_COMMAND)
    return;
#else
  ...
  if (!menu.gui || GetSetSDLValue(1, fullScreenString, 0))
    return;
  if (!GetMenu(GetHWND()))
    return;
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  if (mainMenu.mainMenuWM_COMMAND((unsigned int)LOWORD(wParam)))
    return;
#endif
#else
  (void)Message; // UNUSED
...
void DOSBox_SetSysMenu(void) {
  ...
  sysmenu =
      GetSystemMenu(GetHWND(), TRUE); // revert, so we can reapply menu items
  ...
  {
    key = "togmenu";
    msg = mainMenu.get_item("mapper_" + key).get_text() +
          (get_mapper_shortcut(key.c_str()).size()
               ? "\t" + get_mapper_shortcut(key.c_str())
               : "");
    memset(&mii, 0, sizeof(mii));
    mii.cbSize = sizeof(mii);
    mii.fMask = MIIM_ID | MIIM_STRING | MIIM_STATE;
    mii.fState = (menu.toggle ? MFS_CHECKED : 0) |
                 (GFX_GetPreventFullscreen() ? MFS_DISABLED : MFS_ENABLED);
...
void SearchFolder(char path[MAX_PATH], char drive, std::string drive_type) {
  ...
  if (hFind != INVALID_HANDLE_VALUE)
    MountDrive_2(drive, path, drive_type);
  FindClose(hFind);
}
...
umount:
  if (i_drive - 'A' < DOS_DRIVES && i_drive - 'A' >= 0 &&
    ...
    case 0:
      ...
      LOG_MSG("GUI:Drive %c has successfully been removed.", i_drive);
      break;
    case 1:
      LOG_MSG("GUI:Virtual Drives can not be unMOUNTed.");
      break;
    case 2:
      LOG_MSG(MSCDEX_Output(1).c_str());
      break;
    }
  }
}

void MountDrive_2(char drive, const char drive2[DOS_PATHLENGTH],
...
bool DOSBox_Kor(void) {
  if (menu.compatible)
    return false;
  char Buffer[30];
  GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SABBREVLANGNAME, Buffer,
                sizeof(Buffer));
...
void MENU_Check_Drive(HMENU handle, int cdrom, int floppy, int local, int image,
                      int automount, int umount, char drive) {
#if !defined(HX_DOS)
  std::string full_drive(1, drive);
  Section_prop *sec = static_cast<Section_prop *>(control->GetSection("dos"));
  full_drive += ":\\";
  EnableMenuItem(handle, cdrom,
                 (Drives[drive - 'A'] || menu.boot) ? MF_GRAYED : MF_ENABLED);
  EnableMenuItem(handle, floppy,
                 (Drives[drive - 'A'] || menu.boot) ? MF_GRAYED : MF_ENABLED);
  EnableMenuItem(handle, local,
                 (Drives[drive - 'A'] || menu.boot) ? MF_GRAYED : MF_ENABLED);
  EnableMenuItem(handle, image,
                 (Drives[drive - 'A'] || menu.boot) ? MF_GRAYED : MF_ENABLED);
  if (sec)
    EnableMenuItem(handle, automount,
                   AUTOMOUNT(full_drive.c_str(), drive) && !menu.boot &&
                           sec->Get_bool("automount")
                       ? MF_ENABLED
                       : MF_GRAYED);
  EnableMenuItem(handle, umount,
                 (!Drives[drive - 'A']) || menu.boot ? MF_GRAYED : MF_ENABLED);
#endif
}
...
void reflectmenu_INITMENU_cb() {
  /* WARNING: SDL calls this from Parent Window Thread!
              This executes in the context of the Parent Window Thread, NOT the
     main thread! As stupid as that seems, this is the only way the Parent
     Window Thread can make sure to keep Windows waiting while we take our time
     to reset the checkmarks in the menus before the menu is displayed. */
  Reflect_Menu();
}
#endif
...
void DOSBoxMenu::layoutMenu(void) {
  ...
  for (auto i = display_list.disp_list.begin();
       i != display_list.disp_list.end(); i++) {
  ...
  for (auto i = display_list.disp_list.begin();
       i != display_list.disp_list.end(); i++)
    get_item(*i).placeItemFinal(*this, /*finalwidth*/ x - menuBox.x,
                                /*toplevel*/ true);
...
void DOSBoxMenu::item::layoutSubmenu(DOSBoxMenu &menu, bool isTopLevel) {
  ...
  for (auto i = display_list.disp_list.begin();
    ...
    if (item.get_type() == DOSBoxMenu::vseparator_type_id) {
      for (; arr_follow < i; arr_follow++)
        menu.get_item(*arr_follow)
            .placeItemFinal(menu, /*finalwidth*/ maxx - minx,
                            /*toplevel*/ false);
  ...
  for (; arr_follow < display_list.disp_list.end(); arr_follow++)
    menu.get_item(*arr_follow)
        .placeItemFinal(menu, /*finalwidth*/ maxx - minx, /*toplevel*/ false);
...
void DOSBoxMenu::item::placeItemFinal(DOSBoxMenu &menu, int finalwidth,
                                      bool isTopLevel) {
  if (type < separator_type_id) {
    int x = 0, rx = 0;
    ...
    /* check */
    if (x > rx)
      LOG_MSG("placeItemFinal warning: text and shorttext overlap by %d pixels",
              x - rx);
...
void DOSBoxMenu::item::placeItem(DOSBoxMenu &menu, int x, int y,
                                 bool isTopLevel) {
  if (type < separator_type_id) {
    screenBox.x = x;
    screenBox.y = y;
    ...
    if (!isTopLevel && !shortcut_text.empty()) {
      ...
      shortBox.w += (unsigned int)menu.fontCharWidth *
                    (unsigned int)shortcut_text.length();
  ...
  if (OpenGL_using()) {
    ...
    if (sdl.surface->format->BitsPerPixel == 32) {
      ...
      while (h-- > 0) {
        ...
        for (unsigned int c = 0; c < (unsigned int)w; c++)
          row[c] = (row[c] >> 2) & mask;
      }
...
void MenuDrawRect(int x, int y, int w, int h, Bitu color) {
  if (OpenGL_using()) {
    ...
    if (color >= 0x1000000)
      color = ((color / 0x1000000) % 0x100) +
              ((color / 0x10000) % 0x100) * 0x100 +
              ((color / 0x100) % 0x100) * 0x10000;
#endif
    glColor3ub((color >> 16UL) & 0xFF, (color >> 8UL) & 0xFF,
               (color >> 0UL) & 0xFF);
    glBegin(GL_QUADS);
    glVertex2i(x, y);
    glVertex2i(x + w, y);
...
void MenuDrawTextChar(int &x, int y, unsigned char c, Bitu color, bool check) {
  ...
  if (x < 0 || y < 0 || (unsigned int)(x + 8) > (unsigned int)sdl.surface->w ||
  ...
  if (OpenGL_using()) {
#if C_OPENGL
    if ((IS_PC98_ARCH || IS_JEGA_ARCH || isDBCSCP()) && loadlang &&
        (c || !check)) {
      glBindTexture(GL_TEXTURE_2D,
                    prevc ? SDLDrawGenDBCSFontTexture : SDLDrawGenFontTexture);
      glPushMatrix();
      glMatrixMode(GL_TEXTURE);
      glLoadIdentity();
      glScaled(1.0 / SDLDrawGenFontTextureWidth,
               1.0 / SDLDrawGenFontTextureHeight, 1.0);
      glColor4ub((color >> 16UL) & 0xFF, (color >> 8UL) & 0xFF,
                 (color >> 0UL) & 0xFF, 0xFF);
    ...
    for (int i = 0; i < (prevc ? 2 : 1); i++) {
      ...
      glBegin(GL_QUADS);
      // lower left
      glTexCoord2i((int)tx + 0, (int)ty);
      glVertex2i((int)x, (int)y);
      // lower right
      glTexCoord2i((int)tx + 8, (int)ty);
      glVertex2i((int)x + 8, (int)y);
      // upper right
      glTexCoord2i((int)tx + 8, (int)ty + (int)fontHeight);
      glVertex2i((int)x + 8, (int)y + (int)fontHeight);
      // upper left
      glTexCoord2i((int)tx + 0, (int)ty + (int)fontHeight);
    ...
    for (int i = 0; i < (prevc ? 2 : 1); i++) {
      if (font_16_init && dos.loaded_codepage && dos.loaded_codepage != 437 &&
          !check && prevc != 1)
        bmp = (unsigned char *)int10_font_16_init +
              ((i || !prevc ? c : prevc) * fontHeight);
      else if (prevc != 1)
        bmp = (unsigned char *)int10_font_16 +
              ((i || !prevc ? c : prevc) * fontHeight);

      scan = (unsigned char *)sdl.surface->pixels;
      scan += (unsigned int)y * (unsigned int)sdl.surface->pitch;
      scan += (unsigned int)x *
              (((unsigned int)sdl.surface->format->BitsPerPixel + 7u) / 8u);
...
void MenuDrawTextChar2x(int &x, int y, unsigned char c, Bitu color,
  ...
  if (OpenGL_using()) {
    ...
    for (int i = 0; i < (prevc ? 2 : 1); i++) {
      ...
      // upper right
      glTexCoord2i((int)tx + 8, (int)ty + (int)fontHeight);
      glVertex2i(x + (8 * 2), y + ((int)fontHeight * 2));
      // upper left
      glTexCoord2i((int)tx + 0, (int)ty + (int)fontHeight);
      glVertex2i(x, y + ((int)fontHeight * 2));
      glEnd();
      x += (int)mainMenu.fontCharWidth;
    }
...
void MenuDrawText(int x, int y, const char *text, Bitu color,
  ...
  if (OpenGL_using()) {
    ...
    glBindTexture(GL_TEXTURE_2D,
                  use0 ? SDLDrawGenDBCSFontTexture : SDLDrawGenFontTexture);
...
void DOSBoxMenu::item::drawMenuItem(DOSBoxMenu &menu) {
  ...
  if (checkBox.w != 0 && checkBox.h != 0) {
    ...
    MenuDrawText(screenBox.x + checkBox.x, screenBox.y + checkBox.y, str,
                 fgcheckcolor, true);
  }
  if (textBox.w != 0 && textBox.h != 0)
    MenuDrawText(screenBox.x + textBox.x, screenBox.y + textBox.y, text.c_str(),
                 fgcolor);
  if (shortBox.w != 0 && shortBox.h != 0)
    MenuDrawText(screenBox.x + shortBox.x, screenBox.y + shortBox.y,
                 shortcut_text.c_str(), fgshortcolor);

  if (type == submenu_type_id && borderTop /*not toplevel*/)
    MenuDrawText((int)((int)screenBox.x + (int)screenBox.w -
                       (int)mainMenu.fontCharWidth - 1),
                 (int)((int)screenBox.y + (int)textBox.y), "\x10",
                 fgcheckcolor);

  if (type == separator_type_id)
    MenuDrawRect((int)screenBox.x, (int)screenBox.y + ((int)screenBox.h / 2),
                 (int)screenBox.w, 1, fgcolor);
  else if (type == vseparator_type_id)
    MenuDrawRect((int)screenBox.x + ((int)screenBox.w / 2), (int)screenBox.y, 1,
                 (int)screenBox.h, fgcolor);
...
void DOSBoxMenu::displaylist::DrawDisplayList(DOSBoxMenu &menu,
  ...
  for (auto &id : disp_list) {
    ...
    if (updateScreen)
      item.updateScreenFromItem(menu);
  }
}

DOSBoxMenu::item_handle_t
DOSBoxMenu::displaylist::itemFromPoint(DOSBoxMenu &menu, int x, int y) {
  for (auto &id : disp_list) {
    DOSBoxMenu::item &item = menu.get_item(id);
    if (x >= item.screenBox.x && y >= item.screenBox.y) {
      int sx = x - item.screenBox.x;
      int sy = y - item.screenBox.y;
      int adj = (this != &menu.display_list &&
                 item.get_type() == DOSBoxMenu::submenu_type_id)
                    ? 2
                    : 0;
...
void DOSBoxMenu::item::drawBackground(DOSBoxMenu &menu) {
  ...
  MenuDrawRect(popupBox.x, popupBox.y + popupBox.h - 1, popupBox.w, 1,
               bordercolor);

  MenuDrawRect(popupBox.x, popupBox.y, 1, popupBox.h, bordercolor);
  MenuDrawRect(popupBox.x + popupBox.w - 1, popupBox.y, 1, popupBox.h,
               bordercolor);

  if (type == DOSBoxMenu::submenu_type_id) {
    MenuShadeRect((int)popupBox.x + (int)popupBox.w,
                  (int)popupBox.y + (int)DOSBoxMenu::dropshadowY,
                  (int)DOSBoxMenu::dropshadowX, (int)popupBox.h);
    MenuShadeRect((int)popupBox.x + (int)DOSBoxMenu::dropshadowX,
                  (int)popupBox.y + (int)popupBox.h,
                  (int)popupBox.w - (int)DOSBoxMenu::dropshadowX,
                  (int)DOSBoxMenu::dropshadowY);
  }
}
#endif
