...
static const char advinterp2x_glsl[] =
    ...
    "	vec3 c1 = texture2D(rubyTexture, "
    "(base-vec2(0.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c7 = texture2D(rubyTexture, "
    "(base+vec2(0.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c3 = texture2D(rubyTexture, "
    "(base-vec2(1.0,0.0))/rubyTextureSize).xyz;\n"
    "	vec3 c5 = texture2D(rubyTexture, "
    "(base+vec2(1.0,0.0))/rubyTextureSize).xyz;\n"
    ""
    ...
    "	gl_FragColor = vec4(mix(mix(c0,c1,coord.x), mix(c2,c3,coord.x), "
    "coord.y), 1.0);\n"
    "#endif\n"
    "}\n"
    "#endif";
...
static const char advinterp3x_glsl[] =
    ...
    "  gl_Position = a_position;\n"
    "  v_texCoord = "
    "vec2(a_position.x+1.0,1.0-a_position.y)/2.0*rubyInputSize*3.0;\n"
    "}\n"
    ""
    "#elif defined(FRAGMENT)\n"
    ""
    ...
    "	vec3 c0 = texture2D(rubyTexture, "
    "(base-vec2(1.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c1 = texture2D(rubyTexture, "
    "(base-vec2(0.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c2 = texture2D(rubyTexture, "
    "(base-vec2(-1.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c3 = texture2D(rubyTexture, "
    "(base-vec2(1.0,0.0))/rubyTextureSize).xyz;\n"
    "	vec3 c4 = texture2D(rubyTexture, base/rubyTextureSize).xyz;\n"
    "	vec3 c5 = texture2D(rubyTexture, "
    "(base+vec2(1.0,0.0))/rubyTextureSize).xyz;\n"
    "	vec3 c6 = texture2D(rubyTexture, "
    "(base+vec2(-1.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c7 = texture2D(rubyTexture, "
    "(base+vec2(0.0,1.0))/rubyTextureSize).xyz;\n"
    "	vec3 c8 = texture2D(rubyTexture, "
    "(base+vec2(1.0,1.0))/rubyTextureSize).xyz;\n"
    ""
    "	bool outer = c1 != c7 && c3 != c5;\n"
    ""
...
static const char advmame3x_glsl[] =
    ...
    "	if (l==h) left.x = 0.0; // hack for center pixel, will ensure "
    "outer==false\n"
    "	if (m.y) {\n"
    "		// swap\n"
    "		left -= up;\n"
...
static const char rgb2x_glsl[] =
    ...
    "	vec2 mid = vec2(0.5);\n"
    "	vec4 s = texture2D(rubyTexture, "
    "(floor(coord/vec2(2.0))+mid)/rubyTextureSize);\n"
    ""
    "	coord = max(floor(mod(coord, 2.0)), 0.0);\n"
    "	/* 2x2 output:\n"
    "	 *    |x=0|x=1\n"
...
static const char rgb3x_glsl[] =
    ...
    "	vec2 mid = vec2(0.5);\n"
    "	vec4 s = texture2D(rubyTexture, "
    "(floor(coord/vec2(3.0))+mid)/rubyTextureSize);\n"
    ""
...
static const char scan2x_glsl[] =
    ...
    "  vec2 f = (center_dist - clamp(center_dist, -region_range, "
    "region_range)) * prescale + vec2(0.5);\n"
    ""
