...
static inline int conc2d(GetResult, SBPP)(PTYPE A, PTYPE B, PTYPE C, PTYPE D) {
  ...
  const int x = x1 + x2;
  const int y = y1 + y2;
  static const int rmap[3][3] = {{0, 0, -1}, {0, 0, -1}, {1, 1, 0}};
  return rmap[y][x];
}

inline void conc2d(Super2xSaI, SBPP)(PTYPE *line0, PTYPE *line1,
                                     const PTYPE *fc) {
...
inline void conc2d(SuperEagle, SBPP)(PTYPE *line0, PTYPE *line1,
                                     const PTYPE *fc) {
  // --------------------------------------
  if (C4 != C8) {
    if (C7 == C5) {
