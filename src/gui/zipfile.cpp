...
off_t ZIPFileEntry::seek_file(off_t pos) {
  if (file == NULL || file_offset == (off_t)0 || can_write)
    return (off_t)(-1LL);
  if (pos < (off_t)0)
    pos = (off_t)0;
  if (pos > file_length)
...
int ZIPFileEntry::read(void *buffer, size_t count) {
  if (file == NULL || file_offset == (off_t)0)
    return -1;
  ...
  if (mread > 0) {
    if (seek_file(position) != position)
      return -1;
...
ZIPFileEntry *ZIPFile::get_entry(const char *name) {
  ...
  if (!current_entry.empty() && current_entry != name)
    return NULL;
...
ZIPFileEntry *ZIPFile::new_entry(const char *name) {
  if (file_fd < 0 || !can_write || wrote_trailer)
    return NULL;
  ...
  if ((size_t)write(ent->name.c_str(), ent->name.length()) !=
      ent->name.length()) {
    close_current();
    return NULL;
  }
...
off_t ZIPFile::end_of_file(void) { return lseek(file_fd, 0, SEEK_END); }
...
void ZIPFile::close_current(void) {
  ...
  if (!current_entry.empty()) {
    ...
    if (ent != NULL && ent->can_write) {
      ...
      if (seek_file(ent->file_header_offset) == ent->file_header_offset &&
          read(&hdr, sizeof(hdr)) == sizeof(hdr)) {
        hdr.compressed_size = hdr.uncompressed_size =
            htole32(((uint32_t)ent->file_length));
        hdr.crc_32 = htole32(zipcrc_finalize(ent->write_crc));

        if (seek_file(ent->file_header_offset) == ent->file_header_offset &&
            write(&hdr, sizeof(hdr)) == sizeof(hdr)) {
          /* good */
        }
      }
    }
...
int ZIPFile::open(const char *path, int mode) {
  ...
  if ((mode & 3) == O_WRONLY) {
    LOG_MSG(
        "WARNING: ZIPFile attempt to open with O_WRONLY, which will not work");
    return -1;
  }
  ...
  /* WARNING: This assumes O_RDONLY, O_WRONLY, O_RDWR are defined as in Linux
   * (0, 1, 2) in the low two bits */
  if ((mode & 3) == O_RDWR)
    can_write = true;
  else
    can_write = false;
  ...
  if ((mode & 3) == O_RDONLY) {
     ...
     * This code only expects to handle the ZIP files it generated, not ZIP
     * files in general. */
    if (fsz < 64 || seek_file(0) != 0 || read(tmp, 4) != 4 ||
        memcmp(tmp, "PK\x03\x04", 4) != 0) {
     ...
     * this code DOES NOT SUPPORT the ZIP comment field, nor will this code
     * generate one. */
    if (seek_file(fsz - (off_t)sizeof(ehdr)) != (fsz - (off_t)sizeof(ehdr)) ||
        (size_t)read(&ehdr, sizeof(ehdr)) != sizeof(ehdr) ||
        ehdr.sig != PKZIP_CENTRAL_DIRECTORY_END_SIG ||
        ehdr.size_of_central_directory > 0x100000u /*absurd size*/ ||
        ehdr.offset_of_central_directory_from_start_disk == 0 ||
        (off_t)ehdr.offset_of_central_directory_from_start_disk >= fsz) {
      LOG_MSG("Cannot locate Central Directory");
      close();
      return -1;
    }
    if (seek_file((off_t)ehdr.offset_of_central_directory_from_start_disk) !=
        (off_t)ehdr.offset_of_central_directory_from_start_disk) {
    ...
    /* read the central directory */
    {
      ...
      while (remain >=
             (long)sizeof(struct pkzip_central_directory_header_main)) {
        if (read(&chdr, sizeof(chdr)) != sizeof(chdr))
          break;
        remain -= (long)sizeof(chdr);

        if (chdr.sig != PKZIP_CENTRAL_DIRECTORY_HEADER_SIG)
          break;
        if (chdr.filename_length >= sizeof(tmp))
          break;

        tmp[chdr.filename_length] = 0;
        if (chdr.filename_length != 0) {
          if (read(tmp, chdr.filename_length) != chdr.filename_length)
            break;
        ...
        ent->file_header_offset =
            (off_t)htole32(chdr.relative_offset_of_local_header);
        ent->file_offset = ent->file_header_offset +
                           (off_t)sizeof(struct ZIPLocalFileHeader) +
                           (off_t)htole16(chdr.filename_length) +
                           (off_t)htole16(chdr.extra_field_length);
        ent->position = 0;
        ent->name = (char *)tmp;
        ent->file = this;
      }
...
void ZIPFile::writeZIPFooter(void) {
  ...
  if (file_fd < 0 || wrote_trailer || !can_write)
    return;
  ...
  for (auto i = entries.begin(); i != entries.end(); i++) {
    ...
    chdr.general_purpose_bit_flag =
        htole16(0 << 1); /* just lie and say that "normal" deflate was used */
    ...
    chdr.relative_offset_of_local_header =
        (uint32_t)htole32(ent.file_header_offset);
    chdr.crc32 = htole32(zipcrc_finalize(ent.write_crc));

    if (write(&chdr, sizeof(chdr)) != sizeof(chdr))
      break;
...
zip_nv_pair_map::zip_nv_pair_map(ZIPFileEntry &ent) { read_nv_pairs(ent); }
...
void zip_nv_pair_map::process_line(
    char *line /*will modify, assume caller has put NUL at the end*/) {
