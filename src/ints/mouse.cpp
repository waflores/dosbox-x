...
// Unlock sequences taken from
// https://www.scs.stanford.edu/10wi-cs140/pintos/specs/kbd/scancodes-12.html
static const std::vector<uint32_t> UNLOCK_SEQ_INTELLIMOUSE = {
    RATE_200, RATE_100, RATE_80};
...
static uint16_t defaultCursorMaskNEC[CURSORY * 2] = {
    ...
    0xff80, 0xfb80, 0xf1c0, 0xe1c0, 0xc0e0, 0x80e0, 0x0070, 0x0070,
    0x0038, 0x0038, 0x001c, 0x001c, 0x000e, 0x000e, 0x0007, 0x0007};
...
static struct {
  ...
  int scrollwheel;
  uint8_t ps2_type;
  uint8_t ps2_rate; // sampling rate is not really emulated, but needed for
...
inline uint8_t GetWheel8bit() {
  /* CuteMouse wheel extension allows for -0x80,0x7F range, but let's keep it
   * symmetric */
  int8_t tmp = clamp(mouse.wheel, static_cast<int16_t>(-0x7F),
                     static_cast<int16_t>(0x7F));
  mouse.wheel = 0;
...
uint8_t Mouse_PS2GetType(void) { return mouse.ps2_type; }
...
/* set to true in case of shitty INT 15h device callbacks that fail to preserve
 * CPU registers */
...
unsigned int mouse_report_rate =
    200; /* DOSBox SVN compatible default (MOUSE_DELAY = 5.0 ms) */
pic_tickindex_t MOUSE_DELAY = 5.0; /* This was once a hard #define */

void UpdateMouseReportRate(void) {
  /* If the user did not specify an explicit rate, then whatever the current
   * rate goes and update MOUSE_DELAY. This is done so that PS/2 mouse emulation
   * can accept the commands to change reporting rate and have it work */
...
bool MouseInterruptEnabled(void) {
  if (!IS_PC98_ARCH && KEYBOARD_AUX_Active())
    return true;
  if (KEYBOARD_PS2REPORT_Active()) // FIXME: INT 15h needs to issue keyboard
                                   // command "enable AUX"
    return true;
  if (mouse.polled)
    return true;
...
INLINE void Mouse_AddEvent(uint8_t type) {
  if (mouse.events < QUEUE_SIZE) {
    if (mouse.events > 0) {
      ...
      if (type == MOUSE_HAS_MOVED || type == MOUSE_WHEEL_MOVED)
        return;
 ...
 */
extern void WriteChar(uint16_t col, uint16_t row, uint8_t page, uint16_t chr,
...
void RestoreCursorBackgroundText() {
  ...
  if (mouse.background) {
    WriteChar((uint16_t)mouse.backposx, (uint16_t)mouse.backposy,
              real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE), mouse.backData[0],
...
void DrawCursorText() {
  ...
  if (mouse.cursorType == 0 ||
      mouse.cursorType == 2 /*Microsoft Word 5.5 even in text mode*/) {
    uint16_t result;
    ReadCharAttr((uint16_t)mouse.backposx, (uint16_t)mouse.backposy, page,
                 &result);
    ...
    WriteChar((uint16_t)mouse.backposx, (uint16_t)mouse.backposy, page,
              (uint8_t)(result & 0xFF), (uint8_t)(result >> 8), true);
  } else {
...
void PC98_XorPixel(uint16_t x, uint16_t y, uint8_t plane) {
  ...
  real_writeb(pc98_graph_seg[plane], addr,
              real_readb(pc98_graph_seg[plane], addr) ^ (1 << (7 - (x % 8))));
}
  ...
  if (mouse.background) {
    ...
    if (pc98_nec_mouse) {
      for (y = y1; y <= y2; y++) {
        ...
        for (x = x1; x <= x2; x++) {
          if (cuMask & HIGHESTBIT)
            PC98_XorPixel((uint16_t)x, (uint16_t)y, pc98_nec_mouse_plane);
          cuMask <<= 1;
          dataPos++;
        }
        dataPos += addx2;
      ...
      for (y = y1; y <= y2; y++) {
        ...
        for (x = x1; x <= x2; x++) {
          if (IS_PC98_ARCH)
            PC98_PutPixel((uint16_t)x, (uint16_t)y, mouse.backData[dataPos++]);
          else
            INT10_PutPixel((uint16_t)x, (uint16_t)y, mouse.page,
...
void DrawCursor() {
  ...
  // >>> removed because BIOS page is not actual page in some cases, e.g. QQP
  // games if (real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAGE)!=mouse.page) return;

  // Check if cursor in update region
  /*  if ((POS_X >= mouse.updateRegion_x[0]) && (POS_X <=
     mouse.updateRegion_x[1]) && (POS_Y >= mouse.updateRegion_y[0]) && (POS_Y <=
     mouse.updateRegion_y[1])) { if (CurMode->type==M_TEXT16)
              RestoreCursorBackgroundText();
          else
              RestoreCursorBackground();
          mouse.shown--;
  ...
  int16_t xratio = 640;
  if (CurMode->swidth > 0)
    xratio /= (uint16_t)CurMode->swidth;
  if (xratio == 0)
    xratio = 1;
  ...
  if (!pc98_nec_mouse) {
    ...
    for (y = y1; y <= y2; y++) {
      ...
      for (x = x1; x <= x2; x++) {
        if (IS_PC98_ARCH)
          PC98_GetPixel((uint16_t)x, (uint16_t)y, &mouse.backData[dataPos++]);
        else
          INT10_GetPixel((uint16_t)x, (uint16_t)y, mouse.page,
                         &mouse.backData[dataPos++]);
      }
      dataPos += addx2;
    }
  ...
  for (y = y1; y <= y2; y++) {
    ...
    for (x = x1; x <= x2; x++) {
      if (pc98_nec_mouse) {
        ...
        // Set Pixel
        if (IS_PC98_ARCH)
          PC98_PutPixel((uint16_t)x, (uint16_t)y, pixel);
        else
...
#else
static inline bool GFX_IsFullscreen(void) { return false; }
#endif
#include "render.h"
...
void Mouse_CursorMoved(float xrel, float yrel, float x, float y, bool emulate) {
...
#if C_DEBUG
  /* if debugging the code don't let mouse movement over the emulator window
   * cause problems */
  if (IsDebuggerActive())
    return;
#endif
  ...
  if (user_cursor_locked) {
    /* either device reports relative motion ONLY, and therefore requires that
     * the user has captured the mouse */
    ...
    if (mouse.mickey_x >= 32768)
      mouse.mickey_x -= 65536;
    else if (mouse.mickey_x <= -32769)
      mouse.mickey_x += 65536;
    if (mouse.mickey_y >= 32768)
      mouse.mickey_y -= 65536;
    else if (mouse.mickey_y <= -32769)
      mouse.mickey_y += 65536;
  }
  ...
  if (emulate) {
    ...
    if (CurMode->type == M_TEXT) {
      ...
      mouse.y = y *
                (IS_EGAVGA_ARCH ? (real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) + 1)
                                : 25) *
                8;
      /* NTS: DeluxePaint II enhanced sets a large range (5112x3832) for VGA
       * mode 0x12 640x480 16-color */
    } else {
  ...
  if (user_cursor_locked) {
    ...
    mouse.ps2x += xrel;
    mouse.ps2y += yrel;
    if (mouse.ps2x >= 32768.0)
      mouse.ps2x -= 65536.0;
    else if (mouse.ps2x <= -32769.0)
      mouse.ps2x += 65536.0;
    if (mouse.ps2y >= 32768.0)
      mouse.ps2y -= 65536.0;
    else if (mouse.ps2y <= -32769.0)
      mouse.ps2y += 65536.0;
  }
...
uint8_t Mouse_GetButtonState(void) { return mouse.buttons; }
...
const char *Mouse_GetSelected(int x1, int y1, int x2, int y2, int w, int h,
  ...
  if (IS_PC98_ARCH) {
    ...
    r = (uint16_t)(IS_EGAVGA_ARCH ? real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS)
                                  : 24) +
        1;
  }
  ...
  if (ttfuse && isDBCSCP() && dbcs_sbcs &&
      !(c1 == 0 && c2 == (int)(ttf.cols - 1) && r1 == 0 &&
        r2 == (int)(ttf.lins - 1))) {
    ttf_cell *curAC = curAttrChar;
    for (int y = 0; y < (int)ttf.lins; y++) {
      if (y >= r1 && y <= r2) {
        for (unsigned int x = 0; x < ttf.cols; x++)
          if ((int)x >= c1 && (int)x <= c2 &&
              curAC[rtl ? ttf.cols - x - 1 : x].selected) {
            if ((int)x == c1 && c1 > 0 &&
                curAC[rtl ? ttf.cols - x - 1 : x].skipped &&
                !curAC[rtl ? ttf.cols - x - 2 : x - 1].selected &&
                curAC[rtl ? ttf.cols - x - 2 : x - 1].doublewide) {
              ReadCharAttr(rtl ? ttf.cols - x - 2 : x - 1, y, page, &result);
              text[len++] = result;
            }
            ReadCharAttr(rtl ? ttf.cols - x - 1 : x, y, page, &result);
            if (curAC[rtl ? ttf.cols - x - 1 : x].boxdraw ||
                (!x && curAC[rtl ? ttf.cols - x : x + 1].boxdraw))
              bdlist.push_back(len);
            text[len++] = result;
            if ((int)x == c2 && c2 < (int)(ttf.cols - 1) &&
                curAC[rtl ? ttf.cols - x - 1 : x].doublewide &&
                !curAC[rtl ? ttf.cols - x : x + 1].selected &&
                curAC[rtl ? ttf.cols - x : x + 1].skipped) {
              ReadCharAttr(rtl ? ttf.cols - x : x + 1, y, page, &result);
              text[len++] = result;
            }
          }
        while (len > 0 && text[len - 1] == 32)
          text[--len] = 0;
        if (y < r2)
          text[len++] = '\n';
      }
    ...
    for (int i = r1; i <= r2; i++) {
      ...
      if ((showdbcs && !ttfuse && !IS_DOSV && isDBCSCP()
          ...
          (IS_DOSV && DOSV_CheckCJKVideoMode()) ||
          (IS_J3100 && J3_IsJapanese())) {
        for (int k = 0; k < c1; k++) {
          if (lead1)
            lead1 = false;
          else if (isKanji1(real_readb(seg, (i * c + k) * 2)))
            lead1 = true;
        }
      }
      for (int j = c1 + (lead1 ? 1 : 0); j <= c2; j++) {
        if (IS_PC98_ARCH) {
          uint16_t address =
              ((i * 80) + (ttfuse && rtl ? ttfcols - j - 1 : j)) * 2;
          PhysPt where = CurMode->pstart + address;
          result = mem_readw(where);
          if (!result || (result == 0xFE && j && mem_readw(where - 2) == 0))
            result = 32;
          if ((result & 0xFF00u) != 0u && (result & 0x7Cu) == 0x08u &&
              (result % 0xff) - (result / 0x100) == 0xB) {
          ...
          } else if ((result & 0xFF00u) != 0u && (result & 0x7Cu) != 0x08u &&
                     result == mem_readw(where + (ttfuse && rtl ? -2 : 2)) &&
                     ++j < c) {
            result &= 0x7F7F;
            ...
            if (j1 > 32 && j1 < 127 && j2 > 32 && j2 < 127) {
              ...
              if (!ttfuse || !rtl)
                text[len++] = j2 + (j1 % 2 ? 31 + (j2 / 96) : 126);
              if (del_flag && (text[len - 1] & 0xFF) == 0x7F)
                text[len - 1]++;
            }
          } else if (j == c1 && c1 > 0) {
            uint16_t prevres = mem_readw(where - (ttfuse && rtl ? -2 : 2));
            if (!((prevres & 0xFF00u) != 0u && (prevres & 0xFCu) != 0x08u &&
                  prevres == result))
              text[len++] = result;
          } else if (result)
            text[len++] = result;
...
#endif
                                          && showdbcs)
                          ? std::find(jtbs.begin(), jtbs.end(),
                                      std::make_pair(i, j)) != jtbs.end()
                          : false;
          if (!isJEGAEnabled() || j > c1 || !find) {
            ...
            if (ttfuse && isDBCSCP()) {
              ...
              if (curAC[rtl ? ttfcols - j - 1 : j].boxdraw ||
                  (!j && curAC[rtl ? ttf.cols - j : j + 1].boxdraw))
                bdlist.push_back(len);
            } else
#endif
                if (isDBCSCP()
...
#endif
                    && showdbcs &&
                    std::find(dbox.begin(), dbox.end(), std::make_pair(i, j)) !=
                        dbox.end())
              bdlist.push_back(len);
            text[len++] = result;
            if (dos.loaded_codepage == 932 && find && del_flag &&
                (text[len - 1] & 0xFF) == 0x7F)
              text[len - 1]++;
          }
          if ((isJEGAEnabled() || (isDBCSCP()
                                   ...
                                   && showdbcs)) &&
              j == c2 && c2 < c - 1 &&
              std::find(jtbs.begin(), jtbs.end(), std::make_pair(i, j + 1)) !=
                  jtbs.end()) {
            ReadCharAttr(ttfuse && rtl ? (ttfcols - j) : (j + 1), i, page,
                         &result);
            text[len++] = result;
            if (dos.loaded_codepage == 932 && del_flag &&
                (text[len - 1] & 0xFF) == 0x7F)
              text[len - 1]++;
          }
        }
      ...
      if (ttfuse && rtl)
        std::reverse(text + startlen, text + len);
      while (len > 0 && text[len - 1] == 32) {
        text[--len] = 0;
        bdlist.remove(len);
      }
...
void Mouse_Select(int x1, int y1, int x2, int y2, int w, int h, bool select) {
  ...
  ttfcols = ttf.cols;
  if (ttfuse &&
      (!IS_EGAVGA_ARCH || CurMode->mode != 3 || (isDBCSCP() && dbcs_sbcs))) {
    ttf_cell *newAC = newAttrChar;
    for (unsigned int y = 0; y < ttf.lins; y++) {
      if ((int)y >= r1 && (int)y <= r2)
        for (int x = 0; x < (int)ttf.cols; x++)
          if ((x >= c1 ||
               ((IS_PC98_ARCH || (isDBCSCP() && dbcs_sbcs)) && c1 > 0 &&
                x == c1 - 1 &&
                (newAC[rtl ? ttf.cols - x - 1 : x].chr & 0xFF00) &&
                (newAC[rtl ? ttf.cols - x : x + 1].chr & 0xFF) == 32)) &&
              x <= c2)
    ...
    for (int i = r1; i <= r2; i++)
      for (int j = c1; j <= c2; j++) {
        if (IS_PC98_ARCH) {
          ...
          real_writeb(seg, (i * c + j) * 2 + 1,
                      attr / 0x10 + (attr & 0xF) * 0x10);
          if (j == c2)
            WriteCharTopView(c * i * 2, j + 1);
        } else if (CurMode->type != M_DCGA)
          real_writeb(
              0xb800, (i * c + (ttfuse && rtl ? ttfcols - j - 1 : j)) * 2 + 1,
              real_readb(0xb800,
                         (i * c + (ttfuse && rtl ? ttfcols - j - 1 : j)) * 2 +
                             1) ^
                  119);
      }
}
...
void Mouse_WheelMoved(int32_t scroll) {
  mouse.wheel =
      clamp(scroll + mouse.wheel, -0x7FFF,
            0x7FFF); /* limit is -0x8000,0x7FFF, but let's keep it symmetric */
...
static void Mouse_ResetHardware(void) {
  ...
  if (IS_PC98_ARCH) {
    ...
    // NEC MOUSE.COM behavior: Driver startup and INT 33h AX=0 automatically
    // show the graphics layer. Some games by "Orange House" depend on this
...
void Mouse_AfterNewVideoMode(bool setmode) {
  ...
  /* If new video mode is SVGA and this is NOT a mouse driver reset, then do not
   * reset min/max. This is needed for "down-by-the-laituri-peli" (some sort of
   * Finnish band tour simulation?) which sets the min/max range and THEN sets
   * up 640x480 256-color mode through the VESA BIOS. Without this fix, the
   * cursor is constrained to the upper left hand quadrant of the screen. */
  ...
  default:
    ...
    /* If new video mode is SVGA and this is NOT a mouse driver reset, then do
     * not reset min/max. This is needed for "down-by-the-laituri-peli" (some
     * sort of Finnish band tour simulation?) which sets the min/max range and
     * THEN sets up 640x480 256-color mode through the VESA BIOS. Without this
     * fix, the cursor is constrained to the upper left hand quadrant of the
     * screen. */
    if (!setmode) {
      if (CurMode != NULL) {
        mouse.first_range_setx = true;
...
static Bitu INT33_Handler(void) {
  //  LOG(LOG_MOUSE,LOG_NORMAL)("MOUSE: %04X %X %X %d
  //  %d",reg_ax,reg_bx,reg_cx,POS_X,POS_Y);
  switch (reg_ax) {
  ...
  case 0x07: /* MS MOUSE v1.0+ - DEFINE HORIZONTAL CURSOR RANGE */
    if (pc98_nec_mouse) {
      ...
      // Lemmings sets 1-640 and wants that. Ironseed sets 0-640 but doesn't
      // like 640 Ironseed works if newvideo mode with mode 13 sets 0-639 Larry
      // 6 actually wants newvideo mode with mode 13 to set it to 0-319
      int16_t max, min;
      ...
      LOG(LOG_MOUSE, LOG_NORMAL)("Define Horizontal range min:%d max:%d", min,
                                 max);

      /* NTS: The mouse in VESA BIOS modes would ideally start with the x and y
       * ranges that fit the screen, but I'm not so sure mouse drivers even pay
       * attention to VESA BIOS modes so it's not certain what comes out.
       * However some demoscene productions like "Aqua" will set their own mouse
       * range and draw their own cursor. The menu in "Aqua" will set up 640x480
       * 256-color mode and then set a mouse range of x=0-1279 and y=0-479.
       * Using the FIRST range set after mode set is the only way to make sure
       * mouse pointer integration tracks the guest pointer properly. */
      if (mouse.first_range_setx || mouse.buttons == 0) {
        if (mouse.min_x == 0 && mouse.max_x > 0 && assume_max_x == 0) {
          // most games redefine the range so they can use a saner range
          // matching the screen
          int16_t nval = mouse.max_x;

          if (CurMode->type == M_TEXT) {
            // Text is reported as if each row is 8 lines high (CGA compat) even
            // if EGA 14-line or VGA 16-line, and 8 pixels wide even if EGA/VGA
            // 9-pixels/char is enabled.
            //
            // Apply sanity rounding.
            //
            // FreeDOS EDIT: The max is set to just under 640x400, so that the
            // cursor only has
            //               room for ONE PIXEL in the last row and column.
            if (nval >= ((int16_t)(CurMode->twidth * 8) - 32) &&
                nval <= ((int16_t)(CurMode->twidth * 8) + 32))
              nval = (int16_t)CurMode->twidth * 8;
          } else {
            // Apply sanity rounding.
            //
            // Daggerfall: Sets max x to 310 instead of 320, probably to prevent
            // drawing the cursor
            //             partially offscreen. */
            if (nval >= ((int16_t)CurMode->swidth - 32) &&
                nval <= ((int16_t)CurMode->swidth + 32))
              nval = (int16_t)CurMode->swidth;
            else if (nval >= (((int16_t)CurMode->swidth - 32) * 2) &&
                     nval <= (((int16_t)CurMode->swidth + 32) * 2))
              nval = (int16_t)CurMode->swidth * 2;
          }

          if (mouse.max_screen_x != nval) {
            mouse.max_screen_x = nval;
            LOG(LOG_MOUSE, LOG_NORMAL)("Define Horizontal range min:%d max:%d "
  ...
  case 0x08: /* MS MOUSE v1.0+ - DEFINE VERTICAL CURSOR RANGE */
    if (pc98_nec_mouse) {
      ...
      // especially the cases where sheight= 400 and we set it with the
      // mouse_reset to 200 disabled it at the moment. Seems to break Syndicate
      // which wants 400 in mode 13
      int16_t max, min;
      if ((int16_t)reg_cx < (int16_t)reg_dx) {
        min = (int16_t)reg_cx;
        max = (int16_t)reg_dx;
      ...
      LOG(LOG_MOUSE, LOG_NORMAL)("Define Vertical range min:%d max:%d", min,
                                 max);
      ...
      if (mouse.first_range_sety || mouse.buttons == 0) {
        if (mouse.min_y == 0 && mouse.max_y > 0 && assume_max_y == 0) {
          ...
          if (CurMode->type == M_TEXT) {
            ...
            // cursor only has
            //               room for ONE PIXEL in the last row and column.
            if (nval >= ((int16_t)(CurMode->theight * 8) - 32) &&
                nval <= ((int16_t)(CurMode->theight * 8) + 32))
            ...
            // drawing the cursor
            //             partially offscreen. */
            if (nval >= ((int16_t)CurMode->sheight - 32) &&
                nval <= ((int16_t)CurMode->sheight + 32))
              nval = (int16_t)CurMode->sheight;
            else if (nval >= (((int16_t)CurMode->sheight - 32) * 2) &&
                     nval <= (((int16_t)CurMode->sheight + 32) * 2))
  ...
  case 0x11: /* Genius Mouse 9.06 - GET NUMBER OF BUTTONS */
    if (pc98_nec_mouse) {
      ...
      /* Previous implementation provided Genius mouse-specific function to get
         number of buttons (https://sourceforge.net/p/dosbox/patches/32/), it
         was returning 0xffff in reg_ax and number of buttons in reg_bx; I
  ...
  case 0x12: /* MS MOUSE - SET LARGE GRAPHICS CURSOR BLOCK */
    if (pc98_nec_mouse) {
      ...
      LOG(LOG_MOUSE,
          LOG_ERROR)("Set large graphics cursor block not implemented");
    }
    break;
  ...
  case 0x18: /* MS MOUSE v6.0+ - SET ALTERNATE MOUSE USER HANDLER */
    LOG(LOG_MOUSE,
        LOG_ERROR)("Set alternate mouse user handler not implemented");
    break;
  case 0x19: /* MS MOUSE v6.0+ - RETURN USER ALTERNATE INTERRUPT VECTOR */
    LOG(LOG_MOUSE,
        LOG_ERROR)("Return user alternate interrupt vector not implemented");
    break;
  case 0x1a: /* MS MOUSE v6.0+ - SET MOUSE SENSITIVITY */
    // ToDo : double mouse speed value
    Mouse_SetSensitivity(reg_bx, reg_cx, reg_dx);
    LOG(LOG_MOUSE, LOG_NORMAL)("Set sensitivity used with %d %d (%d)", reg_bx,
                               reg_cx, reg_dx);
    break;
  ...
  case 0x23: /* MS MOUSE v6.0+ - GET LANGUAGE FOR MESSAGES */
    ...
    break;
  case 0x24: /* MS MOUSE v6.26+ - GET SOFTWARE VERSION, MOUSE TYPE, AND IRQ
                NUMBER */
  ...
  case 0x25: /* MS MOUSE v6.26+ - GET GENERAL DRIVER INFORMATION */
    ...
    //         bits 4-5: cursor type  00=software text cursor 01=hardware text
    //         cursor 1X=graphics cursor bits 0-3: Function 28 mouse interrupt
    //         rate
    //       AL = Number of MDDS (?)
    //       BX = fCursor lock
    //       CX = FinMouse code
    ...
    reg_ah =
        0x40 /*integrated*/ | (mouse.cursorType << 4) | 0x01 /*30 reports/sec*/;
    reg_bx = 0;
    reg_cx = 0;
    reg_dx = 0;
    break;
  ...
  case 0x2a: /* MS MOUSE v7.02+ - GET CURSOR HOT SPOT */
    // NTS: Whoever compiled the INT 33 list in the Microsoft PC sourcebook made
    // a big mistake
    //      here and started counting hexadecimal as decimal. This function is
    //      wrongly listed as INT 33h AH=30H. You can tell by the summary list
    //      which shows both hexadecimal and decimal side by side, this is
    //      listed as AH=30h (42). Decimal 42 as hexadecimal would be 0x2A (0x20
    //      + 0x0A) == (32 + 10) == 42 == 0x2A.
    reg_al = (uint8_t)-mouse.hidden; // Microsoft uses a negative byte counter
  ...
  case 0x2f: /* MS MOUSE v7.02+ - MOUSE HARDWARE RESET */
    LOG(LOG_MOUSE,
        LOG_ERROR)("INT 33 AX=2F mouse hardware reset not implemented");
    break;
  ...
  case 0x32: /* MS MOUSE v7.05+ - GET ACTIVE ADVANCED FUNCTIONS */
    ...
    break;
  case 0x33: /* MS MOUSE v7.05+ - GET SWITCH SETTINGS AND ACCELERATION PROFILE
                DATA */
  ...
  case 0x35: /* MS MOUSE v8.10+ - LCD SCREEN LARGE POINTER SUPPORT */
    LOG(LOG_MOUSE,
        LOG_ERROR)("LCD screen large pointer support not implemented");
    break;
  case 0x4d: /* MS MOUSE - RETURN POINTER TO COPYRIGHT STRING */
    LOG(LOG_MOUSE,
        LOG_ERROR)("Return pointer to copyright string not implemented");
    break;
  ...
  case 0x53C1: /* Logitech CyberMan */
    LOG(LOG_MOUSE, LOG_NORMAL)("Mouse function 53C1 for Logitech CyberMan "
                               "called. Ignored by regular mouse driver.");
    break;
  default:
...
static Bitu PC98_INT15_Handler(void) {
  if (AllowINT33RMAccess() && en_int33)
    DrawCursor();
  return CBRET_NONE;
}
...
static Bitu INT74_Handler(void) {
  if (mouse.events > 0 && !mouse.in_UIR) {
    ...
    /* INT 33h emulation: HERE within the IRQ 12 handler is the appropriate
     * place to redraw the cursor. OSes like Windows 3.1 expect real-mode code
     * to do it in response to IRQ 12, not "out of the blue" from the SDL event
     * handler like the original DOSBox code did it. Doing this allows the INT
     * 33h emulation to draw the cursor while not causing Windows 3.1 to crash
     * or behave erratically. */
    if (AllowINT33RMAccess() && en_int33)
      DrawCursor();

    /* Check for an active Interrupt Handler that will get called */
    if (AllowINT33RMAccess() &&
        (mouse.sub_mask & mouse.event_queue[mouse.events].type)) {
      ...
      DoPS2Callback(mouse.event_queue[mouse.events].buttons,
                    static_cast<int16_t>(mouse.ps2x),
                    static_cast<int16_t>(mouse.ps2y));
    } else {
...
void BIOS_PS2Mouse_Startup(Section *sec) {
  if (IS_PC98_ARCH) {
    ...
    callback_pc98_mouse.Install(&PC98_INT15_Handler, CB_IRET_EOI_PIC2,
                                "PC-98 Mouse");
  ...
  if (!enable_slave_pic || machine == MCH_PCJR)
    return;
  ...
  if (MouseTypeNone()) {
    LOG(LOG_MOUSE, LOG_WARN)(
        "INT 15H PS/2 emulation NOT enabled. biosps2=1 but mouse type=none");
  } else {
    LOG(LOG_MOUSE, LOG_NORMAL)("INT 15H PS/2 emulation enabled");
    bios_enable_ps2();
  }

  ps2_callback_save_regs =
      section->Get_bool("int15 mouse callback does not preserve registers");
  ...
  CALLBACK_Setup(int74_ret_callback, &INT74_Ret_Handler, CB_IRQ12_RET,
                 "int 74 ret");
...
void MOUSE_Startup(Section *sec) {
  ...
  en_int33_hide_if_intsub =
      section->Get_bool("int33 hide host cursor if interrupt subroutine");

  en_int33_hide_if_polling =
      section->Get_bool("int33 hide host cursor when polling");

  en_int33_pc98_show_graphics =
      pc98_section->Get_bool("pc-98 show graphics layer on initialize");
  ...
  CALLBACK_Setup(call_int33, &INT33_Handler, CB_MOUSE, Real2Phys(i33loc),
                 "Mouse");
...
void MOUSE_Init() {
  ...
  // TODO: We need a DOSBox shutdown callback, and we need a shutdown callback
  // for when the DOS kernel begins to unload and on system reset
...
bool MOUSE_IsHidden() {
  ...
  return static_cast<bool>(mouse.hidden) &&
         (PIC_FullIndex() >= (mouse.hidden_at + 100));
}
...
// Basic VMware tools support, based on documentation from
// https://wiki.osdev.org/VMware_tools Mouse support tested using unofficial
// Windows 3.1 driver from https://github.com/NattyNarwhal/vmwmouse

static constexpr uint32_t VMWARE_MAGIC =
    0x564D5868u; // magic number for all VMware calls
...
volatile bool vmware_mouse =
    false; // if true, VMware compatible driver has taken over the mouse

static uint8_t mouse_buttons = 0; // state of mouse buttons, in VMware format
static uint16_t mouse_x =
    0x8000; // mouse position X, in VMware format (scaled from 0 to 0xFFFF)
static uint16_t mouse_y = 0x8000; // ditto
static int8_t mouse_wheel = 0;
static bool mouse_updated = false;

static int16_t mouse_diff_x =
    0; // difference between host and guest mouse x coordinate (in host pixels)
static int16_t mouse_diff_y = 0; // ditto

static bool video_fullscreen = false;
static uint16_t video_res_x =
    1; // resolution to which guest image is scaled, excluding black borders
static uint16_t video_res_y = 1;
static uint16_t video_clip_x =
    0; // clipping value - size of black border (one side)
...
void VMWARE_MousePosition(uint16_t pos_x, uint16_t pos_y) {
  ...
  mouse_x =
      (std::min)(0xFFFFu, static_cast<uint32_t>(
                              tmp_x * 0xFFFF / (video_res_x - 1) + 0.499));
  mouse_y =
      (std::min)(0xFFFFu, static_cast<uint32_t>(
                              tmp_y * 0xFFFF / (video_res_y - 1) + 0.499));
...
void VMWARE_ScreenParams(uint16_t clip_x, uint16_t clip_y, uint16_t res_x,
                         uint16_t res_y, bool fullscreen) {
  video_clip_x = clip_x;
  video_clip_y = clip_y;
  video_res_x = res_x;
  video_res_y = res_y;
  ...
  mouse_diff_x = clamp(static_cast<int32_t>(mouse_diff_x), -video_clip_x,
                       static_cast<int32_t>(video_clip_x));
  mouse_diff_y = clamp(static_cast<int32_t>(mouse_diff_y), -video_clip_y,
                       static_cast<int32_t>(video_clip_y));
}
...
private:
  void getBytes(std::ostream &stream) override {
    ...
    if (mouse.screenMask == defaultScreenMask)
      screenMask_idx = 0x00;
    else if (mouse.screenMask == userdefScreenMask)
      screenMask_idx = 0x01;

    if (mouse.cursorMask == defaultCursorMask)
      cursorMask_idx = 0x00;
    else if (mouse.cursorMask == userdefCursorMask)
      cursorMask_idx = 0x01;
  ...
  void setBytes(std::istream &stream) override {
    ...
    if (screenMask_idx == 0x00)
      mouse.screenMask = defaultScreenMask;
    else if (screenMask_idx == 0x01)
      mouse.screenMask = userdefScreenMask;

    if (cursorMask_idx == 0x00)
      mouse.cursorMask = defaultCursorMask;
    else if (cursorMask_idx == 0x01)
      mouse.cursorMask = userdefCursorMask;
