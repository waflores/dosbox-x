...
bool isDBCSCP(), isKanji1_gbk(uint8_t chr), shiftjis_lead_byte(int c),
    CheckDBCSCP(int32_t codepage);
...
#define STATIC_ASSERT(cond)                                                    \
  typedef char STATIC_ASSERTN(__LINE__, __COUNTER__)[(cond) ? 1 : -1]
...
bool DriveFileIterator(DOS_Drive *drv,
                       void (*func)(const char *path, bool is_dir,
                                    uint32_t size, uint16_t date, uint16_t time,
                                    uint8_t attr, Bitu data),
                       Bitu data, int timeout) {
  if (!drv)
    return true;
  uint32_t starttick = GetTicks();
  struct Iter {
    static bool ParseDir(DOS_Drive *drv, uint32_t startticks,
                         const std::string &dir, std::vector<std::string> &dirs,
                         void (*func)(const char *path, bool is_dir,
                                      uint32_t size, uint16_t date,
                                      uint16_t time, uint8_t attr, Bitu data),
                         Bitu data, uint32_t timeout) {
      size_t dirlen = dir.length();
      if (dirlen + DOS_NAMELENGTH >= DOS_PATHLENGTH)
        return true;
      ...
      for (bool more = drv->FindFirst((char *)dir.c_str(), dta); more;
           more = drv->FindNext(dta)) {
        if (startticks && timeout > 0 &&
            GetTicks() - startticks > timeout * 1000) {
          LOG_MSG("Timeout iterating directories");
          dos.dta(save_dta);
          return false;
        }
        char dta_name[DOS_NAMELENGTH_ASCII], lname[LFN_NAMELENGTH + 1];
        uint32_t dta_size, dta_hsize;
        uint16_t dta_date, dta_time;
        uint8_t dta_attr;
        dta.GetResult(dta_name, lname, dta_size, dta_hsize, dta_date, dta_time,
                      dta_attr);
        strcpy(full_path + dirlen, dta_name);
        bool is_dir = !!(dta_attr & DOS_ATTR_DIRECTORY);
        // if (is_dir) printf("[%s] [%s] %s (size: %u - date: %u - time: %u -
        // attr: %u)\n", (const char*)data, (dta_attr == 8 ? "V" : (is_dir ? "D"
        // : "F")), full_path, dta_size, dta_date, dta_time, dta_attr);
        if (dta_name[0] == '.' && dta_name[dta_name[1] == '.' ? 2 : 1] == '\0')
          continue;
        if (is_dir)
          dirs.emplace_back(full_path);
  ...
  while (dirs.size()) {
    ...
    dirs.pop_back();
    if (!Iter::ParseDir(drv, starttick, dir.c_str(), dirs, func, data, timeout))
      return false;
  }
  return true;
}
...
template <typename TVal> struct StringToPointerHashMap {
  ...
  ~StringToPointerHashMap() {
    free(keys);
    free(vals);
  }

  static uint32_t Hash(const char *str, uint32_t str_limit = 0xFFFF,
                       uint32_t hash_init = (uint32_t)0x811c9dc5) {
    for (const char *e = str + str_limit; *str && str != e;)
      hash_init = ((hash_init * (uint32_t)0x01000193) ^ (uint32_t)*(str++));
    return hash_init;
  }

  TVal *Get(const char *str, uint32_t str_limit = 0xFFFF,
            uint32_t hash_init = (uint32_t)0x811c9dc5) const {
    if (len == 0)
      return NULL;
    for (uint32_t key0 = Hash(str, str_limit, hash_init),
                  key = (key0 ? key0 : 1), i = key;
         ; i++) {
  ...
  void Put(const char *str, TVal *val, uint32_t str_limit = 0xFFFF,
           uint32_t hash_init = (uint32_t)0x811c9dc5) {
    if (len * 2 >= maxlen)
      Grow();
    for (uint32_t key0 = Hash(str, str_limit, hash_init),
  ...
  bool Remove(const char *str, uint32_t str_limit = 0xFFFF,
              uint32_t hash_init = (uint32_t)0x811c9dc5) {
    if (len == 0)
      return false;
    for (uint32_t key0 = Hash(str, str_limit, hash_init),
  ...
  TVal *GetAtIndex(uint32_t idx) const {
    return (keys[idx] ? vals[idx] : NULL);
  }

  struct Iterator {
    Iterator(StringToPointerHashMap<TVal> &_map, uint32_t _index)
        : map(_map), index(_index - 1) {
      this->operator++();
    }
    ...
    Iterator &operator++() {
      ...
      if (++index > map.maxlen)
        index = map.maxlen + 1;
      while (index <= map.maxlen && !map.keys[index])
        index++;
      return *this;
    }
  };
...
private:
  ...
  void Grow() {
    uint32_t oldMax = maxlen, oldCap = (maxlen ? oldMax + 1 : 0),
             *oldKeys = keys;
    TVal **oldVals = vals;
...
enum {
  DOS_ATTR_LONG_NAME = (DOS_ATTR_READ_ONLY | DOS_ATTR_HIDDEN | DOS_ATTR_SYSTEM |
                        DOS_ATTR_VOLUME),
  DOS_ATTR_LONG_NAME_MASK =
      (DOS_ATTR_READ_ONLY | DOS_ATTR_HIDDEN | DOS_ATTR_SYSTEM |
       DOS_ATTR_VOLUME | DOS_ATTR_DIRECTORY | DOS_ATTR_ARCHIVE),
...
struct fatFromDOSDrive {
  ...
  uint32_t sect_boot_pc98, sect_disk_end, sect_files_end, sect_files_start,
      sect_dirs_start, sect_root_start, sect_fat2_start, sect_fat1_start;

  struct ffddBuf {
    uint8_t data[BYTESPERSECTOR];
  };
  struct ffddSec {
    uint32_t cursor = NULL_CURSOR;
  };
  ...
  fatFromDOSDrive(DOS_Drive *drv, uint32_t freeMB, int timeout) : drive(drv) {
    ...
    memset(&cacheSectorNumber[1], 0,
           sizeof(cacheSectorNumber) - sizeof(cacheSectorNumber[0]));
    ...
    struct Iter {
      static void SetFAT(fatFromDOSDrive &ffdd, size_t idx, uint32_t val) {
        while (idx >= (uint64_t)ffdd.fat.size() * 8 / ffdd.fatSz) {
          // FAT12 table grows in steps of 3 sectors otherwise the table doesn't
          // align
          size_t addSz =
              (ffdd.fatSz != 12 ? BYTESPERSECTOR : (BYTESPERSECTOR * 3));
        ...
        if (ffdd.fatSz == 32) // FAT32
          ...
          var_write((uint16_t *)&ffdd.fat[idx + idx / 2],
                    (uint16_t)((var_read((uint16_t *)&ffdd.fat[idx + idx / 2]) &
                                0xF) |
                               ((val & 0xFFF) << 4)));
      ...
      static direntry *AddDirEntry(fatFromDOSDrive &ffdd, bool useFAT16Root,
                                   size_t &diridx) {
        const uint32_t entriesPerCluster =
            ffdd.sectorsPerCluster * BYTESPERSECTOR / sizeof(direntry);
        if (!useFAT16Root && (diridx % entriesPerCluster) == 0) {
          // link fat (was set to 0xFFFF before but now we knew the chain
          // continues)
          if (diridx)
            SetFAT(ffdd, 2 + (diridx - 1) / entriesPerCluster,
                   (uint32_t)(2 + ffdd.dirs.size() / entriesPerCluster));
          ...
          SetFAT(ffdd, 2 + diridx / entriesPerCluster,
                 (uint32_t)0xFFFFFFFF); // set as last cluster in chain for now
        } else if (useFAT16Root && diridx && (diridx % 512) == 0) {
          // this actually should never be larger than 512 for some FAT16
          // drivers
      ...
      static void ParseDir(fatFromDOSDrive &ffdd, char *dir,
                           const StringToPointerHashMap<void> *filter,
                           int dirlen = 0, uint16_t parentFirstCluster = 0) {
        if (ffdd.tomany)
          return;
        const bool useFAT16Root = (!dirlen && ffdd.fatSz != 32),
                   readOnly = ffdd.readOnly;
        const size_t firstidx = (!useFAT16Root ? ffdd.dirs.size() : 0);
        const uint32_t sectorsPerCluster = ffdd.sectorsPerCluster,
                       bytesPerCluster = sectorsPerCluster * BYTESPERSECTOR,
                       entriesPerCluster = bytesPerCluster / sizeof(direntry);
        const uint16_t myFirstCluster =
            (dirlen ? (uint16_t)(2 + firstidx / entriesPerCluster)
                    : (uint16_t)0);
        ...
        skipintprog = true;
        for (bool more = ffdd.drive->FindFirst(finddir, dta); more;
             more = ffdd.drive->FindNext(dta)) {
          char dta_name[DOS_NAMELENGTH_ASCII], lname[LFN_NAMELENGTH + 1];
          uint32_t dta_size, dta_hsize;
          uint16_t dta_date, dta_time;
          uint8_t dta_attr;
          dta.GetResult(dta_name, lname, dta_size, dta_hsize, dta_date,
                        dta_time, dta_attr);
          // LOG_MSG("dta_name %s lname %s\n", dta_name, lname);
          const char *fend = dta_name + strlen(dta_name);
          const bool dot = (dta_name[0] == '.' && dta_name[1] == '\0'),
                     dotdot = (dta_name[0] == '.' && dta_name[1] == '.' &&
                               dta_name[2] == '\0');
          if (!dirlen && (dot || dotdot))
            continue; // root shouldn't have dot entries (yet localDrive
                      // does...)

          ffddFile f;
          memcpy(f.path, dir, dirlen);
          memcpy(f.path + dirlen, dta_name, fend - dta_name + 1);
          if (filter && filter->Get(f.path, sizeof(f.path)))
            continue;

          const bool isLongFileName =
              (!dot && !dotdot && !(dta_attr & DOS_ATTR_VOLUME));
          if (isLongFileName) {
            bool lead = false;
            size_t len = 0, lfnlen = strlen(lname);
            uint16_t *lfnw =
                (uint16_t *)malloc((lfnlen + 1) * sizeof(uint16_t));
            if (lfnw == NULL)
              continue;
            char text[3];
            uint16_t uname[4];
              ...
              if (lead) {
                ...
                if (CodePageGuestToHostUTF16(uname, text) && uname[0] != 0 &&
                  ...
                  lfnw[len++] = lname[i];
                         ...
                         ((IS_PC98_ARCH &&
                           shiftjis_lead_byte(lname[i] & 0xFF)) ||
                          (isDBCSCP() && isKanji1_gbk(lname[i] & 0xFF))))
                lead = true;
              else if (dos.loaded_codepage != 437) {
                text[0] = lname[i] & 0xFF;
                text[1] = 0;
                lfnw[len++] = CodePageGuestToHostUTF16(uname, text) &&
                                      uname[0] != 0 && uname[1] == 0
                                  ? uname[0]
                                  : lname[i];
              } else
            ...
            for (size_t i = 0, lfnblocks = (len + 12) / 13; i != lfnblocks;
                 i++) {
              lfndirentry *le =
                  (lfndirentry *)AddDirEntry(ffdd, useFAT16Root, diridx);
          ...
          if (fext++)
            memcpy(e->entryname + 8, fext, fend - fext);

          e->attrib = dta_attr | (readOnly ? DOS_ATTR_READ_ONLY : 0) |
                      (isLongFileName ? DOS_ATTR_PENDING_SHORT_NAME : 0);
          if (dos.version.major >= 7) {
            var_write(&e->crtTime,
                      dta_time); // create date/time is DOS 7.0 and up only
            var_write(&e->crtDate,
                      dta_date); // create date/time is DOS 7.0 and up only
          }
          ...
          if (dot) {
            ...
            uint32_t numSects = (dta_size + bytesPerCluster - 1) /
                                bytesPerCluster * sectorsPerCluster;
            try {
              ffdd.fileAtSector.resize(ffdd.fileAtSector.size() + numSects,
                                       fileIdx);
            } catch (...) {
        ...
        // Now fill out the subdirectories (can't be done above because only one
        // dos.dta can run simultaneously
        std::vector<direntry> &entries =
            (!useFAT16Root ? ffdd.dirs : ffdd.root);
        for (size_t ei = firstidx; ei != diridx; ei++) {
          ...
          {
            if (entryname[0] == '.' &&
                entryname[entryname[1] == '.' ? 2 : 1] == ' ')
              continue;
            for (int i = 0; i != 8 && entryname[i] != ' '; i++)
              dir[totlen++] = entryname[i];
            if (entryname[8] != ' ')
              dir[totlen++] = '.';
            for (int i = 8; i != 11 && entryname[i] != ' '; i++)
              dir[totlen++] = entryname[i];
          }
          if (e.attrib & DOS_ATTR_PENDING_SHORT_NAME) // convert LFN to SFN
          {
            uint8_t chksum = 0;
            for (int i = 0; i != 11;)
              chksum = (chksum >> 1) + (chksum << 7) + e.entryname[i++];
            for (lfndirentry *le = (lfndirentry *)&e;
                 le-- == (lfndirentry *)&e || !(le[1].ord & 0x40);)
              le->chksum = chksum;
            e.attrib &= ~DOS_ATTR_PENDING_SHORT_NAME;
          }
          if (e.attrib &
              DOS_ATTR_DIRECTORY) // this reallocates ffdd.dirs so do this last
          {
            var_write(&e.loFirstClust,
                      (uint16_t)(2 + ffdd.dirs.size() / entriesPerCluster));
      ...
      static void SumFileSize(const char *path, bool /*is_dir*/, uint32_t size,
                              uint16_t, uint16_t, uint8_t, Bitu data) {
        if (!((SumInfo *)data)->filter || !((SumInfo *)data)->filter->Get(path))
          ((SumInfo *)data)->used_bytes +=
              (size + (32 * 1024 - 1)) / (32 * 1024) *
              (32 * 1024); // count as 32 kb clusters
      }
    };
    ...
    uint16_t drv_bytes_sector;
    uint8_t drv_sectors_cluster;
    uint16_t drv_total_clusters, drv_free_clusters;
    rsize = true;
    freec = 0;
    drv->AllocationInfo(&drv_bytes_sector, &drv_sectors_cluster,
                        &drv_total_clusters, &drv_free_clusters);
    free_clusters = freec ? freec : drv_free_clusters;
    freeSpace = (uint64_t)drv_bytes_sector * drv_sectors_cluster *
                (freec ? freec : free_clusters);
    freeSpaceMB = freeSpace / (1024 * 1024);
    if (freeMB < freeSpaceMB)
      freeSpaceMB = freeMB;
    rsize = false;
    tomany = false;
    readOnly = free_clusters == 0 || freeSpaceMB == 0;
    if (!DriveFileIterator(drv, Iter::SumFileSize, (Bitu)&sum, timeout))
      return;

    uint32_t usedMB = sum.used_bytes / (1024 * 1024), addFreeMB, totalMB,
             tsizeMB;
    uint64_t tsize = 0;
    if (IS_PC98_ARCH) {
      if (usedMB < 6) {
        ...
        sasi.sectors = 33;
        uint32_t heads = std::ceil(
            (double)(usedMB +
                     (readOnly ? 0 : (usedMB >= 2047 ? freeSpaceMB : 5))) /
            10);
        if (heads > 255) {
          sasi.surfaces = 255;
      ...
      tsizeMB = sasi.sectors * sasi.surfaces * sasi.cylinders /
                (1024 * 1024 / BYTESPERSECTOR);
      if (tsizeMB < usedMB)
        readOnly = true;
      addFreeMB = readOnly ? 0
                           : (usedMB >= 2047
                                  ? freeSpaceMB
                                  : (std::ceil((double)tsize - sum.used_bytes) /
                                         (1024 * 1024) +
                                     1));
    } else
    ...
    } // 32 kb clusters ( 98304 ~        FAT entries)
    else if (totalMB >= 2048) {
      fatSz = 32;
      sectorsPerCluster = 32;
    } // 16 kb clusters (131072 ~ 196608 FAT entries)
    else if (totalMB >= 384) {
      fatSz = 16;
      sectorsPerCluster = 64;
    } // 32 kb clusters ( 12288 ~  65504 FAT entries)
    else if (totalMB >= 192) {
      fatSz = 16;
      sectorsPerCluster = 32;
    } // 16 kb clusters ( 12288 ~  24576 FAT entries)
    else if (totalMB >= 96) {
      fatSz = 16;
      sectorsPerCluster = 16;
    } //  8 kb clusters ( 12288 ~  24576 FAT entries)
    else if (totalMB >= 48) {
      fatSz = 16;
      sectorsPerCluster = 8;
    } //  4 kb clusters ( 12288 ~  24576 FAT entries)
    else if (totalMB >= 12) {
      fatSz = 16;
      sectorsPerCluster = 4;
    } //  2 kb clusters (  6144 ~  24576 FAT entries)
    else if (totalMB >= 4) {
      fatSz = 16;
      sectorsPerCluster = 1;
    } // .5 kb clusters (  8192 ~  24576 FAT entries)
    else if (totalMB >= 2) {
      fatSz = 12;
      sectorsPerCluster = 4;
    } //  2 kb clusters (  1024 ~   2048 FAT entries)
    else if (totalMB >= 1) {
      fatSz = 12;
      sectorsPerCluster = 2;
    } //  1 kb clusters (  1024 ~   2048 FAT entries)
    else {
      fatSz = 12;
      sectorsPerCluster = 1;
    } // .5 kb clusters (       ~   2048 FAT entries)
    ...
    for (uint32_t fileSect = 0, rootOrDir = 0; rootOrDir != 2; rootOrDir++) {
      for (direntry &e : (rootOrDir ? dirs : root)) {
        if (!e.entrysize ||
            (e.attrib & DOS_ATTR_LONG_NAME_MASK) == DOS_ATTR_LONG_NAME)
          continue;
        ...
        uint32_t numClusters =
            (var_read(&e.entrysize) + bytesPerCluster - 1) / bytesPerCluster;
        for (uint32_t i = fileCluster, iEnd = i + numClusters - 1; i != iEnd;
             i++)
          Iter::SetFAT(*this, i, i + 1);
    ...
    // Add at least one page after the last file or FAT spec minimum to make
    // ScanDisk happy (even on read-only disks)
    const uint32_t FATPageClusters = BYTESPERSECTOR * 8 / fatSz,
                   FATMinCluster =
                       (fatSz == 32 ? 65525 : (fatSz == 16 ? 4085 : 0)) +
                       FATPageClusters;
    const uint32_t addFreeClusters =
        ((addFreeMB * (1024 * 1024 / BYTESPERSECTOR)) + sectorsPerCluster - 1) /
        sectorsPerCluster;
    const uint32_t targetClusters =
        fileCluster +
        (addFreeClusters < FATPageClusters ? FATPageClusters : addFreeClusters);
    Iter::SetFAT(
        *this,
        (targetClusters < FATMinCluster ? FATMinCluster : targetClusters) - 1,
        0);
    const uint32_t totalClusters =
        (uint32_t)((uint64_t)fat.size() * 8 / fatSz); // as set by Iter::SetFAT

    // on read-only disks, fill up the end of the FAT table with "Bad sector in
    // cluster or reserved cluster" markers
    if (readOnly)
    ...
    const uint32_t partSize =
        totalClusters * sectorsPerCluster + reservedSectors;

    sect_boot_pc98 = sasi.surfaces * sasi.sectors;
    sect_fat1_start =
        (IS_PC98_ARCH ? sect_boot_pc98 : SECT_BOOT) + reservedSectors;
    sect_fat2_start = sect_fat1_start + sectorsPerFat;
    sect_root_start = sect_fat2_start + sectorsPerFat;
    sect_dirs_start = sect_root_start +
                      ((root.size() * sizeof(direntry) + BYTESPERSECTOR - 1) /
                       BYTESPERSECTOR);
    sect_files_start = sect_dirs_start +
                       ((dirs.size() * sizeof(direntry) + BYTESPERSECTOR - 1) /
                        BYTESPERSECTOR);
    sect_files_end = sect_files_start + fileAtSector.size();
    sect_disk_end = (IS_PC98_ARCH ? sect_boot_pc98 : SECT_BOOT) + partSize;
    if (IS_PC98_ARCH && tsize / BYTESPERSECTOR - 8 > sect_disk_end)
      sect_disk_end = tsize / BYTESPERSECTOR - 8;
    if (sect_disk_end < sect_files_end)
      return;
    ...
    if (!serial) {
      ...
      if (root.size())
        serial = DriveCalculateCRC32((uint8_t *)&root[0],
                                     root.size() * sizeof(direntry), serial);
      if (dirs.size())
        serial = DriveCalculateCRC32((uint8_t *)&dirs[0],
                                     dirs.size() * sizeof(direntry), serial);
    }

    memset(&mbr, 0, sizeof(mbr));
    // memcpy(&mbr,freedos_mbr,512);
    var_write((uint32_t *)&mbr.booter[440], serial); // 4 byte disk serial
                                                     // number
    var_write(&mbr.pentry[0].bootflag, 0x80); // Active bootable
    if ((sect_disk_end - 1) / (HEADCOUNT * SECTORSPERTRACK) > 0x3FF) {
      mbr.pentry[0].beginchs[0] = mbr.pentry[0].beginchs[1] =
          mbr.pentry[0].beginchs[2] = 0;
      mbr.pentry[0].endchs[0] = mbr.pentry[0].endchs[1] =
          mbr.pentry[0].endchs[2] = 0;
    } else {
      chs_write(mbr.pentry[0].beginchs,
                IS_PC98_ARCH ? sect_boot_pc98 : SECT_BOOT);
      chs_write(mbr.pentry[0].endchs, sect_disk_end - 1);
    }
    var_write(&mbr.pentry[0].absSectStart,
              IS_PC98_ARCH ? sect_boot_pc98 : SECT_BOOT);
    ...
    memcpy(bootsec.oemname, fatSz == 32 ? "MSWIN4.1" : "MSDOS5.0",
           sizeof(bootsec.oemname));
    ...
    var_write(&bootsec.totalsectorcount,
              0); // 16 bit field is 0, actual value is in totalsecdword
    var_write(&bootsec.mediadescriptor, 0xF8); // also in FAT[0]
    var_write(&bootsec.sectorspertrack,
              IS_PC98_ARCH ? sasi.sectors : SECTORSPERTRACK);
    var_write(&bootsec.headcount, IS_PC98_ARCH ? sasi.surfaces : HEADCOUNT);
    var_write(&bootsec.hiddensectorcount,
              IS_PC98_ARCH ? sect_boot_pc98 : SECT_BOOT);
    ...
    {
      var_write(&mbr.pentry[0].parttype,
                (fatSz == 12
                     ? 0x01
                     : (sect_disk_end < 65536 ? 0x04 : 0x06))); // FAT12/16
      ...
      var_write((uint32_t *)&bootsec.bootcode[3],
                serial + 1); // 4 byte partition serial number
    ...
    } else                                               // FAT32
    {
      ...
      var_write((uint32_t *)&bootsec.bootcode[8],
                (uint32_t)2); // First cluster number of the root directory
      var_write((uint16_t *)&bootsec.bootcode[12],
                (uint16_t)1); // Sector of FSInfo structure in offset from top
                              // of the FAT32 volume
      var_write((uint16_t *)&bootsec.bootcode[14],
                (uint16_t)6); // Sector of backup boot sector in offset from top
                              // of the FAT32 volume
      bootsec.bootcode[28] = 0x80; // Physical drive (harddisk) flag
      bootsec.bootcode[30] = 0x29; // Extended boot signature
      var_write((uint32_t *)&bootsec.bootcode[31],
                serial + 1); // 4 byte partition serial number
      ...
      var_write((uint32_t *)&fsinfosec[0],
                (uint32_t)0x41615252); // lead signature
      var_write((uint32_t *)&fsinfosec[484],
                (uint32_t)0x61417272); // Another signature
      bool ver71 = dos.version.major > 7 ||
                   (dos.version.major == 7 && dos.version.minor >= 10);
      // Bitu freeclusters = readOnly ? 0x0 : (ver71 ? (Bitu)freeSpace /
      // (BYTESPERSECTOR * sectorsPerCluster) : 0xFFFFFFFF);
      Bitu freeclusters =
          readOnly ? 0x0
                   : (ver71 ? (Bitu)((sect_disk_end - sect_files_end) /
                                     sectorsPerCluster)
                            : 0xFFFFFFFF);
      var_write((uint32_t *)&fsinfosec[488],
                (uint32_t)(freeclusters < 0xFFFFFFFF
                               ? freeclusters
                               : 0xFFFFFFFF)); // last known free cluster count
                                               // (all FF is unknown)
      var_write((uint32_t *)&fsinfosec[492],
                (ver71 ? (sect_files_end / sectorsPerCluster)
                       : 0xFFFFFFFF)); // the cluster number at which the driver
  ...
  uint8_t WriteSector(uint32_t sectnum, const void *data) {
    if (sectnum >= sect_disk_end)
      return 1;
    if (sectnum == SECT_MBR) {
      // Windows 9x writes the disk timestamp into the booter area on startup.
      // Just copy that part over so it doesn't get treated as a difference that
      // needs to be stored.
      memcpy(mbr.booter, data, sizeof(mbr.booter));
    }

    if (readOnly)
      return 0; // just return without error to avoid bluescreens in Windows 9x

    if (sectnum >= diffSectors.size())
      diffSectors.resize(sectnum + 128);
    uint32_t *cursor_ptr = &diffSectors[sectnum].cursor,
             cursor_val = *cursor_ptr;
    ...
    if (!unmodified) {
      ...
      for (uint64_t *p = (uint64_t *)data,
                    *pEnd = p + (BYTESPERSECTOR / sizeof(uint64_t));
           p != pEnd; p++)
        if (*p) {
          is_different = true;
          break;
      ...
      is_different = memcmp(unmodified, data, BYTESPERSECTOR);
    ...
    if (is_different) {
      ...
      cacheSectorNumber[sectnum % CACHECOUNT] =
          (uint32_t)-1; // invalidate cache
  ...
  void *GetUnmodifiedSector(uint32_t sectnum, void *filebuf) {
    if (sectnum >= sect_files_end) {
      ...
      if (df) {
        ...
        if (!df->Read((uint8_t *)filebuf, &read)) {
          read = 0;
          assert(0);
        }
    ...
    } else if (sectnum >= sect_dirs_start)
      return &dirs[(sectnum - sect_dirs_start) *
                   (BYTESPERSECTOR / sizeof(direntry))];
    else if (sectnum >= sect_root_start)
      return &root[(sectnum - sect_root_start) *
                   (BYTESPERSECTOR / sizeof(direntry))];
    else if (sectnum >= sect_fat2_start)
      return &fat[(sectnum - sect_fat2_start) * BYTESPERSECTOR];
    else if (sectnum >= sect_fat1_start)
      return &fat[(sectnum - sect_fat1_start) * BYTESPERSECTOR];
    else if (IS_PC98_ARCH) {
      if (sectnum == 0)
        return &ipl;
      else if (sectnum == 1)
        return &pt;
      else if (sectnum == sect_boot_pc98)
        return &bootsec;
      else if (sectnum == sect_boot_pc98 + 1)
        return fsinfosec;
      else if (sectnum == sect_boot_pc98 + 2)
        return fsinfosec; // additional boot loader code (anything is ok for us
                          // but needs 0x55AA footer signature)
      else if (sectnum == sect_boot_pc98 + 6)
        return &bootsec; // boot sector copy
      else if (sectnum == sect_boot_pc98 + 7)
        return fsinfosec; // boot sector copy
      else if (sectnum == sect_boot_pc98 + 8)
        return fsinfosec; // boot sector copy
      return NULL;
    } else if (sectnum == SECT_BOOT)
      return &bootsec;
    else if (sectnum == SECT_MBR)
      return &mbr;
    else if (sectnum == SECT_BOOT + 1)
      return fsinfosec;
    else if (sectnum == SECT_BOOT + 2)
      return fsinfosec; // additional boot loader code (anything is ok for us
                        // but needs 0x55AA footer signature)
    else if (sectnum == SECT_BOOT + 6)
      return &bootsec; // boot sector copy
    else if (sectnum == SECT_BOOT + 7)
      return fsinfosec; // boot sector copy
    else if (sectnum == SECT_BOOT + 8)
      return fsinfosec; // boot sector copy
    return NULL;
  }
  ...
  uint8_t ReadSector(uint32_t sectnum, void *data) {
    ...
    void *src;
    uint32_t cursor =
        (sectnum >= diffSectors.size() ? NULL_CURSOR
                                       : diffSectors[sectnum].cursor);
    ...
    if (src)
      memcpy(data, src, BYTESPERSECTOR);
    else
...
diskGeo DiskGeometryList[] = {
    {160, 8, 1, 40, 0, 512, 64, 1,
     0xFE}, // IBM PC double density 5.25" single-sided 160KB
    {180, 9, 1, 40, 0, 512, 64, 1,
     0xFC}, // IBM PC double density 5.25" single-sided 180KB
    {200, 10, 1, 40, 0, 512, 64, 2,
     0xFC}, // DEC Rainbow double density 5.25" single-sided 200KB (I think...)
    {320, 8, 2, 40, 1, 512, 112, 2,
     0xFF}, // IBM PC double density 5.25" double-sided 320KB
    {360, 9, 2, 40, 1, 512, 112, 2,
     0xFD}, // IBM PC double density 5.25" double-sided 360KB
    {400, 10, 2, 40, 1, 512, 112, 2,
     0xFD}, // DEC Rainbow double density 5.25" double-sided 400KB (I think...)
    {640, 8, 2, 80, 3, 512, 112, 2,
     0xFB}, // IBM PC double density 3.5" double-sided 640KB
    {720, 9, 2, 80, 3, 512, 112, 2,
     0xF9}, // IBM PC double density 3.5" double-sided 720KB
    {1200, 15, 2, 80, 2, 512, 224, 1,
     0xF9}, // IBM PC double density 5.25" double-sided 1.2MB
    {1440, 18, 2, 80, 4, 512, 224, 1,
     0xF0}, // IBM PC high density 3.5" double-sided 1.44MB
    {1680, 21, 2, 80, 4, 512, 16, 4,
     0xF0}, // IBM PC high density 3.5" double-sided 1.68MB (DMF)
    {2880, 36, 2, 80, 6, 512, 240, 2,
     0xF0}, // IBM PC high density 3.5" double-sided 2.88MB

    {1232, 8, 2, 77, 7, 1024, 192, 1,
     0xFE}, // NEC PC-98 high density 3.5" double-sided 1.2MB "3-mode"
    {1520, 19, 2, 80, 2, 512, 224, 1,
     0xF9}, // IBM PC high density 5.25" double-sided 1.52MB (XDF)
    {1840, 23, 2, 80, 4, 512, 224, 1,
     0xF0}, // IBM PC high density 3.5" double-sided 1.84MB (XDF)
...
void updateDPT(void) {
  ...
  PhysPt dpphysaddr[2] = {CALLBACK_PhysPointer(diskparm0),
                          CALLBACK_PhysPointer(diskparm1)};
  for (int i = 0; i < 2; i++) {
    tmpheads = 0;
    tmpcyl = 0;
    tmpsect = 0;
    ...
    if (imageDiskList[i + 2] != NULL) {
      imageDiskList[i + 2]->Get_Geometry(&tmpheads, &tmpcyl, &tmpsect,
                                         &tmpsize);
    }
    ...
    phys_writeb(dpphysaddr[i] + 0x8,
                tmpcyl == 0 ? 0 : (0xc0 | ((tmpheads > 8) << 3)));
...
void swapInDisks(int drive) {
  ...
  if (swapInDisksSpecificDrive >= 0 && swapInDisksSpecificDrive <= 1 &&
      (drive == -1 || drive == swapInDisksSpecificDrive)) {
    diskswapdrive = swapInDisksSpecificDrive;
    diskswapcount = 1;
  } else if (swapInDisksSpecificDrive != -1 ||
             drive != -1) /* Swap A: and B: drives */
    return;

  /* If only one disk is loaded, this loop will load the same disk in dive A and
   * drive B */
  while (diskcount < diskswapcount) {
    if (diskSwap[swapPos] != NULL) {
      LOG_MSG("Loaded drive %d disk %d from swaplist position %d - \"%s\"",
              (int)diskswapdrive, (int)diskcount, (int)swapPos,
              diskSwap[swapPos]->diskname.c_str());
...
#if 0 /* FIX_ME: Disabled to swap CD by IMGSWAP command (Issue #4932). Revert  \
         this if any regression occurs */
...
void swapInNextDisk(bool pressed) {
  ...
  for (Bitu i = 0; i < 2; i++) { /* Swap A: and B: where DOSBox mainline would
                                    run through ALL drive letters */
    if (Drives[i] != NULL) {
      Drives[i]->EmptyCache();
...
void swapInNextCD(bool pressed) {
  ...
  for (Bitu i = 2; i < DOS_DRIVES;
       i++) { /* Swap C: D: .... Z: if it is a CD/DVD drive */
...
uint8_t imageDisk::Read_AbsoluteSector(uint32_t sectnum, void *data) {
  if (ffdd)
    return ffdd->ReadSector(sectnum, data);
  ...
  if ((bytenum + sector_size) > this->image_length) {
    LOG_MSG("Attempt to read invalid sector in Read_AbsoluteSector for sector "
            "%lu.\n",
  ...
  if (res != bytenum) {
    LOG_MSG("fseek() failed in Read_AbsoluteSector for sector %lu. Want=%llu "
            "Got=%llu\n",
            (unsigned long)sectnum, (unsigned long long)bytenum,
  ...
  if ((unsigned int)got != sector_size) {
    LOG_MSG("fread() failed in Read_AbsoluteSector for sector %lu. Want=%u "
            "got=%d\n",
...
uint8_t imageDisk::Write_AbsoluteSector(uint32_t sectnum, const void *data) {
  if (ffdd)
    return ffdd->WriteSector(sectnum, data);

  uint64_t bytenum;

  bytenum = (uint64_t)sectnum * sector_size;
  if ((bytenum + sector_size) > this->image_length) {
    LOG_MSG("Attempt to read invalid sector in Write_AbsoluteSector for sector "
            "%lu.\n",
            (unsigned long)sectnum);
    return 0x05;
  }
...
uint32_t imageDisk::Get_Reserved_Cylinders() { return reserved_cylinders; }

imageDisk::imageDisk(IMAGE_TYPE class_id) : class_id(class_id) {}

imageDisk::imageDisk(FILE *diskimg, const char *diskName, uint32_t cylinders,
                     uint32_t heads, uint32_t sectors, uint32_t sector_size,
                     bool hardDrive) {
  if (diskName)
...
void imageDisk::UpdateFloppyType(void) {
  ...
  while (DiskGeometryList[i].ksize != 0x0) {
    if ((DiskGeometryList[i].ksize == diskSizeK) ||
        (DiskGeometryList[i].ksize + 1 == diskSizeK)) {
      floppytype = i;
      LOG_MSG("Updating floppy type to %u BIOS type 0x%02x", floppytype,
              GetBiosType());
      break;
    }
    i++;
...
imageDisk::imageDisk(FILE *imgFile, const char *imgName, uint32_t imgSizeK,
                     bool isHardDisk)
    : diskSizeK(imgSizeK), diskimg(imgFile),
      ...
      if (ext != NULL) {
        if (!strcasecmp(ext, ".fdi")) {
          if (imgSizeK >= 160) {
            ...
            // PC-98 .FDI images appear to be 4096 bytes of a short header and
            // many zeros. followed by a straight sector dump of the disk. The
            // header is NOT NECESSARILY 4KB in size, but usually is.
            LOG_MSG("Image file has .FDI extension, assuming FDI image and "
                    "will take on parameters in header.");
            ...
            if (fseek(imgFile, 0, SEEK_SET) == 0 && ftell(imgFile) == 0 &&
              ...
              uint32_t fddsize =
                  host_readd(fdihdr.fddsize); /* includes header */
              uint32_t sectorsize = host_readd(fdihdr.sectorsize);

              if (sectorsize != 0 &&
                  ((sectorsize & (sectorsize - 1)) == 0 /*is power of 2*/) &&
                  sectorsize >= 256 && sectorsize <= 1024 && ofs != 0 &&
                  (ofs % sectorsize) ==
                      0 /*offset is nonzero and multiple of sector size*/
                  &&
                  (ofs % 1024) ==
                      0 /*offset is a multiple of 1024 because of imgSizeK*/
                  && fddsize >= sectorsize &&
                  (fddsize / 1024) <= (imgSizeK + 4)) {
                ...
                image_base = ofs;
                image_length -= ofs;
                LOG_MSG("FDI header: sectorsize is %u bytes/sector, header is "
                        "%u bytes, fdd size (plus header) is %u bytes",
                ...
                LOG_MSG("FDI: Geometry is C/H/S %u/%u/%u", cylinders, heads,
                        sectors);
              } else {
                LOG_MSG("FDI header rejected. sectorsize=%u headersize=%u "
                        "fddsize=%u",
                        sectorsize, ofs, fddsize);
              }
            } else {
    ...
    if (sectors == 0 && heads == 0 && cylinders == 0) {
      ...
      while (DiskGeometryList[i].ksize != 0x0) {
        if ((DiskGeometryList[i].ksize == imgSizeK) ||
          ...
          if (DiskGeometryList[i].ksize != imgSizeK)
            LOG_MSG("ImageLoader: image file with additional data, might not "
                    "load!");
          founddisk = true;
          active = true;
          floppytype = i;
          ...
          LOG_MSG("Identified '%s' as C/H/S %u/%u/%u %u bytes/sector", imgName,
                  cylinders, heads, sectors, sector_size);
          break;
        }
        // Supports cases where the size of a 1.2 Mbytes disk image file is 1.44
        // Mbytes.
        if (DiskGeometryList[i].ksize == 1200 &&
            (imgSizeK > 1200 && imgSizeK <= 1440)) {
          char buff[0x20];
          if (fseek(imgFile, 0, SEEK_SET) == 0 && ftell(imgFile) == 0 &&
              fread(buff, sizeof(buff), 1, imgFile) == 1) {
      ...
      if (ext != NULL) {
        if (!strcasecmp(ext, ".nhd")) {
          if (imgSizeK >= 160) {
            ...
            LOG_MSG("Image file has .NHD extension, assuming NHD image and "
                    "will take on parameters in header.");
            ...
            if (fseek(imgFile, 0, SEEK_SET) == 0 && ftell(imgFile) == 0 &&
              ...
              if (sectorsize != 0 &&
                ...
                image_base = ofs;
                image_length -= ofs;
                LOG_MSG("NHD header: sectorsize is %u bytes/sector, header is "
                        "%u bytes",
                        sectorsize, ofs);

                /* take on the geometry.
                 * PC-98 IPL1 support will need it to make sense of the
                 * partition table. */
                ...
                LOG_MSG("NHD: Geometry is C/H/S %u/%u/%u", cylinders, heads,
                        sectors);
              } else {
        ...
        if (!strcasecmp(ext, ".hdi")) {
          if (imgSizeK >= 160) {
            ...
            // PC-98 .HDI images appear to be 4096 bytes of a short header and
            // many zeros. followed by a straight sector dump of the disk. The
            // header is NOT NECESSARILY 4KB in size, but usually is.
            LOG_MSG("Image file has .HDI extension, assuming HDI image and "
                    "will take on parameters in header.");
            ...
            if (fseek(imgFile, 0, SEEK_SET) == 0 && ftell(imgFile) == 0 &&
              ...
              uint32_t hddsize =
                  host_readd(hdihdr.hddsize); /* includes header */
              ...
              if (sectorsize != 0 &&
                  ...
                  (ofs % 1024) ==
                      0 /*offset is a multiple of 1024 because of imgSizeK*/
                  && hddsize >= sectorsize &&
                  (hddsize / 1024) <= (imgSizeK + 4)) {

                sector_size = sectorsize;
                image_base = ofs;
                image_length -= ofs;
                LOG_MSG("HDI header: sectorsize is %u bytes/sector, header is "
                        "%u bytes, hdd size (plus header) is %u bytes",
                ...
                LOG_MSG("HDI: Geometry is C/H/S %u/%u/%u", cylinders, heads,
                        sectors);
              } else {
                LOG_MSG("HDI header rejected. sectorsize=%u headersize=%u "
                        "hddsize=%u",
                        sectorsize, ofs, hddsize);
              }
            } else {
...
imageDisk::imageDisk(class DOS_Drive *useDrive, unsigned int letter,
                     uint32_t freeMB, int timeout) {
...
void imageDisk::Set_GeometryForHardDisk() {
  ...
  for (int m = (Read_AbsoluteSector(0, &mbrData) ? 0 : 4); m--;) {
    if (!mbrData.pentry[m].partSize)
      continue;
    bootstrap bootbuffer;
    if (Read_AbsoluteSector(mbrData.pentry[m].absSectStart, &bootbuffer))
      continue;
    ...
    uint32_t setCyl =
        (mbrData.pentry[m].absSectStart + mbrData.pentry[m].partSize) /
        (setSect * setHeads);
...
void imageDisk::Set_Geometry(uint32_t setHeads, uint32_t setCyl,
  ...
  if (IS_PC98_ARCH) {
    ...
    if (setCyl > 16384)
      LOG_MSG("Warning: This disk image is too big.");
    else if (setCyl > 8192)
      bigdisk_shift = 4;
    else if (setCyl > 4096)
      bigdisk_shift = 3;
...
uint32_t imageDisk::getSectSize(void) { return sector_size; }
...
signed char INT13_ElTorito_IDEInterface =
    -1; /* (controller * 2) + (is_slave?1:0) */
...
static Bitu INT13_DiskHandler(void) {
  ...
  if (!int13_extensions_enable && reg_ah >= 0x40 && reg_ah <= 0x48) {
    LOG_MSG(
        "Warning: Guest is attempting to use INT 13h extensions (AH=0x%02X). "
        "Set 'int 13 extensions=1' if you want to enable them.\n",
        reg_ah);
    reg_ah = 0xff;
    CALLBACK_SCF(true);
    return CBRET_NONE;
  ...
  // drivenum = 0;
  // LOG_MSG("INT13: Function %x called on drive %x (dos drive %d)", reg_ah,
  // reg_dl, drivenum);

  // NOTE: the 0xff error code returned in some cases is questionable; 0x01
  // seems more correct
  switch (reg_ah) {
  case 0x0: /* Reset disk */
  {
    ...
    if (any_images && driveInactive(drivenum)) {
      ...
      if ((machine == MCH_CGA) || (machine == MCH_AMSTRAD) ||
        ...
        if (((imageDiskList[0]) && (imageDiskList[0]->active)) ||
            ((imageDiskList[1]) && (imageDiskList[1]->active))) {
          if (machine != MCH_PCJR && reg_dl < 0x80)
            reg_ip++;
          last_status = 0x00;
          CALLBACK_SCF(false);
        }
  ...
  case 0x2: /* Read sectors */
    ...
    if (drivenum >= MAX_DISK_IMAGES || imageDiskList[drivenum] == NULL) {
      ...
      if (((reg_dl & 0x80) == 0x80) && (reg_dh == 0) &&
        ...
        if (reg_ch == 0) {
          ...
          real_writeb(SegValue(es), reg_bx + 0x1be,
                      0x80); // first partition is active
          real_writeb(SegValue(es), reg_bx + 0x1c2,
     ...
     * mount a PC-98 disk image (1024 bytes/sector) and try to read it with INT
     * 13h. */
    if (imageDiskList[drivenum]->sector_size > sizeof(sectbuf)) {
      LOG(LOG_MISC, LOG_DEBUG)("INT 13h: Read failed because disk bytes/sector "
                               "on drive %c is too large",
                               (char)drivenum + 'A');
    ...
    /* If the disk changed, the first INT 13h read will signal an error and set
     * AH = 0x06 to indicate disk change */
    if (drivenum < 2 && imageDiskChange[drivenum]) {
      LOG(LOG_MISC, LOG_DEBUG)(
          "INT 13h: Failing first read of drive %c to indicate disk change",
          (char)drivenum + 'A');
    ...
    for (i = 0; i < reg_al; i++) {
      last_status = imageDiskList[drivenum]->Read_Sector(
          (uint32_t)reg_dh, (uint32_t)(reg_ch | ((reg_cl & 0xc0) << 2)),
          (uint32_t)((reg_cl & 63) + i), sectbuf);
      ...
      /* IDE emulation: simulate change of IDE state that would occur on a real
       * machine after INT 13h */
      IDE_EmuINT13DiskReadByBIOS(
          reg_dl, (uint32_t)(reg_ch | ((reg_cl & 0xc0) << 2)), (uint32_t)reg_dh,
  ...
  case 0x3: /* Write sectors */
     ...
     * 13h. */
    if (imageDiskList[drivenum]->sector_size > sizeof(sectbuf)) {
      LOG(LOG_MISC, LOG_DEBUG)("INT 13h: Write failed because disk "
                               "bytes/sector on drive %c is too large",
                               (char)drivenum + 'A');
    ...
    for (i = 0; i < reg_al; i++) {
      ...
      last_status = imageDiskList[drivenum]->Write_Sector(
          (uint32_t)reg_dh, (uint32_t)(reg_ch | ((reg_cl & 0xc0) << 2)),
          (uint32_t)((reg_cl & 63) + i), &sectbuf[0]);
      if (last_status != 0x00) {
        CALLBACK_SCF(true);
        return CBRET_NONE;
      }
  ...
  case 0x04: /* Verify sectors */
    ...
    bufptr = reg_bx;
    for(i=0;i<reg_al;i++) {
        last_status = imageDiskList[drivenum]->Read_Sector((uint32_t)reg_dh,
    (uint32_t)(reg_ch | ((reg_cl & 0xc0)<< 2)), (uint32_t)((reg_cl & 63)+i),
    sectbuf); if(last_status != 0x00) { LOG_MSG("Error in disk read");
            CALLBACK_SCF(true);
            return CBRET_NONE;
        }
        for(t=0;t<512;t++) {
    ...
    reg_ah = 0x00;
    // Qbix: The following codes don't match my specs. al should be number of
    // sector verified reg_al = 0x10; /* CRC verify failed */ reg_al = 0x00; /*
    // CRC verify succeeded */
    CALLBACK_SCF(false);

    break;
  case 0x05: /* Format track */
    /* ignore it. I just fucking want FORMAT.COM to write the FAT structure for
     * God's sake */
  ...
  case 0x08: /* Get drive parameters */
    ...
    uint32_t tmpheads, tmpcyl, tmpsect, tmpsize;
    imageDiskList[drivenum]->Get_Geometry(&tmpheads, &tmpcyl, &tmpsect,
                                          &tmpsize);
    if (tmpcyl == 0)
      LOG(LOG_BIOS, LOG_ERROR)("INT13 DrivParm: cylinder count zero!");
    else
    ...
    /* older BIOSes were known to subtract 1 or 2 additional "reserved"
     * cylinders. some code, such as Windows 3.1 WDCTRL, might assume that fact.
     * emulate that here */
    {
  ...
  case 0x15: /* Get disk type */
    ...
    if (any_images) {
      ...
      reg_ah = (drivenum < 2)
                   ? (int13_disk_change_detect_enable ? 2 : 1)
                   : 3; // With 2 for floppy MSDOS starts calling int 13 ah 16
      if (reg_ah == 3) {
      ...
      if (drivenum < DOS_DRIVES && (Drives[drivenum] || drivenum < 2)) {
        ...
        LOG(LOG_BIOS,
            LOG_WARN)("INT13: no images, but invalid drive for call 15");
        reg_ah = 0xff;
        CALLBACK_SCF(true);
      }
    }
    break;
  case 0x16: /* Detect disk change (apparently added to XT BIOSes in 1986
                according to RBIL) */
  ...
  case 0x41: /* Check Extensions Present */
    if ((reg_bx == 0x55aa) && !(driveInactive(drivenum))) {
      ...
      reg_cx = 0x1;    /* Extended disk access functions (AH=42h-44h,47h,48h)
                          supported */
      CALLBACK_SCF(false);
      break;
    }
  ...
  case 0x42: /* Extended Read Sectors From Drive */
    ...
    if (INT13_ElTorito_NoEmuDriveNumber != 0 &&
        INT13_ElTorito_NoEmuDriveNumber == reg_dl) {
      ...
      for (i = 0; i < dap.num; i++) {
        ...
        diskio_delay(512);
        if (killRead ||
            !src_drive->ReadSectorsHost(sectbuf, false, dap.sector + i, 1)) {
          real_writew(SegValue(ds), reg_si + 2,
                      i); // According to RBIL this should update the number of
                          // blocks field to what was successfully transferred
    ...
    for (i = 0; i < dap.num; i++) {
      last_status =
          imageDiskList[drivenum]->Read_AbsoluteSector(dap.sector + i, sectbuf);
  ...
  case 0x48: { /* get drive parameters */
    ...
    real_writew(segat, bufptr + 0x02,
                0x0003); /* C/H/S valid, DMA boundary errors handled */
    ...
    if (bufsz >= 0x1E)
      real_writed(segat, bufptr + 0x1A,
                  0xFFFFFFFF); /* no EDD information available */

    reg_ah = 0x00;
    CALLBACK_SCF(false);
  } break;
  case 0x4B: /* Terminate disk emulation or get emulation status */
    /* NTS: Windows XP CD-ROM boot requires this call to work or else setup
     * cannot find its own files. */
    if (reg_dl == 0x7F || (INT13_ElTorito_NoEmuDriveNumber != 0 &&
                           INT13_ElTorito_NoEmuDriveNumber == reg_dl)) {
      if (reg_al <= 1) {
        ...
        phys_writeb(p + 0x01,
                    (0 /*no emulation*/) +
                        ((INT13_ElTorito_IDEInterface >= 0) ? 0x40 : 0));
        phys_writeb(p + 0x02, INT13_ElTorito_NoEmuDriveNumber);
        if (INT13_ElTorito_IDEInterface >= 0) {
          phys_writeb(p + 0x03, (unsigned char)(INT13_ElTorito_IDEInterface >>
                                                1)); /* which IDE controller */
          phys_writew(p + 0x08, INT13_ElTorito_IDEInterface &
                                    1); /* bit 0: IDE master/slave */
        } else {
  ...
  default:
    LOG(LOG_BIOS,
        LOG_ERROR)("INT13: Function %x called on drive %x (dos drive %d)",
                   (int)reg_ah, (int)reg_dl, (int)drivenum);
    reg_ah = 0xff;
    CALLBACK_SCF(true);
  }
  return CBRET_NONE;
...
void BIOS_SetupDisks(void) {
  ...
  if ((!bootguest && (bootvm || !use_quick_reboot)) || bootdrive < 0)
    FreeBIOSDiskList();

  /* FIXME: Um... these aren't callbacks. Why are they allocated as callbacks?
   * We have ROM general allocation now. */
...
uint8_t imageDiskVFD::Read_Sector(uint32_t head, uint32_t cylinder,
                                  uint32_t sector, void *data,
                                  unsigned int req_sector_size) {
  const vfdentry *ent;
  ...
  //    LOG_MSG("VFD read sector: CHS %u/%u/%u
  //    sz=%u",cylinder,head,sector,req_sector_size);
  ...
  if (ent->hasSectorData()) {
    ...
    if ((uint32_t)ftell(diskimg) != ent->data_offset)
      return 0x05;
    if (fread(data, req_sector_size, 1, diskimg) != 1)
      return 0x05;
    return 0;
  } else if (ent->hasFill()) {
...
imageDiskVFD::findSector(uint8_t head, uint8_t track,
                         uint8_t sector /*TODO: physical head?*/,
                         unsigned int req_sector_size) {
  ...
  while (i != dents.end()) {
    ...
    if (ent.head == head && ent.track == track && ent.sector == sector &&
...
uint8_t imageDiskVFD::Write_Sector(uint32_t head, uint32_t cylinder,
                                   uint32_t sector, const void *data,
                                   unsigned int req_sector_size) {
  ...
  if (ent->hasSectorData()) {
    ...
    if ((uint32_t)ftell(diskimg) != ent->data_offset)
      return 0x05;
    if (fwrite(data, req_sector_size, 1, diskimg) != 1)
      return 0x05;
    return 0;
  } else if (ent->hasFill()) {
    bool isfill = false;
    ...
    if (isfill) {
      ...
      LOG_MSG(
          "VFD write: changing 'fill' sector to one with data (data at %lu)",
          (unsigned long)new_offset);
...
imageDiskVFD::imageDiskVFD(FILE *imgFile, const char *imgName,
                           uint32_t imgSizeK, bool isHardDisk)
    : imageDisk(ID_VFD) {
  ...
  //  +0x6: unknown
  //  +0x7: unknown
  //  +0x8: absolute data offset (32-bit integer) or 0xFFFFFFFF if the entire
  //  sector is that fill byte
  ...
  if (!memcmp(tmp, "VFD1.", 5)) {
    ...
    while ((entof = ((unsigned long)ftell(diskimg) + 12ul)) <= stop_at) {
      ...
      LOG_MSG("VFD entry: track=%u head=%u sector=%u size=%u fill=0x%2X "
              "has_data=%u has_fill=%u entoff=%lu dataoff=%lu",
              v.track, v.head, v.sector, v.getSectorSize(), v.fillbyte,
              v.hasSectorData(), v.hasFill(), (unsigned long)v.entry_offset,
    ...
    if (!dents.empty()) {
      ...
      /* oh yeah right, sure.
       * I suppose you're one of those FDD images where the sector size is 128
       * bytes/sector in the boot sector and the rest is 256 bytes/sector
       * elsewhere. I have no idea why but quite a few FDD images have this
       * arrangement. */
      ...
      if (sector_size != 0) {
        ...
        while (DiskGeometryList[i].ksize != 0) {
          ...
          if (diskent.bytespersect == sector_size) {
            ...
            if (ent != NULL) {
              LOG_MSG("VFD disk probe: %u/%u/%u exists", 0, 0,
                      diskent.secttrack);
      ...
      if (sector_size != 0 && sectors != 0) {
        ...
        while (DiskGeometryList[i].ksize != 0) {
          ...
          if (diskent.bytespersect == sector_size &&
              diskent.secttrack >= sectors) {
            ent = findSector(0, diskent.cylcount - 1, sectors);
            if (ent != NULL) {
              LOG_MSG("VFD disk probe: %u/%u/%u exists", 0,
                      diskent.cylcount - 1, sectors);
      ...
      // TODO: drive_fat.cpp should use an extension to this API to allow
      // changing the sectors/track
      //       according to what it reads from the MS-DOS BIOS parameter block,
      //       just like real MS-DOS. This would allow better representation of
      //       strange disk formats such as the "extended" floppy format that
      //       Microsoft used to use for Word 95 and Windows 95 install
      //       floppies.
...
enum { D88_TRACKMAX = 164, D88_HEADERSIZE = 0x20 + (D88_TRACKMAX * 4) };
...
typedef struct D88HEAD {
  ...
  uint32_t
      trackp[D88_TRACKMAX]; // +0x20 <array of DWORDs>     164 x 4 = 656 = 0x290
} D88HEAD;                  // =0x2B0 total
...
uint8_t imageDiskD88::Read_Sector(uint32_t head, uint32_t cylinder,
                                  uint32_t sector, void *data,
                                  unsigned int req_sector_size) {
  const vfdentry *ent;
  ...
  //    LOG_MSG("D88 read sector: CHS %u/%u/%u
  //    sz=%u",cylinder,head,sector,req_sector_size);
...
imageDiskD88::findSector(uint8_t head, uint8_t track,
                         uint8_t sector /*TODO: physical head?*/,
                         unsigned int req_sector_size) {
...
uint8_t imageDiskD88::Write_Sector(uint32_t head, uint32_t cylinder,
                                   uint32_t sector, const void *data,
                                   unsigned int req_sector_size) {
  const vfdentry *ent;
...
imageDiskD88::imageDiskD88(FILE *imgFile, const char *imgName,
                           uint32_t imgSizeK, bool isHardDisk)
    : imageDisk(ID_D88) {
  (void)isHardDisk; // UNUSED
  D88HEAD head;
  ...
  fseek(diskimg, 0, SEEK_SET);
  if (fread(&head, sizeof(head), 1, diskimg) != 1)
    return;

  // validate fd_size
  if (host_readd((ConstHostPt)(&head.fd_size)) > (uint32_t)fsz)
    return;
  ...
  for (unsigned int track = 0; track < D88_TRACKMAX; track++) {
    ...
    if (trackoff != 0) {
      ...
      if ((off_t)ftell(diskimg) != (off_t)trackoff)
        continue;
      ...
      do {
        if (fread(&s, sizeof(s), 1, diskimg) != 1)
          break;

        uint16_t sector_count = host_readw((ConstHostPt)(&s.sectors));
        uint16_t sector_size = host_readw((ConstHostPt)(&s.size));

        if (sector_count == 0U || sector_size < 128U)
          break;
        if (sector_count > 128U || sector_size > 16384U)
          break;
        if (s.n > 8U)
          s.n = 8U;
        ...
        LOG_MSG("D88: trackindex=%u C/H/S/sz=%u/%u/%u/%u data-at=0x%lx", track,
                vent.track, vent.head, vent.sector, vent.sector_size,
                (unsigned long)vent.data_offset);
  ...
  if (!dents.empty()) {
    ...
    if (sector_size != 0 && sectors != 0) {
      ...
      while (DiskGeometryList[i].ksize != 0) {
        ...
        if (diskent.bytespersect == sector_size &&
          ...
          ent = findSector(0, diskent.cylcount - 1, sectors);
          if (ent != NULL) {
            LOG_MSG("D88 disk probe: %u/%u/%u exists", 0, diskent.cylcount - 1,
                    sectors);
            if (cylinders < diskent.cylcount)
              cylinders = diskent.cylcount;
          }
    ...
    // TODO: drive_fat.cpp should use an extension to this API to allow changing
    // the sectors/track
    //       according to what it reads from the MS-DOS BIOS parameter block,
    //       just like real MS-DOS. This would allow better representation of
    //       strange disk formats such as the "extended" floppy format that
    //       Microsoft used to use for Word 95 and Windows 95 install floppies.

    LOG_MSG("D88 geometry detection: C/H/S %u/%u/%u %u bytes/sector", cylinders,
            heads, sectors, sector_size);
...
uint8_t imageDiskNFD::Read_Sector(uint32_t head, uint32_t cylinder,
                                  uint32_t sector, void *data,
                                  unsigned int req_sector_size) {
  const vfdentry *ent;
  ...
  //    LOG_MSG("NFD read sector: CHS %u/%u/%u
  //    sz=%u",cylinder,head,sector,req_sector_size);
...
imageDiskNFD::findSector(uint8_t head, uint8_t track,
                         uint8_t sector /*TODO: physical head?*/,
                         unsigned int req_sector_size) {
  if ((size_t)track >= dents.size())
    return NULL;
...
uint8_t imageDiskNFD::Write_Sector(uint32_t head, uint32_t cylinder,
                                   uint32_t sector, const void *data,
                                   unsigned int req_sector_size) {
  const vfdentry *ent;
...
imageDiskNFD::imageDiskNFD(FILE *imgFile, const char *imgName,
                           uint32_t imgSizeK, bool isHardDisk,
  ...
  if (revision == 0) {
    ...
    if (fread(&headr1, sizeof(headr1), 1, diskimg) != 1)
      return;
  } else {
    abort();
  }
  ...
  // validate fd_size
  if (host_readd((ConstHostPt)(&head.headersize)) < sizeof(head))
    return;
  if (host_readd((ConstHostPt)(&head.headersize)) > (uint32_t)fsz)
    return;
  ...
  if (revision == 0) {
    unsigned int secents =
        (host_readd((ConstHostPt)(&head.headersize)) - sizeof(head)) /
        sizeof(NFDHDR_ENTRY);
    if (secents == 0)
      return;
    secents--;
    ...
    for (unsigned int i = 0; i < secents; i++) {
      ...
      if (fread(&e, sizeof(e), 1, diskimg) != 1)
        return;
      seclist.push_back(std::pair<uint32_t, NFDHDR_ENTRY>(ofs, e));

      if (e.log_cyl == 0xFF || e.log_head == 0xFF || e.log_rec == 0xFF ||
          e.sec_len_pow2 > 7)
        continue;

      LOG_MSG("NFD %u/%u: ofs=%lu data=%lu cyl=%u head=%u sec=%u len=%u",
              (unsigned int)i, (unsigned int)secents, (unsigned long)ofs,
              (unsigned long)data_offset, e.log_cyl, e.log_head, e.log_rec,
              128 << e.sec_len_pow2);
      ...
      if (data_offset > (unsigned int)fsz)
        return;
    }
  } else {
    ...
    for (unsigned int ti = 0; ti < 164; ti++) {
      ...
      if ((off_t)ftell(diskimg) != (off_t)trkoff)
        return;
      ...
      LOG_MSG("NFD R1 track ent %u offset %lu sectors %u diag %u", ti,
              (unsigned long)trkoff, sectors, diagcount);
      ...
      for (unsigned int s = 0; s < sectors; s++) {
        ...
        LOG_MSG(
            "NFD %u/%u: ofs=%lu data=%lu cyl=%u head=%u sec=%u len=%u rep=%u",
            s, sectors, (unsigned long)ofs, (unsigned long)data_offset,
            e.log_cyl, e.log_head, e.log_rec, 128 << e.sec_len_pow2, e.byRetry);
      ...
      for (unsigned int d = 0; d < diagcount; d++) {
        ...
        LOG_MSG("NFD diag %u/%u: retry=%u len=%u data=%lu", d, diagcount, retry,
                len, (unsigned long)data_offset);
  ...
  if (!dents.empty()) {
    ...
    if (sector_size != 0 && sectors != 0) {
      ...
      while (DiskGeometryList[i].ksize != 0) {
        ...
        if (diskent.bytespersect == sector_size &&
          ...
          ent = findSector(0, diskent.cylcount - 1, sectors);
          if (ent != NULL) {
            LOG_MSG("NFD disk probe: %u/%u/%u exists", 0, diskent.cylcount - 1,
                    sectors);
            if (cylinders < diskent.cylcount)
              cylinders = diskent.cylcount;
          }
    ...
    LOG_MSG("NFD geometry detection: C/H/S %u/%u/%u %u bytes/sector", cylinders,
            heads, sectors, sector_size);
...
bool PartitionLoadMBR(std::vector<partTable::partentry_t> &parts,
                      imageDisk *loadedDisk) {
  partTable smbr;

  parts.clear();

  if (loadedDisk->getSectSize() > sizeof(smbr))
    return false;
  if (loadedDisk->Read_Sector(0, 0, 1, &smbr) != 0x00)
    return false;
  if (smbr.magic1 != 0x55 || smbr.magic2 != 0xaa)
    return false; /* Must have signature */

  /* first copy the main partition table entries */
  for (size_t i = 0; i < 4; i++)
    parts.push_back(smbr.pentry[i]);

  /* then, enumerate extended partitions and add the partitions within, doing it
   * in a way that allows recursive extended partitions */
  {
    size_t i = 0;

    do {
      if (parts[i].parttype == 0x05 /*extended*/ ||
          parts[i].parttype == 0x0F /*LBA extended*/) {
         ...
         * first entry is the partition, sector start relative to parent
         * partition. second entry points to next link in the list. */
        ...
        while (sect < send) {
          ...
          if (smbr.pentry[0].absSectStart == 0 || smbr.pentry[0].partSize == 0)
            break; // FIXME: Not sure what MS-DOS considers the end of the
                   // linked list

          const size_t idx = parts.size();

          /* Right, get this: absolute start sector in entry #0 is relative to
           * this link in the linked list. The pointer to the next link in the
           * linked list is relative to the parent partition. Blegh. */
          ...
          if ((smbr.pentry[0].absSectStart + smbr.pentry[0].partSize) >
              (parts[i].absSectStart + parts[i].partSize))
            break;

          parts.push_back(smbr.pentry[0]);

          /* Based on MS-DOS 5.0, the 2nd entry is a link to the next entry, but
           * only if start sector is nonzero and type is 0x05/0x0F. I'm not
           * certain if MS-DOS allows the linked list to go either direction,
           * but for the sake of preventing infinite loops stop if the link
           * points to a lower sector number. */
          if (idx < 256 &&
              (smbr.pentry[1].parttype == 0x05 ||
               smbr.pentry[1].parttype == 0x0F) &&
              smbr.pentry[1].absSectStart != 0 &&
              smbr.pentry[1].absSectStart > sect) {
...
bool PartitionLoadIPL1(std::vector<_PC98RawPartition> &parts,
                       imageDisk *loadedDisk) {
  ...
  if (loadedDisk->getSectSize() > sizeof(ipltable))
    return false;

  memset(ipltable, 0, sizeof(ipltable));
  if (loadedDisk->Read_Sector(0, 0, 2, ipltable) != 0)
    return false;

  const unsigned int max_entries =
      std::min(16UL, (unsigned long)(loadedDisk->getSectSize() /
                                     sizeof(_PC98RawPartition)));

  for (size_t i = 0; i < max_entries; i++) {
    const _PC98RawPartition *pe = (_PC98RawPartition *)(ipltable + (i * 32));

    if (pe->mid == 0 && pe->sid == 0 && pe->ipl_sect == 0 &&
        pe->ipl_head == 0 && pe->ipl_cyl == 0 && pe->sector == 0 &&
        pe->head == 0 && pe->cyl == 0 && pe->end_sector == 0 &&
        pe->end_head == 0 && pe->end_cyl == 0)
      continue; /* unused */
...
void LogPrintPartitionTable(const std::vector<_PC98RawPartition> &parts) {
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "IPL #%u: boot=%u active=%u startchs=%u/%u/%u endchs=%u/%u/%u '%s'",
        (unsigned int)i, (part.mid & 0x80) ? 1 : 0, (part.sid & 0x80) ? 1 : 0,
        part.cyl, part.head, part.sector, part.end_cyl, part.end_head,
        part.end_sector,
...
void LogPrintPartitionTable(const std::vector<partTable::partentry_t> &parts) {
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "MBR #%u: bootflag=%u parttype=0x%02x beginchs=0x%02x%02x%02x "
        "endchs=0x%02x%02x%02x start=%llu size=%llu",
        (unsigned int)i, (part.bootflag & 0x80) ? 1 : 0, part.parttype,
        part.beginchs[0], part.beginchs[1], part.beginchs[2], part.endchs[0],
        part.endchs[1], part.endchs[2], (unsigned long long)part.absSectStart,
        (unsigned long long)part.partSize);
  }
}

uint8_t imageDiskEmptyDrive::Read_Sector(uint32_t /*head*/,
                                         uint32_t /*cylinder*/,
                                         uint32_t /*sector*/, void * /*data*/,
                                         unsigned int /*req_sector_size*/) {
  return 0x05;
...
uint8_t imageDiskEmptyDrive::Write_Sector(uint32_t /*head*/,
                                          uint32_t /*cylinder*/,
                                          uint32_t /*sector*/,
                                          const void * /*data*/,
                                          unsigned int /*req_sector_size*/) {
...
uint8_t imageDiskEmptyDrive::Read_AbsoluteSector(uint32_t /*sectnum*/,
                                                 void * /*data*/) {
  return 0x05;
}

uint8_t imageDiskEmptyDrive::Write_AbsoluteSector(uint32_t /*sectnum*/,
                                                  const void * /*data*/) {
  return 0x05;
}
...
uint8_t imageDiskINT13Drive::Read_Sector(uint32_t head, uint32_t cylinder,
                                         uint32_t sector, void *data,
                                         unsigned int req_sector_size) {
  ...
  if (!busy && sector_size == req_sector_size && sector_size <= INT13XferSize) {
    ...
    if (INT13Xfer == 0)
      INT13Xfer = DOS_GetMemory(INT13XferSize / 16u, "INT 13 transfer buffer");
...
uint8_t imageDiskINT13Drive::Write_Sector(uint32_t head, uint32_t cylinder,
                                          uint32_t sector, const void *data,
                                          unsigned int req_sector_size) {
  if (INT13Xfer == 0)
    INT13Xfer = DOS_GetMemory(INT13XferSize / 16u, "INT 13 transfer buffer");
...
uint8_t imageDiskINT13Drive::Write_AbsoluteSector(uint32_t sectnum,
                                                  const void *data) {
  unsigned int c, h, s;
...
void imageDiskINT13Drive::Set_Geometry(uint32_t setHeads, uint32_t setCyl,
                                       uint32_t setSect, uint32_t setSectSize) {
  heads = setHeads;
  cylinders = setCyl;
  sectors = setSect;
...
void imageDiskINT13Drive::Get_Geometry(uint32_t *getHeads, uint32_t *getCyl,
                                       uint32_t *getSect,
                                       uint32_t *getSectSize) {
...
imageDiskINT13Drive::imageDiskINT13Drive(imageDisk *sdisk)
    : imageDisk(ID_INT13) {
  subdisk = sdisk;
  subdisk->Addref();
...
imageDiskINT13Drive::~imageDiskINT13Drive() { subdisk->Release(); }
