...
struct Dynamic_Functionality {
  ...
  uint16_t
      num_colors; /* 27h   WORD   number of colors supported in current mode */
  uint8_t
      num_pages; /* 29h   BYTE   number of pages supported in current mode */
  uint8_t num_scanlines;  /* 2Ah   BYTE   number of scan lines active mode
  ...
  uint8_t misc_flags;     /* 2Dh   BYTE   miscellaneous flags
                                                  ...
                                                  6 PS/2 P70 plasma display (without
                             9-dot wide font) active     7 reserved
                                          */
  uint8_t reserved1[3];   /* 2Eh  3 BYTEs reserved (00h) */
  uint8_t vid_mem; /* 31h   BYTE   video memory available 00h = 64K, 01h = 128K,
                      02h = 192K, 03h = 256K */
  uint8_t savep_state_flag; /* 32h   BYTE   save pointer state flags
                                                    bit 0 512 character set
                               active 1 dynamic save area present 2 alpha font
                               override active 3 graphics font override active
    ...
    if (set) {
      if (entries) {
        ...
        for (uint8_t entry = 0; entry < entries; entry++) {
          dccentry = real_readw(RealSeg(dcctable),
                                RealOff(dcctable) + 0x04 + entry * 2);
      ...
      if (index < entries) {
        dccentry =
            real_readw(RealSeg(dcctable), RealOff(dcctable) + 0x04 + index * 2);
        if ((dccentry & 0xff) == 0)
          dccentry >>= 8;
        else if (dccentry >> 8) {
          uint16_t cfg_mono =
              ((real_readw(BIOSMEM_SEG, BIOSMEM_INITIAL_MODE) & 0x30) == 0x30)
                  ? 1
                  : 0;
          if (cfg_mono ^ (dccentry & 1))
            dccentry = (dccentry << 8) | (dccentry >> 8);
        }
      }
    }
  }
  if (set)
    real_writeb(BIOSMEM_SEG, BIOSMEM_DCC_INDEX, index);
  else
    *dcc = dccentry;
}
...
void INT10_GetFuncStateInformation(PhysPt save) {
  ...
  for (i = 0; i < 0x1e; i++) {
    mem_writeb(save + 0x4 + i,
               real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE + i));
  }
  /* Second area */
  ...
  case M_VGA:
    ...
    break;
  default:
    LOG(LOG_INT10, LOG_ERROR)("Get Func State illegal mode type %d",
                              CurMode->type);
  }
  /* Colour count */
  ...
  case 200:
    mem_writeb(save + 0x2a, 0);
    break;
  case 350:
    mem_writeb(save + 0x2a, 1);
    break;
  case 400:
    mem_writeb(save + 0x2a, 2);
    break;
  case 480:
    mem_writeb(save + 0x2a, 3);
    break;
  }
  /* misc flags */
  if (CurMode->type == M_TEXT)
    mem_writeb(save + 0x2d, 0x21);
  else
    mem_writeb(save + 0x2d, 0x01);
...
void INT10_EGA_RIL_ReadRegisterRange(uint8_t ch, uint8_t cl, uint16_t dx,
                                     PhysPt dst) {
  uint16_t port = 0;
  uint16_t regs = 0;
  EGA_RIL(dx, port, regs);
  if (regs == 0) {
    LOG(LOG_INT10, LOG_ERROR)("EGA RIL range read with port %x called",
                              (int)port);
  } else {
    if (ch < regs) {
      if ((Bitu)ch + cl > regs)
      ...
      LOG(LOG_INT10,
          LOG_ERROR)("EGA RIL range read from %x for invalid register %x",
                     (int)port, (int)ch);
  }
}

void INT10_EGA_RIL_WriteRegisterRange(uint8_t ch, uint8_t cl, uint16_t dx,
                                      PhysPt src) {
  uint16_t port = 0;
  uint16_t regs = 0;
  EGA_RIL(dx, port, regs);
  if (regs == 0) {
    LOG(LOG_INT10, LOG_ERROR)("EGA RIL range write called with port %x",
                              (int)port);
  } else {
    if (ch < regs) {
      if ((Bitu)ch + cl > regs)
      ...
      LOG(LOG_INT10,
          LOG_ERROR)("EGA RIL range write to %x with invalid register %x",
                     (int)port, (int)ch);
  }
}
