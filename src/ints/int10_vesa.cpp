...
uint8_t VESA_GetSVGAInformation(uint16_t seg, uint16_t off) {
  ...
  if (((id == 0x56424532) || (id == 0x32454256)) && (!int10.vesa_oldvbe))
    vbe2 = true;

  /* The reason this is an option is that there are some old DOS games that
   * assume the BIOS fills in only the structure members. These games do not
   * provide enough room for the full 256-byte block. If we zero the entire
   * block, unrelated data next to the buffer gets wiped and the game crashes.
   */
  if (vesa_zero_on_get_information) {
    if (vbe2) {
      for (i = 0; i < 0x200; i++)
        mem_writeb((PhysPt)(buffer + i), 0);
    } else {
      for (i = 0; i < 0x100; i++)
        mem_writeb((PhysPt)(buffer + i), 0);
    }
  ...
  if (!int10.vesa_oldvbe)
    mem_writew(buffer + 0x04, 0x200); // Vesa version 2.0
  else if (!int10.vesa_oldvbe10)
    mem_writew(buffer + 0x04, 0x102); // Vesa version 1.2
  else
    mem_writew(buffer + 0x04, 0x100); // Vesa version 1.0
  if (vbe2) {
    vbe2_pos = 256 + off;

    mem_writed(buffer + 0x06, RealMake(seg, vbe2_pos));
    for (i = 0; i < sizeof(string_oem); i++)
      real_writeb(seg, vbe2_pos++, (uint8_t)string_oem[i]);
    mem_writew(buffer + 0x14, 0x200); // VBE 2 software revision
    mem_writed(buffer + 0x16, RealMake(seg, vbe2_pos));
    for (i = 0; i < sizeof(string_vendorname); i++)
      real_writeb(seg, vbe2_pos++, (uint8_t)string_vendorname[i]);
    mem_writed(buffer + 0x1a, RealMake(seg, vbe2_pos));
    for (i = 0; i < sizeof(string_productname); i++)
      real_writeb(seg, vbe2_pos++, (uint8_t)string_productname[i]);
    mem_writed(buffer + 0x1e, RealMake(seg, vbe2_pos));
    for (i = 0; i < sizeof(string_productrev); i++)
      real_writeb(seg, vbe2_pos++, (uint8_t)string_productrev[i]);
  } else {
    vbe2_pos = 0x20 + off;
  ...
  if (vesa_bios_modelist_in_info) {
     ...
     * NOTICE: This limits the modelist to what is able to fit! Extended modes
     * may not fit, which is why the option is OFF by default. */
  ...
  mem_writed(buffer + 0x0a,
             (enable_vga_8bit_dac ? 1 : 0)); // Capabilities and flags
  mem_writew(buffer + 0x12,
             (uint16_t)(GetReportedVideoMemorySize() /
                        (64 * 1024))); // memory size in 64kb blocks
  return VESA_SUCCESS;
}
...
foundit:
  ...
  if (mblock->type == M_ERROR)
    return 0x01;

  bool allow_res = allow_vesa_lowres_modes || (ModeList_VGA[i].swidth >= 640 &&
                                               ModeList_VGA[i].sheight >= 400);

  unsigned int cwidth = (mblock->pitch != 0) ? mblock->pitch : mblock->swidth;

  switch (mblock->type) {
  case M_PACKED4:
    if (!allow_vesa_4bpp_packed &&
        !(ModeList_VGA[i].mode >= 0x202 && ModeList_VGA[i].mode <= 0x208))
      return VESA_FAIL; // TODO: New option to disable
    pageSize = mblock->sheight * cwidth / 2;
    adj = hack_lfb_xadjust / 2;
    var_write(&minfo.BytesPerScanLine,
              (uint16_t)((((cwidth + 15U) / 8U) & (~1U)) *
                         4));   /* NTS: 4bpp requires even value due to VGA
                                   registers, round up */
    ...
    if (!int10.vesa_nolfb && !int10.vesa_oldvbe)
  ...
  case M_LIN4:
    ...
    var_write(&minfo.BytesPerScanLine,
              (uint16_t)(((cwidth + 15U) / 8U) &
                         (~1U))); /* NTS: 4bpp requires even value due to VGA
                                     registers, round up */
    if (!int10.vesa_oldvbe10) {   /* optional in VBE 1.0 */
      var_write(&minfo.NumberOfPlanes, 0x4);
      var_write(
          &minfo.BitsPerPixel,
          4); // bits per pixel is 4 as specified by VESA BIOS 2.0 specification
      var_write(&minfo.MemoryModel, 3); // ega planar mode
    }
    modeAttributes = 0x1b; // Color, graphics, no linear buffer
    break;
  case M_LIN8:
    if (!allow_vesa_8bpp || !allow_res)
      return VESA_FAIL;
  ...
  if (mblock->type == M_LIN4)
    calcmemsize /= 4u; /* 4bpp planar = 4 bytes per video memory byte */
  ...
  if (int10.vesa_oldvbe10)
    modeAttributes &= ~2; /* clear D1, which indicates whether the optional
                             XResolution, etc. fields are present */

  if (!int10.vesa_oldvbe10)
    var_write(&minfo.NumberOfImagePages,
              (uint8_t)pages); /* did not exist until VBE 1.1 */
  ...
  if (mblock->type == M_TEXT) {
    ...
    if (vbe_window_granularity > 0)
      var_write(&minfo.WinGranularity,
                vbe_window_granularity >> 10u); /* field is in KB */
    else
  ...
  if (!int10.vesa_oldvbe10) {
    ...
    var_write(&minfo.XCharSize,
              (uint8_t)mblock->cwidth); /* optional in VBE 1.0 */
    var_write(&minfo.YCharSize,
              (uint8_t)mblock->cheight); /* optional in VBE 1.0 */
  }

  /* this special hack is for the Windows 3.1 S3 driver in "640x480 (1MB) 16
   * million colors" mode which does no scan line or display offset calls at all
   */
  ...
  if (!int10.vesa_nolfb && !int10.vesa_oldvbe &&
      (modeAttributes & 0x80) /*ModeAttributes indicates an LFB*/)
    var_write(&minfo.PhysBasePtr,
              S3_LFB_BASE + adj +
                  (hack_lfb_yadjust *
                   (long)host_readw((HostPt)(&minfo.BytesPerScanLine))));
...
uint8_t VESA_SetCPUWindow(uint8_t window, uint16_t address) {
  if (window && !vesa_bank_switch_window_mirror)
    return VESA_FAIL;
  ...
  if ((!vesa_bank_switch_window_range_check) ||
      (uint32_t)(address)*vga.svga.bank_size <
          calcmemsize) { /* range check, or silently truncate address depending
                            on dosbox-x.conf setting */
    IO_Write(0x3d4, 0x6a);
    IO_Write(
        0x3d5,
        (uint8_t)address); /* NTS: in vga_s3.cpp this is a 7-bit field,
                              wraparound will occur at address >= 128 but only
                              if emulating a full 64KB bank as normal */
    return VESA_SUCCESS;
  } else
    return VESA_FAIL;
}
...
uint8_t VESA_SetPalette(PhysPt data, Bitu index, Bitu count, bool wait) {
  ...
  if (wait)
    CALLBACK_RunRealFar(RealSeg(int10.rom.wait_retrace),
                        RealOff(int10.rom.wait_retrace));
...
uint8_t VESA_ScanLineLength(uint8_t subcall, uint16_t val, uint16_t &bytes,
  ...
  case 0x00: // set scan length in pixels
    ...
    // NTS: The VESA BIOS standard says a too-large value should return an
    // error.
    //      VBETEST.EXE behavior seems to depend on this call capping the value
    //      and returning success, else it misdraws the screen and might get
    //      stuck drawing junk.
...
static Bitu VESA_PMSetStart(void) {
  uint32_t start =
      (uint32_t)(((unsigned int)reg_dx << 16u) | (unsigned int)reg_cx);
...
Bitu INT10_WriteVESAModeList(Bitu max_modes) {
  ...
  while (ModeList_VGA[i].mode != 0xffff) {
    ...
    /* do not list modes that are marked as do not list */
       ...
       * If the mode exceeds video memory, then the mode will be reported as not
       * supported by VESA BIOS functions.
       *
       * If the SVGA card would accept the mode (generally it's a memsize
       * check), then accept. */
      ...
      if (canuse_mode) {
        if (ModeList_VGA[i].mode >= 0x100) {
          ...
          bool allow3 = allow_hd_vesa_modes ||
                        !(ModeList_VGA[i].special & _HIGH_DEFINITION);
          bool allow4 = allow_unusual_vesa_modes ||
                        !(ModeList_VGA[i].special & _UNUSUAL_MODE);
          bool allow5 = /* either user modified or within the limits of the
                           render scaler architecture */
              (ModeList_VGA[i].special & _USER_MODIFIED) ||
              (ModeList_VGA[i].swidth <= SCALER_MAXWIDTH &&
               ModeList_VGA[i].sheight <= SCALER_MAXHEIGHT);
          bool allow_res = allow1 && allow2 && allow3 && allow4 && allow5;
          bool allow_s3_packed4 =
              (ModeList_VGA[i].mode >= 0x202 && ModeList_VGA[i].mode <= 0x208);

          switch (ModeList_VGA[i].type) {
          case M_LIN32:
            canuse_mode = allow_vesa_32bpp && allow_res;
            break;
          case M_LIN24:
            canuse_mode = allow_vesa_24bpp && allow_res;
            break;
          case M_LIN16:
            canuse_mode = allow_vesa_16bpp && allow_res;
            break;
          case M_LIN15:
            canuse_mode = allow_vesa_15bpp && allow_res;
            break;
          case M_LIN8:
            canuse_mode = allow_vesa_8bpp && allow_res;
            break;
          case M_LIN4:
            canuse_mode = allow_vesa_4bpp && allow_res;
            break;
          case M_PACKED4:
            canuse_mode =
                (allow_vesa_4bpp_packed || allow_s3_packed4) && allow_res;
            break;
          case M_TEXT:
            canuse_mode = allow_vesa_tty && allow_res;
            break;
          default:
            break;
          }
    ...
    if (canuse_mode && vesa_modelist_cap > 0 &&
        (unsigned int)modecount >= (unsigned int)vesa_modelist_cap)
      canuse_mode = false;
    if (canuse_mode && (unsigned int)modecount >= (unsigned int)max_modes)
      canuse_mode = false;

    if (ModeList_VGA[i].type != M_TEXT) {
      if (canuse_mode && vesa_mode_width_cap > 0 &&
          (unsigned int)ModeList_VGA[i].swidth >
              (unsigned int)vesa_mode_width_cap)
        canuse_mode = false;

      if (canuse_mode && vesa_mode_height_cap > 0 &&
          (unsigned int)ModeList_VGA[i].sheight >
              (unsigned int)vesa_mode_height_cap)
        canuse_mode = false;
    }

    if (ModeList_VGA[i].mode >= 0x100 && canuse_mode) {
      if ((!int10.vesa_oldvbe) || (ModeList_VGA[i].mode < 0x120)) {
        phys_writew(PhysMake((uint16_t)0xc000, (uint16_t)mode_wptr),
                    (uint16_t)ModeList_VGA[i].mode);
        mode_wptr += 2;
        modecount++;
      }
    }
  ...
  assert(modecount <=
         int10.rom.vesa_alloc_modes); /* do not overrun the buffer */
...
void INT10_SetupVESA(void) {
  ...
  /* BUGFIX: Generating VESA BIOS data when machine=ega or machine=vgaonly is
   * dumb. Stop wasting ROM space! --J.C. */
  if (machine != MCH_VGA)
    return;
  ...
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      0, NULL, CB_VESA_WAIT, PhysMake(0xc000, int10.rom.used), "");
  callback.rmWindow = CALLBACK_Allocate();
  int10.rom.set_window = RealMake(0xc000, int10.rom.used);
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      callback.rmWindow, VESA_SetWindow, CB_RETF,
  ...
  int10.rom.pmode_interface_window =
      int10.rom.used - RealOff(int10.rom.pmode_interface);
  phys_writew(Real2Phys(int10.rom.pmode_interface) + 0,
              int10.rom.pmode_interface_window);
  callback.pmWindow = CALLBACK_Allocate();
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      callback.pmWindow, VESA_PMSetWindow, CB_RETN,
      PhysMake(0xc000, int10.rom.used), "VESA PM Set Window");
  /* PM Set start call */
  int10.rom.pmode_interface_start =
      int10.rom.used - RealOff(int10.rom.pmode_interface);
  phys_writew(Real2Phys(int10.rom.pmode_interface) + 2,
              int10.rom.pmode_interface_start);
  callback.pmStart = CALLBACK_Allocate();
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      callback.pmStart, VESA_PMSetStart, CB_VESA_PM,
      PhysMake(0xc000, int10.rom.used), "VESA PM Set Start");
  /* PM Set Palette call */
  int10.rom.pmode_interface_palette =
      int10.rom.used - RealOff(int10.rom.pmode_interface);
  phys_writew(Real2Phys(int10.rom.pmode_interface) + 4,
              int10.rom.pmode_interface_palette);
  callback.pmPalette = CALLBACK_Allocate();
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      0, NULL, CB_VESA_PM, PhysMake(0xc000, int10.rom.used), "");
  int10.rom.used += (uint16_t)CALLBACK_Setup(
      callback.pmPalette, VESA_PMSetPalette, CB_RETN,
  ...
  int10.rom.pmode_interface_size =
      int10.rom.used - RealOff(int10.rom.pmode_interface);
}
