...
Bitu XMS_GetEnabledA20(void) {
  if (IS_PC98_ARCH) // NEC PC-98: Port 0xF2, bit 0, cleared if A20 enabled (set
                    // if A20 masked)
...
Bitu XMS_GetTotalHandles(void) { return XMS_HANDLES; }
...
void XMS_ZeroAllocation(MemHandle mem, unsigned int pages) {
  ...
  if ((address + pages) > 0xC0000000UL)
    E_Exit("XMS_ZeroAllocation out of range");
  while (pages != 0) {
...
Bitu XMS_AllocateMemory(Bitu size, uint16_t &handle) { // size = kb
  ...
  if (size != 0) {
    ...
    if (dbg_zero_on_xms_allocmem)
      XMS_ZeroAllocation(mem, (unsigned int)pages);
  } else {
    mem = MEM_GetNextFreePage();
    if (mem == 0)
      LOG(LOG_MISC, LOG_DEBUG)(
          "XMS:Allocate zero pages with no memory left"); // Windows 3.1
                                                          // triggers this
                                                          // surprisingly often!
    if (mem != 0 && dbg_zero_on_xms_allocmem)
      XMS_ZeroAllocation(mem, 1);
  }
...
Bitu XMS_MoveMemory(PhysPt bpt) {
  ...
  if (src_handle) {
    ...
    /* Microsoft TEST.C considers it an error to allow real mode pointers +
     * length to extend past the end of the 8086-accessible conventional memory
     * area. */
  ...
  if (length != 0) {
    ...
    // HIMEM.SYS on 386 and higher is said to use "flat real mode" to copy
    // extended memory. That means if a program calls this function, it
    // implicitly sets up flat real mode. It seems some demoscene stuff in the
    // 1992-1994 timeframe assume this case.
    if (xms_memmove_flatrealmode) {
      LOG(LOG_MISC,
          LOG_DEBUG)("XMS: Memory move/copy is enabling flat real mode");
...
Bitu XMS_GetHandleInformation(Bitu handle, uint8_t &lockCount, uint8_t &numFree,
                              uint32_t &size) {
  if (InvalidHandle(handle))
    return XMS_INVALID_HANDLE;
...
Bitu XMS_LocalEnableA20(void) {
  /* This appears to be how Microsoft HIMEM.SYS implements this. A20 is only
   * enabled if the local enable count was == 0 at entry to this call. */
...
Bitu XMS_LocalDisableA20(void) {
  /* This appears to be how Microsoft HIMEM.SYS implements this. A20 is only
   * disabled if the local enable count was == 1 at entry to this call. */
  if (xms_local_enable_count > 0) {
    if (--xms_local_enable_count == 0)
      XMS_EnableA20(false);
  } else {
...
/* Same as XMS_DOS_LocalA20DisableIfNotEnabled() but this version works by
 * making a call through the XMS entry point. This version must be used if the
 * guest application or shell is calling INT 21h from 80386 virtual 8086 mode
 * (i.e. a Windows 3.1 "DOS Box") because directly touching port 92h might cause
 * the protected mode kernel to crash. Windows 3.1 does not appear to virtualize
 * port 92h. */
void XMS_DOS_LocalA20DisableIfNotEnabled_XMSCALL(void) {
  ...
  LOG(LOG_DOSMISC,
      LOG_DEBUG)("Temporarily disabling A20 gate by calling XMS entry point. "
                 "Hopefully the vm86 protected mode kernel will do its job");

  reg_ah = 0x06; /* local disable */
  CALLBACK_RunRealFar((uint16_t)(xms_callback >> 16ul),
                      (uint16_t)(xms_callback & 0xFFFFul));
...
void XMS_DOS_LocalA20DisableIfNotEnabled(void) {
  ...
  if (XMS_GetEnabledA20()) {
    LOG(LOG_DOSMISC,
        LOG_DEBUG)("Temporarily disabling A20 gate. As a hack this will FORCE "
                   "local A20 enable to zero (from count=%d)",
                   xms_local_enable_count);
 ...
 * a call through the XMS entry point. This version must be used if the guest
 * application or shell is calling INT 21h from 80386 virtual 8086 mode (i.e. a
 * Windows 3.1 "DOS Box") because directly touching port 92h might cause the
 * protected mode kernel to crash. Windows 3.1 does not appear to virtualize
 * port 92h. */
void XMS_DOS_LocalA20EnableIfNotEnabled_XMSCALL(void) {
  ...
  if (reg_ax == 0) {
    /* A20 is not enabled */
    LOG(LOG_DOSMISC,
        LOG_DEBUG)("DOS=HIGH, XMS enabled, A20 gate disabled. Reenabling A20 "
                   "gate on INT 21h call via XMS driver.");
...
void XMS_DOS_LocalA20EnableIfNotEnabled(void) {
  ...
  if (!XMS_GetEnabledA20()) {
    LOG(LOG_DOSMISC, LOG_DEBUG)("DOS=HIGH, XMS enabled, A20 gate disabled. "
                                "Reenabling A20 gate on INT 21h call.");
    XMS_LocalEnableA20();
  }
}
...
Bitu XMS_Handler(void) {
  ...
  case XMS_FREE_HIGH_MEMORY: /* 02 */
    if (xms_hma_exists) {
      if (DOS_IS_IN_HMA())
        LOG(LOG_MISC, LOG_WARN)("DOS application attempted to free HMA while "
                                "DOS kernel occupies it!");
  ...
  case XMS_QUERY_FREE_EXTENDED_MEMORY: /* 08 */
    ...
    if (reg_eax > 65535)
      reg_eax = 65535; /* cap sizes for older DOS programs. newer ones use
                          function 0x88 */
    if (reg_edx > 65535)
      reg_edx = 65535;
    break;
  case XMS_ALLOCATE_ANY_MEMORY: /* 89 */
  { /* Chopping off bits 16-31 to fall through to ALLOCATE_EXTENDED_MEMORY is
       inaccurate. The Extended Memory Specification states you use all of EDX,
       so programs can request 64MB or more. Even if DOSBox does not (yet)
       support >= 64MB of RAM. */
    uint16_t handle = 0;
  ...
  case XMS_GET_EMB_HANDLE_INFORMATION: /* 0e */
    SET_RESULT(XMS_GetHandleInformation(reg_dx, reg_bh, reg_bl, reg_edx),
               false);
    reg_edx &= 0xFFFF;
    break;
  ...
  case XMS_QUERY_ANY_FREE_MEMORY: /* 88 */
    reg_bl = (uint8_t)XMS_QueryFreeMemory(reg_eax, reg_edx);
    reg_ecx = (uint32_t)((MEM_TotalPages() * MEM_PAGESIZE) -
                         1); // highest known physical memory address
    break;
  case XMS_GET_EMB_HANDLE_INFORMATION_EXT: { /* 8e */
    uint8_t free_handles;
    Bitu result =
        XMS_GetHandleInformation(reg_dx, reg_bh, free_handles, reg_edx);
    if (result != 0)
      reg_bl = (uint8_t)result;
    else
      reg_cx = free_handles;
    reg_ax = (result == 0);
  } break;
...
bool XMS_IS_ACTIVE() { return (xms_callback != 0); }

bool XMS_HMA_EXISTS() { return XMS_IS_ACTIVE() && xms_hma_exists; }
...
void RemoveUMBBlock() {
  ...
  if (umb_init) {
    LOG_MSG("Removing UMB block 0x%04x-0x%04x\n", first_umb_seg,
            first_umb_seg + first_umb_size - 1);
    MEM_unmap_physmem(
        (unsigned long)first_umb_seg << 4ul,
        (((unsigned long)first_umb_seg + (unsigned long)first_umb_size)
         << 4ul) -
            1ul);
    umb_init = false;
  }
...
void Update_Get_Desired_Segment(void) {
  ...
  if (IS_PC98_ARCH &&
      (desired_ems_segment == 0xC000 || desired_ems_segment == 0xD000)) {
    /* ok */
...
public:
  XMS(Section *configuration) : Module_base(configuration) {
    ...
    xms_memmove_flatrealmode =
        section->Get_bool("xms memmove causes flat real mode");
    ...
    LOG_MSG("XMS: %u handles allocated for use by the DOS environment",
            XMS_HANDLES);

    /* NTS: Disable XMS emulation if CPU type is less than a 286, because
     * extended memory did not exist until the CPU had enough address lines to
     * read past the 1MB mark.
     *
     *      The other reason we do this is that there is plenty of software that
     * assumes 286+ instructions if they detect XMS services, including but not
     * limited to:
     *
    ...
    /* TODO: Add option to allow users to *force* XMS emulation, overriding this
     * lockout, if they're crazy enough to see what happens or they want to
     * witness the mis-detection mentioned above. */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_286) {
      LOG_MSG("CPU is 80186 or lower model that lacks the address lines needed "
              "for 'extended memory' to exist, disabling XMS");
      return;
    }

    /* NTS: Disable XMS emulation if the memsize setting is 1MB+64KB or less. In
     * that case there is nothing that counts as "extended memory" and therefore
     * no reason to emulate XMS services. */
    /* TODO: Add option to *force* XMS emulation, overriding this lockout, if
     * you really want to emulate HIMEM.SYS loaded and active on a DOS
     * configuration with only 256KB of memory. */
    if (MEM_TotalPages() <=
        0x110) { /* If not enough for 1MB base memory + 64KB HMA area */
    ...
    xms_hma_minimum_alloc =
        (unsigned int)section->Get_int("hma minimum allocation");
    xms_hma_alloc_non_dos_kernel_control =
        section->Get_bool("hma allow reservation");
    if (xms_hma_minimum_alloc > 0xFFF0U)
    ...
    enable_a20_on_windows_init =
        section->Get_bool("enable a20 on windows init");
    dbg_zero_on_xms_allocmem =
        section->Get_bool("zero memory on xms memory allocation");

    if (dbg_zero_on_xms_allocmem) {
      LOG_MSG("Debug option enabled: XMS memory allocation will always clear "
              "memory block before returning\n");
    }

    /* place hookable callback in writable memory area */
    xms_callback = RealMake(DOS_GetMemory(0x1, "xms_callback") - 1, 0x10);
    callbackhandler.Install(&XMS_Handler, CB_HOOKABLE, Real2Phys(xms_callback),
                            "XMS Handler");
    ...
    if (umb_available && machine == MCH_PCJR) {
      LOG(LOG_MISC, LOG_DEBUG)(
          "UMB emulation is incompatible with PCjr emulation, disabling UMBs");
      umb_available = false;
    }
    ...
    // Sanity check
    if (rombios_minimum_location == 0)
      E_Exit("Uninitialized ROM BIOS base");
    ...
    if (first_umb_size == 0)
      first_umb_size = (uint16_t)(ROMBIOS_MinAllocatedLoc() >> 4);

    if (first_umb_seg < 0xC000 || first_umb_seg < DOS_PRIVATE_SEGMENT_END) {
      LOG(LOG_MISC, LOG_WARN)(
          "UMB blocks before 0xD000 conflict with VGA (0xA000-0xBFFF), VGA "
          "BIOS (0xC000-0xC7FF) and DOSBox private area (0x%04x-0x%04x)",
          DOS_PRIVATE_SEGMENT, DOS_PRIVATE_SEGMENT_END - 1);
      first_umb_seg = 0xC000;
      if (first_umb_seg < (Bitu)DOS_PRIVATE_SEGMENT_END)
        first_umb_seg = (Bitu)DOS_PRIVATE_SEGMENT_END;
    }
    if (first_umb_seg >= (rombios_minimum_location >> 4)) {
      LOG(LOG_MISC, LOG_NORMAL)("UMB starting segment 0x%04x conflict with "
                                "BIOS at 0x%04x. Disabling UMBs",
                                (int)first_umb_seg,
                                (int)(rombios_minimum_location >> 4));
      umb_available = false;
    }
    ...
    if (IS_PC98_ARCH) {
      ...
      /* Do not let the private segment overlap with anything else after segment
       * C800:0000 including the SOUND ROM at CC00:0000. Limiting to 32KB also
       * leaves room for UMBs if enabled between C800:0000 and the EMS page
       * frame at (usually) D000:0000 */
    ...
    if (first_umb_size >= (rombios_minimum_location >> 4)) {
      /* we can ask the BIOS code to trim back the region, assuming it hasn't
       * allocated anything there yet */
      LOG(LOG_MISC, LOG_DEBUG)("UMB ending segment 0x%04x conflicts with BIOS "
                               "at 0x%04x, asking BIOS to move aside",
                               (int)first_umb_size,
                               (int)(rombios_minimum_location >> 4));
    ...
    /* 2017/12/24 I just noticed that the EMS page frame will conflict with UMB
     * on standard configuration. In IBM PC mode the EMS page frame is at
     * E000:0000. In PC-98 mode the EMS page frame is at D000:0000. */
    if (ems_available && first_umb_size >= GetEMSPageFrameSegment() &&
      ...
      LOG(LOG_MISC,
          LOG_DEBUG)("UMB overlaps EMS page frame at 0x%04x, truncating region",
                     (unsigned int)GetEMSPageFrameSegment());
      first_umb_size = (uint16_t)(GetEMSPageFrameSegment() - 1);
    }
    /* UMB cannot interfere with EGC 4th graphics bitplane on PC-98 */
    /* TODO: Allow UMB into E000:xxxx if emulating a PC-98 that lacks 16-color
     * mode. */
    if (IS_PC98_ARCH && first_umb_size >= 0xE000) {
      LOG(LOG_MISC, LOG_DEBUG)(
          "UMB overlaps PC-98 EGC 4th graphics bitplane, truncating region");
      first_umb_size = 0xDFFF;
    }
    if (first_umb_size < first_umb_seg) {
      LOG(LOG_MISC, LOG_NORMAL)("UMB end segment below UMB start. I'll just "
                                "assume you mean to disable UMBs then.");
      first_umb_size = first_umb_seg - 1;
      umb_available = false;
    }
    first_umb_size = (first_umb_size + 1 - first_umb_seg);
    if (umb_available) {
      LOG(LOG_MISC, LOG_NORMAL)("UMB assigned region is 0x%04x-0x%04x",
                                (int)first_umb_seg,
                                (int)(first_umb_seg + first_umb_size - 1));
      if (MEM_map_RAM_physmem(
              (unsigned int)first_umb_seg << 4u,
              (((unsigned int)first_umb_seg + (unsigned int)first_umb_size)
               << 4u) -
                  1u)) {
        memset(GetMemBase() + ((unsigned int)first_umb_seg << 4u), 0x00u,
               (unsigned int)first_umb_size << 4u);
      } else {
        LOG(LOG_MISC, LOG_WARN)("Unable to claim UMB region (perhaps adapter "
                                "ROM is in the way). Disabling UMB");
        umb_available = false;
      }
    }
    ...
    if (xms_init_flatrealmode) {
      LOG(LOG_MISC,
          LOG_DEBUG)("XMS: Initialization is enabling flat real mode");
      XMS_InitFlatRealMode();
    }

    /* CP/M compat will break unless a copy of the JMP instruction is mirrored
     * in HMA */
...
void XMS_ShutDown(Section * /*sec*/) { XMS_DoShutDown(); }

bool XMS_Active(void) { return (test != NULL) && xms_init; }
...
void XMS_Init() {
  ...
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(XMS_ShutDown));
}
...
public:
  SerializeXMS() : SerializeGlobalPOD("XMS") { registerPOD(xms_handles); }
} dummy;
} // namespace
