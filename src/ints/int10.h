...
/* FIXME: Wait, what?? What the hell kind of preprocessor macro is this??? Kill
 * these macros! --J.C. */
#define BIOS_NCOLS                                                             \
  uint16_t ncols = IS_PC98_ARCH ? 80 : real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
#define BIOS_NROWS                                                             \
  uint16_t nrows =                                                             \
      IS_PC98_ARCH ? (uint16_t)(real_readb(0x60, 0x112) + 1u)                  \
      : IS_EGAVGA_ARCH                                                         \
          ? ((uint16_t)real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) + 1)           \
          : 25;
#define BIOS_CHEIGHT                                                           \
  uint8_t cheight =                                                            \
      IS_EGAVGA_ARCH ? real_readb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT) : 8;
...
#define _DOUBLESCAN                                                            \
  0x0002 /* CGA/EGA on VGA doublescan (bit 7 of max scanline) */
...
#define _S3_POW2_STRIDE                                                        \
  0x0040 /* Stride must be a power of 2, round up after offset calculation */
#define _HIGH_DEFINITION 0x0080
#define _UNUSUAL_MODE 0x0100
#define _DO_NOT_LIST                                                           \
  0x0200 /* support the mode but do not list in VBE mode enumeration */
#define _USER_DISABLED                                                         \
  0x4000 /* disabled (cannot set mode) but still listed in modelist */
...
/* Font Stuff */
void INT10_LoadFont(PhysPt font, bool reload, uint16_t count, Bitu offset,
                    Bitu map, uint8_t height);
...
void INT10_SetSingleDACRegister(uint8_t index, uint8_t red, uint8_t green,
                                uint8_t blue);
void INT10_GetSingleDACRegister(uint8_t index, uint8_t *red, uint8_t *green,
                                uint8_t *blue);
