 ...
 *       With limit variable to emulate EMM386.EXE limits (32MB limit in
 * MS-DOS/Win9x). With command .COM program in the Z:\ directly to allow
 * enabling/disabling EMS and turning on/off the virtual 8086 mode, just like
 * what EMM386.EXE offers in DOS. With code to create and maintain vm86 page
 * tables when emulating EMM386.EXE, while retaining the 'hardware' based
 * mapping it does now if "mixed" or "board". With configuration parameter to
 * control the number of EMS handles. With configuration parameter to specify
 * what version EMS to emulate. With code to change DOS device EMMXXXX0 <->
 * EMMQXXX0 depending on when you enable/disable EMS at runtime. With runtime
 * enable/disable of VCPI (if nobody is using VCPI). With DOSBox option to force
 * EMM OS handle to be allocated from an within an even megabyte if the user
 *           would rather be able to manage the A20 line without crashing, and
 * another option to specify if A20 should be turned on or left off in this
 * case.
 */
...
Bitu GetEMSPageFrameSegment(void) { return EMM_PAGEFRAME; }

#define EMM_MAX_HANDLES 200U /* 255 Max */
#define EMM_PAGE_SIZE (16U * 1024U)
#define EMM_MAX_PHYS 4U /* 4 16kb pages in pageframe */

Bitu GetEMSPageFrameSize(void) { return EMM_MAX_PHYS * EMM_PAGE_SIZE; }
...
enum {
  ...
  EMS_BOARD, /* act like pre-386 expanded memory, provided by an expansion card
              */
  EMS_EMM386 /* act like 386+ expanded memory, faked using virtual 8086 mode
                (EMM386.EXE style) */
};
    ...
    if (x.pages != NULL_HANDLE) {
      {
        ...
        while (i < sizeof(x.name) && x.name[i] != 0)
          i++;
...
Bitu EMS_Max_Handles(void) { return EMM_MAX_HANDLES; }

bool EMS_Active(void) { return ems_type != EMS_NONE; }
...
bool device_EMM::ReadFromControlChannel(PhysPt bufptr, uint16_t size,
                                        uint16_t *retcode) {
  ...
  case 0x01: {
    ...
    if (size != 6)
      return false;
    if (GEMMIS_seg == 0)
      GEMMIS_seg = DOS_GetMemory(0x20, "GEMMIS_seg");
    ...
    mem_writew(GEMMIS_addr + 0x04,
               GEMMIS_VERSION); // version 1.0 (provide ems information only)
    ...
    for (PhysPt frct = 0; frct < 0x10U / 4U; frct++) {
      ...
      mem_writeb(GEMMIS_addr + 0x0a + frnr,
                 0x03); // frame type: EMS frame in 64k page
      mem_writeb(GEMMIS_addr + 0x0b + frnr, 0xff);   // owner: NONE
      mem_writew(GEMMIS_addr + 0x0c + frnr, 0x7fff); // no logical page number
      mem_writeb(GEMMIS_addr + 0x0e + frnr,
                 (uint8_t)(frct & 0xff));          // physical EMS page number
      mem_writeb(GEMMIS_addr + 0x0f + frnr, 0x00); // EMS frame
    }
    /* build non-EMS ROM frames (0xf000-0x10000) */
    for (PhysPt frct = (EMM_PAGEFRAME4K + 0x10u) / 4u; frct < 0xf0u / 4u;
         frct++) {
      ...
      mem_writeb(GEMMIS_addr + 0x0e + frct * 6u,
                 0xff); // EMS page number (NONE)
    ...
    if (emm_handles[EMM_SYSTEM_HANDLE].pages != NULL_HANDLE) {
      mem_writew(GEMMIS_addr + 0x197,
                 (emm_handles[EMM_SYSTEM_HANDLE].pages + 3u) / 4u);
      mem_writed(GEMMIS_addr + 0x199,
                 (unsigned int)emm_handles[EMM_SYSTEM_HANDLE].mem
                     << 12u); // physical address
    } else {
  ...
  case 0x03:
    ...
    mem_writew(bufptr + 0x00,
               (uint16_t)(MEM_TotalPages() * 4ul)); // max size (kb)
...
void EMS_ZeroAllocation(MemHandle mem, unsigned int pages) {
  ...
  if ((address + pages) > 0xC0000000ul)
    E_Exit("EMS_ZeroAllocation out of range");
  while (pages != 0) {
...
/* NTS: "page" in EMS refers to 16KB regions, not the 4KB memory pages we
 * normally work with */
uint8_t EMM_AllocateMemory(uint16_t pages, uint16_t &dhandle,
  ...
  if (pages) {
    ...
    if (!mem)
      E_Exit("EMS:Memory allocation failure");
    else if (dbg_zero_on_ems_allocmem)
      EMS_ZeroAllocation(mem, pages * 4u);
  }
...
static uint8_t EMM_AllocateSystemHandle(uint16_t pages/*NTS: EMS pages are 16KB, this does not refer to 4KB CPU pages*/) {
  ...
  uint16_t handle =
      EMM_SYSTEM_HANDLE; // emm system handle (reserved for OS usage)
  /* Release memory if already allocated */
  if (emm_handles[handle].pages != NULL_HANDLE) {
    MEM_ReleasePages(emm_handles[handle].mem);
  }

  /* NTS: DOSBox 0.74 and older versions of DOSBox-X allocated EMS memory with
   * the "sequential" param == false */
  /* We now offer the option, if specified in the configuration, to allocate our
   * system handle on an even megabyte. Why? Well, if the system handle exists
   * on an even megabyte, then it no longer matters what state the A20 gate is
   * in. If the DOS game relies on EMM386.EXE but fiddles with the A20 gate
   * while running in virtual 8086 mode, setting this option can help avoid
   * crashes, where normally clearing the A20 gate prevents EMM386.EXE from
   * using its protected mode structures (GDT, page tables) and the sudden
   * aliasing turns them into junk and the system crashes (random contents in
   * DOS conventional memory interpreted as protected mode structures doesn't
   * work very well). */
  mem = 0;
  if (ems_syshandle_on_even_mb) {
    ...
    if (!mem)
      LOG(LOG_MISC,
          LOG_WARN)("EMS: Despite configuration setting, I was unable to "
                    "allocate EMS system handle on even megabyte");
  }
  if (!mem)
    mem = MEM_AllocatePages(pages * 4u, /*sequential=*/true);
  if (!mem)
    E_Exit("EMS:System handle memory allocation failure");
  emm_handles[handle].pages = pages;
  emm_handles[handle].mem = mem;
  LOG(LOG_MISC,
      LOG_DEBUG)("EMS: OS handle allocated %u 16KB pages 0x%08lx-0x%08lx",
                 (unsigned int)pages, (unsigned long)mem * 4096UL,
...
static uint8_t EMM_ReallocatePages(uint16_t handle, const uint16_t &pages) {
  ...
  if (emm_handles[handle].pages != 0) {
    ...
    if (!MEM_ReAllocatePages(emm_handles[handle].mem, pages * 4u, false))
      return EMM_OUT_OF_LOG;
  } else {
    MemHandle mem = MEM_AllocatePages(pages * 4u, false);
    if (!mem)
      E_Exit("EMS:Memory allocation failure during reallocation");
    emm_handles[handle].mem = mem;
  }
  /* Update size */
  emm_handles[handle].pages = pages;
...
static uint8_t EMM_MapPage(Bitu phys_page, uint16_t handle, uint16_t log_page) {
  //	LOG_MSG("EMS MapPage handle %d phys %d log
  ...
  if (log_page == NULL_PAGE) {
    ...
    for (Bitu i = 0; i < 4; i++)
      PAGING_MapPage(EMM_PAGEFRAME4K + phys_page * 4u + i,
                     EMM_PAGEFRAME4K + phys_page * 4u + i);
    PAGING_ClearTLB();
    return EMM_NO_ERROR;
  }
  /* Check for valid handle */
  ...
  if (log_page < emm_handles[handle].pages) {
    ...
    for (Bitu i = 0; i < 4; i++) {
      PAGING_MapPage(EMM_PAGEFRAME4K + (unsigned int)phys_page * 4u + i,
                     (Bitu)memh);
...
static uint8_t EMM_MapSegment(Bitu segment, uint16_t handle,
                              uint16_t log_page) {
  //	LOG_MSG("EMS MapSegment handle %d segment %d log
  //%d",handle,segment,log_page);

  bool valid_segment = false;

  if ((ems_type == EMS_MIXED) || (ems_type == EMS_EMM386)) {
    if (segment < 0xf000U + 0x1000U)
      valid_segment = true;
  } else {
  ...
  if (valid_segment) {
    int32_t tphysPage = ((int32_t)segment - (int32_t)EMM_PAGEFRAME) /
                        (int32_t)(0x1000 / EMM_MAX_PHYS);
...
static uint8_t EMM_SavePageMap(uint16_t handle) {
  /* Check for valid handle */
  if (handle >= EMM_MAX_HANDLES ||
      (handle != 0 && emm_handles[handle].pages == NULL_HANDLE)) {
    return EMM_INVALID_HANDLE;
  }
...
static uint8_t EMM_RestoreMappingTable(void) {
  ...
  for (Bitu i = 0; i < 0x40; i++) {
    /* Skip the pageframe */
    if ((i >= (unsigned int)EMM_PAGEFRAME / 0x400U) &&
        (i <
         ((unsigned int)EMM_PAGEFRAME / 0x400U) + (unsigned int)EMM_MAX_PHYS))
      continue;
...
static uint8_t EMM_PartialPageMapping(void) {
  ...
  case 0x00: /* Save Partial Page Map */
    ...
    data += 2;
    for (; count > 0; count--) {
      uint16_t segment = mem_readw(list);
      list += 2;
      if ((segment >= EMM_PAGEFRAME) && (segment < EMM_PAGEFRAME + 0x1000u)) {
        uint16_t page = (unsigned int)(segment - EMM_PAGEFRAME) /
                        (unsigned int)(EMM_PAGE_SIZE >> 4u);
        mem_writew(data, segment);
        data += 2;
        MEM_BlockWrite(data, &emm_mappings[page], sizeof(EMM_Mapping));
        data += sizeof(EMM_Mapping);
      } else if ((ems_type == EMS_MIXED) || (ems_type == EMS_EMM386) ||
                 ((segment >= EMM_PAGEFRAME - 0x1000) &&
                  (segment < EMM_PAGEFRAME)) ||
                 ((segment >= 0xa000) && (segment < 0xb000))) {
        mem_writew(data, segment);
        data += 2;
        MEM_BlockWrite(data, &emm_segmentmappings[segment >> 10u],
                       sizeof(EMM_Mapping));
        data += sizeof(EMM_Mapping);
      } else {
        return EMM_ILL_PHYS;
  ...
  case 0x01: /* Restore Partial Page Map */
    ...
    data += 2;
    for (; count > 0; count--) {
      uint16_t segment = mem_readw(data);
      data += 2;
      if ((segment >= EMM_PAGEFRAME) && (segment < EMM_PAGEFRAME + 0x1000)) {
        uint16_t page = (unsigned int)(segment - EMM_PAGEFRAME) /
                        (unsigned int)(EMM_PAGE_SIZE >> 4);
        MEM_BlockRead(data, &emm_mappings[page], sizeof(EMM_Mapping));
      } else if ((ems_type == EMS_MIXED) || (ems_type == EMS_EMM386) ||
                 ((segment >= EMM_PAGEFRAME - 0x1000) &&
                  (segment < EMM_PAGEFRAME)) ||
                 ((segment >= 0xa000) && (segment < 0xb000))) {
        MEM_BlockRead(data, &emm_segmentmappings[segment >> 10u],
                      sizeof(EMM_Mapping));
      } else {
        return EMM_ILL_PHYS;
      }
  ...
  case 0x02: /* Get Partial Page Map Array Size */
    ...
    break;
  default:
    LOG(LOG_MISC, LOG_ERROR)("EMS:Call %2X Subfunction %2X not supported",
                             reg_ah, reg_al);
...
static uint8_t MemoryRegion(void) {
  ...
  if (!region.src_type) {
    ...
    if (!ValidHandle(region.src_handle))
      return EMM_INVALID_HANDLE;
    if ((emm_handles[region.src_handle].pages * EMM_PAGE_SIZE) <
        ((region.src_page_seg * EMM_PAGE_SIZE) + region.src_offset +
         region.bytes))
      return EMM_LOG_OUT_RANGE;
    src_handle = emm_handles[region.src_handle].mem;
    PhysPt pages =
        region.src_page_seg * 4u + (region.src_offset / MEM_PAGE_SIZE);
    for (; pages > 0; pages--)
      src_handle = MEM_NextHandle(src_handle);
  ...
  if (!region.dest_type) {
    ...
    if (!ValidHandle(region.dest_handle))
      return EMM_INVALID_HANDLE;
    if (emm_handles[region.dest_handle].pages * EMM_PAGE_SIZE <
        (region.dest_page_seg * EMM_PAGE_SIZE) + region.dest_offset +
            region.bytes)
      return EMM_LOG_OUT_RANGE;
    dest_handle = emm_handles[region.dest_handle].mem;
    PhysPt pages =
        region.dest_page_seg * 4u + (region.dest_offset / MEM_PAGE_SIZE);
    for (; pages > 0; pages--)
      dest_handle = MEM_NextHandle(dest_handle);
  ...
  while (region.bytes > 0) {
    ...
    if (!region.src_type) {
      ...
      if (toread < src_remain) {
        MEM_BlockRead(
            ((unsigned long)src_handle * (unsigned long)MEM_PAGE_SIZE) +
                src_off,
            buf_src, toread);
      } else {
        MEM_BlockRead(
            ((unsigned long)src_handle * (unsigned long)MEM_PAGE_SIZE) +
                src_off,
            buf_src, src_remain);
        MEM_BlockRead(((unsigned long)MEM_NextHandle(src_handle) *
                       (unsigned long)MEM_PAGE_SIZE),
                      &buf_src[src_remain], toread - src_remain);
      }
    }
    /* Check for a move */
    if (reg_al == 1) {
      ...
      if (!region.dest_type) {
        ...
        if (toread < dest_remain) {
          MEM_BlockRead(
              ((unsigned long)dest_handle * (unsigned long)MEM_PAGE_SIZE) +
                  dest_off,
              buf_dest, toread);
        } else {
          MEM_BlockRead(
              ((unsigned long)dest_handle * (unsigned long)MEM_PAGE_SIZE) +
                  dest_off,
              buf_dest, dest_remain);
          MEM_BlockRead(((unsigned long)MEM_NextHandle(dest_handle) *
                         (unsigned long)MEM_PAGE_SIZE),
                        &buf_dest[dest_remain], toread - dest_remain);
        }
      }
      /* Write to the source */
      if (!region.src_type) {
        ...
        if (toread < src_remain) {
          ...
          MEM_BlockWrite(
              ((unsigned long)src_handle * (unsigned long)MEM_PAGE_SIZE) +
                  src_off,
              buf_dest, src_remain);
          MEM_BlockWrite(((unsigned long)MEM_NextHandle(src_handle) *
                          (unsigned long)MEM_PAGE_SIZE),
                         &buf_dest[src_remain], toread - src_remain);
        }
      }
    }
      ...
      if (toread < dest_remain) {
        ...
        MEM_BlockWrite(
            ((unsigned long)dest_handle * (unsigned long)MEM_PAGE_SIZE) +
                dest_off,
            buf_src, dest_remain);
        MEM_BlockWrite(((unsigned long)MEM_NextHandle(dest_handle) *
                        (unsigned long)MEM_PAGE_SIZE),
                       &buf_src[dest_remain], toread - dest_remain);
      }
    }
    /* Advance the pointers */
    if (!region.src_type)
      src_mem += (PhysPt)toread;
    else
      src_handle = MEM_NextHandle(src_handle);
    if (!region.dest_type)
      dest_mem += (PhysPt)toread;
    else
      dest_handle = MEM_NextHandle(dest_handle);
...
static Bitu INT67_Handler(void) {
  ...
  case 0x4b: /* Get Handle Count */
    reg_bx = 0;
    for (unsigned int i = 0; i < EMM_MAX_HANDLES; i++)
      if (emm_handles[i].pages != NULL_HANDLE)
        reg_bx++;
    reg_ah = EMM_NO_ERROR;
    break;
  ...
  case 0x50: /* Map/Unmap multiple handle pages */
    ...
    case 0x00: // use physical page numbers
    {
      ...
      for (int i = 0; i < reg_cx; i++) {
        uint16_t logPage = mem_readw(data);
        data += 2;
        uint16_t physPage = mem_readw(data);
        data += 2;
        reg_ah = EMM_MapPage(physPage, reg_dx, logPage);
        if (reg_ah != EMM_NO_ERROR)
          break;
      }
    } break;
  ...
  case 0x57: /* Memory region */
    reg_ah = MemoryRegion();
    if (reg_ah)
      LOG(LOG_MISC, LOG_ERROR)("EMS:Function 57 move failed");
    break;
  ...
  case 0x59: // Get hardware information
    ...
    case 0x01: // get unallocated raw page count
      reg_dx = (uint16_t)(MEM_TotalPages() / 4); // Not entirely correct but
                                                 // okay
      reg_bx = EMM_GetFreePages();
      break;
    default:
  ...
  case 0x70: /* NEC PC-98 specific function group? */
    if (reg_al == 1) {
      if (IS_PC98_ARCH) {
        ...
        /* NTS: Neither one seems to remap segment B0000h as far as I can tell,
         * so implementing this will be tricky and full of guesswork. */
        /* See also [http://www.koizuka.jp/~koizuka/master.lib/MASTER.MAN.txt],
         * section "ems_enablepageframe - NEC EMSページフレームバンクの操作" */
        if (reg_bx <= 1) {
          LOG(LOG_MISC, LOG_DEBUG)(
              "EMS:Call 70 subfct %2X remapping EMS page frame at B000h to %s "
              "not yet implemented. Hope your DOS application does not rely on "
              "that.",
              reg_al, reg_bx == 1 ? "system memory" : "video memory");
        } else {
          LOG(LOG_MISC, LOG_DEBUG)(
              "EMS:Call 70 subfct %2X given invalid value BX=%04x which will "
              "likely do nothing (master library EMS_ENABLEPAGEFRAME bug)",
              reg_al, reg_bx);
        }
        ...
        LOG(LOG_MISC, LOG_DEBUG)(
            "EMS:Call 70 subfct %2X not supported outside PC-98 mode", reg_al);
        reg_ah = EMM_INVALID_SUB;
      }
    } else {
  ...
  case 0xDE: /* VCPI Functions */
    if (!vcpi.enabled) {
      ...
      case 0x01: { /* VCPI Get Protected Mode Interface */
        ...
        for (ct = 0; ct < 0xff; ct++) {
          real_writeb(SegValue(es), reg_di + ct * 4 + 0x00,
                      0x67); // access bits
          real_writew(SegValue(es), reg_di + ct * 4 + 0x01,
                      ct * 0x10); // mapping
        ...
        for (ct = 0; ct < 4; ct++) {
          ...
          if (handle != 0xffff) {
            uint16_t memh = (uint16_t)MEM_NextHandleAt(
                emm_handles[handle].mem,
                (unsigned int)emm_mappings[ct].page * 4u);
            uint16_t entry_addr = (unsigned int)reg_di +
                                  (unsigned int)(EMM_PAGEFRAME >> 6u) +
                                  (unsigned int)(ct * 0x10u);
            real_writew(SegValue(es), entry_addr + 0x00u + 0x01u,
                        (memh + 0u) * 0x10u); // mapping of 1/4 of page
            real_writew(SegValue(es), entry_addr + 0x04u + 0x01u,
                        (memh + 1u) * 0x10u); // mapping of 2/4 of page
            real_writew(SegValue(es), entry_addr + 0x08u + 0x01u,
                        (memh + 2u) * 0x10u); // mapping of 3/4 of page
            real_writew(SegValue(es), entry_addr + 0x0cu + 0x01u,
                        (memh + 3u) * 0x10u); // mapping of 4/4 of page
          }
        }
        reg_di += 0x400; // advance pointer by 0x100*4

        /* Set up three descriptor table entries, for the VCPI server, which
         * must be 32-bit! */
      ...
      case 0x06: { /* VCPI Get Physical Address of Page in 1st MB */
        if (((unsigned int)(reg_cx << 8u) >= (unsigned int)EMM_PAGEFRAME) &&
            ((unsigned int)(reg_cx << 8u) <
             (unsigned int)EMM_PAGEFRAME + 0x1000u)) {
          ...
          if (handle == 0xffff) {
            ...
            MemHandle memh = MEM_NextHandleAt(
                emm_handles[handle].mem, emm_mappings[phys_page].page * 4u);
      ...
      case 0x0c: { /* VCPI Switch from V86 to Protected Mode */
        ...
        if (CPU_LLDT(new_ldt))
          LOG_MSG("VCPI:Could not load LDT with %x", new_ldt);
        if (CPU_LTR(new_tr))
          LOG_MSG("VCPI:Could not load TR with %x", new_tr);
...
static Bitu VCPI_PM_Handler() {
  ...
  case 0xDE0C: { /* VCPI Switch from Protected Mode to V86 */
    ...
    PhysPt tbaddr =
        (PhysPt)vcpi.private_area + 0x0000u + (0x10u & 0xfff8u) + 5u;
    uint8_t tb = mem_readb(tbaddr);
    mem_writeb(tbaddr, tb & 0xfd);
    ...
    if (CPU_LLDT(0x08))
      LOG_MSG("VCPI:Could not load LDT");
    if (CPU_LTR(0x10))
      LOG_MSG("VCPI:Could not load TR");
...
static Bitu V86_Monitor() {
  ...
  Bitu int_num =
      ((unsigned int)mem_readw(SegPhys(ss) + ((unsigned int)reg_esp &
                                              (unsigned int)cpu.stack.mask)) -
       0x2803u);
  ...
  if ((int_num == (0x0d * 4)) && ((reg_sp & 0xffff) != 0x1fda)) {
    ...
    uint8_t v86_opcode =
        mem_readb(((unsigned int)v86_cs << 4u) + (unsigned int)v86_ip);
    //		LOG_MSG("v86 monitor caught protection violation at %x:%x,
    //opcode=%x",v86_cs,v86_ip,v86_opcode);
    switch (v86_opcode) {
    case 0x0f: // double byte opcode
      v86_opcode =
          mem_readb((unsigned int)(v86_cs << 4u) + (unsigned int)v86_ip + 1u);
      switch (v86_opcode) {
      case 0x20: { // mov reg,CRx
        Bitu rm_val =
            mem_readb((unsigned int)(v86_cs << 4u) + (unsigned int)v86_ip + 2u);
        Bitu which = (unsigned int)(rm_val >> 3u) & 7u;
        if ((rm_val < 0xc0) || (rm_val >= 0xe8))
          E_Exit("Invalid opcode 0x0f 0x20 %x caused a protection fault!",
                 static_cast<unsigned int>(rm_val));
        ...
        mem_writew(SegPhys(ss) + ((reg_esp + 0u) & cpu.stack.mask),
                   (unsigned int)v86_ip + 3u);
      } break;
      case 0x22: { // mov CRx,reg
        ...
        if ((rm_val < 0xc0) || (rm_val >= 0xe8))
          E_Exit("Invalid opcode 0x0f 0x22 %x caused a protection fault!",
                 static_cast<unsigned int>(rm_val));
        uint32_t crx = 0;
        switch (rm_val & 7) {
        case 0:
      ...
      default:
        E_Exit("Unhandled opcode 0x0f %x caused a protection fault!",
               v86_opcode);
      }
      break;
    case 0xe4: // IN AL,Ib
      if (!VCPI_trapio_r(mem_readb((unsigned int)(v86_cs << 4u) +
                                   (unsigned int)v86_ip + 1u),
                         1))
        reg_al = IO_ReadB(mem_readb((unsigned int)(v86_cs << 4) +
                                    (unsigned int)v86_ip + 1)) &
                 0xff;
    ...
    case 0xe5: // IN AX,Ib
      if (!VCPI_trapio_r(mem_readb((unsigned int)(v86_cs << 4u) +
                                   (unsigned int)v86_ip + 1u),
                         2))
        reg_ax = IO_ReadW(mem_readb((unsigned int)(v86_cs << 4) +
                                    (unsigned int)v86_ip + 1)) &
                 0xffff;
      mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 2u);
      break;
    case 0xe6: // OUT Ib,AL
      if (!VCPI_trapio_w(mem_readb((unsigned int)(v86_cs << 4u) +
                                   (unsigned int)v86_ip + 1u),
                         reg_al, 1))
        IO_WriteB(
            mem_readb((unsigned int)(v86_cs << 4) + (unsigned int)v86_ip + 1),
            reg_al);
      mem_writew(SegPhys(ss) + ((reg_esp + 0) & cpu.stack.mask), v86_ip + 2u);
      break;
    case 0xe7: // OUT Ib,AX
    ...
    case 0xec: // IN AL,DX
      ...
      mem_writew(SegPhys(ss) + ((reg_esp + 0) & (unsigned int)cpu.stack.mask),
                 (unsigned int)v86_ip + 1u);
      break;
    case 0xed: // IN AX,DX
  ...
  if (reg_sp != 0x1fdau)
    reg_esp += (2u + 3u * 4u); // Interrupt from within protected mode
  else
    reg_esp += 2;
  ...
  uint16_t return_cs =
      mem_readw(SegPhys(ss) + ((reg_esp + 4u) & cpu.stack.mask));
  uint32_t return_eflags =
      mem_readd(SegPhys(ss) + ((reg_esp + 8u) & cpu.stack.mask));
  ...
  mem_writed(SegPhys(ss) + ((reg_esp + 8u) & cpu.stack.mask),
             return_eflags & (~(FLAG_IF | FLAG_TF)));

  /* Adjust SP of v86-stack */
  uint16_t v86_ss =
      mem_readw(SegPhys(ss) + ((reg_esp + 0x10u) & cpu.stack.mask));
  uint16_t v86_sp =
      mem_readw(SegPhys(ss) + ((reg_esp + 0x0cu) & cpu.stack.mask)) - 6u;
  mem_writew(SegPhys(ss) + ((reg_esp + 0x0cu) & cpu.stack.mask), v86_sp);

  /* Return to original code after v86-interrupt handler */
  mem_writew((unsigned int)(v86_ss << 4u) + (unsigned int)v86_sp + 0u,
             return_ip);
  mem_writew((unsigned int)(v86_ss << 4u) + (unsigned int)v86_sp + 2u,
             return_cs);
  mem_writew((unsigned int)(v86_ss << 4u) + (unsigned int)v86_sp + 4u,
             (uint16_t)(return_eflags & 0xffffu));
  return CBRET_NONE;
}
...
inline void VCPI_iopermw(uint16_t port, bool set) {
  ...
  b = mem_readb((unsigned int)vcpi.private_area + 0x3000u + 0x68u +
                ((unsigned int)port >> 3u));
  if (set)
    b |= 1u << (port & 7u);
  else
    b &= ~(1u << (port & 7u));
  mem_writeb((unsigned int)vcpi.private_area + 0x3000u + 0x68u +
                 ((unsigned int)port >> 3u),
             b);
}
...
static void SetupVCPI() {
  ...
  if (((unsigned int)emm_handles[vcpi.ems_handle].mem << 12u) & (1u << 20u)) {
    LOG(LOG_MISC,
        LOG_DEBUG)("EMS:EMM OS handle is associated with memory on an odd "
                   "megabyte. Enabling A20 gate to avoid corrupting DOS state, "
                   "will restore A20 state after this setup phase.");
    XMS_EnableA20(true);
  }
  ...
  vcpi.private_area =
      (MemHandle)((unsigned int)emm_handles[vcpi.ems_handle].mem << 12u);

  /* GDT */
  mem_writed((unsigned int)vcpi.private_area + 0x0000,
             0x00000000); // descriptor 0
  mem_writed((unsigned int)vcpi.private_area + 0x0004,
             0x00000000); // descriptor 0

  uint32_t ldt_address = ((unsigned int)vcpi.private_area + 0x1000);
  uint16_t ldt_limit = 0xff;
  uint32_t ldt_desc_part =
      (((unsigned int)ldt_address & 0xffff) << 16u) | (unsigned int)ldt_limit;
  mem_writed((unsigned int)vcpi.private_area + 0x0008,
             (unsigned int)ldt_desc_part); // descriptor 1 (LDT)
  ldt_desc_part = (((unsigned int)ldt_address & 0xff0000) >> 16) |
                  ((unsigned int)ldt_address & 0xff000000) | 0x8200;
  mem_writed((unsigned int)vcpi.private_area + 0x000c,
             (unsigned int)ldt_desc_part); // descriptor 1

  uint32_t tss_address = ((unsigned int)vcpi.private_area + 0x3000);
  uint32_t tss_desc_part =
      (((unsigned int)tss_address & 0xffff) << 16u) | (0x0068 + 0x200);
  mem_writed((unsigned int)vcpi.private_area + 0x0010,
             (unsigned int)tss_desc_part); // descriptor 2 (TSS)
  tss_desc_part = (((unsigned int)tss_address & 0xff0000) >> 16) |
                  ((unsigned int)tss_address & 0xff000000) | 0x8900;
  mem_writed((unsigned int)vcpi.private_area + 0x0014,
             (unsigned int)tss_desc_part); // descriptor 2

  /* LDT */
  mem_writed((unsigned int)vcpi.private_area + 0x1000,
             0x00000000); // descriptor 0
  mem_writed((unsigned int)vcpi.private_area + 0x1004,
             0x00000000); // descriptor 0
  uint32_t cs_desc_part =
      (((unsigned int)vcpi.private_area & 0xffff) << 16u) | 0xffff;
  mem_writed((unsigned int)vcpi.private_area + 0x1008,
             (unsigned int)cs_desc_part); // descriptor 1 (code)
  cs_desc_part = (((unsigned int)vcpi.private_area & 0xff0000) >> 16u) |
                 ((unsigned int)vcpi.private_area & 0xff000000) | 0x9a00;
  mem_writed((unsigned int)vcpi.private_area + 0x100c,
             (unsigned int)cs_desc_part); // descriptor 1
  uint32_t ds_desc_part =
      (((unsigned int)vcpi.private_area & 0xffff) << 16u) | 0xffff;
  mem_writed((unsigned int)vcpi.private_area + 0x1010,
             (unsigned int)ds_desc_part); // descriptor 2 (data)
  ds_desc_part = (((unsigned int)vcpi.private_area & 0xff0000) >> 16u) |
                 ((unsigned int)vcpi.private_area & 0xff000000) | 0x9200;
  mem_writed((unsigned int)vcpi.private_area + 0x1014,
             (unsigned int)ds_desc_part); // descriptor 2
  ...
  for (uint16_t int_ct = 0; int_ct < 0x100; int_ct++) {
    ...
    mem_writeb((unsigned int)vcpi.private_area + 0x2800 +
                   (unsigned int)int_ct * 4u + 0,
               0xe8); // call
    mem_writew((unsigned int)vcpi.private_area + 0x2800 +
                   (unsigned int)int_ct * 4u + 1,
               0x05fd - ((unsigned int)int_ct * 4u));
    mem_writeb((unsigned int)vcpi.private_area + 0x2800 +
                   (unsigned int)int_ct * 4u + 3,
               0xcf); // iret (dummy)

    /* put a Gate-Descriptor into the IDT */
    mem_writed((unsigned int)vcpi.private_area + 0x2000 +
                   (unsigned int)int_ct * 8u + 0,
               0x000c0000 | (0x2800 + (unsigned int)int_ct * 4u));
    mem_writed((unsigned int)vcpi.private_area + 0x2000 +
                   (unsigned int)int_ct * 8u + 4,
               0x0000ee00);
  }

  /* TSS */
  for (Bitu tse_ct = 0; tse_ct < 0x68 + 0x2000 /*all 65536 I/O ports*/;
       tse_ct++) {
    /* clear the TSS as most entries are not used here */
    mem_writeb((unsigned int)vcpi.private_area + 0x3000 + (unsigned int)tse_ct,
               0);
  }
  ...
  mem_writed((unsigned int)vcpi.private_area + 0x3066,
             0x0068); // io-map base (map follows, all zero)
...
public:
  ...
  EMS(Section *configuration) : Module_base(configuration) {
    ...
    ems_syshandle_on_even_mb =
        section->Get_bool("ems system handle on even megabyte");
    ...
    LOG_MSG("EMS page frame at 0x%04x-0x%04x", EMM_PAGEFRAME,
            EMM_PAGEFRAME + 0xFFF);
    ...
    if (CPU_ArchitectureType < CPU_ARCHTYPE_386 && ems_type != EMS_BOARD) {
      LOG_MSG("CPU is 286 or lower, setting EMS emulation to ems=emsboard and "
              "disabling VCPI and v86 startup");
     ...
     * Real MS-DOS EMM386.EXE will flat out refuse to load if HIMEM.SYS is not
     * loaded. that also prevents VCPI from working. */
    if (!XMS_Active() && ems_type != EMS_BOARD) {
      if (ems_type == EMS_MIXED) {
        LOG_MSG("EMS changed to board mode and VCPI disabled, because XMS is "
                "not enabled.");
        ems_type = EMS_BOARD;
        ...
        setup_EMS_none();
        ems_type = EMS_NONE;
        LOG_MSG("EMS disabled, EMM386 emulation is impossible when XMS is not "
                "enabled");
        return;
      }
    ...
    dbg_zero_on_ems_allocmem =
        section->Get_bool("zero memory on ems memory allocation");
    if (dbg_zero_on_ems_allocmem) {
      LOG(LOG_MISC,
          LOG_DEBUG)("Debug option enabled: EMS memory allocation will always "
    ...
    if (ENABLE_V86_STARTUP) {
      LOG(LOG_MISC, LOG_WARN)(
          "EMM386 virtual 8086 monitor is not stable! Use with caution!");
    }

    if (ems_type == EMS_BOARD && ENABLE_VCPI) {
      LOG_MSG("VCPI emulation is incompatible with ems=board. Turning off VCPI "
              "emulation");
      ENABLE_VCPI = false;
    }
    if (ems_type != EMS_EMM386 && ENABLE_V86_STARTUP) {
      /* starting up in virtual 8086 mode makes no sense unless emulating
       * EMM386.EXE */
      if (ems_type != EMS_MIXED) {
        /* v86 startup is default. to avoid yelling at the user do not print
         * anything unless the user set it to any value other than the default
         * ems=true */
        LOG_MSG("EMS EMM386.EXE v86 mode is incompatible with ems= setting. "
                "Starting up in real mode.");
      }
      ENABLE_V86_STARTUP = false;
    }
    if (ENABLE_V86_STARTUP && !ENABLE_VCPI) {
      LOG_MSG("EMS: DOSBox-X does not support enabling virtual 8086 mode "
              "without VCPI.");
      ENABLE_V86_STARTUP = false;
    }

    /* TODO: Even in "board" mode EMS emulation is highly dependent on having
     * extended memory to map! If we truly wanted to emulate an EMS board, we'd
     * allocate our own private memory and just map parts of it into the EMS
     * page frame, which would allow emulation to work even if XMS is disabled
     * and/or less than 1MB conventional memory is configured! */
    if (ems_type == EMS_BOARD) {
      if (MEM_TotalPages() <= 0x110) {
        // FIXME: Board emulation should allocate it's own memory and map it
        // into the EMS page frame like a device would,
        //        not use extended memory!
        LOG_MSG("EMS: EMS board mode as implemented still requires extended "
                "memory to work (we'll fix this someday).");
        LOG_MSG("At least 1MB of memory must be configured for EMS emulation "
                "to work.");
        LOG_MSG("Because of this limitation, EMS emulation will run as if no "
                "memory available.");
      }
    }

    oshandle_memsize_16kb =
        (unsigned int)section->Get_int("ems system handle memory size");
    /* convert KB to 16KB pages */
    oshandle_memsize_16kb = (oshandle_memsize_16kb + 15u) / 16u;
    if (oshandle_memsize_16kb == 0)
      oshandle_memsize_16kb = 1;

    ems_baseseg = DOS_GetMemory(2, "ems_baseseg"); // We have 32 bytes

    /* Add a little hack so it appears that there is an actual ems device
     * installed */
    char const *emsname = "EMMXXXX0";
    MEM_BlockWrite(PhysMake(ems_baseseg, 0xa), emsname,
                   (Bitu)(strlen(emsname) + 1));

    call_int67 = CALLBACK_Allocate();
    CALLBACK_Setup(call_int67, &INT67_Handler, CB_IRET,
                   PhysMake(ems_baseseg, 4), "Int 67 ems");
    ...
    if (ems_type == EMS_BOARD) {
      ...
      if (EMM_AllocateSystemHandle(oshandle_memsize_16kb) !=
          EMM_NO_ERROR) { // allocate OS-dedicated handle (ems handle zero,
                          // 384kb)
    ...
    if (ENABLE_VCPI &&
        oshandle_memsize_16kb < (0x4000 / 16384)) { /* at least 16KB */
      LOG_MSG(
          "EMS:System handle memory size too small (<16KB), disabling VCPI");
      ENABLE_VCPI = false;
    }
    ...
    if (ENABLE_VCPI) {
      ...
      /* Install a callback that handles VCPI-requests in protected mode
       * requests */
      ...
      call_win_vm86_ctl.Install(&WinVM86Ctl, CB_RETF,
                                "VM86 mode Windows control");
...

      {
        ...
        mem_writeb((unsigned int)vcpi.private_area + 0x2e00,
                   (uint8_t)0xFE); // GRP 4
        mem_writeb((unsigned int)vcpi.private_area + 0x2e01,
                   (uint8_t)0x38); // Extra Callback instruction
        mem_writew((unsigned int)vcpi.private_area + 0x2e02,
                   call_v86mon.Get_callback()); // The immediate word
        mem_writeb((unsigned int)vcpi.private_area + 0x2e04, (uint8_t)0x66);
        mem_writeb((unsigned int)vcpi.private_area + 0x2e05,
                   (uint8_t)0xCF); // A IRETD Instruction

        XMS_EnableA20(old_a20 != 0);
      }

      /* DOSBox's default EMS emulation provides the EMS memory mapping but
       * without the virtual 8086 mode. But there are DOS games and demos that
       * assume EMM386.EXE == virtual 8086 mode and will fail to detect EMS if
       * the v86 bit in EFLAGS is not set.
       *
       * Examples:
       *   ftp://ftp.scene.org/pub/parties/1994/3s94/demo/friends.zip
       *     - Refuses to run unless it detects Expanded memory
       *     - Does not detect Expanded memory because it doesn't detect virtual
       * 8086 mode
       *     - Therefore, without this option, it is impossible to run the demo.
       */
      if (ENABLE_V86_STARTUP) {
        LOG(LOG_MISC, LOG_DEBUG)("EMS: Now setting up the DOS environment to "
                                 "run in EMM386.EXE virtual 8086 mode");

        /* our V86 state may require A20 enabled to run. the EMM OS handle
         * often resides on an odd megabyte */
        if (((unsigned int)emm_handles[vcpi.ems_handle].mem << 12u) &
            (1u << 20u)) {
          LOG(LOG_MISC, LOG_DEBUG)(
              "EMS:EMM OS handle is associated with memory on an odd megabyte. "
              "Enabling A20 gate to safely enter V86 mode.");
          XMS_EnableA20(true);
        }
...
void EMS_PickPageFrame(void) {
  if (desired_ems_segment == 0)
    Update_Get_Desired_Segment();

  /* the EMS page frame needs to move depending on IBM PC or PC-98 emulation.
   * IBM PC emulation can put the page frame at 0xE000 (as DOSBox has always
   * done). PC-98 emulation needs to move the page frame down because 0xE000 is
   * taken by the 4th EGC bitplane. J-3100 emulation needs to move the page
   * frame down because  0xE000 is used in Kanji ROM. */
...
void EMS_DOSBoot(Section * /*sec*/) { EMS_PickPageFrame(); }

void EMS_ShutDown(Section * /*sec*/) { EMS_DoShutDown(); }
...
void EMS_Init() {
  ...
  AddVMEventFunction(VM_EVENT_DOS_BOOT,
                     AddVMEventFunctionFuncPair(EMS_DOSBoot));
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(EMS_ShutDown));
}
