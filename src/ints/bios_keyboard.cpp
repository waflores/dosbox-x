 ...
 * In recent versions this can disabled by an environment variable which we set
 * in sdlmain.cpp Define the following if this is the case */
#if SDL_VERSION_ATLEAST(1, 2, 14)
#define CAN_USE_LOCK 1
/* For lower versions of SDL we also use a slight hack to get the startup states
 * of numclock and capslock right. The proper way is in the mapper, but the
 * repeating key is an unwanted side effect for lower versions of SDL */
#endif

extern Bitu BIOS_PC98_KEYBOARD_TRANSLATION_LOCATION;

static Bitu call_int16 = 0, call_irq1 = 0, irq1_ret_ctrlbreak_callback = 0,
            call_irq6 = 0, call_irq_pcjr_nmi = 0, call_int48_pcjr = 0;
...
static scancode_tbl scan_to_scanascii[MAX_SCAN_CODE + 1] = {
    ...
    {0x2960, 0x297e, none, 0x29f0, 0x29d1,
     0x29a3}, /* 29 "`" "~"  /  29h "`" "~" "ﾑ" "｣" */
    {none, none, none, none, none, none}, /* 2a L shift */
    {0x2b5c, 0x2b7c, 0x2b1c, 0x2bf0, 0x2bb0,
     0x2bf0}, /* 2b "*" "/"  /  2bh "\\" "|" "ｰ" */
    ...
    //      normal, shift , ctrl  , alt    ,kana  , kana_shift
    {0x565c, 0x567c, none, none, 0x56db, 0x56f0},
    /* 56 (102-key), \_ロ| for AX */                  // 56h \|ﾛ underscore
    {0x8500, 0x8700, 0x8900, 0x8b00, 0x8500, 0x8700}, /* 57 F11 */
    {0x8600, 0x8800, 0x8a00, 0x8c00, 0x8600, 0x8800}, /* 58 F12 */
    {0xab00, 0xac00, 0xad00, 0xae00, 0xab00,
     0xac00}, /* 5a 無変換 (non-conversion) for AX */
    {0xa700, 0xa800, 0xa900, 0xaa00, 0xa700,
     0xa800}, /* 5b 変換 (conversion) for AX */
    {0xd200, 0xd300, 0xd400, 0xd500, 0xd200,
     0xd300} /* 5c AX (additional function key) for AX */
};
...
static scancode_tbl scan_to_scanascii_pc98[0x80] = {
    ...
    {0x285d, 0x287d, 0x281d, none, 0x28d1, 0x28a3},
    /* 28 `~ */                                       // 29h `~ﾑ｣
...
void BIOSKEY_PC98_Write_Tables(void) {
  ...
  /* Assume this function will not be called unless
   * BIOS_PC98_KEYBOARD_TRANSLATION_LOCATION points to ROM BIOS. It's actually
   * not exactly a 1:1 mapping, the empty range between 0x56-0x61 is skipped
   * according to the tables on real hardware. On real hardware the tables are
   * noticeably 0x60 (not 0x80) bytes apart from each other. Special processing
   * is done for the shift state keys that do not involve the table. */

  /* [0] Normal */
  for (i = 0x00; i < 0x56; i++)
    phys_writeb(o + i, scan_to_scanascii_pc98[i].normal);
  for (i = 0x62; i < 0x6C; i++)
    phys_writeb(o + i,
                scan_to_scanascii_pc98[i - 0xC]
                    .normal); /* NTS: 0x62-0x0C = 0x56, 10 codes fill 0x56-0x5F.
                                 0x60-0x56 = 0x0A (10). 0x60 bytes total */
  o += 0x60;

  /* [1] Shift */
  for (i = 0x00; i < 0x56; i++)
    phys_writeb(o + i, scan_to_scanascii_pc98[i].shift);
  for (i = 0x62; i < 0x6C; i++)
    phys_writeb(o + i,
                scan_to_scanascii_pc98[i - 0xC]
                    .shift); /* NTS: 0x62-0x0C = 0x56, 10 codes fill 0x56-0x5F.
                                0x60-0x56 = 0x0A (10). 0x60 bytes total */
  o += 0x60;
...
bool BIOS_AddKeyToBuffer(uint16_t code) {
  if (!IS_PC98_ARCH) {
    if (mem_readb(BIOS_KEYBOARD_FLAGS2) & 8)
      return true;
  }
  ...
  // TODO Maybe beeeeeeep or something although that should happened when
...
bool INT16_get_key(uint16_t &code) { return get_key(code); }
...
void trimKana(void) {
  ...
  if (kana_status & 0x02) { // Kana locked
    ...
    mem_writeb(BIOS_KEYBOARD_AX_KBDSTATUS,
               0xfc & kana_status); // Kana status LED=on and Shift=on
  } else {
    LOG(LOG_KEYBOARD, LOG_NORMAL)("Kana shift ON");
    mem_writeb(BIOS_KEYBOARD_AX_KBDSTATUS,
               0x03 | kana_status); // Kana status LED=on and Shift=on
  }
}

bool INT16_peek_key(uint16_t &code) { return check_key(code); }
...
static Bitu IRQ1_Handler(void) {
  ...
  /* No hack anymore! */
#else
  flags2 &= ~(0x40 + 0x20); // remove numlock/capslock pressed (hack for sdl
                            // only reporting states)
#endif
  if (DOS_LayoutKey(scancode, flags1, flags2, flags3))
    return CBRET_NONE;
  ...
  case 0x9d: /* Ctrl Released */
    if (INT16_AX_GetKBDBIOSMode() == 0x51 && (flags3 & 0x02))
      break; // for AX: Skip if AX KBDBIOS is in JP mode (for AX)
    if (!(flags3 & 0x01)) {
      if (flags3 & 0x02)
        flags3 &= ~0x04;
      else
  ...
  case 0x38: /* Alt Pressed */
    ...
    if (INT16_AX_GetKBDBIOSMode() == 0x51 &&
        (flags3 &
         0x02)) { // for AX: Is AX KBDBIOS in JP mode AND R-Alt pressed?
  ...
  case 0x46: /* Scroll Lock or Ctrl-Break */
     ...
     */
    if ((flags3 & 0x02) ||
        (!(flags3 & 0x10) && (flags1 & 0x04))) { /* Ctrl-Break? */
      ctrlbrk = true;
      /* remove 0xe0-prefix */
      flags3 &= ~0x02;
      mem_writeb(BIOS_KEYBOARD_FLAGS3, flags3);
      ...
      /* NTS: Real hardware shows that, at least through INT 16h, CTRL+BREAK
       * injects scan code word 0x0000 into the keyboard buffer. Upon
       * CTRL+BREAK, 0x0000 appears in the keyboard buffer, which INT 16h
       * AH=1h/AH=11h will signal as an available scan code, and INT 16h
       * AH=0h/10h will return with ZF=0. */
      ...
      flags2 |= 0x10; /* Scroll Lock SDL Seems to do this one fine (so break and
                         make codes) */
    }
    break;
  case 0xc6:
    if ((flags3 & 0x02) ||
        (!(flags3 & 0x10) && (flags1 & 0x04))) { /* Ctrl-Break released? */
                                                 /* nothing to do */
    } else {
      flags1 ^= 0x10;
      flags2 &= ~0x10;
      leds ^= 0x01;
      break; /* Scroll Lock released */
    }
    break;
  case 0xd2: /* NUMPAD insert, ironically, regular one is handled by 0x52 */
    if (flags3 & BIOS_KEYBOARD_FLAGS3_HIDDEN_E0 ||
  ...
  case 0x53: /* del . Not entirely correct, but works fine */
    if (scancode == 0x53 && !(flags3 & 0x01) && !(flags1 & 0x03) &&
        (flags1 & 0x0c) == 0x0c &&
        ((!(flags3 & 0x10) && (flags3 & 0x0c) == 0x0c) ||
         ((flags3 & 0x10) && (flags2 & 0x03) == 0x03))) { /* Ctrl-Alt-Del? */
      throw int(3);
    }
    if (flags3 & 0x02) { /*extend key. e.g key above arrows or arrows*/
      ...
      } else if (((flags1 & 0x3) != 0) ||
                 ((flags1 & 0x20) !=
                  0)) { // Due to |0xe0 results are identical.
    ...
    if (flags1 & 0x08) {
      ...
      add_key(scan_to_scanascii[scancode].control);
               ...
               ((flags1 & 0x20) !=
                0)) { // Xor shift and numlock (both means off)
  ...
  normal_key:
    ...
    if (flags1 & 0x08) { /* Alt is being pressed */
      ...
      asciiscan = scan_to_scanascii[scancode].control;
               ...
               (kanafl &
                0x02)) { // for AX: Kana is active + Shift is being pressed
...
irq1_end:
  ...
  if (leds_orig != leds)
    KEYBOARD_SetLEDs(leds);

  /* update insert cursor */
  /* FIXME: Wait a second... I doubt the BIOS IRQ1 handler does this! The
   * program (or DOS prompt) decides whether INS changes cursor shape! */
  extern bool dos_program_running;
  if (!dos_program_running) {
    /* As long as this hack exists... maintain the cursor's invisible state if
     * invisible so it does not reappear unexpectedly when the user presses
     * Insert. Though long term, this should be handled by the shell, not the
     * keyboard ISR */
    const auto invisible =
        static_cast<bool>(real_readw(BIOSMEM_SEG, BIOSMEM_CURSOR_TYPE) &
                          0x2000); /* NTS: Written (last|(first<<8)) and (first
                                      & 0x20) means invisible */
    const auto flg = mem_readb(BIOS_KEYBOARD_FLAGS1);
    const auto ins =
        static_cast<bool>(flg & BIOS_KEYBOARD_FLAGS1_INSERT_ACTIVE);
    const auto ssl = static_cast<uint8_t>(ins ? CURSOR_SCAN_LINE_INSERT
                                              : CURSOR_SCAN_LINE_NORMAL);
    if (CurMode->type == M_TEXT && inshell) {
      if (machine == MCH_MDA || machine == MCH_HERC)
        INT10_SetCursorShape((ssl + 6) | (invisible ? 0x20 : 0x00),
                             CURSOR_SCAN_LINE_END + 6); /* 14 - 8 = 6 */
      else
        INT10_SetCursorShape(ssl | (invisible ? 0x20 : 0x00),
                             CURSOR_SCAN_LINE_END);
    }
  }
 ...
 * - / 口       --          0x335F      0x331F  --      0x33DB  0x33DB Kana+CTRL
 * = 0x331F
 */
static Bitu IRQ1_Handler_PC98(void) {
  ...
  /* NTS: CWSDPMI (the PC-98 patched version) has an IRQ 1 interrupt handler
   * that reads the keyboard scan code byte ahead of calling this ISR. If we
   * check status first, this ISR will never process keyboard input because
   * RxRDY = 0 (CWSDPMI already read it!). This suggests that perhaps most PC-98
   * BIOSes assume a ready data byte on IRQ 1 and that's how CWSDPMI can get
   * away with doing that (TODO VERIFY THIS ON REAL HARDWARE) */
  do {
    ...
    /* Testing on real hardware shows INT 18h AH=0 returns raw scancode in upper
     * half, ASCII in lower half. Just like INT 16h on IBM PC hardware */
    uint16_t scan_add = sc_8251 << 8U;

    /* NOTES:
     *  - The bitmap also tracks CAPS, and KANA state. It does NOT track NUM
     * state. The bit corresponding to the key code will show a bit set if in
     * that state.
     *  - The STS byte returned by INT 18h AH=02h seems to match the 14th byte
     * of the bitmap... so far.
     *
    ...
    // bool caps_capitals = (modflags & 1) ^ ((modflags >> 1) & 1); /* CAPS XOR
    // SHIFT */ UNUSED

    /* According to Neko Project II, the BIOS maintains a "pressed key" bitmap
     * at 0x50:0x2A. Without this bitmap many PC-98 games are unplayable. */
    /* ALSO note that byte 0xE of this array is the "shift" state byte. */
    {
      ...
      /* mirror CTRL+GRAPH+KANA+CAPS+SHIFT at 0x53A which is returned by INT 18h
       * AH=2 */
      if (o == 0x538)
        mem_writeb(0x53A, c);
    }
    ...
    /* NOTES:
     *  - SHIFT/CTRL scan code changes (no scan code emitted if GRPH is held
     * down for these keys)
     *
     ...
     *          modifier keys. */
    //                  Keyboard layout and processing, copied from an actual
    //                  PC-98 keyboard
    //
        ...
        if (!pc98_force_ibm_layout) {
          if (shift) {
            if (!pc98_force_jis_layout) {
              if (scan_to_scanascii_pc98[sc_8251].shift)
                add_key(scan_to_scanascii_pc98[sc_8251].shift);
            } else {
              if (sc_8251 == 0x1a)
                add_key(scan_add + '`');
              else if (sc_8251 == 0x0c)
                add_key(scan_add + '~');
              else if (scan_to_scanascii_pc98[sc_8251].shift)
                add_key(scan_to_scanascii_pc98[sc_8251].shift);
            }
          } else {
            if (pc98_force_jis_layout && sc_8251 == 0x33)
              add_key(scan_add + '\\');
          ...
          case 0x0C: // pc98_force_ibm_layout
            if (shift)
              ...
              add_key(scan_add + '=');
            break;
          case 0x1A: // pc98_force_ibm_layout  key for '@'(0x1a) is reused in
                     // PC-98 mode
            if (shift)
              add_key(scan_add + '~');
            else if (!modflags) {
...
/* On PCjr the purpose of INT 48h normally is to translate PCjr scan codes to
 * 83-key scan codes and then call INT 9h with that scan code.
 *
 * Some 83-key codes can only be entered by holding Fn and typing another key,
 * and the raw scan codes reflect that. For compatibility with DOS programs, INT
 * 48h has to translate, for example, Fn + 1 into the scan code for F1. */
static Bitu PCjr_INT48_Keyboard_Handler(void) {
  ...
  if ((reg_al & 0x7F) == 0x54) {
    if (reg_al & 0x80 /*release*/)
      pcjr_f &= ~BIOS_KEYBOARD_PCJR_FLAG2_FN_FLAG;
    else
      pcjr_f |= BIOS_KEYBOARD_PCJR_FLAG2_FN_FLAG;
    goto skip_int9;
  }

#define UPDATESHIFT(x)                                                         \
  if (x) {                                                                     \
    flags1 &= ~0x3; /*break*/                                                  \
  } else {                                                                     \
    flags1 |= 0x3; /*make*/                                                    \
...
#define CLEARSHIFT() UPDATESHIFT(1 /*break*/)
  ...
  if (pcjr_f &
      (BIOS_KEYBOARD_PCJR_FLAG2_FN_FLAG | BIOS_KEYBOARD_PCJR_FLAG2_FN_LOCK)) {
    ...
    case 0x0B:
      ...
      break; /*Fn+10 = F10*/
    case 0x10:
      goto skip_int9; /*Fn+Q = Pause*/
    case 0x12:
      goto skip_int9; /*Fn+E = Echo*/
    case 0x19:
      goto skip_int9; /*Fn+P = Print Screen*/
    case 0x1A:
      reg_al = 0x2B | bc;
      UPDATESHIFT(bc);
      break; /*Fn+[ = | which is SHIFT+\ */
    case 0x1B:
      reg_al = 0x29 | bc;
      UPDATESHIFT(bc);
      break; /*Fn+] = ~ which is SHIFT+` */
    case 0x1F:
      reg_al = 0x46 | bc;
      break; /*Fn+S = Scroll lock*/
    case 0x28:
      reg_al = 0x29 | bc;
      CLEARSHIFT();
      break; /*Fn+' = ` which is unshifted ` */
    case 0x35:
      reg_al = 0x2B | bc;
      CLEARSHIFT();
      break; /*Fn+/ = \\ which is unshifted \\ */
    ...
    case 0x4B:
      reg_al = 0x49 | bc;
      break; /*Fn+Left = Page Up*/
    case 0x4D:
      reg_al = 0x51 | bc;
      break; /*Fn+Right = Page Down*/
    case 0x50:
      reg_al = 0x4F | bc;
      break; /*Fn+Down = End*/
    default:
...
#undef CLEARSHIFT
  ...
  IO_Write(0x60, reg_al); /* According to the PCjr BIOS listing, INT 48h does
                             "OUT KB_PORT,AL" before calling INT 9h */
  reg_eip++;              /* skip over IRET */
skip_int9: /* if we do not skip IRET, then INT 48h returns without calling INT
              9h */
...
static Bitu IRQ1_CtrlBreakAfterInt1B(void) { return CBRET_NONE; }
...
Bitu INT16_Handler(void) {
  ...
  case 0x11: /* CHECK FOR KEYSTROKE (enhanced keyboards only) */
    ...
    /* NOTE: The FreeDOS EDIT.COM editor built into DOSBox-X has a problem where
     * if this call return AX=0 and ZF=0, it will treat it the same as if we had
     *       returned ZF=1 to indicate no scan code. Unfortunately scan code
     * 0x0000 is added to the buffer for CTRL+BREAK, meaning that if you hit
     * CTRL+BREAK while using EDIT.COM, you effectively disable all keyboard
     * input to the program until you exit, or trick the program into reading
     * the scan code to remove it from the buffer.
     *
  ...
  case 0x03:              /* SET TYPEMATIC RATE AND DELAY */
    if (reg_al == 0x00) { // set default delay and rate
      ...
      LOG(LOG_BIOS, LOG_ERROR)("INT16:Unhandled Typematic Rate Call %2X BX=%X",
                               reg_al, reg_bx);
    }
    break;
  ...
  case 0x12: /* GET EXTENDED SHIFT STATES */
    reg_al = mem_readb(BIOS_KEYBOARD_FLAGS1);
    reg_ah =
        (mem_readb(BIOS_KEYBOARD_FLAGS2) & 0x73) |
        ((mem_readb(BIOS_KEYBOARD_FLAGS2) & 4) << 5) | // SysReq pressed, bit 7
        (mem_readb(BIOS_KEYBOARD_FLAGS3) &
         0x0c); // Right Ctrl/Alt pressed, bits 2,3
    break;
  case 0x13:
#if (defined(WIN32) && !defined(HX_DOS) || defined(MACOSX)) &&                 \
    !defined(C_SDL2) && defined(SDL_DOSBOX_X_SPECIAL)
#if defined(USE_TTF)
    if ((IS_DOSV || ttf_dosv) && IS_DOS_CJK &&
        (DOSV_GetFepCtrl() & DOSV_FEP_CTRL_IAS)) {
#else
  ...
  case 0x50: // Set/Get JP/US mode in KBD BIOS
    ...
    case 0x00:
      ...
      if (INT16_AX_SetKBDBIOSMode(reg_bx))
        reg_al = 0x00;
      else
        reg_al = 0x01;
      break;
  ...
  case 0xf6:
    ...
    if (J3_IsJapanese()) {
      ...
      size = mem_readw(BIOS_KEYBOARD_BUFFER_END) -
             mem_readw(BIOS_KEYBOARD_BUFFER_START);
      head = mem_readw(BIOS_KEYBOARD_BUFFER_HEAD);
      tail = mem_readw(BIOS_KEYBOARD_BUFFER_TAIL);
      reg_ax = 0;
      if (tail > head) {
...
void BIOS_SetupKeyboard(void) {
  ...
  if (IS_PC98_ARCH) {
    ...
    /* DO NOT set up an INT 16h vector. This exists only for the DOS CONIO
     * emulation. */
  } else {
    /* Allocate/setup a callback for int 0x16 and for standard IRQ 1 handler */
    call_int16 = CALLBACK_Allocate();
  ...
  if (machine == MCH_PCJR) { /* PCjr keyboard interrupt connected to NMI */
    ...
    /* NMI: Read bits from infared port to decode keyboard scan code. If valid,
     * pass it to INT 48h. INT 48h: Track Fn key and other state, convert PCjr
     * scan codes to 83-key compatible scan codes, pass it to INT 9h. INT 9h:
     * Process scan code same as you would on normal IBM PC hardware. */
    call_irq_pcjr_nmi = CALLBACK_Allocate();
    CALLBACK_Setup(call_irq_pcjr_nmi, &PCjr_NMI_Keyboard_Handler, CB_IRET,
                   "PCjr NMI Keyboard");
     ...
     * a+4 = iret (1 bytes)
     *
     * NTS: PCjr NMI doesn't read it from port 60h! But this makes it work in
     * this emulator! */
    phys_writeb(a + 5, 0x50);    /* push ax */
    phys_writeb(a + 6, 0x1e);    /* push ds */
    phys_writew(a + 7, 0xC0C7);  /* mov ax,0x0040    NTS: Do not use PUSH <imm>,
                                    that opcode does not exist on the 8086 */
    ...
    CALLBACK_Setup(call_int48_pcjr, &PCjr_INT48_Keyboard_Handler, CB_IRET,
  ...
  if (IS_PC98_ARCH) {
    CALLBACK_Setup(call_irq1, &IRQ1_Handler_PC98, CB_IRET_EOI_PIC1,
                   Real2Phys(BIOS_DEFAULT_IRQ1_LOCATION),
                   "IRQ 1 Keyboard PC-98");
    RealSetVec(0x09 /*IRQ 1*/, BIOS_DEFAULT_IRQ1_LOCATION);
  } else {
    CALLBACK_Setup(call_irq1, &IRQ1_Handler, CB_IRQ1,
                   Real2Phys(BIOS_DEFAULT_IRQ1_LOCATION), "IRQ 1 Keyboard");
    RealSetVec(0x09 /*IRQ 1*/, BIOS_DEFAULT_IRQ1_LOCATION);
  ...
  CALLBACK_Setup(irq1_ret_ctrlbreak_callback, &IRQ1_CtrlBreakAfterInt1B,
                 CB_IRQ1_BREAK, "IRQ 1 Ctrl-Break callback");
