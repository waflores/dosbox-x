...
void INT10_ToggleBlinkingBit(uint8_t state) {
  if (IS_VGA_ARCH) {
    uint8_t value;
    //	state&=0x01;
    if ((state > 1) && (svgaCard == SVGA_S3Trio))
      return;
    ResetACTL();
...
#if defined(USE_TTF)
  char value[30] = "";
  sprintf(value, "(%d,%d,%d)", red * 255 / 63, green * 255 / 63,
          blue * 255 / 63);
...
#if defined(USE_TTF)
      if (start == 16) {
        sprintf(value, "(%d,%d,%d)", (red << 2 | red >> 4),
                (green << 2 | green >> 4), (blue << 2 | blue >> 4));
...
void INT10_GetDACPage(uint8_t *mode, uint8_t *page) {
  ...
  *page = IO_Read(VGAREG_ACTL_READ_DATA);
  ...
  /* the operations carried out here blanked the display because of the index
   * (0x10/0x14) without bit 5, write a dummy index with bit 5 to re-enable the
   * display. Bugfix for "Blue Force" MS-DOS game.
   *
   * Note that DOSBox SVN had the same bug without this fix, but appeared to
   * work because the AC blanking didn't work. DOSBox SVN has a similar fix as
   * of commit r4297. */
...
void INT10_SetPelMask(uint8_t mask) { IO_Write(VGAREG_PEL_MASK, mask); }

void INT10_GetPelMask(uint8_t &mask) { mask = IO_Read(VGAREG_PEL_MASK); }
