...
static void lio_pset(short x, short y, uint8_t palette) {
  ...
  if (lio_draw.x1 > x || lio_draw.x2 < x || lio_draw.y1 > y ||
      lio_draw.y2 < y) {
    return;
  }
  addr = (y * 80) + (x >> 3);
...
static void lio_gline_sub(int x1, int y1, int x2, int y2, uint16_t ead,
                          uint8_t dad, int palette) {
  for (uint8_t i = 0; i < lio_draw.planes; i++) {
...
static void lio_gline(int xx1, int yy1, int xx2, int yy2, int palette,
                      uint16_t pattern) {
  int swap;
  int width;
  int height;
...
static void lio_glineb(int x1, int y1, int x2, int y2, int palette,
                       uint16_t pattern) {
...
static void lio_gbox(int px1, int py1, int px2, int py2, int palette,
                     uint8_t *tile, uint8_t length) {
  uint16_t pattern;
  uint8_t r;
  uint8_t *tterm;
  int temp;
  ...
  if (x1 > lio_draw.x2 || x2 < lio_draw.x1 || y1 > lio_draw.y2 ||
      y2 < lio_draw.y1) {
    return;
  }
...
static int mono_plane_palette[8][4] = {
    ...
    {0x77, 0x00, 0x77, 0x00}, {0x77, 0x77, 0x77, 0x00},
    {0x07, 0x07, 0x07, 0x07}, {0x77, 0x77, 0x07, 0x07},
    {0x77, 0x07, 0x77, 0x07}, {0x77, 0x77, 0x77, 0x07},
};
...
uint8_t PC98_BIOS_LIO_GSCREEN() {
  ...
  if (lio_palette_mode == 0 && (screen_mode == 1 || screen_mode == 2) &&
      plane < 8) {
...
uint8_t PC98_BIOS_LIO_GVIEW() {
  ...
  if (lio_work.view_x1 >= lio_work.view_x2 ||
      lio_work.view_y1 >= lio_work.view_y2) {
    return LIO_ILLEGALFUNC;
  }
  lio_updatedraw();
  palette = real_readb(seg, off + 8);
  if (palette != 0xff) {
    lio_gbox(lio_work.view_x1, lio_work.view_y1, lio_work.view_x2,
             lio_work.view_y2, palette, NULL, 0);
  }
  palette = real_readb(seg, off + 9);
  if (palette != 0xff) {
    lio_glineb(lio_work.view_x1, lio_work.view_y1, lio_work.view_x2,
               lio_work.view_y2, palette, 0xffff);
  }
  lio_write_parameter();
...
uint8_t PC98_BIOS_LIO_GCLS() {
  ...
  lio_gbox(lio_draw.x1, lio_draw.y1, lio_draw.x2, lio_draw.y2,
           lio_work.back_color, NULL, 0);
...
uint8_t PC98_BIOS_LIO_GLINE() {
  ...
  if (type < 2) {
    if (sw) {
      pattern = (byte_reverse(real_readb(seg, off + 0x0b)) << 8) |
                byte_reverse(real_readb(seg, off + 0x0c));
    }
    if (type == 0) {
    ...
    if (sw == 2) {
      ...
      MEM_BlockRead(
          PhysMake(real_readw(seg, off + 0x10), real_readw(seg, off + 0x0e)),
          tile, length);
    }
    if (sw != 1) {
...
static void lio_circle_pset(short x, short y, uint8_t palette) {
  if (x >= lio_draw.x1 && x <= lio_draw.x2 && y >= lio_draw.y1 &&
      y <= lio_draw.y2) {
...
static void lio_circle_fill(short cx, short cy, short my, short py,
...
uint8_t PC98_BIOS_LIO_GCIRCLE() {
  ...
  if (flag & 0x40) {
    ...
    lio_set_tile(real_readw(seg, off + 0x15), real_readw(seg, off + 0x13),
                 count);
  } else {
    if (count == 0xff) {
      count = palette;
...
static void lio_paint(short x, short y, uint8_t border) {
  ...
  while (!paint_point.empty()) {
    ...
    lx = rx = point.x;
    ly = point.y;
    if (lio_point(lx, ly) == border ||
        lio_draw.paint_work[lx + ly * 640] != 0) {
      continue;
    }
...
uint8_t PC98_BIOS_LIO_GPAINT1() {
  ...
  for (std::vector<PAINT_LINE>::iterator line = paint_line.begin();
       line != paint_line.end(); ++line) {
...
static void lio_putor(PUTCNTX *pt) {
  ...
  if ((pt->shift + width) < 8) {
    mem_writeb(addr,
               mem_readb(addr) | ((uint8_t)((data >> pt->shift) & pt->masklr)));
  } else {
    mem_writeb(addr,
               mem_readb(addr) | ((uint8_t)((data >> pt->shift) & pt->maskl)));
    addr++;
    width -= (8 - pt->shift);
    while (width > 8) {
      width -= 8;
    ...
    if (width) {
      data = (data << 8) + (*src);
      mem_writeb(addr, mem_readb(addr) |
                           ((uint8_t)((data >> pt->shift) & pt->maskr)));
    }
  }
}
...
static void lio_putorn(PUTCNTX *pt) {
  ...
  if ((pt->shift + width) < 8) {
    mem_writeb(addr, mem_readb(addr) |
                         ((uint8_t)((~data) >> pt->shift) & pt->masklr));
  } else {
    mem_writeb(addr,
               mem_readb(addr) | ((uint8_t)((~data) >> pt->shift) & pt->maskl));
    addr++;
...
static void lio_putand(PUTCNTX *pt) {
  ...
  if ((pt->shift + width) < 8) {
    ...
    mem_writeb(addr, mem_readb(addr) &
                         ((uint8_t)((data >> pt->shift) | (~pt->maskl))));
    addr++;
    width -= (8 - pt->shift);
    while (width > 8) {
      width -= 8;
...
static void lio_putxor(PUTCNTX *pt) {
  ...
  if ((pt->shift + width) < 8) {
    ...
    if (width) {
      data = (data << 8) + (*src);
      mem_writeb(addr,
                 mem_readb(addr) ^ ((uint8_t)(data >> pt->shift) & pt->maskr));
    }
  }
}
...
static uint8_t lio_putsub(const LIOPUT *lput) {
  ...
  if (lput->x < lio_draw.x1 || lput->y < lio_draw.y1 ||
      (lput->x + lput->width - 1) > lio_draw.x2 ||
      (lput->y + lput->height - 1) > lio_draw.y2) {
    return LIO_ILLEGALFUNC;
  }
...
static const uint8_t hiragana_font[] = {
    ,
};
...
uint8_t PC98_BIOS_LIO_GGET() {
  ...
  x2 = real_readw(seg, off + 4);
  y2 = real_readw(seg, off + 6);
  if (x1 < lio_draw.x1 || y1 < lio_draw.y1 || x2 > lio_draw.x2 ||
      y2 > lio_draw.y2) {
    return LIO_ILLEGALFUNC;
  }
  x2 = x2 - x1 + 1;
