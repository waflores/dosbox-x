 ...
 * - checksum values are verified; and the backup header is used if the footer
 * cannot be found or has an invalid checksum
 * - code does not prevent loading if the VHD is marked as being part of a saved
 * state
 ...
 * - for differencing disks, parent paths are converted to ASCII; unicode
 * characters will cancel loading
...
imageDiskVHD::ErrorCodes imageDiskVHD::Open(const char *fileName,
                                            const bool readOnly,
                                            imageDisk **disk) {
...
imageDiskVHD::ErrorCodes imageDiskVHD::Open(const char *fileName,
  ...
  if (fseeko64(file, 0L, SEEK_END)) {
    fclose(file);
    return INVALID_DATA;
  }
  if (ftello64(file) < 512) {
    fclose(file);
    return INVALID_DATA;
  }
  // read footer
  if (fseeko64(file, -512L, SEEK_CUR)) {
    fclose(file);
    return INVALID_DATA;
  }
  uint64_t footerPosition =
      (uint64_t)((int64_t)ftello64(file)); /* make sure to sign extend! */
  if ((int64_t)footerPosition < 0LL) {
    fclose(file);
    return INVALID_DATA;
  }
  VHDFooter originalfooter;
  VHDFooter footer;
  if (fread(&originalfooter, 512, 1, file) != 1) {
    fclose(file);
    return INVALID_DATA;
  }
  ...
  if (!footer.IsValid()) {
    ...
    if (fseeko64(file, 0L, SEEK_SET)) {
      fclose(file);
      return INVALID_DATA;
    }
    if (fread(&originalfooter, sizeof(uint8_t), 512, file) != 512) {
      fclose(file);
      return INVALID_DATA;
    }
    // convert from big-endian if necessary
  ...
  if (matchUniqueId && memcmp(matchUniqueId, footer.uniqueId, 16)) {
    fclose(file);
  ...
  if (footer.diskType == VHD_TYPE_FIXED) {
    ...
    if (fread(&buf, sizeof(uint8_t), 512, file) != 512) {
      fclose(file);
      return INVALID_DATA;
    }
    ...
    vhd->heads = sizes[2];
    vhd->sectors = sizes[1];
    vhd->fixedDisk = new imageDisk(file, fileName, vhd->cylinders, vhd->heads,
                                   vhd->sectors, 512, true);
    *disk = vhd;
  ...
  if (fseeko64(file, (off_t)footer.dataOffset, SEEK_SET)) {
    delete vhd;
    return INVALID_DATA;
  }
  if (fread(&dynHeader, sizeof(uint8_t), 1024, file) != 1024) {
    delete vhd;
    return INVALID_DATA;
  }
  // swap byte order and validate checksum
  dynHeader.SwapByteOrder();
  if (!dynHeader.IsValid()) {
    delete vhd;
    return INVALID_DATA;
  }
  // check block size is valid (should be a power of 2, and at least equal to
  // one sector)
  if (dynHeader.blockSize < 512 ||
      (dynHeader.blockSize & (dynHeader.blockSize - 1))) {
    delete vhd;
    return INVALID_DATA;
  }
  ...
  if (footer.diskType == VHD_TYPE_DIFFERENCING) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (dynHeader.parentLocatorEntry[i].platformCode != 0) {
        ...
        uint64_t dataOffset =
            dynHeader.parentLocatorEntry[i].platformDataOffset;
        uint32_t dataLength =
            dynHeader.parentLocatorEntry[i].platformDataLength;
        uint8_t *buffer = nullptr;
        if (dataOffset && dataLength &&
            ((uint64_t)dataOffset + dataLength) <= footerPosition) {
          if (fseeko64(file, (off_t)dataOffset, SEEK_SET)) {
            delete vhd;
            return INVALID_DATA;
          }
          ...
          if (fread(buffer, sizeof(uint8_t), dataLength, file) != dataLength) {
            free(buffer);
            delete vhd;
            return INVALID_DATA;
          }
        ...
        ErrorCodes ret =
            TryOpenParent(fileName, dynHeader.parentLocatorEntry[i], buffer,
                          buffer ? dataLength : 0, &(vhd->parentDisk),
                          dynHeader.parentUniqueId);
        if (buffer)
          free(buffer);
  ...
  uint32_t blockMapSectors =
      ((((sectorsPerBlock /* 4096 sectors/block (typ) = 4096 bits required */
          + 7) /
         8) /* convert to bytes and round up to nearest byte; 4096/8 = 512 bytes
               required */
        + 511) /
       512); /* convert to sectors and round up to nearest sector; 512/512 = 1
                sector */
  // check that the BAT is large enough for the disk
  uint32_t tablesRequired =
      (uint32_t)((footer.currentSize + (dynHeader.blockSize - 1)) /
                 dynHeader.blockSize);
  if (dynHeader.maxTableEntries < tablesRequired) {
    delete vhd;
    return INVALID_DATA;
  }
  // check that the BAT is contained within the file
  if (((uint64_t)dynHeader.tableOffset +
       ((uint64_t)dynHeader.maxTableEntries * (uint64_t)4)) > footerPosition) {
    delete vhd;
    return INVALID_DATA;
  }
...
int iso8859_1_encode(int utf32code) {
  return ((utf32code >= 32 && utf32code <= 126) ||
          (utf32code >= 160 && utf32code <= 255))
             ? utf32code
             : -1;
}

// this function (1) converts data from UTF-16 to a native string for fopen
// (depending on the host OS), and (2) converts slashes, if necessary
bool imageDiskVHD::convert_UTF16_for_fopen(std::string &string,
                                           const void *data,
  ...
  string.reserve((size_t)(string.size() + (dataLength / 2) +
                          10)); // allocate a few extra bytes
  ...
  while (indata < lastchar) {
    ...
    if (utf32code < 0)
      return false;
#if defined(WIN32) || defined(OS2)
    // MSDN docs define fopen to accept strings in the windows default code
    // page, which is typically Windows-1252 convert unicode string to ISO-8859,
    // which is a subset of Windows-1252, and a lot easier to implement
    ...
    tempout = temp;
    if (utf8_encode(&tempout, tempoutmax, (unsigned int)utf32code) < 0)
      return false;
    // and append the byte(s) to the string
    tempout2 = temp;
    while (tempout2 < tempout)
      string += *tempout2++;
#endif
  }
  return true;
...
imageDiskVHD::ErrorCodes
imageDiskVHD::TryOpenParent(const char *childFileName,
                            const imageDiskVHD::ParentLocatorEntry &entry,
                            const uint8_t *data, const uint32_t dataLength,
                            imageDisk **disk, const uint8_t *uniqueId) {
  std::string str = "";
  ...
  case 0x57327275:
    ...
    // convert byte order, and UTF-16 to ASCII, and change backslashes to
    // slashes if on Linux
    if (!convert_UTF16_for_fopen(str, data, dataLength))
      break;
  ...
  return ERROR_OPENING; // return ERROR_OPENING if the file does not exist,
                        // cannot be accessed, etc
}

uint8_t imageDiskVHD::Read_AbsoluteSector(uint32_t sectnum, void *data) {
  if (vhdType == VHD_TYPE_FIXED)
    return fixedDisk->Read_AbsoluteSector(sectnum, data);
  ...
  if (currentBlockAllocated) {
    ...
    if (hasData) {
      if (fseeko64(diskimg,
                   (off_t)(((uint64_t)currentBlockSectorOffset +
                            blockMapSectors + sectorOffset) *
                           512ull),
                   SEEK_SET))
        return 0x05; // can't seek
      if (fread(data, sizeof(uint8_t), 512, diskimg) != 512)
        return 0x05; // can't read
      return 0;
    }
  }
...
bool imageDiskVHD::is_block_allocated(uint32_t blockNumber) {
  if (currentBlockAllocated)
    return true;
  if (parentDisk &&
      ((imageDiskVHD *)parentDisk)->is_block_allocated(blockNumber))
    return true;
  return false;
}
...
uint8_t imageDiskVHD::Write_AbsoluteSector(uint32_t sectnum, const void *data) {
  if (vhdType == VHD_TYPE_FIXED)
    return fixedDisk->Write_AbsoluteSector(sectnum, data);
  uint32_t blockNumber = sectnum / sectorsPerBlock;
  uint32_t sectorOffset = sectnum % sectorsPerBlock;
  if (!loadBlock(blockNumber))
    return 0x05; // can't load block
  if (!currentBlockAllocated) {
    ...
    if (is_zeroed_sector(data)) {
      if (vhdType != VHD_TYPE_DIFFERENCING)
        return 0;
      if (!is_block_allocated(blockNumber))
        return 0;
    }

    if (!copiedFooter) {
      // write backup of footer at start of file (should already exist, but we
      // never checked to be sure it is readable or matches the footer we used)
      if (fseeko64(diskimg, (off_t)0, SEEK_SET))
        return 0x05;
    ...
    // calculate new location of footer, and round up to nearest 512 byte
    // increment "just in case"
    uint64_t newFooterPosition =
        (((footerPosition + blockMapSize + dynamicHeader.blockSize) + 511ull) /
         512ull) *
        512ull;
    // attempt to extend the length appropriately first (on some operating
    // systems this will extend the file)
    if (fseeko64(diskimg, (off_t)newFooterPosition + 512, SEEK_SET))
      return 0x05;
    // now write the footer
    if (fseeko64(diskimg, (off_t)newFooterPosition, SEEK_SET))
      return 0x05;
    if (fwrite(&originalFooter, sizeof(uint8_t), 512, diskimg) != 512)
      return 0x05;
    // save the new block location and new footer position
    uint32_t newBlockSectorNumber =
        (uint32_t)((footerPosition + 511ul) / 512ul);
    footerPosition = newFooterPosition;
    // clear the dirty flags for the new footer position
    for (uint32_t i = 0; i < blockMapSize; i++)
      currentBlockDirtyMap[i] = 0;
    // write the dirty map
    if (fseeko64(diskimg, (off_t)(newBlockSectorNumber * 512ull), SEEK_SET))
      return 0x05;
    if (fwrite(currentBlockDirtyMap, sizeof(uint8_t), blockMapSize, diskimg) !=
        blockMapSize)
      return 0x05;
    // flush the data to disk after expanding the file, before allocating the
    // block in the BAT
    if (fflush(diskimg))
      return 0x05;
    // update the BAT
    if (fseeko64(diskimg,
                 (off_t)(dynamicHeader.tableOffset + (blockNumber * 4ull)),
                 SEEK_SET))
      return 0x05;
  ...
  if (!hasData) {
    ...
    if (fseeko64(diskimg, (off_t)(currentBlockSectorOffset * 512ull), SEEK_SET))
      return 0x05; // can't seek
    if (fwrite(currentBlockDirtyMap, sizeof(uint8_t), blockMapSize, diskimg) !=
        blockMapSize)
      return 0x05;
  ...
  if (fseeko64(diskimg,
               (off_t)(((uint64_t)currentBlockSectorOffset +
                        (uint64_t)blockMapSectors + (uint64_t)sectorOffset) *
                       512ull),
               SEEK_SET))
    return 0x05; // can't seek
...
imageDiskVHD::VHDTypes imageDiskVHD::GetVHDType(const char *fileName) {
  imageDisk *disk;
  if (Open(fileName, true, &disk))
    return VHD_TYPE_NONE;
  const imageDiskVHD *vhd = dynamic_cast<imageDiskVHD *>(disk);
  VHDTypes ret = VHD_TYPE_FIXED;
  if (vhd)
    ret = vhd->footer.diskType; // get the actual type
  delete disk;
  return ret;
}
...
bool imageDiskVHD::loadBlock(const uint32_t blockNumber) {
  if (currentBlock == blockNumber)
    return true;
  if (blockNumber >= dynamicHeader.maxTableEntries)
    return false;
  if (fseeko64(diskimg,
               (off_t)(dynamicHeader.tableOffset + (blockNumber * 4ull)),
               SEEK_SET))
  ...
  if (fread(&blockSectorOffset, sizeof(uint8_t), 4, diskimg) != 4)
    return false;
  ...
  if (blockSectorOffset == 0xFFFFFFFFul) {
    ...
    if (fseeko64(diskimg, (off_t)(blockSectorOffset * (uint64_t)512), SEEK_SET))
      return false;
    currentBlock = 0xFFFFFFFFul;
    currentBlockAllocated = true;
    currentBlockSectorOffset = blockSectorOffset;
    if (fread(currentBlockDirtyMap, sizeof(uint8_t), blockMapSize, diskimg) !=
        blockMapSize)
      return false;
    currentBlock = blockNumber;
  }
...
void imageDiskVHD::VHDFooter::SwapByteOrder() {
  ...
  // however, for our purposes (comparing to the parent guid on differential
  // disks),
...
uint32_t imageDiskVHD::VHDFooter::CalculateChecksum() {
  ...
  // because of that, the byte order doesn't matter when calculating the
  // checksum however, the checksum must be stored in the correct byte order or
  // it will not match
...
bool imageDiskVHD::VHDFooter::IsValid() {
  return (memcmp(cookie, "conectix", 8) == 0 &&
          fileFormatVersion >= 0x00010000 && fileFormatVersion <= 0x0001FFFF &&
...
void imageDiskVHD::DynamicHeader::SwapByteOrder() {
  ...
  for (int i = 0; i < 8; i++) {
    parentLocatorEntry[i].platformCode =
        SDL_SwapBE32(parentLocatorEntry[i].platformCode);
    parentLocatorEntry[i].platformDataSpace =
        SDL_SwapBE32(parentLocatorEntry[i].platformDataSpace);
    parentLocatorEntry[i].platformDataLength =
        SDL_SwapBE32(parentLocatorEntry[i].platformDataLength);
    parentLocatorEntry[i].reserved =
        SDL_SwapBE32(parentLocatorEntry[i].reserved);
    parentLocatorEntry[i].platformDataOffset =
        SDL_SwapBE64(parentLocatorEntry[i].platformDataOffset);
  }
...
bool imageDiskVHD::DynamicHeader::IsValid() {
  return (memcmp(cookie, "cxsparse", 8) == 0 && headerVersion >= 0x00010000 &&
          headerVersion <= 0x0001FFFF && checksum == CalculateChecksum());
}
...
bool imageDiskVHD::UpdateUUID() {
  ...
  footer.SwapByteOrder();
  if (fseeko64(diskimg, footerPosition, SEEK_SET))
    return false;
  if (fwrite(&originalFooter, 1, 512, diskimg) != 512)
    return false;
  if (vhdType != VHD_TYPE_FIXED) {
...
void imageDiskVHD::SizeToCHS(uint64_t size, uint16_t *c, uint8_t *h,
                             uint8_t *s) {
...
uint32_t imageDiskVHD::CreateDynamic(const char *filename, uint64_t size) {
  ...
  if (filename == NULL)
    return ERROR_OPENING;
  if (size < 3145728 ||
      size > 2190433320960) // 2040GB is the Windows 11 mounter limit
  ...
  SizeToCHS(size, &footer.geometry.cylinders, &footer.geometry.heads,
            &footer.geometry.sectors);
...
uint32_t imageDiskVHD::CreateDifferencing(const char *filename,
                                          const char *basename) {
  LOG_MSG("CreateDifferencing filename=\"%s\" basename=\"%s\"", filename,
          basename);
  ...
  // Locators - Windows 11 wants at least the relative W2ru locator, or won't
  // mount!
#if defined(WIN32)
  ...
  header.parentLocatorEntry[1].platformDataOffset =
      header.parentLocatorEntry[0].platformDataOffset + plat1;
  ...
  for (uint32_t i = 0; i < len1; i++)
    ...
    w_basename[i] =
        SDL_SwapLE16(absBasePathName[i] == '/' ? (uint16_t)'\\'
                                               : (uint16_t)absBasePathName[i]);
  if (fwrite(w_basename, 1, plat1, vhd) != plat1)
    STATUS = ERROR_WRITING;
  ...
  for (uint32_t i = 0; i < len2; i++)
    // dirty hack to quickly convert ASCII -> UTF-16 *LE* and fix slashes
    w_basename[i] =
        SDL_SwapLE16(relpath[i] == '/' ? (uint16_t)'\\' : (uint16_t)relpath[i]);
  if (fwrite(w_basename, 1, plat2, vhd) != plat2)
    STATUS = ERROR_WRITING;
...
uint32_t imageDiskVHD::GetInfo(const char *filename, VHDInfo **info) {
  ...
  if (filename == NULL)
    return ERROR_OPENING;
  if (imageDiskVHD::Open(filename, true, (imageDisk **)&vhd) !=
...
bool imageDiskVHD::MergeSnapshot(uint32_t *totalSectorsMerged,
                                 uint32_t *totalBlocksUpdated) {
  if (totalSectorsMerged == NULL || totalBlocksUpdated == NULL)
    return false;
  if (vhdType != VHD_TYPE_DIFFERENCING) {
  ...
  *totalBlocksUpdated = 0;
    ...
    for (uint32_t sector = 0; sector < sectorsPerBlock; sector++) {
      ...
      if (hasData) {
        ...
        if (parentDisk->Write_AbsoluteSector(absoluteSector, data)) {
          LOG_MSG("Couldn't update parent's sector %d, merging aborted!",
                  absoluteSector);
          return false;
        }
      }
  ...
  LOG_MSG("Merged %d sectors in %d blocks", *totalSectorsMerged,
          *totalBlocksUpdated);
...
uint32_t imageDiskVHD::CreateSnapshot() {
  ...
  mk_uuid(buf);
  char name[255];
  sprintf(name, "{%08x-%08x-%08x-%08x}.vhd", *((uint32_t *)(buf)),
...
void imageDiskVHD::DetectGeometry(uint8_t *buf, Bitu sizes[],
