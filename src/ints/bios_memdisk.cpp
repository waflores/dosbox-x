...
imageDiskMemory::imageDiskMemory(uint32_t imgSizeK) : imageDisk(ID_MEMORY) {
  ...
  //   it enforces a minimum drive size of 32kb, since a hard drive cannot be
  //   formatted as FAT12 with a smaller partition the code works properly
  //   throughout the range of a 32-bit unsigned integer, however:
  //     a) for drives requesting more than 8,225,280kb, the number of cylinders
  //     will exceed 1024 b) for drives requesting ULONG_MAX kb, the drive it
  //     creates will be slightly larger than ULONG_MAX kb, due to rounding
  ...
  // since this is a virtual drive, we are not restricted to ATA limits, so go
  // with BIOS/MBR limits
  ...
  // use 32kb as minimum drive size, since it is not possible to format a
  // smaller drive that has 16 sectors
  if (imgSizeK < 32)
    imgSizeK = 32;
  ...
  uint64_t total_sectors =
      ((uint64_t)imgSizeK * 1024 + sector_size - 1) / sector_size;
  ...
  if (total_sectors <= 1024 * 16 * 16) {
    sectors = 16;
    heads =
        (uint32_t)((total_sectors + (1024 * sectors - 1)) / (1024 * sectors));
    cylinders =
        (uint32_t)((total_sectors + (sectors * heads - 1)) / (sectors * heads));
...
imageDiskMemory::imageDiskMemory(const diskGeo &floppyGeometry)
    : imageDisk(ID_MEMORY) {
  init(floppyGeometry, false, nullptr);
}

// Create a hard drive image of a specified geometry
imageDiskMemory::imageDiskMemory(uint16_t cylinders, uint16_t heads,
...
void imageDiskMemory::init(diskGeo diskParams, bool isHardDrive,
  ...
  uint64_t absoluteSectors =
      (uint64_t)diskParams.cylcount * (uint64_t)diskParams.headscyl;
  ...
  uint64_t diskSizeK =
      ((uint64_t)diskParams.headscyl * (uint64_t)diskParams.cylcount *
           (uint64_t)diskParams.secttrack * (uint64_t)diskParams.bytespersect +
       (uint64_t)1023) /
      (uint64_t)1024;
  ...
  // split the sectors into chunks, which are allocated together on an as-needed
  // basis assuming a maximum of 63 heads and 255 sectors, this formula gives a
  // nice chunk size that scales with the drive size for any drives under 64mb,
  // the chunks are 8kb each, and the map consumes 1/1000 of the total drive
  // size then the chunk size grows up to a 8gb drive, while the map consumes
  // 30-60k of memory (on 64bit PCs) with a 8gb drive, the chunks are about 1mb
  // each, and the map consumes about 60k of memory and for larger drives (with
  // over 1023 cylinders), the chunks remain at 1mb each while the map grows
  this->sectors_per_chunk =
      (diskParams.headscyl + 7u) / 8u * diskParams.secttrack;
  this->total_chunks = (uint32_t)((absoluteSectors + sectors_per_chunk - 1u) /
                                  sectors_per_chunk);
  this->chunk_size = sectors_per_chunk * diskParams.bytespersect;
  // allocate a map of chunks that have been allocated and their memory
  // addresses
  ChunkMap = (uint8_t **)malloc(total_chunks * sizeof(uint8_t *));
  if (ChunkMap == NULL) {
    LOG_MSG("Error allocating memory map in imageDiskMemory constructor for "
            "%lu clusters.\n",
...
void imageDiskMemory::Set_Geometry(uint32_t setHeads, uint32_t setCyl,
                                   uint32_t setSect, uint32_t setSectSize) {
  if (setHeads != this->heads || setCyl != this->cylinders ||
      setSect != this->sectors || setSectSize != this->sector_size) {
...
uint8_t imageDiskMemory::Read_AbsoluteSector(uint32_t sectnum, void *data) {
  ...
  // calculate which chunk the sector is located within, and which sector within
  // the chunk
  ...
  // if the chunk has not yet been allocated, return underlying image if any, or
  // else zeros
  if (!datalocation) {
    if (this->underlyingImage) {
...
uint8_t imageDiskMemory::Write_AbsoluteSector(uint32_t sectnum,
                                              const void *data) {
  // sector number is a zero-based offset
  ...
  if (datalocation == NULL) {
    ...
    if (datalocation == NULL) {
      LOG_MSG(
          "Could not allocate memory in Write_AbsoluteSector for sector %lu.\n",
          (unsigned long)sectnum);
      return 0x05;
    }
    // save the memory chunk address within the memory map
    ChunkMap[chunknum] = datalocation;
    // initialize the memory chunk to all zeros (since we are only writing to a
    // single sector within this chunk)
    memset((void *)datalocation, 0, chunk_size);
    // if there is an underlying image, read from the underlying image to fill
    // the chunk
    if (this->underlyingImage) {
      uint32_t chunkFirstSector = chunknum * this->sectors_per_chunk;
      uint32_t sectorsToCopy = this->sectors_per_chunk;
      // if this is the last chunk, don't read past the end of the original
      // image
      if ((chunknum + 1) == this->total_chunks)
        sectorsToCopy = this->total_sectors - chunkFirstSector;
      // copy the sectors
      uint8_t *target = datalocation;
      for (uint32_t i = 0; i < sectorsToCopy; i++) {
        this->underlyingImage->Read_AbsoluteSector(i + chunkFirstSector,
                                                   target);
...
uint8_t imageDiskMemory::Format() {
  ...
  if (this->sector_size != 512) {
    LOG_MSG("imageDiskMemory::Format only designed for disks with 512-byte "
            "sectors.\n");
    return 0x01;
  }
  if (this->sectors > 63) {
    LOG_MSG("imageDiskMemory::Format only designed for disks with <= 63 "
            "sectors.\n");
    return 0x02;
  }
  if (this->heads > 255) {
    LOG_MSG(
        "imageDiskMemory::Format only designed for disks with <= 255 heads.\n");
    return 0x03;
  }
  if (this->cylinders <= this->Get_Reserved_Cylinders()) {
    LOG_MSG(
        "Invalid number of reserved cylinders in imageDiskMemory::Format\n");
    return 0x06;
  }
  uint32_t reported_cylinders =
      this->cylinders - this->Get_Reserved_Cylinders();
  if (reported_cylinders > 1024) {
    LOG_MSG("imageDiskMemory::Format only designed for disks with <= 1024 "
            "cylinders.\n");
    return 0x04;
  }
  uint32_t reported_total_sectors =
      reported_cylinders * this->heads * this->sectors;

  // plan the drive
  // write a MBR?
  bool writeMBR = this->hardDrive;
  uint32_t partitionStart =
      writeMBR ? this->sectors : 0; // must be aligned with the start of a head
                                    // (multiple of this->sectors)
  uint32_t partitionLength =
      reported_total_sectors -
      partitionStart; // must be aligned with the start of a head (multiple of
                      // this->sectors)
  // figure out the media id
  uint8_t mediaID = this->hardDrive ? 0xF8 : this->floppyInfo.mediaid;
  // figure out the number of root entries and minimum number of sectors per
  // cluster
  ...
  uint32_t sectors_per_cluster =
      this->hardDrive
          ? 4
          : this->floppyInfo
                .sectcluster; // fat requires 2k clusters minimum on hard drives
  ...
  if (!this->CalculateFAT(partitionStart, partitionLength, this->hardDrive,
                          root_ent, &root_sectors, &sectors_per_cluster,
                          &isFat16, &fatSectors, &reservedSectors)) {
    LOG_MSG("imageDiskMemory::Format could not calculate FAT sectors.\n");
    return 0x05;
  }

  LOG_MSG("Formatting FAT%u %s drive C/H/S %u/%u/%u with %u bytes/sector, %u "
          "root entries, %u-byte clusters, media id 0x%X\n",
          (unsigned int)(isFat16 ? 16 : 12),
          this->hardDrive ? "hard" : "floppy", reported_cylinders, this->heads,
          this->sectors, this->sector_size, root_ent,
          (sectors_per_cluster * this->sector_size), (unsigned int)mediaID);
  ...
  if (writeMBR) {
    ...
    sbuf[0x1c4] =
        this->sectors |
        (((this->cylinders - 1 - this->Get_Reserved_Cylinders()) & 0x300) >> 2);
  ...
  // reserved sectors: 1 for floppies (the boot sector), or align to 4k boundary
  // for hard drives (not required to align to 4k boundary)
  ...
  // total sectors (<= 65535)
  if (partitionLength < 0x10000u)
    host_writew(&sbuf[0x13], (uint16_t)partitionLength);
  // media descriptor
  sbuf[0x15] = mediaID;
  // sectors per FAT
  ...
  // sectors (>= 65536)
  if (partitionLength >= 0x10000u)
    host_writed(&sbuf[0x20], partitionLength);
  // BIOS drive
  ...
  for (uint32_t i = partitionStart + 1;
       i < partitionStart + reservedSectors + fatSectors + fatSectors +
               root_sectors;
       i++) {
  ...
  this->Write_AbsoluteSector(partitionStart + reservedSectors + fatSectors,
                             sbuf);
...
// Calculate the number of sectors per cluster, the number of sectors per fat,
// and if it is FAT12/FAT16 Note that sectorsPerCluster is required to be set to
// the minimum, which will be 2 for certain types of floppies
bool imageDiskMemory::CalculateFAT(uint32_t partitionStartingSector,
                                   uint32_t partitionLength, bool isHardDrive,
                                   uint32_t rootEntries, uint32_t *rootSectors,
                                   uint32_t *sectorsPerCluster, bool *isFat16,
                                   uint32_t *fatSectors,
                                   uint32_t *reservedSectors) {
  // check for null references
  if (rootSectors == NULL || sectorsPerCluster == NULL || isFat16 == NULL ||
      fatSectors == NULL || reservedSectors == NULL)
    return false;
  ...
  if ((rootEntries > (isHardDrive ? 512u : 240u)) ||
      (rootEntries / 16) * 16 != rootEntries || rootEntries == 0) {
  ...
  *rootSectors = rootEntries * 32 / 512;
  ...
  //   minimum sectors = root sectors + 1 for boot sector + 1 for fat #1 + 1 for
  //   fat #2 + 1 cluster for data + add 7 for hard drives due to allow for 4k
  //   alignment
  if (partitionLength <
      (*rootSectors + 3 + *sectorsPerCluster + (isHardDrive ? 7 : 0))) {
  ...
  *reservedSectors = 1;
  ...
  // Additional alignment can be made by increasing the number of reserved
  // sectors,
  //   or by increasing the reservedSectors value -- both done after the
  //   calculation
  if (isHardDrive && ((partitionStartingSector + *reservedSectors) % 2 == 1))
    (*reservedSectors)++;

  // compute the number of fat sectors and data sectors
  uint32_t dataSectors;
  uint32_t dataClusters;
  // first try FAT12 - compute the minimum number of fat sectors necessary using
  // the minimum number of sectors per cluster
  *fatSectors =
      (((uint64_t)partitionLength - *reservedSectors - *rootSectors + 2) * 3 +
       *sectorsPerCluster * 1024 + 5) /
      (*sectorsPerCluster * 1024 + 6);
  dataSectors = partitionLength - *reservedSectors - *rootSectors -
                *fatSectors - *fatSectors;
  dataClusters = dataSectors / *sectorsPerCluster;
  // check if this calculation makes the drive too big to fit within a FAT12
  // drive
  if (dataClusters >= 4085) {
    // so instead let's calculate for FAT16, and increase the number of sectors
    // per cluster if necessary note: changing the drive to FAT16 will reducing
    // the number of data sectors, which might change the drive
    //   from a FAT16 drive to a FAT12 drive.  however, the only difference is
    //   that there are unused fat sectors allocated.  we cannot allocate them,
    //   or else the drive will change back to a FAT16 drive, so, just leave it
    //   as-is
    do {
      // compute the minimum number of fat sectors necessary starting with the
      // minimum number of sectors per cluster
      //   the +2 is for the two reserved data sectors (sector #0 and sector #1)
      //   which are not stored on the drive, but are in the map rounds up and
      //   assumes 512 byte sector size
      *fatSectors = ((uint64_t)partitionLength - *reservedSectors -
                     *rootSectors + 2 + (*sectorsPerCluster * 256 + 1)) /
                    (*sectorsPerCluster * 256 + 2);
  ...
  if (isHardDrive) {
    ...
    *reservedSectors = (partitionStartingSector + *reservedSectors +
                        *rootSectors + *fatSectors + *fatSectors) %
                       8;
    ...
    // note: by reducing the number of data sectors, the drive might have
    // changed from a FAT16 drive to a FAT12 drive.
    //   however, the only difference is that there are unused fat sectors
    //   allocated.  we cannot allocate them, or else the drive alignment will
    //   change again, and most likely the drive would change back into a FAT16
    //   drive. so, just leave it as-is
  }
