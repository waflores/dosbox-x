...
static void MCGA2_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                          uint8_t rnew, PhysPt base) {
  BIOS_CHEIGHT;
...
static void CGA2_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                         uint8_t rnew, PhysPt base) {
  BIOS_CHEIGHT;
...
static void CGA4_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                         uint8_t rnew, PhysPt base) {
  BIOS_CHEIGHT;
  PhysPt dest = base + ((CurMode->twidth * rnew) * (cheight / 2) + cleft) * 2;
  PhysPt src = base + ((CurMode->twidth * rold) * (cheight / 2) + cleft) * 2;
  Bitu copy = (Bitu)(cright - cleft) * 2u;
  Bitu nextline = (Bitu)CurMode->twidth * 2u;
  for (Bitu i = 0; i < cheight / 2U; i++) {
    MEM_BlockCopy(dest, src, copy);
    MEM_BlockCopy(dest + 8 * 1024, src + 8 * 1024, copy);
    dest += nextline;
...
static void TANDY16_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                            uint8_t rnew, PhysPt base) {
  BIOS_CHEIGHT;
  uint8_t banks = CurMode->twidth / 10;
  PhysPt dest =
      base + ((CurMode->twidth * rnew) * (cheight / banks) + cleft) * 4;
  PhysPt src =
      base + ((CurMode->twidth * rold) * (cheight / banks) + cleft) * 4;
  Bitu copy = (Bitu)(cright - cleft) * 4u;
  Bitu nextline = (Bitu)CurMode->twidth * 4u;
  for (Bitu i = 0; i < static_cast<Bitu>(cheight / banks); i++) {
    for (Bitu b = 0; b < banks; b++)
      MEM_BlockCopy(dest + b * 8 * 1024, src + b * 8 * 1024, copy);
    dest += nextline;
    src += nextline;
  }
}
...
static inline uint8_t CheckCopyBankSelect(uint8_t select, PhysPt &src,
                                          PhysPt &dest, PhysPt &src_pt,
                                          PhysPt &dest_pt) {
  if (svgaCard == SVGA_TsengET4K || svgaCard == SVGA_S3Trio) {
    if (src_pt >= 0x10000) {
      if ((select & 0xf0) == 0x00) {
...
static void EGA16_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                          uint8_t rnew, PhysPt base) {
  PhysPt src, dest;
  Bitu copy;
  BIOS_CHEIGHT;
  ...
  if (svgaCard != SVGA_TsengET4K && svgaCard != SVGA_S3Trio) {
    ...
    for (; copy > 0; copy--) {
      for (Bitu x = 0; x < rowsize; x++)
        mem_writeb(dest + x, mem_readb(src + x));
      dest += nextline;
      src += nextline;
    }
  } else {
...
static uint8_t CopyRowMask(PhysPt base, PhysPt dest_pt, PhysPt src_pt,
                           uint8_t mask, uint8_t src_bank, uint8_t dest_bank,
                           uint8_t last_bank) {
  uint8_t no;
  uint8_t plane[4];
...
static void EGA16_CopyRow_24(uint8_t cleft, uint8_t cright, uint8_t rold,
                             uint8_t rnew, PhysPt base) {
  PhysPt src, dest;
  Bitu rowsize;
  ...
  for (Bitu copy = 0; copy < 24; copy++) {
    ...
    for (Bitu x = 0; x < rowsize; x++) {
      ...
      if (x == 0 && (cleft & 1)) {
        last_bank = CopyRowMask(base, dest_pt, src_pt, 0x0f, src_bank,
                                dest_bank, last_bank);
      } else if (x == rowsize - 1 && (cright & 1)) {
        last_bank = CopyRowMask(base, dest_pt, src_pt, 0xf0, src_bank,
                                dest_bank, last_bank);
      } else {
        if (svgaCard == SVGA_S3Trio && src_bank != last_bank) {
          IO_Write(0x3d4, 0x6a);
...
static void VGA_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                        uint8_t rnew, PhysPt base) {
  PhysPt src, dest;
  Bitu copy;
  BIOS_CHEIGHT;
...
static void TEXT_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                         uint8_t rnew, PhysPt base, uint8_t ncols) {
  PhysPt src, dest;
...
static void PC98_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                         uint8_t rnew, PhysPt base) {
  PhysPt src, dest;
...
static void DCGA_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                         uint8_t rnew, PhysPt base) {
...
static void DCGA_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                         PhysPt base, uint8_t attr) {
  uint8_t cheight = real_readb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT);
...
static void DOSV_Text_CopyRow(uint8_t cleft, uint8_t cright, uint8_t rold,
                              uint8_t rnew) {
...
static void DOSV_Text_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                              uint8_t attr) {
  Bitu textdest =
      real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 2 * row + cleft * 2;
  Bitu textcopy = (cright - cleft);
  uint16_t seg = GetTextSeg();
  for (Bitu x = 0; x < textcopy; x++) {
...
static void MCGA2_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                          PhysPt base, uint8_t attr) {
  BIOS_CHEIGHT;
  PhysPt dest = base + ((CurMode->twidth * row) * cheight + cleft);
  Bitu copy = (Bitu)(cright - cleft);
  Bitu nextline = CurMode->twidth;
  attr = (attr & 0x3) | ((attr & 0x3) << 2) | ((attr & 0x3) << 4) |
         ((attr & 0x3) << 6);
  for (Bitu i = 0; i < cheight; i++) {
    for (Bitu x = 0; x < copy; x++) {
      mem_writeb(dest + x, attr);
...
static void CGA2_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                         PhysPt base, uint8_t attr) {
  BIOS_CHEIGHT;
...
static void CGA4_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                         PhysPt base, uint8_t attr) {
  BIOS_CHEIGHT;
...
static void TANDY16_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                            PhysPt base, uint8_t attr) {
  BIOS_CHEIGHT;
  uint8_t banks = CurMode->twidth / 10;
  PhysPt dest =
      base + ((CurMode->twidth * row) * (cheight / banks) + cleft) * 4;
  Bitu copy = (Bitu)(cright - cleft) * 4u;
  ...
  for (Bitu i = 0; i < static_cast<Bitu>(cheight / banks); i++) {
    for (Bitu x = 0; x < copy; x++) {
      for (Bitu b = 0; b < banks; b++)
        mem_writeb(dest + b * 8 * 1024 + x, attr);
    }
    dest += nextline;
  }
}
...
static void EGA16_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                          PhysPt base, uint8_t attr) {
  /* EGA 640x350 4-color needs bits 0+1 and 2+3 ORed together as the IBM EGA
   * BIOS does [https://ibmmuseum.com/Adapters/Video/EGA/IBM_EGA_Manual.pdf] */
  if (machine == MCH_EGA && vga.mem.memsize < (128 * 1024) &&
      CurMode->sheight == 350)
    attr = EGA4ColorRemap(attr);
  ...
  for (; copy > 0; copy--) {
    for (Bitu x = 0; x < rowsize; x++)
      mem_writeb(dest + x, 0xff);
    dest += nextline;
  }
  IO_Write(0x3cf, 0);
}

static void VGA_FillRow(uint8_t cleft, uint8_t cright, uint8_t row, PhysPt base,
                        uint8_t attr) {
  /* Write some bytes */
  BIOS_CHEIGHT;
...
static void PC98_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                         PhysPt base, uint8_t attr) {
  /* Do some filing */
  PhysPt dest;
...
static void TEXT_FillRow(uint8_t cleft, uint8_t cright, uint8_t row,
                         PhysPt base, uint8_t attr, uint8_t ncols) {
  /* Do some filing */
  PhysPt dest;
...
void DOSV_FillScreen() {
  ...
  for (int i = 0; i < nrows; i++)
    DOSV_Text_FillRow(0, ncols, i, 7);
  if (IsExtendAttributeMode())
    memset(ExtendAttribute, 0, sizeof(ExtendAttribute));
}

static uint16_t font_net_data[2][16] = {
    {0x2449, 0x0000, 0x0000, 0x2449, 0x0000, 0x0000, 0x2449, 0x0000, 0x0000,
     0x2449, 0x0000, 0x0000, 0x2449, 0x0000, 0x0000, 0x2449},
    {0x0000, 0x4444, 0x0000, 0x0000, 0x0000, 0x4444, 0x0000, 0x0000, 0x0000,
     0x4444, 0x0000, 0x0000, 0x0000, 0x4444, 0x0000, 0x0000}};
...
void WriteCharDOSVSbcs24(uint8_t col, uint8_t row, uint8_t chr, uint8_t attr) {
  ...
  for (uint8_t y = 0; y < 24; y++) {
    ...
    for (uint8_t x = 0; x < 2; x++) {
      ...
      IO_Write(0x3ce, 8);
      IO_Write(0x3cf, data[x] ^ mask_data[col & 1][x]);
      IO_Write(0x3ce, 0);
      IO_Write(0x3cf, back);
      dummy = real_readb(0xa000, off);
      real_writeb(0xa000, off, 0xff);
...
void WriteCharDOSVDbcs24(uint16_t col, uint16_t row, uint16_t chr,
                         uint8_t attr) {
  uint8_t back, select;
  uint8_t *font;
  Bitu off;
  ...
  uint8_t data[4];
  uint8_t mask_data[2][4] = {{0xff, 0xff, 0xff, 0x00},
                             {0x0f, 0xff, 0xff, 0xf0}};
  for (uint8_t y = 0; y < 24; y++) {
    if (col & 1) {
      data[0] = *font >> 4;
...
static void DrawCharDOSVDbcsHalf(Bitu off, uint8_t *font, uint8_t attr,
                                 Bitu width, uint8_t height, uint8_t select,
                                 bool xor_flag) {
  volatile uint8_t dummy;
  uint8_t data;
  if (xor_flag) {
...
static inline void DrawCharDOSVDbcs(Bitu off, uint16_t *font, uint8_t attr,
                                    Bitu width, uint8_t height,
                                    uint8_t select) {
...
void WriteCharDOSVDbcs(uint16_t col, uint16_t row, uint16_t chr, uint8_t attr) {
  ...
  if (IS_DOSV && !IS_JDOSV && col == pcol + 2 && row == prow &&
      CheckBoxDrawing(pchr / 0x100, pchr & 0xFF, chr / 0x100, chr & 0xFF)) {
...
void WriteCharTopView(uint16_t off, int count) {
  ...
  while (count > 0) {
    ...
    if (isKanji1(code) && isKanji2(real_readb(seg, off + 2)) &&
        (IS_JDOSV || dos.loaded_codepage == 932 || col < width - 1)) {
      off += 2;
      if (IS_J3100 && J3_IsJapanese())
        WriteCharDCGADbcs(col, row,
                          ((uint16_t)code << 8) | real_readb(seg, off), attr);
      else {
        prevattr = attr;
        WriteCharDOSVDbcs(col, row,
                          ((uint16_t)code << 8) | real_readb(seg, off), attr);
      }
      count--;
      col++;
...
void INT10_ScrollWindow(uint8_t rul, uint8_t cul, uint8_t rlr, uint8_t clr,
                        int8_t nlines, uint8_t attr, uint8_t page) {
  ...
  if (page == 0xff)
    base += real_readw(BIOSMEM_SEG, BIOSMEM_CURRENT_START);
  else
  ...
  while (start != end) {
    ...
    switch (CurMode->type) {
    case M_PC98:
      PC98_CopyRow(cul, clr, start, start + nlines, base);
      break;
    case M_TEXT:
      TEXT_CopyRow(cul, clr, start, start + nlines, base, ncols);
      break;
    case M_DCGA:
...
filling:
  ...
  for (; nlines > 0; nlines--) {
    switch (CurMode->type) {
    case M_PC98:
      PC98_FillRow(cul, clr, start, base, attr);
      break;
    case M_TEXT:
      TEXT_FillRow(cul, clr, start, base, attr, ncols);
      break;
    case M_DCGA:
    ...
    case M_EGA:
      if (IS_DOSV && DOSV_CheckCJKVideoMode()) {
        DOSV_Text_FillRow(cul, clr, start, attr);
        WriteCharTopView(real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 2 * start +
                             cul * 2,
                         clr - cul);
      } else
...
void INT10_SetActivePage(uint8_t page) {
  ...
  if (page > 7)
    LOG(LOG_INT10, LOG_ERROR)("INT10_SetActivePage page %d", page);
...
void INT10_SetCursorShape(uint8_t first, uint8_t last) {
  ...
  if (machine == MCH_HERC ||
      !(real_readb(BIOSMEM_SEG, BIOSMEM_VIDEO_CTL) &
        0x8)) { /* if video subsystem is ACTIVE (bit is cleared)
                   [https://www.stanislavs.org/helppc/bios_data_area.html] */
    ...
    /* FIXME: Some sources including
     * [https://www.stanislavs.org/helppc/bios_data_area.html]
     * [https://www.matrix-bios.nl/system/bda.html] suggest CGA alphanumeric
     * cursor emulation occurs when bit 0 is SET. This code checks whether the
     * bit is CLEARED. This test and emulation may have the bit backwards.
     * VERIFY ON REAL HARDWARE -- J.C */
    if (machine == MCH_HERC || !(real_readb(BIOSMEM_SEG, BIOSMEM_VIDEO_CTL) &
                                 0x1)) { // set by int10 fun12 sub34
      //          if (CurMode->mode>0x3) goto dowrite;    //Only mode 0-3 are
      //          text modes on cga
      if ((first & 0xe0) || (last & 0xe0))
        goto dowrite;
      uint8_t cheight = ((machine == MCH_HERC)
                             ? 14
      ...
      if (last < first) {
        ...
        first = last;
        last = cheight;
        /* Test if this might be a cga style cursor set, if not don't do
         * anything */
...
dowrite:
  uint16_t base = real_readw(BIOSMEM_SEG, BIOSMEM_CRTC_ADDRESS);
  IO_Write(base, 0xa);
  IO_Write(base + 1u, first);
  IO_Write(base, 0xb);
  IO_Write(base + 1u, last);
}
...
void INT10_SetCursorPos(uint8_t row, uint8_t col, uint8_t page) {
  ...
  //          The DOS installer for Elder Scrolla Arena is obviously trying to
  //          put something on the last row of the display, however it sets the
  //          cursor position to row=25. row is 0-based so that would actually
  //          put it one row beyond the end of the display. This fixes the
  //          "Press F1 for"... message at the bottom.
  // Assume MDA/CGA do not range check because the rows value in the BDA did not
  // exist in the original IBM PC/XT BIOS.
  ...
  if (IS_DOSV && DOSV_CheckCJKVideoMode())
    ...
    J3_OffCursor();
  if (page > 7)
    LOG(LOG_INT10, LOG_ERROR)("INT10_SetCursorPos page %d", page);
  // Bios cursor pos
  if (IS_PC98_ARCH) {
  ...
  if (page == current) {
    ...
    uint16_t address =
        (ncols * row) + col +
        (IS_PC98_ARCH ? 0
                      : (real_readw(BIOSMEM_SEG, BIOSMEM_CURRENT_START) / 2));
    if (IS_PC98_ARCH) {
...
void ReadCharAttr(uint16_t col, uint16_t row, uint8_t page, uint16_t *result) {
  ...
  default:
    if (isJEGAEnabled()) {
      ...
      *result = real_readw(
          GetTextSeg(),
          (row * real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) + col) * 2);
      return;
    }
    fontdata = RealGetVec(0x43);
    break;
  ...
  for (uint16_t chr = 0; chr < 256; chr++) {
    ...
    for (uint8_t h = 0; h < cheight; h++) {
      ...
      if (bitline != vidline) {
        ...
        fontdata =
            RealMake(RealSeg(fontdata), RealOff(fontdata) + cheight - h - 1);
        error = true;
        break;
      }
    }
...
void INT10_ReadString(uint8_t row, uint8_t col, uint8_t flag, uint8_t attr,
  ...
  while (count > 0) {
    ...
    if (flag == 0x11) {
      mem_writeb(
          string,
          IsExtendAttributeMode()
              ? ExtendAttribute[row * real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) +
                                col]
              : 0);
      string++;
      mem_writeb(string, 0);
...
void INT10_PC98_CurMode_Relocate(void) { /* deprecated */ }

/* Draw DBCS char in graphics mode*/
void WriteCharJ(uint16_t col, uint16_t row, uint8_t page, uint8_t chr,
...
void WriteCharV(uint16_t col, uint16_t row, uint8_t chr, uint8_t attr,
                bool useattr) {
  DOSV_OffCursor();
...
void WriteChar(uint16_t col, uint16_t row, uint8_t page, uint16_t chr,
  ...
  PhysPt fontdata;
  uint16_t cols = real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
  uint8_t back, cheight = IS_PC98_ARCH ? 16
                          : (machine == MCH_CGA || machine == MCH_AMSTRAD)
                              ? 8
                              : real_readb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT);
  ...
  case M_DCGA: {
    if (J3_IsJapanese()) {
      J3_OffCursor();
      real_writeb(BIOSMEM_J3_SEG, BIOSMEM_J3_MODE,
                  real_readb(BIOSMEM_J3_SEG, BIOSMEM_J3_MODE) | 0x20);
      uint16_t seg = GetTextSeg();
    ...
    if (J3_IsJapanese()) {
      real_writeb(BIOSMEM_J3_SEG, BIOSMEM_J3_MODE,
                  real_readb(BIOSMEM_J3_SEG, BIOSMEM_J3_MODE) & 0xdf);
    }
  }
    return;
  case M_CGA4:
    ...
    if (isJEGAEnabled()) {
      ...
      if (isKanji1(chr) && prevchr == 0 && (IS_JDOSV || col < width - 1)) {
        ...
        prow = row;
                 ...
                 ((IS_JDOSV & !col) || (col && row == prow))) {
        WriteCharDOSVDbcs((col ? col : cols) - 1, row - (!row || col ? 0 : 1),
                          (prevchr << 8) | chr, attr);
        prevchr = 0;
        return;
      } else
  ...
  Bitu x = col * 8u, y = (unsigned int)(row * (unsigned int)cheight *
                                        (cols / CurMode->twidth));
...
void INT10_WriteChar(uint16_t chr, uint8_t attr, uint8_t page, uint16_t count,
                     bool showattr) {
...
static void INT10_TeletypeOutputAttr(uint8_t chr, uint8_t attr, bool useattr,
                                     uint8_t page) {
  BIOS_NCOLS;
  BIOS_NROWS;
  uint8_t cur_row = CURSOR_POS_ROW(page);
  ...
  case 7: /* Beep */
    ...
    return; /* don't do anything else, not even scrollup on last line (fix for
               Elder Scrolls Arena installer) */
  default:
    /* Return if the char code is DBCS at the end of the line (for DOS/V) */
    if (cur_col + 1 == ncols && IS_DOSV && DOSV_CheckCJKVideoMode() &&
        isKanji1(chr) && prevchr == 0) {
  ...
  if (cur_row == nrows) {
    ...
    INT10_ScrollWindow(0, 0, (uint8_t)(nrows - 1), (uint8_t)(ncols - 1), -1,
                       fill, page);
    cur_row--;
  }
...
void INT10_TeletypeOutputAttr(uint8_t chr, uint8_t attr, bool useattr) {
  INT10_TeletypeOutputAttr(chr, attr, useattr,
...
#define EXTEND_ATTRIBUTE_ALL                                                   \
  (EXTEND_ATTRIBUTE_HORIZON_LINE | EXTEND_ATTRIBUTE_VERTICAL_LINE |            \
   EXTEND_ATTRIBUTE_UNDER_LINE)

static void DrawExtendAttribute(uint16_t col, uint16_t row, uint8_t attr,
                                uint8_t ex_attr) {
  Bitu off, off_start;
  volatile uint8_t dummy;
  Bitu width = real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
  ...
  if (ex_attr & EXTEND_ATTRIBUTE_HORIZON_LINE) {
    ...
    if (height == 24) {
      IO_Write(0x3ce, 0x08);
      IO_Write(0x3cf, mask_data[col & 1][0]);
      dummy = real_readb(0xa000, off);
      real_writeb(0xa000, off, 0x07);
      off++;
      select = CheckBankSelect(select, off);
      IO_Write(0x3ce, 0x08);
      IO_Write(0x3cf, mask_data[col & 1][1]);
      dummy = real_readb(0xa000, off);
      real_writeb(0xa000, off, 0x07);
    } else {
      IO_Write(0x3ce, 0x08);
...
void INT10_WriteString(uint8_t row, uint8_t col, uint8_t flag, uint8_t attr,
  ...
  while (count > 0) {
    ...
    string++;
    if ((flag & 2) != 0 ||
        (DOSV_CheckCJKVideoMode() && (flag == 0x20 || flag == 0x21))) {
      attr = mem_readb(string);
      string++;
      if (flag == 0x21) {
        ex_attr = mem_readb(string);
        string += 2;
        ExtendAttribute[row * real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) + col] =
            ex_attr;
      }
    }
...
bool INT10_GetInsertState() {
  if (IS_PC98_ARCH) {
    ...
    const auto state =
        static_cast<bool>(flags & BIOS_KEYBOARD_FLAGS1_INSERT_ACTIVE);
    return state;
  }
}
