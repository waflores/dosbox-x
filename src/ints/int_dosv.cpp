...
const char jfont_name[] =
    "\x082\x06c\x082\x072\x020\x083\x053\x083\x056\x083\x062\x083\x04e";
...
bool isKanji1(uint8_t chr) {
  ...
  else if (dos.loaded_codepage == 949 || dos.loaded_codepage == 950 ||
           dos.loaded_codepage == 951 || IS_KDOSV || IS_TDOSV)
...
#if defined(USE_TTF)
  if (dos.loaded_codepage == 936 || dos.loaded_codepage == 949 ||
      dos.loaded_codepage == 950 || dos.loaded_codepage == 951 ||
      ((IS_DOSV || ttf_dosv) && !IS_JDOSV))
#else
  if (dos.loaded_codepage == 936 || dos.loaded_codepage == 949 ||
      dos.loaded_codepage == 950 || dos.loaded_codepage == 951 ||
      (IS_DOSV && !IS_JDOSV))
#endif
    return chr >= (dos.loaded_codepage == 936 && !gbk ? 0xa1 : 0x40) &&
           chr <= 0xfe;
  else
    return (chr >= 0x40 && chr <= 0x7e) || (del_flag && chr == 0x7f) ||
           (chr >= 0x80 && chr <= 0xfc);
}
...
extern "C" {
#endif
bool tfd_isKanji1(uint8_t chr) { return isKanji1(chr); }
bool tfd_isKanji2(uint8_t chr) { return isKanji2(chr); }
#ifdef __cplusplus
}
#endif
...
int32_t read_int32(FILE *file) {
  return format.is_little_endian() ? read_int32_little(file)
                                   : read_int32_big(file);
}
...
void bit_order_invert(uint8_t *data, int size) {
  static const uint8_t invert[16] = {0, 8, 4, 12, 2, 10, 6, 14,
                                     1, 9, 5, 13, 3, 11, 7, 15};
...
void read_metric(FILE *file, metric_t *m, bool compressed) {
  m->leftSideBearing =
      compressed ? (int16_t)((uint8_t)read8(file) - 0x80) : read_int16(file);
  m->rightSideBearing =
      compressed ? (int16_t)((uint8_t)read8(file) - 0x80) : read_int16(file);
  m->characterWidth =
      compressed ? (int16_t)((uint8_t)read8(file) - 0x80) : read_int16(file);
...
bool readPCF(FILE *file, int height) {
  ...
  if (read_bytes == 0)
    version = read_int32_big(file);
  if (version != make_int32(1, 'f', 'c', 'p'))
    return false;
  nTables = read_int32_little(file);
  if ((tables = new table_t[nTables]) == NULL)
    return false;
  for (i = 0; i < nTables; i++) {
  ...
  if (!seek(file, PCF_PROPERTIES) || !(format.id == PCF_DEFAULT_FORMAT) ||
      !seek(file, PCF_METRICS))
    return false;
  ...
  case PCF_DEFAULT_FORMAT:
    ...
    if ((metrics = new metric_t[nMetrics]) == NULL)
      return false;
  ...
  if ((bitmapOffsets = new uint32_t[nBitmaps]) == NULL)
    return false;
  ...
  if ((bitmaps = new uint8_t[bitmapSize]) == NULL)
    return false;
  if (read_byte8s(file, bitmaps, bitmapSize) == NULL) {
    delete[] bitmaps;
    return false;
  }
  ...
  if ((format.bit == format.byte) != (BDF_format.bit == BDF_format.byte)) {
    switch (1 << (BDF_format.bit == BDF_format.byte ? format.scan
                                                    : BDF_format.scan)) {
    case 1:
      break;
    case 2:
  ...
  if (!seek(file, PCF_BDF_ENCODINGS)) {
    delete[] bitmaps;
    return false;
  }
  format = read_format32_little(file);
  ...
  uint16_t *encodings;
  if ((encodings = new uint16_t[nEncodings]) == NULL) {
    delete[] bitmaps;
    return false;
  }
  for (i = 0; i < nEncodings; i++)
    encodings[i] = read_int16(file);
  ...
  if (height == 16) {
    Prop_path *pathprop =
        static_cast<Section_prop *>(control->GetSection("dosv"))
            ->Get_path("fontxdbcs14");
    if (pathprop && !pathprop->realpath.size())
      apply14 = true;
  }
  for (i = 0; i < nEncodings; i++) {
    if (encodings[i] == 0xffff)
      continue;
    ...
    if ((m.height() == height || m.height() == 15) && charcode >= 0x100 &&
        charcode <= 0xffff) {
      ...
      if (CodePageHostToGuestUTF16(text, uname)) {
        ...
        if ((height == 14 || (height == 16 && m.height() == 15 && apply14)) &&
            jfont_cache_dbcs_14[code] == 0) {
...
bool readBDF(FILE *file, int height) {
  ...
  int fontboundingbox_width, fontboundingbox_height, fontboundingbox_xoff,
      fontboundingbox_yoff;
  int chars, i, j, n, scanline, encoding, bbx, bby, bbw, bbh, width;
  unsigned *width_table, *encoding_table;
  unsigned char *bitmap;
  fontboundingbox_width = fontboundingbox_height = fontboundingbox_xoff =
      fontboundingbox_yoff = chars = 0;
  ...
  if (fontboundingbox_width <= 0 || fontboundingbox_height <= 0 || chars <= 0)
    return false;
  width_table = (unsigned *)malloc(chars * sizeof(*width_table));
  encoding_table = (unsigned *)malloc(chars * sizeof(*encoding_table));
  bitmap = (unsigned char *)malloc(((fontboundingbox_width + 7) / 8) *
                                   fontboundingbox_height);
  if (!width_table || !encoding_table || !bitmap)
    return false;
  scanline = encoding = -1;
  n = bbx = bby = bbw = bbh = 0;
  width = INT_MIN;
      ...
      if (bbx && !(bbx < 0 || bbx > 7)) {
        ...
        for (y = 0; y < fontboundingbox_height; ++y) {
          ...
          for (x = 0; x < fontboundingbox_width; x += 8) {
            ...
            bitmap[y * ((fontboundingbox_width + 7) / 8) + x / 8] =
                c >> bbx | o;
            o = c << (8 - bbx);
          }
        }
      }
      if ((width == height || width == 15) && encoding >= 0x100 &&
          encoding <= 0xffff) {
        char text[10];
        uint16_t uname[4];
        uname[0] = encoding;
        ...
        if (CodePageHostToGuestUTF16(text, uname)) {
          Bitu code = (text[0] & 0xff) * 0x100 + (text[1] & 0xff);
          if ((height == 14 || (height == 16 && width == 15 && apply14)) &&
              jfont_cache_dbcs_14[code] == 0) {
            memcpy(&jfont_dbcs_14[code * 28], bitmap, 28);
            jfont_cache_dbcs_14[code] = 1;
          }
...
bool LoadFontxFile(const char *fname, int height, bool dbcs) {
  ...
  if (!mfile && exepath.size())
    mfile = fopen((exepath + fname).c_str(), "rb");
  if (!mfile && config_path.size())
    mfile = fopen((config_path + fname).c_str(), "rb");
  if (!mfile && res_path.size())
    mfile = fopen((res_path + fname).c_str(), "rb");
  if (!mfile) {
#if defined(LINUX)
  ...
  if (getfontx2header(mfile, &head) != 0) {
    if (!dbcs) {
      ...
      if (sz == 256 * 15) {
        if (height == 16)
          for (int i = 0; i < 256; i++)
            fread(&jfont_sbcs_16[i * 16], sizeof(uint8_t), 15, mfile);
        else if (height == 19)
          for (int i = 0; i < 256; i++)
            fread(&jfont_sbcs_19[i * 19 + 2], sizeof(uint8_t), 15, mfile);
    ...
    } else if ((height == 14 || height == 16) && isDBCSCP() &&
               (readBDF(mfile, height) || readPCF(mfile, height))) {
      fclose(mfile);
      return true;
...
static uint8_t linux_symbol_16[] = {
    ...
    0x03, 0xe0, 0x04, 0x90, 0x08, 0x80, 0x08, 0x80, 0x08, 0x80, 0x08, 0x80,
    0x04, 0x90, 0x03, 0xe0, 0x00, 0x80, 0x00, 0x00,
    // 0x8192
    0x00, 0x00, 0x01, 0xe0, 0x02, 0x10, 0x04, 0x00, 0x04, 0x00, 0x04, 0x00,
    0x04, 0x00, 0x04, 0x00, 0x3f, 0xe0, 0x04, 0x00, 0x04, 0x00, 0x04, 0x00,
    0x04, 0x00, 0x08, 0x00, 0x1f, 0xf8, 0x00, 0x00,
    // 0x81ca
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x3f, 0xfc, 0x00, 0x04, 0x00, 0x04, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00,
...
static uint8_t linux_symbol_14[] = {
    ...
    0x00, 0x08, 0x00, 0x04,
    // 0x8191
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x0f, 0x80, 0x12, 0x40,
    0x22, 0x00, 0x22, 0x00, 0x22, 0x00, 0x22, 0x00, 0x12, 0x40, 0x0f, 0x80,
    0x02, 0x00, 0x00, 0x00,
    // 0x8192
    0x00, 0x00, 0x03, 0xc0, 0x04, 0x20, 0x08, 0x00, 0x08, 0x00, 0x08, 0x00,
    0x08, 0x00, 0x3f, 0xc0, 0x08, 0x00, 0x08, 0x00, 0x08, 0x00, 0x10, 0x00,
    0x3f, 0xf0, 0x00, 0x00,
    // 0x81ca
...
static uint8_t linux_symbol_24[] = {
    ...
    0x00, 0x08, 0x00, 0x00, 0x7f, 0x00, 0x00, 0xc8, 0x80, 0x01, 0x88, 0xc0,
    0x01, 0x88, 0x40, 0x03, 0x08, 0x00, 0x03, 0x08, 0x00, 0x03, 0x08, 0x00,
    0x03, 0x08, 0x00, 0x01, 0x88, 0x40, 0x01, 0x88, 0xc0, 0x00, 0xc9, 0x80,
    0x00, 0x7f, 0x00, 0x00, 0x08, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00,
    // 0x8192
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3f, 0x00, 0x00, 0x40, 0x80,
    0x00, 0x80, 0x40, 0x01, 0x80, 0x00, 0x01, 0x80, 0x00, 0x01, 0x80, 0x00,
    0x01, 0x80, 0x00, 0x01, 0x80, 0x00, 0x01, 0x80, 0x00, 0x01, 0x80, 0x00,
    0x01, 0x80, 0x00, 0x1f, 0xff, 0x00, 0x01, 0x80, 0x00, 0x01, 0x80, 0x00,
    0x01, 0x80, 0x00, 0x01, 0x80, 0x00, 0x03, 0x80, 0x00, 0x03, 0x00, 0x00,
    0x07, 0x00, 0x00, 0x0f, 0xff, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // 0x81ca
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1f, 0xff, 0xf8, 0x00, 0x00, 0x08,
...
bool GetWindowsFont(Bitu code, uint8_t *buff, int width, int height) {
  ...
  size_t len;
  wchar_t wtext[4];
  if ((height == 24 && font_set24 == NULL) ||
      (height == 14 && font_set14 == NULL) ||
      (height != 24 && height != 14 && font_set16 == NULL))
    return false;
  if (code < 0x100) {
    if (code == 0x5c && (IS_JDOSV || dos.loaded_codepage == 932)) // yen
      wtext[0] = 0xa5;
    else if (code >= 0xa1 && code <= 0xdf &&
             (IS_JDOSV || dos.loaded_codepage == 932)) // half kana
    ...
    src[1] = code & 0xff;
    src[2] = 0;
    if (!CodePageGuestToHostUTF16((uint16_t *)wtext, src))
      return false;
  }
  ...
  if (xfont_16 != NULL && width == 16) {
    ...
    if (lr.width <= width)
      return false;
    XwcDrawString(font_display, font_pixmap, fontset, font_gc, 0,
                  lr.height - (ir.height + ir.y), wtext, len);
  }
  image =
      XGetImage(font_display, font_pixmap, (height == 14 && IS_PDOSV) ? 1 : 0,
                0, width, lr.height, ~0, XYPixmap);
  if (image != NULL) {
    int x, y;
    for (y = 0; y < height; y++) {
      uint8_t data = 0;
  ...
  if (font != NULL) {
    ...
    if (size > 0) {
      ...
      for (Bitu y = off_y; y < off_y + gm.gmBlackBoxY; y++) {
        ...
        for (Bitu x = gm.gmptGlyphOrigin.x;
             x < gm.gmptGlyphOrigin.x + gm.gmBlackBoxX; x++) {
          uint8_t src = *((uint8_t *)fontbuff + count * (y - off_y) +
                          ((x - gm.gmptGlyphOrigin.x) / 8));
...
void GetDbcsFrameFont(Bitu code, uint8_t *buff) {
  if (!IS_JDOSV && dos.loaded_codepage != 932)
    code = GetConvertedCode(code, 932, true);
...
bool isFrameFont(int code, int height) {
  ...
  if ((height == 14 || height == 16) &&
      (code == -0x2550 || code == -0x2551 || code == -0x2554 ||
       code == -0x2557 || code == -0x255a || code == -0x255d))
    return true;
  return code >= 0x849f && code <= 0x84be;
}

bool isUserFont(Bitu code) {
  return ((code >= 0x8140 && code <= 0xa0fe) ||
          (code >= 0xc6a1 && code <= 0xc8fe) ||
          (code >= 0xfa40 && code <= 0xfefe)) &&
         (IS_TDOSV || dos.loaded_codepage == 950 ||
          dos.loaded_codepage == 951) &&
         !chinasea;
}
...
uint8_t *GetDbcsFont(Bitu code) {
  ...
  if (jfont_cache_dbcs_16[code] == 0) {
    if (fontdata16 && fontsize16) {
      if (dos.loaded_codepage == 936 && !(fontsize16 % 16) &&
          (code / 0x100) > 0xa0 && (code / 0x100) < 0xff &&
          (code % 0x100) > 0xa0 && (code % 0x100) < 0xfe) {
        int offset = (94 * (unsigned int)((code / 0x100) - 0xa0 - 1) +
                      ((code % 0x100) - 0xa0 - 1)) *
                     32;
      ...
      if (((dos.loaded_codepage == 936 && gbk) || dos.loaded_codepage == 950 ||
           dos.loaded_codepage == 951) &&
          !(fontsize16 % 15) && isKanji1(code / 0x100)) {
        Bitu c = code;
        if (dos.loaded_codepage == 936)
          code = GetConvertedCode(code, 950);
        int offset = -1,
            ser = (code / 0x100 - 161) * 157 +
                  ((code % 0x100) - ((code % 0x100) > 160 ? 161 : 64)) +
                  ((code % 0x100) > 160 ? 64 : 1);
        if (code) {
          if (ser >= 472 && ser <= 5872)
            offset = (ser - 472) * 30;
          else if (ser >= 6281 && ser <= 13973)
            offset = (ser - 6281) * 30 + 162030;
        }
        code = c;
        if (offset > -1) {
        ...
        if (!code) {
          if (!getwqy16) {
            ...
            if (readBDF(mfile, 16) || readPCF(mfile, 16)) {
              fclose(mfile);
              if (jfont_cache_dbcs_16[oldcode] != 0)
                return &jfont_dbcs_16[oldcode * 32];
            } else
              fclose(mfile);
          }
          return jfont_dbcs;
...
uint8_t *GetDbcs14Font(Bitu code, bool &is14) {
  ...
  if (jfont_cache_dbcs_14[code] == 0) {
    if (fontdata14 && fontsize14) {
      if (dos.loaded_codepage == 936 && !(fontsize14 % 14) &&
          (code / 0x100) > 0xa0 && (code / 0x100) < 0xff &&
          (code % 0x100) > 0xa0 && (code % 0x100) < 0xfe) {
        int offset = (94 * (unsigned int)((code / 0x100) - 0xa0 - 1) +
                      ((code % 0x100) - 0xa0 - 1)) *
      ...
      if (((dos.loaded_codepage == 936 && gbk) || dos.loaded_codepage == 950 ||
           dos.loaded_codepage == 951) &&
          !(fontsize14 % 15) && isKanji1(code / 0x100)) {
        Bitu c = code;
        if (dos.loaded_codepage == 936)
          code = GetConvertedCode(code, 950);
        int offset = -1,
        ...
        if (!code) {
          if (!getwqy14) {
            ...
            if (readBDF(mfile, 14) || readPCF(mfile, 14)) {
              fclose(mfile);
              if (jfont_cache_dbcs_14[oldcode] != 0)
                return &jfont_dbcs_14[oldcode * 28];
            } else
              fclose(mfile);
          }
          is14 = false;
...
uint8_t *GetDbcs24Font(Bitu code) {
  ...
  if (jfont_cache_dbcs_24[code] == 0) {
    if (fontdata24 && fontsize24) {
      if (dos.loaded_codepage == 936 && !(fontsize24 % 24) &&
          (code / 0x100) > 0xa0 && (code / 0x100) < 0xff &&
          (code % 0x100) > 0xa0 && (code % 0x100) < 0xfe) {
        int offset = (94 * (unsigned int)((code / 0x100) - 0xa0 - 1) +
                      ((code % 0x100) - 0xa0 - 1)) *
      ...
      } else if ((dos.loaded_codepage == 950 || dos.loaded_codepage == 951) &&
                 !(fontsize24 % 24) && isKanji1(code / 0x100)) {
        int offset = -1,
            ser = (code / 0x100 - 161) * 157 +
                  ((code % 0x100) - ((code % 0x100) > 160 ? 161 : 64)) +
                  ((code % 0x100) > 160 ? 64 : 1);
    ...
    if (code >= 0x809e && code < 0x80fe) {
      if (GetWindowsFont(code - 0x807e, jfont_dbcs, 12, 24)) {
        ...
        for (no = 0; no < 24; no++) {
          ...
          jfont_dbcs_24[pos + no * 3 + 1] =
              jfont_dbcs[no * 2 + 1] | (jfont_dbcs[no * 2] >> 4);
          jfont_dbcs_24[pos + no * 3 + 2] =
              (jfont_dbcs[no * 2] << 4) | (jfont_dbcs[no * 2 + 1] >> 4);
        }
        jfont_cache_dbcs_24[code] = 1;
...
uint8_t *GetSbcsFont(Bitu code) { return &jfont_sbcs_16[code * 16]; }

uint8_t *GetSbcs19Font(Bitu code) { return &jfont_sbcs_19[code * 19]; }

uint8_t *GetSbcs24Font(Bitu code) { return &jfont_sbcs_24[code * 2 * 24]; }
...
void InitFontHandle() {
  ...
  if (font_display) {
    if (!font_set16) {
      if (IS_TDOSV) {
        xfont_16 = XLoadQueryFont(
            font_display,
            "-wenquanyi-*-medium-r-normal-*-16-*-*-*-*-*-iso10646-*");
      }
      font_set16 =
          XCreateFontSet(font_display, "-*-*-medium-r-normal--16-*-*-*",
                         &missing_list, &missing_count, &def_string);
      XFreeStringList(missing_list);
    }
    if (!font_set14) {
      if (IS_TDOSV) {
        font_set14 = XCreateFontSet(font_display,
                                    "-wenquanyi-*-medium-r-normal--14-*-*-*",
                                    &missing_list, &missing_count, &def_string);
      } else {
        font_set14 =
            XCreateFontSet(font_display, "-*-*-medium-r-normal--14-*-*-*",
                           &missing_list, &missing_count, &def_string);
      }
      XFreeStringList(missing_list);
    }
    if (!font_set24) {
      font_set24 =
          XCreateFontSet(font_display, "-*-*-medium-r-normal--24-*-*-*",
                         &missing_list, &missing_count, &def_string);
      XFreeStringList(missing_list);
    }
    if (!font_window) {
      font_window = XCreateSimpleWindow(
          font_display, DefaultRootWindow(font_display), 0, 0, 32, 32, 0,
          BlackPixel(font_display, DefaultScreen(font_display)),
          WhitePixel(font_display, DefaultScreen(font_display)));
      font_pixmap = XCreatePixmap(font_display, font_window, 32, 32,
                                  DefaultDepth(font_display, 0));
  ...
  if (jfont_16 == NULL || jfont_14 == NULL || jfont_24 == NULL) {
    ...
    lf.lfHeight = 16;
    lf.lfCharSet =
        IS_KDOSV || (!IS_DOSV && dos.loaded_codepage == 949)
            ? HANGUL_CHARSET
            : (IS_PDOSV || (!IS_DOSV && dos.loaded_codepage == 936)
                   ? GB2312_CHARSET
                   : (IS_TDOSV || (!IS_DOSV && (dos.loaded_codepage == 950 ||
                                                dos.loaded_codepage == 951))
                          ? CHINESEBIG5_CHARSET
                          : SHIFTJIS_CHARSET));
...
bool MakeSbcs16Font() {
  ...
  if (fail)
    memcpy(jfont_sbcs_16, JPNHN16X + NAME_LEN + ID_LEN + 3, SBCS16_LEN);
  if (IS_JDOSV || IS_JEGA_ARCH)
    memcpy(jfont_sbcs_16, dosv_font16_data, sizeof(dosv_font16_data));
  else if (IS_DOSV)
    for (Bitu ct = 0; ct < 0x100; ct++)
      memcpy(&jfont_sbcs_16[ct * 16], &int10_font_16[ct * 16], 16);
  return true;
}
...
bool MakeSbcs19Font() {
  ...
  if (fail)
    memcpy(jfont_sbcs_19, JPNHN19X + NAME_LEN + ID_LEN + 3, SBCS19_LEN);
  if (IS_JDOSV)
    ...
    for (Bitu ct = 0; ct < 0x100; ct++)
      memcpy(&jfont_sbcs_19[ct * 19], &int10_font_19[ct * 19], 19);
  return true;
}
...
bool MakeSbcs24Font() {
  ...
  if (fail)
    memcpy(jfont_sbcs_24, JPNHN24X + NAME_LEN + ID_LEN + 3, SBCS24_LEN);
  if (IS_JDOSV || IS_JEGA_ARCH)
    memcpy(jfont_sbcs_24, dosv_font24_data, sizeof(dosv_font24_data));
  else if (IS_DOSV)
    for (Bitu ct = 0; ct < 0x100; ct++)
      memcpy(&jfont_sbcs_24[ct * 24 * 2], &int10_font_24[ct * 24 * 2], 48);
  return true;
}
    ...
    if (pathprop && !reinit) {
      ...
      if (!LoadFontxFile(path.c_str(), 24, false)) {
        ...
        if (!CheckEmptyData(&jfont_sbcs_24[0x7f * 2 * 24], 2 * 24))
          memcpy(&jfont_sbcs_24[0x5c * 2 * 24], &jfont_sbcs_24[0x7f * 2 * 24],
                 2 * 24);
      }
    } else if (!reinit) {
      if (!MakeSbcs24Font()) {
...
uint8_t GetTrueVideoMode() { return TrueVideoMode; }

void SetTrueVideoMode(uint8_t mode) { TrueVideoMode = mode; }

bool DOSV_CheckJapaneseVideoMode() {
  if (IS_DOS_JAPANESE && (TrueVideoMode == 0x03 || TrueVideoMode == 0x12 ||
                          (TrueVideoMode >= 0x70 && TrueVideoMode <= 0x73) ||
                          TrueVideoMode == 0x78)) {
    return true;
  }
  return false;
...
bool DOSV_CheckCJKVideoMode() {
  if (IS_DOS_CJK && (TrueVideoMode == 0x03 || TrueVideoMode == 0x12 ||
                     (TrueVideoMode >= 0x70 && TrueVideoMode <= 0x73) ||
                     TrueVideoMode == 0x78)) {
    return true;
  }
...
static CallBack_Handler font_handler_list[] = {
    font8x16,    font8x19,        font16x16,       font12x24,
    font24x24,   write_font16x16, write_font24x24,
...
static Bitu dosv_cursor_24[] = {0, 3, 8, 12, 15, 18, 21, 23};

static Bitu dosv_cursor_19[] = {0, 3, 6, 9, 11, 13, 16, 18};

static Bitu dosv_cursor_16[] = {0, 3, 5, 7, 9, 11, 13, 15};
...
void DOSV_CursorXor24(Bitu x, Bitu y, Bitu start, Bitu end) {
  ...
  while (start <= end) {
    if (dosv_cursor_stat == 2) {
      uint8_t cursor_data[2][4] = {{0xff, 0xff, 0xff, 0x00},
                                   {0x0f, 0xff, 0xff, 0xf0}};
...
void DOSV_CursorXor(Bitu x, Bitu y) {
  ...
  if (start != 0x20 && start <= end) {
    ...
    if (height == 24) {
      if (start < 8)
        start = dosv_cursor_24[start];
      if (end < 8)
        end = dosv_cursor_24[end];
      ...
      if (end == 28) {
        ...
        if (start < 8)
          start = dosv_cursor_19[start];
        if (end < 8)
          end = dosv_cursor_19[end];
      }
    } else if (height == 16) {
      if (end == 28) {
        end = 15;
        ...
        if (start < 8)
          start = dosv_cursor_16[start];
        if (end < 8)
          end = dosv_cursor_16[end];
      }
    }
    IO_Write(0x3ce, 0x05);
    IO_Write(0x3cf, 0x03);
...
uint16_t GetTextSeg() { return jtext_seg; }
...
uint8_t GetKanjiAttr() {
  return GetKanjiAttr(real_readb(BIOSMEM_SEG, BIOSMEM_CURSOR_POS),
                      real_readb(BIOSMEM_SEG, BIOSMEM_CURSOR_POS + 1));
}
...
void INT8_DOSV() {
  ...
  if (!CheckAnotherDisplayDriver() &&
      real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) != 0x72 &&
      real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) != 0x12) {
    if (dosv_cursor_stat == 0) {
      Bitu x = real_readb(BIOSMEM_SEG, BIOSMEM_CURSOR_POS);
      Bitu y = real_readb(BIOSMEM_SEG, BIOSMEM_CURSOR_POS + 1) *
               real_readb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT);
...
enum DOSV_FEP_CTRL DOSV_GetFepCtrl() { return dosv_fep_ctrl; }
...
void DOSV_Setup() {
  ...
  for (Bitu ct = 0; ct < DOSV_FONT_MAX; ct++) {
    ...
    dosv_font_handler_offset[ct] =
        CALLBACK_PhysPointer(dosv_font_handler[ct]) & 0xffff;
    phys_writeb(CALLBACK_PhysPointer(dosv_font_handler[ct]) + 0, 0xFE);
    phys_writeb(CALLBACK_PhysPointer(dosv_font_handler[ct]) + 1, 0x38);
    phys_writew(CALLBACK_PhysPointer(dosv_font_handler[ct]) + 2,
                (uint16_t)dosv_font_handler[ct]);
...
uint16_t GetGaijiSeg() { return gaiji_seg; }
  ...
  for (code = 0; code < 256; code++) {
    for (int y = 0; y < 16; y++) {
      if (code >= 0x20 && code < 0x80) {
        phys_writeb(0xf0000 + j3_font_offset + code * 16 + y,
                    jfont_sbcs_16[code * 16 + y]);
        if (code == 0x5c) {
...
void INT8_J3() {
  ...
  j3_timer++;
  if ((j3_timer & 0x03) == 0) {
    if ((real_readb(BIOSMEM_J3_SEG, BIOSMEM_J3_BLINK) & 0x01) == 0 ||
        j3_cursor_stat == 0) {
...
static Bitu text_color_list[colorMax] = {0x3963f7, 0x3a4b51, 0xff321b,
                                         0xffffff};

static Bitu back_color_list[colorMax] = {0xbed7d4, 0xeaf3f2, 0x6a1d22,
                                         0x000000};
...
static struct J3_MACHINE_LIST {
  ...
  enum J3_COLOR color;
    ...
    {"gt", 0x3130, colorPlasma},    {"sgt", 0xfc27, colorPlasma},
    {"gx", 0xfc2d, colorPlasma},    {"gl", 0xfc2b, colorLcdBlue},
    {"sl", 0xfc2c, colorLcdBlue},   {"sgx", 0xfc26, colorPlasma},
    {"ss", 0x3131, colorLcdBlue},   {"gs", 0xfc2a, colorLcdBlue},
    {"sx", 0xfc36, colorLcdBlue},   {"sxb", 0xfc36, colorLcdBlue},
    {"sxw", 0xfc36, colorLcdWhite}, {"sxp", 0xfc36, colorPlasma},
    {"ez", 0xfc87, colorLcdWhite},  {"zs", 0xfc25, colorNormal},
    {"zx", 0xfc4e, colorNormal},    {NULL, 0, colorMax}};

uint16_t J3_GetMachineCode() { return j3_machine_code; }
