...
void INT10_PutPixel(uint16_t x, uint16_t y, uint8_t page, uint8_t color) {
  ...
  case M_CGA2:
    if (machine == MCH_MCGA &&
        real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) == 0x11) {
  ...
  case M_TANDY16: {
    ...
    bool is_32k =
        (real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) >= 9) ? true : false;
    ...
    if (is_32k) {
      ...
      offset = ((unsigned int)y >> 2u) * ((unsigned int)CurMode->swidth >> 1u) +
               ((unsigned int)x >> 1u);
      ...
      offset = ((unsigned int)y >> 1u) * ((unsigned int)CurMode->swidth >> 1u) +
               ((unsigned int)x >> 1u);
  ...
  case M_EGA: {
    ...
    if (CurMode->plength != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE))
      LOG(LOG_INT10,
          LOG_ERROR)("PutPixel_EGA_p: %x!=%x", (int)CurMode->plength,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE));
    if (CurMode->swidth != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8)
      LOG(LOG_INT10,
          LOG_ERROR)("PutPixel_EGA_w: %x!=%x", (int)CurMode->swidth,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8);
    PhysPt off =
        0xa0000u +
        (unsigned int)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE) * page +
        (((unsigned int)y * real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8u +
          (unsigned int)x) >>
         3u);
  ...
  case M_LIN8: {
    if (CurMode->swidth != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8u)
      LOG(LOG_INT10,
          LOG_ERROR)("PutPixel_VGA_w: %x!=%x", (int)CurMode->swidth,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8u);
    PhysPt off =
        S3_LFB_BASE +
        y * (unsigned int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8u + x;
  ...
  default:
    if (GCC_UNLIKELY(!putpixelwarned)) {
      putpixelwarned = true;
      LOG(LOG_INT10, LOG_ERROR)("PutPixel unhandled mode type %d",
                                (int)CurMode->type);
    }
    break;
  }
...
void INT10_GetPixel(uint16_t x, uint16_t y, uint8_t page, uint8_t *color) {
  ...
  case M_EGA: {
    ...
    if (CurMode->plength != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE))
      LOG(LOG_INT10,
          LOG_ERROR)("GetPixel_EGA_p: %x!=%x", (int)CurMode->plength,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE));
    if (CurMode->swidth != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8)
      LOG(LOG_INT10,
          LOG_ERROR)("GetPixel_EGA_w: %x!=%x", (int)CurMode->swidth,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8);
    PhysPt off =
        0xa0000u +
        (unsigned int)real_readw(BIOSMEM_SEG, BIOSMEM_PAGE_SIZE) * page +
        (((unsigned int)y *
              (unsigned int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8u +
          (unsigned int)x) >>
         3u);
    Bitu shift = 7u - (x & 7u);
    /* Set the read map */
  ...
  case M_LIN8: {
    if (CurMode->swidth != (Bitu)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8)
      LOG(LOG_INT10,
          LOG_ERROR)("GetPixel_VGA_w: %x!=%x", (int)CurMode->swidth,
                     (int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) * 8);
    PhysPt off = S3_LFB_BASE +
                 (unsigned int)y *
                     (unsigned int)real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS) *
                     8u +
                 (unsigned int)x;
    *color = mem_readb(off);
    break;
  ...
  default:
    LOG(LOG_INT10, LOG_ERROR)("GetPixel unhandled mode type %d",
                              (int)CurMode->type);
    break;
  }
}
