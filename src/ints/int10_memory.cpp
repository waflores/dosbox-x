...
static uint8_t static_functionality[0x10] = {
    ...
    /* b */ 0x0e, // Support for Display combination, intensity/blinking and
                  // video state saving/restoring
    /* c */ 0x00, // reserved
    /* d */ 0x00, // reserved
...
static uint16_t map_offset[8] = {0x0000, 0x4000, 0x8000, 0xc000,
...
void INT10_LoadFont(PhysPt font, bool reload, uint16_t count, Bitu offset,
  ...
  PhysPt ftwhere =
      PhysMake(0xa000, map_offset[map & 0x7] + (uint16_t)(offset * 32));
  ...
  IO_Write(0x3c4, 0x04);
  IO_Write(0x3c5, 0x05 | m64k); // odd/even off in SEQ
  IO_Write(0x3ce, 0x04);
  ...
  IO_Write(0x3c4, 0x04);
  IO_Write(0x3c5, 0x01 | m64k); // odd/even on in SEQ
  IO_Write(0x3ce, 0x04);
  ...
  if (reload) {
    ...
    IO_Write(base, 0x9);
    IO_Write(base + 1u,
             (IO_Read(base + 1u) & 0xe0u) | ((unsigned int)height - 1u));
    // Vertical display end
    Bitu rows = CurMode->sheight / height;
    Bitu vdend =
        rows * height * ((CurMode->sheight == 200 && IS_VGA_ARCH) ? 2 : 1) - 1;
    IO_Write(base, 0x12);
    ...
    // Cursor shape
    if (height >= 14)
      height--; // move up one line on 14+ line fonts
    INT10_SetCursorShape(height - 2, height - 1);
    // Clip the cursor within range in case it is now beyond the new height. To
    // fix 28.COM and 50.COM (issue #1324 and #1325).
    // FIXME: Remove this code *IF* it turns out BIOSes do not clip the row
    // value when calling INT 10h AX=1111h/1113h
    //        If BIOSes are inconsistent about it, note which ones do and don't
    //        and try to match behavior with machine= type. At the very least,
    //        if the IBM PS/2 VGA BIOS does not, then this code should be coded
    //        NOT to clip the cursor when machine=vgaonly.
    {
...
// FIXME: ripsaw8080 noted that while the svgaCard==SVGA_None conditions are
// appropriate for DOSBox SVN because the vgaonly machine type is alone in
// displaying 9-dot character cells, but in DOSBox-X (as in DOSBox SVN Daum) the
// char9= setting should be respected. As it is now, users will not get the
// wider symbols when char9= is true and machine= is an SVGA machine type. SVN
// commit reference: https://sourceforge.net/p/dosbox/code-0/3932/
void INT10_ReloadFont(void) {
  ...
  case 8:
    ...
    break;
  case 14:
    if (IS_VGA_ARCH && svgaCard == SVGA_None && CurMode->mode == 7)
      map = 0x80;
    INT10_LoadFont(Real2Phys(int10.rom.font_14), false, 256, 0, map, 14);
    break;
  case 19:
    if (IS_EGA_ARCH && svgaCard == SVGA_None)
      map = 0x80;
...
void INT10_SetupRomMemory(void) {
  /* if no space allocated for video BIOS (such as machine=cga) then return
   * immediately */
  if (VGA_BIOS_Size == 0) {
    int10.rom.font_8_first = RealMake(
        0xF000, 0xFA6E); /* why duplicate data? use the copy in the ROM BIOS */
    ...
    if (IS_EGAVGA_ARCH || VIDEO_BIOS_enable_CGA_8x8_second_half) {
      ...
      if (rom == 0)
        E_Exit("Unable to alloc CGA 8x8 second half");
      int10.rom.font_8_second =
          RealMake((unsigned int)rom >> 4u, (unsigned int)rom & 0xFu);
    ...
    if (machine == MCH_MCGA) {
      Bitu base =
          ROMBIOS_GetMemory((Bitu)(256 * 16), "MCGA 16-line font", 1, 0u);
      if (base == 0)
        E_Exit("Unable to alloc MCGA 16x font");

      for (unsigned int i = 0; i < 256 * 16; i++)
        phys_writeb((PhysPt)base + i, int10_font_16_mcga[i]);

      int10.rom.font_16 =
          RealMake((uint16_t)(base >> 4u), (uint16_t)(base & 0xF));

      // MCGA has the pointer at 40:A8 (BIOSMEM_VS_POINTER), confirmed on real
      // hardware. It points into the BIOS, because MCGA systems do not have a
      // BIOS at C000:0000
      Bitu vptr = ROMBIOS_GetMemory(
          (Bitu)(0x600), "MCGA video save pointer and structs", 1, 0u);
      Bitu vptrseg = vptr >> 4;
      Bitu vptroff = vptr & 0xF;
      vptr -= vptroff;
      Bitu vptroff_limit = vptroff + 0x600;

      int10.rom.video_parameter_table =
          RealMake((uint16_t)vptrseg, (uint16_t)vptroff);
      ...
      int10.rom.video_save_pointers =
          RealMake((uint16_t)vptrseg, (uint16_t)vptroff);
      phys_writed((PhysPt)(vptr + vptroff), int10.rom.video_parameter_table);
      vptroff += 4;
      phys_writed(
          (PhysPt)(vptr + vptroff),
          int10.rom.video_dynamic_save_area); // dynamic save area pointer
      vptroff += 4;
      phys_writed((PhysPt)(vptr + vptroff),
                  0); // alphanumeric character set override
      vptroff += 4;
      phys_writed((PhysPt)(vptr + vptroff),
                  0); // graphics character set override
      vptroff += 4;
      phys_writed((PhysPt)(vptr + vptroff), 0); // secondary save pointer table
      vptroff += 4;
      phys_writed((PhysPt)(vptr + vptroff), 0);
      vptroff += 4;
      phys_writed((PhysPt)(vptr + vptroff), 0);
      vptroff += 4;

      if (vptroff > vptroff_limit)
        E_Exit("MCGA ptr overrun");
    }
  ...
  if (IS_EGAVGA_ARCH) {
    ...
    if (VGA_ROM_BIOS_ENTRY_cb == 0) {
      ...
      CALLBACK_Setup(VGA_ROM_BIOS_ENTRY_cb, VGA_ROM_BIOS_ENTRY_callback_func,
                     CB_RETF, "VGA ROM BIOS boot up entry point");
    }
    ...
    if (IS_VGA_ARCH) {
      ...
      case SVGA_ParadisePVGA1A:
        ...
        break;
      case SVGA_ATI:
        /* Visit the PCem project and browse their ROM collection, examine the
         * ATI BIOS images and notice they only put "IBM" up there. See also
         * this ATI VGA Wonder XL VGA ROM BIOS image here:
         * [https://archive.org/details/ati_vgawonder_xl_rom]
         *
         * See also this documentation:
         * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/ATI%2c%20Array%20Technology%20Inc/ATI%20Technologies%20Super%20VGA%20Chipsets%20%28WHATVGA%20v2%2e00%29%20%281995%2d09%2d29%29%2etxt]
         *
         * See also the WHATVGA.EXE source code (written in Pascal) */
        /* FIXME: There is a "atimach64vt2pci.bin" image where the signature at
         * 0x31 is "3?" instead of "31" and byte 0x43 is 0x40 instead of 0x20.
         * Why? */
        phys_writew(rom_base + 0x10,
                    0x1CE); /*ATI extended registers I/O base port*/
        phys_writes(rom_base + 0x31, "761295520",
                    9 /*length of string*/); /* ATI product ID */
        phys_writes(rom_base + 0x40, "31",
                    2 /*length of string*/); /* ATI Wonder/Mach series */
        switch (atiCard) {
        case ATI_EGAVGAWonder: // ATI 18800 EGA/VGA Wonder
          phys_writeb(
              rom_base + 0x43,
              0x31); // NTS: the 18800-1 uses code 0x32 according to WHATVGA
          break;
        case ATI_VGAWonder: // ATI 28800-1 VGA Wonder
          phys_writeb(rom_base + 0x43,
                      0x32); // NTS: 28800-2 is 0x33, what is 28800-1?
          break;
        ...
        case ATI_Mach8:                       // ATI 38800-1
          phys_writeb(rom_base + 0x43, 0x38); // FIXME: GUESS because '8' might
                                              // make sense for Mach8, I dunno
          break;
        ...
        case ATI_Mach64: // ATI 88800GX
          phys_writeb(rom_base + 0x43,
                      0x20); // confirmed by PCem rom image mach64g/bios.bin
          break;
        default:
          break;
        };
    ...
    //
    // SuperCalc 3 and 4 fails to detect EGA/VGA if INT 10h points at a location
    // higher than segment 0xFE00 in the system BIOS. It will also not attempt
    // EGA/VGA detect unless there is an adapter ROM signature (AA55h) at
    // 0xC000:0x0000. The check does not attempt EGA/VGA detection if ((segment
    // >> 4) + offset) >= 0xFE00.
    //
    // The idea is to point INT 10h at this JMP so that DOS programs like
    // SuperCalc will think there is in fact an EGA/VGA BIOS and that INT 10h is
    // provided by the EGA/VGA BIOS so it can function normally.
    //
    ...
    if (int10_vga_bios_vector) {
      ...
      if (biosint10 != 0) {
        LOG(LOG_MISC,
            LOG_DEBUG)("Redirecting INT 10h to point at the VGA BIOS");
        ...
        /* WARNING: This overwrites the INT 10 startup code's vector
         * successfully only because this code is called AFTER it has
         * initialized the INT 10h vector. If initialization order changes this
         * could stop working. */
  ...
  if (IS_EGAVGA_ARCH || VIDEO_BIOS_always_carry_14_high_font) {
    ...
    int10.rom.font_14 =
        0; /* why write the 14-high version if not emulating EGA/VGA? */
  ...
  if (IS_VGA_ARCH || VIDEO_BIOS_always_carry_16_high_font) {
    ...
    int10.rom.font_16 =
        0; /* why write the 16-high version if not emulating VGA? */
    int10.rom.font_16_alternate = 0;
    /* FIXME: Does the EGA BIOS have the 16-high font, or just 14-high? */
  }
  if (IS_VGA_ARCH) { /* according to sources this static state stuff only
                        applies to VGA, right? */
  ...
  if (IS_EGAVGA_ARCH) {
    ...
    if (IS_VGA_ARCH) {
      ...
      int10.rom.used += 4;
      phys_writed(rom_base + int10.rom.used,
                  0); // alphanumeric charset override
      int10.rom.used += 4;
    ...
    if (IS_VGA_ARCH) {
      phys_writed(rom_base + int10.rom.used,
                  int10.rom.video_save_pointer_table);
    } else {
...
void INT10_SetupRomMemoryChecksum(void) {
  if (IS_EGAVGA_ARCH) { // EGA/VGA. Just to be safe
    ...
    uint8_t sum = 0;
    PhysPt rom_base = PhysMake(0xc000, 0);
    unsigned int last_rombyte = (unsigned int)VGA_BIOS_Size - 1; // 32 KB
                                                                 // romsize
...
uint8_t int10_font_19[256 * 19] = {
    ...
    ,
    ...
    0xDB, 0xDB, 0xDB, 0xDB, 0x7B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x00,
    ...
    0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x3C, 0x7E, 0x18,
    ...
    0xC0, 0xC0, 0xC0, 0xC0, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x3C, 0x3C, 0x3C, 0x3C, 0x18,
    ...
    0x6C, 0x6C, 0x6C, 0x6C, 0xFE, 0x6C, 0x6C, 0x6C, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x18, 0x18, 0x7C, 0xC6, 0xC2, 0xC0, 0xC0, 0x7C, 0x06, 0x06, 0x06,
    0x86, 0xC6, 0x7C, 0x18, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC6, 0xC6,
    0xCC, 0x0C, 0x18, 0x18, 0x30, 0x30, 0x60, 0x66, 0xC6, 0xC6, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x6C, 0x6C, 0x6C, 0x38, 0x76, 0xDC,
    0xDC, 0xCC, 0xCC, 0xCC, 0x76, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18,
    0x18, 0x18, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x18, 0x30, 0x30, 0x30,
    0x30, 0x30, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C,
    ...
    ,
    ...
    ,
    ...
    ,
    ...
    ,
    ...
    0xDD, 0x77, 0xDD, 0x77, 0xDD, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0xF8, 0x18, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x18, 0xF8, 0x18, 0xF8, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0xF6,
    ...
    0xF8, 0x18, 0xF8, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x36,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0xF6, 0x06, 0xF6, 0x36, 0x36,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    0x36, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFE, 0x06, 0xF6,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    0x36, 0x36, 0x36, 0x36, 0xF6, 0x06, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0xFF, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x1F, 0x18, 0x1F, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
    0x18, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x37, 0x36,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    ...
    0x00, 0x00, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0xF7, 0x00,
    0xF7, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x18, 0x18, 0x18,
    0x18, 0x18, 0x18, 0x18, 0x18, 0xFF, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1F,
    0x18, 0x1F, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3F, 0x36, 0x36, 0x36, 0x36,
    0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    0x36, 0x36, 0xFF, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36, 0x36,
    ...
    ,
    ...
    0x0C, 0x0C, 0x0C, 0xEC, 0x6C, 0x6C, 0x3C, 0x1C, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0xD8, 0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x6C, 0x0C,
    0x18, 0x30, 0x64, 0x7C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
uint8_t int10_font_14_alternate[20 * 15 + 1] = {
    ...
    };
...
uint8_t int10_font_16_alternate[19 * 17 + 1] = {
    ...
    0xcc, 0x18, 0x30, 0x66, 0xce, 0x96, 0x3e, 0x06, 0x06, 0x00, 0x00, 0x00};
