...
void INT10_ReadString(uint8_t row, uint8_t col, uint8_t flag, uint8_t attr,
...
Bitu INT10_Handler(void) {
  // NTS: We do have to check the "current video mode" from the BIOS data area
  // every call.
  //      Some OSes like Windows 95 rely on overwriting the "current video mode"
  //      byte in the BIOS data area to play tricks with the BIOS. If we don't
  //      call this, tricks like the Windows 95 boot logo or INT 10h
  //      virtualization in Windows 3.1/9x/ME within the DOS "box" will not work
  //      properly.
  if (IS_DOSV && DOSV_CheckCJKVideoMode() && reg_ah != 0x03)
    DOSV_OffCursor();
  ...
  case 0x00: /* Set VideoMode */
    ...
    if (IS_DOSV && IS_DOS_CJK &&
        (reg_al == 0x03 || (reg_al >= 0x70 && reg_al <= 0x73) ||
         reg_al == 0x78)) {
      uint8_t mode = 0x03;
      if (reg_al == 0x03 || reg_al == 0x72 || reg_al == 0x73) {
        ...
        mode = 0x70;
        enum DOSV_VTEXT_MODE vtext_mode =
            DOSV_GetVtextMode((reg_al == 0x78) ? 1 : 0);
        if (vtext_mode == DOSV_VTEXT_XGA || vtext_mode == DOSV_VTEXT_XGA_24) {
        ...
        } else if (vtext_mode == DOSV_VTEXT_SXGA ||
                   vtext_mode == DOSV_VTEXT_SXGA_24) {
          if (svgaCard == SVGA_TsengET4K) {
          ...
          INT10_SetVideoMode((svgaCard == SVGA_TsengET3K)        ? 0x29
                             : (svgaCard == SVGA_ParadisePVGA1A) ? 0x58
                                                                 : 0x6a);
          INT10_SetDOSVModeVtext(mode, vtext_mode);
        } else {
          INT10_SetVideoMode(0x12);
  ...
  case 0x05: /* Set Active Page */
    ...
    break;
  case 0x06: /* Scroll Up */
    INT10_ScrollWindow(reg_ch, reg_cl, reg_dh, reg_dl, -(int8_t)reg_al, reg_bh,
                       0xFF);
    break;
  ...
  case 0x10: /* Palette functions */
    if (machine == MCH_MCGA) {
      if (!(reg_al == 0x10 || reg_al == 0x12 || reg_al == 0x15 ||
            reg_al == 0x17 || reg_al == 0x18 || reg_al == 0x19))
        break;
    } else if (machine == MCH_PCJR) {
      if (reg_al > 0x02) /* "Looking at the PCjr tech ref page A-61, ... the
                            BIOS listing stops at subfunction 2." */
        break;
    } else {
      if (!IS_EGAVGA_ARCH && (reg_al > 0x02))
        break;
      else if (!IS_VGA_ARCH && (reg_al > 0x03))
        break;
    }
    ...
    case 0xF0:
    case 0xF1:
    case 0xF2: /* ET4000 Sierra HiColor DAC support */
    ...
    default:
      LOG(LOG_INT10, LOG_ERROR)(
          "Function 10:Unhandled EGA/VGA Palette Function %2X", reg_al);
      break;
    }
    break;
  case 0x11: /* Character generator functions */
    ...
    case 0x10:
      INT10_LoadFont(SegPhys(es) + reg_bp, reg_al == 0x10, reg_cx, reg_dx,
                     reg_bl & 0x7f, reg_bh);
      break;
    case 0x01: /* Load 8x14 font */
    case 0x11:
      INT10_LoadFont(Real2Phys(int10.rom.font_14), reg_al == 0x11, 256, 0,
                     reg_bl & 0x7f, 14);
      break;
    case 0x02: /* Load 8x8 font */
    case 0x12:
#if defined(USE_TTF)
      if (ttf.inUse && reg_al == 0x12 &&
          (real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) == 0x03 ||
           real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_MODE) == 0x55)) {
...
#endif
      INT10_LoadFont(Real2Phys(int10.rom.font_8_first), reg_al == 0x12, 256, 0,
                     reg_bl & 0x7f, 8);
      break;
    case 0x03: /* Set Block Specifier */
      ...
      if (ttf.inUse && wpType == 1) {
        DOS_Block dos;
        if (mem_readd(((dos.psp() - 1) << 4) + 8) ==
            0x5057) // Name of MCB PSP should be WP
          wpExtChar = reg_bl != 0;
      }
#endif
      IO_Write(0x3c4, 0x3);
    ...
    case 0x14:
      if (!IS_VGA_ARCH)
        break;
      INT10_LoadFont(Real2Phys(int10.rom.font_16), reg_al == 0x14, 256, 0,
                     reg_bl & 0x7f, 16);
      break;
      /* Graphics mode calls */
    ...
    case 0x30: /* Get Font Information */
      ...
      case 0x07: /* alpha alternate 9x16 */
        ...
        break;
      default:
        LOG(LOG_INT10, LOG_ERROR)("Function 11:30 Request for font %2X",
                                  reg_bh);
        break;
      }
    ...
    default:
      LOG(LOG_INT10, LOG_ERROR)(
          "Function 11:Unsupported character generator call %2X", reg_al);
      break;
    }
    if ((reg_al & 0xf0) == 0x10)
  ...
  case 0x12: /* alternate function select */
    ...
    if (reg_bh == 0x55 && IS_VGA_ARCH &&
        svgaCard ==
            SVGA_ATI) { /* ATI VGA ALTERNATE FUNC SELECT ENHANCED FEATURES */
      /* NTS: When WHATVGA detects an ATI chipset, all modesetting within the
       * program REQUIRES this function or else it does not change the video
       * mode. This function is heavily relied on by WHATVGA to detect whether
       * or not the video mode exists, INCLUDING the base VGA standard modes! */
      if (!IS_VGA_ARCH)
        break;
      ...
      case 0x02: /*get status*/
        reg_al = 0x08 /*enhanced features enabled*/ |
                 (2 /*multisync monitor*/ << 5u);
        break;
      case 0x06: { /*get mode table*/
        unsigned char video_mode = reg_al;
        /* entry: AL = video mode
         * exit:  If mode valid: ES:BP = pointer to parameter table (ATI mode
         * table), ES:SI = pointer to parameter table supplement If mode
         * invalid: ES and BP unchanged, SI unchanged
         *
         * Also DI == 0?
         * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20IBM%20compatible/Video/VGA/SVGA/ATI%2c%20Array%20Technology%20Inc/ATI%20Mach64%20BIOS%20Kit%20Technical%20Reference%20Manuals%20PN%20BIO%2dC012XX1%2d05%20%281994%29%20v5%2epdf]
         */
        /* FIXME! Need to return pointers to REAL table data!
         *        This satisfies WHATVGA for now which only cares that this
         * interrupt call signals the mode exists, although the video mode info
         * it shows is understandably weird and wacky. */
        /* TODO: Scan an actual parameter table, including one that includes the
         * vendor specific INT 10h modes for SVGA, here */
        LOG(LOG_INT10, LOG_DEBUG)(
            "ATI VGA INT 12h Get Mode Table for mode 0x%2x", video_mode);
        if ((video_mode >= 0 && video_mode <= 7) ||
            (video_mode >= 13 && video_mode <= 19)) { // standard VGA for now!
          SegSet16(es, 0xABCD);
          reg_bp = 0x1234;
        }
      } break;
      ...
      default:
        LOG(LOG_INT10,
            LOG_DEBUG)("Unhandled ATI VGA INT 12h function: AX=%04x BX=%04x",
                       reg_ax, reg_bx);
        break;
      }
    }
    ...
    case 0x10: /* Get EGA Information */
      if (machine != MCH_MCGA) {
        /* This call makes no sense on MCGA systems because it's designed to
         * return EGA information.
         *
         * Furthermore the MS-DOS port of Thexder calls this function and
         * validates the values returned from this call in a way that suggests
         * MCGA BIOSes do not respond to this call:
         *
         ...
         * 0302:0000038B 7708                ja   00000395 ($+8)         (no
         * jmp) 0302:0000038D 0AFF                or   bh,bh 0302:0000038F 7504
         * jne  00000395 ($+4)         (no jmp) 0302:00000391 8D16CD02 lea
         * dx,[02CD]              ds:[02CD]=616D 0302:00000395 E82C00 call
         * 000003C4 ($+2c)
         *
         * Basically all checks confirm the returned values are out of range and
         * invalid for what the BIOS call would normally return for EGA. Either
         * MCGA BIOS leaves them unchanged or changes them to some other value
         * entirely.
         *
    ...
    case 0x30: /* Select vertical resolution */
    {
      ...
      LOG(LOG_INT10, LOG_WARN)(
          "Function 12:Call %2X (select vertical resolution)", reg_bl);
 ...
 */
  ...
  case 0x13: /* Write String */
    if ((reg_al & 0x10) != 0 && DOSV_CheckCJKVideoMode())
      INT10_ReadString(reg_dh, reg_dl, reg_al, reg_bl, SegPhys(es) + reg_bp,
                       reg_cx, reg_bh);
    else
      INT10_WriteString(reg_dh, reg_dl, reg_al, reg_bl, SegPhys(es) + reg_bp,
                        reg_cx, reg_bh);
    break;
  case 0x18:
#if defined(USE_TTF)
  ...
  case 0x4f: /* VESA Calls */
    ...
    if (int10.vesa_oldvbe10 &&
        reg_al >=
            6) /* Functions 6 and up did not exist until VBE 1.2 (or 1.1?) */
      break;
    switch (reg_al) {
    ...
    case 0x05:
      if (reg_bh == 0) { /* Set CPU Window */
        ...
        LOG(LOG_INT10, LOG_ERROR)("Unhandled VESA Function %X Subfunction %X",
                                  reg_al, reg_bh);
        reg_ah = 0x01;
      }
      break;
    ...
    case 0x08:
      ...
      case 0x00: /* Set DAC width */
        if (CurMode->type == M_LIN8) {
          /* TODO: If there is a bit on S3 cards to control DAC width in
           * "pseudocolor" modes, replace this code with something to write that
           * bit instead of internal state change like this. */
          ...
          LOG(LOG_INT10, LOG_NORMAL)(
              "VESA BIOS called to set VGA DAC width to %u bits", reg_bh);
        } else {
          reg_al = 0x4f;
          reg_ah = 0x3;
        }
    ...
    case 0x0a: /* Get Pmode Interface */
      ...
      case 0x01: /* Get code for "set window" */
        SegSet16(es, RealSeg(int10.rom.pmode_interface));
        reg_di = RealOff(int10.rom.pmode_interface) +
                 (uint32_t)int10.rom.pmode_interface_window;
        reg_cx =
            int10.rom.pmode_interface_start - int10.rom.pmode_interface_window;
        reg_ax = 0x004f;
        break;
      case 0x02: /* Get code for "set display start" */
        SegSet16(es, RealSeg(int10.rom.pmode_interface));
        reg_di = RealOff(int10.rom.pmode_interface) +
                 (uint32_t)int10.rom.pmode_interface_start;
        reg_cx =
            int10.rom.pmode_interface_palette - int10.rom.pmode_interface_start;
        reg_ax = 0x004f;
        break;
      case 0x03: /* Get code for "set palette" */
        SegSet16(es, RealSeg(int10.rom.pmode_interface));
        reg_di = RealOff(int10.rom.pmode_interface) +
                 (uint32_t)int10.rom.pmode_interface_palette;
        reg_cx =
            int10.rom.pmode_interface_size - int10.rom.pmode_interface_palette;
        reg_ax = 0x004f;
        break;
      default:
        reg_ax = 0x014f;
  ...
  case 0x50: // Set/Read JP/US mode of CRT BIOS
    ...
    case 0x00:
      ...
      if (INT10_AX_SetCRTBIOSMode(reg_bx))
        reg_al = 0x00;
      else
        reg_al = 0x01;
      break;
  ...
  case 0x51: // Save/Read JFONT pattern
    ...
    break;
  case 0x52: // Set/Read virtual text ram buffer when the video mode is JEGA
             // graphic mode
    if (INT10_AX_GetCRTBIOSMode() == 0x01)
      break; // exit if CRT BIOS is in US mode
    ...
    case 0x00: {
      if (reg_bx == 0)
        real_writew(BIOSMEM_AX_SEG, BIOSMEM_AX_VTRAM_SEGADDR, 0);
      else {
        LOG(LOG_INT10,
            LOG_NORMAL)("AX CRT BIOS set VTRAM segment address at %x", reg_bx);
    ...
    break;
  case 0x82: // Set/Read the scroll mode when the video mode is JEGA graphic
             // mode
    if (J3_IsJapanese() || (IS_J3100 && GetTrueVideoMode() == 0x75)) {
      if (reg_al == 0x00) {
        ...
        if (section && section->Get_bool("j3100colorscroll"))
          reg_bl = 0x01;
      }
      break;
    }
  ...
  case 0x83: // Read the video RAM address and virtual text video RAM
    ...
    // Out: AX=base address of video RAM, ES:BX=base address of virtual text
    // video RAM
    if (INT10_AX_GetCRTBIOSMode() == 0x01)
      break; // exit if CRT BIOS is in US mode
  ...
  case 0xf1:
    ...
    break;
  case 0xf2:
    INT10_EGA_RIL_ReadRegisterRange(reg_ch, reg_cl, reg_dx,
                                    SegPhys(es) + reg_bx);
    break;
  case 0xf3:
    INT10_EGA_RIL_WriteRegisterRange(reg_ch, reg_cl, reg_dx,
                                     SegPhys(es) + reg_bx);
    break;
  case 0xf4:
...
static void INT10_Seg40Init(void) {
  // Clear the screen.
  //
  // The byte value at 40:87 (BIOSMEM_VIDEO_CTL) only means something if
  // EGA/VGA. Should be zero for PCjr (which defines it as function key), and
  // Tandy (which does not define it), and MDA/Hercules/CGA (which also does not
  // define this byte).
  //
  // Furthermore, some games such as "Road Runner" by Mindscape use that byte as
  // part of its detection routines to differentiate between CGA, EGA, and
  // Tandy. For the game to work properly in Tandy emulation, this byte must be
  // zero.
  if (IS_EGAVGA_ARCH) {
    ...
    // NTS: This is direclty from the configuration switches. The bits are
    // inverted,
    //      1=OFF 0=ON.
    //
    ...
    //      ON  ON  ON  OFF       1000        Enhanced Display Emulation Mode
    //      (MDA secondary) OFF ON  ON  OFF       1001        Enhanced Display
    //      Hi Res Mode (MDA secondary) ON  OFF ON  OFF       1010 Monochrome
    //      (CGA is secondary 40x25) OFF OFF ON  OFF       1011 Monochrome (CGA
    //      is secondary 80x25)
    //
    ...
    real_writeb(BIOSMEM_SEG, BIOSMEM_SWITCHES,
                0xF0 | ((!IS_VGA_ARCH && ega200) ? 0x7 : 0x9));
    ...
    if (machine == MCH_TANDY || machine == MCH_PCJR)
      real_writeb(BIOSMEM_SEG, BIOSMEM_CHAR_HEIGHT,
                  8); /* FIXME: INT 10h teletext routines depend on this */
    else
  ...
  if (control->opt_display2)
    real_writeb(BIOSMEM_SEG, BIOSMEM_MODESET_CTL,
                0x10 | (DISP2_Active() ? 0 : 1));
  else
#endif
      if (IS_EGAVGA_ARCH) {
    real_writeb(BIOSMEM_SEG, BIOSMEM_MODESET_CTL,
                0x51); // why is display switching enabled (bit 6) ?
  }
  // Set the default MSR
...
static void SetupTandyBios(void) {
  if (machine == MCH_TANDY) {
    ...
    static uint8_t TandyConfig[130] = {
        };

    LOG(LOG_MISC,
        LOG_DEBUG)("Initializing Tandy video state (video BIOS init)");
...
bool Load_FONT_ROM(void) {
  ...
  FILE *fp =
      Try_Load_FontFiles({"FONT.ROM", "font.rom", "FONT.rom", "font.ROM"});
  if (!fp) {
  ...
  for (lowbyte = 0x01; lowbyte < 0x5D; lowbyte++) {
    fseek(fp, long(0x1800u + ((lowbyte - 0x01u) * 96u * 16u * 2u /*16 wide*/)),
          SEEK_SET);
    if (fread(tmp, 96 * 16 * 2 /*16 wide*/, 1, fp) != 1)
      goto fail;
...
void Load_Anex86_Font(const char *fontname, bool &sbcs_ok, bool &dbcs_ok) {
  unsigned char
      tmp[(2048 / 8) * 16]; /* enough for one 2048x16 row and bitmap header */
  unsigned int hibyte, lowbyte, r;
  ...
  if (fread(tmp, 14, 1, fp) != 1)
    goto fail; // BITMAPFILEHEADER
  if (memcmp(tmp, "BM", 2) != 0)
    goto fail;                              // must be "BM"
  bmp_ofs = host_readd((HostPt)(tmp + 10)); // bOffBits

  if (fread(tmp, 40, 1, fp) != 1)
    goto fail; // BITMAPINFOHEADER
  if (host_readd((HostPt)(tmp + 0)) != 40)
    goto fail; // biSize == 40 or else
  if (host_readd((HostPt)(tmp + 4)) != 2048)
    goto fail; // biWidth == 2048 or else
  if (host_readd((HostPt)(tmp + 8)) != 2048)
    goto fail; // biHeight == 2048 or else
  if (host_readw((HostPt)(tmp + 12)) != 1)
    goto fail; // biPlanes == 1 or else
  if (host_readw((HostPt)(tmp + 14)) != 1)
    goto fail; // biBitCount == 1 or else
  if (host_readd((HostPt)(tmp + 16)) != 0)
    goto fail; // biCompression == 0 or else

  /* first row is 8x16 single width */
  fseek(fp, long(bmp_ofs + ((2048u - 16u) * (2048u / 8u))),
        SEEK_SET); /* arrrgh bitmaps are upside-down */
  if (fread(tmp, (2048 / 8) * 16, 1, fp) != 1)
    goto fail;
  if (!sbcs_ok) {
    for (lowbyte = 0; lowbyte < 256; lowbyte++) {
      for (r = 0; r < 16; r++) {
        vga.draw.font[(lowbyte * 16) + r] =
            tmp[lowbyte + ((15 - r /*upside-down!*/) * (2048 / 8))] ^
            0xFF /*ANEX86 has inverse color scheme*/;
      }
    }
    sbcs_ok = true;
  }
  ...
  if (!dbcs_ok) {
    for (hibyte = 1; hibyte < 128; hibyte++) {
      fseek(fp, long(bmp_ofs + ((2048u - (16u * hibyte) - 16u) * (2048u / 8u))),
            SEEK_SET); /* arrrgh bitmaps are upside-down */
      if (fread(tmp, (2048 / 8) * 16, 1, fp) != 1)
        goto fail;
...
void Load_FontX2_As_PC98(Section_prop *section, bool &sbcs_ok, bool &dbcs_ok) {
  ...
  if (pathprop) {
    ...
    if (LoadFontxFile(path.c_str(), 16, false)) {
      ...
      bool flag = true;
      // In the case of a normal FONTX2 file for DOS/V, character codes 80h to
      // AFh are empty. For FONTX2 files created using MKXFNT98.EXE, character
      // codes 80h to AFh are not empty.
      ...
      // When judged as a FONTX2 file for DOS/V, character codes 00h to 1Fh, 80h
      // to AFh, and E0h to FFh use font data for PC-98.
      if (flag || pc98_symbol) {
        memcpy(&vga.draw.font, &pc98_freecg_sbcs, 0x20 * 16);
        memcpy(&vga.draw.font[0x80 * 16], &pc98_freecg_sbcs[0x80 * 16],
               0x20 * 16);
        memcpy(&vga.draw.font[0xe0 * 16], &pc98_freecg_sbcs[0xe0 * 16],
               0x20 * 16);
      }
      sbcs_ok = true;
    }
...
Bitu INT10_AX_GetCRTBIOSMode(void) {
  if (!IS_JEGA_ARCH)
    return 0x01;
  if (real_readb(BIOSMEM_AX_SEG, BIOSMEM_AX_JPNSTATUS) & 0x80)
    return 0x51; // if in US mode
  else
    return 0x01;
}
...
Bitu INT16_AX_GetKBDBIOSMode(void) {
  if (!IS_JEGA_ARCH)
    return 0x01;
  if (real_readb(BIOSMEM_AX_SEG, BIOSMEM_AX_JPNSTATUS) & 0x40)
    return 0x51; // if in US mode
  else
    return 0x01;
}
...
void INT10_Startup(Section *sec) {
  ...
  vesa_set_display_vsync_wait =
      video_section->Get_int("vesa set display vsync");
  vesa_bank_switch_window_range_check =
      video_section->Get_bool("vesa bank switching window range check");
  vesa_bank_switch_window_mirror =
      video_section->Get_bool("vesa bank switching window mirroring");
  vesa_zero_on_get_information =
      video_section->Get_bool("vesa zero buffer on get information");
  unmask_irq0_on_int10_setmode =
      video_section->Get_bool("unmask timer on int 10 setmode");
  int16_unmask_irq1_on_read =
      static_cast<Section_prop *>(control->GetSection("dosbox"))
          ->Get_bool("unmask keyboard on int 16 read");
  int16_ah_01_cf_undoc =
      static_cast<Section_prop *>(control->GetSection("dosbox"))
          ->Get_bool("int16 keyboard polling undocumented cf behavior");
  ...
  if (!IS_PC98_ARCH) {
    if (!VGA_BIOS_use_rom) {
      ...
      LOG(LOG_MISC, LOG_DEBUG)("INT 10: VGA bios used %d / %d memory",
                               (int)int10.rom.used, (int)VGA_BIOS_Size);
      if (int10.rom.used > VGA_BIOS_Size &&
          size_override >
              -512) /* <- this is fatal, it means the Setup() functions
                       scrozzled over the adjacent ROM or RAM area */
        E_Exit("VGA BIOS size too small %u > %u", (unsigned int)int10.rom.used,
               (unsigned int)VGA_BIOS_Size);
    }

    /* If machine=vgaonly, make a dummy stub routine at 0xF000:0xF065 if the
     * user is using the stock IBM VGA ROM BIOS. It calls INT 42h for unknown
     * functions. INT 42h is NOT the previous INT 10h handler but is a hardcoded
     * address specific to VGA-based IBM PCs of the time period. */
    if (IS_VGA_ARCH && svgaCard == SVGA_None && VGA_BIOS_use_rom) {
      LOG(LOG_MISC, LOG_DEBUG)("Creating dummy IBM BIOS INT 10h stub for VGA "
                               "BIOS ROM image at F000:F065");
      // TODO: This may have to identify the ROM image by file name in case
      // other ROM images have other
      //       specific requirements or hardcoded addresses.
      PhysPt base =
          ROMBIOS_GetMemory(3, "IBM BIOS dummy INT 10h stub for VGA BIOS",
                            /*align*/ 1, 0xFF065); /* 0xF000:0xF065 */
      phys_writew(base + 0, 0xC031);               // XOR AX,AX
      phys_writeb(base + 2, 0xCF);                 // IRET
    }

    /* NTS: Uh, this does seem bass-ackwards... INT 10h making the VGA BIOS
     * appear. Can we refactor this a bit? */
    if (VGA_BIOS_Size > 0) {
      LOG(LOG_MISC, LOG_DEBUG)("VGA BIOS occupies segment 0x%04x-0x%04x",
    ...
    /* load PC-98 character ROM data, if possible */
    {
      ...
      if (!sbcs_ok)
        Load_FontX2_As_PC98(pc98_section, sbcs_ok, dbcs_ok);
      /* We can use ANEX86.BMP from the Anex86 emulator */
      if (!sbcs_ok || !dbcs_ok)
        Load_Anex86_Font(anex86_font, sbcs_ok, dbcs_ok);
      /* Failing all else we can use the internal FREECG 8x16 font and default
       * Japanese font to show text on the screen. */
      if (!sbcs_ok || !dbcs_ok)
        Load_JFont_As_PC98(sbcs_ok, dbcs_ok);
    }

    CurMode = &PC98_Mode;

    /* FIXME: This belongs in MS-DOS kernel init, because these reside in the
     * CON driver */
    /* Some PC-98 game behavior seems to suggest the BIOS data area stretches
     * all the way from segment 0x40:0x00 to segment 0x7F:0x0F inclusive.
     * Compare that to IBM PC platform, where segment fills only 0x40:0x00 to
     * 0x50:0x00 inclusive and extra state is held in the "Extended BIOS Data
     * Area".
     */
     ...
     * Touhou Project will not clear/format the text layer properly without this
     * variable. */
    ...
    real_writeb(0x60, 0x112,
                25 - 1 - 1);        /* scroll range lower limit (usually 23 when
                                       function key row is visible) */
