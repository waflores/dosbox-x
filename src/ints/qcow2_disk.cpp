...
// Public Constructor.
QCow2Image::QCow2Image(QCow2Image::QCow2Header &qcow2Header, FILE *qcow2File,
                       const char *imageName, uint32_t sectorSizeBytes)
    : file(qcow2File), header(qcow2Header), sector_size(sectorSizeBytes),
      backing_image(NULL) {
  ...
  if (header.backing_file_offset != 0 && header.backing_file_size != 0) {
    ...
    if (backing_file_name[0] != 0x2F) {
      for (int image_name_index = (int)strlen(imageName); image_name_index > -1;
           image_name_index--) {
        if (imageName[image_name_index] == 0x2F) {
          int full_name_length = (int)((unsigned int)header.backing_file_size +
                                       (unsigned int)image_name_index + 2u);
          char *full_name = new char[full_name_length];
          for (int full_name_index = 0; full_name_index < full_name_length;
               full_name_index++) {
            if (full_name_index <= image_name_index) {
              full_name[full_name_index] = imageName[full_name_index];
            } else {
              full_name[full_name_index] =
                  backing_file_name[full_name_index - (image_name_index + 1)];
            }
          }
          delete[] backing_file_name;
    ...
    if (backing_file != NULL) {
      ...
      backing_image = new QCow2Image(backing_header, backing_file,
                                     backing_file_name, sectorSizeBytes);
    } else {
...
uint8_t QCow2Image::read_sector(uint32_t sectnum, uint8_t *data) {
  ...
  return read_allocated_data(data_cluster_offset + (address & cluster_mask),
                             data, sector_size);
}
...
uint8_t QCow2Image::write_sector(uint32_t sectnum, const uint8_t *data) {
  ...
  if (data_cluster_offset == 0) {
    ...
    if (0 !=
        write_l2_table_entry(l2_table_offset, address, data_cluster_offset)) {
      return 0x05;
    }
    uint8_t *cluster_buffer = new uint8_t[cluster_size];
  ...
  return write_data(data_cluster_offset + (address & cluster_mask), data,
                    sector_size);
}
...
// Helper functions for endianness. QCOW format is big endian so we need
// different functions than those defined in mem.h.
#if defined(WORDS_BIGENDIAN) || !defined(C_UNALIGNED_MEMORY)
inline uint16_t QCow2Image::host_read16(uint16_t buffer) { return buffer; }

inline uint32_t QCow2Image::host_read32(uint32_t buffer) { return buffer; }

inline uint64_t QCow2Image::host_read64(uint64_t buffer) { return buffer; }
#else
...
inline uint64_t QCow2Image::host_read64(uint64_t buffer) {
  return (buffer >> 56) | (((buffer >> 48) & 0xff) << 8) |
...
uint8_t QCow2Image::pad_file(uint64_t &new_file_length) {
  ...
  const uint64_t padding_size =
      (cluster_size - (old_file_length % cluster_size)) % cluster_size;
...
uint8_t QCow2Image::read_allocated_data(uint64_t file_offset, uint8_t *data,
                                        uint64_t data_size) {
...
inline uint8_t QCow2Image::read_l1_table(uint64_t address,
                                         uint64_t &l2_table_offset) {
  const uint64_t l1_entry_offset =
      header.l1_table_offset + ((address >> l1_bits) << 3);
...
inline uint8_t QCow2Image::read_l2_table(uint64_t l2_table_offset,
                                         uint64_t address,
                                         uint64_t &data_cluster_offset) {
  const uint64_t l2_entry_offset =
      l2_table_offset + (((address >> header.cluster_bits) & l2_mask) << 3);
  return read_table(l2_entry_offset, table_entry_mask, data_cluster_offset);
}

// Read the refcount table to get the offset of the refcount cluster for a given
// address.
inline uint8_t
QCow2Image::read_refcount_table(uint64_t data_cluster_offset,
                                uint64_t &refcount_cluster_offset) {
  const uint64_t refcount_entry_offset =
      header.refcount_table_offset +
      (((data_cluster_offset / cluster_size) >> refcount_bits) << 3);
  return read_table(refcount_entry_offset, empty_mask, refcount_cluster_offset);
}

// Read a table entry at the given offset.
inline uint8_t QCow2Image::read_table(uint64_t entry_offset,
                                      uint64_t entry_mask,
                                      uint64_t &entry_value) {
  (void)entry_mask; // UNUSED
  uint64_t buffer;
  if (0 !=
      read_allocated_data(entry_offset, (uint8_t *)&buffer, sizeof buffer)) {
    return 0x05;
  }
...
inline uint8_t
QCow2Image::read_unallocated_cluster(uint64_t data_cluster_number,
                                     uint8_t *data) {
  if (backing_image == NULL) {
...
inline uint8_t QCow2Image::read_unallocated_sector(uint32_t sectnum,
                                                   uint8_t *data) {
  if (backing_image == NULL) {
...
uint8_t QCow2Image::update_reference_count(uint64_t cluster_offset,
  ...
  if (0 == refcount_cluster_offset) {
    ...
    std::fill(cluster_buffer, cluster_buffer + cluster_size, 0);
    if (0 !=
        write_refcount_table_entry(cluster_offset, refcount_cluster_offset)) {
      return 0x05;
    }
    if (0 !=
        write_data(refcount_cluster_offset, cluster_buffer, cluster_size)) {
      return 0x05;
    }
    if (0 !=
        write_refcount(refcount_cluster_offset, refcount_cluster_offset, 0x1)) {
      return 0x05;
    }
  }
...
uint8_t QCow2Image::write_data(uint64_t file_offset, const uint8_t *data,
                               uint64_t data_size) {
  if (0 != fseeko64(file, (off_t)file_offset, SEEK_SET)) {
    return 0x05;
  }
...
inline uint8_t QCow2Image::write_l1_table_entry(uint64_t address,
                                                uint64_t l2_table_offset) {
  const uint64_t l1_entry_offset =
      header.l1_table_offset + ((address >> l1_bits) << 3);
...
inline uint8_t QCow2Image::write_l2_table_entry(uint64_t l2_table_offset,
                                                uint64_t address,
                                                uint64_t data_cluster_offset) {
  const uint64_t l2_entry_offset =
      l2_table_offset + (((address >> header.cluster_bits) & l2_mask) << 3);
...
// Write a refcount.
inline uint8_t QCow2Image::write_refcount(uint64_t cluster_offset,
                                          uint64_t refcount_cluster_offset,
                                          uint16_t refcount) {
  const uint64_t refcount_offset =
      refcount_cluster_offset +
      (((cluster_offset / cluster_size) & refcount_mask) << 1);
...
inline uint8_t
QCow2Image::write_refcount_table_entry(uint64_t cluster_offset,
                                       uint64_t refcount_cluster_offset) {
  const uint64_t refcount_entry_offset =
      header.refcount_table_offset +
      (((cluster_offset / cluster_size) >> refcount_bits) << 3);
  return write_table_entry(refcount_entry_offset, refcount_cluster_offset);
}

// Write a table entry at the given offset.
inline uint8_t QCow2Image::write_table_entry(uint64_t entry_offset,
                                             uint64_t entry_value) {
...
// Public Constructor.
QCow2Disk::QCow2Disk(QCow2Image::QCow2Header &qcow2Header, FILE *qcow2File,
                     const char *imgName, uint32_t imgSizeK,
                     uint32_t sectorSizeBytes, bool isHardDisk)
    : imageDisk(qcow2File, (const char *)imgName, imgSizeK, isHardDisk),
      qcowImage(qcow2Header, qcow2File, (const char *)imgName,
                sectorSizeBytes) {}
