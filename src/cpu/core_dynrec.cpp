...
#define DRCD_REG_BYTE(reg, idx)                                                \
  (&cpu_regs.regs[reg].byte[idx ? BH_INDEX : BL_INDEX])
// access to  16/32bit general registers
#define DRCD_REG_WORD(reg, dwrd)                                               \
  ((dwrd) ? ((void *)(&cpu_regs.regs[reg].dword[DW_INDEX]))                    \
          : ((void *)(&cpu_regs.regs[reg].word[W_INDEX])))
...
static struct {
  BlockReturnDynRec (*runcode)(
      uint8_t *);             // points to code that can start a block
 ...
 * Windows RT 8.x only runs ARMv7 Thumb-2 code, which we don't have a backend
 * for Attempting to use dynarec on RT will result in an instant crash. So we
 * need to detect the Windows version before enabling dynarec. */
...
bool IsWin10OrGreater() {
  ...
  if (handle) {
    ...
    RtlGetVersionPtr getVersionPtr =
        (RtlGetVersionPtr)GetProcAddress(handle, "RtlGetVersion");
...
CacheBlockDynRec *LinkBlocks(BlockReturnDynRec ret) {
  ...
  CodePageHandlerDynRec *temp_handler =
      (CodePageHandlerDynRec *)get_tlb_readhandler(temp_ip);
...
Bits CPU_Core_Dynrec_Run(void) {
  ...
  if (!is_win10) {
    if (winrt_warning) {
      LOG_MSG("Dynamic core warning: Windows RT 8.x requires ARMv7 Thumb-2 "
              "dynarec core, which is not supported yet.");
      winrt_warning = false;
    }
  ...
  if (paging.enabled && !use_dynamic_core_with_paging) {
    if (paging_warning) {
      LOG_MSG(
          "Dynamic core warning: The guest OS/Application has just switched on "
          "80386 paging, which is not supported by the dynamic core. The "
          "normal core will be used until paging is switched off again.");
  ...
  for (;;) {
    ...
    if (!block) {
      ...
      if (!chandler->invalidation_map ||
        ...
        dosbox_allow_nonrecursive_page_fault = true;
        // let the normal core handle this instruction to avoid zero-sized
        // blocks
  ...
  run_block:
    ...
    //		BlockReturnDynRec ret=((BlockReturnDynRec
    //(*)(void))(block->cache.start))();
    ...
    if (sizeof(CPU_Cycles) > 4) {
      ...
      //       that when CPU_Cycles goes negative it doesn't suddenly appear as
    ...
    case BR_CallBack:
      // the callback code is executed in dosbox-x.conf, return the callback
      // number
      FillFlags();
...
void CPU_Core_Dynrec_Cache_Close(void) { cache_close(); }

void CPU_Core_Dynrec_Cache_Reset(void) { cache_reset(); }
#endif
