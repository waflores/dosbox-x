...
public:
  UNBLOCKED_LOG(LOG_TYPES type, LOG_SEVERITIES severity)
      : LOG(type, severity) {}
};
...
int64_t CPU_RDTSC() {
  ...
  return (int64_t)(((PIC_FullIndex() - rdtsc_pic_base) * rate) +
                   rdtsc_count_base);
}
 ...
 * in real mode. I'm guessing that what Project Angel is doing, is using the
 * same abuse of protected mode to also set the B (big) bit in the code segment
 * so that its code segment can extend past 64KB (huge unreal mode), which works
 * until something like an interrupt chops off the top 16 bits of the
 * instruction pointer.
 *
 ...
 *
 * J.C. TODO: Write a program that abuses the B (big) bit in real mode in the
 * same way that Project Angel supposedly does, see if it works, then test it
 *            and Project Angel on some old 386/486/Pentium systems lying around
 * to see how compatible such abuse is with PC hardware. That would make a good
 * Hackipedia.org episode as well. --J.C.
 *
 ...
 *             music as well. Now as for reliability... testing shows that one
 * of three things can happen when you run the demo:
 *
 *             1) the demo hangs on startup, either right away or after it
 * starts the ominous music (if you sit for 30 seconds waiting for the music to
 * build up and nothing happens, consider closing the emulator and trying
 * again).
 *
 *             2) the demo runs perfectly fine, but timing is slightly screwed
 * up, and parts of the music sound badly out of sync with each other, or
 * randomly slows to about 1/2 speed in some sections, animations run slow
 * sometimes. If this happens, make sure you didn't set forcerate=ntsc.
 *
 *             3) the demo runs perfectly fine, with no timing issues, except
 * that DOSBox's S3 emulation is not quite on-time and the bottom 1/4th of the
 * screen flickers with the contents of the next frame that the demo is still
 * drawing :(
 *
 *             --J.C. */
...
Bitu CPU_extflags_toggle =
    0; // ID and AC flags may be toggled depending on emulated CPU architecture
...
void menu_update_dynamic() {
  ...
  if (found != std::string::npos && text.substr(found + 1).size() &&
      *text.substr(found + 1).c_str() == '(')
    text = text.substr(0, found);
#if (C_DYNREC)
  if ((core == "dynamic" && GetDynamicType() == 2) || core == "dynamic_rec" ||
      save_dynamic_rec || !BOTH_DYNAMIC) {
...
void menu_update_autocycle(void) {
  ...
  item.check(CPU_CycleAutoAdjust ||
...
void CPU_NMI_Interrupt() {
  ...
  if (CPU_NMI_active)
    E_Exit("CPU_NMI_Interrupt() called while NMI already active");
  CPU_NMI_active = true;
...
void CPU_Push16(uint16_t value) {
  uint32_t new_esp =
      (reg_esp & cpu.stack.notmask) | ((reg_esp - 2) & cpu.stack.mask);
...
void CPU_Push32(uint32_t value) {
  uint32_t new_esp =
      (reg_esp & cpu.stack.notmask) | ((reg_esp - 4) & cpu.stack.mask);
...
void CPU_SetFlags(Bitu word, Bitu mask) {
   ...
   * For more information read about the Intel CPU detection algorithm and other
   * bits of info: [http://www.rcollins.org/ddj/Sep96/Sep96.html] */
  ...
  // ^ NTS: Notice the DF flag is bit 10. This code computes (reg_flags &
  // FLAG_DF) >> 9 on purpose.
  //        It's not a typo (9 vs 10), it's done to set cpu.direction to either
  //        1 or -1.
}
...
public:
  ...
  void SaveSelector(void) { cpu.gdt.SetDescriptor(selector, desc); }
...
bool CPU_SwitchTask(Bitu new_tss_selector, TSwitchType tstype,
  ...
  /* this code isn't very easy to make interruptible. so temporarily revert to
   * recursive PF handling method */
  ...
  if (!new_tss.SetSelector(new_tss_selector))
    E_Exit("Illegal TSS for switch, selector=%x, switchtype=%lx",
           (int)new_tss_selector, (unsigned long)tstype);
  ...
  Bitu new_cr3 = 0;
  uint32_t new_eax, new_ebx, new_ecx, new_edx, new_esp, new_ebp, new_esi,
      new_edi;
  ...
  if (tstype == TSwitch_CALL_INT) {
    if (new_tss.is386) {
      mem_writed(new_tss.base + offsetof(TSS_32, back),
                 (uint32_t)cpu_tss.selector);
    } else {
      mem_writew(new_tss.base + offsetof(TSS_16, back),
                 (uint16_t)cpu_tss.selector);
    }
  ...
  if (reg_flags & FLAG_VM) {
    ...
    doconforming:
      ...
      Segs.limit[cs] =
          do_seg_limits ? (PhysPt)cs_desc.GetLimit() : ((PhysPt)(~0UL));
...
void CPU_Exception(Bitu which, Bitu error) {
  ...
  if (CPU_Exception_Level[which] != 0) {
    if (CPU_Exception_Level[EXCEPTION_DF] != 0 && cpu_triple_fault_reset) {
      if (always_report_triple_fault || !has_printed_triple_fault) {
        LOG_MSG("CPU_Exception: Double fault already in progress == Triple "
                "Fault. Resetting CPU.");
        has_printed_triple_fault = true;
      }

      // Triple fault -> special shutdown cycle -> reset signal -> reset.
      // Sickening, I know, but that's how IBM wired things a long long time
      // ago.
    ...
    if (always_report_double_fault || !has_printed_double_fault) {
      LOG_MSG("CPU_Exception: Exception %d already in progress, triggering "
  ...
  if (cpu_double_fault_enable) {
    /* NTS: Putting some thought into it, I don't think divide by zero counts as
     * something to throw a double fault over. I may be wrong. The behavior of
     * Intel processors will ultimately decide.
     *
     *      Until then, don't count Divide Overflow exceptions, so that the "EFP
     * loader" can do its disgusting anti-debugger hackery when loading parts of
     * a demo. --J.C. */
    if (!(which == 0 /*divide by zero/overflow*/)) {
      /* CPU_Interrupt() could cause another fault during memory access. This
  ...
  /* allow recursive page faults. required for multitasking OSes like
   * Windows 95. we set this AFTER CPU_Interrupt so that if CPU_Interrupt faults
   * while starting a page fault we still trigger double fault. */
...
void CPU_Interrupt(Bitu num, Bitu type, uint32_t oldeip) {
  ...
  if (type !=
      CPU_INT_SOFTWARE) { /* CPU core already takes care of SW interrupts */
#if !defined(HX_DOS)
  ...
  if (type == CPU_INT_SOFTWARE && boothax == BOOTHAX_MSDOS) {
    if (num == 0x21 && boothax == BOOTHAX_MSDOS) {
      if (dos_kernel_disabled) {
        if ((reg_ah == 0x4A /*alloc*/ || reg_ah == 0x49 /*free*/) &&
            guest_msdos_LoL == 0) { /* needed for MS-DOS 3.3 */
          if (SegValue(cs) != CB_SEG) {
            ...
            /* Read off the MCB chain base (WARNING: Only works with MS-DOS 3.3
             * or later) */
            guest_msdos_mcb_chain = real_readw(guest_msdos_LoL >> 16,
                                               (guest_msdos_LoL & 0xFFFF) - 2);
#if 1
            LOG_MSG("List of Lists: %04x:%04x", guest_msdos_LoL >> 16,
                    guest_msdos_LoL & 0xFFFF);
        ...
        if (reg_ah == 0x52) { /* get list of lists. MS-DOS 5.0 and higher call
...
//		E_Exit("Call to interrupt 0xCD this is BAD");
  ...
  if (!cpu.pmode) {
    ...
    if ((reg_flags & FLAG_VM) && (type & CPU_INT_SOFTWARE) &&
      ...
      //			LOG_MSG("Software int in v86, AH %X IOPL
      //%x",reg_ah,(reg_flags & FLAG_IOPL) >>12);
    ...
    if (!cpu.idt.GetDescriptor(num << 3, gate)) {
      ...
      CPU_Exception(EXCEPTION_GP,
                    num * 8 + 2 + ((type & CPU_INT_SOFTWARE) ? 0 : 1));
      return;
    }
    ...
    try {
      ...
      case DESC_386_TRAP_GATE: {
        ...
        CPU_CHECK_COND((gate_sel & 0xfffc) == 0,
                       "INT:Gate with CS zero selector", EXCEPTION_GP,
                       (type & CPU_INT_SOFTWARE) ? 0 : 1)
        CPU_CHECK_COND(!cpu.gdt.GetDescriptor(gate_sel, cs_desc),
                       "INT:Gate with CS beyond limit", EXCEPTION_GP,
                       (gate_sel & 0xfffc) +
                           ((type & CPU_INT_SOFTWARE) ? 0 : 1))

        Bitu cs_dpl = cs_desc.DPL();
        CPU_CHECK_COND(
            cs_dpl > cpu.cpl, "Interrupt to higher privilege", EXCEPTION_GP,
            (gate_sel & 0xfffc) + ((type & CPU_INT_SOFTWARE) ? 0 : 1))
        switch (cs_desc.Type()) {
        case DESC_CODE_N_NC_A:
        case DESC_CODE_N_NC_NA:
        case DESC_CODE_R_NC_A:
        case DESC_CODE_R_NC_NA:
          if (cs_dpl < cpu.cpl) {
            ...
            CPU_CHECK_COND(
                !cs_desc.saved.seg.p, "INT:Inner level:CS segment not present",
                EXCEPTION_NP,
                (gate_sel & 0xfffc) + ((type & CPU_INT_SOFTWARE) ? 0 : 1))
            ...
            CPU_CHECK_COND((n_ss & 0xfffc) == 0,
                           "INT:Gate with SS zero selector", EXCEPTION_TS,
                           (type & CPU_INT_SOFTWARE) ? 0 : 1)
            Descriptor n_ss_desc;
            CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_ss, n_ss_desc),
                           "INT:Gate with SS beyond limit", EXCEPTION_TS,
                           (n_ss & 0xfffc) +
                               ((type & CPU_INT_SOFTWARE) ? 0 : 1))
            CPU_CHECK_COND(
                ((n_ss & 3) != cs_dpl) || (n_ss_desc.DPL() != cs_dpl),
                "INT:Inner level with CS_DPL!=SS_DPL and SS_RPL", EXCEPTION_TS,
                (n_ss & 0xfffc) + ((type & CPU_INT_SOFTWARE) ? 0 : 1))
            ...
            default:
              E_Exit(
                  "INT:Inner level:Stack segment not writable."); // or
                                                                  // #TS(ss_sel+EXT)
            }
            ...
            Segs.limit[ss] =
                do_seg_limits ? (PhysPt)n_ss_desc.GetLimit() : ((PhysPt)(~0UL));
            ...
            //						LOG_MSG("INT:Gate to
            //inner level SS:%X SP:%X",n_ss,n_esp);
            goto do_interrupt;
          }
          if (cs_dpl != cpu.cpl)
        ...
        do_interrupt:
          if (gate.Type() & 0x8) { /* 32-bit Gate */
            ...
            if (type & CPU_INT_HAS_ERROR)
              CPU_Push32((uint32_t)cpu.exception.error);
          } else { /* 16-bit gate */
            CPU_Push16(reg_flags & 0xffff);
            CPU_Push16(SegValue(cs));
            CPU_Push16(oldeip);
            if (type & CPU_INT_HAS_ERROR)
              CPU_Push16((uint16_t)cpu.exception.error);
          }
          break;
        default:
          E_Exit("INT:Gate Selector points to illegal descriptor with type %x",
                 (int)cs_desc.Type());
        }
        ...
        SETFLAGBIT(NT, false);
        SETFLAGBIT(VM, false);
        LOG(LOG_CPU, LOG_NORMAL)("INT:Gate to %X:%X big %d %s", gate_sel,
                                 gate_off, cs_desc.Big(),
      ...
      case DESC_TASK_GATE:
        ...
        return;
      default:
        E_Exit("Illegal descriptor type %X for int %X", (int)gate.Type(),
               (int)num);
      }
...
/* NTS: It sounds like Intel processors only change the lower 16 bits of SP on
 * IRETD if the stack is 16 bits. See also
 * [https://devblogs.microsoft.com/oldnewthing/20160404-00/?p=93261]. Make sure
 * this code emulates that bug. Some buggy games might rely on that. */
...
void CPU_IRET(bool use32, uint32_t oldeip) {
   ...
   */
  /* FIXME: If the IRET causes an exception, is it still considered the end of
   * the NMI? */
  ...
  if (!cpu.pmode) { /* RealMode IRET */
      ...
      if ((reg_flags & FLAG_IOPL) != FLAG_IOPL) {
        ...
        try {
          if (use32) {
            ...
            uint32_t tempesp = (reg_esp & cpu.stack.notmask) |
                               ((reg_esp + 4) & cpu.stack.mask);
            uint32_t new_cs =
                mem_readd(SegPhys(ss) + (tempesp & cpu.stack.mask));
            ...
            uint16_t new_cs =
                mem_readw(SegPhys(ss) + (tempesp & cpu.stack.mask));
            tempesp = (tempesp & cpu.stack.notmask) |
    ...
    if (GETFLAG(NT)) {
      ...
      CPU_CHECK_COND(!cpu_tss.IsValid(), "TASK Iret without valid TSS",
    ...
    if (use32) {
      ...
      tempesp =
          (tempesp & cpu.stack.notmask) | ((tempesp + 4) & cpu.stack.mask);
      ...
      if ((n_flags & FLAG_VM) && (cpu.cpl == 0)) {
        ...
        try {
          ...
          LOG(LOG_CPU, LOG_NORMAL)(
              ...
              SegValue(cs), reg_eip, SegValue(ss), reg_esp, reg_flags);
          return;
        } catch (const GuestPageFaultException &pf) {
          (void)pf; // UNUSED
      ...
      tempesp =
          (tempesp & cpu.stack.notmask) | ((tempesp + 2) & cpu.stack.mask);
    ...
    CPU_CHECK_COND((n_cs_sel & 0xfffc) == 0, "IRET:CS selector zero",
                   EXCEPTION_GP, 0)
    Bitu n_cs_rpl = n_cs_sel & 3;
    Descriptor n_cs_desc;
    CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_cs_sel, n_cs_desc),
                   "IRET:CS selector beyond limits", EXCEPTION_GP,
                   n_cs_sel & 0xfffc)
    CPU_CHECK_COND(n_cs_rpl < cpu.cpl, "IRET to lower privilege", EXCEPTION_GP,
                   n_cs_sel & 0xfffc)
    ...
    case DESC_CODE_R_NC_A:
    case DESC_CODE_R_NC_NA:
      CPU_CHECK_COND(n_cs_rpl != n_cs_desc.DPL(), "IRET:NC:DPL!=RPL",
    ...
    CPU_CHECK_COND(!n_cs_desc.saved.seg.p, "IRET with nonpresent code segment",
    ...
    if (n_cs_rpl == cpu.cpl) {
      ...
      Segs.limit[cs] =
          do_seg_limits ? (PhysPt)n_cs_desc.GetLimit() : ((PhysPt)(~0UL));
      ...
      DestroyConditionFlags();
      LOG(LOG_CPU, LOG_NORMAL)("IRET:Same level:%X:%X big %d", n_cs_sel, n_eip,
                               cpu.code.big);
    } else {
      ...
      CPU_CHECK_COND((n_ss & 0xfffc) == 0, "IRET:Outer level:SS selector zero",
                     EXCEPTION_GP, 0)
      CPU_CHECK_COND((n_ss & 3) != n_cs_rpl, "IRET:Outer level:SS rpl!=CS rpl",
                     EXCEPTION_GP, n_ss & 0xfffc)
      Descriptor n_ss_desc;
      CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_ss, n_ss_desc),
                     "IRET:Outer level:SS beyond limit", EXCEPTION_GP,
                     n_ss & 0xfffc)
      CPU_CHECK_COND(n_ss_desc.DPL() != n_cs_rpl,
                     "IRET:Outer level:SS dpl!=CS rpl", EXCEPTION_GP,
                     n_ss & 0xfffc)
      ...
      CPU_CHECK_COND(!n_ss_desc.saved.seg.p,
                     "IRET:Outer level:Stack segment not present", EXCEPTION_NP,
                     n_ss & 0xfffc)
      ...
      LOG(LOG_CPU, LOG_NORMAL)("IRET:Outer level:%X:%X big %d", n_cs_sel, n_eip,
                               cpu.code.big);
    }
    return;
  }
...
void CPU_JMP(bool use32, Bitu selector, Bitu offset, uint32_t oldeip) {
  if (!cpu.pmode || (reg_flags & FLAG_VM)) {
    ...
    CPU_CHECK_COND((selector & 0xfffc) == 0, "JMP:CS selector zero",
                   EXCEPTION_GP, 0)
    Bitu rpl = selector & 3;
    Descriptor desc;
    CPU_CHECK_COND(!cpu.gdt.GetDescriptor(selector, desc),
                   "JMP:CS beyond limits", EXCEPTION_GP, selector & 0xfffc)
    switch (desc.Type()) {
    case DESC_CODE_N_NC_A:
    case DESC_CODE_N_NC_NA:
    case DESC_CODE_R_NC_A:
    case DESC_CODE_R_NC_NA:
      CPU_CHECK_COND(rpl > cpu.cpl, "JMP:NC:RPL>CPL", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(cpu.cpl != desc.DPL(), "JMP:NC:RPL != DPL", EXCEPTION_GP,
                     selector & 0xfffc)
      LOG(LOG_CPU, LOG_NORMAL)("JMP:Code:NC to %X:%X big %d", selector, offset,
                               desc.Big());
      goto CODE_jmp;
    case DESC_CODE_N_C_A:
    case DESC_CODE_N_C_NA:
    case DESC_CODE_R_C_A:
    case DESC_CODE_R_C_NA:
      LOG(LOG_CPU, LOG_NORMAL)("JMP:Code:C to %X:%X big %d", selector, offset,
                               desc.Big());
      CPU_CHECK_COND(cpu.cpl < desc.DPL(), "JMP:C:CPL < DPL", EXCEPTION_GP,
                     selector & 0xfffc)
    CODE_jmp:
      if (!desc.saved.seg.p) {
        // win
      ...
      Segs.limit[cs] =
          do_seg_limits ? (PhysPt)desc.GetLimit() : ((PhysPt)(~0UL));
      ...
      return;
    case DESC_386_TSS_A:
      CPU_CHECK_COND(desc.DPL() < cpu.cpl, "JMP:TSS:dpl<cpl", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(desc.DPL() < rpl, "JMP:TSS:dpl<rpl", EXCEPTION_GP,
                     selector & 0xfffc)
      LOG(LOG_CPU, LOG_NORMAL)("JMP:TSS to %X", selector);
      CPU_SwitchTask(selector, TSwitch_JMP, oldeip);
      break;
    case DESC_386_CALL_GATE: /* CAUTION: Copy-pasta from CPU_CALL() with stack
                                switching removed */
    case DESC_286_CALL_GATE:
    case DESC_TASK_GATE: {
      CPU_CHECK_COND(desc.DPL() < cpu.cpl, "JMP:Gate:Gate DPL<CPL",
                     EXCEPTION_GP, selector & 0xfffc)
      CPU_CHECK_COND(desc.DPL() < rpl, "JMP:Gate:Gate DPL<RPL", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(!desc.saved.seg.p, "JMP:Gate:Segment not present",
                     EXCEPTION_NP, selector & 0xfffc)
      Descriptor n_cs_desc;
      Bitu n_cs_sel = desc.GetSelector();

      CPU_CHECK_COND((n_cs_sel & 0xfffc) == 0, "JMP:Gate:CS selector zero",
                     EXCEPTION_GP, 0)
      CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_cs_sel, n_cs_desc),
                     "JMP:Gate:CS beyond limits", EXCEPTION_GP,
                     n_cs_sel & 0xfffc)
      Bitu n_cs_dpl = n_cs_desc.DPL();
      CPU_CHECK_COND(n_cs_dpl > cpu.cpl, "JMP:Gate:CS DPL>CPL", EXCEPTION_GP,
                     n_cs_sel & 0xfffc)

      CPU_CHECK_COND(!n_cs_desc.saved.seg.p, "JMP:Gate:CS not present",
                     EXCEPTION_NP, n_cs_sel & 0xfffc)

      Bitu n_eip = desc.GetOffset();
      switch (n_cs_desc.Type()) {
      case DESC_386_TSS_A:
        CPU_CHECK_COND(n_cs_desc.DPL() < cpu.cpl, "JMP:TSS:dpl<cpl",
                       EXCEPTION_GP, n_cs_sel & 0xfffc)
        CPU_CHECK_COND(n_cs_desc.DPL() < rpl, "JMP:TSS:dpl<rpl", EXCEPTION_GP,
                       n_cs_sel & 0xfffc)
...
void CPU_CALL(bool use32, Bitu selector, Bitu offset, uint32_t oldeip) {
  ...
  if (!cpu.pmode || (reg_flags & FLAG_VM)) {
    ...
    CPU_CHECK_COND((selector & 0xfffc) == 0, "CALL:CS selector zero",
                   EXCEPTION_GP, 0)
    Bitu rpl = selector & 3;
    Descriptor call;
    CPU_CHECK_COND(!cpu.gdt.GetDescriptor(selector, call),
                   "CALL:CS beyond limits", EXCEPTION_GP, selector & 0xfffc)
    ...
    case DESC_CODE_R_NC_A:
    case DESC_CODE_R_NC_NA:
      CPU_CHECK_COND(rpl > cpu.cpl, "CALL:CODE:NC:RPL>CPL", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(call.DPL() != cpu.cpl, "CALL:CODE:NC:DPL!=CPL",
                     EXCEPTION_GP, selector & 0xfffc)
    ...
    case DESC_CODE_R_C_A:
    case DESC_CODE_R_C_NA:
      CPU_CHECK_COND(call.DPL() > cpu.cpl, "CALL:CODE:C:DPL>CPL", EXCEPTION_GP,
                     selector & 0xfffc)
    ...
    call_code:
      ...
      Segs.limit[cs] =
          do_seg_limits ? (PhysPt)call.GetLimit() : ((PhysPt)(~0UL));
    ...
    case DESC_286_CALL_GATE:
    case DESC_TASK_GATE: {
      CPU_CHECK_COND(call.DPL() < cpu.cpl, "CALL:Gate:Gate DPL<CPL",
                     EXCEPTION_GP, selector & 0xfffc)
      CPU_CHECK_COND(call.DPL() < rpl, "CALL:Gate:Gate DPL<RPL", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(!call.saved.seg.p, "CALL:Gate:Segment not present",
                     EXCEPTION_NP, selector & 0xfffc)
      Descriptor n_cs_desc;
      Bitu n_cs_sel = call.GetSelector();

      CPU_CHECK_COND((n_cs_sel & 0xfffc) == 0, "CALL:Gate:CS selector zero",
                     EXCEPTION_GP, 0)
      CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_cs_sel, n_cs_desc),
                     "CALL:Gate:CS beyond limits", EXCEPTION_GP,
                     n_cs_sel & 0xfffc)
      Bitu n_cs_dpl = n_cs_desc.DPL();
      CPU_CHECK_COND(n_cs_dpl > cpu.cpl, "CALL:Gate:CS DPL>CPL", EXCEPTION_GP,
                     n_cs_sel & 0xfffc)

      CPU_CHECK_COND(!n_cs_desc.saved.seg.p, "CALL:Gate:CS not present",
                     EXCEPTION_NP, n_cs_sel & 0xfffc)

      Bitu n_eip = call.GetOffset();
      switch (n_cs_desc.Type()) {
      case DESC_386_TSS_A:
        CPU_CHECK_COND(n_cs_desc.DPL() < cpu.cpl, "CALL:TSS:dpl<cpl",
                       EXCEPTION_GP, n_cs_sel & 0xfffc)
        CPU_CHECK_COND(n_cs_desc.DPL() < rpl, "CALL:TSS:dpl<rpl", EXCEPTION_GP,
                       n_cs_sel & 0xfffc)
      ...
      case DESC_CODE_R_NC_NA:
        ...
        if (n_cs_dpl < cpu.cpl) {
          ...
          CPU_CHECK_COND((n_ss_sel & 0xfffc) == 0,
                         "CALL:Gate:NC:SS selector zero", EXCEPTION_TS, 0)
          CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_ss_sel, n_ss_desc),
                         "CALL:Gate:Invalid SS selector", EXCEPTION_TS,
                         n_ss_sel & 0xfffc)
          CPU_CHECK_COND(((n_ss_sel & 3) != n_cs_desc.DPL()) ||
          ...
          CPU_CHECK_COND(!n_ss_desc.saved.seg.p,
                         "CALL:Gate:Stack segment not present", EXCEPTION_SS,
                         n_ss_sel & 0xfffc)
          ...
          if (call.saved.gate.paramcount & 31) {
            if (call.Type() == DESC_386_CALL_GATE) {
              for (int8_t i = (call.saved.gate.paramcount & 31) - 1; i >= 0;
                   i--)
          ...
          /* this code isn't very easy to make interruptible. so temporarily
           * revert to recursive PF handling method */
          dosbox_allow_nonrecursive_page_fault = false;
    ...
    break;
    case DESC_386_TSS_A:
      CPU_CHECK_COND(call.DPL() < cpu.cpl, "CALL:TSS:dpl<cpl", EXCEPTION_GP,
                     selector & 0xfffc)
      CPU_CHECK_COND(call.DPL() < rpl, "CALL:TSS:dpl<rpl", EXCEPTION_GP,
                     selector & 0xfffc)

      CPU_CHECK_COND(!call.saved.seg.p, "CALL:TSS:Segment not present",
                     EXCEPTION_NP, selector & 0xfffc)
...
void CPU_RET(bool use32, Bitu bytes, uint32_t oldeip) {
  ...
  if (!cpu.pmode || (reg_flags & FLAG_VM)) {
    try {
       ...
       *      will break if the upper 16 bits of ESP are made nonzero. I'm
       * guessing the RETF used in that way is an elaborate way of adding 2 to
       * the stack pointer instead of subtracting. --Jonathan C. */
      reg_esp = (reg_esp & cpu.stack.notmask) |
                ((reg_esp + (uint32_t)bytes) & cpu.stack.mask);
      SegSet16(cs, new_cs);
      reg_eip = new_ip;
      if (!cpu_allow_big16)
        cpu.code.big = false;
    ...
    if (!use32)
      selector = mem_readw(SegPhys(ss) + (reg_esp & cpu.stack.mask) + 2);
    else
      selector =
          mem_readd(SegPhys(ss) + (reg_esp & cpu.stack.mask) + 4) & 0xffff;
    ...
    CPU_CHECK_COND((selector & 0xfffc) == 0, "RET:CS selector zero",
                   EXCEPTION_GP, 0)
    CPU_CHECK_COND(!cpu.gdt.GetDescriptor(selector, desc),
                   "RET:CS beyond limits", EXCEPTION_GP, selector & 0xfffc)
    ...
    if (cpu.cpl == rpl) {
      ...
      case DESC_CODE_R_NC_NA:
        CPU_CHECK_COND(cpu.cpl != desc.DPL(),
                       "RET to NC segment of other privilege", EXCEPTION_GP,
                       selector & 0xfffc)
        goto RET_same_level;
      case DESC_CODE_N_C_A:
      case DESC_CODE_N_C_NA:
      ...
      case DESC_CODE_R_C_NA:
        CPU_CHECK_COND(desc.DPL() > cpu.cpl,
                       "RET to C segment of higher privilege", EXCEPTION_GP,
                       selector & 0xfffc)
        break;
      case 0:
        break;
    ...
    RET_same_level:
      ...
      case DESC_CODE_R_NC_NA:
        CPU_CHECK_COND(desc.DPL() != rpl,
                       "RET to outer NC segment with DPL!=RPL", EXCEPTION_GP,
                       selector & 0xfffc)
        break;
      case DESC_CODE_N_C_A:
      case DESC_CODE_N_C_NA:
      case DESC_CODE_R_C_A:
      case DESC_CODE_R_C_NA:
        CPU_CHECK_COND(desc.DPL() > rpl, "RET to outer C segment with DPL>RPL",
                       EXCEPTION_GP, selector & 0xfffc)
        break;
      case 0:
        break;
      default:
        E_Exit("RET from illegal descriptor type %X",
               (int)desc.Type()); // or #GP(selector)
      }

      CPU_CHECK_COND(!desc.saved.seg.p, "RET:Outer level:CS not present",
                     EXCEPTION_NP, selector & 0xfffc)
      ...
      CPU_CHECK_COND((n_ss & 0xfffc) == 0,
                     "RET to outer level with SS selector zero", EXCEPTION_GP,
                     0)

      Descriptor n_ss_desc;
      CPU_CHECK_COND(!cpu.gdt.GetDescriptor(n_ss, n_ss_desc),
                     "RET:SS beyond limits", EXCEPTION_GP, n_ss & 0xfffc)
      ...
      default:
        E_Exit(
            "RET:SS selector type no writable data segment"); // or
                                                              // #GP(selector)
      }
      CPU_CHECK_COND(!n_ss_desc.saved.seg.p, "RET:Stack segment not present",
      ...
      //			LOG(LOG_MISC,LOG_ERROR)("RET - Higher level to
      //%X:%X RPL %X DPL %X",selector,offset,rpl,desc.DPL());
      return;
    }
    LOG(LOG_CPU, LOG_NORMAL)("Prot ret %lX:%lX", (unsigned long)selector,
                             (unsigned long)offset);
  }
  assert(1);
}

Bitu CPU_SLDT(void) { return cpu.gdt.SLDT(); }
...
Bitu CPU_STR(void) { return cpu_tss.selector; }
...
bool CPU_LTR(Bitu selector) {
  ...
  if ((desc.Type() == DESC_286_TSS_A) || (desc.Type() == DESC_386_TSS_A)) {
    ...
    if (!cpu_tss.SetSelector(selector))
      E_Exit("LTR failed, selector=%X", (int)selector);
    cpu_tss.desc.SetBusy(true);
    cpu_tss.SaveSelector();
  } else {
    /* Descriptor was no available TSS descriptor */
    LOG(LOG_CPU, LOG_NORMAL)("LTR failed, selector=%X (type=%X)", selector,
                             desc.Type());
    return CPU_PrepareException(EXCEPTION_GP, selector);
  }
  return false;
...
Bitu CPU_SGDT_base(void) { return cpu.gdt.GetBase(); }
Bitu CPU_SGDT_limit(void) { return cpu.gdt.GetLimit(); }

Bitu CPU_SIDT_base(void) { return cpu.idt.GetBase(); }
Bitu CPU_SIDT_limit(void) { return cpu.idt.GetLimit(); }
...
void CPU_Snap_Back_To_Real_Mode() {
  ...
  cpu.idt.SetBase(
      0); /* or ELSE weird things will happen when INTerrupts are run */
...
void CPU_Snap_Back_Forget() { snap_cpu_snapped = false; }
    ...
    if (value & CR0_PROTECTION) {
      ...
      if (CPU_AutoDetermineMode & CPU_AUTODETERMINE_CYCLES) {
        ...
        if (!printed_cycles_auto_info) {
          ...
          LOG_MSG("DOSBox-X has switched to max cycles, because of the "
                  "setting: cycles=auto.\nIf the game runs too fast, try a "
                  "fixed cycles amount in DOSBox-X's options.");
        }
...
Bitu CPU_SMSW(void) { return cpu.cr0; }
...
bool CPU_LMSW(Bitu word) {
  ...
  if (cpu.pmode && (cpu.cpl > 0))
    return CPU_PrepareException(EXCEPTION_GP, 0);
  word &= 0xf;
  if ((cpu.cr0 & 1 /*PE bit*/) && !lmsw_allow_clear_pe_bit)
    word |= 1 /*PE bit, stuck on, cannot exit protected mode, 286 style*/;
...
/* This is called by the XMS emulation to set up Flat Real Mode, at least for
 * segment registers DS and ES */
...
bool CPU_SetSegGeneral(SegNames seg, uint16_t value) {
  if (!cpu.pmode || (reg_flags & FLAG_VM)) {
    ...
    /* real mode: loads do not change the limit. "Flat real mode" would not be
     * possible otherwise. vm86: loads are fixed at 64KB (right?) */
    if (reg_flags & FLAG_VM)
    ...
    if (seg == ss) {
      ...
      if ((value & 0xfffc) == 0) {
        //				E_Exit("CPU_SetSegGeneral: Stack segment
        //zero");
        return CPU_PrepareException(EXCEPTION_GP, 0);
      }
      Descriptor desc;
...
bool CPU_PopSeg(SegNames seg, bool use32) {
  ...
  uint32_t new_esp =
      (reg_esp & cpu.stack.notmask) | ((reg_esp + addsp) & cpu.stack.mask);
  if (CPU_SetSegGeneral(seg, (uint16_t)val))
    return true;
  reg_esp = new_esp;
  return false;
}
...
bool CPU_CPUID(void) {
  ...
  case 0: /* Vendor ID String and maximum level? */
    ...
    if (cpu_custom_cpuid[0] != 0) {
      reg_ebx = cpu_custom_cpuid[0] | (cpu_custom_cpuid[1] << 8) |
                (cpu_custom_cpuid[2] << 16) | (cpu_custom_cpuid[3] << 24);
      reg_edx = cpu_custom_cpuid[4] | (cpu_custom_cpuid[5] << 8) |
                (cpu_custom_cpuid[6] << 16) | (cpu_custom_cpuid[7] << 24);
      reg_ecx = cpu_custom_cpuid[8] | (cpu_custom_cpuid[9] << 8) |
                (cpu_custom_cpuid[10] << 16) | (cpu_custom_cpuid[11] << 24);
    } else {
  ...
  case 1: /* get processor type/family/model/stepping and feature flags */
    if ((CPU_ArchitectureType == CPU_ARCHTYPE_486NEW) ||
      ...
      if (enable_pse)
        reg_edx |= 0x08; /* Page Size Extension */
    } else if (CPU_ArchitectureType == CPU_ARCHTYPE_PENTIUM) {
      ...
      if (enable_pse)
        reg_edx |= 0x08; /* Page Size Extension */
      if (enable_pse_extbits)
        reg_edx |= 0x20000; /* PSE 36-bit */
      if (enable_cmpxchg8b)
      ...
      reg_edx =
          0x00800010 |
          (enable_fpu ? 1 : 0); /* FPU+TimeStamp/RDTSC+MMX+ModelSpecific/MSR */
      if (enable_msr)
        reg_edx |= 0x20; /* ModelSpecific/MSR */
      if (enable_pse)
        reg_edx |= 0x08; /* Page Size Extension */
      ...
      /* NTS: Most operating systems will not attempt SYSENTER/SYSEXIT unless
       * this returns model 3, stepping 3, or higher. */
      /* From Intel
       ...
       * architecture in the Pentium II processor."
       *
       ...
       * instructions are actually present"
       *
       ...
       *
       * Therefore, always return with bit 11 (SEP) set whether or not SYSCALL
       * is enabled because Intel made a stupid mistake.
       *
       * This website
       * [https://www.geoffchappell.com/studies/windows/km/cpu/sep.htm?tx=256]
       * notes how the Windows NT kernel follows this rule, and the Linux kernel
       * does too */
      ...
      if (enable_cmpxchg8b)
        reg_edx |= 0x100; /* CMPXCHG8B */
      reg_edx |= 0x800; /* SEP Fast System Call aka SYSENTER/SYSEXIT [SEE NOTES
                           AT TOP OF THIS IF STATEMENT] */
    } else if (CPU_ArchitectureType == CPU_ARCHTYPE_PENTIUMIII ||
  ...
  case 2: /* Processor Configuration Descriptor(s) */
    if (CPU_ArchitectureType == CPU_ARCHTYPE_PENTIUMIII) {
      /* NTS: Windows XP considers these values SOOOOO IMPORTANT that if we
       * don't return any here the NT kernel will just spin endlessly in an
       * infinite loop with interrupts disabled instead of, you know, booting
       * up. But only if it sees a Pentium III. */
...
/* this is used by dynamic core as a workaround for sending the reset signal
 * into the emulator because C++ exceptions don't work from within dynamically
 * generated code. */
...
void CPU_SetResetSignal(int x) {
  LOG_MSG("CPU: Queuing reset signal, to send when dynamic core has completed "
          "execution.");
...
bool CPU_IsHLTed(void) { return (cpudecoder == &HLT_Decode); }

void CPU_HLT(uint32_t oldeip) {
  /* Since cpu.hlt.old_decoder assigns the current decoder to old, and relies on
   * restoring it back when finished, setting cpudecoder to HLT_Decode while
   * already HLT_Decode effectively hangs DOSBox-X and makes it complete
   * unresponsive. Don't want that! */
  if (cpudecoder == &HLT_Decode)
    E_Exit("CPU_HLT attempted to set HLT_Decode while CPU decoder already "
           "HLT_Decode.\n\nIf you see this message while installing FreeDOS, "
           "please use the normal CPU core.");
...
void CPU_ENTER(bool use32, Bitu bytes, Bitu level) {
  ...
  if (!use32) {
    ...
    /* If 16-bit real mode or stack is 16-bit, even with 66h prefix, mask EBP by
     * 0xFFFF (Fix for Finster by Mad Scientists, 1996). This is not how Intel
     * processors work, as verified on real 486 hardware. Unsurprisingly,
     * Finster does not appear to run properly on real hardware either, though I
     * can't get the game to get that far. --Jonathan C. */
    reg_ebp =
        (reg_esp - 4) & (mask_stack_ptr_on_enter ? cpu.stack.mask : 0xFFFFFFFF);
  ...
  if (CPU_CycleAutoAdjust) {
    ...
    if (CPU_AutoDetermineMode & CPU_AUTODETERMINE_CYCLES) {
      ...
      if (CPU_CycleMax > 15000 && cpudecoder != &CPU_Core_Dyn_X86_Run)
        LOG_MSG("CPU speed: fixed %ld cycles. If you need more than 20000, try "
                "core=dynamic in DOSBox-X's options.",
                (unsigned long)CPU_CycleMax);
      else
...
void CPU_CycleDecrease(bool pressed) {
  ...
  if (CPU_CycleAutoAdjust) {
    ...
    if (CPU_CyclePercUsed <= 70)
      LOG_MSG("CPU speed: max %ld percent. If the game runs too fast, try a "
              "fixed cycles amount in DOSBox-X's options.",
              (unsigned long)CPU_CyclePercUsed);
    else
      LOG_MSG("CPU speed: max %ld percent.", (unsigned long)CPU_CyclePercUsed);
    GFX_SetTitle((int32_t)CPU_CyclePercUsed, -1, -1, false);
  } else {
    ...
    if (CPU_CycleDown < 100) {
      CPU_CycleMax =
          (int32_t)(CPU_CycleMax / (1 + (float)CPU_CycleDown / 100.0));
    } else {
...
public:
  Weitek_PageHandler(HostPt /*addr*/) { flags = PFLAG_NOCODE; }
...
void Weitek_PageHandler::writeb(PhysPt addr, uint8_t val) {
  LOG_MSG("Weitek stub: writeb at 0x%lx val=0x%lx", (unsigned long)addr,
          (unsigned long)val);
}
...
void Weitek_PageHandler::writew(PhysPt addr, uint16_t val) {
  LOG_MSG("Weitek stub: writew at 0x%lx val=0x%lx", (unsigned long)addr,
          (unsigned long)val);
}
...
void Weitek_PageHandler::writed(PhysPt addr, uint32_t val) {
  LOG_MSG("Weitek stub: writed at 0x%lx val=0x%lx", (unsigned long)addr,
          (unsigned long)val);
}
...
bool CpuType_Auto(DOSBoxMenu *const menu, DOSBoxMenu::item *const menuitem) {
  ...
  Section *sec = control->GetSection("cpu");
  if (sec)
    sec->HandleInputline("cputype=auto");
  return true;
}
...
bool CpuType_ByName(DOSBoxMenu *const menu, DOSBoxMenu::item *const menuitem) {
  ...
  Section *sec = control->GetSection("cpu");
  if (sec)
    sec->HandleInputline(std::string("cputype=") + name);
  return true;
}
...
static Bitu weitek_lfb_pages =
    0x2000000UL >> 12UL; /* "The coprocessor will respond to memory addresses
  ...
  if (CPU_ArchitectureType == CPU_ARCHTYPE_386 ||
    ...
    if (enable_weitek) {
      ...
      if (weitek_lfb_cb == MEM_Callout_t_none) {
        ...
        if (weitek_lfb_cb == MEM_Callout_t_none)
          E_Exit("Unable to allocate weitek cb for LFB");
      }

      {
        ...
        cb->Install(
            weitek_lfb >> 12UL,
            MEMMASK_Combine(MEMMASK_FULL, MEMMASK_Range(weitek_lfb_pages)),
            weitek_memio_cb);
...
public:
  CPU(Section *configuration) : Module_base(configuration) {
    ...
    SegSet16(cs, 0);
    Segs.limit[cs] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[cs] = false;
    SegSet16(ds, 0);
    Segs.limit[ds] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[ds] = false;
    SegSet16(es, 0);
    Segs.limit[es] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[es] = false;
    SegSet16(fs, 0);
    Segs.limit[fs] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[fs] = false;
    SegSet16(gs, 0);
    Segs.limit[gs] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[gs] = false;
    SegSet16(ss, 0);
    Segs.limit[ss] = do_seg_limits ? 0xFFFF : ((PhysPt)(~0UL));
    Segs.expanddown[ss] = false;

    CPU_SetFlags(FLAG_IF, FMASK_ALL); // Enable interrupts
    fpu.mxcsr = 0x1F80;               // The default MXCSR value is...
  ...
  bool Change_Config(Section *newconfig) override {
    ...
    dos_clear_tf_on_int01 =
        section->Get_bool("clear trap flag on unhandled int 1");
    cpu_allow_big16 = section->Get_bool("realbig16");

    const char *dynamic_core_paging =
        section->Get_string("use dynamic core with paging on");
    auto_determine_dynamic_core_paging =
        !strlen(dynamic_core_paging) ||
        !strcasecmp(dynamic_core_paging, "auto") ||
        !strcasecmp(dynamic_core_paging, "-1");
    if (auto_determine_dynamic_core_paging) {
      int coretype = CPU_IsDynamicCore();
      use_dynamic_core_with_paging =
          coretype == 1
              ? PAGING_Enabled() && dos_kernel_disabled
              : (coretype == 2 ? PAGING_Enabled() && !dos_kernel_disabled
                               : PAGING_Enabled());
    } else {
      use_dynamic_core_with_paging = !strcasecmp(dynamic_core_paging, "true") ||
                                     !strcasecmp(dynamic_core_paging, "1");
    }
    ...
    if (cpu_allow_big16) {
      ...
      LOG(LOG_CPU,
          LOG_DEBUG)("Emulation of the B (big) bit in real mode enabled\n");
    }

    always_report_double_fault =
        section->Get_bool("always report double fault");
    always_report_triple_fault =
        section->Get_bool("always report triple fault");

    dynamic_core_cache_block_size =
        section->Get_int("dynamic core cache block size");
    if (dynamic_core_cache_block_size < 1 ||
        dynamic_core_cache_block_size > 65536)
      dynamic_core_cache_block_size = 32;
        ...
        if (cmd.FindCommand((unsigned int)cmdnum, str)) {
          if (str.find('%') == str.length() - 1) {
            ...
            if ((percval > 0) && (percval <= 105))
              CPU_CyclePercUsed = (int32_t)percval;
          } else if (str == "limit") {
            cmdnum++;
            if (cmd.FindCommand((unsigned int)cmdnum, str)) {
              int cyclimit = 0;
    ...
    if (enable_syscall && !enable_msr) {
      UNBLOCKED_LOG(LOG_CPU,
                    LOG_WARN)("syscall (SYSENTER/SYSEXIT) requires that model "
                              "specific registers (RDMSR/WRMSR) are enabled");
    ...
    if (core == "normal") {
...
#endif
#if (C_DYNAMIC_X86)
    } else if ((core == "dynamic" && GetDynamicType() == 1) ||
               core == "dynamic_x86") {
      cpudecoder = &CPU_Core_Dyn_X86_Run;
...
#endif
#if (C_DYNREC)
    } else if ((core == "dynamic" && GetDynamicType() == 2) ||
               core == "dynamic_rec") {
      cpudecoder = &CPU_Core_Dynrec_Run;
#endif
    } else {
      ...
      cpudecoder = &CPU_Core_Normal_Run;
      LOG_MSG("CPU:Unknown core type %s, switching back to normal.",
              core.c_str());
    }

#if (C_DYNAMIC_X86)
    CPU_Core_Dyn_X86_Cache_Init((core == "dynamic" && GetDynamicType() == 1) ||
                                (core == "dynamic_x86") ||
                                (core == "dynamic_nodhfpu"));
#endif
#if (C_DYNREC)
    CPU_Core_Dynrec_Cache_Init((core == "dynamic" && GetDynamicType() == 2) ||
                               (core == "dynamic_rec"));
#endif
    ...
    if (cputype == "auto") {
      ...
      cpudecoder = &CPU_Core8086_Normal_Run;
    } else if (cputype == "8086_prefetch") { /* 6-byte prefetch queue ref
                                                [http://www.phatcode.net/res/224/files/html/ch11/11-02.html]
                                              */
      CPU_ArchitectureType = CPU_ARCHTYPE_8086;
      if (core == "normal") {
        cpudecoder = &CPU_Core8086_Prefetch_Run;
        CPU_PrefetchQueueSize = 4; /* Emulate the 8088, which was more common in
                                      home PCs than having an 8086 */
      } else if (core == "auto") {
        cpudecoder = &CPU_Core8086_Prefetch_Run;
        CPU_PrefetchQueueSize = 4; /* Emulate the 8088, which was more common in
                                      home PCs than having an 8086 */
      ...
      cpudecoder = &CPU_Core286_Normal_Run;
    } else if (cputype == "80186_prefetch") { /* 6-byte prefetch queue ref
                                                 [http://www.phatcode.net/res/224/files/html/ch11/11-02.html]
                                               */
      CPU_ArchitectureType = CPU_ARCHTYPE_80186;
      if (core == "normal") {
        cpudecoder = &CPU_Core286_Prefetch_Run; /* TODO: Alternate 16-bit only
                                                   decoder for 286 that does NOT
                                                   include 386+ instructions */
      ...
      cpudecoder = &CPU_Core286_Normal_Run;
    } else if (cputype == "286_prefetch") { /* 6-byte prefetch queue ref
                                               [http://www.phatcode.net/res/224/files/html/ch11/11-02.html]
                                             */
      CPU_ArchitectureType = CPU_ARCHTYPE_286;
      if (core == "normal") {
...
#else
      E_Exit("Pentium MMX emulation requires FPU emulation, which was not "
...

    {
      ...
      if (s != NULL && *s != 0) {
        ...
        while (*s != 0 && o < 12)
          cpu_custom_cpuid[o++] = *s++;
        while (o < 12)
          cpu_custom_cpuid[o++] = ' ';

        if (*s != 0 && o == 12)
          LOG_MSG("WARNING: CPUID string '%s' truncated. Maximum 12 characters",
                  os);
      }
    }
    ...
    if (enable_fpu) {
      if (CPU_ArchitectureType >= CPU_ARCHTYPE_486OLD) {
        if (FPU_ArchitectureType < FPU_ARCHTYPE_BEST) {
          LOG_MSG("WARNING: 486 or higher with 387 or lower FPU is an unusual "
                  "combination");
        }
      } else if (CPU_ArchitectureType >= CPU_ARCHTYPE_286) {
        if (FPU_ArchitectureType == FPU_ARCHTYPE_8087 ||
            FPU_ArchitectureType > FPU_ARCHTYPE_387) {
          LOG_MSG("WARNING: 286/386 with either 8087 or higher than 387 is an "
                  "unusual combination");
        }
      } else { /* 8086/80186 */
        if (FPU_ArchitectureType >= FPU_ARCHTYPE_287) {
          LOG_MSG("WARNING: 8086/80186 with anything higher than 8087 is an "
                  "unusual combination");
        }
      }
    }

    if (!enable_fpu && (cputype == "pentium" || cputype == "pentium_mmx" ||
                        cputype == "ppro_slow" || cputype == "pentium_ii"))
      LOG_MSG("WARNING: Disabling FPU support for this CPU type is unusual, "
              "may confuse DOS programs");
...

    {
      ...
      LOG(LOG_CPU, LOG_DEBUG)("PSE extensions: enabled=%u bits=%u", enable_pse,
                              enable_pse_extbits);
    }

    /* because of the way the BIOS writes certain entry points, a reboot is
     * required if changing between specific levels of CPU. These entry points
     * will fault the CPU otherwise. */
    ...
    if (cpu_rep_max < 0)
      cpu_rep_max = 4; /* compromise to help emulation speed without too much
                          loss of accuracy */
    ...
    if (enable_cmpxchg8b && CPU_ArchitectureType >= CPU_ARCHTYPE_PENTIUM)
      LOG_MSG("Pentium CMPXCHG8B emulation is enabled");
    ...
    const char *mask_stack_ptr_enter_leave =
        section->Get_string("mask stack pointer for enter leave instructions");
    if (!strcmp(mask_stack_ptr_enter_leave, "true") ||
        !strcmp(mask_stack_ptr_enter_leave, "1")) {
      mask_stack_ptr_on_enter = true;
    } else if (!strcmp(mask_stack_ptr_enter_leave, "false") ||
               !strcmp(mask_stack_ptr_enter_leave, "0")) {
    ...
    const char *lmsw_allow_pe_clear =
        section->Get_string("allow lmsw to exit protected mode");
    if (!strcmp(lmsw_allow_pe_clear, "true") ||
        !strcmp(lmsw_allow_pe_clear, "1")) {
      lmsw_allow_clear_pe_bit = true;
    } else if (!strcmp(lmsw_allow_pe_clear, "false") ||
               !strcmp(lmsw_allow_pe_clear, "0")) {
      lmsw_allow_clear_pe_bit = false;
    } else {
      /* Unless I see otherwise, all x86 architectures do not allow LMSW to
       * clear the PE bit. This is required for our VCPI implementation to work
       * because DOS4GW when using VCPI likes to SMSW CX, and off the PE bit,
       * and LMSW back, then continue on using the PROTECTED MODE segment
       * values. If allowed to clear PE, this would cause a crash. Why is DOS4GW
       * masking off PE when it clearly understands it cannot anyway? It does
...
uint16_t CPU_FindDecoderType(CPU_Decoder *decoder) {
  ...
  if (0) {
    ...
    decoder_idx = 0;
  else if (cpudecoder == &CPU_Core_Prefetch_Run)
    decoder_idx = 1;
#if !defined(C_EMSCRIPTEN)
    ...
    decoder_idx = 3;
  else if (cpudecoder == &CPU_Core_Normal_Trap_Run)
    decoder_idx = 100;
#endif
#if C_DYNAMIC_X86
  else if (cpudecoder == &CPU_Core_Dyn_X86_Run)
    decoder_idx = 4;
#endif
#if (C_DYNREC)
    ...
    decoder_idx = 100;
#if C_DYNAMIC_X86
  else if (cpudecoder == &CPU_Core_Dyn_X86_Trap_Run)
    decoder_idx = 101;
#endif
#if (C_DYNREC)
  else if (cpudecoder == &CPU_Core_Dynrec_Trap_Run)
    decoder_idx = 102;
#endif
...
CPU_Decoder *CPU_IndexDecoderType(uint16_t decoder_idx) {
  ...
  case 1:
    return &CPU_Core_Prefetch_Run;
#if !defined(C_EMSCRIPTEN)
  case 2:
    return &CPU_Core_Simple_Run;
  case 3:
    return &CPU_Core_Full_Run;
#endif
#if C_DYNAMIC_X86
  case 4:
    return &CPU_Core_Dyn_X86_Run;
#endif
#if (C_DYNREC)
  case 5:
    return &CPU_Core_Dynrec_Run;
#endif
  case 100:
    return &CPU_Core_Normal_Trap_Run;
#if C_DYNAMIC_X86
  case 101:
    return &CPU_Core_Dyn_X86_Trap_Run;
#endif
#if (C_DYNREC)
  case 102:
    return &CPU_Core_Dynrec_Trap_Run;
#endif
  case 200:
    return &HLT_Decode;
  default:
...
Bitu vm86_fake_io_offs[3 * 2] = {
    0}; /* offsets from base off because of dynamic core cache */
...
Bitu CPU_ForceV86FakeIO_In(Bitu port, Bitu len) {
  ...
  CALLBACK_RunRealFar(
      (uint16_t)vm86_fake_io_seg,
      (uint16_t)vm86_fake_io_offs[(len == 4 ? 2 : (len - 1)) + 0]);
...
void CPU_ForceV86FakeIO_Out(Bitu port, Bitu val, Bitu len) {
  ...
  CALLBACK_RunRealFar(
      (uint16_t)vm86_fake_io_seg,
      (uint16_t)vm86_fake_io_offs[(len == 4 ? 2 : (len - 1)) + 3]);
...
/* NTS: Windows XP does not set MSR 0x175, which means the SYSENTER entry point
 * begins to run with ESP == 0. But it loads ESP right away. */
/* FIXME: Why does this occasionally cause Windows XP to crash? */
bool CPU_SYSENTER() {
  if (!enable_syscall)
    return false;
  if (!cpu.pmode || cpu_sep_cs == 0)
    return false; /* CS != 0 and not real mode */
  ...
  //	UNBLOCKED_LOG(LOG_CPU,LOG_DEBUG)("SYSENTER: From CS=%04x
  //EIP=%08x",(unsigned int)Segs.val[cs],(unsigned int)reg_eip - 2);
  ...
  /* NTS: Do NOT use SetSegGeneral, SYSENTER is documented to set CS and SS
   * based on what was given to the MSR, but with fixed and very specific
   * descriptor cache values that represent 32-bit flat segments with base == 0
   * and limit == 4GB. */
  ...
  Segs.val[ss] = (cpu_sep_cs & 0xFFFC) +
                 0x8; /* Yes, really. Look it up in Intel's documentation */
  Segs.phys[ss] = 0;
  ...
  //	UNBLOCKED_LOG(LOG_CPU,LOG_DEBUG)("SYSENTER: CS=%04x EIP=%08x
  //ESP=%08x",(unsigned int)Segs.val[cs],(unsigned int)reg_eip,(unsigned
  //int)reg_esp);
  return true;
}
...
bool CPU_SYSEXIT() {
  if (!enable_syscall)
    return false;
  if (!cpu.pmode || cpu_sep_cs == 0 || cpu.cpl != 0)
    return false; /* CS != 0 and not real mode, or not ring 0 */
  ...
  //	UNBLOCKED_LOG(LOG_CPU,LOG_DEBUG)("SYSEXIT: From CS=%04x
  //EIP=%08x",(unsigned int)Segs.val[cs],(unsigned int)reg_eip - 2);
   ...
   * and limit == 4GB. */
  Segs.val[cs] =
      (cpu_sep_cs & 0xFFFC) + 0x10 +
      3 /*RPL*/; /* Yes, really. Look it up in Intel's documentation */
  Segs.phys[cs] = 0;
  Segs.limit[cs] = 0xFFFFFFFF;
  Segs.expanddown[cs] = false;
  cpu.code.big = true;
  ...
  //	UNBLOCKED_LOG(LOG_CPU,LOG_DEBUG)("SYSEXIT: CS=%04x EIP=%08x
  //ESP=%08x",(unsigned int)Segs.val[cs],(unsigned int)reg_eip,(unsigned
  //int)reg_esp);
  return true;
}
...
bool CPU_RDMSR() {
  ...
  case 0x00000017: /* Known in Linux kernel as MSR_IA32_PLATFORM_ID */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    /* NTS: Windows ME assumes this MSR is present if we report ourself as a
     * Pentium III and will BSOD if this does not return a value */
  ...
  case 0x0000001b: /* Local APIC */
    /* NTS: Windows ME assumes this MSR is present if we report ourself as a
     * Pentium II or Pentium Pro, instead of, you know, using CPUID */
  ...
  case 0x0000002a: /* MSR_IA32_EBL_CR_POWERON */
    ...
    return true;
  case 0x0000008b: /* Intel microcode revision... Windows ME insists on reading
                      this at startup if Pentium II and stepping 3 */
    /* NTS: Windows 98 will also read this register on boot up if emulating a
     * Pentium Pro */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII &&
        !(CPU_ArchitectureType == CPU_ARCHTYPE_PPROSLOW))
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "RDMSR: Guest is reading Intel microcode revision");
    if (CPU_ArchitectureType >= CPU_ARCHTYPE_PENTIUMIII) {
      // Taken from an actual Pentium III system. Windows ME will try to update
      // microcode if major version is too low, this value stops that. Also seen
      // on a 500MHz part: EDX=0xA EAX=0x0
      reg_edx = 0xE;
      reg_eax = 0x0;
    } else {
      // FIXME: This is a guess. Pull out the Pentium II DOS system and see what
      // comes back for this. These values are large enough to prevent Windows
      // ME from trying to update microcode.
      reg_edx = 0x003F003F;
      reg_eax = 0x001E03FF;
    }
    return true;
  case 0x000000ce: /* MSR_PLATFORM_INFO? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU,
                  LOG_NORMAL)("RDMSR: Attempt to read MSR_PLATFORM_INFO");
    reg_edx = reg_eax = 0;
    return true;
  ...
  case 0x0000011e: /* MSR_IA32_BBL_CR_CTL3 */
    ...
    return true;
  case 0x00000140: /* IA32_MISC_ENABLE
                      [https://www.geoffchappell.com/studies/windows/km/cpu/msr/misc_enable.htm]
                    */
    /* Linux kernel assumes this MSR is present if Pentium III and will crash
     * otherwise */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    reg_edx = reg_eax = 0;
  ...
  case 0x00000174: /* SYSENTER CS selector */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII || !enable_syscall)
      return false;
    reg_edx = 0;
    reg_eax = cpu_sep_cs;
    return true;
  ...
  case 0x00000186: /* MSR_P6_EVNTSEL0? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU,
                  LOG_NORMAL)("RDMSR: Attempt to read MSR_P6_EVNTSEL0");
    reg_edx = reg_eax = 0;
    return true;
  case 0x00000187: /* MSR_P6_EVNTSEL1? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU,
                  LOG_NORMAL)("RDMSR: Attempt to read MSR_P6_EVNTSEL1");
    reg_edx = reg_eax = 0;
    return true;
  default:
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)("RDMSR: Unknown register 0x%08lx",
...
bool CPU_WRMSR() {
  ...
  //	UNBLOCKED_LOG(LOG_CPU,LOG_DEBUG)("WRMSR ECX=%08x
  //EDX:EAX=%08x:%08x",reg_ecx,reg_edx,reg_eax);

  switch (reg_ecx) {
  case 0x00000010: /* You can change the time stamp counter by writing this MSR
                    */
    rdtsc_count_base =
        ((uint64_t)reg_edx << (uint64_t)32ul) + (uint64_t)reg_eax;
    rdtsc_pic_base = PIC_FullIndex();
    return true;
  case 0x0000001b: /* Local APIC */
    /* NTS: Windows ME assumes this MSR is present if we report ourself as a
     * Pentium II, instead of, you know, using CPUID. It will also set the
     * enable bit, even if this register was 0x00000000 when it booted.
     * Fortunately, Windows ME still runs properly if we silently ignore the
     * write and leave it 0x00000000. */
    /* NTS: Apparently the Linux kernel also assumes this register is present.
     */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII &&
        !(CPU_ArchitectureType == CPU_ARCHTYPE_PPROSLOW))
    ...
    if (reg_eax & 0x800)
      UNBLOCKED_LOG(LOG_CPU,
                    LOG_WARN)("Guest OS is attempting to enable the Local APIC "
                              "which we do not emulate yet");
    return true;
  case 0x00000079: /* Intel microcode update (EDX:EAX contains a virtual memory
                      address of a microcode blob) */
    /* NTS: Windows ME, if it sees a Pentium II stepping 3 or higher, will
     * attempt to do a microcode update at startup. Why? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII)
      return false;
    UNBLOCKED_LOG(LOG_CPU,
                  LOG_NORMAL)("WRMSR: Guest is attempting to update microcode "
                              "(is that you Windows ME?) EDX:EAX=%08x:%08x",
                              reg_edx, reg_eax);
    return true;
  case 0x0000008b: /* Intel microcode revision... why is Windows ME writing this
                      register before reading it? */
    /* NTS: Windows 98 will also write this register on boot up if emulating a
     * Pentium Pro */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII &&
        !(CPU_ArchitectureType == CPU_ARCHTYPE_PPROSLOW))
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "WRMSR: Attempt to write Intel microcode revision (is that you Windows "
        "ME?) EDX:EAX=%08x:%08x",
        reg_edx, reg_eax);
    return true;
  case 0x000000ce: /* MSR_PLATFORM_INFO? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "WRMSR: Attempt to write MSR_PLATFORM_INFO EDX:EAX=%08x:%08x", reg_edx,
        reg_eax);
    return true;
  case 0x00000119: /* MSR_IA32_BBL_CR_CTL */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII)
    ...
    if (p3psn.enabled && (reg_eax & 0x200000)) {
      UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)("WRMSR: MSR_IA32_BBL_CR_CTL guest is "
  ...
  case 0x00000140: /* IA32_MISC_ENABLE
    ...
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)("WRMSR: IA32_MISC_ENABLE %08x:%08x",
                                       reg_edx, reg_eax);
    return true;
  case 0x00000174: /* SYSENTER CS selector */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMII || !enable_syscall)
  ...
  case 0x00000186: /* MSR_P6_EVNTSEL0? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "WRMSR: Attempt to read MSR_P6_EVNTSEL0 EDX:EAX=%08x:%08x", reg_edx,
        reg_eax);
    return true;
  case 0x00000187: /* MSR_P6_EVNTSEL1? */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII)
      return false;
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "WRMSR: Attempt to read MSR_P6_EVNTSEL1 EDX:EAX=%08x:%08x", reg_edx,
        reg_eax);
    return true;
  default:
    UNBLOCKED_LOG(LOG_CPU, LOG_NORMAL)(
        "WRMSR: Unknown register 0x%08lx (write 0x%08lx:0x%08lx)",
        (unsigned long)reg_ecx, (unsigned long)reg_edx, (unsigned long)reg_eax);
    break;
  }
...
/* NTS: Hopefully by implementing this Windows ME can stop randomly crashing
 * when cputype=pentium */
/* NTS: Linux kernels compiled for "i686" rely on this instruction too. */
void CPU_CMPXCHG8B(PhysPt eaa) {
  uint32_t hi, lo;

  /* NTS: We assume that, if reading doesn't cause a page fault, writing won't
   * either */
...
private:
  ...
  void setBytes(std::istream &stream) override {
    ...
    if (decoder_idx < 100) {
      ...
      // trap -> run
      case 100:
        cpudecoder = CPU_IndexDecoderType(0);
        break;
      case 101:
        cpudecoder = CPU_IndexDecoderType(4);
        break;
      case 102:
        cpudecoder = CPU_IndexDecoderType(5);
        break;
    ...
    // switch to trap core
      ...
      case 2:
      case 3:
        cpudecoder = CPU_IndexDecoderType(100);
        break;
      case 4:
        cpudecoder = CPU_IndexDecoderType(101);
        break;
      case 5:
        cpudecoder = CPU_IndexDecoderType(102);
        break;
