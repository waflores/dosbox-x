...
/* CallBack are located at 0xF000:0x1000  (see CB_SEG and CB_SOFFSET in
   callback.h) And they are 16 bytes each and you can define them to behave in
   certain ways like a far return or and IRET
*/
...
void DBG_CALLBACK_Dump(void) {
  ...
  for (Bitu i = 0; i < CB_MAX; i++) {
    ...
    LOG_MSG(
        "  [%u] func=%p desc='%s'", (unsigned int)i,
        (void *)((
            uintptr_t)CallBack_Handlers[i]), /* shut the compiler up by func ->
                                                uintptr_t -> void* conversion */
...
uint8_t CALLBACK_Allocate(void) {
  for (uint8_t i = 1; (i < CB_MAX); i++) {
    if (CallBack_Handlers[i] == &illegal_handler) {
      if (CallBack_Description[i] != NULL)
        LOG_MSG("CALLBACK_Allocate() warning: empty slot still has description "
                "string!\n");
...
static Bitu stop_handler(void) { return CBRET_STOP; }
...
void CALLBACK_RunRealFarInt(uint16_t seg, uint16_t off) {
  ...
  if (cpu.pmode && !(reg_flags & FLAG_VM)) {
    mem_writew(SegPhys(ss) + reg_sp + 0,
               RealOff(CALLBACK_RealPointer(call_stop)));
    mem_writew(SegPhys(ss) + reg_sp + 2,
               RealSeg(CALLBACK_RealPointer(call_stop)));
    mem_writew(SegPhys(ss) + reg_sp + 4, (uint16_t)reg_flags);
  } else {
    // See commit
    // [https://github.com/joncampbell123/dosbox-x/commit/00378e8cc2bc5a71c3691be5f3bfd20246a26874]
    // which mentions UNLZEXE and something about the expectation of stack
    // pointer wraparound in real mode for this case.
...
template <std::uint32_t FLAG> inline void CALLBACK_SET_FLAG(bool const val) {
  uint32_t tempf;
  ...
  if (cpu.pmode && !(reg_flags & FLAG_VM)) {
    ...
    if (cpu.stack.big)
      tempf =
          real_readd(SegValue(ss), reg_esp + 8); // first word past FAR 32:32
    else
...
void CALLBACK_SetDescription(Bitu nr, const char *descr) {
  if (CallBack_Description[nr])
    delete[] CallBack_Description[nr];
...
Bitu CALLBACK_SetupExtra(Bitu callback, Bitu type, PhysPt physAddress,
  ...
  case CB_IRET_EOI_PIC1:
    ...
    phys_writeb(physAddress + 0x03,
                (uint8_t)0xe6); // out 0x20, al (IBM) / out 0x00, al (PC-98)
  ...
  case CB_IRQ0:                                     // timer int8
    ...
    phys_writew(
        physAddress + 0x09,
        (uint16_t)(IS_PC98_ARCH
                       ? 0x00e6
                       : 0x20e6)); // out 0x20, al / out 0x00, al (PC-98)
                                   // (FIXME: PC-98 does not have INT 1Ch)
  ...
  case CB_IRQ1: // keyboard int9
    if (machine == MCH_PCJR) {
      /*      IBM PCjr reads the infrared input on NMI interrupt, which then
       * calls INT 48h to translate to IBM PC/XT scan codes before passing AL
       * directly to IRQ1 (INT 9). PCjr keyboard handlers, including games made
       * for the PCjr, assume the scan code is in AL and do not read the I/O
       * port. */
      if (use_cb) {
        phys_writeb(physAddress + 0x00, (uint8_t)0xFE); // GRP 4
        phys_writeb(physAddress + 0x01,
        ...
        phys_writew(physAddress + 0x02,
                    (uint16_t)callback); // The immediate word
        physAddress += 4;
      }
      ...
      phys_writeb(physAddress + 0x00, (uint8_t)0x50); // push ax
      if (IS_PC98_ARCH) {
        /* NTS: NEC PC-98 does not have keyboard input on port 60h, it's a 8251
         * UART elsewhere. */
      ...
      if (IS_PC98_ARCH || IS_TANDY_ARCH) {
        ...
        phys_writew(physAddress + 0x06,
                    (uint16_t)0x9090); // nop, nop (PC-98 does not have INT 15h
                                       // keyboard hook)
      } else {
      ...
      if (use_cb) {
        ...
        phys_writeb(physAddress + 0x0b,
                    (uint8_t)0x38); // Extra Callback instruction
        phys_writew(physAddress + 0x0c,
                    (uint16_t)callback); // The immediate word
        // jump here to (skip):
        physAddress += 6;
      }
      ...
      phys_writew(physAddress + 0x0b,
                  (uint16_t)(IS_PC98_ARCH ? 0x00e6 : 0x20e6)); // out 0x20, al
  ...
  case CB_IRQ1_BREAK: // return from int9, when Ctrl-Break is detected; invokes
                      // int 1b
  ...
  case CB_INT29: // fast console output
    if (IS_PC98_ARCH)
      LOG_MSG("WARNING: CB_INT29 callback setup not appropriate for PC-98 mode "
              "(INT 10h no longer BIOS call)");
    if (use_cb) {
      phys_writeb(physAddress + 0x00, (uint8_t)0xFE); // GRP 4
      phys_writeb(physAddress + 0x01,
                  (uint8_t)0x38); // Extra Callback instruction
  ...
  case CB_TDE_IRET: // TandyDAC end transfer
    ...
    /*	case CB_IPXESR:		// IPX ESR
                    if (!use_cb) E_Exit("ipx esr must implement a callback
       handler!"); phys_writeb(physAddress+0x00,(uint8_t)0x1e);		// push
       ds phys_writeb(physAddress+0x01,(uint8_t)0x06);		// push es
                    phys_writew(physAddress+0x02,(uint16_t)0xa00f);	// push
       fs phys_writew(physAddress+0x04,(uint16_t)0xa80f);	// push gs
                    phys_writeb(physAddress+0x06,(uint8_t)0x60);
       // pusha phys_writeb(physAddress+0x07,(uint8_t)0xFE);		//GRP 4
                    phys_writeb(physAddress+0x08,(uint8_t)0x38);
       //Extra Callback instruction
                    phys_writew(physAddress+0x09,(uint16_t)callback);	//The
       immediate word phys_writeb(physAddress+0x0b,(uint8_t)0xCB);
       //A RETF Instruction return 0x0c; case CB_IPXESR_RET:		// IPX
       ESR return if (use_cb) E_Exit("ipx esr return must not implement a
       callback handler!"); phys_writeb(physAddress+0x00,(uint8_t)0xfa);
       // cli phys_writew(physAddress+0x01,(uint16_t)0x20b0);	// mov al, 0x20
                    phys_writew(physAddress+0x03,(uint16_t)0xa0e6);	// out
       0xa0, al phys_writew(physAddress+0x05,(uint16_t)0x20e6);	// out 0x20, al
                    phys_writeb(physAddress+0x07,(uint8_t)0x61);
       // popa phys_writew(physAddress+0x08,(uint16_t)0xA90F);	// pop gs
                    phys_writew(physAddress+0x0a,(uint16_t)0xA10F);	// pop
       fs phys_writeb(physAddress+0x0c,(uint8_t)0x07);		// pop es
                    phys_writeb(physAddress+0x0d,(uint8_t)0x1f);
       // pop ds phys_writeb(physAddress+0x0e,(uint8_t)0xcf);		//An
       IRET Instruction return 0x0f; */
  case CB_INT21:
    phys_writeb(physAddress + 0x00, (uint8_t)0xFB); // STI
    if (use_cb) {
      phys_writeb(physAddress + 0x01, (uint8_t)0xFE); // GRP 4
  ...
  case CB_VESA_WAIT:
    if (use_cb)
      E_Exit("VESA wait must not implement a callback handler!");
    phys_writeb(physAddress + 0x00, (uint8_t)0xFB);    // sti
  ...
  case CB_IRET_EOI_PIC2:
    ...
    phys_writeb(physAddress + 0x02, (uint8_t)0x20);
    phys_writeb(physAddress + 0x03,
                (uint8_t)0xe6); // out 0xA0, al (IBM) / out 0x08, al (PC-98)
    phys_writeb(physAddress + 0x04, (uint8_t)(IS_PC98_ARCH ? 0x08 : 0xA0));
    phys_writeb(physAddress + 0x05,
                (uint8_t)0xe6); // out 0x20, al (IBM) / out 0x00, al (PC-98)
...
bool CALLBACK_Setup(Bitu callback, CallBack_Handler handler, Bitu type,
  ...
  if (callback >= CB_MAX)
    return false;
  CALLBACK_SetupExtra(callback, type, CALLBACK_PhysPointer(callback) + 0,
                      (handler != NULL));
...
Bitu CALLBACK_Setup(Bitu callback, CallBack_Handler handler, Bitu type,
                    PhysPt addr, const char *descr) {
  if (callback >= CB_MAX)
    return 0;
  ...
  if (m_type == CALLBACK_HandlerObject::SETUP) {
    if (vectorhandler.installed && MemBase != NULL && !cpu.pmode) {
      ...
      if (RealGetVec(vectorhandler.interrupt) == Get_RealPointer()) {
        ...
        LOG(LOG_MISC, LOG_WARN)("Interrupt vector changed on %X %s",
                                vectorhandler.interrupt,
                                CALLBACK_GetDescription(m_callback));
    }
...
void CALLBACK_HandlerObject::Install(CallBack_Handler handler, Bitu type,
...
void CALLBACK_HandlerObject::Install(CallBack_Handler handler, Bitu type,
                                     PhysPt addr, const char *description) {
  if (!installed) {
    installed = true;
    m_type = SETUP;
    m_callback = CALLBACK_Allocate();
...
void CALLBACK_HandlerObject::Allocate(CallBack_Handler handler,
                                      const char *description) {
  if (!installed) {
    installed = true;
    m_type = NONE;
...
void CALLBACK_HandlerObject::Set_RealVec(uint8_t vec, bool reinstall) {
  if (!vectorhandler.installed || reinstall) {
    ...
    E_Exit("double usage of vector handler");
}
...
void CALLBACK_Init() {
  {
     ...
     *
     * CB_MAX entries CB_SIZE each, where executable x86 code is written per
     * callback, followed by 256 entries 6 bytes each corresponding to an
     * interrupt call */
    Bitu o;

    LOG(LOG_MISC, LOG_DEBUG)("Initializing DOSBox callback instruction system");

    o = ROMBIOS_GetMemory((CB_MAX * CB_SIZE) + (256 * 6),
                          "DOSBox callback area", /*align*/ 4);
    if (o == 0)
      E_Exit("Cannot allocate callback area");
    CB_SOFFSET = o & 0xFFFF;
    CB_SEG = (o >> 4) & 0xF000;
    if (((Bitu)CB_SOFFSET + (CB_MAX * CB_SIZE) + (256 * 6)) > 0x10000)
      E_Exit("Callback area spans 64KB segment");

    o = ROMBIOS_GetMemory(14 /*2+2+3+2+2+3*/, "DOSBox vm86 hack", /*align*/ 4);
    if (o == 0)
      E_Exit("Cannot allocate vm86 hack");
    vm86_fake_io_off = o & 0xFFFF;
    vm86_fake_io_seg = (o >> 4) & 0xF000;
    if ((vm86_fake_io_off + 14) > 0x1000000)
      E_Exit("vm86 area spans 64KB segment");
  }

  LOG(LOG_CPU, LOG_DEBUG)("Callback area starts at %04x:%04x", CB_SEG,
                          CB_SOFFSET);
  ...
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x00,
              (uint8_t)0xec); // in al, dx
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x01, (uint8_t)0xcb); // retf
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x02,
              (uint8_t)0xed); // in ax, dx
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x03, (uint8_t)0xcb); // retf
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x04,
              (uint8_t)0x66); // in eax, dx
  ...
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x08,
              (uint8_t)0xee); // out dx, al
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x09, (uint8_t)0xcb); // retf
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x0a,
              (uint8_t)0xef); // out dx, ax
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x0b, (uint8_t)0xcb); // retf
  phys_writeb(CALLBACK_PhysPointer(call_priv_io) + 0x0c,
              (uint8_t)0x66); // out dx, eax
