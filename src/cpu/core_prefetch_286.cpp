...
/* wrap original variable name in inline function prior to #define, use inline
 * function name in redefine of do_seg_limits */
static inline bool _seg_limit_check(void) { return do_seg_limits; }

/* Do not emulate segment limit exceptions on 80186, DO emulate on 286. */
#define do_seg_limits                                                          \
  (_seg_limit_check() && CPU_ArchitectureType >= CPU_ARCHTYPE_286)
...
#define LoadMq(off)                                                            \
  ((uint64_t)((uint64_t)mem_readd_inline(off + 4) << 32 |                      \
              (uint64_t)mem_readd_inline(off)))
#define SaveMb(off, val) mem_writeb_inline(off, val)
#define SaveMw(off, val) mem_writew_inline(off, val)
#define SaveMd(off, val) mem_writed_inline(off, val)
#define SaveMq(off, val)                                                       \
  {                                                                            \
    mem_writed_inline(off, val & 0xffffffff);                                  \
    mem_writed_inline(off + 4, (val >> 32) & 0xffffffff);                      \
  }
