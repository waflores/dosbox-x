...
template <class T> static inline void prefetch_read_check(const Bitu w) {
  ...
  if (!pq_valid)
    E_Exit("CPU: Prefetch read when not valid!");
  if (w < pq_start)
    E_Exit("CPU: Prefetch read below prefetch base");
  if ((w + sizeof(T)) > pq_fill)
    E_Exit("CPU: Prefetch read beyond prefetch fill");
#endif
}
...
static inline void prefetch_lazyflush(const Bitu w) {
  ...
  if ((w - pq_start) >= pq_limit) {
    memmove(prefetch_buffer, prefetch_buffer + prefetch_unit,
            pq_limit - prefetch_unit);
...
template <class T> static inline T Fetch(void) {
  ...
  if (prefetch_hit<T>(core.cseip)) {
    ...
    prefetch_init(core.cseip &
                  (~(prefetch_unit -
                     1ul))); /* fill prefetch starting on DWORD boundary */
    prefetch_refill(pq_start +
                    pq_reload); /* perhaps in the time it takes for a prefetch
                                   miss the 80486 can load two DWORDs */
    temp = prefetch_read<T>(core.cseip);
#ifdef PREFETCH_DEBUG
    pq_miss++;
#endif
