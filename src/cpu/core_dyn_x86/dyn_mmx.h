...
static void MMX_LOAD_32(PhysPt addr) { mmxtmp.ud.d0 = mem_readd(addr); }

static void MMX_STORE_32(PhysPt addr) { mem_writed(addr, mmxtmp.ud.d0); }
...
static void dyn_mmx_mem(Bitu op, Bitu reg = decode.modrm.reg,
                        void *mem = &mmxtmp) {
#if C_TARGETCPU == X86
  cache_addw(0x0F | (op << 8));
...
static void dyn_mmx_op(Bitu op) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea();
    // dyn_call_function_pagefault_check((void*)&gen_mmx_op, "%I%I%I%Ddr", op,
    // decode.modrm.val, imm, DREG(EA));
    dyn_call_function_pagefault_check((void *)&MMX_LOAD_64, "%Ddr", DREG(EA));
    dyn_mmx_mem(op);
  } else {
    // if ((op == 0x71) || (op == 0x72) || (op == 0x73)) decode_fetchb_imm(imm);
    // dyn_call_function_pagefault_check((void*)&gen_mmx_op, "%I%I%I", op,
    // decode.modrm.val, imm);
