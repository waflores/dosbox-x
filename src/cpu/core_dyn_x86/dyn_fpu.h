...
static void FPU_FDECSTP() { TOP = (TOP - 1) & 7; }

static void FPU_FINCSTP() { TOP = (TOP + 1) & 7; }

static void FPU_FNSTCW(PhysPt addr) { mem_writew(addr, fpu.cw); }

static void FPU_FFREE(Bitu st) { fpu.tags[st] = TAG_Empty; }
...
static void dyn_fpu_esc0() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    dyn_fill_ea();
    dyn_call_function_pagefault_check((void *)&FPU_FLD_F32_EA, "%Drd",
                                      DREG(EA));
    gen_load_host(&FPUSW, DREG(TMPB), 4);
    gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
    gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
...
static void dyn_fpu_esc1() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    case 0x00: /* FLD float*/
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FLD_F32, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
    case 0x01: /* UNKNOWN */
    ...
    case 0x03: /* FSTP float*/
      ...
      break;
    case 0x04: /* FLDENV */
      dyn_call_function_pagefault_check((void *)&FPU_FLDENV, "%Drd%Ib",
                                        DREG(EA), !decode.big_op);
      break;
    case 0x05: /* FLDCW */
      dyn_call_function_pagefault_check((void *)&FPU_FLDCW, "%Drd", DREG(EA));
      break;
    case 0x06: /* FSTENV */
      dyn_call_function_pagefault_check((void *)&FPU_FSTENV, "%Drd%Ib",
                                        DREG(EA), !decode.big_op);
      break;
    case 0x07: /* FNSTCW*/
...
static void dyn_fpu_esc2() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    dyn_fill_ea();
    dyn_call_function_pagefault_check((void *)&FPU_FLD_I32_EA, "%Drd",
                                      DREG(EA));
    gen_load_host(&FPUSW, DREG(TMPB), 4);
    gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
    gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
...
static void dyn_fpu_esc3() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    case 0x04:
      ...
      case 0x04: // FNSETPM
      case 0x05: // FRSTPM
        //				LOG(LOG_FPU,LOG_ERROR)("80267 protected
        //mode (un)set. Nothing done");
        break;
      default:
    ...
    case 0x00: /* FILD */
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FLD_I32, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
    case 0x01: /* FISTTP */
...
static void dyn_fpu_esc4() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    dyn_fill_ea();
    dyn_call_function_pagefault_check((void *)&FPU_FLD_F64_EA, "%Drd",
                                      DREG(EA));
    gen_load_host(&FPUSW, DREG(TMPB), 4);
    gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
    gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
...
static void dyn_fpu_esc5() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    case 0x00: /* FLD double real*/
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FLD_F64, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
    ...
    case 0x03: /* FSTP double real*/
      ...
      break;
    case 0x04: /* FRSTOR */
      dyn_call_function_pagefault_check((void *)&FPU_FRSTOR, "%Drd%Ib",
                                        DREG(EA), !decode.big_op);
      break;
    case 0x06: /* FSAVE */
      dyn_call_function_pagefault_check((void *)&FPU_FSAVE, "%Drd%Ib", DREG(EA),
                                        !decode.big_op);
      break;
    case 0x07: /*FNSTSW */
      gen_protectflags();
      gen_load_host(&fpu.sw, DREG(TMPB), 4);
      dyn_call_function_pagefault_check((void *)&mem_writew, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
    default:
...
static void dyn_fpu_esc6() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    dyn_fill_ea();
    dyn_call_function_pagefault_check((void *)&FPU_FLD_I16_EA, "%Drd",
                                      DREG(EA));
    gen_load_host(&FPUSW, DREG(TMPB), 4);
    gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
    gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
...
static void dyn_fpu_esc7() {
  ...
  if (decode.modrm.val >= 0xc0) {
    ...
    case 0x00: /* FILD Bit16s */
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FLD_I16, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
    case 0x01:
    ...
    case 0x04: /* FBLD packed BCD */
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FBLD, "%Drd%Drd", DREG(EA),
                                        DREG(TMPB));
      break;
    case 0x05: /* FILD Bit64s */
      ...
      gen_sop_word_imm(SHIFT_SHR, true, DREG(TMPB), 11);
      gen_dop_word_imm(DOP_AND, true, DREG(TMPB), 7);
      dyn_call_function_pagefault_check((void *)&FPU_FLD_I64, "%Drd%Drd",
                                        DREG(EA), DREG(TMPB));
      break;
