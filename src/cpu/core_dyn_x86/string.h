...
static void dyn_string(STRING_OP_DYNX86 op) {
  ...
  if (usedi) {
    ...
    if (cmp) {
      switch (op & 3) {
      case 0:
        dyn_read_byte(DREG(EA), DREG(TMPD), false);
        break;
      case 1:
        dyn_read_word(DREG(EA), DREG(TMPD), false);
        break;
      case 2:
        dyn_read_word(DREG(EA), DREG(TMPD), true);
        break;
      }
      gen_needflags();
    }
    ...
    if (cmp) {
      ...
      if (decode.rep) {
        ...
        rep_cmp_jmp =
            gen_create_branch_long(decode.rep == REP_NZ ? BR_Z : BR_NZ);
      }
    }
  }
