...
static void gen_clearzeroflag(void) {
  if (x86gen.flagsactive)
    IllegalOption("gen_clearzeroflag");
...
static void gen_dop_byte(DualOps op, DynReg *dr1, uint8_t di1, DynReg *dr2,
                         uint8_t di2) {
  GenReg *gr1 = FindDynReg(dr1);
...
static void gen_dop_byte_imm_mem(DualOps op, DynReg *dr1, uint8_t di1,
                                 void *data) {
  GenReg *gr1 = FindDynReg(dr1);
  uint16_t tmp;
  switch (op) {
...
static void gen_dop_word(DualOps op, bool dword, DynReg *dr1, DynReg *dr2) {
  ...
  case DOP_XCHG:
    ...
    if (dword && !((dr1->flags & DYNFLG_HAS8) ^ (dr2->flags & DYNFLG_HAS8))) {
      dr1->genreg = gr2;
      dr1->genreg->dynreg = dr1;
      dr2->genreg = gr1;
      dr2->genreg->dynreg = dr2;
      dr1->flags |= DYNFLG_CHANGED;
      return;
    }
    tmp = 0x87;
...
static void gen_sop_word(SingleOps op, bool dword, DynReg *dr1) {
  ...
  switch (op) {
  case SOP_INC:
    cache_addb(0x40 + gr1->index);
    break;
  case SOP_DEC:
    cache_addb(0x48 + gr1->index);
    break;
  case SOP_NOT:
    cache_addw(0xd0f7 + (gr1->index << 8));
    break;
  case SOP_NEG:
    cache_addw(0xd8f7 + (gr1->index << 8));
    break;
  default:
...
static void gen_shift_byte_cl(Bitu op, DynReg *dr1, uint8_t di1,
                              DynReg *drecx) {
...
static void gen_shift_word_imm(Bitu op, bool dword, DynReg *dr1, uint8_t imm) {
  ...
  if (!dword) {
    cache_addd(0x66 |
               ((0xc0c1 + ((uint16_t)op << 11) + (gr1->index << 8)) | imm << 16)
                   << 8);
  } else {
...
static void gen_mul_byte(bool imul, DynReg *dyn_ax, DynReg *dr1, uint8_t di1) {
  ...
  GenReg *gr1 = FindDynReg(dr1);
  if (imul)
    cache_addw(0xe8f6 + ((gr1->index + di1) << 8));
  else
    cache_addw(0xe0f6 + ((gr1->index + di1) << 8));
  dyn_ax->flags |= DYNFLG_CHANGED;
}
...
static void gen_mul_word(bool imul, DynReg *dyn_ax, DynReg *dyn_dx, bool dword,
  ...
  if (!dword)
    cache_addb(0x66);
  if (imul)
    cache_addw(0xe8f7 + (gr1->index << 8));
  else
    cache_addw(0xe0f7 + (gr1->index << 8));
  dyn_ax->flags |= DYNFLG_CHANGED;
  dyn_dx->flags |= DYNFLG_CHANGED;
}
...
static void gen_dshift_imm(bool dword, bool left, DynReg *dr1, DynReg *dr2,
  ...
  if (!dword)
    cache_addb(0x66);
  if (left)
    cache_addw(0xa40f); // SHLD IMM
  else
    cache_addw(0xac0f); // SHRD IMM
...
static void gen_dshift_cl(bool dword, bool left, DynReg *dr1, DynReg *dr2,
  ...
  if (!dword)
    cache_addb(0x66);
  if (left)
    cache_addw(0xa50f); // SHLD CL
  else
    cache_addw(0xad0f); // SHRD CL
  cache_addb(0xc0 + gr1->index + (gr2->index << 3));
  dr1->flags |= DYNFLG_CHANGED;
}
...
static void gen_call_function(void *func, char const *ops, ...) {
  ...
  cache_addb(0xe8);
  cache_addd((uintptr_t)func - (uintptr_t)cache_rwtox(cache.pos) -
             (uintptr_t)4);
...
static void gen_save_flags(DynReg *dynreg) {
  if (GCC_UNLIKELY(x86gen.flagsactive))
