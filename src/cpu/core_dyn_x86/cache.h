...
public:
  CodePageHandler() { invalidation_map = NULL; }
  void SetupAt(Bitu _phys_page, PageHandler *_old_pagehandler) {
    phys_page = _phys_page;
    old_pagehandler = _old_pagehandler;
    flags = old_pagehandler->flags |
  ...
  void writeb(PhysPt addr, uint8_t val) override {
    if (GCC_UNLIKELY(old_pagehandler->flags & PFLAG_HASROM))
      return;
    if (GCC_UNLIKELY((old_pagehandler->flags & PFLAG_READABLE) !=
                     PFLAG_READABLE)) {
      E_Exit("wb:non-readable code page found that is no ROM page");
    }
    addr &= 4095;
    if (host_readb(hostmem + addr) == (uint8_t)val)
      return;
  ...
  void writew(PhysPt addr, uint16_t val) override {
    ...
    addr &= 4095;
    if (host_readw(hostmem + addr) == (uint16_t)val)
      return;
  ...
  void writed(PhysPt addr, uint32_t val) override {
    ...
    addr &= 4095;
    if (host_readd(hostmem + addr) == (uint32_t)val)
      return;
  ...
  if (written > block->cache.size) {
    if (!block->cache.next) {
      if (written > block->cache.size + CACHE_MAXSIZE)
        E_Exit("CacheBlock overrun 1 %d", (int)(written - block->cache.size));
    } else {
      E_Exit("CacheBlock overrun 2 written %d size %d", (int)written,
             (int)block->cache.size);
    }
  } else {
    Bitu new_size;
