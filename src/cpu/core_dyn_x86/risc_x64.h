...
static const int reg_args[4] = {X64_REG_RCX, X64_REG_RDX, X64_REG_R8,
                                X64_REG_R9};
#define ARG0_REG 1
#define ARG1_REG 2
#define CALLSTACK 40
...
static const int reg_args[4] = {X64_REG_RDI, X64_REG_RSI, X64_REG_RDX,
                                X64_REG_RCX};
#define ARG0_REG 7
#define ARG1_REG 6
#define CALLSTACK 8
...
public:
  ...
  opcode(int reg, bool dword = true, Bitu acc = 1)
      : is_word(!dword), imm_size(0), rex(0) {
    setreg(reg, acc);
  }
  ...
  opcode &setimm(uint64_t _imm, int size) {
    imm = _imm;
    imm_size = size;
    return *this;
  }
  ...
  opcode &setreg(int r,
                 Bitu acc = 1); // acc: 0=low byte, 1=word/dword, 4=high byte
        ...
        if ((modrm & 7) == 5) {
          ...
          if ((int32_t)diff == diff)
            ...
            if ((int32_t)offset != offset)
              IllegalOption("opcode::Emit: bad RIP address");
          ...
          break;
      case 2:
        cache_addd((uint32_t)offset);
        break;
      case 1:
        cache_addb((uint8_t)offset);
        break;
      }
    }
    EmitImm();
...
opcode &opcode::setreg(int r, Bitu acc) {
  if (acc == 4) {
    if (r > 3 || rex)
      IllegalOption("opcode::setreg: cannot encode high byte");
    r += 4;
  } else if (acc == 0 && r > 3)
    rex |= 0x40;
  reg = r;
...
opcode &opcode::setrm(int r, Bitu acc) {
  ...
  if (acc == 4) {
    if (r > 3 || rex)
      IllegalOption("opcode::setrm: cannot encode high byte");
    r += 4;
  } else if (acc == 0 && r > 3)
    rex |= 0x40;
...
opcode &opcode::setabsaddr(void *addr) {
   ...
   * < 0x80000000 or >= 0xFFFFFFFF80000000 (signed 32-bit absolute) enc:
   * modrm+sib+4 bytes
   */
  if (reg >= 8)
    rex |= 0x44;
...
static BlockReturnDynX86 gen_runcodeInit(uint8_t *code) {
  ...
  // mov rbp, &cpu_regs
  /* NTS: For VS2019, mere typecasting won't correctly detect whether the
     pointer is small enough to fit in 32 bits. We have to mask and compare. If
     the pointer is above 4GB and the wrong choice is made, dynamic core will
     crash. --J.C. */
  if ((((Bitu)&cpu_regs) & (Bitu)0xFFFFFFFFul) == (Bitu)&cpu_regs)
    opcode(5).setimm((Bitu)&cpu_regs, 4).Emit8Reg(0xB8);
  else
    opcode(5).set64().setimm((Bitu)&cpu_regs, 8).Emit8Reg(0xB8);
  opcode(13).Emit8Reg(0x50); // push r13
  opcode(12).Emit8Reg(0x50); // push r12
  opcode(3).Emit8Reg(0x50);  // push rbx
  opcode(0)
      .setea(5, -1, 0, offsetof(CPU_Regs, flags))
      .Emit8(0x8B); // mov eax, [reg_flags(rbp)]
#if defined(_WIN64)
  ...
  *(uint32_t *)diff = (uint32_t)(cache.pos - diff - 4);
  ...
  opcode(1)
      .setea(5, -1, 0, offsetof(CPU_Regs, flags))
      .Emit8(0x33);                                     // xor ecx, reg_flags
  opcode(4).setrm(1).setimm(FMASK_TEST, 4).Emit8(0x81); // and ecx,FMASK_TEST
  opcode(1)
      .setea(5, -1, 0, offsetof(CPU_Regs, flags))
      .Emit8(0x31); // xor reg_flags, ecx

  opcode(4)
      .set64()
      .setea(4, -1, 0, CALLSTACK)
      .Emit8(0x8B); // mov rsp, [rsp+8/40]
#if defined(_WIN64)
...
static void gen_preloadreg(DynReg *dynreg) { FindDynReg(dynreg); }
...
static void gen_discardflags(void) {
  if (!x64gen.flagsactive) {
    ...
    opcode(0)
        .set64()
        .setrm(4)
        .setimm(CALLSTACK + 8, 1)
        .Emit8(0x83); // add rsp,16/48
  }
}
...
static void gen_needcarry(void) {
  if (!x64gen.flagsactive) {
    ...
    opcode(4)
        .setea(4, -1, 0, CALLSTACK)
        .setimm(0, 1)
        .Emit16(0xBA0F); // bt [rsp+8/40], 0
    opcode(4)
        .set64()
        .setea(4, -1, 0, CALLSTACK + 8)
        .Emit8(0x8D); // lea rsp, [rsp+16/48]
  }
}
...
static void gen_setzeroflag(void) {
        ...
        opcode(1).setea(4,-1,0,CALLSTACK).setimm(0x40,1).Emit8(0x83); // or
dword [rsp+8/40],0x40
}*/
...
static void gen_clearzeroflag(void) {
        ...
        opcode(4).setea(4,-1,0,CALLSTACK).setimm(~0x40,1).Emit8(0x83); // and
dword [rsp+8/40],~0x40
}*/
...
static void gen_load_arg_reg(int argno, DynReg *dr, const char *s) {
  ...
  case 'h':
    if (src) {
      if (src->index > 3 || gen->index > 3) {
        ...
        op.setrm(src->index, 4);
    } else
      op.setabsaddr(((uint8_t *)dr->data) + 1);
    op.Emit16(0xB60F); // movzx r32, r/m8
    break;
  case 'l':
    if (src)
      op.setrm(src->index, 0);
    else
      op.setabsaddr(dr->data);
    op.Emit16(0xB60F); // movzx r32, r/m8
    break;
  case 'w':
    if (src)
  ...
  case 'd':
    if (src) {
      if (src != gen)
        op.setrm(src->index).Emit8(0x8B);
    } else
      op.setabsaddr(dr->data).Emit8(0x8B);
    break;
  default:
...
static void gen_load_imm(int index, Bitu imm) {
  if (imm == 0)
    ...
    opcode(0).set64().setimm(imm, 4).setrm(index).Emit8(
        0xC7); // mov r64, simm32
  else
    opcode(index).set64().setabsaddr((void *)imm).Emit8(0x8D); // lea r64, [imm]
}

static void gen_dop_byte(DualOps op, DynReg *dr1, Bitu di1, DynReg *dr2,
                         Bitu di2) {
  uint8_t tmp;
...
static void gen_dop_byte_imm(DualOps op, DynReg *dr1, Bitu di1, Bitu imm) {
  ...
  case DOP_TEST:
    ...
    goto nochange;
  case DOP_MOV:
    i.setreg(dst, di1).Emit8Reg(0xB0);
    dr1->flags |= DYNFLG_CHANGED;
    return;
  default:
...
static void gen_dop_byte_imm_mem(DualOps op, DynReg *dr1, Bitu di1,
                                 void *data) {
  opcode i;
  Bits addr = (Bits)data;
  ...
  if (ripdiff < 0)
    ripdiff = ~ripdiff + 32;
  if ((int32_t)addr == addr || (int32_t)rbpdiff == rbpdiff ||
      ripdiff < 0x7FFFFFE0ll)
    ...
    if ((uint32_t)addr == (Bitu)addr)
      opcode(src).setimm(addr, 4).Emit8Reg(0xB8);
    else
      opcode(src).setimm(addr, 8).set64().Emit8Reg(0xB8);
...
static void gen_extend_word(bool sign, DynReg *ddr, DynReg *dsr) {
  if (ddr == dsr && dsr->genreg == NULL)
    opcode(FindDynReg(ddr, true)->index)
        .setabsaddr(dsr->data)
        .Emit16(sign ? 0xBF0F : 0xB70F);
  else {
    int src = FindDynReg(dsr)->index;
    int dst = FindDynReg(ddr, true)->index;
    if (sign && (src | dst) == 0)
      cache_addb(0x98); // cwde
    else
      opcode(dst).setrm(src).Emit16(sign ? 0xBF0F
                                         : 0xB70F); // movsx/movzx dst32, src16
  }

  ddr->flags |= DYNFLG_CHANGED;
}

static void gen_extend_byte(bool sign, bool dword, DynReg *ddr, DynReg *dsr,
                            Bitu dsi) {
  if (ddr == dsr && dword && dsr->genreg == NULL) {
    opcode op = opcode(FindDynReg(ddr, true)->index);
    if (dsi)
      op.setabsaddr((void *)(((uint8_t *)dsr->data) + 1));
    else
      op.setabsaddr(dsr->data);
    ...
    if (sign && !dword && (src | dst | dsi) == 0)
      cache_addw(0x9866); // cbw
    else
...
static void gen_lea(DynReg *ddr, DynReg *dsr1, DynReg *dsr2, Bitu scale,
                    Bits imm) {
...
static void gen_sop_word_imm(ShiftOps op, bool dword, DynReg *dr1,
                             uint8_t imm) {
  uint8_t tmp = 0xC1;
  int dst = FindDynReg(dr1, dword && (unsigned int)op == (unsigned int)DOP_MOV)
                ->index;
  opcode i;
  i.setimm(imm, 1);
...
static void gen_dop_word_var(DualOps op, bool dword, DynReg *dr1, void *drd) {
  opcode i = opcode(FindDynReg(dr1, dword && op == DOP_MOV)->index, dword)
                 .setabsaddr(drd);
  gen_dop_word(op, dr1, i);
}

static void gen_dop_word_imm_mem(DualOps op, bool dword, DynReg *dr1,
                                 void *data) {
  opcode i;
  Bits addr = (Bits)data;
  Bits rbpdiff = addr - (Bits)&cpu_regs;
  ...
  if ((int32_t)addr == addr || (int32_t)rbpdiff == rbpdiff ||
      ripdiff < 0x7FFFFFE0ll)
    i = opcode(FindDynReg(dr1, dword && op == DOP_MOV)->index, dword)
            .setabsaddr(data);
...
static void gen_imul_word(bool dword, DynReg *dr1, DynReg *dr2) {
  // dr1 = dr1*dr2
  opcode(FindDynReg(dr1)->index, dword)
      .setrm(FindDynReg(dr2)->index)
      .Emit16(0xAF0F);
  dr1->flags |= DYNFLG_CHANGED;
}
...
static void gen_imul_word_imm(bool dword, DynReg *dr1, DynReg *dr2, Bits imm) {
  ...
  if (dr1 == dr2 && dword && dr1->genreg == NULL)
    ...
    op = opcode(FindDynReg(dr1, dword && dr1 != dr2)->index, dword)
             .setrm(FindDynReg(dr2)->index);

  if ((int8_t)imm == imm)
    op.setimm(imm, 1).Emit8(0x6B);
  else
    op.setimm(imm, dword ? 4 : 2).Emit8(0x69);
  dr1->flags |= DYNFLG_CHANGED;
}
...
static void gen_shift_word_imm(Bitu op, bool dword, DynReg *dr1, uint8_t imm) {
  ...
  if (imm == 1)
    ...
    inst.setimm(imm, 1).Emit8(0xC1);
  dr1->flags |= DYNFLG_CHANGED;
}

static void gen_cbw(bool dword, DynReg *dyn_ax) {
  if (dword)
    gen_extend_word(true, dyn_ax, dyn_ax);
  else
    gen_extend_byte(true, false, dyn_ax, dyn_ax, 0);
}
...
static void gen_mul_word(bool imul, DynReg *dyn_ax, DynReg *dyn_dx, bool dword,
                         DynReg *dr1) {
  ForceDynReg(x64gen.regs[X64_REG_RAX], dyn_ax);
  if (dword && dyn_dx != dr1) {
...
static void gen_dshift_imm(bool dword, bool left, DynReg *dr1, DynReg *dr2,
                           Bitu imm) {
  // shld/shrd imm
  opcode(FindDynReg(dr2)->index, dword)
      .setimm(imm, 1)
      .setrm(FindDynReg(dr1)->index)
      .Emit16(left ? 0xA40F : 0xAC0F);
  dr1->flags |= DYNFLG_CHANGED;
}

static void gen_dshift_cl(bool dword, bool left, DynReg *dr1, DynReg *dr2,
                          DynReg *drecx) {
  ForceDynReg(x64gen.regs[X64_REG_RCX], drecx);
  // shld/shrd cl
  opcode(FindDynReg(dr2)->index, dword)
  ...
  while (*ops) {
    if (*ops++ == '%') {
      ...
      case 'I': /* immediate value */
        ...
        gen->Clear();
        if (*ops++ != 'p')
          gen_load_imm(gen->index, va_arg(params, uint32_t));
        else
          gen_load_imm(gen->index, va_arg(params, Bitu));
        break;
...
static void gen_call_write(DynReg *dr, uint32_t val, Bitu write_size) {
  ...
  switch (write_size) {
  case 1:
    func = (void *)(use_dynamic_core_with_paging ? mem_writeb_checked_pagefault
                                                 : mem_writeb_checked);
    break;
  case 2:
    func = (void *)(use_dynamic_core_with_paging ? mem_writew_checked_pagefault
                                                 : mem_writew_checked);
    break;
  case 4:
    func = (void *)(use_dynamic_core_with_paging ? mem_writed_checked_pagefault
                                                 : mem_writed_checked);
    break;
  default:
    IllegalOption("gen_call_write");
  }
...
static void gen_jmp_ptr(void *_ptr, int32_t imm = 0) {
  ...
  if ((uint32_t)ptr == ptr) {
    ...
    opcode(0).set64().setimm(ptr, 8).Emit8Reg(0xA1);
  opcode(4).setea(0, -1, 0, imm).Emit8(0xFF); // jmp [rax+imm]
}

static void gen_save_flags(DynReg *dynreg) {
  if (GCC_UNLIKELY(x64gen.flagsactive))
    IllegalOption("gen_save_flags");
  opcode(FindDynReg(dynreg)->index)
      .setea(4, -1, 0, CALLSTACK)
      .Emit8(0x8B); // mov reg32, [rsp+8/40]
  dynreg->flags |= DYNFLG_CHANGED;
}
...
static void gen_load_flags(DynReg *dynreg) {
  ...
  opcode(FindDynReg(dynreg)->index)
      .setea(4, -1, 0, CALLSTACK)
      .Emit8(0x89); // mov [rsp+8/40],reg32
}

static void gen_save_host_direct(void *data, Bitu imm) {
  if ((int32_t)imm != (Bits)imm) {
    opcode(0).setimm(imm, 4).setabsaddr(data).Emit8(
        0xC7); // mov dword[], imm32 (low dword)
    opcode(0)
        .setimm(imm >> 32, 4)
        .setabsaddr((uint8_t *)data + 4)
        .Emit8(0xC7); // high dword
  } else
    opcode(0).set64().setimm(imm, 4).setabsaddr(data).Emit8(
        0xC7); // mov qword[], int32_t
}
...
static void gen_return(BlockReturnDynX86 retcode) {
  ...
  opcode(0).set64().setrm(4).setimm(CALLSTACK + 8, 1).Emit8(0x83); // add
                                                                   // rsp,16/48
  if (retcode == 0)
    cache_addw(0xc033); // xor eax,eax
  else {
...
static void gen_return_fast(BlockReturnDynX86 retcode,
                            bool ret_exception = false) {
  if (GCC_UNLIKELY(x64gen.flagsactive))
...
// DO NOT USE opcode::setabsaddr IN THIS FUNCTION (RBP unavailable at execution
// time)
static void gen_dh_fpu_saveInit(void) {
  uint8_t *oldpos = cache.pos;
  ...
  if ((uint32_t)addr == addr)
    opcode(0).setimm(addr, 4).Emit8Reg(0xB8);
  else
    opcode(0).set64().setimm(addr, 8).Emit8Reg(0xB8);
  ...
  opcode(0)
      .setimm(0, 1)
      .setea(0, -1, 0, offsetof(dyn_dh_fpu_struct, state_used))
      .Emit8(0xc6);
  // or byte [dyn_dh_fpu.state.cw], 0x3F
  opcode(1)
      .setimm(0x3F, 1)
      .setea(0, -1, 0, offsetof(dyn_dh_fpu_struct, state.cw))
      .Emit8(0x80);
  cache_addb(0xC3); // RET
