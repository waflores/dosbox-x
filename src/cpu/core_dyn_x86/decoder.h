...
enum REP_Type { REP_NONE = 0, REP_NZ, REP_Z };
...
static bool MakeCodePage(LinearPt lin_addr, CodePageHandler *&cph) {
  ...
  if (!cache.free_pages) {
    if (cache.used_pages != decode.page.code)
      ...
      if ((cache.used_pages->next) &&
          (cache.used_pages->next != decode.page.code))
  ...
  if (cache.last_page)
    cache.last_page->next = cpagehandler;
  cache.last_page = cpagehandler;
  if (!cache.used_pages)
    cache.used_pages = cpagehandler;
...
static INLINE void decode_increase_wmapmask(Bitu size) {
  ...
  switch (size) {
  case 1:
    activecb->cache.wmapmask[mapidx] += 0x01;
    break;
  case 2:
    (*(uint16_t *)&activecb->cache.wmapmask[mapidx]) += 0x0101;
    break;
  case 4:
    (*(uint32_t *)&activecb->cache.wmapmask[mapidx]) += 0x01010101;
    break;
  }
}
...
static void dyn_set_eip_last_end(DynReg *endreg) {
  ...
  gen_dop_word_imm(DOP_ADD, decode.big_op, DREG(EIP),
                   decode.op_start - decode.code_start);
  decode.eip_location =
      decode.op_start; // this is the only place where a pagefault can happen
                       // after changing eip
}

static INLINE void dyn_set_eip_end(void) {
  gen_protectflags();
  gen_dop_word_imm(DOP_ADD, cpu.code.big, DREG(EIP),
...
static INLINE void dyn_set_eip_end(DynReg *endreg) {
  ...
  gen_protectflags();
  if (cpu.code.big)
    gen_dop_word(DOP_MOV, true, DREG(TMPW), DREG(EIP));
  else
    gen_extend_word(false, DREG(TMPW), DREG(EIP));
  gen_dop_word_imm(DOP_ADD, cpu.code.big, DREG(TMPW),
                   decode.code - decode.code_start);
}
...
static BlockReturnDynX86 DynRunException(uint32_t eip_add, uint32_t cycle_sub,
                                         uint32_t dflags) {
  reg_flags = (dflags & FMASK_TEST) | (reg_flags & (~FMASK_TEST));
  reg_eip += eip_add;
  CPU_Cycles -= cycle_sub;
  if (cpu.exception.which == SMC_CURRENT_BLOCK)
    return BR_SMCBlock;
...
static BlockReturnDynX86 DynRunPageFault(uint32_t eip_add, uint32_t cycle_sub,
                                         uint32_t pf_restore, uint32_t dflags) {
...
static void DynDebugPageFault(uint32_t eip_add, const char *debug_message) {
  LOG_MSG("Page fault during dynamic. EIP=%08X caller: %s", reg_eip + eip_add,
          debug_message);
}
#endif
...
Bitu call_function_pagefault_safe_0() {
  DYN_PAGEFAULT_CHECK({ return ((Bitu (*)())core_dyn.call_func)(); });
}

Bitu call_function_pagefault_safe_1(Bitu arg1) {
  DYN_PAGEFAULT_CHECK({ return ((Bitu (*)(Bitu))core_dyn.call_func)(arg1); });
}
...
Bitu call_function_pagefault_safe_4(Bitu arg1, Bitu arg2, Bitu arg3,
                                    Bitu arg4) {
  DYN_PAGEFAULT_CHECK({
    return ((Bitu (*)(Bitu, Bitu, Bitu, Bitu))core_dyn.call_func)(arg1, arg2,
                                                                  arg3, arg4);
  });
}
...
static void *get_wrapped_call_function(const char *ops) {
  ...
  switch (num_args) {
  case 0:
    return (void *)&call_function_pagefault_safe_0;
  case 1:
    return (void *)&call_function_pagefault_safe_1;
  case 2:
    return (void *)&call_function_pagefault_safe_2;
  case 3:
    return (void *)&call_function_pagefault_safe_3;
  case 4:
    return (void *)&call_function_pagefault_safe_4;
  default:
    IllegalOption(
        "dyn_call_function_pagefault_check unsupported number of arguments");
    return NULL;
  }
}
...
static void dyn_fill_blocks(void) {
  for (Bitu sct = 0; sct < used_save_info; sct++) {
    ...
    case db_exception:
      ...
      if (cpu.code.big)
        gen_call_function((void *)&DynRunException, "%Id%Id%F",
                          save_info[sct].eip_change, save_info[sct].cycles);
      else
        gen_call_function((void *)&DynRunException, "%Iw%Id%F",
                          save_info[sct].eip_change, save_info[sct].cycles);
    ...
    case page_fault:
      ...
      if (save_info[sct].pf_restore.data.dh_fpu_inst)
        dh_fpu_mem_revert(save_info[sct].pf_restore.data.dh_fpu_inst,
                          save_info[sct].pf_restore.data.dh_fpu_group);
#endif
#ifdef DYN_DEBUG_PAGEFAULT
      gen_call_function((void *)&DynDebugPageFault, "%Id%Ip",
                        save_info[sct].eip_change,
                        save_info[sct].debug_message);
#endif
      if (cpu.code.big)
        gen_call_function((void *)&DynRunPageFault, "%Id%Id%Id%F",
                          save_info[sct].eip_change, save_info[sct].cycles,
                          save_info[sct].pf_restore.dword);
      else
        gen_call_function((void *)&DynRunPageFault, "%Iw%Id%Id%F",
                          save_info[sct].eip_change, save_info[sct].cycles,
                          save_info[sct].pf_restore.dword);
      gen_return_fast(BR_Normal, true);
      break;
...
#else // X86_64
      opcode(7)
          .setabsaddr(&dyn_dh_fpu.host_cw)
          .Emit8(0xD9); // FNSTCW [&fpu.host_cw]
      opcode(4)
          .setabsaddr(&dyn_dh_fpu.state)
          .Emit8(0xDD); // FRSTOR [&fpu.state]
      opcode(0)
          .setimm(1, 1)
          .setabsaddr(&dyn_dh_fpu.state_used)
          .Emit8(0xC6); // mov byte[], imm8
#endif
...
static void dyn_read_byte(DynReg *addr, DynReg *dst, bool high) {
  gen_protectflags();
  dyn_call_function_pagefault_check((void *)&mem_readb_checked, "%Dd%Ip", addr,
                                    &core_dyn.readdata);
...
static void dyn_write_byte(DynReg *addr, DynReg *val, bool high) {
  ...
  if (high)
    dyn_call_function_pagefault_check(
        (void *)&mem_writeb_checked, "%Dd%Dh", addr,
        val) else dyn_call_function_pagefault_check((void *)&mem_writeb_checked,
                                                    "%Dd%Dd", addr, val)
        dyn_check_bool_exception_al();
}
static void dyn_read_word(DynReg *addr, DynReg *dst, bool dword) {
  ...
  if (dword)
    dyn_call_function_pagefault_check(
        (void *)&mem_readd_checked, "%Dd%Ip", addr,
        &core_dyn
             .readdata) else dyn_call_function_pagefault_check((void
                                                                    *)&mem_readw_checked,
                                                               "%Dd%Ip", addr,
                                                               &core_dyn
                                                                    .readdata)
...
static void dyn_write_word(DynReg *addr, DynReg *val, bool dword) {
  ...
  if (dword)
    dyn_call_function_pagefault_check(
        (void *)&mem_writed_checked_pf, "%Dd%Dd", addr,
        val) else dyn_call_function_pagefault_check((void
                                                         *)&mem_writew_checked_pf,
                                                    "%Dd%Dd", addr, val)
        dyn_check_bool_exception_al();
}
static void dyn_read_byte_release(DynReg *addr, DynReg *dst, bool high) {
  gen_protectflags();
  dyn_call_function_pagefault_check((void *)&mem_readb_checked, "%Drd%Ip", addr,
                                    &core_dyn.readdata);
  dyn_check_bool_exception_al();
  gen_mov_host(&core_dyn.readdata, dst, 1, high ? 4 : 0);
}
static void dyn_write_byte_release(DynReg *addr, DynReg *val, bool high) {
  ...
  if (high)
    dyn_call_function_pagefault_check(
        (void *)&mem_writeb_checked, "%Drd%Dh", addr,
        val) else dyn_call_function_pagefault_check((void *)&mem_writeb_checked,
                                                    "%Drd%Dd", addr, val)
        dyn_check_bool_exception_al();
}
static void dyn_read_word_release(DynReg *addr, DynReg *dst, bool dword) {
  ...
  if (dword)
    dyn_call_function_pagefault_check(
        (void *)&mem_readd_checked, "%Drd%Ip", addr,
        &core_dyn
             .readdata) else dyn_call_function_pagefault_check((void
                                                                    *)&mem_readw_checked,
                                                               "%Drd%Ip", addr,
...
static void dyn_write_word_release(DynReg *addr, DynReg *val, bool dword) {
  ...
  if (dword)
    dyn_call_function_pagefault_check(
        (void *)&mem_writed_checked, "%Drd%Dd", addr,
        val) else dyn_call_function_pagefault_check((void *)&mem_writew_checked,
                                                    "%Drd%Dd", addr, val)
        dyn_check_bool_exception_al();
}
...
static bool mem_readb_checked_dcx86(PhysPt address) {
  return get_tlb_readhandler(address)->readb_checked(
      address, (uint8_t *)(&core_dyn.readdata));
}

static bool mem_readb_checked_dcx86_pagefault(PhysPt address) {
  DYN_PAGEFAULT_CHECK({ return mem_readb_checked_dcx86(address); });
}
...
static void dyn_read_byte(DynReg *addr, DynReg *dst, bool high,
  ...
  cache_addb(0xe8);
  cache_addd((uintptr_t)(&(use_dynamic_core_with_paging
                               ? mem_readb_checked_dcx86_pagefault
                               : mem_readb_checked_dcx86)) -
             (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
...
static bool mem_readd_checked_dcx86(PhysPt address) {
  if ((address & 0xfff) < 0xffd) {
    ...
    if (tlb_addr) {
      ...
      return get_tlb_readhandler(address)->readd_checked(
          address, (uint32_t *)&core_dyn.readdata);
    }
  } else
    return mem_unalignedreadd_checked(address, (uint32_t *)&core_dyn.readdata);
}

static bool mem_readd_checked_dcx86_pagefault(PhysPt address) {
  DYN_PAGEFAULT_CHECK({ return mem_readd_checked_dcx86(address); });
}
  ...
  if ((address & 0xfff) < 0xfff) {
    ...
    if (tlb_addr) {
      *(uint16_t *)&core_dyn.readdata = host_readw(tlb_addr + address);
      ...
      return get_tlb_readhandler(address)->readw_checked(
          address, (uint16_t *)&core_dyn.readdata);
    }
  } else
    return mem_unalignedreadw_checked(address, (uint16_t *)&core_dyn.readdata);
}

static bool mem_readw_checked_dcx86_pagefault(PhysPt address) {
  DYN_PAGEFAULT_CHECK({ return mem_readw_checked_dcx86(address); });
}
...
static void dyn_read_word(DynReg *addr, DynReg *dst, bool dword,
  ...
  if (dword)
    cache_addd((uintptr_t)(&(use_dynamic_core_with_paging
                                 ? mem_readd_checked_dcx86_pagefault
                                 : mem_readd_checked_dcx86)) -
               (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
  else
    cache_addd((uintptr_t)(&(use_dynamic_core_with_paging
                                 ? mem_readw_checked_dcx86_pagefault
                                 : mem_readw_checked_dcx86)) -
               (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
  cache_addw(0xc483); // add esp,4
  cache_addb(0x04);
...
static void dyn_write_byte(DynReg *addr, DynReg *val, bool high,
  ...
  cache_addw(0x0488 + (genreg->index << 11) +
             (high ? 0x2000 : 0)); // mov [eax+ecx],reg
  cache_addb(0x08);
  ...
  cache_addb(0xe8);
  cache_addd(
      (uintptr_t)(&(use_dynamic_core_with_paging ? mem_writeb_checked_pagefault
                                                 : mem_writeb_checked)) -
      (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
...
static void dyn_write_word(DynReg *addr, DynReg *val, bool dword,
  ...
  if (dword)
    cache_addd((uintptr_t)(&(use_dynamic_core_with_paging
                                 ? mem_writed_checked_pagefault
                                 : mem_writed_checked)) -
               (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
  else
    cache_addd((uintptr_t)(&(use_dynamic_core_with_paging
                                 ? mem_writew_checked_pagefault
                                 : mem_writew_checked)) -
               (uintptr_t)cache_rwtox(cache.pos) - (uintptr_t)4);
  cache_addw(0xc483); // add esp,8
  cache_addb(0x08);
...
#else // X86_64
bool mem_unalignedreadw_checked_pagefault(const PhysPt address,
                                          uint16_t *const val) {
  DYN_PAGEFAULT_CHECK({ return mem_unalignedreadw_checked(address, val); });
}
bool mem_unalignedreadd_checked_pagefault(const PhysPt address,
                                          uint32_t *const val) {
  DYN_PAGEFAULT_CHECK({ return mem_unalignedreadd_checked(address, val); });
}
bool mem_unalignedwritew_checked_pagefault(const PhysPt address,
                                           uint16_t const val) {
  DYN_PAGEFAULT_CHECK({ return mem_unalignedwritew_checked(address, val); });
}
bool mem_unalignedwrited_checked_pagefault(const PhysPt address,
                                           uint32_t const val) {
  DYN_PAGEFAULT_CHECK({ return mem_unalignedwrited_checked(address, val); });
}
...
static bool mem_writed_checked_dcx64_pagefault(PhysPt address, Bitu val) {
  DYN_PAGEFAULT_CHECK({
    return get_tlb_writehandler(address)->writed_checked(address,
                                                         (uint32_t)val);
  });
}
static bool mem_writew_checked_dcx64_pagefault(PhysPt address, Bitu val) {
  DYN_PAGEFAULT_CHECK({
    return get_tlb_writehandler(address)->writew_checked(address,
                                                         (uint16_t)val);
  });
}
...
static void dyn_read_word(DynReg *addr, DynReg *dst, bool dword,
  ...
  opcode(tmp)
      .set64()
      .setea(gensrc->index, -1, 0, dword ? 3 : 1)
      .Emit8(0x8D); // lea tmp, [dst+(dword?3:1)]
  if (dword) {
    opcode(4).set64().setimm(~0xFFF, 4).setrm(tmp).Emit8(
        0x81);                                            // and tmp, ~0xFFF
  ...
  gen_load_imm(
      tmp, (Bitu)(dword ? (void *)(use_dynamic_core_with_paging
                                       ? mem_unalignedreadd_checked_pagefault
                                       : mem_unalignedreadd_checked)
                        : (void *)(use_dynamic_core_with_paging
                                       ? mem_unalignedreadw_checked_pagefault
                                       : mem_unalignedreadw_checked)));
  uint8_t *page_jmp = gen_create_short_jump();

  gen_fill_branch(nomap);
  gen_load_imm(tmp,
               (Bitu)(dword ? (void *)(use_dynamic_core_with_paging
                                           ? mem_readd_checked_dcx64_pagefault
                                           : mem_readd_checked_dcx64)
                            : (void *)(use_dynamic_core_with_paging
                                           ? mem_readw_checked_dcx64_pagefault
                                           : mem_readw_checked_dcx64)));
...
static void dyn_read_byte(DynReg *addr, DynReg *dst, bool high,
  ...
  if (high && gendst->index > 3)
    IllegalOption("dyn_read_byte");
  ...
  gendst->Clear();
  gen_call_ptr((void *)(use_dynamic_core_with_paging
                            ? mem_readb_checked_dcx64_pagefault
                            : mem_readb_checked_dcx64));
  if (use_dynamic_core_with_paging)
    dyn_check_pagefault();
  dyn_check_bool_exception_al();
...
static void dyn_write_word(DynReg *addr, DynReg *val, bool dword,
  ...
  opcode(tmp)
      .set64()
      .setea(gendst->index, -1, 0, dword ? 3 : 1)
      .Emit8(0x8D); // lea tmp, [dst+(dword?3:1)]
  if (dword) {
    opcode(4).set64().setimm(~0xFFF, 4).setrm(tmp).Emit8(
        0x81);                                            // and tmp, ~0xFFF
  ...
  gen_load_imm(
      tmp, (Bitu)(dword ? (void *)(use_dynamic_core_with_paging
                                       ? mem_unalignedwrited_checked_pagefault
                                       : mem_unalignedwrited_checked)
                        : (void *)(use_dynamic_core_with_paging
                                       ? mem_unalignedwritew_checked_pagefault
                                       : mem_unalignedwritew_checked)));
  uint8_t *page_jmp = gen_create_short_jump();
  gen_fill_branch(nomap);
  gen_load_imm(tmp,
               (Bitu)(dword ? (void *)(use_dynamic_core_with_paging
                                           ? mem_writed_checked_dcx64_pagefault
                                           : mem_writed_checked_dcx64)
                            : (void *)(use_dynamic_core_with_paging
                                           ? mem_writew_checked_dcx64_pagefault
                                           : mem_writew_checked_dcx64)));
  gen_fill_short_jump(page_jmp);
...
static void dyn_write_byte(DynReg *addr, DynReg *val, bool high,
  ...
  gen_call_ptr((void *)(use_dynamic_core_with_paging
                            ? mem_writeb_checked_dcx64_pagefault
                            : mem_writeb_checked_dcx64));
  if (use_dynamic_core_with_paging)
    dyn_check_pagefault();
  dyn_check_bool_exception_al();
...
static void dyn_push_unchecked(DynReg *dynreg) {
  ...
  if (decode.big_op) {
    dyn_call_function_pagefault_check((void *)&mem_writed, "%Drd%Dd",
                                      DREG(STACK), dynreg);
  } else {
    // Can just push the whole 32-bit word as operand
    dyn_call_function_pagefault_check((void *)&mem_writew, "%Drd%Dd",
                                      DREG(STACK), dynreg);
  }
}
...
static void dyn_push(DynReg *dynreg) {
  ...
  gen_dop_word(DOP_ADD, true, DREG(STACK), DREG(SS));
  if (decode.big_op) {
    dyn_call_function_pagefault_check((void *)&mem_writed_checked, "%Drd%Dd",
                                      DREG(STACK), dynreg);
  } else {
    // Can just push the whole 32-bit word as operand
    dyn_call_function_pagefault_check((void *)&mem_writew_checked, "%Drd%Dd",
                                      DREG(STACK), dynreg);
  }
  dyn_check_bool_exception_al();
...
static void dyn_pop(DynReg *dynreg, bool checked = true) {
  ...
  if (checked) {
    if (decode.big_op) {
      dyn_call_function_pagefault_check((void *)&mem_readd_checked, "%Drd%Ip",
                                        DREG(STACK), &core_dyn.readdata);
    } else {
      dyn_call_function_pagefault_check((void *)&mem_readw_checked, "%Drd%Ip",
                                        DREG(STACK), &core_dyn.readdata);
    }
    dyn_check_bool_exception_al();
    ...
    if (decode.big_op) {
      dyn_call_function_pagefault_check((void *)&mem_readd, "%Rd%Drd", dynreg,
                                        DREG(STACK));
    } else {
      dyn_call_function_pagefault_check((void *)&mem_readw, "%Rw%Drd", dynreg,
                                        DREG(STACK));
    }
  }
  if (dynreg != DREG(ESP)) {
...
static void dyn_fill_ea(bool addseg = true, DynReg *reg_ea = DREG(EA)) {
  ...
  if (!decode.big_addr) {
    ...
    case 4: /* SI */
      if (imm)
        gen_lea(reg_ea, DREG(ESI), nullptr, 0, imm);
      else
        extend_src = DREG(ESI);
      segbase = DREG(DS);
      break;
    case 5: /* DI */
      if (imm)
        gen_lea(reg_ea, DREG(EDI), nullptr, 0, imm);
      else
        extend_src = DREG(EDI);
      segbase = DREG(DS);
      break;
    ...
    case 7: /* BX */
      if (imm)
        gen_lea(reg_ea, DREG(EBX), nullptr, 0, imm);
      else
        extend_src = DREG(EBX);
      segbase = DREG(DS);
      break;
  ...
  skip_extend_word:
    if (addseg) {
      gen_lea(reg_ea, reg_ea, decode.segprefix ? decode.segprefix : segbase, 0,
              0);
    }
  } else {
    Bits imm = 0;
    DynReg *base = nullptr;
    DynReg *scaled = nullptr;
    Bitu scale = 0;
    switch (decode.modrm.rm) {
    case 0:
      base = DREG(EAX);
      ...
      if (decode_fetchd_imm(val)) {
        ...
        if (!addseg) {
          ...
          if (!base) {
            ...
            gen_lea(reg_ea, DREG(EA),
                    decode.segprefix ? decode.segprefix : segbase, 0, 0);
          }
        }
        if (reg_ea != DREG(EA))
          gen_releasereg(DREG(EA));
    ...
    if (!addseg) {
      ...
      DynReg **seg = decode.segprefix ? &decode.segprefix : &segbase;
      if (!base)
        gen_lea(reg_ea, *seg, scaled, scale, imm);
      else if (!scaled)
        gen_lea(reg_ea, base, *seg, 0, imm);
      else {
        gen_lea(DREG(EA), base, scaled, scale, imm);
        gen_lea(reg_ea, DREG(EA), decode.segprefix ? decode.segprefix : segbase,
                0, 0);
        if (reg_ea != DREG(EA))
          gen_releasereg(DREG(EA));
      }
...
static void dyn_dop_ebgb(DualOps op) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea();
    if ((op <= DOP_TEST) && (op != DOP_ADC && op != DOP_SBB))
      set_skipflags(true);
    dyn_read_byte(DREG(EA), DREG(TMPB), false);
    if (op <= DOP_TEST) {
      if (op == DOP_ADC || op == DOP_SBB)
        gen_needcarry();
      else
        set_skipflags(false);
    }
    ...
    gen_dop_byte(op, &DynRegs[decode.modrm.rm & 3], decode.modrm.rm & 4, rm_reg,
                 decode.modrm.reg & 4);
  }
}
...
static void dyn_dop_ebgb_xchg(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    gen_dop_byte(DOP_XCHG, &DynRegs[decode.modrm.rm & 3], decode.modrm.rm & 4,
                 rm_reg, decode.modrm.reg & 4);
}
...
static void dyn_dop_gbeb(DualOps op) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    gen_dop_byte(op, rm_reg, decode.modrm.reg & 4,
...
static void dyn_mov_ebib(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    gen_dop_byte_imm(DOP_MOV, &DynRegs[decode.modrm.rm & 3],
                     decode.modrm.rm & 4, decode_fetchb());
  }
}
...
static void dyn_mov_ebgb(void) {
  dyn_get_modrm();
  DynReg *rm_reg = &DynRegs[decode.modrm.reg & 3];
  Bitu rm_regi = decode.modrm.reg & 4;
  if (decode.modrm.mod < 3) {
    dyn_fill_ea();
    dyn_write_byte_release(DREG(EA), rm_reg, rm_regi != 0);
  } else {
    gen_dop_byte(DOP_MOV, &DynRegs[decode.modrm.rm & 3], decode.modrm.rm & 4,
                 rm_reg, rm_regi);
  }
}
...
static void dyn_mov_gbeb(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    gen_dop_byte(DOP_MOV, rm_reg, rm_regi, &DynRegs[decode.modrm.rm & 3],
                 decode.modrm.rm & 4);
  }
}
...
static void dyn_dop_evgv_xchg(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    if (!decode.big_op)
      gen_dop_word(DOP_MOV, true, tmp, rm_reg);
    ...
    gen_dop_word(DOP_XCHG, decode.big_op, &DynRegs[decode.modrm.rm], rm_reg);
}
...
static void dyn_imul_gvev(Bitu immsize) {
  ...
  switch (immsize) {
  case 0:
    gen_imul_word(decode.big_op, rm_reg, src);
    break;
  case 1:
    gen_imul_word_imm(decode.big_op, rm_reg, src, (int8_t)decode_fetchb());
    break;
  case 2:
    gen_imul_word_imm(decode.big_op, rm_reg, src, (int16_t)decode_fetchw());
    break;
  case 4:
    gen_imul_word_imm(decode.big_op, rm_reg, src, (int32_t)decode_fetchd());
    break;
  }
  gen_releasereg(DREG(TMPW));
}
...
static void dyn_mov_eviv(void) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea();
    gen_call_write(DREG(EA), decode.big_op ? decode_fetchd() : decode_fetchw(),
                   decode.big_op ? 4 : 2);
    if (use_dynamic_core_with_paging)
      dyn_check_pagefault();
    dyn_check_bool_exception_al();
  } else {
    gen_dop_word_imm(DOP_MOV, decode.big_op, &DynRegs[decode.modrm.rm],
                     decode.big_op ? decode_fetchd() : decode_fetchw());
  }
}
...
static void dyn_mov_ev_gb(bool sign) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    gen_extend_byte(sign, decode.big_op, rm_reg, &DynRegs[decode.modrm.rm & 3],
                    decode.modrm.rm & 4);
  }
}
...
static void dyn_dshift_ev_gv(bool left, bool immediate) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    ea_reg = &DynRegs[decode.modrm.rm];
  gen_needflags();
  if (immediate)
    gen_dshift_imm(decode.big_op, left, ea_reg, rm_reg, decode_fetchb());
  else
    gen_dshift_cl(decode.big_op, left, ea_reg, rm_reg, DREG(ECX));
  if (decode.modrm.mod < 3) {
    dyn_write_word_release(DREG(EA), DREG(TMPW), decode.big_op);
    gen_releasereg(DREG(TMPW));
  }
...
static DualOps grp1_table[8] = {DOP_ADD, DOP_OR,  DOP_ADC, DOP_SBB,
                                DOP_AND, DOP_SUB, DOP_XOR, DOP_CMP};
...
static void dyn_grp1_ev_ivx(bool withbyte) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    if (!withbyte) {
      ...
      gen_dop_word_imm(op, decode.big_op, &DynRegs[decode.modrm.rm],
                       (int8_t)decode_fetchb());
    }
  }
}
...
static void dyn_grp2_eb(grp2_types type) {
  ...
  switch (type) {
  case grp2_1:
    /* rotates (first 4 ops) alter cf/of only; shifts (last 4 ops) alter all
     * flags */
    if (decode.modrm.reg < 4)
      gen_needflags();
    else
      gen_discardflags();
...
static void dyn_grp3_eb(void) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea();
    if ((decode.modrm.reg == 0) || (decode.modrm.reg == 3))
      set_skipflags(true);
    dyn_read_byte(DREG(EA), DREG(TMPB), false);
    src = DREG(TMPB);
    src_i = 0;
  ...
  case 0x7: /* idiv Eb */
    ...
    if (decode.modrm.mod == 3)
      gen_dop_byte(DOP_MOV, DREG(TMPB), 0, &DynRegs[decode.modrm.rm & 3],
                   decode.modrm.rm & 4);
    gen_releasereg(DREG(EAX));
    gen_call_function((decode.modrm.reg == 6) ? (void *)&dyn_helper_divb
                                              : (void *)&dyn_helper_idivb,
...
static void dyn_grp3_ev(void) {
  ...
  case 0x0: /* test ev,iv */
    set_skipflags(false);
    gen_dop_word_imm(DOP_TEST, decode.big_op, src,
                     decode.big_op ? decode_fetchd() : decode_fetchw());
    goto skipsave;
  case 0x2: /* NOT Ev */
  ...
  case 0x7: /* idiv Eb */
    /* EAX could be used, so precache it */
    if (decode.modrm.mod == 3)
      gen_dop_word(DOP_MOV, decode.big_op, DREG(TMPW),
                   &DynRegs[decode.modrm.rm]);
    gen_releasereg(DREG(EAX));
    gen_releasereg(DREG(EDX));
    void *func = (decode.modrm.reg == 6)
                     ? (decode.big_op ? (void *)&dyn_helper_divd
                                      : (void *)&dyn_helper_divw)
                     : (decode.big_op ? (void *)&dyn_helper_idivd
                                      : (void *)&dyn_helper_idivw);
...
static void dyn_load_seg(SegNames seg, DynReg *src) {
  dyn_call_function_pagefault_check((void *)&CPU_SetSegGeneral, "%Rd%Id%Drw",
                                    DREG(TMPB), seg, src);
  dyn_check_bool_exception(DREG(TMPB));
  gen_releasereg(DREG(TMPB));
...
static void dyn_load_seg_off_ea(SegNames seg) {
  if (decode.modrm.mod < 3) {
    ...
    dyn_read_word_release(DREG(EA), DREG(TMPW), decode.big_op);
    dyn_load_seg(seg, DREG(TMPB));
    gen_releasereg(DREG(TMPB));
    gen_dop_word(DOP_MOV, decode.big_op, &DynRegs[decode.modrm.reg],
                 DREG(TMPW));
    gen_releasereg(DREG(TMPW));
  } else {
...
static void dyn_pop_seg(SegNames seg) {
  gen_releasereg(DREG(ESP));
  dyn_call_function_pagefault_check((void *)&CPU_PopSeg, "%Rd%Id%Id",
                                    DREG(TMPB), seg, decode.big_op);
  dyn_check_bool_exception(DREG(TMPB));
  gen_releasereg(DREG(TMPB));
  gen_releasereg(&DynRegs[G_ES + seg]);
  gen_releasereg(DREG(ESP));
...
static void dyn_pop_ev(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    if (decode.big_op)
      dyn_call_function_pagefault_check(
          (void *)&mem_writed_inline, "%Drd%Dd", DREG(EA),
          DREG(
              TMPW)) else dyn_call_function_pagefault_check((void
                                                                 *)&mem_writew_inline,
                                                            "%Drd%Dd", DREG(EA),
                                                            DREG(TMPW))
  } else {
...
static void dyn_enter(void) {
  ...
  dyn_call_function_pagefault_check((void *)&CPU_ENTER, "%Id%Id%Id",
                                    decode.big_op, bytes, level);
}
...
static void dyn_segprefix(SegNames seg) {
  //	if (GCC_UNLIKELY((Bitu)(decode.segprefix)))
  //IllegalOption("dyn_segprefix");
...
static void dyn_exit_link(Bits eip_change) {
  gen_protectflags();
  gen_dop_word_imm(DOP_ADD, decode.big_op, DREG(EIP),
                   (decode.code - decode.code_start) + eip_change);
  dyn_reduce_cycles();
  dyn_save_critical_regs();
...
enum LoopTypes { LOOP_NONE, LOOP_NE, LOOP_E, LOOP_JCXZ };

static void dyn_loop(LoopTypes type) {
  Bits eip_add = (int8_t)decode_fetchb();
  uint8_t *branch1 = nullptr;
  uint8_t *branch2 = nullptr;
...
static void dyn_ret_near(Bitu bytes) {
  ...
  dyn_pop(DREG(EIP));
  if (bytes)
    gen_dop_word_imm(DOP_ADD, true, DREG(ESP), bytes);
  dyn_save_critical_regs();
...
static void dyn_call_near_imm(void) {
  ...
  if (cpu.code.big)
    gen_dop_word(DOP_MOV, true, DREG(EIP), DREG(TMPW));
  else
    gen_extend_word(false, DREG(EIP), DREG(TMPW));
  dyn_reduce_cycles();
  dyn_save_critical_regs();
  gen_jmp_ptr(&decode.block->link[0].to, offsetof(CacheBlock, cache.xstart));
  dyn_closeblock();
...
static void dyn_ret_far(Bitu bytes) {
  ...
  dyn_save_critical_regs();
  dyn_call_function_pagefault_check((void *)&CPU_RET, "%Id%Id%Drd",
                                    decode.big_op, bytes, DREG(TMPW));
...
static void dyn_call_far_imm(void) {
  ...
  dyn_flags_gen_to_host();
  dyn_save_critical_regs();
  dyn_call_function_pagefault_check((void *)&CPU_CALL, "%Id%Id%Id%Drd",
                                    decode.big_op, sel, off, DREG(TMPW));
  gen_return_fast(BR_Normal);
  dyn_closeblock();
}
...
static void dyn_jmp_far_imm(void) {
  ...
  dyn_flags_gen_to_host();
  dyn_save_critical_regs();
  dyn_call_function_pagefault_check((void *)&CPU_JMP, "%Id%Id%Id%Drd",
                                    decode.big_op, sel, off, DREG(TMPW));
  gen_return_fast(BR_Normal);
  dyn_closeblock();
}
...
static void dyn_iret(void) {
  ...
  dyn_set_eip_last_end(DREG(TMPW));
  dyn_save_critical_regs();
  dyn_call_function_pagefault_check((void *)&CPU_IRET, "%Id%Drd", decode.big_op,
                                    DREG(TMPW));
...
static void dyn_interrupt(Bitu num) {
  ...
  dyn_set_eip_last_end(DREG(TMPW));
  dyn_save_critical_regs();
  dyn_call_function_pagefault_check((void *)&CPU_Interrupt, "%Id%Id%Drd", num,
                                    CPU_INT_SOFTWARE, DREG(TMPW));
  gen_return_fast(BR_Normal);
  dyn_closeblock();
}
#endif
...
static bool dyn_io_readB(Bitu port) {
  bool ex = CPU_IO_Exception(port, 1);
  if (!ex)
    core_dyn.readdata = IO_ReadB(port);
  return ex;
}
...
static bool dyn_io_readW(Bitu port) {
  bool ex = CPU_IO_Exception(port, 2);
  if (!ex)
    core_dyn.readdata = IO_ReadW(port);
  return ex;
}
...
static bool dyn_io_readD(Bitu port) {
  bool ex = CPU_IO_Exception(port, 4);
  if (!ex)
    core_dyn.readdata = IO_ReadD(port);
  return ex;
}
...
static void dyn_xlat(void) {
  ...
  if (!decode.big_addr)
    gen_extend_word(false, DREG(TMPW), DREG(TMPW));
  gen_lea(DREG(TMPW), DREG(TMPW),
          decode.segprefix ? decode.segprefix : DREG(DS), 0, 0);
...
static void dyn_larlsl(bool islar) {
  ...
  if (!cpu.code.big) {
    ...
    gen_dop_word_imm(DOP_SUB, true, DREG(TMPW),
                     1); // tmpw = cpu.pmode ? 0:0xffffffff
  ...
  if (decode.modrm.mod < 3) {
    ...
    dyn_read_word_release(DREG(EA), DREG(TMPW), false);
    dyn_flags_gen_to_host();
    dyn_call_function_pagefault_check(func, "%Drw%Ip", DREG(TMPW),
                                      &core_dyn.readdata);
  } else {
    dyn_flags_gen_to_host();
    dyn_call_function_pagefault_check(func, "%Dw%Ip", &DynRegs[decode.modrm.rm],
                                      &core_dyn.readdata);
  }
  ...
  gen_mov_host(&core_dyn.readdata, &DynRegs[decode.modrm.reg],
               decode.big_op ? 4 : 2);
  gen_fill_branch(br);
}
...
static CacheBlock *CreateCacheBlock(CodePageHandler *codepage, PhysPt start,
                                    Bitu max_opcodes) {
  Bits i;
  ...
  restart_prefix:
    ...
    case 0x00:
      dyn_dop_ebgb(DOP_ADD);
      break;
    case 0x01:
      dyn_dop_evgv(DOP_ADD);
      break;
    case 0x02:
      dyn_dop_gbeb(DOP_ADD);
      break;
    case 0x03:
      dyn_dop_gvev(DOP_ADD);
      break;
    case 0x04:
      gen_discardflags();
    ...
    case 0x05:
      gen_discardflags();
      dyn_dop_word_imm(DOP_ADD, decode.big_op, DREG(EAX));
      break;
    case 0x06:
      dyn_push_seg(es);
      break;
    ...
    case 0x0c:
      gen_discardflags();
      gen_dop_byte_imm(DOP_OR, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x0d:
      gen_discardflags();
      gen_dop_word_imm(DOP_OR, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x0e:
      dyn_push_seg(cs);
    ...
    case 0x0f: {
      ...
      case 0x8f:
        dyn_branched_exit((BranchTypes)(dual_code & 0xf),
                          decode.big_op ? (int32_t)decode_fetchd()
                                        : (int16_t)decode_fetchw());
        goto finish_block;
      /* SETcc */
      case 0x90:
      case 0x91:
      ...
      case 0x9e:
      case 0x9f:
        dyn_set_cc((BranchTypes)(dual_code & 0xf));
        break;
      /* PUSH/POP FS */
      case 0xa0:
        dyn_push_seg(fs);
      ...
      /* SHLD Imm/cl*/
      case 0xa4:
        dyn_dshift_ev_gv(true, true);
        break;
      case 0xa5:
        dyn_dshift_ev_gv(true, false);
        break;
      /* PUSH/POP GS */
      case 0xa8:
        dyn_push_seg(gs);
      ...
      /* SHRD Imm/cl*/
      case 0xac:
        dyn_dshift_ev_gv(false, true);
        break;
      case 0xad:
        dyn_dshift_ev_gv(false, false);
        break;
      /* Imul Ev,Gv */
      case 0xaf:
        dyn_imul_gvev(0);
      ...
      case 0xfd:
      case 0xfe:
        if (CPU_ArchitectureType < CPU_ARCHTYPE_PMMXSLOW)
          goto illegalopcode;
        dyn_mmx_check();
    ...
    case 0x10:
      dyn_dop_ebgb(DOP_ADC);
      break;
    case 0x11:
      dyn_dop_evgv(DOP_ADC);
      break;
    case 0x12:
      dyn_dop_gbeb(DOP_ADC);
      break;
    case 0x13:
      dyn_dop_gvev(DOP_ADC);
      break;
    case 0x14:
      gen_needcarry();
      gen_dop_byte_imm(DOP_ADC, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x15:
      gen_needcarry();
      gen_dop_word_imm(DOP_ADC, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x16:
      dyn_push_seg(ss);
    ...
    case 0x18:
      dyn_dop_ebgb(DOP_SBB);
      break;
    case 0x19:
      dyn_dop_evgv(DOP_SBB);
      break;
    case 0x1a:
      dyn_dop_gbeb(DOP_SBB);
      break;
    case 0x1b:
      dyn_dop_gvev(DOP_SBB);
      break;
    case 0x1c:
      gen_needcarry();
      gen_dop_byte_imm(DOP_SBB, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x1d:
      gen_needcarry();
      gen_dop_word_imm(DOP_SBB, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x1e:
      dyn_push_seg(ds);
    ...
    case 0x1f:
      ...
      break;
    case 0x20:
      dyn_dop_ebgb(DOP_AND);
      break;
    case 0x21:
      dyn_dop_evgv(DOP_AND);
      break;
    case 0x22:
      dyn_dop_gbeb(DOP_AND);
      break;
    case 0x23:
      dyn_dop_gvev(DOP_AND);
      break;
    case 0x24:
      gen_discardflags();
      gen_dop_byte_imm(DOP_AND, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x25:
      gen_discardflags();
      dyn_dop_word_imm(DOP_AND, decode.big_op, DREG(EAX));
      break;
    case 0x26:
      dyn_segprefix(es);
      goto restart_prefix;
    ...
    case 0x28:
      dyn_dop_ebgb(DOP_SUB);
      break;
    case 0x29:
      dyn_dop_evgv(DOP_SUB);
      break;
    case 0x2a:
      dyn_dop_gbeb(DOP_SUB);
      break;
    case 0x2b:
      dyn_dop_gvev(DOP_SUB);
      break;
    case 0x2c:
      gen_discardflags();
      gen_dop_byte_imm(DOP_SUB, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x2d:
      gen_discardflags();
      gen_dop_word_imm(DOP_SUB, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x2e:
      dyn_segprefix(cs);
    ...
    case 0x30:
      dyn_dop_ebgb(DOP_XOR);
      break;
    case 0x31:
      dyn_dop_evgv(DOP_XOR);
      break;
    case 0x32:
      dyn_dop_gbeb(DOP_XOR);
      break;
    case 0x33:
      dyn_dop_gvev(DOP_XOR);
      break;
    case 0x34:
      gen_discardflags();
      gen_dop_byte_imm(DOP_XOR, DREG(EAX), 0, decode_fetchb());
      break;
    case 0x35:
      gen_discardflags();
      gen_dop_word_imm(DOP_XOR, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x36:
      dyn_segprefix(ss);
    ...
    case 0x38:
      dyn_dop_ebgb(DOP_CMP);
      break;
    case 0x39:
      dyn_dop_evgv(DOP_CMP);
      break;
    case 0x3a:
      dyn_dop_gbeb(DOP_CMP);
      break;
    case 0x3b:
      dyn_dop_gvev(DOP_CMP);
      break;
    case 0x3c:
      gen_discardflags();
    ...
    case 0x3d:
      gen_discardflags();
      gen_dop_word_imm(DOP_CMP, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    case 0x3e:
      dyn_segprefix(ds);
    ...
    case 0x67:
      ...
      goto restart_prefix;
    case 0x68: /* PUSH Iv */
      gen_dop_word_imm(DOP_MOV, decode.big_op, DREG(TMPW),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      dyn_push(DREG(TMPW));
      gen_releasereg(DREG(TMPW));
      break;
    /* Imul Ivx */
    case 0x69:
      dyn_imul_gvev(decode.big_op ? 4 : 2);
      break;
    case 0x6a: /* PUSH Ibx */
    ...
    case 0x80:
      ...
      break;
    case 0x81:
      dyn_grp1_ev_ivx(false);
      break;
    case 0x82:
      dyn_grp1_eb_ib();
      break;
    case 0x83:
      dyn_grp1_ev_ivx(true);
      break;
    /* TEST Gb,Eb Gv,Ev */
    case 0x84:
      dyn_dop_gbeb(DOP_TEST);
      break;
    case 0x85:
      dyn_dop_gvev(DOP_TEST);
      break;
    /* XCHG Eb,Gb Ev,Gv */
    case 0x86:
      dyn_dop_ebgb_xchg();
    ...
    case 0x8d:
      dyn_get_modrm();
      if (GCC_UNLIKELY(decode.modrm.mod == 3))
        goto illegalopcode; // Direct register causes #UD exception
      if (decode.big_op) {
    ...
    case 0x8e:
      dyn_get_modrm();
      if (GCC_UNLIKELY((SegNames)decode.modrm.reg == cs))
        goto illegalopcode;
      dyn_mov_seg_ev();
      break;
    /* POP Ev */
    ...
    case 0x9c: // PUSHF
      ...
      gen_releasereg(DREG(ESP));
      dyn_flags_gen_to_host();
      dyn_call_function_pagefault_check((void *)&CPU_PUSHF, "%Rd%Id",
                                        DREG(TMPB), decode.big_op);
      dyn_check_bool_exception(DREG(TMPB));
      gen_releasereg(DREG(TMPB));
      break;
    case 0x9d: // POPF
      gen_releasereg(DREG(ESP));
      gen_releasereg(DREG(FLAGS));
      dyn_call_function_pagefault_check((void *)&CPU_POPF, "%Rd%Id", DREG(TMPB),
                                        decode.big_op);
      dyn_check_bool_exception(DREG(TMPB));
      dyn_flags_host_to_gen();
      gen_releasereg(DREG(TMPB));
    ...
    case 0xa0:
      gen_lea(DREG(EA), decode.segprefix ? decode.segprefix : DREG(DS), nullptr,
              0, decode.big_addr ? decode_fetchd() : decode_fetchw());
    ...
    case 0xa2:
      if (decode.big_addr) {
        Bitu val;
        if (decode_fetchd_imm(val)) {
          gen_lea_imm_mem(DREG(EA),
                          decode.segprefix ? decode.segprefix : DREG(DS),
                          (void *)val);
        } else {
          gen_lea(DREG(EA), decode.segprefix ? decode.segprefix : DREG(DS),
                  nullptr, 0, (Bits)val);
        }
        dyn_write_byte_release(DREG(EA), DREG(EAX), false);
      } else {
        gen_lea(DREG(EA), decode.segprefix ? decode.segprefix : DREG(DS),
                nullptr, 0, decode_fetchw());
        dyn_write_byte_release(DREG(EA), DREG(EAX), false);
      }
      break;
    ...
    /* CMPSB/W/D*/
    case 0xa6:
      dyn_string(STR_CMPSB);
      break;
    case 0xa7:
      dyn_string(decode.big_op ? STR_CMPSD : STR_CMPSW);
      break;
    /* TEST AL,AX Imm */
    case 0xa8:
      gen_discardflags();
      gen_dop_byte_imm(DOP_TEST, DREG(EAX), 0, decode_fetchb());
      break;
    case 0xa9:
      gen_discardflags();
      gen_dop_word_imm(DOP_TEST, decode.big_op, DREG(EAX),
                       decode.big_op ? decode_fetchd() : decode_fetchw());
      break;
    /* STOSB/W/D*/
    case 0xaa:
    ...
    /* SCASB/W/D*/
    case 0xae:
      dyn_string(STR_SCASB);
      break;
    case 0xaf:
      dyn_string(decode.big_op ? STR_SCASD : STR_SCASW);
      break;
    ...
    case 0xb6:
    case 0xb7:
      gen_dop_byte_imm(DOP_MOV, &DynRegs[opcode & 3], opcode & 4,
                       decode_fetchb());
      break;
    ...
    case 0xbf:
      if (decode.big_op) {
        dyn_dop_word_imm(DOP_MOV, decode.big_op, &DynRegs[opcode & 7]);
        break;
      } else {
        gen_dop_word_imm(DOP_MOV, decode.big_op, &DynRegs[opcode & 7],
                         decode_fetchw());
        break;
      }
      break;
    // GRP2 Eb/Ev,Ib
    case 0xc0:
      dyn_grp2_eb(grp2_imm);
      break;
    case 0xc1:
      dyn_grp2_ev(grp2_imm);
      break;
    // RET near Iw / Ret
    case 0xc2:
      dyn_ret_near(decode_fetchw());
      goto finish_block;
    case 0xc3:
      dyn_ret_near(0);
      goto finish_block;
    ...
    case 0xd6:
      gen_needflags();
      gen_protectflags();
      gen_dop_byte(DOP_SBB, DREG(EAX), 0, DREG(EAX), 0);
      break;
    // XLAT
    case 0xd7:
      dyn_xlat();
    ...
    // IN AL/AX,imm
    case 0xe4:
      dyn_call_function_pagefault_check((void *)&dyn_io_readB, "%Id",
                                        decode_fetchb());
      dyn_check_bool_exception_al();
    ...
    case 0xe5:
      if (!decode.big_op) {
        dyn_call_function_pagefault_check((void *)&dyn_io_readW, "%Id",
                                          decode_fetchb());
      } else {
        dyn_call_function_pagefault_check((void *)&dyn_io_readD, "%Id",
                                          decode_fetchb());
      }
      dyn_check_bool_exception_al();
    ...
    // OUT imm,AL
    case 0xe6:
      dyn_call_function_pagefault_check((void *)&dyn_io_writeB, "%Id%Dl",
                                        decode_fetchb(), DREG(EAX));
      dyn_check_bool_exception_al();
      break;
    case 0xe7:
      if (!decode.big_op) {
        dyn_call_function_pagefault_check((void *)&dyn_io_writeW, "%Id%Dw",
                                          decode_fetchb(), DREG(EAX));
      } else {
        dyn_call_function_pagefault_check((void *)&dyn_io_writeD, "%Id%Dd",
                                          decode_fetchb(), DREG(EAX));
      }
      dyn_check_bool_exception_al();
      break;
    case 0xe8: /* CALL Ivx */
      ...
      goto finish_block;
    case 0xe9: /* Jmp Ivx */
      dyn_exit_link(decode.big_op ? (int32_t)decode_fetchd()
                                  : (int16_t)decode_fetchw());
      goto finish_block;
    ...
    case 0xec:
      ...
      dyn_call_function_pagefault_check((void *)&dyn_io_readB, "%Dw",
                                        DREG(EDX));
      dyn_check_bool_exception_al();
      gen_mov_host(&core_dyn.readdata, DREG(EAX), 1);
      break;
    case 0xed:
      dyn_save_vmware_relevant_regs();
      if (!decode.big_op) {
        dyn_call_function_pagefault_check((void *)&dyn_io_readW, "%Dw",
                                          DREG(EDX));
      } else {
        dyn_call_function_pagefault_check((void *)&dyn_io_readD, "%Dw",
                                          DREG(EDX));
      }
      dyn_check_bool_exception_al();
      gen_mov_host(&core_dyn.readdata, DREG(EAX), decode.big_op ? 4 : 2);
    ...
    /* OUT DX,AL/AX */
    case 0xee:
      dyn_call_function_pagefault_check((void *)&dyn_io_writeB, "%Dw%Dl",
                                        DREG(EDX), DREG(EAX));
      dyn_check_bool_exception_al();
      break;
    case 0xef:
      if (!decode.big_op) {
        dyn_call_function_pagefault_check((void *)&dyn_io_writeW, "%Dw%Dw",
                                          DREG(EDX), DREG(EAX));
      } else {
        dyn_call_function_pagefault_check((void *)&dyn_io_writeD, "%Dw%Dd",
                                          DREG(EDX), DREG(EAX));
      }
      dyn_check_bool_exception_al();
      break;
    ...
    case 0xfa: // CLI
      ...
      break;
    case 0xfb:             // STI
      goto illegalopcode2; // signal "illegal" opcode so the core reflects to
                           // normal core which can then properly do the STI
                           // delay
    case 0xfc:             // CLD
      gen_protectflags();
    ...
    case 0xfe:
      ...
      case 0x1: // DEC Eb
        if (decode.modrm.mod < 3) {
          ...
          dyn_read_byte(DREG(EA), DREG(TMPB), false);
          gen_needcarry();
          gen_sop_byte(decode.modrm.reg == 0 ? SOP_INC : SOP_DEC, DREG(TMPB),
                       0);
          dyn_write_byte_release(DREG(EA), DREG(TMPB), false);
          gen_releasereg(DREG(TMPB));
        } else {
    ...
    case 0xff: {
      ...
      case 0x1: // DEC Ev
        gen_needcarry();
        gen_sop_word(decode.modrm.reg == 0 ? SOP_INC : SOP_DEC, decode.big_op,
                     src);
        if (decode.modrm.mod < 3) {
          dyn_write_word_release(DREG(EA), DREG(TMPW), decode.big_op);
          gen_releasereg(DREG(TMPW));
        ...
        break;
      case 0x2: /* CALL Ev */
        gen_lea(DREG(TMPB), DREG(EIP), nullptr, 0,
                decode.code - decode.code_start);
        dyn_push(DREG(TMPB));
        gen_releasereg(DREG(TMPB));
      ...
      case 0x5: /* JMP Ep */
        ...
        if (decode.big_op) {
          dyn_call_function_pagefault_check(
              ...
              "%Id%Drw%Drd%Drd", decode.big_op, DREG(EA), DREG(TMPW),
              DREG(TMPB));
        } else {
          dyn_call_function_pagefault_check(
              decode.modrm.reg == 3 ? (void *)&CPU_CALL : (void *)&CPU_JMP,
              "%Id%Drw%Drw%Drd", decode.big_op, DREG(EA), DREG(TMPW),
              DREG(TMPB));
        }
        dyn_flags_host_to_gen();
        goto core_close_block;
