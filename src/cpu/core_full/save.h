...
case S_Eb:
  if (inst.rm < 0xc0)
    SaveMb(inst.rm_eaa, inst_op1_b);
  else
    reg_8(inst.rm_eai) = inst_op1_b;
  break;
case S_Gb:
...
case S_Ew:
  if (inst.rm < 0xc0)
    SaveMw(inst.rm_eaa, inst_op1_w);
  else
    reg_16(inst.rm_eai) = inst_op1_w;
  break;
case S_Gw:
...
case S_Ed:
  if (inst.rm < 0xc0)
    SaveMd(inst.rm_eaa, inst_op1_d);
  else
    reg_32(inst.rm_eai) = inst_op1_d;
  break;
...
case S_REGd:
  ...
  break;
case S_SEGm:
  if (CPU_SetSegGeneral((SegNames)inst.rm_index, inst_op1_w))
    RunException();
  break;
case S_SEGGw:
  if (CPU_SetSegGeneral((SegNames)inst.code.extra, inst_op2_w))
    RunException();
  reg_16(inst.rm_index) = inst_op1_w;
  break;
case S_SEGGd:
...
case 0:
  break;
default:
  LOG(LOG_CPU, LOG_ERROR)("SAVE:Unhandled code %d entry %lx", inst.code.save,
