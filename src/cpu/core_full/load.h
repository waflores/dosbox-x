...
l_MODRMswitch:
  ...
  case M_Ebx:
    if (inst.rm < 0xc0)
      inst_op1_ds = (int8_t)LoadMb(inst.rm_eaa);
    else
      inst_op1_ds = (int8_t)reg_8(inst.rm_eai);
    break;
  case M_EbIb:
    inst_op2_d = Fetchb();
  case M_Eb:
    if (inst.rm < 0xc0)
      inst_op1_d = LoadMb(inst.rm_eaa);
    else
      inst_op1_d = reg_8(inst.rm_eai);
    break;
  case M_EbGb:
    if (inst.rm < 0xc0)
      inst_op1_d = LoadMb(inst.rm_eaa);
  ...
  case M_GbEb:
    if (inst.rm < 0xc0)
      inst_op2_d = LoadMb(inst.rm_eaa);
    else
      inst_op2_d = reg_8(inst.rm_eai);
  case M_Gb:
  ...
  case M_Ewx:
    if (inst.rm < 0xc0)
      inst_op1_ds = (int16_t)LoadMw(inst.rm_eaa);
    else
      inst_op1_ds = (int16_t)reg_16(inst.rm_eai);
    break;
  case M_EwIb:
    inst_op2_d = Fetchb();
    goto l_M_Ew;
  ...
  case M_Ew:
    if (inst.rm < 0xc0)
      ...
      inst_op1_d = reg_16(inst.rm_eai);
    break;
  case M_GwEw:
    if (inst.rm < 0xc0)
      inst_op2_d = LoadMw(inst.rm_eaa);
    else
      inst_op2_d = reg_16(inst.rm_eai);
  case M_Gw:
  ...
  case M_Edx:
    if (inst.rm < 0xc0)
      inst_op1_d = (uint32_t)((int32_t)LoadMd(inst.rm_eaa));
    else
      inst_op1_d = (uint32_t)((int32_t)reg_32(inst.rm_eai));
    break;
  case M_EdIb:
    inst_op2_d = Fetchb();
    goto l_M_Ed;
  ...
  case M_Ed:
    if (inst.rm < 0xc0)
      ...
      inst_op1_d = reg_32(inst.rm_eai);
    break;
  case M_GdEd:
    if (inst.rm < 0xc0)
      inst_op2_d = LoadMd(inst.rm_eaa);
    else
      inst_op2_d = reg_32(inst.rm_eai);
  case M_Gd:
  ...
  case 0:
    break;
  default:
    LOG(LOG_CPU, LOG_ERROR)("MODRM:Unhandled load %d entry %lx",
                            inst.code.extra, (unsigned long)inst.entry);
    break;
  }
  break;
case L_POPw:
...
case D_POPAd: {
  ...
  goto nextopcode;
case D_POPSEGw:
  if (CPU_PopSeg((SegNames)inst.code.extra, false))
    RunException();
  goto nextopcode;
case D_POPSEGd:
  if (CPU_PopSeg((SegNames)inst.code.extra, true))
    RunException();
  goto nextopcode;
case D_SETALC:
...
case D_STD:
  ...
  goto nextopcode;
case D_PUSHF:
  if (CPU_PUSHF(inst.code.extra))
    RunException();
  goto nextopcode;
case D_POPF:
  if (CPU_POPF(inst.code.extra))
    RunException();
  if (GETFLAG(IF) && PIC_IRQCheck) {
    SaveIP();
    return CBRET_NONE;
...
case D_NOP:
  goto nextopcode;
case D_LOCK: /* FIXME: according to intel, LOCK should raise an exception if
                it's not followed by one of a small set of instructions;
                probably doesn't matter for our purposes as it is a pentium
                prefix anyhow */
...
case D_RDTSC: {
  ...
  int64_t tsc = (int64_t)(PIC_FullIndex() *
                          (double)(CPU_CycleAutoAdjust ? 70000 : CPU_CycleMax));
  reg_edx = (uint32_t)(tsc >> 32);
  reg_eax = (uint32_t)(tsc & 0xffffffff);
  break;
}
default:
  LOG(LOG_CPU, LOG_ERROR)("LOAD:Unhandled code %d opcode %lx", inst.code.load,
