...
case O_INT:
  ...
  continue;
case O_INb:
  if (CPU_IO_Exception(inst_op1_d, 1))
    RunException();
  reg_al = IO_ReadB(inst_op1_d);
  goto nextopcode;
case O_INw:
  if (CPU_IO_Exception(inst_op1_d, 2))
    RunException();
  reg_ax = IO_ReadW(inst_op1_d);
  goto nextopcode;
case O_INd:
  if (CPU_IO_Exception(inst_op1_d, 4))
    RunException();
...
case O_GRP6d:
  ...
  case 0x02: /* LLDT */
    ...
    if (CPU_LLDT(inst_op1_d))
      RunException();
    goto nextopcode; /* Else value will saved */
  case 0x03:         /* LTR */
    if (cpu.cpl)
      EXCEPTION(EXCEPTION_GP);
    if (CPU_LTR(inst_op1_d))
      RunException();
    goto nextopcode; /* Else value will saved */
  case 0x04:         /* VERR */
    CPU_VERR(inst_op1_d);
  ...
  case 0x05:         /* VERW */
    ...
    goto nextopcode; /* Else value will saved */
  default:
    LOG(LOG_CPU, LOG_ERROR)("Group 6 Illegal subfunction %lx",
                            (unsigned long)inst.rm_index);
    goto illegalopcode;
  }
  break;
case O_GRP7w:
case O_GRP7d:
  ...
  case 2: /* LGDT */
    if (cpu.pmode && cpu.cpl)
      EXCEPTION(EXCEPTION_GP);
    CPU_LGDT(LoadMw(inst.rm_eaa),
             LoadMd(inst.rm_eaa + 2) &
                 ((inst.code.op == O_GRP7w) ? 0xFFFFFF : 0xFFFFFFFF));
    goto nextopcode;
  case 3: /* LIDT */
    if (cpu.pmode && cpu.cpl)
      EXCEPTION(EXCEPTION_GP);
    CPU_LIDT(LoadMw(inst.rm_eaa),
             LoadMd(inst.rm_eaa + 2) &
                 ((inst.code.op == O_GRP7w) ? 0xFFFFFF : 0xFFFFFFFF));
    goto nextopcode;
  case 4: /* SMSW */
  ...
  case 7: /* INVLPG */
    ...
    goto nextopcode;
  default:
    LOG(LOG_CPU, LOG_ERROR)("Group 7 Illegal subfunction %lx",
                            (unsigned long)inst.rm_index);
    goto illegalopcode;
  }
  break;
case O_M_CRx_Rd:
  if (CPU_WRITE_CRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_M_Rd_CRx:
  if (CPU_READ_CRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_M_DRx_Rd:
  if (CPU_WRITE_DRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_M_Rd_DRx:
  if (CPU_READ_DRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_M_TRx_Rd:
  if (CPU_WRITE_TRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_M_Rd_TRx:
  if (CPU_READ_TRX(inst.rm_index, inst_op1_d))
    RunException();
  break;
case O_LAR: {
  Bitu ar = inst_op2_d;
...
case O_CMPXCHG:
  ...
  if (inst_op1_d == reg_eax) {
    ...
    if (inst.rm < 0xc0)
      SaveMd(inst.rm_eaa, inst_op1_d); // early write-pf
    SETFLAGBIT(ZF, 1);
  } else {
    if (inst.rm < 0xc0)
      SaveMd(inst.rm_eaa, inst_op1_d); // early write-pf
...
case O_FPU:
  ...
  case 0x00:
    FPU_ESC0_EA(inst.rm, inst.rm_eaa);
    break;
  case 0x01:
    FPU_ESC1_EA(inst.rm, inst.rm_eaa, inst.code.extra);
    break;
  case 0x02:
    FPU_ESC2_EA(inst.rm, inst.rm_eaa);
    break;
  case 0x03:
    FPU_ESC3_EA(inst.rm, inst.rm_eaa);
    break;
  case 0x04:
    FPU_ESC4_EA(inst.rm, inst.rm_eaa);
    break;
  case 0x05:
    FPU_ESC5_EA(inst.rm, inst.rm_eaa, inst.code.extra);
    break;
  case 0x06:
    FPU_ESC6_EA(inst.rm, inst.rm_eaa);
    break;
  case 0x07:
    FPU_ESC7_EA(inst.rm, inst.rm_eaa);
    break;

  case 0x08:
    FPU_ESC0_Normal(inst.rm);
    break;
  case 0x09:
    FPU_ESC1_Normal(inst.rm);
    break;
  case 0x0a:
    FPU_ESC2_Normal(inst.rm);
    break;
  case 0x0b:
    FPU_ESC3_Normal(inst.rm);
    break;
  case 0x0c:
    FPU_ESC4_Normal(inst.rm);
    break;
  case 0x0d:
    FPU_ESC5_Normal(inst.rm);
    break;
  case 0x0e:
    FPU_ESC6_Normal(inst.rm);
    break;
  case 0x0f:
    FPU_ESC7_Normal(inst.rm);
    break;
  }
  goto nextopcode;
#else
...
case 0:
  break;
default:
  LOG(LOG_CPU, LOG_ERROR)("OP:Unhandled code %d entry %lx", inst.code.op,
                          (unsigned long)inst.entry);
}
