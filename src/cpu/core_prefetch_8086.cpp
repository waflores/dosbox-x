...
/* Do not emulate segment limit exceptions on 8086. This turns the if()
 * statements into if (0) and the C++ compiler optimizer should then completely
 * omit the code for the 8086 cputype. */
#define do_seg_limits (0)

/* 8086: Apparently exceptions like DIVIDE BY ZERO are given a return address of
...
/* NTS: we special case writes to seg:ffff to emulate 8086 behavior where word
 * read/write wraps around the 64KB segment */
#define LoadMb(off) mem_readb_inline(off)

static inline uint16_t LoadMw(Bitu off) {
  if (last_ea86_offset == 0xffff)
    return (mem_readb_inline((PhysPt)off) |
            (mem_readb_inline((PhysPt)(off - 0xffff)) << 8));
...
static INLINE void FetchDiscardb() { FetchDiscard<uint8_t>(); }

static INLINE uint8_t FetchPeekb() { return FetchPeek<uint8_t>(); }

static uint8_t Fetchb() { return Fetch<uint8_t>(); }

static uint16_t Fetchw() { return Fetch<uint16_t>(); }

static uint32_t Fetchd() { return Fetch<uint32_t>(); }
...
Bits CPU_Core8086_Prefetch_Run(void) {
  ...
  pq_limit = (max(CPU_PrefetchQueueSize, (unsigned int)(4ul + prefetch_unit)) +
              prefetch_unit - 1ul) &
             (~(prefetch_unit - 1ul));
  ...
  restart_opcode:
    ...
    invalidate_pq = false;
    if (core.opcode_index & OPCODE_0F)
      invalidate_pq = true;
    else
      switch (next_opcode) {
      case 0x60:
...
skip_saveip:
  ...
  if (PIC_FullIndex() > pq_next_dbg) {
    LOG_MSG("Prefetch core debug: prefetch cache hit=%u miss=%u", pq_hit,
            pq_miss);
    pq_next_dbg += 500.0;
  }
#endif
 ...
 * See also [https://www.pcjs.org/pubs/pc/reference/intel/8086/] and
