...
CASE_0F_MMX(0x10) {
  ...
  case MP_NONE: /* 0F 10 MOVUPS reg, r/m */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII || !CPU_SSE())
      goto illegal_opcode;
    if (rm >= 0xc0) {
...
CASE_0F_MMX(0xc2) {
  ...
  case MP_NONE: /* 0F C2 CMPPS reg, r/m, imm8 */
    if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUMIII || !CPU_SSE())
      goto illegal_opcode;
    if (rm >= 0xc0) {
      /* FIXME: Documentation says these are "reserved", doesn't say it causes a
       * #UD, what really happens? */
      if ((cf = Fetchb()) > 7)
        goto illegal_opcode;
...
CASE_0F_MMX(0xD8) /* PSUBUSB Pq,Qq */
{
  ...
  result.q = 0;
  result.ub.b0 =
      SaturateWordSToByteU((int16_t)dest->ub.b0 - (int16_t)src.ub.b0);
  result.ub.b1 =
      SaturateWordSToByteU((int16_t)dest->ub.b1 - (int16_t)src.ub.b1);
  result.ub.b2 =
      SaturateWordSToByteU((int16_t)dest->ub.b2 - (int16_t)src.ub.b2);
  result.ub.b3 =
      SaturateWordSToByteU((int16_t)dest->ub.b3 - (int16_t)src.ub.b3);
  result.ub.b4 =
      SaturateWordSToByteU((int16_t)dest->ub.b4 - (int16_t)src.ub.b4);
  result.ub.b5 =
      SaturateWordSToByteU((int16_t)dest->ub.b5 - (int16_t)src.ub.b5);
  result.ub.b6 =
      SaturateWordSToByteU((int16_t)dest->ub.b6 - (int16_t)src.ub.b6);
  result.ub.b7 =
      SaturateWordSToByteU((int16_t)dest->ub.b7 - (int16_t)src.ub.b7);
  dest->q = result.q;
  break;
}
CASE_0F_MMX(0xD9) /* PSUBUSW Pq,Qq */
{
  ...
  result.q = 0;
  result.uw.w0 =
      SaturateDwordSToWordU((int32_t)dest->uw.w0 - (int32_t)src.uw.w0);
  result.uw.w1 =
      SaturateDwordSToWordU((int32_t)dest->uw.w1 - (int32_t)src.uw.w1);
  result.uw.w2 =
      SaturateDwordSToWordU((int32_t)dest->uw.w2 - (int32_t)src.uw.w2);
  result.uw.w3 =
      SaturateDwordSToWordU((int32_t)dest->uw.w3 - (int32_t)src.uw.w3);
  dest->q = result.q;
  break;
}
CASE_0F_MMX(0xda) {
...
CASE_0F_MMX(0xE9) /* PSUBSW Pq,Qq */
{
  ...
  dest->sw.w0 =
      SaturateDwordSToWordS((int32_t)dest->sw.w0 - (int32_t)src.sw.w0);
  dest->sw.w1 =
      SaturateDwordSToWordS((int32_t)dest->sw.w1 - (int32_t)src.sw.w1);
  dest->sw.w2 =
      SaturateDwordSToWordS((int32_t)dest->sw.w2 - (int32_t)src.sw.w2);
  dest->sw.w3 =
      SaturateDwordSToWordS((int32_t)dest->sw.w3 - (int32_t)src.sw.w3);
  break;
}
CASE_0F_MMX(0xea) {
  GetRM;
...
CASE_0F_MMX(0xf7) {
  ...
  case MP_NONE: /* 0F F7 MASKMOVQ reg, r/m */
    if (rm >= 0xc0) {
      /* the MSB of each byte in second operand indicates whether to write the
       * corresponding byte value from the first operand i.e. MASKMOVQ mm1,mm2
       * would selectively write bytes from mm1 according to MSBs of each byte
       * in mm1. The target memory location is DS:EDI (DS can be overridden by a
       * segment prefix). */
      /* On real hardware, this controls the byte enables on the wide data path
       * from the processor. We don't have that here, so we have to break up the
       * writes */
      PhysPt aa =
          BaseDS + (reg_edi & AddrMaskTable[core.prefixes & PREFIX_ADDR]);
      const MMX_reg *data = reg_mmx[reg];
      const MMX_reg *msk = reg_mmx[rm & 7];
      if (msk->ub.b0 & 0x80)
        SaveMb(aa + 0, data->ub.b0);
      if (msk->ub.b1 & 0x80)
        SaveMb(aa + 1, data->ub.b1);
      if (msk->ub.b2 & 0x80)
        SaveMb(aa + 2, data->ub.b2);
      if (msk->ub.b3 & 0x80)
        SaveMb(aa + 3, data->ub.b3);
      if (msk->ub.b4 & 0x80)
        SaveMb(aa + 4, data->ub.b4);
      if (msk->ub.b5 & 0x80)
        SaveMb(aa + 5, data->ub.b5);
      if (msk->ub.b6 & 0x80)
        SaveMb(aa + 6, data->ub.b6);
      if (msk->ub.b7 & 0x80)
        SaveMb(aa + 7, data->ub.b7);
    } else {
      goto illegal_opcode;
    }
    break;
