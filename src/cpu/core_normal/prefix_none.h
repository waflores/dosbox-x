...
SAVEIP;
if (CPU_PopSeg(cs, false))
  RUNEXCEPTION();
goto skip_saveip;
#else
#if CPU_CORE == CPU_ARCHTYPE_286
/* try not to slow down 386+ emulation with a check for 186 every time we hit an
 * opcode starting with 0x0F, do this check only for the 80186 and 286 emulation
 * provided by normal/prefetch 286 core code. */
...
#if CPU_CORE == CPU_ARCHTYPE_8086
JumpCond16_b(TFLG_O);
break; /* alias of 0x70, jo. see also
...
#else
{
  ...
  try {
    uint16_t old_sp =
        (CPU_ArchitectureType >= CPU_ARCHTYPE_286 ? reg_sp : (reg_sp - 10));
    Push_16(reg_ax);
    Push_16(reg_cx);
    Push_16(reg_dx);
    Push_16(reg_bx);
...
CASE_W(0x8d) /* LEA Gw */
{
  GetRMrw;
  if (rm >= 0xc0)
    goto illegal_opcode; // Direct register causes #UD exception
...
{ /* NTS: GetEADirect may jump instead to the GP# trigger code if the offset
     exceeds the segment limit. For whatever reason, NOT signalling GP# in that
     condition prevents Windows 95 OSR2 from starting a DOS VM. Weird. */
  GetEADirect(1);
  reg_al = LoadMb(eaa);
}
break;
...
opcode_c2:
  ...
  try {
    /* this is structured either to complete RET or leave registers unmodified
     * if interrupted by page fault */
...
CASE_W(0xc4) /* LES */
{
  ...
  if (do_lds_wraparound &&
      !cpu.code.big /*8086 table is missing latter half for 32-bit!*/) {
    /* stack underflow 64KB wraparound?
     * [https://github.com/joncampbell123/dosbox-x/issues/5621] */
    GetEAaN8086; /* 8086 version that also sets last_ea86_offset */
    if (last_ea86_offset > (0x10000u - 4u)) {
      if (CPU_SetSegGeneral(es, LoadMw(eaa + 2 - 0x10000)))
        RUNEXCEPTION();
      *rmrw = LoadMw(eaa);
      break;
    }
  ...
  GetEAaN;
#endif
  if (CPU_SetSegGeneral(es, LoadMw(eaa + 2)))
    RUNEXCEPTION();
  *rmrw = LoadMw(eaa);
  break;
}
CASE_W(0xc5) /* LDS */
{
  ...
  if (do_lds_wraparound &&
    ...
    GetEAaN8086; /* 8086 version that also sets last_ea86_offset */
    if (last_ea86_offset > (0x10000u - 4u)) {
      if (CPU_SetSegGeneral(ds, LoadMw(eaa + 2 - 0x10000)))
        RUNEXCEPTION();
      *rmrw = LoadMw(eaa);
      break;
    }
  ...
  GetEAaN;
#endif
  if (CPU_SetSegGeneral(ds, LoadMw(eaa + 2)))
    RUNEXCEPTION();
  *rmrw = LoadMw(eaa);
  break;
}
...
//		LOG(LOG_CPU,LOG_NORMAL)("CPU:LOCK"); /* FIXME: see case D_LOCK
//in core_full/load.h */
break;
#if CPU_CORE >= CPU_ARCHTYPE_80186
CASE_B(0xf1) /* ICEBP */
...
//      CLI
//
//      The FM music driver for the PC-98 version of Peret em Heru appears to
//      have STI+CLI sequences for some reason in certain subroutines within the
//      FM music interrupt handler, which should not be trigger points to
//      process interrupts because the FM interrupt is non-reentrant and Peret
//      is also calling another entry point to the FM driver from IRQ 2 (vsync).
//      If IRQ 2 is processed while the FM interrupt is processing at the STI,
//      the stack switch will overwrite the first and the FM interrupt will
//      return by stale data and crash.
//
//      [https://github.com/joncampbell123/dosbox-x/issues/1162]
//
// NTS: The prior fix that set CPU_Cycles = 4 causes the normal core to get
// stuck
//      and never break out (hanging DOSBox-X) when running PC-98 game Night
//      Slave, so that isn't a long-term option.
//
//      Capping CPU_Cycles = 2 seems to break Commander Keen games because of
//      the way the video vsync and wait loop works. It waits for retrace/vsync
//      with interrupts disabled, then on vertical retrace, briefly enables
//      interrupts to allow them to run by jumping to a STI + JMP short $+2 +
//      CLI sequence. Note the code deliberately uses a JMP short delay to avoid
//      STI + CLI and make sure interrupts process.
//
// NTS: New idea: On STI, force the core to execute another instruction
// immediately.
//      If there isn't enough CPU cycles, then break out of the loop. If CLI
//      follows immediately, then it will undo the effects of STI without a
//      chance to process interrupts.
if (CPU_STI())
  RUNEXCEPTION();
if (GETFLAG(IF) && PIC_IRQCheck) {
...
CASE_W(0xff) /* GRP5 Ew */
{
  ...
  case 0x01: /* DEC Ew */
    ...
    break;
  case 0x02: /* CALL Ev */
  {          /* either EIP is set to the call address or EIP does not change if
                interrupted by PF */
    uint16_t new_eip;
    if (rm >= 0xc0) {
      GetEArw;
