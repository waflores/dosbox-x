...
{
  if (((cpu.pmode) && (reg_flags & FLAG_VM)) || (!cpu.pmode))
    goto illegal_opcode;
  GetRMrw;
  if (rm >= 0xc0) {
    GetEArd;
    Bitu new_sel = (uint16_t)*eard;
...
CASE_D(0xc4) /* LES */
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(es, LoadMw(eaa + 4)))
    RUNEXCEPTION();
  *rmrd = LoadMd(eaa);
  break;
}
CASE_D(0xc5) /* LDS */
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(ds, LoadMw(eaa + 4)))
    RUNEXCEPTION();
  *rmrd = LoadMd(eaa);
  break;
}
