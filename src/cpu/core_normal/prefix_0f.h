  ...
  goto illegal_opcode;
{
  ...
  case 0x01: /* STR */
  {
    ...
    if (rm >= 0xc0) {
      GetEArw;
      *earw = (uint16_t)saveval;
    } else {
      GetEAa;
      SaveMw(eaa, (uint16_t)saveval);
    }
  } break;
  case 0x02:
  case 0x03:
  ...
  goto illegal_opcode;
{
  ...
  if (rm < 0xc0) { // First ones all use EA
    ...
    Bitu limit;
    switch (which) {
      /* NTS: The 286 is documented to write 0xFF in the most significant byte
       * of the 6-byte field, while 386 and later is documented to write 0x00
       * (or the MSB of the base if 32-bit form). Some programs, including
       * Microsoft Windows 3.0, execute SGDT and then compare the most
       *      significant byte against 0xFF. It does NOT use the standard Intel
       * detection process. */
    case 0x00: /* SGDT */
      SaveMw(eaa, (uint16_t)CPU_SGDT_limit());
      SaveMd(eaa + 2,
             (uint32_t)(CPU_SGDT_base() |
                        (CPU_ArchitectureType < CPU_ARCHTYPE_386 ? 0xFF000000UL
                                                                 : 0)));
      break;
...
CASE_0F_B(0x19)
CASE_0F_B(0x1A) CASE_0F_B(0x1B) CASE_0F_B(0x1C) CASE_0F_B(0x1D) CASE_0F_B(0x1E)
    CASE_0F_B(0x1F) /* hinting NOPs */
...
CASE_0F_B(0x31) /* RDTSC */
{
  if (CPU_ArchitectureType < CPU_ARCHTYPE_PENTIUM)
    goto illegal_opcode;
  /* Use a fixed number when in auto cycles mode as else the reported value
   * changes constantly */
  ...
  goto illegal_opcode;
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(ss, LoadMw(eaa + 2)))
    RUNEXCEPTION();
  *rmrw = LoadMw(eaa);
  break;
}
  ...
  goto illegal_opcode;
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(fs, LoadMw(eaa + 2)))
    RUNEXCEPTION();
  *rmrw = LoadMw(eaa);
  break;
}
  ...
  goto illegal_opcode;
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(gs, LoadMw(eaa + 2)))
    RUNEXCEPTION();
  *rmrw = LoadMw(eaa);
  break;
}
  ...
  goto illegal_opcode;
{
  ...
  if (rm >= 0xc0) {
    GetEArb;
    *rmrw = (uint16_t)(*(int8_t *)earb);
  } else {
    GetEAa;
    *rmrw = (uint16_t)LoadMbs(eaa);
  }
  break;
}
CASE_0F_B(0xc0) /* XADD Gb,Eb */
{
  ...
  if (rm >= 0xc0) {
    ...
    *earb += oldrmrb;
    ...
    GetEAa;
    lf_var1b = *rmrb = LoadMb(eaa);
    SaveMb(eaa, lf_resb = *rmrb + oldrmrb);
  }
  lflags.type = t_ADDb;
  break;
}
CASE_0F_W(0xc1) /* XADD Gw,Ew */
{
  ...
  if (rm >= 0xc0) {
    ...
    *earw += oldrmrw;
    ...
    GetEAa;
    lf_var1w = *rmrw = LoadMw(eaa);
    SaveMw(eaa, lf_resw = *rmrw + oldrmrw);
  }
  lflags.type = t_ADDw;
  break;
}
