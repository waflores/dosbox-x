...
CASE_0F_D(0xb2) /* LSS Ed */
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(ss, LoadMw(eaa + 4)))
    RUNEXCEPTION();
  *rmrd = LoadMd(eaa);
  break;
}
...
CASE_0F_D(0xb4) /* LFS Ed */
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(fs, LoadMw(eaa + 4)))
    RUNEXCEPTION();
  *rmrd = LoadMd(eaa);
  break;
}
CASE_0F_D(0xb5) /* LGS Ed */
{
  ...
  if (rm >= 0xc0)
    goto illegal_opcode;
  GetEAa;
  if (CPU_SetSegGeneral(gs, LoadMw(eaa + 4)))
    RUNEXCEPTION();
  *rmrd = LoadMd(eaa);
  break;
}
...
CASE_0F_D(0xbe) /* MOVSX Gd,Eb */
{
  ...
  if (rm >= 0xc0) {
    GetEArb;
    *rmrd = (uint32_t)(*(int8_t *)earb);
  } else {
    GetEAa;
    *rmrd = (uint32_t)LoadMbs(eaa);
  }
  break;
}
CASE_0F_D(0xbf) /* MOVSX Gd,Ew */
{
  ...
  if (rm >= 0xc0) {
    GetEArw;
    *rmrd = (uint32_t)(*(int16_t *)earw);
  } else {
    GetEAa;
    *rmrd = (uint32_t)LoadMws(eaa);
  }
  break;
}
CASE_0F_D(0xc1) /* XADD Gd,Ed */
{
  ...
  if (rm >= 0xc0) {
    ...
    *eard += oldrmrd;
    ...
    GetEAa;
    lf_var1d = *rmrd = LoadMd(eaa);
    SaveMd(eaa, lf_resd = *rmrd + oldrmrd);
  }
  lflags.type = t_ADDd;
  break;
}
