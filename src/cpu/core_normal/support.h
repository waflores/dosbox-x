...
static INLINE int8_t Fetchbs() { return (int8_t)Fetchb(); }
static INLINE int16_t Fetchws() { return (int16_t)Fetchw(); }

static INLINE int32_t Fetchds() { return (int32_t)Fetchd(); }
...
/* NTS: At first glance, this looks like code that will only fetch the delta for
 * conditional jumps if the condition is true. Further examination shows that
 * DOSBox's core has two different CS:IP variables, reg_ip and core.cseip which
 * Fetchb() modifies. */
...
/* Most SSE instructions require memory access aligned to the SSE data type,
...
static INLINE void SSE_MULSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_SQRTSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_MOVSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_COMISS_common(const XMM_Reg &d, const XMM_Reg &s) {
  ...
  if (isnan(d.f32[0].v) || isnan(s.f32[0].v))
    ...
    reg_flags |= FLAG_ZF;
  else if (d.f32[0].v > s.f32[0].v) { /* no change */
  } else                              /* d < s */
    reg_flags |= FLAG_CF;
}
...
static INLINE void SSE_MOVMSKPS(uint32_t &d, const XMM_Reg &s) {
  ...
  d = (s.f32[0].f.sign ? 0x1 : 0x0) + (s.f32[1].f.sign ? 0x2 : 0x0) +
      (s.f32[2].f.sign ? 0x4 : 0x0) + (s.f32[3].f.sign ? 0x8 : 0x0);
}
...
static INLINE void SSE_RSQRTSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_RCPSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_ADDSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_SUBSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_DIVSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_MINSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP
...
static INLINE void SSE_MAXSS(XMM_Reg &d, const XMM_Reg &s) { STEP(0); }
#undef STEP

////

#define STEP(i) SSE_CMPPS_i(d.u32[i], d.f32[i], s.f32[i], cf)
static INLINE void SSE_CMPPS_i(uint32_t &d, const FPU_Reg_32 &s1,
                               const FPU_Reg_32 &s2, const uint8_t cf) {
  switch (cf) {
  case 0: /*CMPEQPS*/
    d = (s1.v == s2.v) ? (uint32_t)0xFFFFFFFFul : (uint32_t)0x00000000ul;
    break;
  case 1: /*CMPLTPS*/
    d = (s1.v < s2.v) ? (uint32_t)0xFFFFFFFFul : (uint32_t)0x00000000ul;
    break;
  case 2: /*CMPLEPS*/
    ...
    break;
  case 3: /*CMPUNORDPS*/
    d = (isnan(s1.v) || isnan(s2.v)) ? (uint32_t)0xFFFFFFFFul
...
static INLINE void MMX_PMOVMSKB(uint32_t &d, const MMX_reg &s) {
  d = ((s.ub.b7 & 0x80u) ? 0x80 : 0x00) | ((s.ub.b6 & 0x80u) ? 0x40 : 0x00) |
      ((s.ub.b5 & 0x80u) ? 0x20 : 0x00) | ((s.ub.b4 & 0x80u) ? 0x10 : 0x00) |
      ((s.ub.b3 & 0x80u) ? 0x08 : 0x00) | ((s.ub.b2 & 0x80u) ? 0x04 : 0x00) |
      ((s.ub.b1 & 0x80u) ? 0x02 : 0x00) | ((s.ub.b0 & 0x80u) ? 0x01 : 0x00);
}

static INLINE void SSE_PMOVMSKB(uint32_t &d, const XMM_Reg &s) {
  d = ((s.u8[15] & 0x80u) ? 0x8000 : 0x0000) |
...
#define STEP(i)                                                                \
  d.ub.b##i =                                                                  \
      (uint8_t)(((uint16_t)(d.ub.b##i) + (uint16_t)(s.ub.b##i) + 1u) >> 1u)
  STEP(0);
  STEP(1);
  STEP(2);
  STEP(3);
...
#define STEP(i)                                                                \
  d.u8[i] = (uint8_t)(((uint16_t)(d.u8[i]) + (uint16_t)(s.u8[i]) + 1u) >> 1u)
  STEP(0);
  STEP(1);
  STEP(2);
  STEP(3);
...
#define STEP(i)                                                                \
  d.uw.w##i =                                                                  \
      (uint16_t)(((uint32_t)(d.uw.w##i) + (uint32_t)(s.uw.w##i) + 1u) >> 1u)
  STEP(0);
  STEP(1);
  STEP(2);
  STEP(3);
...
#define STEP(i)                                                                \
  d.u16[i] =                                                                   \
      (uint16_t)(((uint32_t)(d.u16[i]) + (uint32_t)(s.u16[i]) + 1u) >> 1u)
  STEP(0);
  STEP(1);
  STEP(2);
  STEP(3);
...
#define STEP(i)                                                                \
  d.uwa[i] = (uint16_t)(((uint32_t)(d.uwa[i]) * (uint32_t)(s.uwa[i])) >>       \
                        (uint32_t)16u)
  STEP(0);
  STEP(1);
  STEP(2);
...
#define STEP(i)                                                                \
  d.u16[i] = (uint16_t)(((uint32_t)(d.u16[i]) * (uint32_t)(s.u16[i])) >>       \
                        (uint32_t)16u)
  STEP(0);
  STEP(1);
  STEP(2);
...
#define STEP(i) (uint16_t)abs((int16_t)(d.ub.b##i) - (int16_t)(s.ub.b##i))
  d.uw.w0 = STEP(0) + STEP(1) + STEP(2) + STEP(3) + STEP(4) + STEP(5) +
            STEP(6) + STEP(7);
...
#define STEP(i) (uint16_t)abs((int16_t)(d.u8[i]) - (int16_t)(s.u8[i]))
  d.u16[0] = STEP(0) + STEP(1) + STEP(2) + STEP(3) + STEP(4) + STEP(5) +
             STEP(6) + STEP(7);
  d.u16[4] = STEP(8) + STEP(9) + STEP(10) + STEP(11) + STEP(12) + STEP(13) +
             STEP(14) + STEP(15);
