...
#define ADCB(op1, op2, load, save)                                             \
  ...
  lf_var1b = load(op1);                                                        \
  lf_var2b = op2;                                                              \
  lf_resb = (unsigned int)lf_var1b + (unsigned int)lf_var2b +                  \
            (unsigned int)lflags.oldcf;                                        \
  save(op1, lf_resb);                                                          \
  lflags.type = t_ADCb;
...
#define ADCW(op1, op2, load, save)                                             \
  ...
  lf_var1w = load(op1);                                                        \
  lf_var2w = op2;                                                              \
  lf_resw = (unsigned int)lf_var1w + (unsigned int)lf_var2w +                  \
            (unsigned int)lflags.oldcf;                                        \
  save(op1, lf_resw);                                                          \
  lflags.type = t_ADCw;
...
#define PARITY16(x)                                                            \
  (parity_lookup[((unsigned int)((uint16_t)(x)) >> 8u) & 0xffu] ^              \
   parity_lookup[((uint8_t)(x)) & 0xffu] ^ FLAG_PF)
#define PARITY32(x)                                                            \
  (PARITY16(((uint16_t)(x)) & 0xffffu) ^                                       \
   PARITY16(((unsigned int)((uint32_t)(x)) >> 16u) & 0xffffu) ^ FLAG_PF)
...
#define DIVW(op1, load, save)                                                  \
  {                                                                            \
    ...
    SETFLAGBIT(CF, ((rem & 3) >= 1 && (rem & 3) <= 2));                        \
    SETFLAGBIT(PF,                                                             \
               PARITY16(rem & 0xffff) ^ PARITY16(quo16 & 0xffff) ^ FLAG_PF);   \
  }
...
/* NTS: 8086 processors are said to treat a signed divide result of 0x8000
 * (-32768) as a divide overflow, while later processors allow it.
 *
 *      See also:
...
/* NTS: A previous patch attempted to fix this by identifying quo == 0x80, quo
 * == 0x8000, etc. The problem with that is that a signed divide does not give
 * quo == 0x80/0x8000/etc, it gives -0x80/-0x8000/etc. The 8086 signed divide
 * issue checks for THAT.
 *
 *      Note that each quotient overflow check uses the next largest datatype to
 * check it, which would allow -32768 to work.
 *
 *      The 8086 cannot ever trigger IDIVD so IDIVD does not need to check for
 * the 8086 case.
 *
 *      Note Microsoft Flight Simulator 1.0 is affected here because of some
 * internal math like FD28000h / 1FA5h = 8000h which is technically incorrect
 * because the result is 32768, not -32768
 * [https://github.com/joncampbell123/dosbox-x/commit/9be458ba0689d8c60c962808ead4cb11b99c6740].
 *
 *      Some confusion exists here whether to throw an exception for x / y =
 * 32768, and whether to throw an exception for x / y = -32768, more testing is
 * needed on 8086 and 386 processors. */
...
#define IDIVB(op1, load, save)                                                 \
  {                                                                            \
    ...
    if (CPU_CORE == CPU_ARCHTYPE_8086 && (quo == -0x80 || quo == 0x80))        \
      EXCEPTION(0);                                                            \
...
#define IDIVW(op1, load, save)                                                 \
  {                                                                            \
    ...
    if (val == 0)                                                              \
      EXCEPTION(0);                                                            \
    Bits num =                                                                 \
        (int32_t)(((unsigned int)reg_dx << 16u) | (unsigned int)reg_ax);       \
    Bits quo = num / val;                                                      \
    int16_t rem = (int16_t)(num % val);                                        \
    int16_t quo16s = (int16_t)(quo & 0xffff);                                  \
    if (CPU_CORE == CPU_ARCHTYPE_8086 && (quo == -0x8000 || quo == 0x8000))    \
      EXCEPTION(0);                                                            \
    ...
    SETFLAGBIT(CF, ((rem & 3) >= 1 && (rem & 3) <= 2));                        \
    SETFLAGBIT(PF,                                                             \
               PARITY16(rem & 0xffff) ^ PARITY16(quo16s & 0xffff) ^ FLAG_PF);  \
  }
...
#define IDIVD(op1, load, save)                                                 \
  {                                                                            \
    ...
    if (val == 0)                                                              \
      EXCEPTION(0);                                                            \
    int64_t num =                                                              \
        (int64_t)((((uint64_t)reg_edx) << (uint64_t)32) | (uint64_t)reg_eax);  \
    int64_t quo = num / val;                                                   \
    ...
    SETFLAGBIT(CF, ((rem & 3) >= 1 && (rem & 3) <= 2));                        \
    SETFLAGBIT(PF, PARITY32((uint32_t)rem & 0xffffffffu) ^                     \
                       PARITY32((uint32_t)quo32s & 0xffffffffu) ^ FLAG_PF);    \
  }
...
#define IMULD(op1, load, save)                                                 \
  {                                                                            \
    int64_t temps =                                                            \
        ((int64_t)((int32_t)reg_eax)) * ((int64_t)((int32_t)(load(op1))));     \
...
/* FIXME: For 8086 core we care mostly about whether or not SHL/SHR emulate the
 * non-masked shift count. Note that running this code compiled for Intel
 * processors naturally imposes the shift count, compilers generate a shift
 * instruction and do not consider the CPU masking the bit count.
 *
 *        unsigned int a = 0x12345678,b = 0x20,c;
 *        c = a << b;      <-- on Intel x86 builds, c == a because GCC will
 * compile to shl eax,cl and Intel processors will act like c == a << (b&0x1F).
 *
 *        What we care about is that shift counts greater than or equal to the
 * width of the target register come out to zero, or for SAR, that all bits
 * become copies of the sign bit. When emulating the 8086 we want DOS programs
 * to be able to test that we are an 8086 by executing shl ax,cl with cl == 32
 * and to get AX == 0 as a result instead of AX unchanged. the shift count mask
 * test is one of the several tests DOS programs may do to differentiate 8086
 * from 80186. */
#define GRP2B(blah)                                                            \
  {                                                                            \
    GetRM;                                                                     \
    ...
    if (rm >= 0xc0) {                                                          \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x01:                                                               \
        RORB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x02:                                                               \
        RCLB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x03:                                                               \
        RCRB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x05:                                                               \
        SHRB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      case 0x07:                                                               \
        SARB(*earb, val, LoadRb, SaveRb);                                      \
        break;                                                                 \
      }                                                                        \
    } else {                                                                   \
      GetEAa;                                                                  \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x01:                                                               \
        RORB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x02:                                                               \
        RCLB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x03:                                                               \
        RCRB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x05:                                                               \
        SHRB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      case 0x07:                                                               \
        SARB(eaa, val, LoadMb, SaveMb);                                        \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
  }
...
#define GRP2W(blah)                                                            \
  {                                                                            \
    ...
    if (rm >= 0xc0) {                                                          \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x01:                                                               \
        RORW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x02:                                                               \
        RCLW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x03:                                                               \
        RCRW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x05:                                                               \
        SHRW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      case 0x07:                                                               \
        SARW(*earw, val, LoadRw, SaveRw);                                      \
        break;                                                                 \
      }                                                                        \
    } else {                                                                   \
      GetEAa;                                                                  \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x01:                                                               \
        RORW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x02:                                                               \
        RCLW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x03:                                                               \
        RCRW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x05:                                                               \
        SHRW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      case 0x07:                                                               \
        SARW(eaa, val, LoadMw, SaveMw);                                        \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
  }
...
#define GRP2D(blah)                                                            \
  {                                                                            \
    ...
    if (rm >= 0xc0) {                                                          \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x01:                                                               \
        RORD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x02:                                                               \
        RCLD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x03:                                                               \
        RCRD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x05:                                                               \
        SHRD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      case 0x07:                                                               \
        SARD(*eard, val, LoadRd, SaveRd);                                      \
        break;                                                                 \
      }                                                                        \
    } else {                                                                   \
      GetEAa;                                                                  \
      ...
      switch (which) {                                                         \
      case 0x00:                                                               \
        ROLD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x01:                                                               \
        RORD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x02:                                                               \
        RCLD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x03:                                                               \
        RCRD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x04: /* SHL and SAL are the same */                                \
      case 0x06:                                                               \
        SHLD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x05:                                                               \
        SHRD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      case 0x07:                                                               \
        SARD(eaa, val, LoadMd, SaveMd);                                        \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
  }
...
#define DSHLW(op1, op2, op3, load, save)                                       \
  ...
  if (!val)                                                                    \
    break;                                                                     \
  lf_var2b = val;                                                              \
  lf_var1d = ((unsigned int)load(op1) << 16u) | op2;                           \
  uint32_t tempd = lf_var1d << lf_var2b;                                       \
  if (lf_var2b > 16u)                                                          \
    tempd |= ((unsigned int)op2 << (lf_var2b - 16u));                          \
...
#define DSHLD(op1, op2, op3, load, save)                                       \
  ...
  lf_var2b = val;                                                              \
  lf_var1d = load(op1);                                                        \
  lf_resd = ((unsigned int)lf_var1d << lf_var2b) |                             \
            ((unsigned int)op2 >> (32u - lf_var2b));                           \
  save(op1, lf_resd);                                                          \
  lflags.type = t_DSHLd;
...
#define DSHRW(op1, op2, op3, load, save)                                       \
  ...
  if (!val)                                                                    \
    break;                                                                     \
  lf_var2b = val;                                                              \
  lf_var1d = ((unsigned int)op2 << 16u) | load(op1);                           \
  uint32_t tempd = (unsigned int)lf_var1d >> lf_var2b;                         \
  if (lf_var2b > 16u)                                                          \
    tempd |= ((unsigned int)op2 << (32u - lf_var2b));                          \
