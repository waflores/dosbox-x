...
enum DynAccess { DA_d, DA_w, DA_bh, DA_bl };
...
#define DYN_PAGEFAULT_CHECK(x)                                                 \
  {                                                                            \
    try {                                                                      \
      ...
      DYN_PF_LOG_MSG("Caught pagefault at %s:%d (%s)", __FILE__, __LINE__,     \
                     __FUNCTION__);                                            \
    }                                                                          \
    return 0;                                                                  \
  }

static INLINE bool mem_writeb_checked_pagefault(const PhysPt address,
                                                const uint8_t val) {
  DYN_PAGEFAULT_CHECK({ return mem_writeb_checked(address, val); });
}

static INLINE bool mem_writew_checked_pagefault(const PhysPt address,
                                                const uint16_t val) {
  DYN_PAGEFAULT_CHECK({ return mem_writew_checked(address, val); });
}

static INLINE bool mem_writed_checked_pagefault(const PhysPt address,
                                                const uint32_t val) {
  DYN_PAGEFAULT_CHECK({ return mem_writed_checked(address, val); });
}
...
static void dyn_flags_gen_to_host(void) {
  ...
  gen_dop_word(DOP_OR, true, DREG(FLAGS),
               DREG(EXIT)); // flags are marked for save
  gen_releasereg(DREG(EXIT));
  gen_releasereg(DREG(FLAGS));
}
...
static void dyn_restoreregister(DynReg *src_reg, DynReg *dst_reg) {
  dst_reg->flags = src_reg->flags;
  dst_reg->genreg = src_reg->genreg;
  dst_reg->genreg->dynreg =
      dst_reg; // necessary when register has been released
}
...
static void dh_fpu_enter_normal_core(void) {
  ...
  if (!using_normal_core) {
    ...
    if (dyn_dh_fpu
            .dh_fpu_enabled) { /* If NOT enabled, the code below will obliterate
                                  FPU state and problems result */
...
void dyn_core_dh_debug_flush(void) { dh_fpu_enter_normal_core(); }
#else
static Bits Safe_CPU_Core_Normal_Run(void) { return CPU_Core_Normal_Run(); }
static BlockReturnDynX86 safe_gen_runcode(uint8_t *code) {
  return gen_runcode(code);
}
...
restart_core:
  ...
  if (!block) {
    if (!chandler->invalidation_map ||
        (chandler->invalidation_map[ip_point & 4095] < 4)) {
      ...
      while (decoder_pagefault.had_pagefault) {
        ...
        // We can't throw exception during the creation of the block, as it will
        // corrupt things If a page fault occurred, invalidated the block, and
        // try to create smaller block If the page fault is in the current
        // instruction, throw exception
        block->Clear();
        if (cache_size == 1)
          throw GuestPageFaultException(decoder_pagefault.lin_addr,
                                        decoder_pagefault.page_addr,
                                        decoder_pagefault.faultcode);
        cache_size /= 2;
        decoder_pagefault.had_pagefault = false;
        block = CreateCacheBlock(chandler, ip_point, cache_size);
      }
...
run_block:
  ...
  BlockReturnDynX86 ret =
      safe_gen_runcode((uint8_t *)cache_rwtox(block->cache.start));

  if (sizeof(CPU_Cycles) > 4) {
    // HACK: All dynrec cores for each processor assume CPU_Cycles is 32-bit
    // wide.
    //       The purpose of this hack is to sign-extend the lower 32 bits so
    //       that when CPU_Cycles goes negative it doesn't suddenly appear as a
  ...
  case BR_SMCBlock:
    //		LOG_MSG("selfmodification of running block at
    //%x:%x",SegValue(cs),reg_eip);
  ...
  case BR_Link2: {
    ...
    CodePageHandler *temp_handler =
        (CodePageHandler *)get_tlb_readhandler(temp_ip);
    if (temp_handler->flags &
...
void CPU_Core_Dyn_X86_Shutdown(void) { gen_free(); }
...
void CPU_Core_Dyn_X86_Cache_Close(void) { cache_close(); }

void CPU_Core_Dyn_X86_Cache_Reset(void) { cache_reset(); }
