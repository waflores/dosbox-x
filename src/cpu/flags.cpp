...
/*
        Lazy flag system was designed after the same kind of system used in
   Bochs. Probably still some bugs left in here.
*/
...
uint32_t get_OF(void) {
  ...
  case t_ADDd:
  case t_ADCd:
    return ((lf_var1d ^ lf_var2d ^ 0x80000000) & (lf_resd ^ lf_var2d)) &
           0x80000000;
  case t_SBBb:
  case t_SUBb:
  case t_CMPb:
...
#define PARITY16(x)                                                            \
  (parity_lookup[((x) >> 8) & 0xff] ^ parity_lookup[(x) & 0xff] ^ FLAG_PF)
#define PARITY32(x)                                                            \
  (PARITY16((x) & 0xffff) ^ PARITY16(((x) >> 16) & 0xffff) ^ FLAG_PF)

uint16_t parity_lookup[256] = {
    FLAG_PF, 0,       0,       FLAG_PF, 0,       FLAG_PF, FLAG_PF, 0,
    0,       FLAG_PF, FLAG_PF, 0,       FLAG_PF, 0,       0,       FLAG_PF,
    0,       FLAG_PF, FLAG_PF, 0,       FLAG_PF, 0,       0,       FLAG_PF,
    FLAG_PF, 0,       0,       FLAG_PF, 0,       FLAG_PF, FLAG_PF, 0,
    0,       FLAG_PF, FLAG_PF, 0,       FLAG_PF, 0,       0,       FLAG_PF,
    FLAG_PF, 0,       0,       FLAG_PF, 0,       FLAG_PF, FLAG_PF, 0,
...
#define DOFLAG_AF                                                              \
  reg_flags =                                                                  \
      (reg_flags & ~FLAG_AF) | (((lf_var1b ^ lf_var2b) ^ lf_resb) & 0x10U);
...
#define DOFLAG_SFb                                                             \
  reg_flags = (reg_flags & ~FLAG_SF) | ((lf_resb & 0x80U) >> 0U);
#define DOFLAG_SFw                                                             \
  reg_flags = (reg_flags & ~FLAG_SF) | ((lf_resw & 0x8000U) >> 8U);
#define DOFLAG_SFd                                                             \
  reg_flags = (reg_flags & ~FLAG_SF) | ((lf_resd & 0x80000000U) >> 24U);
...
Bitu FillFlags(void) {
  ...
  case t_ADDw:
    ...
    DOFLAG_SFw;
    SET_FLAG(OF,
             ((lf_var1w ^ lf_var2w ^ 0x8000) & (lf_resw ^ lf_var1w)) & 0x8000);
    DOFLAG_PF;
    break;
  case t_ADDd:
    ...
    DOFLAG_ZFd;
    DOFLAG_SFd;
    SET_FLAG(OF, ((lf_var1d ^ lf_var2d ^ 0x80000000) & (lf_resd ^ lf_var1d)) &
                     0x80000000);
    DOFLAG_PF;
    break;
  case t_ADCb:
    SET_FLAG(CF,
             (lf_resb < lf_var1b) || (lflags.oldcf && (lf_resb == lf_var1b)));
    DOFLAG_AF;
    DOFLAG_ZFb;
    DOFLAG_SFb;
    SET_FLAG(OF, ((lf_var1b ^ lf_var2b ^ 0x80) & (lf_resb ^ lf_var1b)) & 0x80);
  ...
  case t_ADCw:
    SET_FLAG(CF,
             (lf_resw < lf_var1w) || (lflags.oldcf && (lf_resw == lf_var1w)));
    DOFLAG_AF;
    DOFLAG_ZFw;
    DOFLAG_SFw;
    SET_FLAG(OF,
  ...
  case t_ADCd:
    SET_FLAG(CF,
             (lf_resd < lf_var1d) || (lflags.oldcf && (lf_resd == lf_var1d)));
    DOFLAG_AF;
    DOFLAG_ZFd;
    DOFLAG_SFd;
    SET_FLAG(OF, ((lf_var1d ^ lf_var2d ^ 0x80000000) & (lf_resd ^ lf_var1d)) &
  ...
  case t_SBBw:
    SET_FLAG(CF,
             (lf_var1w < lf_resw) || (lflags.oldcf && (lf_var2w == 0xffff)));
    DOFLAG_AF;
    DOFLAG_ZFw;
    DOFLAG_SFw;
    ...
    break;
  case t_SBBd:
    SET_FLAG(CF, (lf_var1d < lf_resd) ||
                     (lflags.oldcf && (lf_var2d == 0xffffffff)));
    DOFLAG_AF;
    DOFLAG_ZFd;
    DOFLAG_SFd;
  ...
  case t_SHLb:
    ...
    DOFLAG_ZFb;
    DOFLAG_SFb;
    SET_FLAG(OF, ((unsigned int)lf_resb >> 7U) ^
                     GETFLAG(CF)); /* MSB of result XOR CF. WARNING: This only
                                      works because FLAGS_CF == 1 */
    DOFLAG_PF;
  ...
  case t_SHLw:
    ...
    DOFLAG_ZFw;
    DOFLAG_SFw;
    SET_FLAG(OF, ((unsigned int)lf_resw >> 15U) ^
                     GETFLAG(CF)); /* MSB of result XOR CF. WARNING: This only
                                      works because FLAGS_CF == 1 */
    DOFLAG_PF;
  ...
  case t_SHLd:
    ...
    DOFLAG_ZFd;
    DOFLAG_SFd;
    SET_FLAG(OF, ((unsigned int)lf_resd >> 31U) ^
                     GETFLAG(CF)); /* MSB of result XOR CF. WARNING: This only
                                      works because FLAGS_CF == 1 */
    DOFLAG_PF;
...
void DestroyConditionFlags(void) { lflags.type = t_UNKNOWN; }
