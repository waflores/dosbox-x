...
#define LoadMq(off)                                                            \
  ((uint64_t)((uint64_t)mem_readd(off + 4) << 32 | (uint64_t)mem_readd(off)))
...
#define SaveMq(off, val)                                                       \
  {                                                                            \
    mem_writed(off, val & 0xffffffff);                                         \
    mem_writed(off + 4, (val >> 32) & 0xffffffff);                             \
  }
...
#define DO_PREFIX_ADDR()                                                       \
  core.prefixes =                                                              \
      (core.prefixes & ~PREFIX_ADDR) | (cpu.code.big ^ PREFIX_ADDR);           \
...
#define GETIP                                                                  \
  ((uint32_t)((uintptr_t)core.cseip - (uintptr_t)SegBase(cs) -                 \
              (uintptr_t)MemBase))
#define SAVEIP reg_eip = GETIP;
#define LOADIP                                                                 \
  core.cseip = ((HostPt)((uintptr_t)MemBase + (uintptr_t)SegBase(cs) +         \
                         (uintptr_t)reg_eip));
...
static INLINE void FetchDiscardb() { core.cseip += 1; }
 ...
 *      from ROM provided by an adapter since that requires memory I/O
 * callbacks. */
Bits CPU_Core_Simple_Run(void) {
  ...
  safety_limit =
      (HostPt)((size_t)MemBase + ((size_t)MEM_TotalPages() * (size_t)4096) -
               (size_t)16384); /* safety margin */

  LOADIP;
  if (core.cseip >=
      safety_limit) /* beyond the safety limit, use the normal core */
    return CPU_Core_Normal_Run();

  while (CPU_Cycles-- > 0) {
    LOADIP;

    /* Simple core optimizes for non-paged linear memory access and can break
     * (segfault) if beyond end of memory */
    if (core.cseip >= safety_limit)
      break;
