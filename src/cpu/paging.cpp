...
HostPt PageHandler::GetHostReadPt(PageNum /*phys_page*/) { return nullptr; }

HostPt PageHandler::GetHostWritePt(PageNum /*phys_page*/) { return nullptr; }
...
Bits PageFaultCore(void) {
  ...
  CPU_Cycles = 1;
  Bits ret = CPU_Core_Full_Run(); // FIXME: What is the Full core doing right
                                  // that Normal core is doing wrong here?
  CPU_CycleLeft += CPU_Cycles;
  if (ret < 0)
    E_Exit("Got a dosbox close machine in pagefault core?");
  if (ret)
    return ret;
  if (!pf_queue.used)
    E_Exit("PF Core without PF");
...
static inline uint8_t page_access_bits(const X86PageEntry &dir_entry,
                                       const X86PageEntry &table_entry) {
...
// This table can figure out if we are going to fault right now in the init
// handler (1=fault) bit0-1 ACCESS_ type bit2   1=user mode bit3   1=writing
// bit4   wp
...
static inline PhysPt GetPageDirectoryEntryAddr(LinearPt lin_addr) {
  return PhysPt(
      paging.base.addr +
      ((lin_addr >> 20u) & 0xffcu)); /* equiv: ((lin_addr >> 22) & 0x3ff) * 4 */
}
static inline PhysPt GetPageTableEntryAddr(LinearPt lin_addr,
                                           const X86PageEntry &dir_entry) {
  return PhysPt(dir_entry.dirblock.base << PhysPt(12u)) +
         PhysPt((lin_addr >> LinearPt(10u)) &
                0xffcu); /* equiv: ((lin_addr >> 12) & 0x3ff) * 4 */
}

bool use_dynamic_core_with_paging =
    false; /* allow dynamic core even with paging (AT YOUR OWN RISK!!!!) */
bool auto_determine_dynamic_core_paging =
    false; /* enable use_dynamic_core_with_paging when paging is enabled */
bool dosbox_allow_nonrecursive_page_fault =
    false; /* when set, do nonrecursive mode (when executing instruction) */
...
void PAGING_PageFault(PhysPt lin_addr, Bitu page_addr, Bitu faultcode) {
  ...
  LOG(LOG_PAGING, LOG_NORMAL)("PageFault at %lX type [%lx] queue %d",
                              (unsigned long)lin_addr, (unsigned long)faultcode,
                              (int)pf_queue.used);
  //	LOG_MSG("EAX:%04X ECX:%04X EDX:%04X
  //EBX:%04X",reg_eax,reg_ecx,reg_edx,reg_ebx); 	LOG_MSG("CS:%04X EIP:%08X
  //SS:%04x SP:%08X",SegValue(cs),reg_eip,SegValue(ss),reg_esp);
  entry->cs = SegValue(cs);
  entry->eip = reg_eip;
  entry->page_addr = page_addr; /*misnamed: this is the physical memory address
                                   of the page table entry to update/check */
  entry->mpl = cpu.mpl;
  cpu.mpl = 3;
...
//	DEBUG_EnableDebugger();
  ...
  DOSBOX_RunMachine();
  pf_queue.used--;
  LOG(LOG_PAGING, LOG_NORMAL)("Left PageFault for %lx queue %d",
                              (unsigned long)lin_addr, (int)pf_queue.used);
...
// prepare_only: true in case the calling core handles the fault, else the
// PageFaultCore does NTS: page_addr is misnamed, it is the physical memory
// address of the page table entry to update/check.
//      furthermore for 4MB pages it is the page directory entry representing
//      the 4MB page.
static void PAGING_NewPageFault(PhysPt lin_addr, Bitu page_addr,
  ...
  if (prepare_only) {
    ...
    old_cpudecoder = cpudecoder;
    cpudecoder = &PageFaultCore;
    LOG(LOG_PAGING, LOG_NORMAL)("Recursive PageFault for %lx used=%d",
                                (unsigned long)lin_addr, (int)pf_queue.used);
    if (pf_queue.used >= PF_QUEUESIZE)
      E_Exit("PF queue overrun.");
    if (pf_queue.used != 0)
      LOG_MSG("Warning: PAGING_NewPageFault() more than one level, now using "
              "level %d\n",
              (int)pf_queue.used + 1);
    PF_Entry *entry = &pf_queue.entries[pf_queue.used++];
    entry->cs = SegValue(cs);
...
private:
  void work(PhysPt addr) {
    ...
    const PageNum phys_page =
        PageNum(paging.tlb.phys_page[lin_page] & PHYSPAGE_ADDR);
    ...
    if (!dir_entry.dirblock.p)
      E_Exit("PageFoilHandler unexpected directory entry not present (line %d)",
             __LINE__);

    if (do_pse && dir_entry.dirblock.ps) {
      // for debugging...
      if (dir_entry.dirblock4mb.getBase(lin_page) != phys_page)
        E_Exit("PageFoilHandler unexpected page address changed from TLB copy "
               "(line %d)",
               __LINE__);
      ...
      if (!table_entry.block.p)
        E_Exit("PageFoilHandler unexpecteed table entry not present (line %d)",
               __LINE__);
      ...
      // if (table_entry.block.d) E_Exit("% 8x Already
      // dirty!!",table_entry.load);
    ...
    if (handler->getFlags() & PFLAG_WRITEABLE)
      paging.tlb.write[lin_page] =
          handler->GetHostWritePt(phys_page) - (lin_page << 12);
    else
  ...
  void read() { E_Exit("The page foiler shouldn't be read."); }
...
private:
  ...
  bool hack_check(PhysPt addr) {
    ...
    if (!do_pse) {
      const uint8_t old_attirbs = uint8_t(paging.tlb.phys_page[addr >> 12] >>
                                          PHYSPAGE_ACCESS_BITS_SHIFT);
      X86PageEntry dir_entry, table_entry;
  ...
  void Exception(PhysPt addr, bool writing, bool checked) {
    ...
    // LOG_MSG("XCEPT LIN% 8x wr%d, ch%d, cpl%d, mpl%d",addr, writing, checked,
    // cpu.cpl, cpu.mpl);
    PhysPt tableaddr = 0;
    if (!checked) {
      ...
      dir_entry.load = phys_readd(GetPageDirectoryEntryAddr(addr));
      if (!dir_entry.dirblock.p)
        E_Exit("Undesired situation 1 in exception handler.");

      if (do_pse && dir_entry.dirblock.ps)
        E_Exit("PSE and Exception not yet supported"); // TODO
  ...
  void writeb_through(PhysPt addr, uint8_t val) {
    ...
    PageHandler *handler = MEM_GetPageHandler(phys_page);
    if (handler->getFlags() & PFLAG_WRITEABLE) {
      return host_writeb(handler->GetHostWritePt(phys_page) + (addr & 0xfff),
                         val);
    } else
      return handler->writeb(addr, val);
  }
  ...
  void writew_through(PhysPt addr, uint16_t val) {
    ...
    PageHandler *handler = MEM_GetPageHandler(phys_page);
    if (handler->getFlags() & PFLAG_WRITEABLE) {
      return host_writew(handler->GetHostWritePt(phys_page) + (addr & 0xfff),
                         val);
    } else
      return handler->writew(addr, val);
  }
  ...
  void writed_through(PhysPt addr, uint32_t val) {
    ...
    PageHandler *handler = MEM_GetPageHandler(phys_page);
    if (handler->getFlags() & PFLAG_WRITEABLE) {
      return host_writed(handler->GetHostWritePt(phys_page) + (addr & 0xfff),
                         val);
    } else
      return handler->writed(addr, val);
  }
...
public:
  ...
  // returning true means an exception was triggered for these _checked
  // functions
  bool readb_checked(PhysPt addr, uint8_t *val) override {
    (void)val; // UNUSED
...
static void PAGING_LinkPageNew(PageNum lin_page, PageNum phys_page,
                               const uint8_t linkmode, bool dirty);

static INLINE void InitPageCheckPresence(PhysPt lin_addr, bool writing,
                                         X86PageEntry &table,
                                         X86PageEntry &entry) {
...
static INLINE bool InitPageCheckPresence_CheckOnly(PhysPt lin_addr,
                                                   bool writing,
                                                   X86PageEntry &table,
                                                   X86PageEntry &entry) {
  Bitu lin_page = lin_addr >> 12;
...
public:
  ...
  bool InitPage(PhysPt lin_addr, bool writing, bool prepare_only) {
    ...
    initpage_retry:
      ...
      // Range check to avoid emulator segfault: phys_readd() reads from
      // MemBase+addr and does NOT range check. Needed to avoid segfault when
      // running 1999 demo "Void Main" in a bootable Windows 95 image in pure
      // DOS mode. 2024/12/22: phys_readx() does range checking now
      dir_entry.load = phys_readd(dirEntryAddr);
      ...
      if (!dir_entry.dirblock.p) {
        ...
        if (prepare_only)
          ...
          goto initpage_retry; // TODO maybe E_Exit after a few loops
      }
      ...
      if (do_pse && dir_entry.dirblock.ps) { // 4MB PSE page
        ...
        /* LOG_MSG("INITPSE lin=0x%x phys=0x%lx base22=0x%x base32=0x%x",
                ...
                (unsigned
           long)(((dir_entry.dirblock4mb.base22<<10ul)|((dir_entry.dirblock4mb.base32&enable_pse_extmask)<<20ul)|(lin_page&0x3FFul))<<12ul),
                (unsigned int)dir_entry.dirblock4mb.base22,
                (unsigned int)dir_entry.dirblock4mb.base32); */
        // finally install the new page
        PAGING_LinkPageNew(lin_page, dir_entry.dirblock4mb.getBase(lin_page),
                           result, dirty);
      } else {
        const PhysPt tableEntryAddr =
            GetPageTableEntryAddr(lin_addr, dir_entry);
        // Range check to avoid emulator segfault: phys_readd() reads from
        // MemBase+addr and does NOT range check. 2024/12/22: phys_readx() does
        // range checking now
        table_entry.load = phys_readd(tableEntryAddr);
        ...
        const uint8_t ft_index = result | (writing ? 8u : 0u) |
                                 (isUser ? 4u : 0u) | (paging.wp ? 16u : 0u);
        ...
        if (GCC_UNLIKELY(fault_table[ft_index])) {
          ...
          if (prepare_only)
            ...
            goto initpage_retry; // unlikely to happen?
        }
        ...
        /*
           LOG_MSG("INIT  %s LIN% 8x PHYS% 5x wr%x ch%x wp%x d%x c%x m%x a%x
           [%x/%x/%x]", mtr[result], lin_addr, table_entry.block.base, writing,
           prepare_only, paging.wp, dirty, cpu.cpl, cpu.mpl,
  ...
  void InitPageForced(LinearPt lin_addr) {
    ...
    if (paging.enabled) {
      ...
      if (!table.block.a) {
        ...
        phys_writed((PhysPt)((paging.base.page << 12) + (lin_page >> 10) * 4),
                    table.load);
      }
      ...
      if (do_pse && table.dirblock.ps) { // 4MB PSE page
        ...
        if (!entry.block.a) {
          ...
          phys_writed((table.block.base << 12) + (lin_page & 0x3ff) * 4,
                      entry.load);
        }
...
bool PAGING_MakePhysPage(PageNum &page) {
  ...
  if (paging.enabled) {
    ...
    X86PageEntry dir_entry;
    dir_entry.load =
        phys_readd(GetPageDirectoryEntryAddr((PhysPt)(page << 12)));
    if (!dir_entry.dirblock.p)
      return false;
    ...
    if (do_pse && dir_entry.dirblock.ps) {
      ...
      tbl_entry.load =
          phys_readd(GetPageTableEntryAddr((PhysPt)(page << 12), dir_entry));
...
Bitu PAGING_GetDirBase(void) { return paging.cr3; }
...
void PAGING_ClearTLB(void) {
  ...
  //		paging.links.used, paging.kro_links.used, paging.krw_links.used,
  //paging.ure_links.used);
...
static void PAGING_LinkPageNew(PageNum lin_page, PageNum phys_page,
                               const uint8_t linkmode, bool dirty) {
  const uint8_t xlat_index =
      linkmode | (paging.wp ? 8 : 0) | ((cpu.cpl == 3) ? 4 : 0);
  ...
  // NTS: phys_page is not used to index anything, however it must not use bits
  // 31-30 used for other info. Stay within PHYSPAGE_ADDR.
  ...
  // needed in the exception handler and foiler so they can replace themselves
  // appropriately bit31-30 ACMAP_ bit29	dirty these bits are shifted off
  // at the places paging.tlb.phys_page is read
  paging.tlb.phys_page[lin_page] =
      (uint32_t)(phys_page | (linkmode << 30u) | (dirty ? PHYSPAGE_DIRTY : 0));
  switch (outcome) {
  case ACMAP_RW:
    // read
    if (handler->getFlags() & PFLAG_READABLE)
  ...
  paging.links.entries[paging.links.used++] =
      (uint32_t)lin_page; // "master table"
}
...
void PAGING_SwitchCPL(bool isUser) {
  ...
  //		isUser, paging.kro_links.used, paging.krw_links.used,
  //paging.ure_links.used);
  ...
  if (GCC_UNLIKELY(paging.wp)) {
    ...
    if (isUser) {
      ...
      for (Bitu i = 0; i < paging.kr_links.used; i++) {
        ...
        const PageNum phys_page =
            paging.tlb.phys_page[tlb_index] & PHYSPAGE_ADDR;
        PageHandler *const handler = MEM_GetPageHandler(phys_page);
...
public:
  InitPageUserROHandler() { flags = PFLAG_INIT | PFLAG_NOCODE; }
  void writeb(PhysPt addr, uint8_t val) override {
    InitPage(addr, true, false);
  ...
  bool writeb_checked(PhysPt addr, uint8_t val) override {
    ...
    if (writecode) {
      HostPt tlb_addr;
      if (writecode > 1)
        tlb_addr = get_tlb_read(addr);
      else
        tlb_addr = get_tlb_write(addr);
  ...
  bool InitPage(PhysPt lin_addr, bool writing, bool prepare_only) {
    ...
    if (paging.enabled) {
      ...
      LOG(LOG_PAGING, LOG_NORMAL)("Page access denied: cpl=%i, %x:%x:%x:%x",
                                  (int)cpu.cpl, entry.block.us, table.block.us,
                                  entry.block.wr, table.block.wr);
      PAGING_PageFault(lin_addr,
                       (table.block.base << 12) + (lin_page & 0x3ff) * 4, 0x07);
  ...
  uint8_t InitPageCheckOnly(const PhysPt lin_addr) {
    ...
    if (paging.enabled) {
      ...
      X86PageEntry table, entry;
      if (!InitPageCheckPresence_CheckOnly(lin_addr, true, table, entry))
        return 0;

      if (InitPage_CheckUseraccess(entry.block.us, table.block.us) ||
          (((entry.block.wr == 0) || (table.block.wr == 0)))) {
        LOG(LOG_PAGING, LOG_NORMAL)(
            "Page access denied: cpl=%i, %x:%x:%x:%x", (int)cpu.cpl,
            entry.block.us, table.block.us, entry.block.wr, table.block.wr);
        paging.cr2 = lin_addr;
        cpu.exception.which = EXCEPTION_PF;
...
void PAGING_Enable(bool enabled) {
  ...
  if (auto_determine_dynamic_core_paging) {
    ...
    if (coretype)
      use_dynamic_core_with_paging = coretype == 1
                                         ? enabled && dos_kernel_disabled
                                         : enabled && !dos_kernel_disabled;
  }
  if (enabled) {
...
bool PAGING_Enabled(void) { return paging.enabled; }
...
void PAGING_Init() {
  ...
  LOG(LOG_MISC, LOG_DEBUG)(
      "Initializing paging system (CPU linear -> physical mapping system)");
...
uint16_t PageHandler_HostPtReadW(PageHandler *p, PhysPt addr) {
  if ((addr & 0xFFF) < 0xFFF) {
    return host_readw(
        &(p->GetHostReadPt(PAGING_GetPhysicalPageNumber(addr))[addr & 0xFFF]));
  } else {
...
void PageHandler_HostPtWriteW(PageHandler *p, PhysPt addr, uint16_t val) {
  if ((addr & 0xFFF) < 0xFFF) {
    host_writew(
        &(p->GetHostWritePt(PAGING_GetPhysicalPageNumber(addr))[addr & 0xFFF]),
        val);
  } else {
