...
/*
        The functions in this file are called almost exclusively by
   decoder.h, they translate an (or a set of) instruction(s) into hostspecific
   code and use the lower-level functions from decoder_basic.h and generating
...
static void dyn_dop_ebgb(DualOps op) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP2, decode.modrm.reg & 3,
                                            ((decode.modrm.reg >> 2) & 1));
    ...
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP1, decode.modrm.rm & 3,
                                            ((decode.modrm.rm >> 2) & 1));
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP2, decode.modrm.reg & 3,
                                            ((decode.modrm.reg >> 2) & 1));
    dyn_dop_byte_gencall(op);
    if ((op != DOP_CMP) && (op != DOP_TEST))
      MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_RETOP, decode.modrm.rm & 3,
                                     ((decode.modrm.rm >> 2) & 1));
  }
}
...
static void dyn_dop_ebgb_mov(void) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea(FC_ADDR);
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.reg & 3,
                                 ((decode.modrm.reg >> 2) & 1));
    dyn_write_byte(FC_ADDR, FC_TMP_BA1);
  } else {
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.reg & 3,
                                 ((decode.modrm.reg >> 2) & 1));
    MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.rm & 3,
                                   ((decode.modrm.rm >> 2) & 1));
  }
}
...
static void dyn_dop_ebgb_xchg(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_TMP_BA2, decode.modrm.reg & 3,
                                 ((decode.modrm.reg >> 2) & 1));
    ...
    MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.reg & 3,
                                   ((decode.modrm.reg >> 2) & 1));
  } else {
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.rm & 3,
                                 ((decode.modrm.rm >> 2) & 1));
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_TMP_BA2, decode.modrm.reg & 3,
                                 ((decode.modrm.reg >> 2) & 1));
    MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_TMP_BA1, decode.modrm.reg & 3,
                                   ((decode.modrm.reg >> 2) & 1));
    MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_TMP_BA2, decode.modrm.rm & 3,
                                   ((decode.modrm.rm >> 2) & 1));
  }
}
...
static void dyn_dop_gbeb(DualOps op) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP1, decode.modrm.reg & 3,
                                            ((decode.modrm.reg >> 2) & 1));
    dyn_dop_byte_gencall(op);
    if ((op != DOP_CMP) && (op != DOP_TEST))
      MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_RETOP, decode.modrm.reg & 3,
                                     ((decode.modrm.reg >> 2) & 1));
  } else {
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP2, decode.modrm.rm & 3,
                                            ((decode.modrm.rm >> 2) & 1));
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP1, decode.modrm.reg & 3,
                                            ((decode.modrm.reg >> 2) & 1));
    dyn_dop_byte_gencall(op);
...
static void dyn_dop_eviv_mov(void) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea(FC_ADDR);
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP1, decode_fetchd());
    else
      gen_mov_word_to_reg_imm(FC_OP1, decode_fetchw());
    dyn_write_word(FC_ADDR, FC_OP1, decode.big_op);
  } else {
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP1, decode_fetchd());
...
static void dyn_dop_byte_imm(DualOps op, uint8_t reg, uint8_t idx) {
  ...
  dyn_dop_byte_gencall(op);
  if ((op != DOP_CMP) && (op != DOP_TEST))
    MOV_REG_BYTE_FROM_HOST_REG_LOW(FC_RETOP, reg, idx);
}
...
static void dyn_prep_word_imm(uint8_t reg) {
  ...
  if (decode.big_op)
    gen_mov_dword_to_reg_imm(FC_OP2, (uint32_t)val);
  else
    gen_mov_word_to_reg_imm(FC_OP2, (uint16_t)val);
}
...
static void dyn_dop_word_imm_old(DualOps op, uint8_t reg, Bitu imm) {
  ...
  if (decode.big_op)
    ...
    gen_mov_word_to_reg_imm(FC_OP2, (uint16_t)imm);
  dyn_dop_word_gencall(op, decode.big_op);
  if ((op != DOP_CMP) && (op != DOP_TEST))
    MOV_REG_WORD_FROM_HOST_REG(FC_RETOP, reg, decode.big_op);
}
...
static void dyn_mov_word_imm(uint8_t reg) {
  ...
  if (decode.big_op)
    gen_mov_dword_to_reg_imm(FC_OP1, (uint32_t)val);
  else
    gen_mov_word_to_reg_imm(FC_OP1, (uint16_t)val);
  MOV_REG_WORD_FROM_HOST_REG(FC_OP1, reg, decode.big_op);
}
...
static void dyn_mov_byte_al_direct(uint32_t imm) {
  MOV_SEG_PHYS_TO_HOST_REG(
      FC_ADDR, (decode.seg_prefix_used ? decode.seg_prefix : DRC_SEG_DS));
...
static void dyn_pop_reg(uint8_t reg) {
  if (decode.big_op)
    gen_call_function_raw(dynrec_pop_dword);
  else
...
static void dyn_segprefix(uint8_t seg) {
  //	if (GCC_UNLIKELY(decode.seg_prefix_used))
  //IllegalOptionDynrec("dyn_segprefix");
...
static void dyn_mov_seg_ev(void) {
  dyn_get_modrm();
  if (GCC_UNLIKELY(decode.modrm.reg == DRC_SEG_CS))
...
static void dyn_imul_gvev(Bitu immsize) {
  ...
  case 1:
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP2, (int8_t)decode_fetchb());
    else
      gen_mov_word_to_reg_imm(FC_OP2, (int8_t)decode_fetchb());
    break;
  case 2:
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP2, (int16_t)decode_fetchw());
    else
      gen_mov_word_to_reg_imm(FC_OP2, (int16_t)decode_fetchw());
    break;
  case 4:
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP2, (int32_t)decode_fetchd());
    else
      gen_mov_word_to_reg_imm(FC_OP2, (uint16_t)((int32_t)decode_fetchd()));
    break;
  }

  if (decode.big_op)
    gen_call_function_raw(dynrec_dimul_dword);
  else
    gen_call_function_raw(dynrec_dimul_word);
...
static void dyn_dshift_ev_gv(bool left, bool immediate) {
  ...
  MOV_REG_WORD_TO_HOST_REG(FC_OP2, decode.modrm.reg, decode.big_op);
  if (immediate)
    gen_mov_byte_to_reg_low_imm(FC_OP3, decode_fetchb());
  else
    MOV_REG_BYTE_TO_HOST_REG_LOW(FC_OP3, DRC_REG_ECX, 0);
  if (decode.big_op)
    dyn_dpshift_dword_gencall(left);
  else
    dyn_dpshift_word_gencall(left);
...
static void dyn_grp1_ev_iv(bool withbyte) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    if (!withbyte) {
      ...
      Bits imm = withbyte ? (int8_t)decode_fetchb()
...
static void dyn_grp3_eb(void) {
  ...
  if (decode.modrm.mod < 3) {
    dyn_fill_ea(FC_ADDR);
    if ((decode.modrm.reg == 2) || (decode.modrm.reg == 3))
      gen_protect_addr_reg();
    dyn_read_byte_canuseword(FC_ADDR, FC_OP1);
  } else {
    MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(FC_OP1, decode.modrm.rm & 3,
...
static void dyn_grp3_ev(void) {
  ...
  case 0x0: // test ev,iv
    if (decode.big_op)
      gen_mov_dword_to_reg_imm(FC_OP2, decode_fetchd());
    else
      gen_mov_word_to_reg_imm(FC_OP2, decode_fetchw());
  ...
  case 0x4: // mul Eb
    if (decode.big_op)
      gen_call_function_raw(dynrec_mul_dword);
    else
      gen_call_function_raw(dynrec_mul_word);
    return;
  case 0x5: // imul Eb
    if (decode.big_op)
      gen_call_function_raw(dynrec_imul_dword);
    else
      gen_call_function_raw(dynrec_imul_word);
    return;
  case 0x6: // div Eb
    if (decode.big_op)
      gen_call_function_raw(dynrec_div_dword);
    else
      gen_call_function_raw(dynrec_div_word);
    dyn_check_exception(FC_RETOP);
    return;
  case 0x7: // idiv Eb
    if (decode.big_op)
      gen_call_function_raw(dynrec_idiv_dword);
    else
      gen_call_function_raw(dynrec_idiv_word);
    dyn_check_exception(FC_RETOP);
    return;
  }
  // Save the result if memory op
...
static Bitu dyn_grp4_ev(void) {
  ...
  case 0x0: // INC Ev
  case 0x1: // DEC Ev
    dyn_sop_word_gencall(decode.modrm.reg == 0 ? SOP_INC : SOP_DEC,
                         decode.big_op);
    if (decode.modrm.mod < 3) {
      gen_restore_addr_reg();
      dyn_write_word(FC_ADDR, FC_RETOP, decode.big_op);
  ...
  case 0x2: // CALL Ev
    ...
    gen_protect_addr_reg();
    gen_mov_word_to_reg(FC_OP1,
                        decode.big_op ? (void *)(&reg_eip) : (void *)(&reg_ip),
                        decode.big_op);
    ...
    gen_mov_word_from_reg(
        FC_ADDR, decode.big_op ? (void *)(&reg_eip) : (void *)(&reg_ip),
        decode.big_op);
    return 1;
  case 0x4: // JMP Ev
    gen_mov_word_from_reg(
        FC_OP1, decode.big_op ? (void *)(&reg_eip) : (void *)(&reg_ip),
        decode.big_op);
    return 1;
  case 0x3: // CALL Ep
  case 0x5: // JMP Ep
...
static bool dyn_grp6(void) {
  ...
  case 0x00: // SLDT
  case 0x01: // STR
    if (decode.modrm.reg == 0)
      gen_call_function_raw(CPU_SLDT);
    else
      gen_call_function_raw(CPU_STR);
    if (decode.modrm.mod < 3) {
      dyn_fill_ea(FC_ADDR);
  ...
  case 0x05: // VERW
    ...
    case 0x02: // LLDT
               //					if (cpu.cpl) return
               //CPU_PrepareException(EXCEPTION_GP,0);
      if (cpu.cpl)
        E_Exit("lldt cpl>0");
...
static bool dyn_grp7(void) {
  ...
  if (decode.modrm.mod < 3) {
    ...
    case 0x03: // LIDT
      ...
      if (cpu.pmode && cpu.cpl)
        IllegalOptionDynrec("lgdt nonprivileged");
      dyn_fill_ea(FC_ADDR);
      gen_protect_addr_reg();
      dyn_read_word(FC_ADDR, FC_OP1, false);
      gen_extend_word(false, FC_OP1);
      ...
      if (!decode.big_op)
        gen_and_imm(FC_OP2, 0xffffff);

      gen_restore_reg(FC_OP1);
      if (decode.modrm.reg == 2)
        gen_call_function_RR(CPU_LGDT, FC_OP1, FC_OP2);
      else
        gen_call_function_RR(CPU_LIDT, FC_OP1, FC_OP2);
      break;
    case 0x04: // SMSW
    ...
    case 0x07: // INVLPG
               ...
               //EXCEPTION(EXCEPTION_GP);
      if (cpu.pmode && cpu.cpl)
        IllegalOptionDynrec("invlpg nonprivileged");
      gen_call_function_raw(PAGING_ClearTLB);
      break;
    default:
      IllegalOptionDynrec("dyn_grp7_1");
    }
  } else {
    switch (decode.modrm.reg) {
    case 0x04: // SMSW
    ...
    case 0x06: // LMSW
      ...
      return true;
    default:
      IllegalOptionDynrec("dyn_grp7_2");
    }
  }
  return false;
}
...
static void dyn_larlsl(bool is_lar) {
        ...
        gen_extend_word(false,FC_RETOP);
        if (is_lar)
gen_call_function((void*)CPU_LAR,"%R%A",FC_RETOP,(DRC_PTR_SIZE_IM)&core_dynrec.readdata);
        else
gen_call_function((void*)CPU_LSL,"%R%A",FC_RETOP,(DRC_PTR_SIZE_IM)&core_dynrec.readdata);
...
static void dyn_mov_from_crx(void) {
  dyn_get_modrm();
  gen_call_function_IA(CPU_READ_CRX, decode.modrm.reg,
                       (DRC_PTR_SIZE_IM)&core_dynrec.readdata);
  dyn_check_exception(FC_RETOP);
  gen_mov_word_to_reg(FC_OP2, &core_dynrec.readdata, true);
...
static void dyn_exit_link(int32_t eip_change) {
  gen_add_direct_word(&reg_eip, (decode.code - decode.code_start) + eip_change,
                      decode.big_op);
  dyn_reduce_cycles();
  gen_jmp_ptr(&decode.block->link[0].to,
              offsetof(CacheBlockDynRec, cache.xstart));
  dyn_closeblock();
}
...
static void dyn_branched_exit(BranchTypes btype, int32_t eip_add) {
  ...
  gen_jmp_ptr(&decode.block->link[1].to,
              offsetof(CacheBlockDynRec, cache.xstart));
  dyn_closeblock();
}
...
static void dyn_ret_near(uint16_t bytes) {
  ...
  gen_mov_word_from_reg(
      FC_RETOP, decode.big_op ? (void *)(&reg_eip) : (void *)(&reg_ip), true);

  if (bytes)
    gen_add_direct_word(&reg_esp, bytes, true);
  dyn_return(BR_Normal);
  dyn_closeblock();
}
...
static void dyn_string(StringOps op) {
  ...
  uint8_t di_base_addr =
      decode.seg_prefix_used ? decode.seg_prefix : DRC_SEG_DS;
  switch (op) {
  case STR_MOVSB:
    if (decode.big_addr)
      gen_call_function_mm(dynrec_movsb_dword,
                           (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_mm(dynrec_movsb_word, (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;
  case STR_MOVSW:
    if (decode.big_addr)
      gen_call_function_mm(dynrec_movsw_dword,
                           (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_mm(dynrec_movsw_word, (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;
  case STR_MOVSD:
    if (decode.big_addr)
      gen_call_function_mm(dynrec_movsd_dword,
                           (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_mm(dynrec_movsd_word, (Bitu)DRCD_SEG_PHYS(di_base_addr),
                           (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;

  case STR_LODSB:
    if (decode.big_addr)
      gen_call_function_m(dynrec_lodsb_dword,
                          (Bitu)DRCD_SEG_PHYS(di_base_addr));
    else
      gen_call_function_m(dynrec_lodsb_word, (Bitu)DRCD_SEG_PHYS(di_base_addr));
    break;
  case STR_LODSW:
    if (decode.big_addr)
      gen_call_function_m(dynrec_lodsw_dword,
                          (Bitu)DRCD_SEG_PHYS(di_base_addr));
    else
      gen_call_function_m(dynrec_lodsw_word, (Bitu)DRCD_SEG_PHYS(di_base_addr));
    break;
  case STR_LODSD:
    if (decode.big_addr)
      gen_call_function_m(dynrec_lodsd_dword,
                          (Bitu)DRCD_SEG_PHYS(di_base_addr));
    else
      gen_call_function_m(dynrec_lodsd_word, (Bitu)DRCD_SEG_PHYS(di_base_addr));
    break;

  case STR_STOSB:
    if (decode.big_addr)
      gen_call_function_m(dynrec_stosb_dword, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_m(dynrec_stosb_word, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;
  case STR_STOSW:
    if (decode.big_addr)
      gen_call_function_m(dynrec_stosw_dword, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_m(dynrec_stosw_word, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;
  case STR_STOSD:
    if (decode.big_addr)
      gen_call_function_m(dynrec_stosd_dword, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    else
      gen_call_function_m(dynrec_stosd_word, (Bitu)DRCD_SEG_PHYS(DRC_SEG_ES));
    break;
  default:
    IllegalOptionDynrec("dyn_string");
  }
  if (decode.rep)
  ...
  if (op < STR_SCASB) {
    ...
    save_info_dynrec[used_save_info_dynrec].branch_pos =
        gen_create_branch_long_nonzero(FC_RETOP, true);
...
static void dyn_leave(void) {
  if (decode.big_op)
    gen_call_function_raw(dynrec_leave_dword);
  else
    gen_call_function_raw(dynrec_leave_word);
}
