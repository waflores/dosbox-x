...
enum SingleOps { SOP_INC, SOP_DEC, SOP_NOT, SOP_NEG };
...
// rotate operand type
enum grp2_types { grp2_1, grp2_imm, grp2_cl };
...
static struct DynDecodeDynRec {
  ...
  struct {
    ...
    uint8_t *wmap;   // write map that indicates code presence for every byte of
                     // this page
...
static void INLINE decode_increase_wmapmask(Bitu size) {
  ...
  if (GCC_UNLIKELY(!activecb->cache.wmapmask)) {
    ...
    activecb->cache.maskstart =
        (uint16_t)decode.page.index; // start of buffer is current code position
...
#define MOV_SEG_VAL_TO_HOST_REG(host_reg, seg_index)                           \
  gen_mov_seg16_to_reg(host_reg, (DRC_PTR_SIZE_IM)(DRCD_SEG_VAL(seg_index)) -  \
                                     (DRC_PTR_SIZE_IM)(&Segs))

#define MOV_SEG_PHYS_TO_HOST_REG(host_reg, seg_index)                          \
  gen_mov_seg32_to_reg(host_reg, (DRC_PTR_SIZE_IM)(DRCD_SEG_PHYS(seg_index)) - \
                                     (DRC_PTR_SIZE_IM)(&Segs))
#define ADD_SEG_PHYS_TO_HOST_REG(host_reg, seg_index)                          \
  gen_add_seg32_to_reg(host_reg, (DRC_PTR_SIZE_IM)(DRCD_SEG_PHYS(seg_index)) - \
                                     (DRC_PTR_SIZE_IM)(&Segs))

#else

#define MOV_SEG_VAL_TO_HOST_REG(host_reg, seg_index)                           \
  gen_mov_word_to_reg(host_reg, DRCD_SEG_VAL(seg_index), false)

#define MOV_SEG_PHYS_TO_HOST_REG(host_reg, seg_index)                          \
  gen_mov_word_to_reg(host_reg, DRCD_SEG_PHYS(seg_index), true)
#define ADD_SEG_PHYS_TO_HOST_REG(host_reg, seg_index)                          \
  gen_add(host_reg, DRCD_SEG_PHYS(seg_index))

#endif

#ifdef DRC_USE_REGS_ADDR

#define MOV_REG_VAL_TO_HOST_REG(host_reg, reg_index)                           \
  gen_mov_regval32_to_reg(host_reg,                                            \
                          (DRC_PTR_SIZE_IM)(DRCD_REG_VAL(reg_index)) -         \
                              (DRC_PTR_SIZE_IM)(&cpu_regs))
#define ADD_REG_VAL_TO_HOST_REG(host_reg, reg_index)                           \
  gen_add_regval32_to_reg(host_reg,                                            \
                          (DRC_PTR_SIZE_IM)(DRCD_REG_VAL(reg_index)) -         \
                              (DRC_PTR_SIZE_IM)(&cpu_regs))

#define MOV_REG_WORD16_TO_HOST_REG(host_reg, reg_index)                        \
  gen_mov_regval16_to_reg(host_reg,                                            \
                          (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, false)) - \
                              (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_WORD32_TO_HOST_REG(host_reg, reg_index)                        \
  gen_mov_regval32_to_reg(host_reg,                                            \
                          (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, true)) -  \
                              (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_WORD_TO_HOST_REG(host_reg, reg_index, dword)                   \
  gen_mov_regword_to_reg(host_reg,                                             \
                         (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, dword)) -  \
                             (DRC_PTR_SIZE_IM)(&cpu_regs),                     \
                         dword)

#define MOV_REG_WORD16_FROM_HOST_REG(host_reg, reg_index)                      \
  gen_mov_regval16_from_reg(                                                   \
      host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, false)) -           \
                    (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_WORD32_FROM_HOST_REG(host_reg, reg_index)                      \
  gen_mov_regval32_from_reg(                                                   \
      host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, true)) -            \
                    (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_WORD_FROM_HOST_REG(host_reg, reg_index, dword)                 \
  gen_mov_regword_from_reg(                                                    \
      host_reg,                                                                \
      (DRC_PTR_SIZE_IM)(DRCD_REG_WORD(reg_index, dword)) -                     \
          (DRC_PTR_SIZE_IM)(&cpu_regs),                                        \
      dword)

#define MOV_REG_BYTE_TO_HOST_REG_LOW(host_reg, reg_index, high_byte)           \
  gen_mov_regbyte_to_reg_low(                                                  \
      host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_BYTE(reg_index, high_byte)) -       \
                    (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(host_reg, reg_index,           \
                                                high_byte)                     \
  gen_mov_regbyte_to_reg_low_canuseword(                                       \
      host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_BYTE(reg_index, high_byte)) -       \
                    (DRC_PTR_SIZE_IM)(&cpu_regs))
#define MOV_REG_BYTE_FROM_HOST_REG_LOW(host_reg, reg_index, high_byte)         \
  gen_mov_regbyte_from_reg_low(                                                \
      host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_BYTE(reg_index, high_byte)) -       \
                    (DRC_PTR_SIZE_IM)(&cpu_regs))

#define MOV_FLAGS_TO_HOST_REG(host_reg)                                        \
  gen_mov_regval32_to_reg(host_reg, (DRC_PTR_SIZE_IM)(DRCD_REG_FLAGS) -        \
                                        (DRC_PTR_SIZE_IM)(&cpu_regs))

#else

#define MOV_REG_VAL_TO_HOST_REG(host_reg, reg_index)                           \
  gen_mov_word_to_reg(host_reg, DRCD_REG_VAL(reg_index), true)
#define ADD_REG_VAL_TO_HOST_REG(host_reg, reg_index)                           \
  gen_add(host_reg, DRCD_REG_VAL(reg_index))

#define MOV_REG_WORD16_TO_HOST_REG(host_reg, reg_index)                        \
  gen_mov_word_to_reg(host_reg, DRCD_REG_WORD(reg_index, false), false)
#define MOV_REG_WORD32_TO_HOST_REG(host_reg, reg_index)                        \
  gen_mov_word_to_reg(host_reg, DRCD_REG_WORD(reg_index, true), true)
#define MOV_REG_WORD_TO_HOST_REG(host_reg, reg_index, dword)                   \
  gen_mov_word_to_reg(host_reg, DRCD_REG_WORD(reg_index, dword), dword)

#define MOV_REG_WORD16_FROM_HOST_REG(host_reg, reg_index)                      \
  gen_mov_word_from_reg(host_reg, DRCD_REG_WORD(reg_index, false), false)
#define MOV_REG_WORD32_FROM_HOST_REG(host_reg, reg_index)                      \
  gen_mov_word_from_reg(host_reg, DRCD_REG_WORD(reg_index, true), true)
#define MOV_REG_WORD_FROM_HOST_REG(host_reg, reg_index, dword)                 \
  gen_mov_word_from_reg(host_reg, DRCD_REG_WORD(reg_index, dword), dword)

#define MOV_REG_BYTE_TO_HOST_REG_LOW(host_reg, reg_index, high_byte)           \
  gen_mov_byte_to_reg_low(host_reg, DRCD_REG_BYTE(reg_index, high_byte))
#define MOV_REG_BYTE_TO_HOST_REG_LOW_CANUSEWORD(host_reg, reg_index,           \
                                                high_byte)                     \
  gen_mov_byte_to_reg_low_canuseword(host_reg,                                 \
                                     DRCD_REG_BYTE(reg_index, high_byte))
#define MOV_REG_BYTE_FROM_HOST_REG_LOW(host_reg, reg_index, high_byte)         \
  gen_mov_byte_from_reg_low(host_reg, DRCD_REG_BYTE(reg_index, high_byte))

#define MOV_FLAGS_TO_HOST_REG(host_reg)                                        \
  gen_mov_word_to_reg(host_reg, DRCD_REG_FLAGS, true)

#endif

#define DYN_LEA_MEM_MEM(ea_reg, op1, op2, scale, imm)                          \
  dyn_lea_mem_mem(ea_reg, op1, op2, scale, imm)

#if defined(DRC_USE_REGS_ADDR) && defined(DRC_USE_SEGS_ADDR)

#define DYN_LEA_SEG_PHYS_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)     \
  dyn_lea_segphys_regval(ea_reg, op1_index, op2_index, scale, imm)
#define DYN_LEA_REG_VAL_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)      \
  dyn_lea_regval_regval(ea_reg, op1_index, op2_index, scale, imm)
#define DYN_LEA_MEM_REG_VAL(ea_reg, op1, op2_index, scale, imm)                \
  dyn_lea_mem_regval(ea_reg, op1, op2_index, scale, imm)

#elif defined(DRC_USE_REGS_ADDR)

#define DYN_LEA_SEG_PHYS_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)     \
  dyn_lea_mem_regval(ea_reg, DRCD_SEG_PHYS(op1_index), op2_index, scale, imm)
#define DYN_LEA_REG_VAL_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)      \
  dyn_lea_regval_regval(ea_reg, op1_index, op2_index, scale, imm)
#define DYN_LEA_MEM_REG_VAL(ea_reg, op1, op2_index, scale, imm)                \
  dyn_lea_mem_regval(ea_reg, op1, op2_index, scale, imm)
...
#define DYN_LEA_SEG_PHYS_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)     \
  dyn_lea_segphys_mem(ea_reg, op1_index, DRCD_REG_VAL(op2_index), scale, imm)
#define DYN_LEA_REG_VAL_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)      \
  dyn_lea_mem_mem(ea_reg, DRCD_REG_VAL(op1_index), DRCD_REG_VAL(op2_index),    \
                  scale, imm)
#define DYN_LEA_MEM_REG_VAL(ea_reg, op1, op2_index, scale, imm)                \
  dyn_lea_mem_mem(ea_reg, op1, DRCD_REG_VAL(op2_index), scale, imm)

#else

#define DYN_LEA_SEG_PHYS_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)     \
  dyn_lea_mem_mem(ea_reg, DRCD_SEG_PHYS(op1_index), DRCD_REG_VAL(op2_index),   \
                  scale, imm)
#define DYN_LEA_REG_VAL_REG_VAL(ea_reg, op1_index, op2_index, scale, imm)      \
  dyn_lea_mem_mem(ea_reg, DRCD_REG_VAL(op1_index), DRCD_REG_VAL(op2_index),    \
                  scale, imm)
...
static INLINE void dyn_set_eip_last_end(HostReg reg) {
  ...
  gen_add_direct_word(&reg_eip, decode.op_start - decode.code_start,
                      decode.big_op);
}
...
static INLINE void dyn_set_eip_end(HostReg reg, uint32_t imm = 0) {
  gen_mov_word_to_reg(reg, &reg_eip,
                      true); // get_extend_word will mask off the upper bits
  // gen_mov_word_to_reg(reg,&reg_eip,decode.big_op);
  gen_add_imm(reg, (uint32_t)(decode.code - decode.code_start + imm));
  if (!decode.big_op)
    gen_extend_word(false, reg);
}
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_R(const T func, Bitu op) {
  gen_load_param_reg(op, 0);
  return gen_call_function_setup(func, 1);
}

template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_R3(const T func, Bitu op) {
  gen_load_param_reg(op, 2);
  return gen_call_function_setup(func, 3, true);
}

template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_RI(const T func, Bitu op1,
                                                   Bitu op2) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_RA(const T func, Bitu op1,
                                                   DRC_PTR_SIZE_IM op2) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_RR(const T func, Bitu op1,
                                                   Bitu op2) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_IR(const T func, Bitu op1,
                                                   Bitu op2) {
  gen_load_param_reg(op2, 1);
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_I(const T func, Bitu op) {
  gen_load_param_imm(op, 0);
  return gen_call_function_setup(func, 1);
}

template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_II(const T func, Bitu op1,
                                                   Bitu op2) {
  gen_load_param_imm(op2, 1);
  gen_load_param_imm(op1, 0);
  return gen_call_function_setup(func, 2);
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_III(const T func, Bitu op1,
                                                    Bitu op2, Bitu op3) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_IA(const T func, Bitu op1,
                                                   DRC_PTR_SIZE_IM op2) {
  gen_load_param_addr(op2, 1);
  gen_load_param_imm(op1, 0);
  return gen_call_function_setup(func, 2);
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_IIR(const T func, Bitu op1,
                                                    Bitu op2, Bitu op3) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_IIIR(const T func, Bitu op1,
                                                     Bitu op2, Bitu op3,
                                                     Bitu op4) {
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_IRRR(const T func, Bitu op1,
                                                     Bitu op2, Bitu op3,
                                                     Bitu op4) {
  gen_load_param_reg(op4, 3);
  gen_load_param_reg(op3, 2);
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_m(const T func, Bitu op) {
  gen_load_param_mem(op, 2);
  return gen_call_function_setup(func, 3, true);
}

template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_mm(const T func, Bitu op1,
                                                   Bitu op2) {
...
static void dyn_fill_blocks(void) {
  for (Bitu sct = 0; sct < used_save_info_dynrec; sct++) {
    ...
    case db_exception:
      ...
      if (cpu.code.big)
        gen_call_function_II(DynRunException, save_info_dynrec[sct].eip_change,
                             save_info_dynrec[sct].cycles);
      else
        gen_call_function_II(DynRunException,
                             save_info_dynrec[sct].eip_change & 0xffff,
                             save_info_dynrec[sct].cycles);
    ...
    case string_break:
      ...
      gen_add_direct_word(&reg_eip, save_info_dynrec[sct].eip_change,
                          decode.big_op);
      dyn_return(BR_Cycles);
      break;
    case trap:
...
static void dyn_check_exception(HostReg reg) {
  save_info_dynrec[used_save_info_dynrec].branch_pos =
      gen_create_branch_long_nonzero(reg, false);
  if (!decode.cycles)
    decode.cycles++;
  save_info_dynrec[used_save_info_dynrec].cycles = decode.cycles;
  // in case of an exception eip will point to the start of the current
  // instruction
...
static void dyn_check_trapflag(void) {
  ...
  save_info_dynrec[used_save_info_dynrec].branch_pos =
      gen_create_branch_long_nonzero(FC_OP1, true);
  save_info_dynrec[used_save_info_dynrec].cycles = decode.cycles;
  save_info_dynrec[used_save_info_dynrec].eip_change =
  ...
  if (tlb_addr) {
    *((uint8_t *)(&core_dynrec.readdata)) = host_readb(tlb_addr + address);
    ...
    return get_tlb_readhandler(address)->readb_checked(
        address, (uint8_t *)(&core_dynrec.readdata));
  }
}
    ...
    if (tlb_addr) {
      *((uint16_t *)(&core_dynrec.readdata)) = host_readw(tlb_addr + address);
      ...
      return get_tlb_readhandler(address)->readw_checked(
          address, (uint16_t *)(&core_dynrec.readdata));
  } else
    return mem_unalignedreadw_checked(address,
                                      ((uint16_t *)(&core_dynrec.readdata)));
}
    ...
    if (tlb_addr) {
      *((uint32_t *)(&core_dynrec.readdata)) = host_readd(tlb_addr + address);
      ...
      return get_tlb_readhandler(address)->readd_checked(
          address, (uint32_t *)(&core_dynrec.readdata));
  } else
    return mem_unalignedreadd_checked(address,
...
static void dyn_read_word(HostReg reg_addr, HostReg reg_dst, bool dword) {
  gen_mov_regs(FC_OP1, reg_addr);
  if (dword)
    gen_call_function_raw(mem_readd_checked_drc);
  else
    gen_call_function_raw(mem_readw_checked_drc);
  dyn_check_exception(FC_RETOP);
...
static void dyn_write_word(HostReg reg_addr, HostReg reg_val, bool dword) {
  ...
  gen_mov_regs(FC_OP1, reg_addr);
  if (dword)
    gen_call_function_raw(mem_writed_checked_drc);
  else
    gen_call_function_raw(mem_writew_checked_drc);
  dyn_check_exception(FC_RETOP);
}
...
static void dyn_lea_mem_mem(HostReg ea_reg, void *op1, void *op2, Bitu scale,
                            Bits imm) {
  if (scale || imm) {
    if (op1 != NULL) {
...
static void dyn_lea_regval_regval(HostReg ea_reg, Bitu op1_index,
                                  Bitu op2_index, Bitu scale, Bits imm) {
  if (scale || imm) {
...
static void dyn_lea_mem_regval(HostReg ea_reg, void *op1, Bitu op2_index,
                               Bitu scale, Bits imm) {
  if (scale || imm) {
    if (op1 != NULL) {
      gen_mov_word_to_reg(ea_reg, op1, true);
...
static void dyn_lea_segphys_regval(HostReg ea_reg, Bitu op1_index,
                                   Bitu op2_index, Bitu scale, Bits imm) {
  if (scale || imm) {
...
static void dyn_lea_segphys_mem(HostReg ea_reg, Bitu op1_index, void *op2,
                                Bitu scale, Bits imm) {
  if (scale || imm) {
    MOV_SEG_PHYS_TO_HOST_REG(ea_reg, op1_index);
    gen_mov_word_to_reg(TEMP_REG_DRC, op2, true);
...
static void dyn_fill_ea(HostReg ea_reg, bool addseg = true) {
  ...
  skip_extend_word:
    if (addseg) {
      ...
      ADD_SEG_PHYS_TO_HOST_REG(
          ea_reg, (decode.seg_prefix_used ? decode.seg_prefix : seg_base));
    }
  } else {
    Bits imm = 0;
    uint8_t base_reg = 0;
    switch (decode.modrm.rm) {
    case 0:
      base_reg = DRC_REG_EAX;
      break;
    case 1:
      base_reg = DRC_REG_ECX;
      break;
    case 2:
      base_reg = DRC_REG_EDX;
      break;
    case 3:
      base_reg = DRC_REG_EBX;
      break;
    case 4: // SIB
    {
      Bitu sib = decode_fetchb();
      bool scaled_reg_used = false;
      static uint8_t scaledtable[8] = {DRC_REG_EAX, DRC_REG_ECX, DRC_REG_EDX,
                                       DRC_REG_EBX, 0,           DRC_REG_EBP,
                                       DRC_REG_ESI, DRC_REG_EDI};
      // see if scaling should be used and which register is to be scaled in
      // this case
      if (((sib >> 3) & 7) != 4)
        scaled_reg_used = true;
      ...
      case 3:
        ...
        break;
      case 4:
        base_reg = DRC_REG_ESP;
        seg_base = DRC_SEG_SS;
        break;
      case 5:
        if (decode.modrm.mod) {
          base_reg = DRC_REG_EBP;
          seg_base = DRC_SEG_SS;
        } else {
          ...
          if (decode_fetchd_imm(val)) {
            ...
            if (!addseg) {
              ...
              if (!scaled_reg_used) {
                MOV_SEG_PHYS_TO_HOST_REG(
                    ea_reg,
                    (decode.seg_prefix_used ? decode.seg_prefix : seg_base));
              } else {
                DYN_LEA_SEG_PHYS_REG_VAL(
                    ea_reg,
                    (decode.seg_prefix_used ? decode.seg_prefix : seg_base),
                    scaled_reg, scale, 0);
              }
        ...
        break;
      case 6:
        base_reg = DRC_REG_ESI;
        break;
      case 7:
        base_reg = DRC_REG_EDI;
        break;
      }
...
static void InitFlagsOptimization(void) { mf_functions_num = 0; }
...
static void InvalidateFlags(void) {
  ...
  for (Bitu ct = 0; ct < mf_functions_num; ct++) {
    gen_fill_function_ptr(mf_functions[ct].pos, mf_functions[ct].fct_ptr,
                          mf_functions[ct].ftype);
  }
  mf_functions_num = 0;
#endif
}
...
// the flags are not required before
template <typename T>
static void InvalidateFlags(const T current_simple_function, Bitu flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
  for (Bitu ct = 0; ct < mf_functions_num; ct++) {
    gen_fill_function_ptr(mf_functions[ct].pos, mf_functions[ct].fct_ptr,
                          mf_functions[ct].ftype);
  ...
  mf_functions[0].fct_ptr =
      reinterpret_cast<void *>((uintptr_t)current_simple_function);
...
template <typename T>
static void InvalidateFlagsPartially(const T current_simple_function,
                                     Bitu flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
  mf_functions[mf_functions_num].pos = cache.pos;
  mf_functions[mf_functions_num].fct_ptr =
      reinterpret_cast<void *>((uintptr_t)current_simple_function);
...
template <typename T>
static void InvalidateFlagsPartially(const T current_simple_function,
                                     DRC_PTR_SIZE_IM cpos, Bitu flags_type) {
#ifdef DRC_FLAGS_INVALIDATION
