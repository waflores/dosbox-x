...
// use FC_REGS_ADDR to hold the address of "cpu_regs" and to access it using
// FC_REGS_ADDR
#define DRC_USE_REGS_ADDR
// use FC_SEGS_ADDR to hold the address of "Segs" and to access it using
// FC_SEGS_ADDR
...
#define FC_ADDR                                                                \
  HOST_r19 // has to be saved across calls, see DRC_PROTECT_ADDR_REG
...
// special register that holds the third parameter for _R3 calls (byte
// accessible)
...
// used to hold the address of "cpu_regs" - preferably filled in function
// gen_run_code
#define FC_REGS_ADDR HOST_r20

// used to hold the address of "Segs" - preferably filled in function
// gen_run_code
#define FC_SEGS_ADDR HOST_r21

// used to hold the address of "core_dynrec.readdata" - filled in function
// gen_run_code
...
// movz dst, #(imm lsl simm)		@	0 <= imm <= 65535	&
// simm = 0/16
#define MOVZ(dst, imm, simm)                                                   \
  (0x52800000 + (dst) + ((imm) << 5) + ((simm) ? 0x00200000 : 0))
// movn dst, #(imm lsl simm)		@	0 <= imm <= 65535	&
// simm = 0/16
#define MOVN(dst, imm, simm)                                                   \
  (0x12800000 + (dst) + ((imm) << 5) + ((simm) ? 0x00200000 : 0))
// movk dst, #(imm lsl simm)		@	0 <= imm <= 65535	&
// simm = 0/16
#define MOVK(dst, imm, simm)                                                   \
  (0x72800000 + (dst) + ((imm) << 5) + ((simm) ? 0x00200000 : 0))
// movz dst, #(imm lsl simm)		@	0 <= imm <= 65535	&
// simm = 0/16/32/48
#define MOVZ64(dst, imm, simm)                                                 \
  (0xd2800000 + (dst) + ((imm) << 5) + (((simm) >> 4) << 21))
// movk dst, #(imm lsl simm)		@	0 <= imm <= 65535	&
// simm = 0/16/32/48
#define MOVK64(dst, imm, simm)                                                 \
  (0xf2800000 + (dst) + ((imm) << 5) + (((simm) >> 4) << 21))
// lslv dst, src, rreg
#define LSLV(dst, src, rreg)                                                   \
  (0x1ac02000 + (dst) + ((src) << 5) + ((rreg) << 16))
// lsrv dst, src, rreg
#define LSRV(dst, src, rreg)                                                   \
  (0x1ac02400 + (dst) + ((src) << 5) + ((rreg) << 16))
// asrv dst, src, rreg
#define ASRV(dst, src, rreg)                                                   \
  (0x1ac02800 + (dst) + ((src) << 5) + ((rreg) << 16))
// rorv dst, src, rreg
#define RORV(dst, src, rreg)                                                   \
  (0x1ac02c00 + (dst) + ((src) << 5) + ((rreg) << 16))
// lslv dst, src, rreg
#define LSLV64(dst, src, rreg)                                                 \
  (0x9ac02000 + (dst) + ((src) << 5) + ((rreg) << 16))
// lsrv dst, src, rreg
#define LSRV64(dst, src, rreg)                                                 \
  (0x9ac02400 + (dst) + ((src) << 5) + ((rreg) << 16))
// lsr dst, src, #imm
#define LSR64_IMM(dst, src, imm) UBFM64(dst, src, imm, 63)

// arithmetic
// add dst, src, #(imm lsl simm)		@	0 <= imm <= 4095
// &	simm = 0/12
#define ADD_IMM(dst, src, imm, simm)                                           \
  (0x11000000 + (dst) + ((src) << 5) + ((imm) << 10) +                         \
   ((simm) ? 0x00400000 : 0))
// add dst, src1, src2, lsl #imm
#define ADD_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x0b000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// sub dst, src, #(imm lsl simm)		@	0 <= imm <= 4095
// &	simm = 0/12
#define SUB_IMM(dst, src, imm, simm)                                           \
  (0x51000000 + (dst) + ((src) << 5) + ((imm) << 10) +                         \
   ((simm) ? 0x00400000 : 0))
// sub dst, src1, src2, lsl #imm
#define SUB_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x4b000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// cmp src, #(imm lsl simm)		@	0 <= imm <= 4095	&
// simm = 0/12
#define CMP_IMM(src, imm, simm)                                                \
  (0x7100001f + ((src) << 5) + ((imm) << 10) + ((simm) ? 0x00400000 : 0))
// nop
#define NOP (0xd503201f)

// logical
// and dst, src1, src2, lsl #imm		@	0 <= imm <= 31
#define AND_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x0a000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// orr dst, src1, src2, lsl #imm		@	0 <= imm <= 31
#define ORR_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x2a000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// eor dst, src1, src2, lsl #imm		@	0 <= imm <= 31
#define EOR_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x4a000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// bic dst, src1, src2, lsl #imm		@	0 <= imm <= 31
#define BIC_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0x0a200000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))
// and dst, src1, src2, lsl #imm		@	0 <= imm <= 63
#define AND64_REG_LSL_IMM(dst, src1, src2, imm)                                \
  (0x8a000000 + (dst) + ((src1) << 5) + ((src2) << 16) + ((imm) << 10))

// load
// ldr reg, [pc, #imm]		@	-1M <= imm < 1M	&	imm mod 4 = 0
#define LDR64_PC(reg, imm) (0x58000000 + (reg) + (((imm) << 3) & 0x00ffffe0))
// ldp reg1, reg2 [addr, #imm]		@	-512 <= imm < 512	&
// imm mod 8 = 0
#define LDP64_IMM(reg1, reg2, addr, imm)                                       \
  (0xa9400000 + (reg1) + ((reg2) << 10) + ((addr) << 5) + ((imm) << 12))
// ldr reg, [addr, #imm]		@	0 <= imm < 32768	&
// imm mod 8 = 0
#define LDR64_IMM(reg, addr, imm)                                              \
  (0xf9400000 + (reg) + ((addr) << 5) + ((imm) << 7))
// ldr reg, [addr, #imm]		@	0 <= imm < 16384	&
// imm mod 4 = 0
#define LDR_IMM(reg, addr, imm)                                                \
  (0xb9400000 + (reg) + ((addr) << 5) + ((imm) << 8))
// ldrh reg, [addr, #imm]		@	0 <= imm < 8192	&	imm mod
// 2 = 0
#define LDRH_IMM(reg, addr, imm)                                               \
  (0x79400000 + (reg) + ((addr) << 5) + ((imm) << 9))
// ldrb reg, [addr, #imm]		@	0 <= imm < 4096
#define LDRB_IMM(reg, addr, imm)                                               \
  (0x39400000 + (reg) + ((addr) << 5) + ((imm) << 10))
// ldr reg, [addr1, addr2, lsl #imm]		@	imm = 0/2
#define LDR64_REG_LSL_IMM(reg, addr1, addr2, imm)                              \
  (0xf8606800 + (reg) + ((addr1) << 5) + ((addr2) << 16) +                     \
   ((imm) ? 0x00001000 : 0))
// ldur reg, [addr, #imm]		@	-256 <= imm < 256
#define LDUR64_IMM(reg, addr, imm)                                             \
  (0xf8400000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// ldur reg, [addr, #imm]		@	-256 <= imm < 256
#define LDUR_IMM(reg, addr, imm)                                               \
  (0xb8400000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// ldurh reg, [addr, #imm]		@	-256 <= imm < 256
#define LDURH_IMM(reg, addr, imm)                                              \
  (0x78400000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// ldurb reg, [addr, #imm]		@	-256 <= imm < 256
#define LDURB_IMM(reg, addr, imm)                                              \
  (0x38400000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))

// store
// stp reg1, reg2 [addr, #imm]		@	-512 <= imm < 512	&
// imm mod 8 = 0
#define STP64_IMM(reg1, reg2, addr, imm)                                       \
  (0xa9000000 + (reg1) + ((reg2) << 10) + ((addr) << 5) + ((imm) << 12))
// str reg, [addr, #imm]		@	0 <= imm < 32768	&
// imm mod 8 = 0
#define STR64_IMM(reg, addr, imm)                                              \
  (0xf9000000 + (reg) + ((addr) << 5) + ((imm) << 7))
// str reg, [addr, #imm]		@	0 <= imm < 16384	&
// imm mod 4 = 0
#define STR_IMM(reg, addr, imm)                                                \
  (0xb9000000 + (reg) + ((addr) << 5) + ((imm) << 8))
// strh reg, [addr, #imm]		@	0 <= imm < 8192	&	imm mod
// 2 = 0
#define STRH_IMM(reg, addr, imm)                                               \
  (0x79000000 + (reg) + ((addr) << 5) + ((imm) << 9))
// strb reg, [addr, #imm]		@	0 <= imm < 4096
#define STRB_IMM(reg, addr, imm)                                               \
  (0x39000000 + (reg) + ((addr) << 5) + ((imm) << 10))
// stur reg, [addr, #imm]		@	-256 <= imm < 256
#define STUR64_IMM(reg, addr, imm)                                             \
  (0xf8000000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// stur reg, [addr, #imm]		@	-256 <= imm < 256
#define STUR_IMM(reg, addr, imm)                                               \
  (0xb8000000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// sturh reg, [addr, #imm]		@	-256 <= imm < 256
#define STURH_IMM(reg, addr, imm)                                              \
  (0x78000000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
// sturb reg, [addr, #imm]		@	-256 <= imm < 256
#define STURB_IMM(reg, addr, imm)                                              \
  (0x38000000 + (reg) + ((addr) << 5) + (((imm) << 12) & 0x001ff000))
...
// bit field
// bfi dst, src, #lsb, #width		@	lsb >= 0, width >= 1, lsb+width
// <= 32
#define BFI(dst, src, lsb, width)                                              \
  BFM(dst, src, (32 - (lsb)) & 0x1f, (width) - 1)
// bfm dst, src, #rimm, #simm		@	0 <= rimm < 32, 0 <= simm < 32
#define BFM(dst, src, rimm, simm)                                              \
  (0x33000000 + (dst) + ((src) << 5) + ((rimm) << 16) + ((simm) << 10))
// sbfm dst, src, #rimm, #simm		@	0 <= rimm < 32, 0 <= simm < 32
#define SBFM(dst, src, rimm, simm)                                             \
  (0x13000000 + (dst) + ((src) << 5) + ((rimm) << 16) + ((simm) << 10))
// ubfm dst, src, #rimm, #simm		@	0 <= rimm < 32, 0 <= simm < 32
#define UBFM(dst, src, rimm, simm)                                             \
  (0x53000000 + (dst) + ((src) << 5) + ((rimm) << 16) + ((simm) << 10))
// bfi dst, src, #lsb, #width		@	lsb >= 0, width >= 1, lsb+width
// <= 64
#define BFI64(dst, src, lsb, width)                                            \
  BFM64(dst, src, (64 - (lsb)) & 0x3f, (width) - 1)
// bfm dst, src, #rimm, #simm		@	0 <= rimm < 64, 0 <= simm < 64
#define BFM64(dst, src, rimm, simm)                                            \
  (0xb3400000 + (dst) + ((src) << 5) + ((rimm) << 16) + ((simm) << 10))
// ubfm dst, src, #rimm, #simm		@	0 <= rimm < 64, 0 <= simm < 64
#define UBFM64(dst, src, rimm, simm)                                           \
  (0xd3400000 + (dst) + ((src) << 5) + ((rimm) << 16) + ((simm) << 10))
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg, uint32_t imm) {
  if ((imm & 0xffff0000) == 0) {
    ...
    cache_addd(
        MOVZ(dest_reg, imm >> 16, 16)); // movz dest_reg, #(imm >> 16), lsl #16
  } else if (((~imm) & 0xffff0000) == 0) {
    cache_addd(MOVN(dest_reg, ~imm, 0)); // movn dest_reg, #(~imm)
  } else if (((~imm) & 0x0000ffff) == 0) {
    cache_addd(MOVN(dest_reg, (~imm) >> 16,
                    16)); // movn dest_reg, #(~imm >> 16), lsl #16
  } else {
    cache_addd(
        MOVZ(dest_reg, imm & 0xffff, 0)); // movz dest_reg, #(imm & 0xffff)
    cache_addd(
        MOVK(dest_reg, imm >> 16, 16)); // movk dest_reg, #(imm >> 16), lsl #16
  }
}

// helper function
static bool gen_mov_memval_to_reg_helper(HostReg dest_reg, uint64_t data,
                                         Bitu size, HostReg addr_reg,
                                         uint64_t addr_data) {
  switch (size) {
  case 8:
    if (((data & 7) == 0) && (data >= addr_data) &&
        (data < addr_data + 32768)) {
      cache_addd(LDR64_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldr dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data + 256) && (data >= addr_data - 256)) {
      cache_addd(LDUR64_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldur dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
    break;
  case 4:
    if (((data & 3) == 0) && (data >= addr_data) &&
        (data < addr_data + 16384)) {
      cache_addd(LDR_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldr dest_reg, [addr_reg, #(data - addr_data)]
      return true;
  ...
  case 2:
    if (((data & 1) == 0) && (data >= addr_data) && (data < addr_data + 8192)) {
      cache_addd(LDRH_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldrh dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data + 256) && (data >= addr_data - 256)) {
      cache_addd(LDURH_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldurh dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 4096)) {
      cache_addd(LDRB_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldrb dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data) && (data >= addr_data - 256)) {
      cache_addd(LDURB_IMM(
          dest_reg, addr_reg,
          data - addr_data)); // ldurb dest_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
  default:
    break;
...
static bool gen_mov_memval_to_reg(HostReg dest_reg, void *data, Bitu size) {
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint64_t)data, size, FC_REGS_ADDR,
                                   (uint64_t)&cpu_regs))
    return true;
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint64_t)data, size,
                                   readdata_addr,
                                   (uint64_t)&core_dynrec.readdata))
    return true;
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint64_t)data, size, FC_SEGS_ADDR,
                                   (uint64_t)&Segs))
    return true;
  return false;
}
...
static void gen_mov_qword_to_reg_imm(HostReg dest_reg, uint64_t imm) {
  ...
  if ((imm & 0xffff) != 0) {
    cache_addd(
        MOVZ64(dest_reg, imm & 0xffff, 0)); // movz dest_reg, #(imm & 0xffff)
    isfirst = false;
  }
  if (((imm >> 16) & 0xffff) != 0) {
    if (isfirst) {
      isfirst = false;
      cache_addd(MOVZ64(dest_reg, (imm >> 16) & 0xffff,
                        16)); // movz dest_reg, #((imm >> 16) & 0xffff), lsl #16
    } else {
      cache_addd(MOVK64(dest_reg, (imm >> 16) & 0xffff,
                        16)); // movk dest_reg, #((imm >> 16) & 0xffff), lsl #16
    }
  }
  if (((imm >> 32) & 0xffff) != 0) {
    if (isfirst) {
      isfirst = false;
      cache_addd(MOVZ64(dest_reg, (imm >> 32) & 0xffff,
                        32)); // movz dest_reg, #((imm >> 32) & 0xffff), lsl #32
    } else {
      cache_addd(MOVK64(dest_reg, (imm >> 32) & 0xffff,
                        32)); // movk dest_reg, #((imm >> 32) & 0xffff), lsl #32
    }
  }
  if (((imm >> 48) & 0xffff) != 0) {
    if (isfirst) {
      isfirst = false;
      cache_addd(MOVZ64(dest_reg, (imm >> 48) & 0xffff,
                        48)); // movz dest_reg, #((imm >> 48) & 0xffff), lsl #48
    } else {
      cache_addd(MOVK64(dest_reg, (imm >> 48) & 0xffff,
                        48)); // movk dest_reg, #((imm >> 48) & 0xffff), lsl #48
    }
  }
  if (isfirst) {
...
static void gen_mov_word_to_reg_helper(HostReg dest_reg, void *data, bool dword,
                                       HostReg data_reg) {
  if (dword) {
...
// helper function
static bool gen_mov_memval_from_reg_helper(HostReg src_reg, uint64_t data,
                                           Bitu size, HostReg addr_reg,
                                           uint64_t addr_data) {
  switch (size) {
  case 8:
    if (((data & 7) == 0) && (data >= addr_data) &&
        (data < addr_data + 32768)) {
      cache_addd(STR64_IMM(
          src_reg, addr_reg,
          data - addr_data)); // str src_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data + 256) && (data >= addr_data - 256)) {
      cache_addd(STUR64_IMM(
          src_reg, addr_reg,
          data - addr_data)); // stur src_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
    break;
  case 4:
  ...
  case 2:
    if (((data & 1) == 0) && (data >= addr_data) && (data < addr_data + 8192)) {
      cache_addd(STRH_IMM(
          src_reg, addr_reg,
          data - addr_data)); // strh src_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data + 256) && (data >= addr_data - 256)) {
      cache_addd(STURH_IMM(
          src_reg, addr_reg,
          data - addr_data)); // sturh src_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 4096)) {
      cache_addd(STRB_IMM(
          src_reg, addr_reg,
          data - addr_data)); // strb src_reg, [addr_reg, #(data - addr_data)]
      return true;
    } else if ((data < addr_data) && (data >= addr_data - 256)) {
      cache_addd(STURB_IMM(
          src_reg, addr_reg,
          data - addr_data)); // sturb src_reg, [addr_reg, #(data - addr_data)]
      return true;
    }
  default:
    break;
...
static bool gen_mov_memval_from_reg(HostReg src_reg, void *dest, Bitu size) {
  if (gen_mov_memval_from_reg_helper(src_reg, (uint64_t)dest, size,
                                     FC_REGS_ADDR, (uint64_t)&cpu_regs))
    return true;
  if (gen_mov_memval_from_reg_helper(src_reg, (uint64_t)dest, size,
                                     readdata_addr,
                                     (uint64_t)&core_dynrec.readdata))
    return true;
  if (gen_mov_memval_from_reg_helper(src_reg, (uint64_t)dest, size,
                                     FC_SEGS_ADDR, (uint64_t)&Segs))
    return true;
  return false;
}

// helper function for gen_mov_word_from_reg
static void gen_mov_word_from_reg_helper(HostReg src_reg, void *dest,
                                         bool dword, HostReg data_reg) {
  if (dword) {
...
static void INLINE gen_mov_byte_to_reg_low_canuseword(HostReg dest_reg,
                                                      void *data) {
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void INLINE gen_mov_byte_to_reg_low_imm_canuseword(HostReg dest_reg,
                                                          uint8_t imm) {
...
static void gen_add_imm(HostReg reg, uint32_t imm) {
  ...
  if (imm < 4096) {
    ...
    cache_addd(ADD_IMM(reg, reg, imm >> 12,
                       12)); // add reg, reg, #(imm >> 12), lsl #12
  } else if (imm2 < 4096) {
    cache_addd(SUB_IMM(reg, reg, imm2, 0)); // sub reg, reg, #(-imm)
  } else if ((imm2 & 0xff000fff) == 0) {
    cache_addd(SUB_IMM(reg, reg, imm2 >> 12,
                       12)); // sub reg, reg, #(-imm >> 12), lsl #12
...
static void gen_and_imm(HostReg reg, uint32_t imm) {
  ...
  if (!imm) {
    ...
    cache_addd(
        MOVZ(temp2, imm2 >> 16, 16)); // movz temp2, #(~imm >> 16), lsl #16
    cache_addd(BIC_REG_LSL_IMM(reg, reg, temp2, 0)); // bic reg, reg, temp2
  } else {
    gen_mov_dword_to_reg_imm(temp2, imm);
...
// add a 32bit (dword==true) or 16bit (dword==false) constant value to a memory
// value
...
// subtract a 32bit (dword==true) or 16bit (dword==false) constant value from a
// memory value
static void gen_sub_direct_word(void *dest, uint32_t imm, bool dword) {
  ...
  if (imm < 4096) {
    ...
    cache_addd(SUB_IMM(temp3, temp3, imm >> 12,
                       12)); // sub temp3, temp3, #(imm >> 12), lsl #12
  } else if (imm2 < 4096) {
    cache_addd(ADD_IMM(temp3, temp3, imm2, 0)); // add temp3, temp3, #(-imm)
  } else if ((imm2 & 0xff000fff) == 0) {
    cache_addd(ADD_IMM(temp3, temp3, imm2 >> 12,
                       12)); // add temp3, temp3, #(-imm >> 12), lsl #12
  } else if (imm2 < 0x10000) {
    cache_addd(MOVZ(temp2, imm2, 0)); // movz temp2, #(-imm)
    cache_addd(
        ADD_REG_LSL_IMM(temp3, temp3, temp2, 0)); // add temp3, temp3, temp2
  } else {
    gen_mov_dword_to_reg_imm(temp2, imm);
    cache_addd(
        SUB_REG_LSL_IMM(temp3, temp3, temp2, 0)); // sub temp3, temp3, temp2
  }
...
static INLINE void gen_lea(HostReg dest_reg, HostReg scale_reg, Bitu scale,
                           Bits imm) {
  cache_addd(
      ADD_REG_LSL_IMM(dest_reg, dest_reg, scale_reg,
                      scale)); // add dest_reg, dest_reg, scale_reg, lsl #scale
...
static INLINE void gen_lea(HostReg dest_reg, Bitu scale, Bits imm) {
  if (scale) {
    cache_addd(MOV_REG_LSL_IMM(dest_reg, dest_reg,
                               scale)); // mov dest_reg, dest_reg, lsl #scale
  }
  gen_add_imm(dest_reg, imm);
}
...
template <typename T> static void INLINE gen_call_function_raw(const T func) {
  cache_addd(MOVZ64(temp1, ((uint64_t)func) & 0xffff,
                    0)); // movz dest_reg, #(func & 0xffff)
  cache_addd(MOVK64(temp1, (((uint64_t)func) >> 16) & 0xffff,
                    16)); // movk dest_reg, #((func >> 16) & 0xffff), lsl #16
  cache_addd(MOVK64(temp1, (((uint64_t)func) >> 32) & 0xffff,
                    32)); // movk dest_reg, #((func >> 32) & 0xffff), lsl #32
  cache_addd(MOVK64(temp1, (((uint64_t)func) >> 48) & 0xffff,
                    48)); // movk dest_reg, #((func >> 48) & 0xffff), lsl #48
...
template <typename T>
static DRC_PTR_SIZE_IM INLINE gen_call_function_setup(const T func,
                                                      Bitu paramcount,
                                                      bool fastcall = false) {
...
static void gen_jmp_ptr(void *ptr, Bits imm = 0) {
  ...
  if (((imm & 7) == 0) && (imm >= 0) && (imm < 32768)) {
    ...
    cache_addd(
        LDR64_REG_LSL_IMM(temp1, temp3, temp2, 0)); // ldr temp1, [temp3, temp2]
  }
...
#endif
  *(uint32_t *)data = ((*(uint32_t *)data) & 0xff00001f) |
                      ((((uint64_t)cache.pos - data) << 3) & 0x00ffffe0);
}

// conditional jump if register is nonzero
// for isdword==true the 32bit of the register are tested
// for isdword==false the lowest 8bit of the register are tested
static DRC_PTR_SIZE_IM gen_create_branch_long_nonzero(HostReg reg,
                                                      bool isdword) {
  if (isdword) {
...
// calculate long relative offset and fill it into the location pointed to by
// data
static void INLINE gen_fill_branch_long(DRC_PTR_SIZE_IM data) {
  // optimize for shorter branches ?
  *(uint32_t *)data = ((*(uint32_t *)data) & 0xfc000000) |
                      ((((uint64_t)cache.pos - data) >> 2) & 0x03ffffff);
}
...
static void gen_run_code(void) {
  ...
  cache_addd(STP64_IMM(FC_ADDR, FC_REGS_ADDR, HOST_sp,
                       16)); // stp FC_ADDR, FC_REGS_ADDR, [sp, #16]
  cache_addd(STP64_IMM(FC_SEGS_ADDR, readdata_addr, HOST_sp,
                       32)); // stp FC_SEGS_ADDR, readdata_addr, [sp, #32]
  ...
  *(uint32_t *)pos1 = LDR64_PC(
      FC_SEGS_ADDR, cache.pos - pos1); // ldr FC_SEGS_ADDR, [pc, #(&Segs)]
  cache_addq((uint64_t)&Segs);         // address of "Segs"

  *(uint32_t *)pos2 = LDR64_PC(
      FC_REGS_ADDR, cache.pos - pos2); // ldr FC_REGS_ADDR, [pc, #(&cpu_regs)]
  cache_addq((uint64_t)&cpu_regs);     // address of "cpu_regs"

  *(uint32_t *)pos3 = LDR64_PC(
      readdata_addr,
      cache.pos - pos3); // ldr readdata_addr, [pc, #(&core_dynrec.readdata)]
  cache_addq(
      (uint64_t)&core_dynrec.readdata); // address of "core_dynrec.readdata"
...
static void gen_return_function(void) {
  cache_addd(LDP64_IMM(FC_ADDR, FC_REGS_ADDR, HOST_sp,
                       16)); // ldp FC_ADDR, FC_REGS_ADDR, [sp, #16]
  cache_addd(LDP64_IMM(FC_SEGS_ADDR, readdata_addr, HOST_sp,
                       32)); // ldp FC_SEGS_ADDR, readdata_addr, [sp, #32]
...
static void gen_fill_function_ptr(uint8_t *pos, void *fct_ptr,
                                  Bitu flags_type) {
  ...
  case t_ADDd:
    ...
    *(uint32_t *)(pos + 8) =
        ADD_REG_LSL_IMM(FC_RETOP, HOST_w0, HOST_w1, 0); // add FC_RETOP, w0, w1
  ...
  case t_ORd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        ORR_REG_LSL_IMM(FC_RETOP, HOST_w0, HOST_w1, 0); // orr FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;                      // nop
    *(uint32_t *)(pos + 16) = NOP;                      // nop
    break;
  case t_ANDb:
  ...
  case t_ANDd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        AND_REG_LSL_IMM(FC_RETOP, HOST_w0, HOST_w1, 0); // and FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;                      // nop
    *(uint32_t *)(pos + 16) = NOP;                      // nop
    break;
  case t_SUBb:
  ...
  case t_SUBd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        SUB_REG_LSL_IMM(FC_RETOP, HOST_w0, HOST_w1, 0); // sub FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;                      // nop
    *(uint32_t *)(pos + 16) = NOP;                      // nop
    break;
  case t_XORb:
  ...
  case t_XORd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        EOR_REG_LSL_IMM(FC_RETOP, HOST_w0, HOST_w1, 0); // eor FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;                      // nop
    *(uint32_t *)(pos + 16) = NOP;                      // nop
    break;
  case t_CMPb:
  ...
  case t_INCd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        ADD_IMM(FC_RETOP, HOST_w0, 1, 0); // add FC_RETOP, w0, #1
    *(uint32_t *)(pos + 12) = NOP;        // nop
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_DECb:
  ...
  case t_DECd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        SUB_IMM(FC_RETOP, HOST_w0, 1, 0); // sub FC_RETOP, w0, #1
    *(uint32_t *)(pos + 12) = NOP;        // nop
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_SHLb:
  ...
  case t_SHLd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        LSLV(FC_RETOP, HOST_w0, HOST_w1); // lslv FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;        // nop
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_SHRb:
    ...
    *(uint32_t *)(pos + 8) = NOP;                     // nop
    *(uint32_t *)(pos + 12) =
        LSRV(FC_RETOP, FC_RETOP, HOST_w1); // lsrv FC_RETOP, FC_RETOP, w1
    *(uint32_t *)(pos + 16) = NOP;         // nop
    break;
  case t_SHRw:
    *(uint32_t *)pos = NOP;                           // nop
  ...
  case t_SHRd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        LSRV(FC_RETOP, HOST_w0, HOST_w1); // lsrv FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;        // nop
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_SARb:
    ...
    *(uint32_t *)(pos + 8) = NOP;                     // nop
    *(uint32_t *)(pos + 12) =
        ASRV(FC_RETOP, FC_RETOP, HOST_w1); // asrv FC_RETOP, FC_RETOP, w1
    *(uint32_t *)(pos + 16) = NOP;         // nop
    break;
  case t_SARw:
    *(uint32_t *)pos = NOP;                           // nop
  ...
  case t_SARd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        ASRV(FC_RETOP, HOST_w0, HOST_w1); // asrv FC_RETOP, w0, w1
    *(uint32_t *)(pos + 12) = NOP;        // nop
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_RORb:
    ...
    *(uint32_t *)(pos + 8) =
        BFI(HOST_w0, HOST_w0, 16, 16); // bfi w0, w0, 16, 16
    *(uint32_t *)(pos + 12) =
        RORV(FC_RETOP, HOST_w0, HOST_w1); // rorv FC_RETOP, w0, w1
    *(uint32_t *)(pos + 16) = NOP;        // nop
    break;
  case t_RORw:
    *(uint32_t *)pos = NOP; // nop
  ...
  case t_ROLb:
    ...
    *(uint32_t *)(pos + 4) = BFI(HOST_w0, HOST_w0, 8, 8); // bfi w0, w0, 8, 8
    *(uint32_t *)(pos + 8) =
        SUB_REG_LSL_IMM(HOST_w2, HOST_w2, HOST_w1, 0); // sub w2, w2, w1
    *(uint32_t *)(pos + 12) =
        BFI(HOST_w0, HOST_w0, 16, 16); // bfi w0, w0, 16, 16
    *(uint32_t *)(pos + 16) =
        RORV(FC_RETOP, HOST_w0, HOST_w2); // rorv FC_RETOP, w0, w2
    break;
  case t_ROLw:
    *(uint32_t *)pos = MOVZ(HOST_w2, 32, 0); // movz w2, #32
    *(uint32_t *)(pos + 4) =
  ...
  case t_NEGd:
    *(uint32_t *)pos = NOP;       // nop
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) = SUB_REG_LSL_IMM(FC_RETOP, HOST_wzr, HOST_w0,
                                             0); // sub FC_RETOP, wzr, w0
    *(uint32_t *)(pos + 12) = NOP;               // nop
    *(uint32_t *)(pos + 16) = NOP;               // nop
    break;
  case t_DSHLd:
    ...
    *(uint32_t *)(pos + 4) =
        BFI64(HOST_x1, HOST_x0, 32, 32); // bfi x1, x0, 32, 32
    *(uint32_t *)(pos + 8) =
        AND64_REG_LSL_IMM(HOST_x2, HOST_x2, HOST_x3, 0); // and x2, x2, x3
    *(uint32_t *)(pos + 12) =
        LSLV64(FC_RETOP, HOST_x1, HOST_x2); // lslv FC_RETOP, x1, x2
    *(uint32_t *)(pos + 16) =
        LSR64_IMM(FC_RETOP, FC_RETOP, 32); // lsr FC_RETOP, FC_RETOP, #32
    break;
  case t_DSHRd:
    *(uint32_t *)pos = MOVZ64(HOST_x3, 0x1f, 0); // movz x3, #0x1f
    *(uint32_t *)(pos + 4) =
        BFI64(HOST_x0, HOST_x1, 32, 32); // bfi x0, x1, 32, 32
    *(uint32_t *)(pos + 8) =
        AND64_REG_LSL_IMM(HOST_x2, HOST_x2, HOST_x3, 0); // and x2, x2, x3
    *(uint32_t *)(pos + 12) = NOP;                       // nop
    *(uint32_t *)(pos + 16) =
        LSRV64(FC_RETOP, HOST_x0, HOST_x2); // lsrv FC_RETOP, x0, x2
    break;
  default:
    *(uint32_t *)pos = MOVZ64(temp1, ((uint64_t)fct_ptr) & 0xffff,
                              0); // movz temp1, #(fct_ptr & 0xffff)
    *(uint32_t *)(pos + 4) =
        MOVK64(temp1, (((uint64_t)fct_ptr) >> 16) & 0xffff,
               16); // movk temp1, #((fct_ptr >> 16) & 0xffff), lsl #16
    *(uint32_t *)(pos + 8) =
        MOVK64(temp1, (((uint64_t)fct_ptr) >> 32) & 0xffff,
               32); // movk temp1, #((fct_ptr >> 32) & 0xffff), lsl #32
    *(uint32_t *)(pos + 12) =
        MOVK64(temp1, (((uint64_t)fct_ptr) >> 48) & 0xffff,
               48); // movk temp1, #((fct_ptr >> 48) & 0xffff), lsl #48
    break;
  }
#else
  *(uint32_t *)pos = MOVZ64(temp1, ((uint64_t)fct_ptr) & 0xffff,
...
static void cache_block_closing(uint8_t *block_start, Bitu block_size) {
  ...
  __builtin___clear_cache((char *)block_start,
                          (char *)(block_start + block_size));
#endif
}
...
static void gen_mov_seg16_to_reg(HostReg dest_reg, Bitu index) {
  cache_addd(LDRH_IMM(dest_reg, FC_SEGS_ADDR,
                      index)); // ldrh dest_reg, [FC_SEGS_ADDR, #index]
}
...
static void gen_mov_seg32_to_reg(HostReg dest_reg, Bitu index) {
  cache_addd(LDR_IMM(dest_reg, FC_SEGS_ADDR,
                     index)); // ldr dest_reg, [FC_SEGS_ADDR, #index]
}
...
static void gen_add_seg32_to_reg(HostReg reg, Bitu index) {
  cache_addd(
      LDR_IMM(temp1, FC_SEGS_ADDR, index)); // ldr temp1, [FC_SEGS_ADDR, #index]
...
static void gen_mov_regval16_to_reg(HostReg dest_reg, Bitu index) {
  cache_addd(LDRH_IMM(dest_reg, FC_REGS_ADDR,
                      index)); // ldrh dest_reg, [FC_REGS_ADDR, #index]
}
...
static void gen_mov_regval32_to_reg(HostReg dest_reg, Bitu index) {
  cache_addd(LDR_IMM(dest_reg, FC_REGS_ADDR,
                     index)); // ldr dest_reg, [FC_REGS_ADDR, #index]
}
...
static void gen_mov_regbyte_to_reg_low(HostReg dest_reg, Bitu index) {
  cache_addd(LDRB_IMM(dest_reg, FC_REGS_ADDR,
                      index)); // ldrb dest_reg, [FC_REGS_ADDR, #index]
}
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void gen_mov_regbyte_to_reg_low_canuseword(HostReg dest_reg,
                                                  Bitu index) {
  cache_addd(LDRB_IMM(dest_reg, FC_REGS_ADDR,
                      index)); // ldrb dest_reg, [FC_REGS_ADDR, #index]
}
...
static void gen_add_regval32_to_reg(HostReg reg, Bitu index) {
  cache_addd(
      LDR_IMM(temp2, FC_REGS_ADDR, index)); // ldr temp2, [FC_REGS_ADDR, #index]
  cache_addd(ADD_REG_LSL_IMM(reg, reg, temp2, 0)); // add reg, reg, temp2
}

// move 16bit of register into cpu_regs[index] using FC_REGS_ADDR (index modulo
// 2 must be zero)
static void gen_mov_regval16_from_reg(HostReg src_reg, Bitu index) {
  cache_addd(STRH_IMM(src_reg, FC_REGS_ADDR,
                      index)); // strh src_reg, [FC_REGS_ADDR, #index]
}

// move 32bit of register into cpu_regs[index] using FC_REGS_ADDR (index modulo
// 4 must be zero)
static void gen_mov_regval32_from_reg(HostReg src_reg, Bitu index) {
  cache_addd(STR_IMM(src_reg, FC_REGS_ADDR,
                     index)); // str src_reg, [FC_REGS_ADDR, #index]
}
...
static void gen_mov_regbyte_from_reg_low(HostReg src_reg, Bitu index) {
  cache_addd(STRB_IMM(src_reg, FC_REGS_ADDR,
                      index)); // strb src_reg, [FC_REGS_ADDR, #index]
}
