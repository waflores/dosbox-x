...
// This function generates an instruction with register addressing and a memory
// location
static INLINE void gen_reg_memaddr(HostReg reg, void *data, uint8_t op,
                                   uint8_t prefix = 0) {
  int64_t diff =
      (int64_t)data - ((int64_t)cache_rwtox(cache.pos + (prefix ? 7 : 6)));
  //	if ((diff<0x80000000LL) && (diff>-0x80000000LL)) { //clang messes itself
  //up on this...
  if ((diff >> 63) ==
      (diff >>
       31)) { // signed bit extend, test to see if value fits in an int32_t
    ...
    // mov reg,[data] (or similar, depending on the op) when absolute address of
    // data is <4GB
    if (prefix)
      cache_addb(prefix);
    cache_addb(op);
    ...
    // load 64-bit data into tmp_reg and do mov reg,[tmp_reg] (or similar,
    // depending on the op)
...
static INLINE void gen_memaddr(uint8_t modreg, void *data, Bitu off, Bitu imm,
                               uint8_t op, uint8_t prefix = 0) {
  ...
  if ((diff >> 63) == (diff >> 31)) {
    ...
    switch (off) {
    case 1:
      cache_addb(((uint8_t)imm & 0xff));
      break;
    case 2:
      cache_addw(((uint16_t)imm & 0xffff));
      break;
    case 4:
      cache_addd(((uint32_t)imm & 0xffffffff));
      break;
    }
...
static void gen_mov_word_to_reg(HostReg dest_reg, void *data, bool dword,
  ...
  if (!dword)
    gen_reg_memaddr(
        dest_reg, data, 0xb7,
        0x0f); // movzx reg,[data] - zero extend data, fixes LLVM compile where
               // the called function does not extend the parameters
  else
    gen_reg_memaddr(dest_reg, data, 0x8b, prefix); // mov reg,[data]
}
...
static void gen_mov_word_from_reg(HostReg src_reg, void *dest, bool dword,
                                  uint8_t prefix = 0) {
  gen_reg_memaddr(src_reg, dest, 0x89,
                  (dword ? prefix : 0x66)); // mov [data],reg
}
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void gen_mov_byte_to_reg_low_imm_canuseword(HostReg dest_reg,
                                                   uint8_t imm) {
  cache_addb(0xb8 + dest_reg); // mov reg,imm
  cache_addd((uint32_t)imm);
}
...
static void INLINE gen_mov_direct_ptr(void *dest, DRC_PTR_SIZE_IM imm) {
  ...
  gen_mov_word_from_reg(HOST_EAX, dest, true,
                        0x48); // 0x48 prefixes full 64-bit mov
}
...
static void gen_add_direct_word(void *dest, uint32_t imm, bool dword) {
  ...
  gen_memaddr(0x4, dest, (dword ? 4 : 2), imm, 0x81,
              (dword ? 0 : 0x66)); // add [data],imm
}
...
static void gen_sub_direct_word(void *dest, uint32_t imm, bool dword) {
  ...
  gen_memaddr(0x2c, dest, (dword ? 4 : 2), imm, 0x81,
              (dword ? 0 : 0x66)); // sub [data],imm
}
...
static INLINE void gen_lea(HostReg dest_reg, HostReg scale_reg, Bitu scale,
  ...
  if (!imm) {
    ...
    imm_size = 1;
    rm_base = 0x40; // Signed byte imm
  } else {
    imm_size = 4;
    rm_base = 0x80; // Signed dword imm
  }
...
template <typename T>
static uint64_t INLINE gen_call_function_setup(const T func, Bitu paramcount,
                                               bool fastcall = false) {
  (void)paramcount;
  (void)fastcall;
...
static void gen_fill_function_ptr(uint8_t *pos, void *fct_ptr,
  ...
  case t_ADDd:
    ...
    *(uint32_t *)(pos + 0) = 0xc001c089 + (FC_OP1 << 11) + (FC_OP2 << 27);
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_ORb:
  case t_ORw:
  case t_ORd:
    // mov eax,FC_OP1; or eax,FC_OP2
    *(uint32_t *)(pos + 0) = 0xc009c089 + (FC_OP1 << 11) + (FC_OP2 << 27);
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_ANDb:
  ...
  case t_ANDd:
    ...
    *(uint32_t *)(pos + 0) = 0xc021c089 + (FC_OP1 << 11) + (FC_OP2 << 27);
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_SUBb:
  ...
  case t_SUBd:
    ...
    *(uint32_t *)(pos + 0) = 0xc029c089 + (FC_OP1 << 11) + (FC_OP2 << 27);
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_XORb:
  ...
  case t_XORd:
    ...
    *(uint32_t *)(pos + 0) = 0xc031c089 + (FC_OP1 << 11) + (FC_OP2 << 27);
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_CMPb:
  ...
  case t_TESTw:
  case t_TESTd:
    *(uint32_t *)(pos + 0) = 0x90900aeb; // skip
    *(uint32_t *)(pos + 4) = 0x90909090;
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_INCb:
  case t_INCw:
  case t_INCd:
    *(uint32_t *)(pos + 0) =
        0xc0ffc089 + (FC_OP1 << 11);     // mov eax,ecx; inc eax
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_DECb:
  ...
  case t_DECd:
    *(uint32_t *)(pos + 0) =
        0xc8ffc089 + (FC_OP1 << 11);     // mov eax, FC_OP1; dec eax
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  case t_NEGb:
  ...
  case t_NEGd:
    *(uint32_t *)(pos + 0) =
        0xd8f7c089 + (FC_OP1 << 11);     // mov eax, FC_OP1; neg eax
    *(uint32_t *)(pos + 4) = 0x909006eb; // skip
    *(uint32_t *)(pos + 8) = 0x90909090;
    return;
  }
