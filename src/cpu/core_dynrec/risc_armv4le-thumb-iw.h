...
/* ARMv4 (little endian) backend by M-HT (thumb version with data pool, requires
 * -mthumb-interwork switch when compiling dosbox) */
...
// add dst, src1, src2
#define ADD_REG(dst, src1, src2)                                               \
  (0x1800 + (dst) + ((src1) << 3) + ((src2) << 6))
// add dst, pc, #imm		@	0 <= imm < 1024	&	imm mod 4 = 0
#define ADD_LO_PC_IMM(dst, imm) (0xa000 + ((dst) << 8) + ((imm) >> 2))
// sub dst, src1, src2
#define SUB_REG(dst, src1, src2)                                               \
  (0x1a00 + (dst) + ((src1) << 3) + ((src2) << 6))
...
// load
// ldr reg, [addr, #imm]		@	0 <= imm < 128	&	imm mod
// 4 = 0
#define LDR_IMM(reg, addr, imm) (0x6800 + (reg) + ((addr) << 3) + ((imm) << 4))
// ldrh reg, [addr, #imm]		@	0 <= imm < 64	&	imm mod
// 2 = 0
...
// ldr reg, [addr1, addr2]
#define LDR_REG(reg, addr1, addr2)                                             \
  (0x5800 + (reg) + ((addr1) << 3) + ((addr2) << 6))

// store
// str reg, [addr, #imm]		@	0 <= imm < 128	&	imm mod
// 4 = 0
#define STR_IMM(reg, addr, imm) (0x6000 + (reg) + ((addr) << 3) + ((imm) << 4))
// strh reg, [addr, #imm]		@	0 <= imm < 64	&	imm mod
// 2 = 0
...
// rimm mod 2 = 0
#define ARM_ADD_IMM(dst, src, imm, rimm)                                       \
  (0xe2800000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))

// load
// ldr reg, [addr, #imm]		@	0 <= imm < 4096
#define ARM_LDR_IMM(reg, addr, imm)                                            \
  (0xe5900000 + ((reg) << 12) + ((addr) << 16) + (imm))

// store
// str reg, [addr, #-(imm)]!		@	0 <= imm < 4096
#define ARM_STR_IMM_M_W(reg, addr, imm)                                        \
  (0xe5200000 + ((reg) << 12) + ((addr) << 16) + (imm))
...
// data pool variables
static uint8_t *cache_datapos =
    NULL;                            // position of data pool in the cache block
static uint32_t cache_datasize = 0;  // total size of data pool
static uint32_t cache_dataindex = 0; // used size of data pool = index of free
                                     // data item (in bytes) in data pool
...
static void cache_checkinstr(uint32_t size) {
  ...
  if (cache.pos + CACHE_DATA_MAX + CACHE_DATA_ALIGN >=
          cache.block.active->cache.start + cache.block.active->cache.size &&
      cache.pos + CACHE_DATA_MIN + CACHE_DATA_ALIGN +
              (CACHE_DATA_ALIGN - CACHE_ALIGN) <
          cache.block.active->cache.start + cache.block.active->cache.size) {
    cache_datapos =
        (uint8_t *)(((Bitu)cache.block.active->cache.start +
                     cache.block.active->cache.size - CACHE_DATA_ALIGN) &
                    ~(CACHE_DATA_ALIGN - 1));
  } else {
    register uint32_t cachemodsize;

    cachemodsize =
        (cache.pos - cache.block.active->cache.start) & (CACHE_MAXSIZE - 1);

    if (cachemodsize + CACHE_DATA_MAX + CACHE_DATA_ALIGN <= CACHE_MAXSIZE ||
        cachemodsize + CACHE_DATA_MIN + CACHE_DATA_ALIGN +
                (CACHE_DATA_ALIGN - CACHE_ALIGN) >
            CACHE_MAXSIZE) {
      cache_datapos = (uint8_t *)(((Bitu)cache.pos + CACHE_DATA_MAX) &
                                  ~(CACHE_DATA_ALIGN - 1));
    } else {
      cache_datapos =
          (uint8_t *)(((Bitu)cache.pos + (CACHE_MAXSIZE - CACHE_DATA_ALIGN) -
                       cachemodsize) &
                      ~(CACHE_DATA_ALIGN - 1));
    }
  }
...
static uint8_t *cache_reservedata(void) {
  ...
  if (GCC_UNLIKELY(cache_datapos == NULL)) {
    if (cache.pos + CACHE_DATA_MIN + CACHE_DATA_ALIGN <
        cache.block.active->cache.start + CACHE_DATA_MAX) {
      cache_datapos =
          (uint8_t *)(((Bitu)cache.block.active->cache.start + CACHE_DATA_MAX) &
                      ~(CACHE_DATA_ALIGN - 1));
    }
  }
  ...
  if (cache_datasize == 0) {
    // set data pool address is too close (or behind)  cache.pos then set new
    // data pool size
    if (cache.pos + CACHE_DATA_MIN + CACHE_DATA_JUMP /*+ CACHE_DATA_ALIGN*/ >
        cache_datapos) {
      if (cache.pos + CACHE_DATA_MAX + CACHE_DATA_ALIGN >=
              cache.block.active->cache.start +
                  cache.block.active->cache.size &&
          cache.pos + CACHE_DATA_MIN + CACHE_DATA_ALIGN +
                  (CACHE_DATA_ALIGN - CACHE_ALIGN) <
              cache.block.active->cache.start +
                  cache.block.active->cache.size) {
        ...
        if (cachemodsize + CACHE_DATA_MAX + CACHE_DATA_ALIGN <= CACHE_MAXSIZE ||
          ...
          cache_datapos =
              (uint8_t *)(((Bitu)cache.pos +
                           (CACHE_MAXSIZE - CACHE_DATA_ALIGN) - cachemodsize) &
                          ~(CACHE_DATA_ALIGN - 1));
        }
      }
    }
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg, uint32_t imm) {
  ...
  if (imm < 256) {
    ...
    cache_checkinstr(4);
    cache_addw(
        MOV_IMM(dest_reg, imm >> scale)); // mov dest_reg, #(imm >> scale)
    cache_addw(
        LSL_IMM(dest_reg, dest_reg, scale)); // lsl dest_reg, dest_reg, #scale
  } else {
    uint32_t diff;
    ...
    if ((diff < 1024) && ((imm & 0x03) == 0)) {
      if (((uint32_t)cache.pos & 0x03) == 0) {
        cache_addw(ADD_LO_PC_IMM(dest_reg,
                                 diff >> 2)); // add dest_reg, pc, #(diff >> 2)
      } else {
        cache_addw(NOP); // nop
        cache_addw(ADD_LO_PC_IMM(
            dest_reg, (diff - 2) >> 2)); // add dest_reg, pc, #((diff - 2) >> 2)
      }
    } else {
      uint8_t *datapos;
      ...
      *(uint32_t *)datapos = imm;
      ...
      if (((uint32_t)cache.pos & 0x03) == 0) {
        cache_addw(LDR_PC_IMM(
            dest_reg,
            datapos - (cache.pos + 4))); // ldr dest_reg, [pc, datapos]
      } else {
        cache_addw(LDR_PC_IMM(
            dest_reg,
            datapos - (cache.pos + 2))); // ldr dest_reg, [pc, datapos]
      }
    }
  }
}
...
static bool gen_mov_memval_to_reg_helper(HostReg dest_reg, uint32_t data,
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 128) &&
          (((data - addr_data) & 3) == 0)) {
        cache_checkinstr(4);
        cache_addw(MOV_LO_HI(templo2, addr_reg)); // mov templo2, addr_reg
        cache_addw(LDR_IMM(
            dest_reg, templo2,
            data - addr_data)); // ldr dest_reg, [templo2, #(data - addr_data)]
        return true;
      }
    }
    break;
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 64) &&
          (((data - addr_data) & 1) == 0)) {
        cache_checkinstr(4);
        cache_addw(MOV_LO_HI(templo2, addr_reg)); // mov templo2, addr_reg
        cache_addw(LDRH_IMM(
            dest_reg, templo2,
            data - addr_data)); // ldrh dest_reg, [templo2, #(data - addr_data)]
        return true;
      }
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 32)) {
      ...
      cache_addw(LDRB_IMM(
          dest_reg, templo2,
          data - addr_data)); // ldrb dest_reg, [templo2, #(data - addr_data)]
      return true;
    }
  default:
    break;
  ...
  if (dword) {
    ...
    if ((uint32_t)data & 3) {
      if (((uint32_t)data & 3) == 2) {
        ...
        cache_addw(
            LDRH_IMM(templo1, data_reg, 2)); // ldrh templo1, [data_reg, #2]
        ...
        cache_addw(ORR(dest_reg, templo1));         // orr dest_reg, templo1
        cache_addw(
            LDRB_IMM(templo1, data_reg, 3)); // ldrb templo1, [data_reg, #3]
    ...
    if ((uint32_t)data & 1) {
      ...
      cache_addw(
          LDRB_IMM(templo1, data_reg, 1));      // ldrb templo1, [data_reg, #1]
      cache_addw(LSL_IMM(templo1, templo1, 8)); // lsl templo1, templo1, #8
      cache_addw(ORR(dest_reg, templo1));       // orr dest_reg, templo1
    } else
#endif
...
static bool gen_mov_memval_from_reg_helper(HostReg src_reg, uint32_t data,
  ...
  case 4:
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 128) &&
        ...
        cache_addw(STR_IMM(
            src_reg, templo2,
            data - addr_data)); // str src_reg, [templo2, #(data - addr_data)]
        return true;
      }
    }
    break;
  case 2:
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 64) &&
        ...
        cache_addw(STRH_IMM(
            src_reg, templo2,
            data - addr_data)); // strh src_reg, [templo2, #(data - addr_data)]
        return true;
      }
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 32)) {
      ...
      cache_addw(STRB_IMM(
          src_reg, templo2,
          data - addr_data)); // strb src_reg, [templo2, #(data - addr_data)]
      return true;
    }
  default:
    break;
  ...
  if (dword) {
    ...
    if ((uint32_t)dest & 3) {
      if (((uint32_t)dest & 3) == 2) {
        ...
        cache_addw(
            STRH_IMM(templo1, data_reg, 2)); // strh templo1, [data_reg, #2]
      } else {
        cache_checkinstr(20);
        ...
        cache_addw(
            STRB_IMM(templo1, data_reg, 1));   // strb templo1, [data_reg, #1]
        cache_addw(MOV_REG(templo1, src_reg)); // mov templo1, src_reg
        cache_addw(LSR_IMM(templo1, templo1, 16)); // lsr templo1, templo1, #16
        cache_addw(
            STRB_IMM(templo1, data_reg, 2));   // strb templo1, [data_reg, #2]
        cache_addw(MOV_REG(templo1, src_reg)); // mov templo1, src_reg
        cache_addw(LSR_IMM(templo1, templo1, 24)); // lsr templo1, templo1, #24
        cache_addw(
            STRB_IMM(templo1, data_reg, 3)); // strb templo1, [data_reg, #3]
      }
    } else
#endif
    {
...
static void gen_add_imm(HostReg reg, uint32_t imm) {
  ...
  if (imm <= 255) {
    ...
    if (val_single_shift(imm2, &scale)) {
      ...
      cache_addw(
          MOV_IMM(templo1, imm2 >> scale)); // mov templo1, #(~imm >> scale)
      if (scale) {
        cache_addw(
            LSL_IMM(templo1, templo1, scale)); // lsl templo1, templo1, #scale
      }
...
static void gen_sub_direct_word(void *dest, uint32_t imm, bool dword) {
  ...
  if (imm <= 255) {
    ...
    if (val_single_shift(imm2, &scale)) {
      ...
      cache_addw(
          ADD_REG(templo3, templo3, templo1)); // add templo3, templo3, templo1
    } else {
      gen_mov_dword_to_reg_imm(templo1, imm);
      cache_checkinstr(2);
      cache_addw(
          SUB_REG(templo3, templo3, templo1)); // sub templo3, templo3, templo1
    }
  }
...
static INLINE void gen_lea(HostReg dest_reg, HostReg scale_reg, Bitu scale,
  ...
  if (scale) {
    cache_checkinstr(4);
    cache_addw(
        LSL_IMM(templo1, scale_reg, scale)); // lsl templo1, scale_reg, #(scale)
    cache_addw(ADD_REG(dest_reg, dest_reg,
                       templo1)); // add dest_reg, dest_reg, templo1
  } else {
    cache_checkinstr(2);
    cache_addw(ADD_REG(dest_reg, dest_reg,
                       scale_reg)); // add dest_reg, dest_reg, scale_reg
  }
...
template <typename T> static void gen_call_function_helper(const T func) {
  ...
  *(uint32_t *)datapos = (uint32_t)func;
  ...
  if (((uint32_t)cache.pos & 0x03) == 0) {
    cache_addw(LDR_PC_IMM(
        templo1, datapos - (cache.pos + 4))); // ldr templo1, [pc, datapos]
    cache_addw(ADD_LO_PC_IMM(
        templo2, 8)); // adr templo2, after_call (add templo2, pc, #8)
    ...
    cache_addw(LDR_PC_IMM(
        templo1, datapos - (cache.pos + 2))); // ldr templo1, [pc, datapos]
    cache_addw(ADD_LO_PC_IMM(
        templo2, 4)); // adr templo2, after_call (add templo2, pc, #4)
    cache_addw(ADD_IMM8(templo2, 1));        // add templo2, #1
    cache_addw(MOV_HI_LO(HOST_lr, templo2)); // mov lr, templo2
    cache_addw(BX(templo1)); // bx templo1     --- switch to arm state
  }
...
static void gen_jmp_ptr(void *ptr, Bits imm = 0) {
  ...
  if ((imm & 0x03) == 0) {
    ...
    if ((imm >= 0) && (imm < 128) && ((imm & 3) == 0)) {
      cache_checkinstr(6);
      cache_addw(
          LDR_IMM(templo2, templo3, imm)); // ldr templo2, [templo3, #imm]
    } else {
      gen_mov_dword_to_reg_imm(templo2, imm);
      cache_checkinstr(6);
      cache_addw(LDR_REG(templo2, templo3,
                         templo2)); // ldr templo2, [templo3, templo2]
    }
#if !defined(C_UNALIGNED_MEMORY)
  } else {
    gen_add_imm(templo3, imm);
...
static void INLINE gen_fill_branch_long(uint32_t data) {
  ...
  *(uint32_t *)data =
      ((uint32_t)cache.pos) + 1; // add 1 to keep processor in thumb state
}
...
static void gen_fill_function_ptr(uint8_t *pos, void *fct_ptr,
  ...
  if ((*(uint16_t *)pos & 0xf000) == 0xe000) {
    if ((*(uint16_t *)pos & 0x0fff) >= ((CACHE_DATA_ALIGN / 2) - 1) &&
      ...
      pos = (uint8_t *)((((uint32_t)(*(uint16_t *)pos & 0x0fff)) << 1) +
                        ((uint32_t)pos + 4));
    }
  }
  ...
  if (((uint32_t)pos & 0x03) == 0) {
    ...
    case t_TESTw:
    case t_TESTd:
      *(uint16_t *)pos = B_FWD(8); // b after_call (pc+8)
      break;
    case t_INCb:
    case t_INCw:
    case t_INCd:
    ...
    case t_SHRb:
      ...
      *(uint16_t *)(pos + 2) = NOP;                           // nop
      *(uint16_t *)(pos + 4) = LSR_IMM(HOST_a1, HOST_a1, 24); // lsr a1, a1, #24
      *(uint16_t *)(pos + 6) = NOP;                           // nop
      *(uint16_t *)(pos + 8) = LSR_REG(HOST_a1, HOST_a2);     // lsr a1, a2
      *(uint16_t *)(pos + 10) = NOP;                          // nop
      break;
    case t_SHRw:
      *(uint16_t *)pos = LSL_IMM(HOST_a1, HOST_a1, 16);       // lsl a1, a1, #16
      *(uint16_t *)(pos + 2) = NOP;                           // nop
      *(uint16_t *)(pos + 4) = LSR_IMM(HOST_a1, HOST_a1, 16); // lsr a1, a1, #16
      *(uint16_t *)(pos + 6) = NOP;                           // nop
      *(uint16_t *)(pos + 8) = LSR_REG(HOST_a1, HOST_a2);     // lsr a1, a2
      *(uint16_t *)(pos + 10) = NOP;                          // nop
      break;
    ...
    case t_SARb:
      *(uint16_t *)pos = LSL_IMM(HOST_a1, HOST_a1, 24);       // lsl a1, a1, #24
      *(uint16_t *)(pos + 2) = NOP;                           // nop
      *(uint16_t *)(pos + 4) = ASR_IMM(HOST_a1, HOST_a1, 24); // asr a1, a1, #24
      *(uint16_t *)(pos + 6) = NOP;                           // nop
      *(uint16_t *)(pos + 8) = ASR_REG(HOST_a1, HOST_a2);     // asr a1, a2
      *(uint16_t *)(pos + 10) = NOP;                          // nop
      break;
    case t_SARw:
      *(uint16_t *)pos = LSL_IMM(HOST_a1, HOST_a1, 16);       // lsl a1, a1, #16
      *(uint16_t *)(pos + 2) = NOP;                           // nop
      *(uint16_t *)(pos + 4) = ASR_IMM(HOST_a1, HOST_a1, 16); // asr a1, a1, #16
      *(uint16_t *)(pos + 6) = NOP;                           // nop
      *(uint16_t *)(pos + 8) = ASR_REG(HOST_a1, HOST_a2);     // asr a1, a2
      *(uint16_t *)(pos + 10) = NOP;                          // nop
      break;
    ...
    case t_RORb:
      *(uint16_t *)pos = LSL_IMM(HOST_a1, HOST_a1, 24); // lsl a1, a1, #24
      *(uint16_t *)(pos + 2) =
          LSR_IMM(templo1, HOST_a1, 8);               // lsr templo1, a1, #8
      *(uint16_t *)(pos + 4) = ORR(HOST_a1, templo1); // orr a1, templo1
      *(uint16_t *)(pos + 6) =
          LSR_IMM(templo1, HOST_a1, 16);              // lsr templo1, a1, #16
    ...
    case t_RORw:
      ...
      *(uint16_t *)(pos + 2) =
          LSR_IMM(templo1, HOST_a1, 16);              // lsr templo1, a1, #16
      *(uint16_t *)(pos + 4) = NOP;                   // nop
      *(uint16_t *)(pos + 6) = ORR(HOST_a1, templo1); // orr a1, templo1
      *(uint16_t *)(pos + 8) = NOP;                   // nop
      *(uint16_t *)(pos + 10) = ROR_REG(HOST_a1, HOST_a2); // ror a1, a2
      break;
    case t_RORd:
    ...
    case t_ROLd:
      ...
      *(uint16_t *)(pos + 2) = NOP;                       // nop
      *(uint16_t *)(pos + 4) = ADD_IMM8(HOST_a2, 32);     // add a2, #32
      *(uint16_t *)(pos + 6) = NOP;                       // nop
      *(uint16_t *)(pos + 8) = ROR_REG(HOST_a1, HOST_a2); // ror a1, a2
      *(uint16_t *)(pos + 10) = NOP;                      // nop
      break;
    case t_NEGb:
    case t_NEGw:
    case t_NEGd:
      ...
      *(uint16_t *)(pos + 2) = B_FWD(6);        // b after_call (pc+6)
      break;
    default:
      *(uint32_t *)((((uint32_t)(*pos)) << 2) + ((uint32_t)pos + 4)) =
          (uint32_t)fct_ptr; // simple_func
      break;
    }
  } else {
    // try to avoid function calls but rather directly fill in code
    ...
    case t_TESTw:
    case t_TESTd:
      *(uint16_t *)pos = B_FWD(6); // b after_call (pc+6)
      break;
    case t_INCb:
    case t_INCw:
    case t_INCd:
    ...
    case t_NEGd:
      ...
      *(uint16_t *)(pos + 2) = B_FWD(4);        // b after_call (pc+4)
      break;
    default:
      *(uint32_t *)((((uint32_t)(*pos)) << 2) + ((uint32_t)pos + 2)) =
          (uint32_t)fct_ptr; // simple_func
      break;
    }
  }
...
static void gen_mov_seg16_to_reg(HostReg dest_reg, Bitu index) {
  ...
  cache_addw(
      LDRH_IMM(dest_reg, templo1, index)); // ldrh dest_reg, [templo1, #index]
}
...
static void gen_mov_seg32_to_reg(HostReg dest_reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo1, FC_SEGS_ADDR)); // mov templo1, FC_SEGS_ADDR
  cache_addw(
      LDR_IMM(dest_reg, templo1, index)); // ldr dest_reg, [templo1, #index]
}
...
static void gen_add_seg32_to_reg(HostReg reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo1, FC_SEGS_ADDR)); // mov templo1, FC_SEGS_ADDR
  cache_addw(
      LDR_IMM(templo2, templo1, index));  // ldr templo2, [templo1, #index]
...
static void gen_mov_regval16_to_reg(HostReg dest_reg, Bitu index) {
  ...
  cache_addw(
      LDRH_IMM(dest_reg, templo2, index)); // ldrh dest_reg, [templo2, #index]
}
...
static void gen_mov_regval32_to_reg(HostReg dest_reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo2, FC_REGS_ADDR)); // mov templo2, FC_REGS_ADDR
  cache_addw(
      LDR_IMM(dest_reg, templo2, index)); // ldr dest_reg, [templo2, #index]
}
...
static void gen_mov_regbyte_to_reg_low(HostReg dest_reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo2, FC_REGS_ADDR)); // mov templo2, FC_REGS_ADDR
  cache_addw(
      LDRB_IMM(dest_reg, templo2, index)); // ldrb dest_reg, [templo2, #index]
}
...
// this function can use FC_OP1/FC_OP2 as dest_reg which are
// not directly byte-accessible on some architectures
static void INLINE gen_mov_regbyte_to_reg_low_canuseword(HostReg dest_reg,
                                                         Bitu index) {
  cache_checkinstr(4);
  cache_addw(MOV_LO_HI(templo2, FC_REGS_ADDR)); // mov templo2, FC_REGS_ADDR
  cache_addw(
...
static void gen_add_regval32_to_reg(HostReg reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo2, FC_REGS_ADDR)); // mov templo2, FC_REGS_ADDR
  cache_addw(
      LDR_IMM(templo1, templo2, index));  // ldr templo1, [templo2, #index]
  cache_addw(ADD_REG(reg, reg, templo1)); // add reg, reg, templo1
}
...
static void gen_mov_regval16_from_reg(HostReg src_reg, Bitu index) {
  ...
  cache_addw(
      STRH_IMM(src_reg, templo1, index)); // strh src_reg, [templo1, #index]
}
...
static void gen_mov_regval32_from_reg(HostReg src_reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo1, FC_REGS_ADDR)); // mov templo1, FC_REGS_ADDR
  cache_addw(
      STR_IMM(src_reg, templo1, index)); // str src_reg, [templo1, #index]
}
...
static void gen_mov_regbyte_from_reg_low(HostReg src_reg, Bitu index) {
  ...
  cache_addw(MOV_LO_HI(templo1, FC_REGS_ADDR)); // mov templo1, FC_REGS_ADDR
  cache_addw(
      STRB_IMM(src_reg, templo1, index)); // strb src_reg, [templo1, #index]
}
