...
#define FC_ADDR                                                                \
  HOST_s0 // has to be saved across calls, see DRC_PROTECT_ADDR_REG
...
static void INLINE mov_imm_to_temp1(uint32_t imm) {
  if (temp1_valid && (temp1_value == imm))
    return;
...
static INLINE void gen_lea(HostReg dest_reg, HostReg scale_reg, Bitu scale,
                           Bits imm) {
  if (scale) {
    cache_addw((scale_reg << 11) +
               (scale << 6)); // sll scale_reg, scale_reg, scale
...
static INLINE void gen_lea(HostReg dest_reg, Bitu scale, Bits imm) {
  if (scale) {
    cache_addw((dest_reg << 11) +
               (scale << 6)); // sll dest_reg, dest_reg, scale
    cache_addw(dest_reg);
  }
  gen_add_imm(dest_reg, imm);
}
...
static void gen_fill_function_ptr(uint8_t *pos, void *fct_ptr,
  ...
  case t_NEGd:
    *(uint32_t *)pos = 0x00041023; // subu $v0, $0, $a0
    break;
  default:
    *(uint32_t *)pos = 0x0c000000 + ((((uint32_t)fct_ptr) >> 2) &
