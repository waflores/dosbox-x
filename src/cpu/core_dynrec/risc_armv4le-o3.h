...
#define FC_ADDR                                                                \
  HOST_v1 // has to be saved across calls, see DRC_PROTECT_ADDR_REG
...
// move
// mov dst, #(imm ror rimm)		@	0 <= imm <= 255	&	rimm mod
// 2 = 0
#define MOV_IMM(dst, imm, rimm)                                                \
  (0xe3a00000 + ((dst) << 12) + (imm) + ((rimm) << 7))
// mov dst, src, lsl #imm
#define MOV_REG_LSL_IMM(dst, src, imm)                                         \
  (0xe1a00000 + ((dst) << 12) + (src) + ((imm) << 7))
// movs dst, src, lsl #imm
#define MOVS_REG_LSL_IMM(dst, src, imm)                                        \
  (0xe1b00000 + ((dst) << 12) + (src) + ((imm) << 7))
// mov dst, src, lsr #imm
#define MOV_REG_LSR_IMM(dst, src, imm)                                         \
  (0xe1a00020 + ((dst) << 12) + (src) + ((imm) << 7))
// mov dst, src, asr #imm
#define MOV_REG_ASR_IMM(dst, src, imm)                                         \
  (0xe1a00040 + ((dst) << 12) + (src) + ((imm) << 7))
// mov dst, src, lsl rreg
#define MOV_REG_LSL_REG(dst, src, rreg)                                        \
  (0xe1a00010 + ((dst) << 12) + (src) + ((rreg) << 8))
// mov dst, src, lsr rreg
#define MOV_REG_LSR_REG(dst, src, rreg)                                        \
  (0xe1a00030 + ((dst) << 12) + (src) + ((rreg) << 8))
// mov dst, src, asr rreg
#define MOV_REG_ASR_REG(dst, src, rreg)                                        \
  (0xe1a00050 + ((dst) << 12) + (src) + ((rreg) << 8))
// mov dst, src, ror rreg
#define MOV_REG_ROR_REG(dst, src, rreg)                                        \
  (0xe1a00070 + ((dst) << 12) + (src) + ((rreg) << 8))
// mvn dst, #(imm ror rimm)		@	0 <= imm <= 255	&	rimm mod
// 2 = 0
#define MVN_IMM(dst, imm, rimm)                                                \
  (0xe3e00000 + ((dst) << 12) + (imm) + ((rimm) << 7))
#if C_TARGETCPU == ARMV7LE
// movw dst, #imm		@	0 <= imm <= 65535
#define MOVW(dst, imm)                                                         \
  (0xe3000000 + ((dst) << 12) + (((imm) & 0xf000) << 4) + ((imm) & 0x0fff))
// movt dst, #imm		@	0 <= imm <= 65535
#define MOVT(dst, imm)                                                         \
  (0xe3400000 + ((dst) << 12) + (((imm) & 0xf000) << 4) + ((imm) & 0x0fff))
#endif

// arithmetic
// add dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define ADD_IMM(dst, src, imm, rimm)                                           \
  (0xe2800000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// add dst, src1, src2, lsl #imm
#define ADD_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe0800000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// sub dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define SUB_IMM(dst, src, imm, rimm)                                           \
  (0xe2400000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// sub dst, src1, src2, lsl #imm
#define SUB_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe0400000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// rsb dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define RSB_IMM(dst, src, imm, rimm)                                           \
  (0xe2600000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// cmp src, #(imm ror rimm)		@	0 <= imm <= 255	&	rimm mod
// 2 = 0
#define CMP_IMM(src, imm, rimm)                                                \
  (0xe3500000 + ((src) << 16) + (imm) + ((rimm) << 7))
// nop
#if C_TARGETCPU == ARMV7LE
...
// logical
// tst src, #(imm ror rimm)		@	0 <= imm <= 255	&	rimm mod
// 2 = 0
#define TST_IMM(src, imm, rimm)                                                \
  (0xe3100000 + ((src) << 16) + (imm) + ((rimm) << 7))
// and dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define AND_IMM(dst, src, imm, rimm)                                           \
  (0xe2000000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// and dst, src1, src2, lsl #imm
#define AND_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe0000000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// orr dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define ORR_IMM(dst, src, imm, rimm)                                           \
  (0xe3800000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// orr dst, src1, src2, lsl #imm
#define ORR_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe1800000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// orr dst, src1, src2, lsr #imm
#define ORR_REG_LSR_IMM(dst, src1, src2, imm)                                  \
  (0xe1800020 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// eor dst, src1, src2, lsl #imm
#define EOR_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe0200000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))
// bic dst, src, #(imm ror rimm)		@	0 <= imm <= 255	&
// rimm mod 2 = 0
#define BIC_IMM(dst, src, imm, rimm)                                           \
  (0xe3c00000 + ((dst) << 12) + ((src) << 16) + (imm) + ((rimm) << 7))
// bic dst, src1, src2, lsl #imm		@	0 <= imm <= 31
#define BIC_REG_LSL_IMM(dst, src1, src2, imm)                                  \
  (0xe1c00000 + ((dst) << 12) + ((src1) << 16) + (src2) + ((imm) << 7))

// load
// ldr reg, [addr, #imm]		@	0 <= imm < 4096
#define LDR_IMM(reg, addr, imm)                                                \
  (0xe5900000 + ((reg) << 12) + ((addr) << 16) + (imm))
// ldr reg, [addr, #-(imm)]		@	0 <= imm < 4096
#define LDR_IMM_M(reg, addr, imm)                                              \
  (0xe5100000 + ((reg) << 12) + ((addr) << 16) + (imm))
// ldrh reg, [addr, #imm]		@	0 <= imm < 256
#define LDRH_IMM(reg, addr, imm)                                               \
  (0xe1d000b0 + ((reg) << 12) + ((addr) << 16) + (((imm) & 0xf0) << 4) +       \
   ((imm) & 0x0f))
// ldrh reg, [addr, #-(imm)]		@	0 <= imm < 256
#define LDRH_IMM_M(reg, addr, imm)                                             \
  (0xe15000b0 + ((reg) << 12) + ((addr) << 16) + (((imm) & 0xf0) << 4) +       \
   ((imm) & 0x0f))
// ldrb reg, [addr, #imm]		@	0 <= imm < 4096
#define LDRB_IMM(reg, addr, imm)                                               \
  (0xe5d00000 + ((reg) << 12) + ((addr) << 16) + (imm))
// ldrb reg, [addr, #-(imm)]		@	0 <= imm < 4096
#define LDRB_IMM_M(reg, addr, imm)                                             \
  (0xe5500000 + ((reg) << 12) + ((addr) << 16) + (imm))
// ldr reg, [addr1, addr2, lsl #imm]		@	0 <= imm < 31
#define LDR_REG_LSL_IMM(reg, addr1, addr2, imm)                                \
  (0xe7900000 + ((reg) << 12) + ((addr1) << 16) + (addr2) + ((imm) << 7))

// store
// str reg, [addr, #imm]		@	0 <= imm < 4096
#define STR_IMM(reg, addr, imm)                                                \
  (0xe5800000 + ((reg) << 12) + ((addr) << 16) + (imm))
// str reg, [addr, #-(imm)]		@	0 <= imm < 4096
#define STR_IMM_M(reg, addr, imm)                                              \
  (0xe5000000 + ((reg) << 12) + ((addr) << 16) + (imm))
// strh reg, [addr, #imm]		@	0 <= imm < 256
#define STRH_IMM(reg, addr, imm)                                               \
  (0xe1c000b0 + ((reg) << 12) + ((addr) << 16) + (((imm) & 0xf0) << 4) +       \
   ((imm) & 0x0f))
// strh reg, [addr, #-(imm)]		@	0 <= imm < 256
#define STRH_IMM_M(reg, addr, imm)                                             \
  (0xe14000b0 + ((reg) << 12) + ((addr) << 16) + (((imm) & 0xf0) << 4) +       \
   ((imm) & 0x0f))
// strb reg, [addr, #imm]		@	0 <= imm < 4096
#define STRB_IMM(reg, addr, imm)                                               \
  (0xe5c00000 + ((reg) << 12) + ((addr) << 16) + (imm))
// strb reg, [addr, #-(imm)]		@	0 <= imm < 4096
#define STRB_IMM_M(reg, addr, imm)                                             \
  (0xe5400000 + ((reg) << 12) + ((addr) << 16) + (imm))
...
#define SXTH(dst, src, rimm)                                                   \
  (0xe6bf0070 + ((dst) << 12) + (src) + (((rimm) & 24) << 7))
// sxtb dst, src, ror #rimm		@	rimm = 0 | 8 | 16 | 24
#define SXTB(dst, src, rimm)                                                   \
  (0xe6af0070 + ((dst) << 12) + (src) + (((rimm) & 24) << 7))
// uxth dst, src, ror #rimm		@	rimm = 0 | 8 | 16 | 24
#define UXTH(dst, src, rimm)                                                   \
  (0xe6ff0070 + ((dst) << 12) + (src) + (((rimm) & 24) << 7))
// uxtb dst, src, ror #rimm		@	rimm = 0 | 8 | 16 | 24
#define UXTB(dst, src, rimm)                                                   \
  (0xe6ef0070 + ((dst) << 12) + (src) + (((rimm) & 24) << 7))

// bit field
// bfi dst, src, #lsb, #width		@	lsb >= 0, width >= 1, lsb+width
// <= 32
#define BFI(dst, src, lsb, width)                                              \
  (0xe7c00010 + ((dst) << 12) + (src) + ((lsb) << 7) +                         \
   (((lsb) + (width) - 1) << 16))
// bfc dst, #lsb, #width		@	lsb >= 0, width >= 1, lsb+width
// <= 32
#define BFC(dst, lsb, width)                                                   \
  (0xe7c0001f + ((dst) << 12) + ((lsb) << 7) + (((lsb) + (width) - 1) << 16))
#endif
...
static void gen_mov_dword_to_reg_imm(HostReg dest_reg, uint32_t imm) {
  ...
  if (val_is_operand2(imm, &scale)) {
    cache_addd(MOV_IMM(dest_reg, imm >> scale,
                       ROTATE_SCALE(scale))); // mov dest_reg, #imm
  } else if (val_is_operand2(~imm, &scale)) {
    cache_addd(MVN_IMM(dest_reg, (~imm) >> scale,
                       ROTATE_SCALE(scale))); // mvn dest_reg, #~imm
  } else {
    ...
    if (imm == 0) {
      ...
      while (imm) {
        ...
        if (first) {
          cache_addd(MOV_IMM(
              dest_reg, imm & 0xff,
              ROTATE_SCALE(scale))); // mov dest_reg, #((imm & 0xff) << scale)
          first = 0;
        } else {
          cache_addd(ORR_IMM(
              dest_reg, dest_reg, imm & 0xff,
              ROTATE_SCALE(
                  scale))); // orr dest_reg, dest_reg, #((imm & 0xff) << scale)
        }
        imm >>= 8;
        scale += 8;
      }
    ...
    if (imm2 == 0) {
      ...
      while (imm2) {
        ...
        if (first) {
          cache_addd(MVN_IMM(
              dest_reg, imm2 & 0xff,
              ROTATE_SCALE(scale))); // mvn dest_reg, #((imm2 & 0xff) << scale)
          first = 0;
        } else {
          cache_addd(BIC_IMM(
              dest_reg, dest_reg, imm2 & 0xff,
              ROTATE_SCALE(
                  scale))); // bic dest_reg, dest_reg, #((imm2 & 0xff) << scale)
        }
        imm2 >>= 8;
        scale += 8;
      }
...
// helper function
static bool gen_mov_memval_to_reg_helper(HostReg dest_reg, uint32_t data,
  ...
  case 4:
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 4096)) {
        ...
        cache_addd(LDR_IMM_M(
            dest_reg, addr_reg,
            addr_data -
                data)); // ldr dest_reg, [addr_reg, #-(addr_data - data)]
        return true;
      }
    }
    break;
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 256)) {
        cache_addd(LDRH_IMM(
            dest_reg, addr_reg,
            data -
                addr_data)); // ldrh dest_reg, [addr_reg, #(data - addr_data)]
        return true;
      } else if ((data < addr_data) && (data > addr_data - 256)) {
        cache_addd(LDRH_IMM_M(
            dest_reg, addr_reg,
            addr_data -
                data)); // ldrh dest_reg, [addr_reg, #-(addr_data - data)]
        return true;
      }
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 4096)) {
      ...
      cache_addd(LDRB_IMM_M(
          dest_reg, addr_reg,
          addr_data - data)); // ldrb dest_reg, [addr_reg, #-(addr_data - data)]
      return true;
    }
  default:
    break;
...
static bool gen_mov_memval_to_reg(HostReg dest_reg, void *data, Bitu size) {
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint32_t)data, size, FC_REGS_ADDR,
                                   (uint32_t)&cpu_regs))
    return true;
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint32_t)data, size,
                                   readdata_addr,
                                   (uint32_t)&core_dynrec.readdata))
    return true;
  if (gen_mov_memval_to_reg_helper(dest_reg, (uint32_t)data, size, FC_SEGS_ADDR,
                                   (uint32_t)&Segs))
    return true;
  return false;
}
  ...
  if (dword) {
    ...
    if ((uint32_t)data & 3) {
      if (((uint32_t)data & 3) == 2) {
        cache_addd(
            LDRH_IMM(dest_reg, data_reg, 0));     // ldrh dest_reg, [data_reg]
        cache_addd(LDRH_IMM(temp2, data_reg, 2)); // ldrh temp2, [data_reg, #2]
        cache_addd(
            ORR_REG_LSL_IMM(dest_reg, dest_reg, temp2,
                            16)); // orr dest_reg, dest_reg, temp2, lsl #16
      } else {
        cache_addd(
            LDRB_IMM(dest_reg, data_reg, 0));     // ldrb dest_reg, [data_reg]
        cache_addd(LDRH_IMM(temp2, data_reg, 1)); // ldrh temp2, [data_reg, #1]
        cache_addd(ORR_REG_LSL_IMM(dest_reg, dest_reg, temp2,
                                   8)); // orr dest_reg, dest_reg, temp2, lsl #8
        cache_addd(LDRB_IMM(temp2, data_reg, 3)); // ldrb temp2, [data_reg, #3]
        cache_addd(
            ORR_REG_LSL_IMM(dest_reg, dest_reg, temp2,
                            24)); // orr dest_reg, dest_reg, temp2, lsl #24
      }
    } else
#endif
    {
...
// helper function
static bool gen_mov_memval_from_reg_helper(HostReg src_reg, uint32_t data,
                                           Bitu size, HostReg addr_reg,
                                           uint32_t addr_data) {
  switch (size) {
  case 4:
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 4096)) {
        ...
        cache_addd(STR_IMM_M(
            src_reg, addr_reg,
            addr_data - data)); // str src_reg, [addr_reg, #-(addr_data - data)]
        return true;
      }
    }
    break;
  case 2:
...
#endif
    {
      if ((data >= addr_data) && (data < addr_data + 256)) {
        ...
        cache_addd(STRH_IMM_M(
            src_reg, addr_reg,
            addr_data -
                data)); // strh src_reg, [addr_reg, #-(addr_data - data)]
        return true;
      }
    }
    break;
  case 1:
    if ((data >= addr_data) && (data < addr_data + 4096)) {
      ...
      cache_addd(STRB_IMM_M(
          src_reg, addr_reg,
          addr_data - data)); // strb src_reg, [addr_reg, #-(addr_data - data)]
      return true;
    }
  default:
    break;
...
static bool gen_mov_memval_from_reg(HostReg src_reg, void *dest, Bitu size) {
  if (gen_mov_memval_from_reg_helper(src_reg, (uint32_t)dest, size,
                                     FC_REGS_ADDR, (uint32_t)&cpu_regs))
    return true;
  if (gen_mov_memval_from_reg_helper(src_reg, (uint32_t)dest, size,
                                     readdata_addr,
                                     (uint32_t)&core_dynrec.readdata))
...
static void gen_add_imm(HostReg reg, uint32_t imm) {
  ...
  if (val_is_operand2(imm, &scale)) {
    cache_addd(ADD_IMM(reg, reg, imm >> scale,
                       ROTATE_SCALE(scale))); // add reg, reg, #imm
  } else if (val_is_operand2(imm2, &scale)) {
    cache_addd(SUB_IMM(reg, reg, imm2 >> scale,
                       ROTATE_SCALE(scale))); // sub reg, reg, #(-imm)
#if C_TARGETCPU == ARMV7LE
...
static void gen_and_imm(HostReg reg, uint32_t imm) {
  ...
  if (!imm) {
    ...
    cache_addd(AND_IMM(reg, reg, imm >> scale,
                       ROTATE_SCALE(scale))); // and reg, reg, #imm
  } else if (val_is_operand2(imm2, &scale)) {
    cache_addd(BIC_IMM(reg, reg, imm2 >> scale,
                       ROTATE_SCALE(scale))); // bic reg, reg, #(~imm)
#if C_TARGETCPU == ARMV7LE
  } else if (imm2 < 0x10000) {
    cache_addd(MOVW(temp2, imm2));                   // movw temp2, #(~imm)
...
static void gen_sub_direct_word(void *dest, uint32_t imm, bool dword) {
  ...
  if (val_is_operand2(imm, &scale)) {
    cache_addd(SUB_IMM(temp3, temp3, imm >> scale,
                       ROTATE_SCALE(scale))); // sub temp3, temp3, #imm
  } else if (val_is_operand2(imm2, &scale)) {
    cache_addd(ADD_IMM(temp3, temp3, imm2 >> scale,
                       ROTATE_SCALE(scale))); // add temp3, temp3, #(-imm)
#if C_TARGETCPU == ARMV7LE
  } else if (imm2 < 0x10000) {
    cache_addd(MOVW(temp2, imm2)); // movw temp2, #(-imm)
    cache_addd(
...
static INLINE void gen_lea(HostReg dest_reg, HostReg scale_reg, Bitu scale,
                           Bits imm) {
  cache_addd(ADD_REG_LSL_IMM(
      dest_reg, dest_reg, scale_reg,
...
#if C_TARGETCPU == ARMV7LE
  cache_addd(
      MOVW(temp1, ((uint32_t)func) & 0xffff)); // movw temp1, #(func & 0xffff)
...
template <typename T>
static uint32_t INLINE gen_call_function_setup(const T func, Bitu paramcount,
                                               bool fastcall = false) {
...
static void gen_jmp_ptr(void *ptr, Bits imm = 0) {
  ...
  if ((imm & 0x03) == 0) {
    ...
    if ((imm >= 0) && (imm < 4096)) {
      ...
      gen_mov_dword_to_reg_imm(temp2, imm);
      cache_addd(
          LDR_REG_LSL_IMM(temp1, temp3, temp2, 0)); // ldr temp1, [temp3, temp2]
    }
#if !(defined(C_UNALIGNED_MEMORY) || (C_TARGETCPU == ARMV7LE))
  } else {
    gen_add_imm(temp3, imm);
    ...
    cache_addd(ORR_REG_LSL_IMM(temp1, temp1, temp2,
                               8));        // orr temp1, temp1, temp2, lsl #8
    cache_addd(LDRB_IMM(temp2, temp3, 2)); // ldrb temp2, [temp3, #2]
    cache_addd(ORR_REG_LSL_IMM(temp1, temp1, temp2,
                               16));       // orr temp1, temp1, temp2, lsl #16
    cache_addd(LDRB_IMM(temp2, temp3, 3)); // ldrb temp2, [temp3, #3]
    cache_addd(ORR_REG_LSL_IMM(temp1, temp1, temp2,
                               24)); // orr temp1, temp1, temp2, lsl #24
  }
#endif
...
static void INLINE gen_fill_branch(DRC_PTR_SIZE_IM data) {
  ...
  *(uint32_t *)data = ((*(uint32_t *)data) & 0xff000000) |
                      ((((uint32_t)cache.pos - (data + 8)) >> 2) & 0x00ffffff);
}
...
static void gen_run_code(void) {
  ...
  cache_addd(
      MOVW(FC_SEGS_ADDR,
           ((uint32_t)&Segs) & 0xffff)); // movw FC_SEGS_ADDR, #(&Segs & 0xffff)
  cache_addd(MOVT(FC_SEGS_ADDR, ((uint32_t)&Segs) >>
                                    16)); // movt FC_SEGS_ADDR, #(&Segs >> 16)

  cache_addd(MOVW(FC_REGS_ADDR,
                  ((uint32_t)&cpu_regs) &
                      0xffff)); // movw FC_REGS_ADDR, #(&cpu_regs & 0xffff)
  cache_addd(
      MOVT(FC_REGS_ADDR, ((uint32_t)&cpu_regs) >>
                             16)); // movt FC_REGS_ADDR, #(&cpu_regs >> 16)

  cache_addd(MOVW(
      readdata_addr,
      ((Bitu)&core_dynrec.readdata) &
          0xffff)); // movw readdata_addr, #(&core_dynrec.readdata & 0xffff)
  cache_addd(
      MOVT(readdata_addr,
           ((Bitu)&core_dynrec.readdata) >>
               16)); // movt readdata_addr, #(&core_dynrec.readdata >> 16)
  ...
  *(uint32_t *)pos1 =
      LDR_IMM(FC_SEGS_ADDR, HOST_pc,
              cache.pos - (pos1 + 8)); // ldr FC_SEGS_ADDR, [pc, #(&Segs)]
  cache_addd((uint32_t)&Segs);         // address of "Segs"

  *(uint32_t *)pos2 =
      LDR_IMM(FC_REGS_ADDR, HOST_pc,
              cache.pos - (pos2 + 8)); // ldr FC_REGS_ADDR, [pc, #(&cpu_regs)]
  cache_addd((uint32_t)&cpu_regs);     // address of "cpu_regs"

  *(uint32_t *)pos3 = LDR_IMM(
      readdata_addr, HOST_pc,
      cache.pos -
          (pos3 + 8)); // ldr readdata_addr, [pc, #(&core_dynrec.readdata)]
  cache_addd(
      (uint32_t)&core_dynrec.readdata); // address of "core_dynrec.readdata"
...
static void gen_fill_function_ptr(uint8_t *pos, void *fct_ptr,
  ...
  case t_ADDd:
    ...
    *(uint32_t *)(pos + 8) =
        ADD_REG_LSL_IMM(FC_RETOP, HOST_a1, HOST_a2, 0); // add FC_RETOP, a1, a2
#if C_TARGETCPU != ARMV7LE
  ...
  case t_ORd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        ORR_REG_LSL_IMM(FC_RETOP, HOST_a1, HOST_a2, 0); // orr FC_RETOP, a1, a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_ANDd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        AND_REG_LSL_IMM(FC_RETOP, HOST_a1, HOST_a2, 0); // and FC_RETOP, a1, a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_SUBd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        SUB_REG_LSL_IMM(FC_RETOP, HOST_a1, HOST_a2, 0); // sub FC_RETOP, a1, a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_XORd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        EOR_REG_LSL_IMM(FC_RETOP, HOST_a1, HOST_a2, 0); // eor FC_RETOP, a1, a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_INCd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        ADD_IMM(FC_RETOP, HOST_a1, 1, 0); // add FC_RETOP, a1, #1
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_DECd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        SUB_IMM(FC_RETOP, HOST_a1, 1, 0); // sub FC_RETOP, a1, #1
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  ...
  case t_SHLd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        MOV_REG_LSL_REG(FC_RETOP, HOST_a1, HOST_a2); // mov FC_RETOP, a1, lsl a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  case t_SHRb:
    ...
    *(uint32_t *)(pos + 8) =
        MOV_REG_LSR_REG(FC_RETOP, HOST_a1, HOST_a2); // mov FC_RETOP, a1, lsr a2
#else
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        AND_IMM(FC_RETOP, HOST_a1, 0xff, 0); // and FC_RETOP, a1, #0xff
    *(uint32_t *)(pos + 12) = MOV_REG_LSR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, lsr a2
#endif
    break;
  case t_SHRw:
    *(uint32_t *)pos = NOP; // nop
...
#else
    *(uint32_t *)(pos + 4) =
        MOV_REG_LSL_IMM(FC_RETOP, HOST_a1, 16); // mov FC_RETOP, a1, lsl #16
    *(uint32_t *)(pos + 8) = MOV_REG_LSR_IMM(
        FC_RETOP, FC_RETOP, 16); // mov FC_RETOP, FC_RETOP, lsr #16
    *(uint32_t *)(pos + 12) = MOV_REG_LSR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, lsr a2
#endif
    break;
  ...
  case t_SARb:
    ...
    *(uint32_t *)(pos + 8) = MOV_REG_ASR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, asr a2
#else
    *(uint32_t *)(pos + 4) =
        MOV_REG_LSL_IMM(FC_RETOP, HOST_a1, 24); // mov FC_RETOP, a1, lsl #24
    *(uint32_t *)(pos + 8) = MOV_REG_ASR_IMM(
        FC_RETOP, FC_RETOP, 24); // mov FC_RETOP, FC_RETOP, asr #24
    *(uint32_t *)(pos + 12) = MOV_REG_ASR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, asr a2
#endif
    break;
  case t_SARw:
    ...
    *(uint32_t *)(pos + 4) =
        MOV_REG_LSL_IMM(FC_RETOP, HOST_a1, 16); // mov FC_RETOP, a1, lsl #16
    *(uint32_t *)(pos + 8) = MOV_REG_ASR_IMM(
        FC_RETOP, FC_RETOP, 16); // mov FC_RETOP, FC_RETOP, asr #16
    *(uint32_t *)(pos + 12) = MOV_REG_ASR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, asr a2
#endif
    break;
  case t_SARd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        MOV_REG_ASR_REG(FC_RETOP, HOST_a1, HOST_a2); // mov FC_RETOP, a1, asr a2
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
  case t_RORb:
    ...
    *(uint32_t *)(pos + 4) =
        BFI(HOST_a1, HOST_a1, 16, 16); // bfi a1, a1, 16, 16
    *(uint32_t *)(pos + 8) =
        MOV_REG_ROR_REG(FC_RETOP, HOST_a1, HOST_a2); // mov FC_RETOP, a1, ror a2
#else
    *(uint32_t *)pos =
        MOV_REG_LSL_IMM(FC_RETOP, HOST_a1, 24); // mov FC_RETOP, a1, lsl #24
    *(uint32_t *)(pos + 4) =
        ORR_REG_LSR_IMM(FC_RETOP, FC_RETOP, FC_RETOP,
                        8); // orr FC_RETOP, FC_RETOP, FC_RETOP, lsr #8
    *(uint32_t *)(pos + 8) =
        ORR_REG_LSR_IMM(FC_RETOP, FC_RETOP, FC_RETOP,
                        16); // orr FC_RETOP, FC_RETOP, FC_RETOP, lsr #16
    *(uint32_t *)(pos + 12) = MOV_REG_ROR_REG(
        FC_RETOP, FC_RETOP, HOST_a2); // mov FC_RETOP, FC_RETOP, ror a2
#endif
    break;
  case t_RORw:
    *(uint32_t *)pos = NOP; // nop
  ...
  case t_ROLw:
    ...
    *(uint32_t *)(pos + 4) =
        RSB_IMM(HOST_a2, HOST_a2, 32, 0); // rsb a2, a2, #32
    *(uint32_t *)(pos + 8) =
        MOV_REG_ROR_REG(FC_RETOP, HOST_a1, HOST_a2); // mov FC_RETOP, a1, ror a2
#else
    *(uint32_t *)pos =
  ...
  case t_NEGd:
    ...
    *(uint32_t *)(pos + 4) = NOP; // nop
    *(uint32_t *)(pos + 8) =
        RSB_IMM(FC_RETOP, HOST_a1, 0, 0); // rsb FC_RETOP, a1, #0
#if C_TARGETCPU != ARMV7LE
    *(uint32_t *)(pos + 12) = NOP; // nop
#endif
    break;
...
#if C_TARGETCPU == ARMV7LE
    *(uint32_t *)pos = MOVW(
        temp1, ((uint32_t)fct_ptr) & 0xffff); // movw temp1, #(fct_ptr & 0xffff)
    *(uint32_t *)(pos + 4) =
        MOVT(temp1, ((uint32_t)fct_ptr) >> 16); // movt temp1, #(fct_ptr >> 16)
#else
...
#else
#if C_TARGETCPU == ARMV7LE
  *(uint32_t *)pos = MOVW(temp1, ((uint32_t)fct_ptr) &
                                     0xffff); // movw temp1, #(fct_ptr & 0xffff)
  *(uint32_t *)(pos + 4) =
      MOVT(temp1, ((uint32_t)fct_ptr) >> 16); // movt temp1, #(fct_ptr >> 16)
#else
  *(uint32_t *)(pos + 12) = (uint32_t)fct_ptr; // simple_func
