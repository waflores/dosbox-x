...
public:
  ...
  struct {
    ...
    uint8_t *xstart; // where in the cache are we, executable mapping (because
                     // so much of the risc generation points at this pointer!)
    Bitu size;
...
public:
  ...
  bool InvalidateRange(Bitu start, Bitu end) {
    ...
    bool is_current_block = false; // if the current block is modified, it has
                                   // to be exited as soon as possible

    uint32_t ip_point = SegPhys(cs) + reg_eip;
    ip_point =
        (uint32_t)((PAGING_GetPhysicalPage(ip_point) - (phys_page << 12)) +
                   (ip_point & 0xfff));
    while (index >= 0) {
      Bitu map = 0;
      // see if there is still some code in the range
      for (Bitu count = start; count <= end; count++)
        map += write_map[count];
      if (!map)
      ...
      while (block) {
        ...
        if (start <= block->page.end && end >= block->page.start) {
          ...
          block->Clear(); // clear the block, decrements the write_map
                          // accordingly
        }
        block = nextblock;
      }
  ...
  // the following functions will clean all cache blocks that are invalid now
  // due to the write
  ...
  void AddCacheBlock(CacheBlockDynRec *block) {
    ...
    block->hash.next =
        hash_map[index]; // link to old block at index from the new block
  ...
  void Release(void) {
    ...
    if (prev)
      ...
      cache.used_pages = next;
    if (next)
      next->prev = prev;
    else
      cache.last_page = prev;
...
public:
  // the write map, there are write_map[i] cache blocks that cover the byte at
  // address i
  ...
  if (hash.index)
    for (Bitu ind = 0; ind < 2; ind++) {
      ...
      if (link[ind].to != &link_blocks[ind]) {
        // not linked to the standard linkcode, find the block that links to
        // this block
...
static void cache_closeblock(void) {
    ...
    if (!block->cache.next) {
      if (written > block->cache.size + CACHE_MAXSIZE)
        E_Exit("CacheBlock overrun 1 %lu",
               (unsigned long)written - block->cache.size);
    } else
      E_Exit("CacheBlock overrun 2 written %lu size %lu",
             (unsigned long)written, (unsigned long)block->cache.size);
  } else {
  ...
  if (!block->cache.next ||
      (block->cache.next->cache.start >
       (cache_code_start_ptr + CACHE_TOTAL - CACHE_MAXSIZE))) {
...
static INLINE void cache_addb(uint8_t val, uint8_t *pos) { *pos = val; }
...
static void cache_ensure_allocation(void) {
  if (cache_code_start_ptr == NULL) {
    cache_dynamic_common_alloc(
        CACHE_TOTAL + CACHE_MAXSIZE); /* sets cache_code_start_ptr/cache_code */
...
static void cache_reset(void) {
  if (cache_initialized) {
    ...
    if (cache_blocks == NULL) {
      cache_blocks =
          (CacheBlockDynRec *)malloc(CACHE_BLOCKS * sizeof(CacheBlockDynRec));
      if (!cache_blocks)
    ...
    link_blocks[0].cache.xstart =
        (uint8_t *)cache_rwtox(link_blocks[0].cache.start);
    dyn_return(BR_Link1, false);
    cache.pos = &cache_code_link_blocks[32];
    link_blocks[1].cache.start = cache.pos;
    link_blocks[1].cache.xstart =
        (uint8_t *)cache_rwtox(link_blocks[1].cache.start);
