...
typedef void(APIENTRYP PFNGLBUFFERDATAARBPROC)(GLenum target, GLsizeiptr size,
                                               const GLvoid *data,
                                               GLenum usage);
...
static_assert(std::is_pod<SDL_OpenGL>::value,
              "SDL_OpenGL must be POD, otherwise memset() is undefined");
...
static char const shader_src_default[] =
    ...
    "  v_texCoord = "
    "vec2(a_position.x+1.0,1.0-a_position.y)/2.0*rubyInputSize/"
    "rubyTextureSize;\n"
    "}\n"
