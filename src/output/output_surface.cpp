...
void OUTPUT_SURFACE_Initialize() {
  // Apparently if the window size changes rapidly enough, SDL2 can be tricked
  // into blitting the wrong dimensions to the window and trigger an X11
  // BadValue error. Set up an error handler that prints the error to STDERR and
  // then returns, instead of the default handler which prints an error and
  // exit()s this program.
...
Bitu OUTPUT_SURFACE_SetSize() {
  ...
  if (GFX_IsFullscreen()) {
    if (sdl.desktop.full.fixed) {
      ...
      sdl.window = GFX_SetSDLWindowMode(
          sdl.desktop.full.width, sdl.desktop.full.height, SCREEN_SURFACE);
      if (sdl.window == NULL)
        E_Exit("Could not set fullscreen video mode %ix%i-%i: %s",
               sdl.desktop.full.width, sdl.desktop.full.height, sdl.desktop.bpp,
               SDL_GetError());
    } else {
      sdl.clip.x = 0;
      sdl.clip.y = 0;
      sdl.window =
          GFX_SetSDLWindowMode(sdl.draw.width, sdl.draw.height, SCREEN_SURFACE);
      if (sdl.window == NULL)
    ...
    /* scale the menu bar if the window is large enough */
    {
      ...
      Bitu final_height = max(max(consider_height, userResizeWindowHeight),
                              (Bitu)(sdl.clip.y + sdl.clip.h));
      Bitu final_width = max(max(consider_width, userResizeWindowWidth),
                             (Bitu)(sdl.clip.x + sdl.clip.w));
      Bitu scale = 1;

      while ((final_width / scale) >= (640 * 2) &&
             (final_height / scale) >= (400 * 2))
        scale++;
    ...
    if (mainMenu.isVisible())
      menuheight = mainMenu.menuBox.h;
#endif

    /* menu size and consideration of width and height */
    Bitu consider_height =
        height + (unsigned int)menuheight + (sdl.overscan_width * 2);
    ...
    if (mainMenu.isVisible()) {
      ...
      if (consider_height <
          (min_sdldraw_menu_height + (sdl.overscan_width * 2) +
           (unsigned int)menuheight))
        consider_height = (min_sdldraw_menu_height + (sdl.overscan_width * 2) +
                           (unsigned int)menuheight);
    }
#endif
    ...
    if (sdl_xbrz.enable) {
      final_height = (int)max(consider_height, userResizeWindowHeight) -
                     (int)menuheight - ((int)sdl.overscan_width * 2);
      final_width = (int)max(consider_width, userResizeWindowWidth) -
                    ((int)sdl.overscan_width * 2);

      sdl.clip.x = sdl.clip.y = 0;
      sdl.clip.w = final_width;
      sdl.clip.h = final_height;
      if (render.aspect)
        aspectCorrectFitClip(sdl.clip.w, sdl.clip.h, sdl.clip.x, sdl.clip.y,
    ...
    /* center the screen in the window */
    {
      ...
      final_height = (int)max(max(consider_height, userResizeWindowHeight),
                              (Bitu)(sdl.clip.y + sdl.clip.h)) -
                     (int)menuheight - ((int)sdl.overscan_width * 2);
      final_width = (int)max(max(consider_width, userResizeWindowWidth),
                             (Bitu)(sdl.clip.x + sdl.clip.w)) -
                    ((int)sdl.overscan_width * 2);
...

    {
      ...
      LOG_MSG("surface consider=%ux%u final=%ux%u",
              (unsigned int)consider_width, (unsigned int)consider_height,
              (unsigned int)final_width, (unsigned int)final_height);
    }

    sdl.window =
        GFX_SetSDLWindowMode(final_width, final_height, SCREEN_SURFACE);
    if (sdl.window == NULL)
      E_Exit("Could not set windowed video mode %ix%i: %s", (int)sdl.draw.width,
             (int)sdl.draw.height, SDL_GetError());
    ...
    if (sdl.surface->w < (sdl.clip.x + sdl.clip.w) ||
      ...
      SDL_SetWindowMinimumSize(sdl.window, sdl.clip.x + sdl.clip.w,
                               sdl.clip.y + sdl.clip.h);
      sdl.window = GFX_SetSDLWindowMode(
          sdl.clip.x + sdl.clip.w, sdl.clip.y + sdl.clip.h, SCREEN_SURFACE);
      if (sdl.window == NULL)
        E_Exit("Could not set windowed video mode %ix%i: %s",
               (int)sdl.draw.width, (int)sdl.draw.height, SDL_GetError());
    }
  }
  sdl.surface = SDL_GetWindowSurface(sdl.window);
  if (sdl.surface == NULL)
  ...
  if (sdl_xbrz.enable) {
    ...
    xBRZ_SetScaleParameters(sdl.draw.width, sdl.draw.height, sdl.clip.w,
                            sdl.clip.h);
    if (sdl_xbrz.scale_on != old_scale_on) {
      // when we are scaling, we ask render code not to do any aspect correction
      // when we are not scaling, render code is allowed to do aspect correction
      // at will due to this, at each scale mode change we need to schedule
      // resize again because window size could change
      PIC_AddEvent(
          VGA_SetupDrawing,
          50); // schedule another resize here, render has already been
               // initialized at this point and we have just changed its option
    }
  }
#endif

  /* WARNING: If the user is resizing our window to smaller than what we want,
   * SDL2 will give us a window surface according to the smaller size, and then
   * we crash!
   *
   *          To avoid this crash, disable rendering until the window is big
   * enough again. */
  if (sdl.surface->w < (sdl.clip.x + sdl.clip.w) ||
      sdl.surface->h < (sdl.clip.y + sdl.clip.h))
...
Bitu OUTPUT_SURFACE_GetBestMode(Bitu flags) {
  ...
  flags &=
      ~GFX_SCALING; // This output does not scale, and the aspect ratio
                    // correction looks like 1990s laptop hardware scaler crap
  if (flags & GFX_LOVE_8)
    testbpp = 8;
  ...
  if (sdl.desktop.fullscreen)
    gotbpp = (unsigned int)SDL_VideoModeOK(640, 480, (int)testbpp,
                                           (unsigned int)SDL_FULLSCREEN |
                                               (unsigned int)SDL_HWSURFACE |
                                               (unsigned int)SDL_HWPALETTE);
  else
    gotbpp = sdl.desktop.bpp;

  /* SDL 1.x and sometimes SDL 2.x mistake 15-bit 5:5:5 RGB for 16-bit 5:6:5 RGB
   * which causes colors to mis-display. This seems to be common with Windows
   * and Linux. If SDL said 16-bit but the bit masks suggest 15-bit, then make
   * the correction now. */
  if (gotbpp == 16) {
    if (sdl.surface->format->Gshift == 5 &&
        sdl.surface->format->Gmask == (31U << 5U)) {
      LOG_MSG("NOTE: SDL returned 16-bit/pixel mode (5:6:5) but failed to "
              "recognize your screen is 15-bit/pixel mode (5:5:5)");
      gotbpp = 15;
    }
  }
...
Bitu OUTPUT_SURFACE_SetSize() {
  ...
  // localize width and height variables because they can be locally adjusted by
...
retry:
  ...
  But on Windows, we're still required to ask for 16bpp to get the 15bpp mode we
  want. */
  if (bpp == 15) {
    if (sdl.surface->format->Gshift == 5 &&
        sdl.surface->format->Gmask == (31U << 5U)) {
      LOG_MSG("SDL hack: Asking for 16-bit color (5:6:5) to get SDL to give us "
              "15-bit color (5:5:5) to match your screen.");
      bpp = 16;
    }
  }
#endif
  ...
  // there is a small problem we need to solve here: aspect corrected windows
  // can be smaller than needed due to source with non-4:3 pixel ratio if we
  // detect non-4:3 pixel ratio here with aspect correction on, we correct it so
  // original fits into resized window properly
  if (render.aspect)
    aspectCorrectExtend(width, height);
#endif

  sdl.clip.w = (Uint16)width;
  sdl.clip.h = (Uint16)height;
  if (sdl.desktop.fullscreen) {
    Uint32 wflags =
    ...
    if (sdl.desktop.full.fixed) {
      ...
      int fw =
          (std::max)((int)sdl.desktop.full.width, (sdl.clip.x + sdl.clip.w));
      int fh =
          (std::max)((int)sdl.desktop.full.height, (sdl.clip.y + sdl.clip.h));
...

    {
      ...
      sdl.surface = SDL_SetVideoMode(
          (int)final_width, (int)final_height, (int)bpp,
          (unsigned int)((sdl.draw.flags & GFX_CAN_RANDOM) ? SDL_SWSURFACE
                                                           : SDL_HWSURFACE) |
#ifdef SDL_DOSBOX_X_SPECIAL
              (unsigned int)SDL_HAX_NOREFRESH |
              (unsigned int)(setSizeButNotResize() ? SDL_HAX_NORESIZEWINDOW
                                                   : 0) |
#endif
      ...
      memset(sdl.surface->pixels, 0,
             (unsigned int)sdl.surface->pitch * (unsigned int)sdl.surface->h);
    ...
    if (sdl.surface == NULL) {
      ...
      if (!sdl.using_windib) {
        LOG_MSG("Failed to create hardware surface.\nRestarting video "
                "subsystem with windib enabled.");
        putenv("SDL_VIDEODRIVER=windib");
        sdl.using_windib = true;
      } else {
        LOG_MSG("Failed to create hardware surface.\nRestarting video "
                "subsystem with directx enabled.");
      ...
      sdl.surface =
          SDL_SetVideoMode((int)width, (int)height, (int)bpp, SDL_HWSURFACE);
      sdl.deferred_resize = false;
      sdl.must_redraw_all = true;
...
#endif
    if (sdl.surface == NULL)
      E_Exit("Could not set windowed video mode %ix%i-%i: %s", (int)width,
             (int)height, (int)bpp, SDL_GetError());
  }
  ...
  if (sdl.surface) {
    ...
    if (retFlags && (sdl.surface->flags & SDL_DOUBLEBUF)) {
      sdl.blit.surface = SDL_CreateRGBSurface(
          (Uint32)SDL_HWSURFACE, (int)sdl.draw.width, (int)sdl.draw.height,
          (int)sdl.surface->format->BitsPerPixel,
          (Uint32)sdl.surface->format->Rmask,
          (Uint32)sdl.surface->format->Gmask,
          (Uint32)sdl.surface->format->Bmask, (Uint32)0u);
    ...
    if (sdl_xbrz.enable) {
      bool old_scale_on = sdl_xbrz.scale_on;
      xBRZ_SetScaleParameters((int)sdl.draw.width, (int)sdl.draw.height,
                              sdl.clip.w, sdl.clip.h);
      if (sdl_xbrz.scale_on != old_scale_on) {
        // when we are scaling, we ask render code not to do any aspect
        // correction when we are not scaling, render code is allowed to do
        // aspect correction at will due to this, at each scale mode change we
        // need to schedule resize again because window size could change
        PIC_AddEvent(VGA_SetupDrawing,
                     50); // schedule another resize here, render has already
                          // been initialized at this point and we have just
                          // changed its option
      }
    }
#endif
...
bool OUTPUT_SURFACE_StartUpdate(uint8_t *&pixels, Bitu &pitch) {
  ...
  if (sdl_xbrz.enable && sdl_xbrz.scale_on) {
    ...
    pixels = sdl_xbrz.renderbuf.empty()
                 ? nullptr
                 : reinterpret_cast<uint8_t *>(&sdl_xbrz.renderbuf[0]);
    ...
    pixels = sdl.aspectbuf.empty()
                 ? nullptr
                 : reinterpret_cast<uint8_t *>(&sdl.aspectbuf[0]);
    pitch = sdl.draw.width * sizeof(uint32_t);
  } else
#endif
  {
...
void OUTPUT_SURFACE_EndUpdate(const uint16_t *changedLines) {
  ...
  if (sdl_xbrz.enable && sdl_xbrz.scale_on) {
    ...
    if (sdl_xbrz.renderbuf.size() ==
            (unsigned int)srcWidth * (unsigned int)srcHeight &&
        srcWidth > 0 && srcHeight > 0) {
      // please use sdl.clip to keep screen positioning consistent with the rest
      // of the emulator
      ...
      const uint32_t *renderBuf =
          &sdl_xbrz.renderbuf[0]; // help VS compiler a little + support capture
                                  // by value
      xbrzBuf = &sdl_xbrz.pixbuf[0];
      xBRZ_Render(renderBuf, xbrzBuf, changedLines, (int)srcWidth,
                  (int)srcHeight, sdl_xbrz.scale_factor);

      // 2. nearest neighbor/bilinear scale xbrz buffer into output surface
      // clipping area
      const bool mustLock = SDL_MUSTLOCK(sdl.surface);
      if (mustLock)
        SDL_LockSurface(sdl.surface);
      if (sdl.surface->pixels) // if locking fails, this can be nullptr, also
                               // check if we really need to draw
      {
        uint32_t *clipTrg = reinterpret_cast<uint32_t *>(
            static_cast<char *>(sdl.surface->pixels) +
            clipY * sdl.surface->pitch +
            (unsigned int)clipX * sizeof(uint32_t));
        xBRZ_PostScale(&xbrzBuf[0], (int)xbrzWidth, (int)xbrzHeight,
                       (int)(xbrzWidth * sizeof(uint32_t)), &clipTrg[0],
                       clipWidth, clipHeight, sdl.surface->pitch,
                             ...
                             // check if we really need to draw
    {
      uint32_t *clipTrg = reinterpret_cast<uint32_t *>(
          static_cast<char *>(sdl.surface->pixels) +
          clipY * sdl.surface->pitch + clipX * sizeof(uint32_t));
      xBRZ_PostScale(&sdl.aspectbuf[0], sdl.draw.width, sdl.draw.height,
                     sdl.draw.width * sizeof(uint32_t), &clipTrg[0], clipWidth,
                     clipHeight, sdl.surface->pitch,
                     (render.aspect == ASPECT_BILINEAR),
                     C_SURFACE_POSTRENDER_ASPECT_BATCH_SIZE);
    }
...
#endif /*C_SURFACE_POSTRENDER_ASPECT*/
  {
    if (SDL_MUSTLOCK(sdl.surface)) {
      if (sdl.blit.surface) {
        ...
        int Blit =
            SDL_BlitSurface(sdl.blit.surface, nullptr, sdl.surface, &sdl.clip);
