...
void d3d_init(void) {
  ...
  if (!sdl.using_windib &&
      (sdl_videodrv == NULL || (strcasecmp(sdl_videodrv, "dummy") &&
                                strcasecmp(sdl_videodrv, "directx")))) {
...
#if defined(C_SDL2)
    else if (d3d->InitializeDX(wmi.info.win.window, sdl.desktop.doublebuf) !=
             S_OK)
#else
...
Bitu OUTPUT_DIRECT3D_SetSize() {
  ...
  // when xBRZ scaler is used, we can adjust render target size to exactly what
  // xBRZ scaler will output, leaving final scaling to default D3D scaler /
  // shaders
#if C_XBRZ
  if (sdl_xbrz.enable &&
  ...
  if ((!d3d->square) && (!d3d->pow2)) {
    ...
    int texsize =
        2 << int_log2(
            (int)(adjTexWidth > adjTexHeight ? adjTexWidth : adjTexHeight));
  ...
  LOG(LOG_MISC,
      LOG_DEBUG)("GFX_SetSize Direct3D texture=%ux%u window=%ux%u "
                 "clip=x,y,w,h=%d,%d,%d,%d",
                 (unsigned int)d3d->dwTexWidth, (unsigned int)d3d->dwTexHeight,
  ...
  if (section) {
    ...
    std::string path = prop->GetSection()->Get_string("type");
    ResolvePath(path);
    d3d->LoadPixelShader(path.c_str(), sdl.draw.scalex, sdl.draw.scaley,
                         (f == "forced"));
  } else {
  ...
  // D3D will hang or crash when using fullscreen with ddraw surface, therefore
  // we hack SDL to provide a GDI window with an additional 0x40 flag. If this
  // fails or stock SDL is used, use WINDIB output
  void GFX_SetResizeable(bool enable);
  if (GCC_UNLIKELY(d3d->bpp16)) {
#if defined(C_SDL2)
    GFX_SetResizeable(true);
    sdl.window =
        GFX_SetSDLWindowMode(windowWidth, windowHeight, SCREEN_SURFACE);
    if (sdl.window != NULL)
    ...
    sdl.surface = SDL_SetVideoMode(
        windowWidth, windowHeight, 16,
        sdl.desktop.fullscreen ? SDL_FULLSCREEN | 0x40 : SDL_RESIZABLE | 0x40);
  ...
  if (sdl.surface == NULL)
    E_Exit("Could not set video mode %ix%i-%i: %s", sdl.clip.w, sdl.clip.h,
           d3d->bpp16 ? 16 : 32, SDL_GetError());

  if (d3d->dynamic)
    retFlags |= GFX_HARDWARE;

  if (GCC_UNLIKELY(d3d->Resize3DEnvironment(
                       windowWidth, windowHeight, sdl.clip.x, sdl.clip.y,
                       sdl.clip.w, sdl.clip.h, adjTexWidth, adjTexHeight,
                       sdl.desktop.fullscreen) != S_OK))
    retFlags = 0;

#if LOG_D3D
  LOG_MSG("SDL:D3D:Display mode set to: %dx%d with %fx%f scale", sdl.clip.w,
          sdl.clip.h, sdl.draw.scalex, sdl.draw.scaley);
#endif
...
#if C_XBRZ
  if (sdl_xbrz.enable && sdl_xbrz.scale_on) {
    // we have xBRZ pseudo render buffer to be output to the pre-sized texture,
    // do the xBRZ part
    const int srcWidth = sdl.draw.width;
    const int srcHeight = sdl.draw.height;
    if (sdl_xbrz.renderbuf.size() == srcWidth * srcHeight && srcWidth > 0 &&
        srcHeight > 0) {
      ...
      xBRZ_Render(renderBuf, xbrzBuf, changedLines, srcWidth, srcHeight,
                  sdl_xbrz.scale_factor);

      // D3D texture can be not of exactly size we expect, so we copy xBRZ
      // buffer to the texture there, adjusting for texture pitch
      uint8_t *tgtPix;
      Bitu tgtPitch;
      if (d3d->LockTexture(tgtPix, tgtPitch) &&
          tgtPix) // if locking fails, target texture can be nullptr
      {
        ...
        for (int i = 0; i < xbrzHeight; i += sdl_xbrz.task_granularity) {
          tg.run([=] {
            ...
            xbrz::pitchChange(&xbrzBuf[0], &tgtTex[0], xbrzWidth, xbrzHeight,
                              (int)(xbrzWidth * sizeof(uint32_t)),
                              (int)tgtPitch, i, iLast,
...
#else
        xbrz::pitchChange(&xbrzBuf[0], &tgtTex[0], xbrzWidth, xbrzHeight,
                          xbrzWidth * sizeof(uint32_t), tgtPitch, 0, xbrzHeight,
                          [](uint32_t pix) { return pix; });
#endif
      }
    }
