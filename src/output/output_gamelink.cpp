...
void OUTPUT_GAMELINK_Select() {
  ...
  if (!sdl.gamelink.enable) {
    ...
    LOG_MSG("OUTPUT_GAMELINK: Not enabled via `gamelink master = true`, "
            "falling back to `output=opengl`.");
    OUTPUT_OPENGL_Select(GLBilinear);
#else  // MACOSX
    LOG_MSG("OUTPUT_GAMELINK: Not enabled via `gamelink master = true`, "
            "falling back to `output=surface`.");
...
void OUTPUT_GAMELINK_InputEvent() {
  ...
  if (GameLink::In(&sdl.gamelink.input, &sdl.gamelink.audio)) {
    ...
    Mouse_CursorMoved(sdl.gamelink.input.mouse_dx, sdl.gamelink.input.mouse_dy,
                      0, 0, true /*emulate*/);
    ...
    for (uint8_t blk = 0; blk < 8; ++blk) {
      ...
      for (uint8_t bit = 0; bit < 32; ++bit) {
        const SDL_Scancode scancode =
            windows_scancode_table[static_cast<int>((blk * 32) + bit)];
    ...
    memcpy(&sdl.gamelink.input_prev, &sdl.gamelink.input,
           sizeof(GameLink::sSharedMMapInput_R2));
  }
}

bool OUTPUT_GAMELINK_InitTrackingMode() {
  // LOG_MSG("OUTPUT_GAMELINK: Game Link %s",
  // sdl.gamelink.enable?"enabled":"disabled");
  if (!sdl.gamelink.enable)
    return false;
  ...
  if (iresult != 1) {
    ...
    MessageBoxA(
        NULL,
        "ERROR: Couldn't initialise Game Link inter-process communication.",
        "DOSBox \"Game Link\" Error", MB_OK | MB_ICONSTOP);
#else  // WIN32
...
Bitu OUTPUT_GAMELINK_SetSize() {
  ...
  if (!sdl.gamelink.framebuf) {
    sdl.gamelink.framebuf = malloc(SCALER_MAXWIDTH * SCALER_MAXHEIGHT *
                                   4); // 32 bit color frame buffer
  }
  ...
  LOG_MSG("GAMELINK: rendersize=%ux%u", (unsigned int)sdl.draw.width,
          (unsigned int)sdl.draw.height);

#if C_XBRZ
  if (sdl_xbrz.enable) {
    bool old_scale_on = sdl_xbrz.scale_on;
    xBRZ_SetScaleParameters(sdl.draw.width, sdl.draw.height,
  ...
  if (sdl.clip.w > GameLink::sSharedMMapFrame_R1::MAX_WIDTH ||
      sdl.clip.h > GameLink::sSharedMMapFrame_R1::MAX_HEIGHT) {
#ifdef WIN32
    MessageBoxA(
        NULL, "ERROR: Game Link output resolution too big (windowresolution).",
        "DOSBox \"Game Link\" Error", MB_OK | MB_ICONSTOP);
#else  // WIN32
  ...
  // FIXME: find proper way to refresh the menu bar without causing infinite
  // recursion
...
void OUTPUT_GAMELINK_Transfer() {
  ...
  GameLink::Out((uint16_t)sdl.clip.w + 2 * sdl.clip.x,
                (uint16_t)sdl.clip.h + 2 * sdl.clip.y, render.src.ratio,
                sdl.gamelink.want_mouse, RunningProgram, RunningProgramHash,
                (const uint8_t *)sdl.gamelink.framebuf, MemBase);
}
  ...
  if (sdl_xbrz.enable && sdl_xbrz.scale_on) {
    ...
    if (sdl_xbrz.renderbuf.size() ==
      ...
      uint32_t *clipTrg = reinterpret_cast<uint32_t *>(
          static_cast<char *>(sdl.gamelink.framebuf) +
          clipY * sdl.gamelink.pitch + (unsigned int)clipX * sizeof(uint32_t));
      xBRZ_PostScale(&xbrzBuf[0], (int)xbrzWidth, (int)xbrzHeight,
                     (int)(xbrzWidth * sizeof(uint32_t)), &clipTrg[0],
                     clipWidth, clipHeight, sdl.gamelink.pitch,
