...
static void PPScale(uint16_t fixed_w, uint16_t fixed_h, uint16_t *window_w,
                    uint16_t *window_h) {
  ...
  int x = (aspect_ratio_x > 0 && aspect_ratio_y > 0)
              ? aspect_ratio_x
              : ((aspect_ratio_x == -1 && aspect_ratio_y == -1) ? sdl.draw.width
                                                                : 4);
  int y =
      (aspect_ratio_x > 0 && aspect_ratio_y > 0)
          ? aspect_ratio_y
          : ((aspect_ratio_x == -1 && aspect_ratio_y == -1) ? sdl.draw.height
                                                            : 3);
  ...
  *window_h = fixed_h;
  ...
  if (fixed_w < min_w)
    fixed_w = *window_w = min_w;
  if (fixed_h < min_h)
    fixed_h = *window_h = min_h;

  pp_getscale(orig_w, orig_h, par, fixed_w, fixed_h, 1.14, &sx, &sy);
  ...
  LOG_MSG("OpenGL PP: [%ix%i]: %ix%i (%3.2f) -> [%ix%i] -> %ix%i (%3.2f)",
          fixed_w, fixed_h, orig_w, orig_h, par, sx, sy, sdl.clip.w, sdl.clip.h,
          (double)sy / sx);
}
...
retry:
  ...
  /* SDL drawn menus cannot coexist with 3Dfx emulation. In fact, there is a
   * serious bug in SDL1 builds that rapidly expands the vertical size of the
   * menu every frame. */
  if (Voodoo_OGL_GetWidth() != 0 && Voodoo_OGL_GetHeight() != 0 &&
      Voodoo_OGL_Active() && sdl.desktop.prevent_fullscreen) {
  } else {
  ...
  if (Voodoo_OGL_GetWidth() != 0 && Voodoo_OGL_GetHeight() != 0 &&
    ...
    if (render.aspect)
      aspectCorrectExtend(windowWidth, windowHeight);
    sdl.clip.w = windowWidth;
    sdl.clip.h = windowHeight;
  }

  LOG(LOG_MISC,
      LOG_DEBUG)("GFX_SetSize OpenGL window=%ux%u clip=x,y,w,h=%d,%d,%d,%d",
                 (unsigned int)windowWidth, (unsigned int)windowHeight,
                 (unsigned int)sdl.clip.x, (unsigned int)sdl.clip.y,
                 (unsigned int)sdl.clip.w, (unsigned int)sdl.clip.h);
  ...
  sdl.window = GFX_SetSDLWindowMode(
      ...
      (sdl_flags & SDL_WINDOW_OPENGL) ? SCREEN_OPENGL : SCREEN_SURFACE);
  if (sdl.window != NULL)
    sdl.surface = SDL_GetWindowSurface(sdl.window);
#elif defined(SDL_DOSBOX_X_SPECIAL)
  sdl.surface = SDL_SetVideoMode(
      windowWidth, windowHeight, (int)bpp,
      (unsigned int)sdl_flags |
          (unsigned int)(setSizeButNotResize() ? SDL_HAX_NORESIZEWINDOW : 0));
#else
  sdl.surface = SDL_SetVideoMode(windowWidth, windowHeight, (int)bpp,
                                 (unsigned int)sdl_flags);
#endif
...
void OUTPUT_OPENGL_Initialize() { memset(&sdl_opengl, 0, sizeof(sdl_opengl)); }
...
void OUTPUT_OPENGL_Select(GLKind kind) {
  ...
  if (sdl.surface == NULL) {
    ...
    glShaderSource =
        (PFNGLSHADERSOURCEPROC_NP)SDL_GL_GetProcAddress("glShaderSource");
    glUniform2f = (PFNGLUNIFORM2FPROC)SDL_GL_GetProcAddress("glUniform2f");
    glUniform1i = (PFNGLUNIFORM1IPROC)SDL_GL_GetProcAddress("glUniform1i");
    glUseProgram = (PFNGLUSEPROGRAMPROC)SDL_GL_GetProcAddress("glUseProgram");
    glVertexAttribPointer = (PFNGLVERTEXATTRIBPOINTERPROC)SDL_GL_GetProcAddress(
        "glVertexAttribPointer");
    sdl_opengl.use_shader =
        (glAttachShader && glCompileShader && glCreateProgram &&
         glDeleteProgram && glDeleteShader && glEnableVertexAttribArray &&
         glGetAttribLocation && glGetProgramiv && glGetProgramInfoLog &&
         glGetShaderiv && glGetShaderInfoLog && glGetUniformLocation &&
         glLinkProgram && glShaderSource && glUniform2f && glUniform1i &&
    ...
    glMapBufferARB =
        (PFNGLMAPBUFFERARBPROC)SDL_GL_GetProcAddress("glMapBufferARB");
    glUnmapBufferARB =
        (PFNGLUNMAPBUFFERARBPROC)SDL_GL_GetProcAddress("glUnmapBufferARB");
    const char *gl_ext = (const char *)glGetString(GL_EXTENSIONS);
    if (gl_ext && *gl_ext) {
      sdl_opengl.packed_pixel = (strstr(gl_ext, "EXT_packed_pixels") != NULL);
      sdl_opengl.paletted_texture =
          (strstr(gl_ext, "EXT_paletted_texture") != NULL);
      // sdl_opengl.pixel_buffer_object=(strstr(gl_ext,"GL_ARB_pixel_buffer_object")
      // != NULL ) && glGenBuffersARB && glBindBufferARB && glDeleteBuffersARB
    ...
    sdl_opengl.pixel_buffer_object = false;
#endif
    // LOG_MSG("OpenGL extension: pixel_buffer_object
    // %d",sdl_opengl.pixel_buffer_object);
...
Bitu OUTPUT_OPENGL_GetBestMode(Bitu flags) {
  if (!(flags & GFX_CAN_32))
    return 0; // OpenGL requires 32-bit output mode
  flags |= GFX_SCALING;
...
static GLuint BuildShader(GLenum type, const char *shaderSrc) {
  ...
  if (!compiled) {
    ...
    if (infoLog) {
      ...
      LOG_MSG("Error getting shader compilation log");
...
void UpdateSDLDrawTexture() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, (int)SDLDrawGenFontTextureWidth,
               (int)SDLDrawGenFontTextureHeight, 0, GL_BGRA_EXT,
  ...
  /* load the font */
  {
    ...
    for (c = 0; c < 256; c++) {
      ...
      for (y = 0; y < 16; y++)
        for (x = 0; x < 8; x++)
          tmp[(y * 8) + x] =
              (bmp[y] & (0x80 >> x)) ? 0xFFFFFFFFUL : 0x00000000UL;

      glTexSubImage2D(GL_TEXTURE_2D, /*level*/ 0, /*x*/ (int)((c % 16) * 8),
                      /*y*/ (int)((c / 16) * 16), 8, 16, GL_BGRA_EXT,
                      GL_UNSIGNED_INT_8_8_8_8_REV, (void *)tmp);
    }
...
void UpdateSDLDrawDBCSTexture(Bitu code) {
  ...
  /* load the font */
  {
    ...
    for (c = 0; c < (code ? 2 : 1); c++) {
      unsigned char *bmp =
          code ? GetDbcsFont(code) : (int10_font_16 + 0xFB * 16);
      for (y = 0; y < 16; y++)
        for (x = 0; x < 8; x++)
          tmp[(y * 8) + x] = (bmp[code ? (y * 2 + c) : y] & (0x80 >> x))
                                 ? 0xFFFFFFFFUL
                                 : 0x00000000UL;

      glTexSubImage2D(GL_TEXTURE_2D, /*level*/ 0, /*x*/ (int)(c * 8), /*y*/ 0,
                      8, 16, GL_BGRA_EXT, GL_UNSIGNED_INT_8_8_8_8_REV,
                      (void *)tmp);
    }
    lastcp = dos.loaded_codepage;
    dos.loaded_codepage = cp;
...
Bitu OUTPUT_OPENGL_SetSize() {
  ...
  /* NTS: Apparently calling glFinish/glFlush before setup causes a segfault
   * within the OpenGL library on Mac OS X. */
  ...
  if (sdl_opengl.pixel_buffer_object) {
    ...
    if (sdl_opengl.buffer)
      glDeleteBuffersARB(1, &sdl_opengl.buffer);
    sdl_opengl.buffer = 0;
  }
  ...
  if (sec) {
#if defined(C_SDL2)
    SDL_GL_SetSwapInterval((!strcmp(sec->Get_string("vsyncmode"), "host")) ? 1
                                                                           : 0);
#elif SDL_VERSION_ATLEAST(1, 2, 11)
    SDL_GL_SetAttribute(SDL_GL_SWAP_CONTROL,
                        (!strcmp(sec->Get_string("vsyncmode"), "host")) ? 1
                                                                        : 0);
#endif
  }
...
#if defined(C_SDL2)
  if (SetupSurfaceScaledOpenGL(SDL_WINDOW_RESIZABLE, 32) == NULL)
    SetupSurfaceScaledOpenGL(SDL_WINDOW_RESIZABLE, 16);
#else
  if (SetupSurfaceScaledOpenGL(SDL_RESIZABLE, 32) == NULL)
    SetupSurfaceScaledOpenGL(SDL_RESIZABLE, 16);
#endif
  if (!sdl.surface || sdl.surface->format->BitsPerPixel < 15) {
    LOG_MSG("SDL:OPENGL:Can't open drawing surface, are you running in "
            "16bpp(or higher) mode?");
    return 0;
  }
  ...
  if (sdl_xbrz.enable &&
      xBRZ_SetScaleParameters((int)adjTexWidth, (int)adjTexHeight,
                              (int)sdl.clip.w, (int)sdl.clip.h)) {
    adjTexWidth = adjTexWidth * (unsigned int)sdl_xbrz.scale_factor;
    adjTexHeight = adjTexHeight * (unsigned int)sdl_xbrz.scale_factor;
  }
#endif

  int texsize = 2 << int_log2((int)(adjTexWidth > adjTexHeight ? adjTexWidth
                                                               : adjTexHeight));
  if (texsize > sdl_opengl.max_texsize) {
    LOG_MSG("SDL:OPENGL:No support for texturesize of %d (max size is %d), "
            "falling back to surface",
            texsize, sdl_opengl.max_texsize);
    return 0;
  }

  if (sdl_opengl.use_shader && sdl_opengl.shader_src == NULL &&
      !sdl_opengl.shader_def)
    sdl_opengl.use_shader = false;
  if (sdl_opengl.use_shader) {
    GLuint prog = 0;
    // reset error
    ...
    if (glGetError() == GL_NO_ERROR &&
        (sdl_opengl.program_object == 0 || prog != sdl_opengl.program_object)) {
      ...
      if (sdl_opengl.program_object == 0) {
        ...
        if (src == NULL && !LoadGLShaders(shader_src_default, &vertexShader,
                                          &fragmentShader)) {
        ...
        if (!sdl_opengl.program_object) {
          ...
          LOG_MSG("SDL:OPENGL:Can't create program object, falling back to "
                  "surface");
          return 0;
        }
#if DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW
        ...
        glGetProgramiv(sdl_opengl.program_object, GL_LINK_STATUS,
                       &isProgramLinked);
        if (!isProgramLinked) {
          char *infoLog = NULL;
          GLint infoLen = 0;

          glGetProgramiv(sdl_opengl.program_object, GL_INFO_LOG_LENGTH,
                         &infoLen);
          if (infoLen > 1)
            infoLog = (char *)malloc(infoLen);
          if (infoLog) {
            glGetProgramInfoLog(sdl_opengl.program_object, infoLen, NULL,
                                infoLog);
            LOG_MSG("SDL:OPENGL:Error linking program:\n %s", infoLog);
            free(infoLog);
          } else
            LOG_MSG("SDL:OPENGL:Failed to retrieve program link log");
        ...
        glVertexAttribPointer(u, 2, GL_FLOAT, GL_FALSE, 0,
                              sdl_opengl.vertex_data);
        glEnableVertexAttribArray(u);

        u = glGetUniformLocation(sdl_opengl.program_object, "rubyTexture");
        glUniform1i(u, 0);

        sdl_opengl.ruby.texture_size =
            glGetUniformLocation(sdl_opengl.program_object, "rubyTextureSize");
        sdl_opengl.ruby.input_size =
            glGetUniformLocation(sdl_opengl.program_object, "rubyInputSize");
        sdl_opengl.ruby.output_size =
            glGetUniformLocation(sdl_opengl.program_object, "rubyOutputSize");
        sdl_opengl.ruby.frame_count =
            glGetUniformLocation(sdl_opengl.program_object, "rubyFrameCount");
  ...
  if (sdl_opengl.pixel_buffer_object) {
    ...
    glBufferDataARB(GL_PIXEL_UNPACK_BUFFER_EXT,
                    (int)(adjTexWidth * adjTexHeight * 4), NULL,
                    GL_STREAM_DRAW_ARB);
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_EXT, 0);
  } else {
  ...
  if (sdl.desktop.fullscreen && sdl_opengl.use_shader)
    glViewport((sdl.surface->w - sdl.clip.w) / 2,
               (sdl.surface->h - sdl.clip.h) / 2, sdl.clip.w, sdl.clip.h);
  else
    glViewport(0, 0, sdl.surface->w, sdl.surface->h);
  if (sdl_opengl.texture > 0)
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texsize, texsize, 0, GL_BGRA_EXT,
               GL_UNSIGNED_BYTE, nullptr);
  ...
  sdl_opengl.menudraw_countdown =
      3; // two GL buffers with possible triple buffering behind our back
  ...
  if (sdl_opengl.program_object) {
    ...
    glUniform2f(sdl_opengl.ruby.input_size, (float)adjTexWidth,
                (float)adjTexHeight);
    ...
    glTexCoord2i(0, 0);
    glVertex2i((GLint)sdl.clip.x, (GLint)sdl.clip.y); // lower left
    glTexCoord2i((GLint)adjTexWidth, 0);
    glVertex2i((GLint)sdl.clip.x + (GLint)sdl.clip.w,
               (GLint)sdl.clip.y); // lower right
    glTexCoord2i((GLint)adjTexWidth, (GLint)adjTexHeight);
    glVertex2i((GLint)sdl.clip.x + (GLint)sdl.clip.w,
               (GLint)sdl.clip.y + (GLint)sdl.clip.h); // upper right
    glTexCoord2i(0, (GLint)adjTexHeight);
    glVertex2i((GLint)sdl.clip.x,
               (GLint)sdl.clip.y + (GLint)sdl.clip.h); // upper left
...
bool OUTPUT_OPENGL_StartUpdate(uint8_t *&pixels, Bitu &pitch) {
...
#endif
  {
    if (sdl_opengl.pixel_buffer_object) {
      glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_EXT, sdl_opengl.buffer);
      pixels =
          (uint8_t *)glMapBufferARB(GL_PIXEL_UNPACK_BUFFER_EXT, GL_WRITE_ONLY);
    } else {
...
#if C_XBRZ
    if (sdl_xbrz.enable && sdl_xbrz.scale_on) {
      // OpenGL pixel buffer is precreated for direct xBRZ output, while xBRZ
      // render buffer is used for rendering
      ...
      if (sdl_xbrz.renderbuf.size() ==
        ...
        const uint32_t *renderBuf =
            &sdl_xbrz.renderbuf[0]; // help VS compiler a little + support
                                    // capture by value
        uint32_t *trgTex;
        if (sdl_opengl.pixel_buffer_object) {
          glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_EXT, sdl_opengl.buffer);
          trgTex = (uint32_t *)glMapBufferARB(GL_PIXEL_UNPACK_BUFFER_EXT,
                                              GL_WRITE_ONLY);
        } else {
        ...
        if (trgTex)
          xBRZ_Render(renderBuf, trgTex, changedLines, (int)srcWidth,
      ...
      if (sdl_opengl.pixel_buffer_object) {
        ...
        glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0,
            (int)(sdl.draw.width * (unsigned int)sdl_xbrz.scale_factor),
            (int)(sdl.draw.height * (unsigned int)sdl_xbrz.scale_factor),
            GL_BGRA_EXT,
      ...
      if (sdl_opengl.pixel_buffer_object) {
        ...
        glBindTexture(GL_TEXTURE_2D, sdl_opengl.texture);
        glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0, (int)sdl.draw.width, (int)sdl.draw.height,
            GL_BGRA_EXT,
#if defined(MACOSX)
            // needed for proper looking graphics on macOS 10.12, 10.13
            GL_UNSIGNED_INT_8_8_8_8,
        ...
        while (y < sdl.draw.height) {
          if (!(index & 1)) {
            ...
            uint8_t *pixels =
                (uint8_t *)sdl_opengl.framebuf + y * sdl_opengl.pitch;
            Bitu height = changedLines[index];
            glTexSubImage2D(
                GL_TEXTURE_2D, 0, 0, (int)y, (int)sdl.draw.width, (int)height,
                GL_BGRA_EXT,
#if defined(MACOSX) && !defined(C_SDL2)
                // needed for proper looking graphics on macOS 10.12, 10.13
                GL_UNSIGNED_INT_8_8_8_8,
