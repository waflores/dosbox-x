...
std::map<int, int> lowboxdrawmap{
    {1, 201},    {2, 187},    {3, 200},    {4, 188},    {5, 186},
    {6, 205},    {0xe, 178},  {0x10, 206}, {0x14, 177}, {0x15, 202},
    {0x16, 203}, {0x17, 185}, {0x19, 204}, {0x1a, 176},
};
std::map<int, int> pc98boxdrawmap{
    {0xB4, 0x67}, {0xB9, 0x6e}, {0xBA, 0x46}, {0xBB, 0x56}, {0xBC, 0x5E},
    {0xB3, 0x45}, {0xBF, 0x53}, {0xC0, 0x57}, {0xC1, 0x77}, {0xC2, 0x6f},
    {0xC3, 0x5f}, {0xC4, 0x43}, {0xC5, 0x7f}, {0xC8, 0x5A}, {0xC9, 0x52},
    {0xCA, 0x7e}, {0xCB, 0x76}, {0xCC, 0x66}, {0xCD, 0x44}, {0xD9, 0x5B},
    {0xDA, 0x4f},
};
uint16_t cpMap_AX[32] = {0x0020, 0x00b6, 0x2195, 0x2194, 0x2191, 0x2193, 0x2192,
                         0x2190, 0x2500, 0x2502, 0x250c, 0x2510, 0x2518, 0x2514,
                         0x251c, 0x252c, 0x2524, 0x2534, 0x253c, 0x2550, 0x2551,
                         0x2554, 0x2557, 0x255d, 0x255a, 0x2560, 0x2566, 0x2563,
                         0x2569, 0x256c, 0x00ab, 0x00bb};
...
static unsigned long ttfSize = sizeof(DOSBoxTTFbi), ttfSizeb = 0, ttfSizei = 0,
                     ttfSizebi = 0;
static void *ttfFont = DOSBoxTTFbi, *ttfFontb = NULL, *ttfFonti = NULL,
            *ttfFontbi = NULL;
extern int posx, posy, eurAscii, transparency, NonUserResizeCounter;
extern bool rtl, gbk, chinasea, switchttf, force_conversion, blinking, showdbcs,
    loadlang, window_was_maximized;
...
bool colorChanged = false, justChanged = false, staycolors = false,
     firstsize = true, ttfswitch = false, switch_output_from_ttf = false;
bool init_once = false, init_twice = false;
extern bool finish_prepare;
bool is_ttfswitched_on = false;

int menuwidth_atleast(int width), FileDirExistCP(const char *name),
    FileDirExistUTF8(std::string &localname, const char *name);
void AdjustIMEFontSize(void), refreshExtChar(void), initcodepagefont(void),
    change_output(int output), drawmenu(Bitu val), KEYBOARD_Clear(void),
    RENDER_Reset(void), DOSBox_SetSysMenu(void),
    GetMaxWidthHeight(unsigned int *pmaxWidth, unsigned int *pmaxHeight),
    SetWindowTransparency(int trans), resetFontSize(void),
    RENDER_CallBack(GFX_CallBackFunctions_t function);
bool isDBCSCP(void), InitCodePage(void),
    CodePageGuestToHostUTF16(uint16_t *d /*CROSS_LEN*/,
                             const char *s /*CROSS_LEN*/),
    systemmessagebox(char const *aTitle, char const *aMessage,
...
Bitu OUTPUT_TTF_SetSize() {
  bool text = CurMode && (CurMode->type == 0 || CurMode->type == 2 ||
                          CurMode->type == M_TEXT || IS_PC98_ARCH);
  if (text) {
  ...
  if (ttf.inUse && ttf.fullScrn) {
...
#else
    sdl.surface = SDL_SetVideoMode(maxWidth, maxHeight, 32,
                                   SDL_SWSURFACE |
#if defined(WIN32)
                                       SDL_NOFRAME
#else
    ...
    if (sdl.displayNumber > 0) {
      ...
      MoveWindow(GetHWND(), info.rcMonitor.left, info.rcMonitor.top,
...
#if defined(C_SDL2)
    GFX_SetResizeable(false);
    sdl.window = GFX_SetSDLSurfaceWindow(sdl.draw.width + sdl.clip.x,
                                         sdl.draw.height + sdl.clip.y);
    sdl.surface = sdl.window ? SDL_GetWindowSurface(sdl.window) : NULL;
    if (firstsize && (posx < 0 || posy < 0) && !(posx == -2 && posy == -2) &&
        text) {
      firstsize = false;
      if (sdl.displayNumber == 0)
        SDL_SetWindowPosition(sdl.window, SDL_WINDOWPOS_CENTERED,
...
#else
    sdl.surface =
        SDL_SetVideoMode(sdl.draw.width + sdl.clip.x,
                         sdl.draw.height + sdl.clip.y, 32, SDL_SWSURFACE);
#endif
  }
  if (!sdl.surface)
...
void setVGADAC() {
  if (ttf.inUse && CurMode && IS_VGA_ARCH) {
    ...
    for (uint8_t i = 0; i < 0x10; i++) {
      ...
      IO_WriteB(VGAREG_DAC_DATA,
                altBGR0[i].red >> 2); // Convert 8-bit to 6-bit for VGA DAC
...
bool setColors(const char *colorArray, int n) {
  ...
  for (int colNo = n > -1 ? n : 0; colNo < (n > -1 ? n + 1 : 16); colNo++) {
    if (sscanf(nextRGB, " ( %d , %d , %d)", &rgbVal[0], &rgbVal[1],
               &rgbVal[2]) == 3) { // Decimal: (red,green,blue)
      for (int i = 0; i < 3; i++) {
      ...
      while (*nextRGB != ')')
        nextRGB++;
      nextRGB++;
    } else if (sscanf(nextRGB, " #%6x", ((uint32_t *)(&rgbVal[3]))) ==
               1) { // Hexadecimal
...
bool readTTFStyle(unsigned long &size, void *&font, FILE *fh) {
  ...
  if (pos != -1L) {
    ...
    if (font && !fseek(fh, 0, SEEK_SET) &&
        fread(font, 1, (size_t)size, fh) == (size_t)size) {
      fclose(fh);
      return true;
    }
  }
...
bool readTTF(const char *fName, bool bold, bool ital) {
  ...
  if (!ttf_fh) {
    ...
    if (exepath.size()) {
      strcpy(strrchr(strcpy(ttfPath, exepath.c_str()), CROSS_FILESPLIT) + 1,
             fName); // Try to load it from where DOSBox-X was started
      strcat(ttfPath, ".ttf");
      ttf_fh = fopen(ttfPath, "rb");
      if (!ttf_fh) {
        strcpy(strrchr(strcpy(ttfPath, exepath.c_str()), CROSS_FILESPLIT) + 1,
  ...
  if (!ttf_fh) {
    std::string basedir =
        static_cast<Section_prop *>(control->GetSection("printer"))
            ->Get_string("fontpath");
  ...
  if (!failName.size() || failName.compare(fName)) {
    ...
    for (size_t i = 0; i < strlen(fName); i++) {
      if ((unsigned char)fName[i] > 0x7f)
        alllower = false;
      break;
    }
    std::string message =
        alllower ? ("Could not load font file: " + std::string(fName) +
                    (strlen(fName) < 5 ||
                             strcasecmp(fName + strlen(fName) - 4, ".ttf")
                         ? ".ttf"
                         : ""))
                 : "Could not load the specified font file.";
...
void SetBlinkRate(Section_prop *section) {
  ...
  unsigned int num = -1;
  if (!strcasecmp(blinkCstr, "false") || !strcmp(blinkCstr, "-1"))
    blinkCursor = -1;
  else if (1 == sscanf(blinkCstr, "%u", &num) && int(num) >= 0 && int(num) <= 7)
    blinkCursor = num;
  else
    blinkCursor = IS_PC98_ARCH
                      ? 6
                      : 4; // default cursor blinking is slower on PC-98 systems
}
...
int setTTFMap(bool changecp) {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    uname[0] = 0;
    uname[1] = 0;
    if (cp == 932 && (halfwidthkana || IS_JEGA_ARCH))
      forceswk = true;
    if (CheckDBCSCP(cp))
      dos.loaded_codepage = 437;
    if (CodePageGuestToHostUTF16(uname, text)) {
      wcTest[i] = uname[1] == 0 ? uname[0] : i;
      if (cp == 932 && lowboxdrawmap.find(i) != lowboxdrawmap.end() &&
          TTF_GlyphIsProvided(ttf.SDL_font, wcTest[i]))
        cpMap[i] = wcTest[i];
    }
    forceswk = false;
    if (CheckDBCSCP(cp))
  ...
  uint16_t unimap;
  int notMapped = 0;
  for (int y = ((cpMap[1] != 1 && cpMap[1] != 0x263A) || cp == 867 ||
                        (customcp &&
                         (dos.loaded_codepage == customcp || changecp)) ||
                        (altcp && (dos.loaded_codepage == altcp || changecp))
                    ? 0
                    : 8);
       y < 16; y++)
    for (int x = 0; x < 16; x++) {
      if (y < 8 && (wcTest[y * 16 + x] == y * 16 + x ||
                    wcTest[y * 16 + x] == cp437_to_unicode[y * 16 + x]))
        unimap = cpMap_copy[y * 16 + x];
      else
...
void GFX_SelectFontByPoints(int ptsize) {
  ...
  if (ttf.SDL_fontb)
    TTF_CloseFont(ttf.SDL_fontb);
  if (ttf.SDL_fonti)
    TTF_CloseFont(ttf.SDL_fonti);
  if (ttf.SDL_fontbi)
    TTF_CloseFont(ttf.SDL_fontbi);
...
void SetOutputSwitch(const char *outputstr) {
  ...
  if (!strcasecmp(outputstr, "direct3d") || !strcasecmp(outputstr, "auto"))
    ...
    switchoutput = 4;
  else if (!strcasecmp(outputstr, "opengl") ||
           !strcasecmp(outputstr, "openglnq") || !strcasecmp(outputstr, "auto"))
    switchoutput = 3;
  else
#endif
...
void OUTPUT_TTF_Select(int fsize) {
  if (!initttf && TTF_Init()) { // Init SDL-TTF
    std::string message =
        "Could not init SDL-TTF: " + std::string(SDL_GetError());
  ...
  if (fsize == 3)
    ...
    if (!*fName) {
      std::string mtype(
          static_cast<Section_prop *>(control->GetSection("dosbox"))
              ->Get_string("machine"));
      if (IS_PC98_ARCH || mtype.substr(0, 4) == "pc98" ||
          (!notrysgf && InitCodePage() && isDBCSCP()))
        trysgf = true;
    }
    force_conversion = false;
    dos.loaded_codepage = cp;
    if (trysgf)
      failName = "SarasaGothicFixed";
    if ((!*fName && !trysgf) ||
        !readTTF(*fName ? fName : failName.c_str(), false, false)) {
      ttfFont = DOSBoxTTFbi;
      ttfFontb = NULL;
      ttfFonti = NULL;
      ttfFontbi = NULL;
      ...
      std::string message = "";
      if (*fbName)
        message = "A valid ttf.font setting is required for the ttf.fontbold "
                  "setting: " +
                  std::string(fbName);
      else if (*fiName)
        message = "A valid ttf.font setting is required for the ttf.fontital "
                  "setting: " +
                  std::string(fiName);
      else if (*fbiName)
        message = "A valid ttf.font setting is required for the ttf.fontboit "
                  "setting: " +
                  std::string(fbiName);
    ...
    staycolors =
        strlen(colors) &&
        *colors == '+'; // Always switch to preset value when '+' is specified
    if ((*colors && (!init_once || !init_twice)) || staycolors) {
      if (!setColors(colors, -1)) {
        LOG_MSG("Incorrect color scheme: %s", colors);
        // setColors("#000000 #0000aa #00aa00 #00aaaa #aa0000 #aa00aa #aa5500
      ...
      char value[30];
      for (int i = 0; i < 16; i++) {
        sprintf(value, "#%02x%02x%02x", rgbcolors[i].red, rgbcolors[i].green,
                rgbcolors[i].blue);
    ...
    if (strlen(wpstr) > 1) {
      if (!strncasecmp(wpstr, "WP", 2))
        ...
        wpType = 2;
      else if (!strncasecmp(wpstr, "XY", 2))
        wpType = 3;
      else if (!strncasecmp(wpstr, "FE", 2))
        wpType = 4;
    ...
    if (winPerc > 100 || (fsize == 2 && GFX_IsFullscreen()) ||
        (fsize != 1 && fsize != 2 &&
         (control->opt_fullscreen ||
    ...
    if (fsize && !IS_PC98_ARCH && !IS_EGAVGA_ARCH)
      ttf.lins = 25;
    if ((!CurMode || CurMode->type != M_TEXT) && !IS_PC98_ARCH) {
      ...
      if (IS_PC98_ARCH) {
        ...
        c = strcmp(RunningProgram, "LOADLIN")
                ? real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS)
                : 80;
        r = (uint16_t)(IS_EGAVGA_ARCH && strcmp(RunningProgram, "LOADLIN")
      ...
      if (ttf.lins < 1 || ttf.cols < 1) {
        ...
        CheckTTFLimit();
        if (ttf.cols != c || ttf.lins != r)
          alter_vmode = true;
      }
      if (alter_vmode) {
        for (Bitu i = 0; ModeList_VGA[i].mode <= 7;
             i++) { // Set the cols and lins in video mode 2,3,7
...
#endif
  int curSize = fontSize >= MIN_PTSIZE
                    ? fontSize
                    : 30; // no clear idea what would be a good starting value
  int lastGood = -1;
  int trapLoop = 0;
  ...
  if (fontSize < MIN_PTSIZE) {
    while (curSize != lastGood) {
      ...
      if (ttf.cols * ttf.width <= maxWidth &&
          ttf.lins * ttf.height <= maxHeight) { // if it fits on screen
        lastGood = curSize;
        float coveredPerc = float(100 * ttf.cols * ttf.width / maxWidth *
                                  ttf.lins * ttf.height / maxHeight);
        if (trapLoop++ > 4 &&
            coveredPerc <= winPerc) // we can get into a +/-/+/-... loop!
          break;
        curSize = (int)(curSize * sqrt((float)winPerc /
                                       coveredPerc)); // rounding down is ok
    ...
    if (ttf.DOSBox &&
        curSize >
            MIN_PTSIZE) // make it even for DOSBox-X internal font (a bit nicer)
      curSize &= ~1;
  }
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
resize1:
  ...
  GFX_SelectFontByPoints(curSize);
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  if (!ttf.fullScrn && menu_gui && menu.toggle &&
      menuwidth_atleast(ttf.cols * ttf.width + ttf.offX * 2 +
                        GetSystemMetrics(SM_CXBORDER) * 2) > 0) {
    if (ttf.cols * ttf.width > maxWidth || ttf.lins * ttf.height > maxHeight)
...
#endif
resize2:
  if ((fontSize >= MIN_PTSIZE && 100 * ttf.cols * ttf.width / maxWidth *
                                         ttf.lins * ttf.height / maxHeight >
                                     100) ||
      (lesssize && (ttf.cols * ttf.width > maxWidth ||
                    ttf.lins * ttf.height > maxHeight))) {
  ...
  if (ttf.SDL_font && ttf.width) {
    ...
    if (abs(ttf.width - widthb) > 1 || abs(ttf.width - widthm) > 1 ||
        abs(ttf.width - widthx) > 1 || abs(ttf.width - width0) > 1)
      LOG_MSG("TTF: The loaded font is not monospaced.");
    int cp = dos.loaded_codepage;
    if (!cp)
      InitCodePage();
    if ((IS_PC98_ARCH || isDBCSCP()) && dbcs_sbcs) {
      ...
      if (width1 <= ttf.width || width9 <= ttf.width)
        LOG_MSG("TTF: The loaded font may not support DBCS characters.");
      else if ((ttf.width * 2 != width1 || ttf.width * 2 != width9) &&
               ttf.width == widthb && ttf.width == widthm &&
               ttf.width == widthx && ttf.width == width0)
        LOG_MSG("TTF: The loaded font is not monospaced.");
    }
    dos.loaded_codepage = cp;
  }
...
void ResetTTFSize(Bitu /*val*/) { resetFontSize(); }
...
void processWP(uint8_t *pcolorBG, uint8_t *pcolorFG) {
  ...
  if (CurMode->mode == 7) { // Mono (Hercules)
    style =
        showline && (colorFG & 7) == 1 ? TTF_STYLE_UNDERLINE : TTF_STYLE_NORMAL;
    if ((colorFG & 0xa) == colorFG && (colorBG & 15) == 0)
      colorFG = 8;
    else if (colorFG & 7)
      colorFG |= 7;
  } else if (wpType == 1) { // WordPerfect
    if (showital && colorFG == 0xe &&
        (colorBG & 15) == (wpBG > -1 ? wpBG : 1)) {
      style = TTF_STYLE_ITALIC;
      colorFG = wpFG;
    } else if (showline && (colorFG == 1 || colorFG == wpFG + 8) &&
               (colorBG & 15) == 7) {
    ...
    if (colorBG == 1 || colorBG == 5 || colorBG == 6 || colorBG == 7 ||
        colorBG == 11 || colorBG == 12 || colorBG == 13 || colorBG == 15)
      colorFG += 8;
    if (colorBG == 2 || colorBG == 5) {
      ...
      style |= (showital ? TTF_STYLE_ITALIC : 0) |
               (showline ? TTF_STYLE_UNDERLINE : 0);
    } else if (colorBG == 9 || colorBG == 12) {
      style |= (showital ? TTF_STYLE_ITALIC : 0) |
               (showsout ? TTF_STYLE_STRIKETHROUGH : 0);
    } else if (colorBG == 10 || colorBG == 13) {
      style |= (showline ? TTF_STYLE_UNDERLINE : 0) |
               (showsout ? TTF_STYLE_STRIKETHROUGH : 0);
  ...
  if (char512 && wpType == 1) {
    if (wpExtChar && (colorFG & 8)) { // WordPerfect high bit = second character
                                      // set (if 512char active)
      charSet = 1;
      colorFG &= 7;
    }
  }
  if (showbold &&
      (colorFG == wpFG + 8 ||
       (wpType == 1 && (wpVersion < 1 || wpVersion > 5) && colorFG == 3 &&
        (colorBG & 15) == (wpBG > -1 ? wpBG : 1)))) {
    if (ttf.SDL_fontbi || !(style & TTF_STYLE_ITALIC) || wpType == 4)
      style |= TTF_STYLE_BOLD;
    if ((ttf.SDL_fontbi && (style & TTF_STYLE_ITALIC)) ||
        (ttf.SDL_fontb && !(style & TTF_STYLE_ITALIC)) || wpType == 4)
      colorFG = wpFG;
  }
  if (style)
...
void GFX_EndTextLines(bool force) {
  if (!force && !IS_PC98_ARCH && (!CurMode || CurMode->type != M_TEXT))
    return;
#if DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW
  if (!ttf.fullScrn) {
    mainMenu.setRedraw();
...
#endif
    rect->x = 0;
    rect->y = sdl.draw.height + ttf.offY;
    rect->w = maxWidth;
    rect->h = maxHeight - sdl.draw.height - ttf.offY;
#if defined(C_SDL2)
    SDL_UpdateWindowSurfaceRects(sdl.window, sdl.updateRects, 4);
#else
    SDL_UpdateRects(sdl.surface, 4, sdl.updateRects);
#endif
    rect->x = sdl.draw.width + ttf.offX;
    rect->y = 0;
    rect->w = maxWidth - sdl.draw.width - ttf.offX;
    rect->h = maxHeight;
#if defined(C_SDL2)
    SDL_UpdateWindowSurfaceRects(sdl.window, sdl.updateRects, 4);
#else
  ...
  if (ttf.cursor <
      ttf.cols *
          ttf.lins) // hide/restore (previous) cursor-character if we had one

    //		if (cursor_enabled && (vga.draw.cursor.sline >
    //vga.draw.cursor.eline || vga.draw.cursor.sline > 15)) 		if (ttf.cursor !=
    //vga.draw.cursor.address>>1 || (vga.draw.cursor.enabled !=  cursor_enabled)
    //|| vga.draw.cursor.sline > vga.draw.cursor.eline || vga.draw.cursor.sline
    //> 15)
    if ((ttf.cursor != vga.draw.cursor.address >> 1) ||
        vga.draw.cursor.sline > vga.draw.cursor.eline ||
  ...
  hasfocus =
      ...
      SDL_GetAppState() & SDL_APPINPUTFOCUS
          ...
          ? true
          : false;
  bool focuschanged = lastfocus != hasfocus,
       noframe = !menu.toggle || ttf.fullScrn;
  ...
  for (unsigned int y = 0; y < ttf.lins; y++) {
    ...
    for (unsigned int x = 0; x < ttf.cols; x++) {
      if (((newAC[x] != curAC[x] || newAC[x].selected != curAC[x].selected ||
            (colorChanged && (justChanged || draw)) || force) &&
           (!newAC[x].skipped || force)) ||
          (!y && focuschanged && noframe)) {
        draw = true;
        xmin = min((int)x, xmin);
        ymin = min((int)y, ymin);
        ymax = y;
        ...
        bool colornul =
            staycolors ||
            (IS_VGA_ARCH &&
             (altBGR1[colorBG & 15].red > 4 ||
              altBGR1[colorBG & 15].green > 4 ||
              altBGR1[colorBG & 15].blue > 4 || altBGR1[colorFG & 15].red > 4 ||
              altBGR1[colorFG & 15].green > 4 ||
              altBGR1[colorFG & 15].blue > 4) &&
             rgbColors[colorBG].red < 5 && rgbColors[colorBG].green < 5 &&
             rgbColors[colorBG].blue < 5 && rgbColors[colorFG].red < 5 &&
             rgbColors[colorFG].green < 5 && rgbColors[colorFG].blue < 5);
        ...
        if (noframe && !hasfocus &&
            !y) { // Dim top line if focus is lost in Fullscreen mode
        ...
        if (newAC[x].unicode) {
          ...
          if (ttf_dosv && ascii == 0x5c)
            ascii = 0x9d;
          curAC[x] = newAC[x];
          if (ascii > 175 && ascii < 179 && !IS_PC98_ARCH && !IS_JEGA_ARCH &&
              dos.loaded_codepage != 864 && dos.loaded_codepage != 868 &&
              dos.loaded_codepage != 874 && dos.loaded_codepage != 3021 &&
              !(dos.loaded_codepage == 932 && halfwidthkana) &&
              (dos.loaded_codepage < 1250 || dos.loaded_codepage > 1258) &&
              !(altcp && dos.loaded_codepage == altcp) &&
              !(customcp &&
                dos.loaded_codepage == customcp)) { // special: shade characters
                                                    // 176-178 unless PC-98
            ttf_bgColor.b = (ttf_bgColor.b * (179 - ascii) +
                             ttf_fgColor.b * (ascii - 175)) >>
                            2;
            ttf_bgColor.g = (ttf_bgColor.g * (179 - ascii) +
                             ttf_fgColor.g * (ascii - 175)) >>
                            2;
            ttf_bgColor.r = (ttf_bgColor.r * (179 - ascii) +
                             ttf_fgColor.r * (ascii - 175)) >>
                            2;
...

        {
          ...
          SDL_Surface *textSurface =
              TTF_RenderUNICODE_Shaded(ttf.SDL_font, unimap, ttf_fgColor,
                                       ttf_bgColor, ttf.width * (dw ? 2 : 1));
          ttf_textClip.w = (x - x1) * ttf.width;
          SDL_BlitSurface(textSurface, &ttf_textClip, sdl.surface,
                          &ttf_textRect);
  ...
  if (!force)
    justChanged = false;
  // NTS: Additional fix is needed for the cursor in PC-98 mode; also expect
  // further cleanup
  bcount++;
  if (vga.draw.cursor.enabled &&
      vga.draw.cursor.sline <= vga.draw.cursor.eline &&
      vga.draw.cursor.sline <= 16 && blinkCursor) { // Draw cursor?
    ...
    if (newPos < ttf.cols * ttf.lins) { // If on screen
      ...
      if (blinkCursor > -1)
        vga.draw.cursor.blinkon =
            (vga.draw.cursor.count & 1 << blinkCursor) ? true : false;

      if (ttf.cursor != newPos || vga.draw.cursor.sline != prev_sline ||
          ((blinkstate != vga.draw.cursor.blinkon) &&
           blinkCursor > -1)) { // If new position or shape changed, force draw
      ...
      //ymax  && (GetTickCount()&0x400))	// If overdrawn previously (or
      //new shape)
      if (x >= xmin && x <= xmax && y >= ymin && y <= ymax &&
          !newAttrChar[ttf.cursor]
               .skipped) { // If overdrawn previously (or new shape)
        uint8_t colorBG = newAttrChar[ttf.cursor].bg;
        uint8_t colorFG = newAttrChar[ttf.cursor].fg;
        processWP(&colorBG, &colorFG);

        /* Don't do this in PC-98 mode, the bright pink cursor in EDIT.COM looks
         * awful and not at all how the cursor is supposed to look --J.C. */
        if (!IS_PC98_ARCH && blinking && colorBG & 8) {
          colorBG -= 8;
          if ((bcount / 8) % 2)
            colorFG = colorBG;
        }
        bool dw = newAttrChar[ttf.cursor].unicode &&
                  newAttrChar[ttf.cursor].doublewide;
        ...
        unimap[0] = newAttrChar[ttf.cursor].unicode
                        ? newAttrChar[ttf.cursor].chr
                        : cpMap[newAttrChar[ttf.cursor].chr & 255];
        if (dw) {
        ...
        ttf_textRect.x =
            ttf.offX + (rtl ? (ttf.cols - x - (dw ? 2 : 1)) : x) * ttf.width;
        ttf_textRect.y = ttf.offY + y * ttf.height;
        ...
        if (vga.draw.cursor.blinkon || blinkCursor < 0) {
          ...
          textSurface =
              TTF_RenderUNICODE_Shaded(ttf.SDL_font, unimap, ttf_bgColor,
                                       ttf_fgColor, ttf.width * (dw ? 2 : 1));
          ttf_textClip.y =
              (ttf.height *
               (vga.draw.cursor.sline > 15 ? 15 : vga.draw.cursor.sline)) >>
              4;
          ttf_textClip.h =
              ttf.height - ttf_textClip.y; // for now, cursor to bottom
  ...
  if (xmin <= xmax) { // if any changes
    SDL_Rect *rect = &sdl.updateRects[0];
    rect->x = ttf.offX + (rtl ? (ttf.cols - xmax - 1) : xmin) * ttf.width;
    rect->y = ttf.offY + ymin * ttf.height;
    rect->w = (xmax - xmin + 1) * ttf.width;
    rect->h = (ymax - ymin + 1) * ttf.height;
#if defined(C_SDL2)
    SDL_UpdateWindowSurfaceRects(sdl.window, sdl.updateRects, 4);
#else
    SDL_UpdateRects(sdl.surface, 4, sdl.updateRects);
...
FT_Face GetTTFFace() {
  if (ttf.inUse && ttfFont && ttfSize) {
    TTF_Font *font = TTF_OpenFontRW(SDL_RWFromConstMem(ttfFont, (int)ttfSize),
                                    1, ttf.pointsize);
...
void resetFontSize() {
  if (ttf.inUse) {
    ...
    GFX_SetSize(720 + sdl.clip.x, 400 + sdl.clip.y, sdl.draw.flags,
                sdl.draw.scalex, sdl.draw.scaley, sdl.draw.callback);
...
void increaseFontSize() {
  ...
  if (ttf.inUse) { // increase fontsize
    ...
    if (ttf.cols * ttf.width <= maxWidth &&
      ...
      GFX_SetSize(720 + sdl.clip.x, 400 + sdl.clip.y, sdl.draw.flags,
                  sdl.draw.scalex, sdl.draw.scaley, sdl.draw.callback);
      wmemset((wchar_t *)curAttrChar, -1,
              ttf.cols * ttf.lins); // force redraw of complete window
      GFX_EndTextLines(true);
    } else
      GFX_SelectFontByPoints(ttf.pointsize - inc);
  }
...
void ttf_reset_colors() {
  if (ttf.inUse) {
    SetVal("ttf", "colors", "");
    setColors("#000000 #0000aa #00aa00 #00aaaa #aa0000 #aa00aa #aa5500 #aaaaaa "
...
void ResetColors_mapper_shortcut(bool pressed) {
  if (!pressed || (!ttf.inUse && !IS_VGA_ARCH))
    return;
  ttf_reset_colors();
}
...
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
  int last = 0;
  while (TTF_using() && !GFX_IsFullscreen() && menu_gui && menu.toggle &&
         menuwidth_atleast(ttf.cols * ttf.width + ttf.offX * 2 +
                           GetSystemMetrics(SM_CXBORDER) * 2) > 0 &&
         ttf.pointsize > last) {
    last = ttf.pointsize;
...
void ttfreset(Bitu /*val*/) { ttf_reset(); }

void ttf_setlines(int cols, int lins) {
  if (cols > 0)
    SetVal("ttf", "cols", std::to_string(cols));
  if (lins > 0)
    SetVal("ttf", "lins", std::to_string(lins));
  firstset = true;
  ttf_reset();
  real_writeb(BIOSMEM_SEG, BIOSMEM_NB_COLS, ttf.cols);
  if (IS_EGAVGA_ARCH)
...
void ttf_switch_on(bool ss = true) {
  if ((ss && ttfswitch) || (!ss && switch_output_from_ttf)) {
    ...
    // change_output(0); // call OUTPUT_SURFACE_Select() to initialize output
    // before enabling TTF output on Windows builds
#endif
    change_output(10); // call OUTPUT_TTF_Select()
    SetVal("sdl", "output", "ttf");
    std::string showdbcsstr =
        static_cast<Section_prop *>(control->GetSection("dosv"))
            ->Get_string("showdbcsnodosv");
    ...
    if (ttf.fullScrn) {
      ...
      OUTPUT_TTF_Select(3);
#if DOSBOXMENU_TYPE == DOSBOXMENU_HMENU
      if (gl && GFX_IsFullscreen()) { // Hack for full-screen switch from OpenGL
                                      // outputs
        GFX_SwitchFullScreen();
        GFX_SwitchFullScreen();
      }
    ...
    SetWindowTransparency(
        static_cast<Section_prop *>(control->GetSection("sdl"))
            ->Get_int("transparency"));
#endif
    if (!IS_PC98_ARCH && vga.draw.address_add != ttf.cols * 2)
      checkcol = ss ? 2 : 1;
  }
