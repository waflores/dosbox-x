     ...
     )                   /* returns -1 on error and 0 on success */
{
  ...
  if /* sanity checks: */
      (win <= 0 || hin <= 0 || win > wout || hin > hout || par <= 0.0)
    return -1;
     ...
     )                       /* return -1 on error and 0 on success */
{
  ...
  if /* minimal sanity checks: */
      (bypp < 1 || sx < 1 || sy < 1 || *rw < 1 || *rh < 1 || simg == NULL ||
       dimg == NULL || spitch < *rw * bypp || dpitch < *rw * sx * bypp)
    return -1;
  ...
  for (y = 0; y < *rh; y += 1) {
    ...
    while (1) {
      ...
      if (iy == sy)
        break;                     /* terminate if source row scaled */
