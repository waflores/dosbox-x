...
ScalingEffect::ScalingEffect(LPDIRECT3DDEVICE9 pd3dDevice)
    : m_scale(0.0f), m_forceupdate(false), m_pd3dDevice(pd3dDevice),
      m_pEffect(0) {
...
ScalingEffect::~ScalingEffect(void) { KillThis(); }
...
HRESULT ScalingEffect::LoadEffect(const TCHAR *filename) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = lpEffectCompiler->CompileEffect(D3DXSHADER_DEBUG, &lpBufferEffect,
                                         &lpErrors);
#else
  ...
  if (SUCCEEDED(hr)) {
    hr = D3DXCreateEffect(m_pd3dDevice, lpBufferEffect->GetBufferPointer(),
                          lpBufferEffect->GetBufferSize(), NULL, NULL, 0, NULL,
                          &m_pEffect, &lpErrors);
...
HRESULT ScalingEffect::ParseParameters(LPD3DXEFFECTCOMPILER lpEffectCompiler) {
  ...
  // Look at parameters for semantics and annotations that we know how to
  // interpret
  ...
  for (UINT iParam = 0; iParam < m_EffectDesc.Parameters; iParam++) {
    ...
    if (ParamDesc.Semantic != NULL) {
      if (ParamDesc.Class == D3DXPC_MATRIX_ROWS ||
        ...
        if (strcmpi(ParamDesc.Semantic, "sourcedims") == 0)
          ...
          if (m_iDim[0] && m_iDim[1] &&
              ((hr = m_pEffect->SetFloatArray(hParam, m_iDim, 2)) != D3D_OK)) {
            m_strErrors += "Could not set inputdims parameter!";
            return hr;
          }
        } else if (strcmpi(ParamDesc.Semantic, "outputdims") == 0) {
          if (m_oDim[0] && m_oDim[1] &&
              ((hr = m_pEffect->SetFloatArray(hParam, m_oDim, 2)) != D3D_OK)) {
        ...
        if (strcmpi(ParamDesc.Semantic, "COMBINETECHNIQUE") == 0) {
          ...
          m_CombineTechniqueEffectHandle =
              m_pEffect->GetTechniqueByName(pstrTechnique);
        } else if (strcmpi(ParamDesc.Semantic, "PREPROCESSTECHNIQUE") == 0) {
          m_pEffect->GetString(hParam, &pstrTechnique);
          m_PreprocessTechnique1EffectHandle =
              m_pEffect->GetTechniqueByName(pstrTechnique);
    ...
    /*	if(pstrName != NULL) {
                pTex = NULL;
                DXUtil_ConvertAnsiStringToGenericCch(strPath, pstrName,
       MAX_PATH); if(pstrTextureType != NULL) { if(strcmpi(pstrTextureType,
       "volume") == 0) { LPDIRECT3DVOLUMETEXTURE9 pVolumeTex = NULL;
                        if(SUCCEEDED(hr =
       D3DXCreateVolumeTextureFromFileEx(m_pd3dDevice, strPath, Width, Height,
       Depth, 1, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT,
       0, NULL, NULL, &pVolumeTex))) { pTex = pVolumeTex;
                        }
                        else {
                        ...
                        if(SUCCEEDED(hr =
       D3DXCreateCubeTextureFromFileEx(m_pd3dDevice, strPath, Width,
       D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT,
       D3DX_DEFAULT, 0, NULL, NULL, &pCubeTex))) { pTex = pCubeTex;
                        }
                        else {
                    ...
                    if(SUCCEEDED(hr = D3DXCreateTextureFromFileEx(m_pd3dDevice,
       strPath, Width, Height, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED,
       ...
       &p2DTex))) { pTex = p2DTex;
                ...
                if(SUCCEEDED(hr) && pTex != NULL) {
                    m_pEffect->SetTexture(m_pEffect->GetParameter(NULL, iParam),
       pTex); SAFE_RELEASE(pTex);
                }
            }
            else */
    if (pstrFunction != NULL) {
      ...
      if (SUCCEEDED(hr = lpEffectCompiler->CompileShader(
                        pstrFunction, pstrTarget, 0, &pTextureShader, &lpErrors,
                        NULL))) {
        SAFE_RELEASE(lpErrors);
        ...
        D3DXCreateTextureShader((DWORD *)pTextureShader->GetBufferPointer(),
                                &ppTextureShader);
#endif
        ...
        if (pstrTextureType != NULL) {
          if (strcmpi(pstrTextureType, "volume") == 0) {
            ...
            if (SUCCEEDED(hr = D3DXCreateVolumeTexture(
                              m_pd3dDevice, Width, Height, Depth, 1, 0,
                              D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pVolumeTex))) {
#if D3DX_SDK_VERSION >= 22
              if (SUCCEEDED(hr = D3DXFillVolumeTextureTX(pVolumeTex,
                                                         ppTextureShader))) {
#else
              if (SUCCEEDED(hr = D3DXFillVolumeTextureTX(
                                pVolumeTex,
            ...
            LPDIRECT3DCUBETEXTURE9 pCubeTex = NULL;
            if (SUCCEEDED(hr = D3DXCreateCubeTexture(
                              m_pd3dDevice, Width, D3DX_DEFAULT, 0,
                              D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pCubeTex))) {
#if D3DX_SDK_VERSION >= 22
              if (SUCCEEDED(
                      hr = D3DXFillCubeTextureTX(pCubeTex, ppTextureShader))) {
#else
          ...
          if (SUCCEEDED(hr = D3DXCreateTexture(m_pd3dDevice, Width, Height,
                                               ...
                                               D3DPOOL_MANAGED, &p2DTex))) {
#if D3DX_SDK_VERSION >= 22
            if (SUCCEEDED(hr = D3DXFillTextureTX(p2DTex, ppTextureShader))) {
#else
            if (SUCCEEDED(hr = D3DXFillTextureTX(
                              p2DTex,
                              (DWORD *)pTextureShader->GetBufferPointer(), NULL,
                              0))) {
#endif
              pTex = p2DTex;
            }
...
// Set Source texture
HRESULT
ScalingEffect::SetTextures(LPDIRECT3DTEXTURE9 lpSource,
...
HRESULT ScalingEffect::SetMatrices(D3DXMATRIX &matProj, D3DXMATRIX &matView,
                                   D3DXMATRIX &matWorld) {
  HRESULT hr = S_OK;
...
HRESULT ScalingEffect::Begin(Pass pass, UINT *pPasses) {
  ...
  return m_pEffect->Begin(pPasses,
...
HRESULT ScalingEffect::End() { return m_pEffect->End(); }
...
HRESULT ScalingEffect::Validate() {
  ...
  if (!m_WorkingTexture1EffectHandle && m_PreprocessTechnique1EffectHandle) {
    m_strErrors +=
        "Effect doesn't have a WORKINGTEXTURE handle but uses preprocess steps";
    return E_FAIL;
  }

  if (!m_WorkingTexture2EffectHandle && m_PreprocessTechnique2EffectHandle) {
    m_strErrors += "Effect doesn't have a WORKINGTEXTURE1 handle but uses 2 "
                   "preprocess steps";
    return E_FAIL;
  }
