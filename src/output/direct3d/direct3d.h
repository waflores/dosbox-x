...
private:
...
#if !(C_D3DSHADERS)
  D3DXMATRIX *MatrixOrthoOffCenterLH(D3DXMATRIX *, float, float, float, float,
                                     float, float);
  ...
  static int EntryPoint(void *pthis) {
    CDirect3D *pt = (CDirect3D *)pthis;
    return pt->Start();
  }
...
public:
  ...
  bool square = false, pow2 = false, dynamic = false,
       bpp16; // Texture limitations
  ...
  HRESULT Resize3DEnvironment(Bitu, Bitu, Bitu, Bitu, Bitu, Bitu, Bitu, Bitu,
                              bool fullscreen = false);
  bool LockTexture(uint8_t *&pixels, Bitu &pitch);
  bool UnlockTexture(const uint16_t *changed);

  CDirect3D(uint32_t width = 640, uint32_t height = 400)
...
#if C_D3DSHADERS
    if (psEffect)
      return psEffect->getForceUpdate();
#endif
    return false;
  }
