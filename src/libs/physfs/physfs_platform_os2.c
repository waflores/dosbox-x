...
static int(_System *pUniUconvToUcs)(UconvObject, void **, size_t *, UniChar **,
                                    size_t *, size_t *) = NULL;
static int(_System *pUniUconvFromUcs)(UconvObject, UniChar **, size_t *,
...
static char *cvtUtf8ToCodepage(const char *utf8str) {
  ...
  if (!uconvdll) {
    ...
    rc = pUniUconvFromUcs(uconv, &uc2str, &unilen, (void **)&cpstr, &cplen,
                          &subs);
...
static char *cvtCodepageToUtf8(const char *cpstr) {
  ...
  if (!uconvdll) {
    ...
    rc =
        pUniUconvToUcs(uconv, (void **)&cpstr, &cplen, &uc2str, &unilen, &subs);
...
static char *cvtPathToCorrectCase(char *buf) {
  ...
  while ((fname != NULL) && (*fname != '\0')) {
    ...
    *(fname - 1) = '\0'; /* isolate parent dir string. */
    ...
    rc = DosFindFirst((unsigned char *)spec, &hdir, FILE_DIRECTORY, &fb,
...
static void prepUnicodeSupport(void) {
  ...
  char buf[CCHMAXPATH];
  UniChar defstr[] = {0};
  if (DosLoadModule((PSZ)buf, sizeof(buf) - 1, (PCSZ) "uconv", &uconvdll) ==
      NO_ERROR) {
#define LOAD(x)                                                                \
  (DosQueryProcAddr(uconvdll, 0, (PCSZ) #x, (PFN *)&p##x) == NO_ERROR)
    ok = LOAD(UniCreateUconvObject) && LOAD(UniFreeUconvObject) &&
         LOAD(UniUconvToUcs) && LOAD(UniUconvFromUcs);
#undef LOAD
  } /* else */
...
static int isCdRomDrive(ULONG drive) {
  ...
  rc = DosDevIOCtl(hfile, IOCTL_CDROMDISK, CDROMDISK_GETDRIVER, &param,
                   sizeof(param), &ul1, &data, sizeof(data), &ul2);
...
__PHYSFS_platformEnumerate(const char *dirname,
...
static HFILE openFile(const char *filename, const ULONG flags,
                      const ULONG mode) {
  ...
  rc = DosOpen((PCSZ)cpfname, &hfile, &action, 0, FILE_NORMAL, flags, mode,
               NULL);
...
void *__PHYSFS_platformOpenRead(const char *filename) {
  ...
  return (void *)openFile(
      filename, OPEN_ACTION_OPEN_IF_EXISTS | OPEN_ACTION_FAIL_IF_NEW,
...
void *__PHYSFS_platformOpenWrite(const char *filename) {
  return (void *)openFile(
      filename, OPEN_ACTION_REPLACE_IF_EXISTS | OPEN_ACTION_CREATE_IF_NEW,
      OPEN_FLAGS_FAIL_ON_ERROR | OPEN_FLAGS_NO_LOCALITY | OPEN_FLAGS_NOINHERIT |
...
void *__PHYSFS_platformOpenAppend(const char *filename) {
   ...
   */
  hfile = openFile(
      filename, OPEN_ACTION_OPEN_IF_EXISTS | OPEN_ACTION_CREATE_IF_NEW,
      OPEN_FLAGS_FAIL_ON_ERROR | OPEN_FLAGS_NO_LOCALITY | OPEN_FLAGS_NOINHERIT |
...
PHYSFS_sint64 __PHYSFS_platformRead(void *opaque, void *buf,
                                    PHYSFS_uint64 len) {
  ULONG br = 0;
  APIRET rc;
...
int __PHYSFS_platformDelete(const char *path) {
  ...
  rc = (fs.attrFile & FILE_DIRECTORY) ? DosDeleteDir((PCSZ)path)
                                      : DosDelete((PCSZ)path);
  GOTO_IF(rc != NO_ERROR, errcodeFromAPIRET(rc), done);
  retval = 1; /* success */
...
int __PHYSFS_platformStat(const char *filename, PHYSFS_Stat *stat,
  ...
  stat->accesstime =
      os2TimeToUnixTime(&fs.fdateLastAccess, &fs.ftimeLastAccess);
