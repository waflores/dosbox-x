...
int __PHYSFS_msvc_vsnprintf(char *outBuf, size_t size, const char *format,
                            va_list ap) {
  int count = -1;
...
static PHYSFS_sint64 nativeIo_read(PHYSFS_Io *io, void *buf,
                                   PHYSFS_uint64 len) {
...
static PHYSFS_sint64 memoryIo_read(PHYSFS_Io *io, void *buf,
                                   PHYSFS_uint64 len) {
...
static PHYSFS_sint64 handleIo_read(PHYSFS_Io *io, void *buf,
                                   PHYSFS_uint64 len) {
...
int PHYSFS_setWriteDir(const char *newDir) {
  ...
  if (writeDir != NULL) {
    BAIL_IF_MUTEX_ERRPASS(!freeDirHandle(writeDir, openWriteList), stateLock,
                          0);
    writeDir = NULL;
  } /* if */
...
static int doMount(PHYSFS_Io *io, const char *fname, const char *mountPoint,
                   int appendToPath) {
  DirHandle *dh;
  DirHandle *prev = NULL;
  DirHandle *i;
...
int PHYSFS_mountIo(PHYSFS_Io *io, const char *fname, const char *mountPoint,
                   int appendToPath) {
...
setSaneCfgEnumCallback(void *_data, const char *dir, const char *f) {
  (void)dir;
  ...
  /*return (data->errcode != PHYSFS_ERR_OK) ? PHYSFS_ENUM_ERROR :
   * PHYSFS_ENUM_OK;*/
...
static int locateInStringList(const char *str, char **list,
                              PHYSFS_uint32 *pos) {
...
static PHYSFS_EnumerateCallbackResult
enumFilesCallback(void *data, const char *origdir, const char *str) {
  (void)origdir; // UNUSED
  PHYSFS_uint32 pos;
  void *ptr;
  char *newstr;
 ...
 */
static PHYSFS_EnumerateCallbackResult
enumerateFromMountPoint(DirHandle *i, const char *arcfname,
                        PHYSFS_EnumerateCallback callback, const char *_fname,
                        void *data) {
...
static PHYSFS_EnumerateCallbackResult
enumCallbackFilterSymLinks(void *_data, const char *origdir,
                           const char *fname) {
    ...
    for (i = searchPath; (retval == PHYSFS_ENUM_OK) && i; i = i->next) {
      ...
      if (partOfMountPoint(i, arcfname))
        ...
        if (statbuf.filetype != PHYSFS_FILETYPE_DIRECTORY)
          ...
          retval =
              i->funcs->enumerate(i->opaque, arcfname,
                                  enumCallbackFilterSymLinks, _fn, &filterdata);
          if (retval == PHYSFS_ENUM_ERROR) {
...
static PHYSFS_EnumerateCallbackResult
enumFilesCallbackAlwaysSucceed(void *d, const char *origdir,
                               const char *fname) {
...
static inline PHYSFS_uint32 hashPathName(__PHYSFS_DirTree *dt,
                                         const char *name) {
...
void *__PHYSFS_DirTreeAdd(__PHYSFS_DirTree *dt, char *name, const int isdir) {
  __PHYSFS_DirTreeEntry *retval =
      (__PHYSFS_DirTreeEntry *)__PHYSFS_DirTreeFind(dt, name);
  if (!retval) {
...
__PHYSFS_DirTreeEnumerate(void *opaque, const char *dname,
                          PHYSFS_EnumerateCallback cb, const char *origdir,
                          void *callbackdata) {
  PHYSFS_EnumerateCallbackResult retval = PHYSFS_ENUM_OK;
  __PHYSFS_DirTree *tree = (__PHYSFS_DirTree *)opaque;
  const __PHYSFS_DirTreeEntry *entry =
      (const __PHYSFS_DirTreeEntry *)__PHYSFS_DirTreeFind(tree, dname);
