...
static inline int readui32v(PHYSFS_Io *io, PHYSFS_uint32 *val) {
  PHYSFS_uint32 v;
...
static int vdfLoadEntries(PHYSFS_Io *io, const PHYSFS_uint32 count,
  ...
  for (i = 0; i < count; i++) {
    ...
    BAIL_IF_ERRPASS(!readui32v(io, &jump), 0);
    BAIL_IF_ERRPASS(!readui32v(io, &size), 0);
    BAIL_IF_ERRPASS(!readui32v(io, &type), 0);
    BAIL_IF_ERRPASS(!readui32v(io, &attr), 0);
...
static void *VDF_openArchive(PHYSFS_Io *io, const char *name, int forWriting,
  ...
  *claimed = 1;
  ...
  BAIL_IF_ERRPASS(!readui32v(io, &count), NULL);
  BAIL_IF_ERRPASS(!__PHYSFS_readAll(io, ignore, 4), NULL); /* numFiles */
  BAIL_IF_ERRPASS(!readui32v(io, &timestamp), NULL);
  BAIL_IF_ERRPASS(!readui32v(io, &dataSize), NULL);      /* dataSize */
  BAIL_IF_ERRPASS(!readui32v(io, &rootCatOffset), NULL); /* rootCatOff */
  BAIL_IF_ERRPASS(!readui32v(io, &version), NULL);
...
const PHYSFS_Archiver __PHYSFS_Archiver_VDF = {
    CURRENT_PHYSFS_ARCHIVER_API_VERSION,
    {
        ...
        "https://github.com/frabert", 0, /* supportsSymlinks */
    },
    VDF_openArchive,
    UNPK_enumerate,
    UNPK_openRead,
