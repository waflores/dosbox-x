...
static inline HANDLE winFindFirstFileW(const WCHAR *path,
                                       LPWIN32_FIND_DATAW d) {
#ifdef PHYSFS_PLATFORM_WINRT
  return FindFirstFileExW(path, FindExInfoStandard, d, FindExSearchNameMatch,
                          NULL, 0);
#else
...
#else
  return CreateFileW(wfname, mode, share, NULL, creation, FILE_ATTRIBUTE_NORMAL,
                     NULL);
#endif
} /* winCreateFileW */
...
static LRESULT CALLBACK detectCDWndProc(HWND hwnd, UINT msg, WPARAM wp,
                                        LPARAM lparam) {
...
static DWORD WINAPI detectCDThread(LPVOID arg) {
  ...
  detectCDHwnd = CreateWindowExA(
      0, classname, winname, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
...
static void detectAvailableCDs(PHYSFS_StringCallback cb, void *data) {
  ...
  if (!detectCDThreadHandle) {
    ...
    detectCDThreadHandle =
        CreateThread(NULL, 0, detectCDThread, &initialDetectDone, 0, NULL);
    if (detectCDThreadHandle)
...
char *__PHYSFS_platformCalcPrefDir(const char *org, const char *app) {
  ...
  if (!SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, NULL,
                                  0, path)))
...
char *__PHYSFS_platformCalcUserDir(void) {
  ...
  pGetDir = (fnGetUserProfDirW)GetProcAddress((HMODULE)lib,
...
int __PHYSFS_platformSeek(void *opaque, PHYSFS_uint64 pos) {
  ...
  BAIL_IF(!winSetFilePointer(h, spos, NULL, FILE_BEGIN), errcodeFromWinApi(),
          0);
...
PHYSFS_sint64 __PHYSFS_platformTell(void *opaque) {
  HANDLE h = (HANDLE)opaque;
  PHYSFS_sint64 pos = 0;
  BAIL_IF(!winSetFilePointer(h, 0, &pos, FILE_CURRENT), errcodeFromWinApi(),
          -1);
  return pos;
...
int __PHYSFS_platformStat(const char *filename, PHYSFS_Stat *st,
  ...
  else if (winstat.dwFileAttributes &
           (FILE_ATTRIBUTE_OFFLINE | FILE_ATTRIBUTE_DEVICE)) {
    st->filetype = PHYSFS_FILETYPE_OTHER;
    st->filesize =
        (((PHYSFS_uint64)winstat.nFileSizeHigh) << 32) | winstat.nFileSizeLow;
  } /* else if */
