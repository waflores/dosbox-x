...
static PHYSFS_uint32 zip_crypto_crc32(const PHYSFS_uint32 crc,
                                      const PHYSFS_uint8 val) {
  int i;
...
static void zip_update_crypto_keys(PHYSFS_uint32 *keys,
                                   const PHYSFS_uint8 val) {
...
static PHYSFS_sint64 zip_read_decrypt(ZIPfileinfo *finfo, void *buf,
                                      PHYSFS_uint64 len) {
...
static int zip_prep_crypto_keys(ZIPfileinfo *finfo,
                                const PHYSFS_uint8 *crypto_header,
  ...
  const PHYSFS_uint8 verifier =
      (PHYSFS_uint8)((usedate ? (entry->dos_mod_time >> 8)
                              : (entry->crc >> 24)) &
                     0xFF);
...
static PHYSFS_sint64 ZIP_write(PHYSFS_Io *io, const void *b,
                               PHYSFS_uint64 len) {
  (void)io;  // UNUSED
  (void)b;   // UNUSED
  (void)len; // UNUSED
...
static PHYSFS_sint64 zip_find_end_of_central_dir(PHYSFS_Io *io,
                                                 PHYSFS_sint64 *len) {
...
static int zip_parse_local(PHYSFS_Io *io, ZIPentry *entry) {
  ...
  BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
  BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) && (ui32 != entry->compressed_size),
          PHYSFS_ERR_CORRUPT, 0);

  BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
  BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) && (ui32 != entry->uncompressed_size),
          PHYSFS_ERR_CORRUPT, 0);
...
static int zip_entry_is_symlink(const ZIPentry *entry) {
  return ((entry->resolved == ZIP_UNRESOLVED_SYMLINK) ||
          (entry->resolved == ZIP_BROKEN_SYMLINK) || (entry->symlink));
...
static ZIPentry *zip_load_entry(ZIPinfo *info, const int zip64,
  ...
  if ((zip64) && ((offset == 0xFFFFFFFF) || (starting_disk == 0xFFFFFFFF) ||
...
static int zip_parse_end_of_central_dir(ZIPinfo *info,
  ...
  rc = zip64_parse_end_of_central_dir(info, data_start, dir_ofs, entry_count,
                                      pos - 20);
