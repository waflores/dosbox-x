...
int VideoCodec::NeededSize(int _width, int _height, zmbv_format_t _format) {
  ...
  f = f * _width * _height + 2 * (1 + (_width / 8)) * (1 + (_height / 8)) +
      1024;
  return f + f / 1000;
}

bool VideoCodec::SetupBuffers(zmbv_format_t _format, int blockwidth,
                              int blockheight) {
  FreeBuffers();
  palsize = 0;
  switch (_format) {
  ...
  for (int b = 0; b < blockcount; b++) {
    ...
    for (int v = 0; v < VectorCount && possibles; v++) {
      ...
      if (PossibleBlock<P>(vx, vy, block) < 4) {
        possibles--;
        //				if (!possibles) Msg("Ran out of
        //possibles, at %d of %d best %d\n",v,VectorCount,bestchange);
...
bool VideoCodec::PrepareCompressFrame(int flags, zmbv_format_t _format,
                                      char *pal, void *writeBuf,
                                      int writeSize) {
  int i;
  ...
  if (flags & 1) {
    ...
    *firstByte |= Mask_KeyFrame;
    KeyframeHeader *header =
        (KeyframeHeader *)(compress.writeBuf + compress.writeDone);
    ...
    if (palsize && pal && memcmp(pal, palette, (unsigned int)palsize * 4u)) {
      *firstByte |= Mask_DeltaPalette;
      for (i = 0; i < palsize; i++) {
        work[workUsed++] =
            (unsigned char)palette[i * 4 + 0] ^ (unsigned char)pal[i * 4 + 0];
        work[workUsed++] =
            (unsigned char)palette[i * 4 + 1] ^ (unsigned char)pal[i * 4 + 1];
        work[workUsed++] =
            (unsigned char)palette[i * 4 + 2] ^ (unsigned char)pal[i * 4 + 2];
      }
...
void VideoCodec::CompressLines(int lineCount, void *lineData[]) {
  ...
  unsigned char *destStart =
      newframe +
      pixelsize * (MAX_VECTOR + (compress.linesDone + MAX_VECTOR) * pitch);
...
int VideoCodec::FinishCompressFrame(void) {
  ...
  if (firstByte & Mask_KeyFrame) {
    ...
    for (i = 0; i < height; i++) {
      memcpy(&work[workUsed], readFrame,
             (unsigned int)width * (unsigned int)pixelsize);
  ...
  zstream.avail_out =
      (unsigned int)compress.writeSize - (unsigned int)compress.writeDone;
  zstream.total_out = 0;
...
bool VideoCodec::DecompressFrame(void *framedata, int size) {
  ...
  if (tag & Mask_KeyFrame) {
    ...
    size -= (int)sizeof(KeyframeHeader);
    data += sizeof(KeyframeHeader);
    if (size <= 0)
      return false;
    if (header->low_version != DBZV_VERSION_LOW ||
        header->high_version != DBZV_VERSION_HIGH)
      return false;
    if (format != (zmbv_format_t)header->format &&
        !SetupBuffers((zmbv_format_t)header->format, header->blockwidth,
                      header->blockheight))
      return false;
  ...
  if (tag & Mask_KeyFrame) {
    ...
    newframe = buf1;
    oldframe = buf2;
    unsigned char *writeframe =
        newframe + pixelsize * (MAX_VECTOR + MAX_VECTOR * pitch);
    for (i = 0; i < height; i++) {
      memcpy(writeframe, &work[workPos],
             (unsigned int)width * (unsigned int)pixelsize);
