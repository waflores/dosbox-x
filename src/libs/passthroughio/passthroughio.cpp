...
#if defined __i386__ || defined __x86_64__ // GCC or Clang
  __asm__ __volatile__("outw %1, %0" : : "d"(port), "a"(value));
#elif defined _MSC_VER && (defined _M_IX86 || defined _M_X64)
#if _MSC_VER >= 1900 // Visual Studio 2015 or later
...
static bool passthroughIO_enabled =
    false; // must default to false for UNIX version
#endif
...
static void dlportio_output_byte(uint16_t port, uint8_t value) {
  DlPortWritePortUchar(port, value);
}

static uint16_t dlportio_input_word(uint16_t port) {
  return DlPortReadPortUshort(port);
}
static void dlportio_output_word(uint16_t port, uint16_t value) {
  DlPortWritePortUshort(port, value);
}

static uint32_t dlportio_input_dword(uint16_t port) {
...
static bool loadIODriver(const char *filename, HMODULE &driver_handle) {
  ...
  if ((driver_handle = LoadLibrary(filename)) == NULL) {
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                  ...
                  NULL, GetLastError(),
                  MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPTSTR)&strptr,
                  0, NULL);
...
bool initPassthroughIO(void) {
  ...
  if (!passthroughIO_enabled) { // We may be called more than once.
    ...
    LOG_MSG("Pass-through I/O caused exception. Right driver required "
    ...
    // if io_driver is non-null the driver stopped working since the previous
    // call
    resetPassthroughIO();
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4191) // 'operator/operation' : unsafe conversion from
...
#pragma GCC diagnostic ignored "-Wcast-function-type"
    ...
    /*
      ...
      prefer it.
    */
    DlPortReadPortUchar = (UCHAR(__stdcall *)(USHORT))GetProcAddress(
        io_driver, "DlPortReadPortUchar");
    ...
    DlPortWritePortUchar = (void(__stdcall *)(USHORT, UCHAR))GetProcAddress(
        io_driver, "DlPortWritePortUchar");
    ...
    DlPortReadPortUshort = (USHORT(__stdcall *)(USHORT))GetProcAddress(
        io_driver, "DlPortReadPortUshort");
    if (DlPortReadPortUshort == NULL) {
      resetPassthroughIO();
      return false;
    }
    input_word = dlportio_input_word;

    DlPortWritePortUshort = (void(__stdcall *)(USHORT, USHORT))GetProcAddress(
        io_driver, "DlPortWritePortUshort");
    if (DlPortWritePortUshort == NULL) {
      resetPassthroughIO();
      return false;
    }
    output_word = dlportio_output_word;

    DlPortReadPortUlong = (ULONG(__stdcall *)(ULONG))GetProcAddress(
        io_driver, "DlPortReadPortUlong");
    if (DlPortReadPortUlong == NULL) {
      resetPassthroughIO();
      return false;
    }
    input_dword = dlportio_input_dword;

    DlPortWritePortUlong = (void(__stdcall *)(ULONG, ULONG))GetProcAddress(
        io_driver, "DlPortWritePortUlong");
    if (DlPortWritePortUlong == NULL) {
      resetPassthroughIO();
      return false;
    }
...
bool initPassthroughIO(void) {
  if (!passthroughIO_enabled) { // We may be called more than once.
    ...
    if (io_fd == -1) {
      LOG_MSG("Error: Could not open I/O port device (/dev/io). Root "
              "privileges required.");
      return false;
    }
    passthroughIO_enabled = true;
#endif
#if (defined __i386__ ||                                                       \
     defined __x86_64__) && /* x86 or AMD64 (GCC or Clang) */                  \
    (defined __OpenBSD__ || defined __NetBSD__ ||                              \
     (defined LINUX && !defined __ANDROID__)) /* OpenBSD, NetBSD or Linux */
...
#endif
#endif
      LOG_MSG("Error: Could not set the I/O privilege level to 3. Root "
              "privileges required.");
      return false;
    }
    passthroughIO_enabled = true;
