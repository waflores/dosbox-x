...
static void SDLAudioCallback(void *data, void *stream, int len) {
...
void fluid_sdl2_audio_driver_settings(fluid_settings_t *settings) {
  ...
  fluid_settings_register_str(settings, "audio.sdl2.device", "default", 0, NULL,
                              NULL);
...
fluid_audio_driver_t *new_fluid_sdl2_audio_driver(fluid_settings_t *settings,
  ...
  if (!SDL_WasInit(SDL_INIT_AUDIO)) {
    FLUID_LOG(FLUID_ERR, "Failed to create SDL2 audio driver, because the "
                         "audio subsystem of SDL2 is not initialized.");
    return NULL;
  }
  ...
  if (period_size < 1024) {
    ...
    if ((period_size & (period_size - 1)) != 0) {
      FLUID_LOG(FLUID_ERR,
                "\"audio.period-size\" must be a power of 2 for SDL2");
      return NULL;
    }
  }
  ...
  if (fluid_settings_dupstr(settings, "audio.sdl2.device", &device) ==
          FLUID_OK &&
