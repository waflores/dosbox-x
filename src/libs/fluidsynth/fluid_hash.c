 ...
 */
static void fluid_hashtable_remove_node(fluid_hashtable_t *hashtable,
                                        fluid_hashnode_t ***node_ptr_ptr,
                                        int notify) {
  fluid_hashnode_t **node_ptr, *node;
 ...
 * for the key and value of the hash node.
 */
static void fluid_hashtable_remove_all_nodes(fluid_hashtable_t *hashtable,
                                             int notify) {
 ...
 */
static inline void fluid_hashtable_maybe_resize(fluid_hashtable_t *hashtable) {
 ...
 *   #fluid_hashtable_t data structure. The fluid_direct_hash(),
 *fluid_int_hash() and fluid_str_hash() functions are provided for some common
 *types of keys. If hash_func is %NULL, fluid_direct_hash() is used.
 * @key_equal_func: a function to check two keys for equality.  This is
 *   used when looking up keys in the #fluid_hashtable_t.  The
 *fluid_direct_equal(), fluid_int_equal() and fluid_str_equal() functions are
 *provided for the most common types of keys. If @key_equal_func is %NULL, keys
 *are compared directly in a similar fashion to fluid_direct_equal(), but
 ...
 *
 * Creates a new #fluid_hashtable_t like fluid_hashtable_new() with a reference
 *count of 1 and allows to specify functions to free the memory allocated for
 *the key and value that get called when removing the entry from the
 *#fluid_hashtable_t.
 *
 * Return value: a new #fluid_hashtable_t.
 **/
...
new_fluid_hashtable_full(fluid_hash_func_t hash_func,
  ...
  FLUID_MEMSET(hashtable->nodes, 0,
               hashtable->size * sizeof(fluid_hashnode_t *));
 ...
 * Since: 2.16
 **/
void fluid_hashtable_iter_init(fluid_hashtable_iter_t *iter,
                               fluid_hashtable_t *hashtable) {
 ...
 * Since: 2.16
 **/
int fluid_hashtable_iter_next(fluid_hashtable_iter_t *iter, void **key,
                              void **value) {
  RealIter *ri = (RealIter *)iter;
 ...
 * you should either free them first or create the #fluid_hashtable_t with
 *destroy notifiers using fluid_hashtable_new_full(). In the latter case the
 *destroy functions you supplied will be called on all keys and values during
 ...
 * new node.
 */
static void fluid_hashtable_insert_internal(fluid_hashtable_t *hashtable,
                                            void *key, void *value,
                                            int keep_new_key) {
  fluid_hashnode_t **node_ptr, *node;
 ...
 **/
void fluid_hashtable_insert(fluid_hashtable_t *hashtable, void *key,
                            void *value) {
 ...
 * @value_destroy_func when creating the #fluid_hashtable_t, the old value is
 *freed using that function. If you supplied a @key_destroy_func when creating
 *the #fluid_hashtable_t, the old key is freed using that function.
 **/
void fluid_hashtable_replace(fluid_hashtable_t *hashtable, void *key,
                             void *value) {
 ...
 */
static int fluid_hashtable_remove_internal(fluid_hashtable_t *hashtable,
 ...
 *
 * If the #fluid_hashtable_t was created using fluid_hashtable_new_full(), the
 *keys and values are freed using the supplied destroy functions, otherwise you
 ...
 * #fluid_hashtable_t. If you supplied key or value destroy functions when
 *creating the #fluid_hashtable_t, they are used to free the memory allocated
 *for the removed keys and values.
 *
 ...
 **/
unsigned int fluid_hashtable_foreach_remove(fluid_hashtable_t *hashtable,
                                            fluid_hr_func_t func,
                                            void *user_data) {
  fluid_return_val_if_fail(hashtable != NULL, 0);
  fluid_return_val_if_fail(func != NULL, 0);

  return fluid_hashtable_foreach_remove_or_steal(hashtable, func, user_data,
                                                 TRUE);
}
 ...
 * Since: 2.4
 **/
void *fluid_hashtable_find(fluid_hashtable_t *hashtable,
                           fluid_hr_func_t predicate, void *user_data) {
  fluid_hashnode_t *node;
  int i;
 ...
 * Returns: %TRUE if the two keys match.
 */
int fluid_direct_equal(const void *v1, const void *v2) { return v1 == v2; }
 ...
 * Returns: a hash value corresponding to the key.
 */
unsigned int fluid_int_hash(const void *v) { return *(const int *)v; }
#endif