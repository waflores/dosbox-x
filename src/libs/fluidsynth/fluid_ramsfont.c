...
fluid_rampreset_izoneforsample(fluid_rampreset_t *preset,
...
static int fluid_rampreset_noteon(fluid_rampreset_t *preset,
...
static fluid_preset_t *fluid_ramsfont_sfont_get_preset(fluid_sfont_t *sfont,
  ...
  rampreset =
      fluid_ramsfont_get_preset((fluid_ramsfont_t *)sfont->data, bank, prenum);
...
static int fluid_rampreset_preset_noteon(fluid_preset_t *preset,
                                         fluid_synth_t *synth, int chan,
                                         int key, int vel) {
  return fluid_rampreset_noteon((fluid_rampreset_t *)preset->data, synth, chan,
                                key, vel);
}
...
static int delete_fluid_ramsfont(fluid_ramsfont_t *sfont) {
  ...
  for (list = sfont->sample; list; list = fluid_list_next(list)) {
    /* in ram soundfonts, the samples hold their data : so we should free it
     * ourselves */
    fluid_sample_t *sam = (fluid_sample_t *)fluid_list_get(list);
 ...
 * @param name Name to assign (should be 20 chars in length with a NULL
 * terminator)
 * @return #FLUID_OK
 */
int fluid_ramsfont_set_name(fluid_ramsfont_t *sfont, const char *name) {
 ...
 */
int fluid_ramsfont_add_izone(fluid_ramsfont_t *sfont, unsigned int bank,
 ...
 * @return #FLUID_OK on success, #FLUID_FAILED otherwise
 */
int fluid_ramsfont_remove_izone(fluid_ramsfont_t *sfont, unsigned int bank,
                                unsigned int num, fluid_sample_t *sample) {
  int err;
  fluid_rampreset_t *preset = fluid_ramsfont_get_preset(sfont, bank, num);
  if (preset == NULL) {
...
/**
 * Sets a generator on an instrument zone identified by \a bank, \a num and \a
 * sample
 * @param sfont RAM SoundFont
 * @param bank Preset bank number
 * @param num Preset program number
 ...
 * @return #FLUID_OK on success, #FLUID_FAILED otherwise
 */
int fluid_ramsfont_izone_set_gen(fluid_ramsfont_t *sfont, unsigned int bank,
                                 unsigned int num, fluid_sample_t *sample,
 ...
 * @param num Preset program number
 * @param sample Sample of the instrument zone
 * @param on TRUE to enable looping, FALSE for one shot (\a loopstart and \a
 * loopend not used)
 * @param loopstart Loop start, in frames (counted from 0)
 * @param loopend Loop end, in frames (counted from last frame, thus is < 0)
 * @return #FLUID_OK on success, #FLUID_FAILED otherwise
 */
int fluid_ramsfont_izone_set_loop(fluid_ramsfont_t *sfont, unsigned int bank,
                                  unsigned int num, fluid_sample_t *sample,
...
static int fluid_rampreset_izone_set_loop(fluid_rampreset_t *preset,
  ...
  /* NOTE : We should check that (sample->startloop + loopStart <=
   * sample->endloop - loopend - 32) */
  ...
  /* If the loop points are the whole samples, we are supposed to
     ...
     the end to the start margin), but it works fine without this. Maybe some
     time it will be needed (see SAMPLE_LOOP_MARGIN) -- Antoie Schmitt May 2003
   */
    ...
    if (fluid_preset_zone_inside_range(preset_zone, key, vel)) {
      ...
      while (inst_zone != NULL) {
        ...
        if (fluid_inst_zone_inside_range(inst_zone, key, vel) &&
          ...
          for (i = 0; i < GEN_LAST; i++) {
            ...
            if (inst_zone->gen[i].flags) {
              ...
              fluid_voice_gen_set(voice, i,
                                  (float)global_inst_zone->gen[i].val);
           ...
           * event. Exclusive class may only terminate older voices. That avoids
           * killing voices, which have just been created. (a noteon event can
           * create several voice processes with the same exclusive class - for
           * example when using stereo samples)
           */
        }
...
int fluid_sample_set_sound_data(fluid_sample_t *sample, short *data,
  ...
  /* in all cases, the sample has ownership of the data : it will release it in
   * the end */
  ...
  /* only used as markers for the LOOP generators : set them on the first real
   * frame */
 ...
 */
fluid_sample_t *new_fluid_ramsample(void) {
