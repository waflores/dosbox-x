...
#define fluid_private_init(_priv)                                              \
  FLUID_STMT_START {                                                           \
...
#define fluid_thread_get_id()                                                  \
  GetCurrentThreadId() /* Get unique "ID" for current thread */
...
#define fluid_thread_get_id()                                                  \
  pthread_self() /* Get unique "ID" for current thread */
...
typedef int (*fluid_server_func_t)(void *data, fluid_socket_t client_socket,
                                   char *addr);
...
/** Macro to create a variable and assign the current reference time for
 * profiling. So we don't get unused variable warnings when profiling is
 * disabled. */
