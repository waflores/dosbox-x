...
struct _fluid_overflow_prio_t {
  fluid_real_t
      percussion; /**< Is this voice on the drum channel? Then add this score */
  fluid_real_t released;  /**< Is this voice in release stage? Then add this
                             score (usually negative) */
  fluid_real_t sustained; /**< Is this voice sustained? Then add this score
                             (usually negative) */
  fluid_real_t volume; /**< Multiply current (or future) volume (a value between
                          0 and 1) */
  fluid_real_t age; /**< This score will be divided by the number of seconds the
                       voice has lasted */
};
...
struct _fluid_voice_t {
  ...
  /* basic parameters */
  fluid_real_t
      output_rate; /* the sample rate of the synthesizer (dupe in rvoice) */
  ...
  fluid_rvoice_t
      *overflow_rvoice;  /* Used temporarily and only in overflow situations */
  int can_access_rvoice; /* False if rvoice is being rendered in separate thread
                          */
  int can_access_overflow_rvoice; /* False if overflow_rvoice is being rendered
                                     in separate thread */
...
/**  fluid_voice_release
 ...
 See fluid_synth_damp_voices_LOCAL(),
 fluid_synth_damp_voices_by_sostenuto_LOCAL, fluid_voice_noteoff(),
 fluid_synth_stop_LOCAL().
*/
 ...
 */
static FLUID_INLINE void fluid_voice_unlock_rvoice(fluid_voice_t *voice) {
...
#define _PLAYING(voice)                                                        \
  (((voice)->status == FLUID_VOICE_ON) || _SUSTAINED(voice) ||                 \
...
#define _HELD_BY_SOSTENUTO(voice)                                              \
...
#define fluid_voice_get_loudness(voice)                                        \
  (fluid_adsr_env_get_max_val(&voice->volenv))
