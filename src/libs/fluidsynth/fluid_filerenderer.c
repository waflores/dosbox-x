...
/* File endian byte order names. */
const char *endian_names[] = {"cpu", NULL};
...
void fluid_file_renderer_settings(fluid_settings_t *settings) {
  ...
  fluid_settings_register_str(settings, "audio.file.type", "auto", 0, NULL,
                              NULL);
  fluid_settings_register_str(settings, "audio.file.format", "s16", 0, NULL,
                              NULL);
  fluid_settings_register_str(settings, "audio.file.endian", "auto", 0, NULL,
                              NULL);
  ...
  fluid_settings_register_str(settings, "audio.file.name", "fluidsynth.raw", 0,
                              NULL, NULL);
  fluid_settings_register_str(settings, "audio.file.type", "raw", 0, NULL,
                              NULL);
  fluid_settings_add_option(settings, "audio.file.type", "raw");
  fluid_settings_register_str(settings, "audio.file.format", "s16", 0, NULL,
                              NULL);
  fluid_settings_add_option(settings, "audio.file.format", "s16");
  fluid_settings_register_str(settings, "audio.file.endian", "cpu", 0, NULL,
                              NULL);
 ...
 *   - audio.file.endian: Endian byte order, "auto" for file type's default byte
 * order
...
fluid_file_renderer_t *new_fluid_file_renderer(fluid_synth_t *synth) {
  ...
  dev->synth = synth;
  fluid_settings_getint(synth->settings, "audio.period-size",
                        &dev->period_size);
  ...
  retval =
      fluid_file_renderer_parse_options(type, format, endian, filename, &info);
  ...
  /* Search for valid format for given file type, if invalid and no format was
   * specified. To handle Ogg/Vorbis and possibly future file types with new
   * formats. Checking if format is SF_FORMAT_PCM_16 isn't a fool proof way to
   * check if format was specified or not (if user specifies "s16" itself), but
   * should suffice. */
  ...
  if (!dev->sndfile) {
    FLUID_LOG(FLUID_ERR, "Failed to open audio file '%s' for writing",
              filename);
    goto error_recovery;
  }
 ...
 * @since 1.1.7
 */
int fluid_file_set_encoding_quality(fluid_file_renderer_t *r, double q) {
#if LIBSNDFILE_SUPPORT
  if (sf_command(r->sndfile, SFC_SET_VBR_ENCODING_QUALITY, &q,
                 sizeof(double)) == 0)
    return FLUID_OK;
  else
#endif
...
void delete_fluid_file_renderer(fluid_file_renderer_t *dev) {
  ...
  if (dev->sndfile != NULL) {
    ...
    if (retval != 0)
      FLUID_LOG(FLUID_WARN, "Error closing audio file: %s",
...
int fluid_file_renderer_process_block(fluid_file_renderer_t *dev) {
  ...
  fluid_synth_write_s16(dev->synth, dev->period_size, dev->buf, 0, 2, dev->buf,
                        1, 2);

  for (offset = 0; offset < dev->buf_size; offset += n) {

    n = (int)fwrite((char *)dev->buf + offset, 1, dev->buf_size - offset,
                    dev->file);
    if (n < 0) {
...
/**
 * Parse a colon separated format string and configure an SF_INFO structure
 * accordingly.
 * @param filetype File type string (NULL or "auto" to attempt to identify
 * format by filename extension, with fallback to "wav")
 * @param format File audio format string or NULL to use "s16"
 * @param endian File endian string or NULL to use "auto" which uses the file
 * type's default endian byte order.
 * @param filename File name (used by "auto" type to determine type, based on
 * extension)
...
static int fluid_file_renderer_parse_options(char *filetype, char *format,
  ...
  if (!filetype || FLUID_STRCMP(filetype, "auto") == 0) {
    ...
    if (s && s[1] != '\0') {
      if (!fluid_file_renderer_find_file_type(s + 1, &type))
        FLUID_LOG(FLUID_WARN, "Failed to determine audio file type from "
                              "filename, defaulting to WAV");
    }
  } else if (!fluid_file_renderer_find_file_type(filetype, &type)) {
    FLUID_LOG(FLUID_ERR, "Invalid or unsupported audio file type '%s'",
              filetype);
    return FALSE;
  }
  ...
  if (format) {
    ...
    if (!format_names[i]) {
      FLUID_LOG(FLUID_ERR, "Invalid or unsupported file audio format '%s'",
                format);
      return FALSE;
    }
  ...
  if (endian) {
    ...
    if (!endian_names[i]) {
      FLUID_LOG(FLUID_ERR, "Invalid or unsupported endian byte order '%s'",
                endian);
      return FALSE;
    }
