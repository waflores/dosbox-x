...
struct _fluid_midi_router_t {
  ...
  fluid_midi_router_rule_t
      *rules[FLUID_MIDI_ROUTER_RULE_COUNT]; /* List of rules for each rule type
                                             */
  fluid_midi_router_rule_t
      *free_rules; /* List of rules to free (was waiting for final events which
                      were received) */

  handle_midi_event_func_t
      event_handler;        /* Callback function for generated events */
  ...
  fluid_midi_router_rule_t
      *cmd_rule; /* Rule currently being processed by shell command handler */
...
struct _fluid_midi_router_rule_t {
  ...
  char keys_cc[128];  /* Flags, whether a key is down / controller is set
                         (sustain) */
  fluid_midi_router_rule_t *next; /* next entry */
  int waiting; /* Set to TRUE when rule has been deactivated but there are still
                  pending_events */
};
 ...
 * @param event_handler_data Caller defined data pointer which gets passed to
 * 'handler'
 ...
 * a #fluid_synth_t passed as the \a event_handler_data parameter for this
 * purpose.
 */
 ...
 */
int delete_fluid_midi_router(fluid_midi_router_t *router) {
...
int fluid_midi_router_set_default_rules(fluid_midi_router_t *router) {
  ...
  for (i = 0; i < FLUID_MIDI_ROUTER_RULE_COUNT; i++) {
    ...
    if (!new_rules[i]) { /* Free already allocated rules */
      for (i2 = 0; i2 < i; i2++)
 ...
 * @param rule Rule to add (used directly and should not be accessed again
 * following a successful call to this function).
 * @param type The type of rule to add (#fluid_midi_router_rule_type)
 * @return #FLUID_OK on success, #FLUID_FAILED otherwise (invalid rule for
 * example)
 * @since 1.1.0
 */
int fluid_midi_router_add_rule(fluid_midi_router_t *router,
  ...
  fluid_return_val_if_fail(type >= 0 && type < FLUID_MIDI_ROUTER_RULE_COUNT,
                           FLUID_FAILED);
 ...
 * @param mul Value which is multiplied by matching event's channel value (1.0
 * to not modify)
 * @param add Value which is added to matching event's channel value (0 to not
 * modify)
 * @since 1.1.0
 *
 ...
 * and \a max). If \a min is greater than \a max then rule is inverted and
 * matches everything except in *between* the defined range (so \a min and \a
 * max would match).
 *
 * The \a mul and \a add values are used to modify event channel values prior to
 * sending the event, if the rule matches.
 */
void fluid_midi_router_rule_set_chan(fluid_midi_router_rule_t *rule, int min,
                                     int max, float mul, int add) {
  fluid_return_if_fail(rule != NULL);
  rule->chan_min = min;
  rule->chan_max = max;
  rule->chan_mul = mul;
 ...
 * @param min Minimum value for rule match
 * @param max Maximum value for rule match
 * @param mul Value which is multiplied by matching event's 1st parameter value
 * (1.0 to not modify)
 * @param add Value which is added to matching event's 1st parameter value (0 to
 * not modify)
 * @since 1.1.0
 *
 ...
 * change events its the MIDI program #, for pitch bend events its the bend
 * value, for channel pressure its the channel pressure value and for key
 * pressure its the MIDI note number.
 *
 * Pitch bend values have a maximum value of 16383 (8192 is pitch bend center)
 * and all other events have a max of 127.  All events have a minimum value of
 * 0.
 *
 ...
 * max would match).
 *
 * The \a mul and \a add values are used to modify event 1st parameter values
 * prior to sending the event, if the rule matches.
 */
void fluid_midi_router_rule_set_param1(fluid_midi_router_rule_t *rule, int min,
                                       int max, float mul, int add) {
  fluid_return_if_fail(rule != NULL);
  rule->par1_min = min;
  rule->par1_max = max;
 ...
 * @param min Minimum value for rule match
 * @param max Maximum value for rule match
 * @param mul Value which is multiplied by matching event's 2nd parameter value
 * (1.0 to not modify)
 * @param add Value which is added to matching event's 2nd parameter value (0 to
 * not modify)
 * @since 1.1.0
 *
 * The 2nd parameter of an event depends on the type of event.  For note events
 * its the MIDI velocity, for CC events its the control value and for key
 * pressure events its the key pressure value.  All other types lack a 2nd
 * parameter.
 *
 ...
 * max would match).
 *
 * The \a mul and \a add values are used to modify event 2nd parameter values
 * prior to sending the event, if the rule matches.
 */
void fluid_midi_router_rule_set_param2(fluid_midi_router_rule_t *rule, int min,
                                       int max, float mul, int add) {
  fluid_return_if_fail(rule != NULL);
  rule->par2_min = min;
  rule->par2_max = max;
 ...
 *
 * In default mode, a noteon event is just forwarded to the synth's 'noteon'
 * function, a 'CC' event to the synth's 'CC' function and so on.
 *
 * The router can be used to:
 * - filter messages (for example: Pass sustain pedal CCs only to selected
 * channels)
 * - split the keyboard (noteon with notenr < x: to ch 1, >x to ch 2)
 * - layer sounds (for each noteon received on ch 1, create a noteon on ch1,
 * ch2, ch3,...)
 * - velocity scaling (for each noteon event, scale the velocity by 1.27 to give
 * DX7 users a chance)
...
int fluid_midi_router_handle_midi_event(void *data, fluid_midi_event_t *event) {
  ...
  int event_has_par2 =
      0; /* Flag, indicates that current event needs two parameters */
  ...
  for (rule = rulep ? *rulep : NULL; rule; prev_rule = rule, rule = next_rule) {
    ...
    next_rule =
        rule->next; /* Rule may get removed from list, so get next here */

    /* Channel window */
    if (rule->chan_min >
        rule->chan_max) { /* Inverted rule: Exclude everything between max and
                             min (but not min/max) */
    ...
    /* Par 1 window */
    if (rule->par1_min >
        rule->par1_max) { /* Inverted rule: Exclude everything between max and
                             min (but not min/max) */
    ...
    if (event_has_par2 && event->type != NOTE_OFF) {
      if (rule->par2_min >
          rule->par2_max) { /* Inverted rule: Exclude everything between max and
                               min (but not min/max) */
        if (event_par2 > rule->par2_max && event_par2 < rule->par2_min)
          continue;
      } else /* Normal rule: Exclude everything < max or > min (but not
                min/max)*/
      {
        if (event_par2 > rule->par2_max || event_par2 < rule->par2_min)
          continue;
     ...
     * par1 (maybe par2). We keep track on the state of noteon and sustain pedal
     * events. If the application tries to delete a rule, it will only be fully
     * removed, if pending noteoff / pedal off events have arrived. In the
     * meantime while waiting, it will only let through 'negative' events
     * (noteoff or pedal up).
     */
      ...
      if (rule->keys_cc[par1] == 0) {
        ...
        rule->pending_events++;
               ...
               (event->type == CONTROL_CHANGE && par1 == SUSTAIN_SWITCH &&
                par2 < 64)) { /* Noteoff or sustain pedal up event generated */
      if (rule->keys_cc[par1] > 0) {
        rule->keys_cc[par1] = 0;
        rule->pending_events--;
        ...
        if (rule->waiting) {
          if (rule->pending_events == 0) { /* Remove rule from rule list */
            if (prev_rule)
              prev_rule->next = next_rule;
            else
              *rulep = next_rule;
            ...
            rule =
                prev_rule; /* Set rule to previous rule, which gets assigned to
                              the next prev_rule value (in for() statement) */
          }
  ...
  send_event:
    ...
    /* FIXME - What should be done on failure?  For now continue to process
     * events, but return failure to caller. */
    if (router->event_handler(router->event_handler_data, &new_event) !=
        FLUID_OK)
...
#define CHECK_VALID_ROUTER(_router, _out)                                      \
  if (router == NULL) {                                                        \
    fluid_ostream_printf(                                                      \
        out, "cannot execute router command without a midi router.\n");        \
    return FLUID_FAILED;                                                       \
  }
...
int fluid_midi_router_handle_end(fluid_synth_t *synth, int ac, char **av,
  ...
  /* Add the rule */
  if (fluid_midi_router_add_rule(router, router->cmd_rule,
                                 router->cmd_rule_type) != FLUID_OK)
...
int fluid_midi_router_handle_chan(fluid_synth_t *synth, int ac, char **av,
  ...
  if (ac != 4) {
    fluid_ostream_printf(out,
                         "router_chan needs four args: min, max, mul, add.");
    return FLUID_FAILED;
  }
...
int fluid_midi_router_handle_par1(fluid_synth_t *synth, int ac, char **av,
  ...
  if (ac != 4) {
    fluid_ostream_printf(out,
                         "router_par1 needs four args: min, max, mul, add.");
    return FLUID_FAILED;
  }
...
int fluid_midi_router_handle_par2(fluid_synth_t *synth, int ac, char **av,
  ...
  if (ac != 4) {
    fluid_ostream_printf(out,
                         "router_par2 needs four args: min, max, mul, add.");
    return FLUID_FAILED;
  }
 ...
 * stdout.  Useful for adding into a MIDI router chain for debugging MIDI
 * events.
 */
