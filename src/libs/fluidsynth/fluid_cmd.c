...
#define MAX_COMMAND_LEN                                                        \
  1024 /* max command length accepted by fluid_command() */
...
static void fluid_shell_init(fluid_shell_t *shell, fluid_settings_t *settings,
                             fluid_cmd_handler_t *handler, fluid_istream_t in,
                             fluid_ostream_t out);
...
void fluid_shell_settings(fluid_settings_t *settings) {
  ...
  fluid_settings_register_int(settings, "shell.port", 9800, 1, 65535, 0, NULL,
                              NULL);
}
...
fluid_cmd_t fluid_commands[] = {
    {"help", "general", (fluid_cmd_func_t)fluid_handle_help, NULL,
     "help                       Show help topics ('help TOPIC' for more "
     "info)"},
    ...
    {"pitch_bend_range", "event",
     (fluid_cmd_func_t)fluid_handle_pitch_bend_range, NULL,
    ...
    {"load", "general", (fluid_cmd_func_t)fluid_handle_load, NULL,
     "load file [reset] [bankofs] Load SoundFont (reset=0|1, def 1; bankofs=n, "
     "def 0)"},
    {"unload", "general", (fluid_cmd_func_t)fluid_handle_unload, NULL,
     "unload id [reset]          Unload SoundFont by ID (reset=0|1, default "
     "1)"},
    ...
    {"inst", "general", (fluid_cmd_func_t)fluid_handle_inst, NULL,
     "inst font                  Print out the available instruments for the "
     "font"},
    ...
    {"cho_set_speed", "chorus", (fluid_cmd_func_t)fluid_handle_chorusspeed,
     NULL, "cho_set_speed num          Set mod speed of chorus to num (Hz)"},
    ...
    {"reset", "general", (fluid_cmd_func_t)fluid_handle_reset, NULL,
     "reset                      System reset (all notes off, reset "
     "controllers)"},
    ...
    {"info", "settings", (fluid_cmd_func_t)fluid_handle_info, NULL,
     "info name                  Get information about a controller or "
     "settings"},
...
/* LADSPA-related commands */
    ...
    {"ladspa_add", "ladspa", (fluid_cmd_func_t)fluid_LADSPA_handle_add, NULL,
     "ladspa_add lib plugin n1 <- p1 n2 -> p2 ... Loads and connects LADSPA "
     "plugin"},
    {"ladspa_start", "ladspa", (fluid_cmd_func_t)fluid_LADSPA_handle_start,
     NULL, "ladspa_start               Starts LADSPA effect unit"},
    {"ladspa_declnode", "ladspa",
     (fluid_cmd_func_t)fluid_LADSPA_handle_declnode, NULL,
     "ladspa_declnode node value Declares control node `node' with value "
     "`value'"},
    {"ladspa_setnode", "ladspa", (fluid_cmd_func_t)fluid_LADSPA_handle_setnode,
     NULL, "ladspa_setnode node value  Assigns `value' to `node'"},
#endif
    {"router_clear", "router", (fluid_cmd_func_t)fluid_midi_router_handle_clear,
     NULL,
     "router_clear               Clears all routing rules from the midi "
     "router"},
    {"router_default", "router",
     (fluid_cmd_func_t)fluid_midi_router_handle_default, NULL,
     "router_default             Resets the midi router to default state"},
    {"router_begin", "router", (fluid_cmd_func_t)fluid_midi_router_handle_begin,
     NULL,
     "router_begin [note|cc|prog|pbend|cpress|kpress]: Starts a new routing "
     "rule"},
    {"router_chan", "router", (fluid_cmd_func_t)fluid_midi_router_handle_chan,
     NULL,
     "router_chan min max mul add      filters and maps midi channels on "
     "current rule"},
    {"router_par1", "router", (fluid_cmd_func_t)fluid_midi_router_handle_par1,
     NULL,
     "router_par1 min max mul add      filters and maps parameter 1 (key/ctrl "
     "nr)"},
    {"router_par2", "router", (fluid_cmd_func_t)fluid_midi_router_handle_par2,
     NULL,
     "router_par2 min max mul add      filters and maps parameter 2 (vel/cc "
     "val)"},
    {"router_end", "router", (fluid_cmd_func_t)fluid_midi_router_handle_end,
     NULL,
 ...
 */
int fluid_command(fluid_cmd_handler_t *handler, const char *cmd,
                  fluid_ostream_t out) {
...
fluid_shell_t *new_fluid_shell(fluid_settings_t *settings,
                               fluid_cmd_handler_t *handler, fluid_istream_t in,
                               fluid_ostream_t out, int thread) {
  ...
  /*  if (thread) {
      shell->thread = new_fluid_thread((fluid_thread_func_t) fluid_shell_run,
    shell, 0, TRUE); if (shell->thread == NULL) { delete_fluid_shell(shell);
        return NULL;
      }
    } else {
    */
...
static int fluid_shell_run(fluid_shell_t *shell) {
  ...
  if (shell->settings)
    fluid_settings_dupstr(shell->settings, "shell.prompt",
                          &prompt); /* ++ alloc prompt */

  /* handle user input */
  while (cont) {

    n = fluid_istream_readline(shell->in, shell->out, prompt ? prompt : "",
                               workline, FLUID_WORKLINELENGTH);
...
void fluid_usershell(fluid_settings_t *settings, fluid_cmd_handler_t *handler) {
  fluid_shell_t shell;
  fluid_shell_init(&shell, settings, handler, fluid_get_stdin(),
                   fluid_get_stdout());
  fluid_shell_run(&shell);
}
 ...
 * @return Returns \a buf pointer or NULL if no user command file for this
 * system type.
 */
 ...
 * @param buf Caller supplied string buffer to store file name to.
 * @param len Length of \a buf
 * @return Returns \a buf pointer or NULL if no system command file for this
 * system type.
 */
...
int fluid_handle_noteoff(fluid_synth_t *synth, int ac, char **av,
                         fluid_ostream_t out) {
  if (ac < 2) {
...
int fluid_handle_pitch_bend(fluid_synth_t *synth, int ac, char **av,
                            fluid_ostream_t out) {
  if (ac < 2) {
...
int fluid_handle_pitch_bend_range(fluid_synth_t *synth, int ac, char **av,
                                  fluid_ostream_t out) {
  int channum;
  int value;
  if (ac < 2) {
...
int fluid_handle_cc(fluid_synth_t *synth, int ac, char **av,
                    fluid_ostream_t out) {
  if (ac < 3) {
...
int fluid_handle_prog(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  if (ac < 2) {
...
int fluid_handle_select(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  int sfont_id;
  int chan;
  int bank;
...
int fluid_handle_inst(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  int font;
  fluid_sfont_t *sfont;
...
int fluid_handle_channels(fluid_synth_t *synth, int ac, char **av,
                          fluid_ostream_t out) {
  ...
  for (i = 0; i < fluid_synth_count_midi_channels(synth); i++) {
    ...
    if (!verbose)
      ...
      fluid_ostream_printf(out, "chan %d, sfont %d, bank %d, preset %d, %s\n",
                           i, info.sfont_id, info.bank, info.program,
                           info.assigned ? info.name : "no preset");
  }
...
int fluid_handle_load(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  char buf[1024];
  int id;
  int reset = 1;
...
int fluid_handle_unload(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  int reset = 1;
  if (ac < 1) {
...
int fluid_handle_reload(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  if (ac < 1) {
...
int fluid_handle_fonts(fluid_synth_t *synth, int ac, char **av,
                       fluid_ostream_t out) {
  int i;
  fluid_sfont_t *sfont;
  int num;
...
int fluid_handle_mstat(fluid_synth_t *synth, int ac, char **av,
                       fluid_ostream_t out) {
  /*    fluid_ostream_printf(out, "Dvr=%s, Dev=%s\n",  */
  /*  	 fluid_midi_handler_get_driver_name(midi), */
  /*  	 fluid_midi_handler_get_device_name(midi)); */
  /*    fluid_ostream_printf(out, "Stat=%s, On=%d, Off=%d, Prog=%d, Pbend=%d,
   * Err=%d\n",  */
...
int fluid_handle_reverbpreset(fluid_synth_t *synth, int ac, char **av,
                              fluid_ostream_t out) {
...
int fluid_handle_reverbsetroomsize(fluid_synth_t *synth, int ac, char **av,
                                   fluid_ostream_t out) {
  ...
  fluid_synth_set_reverb_full(synth, FLUID_REVMODEL_SET_ROOMSIZE, room_size,
                              0.0, 0.0, 0.0);
  return 0;
}
...
int fluid_handle_reverbsetdamp(fluid_synth_t *synth, int ac, char **av,
                               fluid_ostream_t out) {
  fluid_real_t damp;
  if (ac < 1) {
  ...
  fluid_synth_set_reverb_full(synth, FLUID_REVMODEL_SET_DAMPING, 0.0, damp, 0.0,
                              0.0);
  return 0;
}
...
int fluid_handle_reverbsetwidth(fluid_synth_t *synth, int ac, char **av,
                                fluid_ostream_t out) {
  fluid_real_t width;
  if (ac < 1) {
  ...
  fluid_synth_set_reverb_full(synth, FLUID_REVMODEL_SET_WIDTH, 0.0, 0.0, width,
                              0.0);
  return 0;
}
...
int fluid_handle_reverbsetlevel(fluid_synth_t *synth, int ac, char **av,
                                fluid_ostream_t out) {
  fluid_real_t level;
  if (ac < 1) {
  ...
  if (abs((int)level) > 30) {
    fluid_ostream_printf(
        out, "rev_setlevel: Value too high! (Value of 10 =+20 dB)\n");
    return 0;
  }
  fluid_synth_set_reverb_full(synth, FLUID_REVMODEL_SET_LEVEL, 0.0, 0.0, 0.0,
                              level);
  return 0;
}
...
int fluid_handle_reverb(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  if (ac < 1) {
  ...
  if ((strcmp(av[0], "0") == 0) || (strcmp(av[0], "off") == 0)) {
    ...
    fluid_ostream_printf(out, "reverb: invalid arguments %s [0|1|on|off]",
                         av[0]);
    return -1;
  }
...
int fluid_handle_chorusnr(fluid_synth_t *synth, int ac, char **av,
                          fluid_ostream_t out) {
  int nr;
  if (ac < 1) {
...
int fluid_handle_choruslevel(fluid_synth_t *synth, int ac, char **av,
                             fluid_ostream_t out) {
  fluid_real_t level;
  if (ac < 1) {
  ...
  level = (fluid_real_t)atof(av[0]);
  return fluid_synth_set_chorus_full(synth, FLUID_CHORUS_SET_LEVEL, 0, level,
                                     0.0, 0.0, 0);
}

/* Purpose:
 * Response to 'chorus_setspeed' command */
int fluid_handle_chorusspeed(fluid_synth_t *synth, int ac, char **av,
                             fluid_ostream_t out) {
  fluid_real_t speed;
  if (ac < 1) {
  ...
  return fluid_synth_set_chorus_full(synth, FLUID_CHORUS_SET_SPEED, 0, 0.0,
                                     speed, 0.0, 0);
}

/* Purpose:
 * Response to 'chorus_setdepth' command */
int fluid_handle_chorusdepth(fluid_synth_t *synth, int ac, char **av,
                             fluid_ostream_t out) {
  fluid_real_t depth;
  if (ac < 1) {
  ...
  return fluid_synth_set_chorus_full(synth, FLUID_CHORUS_SET_DEPTH, 0, 0.0, 0.0,
                                     depth, 0);
}

int fluid_handle_chorus(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  if (ac < 1) {
  ...
  if ((strcmp(av[0], "0") == 0) || (strcmp(av[0], "off") == 0)) {
    ...
    fluid_ostream_printf(out, "chorus: invalid arguments %s [0|1|on|off]",
                         av[0]);
    return -1;
  }
 ...
 */
int fluid_handle_echo(fluid_cmd_handler_t *handler, int ac, char **av,
                      fluid_ostream_t out) {
  if (ac < 1) {
...
int fluid_handle_source(fluid_cmd_handler_t *handler, int ac, char **av,
                        fluid_ostream_t out) {
  if (ac < 1) {
...
int fluid_handle_gain(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  float gain;
...
int fluid_handle_interp(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  int interp;
...
/* Purpose:
 * Response to 'interp' command. */
int fluid_handle_interpc(fluid_synth_t *synth, int ac, char **av,
                         fluid_ostream_t out) {
  int interp;
  int chan;
...
int fluid_handle_tuning(fluid_synth_t *synth, int ac, char **av,
                        fluid_ostream_t out) {
  char *name;
  int bank, prog;
...
int fluid_handle_tune(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  int bank, prog, key;
  double pitch;
...
int fluid_handle_settuning(fluid_synth_t *synth, int ac, char **av,
                           fluid_ostream_t out) {
  int chan, bank, prog;
...
int fluid_handle_resettuning(fluid_synth_t *synth, int ac, char **av,
                             fluid_ostream_t out) {
  int chan;
...
int fluid_handle_tunings(fluid_synth_t *synth, int ac, char **av,
                         fluid_ostream_t out) {
  int bank, prog;
  char name[256];
  int count = 0;
...
int fluid_handle_dumptuning(fluid_synth_t *synth, int ac, char **av,
                            fluid_ostream_t out) {
  int bank, prog, i;
  double pitch[128];
  char name[256];
...
int fluid_handle_set(fluid_synth_t *synth, int ac, char **av,
                     fluid_ostream_t out) {
  int hints;
  int ival;
  ...
  case FLUID_INT_TYPE:
    ...
    if (hints & FLUID_HINT_TOGGLED) {
      if (FLUID_STRCMP(av[1], "yes") == 0 || FLUID_STRCMP(av[1], "True") == 0 ||
          FLUID_STRCMP(av[1], "TRUE") == 0 ||
          FLUID_STRCMP(av[1], "true") == 0 || FLUID_STRCMP(av[1], "T") == 0)
        ival = 1;
      else
        ival = atoi(av[1]);
    } else
...
int fluid_handle_get(fluid_synth_t *synth, int ac, char **av,
                     fluid_ostream_t out) {
  if (ac < 1) {
...
static void fluid_handle_settings_iter1(void *data, char *name, int type) {
  struct _fluid_handle_settings_data_t *d =
...
int fluid_handle_settings(fluid_synth_t *synth, int ac, char **av,
                          fluid_ostream_t out) {
  struct _fluid_handle_settings_data_t data;
  ...
  fluid_settings_foreach(fluid_synth_get_settings(synth), &data,
                         fluid_handle_settings_iter1);
  fluid_settings_foreach(fluid_synth_get_settings(synth), &data,
                         fluid_handle_settings_iter2);
  return 0;
}
...
void fluid_handle_print_option(void *data, char *name, char *option) {
  struct _fluid_handle_option_data_t *d =
...
int fluid_handle_info(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  ...
  case FLUID_NUM_TYPE: {
    ...
    fluid_ostream_printf(out, "Real-time:     %s\n",
                         fluid_settings_is_realtime(settings, av[0]) ? "yes"
                                                                     : "no");
    break;
  }
  ...
  case FLUID_STR_TYPE: {
    ...
    fluid_settings_foreach_option(settings, av[0], &data,
                                  fluid_handle_print_option);
...
int fluid_handle_reset(fluid_synth_t *synth, int ac, char **av,
                       fluid_ostream_t out) {
  fluid_synth_system_reset(synth);
  return 0;
}

int fluid_handle_quit(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  fluid_ostream_printf(out, "cheers!\n");
  return -2;
}

int fluid_handle_help(fluid_synth_t *synth, int ac, char **av,
                      fluid_ostream_t out) {
  /* Purpose:
...
static void fluid_cmd_handler_destroy_hash_value(void *value) {
...
fluid_cmd_handler_t *new_fluid_cmd_handler(fluid_synth_t *synth) {
  ...
  fluid_cmd_t source = {"source", "general",
                        (fluid_cmd_func_t)fluid_handle_source, NULL,
                        "source filename            Load a file and parse "
                        "every line as a command"};

  handler = new_fluid_hashtable_full(fluid_str_hash, fluid_str_equal, NULL,
 ...
 */
int fluid_cmd_handler_unregister(fluid_cmd_handler_t *handler,
                                 const char *cmd) {
  return fluid_hashtable_remove(handler, cmd);
}

int fluid_cmd_handler_handle(fluid_cmd_handler_t *handler, int ac, char **av,
                             fluid_ostream_t out) {
  fluid_cmd_t *cmd;
...
fluid_server_t *new_fluid_server(fluid_settings_t *settings,
  ...
  server->socket = new_fluid_server_socket(
      port, (fluid_server_func_t)fluid_server_handle_connection, server);
...
void fluid_server_remove_client(fluid_server_t *server,
                                fluid_client_t *client) {
  fluid_mutex_lock(server->mutex);
...
static void fluid_client_run(fluid_client_t *client) {
  fluid_shell_t shell;
  fluid_shell_init(&shell, client->settings, client->handler,
...
fluid_client_t *new_fluid_client(fluid_server_t *server,
  ...
  /*client->thread = new_fluid_thread((fluid_thread_func_t) fluid_client_run,
  client, 0, FALSE);
