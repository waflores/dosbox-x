...
BOOL CALLBACK fluid_dsound_enum_callback(LPGUID guid, LPCTSTR description,
                                         LPCTSTR module, LPVOID context) {
  fluid_settings_t *settings = (fluid_settings_t *)context;
  fluid_settings_add_option(settings, "audio.dsound.device",
                            (const char *)description);

  return TRUE;
}

BOOL CALLBACK fluid_dsound_enum_callback2(LPGUID guid, LPCTSTR description,
                                          LPCTSTR module, LPVOID context) {
...
fluid_audio_driver_t *new_fluid_dsound_audio_driver(fluid_settings_t *settings,
  ...
  } else if (fluid_settings_str_equal(settings, "audio.sample-format",
                                      "16bits")) {
  ...
  /* get the selected device name. if none is specified, use NULL for the
   * default device. */
  if (fluid_settings_dupstr(settings, "audio.dsound.device", &devsel.devname) ==
          FLUID_OK /* ++ alloc device name */
      && devsel.devname && strlen(devsel.devname) > 0) {
    /* look for the GUID of the selected device */
    DirectSoundEnumerate((LPDSENUMCALLBACK)fluid_dsound_enum_callback2,
                         (void *)&devsel);
  }
  ...
  hr = IDirectSound_SetCooperativeLevel(dev->direct_sound, GetDesktopWindow(),
                                        DSSCL_PRIORITY);
  ...
  hr = IDirectSound_CreateSoundBuffer(dev->direct_sound, &desc,
                                      &dev->prim_buffer, NULL);
  ...
  if (hr != DS_OK) {
    FLUID_LOG(FLUID_WARN, "Can't set format of primary sound buffer: %s",
  ...
  if (hr != DS_OK) {
    FLUID_LOG(FLUID_ERR, "dsound: Can't create sound buffer: %s",
              fluid_win32_error(hr));
    goto error_recovery;
  }

  /* Lock */
  hr = IDirectSoundBuffer_Lock(dev->sec_buffer, 0, 0, (void *)&buf1, &bytes1, 0,
  ...
  dev->thread = CreateThread(NULL, 0, fluid_dsound_audio_run, (LPVOID)dev, 0,
                             &dev->threadID);
...
static DWORD WINAPI fluid_dsound_audio_run(LPVOID lpParameter) {
  ...
  for (;;) {
    ...
    if (bytes >= dev->buffer_byte_size) {
      ...
      /* Lock */
      res = IDirectSoundBuffer_Lock(dev->sec_buffer, cur_position, bytes,
                                    (void *)&buf1, &bytes1, (void *)&buf2,
                                    &bytes2, 0);

      if ((res != DS_OK) || (buf1 == NULL)) {
        FLUID_LOG(FLUID_PANIC, "Failed to lock the audio buffer. System lockup "
                               "might follow. Exiting.");
        ExitProcess(0);
      }
