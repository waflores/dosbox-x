...
#if JACK_SUPPORT
    {"jack", new_fluid_jack_midi_driver, delete_fluid_jack_midi_driver,
     fluid_jack_midi_driver_settings},
#endif
#if OSS_SUPPORT
    {"oss", new_fluid_oss_midi_driver, delete_fluid_oss_midi_driver,
     fluid_oss_midi_driver_settings},
#endif
#if ALSA_SUPPORT
    {"alsa_raw", new_fluid_alsa_rawmidi_driver,
     delete_fluid_alsa_rawmidi_driver, fluid_alsa_rawmidi_driver_settings},
    {"alsa_seq", new_fluid_alsa_seq_driver, delete_fluid_alsa_seq_driver,
     fluid_alsa_seq_driver_settings},
#endif
#if WINMIDI_SUPPORT
    {"winmidi", new_fluid_winmidi_driver, delete_fluid_winmidi_driver,
     fluid_winmidi_midi_driver_settings},
#endif
#if MIDISHARE_SUPPORT
    {"midishare", new_fluid_midishare_midi_driver,
     delete_fluid_midishare_midi_driver, NULL},
#endif
#if COREMIDI_SUPPORT
    {"coremidi", new_fluid_coremidi_driver, delete_fluid_coremidi_driver,
...
void fluid_midi_driver_settings(fluid_settings_t *settings) {
  ...
  fluid_settings_register_str(settings, "midi.driver", "winmidi", 0, NULL,
                              NULL);
#elif MIDISHARE_SUPPORT
  fluid_settings_register_str(settings, "midi.driver", "midishare", 0, NULL,
                              NULL);
#elif COREMIDI_SUPPORT
  fluid_settings_register_str(settings, "midi.driver", "coremidi", 0, NULL,
                              NULL);
#else
 ...
 * @param handler MIDI handler callback (for example:
 * fluid_midi_router_handle_midi_event() for MIDI router)
...
fluid_midi_driver_t *new_fluid_midi_driver(fluid_settings_t *settings,
  ...
  for (i = 0; fluid_midi_drivers[i].name != NULL; i++) {
    if (fluid_settings_str_equal(settings, "midi.driver",
  ...
  FLUID_LOG(FLUID_ERR,
            "Couldn't find the requested midi driver. Valid drivers are: %s.",
