...
#define fluid_mwinapi_call(_f, _r, ...)                                        \
  FLUID_STMT_START {                                                           \
    if ((__VA_ARGS__) == (_r))                                                 \
...
#define fluid_mutex_init(_m)                                                   \
  fluid_mwinapi_call(CreateMutex, NULL, ((_m) = CreateMutex(NULL, FALSE, NULL)))
#define fluid_mutex_destroy(_m)                                                \
  fluid_mwinapi_call(CloseHandle, 0, CloseHandle((_m)))
#define fluid_mutex_lock(_m) fluid_win_mutex_lock(&(_m))
#define fluid_mutex_unlock(_m) fluid_win_mutex_unlock(&(_m))

static void fluid_win_mutex_lock(fluid_mutex_t *mutex) {
  if (*mutex == NULL)
...
static void fluid_win_mutex_unlock(fluid_mutex_t *mutex) {
  if (*mutex == NULL)
    return;
...
#define fluid_cond_signal(cond)                                                \
  FLUID_STMT_START {                                                           \
    fluid_mwinapi_call(ReleaseSemaphore, 0, ReleaseSemaphore(*cond, 1, NULL)); \
  }                                                                            \
  FLUID_STMT_END
#define fluid_cond_broadcast(cond) fluid_cond_signal(cond)
#define fluid_cond_wait(cond, mutex)                                           \
  FLUID_STMT_START {                                                           \
    fluid_mwinapi_call(SignalObjectAndWait, WAIT_FAILED,                       \
                       SignalObjectAndWait(*mutex, *cond, INFINITE, FALSE));   \
    fluid_cond_mutex_lock(mutex);                                              \
  }                                                                            \
  FLUID_STMT_END

static FLUID_INLINE fluid_cond_t *new_fluid_cond(void) {
...
#define fluid_rec_mutex_init(_m)                                               \
  FLUID_STMT_START {                                                           \
    ...
    fluid_pthread_call(pthread_mutexattr_init, &_mp->attr);                    \
    fluid_pthread_call(pthread_mutexattr_settype, &_mp->attr,                  \
                       PTHREAD_MUTEX_RECURSIVE);                               \
    fluid_pthread_call(pthread_mutex_init, &_mp->mutex, &_mp->attr);           \
  }                                                                            \
  FLUID_STMT_END
#define fluid_rec_mutex_destroy(_m)                                            \
  FLUID_STMT_START {                                                           \
    fluid_rec_mutex_t *_mp = &(_m);                                            \
    fluid_pthread_call(pthread_mutexattr_destroy, &_mp->attr);                 \
