 ...
 */
void *fluid_get_hinstance(void) { return (void *)fluid_hinstance; }

static long FAR PASCAL fluid_win32_wndproc(HWND hWnd, UINT message,
...
int fluid_win32_create_window(void) {
  ...
  fluid_wnd =
      CreateWindow((LPSTR) "FluidSynth", (LPSTR) "FluidSynth",
                   WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 400, 300,
                   (HWND)NULL, (HMENU)NULL, fluid_hinstance, (LPSTR)NULL);
  if (fluid_wnd == NULL) {
...
HWND fluid_win32_get_window(void) { return fluid_wnd; }
