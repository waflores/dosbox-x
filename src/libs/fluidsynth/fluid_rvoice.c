 ...
 */
static inline int fluid_rvoice_calc_amp(fluid_rvoice_t *voice) {
  ...
  if (fluid_adsr_env_get_section(&voice->envlfo.volenv) ==
    ...
    target_amp = fluid_atten2amp(voice->dsp.attenuation) *
                 fluid_cb2amp(fluid_lfo_get_val(&voice->envlfo.modlfo) *
    ...
    target_amp =
        fluid_atten2amp(voice->dsp.attenuation) *
        fluid_cb2amp(
            960.0f * (1.0f - fluid_adsr_env_get_val(&voice->envlfo.volenv)) +
 ...
 */
static void fluid_rvoice_check_sample_sanity(fluid_rvoice_t *voice) {
  ...
  if (voice->dsp.start > voice->dsp.end) {
    ...
    voice->dsp.start = voice->dsp.end;
    voice->dsp.end = temp;
    /*FLUID_LOG(FLUID_DBG, "Loop / sample sanity check: Changing order of start
     * / end points!"); */
  }
  ...
  if ((voice->dsp.samplemode == FLUID_LOOP_UNTIL_RELEASE) ||
    ...
    if (voice->dsp.loopstart > voice->dsp.loopend) {
      ...
      /*FLUID_LOG(FLUID_DBG, "Loop / sample sanity check: Changing order of loop
       * points!"); */
    }
    ...
    if ((int)voice->dsp.loopstart >= (int)voice->dsp.sample->loopstart &&
      ...
      /* Is there a valid peak amplitude available for the loop, and can we use
       * it? */
      if (voice->dsp.sample->amplitude_that_reaches_noise_floor_is_valid &&
          voice->dsp.samplemode == FLUID_LOOP_DURING_RELEASE) {
        voice->dsp.amplitude_that_reaches_noise_floor_loop =
            (fluid_real_t)(voice->dsp.sample
                               ->amplitude_that_reaches_noise_floor /
                           voice->dsp.synth_gain);
      } else {
        /* Worst case */
        voice->dsp.amplitude_that_reaches_noise_floor_loop =
            voice->dsp.amplitude_that_reaches_noise_floor_nonloop;
  ...
  if (((voice->dsp.samplemode == FLUID_LOOP_UNTIL_RELEASE) &&
       (fluid_adsr_env_get_section(&voice->envlfo.volenv) <
   ...
   * loop from %i to %i", voice->dsp.start, voice->dsp.end,
   * voice->dsp.loopstart, voice->dsp.loopend); */
...
int fluid_rvoice_write(fluid_rvoice_t *voice, fluid_real_t *dsp_buf) {
  ...
  voice->dsp.phase_incr =
      fluid_ct2hz_real(voice->dsp.pitch +
                       fluid_lfo_get_val(&voice->envlfo.modlfo) *
                           voice->envlfo.modlfo_to_pitch +
                       fluid_lfo_get_val(&voice->envlfo.viblfo) *
                           voice->envlfo.viblfo_to_pitch +
                       fluid_adsr_env_get_val(&voice->envlfo.modenv) *
                           voice->envlfo.modenv_to_pitch) /
...
static inline fluid_real_t *get_dest_buf(fluid_rvoice_buffers_t *buffers,
                                         int index, fluid_real_t **dest_bufs,
                                         int dest_bufcount) {
 ...
 * @param samplecount Number of samples to process (no FLUID_BUFSIZE
 * restriction)
...
void fluid_rvoice_buffers_mix(fluid_rvoice_buffers_t *buffers,
  ...
  for (i = 0; i < bufcount; i++) {
    ...
    next_buf = (i + 1 >= bufcount
                    ? NULL
                    : get_dest_buf(buffers, i + 1, dest_bufs, dest_bufcount));
 ...
 */
static int fluid_rvoice_buffers_check_bufnum(fluid_rvoice_buffers_t *buffers,
                                             unsigned int bufnum) {
  unsigned int i;
...
void fluid_rvoice_noteoff(fluid_rvoice_t *voice, unsigned int min_ticks) {
  ...
  if (fluid_adsr_env_get_section(&voice->envlfo.volenv) ==
    ...
    if (fluid_adsr_env_get_val(&voice->envlfo.volenv) > 0) {
      fluid_real_t lfo = fluid_lfo_get_val(&voice->envlfo.modlfo) *
                         -voice->envlfo.modlfo_to_vol;
      fluid_real_t amp =
          (fluid_real_t)(fluid_adsr_env_get_val(&voice->envlfo.volenv) *
                         pow(10.0, lfo / -200));
      fluid_real_t env_value =
          -(fluid_real_t)((-200 * log(amp) / log(10.0) - lfo) / 960.0 - 1);
...
void fluid_rvoice_set_output_rate(fluid_rvoice_t *voice, fluid_real_t value) {
  voice->dsp.output_rate = value;
}

void fluid_rvoice_set_interp_method(fluid_rvoice_t *voice, int value) {
  voice->dsp.interp_method = value;
}

void fluid_rvoice_set_root_pitch_hz(fluid_rvoice_t *voice, fluid_real_t value) {
...
void fluid_rvoice_set_min_attenuation_cB(fluid_rvoice_t *voice,
                                         fluid_real_t value) {
  voice->dsp.min_attenuation_cB = value;
}

void fluid_rvoice_set_viblfo_to_pitch(fluid_rvoice_t *voice,
                                      fluid_real_t value) {
  voice->envlfo.viblfo_to_pitch = value;
}

void fluid_rvoice_set_modlfo_to_pitch(fluid_rvoice_t *voice,
                                      fluid_real_t value) {
...
void fluid_rvoice_set_modenv_to_pitch(fluid_rvoice_t *voice,
                                      fluid_real_t value) {
