...
static void fluid_channel_init(fluid_channel_t *chan) {
  ...
  chan->channel_type =
      (chan->channum == 9) ? CHANNEL_TYPE_DRUM : CHANNEL_TYPE_MELODIC;
  prognum = 0;
...
int fluid_channel_set_preset(fluid_channel_t *chan, fluid_preset_t *preset) {
  ...
  if (chan->preset) {
    ...
    delete_fluid_preset(chan->preset);
    fluid_synth_sfont_unref(chan->synth,
                            sfont); /* -- unref preset's SoundFont */
  }
...
void fluid_channel_set_sfont_bank_prog(fluid_channel_t *chan, int sfontnum,
...
void fluid_channel_set_bank_msb(fluid_channel_t *chan, int bankmsb) {
  ...
  if (style == FLUID_BANK_STYLE_XG) {
    ...
    chan->channel_type =
        (120 <= bankmsb) ? CHANNEL_TYPE_DRUM : CHANNEL_TYPE_MELODIC;
    return;
  }

  if (style == FLUID_BANK_STYLE_GM || chan->channel_type == CHANNEL_TYPE_DRUM)
    return; /* ignored */
