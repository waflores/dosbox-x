...
#define fluid_atomic_pointer_set(_pp, _val)                                    \
  __atomic_store_n(_pp, _val, fluid_atomic_order)

static FLUID_INLINE int
fluid_atomic_pointer_compare_and_exchange(volatile void *_pi,
                                          volatile void *_old, void *_new) {
...
#define fluid_atomic_int_exchange_and_add(_pi, _val)                           \
  __sync_add_and_fetch(_pi, _val)

static FLUID_INLINE int fluid_atomic_int_get(volatile int *_pi) {
  __sync_synchronize();
  return (int)*_pi;
}

static FLUID_INLINE void fluid_atomic_int_set(volatile int *_pi, int _val) {
  *_pi = _val;
  __sync_synchronize();
}
...
static FLUID_INLINE void *fluid_atomic_pointer_get(volatile void *_pi) {
  __sync_synchronize();
  return *(void **)_pi;
}

static FLUID_INLINE void fluid_atomic_pointer_set(volatile void *_pi,
                                                  void *_val) {
  *(void **)_pi = _val;
  __sync_synchronize();
}
...
#define fluid_atomic_int_set(_pi, _val)                                        \
  (void)(InterlockedExchange((LONG volatile *)(_pi), (LONG)(_val)))
#define fluid_atomic_int_dec_and_test(_pi)                                     \
  (InterlockedDecrement((LONG volatile *)(_pi)) == 0)
...
#define fluid_atomic_pointer_set(_pp, _val)                                    \
  (void)(InterlockedExchangePointer(_pp, _val))
...
#define fluid_atomic_int_add(_pi, _val)                                        \
  (void)(fluid_atomic_int_exchange_and_add(_pi, _val))
#define fluid_atomic_int_inc(_pi) fluid_atomic_int_add(_pi, 1)

static FLUID_INLINE void fluid_atomic_float_set(volatile float *fptr,
                                                float val) {
  sint32 ival;
  memcpy(&ival, &val, 4);
