...
static fluid_preset_t *fluid_synth_get_preset_by_sfont_name(
    fluid_synth_t *synth, const char *sfontname, unsigned int banknum,
    unsigned int prognum);

static void fluid_synth_update_presets(fluid_synth_t *synth);
static int fluid_synth_update_sample_rate(fluid_synth_t *synth, char *name,
                                          double value);
static int fluid_synth_update_gain(fluid_synth_t *synth, char *name,
                                   double value);
static void fluid_synth_update_gain_LOCAL(fluid_synth_t *synth);
static int fluid_synth_update_polyphony(fluid_synth_t *synth, char *name,
                                        int value);
...
static int fluid_synth_replace_tuning_LOCK(fluid_synth_t *synth,
                                           fluid_tuning_t *tuning, int bank,
                                           int prog, int apply);
...
static void fluid_synth_set_gen_LOCAL(fluid_synth_t *synth, int chan, int param,
...
static void fluid_synth_register_overflow(fluid_settings_t *settings,
  ...
  fluid_settings_register_num(settings, "synth.overflow.percussion", 4000,
                              -10000, 10000, 0, update_func, update_data);
  ...
  fluid_settings_register_num(settings, "synth.overflow.age", 1000, -10000,
                              10000, 0, update_func, update_data);
...
void fluid_synth_settings(fluid_settings_t *settings) {
  ...
  fluid_settings_register_int(settings, "synth.effects-channels", 2, 2, 2, 0,
                              NULL, NULL);
  fluid_settings_register_num(settings, "synth.sample-rate", 44100.0f, 8000.0f,
                              96000.0f, 0, NULL, NULL);
  fluid_settings_register_int(settings, "synth.device-id", 0, 0, 126, 0, NULL,
                              NULL);
  fluid_settings_register_int(settings, "synth.cpu-cores", 1, 1, 256, 0, NULL,
                              NULL);

  fluid_settings_register_int(settings, "synth.min-note-length", 10, 0, 65535,
                              0, NULL, NULL);
  ...
  fluid_settings_register_str(settings, "synth.midi-bank-select", "gs", 0, NULL,
                              NULL);
...
void fluid_sample_timer_process(fluid_synth_t *synth) {
  ...
  for (st = synth->sample_timers; st; st = stnext) {
    /* st may be freed in the callback below. cache it's successor now to avoid
     * use after free */
    stnext = st->next;
    if (st->isfinished) {
      continue;
    ...
    msec =
        (long)(1000.0 * ((double)(ticks - st->starttick)) / synth->sample_rate);
...
int delete_fluid_sample_timer(fluid_synth_t *synth,
...
static FLUID_INLINE void fluid_synth_update_mixer(fluid_synth_t *synth,
  ...
  fluid_rvoice_eventhandler_push(synth->eventhandler, method,
                                 synth->eventhandler->mixer, intparam,
                                 realparam);
}
...
fluid_synth_t *new_fluid_synth(fluid_settings_t *settings) {
  ...
  fluid_settings_register_num(
      settings, "synth.sample-rate", 44100.0f, 8000.0f, 96000.0f, 0,
  ...
  fluid_settings_register_int(
      settings, "synth.polyphony", synth->polyphony, 1, 65535, 0,
      (fluid_int_update_t)fluid_synth_update_polyphony, synth);
  fluid_settings_register_int(
      settings, "synth.device-id", synth->device_id, 126, 0, 0,
      (fluid_int_update_t)fluid_synth_update_device_id, synth);

  fluid_synth_register_overflow(
      settings, (fluid_num_update_t)fluid_synth_update_overflow, synth);
  ...
  if (synth->effects_channels < 2) {
    FLUID_LOG(FLUID_WARN,
              "Invalid number of effects channels (%d)."
  ...
  synth->eventhandler = new_fluid_rvoice_eventhandler(
      i, synth->polyphony * 64, synth->polyphony, nbuf, synth->effects_channels,
      (fluid_real_t)synth->sample_rate);
  ...
  fluid_rvoice_eventhandler_push5(
      synth->eventhandler, fluid_rvoice_mixer_set_reverb_params,
      synth->eventhandler->mixer, FLUID_REVMODEL_SET_ALL,
      synth->reverb_roomsize, synth->reverb_damping, synth->reverb_width,
  ...
  else if (fluid_settings_str_equal(settings, "synth.midi-bank-select",
                                    "mma") == 1)
...
char *fluid_synth_error(fluid_synth_t *synth) { return fluid_error(); }
  ...
  if (channel->preset == NULL) {
    if (synth->verbose) {
      FLUID_LOG(FLUID_INFO,
                ...
                vel, 0, fluid_synth_get_ticks(synth) / 44100.0f,
...
/* Damp voices on a channel (turn notes off), if they're sustained by
   sustain pedal */
static int fluid_synth_damp_voices_by_sustain_LOCAL(fluid_synth_t *synth,
                                                    int chan) {
  fluid_voice_t *voice;
  int i;
...
/* Damp voices on a channel (turn notes off), if they're sustained by
   sostenuto pedal */
static int fluid_synth_damp_voices_by_sostenuto_LOCAL(fluid_synth_t *synth,
                                                      int chan) {
  fluid_voice_t *voice;
  int i;
...
static int fluid_synth_cc_LOCAL(fluid_synth_t *synth, int channum, int num) {
  ...
  case DATA_ENTRY_MSB: {
    ...
    } else if (fluid_channel_get_cc(chan, RPN_MSB) ==
               0) /* RPN is active: MSB = 0? */
    {
      ...
      case RPN_PITCH_BEND_RANGE: /* Set bend range in semitones */
        ...
        fluid_synth_update_pitch_wheel_sens_LOCAL(
            synth, channum); /* Update bend range */
        /* FIXME - Handle LSB? (Fine bend range in cents) */
        break;
      case RPN_CHANNEL_FINE_TUNE: /* Fine tune is 14 bit over 1 semitone (+/- 50
                                     cents, 8192 = center) */
        fluid_synth_set_gen_LOCAL(synth, channum, GEN_FINETUNE,
                                  (data - 8192) / 8192.0 * 50.0, FALSE);
        break;
      case RPN_CHANNEL_COARSE_TUNE: /* Coarse tune is 7 bit and in semitones (64
                                       is center) */
        fluid_synth_set_gen_LOCAL(synth, channum, GEN_COARSETUNE, value - 64,
                                  FALSE);
        break;
      case RPN_TUNING_PROGRAM_CHANGE:
        fluid_channel_set_tuning_prog(chan, value);
        fluid_synth_activate_tuning(
            synth, channum, fluid_channel_get_tuning_bank(chan), value, TRUE);
        break;
...
int fluid_synth_sysex(fluid_synth_t *synth, const char *data, int len,
  ...
  if ((data[0] == MIDI_SYSEX_UNIV_NON_REALTIME ||
    ...
    fluid_synth_api_enter(synth);
    result =
        fluid_synth_sysex_midi_tuning(synth, data, len, response, response_len,
...
static int fluid_synth_sysex_midi_tuning(fluid_synth_t *synth, const char *data,
  ...
  case MIDI_SYSEX_TUNING_BULK_DUMP_REQ_BANK:
    ...
    *resptr++ = prog;
    ...
    if (msgid == MIDI_SYSEX_TUNING_BULK_DUMP_REQ) { /* NOTE: Checksum is not as
...
int fluid_synth_pitch_wheel_sens(fluid_synth_t *synth, int chan, int val) {
  ...
  fluid_return_val_if_fail(
      val >= 0 && val <= 72,
      FLUID_FAILED); /* 6 octaves!?  Better than no limit.. */
  FLUID_API_ENTRY_CHAN(FLUID_FAILED);
 ...
 * @param synth FluidSynth instance
 * @param chan MIDI channel number (0 to MIDI channel count - 1)
 * @param preset Preset to assign to channel or NULL to clear (ownership is
 * taken over)
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 */
static int fluid_synth_set_preset(fluid_synth_t *synth, int chan,
 ...
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 */
/* FIXME - Currently not real-time safe, due to preset allocation and mutex
 * lock, and may be called from within synthesis context. */
...
int fluid_synth_program_change(fluid_synth_t *synth, int chan, int prognum) {
  ...
  fluid_channel_set_sfont_bank_prog(
      channel, preset ? fluid_sfont_get_id(preset->sfont) : 0, -1, prognum);
...
int fluid_synth_get_program(fluid_synth_t *synth, int chan,
  ...
  /* 128 indicates that the preset is unset.  Set to 0 to be backwards
   * compatible. */
...
/**
 * Select an instrument on a MIDI channel by SoundFont ID, bank and program
 * numbers.
 * @param synth FluidSynth instance
 * @param chan MIDI channel number (0 to MIDI channel count - 1)
 * @param sfont_id ID of a loaded SoundFont
...
/**
 * Select an instrument on a MIDI channel by SoundFont name, bank and program
 * numbers.
 * @param synth FluidSynth instance
 * @param chan MIDI channel number (0 to MIDI channel count - 1)
...
void fluid_synth_set_sample_rate(fluid_synth_t *synth, float sample_rate) {
  ...
  fluid_synth_update_mixer(synth, fluid_rvoice_mixer_set_samplerate, 0,
                           sample_rate);
  fluid_synth_api_exit(synth);
}
...
static int fluid_synth_update_polyphony_LOCAL(fluid_synth_t *synth,
  ...
  if (new_polyphony > synth->nvoice) {
    /* Create more voices */
    fluid_voice_t **new_voices =
        FLUID_REALLOC(synth->voice, sizeof(fluid_voice_t *) * new_polyphony);
    if (new_voices == NULL)
      return FLUID_FAILED;
...
int fluid_synth_write_s16(fluid_synth_t *synth, int len, void *lout, int loff,
  ...
  for (i = 0, j = loff, k = roff; i < len; i++, cur++, j += lincr, k += rincr) {
    ...
    left_sample =
        (fluid_real_t)roundi(left_in[0][cur] * 32766.0f + rand_table[0][di]);
    right_sample =
        (fluid_real_t)roundi(right_in[0][cur] * 32766.0f + rand_table[1][di]);
...
static void fluid_synth_check_finished_voices(fluid_synth_t *synth) {
  ...
  while (NULL != (fv = fluid_rvoice_eventhandler_get_finished_voice(
                      synth->eventhandler))) {
...
fluid_synth_free_voice_by_kill_LOCAL(fluid_synth_t *synth) {
  ...
  for (i = 0; i < synth->polyphony; i++) {
    ...
    this_voice_prio =
        fluid_voice_get_overflow_prio(voice, &synth->overflow, ticks);
 ...
 * event. The returned voice comes with default modulators and generators. A
 * single noteon event may create any number of voices, when the preset is
 * layered.
 *
...
fluid_voice_t *fluid_synth_alloc_voice(fluid_synth_t *synth,
  ...
  if (synth->verbose) {
    ...
    FLUID_LOG(FLUID_INFO, "noteon\t%d\t%d\t%d\t%05d\t%.3f\t%.3f\t%.3f\t%d",
              chan, key, vel, synth->storeid, (float)ticks / 44100.0f,
  ...
  if (fluid_voice_init(voice, sample, channel, key, vel, synth->storeid, ticks,
                       synth->gain) != FLUID_OK) {
...
fluid_synth_kill_by_exclusive_class_LOCAL(fluid_synth_t *synth,
  ...
  for (i = 0; i < synth->polyphony; i++) {
     ...
     * class and is not part of the same noteon event (voice group), then kill
     * it */

    if (_PLAYING(existing_voice) && existing_voice->chan == new_voice->chan &&
...
int fluid_synth_sfload(fluid_synth_t *synth, const char *filename,
  ...
  for (list = synth->loaders; list; list = fluid_list_next(list)) {
    ...
    if (sfont != NULL) {
      ...
      synth->sfont_info = fluid_list_prepend(synth->sfont_info,
                                             sfont_info); /* prepend to list */
      fluid_hashtable_insert(synth->sfont_hash, sfont,
                             sfont_info); /* Hash sfont->sfont_info */
...
int fluid_synth_sfunload(fluid_synth_t *synth, unsigned int id,
  ...
  /* reset the presets for all channels (SoundFont will be freed when there are
   * no more references) */
  if (reset_presets)
    fluid_synth_program_reset(synth);
  else
...
void fluid_synth_sfont_unref(fluid_synth_t *synth, fluid_sfont_t *sfont) {
  ...
  fluid_return_if_fail(sfont_info !=
                       NULL); /* Shouldn't happen, programming error if so */

  if (refcount == 0) /* No more references? - Attempt delete */
  {
    if (delete_fluid_sfont(sfont_info->sfont) ==
        0) /* SoundFont loader can block SoundFont unload */
    {
      FLUID_FREE(sfont_info);
      FLUID_LOG(FLUID_DBG, "Unloaded SoundFont");
    } /* spin off a timer thread to unload the sfont later (SoundFont loader
         blocked unload) */
    else
      new_fluid_timer(100, fluid_synth_sfunload_callback, sfont_info, TRUE,
                      TRUE, FALSE);
  }
}
...
/**
 * Reload a SoundFont.  The SoundFont retains its ID and index on the SoundFont
 * stack.
 * @param synth SoundFont instance
...
int fluid_synth_sfreload(fluid_synth_t *synth, unsigned int id) {
  ...
  for (list = synth->loaders; list; list = fluid_list_next(list)) {
    ...
    if (sfont != NULL) {
      ...
      synth->sfont_info = fluid_list_insert_at(
          synth->sfont_info, index,
          sfont_info); /* insert the sfont at the same index */
      fluid_hashtable_insert(synth->sfont_hash, sfont,
                             sfont_info); /* Hash sfont->sfont_info */
...
/**
 * Add a SoundFont.  The SoundFont will be added to the top of the SoundFont
 * stack.
 * @param synth FluidSynth instance
...
int fluid_synth_add_sfont(fluid_synth_t *synth, fluid_sfont_t *sfont) {
  ...
  sfont->id = sfont_id = ++synth->sfont_id;
  synth->sfont_info =
      fluid_list_prepend(synth->sfont_info, sfont_info); /* prepend to list */
  fluid_hashtable_insert(synth->sfont_hash, sfont,
                         sfont_info); /* Hash sfont->sfont_info */
...
void fluid_synth_remove_sfont(fluid_synth_t *synth, fluid_sfont_t *sfont) {
  ...
  for (list = synth->sfont_info; list; list = fluid_list_next(list)) {
    ...
    if (sfont_info->sfont == sfont) {
      ...
      /* Remove from SoundFont hash regardless of refcount (SoundFont delete is
       * up to caller) */
      fluid_hashtable_remove(synth->sfont_hash, sfont_info->sfont);
      break;
    }
 ...
 * @since 1.1.1
 */
int fluid_synth_get_channel_info(fluid_synth_t *synth, int chan,
  ...
  if (preset) {
    ...
    info->assigned = FALSE;
    fluid_channel_get_sfont_bank_prog(channel, &info->sfont_id, &info->bank,
                                      &info->program);
    info->name[0] = '\0';
  }
 ...
 * Get list of voices.
 * @param synth FluidSynth instance
 * @param buf Array to store voices to (NULL terminated if not filled
 * completely)
...
void fluid_synth_set_reverb(fluid_synth_t *synth, double roomsize,
  ...
  fluid_synth_set_reverb_full(synth, FLUID_REVMODEL_SET_ALL, roomsize, damping,
                              width, level);
}
 ...
 * @param synth FluidSynth instance
 * @param set Flags indicating which parameters should be set
...
int fluid_synth_set_reverb_full(fluid_synth_t *synth, int set, double roomsize,
  ...
  fluid_rvoice_eventhandler_push5(
      synth->eventhandler, fluid_rvoice_mixer_set_reverb_params,
      synth->eventhandler->mixer, set, (fluid_real_t)roomsize,
      (fluid_real_t)damping, (fluid_real_t)width, (fluid_real_t)level, 0.0f);
 ...
 * @param type Chorus waveform type (#fluid_chorus_mod)
 */
int fluid_synth_set_chorus_full(fluid_synth_t *synth, int set, int nr,
                                double level, double speed, double depth_ms,
                                int type) {
  fluid_return_val_if_fail(synth != NULL, FLUID_FAILED);
  ...
  fluid_rvoice_eventhandler_push5(
      synth->eventhandler, fluid_rvoice_mixer_set_chorus_params,
      synth->eventhandler->mixer, set, (fluid_real_t)nr, (fluid_real_t)level,
      (fluid_real_t)speed, (fluid_real_t)depth_ms, (fluid_real_t)type);
 ...
 * @param synth FluidSynth instance
 * @param chan MIDI channel to set interpolation method on or -1 for all
 * channels
...
int fluid_synth_create_octave_tuning(fluid_synth_t *synth, int bank, int prog,
                                     const char *name, const double *pitch) {
  return fluid_synth_activate_octave_tuning(synth, bank, prog, name, pitch,
                                            FALSE);
}
...
int fluid_synth_tune_notes(fluid_synth_t *synth, int bank, int prog, int len,
                           const int *key, const double *pitch, int apply) {
...
static int fluid_synth_set_tuning_LOCAL(fluid_synth_t *synth, int chan,
  ...
  fluid_channel_set_tuning(channel,
                           tuning); /* !! Takes over callers reference */
...
/**
 * Clear tuning scale on a MIDI channel (set it to the default well-tempered
 * scale).
 * @param synth FluidSynth instance
 * @param chan MIDI channel number (0 to MIDI channel count - 1)
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 ...
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 */
int fluid_synth_setstr(fluid_synth_t *synth, const char *name,
                       const char *str) {
  fluid_return_val_if_fail(synth != NULL, FLUID_FAILED);
  fluid_return_val_if_fail(name != NULL, FLUID_FAILED);
 ...
 * @param absolute 0 to assign a relative value, non-zero to assign an absolute
 * value
 * @param normalized 0 if value is specified in the native units of the
 * generator, non-zero to take the value as a 0.0-1.0 range and apply it to the
 * valid generator effect range (scaled and shifted as necessary).
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 * @since 1.1.0
 *
...
int fluid_synth_handle_midi_event(void *data, fluid_midi_event_t *event) {
  ...
  case MIDI_SYSEX:
    return fluid_synth_sysex(synth, event->paramptr, event->param1, NULL, NULL,
                             NULL, FALSE);
  }
  return FLUID_FAILED;
}
