...
static fluid_str_setting_t *new_fluid_str_setting(const char *value, char *def,
                                                  int hints,
...
static fluid_set_setting_t *new_fluid_set_setting(void) {
  ...
  setting->hashtable = new_fluid_hashtable_full(
      fluid_str_hash, fluid_str_equal, fluid_settings_key_destroy_func,
...
static void delete_fluid_set_setting(fluid_set_setting_t *setting) {
  if (setting) {
 ...
 */
static int fluid_settings_get(fluid_settings_t *settings, const char *name,
  ...
  for (n = 0; n < ntokens; n++) {
    ...
    table = (node->type == FLUID_SET_TYPE)
                ? ((fluid_set_setting_t *)node)->hashtable
                : NULL;
  }
 ...
 */
static int fluid_settings_set(fluid_settings_t *settings, const char *name,
                              void *value) {
  fluid_hashtable_t *table = settings;
  fluid_setting_node_t *node;
  char *tokens[MAX_SETTINGS_TOKENS];
...
/** returns 1 if the value has been registered correctly, 0
    otherwise */
int fluid_settings_register_str(fluid_settings_t *settings, char *name,
  ...
  if (!fluid_settings_get(settings, name, &node)) {
    ...
    if (retval != 1)
      delete_fluid_str_setting(setting);
  } else {
...
/** returns 1 if the value has been register correctly, zero
    otherwise */
int fluid_settings_register_num(fluid_settings_t *settings, char *name,
  ...
  if (!fluid_settings_get(settings, name, &node)) {
    ...
    retval = fluid_settings_set(settings, name, setting);
    if (retval != 1)
      delete_fluid_num_setting(setting);
  } else {
...
int fluid_settings_register_int(fluid_settings_t *settings, char *name, int def,
  ...
  if (!fluid_settings_get(settings, name, &node)) {
    ...
    retval = fluid_settings_set(settings, name, setting);
    if (retval != 1)
      delete_fluid_int_setting(setting);
  } else {
 ...
 * @return the type for the named setting, or #FLUID_NO_TYPE when it does not
 * exist
 */
 ...
 * @param settings a settings object
 * @param name a setting's name
 * @return the hints associated to the named setting if it exists, zero
 * otherwise
 */
...
int fluid_settings_setstr(fluid_settings_t *settings, const char *name,
  ...
  if (fluid_settings_get(settings, name, &node)) {
    if (node->type == FLUID_STR_TYPE) {
      ...
      if (setting->update)
        (*setting->update)(setting->data, name, str);
      retval = 1;
    } else if (node->type == FLUID_INT_TYPE) /* Handle yes/no for boolean values
                                                for backwards compatibility */
    {
      ...
      if (setting->hints & FLUID_HINT_TOGGLED) {
        if (FLUID_STRCMP(str, "yes") == 0) {
          setting->value = TRUE;
          if (setting->update)
            (*setting->update)(setting->data, name, TRUE);
        } else if (FLUID_STRCMP(str, "no") == 0) {
          setting->value = FALSE;
          if (setting->update)
            (*setting->update)(setting->data, name, FALSE);
        }
      }
    }
  } else {
 ...
 * @param len Size of 'str' buffer (no more than len bytes will be written,
 * which will always include a zero terminator)
...
int fluid_settings_copystr(fluid_settings_t *settings, const char *name,
  ...
  if (fluid_settings_get(settings, name, &node)) {
    if (node->type == FLUID_STR_TYPE) {
      ...
      retval = 1;
    } else if (node->type == FLUID_INT_TYPE) /* Handle boolean integers for
  ...
  if (fluid_settings_get(settings, name, &node)) {
    if (node->type == FLUID_STR_TYPE) {
      ...
      if (!setting->value || *str)
        retval = 1; /* Don't set to 1 if out of memory */
    } else if (node->type == FLUID_INT_TYPE) /* Handle boolean integers for
                                                backwards compatibility */
    {
      fluid_int_setting_t *setting = (fluid_int_setting_t *)node;
...
/**
 * Iterate the available options for a named string setting, calling the
 * provided callback function for each existing option.
 *
 * @param settings a settings object
 * @param name a setting's name
 ...
 * @since 1.1.0
 */
char *fluid_settings_option_concat(fluid_settings_t *settings, const char *name,
  ...
  if (count > 1)
    len += (int)((count - 1) * strlen(separator));
  len++; /* For terminator */
...
static int fluid_settings_foreach_iter(void *key, void *value, void *data) {
