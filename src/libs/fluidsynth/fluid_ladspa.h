...
/* How many tokens are allowed in one command line? (for example 152 => max. 50
 * port plugin allowed) */
 ...
 * Basically it's a real-valued variable (control node) or a real-valued buffer
 * (audio node).
 */
typedef struct {
  LADSPA_Data *buf; /*Either the buffer (Audio node) or a single control value
                       (Control node)*/
...
typedef struct {
   ...
   * A LADSPA plugin descriptor points to the code, which is executed, when a
   * plugin is run. The plugin instance is given as a parameter, when calling.
   */
  int NumberPlugins;
   ...
   * During the setup phase, each ladspa_add command creates one command
   * sequence. For example:
   * ./aw.so alienwah_stereo Input <- Master_L_Synth Output -> Master_R_Synth
  ...
  /* User control nodes
   * A user control node is declared at any time before the ladspa_start
   * command. It acts as a constant node, but it has a name and can be changed
   * with the ladspa_nodeset command. */
   ...
   * A possible conflict situation arises, when fluid_clear is called, and
   * starts to destroy the plugins. But the synthesis thread still processes
   * plugins at the same time. The consequences are ugly. Therefore ladspa_clear
   * waits for acknowledgement from the synthesis thread, that the Fx unit is
   * bypassed. 'cond' is used for the communication, the mutex is required for
 ...
 */
void fluid_LADSPA_run(fluid_LADSPA_FxUnit_t *Fx_unit, fluid_real_t *left_buf[],
                      fluid_real_t *right_buf[], fluid_real_t *fx_left_buf[],
                      fluid_real_t *fx_right_buf[]);

/* Purpose:
 * Returns the node belonging to Name or NULL, if not found
 */
fluid_LADSPA_Node_t *fluid_LADSPA_RetrieveNode(fluid_LADSPA_FxUnit_t *FxUnit,
                                               char *Name);

/* Purpose:
 * Creates a new node with the given characteristics.
 */
fluid_LADSPA_Node_t *fluid_LADSPA_CreateNode(fluid_LADSPA_FxUnit_t *FxUnit,
                                             char *Name, int flags);
 ...
 * Note: It would be more efficient to keep the libraries. But then the user
 * would have to restart fluidsynth each time a plugin is recompiled.
 */
 ...
 * Those functions are called from fluid_cmd, when a command is entered on the
 * command line.
 */
 ...
 * would have to restart fluidsynth each time a plugin is recompiled.
 */
int fluid_LADSPA_handle_clear(fluid_synth_t *synth, int ac, char **av,
                              fluid_ostream_t out);
 ...
 */
int fluid_LADSPA_handle_start(fluid_synth_t *synth, int ac, char **av,
                              fluid_ostream_t out);
 ...
 */
int fluid_LADSPA_handle_add(fluid_synth_t *synth, int ac, char **av,
                            fluid_ostream_t out);

/* Purpose:
 * Declares a user control node and a value; for further processing in
 * ladspa_start.
 */
int fluid_LADSPA_handle_declnode(fluid_synth_t *synth, int ac, char **av,
                                 fluid_ostream_t out);

/* Purpose:
 * Assigns a value to the a user control node
 */
int fluid_LADSPA_handle_setnode(fluid_synth_t *synth, int ac, char **av,
                                fluid_ostream_t out);
