...
static int fluid_cached_sampledata_load(char *filename, unsigned int samplepos,
  ...
  for (cached_sampledata = all_cached_sampledata; cached_sampledata;
    ...
    if (cached_sampledata->samplesize != samplesize) {
      FLUID_LOG(FLUID_ERR,
                "Cached size of soundfont doesn't match actual size of "
                "soundfont (cached: %u. actual: %u)",
...
fluid_defsfont_t *new_fluid_defsfont(fluid_settings_t *settings) {
  ...
  /* Initialise preset cache, so we don't have to call malloc on program
     changes. Usually, we have at most one preset per channel plus one
     temporarily used, so optimise for that case. */
...
int fluid_defpreset_get_num(fluid_defpreset_t *preset) { return preset->num; }
            ...
            if ((i != GEN_STARTADDROFS) && (i != GEN_ENDADDROFS) &&
              ...
              if (preset_zone->gen[i].flags) {
                ...
                fluid_voice_gen_incr(voice, i,
                                     (float)global_preset_zone->gen[i].val);
              } else {
...
int fluid_preset_zone_import_sfont(fluid_preset_zone_t *zone, SFZone *sfzone,
  ...
  for (count = 0, r = sfzone->mod; r != NULL; count++) {
    ...
    /* *** Source *** */
    mod_dest->src1 = mod_src->src & 127; /* index of source 1, seven-bit value,
                                            SF2.01 section 8.2, page 50 */
    mod_dest->flags1 = 0;
    ...
    /* *** Dest *** */
    mod_dest->dest =
        (unsigned char)mod_src->dest; /* index of controlled generator */
...
int fluid_inst_zone_import_sfont(fluid_inst_zone_t *zone, SFZone *sfzone,
  ...
  for (count = 0, r = sfzone->mod; r != NULL; count++) {
     ...
     * context: The second modulator overwrites the first one, if they only
     * differ in amount. */
    if (count == 0) {
      zone->mod = mod_dest;
    } else {
...
static int fixup_pgen(SFData *sf) {
  ...
  while (p) {
    ...
    while (p2) { /* traverse this preset's zones */
      z = (SFZone *)(p2->data);
      if ((i = (int)POINTER_TO_INT(z->instsamp))) { /* load instrument # */
...
static int fixup_igen(SFData *sf) {
  ...
  while (p) {
    ...
    while (p2) { /* traverse instrument's zones */
      z = (SFZone *)(p2->data);
      if ((i = (int)POINTER_TO_INT(z->instsamp))) { /* load sample # */
