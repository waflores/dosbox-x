...
static char *fluid_file_read_full(fluid_file fp, size_t *length) {
  ...
  if (n != buflen) {
    FLUID_LOG(FLUID_ERR, "Only read %d bytes; expected %d", n, buflen);
    FLUID_FREE(buffer);
    return NULL;
  };
  *length = n;
...
/*
 * Gets the next byte in a MIDI file, taking into account previous running
 * status.
 *
 ...
 */
int fluid_midi_file_read_track(fluid_midi_file *mf, fluid_player_t *player,
                               int num) {
  fluid_track_t *track;
...
int fluid_midi_file_read_event(fluid_midi_file *mf, fluid_track_t *track) {
  ...
  if ((status == MIDI_SYSEX)) { /* system exclusif */
    ...
    if (mf->varlen) {
      ...
      /* Add SYSEX event and indicate that its dynamically allocated and should
       * be freed with event */
 ...
 */
int fluid_midi_event_get_type(fluid_midi_event_t *evt) { return evt->type; }
 ...
 * @return Value field
 */
int fluid_midi_event_get_value(fluid_midi_event_t *evt) { return evt->param2; }
 ...
 */
int fluid_midi_event_get_pitch(fluid_midi_event_t *evt) { return evt->param1; }
 ...
 *   should be freed when the event is freed (only applies if event gets
 * destroyed with delete_fluid_midi_event())
 * @return Always returns #FLUID_OK
 *
 * NOTE: Unlike the other event assignment functions, this one sets evt->type.
 */
int fluid_midi_event_set_sysex(fluid_midi_event_t *evt, void *data, int size,
                               int dynamic) {
  evt->type = MIDI_SYSEX;
  evt->paramptr = data;
  evt->param1 = size;
 ...
 */
char *fluid_track_get_name(fluid_track_t *track) { return track->name; }
 ...
 */
int fluid_track_send_events(fluid_track_t *track, fluid_synth_t *synth,
                            fluid_player_t *player, unsigned int ticks) {
  int status = FLUID_OK;
...
fluid_player_t *new_fluid_player(fluid_synth_t *synth) {
  ...
  player->use_system_timer = fluid_settings_str_equal(
      synth->settings, "player.timing-source", "system");
 ...
 * @return #FLUID_OK or #FLUID_FAILED
 */
int fluid_player_add_mem(fluid_player_t *player, const void *buffer,
                         size_t len) {
...
int fluid_player_load(fluid_player_t *player, fluid_playlist_item *item) {
  ...
  if (item->filename != NULL) {
    ...
    FLUID_LOG(FLUID_DBG, "%s: %d: Loading midifile from memory (%p)", __FILE__,
              __LINE__, item->buffer);
...
int fluid_player_callback(void *data, unsigned int msec) {
  ...
  do {
    ...
    if (status == FLUID_PLAYER_DONE) {
      FLUID_LOG(FLUID_DBG, "%s: %d: Duration=%.3f sec", __FILE__, __LINE__,
                (msec - player->begin_msec) / 1000.0);
      loadnextfile = 1;
    }
  } while (loadnextfile);
...
int fluid_player_play(fluid_player_t *player) {
  ...
  if (player->use_system_timer) {
    player->system_timer =
        new_fluid_timer((int)player->deltatime, fluid_player_callback,
                        (void *)player, TRUE, FALSE, TRUE);
    if (player->system_timer == NULL) {
      return FLUID_FAILED;
    }
  } else {
    player->sample_timer = new_fluid_sample_timer(
        player->synth, fluid_player_callback, (void *)player);
 ...
 * @since 1.1.0
 */
int fluid_player_get_status(fluid_player_t *player) { return player->status; }
...
int fluid_player_set_midi_tempo(fluid_player_t *player, int tempo) {
  ...
  player->deltatime =
      (double)tempo / player->division / 1000.0; /* in milliseconds */
  player->start_msec = player->cur_msec;
  player->start_ticks = player->cur_ticks;

  FLUID_LOG(FLUID_DBG,
            "tempo=%d, tick time=%f msec, cur time=%d msec, cur tick=%d", tempo,
            player->deltatime, player->cur_msec, player->cur_ticks);
...
fluid_midi_parser_t *new_fluid_midi_parser() {
  ...
  parser->status = 0; /* As long as the status is 0, the parser won't do
                         anything -> no need to initialize all the fields. */
  return parser;
}
 ...
 *   not be modified or freed and is only valid until next call to this
 * function.
 */
fluid_midi_event_t *fluid_midi_parser_parse(fluid_midi_parser_t *parser,
  ...
  /* Status byte? - If previous message not yet complete, it is discarded
   * (re-sync). */
  if (c & 0x80) {
  ...
  parser->nr_bytes = 0; /* Reset data size, in case there are additional running
                           status messages */
