...
struct _fluid_mixer_fx_t {
  ...
  int mix_fx_to_out; /**< Should the effects be mixed in with the primary
                        output? */
};
...
struct _fluid_rvoice_mixer_t {
  ...
  void (*remove_voice_callback)(
      void *, fluid_rvoice_t *); /**< Used by mixer only: Receive this callback
                                    every time a voice is removed */
  void *remove_voice_callback_userdata;

  fluid_rvoice_t **rvoices; /**< Read-only: Voices array, sorted so that all
                               nulls are last */
  ...
  fluid_LADSPA_FxUnit_t
      *LADSPA_FxUnit; /**< Used by mixer only: Effects unit for LADSPA support.
                         Never created or freed */
#endif

#ifdef ENABLE_MIXER_THREADS
  //  int sleeping_threads;        /**< Atomic: number of threads currently
  //  asleep */ int active_threads;          /**< Atomic: number of threads in
  //  the thread loop */
  int threads_should_terminate; /**< Atomic: Set to TRUE when threads should
                                   terminate */
  int current_rvoice; /**< Atomic: for the threads to know next voice to  */
  fluid_cond_t
      *wakeup_threads; /**< Signalled when the threads should wake up */
  fluid_cond_mutex_t *wakeup_threads_m; /**< wakeup_threads mutex companion */
  fluid_cond_t *thread_ready; /**< Signalled from thread, when the thread has a
                                 buffer ready for mixing */
  ...
  fluid_mixer_buffers_t
      *threads; /**< Array of mixer threads (thread_count in length) */
#endif
};
...
fluid_rvoice_mixer_process_fx(fluid_rvoice_mixer_t *mixer) {
  ...
  if (mixer->fx.with_reverb) {
    if (mixer->fx.mix_fx_to_out) {
      for (i = 0; i < mixer->current_blockcount * FLUID_BUFSIZE;
           i += FLUID_BUFSIZE)
        fluid_revmodel_processmix(
            mixer->fx.reverb,
            &mixer->buffers.fx_left_buf[SYNTH_REVERB_CHANNEL][i],
            &mixer->buffers.left_buf[0][i], &mixer->buffers.right_buf[0][i]);
    } else {
      for (i = 0; i < mixer->current_blockcount * FLUID_BUFSIZE;
           i += FLUID_BUFSIZE)
  ...
  if (mixer->LADSPA_FxUnit) {
    ...
    FLUID_DECLARE_VLA(fluid_real_t *, fx_right_buf,
                      mixer->buffers.fx_buf_count);
...
void fluid_rvoice_mixer_set_finished_voices_callback(
    fluid_rvoice_mixer_t *mixer, void (*func)(void *, fluid_rvoice_t *),
    void *userdata) {
 ...
 */
static int fluid_mix_one(fluid_rvoice_t *rvoice, fluid_real_t **bufs,
                         unsigned int bufcount, int blockcount) {
  int i, result = 0;
  ...
  for (i = 0; i < blockcount; i++) {
    ...
    if (s == -1) {
      ...
      FLUID_MEMSET(&local_buf[FLUID_BUFSIZE * i], 0,
 ...
 * Note: Make sure outbufs has 2 * (buf_count + fx_buf_count) elements before
 * calling
 */
static FLUID_INLINE int
fluid_mixer_buffers_prepare(fluid_mixer_buffers_t *buffers,
                            fluid_real_t **outbufs) {
   ...
   * in that case. */
  reverb_buf = buffers->mixer->fx.with_reverb
                   ? buffers->fx_left_buf[SYNTH_REVERB_CHANNEL]
                   : NULL;
  chorus_buf = buffers->mixer->fx.with_chorus
                   ? buffers->fx_left_buf[SYNTH_CHORUS_CHANNEL]
                   : NULL;
...
static FLUID_INLINE void fluid_finish_rvoice(fluid_mixer_buffers_t *buffers,
                                             fluid_rvoice_t *rvoice) {
  if (buffers->finished_voice_count < buffers->mixer->polyphony)
    buffers->finished_voices[buffers->finished_voice_count++] = rvoice;
  else
    FLUID_LOG(FLUID_ERR,
              "Exceeded finished voices array, try increasing polyphony");
}
...
static FLUID_INLINE void
fluid_rvoice_mixer_process_finished_voices(fluid_rvoice_mixer_t *mixer) {
#ifdef ENABLE_MIXER_THREADS
  int i;
...
fluid_mixer_buffers_render_one(fluid_mixer_buffers_t *buffers,
  ...
  int s =
      fluid_mix_one(voice, bufs, bufcount, buffers->mixer->current_blockcount);
...
int fluid_rvoice_mixer_add_voice(fluid_rvoice_mixer_t *mixer,
                                 fluid_rvoice_t *voice) {
  int i;
  ...
  for (i = 0; i < mixer->active_voices; i++) {
    if (mixer->rvoices[i] == voice) {
      FLUID_LOG(FLUID_ERR, "Internal error: Trying to replace an existing "
                           "rvoice in fluid_rvoice_mixer_add_voice?!");
      return FLUID_FAILED;
    }
  ...
  FLUID_LOG(FLUID_ERR,
            "Trying to exceed polyphony in fluid_rvoice_mixer_add_voice");
  return FLUID_FAILED;
}

static int fluid_mixer_buffers_update_polyphony(fluid_mixer_buffers_t *buffers,
                                                int value) {
  void *newptr;
  ...
  newptr =
      FLUID_REALLOC(buffers->finished_voices, value * sizeof(fluid_rvoice_t *));
...
static void fluid_render_loop_singlethread(fluid_rvoice_mixer_t *mixer) {
  int i;
  FLUID_DECLARE_VLA(fluid_real_t *, bufs,
                    mixer->buffers.buf_count * 2 +
                        mixer->buffers.fx_buf_count * 2);
...
fluid_mixer_buffers_zero(fluid_mixer_buffers_t *buffers) {
  int i;
  int size =
      buffers->mixer->current_blockcount * FLUID_BUFSIZE * sizeof(fluid_real_t);
...
static int fluid_mixer_buffers_init(fluid_mixer_buffers_t *buffers,
  ...
  FLUID_MEMSET(buffers->left_buf, 0,
               buffers->buf_count * sizeof(fluid_real_t *));
  FLUID_MEMSET(buffers->right_buf, 0,
               buffers->buf_count * sizeof(fluid_real_t *));
  ...
  FLUID_MEMSET(buffers->fx_left_buf, 0,
               buffers->fx_buf_count * sizeof(fluid_real_t *));
  FLUID_MEMSET(buffers->fx_right_buf, 0,
               buffers->fx_buf_count * sizeof(fluid_real_t *));
 ...
 */
void fluid_rvoice_mixer_set_samplerate(fluid_rvoice_mixer_t *mixer,
...
static void fluid_mixer_buffers_free(fluid_mixer_buffers_t *buffers) {
  int i;
...
void fluid_rvoice_mixer_set_reverb_enabled(fluid_rvoice_mixer_t *mixer,
                                           int on) {
  mixer->fx.with_reverb = on;
}

void fluid_rvoice_mixer_set_chorus_enabled(fluid_rvoice_mixer_t *mixer,
                                           int on) {
...
void fluid_rvoice_mixer_set_chorus_params(fluid_rvoice_mixer_t *mixer, int set,
  ...
  fluid_chorus_set(mixer->fx.chorus, set, nr, (float)level, (float)speed,
                   (float)depth_ms, type);
}
void fluid_rvoice_mixer_set_reverb_params(fluid_rvoice_mixer_t *mixer, int set,
                                          double roomsize, double damping,
                                          double width, double level) {
  fluid_revmodel_set(mixer->fx.reverb, set, (float)roomsize, (float)damping,
...
int fluid_rvoice_mixer_get_fx_bufs(fluid_rvoice_mixer_t *mixer,
                                   fluid_real_t ***fx_left,
                                   fluid_real_t ***fx_right) {
 ...
 * thread) */
static void fluid_mixer_thread_func(void *data) {
  fluid_mixer_buffers_t *buffers = data;
  fluid_rvoice_mixer_t *mixer = buffers->mixer;
  int hasValidData = 0;
  FLUID_DECLARE_VLA(fluid_real_t *, bufs,
                    buffers->buf_count * 2 + buffers->fx_buf_count * 2);
  int bufcount = 0;
  ...
  while (!fluid_atomic_int_get(&mixer->threads_should_terminate)) {
    ...
    if (rvoice == NULL) {
      ...
      fluid_atomic_int_set(&buffers->ready,
                           hasValidData ? THREAD_BUF_VALID : THREAD_BUF_NODATA);
...
static void fluid_mixer_buffers_mix(fluid_mixer_buffers_t *dest,
                                    fluid_mixer_buffers_t *src) {
  int i, j;
 ...
 */
static FLUID_INLINE int fluid_mixer_mix_in(fluid_rvoice_mixer_t *mixer,
                                           int extra_threads) {
...
static void fluid_render_loop_multithread(fluid_rvoice_mixer_t *mixer) {
  int i, bufcount;
  ...
  while (fluid_mixer_mix_in(mixer, extra_threads)) {
    ...
    if (rvoice != NULL) {
      ...
      // If no voices, wait for mixes. Make sure one is still processing to
      // avoid deadlock
      int is_processing = 0;
      // waits++;
      fluid_cond_mutex_lock(mixer->thread_ready_m);
  ...
  // FLUID_LOG(FLUID_DBG, "Blockcount: %d, mixed %d of %d voices myself, waits =
  // %d", 	    mixer->current_blockcount, test, mixer->active_voices, waits);
}
 ...
 */
void fluid_rvoice_mixer_set_threads(fluid_rvoice_mixer_t *mixer,
                                    int thread_count, int prio_level) {
#ifdef ENABLE_MIXER_THREADS
  char name[16];
  int i;
  ...
  for (i = 0; i < thread_count; i++) {
    ...
    b->thread =
        new_fluid_thread(name, fluid_mixer_thread_func, b, prio_level, 0);
    if (!b->thread)
      return;
  }
 ...
 * @param blockcount number of blocks to render, each having FLUID_BUFSIZE
 * samples
