...
#define EVENTFUNC_R1(proc, type)                                               \
  if (event->method == proc) {                                                 \
    if (event->intparam != 0) {                                                \
      FLUID_LOG(FLUID_DBG, "IR-mismatch");                                     \
    }                                                                          \
...
void fluid_rvoice_event_dispatch(fluid_rvoice_event_t *event) {
  EVENTFUNC_PTR(fluid_rvoice_mixer_add_voice, fluid_rvoice_mixer_t *,
                fluid_rvoice_t *);
  ...
  FLUID_LOG(FLUID_ERR,
            "fluid_rvoice_event_dispatch: Unknown method %p to dispatch!",
            event->method);
}
...
int fluid_rvoice_eventhandler_push(fluid_rvoice_eventhandler_t *handler,
  ...
  event =
      handler->is_threadsafe
          ? fluid_ringbuffer_get_inptr(handler->queue, handler->queue_stored)
          : &local_event;
...
int fluid_rvoice_eventhandler_push5(fluid_rvoice_eventhandler_t *handler,
                                    ...
                                    fluid_real_t r3, fluid_real_t r4,
                                    fluid_real_t r5) {
  fluid_rvoice_event_t *event;
  fluid_rvoice_event_t local_event;
  event =
...
static void finished_voice_callback(void *userdata, fluid_rvoice_t *rvoice) {
  fluid_rvoice_eventhandler_t *eventhandler = userdata;
  fluid_rvoice_t **vptr =
      fluid_ringbuffer_get_inptr(eventhandler->finished_voices, 0);
  if (vptr == NULL)
    return; // Buffer full
  *vptr = rvoice;
...
new_fluid_rvoice_eventhandler(int is_threadsafe, int queuesize,
                              int finished_voices_size, int bufs, int fx_bufs,
                              fluid_real_t sample_rate) {
  fluid_rvoice_eventhandler_t *eventhandler =
      FLUID_NEW(fluid_rvoice_eventhandler_t);
  ...
  eventhandler->finished_voices =
      new_fluid_ringbuffer(finished_voices_size, sizeof(fluid_rvoice_t *));
  if (eventhandler->finished_voices == NULL)
    goto error_recovery;

  eventhandler->queue =
      new_fluid_ringbuffer(queuesize, sizeof(fluid_rvoice_event_t));
  ...
  if (eventhandler->mixer == NULL)
    goto error_recovery;
  fluid_rvoice_mixer_set_finished_voices_callback(
      eventhandler->mixer, finished_voice_callback, eventhandler);
  return eventhandler;
...
int fluid_rvoice_eventhandler_dispatch_count(
    fluid_rvoice_eventhandler_t *handler) {
 ...
 */
int fluid_rvoice_eventhandler_dispatch_all(
    fluid_rvoice_eventhandler_t *handler) {
  fluid_rvoice_event_t *event;
  int result = 0;
...
void delete_fluid_rvoice_eventhandler(fluid_rvoice_eventhandler_t *handler) {
  if (handler == NULL)
    return;
