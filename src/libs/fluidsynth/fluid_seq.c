...
static short _fluid_seq_queue_pre_insert(fluid_sequencer_t *seq,
                                         fluid_event_t *evt);
static void _fluid_seq_queue_pre_remove(fluid_sequencer_t *seq, short src,
                                        short dest, int type);
static int _fluid_seq_queue_process(void *data,
                                    unsigned int msec); // callback from timer
static void _fluid_seq_queue_insert_entry(fluid_sequencer_t *seq,
                                          fluid_evt_entry *evtentry);
static void _fluid_seq_queue_remove_entries_matching(fluid_sequencer_t *seq,
                                                     fluid_evt_entry *temp);
static void _fluid_seq_queue_send_queued_events(fluid_sequencer_t *seq);
static void _fluid_free_evt_queue(fluid_evt_entry **first,
                                  fluid_evt_entry **last);
...
void delete_fluid_sequencer(fluid_sequencer_t *seq) {
  ...
  while (seq->clients) {
    fluid_sequencer_client_t *client =
        (fluid_sequencer_client_t *)seq->clients->data;
  ...
  /*	if (seq->clients) {
                  ...
                  while (tmp != NULL) {
                          fluid_sequencer_client_t *client =
     (fluid_sequencer_client_t*)tmp->data; if (client->name)
     FLUID_FREE(client->name); tmp = tmp->next;
                  }
 ...
 * @param seq Sequencer object
 */
char *fluid_seq_gettrace(fluid_sequencer_t *seq) { return seq->tracebuf; }
#else
  ...
  while (tmp) {
    ...
    if (client->id == id) {
      ...
      if (client->callback != NULL && evt != NULL)
        (client->callback)(fluid_sequencer_get_tick(seq), evt, seq,
                           client->data);
      if (client->name)
 ...
 * @param seq Sequencer object
 * @param id Client ID
 * @return TRUE if client is a destination client, FALSE otherwise or if not
 * found
 */
 ...
 * @param time Time value in ticks (in milliseconds with the default time scale
 * of 1000).
 * @param absolute TRUE if \a time is absolute sequencer time (time since
 * sequencer creation), FALSE if relative to current time.
 ...
 */
void fluid_sequencer_remove_events(fluid_sequencer_t *seq, short source,
                                   short dest, int type) {
...
unsigned int fluid_sequencer_get_tick(fluid_sequencer_t *seq) {
  unsigned int absMs = seq->useSystemTimer
                           ? (int)fluid_curtime()
                           : fluid_atomic_int_get(&seq->currentMs);
  double nowFloat;
  unsigned int now;
 ...
 *
 * If there are already scheduled events in the sequencer and the scale is
 * changed the events are adjusted accordingly.
 */
void fluid_sequencer_set_time_scale(fluid_sequencer_t *seq, double scale) {
  ...
  if (seq->scale != scale) {
    ...
    seq->queue0StartTime = (int)((seq->queue0StartTime + seq->prevCellNb) *
                                     (seq->scale / oldScale) -
                                 seq->prevCellNb);
    ...
    if (seq->useSystemTimer) {
      seq->timer =
          new_fluid_timer((int)(1000 / seq->scale), _fluid_seq_queue_process,
...
static short _fluid_seq_queue_init(fluid_sequencer_t *seq, int maxEvents) {
...
static int _fluid_seq_queue_process(void *data, unsigned int msec) {
 ...
 * @param seq Sequencer object
 * @param msec Time to advance sequencer to (absolute time since sequencer
 * start).
 * @since 1.1.0
 */
...
static void _fluid_seq_queue_insert_queue0(fluid_sequencer_t *seq,
                                           fluid_evt_entry *tmp, int cell) {
...
static void _fluid_seq_queue_insert_queue1(fluid_sequencer_t *seq,
                                           fluid_evt_entry *tmp, int cell) {
...
static void _fluid_seq_queue_insert_queue_later(fluid_sequencer_t *seq,
                                                fluid_evt_entry *evtentry) {
  ...
  /* first? */
  if ((seq->queueLater == NULL) || (seq->queueLater->evt.time > time)) {
...
static int _fluid_seq_queue_matchevent(fluid_event_t *evt, int templType,
                                       short templSrc, short templDest) {
  int eventType;
...
static void _fluid_seq_queue_send_cell_events(fluid_sequencer_t *seq,
                                              int cellNb) {
  fluid_evt_entry *next;
  fluid_evt_entry *tmp;
...
static void _fluid_seq_queue_slide(fluid_sequencer_t *seq) {
  short i;
  fluid_evt_entry *next;
  fluid_evt_entry *tmp;
  int count = 0;
