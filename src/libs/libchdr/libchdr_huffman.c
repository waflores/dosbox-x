...
****************************************************************************
    ...
    of samples used, as follows:
        ...
        16 bits -> 2584..4180 samples   -> note that a 4k data size guarantees
codelength <= 16 bits 17 bits -> 4181..6764 samples 18 bits -> 6765..10945
samples 19 bits -> 10946..17710 samples 20 bits -> 17711..28656 samples 21 bits
-> 28657..46367 samples 22 bits -> 46368..75024 samples 23 bits -> 75025..121392
samples 24 bits -> 121393..196417 samples 25 bits -> 196418..317810 samples 26
bits -> 317811..514228 samples 27 bits -> 514229..832039 samples 28 bits ->
832040..1346268 samples 29 bits -> 1346269..2178308 samples 30 bits ->
2178309..3524577 samples 31 bits -> 3524578..5702886 samples 32 bits ->
5702887..9227464 samples

    Looking at it differently, here is where powers of 2 fall into these
buckets:
...
struct huffman_decoder *create_huffman_decoder(int numcodes, int maxbits) {
  ...
  decoder->lookup =
      (lookup_value *)malloc(sizeof(lookup_value) * (1 << maxbits));
...
enum huffman_error huffman_import_tree_rle(struct huffman_decoder *decoder,
  ...
  return bitstream_overflow(bitbuf) ? HUFFERR_INPUT_BUFFER_TOO_SMALL
                                    : HUFFERR_NONE;
}
...
enum huffman_error
huffman_compute_tree_from_histo(struct huffman_decoder *decoder) {
  uint32_t i;
...
int huffman_build_tree(struct huffman_decoder *decoder, uint32_t totaldata,
  ...
  struct node_t **list =
      (struct node_t **)malloc(sizeof(struct node_t *) * decoder->numcodes * 2);
  ...
  for (curcode = 0; curcode < decoder->numcodes; curcode++)
    if (decoder->datahisto[curcode] != 0) {
      ...
      /* scale the weight by the current effective length, ensuring we don't go
       * to 0 */
      decoder->huffnode[curcode].weight =
          ((uint64_t)decoder->datahisto[curcode]) * ((uint64_t)totalweight) /
          ((uint64_t)totaldata);
  ...
  while (listitems > 1) {
    ...
    for (curitem = 0; curitem < listitems; curitem++)
      if (newnode->weight > list[curitem]->weight) {
        memmove(&list[curitem + 1], &list[curitem],
...
enum huffman_error
huffman_assign_canonical_codes(struct huffman_decoder *decoder) {
  uint32_t curcode;
  int codelen;
  uint32_t curstart = 0;
