  ...
  for (;;) {
    ...
    if (size == 0) {
      /* we call ReadBlock() after NeedMove() and MoveBlock().
         ...
         We can go here, if we will call ReadBlock() before NeedMove(),
         MoveBlock().
      */
    ...
    */
    p->result =
        ISeqInStream_Read(p->stream, p->bufBase + (dest - p->bufBase), &size);
    if (p->result != SZ_OK)
      return;
    if (size == 0) {
      p->streamEndWasReached = 1;
    ...
    if (GET_AVAIL_BYTES(p) > p->keepSizeAfter)
      return;
    /* here and in another (p->keepSizeAfter) checks we keep on 1 byte more than
       was requested by Create() function (GET_AVAIL_BYTES(p) >=
       p->keepSizeAfter) - minimal required size */
  }

  // on exit: (p->result != SZ_OK || p->streamEndWasReached ||
  // GET_AVAIL_BYTES(p) > p->keepSizeAfter)
}

Z7_NO_INLINE
void MatchFinder_MoveBlock(CMatchFinder *p) {
  const size_t offset = (size_t)(p->buffer - p->bufBase) - p->keepSizeBefore;
  const size_t keepBefore =
      (offset & (kBlockMoveAlign - 1)) + p->keepSizeBefore;
  p->buffer = p->bufBase + keepBefore;
  memmove(p->bufBase, p->bufBase + (offset & ~((size_t)kBlockMoveAlign - 1)),
...
static UInt32 GetBlockSize(CMatchFinder *p, UInt32 historySize) {
  ...
  */
  ...
  if (p->keepSizeBefore < historySize ||
      blockSize < p->keepSizeBefore) // if 32-bit overflow
    return 0;

  {
    ...
    const UInt32 reserve =
        (blockSize >> (blockSize < ((UInt32)1 << 30) ? 1 : 2)) + (1 << 12) +
        kBlockMoveAlign + kBlockSizeAlign; // do not overflow 32-bit here
...
static UInt32 MatchFinder_GetHashMask2(CMatchFinder *p, UInt32 hs) {
  ...
  if (hs >= (1 << 24)) {
    if (p->numHashBytes == 3)
      hs = (1 << 24) - 1;
    /* if (bigHash) mode, GetHeads4b() in LzFindMt.c needs (hs >= ((1 << 24) -
     * 1))) */
  }
...
int MatchFinder_Create(CMatchFinder *p, UInt32 historySize,
  ...
  if (p->directInput)
    p->blockSize = 0;
  if (p->directInput ||
      LzInWindow_Create2(p, GetBlockSize(p, historySize), alloc)) {
    size_t hashSizeSum;
    {
      UInt32 hs;
      UInt32 hsCur;
      ...
      if (p->numHashOutBits != 0) {
        ...
        if (p->expectedDataSize < historySize) {
          const UInt32 hs2 =
              MatchFinder_GetHashMask2(p, (UInt32)p->expectedDataSize);
          if (hsCur > hs2)
            hsCur = hs2;
        }
      } else {
...

    {
      ...
      p->cyclicBufferSize =
          newCyclicBufferSize; // it must be = (historySize + 1)
...
static void MatchFinder_SetLimits(CMatchFinder *p) {
  ...
  if (n == 0)
    n = (UInt32)(Int32)-1; // we allow (pos == 0) at start even with
                           // (kMaxValForNormalize == 0)
...
void MatchFinder_Init(void *_p) {
  ...
  // p->son[0] = p->son[1] = 0; // unused: we can init skipped record for
  // speculated accesses.
...
#else
      _mm_set_epi32((Int32)subValue, (Int32)subValue, (Int32)subValue,
                    (Int32)subValue);
#endif
...
// AVX2:
#define SASUB_256(i)                                                           \
  *(__m256i *)(void *)(items + (i) * 8) = _mm256_sub_epi32(                    \
      _mm256_max_epu32(*(const __m256i *)(const void *)(items + (i) * 8),      \
                       sub2),                                                  \
      sub2);
...
typedef void(Z7_FASTCALL *LZFIND_SATUR_SUB_CODE_FUNC)(UInt32 subValue,
                                                      CLzRef *items,
                                                      const CLzRef *lim);
...
// #define SASUB_32(i)  { UInt32 v = items[i];  UInt32 m = v - subValue;  if (v
// < subValue) m = kEmptyHashValue;  items[i] = m; }
#define SASUB_32(i)                                                            \
  {                                                                            \
    UInt32 v = items[i];                                                       \
    if (v < subValue)                                                          \
...
Z7_NO_INLINE
static void Z7_FASTCALL LzFind_SaturSub_32(UInt32 subValue, CLzRef *items,
                                           const CLzRef *lim) {
  Z7_PRAGMA_OPT_DISABLE_LOOP_UNROLL_VECTORIZE
  do {
...
#define LZFIND_NORM_ALIGN_BLOCK_SIZE (1 << 7)
  Z7_PRAGMA_OPT_DISABLE_LOOP_UNROLL_VECTORIZE
  for (; numItems != 0 &&
         ((unsigned)(ptrdiff_t)items & (LZFIND_NORM_ALIGN_BLOCK_SIZE - 1)) != 0;
       numItems--) {
    SASUB_32(0)
    items++;
  }
...
static void MatchFinder_CheckLimits(CMatchFinder *p) {
  if ( // !p->streamEndWasReached && p->result == SZ_OK &&
    ...
    // we try to read only in exact state (p->keepSizeAfter ==
    // GET_AVAIL_BYTES(p))
  ...
  if (p->pos == kMaxValForNormalize)
    if (GET_AVAIL_BYTES(p) >=
        p->numHashBytes) // optional optimization for last bytes of data.
    /*
    ...
    */
    {
      ...
      const UInt32 subValue =
          (p->pos - p->historySize - 1) /* & ~(UInt32)(kNormalizeAlign - 1) */;
      // const UInt32 subValue = (1 << 15); // for debug
      // printf("\nMatchFinder_Normalize() subValue == 0x%x\n", subValue);
      MatchFinder_REDUCE_OFFSETS(p, subValue) MatchFinder_Normalize3(
          subValue, p->hash, (size_t)p->hashMask + 1 + p->fixedHashSize);
      {
...
*/
Z7_FORCE_INLINE
static UInt32 *Hc_GetMatchesSpec(size_t lenLimit, UInt32 curMatch, UInt32 pos,
...
UInt32 *GetMatchesSpec1(UInt32 lenLimit, UInt32 curMatch, UInt32 pos,
                        const Byte *cur, CLzRef *son, size_t _cyclicBufferPos,
                        UInt32 _cyclicBufferSize, UInt32 cutValue, UInt32 *d,
                        UInt32 maxLen) {
      ...
      const UInt32 delta = pos - curMatch;
      {
        ...
        if (pb[len] < cur[len]) {
          *ptr1 = curMatch;
          ...
          // if (curMatch2 >= curMatch) { *ptr0 = *ptr1 = kEmptyHashValue;
          // return NULL; } curMatch = curMatch2;
          curMatch = pair[1];
          ptr1 = pair + 1;
          len1 = len;
        } else {
...
static void MatchFinder_MovePos(CMatchFinder *p){
    /* we go here at the end of stream data, when (avail < num_hash_bytes)
       ...
       if (p->btMode)
          p->sons[(p->cyclicBufferPos << p->btMode) + 1] = 0;  //
       kEmptyHashValue
    */
    MOVE_POS}
...
#define MF_PARAMS(p)                                                           \
  lenLimit, curMatch, p->pos, p->buffer, p->son, p->cyclicBufferPos,           \
