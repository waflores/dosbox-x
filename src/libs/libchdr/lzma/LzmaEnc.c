...
/* the following LzmaEnc_* declarations is internal LZMA interface for LZMA2
 * encoder */

SRes LzmaEnc_PrepareForLzma2(CLzmaEncHandle p, ISeqInStreamPtr inStream,
                             UInt32 keepWindowSize, ISzAllocPtr alloc,
                             ISzAllocPtr allocBig);
SRes LzmaEnc_MemPrepare(CLzmaEncHandle p, const Byte *src, SizeT srcLen,
                        UInt32 keepWindowSize, ISzAllocPtr alloc,
                        ISzAllocPtr allocBig);
SRes LzmaEnc_CodeOneMemBlock(CLzmaEncHandle p, BoolInt reInit, Byte *dest,
...
// #define GET_const_CLzmaEnc_p  const CLzmaEnc *p = (const CLzmaEnc*)(const
// void *)p;
...
#define COPY_LZMA_ENC_STATE(d, s, p)                                           \
  ...
  memcpy((d)->litProbs, (s)->litProbs,                                         \
         ((size_t)0x300 * sizeof(CLzmaProb)) << (p)->lclp);
...
SRes LzmaEnc_SetProps(CLzmaEncHandle p, const CLzmaEncProps *props2) {
  ...
  */
  ...
  p->matchFinderMt.btSync.affinity = p->matchFinderMt.hashSync.affinity =
      props.affinity;
#endif
...
#define RangeEnc_GetProcessed_sizet(p)                                         \
  ((size_t)(p)->processed + (size_t)((p)->buf - (p)->bufBase) +                \
   (size_t)(p)->cacheSize)
...
static void LzmaEnc_InitPriceTables(CProbPrice *ProbPrices) {
  ...
  for (i = 0; i < (kBitModelTotal >> kNumMoveReducingBits); i++) {
    ...
    ProbPrices[i] =
        (CProbPrice)(((unsigned)kNumBitModelTotalBits << kCyclesBits) - 15 -
                     bitCount);
...
Z7_NO_INLINE static void Z7_FASTCALL
    ...
    return 0;
  {
      ...
      return len;
    {
        ...
        numAvail = LZMA_MATCH_LEN_MAX;
      {
        ...
        const ptrdiff_t dif =
            (ptrdiff_t)-1 - (ptrdiff_t)p->matches[(size_t)numPairs - 1];
...
static void LzmaEnc_Destruct(CLzmaEnc *p, ISzAllocPtr alloc,
                             ISzAllocPtr allocBig) {
#ifndef Z7_ST
        ...
        if (!p->fastMode) {
          ...
          */
          if (p->matchPriceCount >= 64) {
            ...
            FillDistancesPrices(p);
            // }}
            LenPriceEnc_UpdateTables(&p->lenEnc, (unsigned)1 << p->pb,
                                     &p->lenProbs, p->ProbPrices);
          }
          if (p->repLenEncCounter <= 0) {
            p->repLenEncCounter = REP_LEN_COUNT;
            LenPriceEnc_UpdateTables(&p->repLenEnc, (unsigned)1 << p->pb,
...
static SRes LzmaEnc_Alloc(CLzmaEnc *p, UInt32 keepWindowSize, ISzAllocPtr alloc,
...

  {
    ...
    if (!p->litProbs || !p->saveState.litProbs || p->lclp != lclp) {
      LzmaEnc_FreeLits(p, alloc);
      p->litProbs = (CLzmaProb *)ISzAlloc_Alloc(
          alloc, ((size_t)0x300 * sizeof(CLzmaProb)) << lclp);
  ...
  if (dictSize == ((UInt32)2 << 30) || dictSize == ((UInt32)3 << 30)) {
    /* 21.03 : here we reduce the dictionary for 2 reasons:
       1) we don't want 32-bit back_distance matches in decoder for 2 GB
       dictionary. 2) we want to elimate useless last MatchFinder_Normalize3()
       for corner cases, where data size is aligned for 1 GB: 5/6/8 GB. That
       reducing must be >= 1 for such corner cases. */
    dictSize -= 1;
  }
...
#endif
  {
    if (!MatchFinder_Create(&MFB, dictSize, beforeSize, p->numFastBytes,
                            LZMA_MATCH_LEN_MAX + 1 /* 21.03 */
                            ,
                            allocBig))
      return SZ_ERROR_MEM;
...
static void LzmaEnc_InitPrices(CLzmaEnc *p) {
  ...
  LenPriceEnc_UpdateTables(&p->lenEnc, (unsigned)1 << p->pb, &p->lenProbs,
                           p->ProbPrices);
  LenPriceEnc_UpdateTables(&p->repLenEnc, (unsigned)1 << p->pb, &p->repLenProbs,
                           p->ProbPrices);
}
...
static SRes LzmaEnc_Prepare(CLzmaEncHandle p, ISeqOutStreamPtr outStream,
                            ISeqInStreamPtr inStream, ISzAllocPtr alloc,
                            ISzAllocPtr allocBig) {
  // GET_CLzmaEnc_p
...
static size_t SeqOutStreamBuf_Write(ISeqOutStreamPtr pp, const void *data,
                                    size_t size) {
  Z7_CONTAINER_FROM_VTBL_TO_DECL_VAR_pp_vt_p(
      CLzmaEnc_SeqOutStreamBuf) if (p->rem < size) {
    size = p->rem;
    p->overflow = True;
  }
  if (size != 0) {
...
SRes LzmaEnc_Encode(CLzmaEncHandle p, ISeqOutStreamPtr outStream,
                    ISeqInStreamPtr inStream, ICompressProgressPtr progress,
                    ISzAllocPtr alloc, ISzAllocPtr allocBig) {
  // GET_CLzmaEnc_p
...
SRes LzmaEnc_MemEncode(CLzmaEncHandle p, Byte *dest, SizeT *destLen,
                       ...
                       ICompressProgressPtr progress, ISzAllocPtr alloc,
                       ISzAllocPtr allocBig) {
  SRes res;
  // GET_CLzmaEnc_p
