...
#include "Alloc.h"
    ...
    (!defined(Z7_WIN32_WINNT_MIN) ||                                           \
     Z7_WIN32_WINNT_MIN < 0x0502) // < Win2003 (xp-64)
#define Z7_USE_DYN_GetLargePageMinimum
#endif

// for debug:
#if 0
#if defined(__CHERI__) && defined(__SIZEOF_POINTER__) &&                       \
...
static void ConvertUInt64ToString(UInt64 val,
                                  char *s){CONVERT_INT_TO_STR(char, 24)}
...
static void PrintLn(void) { Print("\n"); }
...
/*
by specification:
  malloc(non_NULL, 0)   : returns NULL or a unique pointer value that can later
be successfully passed to free() realloc(NULL, size)   : the call is equivalent
to malloc(size) realloc(non_NULL, 0)  : the call is equivalent to free(ptr)
...
void SetLargePageSize(void) {
  ...
  const Func_GetLargePageMinimum fn =
      (Func_GetLargePageMinimum)Z7_CAST_FUNC_C GetProcAddress(
...
void *BigAlloc(size_t size) {
...
#ifdef Z7_LARGE_PAGES
  {
    ...
    if (ps != 0 && ps <= (1 << 30) && size > (ps / 2)) {
      ...
      size2 = (size + ps) & ~ps;
      if (size2 >= size) {
        void *p = VirtualAlloc(NULL, size2, MEM_COMMIT | MY_MEM_LARGE_PAGES,
                               PAGE_READWRITE);
        if (p) {
...
static void *AlignOffsetAlloc_Alloc(ISzAllocPtr pp, size_t size) {
  ...
  return z7_AlignedAlloc(size);
#else
  const CAlignOffsetAlloc *p =
      Z7_CONTAINER_FROM_VTBL_CONST(pp, CAlignOffsetAlloc, vt);
  void *adr;
  void *pAligned;
  size_t newSize;
  size_t extra;
