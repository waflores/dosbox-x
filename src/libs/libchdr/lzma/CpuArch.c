...
/* there was some CLANG/GCC compilers that have issues with
   ...
   compiler's <cpuid.h> contains the macro __cpuid() that is similar to our
   code. The history of __cpuid() changes in CLANG/GCC: GCC: 2007: it preserved
   ebx for (__PIC__ && __i386__) 2013: it preserved rbx and ebx for __PIC__
   ...
   Why CLANG cares about 64-bit mode only, and doesn't care about ebx (in
   32-bit)? Do we need __PIC__ test for CLANG or we must care about rbx even if
...
/* "=&r" selects free register. It can select even rbx, if that register is
   free.
...
#define x86_cpuid_MACRO(p, func)                                               \
  {                                                                            \
    __asm__ __volatile__(ASM_LN "cpuid"                                        \
                         : "=a"((p)[0]), "=b"((p)[1]), "=c"((p)[2]),           \
                           "=d"((p)[3])                                        \
                         : "0"(func), "2"(0));                                 \
  }
...
void Z7_FASTCALL z7_x86_cpuid(UInt32 p[4],
                              UInt32 func){x86_cpuid_MACRO(p, func)}
...
#define EFALGS_CPUID_BIT 21
  ...
  __asm__ __volatile__(ASM_LN "pushf" ASM_LN "pushf" ASM_LN "pop     %0"
                       ...
                       ASM_LN "btc     %1, %0" ASM_LN "push    %0" ASM_LN
                              "popf" ASM_LN "pushf" ASM_LN "pop     %0" ASM_LN
                              "xorl    (%%esp), %0"
...
UInt32 Z7_FASTCALL z7_x86_cpuid_GetMaxFunc(void) { return 0; }
...
/*
 __cpuid (func == (0 or 7)) requires subfunction number in ECX.
  MSDN: The __cpuid intrinsic clears the ECX register before calling the cpuid
instruction.
   __cpuid() in new MSVC clears ECX.
   __cpuid() in old MSVC (14.00) x64 doesn't clear ECX
 We still can use __cpuid for low (func) values that don't require ECX,
 but __cpuid() in old MSVC will be incorrect for some func values: (func == 7).
 So here we use the hack for old MSVC to send (subFunction) in ECX register to
cpuid instruction, where ECX value is first parameter for FASTCALL / NO_INLINE
func, So the caller of MY_cpuidex_HACK() sets ECX as subFunction, and old MSVC
for __cpuid() doesn't change ECX and cpuid instruction gets (subFunction) value.

DON'T remove Z7_NO_INLINE and Z7_FASTCALL for MY_cpuidex_HACK(): !!!
*/
static Z7_NO_INLINE void Z7_FASTCALL MY_cpuidex_HACK(Int32 subFunction,
                                                     Int32 func,
                                                     Int32 *CPUInfo) {
...
BoolInt CPU_Is_InOrder()
{
  ...
  switch (x86cpuid_GetFirm(&p))
  {
    ...
    case CPU_FIRM_AMD: return (family < 5 || (family == 5 && (model < 6 || model
== 0xA))); case CPU_FIRM_VIA: return (family < 6 || (family == 6 && model <
0xF));
  }
...
/* for legacy SSE ia32: there is no user-space cpu instruction to check
   ...
   So we need some OS-specific function to check that it's safe to use SSE
   registers.
*/
...
// we don't define ATTRIB_XGETBV here, because asm version is better for
// inlining. #define ATTRIB_XGETBV __attribute__((__target__("xsave")))
#else
#define ATTRIB_XGETBV
#endif
#endif
...
*/
  ...
  /*
    ...
    At OS statring:
      OS sets CR4.OSXSAVE flag to signal the processor that OS supports the
    XSAVE extensions. Also OS sets bitmask in XCR0 register that defines what
  ...
  */
  {
    ...
    if (0 == (1 & (c >> 28) // AVX instructions are supported by hardware
              & (c >> 27))) // OSXSAVE bit: XSAVE and related instructions are
                            // enabled by OS.
      return False;
  }
  ...
  /* also we can check
     ...
     But that check is redundant, because if OSXSAVE bit is set, then XSAVE is
     also set */
  ...
  */

  {
    ...
    // printf("\n=== XGetBV=%d\n", bm);
    return 1 & (BoolInt)(bm >> 1) // SSE state is supported (set by OS) for
                                  // storing/restoring
...
BoolInt CPU_IsSupported_CRC32(void) {
  return IsProcessorFeaturePresent(PF_ARM_V8_CRC32_INSTRUCTIONS_AVAILABLE) ? 1
                                                                           : 0;
}
BoolInt CPU_IsSupported_CRYPTO(void) {
  return IsProcessorFeaturePresent(PF_ARM_V8_CRYPTO_INSTRUCTIONS_AVAILABLE) ? 1
                                                                            : 0;
}
...
static void Print_sysctlbyname(const char *name)
{
  ...
  {
    int i;
    printf("\nres = %d : %s : '%s' : bufSize = %d, numeric", res, name, buf,
(unsigned)bufSize); for (i = 0; i < 20; i++) printf(" %2x",
(unsigned)(Byte)buf[i]);
...
#define MY_HWCAP_CHECK_FUNC_2(name1, name2)                                    \
  BoolInt CPU_IsSupported_##name1(void) {                                      \
    return (MY_getauxval(AT_HWCAP) & (HWCAP_##name2));                         \
  }
...
#define MY_HWCAP_CHECK_FUNC(name)                                              \
  BoolInt CPU_IsSupported_##name(void) {                                       \
    return (MY_getauxval(AT_HWCAP2) & (HWCAP2_##name));                        \
  }
