...
#include "RotateDefs.h"
    ...
    (MY_CPU_SIZEOF_POINTER == 4 || MY_CPU_SIZEOF_POINTER == 8)
...
#define Z7_BRANCH_FUNC_MAIN(name)                                              \
  static Z7_FORCE_INLINE Z7_ATTRIB_NO_VECTOR Byte *Z7_BRANCH_CONV(name)(       \
      Byte * p, SizeT size, UInt32 pc, int encoding)
  ...
  BR_EXTERNAL_FOR {
    ...
    for (;;) {
      ...
      if Z7_UNLIKELY (((v - 0x94000000) & 0xfc000000) == 0) {
        ...
        v &= 0x03ffffff;
        v |= 0x94000000;
        SetUi32a(p - 4, v) BR_NEXT_ITERATION
      }
      // v = rotlFixed(v, 8);  v += (flag << 8) - 0x90;  if Z7_UNLIKELY((v &
      // ((mask << 8) + 0x9f)) == 0)
      v -= 0x90000000;
...
Z7_BRANCH_FUNC_MAIN(BranchConv_ARM) {
  ...
  lim = p + size;
  BR_PC_INIT
  /* in ARM: branch offset is relative to the +2 instructions from current
     instruction. (p) will point to next instruction */
  pc += 8 - 4;
...
Z7_BRANCH_FUNC_MAIN(BranchConv_PPC) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if Z7_UNLIKELY (((v - Z7_CONV_BE_TO_NATIVE_CONST32(0x48000001)) &
...
Z7_BRANCH_FUNC_MAIN(BranchConv_SPARC) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      { const UInt32 v = GetUi16a(p) & 0xc0ff; p += 4; if (v == 0x40 || v ==
      0xc07f) break; }
      */
      v = GetBe32a(p);
      p += 4;
...
Z7_BRANCH_FUNC_MAIN(BranchConv_ARMT) {
  ...
  BR_PC_INIT
/* in ARM: branch offset is relative to the +2 instructions from current
   instruction. (p) will point to the +2 instructions from current instruction
 */
// pc += 4 - 4;
// if (encoding) pc -= 0xf800 << 1; else pc += 0xf800 << 1;
// #define ARMT_TAIL_PROC { goto armt_tail; }
#define ARMT_TAIL_PROC                                                         \
  {                                                                            \
    return p;                                                                  \
  }
  ...
  do {
    ...
    */
    ...
    // unsigned a1 = p[1];  b1 = p[3];  p += 2;  if Z7_LIKELY((b1 & (a1 ^ 8)) <
    // 0xf8)
    for (;;) {
      ...
      */
      // if (p >= lim) { ARMT_TAIL_PROC }  b3 = b1 + 8;  b1 = p[3];  p += 2;  if
      // ((b3 & b1) >= 0xf8) break;
      if Z7_UNLIKELY (p >= lim) {
        ARMT_TAIL_PROC
      }
    ...
    }
    {
      ...
      UInt32 v =
          ((UInt32)GetUi16a(p - 2) << 11) | ((UInt32)GetUi16a(p) & 0x7FF);
      /*
      UInt32 v =
            ((UInt32)p[1 - 2] << 19)
          + (((UInt32)p[1] & 0x7) << 8)
    ...
    }
    {
      ...
      do {
        ...
        if (((t >> m) & (0x70 << 1)) == 0 &&
          ...
          {
            if (encoding) {
              // pc &= ~(0xc00000 << 1); // we just need to clear at least 2
              // bits
              pc &= (0x1fffff << 1) | 1;
              v += pc;
            } else {
...
#define RISCV_GET_UI32(p)                                                      \
  ((UInt32)GetUi16a(p) + ((UInt32)GetUi16a((p) + 2) << 16))
#define RISCV_SET_UI32(p, v)                                                   \
  {SetUi16a(p, (UInt16)(v)) SetUi16a((p) + 2, (UInt16)(v >> 16))}
#endif
    ...
    if (v & 0xe80) // (not x0) and (not x2)
    {
      ...
      if (RISCV_CHECK_1(v, b)) {
        ...
        a &= 0xfffff000;
        {
          ...
          if (t != -1)
            a += (b >> 20) -
                 ((b >> 19) & 0x1000); // arithmetic right shift emulation
          else
#endif
            a += (UInt32)((Int32)b >>
                          20); // arithmetic right shift (sign-extension).
        }
...
Byte *Z7_BRANCH_CONV_DEC(RISCV)(Byte *p, SizeT size, UInt32 pc) {
  ...
  if ((v & 8) == 0) {
    ...
    }
    {
      ...
      v = Z7_BSWAP32(v) >> 15
#else
      v = (UInt32)p[3] << 1 | (UInt32)p[2] << 9
#endif
          | (UInt32)((a & 0xf000) << 5);
      BR_CONVERT_VAL_DEC(v)
      a = a_old | (v << 11 & 1u << 31) | (v << 20 & 0x3ff << 21) |
          (v << 9 & 1 << 20) | (v & 0xff << 12);
      RISCV_SET_UI32(p, a)
    }
    p += 4;
