...
enum {
  ...
  CD_TRACK_RAW_DONTCARE /* special flag for cdrom_read_data: just return me
                           whatever is there */
};
...
#define CD_FLAG_GDROM                                                          \
  0x00000001 /* disc is a GD-ROM, all tracks should be stored with GD-ROM      \
                metadata */
#define CD_FLAG_GDROMLE                                                        \
  0x00000002 /* legacy GD-ROM, with little-endian CDDA data */
...
static inline uint32_t msf_to_lba(uint32_t msf) {
  return (((msf & 0x00ff0000) >> 16) * 60 * 75) +
         (((msf & 0x0000ff00) >> 8) * 75) + ((msf & 0x000000ff) >> 0);
}
...
static inline uint32_t lba_to_msf(uint32_t lba) {
  ...
  return ((m / 10) << 20) | ((m % 10) << 16) | ((s / 10) << 12) |
         ((s % 10) << 8) | ((f / 10) << 4) | ((f % 10) << 0);
}
