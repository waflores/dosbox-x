 ...
 *      - Generate a hash for current input posistion and split it into a one
 * byte of tag and `rowHashLog` bits of index.
 *           - The hash is salted by a value that changes on every contex reset,
 * so when the same table is used we will avoid collisions that would otherwise
 * slow us down by intorducing phantom matches.
