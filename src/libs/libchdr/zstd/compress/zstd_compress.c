...
ZSTD_CCtx *ZSTD_initStaticCCtx(void *workspace, size_t workspaceSize) {
  ...
  cctx->entropyWorkspace = (U32 *)ZSTD_cwksp_reserve_object(
      &cctx->workspace, ENTROPY_WORKSPACE_SIZE);
...
static size_t ZSTD_estimateCCtxSize_usingCCtxParams_internal(
  ...
  size_t const externalSeqSpace =
      useSequenceProducer ? ZSTD_cwksp_aligned_alloc_size(maxNbExternalSeq *
...
ZSTD_estimateCStreamSize_usingCCtxParams(const ZSTD_CCtx_params *params) {
      ...
      "Estimate CCtx size is supported for single-threaded compression only.");
  {
    ...
    return ZSTD_estimateCCtxSize_usingCCtxParams_internal(
        ...
        outBuffSize, ZSTD_CONTENTSIZE_UNKNOWN, ZSTD_hasExtSeqProd(params),
        params->maxBlockSize);
  }
}
...

  {
    ...
    { /* Check if workspace is large enough, alloc a new one if needed */
      ...
      if (resizeWorkspace) {
        ...
        zc->entropyWorkspace =
            (U32 *)ZSTD_cwksp_reserve_object(ws, ENTROPY_WORKSPACE_SIZE);
    ...
    if (ZSTD_hasExtSeqProd(params)) {
      ...
      zc->extSeqBuf = (ZSTD_Sequence *)ZSTD_cwksp_reserve_aligned(
...
MEM_STATIC size_t ZSTD_entropyCompressSeqStore_internal(
  ...
  /* Compress literals */
  {
    ...
    size_t const numSequences =
        (size_t)(seqStorePtr->sequences - seqStorePtr->sequencesStart);
    size_t const numLiterals =
        (size_t)(seqStorePtr->lit - seqStorePtr->litStart);
...
ZSTD_buildEntropyStatisticsAndEstimateSubBlockSize(seqStore_t *seqStore,
  ...
  FORWARD_IF_ERROR(
      ZSTD_buildBlockEntropyStats(seqStore, &zc->blockState.prevCBlock->entropy,
                                  ...
                                  &zc->appliedParams, entropyMetadata,
...
static size_t ZSTD_loadDictionaryContent(ZSTD_matchState_t *ms, ldmState_t *ls,
  ...
  if (params->cParams.strategy < ZSTD_btultra) {
    U32 maxDictSize =
        8U << MIN(MAX(params->cParams.hashLog, params->cParams.chainLog), 28);
    if (srcSize > maxDictSize) {
      ip = iend - maxDictSize;
      src = ip;
      srcSize = maxDictSize;
...
static ZSTD_CDict *ZSTD_createCDict_advanced_internal(
    ...
    U32 enableDedicatedDictSearch, ZSTD_customMem customMem) {
  if ((!customMem.customAlloc) ^ (!customMem.customFree))
    return NULL;
  ...
  while (someMoreWork) {
    ...
    case zcss_load:
      if ((flushMode == ZSTD_e_end) &&
          ((size_t)(oend - op) >=
               ZSTD_compressBound(iend - ip) /* Enough output space */
           ||
        ...
        size_t const cSize =
            ZSTD_compressEnd_public(zcs, op, oend - op, ip, iend - ip);
...
static size_t ZSTD_CCtx_init_compressStream2(ZSTD_CCtx *cctx,
...
#endif /* ZSTD_MULTITHREAD */
  {
    ...
    if (cctx->appliedParams.inBufferMode == ZSTD_bm_buffered) {
       ...
       */
      cctx->inBuffTarget =
          cctx->blockSize + (cctx->blockSize == pledgedSrcSize);
    } else {
      cctx->inBuffTarget = 0;
    }
 ...
 */
static size_t ZSTD_validateSequence(U32 offCode, U32 matchLength, U32 minMatch,
  ...
  RETURN_ERROR_IF(offCode > OFFSET_TO_OFFBASE(offsetBound),
...
size_t ZSTD_copySequencesToSeqStoreExplicitBlockDelim(
  ...
  DEBUGLOG(5,
           "ZSTD_copySequencesToSeqStoreExplicitBlockDelim (blockSize = %zu)",
           blockSize);
...
size_t ZSTD_copySequencesToSeqStoreNoBlockDelim(
  ...
  DEBUGLOG(5,
           "ZSTD_copySequencesToSeqStoreNoBlockDelim: idx: %u PIS: %u "
           "blockSize: %zu",
...
static size_t ZSTD_compressSequences_internal(ZSTD_CCtx *cctx, void *dst,
  ...
  while (remaining) {
    ...
    size_t blockSize = determine_blockSize(cctx->appliedParams.blockDelimiters,
                                           cctx->blockSize, remaining, inSeqs,
                                           inSeqsSize, seqPos);
    U32 const lastBlock = (blockSize == remaining);
    FORWARD_IF_ERROR(blockSize, "Error while trying to determine block size");
    assert(blockSize <= remaining);
    ZSTD_resetSeqStore(&cctx->seqStore);
    DEBUGLOG(5, "Working on new block. Blocksize: %zu (total:%zu)", blockSize,
             (ip - (const BYTE *)src) + blockSize);

    additionalByteAdjustment =
        sequenceCopier(cctx, &seqPos, inSeqs, inSeqsSize, ip, blockSize,
    ...
    if (blockSize < MIN_CBLOCK_SIZE + ZSTD_blockHeaderSize + 1 + 1) {
      ...
      DEBUGLOG(5, "Block too small, writing out nocompress block: cSize: %zu",
               cBlockSize);
      cSize += cBlockSize;
      ip += blockSize;
      op += cBlockSize;
