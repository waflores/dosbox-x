                  ...
                  ""); /* The last field, Reserved, must be all-zeroes. */
  {
    ...
    /* Build DTables */
    {
      size_t const llhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, LL_base, LL_bits, LL_defaultDTable, dctx->fseEntropy,
          dctx->ddictIsCold, nbSeq, dctx->workspace, sizeof(dctx->workspace),
...

    {
      size_t const ofhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, OF_base, OF_bits, OF_defaultDTable, dctx->fseEntropy,
          dctx->ddictIsCold, nbSeq, dctx->workspace, sizeof(dctx->workspace),
          ZSTD_DCtx_get_bmi2(dctx));
...

    {
      size_t const mlhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, ML_base, ML_bits, ML_defaultDTable, dctx->fseEntropy,
          dctx->ddictIsCold, nbSeq, dctx->workspace, sizeof(dctx->workspace),
          ZSTD_DCtx_get_bmi2(dctx));
...
static void ZSTD_safecopyDstBeforeSrc(BYTE *op, const BYTE *ip,
                                      ptrdiff_t length) {
  ptrdiff_t const diff = op - ip;
  BYTE *const oend = op + length;
...
size_t ZSTD_prefetchMatch(size_t prefetchPos, seq_t const sequence,
  ...
  prefetchPos += sequence.litLength;
  {
    ...
    const BYTE *const match = ZSTD_wrappedPtrSub(
        ZSTD_wrappedPtrAdd(matchBase, prefetchPos), sequence.offset);
    PREFETCH_L1(match);
    PREFETCH_L1(match +
                CACHELINE_SIZE); /* note : it's safe to invoke PREFETCH() on any
...
size_t ZSTD_decompressBlock_internal(
  ...
  /* Build Decoding Tables */
  {
    ...
    size_t const totalHistorySize =
        ZSTD_totalHistorySize(ZSTD_maybeNullPtrAdd((BYTE *)dst, blockSizeMax),
