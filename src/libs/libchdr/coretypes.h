...
typedef struct chd_core_file {
  /*
   * arbitrary pointer to data the implementation uses to implement the below
   * functions
   */
  void *argp;
...
static inline int core_fclose(core_file *fp) { return fp->fclose(fp); }
...
static inline uint64_t core_fsize(core_file *fp) { return fp->fsize(fp); }
