...
static size_t flac_decoder_read_callback(void *userdata, void *buffer,
                                         size_t bytes);
static drflac_bool32 flac_decoder_seek_callback(void *userdata, int offset,
                                                drflac_seek_origin origin);
static void flac_decoder_metadata_callback(void *userdata,
                                           drflac_metadata *metadata);
static void flac_decoder_write_callback(void *userdata, void *buffer,
                                        size_t bytes);

/* getters (valid after reset) */
static uint32_t sample_rate(flac_decoder *decoder) {
  return decoder->sample_rate;
}
static uint8_t channels(flac_decoder *decoder) { return decoder->channels; }
static uint8_t bits_per_sample(flac_decoder *decoder) {
  return decoder->bits_per_sample;
}
...
int flac_decoder_reset(flac_decoder *decoder, uint32_t sample_rate,
                       uint8_t num_channels, uint32_t block_size,
  ...
  static const uint8_t s_header_template[0x2a] = {
      ...
      0x0A, 0xC4, 0x42, 0xF0, 0x00, 0x00, 0x00,
      0x00, /* +12: sample rate (0x0ac44 == 44100), */
            /*      numchannels (2), sample bits (16), */
            /*      samples in stream (0 == unknown) */
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, /* +1A: MD5 signature (0 == none) */
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00 /* +2A: start of stream data */
  };
  memcpy(decoder->custom_header, s_header_template, sizeof(s_header_template));
  decoder->custom_header[0x08] = decoder->custom_header[0x0a] =
      (block_size * num_channels) >> 8;
  decoder->custom_header[0x09] = decoder->custom_header[0x0b] =
      (block_size * num_channels) & 0xff;
...
int flac_decoder_decode_interleaved(flac_decoder *decoder, int16_t *samples,
                                    uint32_t num_samples, int swap_endian) {
...
#define BUFFER 2352 /* bytes per CD audio sector */
  ...
  while (decoder->uncompressed_offset < decoder->uncompressed_length) {
    ...
    flac_decoder_write_callback(decoder, buffer,
                                frames * sizeof(*buffer) * channels(decoder));
    num_samples -= frames;
  }
  return 1;
}
...
uint32_t flac_decoder_finish(flac_decoder *decoder) {
  ...
  position -=
      DRFLAC_CACHE_L2_LINES_REMAINING(&flac->bs) * sizeof(drflac_cache_t);
...
static size_t flac_decoder_read_callback(void *userdata, void *buffer,
  ...
  if (outputpos < bytes &&
      decoder->compressed_offset < decoder->compressed_length) {
    uint32_t bytes_to_copy =
        MIN(bytes - outputpos,
            decoder->compressed_length - decoder->compressed_offset);
    memcpy(&dst[outputpos],
           decoder->compressed_start + decoder->compressed_offset,
           bytes_to_copy);
  ...
  if (outputpos < bytes &&
      decoder->compressed_offset <
          decoder->compressed_length + decoder->compressed2_length) {
    uint32_t bytes_to_copy =
        MIN(bytes - outputpos,
            decoder->compressed2_length -
                (decoder->compressed_offset - decoder->compressed_length));
    memcpy(&dst[outputpos],
           decoder->compressed2_start + decoder->compressed_offset -
               decoder->compressed_length,
           bytes_to_copy);
    outputpos += bytes_to_copy;
    decoder->compressed_offset += bytes_to_copy;
  }
...
static void flac_decoder_write_callback(void *userdata, void *buffer,
  ...
  if (decoder->uncompressed_start[1] == NULL) {
    ...
    for (sampnum = 0;
         sampnum < blocksize && offset < decoder->uncompressed_length;
         sampnum++, offset++)
    ...
    for (sampnum = 0;
      ...
      for (chan = 0; chan < sampch; chan++) {
        ...
        if (decoder->uncompressed_start[chan] != NULL)
          decoder->uncompressed_start[chan][offset] =
              (int16_t)((usample << shift) | (usample >> shift));
      }
  }
