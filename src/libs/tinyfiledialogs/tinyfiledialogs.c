                                ...
                                yesnocancel */
{
  ...
  if (aTitle) {
    if (tinyfd_winUtf8 || !tfd_isDBCSCP())
                                ...
                                yesnocancel */
{
  ...
  // if (tfd_quoteDetected(aTitle)) return tinyfd_messageBox("INVALID TITLE WITH
  // QUOTES", aMessage, aDialogType, aIconType, aDefaultButton); if
  // (tfd_quoteDetected(aMessage)) return tinyfd_messageBox(aTitle, "INVALID
...
char *
tinyfd_colorChooser(char const *aTitle,         /* NULL or "" */
                                ...
                                yesnocancel */
{
  ...
  ULONG result = WinMessageBox(HWND_DESKTOP, HWND_DESKTOP, (PCSZ)aMessage,
                               (PCSZ)aTitle, 1, style);
...
MRESULT EXPENTRY InputBox_Callback(HWND hWnd, ULONG message, MPARAM wParam,
                                   MPARAM lParam) {
  int wmID;
...
/* returns NULL on cancel */
{
  ...
  ULONG result = WinDlgBox(HWND_DESKTOP, WinQueryActiveWindow(HWND_DESKTOP),
                           InputBox_Callback, NULLHANDLE, 100, &data);
  if (result != DID_ERROR) {
    return (char *)result;
  }
  return NULL;
...
inline static void setupFILEDLG(PFILEDLG filedlg, char const *aTitle,
...
/* returns NULL on cancel */
{
  ...
  setupFILEDLG(&filedlg, aTitle, aNumOfFilterPatterns, aFilterPatterns,
               aAllowMultipleSelects);
...
/* aoResultRGB also contains the result */
{
  ...
  if (aDefaultHexRGB != NULL) {
    ...
    sprintf(color, "#%02hx%02hx%02hx", aDefaultRGB[0], aDefaultRGB[1],
            aDefaultRGB[2]);
  }

  char *result = tinyfd_inputBox(
      aTitle, "Enter a color in the format '#RRGGBB'", (char *)color);
                                ...
                                yesnocancel */
{
  ...
  // if (tfd_quoteDetected(aMessage)) return tinyfd_messageBox(aTitle, "INVALID
  // MESSAGE WITH QUOTES", aDialogType, aIconType, aDefaultButton);
