...
inline uint32_t gradientRGB(
    uint32_t pixFront,
    uint32_t
        pixBack) // blend front color with opacity M / N over opaque background:
...
inline double distYCbCrBuffered(uint32_t pix1, uint32_t pix2) {
  ...
  return diffToDist[size_t(
      (((r_diff + 0xFF) / 2) << 16) | // slightly reduce precision (division by
                                      // 2) to squeeze value into single byte
  ...
  const int g_diff_tmp = ((pix1 & 0x00FF00) + 0x00FF00 - (pix2 & 0x00FF00)) /
                         2; // slightly reduce precision (division by 2) to
                            // squeeze value into single byte
  const int b_diff_tmp = ((pix1 & 0x0000FF) + 0x0000FF - (pix2 & 0x0000FF)) / 2;

  return diffToDist[(r_diff_tmp & 0xFF0000) | (g_diff_tmp & 0x00FF00) |
                    (b_diff_tmp & 0x0000FF)];
#endif
}
                          ...
                          &cfg) // result: F, G, J, K corners of "GradientType"
{
  ...
  double jg = dist(ker.i, ker.f) + dist(ker.f, ker.c) + dist(ker.n, ker.k) +
              dist(ker.k, ker.h) + weight * dist(ker.j, ker.g);
  double fk = dist(ker.e, ker.j) + dist(ker.j, ker.o) + dist(ker.b, ker.g) +
              dist(ker.g, ker.l) + weight * dist(ker.f, ker.k);
...
FORCE_INLINE // perf: quite worth it!
    ...
    blendPixel(const Kernel_3x3 &ker, uint32_t *target, int trgWidth,
               unsigned char blendInfo, // result of preprocessing all four
                                        // corners of pixel "e"
...
template <class Scaler, class ColorDistance> // scaler policy: see "Scaler2x"
                                             // reference implementation
                                             void scaleImage(
                                                 const uint32_t *src,
                                                 uint32_t *trg, int srcWidth,
                                                 int srcHeight,
                                                 const xbrz::ScalerCfg &cfg,
  ...
  //"use" space at the end of the image as temporary buffer for "on the fly
  //preprocessing": we even could use larger area of "sizeof(uint32_t) *
  //srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting
  //before accessing
  const int bufferSize = srcWidth;
  unsigned char *preProcBuffer =
      reinterpret_cast<unsigned char *>(trg + (size_t)yLast * Scaler::scale *
                                                  trgWidth) -
      bufferSize;
  ...
  if (yFirst > 0) {
    ...
    const uint32_t *s_p1 =
        src + (size_t)srcWidth * std::min(y + 1, srcHeight - 1);
    const uint32_t *s_p2 =
        src + (size_t)srcWidth * std::min(y + 2, srcHeight - 1);
  ...
  for (int y = yFirst; y < yLast; ++y) {
    uint32_t *out = trg + (size_t)Scaler::scale * y *
                              trgWidth; // consider MT "striped" access
    ...
    for (int x = 0; x < srcWidth; ++x, out += Scaler::scale) {
      ...
      const int x_m1 = std::max(
          x - 1, 0); // perf: prefer array indexing to additional pointers!
      const int x_p1 = std::min(x + 1, srcWidth - 1);
      const int x_p2 = std::min(x + 2, srcWidth - 1);
      ...
      unsigned char blend_xy = 0; // for current (x, y) position
      {
        ...
        */
        ...
        blend_xy1 = 0;
        setTopL(blend_xy1, res.blend_k); // set 1st known corner for (x + 1, y +
                                         // 1) and buffer for use on next column
      ...
      fillBlock(out, trgWidth * (int)sizeof(uint32_t), ker4.f, Scaler::scale,
                Scaler::scale);
      // place *after* preprocessing step, to not overwrite the results while
      // processing the the last pixel!
...
void xbrz::bilinearScale(const uint32_t *src, int srcWidth, int srcHeight,
  ...
  bilinearScale(src, srcWidth, srcHeight, srcWidth * (int)sizeof(uint32_t), trg,
                trgWidth, trgHeight, trgWidth * (int)sizeof(uint32_t), 0,
                trgHeight, [](uint32_t pix) { return pix; });
}

void xbrz::nearestNeighborScale(const uint32_t *src, int srcWidth,
                                int srcHeight,
                                /**/ uint32_t *trg, int trgWidth,
                                int trgHeight) {
  nearestNeighborScale(src, srcWidth, srcHeight,
                       srcWidth * (int)sizeof(uint32_t), trg, trgWidth,
                       trgHeight, trgWidth * (int)sizeof(uint32_t), 0,
                       trgHeight, [](uint32_t pix) { return pix; });
}
