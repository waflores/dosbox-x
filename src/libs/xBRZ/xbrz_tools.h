...
inline uint32_t makePixel(unsigned char a, unsigned char r, unsigned char g,
                          unsigned char b) {
  return ((uint32_t)a << (uint32_t)24) | ((uint32_t)r << (uint32_t)16) |
         ((uint32_t)g << (uint32_t)8) | (uint32_t)b;
}
inline uint32_t makePixel(unsigned char r, unsigned char g, unsigned char b) {
  return ((uint32_t)r << (uint32_t)16) | ((uint32_t)g << (uint32_t)8) |
         (uint32_t)b;
}

inline uint32_t rgb555to888(uint16_t pix) {
  return (((uint32_t)pix & (uint32_t)0x7C00) << (uint32_t)9) |
         (((uint32_t)pix & (uint32_t)0x03E0) << (uint32_t)6) |
         (((uint32_t)pix & (uint32_t)0x001F) << (uint32_t)3);
}
inline uint32_t rgb565to888(uint16_t pix) {
  return (((uint32_t)pix & (uint32_t)0xF800) << (uint32_t)8) |
         (((uint32_t)pix & (uint32_t)0x07E0) << (uint32_t)5) |
         (((uint32_t)pix & (uint32_t)0x001F) << (uint32_t)3);
}
...
template <class Pix>
inline void fillBlock(Pix *trg, int pitch, Pix col, int blockWidth,
                      int blockHeight) {
...
// pitch change (use to change image pitch without any scaling, useful for
// fitting scaled image into D3D texture)
template <class PixSrc, class PixTrg, class PixConverter>
void pitchChange(const PixSrc *src, PixTrg *trg, int width, int height,
                 int srcPitch, int trgPitch, int yFirst, int yLast,
  ...
  if (yFirst >= yLast || height <= 0 || width <= 0)
    return;
...
void nearestNeighborScale(const PixSrc *src, int srcWidth, int srcHeight,
                          ...
                          int trgPitch, int yFirst, int yLast,
                          PixConverter pixCvrt /*convert PixSrc to PixTrg*/) {
  static_assert(std::is_integral<PixSrc>::value,
                "PixSrc* is expected to be cast-able to char*");
  static_assert(std::is_integral<PixTrg>::value,
...
void nearestNeighborScaleOverSource(
    const PixSrc *src, int srcWidth, int srcHeight, int srcPitch,
    /**/ PixTrg *trg, int trgWidth, int trgHeight, int trgPitch, int yFirst,
    int yLast, PixConverter pixCvrt /*convert PixSrc to PixTrg*/) {
  static_assert(std::is_integral<PixSrc>::value,
                "PixSrc* is expected to be cast-able to char*");
  static_assert(std::is_integral<PixTrg>::value,
                "PixTrg* is expected to be cast-able to char*");
...
void bilinearScale(const uint32_t *src, int srcWidth, int srcHeight,
  ...
  //     -> precalculation gives significant boost; std::vector<> memory
