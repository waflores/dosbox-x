...
int titlebox_sysmenu_width = 20; // includes black divider line // TODO move to
                                 // some theme constants class
...
bool ToplevelWindow::mouseDoubleClicked(int x, int y, MouseButton button) {
  if (button == Left && x < (6 + titlebox_sysmenu_width) && x > 6 &&
      y >= titlebar_y_start && y < titlebar_y_stop) {
...
void Drawable::drawText(const String &text, bool interpret, Size start,
                        Size len) {
  if (interpret) {
    if (len > text.size() - start)
      len = (Size)(text.size() - start);
    len += start;
    ...
    while (start < len) {
      ...
      case Font::Tab:
        ...
        wordstart++;
        gotoXY((((int)(x / font->getWidth() / 8)) + 1) * 8 * font->getWidth(),
               y);
        break;
      case Font::Space:
        if (wordstart != start) {
          ...
          if (c == 'm' && start < len) {
            ...
            while (c != 'm') {
              ...
              case 0:
                ...
                break;
              case 1:
                setColor(color | 0x00808080u);
                break;
              case 30:
                setColor((Color::Black | bright) & intensity);
                break;
              case 31:
              ...
              case 32:
                ...
                break;
              case 33:
                setColor(Color::Yellow & intensity);
                break;
              case 34:
                setColor(Color::Blue & intensity);
                break;
              case 35:
                setColor(Color::Magenta & intensity);
                break;
              case 36:
                setColor(Color::Cyan & intensity);
                break;
              case 37:
                setColor(Color::White & intensity);
                break;
              default:
                break;
              }
...
bool ToplevelWindow::mouseDown(int x, int y, MouseButton button) {
  if (button == Left && x >= (6 + titlebox_sysmenu_width) && x < width - 6 &&
      y >= titlebar_y_start && y < titlebar_y_stop) {
    dragx = x;
    dragy = y;
    mouseChild = NULL;
    ...
    return true;
  } else if (button == Left && x < (6 + titlebox_sysmenu_width) && x >= 6 &&
             y >= titlebar_y_start && y < titlebar_y_stop) {
    mouseChild = NULL;
    raise();
...
Drawable::Drawable(int w, int h, RGB clear)
    : buffer(new RGB[w * h]), width(w), height(h), owner(true),
      color(Color::Black), font(NULL), lineWidth(1), tx(0), ty(0), cx(0), cy(0),
      cw(w), ch(h), fw(w), fh(h), x(0), y(0) {
  this->clear(clear);
}

Drawable::Drawable(Drawable &src, RGB clear)
    : buffer(new RGB[src.cw * src.ch]), width(src.cw), height(src.ch),
      owner(true), color(src.color), font(src.font), lineWidth(src.lineWidth),
      tx(0), ty(0), cx(0), cy(0), cw(src.cw), ch(src.ch), fw(src.fw),
      fh(src.fh), x(src.x), y(src.y) {
  if (clear != 0) {
    this->clear(clear);
  } else {
    for (unsigned int h = 0; (int)h < src.ch; h++) {
      memcpy(buffer + (size_t)src.cw * h,
             src.buffer + src.width * (h + (size_t)src.ty) + src.tx,
             4u * (size_t)src.cw);
    }
  }
}
...
Drawable::Drawable(Drawable &src, int x, int y, int w, int h)
    : buffer(src.buffer), width(src.width), height(src.height), owner(false),
      color(src.color), font(src.font), lineWidth(src.lineWidth),
      tx(src.tx + x), ty(src.ty + y), cx(imax(imax(-x, src.cx - x), 0)),
      cy(imax(imax(-y, src.cy - y), 0)), cw(imax(0, imin(src.cw - x, w))),
      ch(imax(0, imin(src.ch - y, h))), fw(w), fh(h),
...
void Drawable::drawCircle(int d) {
  int xo = 0, yo = d / 2, rest = (d + 1) / 2 - yo, x0 = x, y0 = y,
      rsq = d * d / 4, lwo = lineWidth / 2;

  while (xo <= yo) {
    while (xo * xo + (2 * yo - 1) * (2 * yo - 1) / 4 > rsq)
      yo--;
...
void Drawable::fill() {
  ...
  for (x--; x >= 0 && getPixel() == color; x--)
    drawPixel();
  xmin = ++x;
  for (x = x0; x < cw && getPixel() == color; x++)
    drawPixel();
  y++;
  for (x--; x >= xmin; x--) {
    if (getPixel() == color)
  ...
  for (h = imax(d.cy, -ty - y); h < sch && y + h < ch; h++) {
    ...
    for (w = imax(d.cx, -tx - x); w < scw && x + w < cw; w++) {
      ...
      if (rop == 0) {
        ...
        unsigned int magval = ((destb & Color::MagentaMask) * dop +
                               (srcb & Color::MagentaMask) * sop);
        dest[w] = (((magval & 0xffff) / rop) & Color::BlueMask) |
                  (((magval & 0xffff0000) / rop) & Color::RedMask) |
                  ((((destb & Color::GreenMask) * dop +
                     (srcb & Color::GreenMask) * sop) /
                    rop) &
                   Color::GreenMask) |
...
#define move(x)                                                                \
  (ptr +=                                                                      \
   (((x) + bit) / 8 - (((x) + bit) < 0)) * (prvc == 1 || prvc == 2 ? 2 : 1),   \
   bit = ((x) + bit + (((x) + bit) < 0 ? 8 : 0)) % 8)
  ...
  for (i = 0; i < (prvc ? 2 : 1); i++) {
    ...
    w = widths != NULL && prvc != 1 && prvc != 2 ? widths[i || !prvc ? c : prvc]
                                                 : width;
    h = ascents != NULL && prvc != 1 && prvc != 2
            ? ascents[i || !prvc ? c : prvc]
            : ht;
    if (!i) {
      if (rs == 0)
        rs = isign(col_step) * w;
...
void Timer::check(unsigned int ticks) {
  ...
  std::multimap<unsigned int, Timer_Callback *, Timer::ltuint>::iterator old,
      i = timers.lower_bound(Timer::ticks + 1);
  Timer::ticks += ticks;
...
unsigned int Timer::next() {
  ...
  std::multimap<unsigned int, Timer_Callback *, Timer::ltuint>::iterator i =
      timers.upper_bound(ticks);
...
BitmapFont::BitmapFont(const unsigned char *data, int height, int ascent,
                       bool owner, int width, bool background_set, int col_step,
                       int row_step, int character_step, Char last,
                       const int *widths, const int *ascents,
                       const unsigned char *const *char_position,
                       const Font::SpecialChar *special)
    : bitmap(data), width(width), height(height), ascent(ascent),
      widths(widths), ascents(ascents), background_set(background_set),
      col_step(col_step), row_step(row_step),
      character_step(
          character_step
              ? character_step
              : abs((row_step ? row_step : width * col_step) * height)),
...
Window::Window(Window *parent, int x, int y, int w, int h)
    : width(w), height(h), x(x), y(y), dirty(true), visible(true),
      tabbable(true), parent(parent), mouseChild(NULL), transient(false),
      toplevel(false), mouse_in_window(false), first_tabbable(false),
      last_tabbable(false) {
  parent->addChild(this);
}

Window::Window()
    : width(0), height(0), x(0), y(0), dirty(false), visible(true),
      tabbable(true), parent(NULL), mouseChild(NULL), transient(false),
      toplevel(false), mouse_in_window(false) {}

Window::~Window() {
  // WARNING: Child windows will call parent->removeChild() each child we delete
  // here.
  //          That means the list is modified per call, therefore it's not safe
  //          to blindly iterate over the children list.
  //
  //          Furthermore, modifying the code so that removeChild() does nothing
  //          during the destructor, and then iterating over the children list
  //          normally and deleting the child windows, works OK except for a
  //          segfault when the user uses the "Close" menu item from the system
  //          menu.
  //
  //          This code is a good framework for the GUI but a knotted mess when
  //          it comes to pointer ownership and when things are valid.
  while (!children.empty())
    delete children.front();
  if (parent)
    parent->removeChild(this);
  if (parent && parent->mouseChild == this)
    parent->mouseChild = NULL;
}
...
void Window::paintAll(Drawable &d) const {
  ...
  while (i != children.end()) {
    ...
    if (child->visible) {
      Drawable *cd =
          new Drawable(d, child->x, child->y, child->width, child->height);
      child->paintAll(*cd);
      delete cd;
    }
  }
...
bool Window::keyDown(const Key &key) {
  if (children.empty())
    return false;
  if ((*children.rbegin())->keyDown(key))
    return true;
  if (key.ctrl || key.alt || key.windows || key.special != Key::Tab)
    return false;
  ...
  if (key.shift) {
    ...
    while (i != e) {
      ...
      if ((*i)->tabbable) {
        // WARNING: remember raise() changes the order of children, therefore
        // using
        //          *i after raise() is invalid (stale reference)
        if ((*i) != children.back())
          children.back()->onTabbing(ONTABBING_REVTABFROMTHIS);
    ...
    while (i != e) {
      ...
      if ((*i)->tabbable) {
        ...
        //          *i after raise() is invalid (stale reference)
        if ((*i) != children.back())
          children.back()->onTabbing(ONTABBING_TABFROMTHIS);
...
void WindowInWindow::scrollToWindow(Window *child) {
  if (child->parent != this) {
    fprintf(stderr,
            "BUG: scrollToWindow given a window not a child of this parent\n");
    return;
  }
  ...
  if (scroll_pos_y < 0)
    scroll_pos_y = 0;
  if (scroll_pos_x > scroll_pos_w)
    scroll_pos_x = scroll_pos_w;
  if (scroll_pos_y > scroll_pos_h)
...
bool Window::mouseMoved(int x, int y) {
  ...
  while (!end) {
    ...
    if (w->visible && x >= w->x && x <= w->x + w->width && y >= w->y &&
        y <= w->y + w->height && w->mouseMoved(x - w->x, y - w->y)) {
...
bool Window::mouseDown(int x, int y, MouseButton button) {
  ...
  bool doraise =
      !(button == GUI::WheelUp ||
        button == GUI::WheelDown); /* do not raise if the scroll wheel */
  Window *last = NULL;
...
bool Window::mouseDoubleClicked(int x, int y, MouseButton button) {
  if (mouseChild == NULL)
    return false;
  return mouseChild->mouseDoubleClicked(x - mouseChild->x, y - mouseChild->y,
                                        button);
}
...
bool BorderedWindow::mouseDown(int x, int y, MouseButton button) {
  mouseChild = NULL;
  if (x > width - border_right || y > height - border_bottom)
    return false;
  x -= border_left;
  y -= border_top;
  if (x < 0 || y < 0)
...
bool BorderedWindow::mouseUp(int x, int y, MouseButton button) {
  if (mouseChild == NULL &&
      (x > width - border_right || y > height - border_bottom))
    return false;
  x -= border_left;
  y -= border_top;
  if (mouseChild == NULL && (x < 0 || y < 0))
    return false;
...
void ToplevelWindow::paint(Drawable &d) const {
  ...
  //      us to check if we'd be at the top anyway without the transient
  //      windows.
  if (!active) {
  ...
  // title
  d.setColor(active ? DefaultTheme.ActiveTitleBar
                    : DefaultTheme.InactiveTitleBar);
  ...
  d.setFont(titleFont);
  d.setColor(active ? DefaultTheme.ActiveTitleBarText
                    : DefaultTheme.InactiveTitleBarText);
...
void Input::paint(Drawable &d) const {
  ...
  Drawable dr(d1, (multi ? 0 : -offset), (multi ? -offset : 0),
              width - 6 + (multi ? 0 : offset),
              height - 8 + (multi ? offset : 0));
  ...
  if (sx != ex || sy != ey) {
    ...
    if (sy == ey)
      dr.fillRect(sx, sy - f->getAscent(), ex - sx, f->getHeight() + 1);
    else {
      dr.fillRect(sx, sy - f->getAscent(), width - sx + offset, f->getHeight());
      dr.fillRect(0, sy - f->getAscent() + f->getHeight(), width + offset,
                  ey - sy - f->getHeight());
  ...
  if (blink && hasFocus()) {
    if (insert)
      dr.drawLine(posx, posy, posx, posy + f->getHeight() + 1);
    else
      dr.fillRect(posx, posy, f->getWidth(text[pos]), f->getHeight() + 1);
  }
}
...
Size Input::findPos(int x, int y) {
  ...
  int line = 0;
  Size pos = 0;
  while (line < y && pos < text.size())
    if (f->toSpecial(text[pos++]) == Font::LF)
      line++;
...
bool Input::mouseDown(int x, int y, MouseButton button) {
  ...
  if (button == Middle)
    keyDown(Key(0, Key::Insert, true, false, false, false));
  return true;
}
...
bool Input::keyDown(const Key &key) {
  ...
  case Key::None:
    ...
    if (start_sel != end_sel)
      clearSelection();
    if (insert || pos >= text.size())
      text.insert(text.begin() + int(pos++), key.character);
    else
      text[pos++] = key.character;
    break;
  case Key::Left:
    if (pos > 0)
      pos--;
  ...
  case Key::Down:
    if (multi)
      pos = findPos(posx + 3, posy - offset + f->getHeight() + 4);
    else
      return false;
    break;
  case Key::Up:
  ...
  case Key::Home:
    if (multi) {
      while (pos > 0 && f->toSpecial(text[pos - 1]) != Font::LF)
        pos--;
    } else
      pos = 0;
    break;
  case Key::End:
    if (multi) {
      while (pos < text.size() && f->toSpecial(text[pos]) != Font::LF)
        pos++;
    } else
      pos = (Size)text.size();
    break;
  case Key::Backspace:
    if (!key.shift && start_sel != end_sel)
      clearSelection();
    else if (pos > 0) {
  ...
  case Key::Enter:
    if (multi) {
      if (start_sel != end_sel)
        clearSelection();
      if (insert || pos >= text.size())
        text.insert(text.begin() + int(pos++), f->fromSpecial(Font::LF));
      else
        text[pos++] = f->fromSpecial(Font::LF);
    } else
      executeAction(text);
    break;
  case Key::Tab:
    if (multi && enable_tab_input) {
      if (start_sel != end_sel)
        clearSelection();
      if (insert || pos >= text.size())
        text.insert(text.begin() + int(pos++), f->fromSpecial(Font::Tab));
      else
        text[pos++] = f->fromSpecial(Font::Tab);
    } else
      return false;
    break;
  default:
  ...
  if (!key.ctrl) {
    if (!key.shift || key.special == Key::None)
      start_sel = end_sel = pos;
    else
      end_sel = pos;
  }
...
void BorderedWindow::paintAll(Drawable &d) const {
  this->paint(d);
  Drawable dchild(d, border_left, border_top,
  ...
  for (std::list<Window *>::const_iterator i = children.begin();
    ...
    Window *child = *i;
    if (child->isVisible()) {
      Drawable cd(dchild, child->getX(), child->getY(), child->getWidth(),
                  child->getHeight());
      child->paintAll(cd);
    }
  }
...
// TODO delete old style once satisfied
  ...
  if (pressed) {
    ...
    d.drawLine(2 + offset, height - 3 - offset, width - 2 - offset,
               height - 3 - offset);
    d.drawLine(width - 3 - offset, 2 + offset, width - 3 - offset,
               height - 2 - offset);

    d.setColor(CurrentTheme.Border);

    d.drawLine(width - 2 - offset, 1 + offset, width - 2 - offset,
               height - 2 - offset);
    d.drawLine(1 + offset, height - 2 - offset, width - 2 - offset,
               height - 2 - offset);
  }
#endif
}
...
bool Checkbox::keyUp(const Key &key) {
  if (key.ctrl || key.alt || key.windows ||
      (key.character != ' ' && key.special != Key::Enter))
    return false;
  mouseUp(0, 0, Left);
...
Radiobox::Radiobox(Window *parent, int x, int y, int w, int h)
    : BorderedWindow(parent, x, y, w, h, 16, 0, 0, 0),
      ActionEventSource("GUI::Radiobox"), checked(0), pressed(0) {}
...
void Radiobox::paint(Drawable &d) const {
  ...
  d.setColor(
      pressed
          ...
          : CurrentTheme
                .EditableBackground); // do not omit this draw, doing so makes
                                      // the radio button look a little crappy
...
void Menu::paint(Drawable &d) const {
  ...
  for (std::vector<String>::const_iterator i = items.begin(); i != items.end();
       ++i) {
    if ((*i).empty()) {
...
void Menubar::paint(Drawable &d) const {
  ...
  for (std::vector<Menu *>::const_iterator i = menus.begin(); i != menus.end();
       ++i, ++index) {
...
void Frame::paint(Drawable &d) const {
  ...
  d.drawLine(8, top);
  d.drawLine((label.empty() ? 8 : f->getWidth(label) + 14), top, width - 2,
             top);
  ...
  d.drawLine(8, top + 1);
  d.drawLine((label.empty() ? 8 : f->getWidth(label) + 14), top + 1, width - 3,
             top + 1);
...
Screen *Window::getScreen() {
  return (parent == NULL ? dynamic_cast<Screen *>(this) : parent->getScreen());
}

Screen::Screen(unsigned int width, unsigned int height)
    : Window(), buffer(new Drawable((int)width, (int)height)),
      buffer_i_alloc(true) {
...
Screen::Screen(Drawable *d)
    : Window(), buffer(d),
      buffer_i_alloc(
          true) /* our primary use is from ScreenSDL that calls new */
{
...
void Screen::paint(Drawable &d) const { d.clear(Color::Transparent); }
...
unsigned int Screen::update(void *surface, unsigned int ticks) {
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++, buf++) {
      ...
      bval = ((((sval & Color::MagentaMask) * a +
                (bval & Color::MagentaMask) * (256 - a)) >>
               8) &
              Color::MagentaMask) |
             ((((sval & Color::GreenMask) * a +
                (bval & Color::GreenMask) * (256 - a)) >>
               8) &
              Color::GreenMask);
...
static GUI::Char SDLSymToChar(const SDL_Keysym &key) {
  ...
  if (key.sym < 0x40000000) { // key.sym of printable characters are <
                              // 0x40000000 except for those of Keypad
    switch (ret) {
    case 45: // '-'
    ...
    case 49: // '\\'
      ret = is_shift ? (GUI::Char)('|') : (GUI::Char)('\\');
      break;
    // case 50: /* FIX_ME: Key with this scancode is not known. Please report if
    ...
    default:
      if (ret == 50)
        LOG_MSG("gui_tk: scancode 0x50 sym=%x", key.sym);
...
#endif
{
  ...
  switch (key.sym) {
#if !defined(C_SDL2) /* hack for SDL1 that fails to send char code for         \
                        spacebar up event */
  case SDLK_SPACE:
    return Key(' ', ksym, (key.mod & KMOD_SHIFT) > 0, (key.mod & KMOD_CTRL) > 0,
               (key.mod & KMOD_ALT) > 0, (key.mod & KMOD_META) > 0);
#endif
  case SDLK_ESCAPE:
    ksym = GUI::Key::Escape;
    break;
  case SDLK_BACKSPACE:
    ksym = GUI::Key::Backspace;
    break;
  case SDLK_TAB:
    ksym = GUI::Key::Tab;
    break;
...
#endif
  case SDLK_CAPSLOCK:
    ksym = GUI::Key::CapsLock;
    break;
#if !defined(C_SDL2)
  case SDLK_NUMLOCK:
    ksym = GUI::Key::NumLock;
    break;
  case SDLK_SCROLLOCK:
    ksym = GUI::Key::ScrollLock;
    break;
#endif
  case SDLK_F1:
  case SDLK_F2:
  ...
  case SDLK_RALT:
#if defined(C_SDL2)
    return Key(0, ksym, (key.mod & KMOD_SHIFT) > 0, (key.mod & KMOD_CTRL) > 0,
               (key.mod & KMOD_ALT) > 0, false);
#else
    return Key(0, ksym, (key.mod & KMOD_SHIFT) > 0, (key.mod & KMOD_CTRL) > 0,
               (key.mod & KMOD_ALT) > 0, (key.mod & KMOD_META) > 0);
#endif
...
#if defined(C_SDL2)
  return Key(SDLSymToChar(key), ksym, (key.mod & KMOD_SHIFT) > 0,
             (key.mod & KMOD_CTRL) > 0, (key.mod & KMOD_ALT) > 0, false);
#else
  return Key(key.unicode, ksym, (key.mod & KMOD_SHIFT) > 0,
             (key.mod & KMOD_CTRL) > 0, (key.mod & KMOD_ALT) > 0,
             (key.mod & KMOD_META) > 0);
#endif
}

/** \brief Internal class that handles different screen bit depths and layouts
 * the SDL way */
...
public:
  SDL_Drawable(int width, int height, RGB clear = Color::Transparent)
      : Drawable(width, height, clear),
        surface(SDL_CreateRGBSurfaceFrom(buffer, width, height, 32, width * 4,
                                         Color::RedMask, Color::GreenMask,
                                         Color::BlueMask, Color::AlphaMask)) {
#if !defined(C_SDL2)
    surface->flags |= SDL_SRCALPHA;
#endif
  }

  ~SDL_Drawable() { SDL_FreeSurface(surface); }

  static bool get_dest_pix(char *sp, SDL_PixelFormat *sf, SDL_PixelFormat *df,
                           Uint32 *pix) {
    Uint8 r, g, b, a;
...
ScreenSDL::ScreenSDL(SDL_Surface *surface, int scale)
    : Screen(new SDL_Drawable(surface->w / scale, surface->h / scale)),
      surface(surface), downx(0), downy(0), lastclick(0), lastdown(0),
      scale(scale) {
...
bool ScreenSDL::event(SDL_Event &event) {
  ...
  if (event.type == SDL_MOUSEMOTION || event.type == SDL_MOUSEBUTTONDOWN ||
    ...
    if (event.button.which ==
        SDL_TOUCH_MOUSEID) /* don't handle mouse events faked by touchscreen */
      return true; /*eat the event or else it will just keep calling objects
                      until processed*/
  }
#endif
  ...
  case SDL_FINGERMOTION: {
    ...
    if (event.type == SDL_FINGERUP || event.type == SDL_FINGERDOWN) {
      ...
      fake.type = (event.type == SDL_FINGERUP) ? SDL_MOUSEBUTTONUP
                                               : SDL_MOUSEBUTTONDOWN;
      comb |= this->event(fake);
    }
  ...
  case SDL_KEYUP: {
    ...
    if (key.special == GUI::Key::None && key.character == 0)
      break;
    if (key.special == GUI::Key::CapsLock || key.special == GUI::Key::NumLock)
      keyDown(key);
    return keyUp(key);
  }
  case SDL_KEYDOWN: {
    const Key &key = SDL_to_GUI(event.key.keysym);
    // LOG_MSG("scancode=%x, sim=%x", event.key.keysym.scancode,
  ...
  case SDL_MOUSEBUTTONDOWN:
    ...
    rc = mouseDown(event.button.x, event.button.y,
                   SDL_to_GUI(event.button.button));
  ...
  case SDL_MOUSEBUTTONUP:
    event.button.x /= scale;
    event.button.y /= scale;
    rc = mouseUp(event.button.x, event.button.y,
                 SDL_to_GUI(event.button.button));
    if (lastdown != 0 && abs(event.button.x - downx) < 10 &&
        abs(event.button.y - downy) < 10) {
...
void WindowInWindow::paintScrollBar3DInset(Drawable &dscroll, int x, int y,
                                           int w, int h) const {
...
void WindowInWindow::paintScrollBar3DOutset(Drawable &dscroll, int x, int y,
                                            int w, int h) const {
...
void WindowInWindow::paintScrollBarThumb(Drawable &dscroll,
  ...
  // 3D outset style, 1 pixel inward each side, inside the black rectangle we
  // just drew
  paintScrollBar3DOutset(dscroll, vsl.xleft + 1, vsl.ytop + 1,
                         vsl.thumbwidth - 2, vsl.thumbheight - 2);
}

void WindowInWindow::paintScrollBarBackground(
    Drawable &dscroll, const vscrollbarlayout &vsl) const {
  /* scroll bar border, background */
  dscroll.setColor(vsl.disabled ? CurrentTheme.Shadow3D : Color::Black);
  dscroll.drawRect(vsl.scrollthumbRegion.x, vsl.scrollthumbRegion.y,
                   vsl.scrollthumbRegion.w, vsl.scrollthumbRegion.h);

  dscroll.setColor(CurrentTheme.Background);
  dscroll.fillRect(vsl.scrollthumbRegion.x + 1, vsl.scrollthumbRegion.y + 1,
                   vsl.scrollthumbRegion.w - 2, vsl.scrollthumbRegion.h - 2);
}

void WindowInWindow::paintScrollBarThumbDragOutline(
    Drawable &dscroll, const vscrollbarlayout &vsl) const {
  // Windows 3.1 also draws an inverted dotted rectangle around the thumb where
  // it WOULD be before quantization to scroll position.
  int x = 0;
  int y = (drag_y - vsl.scrollthumbRegion.y) - ((vsl.thumbheight + 2) / 2);
  if (y < 0)
    y = 0;
  if (y > vsl.thumbtravel)
    y = vsl.thumbtravel;
  dscroll.setColor(CurrentTheme.Light3D);
  dscroll.drawDotRect(x + vsl.scrollthumbRegion.x, y + vsl.scrollthumbRegion.y,
                      vsl.thumbwidth - 1, vsl.thumbheight - 1);
}
...
void WindowInWindow::getVScrollInfo(vscrollbarlayout &vsl) const {
  ...
  if (vscroll_display_width >= 4 && height >= 4) {
     ...
     * this code could adapt to the more range-aware visual style of Windows 95
     * later. */
    ...
    vsl.scrollthumbRegion.y =
        vsl.thumbheight - 1; /* relative to scrollbarRegion */
    vsl.scrollthumbRegion.w = vsl.scrollbarRegion.w;
    vsl.scrollthumbRegion.h =
        vsl.scrollbarRegion.h - ((vsl.thumbheight - 1) * 2);

    vsl.thumbtravel = vsl.scrollthumbRegion.h - vsl.thumbheight;
    if (vsl.thumbtravel < 0)
      vsl.thumbtravel = 0;
    vsl.drawthumb = (vsl.thumbheight <= vsl.scrollthumbRegion.h) &&
                    (!vsl.disabled) && (scroll_pos_h > 0);
...
void WindowInWindow::paintScrollBarArrowInBox(Drawable &dscroll, const int x,
                                              const int y, const int w,
                                              const int h, bool downArrow,
                                              bool disabled) const {
  const int aw = 7;
  const int ah = 7;
  ...
  if (downArrow) {
    ...
    for (int c = 0; c <= 3; c++)
      dscroll.drawLine(ax + 3 - (3 - c), ay + 3 + c, ax + 3 + (3 - c),
                       ay + 3 + c);
  } else {
    for (int c = 0; c <= 3; c++)
      dscroll.drawLine(ax + 3 - c, ay + c, ax + 3 + c, ay + c);
...
void WindowInWindow::paintAll(Drawable &d) const {
  ...
  for (std::list<Window *>::const_iterator i = children.begin();
    ...
    Window *child = *i;
    if (child->isVisible()) {
      Drawable cd(dchild, child->getX() + xadj, child->getY() + yadj,
  ...
  if (vscroll) {
    ...
    getVScrollInfo(vsl);
    if (vsl.draw) {
      Drawable dscroll(d, vsl.scrollbarRegion.x, vsl.scrollbarRegion.y,
                       vsl.scrollbarRegion.w, vsl.scrollbarRegion.h);
      ...
      /* up arrow */
      {
        ...
        const int h =
            vsl.scrollthumbRegion.y + 1; /* want black border of thumb to
                                            overlap our black border 1 pixel */

        // black border
        dscroll.setColor(vsl.disabled ? CurrentTheme.Shadow3D : Color::Black);
        dscroll.drawRect(x, y, w, h);
        dscroll.setColor(
            CurrentTheme.ButtonFiller); // NTS: Actually, scroll bar buttons are
                                        // always gray regardless of theme
        dscroll.fillRect(x + 1, y + 1, w - 2, h - 2);

        // 3D outset style, 1 pixel inward each side, inside the black rectangle
        // we just drew
        ...
        // up arrow
        const int nudge =
            ((vscroll_uparrowhold && vscroll_uparrowdown) ? 1 : 0);
        paintScrollBarArrowInBox(dscroll, x + nudge, y + nudge, w, h,
                                 /*downArrow*/ false, vsl.disabled);
      }

      /* down arrow */
      {
        const int x = 0;
        const int y = vsl.scrollthumbRegion.y + vsl.scrollthumbRegion.h -
                      1; /* want black border of thumb to overlap our black
                            border 1 pixel */
        const int w = vsl.scrollbarRegion.w;
        ...
        // down arrow
        const int nudge =
            ((vscroll_downarrowhold && vscroll_downarrowdown) ? 1 : 0);
        paintScrollBarArrowInBox(dscroll, x + nudge, y + nudge, w, h,
                                 /*downArrow*/ true, vsl.disabled);
      }

      if (vsl.drawthumb) {
        paintScrollBarThumb(dscroll, vsl);
        if (vscroll_dragging)
          paintScrollBarThumbDragOutline(dscroll, vsl);
      }
    }
  }
}
  ...
  if (wnd != NULL) {
    if (wnd->vscroll_downarrowhold) {
      ...
      if (tdelta >= vscroll_holdwait) {
        ...
        const int pdelta =
            (int)((tdelta * vscroll_unitpersec) / 1000) + vscroll_unitinit;
...
bool WindowInWindow::mouseDragged(int x, int y, MouseButton button) {
  if (vscroll_uparrowhold) {
    ...
    if (x >= vsl.scrollbarRegion.x &&
        x < (vsl.scrollbarRegion.x + vsl.scrollbarRegion.w) && y >= 0 &&
        y < vsl.scrollthumbRegion.y) {
  ...
  if (vscroll_downarrowhold) {
    ...
    if (x >= vsl.scrollbarRegion.x &&
        x < (vsl.scrollbarRegion.x + vsl.scrollbarRegion.w) &&
        y >= (vsl.scrollthumbRegion.y + vsl.scrollthumbRegion.h) &&
        y < height) {
  ...
  if (vscroll_dragging) {
    ...
    double npos = (double(y - vsl.scrollthumbRegion.y - (vsl.thumbheight / 2)) *
                   scroll_pos_h) /
                  vsl.thumbtravel;
    int nipos = int(floor(npos + 0.5));
    if (nipos < 0)
      nipos = 0;
    if (nipos > scroll_pos_h)
      nipos = scroll_pos_h;
    scroll_pos_y = nipos;
    return true;
  }
...
bool WindowInWindow::mouseDown(int x, int y, MouseButton button) {
  if (vscroll && x >= (width - vscroll_display_width) && button == GUI::Left) {
    ...
    if (y < vsl.scrollthumbRegion.y) {
      ...
      drag_timer.remove(&drag_timer_cb);
      drag_timer.add(&drag_timer_cb, 55);
    } else if (y >= vsl.scrollthumbRegion.y &&
               y < (vsl.scrollthumbRegion.y + vsl.scrollthumbRegion.h)) {
      ...
      double npos =
          (double(y - vsl.scrollthumbRegion.y - (vsl.thumbheight / 2)) *
           scroll_pos_h) /
          vsl.thumbtravel;
      int nipos = int(floor(npos + 0.5));
      if (nipos < 0)
