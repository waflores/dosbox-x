 ...
 * \li no dependencies apart from standards-conformant ANSI C++ (including a
 *little STL)
 ...
 *
 * It doesn't provide an own event loop. Instead, it relies on you passing
 *events and updating the screen regularly. This way, it can easily be
 *integrated with any event loop available (SDL, Qt, glib, X11, Win32, selfmade,
 *...)
 *
 * Many functions and concepts were taken from other well-known toolkits, so if
 *you know Qt, Java or wxWindows, you may encounter (intentional) similarities.
 *However, simplicity of code has been given priority over maximum optimization
 *and pixel-exact replication of their appearance.
 *
 * A note about font support and encodings: All text-related functions use
 *templates to support any encoding. Fonts define the charset.
 *
 ...
 * GUI::ToplevelWindow *frame = new GUI::ToplevelWindow(&guiscreen, 205, 100,
 *380, 250, "gui::tk example");
 *
 ...
 * \endcode
 *
 * Look at gui_tk.h for more detailed documentation and reference of all
 *classes.
 *
 * A test program that shows off the capabilities of gui::tk is available as
 *part of gui_tk.cpp.
 *
 * \section l License
 *
 ...
 *
 *  All you need to do is to include "gui_tk.h". If you want SDL support, then
 * include \c SDL.h first.
 */
...
static inline RGB rgba(unsigned int r, unsigned int g, unsigned int b,
                       unsigned int a = 0) {
  return (((r & 255) << RedShift) | ((g & 255) << GreenShift) |
          ((b & 255) << BlueShift) | ((a & 255) << AlphaShift));
}

/// Get red value (0-255) from an RGB value.
static inline unsigned int R(RGB val) {
  return ((val & Color::RedMask) >> Color::RedShift);
}
/// Get green value (0-255) from an RGB value.
static inline unsigned int G(RGB val) {
  return ((val & Color::GreenMask) >> Color::GreenShift);
}
/// Get blue value (0-255) from an RGB value.
static inline unsigned int B(RGB val) {
  return ((val & Color::BlueMask) >> Color::BlueShift);
}
/// Get alpha value (0-255) from an RGB value.
static inline unsigned int A(RGB val) {
  return ((val & Color::AlphaMask) >> Color::AlphaShift);
}
...
public:
  ...
  /** It is modeled after PC keyboards. When you feed keyboard events to a
   * GUI::Screen, try to map native keys to this set. Some special keys have a
   * character value. Set to \c None if the key has no special meaning. */
  enum Special {
    None,
    F1,
    F2,
  ...
  bool alt;
  /// Set if the "Windows"/Meta (PC) or Super (classic Unix) key is currently
  /// down.
  /** Do not depend too heavily on this, as many keyboards and systems do not
   * have such a key. */
  bool windows;

  /// Constructor.
  Key(GUI::Char character, Special special, bool shift, bool ctrl, bool alt,
      bool windows)
      : character(character), special(special), shift(shift), ctrl(ctrl),
        alt(alt), windows(windows) {}
};
 ...
 *
 *  It is used to deal with string encoding. It is intended to be used with
 * template specializations. Having such a specialization means you can feed the
 * corresponding type to all functions that expect a string -- without any
 * conversion. You can add specializations yourself in your program's code to
 * deal with unsupported types.
 *
 *  As an example, see the std::string version.
 *
 *  Encodings, as opposed to character sets, define how bytes map to character
 * values. ASCII, for example, has an \em encoding where each byte is one
 * character, while the ASCII \em character \em set says that values 0-127 are
 * valid and 65 is the upper case letter 'A'. UTF-8 has 1-6 bytes per character,
 * and the character set is unicode.
 *
 *  GUI::Font deals with the character set, and this class encapsulates
 * encodings.
 *
 */
...
protected:
  ...
  static void getString(String &dest, const STR &src) {
    (void)dest;
    (void)src;
    STR::_this_string_type_is_not_supported_();
  }

  /** \brief Converts a string object to native representation.
   *
   *  If some characters cannot be converted, they should silently be skipped.
   * Apart from that,
...
template <typename STR, Size N>
class NativeString<STR[N]> : public NativeString<STR *> {};
template <typename STR, Size N>
class NativeString<const STR[N]> : public NativeString<STR *> {};
template <typename STR>
class NativeString<const STR *> : public NativeString<STR *> {};

/// 'less-than' comparison between pointer addresses
struct ltvoid {
  bool operator()(const void *s1, const void *s2) const { return s1 < s2; }
};

class Refcount {
public:
  /* NTS: Refcount starts at zero because there's so much of this damn code that
   * expects to create UI elements with just "new Window blah blah" and for the
   * class to delete it. This makes the transition to proper refcounting less
   * painful and more gradual. */
  Refcount() : _refcount(0) {}
  virtual ~Refcount() {
    if (_refcount != 0)
      fprintf(stderr,
              "WARNING: GUI_TK::Refcount object %p refcount is nonzero (%d) on "
              "destructor\n",
              (void *)this, _refcount);
  }

public:
  int addref(void) { return ++_refcount; }
  int release(void) {
    int ref = --_refcount;

    if (ref < 0)
      fprintf(stderr,
              "WARNING: GUI_TK::Refcount object %p refcount is negative (%d) "
              "after release\n",
              (void *)this, _refcount);
    if (ref == 0)
      delete this;
...
public:
  ...
  RefcountAuto(C *const np) : _ptr(np) {
    if (_ptr != NULL)
      _ptr->addref();
  }
  virtual ~RefcountAuto() {
    if (_ptr != NULL)
      _ptr->release();
  }
...
public:
  ...
  bool operator==(const C *const np) const { return (_ptr == np); }
  bool operator!=(const C *const np) const { return !(*this == np); }
  bool operator!() const { /* ex: if (!ptr) ... equiv. if (ptr == NULL) */
    return (_ptr == NULL);
  }
  C *getptr(void) const { return _ptr; }
  C &operator*(void) const { return *_ptr; }
  C *operator->(void) const { return _ptr; }
 ...
 *
 *  This is intended as internal helper class to allow gui::tk to work with any
 * kind of string objects. While you can use this in normal application code,
 * you should better use the string class of your application framework (like
 * Qt). If you don't have any, use std::string.
 *
 *  It supports arbitrary characters, no character set is implied. Conversion
 * from/to usual string types like \c char* is automatic but not thread-safe for
 * non-class types.
 */
...
public:
  ...
  template <typename STR> String(const STR &src) {
    NativeString<STR>::getString(*this, src);
  }

  /// Taken from STL.
  template <class InputIterator>
  String(InputIterator a, InputIterator b) : std::vector<Char>(a, b) {}
  ...
  /// Deallocate a String.
  ~String() {
    for (std::map<const class std::type_info *, Native *, ltvoid>::iterator i =
             strings.begin();
         i != strings.end(); ++i)
      delete (*i).second;
  };
  ...
  /** For pointer types like \c char*, the returned pointer is usually only
   * valid as long as this object exists, or until it is modified and cast to
   * the same type again. */
  template <typename T> operator T() const {
    return NativeString<T>::getNative(*this);
  }

  /// Compare with native representation.
  template <typename T> bool operator==(const T &src) const {
    return *this == String(src);
  }
  /// Compare with other Strings.
  bool operator==(const String &src) const {
    return *(std::vector<Char> *)this == src;
  }
...
template <typename STR>
void NativeString<STR *>::getString(String &dest, const STR *src) {
  Size strlen = 0;
  while (src[strlen])
    strlen++;
  dest.resize(strlen);
  for (strlen = 0; src[(unsigned int)strlen]; strlen++)
    dest[(unsigned int)strlen] =
        (unsigned int)(sizeof(STR) == 1   ? (unsigned char)src[strlen]
                       : sizeof(STR) == 2 ? (unsigned short)src[strlen]
                                          : src[strlen]);
}
...
template <>
class NativeString<const std::string *> : public NativeString<std::string *> {};
 ...
 *  \em not a GUI window with decorations like border and title bar. See
 * ToplevelWindow for that.
 */
...
protected:
  ...
  /** It receives all drag/up/click/double-click events until an up event is
   * received */
  Window *mouseChild;
...
protected:
  ...
  virtual void focusChanged(bool gained) {
    if (children.size() > 0)
      children.back()->focusChanged(gained);
  }

public:
  /// Add an event handler.
  void addWindowHandler(Window_Callback *handler) {
    movehandlers.push_back(handler);
  }
  ...
  /// Move this window to the given position relative to the parent window's
  /// origin.
  /** If that would move part of the window outside of this window's area,
   *  the outside area will silently be clipped while drawing. */
  ...
  /// Return this window's contents' X position relative to the screen's top
  /// left corner
  virtual int getScreenX() const {
    return (parent == NULL ? 0 : parent->getScreenX() + x);
  }
  /// Return this window's contents' Y position relative to the screen's top
  /// left corner
  virtual int getScreenY() const {
    return (parent == NULL ? 0 : parent->getScreenY() + y);
  }
  ...
  /** By default, most windows are shown when created. Hidden windows do not
   * receive any events. */
  ...
  virtual bool isVisible() const {
    return (!parent || parent->isVisible()) && visible;
  }
  ...
  virtual bool hasFocus() const {
    return parent->hasFocus() && *parent->children.rbegin() == this;
  }
  ...
  /// Mouse was moved while a button was pressed. Returns true if event was
  /// handled.
  ...
  /** Clicking means pressing and releasing the mouse button while not moving
   * it. */
  ...
  /// Mouse was pressed outside the bounds of the window, if this window has
  /// focus (for transient windows). Returns true if event was handled.
  ...
  bool handleTab(const bool tab_quit, const Iterator &i,
                 const Iterator &e) const {
    return tab_quit == false &&
           (toplevel /*prevent TAB escape to another window*/ || i != e);
  }
  ...
  virtual bool raise() {
    ...
    for (Window *cur = parent->children.back(); cur != this;
         cur = parent->children.back()) {
  ...
  /// Put this window below all of it's siblings. Does not preserve relative
  /// order.
  virtual void lower() {
  ...
  unsigned int getChildCount(void) { return (unsigned int)children.size(); }
};
...
protected:
  ...
  struct ltuint {
    bool operator()(Ticks i1, Ticks i2) const { return (i1 < i2); }
  };
...
public:
  ...
  static void add(Timer_Callback *cb, const Ticks ticks) {
    timers.insert(
        std::pair<const Ticks, Timer_Callback *>(ticks + Timer::ticks, cb));
  }
...
public:
  ...
  virtual void paintScrollBarArrowInBox(Drawable &dscroll, const int x,
                                        const int y, const int w, const int h,
                                        bool downArrow, bool disabled) const;
  virtual void
  paintScrollBarThumbDragOutline(Drawable &dscroll,
                                 const vscrollbarlayout &vsl) const;
  virtual void paintScrollBarBackground(Drawable &dscroll,
                                        const vscrollbarlayout &vsl) const;
  virtual void paintScrollBarThumb(Drawable &dscroll,
                                   vscrollbarlayout &vsl) const;
  virtual void paintScrollBar3DOutset(Drawable &dscroll, int x, int y, int w,
                                      int h) const;
  virtual void paintScrollBar3DInset(Drawable &dscroll, int x, int y, int w,
                                     int h) const;
...
/** \brief A Screen represents the framebuffer that is the final destination of
 * the GUI.
 *
 *  It's main purpose is to manage the current contents of the surface and to
 * combine it with all GUI elements. You can't resize and move the screen.
 * Requests to do so will be ignored.
 *
 *  This is an abstract base class. You need to use a subclass which implements
 * rgbToSurface and surfaceToRGB.
 *
 *  To make things work, Screen needs events. Call the mouse and key event
 * functions (see Window) whenever such an event occurs. Call update(void
 * *surface, int ticks) regularly, if possible about every 40msec (25 fps). If
 * nothing has changed, screen updates are quite fast.
 */
...
protected:
  ...
  /// Store a single RGB triple (8 bit each) as a native pixel value and advance
  /// pointer.
...
public:
  ...
  template <typename STR> void setClipboard(const STR s) {
    this->setClipboard(String(s));
  }
  ...
  /// Update the given surface with this screen's content, fully honouring the
  /// alpha channel.
  /** \p ticks can be set to a different value depending on how much time has
   * passed. Timing doesn't need to be perfect, but try to call this at least
   * every 40 msec. Each tick amounts to about 10 msec. Returns the number of
   * ticks until the next event is scheduled, or 0 if none. */
...
protected:
  ...
  void rgbToSurface(RGB color, void **pixel) override {
    RGB **p = (RGB **)pixel;
    **p = color;
    (*p)++;
  };
 ...
 *  application event loop, you have to call update() and event() regularly
 * until you decide to quit the application.
 *
...
protected:
  ...
  /// not used.
  RGB surfaceToRGB(void *pixel) override {
    (void)pixel;
    return 0;
  };
...
public:
   ...
   *
   *  The dimensions of this surface will define the screen dimensions. Changing
   * the surface later on will not change the available area.
   */
 ...
 *
 *  Some drawing primitives are overloaded to take full coordinates. These
 * ignore the current position, but update it just like their regular brethren.
 */
class Drawable {
protected:
...
public:
  ...
  /** Optionally, the area is cleared with a given color (default: fully
   * transparent). */
  ...
  /// Shallow-copying copy constructor with clip & translate. See
  /// setClipTranslate(int x, int y, int w, int h).
  ...
  void drawPixel() {
    if (x >= cx && x < cw && y >= cy && y < ch)
      buffer[x + tx + (y + ty) * width] = color;
  };
  ...
  /// Draw a rectangle with top left at the current position and size \p w, \p
  /// h.
  /** The current position is not changed. */
  void drawRect(int w, int h);
  /// Draw a rectangle with top left at the given coordinates and size \p w, \p
  /// h.
  ...
  /// Draw a filled rectangle with top left at the current position and size \p
  /// w, \p h.
  /** The current position is not changed. */
  void fillRect(int w, int h);
  /// Draw a filled rectangle with top left at the given coordinates and size \p
  /// w, \p h.
  /** The current position is set to the top left corner. */
  ...
  /** Current position is the leftmost pixel on the baseline of the character.
      ...
      backspace, line-feed, tab and ANSI colors are interpreted, and text is
     word-wrapped at the window borders. You can optionally pass start and
     length of a substring to print */
  void drawText(const String &text, bool interpret = true, Size start = 0,
                Size len = (Size)-1);

  /// Draw a text string at a fixed position.
  /** see drawText(const String& text, bool interpret, Size start, Size len) */
  template <typename STR>
  void drawText(int x, int y, const STR text, bool interpret, Size start,
                Size len = (Size)-1) {
    gotoXY(x, y);
    drawText(String(text), interpret, start, len);
  }
...
protected:
  ...
  friend void Drawable::drawText(const String &s, bool interpret, Size start,
                                 Size len);
  ...
  /// Draw \p len characters to a drawable at the current position, starting at
  /// string position \p start.
...
public:
  ...
  /// Convert a character to an equivalent SpecialChar. May return values not in
  /// SpecialChar.
  virtual SpecialChar toSpecial(Char c) const {
    return (SpecialChar)(c < 255 ? c : 255);
  }
...
protected:
  ...
  /// Array of character widths. If font is fixed-width, this is NULL and \a
  /// width is used.
  const int *const widths;

  /// Array of character ascents. If this is NULL, \a ascent is used for all
  /// characters.
  /** This allows character data to be flush to the top or bottom of it's bitmap
   * area. */
  ...
  /// Array of pointers to font data. If set, neither \a bitmap nor \a
  /// character_step are used.
...
public:
  /// Constructor.
  /** The default values provide an 8 bit wide fixed-width pixel layout with
   * each byte a row, arranged top-to-bottom just like a PC's VGA font. See the
   * individual member documentation for details on the parameters. */
  BitmapFont(const unsigned char *data, int height, int ascent,
             bool owner = false, int width = 8, bool background_set = false,
             int col_step = -1, int row_step = 8, int character_step = 0,
             Char last = 256, const int *widths = NULL,
  ...
  /// Retrieve width of a character.
  int getWidth(Char c = 'M') const override {
    return (widths != NULL ? widths[c] : width);
  };

  /// Convert a character to an equivalent SpecialChar. See Font::toSpecial(Char
  /// c)
  SpecialChar toSpecial(Char c) const override {
    return (special != NULL ? special[c] : Font::toSpecial(c));
  }

  /// Convert a character to an equivalent character. See
  /// Font::fromSpecial(SpecialChar c).
  Char fromSpecial(SpecialChar c) const override {
    if (special == NULL)
      return Font::fromSpecial(c);
    Char i = 0;
    while (special[i] != c)
      i++;
    return i;
...
public:
  ...
  /** If the event source doesn't provide an additional argument, the name will
   * be used. */
...
/** Action events are events generated by GUI elements like Buttons, Menus and
 * by pressing Enter in an Input. All of these are handled similarly: The source
 * of such an event has a name, and the Event may also be connected with a
 * String describing what was executed, like the name of the Menu entry or the
...
protected:
  ...
  /** The name is primarily meant for your own purposes, for example
   * identification of the activated element. One exception are Menubars, which
   * display the name of their Menus. */
  String name;

public:
  /// Create a named event source.
  template <typename STR>
  ActionEventSource(const STR name) : name(String(name)) {}
  ...
  template <typename STR> void setName(const STR name) {
  ...
  void executeAction(const String &arg) {
    std::list<ActionEventSource_Callback *>::iterator i =
        actionHandlers.begin();
...
/// Internal class for windows whose child content should not span the entire
/// area.
...
protected:
  ...
  BorderedWindow(Window *parent, int x, int y, int w, int h, int bl, int bt,
                 int br, int bb)
      : Window(parent, x, y, w, h), border_left(bl), border_top(bt),
        border_right(br), border_bottom(bb) {}
...
/// A text label
/** Text labels are positioned relative to the top left corner of their bounding
 * box. They size themselves automatically and display their text in
...
public:
  ...
  /** If \p width is given, the resulting label is a word-wrapped multiline
   * label */
  template <typename STR>
  Label(Window *parent, int x, int y, const STR text, int width = 0,
        const Font *font = Font::getFont("default"),
        RGB color = CurrentTheme.TextColor)
      : Window(parent, x, y, (width ? width : 1), 1), font(font), color(color),
        text(text), interpret(width != 0) {
    Label::resize();
    tabbable = false;
  }
  ...
  void resize(int w = -1, int h = -1) override {
    ...
    if (interpret)
      Window::resize(w, d.getY() - font->getAscent() + font->getHeight());
    else
      Window::resize(d.getX(), font->getHeight());
  ...
  void paint(Drawable &d) const override {
    ...
    d.drawText(CurrentTheme.FocusPaddingHorizontal + (width & 1),
               font->getAscent(), text, interpret, 0);
 ...
 *  It supports selection, the clipboard and all well-known key bindings (except
 * undo).
 */
...
protected:
  ...
  void checkOffset() {
    ...
    if (multi) {
      ...
      posy = d.getY();
      posx = d.getX();
      if (posy - offset > height - 8 - f->getHeight())
        offset = posy - height + 8 + f->getHeight();
      if (posy - offset < 0)
        offset = posy;
    } else {
      posy = 0;
      posx = f->getWidth(text, 0, pos);
      if (f->getWidth(text, 0, pos + 1) - offset > width - 10)
        offset = f->getWidth(text, 0, pos + 1) - width + 10;
      if (f->getWidth(text, 0, (pos > 0 ? pos - 1 : 0)) - offset < 0)
        offset = f->getWidth(text, 0, (pos > 0 ? pos - 1 : 0));
    }
    lastpos = pos;
    setDirty();
  }
...
public:
  /// Create an input with given position and width. If not set, height is
  /// calculated from the font and input is single-line.
  Input(Window *parent, int x, int y, int w, int h = 0)
      : Window(parent, x, y, w,
               (h ? h : Font::getFont("input")->getHeight() + 10)),
        ActionEventSource("GUI::Input"), text(""), pos(0), lastpos(0), posx(0),
        posy(0), start_sel(0), end_sel(0), blink(true), insert(true),
        multi(h != 0), offset(0), enable_tab_input(false) {
    Timer::add(this, 30);
  }

  ~Input() { Timer::remove(this); }
  ...
  /// Clear selected area.
  void clearSelection() {
    text.erase(text.begin() + int(pos = imin(start_sel, end_sel)),
               text.begin() + int(imax(start_sel, end_sel)));
    start_sel = end_sel = pos;
  }

  /// Copy selection to clipboard.
  void copySelection() {
    setClipboard(String(text.begin() + int(imin(start_sel, end_sel)),
                        text.begin() + int(imax(start_sel, end_sel))));
  }
...
class ToplevelWindow : public BorderedWindow,
                       public ActionEventSource_Callback {
protected:
  /// Title text
  String title;
...
public:
  ...
  /// Put window on top of all other windows without changing their relative
  /// order
  bool raise() override {
    Window *last = parent->children.back();
    parent->children.remove(this);
  ...
  /// Set a new title.
  template <typename STR> void setTitle(const STR title) {
    this->title = title;
    setDirty();
  }
 ...
 *  but that parent is not their real parent in the window hierarchy: They are
 *always top-level elements, thus they are not clipped to their logical parent's
 *area, they can freely overlay any part of the screen. As another consequence,
 *they are not automatically deleted when their logical parent is deleted.
 *
 *  You should observe the following points:
 *    \li TransientWindows behave mostly as if their logical parent was their
 *true parent, but are not restricted to it's area
 *    \li TransientWindows are deleted automatically when the ToplevelWindow is
 *deleted in which their logical parent resides; do NOT delete them in your
 *destructor or bad things will happen
 *    \li only the logical parent object can show/hide a TransientWindow at will
 *    \li it will close automatically upon clicking other GUI elements, \em
 *except for the logical parent and it's children
 */
class TransientWindow : public Window,
                        Window_Callback,
                        ToplevelWindow_Callback {
protected:
...
public:
  ...
  /// No-op
  bool windowClosing(ToplevelWindow *win) override {
    (void)win;
    return true;
  }
  ...
  TransientWindow(Window *parent, int x, int y, int w, int h)
      : Window(parent->getScreen(), x + parent->getScreenX(),
               y + parent->getScreenY(), w, h),
  ...
  void windowMoved(Window *src, int x, int y) override {
    (void)src;
    (void)x;
    (void)y;
    move(relx, rely);
...
/// A popup menu.
/** Popup menus are used as context menus or as part of a menu bar. Menus are
 * not visible when created. Menus have a name which can be used in event
 * handlers to identify it, and it is used in Menubars as well.
 *
 *  Currently, menu entries are not interpreted in any way. This may change in
 * the future. To ensure upwards compatibility, avoid the \c '&' character in
 * menu entries, since it may have a special meaning in future versions. Menus
 * use the Font named "menu".
 *
...
protected:
  ...
  virtual void selectItem(int x, int y) {
    ...
    if (selected >= 0 && items[(unsigned int)selected].size() == 0)
      selected = -1;
  }
...
public:
  ...
  /** Size is determined dynamically. \a parent is the logical parent. The true
   * parent object is always the screen the logical parent resides on. */
  template <typename STR>
  Menu(Window *parent, int x, int y, const STR name)
      : TransientWindow(parent, x, y, 4, 4), ActionEventSource(name),
        selected(-1) {
    ...
    if (visible) {
      if (key.special == Key::Up) {
        ...
        if ((size_t)(++selected) == items.size())
          selected = 0;
      } else if (key.special == Key::Enter) {
        execute();
        return true;
      } else if (key.special == Key::Escape) {
        setVisible(false);
        selected = -1;
        return true;
      } else
        return true;
      if (items[(unsigned int)selected].size() == 0 && items.size() > 1)
        return keyDown(key);
      if (selected < 0)
        selected = (int)(items.size() - 1);
...
public:
  ...
  /** If a size is specified, text is centered. Otherwise, button size is
   * adjusted for the text. */
  template <typename T>
  Button(Window *parent, int x, int y, const T text, int w = -1,
         int h = static_cast<int>(CurrentTheme.ButtonHeight));
...
/** Menu bars aggregate several Menus. They have a simple border at the bottom.
 * If your application has a work area with 3D sunken look, place it so that it
 * overlaps the menu by one row.
 *
 *  As with Menus, avoid the character \c '&' in Menu names as it may
 *  have a special meaning in future versions. Menubars use the Font named
 * "menu".
 */
class Menubar : public Window,
                public ActionEventSource,
...
public:
  ...
  Menubar(Window *parent, int x, int y, int w)
      : Window(parent, x, y, w, Font::getFont("menu")->getHeight() + 5),
        ActionEventSource("GUI::Menubar"), selected(-1), lastx(0) {
    tabbable = false;
  }
  ...
  /// Add a Menuitem.
  template <typename STR> void addItem(int index, const STR name) {
    menus[(unsigned int)index]->addItem(name);
  }

  /// Remove a Menuitem.
  template <typename STR> void removeItem(int index, const STR name) {
    menus[(unsigned int)index]->removeItem(name);
  }
  ...
  bool mouseDown(int x, int y, MouseButton button) override {
    ...
    if (selected >= 0 && !menus[(unsigned int)selected]->isVisible())
      oldselected = -1;
    if (selected >= 0)
      menus[(unsigned int)selected]->setVisible(false);
    if (x < 0 || x >= lastx)
      return true;
    for (selected = (int)(menus.size() - 1);
         menus[(unsigned int)selected]->getX() > x; selected--) {
    };
    if (oldselected == selected)
      selected = -1;
    else
      menus[(unsigned int)selected]->setVisible(true);
    return true;
  }
...
public:
  ...
  Checkbox(Window *parent, int x, int y, int w, int h)
      : BorderedWindow(parent, x, y, w, h, 16, 0, 0, 0),
        ActionEventSource("GUI::Checkbox"), checked(0), pressed(0) {}

  /// Create a checkbox with text label.
  /** If a size is specified, text is centered. Otherwise, checkbox size is
   * adjusted for the text. */
  template <typename T>
...
public:
   ...
   * adjusted for the text. */
  template <typename T>
  Radiobox(Window *parent, int x, int y, const T text, int w = -1, int h = -1);
...
/** These can be used as generic grouping elements and also serve as aggregators
 * for RadioBoxes.
 */
class Frame : public BorderedWindow,
              public ActionEventSource,
              protected ActionEventSource_Callback {
protected:
  ...
  /// Execute handlers.
  void actionExecuted(ActionEventSource *src, const String &arg) override {
    // HACK: Attempting to cast a String to void causes "forming reference to
    // void" errors when building with GCC 4.7
    (void)arg.size(); // UNUSED
    for (std::list<Window *>::iterator i = children.begin();
         i != children.end(); ++i) {
      Radiobox *r = dynamic_cast<Radiobox *>(*i);
      if (r != NULL && src != dynamic_cast<ActionEventSource *>(r))
        r->setChecked(false);
    }
...
public:
  ...
  Frame(Window *parent, int x, int y, int w, int h)
      : BorderedWindow(parent, x, y, w, h, 5, 5, 5, 5),
        ActionEventSource("GUI::Frame"), selected(0) {}

  /// Create a frame with text label.
  template <typename T>
  Frame(Window *parent, int x, int y, int w, int h, const T text)
      : BorderedWindow(parent, x, y, w, h, 5,
                       Font::getFont("default")->getHeight() + 2, 5, 5),
...
public:
  ...
  template <typename STR>
  MessageBox2(Screen *parent, int x, int y, int width, const STR title,
              const STR text)
      : ToplevelWindow(parent, x, y, width, 1, title) {
    wiw = new WindowInWindow(this, 5, 5,
                             width - border_left - border_right - 10, 70);
    message =
        new Label(wiw, 0, 0, text, width - border_left - border_right - 10);
    close = new GUI::Button(this, (width - border_left - border_right - 70) / 2,
                            10, MSG_Get("CLOSE"), 70);
  ...
  bool keyDown(const GUI::Key &key) override {
    if (GUI::ToplevelWindow::keyDown(key))
      return true;
    return false;
  }

  bool keyUp(const GUI::Key &key) override {
    if (GUI::ToplevelWindow::keyUp(key))
      return true;
...
public:
  ...
  /// Create a new message box
  template <typename STR>
  MessageBox3(Screen *parent, int x, int y, int width, const STR title,
              const STR text)
      : ToplevelWindow(parent, x, y, width, 1, title) {
    wiw = new WindowInWindow(this, 5, 5,
                             width - border_left - border_right - 10, 70);
    message =
        new Label(wiw, 0, 0, text, width - border_left - border_right - 10);
    debugcmd = new GUI::Button(
        this, (width - border_left - border_right - 30 - 70 * 4) / 2, 10,
        MSG_Get("DEBUGCMD"), 210);
    debugcmd->addActionHandler(this);
    close =
        new GUI::Button(this, (width - border_left - border_right + 70 * 2) / 2,
                        10, MSG_Get("CLOSE"), 70);
    close->addActionHandler(this);
    setText(text);
...
template <typename STR>
ToplevelWindow::ToplevelWindow(Screen *parent, int x, int y, int w, int h,
                               const STR title)
    : BorderedWindow(parent, x, y, w, h, 6, titlebar_y_stop, 6, 3),
      title(title), dragx(-1), dragy(-1), closehandlers(),
      systemMenu(new Menu(this, -1, -2, "System Menu")) {
...
template <typename STR>
Button::Button(Window *parent, int x, int y, const STR text, int w, int h)
    : BorderedWindow(parent, x, y, w, h, 6, 4, 6, 4), ActionEventSource(text),
      pressed(0) {

  Label *l = new Label(this, 0, 0, text);
  l->allow_focus = true;
  if (width < 0)
    resize(l->getWidth() + border_left + border_right + 10, height);
  if (height < 0)
    resize(width, l->getHeight() + border_top + border_bottom + 6);
  l->move((width - border_left - border_right - l->getWidth()) / 2,
          (height - border_top - border_bottom - l->getHeight()) / 2);
}

template <typename STR>
Checkbox::Checkbox(Window *parent, int x, int y, const STR text, int w, int h)
    : BorderedWindow(parent, x, y, w, h, 16, 0, 0, 0), ActionEventSource(text),
      checked(0), pressed(0) {
  Label *l = new Label(this, 0, 0, text);
  if (width < 0)
    resize(l->getWidth() + border_left + border_right + 4, height);
  if (height < 0)
    resize(width, l->getHeight() + border_top + border_bottom + 4);
  l->move((width - border_left - border_right - l->getWidth()) / 2,
          (height - border_top - border_bottom - l->getHeight()) / 2);
}

template <typename STR>
Radiobox::Radiobox(Window *parent, int x, int y, const STR text, int w, int h)
    : BorderedWindow(parent, x, y, w, h, 16, 0, 0, 0), ActionEventSource(text),
      checked(0), pressed(0) {
  Label *l = new Label(this, 0, 0, text);
  if (width < 0)
