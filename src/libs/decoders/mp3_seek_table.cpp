 ...
 *
 * Problem:
 *          Seeking within an MP3 file to an exact time-offset, such as is
 expected
 *          within DOS games, is extremely difficult because the MP3 format
 doesn't
 *          provide a defined relationship between the compressed data stream
 positions
 *          versus decompressed PCM times.
 *
 * Solution:
 *          To solve this, we step through each compressed MP3 frames in
 *          the MP3 file (without decoding the actual audio) and keep a record
 of the
 *          decompressed "PCM" times for each frame.  We save this relationship
 to
 *          to a local fie, called a fast-seek look-up table, which we can
 quickly
 *          reuse every subsequent time we need to seek within the MP3 file.
 This allows
 *          seeks to be performed extremely fast while being PCM-exact.
 *
 *          This "fast-seek" file can hold data for multiple MP3s to avoid
 *          creating an excessive number of files in the local working
 directory.
 *
 * Challenges:
 *       1. What happens if an MP3 file is changed but the MP3's filename
 remains the same?
 *
 ...
 *          cross-compatible regardless of where they were written to or read
 from.
 *
 ...
 * The seek table handler makes use of the following single-header public
 libraries:
...
//
// This function tries to files as-close to the middle of the MP3 file as
// possible, and use that feed the hash function in hopes of the most
// uniqueness. We're trying to avoid content that might be duplicated across
// MP3s, like:
...
Uint64 calculate_stream_hash(struct SDL_RWops *const context) {
  // Save the current stream position, so we can restore it at the end of the
  // function.
  ...
  if (stream_size <= 0) {
    // LOG_MSG("MP3: get_stream_size returned %d, but should be positive",
    // stream_size);
    return 0;
  }

  // Seek to the middle of the file while taking into account version small
  // files.
  ...
  while (total_bytes_read < static_cast<size_t>(tail_size)) {
    ...
    const size_t bytes_read =
        SDL_RWread(context, buffer.data(), 1, (int)buffer.size());
...
//
Uint64 generate_new_seek_points(
    const char *filename, const Uint64 &stream_hash, drmp3 *const p_dr,
  ...
  // Get the number of compressed MP3 frames and the number of uncompressed PCM
  // frames.
  drmp3_bool32 result = drmp3_get_mp3_and_pcm_frame_count(
      p_dr, &mp3_frame_count, &pcm_frame_count);

  if (result != DRMP3_TRUE || mp3_frame_count < FRAMES_PER_SEEK_POINT ||
      pcm_frame_count < FRAMES_PER_SEEK_POINT) {
    // LOG_MSG("MP3: failed to determine or find sufficient mp3 and pcm
    // frames");
    return 0;
  }

  // Based on the number of frames found in the file, we size our seek-point
  // vector accordingly. We then pass our sized vector into dr_mp3 which
  // populates the decoded PCM times. We also take into account the desired
  // number of "FRAMES_PER_SEEK_POINT", which is defined above.
  ...
  result = drmp3_calculate_seek_points(
      p_dr, &num_seek_points,
  ...
  // points in the num_seek_points variable; so if this differs from expected
  // then we need to resize (ie: shrink) our vector.
  ...
  // scenario the seek table will be generated on-the-fly on every start of
  // DOSBox.
  if (outfile.is_open()) {
    Archive<ofstream> serialize(outfile);
    serialize << SEEK_TABLE_IDENTIFIER << seek_points_table
              << pcm_frame_count_table;
    outfile.close();
  }

  // Finally, we return the number of decoded PCM frames for this given file,
  // which doubles as a success-code.
  return pcm_frame_count;
}

// This function attempts to fetch a seek-table for a given mp3 stream from the
// fast-seek file. If anything is amiss then this function fails.
//
Uint64 load_existing_seek_points(
    const char *filename, const Uint64 &stream_hash,
    map<Uint64, vector<drmp3_seek_point_serial>> &seek_points_table,
    map<Uint64, drmp3_uint64> &pcm_frame_count_table,
  ...
  if (get_file_size(filename) <
      static_cast<int64_t>(sizeof(SEEK_TABLE_IDENTIFIER))) {
    return 0;
  }
...
// This function attempts to populate our seek table for the given mp3 stream,
// first attempting to read it from the fast-seek file and (if it can't be read
// for any reason), it calculates new data.  It makes use of the above two
// functions.
//
uint64_t populate_seek_points(struct SDL_RWops *const context, mp3_t *p_mp3,
  ...
  // Attempt to fetch the seek points and pcm count from an existing look up
  // table file.
  map<Uint64, vector<drmp3_seek_point_serial>> seek_points_table;
  map<Uint64, drmp3_uint64> pcm_frame_count_table;
  drmp3_uint64 pcm_frame_count = load_existing_seek_points(
      seektable_filename, stream_hash, seek_points_table, pcm_frame_count_table,
      p_mp3->seek_points_vector);

  // Otherwise calculate new seek points and save them to the fast-seek file.
  if (pcm_frame_count == 0) {
    pcm_frame_count = generate_new_seek_points(
        seektable_filename, stream_hash, p_mp3->p_dr, seek_points_table,
        pcm_frame_count_table, p_mp3->seek_points_vector);
    if (pcm_frame_count == 0) {
      // LOG_MSG("MP3: could not load existing or generate new seek points for
      // the stream");
      return 0;
    }
  }

  // Finally, regardless of which scenario succeeded above, we now have our seek
  // points! We bind our seek points to the dr_mp3 object which will be used for
  // fast seeking.
  if (drmp3_bind_seek_table(
          p_mp3->p_dr, static_cast<uint32_t>(p_mp3->seek_points_vector.size()),
          reinterpret_cast<drmp3_seek_point *>(
              p_mp3->seek_points_vector.data())) != DRMP3_TRUE) {
    return 0;
  }
