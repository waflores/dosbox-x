...
static int op_open1(OggOpusFile *_of, void *_stream,
...
static int op_get_link_from_serialno(const OggOpusFile *_of, int _cur_link,
                                     opus_int64 _page_offset,
                                     ogg_uint32_t _serialno) {
...
static int op_fetch_and_process_page(OggOpusFile *_of, ogg_page *_og,
                                     opus_int64 _page_offset, int _spanp,
                                     int _ignore_holes) {
  OggOpusLink *links;
  ogg_uint32_t cur_serialno;
  int seekable;
    ...
    if (OP_UNLIKELY(_of->ready_state < OP_STREAMSET)) {
      if (seekable) {
        ...
        if (links[cur_link].serialno != serialno) {
          ...
          if (OP_LIKELY(cur_link + 1 < _of->nlinks &&
                        links[cur_link + 1].serialno == serialno)) {
            cur_link++;
          } else {
            int new_link;
            new_link = op_get_link_from_serialno(_of, cur_link, _page_offset,
                                                 serialno);
