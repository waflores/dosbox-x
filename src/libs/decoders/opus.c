/*
 * This DOSBox Ogg Opus decoder backend is written and copyright 2019 Kevin R
 Croft (krcroft@gmail.com)
 *
...
// The minimum buffer samples per channel: 120 ms @ 48 samples/ms, defined by
// opus
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_OPUS = {
    {extensions_opus, "Ogg Opus audio using libopusfile",
     "Kevin R Croft <krcroft@gmail.com>", "The DOSBox-X project"},
 ...
 *    Returns: The number of bytes successfully read, or a negative value on
 * error.
 *
...
static Sint32 RWops_opus_read(void *stream, unsigned char *ptr, Sint32 nbytes) {
  const Sint32 bytes_read = SDL_RWread((SDL_RWops *)stream, (void *)ptr,
                                       sizeof(unsigned char), (size_t)nbytes);
 ...
 *       opus_int64 _offset, --> Sets the position indicator for _stream in
 * bytes int _whence         --> If whence is set to SEEK_SET, SEEK_CUR, or
 * SEEK_END, the offset is relative to the start of the stream, the current
 * position indicator, or end-of-file, respectively Returns: 0  Success, or -1
 * if seeking is not supported or an error occurred. define	SEEK_SET
 * 0 define	SEEK_CUR	1 define	SEEK_END	2
 *
 * SDL: Sint64 SDL_RWseek
 *      SDL_RWops* context --> a pointer to an SDL_RWops structure
 *      Sint64     offset, --> offset, in bytes
 *      Sint32     whence  --> an offset in bytes, relative to whence location;
 * can be negative Returns the final offset in the data stream after the seek or
 * -1 on error. RW_SEEK_SET   0 RW_SEEK_CUR   1 RW_SEEK_END   2
 */
static Sint32 RWops_opus_seek(void *stream, const opus_int64 offset,
                              const Sint32 whence) {
  const Sint64 offset_after_seek =
      SDL_RWseek((SDL_RWops *)stream, offset, whence);
...
// Populate the opus callback object (in prescribed order), with our callback
// functions.
static const OpusFileCallbacks RWops_opus_callbacks = {
    RWops_opus_read, RWops_opus_seek, RWops_opus_tell, RWops_opus_close};
...
static const char *opus_error(const Sint8 errnum) {
  ...
  case OP_EBADLINK: // -137
    return ("We failed to find data we had seen before, or the bitstream "
            ...
            "impossible");
  case OP_ENOSEEK: // -138
    return ("An operation that requires seeking was requested on an unseekable "
            "stream");
...
static __inline__ void output_opus_info(const OggOpusFile *of,
                                        const OpusHead *oh) {
#if (defined DEBUG_CHATTER)
 ...
 * Opus Open
 * =========
 *  - Creates a new opus file object by using our callback structure for all IO
 * operations.
 *  - We also initialize and allocate memory for fields in the opus_t decode
 * structure.
...
static Sint32 opus_open(Sound_Sample *sample, const char *ext) {
  ...
  OggOpusFile *of =
      op_open_callbacks(internal->rw, &RWops_opus_callbacks, NULL, 0, &rcode);
  if (rcode != 0) {
    op_free(of);
    of = NULL;
  ...
  if (sample->desired.rate != 0 && sample->desired.rate != OPUS_SAMPLE_RATE &&
      getenv("SDL_DONT_RESAMPLE") == NULL) {

    // Opus resamples all inputs to 48kHz. By default (if env-var
    // SDL_DONT_RESAMPLE doesn't exist) we resample to the desired rate so the
    // receiving SDL_sound application doesn't have to. This avoids breaking
    // applications that don't expect 48kHz audio and also gives us
    // quality-control by using the speex resampler, which has a noise floor of
    // -140 dB, which is ~40dB lower than the -96dB offered by 16-bit CD-quality
    // audio.
    //
    ...
    decoder->resampler = speex_resampler_init(
        oh->channel_count, OPUS_SAMPLE_RATE, sample->desired.rate,
  ...
  decoder->buffer_size =
      oh->channel_count * OPUS_MIN_BUFFER_SAMPLES_PER_CHANNEL * 1.5;
  decoder->buffer =
      (opus_int16 *)SDL_malloc(decoder->buffer_size * sizeof(opus_int16));

  // Gather static properties about our stream (channels, seek-ability, format,
  // and duration)
  sample->actual.channels = (Uint8)(oh->channel_count);
  sample->flags =
      (Sound_SampleFlags)(op_seekable(of) ? SOUND_SAMPLEFLAG_CANSEEK : 0);
  sample->actual.format = AUDIO_S16LSB; // returns least-significant-byte order
                                        // regardless of architecture

  ogg_int64_t total_time =
      op_pcm_total(of, -1); // total PCM samples in the stream
  internal->total_time =
      total_time == OP_EINVAL ? -1 : // total milliseconds in the stream
 ...
 */
static void opus_close(Sound_Sample *sample) {
  /* From the Opus docs: if opening a stream/file/or using op_test_callbacks()
   * fails then we are still responsible for freeing the OggOpusFile with
   * op_free().
   */
  Sound_SampleInternal *internal = (Sound_SampleInternal *)sample->opaque;
...
static Uint32 opus_read(Sound_Sample *sample) {
  ...
  const Uint16 requested_output_size =
      internal->buffer_size / sizeof(opus_int16);
  const Uint16 derived_consumption_size =
      (requested_output_size * d->rate_ratio) + 0.5;
  ...
  while (!have_consumed) {
    ...
    if (unconsumed_size >= derived_consumption_size || d->eof) {
      ...
      if (d->resampler)
        (void)speex_resampler_process_int(
            d->resampler, 0, d->buffer + d->consumed, &actual_consumed_size,
            output_buffer, &actual_output_size);
      ...
      // Otherwise copy the bytes
        ...
        SDL_memcpy(output_buffer, d->buffer + d->consumed,
                   actual_output_size * sizeof(opus_int16));
      }
      ...
      rcode = actual_output_size *
              sizeof(opus_int16); // covert from samples to bytes
      ...
      // wrap ...
      if (d->frame_size > 0) {
        SDL_memcpy(d->buffer, d->buffer + d->consumed,
      ...
      while (rcode > 0 && d->buffer_size - d->decoded >= d->frame_size) {
        ...
        rcode = sample->actual.channels * op_read(d->of, d->buffer + d->decoded,
                                                  d->buffer_size - d->decoded,
                                                  NULL);
        ...
        if (rcode > 0)
          ...
          d->eof = SDL_TRUE; // done
        else if (rcode == OP_HOLE)
          rcode = 1; // hole in the data, carry on
        else         // (rcode  < 0)                             // error
...
static Sint32 opus_seek(Sound_Sample *sample, const Uint32 ms) {
  ...
  const float seconds =
      ((int)total_seconds % 60) + (total_seconds - (int)total_seconds);
  ...
  // If not, check if we can jump within our circular buffer (and not actually
  // seek!) In this scenario, we don't have to waste our currently decoded
  // samples or incur the cost of 80ms of pre-roll decoding behind the scene in
  // libopus.
  else {
    ...
    if ((ogg_uint64_t)desired_pcm >= pcm_start &&
        (ogg_uint64_t)desired_pcm <= pcm_end) {

      SNDDBG(("Opus seek avoided:      "
              "{requested_time: '%02d:%02d:%.2f', becomes_opus_pcm: %ld, "
              "buffer_start: %ld, buffer_end: %ld}\n",
