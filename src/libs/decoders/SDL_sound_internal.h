...
#define assert(x)                                                              \
  if (!x) {                                                                    \
    fprintf(stderr, "Assertion failed in %s, line %s.\n", __FILE__, __LINE__); \
    fclose(stderr);                                                            \
    fclose(stdout);                                                            \
    exit(1);                                                                   \
  }
...
extern SNDDECLSPEC int
Sound_BuildAudioCVT(Sound_AudioCVT *cvt, Uint16 src_format, Uint8 src_channels,
                    Uint32 src_rate, Uint16 dst_format, Uint8 dst_channels,
                    Uint32 dst_rate, Uint32 dst_size);
