...
static drwav_bool32 wav_seek(void *pUserData, int offset,
                             drwav_seek_origin origin) {
  const int whence =
      (origin == drwav_seek_origin_start) ? RW_SEEK_SET : RW_SEEK_CUR;
  Sound_Sample *sample = (Sound_Sample *)pUserData;
  Sound_SampleInternal *internal = (Sound_SampleInternal *)sample->opaque;
  return (SDL_RWseek(internal->rw, offset, whence) != -1) ? DRWAV_TRUE
                                                          : DRWAV_FALSE;
} /* wav_seek */

static int WAV_init(void) { return 1; /* always succeeds. */ } /* WAV_init */

static void WAV_quit(void) { /* it's a no-op. */ } /* WAV_quit */
...
static int WAV_open(Sound_Sample *sample, const char *ext) {
  ...
  drwav_result result =
      drwav_init_ex(dr, wav_read, wav_seek, NULL, sample, NULL, 0, NULL);
...
static Uint32 WAV_read(Sound_Sample *sample) {
  ...
  const drwav_uint64 frames_read = drwav_read_pcm_frames_s16(
      dr, internal->buffer_size / (dr->channels * sizeof(drwav_int16)),
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_WAV = {
    {extensions_wav, "WAV Audio Codec", "The DOSBox-X project"},
