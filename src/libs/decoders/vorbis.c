...
static const char *vorbis_error_string(const int err) {
  ...
  case VORBIS_file_open_failure:
    return "VORBIS: failed opening the file";
  ...
  case VORBIS_ogg_skeleton_not_supported:
    return "VORBIS: multi-track streams are not supported; "
...
static void VORBIS_quit(void) { /* it's a no-op. */ } /* VORBIS_quit */
...
static int VORBIS_open(Sound_Sample *sample, const char *ext) {
  ...
  sample->actual.format =
      AUDIO_S16SYS; // returns byte-order native to the running architecture
...
static Uint32 VORBIS_read(Sound_Sample *sample) {
  ...
  rc = stb_vorbis_get_samples_short_interleaved(
      stb, channels, (int16_t *)internal->buffer, want_samples);
  retval = (Uint32)(rc * channels *
                    sizeof(int16_t)); /* rc == number of sample frames read */
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_VORBIS = {
    {extensions_vorbis, "Ogg Vorbis audio", "The DOSBox-X project"},
