 ...
 *  ---------------------------------------
 *  This decoder makes use of the dr_mp3 library by David Reid
 * (mackron@gmail.com)
...
static size_t mp3_read(void *const pUserData, void *const pBufferOut,
                       const size_t bytesToRead) {
  Uint8 *ptr = static_cast<Uint8 *>(pBufferOut);
  Sound_Sample *const sample = static_cast<Sound_Sample *>(pUserData);
  const Sound_SampleInternal *const internal =
      static_cast<const Sound_SampleInternal *>(sample->opaque);
  SDL_RWops *rwops = internal->rw;
  size_t bytes_read = 0;

  while (bytes_read < bytesToRead) {
    const size_t rc =
        SDL_RWread(rwops, ptr, 1, (int)(bytesToRead - bytes_read));
    if (rc == 0) {
...
static drmp3_bool32 mp3_seek(void *const pUserData, const Sint32 offset,
                             const drmp3_seek_origin origin) {
  const Sint32 whence =
      (origin == drmp3_seek_origin_start) ? RW_SEEK_SET : RW_SEEK_CUR;
  Sound_Sample *const sample = static_cast<Sound_Sample *>(pUserData);
  Sound_SampleInternal *const internal =
      static_cast<Sound_SampleInternal *>(sample->opaque);
  return (SDL_RWseek(internal->rw, offset, whence) != -1) ? DRMP3_TRUE
                                                          : DRMP3_FALSE;
} /* mp3_seek */

static Sint32 MP3_init(void) { return 1; /* always succeeds. */ } /* MP3_init */

static void MP3_quit(void) { /* it's a no-op. */ } /* MP3_quit */
...
static Uint32 MP3_read(Sound_Sample *const sample) {
  ...
  drmp3_uint16 remaining_frames =
      (drmp3_uint16)((internal->buffer_size / sizeof(drmp3_int16)) / channels);

  // LOG_MSG("read: remaining_frames: %u", remaining_frames);
  drmp3_uint16 total_samples_read = 0;
  while (remaining_frames > 0) {
    const drmp3_uint16 num_frames =
        (remaining_frames > in_buffer_frame_capacity) ? in_buffer_frame_capacity
                                                      : remaining_frames;

    // LOG_MSG("read-while: num_frames: %u", num_frames);
    const drmp3_uint16 frames_just_read =
        (drmp3_uint16)drmp3_read_pcm_frames_f32(p_mp3->p_dr, num_frames,
                                                in_buffer);
...
static int32_t MP3_open(Sound_Sample *const sample, const char *const ext) {
  ...
  // Open the MP3
  if (drmp3_init(p_mp3->p_dr, mp3_read, mp3_seek, sample, nullptr) !=
      DRMP3_TRUE) {
  ...
  sample->actual.format =
      AUDIO_S16SYS; // native byte-order based on architecture
  // total_time needs milliseconds
  internal->total_time = static_cast<int32_t>(
      ceil_udivide(num_frames * 1000u, sample->actual.rate));
  return 1; // success
} /* MP3_open */
...
extern const Sound_DecoderFunctions __Sound_DecoderFunctions_MP3 = {
    {extensions_mp3, "MPEG-1 Audio Layer I-III", "The DOSBox-X project"},
