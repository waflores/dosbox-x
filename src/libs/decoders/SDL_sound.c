...
static decoder_element decoders[] = {
    {0, &__Sound_DecoderFunctions_WAV},  {0, &__Sound_DecoderFunctions_VORBIS},
    {0, &__Sound_DecoderFunctions_OPUS}, {0, &__Sound_DecoderFunctions_FLAC},
    {0, &__Sound_DecoderFunctions_MP3},  {0, NULL}};
...
int Sound_Init(void) {
  ...
  available_decoders =
      (const Sound_DecoderInfo **)malloc((total) * sizeof(Sound_DecoderInfo *));
...
static Sound_Sample *alloc_sample(SDL_RWops *rw, Sound_AudioInfo *desired,
  ...
  Sound_SampleInternal *internal =
      (Sound_SampleInternal *)malloc(sizeof(Sound_SampleInternal));
...
static int init_sample(const Sound_DecoderFunctions *funcs,
  ...
  if (Sound_BuildAudioCVT(&internal->sdlcvt, sample->actual.format,
                          sample->actual.channels, sample->actual.rate,
                          desired.format, desired.channels, desired.rate,
  ...
  if (internal->sdlcvt.len_mult > 1) {
    Uint8 *rc = (Uint8 *)realloc(sample->buffer, sample->buffer_size *
                                                     internal->sdlcvt.len_mult);
    if (rc == NULL) {
      funcs->close(sample);
      SDL_RWseek(internal->rw, pos, SEEK_SET); /* set for next try... */
      return (0);
...
Sound_Sample *Sound_NewSampleFromMem(const Uint8 *data, Uint32 size,
                                     const char *ext, Sound_AudioInfo *desired,
                                     Uint32 bufferSize) {
  SDL_RWops *rw;
...
int Sound_SetBufferSize(Sound_Sample *sample, Uint32 newSize) {
  ...
  internal = ((Sound_SampleInternal *)sample->opaque);
  newBuf =
      (Uint8 *)realloc(sample->buffer, newSize * internal->sdlcvt.len_mult);
