...
/* Using two guards since this file may be included twice with different
 * MT32EMU_C_ENUMERATIONS define. */

#if (!defined MT32EMU_CPP_ENUMERATIONS_H &&                                    \
     !defined MT32EMU_C_ENUMERATIONS) ||                                       \
...
#define MT32EMU_SAMPLERATE_CONVERSION_QUALITY_NAME                             \
...
#define MT32EMU_SAMPLERATE_CONVERSION_QUALITY(ident)                           \
  SamplerateConversionQuality_##ident
...
/**
 * Methods for emulating the connection between the LA32 and the DAC, which
 * involves some hacks in the real devices for doubling the volume. See also
 * http://en.wikipedia.org/wiki/Roland_MT-32#Digital_overflow
 */
enum MT32EMU_DAC_INPUT_MODE_NAME {
   ...
   * Nicer overdrive characteristics than the DAC hacks (it simply clips samples
   * within range) Much less likely to overdrive than any other mode. Half the
   * volume of any of the other modes. Perfect for developers while debugging :)
   */
  MT32EMU_DAC_INPUT_MODE(PURE),

  /**
   * Re-orders the LA32 output bits as in early generation MT-32s (according to
   * Wikipedia). Bit order at DAC (where each number represents the original
   * LA32 output bit number, and XX means the bit is always low): 15 13 12 11 10
   * 09 08 07 06 05 04 03 02 01 00 XX
   */
  MT32EMU_DAC_INPUT_MODE(GENERATION1),

  /**
   * Re-orders the LA32 output bits as in later generations (personally
   * confirmed on my CM-32L - KG). Bit order at DAC (where each number
   * represents the original LA32 output bit number): 15 13 12 11 10 09 08 07 06
   * 05 04 03 02 01 00 14
   */
  MT32EMU_DAC_INPUT_MODE(GENERATION2)
};

/** Methods for emulating the effective delay of incoming MIDI messages
 * introduced by a MIDI interface. */
enum MT32EMU_MIDI_DELAY_MODE_NAME {
  /** Process incoming MIDI events immediately. */
  MT32EMU_MIDI_DELAY_MODE(IMMEDIATE),

  /**
   * Delay incoming short MIDI messages as if they where transferred via a MIDI
   * cable to a real hardware unit and immediate sysex processing. This ensures
   * more accurate timing of simultaneous NoteOn messages.
   */
  MT32EMU_MIDI_DELAY_MODE(DELAY_SHORT_MESSAGES_ONLY),

  /** Delay all incoming MIDI events as if they where transferred via a MIDI
     cable to a real hardware unit.*/
  MT32EMU_MIDI_DELAY_MODE(DELAY_ALL)
};

/** Methods for emulating the effects of analogue circuits of real hardware
 * units on the output signal. */
enum MT32EMU_ANALOG_OUTPUT_MODE_NAME {
  /** Only digital path is emulated. The output samples correspond to the
     digital signal at the DAC entrance. */
  MT32EMU_ANALOG_OUTPUT_MODE(DIGITAL_ONLY),
  /** Coarse emulation of LPF circuit. High frequencies are boosted, sample rate
     remains unchanged. */
  MT32EMU_ANALOG_OUTPUT_MODE(COARSE),
  /**
   * Finer emulation of LPF circuit. Output signal is upsampled to 48 kHz to
   * allow emulation of audible mirror spectra above 16 kHz, which is passed
   * through the LPF circuit without significant attenuation.
   */
  MT32EMU_ANALOG_OUTPUT_MODE(ACCURATE),
  /**
   * Same as AnalogOutputMode_ACCURATE mode but the output signal is 2x
   * oversampled, i.e. the output sample rate is 96 kHz. This makes subsequent
   * resampling easier. Besides, due to nonlinear passband of the LPF emulated,
   * it takes fewer number of MACs compared to a regular LPF FIR
   * implementations.
   */
...
#endif /* #if (!defined MT32EMU_CPP_ENUMERATIONS_H && !defined                 \
          MT32EMU_C_ENUMERATIONS) || (!defined MT32EMU_C_ENUMERATIONS_H &&     \
          defined MT32EMU_C_ENUMERATIONS) */
