...
void Poly::setPart(Part *usePart) { part = usePart; }

void Poly::reset(unsigned int newKey, unsigned int newVelocity, bool newSustain,
                 Partial **newPartials) {
  if (isActive()) {
    // This should never happen
    part->getSynth()->printDebug(
        "Resetting active poly. Active partial count: %i\n",
        activePartialCount);
    for (int i = 0; i < 4; i++) {
...
bool Poly::noteOff(bool pedalHeld) {
  ...
  // eventually anyway. Key 0 (only used by special cases on rhythm part) reacts
  // to note off even if non-sustaining or pedal held.
 ...
 * For rhythm on MT-32, this is 0 or 1 (special cases) or within the range 24
 * to 87. For rhythm on devices with extended PCM sounds (e.g. CM-32L), this is
 * 0, 1 or 24 to 108
 */
unsigned int Poly::getKey() const { return key; }

unsigned int Poly::getVelocity() const { return velocity; }

bool Poly::canSustain() const { return sustain; }

PolyState Poly::getState() const { return state; }

unsigned int Poly::getActivePartialCount() const { return activePartialCount; }

bool Poly::isActive() const { return state != POLY_Inactive; }
...
Poly *Poly::getNext() const { return next; }

void Poly::setNext(Poly *poly) { next = poly; }
