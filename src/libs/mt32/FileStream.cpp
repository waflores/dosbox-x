...
// This initialises C locale with the user-preferred system locale once
// facilitating access to ROM files with localised pathnames. This is only
// necessary in rare cases e.g. when building shared library statically linked
// with C runtime with old MS VC versions, so that the C locale set by the
// client application does not have effect, and thus such ROM files cannot be
// opened.
