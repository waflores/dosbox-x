...
public:
  FIRResampler(const unsigned int upsampleFactor, const double downsampleFactor,
               const FIRCoefficient kernel[], const unsigned int kernelLength);
  ~FIRResampler();
...
private:
  const struct Constants {
    ...
    // Index of last delay line element, generally greater than numberOfTaps to
    // form a proper binary mask
