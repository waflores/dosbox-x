...
// (zero-order numerator coefficient is always zero, zero-order denominator
// coefficient is always unity)
struct IIRSection {
  IIRCoefficient num1;
  IIRCoefficient num2;
  IIRCoefficient den1;
...
public:
  enum Quality {
    ...
    CUSTOM,
    // Use fast elliptic filter with symmetric ripple: N=8, Ap=As=-99 dB,
    // fp=0.125, fs = 0.25 (in terms of sample rate)
    FAST,
    // Use average elliptic filter with symmetric ripple: N=12, Ap=As=-106 dB,
    // fp=0.193, fs = 0.25 (in terms of sample rate)
    GOOD,
    // Use sharp elliptic filter with symmetric ripple: N=18, Ap=As=-106 dB,
    // fp=0.238, fs = 0.25 (in terms of sample rate)
    BEST
  };

  // Returns the retained fraction of the passband for the given standard
  // quality value
  static double getPassbandFractionForQuality(Quality quality);

protected:
  explicit IIRResampler(const Quality quality);
  explicit IIRResampler(const unsigned int useSectionsCount,
  ...
  const struct Constants {
    ...
    Constants(const unsigned int useSectionsCount, const IIRCoefficient useFIR,
              const IIRSection useSections[], const Quality quality);
  } constants;
}; // class IIRResampler
...
public:
  ...
  explicit IIR2xInterpolator(const unsigned int useSectionsCount,
                             const IIRCoefficient useFIR,
                             const IIRSection useSections[]);
...
public:
  ...
  explicit IIR2xDecimator(const unsigned int useSectionsCount,
                          const IIRCoefficient useFIR,
                          const IIRSection useSections[]);
