...
FIRResampler::Constants::Constants(const unsigned int upsampleFactor,
  ...
  unsigned int minDelayLineLength =
      static_cast<unsigned int>(ceil(double(kernelLength) / upsampleFactor));
...
FIRResampler::FIRResampler(const unsigned int upsampleFactor,
                           const double downsampleFactor,
                           const FIRCoefficient kernel[],
                           const unsigned int kernelLength)
    : constants(upsampleFactor, downsampleFactor, kernel, kernelLength),
      ringBufferPosition(0), phase(constants.numberOfPhases) {}
...
void FIRResampler::process(const FloatSample *&inSamples,
...
unsigned int
FIRResampler::estimateInLength(const unsigned int outLength) const {
  return static_cast<unsigned int>(
      (outLength * constants.phaseIncrement + phase) /
      constants.numberOfPhases);
}
...
void FIRResampler::getOutSamplesStereo(FloatSample *&outSamples) {
  ...
  if (constants.usePhaseInterpolation) {
    ...
    unsigned int maxTapIx = phaseFraction == 0 ? constants.numberOfTaps
                                               : constants.numberOfTaps - 1;
    for (unsigned int tapIx = static_cast<unsigned int>(phase);
         tapIx < maxTapIx; tapIx += constants.numberOfPhases) {
      FIRCoefficient tap = FIRCoefficient(
          constants.taps[tapIx] +
          (constants.taps[tapIx + 1] - constants.taps[tapIx]) * phaseFraction);
    ...
    for (unsigned int tapIx = static_cast<unsigned int>(phase);
         tapIx < constants.numberOfTaps; tapIx += constants.numberOfPhases) {
