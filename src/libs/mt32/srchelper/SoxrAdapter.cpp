...
size_t SoxrAdapter::getInputSamples(void *input_fn_state, soxr_in_t *data,
                                    size_t requested_len) {
  unsigned int length = requested_len < 1
                            ? 1
                            : (MAX_SAMPLES_PER_RUN < requested_len
                                   ? MAX_SAMPLES_PER_RUN
                                   : static_cast<unsigned int>(requested_len));
...
SoxrAdapter::SoxrAdapter(Synth &useSynth, double targetSampleRate,
  ...
  soxr_error_t error;
  resampler = soxr_create(synth.getStereoOutputSampleRate(), targetSampleRate,
                          CHANNEL_COUNT, &error, &ioSpec, &qSpec, &rtSpec);
  if (error != NULL) {
  ...
  error =
      soxr_set_input_fn(resampler, getInputSamples, this, MAX_SAMPLES_PER_RUN);
  if (error != NULL) {
    synth.printDebug(
        "SoxrAdapter: Installing sample feed for SOXR failed: %s\n",
        soxr_strerror(error));
    soxr_delete(resampler);
    resampler = NULL;
  }
...
void SoxrAdapter::getOutputSamples(float *buffer, unsigned int length) {
  ...
  while (length > 0) {
    ...
    if (error != NULL) {
      synth.printDebug(
          "SoxrAdapter: SOXR error during processing: %s > resetting\n",
          soxr_strerror(error));
