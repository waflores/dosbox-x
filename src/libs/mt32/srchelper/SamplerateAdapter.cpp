...
long SamplerateAdapter::getInputSamples(void *cb_data, float **data) {
  ...
  unsigned int length = instance->inBufferSize < 1
                            ? 1
                            : (MAX_SAMPLES_PER_RUN < instance->inBufferSize
                                   ? MAX_SAMPLES_PER_RUN
                                   : instance->inBufferSize);
...
SamplerateAdapter::SamplerateAdapter(Synth &useSynth, double targetSampleRate,
                                     SamplerateConversionQuality quality)
    : synth(useSynth), inBuffer(new float[CHANNEL_COUNT * MAX_SAMPLES_PER_RUN]),
      inBufferSize(MAX_SAMPLES_PER_RUN),
      inputToOutputRatio(useSynth.getStereoOutputSampleRate() /
                         targetSampleRate),
      outputToInputRatio(targetSampleRate /
                         useSynth.getStereoOutputSampleRate()) {
  int error;
  int conversionType;
  switch (quality) {
  ...
  resampler = src_callback_new(getInputSamples, conversionType, CHANNEL_COUNT,
                               &error, this);
  if (error != 0) {
    synth.printDebug(
        "SamplerateAdapter: Creation of Samplerate instance failed: %s\n",
        src_strerror(error));
...
void SamplerateAdapter::getOutputSamples(float *buffer, unsigned int length) {
  ...
  while (length > 0) {
    ...
    long gotFrames =
        src_callback_read(resampler, outputToInputRatio, long(length), buffer);
    int error = src_error(resampler);
    if (error != 0) {
      synth.printDebug("SamplerateAdapter: Samplerate error during processing: "
                       "%s > resetting\n",
                       src_strerror(error));
    ...
    if (gotFrames <= 0) {
      synth.printDebug(
          "SamplerateAdapter: got %li frames from Samplerate, weird\n",
          gotFrames);
    }
