...
/** Facilitates emulation of internal state of the MIDI MESSAGE LED and the
 * MT-32 LCD. */
class Display {
public:
  ...
  Display(Synth &synth);
  void checkDisplayStateUpdated(bool &midiMessageLEDState,
                                bool &midiMessageLEDUpdated, bool &lcdUpdated);
  /** Returns whether the MIDI MESSAGE LED is ON and fills the targetBuffer
   * parameter. */
  ...
  bool customDisplayMessageReceived(const Bit8u *message, Bit32u startIndex,
                                    Bit32u length);
