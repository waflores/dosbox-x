...
class Part {
private:
  // Direct pointer to sysex-addressable memory dedicated to this part (valid
  // for parts 1-8, NULL for rhythm)
...
protected:
  ...
  Bit16u rpn;
  Bit16u
      pitchBenderRange; // (patchTemp->patch.benderRange * 683) at the time of
                        // the last MIDI program change or MIDI data entry.

  void backupCacheToPartials(PatchCache cache[4]);
  void cacheTimbre(PatchCache cache[4], const TimbreParam *timbre);
  void playPoly(const PatchCache cache[4],
                const MemParams::RhythmTemp *rhythmTemp, unsigned int midiKey,
                unsigned int key, unsigned int velocity);
...
public:
  ...
  void setVolume(unsigned int midiVolume); // Valid range 0..127, as defined for
                                           // MIDI controller 7.
  ...
  // These are rather specialised, and should probably only be used by
  // PartialManager
  bool abortFirstPoly(PolyState polyState);
  // Abort the first poly in PolyState_HELD, or if none exists, the first active
  // poly in any state.
