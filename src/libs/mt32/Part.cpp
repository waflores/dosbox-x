...
static const Bit8u PartialStruct[13] = {0, 0, 2, 2, 1, 3, 3, 0, 3, 0, 2, 1, 3};

static const Bit8u PartialMixStruct[13] = {0, 1, 0, 1, 1, 0, 1,
                                           3, 3, 2, 2, 2, 2};
...
Bit32s Part::getPitchBend() const { return pitchBend; }

void Part::setBend(unsigned int midiBend) {
  // CONFIRMED:
  pitchBend = ((signed(midiBend) - 8192) * pitchBenderRange) >>
              14; // PORTABILITY NOTE: Assumes arithmetic shift
}

Bit8u Part::getModulation() const { return modulation; }
...
void RhythmPart::refresh() {
  ...
  for (unsigned int drumNum = 0;
       drumNum < synth->controlROMMap->rhythmSettingsCount; drumNum++) {
...
const char *Part::getCurrentInstr() const { return &currentInstr[0]; }
...
void Part::setPatch(const PatchParam *patch) { patchTemp->patch = *patch; }

void RhythmPart::setTimbre(TimbreParam * /*timbre*/) {
  synth->printDebug(
      "%s: Attempted to call setTimbre() - doesn't make sense for rhythm",
      name);
}

void Part::setTimbre(TimbreParam *timbre) { *timbreTemp = *timbre; }

unsigned int RhythmPart::getAbsTimbreNum() const {
  synth->printDebug(
      "%s: Attempted to call getAbsTimbreNum() - doesn't make sense for rhythm",
      name);
  return 0;
}
...
void RhythmPart::setProgram(unsigned int patchNum) {
  synth->printDebug("%s: Attempt to set program (%d) on rhythm is invalid",
                    name, patchNum);
}
#else
...
void Part::backupCacheToPartials(PatchCache cache[4]) {
  ...
  for (Poly *poly = activePolys.getFirst(); poly != NULL;
       poly = poly->getNext()) {
...
void Part::cacheTimbre(PatchCache cache[4], const TimbreParam *timbre) {
  ...
  for (int t = 0; t < 4; t++) {
    ...
    case 0:
      cache[t].PCMPartial =
          (PartialStruct[int(timbre->common.partialStructure12)] & 0x2) ? true
                                                                        : false;
      cache[t].structureMix =
          PartialMixStruct[int(timbre->common.partialStructure12)];
      cache[t].structurePosition = 0;
      cache[t].structurePair = 1;
      break;
    case 1:
      cache[t].PCMPartial =
          (PartialStruct[int(timbre->common.partialStructure12)] & 0x1) ? true
                                                                        : false;
      cache[t].structureMix =
          PartialMixStruct[int(timbre->common.partialStructure12)];
    ...
    case 2:
      cache[t].PCMPartial =
          (PartialStruct[int(timbre->common.partialStructure34)] & 0x2) ? true
                                                                        : false;
      cache[t].structureMix =
          PartialMixStruct[int(timbre->common.partialStructure34)];
      cache[t].structurePosition = 0;
      cache[t].structurePair = 3;
      break;
    case 3:
      cache[t].PCMPartial =
          (PartialStruct[int(timbre->common.partialStructure34)] & 0x1) ? true
                                                                        : false;
      cache[t].structureMix =
          PartialMixStruct[int(timbre->common.partialStructure34)];
      cache[t].structurePosition = 1;
  ...
  // synth->printDebug("Res 1: %d 2: %d 3: %d 4: %d", cache[0].waveform,
  // cache[1].waveform, cache[2].waveform, cache[3].waveform);

#if MT32EMU_MONITOR_INSTRUMENTS > 0
  synth->printDebug("%s (%s): Recached timbre", name, currentInstr);
  for (int i = 0; i < 4; i++) {
    synth->printDebug(
        " %d: play=%s, pcm=%s (%d), wave=%d", i,
        cache[i].playPartial ? "YES" : "NO", cache[i].PCMPartial ? "YES" : "NO",
        timbre->partial[i].wg.pcmWave, timbre->partial[i].wg.waveform);
  }
#endif
}

const char *Part::getName() const { return name; }
...
void Part::setVolume(unsigned int midiVolume) {
  ...
  // synth->printDebug("%s (%s): Set volume to %d", name, currentInstr,
  // midiVolume);
}
...
void Part::setVolumeOverride(Bit8u volume) {
  ...
  // For that to achieve, we have to actually stop processing NoteOn MIDI
  // messages; merely returning 0 volume is not enough - the output may still be
  // generated at a very low level. But first, we have to stop all the currently
  // playing polys. This behaviour may also help with performance issues,
  // because parts muted this way barely consume CPU resources.
  if (volume == 0)
    allSoundOff();
}

Bit8u Part::getVolumeOverride() const { return volumeOverride; }

Bit8u Part::getExpression() const { return expression; }
...
void RhythmPart::setPan(unsigned int midiPan) {
  // CONFIRMED: This does change patchTemp, but has no actual effect on
  // playback.
#if MT32EMU_MONITOR_MIDI > 0
  synth->printDebug("%s: Pointlessly setting pan (%d) on rhythm part", name,
                    midiPan);
#endif
  Part::setPan(midiPan);
}
...
/**
 * Applies key shift to a MIDI key and converts it into an internal key value in
 * the range 12-108.
 */
unsigned int Part::midiKeyToKey(unsigned int midiKey) {
  if (synth->controlROMFeatures->quirkKeyShift) {
    // NOTE: On MT-32 GEN0, key isn't adjusted, and keyShift is applied further
    // in TVP, unlike newer units:
    return midiKey;
  }
...
void RhythmPart::noteOn(unsigned int midiKey, unsigned int velocity) {
  if (midiKey < 24 || midiKey > 108) { /*> 87 on MT-32)*/
    synth->printDebug("%s: Attempted to play invalid key %d (velocity %d)",
                      name, midiKey, velocity);
    return;
  }
  ...
  int drumTimbreNum = rhythmTemp[drumNum].timbre;
  const int drumTimbreCount =
      64 +
      synth->controlROMMap->timbreRCount; // 94 on MT-32, 128 on LAPC-I/CM32-L
  if (drumTimbreNum == 127 ||
      drumTimbreNum >=
          drumTimbreCount) { // timbre #127 is OFF, no sense to play it
    synth->printDebug("%s: Attempted to play unmapped key %d (velocity %d)",
                      name, midiKey, velocity);
    return;
  }
...
#if MT32EMU_MONITOR_INSTRUMENTS > 0
  synth->printDebug("%s (%s): Start poly (drum %d, timbre %d): midiKey %u, key "
                    "%u, velo %u, mod %u, exp %u, bend %u",
                    name, currentInstr, drumNum, absTimbreNum, midiKey, key,
                    velocity, modulation, expression, pitchBend);
#if MT32EMU_MONITOR_INSTRUMENTS > 1
  // According to info from Mok, keyShift does not appear to affect anything on
  // rhythm part on LAPC-I, but may do on MT-32 - needs investigation
  synth->printDebug(
      " Patch: (timbreGroup %u), (timbreNum %u), (keyShift %u), fineTune %u, "
      "benderRange %u, assignMode %u, (reverbSwitch %u)",
      patchTemp->patch.timbreGroup, patchTemp->patch.timbreNum,
      patchTemp->patch.keyShift, patchTemp->patch.fineTune,
      patchTemp->patch.benderRange, patchTemp->patch.assignMode,
      patchTemp->patch.reverbSwitch);
  synth->printDebug(" PatchTemp: outputLevel %u, (panpot %u)",
                    patchTemp->outputLevel, patchTemp->panpot);
  synth->printDebug(
      " RhythmTemp: timbre %u, outputLevel %u, panpot %u, reverbSwitch %u",
      rhythmTemp[drumNum].timbre, rhythmTemp[drumNum].outputLevel,
      rhythmTemp[drumNum].panpot, rhythmTemp[drumNum].reverbSwitch);
#endif
#endif
...
#if MT32EMU_MONITOR_INSTRUMENTS > 0
  synth->printDebug("%s (%s): Start poly: midiKey %u, key %u, velo %u, mod %u, "
                    "exp %u, bend %u",
                    name, currentInstr, midiKey, key, velocity, modulation,
...
void Part::playPoly(const PatchCache cache[4],
                    const MemParams::RhythmTemp *rhythmTemp,
                    unsigned int midiKey, unsigned int key,
                    unsigned int velocity) {
  // CONFIRMED: Even in single-assign mode, we don't abort playing polys if the
  // timbre to play is completely muted.
  unsigned int needPartials = cache[0].partialCount;
  if (needPartials == 0) {
    synth->printDebug("%s (%s): Completely muted instrument", name,
                      currentInstr);
    return;
  }
  ...
  if (!synth->partialManager->freePartials(needPartials, partNum)) {
    ...
    synth->printDebug("%s (%s): Insufficient free partials to play key %d "
                      "(velocity %d); needed=%d, free=%d, assignMode=%d",
                      name, currentInstr, midiKey, velocity, needPartials,
                      synth->partialManager->getFreePartialCount(),
                      patchTemp->patch.assignMode);
  ...
  if (poly == NULL) {
    synth->printDebug("%s (%s): No free poly to play key %d (velocity %d)",
                      name, currentInstr, midiKey, velocity);
    return;
  }
  ...
  for (int x = 0; x < 4; x++) {
    if (partials[x] != NULL) {
      ...
      synth->printDebug("%s (%s): Allocated partial %d", name, currentInstr,
                        partials[x]->debugGetPartialNum());
#endif
      partials[x]->startPartial(this, poly, &cache[x], rhythmTemp,
                                partials[cache[x].structurePair]);
    }
  }
...
void Part::allNotesOff() {
  // The MIDI specification states - and Mok confirms - that all notes off
  // (0x7B) should treat the hold pedal as usual.
  for (Poly *poly = activePolys.getFirst(); poly != NULL;
       poly = poly->getNext()) {
    // FIXME: This has special handling of key 0 in NoteOff that Mok has not yet
    // confirmed applies to AllNotesOff. if (poly->canSustain() ||
    // poly->getKey() == 0) {
    // FIXME: The real devices are found to be ignoring non-sustaining polys
    // while processing AllNotesOff. Need to be confirmed.
    if (poly->canSustain()) {
...
void Part::allSoundOff() {
  // MIDI "All sound off" (0x78) should release notes immediately regardless of
  // the hold pedal. This controller is not actually implemented by the synths,
  // though (according to the docs and Mok) - we're only using this method
  // internally.
  for (Poly *poly = activePolys.getFirst(); poly != NULL;
       poly = poly->getNext()) {
    poly->startDecay();
...
void RhythmPart::noteOff(unsigned int midiKey) { stopNote(midiKey); }

void Part::noteOff(unsigned int midiKey) { stopNote(midiKeyToKey(midiKey)); }
...
void Part::stopNote(unsigned int key) {
  ...
  for (Poly *poly = activePolys.getFirst(); poly != NULL;
       poly = poly->getNext()) {
    // Generally, non-sustaining instruments ignore note off. They die away
    // eventually anyway. Key 0 (only used by special cases on rhythm part)
    // reacts to note off even if non-sustaining or pedal held.
...
const MemParams::PatchTemp *Part::getPatchTemp() const { return patchTemp; }

unsigned int Part::getActivePartialCount() const { return activePartialCount; }

const Poly *Part::getFirstActivePoly() const { return activePolys.getFirst(); }
...
Synth *Part::getSynth() const { return synth; }
...
void Part::polyStateChanged(PolyState oldState, PolyState newState) {
  ...
  case POLY_Playing:
    if (activeNonReleasingPolyCount++ == 0)
      synth->voicePartStateChanged(partNum, true);
    break;
  case POLY_Releasing:
  case POLY_Inactive:
    if (oldState == POLY_Playing || oldState == POLY_Held) {
      if (--activeNonReleasingPolyCount == 0)
        synth->voicePartStateChanged(partNum, false);
    }
    break;
  default:
    break;
  ...
  synth->printDebug(
      "Part %d: Changed poly state %d->%d, activeNonReleasingPolyCount=%d",
      partNum, oldState, newState, activeNonReleasingPolyCount);
#endif
}
...
Poly *PolyList::getFirst() const { return firstPoly; }

Poly *PolyList::getLast() const { return lastPoly; }

void PolyList::prepend(Poly *poly) {
#ifdef MT32EMU_POLY_LIST_DEBUG
  if (poly->getNext() != NULL) {
    printf(
        "PolyList: Non-NULL next field in a Poly being prepended is ignored\n");
  }
#endif
...
void PolyList::append(Poly *poly) {
  ...
  if (poly->getNext() != NULL) {
    printf(
        "PolyList: Non-NULL next field in a Poly being appended is ignored\n");
  }
#endif
  poly->setNext(NULL);
  if (lastPoly != NULL) {
