 ...
 * runtime. Sadly, per-symbol versioning is unavailable on many platforms, and
 * even where it is, it's still not too easy to maintain for a C++ library.
 * Therefore, the goal here is just to ensure that the client application
 * quickly bails out when attempted to run with an older version of shared
 * library, as well as to produce a more readable error message indicating a
 * version mismatch rather than a report about some missing symbols with
 * unreadable mangled names. This is an optional feature, since it adds some
 * minor burden to both the library and client applications code, albeit it is
 * ought to work on platforms that do not implement symbol versioning.
 */
...
#define MT32EMU_VERSION_TAG                                                    \
  MT32EMU_BUILD_VERSION_TAG(MT32EMU_VERSION_MAJOR, MT32EMU_VERSION_MINOR)
