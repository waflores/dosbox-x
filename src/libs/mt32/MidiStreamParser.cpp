...
DefaultMidiStreamParser::DefaultMidiStreamParser(
    Synth &useSynth, Bit32u initialStreamBufferCapacity)
    : MidiStreamParser(initialStreamBufferCapacity), synth(useSynth),
      timestampSet(false) {}
...
void DefaultMidiStreamParser::resetTimestamp() { timestampSet = false; }
...
void DefaultMidiStreamParser::handleSysex(const Bit8u *stream,
                                          const Bit32u length) {
  do {
    if (timestampSet) {
...
void DefaultMidiStreamParser::handleSystemRealtimeMessage(
...
MidiStreamParserImpl::MidiStreamParserImpl(MidiReceiver &useReceiver,
                                           MidiReporter &useReporter,
                                           Bit32u initialStreamBufferCapacity)
...
MidiStreamParserImpl::~MidiStreamParserImpl() { delete[] streamBuffer; }
...
void MidiStreamParserImpl::processShortMessage(const Bit32u message) {
  ...
  if (0xF8 <= status) {
    ...
    midiReceiver.handleShortMessage((message << 8) | status);
             ...
             status) { // If no running status available yet, skip this message
    midiReceiver.handleShortMessage(message);
  }
}

// We deal with SysEx messages below 512 bytes long in most cases. Nevertheless,
// it seems reasonable to support a possibility to load bulk dumps using a
// single message. However, this is known to fail with a real device due to
// limited input buffer size.
bool MidiStreamParserImpl::checkStreamBufferCapacity(
...
// Checks input byte whether it is a status byte. If not, replaces it with
// running status when available. Returns true if the input byte was changed to
// running status.
bool MidiStreamParserImpl::processStatusByte(Bit8u &status) {
  if (status < 0x80) {
    ...
    if (runningStatus < 0x80) {
      ...
      midiReporter.printDebug("processStatusByte: No valid running status yet, "
...
// Returns # of bytes parsed
Bit32u MidiStreamParserImpl::parseShortMessageDataBytes(const Bit8u stream[],
                                                        Bit32u length) {
  ...
  while ((streamBufferSize < shortMessageLength) && (length-- > 0)) {
    ...
    if (dataByte < 0x80) {
      ...
      char s[128];
      sprintf(s,
              "parseShortMessageDataBytes: Invalid short message: status %02x, "
              "expected length %i, actual %i -> ignored",
              *streamBuffer, shortMessageLength, streamBufferSize);
...
// Returns # of bytes parsed
Bit32u MidiStreamParserImpl::parseSysex(const Bit8u stream[],
                                        const Bit32u length) {
  // Find SysEx length
  ...
  if (checkStreamBufferCapacity(false)) {
    ...
    // Not enough buffer capacity, don't care about the real buffer content,
    // just mark the first byte
    *streamBuffer = *stream;
...
// Returns # of bytes parsed
Bit32u MidiStreamParserImpl::parseSysexFragment(const Bit8u stream[],
                                                const Bit32u length) {
  Bit32u parsedLength = 0;
  while (parsedLength < length) {
    ...
    if (nextByte != 0xF7) {
      // Illegal status byte in SysEx message, aborting
      midiReporter.printDebug("parseSysexFragment: SysEx message lacks "
                              "end-of-sysex (0xf7), ignored");
    ...
    midiReporter.printDebug("parseSysexFragment: streamBuffer overrun while "
                            "receiving SysEx message, ignored. Max allowed "
                            "size of fragmented SysEx is 32768 bytes.");
    streamBufferSize = 0; // Clear streamBuffer
    break;
  }
  return parsedLength;
