...
// MT32EMU_MEMADDR() converts from sysex-padded, MT32EMU_SYSEXMEMADDR converts
// to it Roland provides documentation using the sysex-padded addresses, so we
// tend to use that in code and output
#define MT32EMU_MEMADDR(x)                                                     \
  ((((x) & 0x7f0000) >> 2) | (((x) & 0x7f00) >> 1) | ((x) & 0x7f))
#define MT32EMU_SYSEXMEMADDR(x)                                                \
  ((((x) & 0x1FC000) << 2) | (((x) & 0x3F80) << 1) | ((x) & 0x7f))
...
struct TimbreParam {
  ...
  struct PartialParam {
    struct WGParam {
      ...
      Bit8u pitchKeyfollow; // 0-16 (-1, -1/2, -1/4, 0, 1/8, 1/4, 3/8, 1/2, 5/8,
                            // 3/4, 7/8, 1, 5/4, 3/2, 2, s1, s2)
      Bit8u pitchBenderEnabled; // 0-1 (OFF, ON)
      Bit8u waveform; // MT-32: 0-1 (SQU/SAW); LAPC-I: WG WAVEFORM/PCM BANK 0 -
                      // 3 (SQU/1, SAW/1, SQU/2, SAW/2)
    ...
    struct TVFParam {
      ...
      Bit8u keyfollow; // -1, -1/2, -1/4, 0, 1/8, 1/4, 3/8, 1/2, 5/8, 3/4, 7/8,
                       // 1, 5/4, 3/2, 2
...
struct MemParams {
  ...
  // The LAPC-I documentation specified an additional area for rhythm at the
  // end, where all parameters but fine tune, assign mode and output level are
  // ignored
  struct PatchTemp {
    PatchParam patch;
  ...
  struct RhythmTemp {
    Bit8u timbre;       // TIMBRE  0-94 (M1-M64,R1-30,OFF); LAPC-I: 0-127
                        // (M01-M64,R01-R63)
    Bit8u outputLevel;  // OUTPUT LEVEL 0-100
    Bit8u panpot;       // PANPOT 0-14 (R-L)
    Bit8u reverbSwitch; // REVERB SWITCH 0-1 (OFF,ON)
  ...
  // NOTE: There are only 30 timbres in the "rhythm" bank for MT-32; the
  // additional 34 are for LAPC-I and above
  struct PaddedTimbre {
    TimbreParam timbre;
    Bit8u padding[10];
  } MT32EMU_ALIGN_PACKED
      timbres[64 + 64 + 64 + 64]; // Group A, Group B, Memory, Rhythm
...
struct ControlROMFeatureSet {
  ...
  // Features below don't actually depend on control ROM version, which is used
  // to identify hardware model
...
// This is basically a per-partial, pre-processed combination of timbre and
// patch/rhythm settings
struct PatchCache {
  bool playPartial;
  bool PCMPartial;
  int pcm;
