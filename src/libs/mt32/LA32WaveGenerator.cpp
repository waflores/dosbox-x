...
Bit16u LA32Utilites::interpolateExp(const Bit16u fract) {
  ...
  Bit16u expTabEntry1 =
      expTabIndex == 0 ? 8191
                       : (8191 - Tables::getInstance().exp9[expTabIndex - 1]);
...
void LA32Utilites::addLogSamples(LogSample &logSample1,
                                 const LogSample &logSample2) {
...
Bit32u
LA32WaveGenerator::getResonanceWaveLengthFactor(Bit32u effectiveCutoffValue) {
  // resonanceWaveLengthFactor = (Bit32u)EXP2F(12.0f + effectiveCutoffValue /
  // 4096.0f);
  Bit32u resonanceWaveLengthFactor =
      LA32Utilites::interpolateExp(~effectiveCutoffValue & 4095);
...
Bit32u LA32WaveGenerator::getHighLinearLength(Bit32u effectiveCutoffValue) {
  ...
  // highLinearLength = EXP2F(19.0f - effectivePulseWidthValue / 4096.0f +
  // effectiveCutoffValue / 4096.0f) - 2 * SINE_SEGMENT_RELATIVE_LENGTH;
...
void LA32WaveGenerator::computePositions(Bit32u highLinearLength,
                                         Bit32u lowLinearLength,
                                         Bit32u resonanceWaveLengthFactor) {
  // Assuming 12-bit multiplication used here
  squareWavePosition = resonanceSinePosition =
      (wavePosition >> 8) * (resonanceWaveLengthFactor >> 4);
...
void LA32WaveGenerator::advancePosition() {
  ...
  Bit32u effectiveCutoffValue = (cutoffVal > MIDDLE_CUTOFF_VALUE)
                                    ? (cutoffVal - MIDDLE_CUTOFF_VALUE) >> 10
                                    : 0;
  Bit32u resonanceWaveLengthFactor =
      getResonanceWaveLengthFactor(effectiveCutoffValue);
  Bit32u highLinearLength = getHighLinearLength(effectiveCutoffValue);
  Bit32u lowLinearLength = (resonanceWaveLengthFactor << 8) -
                           4 * SINE_SEGMENT_RELATIVE_LENGTH - highLinearLength;
  computePositions(highLinearLength, lowLinearLength,
                   resonanceWaveLengthFactor);

  resonancePhase =
      ResonancePhase(((resonanceSinePosition >> 18) +
                      (phase > POSITIVE_FALLING_SINE_SEGMENT ? 2 : 0)) &
                     3);
}
...
void LA32WaveGenerator::generateNextSquareWaveLogSample() {
  ...
  case NEGATIVE_FALLING_SINE_SEGMENT:
    logSampleValue =
        Tables::getInstance().logsin9[(squareWavePosition >> 9) & 511];
    break;
  ...
  squareLogSample.logValue =
      logSampleValue < 65536 ? Bit16u(logSampleValue) : 65535;
  squareLogSample.sign = phase < NEGATIVE_FALLING_SINE_SEGMENT
                             ? LogSample::POSITIVE
                             : LogSample::NEGATIVE;
}
...
void LA32WaveGenerator::generateNextResonanceWaveLogSample() {
  Bit32u logSampleValue;
  if (resonancePhase == POSITIVE_FALLING_RESONANCE_SINE_SEGMENT ||
      resonancePhase == NEGATIVE_RISING_RESONANCE_SINE_SEGMENT) {
    logSampleValue =
        Tables::getInstance().logsin9[~(resonanceSinePosition >> 9) & 511];
  } else {
    logSampleValue =
        Tables::getInstance().logsin9[(resonanceSinePosition >> 9) & 511];
  }
  ...
  // From the digital captures, the decaying speed of the resonance sine is
  // found a bit different for the positive and the negative segments
  Bit32u decayFactor = phase < NEGATIVE_FALLING_SINE_SEGMENT
                           ? resAmpDecayFactor
                           : resAmpDecayFactor + 1;
  // Unsure about resonanceSinePosition here. It's possible that dedicated
  // counter & decrement are used. Although, cutoff is finely ramped, so maybe
  // not.
  logSampleValue += resonanceAmpSubtraction +
                    (((resonanceSinePosition >> 4) * decayFactor) >> 8);

  // To ensure the output wave has no breaks, two different windows are applied
  // to the beginning and the ending of the resonance sine segment
  if (phase == POSITIVE_RISING_SINE_SEGMENT ||
      phase == NEGATIVE_FALLING_SINE_SEGMENT) {
    // The window is synchronous sine here
    logSampleValue +=
        Tables::getInstance().logsin9[(squareWavePosition >> 9) & 511] << 2;
  } else if (phase == POSITIVE_FALLING_SINE_SEGMENT ||
             phase == NEGATIVE_RISING_SINE_SEGMENT) {
    // The window is synchronous square sine here
    logSampleValue +=
        Tables::getInstance().logsin9[~(squareWavePosition >> 9) & 511] << 3;
  }

  if (cutoffVal < MIDDLE_CUTOFF_VALUE) {
    // For the cutoff values below the cutoff middle point, it seems the amp of
    // the resonance wave is exponentially decayed
    logSampleValue += 31743 + ((MIDDLE_CUTOFF_VALUE - cutoffVal) >> 9);
  } else if (cutoffVal < RESONANCE_DECAY_THRESHOLD_CUTOFF_VALUE) {
    // For the cutoff values below this point, the amp of the resonance wave is
    // sinusoidally decayed
    Bit32u sineIx = (cutoffVal - MIDDLE_CUTOFF_VALUE) >> 13;
    logSampleValue += Tables::getInstance().logsin9[sineIx] << 2;
  }

  // After all the amp decrements are added, it should be safe now to adjust the
  // amp of the resonance wave to what we see on captures
  logSampleValue -= 1 << 12;

  resonanceLogSample.logValue =
      logSampleValue < 65536 ? Bit16u(logSampleValue) : 65535;
  resonanceLogSample.sign =
      resonancePhase < NEGATIVE_FALLING_RESONANCE_SINE_SEGMENT
          ? LogSample::POSITIVE
          : LogSample::NEGATIVE;
}

void LA32WaveGenerator::generateNextSawtoothCosineLogSample(
    LogSample &logSample) const {
  Bit32u sawtoothCosinePosition = wavePosition + (1 << 18);
  if ((sawtoothCosinePosition & (1 << 18)) > 0) {
    logSample.logValue =
        Tables::getInstance().logsin9[~(sawtoothCosinePosition >> 9) & 511];
  } else {
    logSample.logValue =
        Tables::getInstance().logsin9[(sawtoothCosinePosition >> 9) & 511];
  }
  logSample.logValue <<= 2;
  logSample.sign = ((sawtoothCosinePosition & (1 << 19)) == 0)
                       ? LogSample::POSITIVE
                       : LogSample::NEGATIVE;
}

void LA32WaveGenerator::pcmSampleToLogSample(LogSample &logSample,
                                             const Bit16s pcmSample) const {
...
void LA32WaveGenerator::generateNextPCMWaveLogSamples() {
  // This should emulate the ladder we see in the PCM captures for pitches 01,
  // 02, 07, etc. The most probable cause is the factor in the interpolation
  // formula is one bit less accurate than the sample position counter
  ...
  if (pcmWaveInterpolated) {
    ...
    if (pcmWaveTableIx < pcmWaveLength) {
      ...
      if (pcmWaveLooped) {
        ...
        pcmSampleToLogSample(secondPCMLogSample,
                             pcmWaveAddress[pcmWaveTableIx]);
      } else {
...
void LA32WaveGenerator::initSynth(const bool useSawtoothWaveform,
                                  const Bit8u usePulseWidth,
                                  const Bit8u useResonance) {
  ...
  resAmpDecayFactor = Tables::getInstance().resAmpDecayFactor[resonance >> 2]
                      << 2;
...
void LA32WaveGenerator::initPCM(const Bit16s *const usePCMWaveAddress,
                                const Bit32u usePCMWaveLength,
                                const bool usePCMWaveLooped,
                                const bool usePCMWaveInterpolated) {
...
void LA32WaveGenerator::generateNextSample(const Bit32u useAmp,
                                           const Bit16u usePitch,
                                           const Bit32u useCutoffVal) {
  if (!active) {
    return;
  }
  ...
  // The 240 cutoffVal limit was determined via sample analysis (internal Munt
  // capture IDs: glop3, glop4). More research is needed to be sure that this is
  // correct, however.
  cutoffVal =
      (useCutoffVal > MAX_CUTOFF_VALUE) ? MAX_CUTOFF_VALUE : useCutoffVal;
...
void LA32WaveGenerator::deactivate() { active = false; }

bool LA32WaveGenerator::isActive() const { return active; }

bool LA32WaveGenerator::isPCMWave() const { return pcmWaveAddress != NULL; }
...
void LA32IntPartialPair::initSynth(const PairType useMaster,
...
void LA32IntPartialPair::initPCM(const PairType useMaster,
...
void LA32IntPartialPair::generateNextSample(const PairType useMaster,
                                            const Bit32u amp,
                                            const Bit16u pitch,
                                            const Bit32u cutoff) {
  if (useMaster == MASTER) {
...
Bit16s LA32IntPartialPair::unlogAndMixWGOutput(const LA32WaveGenerator &wg) {
  ...
  if (wg.isPCMWave()) {
    return Bit16s(firstSample + (((Bit32s(secondSample) - Bit32s(firstSample)) *
                                  wg.getPCMInterpolationFactor()) >>
                                 7));
  }
...
static inline Bit16s produceDistortedSample(Bit16s sample) {
  return ((sample & 0x2000) == 0) ? Bit16s(sample & 0x1fff)
                                  : Bit16s(sample | ~0x1fff);
}
...
Bit16s LA32IntPartialPair::nextOutSample() {
  ...
  Bit16s masterSample = unlogAndMixWGOutput(
      master); // Store master partial sample for further mixing

  /* SEMI-CONFIRMED from sample analysis:
   * We observe that for partial structures with ring modulation the
   * interpolation is not applied to the slave PCM partial. It's assumed that
   * the multiplication circuitry intended to perform the interpolation on the
   * slave PCM partial is borrowed by the ring modulation circuit (or the LA32
   * chip has a similar lack of resources assigned to each partial pair).
   */
  Bit16s slaveSample = slave.isPCMWave()
                           ? LA32Utilites::unlog(slave.getOutputLogSample(true))
                           : unlogAndMixWGOutput(slave);

  /* SEMI-CONFIRMED: Ring modulation model derived from sample analysis of
   * specially constructed patches which exploit distortion. LA32 ring modulator
   * found to produce distorted output in case if the absolute value of maximal
   * amplitude of one of the input partials exceeds 8191. This is easy to
   * reproduce using synth partials with resonance values close to the maximum.
   * It looks like an integer overflow happens in this case. As the distortion
   * is strictly bound to the amplitude of the complete mixed square + resonance
   * wave in the linear space, it is reasonable to assume the ring modulation is
   * performed also in the linear space by sample multiplication. Most probably
   * the overflow is caused by limited precision of the multiplication circuit
   * as the very similar distortion occurs with panning.
   */
  Bit16s ringModulatedSample =
      Bit16s((Bit32s(produceDistortedSample(masterSample)) *
              Bit32s(produceDistortedSample(slaveSample))) >>
             13);
