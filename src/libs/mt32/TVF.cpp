...
enum {
  ...
  // When this is the target phase, immediately goes to PHASE_RELEASE unless the
  // poly is set to sustain. Otherwise level[3] is continued with increment 0 -
  // no phase change will occur until some external influence (like pedal
  // release)
  PHASE_SUSTAIN = 5,

  // 0 is targeted within time[4] (the time calculation is quite different from
  // the other phases)
  PHASE_RELEASE = 6,

  // 0 is targeted with increment 0 (thus theoretically staying that way
  // forever)
  PHASE_DONE = 7
};

static int calcBaseCutoff(const TimbreParam::PartialParam *partialParam,
                          Bit32u basePitch, unsigned int key,
                          bool quirkTVFBaseCutoffLimit) {
  // This table matches the values used by a real LAPC-I.
  static const Bit8s biasLevelToBiasMult[] = {85, 42, 21,  16,  10,  5,   2,  0,
                                              -2, -5, -10, -16, -21, -74, -85};
  // These values represent unique options with no consistent pattern, so we
  // have to use something like a table in any case. The table entries, when
  // divided by 21, match approximately what the manual claims: -1, -1/2, -1/4,
  // 0, 1/8, 1/4, 3/8, 1/2, 5/8, 3/4, 7/8, 1, 5/4, 3/2, 2, s1, s2 Note that the
  // entry for 1/8 is rounded to 2 (from 1/8 * 21 = 2.625), which seems
  // strangely inaccurate compared to the others.
  static const Bit8s keyfollowMult21[] = {-21, -10, -5, 0,  2,  5,  8,  10, 13,
                                          16,  18,  21, 26, 32, 42, 21, 21};
  int baseCutoff = keyfollowMult21[partialParam->tvf.keyfollow] -
                   keyfollowMult21[partialParam->wg.pitchKeyfollow];
  ...
  if ((biasPoint & 0x40) == 0) {
    ...
    if (bias > 0) {
      ...
      baseCutoff +=
          bias *
          biasLevelToBiasMult
              [partialParam->tvf.biasLevel]; // Calculation range: -7140 to 7140
    ...
    if (bias < 0) {
      baseCutoff +=
          bias *
          biasLevelToBiasMult
              [partialParam->tvf.biasLevel]; // Calculation range: -6375 to 6375
  ...
  baseCutoff += 2056;
  baseCutoff >>= 4; // PORTABILITY NOTE: Hmm... Depends whether it could've been
                    // below -2056, but maybe arithmetic shift assumed?
  if (baseCutoff > 255) {
    baseCutoff = 255;
  }
...
void TVF::startRamp(Bit8u newTarget, Bit8u newIncrement, int newPhase) {
  ...
  partial->getSynth()->printDebug(
      "[+%lu] [Partial %d] TVF,ramp,%x,%s%x,%d", partial->debugGetSampleNum(),
      partial->debugGetPartialNum(), newTarget,
      (newIncrement & 0x80) ? "-" : "+", (newIncrement & 0x7F), newPhase);
#endif
}

void TVF::reset(const TimbreParam::PartialParam *newPartialParam,
  ...
  baseCutoff = calcBaseCutoff(
      newPartialParam, basePitch, key,
      partial->getSynth()->controlROMFeatures->quirkTVFBaseCutoffLimit);
#if MT32EMU_MONITOR_TVF >= 1
  partial->getSynth()->printDebug("[+%lu] [Partial %d] TVF,base,%d",
                                  partial->debugGetSampleNum(),
                                  partial->debugGetPartialNum(), baseCutoff);
#endif
  ...
  newLevelMult +=
      (signed(key) - 60) >> (4 - newPartialParam->tvf.envDepthKeyfollow);
  if (newLevelMult < 0) {
    newLevelMult = 0;
  }
  ...
  if (newPartialParam->tvf.envTimeKeyfollow != 0) {
    keyTimeSubtraction =
        (signed(key) - 60) >> (5 - newPartialParam->tvf.envTimeKeyfollow);
  } else {
...
Bit8u TVF::getBaseCutoff() const { return baseCutoff; }

void TVF::handleInterrupt() { nextPhase(); }
...
void TVF::nextPhase() {
  ...
  case PHASE_SUSTAIN:
  case PHASE_RELEASE:
    // FIXME: Afaict newPhase should never be PHASE_RELEASE here. And if it
    // were, this is an odd way to handle it.
    if (!partial->getPoly()->canSustain()) {
      phase = newPhase; // FIXME: Correct?
      startDecay(); // FIXME: This should actually start decay even if phase is
                    // already 6. Does that matter?
      return;
    }
  ...
  int envTimeSetting =
      partialParam->tvf.envTime[envPointIndex] - keyTimeSubtraction;
  ...
  if (envTimeSetting > 0) {
    ...
    newIncrement =
        tables
            ->envLogarithmicTime[targetDelta < 0 ? -targetDelta : targetDelta] -
        envTimeSetting;
    if (newIncrement <= 0) {
      newIncrement = 1;
    }
