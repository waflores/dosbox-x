...
bool Network_SetNamedPipeState(uint16_t entry, uint16_t mask,
                               uint16_t &errorcode) {
  ...
  if (SetNamedPipeHandleState(hNamedPipe, &dwMode, NULL, NULL))
    ...
    errorcode = Network_ToDosError(GetLastError());

  return (errorcode == 0);
} // bool	Network_SetNamedPipeState(uint16_t entry,uint16_t mask,uint16_t&
  // errorcode)

bool Network_PeekNamedPipe(uint16_t entry, unsigned char *lpBuffer,
                           uint16_t nBufferSize, uint16_t &cxRead,
                           uint16_t &siLeft, uint16_t &dxLeftCurrent,
                           uint16_t &diStatus, uint16_t &errorcode) {
  int ifd = NetworkHandleList[entry];
  HANDLE hNamedPipe = (HANDLE)_get_osfhandle(ifd);

  DWORD BytesRead, TotalBytesAvail, BytesLeftThisMessage;
  if (PeekNamedPipe(hNamedPipe, lpBuffer, nBufferSize, &BytesRead,
...
bool Network_TranscateNamedPipe(uint16_t entry, unsigned char *lpInBuffer,
                                uint16_t nInBufferSize,
                                unsigned char *lpOutBuffer,
                                uint16_t nOutBufferSize, uint16_t &cxRead,
                                uint16_t &errorcode) {
  int ifd = NetworkHandleList[entry];
  HANDLE hNamedPipe = (HANDLE)_get_osfhandle(ifd);
  ...
  if (TransactNamedPipe(hNamedPipe, lpInBuffer, nInBufferSize, lpOutBuffer,
                        nOutBufferSize, &BytesRead, NULL)) {
    cxRead = (uint16_t)BytesRead;
    errorcode = 0;
  } else
    errorcode = Network_ToDosError(GetLastError());
