...
int clock_gettime(clockid_t clk_id, struct timespec *tp) {
  ...
  case CLOCK_PROCESS_CPUTIME_ID: {
    ...
    uint64_t usec =
        info.user_time.seconds * 1000000 + info.user_time.microseconds +
...
/* This is the LFN filefind handle that is currently being used, with normal
 * values between 0 and 254 for LFN calls. The value LFN_FILEFIND_INTERNAL and
 * LFN_FILEFIND_IMG are used internally by LFN and image handling functions. For
 * non-LFN calls the value is fixed to be LFN_FILEFIND_NONE as defined in
 * drives.h. */
int lfn_filefind_handle = LFN_FILEFIND_NONE;
bool isDBCSCP(), isKanji1(uint8_t chr), shiftjis_lead_byte(int c);

bool DOS_GetFileAttrEx(char const *const name, struct stat *status,
                       uint8_t hdrive) {
...
uint8_t DOS_GetDefaultDrive(void) {
  ...
  if (d != dos.current_drive)
    LOG(LOG_DOSMISC,
        LOG_ERROR)("SDA drive %d not the same as dos.current_drive %d", d,
                   dos.current_drive);
...
bool DOS_MakeName(char const *const name, char *const fullname, uint8_t *drive,
  ...
  if (isVolume) {
  ...
  } else if (strlen(names) == 14 && name_int[1] == ':' && name_int[2] != '\\' &&
             name_int[9] == ' ' && name_int[10] == '.') {
    for (unsigned int i = 0; i < strlen(names); i++)
      if (i < 10 && name_int[i] == 32) {
        ...
        name_int[i] = toupper(name_int[i]);
#if defined(WIN32) &&                                                          \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  } else if (Network_IsNetworkResource(name)) {
    unsigned int j = 0, k = name[0] == '"' ? 1 : 0;
    if (strlen(name) == 2 + k || name[2 + k] == '*' || name[2 + k] == '?' ||
        name[2 + k] == '\\' || (strlen(name) == 3 + k && name[2 + k] == '"')) {
  ...
  *drive = DOS_GetDefaultDrive();
  ...
  while (r < DOS_PATHLENGTH && name_int[r] != 0) {
    ...
    if (c == '/')
      ...
      if ((c >= 'a') && (c <= 'z'))
        ...
        continue; /* should be separator */
    }
    upname[w++] = (char)c;
    if (((IS_PC98_ARCH && shiftjis_lead_byte(c)) ||
         (isDBCSCP() && isKanji1(c))) &&
        r < DOS_PATHLENGTH) {
      /* The trailing byte is NOT ASCII and SHOULD NOT be converted to uppercase
       * like ASCII */
      upname[w++] = name_int[r++];
    }
  }
  ...
  while (fullname[t] != 0) {
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(fullname[t])) ||
             (isDBCSCP() && isKanji1(fullname[t])))
      lead = true;
    else if ((fullname[t] == '\\') && (fullname[t + 1] != 0))
      lastdir = t;
    t++;
  }
  r = 0;
  ...
  while (!stop) {
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(upname[r])) ||
             (isDBCSCP() && isKanji1(upname[r])))
      lead = true;
...
bool DOS_GetSFNPath(char const *const path, char *SFNPath, bool LFN) {
    ...
    *s = 0;
    ...
    if (!strrchr(p, '*') && !strrchr(p, '?')) {
      *s = '\\';
      ...
      if (DOS_FindFirst(pdir, 0xffff & DOS_ATTR_DIRECTORY & ~DOS_ATTR_VOLUME,
                        false)) {
  ...
  if (p) {
    ...
    lfn_filefind_handle = LFN_FILEFIND_INTERNAL;
    if (!strrchr(p, '*') && !strrchr(p, '?') &&
        DOS_FindFirst(pdir, 0xffff & ~DOS_ATTR_VOLUME, false)) {
      dta.GetResult(name, lname, size, hsize, date, time, attr);
      strcat(SFNPath, name);
      strcat(LFNPath, lname);
    } else if (checkwat) {
...
bool DOS_RemoveDir(char const *const dir) {
  ...
  if (strcasecmp(currdir, fulldir) == 0 ||
      (uselfn && strcasecmp(lcurrdir, fulldir) == 0)) {
...
static bool PathExists(char const *const name) {
  ...
  *lead = 0;
  uint8_t drive;
  char fulldir[DOS_PATHLENGTH];
  if (!DOS_MakeName(temp, fulldir, &drive))
    return false;
  if (!Drives[drive]->TestDir(fulldir))
    return false;
  return true;
...
bool DOS_Rename(char const *const oldname, char const *const newname) {
  ...
  if (!DOS_MakeName(newname, fullnew, &drivenew))
    return false;
  while (strlen(fullnew) && (*(fullnew + strlen(fullnew) - 1) == '.' ||
                             *(fullnew + strlen(fullnew) - 1) == ' '))
    *(fullnew + strlen(fullnew) - 1) = 0;
#if defined(WIN32) &&                                                          \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (Network_IsNetworkResource(oldname))
    return Network_Rename(oldname, newname);
#endif
  ...
  if ((DOS_FindDevice(oldname) != DOS_DEVICES) ||
    ...
    if (!control->SecureMode() && (dos_clipboard_device_access == 3 ||
      ...
      if (DOS_FindDevice(oldname) == DOS_DEVICES) {
        ...
        if (find_last == NULL)
          ...
          find_last++;
        if (!strcasecmp(find_last, *dos_clipboard_device_name
  ...
  if (Drives[drivenew]->GetFileAttr(fullnew, &attr) &&
      !(uselfn && !force_sfn && strcmp(fullold, fullnew) &&
        !strcasecmp(fullold, fullnew))) {
    DOS_SetError(DOSERR_ACCESS_DENIED);
    return false;
  }
  ...
  if (clip) {
    ...
    bool failed = false;
    if (DOS_OpenFile(oldname, OPEN_READ, &sourceHandle) &&
        DOS_OpenFile(newname, OPEN_WRITE, &targetHandle)) {
      do {
        if (!DOS_ReadFile(sourceHandle, buffer, &toread) ||
            !DOS_WriteFile(targetHandle, buffer, &toread))
          failed = true;
      } while (toread == 0x8000);
      if (!DOS_CloseFile(sourceHandle) || !DOS_CloseFile(targetHandle))
        failed = true;
    } else
      failed = true;
...
bool DOS_FindFirst(const char *search, uint16_t attr, bool fcb_findfirst) {
  ...
  if (len && check_last_split_char(search, len, '\\') &&
      !((len > 2) && (search[len - 2] == ':') && (attr == DOS_ATTR_VOLUME))) {
    // Dark Forces installer, but c:\ is alright for volume labels(exclusively
    // set)
    DOS_SetError(DOSERR_NO_MORE_FILES);
    return false;
  }
  if (!DOS_MakeName(search, fullsearch, &drive, attr == DOS_ATTR_VOLUME))
    return false;
  // Check for devices. FindDevice checks for leading subdir as well
  bool device = false;
  // if (attr & DOS_ATTR_DEVICE) /* FIX_ME: This line deleted to fix launching
  // Blocek editor (Issue #4385), revert this if this induces other errors */
  ...
  if (net)
    forcelfn = true;
  char *p = net ? strchr_dbcs(fullsearch + (fullsearch[0] == '"' ? 3 : 2), '\\')
                : NULL;
...
#if defined(WIN32) &&                                                          \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (!strlen(dir) && Network_IsNetworkResource(pattern)) {
    forcelfn = false;
    return false;
  }
#endif
  ...
  if (device) {
    ...
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (net)
    return Network_FindFirst(dir, dta);
#endif
...
bool DOS_FindNext(void) {
  ...
  if (uselfn && (i >= DOS_DRIVES || !Drives[i]))
    i = sdrive;
...
#if defined(WIN32) &&                                                          \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  unsigned int pos = lfn_filefind_handle >= LFN_FILEFIND_MAX
                         ? dta.GetDirID()
                         : lfn_id[lfn_filefind_handle];
  if (pos == 65534)
    return Network_FindNext(dta);
#endif
...
bool DOS_ReadFile(uint16_t entry, uint8_t *data, uint16_t *amount, bool fcb) {
  ...
  if (log_fileio) {
    LOG(LOG_FILES, LOG_DEBUG)("Reading %d bytes from %s ", *amount,
                              Files[handle]->name);
  }
  /*
...
bool DOS_WriteFile(uint16_t entry, const uint8_t *data, uint16_t *amount,
                   bool fcb) {
  uint32_t handle = fcb ? entry : RealHandle(entry);
  if (handle >= DOS_FILES) {
    DOS_SetError(DOSERR_INVALID_HANDLE);
  ...
  if (log_fileio) {
    LOG(LOG_FILES, LOG_DEBUG)("Writing %d bytes to %s", *amount,
                              Files[handle]->name);
  }
  /*
...
bool DOS_SeekFile(uint16_t entry, uint32_t *pos, uint32_t type, bool fcb) {
  ...
  if (log_fileio) {
    LOG(LOG_FILES,
        LOG_DEBUG)("Seeking to %d bytes from position type (%d) in %s ", *pos,
                   type, Files[handle]->name);
  }
...
bool DOS_CloseFile(uint16_t entry, bool fcb, uint8_t *refcnt) {
  ...
  if (refcnt != NULL)
    *refcnt = static_cast<uint8_t>(refs + 1);
  return true;
}
...
bool DOS_CreateFile(char const *name, uint16_t attributes, uint16_t *entry,
                    bool fcb) {
  ...
  LOG(LOG_FILES, LOG_NORMAL)("file create attributes %X file %s", attributes,
                             name);
  char fullname[DOS_PATHLENGTH];
  uint8_t drive;
  DOS_PSP psp(dos.psp());
  ...
  uint8_t handle = (uint8_t)DOS_FILES;
  uint8_t i;
  ...
  *entry = fcb ? handle : psp.FindFreeFileEntry();
  ...
  bool foundit =
      Drives[drive]->FileCreate(&Files[handle], fullname, attributes);
  if (foundit) {
    if (Files[handle]) {
    ...
    if (dos.errorcode == DOSERR_ACCESS_DENIED ||
        dos.errorcode == DOSERR_WRITE_PROTECTED)
      return false;
    if (!PathExists(name))
      DOS_SetError(DOSERR_PATH_NOT_FOUND);
    else
...
bool DOS_OpenFile(char const *name, uint8_t flags, uint16_t *entry, bool fcb) {
  ...
  if (flags > 2)
    LOG(LOG_FILES,
        LOG_NORMAL)("Special file open command %X file %s", flags,
                    name); // FIXME: Why? Is there something about special opens
                           // DOSBox doesn't handle properly?
  else
  ...
  *entry = fcb ? handle : psp.FindFreeFileEntry();
  ...
  if (device) {
    ...
    exists = Drives[drive]->FileOpen(&Files[handle], fullname, flags) ||
             Drives[drive]->FileOpen(&Files[handle], upcase(fullname), flags);
    if (exists)
      Files[handle]->SetDrive(drive);
  ...
  if (exists || device) {
    ...
    if ((((flags & 3) != OPEN_READ) ||
        ...
        Drives[drive]->FileExists(fullname))
      DOS_SetError(DOSERR_ACCESS_DENIED);
    else {
      if (!PathExists(name))
        DOS_SetError(DOSERR_PATH_NOT_FOUND);
...
bool DOS_OpenFileExtended(char const *name, uint16_t flags, uint16_t createAttr,
...
bool DOS_UnlinkFile(char const *const name) {
  ...
  if (!DOS_MakeName(name, fullname, &drive))
    ...
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (Network_IsNetworkResource(name))
    return Network_UnlinkFile(name);
#endif
  if (Drives[drive]->FileUnlink(fullname)) {
    return true;
  } else if (uselfn && !force_sfn &&
             (strchr(fullname, '*') ||
              strchr(fullname, '?'))) { // Wildcard delete as used by MS-DOS 7+
                                        // "DEL *.*" in LFN mode
    char dir[DOS_PATHLENGTH], temp[DOS_PATHLENGTH], fname[DOS_PATHLENGTH],
        spath[DOS_PATHLENGTH], pattern[DOS_PATHLENGTH];
    if (!DOS_Canonicalize(name, fullname) || !strlen(fullname)) {
      DOS_SetError(DOSERR_PATH_NOT_FOUND);
      return false;
    }
    if (!strchr(name, '\"') ||
        !DOS_GetSFNPath(("\"" + std::string(fullname) + "\"").c_str(), fname,
                        false))
    ...
    cdirs.clear();
    strcpy(spath, dir);
    if (!DOS_GetSFNPath(dir, spath, false))
      strcpy(spath, dir);
    ...
    bool ret = DOS_FindFirst(
        ((pfull.length() && pfull[0] == '"' ? "" : "\"") + pfull +
         (pfull.length() && pfull[pfull.length() - 1] == '"' ? "" : "\""))
            .c_str(),
        0xffu & ~DOS_ATTR_VOLUME & ~DOS_ATTR_DIRECTORY);
    if (ret)
      do {
        ...
        uint8_t find_attr;
        dta.GetResult(find_name, lfind_name, find_size, find_hsize, find_date,
                      find_time, find_attr);
        if (!(find_attr & DOS_ATTR_DIRECTORY) && strlen(find_name) &&
            !strchr(find_name, '*') && !strchr(find_name, '?')) {
          strcpy(temp, dir);
          if (strlen(temp) && temp[strlen(temp) - 1] != '\\')
            strcat(temp, "\\");
...
// returns false when using on cdrom (stonekeep)
{
  ...
  /* This function must prevent changing a file into a directory, volume label
   * into a file, etc. Also Windows 95 setup likes to create WINBOOT.INI as a
   * file and then chattr it into a directory for some stupid reason. */
  uint16_t old_attr;
#if defined(WIN32) &&                                                          \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if ((Network_IsNetworkResource(name) &&
       !Network_GetFileAttr(name, &old_attr)) ||
      (!Network_IsNetworkResource(name) &&
       !Drives[drive]->GetFileAttr(fullname, &old_attr)))
#else
  ...
  if ((old_attr ^ attr) &
      DOS_ATTR_VOLUME) { /* change in volume label attribute */
  ...
  if ((old_attr ^ attr) &
      DOS_ATTR_DIRECTORY) /* change in directory attribute (ex: Windows 95
                             SETUP.EXE vs WINBOOT.INI) */
    LOG(LOG_DOSMISC, LOG_WARN)(
        "Attempted to change directory attribute of '%s' with SetFileAttr",
        name);
  ...
  attr = (attr & ~attr_mask) | (old_attr & attr_mask);
    ...
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (Network_IsNetworkResource(name))
    return Network_SetFileAttr(name, attr);
#endif
  return Drives[drive]->SetFileAttr(fullname, attr);
}

bool DOS_Canonicalize(char const *const name, char *const big) {
  // TODO Add Better support for devices and shit but will it be needed i doubt
  // it :)
  uint8_t drive;
  char fullname[DOS_PATHLENGTH];
  if (!DOS_MakeName(name, fullname, &drive))
...
/* Common routine to take larger allocation information (such as FAT32) and
 * convert it to values that are suitable for use with older DOS programs that
 * pre-date FAT32 and partitions 2GB or larger. This is what Windows 95 OSR2 and
 * higher do with FAT32 partitions anyway, as documented by Microsoft. */
bool DOS_CommonFAT32FAT16DiskSpaceConv(uint16_t *bytes, uint8_t *sectors,
                                       uint16_t *clusters, uint16_t *free,
                                       const uint32_t bytes32,
                                       const uint32_t sectors32,
                                       const uint32_t clusters32,
                                       const uint32_t free32) {
  uint32_t cdiv = 1;

  if (sectors32 > 128 || bytes32 > 0x8000)
    return false;

  /* This function is for the old API. It is necessary to adjust the values so
   * that they never overflow 16-bit unsigned integers and never multiply out to
   * a number greater than just under 2GB. Because old DOS programs use 32-bit
   * signed integers for disk total/free and FAT12/FAT16 filesystem limitations.
   */
  /* NTS: Make sure (bytes per sector * sectors per cluster) is less than
   * 0x10000, or else FORMAT.COM will crash with divide by zero or produce
   * incorrect results when run with "FORMAT /S" */
  while ((clusters32 > 0xFFFFu || free32 > 0xFFFFu) &&
         (sectors32 * cdiv) <= 64u &&
         (bytes32 * sectors32 * cdiv) < 0x8000 /*Needed for FORMAT.COM*/)
    cdiv *= 2u;

  /* The old API must never report more than just under 2GB for total and free
   */
  const uint32_t clust2gb =
      (uint32_t)0x7FFF8000ul / (uint32_t)bytes32 / (sectors32 * cdiv);
...
bool DOS_GetFreeDiskSpace(uint8_t drive, uint16_t *bytes, uint8_t *sectors,
...

  {
    ...
    if ((dos.version.major > 7 ||
        ...
        Drives[drive]->AllocationInfo32(&bytes32, &sectors32, &clusters32,
                                        &free32) &&
        DOS_CommonFAT32FAT16DiskSpaceConv(bytes, sectors, clusters, free,
                                          bytes32, sectors32, clusters32,
                                          free32))
      return true;
  }
...
bool DOS_GetFreeDiskSpace32(uint8_t drive, uint32_t *bytes, uint32_t *sectors,
                            uint32_t *clusters, uint32_t *free) {
  if (drive == 0)
    drive = DOS_GetDefaultDrive();
  else
    drive--;
  ...
  if ((dos.version.major > 7 ||
       (dos.version.major == 7 && dos.version.minor >= 10)) &&
      Drives[drive]->AllocationInfo32(bytes, sectors, clusters, free))
    return true;

  {
    uint8_t sectors8;
    uint16_t bytes16, clusters16, free16;
    if (Drives[drive]->AllocationInfo(&bytes16, &sectors8, &clusters16,
                                      &free16)) {
      *free = free16;
      *bytes = bytes16;
      *sectors = sectors8;
...
bool DOS_CreateTempFile(char *const name, uint16_t *entry) {
  ...
  if (namelen == 0) {
    ...
    if (!check_last_split_char(name, namelen, '\\') &&
        (name[namelen - 1] != '/')) {
      tempname[0] = '\\';
      tempname++;
    }
  ...
  do {
    ...
    tempname[8] = 0;
    // if (DOS_FileExists(name)) {cont=true;continue;} // FIXME: Check name
    // uniqueness
...
char DOS_ToUpper(char c) {
  ...
  if (uc > 0x60 && uc < 0x7B)
    ...
    const unsigned char t[0x25] = {
        0x00, 0x9a, 0x45, 0x41, 0x8E, 0x41, 0x8F, 0x80, 0x45, 0x45,
        0x45, 0x49, 0x49, 0x49, 0x00, 0x00, 0x00, 0x92, 0x00, 0x4F,
        0x99, 0x4F, 0x55, 0x55, 0x59, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x41, 0x49, 0x4F, 0x55, 0xA5};
    if (t[uc - 0x80])
      uc = t[uc - 0x80];
  }
...
#pragma pack()
  ...
  if (string[1] == ':') {
    ...
    if (!isvalid(ascii_toupper(d))) {
      string += 2;
      goto savefcb;
    } // TODO check (for ret value)
    fcb_name.part.drive[0] = 0;
    hasdrive = true;
    if (isalpha(d) && Drives[ascii_toupper(d) -
                             'A']) { // Floppies under dos always exist, but
                                     // don't bother with that at this level
      ; // THIS* was here
    } else
      ret = 0xff;
    fcb_name.part.drive[0] =
        DOS_ToUpper(string[0]) - 'A' +
        1; // Always do THIS* and continue parsing, just return the right code
    string += 2;
  }
  ...
  while (true) {
    ...
    if ((IS_PC98_ARCH && shiftjis_lead_byte(nc)) ||
      ...
      char ncs = (char)ascii_toupper(nc); // Should use DOS_ToUpper, but then
                                          // more calls need to be changed.
      if (ncs == '*') { // Handle *
        fill = '?';
        ncs = '?';
      }
      if (ncs == '?' && !ret && index < 8)
        ret = 1;           // Don't override bad drive
...
savefcb:
  ...
  fcb.SetName((unsigned char)fcb_name.part.drive[0], fcb_name.part.name,
              fcb_name.part.ext);
...
static void DTAExtendNameVolumeLabel(const char *const name,
...
bool DOS_FCBCreate(uint16_t seg, uint16_t offset) {
  ...
  fcb.GetAttr(attr);
  if (!attr)
    attr = DOS_ATTR_ARCHIVE; // Better safe than sorry
...
bool DOS_FCBOpen(uint16_t seg, uint16_t offset) {
  ...
  if (strpbrk(shortname, "*?")) {
    ...
    char name[DOS_NAMELENGTH_ASCII], lname[LFN_NAMELENGTH], file_name[9],
        ext[4];
    uint32_t size, hsize;
    uint16_t date, time;
    uint8_t attr;
...
uint8_t DOS_FCBRead(uint16_t seg, uint16_t offset, uint16_t recno) {
  ...
  if (!DOS_SeekFile(fhandle, &pos, DOS_SEEK_SET, true))
    return FCB_READ_NODATA;
  uint16_t toread = rec_size;
  if (!DOS_ReadFile(fhandle, dos_copybuf, &toread, true))
    return FCB_READ_NODATA;
  if (toread == 0)
    return FCB_READ_NODATA;
  ...
  MEM_BlockWrite(Real2Phys(dos.dta()) + (PhysPt)(recno * rec_size), dos_copybuf,
                 rec_size);
  if (++cur_rec > 127u) {
    cur_block++;
    cur_rec = 0;
...
uint8_t DOS_FCBWrite(uint16_t seg, uint16_t offset, uint16_t recno) {
  ...
  MEM_BlockRead(Real2Phys(dos.dta()) + (PhysPt)(recno * rec_size), dos_copybuf,
                rec_size);
  uint16_t towrite = rec_size;
  if (!DOS_WriteFile(fhandle, dos_copybuf, &towrite, true))
    return FCB_ERR_WRITE;
  uint32_t size;
  uint16_t date, time;
...
uint8_t DOS_FCBRandomRead(uint16_t seg, uint16_t offset, uint16_t *numRec,
                          bool restore) {
   ...
   * before the read!!!!!!!!! and the random data is not updated! (user must do
   * this) Random block read updates these fields to reflect the state after the
   * read!
   */
  DOS_FCB fcb(seg, offset);
  uint32_t random;
  ...
  *numRec = count;
  ...
  /* Update the random record pointer with new position only when restore is
   * false*/
  if (!restore)
    fcb.SetRandom(new_block * 128u + new_rec);
  return error;
}

uint8_t DOS_FCBRandomWrite(uint16_t seg, uint16_t offset, uint16_t *numRec,
                           bool restore) {
  /* see FCB_RandomRead */
  DOS_FCB fcb(seg, offset);
  uint32_t random;
  ...
  if (*numRec > 0) {
    ...
    /* Write records */
    for (count = 0; count < *numRec; count++) {
      error = DOS_FCBWrite(seg, offset,
                           count); // dos_fcbwrite return 0 false when true...
      if (error != FCB_SUCCESS)
        break;
    }
    *numRec = count;
...
bool DOS_FCBGetFileSize(uint16_t seg, uint16_t offset) {
  ...
  DOS_FCB fcb(seg, offset);
  fcb.GetName(shortname);
  if (!DOS_OpenFile(shortname, OPEN_READ, &entry, true))
    return false;
  uint32_t size = 0;
  ...
  if (rec_size == 0)
    rec_size = 128; // Use default if missing.
  ...
  /* Special case: ????????.??? and DOS_ATTR_VOLUME */
  {
    ...
    if (attr & DOS_ATTR_VOLUME) {
      ...
      if (!strcmp(shortname, "???????????")) {
        ...
        /* MS-DOS 6.22 LABEL.EXE will explicitly request to delete the volume
         * label by the volume label not ????????.???? */
        if (!label.empty()) {
      ...
      LOG(LOG_DOSMISC, LOG_NORMAL)(
          "FCB delete volume label not found (current='%s' asking='%s')",
          label.c_str(), shortname);
...
bool DOS_FCBRenameFile(uint16_t seg, uint16_t offset) {
  ...
  fcbnew.SetPtPhys(fcbnew.GetPtPhys() +
                   0x10u); // HACK: FCB NEW memory offset is affected by whether
                           // FCB OLD is extended
  if (!fcbold.Valid())
    return false;
...

  {
    ...
    fcbold.GetAttr(attr);
    /* According to RBIL and confirmed with SETLABEL.ASM in DOSLIB2, you can
     * rename a volume label dirent as well with this function */
    if (attr & DOS_ATTR_VOLUME) {
      ...
      if (!label.empty()) {
        if (!strcmp(oldname, "???????????") || label == oldname) {
          ...
          LOG(LOG_DOSMISC, LOG_NORMAL)(
              "FCB rename volume label to '%s' from '%s'", newname, oldname);
          return true;
        } else {
          LOG(LOG_DOSMISC, LOG_NORMAL)("FCB rename volume label rejected, does "
                                       "not match current label '%s' from '%s'",
                                       newname, oldname);
          DOS_SetError(DOSERR_FILE_NOT_FOUND); // right?
          return false;
        }
        ...
        LOG(LOG_DOSMISC,
            LOG_NORMAL)("FCB rename volume label rejected, no label set");
        DOS_SetError(DOSERR_FILE_NOT_FOUND); // right?
        return false;
      }
    }
  ...
  /* Check, if sourcefile is still open. This was possible in DOS, but modern
   * oses don't like this */
  uint8_t drive;
  char fullname[DOS_PATHLENGTH];
...
bool DOS_FileExists(char const *const name) {
  ...
  if (!DOS_MakeName(name, fullname, &drive))
    ...
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
  if (Network_IsNetworkResource(name))
    return Network_FileExists(fullname);
#endif
...
bool DOS_GetAllocationInfo(uint8_t drive, uint16_t *_bytes_sector,
  ...
  Drives[drive]->AllocationInfo(_bytes_sector, _sectors_cluster,
                                _total_clusters, &_free_clusters);
...
void POD_Save_DOS_Files(std::ostream &stream) {
  ...
  if (!dos_kernel_disabled) {
    ...
    for (int i = 2; i < DOS_DRIVES + 2; i++) {
      ...
      *overlaydir = 0;
      if (!strncmp(dinfo, "local directory ", 16) ||
          !strncmp(dinfo, "CDRom ", 6) ||
          !strncmp(dinfo, "PhysFS directory ", 17) ||
          !strncmp(dinfo, "PhysFS CDRom ", 13)) {
        localDrive *ldp = dynamic_cast<localDrive *>(Drives[lcv]);
        if (!ldp)
          ldp = dynamic_cast<cdromDrive *>(Drives[lcv]);
        if (!ldp)
          ldp = dynamic_cast<physfsDrive *>(Drives[lcv]);
        if (ldp) {
        ...
        if (fdp) {
          opts.bytesector = fdp->loadedDisk ? fdp->loadedDisk->sector_size
                                            : fdp->opts.bytesector;
          opts.cylsector =
              fdp->loadedDisk ? fdp->loadedDisk->sectors : fdp->opts.cylsector;
          opts.headscyl =
              fdp->loadedDisk ? fdp->loadedDisk->heads : fdp->opts.headscyl;
          opts.cylinders = fdp->loadedDisk ? fdp->loadedDisk->cylinders
                                           : fdp->opts.cylinders;
    ...
    for (unsigned int lcv = 0; lcv < DOS_FILES; lcv++) {
      ...
      if (!Files[lcv] || Files[lcv]->GetName() == NULL)
        ...
        if (strcmp(Files[lcv]->GetName(), "NUL") == 0)
          file_valid = 0xfe; // earth 2140 needs this
        if (strcmp(Files[lcv]->GetName(), "CON") == 0)
          file_valid = 0xfe;
        if (strcmp(Files[lcv]->GetName(), "LPT1") == 0)
          file_valid = 0xfe;
        if (strcmp(Files[lcv]->GetName(), "PRN") == 0)
          file_valid = 0xfe;
        if (strcmp(Files[lcv]->GetName(), "AUX") == 0)
          file_valid = 0xfe;
        if (strcmp(Files[lcv]->GetName(), "EMMXXXX0") == 0)
          file_valid = 0xfe; // raiden needs this
      }
  ...
  for (int i = 2; i < MAX_DISK_IMAGES + 2; i++) {
    ...
    if (!valid)
      continue;
    char diskname[256];
    imageDiskElToritoFloppy *image =
        dynamic_cast<imageDiskElToritoFloppy *>(imageDiskList[d]);
    strcpy(diskname, image != NULL ? "El Torito floppy drive"
                                   : imageDiskList[d]->diskname.c_str());
    ...
    if (image != NULL) {
      ...
      imageDiskMemory *idmem =
          dynamic_cast<imageDiskMemory *>(imageDiskList[d]);
...
void POD_Load_DOS_Files(std::istream &stream) {
  ...
  if (!dos_kernel_disabled) {
    ...
    for (int i = 2; i < DOS_DRIVES + 2; i++) {
      ...
      if (Drives[lcv] && strcasecmp(Drives[lcv]->info, dinfo) &&
          (!strncmp(dinfo, "local directory ", 16) ||
           !strncmp(dinfo, "CDRom ", 6) ||
           !strncmp(dinfo, "PhysFS directory ", 17) ||
           !strncmp(dinfo, "PhysFS CDRom ", 13) ||
           (!strncmp(dinfo, "isoDrive ", 9) ||
            !strncmp(dinfo, "fatDrive ", 9))))
        unmount(lcv);
      if (!Drives[lcv]) {
        std::vector<std::string> options;
        if (!strncmp(dinfo, "local directory ", 16)) {
          Drives[lcv] =
              new localDrive(dinfo + 16, lalloc.bytes_sector,
                             lalloc.sectors_cluster, lalloc.total_clusters,
                             lalloc.free_clusters, lalloc.mediaid, options);
          if (Drives[lcv]) {
            DOS_EnableDriveMenu('A' + lcv);
            mem_writeb(Real2Phys(dos.tables.mediaid) +
                           lcv * dos.tables.dpb_size,
                       lalloc.mediaid);
            if (strlen(overlaydir)) {
              uint8_t error = 0;
              Drives[lcv] = new Overlay_Drive(
                  dynamic_cast<localDrive *>(Drives[lcv])->getBasedir(),
                  overlaydir, oalloc.bytes_sector, oalloc.sectors_cluster,
                  oalloc.total_clusters, oalloc.free_clusters, oalloc.mediaid,
                  error, options);
            }
          } else
          ...
          if (!strncmp(dinfo, "PhysFS CDRom ", 13)) {
            ...
            Drives[lcv] = new physfscdromDrive(
                'A' + lcv, (":" + str + "\\").c_str(), lalloc.bytes_sector,
                lalloc.sectors_cluster, lalloc.total_clusters, 0,
                lalloc.mediaid, error, options);
          } else
            Drives[lcv] = new cdromDrive(
                'A' + lcv, dinfo + 6, lalloc.bytes_sector,
                lalloc.sectors_cluster, lalloc.total_clusters,
                lalloc.free_clusters, lalloc.mediaid, error, options);
          if (Drives[lcv]) {
            DOS_EnableDriveMenu('A' + lcv);
            mem_writeb(Real2Phys(dos.tables.mediaid) +
                           lcv * dos.tables.dpb_size,
          ...
          if (found != std::string::npos)
            str = str.substr(0, found);
          Drives[lcv] = new physfsDrive(
              'A' + lcv, (":" + str + "\\").c_str(), lalloc.bytes_sector,
              lalloc.sectors_cluster, lalloc.total_clusters,
              lalloc.free_clusters, lalloc.mediaid, error, options);
          if (Drives[lcv]) {
            if (strlen(overlaydir))
              dynamic_cast<physfsDrive *>(Drives[lcv])
                  ->setOverlaydir(overlaydir);
            DOS_EnableDriveMenu('A' + lcv);
            mem_writeb(Real2Phys(dos.tables.mediaid) +
                           lcv * dos.tables.dpb_size,
                       lalloc.mediaid);
          ...
          int error = -1;
          isoDrive *newDrive =
              new isoDrive('A' + lcv, dinfo + 9, mediaid, error, options);
          if (!error) {
            Drives[lcv] = newDrive;
          ...
          if (opts.mounttype == 1) {
            imageDisk *newImage = new imageDiskElToritoFloppy(
                (unsigned char)opts.CDROM_drive, opts.cdrom_sector_offset,
                opts.floppy_emu_type);
            if (newImage != NULL) {
              newImage->Addref();
              ...
              LOG_MSG(
                  "Error: Cannot restore drive from El Torito floppy image.\n");

          } else if (opts.mounttype == 2) {
            imageDiskMemory *image =
                CreateRamDrive(sizes, 0, lcv < 2 && sizes[0] == 0, NULL);
            ...
            if (imageDiskVHD::Open(dinfo + 9, false, &vhdImage) ==
                imageDiskVHD::OPEN_SUCCESS)
              newDrive = new fatDrive(vhdImage, options);
            vhdImage = NULL;
          } else if (!opts.mounttype && *(dinfo + 9))
            newDrive = new fatDrive(dinfo + 9, opts.bytesector, opts.cylsector,
                                    opts.headscyl, opts.cylinders, options);
      ...
      if (Drives[lcv])
        Drives[lcv]->LoadState(stream);
    }

    // Alien Carnage - game creates and unlinks temp files
    // Here are two situations
    // 1. Game already unlinked temp file, but information about file is still
    // in Files[] and we saved it. In this case we must only pop old data from
    // stream by loading. This is fixed.
    // 2. Game still did not unlink file, We saved this information. Then was
    // game restarted and temp files were removed. Then we try load save state,
    // but we don't have temp file. This is not fixed
    DOS_File *dummy = NULL;
    ...
    for (int i = 2; i < DOS_DRIVES + 2; i++) {
      ...
      if (!Drives[lcv] && !strncmp(dinfo, "isoDrive ", 9)) {
        ...
        if (!error) {
          ...
          DriveManager::AppendDisk(lcv, newDrive);
          DriveManager::InitializeDrive(lcv);
          mem_writeb(Real2Phys(dos.tables.mediaid) + lcv * dos.tables.dpb_size,
                     mediaid);
          clist.push_back(lcv);
        }
      }
  ...
  for (int i = 2; i < MAX_DISK_IMAGES + 2; i++) {
    ...
    if (!valid) {
      ...
      if (i == MAX_DISK_IMAGES - 1)
        for (auto &it : clist) {
          ...
          if (ide_index >= 0)
            IDE_CDROM_Attach(ide_index, ide_slave, it);
        }
      continue;
    }
    char diskname[256];
    ...
    if (imageDiskList[d] &&
        ((opts.mounttype == 1 &&
          dynamic_cast<imageDiskElToritoFloppy *>(imageDiskList[d]) == NULL) ||
         (opts.mounttype != 1 &&
          strcasecmp(imageDiskList[d]->diskname.c_str(), diskname)))) {
      if (d > 1)
        IDE_Hard_Disk_Detach(d);
      imageDiskList[d]->Release();
      imageDiskList[d] = NULL;
    ...
    if (imageDiskList[d])
      ...
      imageDisk *image = new imageDiskElToritoFloppy(
          (unsigned char)opts.CDROM_drive, opts.cdrom_sector_offset,
          opts.floppy_emu_type);
      if (image)
        AttachToBiosByLetter(image, 'A' + d);
      else
        LOG_MSG("Warning: Cannot restore drive number from El Torito floppy "
                "image.\n");
    } else if (opts.mounttype == 2) {
      imageDiskMemory *image =
          CreateRamDrive(sizes, 0, d < 2 && sizes[0] == 0, NULL);
      if (image != NULL && image->Format() == 0x00)
        AttachToBiosAndIdeByLetter(image, d, (unsigned char)ide_index,
                                   ide_slave);
    } else if (opts.mounttype == 3 && *diskname) {
      imageDisk *image = NULL;
      if (imageDiskVHD::Open(diskname, false, &image) ==
          imageDiskVHD::OPEN_SUCCESS)
        AttachToBiosAndIdeByLetter(image, 'A' + d, (unsigned char)ide_index,
                                   ide_slave);
      else
      ...
      std::vector<std::string> options;
      fatDrive *newDrive =
          new fatDrive(diskname, opts.bytesector, opts.cylsector, opts.headscyl,
                       opts.cylinders, options);
