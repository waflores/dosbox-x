...
char sfn[DOS_NAMELENGTH_ASCII], vfnames[MAX_VFILES][CROSS_LEN],
    vfsnames[MAX_VFILES][DOS_NAMELENGTH_ASCII];
  ...
  if (t > strlen(n) || k == 1 || k == 10 || k == 100 || k == 1000 ||
    ...
    *sfn = 0;
    ...
    while (*n != 0 && *n != '.' && i < m) {
      ...
      if (!lead && ((IS_PC98_ARCH && shiftjis_lead_byte(*n & 0xFF)) ||
    ...
    while (cur_file) {
      if (onpos == cur_file->onpos &&
          (strcasecmp(sfn, cur_file->name) == 0 ||
           (uselfn && strcasecmp(sfn, cur_file->lname) == 0))) {
        found = true;
        break;
      }
...
void VFILE_Register(const char *name, uint8_t *data, uint32_t size,
  ...
  if (strlen(dir) > 2 && dir[0] == '/' && dir[strlen(dir) - 1] == '/') {
    for (unsigned int i = 1; i < vfpos; i++)
      if (!strcasecmp((std::string(vfsnames[i]) + "/").c_str(), dir + 1) ||
          !strcasecmp((std::string(vfnames[i]) + "/").c_str(), dir + 1)) {
        onpos = i;
        break;
      }
    if (onpos == 0)
  ...
  while (cur_file) {
    if (onpos == cur_file->onpos &&
        (strcasecmp(name, cur_file->name) == 0 ||
         (uselfn && strcasecmp(name, cur_file->name) == 0)))
      return;
    cur_file = cur_file->next;
  }
  std::string sname =
      filename_not_strict_8x3(name) ? VFILE_Generate_SFN(name, onpos) : name;
  if (in)
    for (std::string file; in >> file;) {
      ...
      if (!strcasecmp(fullname, file.c_str()) ||
          !strcasecmp(("\"" + std::string(fullname) + "\"").c_str(),
                      file.c_str()) ||
          !strcasecmp(fullsname, file.c_str()) ||
          !strcasecmp(("\"" + std::string(fullsname) + "\"").c_str(),
                      file.c_str()))
        return;
      if (!strcasecmp(("/" + std::string(fullname)).c_str(), file.c_str()) ||
          !strcasecmp(("/\"" + std::string(fullname) + "\"").c_str(),
                      file.c_str()) ||
          !strcasecmp(("/" + std::string(fullsname)).c_str(), file.c_str()) ||
          !strcasecmp(("/\"" + std::string(fullsname) + "\"").c_str(),
                      file.c_str())) {
        hidden = true;
        break;
...
void VFILE_Remove(const char *name, const char *dir = "") {
  ...
  while (chan) {
    if (onpos == chan->onpos &&
        (strcmp(name, chan->name) == 0 || strcmp(name, chan->lname) == 0)) {
      *where = chan->next;
      if (chan == first_file)
...
uint32_t VFILE_GetCPIData(const char *filename, std::vector<uint8_t> &cpibuf) {
  ...
  VFILE_Block *chan = first_file;
  while (chan) {
    if (onpos == chan->onpos && (strcmp(filename, chan->name) == 0 ||
                                 strcmp(filename, chan->lname) == 0)) {
      if (chan->size > 65535)
        return 0;
      for (size_t bct = 0; bct < chan->size; bct++)
        cpibuf.push_back(chan->data[bct]);
      return chan->size;
    }
    chan = chan->next;
  }
...
Virtual_File::Virtual_File(uint8_t *in_data, uint32_t in_size)
    : file_size(in_size), file_data(in_data) {
...
bool Virtual_File::Close() { return true; }
...
bool Virtual_Drive::FileOpen(DOS_File **file, const char *name,
                             uint32_t flags) {
  if (*name == 0) {
  ...
  while (cur_file) {
    ...
    if (strcasecmp(
            name,
            (std::string(onpos ? vfsnames[onpos] + std::string(1, '\\') : "") +
             cur_file->name)
                .c_str()) == 0 ||
        (uselfn &&
         (strcasecmp(name,
          ...
          strcasecmp(
              name,
              (std::string(onpos ? vfnames[onpos] + std::string(1, '\\') : "") +
               cur_file->name)
                  .c_str()) == 0 ||
          strcasecmp(
              name,
...
bool Virtual_Drive::TestDir(const char *dir) {
  ...
  while (cur_file) {
    if (cur_file->isdir &&
        (!strcasecmp(cur_file->name, dir) || !strcasecmp(cur_file->lname, dir)))
      return true;
    cur_file = cur_file->next;
  }
  return false;
...
bool Virtual_Drive::FileStat(const char *name,
  ...
  while (cur_file) {
    ...
    if (strcasecmp(
      ...
      stat_block->attr =
          cur_file->isdir
              ? (cur_file->hidden ? DOS_ATTR_DIRECTORY | DOS_ATTR_HIDDEN
                                  : DOS_ATTR_DIRECTORY)
              : (cur_file->hidden ? DOS_ATTR_ARCHIVE | DOS_ATTR_HIDDEN
                                  : DOS_ATTR_ARCHIVE);
...
bool Virtual_Drive::FindFirst(const char *_dir, DOS_DTA &dta,
                              bool fcb_findfirst) {
  unsigned int onpos = 0;
  if (*_dir) {
    if (FileExists(_dir)) {
  ...
  if (lfn_filefind_handle >= LFN_FILEFIND_MAX) {
    dta.SetDirID(onpos);
    search_file =
        (attr & DOS_ATTR_DIRECTORY) && onpos > 0 ? parent_dir : first_file;
  } else {
    lfn_id[lfn_filefind_handle] = onpos;
    lfn_search[lfn_filefind_handle] =
        (attr & DOS_ATTR_DIRECTORY) && onpos > 0 ? parent_dir : first_file;
  }
  if (attr == DOS_ATTR_VOLUME) {
    ...
    if (WildFileCmp(".", pattern)) {
      dta.SetResult(".", ".", 0, 0, DOS_PackDate(2002, 10, 1),
...
bool Virtual_Drive::FindNext(DOS_DTA &dta) {
  ...
  if ((lfn_filefind_handle >= LFN_FILEFIND_MAX && search_file == parent_dir) ||
      (lfn_filefind_handle < LFN_FILEFIND_MAX &&
       lfn_search[lfn_filefind_handle] == parent_dir)) {
  ...
  if (lfn_filefind_handle >= LFN_FILEFIND_MAX)
    while (search_file) {
      if (!(skipintprog && search_file->intprog) && pos == search_file->onpos &&
          ((attr & DOS_ATTR_DIRECTORY) || !search_file->isdir) &&
          (WildFileCmp(search_file->name, pattern) ||
           LWildFileCmp(search_file->lname, pattern))) {
        dta.SetResult(
            search_file->name, search_file->lname, search_file->size, 0,
            search_file->date, search_file->time,
            search_file->isdir
                ? (search_file->hidden ? DOS_ATTR_DIRECTORY | DOS_ATTR_HIDDEN
                                       : DOS_ATTR_DIRECTORY)
                : (search_file->hidden ? DOS_ATTR_ARCHIVE | DOS_ATTR_HIDDEN
                                       : DOS_ATTR_ARCHIVE));
    ...
    while (lfn_search[lfn_filefind_handle]) {
      if (!(skipintprog && search_file->intprog) &&
          pos == lfn_search[lfn_filefind_handle]->onpos &&
          ((attr & DOS_ATTR_DIRECTORY) ||
           !lfn_search[lfn_filefind_handle]->isdir) &&
          (WildFileCmp(lfn_search[lfn_filefind_handle]->name, pattern) ||
           LWildFileCmp(lfn_search[lfn_filefind_handle]->lname, pattern))) {
        dta.SetResult(lfn_search[lfn_filefind_handle]->name,
                      lfn_search[lfn_filefind_handle]->lname,
                      lfn_search[lfn_filefind_handle]->size, 0,
                      lfn_search[lfn_filefind_handle]->date,
                      lfn_search[lfn_filefind_handle]->time,
                      lfn_search[lfn_filefind_handle]->isdir
                          ? (lfn_search[lfn_filefind_handle]->hidden
                                 ? DOS_ATTR_DIRECTORY | DOS_ATTR_HIDDEN
                                 : DOS_ATTR_DIRECTORY)
                          : (lfn_search[lfn_filefind_handle]->hidden
                                 ? DOS_ATTR_ARCHIVE | DOS_ATTR_HIDDEN
                                 : DOS_ATTR_ARCHIVE));
    ...
    if (strcasecmp(
      ...
      *attr = cur_file->isdir
                  ? (cur_file->hidden ? DOS_ATTR_DIRECTORY | DOS_ATTR_HIDDEN
                                      : DOS_ATTR_DIRECTORY)
                  : (cur_file->hidden ? DOS_ATTR_ARCHIVE | DOS_ATTR_HIDDEN
                                      : DOS_ATTR_ARCHIVE); // Maybe readonly ?
      return true;
    }
    cur_file = cur_file->next;
  }
...
bool Virtual_Drive::AllocationInfo(uint16_t *_bytes_sector,
...
uint8_t Virtual_Drive::GetMediaByte(void) { return 0xF8; }
...
bool Virtual_Drive::isRemovable(void) { return false; }

Bits Virtual_Drive::UnMount(void) { return 1; }

char const *Virtual_Drive::GetLabel(void) { return "DOSBOX-X"; }
