...
#define REDBOOK_FRAME_PADDING                                                  \
  150u // The relationship between High Sierra sectors and Redbook
...
//!
//! \description CD-ROM time is represented as minutes, seconds, and frames (75
//! per second)
typedef struct SMSF {
...
inline constexpr T1 ceil_udivide(const T1 x, const T2 y) noexcept {
  static_assert(std::is_unsigned<T1>::value,
                "First parameter should be unsigned");
  static_assert(std::is_unsigned<T2>::value,
                "Second parameter should be unsigned");
...
//!
//! \brief This provides the base C++ class for a CD-ROM interface in CD-ROM
//! emulation
class CDROM_Interface {
public:
  enum INTERFACE_TYPE { ID_BASE = 0, ID_FAKE, ID_IMAGE };

  //	CDROM_Interface						(void);
  virtual ~CDROM_Interface(void) {};

  //! \brief Set the device associated with this interface, if supported by
  //! emulation
  ...
  virtual bool GetAudioTrackInfo(int track, TMSF &start,
                                 unsigned char &attr) = 0;

  //! \brief Get subchannel data of the sectors at the current position, and
  ...
  virtual bool GetMediaTrayStatus(bool &mediaPresent, bool &mediaChanged,
                                  bool &trayOpen) = 0;
  ...
  virtual bool ReadSectors(PhysPt buffer, bool raw, unsigned long sector,
                           unsigned long num) = 0;

  //! \brief Read sector data into host memory (for IDE emulation)
  virtual bool ReadSectorsHost(void *buffer, bool raw, unsigned long sector,
                               unsigned long num) = 0;
...
//!
//! \brief This connects CD-ROM emulation to the CD-ROM functions provided by
//! SDL 1.x
...
public:
  ...
  bool ReadSectors(PhysPt /*buffer*/, bool /*raw*/, unsigned long /*sector*/,
                   unsigned long /*num*/) override {
    return false;
  };
  /* This is needed for IDE hack, who's buffer does not exist in DOS physical
   * memory */
...
public:
  ...
  bool GetMediaTrayStatus(bool &mediaPresent, bool &mediaChanged,
                          bool &trayOpen) override;
  bool PlayAudioSector(unsigned long /*start*/,
...
//!
//! \brief This provides CD-ROM emulation from .ISO and .BIN/.CUE images on the
//! host system
...
private:
  ...
  private:
    ...
    */
    uint8_t *hunk_buffer =
        nullptr; // buffer to hold one hunk // size of hunks in CHD up to 1 MiB
  ...
  public:
    bool skip_sync =
        false; // this will fail if a CHD contains 2048 and 2352 sector tracks
  };
...
public:
  ...
  //!
  //! \description images[] is static and not specific to any C++ class
  //! instance.
...
private:
  ...
  bool AddTrack(Track &curr, int &shift, int prestart, int &totalPregap,
                int currPregap);
  // member variables
