...
bool Network_FindNext(DOS_DTA &dta) {
  uint8_t fattr;
  char pattern[CROSS_LEN], sfn[DOS_NAMELENGTH_ASCII];
  ...
  if (hFind != INVALID_HANDLE_VALUE) {
    while (usefdw ? FindNextFileW(hFind, &fdw) : FindNextFile(hFind, &fda)) {
      if ((fattr & DOS_ATTR_DIRECTORY) ||
          !(fdw.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
        FileTimeToSystemTime(
            usefdw ? &fdw.ftLastWriteTime : &fda.ftLastWriteTime, &lt);
        if (SystemTimeToTzSpecificLocalTime(NULL, &lt, &local))
          lt = local;
        if (usefdw && wcslen(fdw.cAlternateFileName) &&
            CodePageHostToGuestUTF16(pattern,
                                     (uint16_t *)fdw.cAlternateFileName))
          name = pattern;
        else if (!usefdw && *fda.cAlternateFileName)
          name = fda.cAlternateFileName;
        else {
          name = !usefdw ? fda.cFileName
                         : (CodePageHostToGuestUTF16(pattern,
                                                     (uint16_t *)fdw.cFileName)
                                ? pattern
                                : "");
          if (name == "." || name == "..")
            ;
        ...
        attr = ((usefdw ? fdw.dwFileAttributes : fda.dwFileAttributes) &
                        FILE_ATTRIBUTE_DIRECTORY
                    ? DOS_ATTR_DIRECTORY
                    : 0) |
               ((usefdw ? fdw.dwFileAttributes : fda.dwFileAttributes) & 0x3f);
        dta.SetResult(
            name.c_str(),
            !usefdw
                ? fda.cFileName
                : (CodePageHostToGuestUTF16(pattern, (uint16_t *)fdw.cFileName)
                       ? pattern
                       : name.c_str()),
            usefdw ? fdw.nFileSizeLow : fda.nFileSizeLow,
            usefdw ? fdw.nFileSizeHigh : fda.nFileSizeHigh, date, time, attr);
        return true;
      }
    }
    FindClose(hFind);
...
bool Network_FindFirst(const char *_dir, DOS_DTA &dta) {
  ...
  if (fattr == DOS_ATTR_VOLUME)
    return false;
  std::string search = std::string(_dir) + "\\" + std::string(pattern);
  if (search.size() > 4 && search[0] == '\\' && search[1] == '\\' &&
      search[2] != '\\' &&
      std::count(search.begin() + 3, search.end(), '\\') == 1)
    search += "\\";
  usefdw = CodePageGuestToHostUTF16(namehost, search.c_str());
  hFind = usefdw ? FindFirstFileW((LPCWSTR)namehost, &fdw)
                 : FindFirstFile(search.c_str(), &fda);
  if (hFind != INVALID_HANDLE_VALUE) {
    do {
      if ((fattr & DOS_ATTR_DIRECTORY) ||
          !((usefdw ? fdw.dwFileAttributes : fda.dwFileAttributes) &
            FILE_ATTRIBUTE_DIRECTORY)) {
        FileTimeToSystemTime(
            usefdw ? &fdw.ftLastWriteTime : &fda.ftLastWriteTime, &lt);
        if (SystemTimeToTzSpecificLocalTime(NULL, &lt, &local))
...
bool Network_MakeDir(char const *const dir) {
  ...
  if (wc ? CreateDirectoryW((LPCWSTR)namehost, NULL)
...
bool Network_RemoveDir(char const *const dir) {
  ...
  if (wc ? RemoveDirectoryW((LPCWSTR)namehost) : RemoveDirectory(name.c_str()))
    return true;
  uint16_t error = (uint16_t)GetLastError();
  DOS_SetError((error == ERROR_DIRECTORY || error == ERROR_DIR_NOT_EMPTY)
...
bool Network_Rename(char const *const oldname, char const *const newname) {
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name1.c_str()) &&
            CodePageGuestToHostUTF16(namehost1, name2.c_str());
  if (wc ? MoveFileW((LPCWSTR)namehost, (LPCWSTR)namehost1)
         : MoveFile(name1.c_str(), name2.c_str()))
    return true;
  uint16_t error = (uint16_t)GetLastError();
...
bool Network_SetFileAttr(char const *const filename, uint16_t attr) {
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name.c_str());
  if (wc ? !SetFileAttributesW((LPCWSTR)namehost, attr)
         : !SetFileAttributes(name.c_str(), attr)) {
    DOS_SetError((uint16_t)GetLastError());
    return false;
  }
  return true;
...
bool Network_GetFileAttr(const char *filename, uint16_t *attr) {
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name.c_str());
  Bitu attribs = wc ? GetFileAttributesW((LPCWSTR)namehost)
                    : GetFileAttributes(name.c_str());
...
bool Network_GetFileDate(uint16_t entry, uint16_t *&otime, uint16_t *&odate) {
  ...
  if (fstat(RealHandle(entry), &temp_stat))
    return false;
  const struct tm *ltime;
  if ((ltime = localtime(&temp_stat.st_mtime)) != 0) {
    *otime = DOS_PackTime((uint16_t)ltime->tm_hour, (uint16_t)ltime->tm_min,
...
bool Network_FileExists(const char *filename) {
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name.c_str());
  DWORD dwAttrib = wc ? GetFileAttributesW((LPCWSTR)namehost)
                      : GetFileAttributes(name.c_str());
  return (dwAttrib != INVALID_FILE_ATTRIBUTES &&
          !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY));
}

bool Network_CreateFile(char const *filename, uint16_t attributes,
                        uint16_t *entry) {
  int attribs = FILE_ATTRIBUTE_NORMAL;
  if (attributes & 3) // Read-only (1), Hidden (2), System (4) are the same in
                      // DOS and Windows
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name.c_str());
  int handle =
      wc ? _wsopen((LPCWSTR)namehost, _O_CREAT | _O_TRUNC | O_RDWR | O_BINARY,
                   _SH_DENYNO, _S_IREAD | _S_IWRITE)
         : _sopen(name.c_str(), _O_CREAT | _O_TRUNC | O_RDWR | O_BINARY,
                  _SH_DENYNO, _S_IREAD | _S_IWRITE);
  ...
  return false;
} // bool	Network_CreateFile(char const * filename,uint16_t
  // attributes,uint16_t * entry)
...
bool Network_OpenFile(const char *filename, uint8_t flags, uint16_t *entry) {
  ...
  // 000:compatibility mode 001:deny all 010:deny write 011:deny read 100:deny
  // none
  switch ((flags >> 4) & 0x4) {
  case 0:
    shflag |= _SH_DENYNO;
  ...
  bool wc = CodePageGuestToHostUTF16(namehost, name.c_str());
  int handle =
      wc ? _wsopen((LPCWSTR)namehost, oflag, shflag, _S_IREAD | _S_IWRITE)
         : _sopen(name.c_str(), oflag, shflag, _S_IREAD | _S_IWRITE);
...
bool Network_CloseFile(uint16_t entry) {
  ...
  //_ASSERT_EXPR( ( _Expr_val ), _CRT_WIDE(#(handle >= 0 && (unsigned)handle <
  //(unsigned)_nhandle)) );
...
bool Network_WriteFile(uint16_t entry, const uint8_t *data, uint16_t *amount) {
  ...
  return (towrite != -1);
} // bool	Network_WriteFile(uint16_t entry,uint8_t * data,uint16_t *
  // amount)
