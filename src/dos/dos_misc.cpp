...
static Bitu INT2A_Handler(void) { return CBRET_NONE; }

extern const char *RunningProgram;
extern std::string strPasteBuffer;
extern bool i4dos, shellrun, clipboard_dosapi, swapad;
extern RealPt
    DOS_DriveDataListHead; // INT 2Fh AX=0803h DRIVER.SYS drive data table list
...
static bool DOS_MultiplexFunctions(void) {
  ...
  case 0x0802: /* DRIVER.SYS function */
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "Unhandled DRIVER.SYS call AX=%04x BX=%04x CX=%04x DX=%04x BP=%04x",
        reg_ax, reg_bx, reg_cx, reg_dx, reg_bp);
    break;
  case 0x0803: /* DRIVER.SYS function */
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "Unhandled DRIVER.SYS call AX=%04x BX=%04x CX=%04x DX=%04x BP=%04x",
        reg_ax, reg_bx, reg_cx, reg_dx, reg_bp);
    // FIXME: Windows 95 SCANDISK.EXE relies on the drive data table list
    // pointer provided by this call.
    //        Returning DS:DI unmodified or set to 0:0 will only send it off
    //        into the weeds chasing random data as a linked list. However
    //        looking at the code DI=0xFFFF is sufficient to prevent that until
    //        DOSBox-X emulates DRIVER.SYS functions and provides the
    //        information it expects according to RBIL. BUT, Windows 95 setup
    //        checks if the pointer is NULL, and considers 0:FFFF valid >_<.
  ...
  case 0x1000: /* SHARE.EXE installation check */
    if (enable_share_exe) {
      reg_al = 0xff; /* Report that share.exe is installed. Of course not all
                        SHARE functions are emulated. */
    }
    return true;
  case 0x1216: /* GET ADDRESS OF SYSTEM FILE TABLE ENTRY */
    ...
    if (reg_bx < 16) {
      ...
      // DEBUG
      //			LOG_MSG("handle=%u rel=%u
      //initsft=%08x",reg_bx,rel_entry,(unsigned int)sftrealpt);
      ...
      while (1) {
        ...
        // DEBUG
        //				LOG_MSG("handle=%u rel=%u
        //nextsft=%08x",reg_bx,rel_entry,(unsigned int)sftrealpt);
      }
      ...
      // DEBUG
      //			LOG_MSG("handle=%u rel=%u
      //finalsft=%08x",reg_bx,rel_entry,(unsigned int)sftrealpt);

      if (Files[reg_bx])
        mem_writeb(sftptr + sftofs, (uint8_t)(Files[reg_bx]->refCtr));
      else
        mem_writeb(sftptr + sftofs, 0);
      ...
      if (handle >= DOS_FILES) {
        ...
        mem_writeb(sftptr + sftofs + 0x04,
                   (uint8_t)(Files[reg_bx]->attr));       // file attribute
        ...
        mem_writew(sftptr + sftofs + 0x0d,
                   Files[reg_bx]->time); // packed file time
        mem_writew(sftptr + sftofs + 0x0f,
                   Files[reg_bx]->date); // packed file date
        uint32_t curpos = 0;
      ...
      if (strrchr_dbcs((char *)filename, '\\'))
        filename = strrchr_dbcs((char *)filename, '\\') + 1;
      if (strrchr(filename, '/'))
      ...
      if (dotpos) {
        ...
        for (i = 0; i < nlen; i++)
          mem_writeb((PhysPt)(sftptr + sftofs + 0x20u + i),
        ...
        if (extlen > 3)
          extlen = 3;
        for (i = 0; i < extlen; i++)
          mem_writeb((PhysPt)(sftptr + sftofs + 0x28u + i),
                     (unsigned char)dotpos[i]);
        for (i = extlen; i < 3; i++)
  ...
  case 0x1600: /* Windows enhanced mode installation check */
    // Leave AX as 0x1600, indicating that neither Windows 3.x enhanced mode,
    // Windows/386 2.x nor Windows 95 are running, nor is XMS version 1 driver
    // installed
    if (!control->SecureMode() &&
        ((reg_sp == 0xFFF6 && mem_readw(SegPhys(ss) + reg_sp) == 0x142A) ||
         (reg_sp == 0xFF88 && mem_readw(SegPhys(ss) + reg_sp) == 0xFF9D) ||
         !strcmp(RunningProgram, "DOSCLIP") ||
         !strcmp(RunningProgram, "TOCLIP"))) // Hack for DOSCLIP/TOCLIP
      reg_ax = 0x301;
    return true;
  case 0x1605: /* Windows init broadcast */
    if (enable_a20_on_windows_init) {
      /* This hack exists because Windows 3.1 doesn't seem to enable A20 first
       * during an initial critical period where it assumes it's on, prior to
       * checking and enabling/disabling it.
       *
       * Note that Windows 3.1 also makes this mistake in Standard/286 mode, but
       * it doesn't even make this callout, so this hack is useless unless you
       * are using Enhanced/386 mode. If you want to run Windows 3.1 Standard
       * mode with a20=mask you will have to run builtin command "a20gate on" to
       * turn on the A20 gate prior to starting Windows. */
    ...
    /* TODO: Don't show this by default. Show if the user wants it by a) setting
     * something to "true" in dosbox-x.conf or b) running a builtin command in
     * Z:\ */
    LOG_MSG("DEBUG: INT 2Fh Windows 286/386 DOSX init broadcast issued "
            "(ES:BX=%04x:%04x DS:SI=%04x:%04x CX=%04x DX=%04x DI=%04x(aka "
            "version %u.%u))",
    ...
    /* NTS: The way this protocol works, is that when you (the program hooking
     * this call) receive it, you first pass the call down to the previous INT
     * 2Fh handler with registers unmodified, and then when the call unwinds
     * back up the chain, THEN you modify the results to notify yourself to
     * Windows. So logically, since we're the DOS kernel at the end of the
     * chain, we should still see ES:BX=0000:0000 and DS:SI=0000:0000 and
     * CX=0000 unmodified from the way the Windows kernel issued the call. If
     * that's not the case, then we need to issue a warning because some bastard
     * on the call chain is ruining it for all of us. */
    if (SegValue(es) != 0 || reg_bx != 0 || SegValue(ds) != 0 || reg_si != 0 ||
        reg_cx != 0) {
      LOG_MSG("WARNING: Some registers at this point (the top of the call "
              "chain) are nonzero.\n");
      LOG_MSG("         That means a TSR or other entity has modified "
              "registers on the way down\n");
      LOG_MSG("         the call chain. The Windows init broadcast is supposed "
              "to be handled\n");
      LOG_MSG("         going down the chain by calling the previous INT 2Fh "
              "handler with registers\n");
      LOG_MSG("         unmodified, and only modify registers on the way back "
              "up the chain!\n");
    }

    // Dummy data is required for the Microsoft version of Japanese Windows 3.1
    // in enhanced mode.
    ...
    /* If EMS emulation is providing VCPI and it is enabled (the system is in
     * vm86 mode), provide Windows a callback function to control it */
    {
      ...
      if (p != 0) {
        ...
        reg_si = p & 0xFFFFu;
        LOG_MSG(
            "DEBUG: Providing Windows our VCPI vm86 control entry point 0x%lx",
            (unsigned long)p);
      }
    }
  ...
  case 0x1606:    /* Windows exit broadcast */
     ...
     * Z:\ */
    LOG_MSG(
        "DEBUG: INT 2Fh Windows 286/386 DOSX exit broadcast issued (DX=0x%04x)",
        reg_dx);
    if (reg_dx & 0x0001)
      LOG_MSG(" [286 DOS extender]");
    else
  ...
  case 0x1607:
    /* TODO: Don't show this by default. Show if the user wants it by a) setting
     * something to "true" in dosbox-x.conf or b) running a builtin command in
     * Z:\ Additionally, if the user WANTS to see every invocation of the IDLE
     * call, then allow them to enable that too */
    if (reg_bx != 0x18) { /* don't show the idle call. it's used too often */
      const char *str = Win_NameThatVXD(reg_bx);

      if (str == NULL)
        str = "??";
      LOG_MSG("DEBUG: INT 2Fh Windows virtual device '%s' callout "
  ...
  case 0x160A: {
    ...
    DOS_MCB psp_mcb(dos.psp() - 1);
    psp_mcb.GetFileName(psp_name);
    // Report Windows version 4.0 (95) to PEDIT and NESTICLE x.xx so that they
    // use LFN when available
    if (uselfn &&
        (!strcmp(psp_name, "PEDIT") || !strcmp(psp_name, "PEDITLGT") ||
         ((!strcmp(psp_name, "EDIT") || reg_sp / 0x100 == 0xF) &&
          mem_readw(SegPhys(ss) + reg_sp) == 0x4A) ||
         !strcmp(psp_name, "NESTICLE") ||
         (reg_sp == 0x220A &&
          mem_readw(SegPhys(ss) + reg_sp) / 0x100 == 0x1F))) {
      reg_ax = 0;
      reg_bx = 0x400;
      reg_cx = 2;
      return true;
  ...
  case 0x168f: /*  Close awareness crap */
    ...
    return true;
  case 0x1700:
    if (control->SecureMode() || !clipboard_dosapi)
      return false;
    {
      static constexpr std::array<const char *, 7> blacklisted{
          "DEFRAG",  "DISKEDIT", "NDD",     "NDIAGS",
          "UNERASE", "UNFORMAT", "WINCHECK"};
      char psp_name[9];
      DOS_MCB psp_mcb(dos.psp() - 1);
      psp_mcb.GetFileName(psp_name);
      // NTS: DEFRAG.EXE for MS-DOS 6.22 assumes Windows is running if this call
      // responds affirmatively, because it would mean WINOLDAP is resident.
      for (auto prog : blacklisted)
        if (!std::strcmp(psp_name, prog))
          return false;
      // Special case for INSTALL/INSTALLD
      if (((!strcmp(psp_name, "INSTALL") || !strcmp(psp_name, "INSTALLD")) &&
           reg_sp >= 0xD000 && mem_readw(SegPhys(ss) + reg_sp) / 0x100 == 0x1E))
        return false;
    }
    reg_al = 1;
    reg_ah = 1;
    ...
    if ((reg_dx == 1 || reg_dx == 7)
      ...
      *(text + reg_cx - 1) = 0;
      ...
      if (result.size() && result.back() == 10)
        result.pop_back();
#if defined(C_SDL2)
      if (SDL_SetClipboardText(result.c_str()) == 0)
        reg_ax++;
#else
      if (SetClipboard(result))
        reg_ax++;
#endif
#else
    ) {
  ...
  case 0x1704:
    ...
    if ((reg_dx == 1 || reg_dx == 7)
      ...
      uint8_t *text;
      for (size_t i = 0; i < strPasteBuffer.length(); i++)
        if (strPasteBuffer[i] == 10 || strPasteBuffer[i] == 13)
          extra++;
      if (strPasteBuffer.length() &&
          (text = (uint8_t *)malloc(strPasteBuffer.length() + extra))) {
        while (strPasteBuffer.length()) {
          ...
          if (head > 31 || head == 9 || head == 10 || head == 13)
            text[size++] = head;
          if (head == 13 &&
              (strPasteBuffer.length() < 2 || strPasteBuffer[1] != 10))
            text[size++] = 10;
  ...
  case 0x1705:
    ...
    if ((reg_dx == 1 || reg_dx == 7)
        ...
        && OpenClipboard(NULL)) {
      if (HANDLE text = GetClipboardData(reg_dx == 1 ? CF_TEXT : CF_OEMTEXT)) {
        MEM_BlockWrite(SegPhys(es) + reg_bx, text,
                       (Bitu)(strlen((char *)text) + 1));
        reg_ax++;
      }
      CloseClipboard();
#else
  ...
  case 0x1a00: /* ANSI.SYS installation check (MS-DOS 4.0 or higher) */
    if (IS_PC98_ARCH) {
      /* NTS: PC-98 MS-DOS has ANSI handling directly within the kernel HOWEVER
       * it does NOT respond to this INT 2Fh call. */
      return true;
    } else if (ANSI_SYS_installed()) {
      /* See also: [http://www.delorie.com/djgpp/doc/rbinter/id/71/46.html] */
      /* Reported behavior was confirmed with ANSI.SYS loaded on a Windows 95
       * MS-DOS boot disk, result AX=1AFF */
      reg_al = 0xFF; /* DOSBox/DOSBox-X console device emulates ANSI.SYS, so
                        respond like it's installed */
      return true;
    } else {
      /* MS-DOS without ANSI.SYS loaded doesn't modify any registers in response
       * to this call. */
      return true;
    }
  ...
  case 0x4680: /* Windows v3.0 check */
    // Leave AX as 0x4680, indicating that Windows 3.0 is not running in real
    // (/R) or standard (/S) mode, nor is DOS 5 DOSSHELL active
    return true;
  case 0x4a01: { /* Query free hma space */
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "HMA query: start=0x%06x limit=0x%06x free=0x%06x -> bx=%u %04x:%04x",
        start, limit, DOS_HMA_GET_FREE_SPACE(), (int)reg_bx, (int)SegValue(es),
        (int)reg_di);
  }
    return true;
  case 0x4a02: { /* ALLOCATE HMA SPACE */
    ...
    /* NTS: According to RBIL, Windows 95 adds a deallocate function and changes
     * HMA allocation up to follow a MCB chain structure. Which is something
     * we're probably not going to add for awhile. */
    /* FIXME: So, according to Ralph Brown Interrupt List, MS-DOS 5 and 6 liked
     * to round up to the next paragraph? */
    if (dos.version.major < 7 && (reg_bx & 0xF) != 0)
      reg_bx = (reg_bx + 0xF) & (~0xF);

    uint32_t start = DOS_HMA_FREE_START();
    if ((start + reg_bx) > limit) {
      LOG(LOG_DOSMISC, LOG_DEBUG)("HMA allocation: rejected (not enough room) "
                                  "for %u bytes (0x%x + 0x%x > 0x%x)",
                                  reg_bx, (unsigned int)start,
                                  (unsigned int)reg_bx, (unsigned int)limit);
      reg_bx = 0;
      reg_di = 0xFFFF;
      SegSet16(es, 0xFFFF);
      return true;
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)("HMA allocation: %u bytes at FFFF:%04x", reg_bx,
                                reg_di);
  ...
  case 0x4a10: { /* Microsoft SmartDrive (SMARTDRV) API */
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "Unhandled SMARTDRV call AX=%04x BX=%04x CX=%04x DX=%04x BP=%04x",
        reg_ax, reg_bx, reg_cx, reg_dx, reg_bp);
  }
    return true;
  case 0x4a11: { /* Microsoft DoubleSpace (DBLSPACE.BIN) API */
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "Unhandled DBLSPACE call AX=%04x BX=%04x CX=%04x DX=%04x BP=%04x",
        reg_ax, reg_bx, reg_cx, reg_dx, reg_bp);
  }
    return true;
