...
// #define MAXENV 65535u						/*
// mainline DOSBOx: original value was 32768u */ #define ENV_KEEPFREE 1024
// /* keep unallocated by environment variables (original value mainline DOSBox:
// 83) */
...
void DOS_UpdatePSPName(void) {
  ...
  name[8] = 0;
  if (!strlen(name))
    strcpy(name, "DOSBOX-X");
  for (Bitu i = 0; i < 8;
       i++) { // Don't put garbage in the title bar. Mac OS X doesn't like it
    if (name[i] == 0)
      break;
    if (!isprint(*reinterpret_cast<unsigned char *>(&name[i])))
      name[i] = '?';
  }
  RunningProgram = name;
...
static bool MakeEnv(const char *name, uint16_t *segment) {
  ...
  if (parentenv) {
    for (envsize = 0;; envsize++) {
      ...
      if (mem_readw(envread + envsize) == 0)
        break;
    }
  ...
  /* If you look at environment blocks in real DOS, you see \x01 and then the
   * name of your program follow the environment block for some reason. If the
   * user set the "keep free" below 83 bytes, then don't do it. This might break
   * some games that look for that string, but that's the user's fault for
...
bool DOS_Execute(const char *name, PhysPt block_pt, uint8_t flags) {
  ...
  if (flags != LOADNGO && flags != OVERLAY && flags != LOAD) {
    ...
    return false;
    //		E_Exit("DOS:Not supported execute mode %d for file
    //%s",flags,name);
  }
  ...
  if (!DOS_OpenFile(name, OPEN_READ, &fhandle)) {
    ...
    bool shellcom = (!strcasecmp(name + fLen - 8, "4DOS.COM") &&
                     (fLen == 8 || *(name + fLen - 9) == '\\')) ||
                    (!strcasecmp(name + fLen - 11, "COMMAND.COM") &&
                     (fLen == 11 || *(name + fLen - 12) ==
                                        '\\')); // Trap 4DOS.COM and COMMAND.COM
    ...
    if (!shellcom ||
        !DOS_OpenFile(!strcasecmp(name + fLen - 8, "4DOS.COM") ? z4dos : zcmd,
                      OPEN_READ, &fhandle)) {
  ...
  if (flags != OVERLAY) {
    ...
    if (iscom) {
      ...
      if (machine == MCH_PCJR) {
        ...
        DOS_ReadFile(fhandle, loadbuf, &dataread);
        if (dataread < 0x1800)
          maxsize = ((dataread + 0x10) >> 4) + 0x20;
        if (minsize > maxsize)
          minsize = maxsize;
      }
    } else { /* Exe size calculated from header */
      minsize =
          long2para(imagesize + ((unsigned int)head.minmemory << 4u) + 256u);
      if (head.maxmemory != 0)
        maxsize =
            long2para(imagesize + ((unsigned int)head.maxmemory << 4u) + 256u);
      else
        maxsize = 0xffffu;

      /* Bugfix: scene.org mirrors/hornet/demos/1991/putrefac.zip Putrefaction
       * !PF.{3} has an EXE header that specifies a maxsize less than minsize,
       * and a initial stack pointer that is only valid if we use the maxsize.
       *
       *         This allows it to run without the SS:IP out of range error
       * below. */
      if (maxsize < minsize)
        maxsize = minsize;
    }
    ...
    if (!iscom) {
      ...
      if ((head.minmemory == 0) && (head.maxmemory == 0)) {
        ...
        if (loadseg < (pspseg + 16))
          loadseg = pspseg + 16;
        if ((unsigned int)(loadseg + ((imagesize + 0xF) / 0x10)) >
            (unsigned int)(pspseg + memsize))
          E_Exit("EXE loading error, unable to load to top of block, nor able "
                 "to fit into block");
      }
    }
  } else
  ...
  if (iscom) { /* COM Load 64k - 256 bytes max */
    ...
    readsize = pos;
    if (readsize > (0xffff - 256))
      readsize = 0xffff - 256;
    pos += 256; /* plus stack */
    if (pos > (unsigned int)(memsize * 0x10))
    ...
    if (imagesize > (unsigned int)(memsize * 0x10))
      E_Exit("DOS:Not enough memory for EXE image");
    pos = headersize;
    DOS_SeekFile(fhandle, &pos, DOS_SEEK_SET);
    while (imagesize > 0x7FFF) {
      ...
      MEM_BlockWrite(loadaddress, loadbuf, readsize);
      //			if (readsize!=0x8000)
      //LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
      loadaddress += 0x8000;
      imagesize -= 0x8000;
    }
    if (imagesize > 0) {
      readsize = (uint16_t)imagesize;
      DOS_ReadFile(fhandle, loadbuf, &readsize);
      checksum = crc32(checksum, loadbuf, readsize);
      checksum_bytes += readsize;
      MEM_BlockWrite(loadaddress, loadbuf, readsize);
    ...
    for (i = 0; i < head.relocations; i++) {
      readsize = 4;
      DOS_ReadFile(fhandle, (uint8_t *)&relocpt, &readsize);
  ...
  /* Setup a psp */
  if (flags != OVERLAY) {
    // Create psp after closing exe, to avoid dead file handle of exe in copied
    // psp
  ...
  if (flags == OVERLAY) {
    reg_ax =
        0; // Testing with MS-DOS (6.22) shows that if INT 21 AX=4B is called
           // with the overlay flag (AL==3), then AX and DX are 0 on return.
    reg_dx = 0;
  ...
  if (iscom) {
    ...
    /* FIXME: I've heard of EXE files with entry points like FFFF:0100 or
     * something (COM images turned EXE if I recall). Does this check validate
     * those too? */
    csip = RealMake(loadseg + head.initCS, head.initIP);
    sssp = RealMake(loadseg + head.initSS, head.initSP);
    if (sssp >= RealMake(pspseg + memsize, 0))
      E_Exit("DOS:Initial SS:IP beyond allocated memory block for EXE image");
    if (csip >= RealMake(pspseg + memsize, 0))
      E_Exit("DOS:Initial CS:IP beyond allocated memory block for EXE image");
    if (head.initSP < 4)
      LOG(LOG_EXEC, LOG_ERROR)("stack underflow/wrap at EXEC SS:SP=%04x:%04x",
  ...
  if ((flags == LOAD) || (flags == LOADNGO)) {
    /* Get Caller's program CS:IP of the stack and set termination address to
     * that */
    RealSetVec(0x22, RealMake(real_readw(SegValue(ss), reg_sp + 2),
    ...
    /* Setup bx, contains a 0xff in bl and bh if the drive in the fcb is not
     * valid */
    ...
    if ((d1 >= DOS_DRIVES) || !Drives[d1])
      reg_bl = 0xFF;
    else
      reg_bl = 0;
    uint8_t d2 = fcb2.GetDrive();
    if ((d2 >= DOS_DRIVES) || !Drives[d2])
      reg_bh = 0xFF;
    else
      reg_bh = 0;
  ...
  if (a20_off_if_loading_low && pspseg < 0x1000 && xms_init &&
      XMS_GetEnabledA20() &&
      !(cpu.cr0 & CR0_PROTECTION) /*not protected/vm86 mode*/) {
    LOG(LOG_EXEC, LOG_DEBUG)("Program is being loaded below 64KB, disabling "
                             "A20 gate to try to avoid some common crashes");
    XMS_EnableA20(false);
  }
