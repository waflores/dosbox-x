...
public:
  ...
  Bitu switch_keyboard_layout(const char *new_layout,
                              keyboard_layout *&created_layout,
                              int32_t &tried_cp);
...
private:
  ...
  Bitu read_keyboard_file(const char *keyboard_file_name,
                          int32_t specific_layout, int32_t requested_codepage);
  bool map_key(Bitu key, uint16_t layouted_key, bool is_command,
               bool is_keypair);
};
...
Bitu keyboard_layout::read_keyboard_file(const char *keyboard_file_name,
                                         int32_t req_cp) {
...
void keyboard_layout::read_keyboard_file(int32_t specific_layout) {
  if (strcmp(current_keyboard_file_name, "none"))
    this->read_keyboard_file(current_keyboard_file_name, specific_layout,
                             dos.loaded_codepage);
}

static uint32_t read_kcl_file(const char *kcl_file_name, const char *layout_id,
                              bool first_id_only) {
...
static uint32_t read_kcl_data(struct BuiltinFileBlob bfb, const char *layout_id,
                              bool first_id_only) {
...
Bitu keyboard_layout::read_keyboard_file(const char *keyboard_file_name,
                                         int32_t specific_layout,
                                         int32_t requested_codepage) {
  this->reset();

  if (specific_layout == -1)
    strcpy(current_keyboard_file_name, keyboard_file_name);
  ...
  if (tempfile == NULL) {
    ...
    if ((start_pos = read_kcl_file("keyboard.sys", keyboard_file_name, true))) {
      tempfile = OpenDosboxFile("keyboard.sys");
                    ...
                    read_kcl_file("keybrd2.sys", keyboard_file_name, true))) {
      tempfile = OpenDosboxFile("keybrd2.sys");
    } else if ((start_pos =
                    read_kcl_file("keybrd3.sys", keyboard_file_name, true))) {
      tempfile = OpenDosboxFile("keybrd3.sys");
    } else if ((start_pos =
                    read_kcl_file("keybrd4.sys", keyboard_file_name, true))) {
      tempfile = OpenDosboxFile("keybrd4.sys");
    } else if ((start_pos =
                    read_kcl_file("keyboard.sys", keyboard_file_name, false))) {
      tempfile = OpenDosboxFile("keyboard.sys");
    } else if ((start_pos =
                    read_kcl_file("keybrd2.sys", keyboard_file_name, false))) {
      tempfile = OpenDosboxFile("keybrd2.sys");
    } else if ((start_pos =
                    read_kcl_file("keybrd3.sys", keyboard_file_name, false))) {
      tempfile = OpenDosboxFile("keybrd3.sys");
    } else if ((start_pos =
                    read_kcl_file("keybrd4.sys", keyboard_file_name, false))) {
      tempfile = OpenDosboxFile("keybrd4.sys");
    } else if ((start_pos = read_kcl_data(bfb_KEYBOARD_SYS, keyboard_file_name,
                                          true))) {
      ...
      for (Bitu ct = start_pos + 2; ct < bfb_KEYBOARD_SYS.length; ct++)
        read_buf[read_buf_size++] = bfb_KEYBOARD_SYS.data[ct];
    } else if ((start_pos = read_kcl_data(bfb_KEYBRD2_SYS, keyboard_file_name,
                                          false))) {
      assert(read_buf_size == 0);
      for (Bitu ct = start_pos + 2; ct < bfb_KEYBRD2_SYS.length; ct++)
        read_buf[read_buf_size++] = bfb_KEYBRD2_SYS.data[ct];
    } else if ((start_pos = read_kcl_data(bfb_KEYBRD3_SYS, keyboard_file_name,
                                          false))) {
      assert(read_buf_size == 0);
      for (Bitu ct = start_pos + 2; ct < bfb_KEYBRD3_SYS.length; ct++)
        read_buf[read_buf_size++] = bfb_KEYBRD3_SYS.data[ct];
    } else if ((start_pos = read_kcl_data(bfb_KEYBRD4_SYS, keyboard_file_name,
                                          false))) {
      assert(read_buf_size == 0);
      for (Bitu ct = start_pos + 2; ct < bfb_KEYBRD4_SYS.length; ct++)
        read_buf[read_buf_size++] = bfb_KEYBRD4_SYS.data[ct];
    } else {
      LOG(LOG_BIOS, LOG_ERROR)("Keyboard layout file %s not found",
                               keyboard_file_name);
    ...
    if (tempfile) {
      ...
      read_buf_size =
          (uint32_t)fread(read_buf, sizeof(uint8_t), 65535, tempfile);
      fclose(tempfile);
    }
    start_pos = 0;
  } else {
    ...
    if ((dr < 4) || (read_buf[0] != 0x4b) || (read_buf[1] != 0x4c) ||
        (read_buf[2] != 0x46)) {
      LOG(LOG_BIOS, LOG_ERROR)("Invalid keyboard layout file %s",
                               keyboard_file_name);
  ...
  if (additional_planes > (layout_pages - 4))
    additional_planes = (layout_pages - 4);
  ...
  // check all submappings and use them if general submapping or same codepage
  // submapping
  for (uint16_t sub_map = 0;
       (sub_map < submappings) && (!found_matching_layout); sub_map++) {
    uint16_t submap_cp, table_offset;

    if ((sub_map != 0) && (specific_layout != -1))
      sub_map = (uint16_t)(specific_layout & 0xffff);

    // read codepage of submapping
    submap_cp = host_readw(&read_buf[start_pos + 0x14u + sub_map * 8u]);
    if ((submap_cp != 0) && (submap_cp != requested_codepage) &&
        (specific_layout == -1))
    ...
    for (uint32_t i = 0; i < bytes_read;) {
      ...
      if (scan == 0)
        break;
      uint8_t scan_length =
          (read_buf[read_buf_pos] & 7) + 1; // length of data struct
      read_buf_pos += 2;
      i += 3;
      if (((scan & 0x7f) <= MAX_SCAN_CODE) && (scan_length > 0)) {
        // add all available mappings
        for (uint16_t addmap = 0; addmap < scan_length; addmap++) {
          if (addmap > additional_planes + 2)
            break;
          Bitu charptr =
              read_buf_pos +
              addmap * ((read_buf[read_buf_pos - 2u] & 0x80u) ? 2u : 1u);
          uint16_t kchar = read_buf[charptr];

          if (kchar != 0) { // key remapped
            if (read_buf[read_buf_pos - 2] & 0x80)
              kchar |= read_buf[charptr + 1] << 8; // scancode/char pair
            // overwrite mapping
            current_layout[scan * layout_pages + addmap] = kchar;
            // clear command bit
            current_layout[scan * layout_pages + layout_pages - 2] &=
                ~(1 << addmap);
            // add command bit
            current_layout[scan * layout_pages + layout_pages - 2] |=
                (read_buf[read_buf_pos - 1] & (1 << addmap));
          }
        }

        // calculate max length of entries, taking into account old number of
        // entries
        uint8_t new_flags =
            current_layout[scan * layout_pages + layout_pages - 1] & 0x7;
        if ((read_buf[read_buf_pos - 2] & 0x7) > new_flags)
          new_flags = read_buf[read_buf_pos - 2] & 0x7;

        // merge flag bits in as well
        new_flags |= (read_buf[read_buf_pos - 2] |
                      current_layout[scan * layout_pages + layout_pages - 1]) &
                     0xf0;
  ...
  if (found_matching_layout) {
    if (specific_layout == -1)
      LOG(LOG_BIOS, LOG_NORMAL)("Keyboard layout %s successfully loaded",
                                keyboard_file_name);
    else
      LOG(LOG_BIOS, LOG_NORMAL)("Keyboard layout %s (%i) successfully loaded",
                                keyboard_file_name, specific_layout);
    this->use_foreign_layout = true;
    return KEYB_NOERROR;
  }

  LOG(LOG_BIOS, LOG_ERROR)("No matching keyboard layout found in %s",
                           keyboard_file_name);
...
bool keyboard_layout::layout_key(Bitu key, uint8_t flags1, uint8_t flags2,
                                 uint8_t flags3) {
  ...
  bool is_special_pair =
      (current_layout[key * layout_pages + layout_pages - 1] & 0x80) == 0x80;
  ...
  if ((((flags1 & used_lock_modifiers) & 0x7c) == 0) && ((flags3 & 2) == 0)) {
    ...
    if ((((flags1 & 2) >> 1) | (flags1 & 1)) ^
        (((current_layout[key * layout_pages + layout_pages - 1] & 0x40) &
          (flags1 & 0x40)) >>
         6)) {
      // shift plane
      if (current_layout[key * layout_pages + 1] != 0) {
        ...
        bool is_command =
            (current_layout[key * layout_pages + layout_pages - 2] & 2) != 0;
      ...
      if (current_layout[key * layout_pages] != 0) {
        ...
        bool is_command =
            (current_layout[key * layout_pages + layout_pages - 2] & 1) != 0;
        if (this->map_key(key, current_layout[key * layout_pages], is_command,
                          is_special_pair))
          return true;
      }
    }
  ...
  uint16_t current_flags =
      (flags1 & 0x7f) | (((flags2 & 3) | (flags3 & 0xc)) << 8);
  if (flags1 & 3)
  ...
  for (uint16_t cplane = 0; cplane < additional_planes; cplane++) {
    ...
    if (((current_flags & req_flags) == req_flags) &&
        ...
        ((current_flags & current_layout_planes[cplane].forbidden_flags) ==
         0) &&
        ((user_keys & current_layout_planes[cplane].forbidden_userflags) ==
         0)) {
      // remap key
      if (current_layout[key * layout_pages + 2 + cplane] != 0) {
        // check if command-bit is set for this plane
        bool is_command =
            ((current_layout[key * layout_pages + layout_pages - 2] >>
              (cplane + 2)) &
             1) != 0;
...
bool keyboard_layout::map_key(Bitu key, uint16_t layouted_key, bool is_command,
                              bool is_keypair) {
  if (is_command) {
        ...
        for (uint16_t i = 0; i < diacritics_length; i++) {
          if (diacritics[diacritics_start + i * 2] == (layouted_key & 0xff)) {
            ...
            if (diacritics[diacritics_start + i * 2 + 1] == 0xe0 ||
                diacritics[diacritics_start + i * 2 + 1] == 0xf0)
              BIOS_AddKeyToBuffer(diacritics[diacritics_start + i * 2 + 1]);
            else
              BIOS_AddKeyToBuffer((uint16_t)(key << 8) |
    ...
    if (is_keypair) {
      if (kana_input)
        ...
        BIOS_AddKeyToBuffer(layouted_key);
    } else {
      if (kana_input)
        kana_input = false;
      else
        BIOS_AddKeyToBuffer((uint16_t)(key << 8) | (layouted_key & 0xff));
    }
...
uint32_t get_builtin_codepage(struct BuiltinFileBlob bfb) {
  ...
  if (size)
    for (size_t bct = 0; bct < size; bct++)
      cpi_buf[bct] = cpibuf[bct];
  else {
    size = bfb.length;
    for (size_t bct = 0; bct < size; bct++)
      cpi_buf[bct] = bfb.data[bct];
  }
  return size;
}
...
void initcodepagefont() {
  ...
  /* If you don't load COUNTRY.SYS or do any codepage font commands on stock
   * MS-DOS then there is no loading of it from disk at all */
  if (GetDefaultCP() == dos.loaded_codepage)
    return;
  ...
  uint16_t seg = 0;
  uint16_t size =
      0x2000; // NTS: Based on loading up to 64KB of CPI/CPX + assignment of
              // stack pointer at seg+0x1000:0xFFFE. The old value 0x1500 caused
              // crashes with low memory amounts.
  ...
  start_pos += 4;
  for (uint16_t test_codepage = 0; test_codepage < number_of_codepages;
       test_codepage++) {
    ...
    if ((device_type == 0x0001) && (font_type == 0x0001) &&
        (font_codepage == dos.loaded_codepage)) {
      uint16_t number_of_fonts =
          host_readw(&cpibuf[font_data_header_pt + 0x02]);
      uint32_t font_data_start = font_data_header_pt + 0x06;
      for (uint16_t current_font = 0; current_font < number_of_fonts;
           current_font++) {
        uint8_t font_height = cpibuf[font_data_start];
        font_data_start += 6;
        if (font_height == 0x10) {
          for (uint16_t i = 0; i < 256 * 16; i++)
            int10_font_16_init[i] = eurAscii > 32 && i / 16 == eurAscii
                                        ? euro_16[i % 16]
                                        : cpibuf[font_data_start + i];
          font_16_init = true;
        } else if (font_height == 0x0e) {
          for (uint16_t i = 0; i < 256 * 14; i++)
            int10_font_14_init[i] = eurAscii > 32 && i / 14 == eurAscii
                                        ? euro_14[i % 14]
                                        : cpibuf[font_data_start + i];
          font_14_init = true;
        }
...
Bitu keyboard_layout::read_codepage_file(const char *codepage_file_name,
  ...
  if (!strcmp(cp_filename, "none"))
    return KEYB_NOERROR;

  if (codepage_id == dos.loaded_codepage)
    return KEYB_NOERROR;
  ...
  if (!strcmp(cp_filename, "auto")) {
    ...
    case 30007:
    case 30009:
      sprintf(cp_filename, "EGA11.CPI");
      break;
    case 30003:
    case 30029:
    case 30030:
    case 58335:
      sprintf(cp_filename, "EGA12.CPI");
      break;
    case 895:
    case 30002:
    case 58152:
    ...
    case 30039:
    case 30040:
      sprintf(cp_filename, "EGA17.CPI");
      break;
    case 856:
    case 3846:
    case 3848:
      sprintf(cp_filename, "EGA18.CPI");
      break;
    default:
  ...
  if (tempfile == NULL) {
    ...
    if (strsz) {
      char plc =
          (char)toupper(*reinterpret_cast<unsigned char *>(&nbuf[strsz - 1]));
      if (plc == 'I') {
  ...
  if (tempfile == NULL) {
    ...
    if ((cpi_buf[0] != 0xff) || (cpi_buf[1] != 0x46) || (cpi_buf[2] != 0x4f) ||
      ...
      if ((cpi_buf[0] == 0x7f) && (cpi_buf[1] != 0x44) &&
          (cpi_buf[2] != 0x52) && (cpi_buf[3] != 0x46) &&
          (cpi_buf[4] != 0x5f)) {
          ...
          if (next_byte == 0x50) {
            ...
            if (next_byte == 0x58) {
              ...
              if (next_byte == 0x21) {
                ...
                if (readResult != 1) {
                  LOG(LOG_IO,
                      LOG_ERROR)("Reading error in read_codepage_file\n");
                }
                found_at_pos++;
                upxfound = true;
                break;
      ...
      if (!upxfound) {
        LOG(LOG_BIOS, LOG_ERROR)("Codepage file %s invalid: %x", cp_filename,
                                 cpi_buf[0]);
        return KEYB_INVALIDCPFILE;
      } else {
        if (next_byte < 10)
          E_Exit("UPX-compressed cpi file, but upx-version too old");

        // read in compressed CPX-file
        fseek(tempfile, 0, SEEK_SET);
        size_of_cpxdata =
            (uint32_t)fread(cpi_buf, sizeof(uint8_t), 65536, tempfile);
      }
    } else {
  ...
  if (upxfound) {
    if (size_of_cpxdata > 0xfe00)
      E_Exit("Size of cpx-compressed data too big");
    ...
    if (!DOS_AllocateMemory(&seg, &size))
      E_Exit("Not enough free low memory to unpack data");
    MEM_BlockWrite(((unsigned int)seg << 4u) + 0x100u, cpi_buf,
                   size_of_cpxdata);
        ...
        if (font_height == 0x10) {
          ...
          if (int10.rom.font_8_first != 0) {
            ...
            for (uint16_t i = 0; i < 128 * 8; i++) {
              phys_writeb(font8pt + i, eurAscii > 32 && i / 8 == eurAscii
                                           ? euro_08[i % 8]
                                           : cpi_buf[font_data_start + i]);
            }
            font_changed = true;
          ...
          if (int10.rom.font_8_second != 0) {
            ...
            for (uint16_t i = 0; i < 128 * 8; i++) {
              phys_writeb(font8pt + i,
                          eurAscii > 127 && (i + 128) / 8 == eurAscii
                              ? euro_08[i % 8]
                              : cpi_buf[font_data_start + i + 128 * 8]);
            }
            font_changed = true;
          }
        }
...
Bitu keyboard_layout::switch_keyboard_layout(const char *new_layout,
                                             keyboard_layout *&created_layout,
                                             int32_t &tried_cp) {
  if (strncasecmp(new_layout, "US", 2)) {
    ...
    if (language_code_found) {
      ...
      // ...else keyboard layout loaded successfully, change codepage
      // accordingly
...
bool DOS_LayoutKey(Bitu key, uint8_t flags1, uint8_t flags2, uint8_t flags3) {
  if (loaded_layout)
    return loaded_layout->layout_key(key, flags1, flags2, flags3);
  else
    return false;
}
...
Bitu DOS_SwitchKeyboardLayout(const char *new_layout, int32_t &tried_cp) {
  if (loaded_layout) {
    ...
    Bitu ret_code = loaded_layout->switch_keyboard_layout(
        new_layout, changed_layout, tried_cp);
    if (changed_layout) {
...
public:
  DOS_KeyboardLayout(Section *configuration) : Module_base(configuration) {
    ...
    dos.loaded_codepage =
        GetDefaultCP(); // default codepage already initialized
    int tocp =
        !strcmp(layoutname, "jp") || IS_JDOSV
            ? 932
            : (!strcmp(layoutname, "ko") || IS_KDOSV
                   ? 949
                   : (!strcmp(layoutname, "tw") || !strcmp(layoutname, "hk") ||
                              !strcmp(layoutname, "zht") || IS_TDOSV
                          ? 950
                          : (!strcmp(layoutname, "cn") ||
                                     !strcmp(layoutname, "zh") ||
                                     !strcmp(layoutname, "zhs") || IS_PDOSV
                                 ? 936
                                 : (!strcmp(layoutname, "us") ? 437 : 0))));
#if defined(WIN32)
    if (dos.loaded_codepage == 932 && !IS_PC98_ARCH &&
        GetKeyboardType(0) == 7 && !strcmp(layoutname, "auto"))
      layoutname = "jp106";
#endif
    if (tocp && strcmp(layoutname, "jp106") && strcmp(layoutname, "jp") &&
        strcmp(layoutname, "ko") && strcmp(layoutname, "cn") &&
        strcmp(layoutname, "hk") && strcmp(layoutname, "tw"))
      layoutname = "us";
    ...
    if (IS_PC98_ARCH) {
      ...
      if (GetKeyboardLayoutName(layoutID_string)) {
        if (strlen(layoutID_string) == 8) {
          int cur_kb_layout_by_name =
              (int)ConvHexWord((char *)&layoutID_string[4]);
          layoutID_string[4] = 0;
      ...
      switch (cur_kb_layout) {
        /*				case 1026: // Bulgaria, CP 915, Alt CP
           850 layoutname = "bg241"; break; */
      ...
      case 1046: // Brazil, CP 850, Alt CP 437
        ...
        wants_dos_codepage = GetDefaultCP();
        break;
        /*				case 1048: // Romania, CP  852, Alt CP
           850 layoutname = "ro446"; break; */
      ...
      case 1051: // Slovakia, CP 852, Alt CP 850
        layoutname = "sk";
        break;
        /*				case 1052: // Albania, CP 850, Alt CP
           437 layoutname = "sq448"; break; */
      ...
      case 1061: // Estonia, CP 922, Alt CP 850
        ...
        /*				case 1062: // Latvia, CP 921, Alt CP 850
                                                layoutname = "lv";
                                                break; */
        /*				case 1063: // Lithuania, CP 921, Alt CP
           850 layoutname = "lt221"; break; */
    ...
    if (wants_dos_codepage > 0) {
      if ((loaded_layout->read_codepage_file(
              "auto", (int32_t)wants_dos_codepage)) == KEYB_NOERROR) {
    ...
    /*		if (strncmp(layoutname,"auto",4) &&
       strncmp(layoutname,"none",4)) { LOG_MSG("Loading DOS keyboard layout %s
       ...",layoutname);
                    } */
    if (!strcmp(layoutname, "cn") || !strcmp(layoutname, "hk") ||
        !strcmp(layoutname, "tw") || !strcmp(layoutname, "ko")) {
      loaded_layout->read_keyboard_file("us", 437);
      if (!strcmp(layoutname, "cn"))
        tocp = 936;
      if (!strcmp(layoutname, "hk") || !strcmp(layoutname, "tw"))
        tocp = 950;
      if (!strcmp(layoutname, "ko"))
        tocp = 949;
    }
    if (!tocp &&
        loaded_layout->read_keyboard_file(layoutname, dos.loaded_codepage)) {
      if (strncmp(layoutname, "auto", 4)) {
      ...
      if (lcode) {
        LOG_MSG("DOS keyboard layout loaded with main language code %s for "
                "layout %s",
                lcode, layoutname);
      }
    }
    if ((tocp || msgcodepage) && !IS_PC98_ARCH) {
      ...
      if ((dos.loaded_codepage == 932 || tocp == 932) &&
          (!strcmp(layoutname, "jp106") || !strcmp(layoutname, "jp")))
        loaded_layout->read_keyboard_file(layoutname, 932);
      if (!strcmp(layoutname, "ko") || !strcmp(layoutname, "cn") ||
          !strcmp(layoutname, "tw") || !strcmp(layoutname, "hk")) {
        loaded_layout->read_keyboard_file("us", 437);
      ...
      if (!TTF_using() || (TTF_using() && isSupportedCP(tocp))) {
        toSetCodePage(NULL, msgcodepage ? msgcodepage : tocp,
                      msgcodepage ? -1 : -2);
#if C_OPENGL && DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW
        if (OpenGL_using() && control->opt_lang.size() && lastcp &&
            lastcp != dos.loaded_codepage)
...
void DOS_KeyboardLayout_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(DOS_KeyboardLayout_ShutDown));
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(DOS_KeyboardLayout_ShutDown));
}

static const std::set<int> supportedCodepages = {
    437, 737, 775, 808,  850,  852,  853,  855,  856,  857,  858,  859,  860,
    861, 862, 863, 864,  865,  866,  867,  868,  869,  872,  874,  932,  936,
    949, 950, 951, 1250, 1251, 1252, 1253, 1254, 1255, 1256, 1257, 1258, 3021};

bool isSupportedCP(int cp) {
  if (supportedCodepages.count(cp))
    return true;
  if (customcp && cp == customcp)
    return true;
  if (altcp && cp == altcp)
    return true;
  return false;
