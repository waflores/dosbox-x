...
RealPt DOS_DriveDataListHead =
    0; // INT 2Fh AX=0803h DRIVER.SYS drive data table list
...
void DOS_GetMemory_reset() { dos_memseg = 0; }

void DOS_FreeTableMemory() { dos_memseg = DOS_PRIVATE_SEGMENT; }
...
void DOS_GetMemory_unmap() {
  if (DOS_PRIVATE_SEGMENT != 0) {
    LOG(LOG_DOSMISC, LOG_DEBUG)("Unmapping DOS private segment 0x%04x-0x%04x",
                                DOS_PRIVATE_SEGMENT,
                                DOS_PRIVATE_SEGMENT_END - 1u);
    if (DOS_PRIVATE_SEGMENT >= 0xA000u)
      MEM_unmap_physmem((Bitu)DOS_PRIVATE_SEGMENT << 4u,
                        ((Bitu)DOS_PRIVATE_SEGMENT_END << 4u) - 1u);
...
/* Some DOS games and demoscene stuff has fast and loose VRAM rendering code
 * that can spill past video RAM into whatever follows in C0000h. If the DOSBox
 * private area is there, then that sloppiness will corrupt whatever data
 * structures and interrupts we put there and cause crashes. To prevent that, we
 * allow write-protecting the area (DOSBox-X can still write to it using
 * phys_writeb). */
void DOS_Private_UMB_Lock(const bool lock) {
  if (DOS_PRIVATE_SEGMENT >= 0xA000) {
    if (lock)
      MEM_map_ROM_physmem((Bitu)DOS_PRIVATE_SEGMENT << 4u,
                          ((Bitu)DOS_PRIVATE_SEGMENT_END << 4u) - 1u);
    else
      MEM_map_RAM_physmem((Bitu)DOS_PRIVATE_SEGMENT << 4u,
                          ((Bitu)DOS_PRIVATE_SEGMENT_END << 4u) - 1u);
  }
}
...
void DOS_GetMemory_Choose() {
  if (DOS_PRIVATE_SEGMENT == 0) {
    ...
    /* NTS: Code has been arranged so that DOS kernel init follows BIOS INT10h
     * init */
    ...
    DOS_PRIVATE_SEGMENT_END =
        (uint16_t)(DOS_PRIVATE_SEGMENT + DOS_PRIVATE_SEGMENT_Size);
    ...
    if (DOS_PRIVATE_SEGMENT >= 0xA000) {
      memset(GetMemBase() + ((Bitu)DOS_PRIVATE_SEGMENT << 4u), 0x00,
             (Bitu)(DOS_PRIVATE_SEGMENT_END - DOS_PRIVATE_SEGMENT) << 4u);
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)("DOS private segment set to 0x%04x-0x%04x",
                                DOS_PRIVATE_SEGMENT,
                                DOS_PRIVATE_SEGMENT_END - 1);
  }
}
...
uint16_t DOS_GetMemory(uint16_t pages, const char *who) {
  if (who == NULL)
    who = "";
  if (dos_memseg == 0) {
    if (DOS_GetMemory_unmapped)
      E_Exit("DOS:Attempt to use DOS_GetMemory() when private area was "
             "unmapped by BOOT");
    if (DOS_PRIVATE_SEGMENT == 0)
      DOS_GetMemory_Choose();
    dos_memseg = DOS_PRIVATE_SEGMENT;
    if (dos_memseg == 0)
      E_Exit("DOS:DOS_GetMemory() before private area has been initialized");
  }

  if (((Bitu)pages + (Bitu)dos_memseg) > DOS_PRIVATE_SEGMENT_END) {
    LOG(LOG_DOSMISC, LOG_ERROR)("DOS_GetMemory(%u) failed for '%s' "
                                "(alloc=0x%04x segment=0x%04x end=0x%04x)",
                                pages, who, dos_memseg, DOS_PRIVATE_SEGMENT,
                                DOS_PRIVATE_SEGMENT_END);
...
void SetupDBCSTable() {
  if (enable_dbcs_tables) {
    ...
    // Values are copied from INT 21h AX=6300h as returned by an MS-DOS 6.22
    // boot disk
    if (IS_PC98_ARCH || IS_JEGA_ARCH || IS_JDOSV ||
        dos.loaded_codepage == 932) { // Japanese
      ...
      mem_writeb(Real2Phys(dos.tables.dbcs) + 2,
                 gbk ? 0x81 : 0xA1); // low/high DBCS pair
      ...
      mem_writeb(Real2Phys(dos.tables.dbcs) + 3, 0xFE);
      mem_writed(Real2Phys(dos.tables.dbcs) + 4, 0);
    } else if (IS_TDOSV || dos.loaded_codepage == 950 ||
               dos.loaded_codepage == 951) { // Traditional Chinese
      mem_writew(Real2Phys(dos.tables.dbcs) + 0, 0x0004);
      mem_writeb(Real2Phys(dos.tables.dbcs) + 2, 0x81); // low/high DBCS pair
      mem_writeb(Real2Phys(dos.tables.dbcs) + 3, 0xFE);
      mem_writed(Real2Phys(dos.tables.dbcs) + 4, 0);
  ...
  if (dos.loaded_codepage == 950 && !chinasea)
    makestdcp950table();
  else if (dos.loaded_codepage == 951 && chinasea)
    makeseacp951table();
}
...
void DOS_SetupTables(void) {
  ...
  dos.tables.tempdta_fcbdelete =
      RealMake(DOS_GetMemory(4, "dos.tables.fcbdelete"), 0);
  ...
  if (IS_DOSV) {
    ...
    real_writew(seg_win_startup_info, 0x0e,
                0x0012); // instance data record offset
    real_writew(seg_win_startup_info, 0x10,
                seg_win_startup_info); // instance data record segment
    real_writew(seg_win_startup_info, 0x12, 0x001c); // instance data offset
    real_writew(seg_win_startup_info, 0x14,
                seg_win_startup_info);               // instance data segment
  ...
  if (enable_collating_uppercase) {
    dos.tables.collatingseq =
        RealMake(DOS_GetMemory(25, "dos.tables.collatingseq"), 0);
    mem_writew(Real2Phys(dos.tables.collatingseq), 0x100);
    for (i = 0; i < 256; i++)
      mem_writeb(Real2Phys(dos.tables.collatingseq) + i + 2, (uint8_t)i);
    dos.tables.upcase = dos.tables.collatingseq + 258;
    mem_writew(Real2Phys(dos.tables.upcase), 0x80);
    for (i = 0; i < 128; i++)
      mem_writeb(Real2Phys(dos.tables.upcase) + i + 2, (uint8_t)0x80 + i);
  } else {
  ...
  dos.tables.dpb = DOS_GetMemory(
      ((DOS_DRIVES * dos.tables.dpb_size) + 15u) / 16u, "dos.tables.dpb");
  ...
  for (i = 0; i < DOS_DRIVES; i++) {
    real_writeb(dos.tables.dpb, i * dos.tables.dpb_size,
                (uint8_t)i); // drive number
    real_writeb(dos.tables.dpb, i * dos.tables.dpb_size + 1,
                (uint8_t)i); // unit number
    ...
    real_writew(dos.tables.dpb, i * dos.tables.dpb_size + 6,
                0x0001); // reserved sectors at the beginning of the drive
    mem_writew(Real2Phys(dos.tables.mediaid) + i * dos.tables.dpb_size, 0u);
    real_writew(dos.tables.dpb,
                i * dos.tables.dpb_size +
                    (dos.version.major >= 4 ? 0x1F : 0x1E),
                0xFFFF); // number of free clusters or 0xFFFF if unknown

    // next DPB pointer
    if ((i + 1) < DOS_DRIVES)
      real_writed(dos.tables.dpb,
                  i * dos.tables.dpb_size +
                      (dos.version.major >= 4 ? 0x19 : 0x18),
                  RealMake(dos.tables.dpb, (i + 1) * dos.tables.dpb_size));
    else
      real_writed(dos.tables.dpb,
                  i * dos.tables.dpb_size +
                      (dos.version.major >= 4 ? 0x19 : 0x18),
                  0xFFFFFFFF); // ED4.EXE (provided by Yksoft1) expects this, or
                               // else loops forever
  }
  ...
  /* Windows 95 FORMAT.COM does not use INT 21h to determine the boot drive. It
   * uses the List of Lists and reads it directly from this infoblock. This
   * value matches the hardcoded "C" drive the INT 21h emulation returns.
   *
   * No error checking of any kind is done by FORMAT.COM either, so if this
   * value is zero, it will try to query for "@:\\WINBOOT.SYS" which of course
   * does not exist.
   *
   * I'm given the impression from DOS_FindDevice() that someone probably ran
   * into
   * "@:" paths and did not understand this problem, which is fair enough. */
  dos_infoblock.SetBootDrive(
      3); /* Drive C: (TODO: Make configurable, or else initially set to Z: then
             re-set to the first drive mounted */
   ...
   * See also:
   * [http://hackipedia.org/browse.cgi/Computer/Platform/PC%2c%20NEC%20PC%2d98/Collections/Undocumented%209801%2c%209821%20Volume%202%20%28webtech.co.jp%29/memdos%2etxt]
   ...
   * MS-DOS. */
  DOS_DriveDataListHead =
      RealMake(DOS_GetMemory(1 /*paragraph*/, "driver.sys.data.list"), 0);
  mem_writew(Real2Phys(DOS_DriveDataListHead) + 0x00,
             0xFFFF); /* list termination */
