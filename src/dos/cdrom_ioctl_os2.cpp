...
bool CDROM_Interface_Ioctl::GetUPC(unsigned char &attr, char *upc) {
  ...
  ULONG ulAction = 0;
  APIRET rc =
      DosOpen((unsigned char *)device_name, &cdrom_fd, &ulAction, 0L,
  ...
  char sig[] = {'C', 'D', '0', '1'};
  ULONG sigsize = 4;
  rc = DosDevIOCtl(cdrom_fd, IOCTL_CDROMDISK, CDROMDISK_GETUPC, sig, sigsize,
                   &sigsize, data, len, &len);
  if (rc != NO_ERROR) {
    return false;
  }
...
bool CDROM_Interface_Ioctl::ReadSectors(PhysPt buffer, bool raw,
  ...
  if (raw) {
    ...
    rc = DosDevIOCtl(cdrom_fd, IOCTL_CDROMDISK, CDROMDISK_SEEK, &param_seek,
                     paramsize, &paramsize, 0, 0, 0);
    if (rc != NO_ERROR) {
      return false;
    }
    ...
    ULONG len = buflen;
    rc = DosDevIOCtl(cdrom_fd, IOCTL_CDROMDISK, CDROMDISK_READLONG, &param_read,
                     paramsize, &paramsize, buf, len, &len);
    if (rc != NO_ERROR) {
      return false;
    }
  } else {
