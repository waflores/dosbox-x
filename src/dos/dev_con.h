...
uint8_t DefaultANSIAttr() { return IS_PC98_ARCH ? 0xE1 : 0x07; }
...
public:
  ...
  bool Close() override;
  uint8_t GetAnsiAttr(void) { return ansi.attr; }
  void SetAnsiAttr(uint8_t attr) { ansi.attr = attr; }
  ...
  void ClearKeyMap() { key_map.clear(); }
...
private:
  ...
  uint8_t readcache;
  bool lasthat;
  struct ansi { /* should create a constructor, which would fill them with the
                   appropriate values */
    bool installed; // ANSI.SYS is installed (and therefore escapes are handled)
    bool esc;
    bool sci;
    bool equcurp; // ????? ESC = Y X      cursor pos    (not sure if PC-98
                  // specific or general to DOS ANSI.SYS)
    bool
        pc98rab; // PC-98 ESC [ > ...    (right angle bracket) I will rename
                 // this variable if MS-DOS ANSI.SYS also supports this sequence
  ...
  void ESC_BRACKET_B(void) {
    ...
    if (!IS_PC98_ARCH)
      ansi.nrows =
          IS_EGAVGA_ARCH ? (real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) + 1) : 25;
    tempdata = (ansi.data[0] ? ansi.data[0] : 1);
  ...
  void ESC_BRACKET_J() {
    ...
    if (IS_PC98_ARCH) {
      ...
      if (ansi.data[0] == 0) {
        INT10_ScrollWindow(row, col, row, (uint8_t)ansi.ncols, 0, ansi.attr,
                           page);
        INT10_ScrollWindow(row + 1, 0, (uint8_t)ansi.nrows, (uint8_t)ansi.ncols,
                           0, ansi.attr, page);
      } else if (ansi.data[0] == 1) {
        INT10_ScrollWindow(0, 0, row - 1, (uint8_t)ansi.ncols, 0, ansi.attr,
                           page);
        INT10_ScrollWindow(row, 0, row, col, 0, ansi.attr, page);
      } else if (ansi.data[0] == 2) {
        INT10_ScrollWindow(0, 0, (uint8_t)ansi.nrows, (uint8_t)ansi.ncols, 0,
                           ansi.attr, page);
  ...
  void ESC_BRACKET_L() {
    ...
    INT10_ScrollWindow(row, 0, ansi.nrows - 1, ansi.ncols - 1,
                       ansi.data[0] ? ansi.data[0] : 1, ansi.attr, 0xFF);
    ClearAnsi();
    Real_INT10_SetCursorPos(row, 0, page);
  }
  ...
  void ESC_EQU_cursor_pos(void) {
    ...
    if (ansi.data[0] >= ansi.nrows)
      ansi.data[0] = (uint8_t)ansi.nrows - 1;
    if (ansi.data[1] >= ansi.ncols)
      ansi.data[1] = (uint8_t)ansi.ncols - 1;
  ...
  static void Real_INT10_SetCursorPos(uint8_t row, uint8_t col, uint8_t page) {
    ...
    if (IS_PC98_ARCH) {
      INT10_Handler();
      if (pc98_column_over && CURSOR_POS_COL(page) != 80 - 1)
   ...
   * This is a separate function so that both Read() and GetInformation() can
   * use it. GetInformation needs to handle the scan code on entry in order to
   * correctly assert whether Read() will return data or not. Some scan codes
   * are ignored by the CON driver, therefore even though the BIOS says there is
   * key data, Read() will not return anything and will block. */
  bool CommonPC98ExtScanConversionToReadBuf(unsigned char code) {
    ...
    case 0x3F: // HELP
    case 0x40: // KEYPAD -
      PC98_GetEditorKeyEscape(/*&*/ esclen, dev_con_readbuf, code);
      dev_con_pos = 0;
      dev_con_max = esclen;
    ...
    case 0x55: // VF4
    case 0x56: // VF5
      PC98_GetVFuncKeyEscape(/*&*/ esclen, dev_con_readbuf, code + 1u - 0x52u);
      dev_con_pos = 0;
      dev_con_max = esclen;
      return (dev_con_max != 0) ? true : false;
    case 0x62: // F1
    ...
    case 0x6A: // F9
    case 0x6B: // F10
      PC98_GetFuncKeyEscape(/*&*/ esclen, dev_con_readbuf, code + 1u - 0x62u);
      dev_con_pos = 0;
      dev_con_max = esclen;
      return (dev_con_max != 0) ? true : false;
    case 0x82: // Shift+F1
    ...
    case 0x8A: // Shift+F9
    case 0x8B: // Shift+F10
      PC98_GetShiftFuncKeyEscape(/*&*/ esclen, dev_con_readbuf,
                                 code + 1u - 0x82u);
      dev_con_pos = 0;
      dev_con_max = esclen;
      return (dev_con_max != 0) ? true : false;
    ...
    case 0x9B: // Control+F10
      if (inhibited_ControlFn()) {
        PC98_GetCtrlFuncKeyEscape(/*&*/ esclen, dev_con_readbuf,
                                  code + 1u - 0x92u);
        dev_con_pos = 0;
        dev_con_max = esclen;
        return (dev_con_max != 0) ? true : false;
      } else if (code == 0x95) { // CTRL+F4   Toggle kanji/graph mode HANDLED
                                 // INTERNALLY, NEVER RETURNED TO CONSOLE
        void pc98_toggle_char_mode(void);
        pc98_toggle_char_mode();
      } else if (code == 0x97) { // CTRL+F6   Toggle 20/25-line text HANDLED
                                 // INTERNALLY, NEVER RETURNED TO CONSOLE
      ...
      } else if (code == 0x98) { // CTRL+F7   Toggle function key row HANDLED
                                 // INTERNALLY, NEVER RETURNED TO CONSOLE
        void pc98_function_row_user_toggle(void);
        pc98_function_row_user_toggle();
      } else if (code == 0x99) { // CTRL+F8   Clear screen, home cursor HANDLED
                                 // INTERNALLY, NEVER RETURNED TO CONSOLE
        uint8_t page = real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE);
    ...
    case 0xC5: // VF4
    case 0xC6: // VF5
      PC98_GetShiftVFuncKeyEscape(/*&*/ esclen, dev_con_readbuf,
                                  code + 1u - 0xC2u);
      dev_con_pos = 0;
      dev_con_max = esclen;
      return (dev_con_max != 0) ? true : false;
    ...
    case 0xD6: // VF5
      if (inhibited_ControlFn()) {
        PC98_GetCtrlVFuncKeyEscape(/*&*/ esclen, dev_con_readbuf,
                                   code + 1u - 0xD2u);
        dev_con_pos = 0;
        dev_con_max = esclen;
        return (dev_con_max != 0) ? true : false;
  ...
  static void Real_INT10_TeletypeOutput(uint8_t xChar, uint8_t xAttr) {
    if (IS_PC98_ARCH && (real_readb(0x60, 0x8A) == 1)) {
      if (con_sjis.take(xChar)) {
        ...
        /* FIXME: I'm not sure what NEC's ANSI driver does if a doublewide
         * character is printed at column 79 */
        if ((cur_col + cw) > ncols) {
  ...
  static void Real_WriteChar(uint8_t cur_col, uint8_t cur_row, uint8_t page,
                             uint8_t chr, uint8_t attr, uint8_t useattr) {
    // Cursor position
    Real_INT10_SetCursorPos(cur_row, cur_col, page);
      ...
      if (IS_PC98_ARCH && (real_readb(0x60, 0x8A) == 1)) {
        if (con_sjis.take(chr)) {
          ...
          if (pc98_column_over || (cw == 2 && (cur_col + cw) > ncols)) {
            ...
            cur_row++;
            if (cur_row == nrows) {
              INT10_ScrollWindow(0, 0, (uint8_t)(nrows - 1),
                                 (uint8_t)(ncols - 1), -1, defattr, 0);
              cur_row--;
            }
            Real_INT10_SetCursorPos(cur_row, cur_col, page);
          }
...
public:
  ...
  void INTDC_CL10h_AH03h(uint16_t raw) {
     ...
     *
     * Technically this means there is a bug where if DL(X) is 0xE0 or larger it
     * will carry into DH(Y) */
    raw += 0x2020;
  ...
  void INTDC_CL10h_AH04h(void) { ESC_D(); }

  void INTDC_CL10h_AH05h(void) { ESC_M(); }

  void INTDC_CL10h_AH06h(uint16_t count) {
    ansi.data[0] = (uint8_t)
        count; /* truncation is deliberate, just like the actual ANSI driver */
    ESC_BRACKET_A();
  }
...
// F7       0x1B 0x4A   <shortcut>  Toggle function row (C1/CU/etc, shortcuts,
// or off) F8       0x1B 0x50   <shortcut>  Clear screen, home cursor F9 0x1B
// 0x51   <shortcut>  -- F10      0x1B 0x5A   <shortcut>  -- INS      0x1B 0x50
// 0x1B 0x50   0x1B 0x50 DEL      0x1B 0x44   0x1B 0x44   0x1B 0x44 ROLL UP  --
// --          -- POLL DOWN--          --          -- COPY     --          -- --
// HOME/CLR 0x1A        0x1E        --
// HELP     --          --          --
...
// Section 4-8.
//
// The PDF documents ANSI codes defined on PC-98, which may or may not be a
// complete listing.
bool device_CON::Read(uint8_t *data, uint16_t *size) {
  ...
  while (*size > count) {
    ...
    static const bool idle_enabled =
        ((Section_prop *)control->GetSection("dos"))->Get_bool("dos idle api");
    if (idle_enabled) {
    ...
    case 13:
      ...
      if (*size > count)
        data[count++] =
            0x0A; // it's only expanded if there is room for it. (NO cache)
      *size = count;
      reg_ax = oldax;
      if (dos.echo) {
#if defined(USE_TTF)
        ...
        if (IS_DOSV || IS_PC98_ARCH) {
          ...
          Real_INT10_TeletypeOutput(
              13, defattr); // maybe don't do this ( no need for it actually )
                            // (but it's compatible)
    ...
    case 8:
      ...
      else if (count) { // Remove data if it exists (extended keys don't go
                        // right)
        uint8_t flag = 0;
...
#if defined(USE_TTF)
      if ((isJEGAEnabled() || IS_PC98_ARCH || IS_DOSV || ttf_dosv) &&
          (reg_ah == 0xf0 || reg_ah == 0xf1)) {
#else
      if ((isJEGAEnabled() || IS_PC98_ARCH || IS_DOSV) &&
          (reg_ah == 0xf0 || reg_ah == 0xf1)) {
#endif
        data[count++] = reg_al;
    ...
    case 0: /* Extended keys in the int 16 0x0 case */
      for (std::vector<key_change>::iterator key = key_map.begin();
           key != key_map.end(); ++key) {
        if (key->src == reg_ah << 8) {
    ...
    if (dos.echo &&
        !binary) { // what to do if *size==1 and character is BS ?????
  ...
  while (*size > count) {
    ...
    if (log_dev_con) {
      if (log_dev_con_str.size() >= 255 || data[count] == '\n' ||
          data[count] == 27) {
        logging_con = true;
        LOG_MSG(log_dev_con == 2 ? "%s" : "DOS CON: %s",
                log_dev_con_str.c_str());
        logging_con = false;
    ...
    if (isDBCSCP() && !dos.direct_output
      ...
      if (!read_kanji1 && isKanji1(data[count])) {
        ...
        if (col == ncols - 1 && isKanji2(data[count])) {
          ...
          if (nrows == row + 1) {
            INT10_ScrollWindow(0, 0, (uint8_t)(nrows - 1), (uint8_t)(ncols - 1),
                               -1, ansi.attr, page);
    ...
    if (ansi.equcurp) { /* proprietary ESC = Y X command */
      ...
      ansi.data[ansi.numberofarg] =
          10 * ansi.data[ansi.numberofarg] + (data[count] - '0');
    } else if (data[count] == ';') {
      ...
      case 'l': /* RESET MODE */
        ...
        case 5: // show/hide cursor
          ...
          real_writeb(0x60, 0x11B,
                      data[count] == 'l' ? 0x01
                                         : 0x00); /* cursor display state */
          break;
        default:
          LOG(LOG_IOCTL, LOG_NORMAL)("ANSI: unhandled esc [ > %d %c",
                                     ansi.data[0], data[count]);
          break;
        };
        ...
        break;
      default:
        LOG(LOG_IOCTL, LOG_NORMAL)("ANSI: unhandled char %c in esc [ >",
                                   data[count]);
        ClearAnsi();
        break;
      }
      ...
      case 'm': /* SGR */
        ...
        for (i = 0; i <= ansi.numberofarg; i++) {
          ...
          if (IS_PC98_ARCH) {
            ...
            if (ansi.data[i] >= 17 && ansi.data[i] <= 23) {
              const uint8_t convtbl[] = {31, 34, 35, 32, 33, 36, 37};
        ...
        if (IS_PC98_ARCH)
          real_writeb(0x60, 0x11D, ansi.attr);
        ClearAnsi();
        break;
      case 'f':
      case 'H':             /* Cursor Pos*/
        ...
        if (ansi.data[1] == 0)
          ansi.data[1] = 1;
        if (ansi.data[0] > ansi.nrows)
          ansi.data[0] = (uint8_t)ansi.nrows;
        if (ansi.data[1] > ansi.ncols)
          ansi.data[1] = (uint8_t)ansi.ncols;
        Real_INT10_SetCursorPos(--(ansi.data[0]), --(ansi.data[1]),
      ...
      case '>': /* proprietary NEC PC-98 MS-DOS codes (??) */
        if (IS_PC98_ARCH) {
          ...
          LOG(LOG_IOCTL,
              LOG_NORMAL)("ANSI: ESC [ > not supported outside PC-98 mode");
          ClearAnsi();
        }
        break;
      case 'n': /* Device Status Report */
        ...
        case 6: /* report active position */
          dev_con_pos = 0;
          dev_con_max =
              sprintf((char *)dev_con_readbuf, "\x1B[%d;%dR",
                      CURSOR_POS_ROW(page) + 1, CURSOR_POS_COL(page) + 1);
          break;
        default:
          LOG(LOG_IOCTL, LOG_NORMAL)(
              "ANSI: unhandled Device Status Report code %d", ansi.data[0]);
          break;
        };
        ClearAnsi();
        break;
...
bool device_CON::Close() { return true; }

extern bool dos_con_use_int16_to_detect_input;

uint16_t device_CON::GetInformation(void) {
  static constexpr auto deviceWord =
      DeviceInfoFlags::Device | DeviceInfoFlags::Special |
      DeviceInfoFlags::StdIn | DeviceInfoFlags::StdOut;

  if (dos_con_use_int16_to_detect_input || IS_PC98_ARCH) {
    uint16_t ret =
        deviceWord | DeviceInfoFlags::EofOnInput; /* No Key Available */

    /* DOSBox-X behavior: Use INT 16h AH=0x11 Query keyboard status/preview key.
     * The reason we do this is some DOS programs actually rely on hooking INT
     * 16h to manipulate, hide, or transform what the DOS CON driver sees as
     * well as itself. Perhaps the most disgusting example of this behavior
     * would be the SCANDISK.EXE utility in Microsoft MS-DOS 6.22, which
     * apparently relies on hooking INT 16h in this way to catch the Escape,
     * CTRL+C, and some other scan codes in order to "eat" the scan codes before
     * they get back to DOS. The reason they can get away with it apparently and
     * still respond properly to those keys, is because the MS-DOS 6.22 CON
     * driver always calls INT 16h AH=0x11 first before calling INT 16h AH=0x10
     * to fetch the scan code.
     *
    ...
    if (readcache || dev_con_pos < dev_con_max)
      return deviceWord; /* key available */

    if (DOS_BreakConioFlag)
      return deviceWord; /* key available */
    ...
    if (!GETFLAG(ZF)) { /* key is present, waiting to be returned on AH=0x10 or
                           AH=0x00 */
      if (IS_PC98_ARCH && reg_al == 0) {
         ...
         * so that Read() returns it immediately instead of doing this
         * conversion itself. This way we never block when we SAID a key was
         * available that gets ignored. */
        ...
        /* need to consume the key. if it generated anything it will be returned
         * to Read() through dev_con_readbuf[] */
        reg_ah = 0x0;
    ...
    /* DOSBox mainline behavior: alternate "fast" way through direct
     * manipulation of keyboard scan buffer */
  ...
  return deviceWord | DeviceInfoFlags::EofOnInput |
         binary; /* No Key Available */
}
...
device_CON::device_CON() {
  ...
  if (IS_PC98_ARCH) {
    /* On real MS-DOS for PC-98, ANSI.SYS is effectively part of the DOS kernel,
     * and cannot be turned off. */
    ansi.installed = true;
  } else {
    /* Otherwise (including IBM systems), ANSI.SYS is not installed by default
     * but can be added to CONFIG.SYS. For compatibility with DOSBox SVN and
     * other forks ANSI.SYS is installed by default. */
...
void device_CON::Output(uint8_t chr) {
  if (!ANSI_SYS_installed() && !IS_PC98_ARCH) {
    ...
    Real_INT10_TeletypeOutput(chr, DefaultANSIAttr());
}

bool device_CON::ANSI_SYS_installed() { return ansi.installed; }
