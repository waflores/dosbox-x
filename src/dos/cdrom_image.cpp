...
CDROM_Interface_Image::BinaryFile::BinaryFile(const char *filename, bool &error)
  ...
  error = file->fail();
#if defined(WIN32) &&                                                          \
    !defined(__MINGW32__) // Wengier: disable for MinGW for now but it appears
                          // to work on my MinGW version
  if (error) {
...
bool CDROM_Interface_Image::BinaryFile::read(uint8_t *buffer, int64_t offset,
                                             int count) {
  if (!seek(offset))
    return false;
...
CDROM_Interface_Image::AudioFile::AudioFile(const char *filename, bool &error)
    : TrackFile(4096) {
  // Use the audio file's actual sample rate and number of channels as opposed
  // to overriding
  ...
  if (sample) {
    ...
    LOG_MSG("CDROM: Loaded %s [%d Hz %d-channel length %.1f min.]",
            filename_only.c_str(), this->getRate(), this->getChannels(),
            this->getLength() / 10584000.0);
  } else
    error = true;
}
...
bool CDROM_Interface_Image::AudioFile::seek(int64_t offset) {
  ...
  LOG_MSG("%s CDROM: seek(%u) took %f ms", get_time(), offset,
...
#if !defined(HX_DOS) &&                                                        \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
void hunk_thread_func(chd_file *chd, int hunk_index, uint8_t *buffer,
                      bool *error) {
...
bool CDROM_Interface_Image::CHDFile::read(uint8_t *buffer, int64_t offset,
                                          int count) {
  ...
  if ((int)needed_hunk != this->hunk_buffer_index) {
    ...
    if (((int)needed_hunk == (this->hunk_buffer_index + 1)) &&
        (!this->hunk_thread_error)) {
    ...
    if (hunk_thread)
      delete this->hunk_thread;
    this->hunk_thread = new std::thread([this, needed_hunk]() {
      hunk_thread_func(this->chd, (int)(needed_hunk + 1),
                       this->hunk_buffer_next, &this->hunk_thread_error);
    });
#endif
  }
  ...
  // so for 2048 sector size images we need to subtract 16 from the offset to
  // account for the missing sync header
  uint8_t *source =
      this->hunk_buffer +
      ((uint64_t)offset - (uint64_t)needed_hunk * this->header->hunkbytes) -
      ((uint64_t)16 * this->skip_sync);
...
bool CDROM_Interface_Image::CHDFile::seek(int64_t offset) {
  ...
  if ((uint32_t)((uint64_t)offset / this->header->hunkbytes) <
      this->header->hunkcount) {
    return true;
  } else {
    return false;
  }
...
uint16_t CDROM_Interface_Image::CHDFile::decode(uint8_t *buffer) {
  ...
  if (this->read(buffer, this->audio_pos, RAW_SECTOR_SIZE)) {
    ...
    // no idea if other platforms need this but on windows this is needed or
    // there is only noise
...
bool CDROM_Interface_Image::SetDevice(char *path, int forceCD) {
  (void)forceCD; // UNUSED
  const bool result = LoadCueSheet(path) || LoadCloneCDSheet(path) ||
                      LoadIsoFile(path) || LoadChdFile(path);
  if (!result && !qmount) {
...
bool CDROM_Interface_Image::GetAudioTracks(int &stTrack, int &end,
                                           TMSF &leadOut) {
  stTrack = 1;
  end = (int)(tracks.size() - 1);
  FRAMES_TO_MSF(tracks[tracks.size() - 1].start + 150, &leadOut.min,
                &leadOut.sec, &leadOut.fr);

#ifdef DEBUG
  LOG_MSG("%s CDROM: GetAudioTracks, stTrack=%d, end=%d, leadOut.min=%d, "
          "leadOut.sec=%d, leadOut.fr=%d",
          get_time(), stTrack, end, leadOut.min, leadOut.sec, leadOut.fr);
#endif

  return true;
}

bool CDROM_Interface_Image::GetAudioTrackInfo(int track, TMSF &start,
                                              unsigned char &attr) {
  if (track < 1 || track > (int)tracks.size())
    return false;
  FRAMES_TO_MSF(tracks[track - 1].start + 150, &start.min, &start.sec,
                &start.fr);
  attr = tracks[track - 1].attr;

#ifdef DEBUG
  LOG_MSG("%s CDROM: GetAudioTrackInfo track=%d MSF %02d:%02d:%02d, attr=%u",
          get_time(), track, start.min, start.sec, start.fr, attr);
#endif
...
bool CDROM_Interface_Image::GetAudioSub(unsigned char &attr,
  ...
  FRAMES_TO_MSF(player.currFrame - tracks[track - 1].start, &relPos.min,
                &relPos.sec, &relPos.fr);
  if (IS_PC98_ARCH && player.playbackRemaining == 0 &&
      !strcmp(RunningProgram, "ITP")) {
    // POLICENAUTS
    // It freeze at the end of the Konami logo or opening.
    // It seems that the end of CD-DA output is checked by the time and frame of
    // the current track, but the time and frame values of the current track do
    // not seem to be correct. For now, this is handled by returning an error
    // when the CD-DA output is finished.
    return false;
  }

#ifdef DEBUG
  LOG_MSG("%s CDROM: GetAudioSub attr=%u, track=%u, index=%u", get_time(), attr,
          track, index);

  LOG_MSG("%s CDROM: GetAudioSub absolute offset (%d), MSF=%d:%d:%d",
          get_time(), player.currFrame, absPos.min, absPos.sec, absPos.fr);
  LOG_MSG("%s CDROM: GetAudioSub relative offset (%d), MSF=%d:%d:%d",
          get_time(), player.currFrame - tracks[track - 1].start, relPos.min,
          relPos.sec, relPos.fr);
#endif
...
#ifdef DEBUG
  LOG_MSG("%s CDROM: GetAudioStatus playing=%d, paused=%d", get_time(), playing,
          pause);
#endif
...
bool CDROM_Interface_Image::GetMediaTrayStatus(bool &mediaPresent,
                                               bool &mediaChanged,
                                               bool &trayOpen) {
  mediaPresent = true;
  mediaChanged = false;
  ...
  LOG_MSG("%s CDROM: GetMediaTrayStatus present=%d, changed=%d, open=%d",
          get_time(), mediaPresent, mediaChanged, trayOpen);
#endif
...
bool CDROM_Interface_Image::PlayAudioSector(unsigned long start,
  ...
  // The CDROM Red Book standard allows up to 99 tracks, which includes the data
  // track
  if (track < 0 || track > 99)
  ...
  // The maximum storage achieved on a CDROM was ~900MB or just under 100
  // minutes with overburning, so use this threshold to sanity-check the start
  // sector.
  else if (start > 450000)
    LOG_MSG("CDROM: Tried to read sector %lu, which is beyond the 100-minute "
            "maximum of a CDROM",
            start);

  // We can't play audio from a data track (as it would result in
  // garbage/static)
  else if (track >= 0 && tracks[track].attr == 0x40)
    LOG_MSG("CDROM: Tried to play the data track. Not doing this track, attr = "
            "[%d, %d, %d]",
            (int)start, (int)track, (int)tracks[track].attr);

  else if (track > end)
    LOG_MSG(
        "CDROM: Tried to load track #  % d, which does not exist in cue sheet",
        track);

  else if (tracks[track].file == NULL)
    LOG_MSG("CDROM: Tried to load track #  % d, but corresponding audio file "
            "not found",
            track);
  ...
  // Checks passed, setup the audio stream
    ...
    uint32_t offset;
    // Convert the playback start sector to a time offset (milliseconds)
    // relative to the track
    offset = tracks[track].skip +
             ((start > tracks[track].start)
                  ? (start - tracks[track].start) * tracks[track].sectorSize
                  : 0);
    // Hack: sometimes start < tracks[track].start makes offset a negative
    // value, which can result inadequate offset values
    if (tracks[track].length - start + tracks[track].start < 50) {
      track++;
      offset = (uint32_t)tracks[track].skip;
      LOG_MSG("CDROM: Fall in to track # %d since offset close to next track",
              track);
      // Hack: sometimes games points to the sector close to the end of previous
      // track
      //       rather than the start of target track, so go to the target track
      //       in such case.
    }
    ...
    if (is_playable) {
      ...
      player.playbackTotal =
          lround(len * tracks[track].sectorSize * bytesPerMs / 176.4);
      player.playbackRemaining = player.playbackTotal;

      LOG_MSG("CDROM: Playing track # %d %.1f min.-mark", tracks[track].number,
              tracks[track].skip * (1 / 10584000.0));

#ifdef DEBUG
      LOG_MSG("CDROM: Playing track %d at %.1f KHz %d-channel at start sector "
              "%lu (%.1f minute-mark), seek %u (skip=%d,dstart=%d,secsize=%d), "
              "for %lu sectors (%.1f seconds)",
              track, rate / 1000.0, channels, start, offset * (1 / 10584000.0),
              offset, tracks[track].skip, tracks[track].start,
              tracks[track].sectorSize, len,
...
void CDROM_Interface_Image::ChannelControl(TCtrl ctrl) {
  ...
  player.ctrlUsed = (ctrl.out[0] != 0 || ctrl.out[1] != 1 ||
                     ctrl.vol[0] < 0xfe || ctrl.vol[1] < 0xfe);
...
bool CDROM_Interface_Image::ReadSectors(PhysPt buffer, bool raw,
...
bool CDROM_Interface_Image::ReadSectorsHost(void *buffer, bool raw,
                                            unsigned long sector,
                                            unsigned long num) {
  ...
  for (unsigned long i = 0; i < num; i++) {
    ...
    if (!success)
      break;
    if (raw && buf[i * sectorSize + 2068] && sector < tracks[0].length &&
        !tracks[0].mode2) {
      // ECMA-130: The Intermediate field shall consist of 8 (00)-bytes recorded
      // in positions 2068 to 2075 We report a non-zero value as a sector read
      // error. This is to satisfy copy protection checks which expect certain
      // sectors to be bad. Some raw CD image formats represent bad sectors on
      // the original media by filling up the entire sector beyond the header
      // with a dummy byte like 0x55.
      return false;
    }
  }
...
int CDROM_Interface_Image::GetTrack(unsigned long sector) {
  /*  vector<Track>::iterator i = tracks.begin();
          ...
          while(i != end) {
                  Track &curr = *i;
                  Track &next = *(i + 1);
                  if (curr.start <= sector && sector < next.start) return
     curr.number; i++;
          }
  */
  ...
  int i = 0;
  while (i < end) {
    if ((tracks[i].pregap ? tracks[i].pregap : tracks[i].start) <= sector &&
        sector < (tracks[i + 1].pregap ? tracks[i + 1].pregap
                                       : tracks[i + 1].start)) {
      // LOG_MSG("CDROM: GetTrack sector=%d start %d next %d number %d", sector,
      // tracks[i].pregap, tracks[i+1].pregap, tracks[i].number);
...
bool CDROM_Interface_Image::ReadSector(uint8_t *buffer, bool raw,
                                       unsigned long sector) {
  const int track = GetTrack(sector) - 1;
  // LOG_MSG("CDROM: Readsector sector=%d, track=%d", sector, track);
  if (track < 0)
    return false;
  int64_t seek = (int64_t)(tracks[track].skip + (sector - tracks[track].start) *
                                                    tracks[track].sectorSize);
  int length = (raw ? RAW_SECTOR_SIZE : COOKED_SECTOR_SIZE);
  if (tracks[track].sectorSize != RAW_SECTOR_SIZE && raw)
    return false;
  if ((tracks[track].sectorSize == RAW_SECTOR_SIZE ||
       tracks[track].sectorSize == 2448) &&
      !tracks[track].mode2 && !raw)
    seek += 16;
  if (tracks[track].mode2 && !raw)
    seek += 24;

  // LOG_MSG("CDROM: ReadSector track=%d, desired raw=%s, sector=%ld,
  // length=%d", track, raw ? "true":"false", sector, length);
...
void CDROM_Interface_Image::CDAudioCallBack(Bitu len) {
    ...
    while (true) {
      ...
      if (player.bufferPos - player.bufferConsumed >= requested) {
...
#if defined(WORDS_BIGENDIAN)
              sample = (int16_t)host_readw(
                  (HostPt)&samples[pos * 2 + player.ctrlData.out[i]]);
#else
              sample = samples[pos * 2 + player.ctrlData.out[i]];
#endif
              samples[pos * 2 + i] =
                  (int16_t)(sample * player.ctrlData.vol[i] / 255.0);
            }
          }
        }
        ...
        (player.channel->*player.addSamples)(
            requested / bytes_per_request,
            (int16_t *)(player.buffer + player.bufferConsumed));
        ...
        // Games can query the current Red Book MSF frame-position, so we keep
        // that up-to-date here. We scale the final number of frames by the
        // percent complete, which avoids having to keep track of the equivalent
        // number of Red Book frames read (which would involve converting the
        // compressed streams data-rate into CDROM Red Book rate, which is more
        // work than simply scaling).
        //
        const float playbackPercentSoFar =
            static_cast<float>(player.playbackTotal -
                               player.playbackRemaining) /
            player.playbackTotal;
        player.currFrame =
            (uint32_t)(player.startFrame +
                       ceil(player.numFrames * playbackPercentSoFar));
        break;
        // printProgress( (player.bufferPos -
        // player.bufferConsumed)/(float)AUDIO_DECODE_BUFFER_SIZE, "consume");
      }

      // 2. Wrap
      // =======
      else {
        memcpy(player.buffer, player.buffer + player.bufferConsumed,
               player.bufferPos - player.bufferConsumed);
        player.bufferPos -= player.bufferConsumed;
        player.bufferConsumed = 0;
        // printProgress( (player.bufferPos -
        // player.bufferConsumed)/(float)AUDIO_DECODE_BUFFER_SIZE, "wrap");
      }
      ...
      while (AUDIO_DECODE_BUFFER_SIZE - player.bufferPos >= chunkSize &&
        ...
        const uint16_t decoded =
            player.trackFile->decode(player.buffer + player.bufferPos);
        player.bufferPos += decoded;

        // if we decoded less than expected, which could be due to EOF or if the
        // CUE file specified an exact "INDEX 01 MIN:SEC:FRAMES" value but the
        // compressed track is ever-so-slightly less than that specified, then
        ...
        // printProgress( (player.bufferPos -
        // player.bufferConsumed)/(float)AUDIO_DECODE_BUFFER_SIZE, "fill");
      } // end of fill-while
  ...
  if (player.playbackRemaining <= 0) {
    player.cd->StopAudio();
    // printProgress( (player.bufferPos -
    // player.bufferConsumed)/(float)AUDIO_DECODE_BUFFER_SIZE, "stop");
  }
}
...
bool CDROM_Interface_Image::LoadIsoFile(char *filename) {
  ...
  // filename, track.sectorSize, track.mode2 ? "true":"false");
...
bool CDROM_Interface_Image::CanReadPVD(TrackFile *file, int sectorSize,
                                       bool mode2) const {
  ...
  // pvd[0] = descriptor type, pvd[1..5] = standard identifier, pvd[6] = iso
  // version (+8 for High Sierra)
...
static void CloneCDEntryToTrack(CDROM_Interface_Image::Track &trk,
                                ImageCCDEntry &ent) {
...
bool CDROM_Interface_Image::LoadCloneCDSheet(char *cuefile) {
  // If we're going to support CUE vs CCD vs anything else then this function
  // must reject any file who's file extension is not .CCD
  {
  ...
  enum { NONE = 0, CLONECD, DISC, ENTRY };
  ...
  while (!in.eof()) {
    ...
    if (*s == '[') {
      ...
      //			LOG_MSG("[%s] '%s'='%s' mode=%u
      //curEnt=%d",currentSection.c_str(),name.c_str(),value.c_str(),(unsigned
      //int)mode,currentEntry);
    }
  }
  ...
  //	LOG_MSG("Version=%d TocEntries=%d
...
bool CDROM_Interface_Image::LoadCueSheet(char *cuefile) {
  // reject any file which are not a CUE sheet, GOG is so smart that they set
  // several different extensions so that we can't assume .cue only.
  // Known extensions at the moment are: .cue, .ins, .dat, .inst (not sure it is
  // an exhaustive list)
  {
    char *s = strrchr(cuefile, '.');
    if (!s)
      return false;
    if (!strcasecmp(s, ".ccd") || !strcasecmp(s, ".chd") ||
        !strcasecmp(s, ".iso") || !strcasecmp(s, ".img") ||
        !strcasecmp(s, ".gog") || !strcasecmp(s, ".mds") ||
        !strcasecmp(s, ".mdf") || !strcasecmp(s, ".bin"))
      return false;
  }
  ...
  while (!in.eof()) {
    ...
    if (command == "TRACK") {
      ...
      // SDL_Sound first tries using a decoder having a matching registered
      // extension as the filename, and then falls back to trying each decoder
    ...
    // ignored commands
    else if (command == "CDTEXTFILE" || command == "FLAGS" ||
             command == "ISRC" || command == "PERFORMER" ||
             command == "POSTGAP" || command == "REM" ||
  ...
  int i = 0;
  while (i <= end) {
    LOG_MSG("CDROM: track %d start %d pregap %d length %d", i, tracks[i].start,
            tracks[i].pregap, tracks[i].length);
    i++;
  }
#endif
...
std::vector<string> split_string_to_list(const std::string &str,
...
bool CDROM_Interface_Image::LoadChdFile(char *chdfile) {
  ...
  */
  ...
  if (!error) {
    ...
    UINT8 flags = 0;
    int total_frames = 0;
    while (chd_get_metadata(file->getChd(), CDROM_TRACK_METADATA2_TAG, index,
                            &metadata, 256, &count, &tag,
                            &flags) == CHDERR_NONE) {
      // parse track
      ...
      // this is needed or track.start and track.length do not match with
      // LoadCueSheet for the same image creates the same track list as
      // LoadCueSheet
      if (currPregap) {
...
bool CDROM_Interface_Image::AddTrack(Track &curr, int &shift, int prestart,
  ...
  if (prestart >= 0) {
    ...
    // LOG_MSG("CDROM Addtrack skip=%d prestart=%d curr.start=%d", skip,
    // prestart, curr.start);
  } else
    skip = 0;
  ...
  if (prev.file == curr.file) {
    // LOG_MSG("CDROM: curr.number, curr.start, shift %d", curr.number,
    // curr.start, shift);
    curr.start += shift;
    if (!prev.length) {
    ...
    curr.skip +=
        prev.skip + prev.length * prev.sectorSize + skip * curr.sectorSize;
    totalPregap += currPregap;
  ...
  LOG_MSG("CDROM: AddTrack number=%d, attr=%d, start=%d, length=%.1fmin.",
          curr.number, curr.attr, curr.start,
          curr.file ? curr.file->getLength() * (1 / 10584000.0) : -1);
  // LOG_MSG("CDROM: AddTrack cur.start=%d cur.len=%d cur.start+len=%d |
...
bool CDROM_Interface_Image::GetRealFileName(string &filename,
...
void CDROM_Image_ShutDown(Section * /*sec*/) { Sound_Quit(); }
