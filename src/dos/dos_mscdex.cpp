...
public:
  ...
  void SetName(char const *_name) {
    MEM_BlockWrite(pt + offsetof(sDeviceHeader, name), _name, 8);
  };
...
public:
  ...
  bool GetAudioStatus(uint8_t subUnit, bool &playing, bool &pause, TMSF &start,
                      TMSF &end);

  bool GetQChannelData(uint8_t subUnit, uint8_t &attr, uint8_t &track,
                       uint8_t &index, TMSF &rel, TMSF &abs);
  ...
  bool GetDirectoryEntry(uint16_t drive, bool copyFlag, PhysPt pathname,
                         PhysPt buffer, uint16_t &error);
  bool ReadVTOC(uint16_t drive, uint16_t volume, PhysPt data, uint16_t &offset,
                uint16_t &error);
  bool ReadSectors(uint16_t drive, uint32_t sector, uint16_t num, PhysPt data);
  bool ReadSectors(uint8_t subUnit, bool raw, uint32_t sector, uint16_t num,
                   PhysPt data);
  bool ReadSectorsMSF(uint8_t subUnit, bool raw, uint32_t start, uint16_t num,
                      PhysPt data);
  ...
  bool GetMediaStatus(uint8_t subUnit, bool &media, bool &changed,
                      bool &trayOpen);
...
CMscdex::CMscdex(const char *_name) {
  ...
  for (uint32_t i = 0; i < MSCDEX_MAX_DRIVES; i++)
    cdrom[i] = nullptr;
...
void CMscdex::GetDrives(PhysPt data) {
  for (uint16_t i = 0; i < GetNumDrives(); i++)
    mem_writeb(data + i, dinfo[i].drive);
}
...
int CMscdex::AddDrive(uint16_t _drive, char *physicalPath, uint8_t &subUnit) {
  subUnit = 0;
  if ((Bitu)GetNumDrives() + 1 >= MSCDEX_MAX_DRIVES)
    return 4;
  if (GetNumDrives()) {
    // Error check, driveletter have to be in a row
    if (dinfo[0].drive - 1 != _drive &&
        dinfo[numDrives - 1].drive + 1 != _drive)
      return 1;
  }
  ...
  case 0x00: {
    LOG(LOG_MISC, LOG_NORMAL)("MSCDEX: Mounting physical cdrom: %s",
                              physicalPath);
#if defined(WIN32)
    // Check OS
    OSVERSIONINFO osi;
    ...
    if ((osi.dwPlatformId == VER_PLATFORM_WIN32_NT) &&
      ...
      if (useCdromInterface == CDROM_USE_IOCTL_DIO) {
        cdrom[numDrives] =
            new CDROM_Interface_Ioctl(CDROM_Interface_Ioctl::CDIOCTL_CDA_DIO);
        LOG(LOG_MISC, LOG_NORMAL)("MSCDEX: IOCTL Interface.");
        break;
      }
      if (useCdromInterface == CDROM_USE_IOCTL_DX) {
        cdrom[numDrives] =
            new CDROM_Interface_Ioctl(CDROM_Interface_Ioctl::CDIOCTL_CDA_DX);
        LOG(LOG_MISC,
            LOG_NORMAL)("MSCDEX: IOCTL Interface (digital audio extraction).");
        break;
      }
      if (useCdromInterface == CDROM_USE_IOCTL_MCI) {
        cdrom[numDrives] =
            new CDROM_Interface_Ioctl(CDROM_Interface_Ioctl::CDIOCTL_CDA_MCI);
        LOG(LOG_MISC,
            LOG_NORMAL)("MSCDEX: IOCTL Interface (media control interface).");
        break;
      }
    }
  ...
  case 0x01: // iso cdrom interface
    LOG(LOG_MISC, LOG_NORMAL)("MSCDEX: Mounting iso file as cdrom: %s",
                              physicalPath);
  ...
  case 0x02: // fake cdrom interface (directories)
  {
    ...
    if (!strcmp(physicalPath, "empty")) {
      ...
      LOG(LOG_MISC, LOG_NORMAL)("MSCDEX: Mounting directory as cdrom: %s",
                                physicalPath);
  ...
  if (rootDriverHeaderSeg == 0) {
    ...
    uint16_t driverSize = sizeof(DOS_DeviceHeader::sDeviceHeader) +
                          10; // 10 = Bytes for 3 callbacks

    /* should have been assigned by SetName() in constructor, with copy stored
     * by DOS_File::SetName */
    assert(name != NULL);
    ...
    uint16_t seg = DOS_GetMemory(driverSize / 16u + ((driverSize % 16u) > 0u),
                                 "MSCDEX device header");
...
bool CMscdex::HasDrive(uint16_t drive) { return (GetSubUnit(drive) != 0xff); }
...
bool CMscdex::GetCDInfo(uint8_t subUnit, uint8_t &tr1, uint8_t &tr2,
                        TMSF &leadOut) {
  ...
  dinfo[subUnit].lastResult =
      cdrom[subUnit]->GetAudioTracks(tr1i, tr2i, leadOut);
...
bool CMscdex::GetTrackInfo(uint8_t subUnit, uint8_t track, uint8_t &attr,
                           TMSF &start) {
  if (subUnit >= numDrives)
    return false;
  dinfo[subUnit].lastResult =
      cdrom[subUnit]->GetAudioTrackInfo(track, start, attr);
  if (!dinfo[subUnit].lastResult) {
    attr = 0;
...
bool CMscdex::PlayAudioSector(uint8_t subUnit, uint32_t sector,
                              uint32_t length) {
  if (subUnit >= numDrives)
    return false;
...
bool CMscdex::GetQChannelData(uint8_t subUnit, uint8_t &attr, uint8_t &track,
                              uint8_t &index, TMSF &rel, TMSF &abs) {
  if (subUnit >= numDrives)
    return false;
  dinfo[subUnit].lastResult =
      cdrom[subUnit]->GetAudioSub(attr, track, index, rel, abs);
  if (!dinfo[subUnit].lastResult) {
    attr = track = index = 0;
...
bool CMscdex::GetAudioStatus(uint8_t subUnit, bool &playing, bool &pause,
                             TMSF &start, TMSF &end) {
  if (subUnit >= numDrives)
    return false;
...
bool CMscdex::StopAudio(uint8_t subUnit) {
  ...
  if (dinfo[subUnit].lastResult) {
    if (dinfo[subUnit].audioPlay) {
      ...
      dinfo[subUnit].audioStart =
          pos.min * 60u * 75u + pos.sec * 75u + pos.fr - 150u;
...
bool CMscdex::ResumeAudio(uint8_t subUnit) {
  if (subUnit >= numDrives)
    return false;
  return dinfo[subUnit].lastResult = PlayAudioSector(
             subUnit, dinfo[subUnit].audioStart, dinfo[subUnit].audioEnd);
}
...
uint32_t CMscdex::GetVolumeSize(uint8_t subUnit) {
  ...
  if (dinfo[subUnit].lastResult)
    return (leadOut.min * 60u * 75u) + (leadOut.sec * 75u) + leadOut.fr;
  return 0;
}

bool CMscdex::ReadVTOC(uint16_t drive, uint16_t volume, PhysPt data,
                       uint16_t &offset, uint16_t &error) {
...
bool CMscdex::ReadSectors(uint8_t subUnit, bool raw, uint32_t sector,
                          uint16_t num, PhysPt data) {
  if (subUnit >= numDrives)
    return false;
  if ((cpu_cycles_count_t)(4u * num * 2048u + 5u) < CPU_Cycles)
    CPU_Cycles -= cpu_cycles_count_t(4u * num * 2048u);
  else
    CPU_Cycles = 5u;
  dinfo[subUnit].lastResult =
      cdrom[subUnit]->ReadSectors(data, raw, sector, num);
  return dinfo[subUnit].lastResult;
}

bool CMscdex::ReadSectorsMSF(uint8_t subUnit, bool raw, uint32_t start,
                             uint16_t num, PhysPt data) {
  if (subUnit >= numDrives)
    return false;
  uint8_t min = (uint8_t)(start >> 16) & 0xFF;
...
// Called from INT 2F
bool CMscdex::ReadSectors(uint16_t drive, uint32_t sector, uint16_t num,
                          PhysPt data) {
  return ReadSectors(GetSubUnit(drive), false, sector, num, data);
}

bool CMscdex::GetDirectoryEntry(uint16_t drive, bool copyFlag, PhysPt pathname,
                                PhysPt buffer, uint16_t &error) {
  char volumeID[6] = {0};
  char searchName[256];
  char entryName[256];
  ...
  if (!ReadSectors(GetSubUnit(drive), false, 16, 1, defBuffer))
    return false;
  MEM_StrCopy(defBuffer + 1, volumeID, 5);
  volumeID[5] = 0;
  bool iso = (strcmp("CD001", volumeID) == 0);
  if (!iso) {
    MEM_StrCopy(defBuffer + 9, volumeID, 5);
    if (strcmp("CDROM", volumeID) != 0)
      E_Exit("MSCDEX: GetDirEntry: Not an ISO 9660 or HSF CD.");
  }
  ...
  while (dirSize > 0) {
    ...
    do {
      ...
      if (entrylen > 0 && entryName[entrylen - 1] == '.')
        entryName[entrylen - 1] = 0;
    ...
    if (foundName) {
      if (foundComplete) {
        if (copyFlag) {
          LOG(LOG_MISC, LOG_WARN)("MSCDEX: GetDirEntry: Copyflag structure not "
                                  "entirely accurate maybe");
          uint8_t readBuf[256];
          uint8_t writeBuf[256];
          MEM_BlockRead(defBuffer + index, readBuf, entryLength);
          writeBuf[0] = readBuf[1]; // 00h	BYTE	length of XAR in Logical
                                    // Block Numbers
          memcpy(&writeBuf[1], &readBuf[0x2],
                 4); // 01h	DWORD	Logical Block Number of file start
          writeBuf[5] = 0;
          writeBuf[6] = 8; // 05h	WORD	size of disk in logical blocks
          memcpy(&writeBuf[7], &readBuf[0xa],
                 4); // 07h	DWORD	file length in bytes
          memcpy(&writeBuf[0xb], &readBuf[0x12],
                 6);                                // 0bh	BYTEs	date and time
          writeBuf[0x11] = iso ? readBuf[0x18] : 0; // 11h	BYTE	time
                                                    // zone
          writeBuf[0x12] = readBuf[iso ? 0x19 : 0x18]; // 12h	BYTE	bit
                                                       // flags
          writeBuf[0x13] = readBuf[0x1a]; // 13h	BYTE	interleave size
          writeBuf[0x14] = readBuf[0x1b]; // 14h	BYTE	interleave skip
                                          // factor
          memcpy(&writeBuf[0x15], &readBuf[0x1c],
                 2); // 15h	WORD	volume set sequence number
...
bool CMscdex::GetCurrentPos(uint8_t subUnit, TMSF &pos) {
  if (subUnit >= numDrives)
    return false;
  if (!dinfo[subUnit].audioPlay) {
    FRAMES_TO_MSF((dinfo[subUnit].audioStart + REDBOOK_FRAME_PADDING), &pos.min,
                  &pos.sec, &pos.fr);
    return true;
  }
  TMSF rel;
  ...
  dinfo[subUnit].lastResult =
      GetQChannelData(subUnit, attr, track, index, rel, pos);
  if (!dinfo[subUnit].lastResult)
    pos.fr = pos.min = pos.sec = 0;
  return dinfo[subUnit].lastResult;
}

bool CMscdex::GetMediaStatus(uint8_t subUnit, bool &media, bool &changed,
                             bool &trayOpen) {
  if (subUnit >= numDrives)
    return false;
  dinfo[subUnit].lastResult =
      cdrom[subUnit]->GetMediaTrayStatus(media, changed, trayOpen);
  return dinfo[subUnit].lastResult;
}
...
uint32_t CMscdex::GetDeviceStatus(uint8_t subUnit) {
  ...
  // Reference (Microsoft MS-DOS CD-ROM Extensions 2.1):
  // https://tinyurl.com/3spx4dfn Bits marked with * have the given function
  // listed in INTERRUP.G, but are listed as "Reserved (all 0)" in Microsoft
  // MS-DOS CD-ROM Extensions 2.1
  uint32_t status =
      ((trayOpen ? 1u : 0u) << 0u) | // Bit 0:       0 = Door closed 1 = Door
                                     // opened
      ((dinfo[subUnit].locked ? 0u : 1u) << 1u) | // Bit 1:       0 = Door
                                                  // locked 1 = Door unlocked
      (1u << 2u) | // Bit 2:       0 = Supports only cooked reading        1 =
                   // Supports cooked and raw reading Bit 3:       0 = Read only
                   // 1 = Read/write
      (1u << 4u) | // Bit 4:       0 = Data read only                      1 =
                   // Data read and plays audio/video tracks Bit 5:       0 = No
                   // interleaving                     1 = Supports interleaving
                   // Bit 6:       Reserved
                   // Bit 7:       0 = No prefetching                      1 =
                   // Supports prefetching requests
      (1u << 8u) | // Bit 8:       0 = No audio channel manipulation       1 =
                   // Supports audio channel manipulation
      (1u << 9u) | // Bit 9:       0 = Supports HSG addressing mode        1 =
                   // Supports HSG and Red Book addressing modes
      ((dinfo[subUnit].audioPlay ? 1u : 0u)
       << 10u) | // Bit 10*:     0 = Audio is not playing                1 =
                 // Audio is playing
      ((media ? 0u : 1u) << 11u); // Bit 11*:     0 = Disk in drive 1 = No disk
                                  // in drive Bit 12*:     0 = Does not support
                                  // R-W subchannels    1 = Supports R-W
                                  // subchannels Bit 13-31:   Reserved (all 0)
  return status;
}
...
bool GetMSCDEXDrive(unsigned char drive_letter, CDROM_Interface **_cdrom) {
  ...
  for (i = 0; i < MSCDEX_MAX_DRIVES; i++) {
    if (mscdex->cdrom[i] == NULL)
      continue;
    if (mscdex->dinfo[i].drive == drive_letter) {
      if (_cdrom)
        *_cdrom = mscdex->cdrom[i];
      return true;
    }
  }
...
static uint16_t MSCDEX_IOCTL_Input(PhysPt buffer, uint8_t drive_unit) {
  ...
  case 0x01: /* Drive head location */
  {
    ...
    if (addr_mode == 0) { // HSG
      ...
      if (frames < REDBOOK_FRAME_PADDING)
        MSCDEX_LOG_ERROR("MSCDEX: Get position: invalid position %d:%d:%d",
                         pos.min, pos.sec, pos.fr);
      else
      ...
      MSCDEX_LOG_ERROR("MSCDEX: Get position: invalid address mode %x",
                       addr_mode);
  ...
  case 0x02: /* Reserved */
    LOG(LOG_MISC, LOG_ERROR)("MSCDEX: Unsupported IOCTL INPUT Subfunction %02X",
                             (int)ioctl_fct);
    return 0x03; // Invalid function
  ...
  case 0x0E:     /* UPC code */
  {
    ...
    for (unsigned int i = 0; i < 7; i++)
      mem_writeb(buffer + 2u + i, (unsigned char)upc[i]);
...
static uint16_t MSCDEX_IOCTL_Output(PhysPt buffer, uint8_t drive_unit) {
  ...
  case 0x04: // Write device control string
    LOG(LOG_MISC, LOG_ERROR)(
        "MSCDEX: Unsupported IOCTL OUTPUT Subfunction %02X", (int)ioctl_fct);
    return 0x03; // Invalid function
...
static Bitu MSCDEX_Strategy_Handler(void) {
  ...
  //	MSCDEX_LOG("MSCDEX: Device Strategy Routine called, request header at
  //%x",curReqheaderPtr);
  return CBRET_NONE;
}
...
static Bitu MSCDEX_Interrupt_Handler(void) {
  ...
  if ((funcNr == 0x03) || (funcNr == 0x0c) || (funcNr == 0x80) ||
      (funcNr == 0x82)) {
    buffer = PhysMake(mem_readw(curReqheaderPtr + 0x10),
                      mem_readw(curReqheaderPtr + 0x0E));
  }

  // Reference (Microsoft MS-DOS CD-ROM Extensions 2.1):
  // https://tinyurl.com/3spx4dfn
  switch (funcNr) {
  case 0x00: /* INIT */
  ...
  case 0x83: /* SEEK */
  {
    ...
    if (mem_readb(curReqheaderPtr + 0x0D)) { // Redbook if non-zero
      ...
      if (start < REDBOOK_FRAME_PADDING) {
        MSCDEX_LOG("MSCDEX: Seek: invalid position %d:%d:%d", msf.min, msf.sec,
                   msf.fr);
        start = 0;
      } else {
...
static bool MSCDEX_Handler(void) {
  if (reg_ah == 0x11) {
    if (reg_al == 0x00) {
      if (real_readw(SegValue(ss), reg_sp + 6) == 0xDADA) {
        if (mscdex->rootDriverHeaderSeg == 0)
          return false;
  ...
  if (mscdex->rootDriverHeaderSeg == 0)
    return false; // not handled if MSCDEX not installed

  PhysPt data = PhysMake(SegValue(es), reg_bx);
  MSCDEX_LOG("MSCDEX: INT 2F AX=%04X BX=%04X CX=%04X", reg_ax, reg_bx, reg_cx);
  CALLBACK_SCF(false); // carry flag cleared for all functions (undocumented);
                       // only set on error
  switch (reg_ax) {
  ...
  case 0x150F: { // Get directory entry
    uint16_t error;
    bool success = mscdex->GetDirectoryEntry(reg_cl, reg_ch & 1, data,
                                             PhysMake(reg_si, reg_di), error);
    reg_ax = error;
    if (!success)
      CALLBACK_SCF(true);
  } break;
...
public:
  device_MSCDEX(const char *devname) {
    SetName(MSCDEX_ValidDevName(devname) ? devname : "MSCD001");
  }
  ...
  uint16_t GetInformation(void) override {
    return DeviceInfoFlags::Device | DeviceInfoFlags::IoctlSupport |
           DeviceInfoFlags::OpenCloseSupport;
  }
...
bool device_MSCDEX::ReadFromControlChannel(PhysPt bufptr, uint16_t size,
                                           uint16_t *retcode) {
...
bool device_MSCDEX::WriteToControlChannel(PhysPt bufptr, uint16_t size,
                                          uint16_t *retcode) {
...
int MSCDEX_AddDrive(char driveLetter, const char *physicalPath,
                    uint8_t &subUnit) {
  int result =
      mscdex->AddDrive(driveLetter - 'A', (char *)physicalPath, subUnit);
  return result;
}
...
bool MSCDEX_HasMediaChanged(uint8_t subUnit) {
  ...
  if (mscdex->GetCDInfo(subUnit, tr1, tr2, leadnew)) {
    bool changed = (leadOut[subUnit].min != leadnew.min) ||
...
void MSCDEX_DOS_ShutDown(Section * /*sec*/) { curReqheaderPtr = 0; }
...
void MSCDEX_Init() {
  ...
  AddVMEventFunction(VM_EVENT_RESET,
                     AddVMEventFunctionFuncPair(MSCDEX_ShutDown));
  AddVMEventFunction(VM_EVENT_DOS_EXIT_BEGIN,
                     AddVMEventFunctionFuncPair(MSCDEX_DOS_ShutDown));
}
...
void POD_Load_DOS_Mscdex(std::istream &stream) {
  if (!dos_kernel_disabled) {
    ...
    for (uint8_t drive_unit = 0; drive_unit < dnum; drive_unit++) {
      ...
      play_len -= (pos.min - start.min) * 75 * 60 + (pos.sec - start.sec) * 75 +
                  (pos.fr - start.fr);
      ...
      if (playing)
        mscdex->PlayAudioMSF(drive_unit, msf_time, play_len);
      if (pause)
        mscdex->PlayAudioMSF(drive_unit, msf_time, 0);
    }
