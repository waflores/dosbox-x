...
char *DBCS_upcase(char *str) {
  for (char *idx = str; *idx;) {
    if ((IS_PC98_ARCH && shiftjis_lead_byte(*idx)) ||
        (isDBCSCP() && isKanji1_gbk(*idx))) {
      /* Shift-JIS is NOT ASCII and should not be converted to uppercase like
       * ASCII. The trailing byte can be mistaken for ASCII */
      idx++;
      if (*idx != 0)
        idx++;
    } else {
...
uint32_t DOS_HMA_LIMIT() {
  if (dos.version.major < 5)
    return 0; /* MS-DOS 5.0+ only */
  if (!DOS_IS_IN_HMA())
    return 0;
  return (0x110000 - 16); /* 1MB + 64KB - 16 bytes == (FFFF:FFFF + 1) ==
                             (0xFFFF0 + 0xFFFF + 1) == 0x10FFF0 */
}
...
uint32_t DOS_HMA_FREE_START() {
  ...
  if (dos_hma_allocator == 0) {
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "Starting HMA allocation from physical address 0x%06x (FFFF:%04x)",
...
void DOS_HMA_CLAIMED(uint16_t bytes) {
  ...
  if (limit == 0)
    E_Exit("HMA allocation bug: Claim function called when HMA allocation is "
           "not enabled");
  if (dos_hma_allocator == 0)
    E_Exit("HMA allocation bug: Claim function called without having "
           "determined start");
  dos_hma_allocator += bytes;
  if (dos_hma_allocator > limit)
    E_Exit("HMA allocation bug: Exceeded limit");
}
...
Bitu DOS_PRIVATE_SEGMENT_Size =
    0x800; // 32KB (0x800 pages), mainline DOSBox behavior
...
uint16_t DOS_Block::psp() const {
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: DOS kernel is disabled (booting a guest OS), and yet "
            "somebody is still asking for DOS's current PSP segment\n");
    return 0x0000;
  }
...
void DOS_Block::psp(uint16_t _seg) const {
  if (dos_kernel_disabled) {
    LOG_MSG(
        "BUG: DOS kernel is disabled (booting a guest OS), and yet somebody is "
        "still attempting to change DOS's current PSP segment\n");
    return;
  }
...
RealPt DOS_Block::dta() const {
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: DOS kernel is disabled (booting a guest OS), and yet "
            "somebody is still asking for DOS's DTA (disk transfer address)\n");
    return 0;
  }
...
void DOS_Block::dta(RealPt _dta) const {
  if (dos_kernel_disabled) {
    LOG_MSG(
        "BUG: DOS kernel is disabled (booting a guest OS), and yet somebody is "
        "still attempting to change DOS's DTA (disk transfer address)\n");
    return;
  }
...
#ifdef WIN32
uint16_t NetworkHandleList[127];
uint8_t dos_copybuf_second[DOS_COPYBUFSIZE];
#endif
...
void DOS_SetError(uint16_t code) { dos.errorcode = code; }
...
static const std::vector<CountryInfo> COUNTRY_INFO = {
    //                           | Date fmt | Date separ | Time fmt | Time separ
    //                           | 1000 separ | Dec separ  |
    {COUNTRYNO::United_States, DATE_MDY, SEP_SLASH, TIME_12H, SEP_COLON,
     SEP_COMMA, SEP_PERIOD}, // en_US
    {COUNTRYNO::Candian_French, DATE_YMD, SEP_DASH, TIME_24H, SEP_COLON,
     SEP_SPACE, SEP_COMMA}, // fr_CA.
    {COUNTRYNO::Latin_America, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON,
     SEP_COMMA, SEP_PERIOD}, // es_419
    {COUNTRYNO::Russia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // ru_RU
    {COUNTRYNO::Greece, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // el_GR
    {COUNTRYNO::Netherlands, DATE_DMY, SEP_DASH, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // nl_NL
    {COUNTRYNO::Belgium, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // fr_BE
    {COUNTRYNO::France, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // fr_FR
    {COUNTRYNO::Spain, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // es_ES
    {COUNTRYNO::Hungary, DATE_YMD, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // hu_HU
    {COUNTRYNO::Yugoslavia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // sr_RS/sr_ME/hr_HR/sk_SK/bs_BA/mk_MK
    {COUNTRYNO::Italy, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // it_IT
    {COUNTRYNO::Romania, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // ro_RO
    {COUNTRYNO::Switzerland, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_APOST, SEP_PERIOD}, // ??_CH
    {COUNTRYNO::Czech_Slovak, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_SPACE, SEP_COMMA}, // cs_CZ
    {COUNTRYNO::Austria, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // de_AT
    {COUNTRYNO::United_Kingdom, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON,
     SEP_COMMA, SEP_PERIOD}, // en_GB
    {COUNTRYNO::Denmark, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // da_DK
    {COUNTRYNO::Sweden, DATE_YMD, SEP_DASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // sv_SE
    {COUNTRYNO::Norway, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // nn_NO
    {COUNTRYNO::Poland, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // pl_PL
    {COUNTRYNO::Germany, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // de_DE
    {COUNTRYNO::Argentina, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // es_AR
    {COUNTRYNO::Brazil, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // pt_BR
    {COUNTRYNO::Malaysia, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // ms_MY
    {COUNTRYNO::Australia, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // en_AU
    {COUNTRYNO::Philippines, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON,
     SEP_COMMA, SEP_PERIOD}, // fil_PH
    {COUNTRYNO::Singapore, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // ms_SG
    {COUNTRYNO::Kazakhstan, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_SPACE, SEP_COMMA}, // kk_KZ
    {COUNTRYNO::Japan, DATE_YMD, SEP_SLASH, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // ja_JP
    {COUNTRYNO::South_Korea, DATE_YMD, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_COMMA, SEP_PERIOD}, // ko_KR
    {COUNTRYNO::Vietnam, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // vi_VN
    {COUNTRYNO::China, DATE_YMD, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // zh_CN
    {COUNTRYNO::Turkey, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // tr_TR
    {COUNTRYNO::India, DATE_DMY, SEP_SLASH, TIME_12H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // hi_IN
    {COUNTRYNO::Niger, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // fr_NE
    {COUNTRYNO::Benin, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // fr_BJ
    {COUNTRYNO::Nigeria, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // en_NG
    {COUNTRYNO::Faeroe_Islands, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // fo_FO
    {COUNTRYNO::Portugal, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // pt_PT
    {COUNTRYNO::Iceland, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // is_IS
    {COUNTRYNO::Albania, DATE_DMY, SEP_PERIOD, TIME_12H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // sq_AL
    {COUNTRYNO::Malta, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // mt_MT
    {COUNTRYNO::Finland, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // fi_FI
    {COUNTRYNO::Bulgaria, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // bg_BG
    {COUNTRYNO::Lithuania, DATE_YMD, SEP_DASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // lt_LT
    {COUNTRYNO::Latvia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // lv_LV
    {COUNTRYNO::Estonia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // et_EE
    {COUNTRYNO::Armenia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // hy_AM
    {COUNTRYNO::Belarus, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // be_BY
    {COUNTRYNO::Ukraine, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // uk_UA
    {COUNTRYNO::Serbia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // sr_RS
    {COUNTRYNO::Montenegro, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // sr_ME
    {COUNTRYNO::Croatia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // hr_HR
    {COUNTRYNO::Slovenia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // sk_SK
    {COUNTRYNO::Bosnia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // bs_BA
    {COUNTRYNO::Macedonia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // mk_MK
    {COUNTRYNO::Taiwan, DATE_YMD, SEP_SLASH, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // zh_TW
    {COUNTRYNO::Arabic, DATE_DMY, SEP_PERIOD, TIME_12H, SEP_COLON, SEP_PERIOD,
     SEP_COMMA}, // ar_??
    {COUNTRYNO::Israel, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // he_IL
    {COUNTRYNO::Mongolia, DATE_YMD, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_COMMA,
     SEP_PERIOD}, // mn_MN
    {COUNTRYNO::Tadjikistan, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON,
     SEP_SPACE, SEP_COMMA}, // tg_TJ
    {COUNTRYNO::Turkmenistan, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_SPACE, SEP_COMMA}, // tk_TM
    {COUNTRYNO::Azerbaijan, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON,
     SEP_PERIOD, SEP_COMMA}, // az_AZ
    {COUNTRYNO::Georgia, DATE_DMY, SEP_PERIOD, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // ka_GE
    {COUNTRYNO::Kyrgyzstan, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // ky_KG
    {COUNTRYNO::Uzbekistan, DATE_DMY, SEP_SLASH, TIME_24H, SEP_COLON, SEP_SPACE,
     SEP_COMMA}, // uz_UZ
    //                           | Date fmt | Date separ | Time fmt | Time separ
    //                           | 1000 separ | Dec separ  |
};
...
void DOS_SetCountry(
    uint16_t country_number) // XXX check where this is called, why always sets
                             // to US locale (is US country info OK?)
{
  ...
  for (auto &current : COUNTRY_INFO) {
    if (current.country_number != country_number)
      continue;
...
const uint8_t DOS_DATE_months[] = {0,  31, 28, 31, 30, 31, 30,
                                   31, 31, 30, 31, 30, 31};

uint8_t GetMonthDays(uint8_t month) { return DOS_DATE_months[month]; }
...
void DOS_AddDays(uint8_t days) {
  ...
  if (dos.date.day > monthlimit) {
    if (dos.date.month == 2 && dos.date.year % 4 == 0 &&
        (dos.date.year % 100 != 0 || dos.date.year % 400 == 0)) {
      // leap year
      if (dos.date.day > 29) {
        dos.date.month++;
        dos.date.day -= 29;
...
//       (This can be controlled with the settings "hard drive data rate limit"
//       and "floppy drive data rate limit") Additionally, allow this to vary
//       per-drive so that Drive D: can be as slow as a 2X IDE CD-ROM drive in
//       PIO mode Drive C: can be as slow as a IDE drive in PIO mode and Drive
//       A: can be as slow as a 3.5" 1.44MB floppy disk
//
// This fixes MS-DOS games that crash or malfunction if the disk I/O is too
// fast. This also fixes "380 volt" and prevents the "city animation" from
// loading too fast for its music timing (and getting stuck)
int disk_data_rate =
    2100000; // 2.1MBytes/sec mid 1990s IDE PIO hard drive without SMARTDRV
int floppy_data_rate;

void diskio_delay(Bits value /*bytes*/, int type = -1) {
  if ((type == 0 && floppy_data_rate != 0) ||
      (type != 0 && disk_data_rate != 0)) {
    double scalar;
    double endtime;
...
static inline void overhead() { reg_ip += 2; }
...
void DOS_InitClock() {
  if (IS_PC98_ARCH) {
    ...
    if (reg_ch >= 0x19 && reg_ch <= 0x20)
      dos.date.year += BCD2BIN(reg_ch) * 100;
    else
      dos.date.year += 1900;
    if (dos.date.year < 1980)
      dos.date.year += 100;
  }
}
...
bool DOS_BreakTest(bool print = true) {
  if (DOS_BreakFlag) {
    ...
    /* invoke INT 23h */
    /* NTS: Some DOS programs provide their own INT 23h which then calls INT 21h
     * AH=0x4C inside the handler! Set a flag so that if that happens, the
     * termination handler will throw us an exception to force our way back here
     * after termination completes!
     *
     *      This fixes: PC Mix compiler PCL.EXE
     *
     *      2023/09/28: Some basic debugging with MS-DOS 6.22 shows the INT 23h
     * handler installed by COMMAND.COM does the same thing (INT 21h AH=0x4C)
     *                  which is normally still there unless the DOS application
     * itself replaces the vector.
     *
    ...
    try {
      ...
      /* 2025/01/20: The throw code used to be int(0) but if that were allowed
       * to bubble up to the main function it would be mistaken as an entirely
       * different signal, so instead use int(9000), a number high enough not to
       * conflict but just low enough to prevent Vegeta from saying his
       * well-known meme-worthy phrase. */
      if (x == 9000) {
        DOS_BreakINT23InProgress = false;
        terminint23 = true;
      } else {
    ...
    if (!terminint23) {
      ...
      if (reg_sp == save_sp || (reg_flags & 1) == 0) {
        ...
        /* program does not wish to continue. it used RETF. pop the remaining
         * flags off */
 ...
 * but, their code also assumes that calling into DOS or the BIOS will unmask
 * the IRQ.
 *
 * This fixes "Rebel by Arkham" which masks IRQ 0-7 (PIC port 21h) in a VERY
 * stingy manner!
 *
 ...
 *             ...
 *             int    21h
 *             ...                  ; demo apparently assumes that INT 21h will
 * unmask IRQ 0 when reading, because .... in     al,21h or     al,3Ah        ;
 * mask IRQ 1, 3, 4, and 5 out    21h,al
 *
 * The demo runs fine anyway, but if we do not unmask IRQ 0 at the INT 21h call,
 * the timer never ticks and the demo does not play any music (goldplay style,
 * of course).
 *
 * This means several things. One is that a disk cache (which may provide the
 * file without using INT 13h) could mysteriously prevent the demo from playing
 * music. Future OS changes, where IRQ unmasking during INT 21h could not occur,
 * would also prevent it from working. I don't know what the programmer was
 * thinking, but side effects like that are not to be relied on!
 *
 * On the other hand, perhaps masking the keyboard (IRQ 1) was intended as an
 * anti-debugger trick? You can't break into the demo if you can't trigger the
 * debugger, after all! The demo can still poll the keyboard controller for ESC
 * or whatever.
 *
 * --J.C. */
...
const char *TranslateHostPath(const char *arg, bool next = false) {
  ...
  while (1) {
    ...
    if (!lastpos && !epos)
      strcpy(result, args.substr(0, spos).c_str());
    else
      strcat(result,
             args.substr(lastpos + epos, spos - lastpos - epos).c_str());
    if (arg[spos] == '"') {
    ...
    if (DOS_MakeName(name.c_str(), fullname, &drive) &&
        (!strncmp(Drives[drive]->GetInfo(), "local ", 6) ||
         !strncmp(Drives[drive]->GetInfo(), "CDRom ", 6))) {
      localDrive *ldp = dynamic_cast<localDrive *>(Drives[drive]);
      Overlay_Drive *odp = dynamic_cast<Overlay_Drive *>(Drives[drive]);
      std::string hostname = "";
      if (odp)
        hostname = odp->GetHostName(fullname);
      else if (ldp)
        hostname = ldp->GetHostName(fullname);
      if (hostname.size()) {
        if (arg[spos] == '"')
          strcat(result, "\"");
...
void HostAppRun() {
  ...
  std::string winPath;
  uint8_t drive;
  if (!DOS_MakeName(fullname, winDirNew, &drive))
    return;
  bool net = false;
  ...
  if (GetCurrentDirectory(512, winDirCur) &&
      (net || !strncmp(Drives[drive]->GetInfo(), "local ", 6) ||
       !strncmp(Drives[drive]->GetInfo(), "CDRom ", 6))) {
    ...
    if (!useoverlay && !net) {
      ...
      if (!PathFileExists(winName)) {
        bool olfn = uselfn;
        uselfn = true;
        if (DOS_GetSFNPath(fullname, dir, true) &&
            DOS_MakeName(("\"" + std::string(dir) + "\"").c_str(), winDirNew,
                         &drive)) {
          strcpy(winName, Drives[drive]->GetBaseDir());
          strcat(winName, winDirNew);
        }
    ...
    if (!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(), "local ", 6) ||
      ...
      strcpy(winDirNew,
             useoverlay ? odp->getOverlaydir() : Drives[drive]->GetBaseDir());
      strcat(winDirNew, Drives[drive]->curdir);
    }
    if (!(isalpha(winDirNew[0]) && winDirNew[1] == ':') &&
        winDirNew[0] != '\\') {
    ...
    if (SetCurrentDirectory(winDirNew) || net) {
      ...
      if (temp == 0)
        temp = (DWORD)SHGetFileInfo(
            (std::string(winDirNew) + "\\" + std::string(fullname)).c_str(), 0,
      ...
      if (startwait && hret > 0) {
        ...
        DWORD exitCode = 0;
        GetExitCodeProcess((HANDLE)hret, &exitCode);
        while (GetExitCodeProcess((HANDLE)hret, &exitCode) &&
...
/* called by shell to flush keyboard buffer right before executing the program
 * to avoid having the Enter key in the buffer to confuse programs that act
 * immediately on keyboard input. */
...
static Bitu DOS_21Handler(void) {
  ...
  /* NTS to ognjenmi: Your INT 21h logging patch was modified to log ALL INT 21h
   * calls (the original placement put it after the ignore case below), and is
   * now conditional on whether INT 21h logging is enabled. Also removed
   * unnecessary copying of reg_al and reg_ah to auto type variables. */
  if (log_int21) {
    LOG(LOG_DOSMISC, LOG_DEBUG)("Executing interrupt 21, ah=%x, al=%x", reg_ah,
                                reg_al);
  }

  /* Real MS-DOS behavior:
   *   If HIMEM.SYS is loaded and CONFIG.SYS says DOS=HIGH, DOS will load itself
   * into the HMA area. To prevent crashes, the INT 21h handler down below will
   * enable the A20 gate before executing the DOS kernel. */
  if (DOS_IS_IN_HMA()) {
    if (cpu.pmode &&
        ((GETFLAG_IOPL < cpu.cpl) || GETFLAG(VM))) /* virtual 8086 mode */
      XMS_DOS_LocalA20EnableIfNotEnabled_XMSCALL();
    else
      XMS_DOS_LocalA20EnableIfNotEnabled();
  }

  if (((reg_ah != 0x50) && (reg_ah != 0x51) && (reg_ah != 0x62) &&
       (reg_ah != 0x64)) &&
      (reg_ah < 0x6c)) {
    DOS_PSP psp(dos.psp());
  ...
  if (reg_ah == 0x06 || reg_ah == 0x07) {
    /* does not check CTRL+BREAK. Some DOS programs do not expect to be
     * interrupted with INT 23h if they read */
    /* keyboard input through this and may cause system instability if
     * terminated. This fixes PC-98 text editor VZ.EXE which will leave it's INT
     * 6h handler in memory if interrupted this way, for example. */
    /* See also:
     *   [http://www.ctyme.com/intr/rb-2558.htm] INT 21h AH=6
     *   [http://www.ctyme.com/intr/rb-2559.htm] INT 21h AH=6 DL=FFh
     */
  } else {
    if (((reg_ah >= 0x01 && reg_ah <= 0x0C) ||
         (reg_ah != 0 && reg_ah != 0x4C && reg_ah != 0x31 && dos.breakcheck)) &&
        !DOS_BreakTest())
      return CBRET_NONE;
  }
  ...
  if (reg_ah != 0x4c && reg_ah != 0x51) {
    packerr = false;
    reqwin = false;
  }
  switch (reg_ah) {
  case 0x00: /* Terminate Program */
    /* HACK for demoscene prod
     * parties/1995/wired95/surprisecode/w95spcod.zip/WINNERS/SURP-KLF
     *
     * This demo starts off by popping 3 words off the stack (the third into ES
     * to get the top of DOS memory which it then uses to draw into VGA memory).
     * Since SP starts out at 0xFFFE, that means SP wraps around to start
     * popping values out of the PSP segment.
     *
     * Real MS-DOS will also start the demo with SP at 0xFFFE.
     *
     * The demo terminates with INT 20h.
     *
     * This code will fail since the stack pointer must wrap back around to read
     * the segment, unless we read by popping. */
    if (reg_sp > 0xFFFA) {
      LOG(LOG_DOSMISC,
          LOG_WARN)("DOS:INT 20h/INT 21h AH=00h WARNING, process terminated "
                    "where stack pointer wrapped around 64K");
      ...
      LOG(LOG_DOSMISC, LOG_DEBUG)(
          "DOS:INT 20h/INT 21h AH=00h recovered CS segment %04x", f_cs);

      DOS_Terminate(f_cs, false, 0);
    } else if (dos.version.major >= 7 && uselfn &&
               mem_readw(SegPhys(ss) + reg_sp) >= 0x2700 &&
               mem_readw(SegPhys(ss) + reg_sp + 2) / 0x100 == 0x90 &&
               dos.psp() / 0x100 >= 0xCC)
      /* Wengier: This case fixes the bug that DIR /S from MS-DOS 7+ could crash
       * hard within DOSBox-X. With this change it should now work properly. */
      DOS_Terminate(dos.psp(), false, 0);
    else
      DOS_Terminate(real_readw(SegValue(ss), reg_sp + 2), false, 0);

    if (DOS_BreakINT23InProgress)
      throw int(9000); /* HACK: Ick */
  ...
  case 0x01: /* Read character from STDIN, with echo */
  {
    ...
    reg_al = c;
    if (dos.version.major > 1)
      DOS_WriteFile(STDOUT, &c,
                    &n); /* RBIL: Character may be redirected under DOS 2 + */
    if (dos.version.major == 1)
      dos.echo = false;
  } break;
  case 0x02: /* Write character to STDOUT */
  {
    ...
    // Not in the official specs, but happens nonetheless. (last written
    // character)
    reg_al = (c == 9)
                 ? 0x20
                 : c; // strangely, tab conversion to spaces is reflected here
  } break;
  ...
  case 0x08: /* Direct Character Input, without echo (checks for breaks
                officially :)*/
  {
    uint8_t c;
    uint16_t n = 1;
  ...
  case 0x09: /* Write string to STDOUT */
  {
    ...
    reqwin =
        (str.length() == 42 &&
         !strncmp(str.c_str(), "This program cannot be run in DOS mode.",
                  39)) ||
        (!strncmp(str.c_str(), "This program requires Microsoft Windows.",
                  40)) ||
        (str.length() == 38 &&
         !strncmp(str.c_str(), "This program must be run under Win32", 36));
  ...
  case 0x0c: /* Flush Buffer and read STDIN call */
  {
    ...
    default:
      //              LOG_ERROR("DOS:0C:Illegal Flush STDIN Buffer call
      //              %d",reg_al);
      reg_al = 0;
      break;
    }
  ...
  case 0x0e: /* Select Default Drive */
    DOS_SetDefaultDrive(reg_dl);
    {
      ...
      for (uint16_t i = drive; i < DOS_DRIVES; i++)
        if (Drives[i])
          drive = i + 1;
      reg_al = drive;
    }
  ...
  case 0x0f: /* Open File using FCB */
    ...
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x0f FCB-fileopen used, result:al=%d",
                             reg_al);
    break;
  case 0x10: /* Close File using FCB */
    ...
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x10 FCB-fileclose used, result:al=%d",
                             reg_al);
    break;
  case 0x11: /* Find First Matching File using FCB */
    if (DOS_FCBFindFirst(SegValue(ds), reg_dx))
      ...
      reg_al = 0xFF;
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x11 FCB-FindFirst used, result:al=%d",
                             reg_al);
    break;
  case 0x12: /* Find Next Matching File using FCB */
    if (DOS_FCBFindNext(SegValue(ds), reg_dx))
      ...
      reg_al = 0xFF;
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x12 FCB-FindNext used, result:al=%d",
                             reg_al);
    break;
  case 0x13: /* Delete File using FCB */
    if (DOS_FCBDeleteFile(SegValue(ds), reg_dx))
      reg_al = 0x00;
    else
      reg_al = 0xFF;
  ...
  case 0x16: /* Create or truncate file using FCB */
    if (DOS_FCBCreate(SegValue(ds), reg_dx))
      reg_al = 0x00;
    else
      reg_al = 0xFF;
  ...
  case 0x17: /* Rename file using FCB */
    if (DOS_FCBRenameFile(SegValue(ds), reg_dx))
      reg_al = 0x00;
    else
      reg_al = 0xFF;
    break;
  ...
  case 0x21: /* Read random record from FCB */
  {
    ...
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x21 FCB-Random read used, result:al=%d",
                             reg_al);
    break;
  case 0x22: /* Write random record to FCB */
  {
    ...
    LOG(LOG_FCB, LOG_NORMAL)("DOS:0x22 FCB-Random write used, result:al=%d",
                             reg_al);
    break;
  case 0x23: /* Get file size for FCB */
    if (DOS_FCBGetFileSize(SegValue(ds), reg_dx))
      reg_al = 0x00;
    else
      reg_al = 0xFF;
    break;
  ...
  case 0x25: /* Set Interrupt Vector */
    ...
    if (IS_PC98_ARCH && reg_al == 0x60 && !strcmp(RunningProgram, "SNDCDDRV") &&
        real_readd(SegValue(ds), reg_dx + 47) == 0x0fa6268b &&
        real_readd(SegValue(ds), reg_dx + 52) == 0x0fa4168e) {
  ...
  case 0x26: /* Create new PSP */
    /* TODO: DEBUG.EXE/DEBUG.COM as shipped with MS-DOS seems to reveal a bug
     * where, when DEBUG.EXE calls this function and you're NOT loading a
     * program to debug, the CP/M CALL FAR instruction's offset field will be
     * off by 2. When does that happen, and how do we emulate that? */
  ...
  case 0x27: /* Random block read from FCB */
    ...
    LOG(LOG_FCB, LOG_NORMAL)(
        "DOS:0x27 FCB-Random(block) read used, result:al=%d", reg_al);
    break;
  case 0x28: /* Random Block write to FCB */
    reg_al = DOS_FCBRandomWrite(SegValue(ds), reg_dx, &reg_cx, false);
    LOG(LOG_FCB, LOG_NORMAL)(
        "DOS:0x28 FCB-Random(block) write used, result:al=%d", reg_al);
    break;
  ...
  case 0x2a: /* Get System Date */
    ...
    if (IS_PC98_ARCH) {
      ...
      // Real hardware testing: DOS appears to read the CMOS once at startup and
      // then the date is stored and counted internally. It does not read via
      // INT 1Ah. This means it is possible for DOS and the BIOS 1Ah/CMOS to
      // have totally different time and date!
      reg_cx = dos.date.year;
      reg_dh = dos.date.month;
      reg_dl = dos.date.day;

      // calculate day of week (we could of course read it from CMOS, but never
      // mind)
      ...
      weekday = (year + year / 4u - year / 100u + year / 400u +
                 (13u * month + 8u) / 5u + day) %
                7;
  ...
  case 0x2b: /* Set System Date */
  {
    ...
    int maxday[12] = {31, 28, 31, 30, 31, 30, 31,
                      31, 30, 31, 30, 31}; /* cannot be const, see maxday[1]++
                                              increment below! */
    ...
    if (reg_cx < 1980 || reg_cx > 9999 || reg_dh < 1 || reg_dh > 12 ||
        reg_dl < 1 || reg_dl > maxday[reg_dh - 1]) {
      reg_al = 0xff; // error!
      break;         // done
    }
  ...
  case 0x2c: { /* Get System Time */
    ...
    if (IS_PC98_ARCH) {
      ...
      // It turns out according to real hardware, that DOS reads the date and
      // time once on startup and then relies on the BIOS_TIMER counter after
      // that for time, and caches the date. So the code prior to the April 2023
      // change was correct after all.
      reg_ax = 0; // get time
      CALLBACK_RunRealInt(0x1a);
      if (reg_al)
        DOS_AddDays(reg_al);
      ...
      Bitu time =
          (Bitu)((100.0 / ((double)PIT_TICK_RATE / 65536.0)) * (double)ticks);
  ...
  case 0x2d: /* Set System Time */
  {
    ...
    if (IS_PC98_ARCH) {
      ...
      const uint32_t csec =
          (((((reg_ch * 60u) + reg_cl) * 60u + reg_dh) * 100u) + reg_dl);
      const uint32_t ticks =
          (uint32_t)((double)csec * ((double)PIT_TICK_RATE / 6553600.0));
      ...
      // could set directly, but setting is safer to do via dedicated call (at
      // least in theory)
  ...
  case 0x31: /* Terminate and stay resident */
    ...
    // crazy like that. --Jonathan C.
    {
      ...
      if (psp_mcb.GetPSPSeg() == 0 /*free block?*/) {
        ...
        LOG(LOG_DOSMISC,
            LOG_DEBUG)("Calling program asking to terminate and stay resident "
                       "has apparently freed its own PSP segment");
  ...
  { /* Officially a dpb should be returned as well. The disk detection part is
       implemented */
  case_0x32_fallthrough:
    ...
    if (drive < DOS_DRIVES && Drives[drive] && !Drives[drive]->isRemovable()) {
      ...
      if (mem_readw(SegPhys(ds) + reg_bx + 0x1F) == 0xFFFF) {
        uint32_t bytes_per_sector, sectors_per_cluster, total_clusters,
            free_clusters;
        if (DOS_GetFreeDiskSpace32(reg_dl, &bytes_per_sector,
                                   &sectors_per_cluster, &total_clusters,
                                   &free_clusters))
  ...
  case 0x33: /* Extended Break Checking */
    ...
    case 5:
      reg_dl = 3;
      break; /* Drive C: (TODO: Make configurable, or else initially set to Z:
                then re-set to the first drive mounted */
  ...
  case 0x36: /* Get Free Disk Space */
  {
    ...
    if (DOS_GetFreeDiskSpace(reg_dl, &bytes, &sectors, &clusters, &free)) {
      ...
      if (drive == 0)
        ...
        drive--;
      if (drive < 2) {
        // floppy drive, non-present drivesdisks issue floppy check through
        // int24 (critical error handler); needed for Mixed up Mother Goose
        // (hook)
  ...
  case 0x37: /* Get/Set Switch char Get/Set Availdev thing */
    // TODO  Give errors for these functions to see if anyone actually uses this
    // shit-
    switch (reg_al) {
    case 0:
      reg_al = 0;
    ...
    case 2:
      reg_al = 0;
      reg_dl = 0xff;
      break; /* AVAILDEV \DEV\ prefix optional */
    case 3:
      LOG(LOG_DOSMISC,
          LOG_DEBUG)("DOS:0x37:Attempted to set AVAILDEV \\DEV\\ prefix use");
      reg_al = 0;
      break;
    }
    break;
  ...
  case 0x3a: /* RMDIR Remove directory */
    ...
    if (DOS_RemoveDir(name1)) {
      ...
      reg_ax = dos.errorcode;
      CALLBACK_SCF(true);
      LOG(LOG_DOSMISC, LOG_NORMAL)("Remove dir failed on %s with error %X",
                                   name1, dos.errorcode);
    }
    force_sfn = false;
    break;
  ...
  case 0x3d: /* OPEN Open existing file */
  {
    ...
    if (IS_DOSV && IS_DOS_JAPANESE) {
      ...
      if (DOS_CheckExtDevice(name_start, false) == 0) {
        if (dos.im_enable_flag) {
          if ((DOSV_GetFepCtrl() & DOSV_FEP_CTRL_IAS) &&
              !strncmp(name_start, "$IBMAIAS", 8)) {
            ias_handle = IAS_DEVICE_HANDLE;
            reg_ax = IAS_DEVICE_HANDLE;
            force_sfn = false;
            CALLBACK_SCF(false);
            break;
          } else if ((DOSV_GetFepCtrl() & DOSV_FEP_CTRL_MSKANJI) &&
                     !strncmp(name_start, "MS$KANJI", 8)) {
        ...
        if (!strncmp(name_start, "$IBMAFNT", 8) ||
            (ttf_dosv && !strncmp(name_start, "$IBMADSP", 8))) {
#else
        if (!strncmp(name_start, "$IBMAFNT", 8)) {
#endif
    ...
    if (DOS_OpenFile(name1, reg_al, &reg_ax)) {
      ...
      if (ttf.inUse && wpType == 1) {
        ...
        if (len > 10 &&
            !strcmp(name1 + len - 11, "\\VGA512.CHM")) // Test for VGA512.CHM
  ...
  case 0x3e: /* CLOSE Close file */
  {
    ...
    uint8_t drive = (handle != 0xff && Files[handle])
                        ? Files[handle]->GetDrive()
                        : DOS_DRIVES;
    unmask_irq0 |= disk_io_unmask_irq0;
  ...
  case 0x3f: /* READ Read from file or device */
    ...
    /* TODO: If handle is STDIN and not binary do CTRL+C checking */
    {
      ...
      if (((uint32_t)toread + (uint32_t)reg_dx) > 0xFFFFUL &&
        ...
        uint16_t nuread =
            (uint16_t)(0x10000UL -
                       (reg_dx & 0xF)); /* FIXME: If MS-DOS 5.0 truncates it any
                                           farther I need to know! */

        if (nuread > toread)
          nuread = toread;
        LOG_MSG("INT 21h READ warning: DX=%04xh CX=%04xh exceeds 64KB, "
                "truncating to %04xh",
                reg_dx, toread, nuread);
        toread = nuread;
      }
      ...
      if (handle >= DOS_FILES || !Files[handle] || !Files[handle]->IsOpen()) {
        ...
        fRead =
            !(((DOS_ExtDevice *)Files[handle])
                  ->CallDeviceFunction(4, 26, SegValue(ds), reg_dx, toread) &
              0x8000);
#if defined(USE_TTF)
  ...
  case 0x40: /* WRITE Write to file or device */
    unmask_irq0 |= disk_io_unmask_irq0;
    {
      ...
      if (((uint32_t)towrite + (uint32_t)reg_dx) > 0xFFFFUL &&
        ...
        if (nuwrite > towrite)
          nuwrite = towrite;
        LOG_MSG("INT 21h WRITE warning: DX=%04xh CX=%04xh exceeds 64KB, "
                "truncating to %04xh",
                reg_dx, towrite, nuwrite);
        towrite = nuwrite;
      }
      ...
      packerr =
          reg_bx == 2 && towrite == 22 &&
          !strncmp((char *)dos_copybuf, "Packed file is corrupt", towrite);
      fWritten = (packerr && !(i4dos && !shellrun) &&
                  (!autofixwarn || (autofixwarn == 2 && infix == 0) ||
                   (autofixwarn == 1 && infix == 1)));
      if (!fWritten) {
        uint32_t handle = RealHandle(reg_bx);
        ...
        if (handle >= DOS_FILES || !Files[handle] || !Files[handle]->IsOpen()) {
          ...
          fWritten =
              !(((DOS_ExtDevice *)Files[handle])
                    ->CallDeviceFunction(8, 26, SegValue(ds), reg_dx, towrite) &
                0x8000);
        } else {
  ...
  case 0x41: /* UNLINK Delete file */
    ...
    if (DOS_UnlinkFile(name1)) {
      diskio_delay_drive((name1[1] == ':') ? toupper(name1[0]) - 'A'
                                           : DOS_GetDefaultDrive(),
                         1024);
      CALLBACK_SCF(false);
    } else {
  ...
  case 0x44: /* IOCTL Functions */
    if (ias_handle != 0 && ias_handle == reg_bx) {
      ...
      if (reg_al == 0) {
        ...
        real_writew(SegValue(ds), reg_dx,
                    DOSV_GetFontHandlerOffset(DOSV_MSKANJI_API));
  ...
  case 0x4b: /* EXEC Load and/or execute program */
  {
    ...
    if (dos_a20_disable_on_exec) {
      if (cpu.pmode && ((GETFLAG_IOPL < cpu.cpl) || GETFLAG(VM))) {
        /* We're running in virtual 8086 mode. Ideally the protected mode kernel
         * would virtualize port 92h, but it seems Windows 3.1 does not do that.
         * Shutting off the A20 gate in protected mode will cause the kernel to
         * CRASH. However Windows 3.1 does intercept HIMEM.SYS calls to
         * enable/disable the A20 gate, so we can accomplish the same effect
         * that way instead and remain compatible with Windows 3.1. */
  ...
  case 0x52: {         /* Get list of lists */
    ...
    reg_bx = RealOff(addr);
    LOG(LOG_DOSMISC,
        LOG_NORMAL)("Call is made for list of lists - let's hope for the best");
    break;
  }
  ...
  case 0x58: /* Get/Set Memory allocation strategy */
    ...
    default:
      LOG(LOG_DOSMISC, LOG_ERROR)(
          "DOS:58:Not Supported Set//Get memory allocation call %X", reg_al);
      reg_ax = 1;
      CALLBACK_SCF(true);
    }
    break;
  ...
  case 0x5c: { /* FLOCK File region locking */
    ...
    // LOG_MSG("LockFile: BX=%d, AL=%d, POS=%d, size=%d", reg_bx, reg_al, pos,
    // size);
    if (!enable_share_exe) {
  ...
  case 0x5e: /* Network and printer functions */
  {
    ...
    if (net) { // Get machine name
...
#endif
      {
        ...
        if (!strcmp(RunningProgram, "4DOS") ||
            (reg_ip == 0xeb31 &&
             (reg_sp == 0xc25e ||
              reg_sp == 0xc26e))) { // 4DOS expects it to be 0 terminated (not
                                    // documented)
          name1[16] = 0;
  ...
  case 0x5f: /* Network redirection */
    ...
    case 0x35: // Peek pipe
    {
      ...
      if (Network_PeekNamedPipe(reg_bx, dos_copybuf, reg_cx, reg_cx, reg_si,
                                reg_dx, reg_di, reg_ax)) {
    ...
    case 0x36: // Transcate pipe
      ...
      if (Network_TranscateNamedPipe(reg_bx, dos_copybuf_second, reg_cx,
                                     dos_copybuf, reg_dx, reg_cx, reg_ax)) {
  ...
  case 0x64: /* Set device driver lookahead flag */
    ...
    break;
  case 0x65: /* Get extended country information and a lot of other useless
                shit*/
  { /* Todo maybe fully support this for now we set it standard for USA */
    LOG(LOG_DOSMISC, LOG_NORMAL)("DOS:65:Extended country information call %X",
                                 reg_ax);
    ...
    case 0x01:
      ...
      if (!countryNo) {
        if (IS_PC98_ARCH)
          ...
          char buffer[128];
          if (GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_ICOUNTRY, buffer,
                            128)) {
      ...
      if (reg_cx > 0x06)
        mem_writew(data + 0x05, dos.loaded_codepage);
      if (reg_cx > 0x08) {
    ...
    case 0x22: /* Capitalize ASCIZ string */
      ...
      if (reg_al == 0x21)
        ...
        len = mem_strlen(data); /* Is limited to 1024 */
      ...
      if (len > DOS_COPYBUFSIZE - 1)
        ...
        for (Bitu count = 0; count < len; count++)
          dos_copybuf[count] = (uint8_t)toupper(
  ...
  case 0x66: /* Get/Set global code page table  */
...
#endif
      {
        LOG(LOG_DOSMISC, LOG_ERROR)("DOS:Setting code page table is not "
                                    "supported for non-TrueType font output");
        CALLBACK_SCF(true);
        dos.errorcode = 2;
        reg_ax = dos.errorcode;
        break;
    ...
    default:
      ...
      CALLBACK_SCF(true);
      LOG(LOG_DOSMISC,
          LOG_ERROR)("DOS:Invalid code page subfunction requested");
      break;
    }
    break;
  case 0x67: /* Set handle count */
    /* Weird call to increase amount of file handles needs to allocate memory if
     * >20 */
    {
      DOS_PSP psp(dos.psp());
  ...
  case 0x6a: /* Commit file */
    // Note: Identical to AH=68h in DOS 5.0-6.0; not known whether this is the
    // case in DOS 4.x
  ...
  case 0x6d: /* ROM - Find first ROM program */
    LOG(LOG_DOSMISC,
        LOG_ERROR)("DOS:ROM - Find first ROM program not implemented");
    goto default_fallthrough;
  case 0x6e: /* ROM - Find next ROM program */
    LOG(LOG_DOSMISC,
        LOG_ERROR)("DOS:ROM - Find next ROM program not implemented");
    goto default_fallthrough;
  ...
  case 0x71: /* Unknown probably 4dos detection */
    LOG(LOG_DOSMISC,
        LOG_NORMAL)("DOS:MS-DOS 7+ long file name support call %2X", reg_al);
    if (!uselfn) {
      reg_ax = 0x7100;
  ...
  case 0x73:
    if (dos.version.major < 7) { // MS-DOS 7+ only for AX=73xxh
      ...
      if (drive < DOS_DRIVES && Drives[drive] &&
          !Drives[drive]->isRemovable() && reg_cx >= 0x3F) {
        fatDrive *fdp;
            ...
            if (bpb.is_fat32()) {
              ...
              /* first 24 bytes after len is DPB */
              {
                const uint32_t srcptr =
                    (dos.tables.dpb << 4) + (drive * dos.tables.dpb_size);
              ...
              mem_writed(ptr + 0x1B,
                         0xFFFFFFFF); // ptr to next DPB if Windows 95 magic SI
                                      // signature (TODO)
              mem_writew(ptr + 0x1F,
                         2); // cluster to start searching when writing (FIXME)
              ...
              mem_writed(
                  ptr + 0x2B,
                  fdp->GetFirstClusterOffset()); /* apparently cluster offset
                                                    relative to the disk not
                                                    volume */
      ...
      if (DOS_GetFreeDiskSpace32(reg_dl, &bytes_per_sector,
        ...
        info->available_sectors_on_drive =
            sectors_per_cluster * (freec ? freec : free_clusters);
        info->total_sectors_on_drive =
            sectors_per_cluster * (totalc ? totalc : total_clusters);
      ...
      LOG(LOG_DOSMISC,
          LOG_ERROR)("DOS:Unhandled call %02X al=%02X (MS-DOS 7.x function)",
                     reg_ah, reg_al);
      CALLBACK_SCF(true);
      reg_ax = 0xffff; // FIXME
    }
  ...
  default_fallthrough:
    if (reg_ah < 0x6b)
      LOG(LOG_DOSMISC, LOG_ERROR)(
          "DOS:Unhandled call %02X al=%02X. Set al to default of 0", reg_ah,
          reg_al); // Less errors. above 0x6c the functions are simply always
                   // skipped, only al is zeroed, all other registers untouched
    reg_al = 0x00; /* default value */
    break;
  }

  /* if INT 21h involves any BIOS calls that need the timer, emulate the fact
   * that the BIOS might unmask IRQ 0 as part of the job (especially INT 13h
   * disk I/O).
   *
   * Some DOS games & demos mask interrupts at the PIC level in a stingy manner
   * that apparently assumes DOS/BIOS will unmask some when called.
   *
   * Examples:
   *   Rebel by Arkham (without this fix, timer interrupt will not fire during
   * demo and therefore music will not play). */
  if (unmask_irq0)
...
static Bitu BIOS_1BHandler(void) {
  ...
  // MS-DOS installs an INT 1Bh handler that sets the "break" flag and then
  // returns immediately. MS-DOS 6.22's interrupt handler is literally two
  // instructions:
  //      MOV BYTE PTR CS:[000Ch],03h
  //      IRET

  /* take note (set flag) and return */
  /* FIXME: Don't forget that on "BOOT" this handler should be unassigned,
   * though having it assigned to the guest OS causes no harm. */
  DOS_BreakFlag = true;
  ...
  while (sectorCnt--) {
    ...
    if (read) {
      if (drive->readSector(sectorNum++, &sectorBuf))
        return 0x0408;
      for (i = 0; i < 512; i++)
        real_writeb(bufferSeg, bufferOff++, sectorBuf[i]);
    } else {
      for (i = 0; i < 512; i++)
        sectorBuf[i] = real_readb(bufferSeg, bufferOff++);
...
static Bitu DOS_25Handler_Actual(bool fat32) {
  if (reg_al >= DOS_DRIVES || !Drives[reg_al] ||
      Drives[reg_al]->isRemovable()) {
    reg_ax = 0x8002;
    SETFLAGBIT(CF, true);
    ...
    if (sector_count != 0 && sector_size != 0) {
      ...
      if (fat32) {
        LOG(LOG_DOSMISC, LOG_DEBUG)(
            "INT 21h AX=7305h READ: sector=%lu count=%lu ptr=%lx method='%s'",
            (unsigned long)sector_num, (unsigned long)req_count,
            (unsigned long)ptr, method);
      } else {
        LOG(LOG_DOSMISC,
            LOG_DEBUG)("INT 25h READ: sector=%lu count=%lu ptr=%lx method='%s'",
                       (unsigned long)sector_num, (unsigned long)req_count,
                       (unsigned long)ptr, method);
      }
    ...
    /* MicroProse installer hack, inherited from DOSBox SVN, as a fallback if
     * INT 25h emulation is not available for the drive. */
    if (reg_cx == 1 && reg_dx == 0 && reg_al >= 2) {
      ...
      LOG(LOG_DOSMISC, LOG_NORMAL)(
          "int 25 called but not as disk detection drive %u", reg_al);
      reg_ax = 0x8002;
      SETFLAGBIT(CF, true);
    }
  }
...
static Bitu DOS_25Handler(void) { return DOS_25Handler_Actual(false); }
...
static Bitu DOS_26Handler_Actual(bool fat32) {
  if (reg_al >= DOS_DRIVES || !Drives[reg_al] ||
    ...
    if (sector_count != 0 && sector_size != 0) {
      ...
      if (fat32) {
        LOG(LOG_DOSMISC, LOG_DEBUG)(
            "INT 21h AX=7305h WRITE: sector=%lu count=%lu ptr=%lx method='%s'",
            (unsigned long)sector_num, (unsigned long)req_count,
            (unsigned long)ptr, method);
      } else {
        LOG(LOG_DOSMISC, LOG_DEBUG)(
            "INT 26h WRITE: sector=%lu count=%lu ptr=%lx method='%s'",
            (unsigned long)sector_num, (unsigned long)req_count,
            (unsigned long)ptr, method);
      }
...
static Bitu DOS_26Handler(void) { return DOS_26Handler_Actual(false); }
...
// XOR AX,AX
// MOV ES,AX
// MOV AL,ES:[0712h]            ; AX = BYTE [0x70:0x12] zero extend (ex. 0x18 ==
// 24) INC AX                       ; AX++                              (ex.
// becomes 0x19 == 25) MOV DX,AX SHL DX,1 SHL DX,1                     ; DX *= 4
// ADD DX,AX                    ; DX += AX     equiv. DX = AX * 5
// MOV CL,4h
// SHL DX,CL                    ; DX <<= 4     equiv. DX = AX * 0x50  or  DX =
// AX * 80
// ...
// MOV AX,0A200h
// MOV ES,AX
...
// REP STOSW
//
// When the routine is done, the graphics layer is obscured by text character
// cells that represent all black (filled in) so that the game can later "punch
// out" the regions of the graphics layer it wants you to see. TH02 relies on
// this as well to flash the screen and open from the center to show the title
// screen. During gameplay, the text layer is used to obscure sprite overdraw
// when a sprite is partially off-screen as well as hidden tile data on the
// right hand half of the screen that the game read/write copies through the GDC
// pattern/tile registers to make the background. When the text layer is not
// present it's immediately apparent that the sprite renderer makes no attempt
// to clip sprites within the screen, but instead relies on the text overlay to
// hide the overdraw.
//
// this means that on PC-98 one of two things are true. either:
//  - NEC's variation of MS-DOS loads the base kernel higher up (perhaps at
//  0x80:0x00?)
//    and the BIOS data area lies from 0x40:00 to 0x7F:00
//
//    or
//
//  - NEC's variation loads at 0x70:0x00 (same as IBM PC MS-DOS) and Touhou
//  Project
...
void INT10_WriteChar_viaRealInt(uint8_t chr, uint8_t attr, uint8_t page,
                                uint16_t count, bool showattr);
void INT10_ScrollWindow_viaRealInt(uint8_t rul, uint8_t cul, uint8_t rlr,
                                   uint8_t clr, int8_t nlines, uint8_t attr,
                                   uint8_t page);
...
bool set_ver(char *s) {
  ...
  if (major >= 0 && major < 100) {
    if (*s == '.' || *s == ' ') {
      s++;
      if (isdigit(*s) && *(s - 1) == '.' && strlen(s) > 2)
        *(s + 2) = 0;
      int minor = isdigit(*s) ? (*(s - 1) == '.' && strlen(s) == 1 ? 10 : 1) *
                                    strtoul(s, (char **)(&s), 10)
                              : -1;
      while (minor > 99)
        minor /= 10;
...
static Bitu DOS_29Handler(void) {
  ...
  if (!int29h_data.ansi.esc) {
    if (reg_al == '\033') {
      ...
      bool scroll = false;
      /* Some sort of "hack" now that '\n' doesn't set col to 0 (int10_char.cpp
       * old chessgame) */
      ...
      reg_ax = tmp_ax;
      int29h_data.lastwrite = reg_al;
      /* use ansi attribute if ansi is enabled, otherwise use DOS default
       * attribute*/
      page = real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE);
      col = CURSOR_POS_COL(page);
    ...
    case 'M': /* scrolling UP*/
    default:
      LOG(LOG_IOCTL, LOG_NORMAL)("ANSI: unknown char %c after an esc",
                                 reg_al); /*prob () */
      ClearAnsi29h();
      break;
    }
    ...
    if (int29h_data.ansi.key) {
      ...
      case '8':
      case '9':
        int29h_data.ansi.data[int29h_data.ansi.numberofarg] =
            10 * int29h_data.ansi.data[int29h_data.ansi.numberofarg] +
            (reg_al - '0');
        break;
      ...
      case 'm': /* SGR */
        for (i = 0; i <= int29h_data.ansi.numberofarg; i++) {
          ...
          case 0:                         /* normal */
            int29h_data.ansi.attr = 0x07; // Real ansi does this as well.
                                          // (should do current defaults)
          ...
          case 5: /* blinking */
            ...
            break;
          case 7:                         /* reverse */
            int29h_data.ansi.attr = 0x70; // Just like real ansi. (should do use
      ...
      case 'f':
      case 'H':                         /* Cursor Pos*/
        if (!int29h_data.ansi.warned) { // Inform the debugger that ansi is
                                        // used.
        ...
        if (int29h_data.ansi.data[0] == 0)
          int29h_data.ansi.data[0] = 1;
        if (int29h_data.ansi.data[1] == 0)
          int29h_data.ansi.data[1] = 1;
        if (int29h_data.ansi.data[0] > nrows)
          int29h_data.ansi.data[0] = (uint8_t)nrows;
        if (int29h_data.ansi.data[1] > ncols)
          int29h_data.ansi.data[1] = (uint8_t)ncols;
        INT10_SetCursorPos_viaRealInt(--(int29h_data.ansi.data[0]),
                                      --(int29h_data.ansi.data[1]),
                                      page); /*ansi=1 based, int10 is 0 based */
        ClearAnsi29h();
        break;
        /* cursor up down and forward and backward only change the row or the
         * col not both */
      case 'A': /* cursor up*/
        col = CURSOR_POS_COL(page);
        row = CURSOR_POS_ROW(page);
      ...
      case 'J': /*erase screen and move cursor home*/
        if (int29h_data.ansi.data[0] == 0)
          int29h_data.ansi.data[0] = 2;
        if (int29h_data.ansi.data[0] !=
            2) { /* every version behaves like type 2 */
          LOG(LOG_IOCTL,
              LOG_NORMAL)("ANSI: esc[%dJ called : not supported handling as 2",
                          int29h_data.ansi.data[0]);
        }
        INT10_ScrollWindow_viaRealInt(0, 0, 255, 255, 0, int29h_data.ansi.attr,
                                      page);
        ClearAnsi29h();
      ...
      case 'I': /* RESET MODE */
        LOG(LOG_IOCTL,
            LOG_NORMAL)("ANSI: set/reset mode called(not supported)");
        ClearAnsi29h();
        break;
      case 'u': /* Restore Cursor Pos */
        INT10_SetCursorPos_viaRealInt(int29h_data.ansi.saverow,
                                      int29h_data.ansi.savecol, page);
        ClearAnsi29h();
        break;
      ...
      case 'K': /* erase till end of line (don't touch cursor) */
        ...
        row = CURSOR_POS_ROW(page);
        ncols = real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
        INT10_WriteChar_viaRealInt(' ', int29h_data.ansi.attr, page,
                                   ncols - col,
                                   true); // Use this one to prevent scrolling
                                          // when end of screen is reached
        // for(i = col;i<(Bitu) ncols; i++) INT10_TeletypeOutputAttr('
        // ',ansi.attr,true);
        INT10_SetCursorPos_viaRealInt(row, col, page);
        ClearAnsi29h();
        break;
      case 'M': /* delete line (NANSI) */
        ...
        ncols = real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
        nrows = real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) + 1;
        INT10_ScrollWindow_viaRealInt(
            row, 0, nrows - 1, ncols - 1,
            int29h_data.ansi.data[0] ? -int29h_data.ansi.data[0] : -1,
            int29h_data.ansi.attr, 0xFF);
        ClearAnsi29h();
        break;
      case '>':
        break;
        ...
        if (J3_IsJapanese()) {
          if (reg_al == 'h') {
            if (int29h_data.ansi.data[int29h_data.ansi.numberofarg] == 5) {
              // disable cursor
              int29h_data.keepcursor =
                  real_readw(BIOSMEM_SEG, BIOSMEM_CURSOR_TYPE);
              INT10_SetCursorShape(0x20, 0x0f);
            }
          } else if (reg_al == 'l') {
            if (int29h_data.ansi.data[int29h_data.ansi.numberofarg] == 5) {
              // enable cursor
              INT10_SetCursorShape(int29h_data.keepcursor >> 8,
                                   int29h_data.keepcursor & 0xff);
            }
          } else if (reg_al == 'p') {
...
public:
  void DOS_Write_HMA_CPM_jmp(void) {
    ...
    // this is needed for "F01D:FEF0" to be a valid jmp whether or not A20 is
    // enabled
    if (dos_in_hma && cpm_compat_mode != CPM_COMPAT_OFF &&
  ...
  DOS(Section *configuration) : Module_base(configuration) {
    ...
    if (*ver) {
      if (set_ver((char *)ver)) {
        ...
        } else if (dos.version.major == 0 || dos.version.major > 8 ||
                   dos.version.minor > 90)
          LOG_MSG(
              "WARNING: DOS version %u.%u is unusual, may confuse DOS programs",
              dos.version.major, dos.version.minor);
      }
    }
    ...
    if (::disk_data_rate < 0) {
      if (pcibus_enable)
        ...
        ::disk_data_rate =
            3500000; /* Probably an average IDE data rate for early 1990s ISA
                        IDE controllers in PIO mode */
    }
    ...
    if (prefix.size())
      prefix_local = prefix + prefix_local.substr(3),
      prefix_overlay = prefix + prefix_overlay.substr(3);

    maxfcb = 100;
    DOS_FILES = 200;
    Section_prop *config_section =
        static_cast<Section_prop *>(control->GetSection("config"));
    if (config_section != NULL && !control->opt_noconfig &&
        !control->opt_securemode && !control->SecureMode()) {
      DOS_FILES = (unsigned int)config_section->Get_int("files");
      if (DOS_FILES == 0) {
        const unsigned int sz =
            std::min((unsigned int)MEM_ConventionalPages(), 0xA0u);
        DOS_FILES = 5u + ((200u - 5u) * sz) / 0xA0u;
      }
      if (DOS_FILES < 8)
        DOS_FILES = 8;
      else if (DOS_FILES > 255)
        DOS_FILES = 255;
      ...
      if (maxfcb < 1)
        ...
        maxfcb = 255;
      char *dosopt = (char *)config_section->Get_string("dos"),
           *r = strchr(dosopt, ',');
      if (r == NULL) {
        if (!strcasecmp(trim(dosopt), "high"))
          dos_in_hma = true;
        else if (!strcasecmp(trim(dosopt), "low"))
          dos_in_hma = false;
        if (!strcasecmp(trim(dosopt), "umb"))
          dos_umb = true;
        else if (!strcasecmp(trim(dosopt), "noumb"))
          dos_umb = false;
      } else {
        *r = 0;
        if (!strcasecmp(trim(dosopt), "high"))
          ...
          dos_in_hma = false;
        *r = ',';
        if (!strcasecmp(trim(r + 1), "umb"))
          dos_umb = true;
        else if (!strcasecmp(trim(r + 1), "noumb"))
          dos_umb = false;
      }
      ...
      if ((!strcasecmp(numlock, "off") && startup_state_numlock) ||
          (!strcasecmp(numlock, "on") && !startup_state_numlock))
        SetNumLock();
#endif
    }
    LOG(LOG_MISC, LOG_DEBUG)("files=%u fcbs=%u", (unsigned int)DOS_FILES,
                             (unsigned int)maxfcb);
    char *r;
#if defined(WIN32)
    unsigned int cp = GetACP();
    const char *cstr = (control->opt_noconfig || !config_section)
                           ? ""
                           : (char *)config_section->Get_string("country");
    r = (char *)strchr(cstr, ',');
    if ((r == NULL || !*(r + 1) || atoi(trim(r + 1)) == cp ||
         (atoi(trim(r + 1)) == 951 && cp == 950)) &&
        GetDefaultCP() == 437) {
      if (cp == 950 && !chinasea)
        makestdcp950table();
      if (cp == 951 && chinasea)
    ...
    freed_mcb_allocate_on_resize =
        section->Get_bool("resized free memory block becomes allocated");
    shell_keyboard_flush =
        section->Get_bool("command shell flush keyboard buffer");
    ...
    a20_off_if_loading_low =
        section->Get_bool("turn off a20 gate on load if loadfix needed");
    ...
    minimum_dos_initial_private_segment =
        section->Get_hex("minimum dos initial private segment");
    dos_con_use_int16_to_detect_input =
        section->Get_bool("con device use int 16h to detect keyboard input");
    dbg_zero_on_dos_allocmem =
        section->Get_bool("zero memory on int 21h memory allocation");
    MAXENV = (unsigned int)section->Get_int(
        "maximum environment block size on exec");
    ENV_KEEPFREE = (unsigned int)section->Get_int(
        "additional environment block size on exec");
    enable_dummy_device_mcb = section->Get_bool("enable dummy device mcb");
    int15_wait_force_unmask_irq =
        section->Get_bool("int15 wait force unmask irq");
    ...
    startnopause = section->Get_bool("startnopause");
    const char *dos_clipboard_device_enable =
        section->Get_string("dos clipboard device enable");
    dos_clipboard_device_access =
        !strcasecmp(dos_clipboard_device_enable, "disabled")
            ? 0
            : (!strcasecmp(dos_clipboard_device_enable, "read")
                   ? 2
                   : (!strcasecmp(dos_clipboard_device_enable, "write")
                          ? 3
                          : (!strcasecmp(dos_clipboard_device_enable, "full") ||
                                     !strcasecmp(dos_clipboard_device_enable,
                                                 "true")
                                 ? 4
                                 : 1)));
    dos_clipboard_device_name =
        section->Get_string("dos clipboard device name");
    ...
    if (dos_clipboard_device_access) {
      bool valid = true;
      char ch[] = "*? .|<>/\\\"";
      if (!*dos_clipboard_device_name ||
          strlen(dos_clipboard_device_name) > 8 ||
          !strcasecmp(dos_clipboard_device_name, "con") ||
          !strcasecmp(dos_clipboard_device_name, "nul") ||
          !strcasecmp(dos_clipboard_device_name, "prn"))
        valid = false;
      else
      ...
      dos_clipboard_device_name =
          valid ? upcase((char *)dos_clipboard_device_name)
                : (char *)dos_clipboard_device_default;
      LOG(LOG_DOSMISC, LOG_NORMAL)(
          "DOS clipboard device (%s access) is enabled with the name %s\n",
          dos_clipboard_device_access == 1
              ? "dummy"
              : (dos_clipboard_device_access == 2
                     ? "read"
                     : (dos_clipboard_device_access == 3 ? "write" : "full")),
          dos_clipboard_device_name);
      ...
      mainMenu.get_item("clipboard_device")
          .set_text(text + ": " + std::string(dos_clipboard_device_name))
    ...
    autofixwarn = autofixwarning == "false" || autofixwarning == "0" ||
                          autofixwarning == "none"
                      ? 0
                      : (autofixwarning == "a20fix"
                             ? 1
                             : (autofixwarning == "loadfix" ? 2 : 3));
    char *cpstr = (char *)section->Get_string("customcodepage");
    r = (char *)strchr(cpstr, ',');
    customcp = 0;
    for (int i = 0; i < 256; i++)
      customcp_to_unicode[i] = 0;
    if (r != NULL) {
      *r = 0;
      ...
      *r = ',';
      ...
      if (!file && exepath.size())
        file = fopen((exepath + CROSS_FILESPLIT + cpfile).c_str(), "r");
      if (file && cp > 0 && cp != 932 && cp != 936 && cp != 949 && cp != 950 &&
        ...
        while (fgets(line, sizeof(line), file)) {
          ...
          if (r)
            *r = 0;
          l = trim(l);
          if (!strlen(l) || strncasecmp(l, "0x", 2))
            continue;
          r = strchr(l, ' ');
          if (!r)
            r = strchr(l, '\t');
          ...
          *r = 0;
          ...
          r = trim(r + 1);
          if (ind > 0xFF || strncasecmp(r, "0x", 2))
            continue;
    ...
    /* If memsize < 16KB then the only way DOS can work properly is to allocate
     * in the UMB private area */
    if (MEM_ConventionalPages() < 4) {
      if (!private_always_from_umb) {
        private_always_from_umb = true;
        LOG(LOG_MISC,
            LOG_DEBUG)("Memory size < 16KB, allocating all DOS kernel "
                       "structures in the private upper memory area");
      }
    }

    /* FIXME: Boot up an MS-DOS system and look at what INT 21h on Microsoft's
     * MS-DOS returns for SDA size and location, then use that here.
     *
    ...
    if (dos_sda_size == 0)
      ...
      DOS_SDA_SEG_SIZE =
          (dos_sda_size + 0xF) & (~0xF); /* round up to paragraph */

    /* msdos 2.x and msdos 5.x modes, if HMA is involved, require us to take the
     * first 256 bytes of HMA in order for "F01D:FEF0" to work properly whether
     * or not A20 is enabled. Our direct mode doesn't jump through that address,
     * and therefore doesn't need it. */
    if (dos_in_hma && cpm_compat_mode != CPM_COMPAT_OFF &&
        cpm_compat_mode != CPM_COMPAT_DIRECT) {
      LOG(LOG_DOSMISC, LOG_DEBUG)("DOS: CP/M compatibility method with DOS in "
                                  "HMA requires mirror of entry point in HMA.");
      if (dos_initial_hma_free > 0xFF00) {
        dos_initial_hma_free = 0xFF00;
        LOG(LOG_DOSMISC,
            LOG_DEBUG)("DOS: CP/M compatibility method requires reduction of "
                       "HMA free space to accommodate.");
      }
    }

    if ((int)MAXENV < 0)
      MAXENV = 65535;
    if ((int)ENV_KEEPFREE < 0)
      ENV_KEEPFREE = 1024;

    LOG(LOG_DOSMISC, LOG_DEBUG)("DOS: MAXENV=%u ENV_KEEPFREE=%u", MAXENV,
                                ENV_KEEPFREE);

    if (ENV_KEEPFREE < 83)
      LOG_MSG("DOS: ENV_KEEPFREE is below 83 bytes. DOS programs that rely on "
              "undocumented data following the environment block may break.");

    if (dbg_zero_on_dos_allocmem) {
      LOG_MSG("Debug option enabled: INT 21h memory allocation will always "
              "clear memory block before returning\n");
    }

    if (minimum_mcb_segment > 0x8000)
      minimum_mcb_segment = 0x8000; /* FIXME: Clip against available memory */
    ...
    if (private_always_from_umb) {
      ...
      if (minimum_mcb_segment == 0)
        DOS_MEM_START =
            IS_PC98_ARCH
                ? 0x80
                : (MEM_ConventionalPages() >= 0x10 /*64KB or more*/
                       ? 0x70
                       : 0x60); /* funny behavior in some games suggests the
                                   MS-DOS kernel loads a bit higher on PC-98 */
      else
        DOS_MEM_START = minimum_mcb_segment;

      if (DOS_MEM_START < 0x40)
        LOG_MSG("DANGER, DANGER! DOS_MEM_START has been set to within the "
                "interrupt vector table! Proceed at your own risk!");
      else if (DOS_MEM_START < 0x50)
        LOG_MSG("WARNING: DOS_MEM_START has been assigned to the BIOS data "
                "area! Proceed at your own risk!");
      else if (DOS_MEM_START < 0x51)
        LOG_MSG("WARNING: DOS_MEM_START has been assigned to segment 0x50, "
                "which some programs may use as the Print Screen flag");
      else if (DOS_MEM_START < 0x80 && IS_PC98_ARCH)
        LOG_MSG(
            "CAUTION: DOS_MEM_START is less than 0x80 which may cause problems "
            "with some DOS games or applications relying on PC-98 BIOS state");
      else if (DOS_MEM_START < 0x70)
        LOG_MSG("CAUTION: DOS_MEM_START is less than 0x70 which may cause "
                "problems with some DOS games or applications");
    } else {
      ...
      if (DOS_PRIVATE_SEGMENT < 0x80 && IS_PC98_ARCH)
        LOG_MSG("DANGER, DANGER! DOS_PRIVATE_SEGMENT has been set too low for "
                "PC-98 emulation!");

      if (MEM_ConventionalPages() > 0x9C)
        DOS_PRIVATE_SEGMENT_END = 0x9C00;
      else
        DOS_PRIVATE_SEGMENT_END =
            (uint16_t)((MEM_ConventionalPages() << (12 - 4)) -
                       1); /* NTS: Remember DOSBox's implementation reuses the
                              last paragraph for UMB linkage */
    }

    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "DOS kernel structures will be allocated from pool 0x%04x-0x%04x",
        DOS_PRIVATE_SEGMENT, DOS_PRIVATE_SEGMENT_END - 1);

    DOS_IHSEG = DOS_GetMemory(1, "DOS_IHSEG");

    /* DOS_INFOBLOCK_SEG contains the entire List of Lists, though the INT 21h
     * call returns seg:offset with offset nonzero */
    /* NTS: DOS_GetMemory() allocation sizes are in PARAGRAPHS (16-byte units)
     * not bytes */
    /* NTS: DOS_INFOBLOCK_SEG must be 0x32 paragraphs. SDA_SEG must be located
     * at INFOBLOCK_SEG+0x32, so that the current PSP segment parameter is
     * exactly at memory location INFOBLOCK_SEG:0x330. The reason for this has
     * to do with Microsoft "Genuine MS-DOS detection" code in QuickBasic 7.1
     * and other programs designed to thwart DR-DOS at the time.
     *
     *      This is probably why DOSBox SVN hardcoded segments in the first
     * place.
     *
     *      See also:
     *
     ...
     */
    DOS_INFOBLOCK_SEG = DOS_GetMemory(
        0x32,
        "DOS_INFOBLOCK_SEG"); // was 0x80  0x32 = 0x20(INFOBLOCK) + 0x08(old
                              // CONDRV_SEG) + 0x0A(CONSTRING_SEG)
    DOS_SDA_SEG =
        DOS_GetMemory(DOS_SDA_SEG_SIZE >> 4,
                      "DOS_SDA_SEG"); // was 0xB2  (0xB2 + 0x56 = 0x108)
    DOS_SDA_OFS = 0;

    /* 2024/06/02: Keep the CON driver away from the clusterfuck of the
     * INFOBLOCK and SFT mess */
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)("   infoblock:    seg 0x%04x",
                                DOS_INFOBLOCK_SEG);
    LOG(LOG_DOSMISC, LOG_DEBUG)("   condrv:       seg 0x%04x", DOS_CONDRV_SEG);
    LOG(LOG_DOSMISC, LOG_DEBUG)("   SDA:          seg 0x%04x:0x%04x %u bytes",
                                DOS_SDA_SEG, DOS_SDA_OFS, DOS_SDA_SEG_SIZE);
    LOG(LOG_DOSMISC, LOG_DEBUG)("   CDS:          seg 0x%04x", DOS_CDS_SEG);
    LOG(LOG_DOSMISC,
        LOG_DEBUG)("[private segment @ this point 0x%04x-0x%04x mem=0x%04lx]",
    ...
    if (IS_PC98_ARCH) {
      // PC-98 also has INT 29h but the behavior of some games suggest that it
      // is handled the same as CON device output. Apparently the reason Touhou
      // Project has been unable to clear the screen is that it uses INT 29h to
      // directly send ANSI codes rather than standard I/O calls to write to the
      // CON device.
    ...
    } else if (section->Get_bool(
                   "ansi.sys")) { // NTS: DOS CON device is not yet initialized,
                                  // therefore will not return correct value of
                                  // "is ANSI.SYS installed"?
      // ANSI.SYS is installed, and it does hook INT 29h as well. Route INT 29h
      // through it.
      callback[6].Install(INT29_HANDLER, CB_IRET, "CON Output Int 29");
    } else {
      // Use the DOSBox SVN callback code that takes the byte and shoves it off
      // to INT 10h. NTS: This is where DOSBox SVN is wrong. DOSBox SVN treats
      // INT 29h as a straight call
    ...
    if (IS_PC98_ARCH) {
      /* Any interrupt vector pointing to the INT stub in the BIOS must be
       * rewritten to point to a JMP to the stub residing in the DOS segment
       * (60h) because some PC-98 resident drivers use segment 60h as a check
       * for installed vs uninstalled (MUSIC.COM, Peret em Heru) */
      ...
      if (veco >= 0xFFF0u)
        E_Exit("INT stub trampoline out of bounds");
    ...
    /* NTS: HMA support requires XMS. EMS support may switch on A20 if VCPI
     * emulation requires the odd megabyte */
    if ((!dos_in_hma || !section->Get_bool("xms")) &&
        (MEM_A20_Enabled() ||
         strcmp(section->Get_string("ems"), "false") != 0) &&
        cpm_compat_mode != CPM_COMPAT_OFF &&
        cpm_compat_mode != CPM_COMPAT_DIRECT) {
      /* hold on, only if more than 1MB of RAM and memory access permits it */
      if (MEM_TotalPages() > 0x100 &&
          MEM_PageMask() > 0xff /*more than 20-bit decoding*/) {
        LOG(LOG_DOSMISC,
            LOG_WARN)("DOS not in HMA or XMS is disabled. This may break "
                      "programs using the CP/M compatibility call method if "
                      "the A20 gate is switched on.");
      }
    }
    ...
    /* move the private segment elsewhere to avoid conflict with the MCB
     * structure. either set to 0 to cause the decision making to choose an
     * upper memory address, or allocate an additional private area and start
     * the MCB just after that */
    if (!private_always_from_umb) {
      DOS_MEM_START =
          DOS_GetMemory(0, "DOS_MEM_START"); // was 0x158 (pass 0 to alloc
                                             // nothing, get the pointer)

      DOS_GetMemory_reset();
      DOS_PRIVATE_SEGMENT = 0;
      DOS_PRIVATE_SEGMENT_END = 0;
      if (!private_segment_in_umb) {
        /* If private segment is not being placed in UMB, then it must follow
         * the DOS kernel. */
        unsigned int seg;
        unsigned int segend;
        ...
        DOS_GetMemory_reset();
        LOG_MSG("Private area, not stored in UMB on request, occupies "
                "0x%04x-0x%04x [dynamic]\n",
                DOS_PRIVATE_SEGMENT, DOS_PRIVATE_SEGMENT_END - 1);
      }
    }
     ...
     *
     * NOTES: A minimum mcb free value of at least 0xE10 is needed for
     * Windows 3.1 386 enhanced to start, else it will complain about
     * insufficient memory (?). To get Windows 3.1 to run, either set "minimum
     * mcb free=e10" or run "LOADFIX" before starting Windows 3.1 */
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)("   min free:     seg 0x%04x",
                                minimum_mcb_free);
    ...
    if (DOS_MEM_START < minimum_mcb_free) {
      ...
      if (sg != 0 && sg < minimum_mcb_free) {
        ...
        if (!DOS_ResizeMemory(sg, &tmp)) {
          LOG(LOG_DOSMISC,
              LOG_DEBUG)("    WARNING: cannot resize min free pad");
        }
      }
    }
    ...
    ZDRIVE_NUM = 25;
    DOS_SDA(DOS_SDA_SEG, DOS_SDA_OFS)
    ...
    if (!strcasecmp(keepstr, "true") || !strcasecmp(keepstr, "1"))
      keep_private_area_on_boot = 1;
    else if (!strcasecmp(keepstr, "false") || !strcasecmp(keepstr, "0"))
      keep_private_area_on_boot = 0;
    else
      keep_private_area_on_boot = addovl;
    dos.direct_output = false;
    dos.internal_output = false;

    std::string fat32setverstr = section->Get_string("fat32setversion");
    if (fat32setverstr == "auto")
      fat32setver = 1;
    else if (fat32setverstr == "manual")
      fat32setver = 0;
    else
      fat32setver = -1;
    ...
    if (lfn == "true" || lfn == "1")
      enablelfn = 1;
    else if (lfn == "false" || lfn == "0")
      enablelfn = 0;
    else if (lfn == "autostart")
      enablelfn = -2;
    else
      enablelfn = -1;

    force_conversion = true;
    mainMenu.get_item("dos_lfn_auto")
        .check(enablelfn == -1)
        .enable(true)
        .refresh_item(mainMenu);
    mainMenu.get_item("dos_lfn_enable")
        .check(enablelfn == 1)
        .enable(true)
        .refresh_item(mainMenu);
    mainMenu.get_item("dos_lfn_disable")
        .check(enablelfn == 0)
        .enable(true)
        .refresh_item(mainMenu);
    force_conversion = false;
    ...
    if (first_run) {
      ...
      enable_config_as_shell_commands =
          section->Get_bool("shell configuration as commands");
    ...
    mainMenu.get_item("dos_win_autorun")
        .check(winautorun)
        .enable(true)
        .refresh_item(mainMenu);
#endif
    ...
    mainMenu.get_item("dos_win_transpath")
        ...
        .refresh_item(mainMenu);
    mainMenu.get_item("dos_win_wait")
        .check(startwait)
        .enable(
#if defined(WIN32) && !defined(HX_DOS)
            true
        ...
        .refresh_item(mainMenu);
    mainMenu.get_item("dos_win_quiet")
        .check(startquiet)
        .enable(
#if defined(WIN32) && !defined(HX_DOS)
            true
  ...
  for (uint16_t i = 0; i < DOS_DRIVES; i++) {
    if (Drives[i] != NULL) {
      if (DriveManager::UnmountDrive(i) == 0)
        Drives[i] = NULL; /* deletes drive but does not set to NULL because
                             surrounding code does that */
      else if (i != ZDRIVE_NUM)
        LOG(LOG_DOSMISC, LOG_DEBUG)(
            "Failed to unmount drive %c",
            i + 'A'); /* probably drive Z: , UnMount() always returns nonzero */
    }
...
void DOS_EnableDriveMenu(char drv) {
  if (drv >= 'A' && drv <= 'Z') {
    ...
    mainMenu.get_item(name)
        .enable(!dos_kernel_disabled && Drives[drv - 'A'] != NULL &&
                (drv - 'A') != ZDRIVE_NUM)
        .refresh_item(mainMenu);
    ...
    mainMenu.get_item(name)
        .enable(Drives[drv - 'A'] != NULL &&
                !dynamic_cast<fatDrive *>(Drives[drv - 'A']))
        .refresh_item(mainMenu);
...
void DOS_DoShutDown() {
  ...
  if (IS_PC98_ARCH)
    update_pc98_function_row(0);

  DOS_Casemap_Free();

  for (char drv = 'A'; drv <= 'Z'; drv++)
    DOS_EnableDriveMenu(drv);
}
...
void DOS_Init() {
  ...
  LOG(LOG_DOSMISC, LOG_DEBUG)("sizeof(union bootSector) = %u",
                              (unsigned int)sizeof(union bootSector));
  LOG(LOG_DOSMISC, LOG_DEBUG)("sizeof(struct FAT_BootSector) = %u",
                              (unsigned int)sizeof(struct FAT_BootSector));
  LOG(LOG_DOSMISC, LOG_DEBUG)("sizeof(direntry) = %u",
                              (unsigned int)sizeof(direntry));
  ...
  AddVMEventFunction(VM_EVENT_DOS_EXIT_KERNEL,
                     AddVMEventFunctionFuncPair(DOS_ShutDown));
  AddVMEventFunction(VM_EVENT_DOS_EXIT_REBOOT_KERNEL,
                     AddVMEventFunctionFuncPair(DOS_ShutDown));
  AddVMEventFunction(VM_EVENT_DOS_SURPRISE_REBOOT,
                     AddVMEventFunctionFuncPair(DOS_OnReset));

  DOSBoxMenu::item *item;

  MAPPER_AddHandler(DOS_RescanAll, MK_nothing, 0, "rescanall", "Rescan drives",
                    &item);
...
void DOS_Int21_713a(char *name1, const char *name2) {
  ...
  *(p + 2) = 0;
  if (DOS_RemoveDir(name1)) {
    ...
    reg_ax = dos.errorcode;
    CALLBACK_SCF(true);
    LOG(LOG_DOSMISC, LOG_NORMAL)("Remove dir failed on %s with error %X", name1,
                                 dos.errorcode);
  }
}
  ...
  *(p + 2) = 0;
  ...
  case 0x07: {
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
      ...
      if (!SetFileTime(hFile, reg_bl == 0x07 ? &time : NULL,
                       reg_bl == 0x05 ? &time : NULL,
  ...
  case 0x08:
    ...
    if (DOS_GetFileAttrEx(name1, &status)) {
      ...
      time_t ttime = reg_bl == 0x04   ? status.st_mtime
                     : reg_bl == 0x06 ? status.st_atime
                                      : status.st_ctime;
      if ((ltime = localtime(&ttime)) != nullptr) {
        reg_cx = DOS_PackTime((uint16_t)ltime->tm_hour, (uint16_t)ltime->tm_min,
                              (uint16_t)ltime->tm_sec);
        reg_di = DOS_PackDate((uint16_t)(ltime->tm_year + 1900),
...
void DOS_Int21_714e(char *name1, char *name2) {
  ...
  *(p + 2) = 0;
  ...
  if (strlen(name2) > 2 && name2[strlen(name2) - 2] == '\\' &&
  ...
  if (!b &&
      !(strlen(name2) == 3 && *(name2 + 1) == ':' && *(name2 + 2) == '\\') &&
      DOS_GetFileAttr(name2, &attribute) && (attribute & DOS_ATTR_DIRECTORY)) {
    strcat(name2, "\\*.*");
    lfn_filefind_handle = handle;
  ...
  if (b) {
    ...
    int c = 0;
    MEM_BlockWrite(SegPhys(es) + reg_di, finddata,
                   dta.GetFindData((int)reg_si, finddata, &c));
    reg_cx = c;
    CALLBACK_SCF(false);
  } else {
    dos.errorcode = error;
...
void DOS_Int21_71a0(char *name1, char *name2) {
   ...
   *		 canonicalize "C:", except the protected mode kernel does not
   *translate DS:DX and ES:DI from protected mode. So DS:DX correctly points to
   *		 ASCII-Z string "C:" but when the jump is made back to real mode
   *and our INT 21h is actually called, DS:DX points to unrelated memory and the
   *string we read is gibberish, and ES:DI likewise point to unrelated memory.
   *
   *		 If we write to ES:DI, we corrupt memory in a way that quickly
   *causes the setup program to fault and crash (often, a Segment Not Present
   *		 exception but sometimes worse).
   *
   *		 The reason nobody ever encounters this error when installing
   *Windows ME normally from a boot disk is because in pure DOS mode where SETUP
   *		 is normally run, LFN functions do not exist. This call, INT 21h
   *AX=71A0h, will normally return an error (CF=1) without reading or writing
   *any memory, and SETUP carries on without crashing.
   *
   *		 Therefore, if you want to install Windows ME from the DOSBox-X
   *DOS environment, you need to disable Long Filename emulation first before
   *		 running SETUP.EXE to avoid crashes and instability during the
   *install process. --J.C. */
  MEM_StrCopy(SegPhys(ds) + reg_dx, name1, DOSNAMEBUF);
  ...
  if (handle < DOS_FILES && Files[handle] && Files[handle]->name != NULL) {
    ...
    if (Drives[drive]) {
      ...
      if (!strncmp(Drives[drive]->GetInfo(), "local ", 6) ||
          !strncmp(Drives[drive]->GetInfo(), "CDRom ", 6)) {
        localDrive *ldp = !strncmp(Drives[drive]->GetInfo(), "local ", 6)
                              ? dynamic_cast<localDrive *>(Drives[drive])
                              : dynamic_cast<cdromDrive *>(Drives[drive]);
        if (ldp != NULL)
    ...
    struct stat status;
    if (DOS_GetFileAttrEx(Files[handle]->name, &status,
                          Files[handle]->GetDrive())) {
      int64_t ff;
      unsigned long st;

      if (status.st_mode & S_IFDIR)
        st = DOS_ATTR_DIRECTORY;
      else
        st = DOS_ATTR_ARCHIVE;

      /* Win32 stat() will not return the correct file size of a file open for
       * writing */
#if defined(WIN32)
...
void DOS_Int21_71a7(const char *name1, const char *name2) {
  ...
  switch (reg_bl) {
  case 0x00: {
    int64_t ff = ((int64_t)mem_readd(SegPhys(ds) + reg_si + 4) << 32) |
                 mem_readd(SegPhys(ds) + reg_si);
    time_t tt = (time_t)((ff - 116444736000000000LL) / 10000000LL);
    struct tm *ftm = localtime(&tt);
    if (ftm != NULL) {
      reg_cx = DOS_PackTime((uint16_t)ftm->tm_hour, (uint16_t)ftm->tm_min,
                            (uint16_t)ftm->tm_sec);
      reg_dx =
          DOS_PackDate((uint16_t)(ftm->tm_year + 1900),
                       (uint16_t)(ftm->tm_mon + 1), (uint16_t)ftm->tm_mday);
  ...
  case 0x01: {
    ...
    ftm.tm_isdst = -1;
    int64_t ff = 116444736000000000LL + (int64_t)mktime(&ftm) * 10000000LL +
                 reg_bh * 100000LL;
  ...
  if (reg_dh == 0 || reg_dh == 1) {
    ...
    if (strcmp(name1, ".") && strcmp(name1, "..")) {
      ...
      for (i = 0; i < 8; j++) {
        ...
        while (name1[j] &&
               (name1[j] <= 32 || name1[j] == 127 || name1[j] == '"' ||
                name1[j] == '+' || name1[j] == '=' || name1[j] == '.' ||
                name1[j] == ',' || name1[j] == ';' || name1[j] == ':' ||
                name1[j] == '<' || name1[j] == '>' || name1[j] == '[' ||
                name1[j] == ']' || name1[j] == '|' || name1[j] == '\\' ||
                name1[j] == '?' || name1[j] == '*'))
      ...
      if (s != NULL) {
        ...
        for (i = 0; i < 3; i++) {
          if (*(s + i + j) == 0)
            break;
          while (*(s + i + j) && (*(s + i + j) <= 32 || *(s + i + j) == 127 ||
                                  *(s + i + j) == '"' || *(s + i + j) == '+' ||
                                  *(s + i + j) == '=' || *(s + i + j) == ',' ||
                                  *(s + i + j) == ';' || *(s + i + j) == ':' ||
                                  *(s + i + j) == '<' || *(s + i + j) == '>' ||
...
void DOS_Int21_71aa(char *name1, const char *name2) {
  ...
  case 2: {
    ...
    if (Drives[drive] &&
        !strncmp(Drives[drive]->GetInfo(), "local directory ", 16)) {
...
private:
  ...
  void setBytes(std::istream &stream) override {
    ...
    if (!olddisable && dos_kernel_disabled)
