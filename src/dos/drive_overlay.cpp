...
// TODO recheck directories under linux with the filename_cache (as one adds the
// dos name (and runs cross_filename on the other))

// TODO Check: Maybe handle file redirection in ccc (opening the new file),
// (call update datetime host there ?)

/* For rename/delete(unlink)/makedir/removedir we need to rebuild the cache.
 * (shouldn't be needed, but cacheout/delete entry currently throw away the
 * cached folder and rebuild it on read. so we have to ensure the rebuilding is
 * controlled through the overlay. In order to not reread the overlay directory
 * contents, the information in there is cached and updated when it changes
 * (when deleting a file or adding one)
 */

// directories that exist only in overlay can not be added to the drive_cache
// currently. Either upgrade addentry to support directories. (without actually
// caching stuff in! (code in testing)) Or create an empty directory in local
// drive base.
...
bool Overlay_Drive::RemoveDir(const char *dir) {
...
#if OVERLAY_DIR
  if (logoverlay)
    LOG_MSG("Overlay: trying to remove directory: %s", dir);
#else
  E_Exit("Overlay: trying to remove directory: %s", dir);
#endif
  /* Overlay: Check if folder is empty (findfirst/next, skipping . and .. and
   * breaking on first file found ?), if so, then it is not too tricky. */
  if (is_dir_only_in_overlay(dir)) {
    ...
    if (temp) {
      ...
      if (strlen(temp_name) > strlen(basedir) &&
          !strncasecmp(temp_name, basedir, strlen(basedir))) {
        strcpy(sdir, temp_name + strlen(basedir) +
                         (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
        strcpy(odir, overlaydir);
        strcat(odir, sdir);
        CROSS_FILENAME(odir);
    ...
    uint16_t olderror =
        dos.errorcode; // FindFirst/Next always set an errorcode, while
                       // RemoveDir itself shouldn't touch it if successful
    DOS_DTA dta(dos.tables.tempdta);
    char stardotstar[4] = {'*', '.', '*', 0};
    dta.SetupSearch(0, (0xff & ~DOS_ATTR_VOLUME),
                    stardotstar); // Fake drive as we don't use it.
    bool ret = this->FindFirst(
        dir, dta, false); // DOS_FindFirst(args,0xffff & ~DOS_ATTR_VOLUME);
    if (!ret) {
      // Path not found. Should not be possible due to removedir doing a
      // testdir, but lets be correct
    ...
    // FindNext is done, restore error code to old value. DOS_RemoveDir will set
    ...
    if (!empty)
      return false;
    if (logoverlay)
      LOG_MSG("directory empty! Hide it.");
    ...
    if (strlen(temp_name) > strlen(basedir) &&
        !strncasecmp(temp_name, basedir, strlen(basedir))) {
      temp_name +=
          strlen(basedir) + (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0);
      strcpy(odir, overlaydir);
...
bool Overlay_Drive::MakeDir(const char *dir) {
  ...
  // DOS_MakeDir tries first, before checking if the directory already exists,
  // so doing it here as well, so that case is handled.
  if (TestDir(dir))
    return false;
  if (overlap_folder == dir)
    return false; // TODO Test
#if OVERLAY_DIR
  if (logoverlay)
    LOG_MSG("Overlay trying to make directory: %s", dir);
#else
  E_Exit("Overlay trying to make directory: %s", dir);
#endif
  /* Overlay: Create in Overlay only and add it to drive_cache + some entries
   * else the drive_cache will try to access it. Needs an AddEntry for
   * directories. */

  // Check if leading dir is marked as deleted.
  if (check_if_leading_is_deleted(dir))
    return false;
  ...
  if (p != NULL) {
    *p = 0;
    ...
    if (strlen(temp_name) > strlen(basedir) &&
        !strncasecmp(temp_name, basedir, strlen(basedir))) {
      strcpy(newdir, temp_name + strlen(basedir) +
                         (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
      strcat(newdir, "\\");
      strcat(newdir, p + 1);
      strcpy(sdir, newdir);
...
bool Overlay_Drive::TestDir(const char *dir) {
  ...
  // Currently using the update_cache cache, alternatively access the directory
  // itself.
  ...
  if (templen && tempdir[templen - 1] == '\\')
    tempdir[templen - 1] = 0;

#if OVERLAY_DIR
  if (is_dir_only_in_overlay(tempdir))
    return true;
#endif

  // Next Check if the directory is marked as deleted or one of its leading
  // directories is. (it still might exists in the localDrive)

  if (is_deleted_path(tempdir))
    return false;
...
public:
  ...
  bool Write(const uint8_t *data, uint16_t *size) override {
    ...
    if (!overlay_active && (f == OPEN_READWRITE || f == OPEN_WRITE)) {
      if (logoverlay)
        LOG_MSG("write detected, switching file for %s", GetName());
      if (*data == 0) {
        if (logoverlay)
          LOG_MSG("OPTIMISE: truncate on switch!!!!");
      }
      uint32_t a = GetTicks();
      bool r = create_copy();
      if (GetTicks() - a > 2) {
        if (logoverlay)
          LOG_MSG("OPTIMISE: switching took %d", GetTicks() - a);
      }
      if (!r)
        return false;
      overlay_active = true;
...
// Create leading directories of a file being overlaid if they exist in the
// original (localDrive). This function is used to create copies of existing
// files, so all leading directories exist in the original.

FILE *Overlay_Drive::create_file_in_overlay(const char *dos_filename,
                                            char const *mode) {
  ...
  wchar_t wmode[8];
  unsigned int tis;
  for (tis = 0; tis < 7 && mode[tis] != 0; tis++)
    wmode[tis] = (wchar_t)mode[tis];
  assert(tis < 7); // guard
  wmode[tis] = 0;
#endif
  FILE *f;
  ...
  if (!f && dir && *dir) {
    if (logoverlay)
      LOG_MSG("Overlay: warning creating a file inside a directory %s",
              dos_filename);
    // ensure they exist, else make them in the overlay if they exist in the
    // original....
    ...
    *p = 0;
    bool found = false;
    for (std::vector<std::string>::iterator it = DOSdirs_cache.begin();
         it != DOSdirs_cache.end(); it += 2)
    ...
    if (!f) {
      strcpy(temp_name,
             dirCache.GetExpandName(GetCrossedName(basedir, dos_filename)));
      if (strlen(temp_name) > strlen(basedir) &&
          !strncasecmp(temp_name, basedir, strlen(basedir))) {
        strcpy(newname, overlaydir);
        strcat(newname, temp_name + strlen(basedir) +
                            (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
        CROSS_FILENAME(newname);
      }
      const host_cnv_char_t *host_name = CodePageGuestToHost(newname);
...
bool OverlayFile::create_copy() {
  ...
  if (drive_set != 0xff && drive_set < DOS_DRIVES && Drives[drive_set]) {
    ...
    if (od) {
      newhandle = od->create_file_in_overlay(GetName(), "wb+"); // todo check
                                                                // wb+
    }
  }
  ...
  char buffer[BUFSIZ];
  size_t s;
  while ((s = fread(buffer, 1, BUFSIZ, lhandle)) != 0)
    fwrite(buffer, 1, s, newhandle);
  fclose(lhandle);
...
static OverlayFile *ccc(DOS_File *file) {
  ...
  if (!l)
    E_Exit("overlay input file is not a LocalFile");
...
Overlay_Drive::Overlay_Drive(const char *startdir, const char *overlay,
                             uint16_t _bytes_sector, uint8_t _sectors_cluster,
                             uint16_t _total_clusters, uint16_t _free_clusters,
                             uint8_t _mediaid, uint8_t &error,
                             std::vector<std::string> &options)
    : localDrive(startdir, _bytes_sector, _sectors_cluster, _total_clusters,
                 _free_clusters, _mediaid, options),
      special_prefix(prefix_overlay.c_str()) {
  optimize_cache_v1 =
      true; // Try to not reread overlay files on deletes. Ideally drive_cache
            // should be improved to handle deletes properly.
  // Currently this flag does nothing, as the current behavior is to not reread
  // due to caching everything.
#if defined(WIN32)
  ...
  if (strlen(overlaydir) >= strlen(basedir)) {
    ...
    if (strncmp(overlaydir, basedir, strlen(basedir)) == 0) {
      ...
      while ((p = strchr_dbcs(p, CROSS_FILESPLIT))) {
        ...
        if (logoverlay)
          LOG_MSG("HIDE directory: %s", dirname);
...
bool Overlay_Drive::FileOpen(DOS_File **file, const char *name,
                             uint32_t flags) {
  if (ovlnocachedir) {
    dirCache.EmptyCache();
    update_cache(true);
  ...
  if (hand) {
    ...
    *file = new LocalFile(name, hand);
    ...
    ; // TODO error handling!!!! (maybe check if it exists and read only (should
      // not happen with overlays)
  }
  ...
  if (!fileopened && !is_deleted_file(name))
    fileopened = localDrive::FileOpen(file, name, OPEN_READ);
  ...
  if (fileopened) {
    ...
    f->flags = flags; // ccc copies the flags of the localfile, which were not
...
bool Overlay_Drive::FileCreate(DOS_File **file, const char *name,
  ...
  // TODO Check if it exists in the dirCache ? // fix addentry ?  or just double
  // check (ld and overlay) AddEntry looks sound to me..
...
void Overlay_Drive::add_DOSname_to_cache(const char *name) {
  for (std::vector<std::string>::const_iterator itc = DOSnames_cache.begin();
       itc != DOSnames_cache.end(); ++itc) {
    if (!strcasecmp((*itc).c_str(), name))
      return;
  }
...
void Overlay_Drive::remove_DOSname_from_cache(const char *name) {
  for (std::vector<std::string>::iterator it = DOSnames_cache.begin();
       it != DOSnames_cache.end(); ++it) {
...
bool Overlay_Drive::Sync_leading_dirs(const char *dos_filename) {
  ...
  while ((leaddir = strchr_dbcs((char *)leaddir, '\\'))) {
    ...
    if (stat(dirCache.GetExpandName(dirnamebase), &basetest) == 0 &&
        basetest.st_mode & S_IFDIR) {
      if (logoverlay)
        LOG_MSG("base exists: %s", dirnamebase);
      ...
      char *temp_name =
          dirCache.GetExpandName(GetCrossedName(basedir, dirname));
      if (strlen(temp_name) > strlen(basedir) &&
          !strncasecmp(temp_name, basedir, strlen(basedir))) {
        temp_name +=
            strlen(basedir) + (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0);
      ...
      if (stat(dirnameoverlay, &overlaytest) == 0) {
        ...
        if ((overlaytest.st_mode & S_IFDIR) == 0)
          return false;
      } else {
        // folder does not exist, make it
        if (logoverlay)
          LOG_MSG("creating %s", dirnameoverlay);
...
void Overlay_Drive::update_cache(bool read_directory_contents) {
  ...
  if (read_directory_contents) {
    ...
    // Ensure hiding of the folder that contains the overlay, if it is part of
    // the base folder.
    add_deleted_path(overlap_folder.c_str(), false);
  }

  // Needs later to support stored renames and removals of files existing in the
  // localDrive plane. and by taking in account if the file names are actually
  // already renamed. and taking in account that a file could have gotten an
  // overlay version and then both need to be removed.
  //
  // Also what about sequences were a base file gets copied to a working save
  // game and then removed/renamed... copy should be safe as then the link with
  // the original doesn't exist. however the working safe can be rather
  // complicated after a rename and delete..

  // Currently directories existing only in the overlay can not be added to
  // drive cache:
  // 1. possible workaround create empty directory in base. Drawback would break
  // the no-touching-of-base.
  // 2. double up Addentry to support directories, (and adding . and .. to the
  // newly directory so it counts as cachedin.. and won't be recached, as
  // otherwise
  ...
  // This function needs to be called after any localDrive function calling
  // cacheout/deleteentry, as those throw away directories. either do this with
  // a parameter stating the part that needs to be rebuild,(directory) or clear
  // the cache by default and do it all.
  ...
  if (read_directory_contents) {
    ...
    if (read_directory_first(dirp, dir_name, dir_sname, is_directory)) {
      if ((strlen(dir_name) > prefix_lengh + 5) &&
    ...
    for (i = dirnames.begin(); i != dirnames.end(); i += 2) {
      ...
      // TODO: Use the dirname\. and dirname\.. for creating fake directories in
      // the driveCache.
      if (ll > 2 && testi[ll - 1] == '.' && testi[ll - 2] == CROSS_FILESPLIT)
        continue;
      if (ll > 3 && testi[ll - 1] == '.' && testi[ll - 2] == '.' &&
          testi[ll - 3] == CROSS_FILESPLIT)
        continue;
...
#if OVERLAY_DIR
      // Good directory, add to DOSdirs_cache if not existing in localDrive.
      // tested earlier to prevent problems with opendir
      ...
      if (read_directory_first(dirp, dir_name, dir_sname, is_directory)) {
        if ((strlen(dir_name) > prefix_lengh + 5) &&
            strncmp(dir_name, special_prefix.c_str(), prefix_lengh) == 0)
          specials.push_back(string(dirpush) + dir_name);
        else if (is_directory) {
          ...
          filenames.push_back(string(dirpush) + dir_name);
        while (read_directory_next(dirp, dir_name, dir_sname, is_directory)) {
          if ((strlen(dir_name) > prefix_lengh + 5) &&
              strncmp(dir_name, special_prefix.c_str(), prefix_lengh) == 0)
            specials.push_back(string(dirpush) + dir_name);
            ...
            filenames.push_back(string(dirspush) + dir_name);
        }
      }
      closedir(dirp);
      for (i = dirnames.begin(); i != dirnames.end(); i += 2) {
        if ((*i) == backupi)
          break; // find current directory again, for the next round.
      }
    }
  }
  ...
  if (read_directory_contents) {
    for (i = filenames.begin(); i != filenames.end(); ++i) {
      ...
      // upcase(dosname);  //Should not be really needed, as uppercase in the
      // overlay is a requirement...
      CROSS_DOSFILENAME(dosname);
      if (logoverlay)
        LOG_MSG("update cache add dosname %s", dosname);
  ...
  for (i = DOSdirs_cache.begin(); i != DOSdirs_cache.end(); i += 2) {
    ...
    if (strlen(sdir)) {
      ...
      if (p == NULL)
        ...
        *p = 0;
        for (std::vector<std::string>::iterator it = DOSdirs_cache.begin();
             it < i && it != DOSdirs_cache.end(); it += 2) {
  ...
  if (read_directory_contents) {
    for (i = specials.begin(); i != specials.end(); ++i) {
      // Specials look like this .DBOVERLAY_YYY_FILENAME.EXT or
      // DIRNAME[\/].DBOVERLAY_YYY_FILENAME.EXT where YYY is the operation
      // involved. Currently only DEL is supported. DEL = file marked as
      // deleted, (but exists in localDrive!)
      std::string name(*i);
      ...
      if (special_operation == "DEL") {
        ...
        while ((s = name.find('/')) != std::string::npos)
          name.replace(s, 1, "\\");
        ...
        if (logoverlay)
          LOG_MSG("unsupported operation %s on %s", special_operation.c_str(),
                  (*i).c_str());
      }
    }
  }
  if (logoverlay)
    LOG_MSG("OPTIMISE: update cache took %d", GetTicks() - a);
}
...
again:
  ...
  if (!is_deleted_file(preldos)) {
    ...
    if (host_name != NULL)
      statok = ht_stat(host_name, &stat_block) == 0;
    if (!statok) {
  ...
  if (statok) {
    if (logoverlay)
      LOG_MSG("using overlay data for %s : %s", uselfn ? lfull_name : full_name,
              ovname);
  } else {
    strcpy(preldos, prel);
    CROSS_DOSFILENAME(preldos);
    if (is_deleted_file(preldos)) { // dir.. maybe lower or keep it as is TODO
      if (logoverlay)
        LOG_MSG("skipping deleted file %s %s %s", preldos,
                uselfn ? lfull_name : full_name, ovname);
      goto again;
    }
    const host_cnv_char_t *host_name =
        CodePageGuestToHost(dirCache.GetExpandName(lfull_name));
    if (ht_stat(host_name, &stat_block) != 0) {
      if (logoverlay)
        LOG_MSG("stat failed for %s . This should not happen.",
                dirCache.GetExpandName(uselfn ? lfull_name : full_name));
  ...
  if (stat_block.st_mode & S_IFDIR)
    ...
    find_attr = 0;
#if defined(WIN32)
  Bitu attribs = host_name == NULL ? INVALID_FILE_ATTRIBUTES
                                   : GetFileAttributesW(host_name);
...
bool Overlay_Drive::FileUnlink(const char *name) {
  ...
  // TODO check the basedir for file existence in order if we need to add the
  // file to deleted file list.
  uint32_t a = GetTicks();
  if (logoverlay)
  ...
  if (stat(overlayname, &temp_stat)) {
    ...
    if (strlen(temp_name) > strlen(basedir) &&
      ...
      if (host_name != NULL && ht_unlink(host_name) == 0)
        removed = true;
    }
  }
  ...
  if (!removed && unlink(overlayname)) {
    ...
    if (host_name == NULL || ht_stat(host_name, &status)) {
      ...
      return true;
      //			E_Exit("trying to remove existing non-overlay
      //file %s",name);
    }
...
#endif
      remove_DOSname_from_cache(
          name); // Should be an else ? although better safe than sorry.
      // Handle this better
    ...
    update_cache(false);
    if (logoverlay)
      LOG_MSG("OPTIMISE: unlink took %d", GetTicks() - a);
    return true;
  }
}
...
bool Overlay_Drive::SetFileAttr(const char *name, uint16_t attr) {
  ...
  if (strlen(temp_name) > strlen(basedir) &&
      !strncasecmp(temp_name, basedir, strlen(basedir)))
    strcpy(tmp, temp_name + strlen(basedir) +
                    (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
  strcpy(overtmpname, overlaydir);
...
#if defined(WIN32)
  host_name = CodePageGuestToHost(overtmpname);
  if (host_name != NULL && SetFileAttributesW(host_name, attr))
    success = true;
  if (!success) {
    host_name = CodePageGuestToHost(overlayname);
    if (host_name != NULL && SetFileAttributesW(host_name, attr))
...
#else
  if (ht_stat(overtmpname, &status) == 0 ||
      ht_stat(overlayname, &status) == 0) {
    if (attr & DOS_ATTR_READ_ONLY)
      status.st_mode &= ~(S_IWUSR | S_IWGRP | S_IWOTH);
    else
      status.st_mode |= S_IWUSR;
    if (chmod(overtmpname, status.st_mode) >= 0 ||
        chmod(overlayname, status.st_mode) >= 0)
      success = true;
  ...
  bool created = false;
  if (host_name != NULL && ht_stat(host_name, &status) == 0 &&
      (status.st_mode & S_IFDIR)) {
    host_name = CodePageGuestToHost(overtmpname);
    int temp = -1;
    if (host_name != NULL) {
    ...
    if (hand) {
      if (logoverlay)
        LOG_MSG("overlay file opened %s", temp_name);
      FILE *layfile = NULL;
      host_name = CodePageGuestToHost(overtmpname);
#ifdef host_cnv_use_wchar
      if (host_name != NULL)
        layfile = _wfopen(host_name, _HT("wb"));
#endif
      if (layfile == NULL)
        layfile = fopen_wrap(overlayname, "wb");
      int numr, numw;
      char buffer[1000];
      while (feof(hand) == 0) {
...
bool Overlay_Drive::GetFileAttr(const char *name, uint16_t *attr) {
  ...
  if (host_name != NULL)
    attribs = GetFileAttributesW(host_name);
  if (attribs == INVALID_FILE_ATTRIBUTES) {
    host_name = CodePageGuestToHost(overlayname);
    if (host_name != NULL)
      attribs = GetFileAttributesW(host_name);
  ...
  if (res || ht_stat(overlayname, &status) == 0) {
    ...
    *attr = isdir ? 0 : DOS_ATTR_ARCHIVE;
    ...
    std::string fname = create_filename_of_special_operation(
        res ? overtmpname : overlayname, "ATR");
...
std::string Overlay_Drive::GetHostName(const char *name) {
  ...
  if (strlen(temp_name) > strlen(basedir) &&
      !strncasecmp(temp_name, basedir, strlen(basedir))) {
    strcpy(overlayname, overlaydir);
    strcat(overlayname, temp_name + strlen(basedir) +
                            (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
    CROSS_FILENAME(overlayname);
    host_name = CodePageGuestToHost(overlayname);
    if (host_name != NULL && ht_stat(host_name, &temp_stat) == 0) {
...
void Overlay_Drive::add_deleted_file(const char *name, bool create_on_disk) {
  ...
  if (!is_deleted_file(tname)) {
    ...
    if (create_on_disk)
      add_special_file_to_disk(name, "DEL");
  }
}

bool Overlay_Drive::add_special_file_to_disk(const char *dosname,
                                             const char *operation,
                                             uint16_t value, bool isdir) {
  ...
  if (!f) {
    ...
    char *temp_name =
        dirCache.GetExpandName(GetCrossedName(basedir, name.c_str()));
    if (strlen(temp_name) > strlen(basedir) &&
        !strncasecmp(temp_name, basedir, strlen(basedir))) {
      temp_name +=
          strlen(basedir) + (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0);
  ...
  if (!f)
    E_Exit("Failed creation of %s", overlayname);
...
void Overlay_Drive::remove_special_file_from_disk(const char *dosname,
                                                  const char *operation) {
  std::string name = create_filename_of_special_operation(dosname, operation);
  char overlayname[CROSS_LEN];
  strcpy(overlayname, overlaydir);
  ...
  const host_cnv_char_t *host_name = CodePageGuestToHost(overlayname);
  if ((host_name == NULL || ht_unlink(host_name) != 0) &&
      unlink(overlayname) != 0 && strcmp(operation, "ATR"))
    E_Exit("Failed removal of %s", overlayname);
}

std::string Overlay_Drive::create_filename_of_special_operation(
...
bool Overlay_Drive::is_dir_only_in_overlay(const char *name) {
  ...
  if (strlen(temp_name) > strlen(basedir) &&
      !strncasecmp(temp_name, basedir, strlen(basedir))) {
    strcpy(fname, temp_name + strlen(basedir) +
                      (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
  ...
  for (std::vector<std::string>::iterator it = DOSdirs_cache.begin();
    ...
    if (!strcasecmp((*it).c_str(), name) ||
        ...
        ((*(it + 1)).length() && !strcasecmp((*(it + 1)).c_str(), name)))
      return true;
  }
  return false;
}
...
bool Overlay_Drive::is_deleted_file(const char *name) {
  if (!name || !*name)
    return false;
  if (deleted_files_in_base.empty())
    return false;
  ...
  for (std::vector<std::string>::iterator it = deleted_files_in_base.begin();
       it != deleted_files_in_base.end(); it++) {
...
void Overlay_Drive::remove_deleted_file(const char *name, bool create_on_disk) {
  ...
  for (std::vector<std::string>::iterator it = deleted_files_in_base.begin();
    ...
    if (!strcasecmp((*it).c_str(), name) || !strcasecmp((*it).c_str(), tname) ||
      ...
      if (create_on_disk)
        remove_special_file_from_disk(name, "DEL");
      return;
    }
  }
}
void Overlay_Drive::add_deleted_path(const char *name, bool create_on_disk) {
  ...
  if (!is_deleted_path(name)) {
    ...
    // Maybe revise that.
    if (create_on_disk)
      add_special_file_to_disk(name, "RMD");
...
bool Overlay_Drive::is_deleted_path(const char *name) {
  if (!name || !*name)
    return false;
  if (deleted_paths_in_base.empty())
    return false;
  std::string sname(name);
  std::string::size_type namelen = sname.length();
  ;
  for (std::vector<std::string>::iterator it = deleted_paths_in_base.begin();
       it != deleted_paths_in_base.end(); ++it) {
    std::string::size_type blockedlen = (*it).length();
    if (namelen < blockedlen)
      continue;
    // See if input starts with name.
    std::string::size_type n = sname.find(*it);
    if (n == 0 && ((namelen == blockedlen) || *(name + blockedlen) == '\\'))
      return true;
  }
  return false;
}
...
void Overlay_Drive::remove_deleted_path(const char *name, bool create_on_disk) {
  for (std::vector<std::string>::iterator it = deleted_paths_in_base.begin();
    ...
    if (!strcasecmp((*it).c_str(), name)) {
      ...
      if (create_on_disk)
        remove_special_file_from_disk(name, "RMD");
      break;
    }
  }
}
...
bool Overlay_Drive::Rename(const char *oldname, const char *newname) {
  ...
  // Tricky function.
  // Renaming directories is currently not fully supported, due the drive_cache
  // not handling that smoothly. So oldname is directory => exit unless it is
  // only in overlay. If oldname is on overlay => simple rename. if oldname is
  // on base => copy file to overlay with new name and mark old file as deleted.
  // More advanced version. keep track of the file being renamed in order to
  // detect that the file is being renamed back.

  char tmp[CROSS_LEN];
  host_cnv_char_t host_nameold[CROSS_LEN], host_namenew[CROSS_LEN];
  uint16_t attr = 0;
  if (!GetFileAttr(oldname, &attr))
    E_Exit("rename, but source doesn't exist, should not happen %s", oldname);
  ht_stat_t temp_stat;
  if (attr & DOS_ATTR_DIRECTORY) {
    // See if the directory exists only in the overlay, then it should be
    // possible.
#if OVERLAY_DIR
    if (localDrive::TestDir(oldname)) {
      LOG_MSG("Overlay: renaming base directory %s to %s not yet supported",
              oldname, newname);
    ...
    if (ht_stat(host_nameold, &temp_stat)) {
      char *temp_name =
          dirCache.GetExpandName(GetCrossedName(basedir, oldname));
      if (strlen(temp_name) > strlen(basedir) &&
          !strncasecmp(temp_name, basedir, strlen(basedir))) {
        temp_name +=
            strlen(basedir) + (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0);
      ...
      if (p != NULL) {
        *p = 0;
        ...
        if (strlen(temp_name) > strlen(basedir) &&
            !strncasecmp(temp_name, basedir, strlen(basedir))) {
          strcpy(ndir, temp_name + strlen(basedir) +
                           (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
          strcat(ndir, "\\");
          strcat(ndir, p + 1);
    ...
    LOG_MSG("Overlay: renaming overlay directory %s to %s not yet supported",
            oldname, newname); // TODO
    DOS_SetError(DOSERR_ACCESS_DENIED);
    return false;
  }
  ...
  // No need to check if the original is marked as deleted, as GetFileAttr would
  // fail if it did.
  ...
  if (ht_stat(host_nameold, &temp_stat) == 0) {
    ...
    temp = rename(host_nameold, host_namenew);
#endif
    // TODO CHECK if base has a file with same oldname!!!!! if it does mark it
    // as deleted!!
    ...
    char buffer[BUFSIZ];
    size_t s;
    while ((s = fread(buffer, 1, BUFSIZ, o)))
      fwrite(buffer, 1, s, n);
    fclose(o);
    fclose(n);
    ...
    temp = 0; // success
    if (logoverlay)
      LOG_MSG("OPTIMISE: update rename with copy took %d", GetTicks() - aa);
  }
  if (temp == 0) {
    // handle the drive_cache (a bit better)
    // Ensure that the file is not marked as deleted anymore.
    if (is_deleted_file(newname))
      remove_deleted_file(newname, true);
    dirCache.EmptyCache();
    update_cache(true);
    if (logoverlay)
      LOG_MSG("OPTIMISE: rename took %d", GetTicks() - a);
  }
  return (temp == 0);
}

bool Overlay_Drive::FindFirst(const char *_dir, DOS_DTA &dta,
                              bool fcb_findfirst) {
  if (logoverlay)
    LOG_MSG("FindFirst in %s", _dir);
...
bool Overlay_Drive::FileStat(const char *name,
  ...
  if (strlen(temp_name) > strlen(basedir) &&
      !strncasecmp(temp_name, basedir, strlen(basedir))) {
    strcpy(overtmpname, overlaydir);
    strcat(overtmpname, temp_name + strlen(basedir) +
                            (*(temp_name + strlen(basedir)) == '\\' ? 1 : 0));
    CROSS_FILENAME(overtmpname);
    host_name = CodePageGuestToHost(overtmpname);
    if (host_name != NULL && ht_stat(host_name, &temp_stat) == 0)
