...
bool DOS_ExtDevice::CheckSameDevice(uint16_t seg, uint16_t s_off,
                                    uint16_t i_off) {
...
uint16_t DOS_ExtDevice::CallDeviceFunction(uint8_t command, uint8_t length,
...
bool DOS_ExtDevice::ReadFromControlChannel(PhysPt bufptr, uint16_t size,
                                           uint16_t *retcode) {
  if (ext.attribute & DeviceAttributeFlags::SupportsIoctl) {
    // IOCTL INPUT
    if ((CallDeviceFunction(3, 26, (uint16_t)(bufptr >> 4),
                            (uint16_t)(bufptr & 0x000f), size) &
         0x8000) == 0) {
...
bool DOS_ExtDevice::WriteToControlChannel(PhysPt bufptr, uint16_t size,
                                          uint16_t *retcode) {
  if (ext.attribute & DeviceAttributeFlags::SupportsIoctl) {
    // IOCTL OUTPUT
    if ((CallDeviceFunction(12, 26, (uint16_t)(bufptr >> 4),
                            (uint16_t)(bufptr & 0x000f), size) &
         0x8000) == 0) {
      *retcode = real_readw(dos.dcp, 18);
      return true;
...
bool DOS_ExtDevice::Close() { return true; }
...
uint16_t DOS_ExtDevice::GetInformation(void) {
  ...
  if (ext.attribute & DeviceAttributeFlags::CharacterDevice)
    ret |= DeviceInfoFlags::Device;
  if (ext.attribute & DeviceAttributeFlags::SupportsIoctl)
    ret |= DeviceInfoFlags::IoctlSupport;
  if (ext.attribute & DeviceAttributeFlags::SupportsRemovable)
    ret |= DeviceInfoFlags::OpenCloseSupport;
  ...
  while (1) {
    ...
    if (no & 0x8000) {
      ...
      if (!strcmp(name, devname)) {
        ...
        if (real_readd(seg, off + 6) == 0 ||
            real_readd(seg, off + 6) == 0xffffffff) {
          return 0;
        }
...
public:
  ...
  uint16_t GetInformation(void) override {
    return DeviceInfoFlags::Device | DeviceInfoFlags::Nul;
  }
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (parallelPortObjects[i] != NULL) {
        ...
        for (uint16_t j = 0; j < *size; j++) {
          if (!parallelPortObjects[i]->Putchar(data[j]))
            return false;
        }
        return true;
      }
...
uint16_t cpMap[512] = { // Codepage is standard 437
    0x0020, 0x263a, 0x263b, 0x2665, 0x2666, 0x2663, 0x2660, 0x2219, 0x25d8,
    ...
    0x255c, 0x255b, 0x2510, // 176 - 223 line/box drawing
    ...
    0x211e, 0x2120, 0x2122, 0x00ae, 0x00a9, 0x00a4, 0x2295, 0x2299, 0x2297,
    0x2296, 0x2a38, 0x21d5, 0x21d4, 0x21c4, 0x2196, 0x2198, // Second bank for
                                                            // WP extended
                                                            // charset
    0x2197, 0x2199, 0x22a4, 0x22a5, 0x22a2, 0x22a3, 0x2213, 0x2243, 0x2260,
    0x2262, 0x00b3, 0x00be, 0x0153, 0x0152, 0x05d0, 0x05d1, 0x0000, 0x2111,
    0x211c, 0x2200, 0x2207, 0x29cb, 0x0394, 0x039b, 0x03a0, 0x039e, 0x03a8,
    0x05d2, 0x03b3, 0x03b7, 0x03b9, 0x03ba, 0x03bb, 0x03bd, 0x03c1, 0x03a5,
    0x03c9, 0x03be, 0x03b6, 0x02bf, 0x03c5, 0x03c8, 0x03c2, 0x2113, 0x03c7,
    0x222e, 0x0178, 0x00cf, 0x00cb, 0x2227, 0x00c1, 0x0107, 0x0106, 0x01f5,
    0x013a, 0x00cd, 0x0139, 0x0144, 0x0143, 0x00d3, 0x0155, 0x0154, 0x015b,
    0x015a, 0x00da, 0x00fd, 0x00dd, 0x017a, 0x0179, 0x00c0, 0x00c8, 0x00cc,
    0x00d2, 0x00d9, 0x0151, 0x0150, 0x0171, 0x0170, 0x016f, 0x016e, 0x010b,
    0x010a, 0x0117, 0x0116, 0x0121, 0x0120, 0x0130, 0x017c, 0x017b, 0x00c2,
    0x0109, 0x0108, 0x00ca, 0x011d, 0x011c, 0x0125, 0x0124, 0x00ce, 0x0135,
    0x0134, 0x00d4, 0x015d, 0x015c, 0x00db, 0x0175, 0x0174, 0x0177, 0x0176,
    0x00e3, 0x00c3, 0x0129, 0x0128, 0x00f5, 0x00d5, 0x0169, 0x0168, 0x0103,
    0x0102, 0x011f, 0x011e, 0x016d, 0x016c, 0x010d, 0x010c, 0x010f, 0x010e,
    0x011b, 0x011a, 0x013e, 0x013d, 0x0148, 0x0147, 0x0159, 0x0158, 0x0161,
    0x0160, 0x0165, 0x0164, 0x017e, 0x017d, 0x00f0, 0x0122, 0x0137, 0x0136,
    0x013c, 0x013b, 0x0146, 0x0145, 0x0157, 0x0156, 0x015f, 0x015e, 0x0163,
    0x0162, 0x00df, 0x0133, 0x0132, 0x00f8, 0x00d8, 0x2218, 0x0123, 0x0000,
    0x01f4, 0x01e6, 0x2202, 0x0000, 0x0020, 0x2309, 0x2308, 0x230b, 0x230a,
    0x23ab, 0x2320, 0x2321, 0x23a9, 0x23a8, 0x23ac, 0x01e7, 0x2020, 0x2021,
    0x201e, 0x222b, 0x222a, 0x2282, 0x2283, 0x2288, 0x2289, 0x2286, 0x2287,
    0x220d, 0x2209, 0x2203, 0x21d1, 0x21d3, 0x21d0, 0x21d2, 0x25a1, 0x2228,
    0x22bb, 0x2234, 0x2235, 0x2237, 0x201c, 0x201d, 0x2026, 0x03b8, 0x0101,
    0x0113, 0x0100, 0x0112, 0x012b, 0x012a, 0x014d, 0x014c, 0x016b, 0x016a,
    0x0105, 0x0104, 0x0119, 0x0118, 0x012f, 0x012e, 0x0173, 0x0172, 0x0111,
    0x0110, 0x0127, 0x0126, 0x0167, 0x0166, 0x0142, 0x0141, 0x0140, 0x013f,
    0x00fe, 0x00de, 0x014a, 0x014b, 0x0149, 0x0000};

uint16_t cpMap_PC98[256] = {
    //  0       1       2       3       4       5       6       7
    0x0020, 0x2401, 0x2402, 0x2403, 0x2404,
    0x2405, 0x2406, 0x2407, // 0x00-0x07   Except NUL, control chars have small
                            // letters spelling out the ASCII control code
    0x2408, 0x2409, 0x240A, 0x240B, 0x240C,
    0x240D, 0x240E, 0x240F, // 0x08-0x0F   i.e. CR is ␍. The mapping here does
                            // not EXACTLY render as it does on real hardware.
    0x2410, 0x2411, 0x2412, 0x2413, 0x2414,
    0x2415, 0x2416, 0x2417, // 0x10-0x17
    0x2418, 0x2419, 0x241A, 0x241B, 0x2192,
    0x2190, 0x2191, 0x2193, // 0x18-0x1F   The last 4 are single-wide arrows
    0x0020, 0x0021, 0x0022, 0x0023, 0x0024,
    0x0025, 0x0026, 0x0027, // 0x20-0x27
    0x0028, 0x0029, 0x002A, 0x002B, 0x002C,
    0x002D, 0x002E, 0x002F, // 0x28-0x2F
    0x0030, 0x0031, 0x0032, 0x0033, 0x0034,
    0x0035, 0x0036, 0x0037, // 0x30-0x37
    0x0038, 0x0039, 0x003A, 0x003B, 0x003C,
    0x003D, 0x003E, 0x003F, // 0x38-0x3F
    0x0040, 0x0041, 0x0042, 0x0043, 0x0044,
    0x0045, 0x0046, 0x0047, // 0x40-0x47
    0x0048, 0x0049, 0x004A, 0x004B, 0x004C,
    0x004D, 0x004E, 0x004F, // 0x48-0x4F
    0x0050, 0x0051, 0x0052, 0x0053, 0x0054,
    0x0055, 0x0056, 0x0057, // 0x50-0x57
    0x0058, 0x0059, 0x005A, 0x005B, 0x00A5,
    0x005D, 0x005E, 0x005F, // 0x58-0x5F   0x5C = Yen
    0x0060, 0x0061, 0x0062, 0x0063, 0x0064,
    0x0065, 0x0066, 0x0067, // 0x60-0x67
    0x0068, 0x0069, 0x006A, 0x006B, 0x006C,
    0x006D, 0x006E, 0x006F, // 0x68-0x6F
    0x0070, 0x0071, 0x0072, 0x0073, 0x0074,
    0x0075, 0x0076, 0x0077, // 0x70-0x77
    0x0078, 0x0079, 0x007A, 0x007B, 0x007C,
    0x007D, 0x007E, 0x0020, // 0x78-0x7F   0x7F = Does not display as anything
    0x2581, 0x2582, 0x2583, 0x2584, 0x2585,
    0x2586, 0x2587, 0x2588, // 0x80-0x87   Block characters, vertical 1/8 to 8/8
    0x258F, 0x258E, 0x258D, 0x258C, 0x258B,
    0x258A, 0x2589, 0x253C, // 0x88-0x8F   Block characters, horizontal 1/8 to
                            // 7/8
    0x2534, 0x252C, 0x2524, 0x251C, 0x0020,
    0x2500, 0x2502, 0x2595, // 0x90-0x97
    0x250C, 0x2510, 0x2514, 0x2518, 0x256D,
    0x256E, 0x2570, 0x256F, // 0x98-0x9F
    0x0020, 0xFF61, 0xFF62, 0xFF63, 0xFF64,
    0xFF65, 0xFF66, 0xFF67, // 0xA0-0xA7   0xA0 = Nothing. 0xA1-0xDF =
                            // Single-wide Katakana
    0xFF68, 0xFF69, 0xFF6A, 0xFF6B, 0xFF6C,
    0xFF6D, 0xFF6E, 0xFF6F, // 0xA8-0xAF
    0xFF70, 0xFF71, 0xFF72, 0xFF73, 0xFF74,
    0xFF75, 0xFF76, 0xFF77, // 0xB0-0xB7
    0xFF78, 0xFF79, 0xFF7A, 0xFF7B, 0xFF7C,
    0xFF7D, 0xFF7E, 0xFF7F, // 0xB8-0xBF
    0xFF80, 0xFF81, 0xFF82, 0xFF83, 0xFF84,
    0xFF85, 0xFF86, 0xFF87, // 0xC0-0xC7
    0xFF88, 0xFF89, 0xFF8A, 0xFF8B, 0xFF8C,
    0xFF8D, 0xFF8E, 0xFF8F, // 0xC8-0xCF
    0xFF90, 0xFF91, 0xFF92, 0xFF93, 0xFF94,
    0xFF95, 0xFF96, 0xFF97, // 0xD0-0xD7
    0xFF98, 0xFF99, 0xFF9A, 0xFF9B, 0xFF9C,
    0xFF9D, 0xFF9E, 0xFF9F, // 0xD8-0xDF
    0x2550, 0x255E, 0x256A, 0x2561, 0x0020,
    0x0020, 0x0020, 0x0020, // 0xE0-0xE7   FIXME: E0h-E3h is an approximation.
                            // Find Unicode approx. for E4h-E7h
    0x2660, 0x2665, 0x2666, 0x2663, 0x25CF,
    0x25CB, 0x2571, 0x2572, // 0xE8-0xEF
    0x2573, 0x0020, 0x5E74, 0x6708, 0x65E5,
    0x0020, 0x5206, 0x0020, // 0xF0-0xF7   FIXME: What is F1h, F5h, F7h?
    0x0020, 0x0020, 0x0020, 0x0020, 0x005C,
    0x0020, 0x0020, 0x0020 // 0xF8-0xFF   0xFC = Backslash
};
...
private:
  ...
  virtual void CommitData() {
        ...
        for (uint32_t i = 0; i < rawdata.size(); i++) {
          if (lead) {
            ...
            if (pchr && isKanji2(rawdata[i] & 0xff)) {
              ...
              fwrite(cpMap + pchr, 1, 2, fh);
            pchr = 0;
          } else if ((IS_PC98_ARCH && shiftjis_lead_byte(rawdata[i] & 0xff)) ||
                     (isDBCSCP() && isKanji1(rawdata[i] & 0xff))) {
            pchr = rawdata[i];
            lead = true;
            continue;
          }
    ...
    for (std::string token; std::getline(iss, token);) {
      if (token.size() && token.back() == 13)
        token.pop_back();
      char *uname = CodePageGuestToHost(token.c_str());
      result +=
          (uname != NULL ? std::string(uname) : token) + std::string(1, 10);
    }
    if (result.size() && result.back() == 10)
      result.pop_back();
#if defined(C_SDL2)
...
public:
  ...
  bool Read(uint8_t *data, uint16_t *size) override {
    ...
    lastwrite = false;
    if (!clipSize) // If no data, we have to read the Windows CLipboard
                   // (clipSize gets reset on device close)
    {
      getClipboard();
      fPointer = 0;
    }
  ...
  uint16_t GetInformation(void) override {
    return DeviceInfoFlags::Device | DeviceInfoFlags::EofOnInput |
           DeviceInfoFlags::Binary;
  }
};
...
bool DOS_Device::Close() { return Devices[devnum]->Close(); }
...
void DOS_Device::SetInformation(uint16_t info) {
  if (Devices[devnum]->IsName("CON") &&
      !(Devices[devnum]->GetInformation() & EXT_DEVICE_BIT)) {
    Devices[devnum]->SetInformation(info);
  }
}

bool DOS_Device::ReadFromControlChannel(PhysPt bufptr, uint16_t size,
                                        uint16_t *retcode) {
  return Devices[devnum]->ReadFromControlChannel(bufptr, size, retcode);
}

bool DOS_Device::WriteToControlChannel(PhysPt bufptr, uint16_t size,
                                       uint16_t *retcode) {
...
DOS_File::DOS_File(const DOS_File &orig)
    : flags(orig.flags), open(orig.open), attr(orig.attr), time(orig.time),
      date(orig.date), refCtr(orig.refCtr), hdrive(orig.hdrive) {
  if (orig.name) {
...
uint8_t DOS_FindDevice(char const *name) {
  ...
  uint8_t drive;
  bool ime_flag = false;
  //	if(!name || !(*name)) return DOS_DEVICES; //important, but makename does
  //it

  // NTS: If a program is trying to open paths like "@:\\WINBOOT.SYS" the most
  // likely
  //      reason is that it's a Microsoft program like FORMAT.COM and it's
  //      reading locations in the DOS kernel that are supposed to hold the
  //      drive letter of the boot drive, but carry zero instead.
  //
  //      "@:" paths are a sign the program is possibly trying to read files
  //      from the boot device but is using nonstandard means to determine that
  //      i.e. not using INT 21h AX=3305h as documented for MS-DOS 4.0 or
  //      higher. Real MS-DOS probably does not accept "@:" drive paths and
  //      neither should we.
  //
  ...
  //      and Windows 95 FORMAT.COM requesting "@:\\WINBOOT.SYS" prior to this
  //      fix sometimes caused a segfault.
  if (*name == '@' && *(name + 1) == ':') {
    LOG_MSG("DOS_FindDevice(): Rejecting path '%s'. @: paths are not valid. It "
            "may be a sign the program is attempting to locate the boot drive "
            "in an undocumented manner not supported by this emulator",
            name);
    return DOS_DEVICES;
  ...
  if (Network_IsNetworkResource(name))
    ...
    *name_part++ = 0;
    ...
    if (!Drives[drive]->TestDir(fullname) && !strcasecmp(name_part, "NUL"))
...
void DOS_AddDevice(DOS_Device *adddev) {
  ...
  if (adddev == NULL)
    E_Exit("DOS_AddDevice() with null ptr");
  for (Bitu i = 0; i < DOS_DEVICES; i++) {
    if (Devices[i] == NULL) {
      //			LOG_MSG("DOS_AddDevice() %s
      //(%p)",adddev->name,(void*)adddev);
      Devices[i] = adddev;
...
void DOS_DelDevice(DOS_Device *dev) {
  ...
  if (dev == NULL)
    return E_Exit("DOS_DelDevice() with null ptr");
  ...
  for (Bitu i = 0; i < DOS_DEVICES; i++) {
    if (Devices[i] !=
        NULL) { /* This code sets Devices[i] to NULL after delete on device name
                   match so don't assume it's non-NULL! */
      if (!strcmp(Devices[i]->name, dev->name)) {
        //				LOG_MSG("DOS_DelDevice() %s
        //(%p)",dev->name,(void*)dev);
        delete Devices[i];
        Devices[i] = nullptr;
        return;
  ...
  /* hm. unfortunately, too much code in DOSBox assumes that we delete the
   * object. prior to this fix, failure to delete caused a memory leak */
  LOG_MSG("WARNING: DOS_DelDevice() failed to match device object '%s' (%p). "
          "Deleting anyway",
          dev->name, (void *)dev);
  delete dev;
}
...
void DOS_ShutdownDevices(void) {
  for (Bitu i = 0; i < DOS_DEVICES; i++) {
    if (Devices[i] != NULL) {
      //			LOG_MSG("DOS: Shutting down device %s
