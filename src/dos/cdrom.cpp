...
bool CDROM_Interface_SDL::ReadSectorsHost(void *buffer, bool raw,
...
bool CDROM_Interface_SDL::GetAudioTracks(int &stTrack, int &end,
                                         TMSF &leadOut) {
  ...
  if (CD_INDRIVE(SDL_CDStatus(cd))) {
    ...
    FRAMES_TO_MSF(cd->track[cd->numtracks].offset, &leadOut.min, &leadOut.sec,
                  &leadOut.fr);
  }
...
bool CDROM_Interface_SDL::GetAudioTrackInfo(int track, TMSF &start,
  ...
  if (CD_INDRIVE(SDL_CDStatus(cd))) {
    FRAMES_TO_MSF(cd->track[track - 1].offset, &start.min, &start.sec,
                  &start.fr);
    attr =
        cd->track[track - 1].type
        << 4; // sdl uses 0 for audio and 4 for data. instead of 0x00 and 0x40
  }
  return CD_INDRIVE(SDL_CDStatus(cd));
}
...
bool CDROM_Interface_SDL::GetAudioSub(unsigned char &attr, unsigned char &track,
  ...
  if (CD_INDRIVE(SDL_CDStatus(cd))) {
    ...
    FRAMES_TO_MSF((unsigned int)cd->cur_frame, &relPos.min, &relPos.sec,
                  &relPos.fr);
    FRAMES_TO_MSF((unsigned int)cd->cur_frame + cd->track[track].offset,
                  &absPos.min, &absPos.sec, &absPos.fr);
  }
  return CD_INDRIVE(SDL_CDStatus(cd));
}
...
bool CDROM_Interface_SDL::PlayAudioSector(unsigned long start,
                                          unsigned long len) {
  (void)start; // POSSIBLY UNUSED
...
int CDROM_GetMountType(const char *path, int forceCD) {
  ...
  int pstat = pref_stat(path, &file_stat),
      hstat = host_name == NULL ? 1 : ht_stat(host_name, &hfile_stat);
  if ((!pstat && (file_stat.st_mode & S_IFREG)) ||
      (pstat && !hstat && (hfile_stat.st_mode & S_IFREG)))
    return 1;
#else
  if ((pref_stat(path, &file_stat) == 0) && (file_stat.st_mode & S_IFREG))
    return 1;
#endif
  return 2;
}
...
bool CDROM_Interface_Fake ::GetAudioTracks(int &stTrack, int &end,
                                           TMSF &leadOut) {
  stTrack = end = 1;
  leadOut.min = 60;
...
bool CDROM_Interface_Fake ::GetAudioSub(unsigned char &attr,
                                        unsigned char &track,
                                        unsigned char &index, TMSF &relPos,
                                        TMSF &absPos) {
  attr = 0;
...
bool CDROM_Interface_Fake::ReadSectorsHost(void *buffer, bool raw,
                                           unsigned long sector,
                                           unsigned long num) {
  (void)buffer; // UNUSED
  (void)sector; // UNUSED
