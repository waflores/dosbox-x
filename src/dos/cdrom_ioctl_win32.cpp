...
bool CDROM_Interface_Ioctl::mci_CDPosition(int *position) {
  *position = 0;
  ...
  case MCI_MODE_PAUSE:
    ...
    if (!mci_CDioctl(MCI_STATUS, flags, &mci_status)) {
      *position = MSF_TO_FRAMES(MCI_MSF_MINUTE(mci_status.dwReturn),
...
bool CDROM_Interface_Ioctl::GetAudioTracks(int &stTrack, int &endTrack,
                                           TMSF &leadOut) {
  CDROM_TOC toc;
  DWORD byteCount;
  BOOL bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_READ_TOC, NULL, 0, &toc,
                               sizeof(toc), &byteCount, NULL);
  if (!bStat)
    return false;
  ...
  if ((use_mciplay || use_dxplay) && (!track_start_valid)) {
    ...
    for (Bits i = toc.FirstTrack; i <= toc.LastTrack + 1; i++) {
      if (((toc.TrackData[i].Control & 1) == 0) || (i == toc.LastTrack + 1)) {
        track_start[track_num] =
            MSF_TO_FRAMES(toc.TrackData[track_num].Address[1],
...
bool CDROM_Interface_Ioctl::GetAudioTrackInfo(int track, TMSF &start,
                                              unsigned char &attr) {
  CDROM_TOC toc;
  DWORD byteCount;
  BOOL bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_READ_TOC, NULL, 0, &toc,
...
bool CDROM_Interface_Ioctl::GetAudioSub(unsigned char &attr,
                                        unsigned char &track,
  ...
  if (use_dxplay) {
    ...
    if (GetAudioTracksAll()) {
      ...
      for (int i = track_start_first; i <= track_start_last; i++) {
        if ((player.currFrame + 150 < track_start[i + 1]) &&
            (player.currFrame + 150 >= track_start[i])) {
          // track found, calculate relative position
          track = i;
          FRAMES_TO_MSF(player.currFrame + 150 - track_start[i], &relPos.min,
  ...
  BOOL bStat =
      DeviceIoControl(hIOCTL, IOCTL_CDROM_READ_Q_CHANNEL, &insub, sizeof(insub),
      ...
      if (GetAudioTracksAll()) {
        for (int i = track_start_first; i <= track_start_last; i++) {
          if ((cur_pos < track_start[i + 1]) && (cur_pos >= track_start[i])) {
            // track found, calculate relative position
            FRAMES_TO_MSF(cur_pos - track_start[i], &relPos.min, &relPos.sec,
                          &relPos.fr);
            break;
          }
        }
...
bool CDROM_Interface_Ioctl::GetAudioStatus(bool &playing, bool &pause) {
  ...
  playing =
      (sub.CurrentPosition.Header.AudioStatus == AUDIO_STATUS_IN_PROGRESS);
...
bool CDROM_Interface_Ioctl::GetMediaTrayStatus(bool &mediaPresent,
  ...
  mediaChanged = (oldLeadOut.min != leadOut.min) ||
...
bool CDROM_Interface_Ioctl::PlayAudioSector(unsigned long start,
                                            unsigned long len) {
  if (use_mciplay) {
    if (!mci_CDPlay(start + 150, len))
      return true;
    if (!mci_CDPlay(start + 150, len - 1))
      return true;
    return false;
  }
  if (use_dxplay) {
  ...
  BOOL bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_PLAY_AUDIO_MSF, &audio,
                               sizeof(audio), NULL, 0, &byteCount, NULL);
  return bStat > 0;
}
...
bool CDROM_Interface_Ioctl::PauseAudio(bool resume) {
  ...
  DWORD byteCount;
  if (resume)
    bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_RESUME_AUDIO, NULL, 0, NULL, 0,
                            &byteCount, NULL);
  else
    bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_PAUSE_AUDIO, NULL, 0, NULL, 0,
                            &byteCount, NULL);
  return bStat > 0;
}
...
bool CDROM_Interface_Ioctl::StopAudio(void) {
  ...
  BOOL bStat;
  DWORD byteCount;
  bStat = DeviceIoControl(hIOCTL, IOCTL_CDROM_STOP_AUDIO, NULL, 0, NULL, 0,
                          &byteCount, NULL);
  return bStat > 0;
}
...
bool CDROM_Interface_Ioctl::LoadUnloadMedia(bool unload) {
  ...
  DWORD byteCount;
  if (unload)
    bStat = DeviceIoControl(hIOCTL, IOCTL_STORAGE_EJECT_MEDIA, NULL, 0, NULL, 0,
                            &byteCount, NULL);
  else
    bStat = DeviceIoControl(hIOCTL, IOCTL_STORAGE_LOAD_MEDIA, NULL, 0, NULL, 0,
                            &byteCount, NULL);
  track_start_valid = false;
  return bStat > 0;
}

bool CDROM_Interface_Ioctl::ReadSector(uint8_t *buffer, bool raw,
  ...
  if (!raw) {
    ...
    int success = 0;
    DWORD newPos =
        SetFilePointer(hIOCTL, sector * COOKED_SECTOR_SIZE, 0, FILE_BEGIN);
    if (newPos != 0xFFFFFFFF)
      success = ReadFile(hIOCTL, buffer, buflen, &byteCount, NULL);
    bStat = (success != 0);
  } else {
    // Raw
    RAW_READ_INFO in;
...
bool CDROM_Interface_Ioctl::ReadSectors(PhysPt buffer, bool raw,
                                        unsigned long sector,
                                        unsigned long num) {
  BOOL bStat;
  DWORD byteCount = 0;
  ...
  if (!raw) {
    ...
    DWORD newPos =
        SetFilePointer(hIOCTL, sector * COOKED_SECTOR_SIZE, 0, FILE_BEGIN);
    if (newPos != 0xFFFFFFFF)
      success = ReadFile(hIOCTL, bufdata, buflen, &byteCount, NULL);
    bStat = (success != 0);
  } else {
    // Raw
    RAW_READ_INFO in;
...
void CDROM_Interface_Ioctl::dx_CDAudioCallBack(Bitu len) {
  ...
  while (player.bufLen < (Bits)len) {
    ...
    if (player.targetFrame > player.currFrame)
      success = player.cd->ReadSector(&player.buffer[player.bufLen], true,
                                      player.currFrame);
    else
      success = false;
  ...
  if (player.ctrlUsed) {
    ...
    for (Bitu pos = 0; pos < len / 4; pos++) {
      ...
      samples[pos * 2 + 0] =
          (int16_t)(sample0 * player.ctrlData.vol[0] / 255.0);
      samples[pos * 2 + 1] =
          (int16_t)(sample1 * player.ctrlData.vol[1] / 255.0);
    }
  }
    ...
    if (Open()) {
      ...
      if (!use_mciplay) {
        if (cdioctl_cda_selected == CDIOCTL_CDA_DX) {
          ...
          if (!player.channel) {
            player.channel =
                MIXER_AddChannel(&dx_CDAudioCallBack, 44100, "CDAUDIO");
          }
...
void CDROM_Interface_Ioctl::Close(void) { CloseHandle(hIOCTL); }

bool CDROM_Interface_Ioctl::ReadSectorsHost(void *buffer, bool raw,
                                            unsigned long sector,
                                            unsigned long num) {
  (void)buffer;
  (void)raw;
