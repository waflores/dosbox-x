 ...
 *        That means 0x9FFF if 640KB or more, or a lesser value if less than
 * 640KB */
...
void DOS_CompressMemory(uint16_t first_segment = 0 /*default*/) {
  ...
  while (mcb.GetType() != 'Z') {
    if (counter++ > 10000000)
      DOS_Mem_E_Exit("DOS_CompressMemory: DOS MCB list corrupted.");
    mcb_next.SetPt((uint16_t)(mcb_segment + mcb.GetSize() + 1));
    if (GCC_UNLIKELY((mcb_next.GetType() != 0x4d) &&
                     (mcb_next.GetType() != 0x5a)))
      DOS_Mem_E_Exit("Corrupt MCB chain");
    if (mcb_segment >= first_segment && (mcb.GetPSPSeg() == MCB_FREE) &&
...
void DOS_FreeProcessMemory(uint16_t pspseg) {
  ...
  for (;;) {
    if (counter++ > 10000000)
      DOS_Mem_E_Exit("DOS_FreeProcessMemory: DOS MCB list corrupted.");
...
uint16_t DOS_GetMemAllocStrategy() { return memAllocStrategy; }
...
void DOS_zeromem(uint16_t seg, uint16_t para) {
  ...
  cnt = ((uint32_t)para << 4);
  if ((ofs + cnt) > 0x100000)
    E_Exit("DOS_zeromem out of range");
  while (cnt != 0) {
    mem_writeb(ofs++, 0);
    cnt--;
  }
...
uint16_t DOS_GetMaximumFreeSize(uint16_t minBlocks) {
  if (memAllocStrategy & 0xc0) {
    auto umbFreeSize =
        GetMaximumMCBFreeSize(dos_infoblock.GetStartOfUMBChain());
    if (memAllocStrategy & 0x40 || umbFreeSize >= minBlocks)
      return umbFreeSize;
  }
...
bool DOS_AllocateMemory(uint16_t *segment, uint16_t *blocks) {
  ...
  if (umb_start == UMB_START_SEG &&
    ...
    static constexpr std::array<const char *, 4> blacklisted{"KNL", "LIMD",
                                                             "PY", "RDFNT"};
    for (auto prog : blacklisted)
      if (!strcmp(psp_name, prog))
        return false;
  }
      ...
      if (block_size < (*blocks)) {
        ...
        *segment = mcb_segment + 1;
        ...
        if (dbg_zero_on_dos_allocmem)
          DOS_zeromem(*segment, *blocks);
#ifdef DEBUG_ALLOC
        LOG(LOG_DOSMISC,
            LOG_DEBUG)("DOS_AllocateMemory(blocks=0x%04x) = 0x%04x-0x%04x",
            ...
            if (found_seg_size == *blocks) {
              ...
              *segment = found_seg + 1;
              ...
              LOG(LOG_DOSMISC, LOG_DEBUG)(
                  "DOS_AllocateMemory(blocks=0x%04x) = 0x%04x-0x%04x", *blocks,
                  *segment, *segment + *blocks - 1);
#endif
              return true;
            }
...
#ifdef DEBUG_ALLOC
  LOG(LOG_DOSMISC,
      LOG_DEBUG)("DOS_AllocateMemory(blocks=0x%04x) = 0x%04x-0x%04x", *blocks,
                 *segment, *segment + *blocks - 1);
#endif
  return false;
}
...
bool DOS_ResizeMemory(uint16_t segment, uint16_t *blocks) {
...
#ifdef DEBUG_ALLOC
  LOG(LOG_DOSMISC, LOG_DEBUG)("DOS_ResizeMemory(seg=0x%04x) blocks=0x%04x",
                              segment, *blocks);
#endif
  ...
  if (mcb.GetPSPSeg() == MCB_FREE && !freed_mcb_allocate_on_resize) {
    *blocks = 0;
    DOS_SetError(DOSERR_MCB_DESTROYED);
    LOG(LOG_DOSMISC, LOG_DEBUG)("DOS application attempted to resize freed "
                                "memory block at segment 0x%x",
  ...
  if (*blocks <= total) {
    if (GCC_UNLIKELY(*blocks == total)) {
      /* Nothing to do, however if the block is freed, canonical MS-DOS behavior
       * is to assign your PSP segment as if allocated (Incentiv by DID, party
       * '94) */
      if (mcb.GetPSPSeg() == MCB_FREE && freed_mcb_allocate_on_resize)
        mcb.SetPSPSeg(dos.psp());
...
bool DOS_LinkUMBsToMemChain(uint16_t linkstate) {
  ...
  if (umb_start != UMB_START_SEG) {
    ...
    return false;
             ...
             dos.loaded_codepage == 951) {
    static constexpr std::array<const char *, 6> blacklisted{
        "KNL", "LIMD", "PY", "RDFNT", "HAN16E", "HAN16V"};
    char psp_name[9];
    DOS_MCB psp_mcb(dos.psp() - 1);
    psp_mcb.GetFileName(psp_name);
    for (auto prog : blacklisted)
  ...
  case 0x0001: // link
    if (mcb.GetType() == 0x5a) {
      if ((mcb_segment + mcb.GetSize() + 1) != umb_start) {
        LOG_MSG("MCB chain no longer goes to end of memory (corruption?), not "
                "linking in UMB!");
        return false;
      }
      mcb.SetType(0x4d);
...
void DOS_SetupMemory(void) {
  ...
  assert(DOS_IHSEG != 0);
  ihseg = DOS_IHSEG;
  ihofs = 0x0E; /* DOS_IHSEG is one paragraph (16 bytes) and code below may
                   write with negative offsets below */

  real_writeb(ihseg, ihofs, (uint8_t)0xCF); // An IRET Instruction
  if (machine != MCH_PCJR)
    RealSetVec(0x02,
               RealMake(ihseg, ihofs)); // BioMenace (segment<0x8000). Else,
                                        // taken by BIOS NMI interrupt
  RealSetVec(0x03, RealMake(ihseg, ihofs)); // Alien Incident (offset!=0)
  RealSetVec(
      0x04,
      RealMake(ihseg, ihofs)); // Shadow President (lower byte of segment!=0)
  RealSetVec(0x0f,
             RealMake(ihseg, ihofs)); // Always a tricky one (soundblaster irq)

  // If INT 01h is executed, take the additional steps to clear the TF flag
  // before return. Needed for 1996 demo "Dyslexia" by Threesome, which, for
  // some reason when Sound Blaster is involved, likes to set the TF (trap flag)
  // during the routine to switch back into protected mode. It will crash right
  // after the LIDT instruction because of the change in interrupt table
  // location.
  //
  // Demo ref:
  // [http://files.scene.org/get:nl-http/mirrors/hornet/demos/1996/d/dyslexia.zip]
  //
  // FIXME: What do real Intel processors do in real mode if a nonzero base
  // address is given in
  //        LIDT? Does it change the memory address of the real mode interrupt
  //        vector table?
  if (dos_clear_tf_on_int01) {
    ...
    real_writew(
        ihseg, ihofs - 6,
        0x6681); // 81 66 06 FEFF  AND WORD PTR [BP+6],FEFF   clear the TF bit
  ...
  if (machine == MCH_TANDY) {
    ...
    /* If there is more than 128KB of RAM, then the MCB chain must be
     * constructed to exclude video memory. In that case, the BIOS values in the
     * BDA will report actual memory size. I think this is what those DOS
     * drivers for PCjr do that enable the use of additional RAM provided by a
     * sidecar.
     *
     * If there is 128KB or less, then construct a normal MCB. The BIOS will
     * subtract 16KB from top of memory. */
