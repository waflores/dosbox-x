...
bool WildFileCmp(const char *file, const char *wild) {
  ...
  if (uselfn && strchr(wild_name, '*')) {
    if (strchr(wild, '.')) {
      if (!wild_match(file_name, wild_name))
        return false;
      goto checkext;
    } else
...
bool LWildFileCmp(const char *file, const char *wild) {
  if ((!uselfn && !wildmount) || !file || !wild || (*file && !*wild) ||
      strlen(wild) > LFN_NAMELENGTH)
    return false;
  char file_name[256];
  char file_ext[256];
  char wild_name[256];
  ...
  if (find_ext) {
    ...
    memcpy(file_ext, find_ext,
           (strlen(find_ext) > 255) ? 255 : strlen(find_ext));
  } else {
    size = strlen(file);
    elen = 0;
...
int get_expanded_files(const std::string &path, std::vector<std::string> &paths,
  ...
  host_cnv_char_t *host_name =
      CodePageGuestToHost((std::string(pdir) + "\\*.*").c_str());
  if (host_name != NULL)
    hFind = FindFirstFileW(host_name, &fdw);
  else
    hFind = FindFirstFile((std::string(pdir) + "\\*.*").c_str(), &fd);
  ...
  for (std::string name : names) {
    if (LWildFileCmp(name.c_str(), pattern))
      files.push_back((readonly ? ":" : "") + std::string(pdir) +
                      std::string(1, CROSS_FILESPLIT) + name);
  }
  wildmount = false;
...
void Set_Label(char const *const input, char *const output, bool cdrom) {
  ...
  // DBCS_upcase(upcasebuf);  /* Another mscdex quirk. Label is not always
  // uppercase. (Daggerfall) */

  while (togo > 0) {
    if (upcasebuf[vnamePos] == 0)
      str_end = true;
    else if (cdrom && vnamePos == 8 && !str_end && upcasebuf[vnamePos] != '.') {
      output[labelPos] = '.'; // add a dot between 8th and 9th character
                              // (Descent 2 installer needs this)
      labelPos++;
    }
    output[labelPos] =
        !str_end ? upcasebuf[vnamePos] : 0x0; // Pad empty characters with 0x00
    labelPos++;
    vnamePos++;
    togo--;
  }
...
const char *DOS_Drive::GetInfo(void) { return info; }
...
1) % DOS_DRIVES; int numDisks = driveInfos[currentDrive].disks.size(); if
...
void DriveManager::CycleDisk(bool pressed) {
        ...
        if (numDisks > 1) {
                ...
                DOS_Drive* oldDisk =
driveInfos[currentDrive].disks[currentDisk]; currentDisk = (currentDisk + 1) %
numDisks; DOS_Drive* newDisk = driveInfos[currentDrive].disks[currentDisk];
...
void DriveManager::CycleDisks(int drive, bool notify, unsigned int position) {
  ...
  if (numDisks > 1) {
    ...
    if (drive < MAX_DISK_IMAGES && imageDiskList[drive] != NULL) {
      ...
      if (imageDiskList[drive] != NULL)
        imageDiskList[drive]->Addref();
      if ((drive == 2 || drive == 3) && imageDiskList[drive]->hardDrive)
        updateDPT();
    }

    // copy working directory, acquire system resources and finally switch to
    // next drive
    ...
    if (notify)
      LOG_MSG("Drive %c: disk %d of %d now active", 'A' + drive,
...
void DriveManager::CycleAllDisks(void) {
  for (int idrive = 0; idrive < 2; idrive++)
    CycleDisks(idrive, true); /* Cycle all DISKS meaning A: and B: */
}

void DriveManager::CycleAllCDs(void) {
  for (unsigned int idrive = 2; idrive < DOS_DRIVES;
       idrive++) { /* Cycle all CDs in C: D: ... Z: */
    ...
    if (numDisks > 1) {
      ...
      if (dynamic_cast<const isoDrive *>(oldDisk) == NULL)
        continue;
      currentDisk = (currentDisk + 1u) % numDisks;
      ...
      LOG_MSG("Drive %c: disk %d of %d now active", 'A' + idrive,
              currentDisk + 1, numDisks);
    }
  }
}
...
int DriveManager::UnmountDrive(int drive) {
  ...
  if (driveInfos[drive].disks.size() == 0) {
    ...
    if (result == 0) {
      ...
      for (unsigned int i = 0; i < (unsigned int)driveInfos[drive].disks.size();
           i++) {
...
char *DriveManager::GetDrivePosition(int drive) {
  sprintf(swappos, "%d / %d", driveInfos[drive].currentDisk + 1,
...
void DriveManager::Init(Section *s) {
  ...
  int13_disk_change_detect_enable =
      section->Get_bool("int 13 disk change detect");
...
char *DOS_Drive::GetBaseDir(void) { return info + 16; }
