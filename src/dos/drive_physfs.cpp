...
static char *normalize(char *name, const char *basedir) {
  ...
  while (last >= 0 && name[last] == '/')
    name[last--] = 0;
  if (last > 0 && name[last] == '.' && name[last - 1] == '/')
    name[last - 1] = 0;
  if (last > 1 && name[last] == '.' && name[last - 1] == '.' &&
      name[last - 2] == '/') {
    name[last - 2] = 0;
...
public:
  physfsFile(const char *name, PHYSFS_file *handle, uint16_t devinfo,
             const char *physname, bool write);
...
bool physfsDrive::AllocationInfo(uint16_t *_bytes_sector,
  ...
  if (wdir) {
    ...
    long unsigned int dwSectPerClust, dwBytesPerSect, dwFreeClusters,
        dwTotalClusters;
    uint8_t drive =
        strlen(wdir) > 1 && wdir[1] == ':' ? toupper(wdir[0]) - 'A' + 1 : 0;
    if (drive > 26)
      drive = 0;
    char root[4] = "A:\\";
    root[0] = 'A' + drive - 1;
    res = GetDiskFreeSpace(drive ? root : NULL, &dwSectPerClust,
    ...
    if (res) {
      ...
      *_total_clusters =
          total > 4194240
              ? 65535
              : (uint16_t)(dwTotalClusters * dwSectPerClust / ratio);
      *_free_clusters =
          dwFreeClusters
              ? (total > 4194240
                     ? 61440
                     : (uint16_t)(dwFreeClusters * dwSectPerClust / ratio))
              : 0;
      if (rsize) {
...
void *physfsDrive::opendir(const char *name) {
  ...
  if (statbuf.filetype != PHYSFS_FILETYPE_DIRECTORY)
    return NULL;
  struct opendirinfo *oinfo =
      (struct opendirinfo *)malloc(sizeof(struct opendirinfo));
  strcpy(oinfo->dir, myname);
  oinfo->files = PHYSFS_enumerateFiles(myname);
  if (oinfo->files == NULL) {
    const PHYSFS_ErrorCode err = PHYSFS_getLastErrorCode();
    const char *errorMessage =
        err ? PHYSFS_getErrorByCode(err) : "Unknown error";
...
void physfsDrive::closedir(void *handle) {
  ...
  if (handle == NULL)
    return;
  if (oinfo->files != NULL)
    PHYSFS_freeList(oinfo->files);
  free(oinfo);
}

bool physfsDrive::read_directory_first(void *dirp, char *entry_name,
                                       char *entry_sname, bool &is_directory) {
  return read_directory_next(dirp, entry_name, entry_sname, is_directory);
}

bool physfsDrive::read_directory_next(void *dirp, char *entry_name,
                                      char *entry_sname, bool &is_directory) {
  ...
  if (!oinfo->files || !oinfo->files[oinfo->pos])
    return false;
  safe_strncpy(entry_name, oinfo->files[oinfo->pos++], CROSS_LEN);
  *entry_sname = 0;
  is_directory = isdir(
      strlen(oinfo->dir)
          ? (std::string(oinfo->dir) + "/" + std::string(entry_name)).c_str()
          : entry_name);
  return true;
}

static uint8_t physfs_used = 0;
physfsDrive::physfsDrive(const char driveLetter, const char *startdir,
  ...
  char *dir = strchr(
      lastdir +
          (((lastdir[0] | 0x20) >= 'a' && (lastdir[0] | 0x20) <= 'z') ? 2 : 0),
      ':');
  while (dir) {
    *dir++ = 0;
    if ((lastdir == newname) &&
        !strchr(
            dir + (((dir[0] | 0x20) >= 'a' && (dir[0] | 0x20) <= 'z') ? 2 : 0),
            ':')) {
      // If the first parameter is a directory, the next one has to be the
      // archive file, do not confuse it with basedir if trailing : is not
      // there!
    ...
    lastdir = dir;
    dir = strchr(lastdir +
                     (((lastdir[0] | 0x20) >= 'a' && (lastdir[0] | 0x20) <= 'z')
                          ? 2
                          : 0),
                 ':');
  }
...
bool physfsDrive::FileCreate(DOS_File **file, const char *name,
                             uint16_t attributes) {
...
bool physfsDrive::Rename(const char *oldname, const char *newname) {
  ...
  if (dir && !strcmp(getOverlaydir(), dir)) {
    ...
    LOG_MSG("PHYSFS: rename not supported (%s -> %s)", newold,
            newnew); // yuck. physfs doesn't have "rename".
  return false;
}
...
unsigned long physfsDrive::GetSerial() { return localDrive::GetSerial(); }
#endif
...
again:
  ...
  if (statbuf.filetype == PHYSFS_FILETYPE_DIRECTORY)
    find_attr = DOS_ATTR_DIRECTORY | DOS_ATTR_ARCHIVE;
  else
  ...
  uint32_t find_size, find_hsize = 0;
...
bool physfsDrive::FindFirst(const char *_dir, DOS_DTA &dta,
                            bool /*fcb_findfirst*/) {
  return localDrive::FindFirst(_dir, dta);
}

bool physfsDrive::isRemote(void) { return localDrive::isRemote(); }

bool physfsDrive::isRemovable(void) { return true; }
...
bool physfsFile::Read(uint8_t *data, uint16_t *size) {
  ...
  // LOG_MSG("Read %i bytes (wanted %i) at %i of %s
  // (%s)",(int)mysize,(int)*size,(int)PHYSFS_tell(fhandle),name,PHYSFS_getLastError());
...
bool physfsFile::Write(const uint8_t *data, uint16_t *size) {
  ...
  if (*size == 0) {
    if (PHYSFS_tell(fhandle) == 0) {
      ...
      LOG_MSG("PHYSFS TODO: truncate not yet implemented (%s at %i)", pname,
              (int)PHYSFS_tell(fhandle));
      return false;
    }
  } else {
    PHYSFS_sint64 mysize = PHYSFS_writeBytes(fhandle, data, *size);
    // LOG_MSG("Wrote %i bytes (wanted %i) at %i of %s
    // (%s)",(int)mysize,(int)*size,(int)PHYSFS_tell(fhandle),name,PHYSFS_getLastError());
    *size = (uint16_t)mysize;
    return true;
  }
...
bool physfsFile::Seek(uint32_t *pos, uint32_t type) {
  ...
  case DOS_SEEK_SET:
    break;
  case DOS_SEEK_CUR:
    mypos += PHYSFS_tell(fhandle);
    break;
  case DOS_SEEK_END:
    mypos += PHYSFS_fileLength(fhandle);
    break;
  default:
  ...
  // LOG_MSG("Seek to %i (%i at %x) of %s
  // (%s)",(int)mypos,(int)*pos,type,name,PHYSFS_getLastError());
...
bool physfsFile::prepareWrite() {
  ...
  if (strcmp(fdir, wdir)) { /* we need COW */
    ...
    PHYSFS_close(fhandle);
    fhandle = whandle;
  } else { // megayuck - physfs on posix platforms uses O_APPEND. We illegally
           // access the fd directly and clear that flag.
...
uint16_t physfsFile::GetInformation(void) { return info; }

physfsFile::physfsFile(const char *_name, PHYSFS_file *handle, uint16_t devinfo,
                       const char *physname, bool write) {
  fhandle = handle;
  info = devinfo;
  strcpy(pname, physname);
  PHYSFS_Stat statbuf;
  if (!PHYSFS_stat(pname, &statbuf))
    return;
  time_t mytime = statbuf.modtime;
  /* Convert the stat to a FileStat */
  struct tm *time;
  if ((time = localtime(&mytime)) != nullptr) {
    this->time = DOS_PackTime((uint16_t)time->tm_hour, (uint16_t)time->tm_min,
                              (uint16_t)time->tm_sec);
    this->date =
        DOS_PackDate((uint16_t)(time->tm_year + 1900),
                     (uint16_t)(time->tm_mon + 1), (uint16_t)time->tm_mday);
...
physfscdromDrive::physfscdromDrive(
    const char driveLetter, const char *startdir, uint16_t _bytes_sector,
    uint8_t _sectors_cluster, uint16_t _total_clusters, uint16_t _free_clusters,
    uint8_t _mediaid, int &error, std::vector<std::string> &options)
    : physfsDrive(driveLetter, startdir, _bytes_sector, _sectors_cluster,
                  _total_clusters, _free_clusters, _mediaid, error, options) {
  // Init mscdex
  if (!mountarc.size()) {
    error = 10;
    return;
...
bool physfscdromDrive::FileOpen(DOS_File **file, const char *name,
                                uint32_t flags) {
...
bool physfscdromDrive::FileCreate(DOS_File **file, const char *name,
                                  uint16_t attributes) {
  (void)file;       // UNUSED
  (void)name;       // UNUSED
  (void)attributes; // UNUSED
...
bool physfscdromDrive::FindFirst(const char *_dir, DOS_DTA &dta,
                                 bool fcb_findfirst) {
...
bool physfscdromDrive::isRemote(void) { return true; }

bool physfscdromDrive::isRemovable(void) { return true; }
