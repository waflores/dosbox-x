...
extern bool filename_not_8x3(const char *n),
    filename_not_strict_8x3(const char *n);
bool CodePageHostToGuestUTF16(char *d /*CROSS_LEN*/,
                              const uint16_t *s /*CROSS_LEN*/);
inline bool CodePageHostToGuestUTF16(uint8_t *d /*CROSS_LEN*/,
                                     const uint8_t *s /*CROSS_LEN*/) {
  std::u16string u16s(reinterpret_cast<const char16_t *>(s));
  return CodePageHostToGuestUTF16(
      reinterpret_cast<char *>(d),
      reinterpret_cast<const uint16_t *>(u16s.c_str()));
}

using namespace std;

static bool islfnchar(const char *s) {
  return (unsigned char)*s <= 32 || (unsigned char)*s == 127 || *s == '\"' ||
         *s == '+' || *s == '=' || *s == ',' || *s == ';' || *s == ':' ||
         *s == '<' || *s == '>' || *s == '?' || *s == '*';
}
...
bool UDFTagId::tagChecksumOK(const unsigned int sz,
...
bool UDFTagId::dataChecksumOK(const unsigned int sz,
                              const unsigned char *b) const {
...
void UDFAnchorVolumeDescriptorPointer::get(UDFTagId &tag, const unsigned int sz,
...
UDFAnchorVolumeDescriptorPointer::UDFAnchorVolumeDescriptorPointer(
    UDFTagId &tag, const unsigned int sz, const unsigned char *b) {
  get(tag, sz, b);
}
...
UDFextent_ad
isoDrive::convertToUDFextent_ad(const UDFshort_ad &s,
                                const uint32_t partition_ref_id) const {
  UDFextent_ad r;
...
bool isoDrive::convertToUDFextent_ad(UDFextent_ad &d,
                                     const UDFextent_ad &s) const {
  d = s;
  return true;
}

bool isoDrive::convertToUDFextent_ad(UDFextent_ad &d, const UDFshort_ad &s,
                                     const uint32_t partition_ref_id) const {
  if (partd.DescriptorTag.TagIdentifier != 0 &&
      (partition_ref_id == partd.PartitionNumber ||
       partition_ref_id == 0xFFFFFFFFu)) {
...
bool isoDrive::convertToUDFextent_ad(UDFextent_ad &d,
                                     const UDFlong_ad &s) const {
  if (partd.DescriptorTag.TagIdentifier != 0 &&
      s.ExtentLocation.PartitionReferenceNumber == partd.PartitionNumber) {
    d.ExtentLocation =
        s.ExtentLocation.LogicalBlockNumber + partd.PartitionStartingLocation;
    d.ExtentLength = s.ExtentLength;
    return true;
  }
...
void UDFPrimaryVolumeDescriptor::get(
    UDFTagId &tag /*already parsed, why parse again?*/, const unsigned int sz,
    const unsigned char *b) {
  if (sz >= 490) {
    DescriptorTag = tag;
...
void UDFFileEntry::get(UDFTagId &tag /*already parsed, why parse again?*/,
                       const unsigned int sz, const unsigned char *b) {
  if (sz >= 176) {
    DescriptorTag = tag;
    ICBTag.get(20, b + 16);
    ...
    case 0: // short_ad
      for (unsigned int i = 0; (i + 8u) <= LengthOfAllocationDescriptors;
           i += 8) {
      ...
      break;
    case 1: // long_ad
      for (unsigned int i = 0; (i + 16u) <= LengthOfAllocationDescriptors;
           i += 16) {
        const size_t si = i + allo;
        UDFlong_ad sad;
      ...
      break;
    case 2: // ext_ad
      for (unsigned int i = 0; (i + 20u) <= LengthOfAllocationDescriptors;
           i += 20) {
        const size_t si = i + allo;
        UDFext_ad sad;
...
UDFFileEntry::UDFFileEntry(UDFTagId &tag /*already parsed, why parse again?*/,
                           const unsigned int sz, const unsigned char *b) {
  get(tag, sz, b);
}
...
void UDFFileIdentifierDescriptor::get(
    UDFTagId &tag /*already parsed, why parse again?*/, const unsigned int sz,
    const unsigned char *b) {
  if (sz >= 38) {
    DescriptorTag = tag;
    ...
    if ((ofs + LengthOfImplementationUse) <= sz &&
...
void UDFPartitionDescriptor::get(
    UDFTagId &tag /*already parsed, why parse again?*/, const unsigned int sz,
    const unsigned char *b) {
  if (sz >= 356) {
    DescriptorTag = tag;
...
void UDFLogicalVolumeDescriptor::get(
    UDFTagId &tag /*already parsed, why parse again?*/, const unsigned int sz,
    const unsigned char *b) {
  if (sz >= 440) {
    DescriptorTag = tag;
...
UDFextents::UDFextents(const struct UDFextent_ad &s) { xl.push_back(s); }
...
uint64_t isoDrive::UDFextent_seek(struct UDFextents &ex, uint64_t ofs) const {
  ...
  if (ex.is_indata) {
    ...
    ex.relofs = (uint32_t)ofs; // NTS: Assume no sane authoring program would
                               // put 4GB of data in extent area!
  } else {
...
unsigned int isoDrive::UDFextent_read(struct UDFextents &ex, unsigned char *buf,
                                      size_t count) const {
  unsigned int rd = 0;
  ...
  if (ex.is_indata) {
    ...
    while (count != size_t(0)) {
      if (ex.extent >= ex.xl.size())
        break;

      const auto &exs = ex.xl[ex.extent];

      assert(ex.relofs <= exs.ex.ExtentLength);
      size_t extodo = (size_t)std::min(
          (uint64_t)(exs.ex.ExtentLength - ex.relofs), (uint64_t)count);
      size_t sctofs = (size_t)(ex.relofs % (uint32_t)COOKED_SECTOR_SIZE);
      size_t sctodo = std::min(extodo, COOKED_SECTOR_SIZE - sctofs);
      uint32_t sector = exs.ex.ExtentLocation +
                        ((ex.relofs - sctofs) / (uint32_t)COOKED_SECTOR_SIZE);
...
public:
  isoFile(isoDrive *drive, const char *name, const FileStat_Block *stat,
          uint32_t offset);
...
isoFile::isoFile(isoDrive *drive, const char *name, const FileStat_Block *stat,
                 uint32_t offset)
    : drive(drive), fileBegin(offset) {
  time = stat->time;
  date = stat->date;
  attr = stat->attr;
  filePos = fileBegin;
...
uint32_t isoFile::GetSeekPos() { return filePos - fileBegin; }
...
isoDrive::isoDrive(char driveLetter, const char *fileName, uint8_t mediaid,
                   int &error, std::vector<std::string> &options) {
  enable_udf = (dos.version.major > 7 ||
                (dos.version.major == 7 && dos.version.minor >= 10)); // default
  ...
  for (const auto &opt : options) {
    ...
    if (name == "rr") { // Enable/disable Rock Ridge extensions
      if (value == "1" || value == "")
        enable_rock_ridge = true; // "-o rr" or "-o rr=1"
      else if (value == "0")
        enable_rock_ridge = false;
    } else if (name == "joliet") { // Enable/disable Joliet extensions
      if (value == "1" || value == "")
        enable_joliet = true; // "-o joliet" or "-o joliet=1"
      else if (value == "0")
        enable_joliet = false;
    } else if (name == "udf") { // Enable/disable UDF
      if (value == "1" || value == "")
        enable_udf = true; // "-o udf" or "-o udf=1"
      else if (value == "0")
        enable_udf = false;
    }
  }
  ...
  if (!error) {
    if (empty_drive) {
      ...
      this->mediaid = mediaid;
      char buffer[32] = {0};
      if (!MSCDEX_GetVolumeName(subUnit, buffer))
        strcpy(buffer, "");
      Set_Label(buffer, discLabel, true);
    } else if (CDROM_Interface_Image::images[subUnit]->HasDataTrack() ==
                   false &&
               CDROM_Interface_Image::images[subUnit]->HasAudioTrack() ==
                   true) { // Audio only cdrom
      strcpy(info, "isoDrive ");
      strcat(info, fileName);
      this->driveLetter = driveLetter;
      this->mediaid = mediaid;
...
int isoDrive::UpdateMscdex(char driveLetter, const char *path,
                           uint8_t &subUnit) {
...
void isoDrive::Activate(void) { UpdateMscdex(driveLetter, fileName, subUnit); }
...
bool isoDrive::FileOpen(DOS_File **file, const char *name, uint32_t flags) {
  ...
  if (is_udf) {
    ...
    UDFFileEntry fe;
    success = lookup(fid, fe, name) &&
              !(fid.FileCharacteristics & 0x02 /*Directory*/);
    if (success) {
      UDFextents fex;

      UDFFileEntryToExtents(fex, fe);

      file_stat.size = (uint32_t)std::min((uint64_t)fe.InformationLength,
                                          (uint64_t)0xFFFFFFFF);
      file_stat.date = DOS_PackDate(fe.ModificationDateAndTime.Year,
                                    fe.ModificationDateAndTime.Month,
                                    fe.ModificationDateAndTime.Day);
      file_stat.time = DOS_PackTime(fe.ModificationDateAndTime.Hour,
                                    fe.ModificationDateAndTime.Minute,
                                    fe.ModificationDateAndTime.Second);
    ...
    if (success) {
      ...
      file_stat.date =
          DOS_PackDate(1900 + de.dateYear, de.dateMonth, de.dateDay);
      file_stat.time = DOS_PackTime(de.timeHour, de.timeMin, de.timeSec);
      file_stat.attr = DOS_ATTR_ARCHIVE | DOS_ATTR_READ_ONLY;
      *file = new isoFile(this, name, &file_stat,
                          EXTENT_LOCATION(de) * ISO_FRAMESIZE);
      (*file)->flags = flags;
    }
  }
  ...
  if (is_udf) {
    ...
    if (attr == DOS_ATTR_VOLUME) {
      ...
      if (WildFileCmp(discLabel, pattern)) {
        // Get Volume Label (DOS_ATTR_VOLUME) and only in basedir and if it
...
bool isoDrive::FindNext(DOS_DTA &dta) {
  ...
  char pattern[CROSS_LEN], findName[DOS_NAMELENGTH_ASCII],
      lfindName[ISO_MAXPATHNAME];
  dta.GetSearchParams(attr, pattern, false);

  int dirIterator = lfn_filefind_handle >= LFN_FILEFIND_MAX
                        ? dta.GetDirID()
                        : sdid[lfn_filefind_handle];
  bool isRoot = dirIterators[dirIterator].root;

  if (is_udf) {
    UDFFileIdentifierDescriptor fid;
    UDFFileEntry fe;
    while (GetNextDirEntry(dirIterator, fid, fe,
                           dirIterators[dirIterator].udfdirext, fname,
                           dirIterators[dirIterator].index)) {
      uint8_t findAttr = DOS_ATTR_ARCHIVE | DOS_ATTR_READ_ONLY;
      if (fid.FileCharacteristics & 0x02 /*Directory*/)
        findAttr |= DOS_ATTR_DIRECTORY;

      /* skip parent directory */
      if (fid.LengthOfFileIdentifier == 0 || (fid.FileCharacteristics & 0x08))
        continue;

      strcpy(lfindName, fullname);
      if (!(isRoot && fname[0] == '.') &&
          (WildFileCmp((char *)fname, pattern) ||
           LWildFileCmp(lfindName, pattern)) &&
          !(~attr & findAttr &
        ...
        uint32_t findSize = (uint32_t)std::min((uint64_t)fe.InformationLength,
                                               (uint64_t)0xFFFFFFFF);
        uint16_t findDate = DOS_PackDate(fe.ModificationDateAndTime.Year,
                                         fe.ModificationDateAndTime.Month,
                                         fe.ModificationDateAndTime.Day);
        uint16_t findTime = DOS_PackTime(fe.ModificationDateAndTime.Hour,
                                         fe.ModificationDateAndTime.Minute,
                                         fe.ModificationDateAndTime.Second);
        dta.SetResult(findName, lfindName, findSize, 0, findDate, findTime,
                      findAttr);
        return true;
      }
    }
...
bool isoDrive::SetFileAttr(const char *name, uint16_t attr) {
  ...
  if (is_udf) {
    UDFFileIdentifierDescriptor fid;
    UDFFileEntry fe;
    DOS_SetError(lookup(fid, fe, name) ? DOSERR_ACCESS_DENIED
...
bool isoDrive::AllocationInfo(uint16_t *bytes_sector, uint8_t *sectors_cluster,
...
bool isoDrive::FileStat(const char *name, FileStat_Block *const stat_block) {
  if (is_udf) {
    ...
    if (success) {
      stat_block->date = DOS_PackDate(fe.ModificationDateAndTime.Year,
                                      fe.ModificationDateAndTime.Month,
                                      fe.ModificationDateAndTime.Day);
      stat_block->time = DOS_PackTime(fe.ModificationDateAndTime.Hour,
                                      fe.ModificationDateAndTime.Minute,
                                      fe.ModificationDateAndTime.Second);
      stat_block->size = (uint32_t)std::min((uint64_t)fe.InformationLength,
                                            (uint64_t)0xFFFFFFFF);
      stat_block->attr = DOS_ATTR_ARCHIVE | DOS_ATTR_READ_ONLY;
      if (fid.FileCharacteristics & 0x02 /*Directory*/)
        stat_block->attr |= DOS_ATTR_DIRECTORY;
    }
...
uint8_t isoDrive::GetMediaByte(void) { return mediaid; }
...
int isoDrive::GetDirIterator(const isoDirEntry *de) {
  ...
  // get start and end sector of the directory entry (pad end sector if
  // necessary)
...
bool isoDrive::GetNextDirEntry(const int dirIteratorHandle,
                               UDFFileIdentifierDescriptor &fid,
                               UDFFileEntry &fe, UDFextents &dirext,
                               char fname[LFN_NAMELENGTH],
                               unsigned int dirIteratorIndex) {
  if (!is_udf)
    return 0;
  ...
  if (UDFextent_read(dirext, dirent, 38) != 38)
    return false;
  ctag.parse(16, dirent);
  if (!ctag.tagChecksumOK(16, dirent) ||
      ctag.TagIdentifier != 257 /*File Identifier*/) {
...

  {
    ...
    if (!fe_found) {
      LOG(LOG_MISC,
          LOG_DEBUG)("UDF: File Identifier cannot find File Entry for '%s'",
                     fid.FileIdentifier.string_value().c_str());
      fe = UDFFileEntry();
    }
  }
  ...
  // NTS: For whatever reason, the first byte may be a control value between
  // 0x01 to 0x08.
  if (*enname >= 0x01 && *enname <= 0x08)
    enname++;
...

  {
     ...
     * I'm not 100% certain but the index appears to be related to the index of
     * the ISO 9660 dirent. This is a guess as to how it works. */
    if (lfn) {
      ...
      while (*s != 0) {
        if (s == ext)
          break; // doesn't match if ext == NULL, so no harm in that case
        if (!lead && ((IS_PC98_ARCH && shiftjis_lead_byte(*s & 0xFF)) ||
                      (isDBCSCP() && isKanji1_gbk(*s & 0xFF)))) {
          if (c >= (7 - tailsize))
            break;
          lead = true;
          *d++ = *s;
...
bool isoDrive::GetNextDirEntry(const int dirIteratorHandle, isoDirEntry *de) {
  ...
  if (dirIterator.valid &&
      ReadCachedSector(&buffer, dirIterator.currentSector)) {
    // check if the next sector has to be read
    if ((dirIterator.pos >= ISO_FRAMESIZE) || (buffer[dirIterator.pos] == 0) ||
    ...
    result = length >= 0;
    if (length > 0)
      dirIterator.pos += (unsigned int)length;
  }
...
bool isoDrive::ReadCachedSector(uint8_t **buffer, const uint32_t sector) {
  ...
  if (!he.valid || he.sector != sector) {
    if (!CDROM_Interface_Image::images[subUnit]->ReadSector(he.data, false,
                                                            sector)) {
      return false;
    }
    he.valid = true;
...
inline bool isoDrive ::readSector(uint8_t *buffer, uint32_t sector) const {
  if (CDROM_Interface_Image::images[subUnit] == nullptr)
    return false;
  return CDROM_Interface_Image::images[subUnit]->ReadSector(buffer, false,
                                                            sector);
}

int isoDrive::readDirEntry(isoDirEntry *de, const uint8_t *data,
                           unsigned int dirIteratorIndex) const {
  // This code is NOT for UDF filesystem access!
  if (is_udf)
    return -1;

  // copy data into isoDirEntry struct, data[0] = length of DirEntry
  //	if (data[0] > sizeof(isoDirEntry)) return -1;//check disabled as
  //isoDirentry is currently 258 bytes large. So it always fits
  ...
  if (IS_DIR(FLAGS2)) {
    if (de->fileIdentLength == 1 && de->ident[0] == 0)
      ...
      if (is_joliet) {
        ...
        for (size_t i = 0; ((const uint16_t *)de->ident)[i] != 0; i++)
          ((uint16_t *)de->ident)[i] = be16toh(((uint16_t *)de->ident)[i]);
        // finally, convert from UCS-16 to local code page, using C++ string
        // construction to make a copy first
    ...
    if (is_joliet) {
      de->ident[de->fileIdentLength + 1] = 0; // for Joliet UCS-16
      // remove any file version identifiers as there are some cdroms that don't
      // have them
      uint16_t *w = (uint16_t *)(de->ident); // remember two NULs were written
                                             // to make a UCS-16 NUL
      while (*w != 0) {
        if (be16toh(*w) == (uint16_t)(';'))
          *w = 0;
        w++;
      }
      // w happens to be at the end of the string now, step back one char and
  ...
  if (is_joliet) {
    ...
    strcpy((char *)fullname, (char *)de->ident);
    if (is_rock_ridge) {
      /* LEN_SKP bytes into the System Use Field (bytes after the final NUL byte
       * of the identifier) */
      /* NTS: This code could never work with Joliet extensions because the code
       * above (currently) overwrites the first byte of the SUSP entries in
       * order to make a NUL-terminated UCS-16 string */
      unsigned char *p = (unsigned char *)de->ident + de->fileIdentLength +
                         1 /*NUL*/ + rr_susp_skip;
      ...
      while ((p + 4) <= f) {
        ...
        if (!memcmp(entry, "NM", 2)) {
          ...
          if (len >= 5 /*flags and at least one char*/ && ver == 1) {
            ...
            if (flags & 0x7) {
              /* CONTINUE (bit 0) not supported yet, or CURRENT directory (bit
               * 1), or PARENT directory (bit 2) */
            } else {
              /* BP6-payload end is the alternate name */
              size_t altnl = len - 5;

              if (altnl > 0)
                memcpy(fullname, entry + 5, altnl);
              fullname[altnl] = 0;
  ...
  bool jolietrr = is_joliet || (is_rock_ridge_name &&
                                filename_not_strict_8x3((char *)de->ident));
  if (!jolietrr && !(dos.version.major >= 7 || uselfn)) {
    ...
    } else if (strlen((char *)de->ident) > 8)
      de->ident[8] = 0;
  }
...
static bool escape_is_joliet(const unsigned char *esc) {
  if (!memcmp(esc, "\x25\x2f\x40", 3) || !memcmp(esc, "\x25\x2f\x43", 3) ||
...
bool isoDrive ::loadImageUDFAnchorVolumePointer(
    UDFAnchorVolumeDescriptorPointer &avdp, uint8_t pvd[COOKED_SECTOR_SIZE],
    uint32_t sector) const {
  UDFTagId aid;

  if (!aid.get(COOKED_SECTOR_SIZE, pvd))
    return false;
  if (aid.TagIdentifier != 2 /*Anchor volume descriptor*/ ||
      aid.TagLocation != sector)
    return false;
...
bool isoDrive ::loadImageUDF() {
  ...
  /* look for the anchor volume descriptor */
  {
    ...
    if (avdex.ExtentLocation == 0 || avdex.ExtentLength == 0)
      return false;
  }

  LOG(LOG_MISC, LOG_DEBUG)(
      "UDF Anchor Volume Descriptor points to location=%lu len=%lu",
      (unsigned long)avdex.ExtentLocation, (unsigned long)avdex.ExtentLength);

  /* within the AVD look for other descriptors */
  for (uint32_t rsec = 0u;
       rsec < size_t(avdex.ExtentLength / COOKED_SECTOR_SIZE); rsec++) {
    uint32_t asec = rsec + avdex.ExtentLocation;
    UDFTagId ctag;

    memset(pvd, 0, 16);
    readSector(pvd, asec);
    if (!ctag.get(COOKED_SECTOR_SIZE, pvd))
      continue;
    if (ctag.TagLocation != asec)
      continue;
    ...
    if (ctag.TagIdentifier == 0x01 /*Primary Volume Descriptor*/) {
      ...
      if (!strcmp((const char *)tpartd.PartitionContents.Identifier,
                  "+NSR02") ||
          !strcmp((const char *)tpartd.PartitionContents.Identifier,
                  "+NSR03")) {
        if (partd.DescriptorTag.TagIdentifier == 0)
          partd = tpartd;
      }
  ...
  // NTS: Someday if DOSBox-X is expected to read UDF 2.50 type discs (like
  // Blu-ray, lol), this code
  //      will need to search the logical volume descriptor for partition maps
  //      that point to a "metadata partition" which then magically defines a
  //      new partition ID and location, within which Blu-ray likes to put the
  //      root directory. Yes, really XD. Then you scan THAT partition for the
  //      root directory and all extents of the root directory referring to
  //      other folders are relative to that metadata partition.

  // DOSBox-X is unlikely to ever read Blu-ray discs, so a simple scan of the
  // main partition is sufficient. NTS: UDF descriptor tags in the partition
  // have sector numbers relative to the partition!
  for (uint32_t rsec = 0; rsec < 256; rsec++) {
    ...
    if (!ctag.get(COOKED_SECTOR_SIZE, pvd))
      continue;
    if (ctag.TagLocation != rsec)
      continue; // "Tag Location" is relative to partition now!

    if (ctag.TagIdentifier == 0x08 /*Terminator Descriptor*/) {
      break;
    } else if (ctag.TagIdentifier == 0x100 /*FileSetDescriptor*/) {
      fsetd.get(ctag, COOKED_SECTOR_SIZE,
                pvd); /* This FSD points at the root directory */
    }
  }

  if (fsetd.DescriptorTag.TagIdentifier == 0) {
    LOG(LOG_MISC,
        LOG_DEBUG)("UDF: Did not find root directory FileSetDescriptor");
    return false;
  }
...
bool isoDrive ::loadImage() {
  ...
  if (loadImageUDF()) {
    ...
    if (enable_udf) {
      ...
      const char *msg = "Found UDF image which requires a reported DOS version "
                        "of 7.10 to mount.\r\n";
  ...
  while (sector < 256) {
    ...
    } else if (pvd[0] == 2) { // supplementary volume (usually Joliet, but not
                              // necessarily)
      if (!strncmp((char *)(&pvd[1]), "CD001", 5) && pvd[6] == 1) {
    ...
    if (pvd[8] == 1 && !strncmp((char *)(&pvd[9]), "CDROM", 5) &&
        pvd[14] == 1) {
      if (choose_iso9660 == 0) {
        choose_iso9660 = sector;
        iso = false;
  ...
  if (choose_joliet && enable_joliet) {
    ...
    LOG(LOG_MISC, LOG_DEBUG)(
        "ISO 9660: Choosing Joliet (unicode) volume at sector %u", sector);
    is_joliet = true;
    iso = true;
  } else if (choose_iso9660) {
    sector = choose_iso9660;
    LOG(LOG_MISC, LOG_DEBUG)("ISO 9660: Choosing ISO 9660 volume at sector %u",
                             sector);
    is_joliet = false;
  } else {
    return false;
  ...
  /* read the first root directory entry, look for Rock Ridge and/or System Use
   * Sharing Protocol extensions. It is rare for Rock Ridge extensions to exist
   * on the Joliet supplementary volume as far as I know. Furthermore the way
   * this code is currently written, Rock Ridge extensions would be ignored
   * anyway for Joliet extensions. */
  if (!is_joliet && enable_rock_ridge) {
    if (DATA_LENGTH(this->rootEntry) >= 33 &&
        EXTENT_LOCATION(this->rootEntry) != 0) {
      ...
      if (pvd[0] >= 33) {
        ...
        if (IS_DIR(rde.fileFlags) &&
            (rde.fileIdentLength == 1 &&
             rde.ident[0] == 0x00) /*The "." directory*/) {
          /* Good. Is there a SUSP "SP" signature following the 1-byte file
           * identifier? */
          /* @Wengier: If your RR-based ISO images put the "SP" signature
           * somewhere else, feel free to adapt this code. The SUSP protocol
           * says it's supposed to be at byte position 1 of the System User
           * Field of the First Directory Record of the root directory. */
          unsigned char *p = &rde.ident[1]; /*first byte following file ident */
          unsigned char *f = &pvd[pvd[0]];

          if ((p + 7 /*SP minimum*/) <= f && p[2] >= 7 /*length*/ &&
              p[3] == 1 /*version*/ && p[4] == 0xBE && p[5] == 0xEF &&
              !memcmp(p, "SP", 2)) {
            /* NTS: The spec counts from 1. We count from zero. Both are
             * provided.
             *
             * BP1/BP2  +0   "SP"
             * BP3      +2   7
             ...
             * BP7      +6   LEN_SKP
             *
             * LEN_SKP = number of bytes to skip in System Use field of each
             * directory record to get to SUSP entries.
             *
             *           @Wengier This is the value you should use when
             * searching for "NM" entries, rather than byte by byte scanning.
             */
            LOG(LOG_MISC,
                LOG_DEBUG)("ISO 9660: Rock Ridge extensions detected");
            is_rock_ridge = true;
            rr_susp_skip = p[6];
          }
        }
  ...
  /* Sanity check: This code does NOT support Rock Ridge extensions when reading
   * the Joliet supplementary volume! */
  if (is_joliet) {
...
bool isoDrive ::lookup(UDFFileIdentifierDescriptor &fid, UDFFileEntry &fe,
                       const char *path) {
  uint8_t pvd[COOKED_SECTOR_SIZE];
  char fname[LFN_NAMELENGTH];
  bool cisdir = false;
  ...
  if (UDFextent_read(rex, pvd, COOKED_SECTOR_SIZE) != COOKED_SECTOR_SIZE)
    return false;
  if (!ctag.get(COOKED_SECTOR_SIZE, pvd))
    return false;
  ...
  // iterate over all path elements (name), and search each of them in the
  // current de
  for (char *name = strtok(isoPath, "/"); NULL != name;
       name = strtok(NULL, "/")) {
    bool found = false;
    ...
    if (cisdir) {
      ...
      int dirIterator = GetDirIterator(fe);
      while (!found && GetNextDirEntry(dirIterator, fid, fe, dirext, fname,
                                       dirIterators[dirIterator].index)) {
        /* skip parent directory */
        if (fid.LengthOfFileIdentifier == 0 || (fid.FileCharacteristics & 0x08))
          continue;

        if (0 == strncasecmp((char *)fname, name, ISO_MAX_FILENAME_LENGTH) ||
            0 == strncasecmp((char *)fullname, name, ISO_MAXPATHNAME)) {
  ...
  for (char *name = strtok(isoPath, "/"); NULL != name;
    ...
    if (IS_DIR(FLAGS2)) {
      ...
      while (!found && GetNextDirEntry(dirIterator, de)) {
        if (!IS_ASSOC(FLAGS2) &&
            ((0 ==
              strncasecmp((char *)de->ident, name, ISO_MAX_FILENAME_LENGTH)) ||
             0 == strncasecmp((char *)fullname, name, ISO_MAXPATHNAME))) {
          found = true;
        }
      }
...
void isoDrive ::EmptyCache(void) {
  ...
  // this function is called at end of DOS_Shell::CMD_COPY and cache is cleared,
  // so only one file is copied.
  if (IS_PC98_ARCH) {
  ...
  is_joliet = false;
  // this->fileName[0]  = '\0'; /* deleted to fix issue #3848. Revert this if
  // there are any flaws */ this->discLabel[0] = '\0'; /* deleted to fix issue
  // #3848. Revert this if there are any flaws */
  nextFreeDirIterator = 0;
  ...
  memset(sectorHashEntries, 0, sizeof(sectorHashEntries));
  memset(&rootEntry, 0, sizeof(isoDirEntry));
  // safe_strncpy(this->fileName, fileName, CROSS_LEN); /* deleted to fix issue
  // #3848. Revert this if there are any flaws */
  loadImage();
}
