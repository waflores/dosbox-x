...
void DOS_InfoBlock::SetLocation(uint16_t segment) {
  ...
  for (uint8_t i = 0; i < sizeof(sDIB); i++)
    mem_writeb(pt + i, 0xff);
  ...
  sSave(sDIB, lastdrive,
        (uint8_t)maxdrive); // increase this if you add drives to cds-chain

  sSave(sDIB, diskInfoBuffer,
  ...
  sSave(sDIB, bufferLocation,
        (uint8_t)0); // buffer in base memory, no workspace
   ...
   * NTS: The INFO block is only given 0x20 paragraphs (0x200 = 512 bytes) which
   * is only enough for a 5-entry SFT table, however that's what MS-DOS does
   * anyway. For any value of FILES you specify, the first SFT is 5 entries and
   * the second table is N - 5 entries. Maybe this is why MS-DOS doesn't let you
   * specify a number below, I think, 6? in FILES= under CONFIG.SYS.
   *
   *      The table must be 5, or else it extends into other DOS kernel
   * structures and memory corruption might happen. */
  if (DOS_FILES < 6)
    E_Exit("[dos] files= setting is too small");

  /* Secondary table */
  unsigned int tbl2_seg = DOS_GetMemory(
      (SftHeaderSize + (SftEntrySize * (DOS_FILES - 5)) + 0xFu) / 16u,
      "SFT secondary table");

  uint16_t sftOffset = 0xCC; /* this offset is fixed according to MS-DOS 5.0,
                                MS-DOS 6.22, Windows 95, etc. */

  /* zero out the table */
  for (unsigned int i = sftOffset; i < 0x200; i++)
    real_writeb(segment, i, 0);

  sSave(sDIB, firstFileTable, RealMake(segment, sftOffset));
  real_writed(segment, sftOffset + 0x00,
              RealMake(tbl2_seg, 0));        // Next File Table
  real_writew(segment, sftOffset + 0x04, 5); // File Table supports 5 files

  if ((sftOffset + SftHeaderSize + (5 * SftEntrySize)) > 0x200)
    E_Exit("Primary SFT is too big");

  /* Windows 3.1 appears to REP SCAS for "CON" in the SFT tables and it will
   * claim that your version of MS-DOS is unsupported if it doesn't see it 3
   * times in the SFT. Why 3? Well, think about it: CON for STDIN, CON for
   * STDOUT, and CON for STDERR. Of course, right?
   *
   * Anyway Windows 3.1 doesn't appear to care exactly where "CON" appears, only
   * that it appears 3 times, so this code writes it out in a manner matching
   * real MS-DOS and according to the Ralph Brown Interrupt List concerning
   * MS-DOS 4.0 to MS-DOS 6.0. Write out the other default handles for good
   * measure. */
  real_writed(segment, sftOffset + SftHeaderSize + (SftEntrySize * 0) + 0x20,
              0x204e4f43); // CON
  real_writed(segment, sftOffset + SftHeaderSize + (SftEntrySize * 1) + 0x20,
              0x204e4f43); // CON
  real_writed(segment, sftOffset + SftHeaderSize + (SftEntrySize * 2) + 0x20,
              0x204e4f43); // CON
  real_writed(segment, sftOffset + SftHeaderSize + (SftEntrySize * 3) + 0x20,
              0x20585541); // AUX
  real_writed(segment, sftOffset + SftHeaderSize + (SftEntrySize * 4) + 0x20,
              0x204e5250); // PRN

  /* zero out the table */
  for (unsigned int i = 0;
       i < (SftHeaderSize + (SftEntrySize * (DOS_FILES - 5))); i++)
    real_writeb(tbl2_seg, i, 0);
...
void DOS_PSP::MakeNew(uint16_t mem_size) {
  ...
  /* Clear it first */
  uint16_t i;
  for (i = 0; i < sizeof(sPSP); i++)
    mem_writeb(pt + i, 0);
  // Set size
  sSave(sPSP, next_seg, (unsigned int)seg + mem_size);
  /* cpm_entry is an alias for the int 30 vector (0:c0 = c:0); the offset is
   * also the maximum program size */
  if (cpm_compat_mode ==
      CPM_COMPAT_MSDOS2) { /* MS-DOS 2.x behavior, where offset is the memory
                              size (for some reason) */
                           /* far call opcode */
    sSave(sPSP, far_call, 0x9a);
    /* and where to call to */
    if (mem_size > CPM_MAX_SEG_SIZE)
      mem_size = CPM_MAX_SEG_SIZE;
    mem_size -= 0x10;
    sSave(sPSP, cpm_entry, RealMake(0xc - mem_size, mem_size << 4));
  } else if (cpm_compat_mode ==
             CPM_COMPAT_MSDOS5) { /* MS-DOS 5.x behavior, for whatever reason */
                                  /* far call opcode */
    sSave(sPSP, far_call, 0x9a);
    /* and where to call to.
    ...
    /* direct and to the point, though impossible to hook INT 21h without
     * patching all PSP segments which is probably why Microsoft never did this
     * in the DOS kernel. Choosing this method removes the need for the copy of
     * INT 30h in the HMA area, and therefore opens up all 64KB of HMA if you
     * want. */
    ...
    /* stick an INT 20h in there to make calling the CP/M entry point an
     * immediate exit */
    /* this is NOT default, but an option for users who are absolutely certain
     * no CP/M code is going to run in their DOS box. */
...
void DOS_PSP::CopyFileTable(DOS_PSP *srcpsp, bool createchildpsp) {
  ...
  for (uint16_t i = 0; i < 20; i++) {
    ...
    if (createchildpsp) { // copy obeying not inherit flag.(but don't duplicate
                          // them)
      bool allowCopy = true; //(handle==0) || ((handle>0) &&
                             //(FindEntryByHandle(handle)==0xff));
      if ((handle < DOS_FILES) && Files[handle] &&
          !(Files[handle]->flags & DOS_NOT_INHERIT) && allowCopy) {
        Files[handle]->AddRef();
...
void DOS_PSP::StoreCommandTail() {
  ...
  MEM_StrCopy(pt + offsetof(sPSP, cmdtail.buffer), storect,
              len > CTBUF ? CTBUF : len);
}

void DOS_PSP::RestoreCommandTail() {
  mem_writeb(pt + offsetof(sPSP, cmdtail.count),
             strlen(storect) > 0 ? (uint8_t)(strlen(storect) - 1) : 0);
  MEM_BlockWrite(pt + offsetof(sPSP, cmdtail.buffer), storect, strlen(storect));
}

void DOS_PSP::SetFCB1(RealPt src) {
  if (src)
    MEM_BlockCopy(PhysMake(seg, offsetof(sPSP, fcb1)), Real2Phys(src), 16);
}

void DOS_PSP::SetFCB2(RealPt src) {
  if (src)
    MEM_BlockCopy(PhysMake(seg, offsetof(sPSP, fcb2)), Real2Phys(src), 16);
}
...
bool DOS_PSP::SetNumFiles(uint16_t fileNum) {
  ...
  if (fileNum > 20 && ((uint32_t)(fileNum + 2) > sGet(sPSP, max_files))) {
    ...
    for (uint16_t i = 0; i < fileNum; i++)
      mem_writeb(Real2Phys(data) + i, (i < 20) ? GetFileHandle(i) : 0xFF);
...
void DOS_DTA::SetupSearch(uint8_t _sdrive, uint8_t _sattr, char *pattern) {
  ...
  if (lfn_filefind_handle < LFN_FILEFIND_NONE || forcelfn) {
    ...
    while (i <= LFN_NAMELENGTH)
      sname[lfn_filefind_handle][i++] = 0;
  }
  for (i = 0; i < 11; i++)
    mem_writeb(pt + offsetof(sDTA, spname) + i, 0);
  ...
  if (find_ext) {
    ...
    find_ext++;
    MEM_BlockWrite(pt + offsetof(sDTA, spext), find_ext,
                   (strlen(find_ext) > 3) ? 3 : (Bitu)strlen(find_ext));
  } else {
    MEM_BlockWrite(pt + offsetof(sDTA, spname), pattern,
                   (strlen(pattern) > 8) ? 8 : (Bitu)strlen(pattern));
  }
...
void DOS_DTA::SetResult(const char *_name, const char *_lname, uint32_t _size,
                        uint32_t _hsize, uint16_t _date, uint16_t _time,
                        uint8_t _attr) {
  fd.size = _size;
  fd.hsize = _hsize;
  fd.mdate = _date;
...
void DOS_DTA::GetResult(char *_name, char *_lname, uint32_t &_size,
                        uint32_t &_hsize, uint16_t &_date, uint16_t &_time,
                        uint8_t &_attr) {
  strcpy(_lname, fd.lname);
  if (fd.sname[0] != 0)
    strcpy(_name, fd.sname);
  else if (strlen(fd.lname) < DOS_NAMELENGTH_ASCII)
    strcpy(_name, fd.lname);
  _size = fd.size;
  _hsize = fd.hsize;
  _date = fd.mdate;
  _time = fd.mtime;
...
uint8_t DOS_DTA::GetSearchDrive(void) {
  return lfn_filefind_handle >= LFN_FILEFIND_MAX ? (uint8_t)sGet(sDTA, sdrive)
                                                 : sdriv[lfn_filefind_handle];
}
...
bool DOS_FCB::Extended(void) { return extended; }
...
void DOS_FCB::GetSizeDateTime(uint32_t &_size, uint16_t &_date,
                              uint16_t &_time) {
...
void DOS_FCB::GetRandom(uint32_t &_random) { _random = sGet(sFCB, rndm); }

void DOS_FCB::SetRandom(uint32_t _random) { sSave(sFCB, rndm, _random); }
...
void DOS_SDA::Init() {
  /* Clear */
  for (uint8_t i = 0; i < sizeof(sSDA); i++)
