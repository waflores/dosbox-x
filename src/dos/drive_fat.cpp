...
extern bool gbk, isDBCSCP(), isKanji1_gbk(uint8_t chr),
    shiftjis_lead_byte(int c);
extern bool CodePageGuestToHostUTF16(uint16_t *d /*CROSS_LEN*/,
                                     const char *s /*CROSS_LEN*/);
extern bool CodePageHostToGuestUTF16(char *d /*CROSS_LEN*/,
...
int PC98AutoChoose_FAT(const std::vector<_PC98RawPartition> &parts,
  ...
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    if (loadedDisk->partitionInUse(i))
      continue;
    uint8_t syss = parts[i].sid;

    /* We're looking for MS-DOS partitions.
     * I've heard that some other OSes were once ported to PC-98, including
     * Windows NT and OS/2, so I would rather not mistake NTFS or HPFS as FAT
     * and cause damage. --J.C.
    ...
    if (!strncasecmp(pe.name, "MS-DOS", 6) ||
        ...
        ((parts[i].mid == 0x20 ||
          (parts[i].mid >= 0xa1 &&
           parts[i].mid <= 0xaf)) // bootable or non-bootable MS-DOS partition
         && (syss == 0x81 || syss == 0x91 || syss == 0xa1 ||
             syss == 0xe1) // is an active MS-DOS partition
         && (parts[i].end_cyl <=
             loadedDisk->cylinders))) { // end_cyl is a valid value
      return (int)i;
    }
  }
...
int MBRAutoChoose_FAT(const std::vector<partTable::partentry_t> &parts,
                      imageDisk *loadedDisk, uint8_t use_ver_maj = 0,
                      uint8_t use_ver_min = 0) {
  uint16_t n = 1;
  const char *msg;
  ...
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    if (pe.parttype == 0x01 /*FAT12*/ || pe.parttype == 0x04 /*FAT16*/ ||
      ...
      if (use_ver_maj < 7 && prompt1) {
        ...
        std::string drive_warn =
            formatString(MSG_Get("IMAGEMOUNT_CHANGE_DOSVER"), dos_ver.c_str(),
                         dos_ver.c_str());
        if (fat32setver == 1 ||
            (fat32setver == -1 &&
             systemmessagebox("Mounting LBA disk image", drive_warn.c_str(),
                              "yesno", "question", 1))) {
          use_ver_maj = dos.version.major = 7;
          use_ver_min = dos.version.minor = 0;
          dos_ver_menu(false);
        } else {
          msg = "LBA disk images are supported but require a higher reported "
                "DOS version.\r\n";
          n = (uint16_t)strlen(msg);
          DOS_WriteFile(STDOUT, (uint8_t *)msg, &n);
          msg = "Please set the reported DOS version to at least 7.0 to mount "
                "this disk image.\r\n";
          n = (uint16_t)strlen(msg);
          DOS_WriteFile(STDOUT, (uint8_t *)msg, &n);
        }
      ...
      if ((use_ver_maj < 7 || (use_ver_maj == 7 && use_ver_min < 10)) &&
          prompt2) {
        ...
        if (fat32setver == 1 ||
            (fat32setver == -1 &&
             systemmessagebox("Mounting FAT32 disk image", drive_warn.c_str(),
                              "yesno", "question", 1))) {
          use_ver_maj = dos.version.major = 7;
          use_ver_min = dos.version.minor = 10;
          dos_ver_menu(true);
        } else {
          msg = "FAT32 disk images are supported but require a higher reported "
                "DOS version.\r\n";
          n = (uint16_t)strlen(msg);
          DOS_WriteFile(STDOUT, (uint8_t *)msg, &n);
          msg = "Please set the reported DOS version to at least 7.10 to mount "
                "this disk image.\r\n";
          n = (uint16_t)strlen(msg);
          DOS_WriteFile(STDOUT, (uint8_t *)msg, &n);
        }
      ...
      if (use_ver_maj > 7 ||
          (use_ver_maj == 7 && use_ver_min >= 10)) /* MS-DOS 7.10 or higher */
        return (int)i;
      else
        prompt2 = false;
    }
...
bool filename_not_8x3(const char *n) {
  ...
  while (*n != 0) {
    if (*n == '.')
      break;
    if ((*n & 0xFF) <= 32 || *n == 127 || *n == '"' || *n == '+' || *n == '=' ||
        *n == ',' || *n == ';' || *n == ':' || *n == '<' || *n == '>' ||
        ((*n == '[' || *n == ']' || *n == '|' || *n == '\\') &&
         (!lead || ((dos.loaded_codepage == 936 || IS_PDOSV) && !gbk))) ||
        *n == '?' || *n == '*')
      return true;
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(*n & 0xFF)) ||
             (isDBCSCP() && isKanji1_gbk(*n & 0xFF)))
      lead = true;
    i++;
    n++;
  }
  if (!i || i > 8)
    return true;
  if (*n == 0)
    return false; /* made it past 8 or less normal chars and end of string:
                     normal */
...
bool filename_not_strict_8x3(const char *n) {
  ...
  for (unsigned int i = 0; i < strlen(n); i++) {
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(n[i] & 0xFF)) ||
             (isDBCSCP() && isKanji1_gbk(n[i] & 0xFF)))
      lead = true;
    else if (n[i] >= 'a' && n[i] <= 'z')
      return true;
  }
...
char *fatDrive::Generate_SFN(const char *path, const char *name) {
  ...
  while (k < 1000000) {
    ...
    strcat(fullname, sfn);
    if (!getFileDirEntry(fullname, &fileEntry, &dirClust, &subEntry,
                         /*dirOk*/ true))
      return sfn;
    k++;
  }
...
public:
  fatFile(const char *name, uint32_t startCluster, uint32_t fileLen,
          fatDrive *useDrive);
...
void time_t_to_DOS_DateTime(uint16_t &t, uint16_t &d, time_t unix_time) {
  ...
  t = ((unsigned int)tm->tm_hour << 11u) + ((unsigned int)tm->tm_min << 5u) +
      ((unsigned int)tm->tm_sec >> 1u);
  d = (((unsigned int)tm->tm_year - 80u) << 9u) +
      (((unsigned int)tm->tm_mon + 1u) << 5u) + (unsigned int)tm->tm_mday;
}

/* IN - char * filename: Name in regular filename format, e.g. bob.txt */
/* OUT - char * filearray: Name in DOS directory format, eleven char, e.g. bob
 * txt */
...
fatFile::fatFile(const char * /*name*/, uint32_t startCluster, uint32_t fileLen,
                 fatDrive *useDrive)
    : firstCluster(startCluster), filelength(fileLen), myDrive(useDrive) {
  uint32_t seekto = 0;
  open = true;
...
bool fatFile::Read(uint8_t *data, uint16_t *size) {
  if ((this->flags & 0xf) ==
      OPEN_WRITE) { // check if file opened in write-only mode
  ...
  while (sizedec != 0) {
    ...
    if (curSectOff >= myDrive->getSectorSize()) {
      currentSector =
          myDrive->getAbsoluteSectFromBytePos(firstCluster, seekpos);
      if (currentSector == 0) {
        /* EOC reached before EOF */
        // LOG_MSG("EOC reached before EOF, seekpos %d, filelen %d", seekpos,
        // filelength);
        *size = sizecount;
        loadedSector = false;
        return true;
      ...
      myDrive->readSector(currentSector, sectorBuffer);
      loadedSector = true;
      // LOG_MSG("Reading absolute sector at %d for seekpos %d", currentSector,
      // seekpos);
    }
    --sizedec;
  }
...
bool fatFile::Write(const uint8_t *data, uint16_t *size) {
  if ((this->flags & 0xf) ==
      OPEN_READ) { // check if file opened in read-only mode
    DOS_SetError(DOSERR_ACCESS_DENIED);
    return false;
  }
  ...
  if (seekpos < filelength && *size == 0) {
    ...
    if (filelength == 0)
      firstCluster =
          0; /* A file of length zero has a starting cluster of zero as well */
    modified = true;
    goto finalizeWrite;
  }
  ...
  if (seekpos > filelength) {
    ...
    if (filelength == 0) {
      ...
      if (firstCluster == 0)
        goto finalizeWrite; // out of space
    ...
    while (filelength < seekpos) {
      if (myDrive->appendCluster(firstCluster) == 0)
        goto finalizeWrite; // out of space
      filelength += clustSize;
    }
    ...
    if (seekpos >= filelength) {
      if (filelength == 0) {
        ...
        currentSector =
            myDrive->getAbsoluteSectFromBytePos(firstCluster, seekpos);
        if (currentSector == 0) {
          /* I guess allocateCluster() didn't work after all. This check is
           * necessary to prevent this condition from treating the BOOT SECTOR
           * as a file. */
          LOG(LOG_DOSMISC, LOG_WARN)(
              "FAT file write: unable to allocate first cluster, erroring out");
          goto finalizeWrite;
        }
        myDrive->readSector(currentSector, sectorBuffer);
        loadedSector = true;
...
bool fatFile::Seek(uint32_t *pos, uint32_t type) {
  ...
  //	LOG_MSG("Seek to %d with type %d (absolute value %d)", *pos, type,
  //seekto);
...
uint16_t fatFile::GetInformation(void) { return 0; }

bool fatFile::UpdateDateTimeFromHost(void) { return true; }

uint32_t fatFile::GetSeekPos() { return seekpos; }
...
uint32_t fatDrive::getClusterValue(uint32_t clustNum) {
  ...
  fatsectnum =
      BPB.v.BPB_RsvdSecCnt + (fatoffset / BPB.v.BPB_BytsPerSec) + partSectOff;
  fatentoff = fatoffset % BPB.v.BPB_BytsPerSec;

  if (BPB.is_fat32()) {
    if (fatsectnum >=
        (BPB.v.BPB_RsvdSecCnt + BPB.v32.BPB_FATSz32 + partSectOff)) {
      LOG(LOG_DOSMISC,
          LOG_ERROR)("Attempt to read cluster entry from FAT that out of range "
                     "(outside the FAT table) cluster %u",
                     (unsigned int)clustNum);
      return 0;
    }
  } else {
    if (fatsectnum >=
        (BPB.v.BPB_RsvdSecCnt + BPB.v.BPB_FATSz16 + partSectOff)) {
      LOG(LOG_DOSMISC,
          LOG_ERROR)("Attempt to read cluster entry from FAT that out of range "
                     "(outside the FAT table) cluster %u",
                     (unsigned int)clustNum);
  ...
  case FAT32:
    clustValue = var_read((uint32_t *)&fatSectBuffer[fatentoff]) &
                 0x0FFFFFFFul; /* Well, actually it's FAT28. Upper 4 bits are
                                  "reserved". */
    break;
  }
...
void fatDrive::setClusterValue(uint32_t clustNum, uint32_t clustValue) {
  ...
  if (BPB.is_fat32()) {
    if (fatsectnum >=
        (BPB.v.BPB_RsvdSecCnt + BPB.v32.BPB_FATSz32 + partSectOff)) {
      LOG(LOG_DOSMISC,
          LOG_ERROR)("Attempt to write cluster entry from FAT that out of "
                     "range (outside the FAT table) cluster %u",
                     (unsigned int)clustNum);
      return;
    }
  } else {
  ...
  for (unsigned int fc = 0; fc < BPB.v.BPB_NumFATs; fc++) {
    writeSector(fatsectnum + (fc * (BPB.is_fat32() ? BPB.v32.BPB_FATSz32
                                                   : BPB.v.BPB_FATSz16)),
                &fatSectBuffer[0]);
    if (fattype == FAT12) {
      if (fatentoff >= (BPB.v.BPB_BytsPerSec - 1U))
        writeSector(fatsectnum + 1u +
                        (fc * (BPB.is_fat32() ? BPB.v32.BPB_FATSz32
                                              : BPB.v.BPB_FATSz16)),
...
bool fatDrive::getEntryName(const char *fullname, char *entname) {
  ...
  for (int i = 0; i < (int)strlen(findFile); i++) {
    if (findFile[i] != ' ' && findFile[i] != '"' && findFile[i] != '+' &&
        findFile[i] != '=' && findFile[i] != ',' && findFile[i] != ';' &&
        findFile[i] != ':' && findFile[i] != '<' && findFile[i] != '>' &&
        !((findFile[i] == '[' || findFile[i] == ']' || findFile[i] == '|' ||
           findFile[i] == '\\') &&
          (!lead || ((dos.loaded_codepage == 936 || IS_PDOSV) && !gbk))) &&
        findFile[i] != '?' && findFile[i] != '*')
      findFile[j++] = findFile[i];
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(findFile[i] & 0xFF)) ||
             (isDBCSCP() && isKanji1_gbk(findFile[i] & 0xFF)))
      lead = true;
  }
  findFile[j] = 0;
...
void fatDrive::UpdateBootVolumeLabel(const char *label) {
  ...
  if (BPB.v.BPB_BootSig == 0x28 || BPB.v.BPB_BootSig == 0x29) {
    ...
    if (upcasebuf[0] == (char)0xe5)
      upcasebuf[0] = 0x05;
    while (i < 11 && *upcaseptr != 0)
      bootbuffer.bpb.v.BPB_VolLab[i++] = *upcaseptr++;
    while (i < 11)
      bootbuffer.bpb.v.BPB_VolLab[i++] = ' ';

    loadedDisk->Write_AbsoluteSector(0 + partSectOff, &bootbuffer);
  }
}

void fatDrive::SetLabel(const char *label, bool /*iscdrom*/,
                        bool /*updatable*/) {
  if (unformatted)
    return;

  direntry sectbuf[MAX_DIRENTS_PER_SECTOR]; /* 16 directory entries per 512 byte
                                               sector */
...
nextfile:
  ...
  if (dirClustNumber == 0) {
    ...
    /* A zero sector number can't happen - we need to allocate more room for
     * this directory*/
    if (tmpsector == 0) {
      if (*label == 0)
        return; // removing volume label, so stop now
      uint32_t newClust;
      ...
      tmpsector = getAbsoluteSectFromChain(dirClustNumber, logentsector);
      if (tmpsector == 0)
        return; /* Give up if still can't get more room for directory */
    }
  ...
  if (*label != 0) {
    ...
    if (sectbuf[entryoffset].entryname[0] == 0x00 ||
      ...
      sectbuf[entryoffset].attrib = DOS_ATTR_VOLUME;
      {
        ...
        s = upcasebuf;
        while (j < 11 && *s != 0)
          sectbuf[entryoffset].entryname[j++] = *s++;
        while (j < 11)
          sectbuf[entryoffset].entryname[j++] = ' ';
      }
      uint16_t ct, cd;
      time_t_to_DOS_DateTime(/*&*/ ct, /*&*/ cd, ::time(NULL));
    ...
    if (sectbuf[entryoffset].attrib == DOS_ATTR_VOLUME) {
      /* TODO: There needs to be a way for FCB delete to erase the volume label
       * by name instead of just picking the first one */
      /* found one */
      sectbuf[entryoffset].entryname[0] = 0xe5;
      writeSector(tmpsector, sectbuf);
...
/* NTS: This function normally will only return files. Every element of the path
 * that is a directory is entered into. If every element is a directory, then
 * this code will fail to locate anything.
 *
 *      If dirOk is set, and all path elements are directories, it will stop at
 * the last one and look it up as if a file. The purpose is to clean up this FAT
 * driver by eliminating all the ridiculous "look up getFileDirEntry but if it
 * fails do a whole different code path that looks it up as if directory"
 * copy-pasta in this code that complicates some functions like the Rename()
 * method.
 *
 *      useEntry is filled with the SFN direntry of the first search result.
 * dirClust is filled in with the starting cluster of the parent directory. Note
 * that even if dirOk is set and the result is a directory, dirClust is the
 * parent directory of that directory. subEntry is the dirent index into the
 * directory.
 *
 *      As a side effect of using FindNextInternal, variable lfnRange will be
 * either cleared or filled in with the subEntry range of dirents that contain
 * the LFN entries (needed for deletion, renaming, rmdir, etc). Not all paths
 * set or clear it, so first call the clear() method before calling. After the
 * call, copy off the value because the next call to FindNextInternal by any
 * part of this code will obliterate the result with a new result. */
bool fatDrive::getFileDirEntry(char const *const filename, direntry *useEntry,
  ...
  if ((len > 0) && (filename[len - 1] != '\\')) {
    ...
    while (findDir != NULL) {
      ...
      if (!FindNextInternal(currentClust, *imgDTA, &foundEntry))
        ...
        // Found something. See if it's a directory (findfirst always finds
        // regular files)
        char find_name[DOS_NAMELENGTH_ASCII], lfind_name[LFN_NAMELENGTH];
        uint16_t find_date, find_time;
        uint32_t find_size, find_hsize;
        uint8_t find_attr;
        imgDTA->GetResult(find_name, lfind_name, find_size, find_hsize,
                          find_date, find_time, find_attr);
        ...
        if (findNext == NULL && dirOk)
          break; /* dirOk means that if the last element is a directory, then
                    refer to the directory itself */
        findDir = findNext;
      }
...
bool fatDrive::getDirClustNum(const char *dir, uint32_t *clustNum,
                              bool parDir) {
  if (unformatted)
    return false;
...
uint8_t fatDrive::readSector(uint32_t sectnum, void *data) {
  ...
  uint32_t cylindersize =
      (unsigned int)BPB.v.BPB_NumHeads * (unsigned int)BPB.v.BPB_SecPerTrk;
...
#else
  uint32_t cylindersize =
      (unsigned int)loadedDisk->heads * (unsigned int)loadedDisk->sectors;
  uint32_t cylinder = sectnum / cylindersize;
  sectnum %= cylindersize;
...
uint32_t fatDrive::getSectorSize(void) { return BPB.v.BPB_BytsPerSec; }

uint32_t fatDrive::getClusterSize(void) {
  if (unformatted)
    return 0;
  return (unsigned int)BPB.v.BPB_SecPerClus *
         (unsigned int)BPB.v.BPB_BytsPerSec;
}

uint32_t fatDrive::getAbsoluteSectFromBytePos(uint32_t startClustNum,
                                              uint32_t bytePos,
...
bool fatDrive::iseofFAT(const uint32_t cv) const {
  ...
  switch (fattype) {
  case FAT12:
    return cv < 2 || cv >= 0xff8;
  case FAT16:
    return cv < 2 || cv >= 0xfff8;
  case FAT32:
    return cv < 2 || cv >= 0x0ffffff8;
  default:
    break;
  }
...
uint32_t fatDrive::getAbsoluteSectFromChain(uint32_t startClustNum,
  ...
  /* startClustNum == 0 means the file is (likely) zero length and has no
   * allocation chain yet. Nothing to map. Without this check, this code would
   * permit the FAT file reader/writer to treat the ROOT DIRECTORY as a file
  ...
  if (ccm != NULL && ccm->current_cluster_no >= 2) {
    /* If the cluster index is the same as last time or farther down, avoid
     * re-reading the entire allocation chain again and start from where we last
     * read from. If the cluster index is going back from current, then re-read
     * the entire allocation chain again. This is the nature of a singly-linked
     * file allocation table and is the reason seek() is faster going forward
     * than backwards in MS-DOS, especially on FAT32 partitions. */
  ...
  while (indxClust < targClust) {
    ...
    if (iseofFAT(testvalue)) {
      if (indxClust != targClust)
        LOG(LOG_MISC, LOG_DEBUG)("FAT: Seek past allocation chain");
      return 0;
    }
...
void fatDrive::deleteClustChain(uint32_t startCluster, uint32_t bytePos) {
  if (unformatted)
    return;
  if (startCluster < 2)
    return; /* do not corrupt the FAT media ID. The file has no chain. Do
               nothing. */
  ...
  while (countClust < endClust) {
    ...
    if (testvalue == 0) {
      LOG(LOG_DOSMISC,
          LOG_WARN)("deleteClusterChain startCluster=%u countClust=%u "
                    "endClust=%u currentClust=%u testvalue=%u eof=%u "
                    "unexpected zero cluster value in FAT table",
                    (unsigned int)startCluster, (unsigned int)countClust,
                    (unsigned int)endClust, (unsigned int)currentClust,
                    (unsigned int)testvalue, (unsigned int)eofClust);
      return;
   ...
   * This condition will NOT occur if bytePos == 0 (i.e. complete file
   * truncation) because countClust == 1 and endClust == 0 */
  if (countClust == endClust) {
    ...
    if (searchFreeCluster > (currentClust - 2))
      searchFreeCluster = currentClust - 2;
    currentClust = testvalue;
    countClust++;
  }
  ...
  while (1) {
    ...
    /* this follows setClusterValue() to make sure the end of the chain is
     * zeroed too */
...
uint32_t fatDrive::appendCluster(uint32_t startCluster) {
  if (unformatted)
    return 0;
  if (startCluster < 2)
    return 0; /* do not corrupt the FAT media ID. The file has no chain. Do
                 nothing. */
  ...
  while (1) {
    ...
    if (testvalue == 0) {
      LOG(LOG_DOSMISC,
          LOG_WARN)("appendCluster currentClust=%u testvalue=%u eof=%u "
                    "unexpected zero cluster value in FAT table",
                    (unsigned int)currentClust, (unsigned int)testvalue,
                    (unsigned int)eofClust);
      return 0;
    } else if (testvalue >= eofClust) {
      break; /* found it! */
    }
...
bool fatDrive::allocateCluster(uint32_t useCluster, uint32_t prevCluster) {
  ...
  if (prevCluster != 0) {
    /* Refuse to allocate cluster if previous cluster value is zero
     * (unallocated) */
    if (!getClusterValue(prevCluster))
      return false;
...
fatDrive::~fatDrive() {
  if (loadedDisk) {
    if (partition_index >= 0)
      loadedDisk->partitionMarkUse(partition_index, false);
    loadedDisk->Release();
    loadedDisk = NULL;
  }
}
...
fatDrive::fatDrive(const char *sysFilename, uint32_t bytesector,
  ...
  std::vector<std::string>::iterator it =
      std::find(options.begin(), options.end(), "readonly");
  ...
  // there are plenty of cases where this code aborts, exits, or re-execs itself
  // (such as reboot) where stdio buffering can cause loss of data.
  ...
  if (qcow2_header.magic == QCow2Image::magic &&
    ...
    loadedDisk = new QCow2Disk(qcow2_header, diskfile, fname, filesize,
                               bytesector, (filesize > 2880));
  } else {
    fseeko64(diskfile, 0L, SEEK_SET);
    assert(sizeof(bootcode) >= 256);
    size_t readResult =
        fread(bootcode, 256, 1, diskfile); // look for magic signatures
    if (readResult != 1) {
    ...
    bool is_hdd = false;
    if ((ext != NULL) && (!strcasecmp(ext, ".hdi") || !strcasecmp(ext, ".nhd")))
      is_hdd = true;
    ...
    if (ext != NULL && !strcasecmp(ext, ".d88")) {
      ...
      fseeko64(diskfile, 0L, SEEK_END);
      filesize = (uint32_t)(ftello64(diskfile) / 1024L);
      loadedDisk = new imageDisk(diskfile, fname, filesize,
                                 (is_hdd | (filesize > 2880)));
    }
  }

  fatDriveInit(sysFilename, bytesector, cylsector, headscyl, cylinders,
               filesize, options);
}
...
fatDrive::fatDrive(imageDisk *sourceLoadedDisk,
  ...
  if (it != options.end())
    readonly = true;
  opts = {0, 0, 0, 0, -1};
  imageDiskElToritoFloppy *idelt =
      dynamic_cast<imageDiskElToritoFloppy *>(sourceLoadedDisk);
  ...
  fatDriveInit("", loadedDisk->sector_size, loadedDisk->sectors,
               loadedDisk->heads, loadedDisk->cylinders, loadedDisk->diskSizeK,
               options);
}

uint8_t fatDrive::Read_AbsoluteSector(uint32_t sectnum, void *data) {
  if (loadedDisk != NULL) {
    /* this will only work if the logical sector size is larger than the disk
     * sector size */
...
uint32_t fatDrive::getSectSize(void) { return sector_size; }
...
void fatDrive::UpdateDPB(unsigned char dos_drive) {
  ...
  if (ptr != PhysPt(0)) {
    ...
    mem_writeb(ptr + 0x04, BPB.v.BPB_SecPerClus -
                               1); // +4 = highest sector within a cluster
    mem_writeb(
        ptr + 0x05,
        bitop::log2(BPB.v.BPB_SecPerClus)); // +5 = shift count to convert
                                            // clusters to sectors
    mem_writew(ptr + 0x06,
               BPB.v.BPB_RsvdSecCnt); // +6 = number of reserved sectors at
                                      // start of partition
    mem_writeb(
        ptr + 0x08,
        BPB.v.BPB_NumFATs); // +8 = number of FATs (file allocation tables)
    mem_writew(ptr + 0x09,
               BPB.v.BPB_RootEntCnt); // +9 = number of root directory entries
    mem_writew(ptr + 0x0B,
               (uint16_t)(firstDataSector -
                          partSectOff)); // +11 = number of first sector
    ...
    if (BPB.is_fat32())
      ...
      mem_writew(ptr + 0x0D,
                 (uint16_t)CountOfClusters + 1); // +13 = highest cluster number

    if (dos.version.major >= 4) {
      mem_writew(ptr + 0x0F,
                 (uint16_t)BPB.v.BPB_FATSz16); // +15 = sectors per FAT

      if (BPB.is_fat32())
        mem_writew(ptr + 0x11, 0xFFFF); // Windows 98 behavior
      else
        mem_writew(ptr + 0x11,
                   (uint16_t)(firstRootDirSect -
                              partSectOff)); // +17 = sector number of first
                                             // directory sector

      mem_writed(ptr + 0x13,
                 0xFFFFFFFF); // +19 = address of device driver header (NOT
                              // IMPLEMENTED) Windows 98 behavior
      mem_writeb(ptr + 0x17, GetMediaByte()); // +23 = media ID byte
      mem_writeb(ptr + 0x18, 0x00);           // +24 = disk accessed
      mem_writew(ptr + 0x1F,
                 0xFFFF); // +31 = number of free clusters or 0xFFFF if unknown
                          // other fields, not implemented
    } else {
      mem_writeb(ptr + 0x0F,
                 (uint8_t)BPB.v.BPB_FATSz16); // +15 = sectors per FAT
      ...
      if (BPB.is_fat32())
        ...
        mem_writew(ptr + 0x10,
                   (uint16_t)(firstRootDirSect -
                              partSectOff)); // +16 = sector number of first
                                             // directory sector

      mem_writed(ptr + 0x12,
                 0xFFFFFFFF); // +18 = address of device driver header (NOT
                              // IMPLEMENTED) Windows 98 behavior
      mem_writeb(ptr + 0x16, GetMediaByte()); // +22 = media ID byte
      mem_writeb(ptr + 0x17, 0x00);           // +23 = disk accessed
      mem_writew(ptr + 0x1E,
                 0xFFFF); // +30 = number of free clusters or 0xFFFF if unknown
                          // other fields, not implemented
    }
  }
}

void fatDrive::fatDriveInit(const char *sysFilename, uint32_t bytesector,
                            uint32_t cylsector, uint32_t headscyl,
                            uint32_t cylinders, uint64_t filesize,
  ...
  for (const auto &opt : options) {
    ...
    if (name == "partidx") {
      ...
      LOG(LOG_DOSMISC, LOG_DEBUG)("FAT: option '%s' = '%s' ignored, unknown",
...

  {
    ...
    if (loadedDisk->getSectSize() > sizeof(bootbuffer)) {
      LOG_MSG("Disk sector/bytes (%u) is too large, not attempting FAT "
              "filesystem access",
    ...
    if (opt_startsector >= Bits(0)) {
      ...
      if (loadedDisk->heads == 0 || loadedDisk->sectors == 0 ||
          loadedDisk->cylinders == 0) {
        created_successfully = false;
        LOG_MSG("No geometry");
        return;
      }
      ...
      if (ptype == "IPL1") {
        ...
        { /* assume chosen_idx is in range */
          ...
          /* unfortunately start and end are in C/H/S geometry, so we have to
           * translate. this is why it matters so much to read the geometry from
           * the HDI header.
           *
           * NOTE: C/H/S values in the IPL1 table are similar to IBM PC except
           * that sectors are counted from 0, not 1 */
          startSector = (pe.cyl * loadedDisk->sectors * loadedDisk->heads) +
                        (pe.head * loadedDisk->sectors) + pe.sector;
...

          {
            ...
            LOG_MSG("Using IPL1 entry %lu name '%s' which starts at sector %lu",
                    (unsigned long)chosen_idx, name.c_str(),
        ...
        if (opt_partition_index >= 0) {
          ...
          if (chosen_idx < 0) {
            /* If no chosen partition by default, but chosen partition if acting
             * like later DOS version, then you need to bump the DOS version
             * number to mount it. NTS: Exit this part with chosen_idx < 0 */
            if (MBRAutoChoose_FAT(parts, loadedDisk, 7, 0) >= 0 ||
                MBRAutoChoose_FAT(parts, loadedDisk, 7, 10) >= 0) {
              LOG_MSG("Partitions are available to mount, but a higher DOS "
                      "version is required");
            }
          }
        }
       ...
       * two drives using the same partition can cause FAT filesystem
       * corruption! */
      ...
      if (headscyl > 0 && cylinders > 0 && cylsector > 0 &&
          bytesector > 0) // manually set geometry if value specified
        loadedDisk->Set_Geometry(headscyl, cylinders, cylsector, bytesector);
      else
        /* Get floppy disk parameters based on image size */
        loadedDisk->Get_Geometry(&headscyl, &cylinders, &cylsector,
                                 &bytesector);
      ...
      if (loadedDisk->heads == 0 || loadedDisk->sectors == 0 ||
          loadedDisk->cylinders == 0) {
        /* Get_Geometry fails for some floppies with weird geometries, so try
         * obtaining the geometry from BPB in such case */
        LOG_MSG("drive_fat.cpp: No geometry, check your image. Try obtaining "
                "from BPB");
      }
    }
    ...
    /* If the sector is full of 0xF6, the partition is brand new and was just
     * created with Microsoft FDISK.EXE (Windows 98 behavior) and therefore
     * there is NO FAT filesystem here. We'll go farther and check if all bytes
     * are just the same.
     *
     * MS-DOS behavior is to let the drive letter appear, but any attempt to
     * access it results in errors like "invalid media type" until you run
     * FORMAT.COM on it, which will then set up the filesystem and allow it to
     * work. */
    if (partSectOff != 0) { /* hard drives only */
      unsigned int i = 1;

      while (i < 128 &&
             ((uint8_t *)(&bootbuffer))[0] == ((uint8_t *)(&bootbuffer))[i])
        i++;

      if (i == 128) {
        LOG_MSG("Boot sector appears to have been created by FDISK.EXE but not "
                "formatted. Allowing mount but filesystem access will not work "
                "until formatted.");
        ...
        strcat(info, wpcolon && strlen(sysFilename) > 1 && sysFilename[0] == ':'
                         ? sysFilename + 1
                         : sysFilename);

        /* NTS: Real MS-DOS behavior is that an unformatted partition is
         * inaccessible but INT 21h AX=440Dh CX=0x0860 (Get Device Parameters)
         *      has a ready-made BPB for use in formatting the partition.
         * FORMAT.COM then formats the filesystem based on the MS-DOS kernel's
         *      recommended BPB. It's not FORMAT.COM that decides the layout it
         * is MS-DOS itself. */
        ...
        /* If the partition is 1GB or larger and emulating MS-DOS 7.10 or
         * higher, default FAT32 */
        if ((part_sectors * uint64_t(sector_size)) >= (1000ull * 2048ull) &&
            (dos.version.major > 7 ||
          ...
          while (BPB.v32.BPB_SecPerClus < 64 &&
                 (part_sectors / uint64_t(BPB.v32.BPB_SecPerClus)) >
                     uint64_t(0x880000u))
          ...
          DataSectors =
              (Bitu)BPB.v32.BPB_TotSec32 -
              ((Bitu)BPB.v32.BPB_RsvdSecCnt +
               ((Bitu)BPB.v32.BPB_NumFATs * (Bitu)BPB.v32.BPB_FATSz32));
          CountOfClusters = DataSectors / BPB.v32.BPB_SecPerClus;

          BPB.v32.BPB_FATSz32 = uint32_t((CountOfClusters * 4ull) +
                                         uint64_t(sector_size) - 1ull) /
          ...
          if ((part_sectors * uint64_t(sector_size)) >=
              (31ull * 1024ull * 1024ull)) { // 31MB
            fattype = FAT16;
            while (BPB.v.BPB_SecPerClus < 32 &&
                   (part_sectors / uint64_t(BPB.v.BPB_SecPerClus)) >
                       uint64_t(0x8800u))
              BPB.v.BPB_SecPerClus *= 2u;
          } else {
            fattype = FAT12;
            while (BPB.v.BPB_SecPerClus < 64 &&
                   (part_sectors / uint64_t(BPB.v.BPB_SecPerClus)) >
                       uint64_t(0x880u))
              BPB.v.BPB_SecPerClus *= 2u;
          }
          ...
          RootDirSectors =
              ((BPB.v.BPB_RootEntCnt * 32u) + (BPB.v.BPB_BytsPerSec - 1u)) /
          ...
          if (fattype == FAT16)
            BPB.v.BPB_FATSz16 = uint32_t((CountOfClusters * 2ull) +
                                         uint64_t(sector_size) - 1ull) /
                                uint64_t(sector_size);
          else
            BPB.v.BPB_FATSz16 =
                uint32_t((((CountOfClusters + 1ull) / 2ull) * 3ull) +
                         uint64_t(sector_size) - 1ull) /
                uint64_t(sector_size);
    ...
    if (!is_hdd) {
      if (loadedDisk->heads == 0 || loadedDisk->sectors == 0 ||
          loadedDisk->cylinders == 0 ||
          loadedDisk->sector_size == 0) { // Set geometry from BPB
        headscyl = bootbuffer.bpb.v.BPB_NumHeads;
        cylsector = bootbuffer.bpb.v.BPB_SecPerTrk;
        bytesector = bootbuffer.bpb.v.BPB_BytsPerSec;
        if (headscyl == 0 || cylsector == 0 || bytesector == 0 ||
            loadedDisk->diskSizeK == 0 ||
            ((bytesector & (bytesector - 1)) != 0) /*not a power of 2*/) {
          ...
          cylinders =
              loadedDisk->diskSizeK * 1024 / headscyl / cylsector / bytesector;
          loadedDisk->Set_Geometry(headscyl, cylinders, cylsector, bytesector);
          LOG_MSG("drive_fat.cpp: Floppy geometry set from BPB information "
                  "SS/C/H/S = %u/%u/%u/%u",
                  loadedDisk->sector_size, loadedDisk->cylinders,
                  loadedDisk->heads, loadedDisk->sectors);
        }
      }
      ...
      } else if (bootbuffer.BS_jmpBoot[0] == 0x60 &&
                 bootbuffer.BS_jmpBoot[1] == 0x1c) {
        LOG_MSG("Experimental: Detected Human68k v1.00 or v2.00 floppy disk. "
                "Assuming PC-98 2HD(1.25MB disk).");
    ...
    LOG(LOG_DOSMISC, LOG_DEBUG)(
        "FAT: BPB says %u sectors/track %u heads %u bytes/sector",
        BPB.v.BPB_SecPerTrk, BPB.v.BPB_NumHeads, BPB.v.BPB_BytsPerSec);
  ...
  /* NTS: Some HDI images of PC-98 games do in fact have BPB_NumHeads == 0. Some
   * like "Amaranth 5" have BPB_SecPerTrk == 0 too! */
  if (!IS_PC98_ARCH) {
    /* a clue that we're not really looking at FAT is invalid or weird values in
     * the boot sector */
    if (BPB.v.BPB_SecPerTrk == 0 ||
        (BPB.v.BPB_SecPerTrk > ((filesize <= 3000) ? 40 : 255)) ||
        (BPB.v.BPB_NumHeads > ((filesize <= 3000) ? 64 : 255))) {
      LOG_MSG("Rejecting image, boot sector has weird values not consistent "
              "with FAT filesystem");
      created_successfully = false;
      return;
    }
  ...
  /* Many HDI images indicate a disk format of 256 or 512 bytes per sector
   * combined with a FAT filesystem that indicates 1024 bytes per sector. */
  if (pc98_512_to_1024_allow &&
    ...
    while ((unsigned int)(BPB.v.BPB_BytsPerSec >> ratioshift) >
           fatDrive::getSectSize())
      ratioshift++;

    unsigned int ratio = 1u << ratioshift;

    LOG_MSG("Disk indicates %u bytes/sector, FAT filesystem indicates %u "
            "bytes/sector. Ratio=%u:1 shift=%u",
            fatDrive::getSectSize(), BPB.v.BPB_BytsPerSec, ratio, ratioshift);

    if ((unsigned int)(BPB.v.BPB_BytsPerSec >> ratioshift) ==
        fatDrive::getSectSize()) {
      assert(ratio >= 2);

      /* the best case conversion is one where the starting sector is a multiple
       * of the ratio, but there are enough PC-98 HDI images (Dragon Knight 4
       * reported by shiningforceforever) that don't fit that model, so we have
       * to make do with a physical sector adjust too. */
      physToLogAdj = partSectOff & (ratio - 1);
      partSectOff >>= ratioshift;
      startSector >>= ratioshift;
      sector_size = BPB.v.BPB_BytsPerSec;
      LOG_MSG("Using logical sector size %u, offset by %u physical sectors",
              sector_size, physToLogAdj);
    }
  }

  /* Sanity checks */
  /* NTS: DOSBox-X *does* support non-standard sector sizes, though not in IBM
   * PC mode and not through INT 13h. In DOSBox-X INT 13h emulation will enforce
   * the standard (512 byte) sector size. In PC-98 mode mounting disk images
   * requires "non-standard" sector sizes because PC-98 floppies (other than
   * ones formatted 1.44MB) generally use 1024 bytes/sector and MAY use 128 or
   * 256 bytes per sector. */
  /* NTS: Loosen geometry checks for PC-98 mode, for two reasons. One, is that
   * the geometry check will fail when logical vs physical sector translation is
   * involved, since it is apparently common for PC-98 HDI images to be
   * formatted with 256, 512, 1024, or in rare cases even 2048 bytes per sector,
   * yet the FAT file format will report a sector size that is a power of 2
   * multiple of the disk sector size. The most common appears to be 512
   * byte/sector HDI images formatted with 1024 byte/sector FAT filesystems.
   *
   *      Second, there are some HDI images that are valid yet the FAT
   * filesystem reports a head count of 0 for some reason (Touhou Project) */
  ...
  if ((BPB.v.BPB_SecPerClus == 0) || (BPB.v.BPB_NumFATs == 0) ||
    ...
    if (isipl1 && !IS_PC98_ARCH &&
        (BPB.v.BPB_NumHeads == 0 || BPB.v.BPB_SecPerTrk == 0 ||
         BPB.v.BPB_SecPerTrk > cylsector)) {
      const char *msg =
          "Please restart DOSBox-X in PC-98 mode to mount HDI disk images.\r\n";
  ...
  /* Sanity check: Root directory count is nonzero if FAT16/FAT12, or is zero if
   * FAT32 */
  if (BPB.is_fat32()) {
    if (BPB.v.BPB_RootEntCnt != 0) {
      LOG_MSG("drive_fat.cpp: Sanity check fail: Root directory count != 0 and "
              "FAT32");
      created_successfully = false;
      return;
    }
   ...
   * MS-DOS itself as I understand it relies on bytes per sector being a power
   * of 2. this is to protect against errant FAT structures and to help prep
   * this code later to work with the 1024 bytes/sector used by PC-98 floppy
   * formats. When done, this code should be able to then handle the FDI/FDD
   * images PC-98 games are normally distributed in on the internet.
   *
  ...
  if (BPB.v.BPB_BytsPerSec < 128 || BPB.v.BPB_BytsPerSec > SECTOR_SIZE_MAX ||
      (BPB.v.BPB_BytsPerSec & (BPB.v.BPB_BytsPerSec - 1)) !=
          0 /*not a power of 2*/
      || pc98fd_sanity_failed) {
    if (is_pc98fd && ((bytesector == 1024) || (bytesector == 512))) {
      LOG_MSG(
          "Experimental: PC-98 autodetect BPB parameters when it is corrupted");
      ...
      if ((bytesector == 1024) && (cylsector == 8) &&
          (cylinders == 77)) { // PC-98 2HD 1.25MB
        BPB.v.BPB_RootEntCnt = 0xc0;
        BPB.v.BPB_TotSec16 = 0x4d0;
        BPB.v.BPB_FATSz16 = 2;
        BPB.v.BPB_Media = 0xfe;
      } else if ((bytesector == 512) && (cylsector == 15) &&
                 (cylinders == 80)) { // PC-98 2HC 1.21MB
        BPB.v.BPB_RootEntCnt = 0xe0;
        BPB.v.BPB_TotSec16 = 0x960;
        BPB.v.BPB_FATSz16 = 7;
        BPB.v.BPB_Media = 0xf9;
      } else if ((bytesector == 512) && (cylsector == 18) &&
                 (cylinders == 80)) { // PC-98 2HD 1.44MB
        BPB.v.BPB_RootEntCnt = 0xe0;
        BPB.v.BPB_TotSec16 = 0xb40;
        BPB.v.BPB_FATSz16 = 9;
        BPB.v.BPB_Media = 0xf0;
      } else if ((bytesector == 512) && (cylsector == 8) &&
                 (cylinders == 80)) { // PC-98 2DD 640kB
        BPB.v.BPB_RootEntCnt = 0x70;
        BPB.v.BPB_TotSec16 = 0x500;
        BPB.v.BPB_FATSz16 = 2;
        BPB.v.BPB_Media = 0xfb;
      } else if ((bytesector == 512) && (cylsector == 9) &&
                 (cylinders == 80)) { // PC-98 2DD 720kB
        BPB.v.BPB_RootEntCnt = 0x70;
        BPB.v.BPB_TotSec16 = 0x5a0;
        BPB.v.BPB_FATSz16 = 3;
        BPB.v.BPB_Media = 0xf9;
      } else {
        LOG_MSG("Experimental: PC-98 assuming BPB parameters to be 1.25MB 2HD "
                "floppy, this may not work.");
        BPB.v.BPB_RootEntCnt = 0xc0;
        BPB.v.BPB_TotSec16 = 0x4d0;
        BPB.v.BPB_Media = 0xfe;
        BPB.v.BPB_FATSz16 = 2;
  ...
  /* another fault of this code is that it assumes the sector size of the medium
   * matches the BPB_BytsPerSec value of the MS-DOS filesystem. if they don't
   * match, problems will result. */
  ...
  /* Filesystem must be contiguous to use absolute sectors, otherwise CHS will
   * be used */
  absolute = IS_PC98_ARCH || ((BPB.v.BPB_NumHeads == headscyl) &&
                              (BPB.v.BPB_SecPerTrk == cylsector));
  LOG(LOG_DOSMISC, LOG_DEBUG)("FAT driver: Using %s sector access",
                              absolute ? "absolute" : "C/H/S");
  ...
  if (BPB.is_fat32()) {
    ...
    RootDirSectors = 0; /* FAT32 root directory has its own allocation chain,
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("INIT FAT: data=%llu root=%llu rootdirsect=%lu datasect=%lu",
                   (unsigned long long)firstDataSector,
                   (unsigned long long)firstRootDirSect,
  ...
  /* just so you know....! */
  if (fattype == FAT32 && (dos.version.major < 7 || (dos.version.major == 7 &&
                                                     dos.version.minor < 10))) {
    LOG_MSG("CAUTION: Mounting FAT32 partition when reported DOS version is "
            "less than 7.10. Disk formatting/repair utilities may mis-identify "
            "the partition.");
  }
...
bool fatDrive::AllocationInfo32(uint32_t *_bytes_sector,
                                uint32_t *_sectors_cluster,
                                uint32_t *_total_clusters,
                                uint32_t *_free_clusters) {
...
bool fatDrive::AllocationInfo(uint16_t *_bytes_sector,
  ...
  if (BPB.is_fat32()) {
    ...
    if (AllocationInfo32(&bytes32, &sectors32, &clusters32, &free32) &&
        DOS_CommonFAT32FAT16DiskSpaceConv(
            _bytes_sector, _sectors_cluster, _total_clusters, _free_clusters,
            bytes32, sectors32, clusters32, free32))
      return true;
    ...
    /* FAT12/FAT16 should never allow more than 0xFFF6 clusters and partitions
     * larger than 2GB */
...
void fatDrive::SetBPB(const FAT_BootSector::bpb_union_t &bpb) {
  ...
  if (CountOfClusters < 4085) {
    /* Volume is FAT12 */
    LOG_MSG("Mounted FAT volume is now FAT12 with %d clusters",
            CountOfClusters);
    fattype = FAT12;
  } else if (CountOfClusters < 65525 || !bpb.is_fat32()) {
    LOG_MSG("Mounted FAT volume is now FAT16 with %d clusters",
            CountOfClusters);
    fattype = FAT16;
  } else {
    LOG_MSG("Mounted FAT volume is now FAT32 with %d clusters",
            CountOfClusters);
    fattype = FAT32;
  }
}

bool fatDrive::FileCreate(DOS_File **file, const char *name,
                          uint16_t attributes) {
  const char *lfn = NULL;
  ...
  if (getFileDirEntry(name, &fileEntry, &dirClust, &subEntry, true /*dirOk*/)) {
    ...
    {
      const uint32_t chk =
          BPB.is_fat32() ? fileEntry.Cluster32() : fileEntry.loFirstClust;
      if (chk != 0)
    ...
    if (!getEntryName(name, &dirName[0]) || !strlen(trim(dirName)))
      return false;
    ...
    /* NTS: "name" is the full relative path. For LFN creation to work we need
     * only the final element of the path */
    if (uselfn && !force_sfn) {
  ...
  *file = new fatFile(
      name, BPB.is_fat32() ? fileEntry.Cluster32() : fileEntry.loFirstClust,
      fileEntry.entrysize, this);
...
bool fatDrive::FileStat(const char * /*name*/,
                        FileStat_Block *const /*stat_block*/) {
  /* TODO: Stub */
  return false;
}
...
bool fatDrive::FileUnlink(const char *name) {
  ...
  lfnRange.clear();
  if (!getFileDirEntry(name, &fileEntry, &dirClust, &subEntry))
    return false; /* Do not use dirOk, DOS should never call this unless a file
                   */
  lfnRange_t dir_lfn_range =
      lfnRange; /* copy down LFN results before they are obliterated by the next
                   call to FindNextInternal. */
  ...
  if (!dir_lfn_range.empty() && (dos.version.major >= 7 || uselfn)) {
    ...
    if (dir_lfn_range.dirPos_end != subEntry)
      LOG_MSG("FAT warning: LFN dirPos_end=%u fileidx=%u (mismatch)",
              dir_lfn_range.dirPos_end, subEntry);
    for (unsigned int didx = dir_lfn_range.dirPos_start;
         didx < dir_lfn_range.dirPos_end; didx++) {
...
bool fatDrive::FindFirst(const char *_dir, DOS_DTA &dta, bool fcb_findfirst) {
  ...
  /* Elder Scrolls Arena does an 8-bit divide using CL=A and then forgets to
   * clear CL when calling INT 21h AH=4Eh, which to us appears as a program
   * searching for files that may also be a volume label and/or hidden
   * (CX=0x000A). */
  /* TODO: Perhaps this check could be rolled into the _dir && *_dir &&
   * dta.GetAttr() == 0x3F check? */
  /* Elder Scrolls Arena: The load game screen doesn't just open SAVEGAME.00, it
   * does an INT 21h AH=4Eh search for it with CX=0x007C??? What the fuck? If we
   * don't watch for this, then players running the game from a image will see
   * their saves, but will not be able to load them ("No save here"). */
  bool ignore_volbit = false;
  if (dta.GetAttr() & DOS_ATTR_VOLUME) {
    if (dta.GetAttr() &
        (0xC0 | DOS_ATTR_HIDDEN | DOS_ATTR_SYSTEM | DOS_ATTR_READ_ONLY)) {
      LOG(LOG_MISC, LOG_DEBUG)("FindFirst() ignoring volume label bit because "
                               "other bits are set (Elder Scrolls Arena fix)");
      ignore_volbit = true;
    }
  }

  // volume label searches always affect root directory, no matter the current
  // directory, at least with FCBs
  if (dta.GetAttr() == DOS_ATTR_VOLUME ||
      ((dta.GetAttr() & DOS_ATTR_VOLUME) && !ignore_volbit &&
       (fcb_findfirst || !(_dir && *_dir && dta.GetAttr() == 0x3F)))) {
...
char *removeTrailingSpaces(char *str) {
  ...
  /* NTS: The loop will exit with 'end' one char behind the last ' ' space
   * character. So to ASCIIZ snip off the space, step forward one and overwrite
   * with NUL. The loop may end with 'end' one char behind 'ptr' if the string
   * was empty "" or nothing but spaces. This is OK because after the step
   * forward, end >= str in all cases. */
  *(++end) = '\0';
  return str;
}
...
uint32_t fatDrive::GetSectorCount(void) {
  return (loadedDisk->heads * loadedDisk->sectors * loadedDisk->cylinders) -
         partSectOff;
}

uint32_t fatDrive::GetSectorSize(void) { return getSectorSize(); }
...
bool fatDrive::FindNextInternal(uint32_t dirClustNumber, DOS_DTA &dta,
  ...
  dirPos =
      lfn_filefind_handle >= LFN_FILEFIND_MAX
          ? dta.GetDirID()
          : dpos[lfn_filefind_handle]; /* NTS: Windows 9x is said to have a
                                          65536 dirent limit even for FAT32, so
                                          dirPos as 16-bit is acceptable */
...
nextfile:
  ...
  if (lfn_filefind_handle >= LFN_FILEFIND_MAX)
    ...
    dpos[lfn_filefind_handle] = dirPos;
  ...
  if (attrs == DOS_ATTR_VOLUME) {
    ...
    labelCache.SetLabel(find_name, false, true);
             ...
             (sectbuf[entryoffset].attrib & 0x3F) ==
                 0x0F) { /* long filename piece */
    struct direntry_lfn *dlfn = (struct direntry_lfn *)(&sectbuf[entryoffset]);

    /* assume last entry comes first, because that's how Windows 9x does it and
     * that is how you're supposed to do it according to Microsoft */
    if (dlfn->LDIR_Ord & 0x40) {
      lfn_max_ord =
          (dlfn->LDIR_Ord & 0x3F); /* NTS: First entry has ordinal 1, this is
                                      the HIGHEST ordinal in the LFN. The other
                                      entries follow in descending ordinal. */
      for (unsigned int i = 0; i < 0x40; i++)
        lfn_ord_found[i] = false;
      lfn_checksum = dlfn->LDIR_Chksum;
      memset(lfind_name, 0, LFN_NAMELENGTH);
      lfnRange.clear();
      lfnRange.dirPos_start =
          dirPos - 1; /* NTS: The code above has already incremented dirPos */
    }

    if (lfn_max_ord != 0 && (dlfn->LDIR_Ord & 0x3F) > 0 &&
        (dlfn->LDIR_Ord & 0x3Fu) <= lfn_max_ord &&
        dlfn->LDIR_Chksum == lfn_checksum) {
      ...
      for (unsigned int i = 0; i < 5; i++) {
        ...
        if (lchar < 0x100 || lchar == 0xFFFF)
          lname[len++] = lchar != 0xFFFF &&
                                 CodePageHostToGuestUTF16(text, &lchar) &&
                                 text[0] && !text[1]
                             ? text[0]
                             : (char)(lchar & 0xFF);
        else {
    ...
    if (~attrs & sectbuf[entryoffset].attrib &
        (DOS_ATTR_DIRECTORY | DOS_ATTR_VOLUME)) {
      lfind_name[0] =
          0; /* LFN code will memset() it in full upon next dirent */
      lfn_max_ord = 0;
      lfnRange.clear();
      goto nextfile;
    }
  ...
  if (lfn_max_ord != 0) {
    ...
    for (unsigned int i = 0; i < lfn_max_ord; i++)
      complete += lfn_ord_found[i] ? 1 : 0;

    if (complete == lfn_max_ord) {
      unsigned char chk = 0;
      for (unsigned int i = 0; i < 11; i++) {
        chk = ((chk & 1u) ? 0x80u : 0x00u) + (chk >> 1u) +
              sectbuf[entryoffset].entryname[i];
      }
  ...
  /* Compare name to search pattern. Skip long filename match if no long
   * filename given. */
  if (attrs == DOS_ATTR_VOLUME) {
    if (!(wild_match(find_name, srch_pattern)))
      goto nextfile;
  } else if (!(WildFileCmp(find_name, srch_pattern) ||
               (lfn_max_ord != 0 && lfind_name[0] != 0 &&
                LWildFileCmp(lfind_name, srch_pattern)))) {
    lfind_name[0] = 0; /* LFN code will memset() it in full upon next dirent */
    lfn_max_ord = 0;
    lfnRange.clear();
    goto nextfile;
  ...
  // Drive emulation does not need to require a LFN in case there is no
  // corresponding 8.3 names.
  if (lfind_name[0] == 0)
    strcpy(lfind_name, find_name);

  copyDirEntry(&sectbuf[entryoffset], foundEntry);

  // dta.SetResult(find_name, foundEntry->entrysize, foundEntry->crtDate,
  // foundEntry->crtTime, foundEntry->attrib);

  dta.SetResult(find_name, lfind_name, foundEntry->entrysize, 0,
                foundEntry->modDate, foundEntry->modTime, foundEntry->attrib);
...
bool fatDrive::FindNext(DOS_DTA &dta) {
  ...
  return FindNextInternal(
      lfn_filefind_handle >= LFN_FILEFIND_MAX
          ? dta.GetDirIDCluster()
          : (dnum[lfn_filefind_handle] ? dnum[lfn_filefind_handle] : 0),
      dta, &dummyClust);
}
...
bool fatDrive::directoryBrowse(uint32_t dirClustNumber, direntry *useEntry,
  ...
  /* NTS: This change provides a massive performance boost for the FAT driver.
   * The previous code, inherited from SVN, would read every directory entry in
   * sequence up to entNum. It would also re-read the sector every dirent even
   * if the same sector, and it called getAbsoluteSectFromChain() every single
   *      time. Which means that for every directory entry this was asked to
   * scan (and called a LOT especially in a directory with more than 100 files!)
   * this code would read the sector, re-read the allocation chain up to the
   * desired offset, and do it entNum times! No wonder it was so slow! --J.C.
   * 2023/04/11 */
  const uint16_t dirPos = (uint16_t)entNum;
  const uint32_t logentsector = ((
      uint32_t)((size_t)dirPos / dirent_per_sector)); /* Logical entry sector */
  ...
  readSector(tmpsector, sectbuf);
  if (sectbuf[entryoffset].entryname[0] == 0x00)
    return false; /* End of directory list? */
  copyDirEntry(&sectbuf[entryoffset], useEntry);
  return true;
}

bool fatDrive::directoryChange(uint32_t dirClustNumber,
                               const direntry *useEntry, int32_t entNum) {
  if (unformatted)
    return false;
...
bool fatDrive::addDirectoryEntry(uint32_t dirClustNumber,
                                 const direntry &useEntry, const char *lfn) {
  if (unformatted)
    return false;
  ...
  if (lfn != NULL && *lfn != 0) {
    ...
    for (const char *scan = lfn; *scan; scan++) {
      if (lead) {
        ...
        uname[0] = 0;
        uname[1] = 0;
        if (CodePageGuestToHostUTF16(uname, text) && uname[0] != 0 &&
            uname[1] == 0) {
          lfnw[len++] = uname[0];
        } else {
          lfnw[len++] = *(scan - 1) & 0xFF;
          if (len < LFN_NAMELENGTH)
            lfnw[len++] = *scan & 0xFF;
        }
      } else if (*(scan + 1) &&
                 ((IS_PC98_ARCH && shiftjis_lead_byte(*scan & 0xFF)) ||
                  (isDBCSCP() && isKanji1_gbk(*scan & 0xFF))))
        lead = true;
      else if (dos.loaded_codepage != 437) {
        text[0] = *scan & 0xFF;
        text[1] = 0;
        lfnw[len++] = CodePageGuestToHostUTF16(uname, text) && uname[0] != 0 &&
                              uname[1] == 0
                          ? uname[0]
                          : (uint16_t)((unsigned char)(*scan));
      } else
  ...
  for (;;) {
    uint32_t logentsector =
        ((uint32_t)((size_t)dirPos /
                    dirent_per_sector)); /* Logical entry sector */
    uint32_t entryoffset =
        ((uint32_t)((size_t)dirPos %
                    dirent_per_sector)); /* Index offset within sector */
    ...
    if ((sectbuf[entryoffset].entryname[0] == 0xe5) ||
        (sectbuf[entryoffset].entryname[0] == 0x00)) {
      if (found == 0)
        dirPosFound = dirPos;
      ...
      if ((++found) >= need) {
        ...
        /* Add LFN entries */
        if (need != 1 /*LFN*/) {
          uint16_t lfnbuf[LFN_NAMELENGTH +
                          13]; /* on disk, LFNs are WCHAR unicode (UCS-16) */
          ...
          /* on disk, LFNs are padded with 0x0000 followed by a run of 0xFFFF to
           * fill the dirent */
          lfnbuf[o++] = 0x0000;
          ...
          for (unsigned int s = 0; s < (need - 1u); s++) {
            ...
            if (dirClustNumber == 0) {
              ...
              tmpsector =
                  getAbsoluteSectFromChain(dirClustNumber, logentsector);
              /* A zero sector number can't happen - we need to allocate more
               * room for this directory*/
              if (tmpsector == 0)
                return false;
            }
            readSector(tmpsector, sectbuf);
            ...
            for (unsigned int i = 0; i < 5; i++)
              dlfn->LDIR_Name1[i] = lfnbuf[lfnsrc++];
            for (unsigned int i = 0; i < 6; i++)
              dlfn->LDIR_Name2[i] = lfnbuf[lfnsrc++];
            for (unsigned int i = 0; i < 2; i++)
              dlfn->LDIR_Name3[i] = lfnbuf[lfnsrc++];
...
bool fatDrive::MakeDir(const char *dir) {
  ...
  if (!getEntryName(dir, &dirName[0]) || !strlen(trim(dirName)))
    return false;
  convToDirFile(&dirName[0], &pathName[0]);

  /* Fail to make directory if something of that name already exists */
  if (getFileDirEntry(dir, &tmpentry, &dummyClust, &subEntry, /*dirOk*/ true))
    return false;
  ...
  /* NTS: "dir" is the full relative path. For LFN creation to work we need only
  ...
  if (BPB.is_fat32() && dirClust == BPB.v32.BPB_RootClus) {
    /* Windows 98 SCANDISK.EXE considers it an error for the '..' entry of a top
     * level directory to point at the actual cluster number of the root
     * directory. The correct value is 0 apparently. */
...
bool fatDrive::RemoveDir(const char *dir) {
  ...
  lfnRange.clear();
  if (!getFileDirEntry(dir, &tmpentry, &dirClust, &subEntry, /*dirOk*/ true))
    return false; /* dirClust is parent dir of directory */
  if (!(tmpentry.attrib & DOS_ATTR_DIRECTORY))
    return false;
  ...
  /* NTS: The code below only cares if there are non-deleted files, not *how
   * many*, therefore the loop will now terminate immediately upon finding one.
   * --J.C. 2023/04/11 */
  uint32_t filecount = 0;
  /* Set to 2 to skip first 2 entries, [.] and [..] */
  int32_t fileidx = 2;
  while (directoryBrowse(dummyClust, &tmpentry, fileidx)) {
    /* Check for non-deleted files. NTS: directoryBrowse() will return false if
     * entryname[0] == 0 */
    if (tmpentry.entryname[0] != 0xe5) {
      filecount++;
      break;
    }
    fileidx++;
  ...
  if (!directoryBrowse(dirClust, &tmpentry, subEntry))
    return false;
  tmpentry.entryname[0] = 0xe5;
  if (!directoryChange(dirClust, &tmpentry, subEntry))
    return false;
...
bool fatDrive::Rename(const char *oldname, const char *newname) {
  ...
  if (getFileDirEntry(newname, &fileEntry2, &dirClust2, &subEntry2,
                      /*dirOk*/ true) &&
      !(uselfn && !force_sfn && strcmp(oldname, newname) &&
        !strcasecmp(oldname, newname)))
    return false;

  /* Can we even get the name of the file itself? */
  if (!getEntryName(newname, &dirName2[0]) || !strlen(trim(dirName2)))
    return false;
  convToDirFile(&dirName2[0], &pathName2[0]);

  /* Can we find the base directory of the new name? (we know the parent dir of
   * oldname in dirClust1) */
  if (!getDirClustNum(newname, &dirClust2, true))
    return false;

  /* NTS: "newname" is the full relative path. For LFN creation to work we need
   * only the final element of the path */
  if (uselfn && !force_sfn) {
  ...
  if (!dir_lfn_range.empty() && (dos.version.major >= 7 || uselfn)) {
    /* last LFN entry should be fileidx */
    assert(dir_lfn_range.dirPos_start < dir_lfn_range.dirPos_end);
    if (dir_lfn_range.dirPos_end != subEntry1)
      LOG_MSG("FAT warning: LFN dirPos_end=%u fileidx=%u (mismatch)",
              dir_lfn_range.dirPos_end, subEntry1);
    for (unsigned int didx = dir_lfn_range.dirPos_start;
         didx < dir_lfn_range.dirPos_end; didx++) {
...
void fatDrive::checkDiskChange(void) {
   ...
   * The demo runs A:\GO.EXE, but the floppy disk doesn't actually exist, it's
   * brought into existence by intercepting INT 13h for floppy I/O and then
   * expecting MS-DOS to call INT 13h to read it. Furthermore, at some parts of
   * the demo, the INT 13h hook changes the root directory and FAT table to make
   * the next "disk" appear, even if the volume label does not. */
  if (loadedDisk->detectDiskChange() && !BPB.is_fat32() && partSectOff == 0) {
    ...
    /* sometimes as part of INT 13h interception the new image will differ from
     * the format we mounted against (DS_BLISS.EXE) */
    /* TODO: Add any other format here */
    if (BPB.v.BPB_Media == 0xF9) {
      LOG(LOG_MISC, LOG_DEBUG)("NEW FAT: changing floppy geometry to 720k "
                               "format according to media byte");
      loadedDisk->Set_Geometry(2 /*heads*/, 80 /*cylinders*/, 9 /*sectors*/,
                               512 /*sector size*/);
    } else if (BPB.v.BPB_Media == 0xF0) {
      /* TODO: By the time DOS supported the 1.44MB format the BPB also provided
       * the geometry too */
      LOG(LOG_MISC, LOG_DEBUG)("NEW FAT: changing floppy geometry to 1.44M "
                               "format according to media byte");
      loadedDisk->Set_Geometry(2 /*heads*/, 80 /*cylinders*/, 18 /*sectors*/,
                               512 /*sector size*/);
    }
    ...
    LOG(LOG_MISC,
        LOG_DEBUG)("NEW FAT: data=%llu root=%llu rootdirsect=%lu datasect=%lu",
                   (unsigned long long)firstDataSector,
                   (unsigned long long)firstRootDirSect,
                   (unsigned long)RootDirSectors, (unsigned long)DataSectors);
  }
