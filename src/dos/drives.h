...
public:
  ...
  bool FindFirst(const char *_dir, DOS_DTA &dta,
                 bool fcb_findfirst = false) override;
  ...
  bool AllocationInfo64(uint32_t *_bytes_sector, uint32_t *_sectors_cluster,
                        uint64_t *_total_clusters,
                        uint64_t *_free_clusters) override;
  ...
  void SetLabel(const char *label, bool iscdrom, bool updatable) override {
    dirCache.SetLabel(label, iscdrom, updatable);
  };
  void *opendir(const char *name) override;
  void closedir(void *handle) override;
  bool read_directory_first(void *handle, char *entry_name, char *entry_sname,
                            bool &is_directory) override;
  bool read_directory_next(void *handle, char *entry_name, char *entry_sname,
                           bool &is_directory) override;
  virtual void remove_special_file_from_disk(const char *dosname,
                                             const char *operation);
  virtual std::string
  create_filename_of_special_operation(const char *dosname,
                                       const char *operation, bool expand);
  virtual bool add_special_file_to_disk(const char *dosname,
...
public:
  physfsDrive(const char driveLetter, const char *startdir,
              uint16_t _bytes_sector, uint8_t _sectors_cluster,
              uint16_t _total_clusters, uint16_t _free_clusters,
              uint8_t _mediaid, int &error, std::vector<std::string> &options);
  bool FileOpen(DOS_File **file, const char *name, uint32_t flags) override;
  bool FileCreate(DOS_File **file, const char *name,
                  uint16_t attributes) override;
  bool FileUnlink(const char *name) override;
...
struct FAT_BPB_MSDOS20 {
  uint16_t BPB_BytsPerSec; /* offset 0x00B size 0x002 Bytes per sector. Formerly
                              bytespersector */
  uint8_t BPB_SecPerClus;  /* offset 0x00D size 0x001 Sectors per cluster, must
                              be a power of 2. Formerly sectorspercluster */
  uint16_t BPB_RsvdSecCnt; /* offset 0x00E size 0x002 Number of reserved sectors
                              starting from partition, to FAT table.
                              reservedsectors */
  uint8_t
      BPB_NumFATs; /* offset 0x010 size 0x001 Number of FAT tables. fatcopies */
  uint16_t BPB_RootEntCnt; /* offset 0x011 size 0x002 Number of 32-byte root
                              directories (FAT12/FAT16), or 0 (FAT32).
                              rootdirentries */
  uint16_t
      BPB_TotSec16; /* offset 0x013 size 0x002 Total sectors of volume if count
                       < 0x10000, or 0 if not. 0 if FAT32. totalsectorcount */
  uint8_t
      BPB_Media; /* offset 0x015 size 0x001 Media type byte. mediadescriptor */
  uint16_t BPB_FATSz16;  /* offset 0x016 size 0x002 Sectors per fat
                            (FAT12/FAT16), or 0 (FAT32). sectorsperfat */
...
struct FAT_BPB_MSDOS30 {
  ...
  uint16_t BPB_FATSz16;   /* offset 0x016 size 0x002 Sectors per fat
                             (FAT12/FAT16), or 0 (FAT32). sectorsperfat */
  uint16_t BPB_SecPerTrk; /* offset 0x018 size 0x002 Sectors per track.
                             sectorspertrack */
  uint16_t
      BPB_NumHeads;     /* offset 0x01A size 0x002 Number of heads. headcount */
  uint32_t BPB_HiddSec; /* offset 0x01C size 0x004 Number of hidden sectors
                           (i.e. starting sector of partition).
                           hiddensectorcount (MS-DOS 3.31) */
} GCC_ATTRIBUTE(packed); /*    ==> 0x020 size 0x015 total */
                         /* ==== ADDITIONAL NOTES (Wikipedia) */
/* offset 0x01C size 0x002 Number of hidden sectors (i.e. starting sector of
 * partition). hiddensectorcount (MS-DOS 3.0) */
/* offset 0x01E size 0x002 Total sectors including hidden (?) if BPB_TotSec16 !=
 * 0 (MS-DOS 3.20) */
...
struct FAT_BPB_MSDOS331 {
  ...
  uint32_t BPB_HiddSec; /* offset 0x01C size 0x004 Number of hidden sectors
                           ...
                           hiddensectorcount (MS-DOS 3.31) */
  uint32_t
      BPB_TotSec32; /* offset 0x020 size 0x004 Total sectors of volume if count
                       >= 0x10000 or FAT32, or 0 if not. totalsecdword */
...
struct FAT_BPB_MSDOS40 {   /* FAT12/FAT16 only */
  ...
  uint32_t
      BPB_TotSec32; /* offset 0x020 size 0x004 Total sectors of volume if count
                       >= 0x10000 or FAT32, or 0 if not. totalsecdword */
  uint8_t
      BPB_DrvNum; /* offset 0x024 size 0x001 Physical (INT 13h) drive number */
  uint8_t BPB_Reserved1; /* offset 0x025 size 0x001 Reserved? */
  uint8_t BPB_BootSig; /* offset 0x026 size 0x001 Extended boot signature. 0x29
                          or 0x28 to indicate following members exist. */
  uint32_t BPB_VolID;  /* offset 0x027 size 0x004 Volume ID, if BPB_BootSig is
                          0x28 or 0x29. */
  uint8_t BPB_VolLab[11];    /* offset 0x02B size 0x00B Volume label, if
                                BPB_BootSig is 0x28 or 0x29. */
  uint8_t BPB_FilSysType[8]; /* offset 0x036 size 0x008 File system type, for
                                display purposes if BPB_BootSig is 0x29. */
...
struct FAT_BPB_MSDOS710_FAT32 { /* FAT32 only */
  ...
  uint16_t
      BPB_ExtFlags; /* offset 0x028 size 0x002 Bitfield: [7:7] 1=one fat active
                       0=mirrored  [3:0]=active FAT if mirroring disabled */
  uint16_t
      BPB_FSVer; /* offset 0x02A size 0x002 Version number. Only 0.0 is defined
                    now. Do not mount if newer version beyond what we support */
  uint32_t BPB_RootClus;  /* offset 0x02C size 0x004 Starting cluster number of
                             the root directory (FAT32) */
  uint16_t BPB_FSInfo;    /* offset 0x030 size 0x002 Sector number in volume of
                             FAT32 FSInfo structure in reserved area */
  uint16_t BPB_BkBootSec; /* offset 0x032 size 0x002 Sector number in volume of
                             FAT32 backup boot sector */
  uint8_t BPB_Reserved[12]; /* offset 0x034 size 0x00C Reserved for future
                               expansion */
  uint8_t
      BS_DrvNum; /* offset 0x040 size 0x001 BPB_DrvNum but moved for FAT32 */
  uint8_t BS_Reserved1; /* offset 0x041 size 0x001 BPB_Reserved1 but moved for
                           FAT32 */
  uint8_t BS_BootSig;   /* offset 0x042 size 0x001 Extended boot signature. 0x29
                           or 0x28 to indicate following members exist. */
  uint32_t BS_VolID;    /* offset 0x043 size 0x004 Volume ID, if BPB_BootSig is
                           0x28 or 0x29. */
  uint8_t BS_VolLab[11]; /* offset 0x047 size 0x00B Volume label, if BPB_BootSig
                            is 0x28 or 0x29. */
  uint8_t BS_FilSysType[8]; /* offset 0x052 size 0x008 File system type, for
                               display purposes if BPB_BootSig is 0x29. */
} GCC_ATTRIBUTE(packed);    /*    ==> 0x05A size 0x04F total */

typedef struct FAT_BPB_MSDOS40 FAT_BPB_MSDOS; /* what we use internally */
typedef struct FAT_BPB_MSDOS710_FAT32
    FAT32_BPB_MSDOS; /* what we use internally */

struct FAT_BootSector {
  /* --------- Common fields: Amalgam of Wikipedia documentation with names from
   * Microsoft's FAT32 whitepaper */
  uint8_t BS_jmpBoot[3]; /* offset 0x000 size 0x003 Jump instruction to boot
                            code. Formerly nearjmp[3] */
  uint8_t BS_OEMName[8]; /* offset 0x003 size 0x008 OEM string. Formerly
                            oemname[8] */
  /* --------- BIOS Parameter Block (converted in place from existing DOSBox-X
   * code) */
  union bpb_union_t {
    ...
    struct FAT_BPB_MSDOS40
        v40; /* offset 0x00B size 0x039 MS-DOS 4.0 BPB (FAT12/FAT16) */
    struct FAT_BPB_MSDOS710_FAT32
        v710_32; /* offset 0x00B size 0x04F MS-DOS 7.10 BPB (FAT32) */

    FAT_BPB_MSDOS v;     /* offset 0x00B ... */
    FAT32_BPB_MSDOS v32; /* offset 0x00B ... */

    inline bool is_fat32(void) const {
      return (v.BPB_RootEntCnt == 0 && v.BPB_TotSec16 == 0 &&
              v.BPB_FATSz16 ==
                  0); /* all fields are "must be set to 0" for FAT32 */
    }
  } bpb;
  /* --------- The rest of the sector ---------- */
  uint8_t
      bootcode[512 - 2 /*magic*/ - sizeof(bpb_union_t) - 8 /*OEM*/ - 3 /*JMP*/];
  uint8_t magic1; /* 0x55 */
  uint8_t magic2; /* 0xaa */
...
static_assert(offsetof(FAT_BootSector, bpb.v710_32.BS_FilSysType) == 0x052,
              "Oops");
static_assert(sizeof(FAT_BootSector) == SECTOR_SIZE_MAX, "Oops");
static_assert(offsetof(FAT_BootSector, bpb.v331.BPB_TotSec32) == 0x020, "Oops");
static_assert(offsetof(FAT_BootSector, bpb.v40.BPB_TotSec32) == 0x020, "Oops");
static_assert(offsetof(FAT_BootSector, bpb.v710_32.BPB_TotSec32) == 0x020,
              "Oops");
...
struct direntry {
  ...
  uint16_t crtTime;      // <- NTS: This field did not appear until MS-DOS 7.0
                         // (Windows 95)
  uint16_t crtDate;      // <- NTS: This field did not appear until MS-DOS 7.0
                         // (Windows 95)
  uint16_t accessDate;   // <- NTS: This field did not appear until MS-DOS 7.0
                         // (Windows 95)
...
struct direntry_lfn {
  uint8_t LDIR_Ord; /* 0x00 Long filename ordinal (1 to 63). bit 6 (0x40) is set
                       if the last entry, which normally comes first in the
                       directory */
...
public:
  fatDrive(const char *sysFilename, uint32_t bytesector, uint32_t cylsector,
           uint32_t headscyl, uint32_t cylinders,
           std::vector<std::string> &options);
  fatDrive(imageDisk *sourceLoadedDisk, std::vector<std::string> &options);
  void fatDriveInit(const char *sysFilename, uint32_t bytesector,
                    uint32_t cylsector, uint32_t headscyl, uint32_t cylinders,
                    uint64_t filesize, const std::vector<std::string> &options);
  ~fatDrive();
  bool FileOpen(DOS_File **file, const char *name, uint32_t flags) override;
  bool FileCreate(DOS_File **file, const char *name,
                  uint16_t attributes) override;
  ...
  bool AllocationInfo(uint16_t *_bytes_sector, uint8_t *_sectors_cluster,
                      uint16_t *_total_clusters,
                      uint16_t *_free_clusters) override;
  bool AllocationInfo32(uint32_t *_bytes_sector, uint32_t *_sectors_cluster,
                        uint32_t *_total_clusters,
                        uint32_t *_free_clusters) override;
  bool FileExists(const char *name) override;
  bool FileStat(const char *name, FileStat_Block *const stat_block) override;
  uint8_t GetMediaByte(void) override;
  bool isRemote(void) override;
...
public:
  ...
  uint32_t getAbsoluteSectFromBytePos(uint32_t startClustNum, uint32_t bytePos,
                                      clusterChainMemory *ccm = NULL);
  uint32_t getSectorCount(void);
  uint32_t getSectorSize(void);
  uint32_t getClusterSize(void);
  uint32_t getAbsoluteSectFromChain(uint32_t startClustNum,
  ...
  bool directoryBrowse(uint32_t dirClustNumber, direntry *useEntry,
                       int32_t entNum, int32_t start = 0);
  bool directoryChange(uint32_t dirClustNumber, const direntry *useEntry,
                       int32_t entNum);
...
private:
  ...
  bool FindNextInternal(uint32_t dirClustNumber, DOS_DTA &dta,
                        direntry *foundEntry);
  bool getDirClustNum(const char *dir, uint32_t *clustNum, bool parDir);
  bool getFileDirEntry(char const *const filename, direntry *useEntry,
                       uint32_t *dirClust, uint32_t *subEntry,
                       bool dirOk = false);
  bool addDirectoryEntry(uint32_t dirClustNumber, const direntry &useEntry,
                         const char *lfn = NULL);
  ...
  struct lfnRange_t {
    ...
    void clear(void) { dirPos_start = dirPos_end = 0; }
    bool empty(void) const { return dirPos_start == dirPos_end; }
  } lfnRange = {0, 0};
  ...
  FAT_BootSector::bpb_union_t BPB =
      {}; // BPB in effect (translated from on-disk BPB as needed)
  bool absolute = false;
  uint8_t fattype = 0;
  ...
  uint32_t physToLogAdj =
      0; // Some PC-98 HDI images have larger logical than physical bytes/sector
         // and the partition is not a multiple of it, so this is needed
...
public:
  /* the driver code must use THESE functions to read the disk, not directly
   * from the disk drive, in order to support a drive with a smaller sector size
   * than the FAT filesystem's "sector".
   *
   * It is very common for instance to have PC-98 HDI images formatted with 256
   * bytes/sector at the disk level and a FAT filesystem marked as having 1024
   * bytes/sector. */
...
public:
  cdromDrive(const char driveLetter, const char *startdir,
             uint16_t _bytes_sector, uint8_t _sectors_cluster,
             uint16_t _total_clusters, uint16_t _free_clusters,
             uint8_t _mediaid, int &error, std::vector<std::string> &options);
  bool FileOpen(DOS_File **file, const char *name, uint32_t flags) override;
...
public:
  physfscdromDrive(const char driveLetter, const char *startdir,
                   uint16_t _bytes_sector, uint8_t _sectors_cluster,
                   uint16_t _total_clusters, uint16_t _free_clusters,
...
public:
  UDF_blob(const std::string &x) : UDF_blob_base() {
    ...
    for (size_t i = 0; i < x.size(); i++)
      p[i] = x[i]; /* Not the NUL at the end, though */
  }
  std::string string_value(void) const {
    return std::string((char *)(&UDF_blob_base::operator[](0)),
                       UDF_blob_base::size());
  }
...
struct UDFAnchorVolumeDescriptorPointer {
  ...
  void get(UDFTagId &tag /*already parsed, why parse again?*/,
...
/* NTS: The structure is the same, what the location/position is relative to is
 * different. It's relative to the partition */
...
struct UDFPrimaryVolumeDescriptor {
  ...
  void get(UDFTagId &tag /*already parsed, why parse again?*/,
           const unsigned int sz, const unsigned char *b);
  UDFPrimaryVolumeDescriptor(UDFTagId &tag /*already parsed, why parse again?*/,
                             const unsigned int sz, const unsigned char *b);
...
struct UDFPartitionDescriptor {
  ...
  void get(UDFTagId &tag /*already parsed, why parse again?*/,
           const unsigned int sz, const unsigned char *b);
  UDFPartitionDescriptor(UDFTagId &tag /*already parsed, why parse again?*/,
                         const unsigned int sz, const unsigned char *b);
...
struct UDFLogicalVolumeDescriptor {
  ...
  void get(UDFTagId &tag /*already parsed, why parse again?*/,
           const unsigned int sz, const unsigned char *b);
  UDFLogicalVolumeDescriptor(UDFTagId &tag /*already parsed, why parse again?*/,
                             const unsigned int sz, const unsigned char *b);
...
struct UDFFileSetDescriptor {                      /* ECMA-167 4/14.1 */
  ...
  void get(UDFTagId &tag /*already parsed, why parse again?*/,
           const unsigned int sz, const unsigned char *b);
  UDFFileSetDescriptor(UDFTagId &tag /*already parsed, why parse again?*/,
                       const unsigned int sz, const unsigned char *b);
...
struct UDFFileEntry {                         /* ECMA-167 4/14.9 */
  ...
  /* NTS: ECMA-167 describes this section as clear as mud, until you finally
   * read the icbtag Flags and realize that the low 3 bits define it as an array
   * of 0=short_ad 1=long_ad 2=ext_ad 3=the file contents take place of the
   * allocation descriptors. */
  std::vector<UDFshort_ad>
      AllocationDescriptors_short_ad; /* @172+L_EA (ICBTag.Flags&7) == 0 */
  std::vector<UDFlong_ad>
      AllocationDescriptors_long_ad; /* @172+L_EA (ICBTag.Flags&7) == 1 */
  std::vector<UDFext_ad>
      AllocationDescriptors_ext_ad; /* @172+L_EA (ICBTag.Flags&7) == 2 */
  std::vector<uint8_t>
      AllocationDescriptors_file; /* @172+L_EA (ICBTag.Flags&7) == 3 */

  void get(UDFTagId &tag /*already parsed, why parse again?*/,
           const unsigned int sz, const unsigned char *b);
  UDFFileEntry(UDFTagId &tag /*already parsed, why parse again?*/,
               const unsigned int sz, const unsigned char *b);
  UDFFileEntry();
};
...
public:
  ...
  bool loadImage();
  bool loadImageUDF();
  bool loadImageUDFAnchorVolumePointer(UDFAnchorVolumeDescriptorPointer &avdp,
...
private:
  int readDirEntry(isoDirEntry *de, const uint8_t *data,
                   unsigned int direntindex) const;
  bool lookup(isoDirEntry *de, const char *path);
  bool lookup(UDFFileIdentifierDescriptor &fid, UDFFileEntry &fe,
              const char *path);
  ...
  bool is_rock_ridge = false; // NTS: Rock Ridge and System Use Sharing Protocol
                              // was detected in the root directory
  bool enable_joliet =
      false; // NTS: "Joliet" is just ISO 9660 with filenames encoded as UTF-16
             // Unicode. One of the few times Microsoft extended something yet
             // kept it simple --J.C.
  bool enable_rock_ridge =
      false; // NTS: Windows 95/98 are unlikely to support Rock Ridge, therefore
             // this is off by default. If they do support RR, let me know
             // --J.C.
...
public:
  UDFextent_ad
  convertToUDFextent_ad(const UDFshort_ad &s,
                        const uint32_t partition_ref_id = 0xFFFFFFFFu) const;
...
public:
  bool
  convertToUDFextent_ad(UDFextent_ad &d, const UDFshort_ad &s,
                        const uint32_t partition_ref_id = 0xFFFFFFFFu) const;
...
public:
  ...
  unsigned int UDFextent_read(struct UDFextents &ex, unsigned char *buf,
                              size_t count) const;
...
private:
  bool GetNextDirEntry(const int dirIteratorHandle,
                       UDFFileIdentifierDescriptor &fid, UDFFileEntry &fe,
                       UDFextents &dirext, char fname[LFN_NAMELENGTH],
...
public:
  Overlay_Drive(const char *startdir, const char *overlay,
                uint16_t _bytes_sector, uint8_t _sectors_cluster,
                uint16_t _total_clusters, uint16_t _free_clusters,
...
private:
  ...
  std::vector<std::string>
      deleted_files_in_base; // Set is probably better, or some other solution
                             // (involving the disk).
  std::vector<std::string>
      deleted_paths_in_base; // Currently only used to hide the overlay folder.
  ...
  void remove_special_file_from_disk(const char *dosname,
                                     const char *operation) override;
  bool add_special_file_to_disk(const char *dosname, const char *operation,
                                uint16_t value = 0,
                                bool isdir = false) override;
  std::string create_filename_of_special_operation(
      const char *dosname, const char *operation, bool expand = false) override;
  ...
  std::vector<std::string>
      DOSdirs_cache; // Can not blindly change its type. it is important that
                     // subdirs come after the parent directory.
  const std::string special_prefix;
};

int get_expanded_files(const std::string &path, std::vector<std::string> &paths,
                       bool readonly);

/* No LFN filefind in progress (SFN call). This index is out of range and meant
 * to indicate no LFN call in progress. */
...
#else
#define ht_stat_t                                                              \
  struct _stat64 /* WTF Microsoft?? Why aren't _stat and _wstat() consistent   \
                    on stat struct type? */
#define ht_stat(x, y) _wstat64(x, y)
#endif
