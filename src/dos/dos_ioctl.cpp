...
bool DOS_IOCTL_AX440D_CH08(uint8_t drive, bool query) {
  ...
  case 0x40: /* Set device parameters */
  {
    ...
    if (mem_readw(ptr + 0xd) == 0 && mem_readw(ptr + 0xf) == 0 &&
        mem_readw(ptr + 0x12) == 0) { // FAT32 BPB?
      bpb.v32.BPB_BytsPerSec =
          mem_readw(ptr + 7); // bytes per sector (Win3 File Mgr. uses it)
      bpb.v32.BPB_SecPerClus = mem_readb(ptr + 9); // sectors per cluster
      bpb.v32.BPB_RsvdSecCnt =
          mem_readw(ptr + 0xa);                   // number of reserved sectors
      bpb.v32.BPB_NumFATs = mem_readb(ptr + 0xc); // number of FATs
      bpb.v32.BPB_RootEntCnt = mem_readw(
          ptr +
          0xd); // number of root entries (Fake, the real BPB value is zero)
      bpb.v32.BPB_TotSec16 =
          mem_readw(ptr + 0xf); // number of small sectors (always zero on BPB
                                // and returned by Win98)
      bpb.v32.BPB_Media = mem_readb(ptr + 0x11); // media type
      bpb.v32.BPB_FATSz32 =
          (uint16_t)mem_readw(ptr + 0x12); // sectors per FAT (FIXME: What does
                                           // Win98 do if this value > 0xFFFF?)
      bpb.v32.BPB_SecPerTrk =
          (uint16_t)mem_readw(ptr + 0x14); // sectors per track
      bpb.v32.BPB_NumHeads = (uint16_t)mem_readw(ptr + 0x16); // number of heads
      bpb.v32.BPB_HiddSec =
          (uint32_t)mem_readd(ptr + 0x18); // number of hidden sectors
      bpb.v32.BPB_TotSec32 =
          (uint32_t)mem_readd(ptr + 0x1c); // number of big sectors
    } else {
  ...
  case 0x60: /* Get device parameters */
    if (query)
      break;
    {
      // mem_writeb(ptr+0,0);					// special
      // functions (call value)
      mem_writeb(ptr + 1, (drive >= 2)
                              ? 0x05
                              : 0x07); // type: hard disk(5), 1.44 floppy(7)
      mem_writew(ptr + 2, (drive >= 2)
                              ? 0x01
                              : 0x00); // attributes: bit 0 set for nonremovable
      ...
      if (usereal) {
        if (fdp->loadedDisk != NULL)
          mem_writew(ptr + 4,
                     fdp->loadedDisk->cylinders); // number of cylinders

        if (bpb.is_fat32()) {
          /* Windows 98 behavior: Some of the FAT32 BPB fields are translated
           * into FAT16 BPB fields even though those fields are zero in the
           * actual BPB */
          mem_writew(ptr + 7, bpb.v32.BPB_BytsPerSec); // bytes per sector (Win3
                                                       // File Mgr. uses it)
          mem_writeb(ptr + 9, bpb.v32.BPB_SecPerClus); // sectors per cluster
          mem_writew(ptr + 0xa,
                     bpb.v32.BPB_RsvdSecCnt); // number of reserved sectors
          mem_writeb(ptr + 0xc, bpb.v32.BPB_NumFATs); // number of FATs
          mem_writew(ptr + 0xd, 0x200); // number of root entries (Fake, the
                                        // real BPB value is zero)
          mem_writew(ptr + 0xf, 0); // number of small sectors (always zero on
                                    // BPB and returned by Win98)
          mem_writeb(ptr + 0x11, bpb.v32.BPB_Media); // media type
          mem_writew(ptr + 0x12,
                     (uint16_t)bpb.v32
                         .BPB_FATSz32); // sectors per FAT (FIXME: What does
                                        // Win98 do if this value > 0xFFFF?)
          mem_writew(ptr + 0x14,
                     (uint16_t)bpb.v32.BPB_SecPerTrk); // sectors per track
          mem_writew(ptr + 0x16,
                     (uint16_t)bpb.v32.BPB_NumHeads); // number of heads
          mem_writed(ptr + 0x18,
                     (uint32_t)bpb.v32.BPB_HiddSec); // number of hidden sectors
          mem_writed(ptr + 0x1c,
                     (uint32_t)bpb.v32.BPB_TotSec32); // number of big sectors
        } else {
          mem_writew(ptr + 7, bpb.v.BPB_BytsPerSec); // bytes per sector (Win3
                                                     // File Mgr. uses it)
          mem_writeb(ptr + 9, bpb.v.BPB_SecPerClus); // sectors per cluster
          mem_writew(ptr + 0xa,
                     bpb.v.BPB_RsvdSecCnt); // number of reserved sectors
          ...
          mem_writew(ptr + 0x12,
                     (uint16_t)bpb.v.BPB_FATSz16); // sectors per FAT
          mem_writew(ptr + 0x14,
                     (uint16_t)bpb.v.BPB_SecPerTrk); // sectors per track
          mem_writew(ptr + 0x16,
                     (uint16_t)bpb.v.BPB_NumHeads); // number of heads
          mem_writed(ptr + 0x18,
                     (uint32_t)bpb.v.BPB_HiddSec); // number of hidden sectors
          mem_writed(ptr + 0x1c,
                     (uint32_t)bpb.v.BPB_TotSec32); // number of big sectors
        }
      } else {
        mem_writew(ptr + 7,
                   0x0200); // bytes per sector (Win3 File Mgr. uses it)
        ...
        mem_writew(ptr + 0xd,
                   (drive >= 2) ? 0x0200 : 0x00E0); // number of root entries
        mem_writew(ptr + 0xf,
                   (drive >= 2) ? 0x0000 : 0x0B40); // number of small sectors
        mem_writew(ptr + 0x11, (drive >= 2) ? 0xF8 : 0xF0); // media type
        mem_writew(ptr + 0x12,
                   (drive >= 2) ? 0x00C9 : 0x0009); // sectors per FAT
        mem_writew(ptr + 0x14,
                   (drive >= 2) ? 0x003F : 0x0012);         // sectors per track
        mem_writew(ptr + 0x16, (drive >= 2) ? 0x10 : 0x02); // number of heads
        mem_writed(ptr + 0x18, 0); // number of hidden sectors
        mem_writed(ptr + 0x1c,
                   (drive >= 2) ? 0x31F11 : 0x00); // number of big sectors
      }
  ...
  case 0x42: /* Format and verify logical device track (FORMAT.COM) */
  {
     ...
     * perhaps there is a reason Linux fdisk warns about sector alignment to
     * sect/track for MS-DOS partitions? */
    {
    ...
    // STUB!
    LOG(LOG_IOCTL, LOG_DEBUG)("DOS:IOCTL Call 0D:42 Drive %2X pretending to "
                              "format device track C/H/S=%u/%u/%u ntracks=%u",
                              drive, cyl, head, sect, ntracks);
  } break;
  case 0x62: /* Verify logical device track (FORMAT.COM) */
  {
    ...
    // STUB!
    LOG(LOG_IOCTL, LOG_DEBUG)("DOS:IOCTL Call 0D:62 Drive %2X pretending to "
                              "verify device track C/H/S=%u/%u/%u ntracks=%u",
                              drive, cyl, head, sect, ntracks);
  } break;
  case 0x46: /* Set volume serial number */
      ...
      break;
    {
      ...
      unsigned long serial_number =
          mem_readd(ptr + 2) ? mem_readd(ptr + 2) : 0x1234;
      if (bpb.is_fat32())
  ...
  case 0x66: /* Get volume serial number */
      ...
      break;
    {
      ...
      if (find_ext) {
        ...
        find_ext++;
        memcpy(buffer + 8, find_ext,
               (strlen(find_ext) > 3) ? 3 : strlen(find_ext));
      } else {
  ...
  case 0x41: /* Write logical device track */
  {
    ...
    LOG(LOG_IOCTL,
        LOG_DEBUG)("DOS:IOCTL Call 0D:41 Write Logical Device Track from Drive "
                   "%2X C/H/S=%u/%u/%u num=%u from %04x:%04x sz=%u",
                   drive, cyl, head, sect, nsect, xfer_addr >> 16,
                   xfer_addr & 0xFFFF, sectsize);
    ...
    while (nsect > 0) {
      ...
      if (status != 0) {
        LOG(LOG_IOCTL, LOG_DEBUG)("IOCTL 0D:61 write error at C/H/S %u/%u/%u",
                                  cyl, head, sect);
  ...
  case 0x61: /* Read logical device track */
  {
    ...
    LOG(LOG_IOCTL,
        LOG_DEBUG)("DOS:IOCTL Call 0D:61 Read Logical Device Track from Drive "
                   "%2X C/H/S=%u/%u/%u num=%u to %04x:%04x sz=%u",
                   drive, cyl, head, sect, nsect, xfer_addr >> 16,
                   xfer_addr & 0xFFFF, sectsize);
    ...
    while (nsect > 0) {
      ...
      if (status != 0) {
        LOG(LOG_IOCTL, LOG_DEBUG)("IOCTL 0D:61 read error at C/H/S %u/%u/%u",
                                  cyl, head, sect);
        DOS_SetError(DOSERR_ACCESS_DENIED); // FIXME
        return false;
      }
  ...
  case 0x6B:
    if (query)
      break;
    LOG(LOG_IOCTL, LOG_ERROR)(
        "DOS:IOCTL Call 0D:%2X Drive %2X volume/drive locking IOCTL, faking it",
        reg_cl, drive);
    break;
  case 0x67: /* Get access flag (whether allowed by driver) */
     ...
     * Real MS-DOS might be more restrictive, especially Windows 95 which
     * requires volume locking before disk access is allowed */
    /* FDISK.EXE needs this IOCTL to determine whether it can read the partition
     * and therefore whether the "system type" is "FAT16", "FAT12", "unknown"
     * etc. */
  ...
  default:
    LOG(LOG_IOCTL,
        LOG_ERROR)("DOS:IOCTL %s %2X:%2X Drive %2X unhandled (CH=08h)",
                   query ? "Query" : "Call", reg_al, reg_cl, drive);
...
bool DOS_IOCTL_AX440D_CH48(uint8_t drive, bool query) {
      ...
      if (usereal) {
        ...
        if (bpb.is_fat32()) {
          ...
          mem_writed(ptr + 0x1c,
                     (uint32_t)bpb.v.BPB_TotSec32); // number of big sectors
          mem_writed(ptr + 0x20,
                     (uint32_t)bpb.v32.BPB_FATSz32); // sectors per FAT
  ...
  default:
    LOG(LOG_IOCTL,
        LOG_ERROR)("DOS:IOCTL Call %02X:%2X Drive %2X unhandled (CH=48h)",
                   reg_al, reg_cl, drive);
    DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
    return false;
  }
...
bool DOS_IOCTL(void) {
  ...
  if ((reg_al < 4) || (reg_al == 0x06) || (reg_al == 0x07) ||
      (reg_al == 0x0a) || (reg_al == 0x0c) || (reg_al == 0x10)) {
    ...
    if (reg_al != 0x0b) {
      drive = reg_bl;
      if ((reg_al == 0x0D || reg_al == 0x11) &&
          (reg_cl == 0x4B || reg_cl == 0x6B))
        drive = reg_bh;
      if (!drive)
  ...
  case 0x00: /* Get Device Information */
    if (Files[handle]->GetInformation() &
        DeviceInfoFlags::Device) { // Check for device
      reg_dx = Files[handle]->GetInformation() & ~EXT_DEVICE_BIT;
      // DOS copies the upper byte of device attributes in DH resulting in two
      // bits being set for a device
  ...
  case 0x02: /* Read from Device Control Channel */
    if ((Files[handle]->GetInformation() &
      ...
      uint16_t retcode = 0;
      if (((DOS_Device *)(Files[handle]))
              ->ReadFromControlChannel(bufptr, reg_cx, &retcode)) {
        reg_ax = retcode;
        return true;
      }
    }
  ...
  case 0x03: /* Write to Device Control Channel */
    if ((Files[handle]->GetInformation() &
      ...
      uint16_t retcode = 0;
      if (((DOS_Device *)(Files[handle]))
              ->WriteToControlChannel(bufptr, reg_cx, &retcode)) {
        reg_ax = retcode;
        return true;
      }
    }
  ...
  case 0x06: /* Get Input Status */
    if (Files[handle]->GetInformation() &
      ...
      LOG(LOG_IOCTL,
          LOG_NORMAL)("06:Used Get Input Status on regular file with handle %d",
                      (int)handle);
    }
    return true;
  case 0x07: /* Get Output Status */
    ...
    LOG(LOG_IOCTL, LOG_NORMAL)("07:Fakes output status is ready for handle %d",
                               (int)handle);
    reg_al = 0xff;
    return true;
  ...
  case 0x09: /* Check if block device remote */
    if ((drive >= 2) && Drives[drive]->isRemote()) {
      ...
      reg_dx = 0x0802; // Open/Close supported; 32bit access supported (any use?
                       // fixes Fable installer)
  ...
  case 0x0A: /* Is Device of Handle Remote? */
    reg_dx = 0x8000;
    LOG(LOG_IOCTL, LOG_NORMAL)("0A:Faked output: device of handle %d is remote",
                               (int)handle);
    return true;
  ...
  case 0x11: /* query generic ioctl capability */
  {
    ...
    if (Drives[drive]->isRemovable()) {
      LOG(LOG_IOCTL, LOG_DEBUG)(
          "Attempt IOCTL AX=%04x CX=%04x on removable drive", reg_ax, reg_cx);
      DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
      return false;
    }
    if (reg_ch == 0x08) {
      ...
      return DOS_IOCTL_AX440D_CH48(
          drive,
          reg_al == 0x11); // Same functions as CH=08h but for FAT32 drives
    } else {
      LOG(LOG_IOCTL, LOG_DEBUG)("Attempt IOCTL AX=%04x CX=%04x", reg_ax,
                                reg_cx);
      DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
      return false;
    }
...
bool DOS_GetSTDINStatus(void) {
  ...
  if (handle == 0xFF)
    return false;
  if (Files[handle] &&
      (Files[handle]->GetInformation() & DeviceInfoFlags::EofOnInput))
    return false;
  return true;
}
