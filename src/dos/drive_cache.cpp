...
bool isDBCSCP(), shiftjis_lead_byte(int c), isKanji1_gbk(uint8_t chr),
    filename_not_8x3(const char *n), filename_not_strict_8x3(const char *n);
...
void DOS_Drive_Cache::SetLabel(const char *vname, bool cdrom,
                               bool allowupdate) {
...
void DOS_Drive_Cache::SetBaseDir(const char *baseDir, DOS_Drive *drive) {
  ...
  uint16_t id;
  if (basePath != baseDir)
    strcpy(basePath, baseDir); /* NTS: pointer check because Valgrind says this
                                  was called with basePath == baseDir */
  this->drive = drive;
  ...
  drives[0] = basePath[0];
#if defined(WIN32)
  if (GetVolumeInformation(drives, labellocal, 256, NULL, NULL, NULL, NULL,
                           0)) {
    bool cdrom = false;
...
#if defined(WIN32)
    // What about OS/2
    if (check_last_split_char(work, len, CROSS_FILESPLIT) && (len >= 2) &&
        (work[len - 2] != ':')) {
#else
  ...
  if (pos) {
    ...
    if (index >= 0) {
      ...
      if (dir)
        for (uint32_t i = 0; i < MAX_OPENDIRS; i++) {
          if ((dirSearch[i] == dir) &&
              ((uint32_t)index <= dirSearch[i]->nextEntry))
...
void DOS_Drive_Cache::AddEntryDirOverlay(const char *path, char *sfile,
                                         bool checkExists) {
  // Get Last part...
  char file[CROSS_LEN];
  char expand[CROSS_LEN];
  ...
  // When adding a directory, the directory we want to operate inside in is the
  // above it. (which can go wrong if the directory already exists.)
  strcpy(dironly, path);
...
#endif
    *post =
        0; // TODO take care of AddEntryDIR D:\\piet) (so mount d d:\ as base)
  ...
  if (pos) {
    ...
    if (checkExists) {
      Bits index = GetLongName(
          dir,
          (char *)(!strlen(sname) || filename_not_strict_8x3(sname) ? file
                                                                    : sname));
      if (index >= 0) {
        // directory already exists, but most likely empty.
        dir = dir->fileList[index];
        if (dir->isOverlayDir && dir->fileList.empty()) {
          // maybe care about searches ? but this function should only run on
          // cache inits/refreshes. add dot entries
    ...
    if (filename_not_strict_8x3(sname))
      sname[0] = 0;
...
void DOS_Drive_Cache::DeleteEntry(const char *path, bool ignoreLastDir) {
  ...
  if (dirSearch[srchNr] && (dirSearch[srchNr]->nextEntry > 0))
...
void DOS_Drive_Cache::CacheOut(const char *path, bool ignoreLastDir) {
  ...
  // delete file objects...
  // Maybe check if it is a file and then only delete the file and possibly the
  // long name. instead of all objects in the dir.
...
bool DOS_Drive_Cache::GetShortName(const char *fullname, char *shortname) {
  ...
  std::vector<CFileInfo *>::size_type filelist_size =
      curDir->longNameList.size();
  if (GCC_UNLIKELY(filelist_size <= 0))
    return false;

  // The orgname part of the list is not sorted (shortname is)! So we can only
  // walk through it.
...
int DOS_Drive_Cache::CompareShortname(const char *compareName,
  ...
  if (cpos) {
    ...
    /*      Bits compareCount1  = (int)cpos - (int)shortName;
            ...
            Bits compareCount2  = lpos ? int(lpos)-int(compareName) :
       strlen(compareName); if (compareCount2>8) compareCount2 = 8;
...
Bitu DOS_Drive_Cache::CreateShortNameID(CFileInfo *curDir, const char *name) {
  ...
  while (low <= high) {
    Bits mid = (low + high) / 2;
    Bits res =
        CompareShortname(name, curDir->longNameList[(size_t)mid]->shortname);
    ...
    if (res > 0)
      ...
      do {
        ...
        mid++;
      } while ((Bitu)mid < curDir->longNameList.size() &&
               (CompareShortname(
                    name, curDir->longNameList[(size_t)mid]->shortname) == 0));
      break;
    }
  }
  return foundNr + 1;
...
// avoided if possible. Hence the tests in GetLongFileName
...
static Bits wine_hash_short_file_name(char *name, char *buffer) {
  ...
  for (p = name + 1, ext = NULL; p < end - 1; p++)
    if (*p == '.')
      ext = p;
  ...
  if (ext) {
    *dst++ = '.';
    for (i = 3, ext++; (i > 0) && ext < end; i--, ext++)
      *dst++ = (*ext < 0 || strchr(invalid_chars, *ext) != NULL)
                   ? '_'
                   : toupper(*ext);
  }
...
Bits DOS_Drive_Cache::GetLongName(CFileInfo *curDir, char *shortName) {
  ...
  if (strlen(shortName) < 8 || shortName[4] != '~' || shortName[5] == '.' ||
      shortName[6] == '.' || shortName[7] == '.')
    return -1; // not available
  // else it's most likely a Wine style short name ABCD~###, # = not dot (length
  // at least 8) The above test is rather strict as the following loop can be
  // really slow if filelist_size is large.
  char buff[CROSS_LEN];
  for (Bitu i = 0; i < filelist_size; i++) {
...
void DOS_Drive_Cache::CreateShortName(CFileInfo *curDir, CFileInfo *info) {
  ...
  if (pos) {
    ...
    if (strlen(pos) > 4 || (strlen(pos) == strlen(tmpName) &&
                            strcmp(tmpName, ".") && strcmp(tmpName, ".."))) {
      while (*tmpName == '.')
        tmpName++;
      createShort = true;
    }
  ...
  if (strcmp(tmpName, ".") && strcmp(tmpName, "..") &&
      filename_not_8x3(tmpName)) {
    createShort = true;
    unsigned int i = 0;
    bool lead = false;
    while (tmpName[i] != 0) {
      if ((tmpName[i] & 0xFF) <= 32 || tmpName[i] == 127 || tmpName[i] == '"' ||
          tmpName[i] == '+' || tmpName[i] == '=' || tmpName[i] == ',' ||
          tmpName[i] == ';' || tmpName[i] == ':' || tmpName[i] == '<' ||
          tmpName[i] == '>' ||
          ((tmpName[i] == '[' || tmpName[i] == ']' || tmpName[i] == '|' ||
            tmpName[i] == '\\') &&
      ...
      if (lead)
        lead = false;
      else if ((IS_PC98_ARCH && shiftjis_lead_byte(tmpName[i] & 0xFF)) ||
               (isDBCSCP() && isKanji1_gbk(tmpName[i] & 0xFF)))
        lead = true;
      i++;
    }
  }
  ...
  if (createShort) {
    ...
    if (GCC_UNLIKELY(info->shortNr > 9999999))
    ...
    if (curDir->longNameList.size() > 0) {
      if (!(strcmp(info->shortname, curDir->longNameList.back()->shortname) <
            0)) {
        ...
        for (it = curDir->longNameList.begin();
             it != curDir->longNameList.end(); ++it) {
        ...
        if (found)
          curDir->longNameList.insert(it, info);
        else
          curDir->longNameList.push_back(info);
      }
    } else {
...
DOS_Drive_Cache::CFileInfo *DOS_Drive_Cache::FindDirInfo(const char *path,
                                                         char *expandedPath) {
  // statics
  ...
  do {
    ...
    if (pos) {
      safe_strncpy(dir, start, pos - start + 1); /*errorcheck = true;*/
    } else {
      strcpy(dir, start);
    }
...
bool DOS_Drive_Cache::OpenDir(CFileInfo *dir, const char *expand,
                              uint16_t &id) {
  if (strlen(expand) >= CROSS_LEN - 1)
    return false;
  id = GetFreeID(dir);
  dirSearch[id] = dir;
...
char *DOS_Drive_Cache::CreateEntry(CFileInfo *dir, const char *name,
                                   const char *sname, bool is_directory,
                                   bool skipSort) {
  ...
  // keep list sorted (so GetLongName works correctly, used by CreateShortName
  // in this routine)
  ...
  if (!IsCachedIn(dirSearch[id])) {
    ...
    if (drive->read_directory_first(dirp, dir_name, dir_sname, is_directory)) {
      CreateEntry(dirSearch[id], dir_name, dir_sname, is_directory,
                  /*skip search*/ true);
      while (
          drive->read_directory_next(dirp, dir_name, dir_sname, is_directory)) {
        CreateEntry(dirSearch[id], dir_name, dir_sname, is_directory,
                    /*skip search*/ true);
      }
    }

    // Insertion skipped sorting, now sort in one go, which should be faster
    // when faced with many files
    std::sort(dirSearch[id]->fileList.begin(), dirSearch[id]->fileList.end(),
              SortByName);
    ...
    /*      if (!dirp) {
                ...
                char buffer[128];
                sprintf(buffer,"DIR: Caching in %s (%d
       Files)",dirPath,dirSearch[srchNr]->fileList.size()); LOG_DEBUG(buffer);
            };*/
  }
  if (SetResult(dirSearch[id], result, lresult, dirSearch[id]->nextEntry))
    return true;
  if (dirSearch[id]) {
    dirSearch[id]->id = MAX_OPENDIRS;
    dirSearch[id] = nullptr;
...
bool DOS_Drive_Cache::SetResult(CFileInfo *dir, char *&result, char *&lresult,
                                Bitu entryNr) {
...
bool DOS_Drive_Cache::FindFirst(char *path, uint16_t &id) {
  ...
  // Find a free slot.
  // If the next one isn't free, move on to the next, if none is free => reset
  // and assume the worst
  uint16_t local_findcounter = 0;
  while (local_findcounter < MAX_OPENDIRS) {
    if (!dirFindFirst[this->nextFreeFindFirst])
      break;
  ...
  if (this->nextFreeFindFirst >= MAX_OPENDIRS)
    this->nextFreeFindFirst = 0; // Increase and wrap around for the next
                                 // search.

  if (local_findcounter == MAX_OPENDIRS) { // Here is the reset from above.
    // no free slot found...
    LOG(LOG_DOSMISC,
        LOG_ERROR)("DIRCACHE: FindFirst/Next: All slots full. Resetting");
  ...
  case ALPHABETICAL:
    break;
    //      case ALPHABETICAL       :
    //      std::sort(dirFindFirst[dirFindFirstID]->fileList.begin(),
    //      dirFindFirst[dirFindFirstID]->fileList.end(), SortByName); break;
  case DIRALPHABETICAL:
    std::sort(dirFindFirst[dirFindFirstID]->fileList.begin(),
              dirFindFirst[dirFindFirstID]->fileList.end(), SortByDirName);
    break;
  case ALPHABETICALREV:
    std::sort(dirFindFirst[dirFindFirstID]->fileList.begin(),
              dirFindFirst[dirFindFirstID]->fileList.end(), SortByNameRev);
    break;
  case DIRALPHABETICALREV:
    std::sort(dirFindFirst[dirFindFirstID]->fileList.begin(),
              dirFindFirst[dirFindFirstID]->fileList.end(), SortByDirNameRev);
    break;
  case NOSORT:
    break;
  }

  //  LOG(LOG_DOSMISC,LOG_ERROR)("DIRCACHE: FindFirst : %s
  //  (ID:%02X)",path,dirFindFirstID);
  id = dirFindFirstID;
  return true;
}
...
bool DOS_Drive_Cache::FindNext(uint16_t id, char *&result, char *&lresult) {
  ...
  if (!SetResult(dirFindFirst[id], result, lresult,
                 dirFindFirst[id]->nextEntry)) {
    // free slot
