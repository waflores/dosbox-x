...
bool qmount = false;
bool nowarn = false;
bool CodePageHostToGuestUTF8(char *d /*CROSS_LEN*/,
                             const char *s /*CROSS_LEN*/),
    CodePageHostToGuestUTF16(char *d /*CROSS_LEN*/,
                             const uint16_t *s /*CROSS_LEN*/);
inline bool CodePageHostToGuestUTF16(char *d /*CROSS_LEN*/,
                                     const char16_t *s /*CROSS_LEN*/) {
  return CodePageHostToGuestUTF16(d, reinterpret_cast<const uint16_t *>(s));
}
extern bool systemmessagebox(char const *aTitle, char const *aMessage,
                             char const *aDialogType, char const *aIconType,
                             int aDefaultButton);
extern bool addovl, addipx, addne2k, prepared, inshell, usecon, uao, loadlang,
    morelen, mountfro[26], mountiro[26], resetcolor, staycolors, printfont,
    notrycp, internal_program;
extern bool clear_screen(), OpenGL_using(void), DOS_SetAnsiAttr(uint8_t attr),
    isDBCSCP();
extern int lastcp, lastmsgcp, tryconvertcp, FileDirExistCP(const char *name),
    FileDirExistUTF8(std::string &localname, const char *name);
extern uint8_t DOS_GetAnsiAttr(void);
extern int toSetCodePage(DOS_Shell *shell, int newCP, int opt);
void MSG_Init(), JFONT_Init(), InitFontHandle(), ShutFontHandle(),
    DOSBox_SetSysMenu(), Load_Language(std::string name);
void DOS_EnableDriveMenu(char drv),
    GFX_SetTitle(int32_t cycles, int frameskip, Bits timing, bool paused),
    UpdateSDLDrawTexture();
void runBoot(const char *str), runMount(const char *str),
    runImgmount(const char *str), runRescan(const char *str), show_prompt(),
    ttf_reset(void);
void getdrivezpath(std::string &path, std::string const &dirname),
    drivezRegister(std::string const &path, std::string const &dir, bool usecp),
    UpdateDefaultPrinterFont(void);
...
void WriteChar(uint16_t col, uint16_t row, uint8_t page, uint16_t chr,
...
void MOUSE::Run(void) {
  ...
  if (!Mouse_Drv) {
    if (cmd->FindExist("/u", false))
      ...
      Mouse_Drv = true;
      mainMenu.get_item("dos_mouse_enable_int33")
          .check(Mouse_Drv)
      ...
      mainMenu.get_item("dos_mouse_y_axis_reverse")
...
static void MOUSE_ProgramStart(Program **make) { *make = new MOUSE; }
...
bool SwitchLanguage(int oldcp, int newcp, bool confirm) {
  ...
  std::string langold = iterold != langcp_map.end() ? iterold->second : "",
              langnew = iternew != langcp_map.end() ? iternew->second : "";
  if (loadlang && langnew.size() &&
      strcasecmp(langold.c_str(), langnew.c_str())) {
    FILE *file = testLoadLangFile(langnew.c_str());
    if (file) {
      fclose(file);
      std::string msg =
          formatString(MSG_Get("PROGRAM_SWITCH_LANG"), newcp, langnew.c_str());
      if (!confirm || (CheckDBCSCP(oldcp) && !CheckDBCSCP(newcp)) ||
          systemmessagebox("DOSBox-X language file", msg.c_str(), "yesno",
                           "question", 2)) {
...
static const char *UnmountHelper(char umount) {
  ...
  if (Drives[i_drive]) {
    ...
    if (image && !partitionMount)
      DetachFromBios(image);
    if (cdrom)
      IDE_CDROM_Detach(i_drive);
    Drives[i_drive] = nullptr;
    DOS_EnableDriveMenu(i_drive + 'A');
    mem_writeb(Real2Phys(dos.tables.mediaid) +
                   (unsigned int)i_drive * dos.tables.dpb_size,
               0);
    ...
    if (cdrom)
      for (int drv = 0; drv < 2; drv++)
        if (Drives[drv]) {
          ...
          if (fdp && fdp->opts.mounttype == 1 &&
              toupper(umount) == fdp->el.CDROM_drive) {
...
void MountHelper(char drive, const char drive2[DOS_PATHLENGTH],
                 std::string const &drive_type) {
  ...
  temp_line = drive2;
  if (temp_line.size() > 3 && temp_line[temp_line.size() - 1] == '\\')
  ...
  if (drive_type == "CDROM") {
    ...
    newdrive = new cdromDrive(drive, temp_line.c_str(), sizes[0], bit8size,
                              sizes[2], 0, mediaid, error, options);
    std::string errmsg;
    switch (error) {
    case 0:
      errmsg = MSG_Get("MSCDEX_SUCCESS");
      break;
    case 1:
      errmsg = MSG_Get("MSCDEX_ERROR_MULTIPLE_CDROMS");
      break;
    case 2:
      errmsg = MSG_Get("MSCDEX_ERROR_NOT_SUPPORTED");
      break;
    case 3:
      errmsg = MSG_Get("MSCDEX_ERROR_PATH");
      break;
    case 4:
      errmsg = MSG_Get("MSCDEX_TOO_MANY_DRIVES");
      break;
    case 5:
      errmsg = MSG_Get("MSCDEX_LIMITED_SUPPORT");
      break;
    default:
      errmsg = MSG_Get("MSCDEX_UNKNOWN_ERROR");
      break;
    }
    if (error) {
      systemmessagebox(error == 5 ? MSG_Get("WARNING") : MSG_Get("ERROR"),
                       errmsg.c_str(), "ok", "error", 1);
      if (error != 5) {
        delete newdrive;
        return;
      }
    ...
    newdrive = new localDrive(temp_line.c_str(), sizes[0], bit8size, sizes[2],
                              sizes[3], mediaid, options);
    newdrive->readonly = mountfro[drive - 'A'];
  }

  if (!newdrive)
    E_Exit("DOS:Can't create drive");
  Drives[drive - 'A'] = newdrive;
  DOS_EnableDriveMenu(drive);
  mem_writeb(Real2Phys(dos.tables.mediaid) +
                 (drive - 'A') * dos.tables.dpb_size,
             mediaid);
  if (drive_type == "CDROM")
...
void MenuMountDrive(char drive, const char drive2[DOS_PATHLENGTH]) {
  ...
  if (control->SecureMode()) {
    systemmessagebox(MSG_Get("ERROR"),
                     MSG_Get("PROGRAM_CONFIG_SECURE_DISALLOW"), "ok", "error",
                     1);
    return;
  }
  DOS_Drive *newdrive;
  ...
  if (type == DRIVE_NO_ROOT_DIR) {
    std::string msg =
        formatString(MSG_Get("PROGRAM_DRIVE_NO_EXIST"), str.c_str());
    systemmessagebox(MSG_Get("ERROR"), msg.c_str(), "ok", "error", 1);
    return;
  } else if (type == DRIVE_CDROM)
    drive_type = MSG_Get("DRIVE_TYPE_CDROM");
  else if (type == DRIVE_REMOVABLE)
    drive_type = drive == 'A' || drive == 'B' ? MSG_Get("DRIVE_TYPE_FLOPPY")
                                              : MSG_Get("DRIVE_TYPE_REMOVABLE");
  else if (type == DRIVE_REMOTE)
    drive_type = MSG_Get("DRIVE_TYPE_NETWORK");
  else
    drive_type = MSG_Get("DRIVE_TYPE_HARD");
  drive_warn = formatString(MSG_Get("PROGRAM_DRIVE_WARN"), drive_type.c_str(),
                            str.c_str());
  if (mountwarning && !systemmessagebox(MSG_Get("WARNING"), drive_warn.c_str(),
                                        "yesno", "warning", 1))
    return;
  ...
  mem_writeb(Real2Phys(dos.tables.mediaid) +
             ...
             mediaid);
  if (type == DRIVE_CDROM)
    LOG_MSG("GUI: Drive %c is mounted as CD-ROM %c:\\", drive, drive);
  else
    LOG_MSG("GUI: Drive %c is mounted as local directory %c:\\", drive, drive);
  if (drive == drive2[0] && strlen(drive2) == 3) {
    // automatic mount
  } else {
    if (type == DRIVE_CDROM)
...
std::string GetNewStr(const char *str) {
  ...
  if (str && dos.loaded_codepage != 437) {
    ...
    wchar_t *wstr = NULL;
    int reqsize =
        MultiByteToWideChar(CP_UTF8, 0, str, (int)(strlen(str) + 1), NULL, 0);
    if (reqsize > 0 && (wstr = new wchar_t[reqsize]) &&
        MultiByteToWideChar(CP_UTF8, 0, str, (int)(strlen(str) + 1), wstr,
                            reqsize) == reqsize) {
      reqsize =
          WideCharToMultiByte(dos.loaded_codepage == 808
                                  ? 866
                                  : (dos.loaded_codepage == 859
                                         ? 858
                                         : (dos.loaded_codepage == 872
                                                ? 855
                                                : (dos.loaded_codepage == 951
                                                       ? 950
                                                       : dos.loaded_codepage))),
                              flags, wstr, -1, NULL, 0, "\x07", NULL);
      if (reqsize > 1 && (temp = new char[reqsize]) &&
          WideCharToMultiByte(dos.loaded_codepage == 808
                                  ? 866
                                  : (dos.loaded_codepage == 859
                                         ? 858
                                         : (dos.loaded_codepage == 872
                                                ...
                                                : (dos.loaded_codepage == 951
                                                       ? 950
                                                       : dos.loaded_codepage))),
                              flags, wstr, -1, (LPSTR)temp, reqsize, "\x07",
                              NULL) == reqsize)
...
void MenuBrowseCDImage(char drive, int num) {
  ...
  if (Drives[drive - 'A'] &&
      !strncmp(Drives[drive - 'A']->GetInfo(), "isoDrive ", 9)) {
#if !defined(HX_DOS)
    std::string drive_info = std::string(Drives[drive - 'A']->GetInfo() + 9);
    std::string drive_letter = dos_kernel_disabled
                                   ? std::to_string(num)
                                   : (std::string(1, drive) + ":");
    std::string drive_warn =
        formatString(MSG_Get("PROGRAM_CDDRIVE_WARN"), drive_letter.c_str(),
                     drive_info.c_str());
    if (!systemmessagebox("Change CD Image", drive_warn.c_str(), "yesno",
                          "question", 1))
      return;
#endif
  } else
  ...
  const char *lFilterPatterns[] = {
      "*.iso", "*.cue", "*.bin", "*.chd", "*.mdf", "*.gog", "*.ins", "*.inst",
      "*.ISO", "*.CUE", "*.BIN", "*.CHD", "*.MDF", "*.GOG", "*.INS", "*.INST"};
  const char *lFilterDescription = "CD image files (*.iso, *.cue, *.bin, "
                                   "*.chd, *.mdf, *.gog, *.ins, *.inst)";
  lTheOpenFileName = tinyfd_openFileDialog(
      "Select a CD image file", "",
      sizeof(lFilterPatterns) / sizeof(lFilterPatterns[0]), lFilterPatterns,
      lFilterDescription, 0);
  ...
  if (lTheOpenFileName) {
    ...
    if (cdrom && dos_kernel_disabled) {
      ...
      if (isempty) {
        newDrive =
            new isoDrive(drive, lTheOpenFileName, mediaid, error, options);
        if (error) {
          delete newDrive;
          systemmessagebox(MSG_Get("ERROR"), MSG_Get("PROGRAM_CDMOUNT_ERROR"),
                           "ok", "error", 1);
    ...
    if ((!isempty || !dos_kernel_disabled) && cdrom)
      DriveManager::ChangeDisk(drive - 'A', cdrom);
  }
  chdir(Temp_CurrentDir);
#endif
}
...
void MenuBrowseFDImage(char drive, int num, int type) {
  ...
  if (type == -1 ||
      (Drives[drive - 'A'] &&
       !strncmp(Drives[drive - 'A']->GetInfo(), "fatDrive ", 9))) {
#if !defined(HX_DOS)
    std::string image =
        type == 1 || (type == -1 && dynamic_cast<imageDiskElToritoFloppy *>(
                                        imageDiskList[drive - 'A']) != NULL)
            ? MSG_Get("ELTORITO_IMAGE")
            : (type == 2 ||
                       (type == -1 && dynamic_cast<imageDiskMemory *>(
                                          imageDiskList[drive - 'A']) != NULL)
                   ? MSG_Get("RAM_FLOPPY_IMAGE")
                   : (type == -1 ? imageDiskList[drive - 'A']->diskname.c_str()
                                 : Drives[drive - 'A']->GetInfo() + 9));
    std::string drive_warn = formatString(
        MSG_Get("PROGRAM_FLOPPY_WARN"),
        (type == -1 ? std::string(1, drive - 'A' + '0')
                    : (dos_kernel_disabled ? std::to_string(num)
                                           : std::string(1, drive) + ":"))
            .c_str(),
        image.c_str());
    if (!systemmessagebox("Change floppy disk image", drive_warn.c_str(),
                          "yesno", "question", 1))
      return;
#endif
  } else
  ...
  char const *lTheOpenFileName;
  std::string files = "", fname = "";
  const char *lFilterPatterns[] = {"*.ima", "*.img", "*.xdf", "*.fdi", "*.hdm",
                                   "*.nfd", "*.d88", "*.IMA", "*.IMG", "*.XDF",
                                   "*.FDI", "*.HDM", "*.NFD", "*.D88"};
  const char *lFilterDescription =
      "Floppy image files (*.ima, *.img, *.xdf, *.fdi, *.hdm, *.nfd, *.d88)";
  lTheOpenFileName = tinyfd_openFileDialog(
  ...
  if (lTheOpenFileName) {
    ...
    if (!newDrive->created_successfully) {
      systemmessagebox(MSG_Get("ERROR"), MSG_Get("PROGRAM_FLOPPYMOUNT_ERROR"),
                       "ok", "error", 1);
      chdir(Temp_CurrentDir);
      return;
    }
...
void MenuBrowseImageFile(char drive, bool arc, bool boot, bool multiple) {
  ...
  if (arc) {
    ...
    lTheOpenFileName = tinyfd_openFileDialog(
        ("Select an archive file for Drive " + str + ":").c_str(), "",
        sizeof(lFilterPatterns) / sizeof(lFilterPatterns[0]), lFilterPatterns,
        lFilterDescription, 0);
    if (lTheOpenFileName)
    ...
    const char *lFilterPatterns[] = {
        "*.ima", "*.img", "*.vhd",  "*.fdi", "*.hdi", "*.nfd", "*.nhd", "*.d88",
        "*.hdm", "*.xdf", "*.iso",  "*.cue", "*.bin", "*.chd", "*.mdf", "*.gog",
        "*.ins", "*.ccd", "*.inst", "*.IMA", "*.IMG", "*.VHD", "*.FDI", "*.HDI",
        "*.NFD", "*.NHD", "*.D88",  "*.HDM", "*.XDF", "*.ISO", "*.CUE", "*.BIN",
        "*.CHD", "*.MDF", "*.GOG",  "*.INS", "*.CCD", "*.INST"};
    const char *lFilterDescription = "Disk/CD image files";
    lTheOpenFileName = tinyfd_openFileDialog(
        ((multiple ? "Select image file(s) for Drive "
                   : "Select an image file for Drive ") +
         str + ":")
            .c_str(),
        "", sizeof(lFilterPatterns) / sizeof(lFilterPatterns[0]),
        lFilterPatterns, lFilterDescription, multiple ? 1 : 0);
    if (lTheOpenFileName)
      fname = "\"" + GetNewStr(lTheOpenFileName) + "\"";
    if (multiple && fname.size()) {
      files = std::regex_replace(fname, std::regex("\\|"), "\" \"");
    }
    while (multiple && lTheOpenFileName &&
           systemmessagebox("Mount image files",
                            MSG_Get("PROGRAM_MOUNT_MORE_IMAGES"), "yesno",
                            "question", 1)) {
      lTheOpenFileName = tinyfd_openFileDialog(
          ("Select image file(s) for Drive " + str + ":").c_str(), "",
          sizeof(lFilterPatterns) / sizeof(lFilterPatterns[0]), lFilterPatterns,
          lFilterDescription, multiple ? 1 : 0);
      if (lTheOpenFileName) {
        fname = "\"" + GetNewStr(lTheOpenFileName) + "\"";
        files =
            files + " " + std::regex_replace(fname, std::regex("\\|"), "\" \"");
  ...
  if (fname.size() || files.size()) {
    ...
    if (!arc && !files.size()) {
      ...
      else if ((!strcasecmp(ext, ".iso")) || (!strcasecmp(ext, ".cue")) ||
               (!strcasecmp(ext, ".bin")) || (!strcasecmp(ext, ".chd")) ||
               (!strcasecmp(ext, ".mdf")) || (!strcasecmp(ext, ".gog")) ||
               (!strcasecmp(ext, ".ins")) || (!strcasecmp(ext, ".inst")))
        strcpy(type, "-t iso ");
      else
        strcpy(type, "");
    } else
    ...
    if (files.size() > CROSS_LEN * 4) {
      systemmessagebox(MSG_Get("ERROR"), MSG_Get("PROGRAM_MOUNT_PATH_TOOLONG"),
                       "ok", "error", 1);
      return;
    }
    ...
    if (boot) {
      ...
      std::string drive_warn = formatString(MSG_Get("PROGRAM_BOOT_FAILED"),
    ...
    chdir(Temp_CurrentDir);
    if (!Drives[drive - 'A']) {
      drive_warn = formatString(MSG_Get("PROGRAM_MOUNT_FAILED"),
                                (std::string(1, drive)).c_str());
      systemmessagebox(MSG_Get("ERROR"), drive_warn.c_str(), "ok", "error", 1);
      return;
    } else if (multiple) {
      std::string readonly =
          mountiro[drive - 'A']
              ...
              : "";
      drive_warn = formatString(
          MSG_Get("PROGRAM_MOUNT_IMAGE"), MSG_Get("DISK_IMAGE"),
          std::string(1, drive).c_str(), files.c_str(), readonly.c_str());
      systemmessagebox(MSG_Get("INFORMATION"), drive_warn.c_str(), "ok", "info",
                       1);
      ...
      std::string readonly =
          mountiro[drive - 'A']
              ? "\n(" + std::string(MSG_Get("READONLY_MODE")) + ")"
              : "";
      std::string image = arc ? std::string(MSG_Get("ARCHIVE"))
                              : std::string(MSG_Get("DISK_IMAGE"));
      drive_warn = formatString(MSG_Get("PROGRAM_MOUNT_IMAGE"), image.c_str(),
                                std::string(1, drive).c_str(), lTheOpenFileName,
                                readonly.c_str());
      systemmessagebox(MSG_Get("INFORMATION"), drive_warn.c_str(), "ok", "info",
                       1);
    }
...
bool CodePageGuestToHostUTF8(char *d /*CROSS_LEN*/,
                             const char *s /*CROSS_LEN*/);
...
void MenuBrowseFolder(char drive, std::string const &drive_type) {
  ...
  if (Drives[drive - 'A']) {
    std::string drive_warn =
        formatString(MSG_Get("PROGRAM_ALREADY_MOUNTED"), str.c_str());
    systemmessagebox(MSG_Get("ERROR"), drive_warn.c_str(), "ok", "error", 1);
    return;
  }
  if (control->SecureMode()) {
  ...
  std::string title = formatString(MSG_Get("PROGRAM_MOUNT_SELECT_DRIVE"),
                                   str.c_str(), MSG_Get(msg_key.c_str()));
  if (drive_type == "CDROM")
  ...
  char const *lTheSelectFolderName =
      tinyfd_selectFolderDialog(title.c_str(), NULL);
#endif
  if (lTheSelectFolderName) {
    MountHelper(drive, GetNewStr(lTheSelectFolderName).c_str(), drive_type);
    std::string drive_warn =
        formatString(MSG_Get("PROGRAM_MOUNT_SUCCESS"),
                     std::string(1, drive).c_str(), lTheSelectFolderName);
    if (Drives[drive - 'A'])
      systemmessagebox(MSG_Get("INFORMATION"), drive_warn.c_str(), "ok", "info",
                       1);
  }
...
void MenuUnmountDrive(char drive) {
  if (!Drives[drive - 'A']) {
    ...
    std::string drive_warn = formatString(MSG_Get("PROGRAM_MOUNT_NOT_MOUNTED"),
                                          drive_letter.c_str());
    systemmessagebox(MSG_Get("ERROR"), drive_warn.c_str(), "ok", "error", 1);
    return;
  }
...
void MenuBootDrive(char drive) {
  ...
  std::string drive_letter(1, drive);
  std::string drive_warn =
      formatString(MSG_Get("PROGRAM_BOOT_FAILED"), drive_letter.c_str());
  systemmessagebox(MSG_Get("ERROR"), drive_warn.c_str(), "ok", "error", 1);
}
...
void MenuBrowseProgramFile() {
  ...
  if (strlen(psp_name) && strcmp(psp_name, "COMMAND")) {
    drive_warn = strcmp(psp_name, "4DOS") ? MSG_Get("PROGRAM_PROGRAM_ALREADY")
  ...
  if (drv == ' ') { // Fallback to C: if no free drive found
    if (!systemmessagebox(MSG_Get("WARNING"),
                          MSG_Get("PROGRAM_WARN_QUICKLAUNCH"), "yesno",
                          "question", 1)) {
      return;
    }
    drv = 'C';
  ...
  char const *lTheOpenFileName =
      tinyfd_openFileDialog("Select an executable file to launch", "", 6,
                            lFilterPatterns, lFilterDescription, 0);

  if (lTheOpenFileName) {
    const char *ext = strrchr(lTheOpenFileName, '.');
    struct stat st;
    std::string localname = lTheOpenFileName;
    std::string base = !FileDirExistCP(lTheOpenFileName) &&
                               FileDirExistUTF8(localname, lTheOpenFileName)
                           ? localname
                           : lTheOpenFileName;
    std::string full = base;
    if (stat(full.c_str(), &st) || !S_ISREG(st.st_mode)) {
      if (ext != NULL) {
        systemmessagebox(MSG_Get("ERROR"), MSG_Get("PROGRAM_NO_EXECUTABLE"),
                         "ok", "error", 1);
        return;
      }
      full = base + ".com";
    ...
    if (ext == NULL) {
      systemmessagebox(MSG_Get("ERROR"), MSG_Get("PROGRAM_NO_EXECUTABLE"), "ok",
                       "error", 1);
      return;
    }
    std::size_t found = full.find_last_of("/\\");
    std::string pathname = full.substr(0, found),
                filename = full.substr(found + 1);
    clearline = true;
...
#if defined(WIN32)
    if (pathname.size() == 2 && pathname.back() == ':')
      strcat(mountstring, "\\");
#endif
    strcat(mountstring, " \"");
    ...
    if (!Drives[drv - 'A']) {
      drive_warn = formatString(MSG_Get("PROGRAM_MOUNT_FAILED"),
                                std::string(1, drv).c_str());
      systemmessagebox(MSG_Get("ERROR"), drive_warn.c_str(), "ok", "error", 1);
      if (!dos_kernel_disabled)
        mainMenu.get_item("mapper_quickrun")
            .enable(true)
            .refresh_item(mainMenu);
      return;
    }
    ...
    char name1[DOS_PATHLENGTH + 2], name2[DOS_PATHLENGTH + 4],
        name3[DOS_PATHLENGTH + 4];
    std::string msg = "\r\nLaunching ";
    ...
    if (!exist) {
      for (int i = 0; i < 1000; i++)
        CALLBACK_Idle();
      drive_warn = formatString(MSG_Get("PROGRAM_EXEC_FINISHED"),
                                std::string(1, drv).c_str());
      if (systemmessagebox(MSG_Get("WARNING"), drive_warn.c_str(), "yesno",
                           "question", 1)) {
        if (Drives[olddrv])
          DOS_SetDefaultDrive(olddrv);
        char temp_str[3] = {0, 0, 0};
        temp_str[0] = drv;
        temp_str[1] = ' ';
        strcpy(mountstring, temp_str);
...
public:
  ...
  void Move_Z(char new_z) {
    ...
    if (i_newz >= 0 && i_newz < DOS_DRIVES) {
      ...
      tempenv += ":\\";
      std::string tempenvZ = std::string(1, 'A' + ZDRIVE_NUM);
      tempenvZ += ":\\";
      std::string tempenvz = std::string(1, 'a' + ZDRIVE_NUM);
      tempenvz += ":\\";
      ...
      if (first_shell->bf) {
        ...
        if (name.length() > 2 && name[0] == 'A' + ZDRIVE_NUM && name[1] == ':')
          name[0] = newz_drive;
      }
      /* Change the active drive */
      if (DOS_GetDefaultDrive() == ZDRIVE_NUM)
        DOS_SetDrive(i_newz);
      ZDRIVE_NUM = i_newz;
    } else
  ...
  void ListMounts(bool quiet, bool local) {
    ...
    if (!quiet) {
      ...
      WriteOut(MSG_Get("PROGRAM_MOUNT_STATUS_FORMAT"), MSG_Get("DRIVE"),
               MSG_Get("TYPE"), MSG_Get("LABEL"));
    }
    ...
    for (int d = 0; d < DOS_DRIVES; d++) {
      if (!Drives[d])
        continue;
      if (local && strncasecmp("local ", Drives[d]->GetInfo(), 6))
        continue;
      if (quiet) {
        WriteOut(MSG_Get("PROGRAM_MOUNT_STATUS_2"), 'A' + d,
                 Drives[d]->GetInfo() +
                     (local && !strncasecmp("local ", Drives[d]->GetInfo(), 6)
                          ? 16
                          : 0));
        continue;
      }
      ...
      WriteOut(MSG_Get("PROGRAM_MOUNT_STATUS_FORMAT"), root,
               Drives[d]->GetInfo(), name);
      none = false;
    }
    if (none && !quiet)
  ...
  void Run(void) override {
    ...
    if (cmd->FindExist("/examples") || cmd->FindExist("-examples")) {
      ...
      WriteOut(MSG_Get("PROGRAM_MOUNT_EXAMPLE"), "d:\\dosprogs", "d:\\dosprogs",
               "\"d:\\dos games\"", "\"d:\\dos games\"", "d:\\dosprogs",
               "d:\\dosprogs", "d:\\dosprogs", "d:\\dosprogs", "d:\\dosprogs",
               "d:\\dosprogs", "d:\\overlaydir");
#else
      WriteOut(MSG_Get("PROGRAM_MOUNT_EXAMPLE"), "~/dosprogs", "~/dosprogs",
               "\"~/dos games\"", "\"~/dos games\"", "~/dosprogs", "~/dosprogs",
               "~/dosprogs", "~/dosprogs", "~/dosprogs", "~/dosprogs",
               "~/overlaydir");
#endif
      return;
    }
    ...
    if (cmd->FindString("-u", umount, false)) {
      ...
      if (!quiet)
        WriteOut(msg, toupper(umount[0]));
      return;
    }

    /* Check for moving Z: */
    /* Only allowing moving it once. It is merely a convenience added for the
     * wine team */
    if (cmd->FindString("-z", newz, false)) {
      if (ZDRIVE_NUM != newz[0] - 'A')
        Move_Z(newz[0]);
      return;
    }
    /* Show list of cdroms */
    ...
    bool iscdrom =
        (type == "cdrom"); // Used for mscdex bug cdrom label name emulation
    ...
    if (type == "floppy" || type == "dir" || type == "cdrom" ||
      ...
      if (cmd->FindString("-freesize", mb_size, true)) {
        ...
        if (type == "floppy") {
          // freesize in kb
          sprintf(teststr, "512,1,2880,%d",
                  freesize * 1024 / (512 * 1) > 2880
                      ? 2880
                      : freesize * 1024 / (512 * 1));
        } else {
          ...
          uint32_t tmp = (uint32_t)freesize * 1024 * 1024 /
                         (type == "cdrom" ? 2048 * 1 : 512 * 32);
          if (tmp > 65534)
            numc = type == "cdrom" ? (tmp + 65535) / 65536 : 64;
          uint32_t free_size_cyl = (uint32_t)freesize * 1024 * 1024 /
                                   (numc * (type == "cdrom" ? 2048 : 512));
          ...
          sprintf(teststr, type == "cdrom" ? "2048,%u,%u,%u" : "512,%u,%u,%u",
                  numc, total_size_cyl, free_size_cyl);
        }
        str_size = teststr;
      }
      ...
      if (count < 4) {
        number[index] =
            0; // always goes correct as index is max 20 at this point.
        sizes[count] = atoi(number);
      }
      ...
      if (!cmd->FindCommand(2, temp_line)) {
        if (Drives[i_drive - 'A']) {
          ...
          if (!quiet)
            WriteOut(MSG_Get("PROGRAM_MOUNT_STATUS_2"), i_drive,
                     info +
                         (local && !strncasecmp("local ", info, 6) ? 16 : 0));
      ...
      if (cmd->FindExist("-u", true)) {
        ...
        if (!quiet)
          WriteOut(msg, toupper(i_drive));
        if (!cmd->FindCommand(2, temp_line) || !temp_line.size())
          return;
        if (curdrv && toupper(i_drive) - 'A' != DOS_GetDefaultDrive())
          removed = true;
      }
      ...
      if (type == "overlay") {
        ...
        bool found = false;
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_MOUNT_ALREADY_MOUNTED"), drive,
                   Drives[drive - 'A']->GetInfo());
        if (nextdrive)
      ...
      temp_line.erase(std::find_if(temp_line.rbegin(), temp_line.rend(),
                                   [](unsigned char ch) {
                                     return !
      ...
      if (path_relative_to_last_config && control->configfiles.size() &&
          !Cross::IsPathAbsolute(temp_line)) {
      ...
      bool is_physfs = temp_line.find(':', ((temp_line[0] | 0x20) >= 'a' &&
                                            (temp_line[0] | 0x20) <= 'z')
                                               ? 2
                                               : 0) != std::string::npos;
      struct stat test;
...
#if defined(RISCOS)
      // If the user provided a RISC OS style path, convert it to a Unix style
      // path
      // TODO: Disable UnixLib's automatic path conversion and use RISC OS style
      // paths internally?
      ...
      if (is_physfs && temp_line.size() > 4 && temp_line[0] == '\'' &&
          toupper(temp_line[1]) >= 'A' && toupper(temp_line[1]) <= 'Z' &&
          temp_line[2] == ':' &&
          (temp_line[3] == '/' || temp_line[3] == '\\') &&
          temp_line.back() == '\'') {
        temp_line = temp_line.substr(1, temp_line.size() - 2);
        is_physfs = temp_line.find(':', ((temp_line[0] | 0x20) >= 'a' &&
                                         (temp_line[0] | 0x20) <= 'z')
                                            ? 2
                                            : 0) != std::string::npos;
      } else if (is_physfs && temp_line.size() > 3 && temp_line[0] == '\'' &&
                 toupper(temp_line[1]) >= 'A' && toupper(temp_line[1]) <= 'Z' &&
                 temp_line[2] == ':' &&
                 (temp_line[3] == '/' || temp_line[3] == '\\')) {
        ...
        if (space != std::string::npos) {
          ...
          std::size_t found =
              line.back() == '\'' ? line.find_last_of('\'') : line.rfind("' ");
      ...
      if (temp_line.size() > 3 && temp_line[temp_line.size() - 1] == '\\')
        temp_line.erase(temp_line.size() - 1, 1);
      if (temp_line.size() == 2 && toupper(temp_line[0]) >= 'A' &&
          toupper(temp_line[0]) <= 'Z' && temp_line[1] == ':')
        temp_line.append("\\");
      if (temp_line.size() > 4 && temp_line[0] == '\\' &&
          temp_line[1] == '\\' && temp_line[2] != '\\' &&
          std::count(temp_line.begin() + 3, temp_line.end(), '\\') == 1)
        temp_line.append("\\");
      notrycp = true;
      ...
      if (!is_physfs && stat(temp_line.c_str(), &test)) {
#endif
#if defined(WIN32)
        if (host_name == NULL || ht_stat(host_name, &htest))
          failed = true;
        useh = true;
      } else if (!is_physfs && tryconvertcp && _waccess(host_name, 0) &&
                 dos.loaded_codepage == 437) {
        uint16_t cp = GetACP(), cpbak = dos.loaded_codepage;
#if defined(USE_TTF)
        if ((ttf.inUse && (cp < 1250 || cp > 1259) && cp != 437 &&
             isSupportedCP(cp)) ||
            (!ttf.inUse &&
...
#endif
        {
          ...
          std::string str =
              formatString(MSG_Get("PROGRAM_ASK_CHCP"), drive, cp, cp);
          if (!host_name || ht_stat(host_name, &htest) ||
              _waccess(host_name, 0) ||
              !systemmessagebox(MSG_Get("PROGRAM_CHANGING_CODEPAGE"),
                                str.c_str(), "yesno", "question", 1))
            ...
            if (OpenGL_using() && control->opt_lang.size() && lastcp &&
      ...
      if (temp_line.size() <= 2) // Seems to be a drive.
      {
        ...
        APIRET rc = DosOpen(
            (unsigned char *)temp_line.c_str(), &cdrom_fd, &ulAction, 0L,
...
#endif
        {
          ...
          /*if (!quiet) {
              ...
              if (temp_line.length()>4) {
                  char ext[5];
                  strncpy(ext, temp_line.substr(temp_line.length()-4).c_str(),
          4); ext[4]=0; if (!strcasecmp(ext, ".iso")||!strcasecmp(ext,
          ".cue")||!strcasecmp(ext, ".bin")||!strcasecmp(ext,
          ".chd")||!strcasecmp(ext, ".mdf")||!strcasecmp(ext,
          ".ima")||!strcasecmp(ext, ".img")||!strcasecmp(ext,
          ".vhd")||!strcasecmp(ext, ".hdi"))
      ...
      uint8_t bit8size = (uint8_t)sizes[1];
      exist =
          drive - 'A' < DOS_DRIVES && drive - 'A' >= 0 && Drives[drive - 'A'];
      if (type == "cdrom") {
        int num = -1;
        ...
        if (is_physfs)
          newdrive =
              new physfscdromDrive(drive, temp_line.c_str(), sizes[0], bit8size,
                                   sizes[2], 0, mediaid, error, options);
        else
          newdrive =
              new cdromDrive(drive, temp_line.c_str(), sizes[0], bit8size,
                             sizes[2], sizes[3], mediaid, error, options);
        ...
        if (!quiet)
          ...
          case 5:
            ...
            break;
          case 10:
            WriteOut(MSG_Get("PROGRAM_MOUNT_PHYSFS_ERROR"));
            WriteOut(MSG_Get("PROGRAM_MOUNT_IMGMOUNT"));
            break;
          default:
        ...
        if (is_physfs) {
          int error = 0;
          newdrive =
              new physfsDrive(drive, temp_line.c_str(), sizes[0], bit8size,
                              sizes[2], sizes[3], mediaid, error, options);
          if (error) {
            if (!quiet) {
              WriteOut(MSG_Get("PROGRAM_MOUNT_PHYSFS_ERROR"));
          ...
          physfscdromDrive *pcdp =
              dynamic_cast<physfscdromDrive *>(Drives[drive - 'A']);
          if (pdp && !pcdp) {
            if (pdp->setOverlaydir(temp_line.c_str()))
              WriteOut(MSG_Get("PROGRAM_MOUNT_OVERLAY_STATUS"),
                       (temp_line +
                        (temp_line.size() && temp_line.back() != CROSS_FILESPLIT
                             ? std::string(1, CROSS_FILESPLIT)
                             : "") +
                        std::string(1, drive) + "_DRIVE")
                           .c_str(),
                       drive);
            else
          ...
          uint8_t o_error = 0;
          newdrive = new Overlay_Drive(base.c_str(), temp_line.c_str(),
                                       sizes[0], bit8size, sizes[2], sizes[3],
                                       mediaid, o_error, options);
          ...
          if (newdrive) {
            if (o_error) {
              ...
              if (o_error == 1)
                WriteOut(MSG_Get("PROGRAM_MOUNT_OVERLAY_MIXED_BASE"));
              else if (o_error == 2)
                WriteOut(MSG_Get("PROGRAM_MOUNT_OVERLAY_SAME_AS_BASE"));
              else
                WriteOut(MSG_Get("PROGRAM_MOUNT_OVERLAY_ERROR"));
              delete newdrive;
              return;
            ...
            if (Drives[drive - 'A'] != NULL) {
              if (dynamic_cast<Overlay_Drive *>(Drives[drive - 'A']) !=
                  NULL) { /* Yeah, this relies on RTTI but it's probably worth
                             it */
                /* Let the user know in case experience with other OSes or
                 * emulators leads them to think that they can "stack" overlays
                 * by mounting multiple times. */
          ...
          newdrive = new localDrive(temp_line.c_str(), sizes[0], bit8size,
                                    sizes[2], sizes[3], mediaid, options);
    ...
    mem_writeb(Real2Phys(dos.tables.mediaid) +
                   ((unsigned int)drive - 'A') * dos.tables.dpb_size,
    ...
    if (!quiet) {
      if (type != "overlay")
        ...
        WriteOut(MSG_Get("PROGRAM_MOUNT_OVERLAY_STATUS"), temp_line.c_str(),
                 drive);
    }
...
static void MOUNT_ProgramStart(Program **make) { *make = new MOUNT; }
...
public:
  void Run(void) override {
    ...
    GUI_Run(false); /* So that I don't have to run the keymapper on every setup
                       of mine just to get the GUI --J.C */
  }
};

static void CFGTOOL_ProgramStart(Program **make) { *make = new CFGTOOL; }
...
void pc98_43d_write(Bitu port, Bitu val, Bitu iolen) {
  ...
  case 0x12: // BIOS
    ...
    break;
  default:
    LOG_MSG("PC-98 43Dh BIOS bank switching write: 0x%02x unknown value",
            (unsigned int)val);
    break;
  }
}
...
FILE *fopen_lock(const char *fname, const char *mode, bool &readonly) {
  ...
  if (lockmount && fmode.size() > 1 && fmode.back() == '+') {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
      ...
      if (host_name != NULL)
        hFile = CreateFileW(host_name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    }
    if (hFile == INVALID_HANDLE_VALUE) {
      fmode.pop_back();
    ...
    if (retfile == NULL) {
      ...
      readonly = true;
      goto next;
    } /* did you know fopen returns NULL if it cannot open the file? */
    int lock = flock(
        fileno(retfile),
        LOCK_EX | LOCK_NB); /* did you know fileno() assumes retfile != NULL and
                               you will segfault if that is wrong? */
    if (lock < 0) {
 ...
 *
 *  \description    Users will use this command to boot a guest operating system
 * from a disk image. Options are provided to specify the device to boot from
 * (if the image is already assigned) or a floppy disk image specified on the
 * command line.
 */
...
private:
   ...
   */
  FILE *getFSFile_mounted(char const *filename, uint32_t *ksize,
    ...
    *error = 0;
    ...
    if (!DOS_MakeName(readonly ? filename + 1 : filename, fullname, &drive))
      return NULL;
  ...
  /*! \brief      Open a file as a disk image and return FILE* handle and size
   */
  FILE *getFSFile(char const *filename, uint32_t *ksize, uint32_t *bsize,
                  bool tryload = false) {
    ...
    if (readonly || !tmpfile) {
      if ((tmpfile = fopen(
               readonly ? filename_s.c_str() + 1 : filename_s.c_str(), "rb"))) {
...
public:
  ...
  void Run(void) override {
    ...
    // This is copy-pasta to be consistent with the IMGMOUNT command which
    // accepts this as either -el-torito or -bootcd. But with one important
    // difference: Unlike IMGMOUNT which only supports bootable floppy
    // emulation, this version lets you specify any other mode, though at this
    // time, only "no emulation" mode is supported. You specify it as a =suffix
    // i.e. -el-torito d:=noemu. No emulation mode is default, the IMGMOUNT
    // --el-torito is still recommended for booting floppy emulation at this
    // time.
    cmd->FindString("-el-torito", el_torito, true);
    if (el_torito == "")
      cmd->FindString("-bootcd", el_torito, true);
    ...
    if (boothax_str == "msdos") // WARNING: For MS-DOS only, including MS-DOS
                                // 7/8 included in Windows 95/98/ME.
      boothax = BOOTHAX_MSDOS; // do NOT use while in the graphical interface of
                               // Windows 95/98/ME especially a DOS VM.
    else if (boothax_str == "")
      boothax = BOOTHAX_NONE;
    else {
      if (!quiet)
        WriteOut(MSG_Get("PROGRAM_BOOT_UNKNOWN_BOOTHAX"));
      return;
    }
    ...
    if (bios_boot) {
      ...
      if (bios.empty()) {
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_BOOT_SPECIFY_FILE"));
        return;
      }
      ...
      if (romfp == NULL) {
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_BOOT_BIOS_OPEN_ERROR"));
        return;
      }
      Bitu loadsz = (isz2 + 0xFU) & (~0xFU);
      if (loadsz == 0)
        loadsz = 0x10;
      if (loadsz > (IS_PC98_ARCH ? 0x18000u : 0x20000u))
        loadsz = (IS_PC98_ARCH ? 0x18000u : 0x20000u);
      Bitu segbase = 0x100000 - loadsz;
      LOG_MSG("Loading BIOS image %s to 0x%lx, 0x%lx bytes", bios.c_str(),
              (unsigned long)segbase, (unsigned long)loadsz);
      fseek(romfp, 0, SEEK_SET);

      // To avoid crashes should any interrupt be called on the way to running
      // the BIOS, don't actually map it in until it's good and ready to go.
      if (custom_bios_image != NULL)
        delete[] custom_bios_image;
      ...
      if (IS_PC98_ARCH) {
        // The PC-98 BIOS has a bank switching system where at least the last
        // 32KB can be switched to an Initial Firmware Test BIOS, which
        // initializes the system then switches back to the full 96KB visible
        // during runtime.
        //
        // We can emulate the same if a file named ITF.ROM exists in the same
        // directory as the BIOS image we were given.
        //
        // To enable multiple ITFs per ROM image, we first try <bios
        // filename>.itf.rom before trying itf.rom, for the user's convenience.
        FILE *itffp;
        ...
        if (itffp == NULL)
          itffp = getFSFile((bios + ".ITF.ROM").c_str(), &isz1, &isz2);
        if (itffp == NULL)
          itffp = getFSFile("itf.rom", &isz1, &isz2);
        if (itffp == NULL)
          itffp = getFSFile("ITF.ROM", &isz1, &isz2);

        if (itffp != NULL && isz2 <= 0x8000ul) {
          LOG_MSG("Found ITF (initial firmware test) BIOS image (0x%lx bytes)",
                  (unsigned long)isz2);
     ...
     *    SS:SP = 0030:00D8
     *
     * Reportedly PC-98 will load to 1FE0:0000 when booting the 1.44MB format
     * (512 bytes per sector). Note that 0x1FC0:0000 leaves enough room for the
     * 1024 bytes per sector format of PC-98.
     */
    ...
    if (!el_torito.empty()) {
      // get el-torito floppy from cdrom mounted at drive letter
      // el_torito_cd_drive
       ...
       * For floppy emulation boot, use IMGMOUNT and then boot the emulated
       * floppy drive. */
      ...
      unsigned long el_torito_base = 0, boot_record_sector = 0,
                    el_torito_rba = (~0ul), el_torito_load_segment = 0,
                    el_torito_sectors = 0 /*VIRTUAL SECTORS*/;
      unsigned char el_torito_mediatype = 0;
      if (!ElTorito_ScanForBootRecord(src_drive, boot_record_sector,
                                      el_torito_base)) {
      ...
      LOG_MSG("El Torito emulation: Found ISO 9660 Boot Record in sector %lu, "
              "pointing to sector %lu\n",
      ...
      for (ent_num = 0; ent_num < (2048 / 0x20); ent_num++) {
        ...
        if (entry[0] == 0x01 /*header*/) {
          if (!ElTorito_ChecksumRecord(entry)) {
            LOG_MSG(
                "Warning: El Torito checksum error in header(0x01) entry\n");
            continue;
          }

          if (header_count != 0) {
            LOG_MSG("Warning: El Torito has more than one Header/validation "
                    "entry\n");
            continue;
          }

          if (header_final) {
            LOG_MSG("Warning: El Torito has an additional header past the "
                    "final header\n");
            continue;
          }
          ...
          LOG_MSG("El Torito entry: first header platform=0x%02x\n",
                  header_platform);
          header_count++;
        } else if (entry[0] == 0x90 /*header, more follows*/ ||
          ...
          header_more = (int)(((unsigned int)entry[2]) +
                              (((unsigned int)entry[3]) << 8u));
          header_platform = entry[1];
          LOG_MSG("El Torito entry: first header platform=0x%02x more=%u "
                  "final=%u\n",
                  header_platform, header_more, header_final);
          header_count++;
        } else {
          if (header_more == 0) {
            LOG_MSG("El Torito entry: Non-header entry count expired, ignoring "
                    "record 0x%02x\n",
                    entry[0]);
            continue;
          ...
          if (entry[0] == 0x44) {
            ...
            if (header_platform == 0x00 /*x86*/) {
              ...
              unsigned short load_segment =
                  ((unsigned int)entry[2]) + (((unsigned int)entry[3]) << 8);
              unsigned char system_type = entry[4];
              unsigned short sector_count =
                  ((unsigned int)entry[6]) + (((unsigned int)entry[7]) << 8);
              unsigned long load_rba = ((unsigned int)entry[8]) +
                                       (((unsigned int)entry[9]) << 8) +
                                       (((unsigned int)entry[10]) << 16) +
                                       (((unsigned int)entry[11]) << 24);

              LOG_MSG("El Torito entry: bootable x86 record mediatype=%u "
                      "load_segment=0x%04x "
                      "system_type=0x%02x sector_count=%u load_rba=%lu\n",
                      mediatype, load_segment, system_type, sector_count,
                      load_rba);
              ...
              if ((mediatype == 0 && el_torito_mode == "noemu")) {
                ...
                el_torito_load_segment =
                    (load_segment != 0) ? load_segment : 0x7C0;
                el_torito_sectors = sector_count; /* VIRTUAL EMULATED sectors
                                                     not CD-ROM SECTORS */
              }
            } else {
              LOG_MSG("El Torito entry: ignoring bootable non-x86 "
                      "(platform_id=0x%02x) record\n",
                      header_platform);
            }
          } else {
      ...
      LOG_MSG("Using: rba=%lu virt-sectors=%lu load=0x%lx mediatype=%u",
              (unsigned long)el_torito_rba, (unsigned long)el_torito_sectors,
              (unsigned long)el_torito_load_segment, el_torito_mediatype);

      load_seg = el_torito_load_segment;

      /* round up to CD-ROM sectors and read */
      unsigned int bootcdsect = (el_torito_sectors + 3u) /
                                4u; /* 4 512-byte sectors per CD-ROM sector */
      if (bootcdsect == 0)
        bootcdsect = 1;
      ...
      for (unsigned int s = 0; s < bootcdsect; s++) {
        ...
        for (i = 0; i < 2048; i++)
          real_writeb((uint16_t)load_seg, (uint16_t)i + (s * 2048), entries[i]);
      }
      ...
      /* this is required if INT 13h extensions are to correctly report what IDE
       * controller the drive is connected to and master/slave */
      {
        int x = IDE_MatchCDROMDrive(el_torito_cd_drive);
        if (x >= 0)
          INT13_ElTorito_IDEInterface = (char)x;
        LOG_MSG("CD-ROM drive IDE interface number %d",
                INT13_ElTorito_IDEInterface);
      }
      ...
      reg_ebp = 0;
      reg_eax = 0;
      /* ISOLINUX clearly assumes DL at entry contains the drive number and at
       * no point from entry to INT 13h does it change the contents of DX */
      ...
      /* forcibly exit the shell, the DOS kernel, and anything else by throwing
       * an exception */
      throw int(2);
    ...
    if (!cmd->GetCount()) {
      ...
      if (drv < 4 && Drives[drv] &&
          !strncmp(Drives[drv]->GetInfo(), "fatDrive ", 9)) {
        drive = 'A' + drv;
        bootbyDrive = true;
      } else {
        printError();
      ...
      while (i < cmd->GetCount()) {
        if (cmd->FindCommand((unsigned int)(i + 1), temp_line)) {
          ...
          if ((temp_line == "-e") || (temp_line == "-E")) {
            ...
            i++;
            if (cmd->FindCommand((unsigned int)(i + 1), temp_line)) {
              for (size_t ct = 0; ct < temp_line.size(); ct++)
                temp_line[ct] = toupper(temp_line[ct]);
              cart_cmd = temp_line;
            } else {
              printError();
              return;
          ...
          bool readonly =
              wpcolon && temp_line.length() > 1 && temp_line[0] == ':';
          if (!quiet)
            WriteOut(MSG_Get("PROGRAM_BOOT_IMAGE_OPEN"),
                     readonly ? temp_line.c_str() + 1 : temp_line.c_str());
          FILE *usefile =
              getFSFile(temp_line.c_str(), &floppysize, &rombytesize);
          if (usefile != NULL) {
            char tmp[256];
            ...
            if (ext != NULL && !strcasecmp(ext, ".d88")) {
              newDiskSwap[i] =
                  new imageDiskD88(usefile, fname, floppysize, false);
            } else if (!memcmp(tmp, "VFD1.", 5)) { /* FDD files */
              newDiskSwap[i] =
                  new imageDiskVFD(usefile, fname, floppysize, false);
            } else if (!memcmp(tmp, "T98FDDIMAGE.R0\0\0", 16)) {
              newDiskSwap[i] =
                  new imageDiskNFD(usefile, fname, floppysize, false, 0);
            } else if (!memcmp(tmp, "T98FDDIMAGE.R1\0\0", 16)) {
              newDiskSwap[i] =
                  new imageDiskNFD(usefile, fname, floppysize, false, 1);
            } else {
              newDiskSwap[i] = new imageDisk(usefile, fname, floppysize, false);
            }
            newDiskSwap[i]->Addref();
            if (newDiskSwap[i]->active && !newDiskSwap[i]->hardDrive)
              incrementFDD(); // moved from imageDisk constructor
    ...
    if (!bootbyDrive) {
      if (i == 0) {
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_BOOT_NOT_SPECIFIED"));
        return;
      }

      if (i > 1) {
        /* if more than one image is given, then this drive becomes the focus of
         * the swaplist */
        if (swapInDisksSpecificDrive >= 0 &&
            swapInDisksSpecificDrive != (drive - 65)) {
          if (!quiet)
            WriteOut(MSG_Get("PROGRAM_BOOT_SWAP_ALREADY"));
          return;
    ...
    //
    // If the user is REALLY REALLY SURE they want to try executing Z80
    // bootsector code as x86, they're free to use --force.
    //
    ...
    if (!force && imageDiskList[drive - 65]->class_id == imageDisk::ID_D88) {
      if (reinterpret_cast<imageDiskD88 *>(imageDiskList[drive - 65])
              ->fd_type_major == imageDiskD88::DISKTYPE_2D) {
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_BOOT_IS_PC88"));
        return;
      }
    }
    ...
    if (imageDiskList[drive - 65]->getSectSize() > sizeof(bootarea)) {
      if (!quiet)
        WriteOut(MSG_Get("PROGRAM_BOOT_BPS_TOOLARGE"));
      return;
    }

    /* clear the disk change flag.
     * Most OSes don't expect the disk change error signal when they first boot
     * up */
    ...
    if (!has_read && IS_PC98_ARCH && drive < 'C') {
      /* this may be one of those odd FDD images where track 0, head 0 is all
       * 128-byte sectors and the rest of the disk is 256-byte sectors. */
      if (imageDiskList[drive - 65]->Read_Sector(0, 0, 1, (uint8_t *)&bootarea,
                                                 128) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 2, (uint8_t *)&bootarea + 128, 128) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 3, (uint8_t *)&bootarea + 256, 128) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 4, (uint8_t *)&bootarea + 384, 128) == 0) {
        LOG_MSG("First sector is 128 byte/sector. Booting from first four "
                "sectors.");
        has_read = true;
    ...
    if (!has_read && IS_PC98_ARCH && drive < 'C') {
      /* another nonstandard one with track 0 having 256 bytes/sector while the
       * rest have 1024 bytes/sector */
      if (imageDiskList[drive - 65]->Read_Sector(0, 0, 1, (uint8_t *)&bootarea,
                                                 256) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 2, (uint8_t *)&bootarea + 256, 256) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 3, (uint8_t *)&bootarea + 512, 256) == 0 &&
          imageDiskList[drive - 65]->Read_Sector(
              0, 0, 4, (uint8_t *)&bootarea + 768, 256) == 0) {
        LOG_MSG(
            "First sector is 256 byte/sector. Booting from first two sectors.");
        has_read = true;
    ...
    if (loadseg_user >
        0) { /* Some PC-98 games have floppy boot code that suggests the boot
                segment isn't always 0x1FC0 like PC-9821 hardware does? */
      load_seg = (unsigned int)loadseg_user;
    } else {
      unsigned int max_seg =
          std::min((unsigned int)(MEM_ConventionalPages() *
                                  (4096u / 16u) /*pages to paragraphs*/),
                   0xC000u);
      if (IS_PC98_ARCH)
        load_seg = std::min(max_seg, 0x2000u /*128KB mark*/) -
                   (bootsize / 16U); /* normally 0x1FC0 (1024 byte/sector) or
                                        0x1FE0 (512 byte/sector) */
      else
        load_seg = std::min(max_seg, 0x800u /*32KB mark*/) -
                   0x40u /*1KB*/; /* normally 0x07C0 */
    }

    if (!has_read) {
      if (imageDiskList[drive - 65]->Read_Sector(0, 0, 1,
                                                 (uint8_t *)&bootarea) != 0) {
        if (!quiet)
          WriteOut(MSG_Get("PROGRAM_BOOT_DRIVE_READERROR"));
        return;
      }
    }
    ...
    } else if ((bootarea.rawdata[56] == 0x50) &&
               (bootarea.rawdata[57] == 0x43) &&
               (bootarea.rawdata[58] == 0x4a) &&
               (bootarea.rawdata[59] == 0x52)) {
      pcjr_hdr_type = 2; // PCJRCart
      pcjr_hdr_length = 0x80;
    }

    if (pcjr_hdr_type > 0) {
      if (machine != MCH_PCJR && !quiet)
        ...
        if (tfile != NULL) {
          ...
          if (drd == 0xb000) {
            for (i = 0; i < 0xb000; i++)
              phys_writeb((PhysPt)(0xf3000 + i), rombuf[i]);
          }
          fclose(tfile);
        }
        ...
        if (usefile_2 != NULL) {
          ...
          // fclose(usefile_2); //usefile_2 is in diskSwap structure which
          // should be deleted to close the file

          /* write cartridge data into ROM */
          for (i = 0; i < rombytesize_2 - pcjr_hdr_length; i++)
            phys_writeb((PhysPt)((romseg_pt << 4) + i), rombuf[i]);
        }
        ...
        // fclose(usefile_1); //usefile_1 is in diskSwap structure which should
        // be deleted to close the file
        /* write cartridge data into ROM */
        for (i = 0; i < rombytesize_1 - pcjr_hdr_length; i++)
          phys_writeb((PhysPt)((romseg << 4) + i), rombuf[i]);
      ...
      if (max_seg < (IS_PC98_ARCH ? 0x2000 : 0x0800))
        LOG(LOG_MISC, LOG_WARN)("Booting a guest OS with too small amount of "
                                "RAM may not work correctly");

      /* Other versions of MS-DOS/PC-DOS have their own requirements about
       * memory:
       *    - IBM PC-DOS 1.0/1.1: not too picky, will boot with as little as
       * 32KB even though it was intended for the average model with 64KB of
       * RAM.
       *
       *    - IBM PC-DOS 2.1: requires at least 44KB of RAM. will crash on boot
       * otherwise.
       *
       *    - MS-DOS 3.2: requires at least 64KB to boot without crashing, 80KB
       * to make it to the command line without halting at "configuration too
       * big for memory"*/

      /* TODO: Need a configuration option or a BOOT command option where the
       * user can dictate where we put the stack: if we put it at 0x7000 or top
       * of memory (default) or just below the boot sector, or... */
      ...
      if (!dos_kernel_disabled &&
          (convimg == 1 || (convertimg && convimg == -1))) {
        unsigned int drv = 2, nextdrv = 2;
        Section_prop *sec =
            static_cast<Section_prop *>(control->GetSection("dosbox"));
        int freeMB = sec->Get_int("convert fat free space"),
            timeout = sec->Get_int("convert fat timeout");
        for (unsigned int d = 2; d < DOS_DRIVES + 2; d++) {
          if (d == DOS_DRIVES)
            drv = 0;
          else if (d == DOS_DRIVES + 1)
            drv = 1;
          else
            drv = d;
          if (Drives[drv] && strncmp(Drives[drv]->GetInfo(), "fatDrive ", 9) &&
              strncmp(Drives[drv]->GetInfo(), "isoDrive ", 9)) {
            if (drv == ZDRIVE_NUM &&
                !static_cast<Section_prop *>(control->GetSection("dos"))
                     ->Get_bool("drive z convert fat"))
              continue;
            while (imageDiskList[nextdrv])
              nextdrv++;
            if (nextdrv >= MAX_DISK_IMAGES)
              break;
            if (quiet < 2) {
            ...
            imageDisk *imagedrv = new imageDisk(
                Drives[drv], drv,
                (convertro || Drives[drv]->readonly || (od && od->ovlreadonly))
                    ? 0
                    : freeMB,
                timeout);
      ...
      for (i = 0; i < bootsize; i++)
        real_writeb((uint16_t)load_seg, (uint16_t)i, bootarea.rawdata[i]);
      ...
      /* NTS: IBM PC and PC-98 both use DMA channel 2 for the floppy, though
       * according to Neko Project II source code, DMA 3 is used for the double
       * density drives (but we don't emulate that yet) */
      ...
      if (IS_PC98_ARCH) {
         ...
         *
         * load_seg at 0x1FE0 which on the original 128KB PC-98 puts it at the
         * top of memory
         *
         */
        ...
        /* It seems 640x200 8-color digital mode is the state of the graphics
         * hardware when the BIOS boots the OS, and some games like Ys II assume
         * the hardware is in this state.
         *
         * If I am wrong, you can pass --pc98-640x400 as a command line option
         * to disable this. */
        if (pc98_640x200) {
        ...
        /* Some HDI images of Orange House games need this option because it
         * assumes NEC MOUSE.COM behavior where mouse driver init and reset show
         * the graphics layer. Unfortunately the HDI image uses QMOUSE which
         * does not show the graphics layer. Use this option with those HDI
         * images to make them playable anyway. */
        if (pc98_show_graphics) {
          reg_eax = 0x4000; // show graphics
          CALLBACK_RunRealInt(0x18);
        } else {
          reg_eax =
              0x4100; // hide graphics (normal state of graphics layer on
                      // startup). INT 33h emulation might have enabled it.
          CALLBACK_RunRealInt(0x18);
        }

        /* PC-98 MS-DOS boot sector behavior suggests that the BIOS does a CALL
         * FAR to the boot sector, and the boot sector can RETF back to the BIOS
         * on failure. */
        ...
        uint8_t RDISK_EQUIP = 0; /* 488h (ref. PC-9800 Series Technical Data
                                    Book - BIOS 1992 page 233 */
        /* bits [7:4] = 640KB FD drives 3:0
         * bits [3:0] = 1MB FD drives 3:0 */
        uint8_t F2HD_MODE = 0; /* 493h (ref. PC-9800 Series Technical Data Book
                                  - BIOS 1992 page 233 */
        /* bits [7:4] = 640KB FD drives 3:0 ??
         * bits [3:0] = 1MB FD drives 3:0 ?? */
        uint8_t F2DD_MODE = 0; /* 5CAh (ref. PC-9800 Series Technical Data Book
                                  - BIOS 1992 page 233 */
        /* bits [7:4] = 640KB FD drives 3:0 ??
         * bits [3:0] = 1MB FD drives 3:0 ?? */
        ...
        for (i = 0; i < 2; i++) {
          if (imageDiskList[i] != NULL) {
            disk_equip |=
                (0x0111u
                 << i); /* 320KB[15:12] 1MB[11:8] 640KB[7:4] unit[1:0] */
            disk_equip_144 |= (1u << i);
            F2HD_MODE |= (0x11u << i);
          }
        }
        ...
        for (i = 0; i < 2; i++) {
          if (imageDiskList[i + 2] != NULL) {
            ...
            mem_writew(m + 2u, (cyls & 0xFFFu) +
                                   (ssize == 512u ? 0x1000u : 0u) +
                                   (ssize == 1024u ? 0x2000u : 0) +
                                   0x8000u /*NP2:hwsec*/);
          }
        }
        ...
        mem_writew(
            0x5AE,
            disk_equip_144); /* disk equipment (drive 0 is present, 1.44MB) */
        ...
        } else if ((ssize == 1024 && heads == 2 && cyls == 77 && sects == 8) ||
                   pc98_sect128) {
          mem_writeb(0x584,
                     0x90 /*type*/ + (drive - 65) /*drive*/); /* 1.2MB 3-mode */
        } else if (ssize == 512 && heads == 2 && cyls == 80 && sects == 18) {
          mem_writeb(0x584,
                     0x30 /*type*/ + (drive - 65) /*drive*/); /* 1.44MB */
        } else {
          // FIXME
          LOG_MSG("PC-98 boot: Unable to determine boot drive type for "
                  "ssize=%u heads=%u cyls=%u sects=%u. Guessing.",
                  ssize, heads, cyls, sects);

          mem_writeb(0x584, (ssize < 1024 ? 0x30 : 0x90) /*type*/ +
                                (drive - 65) /*drive*/);
        }
      } else {
        // Toshiba DOS bootloader checks the floppy disk drives running in the
        // BIOS working area.
        if (IS_J3100) {
        ...
        reg_ebx =
            (uint32_t)(load_seg
                       << 4); // Real code probably uses bx to load the image
        reg_esp = 0x100;
        /* set up stack at a safe place */
        SegSet16(ss, (uint16_t)stack_seg);
        reg_esi = 0;
...
static void BOOT_ProgramStart(Program **make) { *make = new BOOT; }
    ...
    try {
      ...
      if (rom_base) {
        ...
        for (Bitu i = 0; i < data_read; i++)
          phys_writeb((PhysPt)(rom_base + i), rom_buffer[i]);
...
static void LOADROM_ProgramStart(Program **make) { *make = new LOADROM; }
  ...
  void Run(void) override {
    ...
    try {
      ...
      uint32_t rom_base = PhysMake(0xf000, 0); // override regular dosbox bios
                                               /* write buffer into ROM */
      for (Bitu i = 0; i < data_read; i++)
        phys_writeb((PhysPt)(rom_base + i), buffer[i]);
...
static void BIOSTEST_ProgramStart(Program **make) { *make = new BIOSTEST; }
#endif
...
unsigned char this_is_not_a_bootable_partition[512] = {
    ...
    0x00, 0x00, 0x29, 0xce, 0xf2, 0x4e, 0xda, 0x20, 0x20, 0x20, 0x20, 0x20,
...
const uint8_t freedos_mbr[] = {
    0xFA, 0xFC, 0x31, 0xC0, 0x8E, 0xD0, 0x8E, 0xD8, 0xBD, 0x00, 0x7C,
    0x8D, 0x66, 0xE0, 0xFB, 0xB8, 0xE0, 0x1F, 0x8E, 0xC0, 0x89, 0xEE,
    0x89, 0xEF, 0xB9, 0x00, 0x01, 0xF3, 0xA5, 0xEA, 0x22, 0x7C, // 10h
    0xE0, 0x1F, 0x8E, 0xD8, 0x8E, 0xD0, 0x31, 0xC0, 0x8E, 0xC0, 0x8D,
    0xBE, 0xBE, 0x01, 0xF6, 0x05, // 20h
    0x80, 0x75, 0x6D, 0x83, 0xC7, 0x10, 0x81, 0xFF, 0xFE, 0x7D, 0x72,
    0xF2, 0xE8, 0xC4, 0x00, 0x6E, 0x6F, 0x20, 0x61, 0x63, 0x74, 0x69,
    0x76, 0x65, 0x20, 0x70, 0x61, 0x72, 0x74, 0x69, 0x74, 0x69, // 40h
    0x6F, 0x6E, 0x20, 0x66, 0x6F, 0x75, 0x6E, 0x64, 0x00, 0xEB, 0xFE,
    0xE8, 0xA5, 0x00, 0x72, 0x65, 0x61, 0x64, 0x20, 0x65, 0x72, 0x72,
    0x6F, 0x72, 0x20, 0x77, 0x68, 0x69, 0x6C, 0x65, 0x20, 0x72, 0x65,
    0x61, 0x64, 0x69, 0x6E, 0x67, 0x20, 0x64, 0x72, 0x69, 0x76, 0x65,
    0x00, 0xEB, 0xDA, 0xE8, 0x81, 0x00, 0x70, 0x61, 0x72, 0x74, 0x69,
    0x74, 0x69, 0x6F, 0x6E, 0x20, 0x73, 0x69, 0x67, 0x6E, // 80h
    , // 100h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, // 110h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, // 120h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 140h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 180h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x73, 0x73, 0x65, 0xE3,
    0x3F, 0xBF, 0x00, 0x00, // 1B0h
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void lba2chs3(unsigned char *b3, uint32_t lba, const unsigned int gc,
                     const unsigned int gh, const unsigned int gs) {
  const unsigned int s = (unsigned int)(lba % (uint32_t)gs) + 1u;
  lba /= (uint32_t)gs;
  const unsigned int h = (unsigned int)(lba % (uint32_t)gh);
  lba /= (uint32_t)gh;
...
public:
  ...
  bool StartReadDisk(HANDLE f, OVERLAPPED *o, uint8_t *buffer, Bitu offset,
                     Bitu size) const {
  ...
  // 0=still waiting, 1=catastrophic failure, 2=success, 3=sector not found,
  // 4=crc error
  Bitu CheckDiskReadComplete(HANDLE f, OVERLAPPED *o) const {
    ...
    if (b)
      ...
      if (error == ERROR_IO_INCOMPLETE)
        return 0;
      if (error == ERROR_FLOPPY_UNKNOWN_ERROR)
        return 5;
      if (error == ERROR_CRC)
        return 4;
  ...
  Bitu ReadDisk(FILE *f, uint8_t driveletter, Bitu retries_max) {
    ...
    uint8_t drivestring[] = "\\\\.\\x:";
    drivestring[4] = driveletter;
    if (!OpenDisk(&hFloppy, &o, drivestring))
      return false;

    // get drive geom
    DeviceIoControl(hFloppy, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &geom,
                    sizeof(DISK_GEOMETRY), &numret, NULL);
    ...
    WriteOut(MSG_Get("PROGRAM_IMGMAKE_FLREAD"), geom.Cylinders.LowPart,
             geom.TracksPerCylinder, geom.SectorsPerTrack,
             (cyln_size * geom.Cylinders.LowPart) / 1024);
    ...
    for (Bitu i = 0; i < geom.Cylinders.LowPart; i++) {
      ...
      case 1:
        ...
        return false;
      case 2: // success
        for (Bitu m = 0; m < cyln_size / 512; m++)
          WriteOut("\xdb");
        break;
      case 3:
      case 4: // data errors
      case 5:
        ...
        restart_int:
          StartReadDisk(hFloppy, &o, &data[512 * k], cyln_size * i + 512 * k,
                        512);
          do {
            result = CheckDiskReadComplete(hFloppy, &o);
            CALLBACK_Idle();
          ...
          case 5:
            ...
            if (retries)
              goto restart_int;
            const uint8_t badfood[] =
                "IMGMAKE BAD FLOPPY SECTOR \xBA\xAD\xF0\x0D";
  ...
  void Run(void) override {
    ...
    uint16_t alignment = 0; // if nonzero, try to align structures to this
                            // multiple of K (i.e. 4K sector alignment)
    ...
    /*
            ...
            // when only a filename is passed try to create the file on the
       current DOS path
            // if directory+filename are passed first see if directory is a host
       path, if not
    ...
    */
    ...
    if (cmd->FindString("-source", src, true)) {
      ...
      if (!dos_kernel_disabled && getcwd(dirCur, 512) != NULL &&
          (!strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(), "local ", 6) ||
           !strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(), "CDRom ", 6))) {
        Overlay_Drive *ddp =
            dynamic_cast<Overlay_Drive *>(Drives[DOS_GetDefaultDrive()]);
        strcpy(dirNew, ddp != NULL
                           ? ddp->getOverlaydir()
                           : Drives[DOS_GetDefaultDrive()]->GetBaseDir());
    ...
    uint8_t mediadesc =
        0xF8; // media descriptor byte; also used to differ fd and hd
    uint16_t root_ent = 512; // FAT root directory entries: 512 is for harddisks
    uint16_t disksize = 0;   // disk size of floppy images
    uint32_t sect_per_fat = 0; // allowable range: FAT12 1-12, FAT16 16-256,
                               // FAT32 512-2,097,152 sectors
    ...
    if (disktype == "fd_160") {
      ...
      root_ent = 64;
      sect_per_fat = 2;
      sectors_per_cluster = 2; // root_ent?
      disksize = 200;
      is_fd = true;
      ...
      disksize = 1200;
      is_fd = true;
    } else if (disktype == "fd_1440" || disktype == "fd" ||
               disktype == "floppy") {
      c = 80;
      h = 2;
      s = 18;
      ...
      if (!(cmd->FindString("-size", isize, true))) {
        ...
        if (disktype == "vhd" &&
            size > 2040 * 1024 * 1024 * 1024ull) { /*2040GB*/
          // wrong size
          WriteOut(MSG_Get("PROGRAM_IMGMAKE_BADSIZE"));
          return;
        }
        ...
        if (size > 0xFFFFFFFFull /*4GB*/) {
          /* beyond that point it's easier to just map like LBA and be done with
           * it */
          h = 255;
          s = 63;
          c = sectors / (h * s);
          ...
          h = 2;
          while (h * 1023 * 63 < sectors)
            h <<= 1;
          if (h > 255)
            h = 255;
          s = 8;
          while (h * s * 1023 < sectors)
            s *= 2;
          if (s > 63)
            s = 63;
          c = sectors / (h * s);
    ...
    if (cmd->FindExist("-chs", true))
      ...
      lbamode =
          size >
          (2048ULL
           << 20ULL); /* automatically choose LBA for 2GB or larger images */
    ...
    char dirCur[512], dirNew[512];
    if (!dos_kernel_disabled && getcwd(dirCur, 512) != NULL &&
        !strncmp(Drives[DOS_GetDefaultDrive()]->GetInfo(), "local directory",
                 15)) {
      Overlay_Drive *ddp =
          dynamic_cast<Overlay_Drive *>(Drives[DOS_GetDefaultDrive()]);
      strcpy(dirNew, ddp != NULL ? ddp->getOverlaydir()
                                 : Drives[DOS_GetDefaultDrive()]->GetBaseDir());
      strcat(dirNew, Drives[DOS_GetDefaultDrive()]->curdir);
      if (chdir(dirNew) == 0)
        setdir = true;
    ...
    if (!pref_stat(dirname((char *)homepath.c_str()), &test) &&
        test.st_mode & S_IFDIR)
      temp_line = homedir;
#endif
    FILE *f;
    imageDiskVHD *vhd;
    ...
    char extension[6] =
        {}; // care extensions longer than 3 letters such as '.vhdd'
    if (temp_line.find_last_of('.') != std::string::npos) {
      for (unsigned int i = 0; i < sizeof(extension) - 1; i++) {
        if (temp_line.find_last_of('.') + i > temp_line.length() - 1)
          break;
    ...
    if (!dos_kernel_disabled)
      WriteOut(MSG_Get("PROGRAM_IMGMAKE_PRINT_CHS"), temp_line.c_str(), c, h,
               s);
    ...
    if (disktype == "vhd") {
      ...
      if (imageDiskVHD::Open(temp_line.c_str(), false, (imageDisk **)&vhd) !=
          imageDiskVHD::OPEN_SUCCESS) {
        WriteOut(MSG_Get("PROGRAM_VHDMAKE_ERROPEN"), temp_line.c_str());
        return;
      }
    } else {
    ...
    if (bootsect_pos > -1) {
      uint32_t reserved_sectors =
          1; /* 1 for the boot sector + BPB. FAT32 will require more */
       ...
       * Does not apply to FAT32, which makes the root directory an allocation
       * chain like any other directory/file. */
      ...
      if (is_fd)
        ...
        bool dosver_fat32 = (dos.version.major >= 8) ||
                            (dos.version.major == 7 && dos.version.minor >= 10);

        if (vol_sectors >= 2 * 1024 * 1024 * 1024ull / 512ull &&
            !dosver_fat32) /* 2GB or larger */
          FAT = 32;
        else if (vol_sectors >= 512 * 1024 * 1024ull / 512ull &&
                 dosver_fat32) /* 512MB or larger */
          FAT = 32;
        else if (vol_sectors >=
                 12 * 1024 * 1024ull / 512ull) /* 12MB or larger */
          FAT = 16;
        else
          FAT = 12;
      }
      ...
      if (mediadesc == 0xF8) {
        ...
        // OS indicator
        if (FAT < 32 &&
            (bootsect_pos + vol_sectors) < 65536) { /* 32MB or smaller */
          if (FAT >= 16)
      ...
      if (sectors_per_cluster == 0) {
        sectors_per_cluster = 1;
        /* one sector per cluster on anything larger than 200KB is a bit
         * wasteful (large FAT tables). Improve capacity by starting from a
         * larger value.*/
        if (vol_sectors >= 400) {
           ...
           * will crash if sectors/cluster is too small. Ref:
           * [https://github.com/joncampbell123/dosbox-x/issues/1553#issuecomment-651880604]
          ...
          case 12:
            tmp_fatlimit =
                ((((vol_sectors / 20u) * (512u / fat_copies)) / 3u) * 2u) + 2u;
            break;
          case 16:
            tmp_fatlimit =
                (((vol_sectors / 20u) * (512u / fat_copies)) / 2u) + 2u;
            break;
          case 32:
            tmp_fatlimit =
                (((vol_sectors / 20u) * (512u / fat_copies)) / 4u) + 2u;
            break;
          default:
            abort();
          }

          while ((vol_sectors / sectors_per_cluster) >= (tmp_fatlimit - 2u) &&
                 sectors_per_cluster < 0x80u)
            sectors_per_cluster <<= 1;
        }
      }
      ...
      if (bitop::ispowerof2(alignment)) {
        while (sectors_per_cluster < 0x80u && sectors_per_cluster < alignment)
          sectors_per_cluster <<= 1;
      }

      /* FAT12/FAT16 size the root directory so that it is aligned, which then
       * aligns the data area following it */
      if (alignment != 0u && FAT < 32) {
        const uint32_t t_alignment =
            alignment * (512u / 32u); /* 512 bytes/sector, 32 bytes per dirent =
                                         number of root dirents per sector */
        root_ent += t_alignment - 1;
        root_ent -= root_ent % t_alignment;
      }

      while (!is_fd && (vol_sectors / sectors_per_cluster) >= (fatlimit - 2u) &&
             sectors_per_cluster < 0x80u)
        sectors_per_cluster <<= 1;
      ...
      if (FAT < 32)
        host_writew(&sbuf[RootEntCnt], root_ent);
      // sectors (under 32MB) if not FAT32 and less than 65536
      if (FAT < 32 && vol_sectors < 65536ul)
        host_writew(&sbuf[TotSec16], vol_sectors);
      // sectors (32MB or larger or FAT32)
      if (FAT >= 32 || vol_sectors >= 65536ul)
        host_writed(&sbuf[TotSec32], vol_sectors);
      else
        host_writed(&sbuf[TotSec32], 0);
      // media descriptor
      ...
      // sectors per FAT
      if (FAT >= 32)
        sect_per_fat = ((clusters * 4u) + 511u) / 512u;
      else if (FAT >= 16)
        sect_per_fat = ((clusters * 2u) + 511u) / 512u;
      // Use standard sect_per_fat values for standard floppy images, otherwise
      // it may required to be adjusted
      else if (!is_fd || sect_per_fat == 0 ||
               (is_fd && (rootent_changed || (fat_copies != 2))))
        sect_per_fat = ((((clusters + 1u) / 2u) * 3u) + 511u) / 512u;
      ...
      if (alignment != 0u) {
        if (alignment % fat_copies) {
          ...
          const uint32_t t_alignment =
              alignment / fat_copies; /* we could align sect_per_fat based on
                                         ALL copies of the FAT table */
      ...
      if (alignment != 0u) {
        if ((((uint64_t)sect_per_fat * (uint64_t)fat_copies) %
      ...
      uint64_t data_area =
          vol_sectors - reserved_sectors - (sect_per_fat * fat_copies);
      if (FAT < 32) {
      ...
      if (FAT < 32)
        host_writew(&sbuf[FATSz16], (uint16_t)sect_per_fat);
      ...
      /* Too many or to few clusters can foul up FAT12/FAT16/FAT32 detection and
       * cause corruption! */
      ...
      if ((clusters + 2u) > fatlimit) {
        ...
        /* Well, if the user wants an oversized partition, hack the total
         * sectors fields to make it work */
        uint32_t adj_vol_sectors =
            (uint32_t)(reserved_sectors + (sect_per_fat * fat_copies) +
                       (((root_ent * 32u) + 511u) / 512u) +
                       (clusters * sectors_per_cluster));

        // sectors (under 32MB) if not FAT32 and less than 65536
        if (adj_vol_sectors < 65536ul)
          host_writew(&sbuf[TotSec16], (uint16_t)adj_vol_sectors);
        // sectors (32MB or larger or FAT32)
        if (adj_vol_sectors >= 65536ul)
          host_writed(&sbuf[TotSec32], adj_vol_sectors);
        else
          host_writed(&sbuf[TotSec32], 0);
      }
      ...
      /* do not validate alignment of data area, because some of the fd and hd
       * presets might violate it, but certainly make sure that the first
       * cluster is aligned */
      if (alignment != 0u) {
      ...
      if (FAT >= 32) {
        ...
        sbuf[0x28] = 0x00; // FAT is mirrored at runtime because that is what
                           // DOSBox-X's FAT driver does
        host_writew(&sbuf[0x2A], 0x0000); // FAT32 version 0.0
        host_writed(&sbuf[0x2C], 2);      // root directory starting cluster
        host_writew(&sbuf[0x30],
                    1); // sector number in reserved area of FSINFO structure
        host_writew(&sbuf[0x32],
                    6); // sector number in reserved area of backup boot sector
        // BIOS drive
        ...
        if (is_fd) {
          ...
          while (DiskGeometryList[index].cylcount != 0) {
            if (DiskGeometryList[index].ksize == disksize) {
              ...
              host_writew(&sbuf[BytsPerSec],
                          DiskGeometryList[index].bytespersect);
              sbuf[SecPerClus] = spc_changed
                                     ? sectors_per_cluster
                                     : DiskGeometryList[index].sectcluster;
              host_writew(&sbuf[RootEntCnt],
                          rootent_changed
                              ? root_ent
                              : DiskGeometryList[index].rootentries);
        ...
        // file system type
        if (FAT >= 16)
          sprintf((char *)&sbuf[FilSysType], "FAT16   ");
        else
          sprintf((char *)&sbuf[FilSysType], "FAT12   ");
      }
      // boot sector signature
      host_writew(&sbuf[BootSign], 0xAA55);

      // if anything should try to boot this partition, add code to print an
      // error message instead of letting the CPU run wild through not
      // executable code.
      if (FAT >= 32) {
        // the code expects to load a string from a fixed address.
        // we're relocating it to make room for FAT32 structures so some
        // patching is required.
        memcpy(sbuf + BootCode32, this_is_not_a_bootable_partition + 0x3E,
               BootSign - BootCode32);
        host_writew(sbuf + 0x5D, 0x7C77); // 0x7C5D: MOV SI,<stringaddr> we are
                                          // patching the <stringaddr>
      } else {
        memcpy(sbuf + BootCode, this_is_not_a_bootable_partition + 0x3E,
               BootSign - BootCode);
      }
      ...
      if (FAT >= 32) {
        ...
        host_writed(
            &sbuf[0x1e8],
            (uint32_t)(clusters - 1)); /* Last known free cluster count */
        host_writed(&sbuf[0x1ec], 3);  /* Next free cluster. We used 2 for the
                                          root dir, so 3 is next */
      ...
      for (unsigned int fat = 0; fat < fat_copies; fat++) {
        if (disktype != "vhd") {
          fseeko64(
              f,
              (off_t)(((unsigned long long)bootsect_pos + reserved_sectors +
                       (unsigned long long)sect_per_fat *
                           (unsigned long long)fat) *
                      512ull),
              SEEK_SET);
          fwrite(&sbuf, 512, 1, f);
        } else {
          vhd->Write_AbsoluteSector(
              (unsigned long long)bootsect_pos + reserved_sectors +
                  (unsigned long long)sect_per_fat * (unsigned long long)fat,
              sbuf);
        }
      }
      ...
      if (!label.empty()) {
         ...
         * FAT32: This points at the root directory only because this code
         * constructed the root directory allocation chain starting at cluster
         * 2, which is the first cluster of the data area. */
        sec =
            (unsigned long long)bootsect_pos + reserved_sectors +
            ((unsigned long long)sect_per_fat * (unsigned long long)fat_copies);
    ...
    if ((mediadesc == 0xF8) && disktype != "vhd" &&
        !strcasecmp(extension, ".vhd")) {
...
static void IMGMAKE_ProgramStart(Program **make) { *make = new IMGMAKE; }
...
public:
  void ListImgSwaps(void) {
    ...
    WriteOut(MSG_Get("PROGRAM_IMGMOUNT_STATUS_FORMAT"), MSG_Get("DRIVE"),
             MSG_Get("TYPE"), MSG_Get("LABEL"), MSG_Get("SWAP_SLOT"));
    int cols = IS_PC98_ARCH ? 80 : real_readw(BIOSMEM_SEG, BIOSMEM_NB_COLS);
    if (!cols)
      cols = 80;
    for (int p = 1; p < cols; p++)
    ...
    for (int d = 0; d < DOS_DRIVES; d++) {
      if (!Drives[d] ||
          (strncmp(Drives[d]->GetInfo(), "fatDrive ", 9) &&
           strncmp(Drives[d]->GetInfo(), "isoDrive ", 9)) ||
          (int)DriveManager::GetDisksSize(d) < 2)
        continue;
      char root[7] = {(char)('A' + d), ':', '\\', '*', '.', '*', 0};
      bool ret = DOS_FindFirst(root, DOS_ATTR_VOLUME);
      if (ret) {
      ...
      root[1] = 0; // This way, the format string can be reused.
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_STATUS_FORMAT"), root,
               Drives[d]->GetInfo(), name, DriveManager::GetDrivePosition(d));
      none = false;
    }
    if (none)
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_STATUS_NONE"));
  ...
  void Run() override {
    ...
    if (cmd->FindExist("/?", true) || cmd->FindExist("-?", true) ||
        cmd->FindExist("?", true)) {
      resetcolor = true;
    ...
    if (!cmd->FindCommand(1, temp_line) || (temp_line.size() > 2) ||
        ((temp_line.size() > 1) && (temp_line[1] != ':')) ||
        (!(temp_line[0] >= 'A' && temp_line[0] <= 'Z') &&
         !(temp_line[0] >= 'a' && temp_line[0] <= 'z'))) {
      WriteOut(MSG_Get("SHELL_ILLEGAL_DRIVE"));
      return;
    }
    int d =
        temp_line[0] - (temp_line[0] >= 'a' && temp_line[0] <= 'z' ? 'a' : 'A');
    if (!Drives[d] ||
        (strncmp(Drives[d]->GetInfo(), "fatDrive ", 9) &&
         strncmp(Drives[d]->GetInfo(), "isoDrive ", 9)) ||
        (int)DriveManager::GetDisksSize(d) < 2) {
    ...
    if (cmd->FindCommand(2, temp_line)) {
      ...
      if (swap < 1 || swap > DriveManager::GetDisksSize(d)) {
        WriteOut(MSG_Get("PROGRAM_IMGSWAP_ERROR"),
                 DriveManager::GetDisksSize(d));
        return;
      }
      swapInDrive(d, swap);
    } else
...
void IMGSWAP_ProgramStart(Program **make) { *make = new IMGSWAP; }
...
uint8_t EMM_AllocateMemory(uint16_t pages, uint16_t &dhandle,
                           bool can_allocate_zpages);
...
void LOADFIX::Run(void) {
  ...
  if (cmd->GetCount() == 1 &&
      (cmd->FindExist("-?", false) || cmd->FindExist("/?", false))) {
    resetcolor = true;
  ...
  if (cmd->FindCommand(commandNr, temp_line)) {
    if (temp_line[0] == '-' || (temp_line[0] == '/')) {
      ...
      if ((*upcase(&ch) == 'D') || (*upcase(&ch) == 'F')) {
        // Deallocate all
        if (ems) {
          for (auto i = LOADFIX_ems_handles.begin();
               i != LOADFIX_ems_handles.end(); i++) {
          ...
          for (auto i = LOADFIX_xms_handles.begin();
               i != LOADFIX_xms_handles.end(); i++) {
            if (XMS_FreeMemory(*i))
  ...
  if (ems) {
    if (EMS_Active()) {
      ...
      err = EMM_AllocateMemory((uint16_t)(kb / 16u) /*16KB pages*/,
                               /*&*/ handle, false);
      if (err == 0) {
    ...
    if (DOS_AllocateMemory(&segment, &blocks)) {
      ...
      if (cmd->FindCommand(commandNr++, temp_line)) {
        ...
        while (found) {
          if (strlen(args) + temp_line.length() + 1 > 256)
            break;
...
static void LOADFIX_ProgramStart(Program **make) { *make = new LOADFIX; }
...
void RESCAN::Run(void) {
  ...
  if (cmd->FindCommand(1, temp_line)) {
    //-A -All /A /All
    if (temp_line.size() >= 2 && (temp_line[0] == '-' || temp_line[0] == '/') &&
  ...
  if (all) {
    ...
    if (drive < DOS_DRIVES && Drives[drive]) {
      ...
      WriteOut(MSG_Get("SHELL_EXECUTE_DRIVE_NOT_FOUND"), 'A' + drive);
  }
}

static void RESCAN_ProgramStart(Program **make) { *make = new RESCAN; }
...
public:
  void DisplayMount(void) {
    ...
    if (machine == MCH_PC98) {
#if (WIN32)
      WriteOut("\033[44;"
               "1m\x86\x52\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x56\n"
      ...
      WriteOut("  \033[37m  \033[44;1m\x86\x46\033[44;1m\n"
               "\033[44;"
               "1m\x86\x5A\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               ...
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x5E\033[0m\n");
#else
      WriteOut("\033[44;"
               "1m\x86\x52\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               ...
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x56\n"
               "\x86\x46 ");
      ...
      WriteOut("  \033[37m  \x86\x46\n"
               "\x86\x5A\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               ...
               "\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44"
               "\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86\x44\x86"
               "\x44\x86\x44\x86\x44\x86\x5E\033[0m\n");
#endif
    } else {
#if (WIN32)
      WriteOut("\033[44;"
               "1m\xC9\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
      ...
      WriteOut(MSG_Get("PROGRAM_INTRO_MOUNT_EXEN_WINDOWS"));
      WriteOut("\xBA\n"
               "\xC8\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xBC\033[0m\n");
#else
      WriteOut("\033[44;"
               "1m\xC9\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
      ...
      WriteOut("\xBA\n"
               ...
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD\xCD"
               "\xCD\xCD\xCD\xCD\xCD\xBC\033[0m\n");
#endif
    }
  ...
  void DisplayIntro(void) {
    ...
    if (machine == MCH_PC98)
      WriteOut(
          "\033[0m\n"
          "\033[44m\033[K\033[1m\033[1m "
          "\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86"
          "\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43"
          "\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86"
          "\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43"
          "\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86"
          "\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43\x86\x43"
      ...
      WriteOut(
          ...
          "\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4"
          "\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4"
          "\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\xC4\033[0m\n"
          "\033[44m\033[K\033[0m\n");
  }
  void DisplayMenuBefore(void) {
    WriteOut("\033[44m\033[K\033[33m\033[1m   \033[0m ");
  }
  ...
  void Run(void) override {
    ...
    /* Only run if called from the first shell (Xcom TFTD runs any intro file in
     * the path) */
    if (!cmd->FindExist("-run", true) && !cmd->FindExist("/run", true) &&
        DOS_PSP(dos.psp()).GetParent() !=
            DOS_PSP(DOS_PSP(dos.psp()).GetParent()).GetParent())
      return;
...
imageDiskMemory *CreateRamDrive(Bitu sizes[], const int reserved_cylinders,
  ...
  if (sizes[1] == 0) {
    ...
    if (dsk == NULL) {
      ...
      if (forceFloppy) {
        if (obj != NULL)
          obj->WriteOut(MSG_Get("PROGRAM_IMGMOUNT_INVALID_FLOPPYSIZE"));
        return NULL;
      }

      // The fatDrive class is hard-coded to assume that disks 2880KB or smaller
      // are floppies,
      //   whether or not they are attached to a floppy controller.  So, let's
      //   enforce a minimum size of 4096kb for hard drives.  Use the other
      //   constructor for floppy drives.
    ...
    if (dsk == NULL) {
      ...
      dsk = new imageDiskMemory((uint16_t)sizes[3], (uint16_t)sizes[2],
                                (uint16_t)sizes[1], (uint16_t)sizes[0]);
    }
  }
  if (!dsk->active) {
    if (obj != NULL)
      obj->WriteOut(MSG_Get("PROGRAM_IMGMOUNT_CANT_CREATE"));
    delete dsk;
    return NULL;
  }
...
bool AttachToBiosByIndex(imageDisk *image,
                         const unsigned char bios_drive_index) {
  if (bios_drive_index >= MAX_DISK_IMAGES)
    return false;
  if (imageDiskList[bios_drive_index] != NULL) {
    /* Notify IDE ATA emulation if a drive is already there */
    if (bios_drive_index >= 2)
      IDE_Hard_Disk_Detach(bios_drive_index);
...
bool AttachToBiosAndIdeByIndex(imageDisk *image,
                               const unsigned char bios_drive_index,
                               const unsigned char ide_index,
                               const bool ide_slave) {
  if (!AttachToBiosByIndex(image, bios_drive_index))
    return false;
...
bool AttachToBiosByLetter(imageDisk *image, const char drive) {
  if (image->hardDrive) {
    ...
    LOG_MSG("BIOS: Warning: Four hard drives (Disk no. 2-5) attached to BIOS "
            "already. Drive %c not attached",
            drive);
  }
#if 0
    ...
    // for PCs, mount floppies only if A: or B: is specified, and then if so, at
    // specified index
...
bool AttachToBiosAndIdeByLetter(imageDisk *image, const char drive,
                                const unsigned char ide_index,
                                const bool ide_slave) {
  if (image->hardDrive) {
    // for hard drives, mount hard drives at first available index
...
public:
  ...
  void ListImgMounts(void) {
    ...
    bool none = true;
    for (int d = 0; d < DOS_DRIVES; d++) {
      if (!Drives[d] || (strncmp(Drives[d]->GetInfo(), "fatDrive ", 9) &&
                         strncmp(Drives[d]->GetInfo(), "isoDrive ", 9)))
        continue;
      char root[7] = {(char)('A' + d), ':', '\\', '*', '.', '*', 0};
      bool ret = DOS_FindFirst(root, DOS_ATTR_VOLUME);
    ...
    WriteOut(MSG_Get("PROGRAM_IMGMOUNT_STATUS_NUMBER_FORMAT"),
             MSG_Get("DRIVE_NUMBER"), MSG_Get("DISK_NAME"),
             MSG_Get("IDE_POSITION"), MSG_Get("SWAP_SLOT"));
    for (int p = 1; p < cols; p++)
      WriteOut("-");
    WriteOut("\n");
    none = true;
    for (int index = 0; index < MAX_DISK_IMAGES; index++) {
      if (imageDiskList[index]) {
        ...
        WriteOut(MSG_Get("PROGRAM_IMGMOUNT_STATUS_NUMBER_FORMAT"),
                 ...
                 dynamic_cast<imageDiskElToritoFloppy *>(
                     imageDiskList[index]) != NULL
                     ? "El Torito floppy drive"
                     : imageDiskList[index]->diskname.c_str(),
                 GetIDEPosition(index).c_str(), swapstr);
        none = false;
      }
    }
    if (none)
  ...
  void Run(void) override {
    ...
    // show help if /? or -?
    if (cmd->FindExist("/?", true) || cmd->FindExist("-?", true) ||
        cmd->FindExist("?", true) || cmd->FindExist("-help", true)) {
      resetcolor = true;
    ...
    // this code simply sets default type to "floppy" if mounting at A: or B:
    // --- nothing else
    //  get first parameter - which is probably the drive letter to mount at
    //  (A-Z or A:-Z:) - and check it if is A or B or A: or B: default to floppy
    //  for drive letters A and B and numbers 0 and 1
    if (!cmd->FindCommand(1, temp_line) || (temp_line.size() > 2) ||
        ((temp_line.size() > 1) && (temp_line[1] != ':'))) {
      // drive not valid
    } else {
    ...
    if (temp_line.size() == 1 && isdigit(temp_line[0]) && temp_line[0] >= '0' &&
        temp_line[0] < MAX_DISK_IMAGES + '0' && cmd->FindExist("-u", true)) {
      Unmount(temp_line[0]);
      std::string templine;
      if (!cmd->FindCommand(2, templine) || !templine.size())
        return;
    }
    ...
    if (el_torito != "") {
      ...
      // validate the el_torito loading (look for boot image on the cdrom, etc),
      // and
      //   find the el_torito_floppy_base and el_torito_floppy_type values
      if (!PrepElTorito(type, el_torito_cd_drive, el_torito_floppy_base,
                        el_torito_floppy_type))
        return;
    }

    // the user can use -bd to mount partitions from an INT 13h BIOS disk
    // mounted image, meaning a disk image attached to INT 13h using IMGMOUNT
    // <number> -fs none. This way, it is possible to mount multiple partitions
    // from one HDD image.
    cmd->FindString("-bd", bdisk, true);
    if (bdisk != "") {
      bdisk_number = atoi(bdisk.c_str());
      if (bdisk_number < 0 || bdisk_number >= MAX_DISK_IMAGES)
        return;
    ...
    /* DOSBox-X: to please certain 32-bit drivers like Windows 3.1 WDCTRL, or to
     * emulate older h/w configurations, we allow the user or script to specify
     * the number of reserved cylinders. older BIOSes were known to subtract 1
     * or 2 additional cylinders from the total in the fixed disk param table.
     * the -reservecyl option allows the number we subtract from the total in
     * INT 13H to be set */
    ...
    std::transform(ideattach.begin(), ideattach.end(), ideattach.begin(),
                   ::tolower);

    if (isdigit(tdr) &&
        tdr - '0' >=
            2) { // Allocate to respective slots if drive number is specified
      ide_index =
          (tdr - '2') / 2; // Drive number 2 = 1m (index=0, slave=false), 3 = 1s
                           // (index=0, slave=true), ...
      ...
      if (type != "floppy") {
        if (type == "iso") {
          if (!IDE_controller_occupied(1,
                                       false)) { // CD-ROMS default to secondary
                                                 // master if not occupied
            ide_index = 1;
            ide_slave = false;
          }
        }
    ...
    if (type == "floppy") {
      ...
      // str_size=="2048,1,60000,0";   // ignored, see drive_iso.cpp
      // (AllocationInfo)
      fstype = "iso";
    }
    ...
    if (!ReadSizeParameter(str_size, str_chs, type, sizes))
      return;

    if (!rfstype && isdigit(temp_line[0]))
      fstype = "none";
    ...
    if (fstype == "fat" || fstype == "iso") {
      ...
      /* if fs type if "fat" and we're asked to mount *: then check for that
       * here */
      if (temp_line[0] == '*' && temp_line[1] == ':') {
        /* What drives are available? */
        int i_drive = IS_PC98_ARCH ? 'A' : 'C';
        while (i_drive <= 'Z' && Drives[i_drive - 'A'] != NULL &&
               (i_drive - 'A') < DOS_DRIVES)
          i_drive++;
    ...
    // find all file parameters, assuming that all option parameters have been
    // removed
    bool removed = ParseFiles(temp_line, paths,
                              el_torito != "" || type == "ram" || bdisk != "");
    ...
    if (el_torito != "") {
      ...
      if (paths.size() == 0) {
        if (strcasecmp(temp_line.c_str(), "-u") && !qmount)
      ...
      if (!rtype && !rfstype && fstype != "none" && paths[0].length() > 4) {
        ...
        if (ext != NULL) {
          if (!strcasecmp(ext, ".iso") || !strcasecmp(ext, ".cue") ||
              !strcasecmp(ext, ".bin") || !strcasecmp(ext, ".chd") ||
              !strcasecmp(ext, ".mdf") || !strcasecmp(ext, ".gog") ||
              !strcasecmp(ext, ".ins") || !strcasecmp(ext, ".ccd") ||
              !strcasecmp(ext, ".inst")) {
            type = "iso";
            fstype = "iso";
            if (ide_index < 0 || ideattach == "auto") {
              if (!IDE_controller_occupied(
                      1,
                      false)) { // check if secondary master is already occupied
                ide_index = 1;
                ide_slave = false;
              } else
                IDE_Auto(ide_index, ide_slave);
    ...
    if (fstype == "fat") {
      ...
      if (bdisk != "") {
        if (!MountPartitionFat(drive, bdisk_number))
          return;
      } else if (el_torito != "") {
        if (!MountElToritoFat(drive, sizes, el_torito_cd_drive,
                              el_torito_floppy_base, el_torito_floppy_type))
          return;
      } else if (type == "ram") {
        if (!MountRam(sizes, drive, ide_index, ide_slave, roflag))
          return;
      } else {
        // supports multiple files
        if (!MountFat(sizes, drive, type == "hdd", str_size, paths, ide_index,
                      ide_slave, reserved_cylinders, roflag))
          return;
      }
      if (removed && !exist && i_drive < DOS_DRIVES && i_drive >= 0 &&
          Drives[i_drive])
        DOS_SetDefaultDrive(i_drive);
    } else if (fstype == "iso") {
      if (bdisk != "") {
        // TODO
        return;
      ...
      if (el_torito != "") {
        WriteOut(MSG_Get("PROGRAM_ELTORITO_ISOMOUNT")); /* <- NTS: Will never
                                                           implement, either */
        return;
      }
      // supports multiple files
      if (!MountIso(drive, paths, ide_index, ide_slave))
        return;
      if (removed && !exist && i_drive < DOS_DRIVES && i_drive >= 0 &&
          Drives[i_drive])
        DOS_SetDefaultDrive(i_drive);
      ...
      if (el_torito != "") {
        newImage = new imageDiskElToritoFloppy(
            (unsigned char)el_torito_cd_drive, el_torito_floppy_base,
            el_torito_floppy_type);
      } else if (type == "ram") {
        newImage = MountImageNoneRam(sizes, reserved_cylinders, driveIndex < 2);
      } else {
        newImage = MountImageNone(paths[0].c_str(), NULL, sizes,
                                  reserved_cylinders, roflag);
      }
      if (newImage == NULL)
        return;
      newImage->Addref();
      if (newImage->hardDrive && (driveIndex < 2)) {
        ...
        if (AttachToBiosAndIdeByIndex(newImage, (unsigned char)driveIndex,
                                      (unsigned char)ide_index, ide_slave)) {
          WriteOut(MSG_Get("PROGRAM_IMGMOUNT_MOUNT_NUMBER"), drive - '0',
                   (!paths.empty())
                       ? (wpcolon && paths[0].length() > 1 &&
                                  paths[0].c_str()[0] == ':'
                              ? paths[0].c_str() + 1
                              : paths[0].c_str())
                       : (el_torito != ""
          ...
          const char *ext = strrchr(paths[0].c_str(), '.');
          if (ext != NULL) {
            if ((!IS_PC98_ARCH && strcasecmp(ext, ".img") &&
                 strcasecmp(ext, ".ima") && strcasecmp(ext, ".vhd") &&
                 strcasecmp(ext, ".qcow2")) ||
                (IS_PC98_ARCH && strcasecmp(ext, ".hdi") &&
                 strcasecmp(ext, ".nhd") && strcasecmp(ext, ".img") &&
                 strcasecmp(ext, ".ima"))) {
              WriteOut(MSG_Get("PROGRAM_MOUNT_UNSUPPORTED_EXT"), ext);
            }
          }
          if (swapInDisksSpecificDrive == driveIndex ||
              swapInDisksSpecificDrive == -1) {
            for (size_t si = 0; si < MAX_SWAPPABLE_DISKS; si++) {
              if (diskSwap[si] != NULL) {
                diskSwap[si]->Release();
            ...
            if (paths.size() > 1) {
              ...
              for (size_t si = 1; si < MAX_SWAPPABLE_DISKS && si < paths.size();
                   si++) {
                imageDisk *img = MountImageNone(paths[si].c_str(), NULL, sizes,
                                                reserved_cylinders, roflag);
...
private:
  bool ReadSizeParameter(const std::string &str_size,
                         const std::string &str_chs, const std::string &type,
                         Bitu sizes[]) {
    bool isCHS = false;
    const char *scan;
    if (str_chs.size() != 0) {
    ...
    if (isCHS) {
      if (count == 3)
        sizes[count++] = 512; // set sector size automatically
      if (count != 4) {
        WriteOut(MSG_Get("PROGRAM_IMGMOUNT_INVALID_SIZE"));
        return false;
      }
  ...
  bool ParseFiles(std::string &commandLine, std::vector<std::string> &paths,
                  bool nodef) {
    char drive = commandLine[0];
    bool nocont = false;
    int num = 0;
    while (!nocont && cmd->FindCommand((unsigned int)(paths.size() + 1 - num),
                                       commandLine)) {
      bool usedef = false;
      if (!cmd->FindCommand((unsigned int)(paths.size() + 2 - num),
                            commandLine) ||
          !commandLine.size()) {
        if (!nodef && !paths.size()) {
...
#if defined(WIN32) || defined(OS2)
      // Windows: Workaround for LaunchBox
      if (commandLine.size() > 4 && commandLine[0] == '\'' &&
          toupper(commandLine[1]) >= 'A' && toupper(commandLine[1]) <= 'Z' &&
          commandLine[2] == ':' &&
          (commandLine[3] == '/' || commandLine[3] == '\\') &&
          commandLine.back() == '\'')
        commandLine = commandLine.substr(1, commandLine.size() - 2);
      else if (!paths.size() && commandLine.size() > 3 &&
               commandLine[0] == '\'' && toupper(commandLine[1]) >= 'A' &&
               toupper(commandLine[1]) <= 'Z' && commandLine[2] == ':' &&
               (commandLine[3] == '/' || commandLine[3] == '\\')) {
        ...
        if (space != std::string::npos) {
          ...
          if (found != std::string::npos && found > 2) {
            ...
            nocont = true;
            if (line.size() > 3 &&
                !strcasecmp(line.substr(line.size() - 3).c_str(), " -u"))
              Unmount(drive);
          }
        }
      }
      ...
      char tmp[CROSS_LEN];
      bool useh = false, readonly = wpcolon && commandLine.length() > 1 &&
                                    commandLine[0] == ':';
      safe_strncpy(tmp,
                   readonly ? commandLine.c_str() + 1 : commandLine.c_str(),
                   CROSS_LEN);
      pref_struct_stat test;
#if defined(WIN32)
      ht_stat_t htest;
      ...
      if (pref_stat(tmp, &test) &&
          (host_name == NULL || ht_stat(host_name, &htest))) {
        if (pref_stat(tmp, &test) && host_name != NULL)
          useh = true;
#else
      ...
      if (pref_stat(tmp, &test)) {
        ...
        if (!pref_stat(homedir.c_str(), &test)) {
          ...
          if (res) {
            ...
            temp_line = paths[0];
            continue;
          } else if ((!DOS_MakeName(tmp, fullname, &dummy) ||
                      strncmp(Drives[dummy]->GetInfo(), "local directory",
                              15)) &&
                     !qmount) {
            WriteOut(MSG_Get(usedef ? "PROGRAM_IMGMOUNT_DEFAULT_NOT_FOUND"
  ...
  bool Unmount(char &letter) {
    ...
    if (isalpha(letter)) { /* if it's a drive letter, then traditional usage
                              applies */
      ...
      if (i_drive < DOS_DRIVES && i_drive >= 0 && Drives[i_drive]) {
        ...
        case 0: // success
        {
          ...
          if (image && !partitionMount)
            DetachFromBios(image);
          /* If the drive letter is also a CD-ROM drive attached to IDE, then
           * let the IDE code know */
          if (cdrom)
            IDE_CDROM_Detach(i_drive);
          Drives[i_drive] = NULL;
          DOS_EnableDriveMenu(i_drive + 'A');
    ...
    } else if (isdigit(letter)) { /* DOSBox-X: drives mounted by number (INT
                                     13h) can be unmounted this way */
      int index = letter - '0';
  ...
  bool PrepElTorito(const std::string &type, const char &el_torito_cd_drive,
                    unsigned long &el_torito_floppy_base,
                    unsigned char &el_torito_floppy_type) {
     ...
     *        "Superfloppy" or hard disk emulation modes are not yet
     * implemented. This mode will never support "no emulation" boot. */
    if (type != "floppy") {
     ...
     * http://download.intel.com/support/motherboards/desktop/sb/specscdrom.pdf
     */
    /* FIXME: Somebody find me an example of a CD-ROM with bootable code for
     * both x86, PowerPC, and Macintosh. I need an example of such a CD since El
     * Torito allows multiple "headers" */
    ...
    for (ent_num = 0; ent_num < (2048 / 0x20); ent_num++) {
      ...
      if (entry[0] == 0x01 /*header*/) {
        ...
        if (header_count != 0) {
          LOG_MSG(
              "Warning: El Torito has more than one Header/validation entry\n");
          continue;
        }

        if (header_final) {
          LOG_MSG("Warning: El Torito has an additional header past the final "
                  "header\n");
          continue;
        }
        ...
        header_final = (entry[0] == 0x91);
        header_more =
            (int)(((unsigned int)entry[2]) + (((unsigned int)entry[3]) << 8u));
        header_platform = entry[1];
        LOG_MSG(
            "El Torito entry: first header platform=0x%02x more=%u final=%u\n",
            header_platform, header_more, header_final);
        header_count++;
      } else {
        if (header_more == 0) {
        ...
        if (entry[0] == 0x44) {
          ...
          if (header_platform == 0x00 /*x86*/) {
            ...
            if (load_segment != 0 && load_segment != 0x7C0)
              LOG_MSG("El Torito boot warning: load segments other than 0x7C0 "
                      "not supported yet\n");
            if (sector_count != 1)
              LOG_MSG("El Torito boot warning: sector counts other than 1 are "
                      "not supported yet\n");

            if (mediatype < 1 || mediatype > 3) {
              LOG_MSG("El Torito boot entry: media types other than floppy "
                      "emulation not supported yet\n");
              continue;
            }
  ...
  bool MountPartitionFat(const char drive, const int src_bios_disk) {
    ...
    if (src_bios_disk < 2 /*no, don't allow partitions on floppies!*/ ||
        src_bios_disk >= MAX_DISK_IMAGES ||
        imageDiskList[src_bios_disk] == NULL) {
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_NOT_ASSIGNED"));
      return false;
    }

    /* FIXME: IMGMOUNT and MOUNT -u are so hard-coded around C: and BIOS device
     * indexes that some confusion may happen if a partition is C: mounted from,
     * say, BIOS device 0x81 and the wrong thing may get unmounted and detached.
     *        So for sanity reasons, do not allow mounting to a drive letter if
     * a BIOS disk image WOULD normally be associated with it. This is a mess
     * inherited from back when this code forked from DOSBox SVN, because DOSBox
     * SVN makes these hardcoded assumptions. */
  ...
  bool MountElToritoFat(const char drive, const Bitu sizes[],
                        const char el_torito_cd_drive,
                        const unsigned long el_torito_floppy_base,
                        const unsigned char el_torito_floppy_type) {
    unsigned char driveIndex = drive - 'A';
    ...
    imageDisk *newImage = new imageDiskElToritoFloppy(
        (unsigned char)el_torito_cd_drive, el_torito_floppy_base,
        el_torito_floppy_type);
    newImage->Addref();

    DOS_Drive *newDrive = new fatDrive(newImage, options);
    newImage->Release(); // fatDrive calls Addref, and this will release
                         // newImage if fatDrive doesn't use it
    if (!(dynamic_cast<fatDrive *>(newDrive))->created_successfully) {
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_CANT_CREATE"));
      return false;
    }
  ...
  int path_no;
  bool MountFat(Bitu sizes[], const char drive, const bool isHardDrive,
                const std::string &str_size,
                const std::vector<std::string> &paths,
                const signed char ide_index, const bool ide_slave,
                const int reserved_cylinders, bool roflag) {
      ...
      if (paths[i] == "empty") {
        ...
        if (str_size.size() == 0) {
          ...
          if (ext != NULL) {
            ...
            // for all vhd files where the system will autodetect the chs
            // values,
            else if (!strcasecmp(ext, ".vhd")) {
              ro = wpcolon && paths[i].length() > 1 &&
                   paths[i].c_str()[0] == ':';
              // load the file with imageDiskVHD, which supports
              // fixed/dynamic/differential disks
              imageDiskVHD::ErrorCodes ret = imageDiskVHD::Open(
                  ro ? paths[i].c_str() + 1 : paths[i].c_str(), ro || roflag,
                  &vhdImage);
              switch (ret) {
              ...
              case imageDiskVHD::OPEN_SUCCESS: {
                skipDetectGeometry = true;
                const imageDiskVHD *vhdDisk =
                    dynamic_cast<imageDiskVHD *>(vhdImage);
                if (vhdDisk != NULL) {
                  LOG_MSG("VHD image detected SS,S,H,C: %u,%u,%u,%u",
                          (uint32_t)vhdDisk->sector_size,
                          (uint32_t)vhdDisk->sectors, (uint32_t)vhdDisk->heads,
                          (uint32_t)vhdDisk->cylinders);
                  if (vhdDisk->cylinders > 1023)
                    LOG_MSG("WARNING: cylinders>1023, INT13 will not work "
                            "unless extensions are used");
                  if (vhdDisk->GetVHDType() == imageDiskVHD::VHD_TYPE_FIXED)
                    LOG_MSG("VHD is a fixed image");
                  if (vhdDisk->GetVHDType() == imageDiskVHD::VHD_TYPE_DYNAMIC)
                    LOG_MSG("VHD is a dynamic image");
                  if (vhdDisk->GetVHDType() ==
                      imageDiskVHD::VHD_TYPE_DIFFERENCING)
                    LOG_MSG("VHD is a differencing image");
                }
                break;
              }
              case imageDiskVHD::ERROR_OPENING:
                errorMessage = MSG_Get("VHD_ERROR_OPENING");
                break;
              case imageDiskVHD::INVALID_DATA:
                errorMessage = MSG_Get("VHD_INVALID_DATA");
                break;
              case imageDiskVHD::UNSUPPORTED_TYPE:
                errorMessage = MSG_Get("VHD_UNSUPPORTED_TYPE");
                break;
              case imageDiskVHD::ERROR_OPENING_PARENT:
                errorMessage = MSG_Get("VHD_ERROR_OPENING_PARENT");
                break;
              case imageDiskVHD::PARENT_INVALID_DATA:
                errorMessage = MSG_Get("VHD_PARENT_INVALID_DATA");
                break;
              case imageDiskVHD::PARENT_UNSUPPORTED_TYPE:
                errorMessage = MSG_Get("VHD_PARENT_UNSUPPORTED_TYPE");
                break;
              case imageDiskVHD::PARENT_INVALID_MATCH:
                errorMessage = MSG_Get("VHD_PARENT_INVALID_MATCH");
                break;
              case imageDiskVHD::PARENT_INVALID_DATE:
                errorMessage = MSG_Get("VHD_PARENT_INVALID_DATE");
                break;
              default:
                break;
              }
              ...
              if (!newDisk) {
                if (!qmount)
                  WriteOut(MSG_Get("PROGRAM_IMGMOUNT_OPEN_ERROR"), fname);
                return false;
              }
              QCow2Image::QCow2Header qcow2_header =
                  QCow2Image::read_header(newDisk);
              // uint64_t sectors; /* unused */
              uint32_t imagesize;
              sizes[0] = 512; // default sector size
              if (qcow2_header.magic == QCow2Image::magic &&
                  (qcow2_header.version == 2 || qcow2_header.version == 3)) {
                uint32_t cluster_size = 1u << qcow2_header.cluster_bits;
                if ((sizes[0] < 512) || ((cluster_size % sizes[0]) != 0)) {
                  WriteOut(MSG_Get("PROGRAM_IMGMOUNT_INVALID_SECTORSIZE"),
                           cluster_size);
                  return false;
                }
                ...
                sizes[1] = 63; // sectors
                sizes[2] = 16; // heads
                sizes[3] = (uint64_t)qcow2_header.size / sizes[0] / sizes[1] /
                           sizes[2]; // cylinders
                setbuf(newDisk, NULL);
                newImage =
                    new QCow2Disk(qcow2_header, newDisk, fname, imagesize,
                                  (uint32_t)sizes[0], (imagesize > 2880));
                skipDetectGeometry = true;
                ...
                newImage = NULL;
                       ...
                       !strcasecmp(ext, ".ima")) { // Raw MFM image format is
                                                   // typically .img or .ima
        ...
        if (!skipDetectGeometry &&
            !DetectGeometry(NULL, paths[i].c_str(), sizes)) {
      ...
      if (!errorMessage) {
        ...
        if (vhdImage) {
          ...
          LOG_MSG("IMGMOUNT: qcow2 SS,S,H,C: %u,%u,%u,%u",
                  (uint32_t)newImage->sector_size, (uint32_t)newImage->sectors,
                  (uint32_t)newImage->heads, (uint32_t)newImage->cylinders);
          newImage = NULL;
        } else {
          if (roflag)
            options.emplace_back("readonly");
          newDrive = new fatDrive(paths[i].c_str(), (uint32_t)sizes[0],
                                  (uint32_t)sizes[1], (uint32_t)sizes[2],
                                  (uint32_t)sizes[3], options);
        }
        ...
        if (!fdrive->created_successfully) {
          ...
          if (fdrive->req_ver_major > 0) {
            ...
            sprintf(ver_msg, MSG_Get("PROGRAM_IMGMOUNT_DOS_VERSION"),
                    fdrive->req_ver_major, fdrive->req_ver_minor, errorMessage);
            errorMessage = ver_msg;
          }
        } else {
          if (imgDisks.size() == 1)
            mediaid = (int)((unsigned char)fdrive->GetMediaByte());
          if ((vhdImage && ro) || roflag)
    ...
    AddToDriveManager(drive, imgDisks,
                      (mediaid >= 0xF0) ? mediaid
                                        : (isHardDrive ? 0xF8 : 0xF0));
    DOS_EnableDriveMenu(drive);

    std::string tmp(wpcolon && paths[0].length() > 1 &&
                            paths[0].c_str()[0] == ':'
                        ? paths[0].substr(1)
                        : paths[0]);
    for (i = 1; i < paths.size(); i++) {
      tmp +=
          "; " + (wpcolon && paths[i].length() > 1 && paths[i].c_str()[0] == ':'
                      ? paths[i].substr(1)
                      : paths[i]);
    }
    lastmount = drive;
    ...
    if (unformatted) {
      if (!qmount)
        WriteOut(MSG_Get("PROGRAM_MOUNT_NOT_FORMATTED"));
    ...
    unsigned char driveIndex = drive - 'A';
    if (imgDisks.size() == 1 || (imgDisks.size() > 1 && driveIndex < 2 &&
                                 (swapInDisksSpecificDrive == driveIndex ||
                                  swapInDisksSpecificDrive == -1))) {
      imageDisk *image = ((fatDrive *)imgDisks[0])->loadedDisk;
      if (AttachToBiosAndIdeByLetter(image, drive, (unsigned char)ide_index,
                                     ide_slave)) {
        if (swapInDisksSpecificDrive == driveIndex ||
            swapInDisksSpecificDrive == -1) {
          for (size_t si = 0; si < MAX_SWAPPABLE_DISKS; si++) {
      ...
      /* now that the image is attached to INT 13h the INT 13 image can use it
       * now */
  ...
  imageDisk *MountImageNoneRam(Bitu sizes[], const int reserved_cylinders,
                               const bool forceFloppy) {
    imageDiskMemory *dsk =
        CreateRamDrive(sizes, reserved_cylinders, forceFloppy, this);
    if (dsk == NULL)
      return NULL;
  ...
  bool MountRam(Bitu sizes[], const char drive, const signed char ide_index,
                const bool ide_slave, bool roflag) {
    if (Drives[drive - 'A']) {
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_ALREADY_MOUNTED"));
      return false;
    }

    // by default, make a floppy disk if A: or B: is specified (still makes a
    // hard drive if not a recognized size)
    imageDiskMemory *dsk =
        CreateRamDrive(sizes, 0, (drive - 'A') < 2 && sizes[0] == 0, this);
    if (dsk == NULL)
      return false;
    if (dsk->Format() != 0x00) {
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_CANT_CREATE"));
  ...
  void AddToDriveManager(const char drive, DOS_Drive *imgDisk,
                         const uint8_t mediaid) const {
    std::vector<DOS_Drive *> imgDisks = {imgDisk};
    AddToDriveManager(drive, imgDisks, mediaid);
  }

  void AddToDriveManager(const char drive,
                         const std::vector<DOS_Drive *> &imgDisks,
                         const uint8_t mediaid) const {
    std::vector<DOS_Drive *>::size_type ct;
    ...
    for (ct = 0; ct < imgDisks.size(); ct++) {
      ...
      char root[7] = {drive, ':', '\\', '*', '.', '*', 0};
      DOS_FindFirst(root, DOS_ATTR_VOLUME); // force obtaining the label and
                                            // saving it in dirCache
    }
    dos.dta(save_dta);
  }
  ...
  bool DetectGeometry(FILE *file, const char *fileName, Bitu sizes[]) {
    bool yet_detected = false,
         readonly = wpcolon && strlen(fileName) > 1 && fileName[0] == ':';
    FILE *diskfile =
        file == NULL ? fopen64(readonly ? fileName + 1 : fileName, "rb") : file;
#if defined(WIN32)
    if (!diskfile && file == NULL) {
      const host_cnv_char_t *host_name =
          CodePageGuestToHost(readonly ? fileName + 1 : fileName);
      if (host_name != NULL)
        diskfile = _wfopen(host_name, L"rb");
    }
#endif
    if (!diskfile) {
      if (!qmount)
    ...
    /* If the first partition is a Windows 95 FAT32 (LBA) type partition, and we
     * failed to detect, then assume LBA and make up a geometry */
    if (!yet_detected &&
        (ptype == 0x0C /*FAT32+LBA*/ || ptype == 0x0E /*FAT16+LBA*/)) {
      yet_detected = 1;
      assume_lba = true;
      LOG_MSG("Failed to autodetect geometry, assuming LBA approximation based "
              "on first partition type (FAT with LBA)");
    }

    /* If the MBR has only a partition table, but the part that normally
     * contains executable code is all zeros. To avoid false negatives, check
     * only the first 0x20 bytes since at boot time executable code must reside
     * there to do something, and many of these disk images while they ARE
     * mostly zeros, do have some extra nonzero bytes immediately before the
     * partition table at 0x1BE.
     *
     * Modern FAT32 generator tools and older digital cameras will format FAT32
     * like this. These tools are unlikely to support non-LBA disks.
     *
     * To avoid false positives, the partition type has to be something related
     * to FAT */
    if (!yet_detected && (ptype == 0x01 || ptype == 0x04 || ptype == 0x06 ||
      ...
      if (i == 0x20) {
        yet_detected = 1;
        assume_lba = true;
        LOG_MSG("Failed to autodetect geometry, assuming LBA approximation "
                "based on first partition type (FAT-related) and lack of "
                "executable code in the MBR");
      }
    }

    /* If we failed to detect, but the disk image is 4GB or larger, make up a
     * geometry because IDE drives by that point were pure LBA anyway and
     * supported C/H/S for the sake of backward compatibility anyway. fcsize is
     * in 512-byte sectors. */
    if (!yet_detected &&
        fcsize >= ((4ull * 1024ull * 1024ull * 1024ull) / 512ull)) {
      yet_detected = 1;
      assume_lba = true;
      LOG_MSG("Failed to autodetect geometry, assuming LBA approximation based "
              "on size");
    ...
    if (yet_detected) {
      ...
      if (!qmount)
        WriteOut(MSG_Get("PROGRAM_IMGMOUNT_AUTODET_VALUES"), sizes[0], sizes[1],
                 sizes[2], sizes[3]);
      return true;
    } else {
      if (!qmount)
  ...
  bool DetectMFMsectorPartition(uint8_t buf[], uint32_t fcsize,
                                Bitu sizes[]) const {
     ...
     * +C       | 4 bytes   | Number of sectors in partition. 0 may mean, use
     * LBA
     */
    ...
    if ((uint32_t)host_readd(&buf[0x1fa]) !=
        0) { // DOS 2.0-3.21 partition table
      pe1_size = host_readd(&buf[0x1fa]);
      starthead = buf[0x1ef];
      startsect = (buf[0x1f0] & 0x3fu) - 1u;
      startcyl =
          (unsigned char)buf[0x1f1] | (unsigned int)((buf[0x1f0] & 0xc0) << 2u);
      endcyl =
          (unsigned char)buf[0x1f5] | (unsigned int)((buf[0x1f4] & 0xc0) << 2u);
      // ptype = buf[0x1f2];
      heads = buf[0x1f3] + 1u;
      ...
      startcyl =
          (unsigned char)buf[0x1c1] | (unsigned int)((buf[0x1c0] & 0xc0) << 2u);
      endcyl =
          (unsigned char)buf[0x1c5] | (unsigned int)((buf[0x1c4] & 0xc0) << 2u);
      // ptype = buf[0x1c2];
      heads = buf[0x1c3] + 1u;
    ...
    if (pe1_size != 0) {
      uint32_t part_start =
          startsect + sectors * starthead + startcyl * sectors * heads;
      ...
      // into 1023, else bail.
      if (/*(part_len<0) always false because unsigned || */ (part_len >
                                                              pe1_size) ||
          (pe1_size > fcsize) ||
  ...
  bool MountIso(const char drive, const std::vector<std::string> &paths,
                const signed char ide_index, const bool ide_slave) {
    // mount cdrom
    if (Drives[drive - 'A']) {
      WriteOut(MSG_Get("PROGRAM_IMGMOUNT_ALREADY_MOUNTED"));
  ...
  imageDisk *MountImageNone(const char *fileName, FILE *file,
                            const Bitu sizesOriginal[],
                            const int reserved_cylinders, bool roflag) {
    ...
    if (!strcmp(fileName, "empty")) {
      ...
      if (sizes[0] != 0 && sizes[1] != 0 && sizes[2] != 0 && sizes[3] != 0) {
        ...
        emd->diskSizeK =
            ((sizes[0] * sizes[1] * sizes[2] * sizes[3]) + 512) / 1024;
        emd->UpdateFloppyType();
      }
      LOG_MSG("Mounted empty C/H/S/sz %u/%u/%u/%u %uKB",
              (unsigned int)emd->cylinders, (unsigned int)emd->heads,
              (unsigned int)emd->sectors, (unsigned int)emd->sector_size,
    ...
    if (sizes[0] == 0 /* auto detect size */) {
      ...
      if (ext != NULL) {
        if (!strcasecmp(ext, ".vhd")) {
          ...
          imageDiskVHD::ErrorCodes ret = imageDiskVHD::Open(
              ro ? fileName + 1 : fileName, ro || roflag, &newImage);
          switch (ret) {
          case imageDiskVHD::ERROR_OPENING:
            WriteOut(MSG_Get("VHD_ERROR_OPENING"));
            break;
          case imageDiskVHD::INVALID_DATA:
            WriteOut(MSG_Get("VHD_INVALID_DATA"));
            break;
          case imageDiskVHD::UNSUPPORTED_TYPE:
            WriteOut(MSG_Get("VHD_UNSUPPORTED_TYPE"));
            break;
          case imageDiskVHD::ERROR_OPENING_PARENT:
            WriteOut(MSG_Get("VHD_ERROR_OPENING_PARENT"));
            break;
          case imageDiskVHD::PARENT_INVALID_DATA:
            WriteOut(MSG_Get("VHD_PARENT_INVALID_DATA"));
            break;
          case imageDiskVHD::PARENT_UNSUPPORTED_TYPE:
            WriteOut(MSG_Get("VHD_PARENT_UNSUPPORTED_TYPE"));
            break;
          case imageDiskVHD::PARENT_INVALID_MATCH:
            WriteOut(MSG_Get("VHD_PARENT_INVALID_MATCH"));
            break;
          case imageDiskVHD::PARENT_INVALID_DATE:
            WriteOut(MSG_Get("VHD_PARENT_INVALID_DATE"));
            break;
          case imageDiskVHD::UNSUPPORTED_WRITE:
            roflag = true;
            break;
          ...
          assumeHardDisk =
              true; /* bugfix for HDI images smaller than 2.88MB so that the
                       .hdi file is not mistaken for a floppy disk image */
        }
      }
    }
    ...
    FILE *newDisk =
        file == NULL
            ? fopen_lock(fname, readonly || roflag ? "rb" : "rb+", roflag)
            : file;
    if (!newDisk) {
      if (!qmount)
        WriteOut(MSG_Get("PROGRAM_IMGMOUNT_OPEN_ERROR"), fname);
    ...
    if (qcow2_header.magic == QCow2Image::magic &&
      ...
      imagesize = (uint32_t)(qcow2_header.size / 1024L);
      setbuf(newDisk, NULL);
      newImage = new QCow2Disk(qcow2_header, newDisk, fname, imagesize,
                               (uint32_t)sizes[0], (imagesize > 2880));
    } else {
      char tmp[256];
      ...
      if (ext != NULL && !strcasecmp(ext, ".d88")) {
        ...
        setbuf(newDisk, NULL);
        newImage = new imageDiskD88(newDisk, fname, imagesize,
                                    false /*this is a FLOPPY image format*/);
      } else if (!memcmp(tmp, "VFD1.", 5)) { /* FDD files */
        fseeko64(newDisk, 0L, SEEK_END);
        sectors = (uint64_t)ftello64(newDisk) / (uint64_t)sizes[0];
        imagesize = (uint32_t)(sectors / 2); /* orig. code wants it in KBs */
        setbuf(newDisk, NULL);
        newImage = new imageDiskVFD(newDisk, fname, imagesize,
                                    false /*this is a FLOPPY image format*/);
      } else if (!memcmp(tmp, "T98FDDIMAGE.R0\0\0", 16)) {
        fseeko64(newDisk, 0L, SEEK_END);
        sectors = (uint64_t)ftello64(newDisk) / (uint64_t)sizes[0];
        imagesize = (uint32_t)(sectors / 2); /* orig. code wants it in KBs */
        setbuf(newDisk, NULL);
        newImage = new imageDiskNFD(newDisk, fname, imagesize,
                                    false /*this is a FLOPPY image format*/, 0);
      } else if (!memcmp(tmp, "T98FDDIMAGE.R1\0\0", 16)) {
        fseeko64(newDisk, 0L, SEEK_END);
        sectors = (uint64_t)ftello64(newDisk) / (uint64_t)sizes[0];
        imagesize = (uint32_t)(sectors / 2); /* orig. code wants it in KBs */
        setbuf(newDisk, NULL);
        newImage = new imageDiskNFD(newDisk, fname, imagesize,
                                    false /*this is a FLOPPY image format*/, 1);
      } else {
        fseeko64(newDisk, 0L, SEEK_END);
        sectors = (uint64_t)ftello64(newDisk) / (uint64_t)sizes[0];
        imagesize = (uint32_t)(sectors / 2); /* orig. code wants it in KBs */
        setbuf(newDisk, NULL);
        newImage = new imageDisk(newDisk, fname, imagesize,
                                 (imagesize > 2880) || assumeHardDisk);
      }
    }

    /* sometimes imageDisk is able to determine geometry automatically (HDI
     * images) */
    if (newImage) {
      if (newImage->sectors != 0 && newImage->heads != 0 &&
          newImage->cylinders != 0 && newImage->sector_size != 0) {
    ...
    if (sizes[3] /*cylinders*/ == 0 && sizes[2] /*heads*/ == 0) {
      ...
      sizes[3] /*cylinders*/ = (Bitu)(((uint64_t)sectors +
                                       ((uint64_t)sizes[2] /*heads*/ *
                                        (uint64_t)sizes[1] /*sectors/track*/) -
                                       (uint64_t)1u) /
                                      ((uint64_t)sizes[2] /*heads*/ *
                                       (uint64_t)sizes[1] /*sectors/track*/));
    ...
    LOG(LOG_DOSMISC,
        LOG_NORMAL)("Mounting image as C/H/S %u/%u/%u with %u bytes/sector",
                    (unsigned int)sizes[3], (unsigned int)sizes[2],
                    (unsigned int)sizes[1], (unsigned int)sizes[0]);

    if (imagesize > 2880)
      newImage->Set_Geometry((uint32_t)sizes[2], (uint32_t)sizes[3],
                             (uint32_t)sizes[1], (uint32_t)sizes[0]);
    if (reserved_cylinders > 0)
      newImage->Set_Reserved_Cylinders((Bitu)reserved_cylinders);

    return newImage;
  }
};

void IMGMOUNT_ProgramStart(Program **make) { *make = new IMGMOUNT; }
...
Bitu DOS_LoadKeyboardLayout(const char *layoutname, int32_t codepage,
...
void KEYB::Run(void) {
  if (cmd->FindCommand(1, temp_line)) { /* first parameter is layout ID */
    if (cmd->FindString("?", temp_line, false)) {
      ...
      ;
      int32_t tocp =
          !strcasecmp(layout_id, "jp")
              ? 932
              : (!strcasecmp(layout_id, "ko")
                     ? 949
                     : (!strcasecmp(layout_id, "tw") ||
                                !strcasecmp(layout_id, "hk") ||
                                !strcasecmp(layout_id, "zht") ||
                                (!strcasecmp(layout_id, "zh") &&
                                 ((cp == 950 || cp == 951) ||
                                  (!cp_string.size() &&
                                   (dos.loaded_codepage == 950 ||
                                    dos.loaded_codepage == 951))))
                            ? (cp == 951 || (!cp_string.size() &&
                                             dos.loaded_codepage == 951)
                                   ? 951
                                   : 950)
                            : (!strcasecmp(layout_id, "cn") ||
                                       !strcasecmp(layout_id, "zhs") ||
                                       !strcasecmp(layout_id, "zh")
                                   ? 936
                                   : 0)));
      ...
      if (tocp && !IS_PC98_ARCH) {
        ...
        if (!strcasecmp(layout_id, "jp")) {
          keyb_error = DOS_LoadKeyboardLayout(
              "jp", tocp,
              "auto"); /* Load a default layout if not loaded at all */
          if (cp)
            keyb_error = DOS_ChangeCodepage(cp, "auto");
          if (keyb_error == KEYB_NOERROR)
            DOS_ChangeKeyboardLayout("jp", cp ? cp : tocp);
        } else {
          keyb_error = DOS_SwitchKeyboardLayout(
              "us",
              tocp); /* set Korean and Chinese keyboard layout to be "us" */
          /* FIX_ME: Chinese keyboards are identical to US keyboards, but some
           * fixes may be needed for Korean unique keys */
        }
      } else if (cp) {
        ...
        if (!strcasecmp(layout_id, "us")) {
          keyb_error = DOS_LoadKeyboardLayout(
              "us", 437,
              "auto"); /* set 437 as default codepage for US layout */
          dos.loaded_codepage = 437;
        } else {
          if (layout_name == NULL) {
            keyb_error = DOS_LoadKeyboardLayout(
      ...
      case KEYB_NOERROR: {
        ...
        WriteOut(MSG_Get("PROGRAM_KEYB_NOERROR"), layout_id,
                 dos.loaded_codepage);
        runRescan("-A -Q");
#if C_OPENGL && DOSBOXMENU_TYPE == DOSBOXMENU_SDLDRAW
        if (OpenGL_using() && control->opt_lang.size() && lastcp &&
      ...
      case KEYB_FILENOTFOUND:
        if (temp_line != "/?" && temp_line != "-?")
          WriteOut(MSG_Get("PROGRAM_KEYB_FILENOTFOUND"), layout_id);
        WriteOut(MSG_Get("PROGRAM_KEYB_SHOWHELP"));
        break;
      ...
      case KEYB_LAYOUTNOTFOUND:
        WriteOut(MSG_Get("PROGRAM_KEYB_LAYOUTNOTFOUND"), layout_id,
                 cp ? cp : tocp);
        break;
    ...
    /* no parameter in the command line, just output codepage info and possibly
     * loaded layout ID */
    const char *layout_name = DOS_GetLoadedLayout();
    if (layout_name == NULL) {
      WriteOut(MSG_Get("PROGRAM_KEYB_INFO"), dos.loaded_codepage);
    } else {
      WriteOut(MSG_Get("PROGRAM_KEYB_INFO_LAYOUT"), dos.loaded_codepage,
               layout_name);
    }
  }
}

static void KEYB_ProgramStart(Program **make) { *make = new KEYB; }
...
void MODE::Run(void) {
  ...
  if (temp_line == "-?" || temp_line == "/?") {
    ...
    return;
  } else if (strcasecmp(temp_line.c_str(), "con") == 0 ||
             strcasecmp(temp_line.c_str(), "con:") == 0) {
    if (IS_PC98_ARCH)
      return;
    if (cmd->GetCount() < 2) {
      PrintStatus();
    ...
    if (cmd->FindStringBegin("rate=", temp_line, false))
      rate = atoi(temp_line.c_str());
    if (cmd->FindStringBegin("delay=", temp_line, false))
      delay = atoi(temp_line.c_str());
    if (cmd->FindStringBegin("cols=", temp_line, false))
      cols = atoi(temp_line.c_str());
    else
      cols = COLS;
    if (cmd->FindStringBegin("lines=", temp_line, false))
      lines = atoi(temp_line.c_str());
    else
      lines = LINES;
    bool optr = cmd->FindStringBegin("rate=", temp_line, true),
         optd = cmd->FindStringBegin("delay=", temp_line, true),
         optc = cmd->FindStringBegin("cols=", temp_line, true),
         optl = cmd->FindStringBegin("lines=", temp_line, true);
    ...
    if (cmd->GetCount() > 1)
      goto modeparam;
    if (optr && optd) {
      if (rate < 1 || rate > 32 || delay < 1 || delay > 4)
        goto modeparam;
      IO_Write(0x60, 0xf3);
      IO_Write(0x60, (uint8_t)(((delay - 1) << 5) | (32 - rate)));
    }
    if ((optc || optl) && (cols != COLS || lines != LINES)) {
      std::string cmd = "line_" + std::to_string((int)cols) + "x" +
                        std::to_string((int)lines);
      if (!setlines(cmd.c_str()))
        goto modeparam;
    }
    return;
  } else if (cmd->GetCount() > 1)
    goto modeparam;
  else if (strcasecmp(temp_line.c_str(), "mono") == 0)
    mode = 7;
  else if (machine == MCH_HERC || machine == MCH_MDA)
    goto modeparam;
  else if (strcasecmp(temp_line.c_str(), "co80") == 0)
    mode = 3;
  else if (strcasecmp(temp_line.c_str(), "bw80") == 0)
    mode = 2;
  else if (strcasecmp(temp_line.c_str(), "co40") == 0)
    mode = 1;
  else if (strcasecmp(temp_line.c_str(), "bw40") == 0)
    mode = 0;
  else
    goto modeparam;
  mem_writeb(BIOS_CONFIGURATION, (mem_readb(BIOS_CONFIGURATION) & 0xcf) |
                                     ((mode == 7) ? 0x30 : 0x20));
  reg_ax = mode;
  CALLBACK_RunRealInt(0x10);
  return;
...
static void MODE_ProgramStart(Program **make) { *make = new MODE; }
/*
// MORE
...
static void NMITEST_ProgramStart(Program **make) { *make = new NMITEST; }
#endif
...
public:
  void Run() override {
    ...
    if (val == -1)
      msg.append(MSG_Get("PROGRAM_CAPMOUSE_CURRENTLY"));
    msg.append(Mouse_IsLocked() ? MSG_Get("PROGRAM_CAPMOUSE_CAPTURED")
                                : MSG_Get("PROGRAM_CAPMOUSE_RELEASED"));
    msg.append(".\n");
    WriteOut(msg.c_str());
  }
};

void CAPMOUSE_ProgramStart(Program **make) { *make = new CAPMOUSE; }

class LABEL : public Program {
public:
  void Help() { WriteOut(MSG_Get("PROGRAM_LABEL_HELP")); }
  void Run() override {
    /* MS-DOS behavior: If no label provided at the command line, prompt for
     * one.
     *
     * LABEL [drive:] [label]
     *
    ...
    /* then the label. MS-DOS behavior is to treat the rest of the command line,
     * spaces and all, as the label */
    if (*raw != 0) {
      label = raw;
    }

    /* if the label is longer than 11 chars or contains a dot, MS-DOS will
     * reject it and then prompt for another label */
    if (label.length() > 11) {
    ...
    /* if no label provided, MS-DOS will display the current label and serial
     * number and prompt the user to type in a new label. */
    if (label.empty()) {
      std::string clabel = Drives[drive]->GetLabel();

      if (!clabel.empty())
        WriteOut(MSG_Get("PROGRAM_LABEL_VOLUMEIS"), drive + 'A',
                 clabel.c_str());
      else
        WriteOut(MSG_Get("PROGRAM_LABEL_NOLABEL"), drive + 'A');
    }

    /* If no label is provided, MS-DOS will prompt the user whether to delete
     * the label. */
    if (label.empty()) {
      uint8_t c, ans = 0;
      uint16_t s;

      /* It does not make sense to say drive C: has no label, then prompt to
       * delete it */
      if ((*Drives[drive]->GetLabel()) == 0)
        return;
      ...
      } while (!(ans == MSG_Get("INT21_6523_YESNO_CHARS")[0] ||
                 ans == MSG_Get("INT21_6523_YESNO_CHARS")[1]));
      inshell = false;

      if (ans != MSG_Get("INT21_6523_YESNO_CHARS")[0])
        return;
    }
...
void LABEL_ProgramStart(Program **make) { *make = new LABEL; }
...
void MAPPER_AutoType(std::vector<std::string> &sequence, const uint32_t wait_ms,
                     const uint32_t pacing_ms, bool choice);
...
private:
  void PrintUsage();
  void PrintKeys();
  bool ReadDoubleArg(const std::string &name, const char *flag,
                     const double &def_value, const double &min_value,
                     const double &max_value, double &value);
};
...
void AUTOTYPE::PrintKeys() {
  ...
  for (size_t row = 0; row < rows; ++row) {
    for (size_t i = row; i < names.size(); i += rows)
      WriteOut("  %-*s", static_cast<int>(max_length),
               (name[i].size() == 1 && name[i][0] >= 'a' && name[i][0] <= 'z'
                    ? name[i] + " (" + std::string(1, toupper(name[i][0])) + ")"
                    : name[i])
                   .c_str());
    WriteOut_NoParsing("\n");
  }
 ...
 */
template <typename T> T to_finite(const std::string &input) {
 ...
 */
bool AUTOTYPE::ReadDoubleArg(const std::string &name, const char *flag,
                             const double &def_value, const double &min_value,
                             const double &max_value, double &value) {
  bool result = false;
  ...
  if (cmd->FindString(flag, str_value, true)) {
    ...
    if (isfinite(user_value)) {
      ...
      if (fabs(user_value - value) > std::numeric_limits<double>::epsilon())
        WriteOut(MSG_Get("PROGRAM_AUTOTYPE_CLAMP_VALUE"), name.c_str(),
                 user_value, value);
    } else { // Otherwise we couldn't parse their value
      WriteOut(MSG_Get("PROGRAM_AUTOTYPE_INVALID_VALUE"), name.c_str(),
               str_value.c_str());
    }
...
void AUTOTYPE_ProgramStart(Program **make) { *make = new AUTOTYPE; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_ADDKEY_HELP")); }
};
...
static void ADDKEY_ProgramStart(Program **make) { *make = new ADDKEY; }
...
static void LS_ProgramStart(Program **make) { *make = new LS; }
...
void CHOICE_ProgramStart(Program **make) { *make = new CHOICE; }
...
static void COUNTRY_ProgramStart(Program **make) { *make = new COUNTRY; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_UTF8_HELP")); }
};
...
void UTF8::Run() {
  ...
  if (dos.loaded_codepage == 808)
    strcpy(target, "CP866");
  else if (dos.loaded_codepage == 859)
    strcpy(target, "CP858");
  else if (dos.loaded_codepage == 867)
    strcpy(target, "CP862");
  else if (dos.loaded_codepage == 872)
    strcpy(target, "CP855");
  else if (dos.loaded_codepage == 951 && !uao)
    strcpy(target, "BIG5HKSCS");
  else if (dos.loaded_codepage == 951)
    strcpy(target, "CP950");
  else if (!(customcp && dos.loaded_codepage == customcp) &&
           !(altcp && dos.loaded_codepage == altcp))
    sprintf(target, "CP%d", dos.loaded_codepage);
  ...
  while (true) {
    ...
    if (m)
      text += std::string(1, c);
    if (m && first && text.size() == 2 &&
        (((uint8_t)text[0] == 0xFE && (uint8_t)text[1] == 0xFF) ||
         ((uint8_t)text[0] == 0xFF && (uint8_t)text[1] == 0xFE))) {
      WriteOut(MSG_Get("PROGRAM_UTF8_NOT_UTF8"));
      break;
    }
    if (m && first && text.size() == 3 && (uint8_t)text[0] == 0xEF &&
        (uint8_t)text[1] == 0xBB && (uint8_t)text[2] == 0xBF) {
      first = false;
      text = "";
    } else if (!m || c == 0x1A || c == 0xA ||
               (text.size() > 1 && (uint8_t)text[text.size() - 2] == 0xD)) {
      if (c != 0xA && text.size() > 1 && (uint8_t)text[text.size() - 2] == 0xD)
        text[text.size() - 1] = 0xA;
      if (CodePageHostToGuestUTF8(temp, text.c_str())) {
        WriteOut_NoParsing(temp, true);
      } else {
        x->set_src(text.c_str());
        if ((customcp && dos.loaded_codepage == customcp) ||
            (altcp && dos.loaded_codepage == altcp) ||
            x->string_convert_dest(dst) < 0 ||
            (text.size() && !fx->string_convert(dst).size())) {
...
static void UTF8_ProgramStart(Program **make) { *make = new UTF8; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_UTF16_HELP")); }
};
...
void UTF16::Run() {
  ...
  bool first = true;
  while (true) {
    if (!first || (buf[0] == 0xFE && buf[1] == 0xFF) ||
    ...
    if (!m || ch == 0x1A || ch == 0xA || (c > 1 && text[c - 2] == 0xD)) {
      if (ch != 0xA && c > 1 && text[c - 2] == 0xD) {
        text[c - 1] = 0xA;
      }
      wch = new test_char_t[c + 1];
      for (unsigned int i = 0; i < c; i++)
        wch[i] = (test_char_t)text[i];
      wch[c] = 0;
      if (CodePageHostToGuestUTF16(temp, wch)) {
        ...
        if ((customcp && dos.loaded_codepage == customcp) ||
            (altcp && dos.loaded_codepage == altcp) ||
            x->string_convert_dest(dst) < 0 || (c && !dst.size())) {
          WriteOut(MSG_Get("PROGRAM_UTF8_CONVERSION_ERROR"));
          delete[] wch;
          morelen = false;
          return;
...
static void UTF16_ProgramStart(Program **make) { *make = new UTF16; }
#endif
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_VTEXT_HELP")); }
};
...
static void VTEXT_ProgramStart(Program **make) { *make = new VTEXT; }
...
static void DCGA_ProgramStart(Program **make) { *make = new DCGA; }
...
static void TEXT80X25_ProgramStart(Program **make) { *make = new TEXT80X25; }
...
static void TEXT80X43_ProgramStart(Program **make) { *make = new TEXT80X43; }
...
static void TEXT80X50_ProgramStart(Program **make) { *make = new TEXT80X50; }
...
static void TEXT80X60_ProgramStart(Program **make) { *make = new TEXT80X60; }
...
static void TEXT132X25_ProgramStart(Program **make) { *make = new TEXT132X25; }
...
static void TEXT132X43_ProgramStart(Program **make) { *make = new TEXT132X43; }
...
static void TEXT132X50_ProgramStart(Program **make) { *make = new TEXT132X50; }
...
static void TEXT132X60_ProgramStart(Program **make) { *make = new TEXT132X60; }
...
static void HELP_ProgramStart(Program **make) { *make = new HELP; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_DELTREE_HELP")); }
};
...
static void DELTREE_ProgramStart(Program **make) { *make = new DELTREE; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_TREE_HELP")); }
};
...
static void TREE_ProgramStart(Program **make) { *make = new TREE; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_TITLE_HELP")); }
};
...
static void TITLE_ProgramStart(Program **make) { *make = new TITLE; }
...
void VHDMAKE::Run() {
  ...
  if (cmd->FindExist("-i", true) || cmd->FindExist("-info", true)) {
    ...
    WriteOut(MSG_Get("PROGRAM_VHDMAKE_INFO"), filename,
             vhdTypes[(int)info->vhdType], info->vhdSizeMB);
    if (info->vhdType != imageDiskVHD::VHD_TYPE_FIXED)
      WriteOut(MSG_Get("PROGRAM_VHDMAKE_BLOCKSTATS"), info->allocatedBlocks,
               info->totalBlocks);
    else
      WriteOut(".\n");
    if (info->parentInfo != NULL) {
      ...
      while (p != NULL) {
        ...
        for (uint32_t i = 0; i < index; i++)
          WriteOut(" ");
        WriteOut("child of \"%s\" (%s)", p->diskname.c_str(),
                 vhdTypes[(int)p->vhdType]);
        if (p->vhdType != imageDiskVHD::VHD_TYPE_FIXED)
          WriteOut(MSG_Get("PROGRAM_VHDMAKE_BLOCKSTATS"), p->allocatedBlocks,
                   p->totalBlocks);
        else
          WriteOut(".\n");
        p = p->parentInfo;
  ...
  if (cmd->FindExist("-m", true) || cmd->FindExist("-merge", true)) {
    ...
    imageDiskVHD *vhd;
    if (imageDiskVHD::Open(basename, true, (imageDisk **)&vhd) !=
        imageDiskVHD::OPEN_SUCCESS) {
    ...
    if (vhd->MergeSnapshot(&totalSectorsMerged, &totalBlocksUpdated)) {
      WriteOut(MSG_Get("PROGRAM_VHDMAKE_MERGEREPORT"), totalSectorsMerged,
               totalBlocksUpdated, basename, parentName.c_str());
      delete vhd;
      if (remove(basename))
      ...
      if (totalSectorsMerged)
        WriteOut(MSG_Get("PROGRAM_VHDMAKE_MERGEWARNCORRUPTION"),
                 parentName.c_str());
      else
        WriteOut("\n");
    }
...
static void VHDMAKE_ProgramStart(Program **make) { *make = new VHDMAKE; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_COLOR_HELP")); }
};
...
void COLORPGM::Run() {
    ...
    if (fg == bg) {
      ...
      if (bg == '0' || bg == '8')
        ...
        back = true;
             ...
             1) { // Set background color to 0 if only one color is specified
    fg = args[0];
    bg = '0';
    bgc = 40;
    if (fg == '0') {
      ...
      if (fg == '8')
        ...
        back = true;
             ...
             0) { // default is white character on black background
    bg = '0';
    bgc = 40;
    fg = '7';
    fgc = 37;
  ...
  if (!back) {
    ...
    WriteOut(("\033[" + std::string(fgl || bgl ? "0;" : "") +
              std::string(fgl ? "" : "1;") + std::string(bgl ? "" : "5;") +
              std::to_string(fgc) + ";" + std::to_string(bgc) + "m")
                 .c_str());
  ...
  } else if (!dos.return_code)
    PrintUsage();
}

static void COLOR_ProgramStart(Program **make) { *make = new COLORPGM; }
...
bool setVGAColor(const char *colorArray, int j) {
  if (!IS_VGA_ARCH || !CurMode)
    return false;
  const char *nextRGB = colorArray;
  int32_t rgbVal[4] = {-1, -1, -1, -1};
  if (sscanf(nextRGB, " ( %d , %d , %d)", &rgbVal[0], &rgbVal[1], &rgbVal[2]) ==
      3) {
    for (int i = 0; i < 3; i++) {
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_SETCOLOR_HELP")); }
};
...
void SETCOLOR::Run() {
  ...
  if (*args) {
    ...
    if (!strcasecmp(args, "MONO")) {
      if (p == NULL)
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_STATUS"),
                 CurMode->mode == 7   ? MSG_Get("PROGRAM_SETCOLOR_ACTIVE")
                 : CurMode->mode == 3 ? MSG_Get("PROGRAM_SETCOLOR_INACTIVE")
                                      : MSG_Get("PROGRAM_SETCOLOR_UNAVAILABLE"),
                 CurMode->mode);
      else if (!strcmp(trim(p + 1), "+")) {
        if (CurMode->mode != 7)
          INT10_SetVideoMode(7);
        WriteOut(CurMode->mode == 7 ? MSG_Get("PROGRAM_SETCOLOR_MONO_MODE7")
                                    : MSG_Get("PROGRAM_SETCOLOR_MONO_FAIL"));
      } else if (!strcmp(trim(p + 1), "-")) {
        if (CurMode->mode != 3)
          INT10_SetVideoMode(3);
        WriteOut(CurMode->mode == 3 ? MSG_Get("PROGRAM_SETCOLOR_MONO_MODE3")
                                    : MSG_Get("PROGRAM_SETCOLOR_MONO_FAIL"));
      } else
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_MONO_SYNTAX"), trim(p + 1));
    } else if (!strcmp(args, "0") || !strcmp(args, "00") ||
               !strcmp(args, "+0") || !strcmp(args, "-0") ||
               (i > 0 && i < 16)) {
      if (p == NULL) {
#if defined(USE_TTF)
        bool colornul =
            staycolors || (IS_VGA_ARCH &&
                           (altBGR1[i].red > 4 || altBGR1[i].green > 4 ||
                            altBGR1[i].blue > 4) &&
                           rgbcolors[i].red < 5 && rgbcolors[i].green < 5 &&
                           rgbcolors[i].blue < 5);
        altBGR[i].red = colornul || (colorChanged && !IS_VGA_ARCH)
                            ? altBGR1[i].red
                            : rgbcolors[i].red;
        altBGR[i].green = colornul || (colorChanged && !IS_VGA_ARCH)
                              ? altBGR1[i].green
                              : rgbcolors[i].green;
        altBGR[i].blue = colornul || (colorChanged && !IS_VGA_ARCH)
                             ? altBGR1[i].blue
                             : rgbcolors[i].blue;
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_COLOR"), i, altBGR0[i].red,
                 altBGR0[i].green, altBGR0[i].blue, altBGR0[i].red,
                 altBGR0[i].green, altBGR0[i].blue);
#else
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_COLOR"), i, rgbcolors[i].red,
                 rgbcolors[i].green, rgbcolors[i].blue, rgbcolors[i].red,
                 rgbcolors[i].green, rgbcolors[i].blue);
#endif
      }
    } else {
    ...
    if (p != NULL && strcasecmp(args, "MONO")) {
      ...
      if (!strcmp(trim(p + 1), "-")) {
        ...
        for (int colNo = 0; colNo <= i; colNo++) {
          if (sscanf(nextRGB, " ( %d , %d , %d)", &rgbVal[0], &rgbVal[1],
                     &rgbVal[2]) == 3) {
            sprintf(value, "(%d,%d,%d)", rgbVal[0], rgbVal[1], rgbVal[2]);
            while (*nextRGB != ')')
              nextRGB++;
            nextRGB++;
          } else if (sscanf(nextRGB, " #%6x", (unsigned int *)(&rgbVal[0])) ==
                     1) {
      ...
      if (!ttf.inUse) {
        ...
        altBGR[i].blue = (colornul || (colorChanged && !IS_VGA_ARCH))
                             ? altBGR1[i].blue
                             : rgbcolors[i].blue;
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_COLOR"), i, altBGR[i].red,
                 altBGR[i].green, altBGR[i].blue, altBGR[i].red,
                 altBGR[i].green, altBGR[i].blue);
        resetFontSize();
      } else
        WriteOut(MSG_Get("PROGRAM_SETCOLOR_INVALID_VALUE"), value);
#endif
    ...
    for (int i = 0; i < 16; i++) {
      ...
      altBGR[i].blue = colornul || (colorChanged && !IS_VGA_ARCH)
                           ...
                           : rgbcolors[i].blue;
      WriteOut(MSG_Get("PROGRAM_SETCOLOR_COLOR"), i, altBGR[i].red,
               altBGR[i].green, altBGR[i].blue, altBGR[i].red, altBGR[i].green,
               altBGR[i].blue);
#else
      WriteOut(MSG_Get("PROGRAM_SETCOLOR_COLOR"), i, rgbcolors[i].red,
               rgbcolors[i].green, rgbcolors[i].blue, rgbcolors[i].red,
...
static void SETCOLOR_ProgramStart(Program **make) { *make = new SETCOLOR; }
...
private:
  void PrintUsage() { WriteOut(MSG_Get("PROGRAM_INT2FDBG_HELP")); }
};
...
static void INT2FDBG_ProgramStart(Program **make) { *make = new INT2FDBG; }
#endif
...
int zipOutOpenFile(zipFile zf, const char *zfname, zip_fileinfo &zi,
                   const bool compress);
...
public:
  void Run() override {
...
#if defined(WIN32) && !defined(HX_DOS)
    char *cmdstr =
        cmd == NULL ? NULL : (char *)strstr(cmd, cmd[0] == '"' ? "\" " : " ");
    ...
    int state = cmd == NULL ? 0
                : !strcmp(cmd, "-") || !strcasecmp(cmd, "/min") ||
                        !strcasecmp(cmd, "-min")
                    ? 1
                : !strcmp(cmd, "+") || !strcasecmp(cmd, "/max") ||
                        !strcasecmp(cmd, "-max")
                    ? 2
                : !strcasecmp(cmd, "_") || !strcasecmp(cmd, "/hid") ||
                        !strcasecmp(cmd, "-hid")
                    ? 3
                    : 0;
    if (state > 0) {
    ...
    if (cmd != NULL) {
      ...
      if (ret1 != NULL && (ret == NULL || ret1 < ret0))
        ret = ret1;
      if (ret2 != NULL && (ret == NULL || ret2 < ret0))
        ret = ret2;
      if (ret3 != NULL && (ret == NULL || ret3 < ret0))
        ret = ret3;
      if (ret4 != NULL && ret != NULL && ret4 < ret)
        ret = ret4;
      if (ret != NULL && !(ret == ret0 && ret > cmd && *(ret - 1) == ':')) {
        ...
        cmdstr = str;
        *ret = '\0';
        if (*cmd == '"' && strlen(cmdstr) > 0 &&
            cmdstr[strlen(cmdstr) - 2] == '"') {
          cmd++;
          cmdstr[strlen(cmdstr) - 2] = '\0';
        }
      }
    }
    if (cmd == NULL || !strlen(cmd) || !strcmp(cmd, "?") || !strcmp(cmd, "/") ||
        !strcmp(cmd, "/?") || !strcmp(cmd, "-?")) {
      PrintUsage();
      DOS_SetError(0);
      return;
    }
    int sw = state == 0   ? SW_SHOW
             : state == 1 ? SW_MINIMIZE
             : state == 2 ? SW_MAXIMIZE
                          : SW_HIDE;
    bool match = false;
    ...
    if (in)
      for (std::string command; in >> command;) {
        if (!strcasecmp(cmd, command.c_str()) ||
            !strcasecmp(cmd, ("\"" + command + "\"").c_str())) {
          match = true;
          break;
        }
      }
    ...
    if (match) {
      ...
      strcat(dir, " ");
      if (cmdstr != NULL)
        strcat(dir, TranslateHostPath(cmdstr));
      if (!strcasecmp(cmd, "for"))
        strcat(dir, ")");
      ...
      lpExecInfo.lpParameters =
          cmdstr == NULL ? NULL : TranslateHostPath(cmdstr, true);
    }
    bool setdir = false;
    ...
    if (!startquiet)
      WriteOut(MSG_Get("PROGRAM_START_COMMAND"), cmd);
    ...
    if (startwait && lpExecInfo.hProcess != NULL) {
      ...
      do {
        ...
        if (++count == 20000 && ret && exitCode == STILL_ACTIVE && !startquiet)
          WriteOut(MSG_Get("PROGRAM_START_CTRLC"));
...
#else
    if (cmd == NULL || !strlen(cmd) || !strcmp(cmd, "?") || !strcmp(cmd, "/") ||
        !strcmp(cmd, "/?") || !strcmp(cmd, "-?") || !strcasecmp(cmd, "/open") ||
        !strcasecmp(cmd, "-open")) {
      PrintUsage();
      DOS_SetError(0);
      return;
    ...
    ret = system(
        ((open ?
               ...
               : "") +
         std::string(cmd) +
         (startwait || (strlen(cmd) > 2 && !strcmp(cmd + strlen(cmd) - 2, " &"))
              ? ""
              : " &"))
            .c_str());
#else
...
void START_ProgramStart(Program **make) { *make = new START; }
...
int flagged_backup(char *zip) {
  ...
  bool compresssaveparts =
      static_cast<Section_prop *>(control->GetSection("dosbox"))
          ->Get_bool("compresssaveparts");
  ...
  i = 0;
  while (i < MAX_FLAGS && g_flagged_files[i] == NULL)
    i++;
  if (i < MAX_FLAGS) {
    zipFile zf;
    {
    ...
    if (zf != NULL) {
      while (i < MAX_FLAGS) {
        if (g_flagged_files[i] != NULL) {
          uint16_t handle = 0;
          if (DOS_FindDevice(
                  ("\"" + std::string(g_flagged_files[i]) + "\"").c_str()) !=
                  DOS_DEVICES ||
              !DOS_OpenFile(
                  ("\"" + std::string(g_flagged_files[i]) + "\"").c_str(), 0,
                  &handle)) {
          ...
          zip_fileinfo zi;
          zipSetCurrentTime(zi);
          if (zipOutOpenFile(zf, g_flagged_files[i], zi, compresssaveparts) ==
              ZIP_OK) {
            ...
            do {
              ...
              if (n == 0)
                break;
              if (zos.xsputn((zip_ostreambuf::char_type *)buffer, n) < n)
                break;
            } while (1);
    ...
    if (zf != NULL) {
      while (i < MAX_FLAGS) {
        if (g_flagged_files[i] != NULL) {
          ...
          if (unzLocateFile(zf, g_flagged_files[i], 2 /*case insensitive*/) ==
                  UNZ_OK &&
              unzGetCurrentFileInfo64(zf, &file_info, NULL, 0, NULL, 0, NULL,
                                      0) == UNZ_OK) {
            if (unzOpenCurrentFile(zf) == UNZ_OK) {
              ...
              if (DOS_CreateFile(
                      ...
                      0, &handle)) {
                do {
                  uint16_t n = zis.xsgetn((zip_istreambuf::char_type *)buffer,
                                          sizeof(buffer));
                  if (n == 0)
                    break;
...
public:
  void Run(void) override {
    ...
    if (cmd->FindExist("/u", true)) {
      ...
      for (unsigned int i = 1; i <= cmd->GetCount(); i++) {
        ...
        if (*flagfile &&
            DOS_MakeName(((flagfile[0] != '\"' ? "\"" : "") +
                          std::string(flagfile) +
                          (flagfile[strlen(flagfile) - 1] != '\"' ? "\"" : ""))
                             .c_str(),
                         fullname, &drive)) {
          ...
          if (remove) {
            for (lf = 0; lf < MAX_FLAGS; lf++) {
              if (g_flagged_files[lf] != NULL &&
                  !strcasecmp(g_flagged_files[lf], flagfile)) {
                WriteOut(MSG_Get("PROGRAM_FLAGSAVE_UNFLAGGED"),
                         g_flagged_files[lf]);
          ...
          if (!force &&
              !DOS_FileExists(("\"" + std::string(flagfile) + "\"").c_str())) {
  ...
  void printHelp() { WriteOut(MSG_Get("PROGRAM_FLAGSAVE_HLP")); }
};

static void FLAGSAVE_ProgramStart(Program **make) { *make = new FLAGSAVE; }
...
void Add_VFiles(bool usecp) {
  ...
  if (prepared)
    VFILE_Register("CONFIG.SYS", (uint8_t *)config_data,
                   (uint32_t)strlen(config_data));
  ...
  if (startcmd)
    PROGRAMS_MakeFile("START.COM", START_ProgramStart, "/SYSTEM/");
#endif

  if (machine == MCH_CGA)
    PROGRAMS_MakeFile("CGASNOW.COM", CGASNOW_ProgramStart, "/TEXTUTIL/");
...
#if C_IPX
  if (addipx)
    PROGRAMS_MakeFile("IPXNET.COM", IPXNET_ProgramStart, "/SYSTEM/");
#endif
  if (addne2k)
    VFILE_RegisterBuiltinFileBlob(bfb_NE2000_COM, "/SYSTEM/");
  if (addovl)
    VFILE_RegisterBuiltinFileBlob(bfb_GLIDE2X_OVL, "/SYSTEM/");

  /* These are IBM PC/XT/AT ONLY. They will not work in PC-98 mode. */
  if (!IS_PC98_ARCH) {
    VFILE_RegisterBuiltinFileBlob(bfb_SYS_COM,
                                  "/DOS/"); /* may rely on INT 13h or IBM PC
                                               specific functions and layout */
    VFILE_RegisterBuiltinFileBlob(bfb_FORMAT_EXE,
                                  "/DOS/"); /* does not work in PC-98 mode */
    VFILE_RegisterBuiltinFileBlob(
        bfb_DEFRAG_EXE,
        "/DOS/"); /* relies on IBM PC CGA/EGA/VGA alphanumeric display memory */
    VFILE_RegisterBuiltinFileBlob(bfb_FDISK_EXE,
                                  "/BIN/"); /* relies on IBM PC INT 13h */
  ...
  if (prepared)
    VFILE_Register("4DOS.INI", (uint8_t *)i4dos_data,
                   (uint32_t)strlen(i4dos_data), "/4DOS/");
...
void Add_Existing_Drive_Directories() {
  for (auto drive = 'C'; drive < 'Y'; drive++) {
    ...
    LOG_MSG("Mounting directory 'drive%c' found in DOSBox-X directory as drive "
            "%c.\n",
...
void DOS_SetupPrograms(void) {
  ...
  MSG_Add("PROGRAM_IMGMOUNT_STATUS_2",
  ...
  MSG_Add("PROGRAM_IMGSWAP_STATUS",
          "Drives currently available for swapping are:\n");
  MSG_Add("PROGRAM_IMGSWAP_ERROR",
  ...
  MSG_Add("PROGRAM_MOUNT_IMGMOUNT",
          "To mount image files, use the \033[34;1mIMGMOUNT\033[0m command, "
          "not the \033[34;1mMOUNT\033[0m command.\n");
  ...
  MSG_Add(
      ...
      "Usage: \033[34;1m\033[32;1mMOUNT\033[0m \033[37;1mdrive\033[0m "
      "\033[36;1mlocal_directory\033[0m [option]\033[0m\n"
      " \033[37;1mdrive\033[0m            Drive letter where the directory or "
      "drive will be mounted.\n"
      " \033[36;1mlocal_directory\033[0m  Local directory or drive in the host "
      "system to be mounted.\n"
      " [option]         Option(s) for mounting. The following options are "
      "accepted:\n"
      " -t               Specify the drive type the mounted drive to behave "
      "as.\n"
      "                  Supported drive type: dir, floppy, cdrom, overlay\n"
      " (Note that 'overlay' redirects writes for mounted drive to another "
      "directory)\n"
      " -label [name]    Set the volume label name of the drive (all upper "
      "case).\n"
      " -nl              Use next available drive letter if the drive is "
      "mounted.\n"
      " -ro              Mount the drive in read-only mode.\n"
      " -pr              Specify the path is relative to the config file "
      "location.\n"
      " -cd              Generate a list of local CD drive's \"drive #\" "
      "values.\n"
      " -usecd [drive #] For direct hardware emulation such as audio "
      "playback.\n"
      " -ioctl           Use lowest level hardware access (following -usecd "
      "option).\n"
      " -aspi            Use the installed ASPI layer (following -usecd "
      "option).\n"
      " -freesize [size] Specify the free disk space of drive in MB (KB for "
      "floppies).\n"
  ...
  MSG_Add(
      ...
      "The directory has to exist in the host system. If the directory "
      "contains\n"
      "space(s), be sure to properly quote the directory with double quotes,\n"
      "e.g. %s\n\n"
      "Some other usage examples of MOUNT:\n\n"
#if defined(WIN32) || defined(OS2)
      "\033[32;1mMOUNT\033[0m                             - list all mounted "
      "drives\n"
      "\033[32;1mMOUNT -cd\033[0m                         - list all local CD "
      "drives\n"
#else
      "\033[32;1mMOUNT\033[0m                            - list all mounted "
      "drives\n"
      ...
      "drives\n"
#endif
      "\033[32;1mMOUNT d %s\033[0m            - mount the D: drive to the "
      "directory\n"
      "\033[32;1mMOUNT c %s -t cdrom\033[0m      - mount the C: drive as a "
      "CD-ROM drive\n"
      "\033[32;1mMOUNT c %s -ro\033[0m           - mount the C: drive in "
      "read-only mode\n"
      "\033[32;1mMOUNT c %s -label TEST\033[0m   - mount the C: drive with the "
      "label TEST\n"
      "\033[32;1mMOUNT c %s -nocachedir \033[0m  - mount C: without caching "
      "the drive\n"
      "\033[32;1mMOUNT c %s -freesize 128\033[0m - mount C: with 128MB free "
      "disk space\n"
      "\033[32;1mMOUNT c %s -u\033[0m            - force mount C: drive even "
      "if it's mounted\n"
      "\033[32;1mMOUNT c %s -t overlay\033[0m  - mount C: with overlay "
      "directory on top\n"
#if defined(WIN32) || defined(OS2)
      "\033[32;1mMOUNT c -u\033[0m                        - unmount the C: "
      "drive\n"
#else
      "\033[32;1mMOUNT c -u\033[0m                       - unmount the C: "
      "drive\n"
  ...
  MSG_Add("PROGRAM_MOUNT_UMOUNT_NUMBER_SUCCESS",
  ...
  MSG_Add("PROGRAM_MOUNT_PHYSFS_ERROR",
          "Failed to mount the PhysFS drive with the archive file.\n");
  MSG_Add("PROGRAM_MOUNT_OVERLAY_NO_BASE",
          "Please MOUNT a normal directory first before adding an overlay on "
          "top.\n");
  ...
  MSG_Add("PROGRAM_MOUNT_OVERLAY_MIXED_BASE",
          "The overlay needs to be specified using the same addressing as the "
          "underlying drive. No mixing of relative and absolute paths.");
  MSG_Add("PROGRAM_MOUNT_OVERLAY_SAME_AS_BASE",
          "The overlay directory can not be the same as underlying drive.\n");
  MSG_Add("PROGRAM_MOUNT_OVERLAY_ERROR",
  ...
  MSG_Add("PROGRAM_MOUNT_OVERLAY_REPLACE",
  ...
  MSG_Add("PROGRAM_LOADFIX_NOALLOC",
          "Lowest MCB is above 64KB, nothing allocated\n");

  MSG_Add(
      "PROGRAM_LOADFIX_HELP",
      "Loads a program above the first 64 KB memory by reducing the available "
      "memory.\n\n"
      "LOADFIX [-xms] [-ems] [-{ram}] [{program}] [{options}]\n"
      "LOADFIX -f [-xms] [-ems]\n\n"
      "  -xms        Allocates memory from XMS rather than conventional "
      "memory\n"
      "  -ems        Allocates memory from EMS rather than conventional "
      "memory\n"
      "  -{ram}      Specifies the amount of memory to allocate in KB\n"
      "              Defaults to 64KB for conventional memory; 1MB for XMS/EMS "
      "memory\n"
      "  -a          Auto allocates enough memory to fill the lowest 64KB "
      "memory\n"
      "  -f (or -d)  Frees previously allocated memory\n"
      "  {program}   Runs the specified program\n"
      "  {options}   Program options (if any)\n\n"
      "Examples:\n"
      "  \033[32;1mLOADFIX game.exe\033[0m     Allocates 64KB of conventional "
      "memory and runs game.exe\n"
      "  \033[32;1mLOADFIX -a\033[0m           Auto-allocates enough memory "
      "conventional memory\n"
      "  \033[32;1mLOADFIX -128\033[0m         Allocates 128KB of conventional "
      "memory\n"
      "  \033[32;1mLOADFIX -xms\033[0m         Allocates 1MB of XMS memory\n"
      "  \033[32;1mLOADFIX -f\033[0m           Frees allocated conventional "
      "memory\n");
  ...
  MSG_Add("MSCDEX_ERROR_MULTIPLE_CDROMS",
          ...
          "continuous.\n");
  MSG_Add("MSCDEX_ERROR_NOT_SUPPORTED",
  ...
  MSG_Add("MSCDEX_TOO_MANY_DRIVES", "MSCDEX: Failure: Too many CD-ROM drives "
                                    "(max: 5). MSCDEX Installation failed.\n");
  ...
  MSG_Add("PROGRAM_RESCAN_HELP",
          "Rescans for changes on mounted drives made on the host by clearing "
          "caches.\n\n"
          "RESCAN [/A] [/Q]\n"
          "RESCAN [drive:] [/Q]\n\n"
          "  [/A]\t\tRescan all drives\n"
  ...
  MSG_Add(
      "PROGRAM_INTRO",
      "\033[2J\033[32;1mWelcome to DOSBox-X\033[0m, an open-source x86 "
      "emulator with sound and graphics.\n"
      "DOSBox-X creates a shell for you which looks just like the plain DOS.\n"
      "\n"
      "\033[31;1mDOSBox-X will stop/exit without a warning if an error "
      "occurred!\033[0m\n"
      "\n"
      "\n");
  ...
  MSG_Add("PROGRAM_INTRO_MENU_BASIC_HELP",
          "\n\033[1m   \033[1m\033[KMOUNT allows you to connect real hardware "
          "to DOSBox-X's emulated PC.\033[0m\n");
  MSG_Add(
      "PROGRAM_INTRO_MENU_CDROM_HELP",
      "\n\033[1m   \033[1m\033[KTo mount your CD-ROM in DOSBox-X, you need to "
      "specify additional options\n   when mounting the CD-ROM.\033[0m\n");
  MSG_Add("PROGRAM_INTRO_MENU_USAGE_HELP",
          "\n\033[1m   \033[1m\033[KAn overview of the command line options "
          "you can give to DOSBox-X.\033[0m\n");
  MSG_Add("PROGRAM_INTRO_MENU_INFO_HELP",
          "\n\033[1m   \033[1m\033[KHow to get more information about "
          "DOSBox-X.\033[0m\n");
  MSG_Add("PROGRAM_INTRO_MENU_QUIT_HELP",
          "\n\033[1m   \033[1m\033[KExit from Intro.\033[0m\n");
  MSG_Add(
      "PROGRAM_INTRO_USAGE_TOP",
      "\033[2J\033[32;1mAn overview of the command line options you can give "
      "to DOSBox-X.\033[0m\n"
      "Windows users must open cmd.exe or edit the shortcut to DOSBox-X.exe "
      "for this.\n\n"
      "dosbox-x [name] [-exit] [-version] [-fastlaunch] [-fullscreen]\n"
      "         [-conf congfigfile] [-lang languagefile] [-machine "
      "machinetype]\n"
      "         [-startmapper] [-noautoexec] [-scaler scaler | -forcescaler "
      "scaler]\n"
  ...
  MSG_Add(
      ...
      "\tThe machinetype affects both the video card and available sound "
      "cards.\n");
  MSG_Add(
      ...
      "\tProvides command-line option(s) for \"name\" if an executable name "
      "is\n"
      "\tspecified. Multiple -o can be used for multiple executable names.\n\n"
      "\033[33;1m  -c\033[0m command\n"
      "\tRuns the specified command before running name. Multiple commands\n"
      "\tcan be specified. Each command should start with -c, though.\n"
      "\tA command can be: an Internal Program, a DOS command or an "
      "executable\n"
      "\ton a mounted drive.\n\n"
      "\033[33;1m  -set\033[0m <section property=value>\n"
      "\tSets the config option (overriding the config file). Multiple "
      "options\n"
  ...
  MSG_Add(
      ...
      "For information about CD-ROM support, type \033[34;1mintro "
      "cdrom\033[0m\n"
      ...
      "\n"
      "\033[34;1mhttps://dosbox-x.com/\033[0m or "
      "\033[34;1mhttp://dosbox-x.software\033[0m\n"
      "\n"
  ...
  MSG_Add("PROGRAM_INTRO_MOUNT_START",
          ...
          "\n");
  MSG_Add("PROGRAM_INTRO_MOUNT_EXST_WINDOWS",
          "\033[32mmount c c:\\dosgames\\\033[37m will create a C drive with "
          "c:\\dosgames as contents.");
  MSG_Add("PROGRAM_INTRO_MOUNT_EXEN_WINDOWS",
          "c:\\dosgames\\\033[37m is an example. Replace it with your own "
          "games directory.  \033[37m ");
  MSG_Add("PROGRAM_INTRO_MOUNT_EXST_OTHER",
          "\033[32mmount c ~/dosgames\033[37m will create a C drive with "
          "~/dosgames as contents.");
  MSG_Add("PROGRAM_INTRO_MOUNT_EXEN_OTHER",
          "\033[32m~/dosgames\033[37m is an example. Replace it with your own "
          "games directory.\033[37m  ");
  MSG_Add("PROGRAM_INTRO_MOUNT_END",
          "When the mount has successfully completed you can type "
          "\033[34;1mc:\033[0m to go to your freshly\n"
          "mounted C: drive. Typing \033[34;1mdir\033[0m there will show its "
          "contents."
          " \033[34;1mcd\033[0m will allow you to\n"
          "enter a directory (recognised by the \033[33;1m[]\033[0m in a "
          "directory listing).\n"
          "You can run programs/files which end with \033[31m.exe .bat\033[0m "
          "and \033[31m.com\033[0m.\n");
  MSG_Add("PROGRAM_INTRO_CDROM",
          "\033[2J\033[32;1mHow to mount a Real/Virtual CD-ROM Drive in "
          "DOSBox-X:\033[0m\n"
          "DOSBox-X provides CD-ROM emulation on several levels.\n"
          "\n"
          "The \033[33mbasic\033[0m level works on all CD-ROM drives and "
          "normal directories.\n"
          "It installs MSCDEX and marks the files read-only.\n"
          "Usually this is enough for most games:\n"
          "\033[34;1mmount d \033[0;31mD:\\\033[34;1m -t cdrom\033[0m   or   "
          "\033[34;1mmount d C:\\example -t cdrom\033[0m\n"
          "If it doesn't work you might have to tell DOSBox-X the label of the "
          "CD-ROM:\n"
          "\033[34;1mmount d C:\\example -t cdrom -label CDLABEL\033[0m\n"
          "\n"
          "The \033[33mnext\033[0m level adds some low-level support.\n"
          "Therefore only works on CD-ROM drives:\n"
          "\033[34;1mmount d \033[0;31mD:\\\033[34;1m -t cdrom -usecd "
          "\033[33m0\033[0m\n"
          "\n"
          "The \033[33mlast\033[0m level of support depends on your Operating "
          "System:\n"
          "For \033[1mWindows 2000\033[0m, \033[1mWindows XP\033[0m and "
          "\033[1mLinux\033[0m:\n"
          "\033[34;1mmount d \033[0;31mD:\\\033[34;1m -t cdrom -usecd "
          "\033[33m0 \033[34m-ioctl\033[0m\n"
          "For \033[1mWindows 9x\033[0m with a ASPI layer installed:\n"
          "\033[34;1mmount d \033[0;31mD:\\\033[34;1m -t cdrom -usecd "
          "\033[33m0 \033[34m-aspi\033[0m\n"
          "\n"
          "Replace \033[0;31mD:\\\033[0m with the location of your CD-ROM.\n"
          "Replace the \033[33;1m0\033[0m in \033[34;1m-usecd \033[33m0\033[0m "
          "with the number reported for your CD-ROM if you type:\n"
          "\033[34;1mmount -cd\033[0m\n");
  MSG_Add("PROGRAM_BOOT_NOT_EXIST",
  ...
  MSG_Add(
      ...
      "For this command, one can specify a succession of floppy disks "
      "swappable\n"
      "by the menu command, and drive: specifies the mounted drive to boot "
      "from.\n"
      "If no drive letter is specified, this defaults to boot from the A "
      "drive.\n"
      "If no parameter is specified, it will try to boot from the current "
      "drive.\n"
      "The only bootable drive letters are A, C, and D.  For booting from a "
      "hard\n"
      "drive (C or D), ensure the image is already mounted by "
      "\033[34;1mIMGMOUNT\033[0m command.\n\n"
      "The syntax of this command is one of the following:\n\n"
      "\033[34;1mBOOT [driveletter:] "
      "[-convertfat|-convertfatro|-noconvertfat]\033[0m\n\n"
      "\033[34;1mBOOT diskimg1.img [diskimg2.img ...] [-L "
      "driveletter]\033[0m\n\n"
      "Note: An image file with a leading colon (:) will be booted in "
      "write-protected\n"
      "mode if the \"leading colon write protect image\" option is enabled.\n\n"
      "Examples:\n\n"
      "\033[32;1mBOOT A:\033[0m       - boot from drive A: if it is mounted "
      "and bootable.\n"
      "\033[32;1mBOOT :DOS.IMG\033[0m - boot from floppy image DOS.IMG in "
  ...
  MSG_Add("PROGRAM_BOOT_SWAP_ALREADY",
          "Multiple disk images specified and another drive is already "
          "connected to the swap list");
  MSG_Add("PROGRAM_BOOT_NOT_SPECIFIED", "No images specified");
  MSG_Add("PROGRAM_BOOT_IS_PC88",
  ...
  MSG_Add("PROGRAM_LOADROM_HELP",
          "Loads the specified ROM image file for video BIOS or IBM "
          "BASIC.\n\nLOADROM ROM_file\n");
  ...
  MSG_Add("PROGRAM_BIOSTEST_HELP", "Boots into a BIOS image for running CPU "
                                   "tester BIOS.\n\nBIOSTEST image_file\n");
  ...
  MSG_Add("VHD_INVALID_DATA",
          "The specified VHD file is corrupt and cannot be opened.\n");
  MSG_Add("VHD_UNSUPPORTED_TYPE",
          "The specified VHD file is of an unsupported type.\n");
  MSG_Add("VHD_ERROR_OPENING_PARENT",
          "The parent of the specified VHD file could not be found.\n");
  MSG_Add("VHD_PARENT_INVALID_DATA", "The parent of the specified VHD file is "
                                     "corrupt and cannot be opened.\n");
  MSG_Add("VHD_PARENT_UNSUPPORTED_TYPE",
          "The parent of the specified VHD file is of an unsupported type.\n");
  MSG_Add("VHD_PARENT_INVALID_MATCH",
          "The parent of the specified VHD file does not contain the expected "
          "identifier.\n");
  MSG_Add("VHD_PARENT_INVALID_DATE", "The parent of the specified VHD file has "
                                     "been changed and cannot be loaded.\n");

  MSG_Add("PROGRAM_IMGMOUNT_SPECIFY_DRIVE",
          "Must specify drive letter to mount image at.\n");
  MSG_Add("PROGRAM_IMGMOUNT_SPECIFY2",
          "Must specify drive number (0 to %d) to mount image at "
          "(0,1=fda,fdb;2,3=hda,hdb).\n");
  /*MSG_Add("PROGRAM_IMGMOUNT_SPECIFY_GEOMETRY",
      "For \033[33mCD-ROM\033[0m images:   \033[34;1mIMGMOUNT drive-letter
     location-of-image -t iso\033[0m\n"
      "\n"
      "For \033[33mhardrive\033[0m images: Must specify drive geometry for hard
     drives:\n" "bytes_per_sector, sectors_per_cylinder, heads_per_cylinder,
     cylinder_count.\n"
      "\033[34;1mIMGMOUNT drive-letter location-of-image -size
     bps,spc,hpc,cyl\033[0m\n");*/
  ...
  MSG_Add("PROGRAM_IMGMOUNT_AUTODET_VALUES",
          "Image geometry auto detection: -size %u,%u,%u,%u\n");
  MSG_Add("PROGRAM_IMGMOUNT_TYPE_UNSUPPORTED",
          "Type \"%s\" is unsupported. Specify \"hdd\" or \"floppy\" or "
          "\"iso\".\n");
  MSG_Add("PROGRAM_IMGMOUNT_FORMAT_UNSUPPORTED",
          "Format \"%s\" is unsupported. Specify \"fat\" or \"iso\" or "
          "\"none\".\n");
  ...
  MSG_Add("PROGRAM_IMGMOUNT_MULTIPLE_NOTSUPPORTED",
          "Multiple disk images not supported for that drive\n");
  MSG_Add("PROGRAM_IMGMOUNT_HD_FDPOSITION",
          "Cannot mount hard drive in floppy position.\n");
  MSG_Add("PROGRAM_IMGMOUNT_FD_HDPOSITION",
          "Cannot mount floppy in hard drive position.\n");
  MSG_Add("PROGRAM_IMGMOUNT_NOT_ASSIGNED",
  ...
  MSG_Add("PROGRAM_IMGMOUNT_NOT_MOUNTED_NUMBER",
          "Drive number %d is not mounted.\n");
  MSG_Add("PROGRAM_IMGMOUNT_UMOUNT_USAGE",
  ...
  MSG_Add("PROGRAM_IMGMOUNT_CHOOSE_LETTER",
          "Partitions cannot be mounted in conflict with the standard INT 13h "
          "hard disk\n"
          "allotment. Choose a different drive letter to mount to.");
  MSG_Add("PROGRAM_IMGMOUNT_ELTORITO_NO_FILE",
          "Do not specify files when mounting floppy drives from El Torito "
          "bootable CDs\n");
  MSG_Add("PROGRAM_IMGMOUNT_RAMDRIVE_NO_FILE",
          "Do not specify files when mounting RAM drives\n");
  MSG_Add("PROGRAM_IMGMOUNT_INVALID_SECTORSIZE",
          "Sector size must be larger than 512 bytes and evenly divide the "
          "image cluster size of %lu bytes.\n");
  MSG_Add("PROGRAM_IMGMOUNT_OPEN_ERROR", "Unable to open '%s'\n");
  MSG_Add("PROGRAM_IMGMOUNT_QCOW2_INVALID",
  ...
  MSG_Add("PROGRAM_IMGMOUNT_DOS_VERSION",
          "Mounting this image file requires a reported DOS version of %u.%u "
          "or higher.\n%s");
  MSG_Add("PROGRAM_IMGMOUNT_INVALID_FLOPPYSIZE",
          "Floppy size not recognized\n");

  MSG_Add(
      "PROGRAM_IMGMOUNT_HELP",
      "Mounts floppy, hard drive and optical disc images.\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive\033[0m \033[36;1mfile\033[0m "
      "[-ro] [-t floppy] [-fs fat] [-size ss,s,h,c]\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive\033[0m \033[36;1mfile\033[0m "
      "[-ro] [-t hdd] [-fs fat] [-size ss,s,h,c] [-ide controller]\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdriveNum\033[0m "
      "\033[36;1mfile\033[0m [-ro] [-fs none] [-size ss,s,h,c] [-reservecyl "
      "#]\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive\033[0m \033[36;1mfile\033[0m "
      "[-t iso] [-fs iso]\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive\033[0m [-t floppy] -bootcd "
      "cdDrive (or -el-torito cdDrive)\n"
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive\033[0m -t ram -size size\n"
      "\033[32;1mIMGMOUNT\033[0m -u \033[37;1mdrive|driveNum\033[0m (or "
      "\033[32;1mIMGMOUNT\033[0m \033[37;1mdrive|driveNum\033[0m "
      "\033[36;1mfile\033[0m [options] -u)\n"
      " \033[37;1mdrive\033[0m               Drive letter to mount the image "
      "at.\n"
      " \033[37;1mdriveNum\033[0m            Drive number to mount, where 0-1 "
      "are FDDs, 2-5 are HDDs.\n"
      " \033[36;1mfile\033[0m                Image filename(s), or "
      "\033[33;1mIMGMAKE.IMG\033[0m if not specified.\n"
      " -t iso              Image type is optical disc iso or cue / bin "
      "image.\n"
      " -t hdd|floppy|ram   Image type is hard disk (inc. VHD/HDI)|floppy|RAM "
      "drive.\n"
      " -fs iso             Filesystem is ISO 9660 (auto-assumed for .iso/.cue "
      "files).\n"
      " -fs fat             Filesystem is FAT - FAT12, FAT16 and FAT32 are "
      "supported.\n"
      " -fs none            Do not detect filesystem (auto-assumed for drive "
      "numbers).\n"
      " -reservecyl #       Report # number of cylinders less than actual in "
      "BIOS.\n"
      " -ide controller     Specify IDE controller (1m, 1s, 2m, 2s) to mount "
      "drive.\n"
      " -size size|ss,s,h,c Specify the size in KB, or sector size and CHS "
      "geometry.\n"
      " -bootcd cdDrive     Specify the CD drive to load the bootable floppy "
      "from.\n"
      " -o partidx=#        Specify a hard disk partition number to mount as "
      "drive.\n"
      " -ro                 Mount image(s) read-only (or leading ':' for "
      "read-only).\n"
      " -u                  Unmount the drive or drive number.\n"
      " \033[32;1m-examples           Show some usage examples.\033[0m");
  MSG_Add("PROGRAM_IMGMOUNT_EXAMPLE",
          "Some usage examples of IMGMOUNT:\n\n"
          "  \033[32;1mIMGMOUNT\033[0m                       - list mounted "
          "FAT/ISO drives & drive numbers\n"
          "  \033[32;1mIMGMOUNT C\033[0m                     - mount hard disk "
          "image \033[33;1mIMGMAKE.IMG\033[0m as C:\n"
#ifdef WIN32
          "  \033[32;1mIMGMOUNT C c:\\image.img\033[0m        - mount hard "
          "disk image c:\\image.img as C:\n"
          "  \033[32;1mIMGMOUNT D c:\\files\\game.iso\033[0m   - mount CD "
          "image c:\\files\\game.iso as D:\n"
#else
          "  \033[32;1mIMGMOUNT C ~/image.img\033[0m         - mount hard disk "
          "image ~/image.img as C:\n"
          "  \033[32;1mIMGMOUNT D ~/files/game.iso\033[0m    - mount CD image "
          "~/files/game.iso as D:\n"
#endif
          "  \033[32;1mIMGMOUNT D cdaudio.cue\033[0m         - mount cue file "
          "of a cue/bin pair as CD drive\n"
          "  \033[32;1mIMGMOUNT 0 dos.ima\033[0m             - mount floppy "
          "image dos.ima as drive number 0\n"
          "                                   (\033[33;1mBOOT A:\033[0m will "
          "boot from drive if bootable)\n"
          "  \033[32;1mIMGMOUNT A -ro dos.ima\033[0m         - mount floppy "
          "image dos.ima as A: read-only\n"
          "  \033[32;1mIMGMOUNT A :dsk1.img dsk2.img\033[0m  - mount floppy "
          "images dsk1.img and dsk2.img as\n"
          "                                   A:, swappable via menu item "
          "\"Swap floppy\",\n"
          "                                   with dsk1.img read-only (but not "
          "dsk2.img)\n"
          "  \033[32;1mIMGMOUNT A -bootcd D\033[0m           - mount bootable "
          "floppy A: from CD drive D:\n"
          "  \033[32;1mIMGMOUNT C -t ram -size 10000\033[0m  - mount hard "
          "drive C: as a 10MB RAM drive\n"
          "  \033[32;1mIMGMOUNT D d.img -o partidx=4\033[0m  - mount 1st "
          "logical partition of d.img as D:\n"
          "  \033[32;1mIMGMOUNT C disk.img -u\033[0m         - force mount "
          "hard disk image disk.img as C:,\n"
          "                                   auto-unmount drive beforehand if "
          "necessary\n"
          "  \033[32;1mIMGMOUNT A -u\033[0m                  - unmount "
          "previously-mounted drive A:\n");
  MSG_Add(
      "PROGRAM_IMGMAKE_SYNTAX",
      "Creates floppy or hard disk images.\n"
      "Usage: \033[34;1mIMGMAKE [file] [-t type] [[-size size] | [-chs "
      "geometry]] [-spc] [-nofs]\033[0m\n"
      "  \033[34;1m[-bat] [-fat] [-fatcopies] [-rootdir] [-force]"
#ifdef WIN32
      " [-source source] [-retries #]"
#endif
      "\033[0m\n"
      "  file: Image file to create (or \033[33;1mIMGMAKE.IMG\033[0m if not "
      "set) - \033[31;1mpath on the host\033[0m\n"
      "  -t: Type of image.\n"
      "    \033[33;1mFloppy disk templates\033[0m (names resolve to floppy "
      "sizes in KB or fd=fd_1440):\n"
      "     fd_160 fd_180 fd_200 fd_320 fd_360 fd_400 fd_720 fd_1200 fd_1440 "
      "fd_2880\n"
      "    \033[33;1mHard disk templates:\033[0m\n"
      "     hd_250: 250MB, hd_520: 520MB, hd_1gig: 1GB, hd_2gig: 2GB, hd_4gig: "
      "4GB,\n"
      "     hd_8gig: 8GB, hd_st251: 40MB, hd_st225: 20MB (geometry from old "
      "drives)\n"
      "    \033[33;1mCustom hard disks:\033[0m hd vhd (requires -size or "
      "-chs)\n"
      ...
      "  -force: Overwrite existing image file. -chs / -lba: Choose C/H/S or "
      "LBA.\n"
      "  -bat: Create a .bat file with the IMGMOUNT command required for this "
      "image.\n"
      ...
      "  -align: Align filesystem structures. In sectors, or add 'K' suffix "
      "for KB.\n"
#ifdef WIN32
      "  -source: drive letter - if specified the image is read from a floppy "
      "disk.\n"
      "  -retries: how often to retry when attempting to read a bad floppy "
      "disk(1-99).\n"
#endif
      "  \033[32;1m-examples: Show some usage examples.\033[0m");
  MSG_Add("IMAGEMOUNT_CHANGE_DOSVER",
          "Mounting this type of disk images requires a reported DOS version "
          "of %s or higher.\n"
          "Do you want to auto - change the reported DOS version to %s now and "
          "mount the disk image ? ");
  MSG_Add("PROGRAM_IMGMAKE_EXAMPLE",
          "Some usage examples of IMGMAKE:\n\n"
          "  \033[32;1mIMGMAKE -t fd\033[0m                   - create a "
          "1.44MB floppy image \033[33;1mIMGMAKE.IMG\033[0m\n"
          "  \033[32;1mIMGMAKE -t fd_1440 -force\033[0m       - force to "
          "create a floppy image \033[33;1mIMGMAKE.IMG\033[0m\n"
          "  \033[32;1mIMGMAKE dos.img -t fd_2880\033[0m      - create a "
          "2.88MB floppy image named dos.img\n"
          "  \033[32;1mIMGMAKE new.vhd -t vhd -size 520\033[0m- create a 520MB "
          "Dynamic VHD named new.vhd\n"
#ifdef WIN32
          "  \033[32;1mIMGMAKE c:\\disk.img -t hd -size 50\033[0m      - "
          "create a 50MB HDD image c:\\disk.img\n"
          "  \033[32;1mIMGMAKE c:\\disk.img -t hd_520 -nofs\033[0m     - "
          "create a 520MB blank HDD image\n"
          "  \033[32;1mIMGMAKE c:\\disk.img -t hd_2gig -fat 32\033[0m  - "
          "create a 2GB FAT32 HDD image\n"
          "  \033[32;1mIMGMAKE c:\\disk.img -t hd -chs 130,2,17\033[0m - "
          "create a HDD image of specified CHS\n"
          "  \033[32;1mIMGMAKE c:\\disk.img -source a\033[0m           - read "
          "image from physical drive A:\n"
#else
          "  \033[32;1mIMGMAKE ~/disk.img -t hd -size 50\033[0m       - create "
          "a 50MB HDD image ~/disk.img\n"
          "  \033[32;1mIMGMAKE ~/disk.img -t hd_520 -nofs\033[0m      - create "
          "a 520MB blank HDD image\n"
          "  \033[32;1mIMGMAKE ~/disk.img -t hd_2gig -fat 32\033[0m   - create "
          "a 2GB FAT32 HDD image\n"
          "  \033[32;1mIMGMAKE ~/disk.img -t hd -chs 130,2,17\033[0m  - create "
          "a HDD image of specified CHS\n"
#endif
  );
  ...
  MSG_Add("PROGRAM_IMGMAKE_FLREAD2",
          "%s =good, %s =good after retries, ! =CRC error, x =sector not "
          "found, ? =unknown\n\n");
#endif
  MSG_Add("PROGRAM_IMGMAKE_FILE_EXISTS",
          "The file \"%s\" already exists. You can specify \"-force\" to "
          "overwrite.\n");
  MSG_Add("PROGRAM_IMGMAKE_CANNOT_WRITE",
          "The file \"%s\" cannot be opened for writing.\n");
  MSG_Add("PROGRAM_IMGMAKE_NOT_ENOUGH_SPACE",
          "Not enough space available for the image file. Need %llu bytes.\n");
  MSG_Add("PROGRAM_IMGMAKE_PRINT_CHS", "Creating image file \"%s\" with %u "
                                       "cylinders, %u heads and %u sectors\n");
  ...
  MSG_Add("PROGRAM_IMGMAKE_VOLUME_ALIGN",
          "Sanity check failed: Volume size not aligned\n");
  MSG_Add("PROGRAM_IMGMAKE_FAT_ALIGN",
          "Sanity check failed: FAT tables not aligned\n");
  MSG_Add("PROGRAM_IMGMAKE_SECTPERFAT",
          "Error: Generated filesystem has more than 256 sectors per FAT and "
          "is not FAT32\n");
  MSG_Add("PROGRAM_IMGMAKE_VOLSIZE",
          "Sanity check failed: Volume size not aligned\n");
  MSG_Add("PROGRAM_IMGMAKE_CLUSTERS", "Error: Generated filesystem has too few "
                                      "clusters given the parameters\n");

  MSG_Add("PROGRAM_IMGMAKE_CLUSTERCOUNT",
          "Warning: Cluster count is too high given the volume size. Reporting "
          "a\n         smaller sector count.\n");
  MSG_Add("PROGRAM_IMGMAKE_CLUSTER_ALIGN",
          "Sanity check failed: First cluster not aligned\n");
  MSG_Add("PROGRAM_IMGMAKE_CLUSTER_SIZE",
          "WARNING: Cluster sizes >= 64KB are not compatible with MS-DOS and "
          "SCANDISK\n");

  MSG_Add("PROGRAM_KEYB_INFO", "Codepage %i has been loaded\n");
  MSG_Add("PROGRAM_KEYB_INFO_LAYOUT",
          "Codepage %i has been loaded for layout %s\n");
  MSG_Add(
      "PROGRAM_KEYB_SHOWHELP",
      "Configures a keyboard for a specific language.\n\n"
      "Usage: \033[32;1mKEYB\033[0m [keyboard layout ID [codepage number "
      "[codepage file]]]\n"
      "  layout ID        Name of layout or keyboard file (.kl) to load\n"
      "  codepage number  Number of codepage to load\n"
      "  codepage file    .cpi/.cpx file including specified codepage "
      "information\n"
      "  (.kl, .cpi/.cpx files shall be placed under the directory where "
      "dosbox-x\n"
      "   executable exists.)\n\n"
      "Some examples:\n"
      "  \033[32;1mKEYB\033[0m          Display currently loaded codepage.\n"
      "  \033[32;1mKEYB sp\033[0m       Load the Spanish (SP) layout, use an "
      "appropriate codepage.\n"
      "  \033[32;1mKEYB sp 850\033[0m   Load the Spanish (SP) layout, use "
      "codepage 850.\n"
      "  \033[32;1mKEYB sp 850 mycp.cpi\033[0m Same as above, but use file "
      "mycp.cpi.\n"
      "  \033[32;1mKEYB sp_mod 850\033[0m      Load keyboard layout from "
      "sp_mod.kl, use codepage 850.\n");
  MSG_Add("PROGRAM_KEYB_NOERROR",
          "Keyboard layout %s loaded for codepage %i\n");
  MSG_Add("PROGRAM_KEYB_FILENOTFOUND",
          "Keyboard file %s not found (or not a valid ID)\n\n");
  ...
  MSG_Add("PROGRAM_MODE_USAGE",
          ...
          "\033[1mCO80\033[0m, \033[1mBW80\033[0m, \033[1mCO40\033[0m, "
          "\033[1mBW40\033[0m, or \033[1mMONO\033[0m\n"
          "\033[34;1mMODE CON COLS=\033[0mc \033[34;1mLINES=\033[0mn :columns "
          "and lines, c=80 or 132, n=25, 43, 50, or 60\n"
          "\033[34;1mMODE CON RATE=\033[0mr \033[34;1mDELAY=\033[0md "
          ":typematic rates, r=1-32 (32=fastest), d=1-4 (1=lowest)\n");
  ...
  MSG_Add("PROGRAM_MODE_RATE_DELAY",
          "Rate and delay must be specified together\n");

  MSG_Add("PROGRAM_PORT_INVALID_NUMBER",
          "Must specify a port number between 1 and 9.\n");
  MSG_Add("PROGRAM_VHDMAKE_WRITERR",
          "Could not write to new VHD image \"%s\", aborting.\n");
  MSG_Add("PROGRAM_VHDMAKE_REMOVEERR", "Could not erase file \"%s\"\n");
  MSG_Add("PROGRAM_VHDMAKE_RENAME",
          "You'll have to manually rename the newly created VHD image.\n");
  MSG_Add("PROGRAM_VHDMAKE_SUCCESS",
          "New VHD image successfully created. You can mount it with "
          "\033[34;1mIMGMOUNT\033[0m.\n");
  MSG_Add("PROGRAM_VHDMAKE_ERROPEN",
          "Error, could not open image file \"%s\".\n");
  MSG_Add("PROGRAM_VHDMAKE_BADSIZE", "Bad VHD size specified, aborting!\n");
  MSG_Add("PROGRAM_VHDMAKE_FNEEDED",
          "A pre-existing VHD image can't be silently overwritten without -f "
          "option!\n");
  MSG_Add(
  ...
  MSG_Add("PROGRAM_VHDMAKE_INFO",
          "VHD \"%s\" type is %s.\nIts virtual size is %.02f MB");
  MSG_Add("PROGRAM_VHDMAKE_CANTMERGE",
          "%s is not a Differencing disk, can't merge!\n");
  MSG_Add("PROGRAM_VHDMAKE_MERGEREPORT",
          "%d sectors in %d blocks from \"%s\" merged into \"%s\".\n");
  MSG_Add("PROGRAM_VHDMAKE_MERGENODELETE",
          "Couldn't remove snapshot \"%s\", you'll have to do it yourself!");
  MSG_Add("PROGRAM_VHDMAKE_MERGEOKDELETE",
          "Snapshot VHD merged and deleted.\n");
  MSG_Add("PROGRAM_VHDMAKE_MERGEFAILED", "Failure while merging, aborted!\n");
  MSG_Add("PROGRAM_VHDMAKE_MERGEWARNCORRUPTION",
          " Parent \"%s\" contents could be corrupted!\n");
  MSG_Add("PROGRAM_VHDMAKE_ABSPATH",
          "Warning: an absolute path to parent prevents portability.\nPlease "
          "prefer a path relative to the differencing image file!\n");
  MSG_Add(
      "PROGRAM_VHDMAKE_HELP",
      "Creates Dynamic or Differencing VHD images, converts raw images into "
      "Fixed VHD,\n"
      ...
      "\033[32;1mVHDMAKE\033[0m [-f] \033[34;1m-link\033[0m parent.vhd "
      "new.vhd\n"
      "\033[32;1mVHDMAKE\033[0m \033[34;1m-merge\033[0m delta.vhd\n"
      "\033[32;1mVHDMAKE\033[0m \033[34;1m-info\033[0m a.vhd\n"
      " -c | -convert  convert a raw hd image to Fixed VHD, renaming it to "
      "new.vhd\n"
      " -l | -link     create a new Differencing VHD new.vhd and link it to "
      "the\n"
      ...
      " size           disk size (eventually with size unit, Bytes is "
      "implicit)\n"
      "When converting a raw disk image to Fixed VHD, it has to be partitioned "
      "with\n"
      "MBR scheme and formatted with FAT format.\n"
      "When creating a Dynamic VHD, its size must range from 3 MB to 2040 GB.\n"
      "The Dynamic VHD created is not partitioned nor formatted: to directly "
      "mount to\n"
      "a drive letter with \033[34;1mIMGMOUNT\033[0m, please consider using "
      "\033[34;1mIMGMAKE\033[0m instead.\n"
      "A merged snapshot VHD is automatically deleted if merge is "
      "successful.\n");
  MSG_Add(
      "PROGRAM_COLOR_HELP",
  ...
  MSG_Add(
      ...
      "INT2FDBG [option]\n  /I      Installs hook\n\n"
      "It will hook INT 2Fh at the top of the call chain for debugging "
      "information.\n\n"
  ...
  MSG_Add("PROGRAM_CFGTOOL_HELP",
          ...
          "CFGTOOL\n\n"
          "Note: You can also use CONFIG command for command-line "
          "configurations.\n");
  MSG_Add(
      "PROGRAM_IMGSWAP_HELP",
      "Swaps floppy, hard drive and optical disc images.\n\n"
      "\033[32;1mIMGSWAP\033[0m \033[37;1mdrive\033[0m "
      "\033[36;1m[position]\033[0m\n"
  ...
  MSG_Add("PROGRAM_INTRO_HELP",
          ...
          "[CDROM|MOUNT|USAGE|WELCOME]\n");
  MSG_Add("PROGRAM_ELTORITO_LETTER",
          "El Torito emulation requires a proper CD-ROM drive letter\n");
  MSG_Add("PROGRAM_ELTORITO_DRIVE_EXISTS",
          "El Torito CD-ROM drive specified already exists as a non-CD-ROM "
          "device\n");
  MSG_Add("PROGRAM_ELTORITO_NOT_CDDRIVE",
          "El Torito CD-ROM drive specified is not actually a CD-ROM drive\n");
  MSG_Add("PROGRAM_ELTORITO_REQUIRE_FLOPPY",
          "El Torito emulation must be used with -t floppy at this time\n");
  MSG_Add("PROGRAM_ELTORITO_NO_BOOT_RECORD",
          "El Torito CD-ROM boot record not found\n");
  MSG_Add("PROGRAM_ELTORITO_ENTRY_UNREADABLE",
          "El Torito entries unreadable\n");
  MSG_Add("PROGRAM_ELTORITO_NO_BOOTABLE_FLOPPY",
          "El Torito bootable floppy not found\n");
  MSG_Add("PROGRAM_ELTORITO_BOOTABLE_SECTION",
          "Unable to locate bootable section\n");
  MSG_Add("PROGRAM_ELTORITO_BOOTSECTOR", "El Torito boot sector unreadable\n");
  MSG_Add("PROGRAM_ELTORITO_ISOMOUNT",
  ...
  MSG_Add("PROGRAM_START_HELP_WIN",
          ...
          "START opens the Windows command prompt automatically to run these "
          "commands\n"
          "and wait for a key press before exiting (specified by "
          "\"startincon\" option):\n%s\n\n"
          "Note: The path specified in this command is the path on the host "
          "system.\n");
  ...
  MSG_Add("PROGRAM_START_HOST_ERROR", "Error: START cannot launch application "
                                      "to run on your current host system.\n");
  MSG_Add("PROGRAM_START_LAUNCH_ERROR",
  ...
  MSG_Add("PROGRAM_UTF8_CONVERSION_ERROR",
  ...
  MSG_Add("PROGRAM_VTEXT_HELP",
          "Changes V-text mode for the DOS/V emulation.\n\n"
          "VTEXT [mode]\n\n[mode] can be 0, 1, 2, for no V-text, V-text 1, and "
          "V-text 2 respectively.\n\n"
          "Type VTEXT without a parameter to show the current V-text mode "
          "status.\n");
  MSG_Add("PROGRAM_NMITEST_HELP",
          "Generates a non-maskable interrupt (NMI).\n\n"
          "NMITEST\n\nNote: This is a debugging tool to test if the interrupt "
          "handler works properly.\n");
  MSG_Add("PROGRAM_NMITEST_GENERATE_NMI",
          "Generating a non-maskable interrupt (NMI)...\n");
  MSG_Add("PROGRAM_CAPMOUSE_HELP",
  ...
  MSG_Add(
      ...
      "  -w WAIT: seconds before typing begins. Two second default; max of "
      "30.\n"
      "  -p PACE: seconds between each keystroke. Half-second default; max of "
      "10.\n\n"
  ...
  MSG_Add("PROGRAM_AUTOTYPE_NO_BINDINGS",
          "AUTOTYPE: The mapper has no key bindings\n");
  MSG_Add("PROGRAM_AUTOTYPE_CLAMP_VALUE",
  ...
  MSG_Add(
      ...
      "ADDKEY [pmsec] [key]\n\n"
      "For example, the command below types \"dir\" followed by ENTER after 1 "
      "second:\n\n"
  ...
  MSG_Add(
      ...
      "To change the current background and foreground colors, use COLOR "
      "command.\n");
  ...
  MSG_Add("PROGRAM_SETCOLOR_MONO_MODE7",
          "MONO mode status => active (video mode 7)\n");
  MSG_Add("PROGRAM_SETCOLOR_MONO_MODE3",
  ...
  MSG_Add("PROGRAM_SWITCH_LANG",
          "You have changed the active code page to %d. Do you want to load "
          "language file %s for this code page ?");
  MSG_Add(
  ...
  MSG_Add("PROGRAM_MOUNT_SELECT_DRIVE",
          "Select a drive/directory to mount for Drive %s: as %s");
  MSG_Add("PROGRAM_MOUNT_CDROM_SUPPORT",
  ...
  MSG_Add("PROGRAM_WARN_QUICKLAUNCH",
  ...
  MSG_Add("MENU_DRIVE_NOTEXIST",
          "Drive does not exist or is mounted from disk image.");
  MSG_Add("MENU_SAVE_IMAGE_FAILED", "Failed to save disk image.");
  MSG_Add("MENU_JP_CPONLY",
          "This function is only available for the Japanese code page (932).");
  MSG_Add("MENU_CH_CPONLY", "This function is only available for the Chinese "
                            "code pages (936, 950, or 951).");
  MSG_Add("MENU_GLIDE_ERROR", "Glide passthrough cannot be enabled. Check the "
                              "Glide wrapper installation.");
  MSG_Add("MENU_HIGH_INTENSITY_ERROR",
  ...
  MSG_Add(
      ...
      "(You may append 'forced' to the pixelshader setting to force load the "
      "pixel shader without this message)");
  MSG_Add("PIXEL_SHADER_LOADED", "Loaded pixel shader - %s");
  MSG_Add("MAPPEREDITOR_NOT_AVAILABLE",
          "Mapper Editor is not currently available.");
  MSG_Add("OPL_REC_COMPLETED", "Saved Raw OPL output to the file:\n\n%s");
  MSG_Add("OPL_CAPTURE_FAILED",
          "Cannot capture Raw OPL output because ESFM native mode is being "
          "used by the current "
          "application, which is not supported by the Raw OPL format.");
  MSG_Add("TTF_DBCS_ONLY", "This function is only available for the "
                           "Chinese/Japanese/Korean code pages.");
  ...
  const Section_prop *dos_section =
      static_cast<Section_prop *>(control->GetSection("dos"));
