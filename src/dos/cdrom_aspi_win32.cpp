...
// always use a buffer of the maximum struct size (like the union of all 'SRB_*'
// struct types) Thanx SaPu
typedef union {
...
bool GetRegistryValue(HKEY &hKey, char *valueName, char *buffer,
                      ULONG bufferSize)
// hKey has to be open
{
  // Read subkey
  ULONG valType;
  ULONG result;
  result = RegQueryValueEx(hKey, valueName, NULL, &valType,
...
BYTE CDROM_Interface_Aspi::GetHostAdapter(char *hardwareID) {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (sh.hainquiry.SRB_Status != SS_COMP)
      continue;
...
// hKey has to be open
{
  ...
  ULONG index = 0;
  do {
    result = RegEnumKeyEx(hKeyBase, index, &subKey[0], &subKeySize, NULL, NULL,
                          0, &time);
    if (result == ERROR_SUCCESS) {
      // Open Key...
      newKeyResult = RegOpenKeyEx(hKeyBase, subKey, 0, KEY_READ, &hNewKey);
      if (newKeyResult == ERROR_SUCCESS) {
        static const char drive_letter_assignment[] =
            "CurrentDriveLetterAssignment";
        if (GetRegistryValue(hNewKey, (char *)&drive_letter_assignment, buffer,
                             256)) {
          ...
          if (buffer[0] == letter) {
            ...
            bool v2 =
                GetRegistryValue(hNewKey, (char *)SCSI_TargetID, buffer, 256);
            LOG(LOG_MISC, LOG_NORMAL)("SCSI: SCSITargetID found: %s", buffer);
            target = buffer[0] - '0';
            static const char Hardware_ID[] = "HardwareID";
            bool v3 =
                GetRegistryValue(hNewKey, (char *)Hardware_ID, hardwareID, 256);
            RegCloseKey(hNewKey);
            if (v1 && v2 && v3) {
...
bool CDROM_Interface_Aspi::GetVendor(BYTE HA_num, BYTE SCSI_Id, BYTE SCSI_Lun,
                                     char *szBuffer) {
  ASPI_SRB srbExec;
...
bool CDROM_Interface_Aspi::SetDevice(char *path, int forceCD) {
  ...
  if ((osi.dwPlatformId == VER_PLATFORM_WIN32_NT) && (osi.dwMajorVersion > 4)) {
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &hKeyBase) ==
        ERROR_SUCCESS) {
...
bool CDROM_Interface_Aspi::GetAudioTracks(int &stTrack, int &endTrack,
                                          TMSF &leadOut) {
  TOC toc;
...
bool CDROM_Interface_Aspi::GetAudioTrackInfo(int track, TMSF &start,
                                             unsigned char &attr) {
  TOC toc;
  if (GetTOC((LPTOC)&toc) == SS_COMP) {
...
void CDROM_Interface_Aspi::GetIOCTLAdapter(HANDLE hF, int *iDA, int *iDT,
                                           int *iDL) {
  char szBuf[1024];
  PSCSI_ADDRESS pSA;
  DWORD dwRet;
...
bool CDROM_Interface_Aspi::PlayAudioSector(unsigned long start,
                                           unsigned long len) {
  //	SRB_ExecSCSICmd s;
  ASPI_SRB s;
  DWORD dwStatus;
...
bool CDROM_Interface_Aspi::StopAudio(void) { return PauseAudio(false); };
...
bool CDROM_Interface_Aspi::GetAudioSub(unsigned char &attr,
                                       unsigned char &track,
  ...
  if (s.execscsicmd.SRB_Status != SS_COMP)
    return false;
...
bool CDROM_Interface_Aspi::GetUPC(unsigned char &attr, char *upcdata) {
  ...
  for (int i = 0; i < 7; i++)
    upcdata[i] = upc.MediaCatalog[i];
  for (int i = 0; i < 7; i++)
    upcdata[i] =
        (upc.MediaCatalog[i * 2] << 4) | (upc.MediaCatalog[i * 2 + 1] & 0x0F);
...
bool CDROM_Interface_Aspi::GetMediaTrayStatus(bool &mediaPresent,
                                              bool &mediaChanged,
                                              bool &trayOpen) {
...
bool CDROM_Interface_Aspi::ReadSectors(PhysPt buffer, bool raw,
...
bool CDROM_Interface_Aspi::ReadSectorsHost(void *buffer, bool raw,
                                           unsigned long sector,
                                           unsigned long num) {
  (void)buffer;
  (void)raw;
