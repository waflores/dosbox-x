...
typedef size_t (*f_iconv)(iconv_t cd, /* const */ char **inbuf,
                          size_t *inbytesleft, char **outbuf,
                          size_t *outbytesleft);
typedef int *(*f_errno)(void);
typedef int (*f_mbtowc)(csconv_t *cv, const uchar *buf, int bufsize,
                        ushort *wbuf, int *wbufsize);
typedef int (*f_wctomb)(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
                        int bufsize);
...
static int win_iconv_open(rec_iconv_t *cd, const char *tocode,
                          const char *fromcode);
static int win_iconv_close(iconv_t cd);
static size_t win_iconv(iconv_t cd, /* const */ char **inbuf,
                        size_t *inbytesleft, char **outbuf,
                        size_t *outbytesleft);
...
#if defined(USE_LIBICONV_DLL)
static int libiconv_iconv_open(rec_iconv_t *cd, const char *tocode,
                               const char *fromcode);
static PVOID MyImageDirectoryEntryToData(LPVOID Base, BOOLEAN MappedAsImage,
...
static int kernel_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
                         ushort *wbuf, int *wbufsize);
static int kernel_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
                         int bufsize);
static int mlang_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
                        ushort *wbuf, int *wbufsize);
static int mlang_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
                        int bufsize);
static int utf16_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
                        ushort *wbuf, int *wbufsize);
static int utf16_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
                        int bufsize);
static int utf32_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
                        ushort *wbuf, int *wbufsize);
static int utf32_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
                        int bufsize);
static int iso2022jp_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
                            ushort *wbuf, int *wbufsize);
static int iso2022jp_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize,
...
static struct {
  ...
  const char *name;
    ...
    {950, "big5"}, /* ANSI/OEM Traditional Chinese (Taiwan; Hong Kong SAR, PRC);
                      Chinese Traditional (Big5) */
    {950, "big5hkscs"}, /* ANSI/OEM Traditional Chinese (Hong Kong SAR); Chinese
                           Traditional (Big5-HKSCS) */
    {950, "big5-hkscs"}, /* alternative name for it */
    {1026, "IBM1026"},   /* IBM EBCDIC Turkish (Latin 5) */
    {1047, "IBM01047"},  /* IBM EBCDIC Latin 1/Open System */
    {1140, "IBM01140"},  /* IBM EBCDIC US-Canada (037 + Euro symbol); IBM EBCDIC
                            (US-Canada-Euro) */
    {1141, "IBM01141"},  /* IBM EBCDIC Germany (20273 + Euro symbol); IBM EBCDIC
                            (Germany-Euro) */
    {1142, "IBM01142"},  /* IBM EBCDIC Denmark-Norway (20277 + Euro symbol); IBM
                            EBCDIC (Denmark-Norway-Euro) */
    {1143, "IBM01143"},  /* IBM EBCDIC Finland-Sweden (20278 + Euro symbol); IBM
                            EBCDIC (Finland-Sweden-Euro) */
    {1144, "IBM01144"},  /* IBM EBCDIC Italy (20280 + Euro symbol); IBM EBCDIC
                            (Italy-Euro) */
    {1145, "IBM01145"}, /* IBM EBCDIC Latin America-Spain (20284 + Euro symbol);
                           IBM EBCDIC (Spain-Euro) */
    {1146, "IBM01146"}, /* IBM EBCDIC United Kingdom (20285 + Euro symbol); IBM
                           EBCDIC (UK-Euro) */
    {1147, "IBM01147"}, /* IBM EBCDIC France (20297 + Euro symbol); IBM EBCDIC
                           (France-Euro) */
    ...
    {20105, "x-IA5"}, /* IA5 (IRV International Alphabet No. 5, 7-bit); Western
                         European (IA5) */
    ...
    {28592,
     "iso8859-2"}, /* ISO 8859-2 Central European; Central European (ISO) */
    {28592, "iso_8859-2"},
    {28592, "iso_8859_2"},
    ...
    /* 50935 		EBCDIC Simplified Chinese Extended and Simplified
       Chinese */
    ...
    {54936, "GB18030"}, /* Windows XP and later: GB18030 Simplified Chinese (4
                           byte); Chinese Simplified (GB18030) */
...
static compat_t cp932_compat[] = {
    {0x00A5, 0x005C, COMPAT_OUT}, {0x203E, 0x007E, COMPAT_OUT},
...
typedef HRESULT(WINAPI *CONVERTINETSTRING)(LPDWORD lpdwMode,
                                           DWORD dwSrcEncoding,
                                           DWORD dwDstEncoding, LPCSTR lpSrcStr,
                                           LPINT lpnSrcSize, LPBYTE lpDstStr,
                                           LPINT lpnDstSize);
typedef HRESULT(WINAPI *CONVERTINETMULTIBYTETOUNICODE)(
    LPDWORD lpdwMode, DWORD dwSrcEncoding, LPCSTR lpSrcStr,
    LPINT lpnMultiCharCount, LPWSTR lpDstStr, LPINT lpnWideCharCount);
...
typedef HRESULT(WINAPI *ISCONVERTINETSTRINGAVAILABLE)(DWORD dwSrcEncoding,
                                                      DWORD dwDstEncoding);
typedef HRESULT(WINAPI *LCIDTORFC1766A)(LCID Locale, LPSTR pszRfc1766,
                                        int nChar);
typedef HRESULT(WINAPI *LCIDTORFC1766W)(LCID Locale, LPWSTR pszRfc1766,
                                        int nChar);
...
static int load_mlang(void) {
  ...
  IsConvertINetStringAvailable = (ISCONVERTINETSTRINGAVAILABLE)GetProcAddressA(
...
static int win_iconv_close(iconv_t cd UNUSED) { return 0; }

static size_t win_iconv(iconv_t _cd, /* const */ char **inbuf,
                        size_t *inbytesleft, char **outbuf,
                        size_t *outbytesleft) {
  rec_iconv_t *cd = (rec_iconv_t *)_cd;
  ...
  while (*inbytesleft != 0) {
    ...
    insize = cd->from.mbtowc(&cd->from, (const uchar *)*inbuf, *inbytesleft,
                             wbuf, &wsize);
    if (insize == -1) {
    ...
    outsize =
        cd->to.wctomb(&cd->to, wbuf, wsize, (uchar *)*outbuf, *outbytesleft);
    if (outsize == -1) {
      if ((cd->to.flags & FLAG_IGNORE) && errno != E2BIG) {
        cd->to.mode = tomode;
        outsize = 0;
 ...
 */
static int mbtowc_flags(int codepage) {
  return (codepage == 50220 || codepage == 50221 || codepage == 50222 ||
          codepage == 50225 || codepage == 50227 || codepage == 50229 ||
          codepage == 52936 || codepage == 54936 ||
          (codepage >= 57002 && codepage <= 57011) || codepage == 65000 ||
          codepage == 42)
             ? 0
 ...
 */
static int must_use_null_useddefaultchar(int codepage) {
  return (codepage == 65000 || codepage == 65001 || codepage == 50220 ||
          codepage == 50221 || codepage == 50222 || codepage == 50225 ||
          codepage == 50227 || codepage == 50229 || codepage == 52936 ||
          codepage == 54936 || (codepage >= 57002 && codepage <= 57011) ||
          codepage == 42);
}
...
static int libiconv_iconv_open(rec_iconv_t *cd, const char *tocode,
  ...
  if (_iconv_open == NULL || cd->iconv_close == NULL || cd->iconv == NULL ||
      cd->_errno == NULL)
    goto failed;
...
static FARPROC find_imported_function(HMODULE hModule, const char *funcname) {
  ...
  Imp = (PIMAGE_IMPORT_DESCRIPTOR)MyImageDirectoryEntryToData(
      (LPVOID)Base, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT, &Size);
  if (Imp == NULL)
    return NULL;
  for (; Imp->OriginalFirstThunk != 0; ++Imp) {
    ...
    for (; Name->u1.Ordinal != 0; ++Name, ++Address) {
      if (!IMAGE_SNAP_BY_ORDINAL(Name->u1.Ordinal)) {
        ImpName =
            (PIMAGE_IMPORT_BY_NAME)(Base + (DWORD_PTR)Name->u1.AddressOfData);
...
static int sbcs_mblen(csconv_t *cv UNUSED, const uchar *buf UNUSED,
                      int bufsize UNUSED) {
  return 1;
}
...
static int mbcs_mblen(csconv_t *cv, const uchar *buf, int bufsize) {
  ...
  if (cv->codepage == 54936) {
    if (buf[0] <= 0x7F)
      ...
      len = 2;
    else if (buf[0] >= 0x81 && buf[0] <= 0xFE && bufsize >= 4 &&
...
static int kernel_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
  ...
  if (cv->codepage == 20127 && buf[0] >= 0x80)
    return seterror(EILSEQ);
  *wbufsize =
      MultiByteToWideChar(cv->codepage, mbtowc_flags(cv->codepage),
...
static int kernel_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
  ...
  len = WideCharToMultiByte(cv->codepage, flags, (const wchar_t *)wbuf,
                            wbufsize, (char *)buf, bufsize, NULL, p);
  if (len == 0) {
...
static int mlang_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
  ...
  if (len == -1)
    return -1;
  insize = len;
  hr = ConvertINetMultiByteToUnicode(&cv->mode, cv->codepage, (const char *)buf,
                                     &insize, (wchar_t *)wbuf, wbufsize);
...
static int mlang_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf,
  ...
  hr = ConvertINetUnicodeToMultiByte(&cv->mode, cv->codepage,
                                     (const wchar_t *)wbuf, &wbufsize, tmpbuf,
                                     &tmpsize);
...
static iso2022_esc_t iso2022jp_esc[] = {
    ...
    {"\x1B\x24\x40", 3, 2,
     ISO2022JP_CS_JISX0208_1983}, /* unify 1978 with 1983 */
...
static int iso2022jp_mbtowc(csconv_t *cv, const uchar *buf, int bufsize,
  ...
  insize = len + esc_len;
  hr = ConvertINetMultiByteToUnicode(&dummy, cv->codepage, (const char *)tmp,
                                     &insize, (wchar_t *)wbuf, wbufsize);
...
static int iso2022jp_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize,
  ...
  hr = ConvertINetUnicodeToMultiByte(
      &dummy, cv->codepage, (const wchar_t *)wbuf, &wbufsize, tmp, &tmpsize);
  if (hr != S_OK || insize != wbufsize)
    return seterror(EILSEQ);
  else if (bufsize < tmpsize)
    return seterror(E2BIG);
...
int main(int argc, char **argv) {
  ...
  while ((inbytesleft = fread(inbuf + rest, 1, sizeof(inbuf) - rest, in)) !=
             0 ||
         rest != 0) {
    inbytesleft += rest;
    pin = inbuf;
