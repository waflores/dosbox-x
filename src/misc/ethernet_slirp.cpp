...
// libslirp >= 4.8.0 defines slirp_ssize_t but will break compatibility with
// older versions
#ifdef _WIN32
SSIZE_T
#else
...
bool SlirpEthernetConnection::Initialize(Section *dosbox_config) {
  ...
  if (slirp) {
    ...
    forwarded_tcp_ports =
        SetupPortForwards(is_udp, section->Get_string("tcp_port_forwards"));
    is_udp = true;
    ClearPortForwards(is_udp, forwarded_udp_ports);
    forwarded_udp_ports =
        SetupPortForwards(is_udp, section->Get_string("udp_port_forwards"));

    LOG_MSG("SLIRP: Successfully initialized");
    niclist = "You have currently enabled the slirp backend for NE2000 "
              "Ethernet emulation.\nTo show a list of network interfaces "
              "please enable the pcap backend instead.\nCheck [ne2000] and "
              "[ethernet, pcap] sections of the DOSBox-X configuration.";
    return true;
  } else {
...
void SlirpEthernetConnection::ClearPortForwards(
    const bool is_udp, std::map<int, int> &existing_port_forwards) {
  ...
  for (const auto &pair : existing_port_forwards) {
    ...
    if (slirp_remove_hostfwd(slirp, is_udp, host_addr, host_port) >= 0)
      LOG_MSG("SLIRP: Removed old %s port %d:%d forward", protocol, host_port,
              guest_port);
    else
      LOG_MSG("SLIRP: Failed removing old %s port %d:%d forward", protocol,
              host_port, guest_port);
  }
  existing_port_forwards.clear();
}

std::map<int, int> SlirpEthernetConnection::SetupPortForwards(
    const bool is_udp, const std::string &port_forward_rules) {
  ...
  for (auto &forward_rule : split(port_forward_rules, ' ')) {
    ...
    if (forward_rule_parts.size() != 2) {
      LOG_MSG("SLIRP: Invalid %s port forward rule: %s", protocol,
              forward_rule.c_str());
      continue;
    }
    ...
    for (const auto &port_range_part : forward_rule_parts) {
      ...
      if (port_range.size() != 2) {
        LOG_MSG("SLIRP: Invalid %s port range: %s", protocol,
                port_range_part.c_str());
        break;
      }
    ...
    // If both halves of the rule are valid, we will have four ports in the
    // vector
    if (ports.size() != 4) {
      LOG_MSG("SLIRP: Invalid %s port forward rule: %s", protocol,
              forward_rule.c_str());
    ...
    // Sanity check that the maximum range doesn't go beyond the maximum port
    // number
    constexpr auto min_valid_port = 1;
    constexpr auto max_valid_port = 65535;
    const auto range = std::max(host_port_end - host_port_start,
                                guest_port_end - guest_port_start);
    if (host_port_start < min_valid_port ||
        host_port_start + range > max_valid_port ||
        guest_port_start < min_valid_port ||
        guest_port_start + range > max_valid_port) {
      LOG_MSG("SLIRP: Invalid %s port range: %s", protocol,
              forward_rule.c_str());
      continue;
    }
    ...
    LOG_MSG("SLIRP: Processing %s port forward rule: %s", protocol,
            forward_rule.c_str());
    while (n--) {
      // Add the port forward rule
      if (slirp_add_hostfwd(slirp, is_udp, bind_addr, host_port, bind_addr,
                            guest_port) == 0) {
        LOG_MSG("SLIRP: Setup %s port %d:%d forward", protocol, host_port,
                guest_port);
        forwarded_ports[host_port] = guest_port;
      } else {
        LOG_MSG("SLIRP: Failed setting up %s port %d:%d forward", protocol,
                host_port, guest_port);
      }
      ++host_port;
      ++guest_port;
...
struct slirp_timer *SlirpEthernetConnection::TimerNew(SlirpTimerCb cb,
                                                      void *cb_opaque) {
...
void SlirpEthernetConnection::TimerMod(struct slirp_timer *timer,
                                       int64_t expire_time) {
...
void SlirpEthernetConnection::PollsClear() { polls.clear(); }
