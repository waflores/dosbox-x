...
EthernetConnection *OpenEthernetConnection(std::string backendstr) {
  ...
  if (backend == "nothing") {
    ...
    settings = control->GetSection(
        "ethernet, pcap"); // NTS: The Nothing uses no settings, but there is an
                           // assert below to ensure settings != NULL
    if (!conn->Initialize(settings)) {
      delete conn;
      conn = NULL;
    }
  ...
  if (!conn) {
    if (backend == "pcap" || backend == "slirp")
      LOG_MSG("ETHERNET: Backend not supported in this build: %s",
              backend.c_str());
    ...
    LOG_MSG("ETHERNET: NE2000 Ethernet emulation backend selected: %s",
            backend.c_str());
  }
