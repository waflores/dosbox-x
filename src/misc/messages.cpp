...
bool uselangcp =
    false; // True if Language file is loaded via CONFIG -set langcp option. Use
           // codepage specified in the language file
bool isSupportedCP(int newCP),
    CodePageHostToGuestUTF8(char *d /*CROSS_LEN*/, const char *s /*CROSS_LEN*/),
    CodePageGuestToHostUTF8(char *d /*CROSS_LEN*/, const char *s /*CROSS_LEN*/),
    systemmessagebox(char const *aTitle, char const *aMessage,
                     char const *aDialogType, char const *aIconType,
                     int aDefaultButton),
    OpenGL_using(void);
void InitFontHandle(void), ShutFontHandle(void), refreshExtChar(void),
    SetIME(void), runRescan(const char *str), menu_update_dynamic(void),
    menu_update_autocycle(void), update_bindbutton_text(void),
    set_eventbutton_text(const char *eventname, const char *buttonname),
    JFONT_Init(), DOSBox_SetSysMenu(), UpdateSDLDrawTexture(),
    makestdcp950table(), makeseacp951table();
std::string langname = "", langnote = "", GetDOSBoxXPath(bool withexe = false);
extern int lastcp, FileDirExistUTF8(std::string &localname, const char *name),
    toSetCodePage(DOS_Shell *shell, int newCP, int opt);
extern bool dos_kernel_disabled, force_conversion, showdbcs, dbcs_sbcs,
    enableime, tonoime, chinasea, CHCP_changed;
...
std::list<MessageBlock> LangList; // list of translation messages, order
                                  // maintained in inserted order

// Map for fast lookup
std::unordered_map<std::string, std::list<MessageBlock>::iterator> LangMap;

void MSG_Add(const char *_name,
             const char *_val) { // add messages to the translation message list
  // If the message already exists, ignore it
  if (LangMap.find(_name) != LangMap.end())
    return;
...
const char *
MSG_Get(const char *msg) { // add messages to the translation message list
...
std::string formatString(const char *format, ...) {
  /**
   * @brief Generates a formatted string using a format specifier and variable
   * arguments.
   *
   * @param format A format string (e.g., "File: '%s', Error Code: %d, Language:
   * '%s'.")
   * @param ...    Values corresponding to format specifiers (e.g.,
   * "lang_en.msg", 404, "English").
  ...
  std::vector<char> buffer(size +
                           1); // Allocate buffer (+1 for null-terminator)
  va_start(args, format);
...
bool InitCodePage() {
  if (!dos.loaded_codepage || dos_kernel_disabled || force_conversion) {
    if (((control->opt_langcp && msgcodepage != dos.loaded_codepage) ||
         uselangcp) &&
        msgcodepage > 0) {
    ...
    if (section != NULL && !control->opt_noconfig && !IS_PC98_ARCH &&
        !IS_JEGA_ARCH && !IS_DOSV) {
...
void AddMessages() {
  ...
  MSG_Add("CONFIGFILE_INTRO",
          "# This is the configuration file for DOSBox-X %s. (Please use the "
          "latest version of DOSBox-X)\n"
          "# Lines starting with a # are comment lines and are ignored by "
          "DOSBox-X.\n"
          "# They are used to (briefly) document the effect of each option.\n"
          "# To write out ALL options, use command 'config -all' with -wc or "
          "-writeconf options.\n");
  MSG_Add("CONFIG_SUGGESTED_VALUES", "Possible values");
  MSG_Add("CONFIG_ADVANCED_OPTION",
          "Advanced options (see full configuration reference file "
          "[dosbox-x.reference.full.conf] for more details)");
  ...
  MSG_Add(
      "INTRO_MESSAGE",
      "Welcome to DOSBox-X, a free and complete DOS emulation "
      "package.\nDOSBox-X creates a DOS shell which looks like the plain "
  ...
  MSG_Add("SELECT_DIFFERENT_EVENT",
          "Select an event or press Add/Del/Next buttons.");
  MSG_Add("PRESS_JOYSTICK_KEY",
  ...
  MSG_Add("LANG_LOAD_ERROR", "Could not load language message file %s. The "
                             "default language will be used.");
  MSG_Add("LANG_JP_INCOMPATIBLE",
          "You have specified a language file which uses a code page "
          "incompatible with the Japanese PC-98 or JEGA/AX system.\n\n"
          "Are you sure to use the language file for this machine type?");
  MSG_Add("LANG_DOSV_INCOMPATIBLE",
          "You have specified a language file which uses a code page "
          "incompatible with the current DOS/V system.\n\n"
          "Are you sure to use the language file for this system type?");
  MSG_Add("LANG_CHANGE_CP",
          "The specified language file uses code page %d. Do you want to "
          "change to this code page accordingly?");
}

// True if specified codepage is a DBCS codepage
bool CheckDBCSCP(int32_t codepage) {
  if (codepage == 932 || codepage == 936 || codepage == 949 ||
      codepage == 950 || codepage == 951) {
...
void LoadMessageFile(const char *fname) {
  ...
  while (fgets(buffer, buffer_size, mfile)) {
    ...
    if (linein[0] == ':') {
      ...
      if (!strncasecmp(linein.c_str() + 1, "DOSBOX-X:", 9)) {
        ...
        if (colon_pos != std::string::npos && colon_pos > 0 &&
          ...
          if (key == "CODEPAGE") {
            ...
            if (!isSupportedCP(c)) {
              LOG_MSG("Language file: Invalid codepage :DOSBOX-X:CODEPAGE:%d",
                      c);
              loadlangcp = false;
            } else if (((IS_PC98_ARCH || IS_JEGA_ARCH) && c != 437 &&
                        c != 932 &&
              ...
              if (c != dos.loaded_codepage &&
                  (control->opt_langcp || uselangcp || !CHCP_changed ||
                   CheckDBCSCP(c) || !loadlang ||
                   (loadlang &&
                    systemmessagebox("DOSBox-X language file", msg.c_str(),
                                     "yesno", "question", 1)))) {
                loadlangcp = true;
                if (c == 950 && dos.loaded_codepage == 951)
                  msgcodepage = 951; // zh_tw defaults to CP950, but CP951 is
                                     // acceptable as well so keep it
                else if (c == 951 && dos.loaded_codepage == 950)
                  msgcodepage =
                      950; // And vice versa for lang files requiring CP951
                else
                  msgcodepage = c;
      ...
      } else if (!strncasecmp(linein.c_str() + 1, "MENU:", 5) &&
                 linein.size() > 6) {
        name.clear();
        mapper_name.clear();
        menu_name = linein.substr(6);
      } else if (!strncasecmp(linein.c_str() + 1, "MAPPER:", 7) &&
                 linein.size() > 8) {
        name.clear();
        menu_name.clear();
      ...
      if (!name.empty()) {
        ...
        for (char c = 'A'; c <= 'Z'; ++c) {
          std::string mname =
              "drive_" + std::string(1, c) + menu_name.substr(5);
      ...
      size_t temp_size =
          linein.size() * 3 + 1; // Converting to UTF-8 will expand to max.
                                 // 3-bytes / character + null terminator
...
bool MSG_Write(const char *location, const char *name) {
  ...
  for (auto &id : master_list) {
    if (!id.is_allocated() || id.get_type() == DOSBoxMenu::separator_type_id ||
        ...
        (id.get_name().size() > 9 &&
         id.get_name().substr(0, 8) == "command_") ||
        ...
        (id.get_name().size() > 9 && id.get_name().substr(0, 6) == "drive_" &&
         id.get_name()[6] >= 'B' && id.get_name()[6] <= 'Z' &&
         id.get_name()[7] == '_') ||
    ...
    std::string idname =
        (id.get_name().size() > 9 && id.get_name().substr(0, 8) == "drive_A_")
...
void MSG_Init() {
  ...
  if (control->opt_lang != "") {
    ...
    if (control->opt_langcp && msgcodepage > 0 && isSupportedCP(msgcodepage)) {
      ...
      cstr[0] = 0;
      if (sec != NULL) {
        char *countrystr = (char *)sec->Get_string("country"),
             *r = strchr(countrystr, ',');
        if (r != NULL)
          *r = 0;
        if (strlen(countrystr) > 10)
          countrystr[0] = 0;
        sprintf(cstr, "%s,%d", countrystr, msgcodepage);
        SetVal("config", "country", cstr);
        const char *imestr = section->Get_string("ime");
        if (tonoime && !strcasecmp(imestr, "auto") &&
            CheckDBCSCP(msgcodepage)) {
          tonoime = false;
          enableime = true;
          SetIME();
        }
