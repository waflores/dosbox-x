...
#ifdef C_SDL2
extern SDL_AudioDeviceID SDL2_AudioDevice; /* valid IDs are 2 or higher, 1 for
                                              compat, 0 is never a valid ID */
#endif
...
void GFX_LosingFocus(void), GFX_ReleaseMouse(void), MAPPER_ReleaseAllKeys(void),
    resetFontSize(void);
...
public:
  ...
  void set(int value) { slot = value; }

  operator size_t() const { return slot; }
...
size_t GetGameState(void) { return currentSlot; }
...
void SetGameState(int value) {
  ...
  LOG_MSG("Active save slot: %d %s", (int)currentSlot + 1,
          emptySlot ? "[Empty]" : "");
}
...
void LoadGameState(bool pressed) {
  ...
  //    {
  //        LOG_MSG("[%s]: State %d is empty!", getTime().c_str(), currentSlot +
  //        1); return;
  //    }
  if (!GFX_IsFullscreen() && render.aspect)
    GFX_LosingFocus();
  try {
...
std::string GetPlatform(bool save) {
  ...
  if (!save)
    strcat(platform, (std::string(SDL_STRING) + ", ").c_str());
#if defined(_M_X64) || defined(_M_AMD64) || defined(_M_ARM64) ||               \
    defined(_M_IA64) || defined(__ia64__) || defined(__LP64__) ||              \
    defined(_WIN64) || defined(__x86_64__) || defined(__aarch64__) ||          \
...
void ShowStateInfo(bool pressed) {
  ...
  std::string message =
      "Save to: " +
      (use_save_file && savefilename.size()
           ? "File " + savefilename
           : "Slot " + std::to_string(GetGameState_Run() + 1)) +
...
void AddSaveStateMapper() {
  ...
  MAPPER_AddHandler(SaveGameState, MK_s, MMODHOST, "savestate", "Save state",
                    &item);
  item->set_text("Save state");
  MAPPER_AddHandler(LoadGameState, MK_l, MMODHOST, "loadstate", "Load state",
                    &item);
  item->set_text("Load state");
  MAPPER_AddHandler(ShowStateInfo, MK_nothing, 0, "showstate",
                    "Display state info", &item);
  item->set_text("Display state information");
  MAPPER_AddHandler(PreviousSaveSlot, MK_comma, MMODHOST, "prevslot",
                    "Previous save slot", &item);
  item->set_text("Select previous slot");
  MAPPER_AddHandler(NextSaveSlot, MK_period, MMODHOST, "nextslot",
                    "Next save slot", &item);
...
char *itoa(int value, char *str, int radix) {
  ...
  do {
    ...
    *ptr++ = "zyxwvutsrqponmlkjihgfedcba9876543210123456789abcdefghijklmnopqrst"
             "uvwxyz"[35 + (tmp_value - value * radix)];
  } while (value);
...
void SaveState::registerComponent(const std::string &uniqueName,
                                  Component &comp) {
...
int zipOutOpenFile(zipFile zf, const char *zfname, zip_fileinfo &zi,
  ...
  return zipOpenNewFileInZip3_64(
      zf, zfname, &zi, NULL, 0, NULL, 0, NULL /* comment*/,
      (opt_compress_level != 0) ? Z_DEFLATED : 0, opt_compress_level, 0,
      -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, NULL /*password*/,
      0 /*crcFile*/, 1 /*zip64*/);
}

void SaveState::save(size_t slot) { // throw (Error)
  if (slot >= SLOT_COUNT * MAX_PAGE)
    return;
#ifdef C_SDL2
  ...
  if ((MEM_TotalPages() * 4096 / 1024 / 1024) > 1024) {
    LOG_MSG(
        "Stopped. 1 GB is the maximum memory size for saving/loading states.");
  ...
  if (auto_save_state)
    ...
    /* NTS: tinyfd_inputBox() returns a string from an internal statically
     * declared char array. It is not necessary to free the return string, but
     * it is important to understand that the next call to tinyfd_inputBox()
     * will obliterate the previously returned string. See
     * src/libs/tinyfiledialogs/tinyfiledialogs.c line 5069 --J.C. */
    /* NTS: The code was originally written to declare save_remark as char*
     * default assigned to string constant "", but GCC (rightfully so) complains
     * you're pointing char* at something that is stored const by the compiler.
     * "save_remark" is not modified past this point, so it has been changed to
     * const char* and the return value of tinyfd_inputBox() is given to a local
     * temporary char* string where the modification can be made, and *then*
     * assigned to the const char* string for the rest of this function. */
    ...
    GFX_LosingFocus();
    GFX_ReleaseMouse();
    char *new_remark = tinyfd_inputBox(
        "Save state",
        "Please enter remark for the state (optional; 30 characters maximum). "
        "Click the Cancel button to cancel the saving.",
        " ");
    MAPPER_ReleaseAllKeys();
    GFX_LosingFocus();
    if (fs && !GFX_IsFullscreen())
      GFX_SwitchFullScreen();
    if (new_remark == NULL)
      return;
    new_remark = trim(new_remark);
    if (strlen(new_remark) > 30)
      new_remark[30] = 0;
  ...
  slotname << slot + 1;
  temp = path;
  std::string save = use_save_file && savefilename.size()
                         ? savefilename
                         : temp + slotname.str() + ".sav";
...
#endif
    remove(save.c_str());
    zf = zipOpen2_64(save.c_str(), APPEND_STATUS_CREATE, &global_comment,
                     &ffunc);
  }
  if (zf == NULL) {
    save_err = true;
...

  {
    ...
    if ((errclose = zipOutOpenFile(zf, "DOSBox-X_Version", zi,
                                   compresssaveparts)) != ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream emulatorversion(&zos);

    emulatorversion << "DOSBox-X " << VERSION << " (" << SDL_STRING << ")"
                    << std::endl
                    << GetPlatform(true) << std::endl
                    << UPDATED_STR;
    /* 2025/01/12: Backwards compat: The old code compressed data to zlib, even
     * though the ZIP support code already applies compression. This is to tell
     * the old code that we did not compress the data (the ZIP support code did
     * though). */
    emulatorversion << std::endl << "No compression";

    if ((errclose = zos.close()) != ZIP_OK) {
      save_err = true;
      goto done;
    }
  }
  {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, "Program_Name", zi,
                                   compresssaveparts)) != ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream programname(&zos);
  ...
  }
  {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, "Memory_Size", zi, compresssaveparts)) !=
        ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream memorysize(&zos);
  ...
  }
  {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, "Machine_Type", zi,
                                   compresssaveparts)) != ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream machinetype(&zos);
  ...
  }
  {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, "Time_Stamp", zi, compresssaveparts)) !=
        ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream timestamp(&zos);
  ...
  }
  {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, "Save_Remark", zi, compresssaveparts)) !=
        ZIP_OK) {
      save_err = true;
      goto done;
    }
    zip_ostreambuf zos(zf);
    std::ostream saveremark(&zos);
  ...
  for (CompEntry::iterator i = components.begin(); i != components.end(); ++i) {
    zip_fileinfo zi;
    zipSetCurrentTime(zi);
    if ((errclose = zipOutOpenFile(zf, i->first.c_str(), zi,
                                   compresssaveparts)) != ZIP_OK) {
      save_err = true;
      goto done;
    }
...
void SaveState::load(size_t slot) const { // throw (Error)
  ...
  if (check_slot.fail()) {
    ...
    notifyError(use_save_file && savefilename.size()
                    ? "The selected save file is currently empty."
                    : "The selected save slot is an empty slot.",
                false);
    load_err = true;
    return;
  }
  ...
  {
    if ((err = unzLocateFile(zf, "DOSBox-X_Version", 1 /*case sensitive*/)) !=
        UNZ_OK) {
      load_err = true;
      goto done;
    }
    if ((err = unzGetCurrentFileInfo64(zf, &file_info, NULL, 0, NULL, 0, NULL,
                                       0)) != UNZ_OK) {
      load_err = true;
      goto done;
    }
    ...
    char buffer[4096];
    std::streamsize sz =
        zis.xsgetn((zip_istreambuf::char_type *)buffer, sizeof(buffer) - 1);
    buffer[sz] = 0;

    char *p;
    if (strstr(buffer, "\nNo compression") != NULL) {
      /* 2025/01/12: Removal of this string is required to match version string
       * below even if now ignored */
      p = strrchr(buffer, '\n');
      if (p != NULL)
        *p = 0;
    }
    ...
    if (p != NULL)
      *p = 0;
    std::string emulatorversion = std::string("DOSBox-X ") + VERSION +
                                  std::string(" (") + SDL_STRING +
                                  std::string(")");
  ...
  {
    if ((err = unzLocateFile(zf, "Program_Name", 1 /*case sensitive*/)) !=
        UNZ_OK) {
      load_err = true;
      goto done;
    }
    ...
    char buffer[4096];
    size_t length = (size_t)zis.xsgetn((zip_istreambuf::char_type *)buffer,
                                       sizeof(buffer) - 1);
    buffer[length] = 0;

    if (!length || (size_t)length != strlen(RunningProgram) ||
        strncmp(buffer, RunningProgram, length)) {
      ...
      if (length < 9) {
        ...
        void GFX_SetTitle(int32_t cycles, int frameskip, Bits timing,
                          bool paused);
  ...
  {
    if ((err = unzLocateFile(zf, "Memory_Size", 1 /*case sensitive*/)) !=
        UNZ_OK) {
      load_err = true;
      goto done;
    }
  ...
  {
    if ((err = unzLocateFile(zf, "Machine_Type", 1 /*case sensitive*/)) !=
        UNZ_OK) {
      load_err = true;
      goto done;
    }
  ...
  for (CompEntry::const_iterator i = components.begin(); i != components.end();
       ++i) {
    if ((err = unzLocateFile(zf, i->first.c_str(), 1 /*case sensitive*/)) !=
        UNZ_OK) {
      load_err = true;
      goto done;
    }
...
void SaveState::removeState(size_t slot) const {
  ...
  if (loadstateconfirm(4)) {
    ...
    check_slot.open(save.c_str(), std::ifstream::in);
    if (!check_slot.fail())
      notifyError("Failed to remove the state in the save slot.");
    if (page != GetGameState() / SaveState::SLOT_COUNT)
...
std::string SaveState::getName(size_t slot, bool nl) const {
  if (slot >= SLOT_COUNT * MAX_PAGE)
    return "[" + std::string(MSG_Get("EMPTY_SLOT")) + "]";
  std::string path;
  bool Get_Custom_SaveDir(std::string & savedir);
  if (Get_Custom_SaveDir(path)) {
    path += CROSS_FILESPLIT;
  ...
  std::stringstream slotname;
  slotname << slot + 1;
  std::string save = nl && use_save_file && savefilename.size()
                         ? savefilename
                         : temp + slotname.str() + ".sav";
  std::ifstream check_slot;
  check_slot.open(save.c_str(), std::ifstream::in);
  if (check_slot.fail())
    return nl ? "(Empty state)"
              : "[" + std::string(MSG_Get("EMPTY_SLOT")) + "]";
  check_slot.close();
  ...
  if ((err = unzLocateFile(zf, "Program_Name", 1 /*case sensitive*/)) ==
          UNZ_OK &&
      (err = unzGetCurrentFileInfo64(zf, &file_info, NULL, 0, NULL, 0, NULL,
                                     0)) == UNZ_OK) {
    if ((err = unzOpenCurrentFile(zf)) == UNZ_OK) {
      zip_istreambuf zis(zf);
      size_t length = (size_t)zis.xsgetn((zip_istreambuf::char_type *)buffer1,
                                         sizeof(buffer1) - 1);
      buffer1[length] = 0;
      zis.close();
      ret +=
          nl ? "Program: " + (!strlen(buffer1) ? "-" : std::string(buffer1)) +
                   "\n"
  ...
  if ((err = unzLocateFile(zf, "Time_Stamp", 1 /*case sensitive*/)) == UNZ_OK &&
    ...
    if ((err = unzOpenCurrentFile(zf)) == UNZ_OK) {
      ...
      zis.close();
      ret +=
          nl ? "Timestamp: " + (!strlen(buffer1) ? "-" : std::string(buffer1)) +
                   "\n"
             : " (" + std::string(buffer1) + ")";
    }
  }

  if ((err = unzLocateFile(zf, "Save_Remark", 1 /*case sensitive*/)) ==
          UNZ_OK &&
      (err = unzGetCurrentFileInfo64(zf, &file_info, NULL, 0, NULL, 0, NULL,
                                     0)) == UNZ_OK) {
    if ((err = unzOpenCurrentFile(zf)) == UNZ_OK) {
      ...
      if (length != 0)
        ret +=
            nl ? "Remark: " + (!strlen(buffer1) ? "-" : std::string(buffer1)) +
                     "\n"
               : " - " + std::string(buffer1);
    }
  }
