...
extern int autofixwarn, enablelfn, fat32setver, paste_speed, wheel_key,
    freesizecap, wpType, wpVersion, wpBG, wpFG, lastset, blinkCursor,
    msgcodepage;
extern bool dos_kernel_disabled, force_nocachedir, wpcolon, convertimg,
    lockmount, enable_config_as_shell_commands, lesssize, load, winrun,
    winautorun, startcmd, startwait, startquiet, starttranspath, mountwarning,
    wheel_guest, clipboard_dosapi, noremark_save_state, force_load_state,
    sync_time, manualtime, ttfswitch, loadlang, showbold, showital, showline,
    showsout, char512, printfont, rtl, gbk, chinasea, uao, showdbcs, dbcs_sbcs,
    autoboxdraw, halfwidthkana, ticksLocked, outcon, enable_dbcs_tables,
    show_recorded_filename, internal_program, pipetmpdev, notrysgf, uselangcp,
    incall;
...
static uint8_t exe_block[] = {
    ...
    0x30, 0xc0, // 0x10A XOR AL,AL     Clear AL (exit code). Program will write
                // AL to modify exit status
...
bool isDBCSCP(void),
    CheckBoxDrawing(uint8_t c1, uint8_t c2, uint8_t c3, uint8_t c4),
    DOS_SetAnsiAttr(uint8_t attr), GFX_GetPreventFullscreen(void),
    toOutput(const char *what);
void EMS_DoShutDown(void), UpdateDefaultPrinterFont(void),
    GFX_ForceRedrawScreen(void), resetFontSize(void), ttf_reset_colors(void),
    makestdcp950table(void), makeseacp951table(void), clearFontCache(void),
    DOSBox_SetSysMenu(void), MSG_Init(void), initRand(void), PRINTER_Init(void),
    SetKEYBCP(void);
void EMS_Startup(Section *sec), DOSV_SetConfig(Section_prop *section),
    DOSBOX_UnlockSpeed2(bool pressed),
    RebootLanguage(std::string filename, bool confirm = false),
    SetWindowTransparency(int trans), SetOutputSwitch(const char *outputstr),
    runRescan(const char *str), runSerial(const char *str),
    runParallel(const char *str), DOS_AddDays(uint8_t days),
    PRINTER_Shutdown(Section *sec), setAspectRatio(Section_prop *section);
...
static Bitu PROGRAMS_Handler(void) {
  ...
  if (ipe == NULL)
    E_Exit("Attempt to run internal program slot with nothing allocated");
  if (ipe->main == NULL)
    return CBRET_NONE;
  ...
  try { /* "BOOT" can throw an exception (int(2)) */
    new_program->Run();
    delete new_program;
  } catch (...) { /* well if it happened, free the program anyway to avert
                     memory leaks */
    delete new_program;
    throw; /* pass it on */
  }
...
Program::Program() {
  ...
  while (mem_readb(envscan))
    envscan += (PhysPt)(mem_strlen(envscan) + 1);
  envscan += 3;
  CommandTail tail;
...
void Program::WriteExitStatus() {
  /* the exe block was modified that on return to DOS only AH is modified,
   * leaving AL normally zero but we're free to set AL to any other value to set
   * exit code */
  reg_al = exit_status;
}
...
void Program::ChangeToLongCmd() {
   ...
   * it is also done for long arguments too as it is convenient (as the total
   * commandline can be longer than 127 characters. imgmount with lots of
   * parameters Length of arguments can be ~120. but switch when above 100 to be
   * sure
   */
...
void Program::WriteOut(const char *format, ...) {
  ...
  for (uint16_t i = 0; i < size; i++) {
    ...
    if (isDBCSCP() && isKanji1(buf[i]) && i + 1 < size &&
        isKanji2(buf[i + 1])) {
      uint8_t dbchar[3] = {(uint8_t)buf[i], (uint8_t)buf[i + 1], 0};
      s = 2;
      DOS_WriteFile(STDOUT, dbchar,
                    &s); // send two bytes if next character is a DBCS character
      i++;
      s = 1;
      continue;
    }
...
int Program::WriteOut_NoParsing(const char *format, bool dbcs) {
  ...
  for (uint16_t i = 0; i < size; i++) {
    ...
    if (buf[i] == 0xA) {
      ...
      if (outcon)
        rcount++;
    } else if (outcon && lead && CURSOR_POS_COL(page) == ncols - 1 &&
               !(TTF_using()
#if defined(USE_TTF)
                 && autoboxdraw
#endif
                 && CheckBoxDrawing(last3, last2, last_written_character,
                                    buf[i]))) {
      out = 0xD;
      DOS_WriteFile(STDOUT, &out, &s);
      out = 0xA;
...
static bool LocateEnvironmentBlock(PhysPt &env_base, PhysPt &env_fence,
                                   Bitu env_seg) {
  if (env_seg == 0) {
  ...
  DOS_MCB env_mcb((uint16_t)(env_seg - 1)); /* read the environment block's MCB
                                               to determine how large it is */
...
void EnvPhys_StrCpyToCPPString(std::string &result, PhysPt &env_scan,
                               PhysPt env_fence) {
  ...
  while (env_scan < env_fence) {
    char c;
    if ((c = (char)mem_readb(env_scan++)) == 0)
      break;
...
bool EnvPhys_ScanUntilNextString(PhysPt &env_scan, PhysPt env_fence) {
  ...
  while (env_scan < env_fence && mem_readb(env_scan) != 0)
    env_scan++;

  /* if we hit the fence, that's something to warn about. */
  if (env_scan >= env_fence) {
    LOG_MSG("Warning: environment string scan hit the end of the environment "
            "block without terminating NUL\n");
    return false;
  }
  ...
  if (mem_readb(env_scan) != 0) {
    LOG_MSG(
        "Warning: environment string scan scan stopped without hitting NUL\n");
    return false;
  }
...
bool Program::GetEnvStr(const char *entry, std::string &result) {
  ...
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: Program::GetEnvNum() called with DOS kernel disabled (such "
            "as OS boot).\n");
    return false;
  }

  if (!LocateEnvironmentBlock(env_base, env_fence, psp->GetEnvironment())) {
    LOG_MSG("Warning: GetEnvCount() was not able to locate the program's "
            "environment block\n");
    return false;
  }
  ...
  while (env_scan < env_fence) {
    /* "NAME" + "=" + "VALUE" + "\0" */
    /* end of the block is a NULL string meaning a \0 follows the last string's
     * \0 */
    if (mem_readb(env_scan) == 0)
    ...
    if (!EnvPhys_ScanUntilNextString(env_scan, env_fence))
      break;
  }
...
Bitu Program::GetEnvCount(void) {
  ...
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: Program::GetEnvCount() called with DOS kernel disabled (such "
            "as OS boot).\n");
    return 0;
  }
...
bool Program::FirstEnv(const char *entry) {
  ...
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: Program::FirstEnv() called with DOS kernel disabled (such as "
            "OS boot).\n");
    return false;
  }
  ...
  if (found) {
...

    {
      ...
      for (size_t i = 0; i < pl; i++)
        mem_writeb(env_last - (i + 1), mem_readb(env_first - (i + 1)));
...
bool Program::EraseEnv(void) {
  ...
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: Program::EraseEnv() called with DOS kernel disabled (such as "
            "OS boot).\n");
    return false;
  }

  if (!LocateEnvironmentBlock(env_base, env_fence, psp->GetEnvironment())) {
    LOG_MSG("Warning: SetEnv() was not able to locate the program's "
            "environment block\n");
    return false;
  }
...
bool Program::SetEnv(const char *entry, const char *new_string) {
  ...
  if (dos_kernel_disabled) {
    LOG_MSG("BUG: Program::SetEnv() called with DOS kernel disabled (such as "
            "OS boot).\n");
    return false;
  }
    ...
    if (EnvPhys_StrCmp(env_scan, env_fence, bigentry.c_str()) == 0) {
      ...
      if (nsl != 0) {
        if ((env_scan + needs) > env_fence) {
          LOG_MSG("Program::SetEnv() error, insufficient room for environment "
                  "variable %s=%s (replacement)\n",
      ...
      s = env_scan;
      d = env_scan;
      while (s < env_fence && mem_readb(s) != 0)
        s++;
      if (s < env_fence && mem_readb(s) == 0)
        s++;
      ...
      while (s < env_fence) {
        ...
        mem_writeb(d++, b);
        if (zeroes >= 2)
          break; /* two consecutive zeros means the end of the block */
      }
    } else {
      /* scan to next string */
      if (!EnvPhys_ScanUntilNextString(env_scan, env_fence))
  ...
  if (*new_string != 0) {
    ...
    for (const char *s = bigentry.c_str(); *s != 0;)
      mem_writeb(env_scan++, (uint8_t)(*s++));
    mem_writeb(env_scan++, '=');
...
void dos_ver_menu(bool start), ReloadMapper(Section_prop *sec, bool init),
    SetGameState_Run(int value), update_dos_ems_menu(void),
    MountAllDrives(bool quiet), GFX_SwitchFullScreen(void),
    RebootConfig(std::string filename, bool confirm = false);
...
void ApplySetting(std::string pvar, std::string inputline, bool quiet) {
  if (!strcasecmp(pvar.c_str(), "dosbox") || !strcasecmp(pvar.c_str(), "dos") ||
      !strcasecmp(pvar.c_str(), "dosv") || !strcasecmp(pvar.c_str(), "cpu") ||
      !strcasecmp(pvar.c_str(), "sdl") || !strcasecmp(pvar.c_str(), "ttf") ||
      !strcasecmp(pvar.c_str(), "render") ||
      !strcasecmp(pvar.c_str(), "serial") ||
      !strcasecmp(pvar.c_str(), "parallel") ||
      !strcasecmp(pvar.c_str(), "printer")) {
    Section_prop *section =
        static_cast<Section_prop *>(control->GetSection(pvar.c_str()));
    if (section != NULL) {
      if (!strcasecmp(pvar.c_str(), "dosbox")) {
        ...
        if (freesizestr == "fixed" || freesizestr == "false" ||
            freesizestr == "0")
          freesizecap = 0;
        ...
        if (!strcasecmp(inputline.substr(0, 9).c_str(), "saveslot="))
        ...
        if (!strcasecmp(inputline.substr(0, 23).c_str(),
                        "show recorded filename="))
        ...
        if (!strcasecmp(inputline.substr(0, 16).c_str(), "mapper send key=")) {
          ...
          if (mapsendkey == "winlogo")
            sendkeymap = 1;
          else if (mapsendkey == "winmenu")
            sendkeymap = 2;
          else if (mapsendkey == "alttab")
            sendkeymap = 3;
          else if (mapsendkey == "ctrlesc")
            sendkeymap = 4;
          else if (mapsendkey == "ctrlbreak")
            sendkeymap = 5;
          else
            sendkeymap = 0;
...
#else
        if (!strcasecmp(inputline.substr(0, 16).c_str(), "mapperfile_sdl1="))
          ReloadMapper(section, true);
        ...
        if (!strcasecmp(inputline.substr(0, 13).c_str(), "usescancodes=")) {
          void setScanCode(Section_prop * section), loadScanCode(),
              MAPPER_Init();
          setScanCode(section);
          loadScanCode();
          GFX_LosingFocus();
        ...
        if (!strcasecmp(inputline.substr(0, 15).c_str(), "windowposition=")) {
          ...
          if (posx < 0 || posy < 0) {
            ...
            if (SDL_GetDesktopDisplayMode(
                    GetDisplayNumber() ? GetDisplayNumber() - 1 : 0, &dm) ==
                0) {
              posx = (dm.w - w) / 2;
              posy = (dm.h - h) / 2;
            }
          ...
          if (GetDisplayNumber() > 0) {
            ...
            EnumDisplayMonitors(0, 0, EnumDispProc,
                                reinterpret_cast<LPARAM>(&xy));
            HMONITOR monitor =
                MonitorFromRect(&monrect, MONITOR_DEFAULTTONEAREST);
          ...
          MoveWindow(GetHWND(), posx, posy, rect.right - rect.left,
        ...
        if (!strcasecmp(inputline.substr(0, 15).c_str(), "autofixwarning=")) {
        ...
        } else if (!strcasecmp(inputline.substr(0, 16).c_str(),
                               "fat32setversion=")) {
          ...
          EMS_Startup(NULL);
          update_dos_ems_menu();
        } else if (!strcasecmp(inputline.substr(0, 32).c_str(),
                               "shell configuration as commands=")) {
        ...
        } else if (!strcasecmp(inputline.substr(0, 15).c_str(),
                               "starttranspath=")) {
          ...
          mainMenu.get_item("dos_win_wait")
              ...
              .refresh_item(mainMenu);
        } else if (!strcasecmp(inputline.substr(0, 11).c_str(),
                               "startquiet=")) {
        ...
        void ttf_reset(void), ttf_setlines(int cols, int lins),
            SetBlinkRate(Section_prop * section);
        if (!strcasecmp(inputline.substr(0, 5).c_str(), "font=")) {
#if defined(USE_TTF)
          if (TTF_using()) {
            std::string font = section->Get_string("font");
            if (font.empty() && !IS_PC98_ARCH && !isDBCSCP())
              notrysgf = true;
            ttf_reset();
            notrysgf = false;
            int missing = IS_PC98_ARCH ? 0 : setTTFMap(false);
            if (missing > 0 && first_shell)
              first_shell->WriteOut(MSG_Get("SHELL_CMD_CHCP_MISSING"), missing);
...
#endif
        } else if (!strcasecmp(inputline.substr(0, 7).c_str(), "ptsize=") ||
                   !strcasecmp(inputline.substr(0, 8).c_str(), "winperc=")) {
#if defined(USE_TTF)
          lesssize = true;
          if (TTF_using())
            ttf_reset();
          lesssize = false;
#endif
        } else if (!strcasecmp(inputline.substr(0, 5).c_str(), "lins=") ||
                   !strcasecmp(inputline.substr(0, 5).c_str(), "cols=")) {
#if defined(USE_TTF)
          if (TTF_using()) {
            ...
            if (iscol && IS_PC98_ARCH)
              ...
              if (quiet && first_shell)
                first_shell->ShowPrompt();
            } else {
              reg_ax = CurMode->mode;
...
#if defined(USE_TTF)
          if (TTF_using() && !strlen(section->Get_string("colors")))
            ttf_reset_colors();
#endif
          ...
          wpType = wpVersion = 0;
          if (strlen(wpstr) > 1) {
            if (!strncasecmp(wpstr, "WP", 2))
              wpType = 1;
            else if (!strncasecmp(wpstr, "WS", 2))
              wpType = 2;
            else if (!strncasecmp(wpstr, "XY", 3))
              wpType = 3;
            if (strlen(wpstr) > 2 && wpstr[2] >= '1' && wpstr[2] <= '9')
              wpVersion = wpstr[2] - '0';
          }
          mainMenu.get_item("ttf_wpno").check(!wpType).refresh_item(mainMenu);
          mainMenu.get_item("ttf_wpwp")
              .check(wpType == 1)
              .refresh_item(mainMenu);
          mainMenu.get_item("ttf_wpws")
              .check(wpType == 2)
              .refresh_item(mainMenu);
          mainMenu.get_item("ttf_wpxy")
              .check(wpType == 3)
              .refresh_item(mainMenu);
          mainMenu.get_item("ttf_wpfe")
              .check(wpType == 4)
              .refresh_item(mainMenu);
          if (TTF_using())
            resetFontSize();
          ...
          mainMenu.get_item("ttf_showbold")
              .check(showbold)
              .refresh_item(mainMenu);
          if (TTF_using())
            resetFontSize();
          ...
          mainMenu.get_item("ttf_showital")
              .check(showital)
              .refresh_item(mainMenu);
          if (TTF_using())
            resetFontSize();
          ...
          mainMenu.get_item("ttf_showline")
              .check(showline)
              .refresh_item(mainMenu);
          if (TTF_using())
            resetFontSize();
          ...
          mainMenu.get_item("ttf_showsout")
              .check(showsout)
              .refresh_item(mainMenu);
          if (TTF_using())
            resetFontSize();
          ...
          if (TTF_using())
            resetFontSize();
#endif
        } else if (!strcasecmp(inputline.substr(0, 12).c_str(),
                               "righttoleft=")) {
#if defined(USE_TTF)
          ...
          mainMenu.get_item("ttf_printfont")
              .check(printfont)
              .refresh_item(mainMenu);
          UpdateDefaultPrinterFont();
#endif
          ...
          if (TTF_using())
            resetFontSize();
#endif
        } else if (!strcasecmp(inputline.substr(0, 14).c_str(),
                               "halfwidthkana=")) {
#if defined(USE_TTF)
          ...
          if (gbk != section->Get_bool("gbk")) {
            gbk = !gbk;
            if (enable_dbcs_tables && dos.tables.dbcs &&
                (IS_PDOSV || dos.loaded_codepage == 936))
              mem_writeb(Real2Phys(dos.tables.dbcs) + 2, gbk ? 0x81 : 0xA1);
            clearFontCache();
            if (dos.loaded_codepage != 950 && dos.loaded_codepage != 951)
              mainMenu.get_item("ttf_extcharset")
                  .check(dos.loaded_codepage == 936 ? gbk : (gbk && chinasea))
                  .refresh_item(mainMenu);
#if defined(USE_TTF)
            if (TTF_using() && dos.loaded_codepage == 936)
              resetFontSize();
#endif
          }
          ...
          if (chinasea != section->Get_bool("chinasea")) {
            ...
            if (dos.loaded_codepage != 936)
              mainMenu.get_item("ttf_extcharset")
                  .check(dos.loaded_codepage == 950 ||
                                 dos.loaded_codepage == 951
                             ? chinasea
                             : (gbk && chinasea))
                  .refresh_item(mainMenu);
            if ((TTF_using() || showdbcs) &&
...
#if defined(USE_TTF)
          showdbcs = showdbcsstr == "true" || showdbcsstr == "1" ||
                     (showdbcsstr == "auto" && (loadlang || dbcs_sbcs));
#else
          showdbcs = showdbcsstr == "true" || showdbcsstr == "1" ||
                     (showdbcsstr == "auto" && loadlang);
#endif
          if (!IS_EGAVGA_ARCH)
            showdbcs = false;
        } else if (!strcasecmp(inputline.substr(0, 11).c_str(),
                               "fepcontrol=") ||
                   !strcasecmp(inputline.substr(0, 7).c_str(), "vtext1=") ||
                   !strcasecmp(inputline.substr(0, 7).c_str(), "vtext2="))
        ...
        if (!strcasecmp(inputline.substr(0, 6).c_str(), "serial") &&
            inputline[7] == '=') {
          std::string val = section->Get_string("serial" +
                                                std::string(1, inputline[6])),
                      cmd = std::string(1, inputline[6]) + " " +
                            (val.size() ? val : "dummy");
          runSerial(cmd.c_str());
        }
        ...
        if (!strcasecmp(inputline.substr(0, 8).c_str(), "parallel") &&
            inputline[9] == '=') {
          std::string val = section->Get_string("parallel" +
                                                std::string(1, inputline[8])),
                      cmd = std::string(1, inputline[8]) + " " +
                            (val.size() ? val : "disabled");
...
void CONFIG::Run(void) {
  static const char *const params[] = {"-r",
                                       "-wcp",
                                       "-wcd",
                                       "-wc",
                                       "-writeconf",
...
#undef P_ALL
  ...
  enum prs {
    ...
    P_WRITECONF,
    P_WRITECONF2,
    P_WRITECONF_PORTABLE_REBOOT,
    P_WRITECONF_DEFAULT_REBOOT,
    P_WRITECONF_REBOOT,
    P_WRITECONF2_REBOOT,
    P_BOOTCONF,
    P_BOOTCONF2,
  ...
  if (cmd->FindExist("-setup", true))
    all = 2;
  else if (cmd->FindExist("-all", true))
    all = 1;
  else if (cmd->FindExist("-mod", true))
    all = 0;
  if (cmd->FindExist("-norem", true))
    norem = true;
...
#define P_ALL __P_ALL
    ...
    case P_LISTCONF: {
      ...
      if (res != NULL)
        WriteOut(MSG_Get("PROGRAM_CONFIG_WORKDIR"), cwd);
      if (size == 0 && !configfile.size())
        ...
        if (configfile.size())
          WriteOut(MSG_Get("PROGRAM_CONFIG_GLOBAL_CONF"), configfile.c_str());
      }
    ...
    case P_WRITECONF:
    case P_WRITECONF2:
    case P_WRITECONF_REBOOT:
    case P_WRITECONF2_REBOOT:
      if (securemode_check())
        return;
      if (pvars.size() > 1)
        return;
        ...
        if (presult == P_WRITECONF_REBOOT || presult == P_WRITECONF2_REBOOT)
        ...
        if (control->configfiles.size()) {
          ...
          if (presult == P_WRITECONF_REBOOT || presult == P_WRITECONF2_REBOOT)
            RebootConfig(control->configfiles[0]);
        } else
          WriteOut(MSG_Get("PROGRAM_CONFIG_NOCONFIGFILE"));
      }
      break;
    case P_WRITECONF_DEFAULT:
    case P_WRITECONF_DEFAULT_REBOOT: {
      ...
      if (presult == P_WRITECONF_DEFAULT_REBOOT)
        RebootConfig(confname);
      break;
    }
    case P_WRITECONF_PORTABLE:
    case P_WRITECONF_PORTABLE_REBOOT:
      if (securemode_check())
        return;
      if (pvars.size() > 1)
        return;
    ...
    case P_BOOTCONF2:
      ...
      if (pvars.size() > 1)
        ...
        Bitu size = (Bitu)control->configfiles.size();
        if (size == 0)
          RebootConfig("dosbox-x.conf");
        else
          RebootConfig(control->configfiles.front().c_str());
      }
      break;
    case P_NOPARAMS:
      if (!first)
    ...
    case P_HELP3: {
      ...
      if (pvars.size() == 1) {
        ...
        while (true) {
          ...
          if (p == NULL)
            break;
          if (!(all > 0 || (all == -1 && (p->basic() || p->modified())) ||
                (!all && ((p->propname == "rem" &&
                           (!strcmp(pvars[0].c_str(), "4dos") ||
                            !strcmp(pvars[0].c_str(), "config"))) ||
                          p->modified()))))
            continue;
          WriteOut("%s\n", p->propname.c_str());
        }
        ...
        while (true) {
          ...
          if (!strcasecmp(p->propname.c_str(), pvars[1].c_str())) {
            ...
            for (Bitu k = 0; k < pv.size(); k++) {
              ...
              if ((k + 1) < pv.size() &&
                  (strcmp(pvars[0].c_str(), "config") ||
                   p->propname != "numlock" || pv[k + 1].ToString() != ""))
                propvalues += ", ";
            }

            WriteOut(MSG_Get("PROGRAM_CONFIG_HLP_PROPHLP"), p->propname.c_str(),
                     sec->GetName(), p->Get_help(), propvalues.c_str(),
    ...
    case P_GETPROP: {
      ...
      case 1: {
        ...
        if (sec) {
                ...
                for (std::string line; std::getline(in, line);) {
                  ...
                  if (p != NULL) {
                    *p = 0;
                    ...
                    val = trim(val);
                    lowcase(cmd);
                    if (!strcasecmp(pvars[0].c_str(), "4dos") ||
                        !strncmp(cmd, "set ", 4) || !strcmp(cmd, "install") ||
                        !strcmp(cmd, "installhigh") || !strcmp(cmd, "device") ||
                        !strcmp(cmd, "devicehigh"))
                      if (!((!strcmp(cmd, "install") ||
                             !strcmp(cmd, "installhigh") ||
                             !strcmp(cmd, "device") ||
                             !strcmp(cmd, "devicehigh")) &&
                            !strlen(val) && !all))
                        WriteOut("%s=%s\n", cmd, val);
          ...
          if (!sec && pvars[0].size() > 4 &&
            ...
            pvars[0].erase(0, 4);
            sec = control->GetSectionFromProperty(pvars[0].c_str());
          } else if (!sec && pvars[0].size() &&
                     !strcasecmp(pvars[0].c_str(), "langcp")) {
            pvars[0] = "language";
            sec = control->GetSectionFromProperty(pvars[0].c_str());
          }
          if (!sec) {
            unsigned int maxWidth, maxHeight;
            void GetMaxWidthHeight(unsigned int *pmaxWidth,
                                   unsigned int *pmaxHeight),
                GetDrawWidthHeight(unsigned int *pdrawWidth,
                                   unsigned int *pdrawHeight);
            if (!strcasecmp(pvars[0].c_str(), "screenwidth")) {
              ...
              WriteOut("%d\n", rect.right - rect.left);
              first_shell->SetEnv(
                  "CONFIG", std::to_string(rect.right - rect.left).c_str());
            } else if (!strcasecmp(pvars[0].c_str(), "clientheight")) {
              RECT rect;
              GetClientRect(GetHWND(), &rect);
              WriteOut("%d\n", rect.bottom - rect.top);
              first_shell->SetEnv(
                  "CONFIG", std::to_string(rect.bottom - rect.top).c_str());
#endif
#if defined(WIN32)
              ...
              first_shell->SetEnv(
                  "CONFIG",
                  (std::string(1, drive) + ":\\" + std::string(dir)).c_str());
              ...
              WriteOut("%d\n", dos.return_code);
              first_shell->SetEnv("CONFIG",
                                  std::to_string(dos.return_code).c_str());
              ...
              uint8_t add = mem_readb(BIOS_24_HOURS_FLAG);
              mem_writeb(BIOS_24_HOURS_FLAG, 0); // reset the "flag"
              uint16_t cx = (uint16_t)(ticks >> 16u),
              ...
              first_shell->SetEnv(
                  "CONFIG",
                  lastmount ? (std::string(1, lastmount) + ":").c_str() : "");
            } else
              WriteOut(MSG_Get("PROGRAM_CONFIG_PROPERTY_ERROR"));
            return;
          }
      ...
      case 2: {
        ...
        if (val == NO_SUCH_PROPERTY) {
          if (!strcasecmp(pvars[0].c_str(), "config") &&
              (!strcasecmp(pvars[1].c_str(), "set") ||
               !strcasecmp(pvars[1].c_str(), "device") ||
               !strcasecmp(pvars[1].c_str(), "devicehigh") ||
               !strcasecmp(pvars[1].c_str(), "install") ||
               !strcasecmp(pvars[1].c_str(), "installhigh"))) {
            Section_prop *psec = dynamic_cast<Section_prop *>(sec);
            const char *extra = psec->data.c_str();
            if (extra && strlen(extra)) {
              std::istringstream in(extra);
            ...
            WriteOut(MSG_Get("PROGRAM_CONFIG_NO_PROPERTY"), pvars[1].c_str(),
                     pvars[0].c_str());
          return;
        }
        WriteOut("%s\n", val.c_str());
    ...
    next:
      ...
      if (change_success) {
        if (applynew)
          ...
          ApplySetting(pvars[0], inputline, false);
      } else
      ...
      if (pvars.size() < 1) {
        if (control->opt_lang == "") {
          ...
          WriteOut(MSG_Get("PROGRAM_CONFIG_FILE_ERROR"),
                   control->opt_lang.c_str());
          return;
        } else
          WriteOut(MSG_Get("PROGRAM_LANGUAGE_FILE_WHICH"),
                   control->opt_lang.c_str());
...
void CONFIG_ProgramStart(Program **make) { *make = new CONFIG; }

void PROGRAMS_DOS_Boot(Section *) {}

/* FIXME: Rename the function to clarify it does not init programs, it inits the
 * callback mechanism that program generation on drive Z: needs to tie a .COM
 * executable to a callback */
void PROGRAMS_Init() {
  LOG(LOG_MISC, LOG_DEBUG)("PROGRAMS_Init(): initializing Z: drive .COM stub "
  ...
  AddVMEventFunction(VM_EVENT_DOS_INIT_KERNEL_READY,
                     AddVMEventFunctionFuncPair(PROGRAMS_DOS_Boot));
  ...
  MSG_Add(
      ...
      "-wc (or -writeconf) without parameter: Writes to primary loaded config "
      "file.\n"
      "-wc (or -writeconf) with filename: Writes file to the config "
      "directory.\n"
      "-wl (or -writelang) with filename: Writes the current language "
      "strings.\n"
      "-ln (or -langname) Displays (without arguments) or specifies the "
      "language name.\n"
      "-wcp [filename] Writes file to program directory (dosbox-x.conf or "
      "filename).\n"
      "-wcd Writes to the default config file in the config directory.\n"
      "-all, -mod Use with -wc, -wcp, or -wcd to write ALL or only modified "
      "options.\n"
      "-wcboot, -wcpboot, or -wcdboot will reboot DOSBox-X after writing the "
      "file.\n"
      "-bootconf (or -bc) reboots with specified config file (or primary "
      "loaded file).\n"
      "-norem Use this with -wc, -wcp, or -wcd to not write config option "
      "remarks.\n"
      "-l Lists DOSBox-X's configuration parameters.\n"
      "-h, -help, -? Shows this help; add the word \"sections\" for a list of "
      "sections.\n"
      "-h, -help, -? section / property Shows info on a specific section or "
      "property.\n"
      "-axclear Clears the [autoexec] section.\n"
      "-axadd [line] Adds a line to the [autoexec] section.\n"
      "-axtype Prints the content of the [autoexec] section.\n"
      "-avistart, -avistop Starts or stops AVI recording.\n"
      "-securemode Enables secure mode where features like mounting will be "
      "disabled.\n"
      "-startmapper Starts the DOSBox-X mapper editor.\n"
      "-gui Starts the graphical configuration tool.\n"
      "-get \"section property\" returns the value of the property (also to "
      "%%CONFIG%%).\n"
      "-set (-setf for force) \"section property=value\" sets the value of the "
      "property.\n");
  ...
  MSG_Add("PROGRAM_CONFIG_APPLY_RESTART",
  ...
  MSG_Add("PROGRAM_CONFIG_PROPERTY_DUPLICATE",
  ...
  MSG_Add("PROGRAM_CONFIG_PRINT_STARTUP",
          "\nDOSBox-X was started with the following command line "
          "parameters:\n%s\n");
