...
int _wmkdir_p(const wchar_t *pathname) {
  ...
  if (((ps[0] >= 'A' && ps[0] <= 'Z') || (ps[0] >= 'a' && ps[0] <= 'z')) &&
      ps[1] == ':') {
    lt = ps[0];
    if (ps[2] == '\\')
      bs = true;
  ...
  /* if the string starts with leading \\ like a UNC path or we're at the first
   * \ after a drive letter, skip it */
  /* FIXME: Proper UNC path detect and skip? The NT kernel will obviously fail a
   * mkdir() on this part of the path */
  while (*ps == PSEP)
    *pw++ = *ps++;
  assert(pw <= pwf);
...
// NTS: If the compile target uses backslash for path separator (DOS/WINDOWS)
// then this can change for those targets
#define PSEP '/'
