...
char *strchr_dbcs(char *str, char ch) {
  ...
  if ((ch == '\\' && (IS_PC98_ARCH || isDBCSCP())) ||
      (ch == '|' &&
       (IS_PC98_ARCH ||
        (isDBCSCP() && !((dos.loaded_codepage == 936 || IS_PDOSV) && !gbk))))) {
    for (size_t i = 0; i < strlen(str); i++) {
      if (lead)
        lead = false;
      else if ((IS_PC98_ARCH && shiftjis_lead_byte(str[i])) ||
               (isDBCSCP() && isKanji1(str[i])))
        lead = true;
      else if (str[i] == ch) {
        lastpos = i;
        break;
...
char *strtok_dbcs(char *s, const char *d) {
  ...
  bool lead = false;
  for (; input[i] != '\0'; i++) {
    if (!lead && ((IS_PC98_ARCH && shiftjis_lead_byte(input[i])) ||
                  (isDBCSCP() && isKanji1(input[i])))) {
      result[i] = input[i];
      lead = true;
...
bool check_last_split_char(const char *name, size_t len, char split) {
  ...
  if ((IS_PC98_ARCH || isDBCSCP()) && split == '\\') {
    ...
    for (size_t pos = 0; pos < len; pos++) {
      if (lead)
        lead = false;
      else if ((IS_PC98_ARCH && shiftjis_lead_byte(name[pos])) ||
               (isDBCSCP() && isKanji1(name[pos])))
        lead = true;
      else if (pos == len - 1 && name[pos] == split)
        tail = true;
    }
  } else if (len > 0) {
    if (name[len - 1] == split)
...
void strreplace_dbcs(char *str, char o, char n) {
  ...
  while (*str) {
    if (lead)
      lead = false;
    else if ((IS_PC98_ARCH && shiftjis_lead_byte(*str)) ||
             (isDBCSCP() && isKanji1(*str)))
      lead = true;
    else if (*str == o)
      *str = n;
    str++;
...
char *rtrim(char *str) {
  char *p;
  p = strchr(str, '\0');
  while (--p >= str && *reinterpret_cast<unsigned char *>(p) != '\f' &&
...
bool ScanCMDBool(char *cmd, char const *const check) {
  ...
  while ((scan = strchr(scan, '/'))) {
    ...
    scan++;
    if (strncasecmp(scan, check, c_len) == 0 &&
        (scan[c_len] == ' ' || scan[c_len] == '\t' || scan[c_len] == '/' ||
         scan[c_len] == 0)) {
  ...
  for (char c = *scan; (c = *scan); scan++) {
    if (*scan == '"') {
      q++;
               ...
               isspace(*reinterpret_cast<unsigned char *>(&c))) {
      *scan++ = 0;
      break;
    }
  }
...
int utf16le_encode(char **ptr, const char *fence, uint32_t code) {
  ...
  } else if ((code & 0xF800) == 0xD800) { /* do not allow accidental surrogate
                                             pairs (0xD800-0xDFFF) */
    return UTF8ERR_INVALID;
  } else {
    if ((p + 2) > fence)
...
static char buf[1024]; // greater scope as else it doesn't always gets thrown
                       // right (linux/gcc2.95)
