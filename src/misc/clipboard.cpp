...
extern int mouse_start_x, mouse_start_y, mouse_end_x, mouse_end_y, fx, fy,
    selsrow, selscol, selerow, selecol, mbutton;
...
bool Unicode2Ascii(const uint16_t *unicode) {
  ...
  if (CodePageHostToGuestUTF16(temp, unicode) &&
      (clipAscii = (uint8_t *)malloc(strlen(temp) + 1))) {
    morelen = false;
    ret = true;
                                ...
                                : (dos.loaded_codepage == 859
                                       ...
                                       : (dos.loaded_codepage == 872
                                              ...
                                              : (dos.loaded_codepage == 951
                                                     ? 950
                                                     : dos.loaded_codepage))),
                            flags, (LPCWSTR)unicode, -1, NULL, 0, "\x07", NULL);
    ...
    if (WideCharToMultiByte(dos.loaded_codepage == 808
                                ? 866
                                : (dos.loaded_codepage == 859
                                       ? 858
                                       : (dos.loaded_codepage == 872
                                              ...
                                              : (dos.loaded_codepage == 951
                                                     ? 950
                                                     : dos.loaded_codepage))),
                            flags, (LPCWSTR)unicode, -1, (LPSTR)clipAscii,
                            memNeeded, "\x07", NULL) !=
  ...
  for (int i = 0; i < memNeeded; i++)
    if (clipAscii[i] > 31 || clipAscii[i] == 7 || clipAscii[i] == 9 ||
        clipAscii[i] == 10 ||
        clipAscii[i] == 13 // Space and up, or Bell, TAB, CR/LF allowed (others
                           // make no sense when pasting)
        ||
        (dos.loaded_codepage == 932 && (TTF_using() || IS_JEGA_ARCH || showdbcs)
#if defined(USE_TTF)
         && halfwidthkana
#endif
         && ((IS_JEGA_ARCH && clipAscii[i] && clipAscii[i] < 32) ||
             (!IS_PC98_ARCH && !IS_JEGA_ARCH &&
              lowboxdrawmap.find(clipAscii[i]) != lowboxdrawmap.end()))))
...
static void GenKBStroke(const UINT uiScanCode, const bool bDepressed,
                        const SDLMod keymods) {
...
bool PasteClipboardNext() {
  ...
  if (clipboard_biospaste) {
    if (strPasteBuffer[0] == 13) {
      ...
      BIOS_AddKeyToBuffer(strPasteBuffer[0] < 0 ? strPasteBuffer[0] & 0xff
                                                : strPasteBuffer[0]);
    strPasteBuffer = strPasteBuffer.substr(1, strPasteBuffer.length());
    return true;
  }
...
#if defined(WIN32) &&                                                          \
    (!defined(__MINGW32__) || defined(__MINGW64_VERSION_MAJOR))
  SHORT shVirKey = VkKeyScan(
      cKey); // If it fails then MapVirtK will also fail, so no bail yet
  ...
  if (uiScanCode) {
    ...
    const SDLMod sdlmMods =
        (SDLMod)((sdlmModsOn & ~(KMOD_LSHIFT | KMOD_RSHIFT | KMOD_LCTRL |
                                 KMOD_RCTRL | KMOD_LALT | KMOD_RALT)) |
                 (bModShiftOn ? KMOD_LSHIFT : 0) |
                 (bModCntrlOn ? KMOD_LCTRL : 0) | (bModAltOn ? KMOD_LALT : 0));
    ...
    if (bModShift != bModShiftOn)
      sStrokesRequired += 2; // To press/release Shift
    if (bModCntrl != bModCntrlOn)
      sStrokesRequired += 2; // To press/release Control
    if (bModAlt != bModAltOn)
      sStrokesRequired += 2; // To press/release Alt
    /// \fixme Should check if key is already pressed or not so it can toggle
    /// press
    ///        but since we don't actually have any mappings from VK/SC to DI
    ///        codes (which SDL (can) use(s) internally as actually scancodes),
    ///        we can't actually check that ourselves, sadly...
    if (KEYBOARD_BufferSpaceAvail() < (sStrokesRequired + kPasteMinBufExtra))
      return false;

    if (bModShift != bModShiftOn)
      GenKBStroke(uiScanCodeShift, !bModShiftOn, sdlmMods);
    if (bModCntrl != bModCntrlOn)
      GenKBStroke(uiScanCodeCntrl, !bModCntrlOn, sdlmMods);
    if (bModAlt != bModAltOn)
      GenKBStroke(uiScanCodeAlt, !bModAltOn, sdlmMods);
...
#endif
    if (KEYBOARD_BufferSpaceAvail() <
        (10 +
         kPasteMinBufExtra)) // For simplicity, we just mimic Alt+<3 digits>
      return false;
  ...
  strPasteBuffer = strPasteBuffer.substr(
      1, strPasteBuffer
             .length()); // technically -1, but it clamps by itself anyways...
  return true;
}
...
void PasteClipboard(bool bPressed) {
  ...
  if (SDL_GetWMInfo(&wmiInfo) != 1 || !OpenClipboard(wmiInfo.window))
    return;
  ...
  if (szClipboard) {
    ...
    for (size_t i = 0; i < clipSize; ++i) {
      if (clipAscii[i] == 9)
        j++;
      else if (ret && clipAscii[i] == 0x0A &&
               (i == 0 || clipAscii[i - 1] != 0x0D))
        clipAscii[i] = 0x0D;
    }
    if (ret && clipAscii[clipSize - 1] == 0xD)
      clipAscii[--clipSize] = 0;
    // Create a copy of the string, and filter out Linefeed characters (ASCII
    // '10')
    char *szFilteredText =
        reinterpret_cast<char *>(alloca(clipSize + j * 3 + 1));
...
void PasteClipboard(bool bPressed) {
  if (!bPressed || !OpenClipboard(NULL))
    return;
  if (!IsClipboardFormatAvailable(CF_UNICODETEXT)) {
    CloseClipboard();
    return;
  ...
  if (szClipboard) {
    ...
    for (size_t i = 0; i < clipSize; ++i) {
      if (clipAscii[i] == 9)
        j++;
      else if (ret && clipAscii[i] == 0xA &&
               (i == 0 || clipAscii[i - 1] != 0xD))
        clipAscii[i] = 0xD;
    }
    if (ret && clipAscii[clipSize - 1] == 0xD)
      clipAscii[--clipSize] = 0;
...
void PasteClipboard(bool bPressed) {
  ...
  for (unsigned int i = 0; i < strlen(text); i++) {
    if (swapad && text[i] == 0x0A && (i == 0 || text[i - 1] != 0x0D))
      text[i] = 0x0D;
    if (text[i] == 9)
      result += "    ";
    else if (text[i] < 0) {
...
void paste_utf8_prop(Display *dpy, Window w, Atom p) {
  ...
  XGetWindowProperty(dpy, w, p, 0, 0, False, AnyPropertyType, &type, &di, &dul,
                     &size, &prop_ret);
  XFree(prop_ret);
  ...
  XGetWindowProperty(dpy, w, p, 0, size, False, AnyPropertyType, &da, &di, &dul,
                     &dul, &prop_ret);
...
void CopyClipboard(int all) {
  ...
  const char *text =
      (char *)(all == 2
                   ? Mouse_GetSelected(
                         0, 0, (int)(currentWindowWidth - 1 - sdl.clip.x),
                         (int)(currentWindowHeight - 1 - sdl.clip.y),
                         (int)(currentWindowWidth - sdl.clip.x),
                         (int)(currentWindowHeight - sdl.clip.y), &len)
                   : (all == 1
                          ? Mouse_GetSelected(selscol, selsrow, selecol,
                                              selerow, -1, -1, &len)
                          : Mouse_GetSelected(mouse_start_x - sdl.clip.x,
                                              mouse_start_y - sdl.clip.y,
                                              mouse_end_x - sdl.clip.x,
                                              mouse_end_y - sdl.clip.y,
                                              sdl.clip.w, sdl.clip.h, &len)));
  if (OpenClipboard(NULL) && EmptyClipboard()) {
    ...
    for (std::string token; std::getline(iss, token);) {
      if (CodePageGuestToHostUTF16(temp, token.c_str())) {
        ...
        int reqsize = MultiByteToWideChar(
            dos.loaded_codepage == 808
                ? 866
                : (dos.loaded_codepage == 859
                       ? 858
                       : (dos.loaded_codepage == 872
                              ...
                              : (dos.loaded_codepage == 951
                                     ? 950
                                     : dos.loaded_codepage))),
            0, token.c_str(), token.size() + 1, NULL, 0);
        if (reqsize > 0) {
                      ...
                      : (dos.loaded_codepage == 859
                             ...
                             : (dos.loaded_codepage == 872
                                    ...
                                    : (dos.loaded_codepage == 951
                                           ? 950
                                           : dos.loaded_codepage))),
                  0, token.c_str(), token.size() + 1, (LPWSTR)wch,
                  reqsize) == reqsize) {
            result += (wchar_t *)wch;
            delete[] wch;
            continue;
    ...
    bdlist = {};
    HGLOBAL clipbuffer =
        GlobalAlloc(GMEM_DDESHARE, (result.size() + 1) * sizeof(wchar_t));
    LPWSTR buffer = static_cast<LPWSTR>(GlobalLock(clipbuffer));
    if (buffer != NULL) {
      for (unsigned int i = 0; i < result.size(); i++)
        buffer[i] = (wchar_t)result[i];
...
void CopyClipboard(int all) {
  ...
  char *text =
      (char *)(all == 2
                   ? Mouse_GetSelected(
                         0, 0, currentWindowWidth - 1 - sdl.clip.x,
                         currentWindowHeight - 1 - sdl.clip.y,
                         (int)(currentWindowWidth - sdl.clip.x),
                         (int)(currentWindowHeight - sdl.clip.y), &len)
                   : (all == 1
                          ? Mouse_GetSelected(selscol, selsrow, selecol,
...
bool isModifierApplied() {
  return direct_mouse_clipboard || !strcmp(modifier, "none") ||
         ((!strcmp(modifier, "ctrl") || !strcmp(modifier, "lctrl")) &&
          sdl.lctrlstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "ctrl") || !strcmp(modifier, "rctrl")) &&
          sdl.rctrlstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "alt") || !strcmp(modifier, "lalt")) &&
          sdl.laltstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "alt") || !strcmp(modifier, "ralt")) &&
          sdl.raltstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "shift") || !strcmp(modifier, "lshift")) &&
          sdl.lshiftstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "shift") || !strcmp(modifier, "rshift")) &&
          sdl.rshiftstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "ctrlalt") || !strcmp(modifier, "lctrlalt")) &&
          sdl.lctrlstate == SDL_KEYDOWN && sdl.laltstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "ctrlalt") || !strcmp(modifier, "rctrlalt")) &&
          sdl.rctrlstate == SDL_KEYDOWN && sdl.raltstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "ctrlshift") || !strcmp(modifier, "lctrlshift")) &&
          sdl.lctrlstate == SDL_KEYDOWN && sdl.lshiftstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "ctrlshift") || !strcmp(modifier, "rctrlshift")) &&
          sdl.rctrlstate == SDL_KEYDOWN && sdl.rshiftstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "altshift") || !strcmp(modifier, "laltshift")) &&
          sdl.laltstate == SDL_KEYDOWN && sdl.lshiftstate == SDL_KEYDOWN) ||
         ((!strcmp(modifier, "altshift") || !strcmp(modifier, "raltshift")) &&
          sdl.raltstate == SDL_KEYDOWN && sdl.rshiftstate == SDL_KEYDOWN);
}
...
void ClipKeySelect(int sym) {
  ...
  if (mbutton != 4 || (CurMode->type != M_TEXT && !IS_DOSV && !IS_PC98_ARCH))
    return;
  if (sym == SDLK_HOME) {
  ...
  } else if (sym == SDLK_LEFT || sym == SDLK_RIGHT || sym == SDLK_UP ||
             sym == SDLK_DOWN) {
    if (selsrow == -1 || selscol == -1) {
      int p = real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE);
      selscol = CURSOR_POS_COL(p);
      ...
      Mouse_Select(selscol, selsrow, selmark ? selecol : selscol,
                   selmark ? selerow : selsrow, -1, -1, false);
    if (sym == SDLK_LEFT && (selmark ? selecol : selscol) > 0)
      (selmark ? selecol : selscol)--;
             ...
             (selmark ? selecol : selscol) <
                 (IS_PC98_ARCH ? 80
                     ...
                     1)
      (selmark ? selecol : selscol)++;
    else if (sym == SDLK_UP && (selmark ? selerow : selsrow) > 0)
      (selmark ? selerow : selsrow)--;
    else if (sym == SDLK_DOWN &&
             (selmark ? selerow : selsrow) <
                 (IS_PC98_ARCH ? (real_readb(0x60, 0x113) & 0x01 ? 25 : 20) - 1
                               : (IS_EGAVGA_ARCH
                                      ? real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS)
                                      : 24)))
      (selmark ? selerow : selsrow)++;
    Mouse_Select(selscol, selsrow, selmark ? selecol : selscol,
                 selmark ? selerow : selsrow, -1, -1, true);
  }
}
#endif
