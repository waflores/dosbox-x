...
pcap_t *(*PacketOpen)(char const *, int, int, int, struct pcap_rmtauth *,
                      char *) = 0;
int (*PacketNextEx)(pcap_t *, struct pcap_pkthdr **, const u_char **) = 0;
int (*PacketFindALlDevsEx)(char *, struct pcap_rmtauth *, pcap_if_t **,
                           char *) = 0;
...
#pragma GCC diagnostic ignored "-Wcast-function-type"
  ...
  if (!PacketSendPacket)
    PacketSendPacket = (int(__cdecl *)(pcap_t *, const u_char *, int))psp;

  psp = GetProcAddress(pcapinst, "pcap_close");
  if (!PacketClose)
    PacketClose = (void(__cdecl *)(pcap_t *))psp;

  psp = GetProcAddress(pcapinst, "pcap_freealldevs");
  if (!PacketFreealldevs)
    PacketFreealldevs = (void(__cdecl *)(pcap_if_t *))psp;

  psp = GetProcAddress(pcapinst, "pcap_open");
  if (!PacketOpen)
    PacketOpen = (pcap_t * (__cdecl *)(char const *, int, int, int,
                                       struct pcap_rmtauth *, char *)) psp;
  ...
  if (!PacketFindALlDevsEx)
    PacketFindALlDevsEx = (int(__cdecl *)(char *, struct pcap_rmtauth *,
                                          pcap_if_t **, char *))psp;

  psp = GetProcAddress(pcapinst, "pcap_geterr");
  if (!PacketGetError)
    PacketGetError = (char *(__cdecl *)(pcap_t *))psp;
...
PcapEthernetConnection::PcapEthernetConnection() : EthernetConnection() {}
...
bool PcapEthernetConnection::Initialize(Section *config) {
  ...
  if (!strcasecmp(realnicstring, "list")) {
    ...
    LOG_MSG(
        "NOTE: No interface was chosen because the realnic setting is set to "
        "'list', set realnic to the number of the interface you want to use.");
  ...
  char *end;
  int timeout = -1;
  if (!strlen(timeoutstr) ||
      (timeoutstr[0] != '-' &&
       !isdigit(timeoutstr[0]))) { // Default timeout values
#ifdef MACOSX
    timeout = 3000; // For macOS, use 3000ms as it does not appear to support -1
#else
    timeout =
        -1; // For other platforms, use -1 which should mean "non-blocking mode"
#endif
  } else
    timeout = strtoul(timeoutstr, &end, 10);
  // attempt to open it
#ifdef WIN32
  if ((adhandle = pcap_open(currentdev->name, // name of the device
  ...
  if ((adhandle = pcap_open_live(currentdev->name, // name of the device
                                 65536, // portion of the packet to capture
                                 // 65536 = whole packet
...
void PcapEthernetConnection::GetPackets(
    std::function<void(const uint8_t *, int)> callback) {
  int res;
  struct pcap_pkthdr *header;
  u_char *pkt_data;
  // #if 0
  while ((res = pcap_next_ex(adhandle, &header, (const u_char **)&pkt_data)) >
         0) {
