...
static int get_dirname(const char *path, char *dirbuf, size_t size) {
  ...
  while (len > 0 && (path[len - 1] == sep || path[len - 1] == '/' ||
                     path[len - 1] == '\\')) {
    len--;
  }
  if (len == 0)
  ...
  size_t pos = len;
  while (pos > 0) {
    if (path[pos - 1] == sep || path[pos - 1] == '/' || path[pos - 1] == '\\')
      break;
    pos--;
  }
  if (pos == 0)
    return -1; // No separator found (only filename)
...
bool Property::CheckValue(Value const &in, bool warn) {
  ...
  if (warn)
    LOG_MSG("\"%s\" is not a valid value for variable: %s.\nIt might now be "
            ...
            in.ToString().c_str(), propname.c_str(),
            default_value.ToString().c_str());
  return false;
}

/* There are too many reasons I can think of to have similar property names per
 * section without tying them together by name. Sticking them in MSG_Add as
 * CONFIG_ + propname for help strings is just plain dumb. But... in the event
 * that is still useful, I at least left the code conditionally enabled if any
 * part of the code still wants to do that. --J.C */
...
void Property::SetBasic(bool basic) { is_basic = basic; }
  ...
  if (forced) {
    ...
    if (warn)
      LOG_MSG("%s is outside the allowed range %s-%s for variable: %s.\nIt has "
              "been set to the closest boundary: %d.",
              in.ToString().c_str(), min.ToString().c_str(),
              max.ToString().c_str(), propname.c_str(), va);
...
bool Prop_int::CheckValue(Value const &in, bool warn) {
  ...
  if (warn)
    LOG_MSG("%s lies outside the range %s-%s for variable: %s.\nIt might now "
            "be reset to the default value: %s",
            in.ToString().c_str(), min.ToString().c_str(),
            max.ToString().c_str(), propname.c_str(),
            default_value.ToString().c_str());
  return false;
}
...
bool Prop_double::SetValue(std::string const &input) {
  Value val;
  if (!val.SetValue(input.size() ? input : default_value.ToString(),
                    Value::V_DOUBLE))
    return false;
...
bool Prop_string::SetValue(std::string const &input) {
  ...
  return SetVal(val, false, true) ||
         (!suggested_values.empty() && !input.size());
}
...
bool Prop_multival_remain::SetValue(std::string const &input, bool init) {
  ...
  while ((p = section->Get_prop(i++))) {
    ...
    if (p->Get_type() == Value::V_STRING) {
      ...
      // Non-strings can have more things, conversion alone is not enough (as
      ...
      if (!r) {
        if (in.empty() && p->Get_type() == prevtype) {
          ...
          // Something was there to be parsed or not the same type. Invalidate
          // entire property.
          make_default_value();
        }
      }
...
Prop_double *Section_prop::Add_double(string const &_propname,
...
bool Section_prop::HandleInputline(string const &gegevens) {
  ...
  for (it tel = properties.begin(); tel != properties.end(); ++tel) {
    if (!strcasecmp((*tel)->propname.c_str(), name.c_str())) {
      ...
      for (std::list<SectionFunction>::iterator i = onpropchange.begin();
           i != onpropchange.end(); ++i)
        (*i)(this);
...
void Section_prop::PrintData(FILE *outfile, int everything, bool norem) {
  ...
  for (const_it tel = properties.begin(); tel != properties.end(); ++tel) {
    if (!(everything > 0 ||
          (everything == -1 && ((*tel)->basic() || (*tel)->modified())) ||
          (!everything && ((!norem && (*tel)->propname == "rem" &&
                            (!strcasecmp(GetName(), "4dos") ||
                             !strcasecmp(GetName(), "config"))) ||
                           (*tel)->modified()))))
      continue;
  ...
  if (!strcasecmp(GetName(), "config") && len < 11)
    len = 11;
  ...
  for (const_it tel = properties.begin(); tel != properties.end(); ++tel) {
    ...
    std::string pre =
        everything == 2 && !(*tel)->basic() ? "#DOSBOX-X-ADV:" : "";
...
/* NTS: For whatever reason, this string concatenation is reported by Valgrind
 * as a memory leak because std::string's destructor is never given a chance to
 * clear it, or something... I can't quite pinpoint why. Not enough information
 * is provided so far in stack traces. It SHOULD be freed, because Section_line
 * is derived from Section, the constructors are virtual, and therefore
 * std::string should get a chance to free it's memory. */
...
bool Config::PrintConfig(char const *const configfilename, int everything,
                         bool norem) const {
  char temp[50];
  ...
  for (const_it tel = sectionlist.begin(); tel != sectionlist.end(); ++tel) {
    ...
    if (sec) {
      ...
      while ((p = sec->Get_prop(int(i++)))) {
        if (!(everything > 0 ||
              (everything == -1 && (p->basic() || p->modified())) ||
              (!everything &&
               ((p->propname == "rem" &&
                 (!strcmp(temp, "4dos") || !strcmp(temp, "config"))) ||
                p->modified()))))
          continue;
      ...
      if (!norem)
        while ((p = sec->Get_prop(int(i++)))) {
          ...
          if (!(everything == 1 ||
                ...
                (!everything &&
                 ((p->propname == "rem" &&
                   (!strcmp(temp, "4dos") || !strcmp(temp, "config"))) ||
                  p->modified())))) {
            if ((everything == -1 || everything == 2) && !p->basic() &&
                !p->modified() && help.size())
          ...
          std::string pre =
              everything == 2 && !p->basic() ? "#DOSBOX-X-ADV:" : "";
          ...
          if (help != "" || !values.empty()) {
            fprintf(outfile, "%s# %*s: %s", pre.c_str(), intmaxwidth,
                    p->propname.c_str(), help.c_str());

            if (!values.empty()) {
              fprintf(outfile, "%s%s:", prefix,
                      MSG_Get("CONFIG_SUGGESTED_VALUES"));
              std::vector<Value>::const_iterator it = values.begin();
              while (it != values.end()) {
                if ((*it).ToString() != "%u" &&
                    (strcmp(temp, "config") || p->propname != "numlock" ||
                     (*it).ToString() !=
                         "")) { // Hack hack hack. else we need to modify
                                // GetValues, but that one is const...
                  if (it != values.begin())
                    fputs(",", outfile);
      ...
      if ((everything == -1 || everything == 2) && !advopts.empty()) {
        fprintf(outfile,
                everything == 2 ? "#DOSBOX-X-ADV-SEE:#\n#DOSBOX-X-ADV-SEE:# "
                                  "%s:\n#DOSBOX-X-ADV-SEE:# ->"
                                : "#\n# %s:\n# ->",
                MSG_Get("CONFIG_ADVANCED_OPTION"));
        for (std::vector<std::string>::iterator advopt = advopts.begin();
             advopt != advopts.end(); ++advopt)
          fprintf(outfile, " %s%c", advopt->c_str(),
                  advopt + 1 >= advopts.end() ? '\n' : ';');
    ...
    if (!strcmp(temp, "config") || !strcmp(temp, "4dos")) {
      ...
      if (extra && strlen(extra)) {
        ...
        char cmdstr[CROSS_LEN], valstr[CROSS_LEN], *cmd = cmdstr, *val = valstr,
                                                   *p;
        if (in)
          for (std::string line; std::getline(in, line);) {
            ...
            if (p != NULL) {
              *p = 0;
              ...
              val = trim(val);
              lowcase(cmd);
              if (!strcmp(temp, "4dos") || !strncmp(cmd, "set ", 4) ||
                  !strcmp(cmd, "install") || !strcmp(cmd, "installhigh") ||
                  !strcmp(cmd, "device") || !strcmp(cmd, "devicehigh")) {
                (!strncmp(cmd, "set ", 4) ? used1 : used2) = true;
                if (!((!strcmp(cmd, "install") || !strcmp(cmd, "installhigh") ||
                       !strcmp(cmd, "device") || !strcmp(cmd, "devicehigh")) &&
                      !strlen(val) && !everything))
                  fprintf(outfile,
                          strcmp(temp, "4dos") ? "%-11s = %s\n"
                                               : "%-14s = %s\n",
                          cmd, val);
              }
            }
      ...
      if (!strcmp(temp, "config")) {
        if (everything && !used1) {
          fprintf(outfile, "%-11s = %s\n", "set path",
                  "Z:\\;Z:\\SYSTEM;Z:\\BIN;Z:\\DOS;Z:\\4DOS;Z:\\DEBUG;Z:"
                  "\\TEXTUTIL");
          fprintf(outfile, "%-11s = %s\n", "set prompt", "$P$G");
          fprintf(outfile, "%-11s = %s\n", "set temp", "");
        }
        ...
        if (extra && strlen(extra)) {
          ...
          if (everything && rem)
            for (std::string line; std::getline(rem, line);) {
              if (line.length() > CROSS_LEN) {
                strncpy(linestr, line.c_str(), CROSS_LEN);
                linestr[CROSS_LEN] = 0;
              } else
                strcpy(linestr, line.c_str());
              if (!strncasecmp(trim(lin), "rem ", 4) &&
                  *trim(trim(lin) + 4) != '=' && !norem)
                fprintf(outfile, "%s\n", trim(lin));
            }
        }
      }
...
void AddExitFunction(SectionFunction func, const char *name, bool canchange) {
  /* NTS: Add functions so that iterating front to back executes them in First
   * In Last Out order. */
  exitfunctions.emplace_front(Function_wrapper(func, canchange, name));
}

void AddVMEventFunction(enum vm_event event, SectionFunction func,
                        const char *name, bool canchange) {
  assert(event < VM_EVENT_MAX);

  /* NTS: First In First Out order */
  vm_event_functions[event].emplace_back(
      Function_wrapper(func, canchange, name));
}
...
void DispatchVMEvent(enum vm_event event) {
  ...
  for (std::list<Function_wrapper>::iterator i =
           vm_event_functions[event].begin();
       i != vm_event_functions[event].end(); ++i) {
    LOG(LOG_MISC,
        LOG_DEBUG)("Calling event %s handler (%p) '%s'", GetVMEventName(event),
                   (void *)((uintptr_t)((*i).function)), (*i).name.c_str());
    (*i).function(NULL);
  }
...
Config::~Config() {
  std::list<Section *>::iterator
      it; // FIXME: You guys do realize C++ STL provides reverse_iterator?
...
bool Config::ParseConfigFile(char const *const configfilename) {
  LOG(LOG_MISC,
      LOG_DEBUG)("CONFIG: Attempting to load config file #%zu from %s",
                 configfiles.size(), configfilename);

  // static bool first_configfile = true;
  if (strlen(configfilename) >= PATH_MAX) {
    LOG_MSG("Warning: config file path %d characters is too long: %s",
            strlen(configfilename), configfilename);
  }
  ifstream in(configfilename);
  if (!in) {
    LOG(LOG_MISC, LOG_NORMAL)("CONFIG: Failed Loading %s as a config file",
                              configfilename);
    return false;
  }
  ...
  LOG(LOG_MISC, LOG_NORMAL)("CONFIG: Loading %s settings from config file %s",
  ...
  while (getline(in, gegevens)) {
    ...
    if (gegevens.size() > 13 && gegevens.substr(0, 14) == "#DOSBOX-X-ADV:")
    ...
    case '[': {
      ...
      currentsection =
          testsec; /* NTS: If we don't recognize the section we WANT
                      currentsection == NULL so it has no effect */
      testsec = NULL;
    } break;
    case '%':
      if (strcasecmp(currentsection->GetName(), "autoexec"))
        continue;
    default:
      try {
        if (currentsection) {
          bool savedata = !strcasecmp(currentsection->GetName(), "pc98") ||
                          !strcasecmp(currentsection->GetName(), "ttf") ||
                          !strcasecmp(currentsection->GetName(), "4dos") ||
                          !strcasecmp(currentsection->GetName(), "config");
          if (!currentsection->HandleInputline(gegevens) &&
              strcasecmp(currentsection->GetName(), "autoexec"))
            savedata = true;
          if (savedata) {
            Section_prop *section = static_cast<Section_prop *>(currentsection);
            if (section != NULL &&
                !(!strcasecmp(currentsection->GetName(), "4dos") &&
                  (!strncasecmp(gegevens.c_str(), "rem=", 4) ||
                   !strncasecmp(gegevens.c_str(), "rem ", 4)))) {
...
bool CommandLine::FindStringFullBegin(char const *const begin,
                                      std::string &value, bool squote,
                                      bool remove) {
  size_t len = strlen(begin);
  char c = squote ? '\'' : '\"';
  for (cmd_it it = cmds.begin(); it != cmds.end(); ++it) {
...
bool CommandLine::CurrentArgvEnd(void) { return (opt_scan == cmds.end()); }

void CommandLine::EatCurrentArgv(void) {
  if (opt_scan != cmds.end())
    opt_scan = cmds.erase(opt_scan);
}
...
bool CommandLine::NextOptArgv(std::string &argv) {
  ...
  if (!opt_gnu_getopt_singlechar.empty())
    return false;
...
bool CommandLine::GetOpt(std::string &name) {
  ...
  /* if we're still doing GNU getopt single-char switches, then parse another
   * and return */
  ...
  while (opt_scan != cmds.end()) {
    ...
    if (opt_style == CommandLine::either_except && *str == '/' &&
        (strchr(str + 1, '/') == NULL && strchr(str + 1, '\\') == NULL &&
         (strlen(str) < 6 ||
          (strlen(str) > 5 && strcasecmp(str + strlen(str) - 4, ".bat") &&
           strcasecmp(str + strlen(str) - 4, ".com") &&
           strcasecmp(str + strlen(str) - 4, ".exe"))))) {
      /* MS-DOS style /option, with the exception of ending with .BAT, .COM,
       * .EXE */
      ...
      if (opt_eat_argv)
        ...
        ++opt_scan;
      return true;
    } else if ((opt_style == CommandLine::either ||
                opt_style == CommandLine::dos) &&
               *str == '/') {
      ...
      return true;
                ...
                opt_style == CommandLine::gnu ||
                opt_style == CommandLine::gnu_getopt) &&
               *str == '-') {
      str++;                              /* step past '-' */
...
void CommandLine::EndOpt() { opt_scan = cmds.end(); }
...
int CommandLine::GetParameterFromList(const char *const params[],
  ...
  while (it != cmds.end()) {
    ...
    for (unsigned int i = 0; params[i] != NULL; i++) {
      if (*params[i] == 0) {
        LOG_MSG("FIXME: GetParameterFromList: terminating params[] with \"\" "
                "is deprecated. Please terminate the param list with NULL");
        break;
      }
...
const std::string &CommandLine::GetRawCmdline(void) { return raw_cmdline; }
...
CommandLine::CommandLine(char const *const name, char const *const cmdline,
                         enum opt_style opt, bool squote) {
  if (name)
    file_name = name;
