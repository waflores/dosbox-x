...
/* Really, Microsoft, Really?? You're the only compiler I know that doesn't
 * understand ssize_t! */
#if defined(_MSC_VER)
#include <basetsd.h>
...
RegionAllocTracking::Block::Block()
    : start(0), end(0), free(true), fixed(false) {}

RegionAllocTracking::RegionAllocTracking()
    : _min(0), _max(~((Bitu)0)), _max_nonfixed(~((Bitu)0)),
      topDownAlloc(false) {}
...
Bitu RegionAllocTracking::getMemory(Bitu bytes, const char *who, Bitu alignment,
                                    Bitu must_be_at) {
  if (bytes == 0u)
    return alloc_failed;
  if (alignment > 1u && must_be_at != 0u)
    return alloc_failed; /* avoid nonsense! */
  if (who == NULL)
    who = "";
  if (alist.empty())
    E_Exit("getMemory called when '%s' allocation list not initialized",
           name.c_str());
  ...
  if (alignment == 0u)
    ...
    E_Exit("getMemory called with non-power of 2 alignment value %u on '%s'",
           (int)alignment, name.c_str());

  {
    ...
    while ((ssize_t)si >= 0) {
      ...
      if (must_be_at != 0) {
        ...
        if (topDownAlloc) {
          ...
          if (_max_nonfixed < _max) {
            /* if instructed to by caller, disallow any dynamic allocation above
      ...
      base &=
          ~(alignment - 1u); /* NTS: alignment == 16 means ~0xF or 0xFFFF0 */
      ...
      if (base == blk.start &&
          (base + bytes - 1u) == blk.end) { /* easy case: perfect match */
        ...
        alist.insert(
            alist.begin() +
                (std::vector<RegionAllocTracking::Block>::difference_type)si,
            newblk);
        ...
        Block newblk = blk,
              newblk2 = blk; /* this becomes the new block we insert */
      ...
      LOG(LOG_BIOS, LOG_DEBUG)("getMemory in '%s' (0x%05x "
                               "bytes,\"%s\",align=%u,mustbe=0x%05x) = 0x%05x",
                               name.c_str(), (int)bytes, who, (int)alignment,
                               (int)must_be_at, (int)base);
      sanityCheck();
      return base;
    }
  }
  ...
  LOG(LOG_BIOS, LOG_DEBUG)(
      "getMemory in '%s' (0x%05x bytes,\"%s\",align=%u,mustbe=0x%05x) = FAILED",
      name.c_str(), (int)bytes, who, (int)alignment, (int)must_be_at);
  sanityCheck();
  return alloc_failed;
}
...
void RegionAllocTracking::logDump() {
  ...
  for (si = 0; si < alist.size(); si++) {
    Block &blk = alist[si];
    LOG(LOG_MISC, LOG_DEBUG)("     0x%08x-0x%08x free=%u %s", (int)blk.start,
                             (int)blk.end, blk.free ? 1 : 0, blk.who.c_str());
  }
...
void RegionAllocTracking::sanityCheck() {
  ...
  for (si = 1; si < alist.size(); si++) {
    blk = &alist[si];
    if (blk->start != (pblk->end + 1) || blk->start > blk->end ||
        blk->start < _min || blk->end > _max) {
      LOG(LOG_MISC, LOG_DEBUG)(
          "RegionAllocTracking sanity check failure in '%s'", name.c_str());
      logDump();
...
Bitu RegionAllocTracking::freeUnusedMinToLoc(Bitu phys) {
  if (phys <= _min)
    return _min;
  if ((_max + (Bitu)1) != (Bitu)0 && phys > (_max + 1))
    phys = _max + 1;
  ...
  while ((si + 1u) < alist.size()) {
    ...
    if (blk1.free && blk2.free) {
      if ((blk1.end + (Bitu)1u) == blk2.start) {
        ...
        alist.erase(
            alist.begin() +
            (std::vector<RegionAllocTracking::Block>::difference_type)si + 1u);
        continue;
      }
    }
...
bool RegionAllocTracking::freeMemory(Bitu offset) {
  ...
  while (si < alist.size()) {
    ...
    if (offset >= blk.start && offset <= blk.end) {
      LOG(LOG_BIOS, LOG_DEBUG)("freeMemory in '%s' (address=0x%08lx block='%s' "
                               "range=0x%08lx-0x%08lx) success",
                               name.c_str(), (unsigned long)offset,
                               blk.who.c_str(), (unsigned long)blk.start,
  ...
  LOG(LOG_BIOS, LOG_DEBUG)("freeMemory in '%s' (address=0x%08lx) FAILED",
                           name.c_str(), (unsigned long)offset);
  return false;
}
