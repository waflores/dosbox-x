...
void attrib_test(test_state_t *state, const char *seq, unsigned char dispchar,
                 unsigned char exp) {
  int passed;
...
void color_test(test_state_t *state, const color_test_t *test,
                char prefix_attrib, unsigned char prefix_attrib_flag) {
  static const char testchars[] = {'_', 'R', 'G', 'Y', 'B', 'P', 'C', 'W'};
  int i;
  ...
  for (i = 0; i < elementsof(testchars); i++) {
    ...
    attrib_test(state, final_seq, testchars[i],
