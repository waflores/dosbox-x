...
ULONG nChars;
CRegKey regKey;
void SetupKeyDefaultValue(std::string keyName, std::string valueData,
                          std::string valueName = "") {
  if (regKey.Open(HKEY_CURRENT_USER, keyName.c_str()) != ERROR_SUCCESS)
    regKey.Create(HKEY_CURRENT_USER, keyName.c_str());
  nChars = PATH_SIZE;
  if (regKey.Open(HKEY_CURRENT_USER, keyName.c_str()) != ERROR_SUCCESS ||
      regKey.QueryStringValue(valueName.c_str(), path, &nChars) !=
          ERROR_SUCCESS ||
      path != valueData.c_str())
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (strlen(lpCmdLine)) {
    int retval = MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, lpCmdLine,
                                     -1, NULL, 0);
    if (retval) {
      LPWSTR lpWideCharStr = (LPWSTR)malloc(retval * sizeof(WCHAR));
      if (lpWideCharStr != NULL &&
          MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, lpCmdLine, -1,
                              lpWideCharStr, retval)) {
        args = CommandLineToArgvW(lpWideCharStr, &numArgs);
        free(lpWideCharStr);
        if (numArgs > 0 &&
            (!_wcsicmp(args[0], L"/?") || !_wcsicmp(args[0], L"-?"))) {
          MessageBox(0, "DOSBox-X Launcher.", "Information", MB_OK);
          return 0;
        }
        if (numArgs > 0 &&
            (!_wcsicmp(args[0], L"/u") || !_wcsicmp(args[0], L"-u"))) {
          if (regKey.Open(HKEY_CURRENT_USER, launcher.c_str()) ==
                  ERROR_SUCCESS &&
              regKey.Open(HKEY_CURRENT_USER,
                          "Software\\Classes\\Applications") == ERROR_SUCCESS &&
              regKey != NULL)
  ...
  *path = 0;
  nChars = PATH_SIZE;
  if (regKey.Open(HKEY_CURRENT_USER, "SOFTWARE\\DOSBox-X") != ERROR_SUCCESS ||
      regKey.QueryStringValue("Location", path, &nChars) != ERROR_SUCCESS ||
      !PathFileExists(path)) {
    dosbox = std::string(GetDosBoxXPath());
    if (dosbox.size())
      SetupKeyDefaultValue("Software\\DOSBox-X", dosbox, "Location");
  } else
    dosbox = std::string(path);
  SetupKeyDefaultValue(launcher + "\\DefaultIcon", "\"%1\"");
  GetModuleFileName(NULL, path, sizeof(path));
  SetupKeyDefaultValue(launcher + "\\shell\\open\\command",
                       "\"" + std::string(path) +
                           "\" \"%1\" %2 %3 %4 %5 %6 %7 %8 %9");
  ...
  std::string fileType = "";
  if (fileName.size() > 4 &&
      !_stricmp(fileName.substr(fileName.size() - 4).c_str(), ".exe"))
    fileType = "exe";
  else if (fileName.size() > 4 &&
           !_stricmp(fileName.substr(fileName.size() - 4).c_str(), ".com"))
    fileType = "com";
  if (fileType == "")
    return 0;
  std::string fileArgs = "";
  ...
  if (IsMsDosExecutable(fileName.c_str())) {
    ...
    if (p != NULL)
      ...
      *p = 0;
    fileArgs = "-fastlaunch -defaultdir \"" + (*p ? std::string(p) : ".") +
               " \" \"" + fileName + "\"";
    fileName = dosbox;
  }

  ShellExecute(NULL, _T("open"), fileName.c_str(), fileArgs.c_str(), NULL,
               SW_SHOW);
